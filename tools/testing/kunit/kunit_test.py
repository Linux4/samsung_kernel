...
class TestExtKunitconfigGenerator(unittest.TestCase):
    def setUp(self):
        self.test_path = './test_data/bsp_kernel'
    ...
    def test_get_subconfig_childs(self):
        ...
        tobe = [
                ...
                os.path.join(self.test_path, 'kunitconfigs/kunitconfig.xxx.zzz')
                ]
    ...
    def test_get_merge_config_multi(self):
        ek = kunit_kernel.ExtKunitconfigGenerator(['kunit.test1', 'aaa'], path=self.test_path)
        with open(ek.final_config_file, 'r') as fp:
            result = fp.read()
    ...
    def test_get_merge_config_triple(self):
        ek = kunit_kernel.ExtKunitconfigGenerator(['kunit.test1', 'aaa', 'xxx.yyy'], path=self.test_path)
        with open(ek.final_config_file, 'r') as fp:
            result = fp.read()
        tobe = str('CONFIG_KUNIT=y\nCONFIG_KUNIT_TEST1=y\nCONFIG_AAA=y\nCONFIG_XXX_YYY=y\n')
        self.assertEqual(result, tobe)
