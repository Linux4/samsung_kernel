...
#define CP_START_ADDR  CPT_START_ADDR
#else
#define CP_START_ADDR  CPW_START_ADDR 
#endif
...
static int sprd_cproc_probe(struct platform_device *pdev)
{
	...
	/* make the cp area ro */
	//create_mapping(struct map_desc * md, bool force_pages)
	//set_memory_ro(__va(pdata->base), pdata->maxsz >> PAGE_SHIFT);
	extern void set_section_ro(unsigned long virt, unsigned long numsections);
	if ( cproc->initdata->base == CP_START_ADDR) {
		printk("%s %p %x\n", __func__, cproc->initdata->base, cproc->initdata->maxsz);
		set_section_ro(__va(cproc->initdata->base), cproc->initdata->maxsz >> SECTION_SHIFT);
	}
