...
static int prox_led_onoff(struct cm36672p_data *data, bool onoff)
{
	...
	regulator_set_voltage(leda, 3300000, 3300000);
	...
	struct cm36672p_platform_data *pdata)
{
	...
	pr_info("%s, initial CONF1 = 0x%X, CONF3 = 0x%X\n", __func__,
		ps_reg_init_setting[PS_CONF1][CMD],
...
static int cm36672p_i2c_shutdown(struct i2c_client *client)
{
	proximity_regulator_onoff(&client->dev, OFF);
	return 0;
}
