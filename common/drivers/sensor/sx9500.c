...
static ssize_t sx9500_register_write_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	u8 val = 0;

	pr_info("[SX9500]: %s do nothing", __func__);
...
static DEVICE_ATTR(register_write, S_IRUGO | S_IWUSR | S_IWGRP,
		sx9500_register_write_show, sx9500_register_write_store);
static DEVICE_ATTR(register_read, S_IRUGO | S_IWUSR | S_IWGRP,
		sx9500_register_read_show, sx9500_register_read_store);
...
int sx9500_regulator_on(struct sx9500_p *data, bool onoff)
{
	int ret = -1;
	if (!data->vdd) {
		data->vdd = devm_regulator_get(&data->client->dev, "sx9500-i2c,vdd");
		if (!data->vdd) {
		...
		const struct i2c_device_id *id)
{
	...
	ret = request_threaded_irq(data->irq, NULL, sx9500_interrupt_thread,
			IRQF_TRIGGER_FALLING | IRQF_ONESHOT , "sx9500_irq", data);
	if (ret < 0) {
