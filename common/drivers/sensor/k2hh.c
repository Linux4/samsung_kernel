...
static int k2hh_i2c_read(struct k2hh_p *data, unsigned char reg_addr,
...
static int k2hh_i2c_write(struct k2hh_p *data, unsigned char reg_addr,
	unsigned char buf)
{
	int ret, retries = 0;
	struct i2c_msg msg;
...
static int k2hh_parse_dt(struct k2hh_p *data, struct device *dev)
{
	...
	ret = of_property_read_string(node, "stm,reg_vdd", &data->str_vdd);
	if (ret)
		pr_err("[SENSOR] %s - invalid vdd\n", __func__);
	else
		pr_info("[SENSOR] %s - vdd:%s\n", __func__, data->str_vdd);

	ret = of_property_read_string(node, "stm,reg_vio", &data->str_vio);
	if (ret)
		pr_err("[SENSOR] %s - invalid vio\n", __func__);
	else
		pr_info("[SENSOR] %s - vio:%s\n", __func__, data->str_vio);
...
static int k2hh_regulator_onoff(struct k2hh_p *data, bool onoff)
{
	...
	if (onoff) {
		...
		if (ret)
			pr_err("[SENSOR] %s - Failed to enable vio.\n", __func__);
		msleep(30);
	} else {
		...
		if (ret)
			pr_err("[SENSOR] %s - Failed to disable vio.\n", __func__);
	}
