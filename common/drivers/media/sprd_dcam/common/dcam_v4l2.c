...
{
	unsigned long flags;
	printk( "%d :camdrv_ss_s5k4ecgx_AAT_flash_write_data  E\n", count);
	...
	local_irq_save(flags);
		...
		if(count)
		{
			do 
			{
				gpio_set_value(GPIO_CAM_FLASH_TORCH, 0); /*low till T_off */
				udelay(AAT_PULS_LO_TIME);

				gpio_set_value(GPIO_CAM_FLASH_TORCH, 1); /* go high */
				udelay(AAT_PULS_LO_TIME);
			} while (--count);
...
static void camdrv_ss_AAT_flash_write_address(unsigned int type)
{
	unsigned long flags;
	printk( "%d :camdrv_ss_s5k4ecgx_AAT_flash_write_data  E\n", type);
	...
	local_irq_save(flags);
		...
		if(type < FALSH_COMMAND_MAX && type >= FALSH_MOVIE_MODE)
		{
			do 
			{
				gpio_set_value(GPIO_CAM_FLASH_TORCH, 1); /*low till T_off */
				udelay(AAT_PULS_LO_TIME);

				gpio_set_value(GPIO_CAM_FLASH_TORCH, 0); /* go high */
				udelay(AAT_PULS_LO_TIME);
			} while (--type);
			
			gpio_set_value(GPIO_CAM_FLASH_TORCH, 1);
			udelay(AAT_LATCH_TIME);
		}
...
#define T_EOD_L		10
#define T_H_LB		10
#define T_L_LB		4*T_H_LB
#define T_L_HB		10
#define T_H_HB		4*T_L_HB
#define T_RESET		1000	//	700
#define T_RESET2	100
...
LOCAL void KTD2692_set_flash(unsigned int ctl_cmd)
{
	int i=0;
	unsigned long flags;
...
LOCAL int sprd_v4l2_setflash(uint32_t flash_mode)
{
	...
	case FLASH_TORCH:        /*for torch */
		/*low light */
#if defined(CONFIG_MACH_VIVALTO) 
	printk("ywf torch light\n");
	gpio_direction_output(GPIO_CAM_FLASH_TORCH, SPRD_FLASH_ON);
	gpio_direction_output(GPIO_CAM_FLASH_EN, SPRD_FLASH_OFF);
#elif defined(CONFIG_MACH_HIGGS)
	printk("higgs torch light: GPIO_CAM_FLASH_EN: %d\n", GPIO_CAM_FLASH_EN);
	...
	case FLASH_CLOSE:
		/*close the light */
#if defined(CONFIG_MACH_VIVALTO)
		printk("ywf close light\n");
		gpio_direction_output(GPIO_CAM_FLASH_EN, SPRD_FLASH_OFF);
		gpio_direction_output(GPIO_CAM_FLASH_TORCH, SPRD_FLASH_OFF);
#elif defined CONFIG_MACH_HIGGS	
		printk("higgs close light\n");
...
LOCAL int sprd_v4l2_tx_done(struct dcam_frame *frame, void* param)
{
	...
	/*if (0 == dev->dcam_cxt.capture_mode) {
		dev->dcam_cxt.flash_status = FLASH_CLOSE_AFTER_OPEN;
		DCAM_TRACE("V4L2: dev->dcam_cxt.capture_mode =%d\n",dev->dcam_cxt.capture_mode);
			...
			struct v4l2_streamparm *streamparm)
{
	...
	case CAPTURE_SET_FLASH:
		...
		mutex_unlock(&dev->dcam_mutex);
		DCAM_TRACE("ywf V4L2: s_parm, status %d \n", dev->dcam_cxt.flash_status);
		break;
