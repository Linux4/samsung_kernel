	...
	case DCAM_CAP_SYNC_POL:
	{
		...
		if (DCAM_CAP_IF_CCIR == cap_desc->interface) {
			...
			if (sync_pol->vsync_pol > 1 ||
				...
				printk("pclk_src = 0x%x\n", sync_pol->pclk_src);
				if (sync_pol->pclk_src == 0x00) {
					sci_glb_clr(SPRD_PIN_BASE + 0x04, BIT_25);
					sci_glb_clr(SPRD_PIN_BASE + 0x04, BIT_26);
					printk("DCAM: main camera set pclk\n");
				} else if (sync_pol->pclk_src == 0x01) {
					sci_glb_clr(SPRD_PIN_BASE + 0x04, (BIT_25 | BIT_26));
					sci_glb_set(SPRD_PIN_BASE + 0x04, (BIT_25));
					printk("DCAM: sub camera set pclk\n");
				} else {
					printk("DCAM: set pclk src error\n");
				}
			}
		} else {
...
LOCAL void _dcam_path2_set(void)
{
	...
	if(path->valid_param.output_size){
		...
		if (path->output_size.w >(DCAM_PATH2_LINE_BUF_LENGTH/2) &&
			s_p_dcam_mod->dcam_path1.status == DCAM_ST_STOP) {
			DCAM_TRACE("DCAM: path2  line buffer merge \n");
