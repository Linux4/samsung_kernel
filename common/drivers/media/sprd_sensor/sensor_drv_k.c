...
LOCAL int _Sensor_K_WriteReg(SENSOR_REG_BITS_T_PTR pReg)
{
	...
	if(s_p_sensor_mod->cur_i2c_client->addr == 0x0028)//For only SR541, need check each sensor
			delay_value = 0xdddd;
#if defined(CONFIG_MACH_GOYAVE3G) || defined(CONFIG_MACH_GOYAVEWIFI) || defined(CONFIG_MACH_J1POP3G)
...
LOCAL int _sensor_k_wr_regtab(SENSOR_REG_TAB_PTR pRegTab)
{
	...
	{
		//SENSOR_PRINT("_sensor_k_wr_regtab : CAM %s, Line %d, burst_mode=%d, cnt=%d, start \n", __FUNCTION__, __LINE__, pRegTab->burst_mode, cnt);
		ret = _sensor_burst_write_init(sensor_reg_ptr, pRegTab->reg_count);
		//SENSOR_PRINT("_sensor_k_wr_regtab : CAM %s, Line %d, burst_mode=%d, cnt=%d end\n", __FUNCTION__, __LINE__, pRegTab->burst_mode, cnt);
	}
...
//	SENSOR_PRINT("SENSOR: _sensor_burst_write_init\n");
	...
	for( i = 0 ; i < init_table_size ; i++ )
	{
		...
		else
		{
			...
			ret = _burst_write(p_reg_table[i].reg_addr, p_reg_table[i].reg_value);
			if (ret < 0)
			{
...
//	SENSOR_PRINT("_sensor_burst_write_init : E\n");
	if (0 == i2c_client)
	{
...
static DEVICE_ATTR(rear_camtype, S_IRUGO | S_IXOTH, Cam_Sensor_TYPE, NULL); //Read(User, Group, Other), Execute(Other)
...
static DEVICE_ATTR(front_camtype, S_IRUGO | S_IXOTH, Cam_Sensor_TYPE, NULL); // Read(User, Group, Other), Execute(Other)
