...
static void STC311x_SetParam(struct i2c_client *client)
{
 ...
 // must be blocked..   if (BattData.SOCValue[1]!=0) STC31xx_Write(OCVTAB_SIZE, STC311x_REG_OCVTAB+OCVTAB_SIZE*2, (unsigned char *) BattData.SOCValue);
  ...
  if (GG_Ram.reg.CC_cnf !=0 ) STC31xx_WriteWord(client, STC311x_REG_CC_CNF,GG_Ram.reg.CC_cnf);
  if (GG_Ram.reg.VM_cnf !=0 ) STC31xx_WriteWord(client, STC311x_REG_VM_CNF,GG_Ram.reg.VM_cnf);
...
static int STC311x_ReadBatteryData(struct i2c_client *client, STC311x_BattDataTypeDef *BattData)
{
  ...
  if (value>=0x0800) value = 0x800;  /* convert to signed value */
...
static void STC31xx_Work(struct i2c_client *client)
{
	...
	if (res > 0)
	{
		fuelgauge->info.batt_soc = (GasGaugeData.SOC + 5) / 10;
...
bool sec_hal_fg_init(struct i2c_client *client)
{
	...
	__raw_writel((BITS_PIN_DS(1)|BITS_PIN_AF(1)|BIT_PIN_NUL|BIT_PIN_SLP_NUL|BIT_PIN_SLP_Z),  SCI_ADDR(SPRD_PIN_BASE, 0x0210));
	__raw_writel((BITS_PIN_DS(1)|BITS_PIN_AF(1)|BIT_PIN_NUL|BIT_PIN_SLP_NUL|BIT_PIN_SLP_Z),  SCI_ADDR(SPRD_PIN_BASE, 0x0218));
//	__raw_writel((BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPU|BIT_PIN_SLP_WPU|BIT_PIN_SLP_Z),  SCI_ADDR(SPRD_PIN_BASE, 0x0260));
#else
	__raw_writel((BITS_PIN_DS(1)|BITS_PIN_AF(1)|BIT_PIN_NUL|BIT_PIN_SLP_NUL|BIT_PIN_SLP_Z),  SCI_ADDR(SPRD_PIN_BASE, 0x0114));
	__raw_writel((BITS_PIN_DS(1)|BITS_PIN_AF(1)|BIT_PIN_NUL|BIT_PIN_SLP_NUL|BIT_PIN_SLP_Z),  SCI_ADDR(SPRD_PIN_BASE, 0x0118));
#endif
	...
	GasGaugeData.ExternalTemperature = get_battery_data(fuelgauge).ExternalTemperature();
	GasGaugeData.ForceExternalTemperature = get_battery_data(fuelgauge).ForceExternalTemperature;
	GasGaugeData.AvgSOC = 0;
	GasGaugeData.AvgCurrent = 0;
	GasGaugeData.AvgVoltage = 0;
	GasGaugeData.AvgTemperature = 0;
