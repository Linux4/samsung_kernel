...
#ifdef CONFIG_ANDROID_LOW_MEMORY_KILLER_DEBUG_TRACE
#define CREATE_TRACE_POINTS
#include <trace/events/lmk.h>
static int lmk_seq_id = 0;
#endif

#ifdef CONFIG_ANDROID_LOW_MEMORY_KILLER_ENHANCED_KILL
...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
...
#ifdef CONFIG_ANDROID_LOW_MEMORY_KILLER_DEBUG_TRACE
                        trace_lmk_kill_s(selected[i], selected_oom_score_adj[i]);
#endif
	...
	if (selected && (selected_oom_score_adj || is_need_lmk_kill)) {
			struct sysinfo si;
#ifdef CONFIG_ANDROID_LOW_MEMORY_KILLER_DEBUG_TRACE
                        trace_lmk_kill_s(selected, selected_oom_score_adj);
#endif
			si_swapinfo(&si);
			lowmem_print(1, "Killing '%s' (%d), adj %hd,\n" \
				"   to free %ldkB on behalf of '%s' (%d) because\n" \
