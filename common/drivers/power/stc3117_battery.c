...
#ifdef	CONFIG_SPRD_6820i_POWER
#include "sprd_6820i_charge.h"
#endif
#define STC3117_DEBUG
//#define USE_LOW_BAT_DET
#define GPIO_LOW_BAT_DET 140
#ifdef USE_LOW_BAT_DET
//0: normal, 1:low, 2:very low
u8 stc311x_is_low_batt = 0;
EXPORT_SYMBOL(stc311x_is_low_batt);
#endif
#if 1 //temporary
#include <linux/io.h>
...
/* -------------------------------------------------------------------------------- */
/*                                                                                  */
#define BATT_CHG_VOLTAGE   4150   /* min voltage at the end of the charge (mV)      */
...
static void stc311x_work_func(struct stc311x_chip *chip);
 ...
 */
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		stc311x_work_func(chip);
		val->intval = chip->batt_voltage;  /* in uV */
		break;
...
static int STC31xx_Write(int length, int reg , unsigned char *values)
{
	...
	if (ret < 0)
 #ifdef STC3117_DEBUG
		printk("STC3117:%s  R[%d] i2c_err=%d len[%d]================\n",  __func__, reg, ret, length);
 #else
...
static void STC311x_SetParam(void)
{
  ...
  /* init OCV curve */
    for (ii=0;ii<OCVTAB_SIZE;ii++)
    {
      ...
      {
         STC31xx_WriteWord(STC311x_REG_OCVTAB+ii*2, BattData.OCVValue[ii]*100/55);
#ifdef STC3117_DEBUG
         printk("STC3117 Write BattData.OCVValue[%d]=0x%x\n",ii , BattData.OCVValue[ii]*100/55);
#endif
      }
    }
...
#ifdef STC3117_TEMPORARY_TEST  // 001
  ocv = ocv + 20; //
  printk("STC3117: +++++++20++++++\n");
#endif
...
#ifdef STC3117_DEBUG // sayuss : new debug
  printk("STC3117:%s res[0x%x] ocv[%d] curr[%d]\n", __func__, res, ocv, curr);
#endif

  if (curr>=0x2000) curr -= 0x4000;  /* convert to signed value */

#ifdef STC3117_TEMPORARY_TEST // 002
  res = conv(curr, BattData.CurrentFactor);  /* result in mA */
  if(35 < curr || -35 > curr)
  {
  	ocv = ocv - BattData.Rint * curr * 588 / BattData.Rsense / 55000 ;
    printk("STC3117: curr=%d CF=%d ocv=%d\n",res , BattData.CurrentFactor ,ocv );
  }
  else
  {
    printk("STC3117: curr=%d CF=%d \n",res , BattData.CurrentFactor );
  }
#else
  ocv = ocv - BattData.Rint * curr * 588 / BattData.Rsense / 55000 ;
#endif
...
void SOC_correction (GasGauge_DataTypeDef *GG)
{
  ...
  if ( (Var4<(-VAR4MAX)) || (Var4>=VAR4MAX) )
  {
    ...
    STC31xx_WriteWord(STC311x_REG_SOC, SOCopt);
...
#ifdef STC3117_DEBUG
	printk("STC3117: %s  OCVValue[%d]=%d SOCValue=%d ================\n",  __func__, i, BattData.OCVValue[i], BattData.SOCValue[i]);
#endif
  }
  for (i=0;i<NTEMP;i++)
...
#ifdef USE_LOW_BAT_DET
static void stc311x_irq_work(struct work_struct *work)
{
	int state = STC31xx_AlarmGet();
	pr_info("%s %d\n", __func__, state);

	if(state == 1)
		stc311x_is_low_batt = 1;
	else if (state >= 2)
		stc311x_is_low_batt = 2;

	STC31xx_AlarmClear();
}

static irqreturn_t stc311x_irq_handler(int irq, void *dev)
{
	struct stc311x_chip *chip = i2c_get_clientdata(sav_client);
	pr_info("%s\n", __func__);
	wake_lock_timeout(&chip->irq_wakelock, 2*HZ);
	schedule_delayed_work(&chip->irq_work, STC311x_IRQ_WORK_DELAY);
	return IRQ_HANDLED;
}
#endif

#if 1 //to check if works
...
struct stc311x_chip *chip;
	...
	if(chip->pdata->charger_online)
	{
		if(!chip->pdata->charger_online())
		{
			...
			if(chip->batt_voltage < BATT_MIN_VOLTAGE)
			{
				if(check_count > 3)
				{
					chip->batt_soc = 0;
...
#ifdef USE_LOW_BAT_DET
	if(*soc > 15)
		stc311x_is_low_batt = 0;
	else if(chip->batt_voltage > 3400)
		stc311x_is_low_batt = 1;
#endif

	if(*soc == 0 && chip->batt_voltage > 3450)
		*soc = 1;
...
#endif
#if 0
int read_VF(unsigned int *VF)
{
	struct stc311x_chip *chip = i2c_get_clientdata(sav_client);
	static u8 vf_recheck = 0;
	...
	else
	{
		...
		*VF = STC311x_VF_BATTERY_OFF;
		...
		if(chip->pdata->charger_online)
		{
			if(!vf_recheck && !chip->pdata->charger_online())
			{
				printk("read_VF - F/G reset due to Battery OFF  \n");
				STC31xx_WriteByte(STC311x_REG_CTRL, STC311x_SOFTPOR);  /*   set soft POR */
				vf_recheck = 1;
				msleep(1000);
			}
			...
			const struct i2c_device_id *id)
{
  ...
  i2c_set_clientdata(client, chip);
	...
	INIT_DELAYED_WORK(&chip->irq_work, stc311x_irq_work);

	STC31xx_AlarmSet();

	ret = gpio_request(GPIO_LOW_BAT_DET, "stc3115 low bat det");
	if(ret)
		pr_err("%s %d gpio request failed\n", __func__, GPIO_LOW_BAT_DET);
	gpio_direction_input(GPIO_LOW_BAT_DET);

	ret = request_threaded_irq(gpio_to_irq(GPIO_LOW_BAT_DET), stc311x_irq_handler, NULL,
		(IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND), "stc3115 low bat det irq", NULL);
	if(ret)
		pr_err("%s %d request_irq failed\n", __func__, gpio_to_irq(GPIO_LOW_BAT_DET));
...
#ifdef STC3117_DEBUG
	printk("STC3117: probe %s  res_GG[%d]================\n",  __func__, res);
#endif
	...
	INIT_DELAYED_WORK(&chip->work, stc311x_work);
...
static int  stc311x_remove(struct i2c_client *client)
{
	...
	wake_lock_destroy(&chip->irq_wakelock);
	#endif
...
static int __init stc311x_init(void)
{
	...
	__raw_writel((BITS_PIN_DS(1)|BITS_PIN_AF(1)|BIT_PIN_WPU|BIT_PIN_SLP_WPU|BIT_PIN_SLP_Z),  SCI_ADDR(SPRD_PIN_BASE, 0x0210));
	__raw_writel((BITS_PIN_DS(1)|BITS_PIN_AF(1)|BIT_PIN_WPU|BIT_PIN_SLP_WPU|BIT_PIN_SLP_Z),  SCI_ADDR(SPRD_PIN_BASE, 0x0218));
//	__raw_writel((BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPU|BIT_PIN_SLP_WPU|BIT_PIN_SLP_Z),  SCI_ADDR(SPRD_PIN_BASE, 0x0260));
#else
	__raw_writel((BITS_PIN_DS(1)|BITS_PIN_AF(1)|BIT_PIN_WPU|BIT_PIN_SLP_WPU|BIT_PIN_SLP_Z),  SCI_ADDR(SPRD_PIN_BASE, 0x0114));
	__raw_writel((BITS_PIN_DS(1)|BITS_PIN_AF(1)|BIT_PIN_WPU|BIT_PIN_SLP_WPU|BIT_PIN_SLP_Z),  SCI_ADDR(SPRD_PIN_BASE, 0x0118));
#endif
