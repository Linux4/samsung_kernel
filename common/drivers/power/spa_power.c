...
#define SPA_DBG_LEVEL_INTERNAL SPA_DBG_LEVEL1
...
static struct device_attribute ss_batt_ext_attrs[]=
{
	...
	__ATTR(update, 0644, spa_power_attrs_show, spa_power_attrs_store),
};
...
static int spa_get_batt_vf_status(struct spa_power_desc *spa_power_iter)
{
	...
	/* temporary skip vf check for corsica ve ds */
#if defined(CONFIG_MACH_CORSICA_VE)
	if (get_hw_rev() < 0x2)
		value.intval = 1;
#endif
