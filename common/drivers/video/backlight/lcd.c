...
static ssize_t show_lcd_info(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    else
    {
        ...
        {
            case 0x08 : //CPT
            	return sprintf(buf, "CPT_%X%X%X",(panel_id >> 16) & 0xFF,(panel_id >> 8) & 0xFF, panel_id  & 0xFF);
                break;
            case 0x09 : //AUO
            	return sprintf(buf, "AUO_%X%X%X",(panel_id >> 16) & 0xFF,(panel_id >> 8) & 0xFF, panel_id  & 0xFF);
                break;
            case 0x14 : //TIANMA
            	return sprintf(buf, "TIA_%X%X%X",(panel_id >> 16) & 0xFF,(panel_id >> 8) & 0xFF, panel_id  & 0xFF);
                break;
            case 0x17 : //BOE
            	return sprintf(buf, "BOE_%X%X%X",(panel_id >> 16) & 0xFF,(panel_id >> 8) & 0xFF, panel_id  & 0xFF);
                break;
            case 0x18 : //DTC
            	return sprintf(buf, "DTC_%X%X%X",(panel_id >> 16) & 0xFF,(panel_id >> 8) & 0xFF, panel_id  & 0xFF);
                break;
            default :
            	return sprintf(buf, "Unknow. need to check %X", panel_id);
                break;
        }
    }
}
