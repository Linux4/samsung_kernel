...
int offset_cal(int offset,  int value)
{
	int real_value;
...
/* -1 means V1 */
void(*Make_hexa[IV_MAX])(int*, struct SMART_DIM*, int*) = {
	v255_hexa,
	v203_hexa,
	v151_hexa,
	v87_hexa,
...
static void gamma_init(struct SMART_DIM *pSmart, char *result_str, int size)
{
	int str[GAMMA_SET_MAX];
	long long candela_level[IV_MAX] = {-1, };
	int bl_index[IV_MAX] = {-1, };
...
#ifdef SMART_DIMMING_DEBUG
	printk(KERN_INFO "\n candela_0:%llu  candela_3:%llu  candela_11:%llu\n",
	...
	for (cnt = 0; cnt < IV_MAX; cnt++) {
		if (searching_function(candela_level[cnt],
					&(bl_index[cnt]), GAMMA_CURVE_2P2)) { // 2.2 gamma.  Excel sheet: 'Base Luminance'!$C$28
	...
	for (cnt = 1; cnt < IV_MAX; cnt++) {
		...
		if (bl_index[IV_MAX - cnt] == 0)
			bl_index[IV_MAX - cnt] = 1;
	}

#ifdef SMART_DIMMING_DEBUG
	printk(KERN_INFO "\n bl_index_0:%d  bl_index_3:%d  bl_index_11:%d\n",
	...
	for (cnt = 0; cnt < RGB_COMPENSATION; cnt++) {
		...
		if (cnt < 3) {
			...
			str[cnt+3] += (pSmart->rgb_offset)[table_index * 24 + cnt];
		}
	}
	/*subtration MTP_OFFSET value from generated gamma table*/
