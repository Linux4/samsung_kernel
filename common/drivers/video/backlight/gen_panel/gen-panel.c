...
static int gen_panel_set_elvss(struct lcd *lcd, int candela_index)
{
	...
	if (!desc) {
		pr_warn("[LCD] tset cmd not found\n");
		return -EINVAL;
	}
	t_set_cmd = &desc->data[data_offset];

	if (lcd->temperature > 0)
		t_set = (lcd->temperature) & 0x7F;
	else
		t_set = ((lcd->temperature*(-1)) & 0x7F) | 0x80;

	memcpy(t_set_cmd + 5, &t_set, 1);
	gen_panel_write_op_cmds(lcd, PANEL_TSET_CMD);

	if (!elvss_map_table) {
		pr_warn("%s, elvss table not exist\n", __func__);
		return -EINVAL;
	}
	desc_index = elvss_map_table[candela_index];

	/* Write elvss dim offset  -2nd para*/
	desc = find_op_cmd_desc(lcd, PANEL_ELVSS_CMD, 0xB6);
	if (!desc) {
		pr_warn("gamma cmd not found\n");
		return -EINVAL;
	}
	elvss_data = &desc->data[data_offset];

	cmd = find_op_cmd(lcd, PANEL_ELVSS_DIM_OFFSET_CMD);

	memcpy(elvss_data, &cmd->desc[desc_index].data[data_offset], 2);

	if (op_cmd_valid(lcd, PANEL_ELVSS_TEMPERATURE_CMD)) {
		/* Write elvss dim offset  -17th para*/
		if (desc_index <= 17) { /* >= 30cd*/
			memcpy(elvss_data + 16, &lcd->temperature_elvss_value, 1);
		} else {
			cmd = &lcd->op_cmds[PANEL_ELVSS_TEMPERATURE_CMD];

			if(lcd->temperature <= -20)
				desc_index += (cmd->nr_desc / 3) * 2;
			else if(lcd->temperature <= 0)
				desc_index += (cmd->nr_desc / 3) * 1;

			if (desc_index >= cmd->nr_desc || desc_index < 0)
				return -EINVAL;

			memcpy(elvss_data + 16, &cmd->desc[desc_index].data[data_offset], 1);
		}
	}
...
{
	if (!aid_map_table) {
		pr_warn("%s, aid table not exist\n", __func__);
		return -EINVAL;
	}
	gen_panel_write_op_cmd_desc(lcd, PANEL_AID_CMD,
			aid_map_table[candela_index]);
...
static int gen_panel_set_brightness(struct lcd *lcd, int intensity)
{
	...
	if (IS_HBM(lcd->mdnie.config.auto_brightness)) {
		lcd->hbm = 1;
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (rc < 0)
		...
		gen_panel_set_brightness(lcd, bl_info->prev_tune_level);
