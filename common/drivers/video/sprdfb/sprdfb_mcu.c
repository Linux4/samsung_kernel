...
static int32_t lcdc_mcu_send_cmd(uint32_t cmd)
{
	...
	while (lcdc_read(LCM_CTRL) & BIT(20)) {
		if (0x0 == ++i%10000)
			pr_info("sprdfb:[%s] warning: busy waiting!\n",
								__func__);
	}
...
static int32_t lcdc_mcu_send_cmd_data(uint32_t cmd, uint32_t data)
{
	...
	while (lcdc_read(LCM_CTRL) & BIT(20)) {
		if (0x0 == ++i%10000)
			pr_info("sprdfb: [%s] warning: busy waiting 1!\n",
								__func__);
	}
	...
	while (lcdc_read(LCM_CTRL) & BIT(20)) {
		if (0x0 == ++i%10000)
			pr_info("sprdfb: [%s] warning: busy waiting 2!\n",
								__func__);
	}
...
static uint32_t mcu_calc_timing(struct timing_mcu *timing, uint16_t dev_id)
{
	...
	if (SPRDFB_MAINLCD_ID == dev_id) {
		...
		if (IS_ERR(clk))
			pr_err("sprdfb:get clk_dispc_dbi fail!\n");
		else
		...
		if (IS_ERR(clk))
			pr_err("sprdfb: get clk_ahb fail!\n");
		else
...
#define DBI_CYCLES(ns) (((ns) * clk_rate + 1000 - 1) / 1000)
	...
	return (whpw | (wlpw << 6) | (wcss << 12) | (rhpw << 16) |
						(rlpw << 22) | (rcss << 28));
}
...
void mcu_dispc_init_config(struct panel_spec *panel)
{
	...
	if (NULL == panel) {
		pr_err("sprdfb: [%s] fail.(Invalid Param)\n", __func__);
		return;
	}

	if (SPRDFB_PANEL_TYPE_MCU != panel->type) {
		pr_err("sprdfb: [%s] fail.(not  mcu panel)\n", __func__);
		return;
	}
...
void mcu_dispc_set_timing(struct sprdfb_device *dev, uint32_t type)
	...
	case MCU_LCD_REGISTER_TIMING:
		dispc_write(dev->panel_timing.mcu_timing[MCU_LCD_REGISTER_TIMING],
							DISPC_DBI_TIMING1);
		break;
...
void mcu_lcdc_set_timing(struct sprdfb_device *dev, uint32_t type)
{
	...
	switch (type) {
	case MCU_LCD_REGISTER_TIMING:
		lcdc_write(dev->panel_timing.mcu_timing[MCU_LCD_REGISTER_TIMING],
								LCM_TIMING1);
		break;

	case MCU_LCD_GRAM_TIMING:
		lcdc_write(dev->panel_timing.mcu_timing[MCU_LCD_GRAM_TIMING],
								LCM_TIMING1);
		break;
	default:
		break;
...
void mcu_dispc_init_config(struct panel_spec *panel)
{
	...
	pr_debug("sprdfb: [%s] DISPC_DBI_CTRL = %d\n", __func__, dispc_read(DISPC_DBI_CTRL));
}
...
static int32_t sprdfb_mcu_panel_check(struct panel_spec *panel)
{
	...
	if (SPRDFB_PANEL_TYPE_MCU != panel->type) {
		pr_err("sprdfb: [%s] fail. (not mcu param)\n", __func__);
		return false;
	}
...
static void sprdfb_mcu_panel_mount(struct sprdfb_device *dev)
{
	...
	dev->panel_timing.mcu_timing[MCU_LCD_REGISTER_TIMING] =
					mcu_calc_timing(timing, dev->dev_id);
	timing++;
...
{
	pr_info("sprdfb_mcu_panel_before_refresh\n");
...
{
	pr_info("sprdfb_mcu_panel_after_refresh\n");
	if (SPRDFB_MAINLCD_ID == dev->dev_id)
		mcu_dispc_set_timing(dev, MCU_LCD_REGISTER_TIMING);
	else {
	#ifdef CONFIG_FB_SC8825
