...
#if defined (CONFIG_KPLED_FAST_DISCHARGING)
	if ((0 == strcmp(desc->desc.name, "vddkpled") && regs->pd_set))
		ANA_REG_BIC(regs->pd_set, KPLED_PULLDOWN_EN);
#endif
...
#if defined (CONFIG_KPLED_FAST_DISCHARGING)
	if ((0 == strcmp(desc->desc.name, "vddkpled") && regs->pd_set))
		ANA_REG_OR(regs->pd_set, KPLED_PULLDOWN_EN);
#endif
