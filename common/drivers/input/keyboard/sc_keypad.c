...
extern int get_tsp_debug_info(void);
#endif
...
#ifdef CONFIG_MACH_HIGGS
void check_key_for_tsp_debugging (int key, int status){
	static int vol_up_status, vol_down_status, homekey_status, powerkey_status;
	static int two_key_pushed;

	switch (key){

	case 115:
		printk("vol_up_status : %d \n",status); 
		if (status)
			vol_up_status = 1;
		else
			vol_up_status = 0;
	...
	if (two_key_pushed && vol_down_status && homekey_status){
		get_tsp_debug_info();
	}

	if (vol_up_status && homekey_status){
		two_key_pushed = 1;
		printk("two_key_pushed !!! \n");
	}
}
#endif
...
#endif
#ifdef CONFIG_MACH_HIGGS
		check_key_for_tsp_debugging (key, 1);
#endif
	}
...
#endif
#ifdef CONFIG_MACH_HIGGS
		check_key_for_tsp_debugging (key, 0);
#endif
	}
