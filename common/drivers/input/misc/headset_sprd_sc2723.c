...
#ifdef SEC_SYSFS_FOR_FACTORY_TEST
...
#ifdef CONFIG_SEC_NO_HEADSET_IC
...
static void headset_button_work_func(struct work_struct *work)
{
        ...
        ENTER;
	msleep(30); 	// Reliable lab spec : media key shoud be worked when we push the button between 50ms and 100ms 
        ...
        if(1 == button_state_current) {//pressed!
                if(ht->platform_data->nbuttons > 0) {
                        ...
                        if (gpio_get_value(pdata->gpio_detect) == 1)// 1 : plug out  0 : plug in 
			{
				PRINT_INFO("%s detection_gpio is abnormal !! skip button interruption \n",__func__);	
...
#ifdef SEC_SYSFS_FOR_FACTORY_TEST
                        ret_estate = PUSH;
                        if(adc_mic_average >= 40 && adc_mic_average <= 200){
                            hk_adc = (adc_mic_average + 120*5)/6;
                        }else if(adc_mic_average >= 250 && adc_mic_average <= 290){
                            hk_adc = (adc_mic_average + 270*5)/6;
                        }else if(adc_mic_average >= 310 && adc_mic_average <= 350){
                            hk_adc = (adc_mic_average + 330*5)/6;
                        }else{
                            hk_adc = adc_mic_average;
                        }
#endif
                        PRINT_INFO("headset button pressed! current_key_code = %d(0x%04X)\n", current_key_code, current_key_code);
                        //PRINT_INFO("headset button pressed! adc_value = %d\n", adcTuningValue);
                } else {
...
static irqreturn_t headset_detect_irq_handler(int irq, void *dev)
{
        ...
        PRINT_DBG("headset_detect_irq_handler: IRQ_%d(GPIO_%d) = %d, ANA_STS0 = 0x%08X\n",
                  ...
                  sci_adi_read(ANA_AUDCFGA_INT_BASE+ANA_STS0));
        queue_delayed_work(ht->detect_work_queue, &ht->work_detect, msecs_to_jiffies(70));
        return IRQ_HANDLED;
}
...
//================= create sys fs for debug ===================
#ifdef SEC_SYSFS_FOR_FACTORY_TEST
static ssize_t state_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sprd_headset *ht = &headset;
...
#ifdef SEC_SYSFS_ADC_EARJACK
/*************************************
  * add sysfs for factory test. KSND
  * 
 *************************************/
static ssize_t jack_adc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sprd_headset *ht = &headset;
    struct sprd_headset_platform_data *pdata = ht->platform_data;
	...
	val[0] = pdata->adc_threshold_3pole_detect;
	val[1] = pdata->adc_threshold_4pole_detect;
...
//Read Only
static DEVICE_ATTR(jack_adc, S_IRUGO, jack_adc_show, NULL);
...
#endif //SEC_USE_VOICE_ASSIST_KEY
...
static int headset_detect_probe(struct platform_device *pdev)
{
        ...
        device_create_file(devaud, &dev_attr_mic_adc);
        if (ret < 0) {
...
#ifdef SEC_SYSFS_ADC_EARJACK
        device_create_file(devaud, &dev_attr_jack_adc);
        if (ret < 0) {
            pr_err("key_state dev_register failed!\n");
            return ret;
        }
        device_create_file(devaud, &dev_attr_send_end_btn_adc);
        if (ret < 0) {
            pr_err("key_state dev_register failed!\n");
            return ret;
        }
        device_create_file(devaud, &dev_attr_vol_up_btn_adc);
        if (ret < 0) {
            pr_err("key_state dev_register failed!\n");
            return ret;
        }
        device_create_file(devaud, &dev_attr_vol_down_btn_adc);
        if (ret < 0) {
            pr_err("key_state dev_register failed!\n");
            return ret;
        }
#ifdef SEC_USE_VOICE_ASSIST_KEY
        //for Voc CMD key
        device_create_file(devaud, &dev_attr_voc_cmd_btn_adc);
        if (ret < 0) {
            pr_err("Failed to create device file in sysfs entries!\n");
            return ret;
        }
