/*
 * MELFAS MMS400 Touchscreen Driver - Test Functions (Optional)
 *
...
* Process table data
*/
static int mms_proc_table_data(struct mms_ts_info *info, u8 size, u8 data_type_size, u8 data_type_sign, u8 buf_addr_h, u8 buf_addr_l, u8 row_num, u8 col_num, u8 buf_col_num, u8 col_is_x, u8 key_num)
{
	char data[10];
	int ix = 0;
	int iy = 0;
	...
	for(iy = 0; iy < max_y; iy++)
	{
		...
		buf_addr = (buf_addr_h << 8) | buf_addr_l | (offset * iy);
			...
			if(data_type_sign == 0){
				...
				switch(data_type_size){
					case 1:
						uValue = (u8)rbuf[ix];
						printk(" %3u", uValue);
						sprintf(data, " %3u", uValue);
						break;
					case 2:
						uValue = (u16)(rbuf[data_type_size * ix] | (rbuf[data_type_size * ix + 1] << 8));
						printk(" %5u", uValue);
						sprintf(data, " %5u", uValue);
						break;
					case 4:
						uValue = (u32)(rbuf[data_type_size * ix] | (rbuf[data_type_size * ix + 1] << 8) | (rbuf[data_type_size * ix + 2] << 16) | (rbuf[data_type_size * ix + 3] << 24));
						printk(" %6u", uValue);
						sprintf(data, " %6u", uValue);
						break;
					default:
						uValue = (u16)(rbuf[data_type_size * ix] | (rbuf[data_type_size * ix + 1] << 8));
						printk(" %5u", uValue);
				...
				switch(data_type_size){
					case 1:
						sValue = (s8)rbuf[ix];
						printk(" %3d", sValue);
						sprintf(data, " %3d", sValue);
						break;
					case 2:
						sValue = (s16)(rbuf[data_type_size * ix] | (rbuf[data_type_size * ix + 1] << 8));
						printk(" %5d", sValue);
						sprintf(data, " %5d", sValue);
						break;
					case 4:
						sValue = (s32)(rbuf[data_type_size * ix] | (rbuf[data_type_size * ix + 1] << 8) | (rbuf[data_type_size * ix + 2] << 16) | (rbuf[data_type_size * ix + 3] << 24));
						printk(" %6d", sValue);
						sprintf(data, " %6d", sValue);
						break;
					default:
						sValue = (s16)(rbuf[data_type_size * ix] | (rbuf[data_type_size * ix + 1] << 8));
						printk(" %5d", sValue);
...
int mms_run_test(struct mms_ts_info *info, u8 test_type)
{
	...
	dev_dbg(&info->client->dev, "%s - row_num[%d] col_num[%d] buffer_col_num[%d] col_is_x[%d] key_num[%d]\n", __func__, row_num, col_num, buffer_col_num, col_is_x, key_num);	
	...
	//print data
	if(mms_proc_table_data(info, size, data_type_size, data_type_sign, buf_addr_h, buf_addr_l, row_num, col_num, buffer_col_num, col_is_x, key_num)){
...
int mms_get_image(struct mms_ts_info *info, u8 image_type)
{
	...
	switch(image_type){
		case MIP_IMG_TYPE_INTENSITY:
			dev_dbg(&info->client->dev, "=== Filtered Intensity Image ===\n"); 
			sprintf(info->print_buf, "\n=== Intensity Image ===\n\n");
			break;
	...
	//finger num
	wbuf[0] = MIP_R0_IMAGE;
	wbuf[1] = MIP_R1_IMAGE_FINGER_NUM;
	if(mms_i2c_read(info, wbuf, 2, rbuf, 1)){
		dev_err(&info->client->dev, "%s [ERROR] Read finger num\n", __func__); 
		goto ERROR;
	}
	dev_dbg(&info->client->dev, "Finger num : %d\n", rbuf[0]);	
	sprintf(info->print_buf, "Finger num : %d\n", rbuf[0]);	

	//finger area
	size = (rbuf[0] << 2);
	wbuf[0] = MIP_R0_IMAGE;
	wbuf[1] = MIP_R1_IMAGE_FINGER_AREA;
	if(mms_i2c_read(info, wbuf, 2, rbuf, size)){
		dev_err(&info->client->dev, "%s [ERROR] Read finger area\n", __func__); 
		goto ERROR;
	}
	*/
