...
int ist30xx_intr_wait(long ms)                            // int ist30xx_intr_wait Ãß°¡
{
...
#else
int ist30xxb_suspend(struct device *dev)
{
...
int ist30xxb_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct ist30xx_data *data = i2c_get_clientdata(client);
...
void ist30xxb_shutdown(struct i2c_client *client)
{
	struct ist30xx_data *data = i2c_get_clientdata(client);
#if IST30XX_EVENT_MODE
//	unsigned long flags;
...
void ist30xxb_set_ta_mode(int status)
{
	if(ist30xx_initialized){
		...
		if (status == POWER_SUPPLY_TYPE_BATTERY ||
			...
			status = 1;
#if IST30XX_DETECT_TA
		if (status == ist30xx_ta_status)
			return;

		if (ist30xx_ta_status == -1) {
			ist30xx_ta_status = status ? 1 : 0;
			return;
		}
...
void ist30xxb_set_call_mode(int mode)
{
	if(ist30xx_initialized){
		tsp_info("%s(), mode = %d\n", __func__, mode);
...
int ist30xxb_probe(struct i2c_client *		client,
				   const struct i2c_device_id * id)
{
	...
	tsp_charger_status_cb = ist30xxb_set_ta_mode;
...
int ist30xxb_remove(struct i2c_client *client)
{
	struct ist30xx_data *data = i2c_get_clientdata(client);
