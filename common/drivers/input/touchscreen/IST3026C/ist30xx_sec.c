...
void run_raw_read(void *dev_data)
{
	...
	for (i = 0; i < tsp->ch_num.tx; i++) {
		...
		for (j = 0; j < tsp->ch_num.rx; j++) {
			...
			type = ist30xx_check_valid_ch(data, i, j);

			if ((type == TSP_CH_SCREEN) || (type == TSP_CH_GTX)) {
       			max_val = max(max_val, (int)node_value[idx]);
    		    min_val = min(min_val, (int)node_value[idx]);
            }
#if TSP_NODE_DEBUG            
            printk("%5d ", node_value[idx]);
#endif            
		}
#if TSP_NODE_DEBUG        
        printk("\n");
...
void get_cs0_array(void *dev_data)
{
    int i;
    int count = 0;
    char *buf;
	...
	for (i = 0; i < tsp->ch_num.rx * tsp->ch_num.tx; i++) {
		...
		printk("%4d ", ts_cmcs_buf->cs0[i]);
#endif
        count += snprintf(msg, msg_len, "%d,", ts_cmcs_buf->cs0[i]);
    	strncat(buf, msg, msg_len);
	}
...
void get_cs1_array(void *dev_data)
{
    int i;
    int count = 0;
    char *buf;
	...
	for (i = 0; i < tsp->ch_num.rx * tsp->ch_num.tx; i++) {
		...
		printk("%4d ", ts_cmcs_buf->cs1[i]);
#endif
        count += snprintf(msg, msg_len, "%d,", ts_cmcs_buf->cs1[i]);
    	strncat(buf, msg, msg_len);
	}
...
struct tsp_cmd tsp_cmds[] = {
    ...
    { TSP_CMD("get_cs0_array",   get_cs0_array),   },
    { TSP_CMD("get_cs1_array",   get_cs1_array),   },
#endif
#endif
...
int sec_touch_sysfs(struct ist30xx_data *data)
{
	...
	sec_touchscreen = device_create(sec_class, NULL, 0, data,
					"sec_touchscreen");
