...
/* added header file */
#define SUPPORTED_USE_DUAL_FW 1
...
u8 *m_pFirmware [TSP_TYPE_COUNT] = {(u8*)m_firmware_data_01,(u8*)m_firmware_data_00,};
u8 m_FirmwareIdx = 0;
#define CHECK_HWID				1
...
static bool ts_get_raw_data(struct bt432_ts_info *info)
{
	...
	if (read_raw_data(info->client, BT432_RAWDATA_REG, (char *)info->cur_data, sz) < 0) {
...
#endif
	if (read_data(info->client, BT432_POINT_STATUS_REG, (u8 *)(&info->touch_info), 4) < 0) {
...
#if ZINITIX_I2C_CHECKSUM
			if (info->cap_info.i2s_checksum)
				if (i2c_checksum(info, (s16 *)(&info->touch_info.coord[i]),
...
retry_fatal:
	...
	for (i = 0; i < 80; i+=TC_SECTOR_SZ) {
		if (read_firmware_data(info->client, BT432_READ_FLASH,
...
static u8 ts_upgrade_firmware(struct bt432_ts_info *info, const u8 *firmware_data, u32 size)
{
	struct i2c_client *client = info->client;
	u16 flash_addr;
	u16 reg_val;
...
#ifdef SUPPORTED_USE_DUAL_FW
	if (read_data(client, BT432_MODULE_ID, (u8 *)&info->cap_info.module_id, 2) < 0)
...
retry_upgrade:
	...
	if (memcmp((u8 *)&firmware_data[0], (u8 *)&verify_data[0], size) == 0) {
		...
		for (i = 0; i < 5; i++) {
			if (read_data(client, BT432_CHECKSUM_RESULT, (u8 *)&reg_val, 2) < 0) {
				mdelay(10);
				continue;
			}
		}
		...
		if (read_data(client, BT432_MODULE_ID, (u8 *)&reg_val, 2) < 0)
			...
			if(info->cap_info.module_id != reg_val) {
				write_reg(client, BT432_MODULE_ID, info->cap_info.module_id);
...
static bool ts_hw_calibration(struct bt432_ts_info *info)
{
	...
	do {
		...
		if (read_data(client, BT432_EEPROM_INFO_REG, (u8 *)&chip_eeprom_info, 2) < 0)
			return false;
	...
	if (write_reg(client, BT432_TOUCH_MODE, TOUCH_POINT_MODE) != I2C_SUCCESS)
		return false;
...
retry_init:
	...
	if (fw_oneshot_upgrade && (ts_check_need_upgrade(info, cap->fw_version,
		...
		if (ts_upgrade_firmware(info, m_firmware_data, cap->ic_fw_size) == false)
			goto fail_init;
	...
	if (write_reg(client, BT432_INITIAL_TOUCH_MODE, TOUCH_POINT_MODE) != I2C_SUCCESS)
		goto fail_init;
	...
	if (write_reg(client, BT432_INT_ENABLE_FLAG, cap->ic_int_mask) != I2C_SUCCESS)
		goto fail_init;
...
static bool mini_init_touch(struct bt432_ts_info *info)
{
	...
	if (chip_check_sum != 0x55aa) {
		dev_err(&client->dev, "Failed to check firmware checksum(0x%04x)\n", chip_check_sum);
		goto fail_mini_init;
	}
#endif
...
static bool ts_set_touchmode(u16 value)
{
	...
	if (write_reg(misc_info->client, BT432_TOUCH_MODE,
		...
		dev_err(&misc_info->client->dev,
				"TEST Mode : Fail to set ZINITX_TOUCH_MODE %d.\n",
...
static bool ts_set_touchmode2(u16 value)
{
	...
	if (write_reg(misc_info->client, BT432_TOUCH_MODE,
				misc_info->touch_mode) != I2C_SUCCESS)
		dev_err(&misc_info->client->dev,
				"TEST Mode : Fail to set ZINITX_TOUCH_MODE\n");
...
static bool run_reference_DND_read(void *device_data, int button0, int button1) //DND
{
	...
	for (i = 0; i < x_num - 1; i++) {
			for (j = 0; j < y_num; j++) {
				...
				if ((info->pdnd_data[j+i*y_num] < pdnd_min[i][j]) &&
							(info->pdnd_data[j+i*y_num]!=0)) {
...
static int bt432_ts_probe(struct i2c_client *client, const struct i2c_device_id *i2c_id)
{
...
/* FW version read from tsp */
	...
	if (ret) {
		dev_err(&info->client->dev, "unable to register %s input device\r\n",
					info->input_dev->name);
	...
	if (info->irq < 0)
		dev_err(&info->client->dev, "error. gpio_to_irq(..) function is not \
	...
	if (ret) {
		dev_err(&info->client->dev, "unable to register irq.(%s)\r\n",
				info->input_dev->name);
