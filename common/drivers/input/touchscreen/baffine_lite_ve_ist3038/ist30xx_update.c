...
#include "./firmware/grand_neo_ve.h"
#if IST30XX_MULTIPLE_TSP
//#include "./firmware/test_fw.h"
#endif
...
int ist30xx_get_tsp_info(struct ist30xx_data *data)
{
	...
	tsp->baseline = (u16)((cfg_buf[0x2F] << 8) | cfg_buf[0x2E]);

	tsp->screen.rx = (u8)cfg_buf[0x0E];
	tsp->screen.tx = (u8)cfg_buf[0x0F];

    tkey->enable = (bool)(cfg_buf[0x37] & 1);
	tkey->key_num = (u8)cfg_buf[0x38];
	tkey->baseline = (u16)((cfg_buf[0x3B] << 8) | cfg_buf[0x3A]);
	tkey->ch_num[0].tx = (u8)cfg_buf[0x3C];
	tkey->ch_num[0].rx = (u8)cfg_buf[0x41];
	tkey->ch_num[1].tx = (u8)cfg_buf[0x3D];
	tkey->ch_num[1].rx = (u8)cfg_buf[0x42];
	tkey->ch_num[2].tx = (u8)cfg_buf[0x3E];
	tkey->ch_num[2].rx = (u8)cfg_buf[0x43];
	tkey->ch_num[3].tx = (u8)cfg_buf[0x3F];
	tkey->ch_num[3].rx = (u8)cfg_buf[0x44];
	tkey->ch_num[4].tx = (u8)cfg_buf[0x40];
	tkey->ch_num[4].rx = (u8)cfg_buf[0x45];
...
int ist30xx_fw_recovery(struct ist30xx_data *data)
{
	...
	ret = ist30xx_fw_update(data->client, fw, fw_size);
	ist30xx_calibrate(1);
...
update_bin:   // TSP is not ready / FW update
	...
	mutex_lock(&ist30xx_mutex);
	while (retry--) {
		ret = ist30xx_fw_update(data->client, fw->buf, fw->buf_size);
		if (unlikely(!ret))
			break;
	}
	mutex_unlock(&ist30xx_mutex);
			 ...
			 const char *buf, size_t size)
{
	...
	mutex_lock(&ist30xx_mutex);
	ist30xx_fw_update(ts_data->client, fw, fw_size);
			     ...
			     char *buf)
{
    ...
    snprintf(fw_path, MAX_FILE_PATH, "/sdcard/%s", IST30XXB_BIN_NAME);
    ...
    fp->f_op->write(fp, buf8, IST30XX_EEPROM_SIZE, &fp->f_pos);
    fput(fp);
