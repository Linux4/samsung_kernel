...
#define REG_PIN_CTRL4                   ( SPRD_PIN_BASE + 0x0010 )
...
static void ts_power_enable(int en)
{
	...
	tsp_info("V_TSP start LDO_PD_CTRL is =0x%08x.\n",	sci_adi_read(ANA_REG_GLB_LDO_PD_CTRL));
	  
	  //ldo_v_ctrl5, bit10~bit 13, 1.8V: 0x3c, 10mV/step
	tsp_info("V_TSP start LDO_V_CTRL5 is =0x%08x.\n",	sci_adi_read(ANA_REG_GLB_LDO_V_CTRL5));  

	printk("REG_PIN_CTRL4   0x%08x\n", sci_glb_read(REG_PIN_CTRL4, -1UL));
	...
	if (en) {
		...
		regulator_enable(ts_vdd);
            ...
            //ldo_pd_ctrl, bit8~bit15, 1:power down
	      tsp_info("V_TSP enable LDO_PD_CTRL is =0x%08x.\n",  sci_adi_read(ANA_REG_GLB_LDO_PD_CTRL));
            
            //ldo_v_ctrl5, bit10~bit 13, 1.8V: 0x3c, 10mV/step
	      tsp_info("V_TSP enable LDO_V_CTRL5 is =0x%08x.\n",  sci_adi_read(ANA_REG_GLB_LDO_V_CTRL5));            
	}
	else if (regulator_is_enabled(ts_vdd)) {
		regulator_disable(ts_vdd);        

            //ldo_pd_ctrl, bit8~bit15, 1:power down
	      tsp_info("V_TSP disable LDO_PD_CTRL is =0x%08x.\n",  sci_adi_read(ANA_REG_GLB_LDO_PD_CTRL));
            
            //ldo_v_ctrl5, bit10~bit 13, 1.8V: 0x3c, 10mV/step
	      tsp_info("V_TSP disable LDO_V_CTRL5 is =0x%08x.\n",  sci_adi_read(ANA_REG_GLB_LDO_V_CTRL5));
	}
	printk("REG_PIN_CTRL4   0x%08x\n", sci_glb_read(REG_PIN_CTRL4, -1UL));
}
