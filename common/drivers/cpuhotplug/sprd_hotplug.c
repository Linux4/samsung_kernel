...
#ifdef CONFIG_DISABLE_SPRD_HOTPLUG_LCDON
static DECLARE_WAIT_QUEUE_HEAD(sprdhotplug_thread_wait);
static int sprd_hotplug_earlysuspend;
#endif
...
#ifndef CONFIG_DISABLE_SPRD_HOTPLUG_LCDON
...
#ifdef CONFIG_DISABLE_SPRD_HOTPLUG_LCDON
		wait_event_interruptible(sprdhotplug_thread_wait, sprd_hotplug_earlysuspend);
#endif
		dbs_check_cpu_sprd();
...
#ifdef CONFIG_DISABLE_SPRD_HOTPLUG_LCDON
	if(!(sprd_hotplug_earlysuspend) && !(input))
		return count;
#endif
...
#ifdef CONFIG_DISABLE_SPRD_HOTPLUG_LCDON
static void sprd_hotplug_early_suspend(struct early_suspend *h)
{
	sprd_hotplug_earlysuspend=1;
	wake_up_interruptible(&sprdhotplug_thread_wait);
	store_cpu_hotplug_disable(NULL,NULL,"0",0);
}

static void sprd_hotplug_late_resume(struct early_suspend *h)
{
	sprd_hotplug_earlysuspend=0;
	store_cpu_hotplug_disable(NULL,NULL,"1",0);
}

static struct early_suspend sprd_hotplug_early_suspend_desc = {
	.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING,
	.suspend = sprd_hotplug_early_suspend,
	.resume = sprd_hotplug_late_resume,
};
#endif
...
static void __init sprd_hotplug_init(void)
{
	...
	if (!np)
		pr_debug("can't find sprd-hotplug\n");
	else {
		ret = of_property_read_u32(np, "log_enable", &log_enable);
		if (ret)
			pr_debug("can't get log_enable property\n");
	}
#endif
...
#ifdef CONFIG_DISABLE_SPRD_HOTPLUG_LCDON
	register_early_suspend(&sprd_hotplug_early_suspend_desc);
#endif	
