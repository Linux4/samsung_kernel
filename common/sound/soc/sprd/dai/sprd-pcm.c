...
#define pcm_reg_set_val(addr, val, mask)  \
    do {    \
        uint32_t temp;    \
...
#define ANA_CFG0_REG 0xf53f8640
#define AAICR2_AAICR1_REG 0xf53f8668
#define SPK_EN_MASK 0x6000
#define AIL_EN_MASK 0x810
static int sprd_pcm_close(struct snd_pcm_substream *substream)
{
	...
	/*
	* This is for prevent PCM close noise via SPK
	* sejong123.park
	*/
	if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
	{
		uint tempSPK = 0;
		uint tempAIL = 0;
		tempSPK = sci_adi_read(ANA_CFG0_REG);
		tempAIL = sci_adi_read(AAICR2_AAICR1_REG);
		sp_asoc_pr_info("tempAIL : 0x%x!!!!!\n", tempAIL);
		if((tempSPK & SPK_EN_MASK) && !(tempAIL & AIL_EN_MASK)) {
			pcm_reg_set_val(ANA_CFG0_REG, 0, SPK_EN_MASK);
			sp_asoc_pr_info("%s, SPK off reg control!!!!!\n",
				__func__);
		} else {
			sp_asoc_pr_info("%s Playback, but not spk off case!\n",
				__func__);
		}
	}/* end of prevent */
