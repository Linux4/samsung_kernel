...
static inline void emit_sec_log(char *text, size_t size)
{
	if (sec_log_buf && sec_log_ptr) {
		/* Check overflow */
		size_t pos = *sec_log_ptr % (sec_log_size);
...
static int __init sec_log_setup(char *str)
{
	...
	if (reserve_bootmem(base, size, BOOTMEM_EXCLUSIVE)) {
		pr_err("%s: failed reserving size %lu " \
		       "at base 0x%lx\n",
		       __func__, (unsigned long)(size >> 20), base);
		goto out;
	} else {
		pr_err("%s: Success reserving size %ldMB " \
		       "at base 0x%lx\n",
		       __func__, (unsigned long)(size >> 20), base);
	}
	
	sec_log_mag = phys_to_virt(base) + size - 8;
	sec_log_ptr = phys_to_virt(base) + size - 4;
