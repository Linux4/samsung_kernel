...
__adi_read_retry:
	...
	if ((!v) || TO_ADDR(val) != (regPddr & readback_addr_mak)) {
		...
		if(adi_read_try_count>0)
		{
			mdelay(1);
			adi_read_try_count--;
			goto __adi_read_retry;
		}
		return -1;
	}
...
int sci_adi_read(u32 reg)
{
	...
	if (!__adi_addr_check(reg)) {
        ...
        if (!(sci_glb_read(REG_AON_APB_APB_EB0, 0xffffffff) & BIT_SPLK_EB)) {
            printk("hw spinlock has been disable, error!\n");
            BUG_ON(1);
        }
