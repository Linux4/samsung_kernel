...
void bak_restore_mm_scx15(int bak)
{
	...
	if(bak){
		aon_reg = sci_glb_read(REG_AON_APB_APB_EB0, -1UL);
		if (aon_reg & BIT_MM_EB) {
			sci_glb_clr(REG_PMU_APB_PD_MM_TOP_CFG,BIT_PD_MM_TOP_AUTO_SHUTDOWN_EN);
			mm_set = 1;
		} else {
	sci_glb_set(REG_AON_APB_APB_EB0, BIT_MM_EB);
...
uint32_t ldo_slp_ctrl0, ldo_slp_ctrl1, ldo_slp_ctrl2, xtl_wait_ctrl, ldo_slp_ctrl3, ldo_aud_ctrl4, dcdc_core_adi, dcdc_mem_adi, dcdc_arm_adi, dcdc_slp_ctrl0;
...
void bak_last_reg(void)
{
	...
	dcdc_core_adi = sci_adi_read(ANA_REG_GLB_DCDC_CORE_ADI);
	dcdc_mem_adi = sci_adi_read(ANA_REG_GLB_DCDC_MEM_ADI);
	dcdc_arm_adi = sci_adi_read(ANA_REG_GLB_DCDC_ARM_ADI);
	dcdc_slp_ctrl0 = sci_adi_read(ANA_REG_GLB_DCDC_SLP_CTRL0);
#if defined(CONFIG_ARCH_SCX15)
#else
...
void print_last_reg(void)
{
	...
	printk("ANA_REG_GLB_DCDC_CORE_ADI --- 0x%08x\n", dcdc_core_adi);
	printk("ANA_REG_GLB_DCDC_ARM_ADI --- 0x%08x\n", dcdc_arm_adi);
	printk("ANA_REG_GLB_DCDC_MEM_ADI --- 0x%08x\n", dcdc_mem_adi);
...
void int_work_round(void)
{
	...
	/* This line makes earjack interrupt disable after the deep sleep. */
