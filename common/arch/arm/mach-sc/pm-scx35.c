...
#if 1
	printk("REG_PIN_XTL_BUF_EN0~2   0x%08x, 0x%08x, 0x%08x\n",
		sci_glb_read(REG_PIN_XTL_BUF_EN0, -1UL), sci_glb_read(REG_PIN_XTL_BUF_EN1, -1UL), sci_glb_read(REG_PIN_XTL_BUF_EN2, -1UL));
#else
...
#endif
#if 1
	printk("REG_PMU_APB_XTL0~2_REL_CFG 0x%08x, 0x%08x, 0x%08x\n",
		sci_glb_read(REG_PMU_APB_XTL0_REL_CFG, -1UL), sci_glb_read(REG_PMU_APB_XTL1_REL_CFG, -1UL), sci_glb_read(REG_PMU_APB_XTL2_REL_CFG, -1UL));
	printk("REG_PMU_APB_XTLBUF0~1_REL_CFG 0x%08x, 0x%08x\n",
		sci_glb_read(REG_PMU_APB_XTLBUF0_REL_CFG, -1UL), sci_glb_read(REG_PMU_APB_XTLBUF1_REL_CFG, -1UL));
#else
...
void print_last_reg(void)
{
...
#endif
#if 1
	printk("REG_PMU_APB_CP_SLP_STATUS_DBG0~1 ----- 0x%08x, 0x%08x\n", cp_slp_status_dbg0, cp_slp_status_dbg1);
	printk("REG_PMU_APB_PWR_STATUS0~3_DBG ----- 0x%08x, 0x%08x, 0x%08x, 0x%08x\n", pwr_stat0, pwr_stat1, pwr_stat2, pwr_stat3);
#else
	...
	printk("aon apb reg\n");
#if 1
	printk("REG_AON_APB_APB_EB0~1  ----- 0x%08x, 0x%08x\n", apb_eb0, apb_eb1);
#else
	...
	printk("ana reg \n");
#if 1
	printk("ANA_REG_GLB_LDO_SLP_CTRL0~3 --- 0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
		ldo_slp_ctrl0, ldo_slp_ctrl1, ldo_slp_ctrl2, ldo_slp_ctrl3);
#else
...
#if 1
	printk("ANA_REG_GLB_PWR_XTL_EN0~3 -- 0x%08x, 0x%08x, 0x%08x, 0x%08x\n", 
		sci_adi_read(ANA_REG_GLB_PWR_XTL_EN0), sci_adi_read(ANA_REG_GLB_PWR_XTL_EN1), sci_adi_read(ANA_REG_GLB_PWR_XTL_EN2), sci_adi_read(ANA_REG_GLB_PWR_XTL_EN3));
#else
...
int dcdc_topclk_pd(int flag)
{
	...
	if (0x2723A090 != ana_chip_id)
		return 0;

	reg_val = sci_adi_read(ANA_REG_GLB_LDO_DCDC_PD);
	if (1 == flag) {
		sci_adi_raw_write(ANA_REG_GLB_LDO_DCDC_PD, reg_val|(1<<14));
	} else if (0 == flag) {
		sci_adi_raw_write(ANA_REG_GLB_LDO_DCDC_PD, reg_val&~(1<<14));
	} else {
		return -1;
...
#if defined(CONFIG_ADIE_SC2723S)
static int sc2723_vbat_check(void)
{
	int vbat_on = 1;
	int reg = sci_adi_read(ANA_REG_GLB_POR_SRC_FLAG);

	if(BITS_POR_SRC_FLAG(reg) & 0x08)
		vbat_on = 0; 
	pr_info("%s: JIG ON check: %d\n", __func__, !vbat_on);

	return vbat_on;
}

static struct vbat_check vbat_check = {
	.name = "sc2723-vbat-check",
	.id = VBAT_CHECK_ID_JIGON,
	.vbat_check_cb = sc2723_vbat_check,
};
#endif
...
#if defined(CONFIG_ADIE_SC2723S)
	/* TAB E L HW requirement: disable uart regardless of JIG ON
	 * register_vbat_check_cb(&vbat_check);
	 */
#endif
}
