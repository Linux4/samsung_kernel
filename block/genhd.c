...
static int diskstats_show(struct seq_file *seqf, void *v)
{
	...
	char buf[BDEVNAME_SIZE];
	int cpu;
	u64 uptime, in_flight_time, uptime_ms;
	...
	*/
	...
	while ((hd = disk_part_iter_next(&piter))) {
		...
		part_stat_unlock();
		uptime = ktime_to_ms(ktime_get());
                uptime_ms = do_div(uptime, MSEC_PER_SEC);
                in_flight_time = gp->queue->in_flight_time;
                do_div(in_flight_time, USEC_PER_MSEC);
