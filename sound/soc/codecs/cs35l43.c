...
static int cs35l43_pcm_vol_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	return snd_soc_get_volsw_sx(kcontrol, ucontrol);
}
				...
				bool read)
{
	...
	u8 operation;
	unsigned int i, j, num_words = 0, ret = 0;
					...
					struct cs35l43_write_seq *sequence)
{
	struct device *dev = cs35l43->dev;
	u32 *buf;
	u32 addr = 0, prev_addr = 0, value = 0, reg_value;
				...
				struct snd_soc_component *component)
{
	...
	regmap_read(cs35l43->regmap, CS35L43_NG_CONFIG,
			&cs35l43_cal_post_config[0].def);
	regmap_read(cs35l43->regmap, CS35L43_MIXER_NGATE_CH1_CFG,
			&cs35l43_cal_post_config[1].def);
	regmap_read(cs35l43->regmap, CS35L43_MIXER_NGATE_CH2_CFG,
