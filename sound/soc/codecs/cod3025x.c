...
static void cod3025x_jack_det_work(struct work_struct *work)
{
	struct cod3025x_priv *cod3025x =
		container_of(work, struct cod3025x_priv, jack_det_work.work);
	struct cod3025x_jack_det *jackdet = &cod3025x->jack_det;
	...
	/* read adc for mic detect */
		...
		dev_err(cod3025x->dev, " %s mic det adc  %d \n" , __func__, adc);

		if ( adc > cod3025x->mic_adc_range )
...
{
	struct cod3025x_priv *cod3025x =
		container_of(work, struct cod3025x_priv, buttons_work.work);
	...
	if (!jd->jack_det) {
		dev_err(cod3025x->dev, "Skip button events for jack_out\n");
		return;
	}
	if (!jd->mic_det) {
	...
	for ( j=0; j<cod3025x->adc_trace_num2; j++) {
		...
		dev_err(cod3025x->dev, ":button adc avg: %d, devi: %d\n",avg, devi);

		if (devi > ADC_DEVI_THRESHOLD ) {
			queue_delayed_work(cod3025x->buttons_wq,
					&cod3025x->buttons_work, 5);
			for ( i=0; i<ADC_TRACE_NUM; ){
				dev_err(cod3025x->dev, ":retry button_work :  %d %d %d %d %d\n",
	...
	if(adc_final > cod3025x->btn_release_value)
	...
	if(current_button_state == BUTTON_PRESS) {
		...
		dev_err(cod3025x->dev, ":key skipped. ADC %d\n", adc);
	} else {
		jd->button_det = false;
...
static irqreturn_t cod3025x_threaded_isr(int irq, void *data)
{
	...
	if (det_status_change) {
		/* mic detection delay */
		queue_delayed_work(cod3025x->jack_det_wq,
					&cod3025x->jack_det_work, cod3025x->mic_det_delay);
...
int cod3025x_jack_mic_register(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < num_buttons_zones; i++)
		set_bit(cod3025x->jack_buttons_zones[i].code, cod3025x->input->keybit);
	...
	snd_soc_update_bits(codec, COD3025X_05_IRQ1M, IRQ1M_MASK_ALL, 0x02);
	snd_soc_update_bits(codec, COD3025X_06_IRQ2M, IRQ2M_MASK_ALL, 0x02);

	/* Debounce time initial value set */
	snd_soc_write(codec, COD3025X_84_JACK_DET2, 0x02);
	snd_soc_write(codec, COD3025X_87_LDO_DIG, 0x03);
	...
	if(cod3025x->use_ldet_threshold_onem_mode)
	{
		snd_soc_update_bits(codec, COD3025X_83_JACK_DET1,
			CTRV_JD_VTH_MASK, 0x10);
		snd_soc_update_bits(codec, COD3025X_83_JACK_DET1,
...
static void cod3025x_i2c_parse_dt(struct cod3025x_priv *cod3025x)
{
	...
	if (!ret)
		cod3025x->mic_adc_range = mic_range;
	else
		cod3025x->mic_adc_range = 1120;

	ret = of_property_read_u32(dev->of_node, "mic-det-delay", &mic_delay);
	if (!ret)
		cod3025x->mic_det_delay = mic_delay;
	else
		cod3025x->mic_det_delay = 50;

	ret = of_property_read_u32(dev->of_node, "btn-release-value", &btn_rel_val);
	if (!ret)
		cod3025x->btn_release_value = btn_rel_val;
	else
		cod3025x->btn_release_value = 1100;
	...
	if (of_find_property(dev->of_node, "use-ldet-threshold-onem-mode", NULL) != NULL)
		cod3025x->use_ldet_threshold_onem_mode = true;
	else
		cod3025x->use_ldet_threshold_onem_mode = false;
...
static int cod3025x_codec_probe(struct snd_soc_codec *codec)
{
	...
	INIT_DELAYED_WORK(&cod3025x->buttons_work, cod3025x_buttons_work);
	...
	INIT_DELAYED_WORK(&cod3025x->jack_det_work , cod3025x_jack_det_work);

	cod3025x->jack_det_wq = create_singlethread_workqueue("jack_det_wq");
	if (cod3025x->jack_det_wq == NULL) {
...
static int cod3025x_codec_remove(struct snd_soc_codec *codec)
{
	...
	destroy_workqueue(cod3025x->jack_det_wq);
