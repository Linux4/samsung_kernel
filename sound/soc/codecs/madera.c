...
static const struct snd_soc_dapm_route madera_out1hp_routes[] = {
	{"HPOUTL", NULL, "OUT1L"},
	{"HPOUTR", NULL, "OUT1R"},
};

static const struct snd_soc_dapm_route madera_out1ep_routes[] = {
	{"EPOUTN", NULL, "OUT1L"},
	{"EPOUTN", NULL, "OUT1R"},
	{"EPOUTP", NULL, "OUT1L"},
	{"EPOUTP", NULL, "OUT1R"},
			  ...
			  struct snd_ctl_elem_value *ucontrol)
{
	...
	/* Make-before-break so we keep the codec powered-up */
	if (mux) {
		snd_soc_dapm_add_routes(dapm, madera_out1ep_routes,
					ARRAY_SIZE(madera_out1ep_routes));
		snd_soc_dapm_del_routes(dapm, madera_out1hp_routes,
					ARRAY_SIZE(madera_out1hp_routes));
	} else {
		snd_soc_dapm_add_routes(dapm, madera_out1hp_routes,
					ARRAY_SIZE(madera_out1hp_routes));
		snd_soc_dapm_del_routes(dapm, madera_out1ep_routes,
					ARRAY_SIZE(madera_out1ep_routes));
	}

	ret = snd_soc_dapm_sync(dapm);
	if (ret)
		dev_warn(madera->dev, "Failed to dapm sync OUT1 Demux change (%d)\n",
			 ret);
...
static const char * const madera_out1demux_texts[] = {
	"HPOUT",
	"EPOUT",
};

SOC_ENUM_SINGLE_DECL(madera_out1demux_enum,
		     MADERA_OUTPUT_ENABLES_1,
		     MADERA_EP_SEL_SHIFT,
		     madera_out1demux_texts);
EXPORT_SYMBOL_GPL(madera_out1demux_enum);
