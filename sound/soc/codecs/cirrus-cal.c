...
{
	struct wm_adsp *dsp = snd_soc_codec_get_drvdata(amp->codec);
	int ret;
...
static void cirrus_cal_redc_start(struct cirrus_amp *amp)
{
	...
	if (timeout == 0)
		dev_err(amp->codec->dev, "REINIT ready not found\n");
	...
	if (timeout == 0)
		dev_err(amp->codec->dev, "REINIT done not found\n");
...
int cirrus_cal_set_surface_temp(const char *suffix, int temperature)
{
	...
	dev_info(amp->codec->dev, "Set surface temp: %d degrees\n", temperature);
