/*
 * sound/soc/codec/aud3003x-5pin.c
 *
...
static int return_ldet_adc_thd(struct aud3003x_jack *jackdet)
{
	struct earjack_state *jackstate = &jackdet->jack_state;
 ...
 */
static void read_ldet_adc(struct aud3003x_jack *jackdet)
{
	struct aud3003x_priv *aud3003x = jackdet->p_aud3003x;
	struct earjack_state *jackstate = &jackdet->jack_state;
	struct snd_soc_component *codec = jackdet->codec;
	...
	snd_soc_component_update_bits(codec, AUD3003X_D1_DCTR_TEST1,
			T_CTRV_LDET_POP_MASK, T_CTRV_LDET_POP_MASK);
	snd_soc_component_write(codec, AUD3003X_C1_ACTR_JD2, 0x04);
	i2c_client_change(aud3003x, CODEC_CLOSE);
...
/*
 * aud3003x_ldet_adc_work() - Process AP check interrupt
 *
 * @work: delayed workqueue
 *
 ...
 */
static void aud3003x_ldet_adc_work(struct work_struct *work)
{
	struct aud3003x_jack *jackdet =
		container_of(work, struct aud3003x_jack, ldet_adc_work.work);
	struct earjack_state *jackstate = &jackdet->jack_state;
	struct device *dev = jackdet->p_aud3003x->dev;
...
static void aud3003x_jack_parse_dt(struct aud3003x_priv *aud3003x)
{
	...
	if (!ret)
		...
		jackdet->ldet_delay = AUD3003X_LDET_DELAY;
	...
	dev_dbg(dev, "LDET delay: %d, MDET delay: %d, Mic adc range: %d\n",
			jackdet->ldet_delay, jackdet->mdet_delay, jackdet->mic_adc_range);
...
static void aud3003x_jack_register_initialize(struct snd_soc_component *codec)
{
	...
	/* Pin Mode */
	snd_soc_component_write(codec, AUD3003X_D0_DCTR_CM, 0x2B);
...
int aud3003x_jack_probe(struct snd_soc_component *codec)
{
	...
	INIT_DELAYED_WORK(&jackdet->ldet_adc_work, aud3003x_ldet_adc_work);
