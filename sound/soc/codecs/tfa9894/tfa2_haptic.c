...
#define TFA_WRITE_XMEM_MAX_RETRIAL	5
#define TFA_SCALE_SEQUENCER_AMPLITUDE
#if defined(TFA_SCALE_SEQUENCER_AMPLITUDE)
#define TFA_REF_AMPLITUDE	0x36 /* 54 for max 255 */
#endif
#define TFA_PERIODIC_SEQUENCER_CHECK
...
static int tfa2_haptic_current_effect(struct haptic_data *data,
	char *str, int len)
{
	if (!str)
		return -EINVAL;
...
static struct haptic_tone_object *
	get_obj_cache_ptr(struct haptic_data *data, int idx)
{
	...
	if (idx >= (FW_XMEM_NR_OBJECTS1 + FW_XMEM_NR_OBJECTS2))
		...
		obj = (struct haptic_tone_object *)
			&data->object_table2_cache
			[idx - FW_XMEM_NR_OBJECTS1][0];
	else
		obj = (struct haptic_tone_object *)
			&data->object_table1_cache[idx][0];
...
static int get_obj_xmem_addr(int idx)
{
	...
	if (idx >= (FW_XMEM_NR_OBJECTS1 + FW_XMEM_NR_OBJECTS2))
		...
		addr = FW_XMEM_GENOBJECTS2 + idx * FW_XMEM_OBJECTSIZE;
	else
...
static int get_cmdobjsel_index(int idx)
{
	...
	if (idx >= (FW_XMEM_NR_OBJECTS1 + FW_XMEM_NR_OBJECTS2))
		...
		cmdobjsel_idx = idx - FW_XMEM_NR_OBJECTS1
			+ FW_CMDOBJSEL_TABLE2_OFFFSET;
	else
		cmdobjsel_idx = idx;
	...
	struct haptic_data *data, int index)
{
	...
	pr_info("%s: starting obj[%d] of (%d, %d ms)\n",
		__func__, index + 1,
		data->amplitude,
		tfa2_haptic_get_duration(tfa, index));

	tfa->hap_state = TFA_HAP_STATE_RUNNING;

#if defined(TFA_DUCKING_CONTROL)
	if (tfa->volume_db != 0xffffff) {
		rc = tfa2_haptic_write_volume_db
			(tfa->i2c, tfa->volume_db);
		if (rc < 0)
			dev_err(&tfa->i2c->dev,
				"%s: Failed to update attenutation (0x%06x): %d\n",
				__func__, tfa->volume_db, rc);
	}
#endif /* TFA_DUCKING_CONTROL */
	...
	struct haptic_data *data, int index)
{
	...
	int level;
#if defined(MEASURE_START_TIMING)
...
#endif
	int retry = 0;
	struct haptic_tone_object obj_rb;
#if defined(TFA_SEPARATE_WAVE_OBJECT)
	static int ref_level[FW_XMEM_NR_OBJECTS1 + FW_XMEM_NR_OBJECTS2];
#endif
...
#if !defined(TFA_SEPARATE_WAVE_OBJECT)
	level = ((data->amplitude * 0x7fffff + 50) / 100) & 0xffffff;
#else
	if (obj->type == OBJECT_WAVE) {
		if (ref_level[index] == 0) /* keep original level */
			ref_level[index] = obj->level;
		level = ((data->amplitude * ref_level[index] + 50)
			/ 100) & 0xffffff;
	} else {
		level = ((data->amplitude * 0x7fffff + 50)
			/ 100) & 0xffffff;
	}
	dev_dbg(&client->dev, "%s: index=%d, type=%d, level=0x%06x\n",
		__func__, index +1, obj->type, level);
#endif
	...
	if ((obj->type == OBJECT_TONE) || (obj->type == OBJECT_SILENCE)) {
		...
		/* use DSP timer (msec) */
		samples = (data->duration * 48) & 0xffffff;
		object_changed += (obj->duration_cnt_max != samples);
		obj->duration_cnt_max = samples;

		if (data->frequency) {
			/* Q13.11 */
			int freq = (data->frequency << 11) & 0xffffff;
	...
	if (object_changed > 0) {
		dev_info(&client->dev,
			"updated: level=%d, samples=%d, freq=%d (addr=%d)\n",
			obj->level, obj->duration_cnt_max,
			obj->freq, address);

		do {
			/* write parameters */
			rc = tfa2_i2c_write_cf_mem32(client,
				address, (int32_t *)obj,
				FW_XMEM_OBJECTSIZE, TFA2_CF_MEM_XMEM);
			if (rc < 0)
				break;

			/* read back to ensure updating obj */
			rc = tfa2_i2c_read_cf_mem32(client,
				address, (int32_t *)&obj_rb,
				FW_XMEM_OBJECTSIZE, TFA2_CF_MEM_XMEM);
			if (rc < 0)
				break;

			dev_info(&client->dev,
				"read back: level=%d, samples=%d, freq=%d (addr=%d)\n",
				obj_rb.level, obj_rb.duration_cnt_max,
				obj_rb.freq, address);
		} while (((obj->level != obj_rb.level)
			|| (obj->duration_cnt_max != obj_rb.duration_cnt_max)
			|| (obj->freq != obj_rb.freq))
			&& (retry++ < TFA_WRITE_XMEM_MAX_RETRIAL));
	}
	...
	rc = tfa2_i2c_write_cf_mem32(client, cmdobjsel,
		&cmdobjsel_index, 1, TFA2_CF_MEM_XMEM);
...
int tfa2_haptic_stop(struct tfa2_device *tfa,
	struct haptic_data *data, int index)
{
	int silence_obj = FW_HB_SILENCE_OBJ; /* silence object */
#if defined(SET_FINISHED_FLAG)
	int stop_obj = FW_HB_STOP_OBJ; /* finished flag */
#endif
	int cmdobjsel = FW_XMEM_CMDOBJSEL0;
	int rc;
	...
	rc = tfa2_i2c_write_cf_mem32(tfa->i2c, cmdobjsel, &silence_obj,
		1, TFA2_CF_MEM_XMEM);
	rc = tfa2_haptic_write_sampcnt0(tfa->i2c, 0);
	if (rc < 0)
		return rc;
#if defined(SET_FINISHED_FLAG)
	rc = tfa2_i2c_write_cf_mem32(tfa->i2c, cmdobjsel, &stop_obj,
		1, TFA2_CF_MEM_XMEM);
#endif
#if defined(TFA_USE_WAITQUEUE_SEQ)
	wake_up_interruptible(&tfa->waitq_seq);
#endif
...
int tfa2_haptic_get_duration(struct tfa2_device *tfa, int index)
{
	...
	if (index < FW_HB_SEQ_OBJ) {
		...
		struct haptic_tone_object *obj = get_obj_cache_ptr(data, index);
		/* duration is in sample count : 48k is 48 samples/ms */
		int duration = obj->duration_cnt_max / 48;

		if (obj->type != OBJECT_WAVE) {
			/* take into account that duration might get adjusted
			 * in tfa2_haptic_start_object()
			 */
			duration = data->duration ? data->duration : duration;
		}
		/* add object hold time */
		total_duration = duration + data->delay_attack;
	} else {/* sequencer */
		int rc;
		struct tfa2_sequence seq;

		rc = tfa2_hap_cnt_get_sequencer(tfa->cnt,
			tfa->dev_idx, index - FW_HB_SEQ_OBJ, &seq);
		if (rc < 0) {
			pr_err("No sequencer\n");
			return -EINVAL;
		}
...
int tfa2_haptic_update_duration(struct haptic_data *data, int duration)
{
	...
	if ((obj->type == OBJECT_TONE) && (obj->boost_brake_on == 1))
		boost_length = (obj->boost_length / 48);
...
enum tfa_haptic_object_type
	tfa2_haptic_object_type(struct haptic_data *data, int index)
{
	struct haptic_tone_object *obj = get_obj_cache_ptr(data, index);

	if (!obj)
		return OBJECT_TONE; /* sequencer */
...
{
	return tfa2_i2c_read_cf_mem32(client, FW_XMEM_R0,
		p_value, 1, TFA2_CF_MEM_XMEM);
}

int tfa2_haptic_read_f0(struct i2c_client *client, int *p_value)
{
	return tfa2_i2c_read_cf_mem32(client, FW_XMEM_F0,
		p_value, 1, TFA2_CF_MEM_XMEM);
}

int tfa2_haptic_read_sampcnt0(struct i2c_client *client, int *p_value)
{
	return tfa2_i2c_read_cf_mem32(client, FW_XMEM_SAMPCNT0,
		p_value, 1, TFA2_CF_MEM_XMEM);
}

int tfa2_haptic_write_sampcnt0(struct i2c_client *client, int value)
{
	return tfa2_i2c_write_cf_mem32(client, FW_XMEM_SAMPCNT0,
		&value, 1, TFA2_CF_MEM_XMEM);
}

int tfa2_haptic_read_recalc_selector(struct i2c_client *client, int *p_value)
{
	return tfa2_i2c_read_cf_mem32(client, FW_XMEM_RECALCSEL,
		p_value, 1, TFA2_CF_MEM_XMEM);
}

int tfa2_haptic_write_recalc_selector(struct i2c_client *client, int value)
{
	return tfa2_i2c_write_cf_mem32(client, FW_XMEM_RECALCSEL,
		&value, 1, TFA2_CF_MEM_XMEM);
}

int tfa2_haptic_read_volume_db(struct i2c_client *client, int *p_value)
{
	return tfa2_i2c_read_cf_mem32(client, FW_XMEM_VOLUME_DB,
		p_value, 1, TFA2_CF_MEM_XMEM);
}

int tfa2_haptic_write_volume_db(struct i2c_client *client, int value)
{
	return tfa2_i2c_write_cf_mem32(client, FW_XMEM_VOLUME_DB,
		&value, 1, TFA2_CF_MEM_XMEM);
}
...
int tfa2_haptic_disable_f0_trc(struct i2c_client *client, int disable)
{
	...
	return tfa2_i2c_write_cf_mem32(client, FW_XMEM_DISF0TRC,
		&disable_f0_trc, 1, TFA2_CF_MEM_XMEM);
}

int tfa2_haptic_obj0_set(struct i2c_client *client, int objnr)
{
	return tfa2_i2c_write_cf_mem32(client, FW_XMEM_CMDOBJSEL0,
		&objnr, 1, TFA2_CF_MEM_XMEM);
}

int tfa2_haptic_obj0_wait_finish(struct i2c_client *client)
{
	int rc, loop = 500, ready = 0, sampcnt0;
	...
	do {
		...
		if (ready == 1)
			break;
		/* wait to avoid busload // TODO decrease time?... */
...
int tfa2_haptic_recalculate_wait(struct tfa2_device *tfa, int object)
{
	...
	if (object >= 0) {
		...
		tfa2_i2c_read_cf_mem32(tfa->i2c,
			...
			+ offsetof(struct haptic_tone_object,
				duration_cnt_max) / 4,
			&duration,
			1,
			TFA2_CF_MEM_XMEM);
...
static int tfa2_hap_patch_version(struct tfa2_device *tfa, char *string)
{
	...
	rc = tfa2_i2c_read_cf_mem32_dsp_reset(tfa->i2c,
		FW_XMEM_VERSION, &fw_version, 1, TFA2_CF_MEM_XMEM);
	dev_dbg(&tfa->i2c->dev, "%s patch version %d.%d.%d\n", string,
		(fw_version>>16)&0xff, (fw_version>>8)&0xff, fw_version&0xff);

	if (check_haptic_version && (fw_version != FW_VERSION)) {
		dev_err(&tfa->i2c->dev,
			"%s: unsupported firmware version 0x%x, expected 0x%x\n",
...
static int tfa2_hap_recalc(struct tfa2_device *tfa, int object)
{
	...
	if ((f0 > 80) && (f0 < 336)) {
		...
		dev_dbg(&tfa->i2c->dev,
			"Warning: f0:%d out of range [80, 336] Hz, recalculate failed\n",
			f0);
		/* no error return, in case 1st time calibration fails */
	}
...
{
	int rc, hbprofile;
	char profile_name[MAX_CONTROL_NAME];

	strlcpy(profile_name,
		(tfa->need_hb_config & TFA_HB_ROLE_MASK) == TFA_HB_LRA
		? "lra" : "ls", MAX_CONTROL_NAME);
	strlcat(profile_name, "_data.hap", MAX_CONTROL_NAME);

	/* get "*_data.hap" */
	hbprofile = get_hap_profile(tfa, profile_name);
	if (hbprofile < 0) {
		dev_err(&tfa->i2c->dev,
			"No [%s] profile found\n", profile_name);
		return -EINVAL;
	}
	/* write patch from profile */
	rc = tfa2_cnt_write_files_profile(tfa, hbprofile, 0);
	if (rc < 0)
		return rc;
...
void tfa2_haptic_obj_get(struct haptic_data *data, int idx)
{
	...
	data->duration = obj->duration_cnt_max / 48;
}
...
int tfa2_hap_recalculate(struct tfa2_device *tfa, int object)
{
	...
	if (rc < 0) {
		dev_err(&tfa->i2c->dev,
			"Error, waiting for operating state in recalculation\n");
		return rc;
	}
...
int tfa2_hap_calibrate(struct tfa2_device *tfa)
{
	...
	if (!tfa->need_hb_config) {
		dev_err(&tfa->i2c->dev,
			"HB calibration not done, no config found in container\n");
		return -EINVAL;
	}
	...
	msleep_interruptible(1000 + tfa->hap_data.delay_attack);
	...
	/* Check F0/R0 ranges */
	rc = tfa2_i2c_read_cf_mem32(tfa->i2c, FW_XMEM_F0_R0_RANGES,
		range, 4, TFA2_CF_MEM_XMEM);
	if (rc < 0) {
	...
	/* TODO check F0 for non-zero;
	 * always make no sense if the FW uses MTP
	 */
...
int tfa2_hap_sequencer(struct tfa2_device *tfa, struct tfa2_sequence *seq)
{
...
#if defined(TFA_SCALE_SEQUENCER_AMPLITUDE)
	int base_amplitude = tfa->hap_data.amplitude;
#endif
#if defined(TFA_USE_WAITQUEUE_SEQ)
	int loop_check;
#else
#if defined(TFA_PERIODIC_SEQUENCER_CHECK)
	int loop_check;
	ktime_t start_time, stop_time;
	s64 delta_time;
#endif /* TFA_PERIODIC_SEQUENCER_CHECK */
#endif /* TFA_USE_WAITQUEUE_SEQ */

	if (seq == 0 || seq->object == 0)
		return -EINVAL;

#if defined(TFA_SCALE_SEQUENCER_AMPLITUDE)
	dev_dbg(&tfa->i2c->dev, "%s: base [%d] %d in sequence\n",
		__func__, tfa->hap_data.index + 1, base_amplitude);
#endif
	...
	for (i = 0; i < seq->length; i++) {
...
#if defined(TFA_SCALE_SEQUENCER_AMPLITUDE)
			tfa->hap_data.amplitude = base_amplitude
				* seq->level[i] / TFA_REF_AMPLITUDE;
#else
		...
		if (tfa->hap_state != TFA_HAP_STATE_STOPPED) {
			...
			loop_check = jiffies_to_msecs(rc);
#if defined(TFA_ASYNCHRONOUS_WAKE_UP)
			rc = tfa2_haptic_stop(tfa, &tfa->hap_data, idx);
			if (rc < 0) {
				total_duration = rc; /* return value */
				break;
			}
#endif
			dev_info(&tfa->i2c->dev, "%s: stopped in the middle (%d)\n",
				__func__, loop_check);
			break;
		}
#else
		...
		do {
			...
			if (loop_check > 0)
				...
				msleep_interruptible(this_duration % 10);

			/* check if stopped every 10 seconds */
			if (tfa->hap_state == TFA_HAP_STATE_STOPPED)
				break;

			stop_time = ktime_get_boottime();
			delta_time
				= ktime_to_ms(ktime_sub(stop_time, start_time));
			loop_check -= ((int)delta_time - 10);
		} while (loop_check > 0);
	...
	struct tfa2_sequence *seq)
{
	...
	for (i = 0; i < seq->length; i++) {
		...
		this_duration = tfa2_haptic_get_duration(tfa, idx);
		total_duration += (this_duration + 5); /* extra 5 msec */
	}
...
static void tfa2_hap_save_haptic_cache(struct tfa2_device *tfa,
	struct tfa2_sequence *seq)
{
	memcpy(&seq->object_table1_cache_save,
		&tfa->hap_data.object_table1_cache,
		sizeof(tfa->hap_data.object_table1_cache));
	memcpy(&seq->object_table2_cache_save,
		&tfa->hap_data.object_table2_cache,
		sizeof(tfa->hap_data.object_table2_cache));
}

static void tfa2_hap_test_restore_haptic_cache(struct tfa2_device *tfa,
	struct tfa2_sequence *seq)
{
	if (memcmp(&tfa->hap_data.object_table1_cache,
		&seq->object_table1_cache_save,
		sizeof(tfa->hap_data.object_table1_cache))) {
		memcpy(&tfa->hap_data.object_table1_cache,
			&seq->object_table1_cache_save,
			sizeof(tfa->hap_data.object_table1_cache));
	...
	if (memcmp(&tfa->hap_data.object_table2_cache,
		&seq->object_table2_cache_save,
		sizeof(tfa->hap_data.object_table2_cache))) {
		memcpy(&tfa->hap_data.object_table2_cache,
			&seq->object_table2_cache_save,
			sizeof(tfa->hap_data.object_table2_cache));
		tfa2_i2c_write_cf_mem32(tfa->i2c,
			FW_XMEM_GENOBJECTS2,
...
int tfa2_hap_sequencer_object(struct tfa2_device *tfa, int virtual_obj)
{
	...
	rc = tfa2_hap_cnt_get_sequencer(tfa->cnt, dev_idx,
		virtual_obj - FW_HB_SEQ_OBJ - 1, &seq);
	if (rc < 0) {
		pr_err("No sequencer\n");
		return -EINVAL;
	}
...
int tfa2_hap_cnt_sequencer_count(struct tfa_container *cnt, int devidx)
{
	...
	do {
		rc = tfa2_cnt_grep_nth_profile_name(cnt,
			devidx, total, ".seq.hap");
	...
	int devidx, int seqidx, struct tfa2_sequence *seq)
{
	...
	seq_profidx = tfa2_cnt_grep_nth_profile_name(cnt, devidx,
		seqidx, ".seq.hap");
	if (seq_profidx < 0) {
	...
	for (i = 0; i < 4; i++) {
		rc = tfa2_cnt_get_cmd(cnt, devidx,
			seq_profidx, i, &array24, &length24);
		if (rc < 0)
