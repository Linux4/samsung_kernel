/*
 * tfa_cal.c   tfa9xxx calibration in sysfs
 *
...
static char f0_string[FILESIZE_F0_CAL] = {0};
...
static ssize_t tfa_cal_f0_show(struct device *dev,
	struct device_attribute *attr, char *buf);
static ssize_t tfa_cal_f0_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size);
static DEVICE_ATTR(f0, S_IRUGO | S_IWUSR | S_IWGRP,
	tfa_cal_f0_show, tfa_cal_f0_store);
...
static ssize_t tfa_cal_statushap_show(struct device *dev,
	struct device_attribute *attr, char *buf);
static ssize_t tfa_cal_statushap_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size);
static DEVICE_ATTR(statushap, S_IRUGO | S_IWUSR | S_IWGRP,
	tfa_cal_statushap_show, tfa_cal_statushap_store);

static struct attribute *tfa2_cal_attr[] = {
	&dev_attr_rdc.attr,
	&dev_attr_f0.attr,
	&dev_attr_temp.attr,
	&dev_attr_status.attr,
	&dev_attr_statushap.attr,
	NULL,
};

static struct attribute_group tfa2_cal_attr_grp = {
	.attrs = tfa2_cal_attr,
};

/* ---------------------------------------------------------------------- */

extern struct class *g_nxp2_class;
struct device *g_tfa2_dev;
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = tfa2_read_calibrate(RDC_CAL, &value);
	if (ret) {
	...
	struct device_attribute *attr, char *buf)
{
...
#endif
	int ret = -1;
	char cal_done_string[FILESIZE_F0_CAL] = {0};

#if !defined(TFA_WRITE_CAL_TO_FILE)
	if (f0_string[0] != 0) {
		rstr_len = strlcpy(cal_done_string,
			f0_string, FILESIZE_F0_CAL);
		if (rstr_len > 0)
			ret = 0;
		goto tfa_cal_f0_show_exit;
	} else {
		ret = tfa_cal_read_file(FILEPATH_F0_CAL,
			cal_done_string, sizeof(cal_done_string));
		if (ret >= 0) {
			ret = kstrtou16(cal_done_string, 0, &value);
			strlcpy(f0_string, cal_done_string, FILESIZE_F0_CAL);
			pr_info("%s: get from file\n", __func__);
			goto tfa_cal_f0_show_exit;
		}
	}
	...
	pr_info("%s: failed to read %s and get from amplifier\n",
		__func__, FILEPATH_F0_CAL);

	ret = tfa2_read_calibrate(F0_CAL, &value);
	if (ret) {
		pr_info("%s: failed to read data from amplifier\n",
			__func__);
		goto tfa_cal_f0_show_exit;
	...
	snprintf(cal_done_string,
		FILESIZE_F0_CAL, "%d", value);

#if defined(TFA_WRITE_CAL_TO_FILE)
	ret = tfa_cal_write_file(FILEPATH_F0_CAL,
		cal_done_string, sizeof(cal_done_string));
	if (ret < 0) {
		pr_err("%s: failed to write %s\n",
			__func__, FILEPATH_F0_CAL);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = tfa2_run_calibrate(RDC_CAL, &value);
	if (ret) {
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* run calibration and read data to store */
	ret = tfa2_run_calibrate(F0_CAL, &value);
	if (ret) {
		pr_info("%s: failed to calibrate speaker\n", __func__);
		cur_status = 0;
		return size;
...
static int __init tfa9xxx_cal_init(void)
{
	int ret = 0;
	...
	if (g_nxp2_class) {
		g_tfa2_dev = device_create(g_nxp2_class,
			NULL, 1, NULL, TFA_CAL_DEV_NAME);
		if (!IS_ERR(g_tfa2_dev)) {
			ret = sysfs_create_group(&g_tfa2_dev->kobj,
				&tfa2_cal_attr_grp);
			if (ret)
...
module_init(tfa9xxx_cal_init);

static void __exit tfa9xxx_cal_exit(void)
{
	device_destroy(g_nxp2_class, 1);
	class_destroy(g_nxp2_class);
	pr_info("exited\n");
}
module_exit(tfa9xxx_cal_exit);

MODULE_DESCRIPTION("ASoC TFA9XXX calibration driver");
MODULE_LICENSE("GPL");
