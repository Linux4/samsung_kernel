...
const char *tfa2_get_i2c_status_id_string(int status)
{
	if ((status < TFA9XXX_NO_DSP_RESPONSE)
		...
		/* Unspecified error */
		status = TFA9XXX_I2C_REQ_CALIB_FAILED + 1;
...
static const char *tfa2_state_enum_string[] = {
	...
	"TFA_STATE_CLOCK"	/**<  use OSC if no external clock */
};
...
int tfa2_dev_probe(struct tfa2_device *tfa)
{
	...
	tfa->slave_address = tfa->i2c->addr; /* TODO legacy to be removed? */
...
/*	tfa->support_saam = SUPPORT_NOT_SET; */
	...
	tfa->configure_log = tfa2_configure_log; /* default */
	tfa->update_log = tfa2_update_log; /* default */
#endif
...
int tfa2_dev_start_cf(struct tfa2_device *tfa)
{
	...
	rc = tfa2_i2c_write_cf_mem32_dsp_reset(tfa->i2c,
		0x8100, &control, 1, TFA2_CF_MEM_IOMEM);
	if (rc < 0)
		return rc;
	...
	if (tfa->need_hb_config && !(tfa->need_hb_config & TFA_HB_READY)) {
		...
		if (rc < 0)
			return rc;
		/* ready, don't do it again */
		tfa->need_hb_config |= TFA_HB_READY;
	}
...
int tfa2_dev_start(struct tfa2_device *tfa, int next_profile, int vstep)
{
	...
	char next_profile_name[MAX_CONTROL_NAME];
	...
	dev_dbg(&tfa->i2c->dev,
		"HW:%s ", tfa->need_hw_init ? "cold" : "warm");
	dev_dbg(&tfa->i2c->dev,
		"SB:%s ", tfa->need_sb_config ? "cold" : "warm");
	...
	if (tfa->need_hw_init) {
		...
		rc = tfa2_dev_start_hw(tfa, next_profile);
		if (rc < 0) {
			dev_dbg(&tfa->i2c->dev, "%s : error when starting hardware\n",
				__func__);
			goto exit_tfa2_dev_start;
		}
	} else {
		/* start the clocks */
	...
	/* start coolflux if needed:
	 * not in bypass or any firmware config needed
	 */
	...
	if (tfa2_dev_cf_enabled(tfa)
		&& tfa2_dev_cf_configured(tfa)) {
		if (tfa->configure_log) {
			pr_debug("set blackbox to start\n");
			tfa->configure_log(tfa);
		}
	}
#endif
	 ...
	 */
	strlcpy(next_profile_name,
		tfa2_cnt_profile_name(tfa->cnt, tfa->dev_idx, next_profile),
		MAX_CONTROL_NAME);
	if (strnstr(next_profile_name, ".standby",
		strlen(next_profile_name)) != NULL) {
...
exit_tfa2_dev_start:
	...
	/* FOR DEBUG: read status */
	tfa2_debug_status(tfa);
...
int tfa2_dev_stop(struct tfa2_device *tfa)
{
	...
	rc = tfa2_dev_set_state(tfa, TFA_STATE_POWERDOWN | TFA_STATE_MUTE);
...
{
	/* SBSL is not set if config not done */
	return tfa->need_sb_config;
	/* TODO confirm /test
	 * return tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_ACS);
	 */
}
...
#define TFA_STATE_POWERDOWN_POLL	200
...
int tfa2_dev_set_state(struct tfa2_device *tfa, enum tfa_state state)
{
	...
	case TFA_STATE_POWERDOWN: /* PLL in powerdown, Algo up */
		...
		tfa2_i2c_write_bf(tfa->i2c,
			TFA9XXX_BF_MANAOOSC, 1); /* 1 = off */
		tfa2_i2c_write_bf_volatile(tfa->i2c, TFA9XXX_BF_AMPE, 0);
		break;
	case TFA_STATE_POWERUP:
		/**< PLL to powerup, Algo can be up/warm */
		/* power on the sub system */
		if (manstate == 9 || manstate == 6) { /* if already done */
			rc = 0;
			break;
		}
		/* powerup osc */
		tfa2_i2c_write_bf(tfa->i2c,
			TFA9XXX_BF_MANAOOSC, 0); /* 0 = on */
		tfa2_i2c_write_bf_volatile(tfa->i2c, TFA9XXX_BF_MANSCONF, 1);
		rc = tfa2_i2c_write_bf_volatile(tfa->i2c, TFA9XXX_BF_PWDN, 0);
		if (rc < 0)
			return rc;
		tfa2_i2c_write_bf(tfa->i2c, TFA9XXX_BF_AMPE, 1);

#if defined(CHECK_INTERNAL_OSC)
		/* if not on intercal osc then we wait for external clock */
		ext_clock_wait = (tfa2_i2c_read_bf(tfa->i2c,
			TFA9XXX_BF_MANCOLD) == 0);
		if (ext_clock_wait) {
		...
		rc = tfa2_dev_clock_stable_wait(tfa);
		if (rc < 0) {
			dev_err(&tfa->i2c->dev, "%s: clock is not stable\n",
				__func__);
			break;
		}
#endif
	...
	case TFA_STATE_OPERATING: /* Amp and Algo running */
		...
		if (manstate == 6) {
			tfa2_i2c_write_bf_volatile(tfa->i2c,
				TFA9XXX_BF_PWDN, 1);
			tfa2_i2c_write_bf_volatile(tfa->i2c,
				TFA9XXX_BF_PWDN, 0);
			ext_clock_wait = 1;
		...
		/* if in wait2srcsetting and mansconf is done
		 * then we wait for external clock
		 */
		if (manstate == 1) {
			ext_clock_wait = tfa2_i2c_read_bf(tfa->i2c,
				TFA9XXX_BF_MANSCONF)
...
int tfa2_dev_init(struct tfa2_device *tfa)
{
	 ...
	 * Write the register directly to avoid read in bitfield function.
	 * I2CR bit may overwrite full register because it is reset anyway.
	 */
...
int tfa2_dev_set_swprofile(struct tfa2_device *tfa, unsigned short new_value)
{
	...
	tfa2_i2c_write_bf_volatile(tfa->i2c,
		TFA9XXX_BF_SWPROFIL, new_value + 1);
...
void tfa2_dev_update_config_init(struct tfa2_device *tfa)
{
	...
	if (tfa->need_hb_config < 0) {
		...
		if (tfa2_cnt_grep_profile_name(tfa->cnt,
			tfa->dev_idx, "lra_") >= 0)
			tfa->need_hb_config = TFA_HB_LRA;
		else if (tfa2_cnt_grep_profile_name(tfa->cnt,
			tfa->dev_idx, "ls_") >= 0)
...
int tfa2_dev_dsp_system_stable(struct tfa2_device *tfa, int *ready)
{
	...
	if (tfa->reg_time) {
		...
		do {
			/* dummy read */
			tfa2_i2c_read_bf(tfa->i2c, tfa->bf_clks);
		} while (loop--);
	}
 ...
 *		value = 0;
 *	} else {
 *		value = tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_SBSL);
 *	}
 */
	value = tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_SBSL);
...
int tfa2_i2c_rpc_status(struct i2c_client *i2c, int *p_rpc_status)
{
	...
	while (retry_cnt < CF_RPC_RETRY_NUM) {
		...
		if (rc < 0)
			return rc;
		/* ret contains flags, when no error */
		cf_status = (uint16_t)rc;

		if ((cf_status & CF_STATUS_I2C_CMD_ACK))
			/* There is no error and DSP already ACK-ed message */
			break;
		retry_cnt++;
	}
	...
	if ((retry_cnt >= CF_RPC_RETRY_NUM)
		&& !(cf_status & CF_STATUS_I2C_CMD_ACK)) {
		/* Set RPC result initially to TFA9XXX_NO_DSP_RESPONSE.
		 * In case we have comm. problems
		 */
		*p_rpc_status = TFA9XXX_NO_DSP_RESPONSE;
		/* TODO no_dsp_response is misleading, should be error */
		rc = -ENXIO;
	} else {
		/* Get DSP status (could be busy, RPC error or OK) */
		rc = tfa2_i2c_read_cf_mem24(i2c,
			0, xmem0, 3, TFA2_CF_MEM_XMEM);
		/* result is high byte in big endian */
		*p_rpc_status = xmem0[2];
	}
	...
	uint16_t wait_value, int loop)
{
	...
	if (rc == -ETIMEDOUT)
		dev_err(&client->dev,
			"timeout waiting for bitfield:0x%04x, value:%d (%d), %d times\n",
			bf, wait_value, value, loop_arg);
	...
	size_t cmd_len, char *res_buf, size_t res_len)
{
	...
	for (tries = TFA9XXX_WAITRESULT_NTRIES; tries > 0; tries--) {
		...
		if (rc || rpc_status != TFA9XXX_I2C_REQ_BUSY)
			/* Return on I/O error or error status from DSP */
			break;

		/* Add non-busy wait to give DSP processing time */
	...
	uint16_t bitfield, uint16_t value, int write_always)
{
	...
	if (write_always || (oldvalue != newvalue))
		return tfa2_i2c_write_reg(client,
			(bitfield >> 8) & 0xff, newvalue);
	return 0;
}
...
static int write_cf_mem(struct i2c_client *client, uint16_t address,
	int32_t *input, int size,
	enum tfa2_cf_mem type, enum tfa2_cf_flags flags)
{
	int idx = 0;
	uint8_t *output;
	int rc;
	...
	output[idx++] = 0x00;
	if (flags & dsp_reset)
		/* AIF:0 (ON) DMEM:1 (xmem) RST:1 */
		output[idx++] = (type<<1) | 1;
	else
		/* AIF:0 (ON) DMEM:1 (xmem) RST:0 */
		output[idx++] = (type<<1);
	output[idx++] = (address >> 8) & 0xff;
	output[idx++] = address & 0xff;
	...
	const uint8_t *bytes)
{
	...
	while (index + 1 < length) {
		...
		if ((index + size) > length)
			/* outside buffer, error in input data */
			return -EINVAL;
...
int tfa2_calibrate_profile_start(struct tfa2_device *tfa)
{
	...
	cal_profile = tfa2_cnt_grep_profile_name(tfa->cnt,
		tfa->dev_idx, ".cal");
	if (cal_profile < 0) {
...
int tfa2_sb_calibrate(struct tfa2_device *tfa)
{
	...
	/* get SB calibration profile: with .cal extension */
	sbcal_profile = tfa2_cnt_grep_profile_name(tfa->cnt,
		tfa->dev_idx, ".cal");
	if (sbcal_profile < 0) {
		dev_err(&tfa->i2c->dev, "No [*.cal] profile found for calibration\n");
		return -EINVAL;
