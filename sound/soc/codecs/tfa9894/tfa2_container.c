...
/*
 * Get the name of device at a certain index in the container file
 * return device name
 */
...
/*
 * Get the name of profile at certain index for a device in the container file
 * return profile name
 */
	...
	int dev_idx, int n, const char *string)
{
	...
	for (prof = 0; prof < cnt->nprof; prof++) {
		...
		if (profile && strnstr(profile, string, strlen(profile))) {
	...
	for (i = 0; i < length; i++) {
		...
		if (dsc_list[i].type == dsc_default) {
			*default_section_idx = i;
			/* clock dep items cannot be in the default section */
			break;
		}
	}
...
#if defined(TFA_USE_DEFAULT_PROFILE)
/*
...
int tfa2_cnt_write_regs_profile(struct tfa2_device *tfa, int prof_idx)
{
	...
	/* process the list until end of profile or default section */
	...
	struct tfa_desc_ptr *dsc)
{
	...
	case dsc_bit_field:
		...
		rc = tfa2_i2c_write_bf_volatile(tfa->i2c,
			bitf->field, bitf->value);
		break;
	case dsc_file:
		rc = tfa2_cnt_write_file(tfa,
			(struct tfa_file_dsc *)(cnt_offset));
		break;
	case dsc_cmd:
		rc = tfa2_cnt_write_msg_dsc(tfa, dsc);
		break;
	case dsc_cf_mem:
		rc = tfa2_cnt_write_dspmem(tfa,
			(struct tfa_dsp_mem *)(cnt_offset));
		break;
...
int tfa2_cnt_write_msg(struct tfa2_device *tfa, int wlength, char *wbuf)
{
	...
	case 0x106: /* SetLsModel 0x48106*/
		tfa->xmax = (int)(cmd_id24[X_MAX_OFFSET] << 16
			| cmd_id24[X_MAX_OFFSET + 1] << 8
			| cmd_id24[X_MAX_OFFSET + 2])
			/ (TFA2_FW_X_DATA_SCALE / 1000);
		tfa->tmax = (int)(cmd_id24[T_MAX_OFFSET] << 16
			| cmd_id24[T_MAX_OFFSET + 1] << 8
			| cmd_id24[T_MAX_OFFSET + 2])
			/ TFA2_FW_T_DATA_SCALE + 10;
		dev_dbg(&tfa->i2c->dev, "xmax=%d, tmax=%d\n",
			tfa->xmax, tfa->tmax);
		break;
#endif
	...
	if (tfa->verbose)
		dev_dbg(&tfa->i2c->dev, "Writing cmd=0x%06x,size=%d\n",
			cmd_id24[0] << 16 | cmd_id24[1] << 8 | cmd_id24[2],
			wlength);
