...
static int tfa9xxx_device_count;
static int tfa9xxx_sync_count;
static LIST_HEAD(profile_list);        /* list of user selectable profiles */
static int tfa9xxx_mixer_profiles; /* number of user selectable profiles */
static int tfa9xxx_mixer_profile;  /* current mixer profile */
#if defined(TFA_MUTE_BY_MIXER)
static int tfa9xxx_mute_control;  /* current mute_control */
#endif
#if defined(TFA_PERIODIC_STATUS_LOG)
#define TFA_MAX_LOGGING_COUNT 3
static int tfa9xxx_logging_counter;
#endif

static struct snd_kcontrol_new *tfa9xxx_controls;
static struct tfa_container *tfa9xxx_container;

static int tfa9xxx_kmsg_regs;
static int tfa9xxx_ftrace_regs;
	...
	int next_profile, int vstep)
{
	...
	if (tfa9xxx->tfa->volume_db != 0xffffff) {
		int volume_db_update;

		volume_db_update = (tfa9xxx->clk_users > 1)
			? tfa9xxx->tfa->volume_db : 0;

		err = tfa2_haptic_write_volume_db(tfa9xxx->i2c,
			volume_db_update);
		if (err < 0)
			dev_err(&tfa9xxx->i2c->dev,
				"%s: Failed to update attenutation (0x%06x): %d\n",
				__func__, volume_db_update, err);
	}
	...
	if (trace_level & 8) {
		...
		dev_dbg(&tfa9xxx->i2c->dev, "%s: (%d,%d) time = %lld us\n",
			__func__, next_profile, vstep, delta_time);
	}
	...
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	/* enable MCLK before start */
	if (tfa9xxx->clk_users == 0) {
		ret = tfa9xxx_mclk_enable(tfa9xxx);
		if (ret < 0)
			return -EIO;
	}
	...
	/* disable MCLK after stop */
	if (tfa9xxx->clk_users == 0)
		ret = tfa9xxx_mclk_disable(tfa9xxx);
 ...
 */
static int add_sr_to_profile(struct tfa9xxx *tfa9xxx,
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(tfa9xxx, &tfa9xxx_device_list, list) {
		...
		int ready = 0;
		int new_vstep;
		int value = ucontrol->value.integer
			.value[tfa9xxx->tfa->dev_idx];

		vstep = tfa9xxx->prof_vsteps[profile];
		/* TODO */
		vsteps = tfa_cont_get_max_vstep(tfa9xxx->tfa, profile);
...
#if defined(TFA_MUTE_BY_MIXER)
static int tfa9xxx_get_mute_ctl(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tfa9xxx *tfa9xxx = snd_soc_codec_get_drvdata(codec);
	...
	dev_dbg(&tfa9xxx->i2c->dev, "%s: state: %d\n",
		__func__, tfa9xxx_mute_control);

	mutex_lock(&tfa9xxx_mutex);
	ucontrol->value.integer.value[0] = tfa9xxx_mute_control;
	mutex_unlock(&tfa9xxx_mutex);

	return 0;
}

static int tfa9xxx_set_mute_ctl(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tfa9xxx *tfa9xxx = snd_soc_codec_get_drvdata(codec);
	int mute_select = ucontrol->value.integer.value[0];

	dev_dbg(&tfa9xxx->i2c->dev, "%s: state: %d (prev: %d)\n",
		__func__, mute_select, tfa9xxx_mute_control);
	pr_info("%s: set mute control by force (%s)\n", __func__,
		mute_select ? "mute" : "unmute");

	mutex_lock(&tfa9xxx_mutex);
	tfa9xxx_mute_control = mute_select;
	mutex_unlock(&tfa9xxx_mutex);

	return 0;
}
#endif /*  TFA_MUTE_BY_MIXER */
...
int tfa2_run_calibrate(enum tfa_cal_type type, uint16_t *val)
{
	...
	list_for_each_entry(tfa9xxx, &tfa9xxx_device_list, list) {
		...
		if (tfa9xxx->clk_users == 0) {
			ret = tfa9xxx_mclk_enable(tfa9xxx);
			if (ret < 0)
				goto tfa2_run_calibrate_exit;
		}

		ret = tfa2_dev_mtp_set(tfa9xxx->tfa, TFA_MTP_OTC, 1);
		if (ret < 0) {
			dev_err(&tfa9xxx->i2c->dev, "%s: Error in setting OTC %d\n",
				__func__, ret);
			goto tfa2_run_calibrate_exit;
		}
		...
		case RDC_CAL:
			...
			value = tfa2_get_calibration_impedance(tfa9xxx->tfa);
			if (value < 0) {
				dev_err(&tfa9xxx->i2c->dev, "%s: Unable to read R: %d\n",
					__func__, value);
				goto tfa2_run_calibrate_exit;
			}
			dev_info(&tfa9xxx->i2c->dev, "%s: Re25C : %d\n",
				__func__, value);

#if defined(TFA_STORE_CAL_DATA)
			tfa9xxx->tfa->rdc = value;
#endif
			break;
		...
		case F0_CAL:
			...
			value = tfa2_dev_mtp_get(tfa9xxx->tfa, TFA_MTP_F0);
			if (value < 0) {
				dev_err(&tfa9xxx->i2c->dev, "%s: Unable to read F: %d\n",
					__func__, value);
				goto tfa2_run_calibrate_exit;
			}
			value = value / 2 + 80;
...
EXPORT_SYMBOL(tfa2_run_calibrate);
...
int tfa2_read_calibrate(enum tfa_cal_type type, uint16_t *val)
{
	...
	list_for_each_entry(tfa9xxx, &tfa9xxx_device_list, list) {
		switch (type) {
		case RDC_CAL:
			value = tfa9xxx->tfa->rdc;
			dev_info(&tfa9xxx->i2c->dev, "%s: Re25C : %d\n",
				__func__, value);
			break;

		case F0_CAL:
			value = tfa9xxx->tfa->f0;
			dev_info(&tfa9xxx->i2c->dev, "%s: F0 : %d\n",
				__func__, value);
			break;
		}
	...
	*val = (uint16_t)value;
	...
	list_for_each_entry(tfa9xxx, &tfa9xxx_device_list, list) {
		...
		/* enable MCLK before start */
		if (tfa9xxx->clk_users == 0) {
			err = tfa9xxx_mclk_enable(tfa9xxx);
			if (err < 0)
				goto tfa2_read_calibrate_exit;
		}

		mtpex = tfa2_dev_mtp_get(tfa9xxx->tfa, TFA_MTP_EX);
		if (mtpex < 0) {
			dev_err(&tfa9xxx->i2c->dev, "%s: Unable to read EX: %d\n",
				__func__, mtpex);
			goto tfa2_read_calibrate_exit;
		}
		...
		otc = tfa2_dev_mtp_get(tfa9xxx->tfa, TFA_MTP_OTC);
		if (otc < 0) {
			dev_err(&tfa9xxx->i2c->dev, "%s: Unable to read OTC %d\n",
				__func__, otc);
			goto tfa2_read_calibrate_exit;
		}
		...
		case RDC_CAL:
			if ((mtpex > 0) && (otc > 0))
				value = tfa2_dev_mtp_get(tfa9xxx->tfa,
					TFA_MTP_R25C);
			if (value < 0) {
				dev_err(&tfa9xxx->i2c->dev, "%s: Unable to read R: %d\n",
					__func__, value);
		...
		/* disable MCLK after stop */
		if (tfa9xxx->clk_users == 0)
			err = tfa9xxx_mclk_disable(tfa9xxx);
...
tfa2_read_calibrate_exit:
	...
	return -EIO;
#endif /* TFA_STORE_CAL_DATA */
}
EXPORT_SYMBOL(tfa2_read_calibrate);
...
int tfa2_read_cal_temp(uint16_t *val)
{
	...
	list_for_each_entry(tfa9xxx, &tfa9xxx_device_list, list) {
		value = tfa9xxx->tfa->temp;
		dev_info(&tfa9xxx->i2c->dev, "%s: Temp : %d\n",
			__func__, value);
	}
	...
	*val = (uint16_t)value;
	...
	list_for_each_entry(tfa9xxx, &tfa9xxx_device_list, list) {
		...
		if (mtpex == 0) {
			dev_warn(&tfa9xxx->i2c->dev, "%s: Calibration is not stored in MTP\n",
				__func__);
			tfa9xxx->tfa->temp = 25; /* ambient temp */
		}
...
EXPORT_SYMBOL(tfa2_read_cal_temp);
...
static int tfa9xxx_create_controls(struct tfa9xxx *tfa9xxx)
{
	...
	for (prof = 0; prof < nprof; prof++) {
		 ...
		 * add it to list and add a new mixer control (vsteps)
		 * also, if it is a calibration profile, do not add it to list
		 */
		if ((is_profile_in_list(bprofile->basename,
				bprofile->len) == 0)
...
#if defined(VOLUME_FIXED)
			if (tfa_cont_get_max_vstep(tfa9xxx->tfa, prof)) {
				/* TODO */
				...
				tfa9xxx_controls[mix_index]
					.info = tfa9xxx_info_vstep;
				tfa9xxx_controls[mix_index]
					.get = tfa9xxx_get_vstep;
				tfa9xxx_controls[mix_index]
					.put = tfa9xxx_set_vstep;
				tfa9xxx_controls[mix_index]
					.private_value = bprofile->item_id;
				/* save profile index */
				mix_index++;
			}
#endif
		 ...
		 * and add profile index to the supported sample rates
		 * of this mixer profile
		 */
		add_sr_to_profile(tfa9xxx,
...
#if defined(TFA_MUTE_BY_MIXER)
	/* Create a mixer item to mute amplifier */
	name = devm_kzalloc(tfa9xxx->codec->dev, MAX_CONTROL_NAME, GFP_KERNEL);
	if (!name)
		return -ENOMEM;

	scnprintf(name, MAX_CONTROL_NAME, "%s Mute", tfa9xxx->fw.name);
	tfa9xxx_controls[mix_index].name = name;
	tfa9xxx_controls[mix_index].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	tfa9xxx_controls[mix_index].info = snd_soc_info_bool_ext;
	tfa9xxx_controls[mix_index].get = tfa9xxx_get_mute_ctl;
	tfa9xxx_controls[mix_index].put = tfa9xxx_set_mute_ctl;
	/* tfa9xxx_controls[mix_index].private_value = profs; */
	/* save number of profiles */
	mix_index++;
#endif /* TFA_MUTE_BY_MIXER */
	...
	if (dai_drv && num_dai > 0)
		for (i = 0; i < num_dai; i++) {
			...
			snprintf(buf, 50, "%s-%x-%x",
				...
				i2cbus, addr);
			dai_drv[i].playback
				.stream_name = tfa9xxx_devm_kstrdup(dev, buf);
...
static void tfa9xxx_add_widgets(struct tfa9xxx *tfa9xxx)
{
	struct snd_soc_dapm_context *dapm
		= snd_soc_codec_get_dapm(tfa9xxx->codec);
	struct snd_soc_dapm_widget *widgets;
	unsigned int num_dapm_widgets;
	unsigned int num_dapm_routes;

	num_dapm_widgets = ARRAY_SIZE(tfa9xxx_dapm_widgets_common);
	num_dapm_routes = ARRAY_SIZE(tfa9xxx_dapm_routes_common);
...
static void tfa9xxx_dsp_cold_start(struct work_struct *work)
{
	struct tfa9xxx *tfa9xxx;
	int err;
#if defined(TFA_STORE_CAL_DATA)
	int mtpex, otc, value = -1;
#endif

	tfa9xxx = container_of(work, struct tfa9xxx, init_work.work);

	/* update init state */
	tfa2_dev_update_config_init(tfa9xxx->tfa);

	if (tfa9xxx->tfa->need_hw_init == 0
		&& tfa9xxx->tfa->need_sb_config == 0)
		/* amplifier is already warm after cold started */
		return;
	...
	dev_info(&tfa9xxx->i2c->dev,
		"%s: Started for silent cold start (haptic mode %d)\n",
		__func__, tfa9xxx->haptic_mode);
...
#if defined(TFA_DUCKING_CONTROL)
	err = tfa2_haptic_read_volume_db(tfa9xxx->i2c,
		&tfa9xxx->tfa->volume_db);
	if (err < 0) {
		dev_err(&tfa9xxx->i2c->dev,
			"%s: Unable to read attenutation: %d\n",
			__func__, err);
		tfa9xxx->tfa->volume_db = 0xffffff; /* no ducking control */
	} else {
		dev_info(&tfa9xxx->i2c->dev, "%s: attenuation : 0x%06x\n",
			__func__, tfa9xxx->tfa->volume_db);
	}
#endif /* TFA_DUCKING_CONTROL */
	...
	if (strncmp(dflt_prof_name, "", strlen(dflt_prof_name))) {
		...
		for (i = 0; i < nprof; i++) {
			if (strncmp(tfa_cnt_profile_name(tfa9xxx, i),
				dflt_prof_name,
				strlen(tfa_cnt_profile_name(tfa9xxx, i)))
				== 0) {
				tfa9xxx->profile = i;
...
static void tfa9xxx_monitor(struct work_struct *work)
{
	...
	/* reschedule */
#if defined(TFA_PERIODIC_STATUS_LOG)
	if (tfa9xxx_logging_counter < TFA_MAX_LOGGING_COUNT)
		queue_delayed_work(tfa9xxx->tfa9xxx_wq,
...
static int tfa9xxx_dsp_init(struct tfa9xxx *tfa9xxx)
{
	...
	if (do_sync) {
		...
		list_for_each_entry(tfa9xxx, &tfa9xxx_device_list, list) {
			mutex_lock(&tfa9xxx->dsp_lock);
#if defined(TFA_MUTE_BY_MIXER)
			if (tfa9xxx_mute_control) {
				dev_info(&tfa9xxx->i2c->dev,
					"%s: trigger mute by force\n",
					__func__);
				tfa2_dev_set_state(tfa9xxx->tfa,
					TFA_STATE_MUTE);
			} else {
				tfa2_dev_set_state(tfa9xxx->tfa,
					TFA_STATE_UNMUTE);
			 ...
			 */
#if defined(TFA_PERIODIC_STATUS_LOG)
			tfa9xxx_logging_counter = 0; /* reset logging counter */
#endif
	...
	struct snd_soc_dai *dai)
{
	...
	for (prof = 0; prof < nprof; prof++) {
		if (strncmp(basename,
			...
			if (tfa9xxx->rate_constraint.count
				>= TFA9XXX_NUM_RATES) {
...
static int tfa9xxx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		if (tfa9xxx->tfa->update_log) {
			mutex_lock(&tfa9xxx->dsp_lock);
			pr_debug("get data from blackbox\n");
			tfa9xxx->tfa->update_log(tfa9xxx->tfa);
			mutex_unlock(&tfa9xxx->dsp_lock);
		}
#endif
		...
		tfa9xxx_sync_count = 0;
#if defined(TFA_MUTE_BY_MIXER)
		tfa9xxx_mute_control = 0; /* reset mute_control */
#endif
		mutex_unlock(&tfa9xxx_mutex);
		...
		dev_info(&tfa9xxx->i2c->dev, "%s: clk_users %d\n",
			__func__, tfa9xxx->clk_users);
...
static int tfa9xxx_ext_reset(struct tfa9xxx *tfa9xxx)
{
	...
	else if (tfa9xxx->vddd) {
		int ret;

		dev_info(&tfa9xxx->i2c->dev, "%s: reset (regulator for VDDD)\n",
			__func__);

		ret = regulator_disable(tfa9xxx->vddd);
		if (ret != 0)
			dev_warn(&tfa9xxx->i2c->dev, "%s: failed in disabling VDDD: %d\n",
				__func__, ret);
		else
			msleep(I2C_RETRY_DELAY);

		ret = regulator_enable(tfa9xxx->vddd);
		if (ret != 0) {
			dev_warn(&tfa9xxx->i2c->dev, "%s: failed in enabling VDDD: %d\n",
				__func__, ret);
			tfa9xxx->vddd = NULL;
		}
	}
#endif /* TFA_FORCED_POR */
	else {
	   dev_info(&tfa9xxx->i2c->dev, "%s: reset (polarity of VDDD)\n",
		   __func__);

	   /* toggle IPOVDDS (polarity) to set ISTVDDS */
	   tfa2_i2c_write_bf(tfa9xxx->i2c, TFA9XXX_BF_IPOVDDS, 0); /* low */
	   tfa2_i2c_write_bf(tfa9xxx->i2c, TFA9XXX_BF_IPOVDDS, 1); /* high */
	}
...
static int tfa9xxx_probe(struct snd_soc_codec *codec)
{
	...
	INIT_DELAYED_WORK(&tfa9xxx->init_work, tfa9xxx_dsp_cold_start);
	...
	struct tfa9xxx *tfa9xxx, struct device_node *np)
{
...
#endif
	rc = of_property_read_u32(np, "haptic-fw-intensity-max",&tfa9xxx->tfa2_fw_intensity_max);
	if (rc < 0){
		dev_dbg(dev, "No haptic-fw-intensity-max provided , will use vendor default max value\n");
		tfa9xxx->tfa2_fw_intensity_max = 255;
	}
	...
	if (rc)
		...
		snprintf(tfa9xxx->vib_type, sizeof(tfa9xxx->vib_type), "%s", type);
...
retry:
	...
	if (error < 0) {
		if (tfa9xxx->vddd) {
			ret = regulator_disable(tfa9xxx->vddd);
			if (ret != 0)
				dev_warn(&client->dev, "%s: failed in disabling VDDD: %d\n",
					__func__, ret);
			...
			if (ret == 0) {
				...
				ret = regulator_enable(tfa9xxx->vddd);
				if (ret != 0)
					dev_warn(&client->dev, "%s: failed in enabling VDDD back: %d\n",
						__func__, ret);
			}
		}
	}
...
int tfa9xxx_mclk_open(struct tfa9xxx *drv)
{
	...
	if (gpio_is_valid(drv->mclk_gpio)) {
		dev_info(&drv->i2c->dev, "using mclk\n");
#if !defined(TFA_CONTROL_MCLK)
		dev_info(&drv->i2c->dev, "set mclk active\n");
		gpio_set_value_cansleep(drv->mclk_gpio, 1);
#endif /* TFA_CONTROL_MCLK */
	}
#else
	drv->mclk = devm_clk_get(&i2c->dev, "mclk");
	if (IS_ERR(drv->mclk)) {
		dev_warn(&drv->i2c->dev, "mclk not set\n");
		drv->mclk = NULL;
	}

	if (drv->mclk) {
		dev_info(&drv->i2c->dev, "using mclk\n");
		ret = clk_set_rate(drv->mclk, FIXED_MCLK_RATE);
		if (ret < 0) {
			dev_warn(&drv->i2c->dev, "failed in setting rate to mclk\n");
			drv->mclk = NULL;
		}
#if !defined(TFA_CONTROL_MCLK)
		dev_info(&drv->i2c->dev, "set mclk active\n");
		if (drv->mclk) {
			...
			if (ret < 0)
				dev_warn(&drv->i2c->dev, "failed in enabling mclk\n");
		}
#endif /* TFA_CONTROL_MCLK */
	}
...
int tfa9xxx_mclk_close(struct tfa9xxx *drv)
{
	...
	if (drv->mclk) {
		clk_disable_unprepare(drv->mclk);
		devm_clk_put(&drv->i2c->dev, drv->mclk);
	}
#endif /* TFA_CONTROL_MCLK */
	...
	const struct i2c_device_id *id)
{
	...
	/* open MCLK conrol */
	tfa9xxx_mclk_open(tfa9xxx);

#if defined(TFA_FORCED_POR)
	tfa9xxx->vddd = devm_regulator_get(&i2c->dev, "vddd");
	if (IS_ERR(tfa9xxx->vddd)) {
		dev_warn(&i2c->dev, "vddd not set\n");
		tfa9xxx->vddd = NULL;
	}
	...
	if (tfa9xxx->vddd) {
		ret = regulator_enable(tfa9xxx->vddd);
		if (ret != 0) {
			dev_warn(&i2c->dev, "%s: failed in enabling VDDD: %d\n",
				__func__, ret);
			tfa9xxx->vddd = NULL;
		}
	}
	 ...
	 * We create copies of original structures because each device
	 * has its own instance of this structure, with its own address.
	 */
...
static int tfa9xxx_i2c_remove(struct i2c_client *i2c)
{
	...
	/* close MCLK conrol */
	tfa9xxx_mclk_close(tfa9xxx);

#if defined(TFA_FORCED_POR)
	if (tfa9xxx->vddd)
		devm_regulator_put(tfa9xxx->vddd);
#endif
...
static const struct of_device_id tfa9xxx_dt_match[] = {
