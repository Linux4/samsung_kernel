				...
				char *buf, loff_t off, size_t count)
{
	struct device *dev = container_of(kobj, struct device, kobj);
	struct tfa98xx *tfa98xx = dev_get_drvdata(dev);
	size_t write_count = (count > 2) ? 2 : count; /* 2: 16-bit register */
	u8 *data;
	int ret = 0;
	int retries = I2C_RETRIES;
	...
	pr_info("i2c rw write: 0x%x (offset: %d, write_count %d, count %d)\n",
				...
				char *buf, loff_t off, size_t count)
{
	struct device *dev = container_of(kobj, struct device, kobj);
	struct tfa98xx *tfa98xx = dev_get_drvdata(dev);
	size_t read_count = (count > 2) ? 2 : count; /* 2: 16-bit register */
	struct i2c_msg msgs[] = {
		{
	...
	if (off >= PAGE_SIZE ||
		off + count > PAGE_SIZE) {
		pr_info("%s: read out full buffer\n", __func__);
		return 0;
	}

	pr_info("i2c rw read: 0x%x (offset: %d, read_count %d, count %d)\n",
