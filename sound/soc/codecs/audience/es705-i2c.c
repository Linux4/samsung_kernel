...
static int es705_i2c_boot_setup(struct es705_priv *es705)
{
	...
	if (rc < 0) {
		dev_err(es705->dev, "%s(): firmware load failed boot write %d\n",
			__func__, rc);
			   ...
			   const struct i2c_device_id *id)
{
	...
	es705_priv.mclk = devm_clk_get(&i2c->dev, "mclk");
	if (IS_ERR(es705_priv.mclk))
		dev_err(&i2c->dev, "%s: mclk not found\n", __func__);
