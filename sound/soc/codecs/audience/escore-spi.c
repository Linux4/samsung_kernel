...
static int spread_spi_read(struct spi_device *spi, void *buf, size_t len)
{
	int rc = 0;
	int main_size, remain_size, i;
	char *tp = buf;
	main_size = len / BURST_SIZE * BURST_SIZE;
	if (main_size) {
		for (i=0; i<main_size; i+=BURST_SIZE) {
			rc = spi_read(spi, &tp[i], BURST_SIZE);
			if (rc < 0)
				return rc;
		}
	}

	remain_size = len - main_size;
	if (remain_size)
		rc = spi_read(spi, &tp[main_size], remain_size);
	return rc;
}

static int spread_spi_write(struct spi_device *spi, const void *buf, size_t len)
{
	int rc = 0;
	int main_size, remain_size, i;
	char *tp = buf;
	...
	if (main_size) {
		for (i=0; i<main_size; i+=BURST_SIZE) {
			rc = spi_write(spi, &tp[i], BURST_SIZE);
			if (rc < 0)
				return rc;
		}
	}
	...
	remain_size = len - main_size;
	if (remain_size)
		rc = spi_write(spi, &tp[main_size], remain_size);
	return rc;
}
#endif
...
static int escore_spi_read(struct escore_priv *escore, void *buf, int len)
{
	int rc;
#if defined(CONFIG_ARCH_WHALE)
	rc = spread_spi_read(escore_spi, buf, len);
#else
...
#if defined(CONFIG_ARCH_WHALE)
	rc = spread_spi_write(escore_spi, buf, len);
#else
	rc = spi_write(escore_spi, buf, len);
#endif
	if (rem != 0)
#if defined(CONFIG_ARCH_WHALE)
		rc = spread_spi_write(escore_spi, align, 4 - rem);
#else
			    ...
			    const void *buf, int len)
{
	...
	int rem = 0;
	char align[4] = {0};
	//u8 bits_per_word = ES_SPI_8BITS_PER_WORD;
	...
	if (is_vmalloc_or_module_addr(buf)) {
		...
		if (((unsigned long)buf & (~PAGE_MASK))) {
...
#if defined(CONFIG_ARCH_WHALE)
			rc = spread_spi_write(escore_spi, kaddr, bytes);
#else
			...
			if (len == 0) {
				if (rem)
#if defined(CONFIG_ARCH_WHALE)
					rc = spread_spi_write(escore_spi,
							align, 4 - rem);
#else
		...
		while  (dma_bytes > 0) {
			...
			kaddr = pfn_to_kaddr(pfn);
#if defined(CONFIG_ARCH_WHALE)
			rc = spread_spi_write(escore_spi, kaddr,
					(dma_bytes > PAGE_SIZE) ?
					PAGE_SIZE : dma_bytes);
#else
...
#if defined(CONFIG_ARCH_WHALE)
			rc = spread_spi_write(escore_spi, kaddr, non_dma_bytes);
#else
...
static int escore_spi_probe(struct spi_device *spi)
{
	...
	if (escore->high_bw_intf == ES_SPI_INTF) {
		spi_probe_thread = kthread_run(escore_spi_probe_thread,(void *) &spi->dev, "escore spi thread");
...
MODULE_DEVICE_TABLE(spi, escore_spi_id);

static const struct of_device_id escore_of_id[] = {
	{ .compatible = "adnc,earSmart_spi", },
	{ /* sentinel */ }
};

MODULE_DEVICE_TABLE(of, escore_of_id);
...
struct spi_driver escore_spi_driver = {
	.driver = {
		...
		.owner  = THIS_MODULE,
		.pm = &escore_pm_ops,
		.of_match_table = of_match_ptr(escore_of_id),
	},
