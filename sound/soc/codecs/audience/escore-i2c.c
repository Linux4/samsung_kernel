			   ...
			   const struct i2c_device_id *id)
{
	...
	i2c_set_clientdata(i2c, escore);
		...
		pr_debug("enter USE_clk_aux1\n");
		wlan_clk = clk_get(escore->dev, "clk_aux1");
		if (IS_ERR(wlan_clk)) {
			pr_debug("clock: failed to get clk_aux1\n");
		}
		pr_debug("after clk_get clk_aux1\n");
		clk_parent = clk_get(escore->dev, "ext_26m");
		...
		int raw_read = 0;
	//	raw_read = __raw_readl(REG_AON_APB_APB_EB1);
	//	pr_debug("aa clock REG_AON_APB_APB_EB1=%#x raw_read=%#x\n", REG_AON_APB_APB_EB1, raw_read);
#endif
	
#ifdef CODEC_POWER_1V1
		struct device_node *nodep = NULL;
		int gpio_codec_power = -1;
		pr_debug("enter CODEC_POWER_1V1\n");
		if (escore->dev){
			pr_debug("enter escore->dev\n");
			nodep = escore->dev->of_node;
		}
		pr_debug("escore->dev =%lp\n", escore->dev);
		if(nodep){
			pr_debug("parse dt codec_power\n");
				gpio_codec_power = of_get_named_gpio(nodep,
				"adnc,codec_power", 0);
			if (gpio_codec_power < 0) {
				pr_debug("parse from dt gpio_codec_power=%d\n", gpio_codec_power);
				gpio_codec_power = 145;
				pr_debug("codec_power dt failed, and use codec_power=%d forcely\n", gpio_codec_power);
			}
			pr_debug("gpio_request codec_power=%d", gpio_codec_power);
			rc = gpio_request(gpio_codec_power, "codec_power");
			if (rc < 0) {
				printk("gpio_request failed\n");
			}
			rc = gpio_direction_output(gpio_codec_power, 1);
			if (rc < 0) {
				printk("gpio_direction_output failed\n");
			}
			pr_debug("gpio_set_value high\n");
			gpio_set_value(gpio_codec_power, 1);
		}
#endif
