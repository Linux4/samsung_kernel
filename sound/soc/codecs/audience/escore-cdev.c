			       ...
			       size_t count, loff_t *f_pos)
{
	...
	slen -= *f_pos;
	cnt = min(count, slen);
	err = unalign_copy_to_user(buf, readbuf + *f_pos, cnt);
	if (err) {
		dev_dbg(escore->dev, "%s unalign_copy_to_user = %d\n", __func__, err);
		goto OUT;
	}
	*f_pos += cnt;
				...
				size_t count, loff_t *f_pos)
{
	...
	while (used < count) {
		...
		err  = unalign_copy_from_user(data, ptr, frag);
		if (err) {
			      ...
			      size_t count, loff_t *f_pos)
{
	...
	while (wr < count) {
		...
		ret = unalign_copy_from_user(fw_buf, buf + wr, bs);
		if (ret) {
			dev_err(escore->dev,
...
read_next_page:
	...
	while (count_remain > 0) {
		copy_len = min((int)count_remain, (int) length -
			stream_read_off);
		err = unalign_copy_to_user(buf + user_pos, stream_read_page +
				...
				unsigned long arg)
{
	...
	/* Copy structure */
	rc = unalign_copy_from_user(&datablock_copy, argp, sizeof(datablock_copy));
	if (rc) {
	...
	case ADNC_WRITE_HOTWORD_IOCTL:
		...
		/* Copy data */
		rc = unalign_copy_from_user(escore->hotword_buf, datablock_copy.buf,
					datablock_copy.buf_size);
		if (rc) {
					...
					size_t count, loff_t *f_pos)
{
	...
	rc = unalign_copy_to_user(buf, escore->datablock_dev.rdb_read_buffer + *f_pos,
			rcnt);
				...
				size_t count, loff_t *f_pos)
{
	...
	err = unalign_copy_from_user(data_buf, buf, count);
	if (err) {
		dev_err(escore->dev, "%s(): unalign_copy_from_user err: %d\n",
							__func__, err);
		goto OUT;
	}
					...
					size_t count, loff_t *f_pos)
{
	...
	rc = unalign_copy_to_user(buf, cmd_history_buf + pos, size);
	if (rc) {
		pr_err("%s() error in unalign_copy_to_user() %d\n", __func__, rc);
		return rc;
	}
				...
				size_t count, loff_t *f_pos)
{
	...
	err = unalign_copy_from_user(cmd_history_buf, buf, count);
	if (err) {
		pr_err("%s(): unalign_copy_from_user err: %d\n", __func__, err);
		goto out;
	}
