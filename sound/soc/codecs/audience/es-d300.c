...
/* This function must be called with access_lock acquired */
int _es_stop_route(struct escore_priv *escore)
{
...
/* This function must be called with access_lock acquired */
int _es_set_final_route(struct escore_priv *escore)
{
	int rc = 0, i;
	u16 preset;
	u32 resp = 0;
	...
	if (escore->algo_preset != 0) {
		...
		pr_debug("%s:add algo preset: %d", __func__,
				escore->algo_preset);
		rc = escore_write(escore, ES_PRESET, escore->algo_preset);
		if (rc)
...
int es_d300_set_route(struct escore_priv *escore)
{
	int rc;

	mutex_lock(&escore->access_lock);
	rc = _es_set_final_route(escore);
	mutex_unlock(&escore->access_lock);

	return rc;
}

int es_d300_stop_route(struct escore_priv *escore)
{
	int rc;

	mutex_lock(&escore->access_lock);
	rc = _es_stop_route(escore);
	mutex_unlock(&escore->access_lock);
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	return escore_write_locked(&escore_priv, reg, value);
}
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	value = ucontrol->value.enumerated.item[0];
	rc = escore_write_locked(escore, reg, value);
	if (rc) {
		pr_err("%s(): Prepare write message fail %d\n", __func__, rc);
		return rc;
	}
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	if (atomic_read(&escore->active_streams)) {
		...
		pr_debug("%s:add algo preset: %d", __func__,
				escore->algo_preset);
		rc = escore_write_locked(escore, ES_PRESET,
					escore->algo_preset);
		if (rc)
			dev_err(escore_priv.dev, "%s(): Set Preset failed:%d\n",
				__func__, rc);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	rc = escore_read_locked(&escore_priv, reg);
	if (rc < 0) {
...
static const struct snd_soc_dapm_widget es_d300_dapm_widgets[] = {
	...
	/* AIF */
	SND_SOC_DAPM_AIF_IN("PCM0.0 RX", "PORTA Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("PCM0.1 RX", "PORTA Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("PCM0.2 RX", "PORTA Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("PCM0.3 RX", "PORTA Playback", 0,
			SND_SOC_NOPM, 1, 0),

	SND_SOC_DAPM_AIF_IN("PCM0.0 RX", "PORTA Playback fastplayback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("PCM0.1 RX", "PORTA Playback fastplayback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("PCM0.2 RX", "PORTA Playback fastplayback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("PCM0.3 RX", "PORTA Playback fastplayback", 0,
			SND_SOC_NOPM, 1, 0),

	SND_SOC_DAPM_AIF_IN("PCM1.0 RX", "PORTB Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("PCM1.1 RX", "PORTB Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("PCM1.2 RX", "PORTB Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("PCM1.3 RX", "PORTB Playback", 0,
			SND_SOC_NOPM, 1, 0),

	SND_SOC_DAPM_AIF_IN("PCM2.0 RX", "PORTC Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("PCM2.1 RX", "PORTC Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("PCM2.2 RX", "PORTC Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("PCM2.3 RX", "PORTC Playback", 0,
			SND_SOC_NOPM, 1, 0),

	SND_SOC_DAPM_AIF_OUT("PCM0.0 TX", "PORTA Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("PCM0.1 TX", "PORTA Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("PCM0.2 TX", "PORTA Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("PCM0.3 TX", "PORTA Capture", 0,
			SND_SOC_NOPM, 1, 0),

	SND_SOC_DAPM_AIF_OUT("PCM1.0 TX", "PORTB Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("PCM1.1 TX", "PORTB Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("PCM1.2 TX", "PORTB Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("PCM1.3 TX", "PORTB Capture", 0,
			SND_SOC_NOPM, 1, 0),

	SND_SOC_DAPM_AIF_OUT("PCM2.0 TX", "PORTC Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("PCM2.1 TX", "PORTC Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("PCM2.2 TX", "PORTC Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("PCM2.3 TX", "PORTC Capture", 0,
			SND_SOC_NOPM, 1, 0),

	SND_SOC_DAPM_AIF_IN("SBUS.RX0", "SLIM_PORT-1 Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("SBUS.RX1", "SLIM_PORT-1 Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("SBUS.RX2", "SLIM_PORT-2 Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("SBUS.RX3", "SLIM_PORT-2 Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("SBUS.RX4", "SLIM_PORT-3 Playback", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_IN("SBUS.RX5", "SLIM_PORT-3 Playback", 0,
			SND_SOC_NOPM, 1, 0),
	...
	SND_SOC_DAPM_AIF_OUT("SBUS.TX0", "SLIM_PORT-1 Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("SBUS.TX1", "SLIM_PORT-1 Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("SBUS.TX2", "SLIM_PORT-2 Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("SBUS.TX3", "SLIM_PORT-2 Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("SBUS.TX4", "SLIM_PORT-3 Capture", 0,
			SND_SOC_NOPM, 1, 0),
	SND_SOC_DAPM_AIF_OUT("SBUS.TX5", "SLIM_PORT-3 Capture", 0,
			SND_SOC_NOPM, 1, 0),
