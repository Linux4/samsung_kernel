...
//#define NINGLEI_GPIO_NOT_USE_KERNEL_FRAME //es755.c also have one (total two)
...
void escore_gpio_reset(struct escore_priv *escore)
{
	if (escore->pdata->reset_gpio == -1) {
		...
		pr_debug("escore->pdata->reset_gpio=%#x\n", escore->pdata->reset_gpio);
	#ifdef NINGLEI_GPIO_NOT_USE_KERNEL_FRAME
		// gpio 31
		writel_relaxed(0x0,SPRD_GPIO_BASE+0x80);
		...
		pr_debug("after gpio_set_value=0, escore->pdata->reset_gpio=%d\n", escore->pdata->reset_gpio);
		/* Wait 1 ms then pull Reset signal in High */
		usleep_range(1000, 1050);
		gpio_set_value(escore->pdata->reset_gpio, 1);
		/* Wait 10 ms then */
		pr_debug("after gpio_set_value=1, escore->pdata->reset_gpio=%d\n", escore->pdata->reset_gpio);
