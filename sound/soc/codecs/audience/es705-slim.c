			       ...
			       unsigned int ch_cnt)
{
	...
	if (rc < 0) {
		dev_err(&sbdev->dev, "%s(): slim_control_ch() failed try one more: %d\n",
			__func__, rc);
		msleep(200);
		rc = slim_control_ch(sbdev, grph, SLIM_CH_REMOVE, true);
		if (rc < 0) {
...
int es705_remote_cfg_slim_rx(int dai_id)
{
	...
	dev_info(&sbdev->dev, "%s(): es70x rx1(%d) rx2(%d) tx1(%d) tx2(%d)\n",
		__func__, 
...
int es705_remote_cfg_slim_tx(int dai_id)
{
	...
	dev_info(&sbdev->dev, "%s(): Define dai_id = %d\n", __func__, dai_id);
...
int es705_remote_close_slim_tx(int dai_id)
{
	...
	if ((dai_id == ES705_SLIM_1_CAP) || (dai_id == ES705_SLIM_2_CAP)) {
 	...
 	if ((dai_id == ES705_SLIM_2_CAP) && (es705_priv.rx2_route_enable==0))
			dai_id = ES705_SLIM_1_CAP;
 	else if ((dai_id == ES705_SLIM_2_CAP) && ((es705_priv.rx2_route_enable==1)&&(es705_priv.tx2_route_enable==1)))
			dai_id = ES705_SLIM_1_CAP;
#else
 	if ((dai_id == ES705_SLIM_2_CAP) && (es705_priv.rx2_route_enable==0))
			dai_id = ES705_SLIM_1_CAP;
	...
	dev_info(&sbdev->dev, "%s(): Remove dai_id = %d\n", __func__, dai_id);
