...
//#define ES755_BOOTUP_WHILE_1
//#define NINGLEI_GPIO_NOT_USE_KERNEL_FRAME  //escore.c also have one (total two)

//#define use_gpio_request4gpio_reset //for it may release when i2c device failed
				     ...
				     char *buf)
{
	...
	value = escore_read(escore, ES_FW_FIRST_CHAR);
	*verbuf++ = (value & 0x00ff);
	for (idx = 0; idx < (SIZE_OF_VERBUF-2); idx++) {
		value = escore_read(escore, ES_FW_NEXT_CHAR);
		*verbuf++ = (value & 0x00ff);
		if (!value)
			break;
	}
		...
		char *buf)
{
		...
		printk(KERN_EMERG "es755_reset_control_show reset_gpio =%d\n", es755->pdata->reset_gpio);
...
//char* reordered_standard;
#define TEST_SPI_STABILITY 0
static int es755_fw_download(struct escore_priv *es755)
{
	int rc;
#if TEST_SPI_STABILITY
	int i;
	int downloading_err =0;
	int downloading_ok = 0;
#endif
	pr_debug("%s()\n", __func__);
	...
	if (rc != 0)
			...
			downloading_ok++;
	pr_err("%s() bus specific boot finish status %d, error = %d, success = %d\n",
       __func__, rc, downloading_err, downloading_ok);
	#else
		if (rc != 0) {
...
int es755_bootup(struct escore_priv *es755)
{
	...
	do {
	#ifdef ES755_BOOTUP_WHILE_1
		pr_debug("sleep 1s\n");
		msleep(1000);
		pr_debug("after sleep 1000\n");
		escore_gpio_reset(es755);
	#else
		if (retry < ES755_FW_DOWNLOAD_MAX_RETRY)
		{
			pr_debug("before escore_gpio_reset\n");
			escore_gpio_reset(es755);
			pr_debug("after escore_gpio_reset\n");
		}
	#endif
...
static int es755_slim_hw_params(struct snd_pcm_substream *substream,
			       ...
			       struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE &&
			!escore->slim_dai_data[id].ch_act) {

		rc = escore_cfg_slim_tx(escore->gen0_client,
					escore->slim_dai_data[id].ch_num,
					escore->slim_dai_data[id].ch_tot,
					escore->slim_dai_data[id].rate);
		if (rc < 0) {
			pr_err("%s(): SBUS channel configuration failed %d\n",
					__func__, rc);
			return rc;
		}
		escore->slim_dai_data[id].ch_act++;
...
static void es755_stop_route(struct escore_priv *escore)
{
	int rc = 0;
	...
	if (atomic_read(&escore->active_streams)) {
		if (atomic_dec_and_test(&escore->active_streams)) {
			...
			rc = es_d300_stop_route(escore);
			if (rc)
				pr_err("%s: stop algo failed %d\n",
						__func__, rc);
...
static void es755_slim_shutdown(struct snd_pcm_substream *substream,
			       struct snd_soc_dai *dai)
{
	struct snd_soc_codec *codec = dai->codec;
	struct escore_priv *escore = snd_soc_codec_get_drvdata(codec);
	...
	if (id >= 0 && escore->slim_dai_data[id].ch_act) {
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			rc = escore_close_slim_rx(escore->gen0_client,
					escore->slim_dai_data[id].ch_num,
					escore->slim_dai_data[id].ch_tot);
			if (rc) {
				pr_err("%s(): SBUS channel close failed %d\n",
						__func__, rc);
				return;
			}
			es755_stop_route(escore);
	...
	memset(escore->slim_dai_data[id].ch_num, 0,
		(sizeof(u32)*escore->slim_dai_data[id].ch_tot));
	escore->slim_dai_data[id].ch_tot = 0;
	escore->slim_dai_data[id].ch_act = 0;
...
#if 0
static int is_earsmart_porta_set_fmt = 0;
static int is_earsmart_portb_set_fmt = 0;
#endif
static int es755_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	#if 0
	mutex_lock(&escore->access_lock);
	if(!strncmp(dai->name, "earSmart-porta", 14)){
		if(0 == is_earsmart_porta_set_fmt){
			is_earsmart_porta_set_fmt = 1;
		}else{
			pr_info("vbc %s earSmart-porta has been executed so ignore this call.\n", __func__);
			mutex_unlock(&escore->access_lock);
			return 0;
		}
	}
	if(!strncmp(dai->name, "earSmart-portb", 14)){
		if(0 == is_earsmart_portb_set_fmt){
			is_earsmart_portb_set_fmt = 1;
		}else{
			pr_info("vbc %s earSmart-portb has been executed so ignore this call.\n", __func__);
			mutex_unlock(&escore->access_lock);
			return 0;
		}
	}
...
#if 0
static int is_earsmart_porta_hw_params = 0;
static int is_earsmart_portb_hw_params = 0;
#endif
			       ...
			       struct snd_soc_dai *dai)
{
	...
	mutex_lock(&escore->access_lock);
	if(!strncmp(dai->name, "earSmart-porta", 14)){
		if(0 == is_earsmart_porta_hw_params){
			is_earsmart_porta_hw_params = 1;
			pr_info("vbc %s earSmart-porta called.\n", __func__);
		}else{
			pr_info("vbc %s earSmart-porta has been executed so ignored.\n", __func__);
			mutex_unlock(&escore->access_lock);
			return 0;
		}
	}
	if(!strncmp(dai->name, "earSmart-portb", 14)){
		if(0 == is_earsmart_portb_hw_params){
			is_earsmart_portb_hw_params = 1;
			pr_info("vbc %s earSmart-portb called\n", __func__);
		}else{
			pr_info("vbc %s earSmart-portb has been executed so ignored.\n", __func__);
			mutex_unlock(&escore->access_lock);
			return 0;
		}
	}
...
//*******************************************************************
	//Set the bps as 24bit mode, by Richard Ye
	bps = 0x17;
	dev_err(codec->dev, "%s(): format :%d, bps =%x\n",
		__func__, params_format(params), bps);
//*******************************************************************
	if (bps) {
		...
		rc = escore_write_locked(escore, ES_PORT_WORD_LEN, bps);
		if (rc) {
	...
	rc = escore_write_locked(escore, ES_PORT_CLOCK_CONTROL, clock_control);
	if (rc) {
		pr_err("%s(): Preparing write message failed %d\n",
		       __func__, rc);
		return rc;
...
//**************************************************************************
	//Set the duration as 24bits, by Richard Ye
	api_access = &escore->api_access[ES_PORT_FS_DURATION];
	api_access->write_msg[0] |=  ES_API_WORD(ES_SET_DEV_PARAM_ID,
			(pcm_port[id] << 8));
	clock_control = 24;
	rc = escore_write_locked(escore, ES_PORT_FS_DURATION, clock_control);
	/* Clear the Port info in write command */
	api_access->write_msg[0] &= ES_API_WORD(ES_SET_DEV_PARAM_ID, 0x00ff);
//****************************************************************************
...
struct snd_soc_dai_driver es755_dai[] = {
	...
	{
		.name = "earSmart-porta_offload",
		.id = ES_I2S_PORTA,
		.playback = {
			.stream_name = "PORTA Playback offload",
			.channels_min = 1,
			.channels_max = 2,
			.rates = ES_RATES,
			.formats = ES_FORMATS,
		...
		.capture = {
			.stream_name = "PORTA Capture offload",
			.channels_min = 1,
			.channels_max = 2,
			.rates = ES_RATES,
			.formats = ES_FORMATS,
	...
	{
		.name = "earSmart-porta-voice",
		.id = ES_I2S_PORTA,
		.playback = {
			.stream_name = "PORTA Playback voice",
			.channels_min = 1,
			.channels_max = 2,
			.rates = ES_RATES,
			.formats = ES_FORMATS,
		...
		.capture = {
			.stream_name = "PORTA Capture voice",
			.channels_min = 1,
			.channels_max = 2,
			.rates = ES_RATES,
			.formats = ES_FORMATS,
	...
	{
		.name = "earSmart-porta-fastplay",
		.id = ES_I2S_PORTA,
		.playback = {
			.stream_name = "PORTA Playback fastplayback",
			.channels_min = 1,
			.channels_max = 2,
			.rates = ES_RATES,
			.formats = ES_FORMATS,
	...
	},
	{
		.name = "null-codec",
		.playback = {
			.stream_name = "playback null-codec",
			.channels_min = 1,
			.channels_max = 2,
			.rates = ES_RATES,
			.formats = ES_FORMATS,
		...
		.capture = {
			.stream_name = "capture null-codec",
			.channels_min = 1,
			.channels_max = 2,
			.rates = ES_RATES,
			.formats = ES_FORMATS,
...
int es755_start_int_osc(void)
{
	...
	/* Start internal Osc. */
	rc = escore_write(&escore_priv, ES_VS_INT_OSC_MEASURE_START, 0);
	if (rc) {
	...
	do {
		 ...
		 */
		msleep(20);
		rc = escore_read(&escore_priv, ES_VS_INT_OSC_MEASURE_STATUS);
				...
				unsigned int set_power_state_cmd)
{
	...
	while (next_power_state != escore_priv.escore_power_state) {
		...
		case ES_SET_POWER_STATE_NORMAL:
			...
			rc = escore_write(&escore_priv, set_power_state_cmd,
					escore_priv.escore_power_state);
			if (rc) {
				dev_err(escore_priv.dev,
...
static int es755_get_reroute(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct escore_priv *escore = snd_soc_codec_get_drvdata(codec);

	ucontrol->value.enumerated.item[0] = escore->rerouted;

	pr_debug("%s(): Reroute Get:%d\n", __func__, escore->rerouted);
	return 0;
}

static int es755_put_reroute(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct escore_priv *escore = snd_soc_codec_get_drvdata(codec);
	...
	pr_debug("%s(): Reroute Put:%d\n", __func__, value);

	if (value && atomic_read(&escore->active_streams))
		ret = es_d300_set_route(escore);

	escore->rerouted = value;
...
int g_test_val = 0;
static int es755_get_portB_clock(struct snd_kcontrol *kcontrol,
			      struct snd_ctl_elem_value *ucontrol)
{
	return g_test_val;
}

static int es755_put_portB_clock(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct escore_priv *escore = snd_soc_codec_get_drvdata(codec);
	...
	u32 cmd, resp;
	pr_info("%s(): Enable PortB Clock Put:%d\n", __func__, value);
	g_test_val = value;
	if (value)
	                cmd = 0x805201C4;
	else
...
static struct snd_kcontrol_new es755_snd_controls[] = {
	SOC_SINGLE_EXT("Enable PortB Clock", SND_SOC_NOPM, 0, 1, 0, es755_get_portB_clock,
             es755_put_portB_clock),
	...
	SOC_SINGLE_EXT("Reroute", SND_SOC_NOPM, 0, 1, 0, es755_get_reroute,
		       es755_put_reroute),
};
...
static int es755_codec_probe(struct snd_soc_codec *codec)
{
	...
	#if 0
			{
				unsigned short gpio_value_1;
				gpio_value_1 = readl_relaxed((void *)(SPRD_GPIO_BASE+0x380+0x18));
				printk(KERN_ERR "%s, 3 readl gpio int:0x%x\n", __func__, gpio_value_1);
				gpio_value_1 |= BIT(4);
				printk(KERN_ERR "%s, 3 after changing, gpio int:0x%x \n", __func__, gpio_value_1);
				writel_relaxed(gpio_value_1,SPRD_GPIO_BASE+0x380+0x18);
			}
	#endif
	return ret;
}
				...
				unsigned int reg)
{
	...
	ret = escore_write(escore, ES_CODEC_ADDR, reg);
	if (ret < 0) {
	...
	value = escore_read(escore, ES_CODEC_VALUE);
	rc = value;
	if (value < 0) {
	...
	unsigned int value)
{
	...
	ret = escore_write(escore, ES_CODEC_VALUE, value);
	if (ret < 0) {
...
#ifdef NINGLEI_GPIO_NOT_USE_KERNEL_FRAME
	pr_err("init gpio reset NINGLEI_GPIO_NOT_USE_KERNEL_FRAME\n");
	writel_relaxed(0x8000,SPRD_GPIO_BASE+0x88);
	writel_relaxed(0x8000,SPRD_GPIO_BASE+0x84);
	writel_relaxed(0x8000,SPRD_GPIO_BASE+0x80);
#endif
...
pdata->gpiob_gpio = of_get_named_gpio(dev->of_node,
	...
	dev_dbg(dev, "%s: gpiob_gpio %d", __func__, pdata->gpiob_gpio);
*/
#ifdef USE_clk_aux1_es755_populate_dt_pdata

		if (IS_ERR(codec_clk)) {
			dev_err(dev, "%s: Failed to request clk_aux1 from pmic %ld\n",
...
static int es755_mic_config(struct escore_priv *escore)
{
	...
	pr_debug("%s()\n", __func__);
	accdet_reg.fields.mg_select = 1;
	pr_info("%s(), value =%x\n", __func__, accdet_reg.value);
	return escore_write_locked(escore, ES_ACCDET_CONFIG, accdet_reg.value);
}
...
static int es755_button_config(struct escore_priv *escore)
{
	...
	/* Read default value */
	rc = escore_read(escore, ES_BUTTON_CTRL1);
	if (rc < 0) {
		dev_err(escore_priv.dev,
	...
	rc = escore_write(escore, ES_BUTTON_CTRL1, btn_ctl1.value);
	if (rc < 0)
	...
	struct escore_priv *escore, int sys_intr_resp)
{
	...
	if (ES_BUTTON_PRESS_EVENT(sys_intr_resp)) {
		...
		value = escore_read_locked(escore, ES_GET_ACCDET_STATUS);
		if (value < 0) {
		...
		void *dev)
{
	...
	if (action & ES_CODEC_INTR_EVENT) {
		sys_intr_resp = escore_read_locked(escore,
		...
		if (ES_MICDET_EVENT(sys_intr_resp)) {
			...
			value = escore_read_locked(escore,
...
irqreturn_t es755_irq_work(int irq, void *data)
{
	...
	if (!escore || !escore->fw_downloaded) {
...
int es755_core_probe(struct device *dev)
{
...
#ifdef	use_gpio_request4gpio_reset
		pr_debug("gpio_request reset");
	...
	escore_priv.dai_nr = (sizeof(es755_dai)/sizeof(es755_dai[0]));
	...
	#else
	escore_priv.i2s_dai_ops.shutdown = NULL;
	#endif
#endif

#if defined(CONFIG_SND_SOC_ES_SLIM)
	escore_priv.slim_dai_ops.hw_params = es755_slim_hw_params;
	escore_priv.slim_dai_ops.shutdown = es755_slim_shutdown;
#endif
	...
	printk(KERN_ERR "%s, gpio_b_irq_type : 0x%x \n", __func__, pdata->gpio_b_irq_type);
