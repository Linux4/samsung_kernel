...
static int dhwpt_is_enabled = 0;
#define RESET_DHWPT		0x80520000
...
int restore_std_fw(struct es705_priv *es705)
{
	...
	if (es705->pdata->use_dhwpt_in_sbl) {
		dhwpt_mode |= (es705->pdata->dhwpt_mode & 0xff);
		rc = es705_write_then_read(es705, &dhwpt_mode,
			sizeof(dhwpt_mode), &rspn, 0);
		dhwpt_is_enabled = 0;
		dev_info(es705->dev, "%s(): dhwpt mode[0x%x] rspn[ 0x%x]\n",
			__func__, dhwpt_mode, rspn);
		if (rc < 0)
			dev_err(es705->dev, "%s():Failed DHWPT\n", __func__);
		else
			dhwpt_is_enabled = 1;
	}
	...
	if (blocks) {
		...
		if (es705->rdb_wdb_open) {
			...
			for (rem = es705->vs_keyword_param_size; rem > 0;
				...
				if (ret < 0) {
					dev_err(es705->dev,
						"%s(): v-s WDB write over SLIMBus FAIL, offset=%d\n",
						__func__,
						dptr - es705->vs_keyword_param);
					goto EXIT;
				}
		...
		if (onoff) {
			if (es705->mode == SBL) {
				if (es705->pdata->esxxx_clk_cb) {
					es705->pdata->esxxx_clk_cb(es705->dev,
							1);
			...
			} else if (es705->es705_power_state
			...
			rc = es705_write_then_read(es705, &dhwpt_mode,
				   sizeof(dhwpt_mode), &rspn, 0);
			dev_info(es705->dev,
				"%s(): dhwpt mode is 0x%x, rspn is  0x%x\n",
				__func__, dhwpt_mode, rspn);
			if (rc < 0) {
				dev_err(es705->dev,
						"%s():Failed DHWPT\n",
...
static int es705_chk_route_id_for_dhwpt(long route_index)
{
	...
	if (es705->pdata->use_dhwpt) {
		...
		case ROUTE_CT_LOOPBACK:
		case ROUTE_FT_LOOPBACK:
		case ROUTE_HS_LOOPBACK:
			rc = es705_enable_dhwpt(0);
			if (rc) {
				if (es705->pdata->esxxx_clk_cb) {
					es705->pdata->esxxx_clk_cb(es705->dev,
							1);
...
static void es705_switch_vs_streaming_to_normal_mode(struct es705_priv *es705)
{
	...
	if (es705->pdata->esxxx_clk_cb) {
		...
		es705->pdata->esxxx_clk_cb(es705, 1);
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	dev_dbg(es705->dev, "%s(): rate=%d\n", __func__, es705->internal_rate);
	return snprintf(buf, PAGE_SIZE, "rate=%d\n", es705->internal_rate);
}
					 ...
					 const char *buf, size_t count)
{
	...
	if (value == ROUTE_MAX) {
		...
		if (es705->pdata->use_dhwpt_in_sbl)
			es705_enable_dhwpt(1);
		else if (es705->pdata->use_dhwpt && dhwpt_is_enabled)
			es705_enable_dhwpt(0);
...
static int es705_set_lpm(struct es705_priv *es705)
{
	...
	if (es705_priv.pdata->esxxx_clk_cb) {
		/* ext clock off */
		es705_priv.pdata->esxxx_clk_cb(&es705_priv, 0);
...
static void es705_std_fw_loaded(const struct firmware *fw, void *context)
{
	...
	if (es705->pdata->use_dhwpt_in_sbl) {
		...
		rc = es705_write_then_read(es705, &dhwpt_mode,
			sizeof(dhwpt_mode), &rspn, 0);
		dev_info(es705->dev, "%s(): dhwpt mode is 0x%x, rspn is  0x%x\n",
			__func__, dhwpt_mode, rspn);
		dhwpt_is_enabled = 0;
		if (rc < 0)
			dev_err(es705->dev, "%s():Failed DHWPT\n", __func__);
...
err:
	if (es705->pdata->esxxx_clk_cb) {
		es705->pdata->esxxx_clk_cb(es705, 0);
...
int es705_core_init(struct device *dev)
{
	...
	if (!es705_get_stimulate_status()) {
		if (pdata->esxxx_clk_cb) {
			pdata->esxxx_clk_cb(&es705_priv, 1);
