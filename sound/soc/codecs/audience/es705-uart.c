...
static int es705_uart_boot_setup(struct es705_priv *es705)
{
	...
	rc = es705_uart_write_then_read(es705, &sbl_sync_cmd, 1,
					&rspn, match);
	if (rc < 0) {
		dev_err(es705->dev, "%s(): UART baud rate detection fail %d\n",
			__func__, rc);
	...
	if (rc < 0) {
		dev_err(es705->dev, "%s(): uart baud rate set error %d\n",
			__func__, rc);
		goto es705_bootup_failed;
	}
	...
	rc = es705_uart_write_then_read(es705, &sbl_sync_cmd, 1,
					&rspn, match);
	if (rc < 0) {
		dev_err(es705->dev, "%s(): UART FW Download SBL SYNC fail %d\n",
			__func__, rc);
		goto es705_bootup_failed;
	}
	dev_dbg(es705->dev, "%s(): sbl sync ack = 0x%02x\n", __func__, rspn);
	...
	rc = es705_uart_write_then_read(es705, &sbl_boot_cmd, 1,
					&rspn, match);
	if (rc < 0) {
		dev_err(es705->dev, "%s(): UART FW Download BOOT CMD fail %d\n",
			__func__, rc);
		goto es705_bootup_failed;
	}
