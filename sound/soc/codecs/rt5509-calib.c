...
void rt5509_calib_destroy(struct rt5509_chip *chip)
{
	...
	dev_dbg(chip->dev, "%s\n", __func__);
	mutex_destroy(&pcalib_dev->var_lock);
...
int rt5509_calib_create(struct rt5509_chip *chip)
{
	...
	mutex_init(&pcalib_dev->var_lock);
...
static struct device_attribute rt5509_dev_attrs[] = {
	__ATTR(n20db, 0440, rt_calib_dev_attr_show, rt_calib_dev_attr_store),
	__ATTR(n15db, 0440, rt_calib_dev_attr_show, rt_calib_dev_attr_store),
	__ATTR(n10db, 0440, rt_calib_dev_attr_show, rt_calib_dev_attr_store),
	__ATTR(gsense_otp, 0440, rt_calib_dev_attr_show,
	       rt_calib_dev_attr_store),
	__ATTR(rapp, 0440, rt_calib_dev_attr_show, rt_calib_dev_attr_store),
	__ATTR(rspk, 0660, rt_calib_dev_attr_show, rt_calib_dev_attr_store),
	__ATTR(calib_data, 0440, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(dcr_offset, 0440, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(calibrated, 0440, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(chip_rev, 0440, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(rspkmin, 0640, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(rspkmax, 0640, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(alphaspk, 0640, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(t, 0440, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(tmax, 0440, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(xpeak, 0440, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(xmax, 0440, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(xmax_count, 0440, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(tmax_count, 0440, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR(int_flag, 0440, rt_calib_dev_attr_show,
		rt_calib_dev_attr_store),
	__ATTR_NULL,
};
...
static struct attribute *rt5509_cal_dev_attrs[] = {
	...
	&rt5509_dev_attrs[14].attr,
	&rt5509_dev_attrs[15].attr,
	&rt5509_dev_attrs[16].attr,
	&rt5509_dev_attrs[17].attr,
	&rt5509_dev_attrs[18].attr,
	&rt5509_dev_attrs[19].attr,
	NULL,
};
...
enum {
	...
	RT5509_CALIB_DEV_T,
	RT5509_CALIB_DEV_TMAX,
	RT5509_CALIB_DEV_XPEAK,
	RT5509_CALIB_DEV_XMAX,
	RT5509_CALIB_DEV_XMAXCOUNT,
	RT5509_CALIB_DEV_TMAXCOUNT,
	RT5509_CALIB_DEV_INTFLAG,
		...
		struct device_attribute *attr, char *buf)
{
	...
	case RT5509_CALIB_DEV_ALPHASPK:
		ret = scnprintf(buf, PAGE_SIZE, "%d\n", calib_dev->alphaspk);
		break;
	case RT5509_CALIB_DEV_T:
		mutex_lock(&calib_dev->var_lock);
		ret = scnprintf(buf, PAGE_SIZE, "%d.%01d\n", calib_dev->t / 10,
				(int32_t)abs(calib_dev->t) % 10);
		mutex_unlock(&calib_dev->var_lock);
		break;
	case RT5509_CALIB_DEV_TMAX:
		mutex_lock(&calib_dev->var_lock);
		ret = scnprintf(buf, PAGE_SIZE, "%d.%01d\n",
				calib_dev->tmax / 10,
				(int32_t)abs(calib_dev->tmax) % 10);
		calib_dev->tmax = calib_dev->tmax_raw = 0;
		mutex_unlock(&calib_dev->var_lock);
		break;
	case RT5509_CALIB_DEV_XPEAK:
		mutex_lock(&calib_dev->var_lock);
		ret = scnprintf(buf, PAGE_SIZE, "%d.%03d\n",
				calib_dev->xpeak / 1000, calib_dev->xpeak % 1000);
		mutex_unlock(&calib_dev->var_lock);
		break;
	case RT5509_CALIB_DEV_XMAX:
		mutex_lock(&calib_dev->var_lock);
		ret = scnprintf(buf, PAGE_SIZE, "%d.%03d\n",
				calib_dev->xmax / 1000, calib_dev->xmax % 1000);
		calib_dev->xmax = calib_dev->xmax_raw = 0;
		mutex_unlock(&calib_dev->var_lock);
		break;
	case RT5509_CALIB_DEV_XMAXCOUNT:
		mutex_lock(&calib_dev->var_lock);
		ret = scnprintf(buf, PAGE_SIZE, "%d\n", calib_dev->xmax_count);
		calib_dev->xmax_count = 0;
		mutex_unlock(&calib_dev->var_lock);
		break;
	case RT5509_CALIB_DEV_TMAXCOUNT:
		mutex_lock(&calib_dev->var_lock);
		ret = scnprintf(buf, PAGE_SIZE, "%d\n", calib_dev->tmax_count);
		calib_dev->tmax_count = calib_dev->tmax_overcount = 0;
		mutex_unlock(&calib_dev->var_lock);
		break;
	case RT5509_CALIB_DEV_INTFLAG:
		ret = snd_soc_read(chip->codec, RT5509_REG_INTERRUPT);
		if (ret < 0) {
			dev_err(chip->dev, "read int flag fail\n");
			return ret;
		}
		if (!chip->amp_on)
			ret = 0;
