...
#ifdef TFA9874_NONDSP_STEREO//bug 458252,zhanghao1,20190716,add for update NXP SmartPA code
...
#endif//bug 458252,zhanghao1,20190716,add for update NXP SmartPA code
...
//+bug 458252,zhanghao1,20190716,add for RESOURCE_LEAK
...
static int tfa98xx_dbgfs_pga_gain_get(void *data, u64 *val)
{
	struct i2c_client *i2c = (struct i2c_client *)data;
	struct tfa98xx *tfa98xx = i2c_get_clientdata(i2c);
	int value;//bug 458252,zhanghao1,20190716,modify for NO_EFFECT
...
retry:
	...
	if (0)//bug 458252,zhanghao1,20190716,modify for close log
		dev_err(&tfa98xx->i2c->dev, " WR reg=0x%02x, val=0x%04x\n",
		        subaddress, value);//bug 458252,zhanghao1,20190716,delete for DEADCODE
	...
	*val = value & 0xffff;
	...
	if (0)//bug 458252,zhanghao1,20190716,modify for close log
		dev_err(&tfa98xx->i2c->dev, "RD   reg=0x%02x, val=0x%04x\n",
		        subaddress, *val);//bug 458252,zhanghao1,20190716,delete for DEADCODE
	if (tfa98xx_ftrace_regs)
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	*/
	...
	if (tfa98xx->tfa->tfa_family == 2 &&
              tfa98xx->tfa->is_probus_device == 0) {//bug 458252,zhanghao1,20190716,modify for update smartPA code,pop when turn on
		mutex_lock(&tfa98xx->dsp_lock);
		ret = tfa98xx_tfa_start(tfa98xx, tfa98xx->profile, tfa98xx->vstep);
...
//+bug 458252,zhanghao1,20190716,modify for update NXP SmartPA code
#ifdef TFA9874_NONDSP_STEREO
extern int send_tfa_cal_in_band(void *buf, int cmd_size);
...
//+bug 458252,zhanghao1,20190716,add for update NXP SmartPA code
                	/* Start DSP */
...
//+bug 458252,zhanghao1,20190716,delete for audio ATA :hsmic_speaker
...
//+bug 458252,zhanghao1,20190716,add for audio ftm smartPA calibration bringup
static int cali_value = 0;
static ssize_t tfa98xx_calibration_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	int ret= cali_value;
...
static ssize_t tfa98xx_calibration_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t size)
{
	int input = -1;
...
static struct device_attribute dev_attr_cali = {
	.attr = {
		.name = "cali",
		.mode = S_IRUSR | S_IWUSR
	},
	.show = tfa98xx_calibration_show,
	.store = tfa98xx_calibration_store,
};
//-bug 458252,zhanghao1,20190716,add for audio ftm smartPA calibration bringup
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (ret)
		dev_info(&i2c->dev, "error creating sysfs files\n");
//+bug 458252,zhanghao1,20190716,add for audio ftm smartPA calibration bringup
		ret = device_create_file(&i2c->dev, &dev_attr_cali);
	if (ret)
		dev_info(&i2c->dev, "error creating sysfs files\n");
//-bug 458252,zhanghao1,20190716,add for audio ftm smartPA calibration bringup
...
static int tfa98xx_i2c_remove(struct i2c_client *i2c)
{
	...
	device_remove_file(&i2c->dev, &dev_attr_cali);//bug 458252,zhanghao1,20190716,add for audio ftm smartPA calibration bringup
#ifdef CONFIG_DEBUG_FS
