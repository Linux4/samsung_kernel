...
static void get_26m_clk()
{
	struct clk *wlan_clk;
	struct clk *clk_parent;
	wlan_clk = clk_get(NULL, "clk_aux1");
	if (IS_ERR(wlan_clk)) {
		pr_debug("clock: failed to get clk_aux1\n");
	}
	pr_debug("after clk_get clk_aux1\n");
	clk_parent = clk_get(NULL, "ext_26m");
	if (IS_ERR(clk_parent)) {
		pr_debug("failed to get parent ext_26m:\n");
	}
	clk_set_parent(wlan_clk, clk_parent);
	clk_set_rate(wlan_clk, 26000000);
