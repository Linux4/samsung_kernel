...
#define SKIP_AIF2_MUTE_WHEN_AIF3_USED	1
...
static int rt5511_write(struct snd_soc_codec *codec, u32 reg, u32 value)
{
	...
	dev_dbg(codec->dev, "write 0x%x to Reg[0x%x], size = %d\n",
		value, reg, size);
				   ...
				   unsigned int freq_out)
{
	...
	if (mode == RT5511_CKMODE_22_579200)
		regval = RT5511_CKMODE_BASE_44_1K << RT5511_SSRC_CKMODE_SHIFT;
	else if (mode == RT5511_CKMODE_24_576000)
		regval = RT5511_CKMODE_BASE_48_0K << RT5511_SSRC_CKMODE_SHIFT;

	return snd_soc_update_bits(codec, RT5511_ASRC_CTRL1,
				RT5511_SSRC_CKMODE_MASK, regval);
}
...
static const struct soc_enum bypass_cmd_mode_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(onoff_mode_text), onoff_mode_text),
};

static int get_bypass_cmd_mode(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
static int set_bypass_cmd_mode(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
/* Special Control */
	SOC_ENUM_EXT("BypassCmd", bypass_cmd_mode_enum[0],
		get_bypass_cmd_mode, set_bypass_cmd_mode),
};
...
static const struct snd_kcontrol_new rt5511A_tri_state_controls[] = {
...
enum rt_aif1_adc_out_sel {
	...
	RT_AIF1_ADC_OUT_SEL_RSVD1 = 5,
	RT_AIF1_ADC_OUT_SEL_RSVD2 = 6,
	RT_AIF1_ADC_OUT_SEL_MAX = 7,
};

#define RT_AIF1_ADC_OUT_SEL_MASK	(0x03)

static const char * const aif1_adc_out_mux_text[] = {
	"ADC", "DAC", "Mix", "ADC+", "DAC+", "RSVD1", "RSVD2",
};

/* Real maximum value is 3, not 7 */
static const struct soc_enum aif1_adcl_out_mux_enum =
	SOC_ENUM_SINGLE(RT5511_BASS_3D_EN, 6, 7, aif1_adc_out_mux_text);

static const struct soc_enum aif1_adcr_out_mux_enum =
	SOC_ENUM_SINGLE(RT5511_BASS_3D_EN, 4, 7, aif1_adc_out_mux_text);
	...
	struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.enumerated.item[0] &= RT_AIF1_ADC_OUT_SEL_MASK;
	return ret;
}
	...
	struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	extraval = retval & (~RT_AIF1_ADC_OUT_SEL_MASK);
	retval = retval & RT_AIF1_ADC_OUT_SEL_MASK;

	switch (val) {
	case RT_AIF1_ADC_OUT_SEL_ADC_PLUS:
		if (retval != RT_AIF1_ADC_OUT_SEL_ADC)
			retval = (RT_AIF1_ADC_OUT_SEL_MIX);
		break;
	case RT_AIF1_ADC_OUT_SEL_DAC_PLUS:
		if (retval != RT_AIF1_ADC_OUT_SEL_DAC)
			retval = (RT_AIF1_ADC_OUT_SEL_MIX);
		break;
	case RT_AIF1_ADC_OUT_SEL_RSVD1:
	case RT_AIF1_ADC_OUT_SEL_RSVD2:
		break;
	default:
		retval = val;
		break;
	...
	case SND_SOC_BIAS_PREPARE:
		if (codec->dapm.bias_level == SND_SOC_BIAS_STANDBY) {
			...
			snd_soc_update_bits(codec, RT5511_SOURCE_ENABLE,
				RT5511_PATH2_DAC_EN, RT5511_PATH2_DAC_EN);
#endif
...
static int rt5511_set_tristate(struct snd_soc_dai *codec_dai, int tristate)
{
	...
	return snd_soc_update_bits(codec, RT5511A_AIF_TRI_STATE, mask, val);
}
...
static int rt5511_codec_probe(struct snd_soc_codec *codec)
{
	...
	if (rt5511->revision >= RT5511_VID_5511A) {
		snd_soc_add_controls(codec, rt5511A_tri_state_controls,
				ARRAY_SIZE(rt5511A_tri_state_controls));
...
static int __devinit rt5511_probe(struct platform_device *pdev)
{
	int ret;
...
static int __devexit rt5511_remove(struct platform_device *pdev)
{
...
static struct platform_driver rt5511_codec_driver = {
	...
	.probe = rt5511_probe,
	.remove = __devexit_p(rt5511_remove),
};
