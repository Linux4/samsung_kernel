...
struct samsung_dbmd2_event_priv {
...
static ssize_t svoice_keyword_type_show(struct device *dev,
					...
					char *buf)
{
	struct samsung_dbmd2_event_priv *priv= dev_get_drvdata(dev);
	int ret;
...
void samsung_dbmd2_voicewakeup_cb(int word_id)
{

	struct samsung_dbmd2_event_priv *priv;
...
static int samsung_dbmd2_probe(struct platform_device *pdev)
{
	struct samsung_dbmd2_event_priv *priv;
	int ret;

	priv = devm_kzalloc(&pdev->dev,
			sizeof(struct samsung_dbmd2_event_priv), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;

	/* TO DO
	 * It need dbmd patches to register callback func */
	dbmd2_remote_register_event_callback(samsung_dbmd2_voicewakeup_cb);
...
static int samsung_dbmd2_remove(struct platform_device *pdev)
{
	struct samsung_dbmd2_event_priv *priv = platform_get_drvdata(pdev);
...
static const struct of_device_id samsung_dbmd2_of_match[] = {
	{.compatible = "samsung,dbmd2-event",},
	{},
};
MODULE_DEVICE_TABLE(of, samsung_dbmd2_of_match);

static struct platform_driver samsung_dbmd2_event_driver = {
	.driver = {
		.name = "dbmd2-event",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(samsung_dbmd2_of_match),
	},
	.probe = samsung_dbmd2_probe,
	.remove = samsung_dbmd2_remove,
};

module_platform_driver(samsung_dbmd2_event_driver);

MODULE_DESCRIPTION("SAMSUNG DBMD2 event driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:dbmd2-event");
