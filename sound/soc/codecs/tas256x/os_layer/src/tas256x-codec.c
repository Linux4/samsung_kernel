		...
		unsigned int reg)
{
	...
	dev_info(plat_data->dev, "%s, reg=%d, value=%d", __func__, reg, value);
				...
				unsigned int reg, unsigned int value)
{
	...
	dev_info(plat_data->dev, "%s: %d, %d", __func__, reg, value);
		...
		struct snd_soc_dai *dai)
{
	...
	dev_info(plat_data->dev, "%s, stream %s format: %d\n", __func__,
		(substream->stream ==
	...
	if (channels > 2) {
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
			n_result = tas256x_set_tdm_rx_slot(p_tas256x, channels,
				bitwidth);
		else /*Assumed Capture*/
			n_result = tas256x_set_tdm_tx_slot(p_tas256x, channels,
				bitwidth);
		...
		int slots, int slot_width)
{
	...
	dev_info(plat_data->dev, "%s, tx_mask:%d, rx_mask:%d",
...
static int tas256x_mute_stream(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute && (stream ==
		...
		if (p_tas256x->algo_bypass == 0) {
			...
			tas25xx_set_iv_bit_fomat(p_tas256x->curr_mn_iv_width,
				p_tas256x->curr_mn_vbat, 1);
#endif /* CONFIG_TAS25XX_CALIB_VAL_BIG || CONFIG_TISA_DEBUGFS_INTF*/
		}
	}
#endif /* CONFIG_TAS25XX_ALGO */
		...
		if (conf_no == i) {
			for (j = 0; j < (int)cfg_info[i]->real_nblocks; j++) {
				...
				for (k = 0; k < (int)cfg_info[i]->blk_data[j]->nSublocks; k++) {
					rc = tas256x_process_block(pTAS256x,
...
static struct tas256x_config_info *tas256x_add_config(
	unsigned char *config_data, unsigned int config_size)
{
	...
	for (i = 0; i < (int)cfg_info->nblocks; i++) {
		...
		cfg_info->blk_data[i] = kzalloc(sizeof(struct tas256x_block_data),
									GFP_KERNEL);
...
static void tas256x_fw_ready(const struct firmware *pFW, void *pContext)
{
	...
	fw_hdr->checksum = SMS_HTONL(buf[offset], buf[offset + 1],
					buf[offset + 2], buf[offset + 3]);
	offset += 4;
	fw_hdr->binnary_version_num = SMS_HTONL(buf[offset], buf[offset + 1],
					buf[offset + 2], buf[offset + 3]);
	offset += 4;
...
static const struct snd_soc_codec_driver soc_codec_driver_tas256x_mono = {
...
static const struct snd_soc_codec_driver soc_codec_driver_tas256x_stereo = {
	.probe			= tas256x_codec_probe,
	.remove			= tas256x_codec_remove,
	.read			= tas256x_codec_read,
	.write			= tas256x_codec_write,
...
#endif
	.component_driver = {
		.controls		= tas256x_snd_controls_stereo,
		.num_controls		= ARRAY_SIZE(tas256x_snd_controls_stereo),
		.dapm_widgets		= tas256x_dapm_widgets_stereo,
		.num_dapm_widgets	= ARRAY_SIZE(tas256x_dapm_widgets_stereo),
...
int tas256x_register_codec(struct tas256x_priv *p_tas256x)
{
	...
	if (p_tas256x->mn_channels == 2) {
		n_result = snd_soc_register_codec(plat_data->dev,
			&soc_codec_driver_tas256x_stereo,
			tas256x_dai_driver, ARRAY_SIZE(tas256x_dai_driver));
	} else {
		n_result = snd_soc_register_codec(plat_data->dev,
			&soc_codec_driver_tas256x_mono,
			tas256x_dai_driver, ARRAY_SIZE(tas256x_dai_driver));
	}
	return n_result;
}
