 ...
 *
 * r006, 2020.12.18	- initial version  sma1305
 *
...
static int sma1305_startup(struct snd_soc_codec *);
static int sma1305_shutdown(struct snd_soc_codec *);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (sel && !(sma1305->force_amp_power_down))
		sma1305_startup(codec);
	else
		sma1305_shutdown(codec);
...
/* 0x97~0x9A OTP Trimming 0~3 Set */
...
static int sma1305_startup(struct snd_soc_codec *codec)
{
	struct sma1305_priv *sma1305 = snd_soc_codec_get_drvdata(codec);
...
static int sma1305_shutdown(struct snd_soc_codec *codec)
{
	struct sma1305_priv *sma1305 = snd_soc_codec_get_drvdata(codec);
...
static int sma1305_setup_pll(struct snd_soc_codec *codec,
		unsigned int bclk)
{
	struct sma1305_priv *sma1305 = snd_soc_codec_get_drvdata(codec);
	...
	if (sma1305->sys_clk_id == SMA1305_PLL_CLKIN_MCLK) {
		...
		if (i == sma1305->num_of_pll_matches) {
			dev_info(codec->dev, "%s : No matching value between pll table and SCK\n",
				__func__);
			return -EINVAL;
		}
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		case 96000:
			dev_info(codec->dev, "%s : %d rate not support SDO\n",
		...
		if (sma1305->format == SND_SOC_DAIFMT_DSP_A) {
			...
			default:
			dev_err(codec->dev, "%s not support TDM %d bit\n",
			...
			break;
			default:
			dev_err(codec->dev, "%s not support TDM %d channel\n",
			...
			if (sma1305->tdm_slot_rx < params_channels(params))
				...
				dev_err(codec->dev, "%s Incorrect tdm-slot-rx %d set\n",
	...
	/* substream->stream is SNDRV_PCM_STREAM_CAPTURE */
		...
		if (sma1305->format == SND_SOC_DAIFMT_DSP_A) {
			...
			if (sma1305->tdm_slot_tx < params_channels(params))
				...
				dev_err(codec->dev, "%s Incorrect tdm-slot-tx %d set\n",
...
static int sma1305_set_bias_level(struct snd_soc_codec *codec,
...
#ifdef CONFIG_PM
static int sma1305_suspend(struct snd_soc_codec *codec)
{
	dev_info(codec->dev, "%s\n", __func__);
...
static int sma1305_resume(struct snd_soc_codec *codec)
{
	dev_info(codec->dev, "%s\n", __func__);
...
static int sma1305_reset(struct snd_soc_codec *codec)
{
	struct sma1305_priv *sma1305 = snd_soc_codec_get_drvdata(codec);
	int ret;
	...
	if ((sma1305->otp_trm2 & OTP_STAT_MASK) == OTP_STAT_1)
		dev_info(codec->dev, "SMA1305 OTP Status Successful\n");
	else
		dev_info(codec->dev, "SMA1305 OTP Status Fail\n");
...
static int sma1305_probe(struct snd_soc_codec *codec)
{
...
static int sma1305_remove(struct snd_soc_codec *codec)
{
	struct sma1305_priv *sma1305 = snd_soc_codec_get_drvdata(codec);
	...
	sma1305_set_bias_level(codec, SND_SOC_BIAS_OFF);

	return 0;
}

static const struct snd_soc_codec_driver soc_codec_dev_sma1305 = {
	.probe = sma1305_probe,
...
int sma1305_i2c_probe(struct i2c_client *client,
				const struct i2c_device_id *id)
{
	...
	ret = snd_soc_register_codec(&client->dev,
			&soc_codec_dev_sma1305, sma1305_dai, 1);
...
int sma1305_i2c_remove(struct i2c_client *client)
{
	struct sma1305_priv *sma1305 =
