...
{
	unsigned int reg_val = 0;
	struct aw_i2sen_desc *desc = &aw_dev->i2sen_desc;

	aw_dev->ops.aw_i2c_read(aw_dev, desc->reg, &reg_val);
	if ((reg_val & desc->i2sen_check) == desc->i2sen_check) {
		aw_dev_info(aw_dev->dev, "done");
		return 0;
	} else {
		aw_dev_err(aw_dev->dev, "check i2sen fail with reg_val[0x%x] check_val[0x%x]", reg_val, desc->i2sen_check);
		return -EINVAL;
	}
}
...
int aw_device_start(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		ret = aw_dev_reg_update(aw_dev, true);
		if (ret) {
			aw_dev_err(aw_dev->dev, "fw update failed");
		}
	}
