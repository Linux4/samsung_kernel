	...
	struct snd_soc_dai *dai)
{
	...
	if (aw882xx->startup_cnt == 1) {
		aw882xx->need_start = true;
	}

	aw_dev_info(aw882xx->dev, "%s startup_cnt %d, need_start %d", __func__,
		aw882xx->startup_cnt, aw882xx->need_start);

	if (aw882xx->startup_cnt > 1) {
		aw882xx->mute_ignore = true;
		return 0;
	}

	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		aw_dev_info(aw882xx->dev, "%s playback enter", __func__);
		/*load cali re*/
...
static int aw882xx_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	/*Tab A8 code for P211014-04859  by dongtianbao at 20211025 start*/
	aw_dev_info(codec->dev, "%s fmt=0x%x", __func__, fmt);
	/*Tab A8 code for P211014-04859  by dongtianbao at 20211025 end*/
	...
	int clk_id, unsigned int freq, int dir)
{
	...
	/*Tab A8 code for P211014-04859  by dongtianbao at 20211025 start*/
	aw_dev_info(aw882xx->dev, "%s freq=%d", __func__, freq);
	/*Tab A8 code for P211014-04859  by dongtianbao at 20211025 end*/
			...
			struct snd_soc_dai *dai)
{
	...
	/*Tab A8 code for P211014-04859  by dongtianbao at 20211025 start*/
	aw_dev_info(aw882xx->dev, "%s", __func__);
	/*Tab A8 code for P211014-04859  by dongtianbao at 20211025 end*/
	...
	struct snd_soc_dai *dai)
{
	...
	/*Tab A8 code for P211014-04859  by dongtianbao at 20211027 start*/
	if (aw882xx->startup_cnt > 0) {
		aw882xx->startup_cnt--;
	}
	aw_dev_info(aw882xx->dev, "%s startup_cnt %d, mute_ignore %d, need_start %d",
		__func__, aw882xx->startup_cnt, aw882xx->mute_ignore, aw882xx->need_start);
...
static int aw882xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	/*Tab A8 code for P211014-04859  by dongtianbao at 20211027 start*/
	aw_dev_info(aw882xx->dev, "%s mute state %d, startup_cnt %d, mute_ignore %d, need_start %d",
		__func__, mute, aw882xx->startup_cnt, aw882xx->mute_ignore, aw882xx->need_start);

	if (aw882xx->need_start) {
		aw882xx->need_start = false;
	} else if (aw882xx->mute_ignore) {
		aw882xx->mute_ignore = false;
 		return 0;
	} else if (aw882xx->startup_cnt > 1) {
		return 0;
	}
...
static int aw882xx_gpio_request(struct aw882xx *aw882xx)
{
	...
	if (gpio_is_valid(aw882xx->reset_gpio)) {
		...
		if (ret) {
			/* Tab A8 code for SR-AX6300-01-92 by maoruiqian at 20210808 start */
			aw_dev_err(aw882xx->dev, "rst request failed or already request");
			aw882xx->reset_gpio = -1;
			/* Tab A8 code for SR-AX6300-01-92 by maoruiqian at 20210808 end */
		}
	}
	...
	if (gpio_is_valid(aw882xx->irq_gpio)) {
		...
		if (ret) {
			/* Tab A8 code for SR-AX6300-01-92 by maoruiqian at 20210808 start */
			aw_dev_err(aw882xx->dev, "int request failed or already request");
			aw882xx->irq_gpio = -1;
			/* Tab A8 code for SR-AX6300-01-92 by maoruiqian at 20210808 end */
		}
	}
				...
				const struct i2c_device_id *id)
{
	...
	if (ret){
		aw_dev_err(&i2c->dev, "aw882xx gpio request");
		return ret;
	}
	/*Tab A8 code for SR-AX6300-01-101 by wangxiaohui at 20210824 end*/
	/* hardware reset */
