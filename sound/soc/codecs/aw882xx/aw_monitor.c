...
static int aw_monitor_abox_work(struct aw_device *aw_dev)
{
	...
	if(aw_dev->set_ambient_flag == 0) {
		ret = aw_dsp_write_ambient_temperature(aw_dev, &temp, AW_AMBIENT_SMOOTH_DISABLE);
	} else {
		ret = aw_dsp_write_ambient_temperature(aw_dev, &temp, AW_AMBIENT_SMOOTH_ENABLE);
	}
	if (ret < 0) {
