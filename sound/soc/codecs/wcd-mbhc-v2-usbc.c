...
static int wcd_mbhc_initialise(struct wcd_mbhc *mbhc)
{
	...
	if (mbhc->mbhc_cfg->enable_usbc_analog_v2 == 0) {
		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_HS_L_DET_PULL_UP_COMP_CTRL, 0);
...
#ifdef CONFIG_SND_SOC_WCD_MBHC_CCIC_ADAPTOR_JACK_DET
static int MBHC_handle_ccic_notification(struct notifier_block *nb,
				unsigned long action, void *data)
{
	CC_NOTI_TYPEDEF *pnoti = (CC_NOTI_TYPEDEF *)data;

	struct wcd_mbhc *mbhc = container_of(nb, struct wcd_mbhc, ccic_nb);

	if (pnoti->dest != CCIC_NOTIFY_DEV_AUDIO) {
		pr_info("%s destination id is invalid\n", __func__);
		return 0;
	}

	pr_info("%s: Rcvd Noti=> action: %d src:%d dest:%d id:%d sub[%d %d %d]\n",
			__func__, (int)action, pnoti->src, pnoti->dest,
			pnoti->id, pnoti->sub1, pnoti->sub2, pnoti->sub3);
...
void __delayed_register_ccic_notifier(struct work_struct *work)
{
	int ret = 0;
	struct wcd_mbhc *mbhc;
	...
	mbhc = container_of(work, struct wcd_mbhc, ccic_work.work);

	ret = ccic_notifier_register(&mbhc->ccic_nb,
		MBHC_handle_ccic_notification, CCIC_NOTIFY_DEV_AUDIO);

	if (ret < 0) {
		pr_info("%s: CCIC Noti. is not ready. Try again in 4sec...\n",
			__func__);
		schedule_delayed_work(&mbhc->ccic_work, msecs_to_jiffies(4000));
		return;
	}
...
void MBHC_register_ccic_notifier(struct wcd_mbhc *mbhc)
{
	int ret = 0;

	pr_info("%s: Registering CCIC_NOTIFY_DEV_AUDIO.\n", __func__);

	mbhc->ccic_nb.notifier_call = MBHC_handle_ccic_notification;
	mbhc->ccic_nb.priority = 0;
	ret = ccic_notifier_register(&mbhc->ccic_nb,
		MBHC_handle_ccic_notification, CCIC_NOTIFY_DEV_AUDIO);
	...
	if (ret < 0) {
		pr_info("%s: CCIC Noti. is not ready. Try again in 8sec...\n",
			__func__);
		INIT_DELAYED_WORK(&mbhc->ccic_work,
					__delayed_register_ccic_notifier);
		schedule_delayed_work(&mbhc->ccic_work, msecs_to_jiffies(8000));
		return;
	}
...
int wcd_mbhc_start(struct wcd_mbhc *mbhc, struct wcd_mbhc_config *mbhc_cfg)
{
...
#ifdef CONFIG_SND_SOC_WCD_MBHC_CCIC_ADAPTOR_JACK_DET
	/* check if new USBC analog is defined on device tree for kelly */
	...
	gpio_direction_output(mbhc->usbc_ana_sel_gpio, 0);
	if (mbhc->mbhc_cfg->enable_usbc_analog_v2 == 0) {
		/* disable MBHC Headset insert/remove IRQs */
		mbhc->mbhc_cb->free_irq(codec,
				mbhc->intr_ids->mbhc_hs_ins_intr, mbhc);
		mbhc->mbhc_cb->free_irq(codec,
				mbhc->intr_ids->mbhc_sw_intr, mbhc);
		mbhc->mbhc_cb->free_irq(codec,
				mbhc->intr_ids->mbhc_hs_rem_intr, mbhc);
		/* disable ZDET */
		mbhc->impedance_detect = false;
		/* get notification from CCIC when Headset insert/remove */
		MBHC_register_ccic_notifier(mbhc);
	}
#endif
