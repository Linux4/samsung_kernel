...
static void s2803x_alc_enable(bool enable)
{
	if (enable && s2803x->is_alc_enabled)
		return;

	if (!enable && !s2803x->is_alc_enabled)
		return;

	dev_dbg(s2803x->dev, "%s: %s\n",
			__func__, enable ? "enable" : "disable");

	if (enable) {
		s2803x->is_alc_enabled = true;
		/* Update ALC configuration */
		regmap_write(s2803x->regmap, S2803X_REG_69_ALC_GA1, 0x6c);
		regmap_write(s2803x->regmap, S2803X_REG_6A_ALC_GA2, 0x6c);
		regmap_write(s2803x->regmap, S2803X_REG_6B_ALC_LVL, 0x12);
		regmap_write(s2803x->regmap, S2803X_REG_6C_ALC_LVR, 0x12);
		regmap_write(s2803x->regmap, S2803X_REG_6D_ALC_HLD, 0x80);
		regmap_write(s2803x->regmap, S2803X_REG_6E_ALC_ATK, 0x00);
		regmap_write(s2803x->regmap, S2803X_REG_6F_ALC_DCY, 0x00);
		regmap_write(s2803x->regmap, S2803X_REG_70_ALC_NG, 0xa0);
		regmap_write(s2803x->regmap, S2803X_REG_68_ALC_CTL, 0x48);
	} else {
		s2803x->is_alc_enabled = false;
		/* Update the reset values */
		regmap_write(s2803x->regmap, S2803X_REG_68_ALC_CTL, 0x00);
		regmap_write(s2803x->regmap, S2803X_REG_69_ALC_GA1, 0x80);
		regmap_write(s2803x->regmap, S2803X_REG_6A_ALC_GA2, 0x58);
		regmap_write(s2803x->regmap, S2803X_REG_6B_ALC_LVL, 0x1a);
		regmap_write(s2803x->regmap, S2803X_REG_6C_ALC_LVR, 0x1a);
		regmap_write(s2803x->regmap, S2803X_REG_6D_ALC_HLD, 0x12);
		regmap_write(s2803x->regmap, S2803X_REG_6E_ALC_ATK, 0x12);
		regmap_write(s2803x->regmap, S2803X_REG_6F_ALC_DCY, 0x12);
		regmap_write(s2803x->regmap, S2803X_REG_70_ALC_NG, 0x17);
	}

	/* Reset data path after changing ALC configuration */
	s2803x_reset_data();
}
...
/*
 * s2803x_alc_write: Update ALC controls
 *
 * While enabling ALC (Automatic Level Control), various other ALC related
 * registers are also updated. All those changes are done within this function
 * depending on whether ALC needs to be enabled or disabled.
 *
 * Returns the output of snd_soc_put_enum_double() call
 */
static int s2803x_alc_write(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (val) {
		s2803x_alc_enable(true);
		ret = snd_soc_put_enum_double(kcontrol, ucontrol);
	} else {
		ret = snd_soc_put_enum_double(kcontrol, ucontrol);
		s2803x_alc_enable(false);
	}
...
/* Function to update ALC values */
static int s2803x_soc_enum_alc_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	return s2803x_soc_kcontrol_handler(kcontrol, ucontrol,
			s2803x_alc_write);
...
static const struct snd_kcontrol_new s2803x_snd_controls[] = {
	...
	SOC_ENUM_EXT("ALC En", alc_enable_enum,
		s2803x_soc_enum_get, s2803x_soc_enum_alc_put),
};
