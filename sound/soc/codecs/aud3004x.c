...
static void i2c_client_change(struct aud3004x_priv *aud3004x, int client)
{
	...
	if (client != CODEC_CLOSE) {
		...
		codec->regmap = aud3004x->regmap[client];
	} else {
		i2c = aud3004x->i2c_priv[AUD3004D];
		codec->regmap = aud3004x->regmap[AUD3004D];
...
unsigned int aud3004x_read(struct aud3004x_priv *aud3004x, unsigned int reg)
{
	...
	if (client == AUD3004D) {
		ret = snd_soc_component_read32(codec, reg);
	} else {
		i2c = aud3004x->i2c_priv[client];
		codec->regmap = aud3004x->regmap[client];
		ret = snd_soc_component_read32(codec, reg & 0xFF);
		i2c = aud3004x->i2c_priv[AUD3004D];
		codec->regmap = aud3004x->regmap[AUD3004D];
	}
	mutex_unlock(&aud3004x->regmap_lock);
		...
		unsigned int val)
{
	...
	if (client == AUD3004D) {
		ret = snd_soc_component_write(codec, reg, val);
	} else {
		i2c = aud3004x->i2c_priv[client];
		codec->regmap = aud3004x->regmap[client];
		ret = snd_soc_component_write(codec, reg & 0xFF, val);
		i2c = aud3004x->i2c_priv[AUD3004D];
		codec->regmap = aud3004x->regmap[AUD3004D];
	}
	mutex_unlock(&aud3004x->regmap_lock);
		...
		unsigned int mask, unsigned int value)
{
	...
	if (client == AUD3004D) {
		ret = snd_soc_component_update_bits(codec, reg, mask, value);
	} else {
		i2c = aud3004x->i2c_priv[client];
		codec->regmap = aud3004x->regmap[client];
		ret = snd_soc_component_update_bits(codec, reg & 0xFF, mask, value);
		i2c = aud3004x->i2c_priv[AUD3004D];
		codec->regmap = aud3004x->regmap[AUD3004D];
	}
	mutex_unlock(&aud3004x->regmap_lock);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	amic_bias_flag = aud3004x_read(aud3004x, AUD3004X_110_PD_REF) & PDB_MCB_LDO_CODEC_MASK;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (value) {
		...
		/* PDB_VMID ON */
		aud3004x_update_bits(aud3004x, AUD3004X_110_PD_REF,
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ovp_refp = aud3004x_read(aud3004x, AUD3004X_15F_OVP1) & CTRV_SURGE_REFP_MASK;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	aud3004x_update_bits(aud3004x, AUD3004X_15F_OVP1,
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ovp_refn = aud3004x_read(aud3004x, AUD3004X_15F_OVP1) & CTRV_SURGE_REFN_MASK;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ovp_time = aud3004x_read(aud3004x, AUD3004X_160_OVP2) & OVP_TIME_SLOT_MASK;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	aud3004x_update_bits(aud3004x, AUD3004X_160_OVP2,
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ovp_mask_flag = aud3004x_read(aud3004x, AUD3004X_160_OVP2) & OVP_MASK_MASK;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ovp_apon_flag = aud3004x_read(aud3004x, AUD3004X_160_OVP2) & OVP_APON_MASK;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ovp_mute_flag = aud3004x_read(aud3004x, AUD3004X_160_OVP2) & OVP_UNMUTE_MASK;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	value = aud3004x_read(aud3004x, AUD3004X_134_VOL_AD1) & CTVOL_BST1_MASK;

	ucontrol->value.integer.value[0] = value >> CTVOL_BST1_SHIFT;
	return 0;
}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	case CTVOL_ALOG_GAIN_0:
		aud3004x_update_bits(aud3004x, AUD3004X_134_VOL_AD1,
	...
	case CTVOL_ALOG_GAIN_20:
		...
		break;
	default:
		dev_dbg(codec->dev, "%s called, Not supported value: %d\n",
				__func__, value);
		break;
	}
	return 0;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	value = aud3004x_read(aud3004x, AUD3004X_135_VOL_AD2) & CTVOL_BST2_MASK;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	switch (value) {
	case CTVOL_ALOG_GAIN_0:
		aud3004x_update_bits(aud3004x, AUD3004X_135_VOL_AD2,
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	value = aud3004x_read(aud3004x, AUD3004X_136_VOL_AD3) & CTVOL_BST3_MASK;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	switch (value) {
	case CTVOL_ALOG_GAIN_0:
		aud3004x_update_bits(aud3004x, AUD3004X_136_VOL_AD3,
...
static void aud3004x_ovp_on_setting_work(struct work_struct *work)
{
	...
	dev_dbg(codec->dev, "%s called, ovp setting done.\n", __func__);
}
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		/* Testmode Enable */
		aud3004x_update_bits(aud3004x, AUD3004X_B9_ODSEL4,
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_dbg(codec->dev, "%s called, chop: 0x%02x, event: %d, cur_aif: %d\n",
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		case AUD3004X_SAMPLE_RATE_48KHZ:
			...
			/* DAC Trim */
			aud3004x_write(aud3004x, AUD3004X_47_TRIM_DAC0, 0xE9);
			aud3004x_write(aud3004x, AUD3004x_48_TRIM_DAC1, 0x79);
			if (ep_on | lineout_on) {
				/* Compensation Mode selection */
				aud3004x_update_bits(aud3004x, AUD3004X_44_PLAY_MODE2,
						DAC_CH_MODE_MASK,
		...
		case AUD3004X_SAMPLE_RATE_192KHZ:
			/* CP Frequency Control */
			aud3004x_write(aud3004x, AUD3004X_B0_AUTO_COM1, 0x1C);
			aud3004x_update_bits(aud3004x, AUD3004X_150_CTRL_EP,
					EN_CP_AUTOSEL_MASK, 0);

			/* HP Driver Current */
			aud3004x_write(aud3004x, AUD3004X_A6_AUTO_HP7, 0x36);

			/* DAC Trim */
			aud3004x_write(aud3004x, AUD3004X_47_TRIM_DAC0, 0xFF);
			aud3004x_write(aud3004x, AUD3004x_48_TRIM_DAC1, 0x29);
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		aud3004x_write(aud3004x, AUD3004X_160_OVP2, 0x02);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		aud3004x_write(aud3004x, AUD3004X_A6_AUTO_HP7, 0x01);
		...
		unsigned int cur_aifrate)
{
	...
	if (aud3004x->playback_aifrate != cur_aifrate) {
		...
		case AUD3004X_SAMPLE_RATE_48KHZ:
			...
			aud3004x_write(aud3004x, AUD3004X_72_AVC35, 0x38);
			aud3004x_write(aud3004x, AUD3004X_71_AVC34, 0x18);
			aud3004x_write(aud3004x, AUD3004X_74_AVC37, 0xC1);
		...
		case AUD3004X_SAMPLE_RATE_192KHZ:
			...
			/* AVC delay default setting */
			aud3004x_write(aud3004x, AUD3004X_72_AVC35, 0x19);
			aud3004x_write(aud3004x, AUD3004X_71_AVC34, 0x1A);
			aud3004x_write(aud3004x, AUD3004X_74_AVC37, 0x21);
			aud3004x_write(aud3004x, AUD3004X_73_AVC36, 0x18);
...
static void aud3004x_register_initialize(void *context)
{
	...
	/* OVP Setting */
	aud3004x_write(aud3004x, AUD3004X_15F_OVP1, 0xAE);
	...
	aud3004x_update_bits(aud3004x, AUD3004X_102_CLK3_COD,
			SEL_DIGMUTE_HP_MASK, 2 << SEL_DIGMUTE_HP_SHIFT);

	/* Tuning AVC Parameters */
	aud3004x_write(aud3004x, AUD3004X_68_AVC25, 0x08);
