...
#define DBMDX_USE_EXPIRABLE_WAKELOCK    1
#define DBMDX_WAKELOCK_TIMEOUT_MS       70*1000
...
static void spi_transport_enable(struct dbmdx_private *p, bool enable)
{
	...
	if (enable) {
		...
		if (ret)
			...
			dev_info(spi_p->dev, "%s, interface is active\n",
				__func__);
	}
	...
	if (enable) {
		p->wakeup_set(p);
		if (p->asleep) {
			dev_info(spi_p->dev, "%s (DBMDX_MSLEEP_SPI_WAKEUP) --\n", __func__);
...
int spi_common_probe(struct spi_device *client)
{
	...
	if (p->ps_nosuspend_wl)
		...
		dev_err(p->dev,
			"%s: Error creating WS: dbmdx_nosuspend_wakelock_spi\n",
			__func__);
#endif
