 ...
 *
 * r004, 2018.12.03	- initial version  sma1301
 *
...
static const char * const sma1301_release_time_text[] = {
	"Time_48kHz-20ms,96kHz-10ms", "Time_48kHz-40ms,96kHz-20ms",
	"Time_48kHz-60ms,96kHz-30ms", "Time_48kHz-80ms,96kHz-40ms",
	"Time_48kHz-100ms,96kHz-50ms", "Time_48kHz-120ms,96kHz-60ms",
	"Time_48kHz-140ms,96kHz-70ms", "Time_48kHz-160ms,96kHz-80ms",
	"Time_48kHz-180ms,96kHz-90ms", "Time_48kHz-200ms,96kHz-100ms",
	"Time_48kHz-220ms,96kHz-110ms", "Time_48kHz-240ms,96kHz-120ms",
	"Time_48kHz-260ms,96kHz-130ms", "Time_48kHz-280ms,96kHz-140ms",
	"Time_48kHz-300kms,96Hz-150ms", "Time_48kHz-320ms,96kHz-160ms"};
...
SOC_SINGLE("Speaker Mute Switch",
		SMA1301_0E_MUTE_VOL_CTRL, 0, 1, 0),

SOC_ENUM_EXT("Speaker Mode", sma1301_spkmode_enum,
	sma1301_spkmode_get, sma1301_spkmode_put),

SOC_SINGLE("Speaker EQ(1:enable,0:bypass)", SMA1301_11_SYSTEM_CTRL2, 7, 1, 0),
SOC_SINGLE("Speaker Bass(1:enable,0:bypass)", SMA1301_11_SYSTEM_CTRL2, 6, 1, 0),
SOC_SINGLE("Speaker Comp/Limiter(1:enable,0:bypass)",
		SMA1301_11_SYSTEM_CTRL2, 5, 1, 0),
SOC_SINGLE("LR_DATA_SW", SMA1301_11_SYSTEM_CTRL2, 4, 1, 0),
SOC_SINGLE("Mono Mix(1:enable,0:disable)", SMA1301_11_SYSTEM_CTRL2, 0, 1, 0),
...
SOC_SINGLE("SPK modulator sync(1:1/8,0:1/4)",
...
SOC_SINGLE("PLL enable(1:Enable,0:Disable)",
...
SOC_SINGLE("DAC Down Conversion(1:Conversion,0:Normal)",
...
static int sma1301_dai_digital_mute(struct snd_soc_dai *codec_dai, int mute)
{
	...
	if (mute) {
		...
		/* Pop noise improvement : When the SCK clock disappears,
		 * the PWM clock is turned off at the same timing
		 */
		if (sma1301->sys_clk_id == SMA1301_PLL_CLKIN_BCLK)
			regmap_update_bits(sma1301->regmap,
				SMA1301_10_SYSTEM_CTRL1,
				SPK_MODE_MASK, SPK_OFF);
	} else {
		dev_info(codec->dev, "%s : %s\n", __func__, "UNMUTE");
...
static int sma1301_reset(struct snd_soc_codec *codec)
{
	...
	/* Attack level - 0.125FS,
	 * Release time - 40ms(48kHz) & 20ms(96kHz)
	 */
	regmap_write(sma1301->regmap, SMA1301_91_CLASS_G_CTRL,	0x22);
	/* Filtered VDD gain control */
	regmap_write(sma1301->regmap, SMA1301_92_FDPEC_CTRL,	0xD0);
	 ...
	 * - PI compensation, 50pF I-gain, 0.75Mohm P-gain
	 */
	regmap_write(sma1301->regmap, SMA1301_95_BOOST_CTRL2,	0x22);
	 ...
	 * boost voltage - 6.2V, minimum on-time - 62ns
	 */
	regmap_write(sma1301->regmap, SMA1301_97_BOOST_CTRL4,	0xBE);
	/* PLL Lock disable, External clock operation */
	regmap_write(sma1301->regmap, SMA1301_A2_TOP_MAN1,	0xE8);
	...
	regmap_update_bits(sma1301->regmap, SMA1301_3F_ATEST2,
			SPK_OUT_FREQ_MASK, SPK_OUT_FREQ_470kHz);
	...
	if (sma1301->src_bypass == true) {
		...
		regmap_write(sma1301->regmap, SMA1301_0A_SPK_VOL, 0x32);
	} else {
		regmap_update_bits(sma1301->regmap, SMA1301_03_INPUT1_CTRL3,
