...
static const struct madera_compr_dai_mapping cs47l15_compr_dai_map[] = {
	{
...
struct cs47l15 {
	...
	struct cs47l15_compr compr_info[ARRAY_SIZE(cs47l15_compr_dai_map)];
};
				    ...
				    int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		ret = madera_set_adsp_clk(&cs47l15->core.adsp[w->shift], freq);
		if (ret)
			return ret;
		break;
	default:
...
SOC_ENUM_EXT("HPOUT1 Demux", madera_out1demux_enum,
	     snd_soc_get_enum_double, madera_out1_demux_put),
...
static void cs47l15_compr_irq(struct cs47l15 *cs47l15,
			      struct cs47l15_compr *compr)
{
	bool trigger;

	wm_adsp_compr_irq(&compr->adsp_compr, &trigger);
}
...
static int cs47l15_compr_open(struct snd_compr_stream *stream)
{
	...
	compr_idx = madera_get_compr_map_idx(rtd,
					     cs47l15_compr_dai_map,
					     ARRAY_SIZE(cs47l15_compr_dai_map));
	if (compr_idx < 0) {
		dev_err(cs47l15->core.madera->dev,
	...
	return wm_adsp_compr_open(&cs47l15->compr_info[compr_idx].adsp_compr,
				  stream);
}

static void cs47l15_init_compr_info(struct cs47l15 *cs47l15)
{
	struct wm_adsp *dsp;
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(cs47l15->compr_info); ++i) {
		...
		cs47l15->compr_info[i].dai_name =
			cs47l15_compr_dai_map[i].dai_name;

		dsp = &cs47l15->core.adsp[cs47l15_compr_dai_map[i].dsp_num],
		wm_adsp_compr_init(dsp, &cs47l15->compr_info[i].adsp_compr);
	}
}

static void cs47l15_destroy_compr_info(struct cs47l15 *cs47l15)
{
	int i;
...
static int cs47l15_codec_probe(struct snd_soc_codec *codec)
{
	...
	cs47l15->core.madera->dapm = &codec->dapm;
	...
	ret = madera_init_dsp_irq(codec, cs47l15_adsp2_irq, cs47l15);
	if (ret)
		return ret;

	wm_adsp2_codec_probe(&cs47l15->core.adsp[0], codec);

	ret = madera_init_bus_error_irq(codec, 0, cs47l15_dsp_bus_error);
	if (ret) {
...
err_free_dsp_irq:
	madera_destroy_dsp_irq(codec, cs47l15);
	return ret;
}
...
static int cs47l15_probe(struct platform_device *pdev)
{
	...
	BUILD_BUG_ON(ARRAY_SIZE(cs47l15->core.mixer_sources_cache) <
		     ARRAY_SIZE(cs47l15_fx_inputs));
	...
	cs47l15->core.get_sources = cs47l15_get_sources;
	...
	if (madera->pdata.codec.num_fw_defs[0]) {
		cs47l15->core.adsp[0].firmwares
			= madera->pdata.codec.fw_defs[0];

		cs47l15->core.adsp[0].num_firmwares
			= madera->pdata.codec.num_fw_defs[0];
	}
