 ...
 *
 * r014, 2020.05.14	- initial version  sma1303
 *
...
#ifdef CONFIG_SND_SOC_SAMSUNG_AUDIO
	dev_info(codec->dev, "%s: BCLK Enable\n", __func__);
	audmixer_get_sync();
#endif
...
#ifdef CONFIG_SND_SOC_SAMSUNG_AUDIO
	dev_info(codec->dev, "%s: BCLK Disable\n", __func__);
	audmixer_put_sync();
#endif
...
static int sma1303_dai_set_pll(struct snd_soc_dai *dai, int pll_id, int source,
			unsigned int freq_in, unsigned int freq_out)
{
	...
	dev_info(codec->dev, "%s : freq_in = %d Hz\n", __func__, freq_in);

	if (freq_in == sma1303->last_bclk)
		return 0;

	if (!freq_in) {
		dev_err(codec->dev, "PLL determined through hw_params function\n");
		sma1303->hw_param_pll_ignore = false;
		return 0;
	}

	if (sma1303->amp_power_status) {
		sma1303_shutdown(codec);
		ret = sma1303_setup_pll(codec, freq_in);
		sma1303_startup(codec);
	} else
		ret = sma1303_setup_pll(codec, freq_in);

	if (ret < 0) {
		dev_err(codec->dev, "%s : Unsupport input clock %d\n",
				__func__, freq_in);
		sma1303->hw_param_pll_ignore = false;
		return ret;
	}

	sma1303->last_bclk = freq_in;
	sma1303->hw_param_pll_ignore = true;
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (sma1303->hw_param_pll_ignore == false) {
