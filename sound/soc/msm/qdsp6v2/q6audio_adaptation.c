...
int q6audio_dsm_read(int port)
{
	...
	config.get_param.mem_hdr.mem_map_handle = 0;
	config.get_param.module_id = mod;
	config.get_param.param_id = AFE_PARAM_ID_CALIB_RES_CFG;
	config.get_param.mem_hdr.data_payload_addr_lsw = 0;
	config.get_param.mem_hdr.data_payload_addr_msw = 0;
...
static int32_t q6audio_tfadsp_callback(struct apr_client_data *data, void *priv)
{
	...
	if (data->opcode == AFE_PORT_CMDRSP_GET_PARAM_V2 ||
		...
		if (atomic_read(&this_afe.tfa_state) == 1 && data->token ==
			...
			pr_debug("%s:opcode=0x%x, token = %d, payload_size = %d, payload = %d, %d\n",
				__func__, data->opcode,
				data->token, data->payload_size, payload[0], payload[1]);
		...
		int dev, int buf_size, char *buf, int msg_type, int num_msgs)
{
	...
	int index = 0;
	uint8_t *pmem = NULL;
	struct afe_port_cmd_set_param_v3 set_param;
	...
	set_param.port_id = afe_port.amp_rx_id;
	...
	pdata.instance_id = INSTANCE_ID_0;
...
extern int q6afe_pack_and_set_param_in_band(u16 port_id, int index,
...
int q6asm_set_sound_alive(struct audio_client *ac, long *param)
{
	...
	param_info.module_id = ASM_MODULE_ID_PP_SA;
	param_info.instance_id = INSTANCE_ID_0;
	param_info.param_id = ASM_PARAM_ID_PP_SA_PARAMS;
...
int q6asm_set_play_speed(struct audio_client *ac, long *param)
{
	...
	memset(&param_info, 0, sizeof(param_info));
	memset(&cmd, 0, sizeof(cmd));
	param_info.module_id = ASM_MODULE_ID_PP_SA_VSP;
	param_info.instance_id = INSTANCE_ID_0;
	param_info.param_id = ASM_PARAM_ID_PP_SA_VSP_PARAMS;
	param_info.param_size = sizeof(cmd);
...
int q6asm_set_adaptation_sound(struct audio_client *ac, long *param)
{
	...
	memset(&param_info, 0, sizeof(param_info));
	memset(&cmd, 0, sizeof(cmd));
	param_info.module_id = ASM_MODULE_ID_PP_ADAPTATION_SOUND;
	param_info.instance_id = INSTANCE_ID_0;
	param_info.param_id = ASM_PARAM_ID_PP_ADAPTATION_SOUND_PARAMS;
	param_info.param_size = sizeof(cmd);
...
int q6asm_set_sound_balance(struct audio_client *ac, long *param)
{
	...
	memset(&param_info, 0, sizeof(param_info));
	memset(&cmd, 0, sizeof(cmd));
	param_info.module_id = ASM_MODULE_ID_PP_LRSM;
	param_info.instance_id = INSTANCE_ID_0;
	param_info.param_id = ASM_PARAM_ID_PP_LRSM_PARAMS;
	param_info.param_size = sizeof(cmd);
...
int q6asm_set_myspace(struct audio_client *ac, long *param)
{
	...
	memset(&param_info, 0, sizeof(param_info));
	memset(&cmd, 0, sizeof(cmd));
	param_info.module_id = ASM_MODULE_ID_PP_SA_MSP;
	param_info.instance_id = INSTANCE_ID_0;
	param_info.param_id = ASM_MODULE_ID_PP_SA_MSP_PARAM;
	param_info.param_size = sizeof(cmd);
...
int q6asm_set_sound_boost(struct audio_client *ac, long *param)
{
	...
	memset(&param_info, 0, sizeof(param_info));
	memset(&cmd, 0, sizeof(cmd));
	param_info.module_id = ASM_MODULE_ID_PP_SB;
	param_info.instance_id = INSTANCE_ID_0;
	param_info.param_id = ASM_PARAM_ID_PP_SB_PARAM;
	param_info.param_size = sizeof(cmd);
...
int q6asm_set_upscaler(struct audio_client *ac, long *param)
{
	...
	memset(&param_info, 0, sizeof(param_info));
	memset(&cmd, 0, sizeof(cmd));
	param_info.module_id = ASM_MODULE_ID_PP_SA_UPSCALER_COLOR;
	param_info.instance_id = INSTANCE_ID_0;
	param_info.param_id = ASM_PARAM_ID_PP_SA_UPSCALER_COLOR_PARAMS;
	param_info.param_size = sizeof(cmd);
...
int q6asm_set_sb_rotation(struct audio_client *ac, long *param)
{
	...
	param_info.module_id = ASM_MODULE_ID_PP_SB;
	param_info.instance_id = INSTANCE_ID_0;
	param_info.param_id = ASM_PARAM_ID_PP_SB_ROTATION_PARAM;
	param_info.param_size = sizeof(cmd);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((be_idx < MSM_BACKEND_DAI_MAX) && msm_bedai.active) {
		memset(&param_hdr, 0, sizeof(param_hdr));
		param_hdr.module_id = ADM_MODULE_ID_PP_SS_REC;
		param_hdr.instance_id = 0x8000;
		param_hdr.param_id = ADM_PARAM_ID_PP_SS_REC_GETPARAMS;
