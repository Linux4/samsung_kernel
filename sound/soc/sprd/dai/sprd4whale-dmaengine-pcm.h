...
struct sprd_dma_physical_addr {
	u32 dev_addr[2];	/* L/R channels */
};

struct sprd_pcm_dma_params {
	char *name;			/* stream identifier */
	int channels[4];	/* channel id (device id, dma uid)*/
	int irq_type;		/* dma interrupt type */
	int stage;			/* stage num of dma */
	struct sprd_dma_cfg desc[2];	/* dma description struct, 2 stages dma */
	struct sprd_dma_physical_addr dev_paddr[2];	/* device physical address for DMA, 2 stages dma */
};
