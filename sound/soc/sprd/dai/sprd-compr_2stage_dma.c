...
static int sprd_compr_config_sprd_rtd(struct sprd_compr_rtd *srtd,
						struct snd_compr_stream *substream, struct snd_compr_params *params)
{
	int ret = 0;
	int i, count;
	size_t total_size = 0;
	...
	srtd->iram_buff = (void *)ioremap_nocache(s_sprd_compr_dma_paddr, SPRD_COMPR_DMA_DATA_SIZE);
#if COMPR_DUMP_MEM_DEBUG
	g_iram = (uint8_t *)srtd->iram_buff;
	g_frag_size = period;
#endif
	memset(srtd->iram_buff, 0, SPRD_COMPR_DMA_DATA_SIZE);
	/* for 2stage DMA debug */
	/*__iounmap(iram_buff);*/
#if COMPR_DUMP_DEBUG
	srtd->log_file0 = filp_open("/data/dma0.txt", O_RDWR | O_CREAT | O_TRUNC, 0644);
	srtd->log_file1 = filp_open("/data/dma1.txt", O_RDWR | O_CREAT | O_TRUNC, 0644);
#endif
	/* stage 0 */
	srtd->dma_cfg_phy[0] = s_sprd_compr_dma_paddr + SPRD_COMPR_DMA_DATA_SIZE;
	srtd->dma_cfg_virt[0] = ioremap_nocache(srtd->dma_cfg_phy[0], SPRD_COMPR_DMA_DESC_SIZE + SPRD_COMPR_INFO_SIZE);

	/* stage 1 */
	srtd->buffer_paddr = audio_smem_alloc(total_size);
	...
	srtd->buffer = ioremap_nocache(srtd->buffer_paddr, total_size);
#if COMPR_DUMP_MEM_DEBUG
	g_buff = (uint8_t *)srtd->buffer;
#endif
	srtd->dma_cfg_phy[1] = srtd->buffer_paddr + buffer_size;
	srtd->dma_cfg_virt[1] = srtd->buffer + buffer_size;
	...
	goto out;
err3:
	audio_smem_free(srtd->buffer_paddr, total_size);
	__iounmap(srtd->buffer);
	srtd->buffer = 0;
...
static void sprd_compr_dma_buf_done(void *data)
{
	...
	old_fs = get_fs();
	set_fs(get_ds());
	vfs_write(srtd->log_file0, srtd->iram_buff, SPRD_COMPR_DMA_DATA_SIZE, &srtd->log_file0->f_pos);
	set_fs(old_fs);
#endif
			      ...
			      struct snd_compr_params *params)
{
	...
	dma_src_phys = srtd->buffer_paddr;
	dma_des_phys = (dma_addr_t)s_sprd_compr_dma_paddr;
	...
	dma_config_ptr = (struct sprd_dma_cfg *)kzalloc(
		sizeof(struct sprd_dma_cfg) * period_cnt, GFP_KERNEL);
	...
	/* none-wrap */
	/*srtd->params.tran_len = period * ((period_cnt > 1) ? (period_cnt - 1) : period_cnt); */
	srtd->params.tran_len = period * period_cnt / 2;
	...
	for (j = 0; j < 2; j++) {
		...
		if (substream->direction == SND_COMPRESS_PLAYBACK) {
			...
			cfg_ptr->des_addr = SPRD_COMPR_DMA_PADDR_TRAN(dma_des_phys);
		} else {
			cfg_ptr->src_addr = SPRD_COMPR_DMA_PADDR_TRAN(dma_des_phys);
	...
	sp_asoc_pr_info("srtd frag_len=%d, block_len=%d, tran_len=%d, buffer.fragments=%d, fragment_size=%d\n",
			srtd->params.frag_len, srtd->params.block_len, srtd->params.tran_len,
			params->buffer.fragments, params->buffer.fragment_size);

	sp_asoc_pr_info("totsize=%d, period=%d, j=%d, srtd->dma_tx_des[0]=0x%lx\n",
			totsize, period, j, srtd->dma_tx_des[0]);

#if 0
	/*config master chn start slv chn*/
	ret = dma_chn_request->device->device_chn_start_chn(srtd->dma_tx_des[s],
				srtd->dma_chn[0], dma_chn_request, TRANS_DONE, DMA_CHN_GROUP_1);
	if (ret < 0) {
		ret = -6;
		goto hw_param_err;
	}
			      ...
			      struct snd_compr_params *params)
{
	...
	dma_src_phys = (dma_addr_t)s_sprd_compr_dma_paddr;
	if (substream->direction == SND_COMPRESS_PLAYBACK)
	...
	for (j = 0; j < 2; j++) {
		...
		if (substream->direction == SND_COMPRESS_PLAYBACK) {
			cfg_ptr->src_step = 4;
			cfg_ptr->des_step = 0;
			cfg_ptr->src_addr = SPRD_COMPR_DMA_PADDR_TRAN(dma_src_phys);
			cfg_ptr->des_addr = mcdt_dac_dma_phy_addr(MCDT_CHN_COMPR);
		} else {
			cfg_ptr->src_step = 0;
			cfg_ptr->des_step = 4;
			cfg_ptr->src_addr = mcdt_dac_dma_phy_addr(MCDT_CHN_COMPR);
			cfg_ptr->des_addr = SPRD_COMPR_DMA_PADDR_TRAN(dma_src_phys);
		}
	...
	/* config dma channel */
	ret = dmaengine_device_control(dma_chn_request,
							DMA_SLAVE_CONFIG,
							(unsigned long)(dma_config_ptr));
	if (ret < 0) {
		ret = -4;
		goto hw_param_err;
	}
	...
	/* get dma desc from dma config */
	srtd->dma_tx_des[s] =
			dma_chn_request->device->device_prep_dma_memcpy(dma_chn_request, 0, 0, 0,
					DMA_CFG_FLAG|DMA_HARDWARE_FLAG);
	if (!srtd->dma_tx_des[s]) {
		ret = -5;
		goto hw_param_err;
...
static int compr_stream_hw_free(struct snd_compr_stream *cstream)
{
	...
	if (srtd->buffer_paddr) {
		audio_smem_free(srtd->buffer_paddr, srtd->total_size);
		srtd->buffer_paddr = 0;
	}
	if (srtd->buffer) {
		iounmap(srtd->buffer);
...
*/
		/*config master chn start slv chn*/
		ret = srtd->dma_chn[1]->device->device_chn_start_chn(srtd->dma_tx_des[1],
					srtd->dma_chn[0], srtd->dma_chn[1], TRANS_DONE, DMA_CHN_GROUP_1);
		if (ret < 0) {
			ret = -6;
		}

		dmaengine_submit(srtd->dma_tx_des[1]);
		dmaengine_submit(srtd->dma_tx_des[0]);
...
static int sprd_platform_compr_trigger(struct snd_compr_stream *cstream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_STOP:
		...
		memset(srtd->info_vaddr, 0, sizeof(struct sprd_compr_playinfo));
	...
	case SND_COMPR_TRIGGER_DRAIN:
		...
		cmd_drain.reserved1 = srtd->received_total;
		...
		/* if (prtd->received_total > prtd->copied_total) { */
					...
					size_t count)
{
	...
	sp_asoc_pr_info("%s: count = %d,app_pointer=%d,buff_size=%d,received_total=%d\n",
		__func__, count, srtd->app_pointer, srtd->buffer_size, srtd->received_total);
#if COMPR_DUMP_DEBUG
	mm_segment_t *old_fs = 0;
	old_fs = get_fs();
	set_fs(get_ds());
	...
	if (srtd->received_stage0 < runtime->fragment_size) {
		bytes_available = runtime->fragment_size - srtd->received_stage0;
		...
		if (bytes_available >= count) {
			...
			/* copy partial data to stage0 buffer, and copy left to stage1 buffer */
			...
			if (unalign_copy_from_user(dstn, buf, copy))
				...
				/* not return, continue copy left data to stage1 buffer */
			}
		}
	}
...
#if COMPR_DUMP_DEBUG
		vfs_write(srtd->log_file1, dstn, count, &srtd->log_file1->f_pos);
#endif
	} else {
...
#if COMPR_DUMP_DEBUG
		vfs_write(srtd->log_file1, srtd->buffer, count - copy, &srtd->log_file1->f_pos);
#endif
	}
...
copy_done:
	...
	sp_asoc_pr_info("%s: received_stage0=%d,received_stage1=%d,received_total=%d\n",
		__func__, srtd->received_stage0, srtd->received_stage1, srtd->received_total);
...
static void time_handler(unsigned long data)
{
	...
	printk(KERN_EMERG "yintang:done_0=%d, done_1=%d\n", s_buf_done_count0, s_buf_done_count);
	printk(KERN_EMERG "yintang:0x%x,0x%x\n",
		g_iram[0], g_iram[1]);

	printk(KERN_EMERG "yintang:0x%x,0x%x,0x%x,0x%x----0x%x,0x%x,0x%x,0x%x\n",
		g_buff[0], g_buff[1], g_buff[g_frag_size], g_buff[g_frag_size + 1],
		g_buff[g_frag_size * 2], g_buff[g_frag_size * 2 + 1], g_buff[g_frag_size * 3], g_buff[g_frag_size * 3 + 1]);
#if 0
	printk(KERN_EMERG "yintang:0x%x,0x%x,0x%x,0x%x----0x%x,0x%x,0x%x,0x%x\n",
		g_buff[g_frag_size * 4], g_buff[g_frag_size * 4 + 1], g_buff[g_frag_size * 5], g_buff[g_frag_size * 5 + 1],
		g_buff[g_frag_size * 6], g_buff[g_frag_size * 6 + 1], g_buff[g_frag_size * 7], g_buff[g_frag_size * 7 + 1]);
#endif
	return;
}
static int timer_init(void){
