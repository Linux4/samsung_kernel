...
#define TIME_NEED_AFTER_HMICBIAS_ON (350)
...
/* HS03 code for P210928-07436 by LiangWenye at 20211008 start */
extern struct class* audio_class;
/* HS03 code for P210928-07436 by LiangWenye at 20211008 end */
...
{
	/* Tab A8 code for AX6300DEV-1843 by guoweihai at 20211022 start */
	while (hdst->time_after_hmicbias_on > 0 &&
		time_before(jiffies, hdst->time_after_hmicbias_on)) {
		sprd_msleep(10);
		pr_info("[%s][time_after_hmicbias_on] delay to wait hmicbias on for %dms",
		__func__, TIME_NEED_AFTER_HMICBIAS_ON);
	}
	/* Tab A8 code for AX6300DEV-1843 by guoweihai at 20211022 end */
	...
	struct sprd_headset_platform_data *pdata)
{
	...
	/* Tab A8 code for AX6300DEV-1843 by guoweihai at 20211022 start */
	pr_info("[%s][time_after_hmicbias_on] hmicbias on", __func__);
	hdst->time_after_hmicbias_on =
	jiffies + msecs_to_jiffies(TIME_NEED_AFTER_HMICBIAS_ON);
	/* Tab A8 code for AX6300DEV-1843 by guoweihai at 20211022 end */
...
static void headset_detect_all_work_func(struct work_struct *work)
{
	...
	if (!pdata->support_typec_hdst) {
		...
		/*HS03 code for SL6215DEV-3519|SL6215DEV-576 by yuyongxiang at 20211108 start*/
		if (hdst->plug_state_last == 0) {
...
static int sprd_headset_debug_sysfs_init(void)
{
	...
	/* HS03 code for P210928-07436 by LiangWenye at 20211008 start */
	ret = sysfs_create_link(&audio_class->p->subsys.kobj, headset_debug_kobj, "earjack");
	if (ret < 0) {
		pr_err("sysfs_create_link failed, ret = %d\n", ret);
		return ret;
	}
	/* HS03 code for P210928-07436 by LiangWenye at 20211008 end */
