...
static int sprd_headset_debug_sysfs_state_init(void)
{
    int ret, i;
	static struct kobject *headset_debug_kobj;
	static struct kobj_attribute headset_state_debug_attr[] = {
	...
	for (i = 0; i < sizeof(headset_state_debug_attr) /
	     sizeof(headset_state_debug_attr[0]); i++) {
		ret = sysfs_create_file(headset_debug_kobj,
					&headset_state_debug_attr[i].attr);
		if (ret) {
			pr_err("%s create sysfs '%s' failed. ret %d\n",
			       __func__, headset_state_debug_attr[i].attr.name, ret);
			return ret;
		}
	}
...
static ssize_t sprd_headset_support_show(struct device *dev_head,
...
static ssize_t sprd_headset_support_store(struct device *dev_head,
	struct device_attribute *attr, const char *buff, size_t len)
{
	return len;
}

static int sprd_headset_debug_sysfs_support_init(void)
{
    int ret, i;
	struct sprd_headset *hdst = sprd_hdst;
	struct sprd_headset_platform_data *pdata = (hdst ? &hdst->pdata : NULL);
	static struct class *headset_debug_class;
	struct device *headset_dev;
	static struct device_attribute headset_support_debug_attr[] = {
		__ATTR(state, 0644,
		sprd_headset_support_show,
		sprd_headset_support_store),
	};
	...
	if (pdata->support_typec_hdst) {
	    pr_info("%s support typec, not init 3.5mm support node!\n");
		return 0;
	}

	headset_debug_class = class_create(THIS_MODULE, "audio");
	if (headset_debug_class == NULL) {
		ret = -ENOMEM;
		pr_err("%s register sysfs failed. ret %d\n", __func__, ret);
		return ret;
	}

	headset_dev = device_create(headset_debug_class, NULL, MKDEV(0, 0), NULL, "earjack");
	for (i = 0; i < sizeof(headset_support_debug_attr) /
	     sizeof(headset_support_debug_attr[0]); i++) {
		ret = device_create_file(headset_dev,
					&headset_support_debug_attr[i]);
		if (ret) {
			pr_err("%s create sysfs '%s' failed. ret %d\n",
			       __func__, headset_support_debug_attr[i].attr.name, ret);
			return ret;
		}
	}
...
static int sprd_headset_debug_sysfs_init(void)
{
	...
	ret = sprd_headset_debug_sysfs_state_init();
	if (ret != 0) {
	    pr_info("sysfs headset state init failed!\n");
		return ret;
	}

	ret = sprd_headset_debug_sysfs_support_init();
	if (ret != 0) {
	    pr_info("sysfs headset support init failed!\n");
		return ret;
	}
