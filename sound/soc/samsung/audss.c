...
#define USE_EPLL_GATING
#define EXYNOS_AUDSS_SRC_GATE_VAL	(0x0)
#define EXYNOS_AUDSS_DIV_GATE_VAL	(0xF00)
...
#ifdef USE_EPLL_GATING
	clk_enable(audss.mout_epll);
#else
...
void audss_do_disable(void)
{
	...
	spin_unlock(&audss_lock);
#ifdef USE_EPLL_GATING
	writel(EXYNOS_AUDSS_SRC_GATE_VAL,
		audss.base + EXYNOS_CLKSRC_AUDSS_OFFSET);
	writel(EXYNOS_AUDSS_DIV_GATE_VAL,
		audss.base + EXYNOS_CLKDIV_AUDSS_OFFSET);

	clk_disable(audss.mout_epll);
#endif
}

struct clk *audss_get_i2s_opclk(int clk_id)
{
	return clk_id ? audss.dout_i2s : audss.dout_bus;
}
