/*
 * Audio SubSystem driver for Samsung Exynos3475
 *
...
#define AUD_PLL_FREQ			(294912010U)
#define AUD_MI2S_FREQ			(98304000U + 100)

/* SYSREG SFR */
#define EXYNOS3475_DISPAUD_SFR_APB		0x0100
#define EXYNOS3475_DISPAUD_XIU_TOP_DRCG_EN	0x0204
#define EXYNOS3475_DISPAUD_XIU_ASYNC_DRCG_EN	0x0208
#define EXYNOS3475_DISPAUD_XIU_DISPAUD_QOS_EN	0x0600
#define EXYNOS3475_DISPAUD_QOS_SEL		0x0604
#define EXYNOS3475_DISPAUD_CFG			0x1000
#define EXYNOS3475_DISPAUD_INT_MASK		0x101c
#define EXYNOS3475_DISPAUD_FM_BT_MUX_CON	0x1020

#define DISPAUD_CFG_ADMA_SWRST_BIT		2
...
static void lpass_enable_pll(bool on)
{
	if (on) {
		...
		clk_prepare_enable(lpass_cmu.clk_mout_aud_pll_user);
		clk_prepare_enable(lpass_cmu.clk_mout_sclk_mi2s);
	} else {
		clk_disable_unprepare(lpass_cmu.clk_mout_sclk_mi2s);
		clk_disable_unprepare(lpass_cmu.clk_mout_aud_pll_user);
...
static int lpass_set_clk_hierarchy(struct device *dev)
{
	...
	lpass_cmu.clk_mout_aud_pll_user =
		devm_clk_get(dev, "mout_aud_pll_user");
	if (IS_ERR(lpass_cmu.clk_mout_aud_pll_user)) {
		dev_err(dev, "mout_aud_pll_user clk not found\n");
		return PTR_ERR(lpass_cmu.clk_mout_aud_pll_user);
	}

	lpass_cmu.clk_mout_sclk_mi2s = devm_clk_get(dev, "mout_sclk_mi2s");
	if (IS_ERR(lpass_cmu.clk_mout_sclk_mi2s)) {
		dev_err(dev, "mout_sclk_mi2s clk not found\n");
		return PTR_ERR(lpass_cmu.clk_mout_sclk_mi2s);
	}
	...
	lpass_cmu.clk_aclk_dispaud_d = devm_clk_get(dev, "aclk_dispaud_d");
	if (IS_ERR(lpass_cmu.clk_aclk_dispaud_d)) {
		dev_err(dev, "aclk_dispaud_d clk not found\n");
		return PTR_ERR(lpass_cmu.clk_aclk_dispaud_d);
	}

	clk_prepare_enable(lpass_cmu.clk_aclk_dispaud_d);
	...
	ret = clk_set_parent(lpass_cmu.clk_mout_sclk_mi2s,
			lpass_cmu.clk_mout_aud_pll_user);
	if (ret) {
...
{
	writel(1, EXYNOS_PMU_GPIO_MODE_AUD_SYS_PWR_REG);
}

void lpass_release_pad_reg(void)
{
	writel(1 << 28, EXYNOS_PMU_PAD_RETENTION_AUD_OPTION);
	writel(1, EXYNOS_PMU_GPIO_MODE_AUD_SYS_PWR_REG);
}

/* Audio subsystem version */
enum {
	LPASS_VER_3475, /* Island */
	LPASS_VER_MAX
};
...
static int lpass_probe(struct platform_device *pdev)
{
	...
	dev_dbg(dev, "%s: regs_base = %08X (%08X bytes)\n",
...
static const struct of_device_id exynos_lpass_match[] = {
	{
		...
		.data		= &lpass_ver_data[LPASS_VER_3475],
	}, {},
};
