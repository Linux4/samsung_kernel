	...
	int div_id, int div)
{
	...
	case SAMSUNG_I2S_DIV_RCLK:
		...
		/* for the voice call in vibrater mode i2s register are need to
		 * be configure, since I2S CDclk is used by the codec */
	case  SAMSUNG_I2S_DIV_UPDATE:
		bfs = i2s->bfs;
		rfs = i2s->rfs;
		/* If already setup and running */
		if (any_active(i2s) && (get_rfs(i2s) != rfs || get_bfs(i2s) != bfs)) {
		...
		psr = (i2s->rclk_srcrate + (rfs / 2)) / div / rfs;
		writel(((psr - 1) << 8) | PSR_PSREN, i2s->addr + I2SPSR);
		dev_err(&i2s->pdev->dev,
			"RCLK_SRC=%luHz PSR=%u, RCLK=%dfs, BCLK=%dfs\n",
				i2s->rclk_srcrate, psr, rfs, bfs);
...
probe_exit:
	...
	/*  in Exynos7580,codec uses I2sSCDCLK as master clock,
	 *  The register setting of need to be saved, because in some condition
	 *  I2S registers are getting restored with even before it is stored
	 *  in the current driver i2s register is stored only if the dai is
	 * active.
	*/
	if (soc_is_exynos7580()) {
		i2s_reg_save(i2s);
...
static int samsung_i2s_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_SND_SAMSUNG_COMPR
		if (sec_dai->quirks & QUIRK_SEC_DAI_COMPR) {
			sec_dai->i2s_dai_drv.compress_dai = 1;
	...
	pri_dai = i2s_alloc_dai(pdev, false, quirks);
	if (!pri_dai) {
	...
	if (quirks & QUIRK_SEC_DAI) {
		sec_dai = i2s_alloc_dai(pdev, true, quirks);
		if (!sec_dai) {
