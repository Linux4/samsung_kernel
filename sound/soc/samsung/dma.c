...
static int preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	...
	buf->area = dma_alloc_attrs(pcm->card->dev, size, &buf->addr,
				    GFP_KERNEL | GFP_DMA32, &attrs);
#else
					...
					struct device_node *np)
{
	...
	if (of_find_property(np, dma_prop_iommu[stream], NULL)) {
...
#ifdef CONFIG_ARM64
		dma_set_attr(DMA_ATTR_WRITE_COMBINE, &attrs);
		buf->area = dma_alloc_attrs(pcm->card->dev, size, &dma_buf_pa,
					    GFP_KERNEL | GFP_DMA32, &attrs);
#else
		buf->area = dma_alloc_writecombine(pcm->card->dev, size,
		...
		if (ret) {
#ifdef CONFIG_ARM64
			dma_free_attrs(pcm->card->dev, size,
					buf->area, dma_buf_pa, &attrs);
#else
...
#ifdef CONFIG_ARM64
			dma_set_attr(DMA_ATTR_WRITE_COMBINE, &attrs);
			dma_free_attrs(pcm->card->dev, buf->bytes,
					buf->area, buf->addr, &attrs);
#else
