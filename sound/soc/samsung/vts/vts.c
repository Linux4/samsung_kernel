...
#define MIC2_INTERFACE
#define VTS_DUMP_MAGIC "VTS-LOG0"	/* 0x30474F4C2D535456ull */
...
#if !(defined(CONFIG_SOC_EXYNOS9820) || defined(CONFIG_SOC_EXYNOS9630))
	if (!on) {
#ifndef EMULATOR
...
static const char * const vts_cps_sel_texts[] = {"normal",
	"absolute"};
...
static const char * const vts_polarity_output_texts[] = {"right first", "left first"};
	...
	if (start) {
		...
		if (!(data->exec_mode & (0x1 << VTS_SOUND_DETECT_MODE))) {
			if (active_trigger == TRIGGER_SVOICE &&
				...
				memcpy(data->sram_base + SOUND_MODEL_GOOGLE_OFFSET,
					data->google_info.data, data->google_info.actual_sz);
...
#ifdef MIC2_INTERFACE
			ctrl_dmicif_3rd = readl(data->dmic_3rd_base +
					 VTS_DMIC_CONTROL_DMIC_IF);
			ctrl_dmicif_3rd &= ~(0x7 << VTS_DMIC_GAIN_OFFSET);
			writel((ctrl_dmicif_3rd |
				 (data->lpsdgain << VTS_DMIC_GAIN_OFFSET)),
				data->dmic_3rd_base + VTS_DMIC_CONTROL_DMIC_IF);
#endif
		}
...
int vts_set_dmicctrl(struct platform_device *pdev, int micconf_type, bool enable)
{
	...
	if (enable) {
		if (!data->micclk_init_cnt) {
...
#ifdef MIC2_INTERFACE
			ctrl_dmicif_3rd = readl(data->dmic_3rd_base + VTS_DMIC_CONTROL_DMIC_IF);
#endif
			if (data->dmic_if == APDM) {
				vts_cfg_gpio(dev, "amic_default");
				ctrl_dmicif &= ~(1 << VTS_DMIC_DMIC_SEL_OFFSET);	/* DMIC SEL : 0 */
				ctrl_dmicif |= (data->amicgain << VTS_DMIC_GAIN_OFFSET);
#ifdef MIC2_INTERFACE
				ctrl_dmicif_3rd &= ~(1 << VTS_DMIC_DMIC_SEL_OFFSET);
				ctrl_dmicif_3rd |= (data->amicgain << VTS_DMIC_GAIN_OFFSET);
#endif
				} else {
				vts_cfg_gpio(dev, "dmic_default");
				ctrl_dmicif |= (1 << VTS_DMIC_DMIC_SEL_OFFSET);		/* DMIC SEL : 1 */
				ctrl_dmicif |= (data->dmicgain << VTS_DMIC_GAIN_OFFSET);
#ifdef MIC2_INTERFACE
				ctrl_dmicif_3rd |= (1 << VTS_DMIC_DMIC_SEL_OFFSET);
				ctrl_dmicif_3rd |= (data->dmicgain << VTS_DMIC_GAIN_OFFSET);
#endif

				}
#ifdef MIC2_INTERFACE
			ctrl_dmicif_3rd |= (1 << VTS_DMIC_RCH_EN_OFFSET);
			ctrl_dmicif_3rd |= (1 << VTS_DMIC_LCH_EN_OFFSET);
			ctrl_dmicif_3rd |= (1 << VTS_DMIC_HPF_EN_OFFSET);
			ctrl_dmicif_3rd |= (1 << VTS_DMIC_SYS_SEL_OFFSET);
#endif
			/* CONFIG_SOC_EXYNOS9630 fixed INTERNAL_MICCLK */
			...
			writel(ctrl_dmicif, data->dmic_base + VTS_DMIC_CONTROL_DMIC_IF);
#ifdef MIC2_INTERFACE
			writel(ctrl_dmicif_3rd, data->dmic_3rd_base + VTS_DMIC_CONTROL_DMIC_IF);
#endif
...
#ifdef MIC2_INTERFACE
			/* reset VTS Gain to default */
			writel(0x0, data->dmic_3rd_base + VTS_DMIC_CONTROL_DMIC_IF);
			dev_info(dev, "%s Micclk 3rd setting DISABLED\n", __func__);
#endif
		}
...
static int vts_runtime_resume(struct device *dev)
{
	...
	dev_info(dev, "%s : VTS_IRQ_AP_SET_DRAM_BUFFER: OK\n", __func__);
	...
	dev_info(dev, "Time : %d.%d \n", values[1], values[2]);
	result = vts_start_ipc_transaction(dev, data,
		VTS_IRQ_AP_TEST_COMMAND, &values, 0, 2);
	if (result < 0) {
		dev_err(dev, "VTS_KERNEL_TIME ipc failed\n");
	}
...
static int samsung_vts_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_SOC_EXYNOS9820) || defined(CONFIG_SOC_EXYNOS9630)
	data->dmic_3rd_base = samsung_vts_devm_request_and_map(pdev,
		"dmic_3rd", NULL);
	...
	data->dmab.area = dmam_alloc_coherent(dev, BUFFER_BYTES_MAX,
		&data->dmab.addr, GFP_KERNEL);
	...
	data->dmab_model.area = dmam_alloc_coherent(dev,
		VTSDRV_MISC_MODEL_BIN_MAXSZ, &data->dmab_model.addr, GFP_KERNEL);
	...
	result = samsung_vts_devm_request_threaded_irq(pdev, "debuglog_bufzero",
			VTS_IRQ_VTS_DBGLOG_BUFZERO,
			vts_debuglog_bufzero_handler);
	if (result < 0)
		goto error;

	result = samsung_vts_devm_request_threaded_irq(pdev, "debuglog_bufone",
			VTS_IRQ_VTS_DBGLOG_BUFONE,
			vts_debuglog_bufone_handler);
	if (result < 0)
		goto error;

	result = samsung_vts_devm_request_threaded_irq(pdev, "audio_dump",
			VTS_IRQ_VTS_AUDIO_DUMP,
			vts_audiodump_handler);
	if (result < 0)
		goto error;
	...
	result = snd_soc_register_component(dev, &vts_component,
		vts_dai, ARRAY_SIZE(vts_dai));
	if (result < 0) {
	...
	for (i = 0; i < VTS_DUMP_LAST; i++) {
		data->p_dump[i].sram_log = kzalloc(
			VTS_SRAMLOG_SIZE_MAX+sizeof(VTS_DUMP_MAGIC), GFP_KERNEL);
