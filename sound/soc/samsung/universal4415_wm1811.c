...
#include "../../../arch/arm/mach-exynos/board-universal4415.h"
...
static void jack_set_type(struct wm1811_machine_priv *wm1811, int jack_type)
{
	...
	if (jack_type == SEC_HEADSET_4POLE) {
		...
		universal4415_micd_set_rate(wm1811->codec);
...
static void universal4415_micdet(void *data)
{
...
static void universal4415_mic_id(void *data, u16 status)
{
	struct wm1811_machine_priv *wm1811 = data;
	struct wm8994_priv *wm8994 = snd_soc_codec_get_drvdata(wm1811->codec);
...
static void universal4415_headset_stmr(struct work_struct *work)
{
	...
	snd_soc_update_bits(codec, WM8994_DAC1_LEFT_MIXER_ROUTING,
					WM8994_ADCL_TO_DAC1L_MASK, WM8994_ADCL_TO_DAC1L);
	snd_soc_update_bits(codec, WM8994_DAC1_RIGHT_MIXER_ROUTING,
					WM8994_ADCL_TO_DAC1R_MASK, WM8994_ADCL_TO_DAC1R);
}
	...
	if (wm8994->revision > 1) {
		...
		if (wm1811->use_jackdet_type) {
			...
			ret = wm8958_mic_detect(codec, &wm1811->jack, NULL,
					NULL, universal4415_mic_id, wm1811);
			if (ret < 0)
				dev_err(codec->dev, "Failed start detection: %d\n",
					ret);
		}
...
static int unviversal4415_card_suspend_pre(struct snd_soc_card *card)
{
...
static struct snd_soc_card universal4415 = {
	...
	.suspend_pre = unviversal4415_card_suspend_pre,
...
static int __devinit snd_universal4415_probe(struct platform_device *pdev)
{
	...
	wm1811->pll_out = UNIVERSAL4415_DEFAULT_SYNC_CLK;
