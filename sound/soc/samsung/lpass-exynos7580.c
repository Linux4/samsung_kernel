...
int lpass_set_clk_heirachy(struct device *dev)
{
	...
	lpass.clk_mi2s_aud_bclk = clk_get(dev, "mi2s_aud_bclk");
	if (IS_ERR(lpass.clk_mi2s_aud_bclk)) {
		dev_err(dev, "mi2s_aud_bclk\n");
		goto err7;
	}

	clk_prepare_enable(lpass.clk_mi2s_aud_bclk);
...
err8:
	clk_put(lpass.clk_mi2s_aud_bclk);
err7:
	clk_put(lpass.clk_dout_sclk_mixer);
err6:
