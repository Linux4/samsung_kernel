					...
					struct snd_pcm_hw_params *params)
{
	...
	case ABOX_UAIF_DAI_ID(0, 0):
		...
		if (ret < 0) {
			dev_err(card->dev, "codec_dai RT5691_PLL1_S_BCLK not set\n");
			return ret;
		}
		ret = snd_soc_dai_set_sysclk(codec_dai, RT5691_SCLK_S_PLL1, codec_pll_out, SND_SOC_CLOCK_IN);
		if (ret < 0) {
			dev_err(card->dev, "codec_dai RT5691_SCLK_S_PLL1 not set\n");
			return ret;
		}
		break;
	default:
...
static int exynos8825_rt5691_mainmic(struct snd_soc_dapm_widget *w,
...
static int exynos8825_rt5691_submic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8825_rt5691_headset_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
	struct snd_soc_component *component = NULL;
	struct rt5691_priv *rt5691 = NULL;

	component = exynos_drvdata.codec_comp;
	if (component)
		rt5691 = snd_soc_component_get_drvdata(component);

	dev_info(card->dev, "%s ev: %d, jack = %d\n", __func__, event, rt5691 ? rt5691->jack_type : -1);

	return 0;
}

static int exynos8825_rt5691_receiver(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8825_rt5691_speaker(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8825_rt5691_headphone(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
	struct snd_soc_component *component = NULL;
...
static int exynos8825_rt5691_bt_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8825_rt5691_bt_spk(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8825_rt5691_usb_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8825_rt5691_usb_spk(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8825_rt5691_fwd_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8825_rt5691_fwd_spk(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static const struct snd_soc_dapm_widget exynos_widgets[] = {
	SND_SOC_DAPM_MIC("MAINMIC", exynos8825_rt5691_mainmic),
	SND_SOC_DAPM_MIC("SUBMIC", exynos8825_rt5691_submic),
	SND_SOC_DAPM_MIC("HEADSETMIC", exynos8825_rt5691_headset_mic),
	SND_SOC_DAPM_SPK("RECEIVER", exynos8825_rt5691_receiver),
	SND_SOC_DAPM_SPK("SPEAKER", exynos8825_rt5691_speaker),
	SND_SOC_DAPM_HP("HEADPHONE", exynos8825_rt5691_headphone),
	SND_SOC_DAPM_MIC("BLUETOOTH MIC", exynos8825_rt5691_bt_mic),
	SND_SOC_DAPM_SPK("BLUETOOTH SPK", exynos8825_rt5691_bt_spk),
	SND_SOC_DAPM_MIC("USB MIC", exynos8825_rt5691_usb_mic),
	SND_SOC_DAPM_SPK("USB SPK", exynos8825_rt5691_usb_spk),
	SND_SOC_DAPM_MIC("FWD MIC", exynos8825_rt5691_fwd_mic),
	SND_SOC_DAPM_SPK("FWD SPK", exynos8825_rt5691_fwd_spk),
...
static const struct snd_soc_dapm_route exynos_routes[] = {
	{"VTS Virtual Output Mux", "MAINMIC", "MAINMIC"},
};
...
static const struct of_device_id exynos_sound_of_match[] = {
	{ .compatible = "samsung,exynos8825-rt5691", },
	{},
};
