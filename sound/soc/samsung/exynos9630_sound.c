/*
 *  Driver for Madera CODECs on Exynos9630
 *
...
static int exynos9630_late_probe(struct snd_soc_card *card)
{
	...
	if (IS_ENABLED(CONFIG_SND_SOC_TAS2562)) {
...
static const struct snd_soc_dapm_widget exynos9630_supply_widgets[] = {
...
static int exynos9630_probe(struct snd_soc_card *card)
{
	int i;

	return 0;

	for (i = 0; i < ARRAY_SIZE(exynos9630_supply_widgets); i++) {
		const struct snd_soc_dapm_widget *w;
		struct regulator *r;

		w = &exynos9630_supply_widgets[i];
		switch (w->id) {
...
static struct snd_soc_card exynos9630_madera = {
	.name = "Exynos9630-Madera",
	.owner = THIS_MODULE,
	.dai_link = exynos9630_dai,
	.num_links = ARRAY_SIZE(exynos9630_dai),

	.probe = exynos9630_probe,
...
{
	struct snd_soc_card *card = &exynos9630_madera;
...
static int exynos9630_audio_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &exynos9630_madera;
	struct madera_drvdata *drvdata = card->drvdata;
...
static int exynos9630_audio_remove(struct platform_device *pdev)
{
...
static const struct of_device_id exynos9630_audio_of_match[] = {
	{ .compatible = "samsung,exynos9630-madera", },
	{},
};
MODULE_DEVICE_TABLE(of, exynos9630_audio_of_match);
#endif /* CONFIG_OF */

static struct platform_driver exynos9630_audio_driver = {
	.driver		= {
		.name	= "exynos9630-audio",
		.owner	= THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = of_match_ptr(exynos9630_audio_of_match),
	},

	.probe		= exynos9630_audio_probe,
	.remove		= exynos9630_audio_remove,
};

module_platform_driver(exynos9630_audio_driver);

MODULE_DESCRIPTION("ALSA SoC Exynos9630 Audio Driver");
MODULE_AUTHOR("Charles Keepax <ckeepax@opensource.wolfsonmicro.com>");
MODULE_AUTHOR("Gyeongtaek Lee <gt82.lee@samsung.com>");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:exynos9630-madera");
