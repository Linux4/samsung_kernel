...
static unsigned int abox_dma_current_address(struct abox_dma_data *data)
{
	return (snd_soc_component_read(data->cmpnt, DMA_REG_STATUS_ADD) &
			ABOX_DMA_BUF_CURRENT_ADDRESS_MASK) << 4;
}

unsigned int abox_dma_read_pointer(struct abox_dma_data *data)
{
	unsigned int addr, str, end;

	addr = abox_dma_current_address(data);
	str = snd_soc_component_read(data->cmpnt, DMA_REG_BUF_STR);
	end = snd_soc_component_read(data->cmpnt, DMA_REG_BUF_END);

	return (str <= addr && addr < end) ? (addr - str) : 0;
}
...
static irqreturn_t abox_dma_err(int irq, void *dev_id)
{
	...
	abox_err(dev, "Error\n");
	for (reg = DMA_REG_CTRL0; reg <= data->dma_reg_max; reg += 4) {
		val = snd_soc_component_read(cmpnt, reg);
...
static const char * const abox_dma_func_status_texts[] = {
	"Idle", "Normal", "Fade out", "Mute", "Fade in",
};

static const unsigned int abox_dma_func_status_values[] = {
	0, 1, 2, 3, 5,
};

static SOC_VALUE_ENUM_SINGLE_DECL(abox_dma_func_status_enum, DMA_REG_STATUS_FUNC,
		DMA_VOL_STATUS_SHIFT, DMA_VOL_STATUS_MASK,
		abox_dma_func_status_texts, abox_dma_func_status_values);
#endif
...
#if !IS_ENABLED(CONFIG_SOC_S5E8535) && !IS_ENABLED(CONFIG_SOC_S5E8835)
	SOC_ENUM("Func Status", abox_dma_func_status_enum),
#endif
...
{
	struct abox_dma_data *dma_data = snd_soc_component_get_drvdata(cmpnt);

	dma_data->dma_reg_max = DMA_REG_STATUS_ADD;
#if !IS_ENABLED(CONFIG_SOC_S5E8535) && !IS_ENABLED(CONFIG_SOC_S5E8835)
	dma_data->dma_reg_max = DMA_REG_STATUS_FUNC;
#endif
...
static SOC_VALUE_ENUM_SINGLE_DECL(abox_udma_wr_dbg_src_enum, DMA_REG_CTRL,
		ABOX_DMA_DEBUG_SRC_L, ABOX_DMA_DEBUG_SRC_MASK >> ABOX_DMA_DEBUG_SRC_L,
		abox_udma_wr_dbg_src_enum_texts, abox_udma_wr_dbg_src_enum_values);
...
static int abox_udma_wr_dbg_probe(struct snd_soc_component *cmpnt)
{
	int ret;
