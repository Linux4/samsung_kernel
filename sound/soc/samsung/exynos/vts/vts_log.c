...
static void vts_log_flush_work_func(struct work_struct *work)
{
	...
	vts_dev_dbg(dev, "%s: %d %d %d %d\n", __func__, __LINE__,
		logbuf_index, vts_is_running(), log_buffer->size);

	if (!(vts_is_running()) || !(log_buffer->size))
		return;
	...
	if (data->fw_logger_enabled && data->fw_log_obj) {
		...
		memlog_write(data->fw_log_obj, MEMLOG_LEVEL_INFO,
			(log_buffer->addr+log_buffer->size*logbuf_index), log_buffer->size);
		vts_dev_dbg(dev, "fw log sync : %d", memlog_sync_to_file(data->fw_log_obj));
		pm_runtime_put_sync(dev);
	}
}
