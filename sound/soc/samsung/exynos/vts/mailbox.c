...
static bool mailbox_vts_is_on;

void  mailbox_update_vts_is_on(bool on)
{
	mailbox_vts_is_on = on;
}
EXPORT_SYMBOL(mailbox_update_vts_is_on);
...
int mailbox_generate_interrupt(const struct platform_device *pdev, int hw_irq)
{
	...
	dev_dbg(&pdev->dev, "%s: writel(%lx, %lx)", __func__,
		BIT(hw_irq),
		(unsigned long)virt_to_phys(data->sfr_base + MAILBOX_INTGR0));
		...
		const u32 *values, int start, int count)
{
	struct mailbox_data *data = platform_get_drvdata(pdev);
	u32 __iomem *sfr =
		data->sfr_base + MAILBOX_ISSR0 + (start * sizeof(u32));

	dev_dbg(&pdev->dev, "%s(%p, %p, %d, %d)", __func__,
		pdev, values, start, count);
	while (count--)
...
static void mailbox_handle_irq(struct irq_desc *desc)
{
	...
	struct irq_chip_generic *gc =
		vts_irq_get_domain_generic_chip(irq_domain, 0);
	...
	while (stat) {
		...
		generic_handle_irq(irq_find_mapping(irq_domain,
			gc->irq_base + hwirq));
		stat &= ~(1 << hwirq);
	}
...
/* Copy of irq_gc_set_wake().
 * Because it isn't exported, it can't be called in module.
 */
static int mailbox_set_wake(struct irq_data *d, unsigned int on)
{
...
static void __iomem *devm_request_and_ioremap(
...
static int samsung_mailbox_probe(struct platform_device *pdev)
{
	...
	data->irq_domain = irq_domain_add_linear(np,
		MAILBOX_INT1_OFFSET + MAILBOX_INT1_SIZE,
		&vts_irq_generic_chip_ops, NULL);
	if (IS_ERR_OR_NULL(data->irq_domain)) {
		dev_err(dev, "Failed to add irq domain\n");
		return -ENOMEM;
	}

	result = vts_irq_alloc_domain_generic_chips(data->irq_domain,
		MAILBOX_INT1_OFFSET + MAILBOX_INT1_SIZE,
		1, "mailbox_irq_chip",
	...
	gc = vts_irq_get_domain_generic_chip(data->irq_domain, 0);
	...
	gc->chip_types[0].chip.irq_ack		= vts_irq_gc_ack_set_bit;
	gc->chip_types[0].chip.irq_mask		= vts_irq_gc_mask_set_bit;
	gc->chip_types[0].chip.irq_unmask	= vts_irq_gc_mask_clr_bit;
	gc->chip_types[0].regs.mask		= MAILBOX_INTMR1;
	gc->chip_types[0].regs.ack		= MAILBOX_INTCR1;
	gc->wake_enabled			= 0x0000FFFF;
	gc->chip_types[0].chip.irq_set_wake	= mailbox_set_wake;
...
static int samsung_mailbox_remove(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "%s\n", __func__);
	vts_irq_remove_generic_chip(
		vts_irq_get_domain_generic_chip(mailbox_data->irq_domain, 0),
