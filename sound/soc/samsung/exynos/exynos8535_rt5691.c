...
#include "../../codecs/rt5691.h"
#include "../../codecs/rt5691_sysfs_cb.h"
#if IS_ENABLED(CONFIG_SND_SOC_SMA1305)
#include "../../codecs/bigdata_sma_sysfs_cb.h"
#endif

#define AW88230_DAI_ID			0x88230
...
#define MAX_I2C_ERR_COUNT 1000

struct _drvdata {
	struct device *dev;
	struct wakeup_source *ws;
	struct snd_soc_jack rt5691_headset;
	struct snd_soc_component *codec_comp;
	struct snd_soc_component *amp_comp;
};
...
struct _i2c_err_priv {
	struct rt5691_priv *rt5691;
	int err_cnt;
};
struct _i2c_err_priv rt5691_i2c_err_priv;

static int rt5691_i2c_error_cb(void *context)
{
	struct _i2c_err_priv *i2c_err = context;
	struct rt5691_priv *rt5691 = i2c_err->rt5691;
	struct snd_soc_component *component = rt5691->component;

	if (i2c_err->err_cnt < MAX_I2C_ERR_COUNT)
		i2c_err->err_cnt++;

	dev_err(component->dev, "%s: i2c error count %d\n", __func__, i2c_err->err_cnt);
...
static int uaif0_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	int ret = 0;
	struct rt5691_priv *rt5691;
	struct rt5691_i2c_err *rt5691_i2c_err;
	...
	if (!strstr(component->name, "rt5691")) {
		dev_err(card->dev, "%s: no rt5691 component\n", __func__);
		return ret;
	}

	drvdata->codec_comp = component;
	rt5691 = snd_soc_component_get_drvdata(component);
	rt5691_i2c_err = devm_kzalloc(component->dev, sizeof(struct rt5691_i2c_err), GFP_KERNEL);
	rt5691_i2c_err->i2c_err_priv = &rt5691_i2c_err_priv;
	rt5691_i2c_err->i2c_err_cb = rt5691_i2c_error_cb;
	rt5691->i2c_err = rt5691_i2c_err;
	rt5691_i2c_err_priv.rt5691 = rt5691;
	...
	snd_soc_dapm_ignore_suspend(dapm, "AIF Playback");
	snd_soc_dapm_ignore_suspend(dapm, "AIF Capture");
	snd_soc_dapm_sync(dapm);

	register_rt5691_sysfs_cb(component);
...
static int uaif1_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	component = codec_dai->component;
	if (!component) {
		dev_err(card->dev, "%s: no amp component\n", __func__);
		return ret;
	}

	if (strstr(component->name, "aw882xx_smartpa")) {
		drvdata->amp_comp = component;
		dapm = snd_soc_component_get_dapm(component);

		snd_soc_dapm_ignore_suspend(dapm, "Speaker_Playback");
		snd_soc_dapm_ignore_suspend(dapm, "Speaker_Capture");
		snd_soc_dapm_ignore_suspend(dapm, "audio_out");
		snd_soc_dapm_ignore_suspend(dapm, "iv_in");
		snd_soc_dapm_sync(dapm);
	}

#if IS_ENABLED(CONFIG_SND_SOC_SMA1305)
	if (strstr(component->name, "sma1305"))
		register_sma1305_bigdata_cb(component);
#endif
					...
					struct snd_pcm_hw_params *params)
{
	...
	case ABOX_UAIF_DAI_ID(0, 0):
		...
		ret = snd_soc_dai_set_pll(codec_dai, 0, RT5691_PLL1_S_BCLK, codec_pll_in, codec_pll_out);
		if (ret < 0) {
			dev_err(card->dev, "codec_dai RT5691_PLL1_S_BCLK1 not set\n");
			return ret;
		}
		ret = snd_soc_dai_set_sysclk(codec_dai, RT5691_SCLK_S_PLL1, codec_pll_out, SND_SOC_CLOCK_IN);
		if (ret < 0) {
			dev_err(card->dev, "codec_dai RT5691_PLL1_S_PLL1 not set\n");
			return ret;
		}
		break;
...
static int exynos8535_rt5691_mainmic(struct snd_soc_dapm_widget *w,
...
static int exynos8535_rt5691_submic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8535_rt5691_headsetmic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
	struct _drvdata *drvdata = &exynos_drvdata;
...
static int exynos8535_rt5691_receiver(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8535_rt5691_speaker(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8535_rt5691_headphone(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
	struct _drvdata *drvdata = &exynos_drvdata;
...
static int exynos8535_rt5691_bt_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8535_rt5691_bt_out(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8535_rt5691_usb_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8535_rt5691_usb_out(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8535_rt5691_fwd_mic(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static int exynos8535_rt5691_fwd_out(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
...
static const struct snd_soc_dapm_widget exynos_widgets[] = {
	SND_SOC_DAPM_MIC("MAINMIC", exynos8535_rt5691_mainmic),
	SND_SOC_DAPM_MIC("SUBMIC", exynos8535_rt5691_submic),
	SND_SOC_DAPM_MIC("HEADSETMIC", exynos8535_rt5691_headsetmic),
	SND_SOC_DAPM_SPK("RECEIVER", exynos8535_rt5691_receiver),
	SND_SOC_DAPM_SPK("SPEAKER", exynos8535_rt5691_speaker),
	SND_SOC_DAPM_HP("HEADPHONE", exynos8535_rt5691_headphone),
	SND_SOC_DAPM_MIC("BLUETOOTH MIC", exynos8535_rt5691_bt_mic),
	SND_SOC_DAPM_SPK("BLUETOOTH SPK", exynos8535_rt5691_bt_out),
	SND_SOC_DAPM_MIC("USB MIC", exynos8535_rt5691_usb_mic),
	SND_SOC_DAPM_SPK("USB SPK", exynos8535_rt5691_usb_out),
	SND_SOC_DAPM_MIC("FWD MIC", exynos8535_rt5691_fwd_mic),
	SND_SOC_DAPM_SPK("FWD SPK", exynos8535_rt5691_fwd_out),
...
static const struct of_device_id exynos_sound_of_match[] = {
	{ .compatible = "samsung,exynos8535-rt5691", },
	{},
};
