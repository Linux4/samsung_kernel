...
#define SYSREG_AUD_ENABLE_DMIC_AUD (-1)
#endif
...
static int slif_clk_set_rate_enable(struct slif_data *data, struct clk_path* path)
{
	struct device *dev = data->dev;
	int i = 0;
	...
	for (i = 0; (path[i].index >= 0) && (i < CLK_MAX_CNT); i++) {
		...
		if (result < 0) {
			slif_err(dev, "%s: set_rate error index: %d, value: %d, result: %d\n",
		...
		if (!data->clk_src_list[index].enabled) {
			...
			if (result < 0) {
				slif_err(dev, "%s: enable error index: %d, value: %d, result: %d\n",
						__func__, index, value, result);
				continue;
			}
		...
		slif_info(dev,"%s: index:%d, %s: %d -> %d, enabled: %d -> %d\n",
				__func__, index, data->clk_name_list[index], prev_val,
...
void slif_clk_disable(struct slif_data *data, struct clk_path* path)
{
	struct device *dev = data->dev;

	int i, index, value;

	slif_info(dev,"%s: enter\n",
			__func__);
...
#error "SLIF_SOC_VERSION is not defined"
	...
	data->bclk_path[0].value = (clk_val * data->channels) / SLIF_MAX_CHANNEL;
	slif_info(dev, "%s: (%d)\n",
			"bclk", data->bclk_path[0].value);
	slif_clk_set_rate_enable(data, data->bclk_path);
		...
		struct slif_data *data)
{
	...
	slif_clk_disable(data, data->aud_clk_path);
	slif_clk_disable(data, data->bclk_path);
...
static int get_clk_path(struct slif_data *data, char *name, struct clk_path* path)
{
	struct device *dev = data->dev;
	...
	slif_info(dev, "%s: prop_len: %d\n", __func__, prop_len);

	if (!prop) {
		slif_warn(dev, "%s: %s DT property missing\n", __func__, name);
	} else {
		for (i = 0; i < prop_len / 2; i++) {
			p = of_prop_next_u32(prop, p, &value);
			if (!p) {
				slif_warn(dev, "%s: %s index parsing error (%d)\n", __func__, name, i);
				path[i].index = -1;
				return 0;
			}
			path[i].index = value;
			...
			p = of_prop_next_u32(prop, p, &value);
			if (!p) {
				slif_warn(dev, "%s: %s value parsing error (%d)\n", __func__, name, i);
				return 0;
			}
			path[i].value = value;
		}
...
static int get_clk_name_list(struct slif_data *data)
{
	struct device *dev = data->dev;
	struct device_node *np = dev->of_node;
	struct property *prop;
	...
	if (prop) {
		for (i = 0; ((cur = of_prop_next_string(prop, cur)) != NULL) && (i < CLK_MAX_CNT); i++) {
			...
			slif_info(dev, "%s: clk_name_list: %s\n", __func__, data->clk_name_list[i]);
		}
	}
...
int slif_soc_probe(struct slif_data *data)
{
	...
	for (i = 0; i < CLK_MAX_CNT; i++) {
		...
		data->clk_src_list[i].clk_src = devm_clk_get_and_prepare(dev, clk_name);
		data->clk_src_list[i].enabled = false;
		if (IS_ERR(data->clk_src_list[i].clk_src)) {
			slif_warn(dev, "clk_src[%d] is NULL\n", i);
			data->clk_src_list[i].clk_src = NULL;
		}
	}

	result = get_clk_path(data, "slif-aud-clk", data->aud_clk_path);
	if (result) {
		slif_err(dev, "%s: get vts_init_clk property error\n", __func__);
	}

	result = get_clk_path(data, "slif-bclk", data->bclk_path);
	if (result) {
		slif_err(dev, "%s: get vts_boot_clk property error\n", __func__);
	}

	result = get_clk_name_list(data);
	if (result) {
		slif_warn(dev, "%s: get_clk_name_list error\n", __func__);
	}
