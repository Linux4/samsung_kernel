...
void Speaker_ClassD_Open(uint32 in_trimming)
{
	...
	Ana_Set_Reg(SPK_ANA_CON0, SPK_D_TRIM_INDEX << 11, 0xffff);	/* Set SPK_D_TRIM_INDEX PGA gain */
	Ana_Set_Reg(SPK_CON9, 0x2000, 0xffff);	/* Set Fast Vcm mode */

	if (in_trimming == 1)
		Ana_Set_Reg(SPK_CON0, 0x3008, 0xffff);	/* No turn on speaker amp when trimming */
	else
	...
	if (in_trimming == 1)
		Ana_Set_Reg(SPK_CON0, 0x3000, 0xffff);	/* Turn off trim,(set to class D mode) */
	else
	...
	if (in_trimming == 1)
		Ana_Set_Reg(SPK_CON0, 0x3000, 0xffff);	/* set 0dB amplifier gain */
	else
...
void Speaker_ClassD_close(uint32 in_trimming)
{
	...
	Ana_Set_Reg(SPK_ANA_CON0, SPK_D_TRIM_INDEX << 11, 0xffff);
	/* Set SPK_D_TRIM_INDEX PGA gain(level when trimming) */
