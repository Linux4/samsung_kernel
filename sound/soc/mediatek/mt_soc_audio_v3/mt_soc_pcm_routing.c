...
#define TRIM_NUM (10)
	...
	case AUDIO_OFFSET_TRIM_MUX_HPR:{
			...
			if ((out_device == AUDIO_ANALOG_DEVICE_OUT_HEADSETL) ||
				...
				setHpGain(HP_TRIM_INDEX); /* 8: 0dB */
			} else {
#ifdef CONFIG_MTK_SPEAKER
#ifdef TRIM_WITH_SPK
				OpenAnalogSpeaker(true);
				setIntSpkGainZero();
#else
				Ana_Set_Reg(ZCD_CON4, 0x0505, 0xffff); /* set IV buffer gain to 0dB */
#endif
#else
				setLineOutGainZero();
#endif
			setHpGain(SPKHP_TRIM_INDEX); /* 18: -10dB */
			}
			break;
		}
	default:
...
#ifdef TRIM_WITH_SPK
	if ((out_device == AUDIO_ANALOG_DEVICE_OUT_SPEAKER_HEADSET_R) ||
		(out_device == AUDIO_ANALOG_DEVICE_OUT_SPEAKER_HEADSET_L)) {
		Ana_Set_Reg(AUDDEC_ANA_CON0, (0x2 << 11) | (0x1 << 9), 0x1e00);
		/* R hp input mux select "Audio playback", L hp input mux select "LoudSPK playback"  */
		Ana_Set_Reg(SPK_ANA_CON3, 0x4 << 6, 0x01c0);
	...
	for (i = 0; i < (TRIM_NUM + 2); i++) {
		Buffer_tmp[i] = PMIC_IMM_GetOneChannelValue(MT6328_AUX_CH9, off_counter, 0);
	...
	if ((out_device == AUDIO_ANALOG_DEVICE_OUT_SPEAKER_HEADSET_R) ||
		(out_device == AUDIO_ANALOG_DEVICE_OUT_SPEAKER_HEADSET_L)) {
		OpenAnalogSpeaker(false);
		/* restore IV buf mux and hp mux */
		Ana_Set_Reg(AUDDEC_ANA_CON0, (0x0 << 11) | (0x0 << 9), 0x1e00);
		/* R hp input mux select "Open", L hp input mux select "Open"  */
		Ana_Set_Reg(SPK_ANA_CON3, 0x0 << 6, 0x01c0);
		/* Set IV buffer MUX select */
	}
#endif
...
#else
				Ana_Set_Reg(ZCD_CON4, 0x0505, 0xffff); /* set IV buffer gain to 0dB */
				Ana_Set_Reg(SPK_ANA_CON0, SPK_D_TRIM_INDEX << 11, 0x7800);
	...
	for (i = 0; i < (TRIM_NUM + 2); i++) {
		Buffer_tmp[i]  = PMIC_IMM_GetOneChannelValue(MT6328_AUX_CH9, on_counter, 0);
	...
	if ((out_device == AUDIO_ANALOG_DEVICE_OUT_HEADSETL) ||
		...
		/* spk case need to inverse the dc offset!!! */
		/* mHplSpkOffset = Const_DC_OFFSET - (Buffer_on_value - Buffer_offl_value); */
		mHplSpkOffset = Const_DC_OFFSET - (Buffer_on_value - Buffer_offl_value) * 25905/8192;
		/*  25905/8192=3.16227766, for -10dB HP gain */
	...
	setOffsetTrimMux(AUDIO_OFFSET_TRIM_MUX_HPR);
	/* set IV buf mux and hp mux */
	if (out_device == AUDIO_ANALOG_DEVICE_OUT_SPEAKER_HEADSET_L ||
		out_device == AUDIO_ANALOG_DEVICE_OUT_SPEAKER_HEADSET_R) {
		Ana_Set_Reg(AUDDEC_ANA_CON0, (0x2 << 11) | (0x1 << 9), 0x1e00);
		/* R hp input mux select "Audio playback", L hp input mux select "LoudSPK playback"  */
		Ana_Set_Reg(SPK_ANA_CON3, 0x4 << 6, 0x01c0);
		/* Set IV buffer MUX select */
	...
	if ((out_device == AUDIO_ANALOG_DEVICE_OUT_HEADSETL) ||
		...
		/* mHprSpkOffset = (Buffer_on_value - Buffer_offr_value)* 25905/8192 + Const_DC_OFFSET ; */
		/* 25905/8192=3.16227766, for -10dB HP gain */
		mHprSpkOffset = (mHprOffset - Const_DC_OFFSET) * 25905/8192 + Const_DC_OFFSET;
