...
static int sia81xx_probe(struct platform_device *pdev)
{
	...
	if(0 == disable_pin) {
		...
		if (1 == sia81xx->en_dyn_id)
		{
			...
			ret = gpio_get_value(id_pin);
                        if (0 == ret)
                            pr_err("[  err][%s] %s: Audio PA is SIA8102 get gpio_id = %d!!!", LOG_FLAG, __func__,ret);
                        else if (1 == ret) {
                            pr_err("[  err][%s] %s: Audio PA is not SIA8102 get gpio_id = %d!!!!", LOG_FLAG, __func__,ret);
			    flag_id = 1;
                            ret =  -ENODEV;
                            goto put_dev_out;
                        }
