...
int get_trim_buffer_diff(int channels)
{
	...
	if (channels != AUDIO_OFFSET_TRIM_MUX_HPL &&
		...
		pr_info("%s Not support this channels = %d\n", __func__, channels);
		return 0;
	}
	...
	pr_info("#diffValue(%d), onValue(%d), offValue(%d)\n", diffValue, onValue, offValue);
...
static int GetAudioTrimOffsetAverage(int *buffer_value, int trim_num)
{
	...
	for (i = 0; i < trim_num+5; i++) {
		for (j = i+1; j < trim_num+6; j++) {
	...
	tmp = 0;
	/* Discard 3 biggest and 3 smallest values */
	for (i = 0; i <  trim_num; i++)
		tmp = tmp + buffer_value[i+3];
...
int get_audio_trim_offset(int channel)
{
	...
	const int kTrimTimes = 26;
	int counter = 0, averageOffset = 0, averageTimes = 0;
	int trimOffset[kTrimTimes];

	if (channel != AUDIO_OFFSET_TRIM_MUX_HPL &&
	    channel != AUDIO_OFFSET_TRIM_MUX_HPR){
		pr_info("%s Not support channel(%d)\n", __func__, channel);
		return 0;
	}
	...
	averageTimes = kTrimTimes - 6;
	averageOffset = GetAudioTrimOffsetAverage(trimOffset, averageTimes);
	pr_info("[Average %d times] averageOffset = %d\n", averageTimes, averageOffset);
