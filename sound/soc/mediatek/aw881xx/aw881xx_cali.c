...
void aw881xx_set_cali_re_to_dsp(struct aw881xx_cali_attr *cali_attr)
{
	...
	pr_err("%s:re to cali_attr->cali_re :%d dsp_ra =%d\n",
			__func__, cali_attr->cali_re,dsp_ra);
				...
				const char *buf, size_t count)
{
	...
	pr_err("%s:re to cali_attr->cali_re :%d \n",
				...
				struct device_attribute *attr, char *buf)
{
	...
	struct aw881xx_cali_attr *cali_attr = &aw881xx->cali_attr;
	ssize_t len = 0;
	// Bug627652 fujiawen.wt,add,20210308,add smartpa cali
				...
				struct device_attribute *attr, char *buf)
{
	...
	temp = (cali_attr->re*4096/1000);
	imped_integer=cali_attr->re/1000;
	imped_decimals=cali_attr->re%1000;
	if((temp>=27852)&&(temp<=41779))
		result=1;
	else
		result=0;

	len += snprintf(buf + len, PAGE_SIZE - len,
			"re=%u,impedance=%d.%d,result=%d\n", temp,imped_integer,imped_decimals,result);
	//- Bug627652 fujiawen.wt,add,20210308,add smartpa cali
	return len;
}
