...
static uint16_t aw88194_base_addr[AW88194_BASE_ADDR_MAX] = {
 ...
 *
 ******************************************************/
static int aw88194_i2c_writes(struct aw881xx *aw88194,
	uint8_t reg_addr, uint8_t *buf, uint16_t len)
{
	int ret = -1;

	ret = aw881xx_i2c_writes(aw88194, reg_addr, buf, len);
	if (ret < 0)
...
/*
static int aw88194_i2c_reads(struct aw881xx *aw88194,
	uint8_t reg_addr, uint8_t *buf, uint16_t len)
{
	int ret = -1;

	ret = aw881xx_i2c_reads(aw88194, reg_addr, buf, len);
	if (ret < 0)
...
static int aw88194_i2c_write(struct aw881xx *aw88194,
	uint8_t reg_addr, uint16_t reg_data)
{
	int ret = -1;

	ret = aw881xx_i2c_write(aw88194, reg_addr, reg_data);
	if (ret < 0)
...
static int aw88194_i2c_read(struct aw881xx *aw88194,
	uint8_t reg_addr, uint16_t *reg_data)
{
	int ret = -1;

	ret = aw881xx_i2c_read(aw88194, reg_addr, reg_data);
	if (ret < 0)
...
static int aw88194_i2c_write_bits(struct aw881xx *aw88194,
	uint8_t reg_addr, uint16_t mask, uint16_t reg_data)
{
	int ret = -1;

	ret = aw881xx_i2c_write_bits(aw88194, reg_addr, mask, reg_data);
	if (ret < 0)
...
static int aw88194_get_sysint(struct aw881xx *aw88194, uint16_t *sysint)
{
	...
	ret = aw88194_i2c_read(aw88194, AW88194_REG_SYSINT, &reg_val);
	if (ret < 0)
...
static int aw88194_get_icalk(struct aw881xx *aw88194, int16_t *icalk)
{
	...
	ret = aw88194_i2c_read(aw88194, AW88194_REG_EFRM, &reg_val);
	reg_icalk = (uint16_t) reg_val & AW88194_EF_ISENSE_GAINERR_SLP_MASK;

	if ((reg_icalk & AW88194_EF_ISENSE_GAINERR_SLP_PROCT) !=
		AW88194_EF_ISENSE_GAINERR_SLP_PROCT)
		reg_icalk &= AW88194_EF_ISENSE_GAINERR_SLP_PROCT_MASK;
...
static int aw88194_get_vcalk(struct aw881xx *aw88194, int16_t *vcalk)
{
	...
	ret = aw88194_i2c_read(aw88194, AW88194_REG_EFRL, &reg_val);
	reg_val = reg_val >> AW88194_EF_VSENSE_GAIN_SHIFT;

	if ((reg_val & AW88194_EF_VSENSE_GAIN_PROCT) !=
		AW88194_EF_VSENSE_GAIN_PROCT)
		reg_val &= AW88194_EF_VSENSE_GAIN_PROCT_MASK;
...
static int aw88194_start(struct aw881xx *aw88194)
{
	int ret = -1;
	...
	if (ret < 0) {
		...
		if ((aw88194->monitor_flag) &&
			(aw88194->spk_rcv_mode == AW881XX_SPEAKER_MODE)) {
...
static void aw88194_stop(struct aw881xx *aw88194)
{
	pr_debug("%s: enter\n", __func__);
	...
	struct aw88194_container *aw88194_cont, uint16_t base)
{
	...
	for (i = 0; i < aw88194_cont->len;
		...
		aw88194_i2c_writes(aw88194, AW88194_REG_DSPMDAT,
...
static void aw88194_dsp_cfg_loaded(const struct firmware *cont, void *context)
{
	...
	if (ret < 0) {
        ...
        if (aw88194->clk_div2_flag == 1){
            aw88194_i2c_write(aw88194, 0x65, 0x3f07);
        } else {
            aw88194_i2c_write(aw88194, 0x65, 0x7f07);
        }
...
static void aw88194_reg_container_update(struct aw881xx *aw88194,
	struct aw88194_container *aw88194_cont)
{
	...
	for (i = 0; i < aw88194_cont->len; i += 4) {
		reg_addr = (aw88194_cont->data[i + 1] << 8) +
			aw88194_cont->data[i + 0];
		reg_val = (aw88194_cont->data[i + 3] << 8) +
			aw88194_cont->data[i + 2];
		pr_debug("%s: reg=0x%04x, val = 0x%04x\n",
			__func__, reg_addr, reg_val);
		aw88194_i2c_write(aw88194,
...
static int aw88194_load_reg(struct aw881xx *aw88194)
{
	pr_info("%s: enter\n", __func__);
...
static void aw88194_cold_start(struct aw881xx *aw88194)
{
	int ret = -1;
...
void aw88194_smartpa_cfg(void *aw881xx, bool flag)
{
	struct aw881xx *aw88194 = (struct aw881xx *)aw881xx;
	int ret = -1;
	...
	aw88194->work_flag = flag;
	if (flag == true) {
		if ((aw88194->init == AW88194_INIT_ST)
			|| (aw88194->init == AW88194_INIT_NG)) {
...
void aw88194_get_volume(void *aw881xx, unsigned int *vol)
{
	...
	*vol = (reg_val >> AW88194_VOL_REG_SHIFT);
}
...
void aw88194_set_volume(void *aw881xx, unsigned int vol)
{
	...
	/* cal real value */
	reg_val = (vol << AW88194_VOL_REG_SHIFT);

	/* write value */
	aw88194_i2c_write_bits(aw88194, AW88194_REG_HAGCCFG7,
...
int aw88194_update_hw_params(void *aw881xx)
{
    ...
    pr_info("%s: rate:[%d],width:[%d]\n", __func__,\
            aw88194->rate, aw88194->rate);
    if ((aw88194->rate == 32000) || (aw88194->rate == 16000) || \
        (aw88194->rate == 8000)){
            aw88194->clk_div2_flag = 1;
            aw88194_i2c_write(aw88194, 0x65, 0x3f07);
        } else {
            aw88194->clk_div2_flag = 0;
            aw88194_i2c_write(aw88194, 0x65, 0x7f07);
        }
	...
	struct aw881xx *aw88194, uint16_t *reg_val)
{
	...
	ret = aw88194_i2c_read(aw88194, AW88194_REG_HAGCCFG7, reg_val);
	...
	uint16_t gain_value)
{
	...
	ret = aw88194_i2c_write(aw88194, AW88194_REG_HAGCCFG7, reg_val);
	if (ret < 0)
		return ret;
	...
	struct aw881xx *aw88194, uint16_t *reg_val)
{
	...
	ret = aw88194_i2c_read(aw88194, AW88194_REG_SYSCTRL2, reg_val);
	...
	uint16_t bst_ipeak)
{
	...
	ret = aw88194_i2c_write(aw88194, AW88194_REG_SYSCTRL2, reg_val);
	if (ret < 0)
		return ret;
	...
	uint16_t *voltage)
{
	...
	ret = aw88194_i2c_read(aw88194, AW88194_REG_VBAT, voltage);
	if (ret < 0)
		return ret;
	...
	struct aw881xx *aw88194, int16_t *temp)
{
	...
	ret = aw88194_i2c_read(aw88194, AW88194_REG_TEMP, &reg_val);
	if (ret < 0)
		return ret;
	...
	uint16_t *bst_ipeak, uint16_t *gain_db)
{
	...
	if (voltage > AW88194_VOL_LIMIT_40) {
		...
		*bst_ipeak = AW88194_IPEAK_NONE;
		*gain_db = AW88194_GAIN_NONE;
	}
