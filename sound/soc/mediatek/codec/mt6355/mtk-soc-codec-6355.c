...
void CalculateDCCompenForEachdB_L(void)
{
	...
	for (i = 0; i < MAX_HP_GAIN_LEVEL; i++) {  /* 0:8dB, 1:7dB, ...,  18:-10dB */
		...
		pr_info("mHplOffset_CompenBydB[%d]=%d\n", i, mHplOffset_CompenBydB[i]);
	}
}
...
void CalculateDCCompenForEachdB_R(void)
{
	...
	for (i = 0; i < MAX_HP_GAIN_LEVEL; i++) {  /* 0:8dB, 1:7dB, ...,  18:-10dB */
		...
		pr_info("mHprOffset_CompenBydB[%d]=%d\n", i, mHprOffset_CompenBydB[i]);
	}
}
...
static void SetDcCompenSation(bool enable)
{
	...
	pr_info("%s() mAudio_Ana_Volume L = %d, mAudio_Ana_Volume R = %d\n",
	...
	pr_info("%s() comp_indexL = %d, comp_indexR = %d\n", __func__, comp_indexL, comp_indexR);
	...
	pr_info("%s times = %d, lch_value = 0x%x, rch_value = 0x%x\n", __func__, times, lch_value, rch_value);
	if (enable) {
...
static void Audio_Amp_Change(int channels, bool enable, bool is_anc)
{
	pr_info("%s(), enable %d, ANC %d, HSL %d, HSR %d, is_anc %d\n",
		 __func__,
		 enable,
		 ANC_enabled,
		 mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETL],
...
static void Voice_Amp_Change(bool enable)
{
	if (enable) {
		if (GetDLStatus() == false) {
			...
			Ana_Set_Reg(ZCD_CON3, mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HSOUTL], 0x001f);
...
static int Headset_PGAL_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s() set mAudio_Ana_Volume L = %d, mAudio_Ana_Volume R = %d\n",
		__func__, mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL],
		mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTR]);
...
static int Audio_HyBridNLE_TurnOn_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("%s() set mAudio_Ana_Volume L = %d, mAudio_Ana_Volume R = %d\n",
		__func__, mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL],
		mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTR]);
...
static int Audio_HyBridNLE_SetGain_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (nle_channel == AUDIO_NLE_CHANNEL_R) {
		...
		pr_info("%s() set mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTR] =%d\n",
		...
		pr_info("%s() set mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL] =%d\n",
			__func__, mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL]);
	} else
