...
#else
    /*Bug 513881 - if smart pa break or without smart pa,set codec
     dai link to dummy,than device boot up tsx.wt 20191120 begin */
    if(aw88194a_state==0){
            dai_link[0].codec_dai_name =
		    mtk_spk_list[MTK_SPK_NOT_SMARTPA].codec_dai_name;
            dai_link[0].codec_name =
		    mtk_spk_list[MTK_SPK_NOT_SMARTPA].codec_name;
            dai_link[0].ignore_pmdown_time = 1;
		    dev_err(&pdev->dev,
		         "Not found smart pa\n");
    }
