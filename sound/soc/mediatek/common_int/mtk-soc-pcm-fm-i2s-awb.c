...
static unsigned int mfm_i2s_RecordVol = 0x10000;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (GetFmI2sInPathEnable() == true)
		SetHwDigitalGain(mfm_i2s_RecordVol, Soc_Aud_Hw_Digital_Gain_HW_DIGITAL_GAIN2);
...
#ifdef CONFIG_MTK_TC10_FEATURE
		/* Set HW_GAIN */
		SetHwDigitalGainMode(Soc_Aud_Hw_Digital_Gain_HW_DIGITAL_GAIN2, substream->runtime->rate,
				0x40);
