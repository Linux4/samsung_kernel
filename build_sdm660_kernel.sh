...
KERNEL_DEFCONFIG=sdm660_sec_defconfig
DEBUG_DEFCONFIG=sdm660_sec_eng_defconfig
...
if [ "${REGION}" == "${CARRIER}" ]; then
	VARIANT_DEFCONFIG=sdm660_sec_${MODEL}_${CARRIER}_defconfig
else
	VARIANT_DEFCONFIG=sdm660_sec_${MODEL}_${REGION}_${CARRIER}_defconfig
fi
...
FUNC_BUILD_KERNEL()
{
    ...
    echo '--cmdline "androidboot.hardware=qcom msm_rtb.filter=0x37 ehci-hcd.park=3 lpm_levels.sleep_disabled=1 sched_enable_hmp=1 sched_enable_power_aware=1 service_locator.enable=1 swiotlb=1 androidboot.configfs=true androidboot.usbcontroller=a800000.dwc3 androidboot.selinux=permissive androidboot.selinux=permissive" --base 0x00000000 --pagesize 4096 --os_version 8.0.0 --os_patch_level 2018-04-01 --ramdisk_offset 0x02000000 --tags_offset 0x01E00000 '   >  $PRODUCT_OUT/mkbootimg_ver_args.txt   
