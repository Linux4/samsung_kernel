...
class ELF:
    ...
    def get_ftrace_instructions_addr(self, start_addr=None, end_addr=None):
        """
        Search BL instructions involved into FTRACE functioning.
        :param start_addr: start address :int
        :param end_addr: end address: int
        :returns list: [[instr1_addr, length1], [instr2_addr, length2], ...]
        """

        if start_addr and end_addr is None:
            return []

        # find ftrace reference symbol addr.
        branch_link_symbol_name = "_mcount"
        branch_link_symbol_addr = [key for key, value in self.__symbols.items() if value.name == branch_link_symbol_name]
        if len(branch_link_symbol_addr) != 1:
            return []
        
        branch_link_symbol_addr = branch_link_symbol_addr[0]

        # derive binary of the section involves start_addr and end_addr
        section_to_be_processed = self.get_section_by_vaddr(start_addr)
        __hex_dump = self.__readelf_raw(["--hex-dump=" + section_to_be_processed.name, self.__elf_file])
        if len(__hex_dump) == 0:
            return []

        pattern_section_content = "^ *0x[0-9A-Fa-f]{16} (.*) .*.{16}$"
        pattern_section_addr = "^ *(0x[0-9A-Fa-f]{16}).*.*.{16}$"
        
        __section_addr = int(re.findall(pattern_section_addr, __hex_dump, re.MULTILINE)[0], 16)
        
        hex_dump_list = re.findall(pattern_section_content, __hex_dump, re.MULTILINE)
        __hex_dump_str = ''.join(hex_dump_list).replace(" ", "")
        __section_bin = unhexlify(__hex_dump_str)
        
        # Search BL instructions with branch to ftrace related symbol  
        struct_format = '<I'
        struct_size = struct.calcsize(struct_format)

        ftrace_instructions = list()
        current_addr = start_addr
        while current_addr < end_addr:
            opcode_bytes = __section_bin[(current_addr - __section_addr): (current_addr - __section_addr + struct_size)]
            opcode = list(struct.unpack(struct_format, opcode_bytes))[0]

            if self.utils.is_bl(opcode):
                bl_jump_len = self.utils.extract_bl_len(opcode)
                
                # check if branch destination is ftrace debug entry point. 
                # if so, it considered as gap 
                if (bl_jump_len == branch_link_symbol_addr - current_addr):
                    ftrace_instructions.append([current_addr, struct_size])

            current_addr = current_addr + struct_size

        return ftrace_instructions
