...
class IntegrityRoutine(ELF):
    ...
    def get_ftrace_caused_gaps(self, start_addr, end_addr):
        """
        :param start_addr: start address :int
        :param end_addr: end address: int
        :returns list of runtime patched instruction gaps like [[gap_start, gap_end], [gap_start, gap_end], ...]
        """
        all_ftrace_gaps = list()
        ftrace_gaps = self.get_ftrace_instructions_addr(start_addr, end_addr)
        if len(ftrace_gaps) == 0:
            return []

        for ftrace_item in ftrace_gaps:
            all_ftrace_gaps.append([ftrace_item[0], ftrace_item[0] + ftrace_item[1]])

        return all_ftrace_gaps
    ...
    def make_integrity(self, sec_sym, module_name, debug=False, print_reloc_addrs=False, sort_by="address",
        ...
        asm_text_addr_start = self.get_symbol_by_name("first_" + module_name + "_asm_text")
        asm_text_addr_end = self.get_symbol_by_name("last_" + module_name + "_asm_text")
        asm_init_addr_start = self.get_symbol_by_name("first_" + module_name + "_asm_init")
        asm_init_addr_end = self.get_symbol_by_name("last_" + module_name + "_asm_init")
        ...
        gaps.extend(self.get_altinstruction_gaps(asm_text_addr_start.addr, asm_text_addr_end.addr))
        gaps.extend(self.get_altinstruction_gaps(asm_init_addr_start.addr, asm_init_addr_end.addr))
        
        gaps.extend(self.get_ftrace_caused_gaps(text_addr_start.addr, text_addr_end.addr))
        gaps.extend(self.get_ftrace_caused_gaps(init_addr_start.addr, init_addr_end.addr))
        gaps.extend(self.get_ftrace_caused_gaps(asm_text_addr_start.addr, asm_text_addr_end.addr))
        gaps.extend(self.get_ftrace_caused_gaps(asm_init_addr_start.addr, asm_init_addr_end.addr))
