...
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
		dev_info(cs40l26->dev, "%s Stop command skipped\n", __func__);
#else
...
static int samsung_wavetable_swap(struct cs40l26_private *cs40l26, u32 val)
{
	int error;
...
static int samsung_get_ls_calibration_params_temp(struct input_dev *dev, u32 *params_temp)
{
	struct sec_vib_inputff_drvdata *ddata = input_get_drvdata(dev);
	struct cs40l26_private *cs40l26 = ddata->private_data;
	u32 reg;
...
static int samsung_set_ls_calibration_params_temp(struct input_dev *dev, u32 val)
{
	struct sec_vib_inputff_drvdata *ddata = input_get_drvdata(dev);
	struct cs40l26_private *cs40l26 = ddata->private_data;
	u32 reg;
...
static int samsung_set_trigger_cal(struct input_dev *dev, u32 val)
{
	...
	val > CS40L26_CALIBRATION_CONTROL_REQUEST_LS_CALIBRATION) {
...
static int samsung_get_ls_calibration_results(struct input_dev *dev, char *buf)
{
	struct sec_vib_inputff_drvdata *ddata = input_get_drvdata(dev);
	struct cs40l26_private *cs40l26 = ddata->private_data;
...
static int samsung_set_ls_calibration_results(struct input_dev *dev, char *str_full)
{
	struct sec_vib_inputff_drvdata *ddata = input_get_drvdata(dev);
	struct cs40l26_private *cs40l26 = ddata->private_data;
...
static int samsung_get_ls_calibration_results_name(struct input_dev *dev, char *buf)
{
	struct sec_vib_inputff_drvdata *ddata = input_get_drvdata(dev);
	struct cs40l26_private *cs40l26 = ddata->private_data;
...
static u32 samsung_set_f0_stored(struct input_dev *dev, u32 val)
{
	...
	if (val < cs40l26->pdata.samsung_freq_min || val > cs40l26->pdata.samsung_freq_max) {
		dev_err(cs40l26->dev, "%s: %u is out of range\n", __func__, val);
		return -EINVAL;
	}
...
static int cs40l26_composite_upload(struct cs40l26_private *cs40l26, s16 *in_data,
		u32 in_data_nibbles)
{
	u8 ncw_nsections, ncw_global_rep, nsections, global_rep, out_nsections = 0;
	u16 section_size_bytes, header_flags = CS40L26_WT_HEADER_DEFAULT_FLAGS;
	int pos_byte = 0, in_pos_nib = 2, in_data_bytes = 2 * in_data_nibbles;
	int out_data_bytes = 0, data_bytes = 0;
	u32 ep_thresh = 0, ncw_bytes, wlen;
	struct device *dev = cs40l26->dev;
	u8 metadata_id, ep_length, ep_payload, *data, *ncw_data, *out_data;
	...
	error = cl_dsp_memchunk_read(cs40l26->dsp, &ch, 8, &metadata_id);
	if (error)
		return error;

	if (metadata_id == CS40L26_HEADER_METADATA_EP) {
		header_flags |= CS40L26_HEADER_METADATA_FLAG;

		error = cl_dsp_memchunk_read(cs40l26->dsp, &ch, 8, &ep_length);
		if (error)
			return error;

		error = cl_dsp_memchunk_read(cs40l26->dsp, &ch, 8, &ep_payload);
		if (error)
			return error;

		error = cl_dsp_memchunk_read(cs40l26->dsp, &ch, 24, &ep_thresh);
		if (error)
			return error;
	...
	for (i = 0; i < nsections; i++) {
		...
		if (header->type == WT_TYPE_V6_COMPOSITE) {
			...
			if (ncw_global_rep != 0) {
				dev_err(dev, "No NCW support for outer repeat\n");
				error = -EPERM;
				goto data_err_free;
			}
	...
	if (metadata_id == CS40L26_HEADER_METADATA_EP)
		out_data_bytes += CS40L26_METADATA_SIZE_BYTES;
	...
	cl_dsp_memchunk_write(&out_ch, 16, header_flags);
	...
	if (metadata_id == CS40L26_HEADER_METADATA_EP) {
		out_ch.data += data_bytes;
		cl_dsp_memchunk_write(&out_ch, 8, metadata_id);
		cl_dsp_memchunk_write(&out_ch, 8, ep_length);
		cl_dsp_memchunk_write(&out_ch, 8, ep_payload);
		cl_dsp_memchunk_write(&out_ch, 24, ep_thresh);
		cl_dsp_memchunk_write(&out_ch, 24, CS40L26_METADATA_TERMINATOR);
	}
...
static int cs40l26_custom_ram(struct cs40l26_private *cs40l26, u32 *trigger_index)
{
	...
	if (cs40l26->use_sep_index) {
		...
		if (index >= 1000) {
			if (cs40l26->wt_num != CS40L26_LONG_WT_NUM) {
				dev_info(cs40l26->dev, "%s Long Wavetable Swap\n", __func__);
				samsung_wavetable_swap(cs40l26,CS40L26_LONG_WT_NUM);
			}
			index -= 1000;
		}
		else if (cs40l26->wt_num != CS40L26_SHORT_WT_NUM) {
			dev_info(cs40l26->dev, "%s Short Wavetable Swap\n", __func__);
			samsung_wavetable_swap(cs40l26,CS40L26_SHORT_WT_NUM);
		}
		index = cs40l26_index_mapping(index);
	}
	dev_info(cs40l26->dev, "%s Index(%d) effect\n", __func__, index);
...
static void cs40l26_upload_worker(struct work_struct *work)
{
	...
	if (nwaves < 0) {
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
		dev_err(cdev, "%s final cs40l26_num_waves error(%d)\n",
			__func__, error);
		err = true;
#endif
		error = nwaves;
...
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
	dev_info(cdev, "Total number of waveforms = %d\n", nwaves);
#else
...
static const struct sec_vib_inputff_ops cs40l26_vib_ops = {
	...
	.get_ls_temp = samsung_get_ls_calibration_params_temp,
	.set_ls_temp = samsung_set_ls_calibration_params_temp,
	.set_trigger_cal = samsung_set_trigger_cal,
	.get_ls_calib_res = samsung_get_ls_calibration_results,
	.set_ls_calib_res = samsung_set_ls_calibration_results,
	.get_ls_calib_res_name = samsung_get_ls_calibration_results_name,
...
static void samsung_input_data_init(struct cs40l26_private *cs40l26)
{
	...
	cs40l26->sec_vib_ddata.is_ls_calibration = cs40l26->pdata.is_ls_calibration;
...
static int cs40l26_gpio_config(struct cs40l26_private *cs40l26)
{
	...
	if (cs40l26->devid == CS40L26_DEVID_A || cs40l26->devid == CS40L26_DEVID_L27_A)
		val = GENMASK(CS40L26_GPIO4_FALL_IRQ, CS40L26_GPIO2_RISE_IRQ);
	else
		val = 0;
...
static int cs40l26_noise_gate_config(struct cs40l26_private *cs40l26)
{
	...
	enable = cs40l26->calib_fw ? 0 : cs40l26->ng_enable;
...
static int cs40l26_aux_noise_gate_config(struct cs40l26_private *cs40l26)
{
	...
	return cs40l26_wseq_write(cs40l26, CS40L26_MIXER_NGATE_CH1_CFG, CS40L26_AUX_NG_DEFAULT,
			true, CS40L26_WSEQ_OP_WRITE_FULL, &cs40l26->aseq);
}
...
int cs40l26_wt_swap(struct cs40l26_private *cs40l26)
{
	...
	if (i >= 100) {
		dev_err(cs40l26->dev, "Timed out waiting for DSP shutdown\n");
		error = -ETIMEDOUT;
		goto wake;
	}
...
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
	dev_info(cs40l26->dev, "%s wt_file_name: %s %d\n", __func__, wt_file_name, cs40l26->wt_num);
#endif
	...
	dev_info(cs40l26->dev, "%s Loaded Successfully with %d RAM waveforms\n",
			wt_file_name, cs40l26_num_ram_waves(cs40l26));

free:
	kfree(wt_file_name);

wake:
	error_prime = cs40l26_pm_state_transition(cs40l26, CS40L26_PM_STATE_WAKEUP);
	if (error_prime)
		goto timeout_restore;

timeout_restore:
	error_prime = cs40l26_pm_timeout_ms_set(cs40l26, CS40L26_DSP_STATE_ACTIVE, active_timeout);
	if (error_prime)
		goto gpio_restore;

	error_prime = cs40l26_pm_timeout_ms_set(cs40l26, CS40L26_DSP_STATE_STANDBY, stdby_timeout);
	if (error_prime)
		goto gpio_restore;

gpio_restore:
	error_prime = cs40l26_set_gpio_from_dt(cs40l26);
	if (error_prime)
		return error_prime;
...
static int cs40l26_parse_properties(struct cs40l26_private *cs40l26)
{
...
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
	cs40l26->pdata.is_ls_calibration = device_property_present(dev, "samsung,ls-calibration");
	...
	error = device_property_read_u32(dev, "samsung,freq-min", &cs40l26->pdata.samsung_freq_min);
	if (error)
		cs40l26->pdata.samsung_freq_min = 0x250000;
		/* default min frequency is 150Hz */
	else
		cs40l26->pdata.samsung_freq_min *= 0x4000;
		/* converting Hz to 14 bit value, multiplying by 2^14 */

	dev_info(dev, "%s - freq_min is %d\n", __func__, cs40l26->pdata.samsung_freq_min);

	error = device_property_read_u32(dev, "samsung,freq-max", &cs40l26->pdata.samsung_freq_max);
	if (error)
		cs40l26->pdata.samsung_freq_max = 0x2A0000;
		/* default min frequency is 170Hz */
	else
		cs40l26->pdata.samsung_freq_max *= 0x4000;
		/* converting Hz to 14 bit value, multiplying by 2^14 */

	dev_info(dev, "%s - freq_max is %d\n", __func__, cs40l26->pdata.samsung_freq_max);
#endif
