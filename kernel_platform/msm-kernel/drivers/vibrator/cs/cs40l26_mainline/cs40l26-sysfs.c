...
int cs40l26_ls_calibration_check_results(struct cs40l26_private *cs40l26)
{
	u32 reg, return_code;
	int error;
...
EXPORT_SYMBOL_GPL(cs40l26_ls_calibration_check_results);

static ssize_t ls_calibration_results_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct cs40l26_private *cs40l26 = dev_get_drvdata(dev);
	int count = 0, error, i;
	u32 reg, val;
...
static ssize_t ls_calibration_results_name_show(struct device *dev, struct device_attribute *attr,	char *buf)
{
	struct cs40l26_private *cs40l26 = dev_get_drvdata(dev);
	int count = 0, error, i;
	u32 reg, val;
