 ...
 *
 ******************************************************/
#define AW8624X_DRIVER_VERSION "v0.0.0.17"
...
/*
 * set seq0 and seq1 combined(seq0 overdrive and seq1 infinite loop playback)
 */
static void aw8624x_set_combined_seq(struct aw8624x *aw8624x, uint8_t loop)
{
	aw_info("combined wav seq");
	aw8624x_set_wav_seq(aw8624x, 0x00, aw8624x->ram.ram_num);
	aw8624x_set_wav_seq(aw8624x, 0x01, aw8624x->ram.ram_num - 1);
	aw8624x_set_wav_seq(aw8624x, 0x02, 0x00);
	aw8624x_set_wav_loop(aw8624x, 0x00, loop - 1);
	aw8624x_set_wav_loop(aw8624x, 0x01, 0x0f);
}
...
static int aw8624x_parse_dt(struct device *dev, struct aw8624x *aw8624x, struct device_node *np)
{
	...
	val = of_property_read_u32(np, "vib_f0_open_offset", &aw8624x->dts_info.f0_open_offset);
	if (val != 0)
		aw_err("aw8624x_vib_f0_open_offset not found");

	val = of_property_read_u32(np, "vib_f0_close_offset", &aw8624x->dts_info.f0_close_offset);
	if (val != 0)
		aw_err("aw8624x_vib_f0_close_offset not found");

	val = of_property_read_u32(np, "vib_gain_open_offset", &aw8624x->dts_info.gain_open_offset);
	if (val != 0)
		aw_err("aw8624x_vib_gain_open_offset not found");

	val = of_property_read_u32(np, "vib_gain_close_offset", &aw8624x->dts_info.gain_close_offset);
	if (val != 0)
		aw_err("aw8624x_vib_gain_close_offset not found");
				       ...
				       struct ff_effect *old)
{
	...
	if (aw8624x->effect_type == FF_CONSTANT) {
		...
		aw8624x->effect_id = aw8624x->ram.ram_num - 1;
		...
		if (data[0] == aw8624x->ram.ram_num - 1) {
			aw_err("data[0]:%u is neither RAM or RTP seq!!", data[0]);
			mutex_unlock(&aw8624x->lock);
			return -ERANGE;
		}
		aw8624x->effect_id = data[0];
		if (aw8624x->effect_id < aw8624x->ram.ram_num - 1) {
...
#if defined(CONFIG_AW8624X_SAMSUNG_FEATURE) && defined(CONFIG_SEC_VIB_FOLD_MODEL)
static int aw8624x_input_get_fold_event(struct aw8624x *ddata)
{
	if (!ddata) {
		aw_info("[VIB] dev NULL error\n");
		return -EINVAL;
	}

	switch (ddata->sec_vib_ddata.event_idx) {
	case EVENT_CMD_FOLDER_OPEN:
		return EVENT_CMD_FOLDER_OPEN;
	case EVENT_CMD_FOLDER_CLOSE:
		return EVENT_CMD_FOLDER_CLOSE;
...
static void aw8624x_input_set_gain_work_routine(struct work_struct *work)
{
	...
	ret = aw8624x_input_get_fold_event(aw8624x);
	if (ret == EVENT_CMD_FOLDER_OPEN) {
		aw_info("open state, decrease gain %u%%", aw8624x->dts_info.gain_open_offset);
		aw8624x->gain = (100 - aw8624x->dts_info.gain_open_offset) * aw8624x->gain / 100;
	} else if (ret == EVENT_CMD_FOLDER_CLOSE) {
		aw_info("close state, increase gain %u%%", aw8624x->dts_info.gain_close_offset);
		aw8624x->gain = (100 + aw8624x->dts_info.gain_close_offset) * aw8624x->gain / 100;
	}
#endif
#endif
			 ...
			 size_t count)
{
	...
	if ((val > 0) && (val < aw8624x->rtp_num_max)) {
		aw8624x->state = 1;
		aw8624x->effect_id = val + aw8624x->ram.ram_num - 1;
				      ...
				      const char *buf, size_t count)
{
	...
	mutex_lock(&aw8624x->lock);
	aw8624x->input_flag = true;
	if (val > 0 && val < aw8624x->ram.ram_num - 1) {
		aw8624x->activate_mode = AW8624X_RAM_MODE;
		aw8624x->effect_id = val;
	} else if (val == aw8624x->ram.ram_num - 1) {
		aw8624x->activate_mode = AW8624X_RAM_LOOP_MODE;
		aw8624x->effect_id = val;
	} else {
		aw_err("val is error");
...
static void aw8624x_vibrator_work_routine(struct work_struct *work)
{
	...
	if (aw8624x->state) {
		...
		if (aw8624x->activate_mode == AW8624X_RAM_LOOP_MODE) {
			ret = aw8624x_input_get_fold_event(aw8624x);
			if (ret == EVENT_CMD_FOLDER_OPEN) {
				aw_info("open state, decrease 6Hz");
				aw8624x->f0 = aw8624x->sample_param[5] - aw8624x->dts_info.f0_open_offset;
			} else if (ret == EVENT_CMD_FOLDER_CLOSE) {
				aw_info("close state, so increase 2Hz");
				aw8624x->f0 = aw8624x->sample_param[5] + aw8624x->dts_info.f0_close_offset;
			}
		} else {
			aw_info("ram short pattern without offset, increase 5Hz");
			aw8624x->f0 = aw8624x->sample_param[5] + 50;
		}
		ret = aw8624x_judge_cali_range(aw8624x);
		if (ret < 0)
			aw8624x->f0 = aw8624x->sample_param[5];
...
static void aw8624x_rtp_work_routine(struct work_struct *work)
{
	...
	if (ret == EVENT_CMD_FOLDER_OPEN) {
		...
		aw_info("close state, so increase 1Hz");
		aw8624x->f0 = aw8624x->sample_param[5] + aw8624x->dts_info.f0_close_offset;
	}
	ret = aw8624x_judge_cali_range(aw8624x);
	if (ret < 0)
	...
	if (aw8624x->state) {
		pm_stay_awake(aw8624x->dev);
		aw8624x->wk_lock_flag = 1;
		aw8624x->rtp_file_num = aw8624x->effect_id - (aw8624x->ram.ram_num - 1);
...
static void aw8624x_haptic_init(struct aw8624x *aw8624x)
{
	...
	aw_info("aw8624x_f0_data=%u", aw8624x_f0_data);
	for (i = 0; i < 5; i++)
...
u32 samsung_set_f0_stored(struct input_dev *dev, u32 val)
{
	...
	int ret = aw8624x_nv_f0_store(aw8624x, val);
	return ret;
}
...
static int samsung_aw8624x_input_init(struct aw8624x *aw8624x)
{
	...
	aw8624x->sec_vib_ddata.is_f0_tracking = aw8624x->dts_info.is_f0_tracking;
...
#ifdef CONFIG_AW8624X_SAMSUNG_FEATURE
int aw8624x_sys_suspend(struct device *dev)
{

	aw_info("System suspend\n");
	if (!IS_ERR(dev->pins->sleep_state)) {
		aw_info("%s: set sleep pinctrl\n", __func__);
...
int aw8624x_sys_resume(struct device *dev)
{
	aw_info("System resume\n");

	if (!IS_ERR(dev->pins->default_state)) {
		aw_info("%s: set default pinctrl\n", __func__);
...
#ifdef CONFIG_AW8624X_SAMSUNG_FEATURE
const struct dev_pm_ops aw8624x_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(aw8624x_sys_suspend, aw8624x_sys_resume)
};
#endif
