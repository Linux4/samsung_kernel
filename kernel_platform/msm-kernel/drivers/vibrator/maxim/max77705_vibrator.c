...
static int max77705_vib_set_default_duty(struct device *dev, int default_duty)
{
	struct max77705_vibrator_drvdata *ddata = dev_get_drvdata(dev);
	...
	ddata->pdata->normal_ratio = default_duty;

	return 0;
}

static int max77705_vib_get_default_duty(struct device *dev, char *buf)
{
	struct max77705_vibrator_drvdata *ddata = dev_get_drvdata(dev);
...
#if defined(CONFIG_MAX77705_VIB_FOLD_MODEL)
static int max77705_vib_set_fold_open_duty(struct device *dev, int fold_open_duty)
{
	struct max77705_vibrator_drvdata *ddata = dev_get_drvdata(dev);
	...
	ddata->pdata->fold_open_ratio = fold_open_duty;

	return 0;
}

static int max77705_vib_get_fold_open_duty(struct device *dev, char *buf)
{
	struct max77705_vibrator_drvdata *ddata = dev_get_drvdata(dev);

	return snprintf(buf, VIB_BUFSIZE, "%d\n", ddata->pdata->fold_open_ratio);
}

static int max77705_vib_set_fold_close_duty(struct device *dev, int fold_close_duty)
{
	struct max77705_vibrator_drvdata *ddata = dev_get_drvdata(dev);
	...
	ddata->pdata->fold_close_ratio = fold_close_duty;

	return 0;
}

static int max77705_vib_get_fold_close_duty(struct device *dev, char *buf)
{
	struct max77705_vibrator_drvdata *ddata = dev_get_drvdata(dev);

	return snprintf(buf, VIB_BUFSIZE, "%d\n", ddata->pdata->fold_close_ratio);
}
#endif
...
static int set_fold_model_freq(struct max77705_vibrator_drvdata *ddata, int num)
{
	int freq = 0;
	...
	case EVENT_CMD_FOLDER_OPEN:
		...
		break;
	case EVENT_CMD_FOLDER_CLOSE:
		freq = ddata->pdata->fold_close_freq_array[num];
		break;
	case EVENT_CMD_ACCESSIBILITY_BOOST_ON:
	case EVENT_CMD_ACCESSIBILITY_BOOST_OFF:
	case EVENT_CMD_NONE:
	default:
		freq = ddata->pdata->freq_array[num];
		break;
	}
...
static int max77705_vib_set_freq(struct max77705_vibrator_drvdata *ddata, int num)
{
	int freq;
...
static int max77705_vib_set_frequency(struct device *dev, int num)
{
	...
	if (num >= 0 && num < ddata->pdata->freq_nums) {
		ddata->pdata->freq = max77705_vib_set_freq(ddata, num);
		ddata->period = FREQ_DIVIDER / ddata->pdata->freq;
...
static const struct sec_vibrator_ops max77705_multi_freq_vib_ops = {
	...
	.set_default_duty = max77705_vib_set_default_duty,
	.get_default_duty = max77705_vib_get_default_duty,
...
#if defined(CONFIG_MAX77705_VIB_FOLD_MODEL)
	.set_fold_open_duty = max77705_vib_set_fold_open_duty,
	.get_fold_open_duty = max77705_vib_get_fold_open_duty,
	.set_fold_close_duty = max77705_vib_set_fold_close_duty,
	.get_fold_close_duty = max77705_vib_get_fold_close_duty,
...
#if defined(CONFIG_MAX77705_VIB_FOLD_MODEL)
		pdata->fold_open_freq_array = devm_kzalloc(dev,
				sizeof(u32)*pdata->freq_nums, GFP_KERNEL);
		if (!pdata->fold_open_freq_array) {
			pr_err("%s: failed to allocate fold_open_freq_array data\n",
					__func__);
			goto err_parsing_dt;
		}

		ret = of_property_read_u32_array(np, "haptic,fold_open_frequency",
				pdata->fold_open_freq_array, pdata->freq_nums);
		if (ret) {
			pr_err("%s: error to get dt fold open node frequency\n",
					__func__);
			goto err_parsing_dt;
		}

		pdata->fold_open_freq = pdata->fold_open_freq_array[0];

		pdata->fold_close_freq_array = devm_kzalloc(dev,
				sizeof(u32)*pdata->freq_nums, GFP_KERNEL);
		if (!pdata->fold_close_freq_array) {
			pr_err("%s: failed to allocate fold_close_freq_array data\n",
					__func__);
			goto err_parsing_dt;
		}

		ret = of_property_read_u32_array(np, "haptic,fold_close_frequency",
				pdata->fold_close_freq_array, pdata->freq_nums);
		if (ret) {
			pr_err("%s: error to get dt fold close node frequency\n",
					__func__);
			goto err_parsing_dt;
		}

		pdata->fold_close_freq = pdata->fold_close_freq_array[0];
#endif
	} else {
...
#if defined(CONFIG_MAX77705_VIB_FOLD_MODEL)
		ret = of_property_read_u32(np,
				"haptic,fold_open_frequency", &temp);
		if (ret) {
			pr_err("%s: error to get dt fold open node frequency\n",
					__func__);
			goto err_parsing_dt;
		} else
			pdata->fold_open_freq = (int)temp;

		ret = of_property_read_u32(np,
				"haptic,fold_close_frequency", &temp);
		if (ret) {
			pr_err("%s: error to get dt fold close node frequency\n",
					__func__);
			goto err_parsing_dt;
		} else
			pdata->fold_close_freq = (int)temp;
#endif
	}
	...
	if (pdata->freq_nums) {
		...
		for (i = 0; i < pdata->freq_nums; i++) {
			pr_info("frequency[%d]: %d.%dHz\n", i,
					...
					pdata->freq_array[i]%10);
#if defined(CONFIG_MAX77705_VIB_FOLD_MODEL)
			pr_info("fold_open_frequency[%d]: %d.%dHz\n", i,
					pdata->fold_open_freq_array[i]/10,
					pdata->fold_open_freq_array[i]%10);
			pr_info("fold_close_frequency[%d]: %d.%dHz\n", i,
					pdata->fold_close_freq_array[i]/10,
					pdata->fold_close_freq_array[i]%10);
#endif
		}
	} else {
...
#if defined(CONFIG_MAX77705_VIB_FOLD_MODEL)
		pr_info("fold_open_frequency: %d.%dHz\n", pdata->fold_open_freq/10, pdata->fold_close_freq%10);
		pr_info("fold_close_frequency: %d.%dHz\n", pdata->fold_close_freq/10, pdata->fold_close_freq%10);
#endif
	}
