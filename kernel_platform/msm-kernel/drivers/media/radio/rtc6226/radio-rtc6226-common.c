...
/*M55  code for SR-QN6887A-01-574 by tangjie at 2023/10/17 start*/
wait_queue_head_t rtc6226_wq;
int rtc6226_wq_flag = NO_WAIT;
int sf_bl_flag;
/*M55  code for SR-QN6887A-01-574 by tangjie at 2023/10/17 end*/
...
int rtc6226_set_freq(struct rtc6226_device *radio, unsigned int freq)
{
	...
	for (i = 0x12; i < RADIO_REGISTER_NUM; i++) {
		...
		ret = rtc6226_get_all_registers(radio);
	if (ret < 0){
		FMDBG("%s fail to get ALL register\n", __func__);
		goto end;
	}
	for(i = 0; i < 16;i++)
	{
		FMDBG("%s reg[0x%x]=0x%4.4hx\n",__func__,i, radio->registers[i]);
	}
		FMDBG("%s reg[0x12]=0x%4.4hx\n",__func__, radio->registers[0x12]);
		FMDBG("%s reg[0x13]=0x%4.4hx\n",__func__, radio->registers[0x13]);
		FMDBG("%s reg[0x14]=0x%4.4hx\n",__func__, radio->registers[0x14]);
		FMDBG("%s reg[0x15]=0x%4.4hx\n",__func__, radio->registers[0x15]);
		FMDBG("%s reg[0x1c]=0x%4.4hx\n",__func__, radio->registers[0x1c]);
		FMDBG("%s reg[0x1e]=0x%4.4hx\n",__func__, radio->registers[0x1e]);
...
int rtc6226_power_up(struct rtc6226_device *radio)
{
	...
	FMDBG("%s : regE=0x%4.4hx RegF=0x%4.4hx\n", __func__,
		radio->registers[14], radio->registers[15]);
	FMDBG("%s : regI2SCFG[%x]=0x%4.4hx\n", __func__,I2SCFG,
        radio->registers[I2SCFG]);
done:
	FMDBG("%s exit %d\n", __func__, retval);
	return retval;
}
