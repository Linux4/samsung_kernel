...
static void __qc_rst_dump_instr(struct pt_regs *regs)
{
...
static void sec_qc_rst_exinfo_rvh_die_kernel_falut(void *unused,
		const char *msg, unsigned long addr, unsigned int esr,
		struct pt_regs *regs)
{
	__qc_rst_exinfo_store_extc_idx(false);
	__qc_rst_dump_instr(regs);
}
...
{
	if (!user_mode(regs)) {
		__rst_exinfo_save_fault_info(esr,
			__rst_exinfo_esr_get_class_string(esr),	regs->pc,
			regs->sp);
