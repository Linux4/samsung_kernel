...
static void mfc_set_iec_params(struct i2c_client *client, struct mfc_iec_data *data)
{
	...
	int i = 0;
	char str[256] = {0,};
	u16 print_reg[9] = {0x048E, 0x048F, 0x0494, 0x0495, 0x0498, 0x0499, 0x049A, 0x049C, 0x049D};
	...
	/* need to set before MFC_MST_MODE_SEL_REG */
	...
	/* for debug printf */
	for (i = 0; i < 9; i++) {
		mfc_reg_read(client, print_reg[i], &val);
		sprintf(str+strlen(str), "0x%04x:0x%02x, ", print_reg[i], val);
	}
	pr_info("%s : %s\n", __func__, str);
}
