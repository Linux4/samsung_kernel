...
static int stwlc_nvm_write_sector(struct mfc_charger_data *charger, const u8 *data,
									 int data_length, int sector_index)
{
	...
	u16 FWREG_NVM_SECTOR_INDEX_ADDR	= 0x0124;
	u16 FWREG_AUX_DATA_00_ADDR = 0x0200;
	u16 FWREG_SYS_CMD_ADDR = 0x0120;

	if (!charger->pdata->is_rom2) {
		FWREG_NVM_SECTOR_INDEX_ADDR	= 0x010C;
		FWREG_AUX_DATA_00_ADDR = 0x0180;
	...
	err = stwlc_fw_i2c_write(charger->client, FWREG_NVM_SECTOR_INDEX_ADDR, &reg_value, 1);
	if (err != 0)
		return err;
	...
	while (remaining > 0) {
		...
		err = stwlc_fw_i2c_write(charger->client, FWREG_AUX_DATA_00_ADDR+(written_already & 0xff), write_buff + written_already, to_write_now);
		if (err != 0)
			return err;
...
static int stwlc_nvm_write_bulk(struct mfc_charger_data *charger, u8 *data,
									int data_length, u8 sector_index)
{
	...
	while (remaining > 0) {
		...
		err = stwlc_nvm_write_sector(charger, data + written_already,
					  ...
					  const u8 *srcData, int srcOffs, int size)
{
	...
	u16 FWREG_SYS_CMD_ADDR = 0x0120;
	u16 FWREG_OP_MODE_ADDR = 0x017F;
	u16 FWREG_NVM_WR_PWD_ADDR =	0x0122;
	u8 NVM_CFG_START_SECTOR_INDEX = 125;

	// Check revision of the firmware is matched with IC
	if ((fw_header->bin_ver == 0x02) &&
			((charger->pdata->is_rom2 && (fw_header->target != MFC_CHIP_REV_ROM2)) ||
			(!charger->pdata->is_rom2 && (fw_header->target != MFC_CHIP_REV_ROM3)))) {
		pr_info("%s mfc, Revision is not matched with IC [ IC:0x%x, FW:0x%x ]\n", __func__,
			charger->pdata->is_rom2 ? MFC_CHIP_REV_ROM2 : MFC_CHIP_REV_ROM3, fw_header->target);
		return E_UNMATCHED_CHIP_REV_ID;
	}
	...
	err = stwlc_nvm_write_bulk(charger, patch_data, fw_header->sec0_size,
					NVM_PATCH_START_SECTOR_INDEX);
	if (err != OK)
		return err;

	err = stwlc_nvm_write_bulk(charger, cfg_data, fw_header->sec1_size,
...
static unsigned int mfc_get_bin_fw_version(struct mfc_charger_data *charger)
{
	...
	if (charger->pdata->is_rom2)
		charger->fw_path = MFC_FLASH_FW_HEX_STM_ROM2_PATH;
	else
...
static void mfc_rx_phm_work(struct work_struct *work)
{
	...
	if (!is_phm_supported_pad(charger->tx_id)) {
...
static irqreturn_t mfc_wpc_irq_thread(int irq, void *irq_data)
{
	...
	u8 int_state;
	int ret = 0;
	u8 irq_src_l = 0, irq_src_h = 0, irq_src_2 = 0;
	u8 status_l = 0, status_h = 0, status_2 = 0;
	bool end_irq = false;
	union power_supply_propval value;
	...
	ret = mfc_reg_read(charger->client, MFC_INT_A_0_REG, &irq_src_l);
	if (ret < 0) {
		pr_err("%s: Failed to read INT_A_L_REG: %d\n", __func__, ret);
		__pm_relax(charger->wpc_ws);
		return IRQ_HANDLED;
	}
	ret = mfc_reg_read(charger->client, MFC_INT_A_1_REG, &irq_src_h);
	if (ret < 0) {
		pr_err("%s: Failed to read INT_A_H_REG: %d\n", __func__, ret);
		__pm_relax(charger->wpc_ws);
		return IRQ_HANDLED;
	}
	ret = mfc_reg_read(charger->client, MFC_INT_A_2_REG, &irq_src_2);
	if (ret < 0) {
		pr_err("%s: Failed to read INT_A_2_REG: %d\n", __func__, ret);
		__pm_relax(charger->wpc_ws);
		return IRQ_HANDLED;
	}
	...
	ret = mfc_reg_read(charger->client, MFC_STATUS_2_REG, &status_2);
	if (ret < 0) {
		pr_err("%s: Failed to read STATUS_2_REG: %d\n", __func__, ret);
		__pm_relax(charger->wpc_ws);
		return IRQ_HANDLED;
	}

	pr_info("%s: interrupt source(0x%.6x), status(0x%.6x)\n",
		__func__, irq_src_2 << 16 | irq_src_h << 8 | irq_src_l, status_2 << 16 | status_h << 8 | status_l);

	if (irq_src_2 & MFC_INTB_L_MPP_SUPPROT_MASK) {
		if (status_2 & MFC_STAT1_L_MPP_SUPPROT_MASK)
			pr_info("@MPP %s:Tx pad support MPP IRQ\n", __func__);
		else
			pr_info("@MPP %s:Tx pad not support MPP IRQ, Rx will enter EPP mode\n", __func__);
	}

	if (irq_src_2 & MFC_INTB_L_EPP_SUPPROT_MASK) {
		if (status_2 & MFC_STAT1_L_EPP_SUPPROT_MASK)
			pr_info("@EPP %s:Tx pad support EPP IRQ\n", __func__);
		else
			pr_info("@EPP %s:Tx pad not support EPP IRQ, Rx will enter BPP mode\n", __func__);
	}

	if (irq_src_2 & MFC_INTB_L_360K_NEGO_PASS_MASK) {
		pr_info("@MPP %s: Rx MPP nego pass IRQ\n", __func__);
		mfc_mpp_epp_nego_done(charger);
	}

	if (irq_src_2 & MFC_INTB_L_EPP_NEGO_PASS_MASK) {
		pr_info("@EPP %s:Rx EPP nego pass IRQ\n", __func__);
		mfc_mpp_epp_nego_done(charger);
	}

	if (irq_src_2 & MFC_INTB_L_EPP_NEGO_FAIL_MASK) {
		pr_info("@EPP %s:Rx EPP nego fail IRQ\n", __func__);
	}

	if (irq_src_2 & MFC_INTB_L_INCREASE_POWER_MASK) {
		pr_info("@MPP %s:Rx MPP increase power IRQ\n", __func__);
		value.intval = 1;
		psy_do_property("wireless", set, POWER_SUPPLY_EXT_PROP_MPP_ICL_CTRL, value);
	}

	if (irq_src_2 & MFC_INTB_L_DECREASE_POWER_MASK) {
		pr_info("@MPP %s:Rx MPP decrease power IRQ\n", __func__);
		value.intval = 0;
		psy_do_property("wireless", set, POWER_SUPPLY_EXT_PROP_MPP_ICL_CTRL, value);
	}

	if (irq_src_2 & MFC_INTB_L_EXIT_CLOAK_MASK) {
		pr_info("@MPP %s: Exit Cloak IRQ - reason : NOT defined yet, mpp_cloak(%d)\n",
...
INT_END:
	ret = mfc_reg_write(charger->client, MFC_INT_A_CLEAR_0_REG, irq_src_l); // clear int
	ret = mfc_reg_write(charger->client, MFC_INT_A_CLEAR_1_REG, irq_src_h); // clear int
	ret = mfc_reg_write(charger->client, MFC_INT_A_CLEAR_2_REG, irq_src_2); // clear int
	mfc_set_cmd_l_reg(charger, MFC_CMD_CLEAR_INT_MASK, MFC_CMD_CLEAR_INT_MASK); // command
	ret = mfc_reg_read(charger->client, MFC_STATUS_0_REG, &status_l);
	ret = mfc_reg_read(charger->client, MFC_STATUS_1_REG, &status_h);
	ret = mfc_reg_read(charger->client, MFC_STATUS_2_REG, &status_2);
	pr_info("%s: status after clear irq, status(0x%.6x)\n", __func__, status_2 << 16 | status_h << 8 | status_l);
...
static int mfc_chg_parse_dt(struct device *dev, mfc_charger_platform_data_t *pdata)
{
	...
	pdata->is_rom2 = of_property_read_bool(np, "battery,is_rom2");
	if (pdata->is_rom2) {
		pdrc_active = 1;
		pdrc_inactive = 0;
	} else {
...
static bool check_deferred_probe(unsigned int bin_fw_ver)
{
	static int retry_cnt = 10;
						...
						const struct i2c_device_id *id)
{
	...
	if (check_deferred_probe(mfc_get_bin_fw_version(charger))) {
		ret = -EPROBE_DEFER;
		goto err_i2cfunc_not_support;
	}
