...
static int s2asl01_get_inow(struct s2asl01_limiter_data *limiter, int mode)
{
	int inow, ichg, idischg;

	ichg = s2asl01_get_ichg(limiter, mode);
	idischg = s2asl01_get_idischg(limiter, mode);

	inow = (idischg == 0) ? ichg : (idischg * -1);
	return inow;
}
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		val->intval = s2asl01_get_inow(limiter, val->intval);
		break;
...
static int s2asl01_limiter_parse_dt(struct device *dev, struct s2asl01_platform_data *pdata)
{
	...
	if (np == NULL) {
		...
		if (pdata->bat_type & LIMITER_MAIN) {
			...
			np = of_find_node_by_name(NULL, "s2asl01-limiter-sub");
			if (!np) {
				pr_info("%s: s2asl01-limiter-sub is Empty\n", __func__);
			} else {
				ret = pdata->bat_det2 = of_get_named_gpio(np, "limiter,sub_bat_det_gpio", 0);
				if (ret < 0) {
			...
			np = of_find_node_by_name(NULL, "s2asl01-limiter-main");
			if (!np) {
				pr_info("%s: s2asl01-limiter-main is Empty\n", __func__);
			} else {
				ret = pdata->bat_det2 = of_get_named_gpio(np, "limiter,main_bat_det_gpio", 0);
				if (ret < 0) {
...
static void s2asl01_init_regs(struct s2asl01_limiter_data *limiter)
{
	...
	/* SEL_VOLT_REFRESH=1 */
	s2asl01_update_reg(limiter->client, S2ASL01_LIMITER_PM_V_OPTION, 0x1 << 4, 0x30);
...
#if defined(CONFIG_SEC_FACTORY)
	if (gpio_get_value(pdata->bat_det) || gpio_get_value(pdata->bat_det2)) {
		dev_err(&client->dev, "%s main or sub battery is not connected, do not load limiter driver\n",
