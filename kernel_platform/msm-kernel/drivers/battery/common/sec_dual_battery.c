...
static int sec_dual_battery_current(struct sec_dual_battery_info *battery, int bat_type, int mode)
{
	union power_supply_propval value;
	int inow = 0;
...
static int sec_dual_battery_voltage_now(struct sec_dual_battery_info *battery, int bat_type, int mode)
{
	union power_supply_propval value;
	int vcell = 0;
			    ...
			    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		if (value.intval == SEC_DUAL_BATTERY_MAIN)
			val->intval = sec_dual_battery_voltage_now(battery, SEC_DUAL_BATTERY_MAIN, SEC_BATTERY_VOLTAGE_MV);
		else
			val->intval = sec_dual_battery_voltage_now(battery, SEC_DUAL_BATTERY_SUB, SEC_BATTERY_VOLTAGE_MV);
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (value.intval == SEC_DUAL_BATTERY_MAIN)
			val->intval = sec_dual_battery_current(battery, SEC_DUAL_BATTERY_MAIN, SEC_BATTERY_CURRENT_MA);
		else
			val->intval = sec_dual_battery_current(battery, SEC_DUAL_BATTERY_SUB, SEC_BATTERY_CURRENT_MA);
		break;
			    ...
			    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CHARGING_ENABLED:
			/*
			 * SEC_DUAL_BATTERY_BUCK_OFF: sub charge off
			 * SEC_DUAL_BATTERY_CHG_ON: sub charge on
			 * SEC_DUAL_BATTERY_CHG_OFF: sub charge off
			 */
			value.intval = val->intval;
			if (value.intval == SEC_DUAL_BATTERY_CHG_ON)
		...
		struct sec_dual_battery_info *battery)
{
	...
	ret = pdata->main_bat_det_gpio = of_get_named_gpio(np, "battery,main_bat_det_gpio", 0);
	if (ret < 0)
		pr_info("%s : can't get main_bat_det_gpio\n", __func__);

	/* SUB_BATTERY_CON_DET */
	ret = pdata->sub_bat_det_gpio = of_get_named_gpio(np, "battery,sub_bat_det_gpio", 0);
	if (ret < 0)
