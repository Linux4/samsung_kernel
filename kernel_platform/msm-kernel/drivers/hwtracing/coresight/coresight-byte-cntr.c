			...
			char **bufpp)
{
	long rwp_offset, req_size, actual = 0;
            ...
            if (req_size >= PAGE_SIZE) {
                actual = tmc_etr_buf_get_data(etr_buf, *ppos,
                             PAGE_SIZE, bufpp);
            } else if (req_size > 0)
                actual = tmc_etr_buf_get_data(etr_buf, *ppos, req_size, bufpp);
            else if (req_size == 0)
                dev_info(&tmcdrvdata->csdev->dev, " === tmc_etr_flush_remaining_bytes === flush is done. req_size is 0 \n");			
			       ...
			       size_t len, loff_t *ppos)
{
	...
	if (!byte_cntr_data->read_active) {
		if (!byte_cntr_data->return_epipe) {
			byte_cntr_data->return_epipe = true;
			dev_info(&tmcdrvdata->csdev->dev, "Read active is false, return -EPIPE\n");
			ret = -EPIPE;
			goto err0;
		}
...
copy:
	...
	byte_cntr_data->total_read_size += len;
	byte_cntr_data->position = *ppos;
	goto out;
...
void tmc_etr_byte_cntr_start(struct byte_cntr *byte_cntr_data)
{
	...
	byte_cntr_data->total_read_size = 0;
...
void tmc_etr_byte_cntr_stop(struct byte_cntr *byte_cntr_data)
{
	...
	dev_info(&byte_cntr_data->tmcdrvdata->csdev->dev, "copy_to_user total size %ld total irq %ld, rwpoffset %ld, current position %lld\n",
			byte_cntr_data->total_read_size, byte_cntr_data->total_irq, rwp_offset, byte_cntr_data->position);
}
...
static int tmc_etr_byte_cntr_release(struct inode *in, struct file *fp)
{
	...
	if (byte_cntr_data->enable)
      ...
      byte_cntr_data->return_epipe = false;
	disable_irq_wake(byte_cntr_data->byte_cntr_irq);
	mutex_unlock(&byte_cntr_data->byte_cntr_lock);

	dev_info(&byte_cntr_data->tmcdrvdata->csdev->dev, "====== tmc_etr_byte_cntr_release ======= %ld", byte_cntr_data->total_read_size);
	return 0;
}
