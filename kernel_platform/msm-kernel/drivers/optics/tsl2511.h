...
#ifndef __AMS_TSL2511_H
#define __AMS_TSL2511_H
...
#define CONFIG_AMS_OPTICAL_SENSOR_2511
...
#define CONFIG_AMS_OPTICAL_SENSOR_2511_UVA /* 3ch: Photopic, IR, UVA */
...
enum fft_size {
	...
	#define AMS_SAMPLING_TIME					(488)  /* usec */
	...
	//#define AMS_FLICKER_NUM_SAMPLES             (768)
	// Photopic, IR, UV = 128 samples ,  128 * 2byte * 3ch = 768byte
	#define AMS_FLICKER_NUM_SAMPLES             (AMS_CLR_WIDE_FLICKER_NUM_SAMPLES * 2 * 3)
	// Photopic, IR, UV = 128 samples ,  128 * 2byte * 3ch = 768byte
	#define AMS_CLR_WIDE_FLICKER_NUM_SAMPLES    (AMS_FFT_SIZE)
	#else
	...
	#define AMS_FLICKER_THR_LVL                   (400)
	//#define AMS_FLICKER_THR_LVL                   (128)

	//#define AMS_FFT_SIZE FFT_256
	#define AMS_FFT_SIZE FFT_128
#else
	/* 783=> (783+1)*0.625 clock , 490 usec */
	#define AMS_SAMPLING_TIME					(783)
	#define DEFAULT_SAMPLE_TIME_INT_CLK         (352)   // 2048hz
	//#define AMS_SAMPLING_TIME					(1000)  /* usec */
	//#define AMS_SAMPLING_TIME					(500)  /* usec */
	//#define AMS_SAMPLING_TIME					(250)  /* usec */
	...
	// FLICKER SETTING VALUES
#ifdef CONFIG_AMS_OPTICAL_SENSOR_2511_UVA
	#define AMS_FLICKER_NUM_BYTE_SAMPLES             (768)
	// Photopic, IR, UV , BK = 31 samples + 1 (interpolatio samples) ,31* 3ch * 2byte = 186, 32* 3ch * 2 = 192
	#define AMS_FLICKER_NUM_SAMPLES    (31)
	// Clear & wide & uv = 128 samples ,  128 * 2byte * 3ch = 768byte
	#define AMS_CLR_WIDE_FLICKER_NUM_SAMPLES    (768)
#else
	#define AMS_FLICKER_NUM_SAMPLES             (512)
	#define AMS_CLR_WIDE_FLICKER_NUM_SAMPLES    (128) // Clear & wide = 128 samples ,  128 * 2byte * 2ch = 512byte
...
#define AMS_FLICKER_THR_LVL                   (32)
//#define AMS_FLICKER_THR_LVL                   (128)

// average/max thd
#define FLICKER_AVGMAX_THD      65LL

#define AMS_FFT_SIZE FFT_1024 //2hz resolution
//#define AMS_FFT_SIZE FFT_2048 //1hz resolution

/*sw_agc*/
#define AMS_ADC_SATURATION_MAX_COUNT     16383
#define AMS_TSL2511_ADC_HIGH_THRESH     90
#define AMS_TSL2511_ADC_NEAR_HIGH_THRESH 75
#define AMS_TSL2511_ADC_LOW_THRESH      35
#define AMS_TSL2511_ADC_MID_THRESH      50
#define AMS_TSL2511_Y_MAX_GAIN          13
#define AMS_TSL2511_IR_MAX_GAIN         13
#define AMS_TSL2511_UVA_MAX_GAIN        11
#define AMS_TSL2511_BK_MAX_GAIN         11
#define STEP_DOWN 3

#define AMS_TSL2511_Y_MIN_GAIN          0
#define AMS_TSL2511_IR_MIN_GAIN         0
#define AMS_TSL2511_UVA_MIN_GAIN        0
#define AMS_TSL2511_BK_MIN_GAIN         0
...
typedef struct _amsAlsContext {
	...
	uint32_t UVGain;// UV Channel GAIN
	uint32_t BKGain;// BK Channel GAIN
	uint8_t UV_CALIB;
	uint8_t IR_CALIB;
	//uint32_t rawUvCorrected;
...
#define CONFIG_AMS_LITTLE_ENDIAN
#if defined(CONFIG_AMS_LITTLE_ENDIAN)
...
typedef enum _deviceRegisters {
	DEVREG_BK_CALIB, //0x04
	DEVREG_IR_CALIB, //0x05 it should be only OTP_5 read
	DEVREG_UV_CALIB, //0x07 it should be only OTP_7 read
	...
	DEVREG_MEAS_SEQR_STEP0_MOD_GAINX_H,//0xD5
	DEVREG_MEAS_SEQR_STEP1_MOD_GAINX_L,//0xD6
	DEVREG_MEAS_SEQR_STEP1_MOD_GAINX_H,//0xD7
	DEVREG_MEAS_SEQR_STEP2_MOD_GAINX_L,//0xD8
	DEVREG_MEAS_SEQR_STEP3_MOD_GAINX_L,//0xDA
	DEVREG_MEAS_SEQR_STEP0_MOD_PHDX_SMUX,//0xDC
	DEVREG_MEAS_SEQR_STEP1_MOD_PHDX_SMUX,//0xDE
	DEVREG_MEAS_SEQR_AGC_ASAT_PATTERN,//0xDF
	DEVREG_MEAS_SEQR_STEP2_MOD_PHDX_SMUX,//0xE0
	DEVREG_MEAS_SEQR_AGC_PREDICT_PATTERN,// 0xE1
	DEVREG_MEAS_SEQR_STEP3_MOD_PHDX_SMUX_L,//0xE2
	DEVREG_MOD_CALIB_CFG0,//0xE4
	DEVREG_MOD_CALIB_CFG2,//0xE6
	DEVREG_MOD_CALIB_STATUS0,//0xEB
	DEVREG_VSYNC_DELAY_CFG0,//0xF0
	DEVREG_VSYNC_DELAY_CFG1,//0xF1
	...
	DEVREG_FIFO_STATUS0,//0xFD
	DEVREG_FIFO_STATUS1,//0xFE
	DEVREG_FIFO_DATA,//0xFF
	DEVREG_REG_MAX
} ams_deviceRegister_t;

typedef enum _2511_regOptions {
...
typedef enum _2511_regMasks {
	...
	MASK_MOD_CALIB_NTH_ITERATION_RC_ENABLE = 0X80, /* register 0xE6 */
	MASK_MOD_CALIB_NTH_ITERATION_AZ_ENABLE = 0x40, /* register 0xE6 */
	MASK_MOD_CALIB_NTH_ITERATION_AGC_ENABLE = 0x20,
	MASK_MOD_CALIB_RESIDUAL_ENABLE_AUTO_CALIB_ON_GAIN_CHANGE = 0x10,
...
typedef enum _2511_configureFeature {
	AMS_CONFIG_PROX,
	AMS_CONFIG_ALS_LUX,
	AMS_CONFIG_ALS_RGB,
	AMS_CONFIG_ALS_CT,
...
typedef struct _channels_max_1024samples {
	uint16_t y_max;
	uint16_t ir_max;
	uint16_t bk_max;
	uint16_t uva_max;
} channels_max_1024samples;
...
typedef struct _flickerParams {
	...
	als_channels als_raw_data;
	channels_max_1024samples max_samples;
} ams_flicker_ctx_t;

typedef struct _2511Context {
	ams_deviceIdentifier_e deviceId;
	uint64_t timeStamp;
	AMS_PORT_portHndl *portHndl;
	ams_mode_t mode;
#if IS_ENABLED(AMS_PHY_SUPPORT_SHADOW)
	...
	uint8_t shadowStatus3Reg;    // STATUS2
	uint8_t shadowStatus5Reg;    // STATUS2
	uint8_t shadowAlsStatusReg;  // ALS_STATUS
	uint8_t shadowAlsStatus2Reg;  // ALS_STATUS2
	uint8_t shadowFIFOStatusReg;  // FifoStatus
	uint8_t ir_calib_val;
	uint8_t uva_calib_val;
	uint8_t bk_calib_val;
	uint32_t clear_average_fifo;
	uint8_t vsyncControl;  // FifoStatus
	uint32_t wideband_average_fifo;
	uint32_t wb_leak_average_fifo;
	//uint16_t bk_average_fifo;
	bool has_fifo_fd_end_marker;
	bool has_fifo_fd_gain;
	bool has_fifo_fd_checksum;
 	uint32_t adc_max;               /*sw_agc*/
	uint32_t adc_max_near_high_thresh;
	uint32_t adc_max_high_thresh;
	uint32_t adc_max_low_thresh;
	uint32_t adc_max_mid_thresh;

	uint8_t fifo_end_size;
	uint8_t fifo_mod0_gain;
	uint8_t fifo_mod1_gain;
	uint8_t fifo_mod2_gain;
	uint16_t fifo_checksum;
	uint32_t clear_average;
	uint32_t wideband_average;
	uint32_t wb_bk_average;
#ifdef CONFIG_AMS_OPTICAL_SENSOR_2511_UVA
	uint32_t uv_average_fifo; 
	uint32_t uv_average;
#endif
	uint16_t flicker_num_samples;
	uint16_t flicker_1_frame;
	bool hamming_status;
	bool internal_clock_once;
	int sum_bk_bkCal_offset;
	int sum_bk_bkCal_kcoef;
	uint16_t Step_Samples;
	uint8_t Flicker_Channels; //pho , uv , (bk/ir)
	uint8_t Data_bytes;//2byte
	uint8_t Gain_Bytes;//last 2byte
	bool ir_calib_val_valid;
	bool uva_calib_val_valid;
	uint32_t mod1_step_counter;
} ams_deviceCtx_t;
...
struct amsdriver_i2c_platform_data {
	...
	char const *als_name;
	struct tsl2511_parameters parameters;
#if IS_ENABLED(CONFIG_OF)
	struct device_node  *of_node;
#endif
};

struct tsl2511_device_data {
