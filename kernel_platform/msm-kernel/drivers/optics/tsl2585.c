...
#define TSL2585_CHIP_NAME	"TSL2585"
...
#define TSL2585_PROBE_DEFER_ENABLE

#define TSL2585_SLAVE_I2C_ADDR_REVID_V0 0x39
...
#define TSL2585_IOCTL_MAGIC		0xFD
#define TSL2585_IOCTL_READ_FLICKER	_IOR(TSL2585_IOCTL_MAGIC, 0x01, uint16_t *)
...
#define OSC_CAL_SPEC_MARGIN     1000
//#define OSC_CAL_SPEC_IN(X)      (((X > (18000 - OSC_CAL_SPEC_MARGIN)) && (X < (18000 + OSC_CAL_SPEC_MARGIN)))?"PASS":"FAIL")
#define OSC_CAL_SPEC_IN(X)      (((X > 0) && (X < 0xffff))?"PASS":"FAIL")
...
static struct tsl2585_device_data *tsl2585_data;
...
#define OSC_CAL_DEFAULT_PWM_PERIOD	(25000000)
...
#define AMS_GET_MOD_GAIN(scaledGain, scaledGain1, scaledGain2, gain, ret)	{ret = ams_getBuf(ctx->portHndl, DEVREG_MEAS_SEQR_STEP0_MOD_GAINX_L, (char*)&(gain), 2); \
								scaledGain = alsGain_conversion[(gain) & 0x0F]; \
								scaledGain1 = alsGain_conversion[(((gain) & 0xF0) >> 4)]; \
								scaledGain2 = alsGain_conversion[(((gain) & 0xF00) >> 8)];}
...
#define AMS_ALS_SENSOR_6PD_TURNON(ret)  {ret = ams_setByte(ctx->portHndl, DEVREG_MEAS_SEQR_STEP0_MOD_PHDX_SMUX_L, 0xE6); \
					ret = ams_setByte(ctx->portHndl, DEVREG_MEAS_SEQR_STEP0_MOD_PHDX_SMUX_H, 0x07);}
...
#define TSL2585_MEASUREMENT_SEQUENCER_MOD0_FD_PATTERN_SHIFT 0
#define TSL2585_MASK_MEASUREMENT_SEQUENCER_MOD0_FD_PATTERN (0x0F << TSL2585_MEASUREMENT_SEQUENCER_MOD0_FD_PATTERN_SHIFT)

#define TSL2585_MEASUREMENT_SEQUENCER_MOD1_FD_PATTERN_SHIFT 4
#define TSL2585_MASK_MEASUREMENT_SEQUENCER_MOD1_FD_PATTERN (0x0F << TSL2585_MEASUREMENT_SEQUENCER_MOD1_FD_PATTERN_SHIFT)

#define TSL2585_MEASUREMENT_SEQUENCER_MOD2_FD_PATTERN_SHIFT 4
#define TSL2585_MASK_MEASUREMENT_SEQUENCER_MOD2_FD_PATTERN (0x0F << TSL2585_MEASUREMENT_SEQUENCER_MOD2_FD_PATTERN_SHIFT)

#define TSL2585_MEASUREMENT_SEQUENCER_ALS_PATTERN_SHIFT 0
#define TSL2585_MASK_MEASUREMENT_SEQUENCER_ALS_PATTERN (0x0F << TSL2585_MEASUREMENT_SEQUENCER_ALS_PATTERN_SHIFT)

#define TSL2585_MOD_FIFO_FD_GAIN_WRITE_ENABLE_SHIFT 5
#define TSL2585_MASK_MOD_FIFO_FD_GAIN_WRITE_ENABLE (0x01 << TSL2585_MOD_FIFO_FD_GAIN_WRITE_ENABLE_SHIFT)

#define TSL2585_MOD_FIFO_FD_END_MARKER_WRITE_ENABLE_SHIFT 7
#define TSL2585_MASK_MOD_FIFO_FD_END_MARKER_WRITE_ENABLE (0x01 << TSL2585_MOD_FIFO_FD_END_MARKER_WRITE_ENABLE_SHIFT)
...
static ams_deviceIdentifier_t deviceIdentifier[] = {
	{ AMS_DEVICE_ID, AMS_DEVICE_ID_MASK, AMS_REV_ID_2585, AMS_REV_ID_MASK, AMS_TSL2585 },
	{ AMS_DEVICE_ID, AMS_DEVICE_ID_MASK, AMS_REV_ID_UNTRIM, AMS_REV_ID_MASK, AMS_TSL2585_UNTRIM },
...
uint16_t tsl2585_gain_conversion[] = {
	1, /* == 0.5 */
	2,
	4,
	8,
...
{   //data must be twice as long as size
	...
	for (i = 0; i < size; ++i) {
		...
		buffer[i] = (int32_t)ams_isqrt(square); //29bit
	}
}
	...
	// buf_r <= 16bits data
{
	...
	for (i = 0; i < size; i++) {
		...
		if (!tsl2585_data->saturation && data[i] >= 0x3FFF) {
			ALS_info("DEBUG_FLICKER saturation");
			tsl2585_data->saturation = true;
		}
#endif
		buf_r[i] = ((int64_t)data[i]*(int64_t)hamming[i * hamming_step]) >> 2;        // 16+16-2=30
		if (log_cnt > 30)
			ALS_info("DEBUG_FLICKER data[%d] => %d buf[%d] => %d", i, data[i], i, buf_r[i]);
	}
...
static const struct of_device_id tsl2585_match_table[] = {
	{.compatible = "ams,tsl2585",},
	{},
};
...
static long tsl2585_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	void __user *argp = (void __user *)arg;
	int ret = 0;
	//int i = 0;
	int data_length = 0;
	struct tsl2585_device_data *data = container_of(file->private_data, struct tsl2585_device_data, miscdev);

	ALS_dbg("%s - ioctl start, %d\n", __func__, cmd);
	mutex_lock(&data->flickerdatalock);

	switch (cmd) {
		case TSL2585_IOCTL_READ_FLICKER:

			data_length = ams_deviceGetFlickerData(data->deviceCtx, (void*)&data->flicker_data[0]);
			ALS_dbg("%s - TSL2585_IOCTL_READ_FLICKER = %d\n", __func__, data->flicker_data[0]);
...
static const struct file_operations tsl2585_fops = {
	.owner = THIS_MODULE,
	.open = nonseekable_open,
	.unlocked_ioctl = tsl2585_ioctl,
};
...
static void tsl2585_debug_var(struct tsl2585_device_data *data)
{
	ALS_dbg("===== %s =====\n", __func__);
	...
	ALS_dbg("%s pin_als_int %d\n", __func__, data->pin_als_int);
	ALS_dbg("%s pin_als_pwm %d\n", __func__, data->pin_als_pwm);
	ALS_dbg("%s als_irq %d\n", __func__, data->dev_irq);
	ALS_dbg("%s irq_state %d\n", __func__, data->irq_state);
	ALS_dbg("%s osc_cal %d\n", __func__, data->osc_cal);
	ALS_dbg("%s pwm_period %d\n", __func__, data->pwm_period);
	ALS_dbg("===== %s =====\n", __func__);
}

static int tsl2585_write_reg_bulk(struct tsl2585_device_data *device, u8 reg_addr, u8* data, u8 length)
{
	int err = -1;
	int tries = 0;
	int num = 1;
...
static int tsl2585_write_reg(struct tsl2585_device_data *device,
		u8 reg_addr, u8 data)
{
	int err = -1;
	int tries = 0;
...
static int tsl2585_read_reg(struct tsl2585_device_data *device,
...
{
	struct tsl2585_device_data *data = i2c_get_clientdata(portHndl);
	int err = 0;
	uint8_t length = 1;
	...
	err = tsl2585_read_reg(data, deviceRegisterDefinition[reg].address, readData, length);
...
static int ams_setByte(AMS_PORT_portHndl *portHndl, ams_deviceRegister_t reg, uint8_t setData)
{
	...
	err = tsl2585_write_reg(data, deviceRegisterDefinition[reg].address, setData);
...
int ams_setBuf(AMS_PORT_portHndl *portHndl, ams_deviceRegister_t reg, uint8_t *setData, uint8_t length)
{
	...
	err = tsl2585_write_reg_bulk(data, deviceRegisterDefinition[reg].address, setData, length);
	return err;
}
...
static int ams_getWord(AMS_PORT_portHndl *portHndl, ams_deviceRegister_t reg, uint16_t *readData)
{
	...
	err = tsl2585_read_reg(data, deviceRegisterDefinition[reg].address, buffer, length);
...
static int ams_setWord(AMS_PORT_portHndl *portHndl, ams_deviceRegister_t reg, uint16_t setData)
{
	...
	err = tsl2585_write_reg(data, deviceRegisterDefinition[reg].address, buffer[0]);
	err = tsl2585_write_reg(data, deviceRegisterDefinition[reg + 1].address, buffer[1]);
...
int ams_getField(AMS_PORT_portHndl *portHndl, ams_deviceRegister_t reg, uint8_t *setData, ams_regMask_t mask)
{
	...
	err = tsl2585_read_reg(data, deviceRegisterDefinition[reg].address, setData, length);
...
static int ams_setField(AMS_PORT_portHndl *portHndl, ams_deviceRegister_t reg, uint8_t setData, ams_regMask_t mask)
{
	...
	err = tsl2585_read_reg(data, deviceRegisterDefinition[reg].address, &original_data, length);
	if (err < 0)
		return err;
	...
	if (new_data != original_data)
		err = tsl2585_write_reg(data, deviceRegisterDefinition[reg].address, new_data);
...
static int amsAlg_als_processData(amsAlsContext_t *ctx, amsAlsDataSet_t *inputData)
{
	...
	if (inputData->status & ALS_STATUS_RDY) {
		...
		ctx->results.rawWideband = (16 * (inputData->datasetArray->widebandADC)) / 10;
		ctx->results.rawUv = inputData->datasetArray->uvADC;
		ctx->results.irrClear = inputData->datasetArray->clearADC;
		ctx->results.irrWideband = (16 * (inputData->datasetArray->widebandADC)) / 10;
	}
	...
	/*UV Corrected Calcuation*/
	ctx->results.rawUvCorrected = ((100 + (ctx->UV_CALIB - 127)) * ctx->results.rawUv)/100;
	ALS_info("%s - after UV_RAW  :%d , corrected UV : %d cal lib %d\n", __func__,
			ctx->results.rawUv, ctx->results.rawUvCorrected, ctx->UV_CALIB);
...
static void tsl2585_sequencer_init(ams_deviceCtx_t *ctx)
{
	int ret = 0;
	ALS_dbg("%s \n",__func__) ;
	...
	ret = ams_setField(ctx->portHndl,
			...
			0x01,
			TSL2585_MASK_MEASUREMENT_SEQUENCER_ALS_PATTERN );
	if (ret < 0) {
	...
	ret = ams_setField(ctx->portHndl,
			...
			0x01,
			TSL2585_MASK_MEASUREMENT_SEQUENCER_MOD0_FD_PATTERN );
	if (ret < 0) {
	...
	ret = ams_setField(ctx->portHndl,
			DEVREG_MEAS_SEQR_FD_0,
			0x10,
			TSL2585_MASK_MEASUREMENT_SEQUENCER_MOD1_FD_PATTERN );
	if (ret < 0) {
		ALS_err("%s - failed to set SEQR_FD_0",__func__);
	}
	...
	ret = ams_setField(ctx->portHndl,
			DEVREG_MEAS_SEQR_ALS_FD_1,
			0x10,
			TSL2585_MASK_MEASUREMENT_SEQUENCER_MOD2_FD_PATTERN );
	if (ret < 0) {
		ALS_err("%s - failed to set SEQR_ALS_FD_1",__func__);
	}
}

static void tsl2585_fifo_format_init(ams_deviceCtx_t *ctx)
{
	int ret = 0;
	...
	ret = ams_setField(ctx->portHndl,
			DEVREG_MEAS_MODE1,
			HIGH,
			TSL2585_MASK_MOD_FIFO_FD_END_MARKER_WRITE_ENABLE);
	if (ret < 0) {
	...
	ret = ams_setField(ctx->portHndl,
			DEVREG_MEAS_MODE1,
			HIGH,
			TSL2585_MASK_MOD_FIFO_FD_GAIN_WRITE_ENABLE);
	if (ret < 0) {
...
#endif
	tsl2585_sequencer_init(ctx);
	tsl2585_fifo_format_init(ctx);
...
#define TSL2585_THD_CLEAR      1800LL    /* 1.8 * 1000 */
#define TSL2585_THD_RATIO      3LL       /* 0.003 * 1000 */
#define TSL2585_GAIN_MAX       4096LL    /* 4096 */
#define TSL2585_THD_RATIO_AUTO 1000LL       /*  */
//#define MAX_NUM_FLICKER_SAMPLES 2048LL
#define MAX_NUM_FLICKER_SAMPLES AMS_FFT_SIZE
#define MAX_NUM_FLICKER_SAMPLE_BYTES (MAX_NUM_FLICKER_SAMPLES * 4)
...
static uint64_t calc_thd(uint16_t clear_avg_fifo, uint32_t clear_avg, uint32_t *fft_out_data)
{
	...
	if (clear_avg_fifo <= 1) {
		ratio_fixed = (TSL2585_THD_CLEAR * TSL2585_GAIN_MAX);
	} else {
		if (clear_avg) {
			ratio_fixed = (TSL2585_THD_CLEAR << 7)/ clear_avg; // clear_avg left-shifted 7.
		}
		else {
			ratio_fixed = (TSL2585_THD_CLEAR << 7); // clear_avg left-shifted 7.
		}
	}

	if ((TSL2585_THD_RATIO_AUTO) > ratio_fixed) {
		ratio_final = TSL2585_THD_RATIO_AUTO;
	} else {
		ratio_final = ratio_fixed;
	}

	threshold = ((uint64_t)fft_out_data[0] * ratio_final * TSL2585_THD_RATIO)/1000000;

	ALS_info("calc_thd threshold = %lld, fifoout[0] %u, ratio_final %lld \n", threshold, fft_out_data[0], ratio_final);
	return threshold;
}

static uint8_t fifo_out_data[AMS_FLICKER_NUM_SAMPLES];
...
#define CHANNEL 3 //(Photopic, IR, UVA)
...
int get_fft(ams_deviceCtx_t * ctx , uint32_t *out)
{
	...
	static uint32_t wideband_buffer[AMS_FFT_SIZE] = { 0 };
	static uint32_t uv_buffer[AMS_FFT_SIZE] = { 0 };
	ssize_t size = 0;
	int i, j = 0;
	uint16_t clear_gain, wideband_gain = 0;
	uint8_t fifo_mod0_gain, fifo_mod1_gain = 0;
	uint16_t uv_gain;
	uint8_t fifo_mod2_gain=0;
	int ret = 0;

	memset(clear_buffer, 0, AMS_FFT_SIZE * sizeof(uint32_t));
	memset(wideband_buffer, 0, AMS_FFT_SIZE * sizeof(uint32_t));
	memset(uv_buffer, 0, AMS_FFT_SIZE * sizeof(uint32_t));
	memset(buffer, 0x00, AMS_FLICKER_NUM_SAMPLES * 2);

	size = read_fifo(ctx,buffer,AMS_FLICKER_NUM_SAMPLES); //512 byte
	if (size > AMS_FLICKER_NUM_SAMPLES) {
		ALS_err("%s - invalid read_fifo size = %d", __func__, size);
		size = AMS_FLICKER_NUM_SAMPLES;
	};

	kfifo_reset(&ams_fifo);

	/* Separate Clear and Wideband data */
	for (i = 0, j = 0; i < size/BYTE; i+=CHANNEL, j++) { //768byte = 384 sample (Photopic, IR, UVA)
		/* Photopic */
		out[j] = clear_buffer[j] = buffer[i]; //separate 128
		/* IR */
		wideband_buffer[j] = buffer[i + 1]; //separate 128
		/* UVA */
		uv_buffer[j] = buffer[i + 2]; //separate 128
	}
	...
	ctx->uv_average_fifo = calc_average(uv_buffer, AMS_CLR_WIDE_FLICKER_NUM_SAMPLES);//128

	fifo_mod0_gain = ctx->fifo_mod0_gain & 0x0F;//clear
	fifo_mod1_gain = (ctx->fifo_mod0_gain >> 4) & 0x0F;//wide
	fifo_mod2_gain = ctx->fifo_mod2_gain & 0x0F;//uv

	if (fifo_mod0_gain >= (sizeof(tsl2585_gain_conversion) / sizeof(uint16_t))) {
		ALS_err("%s - invalid gain0 = %d (fifo_mod0_gain = 0x%x)", __func__, fifo_mod0_gain, ctx->fifo_mod0_gain);
		fifo_mod0_gain = 1;
	};

	if (fifo_mod1_gain >= (sizeof(tsl2585_gain_conversion) / sizeof(uint16_t))) {
		ALS_err("%s - invalid gain1 = %d (fifo_mod1_gain = 0x%x)", __func__, fifo_mod1_gain, ctx->fifo_mod0_gain);
		fifo_mod1_gain = 1;
	};

	if (fifo_mod2_gain >= (sizeof(tsl2585_gain_conversion) / sizeof(uint16_t))) {
		ALS_err("%s - invalid gain2 = %d (fifo_mod2_gain = 0x%x)", __func__, fifo_mod1_gain, ctx->fifo_mod2_gain);
		fifo_mod2_gain = 1;
	};

	clear_gain = tsl2585_gain_conversion[fifo_mod0_gain];
	wideband_gain = tsl2585_gain_conversion[fifo_mod1_gain];
	uv_gain = tsl2585_gain_conversion[fifo_mod2_gain];
	...
	ctx->uv_average = ((uint32_t)ctx->uv_average_fifo) << 7;
	ctx->uv_average <<= 4;
	ctx->uv_average /= uv_gain;

	ALS_info("%s - cavg_fifo: %d, wavg_fifo: %d, uvavg_fifo: %d, cavg: %d, wavg: %d uv: %d", __func__,
			ctx->clear_average_fifo, ctx->wideband_average_fifo, ctx->uv_average_fifo,
			ctx->clear_average, ctx->wideband_average);
	ALS_info("%s - gain0 = %d , gain1 = %d , gain2 = %d", __func__,
			fifo_mod0_gain, fifo_mod1_gain, fifo_mod2_gain);
...
bool  ccb_sw_bin4096_flicker_GetResult(void * dcbCtx)
{
	...
	int i =0;
	uint16_t mHz = 9999;
	uint resolution = DIV_ROUND_CLOSEST(1000000000, AMS_SAMPLING_TIME) / AMS_FFT_SIZE;
	uint64_t thd =0;
	...
	if(get_fft(ctx,buf)){
		//buf[0] = 0;
		for (i = min_freq; i <= max_freq; ++i) {			// 70~500
			ALS_info("ccb_sw_bin4096_flicker_GetResult: buf[%d]=%10u \n",i,buf[i]);
		...
		ALS_info("DEBUG_FLICKER DC:%u", buf[0]);
		ALS_info("DEBUG_FLICKER buf[%d]=%u, thd %lld \n", max, buf[max],thd);
...
void fifo_get_als(uint32_t *clearband, uint32_t *wideband, uint32_t *uvband, size_t size)
{
	uint32_t widebuf[AMS_FLICKER_THR_LVL];
	uint32_t clearbuf[AMS_FLICKER_THR_LVL];
	uint32_t uvbuf[AMS_FLICKER_THR_LVL];
	size_t widecnt = 0, clearcnt = 0, uvcnt = 0;
	int tmp, i, start, end;
	...
	*uvband = calc_average(uvbuf, uvcnt);
}
...
static bool ccb_FIFOEvent(void *dcbCtx)
{
	...
	num_sample_bytes = num_samples*6;
	...
	if((len = kfifo_len(&ams_fifo)) >= (num_samples)) //waiting len = 768 byte for 384 sample level
		//if((len = kfifo_len(&ams_fifo)) >= (num_sample_bytes)) // 512 SAMPLE * clear & wide
	{
		...
		uint32_t wideband, clearband, uvband;
		uint32_t gain0, gain1, gain2;
		u16 gain;

		inputData.status = ALS_STATUS_RDY;
		inputData.datasetArray = (alsData_t *)&dataSet;
		fifo_get_als(&clearband, &wideband, &uvband, ccbCtx->fifolvl);

		AMS_GET_MOD_GAIN(gain0, gain1, gain2, gain, ret);
		if (ret < 0) {
		...
		ctx->uv_average = uvband << 7;
		ctx->uv_average <<= 4;
		ctx->uv_average /= (gain2/500);

		alsCtx->ClearGain = gain0;
		alsCtx->WBGain = gain1;
		alsCtx->UVGain = gain2;
		alsCtx->UV_CALIB = ctx->shadowUV_CALIB;
		alsCtx->time_us = (AMS_SAMPLING_TIME / 1000) * (ccbCtx->fifolvl / 6);

		dataSet.AdcClear = ctx->clear_average;
		dataSet.AdcWb = ctx->wideband_average;
		dataSet.AdcUv = ctx->uv_average;

		amsAlg_als_processData(&ctx->ccbAlsCtx.ctxAlgAls, &inputData);
		ctx->updateAvailable |= (1 << AMS_AMBIENT_SENSOR);

		ALS_info("cavg_fifo: %d, wavg_fifo: %d, uvavg_fifo: %d,  gain0=%d , gain1= %d, gain2= %d, cavg: %d, wavg: %d, uvavg: %d time_us:%d\n",
				clearband, wideband, uvband, gain0, gain1, gain2,
				ctx->clear_average, ctx->wideband_average, ctx->uv_average, alsCtx->time_us);
	}
#endif
	return false;
}
...
static void ccb_alsGetResult(void *dcbCtx, ams_ccb_als_result_t *exportData)
{
	...
	exportData->rawUvCorrected = ccbCtx->ctxAlgAls.results.rawUvCorrected;
}

static bool _2585_alsSetThreshold(ams_deviceCtx_t *ctx, int32_t threshold)
{
	...
	if (feature == AMS_CONFIG_ALS_LUX) {
		...
		switch (option) {
			...
			case AMS_CONFIG_THRESHOLD: /* set threshold */
				...
				ALS_info("%s - data %d\n", __func__, data);
				_2585_alsSetThreshold(ctx, data);
				break;
			default:
				break;
		}
		...
		switch (option) {
			case AMS_CONFIG_ENABLE: /* power on */
				...
				if (data == 0) {
					...
					ctx->mode |= MODE_FLICKER;
					ams_getByte(ctx->portHndl, DEVREG_UV_CALIB, &ctx->shadowUV_CALIB);
...
static int tsl2585_print_reg_status(void)
{
	int reg, err;
	u8 recvData;

	for (reg = 0; reg < DEVREG_REG_MAX; reg++) {
		err = tsl2585_read_reg(tsl2585_data, deviceRegisterDefinition[reg].address, &recvData, 1);
		if (err != 0) {
...
static int tsl2585_set_sampling_rate(u32 sampling_period_ns)
{
...
static int tsl2585_set_nr_sample(struct tsl2585_device_data *data , u16 fifo_thr)
{
...
static int tsl2585_set_sampling_time(struct tsl2585_device_data *data , u16 time)
{
	ams_deviceCtx_t *ctx = data->deviceCtx;
	int ret = 0;
...
static int tsl2585_hamming_status(struct tsl2585_device_data *data , bool on_off)
{
	ams_deviceCtx_t *ctx = data->deviceCtx;
	int ret = 0;
...
static int tsl2585_polling_enable(struct tsl2585_device_data *data , bool on_off)
{
	ams_deviceCtx_t *ctx = data->deviceCtx;
	int ret = 0;
...
static int tsl2585_pon_reenable(struct tsl2585_device_data *data , bool on_off)
{
	ams_deviceCtx_t *ctx = data->deviceCtx;
	int ret = 0;
...
static int tsl2585_set_fifo_thr(struct tsl2585_device_data *data , u16 fifo_thr)
{
	ams_deviceCtx_t *ctx = data->deviceCtx;
	int ret = 0;
...
static void tsl2585_irq_set_state(struct tsl2585_device_data *data, int irq_enable)
{
...
int tsl2585_power_ctrl(struct tsl2585_device_data *data, int onoff)
{
	...
	if (onoff == PWR_ON) {
		if (data->regulator_state != 0) {
			...
			ALS_dbg("%s - duplicate regulator (increase state : %d)\n", __func__, data->regulator_state);
			return 0;
		}
		...
		if (data->regulator_vdd_1p8) {
			if (!data->vdd_1p8_enable) {
				...
				if (rc) {
					...
					ALS_dbg("%s - enable vdd_1p8 done (state : %d), rc=%d\n", __func__, (data->regulator_state + 1), rc);
		...
		if (data->regulator_state == 0) {
			...
			data->regulator_state--;
			ALS_dbg("%s - duplicate regulator (decrease state : %d)\n", __func__, data->regulator_state);
			return 0;
		}
		data->regulator_state--;
		...
		if (data->regulator_vdd_1p8) {
			if (data->vdd_1p8_enable) {
				...
				if (rc) {
					...
					ALS_dbg("%s - disable vdd_1p8 done (state : %d), rc=%d\n", __func__, data->regulator_state, rc);
				}
			} else {
...
static void report_als(struct tsl2585_device_data *chip)
{
	...
	if (chip->als_input_dev) {
		...
		input_report_rel(chip->als_input_dev, REL_X, outData.wideband + 1);
		input_report_rel(chip->als_input_dev, REL_RY, outData.clear + 1);
		input_report_rel(chip->als_input_dev, REL_RX, outData.rawUvCorrected + 1);
		...
		input_report_abs(chip->als_input_dev, ABS_RX, outData.UVGain + 1);
		input_sync(chip->als_input_dev);

		if (als_cnt++ > 10) {
			ALS_dbg("%s - Photopic:%d, IR:%d, UV:%d(%d), P GAIN:%d, IR GAIN:%d, UVGAIN:%d\n", __func__,
					outData.clear, outData.wideband, outData.rawUv, outData.rawUvCorrected,
					outData.ClearGain, outData.WBGain, outData.UVGain);
			als_cnt = 0;
		} else {
			ALS_info("%s - Photopic:%d, IR:%d, UV:%d(%d), P GAIN:%d, IR GAIN:%d, UVGAIN:%d\n", __func__,
					outData.clear, outData.wideband, outData.rawUv, outData.rawUvCorrected,
					outData.ClearGain, outData.WBGain, outData.UVGain);
		}

#if IS_ENABLED(CONFIG_SENSORS_FLICKER_SELF_TEST)
		als_eol_update_als(outData.ir, outData.clear, outData.wideband, outData.wideband);
#endif
	}
	chip->saturation = false;
}

static void report_flicker(struct tsl2585_device_data *chip)
{
...
{
	ams_apiAls_t outData = {0};
	struct tsl2585_device_data *chip = dev_get_drvdata(dev);
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d,%d,%d\n", outData.rawClear, outData.rawWideband, outData.rawUv);
}

static size_t als_enable_set(struct tsl2585_device_data *chip, uint8_t valueToSet)
{
	int rc = 0;
	...
	rc = tsl2585_polling_enable(chip,chip->sensor_mode);
	if (rc < 0) {
		ALS_err("%s - tsl2585_polling_enable  fail, rc=%d\n", __func__, rc);
		return rc;
	}
...
static ssize_t tsl2585_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
int tsl2585_stop(struct tsl2585_device_data *data);
int tsl2585_start(struct tsl2585_device_data *data);

#if defined(CONFIG_AMS_ALS_COMPENSATION_FOR_AUTO_BRIGHTNESS)
void tsl2585_als_init(struct tsl2585_device_data *data)
{
	ams_deviceCtx_t *ctx = data->deviceCtx;
	int err = 0;
...
int tsl2585_als_start(struct tsl2585_device_data *data)
{
	int err = 0;
	...
	err = tsl2585_power_ctrl(data, PWR_ON);
	if (err < 0) {
...
int tsl2585_als_stop(struct tsl2585_device_data *data)
{
	int err = 0;
	ams_deviceCtx_t *ctx = data->deviceCtx;
	...
	err = tsl2585_power_ctrl(data, PWR_OFF);
	if (err < 0)
...
static ssize_t tsl2585_als_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (value)
		tsl2585_als_start(data);
	else
		tsl2585_als_stop(data);

	return count;
}

static ssize_t tsl2585_als_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_als_data_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	ams_deviceCtx_t *ctx = data->deviceCtx;
...
static ssize_t tsl2585_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	bool value;
...
int tsl2585_start(struct tsl2585_device_data *data)
{
	...
	if (data->regulator_state == 1) {
		...
		if(data->sensor_mode == 0) { /*Camera(flicker) + als sensor*/
			tsl2585_irq_set_state(data, PWR_ON);
		}
...
#endif
		if(data->sensor_mode == 0) { /*Camera(flicker) + als sensor*/
			tsl2585_irq_set_state(data, PWR_OFF);
			usleep_range(1000, 1100);
		}
...
static ssize_t tsl2585_poll_delay_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_poll_delay_store(struct device *dev,
		...
		const char *buf, size_t size)
{
	...
	err = tsl2585_set_sampling_rate(sampling_period_ns);

	if (err > 0)
		data->sampling_period_ns = sampling_period_ns;

	ALS_dbg("%s - tsl2585_poll_delay_store  as %d\n", __func__, sampling_period_ns);
...
static ssize_t tsl2585_fifo_thr_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_fifo_thr_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	...
	err = tsl2585_set_fifo_thr(data,fifo_thr);


	ALS_dbg("%s - tsl2585_fifo_thr_store  as %d\n", __func__, fifo_thr);
...
static ssize_t tsl2585_fd_nr_sample_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_fd_nr_sample_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	...
	err = tsl2585_set_nr_sample(data,fifo_thr);


	ALS_dbg("%s - tsl2585_fd_nr_sample_store  as %d\n", __func__, fifo_thr);
...
static ssize_t tsl2585_sampling_time_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_sampling_time_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	...
	err = tsl2585_set_sampling_time(data,fifo_thr);


	ALS_dbg("%s - tsl2585_set_sampling_time as %d\n", __func__, fifo_thr);
...
static ssize_t tsl2585_hamming_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_hamming_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	...
	err = tsl2585_hamming_status(data,hamming_on_ff);


	ALS_dbg("%s - tsl2585_hamming_store %d \n", __func__, hamming_on_ff);
...
static ssize_t tsl2585_polling_enable_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	...
	err = tsl2585_polling_enable(data,hamming_on_ff);

	ALS_dbg("%s - tsl2585_polling_enable_store %d \n", __func__, hamming_on_ff);
...
static ssize_t tsl2585_regs_write_store (struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t size)
{
	int num = 0;
	...
	ret = tsl2585_read_reg(data, (u8)reg, &r_value, 1);
	if (ret != 0) {
	...
	ret = tsl2585_pon_reenable(data,true);//pon
	...
	ret = tsl2585_write_reg(data, (u8)reg, (u8)w_value);
	if (ret < 0) {
	...
	ret = tsl2585_pon_reenable(data,false);//reenable
...
static ssize_t tsl2585_sensor_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_sensor_mode_store (struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	...
	ALS_dbg("%s - tsl2585_sensor_mode %d \n", __func__, sensor_mode);
...
static DEVICE_ATTR(enable, S_IRUGO | S_IWUSR | S_IWGRP,
		tsl2585_enable_show, tsl2585_enable_store);
static DEVICE_ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
		tsl2585_poll_delay_show, tsl2585_poll_delay_store);
static DEVICE_ATTR(fifo_thr, S_IRUGO | S_IWUSR | S_IWGRP,
		tsl2585_fifo_thr_show, tsl2585_fifo_thr_store);
static DEVICE_ATTR(fd_nr_sample, S_IRUGO | S_IWUSR | S_IWGRP,
		tsl2585_fd_nr_sample_show, tsl2585_fd_nr_sample_store);
static DEVICE_ATTR(sampling_time, S_IRUGO | S_IWUSR | S_IWGRP,
		tsl2585_sampling_time_show, tsl2585_sampling_time_store);
static DEVICE_ATTR(hamming_on_off, S_IRUGO | S_IWUSR | S_IWGRP,
		tsl2585_hamming_show, tsl2585_hamming_store);
static DEVICE_ATTR(poll_enable, S_IRUGO | S_IWUSR | S_IWGRP,
		NULL, tsl2585_polling_enable_store);
static DEVICE_ATTR(2585_regs, S_IRUGO | S_IWUSR | S_IWGRP,
		NULL, tsl2585_regs_write_store);
static DEVICE_ATTR(sensor_mode, S_IRUGO | S_IWUSR | S_IWGRP,
		tsl2585_sensor_mode_show, tsl2585_sensor_mode_store);
...
/* als_sensor sysfs */
static ssize_t tsl2585_name_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	ams_deviceCtx_t *ctx = data->deviceCtx;
	...
	switch (ctx->deviceId) {
		case AMS_TSL2585:
		case AMS_TSL2585_UNTRIM:
			strlcpy(chip_name, TSL2585_CHIP_NAME, sizeof(chip_name));
			break;
		default:
			strlcpy(chip_name, TSL2585_CHIP_NAME, sizeof(chip_name));
			break;
...
static ssize_t tsl2585_vendor_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", VENDOR);
}

static ssize_t tsl2585_flush_store(struct device *dev,
...
static ssize_t tsl2585_int_pin_check_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t tsl2585_read_reg_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_read_reg_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	...
	err = tsl2585_read_reg(data, (u8)cmd, &val, 1);
	if (err != 0) {
		ALS_err("%s - err=%d, val=0x%06x\n",
				__func__, err, val);
		mutex_unlock(&data->i2clock);
...
static ssize_t tsl2585_write_reg_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	...
	err = tsl2585_write_reg(data, (u8)cmd, (u8)val);
	if (err < 0) {
		ALS_err("%s - fail err = %d\n", __func__, err);
		mutex_unlock(&data->i2clock);
		return err;
...
static ssize_t tsl2585_debug_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_debug_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	int err;
	...
	switch (data->debug_mode) {
		case DEBUG_REG_STATUS:
			tsl2585_print_reg_status();
			break;
		case DEBUG_VAR:
			tsl2585_debug_var(data);
			break;
		default:
			break;
	}
...
static ssize_t tsl2585_device_id_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t tsl2585_part_type_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	ams_deviceCtx_t *ctx = data->deviceCtx;
...
static ssize_t tsl2585_i2c_err_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	u32 err_cnt = 0;
...
static ssize_t tsl2585_i2c_err_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_curr_adc_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_curr_adc_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_mode_cnt_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_mode_cnt_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_factory_cmd_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_version_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t tsl2585_sensor_info_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
#if IS_ENABLED(CONFIG_FLICKER_PWM_CALIBRATION)
static ssize_t tsl2585_osc_cal_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	int cal_value = tsl2585_get_pwm_calibration();
	if (cal_value == 0xffff) {
		snprintf(buf, MAX_TEST_RESULT, "FAIL,NO_OSC_CAL");
	} else {
		snprintf(buf, MAX_TEST_RESULT, "%s,%d\n", OSC_CAL_SPEC_IN(cal_value), cal_value);
	}
...
static ssize_t tsl2585_eol_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
...
static ssize_t tsl2585_eol_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	ams_deviceCtx_t *ctx = data->deviceCtx;
	int err = 0;
	static int prev_sensor_mode = 0;
	ssize_t ret = 0;
...
static DEVICE_ATTR(name, S_IRUGO, tsl2585_name_show, NULL);
static DEVICE_ATTR(vendor, S_IRUGO, tsl2585_vendor_show, NULL);
static DEVICE_ATTR(als_flush, S_IWUSR | S_IWGRP, NULL, tsl2585_flush_store);
static DEVICE_ATTR(int_pin_check, S_IRUGO, tsl2585_int_pin_check_show, NULL);
static DEVICE_ATTR(read_reg, S_IRUGO | S_IWUSR | S_IWGRP,
		tsl2585_read_reg_show, tsl2585_read_reg_store);
static DEVICE_ATTR(write_reg, S_IWUSR | S_IWGRP, NULL, tsl2585_write_reg_store);
static DEVICE_ATTR(als_debug, S_IRUGO | S_IWUSR | S_IWGRP,
		tsl2585_debug_show, tsl2585_debug_store);
static DEVICE_ATTR(device_id, S_IRUGO, tsl2585_device_id_show, NULL);
static DEVICE_ATTR(part_type, S_IRUGO, tsl2585_part_type_show, NULL);
static DEVICE_ATTR(i2c_err_cnt, S_IRUGO | S_IWUSR | S_IWGRP, tsl2585_i2c_err_show, tsl2585_i2c_err_store);
static DEVICE_ATTR(curr_adc, S_IRUGO | S_IWUSR | S_IWGRP, tsl2585_curr_adc_show, tsl2585_curr_adc_store);
static DEVICE_ATTR(mode_cnt, S_IRUGO | S_IWUSR | S_IWGRP, tsl2585_mode_cnt_show, tsl2585_mode_cnt_store);
static DEVICE_ATTR(als_factory_cmd, S_IRUGO, tsl2585_factory_cmd_show, NULL);
static DEVICE_ATTR(als_version, S_IRUGO, tsl2585_version_show, NULL);
static DEVICE_ATTR(sensor_info, S_IRUGO, tsl2585_sensor_info_show, NULL);
...
#if IS_ENABLED(CONFIG_FLICKER_PWM_CALIBRATION)
static DEVICE_ATTR(osc_cal, S_IRUGO | S_IWUSR | S_IWGRP, tsl2585_osc_cal_show, tsl2585_osc_cal_store);
#endif
#if defined(CONFIG_AMS_OPTICAL_SENSOR_FLICKER)
static DEVICE_ATTR(flicker_data, S_IRUGO, flicker_data_show, NULL);
#endif
#if IS_ENABLED(CONFIG_SENSORS_FLICKER_SELF_TEST)
static DEVICE_ATTR(eol_mode, S_IRUGO | S_IWUSR | S_IWGRP, tsl2585_eol_mode_show, tsl2585_eol_mode_store);
#endif
#if defined(CONFIG_AMS_ALS_COMPENSATION_FOR_AUTO_BRIGHTNESS)
static DEVICE_ATTR(als_enable, S_IRUGO | S_IWUSR | S_IWGRP,	tsl2585_als_enable_show, tsl2585_als_enable_store);
static DEVICE_ATTR(als_data, S_IRUGO, tsl2585_als_data_show, NULL);
#endif

static struct device_attribute *tsl2585_sensor_attrs[] = {
	&dev_attr_name,
	&dev_attr_vendor,
	&dev_attr_als_flush,
...
static int _2585_handleAlsEvent(ams_deviceCtx_t *ctx);
static void _2585_handleFIFOEvent(ams_deviceCtx_t *ctx);
...
static void _2585_handleFIFOEvent(ams_deviceCtx_t *ctx)
{
	...
	if(ctx->flickerCtx.data_ready == 1) {
		...
		fifo_mod0_gain = ctx->fifo_mod0_gain & 0x0F;//clear
		fifo_mod1_gain = (ctx->fifo_mod0_gain >> 4) & 0x0F;//wide
		fifo_mod2_gain = ctx->fifo_mod2_gain & 0x0F;//wide

		clear_gain = tsl2585_gain_conversion[fifo_mod0_gain] * 500;
		wideband_gain = tsl2585_gain_conversion[fifo_mod1_gain] * 500;
		uv_gain = tsl2585_gain_conversion[fifo_mod2_gain] * 500;

		ctx->ccbAlsCtx.ctxAlgAls.ClearGain = clear_gain;
		ctx->ccbAlsCtx.ctxAlgAls.WBGain = wideband_gain;
		ctx->ccbAlsCtx.ctxAlgAls.UVGain = uv_gain;
		ctx->ccbAlsCtx.ctxAlgAls.UV_CALIB = ctx->shadowUV_CALIB;

		amsAlg_als_processData(&ctx->ccbAlsCtx.ctxAlgAls, &inputData);
		//ctx->flickerCtx.als_data_ready = 1;
#endif // USE_GET_FIFO_ALS
...
static int ams_devicePollingHandler(ams_deviceCtx_t *ctx)
{
	...
	if ((ctx->shadowStatus2Reg & ALS_DATA_VALID) /*|| ctx->alwaysReadAls*/) {
		...
		if (ctx->mode & MODE_ALS_ALL) {
			ALS_info("%s - _2585_handleAlsEvent :%d alwaysReadAls = %d\n", __func__, (ctx->shadowStatus1Reg & AINT), ctx->alwaysReadAls);
			ret = _2585_handleAlsEvent(ctx);
			if (ret < 0) {
				ALS_err("%s - failed to _2585_handleAlsEvent\n", __func__);
				//return ret;
			}
		}
	}
...
static int ams_deviceEventHandler(ams_deviceCtx_t *ctx)
{
	...
	//clear_gain = tsl2585_gain_conversion[ctx->fifo_mod0_gain];
	//wideband_gain = tsl2585_gain_conversion[ctx->fifo_mod1_gain];
...
loop:
	if (loop_cnt) {
		ALS_dbg("%s - loop: DCB 0x%02x, STATUS 0x%02x, ALS_STATUS 0x%02x, ALS_STATUS2 0x%02x, STATUS5 0x%02x\n", __func__, ctx->mode, ctx->shadowStatus1Reg, ctx->shadowAlsStatusReg, ctx->shadowAlsStatus2Reg, status5);
	} else {
		ALS_info("%s - loop: DCB 0x%02x, STATUS 0x%02x, ALS_STATUS 0x%02x, ALS_STATUS2 0x%02x, STATUS5 0x%02x\n", __func__, ctx->mode, ctx->shadowStatus1Reg, ctx->shadowAlsStatusReg, ctx->shadowAlsStatus2Reg, status5);
	}
	...
	if (ctx->shadowStatus1Reg & FINT) {
		...
		if (ctx->mode & MODE_FLICKER) {
			ALS_info("%s - _2585_handleFIFOEvent \n", __func__);
			_2585_handleFIFOEvent(ctx);
		}
	}
#endif
	...
	if ((ctx->shadowStatus1Reg & (MINT|AINT|FINT)) != 0) {
		loop_cnt++;
		ALS_dbg("%s - goto loop (cnt = %d)", __func__, loop_cnt);
		goto loop;
	} else if ((ctx->shadowStatus1Reg & SINT) != 0) {
		ALS_info("%s - clear DEVREG_STATUS = 0x%x, DEVREG_STATUS5 = 0x%x", __func__, ctx->shadowStatus1Reg, status5);
		ret = ams_setByte(ctx->portHndl, DEVREG_STATUS, ctx->shadowStatus1Reg);
		if (ret < 0) {
			ALS_err("%s - failed to clear DEVREG_STATUS\n", __func__);
		}
		...
		if (ret < 0) {
			...
			ret = ams_setByte(ctx->portHndl, DEVREG_STATUS5, status5);
			if (ret < 0) {
				ALS_err("%s - failed clear set DEVREG_STATUS5\n", __func__);
			}
		}
	}
...
irqreturn_t tsl2585_irq_handler(int dev_irq, void *device)
{
	int err;
	struct tsl2585_device_data *data = device;
...
#if defined(CONFIG_AMS_OPTICAL_SENSOR_POLLING)
static void tsl2585_work_func_light(struct work_struct *work)
{
	int err;
	//    int lux;
	int pollingHandled = 0;

	struct tsl2585_device_data *data
		= container_of(work, struct tsl2585_device_data, work_light);
	ALS_info("%s -msec \n", __func__);
...
static enum hrtimer_restart tsl2585_timer_func(struct hrtimer *timer)
{
	struct tsl2585_device_data *data = container_of(timer, struct tsl2585_device_data, timer);
...
static int tsl2585_setup_irq(struct tsl2585_device_data *data)
{
	int errorno = -EIO;
	errorno = request_threaded_irq(data->dev_irq, NULL,
			tsl2585_irq_handler, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
...
static void tsl2585_init_var(struct tsl2585_device_data *data)
{
	data->client = NULL;
	data->dev = NULL;
...
#endif
	data->osc_cal = 0;
	data->pwm_period = OSC_CAL_DEFAULT_PWM_PERIOD;
	data->pin_als_int = -1;
	data->pin_als_pwm = -1;
}

static int tsl2585_parse_dt(struct tsl2585_device_data *data)
{
	...
	u32 osc_cal = 0;
	u32 pwm_period = OSC_CAL_DEFAULT_PWM_PERIOD;

	if (dNode == NULL)
		return -ENODEV;

	if (of_property_read_u32(dNode, "als_rear,osc-cal", &osc_cal) == 0) {
		data->osc_cal = osc_cal;
		ALS_dbg("%s - get osc-cal flag : %d", __func__, data->osc_cal);
	} else {
		ALS_err("%s - get osc-cal flag error", __func__);
	}
	...
	if (data->pin_als_int < 0) {
		...
		ALS_dbg("%s - get als_rear_int : gpio_%d\n", __func__, data->pin_als_int);
	}

	if (data->osc_cal) {
		data->pin_als_pwm = of_get_named_gpio_flags(dNode, "als_rear,pwm-gpio", 0, &flags);
		if (data->pin_als_pwm < 0) {
			ALS_err("%s - get pin_als_pwm  error\n", __func__);
			//return -ENODEV;
		} else {
			ALS_dbg("%s - get pin_als_pwm : gpio_%d\n", __func__, data->pin_als_pwm);
		}
	}

	if (of_property_read_u32(dNode, "als_rear,pwm-period", &pwm_period) == 0) {
		data->pwm_period = pwm_period;
		ALS_dbg("%s - get pwm-period property : %d\n", __func__, data->pwm_period);
	} else {
		ALS_err("%s - can't find pwm-period property. use default period %d", __func__, data->pwm_period);
	}
...
static int tsl2585_setup_gpio(struct tsl2585_device_data *data)
{
	int errorno = -EIO;
...
static int _2585_alsInit(ams_deviceCtx_t *ctx, ams_calibrationData_t *calibrationData)
{
	int ret = 0;
...
static bool ams_deviceGetAls(ams_deviceCtx_t *ctx, ams_apiAls_t *exportData)
{
	...
	exportData->rawUvCorrected = result.rawUvCorrected;
...
#if IS_ENABLED(CONFIG_FLICKER_PWM_CALIBRATION)
	cal_value = tsl2585_get_pwm_calibration();
#endif
	if (cal_value != 0xffff) {
		u16 sample_time_reg;
		u16 osc_mod_clock = DIV_ROUND_CLOSEST(tsl2585_data->pwm_period, cal_value);
		sample_time_reg = DIV_ROUND_CLOSEST(AMS_SAMPLING_TIME, osc_mod_clock) - 1;

		ALS_dbg("%s - Vsync_period25ms: %d, OSC_Clock: %d, Sample_time_reg: %d\n",
				__func__, cal_value, osc_mod_clock, sample_time_reg);

		ret = ams_setByte(ctx->portHndl, DEVREG_SAMPLE_TIME0,   (0xFF) & (sample_time_reg<<0));
		if (ret < 0) {
			ALS_err("%s - failed to Sample time \n", __func__);
		}
		ret = ams_setByte(ctx->portHndl, DEVREG_SAMPLE_TIME1,   (0x07) & (sample_time_reg>>8));
		if (ret < 0) {
			ALS_err("%s - failed to Sample time \n", __func__);
		}
	} else {
		AMS_SET_SAMPLE_TIME(AMS_SAMPLING_TIME/1000, ret);
		if (ret < 0) {
			ALS_err("%s - failed to Sample time \n", __func__);
			return ret;
		}
...
#if defined(CONFIG_AMS_OPTICAL_SENSOR_ALS_CCB)
	ret = _2585_alsInit(ctx, calibrationData);
	if (ret < 0) {
		ALS_err("%s - failed to _2585_alsInit\n", __func__);
		return ret;
	}
#endif
...
static int tsl2585_init_fifo(struct tsl2585_device_data *data)
{
...
{
	u8 status = 0, value = 0, prev_en = 0, prev_int = 0;
	u16 vsync_period = 0, sample_time_reg, osc_mod_clock;
	u16 target_period;

	/* Disable PON/AEN/FDEN */
	ams_getByte(data->client, DEVREG_ENABLE, &prev_en);
	ams_setByte(data->client, DEVREG_ENABLE, 0x00);
	/* Target period : (period/1.38888889us) / 2 */
	/* ex) 40Hz  25ms: (25ms/1.38888889us) / 2 = 9000 = 0x2328 */
	target_period = DIV_ROUND_CLOSEST(data->pwm_period, AMS_USEC_PER_TICK);
	target_period = DIV_ROUND_CLOSEST(target_period, 2);
	ALS_dbg("%s - target period 0x%x", __func__, target_period);
	ams_setByte(data->client, DEVREG_VSYNC_PERIOD_TARGET_L, target_period & 0xff);
	ams_setByte(data->client, DEVREG_VSYNC_PERIOD_TARGET_H, (target_period >> 8) & 0xff);
	/* TRIGGER_MODE: Setting 101b for timer usage only (no oscillator) */
	ams_setByte(data->client, DEVREG_TRIGGER_MODE, 0x05);

	/*INTENAB: Switch off all Interrupts routed to INT pin to make it an input for SYNC */
	ams_getByte(data->client, DEVREG_INTENAB, &prev_int);
	ams_setByte(data->client, DEVREG_INTENAB, 0x00);

	/*VSYNC_GPIO_IN : vsync_gpio_in_en */
	ams_setByte(data->client, DEVREG_VSYNC_GPIO_INT, 0x06);

	/* PON enable for calibration */
	ams_setByte(data->client, DEVREG_ENABLE, 0x01);

	msleep_interruptible(100);	// need delay time

	/* VSYNC_PERIOD: measured period = VSYNC_PERIOD * 1.3888889us */
	ams_getBuf(data->client, DEVREG_VSYNC_PERIOD_L, (uint8_t *)&vsync_period, 2);

	/* TRIGGER_MODE: Setting 101b for timer usage only (no oscillator) */
	ams_setByte(data->client, DEVREG_TRIGGER_MODE, 0x00);
	ams_setByte(data->client, DEVREG_VSYNC_GPIO_INT, 0x02);

	if (vsync_period != 0) {
		osc_mod_clock = DIV_ROUND_CLOSEST(data->pwm_period, vsync_period);
		sample_time_reg = DIV_ROUND_CLOSEST(AMS_SAMPLING_TIME, osc_mod_clock) - 1;

		ALS_dbg("%s - Vsync_period25ms: %d, OSC_Clock: %d, Sample_time_reg: %d\n",
				__func__, vsync_period, osc_mod_clock, sample_time_reg);

		ams_setByte(data->client, DEVREG_SAMPLE_TIME0,   (0xFF) & (sample_time_reg<<0));
		ams_setByte(data->client, DEVREG_SAMPLE_TIME1,   (0x07) & (sample_time_reg>>8));
	}

	ams_setByte(data->client, DEVREG_INTENAB, prev_int);
	ams_setByte(data->client, DEVREG_ENABLE, prev_en);

	return vsync_period;
}

int tsl2585_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int err = -ENODEV;
	struct device *dev = &client->dev;
	static struct tsl2585_device_data *data;
	struct amsdriver_i2c_platform_data *pdata = dev->platform_data;
	ams_deviceInfo_t amsDeviceInfo;
	ams_deviceIdentifier_e deviceId;
	static int g_defered_cnt = 0;

	ALS_dbg("%s - start\n", __func__);
	printk(KERN_ERR "\nams_tsl2585: tsl2585_probe() client->irq= %d\n", client->irq);
	...
	data = devm_kzalloc(dev, sizeof(struct tsl2585_device_data), GFP_KERNEL);
	if (data == NULL) {
	...
	if (!pdata) {
		...
		if (of_match_device(tsl2585_match_table, &client->dev)) {
	...
	data->miscdev.fops = &tsl2585_fops;
	...
	if (data->client->addr == TSL2585_SLAVE_I2C_ADDR_REVID_V0) {
...
#if defined (TSL2585_PROBE_DEFER_ENABLE)
		if (g_defered_cnt >= 3) {
			err = -EIO;
		} else {
			err = -EPROBE_DEFER;
	...
	input_set_capability(data->als_input_dev, EV_REL, REL_X);	/* ir */
	// temporal disable
	//input_set_capability(data->als_input_dev, EV_REL, REL_Y);	/* red */
	//input_set_capability(data->als_input_dev, EV_REL, REL_Z);	/* green */
	input_set_capability(data->als_input_dev, EV_REL, REL_RX);	/* blue => UV */
	input_set_capability(data->als_input_dev, EV_REL, REL_RY);	/* clear */
	input_set_capability(data->als_input_dev, EV_REL, REL_RZ);	/* flicker */
	input_set_capability(data->als_input_dev, EV_REL, REL_MISC);	/* flush */
	input_set_capability(data->als_input_dev, EV_ABS, ABS_X);	/* time_us */
	input_set_capability(data->als_input_dev, EV_ABS, ABS_Y);	/* gain_clear */
	input_set_capability(data->als_input_dev, EV_ABS, ABS_Z);	/* gain_wb */
	input_set_capability(data->als_input_dev, EV_ABS, ABS_RX);	/* gain_uv */
...
#if defined(CONFIG_AMS_OPTICAL_SENSOR_POLLING)
	data->wq = create_singlethread_workqueue("tsl2585_wq");
	if (data->wq) {
		INIT_WORK(&data->work_light, tsl2585_work_func_light);
		...
		data->timer.function = tsl2585_timer_func;
	...
	err = tsl2585_init_fifo(data);
	if (err) {
		ALS_err("%s - error tsl2585_init_fifo\n", __func__);
	}

	tsl2585_irq_set_state(data, PWR_ON); /* For flushing interrupt request */
	err = tsl2585_power_ctrl(data, PWR_OFF);
	if (err < 0) {
	...
	else { // not use gpio based pwm
		tsl2585_osc_cal(data);
	}
#endif

	ALS_dbg("%s - success (%d)", __func__, g_defered_cnt);
	goto done;
...
err_misc_register:
	//	devm_kfree(data);
	ALS_err("%s failed (%d)", __func__, g_defered_cnt);
done:
	return err;
}

int tsl2585_remove(struct i2c_client *client)
{
	struct tsl2585_device_data *data = i2c_get_clientdata(client);
...
static void tsl2585_shutdown(struct i2c_client *client)
{
	ALS_dbg("%s - start\n", __func__);
}
...
static int tsl2585_suspend(struct device *dev)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	int err = 0, cnt = 0;
	...
	tsl2585_pin_control(data, false);
...
static int tsl2585_resume(struct device *dev)
{
	struct tsl2585_device_data *data = dev_get_drvdata(dev);
	int err = 0;
...
static const struct dev_pm_ops tsl2585_pm_ops = {
	.suspend = tsl2585_suspend,
	.resume = tsl2585_resume
};
#endif

static const struct i2c_device_id tsl2585_idtable[] = {
	{ "tsl2585", 0 },
	{ }
};
/* descriptor of the tsl2585 I2C driver */
static struct i2c_driver tsl2585_driver = {
	.driver = {
		.name = "tsl2585",
		.owner = THIS_MODULE,
#if IS_ENABLED(CONFIG_PM)
		.pm = &tsl2585_pm_ops,
#endif
		.of_match_table = tsl2585_match_table,
	},
	.probe = tsl2585_probe,
	.remove = tsl2585_remove,
	.shutdown = tsl2585_shutdown,
	.id_table = tsl2585_idtable,
};

/* initialization and exit functions */
static int __init tsl2585_init(void)
{
	int rc = i2c_add_driver(&tsl2585_driver);
	return rc;
}

static void __exit tsl2585_exit(void)
{
	i2c_del_driver(&tsl2585_driver);
}

module_init(tsl2585_init);
module_exit(tsl2585_exit);

MODULE_SOFTDEP("pre: sensors_core");
MODULE_AUTHOR("Samsung Electronics");
MODULE_DESCRIPTION("TSL2585 ALS Driver");
MODULE_LICENSE("GPL");
