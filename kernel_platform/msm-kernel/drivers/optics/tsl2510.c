...
bool  ccb_sw_bin4096_flicker_GetResult(void * dcbCtx)
{
	...
	if(get_fft(ctx,buf)){
		...
		if ((max == 7) && (buf[6] > buf[5] && buf[6] > buf[7])) {
			if (max2 == 12 || max2 == 18 || max2 == 24 || max2 == 36) {
				if (buf[12] > thd) {
					if (buf[10] > thd) {
						mHz = 100;
					} else {
				...
				mHz = ((max2 * sampling_freq) / AMS_FFT_SIZE);
				ALS_info( "flicker_freq %d\n", mHz);
			}
		} else if (buf[max] > thd) {
...
#if IS_ENABLED(CONFIG_SENSORS_FLICKER_SELF_TEST)
		als_eol_update_als(outData.ir, outData.clear, outData.wideband);
#endif
	}
...
#if IS_ENABLED(CONFIG_ARCH_EXYNOS)
	err = sensors_register(data->dev, data, tsl2510_sensor_attrs, MODULE_NAME_ALS);
#else
	err = sensors_register(&data->dev, data, tsl2510_sensor_attrs, MODULE_NAME_ALS);
#endif
	if (err) {
