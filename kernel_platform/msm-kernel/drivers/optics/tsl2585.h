...
#ifndef __AMS_TSL2585_H
#define __AMS_TSL2585_H
...
#define CONFIG_AMS_OPTICAL_SENSOR_2585
...
#define AMS_SAMPLING_TIME                   (976563)  /* nsec (1024Hz) */

// FLICKER SETTING VALUES
#define AMS_FLICKER_NUM_SAMPLES             (AMS_CLR_WIDE_FLICKER_NUM_SAMPLES * 2 * 3) // Photopic, IR, UV = 128 samples ,  128 * 2byte * 3ch = 768byte
...
typedef enum _deviceRegisters {
	DEVREG_UV_CALIB, //0x08 it should be only OTP read
...
typedef enum _2585_regMasks {
	...
	MASK_AGAIN2 = 0x0F, /* register 0xD5[3:0] */
...
typedef enum _2585_configureFeature {
	AMS_CONFIG_PROX,
	AMS_CONFIG_ALS_LUX,
	AMS_CONFIG_ALS_RGB,
	AMS_CONFIG_ALS_CT,
...
typedef struct _2585Context {
	...
	uint8_t shadowUV_CALIB;  // UV Calibration value
...
struct amsdriver_i2c_platform_data {
	...
	char const *als_name;
	struct tsl2585_parameters parameters;
	struct device_node  *of_node;
};

struct tsl2585_device_data {
