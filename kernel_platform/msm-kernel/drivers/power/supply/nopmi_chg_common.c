...
int nopmi_chg_is_usb_present(struct power_supply *bbc_psy)
{
	...
	ret = power_supply_get_property(bbc_psy,
...
int nopmi_get_quick_charge_type(struct power_supply *usb_psy,uint8_t *get_chgtype)
{
	union power_supply_propval prop = {0, };
	int i = 0;
	int ret = 0;
	...
	if (g_nopmi_chg->pd_active || g_nopmi_chg->onlypd_active) {
		...
		if (cp) {
			...
			pr_err("charger_standalone supply not found\n");
			chg_type = g_nopmi_chg->real_type;
		}
	}
	...
	pr_info("real_type=%d\n",chg_type);
	if(get_chgtype != NULL) {
		pr_err("only get_chgtype so return 0");
		*get_chgtype = chg_type;
		return 0;
	}
	...
	if (prop.intval < 50 || prop.intval >= 550) {
		if (usb_psy && !is_single_flash){
			pr_info("battery temp is under 5 or above 55");
