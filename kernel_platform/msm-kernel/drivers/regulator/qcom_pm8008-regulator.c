...
//M55 code for SR-QN6887A-01-90 by chengzhi at 20231211 begin
    int retryPm8010readNum = 0;
	#if 1
	while (retryPm8010readNum < 5) {
		rc = regmap_bulk_read(regmap, reg, val, count);
		if (rc < 0) {
			pr_err("debugpm8010i retryPm8010readNum=%d failed to read 0x%04x\n",retryPm8010readNum, reg);
		} else {
			pr_debug("debugpm8010i pm8008_read successful\n");
			break;
		}
		msleep(500);
		retryPm8010readNum++;
	...
	return rc;
	#endif
//M55 code for SR-QN6887A-01-90 by chengzhi at 20231211 end
}
...
//M55 code for SR-QN6887A-01-90 by chengzhi at 20231211 begin
    int retryPm8010writeNum = 0;
	#if 1
	pr_debug("Writing 0x%02x to 0x%04x\n", val, reg);
	while (retryPm8010writeNum < 5) {
		rc = regmap_bulk_write(regmap, reg, val, count);
		if (rc < 0) {
			pr_err("debugpm8010i retryPm8010writeNum=%d failed to write 0x%04x\n",retryPm8010writeNum, reg);
		} else {
			pr_debug("debugpm8010i pm8008_write successful\n");
			break;
		}
		msleep(500);
		retryPm8010writeNum++;
				...
				u8 val)
{
	...
	//M55 code for SR-QN6887A-01-90 by chengzhi at 20231211 begin
    int retryPm8010maskwriteNum = 0;
	#if 1
	pr_debug("Writing 0x%02x to 0x%04x with mask 0x%02x\n", val, reg, mask);
	while (retryPm8010maskwriteNum < 5) {
		rc = regmap_update_bits(regmap, reg, mask, val);
		if (rc < 0) {
			pr_err("debugpm8010i retryPm8010maskwriteNum=%d,failed to write 0x%02x to 0x%04x with mask 0x%02x\n",
				retryPm8010maskwriteNum,val, reg, mask);
		} else {
			pr_debug("debugpm8010i pm8008_masked_write successful\n");
			break;
		}
		msleep(500);
		retryPm8010maskwriteNum++;
	}
	return rc;
	#else
	pr_debug("Writing 0x%02x to 0x%04x with mask 0x%02x\n", val, reg, mask);
...
static int pm8008_chip_probe(struct platform_device *pdev)
{
	...
	u8 subtype, rev1;
	struct pm8008_chip *chip;
	//M55 code for SR-QN6887A-01-90 by chengzhi at 20231208 begin
	int retryPm8010probeNum=0;
	//M55 code for SR-QN6887A-01-90 by chengzhi at 20231208 end
...
//M55 code for SR-QN6887A-01-90 by chengzhi at 20231208 begin
	#if 1
	while (retryPm8010probeNum < 5) {
		/* Register chip enable regulator */
		rc = pm8008_chip_init_regulator(chip);
		if (rc < 0) {
			pr_err("debugpm8010i Failed to register chip enable regulator rc=%d, retryPm8010probeNum=%d\n", rc, retryPm8010probeNum);
		} else {
			pr_info("debugpm8010i pm8008_chip_init_regulator successful\n");
			break;
		}
		msleep(500);
		retryPm8010probeNum++;
	}
	if (rc < 0 && retryPm8010probeNum == 5)
	{
		return -EPROBE_DEFER;
	}
	#else
	...
	rc = pm8008_chip_init_regulator(chip);
	if (rc < 0) {
		pr_err("debugpm8010i Failed to register chip enable regulator rc=%d\n", rc);
		return rc;
	}
	#endif
//M55 code for SR-QN6887A-01-90 by chengzhi at 20231208 end
