...
retry:
	...
	usleep_range(DELAY_FOR_POST_TRANSCATION, DELAY_FOR_POST_TRANSCATION+1);
	kfree(pkt);
	kfree(data);
...
static bool ts_read_coord(struct zt75xx_ts_info *info)
{
	...
	if (info->touch_info[0].eid == COORDINATE_EVENT) {
		if (info->touch_info[0].left_event > (MAX_SUPPORTED_FINGER_NUM - 1)) {
			input_err(true, &client->dev, "invalid left event = %d\n", info->touch_info[0].left_event);
			return false;
		}
...
static bool init_touch(struct zt75xx_ts_info *info)
{
	...
	/* get x,y data */
	if (pdata->x_channel == 0 || pdata->y_channel == 0) {
		...
		info->cap_info.x_node_num = pdata->x_channel;
		info->cap_info.y_node_num = pdata->y_channel;
	}
...
#ifdef TCLM_CONCEPT
	input_info(true, &info->client->dev,
			"iq:%d depth:%d lp:%x // v:%02X%02X%02X cal:%02X,C%02XT%04X.%4s%s // #%d %d\n",
			gpio_get_value(info->pdata->gpio_int),
			desc->depth, info->lowpower_mode,
			info->cap_info.ic_revision, info->cap_info.fw_minor_version, info->cap_info.reg_data_version,
...
#else
	input_info(true, &info->client->dev,
			"iq:%d depth:%d lp:%x // v:%02X%02X%02X // #%d %d\n",
			gpio_get_value(info->pdata->gpio_int),
			desc->depth, info->lowpower_mode,
			info->cap_info.ic_revision, info->cap_info.fw_minor_version, info->cap_info.reg_data_version,
			info->print_info_cnt_open, info->print_info_cnt_release);
...
static irqreturn_t zt75xx_touch_work(int irq, void *data)
{
	...
	if (info->work_state != NOTHING) {
		...
		usleep_range(DELAY_FOR_SIGNAL_DELAY, DELAY_FOR_SIGNAL_DELAY+1);
	...
	for (i = 0; i < info->cap_info.multi_fingers; i++) {
		...
		if (tid >= MAX_SUPPORTED_FINGER_NUM) {
			input_err(true, &client->dev, "%s: invalid tid: %d\n", __func__, tid);
			goto out;			
		}
...
static ssize_t get_lp_dump_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	do {
		...
		if (read_raw_data(info->client, ZT75XX_LP_DUMP_REG, (u8 *)string_data, 1004) < 0) {
			input_err(true, &info->client->dev, "%s: error read zinitix tc raw data\n", __func__);
			info->work_state = NOTHING;
			goto read_ic;
		}
...
#if IS_ENABLED(CONFIG_SEC_DEBUG_TSP_LOG)
		snprintf(buffer, sizeof(buffer), "%03d: ", info->lp_dump[info->lp_dump_index] << 8 | info->lp_dump[info->lp_dump_index + 1]);
		strlcat(log_buffer, buffer, sizeof(log_buffer));
		memset(buffer, 0x00, sizeof(buffer));

		for (j = 0; j < 10; j++) {
			snprintf(buffer, sizeof(buffer), "%02X", info->lp_dump[info->lp_dump_index + 2 + j]);
			strlcat(log_buffer, buffer, sizeof(log_buffer));
			memset(buffer, 0x00, sizeof(buffer));
		}
...
static int zt75xx_ts_stop(struct zt75xx_ts_info *info)
{
	...
	if (info->sec.fac_dev)
		get_lp_dump_show(info->sec.fac_dev, NULL, NULL);
...
static ssize_t input_enabled_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct zt75xx_ts_info *info = dev_get_drvdata(dev);
...
static ssize_t input_enabled_store(struct device *dev, struct device_attribute *attr,
...
static struct device_attribute input_attr[] = {
	__ATTR(enabled, 0664, input_enabled_show, input_enabled_store),
};
...
static int ts_set_self_sat_touchmode(u16 value)
{
	...
	input_info(true, &info->client->dev, "%s: touch_testmode = %d\n",
				__func__, info->touch_mode);

	if (info->touch_mode != TOUCH_POINT_MODE) {
		if (write_reg(info->client, ZT75XX_DELAY_RAW_FOR_HOST, RAWDATA_DELAY_FOR_HOST) != I2C_SUCCESS)
			input_err(true, &info->client->dev, "%s: Fail to set ZT75XX_DELAY_RAW_FOR_HOST\n", __func__);
	}
...
static void get_fw_ver_bin(void *device_data)
{
	...
	version = (((info->fw_ic_revision_bin & 0xFF) << 24) | ((info->fw_minor_ver_bin & 0xFF) << 16)
...
static void get_fw_ver_ic(void *device_data)
{
	...
	version = (u32)(((ic_revision & 0xFF) << 24) | ((fw_minor_version & 0xFF) << 16)
...
static void clear_cover_mode(void *device_data)
{
	...
	if (info->work_state != PROBE)
		...
		input_info(true, &info->client->dev, "%s: probe is not done\n", __func__);
...
static ssize_t ta_mode_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct sec_cmd_data *sec = dev_get_drvdata(dev);
				...
				const char *buf, size_t count)
{
	...
	if (buff[0] == DISPLAY_STATE_ON && buff[1] == DISPLAY_EVENT_LATE) {
		if (info->enabled) {
			info->display_state = DISPLAY_STATE_ON;
			input_info(true, &info->client->dev, "%s: device already enabled\n", __func__);
			goto out;
		}
		input_info(true, &info->client->dev, "%s: open(%d)\n", __func__, ret);
		zt75xx_ts_open(info->input_dev);
		info->enabled = 1;
		info->display_state = DISPLAY_STATE_ON;
		...
		if (info->display_state == DISPLAY_STATE_FORCE_ON) {
			input_err(true, &info->client->dev, "%s: skip display_state_force_on\n", __func__);
			goto out;
		}
		if (!info->enabled) {
			input_info(true, &info->client->dev, "%s: device already disabled\n", __func__);
			goto out;
		}
		input_info(true, &info->client->dev, "%s: close(%d)\n", __func__, ret);
		zt75xx_ts_close(info->input_dev);
		info->enabled = 0;
		...
		zt75xx_ts_open(info->input_dev);
		info->enabled = 1;
		info->display_state = DISPLAY_STATE_FORCE_ON;
		input_info(true, &info->client->dev, "%s: DISPLAY_STATE_FORCE_ON(%d)\n", __func__, ret);
		...
		zt75xx_ts_close(info->input_dev);
		info->enabled = 0;
		info->display_state = DISPLAY_STATE_FORCE_OFF;
		input_info(true, &info->client->dev, "%s: DISPLAY_STATE_FORCE_OFF(%d)\n", __func__, ret);
	}
out:
	return count;
}
...
static int init_sec_factory(struct zt75xx_ts_info *info)
{
	...
	info->sec.sysfs_functions->sec_tsp_support_feature_show = support_feature_show;
	info->sec.sysfs_functions->sec_tsp_prox_power_off_show = NULL;
	info->sec.sysfs_functions->sec_tsp_prox_power_off_store = NULL;
	info->sec.sysfs_functions->dualscreen_policy_store = NULL;
			 ...
			 struct zt75xx_ts_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np, "zinitix,x_channel", &temp);
	if (ret)
		input_info(true, dev, "Unable to read x channel\n");
	else
		pdata->x_channel = (u8) temp;

	ret = of_property_read_u32(np, "zinitix,y_channel", &temp);
	if (ret)
		input_info(true, dev, "Unable to read y channel\n");
	else
		pdata->y_channel = (u8) temp;
...
static int zt75xx_ts_remove(struct i2c_client *client)
{
	...
	info->sec.sysfs_functions->sec_tsp_support_feature_show = NULL;
	info->sec.sysfs_functions->sec_tsp_prox_power_off_show = NULL;
	info->sec.sysfs_functions->sec_tsp_prox_power_off_store = NULL;
	info->sec.sysfs_functions->dualscreen_policy_store = NULL;
