...
int stm_ts_write_fw_reg(struct stm_ts_data *ts, u16 address, u8 *data, uint32_t length)
{
	...
	u8 reg[STM_TS_SPI_REG_W_CHUNK + 3] = {0};
	...
	while (remaining > 0) {
		...
		to_write_now = remaining > STM_TS_SPI_REG_W_CHUNK ? STM_TS_SPI_REG_W_CHUNK : remaining;

		reg[0] = STM_TS_CMD_REG_SPI_W; // 0xB2 command
		reg[1] = (u8)((address + written_already) >> 8);
		reg[2] = (u8)((address + written_already) & 0xFF);
		memcpy(&reg[3], &data[address + written_already], to_write_now);
		rc = ts->stm_ts_write(ts, &reg[0], to_write_now + 3, NULL, 0);
		if (rc < 0)
			return rc;
...
int stm_ts_read_fw_reg(struct stm_ts_data *ts, u16 address, u8 *read_data, uint32_t read_length)
{
	...
	u8 reg[STM_TS_SPI_REG_R_CHUNK + 3] = {0};
	...
	while (remaining > 0) {
		...
		to_read_now = remaining > STM_TS_SPI_REG_R_CHUNK ? STM_TS_SPI_REG_R_CHUNK : remaining;

		reg[0] = STM_TS_CMD_REG_SPI_R;	// B1 Command for Read
		reg[1] = (u8)(((address + read_already) >> 8) & 0xFF);
		reg[2] = (u8)((address + read_already) & 0xFF);

		rc = ts->stm_ts_read(ts, &reg[0], 3, &read_data[read_already], to_read_now);
...
int stm_ts_hdm_write_request(struct stm_ts_data *ts)
{
	...
	rc = stm_ts_fw_request(ts, STM_TS_FLASH_SAVE_ADDR, 6, 300);
	if (rc < 0) {
		input_err(true, &ts->client->dev, "%s: ERROR\n", __func__);
		return rc;
	}
...
int stm_ts_write_hdm(struct stm_ts_data *ts, u16 address, u8 *data, int length)
{
	...
	while (remaining > 0) {
		...
		to_write_now = remaining > WRITE_CHUNK_SIZE ? WRITE_CHUNK_SIZE : remaining;

		reg[0] = STM_TS_CMD_FRM_BUFF_W;		// 0xB6
		reg[1] = (u8)((address + written_already) >> 8);
		reg[2] = (u8)((address + written_already) & 0xFF);
		memcpy(&reg[3], &data[address + written_already], to_write_now);
		rc = ts->stm_ts_write(ts, &reg[0], to_write_now + 3, NULL, 0);
...
int stm_ts_fw_update_on_probe(struct stm_ts_data *ts)
{
	...
	ts->plat_data->img_version_of_bin[0] = ts->ic_name_of_bin;
	ts->plat_data->img_version_of_bin[1] = ts->project_id_of_bin;
	...
	if (ts->plat_data->hw_param.checksum_result) {
		input_err(true, &ts->client->dev, "%s: do force update (checksum error)\n", __func__);
		retval = STM_TS_NEED_FW_UPDATE;
	} else if ((ts->ic_name_of_ic == 0xFF) && (ts->project_id_of_ic == 0xFF)
			&& (ts->module_version_of_ic == 0xFF)) {
