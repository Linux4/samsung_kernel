...
int stm_ts_i2c_write(struct stm_ts_data *ts, u8 *reg, int cnum, u8 *data, int len)
{
	...
	if (ts->debug_flag & SEC_TS_DEBUG_PRINT_WRITE_CMD) {
		...
		reg_buff = kzalloc(cnum * 3 + 1, GFP_KERNEL);
		if (!reg_buff)
			goto not_err;

		if (data) {
			data_buff = kzalloc(len * 3 + 1, GFP_KERNEL);
			if (!data_buff) {
				kfree(reg_buff);
				goto not_err;
			}
		...
		for (i = 0; i < cnum; i++) {
			...
			snprintf(cbuff, sizeof(cbuff), "%02X ", reg[i]);
			strlcat(reg_buff, cbuff, cnum * 3);
		}
		...
		if (data) {
			for (i = 0; i < len; i++) {
				...
				strlcat(data_buff, cbuff, len * 3);
			}
		}

		if (data)
			input_info(true, &ts->client->dev, "%s: debug: reg: %s, data: %s\n", __func__, reg_buff, data_buff);
		else
			input_info(true, &ts->client->dev, "%s: debug: reg: %s\n", __func__, reg_buff);
...
static const struct of_device_id stm_ts_match_table[] = {
	{ .compatible = "stm,stm_ts_fst2ba61y",},
	{ },
};
#else
...
MODULE_SOFTDEP("pre: s2mpb02-regulator s2mpb02-sub-reg");
