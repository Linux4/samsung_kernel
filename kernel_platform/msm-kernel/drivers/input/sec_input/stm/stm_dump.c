...
static ssize_t get_cmoffset_dump_v2(struct stm_ts_data *ts, char *buf, u8 position)
{
	...
	for (cm_num = 2; cm_num < 4; ++cm_num) {
		input_info(true, ts->dev, "%s: CM%d read start! buf size:%lu\n", __func__, cm_num, strlen(buf));
		...
		if (ret < 0) {
			...
			input_info(true, ts->dev, "%s: read size = %d, ret = %d\n", __func__, size, ret);
		}
		...
		if (value == 0) {
			...
			input_err(true, ts->dev, "%s: CM%d Data empty\n", __func__, cm_num);
			continue;
		} else if (value != signature) {
			snprintf(buff, sizeof(buff), "CM%d : signature mismatched %08X != %08X\n", cm_num, signature, value);
			strlcat(buf, buff, ts->proc_cmoffset_size);

			input_err(true, ts->dev, "%s: CM%d pos[%d] signature is mismatched %08X != %08X\n",
			...
			input_info(true, ts->dev, "%s: CM%d try cnt:%d value %d signature:%8X\n",
		...
		input_info(true, ts->dev, "%s: CM%d read end!\n", __func__, cm_num);
	}

out:
	input_err(true, ts->dev, "%s: pos:%d, buf size:%zu\n", __func__, position, strlen(buf));
...
static ssize_t get_cmoffset_dump_v1(struct stm_ts_data *ts, char *buf, u8 position)
{
	...
	if (signature != SEC_OFFSET_SIGNATURE) {
		input_err(true, ts->dev, "%s: cmoffset[%d], signature is mismatched\n",
				__func__, position);
					...
					size_t len, loff_t *offset)
{
	...
	if (pos == 0) {
		...
		if (!ts->cmoffset_all_proc) {
			input_err(true, ts->dev, "%s: kzalloc fail (cmoffset_all_proc)\n", __func__);
			return 0;
		}
		...
		input_info(true, ts->dev, "%s: retlen[%ld], retlen_sdc[%ld], retlen_sub[%ld], retlen_main[%ld]\n",
	...
	input_info(true, ts->dev, "%s: total:%ld count:%ld\n", __func__, retlen, count);

	if (copy_to_user(buf, ts->cmoffset_all_proc + pos, count)) {
		input_err(true, ts->dev, "%s: copy_to_user error!\n", __func__);
		return -EFAULT;
	}
	...
	*offset += count;
	...
	if (count < len) {
		input_info(true, ts->dev, "%s: print all & free cmoffset_all_proc\n", __func__);
		if (ts->cmoffset_all_proc)
...
static int stm_ts_init_check(struct stm_ts_data *ts)
{
	...
	if (atomic_read(&ts->plat_data->power_state) == SEC_INPUT_STATE_LPM) {
		input_err(true, ts->dev, "%s: Touch is LP mode\n", __func__);
		return -EPERM;
	}
	...
	if (atomic_read(&ts->sec.cmd_is_running)) {
		input_err(true, ts->dev, "%s: cmd is running\n", __func__);
		return -EBUSY;
	}
...
void stm_ts_init_proc(struct stm_ts_data *ts)
{
	...
	if (!entry_cmoffset_all) {
		input_err(true, ts->dev, "%s: failed to create /proc/tsp_cmoffset_all\n", __func__);
