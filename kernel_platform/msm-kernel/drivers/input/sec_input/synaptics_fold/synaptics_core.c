...
void synaptics_ts_reinit(void *data)
{
	...
	if (sec_input_cmp_ic_status(ts->dev, CHECK_LPMODE)) {
		...
		sec_input_set_grip_type(ts->dev, GRIP_ALL_DATA);
	}
...
{
	if (ts->support_immediate_cmd && ts->plat_data->touch_count == 0)
...
static int synaptics_trusted_touch_pre_func(struct device *dev, int value)
{
	struct synaptics_ts_data *ts = dev_get_drvdata(dev);
...
static int synaptics_trusted_touch_post_func(struct device *dev, int value)
{
	struct synaptics_ts_data *ts = dev_get_drvdata(dev);
	struct sec_cmd_data *sec = &ts->sec;
...
int synaptics_ts_stop_device(void *data)
{
	...
	synaptics_ts_v1_clear_command_processing(ts);
...
int synaptics_ts_init(struct synaptics_ts_data *ts)
{
	...
	if (ts->plat_data->pvm) {
		ts->plat_data->pvm->pre_func = synaptics_trusted_touch_pre_func;
		ts->plat_data->pvm->post_func = synaptics_trusted_touch_post_func;
	}
#endif
...
static int synaptics_notifier_call(struct notifier_block *n, unsigned long event, void *data)
{
	...
	int lcd_type = 0;
	u32 lcd_id_mask = 0x00F000;

	input_dbg(false, ts->dev, "%s: called! event = %ld, %s ev_data->state = %d\n", __func__,
				event, evtdata->display_index ? "SUB" : "MAIN", evtdata->state);

	/* get lcd id */
	if ((GET_DEV_COUNT(ts->multi_dev) == evtdata->display_index + 1)
		&& evtdata->state == PANEL_EVENT_PANEL_STATE_ON) {

		input_info(true, ts->dev, "%s: event = %ld, %s ev_data->state = %d\n", __func__, event,
						evtdata->display_index ? "SUB" : "MAIN", evtdata->state);

		/* applied for only b6 model */
		lcd_type = evtdata->d.screen_mode;
		if (GET_DEV_COUNT(ts->multi_dev) == MULTI_DEV_SUB) {
			if ((lcd_type & lcd_id_mask) == 0x000000) {
				of_property_read_string_index(ts->dev->of_node, "sec,firmware_name", 0, &ts->plat_data->firmware_name);
			} else if ((lcd_type & lcd_id_mask) == 0x004000) {
				of_property_read_string_index(ts->dev->of_node, "sec,firmware_name", 1, &ts->plat_data->firmware_name);
			} else {
				input_err(true, ts->dev, "%s: lcd id is nat matched. set default firmware\n", __func__);
				of_property_read_string_index(ts->dev->of_node, "sec,firmware_name", 0, &ts->plat_data->firmware_name);
			}
		} else {
			if ((lcd_type & lcd_id_mask) == 0x000000) {
				of_property_read_string_index(ts->dev->of_node, "sec,firmware_name", 0, &ts->plat_data->firmware_name);
			} else if ((lcd_type & lcd_id_mask) == 0x008000) {
				of_property_read_string_index(ts->dev->of_node, "sec,firmware_name", 1, &ts->plat_data->firmware_name);
			} else {
				input_err(true, ts->dev, "%s: lcd id is nat matched. set default firmware\n", __func__);
				of_property_read_string_index(ts->dev->of_node, "sec,firmware_name", 0, &ts->plat_data->firmware_name);
		...
		input_info(true, ts->dev, "%s: lcd_id: 0x%X, firmware name : %s\n", __func__,
					lcd_type, ts->plat_data->firmware_name);
	...
	if (event == PANEL_EVENT_UB_CON_STATE_CHANGED) {
		...
		if (GET_DEV_COUNT(ts->multi_dev) == evtdata->display_index + 1) {
			if (evtdata->state == PANEL_EVENT_UB_CON_STATE_DISCONNECTED) {
				input_info(true, ts->dev, "%s: %s UB_CON_DISCONNECTED : synaptics_ts_stop_device\n",
									__func__, evtdata->display_index ? "SUB" : "MAIN");
				if (atomic_read(&ts->plat_data->power_state) != SEC_INPUT_STATE_POWER_OFF) {
					struct irq_desc *desc = irq_to_desc(ts->irq);

					input_err(true, ts->dev, "%s: %s UB_CON_DISCONNECTED is called before close func. set power off\n",
									__func__, evtdata->display_index ? "SUB" : "MAIN");
					mutex_lock(&ts->device_mutex);
					disable_irq(ts->irq);
					while (desc->wake_depth > 0)
				...
				input_info(true, ts->dev, "%s: %s UB_CON_CONNECTED : panel attached!\n",
									__func__, evtdata->display_index ? "SUB" : "MAIN");
				ts->panel_attached = SYNAPTICS_PANEL_ATTACHED;
				if (atomic_read(&ts->plat_data->enabled)) {
					input_info(true, ts->dev, "%s: call start func. attach notifier is called after display on.\n", __func__);
					synaptics_ts_start_device(ts);
				}
			}
		}
	}
