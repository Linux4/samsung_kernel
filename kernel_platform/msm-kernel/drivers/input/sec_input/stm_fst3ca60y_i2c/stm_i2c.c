...
int stm_ts_i2c_write(struct stm_ts_data *ts, u8 *reg, int cnum, u8 *data, int len)
{
	...
	if (ts->change_client_addr > 0 && (ts->client->addr != ts->change_client_addr)) {
		input_info(true, &ts->client->dev, "%s: change i2c client addr:0x%02X -> 0x%02X\n",
				__func__, ts->client->addr, ts->change_client_addr);
		ts->client->addr = ts->change_client_addr;
	}
...
static void stm_ts_i2c_parse_dt(struct device *dev, struct stm_ts_data *ts)
{
	struct device_node *np = dev->of_node;

	if (of_property_read_u32(np, "stm,change_client_addr", &ts->change_client_addr))
		ts->change_client_addr = 0;

	input_info(true, dev, "%s: change_client_addr:0x%02X\n", __func__, ts->change_client_addr);
}

static int stm_ts_i2c_init(struct i2c_client *client)
{
	...
	stm_ts_i2c_parse_dt(&client->dev, ts);
...
static const struct of_device_id stm_ts_match_table[] = {
	{ .compatible = "stm,stm_ts_fst3ca60y",},
	{ },
};
#else
