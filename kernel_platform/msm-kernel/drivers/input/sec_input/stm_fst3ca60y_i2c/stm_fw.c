...
int stm_ts_hdm_write_request(struct stm_ts_data *ts)
{
	...
	if (rc < 0) {
		input_err(true, &ts->client->dev, "%s: ERROR1\n", __func__);
		return rc;
	}

	rc = stm_ts_fw_request(ts, STM_TS_FLASH_SAVE_ADDR, 6, 300);
	if (rc < 0) {
		input_err(true, &ts->client->dev, "%s: ERROR2\n", __func__);
		return rc;
	}
