...
static int stm32_keypad_set_input_dev(struct stm32_keypad_dev *device_data, struct pogo_data_struct pogo_data)
{
	...
	if (device_data->input_dev) {
		if (pogo_data.firm_state) {
			input_err(true, &device_data->pdev->dev, "%s input dev already exist. firm done.\n", __func__);
			return ret;
		}
		input_err(true, &device_data->pdev->dev, "%s input dev already exist. start input_unregister_device\n", __func__);
	...
	if (device_data->dtdata->input_name[0] || device_data->dtdata->input_name[1] || device_data->dtdata->input_name[2])
		device_data->input_dev->name = device_data->dtdata->input_name[pogo_data.module_id];
	else
...
static int stm32_keypad_probe(struct platform_device *pdev)
{
	...
	device_data->keycode = kzalloc((device_data->dtdata->num_row << STM32_KPC_ROW_SHIFT) * sizeof(unsigned short),
			GFP_KERNEL);
