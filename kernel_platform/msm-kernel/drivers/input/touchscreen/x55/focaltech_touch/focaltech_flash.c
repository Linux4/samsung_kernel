...
static int fts_get_fw_file_via_request_firmware(struct fts_upgrade *upg)
{
    ...
    if (upg->ts_data->pdata->type == _FT3658U) {
        ...
        if (fts_data->fw_name) {
            snprintf(fwname, FILE_NAME_LENGTH, fts_data->fw_name);
        } else {
...
#endif
    /*M55 code for QN6887A-246 by zhawei at 20231019 start*/
    fts_read_reg(FTS_REG_FW_VER, &fts_data->firmware_ver);
    /*M55 code for QN6887A-246 by zhawei at 20231019 end*/
    fts_irq_enable();
