        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (err) {
        FTS_ERROR("Failed to handle trusted touch in tvm\n");
        return -EINVAL;
    }
#else
    err = fts_ts_handle_trusted_touch_pvm(info, value);
    if (err) {
        FTS_ERROR("Failed to handle trusted touch in pvm\n");
        return -EINVAL;
    }
#endif
    err = count;
...
bit6 :  0 -> strict mode
        1 -> normal mode
bit7 :  0 -> short press/fast mode
        1 -> long press/slow mode
*/
int fts_fod_enable(u8 value)
{
    int ret = 0;
    u8 reg_addr = 0xCF;
    ret = fts_write_reg(reg_addr, value);
    if (ret < 0) {
        FTS_ERROR("write fod enable reg failed, ret:%d", ret);
        return ret;
    }
    return 0;
}
...
/*
set fod rect reg 0xC3 8bytes
data[0] :  left/X min position high 8bit
data[1] :  left/X min position low 8bit
data[2] :  top/Y min position high 8bit
data[3] :  top/Y min position low 8bit
data[4] :  right/X max position high 8bit
data[5] :  right/X max position low 8bit
data[6] :  bottom/Y max position high 8bit
data[7] :  bottom/Y max position low 8bit
*/
int fts_set_fod_rect(u8 *data)
{
    ...
    write_buf[0] = 0xC3;
    memcpy(write_buf + 1, data, 8);
    ret = fts_write(write_buf, sizeof(write_buf));
    if (ret < 0) {
        FTS_ERROR("write fod rect reg failed, ret:%d", ret);
        return ret;
    }
    return 0;
}
...
int fts_read_fod_info(u8 *data)
{
    ...
    ret = fts_read(&reg_addr, 1, data, 3);
    if (ret < 0) {
        FTS_ERROR("read fod info failed, ret:%d", ret);
        return ret;
    }
    return 0;
}
...
int fts_read_fod_pos(u8 *data)
{
    ...
    ret = fts_read(&reg_addr, 1, data, 28);
    if (ret < 0) {
        FTS_ERROR("read fod pos failed, ret:%d", ret);
        return ret;
    }
    return 0;
}
...
/*
set Aod rect reg 0xC6 8bytes
data[0] :  Width/X high 8bit
data[1] :  Width/X low 8bit
data[2] :  height/Y high 8bit
data[3] :  height/Y low 8bit
data[4] :  origin/X position high 8bit
data[5] :  origin/X position low 8bit
data[6] :  origin/Y position high 8bit
data[7] :  origin/Y position low 8bit
*/
int fts_set_aod_rect(u8 *data)
{
    ...
    ret = fts_write(write_buf, sizeof(write_buf));
    if (ret < 0) {
        FTS_ERROR("write aod rect reg failed, ret:%d", ret);
        return ret;
    }
    return 0;
}
