...
/*M55 code for P240111-04942 by hehaoran at 20240118 start*/
    ...
    if (atomic_read(&fts_data->trusted_touch_transition)
        ...
        ret = wait_for_completion_interruptible_timeout(
            ...
            msecs_to_jiffies(FTS_TIMEOUT_COMERR_TRUSTED_TOUCH));
        if (!ret) {
            FTS_ERROR("%s:Trusted touch usecase underway,wait timeout,skip read i2c!\n", __func__);
            return -EPERM;
        }
    }
#endif
...
/*M55 code for P240111-04942 by hehaoran at 20240118 start*/
    ...
    if (atomic_read(&fts_data->trusted_touch_transition)
        ...
        ret = wait_for_completion_interruptible_timeout(
            ...
            msecs_to_jiffies(FTS_TIMEOUT_COMERR_TRUSTED_TOUCH));
        if (!ret) {
            FTS_ERROR("%s:Trusted touch usecase underway,wait timeout,skip write i2c!\n", __func__);
            return -EPERM;
        }
    }
#endif
