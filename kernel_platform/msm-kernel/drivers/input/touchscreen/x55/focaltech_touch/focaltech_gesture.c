...
void fts_gesture_recovery(struct fts_ts_data *ts_data)
{
    ...
    u8 val = (ts_data->spay_en << 2) | (ts_data->double_tap << 4) | (ts_data->single_tap << 5);
    ...
    if (ts_data->gesture_mode && ts_data->suspended) {
        ...
        tp_set_aod_rect(ts_data->ts_data_info.aod_rect_data);
        /*M55 code for QN6887A-4750 by yuli at 20240112 start*/
        if (ts_data->ts_data_info.aod_rect_data[0] || ts_data->ts_data_info.aod_rect_data[1]
            || ts_data->ts_data_info.aod_rect_data[2] || ts_data->ts_data_info.aod_rect_data[3]) {
            val |= (1 << 7);
        }
        FTS_INFO("val = 0x%02x", val);
...
/*M55 code for P240111-04531 by yuli at 20240116 start*/
/**
 * gesture enable reg 0xD1
 * bit2  :  0 -> spay disable
 *              1 -> spay enable
 * bit4  :  0 -> double_tap disable
 *              1 -> double_tap enable
 * bit5  :  0 -> single_tap disable
 *              1 -> single_tap enable
 * bit7  : 0 -> aod double tap disable
 *              1 -> aod double tap enable
*/
/*M55 code for P240111-04531 by yuli at 20240116 end*/
