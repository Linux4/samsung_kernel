    ...
    struct goodix_ts_board_data *board_data)
{
    ...
    /*M55 code for QN6887A-45 by yubo at 20230911 start*/
    if (tp_get_pcbainfo(EVB0) || tp_get_pcbainfo(EVB1)) {
        gpio_name = "goodix,reset-gpio-unofficial";
    } else {
        gpio_name = "goodix,reset-gpio";
    }

    r = of_get_named_gpio(node, gpio_name, 0);
    if (r < 0) {
        ...
        if (ts_event->event_type & EVENT_TOUCH) {
            ...
            if (get_tp_enable()) {
                goodix_ts_report_finger(core_data->input_dev,
                 ...
                 ts_info("get_tp_enable() is false\n");
            }
...
/*M55 code for SR-QN6887A-01-401 by hehaoran at 20230817 start*/
    ...
    if (cd->touch_environment && !strcmp(cd->touch_environment, "pvm")) {
        ...
        ts_info("success register panel events");
    } else {
        ts_err("fail register panel events, no get touch environent = pvm");
    }
...
void gsx_gesture_wakeup_enable(void)
{
    ts_info("enable double tap");
    goodix_modules.core_data->gesture_type |= GESTURE_DOUBLE_TAP;
}

void gsx_gesture_wakeup_disable(void)
{
...
{
    int ret = 0;
    struct lcm_info *gs_gtp_lcm_info = NULL;
    static char module_name[] = "rm692e5_gt9916_visionox";

    ts_info("++++\n");
    gs_gtp_lcm_info = kzalloc(sizeof(*gs_gtp_lcm_info), GFP_KERNEL);
    if (gs_gtp_lcm_info == NULL) {
        ret = -EPERM;
        kfree(gs_gtp_lcm_info);
    }

    gs_gtp_lcm_info->module_name = module_name;
    gs_gtp_lcm_info->fw_version = (goodix_modules.core_data->fw_version.patch_vid);
    gs_gtp_lcm_info->sec = goodix_modules.core_data->sec;
    gs_gtp_lcm_info->gesture_wakeup_enable = gsx_gesture_wakeup_enable;
    gs_gtp_lcm_info->gesture_wakeup_disable = gsx_gesture_wakeup_disable;
    gs_gtp_lcm_info->ito_test = gsx_ito_test;

    tp_common_init(gs_gtp_lcm_info);
    ts_info("module_name=%s", gs_gtp_lcm_info->module_name);
    ts_info("------\n");
}
/*M55 code for SR-QN6887A-01-401 by yuli at 202300816 end*/
...
static int goodix_ts_probe(struct platform_device *pdev)
{
    ...
    ts_info("IN");
      ...
      if (ret) {
            if (!goodix_check_default_tp(node, "qcom,i2c-touch-active")) {
                ret = -EPROBE_DEFER;
            } else {
                ret = -ENODEV;
            }
...
static int __init goodix_ts_core_init(void)
{
    ...
    /*M55 code for QN6887A-148 by hehaoran at 20231011 start*/
    ...
    if (!tp_get_pcbainfo("rm692e5")) {
        ts_err("panel match failed, no need to load TP!");
        return  0;
    }
    /*M55 code for  SR-QN6887A-01-387 by yuli at 20230923 end*/
    /*M55 code for  QN6887A-148 by hehaoran at 20231011 end*/
