...
int get_pwrkey_state(void)
{
    return g_key_power_state;
}
EXPORT_SYMBOL(get_pwrkey_state);

int get_volumedown_state(void)
{
    return g_key_volumedown_state;
}
EXPORT_SYMBOL(get_volumedown_state);

void set_pwr_voldown_state(int keycode,int pressed)
{
    switch (keycode) {
        case KEY_POWER:
            if (pressed) {
        ...
        case KEY_VOLUMEDOWN:
            if (pressed) {
                g_key_volumedown_state = 1;
            } else {
                g_key_volumedown_state = 0;
            }
            break;
        default:
            break;
...
static irqreturn_t pm8941_pwrkey_irq(int irq, void *_data)
{
	 ...
	 */
	/*M55 code for SR-QN6887A-01-400|QN6887A-3285 by yubo at 20231219 start*/
	if (!pwrkey->last_status && !sts) {
		#ifdef CONFIG_ARCH_X55
		set_pwr_voldown_state(pwrkey->code, 1);
		pr_info("pwrkey->code = %d, 1\n", pwrkey->code);
		#endif    // CONFIG_ARCH_X55
	...
	#ifdef CONFIG_ARCH_X55
	set_pwr_voldown_state(pwrkey->code, sts);
	pr_info("pwrkey->code = %d, sts = %d\n", pwrkey->code, sts);
	#endif    // CONFIG_ARCH_X55
	/*M55 code for SR-QN6887A-01-400|QN6887A-3285 by yubo at 20231219 end*/
