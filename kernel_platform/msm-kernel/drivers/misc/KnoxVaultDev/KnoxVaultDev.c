...
static int i2c_read(struct k250a_dev *k250a_dev, char *buf, size_t count, int timeout)
{
	...
	K250ALOG_IPC(k250a_dev, true, "[%s]l=%d of %d bytes, ret=%d", __FUNCTION__, __LINE__, count,
								ret);
	if (ret <= 0) {
		pr_err("[%s]l=%d: returned %d\n", __FUNCTION__, __LINE__, ret);
		goto err;
	}
			 ...
			 size_t count, loff_t *offset)
{
    ...
    pr_err("[%s]l=%d: NONBLOCK read count=%d, ret=%d\n", __FUNCTION__, __LINE__, count, ret);
    // } else {
#if defined(READ_i2c_SLAVE_BY_INTERRUPT)
...
#if defined(READ_i2c_SLAVE_BY_INTERRUPT)
	K250ALOG_IPC(k250a_dev, true, "[%s]l=%d: irq_enabled=%d", __FUNCTION__, __LINE__,
							i2c_dev->irq_enabled);
...
static int k250a_i2c_dev_resume(struct device *device)
{
	...
	i2c_dev = &k250a_dev->i2c_dev;
#if defined(READ_i2c_SLAVE_BY_INTERRUPT)
	K250ALOG_IPC(k250a_dev, true, "[%s]l=%d: irq_wake_up=%d", __FUNCTION__, __LINE__,
							i2c_dev->irq_wake_up);
