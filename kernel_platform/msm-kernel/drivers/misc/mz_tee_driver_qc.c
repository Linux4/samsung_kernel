...
#define CMD_MZ_TA_LOAD 0x001
#define CMD_MZ_TA_UNLOAD 0x002
...
static MzResult send_cmd(uint16_t cmd, uint32_t tgid, void *encrypt, void *iv);
static MzResult load_trusted_app_qc_impl(void);
static MzResult unload_trusted_app_qc_impl(void);
...
static int tee_msg_thread(void *arg)
{
	for (;;) {
		...
		while (!list_empty(&tee_msg_queue)) {
			...
			if (send_cmd_args->cmd == CMD_MZ_TA_LOAD) {
				rc = load_trusted_app_qc_impl();
			} else if (send_cmd_args->cmd == CMD_MZ_TA_UNLOAD) {
				rc = unload_trusted_app_qc_impl();
			} else {
...
static MzResult send_cmd(uint16_t cmd,
		uint32_t tgid,
		void *encrypt,
		void *iv)
{
...
static MzResult send_cmd_kthread(uint16_t cmd, uint32_t tgid, u8 *encrypt, uint8_t *iv)
{
...
MzResult load_trusted_app_qc(void)
{
	...
	taret = send_cmd_kthread(CMD_MZ_TA_LOAD, 0, 0, 0);
	if (taret != MZ_TA_SUCCESS) {
		pr_err("MZ %s ta return error %d\n", __func__, taret);
		mzret = MZ_TA_FAIL;
	}
...
void unload_trusted_app_qc(void)
{
	...
	taret = send_cmd_kthread(CMD_MZ_TA_UNLOAD, 0, 0, 0);
	if (taret != MZ_TA_SUCCESS)
		pr_err("MZ %s ta return error %d\n", __func__, taret);
}
