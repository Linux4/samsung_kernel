...
int tzdev_smc_channel_reserve(struct tzdev_smc_channel *channel, size_t size)
{
	...
	if (unlikely(ZERO_OR_NULL_PTR(pages)))
		return -ENOMEM;
	channel->pages = pages;
...
int tzdev_smc_channel_init(void)
{
	...
	ret_swd = impl->init_swd(meta_pages);
	if (ret_swd) {
		log_error(tzdev_smc_channel, "Failed to init smc channels (error %d)", ret_swd);
		goto free_channels;
	}
