...
static int shmem_desc_wait_state(struct shmem_desc *desc, enum shmem_state state)
{
	...
	/* Increase timeout into 5 sec for test */
	timeleft = wait_for_completion_interruptible_timeout(&desc->done, 10 * HZ);

	spin_lock(&shmem_lock);
	ret = (timeleft > 0 && desc->state == state) ? 0 : -EADDRNOTAVAIL;
...
int tvm_shmem_release_batch(uint32_t mem_handle)
{
	...
	if (ret) {
		log_error(tzdev_platform, "Not correct state. Should be RELEASED. mem handle: %u\n",
			desc->mem_handle);
	}
	mutex_unlock(&notify_lock);
...
static int tvm_shmem_notify_and_wait(struct shmem_desc *desc)
{
	...
	if (ret) {
		log_error(tzdev_platform, "Not correct state. Should be ACCEPTED: mem handle: %u\n",
			desc->mem_handle);
	}
	mutex_unlock(&notify_lock);
