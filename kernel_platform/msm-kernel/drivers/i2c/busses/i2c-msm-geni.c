...
static void gi2c_gsi_tx_cb(void *ptr)
{
	...
	pr_err("%s: Invalid tx_cb buffer", __func__);
	return;
 }
...
static void gi2c_gsi_rx_cb(void *ptr)
{
	...
	pr_err("%s: Invalid rx_cb buffer", __func__);
	return;
 }
			     ...
			     int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		gi2c->cur = &msgs[i];
		 ...
		 if (!gi2c->cur) {
			I2C_LOG_DBG(gi2c->ipcl, false, gi2c->dev,"%s: Invalid buffer", __func__);
		ret = -ENOMEM;
