...
/*M55 code for SR-QN6887A-01-731 by pengxutao at 20231008 start*/
static void hq_qti_flash_led_brightness_set(struct led_classdev *led_cdev,
					enum led_brightness brightness)
{
	struct led_classdev_flash *fdev;
	struct flash_node_data *fnode;
	...
	for (i = 0; i < led->num_snodes; i++) {
		pr_err("snode[%d] symm %d, enabled %d\n", i,
				led->snode[i].symmetry_en,
				led->snode[i].enabled);
		if (led->snode[i].symmetry_en && led->snode[i].enabled) {
			// qti_flash_led_symmetry_config(&led->snode[i]);
...
/*M55 code for SR-QN6887A-01-731 by pengxutao at 20231008 end*/
...
/*M55 code for SR-QN6887A-01-731 by pengxutao at 20231008 start*/
static struct qti_flash_led *samsung_flash_led=NULL;
			...
			struct flash_node_data *fnode, struct device_node *node)
{
	...
	if(!strncmp(fnode->fdev.led_cdev.name, "led:torch_0", strlen("led:torch_0")))
	{
		pr_err("hqtorchdebug add led:torch_0 is ok\n");
		led_cdev_samsung_torch=&(fnode->fdev.led_cdev);
	}
	#if 0
	if(!strncmp(fnode->cdev.name, "led:torch_1", strlen("led:torch_1")))
	{
		pr_err("hqtorchdebug add led:torch_1 is ok\n");
					...
					const char *buf, size_t len)
{
	...
	{
		pr_err("hqtorchdebug add the torch stength error");
		return -EINVAL;
	}
	switch(samsung_level1)
    {
        case 1001:
			set_torch_strength=60;
            break;
        case 1002:
			set_torch_strength=80;
            break;
        case 1003:
			set_torch_strength=100;
            break;
		case 1004:
        case 1005:
			set_torch_strength=150;
            break;
		case 1006:
        case 1007:
		case 1008:
		case 1009:
			set_torch_strength=200;
            break;
        default:
		samsung_level1=0;
            break;
	...
	if(0==samsung_level1)
	{
		qti_flash_led_switch_brightness_set(led_cdev_samsung_switch,0);
		backuplevel = 0;
		return len;
	}
	if(backuplevel == samsung_level1)
	{
		return len;
	}
	backuplevel = samsung_level1;

	pr_err("hqtorchdebug add set torch value:%d",set_torch_strength);
	hq_qti_flash_led_brightness_set(led_cdev_samsung_torch,set_torch_strength);
	qti_flash_led_switch_brightness_set(led_cdev_samsung_switch,1);
