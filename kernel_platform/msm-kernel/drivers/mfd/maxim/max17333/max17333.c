...
static const struct regmap_irq max17333_intsrc_irqs[] = {
	{ .reg_offset = 0, .mask = MAX17333_PROT_INT_MASK,},
};
...
static int max17333_pmic_irq_int(struct max17333_dev *chip)
{
	...
	rc = regmap_add_irq_chip(chip->regmap_pmic, chip->irq,
			...
			IRQF_SHARED, 0, &max17333_intsrc_irq_chip,
			&chip->irqc_intsrc);
	if (rc != 0) {
...
static void max17333_soc_hold_disable(struct max17333_dev *pmic)
{
	u16 rval;
	int ret;

	max17333_update_bits(pmic->regmap_pmic, REG_SOCHOLD, MAX17333_SOC_HOLD_EN, BIT_CLEAR);

	ret = max17333_read(pmic->regmap_pmic, REG_SOCHOLD, &rval);	
	if (ret < 0)
		return;

	pr_info("%s: soc hold read back : 0x%x\n", __func__, rval);
}
		...
		const struct i2c_device_id *id)
{
	...
	max17333_soc_hold_disable(pmic);
...
#else
static void max17333_i2c_remove(struct i2c_client *client)
#endif
{
...
static int max17333_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(dev) && pmic->irq >= 0)
		enable_irq_wake(pmic->irq);
