...
static struct clk_regmap *disp_cc_pineapple_clocks[] = {
...
static const struct qcom_reset_map disp_cc_pineapple_resets[] = {
	[DISP_CC_MDSS_CORE_BCR] = { 0x8000 },
	[DISP_CC_MDSS_CORE_INT2_BCR] = { 0xa000 },
	[DISP_CC_MDSS_RSCC_BCR] = { 0xc000 },
};

static const struct regmap_config disp_cc_pineapple_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static struct qcom_cc_desc disp_cc_pineapple_desc = {
	.config = &disp_cc_pineapple_regmap_config,
	.clks = disp_cc_pineapple_clocks,
	.num_clks = ARRAY_SIZE(disp_cc_pineapple_clocks),
	.resets = disp_cc_pineapple_resets,
	.num_resets = ARRAY_SIZE(disp_cc_pineapple_resets),
	.clk_regulators = disp_cc_pineapple_regulators,
	.num_clk_regulators = ARRAY_SIZE(disp_cc_pineapple_regulators),
};
...
static int disp_cc_pineapple_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &disp_cc_pineapple_desc);
	if (IS_ERR(regmap))
		return PTR_ERR(regmap);

	ret = qcom_cc_runtime_init(pdev, &disp_cc_pineapple_desc);
	if (ret)
		return ret;
	...
	ret = qcom_cc_really_probe(pdev, &disp_cc_pineapple_desc, regmap);
	if (ret) {
...
static void disp_cc_pineapple_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &disp_cc_pineapple_desc);
}

static const struct dev_pm_ops disp_cc_pineapple_pm_ops = {
	SET_RUNTIME_PM_OPS(qcom_cc_runtime_suspend, qcom_cc_runtime_resume, NULL)
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
				pm_runtime_force_resume)
};

static struct platform_driver disp_cc_pineapple_driver = {
	.probe = disp_cc_pineapple_probe,
	.driver = {
		.name = "disp_cc-pineapple",
		.of_match_table = disp_cc_pineapple_match_table,
		.sync_state = disp_cc_pineapple_sync_state,
		.pm = &disp_cc_pineapple_pm_ops,
	},
};

static int __init disp_cc_pineapple_init(void)
{
	return platform_driver_register(&disp_cc_pineapple_driver);
}
subsys_initcall(disp_cc_pineapple_init);

static void __exit disp_cc_pineapple_exit(void)
{
	platform_driver_unregister(&disp_cc_pineapple_driver);
}
module_exit(disp_cc_pineapple_exit);

MODULE_DESCRIPTION("QTI DISP_CC PINEAPPLE Driver");
