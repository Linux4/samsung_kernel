...
extern bool g_usb_connected_unconfigured;
#define USB_ENUMERATION_CURRENT_MA 100
...
static int msm_hsphy_set_power(struct usb_phy *uphy, unsigned int mA)
{
	...
	/*M55 code for SR-QN6887A-01-536 by shixuanxuan at 20230919 start*/
	if (phy->vbus_draw == USB_ENUMERATION_CURRENT_MA) {
		g_usb_connected_unconfigured = true;
	} else {
		g_usb_connected_unconfigured = false;
	}
	dev_err(phy->phy.dev,"msm_hsphy_set_power:%d\n", phy->vbus_draw);
	/*M55 code for SR-QN6887A-01-536 by shixuanxuan at 20230919 end*/
