...
static ssize_t bist_test_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "snk = %d src = %d src_end = %d sink_bist_reg = %x snk_reg = %x src_reg = %x\n",
		chip->sink_timer, chip->source_timer, chip->source_end_timer,
		chip->port.sink_bist_reg, chip->sink_reg_bist, chip->source_reg_bist);
...
bist_test_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d %d %d %x %x %x", &databuf[0], &databuf[1], &databuf[2], &databuf[3], &databuf[4], &databuf[5]) == 6) {
		chip->sink_timer = databuf[0];
		chip->source_timer = databuf[1];
		chip->source_end_timer = databuf[2];
