...
typedef struct {
	AW_BOOL usb_host_data_capable : 1;
	AW_BOOL usb_device_data_capable : 1;
	ProductType product_type_ufp : 3;
	AW_BOOL modal_op_supported : 1;
	ProductType product_type_dfp : 3;
	AW_U8 connector_type : 2;
	AW_U8:5;
	AW_U16 usb_vid : 16;
} IdHeader;
...
typedef struct {
	...
	AW_BOOL has_product_vdo;
	ProductVdo product_vdo;
	AW_BOOL has_cable_vdo;
	CableVdo cable_vdo;
	AW_BOOL has_ama_vdo;
	AmaVdo ama_vdo;
} Identity;
...
typedef struct {
	...
	AW_U16 svid;
	AW_U32 num_modes;
	AW_U32 modes[MAX_MODES_PER_SVID];
} ModesInfo;
