...
/*M55 code for QN6887A-1094 by zhouchenghua at 20231110 start*/
...
/*M55 code for QN6887A-1094 by zhouchenghua at 20231110 end*/
...
struct fsa4480_priv {
	...
	struct fsa_usb_audio usb_audio;
};
			 ...
			 const struct i2c_device_id *id)
{
	...
	fsa_priv->usb_audio.pin.pinctrl = devm_pinctrl_get(fsa_priv->dev);
	if (IS_ERR_OR_NULL(fsa_priv->usb_audio.pin.pinctrl)) {
		dev_dbg(fsa_priv->dev, "%s: Failed to get pinctrl\n", __func__);
		return PTR_ERR(fsa_priv->usb_audio.pin.pinctrl);
	}

	if (fsa_priv->usb_audio.pin.pinctrl) {
		fsa_priv->usb_audio.pin.pins_audio_enable = pinctrl_lookup_state(
					fsa_priv->usb_audio.pin.pinctrl, "usb_audio_enable");
		if (IS_ERR_OR_NULL(fsa_priv->usb_audio.pin.pins_audio_enable)) {
			fsa_priv->usb_audio.pin.pins_audio_enable = NULL;
			dev_dbg(fsa_priv->dev, "%s: can't get usb audio enable pin\n", __func__);
		}

		if(fsa_priv->usb_audio.pin.pins_audio_enable) {
			rc = pinctrl_select_state(fsa_priv->usb_audio.pin.pinctrl,
						fsa_priv->usb_audio.pin.pins_audio_enable);
			dev_dbg(fsa_priv->dev, "%s: audio enable: %d\n",__func__, rc);
		}
	}
        /*M55 code for QN6887A-1560 by zhouchenghua at 2023/11/22 end*/
	/*M55 code for QN6887A-1094 by zhouchenghua at 20231110 end*/
