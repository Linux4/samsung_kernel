...
/* M55 code for QN6887A-200 by yusen at 20231017 start */
#define BDF_PCBA_INFO_NAME_PREFIX   "bdwlan_pcba_"
/* M55 code added for QN6887A-688 by ZhaoXiangxiang at 20231201 start */
#define PCBA_INFO_NAME              "pcbainfo"
/* M55 code added for QN6887A-688 by ZhaoXiangxiang at 20231201 end */
#define PCBA_INFO_MAXLEN            20
/* M55 code for QN6887A-200 by yusen at 20231017 end */
...
/* M55 code for QN6887A-200 by yusen at 20231017 start */
static int bootparams_get_value(char* perporty, char* value, int value_limit)
{
	const char *bootparams = NULL;
	char * perportystr = NULL;
	struct device_node *np = NULL;
	int count = 0;
	np = of_find_node_by_path("/chosen");
	if (!np) {
		icnss_pr_err("[%s]l=%d: failed to find node\n",
				__FUNCTION__, __LINE__);
		return 0;
	}

	if (of_property_read_string(np, "bootargs", &bootparams)) {
		icnss_pr_err("[%s]l=%d: failed to get bootargs property\n",
				__FUNCTION__, __LINE__);
		of_node_put(np); // Free the node reference
		return 0;
	}

	perportystr = strnstr(bootparams, perporty, strlen(bootparams));
	if (perportystr == NULL) {
		icnss_pr_err("[%s]l=%d: failed to get child property:%s\n",
				__FUNCTION__, __LINE__, perporty);
		of_node_put(np);
		return 0;
	}
	// "pcbainfo=CHN_PCB1_EVB1" -> "CHN_PCB1_EVB1"
	perportystr = strnstr(perportystr, "=", strlen(perportystr));
	if (perportystr == NULL) {
		icnss_pr_err("[%s]l=%d: failed to get child value:%s\n",
				__FUNCTION__, __LINE__, perporty);
		of_node_put(np);
		return 0;
	}
	// "CHN_PCB1_EVB1" -> "CHN" + \0 + "PCB1" ...
	strncpy(value, perportystr+1, value_limit);
	for (count = 0; count < value_limit; count++) {
		if (value[count]=='_') {
			value[count]='\0';
			break;
		}
	}
	icnss_pr_err("[%s]l=%d: child perporty:%s = %s",
			__FUNCTION__, __LINE__, perporty, value);
	of_node_put(np);
	return strlen(value);
}
/* M55 code for QN6887A-200 by yusen at 20231017 end */
				   ...
				   u32 filename_len)
{
	...
	int ret = 0;
	/* M55 code for QN6887A-200 by yusen at 20231017 start */
	char pcba_info_str[PCBA_INFO_MAXLEN +1] = {0 };
	int  pcba_info_len = 0;
	/* M55 code for QN6887A-200 by yusen at 20231017 end */
	switch (bdf_type) {
	case ICNSS_BDF_ELF:
		if (priv->board_id == 0xFF)
			...
			snprintf(filename_tmp, filename_len,
				 ...
				 priv->board_id & 0xFF);
		/* M55 code for QN6887A-200 by yusen at 20231017 start */
		pcba_info_len = bootparams_get_value(PCBA_INFO_NAME, pcba_info_str, PCBA_INFO_MAXLEN);
		if (pcba_info_len > 0) {	// "pcbainfo" perpotry exist
			memset(filename_tmp, 0, ICNSS_MAX_FILE_NAME);
			snprintf(filename_tmp, filename_len,
					BDF_PCBA_INFO_NAME_PREFIX "%s.elf", pcba_info_str);	//load bdwlan_pcba_XXX.elf
		}
		/* M55 code for QN6887A-200 by yusen at 20231017 end */
		break;
	case ICNSS_BDF_BIN:
...
int icnss_wlfw_bdf_dnld_send_sync(struct icnss_priv *priv, u32 bdf_type)
{
	...
	/* M55 code for QN6887A-200 by yusen at 20231017 start */
	if ((ret == -ENOENT) && (bdf_type == ICNSS_BDF_ELF)) { // bdwlan_pcba_XXX.elf doesn't exist
		memset(filename, 0, ICNSS_MAX_FILE_NAME);
		snprintf(filename, sizeof(filename), "%s%s", QCA6750_PATH_PREFIX, ELF_BDF_FILE_NAME); // reload "qca6750/bdwlan.elf"
		icnss_pr_err("Reload %s!!!", filename);
		ret = request_firmware(&fw_entry, filename, &priv->pdev->dev);
	}
	/* M55 code for QN6887A-200 by yusen at 20231017 end */
	if (ret) {
