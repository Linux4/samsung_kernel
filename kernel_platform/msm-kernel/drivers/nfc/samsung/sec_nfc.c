...
/*M55 code for  SR-QN6887A-01-204 by lijun at 2023/10/27 start */
#ifdef CONFIG_SEC_NFC_LDO_EN
extern bool ldo_get_pcba_mode(void) {
    ...
    } else if (strnstr(pcbaload, "EVB", strlen(pcbaload))||strnstr(pcbaload, "EVT", strlen(pcbaload))){
        pr_info("%s: this device do not support NFC PVDD LDO", __func__);
        return false;
    }
    pr_info("%s: this device support NFC PVDD LDO", __func__);
    return true;
}
EXPORT_SYMBOL(ldo_get_pcba_mode);

extern bool get_boot_mode(void);
static int is_poweroff_charging_mode(void) {
    if (get_boot_mode())
...
#endif
/*M55 code for  SR-QN6887A-01-204 by lijun at 2023/10/27 end */
...
#endif
    is_poweroff_charging_mode();
#ifdef CONFIG_SEC_NFC_LDO_EN
/*M55 code for  SR-QN6887A-01-204 by lijun at 2023/10/27 start */
    if (ldo_get_pcba_mode()) {
        ret = gpio_request(pdata->pvdd_en, "ldo_en");
        if (ret < 0) {
            pr_info("failed to request about pvdd_en pin\n");
            goto err_dev_reg;
        }
        if (!is_charging_mode)
...
/*M55 code for QN6887A-1048 by lijun at 2023/11/22 start */
    msleep(25);
    gpio_set_value(pdata->ven, SEC_NFC_PW_ON);
/*M55 code for QN6887A-1048 by lijun at 2023/11/22 end */
#endif
#ifdef CONFIG_SEC_NFC_WAKE_LOCK
