...
#endif
/*M55 code for  SR-QN6887A-01-204 by yusen at 2023/11/01 start */
// #if IS_ENABLED(CONFIG_SPI_MSM_GENI)
#include <linux/spi/spi-msm-geni.h>
// #endif
/*M55 code for  SR-QN6887A-01-204 by yusen at 2023/11/01 end */
#include "wakelock.h"
#include "ese_p3.h"
...
#endif /* CONFIG_SEC_ESE_COLDRESET */
		...
		unsigned long arg)
{
	...
	/*M55 code for  SR-QN6887A-01-204 by yusen at 2023/11/01 start */
	P3_ERR_MSG("[ese_p3_driver] %s IOCTL:0x%X\n", __func__,cmd);
	/*M55 code for  SR-QN6887A-01-204 by yusen at 2023/11/01 end */
		...
		loff_t *offset)
{
	...
	/*M55 code for  SR-QN6887A-01-204 by yusen at 2023/11/01 start */
	int count_t=0;
	unsigned char buffer_dest[MAX_BUFFER_SIZE*3]={0};
	/*M55 code for  SR-QN6887A-01-204 by yusen at 2023/11/01 end */
	...
	/*M55 code for  SR-QN6887A-01-204 by yusen at 2023/11/01 start */
	if (count > 1) {
		for(count_t=0; count_t<count; count_t++)
		{
			sprintf(buffer_dest+3*count_t,"%02x ",tx_buffer[count_t]);
		}
		P3_ERR_MSG("[ese_p3_driver]spip3_write(%ld):%s\n",count, buffer_dest);
	}
	/*M55 code for  SR-QN6887A-01-204 by yusen at 2023/11/01 end */
#ifdef FEATURE_ESE_SPI_DUMMY_ENABLE
		...
		loff_t *offset)
{
	...
	if (count > 1) {
		for(count_t=0; count_t<count; count_t++)
		{
			sprintf(buffer_dest+3*count_t,"%02x ",rx_buffer[count_t]);
		}
		P3_ERR_MSG("[ese_p3_driver]spip3_read(%ld):%s\n", count, buffer_dest);
	}
	/*M55 code for  SR-QN6887A-01-204 by yusen at 2023/11/01 end */
...
/*M55 code for  SR-QN6887A-01-204 by lijun at 2023/09/07 start */
/* M55 code added for QN6887A-688 by ZhaoXiangxiang at 20231201 start */
bool nfc_get_mode(void) {
    struct device_node *of_chosen = NULL;
    const char *nfcload = NULL;

    of_chosen = of_find_node_by_path("/chosen");
    if (!of_chosen) {
        of_chosen = of_find_node_by_path("/chosen@0");
    }
    of_property_read_string(of_chosen, "bootargs", &nfcload);
    if (!nfcload) {
        P3_INFO_MSG("%s: read cmdline failed!", __func__);
    } else if (strnstr(nfcload, "nfcinfo=NFC_ESE", strlen(nfcload))) {
        P3_INFO_MSG("%s: this device support NFC_ESE!", __func__);
        return true;
    }

    return false;
}
/* M55 code added for QN6887A-688 by ZhaoXiangxiang at 20231201 end */
/*M55 code for  SR-QN6887A-01-204 by lijun at 2023/09/07 end */
...
/*M55 code for  SR-QN6887A-01-204 by lijun at 2023/09/07 start */
        if (!nfc_get_mode()) {
            P3_INFO_MSG("This device do not support NFC_ESE, we ignore it!\n");
            return -ENODEV;
        }
/*M55 code for  SR-QN6887A-01-204 by lijun at 2023/09/07 end */
