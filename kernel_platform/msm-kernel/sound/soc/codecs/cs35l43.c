...
{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = "Switch", \
		.info = snd_soc_info_volsw, \
		.get = cs35l43_amp_en_get, .put = cs35l43_amp_en_put, \
		       ...
		       struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		if (cs35l43->low_pwr_mode == CS35L43_LOW_PWR_MODE_HIBERNATE &&
			...
			mutex_lock(&cs35l43->hb_lock);
			mutex_lock(&cs35l43->dsp.pwr_lock);
			cs35l43_exit_hibernate(cs35l43);
			mutex_unlock(&cs35l43->dsp.pwr_lock);
			mutex_unlock(&cs35l43->hb_lock);
		}
