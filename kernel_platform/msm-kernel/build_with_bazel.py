...
class BazelBuilder:
    ...
    def __init__(self, target_list, skip_list, out_dir, dry_run, module, project, variant, chipname, user_opts):
    ...
    def build(self):
        ...
        self.user_opts.append("--verbose_failures")
        # self.user_opts.append("--sandbox_debug")
        self.user_opts.append("--debug_cache_dir_conflict=detect")
        self.user_opts.append("--debug_make_verbosity=D")
    ...
    def write_spf_env_mk(
        self,
        filename
    ):
        if os.getenv('GENERATED_SEC_PRODUCT_FEATURE_LIST_FILE') is None:
            return
        spf_file = os.environ['GENERATED_SEC_PRODUCT_FEATURE_LIST_FILE']
        if os.path.isfile(spf_file):
            f = open(spf_file, 'r')
            out = open(filename, 'a')
...
def main():
    ...
    parser.add_argument(
        ...
        metavar="CHIPNAME",
        default="",
        help="Specify the build chipname (e.g. --chipname sm8650), it set in sm8560 file under buildscript/boot_external/bootloaders",
    )
    ...
    builder = BazelBuilder(args.target, args.skip, args.out_dir, args.dry_run, args.module, args.project, args.variant, args.chipname, user_opts)
    try:
        if args.menuconfig:
