...
class IntegrityRoutine(ELF):
    ...
    def get_filtered_canister_symbols(self, list_object_files, debug=False):
        ...
        if debug:
            print("Number of defined canister symbols in .text/.init.text of SKC object files : {}".
                format(len(text_object_symbols)))
            print("Number of defined canister symbols in .rodata of SKC object files          : {}".
                format(len(rodata_object_symbols)))
        ...
        if debug:
            print("Number of defined canister symbols in .text/.init.text of the kernel ELF : {}".
                format(len(canister_symbols_text_no_matches)))
            print("Number of defined canister symbols in .rodata of the kernel ELF          : {}".
                format(len(canister_symbols_rodata_no_matches)))
    ...
    def print_covered_symbols_info(self, sec_sym, addrs_for_hmac, gaps_cover):
        ...
        print("\nTotal size of the Module, bytes            : {}  ".format(self.utils.human_size(hmac_cover + gaps_cover)))
        print("Size of area covered by fingerprint, bytes : {}  ".format(self.utils.human_size(hmac_cover)))
        print("Size of skipped area (as gaps), bytes      : {}  ".format(self.utils.human_size(gaps_cover)))
        print("Cover ratio, %                             : {:.4}% ".format(percent_cover))
                ...
                with open(out_file_txt, mode="w", encoding="utf-8") as out_ft:
                    ...
                    for vaddr_start, vaddr_end, in vaddr_seq:
                        ...
                        for l_count in range(0, block_size):
                            str_dump = str_dump + self.utils.byte_int_to_hex_str(dump_mem[l_count], False) + " "
                            if (l_count + 1) % 16 == 0:
                                str_dump = str_dump + "\n"
                        str_dump = str_dump + "\n"
