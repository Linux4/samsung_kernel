...
#define XATTR_NAME_DD_POLICY "dd:p"
...
{
	return __get_xattr(inode, XATTR_NAME_DD_POLICY, context, sizeof(struct dd_crypt_context));
}

int dd_write_crypt_context(struct inode *inode, const struct dd_crypt_context *context, void *fs_data)
{
	return __set_xattr(inode, XATTR_NAME_DD_POLICY, context, sizeof(struct dd_crypt_context), fs_data);
}
