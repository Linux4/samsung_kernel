...
export TZ=UTC
export LC_ALL=C
# NOTE: FC Release - Samsung does not use 'git' for managing kernel source
# tree.
...
function check_defconfig() {
    ...
    if [ ${RES} -ne 0 ]; then
      # SS FIXME : diffconfig ( sort and  diff)
      local _defconfig="${KERNEL_DIR}/arch/${ARCH}/configs/${DEFCONFIG}"
      [ ! -f "${_defconfig}" ] && \
        _defconfig="${OUT_DIR}/arch/${ARCH}/configs/${DEFCONFIG}"

      RES=0
      ${KERNEL_DIR}/scripts/diffconfig ${_defconfig} ${OUT_DIR}/defconfig >&2 ||
        RES=$?
