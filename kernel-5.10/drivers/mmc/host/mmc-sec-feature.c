...
static void sd_sec_sdcard_noti_work(struct work_struct *work)
{
	...
	ret = kobject_uevent(&sdcard_sec_dev->kobj, KOBJ_CHANGE);
	if (ret)
...
static void sd_sec_inc_status_err(struct mmc_card *card, u32 status)
{
	 ...
	 */
	if (noti && mmc_card_sd(card) && sdcard_sec_dev)
...
void mmc_set_sec_features(struct platform_device *pdev)
{
	...
	svi.mmc = mmc;
	svi.status = mmc_gpio_get_cd(mmc) ? true : false;
		...
		struct mmc_host *host, bool *allow)
{
	...
	if (status ^ svi.status) {
		pr_err("%s: slot status change detected (%d -> %d)\n",
				mmc_hostname(host), svi.status, status);
		svi.status = status;
	} else {
		pr_err("%s: slot status isn't changed (%d -> %d)\n",
				mmc_hostname(host), svi.status, status);
		*allow = false;
	}
}
