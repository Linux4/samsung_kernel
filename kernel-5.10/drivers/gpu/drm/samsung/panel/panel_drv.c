...
static int panel_set_mask_layer(struct panel_device *panel, void *arg)
{
	...
	if (req_data->req_mask_layer == MASK_LAYER_ON) {
		if (req_data->trigger_time == MASK_LAYER_TRIGGER_BEFORE) {
			...
			panel_mutex_lock(&panel_bl->lock);
			if (check_seqtbl_exist(panel, PANEL_MASK_LAYER_STOP_DIMMING_SEQ)) {
				panel_bl->props.smooth_transition = SMOOTH_TRANS_OFF;
				panel_do_seqtbl_by_name(panel, PANEL_MASK_LAYER_STOP_DIMMING_SEQ);
			}

			/* 1. REQ ON + FRAME START BEFORE */
			if (check_seqtbl_exist(panel, PANEL_MASK_LAYER_BEFORE_SEQ))
				panel_do_seqtbl_by_name(panel, PANEL_MASK_LAYER_BEFORE_SEQ);
			...
			if (check_seqtbl_exist(panel, PANEL_MASK_LAYER_ENTER_BR_SEQ)
				...
				panel_do_seqtbl_by_name(panel, PANEL_MASK_LAYER_ENTER_BR_SEQ);
			...
			if (check_seqtbl_exist(panel, PANEL_MASK_LAYER_AFTER_SEQ))
				panel_do_seqtbl_by_name(panel, PANEL_MASK_LAYER_AFTER_SEQ);
		...
		if (req_data->trigger_time == MASK_LAYER_TRIGGER_BEFORE) {
			...
			if (check_seqtbl_exist(panel, PANEL_MASK_LAYER_EXIT_BR_SEQ)
				&& (panel->state.cur_state != PANEL_STATE_ALPM)) {
...
#endif
				panel_do_seqtbl_by_name(panel, PANEL_MASK_LAYER_EXIT_BR_SEQ);
				wake_up_interruptible_all(&panel_bl->wq.wait);
				panel_mutex_unlock(&panel_bl->lock);
			} else {
				panel_mutex_unlock(&panel_bl->lock);
