...
struct lcm {
	...
	struct gpio_desc *tsp_3p0_en_gpio;
	struct gpio_desc *lcd_3p0_en_gpio;
	struct gpio_desc *lcd_1p8_en_gpio;
...
static int lcm_panel_bias_enable(struct drm_panel *panel)
{
	...
	ctx->lcd_1p8_en_gpio = devm_gpiod_get(ctx->dev, "lcd-1p8-en", GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->lcd_1p8_en_gpio)) {
		dev_info(ctx->dev, "%s: cannot get lcd-1p8-en_gpio %ld\n",
			__func__, PTR_ERR(ctx->lcd_1p8_en_gpio));
		return 0;
	}
	gpiod_set_value(ctx->lcd_1p8_en_gpio, 1);
	devm_gpiod_put(ctx->dev, ctx->lcd_1p8_en_gpio);

	return 0;
}

static int lcm_panel_bias_disable(struct drm_panel *panel)
{
	struct lcm *ctx = panel_to_lcm(panel);
	...
	gpiod_set_value(ctx->lcd_1p8_en_gpio, 0);
	devm_gpiod_put(ctx->dev, ctx->lcd_1p8_en_gpio);
...
static struct LCM_setting_table init_setting_cmd[] = {
	...
	{2, {0x9D, 0x01}},
	/* 4.1.1 DSC Setting */
	{90, {0x9E, 0x11, 0x00, 0x00, 0x89, 0x30, 0x80, 0x09, 0x24,
				0x04, 0x38, 0x00, 0x75, 0x02, 0x1C, 0x02, 0x1C,
				0x02, 0x00, 0x02, 0x52, 0x00, 0x20, 0x0A, 0x27,
				0x00, 0x07, 0x00, 0x0F, 0x01, 0x09, 0x00, 0xDF,
	...
	{2, {0xFE, 0xB0}},
	{2, {0xFE, 0x30}},
	{3, {0x60, 0x01, 0x00}},    // 0x01,0x00 -> 90hz , 0x21,0x00->60hz
	{2, {0x53, 0x20}},
	{3, {0x51, 0x03, 0xFF}},
	{2, {0xF7, 0x0B}},
	{3, {0xFC, 0xA5, 0xA5}},
...
static struct drm_display_mode performance_mode = {
	.clock = 275942,// Pixel Clock = vtotal * htotal * vrefresh(60) / 1000;
	.hdisplay = HAC,
	.hsync_start = HAC + HFP,
	.hsync_end = HAC + HFP + HSA,
	.htotal = HAC + HFP + HSA + HBP,
	.vdisplay = VAC,
	.vsync_start = VAC + VFP + 1230,
	.vsync_end = VAC + VFP + 1230 + VSA,
	.vtotal = VAC + VFP + 1230 + VSA + VBP,
	//.vrefresh = 60,
};

static struct drm_display_mode default_mode = {
//	.clock = 341280,//276092,// Pixel Clock = vtotal * htotal * vrefresh(90) / 1000;\
	// Pixel Clock = vtotal * htotal * vrefresh(90) / 1000;
	.clock = (VAC + VFP + VSA + VBP) * (HAC + HFP + HSA + HBP) * 90 / 1000,
	.hdisplay = HAC,
	.hsync_start = HAC + HFP,
	.hsync_end = HAC + HFP + HSA,
	.htotal = HAC + HFP + HSA + HBP,
...
//	.pll_clk = 210,
	...
	.dsc_params = {
		...
		.dec_delay = 594,
		.scale_value = 32,
		.increment_interval = 2599,//739,//989,
		.decrement_interval = 7,
		.line_bpg_offset = 15,
		.nfl_bpg_offset = 265,//848,//631,
		.slice_bpg_offset = 223,//868,//651,
		.initial_offset = 6144,
		.final_offset = 4336,
		.flatness_minqp = 3,
		.flatness_maxqp = 12,
	...
	.data_rate = 898,//1174,//1663,//587,//795,//898,
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel-s6e8fc3-s2dos15a01-vdo",
		.owner = THIS_MODULE,
