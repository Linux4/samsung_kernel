...
ssize_t	rkp_test_read(struct file *filep, char __user *buffer, size_t count, loff_t *ppos)
{
...
static const struct proc_ops rkp_proc_ops = {
	.proc_read		= rkp_test_read,
};
...
static int __init rkp_test_init(void)
{
	...
	if (proc_create("rkp_test", 0444, NULL, &rkp_proc_ops) == NULL) {
	...
	if (!va)
		return -1;
	uh_call(UH_APP_RKP, RKP_TEST_INIT, va, 0, 0, 1);
...
{
	uh_call(UH_APP_RKP, RKP_TEST_EXIT, (u64)ha1, 0, 0, 1);
