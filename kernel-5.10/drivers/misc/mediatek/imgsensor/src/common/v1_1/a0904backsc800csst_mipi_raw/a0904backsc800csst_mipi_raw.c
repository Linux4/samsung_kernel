...
#include "a0904backsc800csst_mipi_raw.h"

#define PFX "a0904backsc800csst_mipi_raw"
#define printk(format, args...)    printk(PFX "[%s] " format, __FUNCTION__, ##args)

#define SC800CS_ST_SENSOR_GAIN_MAX_VALID_INDEX  6
#define SC800CS_ST_SENSOR_GAIN_MAP_SIZE         6

#define SC800CS_ST_SENSOR_BASE_GAIN           0x400
#define SC800CS_ST_SENSOR_MAX_GAIN            (635 * SC800CS_ST_SENSOR_BASE_GAIN /10)
#define SC800CS_ST_EEPROM_IC_ADDR             0xA0
#define SC800CS_ST_MODULE_ID                  0x04

static DEFINE_SPINLOCK(imgsensor_drv_lock);
/*Tab A9 code for SR-AX6739A-01-224 by hudongdong at 20230519 start */
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = A0904_BACK_SC800CSST_SENSOR_ID,
...
{
	return (((read_cmos_sensor(0x3107) << 8) | read_cmos_sensor(0x3108)) + 1); //0xd127
}
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < SC800CS_ST_SENSOR_BASE_GAIN)
		reg_gain = SC800CS_ST_SENSOR_BASE_GAIN;
	else if (reg_gain > SC800CS_ST_SENSOR_MAX_GAIN)
		reg_gain = SC800CS_ST_SENSOR_MAX_GAIN;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 SC800CS_ST_AGC_Param[SC800CS_ST_SENSOR_GAIN_MAP_SIZE][2] = {
		{  1024,  0x00 },
		{  2048,  0x01 },
		{  4096,  0x03 },
		{  8192,  0x07 },
	...
	for (gain_index = SC800CS_ST_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 0; gain_index--)
		if (reg_gain >= SC800CS_ST_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0x3e09, SC800CS_ST_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * SC800CS_ST_SENSOR_BASE_GAIN / SC800CS_ST_AGC_Param[gain_index][0];
	write_cmos_sensor(0x3e07, (temp_gain >> 3) & 0xff);
	printk("SC800CS_ST_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d, gain = %d\n",
		SC800CS_ST_AGC_Param[gain_index][1], temp_gain, reg_gain, gain);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	imgsensor.i2c_write_id = SC800CS_ST_EEPROM_IC_ADDR;
	module_id = read_cmos_sensor(0x0002);
	if(module_id == SC800CS_ST_MODULE_ID || module_id == 0xff)
	{
		printk("SC800CS_ST read module id success! Current module id is 0x%x", module_id);
		...
		printk("SC800CS_ST i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
		if (*sensor_id != imgsensor_info.sensor_id) {
	...
	{
		printk("SC800CS_ST read module id fail! Current module id is 0x%x", module_id);
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	return ERROR_NONE;
...
UINT32 A0904BACKSC800CSST_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
