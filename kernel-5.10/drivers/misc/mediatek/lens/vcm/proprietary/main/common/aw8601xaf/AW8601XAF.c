...
/*Tab A9 code for  AX6739A-406 by rongyi at 20230718 start */

static int AW8601XAF_WriteReg_Directly(u16 a_u2Data)
{
	int i4RetValue = 0;

	char puSendCmd[2] = { (char)(a_u2Data >> 8), (char)(a_u2Data & 0xFF) };

	g_pstAF_I2Cclient->addr = AW8601XAF_SLAVE_ADDR;
...
{
	//int ret = 0;
	unsigned char adv_mode_cmd[] = { 0xed, 0xab };
	unsigned char alg_mode_cmd[] = { 0x06, 0x00 };
	unsigned char nor_mode_cmd[] = { 0xdf, 0x5b };

	AW_LOGI("Start.");

	if (*g_pAF_Opened == 2) {

		AW8601XAF_WriteRegs(adv_mode_cmd[0], &adv_mode_cmd[1], AW_DATA_BYTE_1);//Advance mode
		AW8601XAF_WriteRegs(alg_mode_cmd[0], &alg_mode_cmd[1], AW_DATA_BYTE_1);//Direct mode
		AW8601XAF_WriteRegs(nor_mode_cmd[0], &nor_mode_cmd[1], AW_DATA_BYTE_1);//Direct mode
		AW8601XAF_WriteReg_Directly(0xECA3);
		AW8601XAF_WriteReg_Directly(0xA114);
		AW8601XAF_WriteReg_Directly(0xF230);
		AW8601XAF_WriteReg_Directly(0xDC51);
		AW8601XAF_WriteReg_Directly(0x12C0);
		mdelay(15);
		AW8601XAF_WriteReg_Directly(0xECA3);
		AW8601XAF_WriteReg_Directly(0xA100);
		AW8601XAF_WriteReg_Directly(0xF200);
		AW8601XAF_WriteReg_Directly(0xDC51);
		AW8601XAF_WriteReg_Directly(0x0004);
		mdelay(25);
	}
	AW8601XAF_PowerDown(g_pstAF_I2Cclient, g_pAF_Opened);
...
/*Tab A9 code for  AX6739A-406 by rongyi at 20230718 end */
