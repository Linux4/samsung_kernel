...
static int32_t cam_ois_mcu_fw_update_rumbac0(struct mcu_info *mcu_info, struct i2c_client *client)
{
	int ret = 0;
	...
	/* OISSTS Read */
	ret = cam_ois_i2c_read_multi(client, MCU_I2C_SLAVE_W_ADDR, 0x0001, &readData, 1);
	if (ret != 0 || readData != 0x01) {/* OISSTS != IDLE */
		LOG_ERR("OISSTS Read, ret = %d, data: %d", ret, readData);
		if (ret == 0)
			ret = -EINVAL;
		goto ERROR;
	}
	...
	// NewFWRev = Read target Binary File address 0xAFF4 - 0xAFF7; (*1)
	/* Update a User Program */
	/* SET FWUP_CTRL REG */
	/* FWUPCTRL REG(0x000C) 1Byte Send */
	cam_ois_i2c_write_one(client, MCU_I2C_SLAVE_W_ADDR, 0x000C, 0xB5);

	/* Wait for FlashROM(11Blocks) Erase Process */
	msleep(90);

	/* Write UserProgram Data */
	fw_size = (fw_size / unit) * unit;
	len = fw_size;
	wbytes = 0;
	for (i = 0; i < fw_size / unit; i++) {
		LOG_INF("write wbytes=%d left len=%d", wbytes, len);
		memcpy(sendData, buffer + wbytes, unit);
		cam_ois_i2c_write_multi(client, MCU_I2C_SLAVE_W_ADDR, 0x0100, sendData, unit);
		wbytes += unit;
		len -= unit;
	...
	/* Error Status read */
	ret = cam_ois_i2c_read_two(client, MCU_I2C_SLAVE_W_ADDR, 0x0006, &readData16);
	/* F/W Update Error check */
	if (ret == 0 && readData16 == 0x0001) {
		/* verify checkSum */
		...
		/* FWUP_CHKSUM REG(0x0008) 4Byte Send */
		cam_ois_i2c_write_multi(client, MCU_I2C_SLAVE_W_ADDR, 0x08, sendData, 4);

		/* (Wait RUMBA Self Reset + OIS Data Section initialize) */
		msleep(190);

		/* F/W Update Error check */
		ret = cam_ois_i2c_read_two(client, MCU_I2C_SLAVE_W_ADDR, 0x0006, &readData16);
		if (ret != 0 || readData16 != 0x0000) {
			LOG_ERR("Error Status after reset, ret = %d, data: %d", ret, readData16);
			if (ret == 0)
				ret = -EINVAL;
			goto ERROR;
		} else {
			ret = cam_ois_i2c_write_one(client, MCU_I2C_SLAVE_W_ADDR, 0x0036, 0x81);
			msleep(100);
		}

	} else {
		LOG_ERR("Error Status, ret = %d, data: %d", ret, readData16);
		if (ret == 0)
			ret = -EINVAL;
		goto ERROR;
	}
...
FW_UPDATE_RETRY:
	...
	if ((strncmp(mcu_info->module_ver, "TSD1", OIS_MCU_VERSION_SIZE) == '\0' &&
		strncmp(mcu_info->phone_ver, "FSD1", OIS_MCU_VERSION_SIZE) == '\0') || // Force update: initial module of A23M & A24-LTE FW
		(strncmp(mcu_info->module_ver, "CSD1", OIS_MCU_VERSION_SIZE) == '\0' &&
		strncmp(mcu_info->phone_ver, "FSD1", OIS_MCU_VERSION_SIZE) == '\0')) { // Force update: module updated with A23M FW & A24-LTE FW
		is_mcu_nack = true;
	...
	if (strncmp(mcu_info->phone_ver, mcu_info->module_ver, OIS_MCU_VERSION_SIZE) == '\0' ||
		...
		if ((strncmp(mcu_info->phone_ver, mcu_info->module_ver, OIS_VER_SIZE) > '\0')
			...
			rc = cam_ois_mcu_fw_update_rumbac0(mcu_info, client);
			if (rc < 0) {
				is_need_retry = true;
