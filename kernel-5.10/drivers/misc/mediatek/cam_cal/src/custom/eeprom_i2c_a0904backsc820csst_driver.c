...
static unsigned int sc820cs_sensor_otp_read_lsc_data(int page,unsigned int ui4_length,unsigned char *pinputdata)
{
	...
	{
		case 2:
		ui4_lsc_offset = 0x82b9;
		break;
		case 6:
		ui4_lsc_offset = otp_group==2 ?0x8bee:ui4_lsc_offset;
		break;
		default:
		break;
	}
...
static int sc820cs_sensor_otp_read_module_info(unsigned char *pinputdata)
{
	...
	if(read_cmos_sensor(0x827a) == 0x01)
	{
		ret = sc820cs_sensor_otp_read_data(0x827b, 15, pinputdata);
		otp_group = 1;
	...
	else if(read_cmos_sensor(0x827a) == 0x13)
	{
		ret = sc820cs_sensor_otp_read_data(0x828a, 15, pinputdata);
		otp_group = 2;
		if(ret == SC820CS_ST_RET_SUCCESS)
		{
...
static int sc820cs_sensor_otp_read_awb_info_group1(unsigned char *pinputdata)
{
	...
	if(read_cmos_sensor(0x8299) == 0x01)
	{
		ret = sc820cs_sensor_otp_read_data(0x829a, 15, pinputdata);
	}
	else
	{
...
static int sc820cs_sensor_otp_read_awb_info_group2(unsigned char *pinputdata)
{
	...
	if(read_cmos_sensor(0x8299) == 0x13)
	{
		ret = sc820cs_sensor_otp_read_data(0x82a9, 15, pinputdata);
	}
	else
	{
...
static int sc820cs_sensor_otp_read_lsc_info_group1(unsigned char *pinputdata)
{
	...
	if(read_cmos_sensor(0x82b8) == 0x01)
	{
		checksum_cal += sc820cs_sensor_otp_read_lsc_data(page, 0x147, pinputdata);
		page = page + 1;

        for(;page <= 5;page++)
		{
			sc820cs_set_page_and_load_data(page);//set page--3,4,5 && load data
			checksum_cal += sc820cs_sensor_otp_read_lsc_data(page,0x186,pinputdata+0x147+(page-3)*0x186);
		}

        sc820cs_set_page_and_load_data(page);//set page6
		checksum_cal += sc820cs_sensor_otp_read_lsc_data(page,0x173,pinputdata+0x147+(page-3)*0x186);
		checksum_cal = checksum_cal%255+1;
		checksum  = read_cmos_sensor(0x8BED);
...
static int sc820cs_sensor_otp_read_lsc_info_group2(unsigned char *pinputdata)
{
	...
	if(read_cmos_sensor(0x82b8) == 0x13)
	{
		page = page + 4;
		sc820cs_set_page_and_load_data(page);//set page:6
		checksum_cal += sc820cs_sensor_otp_read_lsc_data(page, 0x12, pinputdata);
		page++;
        for(;page <= 10;page++)
		{
			sc820cs_set_page_and_load_data(page);//set page:7,8,9,10 && load data
			checksum_cal += sc820cs_sensor_otp_read_lsc_data(page,0x186,pinputdata+0x12+(page-7)*0x186);
		}

        sc820cs_set_page_and_load_data(page);//set page:11
		checksum_cal += sc820cs_sensor_otp_read_lsc_data(page,0x122,pinputdata+0x12+(page-7)*0x186);//read page:11 otp data
		checksum_cal = checksum_cal%255+1;
		checksum  = read_cmos_sensor(0x959C);
		pinputdata[0x74C] = checksum;
		if(checksum_cal == checksum)
		{
...
static int sc820cs_sensor_otp_read_group1(unsigned int ui4_offset,unsigned int ui4_length, unsigned char *pinputdata)
{
	...
	for(threshold=0; threshold<3; threshold++)
	{
		...
		ret = sc820cs_sensor_otp_read_awb_info_group1(pinputdata+15);
		...
		else
		{
			...
			ret = sc820cs_sensor_otp_read_lsc_info_group1(pinputdata+30);
			if(ret == SC820CS_ST_RET_FAIL)
			{
...
static int sc820cs_sensor_otp_read_group2(unsigned int ui4_offset,unsigned int ui4_length, unsigned char *pinputdata)
{
	...
	for(threshold=0; threshold<3; threshold++)
	{
		...
		ret = sc820cs_sensor_otp_read_awb_info_group2(pinputdata+15);
		if(ret == SC820CS_ST_RET_FAIL)
		{
		...
		else
		{
			...
			ret = sc820cs_sensor_otp_read_lsc_info_group2(pinputdata+30);
			if(ret == SC820CS_ST_RET_FAIL)
			{
...
unsigned int a0904backsc820csst_read_region(struct i2c_client *client, unsigned int addr,
