...
// STANLEY SONG
	if( ARRAY_SIZE(power_supply_attrs) < prop) {
		pr_info("[STANLEY SONG] prop : %d\n", prop);
		return 0;
	}
...
int power_supply_uevent(struct device *dev, struct kobj_uevent_env *env)
{
	...
	pr_info("[STANLEY SONG] POWER_SUPPLY_NAME=%s\n", psy->desc->name);  
	...
	pr_info("[STANLEY SONG] num_prop = %d\n", psy->desc->num_properties);
/* STANLEY SONG
	for (j = 0; j < psy->desc->num_properties; j++) {
		pr_info("[STANLEY SONG] num_prop = 0x%x : %d\n", (int)psy->desc->properties[j], (int)psy->desc->properties[j]);  
	}
*/	
	for (j = 0; j < psy->desc->num_properties; j++) {
