...
#include "focaltech_pramtest_ft3418.h"
...
extern int ft3418_pram_test_write_buf(u8 *buf, u32 len);
extern int ft3418_pram_test_ecc_cal(u32 saddr, u32 len);
...
static void get_pram_test(void *device_data)
{
	...
	u8 cmd[4] = { 0 };
	u8 val[5] = { 0 };
	u8 *pb_test_buf = ft3418_pram_test_file;
	u32 pb_test_len = sizeof(ft3418_pram_test_file);
	int result = 0;
	u8 mode_temp = 0;
	...
	ecc_tp = ft3418_pram_test_ecc_cal(0, pb_test_len);
	if (ecc_tp < 0) {
    ...
    if (ecc_host != ecc_tp) {
		FTS_ERROR( "ecc check fail,ecc_host = 0x%x,ecc_tp = 0x%x",ecc_host,ecc_tp);
		goto test_reset;
	}
	// remap pramboot
	cmd[0] = 0x08;
...
static void fix_active_mode(void *device_data)
{
	...
	ret = fts_write_reg(0xB7, enable);
	if (ret < 0) {
		FTS_ERROR("failed to write always active mode %d", enable);
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		return;
	}
