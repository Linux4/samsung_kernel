...
static struct timespec64 timeStart, timeEnd;
...
void jadard_log_touch_int_devation(int finger_event)
{
    if (finger_event == JD_ATTN_IN) {
        ...
        JD_I("Touch latency = %ld us\n", ((timeEnd.tv_sec * 1000000000 + timeEnd.tv_nsec) - (timeStart.tv_sec * 1000000000 + timeStart.tv_nsec)) / 1000);
    }
}
...
int jadard_touch_proc_init(void)
{
    ...
    jadard_proc_diag_apk_file = proc_create(JADARD_PROC_DIAG_APK_FILE, (S_IRUGO),
...
fail_D: remove_proc_entry(JADARD_PROC_DIAG_APK_FILE, pjadard_touch_proc_dir);
fail_C:
    if ( g_common_variable.dbi_dd_reg_mode == JD_DDREG_MODE_1 ) {
