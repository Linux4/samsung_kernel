...
/*Tab A9 code for AX6739A-1271 by suyurui at 20230613 start*/
#define SET_PWR_MODE_RETRY_TIME         3
#define SET_PWR_MODE_RETRY_DELAY        5
/*Tab A9 code for AX6739A-1271 by suyurui at 20230613 end*/
...
int cts_irq_handler(struct cts_device *cts_dev)
{
    ...
    if (unlikely(cts_dev->rtdata.suspended)) {
        ...
        ret = cts_plat_process_touch_msg(cts_dev->pdata, touch_info->msgs,
                touch_info->num_msg, &touch_info->debug_msg);
        if (ret) {
...
int cts_suspend_device(struct cts_device *cts_dev)
{
    int ret;
    /*Tab A9 code for AX6739A-1271 by suyurui at 20230613 start*/
    int count = SET_PWR_MODE_RETRY_TIME;
    /*Tab A9 code for AX6739A-1271 by suyurui at 20230613 end*/
    u8 buf;
...
try_again:
    ...
    if (ret) {
        cts_err("Set pwr mode failed %d", ret);
        if (count--) {
            mdelay(SET_PWR_MODE_RETRY_DELAY);
            goto try_again;
        }
    } else {
...
void cts_disable_esd_protection(struct chipone_ts_data *cts_data)
{
    if (cts_data->esd_workqueue && cts_data->esd_enabled) {
        ...
        /*Tab A9 code for AX6739A-1175 by suyurui at 20230616 start*/
        cancel_delayed_work_sync(&cts_data->esd_work);
        /*Tab A9 code for AX6739A-1175 by suyurui at 20230616 end*/
    }
}
...
int cts_swap_x_y(struct cts_device *cts_dev, u16 *data)
{
    int i = 0;
    int j = 0;
    int k = 0;
    ...
    nodes = cts_dev->fwdata.rows * cts_dev->fwdata.cols;

    tmpdata = (u16 *) kmalloc(cts_dev->hwdata->num_row * cts_dev->hwdata->num_col * 2, GFP_KERNEL);
    if (tmpdata == NULL) {
    ...
    for (i = 0; i < cts_dev->fwdata.rows; i++) {
        for (j = 0, k = 0; j < cts_dev->fwdata.cols; j++) {
            data[nodes - (i + 1) - k * cts_dev->fwdata.rows] =
                tmpdata[j + i * cts_dev->fwdata.cols];
            k++;
        }
    }
...
int cts_swap_cap_x_y(struct cts_device *cts_dev, u8 *data)
{
    int i = 0;
    int j = 0;
    int k = 0;
    ...
    tmpdata = kzalloc(cts_dev->hwdata->num_row * cts_dev->hwdata->num_col, GFP_KERNEL);
    if (tmpdata == NULL) {
        cts_err("Allocate memory for tmpdata failed\n");
        return -ENOMEM;
    }
