...
/*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 start*/
#include <linux/rtc.h>
#include <linux/timekeeping.h>
#define ITO_DATA_OUT_PATH "/data/tpdata/"
#define SHORT_TEST_CSV_FILE            ITO_DATA_OUT_PATH"ShortTest_"
#define OPEN_TEST_CSV_FILE              ITO_DATA_OUT_PATH"OpenTest_"
#define FW_RAWDATA_CSV_FILE         ITO_DATA_OUT_PATH"FWMutualTest_"
#define FW_CC_CSV_FILE                        ITO_DATA_OUT_PATH"FWCCTest_"
#define NOISE_TEST_CSV_FILE             ITO_DATA_OUT_PATH"NoiseTest_"
#define ITO_DATA_OUT_FILEPATH_LEN    128
#define FIXED_CHAR_LEN                  7
#define TIME_COEFFICIENT               60
#define SYSTEM_TIME                         1900
static char* gs_file_path = NULL;
uint32_t rawdata_diff_min_offset_noise = 0;
/*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 end*/

#if LPWG_TEST
#define LPWG_RAWDATA_CSV_FILE "/data/tpdata/LPWGRawdataTest.csv"
#define LPWG_NOISE_CSV_FILE "/data/tpdata/LPWGNoiseTest.csv"
#endif
#if (FDM_TEST && LPWG_TEST)
#define FDM_FW_RAWDATA_CSV_FILE "/data/tpdata/FWFDMMutualTest.csv"
#define FDM_NOISE_TEST_CSV_FILE "/data/tpdata/FDMNoiseTest.csv"
#endif
...
static void nvt_print_criteria(void)
{
    ...
    nvt_print_data_array(PS_Config_Lmt_FDM_Rawdata_P, FDM_X_Channel, Y_Channel);
    printk("PS_Config_Lmt_FDM_Rawdata_N:\n");
    nvt_print_data_array(PS_Config_Lmt_FDM_Rawdata_N, FDM_X_Channel, Y_Channel);
    //---PS_Config_Lmt_FDM_Diff---
    printk("PS_Config_Lmt_FDM_Diff_P:\n");
    nvt_print_data_array(PS_Config_Lmt_FDM_Diff_P, FDM_X_Channel, Y_Channel);
    printk("PS_Config_Lmt_FDM_Diff_N:\n");
    nvt_print_data_array(PS_Config_Lmt_FDM_Diff_N, FDM_X_Channel, Y_Channel);
#endif

#if LPWG_TEST
    //---PS_Config_Lmt_LPWG_Rawdata---
    printk("PS_Config_Lmt_LPWG_Rawdata_P:\n");
    nvt_print_data_array(PS_Config_Lmt_LPWG_Rawdata_P, X_Channel, Y_Channel);
    printk("PS_Config_Lmt_LPWG_Rawdata_N:\n");
    nvt_print_data_array(PS_Config_Lmt_LPWG_Rawdata_N, X_Channel, Y_Channel);
    //---PS_Config_Lmt_LPWG_Diff---
    printk("PS_Config_Lmt_LPWG_Diff_P:\n");
    nvt_print_data_array(PS_Config_Lmt_LPWG_Diff_P, X_Channel, Y_Channel);
    printk("PS_Config_Lmt_LPWG_Diff_N:\n");
    nvt_print_data_array(PS_Config_Lmt_LPWG_Diff_N, X_Channel, Y_Channel);
#endif
...
#endif    // NVT_SAVE_TEST_DATA_IN_FILE
...
static int32_t nvt_read_fw_noise(int32_t *xdata)
{
    ...
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 start*/
        ...
        #endif /* #if TOUCH_KEY_NUM > 0 */
    rawdata_diff_min_offset_noise = rawdata_diff_min_offset;
...
#else
    nvt_print_data_array(RawData_FDM_Diff_Max, FDM_X_Channel, Y_Channel);
#endif    // NVT_SAVE_TEST_DATA_IN_FILE
...
#else
    nvt_print_data_array(RawData_FDM_Diff_Min, 2, Y_Channel);
#endif    // NVT_SAVE_TEST_DATA_IN_FILE
...
#else
    nvt_print_data_array(xdata, FDM_X_Channel, Y_Channel);
#endif    // NVT_SAVE_TEST_DATA_IN_FILE
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
    ...
    //---Download MP FW---
    /*Tab A9 code for AX6739A-1213 by yuli at 20230621 start*/
    if (nvt_update_firmware(ts->platdata->firmware_name_mp) < 0) {
        input_err(true, &ts->client->dev, "update mp firmware failed!\n");
        goto failed_out;
    }
...
failed_out:
    ...
    return -EAGAIN;
    /*Tab A9 code for AX6739A-1213 by yuli at 20230621 end*/
}
...
{
    static char time_data_buf[NVT_NAME_LEN] = {0};
    struct timespec64 tv64 = {0};
    struct rtc_time rtc_now_time = {0};
    ktime_get_real_ts64(&tv64);
    tv64.tv_sec -= (uint64_t)sys_tz.tz_minuteswest * TIME_COEFFICIENT;
...
/*******************************************************
Description:
    Novatek touchscreen /sys/devices/platform/tp_wake_switch/factory_check function.
...
int nvt_selftest_tp_wake_switch(void)
{
    ...
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 start*/
    gs_file_path = kzalloc(ITO_DATA_OUT_FILEPATH_LEN * sizeof(char), GFP_KERNEL);
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 end*/
    if (mutex_lock_interruptible(&ts->lock)) {
        return -ERESTARTSYS;
    }
    ...
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 start*/
    memset(gs_file_path, 0, ITO_DATA_OUT_FILEPATH_LEN * sizeof(char));
    snprintf(gs_file_path, (int)(strlen(FW_RAWDATA_CSV_FILE) + strlen(get_date_time_str())
        + strlen(TestResult_FWMutual ? "FAIL":"PASS") + strlen(ts->module_name) + FIXED_CHAR_LEN),
        "%s%s_%s_%s.csv", FW_RAWDATA_CSV_FILE, TestResult_FWMutual ? "FAIL":"PASS",
        get_date_time_str(), ts->module_name);
    input_info(true, &ts->client->dev, "%s: save rawdata to : %s\n", __func__, gs_file_path);
    #if NVT_SAVE_TEST_DATA_IN_FILE
    // Save Rawdata to CSV file
    if (nvt_save_rawdata_to_csv(RawData_FWMutual, X_Channel, Y_Channel, gs_file_path, 0) < 0) {
        input_err(true, &ts->client->dev, "%s: save rawdata to CSV file failed\n", __func__);
    }
    #endif    // NVT_SAVE_TEST_DATA_IN_FILE
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 end*/
    ...
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 start*/
    memset(gs_file_path, 0, ITO_DATA_OUT_FILEPATH_LEN * sizeof(char));
    snprintf(gs_file_path, (int)(strlen(FW_CC_CSV_FILE) + strlen(get_date_time_str())
        + strlen(TestResult_FW_CC ? "FAIL":"PASS") + strlen(ts->module_name) + FIXED_CHAR_LEN),
        "%s%s_%s_%s.csv", FW_CC_CSV_FILE, TestResult_FW_CC ? "FAIL":"PASS",
        get_date_time_str(), ts->module_name);
    input_info(true, &ts->client->dev, "%s: save CC data to : %s\n", __func__, gs_file_path);
    #if NVT_SAVE_TEST_DATA_IN_FILE
    // Save Rawdata to CSV file
    if (nvt_save_rawdata_to_csv(RawData_FW_CC, X_Channel, Y_Channel, gs_file_path, 0) < 0) {
        input_err(true, &ts->client->dev, "save CC data to CSV file failed\n");
    }
    #endif    // NVT_SAVE_TEST_DATA_IN_FILE
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 end*/
    ...
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 start*/
    memset(gs_file_path, 0, ITO_DATA_OUT_FILEPATH_LEN * sizeof(char));
    snprintf(gs_file_path, (int)(strlen(NOISE_TEST_CSV_FILE) + strlen(get_date_time_str())
        + strlen(TestResult_Noise ? "FAIL":"PASS") + strlen(ts->module_name) + FIXED_CHAR_LEN),
        "%s%s_%s_%s.csv", NOISE_TEST_CSV_FILE, TestResult_Noise ? "FAIL":"PASS",
        get_date_time_str(), ts->module_name);
    input_info(true, &ts->client->dev, "%s: save noise data to : %s\n", __func__, gs_file_path);
    #if NVT_SAVE_TEST_DATA_IN_FILE
    // Save Rawdata to CSV file
    if (nvt_save_rawdata_to_csv(RawData_Diff_Max, X_Channel, Y_Channel, gs_file_path, 0) < 0) {
        input_err(true, &ts->client->dev, "%s: save max noise data failed\n", __func__);
    }

    // Save Rawdata to CSV file
    if (nvt_save_rawdata_to_csv(RawData_Diff_Min, X_Channel, Y_Channel,
       gs_file_path, rawdata_diff_min_offset_noise) < 0) {
        input_err(true, &ts->client->dev, "%s: save min noise data failed\n", __func__);
    }
    #endif    // NVT_SAVE_TEST_DATA_IN_FILE
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 end*/
    ...
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 start*/
    memset(gs_file_path, 0, ITO_DATA_OUT_FILEPATH_LEN * sizeof(char));
    snprintf(gs_file_path, (int)(strlen(SHORT_TEST_CSV_FILE) + strlen(get_date_time_str())
        + strlen(TestResult_Short ? "FAIL":"PASS") + strlen(ts->module_name) + FIXED_CHAR_LEN),
        "%s%s_%s_%s.csv", SHORT_TEST_CSV_FILE, TestResult_Short ? "FAIL":"PASS",
        get_date_time_str(), ts->module_name);
    input_info(true, &ts->client->dev, "%s: save short test data to : %s\n", __func__, gs_file_path);
    #if NVT_SAVE_TEST_DATA_IN_FILE
    // Save Rawdata to CSV file
    if (nvt_save_rawdata_to_csv(RawData_Short, X_Channel, Y_Channel, gs_file_path, 0) < 0) {
        input_err(true, &ts->client->dev, "save short test data to CSV file failed\n");
    }
    #endif    // NVT_SAVE_TEST_DATA_IN_FILE
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 end*/
    ...
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 start*/
    memset(gs_file_path, 0, ITO_DATA_OUT_FILEPATH_LEN * sizeof(char));
    snprintf(gs_file_path, (int)(strlen(OPEN_TEST_CSV_FILE) + strlen(get_date_time_str())
        + strlen(TestResult_Open ? "FAIL":"PASS") + strlen(ts->module_name) + FIXED_CHAR_LEN),
        "%s%s_%s_%s.csv", OPEN_TEST_CSV_FILE, TestResult_Open ? "FAIL":"PASS",
        get_date_time_str(), ts->module_name);
    input_info(true, &ts->client->dev, "%s: save short test data to : %s\n", __func__, gs_file_path);
    #if NVT_SAVE_TEST_DATA_IN_FILE
    // Save Rawdata to CSV file
    if (nvt_save_rawdata_to_csv(RawData_Open, X_Channel, Y_Channel, gs_file_path, 0) < 0) {
        input_err(true, &ts->client->dev, "save open test data to CSV file failed\n");
    }
    #endif    // NVT_SAVE_TEST_DATA_IN_FILE
    /*Tab A9 code for SR-AX6739A-01-705 by yuli at 20230607 end*/
    ...
    NVT_LOG("ITO result: TestResult_Short = %d,TestResult_Open = %d,TestResult_FW_Rawdata = %d,TestResult_Noise = %d\n",
