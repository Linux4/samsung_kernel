...
#else
        ts->isUMS = false;
        input_info(true, &ts->client->dev, "UMS %s Not support\n", __func__);
#endif    // UMS_FW_UPDATE
        break;
    default:
...
static ktime_t mbist_start, mbist_end;
...
static void run_sram_test(void *device_data)
{
    ...
    input_info(true, &ts->client->dev,"SRAM test success! <%ld us>\n",
        (long) ktime_us_delta(mbist_end, mbist_start));
...
static struct sec_cmd sec_cmds[] = {
    ...
    {SEC_CMD_H("high_sensitivity_mode", high_sensitivity_mode),},
    ...
    {SEC_CMD_H("set_touch_frame_rate", set_touch_frame_rate),},
#if SHOW_NOT_SUPPORT_CMD
...
{
    // sysfs_delete_link(&ts->sec.fac_dev->kobj, &ts->input_dev->dev.kobj, "input");    //Kernel-5.10 No EXPORT_ SYMBOL(sysfs_delete_link)
