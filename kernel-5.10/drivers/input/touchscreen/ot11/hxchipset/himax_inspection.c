...
char *g_rslt_data;
bool file_w_flag;
/*Tab A9 code for AX6739A-56 by zhangxiongyi at 20230509 start*/
#define SYSTEM_TIME 1900
#define COMBINED_UNDERLINE 7
extern struct himax_ic_data *ic_data;
#if defined(HX_RW_FILE)
static char gs_file_path[256];
#endif
/*Tab A9 code for AX6739A-56 by zhangxiongyi at 20230509 end*/
...
/*Tab A9 code for AX6739A-57 by suyurui at 20230509 start*/
static int strcpy_idx(char *str, char *start, int end_idx)
/*Tab A9 code for AX6739A-57 by suyurui at 20230509 end*/
{
    int i = 0;
    int start_idx = 0;
    char *result;
...
static void hx_print_fw_info(void)
{
    ...
    /*Tab A9 code for AX6739A-190 by wenghailong at 20230522 start*/
    len += snprintf(prt_data + len, buf_size - len,
        "\nFW_VER = 0x%2.2X\n", fw_ver & 0xFF);
    /*Tab A9 code for AX6739A-190 by wenghailong at 20230522 end*/
...
static int himax_self_test_data_init(void)
{
    ...
    g_hx_criteria_item = hx_get_size_str_arr(g_himax_inspection_mode);
    ...
    g_inspt_crtra_flag = kcalloc(g_hx_criteria_size, sizeof(int), GFP_KERNEL);
    ...
    g_inspection_criteria = kcalloc(g_hx_criteria_size,
            sizeof(int *), GFP_KERNEL);
    ...
    if (g_rslt_data == NULL) {
#if defined(HX_RW_FILE)
        g_rslt_data = kcalloc(g_1kind_raw_size * g_hx_criteria_item,
...
static int himax_chip_self_test(struct seq_file *s, void *v)
{
...
#if defined(HX_ZERO_FLASH)
    /*Tab A9 code for SR-AX6739A-01-282 by wenghailong at 20230502 start*/
    ret = g_core_fp.fp_0f_op_file_dirly(private_ts->himax_mpfw_rq_name);
    /*Tab A9 code for SR-AX6739A-01-282 by wenghailong at 20230502 end*/
    if (ret) {
    ...
    #endif // HX_EXCP_RECOVERY
    /*Tab A9 code for AX6739A-181 by wenghailong at 20230522 end*/
#endif
    ...
    if (file_w_flag) {
        I("Start to write fs!\n");
        if (hx_open_file(gs_file_path) != NO_ERR) {
...
/*Tab A9 code for AX6739A-56 by zhangxiongyi at 20230509 end*/
    ...
    /*Tab A9 code for AX6739A-2410 by zhawei at 20230721 start*/
    ...
    /*Tab A9 code for AX6739A-2410 by zhawei at 20230721 end*/
...
#if defined(HX_RW_FILE)
    snprintf(gs_file_path, (int)(strlen(HX_RSLT_OUT_PATH)
            + strlen(get_date_time_str()) + strlen(test_result ? "FAIL":"PASS") + strlen(private_ts->module_name) + COMBINED_UNDERLINE),
            "%s%s_%s_%s.txt", HX_RSLT_OUT_PATH, test_result ? "FAIL":"PASS", get_date_time_str(), private_ts->module_name);
    file_w_flag = true;
