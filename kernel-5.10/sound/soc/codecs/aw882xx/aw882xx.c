...
#define AW882XX_DRIVER_VERSION "Dv1.13.0.9"
...
void aw882xx_register_i2c_error_callback(const char *suffix, void *func)
{
	int ret = -1;
	...
	if (!strcmp(suffix, "")) {
		dev_index = AW_DEV_CH_PRI_L;
	} else if (!strcmp(suffix, "_r")) {
		dev_index = AW_DEV_CH_PRI_R;
	} else {
		aw_pr_err("match dev_index failed");
		return;
	}
...
static void aw_i2c_err_report(struct aw882xx *aw882xx, int err_type)
{
	...
	if (aw882xx->aw_pa->channel == AW_DEV_CH_PRI_L) {
		suffix = "";
	} else if (aw882xx->aw_pa->channel == AW_DEV_CH_PRI_R) {
		suffix = "_r";
	} else {
		aw_dev_err(aw882xx->dev, "match pa channel failed");
		return;
	}

	if (aw882xx->aw_pa->ops.i2c_callback) {
		aw882xx->aw_pa->ops.i2c_callback((const char*)suffix);
	} else {
