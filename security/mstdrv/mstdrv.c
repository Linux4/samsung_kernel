...
#endif
#if defined(CONFIG_MST_NONSECURE)
static int mst_data;		// MST_D_P pin
static int mst_en;		// MST_D_N pin
...
static void of_mst_hw_onoff(bool on)
{
	...
	if (regulator3_0 == NULL) {
		regulator3_0 = regulator_get(NULL, "pm8150l_l7");
...
{
#if defined(CONFIG_MST_NONSECURE)
	//struct pinctrl *mst_data_pinctrl;
#endif
#if defined(CONFIG_MFC_CHARGER)
	struct device_node *np;
	enum of_gpio_flags irq_gpio_flags;
...
#endif
#if defined(CONFIG_MST_NONSECURE)
	mst_data = of_get_named_gpio(dev->of_node, "sec-mst,mst-data-gpio", OFF);
	if (mst_data < 0) {
		printk("%s : Cannot create the gpio\n", __func__);
		return 1;
	}
	mst_en = of_get_named_gpio(dev->of_node, "sec-mst,mst-en-gpio", OFF);
	if (mst_en < 0) {
		printk("%s : Cannot create the gpio\n", __func__);
		return 1;
	}

	if (gpio_request(mst_data, "sec-mst,mst-data-gpio")) {
		printk("%s : failed to get data gpio : %d\n", __func__, mst_data);
	}
	if (gpio_request(mst_en, "sec-mst,mst-en-gpio")) {
		printk("%s : failed to get en  gpio : %d\n", __func__, mst_en);
	}

	if (mst_data > 0) {
		gpio_direction_output(mst_data, OFF);
		printk("%s : Send Output\n", __func__);
	}
	if (mst_en > 0) {
		gpio_direction_output(mst_en, OFF);
		printk("%s : Send Output\n", __func__);
	}

	/* pinctrl settings */
/*
	mst_data_pinctrl = devm_pinctrl_get_select(dev, "mst_active");
	if (IS_ERR(mst_data_pinctrl)) {
		if (PTR_ERR(mst_data_pinctrl) == -EPROBE_DEFER)
			return -EPROBE_DEFER;
		pr_debug("Target does not use pinctrl\n");
		mst_data_pinctrl = NULL;
	}
*/
#endif
	return 0;
...
static int mst_ldo_device_probe(struct platform_device *pdev)
{
	...
	if (is_mst_support == 1) {
		...
		if (retval) {
			...
			device_destroy(mst_drv_class, 0);
			printk(KERN_ERR
					"[MST] %s: (support)driver initialization failed, retval : %d\n",
					__FILE__, retval);
			goto done;
		}
