...
#define MST_LDO_3P0				"MST_LEVEL_3.0V" // MST LDO regulator
...
static int sec_mst_gpio_init(struct device *dev)
{
	...
	if (ret) {
		printk(KERN_ERR "[MST] failed to get ldo en gpio : %d, %d\n", ret, mst_ldo_en);
	}
	...
	/* pinctrl settings */
	mst_ldo_en_pinctrl = devm_pinctrl_get_select(dev, "mst_active");
	if(IS_ERR(mst_ldo_en_pinctrl)) {
		if (PTR_ERR(mst_ldo_en_pinctrl) == -EPROBE_DEFER)
			return -EPROBE_DEFER;
		pr_debug("Target does not use pinctrl\n");
		mst_ldo_en_pinctrl = NULL;
	} 
	return 0;
}
