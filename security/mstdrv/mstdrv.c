...
#elif defined(CONFIG_NON_SECURE)
...
#if defined(CONFIG_PM_WAKELOCKS)
static struct wakeup_source mst_wakelock;
#else
		...
		size_t count)
{
	...
	switch (test_result[0]) {
		...
		case '2':
			...
			//Will Add here
			r0 = MTK_SIP_KERNEL_MST_TEST_TRANSMIT;
			r1 = 1;
			arm_smccc_smc(r0, r1, r2, r3, 0, 0, 0, 0, &res);
			printk(KERN_INFO "MST_LDO_DRV]]] MTK RET0:%lu\n", res.a0);
#else
			printk(KERN_INFO "%s\n", __func__);
			printk(KERN_INFO "MST_LDO_DRV]]] Track1 data transmit\n");
			//Will Add here
		...
		case '4':
			if (escape_loop) {
#if defined(CONFIG_PM_WAKELOCKS)
				wakeup_source_init(&mst_wakelock, "mst_wakelock");
				__pm_stay_awake(&mst_wakelock);
#else
		...
		case '5':
			if (!escape_loop)
#if defined(CONFIG_PM_WAKELOCKS)
				wakeup_source_trash(&mst_wakelock);
#else
