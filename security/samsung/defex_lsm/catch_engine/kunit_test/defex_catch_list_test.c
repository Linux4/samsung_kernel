...
static void get_local_syscall_test(struct test *test)
{
	...
	EXPECT_PTR_EQ(test, get_local_syscall(__NR_syscalls), (const struct local_syscall_struct *)NULL);
	...
	EXPECT_PTR_NE(test, lss, (const struct local_syscall_struct *)NULL);
	if (lss != NULL)
