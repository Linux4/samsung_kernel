...
#ifdef CONFIG_MALI_PERF_DEBUG_TRACE
TRACE_EVENT(mali_utilization,

	    TP_PROTO(unsigned int gpu, unsigned int gp, unsigned int pp),

	    TP_ARGS(gpu, gp, pp),

	    TP_STRUCT__entry(
		    __field(unsigned int, gpu)
		    __field(unsigned int, gp)
		    __field(unsigned int, pp)
	    ),
	    ...
	    TP_printk("util gpu=%d gp=%d pp=%d", __entry->gpu, __entry->gp, __entry->pp)
	   );

TRACE_EVENT(mali_freq,
