...
#if IS_ENABLED(CONFIG_MTK_PRIO_LATENCY_SENSITIVE_SUPPORT)
int sysctl_prio_sensitive = -1;
ssize_t store_sched_prio_sensitive(struct kobject *kobj, struct kobj_attribute *attr,
const char __user *buf, size_t cnt)
{
	int val;
	...
	sysctl_prio_sensitive = val;

	return cnt;
}

ssize_t show_sched_prio_sensitive(struct kobject *kobj,
struct kobj_attribute *attr, char *buf)
{
	unsigned int len = 0;
	unsigned int max_len = 4096;

	len += snprintf(buf+len, max_len-len, "%d\n", sysctl_prio_sensitive);
...
#if IS_ENABLED(CONFIG_MTK_PRIO_LATENCY_SENSITIVE_SUPPORT)
struct kobj_attribute sched_prio_sensitive =
__ATTR(sched_prio_sensitive, 0644, show_sched_prio_sensitive, store_sched_prio_sensitive);
#endif
