...
#if IS_ENABLED(CONFIG_MTK_PRIO_LATENCY_SENSITIVE_SUPPORT)
bool is_priority_boosting_task(struct task_struct *p)
{

	if (p == NULL)
		return false;

	if (sysctl_prio_sensitive != 0 && p->prio <= sysctl_prio_sensitive)
		return true;
...
#if IS_ENABLED(CONFIG_MTK_PRIO_LATENCY_SENSITIVE_SUPPORT)
	latency_sensitive = is_priority_boosting_task(p);
#endif
