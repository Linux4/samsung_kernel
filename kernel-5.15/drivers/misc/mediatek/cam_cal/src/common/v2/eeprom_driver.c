...
struct stCAM_CAL_CHECKSUM_STRUCT *checkMainData = NULL;
struct stCAM_CAL_CHECKSUM_STRUCT *checkSubData = NULL;
struct stCAM_CAL_CHECKSUM_STRUCT *checkMainMicroData = NULL;
struct stCAM_CAL_CHECKSUM_STRUCT *checkWideData = NULL;
...
void dumpEEPROMData(int u4Length,u8* pu1Params)
{
    ...
    for(i = 0; i < u4Length; i += 16){
        if(u4Length - i  >= 16){
            must_log("cam_eeprom[%d-%d]:0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x 0x%2x ",
            ...
            int j = i;
            for(;j < u4Length;j++)
            must_log("cam_eeprom[%d] = 0x%2x ",j,pu1Params[j]);
        }
    }
    must_log("cam_eeprom end\n");
}
#endif
...
int imgSensorCheckEepromData(struct stCAM_CAL_DATAINFO_STRUCT* pData, struct stCAM_CAL_CHECKSUM_STRUCT* cData){
    ...
    if((pData != NULL)&&(pData->dataBuffer != NULL)&&(cData != NULL)){
        ...
        for((count = 0);count < MAX_ITEM;count++){
            if(cData[count].item < MAX_ITEM) {
                if(buffer[cData[count].flagAdrees]!= cData[count].validFlag){
                    error_log("invalid otp data cItem=%d,flag=%d failed\n", cData[count].item,buffer[cData[count].flagAdrees]);
                    return -ENODEV;
                } else {
                    must_log("check cTtem=%d,flag=%d otp flag data successful!\n", cData[count].item,buffer[cData[count].flagAdrees]);
                }
                sum = 0;
                ...
                if(((sum%0xff)+1)!= buffer[cData[count].checksumAdress]){
                    error_log("checksum cItem=%d,0x%x,length = 0x%x failed\n",cData[count].item,sum,length);
                    return -ENODEV;
                } else {
                    must_log("checksum cItem=%d,0x%x,length = 0x%x successful!\n",cData[count].item,sum,length);
                }
            } else {
                break;
            }
        ...
        error_log("some data not inited!\n");
        return -ENODEV;
    }

    must_log("sensor[0x%x][0x%x] eeprom checksum success\n", pData->sensorID, pData->deviceID);

    return 0;
}
EXPORT_SYMBOL (imgSensorCheckEepromData);

void imgSensorReadEepromData(struct stCAM_CAL_DATAINFO_STRUCT* pData, struct stCAM_CAL_CHECKSUM_STRUCT* checkData){
    if(pData->deviceID)
        must_log("pData->deviceID = %d\n",pData->deviceID);
    if(pData->deviceID == 0x01){
        if(g_eepromMainData != NULL){
            return ;
        }
        must_log("set g_eepromMainData successed\n");
        g_eepromMainData = pData;
        checkMainData = checkData;
    }else if(pData->deviceID == 0x02){
        if(g_eepromSubData != NULL){
            return ;
        }
        must_log("set g_eepromSubData successed\n");
        g_eepromSubData= pData;
        checkSubData = checkData;
    }else if(pData->deviceID == 0x08){
        if(g_eepromMainMicroData != NULL){
            return ;
        }
        must_log("set g_eepromMainMicroData successed\n");
        g_eepromMainMicroData= pData;
        checkMainMicroData = checkData;
    }else if(pData->deviceID == 0x04){
        if(g_eepromWideData != NULL){
            return ;
        }
        must_log("set g_eepromWideData successed\n");
        g_eepromWideData= pData;
        checkWideData = checkData;
    }else{
        error_log("we don't have this devices\n");
        return ;
    }
}
EXPORT_SYMBOL (imgSensorReadEepromData);

int imgSensorReadOTPData(struct stCAM_CAL_DATAINFO_STRUCT* pData){
    int i4RetValue = 0;
    must_log("pData->deviceID = %d\n",pData->deviceID);
    if(pData->deviceID == 0x01){
        if(g_eepromMainData != NULL){
...
EXPORT_SYMBOL (imgSensorReadOTPData);
//-w3 camera bring up --otp porting
			   ...
			   size_t size, loff_t *offset)
{
    ...
    //+w3 camera bring up --otp porting
    must_log("eeprom_read SensorID=%x\n", pdata->sensor_info.sensor_id);
    must_log("eeprom_read %d,1CAM_CALIOC_G_READ start! offset=%llu, length=%lu\n",__LINE__,
       *offset, size);
    if((g_eepromMainData != NULL)&&(W3S5KJN1REARTXD_SENSOR_ID == pdata->sensor_info.sensor_id || W3HI5022QREARJK_SENSOR_ID == pdata->sensor_info.sensor_id)){
        u32 totalLength = (u32)*offset+ (u32)size;
        if(g_eepromMainData->dataBuffer == NULL)
        {
            g_eepromMainData->dataBuffer = kmalloc(g_eepromMainData->dataLength, GFP_KERNEL);
            if(read_region(pdata, g_eepromMainData->dataBuffer, 0x0, g_eepromMainData->dataLength) != g_eepromMainData->dataLength)
            {
                kfree(g_eepromMainData->dataBuffer);
                g_eepromMainData->dataBuffer = NULL;
                error_log("all eeprom data read failed\n");
                return -EFAULT;
            } else {
                must_log("all eeprom data read ok\n");
                if(imgSensorCheckEepromData(g_eepromMainData,checkMainData) != 0){
                    kfree(g_eepromMainData->dataBuffer);
                    g_eepromMainData->dataBuffer = NULL;
...
#if MAIN_OTP_DUMP
		    if(g_eepromMainData->dataBuffer)
        	dumpEEPROMData(g_eepromMainData->dataLength,g_eepromMainData->dataBuffer);
#endif
		}
        if((g_eepromMainData->dataBuffer)&&(totalLength <= g_eepromMainData->dataLength)){
            if(*offset == 1){//check id
                ...
                must_log("%d,ifCAM_CALIOC_G_READ start! offset=%llu, length=%lu\n",__LINE__,
                    *offset,size);
            } else {//read otp data
            ...
            must_log("maybe some error buf(%p)read(%d)have(%d) \n",g_eepromMainData->dataBuffer,totalLength,g_eepromMainMicroData->dataLength);
            return -EFAULT;
        }
    } else if((g_eepromMainMicroData != NULL)&&(W3SC201ACSMICROCXT_SENSOR_ID == pdata->sensor_info.sensor_id || W3GC02M3MICROLH_SENSOR_ID == pdata->sensor_info.sensor_id || W3C2599MICROSJ_SENSOR_ID == pdata->sensor_info.sensor_id)){
        u32 totalLength = (u32)*offset+ (u32)size;
		if(g_eepromMainMicroData->dataBuffer == NULL)
        {
            g_eepromMainMicroData->dataBuffer = kmalloc(g_eepromMainMicroData->dataLength, GFP_KERNEL);
            if(read_region(pdata, g_eepromMainMicroData->dataBuffer, *offset, g_eepromMainMicroData->dataLength) != g_eepromMainMicroData->dataLength)
            {
                kfree(g_eepromMainMicroData->dataBuffer);
                g_eepromMainMicroData->dataBuffer = NULL;
                error_log("all eeprom data read failed\n");
                return -EFAULT;
            } else {
                must_log("all eeprom data read ok\n");
                if(imgSensorCheckEepromData(g_eepromMainMicroData,checkMainMicroData) != 0){
                    kfree(g_eepromMainMicroData->dataBuffer);
                    g_eepromMainMicroData->dataBuffer = NULL;
                    error_log("checksum failed\n");
                    return -EFAULT;
...
#if MAIN_OTP_DUMP
            if(g_eepromMainMicroData->dataBuffer)
            dumpEEPROMData(g_eepromMainMicroData->dataLength,g_eepromMainMicroData->dataBuffer);
#endif
		}
    ...
    } else  if((g_eepromWideData != NULL)&&(W3GC5035WIDESJ_SENSOR_ID == pdata->sensor_info.sensor_id || W3SC520CSWIDELCE_SENSOR_ID == pdata->sensor_info.sensor_id)){
        u32 totalLength = (u32)*offset+ (u32)size;
		if(g_eepromWideData->dataBuffer == NULL)
        {
        g_eepromWideData->dataBuffer = kmalloc(g_eepromWideData->dataLength, GFP_KERNEL);
            if(read_region(pdata, g_eepromWideData->dataBuffer, *offset, g_eepromWideData->dataLength) != g_eepromWideData->dataLength)
            {
                kfree(g_eepromWideData->dataBuffer);
                g_eepromWideData->dataBuffer = NULL;
                error_log("all eeprom data read failed\n");
                return -EFAULT;
            } else {
                must_log("all eeprom data read ok\n");
                if(imgSensorCheckEepromData(g_eepromWideData,checkWideData) != 0){
                    kfree(g_eepromWideData->dataBuffer);
                    g_eepromWideData->dataBuffer = NULL;
                    error_log("checksum failed\n");
                    return -EFAULT;
...
#if MAIN_OTP_DUMP
            if(g_eepromWideData->dataBuffer)
            dumpEEPROMData(g_eepromWideData->dataLength,g_eepromWideData->dataBuffer);
#endif
		}
    ...
    } else if((g_eepromSubData != NULL)&&(W3OV13B10FRONTDC_SENSOR_ID == pdata->sensor_info.sensor_id || W3SC1321CSFRONTLCE_SENSOR_ID == pdata->sensor_info.sensor_id || W3SC1300CSFRONTTXD_SENSOR_ID == pdata->sensor_info.sensor_id || W3GC13A0FRONTLH_SENSOR_ID == pdata->sensor_info.sensor_id)){
        u32 totalLength = (u32)*offset+ (u32)size;
		if(g_eepromSubData->dataBuffer == NULL)
        {
            g_eepromSubData->dataBuffer = kmalloc(g_eepromSubData->dataLength, GFP_KERNEL);
            if(read_region(pdata, g_eepromSubData->dataBuffer, *offset, g_eepromSubData->dataLength) != g_eepromSubData->dataLength)
            {
                kfree(g_eepromSubData->dataBuffer);
                g_eepromSubData->dataBuffer = NULL;
                error_log("all eeprom data read failed\n");
                return -EFAULT;
            } else {
                must_log("all eeprom data read ok\n");
                if(imgSensorCheckEepromData(g_eepromSubData,checkSubData) != 0){
                    kfree(g_eepromSubData->dataBuffer);
                    g_eepromSubData->dataBuffer = NULL;
                    error_log("checksum failed\n");
                    return -EFAULT;
...
#if MAIN_OTP_DUMP
		    if(g_eepromSubData->dataBuffer)
        	dumpEEPROMData(g_eepromSubData->dataLength,g_eepromSubData->dataBuffer);
#endif
		}
        if((g_eepromSubData->dataBuffer)&&(totalLength <= g_eepromSubData->dataLength)){
            ...
            must_log("the camera sensor not have otp data sensor_id:0x%x \n", pdata->sensor_info.sensor_id);
            return -EFAULT;
    }
    //+w3 camera bring up --otp porting
	*offset += size;
