...
#include "w3c2599microsjmipiraw_Sensor.h"
...
static struct stCAM_CAL_DATAINFO_STRUCT w3c2599microsj_eeprom_data ={
	.sensorID= W3C2599MICROSJ_SENSOR_ID,
	.deviceID = 0x08,
	.dataLength = 0x0778,
...
static struct stCAM_CAL_CHECKSUM_STRUCT w3c2599microsj_checksum[6] =
{
...
static kal_uint16 g_tmp1 = 0x64;
static kal_uint16 g_tmp2 = 0x9f;

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = W3C2599MICROSJ_SENSOR_ID ,

    .checksum_value = 0xf7375923,        //checksum value for Camera Auto Test

    .pre = {
        .pclk = 86000000,            //record different mode's pclk//84
        .linelength = 2324,            //record different mode's linelength
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x3224,0x70);
     ...
     write_cmos_sensor(0x3517,0x56);
     write_cmos_sensor(0x0216,0x01);
     write_cmos_sensor(0x0202,0x04);
     write_cmos_sensor(0x0203,0xd0);

     write_cmos_sensor(0x0307,0x56);
     write_cmos_sensor(0x0342,0x09);
     write_cmos_sensor(0x0343,0x14);
     write_cmos_sensor(0x3295,0x06);
     write_cmos_sensor(0x3882,0x05);
     write_cmos_sensor(0x328b,0xc1);
     write_cmos_sensor(0x328d,0x08);
     write_cmos_sensor(0x3904,0x03);
     write_cmos_sensor(0x3219,0x52);
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            ...
            //+w3 camera bring up --otp porting
#if OTP_PORTING
            imgSensorReadEepromData(&w3c2599microsj_eeprom_data,w3c2599microsj_checksum);
            if((w3c2599microsj_eeprom_data.sensorVendorid >> 24)!= read_eeprom(0x0001)) {
                    pr_err("cwd w3c2599microsj get eeprom data failed\n");
                    if(w3c2599microsj_eeprom_data.dataBuffer != NULL) {
                        kfree(w3c2599microsj_eeprom_data.dataBuffer);
                        w3c2599microsj_eeprom_data.dataBuffer = NULL;
                    }
            }
#endif
            //-w3 camera bring up --otp porting
...
UINT32 W3C2599MICROSJ_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
