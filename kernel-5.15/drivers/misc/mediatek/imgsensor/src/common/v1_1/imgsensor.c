...
#include <linux/hardware_info.h> //+S96818AA1-1936,lijiazhen2.wt,ADD,2024/05/6, add camera module info for factory apk
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		...
		err = ERROR_NONE;
		//+S96818AA1-1936,lijiazhen2.wt,ADD,2024/05/6, add camera module info for factory apk
		if(psensor_inst->sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN){
			...
			if(!strcmp(psensor_inst->psensor_list->name, "w3s5kjn1reartxd_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID, "TXD");
			if(!strcmp(psensor_inst->psensor_list->name, "w3hi5022qrearjk_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID, "JK");
			...
			if(!strcmp(psensor_inst->psensor_list->name, "w3ov13b10frontdc_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID, "DC");
			if(!strcmp(psensor_inst->psensor_list->name, "w3sc1300csfronttxd_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID, "TXD");
			if(!strcmp(psensor_inst->psensor_list->name, "w3gc13a0frontlh_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID, "LH");
			if(!strcmp(psensor_inst->psensor_list->name, "w3sc1321csfrontlce_mipi_raw"))
			...
			if(!strcmp(psensor_inst->psensor_list->name, "w3gc5035widesj_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_WIDE_ANGLE_CAM_MOUDULE_ID, "SJ");
			if(!strcmp(psensor_inst->psensor_list->name, "w3gc5035widesj_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_BACK_SUBCAM_MOUDULE_ID, "SJ");
			if(!strcmp(psensor_inst->psensor_list->name, "w3sc520cswidelce_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_WIDE_ANGLE_CAM_MOUDULE_ID, "JK");
			if(!strcmp(psensor_inst->psensor_list->name, "w3sc520cswidelce_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_BACK_SUBCAM_MOUDULE_ID, "JK");
		}else if(psensor_inst->sensor_idx == IMGSENSOR_SENSOR_IDX_SUB2){
			hardwareinfo_set_prop(HARDWARE_MACRO_CAM, psensor_inst->psensor_list->name);
			if(!strcmp(psensor_inst->psensor_list->name, "w3sc201acsmicrocxt_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_MACRO_CAM_MOUDULE_ID, "CXT");
			if(!strcmp(psensor_inst->psensor_list->name, "w3gc02m3microlh_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_MACRO_CAM_MOUDULE_ID, "LH");
			if(!strcmp(psensor_inst->psensor_list->name, "w3c2599microsj_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_MACRO_CAM_MOUDULE_ID, "SJ");
		}
		//-S96818AA1-1936,lijiazhen2.wt,ADD,2024/05/6, add camera module info for factory apk
	}
