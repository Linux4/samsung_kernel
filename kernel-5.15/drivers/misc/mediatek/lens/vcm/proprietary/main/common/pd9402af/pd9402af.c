...
/*
 * PD9402AF voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "PD9402AF_DRV"
...
static int s4AF_WriteReg(u8 a_uLength, u8 a_uAddr, u16 a_u2Data)
{
    ...
    if (a_uLength == 0) {
        if (i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2) < 0) {
			LOG_INF("WriteI2C cmd failed\n");
			return -1;
		}
    } else if (a_uLength == 1) {
		if (i2c_master_send(g_pstAF_I2Cclient, puSendCmd2, 3) < 0) {
			LOG_INF("WriteI2C cmd2 failed\n");
			return -1;
		}
	}
    return 0;
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (setPosition((unsigned short) a_u4Position) == 0) {
		...
		LOG_INF("set I2C failed when move\n");
		ret = -1;
	}
	return ret;
}
...
/* ////////////////////////////////////////////////////////////// */
long PD9402AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
int PD9402AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");

	if (*g_pAF_Opened == 2) {
		LOG_INF("Wait\n");
        pr_info("PD9402A feee \n");

		s4AF_WriteReg(1, 0x03, 0x0138);
		mdelay(15);
		s4AF_WriteReg(1, 0x03, 0x019c);
		mdelay(15);
		s4AF_WriteReg(1, 0x03, 0x0200);
		mdelay(15);
	}
...
int PD9402AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int PD9402AF_GetFileName(unsigned char *pFileName)
{
