...
static int ISP_WriteReg(struct ISP_REG_IO_STRUCT *pRegIo)
{
	...
	pData = vmalloc((pRegIo->Count) * sizeof(struct ISP_REG_STRUCT));
...
static int ISP_open(struct inode *pInode, struct file *pFile)
{
	...
	pFile->private_data =
		vmalloc(sizeof(struct ISP_USER_INFO_STRUCT));
...
/* kernel log limit to (current+150) lines per second */
	...
	IspInfo.BufInfo.Read.pData = vmalloc(ISP_BUF_SIZE);
...
EXIT:
	if (Ret < 0) {
		if (IspInfo.BufInfo.Read.pData != NULL) {
			vfree(IspInfo.BufInfo.Read.pData);
