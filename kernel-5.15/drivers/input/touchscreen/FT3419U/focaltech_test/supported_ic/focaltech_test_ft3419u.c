...
static int ft5452_noise_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    u8 fir = 0;
    int max = 0;
    int *noise_rawdata = NULL;
    u8 reg06_val = 0;
    u8 reg0d_val = 0;
    u8 rawdata_addr = 0;
    bool result = false;
    ...
    for (i = 0; i < 3; i++) {
        ...
        byte_num = node_num * 2;
       // ret = read_rawdata(tdata,FACTORY_REG_LINE_ADDR, 0xAA, rawdata_addr, byte_num,noise_rawdata);
    ...
    if (tdata->func->raw_u16)
        ret = read_mass_data_u16(rawdata_addr, byte_num, noise_rawdata);
    else
        ret = read_mass_data(rawdata_addr, byte_num, noise_rawdata);
    if (ret < 0) {
    ...
    /* show test data */
    show_data(noise_rawdata, false);

    /* compare */
     max = reg0d_val * 4 * thr->basic.noise_max / 100;
    FTS_TEST_INFO("reg0d:%d, max:%d", (int)reg0d_val, max);
    result = compare_data(noise_rawdata, 0, max, 0, 0, false);
...
test_err:
    ...
    fts_test_save_data("Noise Test", CODE_M_NOISE_TEST,
                       noise_rawdata, 0, false, false, *test_result);
...
static int start_test_ft5452(void)
{
    ...
    if (true == test_item->noise_test) {
        ret = ft5452_noise_test(tdata, &temp_result);
...
static int param_init_ft5452(void)
{
    struct mc_sc_threshold *thr = &fts_ftest->ic.mc_sc.thr;
