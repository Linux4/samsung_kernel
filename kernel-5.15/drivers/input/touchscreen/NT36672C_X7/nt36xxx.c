...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	count = of_property_count_strings(np, "novatek,firmware-name");
	if (count <= 0) {
		update_fw_name = BOOT_UPDATE_FIRMWARE_NAME;
		NVT_ERR("novatek, firmware name is NOT defined !!!\n");
	} else {
		of_property_read_string(np, "novatek,firmware-name", &update_fw_name);
		NVT_ERR("novatek, firmware name = %s\n", update_fw_name);
	}
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	gpio_set_value(ts->reset_gpio, 1);
#endif
	if (nvt_update_firmware((char *)update_fw_name)) {
