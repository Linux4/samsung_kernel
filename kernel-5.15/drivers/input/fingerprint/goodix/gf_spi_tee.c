...
{
	int rc = 0;
	gf_debug(INFO_LOG, "[%s] : enter  onoff = %d\n", __func__, onoff);

	if(NULL == gf_dev->vdd) {
		gf_dev->vdd = regulator_get(&gf_dev->spi->dev,"fpvdd");
	...
	/* TODO:  set power  according to actual situation  */
		...
		if(NULL != gf_dev->vdd) {
			...
			if(regulator_count_voltages(gf_dev->vdd)>0) {
				...
				gf_debug(INFO_LOG, "[%s] : regulator_set_voltage rc=%d\n", __func__, rc);
				if(rc) {
			...
			gf_debug(INFO_LOG, "[%s] : regulator_enable rc=%d\n", __func__, rc);
		}
#endif
        gf_debug(INFO_LOG, "[%s] : rc=%d\n", __func__, rc);
        if (rc) {
	...
	} else if (!onoff && enable_status) {
		enable_status = 0;
		gf_debug(INFO_LOG, "[%s] : start power off\n", __func__);
#ifdef GF_POWER_GPIO
		...
		if(NULL != gf_dev->vdd) {
			...
			pr_info("---- regulator_set_load ---- rc = %d\n",rc);
			if(rc<0) {
				pr_err("---- unable to regulator_set_load 0 ----\n");
			}
			if(regulator_is_enabled(gf_dev->vdd)) {
				regulator_disable(gf_dev->vdd);
				pr_info("---- already regulator_disable  ----\n");
			}else{
				pr_info("---- have no regulator_disable  ----\n");
			}
		}else{
			pr_info("---- gf_dev->vdd is null  ----\n");
		}
#endif
        gf_debug(INFO_LOG, "[%s] : rc=%d\n", __func__, rc);
        if (rc) {
...
{
	gf_debug(INFO_LOG, "%s gf_spi_probe enter\n", __func__);
	goodix_dev.spi = spi;
	gf_hw_power_enable(&goodix_dev, 1);
	gf_debug(INFO_LOG, "%s gf_spi_probe exit\n", __func__);
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	case GF_IOC_DISABLE_POWER:
		...
		gf_hw_power_enable(&goodix_dev, 0);
		break;
