...
static atomic64_t *get_total_bytes_ptr(struct dma_heap *heap)
{
	...
	else if (heap == sys_uncached_heap)
		return &sys_uncached_heap_total_bytes;
	else if (heap == mtk_mm_heap)
...
static long get_dma_heap_total_kbytes(struct dma_heap *heap)
{
	...
	return atomic64_read(total_bytes_ptr) >> 10;
}

static void dma_heap_show_mem(void *data, unsigned int filter, nodemask_t *nodemask)
{
	struct dma_heap *heap = (struct dma_heap *)data;
	long total_kbytes = get_dma_heap_total_kbytes(heap);

	pr_info("%s: %ld kB\n", dma_heap_get_name(heap), total_kbytes);
}

static void dma_heap_meminfo_proc_show(void *data, struct seq_file *m)
{
	struct dma_heap *heap = (struct dma_heap *)data;
	long total_kbytes = get_dma_heap_total_kbytes(heap);
...
static int system_heap_create(void)
{
	...
	register_trace_android_vh_show_mem(dma_heap_show_mem, (void *)sys_heap);
	register_trace_android_vh_meminfo_proc_show(dma_heap_meminfo_proc_show, (void *)sys_heap);
	...
	register_trace_android_vh_show_mem(dma_heap_show_mem, (void *)mtk_mm_heap);
	register_trace_android_vh_meminfo_proc_show(dma_heap_meminfo_proc_show, (void *)mtk_mm_heap);
	pr_info("%s add heap[%s] success\n", __func__, exp_info.name);
	...
	register_trace_android_vh_show_mem(dma_heap_show_mem, (void *)sys_uncached_heap);
	register_trace_android_vh_meminfo_proc_show(dma_heap_meminfo_proc_show, (void *)sys_uncached_heap);
	pr_info("%s add heap[%s] success\n", __func__, exp_info.name);
	...
	register_trace_android_vh_show_mem(dma_heap_show_mem, (void *)mtk_mm_uncached_heap);
	register_trace_android_vh_meminfo_proc_show(dma_heap_meminfo_proc_show, (void *)mtk_mm_uncached_heap);
	pr_info("%s add heap[%s] success\n", __func__, exp_info.name);
	return 0;
}
