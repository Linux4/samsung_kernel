...
static void mtk_system_heap_show_mem(void *data, unsigned int filter, nodemask_t *nodemask)
{
	...
	size_byte = atomic64_read(&sys_heap_total_bytes) +
...
static void mtk_system_heap_meminfo(void *data, struct seq_file *m)
{
	...
	show_val_meminfo(m, "system", size_byte >> 10);
}

static void mtk_system_uncached_heap_show_mem(void *data, unsigned int filter, nodemask_t *nodemask)
{
	long size_byte;

	size_byte = atomic64_read(&sys_uncached_heap_total_bytes) +
			atomic64_read(&mtk_mm_uncached_heap_total_bytes);

	pr_info("%s: %ld kB\n", "system-uncached", size_byte >> 10);
}

static void mtk_system_uncached_heap_meminfo(void *data, struct seq_file *m)
{
	long size_byte;
	...
	show_val_meminfo(m, "system-uncached", size_byte >> 10);
}
...
static int system_heap_create(void)
{
	...
	register_trace_android_vh_show_mem(mtk_system_uncached_heap_show_mem, NULL);
	register_trace_android_vh_meminfo_proc_show(mtk_system_uncached_heap_meminfo, NULL);
	return 0;
}
