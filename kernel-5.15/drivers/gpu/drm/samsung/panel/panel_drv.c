...
//bug EXTBP240824-00803,gaobowei.wt,ADD,20240902,cdmq timeout issue
...
extern unsigned char a16_lcm_sn[30];
...
module_param(lcm_id, int, S_IRUGO);

int get_lcm_id(void)
{
	panel_info("%s, lcm_id: %d\n", __func__, lcm_id);
	return lcm_id;
}
...
static int panel_set_node_from_panel_lut(struct panel_device *panel)
{
	...
	panel_err("panel_set_node_from_panel_lut start panel_id=%d\n", panel_id);
...
int panel_probe(struct panel_device *panel)
{
	...
	panel_info("lcm_id = %d, run_count = %d\n", lcm_id, run_count);
	if((lcm_id == 0) && (run_count == 1))
	{
		return 0;
	}
	run_count = 1;
	...
	panel->cpi = info;
	panel_resource_copy(panel, a16_lcm_sn, "date");
	panel_resource_copy(panel, a16_lcm_sn+7, "coordinate");

	snprintf(a16_lcm_sn, 23, "%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X", a16_lcm_sn[0], a16_lcm_sn[1], a16_lcm_sn[2], a16_lcm_sn[3], a16_lcm_sn[4], a16_lcm_sn[5], a16_lcm_sn[6],a16_lcm_sn[7], a16_lcm_sn[8], a16_lcm_sn[9], a16_lcm_sn[10]);
	panel_err("a16_lcm_sn : %s\n", a16_lcm_sn);
	panel_info("-\n");
	//bug EXTBP240824-00803,gaobowei.wt,ADD,20240902,cdmq timeout issue
	probe_flag = 1;
	return 0;
...
int panel_parse_dt(struct panel_device *panel)
{
	...
	panel_err("panel_parse_dt start\n");
...
int panel_device_init(struct panel_device *panel)
{
	...
	if (!panel)
		return -EINVAL;
	panel_err("panel_device_init start\n");
