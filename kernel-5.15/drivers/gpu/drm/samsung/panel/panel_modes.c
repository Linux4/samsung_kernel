		...
		struct panel_display_mode *pdm)
{
	...
	pr_err("of_parse_panel_display_mode start\n");
	...
	pr_err("of_parse_panel_display_mode refresh_rate=%d, lcm_id = %d\n", pdm->refresh_rate, lcm_id);
	if(1 ==lcm_id)
	{
		if(pdm->refresh_rate == 60) {
			pdm->panel_vporch[0] = 14;
			pdm->panel_vporch[1] = 1244;
			pdm->panel_vporch[2] = 2;
			pdm->dsc_en = 0;
			pdm->dsc_slice_h = 40;
		} else {
			pdm->panel_vporch[0] = 14;
			pdm->panel_vporch[1] = 44;
			pdm->panel_vporch[2] = 2;
			pdm->dsc_en = 0;
			pdm->dsc_slice_h = 40;
		}
...
struct panel_display_modes *of_get_panel_display_modes(const struct device_node *np)
{
	...
	pr_err("%pOF: using %s as default panel mode\n", np, entry->name);
