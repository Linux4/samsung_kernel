...
struct lcm {
	...
	struct gpio_desc *pcd_gpio;
	struct gpio_desc *err_fg_gpio;
...
static int hbmmode = 0;
static int MaxBrightness = 2047;
static int hbmstatus = 0x20;

static ssize_t hbm_mode_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	printk("hbm_mode_show ok\n");
	return sprintf(buf, "%d\n", hbmmode);
}

static ssize_t hbm_mode_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t size)
{
	int val = 0;
	int ret = 0;
	printk("hbm_mode_store ok\n");
...
static DEVICE_ATTR(hbmmode, 0664, hbm_mode_show, hbm_mode_store);
...
static struct LCM_setting_table init_setting_cmd[] = {
	...
	/* 4.1.4 FFC SET, 83.5Mbps */
	{3, {0xF0, 0x5A, 0x5A}},
	{8, {0xDF, 0x09, 0x30, 0x95, 0x57, 0x0B, 0x57, 0x0B}},
	{3, {0xF0, 0xA5, 0xA5}},

	/* 4.1.5 TSP Vsync Setting */
	{3, {0xF0, 0x5A, 0x5A}},
	...
	{18,{0xB4, 0x44, 0x00, 0xB0, 0x57, 0x66, 0xA1, 0x15, 0x55,
	...
	/* 4.1.8 Smooth Dimming Setting */
	{3, {0xF0, 0x5A, 0x5A}},
	{4, {0xB0, 0x00, 0x0D, 0xB2}},
	{2, {0xB2, 0x0F}},
	{2, {0xf7, 0x0B}},
	{3, {0xF0, 0xA5, 0xA5}},
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	if (IS_ERR(ctx->vci_en_gpio)) {
		dev_info(ctx->dev, "%s: cannot get vci_en_gpio %ld\n",
			__func__, PTR_ERR(ctx->vci_en_gpio));
		return 0;
	}
	...
	//vddr disable
	ctx->vddr_en_gpio = devm_gpiod_get(ctx->dev, "vddr-en", GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->vddr_en_gpio)) {
		dev_info(ctx->dev, "%s: cannot get vddr_en_gpio %ld\n",
			__func__, PTR_ERR(ctx->vddr_en_gpio));
		return 0;
	}
	gpiod_set_value(ctx->vddr_en_gpio, 0);
	devm_gpiod_put(ctx->dev, ctx->vddr_en_gpio);
	mdelay(1);
...
static struct drm_display_mode performance_mode = {
	.clock = 277401,// Pixel Clock = vtotal * htotal * vrefresh(60) / 1000;
	.hdisplay = HAC,
	.hsync_start = HAC + HFP,
...
static struct drm_display_mode default_mode = {
	.clock = 277401,// Pixel Clock = vtotal * htotal * vrefresh(90) / 1000;
	.hdisplay = HAC,
	.hsync_start = HAC + HFP,
	.hsync_end = HAC + HFP + HSA,
	.htotal = HAC + HFP + HSA + HBP,
	...
	void *handle, unsigned int level)
{
	...
	mapping_level = level * MaxBrightness / 255;
	bl_tb0[1] = ((mapping_level >> 8) & 0xf);
	bl_tb0[2] = (mapping_level & 0xff);
	dimming_speed[1] = hbmstatus;

	pr_info("[%s][s6e8fc3] bl_tb0[0] = 0x%x, bl_tb0[1] = 0x%x, level = %d(0x%x), hbmstatus = 0x%x\n",
		__func__, bl_tb0[1], bl_tb0[2], level, mapping_level, hbmstatus);
...
static struct mtk_panel_params ext_params = {
	...
	.dsc_params = {
		...
		.dec_delay = 526,
		.scale_value = 32,
		.increment_interval = 1114,
		.decrement_interval = 7,
		.line_bpg_offset = 12,
		.nfl_bpg_offset = 559,
		.slice_bpg_offset = 579,
		.initial_offset = 6144,
		.final_offset = 4336,
		.flatness_minqp = 3,
		.flatness_maxqp = 12,
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	device_create_file(dev,&dev_attr_hbmmode);
	...
	ctx->pcd_gpio  = devm_gpiod_get(dev, "pcd", GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->pcd_gpio)) {
		dev_info(dev, "cannot get pcd_gpios %ld\n",
			PTR_ERR(ctx->pcd_gpio));
		return PTR_ERR(ctx->pcd_gpio);
	}
	devm_gpiod_put(ctx->dev, ctx->pcd_gpio);
	gpiod_set_value(ctx->pcd_gpio, 0);

	ctx->err_fg_gpio = devm_gpiod_get(dev, "err-fg", GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->err_fg_gpio)) {
		dev_info(dev, "cannot get err_fg_gpios %ld\n",
			PTR_ERR(ctx->err_fg_gpio));
		return PTR_ERR(ctx->err_fg_gpio);
	}
	devm_gpiod_put(dev, ctx->err_fg_gpio);
	gpiod_set_value(ctx->err_fg_gpio, 0);
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel-s6e8fc3-ams645fw01-vdo-dsc",
		.owner = THIS_MODULE,
