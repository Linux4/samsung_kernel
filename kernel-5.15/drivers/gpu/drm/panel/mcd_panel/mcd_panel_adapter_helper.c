...
#if IS_ENABLED(CONFIG_MTK_ROUND_CORNER_SUPPORT)
				...
				struct mtk_panel *ctx)
{
	...
	pr_err("exynos_display_mode_get_dsc_setting_from_pps_table dsc_en=%d\n", pdm->dsc_en);
...
#if IS_ENABLED(CONFIG_MTK_ROUND_CORNER_SUPPORT)
	ext->round_corner_en = 1,
	ext->corner_pattern_height = ROUND_CORNER_H_TOP,
	ext->corner_pattern_height_bot = ROUND_CORNER_H_BOT,
	ext->corner_pattern_tp_size = sizeof(top_rc_pattern),
	ext->corner_pattern_lt_addr = (void *)top_rc_pattern,
	dev_info(ctx->dev, "%s: ext->round_corner_en : %d\n", __func__, ext->round_corner_en);
#endif
