...
static void sec_dump_provider_clk(struct provider_clk *pvdck)
{
	struct clk *c = pvdck->ck;
	struct clk *p = IS_ERR_OR_NULL(c) ? NULL : clk_get_parent(c);
	struct clk_hw *c_hw = __clk_get_hw(c);
	...
	pr_info("[%10s: %-21s: %3s, %3d, %3d, %10ld, %21s]\n",
		pvdck->provider_name != NULL ? pvdck->provider_name : "/ ",
		clk_hw_get_name(c_hw),
		clkdbg_pvdck_is_on(pvdck) > 0 ? "ON" : "OFF",
		clkchk_pvdck_is_prepared(pvdck),
...
void sec_clkdbg_dump_provider_clks(void)
{
	struct provider_clk *pvdck = get_all_provider_clks();
	int count = 0;
	pr_info("Enabled clocks:\n");
	pr_info("	clock                       ON/OFF     prepare_cnt     enabled_cnt     rate          p_clock\n");
	pr_info("---------------------------------------------------------------------------------------------------\n");

	for (; pvdck->ck != NULL; pvdck++)
		if (clkchk_pvdck_is_enabled(pvdck)) {
			sec_dump_provider_clk(pvdck);
			count++;
		}

	pr_info("Enabled clock count: %d\n", count);
}
EXPORT_SYMBOL(sec_clkdbg_dump_provider_clks);
#endif
