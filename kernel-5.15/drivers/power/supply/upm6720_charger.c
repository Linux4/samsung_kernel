...
//S98901AA1-4519, liwei19.wt, add, 20240509, enable MOS switch when in otg mode
...
static int upm6720_set_busocp_th(struct upm6720 *upm, chg_mode_t mode, int threshold)
{
	...
	if (mode == CHG_SW_CAP_MODE) {
		...
		val = (UPM6720_SW_CAP_BUS_OCP_MAX - UPM6720_SW_CAP_BUS_OCP_BASE - UPM6720_SW_CAP_BUS_OCP_OFFSET) / UPM6720_SW_CAP_BUS_OCP_LSB;
	} else if (mode == CHG_BYPASS_MODE) {
...
void upm6720_shipmode_enable_adc(bool enable)
{
	if (!otg_upm)
		return;

	upm6720_enable_adc(otg_upm, enable);
}
EXPORT_SYMBOL(upm6720_shipmode_enable_adc);
...
static int upm6720_get_adc_data(struct upm6720 *upm, adc_channel_t channel,  int *result)
{
	...
	switch (channel) {
		case UPM_ADC_IBUS:	// !!!only switched cap mode, bypass mode will do later
			...
			if(sval > 2800 && ((sval - upm->pre_ibus) >= 800)) {
				upm_info("IBUS err sval:%d, pre_ibus:%d\n", sval, upm->pre_ibus);
				sval = upm->pre_ibus;
			}
			upm->pre_ibus = sval;
			break;
...
static int upm6720_detect_device(struct upm6720 *upm)
{
	...
	if (ret) {
		upm_err("%s:Failed to detect_device!!! ret=%d\n", __func__, ret);
		return ret;
	} else {
		upm->part_no = (data & UPM6720_DEVICE_ID_MASK) >> UPM6720_DEVICE_ID_SHIFT;
		if(!upm->part_no) {
			ret = __upm6720_write_byte(upm, UPM6720_REG_BE, UPM6720_DEVICE_ENTER);
			if (ret) {
				upm_err("Failed: write UPM6720_REG_BE, ret=%d\n", ret);
				return ret;
			}
			ret = upm6720_read_byte(upm, UPM6720_REG_D3, &data);
			if (ret) {
				upm_err("Failed: read UPM6720_REG_D3, ret=%d\n", ret);
				return ret;
			}
			upm->part_no = (data & UPM6720_DEVICE_DETECT_MASK) >> UPM6720_DEVICE_DETECT_SHIFT;
			ret = __upm6720_write_byte(upm, UPM6720_REG_BE, UPM6720_DEVICE_EXIT);
			if (ret) {
				upm_err("Failed: write UPM6720_REG_BE, ret=%d\n", ret);
				return ret;
			}

			/* part_no: 1 6722;0 6720 */
			if (upm->part_no) {
				upm_info("%s: upm6722 found!!!\n", __func__);
			} else {
				upm_info("%s: upm6720 found!!!\n", __func__);
			}
		} else {
			upm_err("%s: upm->part_no=%d\n", __func__, upm->part_no);
		}
	}
...
static int mtk_upm6720_is_chg_enabled(struct charger_device *chg_dev, bool *en)
{
    ...
    int ret;
    upm_info("mtk_upm6720_is_chg_enabled en=%d\n", *en);
...
static int mtk_upm6720_enable_chg(struct charger_device *chg_dev, bool en)
{
    struct upm6720 *upm = charger_get_data(chg_dev);
    int ret;
    upm_info("mtk_upm6720_enable_chg en=%d\n", en);
...
static int mtk_upm6720_set_vbusovp(struct charger_device *chg_dev, u32 uV)
{
    ...
    return upm6720_set_busovp_th(upm, CHG_SW_CAP_MODE, mv);
}
...
static int mtk_upm6720_set_ibusocp(struct charger_device *chg_dev, u32 uA)
{
    ...
    int ma;
    ma = uA / 1000;
    upm_info("mtk_upm6720_set_ibusocp: ma=%d\n", ma);

    return upm6720_set_busocp_th(upm, CHG_SW_CAP_MODE, ma);
}
...
static int mtk_upm6720_set_vbusovp_alarm(struct charger_device *chg_dev, u32 uV)
{
    ...
    ret = upm6720_set_bus_ovp_alm_th(upm, CHG_SW_CAP_MODE, uV/1000);
    if (ret < 0)
        return ret;
...
int upm6720_set_otg_txmode(bool enable)
{
	...
	if (!otg_upm)
		return 0;
	if (IS_ERR(otg_upm->otg_rdev))
		return 0;

	if (enable) {
		ret = upm6720_otg_enable(otg_upm->otg_rdev);
	} else {
		ret = upm6720_otg_disable(otg_upm->otg_rdev);
	}

	pr_err("upm6720_set_otg_txmode = %d\n", enable);
	return ret;
}
                    ...
                    const struct i2c_device_id *id)
{
    ...
    if (upm->part_no)
        hardwareinfo_set_prop(HARDWARE_CHARGER_PUMP, "UPM6722");
    else
