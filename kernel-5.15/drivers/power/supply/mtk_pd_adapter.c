...
//				MTK_PD_CONNECT_NONE, NULL);
			...
			pr_info("%s: PD_CONNECT_NONE : pdc_clear()\n", __func__);
			//pdc_clear();
#endif
...
//				MTK_PD_CONNECT_HARD_RESET, NULL);
#if IS_ENABLED(CONFIG_BATTERY_SAMSUNG) && IS_ENABLED(CONFIG_PDIC_NOTIFIER)
			pr_info("%s: PD_CONNECT_HARD_RESET : pdc_hard_rst()\n", __func__);
			//pdc_hard_rst();
#endif
			break;
...
static bool pd_is_src_usb_communication_capable(struct adapter_device *dev)
{
	...
	return tcpm_inquire_usb_comm(info->tcpc);
}
#endif
	...
	struct adapter_power_cap *tacap)
{
	...
	} else if (type == MTK_PD || type == MTK_PD_VPDO) {
		pd_cap.nr = 0;
		...
		if (pd_cap.nr != 0) {
			...
			for (i = 0; i < pd_cap.nr; i++) {
				...
				if (pd_cap.type[i] == 0)
					...
					tacap->type[j] = MTK_PD_VPDO;
				else if (pd_cap.type[i] == 3)
					tacap->type[j] = MTK_PD_APDO;
				else
					tacap->type[j] = MTK_CAP_TYPE_UNKNOWN;
#else
				tacap->type[j] = MTK_PD;
#endif
				...
				pr_notice("[%s]:%d mv:[%d,%d] mA:%d type:%d %d\n",
					__func__, i, pd_cap.min_mv[i], pd_cap.max_mv[i],
					pd_cap.ma[i], pd_cap.type[i], pd_cap.type[i]);
			}
