...
static void pd30_set_current_config(struct chg_alg_device *alg)
{
	struct mtk_pd *pd = dev_get_drvdata(&alg->dev);
	int type;

	type = pd_hal_get_pd_adapter_type(alg);
	pd_err("%s set:%d,%d,%d,%d,%d,%d\n", __func__, pd->input_current_limit1, pd->charging_current_limit1,
		pd->input_current_limit2, pd->charging_current_limit2, pd->ref_vbat, type);

	if(pd->state == PD_HW_READY && type == MTK_PD_CONNECT_PE_READY_SNK_PD30) {
		if (pd->input_current_limit1 != -1)
			pd->input_current_limit1 = -1;
		if (pd->charging_current_limit1 != -1)
			pd->charging_current_limit1 = -1;
		if (pd->input_current_limit2 != -1)
...
int __mtk_pdc_setup(struct chg_alg_device *alg, int idx)
{
	...
	if (pd->pd_idx != idx || force_update) {
		...
		if (ret == 0) {
			...
			if (oldmA < pd->cap.ma[idx]) {
				if (pd->sc_input_current > (pd->cap.ma[idx] * 1000)) {
					pd_hal_set_input_current(alg, CHG1,
						pd->cap.ma[idx] * 1000);
				} else {
					pd_hal_set_input_current(alg, CHG1,
...
static int pd_sc_set_charger(struct chg_alg_device *alg)
{
	...
	pd_dbg("%s old_cv=%d, new_cv=%d, pd_6pin_en=%d 6pin_re_en=%d, charging_current_limit1=%d\n", __func__,
		pd->old_cv, pd->cv, pd->pd_6pin_en, pd->stop_6pin_re_en, pd->charging_current_limit1);
