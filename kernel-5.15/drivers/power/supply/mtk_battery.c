...
static int vsys_to_vsoc_for_factory_mode(void)
{
	...
	int vsys = 0;
	int index_value, vsoc = 0;
	int ret;

	gm = get_mtk_battery();
	bs_data = &gm->bs_data;
	if (IS_ERR_OR_NULL(bs_data->chg_psy)) {
		bs_data->chg_psy = devm_power_supply_get_by_phandle(&gm->gauge->pdev->dev, "charger");
		bm_err("%s try to get chg_psy\n", __func__);
	}

	if (IS_ERR_OR_NULL(bs_data->chg_psy)) {
		vsoc = 100;
		bm_err("%s Couldn't get chg_psy, vsoc fixed %d%\n", __func__, vsoc);
		return vsoc;
	} else {
		ret = power_supply_get_property(bs_data->chg_psy, POWER_SUPPLY_PROP_VOLTAGE_MIN, &val);
		if (ret < 0) {
			vsoc = 100;
			bm_err("%s get vsys property fail, vsoc fixed %d%\n", __func__, vsoc);
			return vsoc;
		} else
			vsys = val.intval / 1000;
	}

	for (index_value = 0; index_value < 100; index_value++) {
		if((gm->fg_table_cust_data.fg_profile[1].fg_profile[index_value].voltage/10) < vsys) {
			vsoc =  100 - index_value;
			pr_info("%s vsys: %d, VSOC[%d] : %d\n", __func__,
				vsys, vsoc, gm->fg_table_cust_data.fg_profile[1].fg_profile[index_value].voltage/10);
			break;
		}
	}
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		...
		if (gm->nouse_baton_undet == true) {
			val->intval = 1;
			bm_debug("[%s] nouse_baton_undet force return true\n", __func__);
		}
...
#if IS_ENABLED(CONFIG_BATTERY_SAMSUNG)
			if (val->intval == SEC_BATTERY_VOLTAGE_MV)
				val->intval = bs_data->bat_batt_vol;
#else
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		...
		case SEC_BATTERY_CAPACITY_FULL:
			val->intval = gm->fg_table_cust_data.fg_profile[gm->battery_id].q_max;
			pr_info("[%s] gm->fg_table_cust_data.fg_profile[gm->battery_id].q_max %d\n",
				__func__, gm->fg_table_cust_data.fg_profile[gm->battery_id].q_max);
			break;
		}
		break;
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_PMIC_BAT_VOLTAGE:
		{
			...
			for (j = 0; j < 10; j++) {
				/* There is no OCV value which can read after booting complete. */
				/* There is only OCV value which was saved in booting time. */
				/* ====>  gauge_get_int_property(GAUGE_PROP_BOOT_ZCV) */
				ret = gauge_get_property_control(gm, GAUGE_PROP_BATTERY_VOLTAGE,
				&bs_data->bat_batt_vol, 1);
				...
				if (ret == -EHOSTDOWN)
					...
					ocv_data[j] = bs_data->bat_batt_vol;
			}
			for (j = 1; j < 10; j++) {
				ocv = ocv_data[j];
				k = j;
		...
		case POWER_SUPPLY_EXT_PROP_BATT_DUMP:
			memset(gm->d_buf, 0x0, sizeof(gm->d_buf));
			/* capacity_max not supported, set to 0 */
			snprintf(gm->d_buf, sizeof(gm->d_buf), "%d,%d,%d", gm->vbat * 1000, gm->ui_soc * 10, 0);
			val->strval = gm->d_buf;
			break;
		default:
			return -EINVAL;
	...
	const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_BATT_F_MODE:
			gm->f_mode = val->intval;
			pr_info("%s: mtk-fg-battery: FG f_mode: %s\n", __func__,
					BOOT_MODE_STRING[gm->f_mode]);
			if (gm->f_mode == OB_MODE) {
				disable_fg(gm);
			}
			break;
...
#if IS_ENABLED(CONFIG_SEC_FACTORY) //only factory bin [ALPS07401467/ALPS07762232]
	bm_err("%s oldocv diff thr set 0 in only factory bin \n", __func__);
...
void battery_update(struct mtk_battery *gm)
{
	...
	if (gm->f_mode == OB_MODE) {
		if (gm->disableGM30) {
			bat_data->bat_capacity = vsys_to_vsoc_for_factory_mode();
			bm_err("[%s] bat_capacity =%d\n", __func__, bat_data->bat_capacity);
		}
	}
#endif
...
#if IS_ENABLED(CONFIG_BATTERY_SAMSUNG)
	psy = power_supply_get_by_name("battery"); /*sec_battery.c */
	if (!psy) {
		...
		if (gm->is_fake_soc) {
			if (bat_data->bat_capacity  >= 0) {
				...
				value.intval = bat_data->bat_capacity;
