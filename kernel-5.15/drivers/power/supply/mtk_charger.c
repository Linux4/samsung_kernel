				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "min_charger_voltage", &val) >= 0) {
		...
		chr_err("use min_charger_voltage: %d\n", info->data.min_charger_voltage);
	} else {
	...
	if (of_property_read_u32(np, "min_charger_voltage_2", &val) >= 0) {
		...
		chr_err("use min_charger_voltage_2: %d\n", info->data.min_charger_voltage_2);
	} else {
...
static int mtk_chg_get_vbus_value(void *mdata)
{
	struct mtk_charger *pinfo = mdata;
	int vbus = 0;

	charger_dev_get_vbus_level(pinfo->chg1_dev, &vbus);

	return vbus;
}

static int mtk_chg_set_afc_voltage(int voltage)
{
	int afc_disable = vt_muic_get_afc_disable();
	struct charger_device *chg1_dev;

	chg1_dev = get_charger_by_name("primary_chg");
	if (!chg1_dev) {
		pr_err("%s: chg1_dev is null\n", __func__);
		return - ENODEV;
	}

	if (voltage == 0x9 && afc_disable) {
	...
	charger_dev_afc_set_voltage(chg1_dev, voltage);
	return 0;
}

static void mtk_chg_set_afc_disable(void *mdata)
{
	struct mtk_charger *pinfo = mdata;
	...
	ret = charger_dev_afc_disable_set(pinfo->chg1_dev, afc_disable);
	if (!pinfo->chg1_dev || ret < 0) {
		pr_err("%s: afc_disable_set not supported\n", __func__);
		goto out;
	}

	switch (attached_dev) {
	case ATTACHED_DEV_AFC_CHARGER_PREPARE_MUIC ... ATTACHED_DEV_AFC_CHARGER_DISABLED_MUIC:
		charger_dev_get_vbus_level(pinfo->chg1_dev, &curr);
		vbus = afc_disable ? 5 : 9;

		if (afc_disable && curr == 5)
			vt_muic_set_attached_afc_dev(ATTACHED_DEV_AFC_CHARGER_DISABLED_MUIC);
		else
			charger_dev_afc_set_voltage(pinfo->chg1_dev, vbus);
		break;
	default:
		break;
	}
...
out:
	pr_info("%s afc_disable(%d)\n", __func__, afc_disable);
}

#if IS_ENABLED(CONFIG_HICCUP_CHARGER)
static int mtk_chg_set_hiccup_mode(void *mdata, bool en)
{
	...
	if (en == 1) /* Hiccup mode on */
		charger_dev_enable_hiccup(chg_dev, true);
	else /* Hiccup mode off */
		charger_dev_enable_hiccup(chg_dev, false);

	return 0;
}
#endif /* CONFIG_SEC_HICCUP */

static int mtk_chg_get_adc(void *mdata)
{
	int result = 0;
...
static int mtk_afc_setup(struct vt_muic_ic_data *ic_data)
{
	ic_data->m_ops.afc_set_voltage = mtk_chg_set_afc_voltage;
	ic_data->m_ops.get_adc = mtk_chg_get_adc;
	ic_data->m_ops.get_vbus_value = mtk_chg_get_vbus_value;
	ic_data->m_ops.set_afc_disable = mtk_chg_set_afc_disable;
#if IS_ENABLED(CONFIG_HICCUP_CHARGER)
	ic_data->m_ops.set_hiccup_mode = mtk_chg_set_hiccup_mode;
#endif
...
#if IS_ENABLED(CONFIG_VIRTUAL_MUIC) && IS_ENABLED(CONFIG_MTK_AFC_SUPPORT)
	info->afc_data.mdata = info;
	mtk_afc_setup(&info->afc_data);
	vt_muic_register_ic_data(&info->afc_data);
#endif
