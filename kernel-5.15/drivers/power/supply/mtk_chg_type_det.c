...
struct mtk_ctd_info {
	...
	struct pinctrl_state *ss_discharge_on;
	struct pinctrl_state *ss_discharge_off;
};
				...
				unsigned long event, void *data)
{
...
#if IS_ENABLED(CONFIG_BATTERY_SAMSUNG)
			/* In LPM, when pull out TA, still display charging UI. */
			/* Cable Plug out before power-off */
	...
	case TCP_NOTIFY_EXT_DISCHARGE:
		if (noti->en_state.en) {
			...
			if (mci->ss_discharge_on)
				pinctrl_select_state(mci->pinctrl, mci->ss_discharge_on);
		} else {
			pr_info("%s turn off charger discharge\n", __func__);
			if (mci->ss_discharge_off)
				pinctrl_select_state(mci->pinctrl, mci->ss_discharge_off);
		}

#if IS_ENABLED(CONFIG_MTK_CHARGER)
		charger_dev_enable_discharge(mci->chg_dev, noti->en_state.en);
#endif /* CONFIG_MTK_CHARGER */
		break;
	default:
		break;
...
static void mtk_ctd_parse_dt(struct mtk_ctd_info *mci)
{
	...
	mci->pinctrl = devm_pinctrl_get(mci->dev);
	if (IS_ERR(mci->pinctrl)) {
		ret = PTR_ERR(mci->pinctrl);
		dev_info(mci->dev, "failed to get pinctrl, ret=%d\n", ret);
		return;
	}

	mci->ss_discharge_on =
		pinctrl_lookup_state(mci->pinctrl, "ss_discharge_on");
	if (IS_ERR(mci->ss_discharge_on)) {
		dev_info(mci->dev, "Can NOT find discharge_on\n");
		mci->ss_discharge_on = NULL;
	} else
		dev_info(mci->dev, "Found discharge_on\n");

	mci->ss_discharge_off =
		pinctrl_lookup_state(mci->pinctrl, "ss_discharge_off");
	if (IS_ERR(mci->ss_discharge_off)) {
		dev_info(mci->dev, "Can NOT find discharge_off\n");
		mci->ss_discharge_off = NULL;
	} else
		dev_info(mci->dev, "Found discharge_off\n");
}
...
#if IS_ENABLED(CONFIG_MTK_CHARGER)
	mci->chg_dev = get_charger_by_name("primary_chg");
	if (!mci->chg_dev) {
		dev_notice(mci->dev, "Failed to get chg_dev\n");
		return -ENODEV;
	}
#endif /* CONFIG_MTK_CHARGER */
