...
int pd_hal_set_adapter_cap_type(struct chg_alg_device *alg, enum adapter_cap_type type, int mV, int mA)
{
	...
	pd_dbg("%s %d %d\n", __func__, mV, mA);
	hal = chg_alg_dev_get_drv_hal_data(alg);
	return adapter_dev_set_cap(hal->adapter, type, mV, mA);
}

bool pd_hal_is_src_usb_suspend_support(struct chg_alg_device *alg)
{
	struct pd_hal *hal;
	...
	pd_dbg("%s\n", __func__);
	hal = chg_alg_dev_get_drv_hal_data(alg);
	return adapter_dev_is_src_usb_suspend_support(hal->adapter);
}

bool pd_hal_is_src_usb_communication_capable(struct chg_alg_device *alg)
{
	struct pd_hal *hal;
	...
	pd_dbg("%s\n", __func__);
	hal = chg_alg_dev_get_drv_hal_data(alg);
	return adapter_dev_is_src_usb_communication_capable(hal->adapter);
}
