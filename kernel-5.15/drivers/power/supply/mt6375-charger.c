...
static void mt6375_chg_hvdcp_work_func(struct work_struct *work)
{
	...
	if ((val&0xE)>>1 != 0x7) {
		dev_info(ddata->dev, "AFC not detected 0x%x\n", val);
		goto out;
	}
	...
	/* Check VBUS 9V */
	for (i=0; i<AFC_9V_READ_COUNT; i++) {
		ret = charger_dev_get_vbus(ddata->chgdev, &vbus);
		if (ret < 0) {
			dev_err(ddata->dev, "%s: get VBUS fail\n", ret);
			goto out;
		}
		mt_dbg(ddata->dev, "VBUS: %u\n", vbus);

		if ((vbus >= 8000000) && (vbus <= 10300000))
			break;
		else if(i == AFC_9V_READ_COUNT-1)
			dev_info(ddata->dev, "VBUS 9v not confirmed\n");
...
#ifdef MT6375_AFC_SUPPORT
	/* AFC disable */
	ret = mt6375_chg_field_set(ddata, F_AFC_EN, 0x0);
#endif
