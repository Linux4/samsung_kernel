...
extern int sia81xx_pa_enable(bool enable, int channel);
					...
					int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		/* spk amp on control */
		//S98901AA1-827, zhouweijie.wt, add, 2024/04/11, sia81xx bring up
		sia81xx_pa_enable(true, 0); /* 0:LEFT CHANNEL FOR MONO PA */
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/* spk amp off control */
		//S98901AA1-827, zhouweijie.wt, add, 2024/04/11, sia81xx bring up
		sia81xx_pa_enable(false, 0); /* 0:LEFT CHANNEL FOR MONO PA */
		break;
	default:
		break;
	}
...
//S98901AA1-827, zhouweijie.wt, add, 2024/04/11, modify dailink for aw smartpa
...
static int mt6835_mt6377_dev_probe(struct platform_device *pdev)
{
	...
	for_each_card_prelinks(card, i, dai_link) {
		...
		//S98901AA1-827, zhouweijie.wt, add, 2024/04/11, modify dailink for aw smartpa
		if (smartpa_type == aw88394) {
			if (!strcmp(dai_link->name, "I2S0")) {
				dai_link->num_codecs = ARRAY_SIZE(awinic_codecs);
				dai_link->codecs = awinic_codecs;
				pr_info("%s: name: %s, dai_name: %s\n", __func__, dai_link->codecs->name, dai_link->codecs->dai_name);

			} else if (!strcmp(dai_link->name, "I2S3")) {
				dai_link->num_codecs = ARRAY_SIZE(awinic_codecs);
				dai_link->codecs = awinic_codecs;
				pr_info("%s: name: %s, dai_name: %s\n", __func__, dai_link->codecs->name, dai_link->codecs->dai_name);
			}
	...
	card->dev = &pdev->dev;
	//+S98901AA1-827, zhouweijie.wt, add, 2024/04/11, modify for sia81xx bring up
