...
//+Req, S98901AA1-788, zhangtao10.wt, add, 2024/05/15, headset/headphone status node
...
static u32 config_moisture_detect_2_1_2(void);
...
static u32 adjust_moisture_digital_setting(u32 eintID)
{
	if ((accdet_dts.moisture_detect_mode == 0x1) ||
		...
		(accdet_dts.moisture_detect_mode == 0x6)) {
...
static u32 adjust_moisture_analog_setting(u32 eintID)
{
	...
	if (accdet_dts.moisture_detect_mode == 0x1) {
		...
		/* do nothing */
	} else if (accdet_dts.moisture_detect_mode == 0x6) {
		/* do nothing */
	}
	return 0;
}
...
static u32 adjust_moisture_setting(u32 moistureID, u32 eintID)
{
	...
	if (moistureID == M_PLUG_IN) {
		...
		if (accdet_dts.moisture_detect_mode == 0x5) {
			 ...
			 */
			if (accdet->cur_eint_state == EINT_PLUG_IN) {
				/* for SS mode to align EINT2.0 */
				pr_info("[M_Check_Flow] IN.8 ignore HP in + water in + HP out irq\n");
				clear_accdet_eint(eintID);
				clear_accdet_eint_check(eintID);
				return M_NO_ACT;
			}
		...
		pr_info("[M_Check_Flow] IN.7 cur_eint:%d (0x%x)=0x%x\n",
			...
			ACCDET_EINT0_CMPMEN_SW_ADDR,
			accdet_read(ACCDET_EINT0_CMPMEN_SW_ADDR));
		if (accdet_dts.moisture_detect_mode == 0x6) {
			if (accdet->cur_eint_state == EINT_MOISTURE_DETECTED) {
				pr_info("[M_Check_Flow] IN.9 ss mode ignore water in then HP in\n");
				clear_accdet_eint(eintID);
				clear_accdet_eint_check(eintID);
				return M_NO_ACT;
			}
			if (accdet->cur_eint_state == EINT_PLUG_IN) {
				pr_info("[M_Check_Flow] IN.10 ss mode ignore plugin irq in plug in\n");
				clear_accdet_eint(eintID);
				clear_accdet_eint_check(eintID);
				return M_NO_ACT;
			}
...
static u32 config_moisture_detect_2_1_2(void)
{
	...
	/* EINTVTH1K/5K/10K efuse */
	ret = nvmem_device_read(accdet->accdet_efuse, 121*2, 2, &efuseval);
	eintvth = (int)((efuseval) & ACCDET_CALI_MASK0);
	pr_info("%s RG_MOISTURE3 - 1936 efuse=0x%x,eintvth=0x%x\n",
		__func__, efuseval, eintvth);

	/* set moisture reference voltage MVTH */
	if (eintvth == 0) {
		pr_info("EFUSE RG_MOISTURE3 not set\n");
		/* This is hardcode for no efuse chip */
		pr_info("%s ACCDET_MVTHEN_ADDR = 0x3c\n", __func__);
		accdet_write(ACCDET_MVTHEN_ADDR, 0x3c);
	} else {
		/* setting from ss mode programming guide */
		accdet_update_bits(ACCDET_MVTHEN_ADDR, 7, 0x1, 0);
		accdet_update_bits(ACCDET_MVTHEN_ADDR, 2, 0x1f, ((eintvth) & 0x1f));
	}
	return 0;
}
...
static int accdet_get_dts_data(void)
{
	...
	ret = of_property_read_u32(node, "moisture-detect-enable",
	...
	ret = of_property_read_u32(node, "eint-detect-mode",
	...
	ret = of_property_read_u32(node, "moisture-detect-mode",
		&accdet_dts.moisture_detect_mode);
	if (ret) {
		/* no moisture detection */
		accdet_dts.moisture_detect_mode = 0x4;
	}
	ret = of_property_read_u32(node, "moisture-comp-vth",
		&accdet_dts.moisture_comp_vth);
	if (ret) {
		/* no moisture detection */
		accdet_dts.moisture_comp_vth = 0x0;
	}
	ret = of_property_read_u32(node, "moisture-comp-vref2",
		&accdet_dts.moisture_comp_vref2);
	if (ret) {
		/* no moisture detection */
		accdet_dts.moisture_comp_vref2 = 0x0;
	}
	ret = of_property_read_u32(node, "moisture-use-ext-res",
...
{
	//int accdet_AB = 0;
	pr_info("3.5mm headset/headphone is not supported\n");
...
//+Req, S98901AA1-788, zhangtao10.wt, add, 2024/05/15, headset/headphone status node
	...
	if (ret)
		goto err_chrdevregion;
	// class create
	accdet->accdet_class = class_create(THIS_MODULE, ACCDET_AUDIO_DEVNAME);
	if (!accdet->accdet_class) {
		dev_dbg(&pdev->dev,
			"Error: Create audio class failed (%d)\n", ret);
		ret = -1;
	}
	// device create
	accdet->accdet_earjack = device_create(accdet->accdet_class, NULL, accdet->accdet_devno,
										NULL, ACCDET_AUDIO_EARJACK_DEVNAME);
	if (!accdet->accdet_earjack) {
