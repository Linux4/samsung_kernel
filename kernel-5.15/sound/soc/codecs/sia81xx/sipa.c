...
int sia81xx_pa_enable(bool enable, int channel)
{
	int i = 0;
	sipa_dev_t *si_pa = NULL;
	pr_debug("[debug][%s] %s: pa_enable = %d\r\n",
		LOG_FLAG, __func__, enable);

	mutex_lock(&sipa_mutex);
	for (i = 0; i < ARRAY_SIZE(g_sipa_dev); i++) {
		if ((NULL != g_sipa_dev[i]) && (channel == g_sipa_dev[i]->channel_num)) {
			si_pa = g_sipa_dev[i];
			break;
		}
	}
	...
	if (NULL != si_pa) {
		pr_info("%s: %d channel = %d, chip_type = %d, scene = %d\n",
...
EXPORT_SYMBOL(sia81xx_pa_enable);
...
//+S98901AA1-827, zhouweijie.wt, add, 2024/04/15, sia81xx bring up
	if (si_pa->chip_type == CHIP_TYPE_SIA815T) {
		smartpa_type = snd_soc_set_smartpa_type("sipa_i2c_probe", sia81xx);
	} else {
