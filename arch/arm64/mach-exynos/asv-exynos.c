...
#define INT_MIF_LV_MAX	10

static int dvfs_lv[ASV_TYPE_END] = {[0 ... (ASV_TYPE_END-1)] = INT_MIF_LV_MAX};
static int dvfs_abb[ASV_TYPE_END] = {[0 ... (ASV_TYPE_END-1)] = ABB_BYPASS};
static DEFINE_SPINLOCK(dvfs_abb_slock);

unsigned int register_match_abb(enum asv_type_id target_type, unsigned int target_freq)
{
	struct asv_info *match_asv_info = asv_get(target_type);
	unsigned int target_dvfs_level;
	unsigned int i;
	...
	spin_lock(&dvfs_abb_slock);
	for (i = 0; i < target_dvfs_level; i++)	{
		if (match_asv_info->asv_abb[i].asv_freq == target_freq) {
			dvfs_lv[target_type] = i;
			spin_unlock(&dvfs_abb_slock);
			return 0;
		}
	}

	/* If there is no matched freq, return reset value */
	dvfs_lv[target_type] = INT_MIF_LV_MAX;
	spin_unlock(&dvfs_abb_slock);
...
static enum asv_type_id get_target_type(enum asv_type_id target_type)
{
	enum asv_type_id type = target_type;

	if (type != ID_MIF && type != ID_INT && type != ID_ISP)
		return type;

	if (dvfs_lv[ID_ISP] > dvfs_lv[ID_MIF])
		type = ID_MIF;
	if (dvfs_lv[type] > dvfs_lv[ID_INT])
		type = ID_INT;
...
unsigned int set_match_abb(enum asv_type_id target_type, unsigned int target_abb)
{
	struct asv_info *match_asv_info;
	enum asv_type_id type = target_type;
	unsigned int abb = target_abb;

#if defined (CONFIG_SOC_EXYNOS7580)
	spin_lock(&dvfs_abb_slock);
	dvfs_abb[target_type] = target_abb;
	type = get_target_type(target_type);
	abb = dvfs_abb[type];
	spin_unlock(&dvfs_abb_slock);
#endif
	match_asv_info = asv_get(type);
	...
	match_asv_info->abb_info->target_abb = abb;
