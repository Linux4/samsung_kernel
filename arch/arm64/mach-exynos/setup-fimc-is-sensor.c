...
static int exynos7580_fimc_is_csi0_gate(struct platform_device *pdev, bool mask)
{
	int ret = 0;

	if (mask) {
		fimc_is_disable_dt(pdev, "aclk_csi_link0_1_wrap_clk");
	} else {
		fimc_is_enable_dt(pdev, "aclk_csi_link0_1_wrap_clk");
	}

	return ret;
}

static int exynos7580_fimc_is_csi1_gate(struct platform_device *pdev, bool mask)
{
	int ret = 0;

	if (mask) {
		fimc_is_disable_dt(pdev, "aclk_csi_link1_1_wrap_clk");
	} else {
		fimc_is_enable_dt(pdev, "aclk_csi_link1_1_wrap_clk");
	}

	return ret;
}

static int exynos7580_fimc_is_bns0_gate(struct platform_device *pdev, bool mask)
{
	int ret = 0;

	if (mask) {
		fimc_is_disable_dt(pdev, "aclk_fimc_bns_aclk");
		fimc_is_disable_dt(pdev, "aclk_sasync_fimc_bns_m_to_bus");
		fimc_is_disable_dt(pdev, "aclk_masync_fimc_bns_m_to_bus");
	} else {
		fimc_is_enable_dt(pdev, "aclk_fimc_bns_aclk");
		fimc_is_enable_dt(pdev, "aclk_sasync_fimc_bns_m_to_bus");
		fimc_is_enable_dt(pdev, "aclk_masync_fimc_bns_m_to_bus");
	}

	return ret;
}

static int exynos7580_fimc_is_bns1_gate(struct platform_device *pdev, bool mask)
{
	int ret = 0;

	if (mask) {
		fimc_is_disable_dt(pdev, "aclk_fimc_bns_l_aclk");
		fimc_is_disable_dt(pdev, "aclk_sasync_fimc_bns_l_to_bus");
		fimc_is_disable_dt(pdev, "aclk_masync_fimc_bns_l_to_bus");
	} else {
		fimc_is_enable_dt(pdev, "aclk_fimc_bns_l_aclk");
		fimc_is_enable_dt(pdev, "aclk_sasync_fimc_bns_l_to_bus");
		fimc_is_enable_dt(pdev, "aclk_masync_fimc_bns_l_to_bus");
	}

	return ret;
}

int exynos7580_fimc_is_sensor_iclk_cfg(struct platform_device *pdev,
	u32 scenario,
	u32 channel)
{
	exynos7580_fimc_is_csi0_gate(pdev, false);
	exynos7580_fimc_is_csi1_gate(pdev, false);

	exynos7580_fimc_is_bns0_gate(pdev, false);
	exynos7580_fimc_is_bns1_gate(pdev, false);

	fimc_is_enable_dt(pdev, "mout_phyclk_csi_link0_rx_user");
	fimc_is_enable_dt(pdev, "mout_phyclk_csi_link1_rx_user");

	return 0;
}

int exynos7580_fimc_is_sensor_iclk_on(struct platform_device *pdev,
	u32 scenario,
	u32 channel)
{
	int ret = 0;
	...
	case 0:
		/* CSI */
		exynos7580_fimc_is_csi1_gate(pdev, true);
		/* BNS */
		exynos7580_fimc_is_bns1_gate(pdev, true);

		break;
	case 1:
		/* CSI */
		exynos7580_fimc_is_csi0_gate(pdev, true);
		/* BNS */
		exynos7580_fimc_is_bns0_gate(pdev, true);
	...
	switch (channel) {
	case 0:
		fimc_is_enable_dt(pdev, "mout_aclk_csi_link1_75");
		fimc_is_enable_dt(pdev, "mout_aclk_csi_link1_75_b");
		fimc_is_enable_dt(pdev, "mout_phyclk_csi_link0_rx_user");
		fimc_is_enable_dt(pdev, "mout_phyclk_csi_link1_rx_user");

		fimc_is_set_rate_dt(pdev, "dout_aclk_csi_link1_75",286 * 1000000);
		fimc_is_set_rate_dt(pdev, "dout_pclk_csi_link1_37",95 * 1000000);
...
int exynos7580_fimc_is_sensor_iclk_off(struct platform_device *pdev,
	u32 scenario,
	u32 channel)
{
	int ret = 0;
	...
	switch (channel) {
	case 0:
		fimc_is_disable_dt(pdev, "mout_aclk_csi_link1_75");
		fimc_is_disable_dt(pdev, "mout_aclk_csi_link1_75_b");
		fimc_is_disable_dt(pdev, "mout_phyclk_csi_link0_rx_user");
		break;
	case 1:
		fimc_is_disable_dt(pdev, "mout_aclk_csi_link1_75");
		fimc_is_disable_dt(pdev, "mout_aclk_csi_link1_75_b");
		fimc_is_disable_dt(pdev, "mout_phyclk_csi_link1_rx_user");
		break;
	default:
		pr_err("channel is invalid(%d)\n", channel);
		ret = -EINVAL;
...
int exynos7580_fimc_is_sensor_mclk_on(struct platform_device *pdev,
	u32 scenario,
	u32 channel)
{
	int ret = 0;
	...
	switch (channel) {
	case 0:
		fimc_is_set_parent_dt(pdev, "mout_sclk_isp_sensor0", "oscclk");

		fimc_is_set_rate_dt(pdev, "dout_sclk_isp_sensor0_a", 26 * 1000000);
		fimc_is_set_rate_dt(pdev, "dout_sclk_isp_sensor0_b", 26 * 1000000);

		fimc_is_get_rate_dt(pdev, "dout_sclk_isp_sensor0_b");

		fimc_is_enable_dt(pdev, "sclk_isp_sensor0");
		break;
	case 1:
		fimc_is_set_parent_dt(pdev, "mout_sclk_isp_sensor1", "oscclk");

		fimc_is_set_rate_dt(pdev, "dout_sclk_isp_sensor1_a", 26 * 1000000);
		fimc_is_set_rate_dt(pdev, "dout_sclk_isp_sensor1_b", 26 * 1000000);

		fimc_is_get_rate_dt(pdev, "dout_sclk_isp_sensor1_b");

		fimc_is_enable_dt(pdev, "sclk_isp_sensor1");
		break;
	default:
		pr_err("channel is invalid(%d)\n", channel);
		ret = -EINVAL;
...
int exynos7580_fimc_is_sensor_mclk_off(struct platform_device *pdev,
		u32 scenario,
		u32 channel)
{
	int ret = 0;
	...
	case 0:
		...
		break;
	case 1:
		fimc_is_disable_dt(pdev, "sclk_isp_sensor1");
		break;
	default:
		pr_err("channel is invalid(%d)\n", channel);
		ret = -EINVAL;
...
#elif defined(CONFIG_SOC_EXYNOS7580)
	exynos7580_fimc_is_sensor_iclk_cfg(pdev, scenario, channel);
#endif
	return 0;
}
...
#elif defined(CONFIG_SOC_EXYNOS7580)
	exynos7580_fimc_is_sensor_iclk_on(pdev, scenario, channel);
#endif
	return 0;
}
...
#elif defined(CONFIG_SOC_EXYNOS7580)
	exynos7580_fimc_is_sensor_iclk_off(pdev, scenario, channel);
#endif
	return 0;
}
...
#elif defined(CONFIG_SOC_EXYNOS7580)
	exynos7580_fimc_is_sensor_mclk_on(pdev, scenario, channel);
#endif
	return 0;
}
...
#elif defined(CONFIG_SOC_EXYNOS7580)
	exynos7580_fimc_is_sensor_mclk_off(pdev, scenario, channel);
#endif
	return 0;
}
