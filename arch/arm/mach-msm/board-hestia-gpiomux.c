...
static struct msm_gpiomux_config msm_lcd_configs_rev8[] __initdata = {
	{
		.gpio = 25,/* LCD Reset */
		.settings = {
			[GPIOMUX_ACTIVE]	= &lcd_rst_act_cfg,
...
static struct msm_gpiomux_config msm_sensorhub_configs_rev8[] __initdata = {
	SSPN_CONFIG(64, 0),	/* MCU INT 1 IRQ */
	SSPN_CONFIG(65, 0),	/* MCU INT 2 IRQ */
	SSPN_CONFIG(118, 1),			/* AP INT IRQ */
	SSPN_CONFIG(104, 0),			/* RESET */
...
#define BGM_IN_CFG(gpio_num) IN_PD_2MA_CFG(gpio_num)
...
static struct msm_gpiomux_config msm_bgm_configs_rev08[] __initdata = {
	BGM_IRQ_CFG(72),
	BGM_IN_CFG(115),
};
...
#define EAR_SWITCH_CFG(gpio_num) IN_NP_2MA_CFG(gpio_num)
...
static struct gpiomux_setting wcdcodec_reset_suspended_cfg = {
	.func = GPIOMUX_FUNC_GPIO,
	.drv = GPIOMUX_DRV_2MA,
	.dir = GPIOMUX_OUT_LOW,
	.pull = GPIOMUX_PULL_NONE,
...
static struct msm_gpiomux_config wcdcodec_reset_cfg[] __initdata = {
	{
		...
		.settings = {
			...
			[GPIOMUX_SUSPENDED] = &wcdcodec_reset_suspended_cfg,
		},
	},
};

static struct msm_gpiomux_config wcdcodec_reset_cfg_rev08[] __initdata = {
	{
		.gpio = 88,
		.settings = {
			[GPIOMUX_ACTIVE] = &wcdcodec_reset_active_cfg,
...
static struct msm_gpiomux_config nc_gpio_cfgs_rev8[] __initdata = {
	NC_GPIO_CONFIG(75),
	NC_GPIO_CONFIG(76),
	NC_GPIO_CONFIG(77),
	NC_GPIO_CONFIG(78),
...
void __init msm8226_init_gpiomux(void)
{
	...
	if(system_rev >= 8)
		msm_gpiomux_install(msm_sensorhub_configs_rev8,
			ARRAY_SIZE(msm_sensorhub_configs_rev8));
	else
	...
	if(system_rev >= 8)
		...
		msm_gpiomux_install(msm_bgm_configs_rev08,
			ARRAY_SIZE(msm_bgm_configs_rev08));
	...
	if(system_rev >= 8)
		msm_gpiomux_install_nowrite(msm_lcd_configs_rev8,
				ARRAY_SIZE(msm_lcd_configs_rev8));
	else
	 ...
	 */
	if(system_rev >= 8)
		msm_gpiomux_install(nc_gpio_cfgs_rev8,
			ARRAY_SIZE(nc_gpio_cfgs_rev8));
	else
	...
	if(!poweroff_charging)
	{
		if(system_rev < 8)
			...
			msm_gpiomux_install(wcdcodec_reset_cfg_rev08, ARRAY_SIZE(wcdcodec_reset_cfg_rev08));
	}
#ifdef CONFIG_SND_SOC_MAX98504
