...
static int force_error(const char *val, struct kernel_param *kp)
{
	pr_emerg("WARNING forced error triggered\n");
...
#define FIRST_CRASH_KEY KEY_VOLUMEDOWN
#define SECOND_CRASH_KEY KEY_POWER
#else
#define FIRST_CRASH_KEY KEY_VOLUMEDOWN
#define SECOND_CRASH_KEY KEY_HOME
#endif
...
void sec_debug_check_crash_key(unsigned int code, int value)
{
	...
	switch (state) {
	case NONE:
		if (code == FIRST_CRASH_KEY && value)
			state = STEP1;
		else
			state = NONE;
		break;
	case STEP1:
		if (code == SECOND_CRASH_KEY && value)
			state = STEP2;
		else
			state = NONE;
		break;
