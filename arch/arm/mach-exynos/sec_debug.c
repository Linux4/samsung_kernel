...
struct auxiliary_log {
	...
	struct auxiliary_info DllLog[AUX_LOG_DLL_MAX];
};
...
static atomic_t gExcpAuxDllLogIdx = ATOMIC_INIT(-1);
#endif
...
void sec_debug_aux_log(int idx, char *fmt, ...)
{
	...
	case SEC_DEBUG_AUXLOG_THERMAL_CHANGE:
		...
		strncpy((*gExcpAuxLogPtr).ThermalLog[i].log,
			buf, AUX_LOG_LENGTH);
		break;
	case SEC_DEBUG_AUXLOG_DLL_CHANGE:
		i = atomic_inc_return(&gExcpAuxDllLogIdx)
			& (AUX_LOG_DLL_MAX - 1);
		(*gExcpAuxLogPtr).DllLog[i].time = cpu_clock(cpu);
		(*gExcpAuxLogPtr).DllLog[i].cpu = cpu;
		strncpy((*gExcpAuxLogPtr).DllLog[i].log,
			buf, AUX_LOG_LENGTH);
		break;
	default:
		break;
...
#define SEC_PARAM_FILE_NAME	"/dev/block/platform/dw_mmc.0/by-name/PARAM"	/* parameter block */
#define SEC_PARAM_FILE_OFFSET	0x700224

static bool param_sec_operation(int value)
{
	...
	ret = filp->f_op->write(filp, &val, size, &filp->f_pos);
param_sec_debug_out:
	set_fs(fs);
...
{
	return param_sec_operation(value);
}
...
#endif	/* CONFIG_SEC_MONITOR_BATTERY_REMOVAL */
