...
static const char name_exynos3472[] = "EXYNOS3472";
...
void exynos_panic_debug(void)
{
	/* only for debug */

	pr_err("============ %s ============ \n", __func__);
	pr_err("reg_PHY0_CON13 = 0x%08x\n", __raw_readl(S5P_VA_DMC0 + 0x34));
	pr_err("reg_PHY1_CON13 = 0x%08x\n", __raw_readl(S5P_VA_DMC1 + 0x34));
...
static void __init exynos3_init_clocks(int xtal)
{
	...
	if (soc_is_exynos3250()) {
		...
		exynos3472_register_clocks();
		exynos3472_setup_clocks();
	}
}
...
static int __init exynos4_l2x0_cache_init(void)
{
	...
	if (!(__raw_readl(S5P_VA_L2CC + L2X0_CTRL) & 0x1)) {
		...
		if (soc_is_exynos4415())
			l2x0_saved_regs.tag_latency = 0x120;
		else
