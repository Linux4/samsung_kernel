...
static void exynos_usb_mux_change(struct platform_device *pdev, int val)
{
	...
	if (soc_is_exynos4212() || soc_is_exynos4412() ||
	    soc_is_exynos3470() || soc_is_exynos3472() || soc_is_exynos4415() || soc_is_exynos3250()) {
		is_host = readl(EXYNOS4_USB_CFG);
		writel(val, EXYNOS4_USB_CFG);
	} else if (soc_is_exynos5260()) {
		dev_dbg(&pdev->dev, "Change USB MUX is not used in Exynos5260.\n");
		return;
	} else {
...
static int exynos5_usb_phy30_is_on(void)
{
	if (soc_is_exynos5250() || soc_is_exynos5260())
		return readl(EXYNOS5_USBDEV_PHY_CONTROL) &
			EXYNOS5_USBDEV_PHY_ENABLE ? 1 : 0;
	else
...
static int exynos4_usb_phy20_init(struct platform_device *pdev)
{
...
#if defined(CONFIG_LINK_DEVICE_HSIC)
	if (!strcmp(pdev->name, "exynos-ohci"))
		need_ehci_config = 1;
#endif
	writel(PHY_ENABLE, EXYNOS4x12_USB_PHY_CONTROL);
	writel(PHY_ENABLE, EXYNOS4x12_HSIC0_PHY_CONTROL);
	...
	if (soc_is_exynos3250() || soc_is_exynos3472())
		phyclk |= REF_CLKSEL;
...
static int exynos5_usb_phy_host_resume(struct platform_device *pdev)
{
	...
	if (exynos5_usb_phy20_is_on()) {
		...
		/*
		 * The following 'if' statement handles the situation,
		 * when exit from LPA was initiated by peripheral (cable plug)
		 * which shares the PHY with USB2.0 host controller
		 * (e.g. Exynos5250). In this case exynos5_usb_phy20_init()
		 * is called by UDC driver and HOST_CTRL0_SIDDQ bit is cleared.
		 * On next host runtime resume, since PHY is already active,
		 * we need just make sure it is not suspended and notify host
		 * driver about recent exit from LPA.
		 */
		if (usb_phy_control.lpa_entered) {
#if defined(CONFIG_LINK_DEVICE_HSIC)
			if (!strcmp(pdev->name, "s5p-ehci"))
...
static int exynos5_usb_phy20_init(struct platform_device *pdev)
{
	...
	if (soc_is_exynos5260())
		writel(readl(EXYNOS5_USBDEV_PHY_CONTROL) | EXYNOS5_USBHSIC_PHY_ENABLE,
			EXYNOS5_USBDEV_PHY_CONTROL);
...
static int exynos_usb_dev_phy20_exit(struct platform_device *pdev)
{
	if (soc_is_exynos4212() || soc_is_exynos4412() || soc_is_exynos4415() ||
	    soc_is_exynos3470() || soc_is_exynos3472() || soc_is_exynos3250()) {
...
static int exynos5_usb_phy30_init(struct platform_device *pdev)
{
	...
	case 0:
		reg_base = S5P_VA_USB3_DRD0_PHY;
		writel(readl(EXYNOS5_USBDEV_PHY_CONTROL) | EXYNOS5_USBDEV_PHY_ENABLE,
			EXYNOS5_USBDEV_PHY_CONTROL);
		break;
	case 1:
		reg_base = S5P_VA_USB3_DRD1_PHY;
	...
	if (soc_is_exynos5260())
		writel(readl(reg_base + EXYNOS_USB3_PHYPIPE) | (1 << 4),
			reg_base + EXYNOS_USB3_PHYPIPE);
	else
...
static int s5p_usb_otg_phy_tune(struct s3c_hsotg_plat *pdata, int def_mode)
{
	...
	phytune = readl(EXYNOS5_PHY_HSIC_TUNE2);
	...
	if (def_mode) {
		pr_debug("usb: %s set defult tune=0x%x\n",
				__func__, pdata->def_phytune);
		writel(pdata->def_phytune, EXYNOS5_PHY_HSIC_TUNE2);
	} else {

		phytune &= ~(OTG_TUNE_TXVREFTUNE(0xF));
		phytune |= OTG_TUNE_TXVREFTUNE(0x9); // +12%

		phytune &= ~(OTG_TUNE_SQRXTUNE(0x7));
		phytune |= OTG_TUNE_SQRXTUNE(0x5); // -10%

		udelay(10);
		pr_debug("usb: %s custom tune=0x%x\n",
				__func__, phytune);
		writel(phytune, EXYNOS5_PHY_HSIC_TUNE2);
	}
...
static int exynos5_check_usb_op(void)
{
	...
	/* Simulate hsic2 phy for exynos5260 */
	if (soc_is_exynos5260())
...
int exynos_check_usb_op(void)
{
	if (soc_is_exynos4210() ||
	    ...
	    soc_is_exynos3470() ||
	    soc_is_exynos4415() ||
	    soc_is_exynos3250() || soc_is_exynos3472())
...
#if defined(CONFIG_LINK_DEVICE_HSIC)
	if (!strcmp(pdev->name, "s5p-ehci") && need_ehci_config) {
		need_ehci_config = 0;
		ret = 1;
	}
#endif
