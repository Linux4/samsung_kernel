...
static struct sleep_save exynos4415_clock_save[] = {
	...
	SAVE_ITEM(EXYNOS4415_CLKGATE_SCLK_DMC),
...
static struct sleep_save exynos4_isppll_save[] = {
	SAVE_ITEM(EXYNOS4415_ISPPLL_CON0),
};

static struct sleep_save exynos4_epll_save[] = {
	SAVE_ITEM(EXYNOS4415_EPLL_CON0),
	SAVE_ITEM(EXYNOS4415_EPLL_CON1),
};

static struct sleep_save exynos4_vpll_save[] = {
	SAVE_ITEM(EXYNOS4415_G3DPLL_CON0),
};
#endif
...
static int exynos4415_v_epll_ctrl(struct clk *clk, int enable)
{
	int ret = 0;
	unsigned int tmp;

	pr_debug("EPLL is %s\n", enable ? "on" : "off");
	ret = s5p_gatectrl(EXYNOS4415_EPLL_CON0, clk, enable);
...
static int  exynos4415_mout_epll_ctrl(struct clk *clk, int enable)
{
	struct clk *epll_clk = &exynos4415_clk_mout_epll.clk;
	struct clk *old_parent = epll_clk->parent;
	int i;

	if (enable) {
		((&clk_v_epll)->enable)(&clk_v_epll, enable);

		if (epll_clk->usage && (old_parent != &clk_fout_epll)) {
			for (i = 0; i < epll_clk->usage; i++)
				clk_fout_epll.usage++;
		}

		(epll_clk->ops->set_parent)(epll_clk, &clk_fout_epll);

		if (epll_clk->usage && (old_parent != &clk_fout_epll)) {
			for (i = 0; i < epll_clk->usage; i++)
				clk_ext_xtal_mux.usage--;
		}
	} else {
		for (i = 0; i < (epll_clk->usage + 1); i++)
			clk_ext_xtal_mux.usage++;

		(epll_clk->ops->set_parent)(epll_clk, &clk_ext_xtal_mux);
...
static int exynos4415_v_vpll_ctrl(struct clk *clk, int enable)
{
	int ret = 0;
	unsigned int tmp;

	pr_debug("VPLL is %s\n", enable ? "on" : "off");
	ret = s5p_gatectrl(EXYNOS4415_G3DPLL_CON0, clk, enable);
...
static int exynos4415_v_isp_pll_ctrl(struct clk *clk, int enable)
{
	int ret = 0;
	unsigned int tmp;

	pr_debug("ISPPLL is %s\n", enable ? "on" : "off");
	ret = s5p_gatectrl(EXYNOS4415_ISPPLL_CON0, clk, enable);

	if (enable) {
		/* wait_lock_time */
		do {
			cpu_relax();
			tmp = __raw_readl(EXYNOS4415_ISPPLL_CON0);
		} while (!(tmp & (0x1 << EXYNOS4415_PLL_LOCKED_SHIFT)));
	}
...
static int exynos4415_clk_sclk_dmc_ctrl(struct clk *clk, int enable)
{
	return s5p_gatectrl(EXYNOS4415_CLKGATE_SCLK_DMC, clk, enable);
}
...
static struct clksrc_clk exynos4415_clk_mout_epll = {
	...
	.reg_src_stat = { .reg = EXYNOS4415_CLKMUX_STAT_TOP0, .shift = 4, .size = 3 },
};
...
static struct clksrc_clk exynos4415_clk_sclk_vpll = {
	...
	.reg_src_stat = { .reg = EXYNOS4415_CLKMUX_STAT_TOP0, .shift = 8, .size = 3 },
};
...
static struct clksrc_clk exynos4415_clk_mout_isp_pll = {
	...
	.reg_src_stat = { .reg = EXYNOS4415_CLKMUX_STAT_TOP1, .shift = 28, .size = 3},
};
...
static struct clksrc_clk exynos4415_clk_mout_dmc_bus = {
	...
	.reg_src_stat = { .reg = EXYNOS4415_CLKSRC_STAT_DMC, .shift = 4, .size = 3 },
};
...
static struct clksrc_clk exynos4415_clk_sclk_dphy = {
	.clk	= {
		...
		.enable		= exynos4415_clk_sclk_dmc_ctrl,
		.ctrlbit	= ((1 << 2) | (1 << 3)),
	},
...
static struct vpll_div_data exynos4415_epll_div[] = {
	...
	{786432000, 3, 197, 1, 0x9BB3, 0, 0, 0},
};
...
static unsigned long exynos4415_apll_get_rate(struct clk *clk)
{
	return s5p_get_pll35xx(xtal_rate, __raw_readl(EXYNOS4415_APLL_CON0));
}

static int exynos4415_apll_set_rate(struct clk *clk, unsigned long rate)
{
	clk->rate = rate;
	return 0;
}

static struct clk_ops exynos4415_apll_ops = {
	.get_rate = exynos4415_apll_get_rate,
	.set_rate = exynos4415_apll_set_rate,
};
...
void __init_or_cpufreq exynos4415_setup_clocks(void)
{
	...
	vpll = s5p_get_pll35xx(xtal, __raw_readl(EXYNOS4415_G3DPLL_CON0));
	...
	clk_fout_apll.ops = &exynos4415_apll_ops;
	clk_fout_bpll.ops = &exynos4415_bpll_ops;
	clk_fout_bpll.rate = bpll;
	clk_fout_mpll.rate = mpll;
	clk_fout_epll.ops = &exynos4415_epll_ops;
	clk_fout_epll.rate = epll;
	exynos4415_clk_mout_epll.clk.enable = exynos4415_mout_epll_ctrl;
	clk_v_epll.enable = exynos4415_v_epll_ctrl;
	clk_v_epll.ctrlbit = (1 << 31);

	clk_fout_vpll.ops = &exynos4415_vpll_ops;
	clk_fout_vpll.rate = vpll;
	clk_v_vpll.enable = exynos4415_v_vpll_ctrl;
	clk_v_vpll.ctrlbit = (1 << 31);
	clk_fout_isp_pll.rate = isppll;
	clk_v_isp_pll.enable = exynos4415_v_isp_pll_ctrl;
	clk_v_isp_pll.ctrlbit = (1 << 31);
	...
	clk_enable(&exynos4415_clk_mout_epll.clk);
};
		...
		unsigned long val, void *data)
{
	...
	case FB_BLANK_POWERDOWN:
		...
		clk_enable(&exynos4415_clk_mout_epll.clk);
		clk_enable(&clk_v_isp_pll);
		break;
	case FB_BLANK_UNBLANK:
		clk_disable(&clk_v_vpll);
		clk_disable(&exynos4415_clk_mout_epll.clk);
		clk_disable(&clk_v_isp_pll);
		break;
	default:
		break;
	}
...
static inline void pll_restore(struct sleep_save *arr, int arr_size,
		void *conlock, int locktime_m)
{
	unsigned int p_div, lock_wait = 0;
	void *reg_adr;
	unsigned int reg_val;
	int locktime;

	/* Caution : must below				*/
	/*	PLL45XX_PDIV_MASK = PLL46XX_PDIV_MASK	*/
	/*	PLL45XX_PDIV_SHIFT = PLL46XX_PDIV_SHIFT	*/
	reg_val = arr[0].val;
	reg_adr = arr[0].reg;
	p_div = reg_val & (PLL45XX_PDIV_MASK << PLL45XX_PDIV_SHIFT);
	p_div = p_div >> PLL45XX_PDIV_SHIFT;

	locktime = locktime_m * p_div;
	__raw_writel(locktime, conlock);
	s3c_pm_do_restore_core(arr, arr_size);

	lock_wait = (reg_val & (1 << 31)) ? 1 : 0;
	while (lock_wait) {
		reg_val = __raw_readl(reg_adr);
		if (reg_val & (1 << EXYNOS4415_PLL_LOCKED_SHIFT))
			lock_wait = 0;
	}
}
...
static int exynos4415_clock_suspend(void)
{
	...
	s3c_pm_do_save(exynos4_isppll_save, ARRAY_SIZE(exynos4_isppll_save));
...
static void exynos4415_clock_resume(void)
{
	...
	if (!(exynos4_isppll_save[0].val & (1 << EXYNOS4415_PLL_ENABLE_SHIFT)))
		s3c_pm_do_restore_core(exynos4_isppll_save, ARRAY_SIZE(exynos4_isppll_save));
	pll_restore(exynos4_epll_save, ARRAY_SIZE(exynos4_epll_save),
			EXYNOS4415_EPLL_LOCK, 3000);
	pll_restore(exynos4_vpll_save, ARRAY_SIZE(exynos4_vpll_save),
			EXYNOS4415_G3DPLL_LOCK, 270);
}
...
int __init exynos4415_clock_register_fb(void)
{
	fb_register_client(&fb_block);
	clk_disable(&exynos4415_clk_mout_epll.clk);
	return 0;
}
late_initcall(exynos4415_clock_register_fb);
