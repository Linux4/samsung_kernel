...
static int exynos_pd_status(struct exynos_pm_domain *pd)
{
	...
	if (likely(pd->base)) {
		...
		pd->status = (__raw_readl(pd->base+0x4) & LOCAL_PWR_STATUS);
	}
...
static int exynos_pd_power(struct exynos_pm_domain *pd, int power_flags)
{
	...
	if (likely(pd->base)) {
		...
		timeout = 100;
		/* check STATUS register */
		while ((__raw_readl(pd->base+0x4) & LOCAL_PWR_STATUS) != power_flags) {
			if (timeout == 0) {
...
static int exynos_genpd_power_on(struct generic_pm_domain *genpd)
{
	...
	if (pd->cb && pd->cb->on_active && pd->cb->on_active(pd))
		return 0;
	...
	if (pd->cb && pd->cb->on_post) {
		pd->cb->on_post(pd);
		busmon_timeout_pd_init(pd->name);
	}
...
static int exynos_genpd_power_off(struct generic_pm_domain *genpd)
{
	...
	if (pd->cb && pd->cb->off_active && pd->cb->off_active(pd))
		return 0;
...
static __init int exynos_pm_dt_parse_domains(void)
{
	...
	for_each_compatible_node(np, NULL, "samsung,exynos-pd") {
		...
		pd->priv = exynos_pd_find_data(pd);
...
out:
	for_each_compatible_node(np, NULL, "samsung,exynos-pd") {
		...
		for_each_child_of_node(np, children) {
			...
			sub_pdev = of_find_device_by_node(children);
			if (sub_pdev) {
				sub_pd = platform_get_drvdata(sub_pdev);
				if (sub_pd) {
					kfree(sub_pd->genpd.name);
					kfree(sub_pd);
				}
			}
		}
...
arch_initcall(exynos_pm_domain_init);

static int __init exynos_pm_domain_idle(void)
{
