...
#if defined(CONFIG_PM_DEVFREQ) && defined (CONFIG_PM_RUNTIME)
...
#else
static inline void exynos_mif_notify_power_status(const char *pd_name, unsigned int turn_on){};
static inline void exynos_int_notify_power_status(const char *pd_name, unsigned int turn_on){};
#endif
