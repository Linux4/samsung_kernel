/* linux/arch/arm/mach-exynos/pmu-exynos3472.c
 *
...
static struct exynos_pmu_conf exynos3472_pmu_config[] = {
	...
	{ EXYNOS3_TOP_ASB_RESET_COREBLK_SYS_PWR_REG,		{ 0x1, 0x1, 0x1} },
...
void __iomem *exynos3472_list_feed[] = {
...
static void exynos3472_init_pmu(void)
{
	unsigned int i;
	unsigned int tmp;

	/* Enable only SC_FEEDBACK */
	for (i = 0 ; i < ARRAY_SIZE(exynos3472_list_feed) ; i++) {
		tmp = __raw_readl(exynos3472_list_feed[i]);
		tmp &= ~(EXYNOS3_OPTION_USE_SC_COUNTER);
		tmp |= EXYNOS3_OPTION_USE_SC_FEEDBACK;
		__raw_writel(tmp, exynos3472_list_feed[i]);
	}
}
...
static int __init exynos_pmu_init(void)
{
	...
	if (soc_is_exynos3250() || soc_is_exynos3472()) {
		...
		*/
		...
		exynos_pmu_config = exynos3472_pmu_config;
		pr_info("EXYNOS3472 PMU Initialize\n");
	} else {
