...
int exynos_lpd_prepare(void)
{
	...
	ret = exynos_pm_notify(LPD_PREPARE, -1, &nr_calls);
	read_unlock(&exynos_pm_notifier_lock);

	return ret;
}
EXPORT_SYMBOL_GPL(exynos_lpd_prepare);
...
int exynos_lpd_enter(void)
{
	...
	ret = exynos_pm_notify(LPD_ENTER, -1, &nr_calls);
	if (ret)
		/*
		 * Inform listeners (nr_calls - 1) about failure of CPU PM
		 * PM entry who are notified earlier to prepare for it.
		 */
		exynos_pm_notify(LPD_ENTER_FAIL, nr_calls - 1, NULL);
	read_unlock_irqrestore(&exynos_pm_notifier_lock, flags);

	return ret;
}
EXPORT_SYMBOL_GPL(exynos_lpd_enter);
...
int exynos_lpd_exit(void)
{
	...
	read_lock_irqsave(&exynos_pm_notifier_lock, flags);
	ret = exynos_pm_notify(LPD_EXIT, -1, NULL);
	read_unlock_irqrestore(&exynos_pm_notifier_lock, flags);

	return ret;
}
EXPORT_SYMBOL_GPL(exynos_lpd_exit);
