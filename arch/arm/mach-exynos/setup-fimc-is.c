...
/* Clock Gate func for Exynos4415 */
static int exynos4415_fimc_is_clk_gate(u32 clk_gate_id, bool is_on)
{
	int cfg = 0;
	u32 value = 0;
	...
	if (value > 0) {
		cfg = readl(EXYNOS4415_CLKGATE_IP_ISP0_SUB0);
		if (is_on)
			writel(cfg | value, EXYNOS4415_CLKGATE_IP_ISP0_SUB0);
		else
			writel(cfg & ~(value), EXYNOS4415_CLKGATE_IP_ISP0_SUB0);
		pr_debug("%s :2 [%s] gate(%d) (0x%x) * (0x%x)\n", __func__,
				is_on ? "ON" : "OFF",
	...
	if (value > 0) {
		cfg = readl(EXYNOS4415_CLKGATE_IP_ISP0_SUB1);
		if (is_on)
			writel(cfg | value, EXYNOS4415_CLKGATE_IP_ISP0_SUB1);
		else
			writel(cfg & ~(value), EXYNOS4415_CLKGATE_IP_ISP0_SUB1);
		pr_debug("%s :2 [%s] gate(%d) (0x%x) * (0x%x)\n", __func__,
				is_on ? "ON" : "OFF",
	...
	if (value > 0) {
		cfg = readl(EXYNOS4415_CLKGATE_IP_ISP1_SUB0);
		if (is_on)
			writel(cfg | value, EXYNOS4415_CLKGATE_IP_ISP1_SUB0);
		else
			writel(cfg & ~(value), EXYNOS4415_CLKGATE_IP_ISP1_SUB0);
		pr_debug("%s :2 [%s] gate(%d) (0x%x) * (0x%x)\n", __func__,
				is_on ? "ON" : "OFF",
...
static int exynos4415_fimc_is_set_user_clk_gate(u32 group_id,
		bool is_on,
		u32 user_scenario_id,
		unsigned long msk_state,
		struct exynos_fimc_is_clk_gate_info *gate_info) {
...
static int exynos4415_fimc_is_isp_pll_on(struct platform_device *pdev)
{
	struct clk *mout_isp_pll = NULL;
	struct clk *v_isppll = NULL;
	struct clk *fout_isp_pll = NULL;

	v_isppll = clk_get(&pdev->dev, "v_isppll");
	if (IS_ERR(v_isppll)) {
	...
	fout_isp_pll = clk_get(&pdev->dev, "fout_isp_pll");
	if (IS_ERR(fout_isp_pll)) {
		pr_err("fail to get 'fout_isp_pll'\n");
		return PTR_ERR(fout_isp_pll);
	}

	mout_isp_pll = clk_get(&pdev->dev, "mout_isp_pll");
	if (IS_ERR(mout_isp_pll)) {
		pr_err("fail to get 'ext_xtal'\n");
		clk_put(fout_isp_pll);
		return PTR_ERR(mout_isp_pll);
	}

	clk_set_parent(mout_isp_pll, fout_isp_pll);
...
static int exynos4415_fimc_is_isp_pll_off(struct platform_device *pdev)
{
	struct clk *mout_isp_pll = NULL;
	struct clk *v_isppll = NULL;
	struct clk *ext_xtal = NULL;
...
static int exynos4415_fimc_is_cfg_clk(struct platform_device *pdev)
{
	...
	/* ISP PLL ON */
	exynos4415_fimc_is_isp_pll_on(pdev);
...
static int exynos4415_fimc_is_clk_off(struct platform_device *pdev)
{
	...
	/* ISP PLL OFF */
	exynos4415_fimc_is_isp_pll_off(pdev);
...
{
#if defined(CONFIG_SOC_EXYNOS4415)
	return exynos4415_fimc_is_clk_gate(clk_gate_id, is_on);
#else
	pr_err("%s : can't find!! \n", __func__);
	return -ENODEV;
#endif
...
int exynos_fimc_is_set_user_clk_gate(u32 group_id,
		...
		struct exynos_fimc_is_clk_gate_info *gate_info) {
#if defined(CONFIG_SOC_EXYNOS4415)
	return exynos4415_fimc_is_set_user_clk_gate(group_id, is_on,
			user_scenario_id, msk_state, gate_info);
#else
	pr_err("%s : can't find!! \n", __func__);
	return -ENODEV;
#endif
