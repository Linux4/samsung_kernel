...
void camera_setpin_4h5(void)
{
	...
	SET_PIN(&s5k4h5, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, 8,
		EXYNOS4_GPM3(4), (2 << 16), "GPM3.4 (CAM_FLASH_EN)", 0, PIN_FUNCTION);
	SET_PIN(&s5k4h5, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, 9,
		EXYNOS4_GPM3(6), (2 << 24), "GPM3.6 (CAM_FLASH_TORCH)", 0, PIN_FUNCTION);
	...
	SET_PIN(&s5k4h5, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, 6,
		EXYNOS4_GPM3(4), 1, "GPM3.4 (CAM_FLASH_EN)", 0, PIN_INPUT);
	SET_PIN(&s5k4h5, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, 7,
		EXYNOS4_GPM3(6), 1, "GPM3.6 (CAM_FLASH_TORCH)", 0, PIN_INPUT);
...
void __init exynos4_universal222ap_camera_init(void)
{
	...
	exynos_fimc_is_sensor_set_platdata(system_rev > 0 ? &s5k6b2 : &s5k6a3,
        ...
        s3c_gpio_setpull(gpio_i2c_data35.scl_pin, S3C_GPIO_PULL_NONE);
        s5p_gpio_set_drvstr(gpio_i2c_data35.scl_pin, S5P_GPIO_DRVSTR_LV1);

        s3c_gpio_cfgpin(gpio_i2c_data35.sda_pin, S3C_GPIO_INPUT);
        s3c_gpio_setpull(gpio_i2c_data35.sda_pin, S3C_GPIO_PULL_NONE);
        s5p_gpio_set_drvstr(gpio_i2c_data35.sda_pin, S5P_GPIO_DRVSTR_LV1);
