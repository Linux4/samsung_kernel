...
static int exynos_power_up_cpu(unsigned int phys_cpu)
{
	...
	unsigned int cluster = MPIDR_AFFINITY_LEVEL(cpu_logical_map(phys_cpu), 1);
...
#define A7_RESET_NUM	3
	...
	if (phys_cpu < 4) {
		if (soc_is_exynos3475() || soc_is_exynos5430()) {
			u32 val;
			int i;
			...
			for (i = 0; i < A7_RESET_NUM; i++) {
				...
				val = __raw_readl(EXYNOS_ARM_CORE_STATUS(4 * cluster + phys_cpu));
				val |= (0xF << 8);
				__raw_writel(val, EXYNOS_ARM_CORE_STATUS(4 * cluster + phys_cpu));

				pr_debug("cpu%d: SWRESEET\n", phys_cpu);

				if (i < A7_RESET_NUM -1)
					__raw_writel(0, EXYNOS_PMU_SPARE2);
				__raw_writel((0x1 << 1), EXYNOS_ARM_CORE_RESET(4 * cluster + phys_cpu));
			}
