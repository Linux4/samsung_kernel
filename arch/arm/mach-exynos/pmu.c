...
void __iomem *exynos4415_list_feed[] = {
	EXYNOS_ARM_CORE_OPTION(0),
	EXYNOS_ARM_CORE_OPTION(1),
	EXYNOS_ARM_CORE_OPTION(2),
	EXYNOS_ARM_CORE_OPTION(3),
	EXYNOS4_ARM_COMMON_OPTION,
	EXYNOS4_TOP_PWR_OPTION,
	EXYNOS4415_CAM_OPTION,
	EXYNOS4415_TV_OPTION,
	EXYNOS4415_MFC_OPTION,
	EXYNOS4415_G3D_OPTION,
	EXYNOS4415_LCD0_OPTION,
	EXYNOS4415_ISP0_OPTION,
	EXYNOS4415_ISP1_OPTION,
	EXYNOS4415_MAUDIO_OPTION,
};

void __iomem *exynos4415_list_diable_wfi_wfe[] = {
	EXYNOS4415_ISP_ARM_OPTION,
};
...
static void exynos4415_init_pmu(void)
{
	unsigned int i;
	unsigned int tmp;

	/* Enable only SC_FEEDBACK */
	for (i = 0 ; i < ARRAY_SIZE(exynos4415_list_feed) ; i++) {
		tmp = __raw_readl(exynos4415_list_feed[i]);
		tmp &= ~(EXYNOS5_USE_SC_COUNTER);
		tmp |= EXYNOS5_USE_SC_FEEDBACK;
		__raw_writel(tmp, exynos4415_list_feed[i]);
	}

	/*
	* Disable WFI/WFE on XXX_OPTION
	*/
	for (i = 0 ; i < ARRAY_SIZE(exynos4415_list_diable_wfi_wfe) ; i++) {
		tmp = __raw_readl(exynos4415_list_diable_wfi_wfe[i]);
		tmp &= ~(EXYNOS_USESTANDBY_WFE |
			EXYNOS_USESTANDBY_WFI);
		__raw_writel(tmp, exynos4415_list_diable_wfi_wfe[i]);
	}
}
	...
	if (soc_is_exynos4210()) {
		...
		*/
		...
		for (i = 0; i < 4; i++) {
			...
			value &= ~EXYNOS_USESTANDBY_WFE;
			value |= EXYNOS_USESTANDBY_WFI;
