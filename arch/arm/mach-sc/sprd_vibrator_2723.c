...
static void vibrator_hw_init(void)
{
	...
	if (vibr_vdd)
		sci_adi_write(ANA_VIBRATOR_CTRL0, vibr_vdd, 0xff);
	else
...
static int __init sprd_init_vibrator(void)
{
	...
	np = of_find_node_by_name(NULL, "sprd-vibrator");
	if (!np) {
		pr_debug("can't find sprd-vibrator\n");
	} else {
		ret = of_property_read_u32(np, "pmic_vibr_ldo_in_use",
					&ldo_in_use);
		ret = of_property_read_u32(np, "vibr_voltage_set",
					&vibr_vdd);
	}
#endif
	vibrator_hw_init();
