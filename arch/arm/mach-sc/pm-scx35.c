...
static void sc8830_power_off(void)
{
	...
	sci_adi_raw_write(ANA_REG_GLB_LDO_DCDC_PD_RTCCLR, 0x2);
	sci_adi_raw_write(ANA_REG_GLB_LDO_DCDC_PD_RTCSET, 0X7fff);
#endif

	mdelay(10000);
	pr_emerg("power off failed! \n");
	BUG();
}
...
void __init sc_pm_init(void)
{
	...
	set_reset_vector();	
        ...
        sci_adi_write(ANA_REG_GLB_LDO_DCDC_PD_RTCSET,rd_data,0x2060);
        rd_data=sci_adi_read(ANA_REG_GLB_LDO_DCDC_PD_RTCSET) ;
        
        //ANA_REG_AND(ANA_REG_GLB_LDO_DCDC_PD_RTCCLR, (~(BIT_LDO_RF1_PD_RTCSET | BIT_LDO_RF2_PD_RTCSET | BIT_DCDC_WRF_PD_RTCSET)));

        rd_data=sci_adi_read(ANA_REG_GLB_LDO_DCDC_PD_RTCCLR) ;
        rd_data=rd_data & (~((0x1 << 13) | (0x1 <<5 ) | (0x1 << 6 )));
        sci_adi_write(ANA_REG_GLB_LDO_DCDC_PD_RTCCLR,rd_data,0xffff);
        rd_data=sci_adi_read(ANA_REG_GLB_LDO_DCDC_PD_RTCCLR) ;

        //ANA_REG_OR(ANA_REG_GLB_LDO_PD_CTRL, (BIT_DCDC_WPA_PD));
        
        rd_data=sci_adi_read(ANA_REG_GLB_LDO_PD_CTRL) ;
        rd_data=rd_data | (0x1 << 11) ;
        sci_adi_write(ANA_REG_GLB_LDO_PD_CTRL,rd_data,0x800);
    rd_data=sci_adi_read(ANA_REG_GLB_LDO_PD_CTRL) ;
