...
static unsigned long __initdata degas_input_pin_config_bring[] = {
...
#ifdef CONFIG_KEYBOARD_PXA27x
/* Matrix keys */
static unsigned int __initdata degas_matrix_key_map[] = {
...
static struct pxa27x_keypad_platform_data __initdata pxa27x_keypad_pdata = {
	.matrix_key_rows		= 2,
	.matrix_key_cols		= 1,
	.matrix_key_map			= degas_matrix_key_map,
	.matrix_key_map_size	= ARRAY_SIZE(degas_matrix_key_map),
...
static void __init degas_input_keyboard_init(void)
{
...
static void __init degas_input_tsp_init(void)
{
	int ret = 0;
...
static void __init degas_input_vibrator_init(void)
{
	int ret;
...
void __init degas_input_init(void)
{
	mfp_config(ARRAY_AND_SIZE(degas_input_pin_config_bring));

	degas_input_keyboard_init();
	degas_input_tsp_init();
	degas_input_vibrator_init();
}