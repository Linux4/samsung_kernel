...
static int sr352_power(struct device *dev, int flag)
{
	...
	if (flag) {
		Cam_Printk("\n--------------------- sr352_power ON Start--------------------\n");

		//Cam_Printk("---sr352_power power ON -----the (1) step-----camera: VT Enable OFF-----------\n");
		gpio_direction_output(pwd_sub_en, 0);	/* disable */

		//Cam_Printk("---sr352_power power ON -----the (2) step-----camera: VT Reset OFF-----------\n");
		gpio_direction_output(pwd_sub_rst, 0);	/* disable */

		//Cam_Printk("---sr352_power power ON -----the (3) step-----camera: LDO ON-----------\n");
		//Cam_Printk("---sr352_power power ON -----the (4) step-----camera: Sensor I/O 1.80V ON-----------\n");
		regulator_enable(V_CAM_IO_1dot8V_LDO11);

		//Cam_Printk("---sr352_power power ON -----the (5-1) step-----camera: Sensor AVDD ON-----------\n");
		regulator_enable(V_3M_A2dot8V_LDO8);

		//Cam_Printk("---sr130pc10_power  power ON -----the (5-2) step-----camera: Sub Sensor AVDD ON-----------\n");
		gpio_direction_output(SUB_EN_VT_2dot8V, 1);	/* enable */

		//Cam_Printk("---sr352_power power ON -----the (6) step-----camera: VT Sensor Core ON-----------\n");
		gpio_direction_output(SUB_EN_VT_1dot8V, 1);	/* enable */

		//Cam_Printk("---sr352_power power ON -----the (7) step-----camera: Main Sensor Core ON 1.20V-----------\n");
		regulator_enable(V_3M_CAM_CORE_1dot2V_BUCK5);
		msleep(2);

		//Cam_Printk("---sr352_power power ON -----the (8) step-----camera: set sensor mclk = 24 MHz-----------\n");
		pxa_ccic_enable_mclk(s5k_cam_mclk_en.pcdev, BUS_MIPI);

		//Cam_Printk("---sr352_power power ON -----the (9) step-----camera: standby ON-----------\n");
		gpio_direction_output(pwd_main_en, 1);	/* enable */
		msleep(2);

		//Cam_Printk(" ---sr352_power power ON -----the (10) step-----camera: reset ON-----------\n");
		...
		Cam_Printk("\n--------------------- sr352_power OFF Start--------------------\n");

		//Cam_Printk("---sr352_power power OFF -----the (1) step-----camera: VT Enable OFF-----------\n");
		gpio_direction_output(pwd_sub_en, 0);	/* disable */

		//Cam_Printk("---sr352_power power OFF -----the (2) step-----camera: VT Reset OFF-----------\n");
		gpio_direction_output(pwd_sub_rst, 0);	/* disable */

		//Cam_Printk("---sr352_power power OFF -----the (3) step-----camera: reset OFF-----------\n");
		gpio_direction_output(pwd_main_rst, 0);	/* disable */
		msleep(2);

		//Cam_Printk("---sr352_power power OFF -----the (4) step-----camera: disable sensor mclk-----------\n");
		pxa_ccic_disable_mclk(s5k_cam_mclk_en.pcdev, BUS_MIPI); // mipi disable

		//Cam_Printk("---sr352_power power OFF -----the (5) step-----camera: standby OFF-----------\n");
		gpio_direction_output(pwd_main_en, 0);	/* disable */

		//Cam_Printk("---sr352_power power OFF -----the (6) step-----camera: LDO OFF-----------\n");
		//Cam_Printk("---sr352_power power OFF -----the (7) step-----camera: Main Sensor Core OFF-----------\n");
		regulator_disable(V_3M_CAM_CORE_1dot2V_BUCK5);

		//Cam_Printk("---sr352_power power OFF -----the (8) step-----camera: VT Sensor Core OFF-----------\n");
		...
		//Cam_Printk("---sr352_power power OFF -----the (10) step-----camera:  Sensor I/O OFF-----------\n");
...
#define SENSOR_SDA	mfp_to_gpio(GPIO020_GPIO_20)
#define SENSOR_SCL	mfp_to_gpio(GPIO019_GPIO_19)
...
static struct i2c_board_info camera_i2c_r01[] = {
	{
...
static struct soc_camera_link iclink[] = {
	...
	[ID_SR352] = {
		.bus_id			= 0, /* Must match with the camera ID */
		.board_info		= &camera_i2c[ID_SR352],
...
static struct soc_camera_link iclink_r01[] = {
	[ID_SR130PC10] = {
		.bus_id			= 0, /* Must match with the camera ID */
		.board_info		= &camera_i2c_r01[ID_SR130PC10],
		.i2c_adapter_id		= 8, //i2c bus 8 (Emulated GPIO)
		.power = sr130pc10_power,
		.flags = 0,
		.module_name		= "sr130pc10",
	},
	[ID_SR352] = {
		.bus_id			= 0, /* Must match with the camera ID */
		.board_info		= &camera_i2c_r01[ID_SR352],
		.i2c_adapter_id		= 0, // i2c 0 (HW GPIO)
		.power = sr352_power,
		.flags = V4L2_MBUS_CSI2_LANES,
		.module_name		= "sr352",
	},
...
static struct platform_device __attribute__((unused)) camera_r01[] = {
	[ID_SR130PC10] = {
		.name	= "soc-camera-pdrv",
		.id	= 0,
		.dev	= {
			.platform_data = &iclink_r01[ID_SR130PC10],
		},
	},
	[ID_SR352] = {
		.name	= "soc-camera-pdrv",
		.id	= 1,
		.dev	= {
			.platform_data = &iclink_r01[ID_SR352],
		},
	},
};
...
static struct platform_device *camera_platform_devices_r01[] = {
#if defined(CONFIG_SOC_CAMERA_SR130PC10)
	&camera_r01[ID_SR130PC10],
#endif
#if defined(CONFIG_SOC_CAMERA_SR352)
	&camera_r01[ID_SR352],
#endif
