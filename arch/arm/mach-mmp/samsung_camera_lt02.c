...
static int sr200pc20m_power(struct device *dev, int flag)
{
	...
	if (flag) {
        ...
        /* i2c_pxa_reset may be NULL,so you must check here,zc,2013/4/4 */
        if(NULL != s5k_cam_mclk_en.i2c_pxa_reset)
        {
...
static struct soc_camera_link iclink_r01[] = {
	...
	[ID_SR200PC20M] = {
		.bus_id			= 0, /* Must match with the camera ID */
		.board_info		= &camera_i2c_r01[ID_SR200PC20M],
		.i2c_adapter_id		= 8,//i2c bus 8
		.power = sr200pc20m_power,
		.flags = V4L2_MBUS_CSI2_LANES,
		.module_name		= "samsung_mainsensor",
	},
...
static struct platform_device __attribute__((unused)) camera_r01[] = {
	...
	[ID_SR200PC20M] = {
		...
		.id	= 1,
		.dev	= {
			.platform_data = &iclink_r01[ID_SR200PC20M],
		},
	},
};
