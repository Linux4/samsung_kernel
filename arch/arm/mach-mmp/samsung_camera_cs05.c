...
struct device *dev_r; // Used both s5k4ecgx_jcam.c and samsung_camera_cs05.c
struct device *dev_f; // Used both s5k4ecgx_jcam.c and samsung_camera_cs05.c
...
static int  switch_mclk_gpio_mfp(int pinid) {
	...
	unsigned long gpio_mfps[] = {
		GPIO077_GPIO_77 | MFP_PULL_LOW | MFP_LPM_DRIVE_LOW
	};
...
static int  switch_i2c_gpio_mfp(int pinid) {
	...
	unsigned long gpio_mfps[] = {
		GPIO079_GPIO_79 | MFP_PULL_LOW | MFP_LPM_DRIVE_LOW,
		GPIO080_GPIO_80 | MFP_PULL_LOW | MFP_LPM_DRIVE_LOW,
	};
...
void camera_flash_on_off(int value) // Flash
{
	...
	if(value == POWER_ON || value == RT8547_TORCH_200MA || value == RT8547_TORCH_300MA || value == RT8547_TORCH_400MA)
#else
	if(value == POWER_ON)
#endif
	{
...
#if defined(CONFIG_LEDS_RT8547)
		rear_camera = TRUE;
		rt8547_set_torch_current(value); // Set torch current and current time-out register
		rt8547_set_led_low();
#else
	...
	else
	{
		...
		rear_camera = FALSE;
		rt8547_set_led_off();
		rt8547_set_torch_current(POWER_OFF); // Set default torch current and current time-out register
#else
