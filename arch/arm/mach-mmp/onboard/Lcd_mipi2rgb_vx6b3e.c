...
struct Quicklogic_bridge_info *g_vx6b3e;

void vx6b3e_mipi_out_en(int en)
{
	/*For LP 11 codition of of hi max ldi duing the lcd reset low state*/
	if (en) {
		Quicklogic_i2c_write32(0x11C, 0x984); /*Set HS Mode*/
		Quicklogic_i2c_write32(0x300, 0x1); /*Enable MIPI Out*/
	} else {
		Quicklogic_i2c_write32(0x300, 0x0); /*Disable MIPI Out*/
		Quicklogic_i2c_write32(0x11C, 0x985); /*Set LP Mode*/
	}
}
int vx6b3e_brige_init_with_i2c(void)
{
	...
	usleep_range(1000, 1000); /* For pll locking */
	Quicklogic_i2c_write32(0x700, 0x5C900040);
	...
	Quicklogic_i2c_write32(0x344, 0x13);
	...
	Quicklogic_i2c_write32(0x36C, 0x07070901);
	...
	Quicklogic_i2c_write32(0x380, 0x52E86030);
	Quicklogic_i2c_write32(0x384, 0x28614088);
	Quicklogic_i2c_write32(0x388, 0x001102BD);
	Quicklogic_i2c_write32(0x38C, 0x10600008);
	Quicklogic_i2c_write32(0x394, 0x400852E8);
	/*auto recovery(reset command on LP)
	disable caused by Contention error
	*/
...
int vx6b3e_brige_init(struct pxa168fb_info *fbi)
{
	...
	printk(KERN_INFO "VX6B3E ..END.....\n");

	return 0;
}

int vx6b3e_reset(int on_off)
{

	printk(KERN_INFO "vx6b3e_reset\n");
...
int vx6b3e_power(struct pxa168fb_info *fbi, int on)
{
	...
	printk(KERN_INFO "vx6b3e_power =[%d]\n", on);
...
#if defined(CONFIG_LCD_ESD_RECOVERY)
		esd_det_enable(&pvx6b3e->lcd_esd_det);
#endif
	return 0;
}
...
static int hx8369_cs05_disable(struct pxa168fb_info *fbi)
{
	 ...
	 */
#if defined(CONFIG_LCD_ESD_RECOVERY)
    esd_det_disable(&pvx6b3e->lcd_esd_det);
#endif
#if defined(CONFIG_QUICKLOGIC_BRIDGE)
	/*sleep in*/
	Quicklogic_i2cTomipi_write(DTYPE_DCS_WRITE, QL_VX_LCD_VC,\
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	printk(KERN_INFO "auto_brightness[%d] -> [%d]\n",\
...
#if defined(CONFIG_QUICKLOGIC_BRIDGE)
		if (pvx6b3e->cabc) {
			printk(KERN_INFO "cabc enable!\n");
			Quicklogic_i2cTomipi_write(\
				DTYPE_GEN_LWRITE, QL_VX_LCD_VC,\
				sizeof(hx8369b_dtc_55h),\
					hx8369b_dtc_55h);
		} else {
			printk(KERN_INFO "cabc disable!\n");
			Quicklogic_i2cTomipi_write(\
				DTYPE_GEN_LWRITE, QL_VX_LCD_VC,\
				sizeof(hx8369b_dtc_cabc_dis_55h),\
					hx8369b_dtc_cabc_dis_55h);
		}
#endif
...
static ssize_t vx6b3eRGBOUT_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct Quicklogic_bridge_info *pvx6b3e = g_vx6b3e;
	int value = 0, rc = 0;
	...
	if (rc < 0)
		...
		printk(KERN_INFO "vx6b3eRGBOUT -> [%d]\n", value);

#if defined(CONFIG_QUICKLOGIC_BRIDGE)
		if (value) {
			printk(KERN_INFO "vx6b3eRGBOUT Enable!\n");
			Quicklogic_i2c_write32(0x710, 0x07CD0017);
			Quicklogic_i2c_write32(0x70C, 0x00007FA6);
			Quicklogic_i2c_write32(0x134, 0x00000005);
			Quicklogic_i2c_write32(0x154, 0x00000000);
			Quicklogic_i2c_write32(0x154, 0x80000000);
			usleep_range(1000, 1000); /* For pll locking */
			...
			printk(KERN_INFO "vx6b3eRGBOUT disable!\n");
			Quicklogic_i2c_write32(0x710, 0x07CD0013);
			Quicklogic_i2c_write32(0x70C, 0x00007F86);
			Quicklogic_i2c_write32(0x134, 0x00000015);
			Quicklogic_i2c_write32(0x154, 0x00000000);
			Quicklogic_i2c_write32(0x154, 0x80000000);
			usleep_range(1000, 1000); /* For pll locking */
		}
...
static ssize_t lcd_panelName_show(struct device *dev,\
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "HX8369B_PANEL_BOE");
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	case HX8369B_PANEL_BOE:
		ret = sprintf(buf, "BOE_BF047WVG-100-18P0");
		break;
	default:
...
static DEVICE_ATTR(lcd_MTP, 0644, lcd_MTP_show, NULL);
static DEVICE_ATTR(lcd_panelName, 0644, lcd_panelName_show, NULL);
static DEVICE_ATTR(lcd_type, 0644, lcd_type_show, NULL);
static DEVICE_ATTR(vx6b3eRGBOUT, 0644, NULL, vx6b3eRGBOUT_store);
int calculate_brightness_table(struct Quicklogic_bridge_info *pvx6b3e)
{
	struct Quicklogic_backlight_value *pwm = pvx6b3e->vee_lightValue;

	int brightness = pvx6b3e->current_bl;
	int vx6b3e_brightness = 0;

	if (brightness >= MID_BRIGHTNESS_LEVEL) {
		vx6b3e_brightness  = (brightness - MID_BRIGHTNESS_LEVEL) *
		(pwm->max - pwm->mid) / (MAX_BRIGHTNESS_LEVEL-MID_BRIGHTNESS_LEVEL) + pwm->mid;
	} else if (brightness >= LOW_BRIGHTNESS_LEVEL) {
		vx6b3e_brightness  = (brightness - LOW_BRIGHTNESS_LEVEL) *
		(pwm->mid - pwm->low) / (MID_BRIGHTNESS_LEVEL-LOW_BRIGHTNESS_LEVEL) + pwm->low;
	} else if (brightness >= DIM_BRIGHTNESS_LEVEL) {
		vx6b3e_brightness  = (brightness - DIM_BRIGHTNESS_LEVEL) *
		(pwm->low - pwm->dim) / (LOW_BRIGHTNESS_LEVEL-DIM_BRIGHTNESS_LEVEL) + pwm->dim;
	} else if (brightness > 0)
		vx6b3e_brightness  = pwm->dim;
	else {
		vx6b3e_brightness = 0;
		printk(KERN_INFO "brightness=[%d]:vx5b3d_brightness=[%d]\n",\
			brightness, vx6b3e_brightness);
	}

	return vx6b3e_brightness;
}
...
int hx8369_backlight_updata(void)
{
	...
	hx8369b_backlight_51h[1] = calculate_brightness_table(pvx6b3e);

#if defined(CONFIG_QUICKLOGIC_BRIDGE)
	if (!fbi_global->output_on) {
	/*For first update during lcd resume*/
		Quicklogic_i2cTomipi_write(DTYPE_GEN_LWRITE, QL_VX_LCD_VC,\
		...
		if (!pvx6b3e->cabc)
			Quicklogic_i2cTomipi_write(DTYPE_GEN_LWRITE,\
							QL_VX_LCD_VC,\
				sizeof(hx8369b_dtc_cabc_dis_55h),\
					hx8369b_dtc_cabc_dis_55h);
		else
...
#endif
	printk(KERN_INFO "calculated backlight=[%d][%d]\n",\
			pvx6b3e->current_bl, hx8369b_backlight_51h[1]);
...
{
	dev_dbg(fbi_global->dev, "lcd is %s\n", \
...
static int hx8369_cs05_probe(struct pxa168fb_info *fbi)
{
	...
	printk(KERN_INFO "%s, probe hx8369b\n", __func__);

	/*For Bridge alloc*/
	vx6b3eInfo = kzalloc(sizeof(struct Quicklogic_bridge_info),\
								GFP_KERNEL);
	...
	if (IS_ERR_OR_NULL(vx6b3eInfo->mdnie)) {
		printk(KERN_INFO "failed to create mdnie class\n");
		ret = -EINVAL;
		goto error1;
	}
	vx6b3eInfo->mdnie->dev_attrs = mdnie_attributes;

	vx6b3eInfo->dev_mdnie = device_create(vx6b3eInfo->mdnie, NULL, 0,\
							&vx6b3eInfo, "mdnie");
	...
	if (IS_ERR_OR_NULL(vx6b3eInfo->lcd)) {
		...
		printk(KERN_INFO "Failed to create lcd class!\n");
		ret = -EINVAL;
		goto error1;
	}

	if (device_create_file(&vx6b3eInfo->lcd->dev,\
						&dev_attr_lcd_panelName) < 0)
	...
	if (device_create_file(&vx6b3eInfo->lcd->dev,\
						&dev_attr_vx6b3eRGBOUT) < 0)
		printk(KERN_INFO "Failed to create device file(%s)!\n",\
					dev_attr_vx6b3eRGBOUT.attr.name);
	/*For backlight*/
	vx6b3eInfo->bd = backlight_device_register("panel",\
						vx6b3eInfo->dev_bd,\
						vx6b3eInfo,
						&hx8369_int_pwm_ops, NULL);

	if (device_create_file(&vx6b3eInfo->bd->dev,\
						&dev_attr_auto_brightness) < 0)
		printk(KERN_INFO "Failed to create auto_brightness\n");
	...
	vx6b3eInfo->lcd_panel = get_panel_id();
	vx6b3eInfo->vee_lightValue = &backlight_table[vx6b3eInfo->lcd_panel];
...
#if defined(CONFIG_LCD_ESD_RECOVERY)
	mutex_init(&vx6b3eInfo->esd_lock);

	vx6b3eInfo->lcd_esd_det.name = "Lumiere",
	vx6b3eInfo->lcd_esd_det.mode = ESD_DET_POLLING_BOTHLEVEL,
	vx6b3eInfo->lcd_esd_det.level = ESD_DET_LOW,
	vx6b3eInfo->lcd_esd_det.gpio = 31,
	vx6b3eInfo->lcd_esd_det.state = ESD_DET_NOT_INITIALIZED,
	vx6b3eInfo->prev_screen_state = false;
	vx6b3eInfo->lcd_esd_det.poll_preiod = 1000;
	vx6b3eInfo->lcd_esd_det.pdata = fbi;
	vx6b3eInfo->lcd_esd_det.lock = &vx6b3eInfo->esd_lock;
	vx6b3eInfo->lcd_esd_det.is_active = is_active_for_ESD;
	vx6b3eInfo->lcd_esd_det.recover = pxa168fb_reinit;

	if ((vx6b3eInfo->lcd_panel == 0) || (!lpcharge))
		esd_det_init(&vx6b3eInfo->lcd_esd_det);
#endif
	g_vx6b3e = vx6b3eInfo;
