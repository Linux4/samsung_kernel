...
extern struct pxa168fb_mipi_lcd_driver s6e63m0_lcd_driver;
...
static struct fb_videomode video_modes_emeidkb[] = {
	...
	[0] = {
		...
		.yres = 800,
		.hsync_len = 2,
		.left_margin = 16,   /* left_margin should >=3 */
		.right_margin = 16, /* right_margin should >=62 */
		.vsync_len = 2,
		.upper_margin = 3,  /* upper_margin should >= 6 */
		.lower_margin =128,  /* lower_margin should >= 6 */
		...
		unsigned int spi_gpio_reset, int on)
{
	...
	if (!lcd_avdd) {
		lcd_avdd = regulator_get(NULL, "v_lcd_3v3");
		if (IS_ERR(lcd_avdd)) {
