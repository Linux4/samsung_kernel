...
static int s6d78a0x_init(struct pxa168fb_info *fbi)
{
	...
	pxa168_dsi_cmd_array_tx(fbi, s6d78a0x_video_display_disable_access_register,
			ARRAY_SIZE(s6d78a0x_video_display_disable_access_register));
	pxa168_dsi_cmd_array_tx(fbi, s6d78a0x_video_sleep_out_cmds,
			ARRAY_SIZE(s6d78a0x_video_sleep_out_cmds));
...
{
	printk("%s\n", __func__);
	msleep(S6D78A0X_SLEEP_OUT_DELAY);
#ifdef  CONFIG_LDI_SUPPORT_MDNIE
	isReadyTo_mDNIe = 1;
	set_mDNIe_Mode(&mDNIe_cfg);
#endif	/* CONFIG_LDI_SUPPORT_MDNIE */
...
static int s6d78a0x_disable(struct pxa168fb_info *fbi)
{
	...
	msleep(S6D78A0X_DISP_OFF_DELAY);
	pxa168_dsi_cmd_array_tx(fbi, s6d78a0x_video_sleep_in_cmds,
			ARRAY_SIZE(s6d78a0x_video_sleep_in_cmds));
	msleep(S6D78A0X_SLEEP_IN_DELAY);
	return 0;
}
