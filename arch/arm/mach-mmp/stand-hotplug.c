...
/* For AT+CORECTRL cmd */
static int corectrl_set(struct device *dev, struct device_attribute *attr,
			const char *buf, size_t count)
{
	u32 val;
	int i, restart_hp = 0;
...
static DEVICE_ATTR(hotplug_lock, S_IRUGO | S_IWUSR | S_IWGRP, NULL, corectrl_set);
