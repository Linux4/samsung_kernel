...
static void do_wdt_reset(const char *cmd)
{
	...
	/* read WDT counter value */
	i = 0;
	do {
		count = readl(watchdog_virt_base + TMR_WVR);
		count2 = readl(watchdog_virt_base + TMR_WVR);

		i++;
		if (i > 1000)
			panic("cannot get correct WDT value!");
	} while (count2 != count);
