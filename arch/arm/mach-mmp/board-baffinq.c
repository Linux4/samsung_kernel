...
#if defined(CONFIG_TOUCHSCREEN_MELFAS_MMS144)
void __init board_tsp_init_mms144();
#endif
...
static unsigned long helandkb_pin_config[] __initdata = {
	...
	GPIO001_GPIO_1 | MFP_LPM_INPUT,	/* NC */
...
#define GPIO009_MOTOR_EN	(GPIO009_GPIO_9 | MFP_PULL_LOW ) //  | MFP_PULL_LOW | MFP_LPM_DRIVE_LOW)
...
#define GPIO011_GPI0_TSP_VALID	(GPIO011_GPIO_11 | MFP_LPM_INPUT | MFP_PULL_HIGH)
	...
	GPIO012_GPIO_12 | MFP_LPM_INPUT | MFP_PULL_HIGH,	/* NC */
...
#define GPIO053_AP_AGPS_RESET	(GPIO053_GPIO_53)
	...
	GPIO054_GPIO_54 | MFP_LPM_FLOAT | MFP_PULL_LOW, /* EARSPK_SEL */
#else
...
#define GPIO090_T_FLASH_DETECT	(GPIO090_GPIO_90 | MFP_LPM_INPUT | MFP_PULL_FLOAT)
...
#else
	GPIO091_GPIO_91  | MFP_LPM_INPUT| MFP_PULL_HIGH,   /*NC*/
#endif
...
#define GPIO095_GPIO_BL_PWM	(GPIO095_GPIO_95 | MFP_LPM_DRIVE_HIGH)
	GPIO095_GPIO_BL_PWM,
...
#else
	GPIO126_GPIO_126 |MFP_PULL_LOW |  MFP_LPM_PULL_LOW,
...
//yanli 2013.11.11 change the range of ADC as HW ewquest 16~30->15~31 up;31~63->32~61 down ;0~15->0~14 send/end
...
struct d2199_platform_data d2199_pdata = {
	...
	D2199_MCTL_MODE_INIT(D2199_LDO_15, 0x00, D2199_REGULATOR_OFF_IN_DSM), // V_SD_3.3V
	D2199_MCTL_MODE_INIT(D2199_LDO_16, 0x00, D2199_REGULATOR_OFF_IN_DSM), // V_MMC_3.3V
...
#else	
	int motor_en;
	motor_en = mfp_to_gpio(GPIO009_GPIO_9);
...
struct gpio_keys_button gpio_keys_rev00[] = 
{
	{
...
struct gpio_keys_platform_data gpio_keys_data_rev00 = 
{
	.buttons = gpio_keys_rev00,
	.nbuttons = ARRAY_SIZE(gpio_keys_rev00),
};

struct platform_device gpio_keys_device_rev00 = 
{
	.name = "gpio-keys",
	.dev = 
	{
		.platform_data = &gpio_keys_data_rev00,
	},
};
...
static int gps_enable_control(int flag)
{
	...
	gps_power_en = mfp_to_gpio(GPIO009_GPIO_9);
...
static void __init helandkb_init(void)
{
	...
	if(system_rev == 0x00)
		platform_device_register(&gpio_keys_device_rev00);
	else
...
void dvs_setting_initialize(void)
{
   ...
   static unsigned long dvs_init_output[][2]={
		{GPIO096_GPIO_96 | MFP_PULL_FLOAT, 0},
		{GPIO126_GPIO_126,0},
		{GPIO127_GPIO_127,0},
   };
   mfp_config(ARRAY_AND_SIZE(dvs_init_input));
    for(cnt=0; dvs_init_input[cnt]!=0; cnt++)
	{
   	  gpio = mfp_to_gpio(dvs_init_input[cnt]);
	  gpio_request(gpio, NULL);
	  gpio_direction_input(gpio);
	  gpio_free(gpio);
	  udelay(4);
   	}
	len = sizeof(dvs_init_output)/sizeof(dvs_init_output[0]);
	for(cnt=0; cnt<len; cnt++)
	{
		mfp_config(dvs_init_output[cnt], 1);
		gpio = mfp_to_gpio(dvs_init_output[cnt][0]);
		gpio_request(gpio, NULL);
		gpio_direction_output(gpio, dvs_init_output[cnt][1]);
		gpio_free(gpio);
		udelay(4);
	}
}
void  dvs_setting_sleep(void)
{
	int cnt=0, gpio=0, len=0;
	...
	static unsigned long dvs_sleep_output[][2]={
		{GPIO083_GPIO_83,1},
		{GPIO126_GPIO_126,0},
		{GPIO127_GPIO_127,0},
	};
	mfp_config(ARRAY_AND_SIZE(dvs_sleep_input));
	for(cnt=0; dvs_sleep_input[cnt]!=0; cnt++)
	{
		gpio = mfp_to_gpio(dvs_sleep_input[cnt]);
		gpio_request(gpio, NULL);
		gpio_direction_input(gpio);
		gpio_free(gpio);
		udelay(4);
	}
	len = sizeof(dvs_sleep_output)/sizeof(dvs_sleep_output[0]);
	for(cnt=0; cnt<len; cnt++)
	{
		mfp_config(dvs_sleep_output[cnt], 1);
		gpio = mfp_to_gpio(dvs_sleep_output[cnt][0]);
		gpio_request(gpio, NULL);
		gpio_direction_output(gpio, dvs_sleep_output[cnt][1]);
		gpio_free(gpio);
		udelay(4);
	}
}
