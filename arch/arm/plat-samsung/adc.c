...
static inline void dump_adc_register(struct adc_device *adc)
{
	dev_err(&adc->pdev->dev, "Register dump()\n"
		": ADC2_CON1		0x%08x\n"
		": ADC2_CON2		0x%08x\n"
		": ADC2_STATUS		0x%08x\n"
		": ADC2_DAT		0x%08x\n"
		": ADC2_INTEN		0x%08x\n"
		": ADC2_INT_STATUS	0x%08x\n"
		": ADC2_VERSION		0x%08x\n"
		, readl(adc->regs + SAMSUNG_ADC2_CON1)
		, readl(adc->regs + SAMSUNG_ADC2_CON2)
		, readl(adc->regs + SAMSUNG_ADC2_STATUS)
		, readl(adc->regs + SAMSUNG_ADC2_DAT)
		, readl(adc->regs + SAMSUNG_ADC2_INT_EN)
		, readl(adc->regs + SAMSUNG_ADC2_INT_STATUS)
		, readl(adc->regs + SAMSUNG_ADC2_VERSION));
}
...
int s3c_adc_read(struct s3c_adc_client *client, unsigned int ch)
{
	...
	if (ret < 0) {
		s3c_adc_stop(client);
		dev_warn(&adc_dev->pdev->dev, "%s: %p (ch:%d) is timed out\n",
						__func__, client, client->channel);
		dump_adc_register(adc_dev);
		ret = -ETIMEDOUT;
	}
