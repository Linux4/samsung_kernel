...
if [ "${REGION}" == "${CARRIER}" ]; then
	VARIANT_DEFCONFIG=msm8998_sec_${MODEL}_${CARRIER}_defconfig
else
	VARIANT_DEFCONFIG=msm8998_sec_${MODEL}_${REGION}_${CARRIER}_defconfig
fi
...
FUNC_BUILD_KERNEL()
{
    ...
    echo '--base 0x00000000 --cmdline "androidboot.hardware=qcom user_debug=31 msm_rtb.filter=0x37 ehci-hcd.park=3 lpm_levels.sleep_disabled=1 sched_enable_hmp=1 sched_enable_power_aware=1 service_locator.enable=1 swiotlb=2048" --pagesize 4096 --tags_offset 0x01e00000 --ramdisk_offset 0x02000000 --os_version 8.0.0 --os_patch_level 2018-04-01'   >  $PRODUCT_OUT/mkbootimg_ver_args.txt   
