...
BOARD_KERNEL_CMDLINE="printk.devkmsg=on console=null androidboot.console=ttyMSM0 androidboot.hardware=qcom msm_rtb.filter=0x237 ehci-hcd.park=3 lpm_levels.sleep_disabled=1 androidboot.bootdevice=7824900.sdhci androidboot.usbconfigfs=true loop.max_part=7"
...
KERNEL_DEFCONFIG=samsung/${CHIPSET_NAME}_sec_defconfig

if [ "${SEC_BUILD_OPTION_TYPE}" != "user" ]; then
	DEBUG_DEFCONFIG=samsung/${CHIPSET_NAME}_sec_${SEC_BUILD_OPTION_TYPE}_defconfig
fi

if [ "${REGION}" == "${CARRIER}" ]; then
	VARIANT_DEFCONFIG=samsung/${MODEL}/${CHIPSET_NAME}_sec_${MODEL}_${CARRIER}_defconfig
else
	VARIANT_DEFCONFIG=samsung/${MODEL}/${CHIPSET_NAME}_sec_${MODEL}_${REGION}_${CARRIER}_defconfig
fi				
...
KERNEL_MAKE_ENV+=" KCFLAGS=-mno-android"
...
FUNC_BUILD_KERNEL()
{
	...
	cat ${BUILD_KERNEL_OUT_DIR}/arch/${KERNEL_ARCH}/boot/dts/samsung/*.dtb > $PRODUCT_OUT/dtb.img
			
	$BUILD_KERNEL_DIR/tools/mkdtimg create $PRODUCT_OUT/dtbo.img --page_size=${BOARD_KERNEL_PAGESIZE} $BUILD_KERNEL_OUT_DIR/arch/${KERNEL_ARCH}/boot/dts/samsung/samsung-${CHIPSET_NAME}-*-overlay*.dtbo
