...
#elif defined(CONFIG_ARCH_SC) //______________________________________________________________________________
#if defined(CONFIG_SPRD_CPU_DYNAMIC_HOTPLUG)
//unsigned int half_core_number = CONFIG_NR_CPUS / 2;
#define set_core_num_min_limit(value) { \
	pr_debug("[Input Booster2] %s      set_core_num_min_limit : %d\n", glGage, value); \
	if (_this->cpu_num_limit_handle) { \
		cpu_num_min_limit_put(_this->cpu_num_limit_handle); \
		_this->cpu_num_limit_handle = NULL; \
	} \
	_this->cpu_num_limit_handle = _store_cpu_num_min_limit(value, "Input Booster"); \
	if (IS_ERR(_this->cpu_num_limit_handle)) { \
		_this->cpu_num_limit_handle = NULL; \
	} \
}
#define remove_core_num_min_limit() { \
	pr_debug("[Input Booster2] %s      remove_core_num_min_limit : %d\n", glGage, -1); \
	if (_this->cpu_num_limit_handle) { \
		cpu_num_min_limit_put(_this->cpu_num_limit_handle); \
		_this->cpu_num_limit_handle = NULL; \
	} \
...
#define set_frequency(value) { \
	if (_this->cpu_min_handle) { \
		pr_debug("[Input Booster2] %s      this before cpufreq_limit_put in set_frequency mecro : %d\n", glGage, _this); \
		cpufreq_limit_put(_this->cpu_min_handle);  \
		_this->cpu_min_handle = NULL; \
	}\
	pr_debug("[Input Booster2] %s      set_frequency : %d , min_handle : %d\n", glGage, value, _this->cpu_min_handle); \
	_this->cpu_min_handle = cpufreq_limit_min_freq(value, "Input Booster");  \
	if (IS_ERR(_this->cpu_min_handle)) { \
		_this->cpu_min_handle = NULL; \
	} \
}
#define remove_frequency() { \
	if (_this->cpu_min_handle) { \
		pr_debug("[Input Booster2] %s      this before cpufreq_limit_put in remove_frequency mecro  : %d\n", glGage, _this); \
		cpufreq_limit_put(_this->cpu_min_handle);  \
		_this->cpu_min_handle = NULL; \
	}\
}
...
#define SET_BOOSTER  { \
	pr_debug("[Input Booster2] %s      set_freq_limit : %d\n", glGage, _this->param[_this->index].cpu_freq); \
	set_core_num_min_limit(_this->param[_this->index].core_num_lock) \
	set_frequency(_this->param[_this->index].cpu_freq) \
}
#define REMOVE_BOOSTER  { \
	pr_debug("[Input Booster2] %s      set_freq_limit : %d\n", glGage, -1); \
	remove_frequency() \
	remove_core_num_min_limit() \
}
#define PROPERTY_BOOSTER(_device_param_, _dt_param_, _time_)  { \
	_device_param_.cpu_freq = _dt_param_.cpu_freq; \
	_device_param_.core_num_lock = _dt_param_.core_num_lock; \
	_device_param_.time = _dt_param_._time_; \
}
#define SYSFS_DEFINE(_A_, _B_) \
	_A_ cpu_freq _B_ ; \
	_A_ core_num_lock _B_ ; \
	_A_ head_time _B_ ; \
	_A_ tail_time _B_ ; 
#define SYSFS_COPY_TO_FROM(_A_, _B_) { \
	_A_ cpu_freq = (*cpu_freq == (unsigned int)(-1)) ? _B_ cpu_freq : *cpu_freq; \
	_A_ core_num_lock = (*core_num_lock == (unsigned int)(-1)) ? _B_ core_num_lock : *core_num_lock; \
	_A_ head_time = (*head_time == (unsigned int)(-1)) ? _B_ head_time : *head_time; \
	_A_ tail_time = (*tail_time == (unsigned int)(-1)) ? _B_ tail_time : *tail_time; \
}
#define SYSFS_COPY_FROM(_A_) { \
	cpu_freq = _A_ cpu_freq; \
	core_num_lock = _A_ core_num_lock; \
	head_time = _A_ head_time; \
	tail_time = _A_ tail_time; \
}
#define DTSI_TO { \
	err |= of_property_read_u32_index(cnp, "input_booster,cpu_freqs", i, &dt_infor->param_tables[i].cpu_freq); \
	err |= of_property_read_u32_index(cnp, "input_booster,core_num_lock", i, &dt_infor->param_tables[i].core_num_lock); \
	err |= of_property_read_u32_index(cnp, "input_booster,head_times", i, &temp); dt_infor->param_tables[i].head_time = (u16)temp; \
	err |= of_property_read_u32_index(cnp, "input_booster,tail_times", i, &temp); dt_infor->param_tables[i].tail_time = (u16)temp; \
}

#elif defined(CONFIG_ARCH_MTK_PROJECT) //______________________________________________________________________________
...
#define SET_BOOSTER  { \
	pr_debug("[Input Booster2] %s      mt_ppm_sysboost_core : %d\n", glGage, _this->param[_this->index].core_num_lock); \
	pr_debug("[Input Booster2] %s      mt_ppm_sysboost_freq : %d\n", glGage, _this->param[_this->index].cpu_freq); \
	mt_ppm_sysboost_core(BOOST_BY_PERFSERV, _this->param[_this->index].core_num_lock); \
	mt_ppm_sysboost_freq(BOOST_BY_PERFSERV, _this->param[_this->index].cpu_freq); \
	vcorefs_request_dvfs_opp(KIR_SYSFS, OPPI_PERF);  /* request to HPM, 1866/0.8V */  \
}
#define REMOVE_BOOSTER  { \
	pr_debug("[Input Booster2] %s      mt_ppm_sysboost_core : %d\n", glGage, 0); \
	pr_debug("[Input Booster2] %s      mt_ppm_sysboost_freq : %d\n", glGage, 0); \
	...
	vcorefs_request_dvfs_opp(KIR_SYSFS, OPPI_UNREQ);   /*release request*/ \
}
#define PROPERTY_BOOSTER(_device_param_, _dt_param_, _time_)  { \
	_device_param_.cpu_freq = _dt_param_.cpu_freq; \
	_device_param_.core_num_lock = _dt_param_.core_num_lock; \
...
#define DTSI_TO { \
	err |= of_property_read_u32_index(cnp, "input_booster,cpu_freqs", i, (u32 *)&dt_infor->param_tables[i].cpu_freq); \
	err |= of_property_read_u32_index(cnp, "input_booster,core_num_lock", i, (u32 *)&dt_infor->param_tables[i].core_num_lock); \
	err |= of_property_read_u32_index(cnp, "input_booster,head_times", i, (u32 *)&temp); dt_infor->param_tables[i].head_time = (u16)temp; \
	err |= of_property_read_u32_index(cnp, "input_booster,tail_times", i, (u32 *)&temp); dt_infor->param_tables[i].tail_time = (u16)temp; \
}
...
#define GET_BOOSTER_PARAM(_GENDER_, _HEAD_PARAM_, _TAIL_PARAM_) { \
	...
	for(j = 0;j < (int)(sizeof(levels)/(3*sizeof(int)));j++) {\
		if(_GENDER_ == NULL || _GENDER_->pDT == NULL){\
			break;\
		}\
...
#define CHANGE_BOOSTER { \
	...
	if(dt_gender != NULL && dt_gender->pDT != NULL){\
...
#if defined(CONFIG_ARCH_SC) //______________________________________________________________________________
	struct cpufreq_limit_handle 		*cpu_min_handle;
#if defined(CONFIG_SPRD_CPU_DYNAMIC_HOTPLUG)
	struct cpu_num_min_limit_handle 	*cpu_num_limit_handle;
#endif
#endif
	int index;
	int multi_events;
...
#elif defined(CONFIG_ARCH_SC) //______________________________________________________________________________
SYSFS_CLASS(debug_level, (buf, "%u\n", debug_level), 1)
SYSFS_CLASS(head, (buf, "%u %u %u\n", head_time, cpu_freq, core_num_lock), 3)
SYSFS_CLASS(tail, (buf, "%u %u %u\n", tail_time, cpu_freq, core_num_lock), 3)
SYSFS_CLASS(level, (buf, "%d\n", level), 1)
SYSFS_DEVICE(level, (buf, "%d\n", level), 1)
SYSFS_DEVICE(freq, (buf, "%d %u %u\n", level, cpu_freq, core_num_lock), 3)
SYSFS_DEVICE(time, (buf, "%d %u %u\n", level, head_time, tail_time), 3)
#elif defined(CONFIG_ARCH_MTK_PROJECT)
SYSFS_CLASS(debug_level, (buf, "%u\n", debug_level), 1)
SYSFS_CLASS(head, (buf, "%u %u %u\n", head_time, cpu_freq, core_num_lock), 3)
