...
enum power_supply_charger_type {
	...
	POWER_SUPPLY_USB_CHARGER_TYPE_AFC,//add afc type by guoyanjun
...
enum cm_event_types {
	...
	CM_EVENT_INT,
	CM_EVENT_AFC_CHARGE,//add afc event enum by guoyanjun
	CM_EVENT_OTHERS,
};
...
enum wt_charge_status {
	WT_CHARGE_ATO_DISCHG = BIT(0),
	WT_CHARGE_DEMO_DISCHG = BIT(1),
	WT_CHARGE_SYS_DISCHG = BIT(2),
	WT_CHARGE_BATOVP_DISCHG = BIT(3),
	WT_CHARGE_SLATE_DISCHG = BIT(4),
	WT_CHARGE_FULL_CAP_DISCHG = BIT(5), //bug761719,wujishuai.wt,20220609,add full capacity node
};
...
struct charger_manager {
	...
	struct delayed_work wt_update_work;
	struct delayed_work uvlo_work;
	struct delayed_work ir_compensation_work;
	struct delayed_work cp_work;
	struct delayed_work	fcc_stepper_work; //ExtB[P220411-01778] tankaikun.wt, add, 20220413. Add charger current step control.
	int emergency_stop;
	...
	bool wt_charger_enabled;
	u8 wt_charging_status;
	...
	bool is_pd_charger;
	//-ExtB[P220318-04043] yuecong.wt.20220413 modify pd charger trigger charging VI animation
	bool shipmode_when_shutdown; //bug781105,wujishuai.wt,20220712,add shipmode node
};
