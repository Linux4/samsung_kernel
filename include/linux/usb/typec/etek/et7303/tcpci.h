...
#include <linux/usb/typec/etek/et7303/misc/wakelock.h>
#include <linux/err.h>
#include <linux/cpu.h>
...
extern void tcpc_pd_update_src_cap(struct tcpc_device *tcpc, struct pd_port_power_capabilities *src_cap);
extern int tcpc_pd_init(struct tcpc_device *tcpc);
#endif
...
#if IS_ENABLED(CONFIG_PDIC_POLICY)
	if (mv > 0)
		pdic_policy_send_msg(tcpc->pp_data, MSG_VBUSON, 0, 0);
	else
		pdic_policy_send_msg(tcpc->pp_data, MSG_VBUSOFF, 0, 0);
#endif /* CONFIG_PDIC_POLICY */
