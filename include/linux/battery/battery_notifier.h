...
typedef enum battery_notifier_id_e{
	CHARGER_NOTIFY = 0,
	PDIC_NOTIFY,
} battery_notifier_id_t;

/* CHARGER notifier call sequence,largest priority number device will be called first. */
typedef enum charger_notifier_device_e{
...
typedef enum charger_notifier_event_e{
...
/* charger notifier register/unregister API for used any where want to receive charger attached device attach/detach. */
...
/* PDIC notifier register/unregister API for used any where want to receive charger attached device attach/detach. */

/* PDIC notifier call sequence, largest priority number device will be called first. */

typedef enum pdic_notifier_device_e{
...
typedef enum pdic_notifier_event_e{
