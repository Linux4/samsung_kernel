 ...
 *-----------------------------------
 */
extern int ccic_core_init(void);
extern int ccic_core_register_chip(pccic_data_t pccic_data);
extern void ccic_core_unregister_chip(void);
extern int ccic_register_switch_device(int mode);
extern void ccic_send_dock_intent(int type);
extern void ccic_send_dock_uevent(u32 vid, u32 pid, int state);
extern void *ccic_core_get_drvdata(void);
extern struct device *get_ccic_device(void);
/* ----------------------------------
 *          pdic_misc.c functions 
 *-----------------------------------
 */
extern int ccic_misc_init(pccic_data_t pccic_data);
extern void ccic_misc_exit(void);
/* SEC UVDM Utility function */
extern void set_endian(char *src, char *dest, int size);
extern int get_checksum(char *data, int start_addr, int size);
extern int set_uvdmset_count(int size);
extern void set_msg_header(void *data, int msg_type, int obj_num);
extern void set_uvdm_header(void *data, int vid, int vdm_type);
extern void set_sec_uvdm_header(void *data, int pid, bool data_type, int cmd_type,
		bool dir, int total_set_num, uint8_t received_data);
extern int get_data_size(int first_set, int remained_data_size);
extern void set_sec_uvdm_tx_header(void *data, int first_set, int cur_set, int total_size,
		int remained_size);
extern void set_sec_uvdm_tx_tailer(void *data);
extern void set_sec_uvdm_rx_header(void *data, int cur_num, int cur_set, int ack);
#endif
