...
TRACE_EVENT(exynos_devfreq_in,
	TP_PROTO(u32 flag, u32 oldfreq, u32 newfreq),

	TP_ARGS(flag, oldfreq, newfreq),
	...
	TP_fast_assign(
		__entry->flag = flag;
		__entry->oldfreq = oldfreq;
		__entry->newfreq = newfreq;
	),

	TP_printk("name: %s(%u) freq: %u --> %u",
		show_freq_name(__entry->flag), __entry->flag, __entry->oldfreq, __entry->newfreq)
);

TRACE_EVENT(exynos_devfreq_out,
	TP_PROTO(u32 flag, u32 oldfreq, u32 newfreq),
...
TRACE_EVENT(exynos_cpufreq_in,
	...
	TP_printk("cur:%u freq: %u --> %u",
		__entry->flag, __entry->oldfreq, __entry->newfreq)
);

TRACE_EVENT(exynos_cpufreq_out,
	TP_PROTO(u32 flag, u32 oldfreq, u32 newfreq),
