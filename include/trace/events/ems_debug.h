...
TRACE_EVENT(gsc_decision_activate_check,

	TP_PROTO(struct task_struct *p, unsigned long group_util, u64 now, u64 last_update_time, char *label),

	TP_ARGS(p, group_util, now, last_update_time, label),
	...
	TP_fast_assign(
		memcpy(__entry->comm, p->comm, TASK_COMM_LEN);
		__entry->pid		= p->pid;
		__entry->util			= ml_task_util_est(p);
	...
	TP_printk("comm=%s pid=%d task_util=%lu group_util=%lu now=%llu last_uptime=%llu reason=%s",
		__entry->comm, __entry->pid, __entry->util, __entry->group_util, __entry->now,
