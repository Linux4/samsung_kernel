...
static void store_log(struct dentry *dentry, struct inode *inode, struct path *path, int type)
{
	...
	if (type == DLOG_MM) {
		fslog_dlog_mm("\"%s\" (%lu, %lu)\n", full_path,
				path->dentry->d_inode->i_ino, ino);
		goto out;
	}

	if (type == DLOG_ETC) {
		fslog_dlog_etc("\"%s\" (%lu, %lu)\n", full_path,
				path->dentry->d_inode->i_ino, ino);
		goto out;
	}
...
void dlog_hook(struct dentry *dentry, struct inode *inode, struct path *path)
{
	...
	if (!is_ext(dentry->d_name.name, '.', &ht[DLOG_HT_EXCEPTION])
		...
		store_log(dentry, inode, path, DLOG_ETC);		
...
void dlog_hook_rmdir(struct dentry *dentry, struct path *path)
{
	...
	if (is_core_dir(dentry->d_name.name, &ht[DLOG_HT_CORE_DIR]))
		store_log(dentry, NULL, path, DLOG_RMDIR);
