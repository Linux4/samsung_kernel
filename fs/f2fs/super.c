...
void f2fs_set_sb_extra_flag(struct f2fs_sb_info *sbi, int flag)
{
	...
	if (__sync_dirty_buffer(bh, REQ_SYNC | REQ_FUA))
		f2fs_info(sbi, KERN_WARNING, "extra_flag: EIO");
...
{
	block_t limit = min((sbi->user_block_count << 1) / 100,
...
skip:
	...
	*flags = (*flags & ~MS_LAZYTIME) | (sb->s_flags & MS_LAZYTIME);
	f2fs_info(sbi, KERN_NOTICE, "re-mounted. Opts: %s", data);
...
reset_checkpoint:
	...
	f2fs_notice(sbi, KERN_NOTICE, "Mounted with checkpoint version = %llx"
