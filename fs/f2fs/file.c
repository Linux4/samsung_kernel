...
static int f2fs_ioc_getflags(struct file *filp, unsigned long arg)
{
	...
	struct f2fs_inode_info *fi = F2FS_I(inode);
	unsigned int flags = fi->i_flags & (FS_FL_USER_VISIBLE | F2FS_CORE_FILE_FL);
...
static int f2fs_ioc_setflags(struct file *filp, unsigned long arg)
{
	...
	flags = flags & (FS_FL_USER_MODIFIABLE | F2FS_CORE_FILE_FL);
	flags |= oldflags & ~(FS_FL_USER_MODIFIABLE | F2FS_CORE_FILE_FL);
	fi->i_flags = flags;
