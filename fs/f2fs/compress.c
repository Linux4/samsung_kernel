					...
					enum iostat_type io_type)
{
	...
	if (IS_NOQUOTA(inode)) {
		/*
		 * We need to wait for node_write to avoid
		 * block allocation during checkpoint. This
		 * can only happen to quota writes which can
		 * cause the below discard race condition.
		 */
