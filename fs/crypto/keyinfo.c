...
static int find_and_derive_key(const struct inode *inode,
		...
		const struct fscrypt_mode *mode) __attribute__((unused));
#endif
...
static int find_and_derive_key_iv(const struct inode *inode,
					const struct fscrypt_context *ctx,
					u8 *derived_key, const struct fscrypt_mode *mode, u8 *iv_key)
{
	struct key *key;
	const struct fscrypt_key *payload;
	int err;
	...
	if (ctx->flags & FS_POLICY_FLAG_DIRECT_KEY) {
		...
		err = fscrypt_sec_get_key_aes(payload->raw, ctx, derived_key, mode->keysize, iv_key);
	}
	up_read(&key->sem);
	key_put(key);
	return err;
					...
					struct fscrypt_info *ci)
{
	...
	return find_and_derive_key_iv(inode, ctx, derived_key, mode, iv_key);
#else
