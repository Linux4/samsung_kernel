...
int fscrypt_get_encryption_info(struct inode *inode)
{
	...
	if ((FSCRYPT_SDP_PARSE_FLAG_SDP_ONLY(ctx.knox_flags) & FSCRYPT_KNOX_FLG_SDP_MASK)) {
		...
		if (fscrypt_sdp_is_classified(crypt_info)) {
			res = derive_fek(inode, &ctx, crypt_info, crypt_info->ci_raw_key, mode->keysize);
			if (res)
				goto out;
...
#ifdef CONFIG_FSCRYPT_SDP
sdp_dek:
	memcpy(temp_raw_key, crypt_info->ci_raw_key, mode->keysize);
#endif
...
do_ice:
	...
	if (crypt_info == NULL) //Call only when i_crypt_info is loaded initially
		fscrypt_sdp_finalize_tasks(inode, temp_raw_key, (res ? res : mode->keysize));
