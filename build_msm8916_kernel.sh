...
BUILD_COMMAND=$1
if [ "$BUILD_COMMAND" == "rossa_spr" ]; then
...
elif [ "$BUILD_COMMAND" == "fortuna_aio" ]; then
	PRODUCT_NAME=fortunalteaio;
...
SECFUNC_PRINT_HELP()
{
	...
	echo "$0 \$1 \$2 \$3"
	echo "  \$1 : "
	echo "	for ROSSALTE_USA_SPR use rossa_spr"
	echo "	for ROSSALTE_USA_VZW use rossa_vzw"
	echo "	for ROSSALTE_USA_TFN use rossa_tfn"
	echo "	for ROSSALTE_USA_AIO use rossa_aio"
	echo "	for GRANDPRIMELTE_USA_AIO use fortuna_aio"
