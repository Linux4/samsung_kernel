 ...
 * @reclaim_should_stop: optional function pointer that returns condition
 *                       whether to stop the reclaiming or not.
 *                       If reclaiming is stopped by this condition,
 *                       -EINTR is returned.
 *
...
int reclaim_pages_range(unsigned long start_pfn, unsigned long count,
			int mode, int reason, bool (*reclaim_should_stop)(void))
{
	...
	while (pfn < end_pfn) {
		...
		if (reclaim_should_stop && reclaim_should_stop()) {
			ret = -EINTR;
			goto err;
		}
