        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case CONNECT:
                    ...
                    log("Connecting to " + mDevice+" app Id "+appId);
                    if (appId < 0) {
                        appId= mService.getAppId();
                        log("Connecting to Appid updated  " + mDevice+" app Id "+appId);
                    }
                    if (!mNativeInterface.connectSetDevice(appId, mDevice)) {
                    ...
                    if (mService.okToConnect(mDevice)) {
                        ...
                        Log.w(TAG,
                                "Incoming CsipSetCoordinator Connecting request rejected: "
                                        + mDevice);
                        mNativeInterface.disconnectSetDevice(mService.getAppId(), mDevice);
                    }
                    break;
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case DISCONNECT:
                    ...
                    int appId = message.arg1;
                    if (!mNativeInterface.disconnectSetDevice(appId, mDevice)) {
