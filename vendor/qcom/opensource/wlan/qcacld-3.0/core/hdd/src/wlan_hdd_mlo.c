...
ml_link_state_event_policy[QCA_WLAN_VENDOR_ATTR_LINK_STATE_MAX + 1] = {
...
static uint32_t
hdd_get_ml_link_state_event_len(const struct ml_link_state_info_event *event)
{
	uint32_t len = 0;
	uint32_t info_len = 0;
					      ...
					      struct wlan_objmgr_vdev *vdev)
{
	...
	skb_len = hdd_get_ml_link_state_event_len(link_state_event);
