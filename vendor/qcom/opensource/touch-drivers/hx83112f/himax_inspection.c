...
static int himax_self_test_data_init(void)
{
	...
	char *file_name_2 = himax_mp_card_control_file_name;
	int setting_sz = -1;
	int ret = HX_INSP_OK;
	int err = 0;
	int i = 0;
...
SELF_TEST_FAIL:
    ...
    snprintf(g_file_path, (int)(strlen(HX_RSLT_OUT_PATH)
            + strlen(get_date_time_str())+ strlen(rslt ? "FAIL":"PASS")  + COMBINED_UNDERLINE),
            "%s_%s_%s.txt",HX_RSLT_OUT_PATH,get_date_time_str(),(rslt ? "FAIL":"PASS"));
	file_w_flag = true;
	if (file_w_flag) {
		I("Start to write fs!\n");
...
static uint32_t himax_tp_selftest(void)
{
	int ret1 = HX_INSP_OK;
	uint32_t test_size = hx_s_ic_data->tx_num	* hx_s_ic_data->rx_num
			+ hx_s_ic_data->tx_num + hx_s_ic_data->rx_num;
	...
	ret1 = himax_self_test_data_init();
	if (ret1 > 0) {
		E("%s %d: initialize self test failed\n", __func__, __LINE__);
		if (ret1 == HX_INSP_EFILE) {
			E("%s %d: Self_Test Fail: - Criteria file!\n", __func__, __LINE__);
		}
		goto END;
	}

#if defined(HX_ZERO_FLASH)
	ret1 = hx_s_core_fp._0f_op_file_dirly(g_fw_mp_upgrade_name);
	if (ret1) {
		E("%s %d: upgrade MPFW fail, code[%d]!!\n", __func__, __LINE__, ret1);
		ret1 = HX_INSP_EUPDATE;
		goto UPDATE_MPFW_FAIL;
	}
	if (hx_s_ts->debug_log_level & BIT(4)) {
		hx_s_core_fp._reload_disable(0);
		hx_s_core_fp._power_on_init();
		hx_s_core_fp._read_FW_ver();
		g_mp_fw_ver = (hx_s_ic_data->vendor_cid_maj_ver << 8 | hx_s_ic_data->vendor_cid_min_ver);
	}
#endif
	hx_print_criteria_ver();
#if defined(HX_RW_FILE)
#else
	I("%s %d:No support RW file\n", __func__, __LINE__);
#endif
	...
	for (i = 0; i < HX_CRITERIA_ITEM; i++) {
		...
		if (i < HX_LP_WT_NOISE) {
			if (g_test_item_flag[i] == 1) {
				I("%s %d: %d. %s Start\n", __func__, __LINE__, i, g_hx_inspt_mode[i]);
				ret1 = mpTestFunc(i, test_size);
				I("%s %d: %d. %s End, ret1 = %d\n", __func__, __LINE__, i, g_hx_inspt_mode[i], ret1);

				if (ret1)
					rslt |= 1 << i;

				if (ret1 > HX_INSP_ESPEC)
					goto SELF_TEST_FAIL;
			}
		} else {
			break;
...
SELF_TEST_FAIL:
	...
	snprintf(g_file_path, (int)(strlen(HX_RSLT_OUT_PATH)
			+ strlen(get_date_time_str())+ strlen(rslt ? "FAIL":"PASS")  + COMBINED_UNDERLINE),
			"%s%s_%s.txt",HX_RSLT_OUT_PATH,get_date_time_str(),(rslt ? "FAIL":"PASS"));
	file_w_flag = true;
	if (file_w_flag) {
		I("Start to write fs!\n");
		if (hx_open_file(g_file_path) != NO_ERR) {
...
#if defined(HX_RW_FILE)
	if (file_w_flag) {
		I("%s %d: file OK, now close!\n", __func__, __LINE__);
		hx_close_file();
	} else {
		I("%s %d: file wrong, skip close!\n", __func__, __LINE__);
	}
#endif
...
UPDATE_MPFW_FAIL:
	...
	if (himax_check_mode(HX_RAWDATA)) {
		I("%s %d: try to  Need to back to Normal!\n", __func__, __LINE__);
		himax_switch_mode_inspection(HX_RAWDATA);
		if (hx_s_ts->debug_log_level & BIT(4))
			I("%s %d: start sense on!\n", __func__, __LINE__);
		hx_s_core_fp._sense_on(0);
		if (hx_s_ts->debug_log_level & BIT(4))
			I("%s %d: end sense on!\n", __func__, __LINE__);
		himax_wait_sorting_mode(HX_RAWDATA);
	} else {
		I("%s %d: It has been in Normal!\n", __func__, __LINE__);
		if (hx_s_ts->debug_log_level & BIT(4))
			I("%s %d: start sense on!\n", __func__, __LINE__);
		hx_s_core_fp._sense_on(0);
		if (hx_s_ts->debug_log_level & BIT(4))
	...
	if (rslt == HX_INSP_OK && ret1 == HX_INSP_OK)
		I("%s %d: Self_Test Pass:\n", __func__, __LINE__);
	else
		E("%s %d: Self_Test Fail:\n", __func__, __LINE__);

	I("%s %d: Version : %s\n", __func__, __LINE__, g_str_crtra_ver);

#if defined(HX_ZERO_FLASH)
	if (hx_s_ts->debug_log_level & BIT(4))
		I("%s %d: MP FW CID : 0x%04X\n", __func__, __LINE__, g_mp_fw_ver);
#endif
	if (ret1 == HX_INSP_EUPDATE) {
		E("%s %d: MP FW Update fail!\n", __func__, __LINE__);
		goto UPDATE_FAIL;
	}
...
int lct_himax_tp_selftest_all(void)
{
	size_t ret = 0;
	uint32_t rslt = 0;
	...
	if (hx_s_ts->suspended == 1) {
		E("%s, %d: please do self test in normal active mode\n", __func__, __LINE__);
		return HX_INIT_FAIL;
	}

	if (hx_s_ts->in_self_test == 1) {
		W("%s %d: Self test is running now!\n", __func__, __LINE__);
		return ret;
	}
	...
	if (0 == rslt){
		I("%s %d: himax TP selftest, return 2, pass\n", __func__, __LINE__);
		return 2;	//pass
	} else {
		E("%s %d: himax TP selftest, return 1, fail\n", __func__, __LINE__);
		return 1;	//fail
	}
