...
int lct_cts_tp_selftest_all(void)
{
    ...
    cts_info("lct_cts_tp_selftest_all enter!!!!!!");
    ...
    oem_data->rawdata_test_frames = limit.normal_rawdata_frames;
    oem_data->noise_test_frames = limit.normal_noise_frames;
    oem_data->test_rawdata = limit.normal_rawdata_item;
    oem_data->test_noise = limit.normal_noise_item;
    oem_data->test_open = limit.open_item;
    oem_data->test_short = limit.short_item;
    oem_data->test_comp_cap = limit.comp_cap_item;
    ...
    rawdata_test_param.test_data_buf = oem_data->rawdata_test_data;
    rawdata_test_param.test_data_buf_size = oem_data->rawdata_test_data_buff_size;
    rawdata_test_param.test_data_wr_size = &oem_data->rawdata_test_data_wr_size;

    noise_test_param.test_data_buf = oem_data->noise_test_data;
    noise_test_param.test_data_buf_size = oem_data->noise_test_data_buff_size;
    noise_test_param.test_data_wr_size = &oem_data->noise_test_data_wr_size;

    open_test_param.test_data_buf = oem_data->open_test_data;
    open_test_param.test_data_buf_size = oem_data->open_test_data_buff_size;
    open_test_param.test_data_wr_size = &oem_data->open_test_data_wr_size;

    short_test_param.test_data_buf = oem_data->short_test_data;
    short_test_param.test_data_buf_size = oem_data->short_test_data_buff_size;
    short_test_param.test_data_wr_size = &oem_data->short_test_data_wr_size;

    comp_cap_test_param.test_data_buf = oem_data->comp_cap_test_data;
    comp_cap_test_param.test_data_buf_size = oem_data->comp_cap_test_data_buff_size;
    comp_cap_test_param.test_data_wr_size = &oem_data->comp_cap_test_data_wr_size;

    start_time = ktime_get();

    ret = cts_tcs_get_fw_ver(cts_dev, &fw_version);
    if (ret) {
        cts_err( "Factory test get firmware version failed %d\n", ret);
        fw_version = 0;
    }

    if (limit.int_item) {
        int_pin_test_result = cts_test_int_pin(cts_dev, &int_pin_test_param);
        oem_data->int_pin_test_result = int_pin_test_result;
    }
    if (limit.reset_item) {
        reset_pin_test_result = cts_test_reset_pin(cts_dev, &reset_pin_test_param);
        oem_data->reset_pin_test_result = reset_pin_test_result;
    }
    if (limit.normal_rawdata_item) {
        rawdata_test_result = cts_test_rawdata(cts_dev, &rawdata_test_param);
        oem_data->rawdata_test_result = rawdata_test_result;
    }
    if (limit.normal_noise_item) {
        noise_test_result = cts_test_noise(cts_dev, &noise_test_param);
        oem_data->noise_test_result = noise_test_result;
    }
    if (limit.open_item) {
        open_test_result = cts_test_open(cts_dev, &open_test_param);
        oem_data->open_test_result = open_test_result;
    }
    if (limit.short_item) {
        short_test_result = cts_test_short(cts_dev, &short_test_param);
        oem_data->short_test_result = short_test_result;
    }
    if (limit.comp_cap_item) {
        comp_cap_test_result =
            cts_test_compensate_cap(cts_dev, &comp_cap_test_param);
        oem_data->comp_cap_test_result = comp_cap_test_result;
    }
    ...
    cts_info( "Firmware Version: 0x%04X\n", fw_version);
    ...
    cts_info("lct_cts_tp_selftest_all collect selftest result ret = %d\n!!!", ret);
    if (limit.int_item) {
        sum += ((int_pin_test_result == 0) ? 0 : 1);
    }
    if (limit.reset_item) {
        sum += ((reset_pin_test_result == 0) ? 0 : 1);
    }
    if (limit.normal_rawdata_item) {
        sum += ((rawdata_test_result == 0) ? 0 : 1);
    }
    if (limit.normal_noise_item) {
        sum += ((noise_test_result == 0) ? 0 : 1);
    }
    if (limit.open_item) {
        sum += ((open_test_result == 0) ? 0 : 1);
    }
    if (limit.short_item) {
        sum += ((short_test_result == 0) ? 0 : 1);
    }
    if (limit.comp_cap_item) {
        sum += ((comp_cap_test_result == 0) ? 0 : 1);
    }

    cts_info("show selftest result = %d, note: 0: pass return 2, others: fail return 1\n",sum);
    if (!sum) {
        return 2;//2---PASS
    } else {
        return 1;//1---FAIL
