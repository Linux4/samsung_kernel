...
/** tp ddi start **/
#if RESUME_BY_DDI
extern struct ilitek_resume_ddi ili_ddi;
#endif
/** tp ddi end **/
...
MODULE_PARM_DESC(tpsensor, "ilitek ic vendor");
#endif
int ilitek_tp_module_flage;
...
static int ili_v3_drm_check_dt(struct device_node *np)
{
	...
	for (i = 0; i < count; i++) {
		...
		of_node_put(node);
		if (!IS_ERR(panel)) {
			ILI_INFO("find drm_panel i(%d):node(%s):panel(%p)", i, node->name, panel);
...
static void ilitek_ts_panel_notifier_callback(enum panel_event_notifier_tag tag,
		 struct panel_event_notification *notification, void *client_data)
{
	...
	case DRM_PANEL_EVENT_UNBLANK:
		ILI_INFO("1  Notification type:%d\n", notification->notif_type );
	...
	case DRM_PANEL_EVENT_BLANK:
		ILI_INFO("2  Notification type:%d\n", notification->notif_type );
	...
	case DRM_PANEL_EVENT_BLANK_LP:
		ILI_INFO("3  Notification type: %d\n", notification->notif_type );
		break;
	case DRM_PANEL_EVENT_FPS_CHANGE:
		ILI_INFO("4  Notification type: %d\n", notification->notif_type);
		break;
	default:
		ILI_INFO("5  Notification type: %d\n", notification->notif_type);
		break;
	}

}
static void ilitek_ts_register_for_panel_events(void)
{
	void *cookie = NULL;

	cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
			PANEL_EVENT_NOTIFIER_CLIENT_PRIMARY_TOUCH, active_panel,
			&ilitek_ts_panel_notifier_callback, ilits);
	if (!cookie) {
		pr_err("Failed to register for panel events\n");
		return;
	}

	ILI_INFO("registered for panel notifications panel\n");
...
static int ilitek_plat_probe(void)
{
...
#elif PANEL_EVENT
	if(ili_v3_drm_check_dt(ilits->dev->of_node) < 0)
		ILI_ERR("[ili_v3_drm_check_dt] parse drm-panel fail");
	ilitek_ts_register_for_panel_events();
    ...
    /** tp ddi start **/
#if RESUME_BY_DDI
	ILI_INFO("ili_ddi.flag = %d\n", ili_ddi.flag);
	ili_ddi.flag = 1;
	ili_ddi.ddi_func = ili_resume_by_ddi;
#endif
    /** tp ddi end **/
...
static int __init ilitek_plat_dev_init(void)
{
	...
	if (IS_ERR_OR_NULL(tpsensor)) {
		ILI_ERR("tpsensor ERROR!\n");
		return -ENOMEM;
	} else {
		ILI_INFO("tpsensor = %s\n", tpsensor);
		if (strcmp(tpsensor, "ilitek_ts") == 0) {
			ilitek_tp_module_flage = 1;
			ILI_INFO("TP info: [Vendor]Tianma [IC]ili7807S\n");
		} else if (strcmp(tpsensor, "ILITEK_TM_ILI77600A") == 0) {
			ilitek_tp_module_flage = 2;
			ILI_INFO("TP info: [Vendor]Tianma [IC]ILI77600A\n");
		} else if (strcmp(tpsensor, "ILITEK_INX_ILI77600A") == 0) {
			ilitek_tp_module_flage = 3;
			ILI_INFO("TP info: [Vendor]Innolux [IC]ILI77600A\n");
