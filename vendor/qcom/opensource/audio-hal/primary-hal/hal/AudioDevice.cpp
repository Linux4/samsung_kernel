...
bool speaker_mic = false;

bool route_to_handset_mic = false ;

//zhangsen1 Adapt to the Act
mic_set_t mic_sel;

bool factory_test_mic_check_on_off = false;
...
int AudioDevice::CreateAudioPatch(audio_patch_handle_t *handle,
    ...
    switch (sources[0].type) {
        case AUDIO_PORT_TYPE_DEVICE: // Patch for audio capture or loopback
            if (route_to_handset_mic) {
                route_to_handset_mic = false ;
            } else {
	...
	if(speaker_mic) {
		AHAL_DBG("CreateAudioPatch add speaker-mic");
		speaker_mic = false;
	} else {
		AHAL_DBG("CreateAudioPatch add handset-mic");
...
int AudioDevice::SetParameters(const char *kvpairs) {
    ...
    ret = str_parms_get_str(parms, "factory_test_mic_check", value, sizeof(value));
    if (ret >= 0) {
        if (strcmp(value, "on") == 0) {
			AHAL_DBG("[ZS]factory_test_mic_check on");
			factory_test_mic_check_on_off = true;
        }
    }
    ...
    if (ret >= 0) {
        if (strcmp(value, "off") == 0) {
			AHAL_DBG("[ZS]factory_test_mic_check off");
			factory_test_mic_check_on_off = false;
                        mic_sel = TEST_OFF;
    ...
    if (ret >= 0) {
        if (strcmp(value, "main") == 0) {
			AHAL_DBG("[ZS]SetParameters set handset-mic-act");
            //Switch to a handset-maic-act device
                        mic_sel = TEST_MAIN_MIC;
        }
    }
    ...
    if (ret >= 0) {
        if (strcmp(value, "sub") == 0) {
			AHAL_DBG("[ZS]SetParameters set speaker-mic-act");
            //Switch to a speaker-maic-act device
                        mic_sel = TEST_SUB_MIC;
        }
    }
    ...
    ret = str_parms_get_str(parms, "record_select_mic", value, sizeof(value));
    if (ret >= 0) {
        if (strcmp(value, "speaker-mic") == 0) {
			AHAL_DBG("SetParameters set speaker-mic");
			speaker_mic = true;
        }
    }
