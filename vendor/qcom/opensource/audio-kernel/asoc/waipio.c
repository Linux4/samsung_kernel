...
/*M55 code for QN6887A-38 by yingboyang at 2023/09/07 start*/
#include "codecs/fs19xx/fsm_public.h"
/*M55 code for QN6887A-38 by yingboyang at 2023/09/07 end*/
...
/*M55 code for SR-QN6887A-01-568 by yingboyang at 2023/09/06 start*/
#define WCD_MBHC_HS_V_MAX           1700
/*M55 code for SR-QN6887A-01-568 by yingboyang at 2023/09/06 end*/
				...
				int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		if ((!strcmp(dailink[i].name, LPASS_BE_PRI_MI2S_RX))
				|| (!strcmp(dailink[i].name, LPASS_BE_PRI_MI2S_TX))) {
			pr_info("%s(): update: dailink.name: %s", __func__, dailink[i].name);
			//dailink[i].codecs = mi2s_fsm_codec;
			//dailink[i].num_codecs = ARRAY_SIZE(mi2s_fsm_codec);
			dailink[i].codecs[0].dai_name = cfg->codec_dai_name[0];
			dailink[i].codecs[0].name = cfg->codec_name[0];
			pr_info("%s(): update: codec_dai_name: %s codec_name: %s",
				__func__, dailink[i].codecs[0].dai_name, dailink[i].codecs[0].name);
		}
	}
	...
	struct msm_asoc_mach_data *pdata)
{
	...
	if (!strcmp(match->data, "codec")) {
		...
		if (!rc && val) {
			/*M55 code for QN6887A-38 by yingboyang at 2023/09/07 start*/
			fsm_update_dai_links(msm_mi2s_dai_links, ARRAY_SIZE(msm_mi2s_dai_links));
			/*M55 code for QN6887A-38 by yingboyang at 2023/09/07 end*/
			memcpy(msm_waipio_dai_links + total_links,
					msm_mi2s_dai_links,
