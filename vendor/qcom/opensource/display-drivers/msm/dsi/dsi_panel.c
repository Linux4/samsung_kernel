...
/*M55 code for SR-QN6887A-01-390|QN6887A-401 by hehaoran at 20231027 start*/
#ifdef CONFIG_ARCH_X55
#include "custom_sysfs.h"
#include "dsi_lhbm_alpha_gain.h"
#endif//CONFIG_ARCH_X55
/*M55 code for SR-QN6887A-01-390|QN6887A-401 by hehaoran at 20231027 end*/
...
/*M55 code for QN6887A-401 by hehaoran at 20231027 start*/
#ifdef CONFIG_ARCH_X55
static u32 gs_curr_bl_lvl = 0;
#endif//CONFIG_ARCH_X55
/*M55 code for QN6887A-401 by hehaoran at 20231027 end*/
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	/*M55 code for QN6887A-210 by yubo at 20231024 start*/
	#ifndef CONFIG_ARCH_X55
	...
	#endif
	/*M55 code for QN6887A-210 by yubo at 20231024 end*/
...
/*M55 code for SR-QN6887A-01-432|QN6887A-1550 by hehaoran at 20231122 start*/
#ifdef CONFIG_ARCH_X55
static int dsi_change_aod_lux(struct dsi_panel *panel, u32 bl_lvl)
{
	int brightness = 0;
...
/*M55 code for P231116-06789 by hehaoran at 20231206 end*/
#ifdef CONFIG_CUSTOM_FACTORY_DISPLAY
	brightness = mult_frac(bl_lvl, USER_MAX_BRIGHTNESS, panel->bl_config.bl_max_level) + 1;
#else//CONFIG_CUSTOM_FACTORY_DISPLAY
	brightness = mult_frac(bl_lvl, USER_MAX_BRIGHTNESS, panel->bl_config.bl_max_level / 2) + 1;
#endif//CONFIG_CUSTOM_FACTORY_DISPLAY
/*M55 code for P231116-06789 by hehaoran at 20231206 end*/
	DSI_INFO("current is aod(lp1) mode, change brightness = %d\n", brightness);
	/*M55 code for P231114-07975 by hehaoran at 20231201 start*/
	if (brightness >= 0 && brightness < DSI_AOD_10NIT ) {
		return DSI_AOD_2NIT;
	} else if (brightness >= DSI_AOD_10NIT && brightness < DSI_AOD_30NIT) {
		return DSI_AOD_10NIT;
	} else if (brightness >= DSI_AOD_30NIT && brightness < DSI_AOD_60NIT) {
		return DSI_AOD_30NIT;
	} else if (brightness >= DSI_AOD_60NIT && brightness <= USER_MAX_BRIGHTNESS) {
		return DSI_AOD_60NIT;
	} else {
		DSI_INFO("current is aod(lp1) mode,  brightness change=%d is vailed\n", brightness);
	}
	/*M55 code for P231114-07975 by hehaoran at 20231201 end*/
	return 0;
}

static int dsi_get_aod_mapping(struct dsi_panel *panel, u32 bl_lvl)
{
	bl_lvl = dsi_change_aod_lux(panel, bl_lvl);

	if (strstr(panel->panel_of_node->name, "icna3512_visionox")) {
		switch(bl_lvl) {
			case DSI_AOD_2NIT:
				bl_lvl = 0x88;//2nit
				break;
			...
			case DSI_AOD_60NIT:
				...
				break;
			default:
				DSI_INFO("%s curr bl_lvl = 0x%x is invaild, aod default 30nit will be setted\n", panel->panel_of_node->name,bl_lvl);
				bl_lvl = 0x7FF;
		}
	} else if (strstr(panel->panel_of_node->name, "icna3512_tianma")) {
		switch(bl_lvl) {
			case DSI_AOD_2NIT:
				bl_lvl = 0x111;//2nit
				break;
		...
		DSI_ERR("curr_panel: %s is not support AOD\n", panel->name);
	}
	DSI_INFO("aod is set bl_lvl = 0x%x\n", bl_lvl);
	return bl_lvl;
}
#endif
/*M55 code for SR-QN6887A-01-431|432|QN6887A-1550 by hehaoran at 20231122 end*/
	...
	u32 bl_lvl)
{
	...
	/*M55 code for QN6887A-401|P231114-07975 by hehaoran at 20240112 start*/
	/*M55 code for SR-QN6887A-01-431|432|QN6887A-2055 by hehaoran at 20231208 start*/
	#ifdef CONFIG_ARCH_X55
	gs_curr_bl_lvl = bl_lvl == 0 ? 1 : bl_lvl;
	if (dsi_panel_is_type_oled(panel) &&
		(panel->power_mode == SDE_MODE_DPMS_LP1 ||
		panel->power_mode == SDE_MODE_DPMS_LP2)){
		bl_lvl = dsi_get_aod_mapping(panel, bl_lvl);
	}
	#endif//CONFIG_ARCH_X55
	/*M55 code for SR-QN6887A-01-431|432|QN6887A-2055 by hehaoran at 20231208 end*/
	/*M55 code for QN6887A-401|P231114-07975 by hehaoran at 20240112 end*/
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	#ifdef CONFIG_ARCH_X55
	/*M55 code for SR-QN6887A-01-390 by yuli at 20230904 start*/
	"custom,mdss-dsi-lhbm-on-commands-state",
	"custom,mdss-dsi-lhbm-off-commands-state",
	/*M55 code for SR-QN6887A-01-390 by yuli at 20230904 end*/
	/*M55 code for SR-QN6887A-01-432 by hehaoran at 20231019 start*/
	"custom,mdss-dsi-hbm-on-commands-state",
	"custom,mdss-dsi-hbm-off-commands-state",
	/*M55 code for SR-QN6887A-01-432 by hehaoran at 20231019 end*/
	#endif
	/*M55 code for QN6887A-1361 by yubo at 20231117 end*/
};
...
/*M55 code for P231116-06789 by hehaoran at 20231206 start*/
#ifdef CONFIG_CUSTOM_FACTORY_DISPLAY
		panel->bl_config.bl_max_level = val;
#else//CONFIG_CUSTOM_FACTORY_DISPLAY
		panel->bl_config.bl_max_level = 4095;
#endif//CONFIG_CUSTOM_FACTORY_DISPLAY
/*M55 code for P231116-06789 by hehaoran at 20231206 end*/
	}
...
#else//CONFIG_CUSTOM_FACTORY_DISPLAY
		panel->bl_config.brightness_max_level = 4095;
#endif//CONFIG_CUSTOM_FACTORY_DISPLAY
/*M55 code for P231116-06789 by hehaoran at 20231206 end*/
	}
...
#ifdef CONFIG_ARCH_X55
int mipi_dsi_dcs_set_display_ctrl(struct mipi_dsi_device *dsi, u16 brightness, u8 cmd)
{
	u8 payload[2] = { brightness & 0xff, brightness >> 8 };
	ssize_t err;

	err = mipi_dsi_dcs_write(dsi, cmd,
...
/*M55 code for P231114-07975 by hehaoran at 20240112 start*/
...
#ifdef CONFIG_ARCH_X55
	if (panel->power_mode != SDE_MODE_DPMS_LP2) {
		rc = mipi_dsi_dcs_set_display_ctrl(&panel->mipi_device, ICNA3512_CLOSE_DIMMING,
							MIPI_DCS_WRITE_CONTROL_DISPLAY);
		if (rc < 0) {
			DSI_ERR("%s failed to set ddic diming close!\n",__func__);
		}

		panel->read_bl_lv = dsi_get_aod_mapping(panel, panel->read_bl_lv);
	}
#endif//CONFIG_ARCH_X55
...
/*M55 code for P240118-03676 by hehaoran at 20230127 start*/
	...
	if (panel->power_mode != SDE_MODE_DPMS_LP2) {
		if (panel->bl_config.bl_inverted_dbv) {
			panel->read_bl_lv = (((panel->read_bl_lv & 0xff) << 8) | (panel->read_bl_lv >> 8));
		}

		rc = mipi_dsi_dcs_set_display_brightness(&panel->mipi_device, panel->read_bl_lv);
		if (rc < 0) {
			DSI_ERR("%s failed to set aod lp1 read_bl_lv = 0x%x \n",__func__, panel->read_bl_lv);
		}
	}
#endif//CONFIG_ARCH_X55
/*M55 code for P240118-03676 by hehaoran at 20230127 end*/
...
/*M55 code for P231114-07975 by hehaoran at 20240112 end*/
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	int rc = 0;
/*M55 code for QN6887A-2055 by hehaoran at 20231208 start*/
#ifdef CONFIG_ARCH_X55
	struct mipi_dsi_device *dsi = NULL;
	u32 bl_lvl = 0;
#endif//CONFIG_ARCH_X55
/*M55 code for P240118-03676 by hehaoran at 20230125 start*/
	DSI_INFO("%s +++ \n",__func__);
/*M55 code for P240118-03676 by hehaoran at 20230125 end*/
/*M55 code for QN6887A-2055 by hehaoran at 20231208 end*/
	if (!panel) {
		DSI_ERR("invalid params\n");
		return -EINVAL;
	}
...
/*M55 code for QN6887A-2055|P240118-03676 by hehaoran at 20230124 start*/
#ifdef CONFIG_ARCH_X55
	//3.close ddic dimming
	rc = mipi_dsi_dcs_set_display_ctrl(&panel->mipi_device, ICNA3512_CLOSE_DIMMING,
						MIPI_DCS_WRITE_CONTROL_DISPLAY);
	if (rc < 0) {
		DSI_ERR("%s failed to set ddic diming close!\n",__func__);
	...
	bl_lvl = gs_curr_bl_lvl;
	if (panel->bl_config.bl_inverted_dbv) {
		bl_lvl = (((bl_lvl & 0xff) << 8) | (bl_lvl >> 8));
	}
	rc = mipi_dsi_dcs_set_display_brightness(dsi, bl_lvl);
	if (rc < 0) {
		DSI_ERR("%s failed to set bl_lvl=%d\n",__func__, bl_lvl);
	}

#endif//CONFIG_ARCH_X55
/*M55 code for QN6887A-2055|P240118-03676 by hehaoran at 20230124 end*/
...
/*M55 code for P240118-03676 by hehaoran at 20230125 start*/
#ifdef CONFIG_ARCH_X55
	usleep_range(34 * 1000, 34 * 1001);//1 frame => 33.3~33.4ms
	rc = mipi_dsi_dcs_set_display_ctrl(&panel->mipi_device, ICNA3512_OPEN_DIMMING,
						MIPI_DCS_WRITE_CONTROL_DISPLAY);
	if (rc < 0) {
		DSI_ERR("%s failed to set ddic diming close!\n",__func__);
	}
...
#ifdef CONFIG_ARCH_X55
/*M55 code for QN6887A-401 by hehaoran at 20231027 start*/
int dsi_panel_lhbm_enable(struct dsi_panel *panel)
{
	int rc = 0;
	int gain_index = 0;
	char* temp = NULL;
	...
	cmd = panel->cur_mode->priv_info->cmd_sets[DSI_CMD_SET_LHBM_ON].cmds +
                                            ICNA3512_ALPHA_GAIN_INDEX;
	if (!cmd) {
		DSI_ERR("cmd invalid addr\n");
		return -EINVAL;
	}
	...
	if (!msg) {
		DSI_ERR("msg invalid addr\n");
		return -EINVAL;
	}

	DSI_INFO("gs_curr_bl_lvl = %d\n", gs_curr_bl_lvl);

	if (strstr(panel->panel_of_node->name, "icna3512_visionox")) {
		gain_index = ICNA3512_GVO_ALPHA_GAIN >= gs_curr_bl_lvl ?
                                            ICNA3512_GVO_ALPHA_GAIN - gs_curr_bl_lvl : 0;

		temp = kzalloc(ICNA3512_ALPHA_GAIN_SIZE+1, GFP_KERNEL);
		if (!temp) {
			DSI_ERR("lhbm temp kzalloc mem fail\n");
			return -ENOMEM;
		}
		memcpy(temp, icna3512_gvo_alpha_gain[gain_index], ICNA3512_ALPHA_GAIN_SIZE);
		kfree(msg->tx_buf);
		msg->tx_buf = temp;

	} else if (strstr(panel->panel_of_node->name, "icna3512_tianma")) {
		gain_index = ICNA3512_TM_ALPHA_GAIN >= gs_curr_bl_lvl ?
                                            ICNA3512_TM_ALPHA_GAIN - gs_curr_bl_lvl : 0;
		...
		memcpy(temp, icna3512_tm_alpha_gain[gain_index], ICNA3512_ALPHA_GAIN_SIZE);
		kfree(msg->tx_buf);
		msg->tx_buf = temp;

	} else {
		DSI_ERR("curr_panel: %s is not support LHBM\n",panel->panel_of_node->name);
	}

	mutex_lock(&panel->panel_lock);
	if ((!panel->panel_initialized ) || (!dsi_panel_is_type_oled(panel))) {
		rc = -EINVAL;
		goto exit;
	}
...
/*M55 code for QN6887A-401 by hehaoran at 20231027 end*/
int dsi_panel_lhbm_disable(struct dsi_panel *panel)
{
	int rc = 0;
...
/*M55 code for SR-QN6887A-01-432 by hehaoran at 20231019 start*/
int dsi_panel_hbm_enable(struct dsi_panel *panel)
{
	int rc = 0;
...
int dsi_panel_hbm_disable(struct dsi_panel *panel)
{
	int rc = 0;
...
int dsi_panel_disable(struct dsi_panel *panel)
{
	...
	if (!atomic_read(&panel->esd_recovery_pending)) {
		...
		#ifdef CONFIG_ARCH_X55
		if (atomic_read(&panel->dsi_lhbm_enabled)) {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_LHBM_OFF);
			if (rc) {
				DSI_ERR("[%s] failed to send DSI_CMD_SET_LHBM_OFF cmd, rc=%d\n", panel->name, rc);
			} else {
				atomic_set(&panel->dsi_lhbm_enabled, LHBM_DISENABEL);
			}
		}
		#endif // CONFIG_ARCH_X55
...
int dsi_panel_reset_lock(struct dsi_panel *panel)
{
	int rc = 0;
	if (!panel) {
		DSI_ERR("invalid params\n");
