...
static int dsi_display_parse_boot_display_selection(void)
{
	...
	for (i = 0; i < MAX_DSI_ACTIVE_DISPLAY; i++) {
		...
		if (!pos) {
			DSI_INFO("display name[%s]is not valid\n", disp_buf);
			continue;
		}
...
int dsi_host_transfer_sub(struct mipi_dsi_host *host, struct dsi_cmd_desc *cmd)
{
	...
	if (cmd->ctrl_flags & DSI_CTRL_CMD_BROADCAST) {
		...
		if (((cmd->ctrl_flags & DSI_CTRL_CMD_READ) && rc <= 0) ||
			...
			LCD_ERR(vdd, "cmd_transfer failed, rc=%d, cmd_flags=%x cmd = %x\n",
				   rc, cmd->ctrl_flags, (u8 *)cmd->msg.tx_buf + 0);
			rc = -EINVAL;
		}
#else
		if (rc)
...
static int dsi_display_parse_dt(struct dsi_display *display)
{
	...
	DSI_INFO("ctrl count=%d, phy count=%d\n",
