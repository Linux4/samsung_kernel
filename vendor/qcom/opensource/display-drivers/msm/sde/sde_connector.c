...
{
/*M55 code for QN6887A-1277 by hehaoran at 20231116 start*/
#ifdef CONFIG_ARCH_X55
	u8 *rx_buf = NULL;
	int rx_len = 0;
	unsigned int brightness = 0;
	unsigned char buffer[BL_READ_CMD_LEN] = {0x02, 0x06, 0x01, 0x00, 0x01, 0x00, 0x00, 0x01, 0x52};
	...
	rx_buf = kzalloc(MAX_CMD_RECEIVE_SIZE, GFP_KERNEL);
	if (!rx_buf) {
		return -ENOMEM;
	}

	memset(rx_buf, 0x0, MAX_CMD_RECEIVE_SIZE);
	mutex_lock(&c_conn->lock);
	rx_len = dsi_display_cmd_receive(c_conn->display, buffer + 1,
							BL_READ_CMD_LEN - 1, rx_buf, buffer[0]);
	mutex_unlock(&c_conn->lock);
	if (rx_len <= 0) {
		SDE_ERROR("%s read ddic brightness failed\n",  __func__);
	} else {
		brightness = (rx_buf[0] << 8) + (rx_buf[1] & 0xff);
		SDE_INFO("%s read ddic brightness =%d, rx_buf = 0x%x%x\n",
							__func__, brightness, rx_buf[0], rx_buf[1]);
	}
...
/*M55 code for QN6887A-1277 by hehaoran at 20231116 end*/
}
