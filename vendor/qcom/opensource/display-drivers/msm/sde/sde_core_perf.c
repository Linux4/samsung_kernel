...
static inline bool _sde_core_perf_is_valid_client_vote(struct sde_core_perf_params *perf,
		bool split_vote)
{
	if (perf && split_vote)
		return (perf->core_clk_rate &&
		...
		return (perf->core_clk_rate && perf->bw_ctl[SDE_POWER_HANDLE_DBUS_ID_MNOC]);
		...
		struct sde_core_perf_params *perf)
{
	...
	if (state->active_changed && state->active && !sde_cstate->bw_control) {
		sde_cstate->bw_control = true;
		SDE_DEBUG("crtc:%d forcing bw_control:%d, clk or bw not set in power-on\n",
			       crtc->base.id, sde_cstate->bw_control);
		if (!_sde_core_perf_is_valid_client_vote(perf, sde_cstate->bw_split_vote)) {
...
int __mockable ss_set_normal_sde_core_clk(struct drm_device *ddev)
{
	struct sde_kms *sde_kms;
	struct sde_core_perf *perf;
	u64 clk_rate = 0;
	...
	ret = sde_power_clk_set_rate(perf->phandle, perf->clk_name, clk_rate, 0);
	if (ret)
		SDE_ERROR("failed to set %s clock rate %llu, ret: %d\n",
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (perf_mode == SDE_PERF_MODE_FIXED) {
		...
		ret = sde_power_clk_set_rate(perf->phandle,
				perf->clk_name, perf->max_core_clk_rate, MMRM_CLIENT_DATA_FLAG_RESERVE_ONLY);
		if (ret) {
			SDE_ERROR("failed to set %s clock rate %llu, ret: %d\n",
					perf->clk_name,
