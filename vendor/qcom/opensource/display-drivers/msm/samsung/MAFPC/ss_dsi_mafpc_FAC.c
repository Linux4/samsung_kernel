...
#include "ss_dsi_mafpc_FAC.h"

/*
 * make dsi_panel_cmds using image data
 */
int ss_mafpc_make_img_cmds_FAC(struct samsung_display_driver_data *vdd, char *data,
...
int ss_mafpc_make_img_mass_cmds_FAC(struct samsung_display_driver_data *vdd, char* data,
									u32 data_size, int cmd_type)
{
	struct dsi_cmd_desc *tcmds;
	struct dsi_panel_cmd_set *pcmds;
...
#define BUF_LEN 200
void ss_mafpc_update_enable_cmds_FAC(struct samsung_display_driver_data *vdd)
{
	struct dsi_panel_cmd_set *pcmds;
	...
	cmd_pload = &pcmds->cmds[1].ss_txbuf[7];
...
struct dsi_panel_cmd_set *ss_mafpc_brightness_scale_FAC(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (SS_IS_CMDS_NULL(scale_cmds)) {
		LCD_ERR(vdd, "no Brightness Scale cmd\n");
		return NULL;
	}
...
static int ss_mafpc_img_write(struct samsung_display_driver_data *vdd, bool is_instant)
{
	...
	ss_set_exclusive_tx_packet(vdd, TX_MAFPC_SET_PRE2, 1);
	...
	/* 1 Frame Delay (Worst Case of 48FPS) */
	...
	ss_send_cmd(vdd, TX_MAFPC_SET_PRE2);
	...
	ss_set_exclusive_tx_packet(vdd, TX_MAFPC_SET_PRE2, 0);
...
/* same function with ss_mafpc_brightness_scale_FAC().
 * W 0xB0 0x00 0x09 0x87
 * W 0x87 0xXX 0xXX 0xXX
 */
			 ...
			 size_t total_count, loff_t *ppos)
{
	...
	ss_mafpc_update_enable_cmds_FAC(vdd);
	ss_mafpc_make_img_mass_cmds_FAC(vdd, vdd->mafpc.img_buf, vdd->mafpc.img_size, TX_MAFPC_IMAGE);
...
int ss_mafpc_init_FAC(struct samsung_display_driver_data *vdd)
{
	int ret = 0;
	...
	vdd->mafpc.make_img_mass_cmds = ss_mafpc_make_img_mass_cmds_FAC;
	vdd->mafpc.make_img_cmds = ss_mafpc_make_img_cmds_FAC;
	vdd->mafpc.img_write = ss_mafpc_img_write;
	vdd->mafpc.debug = ss_mafpc_debug;
	vdd->panel_func.br_func[BR_FUNC_MAFPC_SCALE] = ss_mafpc_brightness_scale_FAC;

	vdd->mafpc.brightness_scale_table_size = sizeof(brightness_scale_table);

	vdd->mafpc.enable_cmd_size = MAFPC_ENABLE_COMMAND_LEN_FAC;
	vdd->mafpc.enable_cmd_buf = kzalloc(MAFPC_ENABLE_COMMAND_LEN_FAC, GFP_KERNEL);
	...
	register_op_sym_cb(vdd, "MAFPC", update_mafpc_scale_FAC, true);
