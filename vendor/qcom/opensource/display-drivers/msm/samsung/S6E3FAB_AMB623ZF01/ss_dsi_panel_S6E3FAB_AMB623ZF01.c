...
*/
#include "ss_dsi_panel_S6E3FAB_AMB623ZF01.h"
#include "ss_dsi_mdnie_S6E3FAB_AMB623ZF01.h"
...
static u8 LTPS_SETTING1[VRR_MODE_MAX][8] = {
	...
	{0x32, 0x66, 0x32, 0x66, 0x32, 0x66, 0x32, 0x66},
};

static u8 LTPS_SETTING2[VRR_MODE_MAX][8] = {
	{0x0C, 0x68, 0x0C, 0x68, 0x0C, 0x68, 0x0C, 0x68},
	{0x18, 0x50, 0x18, 0x50, 0x18, 0x50, 0x18, 0x50},
};
...
static u8 VFP_48HZ_SETTING[2] = {0x0D, 0x6C};
static u8 VFP_96HZ_SETTING[2] = {0x02, 0x3F};
static u8 VFP_120HZ_SETTING[2] = {0x00, 0x04};

static u8 VFP_60HZ_SETTING[VRR_MODE_MAX][2] = {
	{0x00, 0x04},	/* 120hs, 60ns */
	{0x08, 0xF4},	/* 60hs */
};
...
static struct dsi_panel_cmd_set *ss_vrr(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (cur_phs) {
		...
		vrr_cmds->cmds[idx].ss_txbuf[2] = 0x80;
		vrr_cmds->cmds[idx].ss_txbuf[3] = 0xEB;
...
static struct dsi_panel_cmd_set * ss_brightness_gamma_mode2_normal(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	pcmds->cmds[idx].ss_txbuf[1] = dim_off ? 0x00 : 0x18;
...
static struct dsi_panel_cmd_set *ss_vint(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (vdd->xtalk_mode)
		...
		vint_cmds->cmds[idx].ss_txbuf[1] = 0x0E; // OFF
			...
			(struct samsung_display_driver_data *vdd, int enable)
{
	...
	struct dsi_panel_cmd_set *set_lpm_bl;
	int lpm_on_idx, lpm_bl_ctrl_idx, wrdisvb_idx;
	int gm2_wrdisbv;
...
{
	u8 valid_checksum[4] = {0xA9, 0x89, 0xA9, 0x89};
	int res;
...
static int ss_gct_write(struct samsung_display_driver_data *vdd)
{
	u8 *checksum;
	int i, idx;
	/* vddm set, 0x0: normal, 0x12: LV, 0x31: HV */
	u8 vddm_set[MAX_VDDM] = {0x0, 0x12, 0x31};
	int ret = 0;
	struct dsi_panel *panel = GET_DSI_PANEL(vdd);
...
{
	struct samsung_display_driver_data *vdd = container_of(vrr , struct samsung_display_driver_data, vrr);
	LCD_INFO(vdd, "+++\n");
...
/* read 60 HS/NS GAMMA value from flash via MIPI */
static int ss_gm2_ddi_flash_prepare(struct samsung_display_driver_data *vdd)
{
	...
	for (i = 0; i < vdd->br_info.gm2_flash_tbl_cnt; i++) {
		fsize = 0xC00000;
		rx_addr = gm2_flash_tbl[i].start_addr;
		rx_size = gm2_flash_tbl[i].buf_len;

		if (i == 0 || i == 1) {
			fsize |= GAMMA_R_SIZE;

			LCD_INFO(vdd, "[%d] buf_len (%d) start_addr (%x) end (%x)\n", rx_size, gm2_flash_tbl[i].end_addr);

			for (addr = rx_addr; addr < gm2_flash_tbl[i].end_addr; addr += GAMMA_R_SIZE) {
				/* read via MIPI */
				LCD_INFO(vdd, "[%d] [%x] [%x]\n", addr - rx_addr, addr, fsize);
				flash_read_bytes(vdd, addr, fsize, GAMMA_R_SIZE, readbuf);
				memcpy(&gm2_flash_tbl[i].raw_buf[addr - rx_addr], readbuf, GAMMA_R_SIZE);
			}
		} else {
			fsize |= rx_size;
			flash_read_bytes(vdd, rx_addr, fsize, rx_size, gm2_flash_tbl[i].raw_buf);
		}
	...
	for (i = GAMMA_SET_MAX - 1, r = 0; i >= GAMMA_SET_0; i--) {
		//memcpy(HS_R_TYPE_BUF[GAMMA_ROOM_60][i], &gm2_flash_tbl[HS].raw_buf[r], GAMMA_R_SIZE);
...
static int ss_gm2_gamma_comp_init(struct samsung_display_driver_data *vdd)
{
	...
	/*****************************************/
	/* 1. read HS120 ORIGINAL MTP GAMMA */
...
#if 0
static char GLOABL_PARA_1[3] = {0xB0, 0x00, 0x00};
static char GLOABL_PARA_2[3] = {0xB0, 0x00, 0x00};
static char GAMMA_1[1+(GAMMA_R_SIZE*2)] = {0x00, };
static char GAMMA_2[1+(GAMMA_R_SIZE*2)] = {0x00, };
...
static struct dsi_panel_cmd_set *ss_brightness_gm2_gamma_comp(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	LCD_INFO(vdd, "COMP romm[%d] - set[%d %d] level [%d]\n",
	...
	if (cur_rr == 120 || (cur_rr == 60 && cur_phs)) {
		...
		LCD_INFO(vdd, "ORIGINAL 120HS gamma\n");
		...
		if (idx >= 0) {
			...
			LCD_INFO(vdd, "COMP 96HS gamma [%d]\n", idx);
		}
	} else if (cur_rr == 60 && cur_hs && !cur_phs) {
		...
		if (idx >= 0) {
			...
			LCD_INFO(vdd, "COMP 60HS gamma [%d]\n", idx);
		}
		...
		if (idx >= 0) {
			...
			LCD_INFO(vdd, "COMP 48HS gamma [%d]\n", idx);
		}
	} else {	/* 60NS */
		set1_data = NS60_R_TYPE_BUF[set1];
		set2_data = NS60_R_TYPE_BUF[set2];
		LCD_INFO(vdd, "ORIGINAL 60NS gamma\n");
	}
	...
	if (idx < 0) {
		...
		set1_data = HS_R_TYPE_BUF[gamma_room][set1];
		set2_data = HS_R_TYPE_BUF[gamma_room][set2];
		LCD_INFO(vdd, "idx [%d] -> use 120 mtp gamma / romm[120]\n", idx);
	}
...
{
	LCD_INFO(vdd, "S6E3FAB_AMB623ZF01 : ++ \n");
