...
*/
#include "B5_S6E3FC5_AMB338EH01_panel.h"
#include "B5_S6E3FC5_AMB338EH01_mdnie.h"
#include "hwparam/B5_S6E3FC5_AMB338EH01.hw_param.h"
...
static char ss_panel_revision(struct samsung_display_driver_data *vdd)
{
	...
	/* ID3[3:0]: revA = 0001, revB = 0010, revC = 0011, revD = 0100 */
...
static int ss_vrr_init(struct vrr_info *vrr)
{
	...
	/* defult: SVGA@60hz NS mode */
	vrr->prev_refresh_rate = vrr->cur_refresh_rate = vrr->adjusted_refresh_rate = 60;
	vrr->prev_sot_hs_mode = vrr->cur_sot_hs_mode = vrr->adjusted_sot_hs_mode = false;
	vrr->prev_phs_mode = vrr->cur_phs_mode = vrr->adjusted_phs_mode = false;
	vrr->max_h_active_support_120hs = 1080; /* supports 120hz until SVGA 1080 */
...
static void update_glut_map(struct samsung_display_driver_data *vdd)
{
	...
	if (ret) {
		/* If not, try parsing from hwparam_header data */
		ret = ss_parse_panel_glut_table(vdd, &vdd->br_info.glut_offset_96hs, GLUT_OFFSET_96HS_V_revA);
		if (ret)
			LCD_ERR(vdd, "Failed to parse GLUT_OFFSET_96HS data\n");
		else
			LCD_INFO(vdd, "Parsing GLUT_OFFSET_96HS data from hwparam_header\n");
	} else
			...
			char *val, struct ss_cmd_desc *cmd)
{
	...
	struct cmd_legoop_map *glut_map = &vdd->br_info.glut_offset_96hs;
	int i = -1, j;
...
static int update_aor_B5_S6E3FC5_AMB338EH01(struct samsung_display_driver_data *vdd,
			char *val, struct ss_cmd_desc *cmd)
{
	struct cmd_ref_state *state = &vdd->cmd_ref_state;
	int cur_rr = state->cur_refresh_rate;
...
{
	LCD_INFO(vdd, "B5_S6E3FC5_AMB338EH01/%s\n", ss_get_panel_name(vdd));
	...
	/* mdnie */
	vdd->mdnie.support_mdnie = true; /* until bringup SEED, disable mdnie */
...
//	vdd->mdnie.mdnie_tune_size[2] = sizeof(BYPASS_MDNIE_3);
	...
	vdd->h_buf = B5_S6E3FC5_AMB338EH01_PDF_DATA;
	vdd->h_size = sizeof(B5_S6E3FC5_AMB338EH01_PDF_DATA);
	...
	register_op_sym_cb(vdd, "AOR", update_aor_B5_S6E3FC5_AMB338EH01, true);
