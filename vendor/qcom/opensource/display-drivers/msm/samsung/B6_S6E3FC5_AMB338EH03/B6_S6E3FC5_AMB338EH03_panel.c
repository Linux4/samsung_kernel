...
*/
#include "B6_S6E3FC5_AMB338EH03_panel.h"
#include "B6_S6E3FC5_AMB338EH03_mdnie.h"
...
#define VIVIDNESS_64h_IDX 2 // 3
#define VIVIDNESS_64h_SIZE 1 // 18

#define VIVIDNESS_65h_IDX1 75 // skintone
#define VIVIDNESS_65h_SIZE1 12

#define VIVIDNESS_65h_IDX2 114 // skintone
#define VIVIDNESS_65h_SIZE2 7
...
static u8 vividness_65h_table1[VIVIDNESS_MAX_IDX][VIVIDNESS_65h_SIZE1] = {
	{0x00, 0x05, 0xD1, 0x00, 0x18, 0x69, 0x01, 0x06, 0x32, 0x01, 0x0B, 0x48}, /* default */
	{0x00, 0x05, 0xD1, 0x00, 0x18, 0x69, 0x01, 0x06, 0x32, 0x01, 0x0B, 0x48},
	{0x00, 0x02, 0xFC, 0x00, 0xFF, 0xFF, 0x01, 0xF0, 0x00, 0x01, 0x08, 0x0A},
};

/* 65h */
static u8 vividness_65h_table2[VIVIDNESS_MAX_IDX][VIVIDNESS_65h_SIZE2] = {
	{0x55, 0x80, 0x80, 0x6E, 0x52, 0xAE, 0x92}, /* default */
	{0x19, 0x80, 0x80, 0x6E, 0x52, 0xAE, 0x92},
	{0x32, 0x80, 0x80, 0x85, 0x52, 0xAE, 0x80},
};
...
static void mdnie_set_vividness(struct samsung_display_driver_data *vdd, struct mdnie_lite_tun_type *tune, struct dsi_cmd_desc *tune_data)
{
	...
	if (idx >= 0 && idx < VIVIDNESS_MAX_IDX) {
		...
		memcpy(&tune_data[mdnie_data->mdnie_step_index[MDNIE_STEP2]].ss_txbuf[VIVIDNESS_65h_IDX2],
			vividness_65h_table2[idx], VIVIDNESS_65h_SIZE2);
	}
}
...
static int update_aor_B6_S6E3FC5_AMB338EH03(struct samsung_display_driver_data *vdd,
			char *val, struct ss_cmd_desc *cmd)
{
	struct cmd_ref_state *state = &vdd->cmd_ref_state;
	int cur_rr = state->cur_refresh_rate;
	...
	LCD_DEBUG(vdd, "bl_idx: %d, bl_lvl: %d, cur_rr: %d, i: %d, aor: 0x%X%X, tx_len: %d\n",
			bl_idx, bl_lvl, cur_rr, i, manual_aor_map->cmds[bl_idx][0],
			manual_aor_map->cmds[bl_idx][1], cmd->tx_len);
...
{
	LCD_INFO(vdd, "B6_S6E3FC5_AMB338EH03/%s\n", ss_get_panel_name(vdd));
...
//	vdd->mdnie.mdnie_tune_size[2] = sizeof(BYPASS_MDNIE_3);
	...
	vdd->h_buf = B6_S6E3FC5_AMB338EH03_PDF_DATA;
	vdd->h_size = sizeof(B6_S6E3FC5_AMB338EH03_PDF_DATA);
	...
	register_op_sym_cb(vdd, "AOR", update_aor_B6_S6E3FC5_AMB338EH03, true);
