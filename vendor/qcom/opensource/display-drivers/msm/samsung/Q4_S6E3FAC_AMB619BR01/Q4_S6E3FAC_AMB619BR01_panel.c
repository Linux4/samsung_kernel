...
*/
#include "Q4_S6E3FAC_AMB619BR01_panel.h"
#include "Q4_S6E3FAC_AMB619BR01_mdnie.h"
...
static int samsung_panel_on_post(struct samsung_display_driver_data *vdd)
{
	...
	if (!SS_IS_CMDS_NULL(pcmds)) {
		...
		memcpy(&pcmds->cmds[idx].ss_txbuf[1], HS60_R_TYPE_COMP[73], GAMMA_R_SIZE);
		memcpy(&pcmds->cmds[idx].ss_txbuf[1 + GAMMA_R_SIZE], HS60_R_TYPE_COMP[255], GAMMA_R_SIZE);
		memcpy(&pcmds->cmds[idx].ss_txbuf[1 + GAMMA_R_SIZE + GAMMA_R_SIZE], HS60_R_TYPE_COMP[510], GAMMA_R_SIZE);
...
#define VFP_SETTING_GPARA_LEN 3
#define VFP_SETTING_CMD_LEN 2
#define TE_SETTING_CMD_LEN 20

static u8 VFP_SETTING_GPARA_120HS[VFP_SETTING_GPARA_LEN] = {0x00, 0x08, 0xF2};
static u8 VFP_SETTING_120HS[VFP_SETTING_CMD_LEN] = {0x00, 0x20};
static u8 VFP_SETTING_120HS_revC[VFP_SETTING_CMD_LEN] = {0x00, 0x1C};

static u8 VFP_SETTING_GPARA_96HS[VFP_SETTING_GPARA_LEN] = {0x00, 0x08, 0xF2};
static u8 VFP_SETTING_96HS[VFP_SETTING_CMD_LEN] = {0x02, 0x70};
static u8 VFP_SETTING_96HS_revC[VFP_SETTING_CMD_LEN] = {0x02, 0x6C};

static u8 VFP_SETTING_GPARA_60HS[VFP_SETTING_GPARA_LEN] = {0x00, 0x08, 0xF2};
static u8 VFP_SETTING_60HS[VFP_SETTING_CMD_LEN] = {0x00, 0x20};
static u8 VFP_SETTING_60HS_revC[VFP_SETTING_CMD_LEN] = {0x00, 0x1C};

static u8 VFP_SETTING_GPARA_60NS[VFP_SETTING_GPARA_LEN] = {0x00, 0x11, 0xF2};
static u8 VFP_SETTING_60NS[VFP_SETTING_CMD_LEN] = {0x00, 0x20};
static u8 VFP_SETTING_60NS_revC[VFP_SETTING_CMD_LEN] = {0x00, 0x1C};

static u8 VFP_SETTING_GPARA_48HS[VFP_SETTING_GPARA_LEN] = {0x00, 0x08, 0xF2};
static u8 VFP_SETTING_48HS[VFP_SETTING_CMD_LEN] = {0x02, 0x70};
static u8 VFP_SETTING_48HS_revC[VFP_SETTING_CMD_LEN] = {0x02, 0x6C};
//static u8 VFP_SETTING_HBM_48HS[VFP_SETTING_CMD_LEN] = {0x02, 0x4C};

#define AOR_SETTING_GPARA_LEN 3
#define AOR_SETTING_CMD_LEN 32
static u8 AOR_SETTING_GPARA_120HS[AOR_SETTING_GPARA_LEN] = {0x00, 0x8B, 0x98};
static u8 AOR_SETTING_120HS[AOR_SETTING_CMD_LEN] = {
	0x09, 0x38, 0x09, 0x38, 0x09, 0x38, 0x09, 0x38,
	0x09, 0x38, 0x09, 0x38, 0x09, 0x38, 0x09, 0x38,
	0x09, 0x38, 0x09, 0x38, 0x09, 0x38, 0x08, 0x4C,
	0x08, 0x4C, 0x01, 0x79, 0x01, 0x79, 0x01, 0x79,
};

static u8 AOR_SETTING_GPARA_96HS[AOR_SETTING_GPARA_LEN] = {0x00, 0x8B, 0x98};
static u8 AOR_SETTING_96HS[AOR_SETTING_CMD_LEN] = {
	0x0B, 0x86, 0x0B, 0x86, 0x0B, 0x86, 0x0B, 0x86,
	0x0B, 0x86, 0x0B, 0x86, 0x0B, 0x86, 0x0B, 0x86,
	0x0B, 0x86, 0x0B, 0x86, 0x0B, 0x86, 0x0A, 0x5F,
	0x0A, 0x5F, 0x01, 0xD8, 0x01, 0xD8, 0x01, 0xD8,
};

static u8 AOR_SETTING_GPARA_60HS[AOR_SETTING_GPARA_LEN] = {0x00, 0xAB, 0x98};
static u8 AOR_SETTING_60HS[AOR_SETTING_CMD_LEN] = {
	0x09, 0x38, 0x09, 0x38, 0x09, 0x38, 0x09, 0x38,
	0x09, 0x38, 0x09, 0x38, 0x09, 0x38, 0x09, 0x38,
	0x09, 0x38, 0x09, 0x38, 0x09, 0x38, 0x08, 0x4C,
	0x08, 0x4C, 0x01, 0x79, 0x01, 0x79, 0x01, 0x79,
...
static u8 AOR_SETTING_GPARA_60NS[AOR_SETTING_GPARA_LEN] = {0x00, 0x6B, 0x98};
static u8 AOR_SETTING_60NS[AOR_SETTING_CMD_LEN] = {
	0x09, 0x38, 0x09, 0x38, 0x09, 0x38, 0x09, 0x38,
	0x09, 0x38, 0x09, 0x38, 0x09, 0x38, 0x09, 0x38,
	0x09, 0x38, 0x09, 0x38, 0x09, 0x38, 0x08, 0x4C,
	0x08, 0x4C, 0x01, 0x79, 0x01, 0x79, 0x01, 0x79,
...
static u8 AOR_SETTING_GPARA_48HS[AOR_SETTING_GPARA_LEN] = {0x00, 0xAB, 0x98};
static u8 AOR_SETTING_48HS[AOR_SETTING_CMD_LEN] = {
	0x0B, 0x86, 0x0B, 0x86, 0x0B, 0x86, 0x0B, 0x86,
	0x0B, 0x86, 0x0B, 0x86, 0x0B, 0x86, 0x0B, 0x86,
	0x0B, 0x86, 0x0B, 0x86, 0x0B, 0x86, 0x0A, 0x5F,
	0x0A, 0x5F, 0x01, 0xD8, 0x01, 0xD8, 0x01, 0xD8,
...
static u8 TE_SETTING_10PHS[TE_SETTING_CMD_LEN] = {
	0x61, 0x00, 0x17, 0x00, 0x00, 0x01, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x04, 0x80, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x10
};
static u8 TE_SETTING_24PHS[TE_SETTING_CMD_LEN] = {
	0x61, 0x00, 0x09, 0x00, 0x00, 0x01, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x04, 0x80, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x10
};
static u8 TE_SETTING_30PHS[TE_SETTING_CMD_LEN] = {
	0x61, 0x00, 0x07, 0x00, 0x00, 0x01, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x04, 0x80, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x10
};
static u8 TE_SETTING_48PHS[TE_SETTING_CMD_LEN] = {
	0x61, 0x00, 0x03, 0x00, 0x00, 0x01, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x03, 0x58, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x10
};
static u8 TE_SETTING_60PHS[TE_SETTING_CMD_LEN] = {
	0x61, 0x00, 0x03, 0x00, 0x00, 0x01, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x04, 0x80, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x10
};
static u8 TE_SETTING_10HS[TE_SETTING_CMD_LEN] = {
	0x61, 0x00, 0x13, 0x00, 0x00, 0x01, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x10
};
static u8 TE_SETTING_24HS[TE_SETTING_CMD_LEN] = {
	0x61, 0x00, 0x07, 0x00, 0x00, 0x01, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x10
};
static u8 TE_SETTING_30HS[TE_SETTING_CMD_LEN] = {
	0x61, 0x00, 0x07, 0x00, 0x00, 0x01, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x10
};
static u8 TE_SETTING_48HS[TE_SETTING_CMD_LEN] = {
	0x04, 0x00, 0x00, 0x00, 0x09, 0x23, 0x00, 0x0F,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00
};
static u8 TE_SETTING_60HS[TE_SETTING_CMD_LEN] = {
	0x04, 0x00, 0x00, 0x00, 0x09, 0x23, 0x00, 0x0F,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00
};
static u8 TE_SETTING_96HS[TE_SETTING_CMD_LEN] = {
	0x61, 0x00, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x03, 0x58, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x10
};
static u8 TE_SETTING_120HS[TE_SETTING_CMD_LEN] = {
	0x61, 0x00, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x04, 0x80, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x10
};
...
static struct dsi_panel_cmd_set *ss_vrr(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (panel && panel->cur_mode) {
		LCD_DEBUG(vdd, "VRR(%d): cur_mode: %dx%d@%d%s, bl_level: %d, is_hbm: %d, is_hmt: %d, %s\n",
				vrr->running_vrr,
				...
				bl_level, is_hbm, 0, ss_get_brr_mode_name(brr_mode));
...
#if 0	// temp code - use phs
	if (cur_md == VRR_10HS) {
		cur_md = VRR_10PHS;
		cur_phs = 1;
	} else if (cur_md == VRR_24HS) {
		cur_md = VRR_24PHS;
		cur_phs = 1;
	} else if (cur_md == VRR_30HS) {
		cur_md = VRR_30PHS;
		cur_phs = 1;
	} else if (cur_md == VRR_48HS) {
		cur_md = VRR_48PHS;
		cur_phs = 1;
	} else if (cur_md == VRR_60HS) {
		cur_md = VRR_60PHS;
		cur_phs = 1;
	}
#endif
	...
	if (idx >= 0) {
		if (cur_md_base == VRR_120HS) {
			memcpy(&vrr_cmds->cmds[idx-1].ss_txbuf[1], VFP_SETTING_GPARA_120HS, sizeof(VFP_SETTING_GPARA_120HS));
			if (vdd->panel_revision < 2) /* revA revB */
				memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], VFP_SETTING_120HS, sizeof(VFP_SETTING_120HS));
			else
				memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], VFP_SETTING_120HS_revC, sizeof(VFP_SETTING_120HS_revC));
		} else if (cur_md_base == VRR_96HS) {
			memcpy(&vrr_cmds->cmds[idx-1].ss_txbuf[1], VFP_SETTING_GPARA_96HS, sizeof(VFP_SETTING_GPARA_96HS));
			if (vdd->panel_revision < 2) /* revA revB */
				memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], VFP_SETTING_96HS, sizeof(VFP_SETTING_96HS));
			else
				memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], VFP_SETTING_96HS_revC, sizeof(VFP_SETTING_96HS_revC));
		} else if (cur_md_base == VRR_60HS) {
			memcpy(&vrr_cmds->cmds[idx-1].ss_txbuf[1], VFP_SETTING_GPARA_60HS, sizeof(VFP_SETTING_GPARA_60HS));
			if (vdd->panel_revision < 2) /* revA revB */
				memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], VFP_SETTING_60HS, sizeof(VFP_SETTING_60HS));
			else
				memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], VFP_SETTING_60HS_revC, sizeof(VFP_SETTING_60HS_revC));
		} else if (cur_md_base == VRR_60NS) {
			memcpy(&vrr_cmds->cmds[idx-1].ss_txbuf[1], VFP_SETTING_GPARA_60NS, sizeof(VFP_SETTING_GPARA_60NS));
			if (vdd->panel_revision < 2) /* revA revB */
				memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], VFP_SETTING_60NS, sizeof(VFP_SETTING_60NS));
			else
				memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], VFP_SETTING_60NS_revC, sizeof(VFP_SETTING_60NS_revC));
		} else if (cur_md_base == VRR_48HS) {
			memcpy(&vrr_cmds->cmds[idx-1].ss_txbuf[1], VFP_SETTING_GPARA_48HS, sizeof(VFP_SETTING_GPARA_48HS));
			if (vdd->panel_revision < 2) /* revA revB */
				memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], VFP_SETTING_48HS, sizeof(VFP_SETTING_48HS));
			else
				memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], VFP_SETTING_48HS_revC, sizeof(VFP_SETTING_48HS_revC));
		} else
	...
	if (idx >= 0) {
		if (cur_md == VRR_120HS)
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_120HS, sizeof(TE_SETTING_120HS));
		else if (cur_md == VRR_96HS)
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_96HS, sizeof(TE_SETTING_96HS));
		else if (cur_md == VRR_60HS)
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_60HS, sizeof(TE_SETTING_60HS));
		else if (cur_md == VRR_60PHS)
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_60PHS, sizeof(TE_SETTING_60PHS));
		else if (cur_md == VRR_48HS)
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_48HS, sizeof(TE_SETTING_48HS));
		else if (cur_md == VRR_48PHS)
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_48PHS, sizeof(TE_SETTING_48PHS));
		else if (cur_md == VRR_30HS)
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_30HS, sizeof(TE_SETTING_30HS));
		else if (cur_md == VRR_30PHS)
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_30PHS, sizeof(TE_SETTING_30PHS));
		else if (cur_md == VRR_24HS)
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_24HS, sizeof(TE_SETTING_24HS));
		else if (cur_md == VRR_24PHS)
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_24PHS, sizeof(TE_SETTING_24PHS));
		else if (cur_md == VRR_10HS)
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_10HS, sizeof(TE_SETTING_10HS));
		else if (cur_md == VRR_10PHS)
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_10PHS, sizeof(TE_SETTING_10PHS));
		else
			memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], TE_SETTING_120HS, sizeof(TE_SETTING_120HS));
...
static struct dsi_panel_cmd_set *ss_manual_aor(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (idx >= 0) {
		...
		/* 98h : 0x4F~0x50 / AOR Value */
		if (bl_level <= MAX_HBM_PF_LEVEL) {
			if (cur_md_base == VRR_96HS || cur_md_base == VRR_48HS) {
				pcmds->cmds[idx].ss_txbuf[2] = (MANUAL_AOR_TABLE[bl_level][MANUAL_AOR_96_48] & 0xFF00) >> 8;
				pcmds->cmds[idx].ss_txbuf[3] = (MANUAL_AOR_TABLE[bl_level][MANUAL_AOR_96_48] & 0x00FF);
			} else if (cur_md_base == VRR_120HS || cur_md_base == VRR_60HS) {
				pcmds->cmds[idx].ss_txbuf[2] = (MANUAL_AOR_TABLE[bl_level][MANUAL_AOR_120_60] & 0xFF00) >> 8;
				pcmds->cmds[idx].ss_txbuf[3] = (MANUAL_AOR_TABLE[bl_level][MANUAL_AOR_120_60] & 0x00FF);
			} else {
				pcmds->cmds[idx].ss_txbuf[2] = 0x00;
				pcmds->cmds[idx].ss_txbuf[3] = 0x00;
			}
		}

		/* 98h : 0x51 / AOR Cycle */
		if (cur_md_base == VRR_48HS)
			pcmds->cmds[idx].ss_txbuf[4] = 0x01;	/* 4 Cycle */
		else
			pcmds->cmds[idx].ss_txbuf[4] = 0x01;
	}
#if 0
	/* BDh : 0x01 / AOR Cycle */
	idx = ss_get_cmd_idx(pcmds, 0x01, 0xBD);
	if (idx >= 0) {
		if (cur_md_base == VRR_48HS)
			pcmds->cmds[idx].ss_txbuf[1] = 0x81;	/* 4 Cycle */
		else
			pcmds->cmds[idx].ss_txbuf[1] = 0x81;
	}
#endif
	return pcmds;
}

static u8 IRC_VAL[IRC_MAX_MODE][35] = {
	{0x65, 0xFD, 0x01, 0x00, 0x80, 0x01, 0xFF, 0x10, 0x69, 0xFF,
 	 0x10, 0xFF, 0x9C, 0x9E, 0x33, 0x09, 0x09, 0x09, 0x24, 0x24,
 	 0x24, 0x30, 0x30, 0x30, 0x39, 0x39, 0x39, 0x3C, 0x3C, 0x3C,
 	 0x96, 0x74, 0xE4, 0x18, 0x64
 	},
	{0x25, 0x8E, 0x03, 0x56, 0x80, 0x01, 0xFF, 0x10, 0x69, 0xFF,
 	 0x10, 0xFF, 0x9C, 0x9E, 0x33, 0x09, 0x09, 0x09, 0x24, 0x24,
 	 0x24, 0x30, 0x30, 0x30, 0x39, 0x39, 0x39, 0x3C, 0x3C, 0x3C,
 	 0x96, 0x74, 0xE4, 0x18, 0x64
	},
...
static struct dsi_panel_cmd_set * ss_brightness_gamma(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (cur_md_base == VRR_120HS || cur_md == VRR_24HS) {
		dim_off = false;
		smooth_frame = 15;
	} else {
		dim_off = true;
	...
	if (vdd->vrr.running_vrr || !vdd->display_on || first_high_level) {
		dim_off = true;
		smooth_frame = 0;
	}

	LCD_INFO(vdd, "NORMAL : prev_bl_level [%d] cur_md [%d] base [%d] smooth_frame [%d]\n",
		vdd->br_info.common_br.prev_bl_level, cur_md, cur_md_base, smooth_frame);
...
static void ss_set_panel_lpm_brightness(struct samsung_display_driver_data *vdd)
{
	...
	int lpm_ctrl_idx, idx, lpm_aor_idx, aor_pload_0, aor_pload_1;
	...
	if (vdd->panel_revision < 2) {
		...
		if (idx < 0) {
			LCD_ERR(vdd, "Invalid AOD cmd idx, lpm_bl_idx(%d)\n", idx);
			return;
		}

		set->cmds[idx].ss_txbuf[1] = set_lpm_bl->cmds->ss_txbuf[1];
		set->cmds[idx].ss_txbuf[2] = set_lpm_bl->cmds->ss_txbuf[2];

		idx = ss_get_cmd_idx(set, 0x4E, 0x98);
			switch (vdd->panel_lpm.lpm_bl_level) {
			case LPM_60NIT:
				set->cmds[idx].ss_txbuf[2] = 0x01;
				set->cmds[idx].ss_txbuf[3] = 0x79;
				break;
			case LPM_30NIT:
				set->cmds[idx].ss_txbuf[2] = 0x04;
				set->cmds[idx].ss_txbuf[3] = 0xF1;
				break;
			case LPM_10NIT:
				set->cmds[idx].ss_txbuf[2] = 0x07;
				set->cmds[idx].ss_txbuf[3] = 0x41;
				break;
			case LPM_2NIT:
			case LPM_1NIT:
			default:
				set->cmds[idx].ss_txbuf[2] = 0x08;
				set->cmds[idx].ss_txbuf[3] = 0x4C;
				break;
			}
	}
			...
			(struct samsung_display_driver_data *vdd, int enable)
{
	...
	int lpm_aor_idx, lpm_bl_idx, lpm_ctrl_idx, aor_pload_0, aor_pload_1;
	int gm2_wrdisbv, idx;
	...
	if (enable) {
		if (vdd->panel_revision < 2) {
			...
			if (lpm_ctrl_idx < 0 || lpm_aor_idx < 0) {
				LCD_ERR(vdd, "Invalid lpm_ctrl_idx(%d) lpm_aor_idx(%d)\n", lpm_ctrl_idx, lpm_aor_idx);
				return;
			}
			...
			lpm_bl_idx = ss_get_cmd_idx(lpm_cmd, 0x00, 0x51);

			lpm_cmd->cmds[lpm_bl_idx].ss_txbuf[1] = lpm_bl_cmd->cmds->ss_txbuf[1];
			lpm_cmd->cmds[lpm_bl_idx].ss_txbuf[2] = lpm_bl_cmd->cmds->ss_txbuf[2];

			idx = ss_get_cmd_idx(lpm_cmd, 0x4E, 0x98);
			switch (vdd->panel_lpm.lpm_bl_level) {
			case LPM_60NIT:
				lpm_cmd->cmds[idx].ss_txbuf[2] = 0x01;
				lpm_cmd->cmds[idx].ss_txbuf[3] = 0x79;
				break;
			case LPM_30NIT:
				lpm_cmd->cmds[idx].ss_txbuf[2] = 0x04;
				lpm_cmd->cmds[idx].ss_txbuf[3] = 0xF1;
				break;
			case LPM_10NIT:
				lpm_cmd->cmds[idx].ss_txbuf[2] = 0x07;
				lpm_cmd->cmds[idx].ss_txbuf[3] = 0x41;
				break;
			case LPM_2NIT:
			case LPM_1NIT:
			default:
				lpm_cmd->cmds[idx].ss_txbuf[2] = 0x08;
				lpm_cmd->cmds[idx].ss_txbuf[3] = 0x4C;
				break;
			}
		}
	} else {
		idx = ss_get_cmd_idx(lpm_cmd, 0x00, 0x51);
		lpm_cmd->cmds[idx].ss_txbuf[1] = (gm2_wrdisbv & 0xFF00) >> 8;
		lpm_cmd->cmds[idx].ss_txbuf[2] = gm2_wrdisbv & 0xFF;
	}
}
...
static int ss_ffc(struct samsung_display_driver_data *vdd, int idx)
{
	...
	/* 120HS */
	cmd_idx = ss_get_cmd_idx(ffc_set, 0x3E, 0xC5);
	...
	/* 60NS */
	cmd_idx = ss_get_cmd_idx(ffc_set, 0x56, 0xC5);
...
static void ss_print_gamma_comp(struct samsung_display_driver_data *vdd)
{
	...
	for (i = 0; i < GAMMA_SET_MAX; i++) {
		...
		for (j = 0; j < GAMMA_R_SIZE; j++)
			scnprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", HS120_R_TYPE_BUF[i][j]);
	...
	for (i = 0; i < GAMMA_SET_MAX; i++) {
		memset(pBuffer, 0x00, 256);
		for (j = 0; j < GAMMA_R_SIZE; j++)
			scnprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", HS60_R_TYPE_BUF[i][j]);
	...
	for (i = 0; i < GAMMA_SET_MAX; i++) {
		...
		for (j = 0; j < GAMMA_V_SIZE; j++)
			scnprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", HS120_V_TYPE_BUF[i][j]);
	...
	for (i = 0; i < GAMMA_SET_MAX; i++) {
		memset(pBuffer, 0x00, 256);
		for (j = 0; j < GAMMA_V_SIZE; j++)
			scnprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", HS60_V_TYPE_BUF[i][j]);
	...
	for (i = 0; i < GAMMA_OFFSET_SIZE; i++) {
		memset(pBuffer, 0x00, 256);
		for (j = 0; j < GAMMA_V_SIZE; j++)
			scnprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", HS60_V_TYPE_COMP[i][j]);
	...
	for (i = 0; i < GAMMA_OFFSET_SIZE; i++) {
		memset(pBuffer, 0x00, 256);
		for (j = 0; j < GAMMA_V_SIZE; j++)
			scnprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", HS48_V_TYPE_COMP[i][j]);
	...
	for (i = 0; i < GAMMA_OFFSET_SIZE; i++) {
		memset(pBuffer, 0x00, 256);
		for (j = 0; j < GAMMA_R_SIZE; j++)
			scnprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", HS60_R_TYPE_COMP[i][j]);
	...
	for (i = 0; i < GAMMA_OFFSET_SIZE; i++) {
		memset(pBuffer, 0x00, 256);
		for (j = 0; j < GAMMA_R_SIZE; j++)
			scnprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", HS48_R_TYPE_COMP[i][j]);
...
{
	struct dsi_panel_cmd_set *pcmds;
	int idx, level, set, dset, seto, use_comp2, fdelay, dummy_offset;
	enum VRR_CMD_RR cur_md;
	enum VRR_CMD_RR cur_md_base;
	u8 *comp_reg;
	s64 t_delta;
	int cur_rr, dummy_size;
	bool cur_hs, cur_phs;
	...
	if (cur_md_base == VRR_120HS || cur_md_base == VRR_96HS ||
		...
		if (cur_md_base == VRR_120HS || cur_md_base == VRR_96HS) {
			...
			pcmds->cmds[2].ss_txbuf[2] = (GAMMA_SET_ADDR_120_TABLE[set - seto] & 0xFF00) >> 8;
			pcmds->cmds[2].ss_txbuf[3] = (GAMMA_SET_ADDR_120_TABLE[set - seto] & 0xFF);
			...
			pcmds->cmds[2].ss_txbuf[2] = (GAMMA_SET_ADDR_60_TABLE[set - seto] & 0xFF00) >> 8;
			pcmds->cmds[2].ss_txbuf[3] = (GAMMA_SET_ADDR_60_TABLE[set - seto] & 0xFF);
		}
		...
		if (use_comp2) {
			...
			if (cur_md_base == VRR_120HS)
				...
				comp_reg = HS96_R_TYPE_COMP[level];
			else if (cur_md_base == VRR_60HS)
				comp_reg = HS60_R_TYPE_COMP[level];
			else
				comp_reg = HS48_R_TYPE_COMP[level];
		...
		LCD_INFO(vdd, "cur_md_base[%d], level[%d] set[%d] offset [%02x%02x]\n",
			cur_md_base, level, set, pcmds->cmds[0].ss_txbuf[2], pcmds->cmds[0].ss_txbuf[3]);
		return pcmds;
	}
	...
	struct dsi_cmd_desc *packet, int *cmd_cnt, enum BR_TYPE br_type)
{
	...
	for (loop = start ; loop < *cmd_cnt ; loop++) {
		if (packet[loop].ss_txbuf[0] == LEVEL_KEY_1 ||
			packet[loop].ss_txbuf[0] == LEVEL_KEY_2 ||
...
{
	LCD_INFO(vdd, "Q4_S6E3FAC_AMB619BR01_HD_init : ++ \n");
