...
*/
#include "ss_dsi_panel_S6E3FAC_AMB655AY01.h"
#include "ss_dsi_mdnie_S6E3FAC_AMB655AY01.h"
...
static struct dsi_panel_cmd_set *ss_vrr(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (idx >= 0) {
		if (cur_md_base == VRR_120HS) {
			...
			if (is_hbm)
				memcpy(&vrr_cmds->cmds[idx].ss_txbuf[1], VFP_SETTING_HBM_48HS, sizeof(VFP_SETTING_HBM_48HS));
			else
	...
	if (cur_md_base != VRR_120HS) {
		vrr_cmds->count = 20;
		/* Manual AOR */
		idx = ss_get_cmd_idx(vrr_cmds, 0x4E, 0x98) + 2;
		if (idx >= 0) { /* Manual AOR Enable */
			if (cur_md_base == VRR_96HS || cur_md_base == VRR_48HS || cur_md_base == VRR_60HS)
			...
			if (bl_level <= MAX_HBM_PF_LEVEL) {
				if (cur_md_base == VRR_96HS) {
					vrr_cmds->cmds[idx].ss_txbuf[2] = (MANUAL_AOR_TABLE[bl_level][MANUAL_AOR_96HS] & 0xFF00) >> 8;
					vrr_cmds->cmds[idx].ss_txbuf[3] = (MANUAL_AOR_TABLE[bl_level][MANUAL_AOR_96HS] & 0x00FF);
				} else if (cur_md_base == VRR_60HS) {
					vrr_cmds->cmds[idx].ss_txbuf[2] = (MANUAL_AOR_TABLE[bl_level][MANUAL_AOR_60HS] & 0xFF00) >> 8;
					vrr_cmds->cmds[idx].ss_txbuf[3] = (MANUAL_AOR_TABLE[bl_level][MANUAL_AOR_60HS] & 0x00FF);
				} else if (cur_md_base == VRR_48HS) {
					vrr_cmds->cmds[idx].ss_txbuf[2] = (MANUAL_AOR_TABLE[bl_level][MANUAL_AOR_48HS] & 0xFF00) >> 8;
					vrr_cmds->cmds[idx].ss_txbuf[3] = (MANUAL_AOR_TABLE[bl_level][MANUAL_AOR_48HS] & 0x00FF);
				} else if (cur_md_base == VRR_60NS) {
...
static void ss_print_gamma_comp(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO(vdd, "== VARIATY_HS120_V_TYPE_BUF ==\n");
	memset(pBuffer, 0x00, 256);
	for (j = 0; j < GAMMA_V_SIZE; j++)
		snprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", VARIATY_HS120_V_TYPE_BUF[j]);
	LCD_INFO(vdd, "READ_V VARIATY_120 : %s\n", pBuffer);
	...
	LCD_INFO(vdd, "== VARIATY_HS60_V_TYPE_BUF ==\n");
	memset(pBuffer, 0x00, 256);
	for (j = 0; j < GAMMA_V_SIZE; j++)
		snprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", VARIATY_HS60_V_TYPE_BUF[j]);
	LCD_INFO(vdd, "READ_V VARIATY_120 : %s\n", pBuffer);
	...
	LCD_INFO(vdd, "== VARIATY_HS96_V_TYPE_COMP ==\n");
	for (i = 0; i < VARIATY_GAMMA_OFFSET_SIZE; i++) {
		memset(pBuffer, 0x00, 256);
		for (j = 0; j < GAMMA_V_SIZE; j++)
			snprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", VARIATY_HS96_V_TYPE_COMP[i][j]);
		LCD_INFO(vdd, "VARIATY_COMP_V 96 LV[%3d] : %s\n", i + VARIATY_LEVEL, pBuffer);
	}
	...
	LCD_INFO(vdd, "== VARIATY_HS60_V_TYPE_COMP ==\n");
	for (i = 0; i < VARIATY_GAMMA_OFFSET_SIZE; i++) {
		memset(pBuffer, 0x00, 256);
		for (j = 0; j < GAMMA_V_SIZE; j++)
			snprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", VARIATY_HS60_V_TYPE_COMP[i][j]);
		LCD_INFO(vdd, "VARIATY_COMP_V 60 LV[%3d] : %s\n", i + VARIATY_LEVEL, pBuffer);
	}
	...
	LCD_INFO(vdd, "== VARIATY_HS48_V_TYPE_COMP ==\n");
	for (i = 0; i < VARIATY_GAMMA_OFFSET_SIZE; i++) {
		memset(pBuffer, 0x00, 256);
		for (j = 0; j < GAMMA_V_SIZE; j++)
			snprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", VARIATY_HS48_V_TYPE_COMP[i][j]);
		LCD_INFO(vdd, "VARIATY_COMP_V 48 LV[%3d] : %s\n", i + VARIATY_LEVEL, pBuffer);
	}
	...
	LCD_INFO(vdd, "== VARIATY_HS96_R_TYPE_COMP ==\n");
	for (i = 0; i < VARIATY_GAMMA_OFFSET_SIZE; i++) {
		memset(pBuffer, 0x00, 256);
		for (j = 0; j < GAMMA_R_SIZE; j++)
			snprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", VARIATY_HS96_R_TYPE_COMP[i][j]);
		LCD_INFO(vdd, "VARIATY_COMP_R 96 LV[%3d] : %s\n", i + VARIATY_LEVEL, pBuffer);
	}
	...
	LCD_INFO(vdd, "== VARIATY_HS60_R_TYPE_COMP ==\n");
	for (i = 0; i < VARIATY_GAMMA_OFFSET_SIZE; i++) {
		memset(pBuffer, 0x00, 256);
		for (j = 0; j < GAMMA_R_SIZE; j++)
			snprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", VARIATY_HS60_R_TYPE_COMP[i][j]);
		LCD_INFO(vdd, "VARIATY_COMP_R 60 LV[%3d] : %s\n", i + VARIATY_LEVEL, pBuffer);
	}
	...
	LCD_INFO(vdd, "== VARIATY_HS48_R_TYPE_COMP ==\n");
	for (i = 0; i < VARIATY_GAMMA_OFFSET_SIZE; i++) {
		memset(pBuffer, 0x00, 256);
		for (j = 0; j < GAMMA_R_SIZE; j++)
			snprintf(pBuffer + strnlen(pBuffer, 256), 256, " %02x", VARIATY_HS48_R_TYPE_COMP[i][j]);
		LCD_INFO(vdd, "VARIATY_COMP_R 48 LV[%3d] : %s\n", i + VARIATY_LEVEL, pBuffer);
	}
}
...
static int ss_gm2_gamma_comp_init(struct samsung_display_driver_data *vdd)
{
	...
	memcpy(VARIATY_GAMMA_OFFSET_96HS_V, VARIATY_GAMMA_OFFSET_96HS_V_revA, sizeof(VARIATY_GAMMA_OFFSET_96HS_V));
	memcpy(VARIATY_GAMMA_OFFSET_60HS_V, VARIATY_GAMMA_OFFSET_60HS_V_revA, sizeof(VARIATY_GAMMA_OFFSET_60HS_V));
	memcpy(VARIATY_GAMMA_OFFSET_48HS_V, VARIATY_GAMMA_OFFSET_48HS_V_revA, sizeof(VARIATY_GAMMA_OFFSET_48HS_V));
	...
	for (i = 0; i < GAMMA_SET_MAX; i++) {
		...
		for (j = 0; j < GAMMA_R_SIZE; ) {
			if (i == GAMMA_SET_2) {
				VARIATY_HS120_V_TYPE_BUF[m2++] = (GET_BITS(HS120_R_TYPE_BUF[i][j], 0, 3) << 8)
										| GET_BITS(HS120_R_TYPE_BUF[i][j+2], 0, 7);
				VARIATY_HS120_V_TYPE_BUF[m2++] = (GET_BITS(HS120_R_TYPE_BUF[i][j+1], 4, 7) << 8)
										| GET_BITS(HS120_R_TYPE_BUF[i][j+3], 0, 7);
				VARIATY_HS120_V_TYPE_BUF[m2++] = (GET_BITS(HS120_R_TYPE_BUF[i][j+1], 0, 3) << 8)
	...
	for (i = 0; i < GAMMA_SET_MAX; i++) {
		...
		for (j = 0; j < GAMMA_R_SIZE; ) {
			if (i == GAMMA_SET_2) {
				VARIATY_HS60_V_TYPE_BUF[m2++] = (GET_BITS(HS60_R_TYPE_BUF[i][j], 0, 3) << 8)
										| GET_BITS(HS60_R_TYPE_BUF[i][j+2], 0, 7);
				VARIATY_HS60_V_TYPE_BUF[m2++] = (GET_BITS(HS60_R_TYPE_BUF[i][j+1], 4, 7) << 8)
										| GET_BITS(HS60_R_TYPE_BUF[i][j+3], 0, 7);
				VARIATY_HS60_V_TYPE_BUF[m2++] = (GET_BITS(HS60_R_TYPE_BUF[i][j+1], 0, 3) << 8)
	...
	/* variaty 96HS */
	v_val = VARIATY_HS120_V_TYPE_BUF;
	for (i = 0; i < VARIATY_GAMMA_OFFSET_SIZE; i++) {
		for (j = 0; j < GAMMA_V_SIZE; j++) {
			/* check underflow & overflow */
			if (v_val[j] + VARIATY_GAMMA_OFFSET_96HS_V[i][j] < 0) {
				VARIATY_HS96_V_TYPE_COMP[i][j] = 0;
			} else {
				if (j <= 5 || j >= (GAMMA_V_SIZE - 3)) {/* 1/13/14 th is 12bit(0xFFF) */
					val = v_val[j] + VARIATY_GAMMA_OFFSET_96HS_V[i][j];
					if (val > 0xFFF)	/* check overflow */
						VARIATY_HS96_V_TYPE_COMP[i][j] = 0xFFF;
					else
						VARIATY_HS96_V_TYPE_COMP[i][j] = val;
				} else {	/* 2 ~ 12th 11bit(0x7FF) */
					val = v_val[j] + VARIATY_GAMMA_OFFSET_96HS_V[i][j];
					if (val > 0x7FF)	/* check overflow */
						VARIATY_HS96_V_TYPE_COMP[i][j] = 0x7FF;
					else
						VARIATY_HS96_V_TYPE_COMP[i][j] = val;
				}
			}
	...
	/* variaty 60HS */
	v_val = VARIATY_HS60_V_TYPE_BUF;
	for (i = 0; i < VARIATY_GAMMA_OFFSET_SIZE; i++) {
		for (j = 0; j < GAMMA_V_SIZE; j++) {
			/* check underflow & overflow */
			if (v_val[j] + VARIATY_GAMMA_OFFSET_60HS_V[i][j] < 0) {
				VARIATY_HS60_V_TYPE_COMP[i][j] = 0;
			} else {
				if (j <= 5 || j >= (GAMMA_V_SIZE - 3)) {/* 1/13/14 th is 12bit(0xFFF) */
					val = v_val[j] + VARIATY_GAMMA_OFFSET_60HS_V[i][j];
					if (val > 0xFFF)	/* check overflow */
						VARIATY_HS60_V_TYPE_COMP[i][j] = 0xFFF;
					else
						VARIATY_HS60_V_TYPE_COMP[i][j] = val;
				} else {	/* 2 ~ 12th 11bit(0x7FF) */
					val = v_val[j] + VARIATY_GAMMA_OFFSET_60HS_V[i][j];
					if (val > 0x7FF)	/* check overflow */
						VARIATY_HS60_V_TYPE_COMP[i][j] = 0x7FF;
					else
						VARIATY_HS60_V_TYPE_COMP[i][j] = val;
				}
			}
	...
	for (i = 0; i < VARIATY_GAMMA_OFFSET_SIZE; i++) {
		for (j = 0; j < GAMMA_V_SIZE; j++) {
			/* check underflow & overflow */
			if (v_val[j] + VARIATY_GAMMA_OFFSET_48HS_V[i][j] < 0) {
				VARIATY_HS48_V_TYPE_COMP[i][j] = 0;
			} else {
				if (j <= 5 || j >= (GAMMA_V_SIZE - 3)) {/* 1/13/14 th is 12bit(0xFFF) */
					val = v_val[j] + VARIATY_GAMMA_OFFSET_48HS_V[i][j];
					if (val > 0xFFF)	/* check overflow */
						VARIATY_HS48_V_TYPE_COMP[i][j] = 0xFFF;
					else
						VARIATY_HS48_V_TYPE_COMP[i][j] = val;
				} else {	/* 2 ~ 12th 11bit(0x7FF) */
					val = v_val[j] + VARIATY_GAMMA_OFFSET_48HS_V[i][j];
					if (val > 0x7FF)	/* check overflow */
						VARIATY_HS48_V_TYPE_COMP[i][j] = 0x7FF;
					else
						VARIATY_HS48_V_TYPE_COMP[i][j] = val;
				}
			}
	...
	for (i = 0; i < VARIATY_GAMMA_OFFSET_SIZE; i++) {
		...
		for (j = 0; j < GAMMA_V_SIZE; j += RGB_MAX) {
			if (j == 0 || j == 3 || j == GAMMA_V_SIZE - 3) {
				VARIATY_HS96_R_TYPE_COMP[i][m++] = GET_BITS(VARIATY_HS96_V_TYPE_COMP[i][j+R], 8, 11);
				VARIATY_HS96_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS96_V_TYPE_COMP[i][j+G], 8, 11) << 4)
											| (GET_BITS(VARIATY_HS96_V_TYPE_COMP[i][j+B], 8, 11));
				VARIATY_HS96_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS96_V_TYPE_COMP[i][j+R], 0, 7));
				VARIATY_HS96_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS96_V_TYPE_COMP[i][j+G], 0, 7));
				VARIATY_HS96_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS96_V_TYPE_COMP[i][j+B], 0, 7));
			} else {	/* 1st ~ 10th */
				VARIATY_HS96_R_TYPE_COMP[i][m++] = GET_BITS(VARIATY_HS96_V_TYPE_COMP[i][j+R], 8, 10);
				VARIATY_HS96_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS96_V_TYPE_COMP[i][j+G], 8, 10) << 4)
											| (GET_BITS(VARIATY_HS96_V_TYPE_COMP[i][j+B], 8, 10));
				VARIATY_HS96_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS96_V_TYPE_COMP[i][j+R], 0, 7));
				VARIATY_HS96_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS96_V_TYPE_COMP[i][j+G], 0, 7));
				VARIATY_HS96_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS96_V_TYPE_COMP[i][j+B], 0, 7));
			}
	...
	for (i = 0; i < VARIATY_GAMMA_OFFSET_SIZE; i++) {
		...
		for (j = 0; j < GAMMA_V_SIZE; j += RGB_MAX) {
			if (j == 0 || j == 3 || j == GAMMA_V_SIZE - 3) {
				VARIATY_HS60_R_TYPE_COMP[i][m++] = GET_BITS(VARIATY_HS60_V_TYPE_COMP[i][j+R], 8, 11);
				VARIATY_HS60_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS60_V_TYPE_COMP[i][j+G], 8, 11) << 4)
											| (GET_BITS(VARIATY_HS60_V_TYPE_COMP[i][j+B], 8, 11));
				VARIATY_HS60_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS60_V_TYPE_COMP[i][j+R], 0, 7));
				VARIATY_HS60_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS60_V_TYPE_COMP[i][j+G], 0, 7));
				VARIATY_HS60_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS60_V_TYPE_COMP[i][j+B], 0, 7));
			} else {	/* 1st ~ 10th */
				VARIATY_HS60_R_TYPE_COMP[i][m++] = GET_BITS(VARIATY_HS60_V_TYPE_COMP[i][j+R], 8, 10);
				VARIATY_HS60_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS60_V_TYPE_COMP[i][j+G], 8, 10) << 4)
											| (GET_BITS(VARIATY_HS60_V_TYPE_COMP[i][j+B], 8, 10));
				VARIATY_HS60_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS60_V_TYPE_COMP[i][j+R], 0, 7));
				VARIATY_HS60_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS60_V_TYPE_COMP[i][j+G], 0, 7));
				VARIATY_HS60_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS60_V_TYPE_COMP[i][j+B], 0, 7));
			}
	...
	for (i = 0; i < VARIATY_GAMMA_OFFSET_SIZE; i++) {
		...
		for (j = 0; j < GAMMA_V_SIZE; j += RGB_MAX) {
			if (j == 0 || j == 3 || j == GAMMA_V_SIZE - 3) {
				VARIATY_HS48_R_TYPE_COMP[i][m++] = GET_BITS(VARIATY_HS48_V_TYPE_COMP[i][j+R], 8, 11);
				VARIATY_HS48_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS48_V_TYPE_COMP[i][j+G], 8, 11) << 4)
											| (GET_BITS(VARIATY_HS48_V_TYPE_COMP[i][j+B], 8, 11));
				VARIATY_HS48_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS48_V_TYPE_COMP[i][j+R], 0, 7));
				VARIATY_HS48_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS48_V_TYPE_COMP[i][j+G], 0, 7));
				VARIATY_HS48_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS48_V_TYPE_COMP[i][j+B], 0, 7));
			} else {	/* 1st ~ 10th */
				VARIATY_HS48_R_TYPE_COMP[i][m++] = GET_BITS(VARIATY_HS48_V_TYPE_COMP[i][j+R], 8, 10);
				VARIATY_HS48_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS48_V_TYPE_COMP[i][j+G], 8, 10) << 4)
											| (GET_BITS(VARIATY_HS48_V_TYPE_COMP[i][j+B], 8, 10));
				VARIATY_HS48_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS48_V_TYPE_COMP[i][j+R], 0, 7));
				VARIATY_HS48_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS48_V_TYPE_COMP[i][j+G], 0, 7));
				VARIATY_HS48_R_TYPE_COMP[i][m++] = (GET_BITS(VARIATY_HS48_V_TYPE_COMP[i][j+B], 0, 7));
			}
...
{
	LCD_INFO(vdd, "S6E3FAC_AMB655AY01 : ++ \n");
