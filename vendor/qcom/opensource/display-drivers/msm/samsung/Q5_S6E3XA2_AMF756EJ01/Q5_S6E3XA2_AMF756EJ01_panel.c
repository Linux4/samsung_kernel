...
*/
#include "Q5_S6E3XA2_AMF756EJ01_panel.h"
#include "Q5_S6E3XA2_AMF756EJ01_mdnie.h"
...
static int update_analog1_Q5_S6E3XA2_AMF756EJ01(
			struct samsung_display_driver_data *vdd,
			char *val, struct ss_cmd_desc *cmd)
{
	...
	memcpy(&cmd->txbuf[i+1], &HS96_R_TYPE_COMP[bl_lvl][37], 17);
...
static int update_aor_Q5_S6E3XA2_AMF756EJ01(struct samsung_display_driver_data *vdd,
			char *val, struct ss_cmd_desc *cmd)
{
	struct cmd_ref_state *state = &vdd->cmd_ref_state;
...
//	vdd->panel_func.pre_brightness = ss_pre_brightness;
	...
	vdd->h_buf = Q5_S6E3XA2_AMF756EJ01_PDF_DATA;
	vdd->h_size = sizeof(Q5_S6E3XA2_AMF756EJ01_PDF_DATA);
	...
	register_op_sym_cb(vdd, "AOR", update_aor_Q5_S6E3XA2_AMF756EJ01, true);
	register_op_sym_cb(vdd, "ANALOG_OFFSET_1",
			update_analog1_Q5_S6E3XA2_AMF756EJ01, true);
