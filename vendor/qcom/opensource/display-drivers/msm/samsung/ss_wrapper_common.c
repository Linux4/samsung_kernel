					...
					char *cmd_name)
{
	...
	for (rev = 1; rev < SUPPORT_PANEL_REVISION; rev++) {
		...
		if (!ss_is_prop_string_style(np, map)) {
				...
				bool is_lp)
{
	...
	while ((data_line = strsep(&data_block, "\r\n")) != NULL) {
		...
		if (!strncasecmp(data_line, CMDSTR_WRITE, strlen(CMDSTR_WRITE))) {
			...
			LCD_INFO_IF(vdd, "WRITE: cnt_val: %d, line: [%s]\n",
					cnt_val, data_line);
			...
			while ((data_val = strsep(&data_line, " \t")) != NULL) {
				...
				if (kstrtou8(data_val, 16, &payload[i])) {
					if (!strncasecmp(data_val, "0xXX" ,4)) {
						LCD_INFO_IF(vdd, "[%d] %02X, xx updatable!\n", i, payload[i]);
				...
				LCD_INFO_IF(vdd, "[%d] %02X", i, payload[i]);
				i++;
			}
			...
			LCD_INFO_IF(vdd, "DELAY: %d ms, line: [%s]\n",
					wait_ms, data_line);
			...
			struct ss_cmd_set *ss_set)
{
	...
	if (--qc_cmd != &qc_set->cmds[qc_set->count - 1] ||
		...
		LCD_ERR(vdd, "unmatched countcritical count err\n");
		return -EFAULT;
	}
					...
					struct ss_cmd_set *set_each)
{
	...
	for (i = 0, j = 0; i < set_each->count; i++) {
		 ...
		 */
		if (set_each->cmds[i].updatable == SS_CMD_UPDATABLE) {
			...
			set_each->cmds[i].updatable = SS_CMD_NO_UPDATABLE;
			j++;
		}
	}
					...
					struct ss_cmd_set *set)
{
	...
	if (!vdd->support_ss_cmd || !set ||
		...
		LCD_INFO(vdd, "doesn't support ss cmd\n");
		return 0;
	}
				...
				struct device_node *np, char *cmd_name)
{
	...
	u32 packet_count = 0;
	bool is_lp = false;
	char map_lp[SS_CMD_PROP_STR_LEN + 3] = {0, };
	 ...
	 */
	strcpy(map_lp, cmd_name);
	strcat(map_lp, "_lp");
	is_lp = of_property_read_bool(np, map_lp);
	...
	rc = ss_wrapper_handle_updatable(vdd, set);
	if (rc) {
