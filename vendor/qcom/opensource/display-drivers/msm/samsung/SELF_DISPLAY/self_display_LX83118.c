...
#include "self_display_LX83118.h"
...
void make_self_dispaly_img_cmds_LX83118(struct samsung_display_driver_data *vdd, int cmd, u32 op)
{
	...
	LCD_DEBUG(vdd, "[DM1_LX83118_CM002] ++\n");
...
void make_mass_self_display_img_cmds_LX83118(struct samsung_display_driver_data *vdd,
		int cmd, u32 op)
{
	struct dsi_cmd_desc *tcmds;
	struct dsi_panel_cmd_set *pcmds;
	...
	LCD_INFO(vdd, "[DM1_LX83118_CM002] [%s] total data size [%d], total cmd len[%d], cmd count [%d]\n",
...
static int self_display_debug(struct samsung_display_driver_data *vdd)
{
	...
	if (is_ss_style_cmd(vdd, RX_SELF_DISP_DEBUG)) {
		...
		ss_panel_data_read(vdd, RX_SELF_DISP_DEBUG, buf, LEVEL_KEY_NONE);
	}
	...
	if (vdd->self_disp.operation[FLAG_SELF_MASK].img_checksum != vdd->self_disp.debug.SM_SUM_O) {
		...
		LCD_INFO(vdd, "self mask img checksum OK!! %X %X\n",
			vdd->self_disp.operation[FLAG_SELF_MASK].img_checksum, vdd->self_disp.debug.SM_SUM_O);
	}
...
static void self_mask_img_write(struct samsung_display_driver_data *vdd)
{
	...
	LCD_DEBUG(vdd, "[DM1_LX83118_CM002] tx self mask ++ (cur_rr:%d)\n", vdd->vrr.cur_refresh_rate);
	LCD_DEBUG(vdd, "[DM1_LX83118_CM002] TX_SELF_MASK_SET_PRE\n");
	ss_send_cmd(vdd, TX_SELF_MASK_SET_PRE);

	usleep_range(1000, 1001); /* 1ms delay between 75h and 5Ch */
	LCD_DEBUG(vdd, "[DM1_LX83118_CM002] TX_SELF_MASK_IMAGE\n");
	ss_send_cmd(vdd, TX_SELF_MASK_IMAGE);
	usleep_range(1000, 1001); /* 1ms delay between 75h and 5Ch */

	LCD_DEBUG(vdd, "[DM1_LX83118_CM002] TX_SELF_MASK_SET_POST\n");
	ss_send_cmd(vdd, TX_SELF_MASK_SET_POST);
	LCD_DEBUG(vdd, "[DM1_LX83118_CM002] tx self mask --\n");
	...
	LCD_INFO(vdd, "[DM1_LX83118_CM002] --\n");
}
...
static int self_mask_on(struct samsung_display_driver_data *vdd, int enable)
{
	...
	LCD_DEBUG(vdd, "[DM1_LX83118_CM002] ++ (%d)\n", enable);
...
static int self_mask_check(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO(vdd, "RX_SELF_MASK_CHECK ++\n");
...
static int self_display_open(struct inode *inode, struct file *file)
{
	...
	LCD_DEBUG(vdd, "[DM1_LX83118_CM002] [open]\n");
...
static int self_display_release(struct inode *inode, struct file *file)
{
	...
	LCD_DEBUG(vdd, "[DM1_LX83118_CM002] [release]\n");
...
int self_display_init_LX83118(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO(vdd, "[DM1_LX83118] Success to register self_disp device..(%d)\n", ret);
	return ret;
}
