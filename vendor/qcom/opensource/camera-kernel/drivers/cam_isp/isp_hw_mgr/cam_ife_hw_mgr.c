...
static int cam_ife_mgr_prog_default_settings(
	bool need_rup_aup, int32_t use_mup, struct cam_ife_hw_mgr_ctx *ctx);
	...
	void                            *ctx)
{
	...
	struct cam_vfe_notify_overflow_config notify_config;
	...
	for (i = 0; i < hw_mgr_ctx->num_base; i++) {
		...
		if (hw_mgr_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_VFE) {
			...
			notify_config.res_id = ife_res_id;
			...
			notify_config.res_id = sfe_res_id;
		} else {
			continue;
		}
		...
		if (hw_if->hw_ops.process_cmd)
			hw_if->hw_ops.process_cmd(hw_if->hw_priv,
				CAM_ISP_HW_DUMP_REG_ON_ERROR,
				&notify_config, sizeof(notify_config));
	}
...
static int cam_ife_mgr_csid_stop_hw(
	struct cam_ife_hw_mgr_ctx *ctx, struct list_head  *stop_list,
		uint32_t  base_idx, uint32_t stop_cmd, bool internal_stop, bool *reset_timed_out)
{
	struct cam_isp_hw_mgr_res      *hw_mgr_res;
	struct cam_isp_resource_node   *isp_res;
	...
	if (cnt) {
		...
		stop.internal_stop = internal_stop;
		hw_intf->hw_ops.stop(hw_intf->hw_priv, &stop, sizeof(stop));
	}

	*reset_timed_out = stop.reset_timed_out;
	return 0;
}
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	*input_size += in_port_length;
	...
	if (in->feature_flag & CAM_ISP_EPD_SUPPORT)
		in_port->epd_supported = 1;
...
static int cam_ife_mgr_stop_hw_in_overflow(void *stop_hw_args)
{
	...
	cam_ife_mgr_csid_stop_hw(ctx, &ctx->res_list_ife_csid,
		master_base_idx, CAM_CSID_HALT_IMMEDIATELY, false, &csid_rst_status);
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		cam_ife_mgr_csid_stop_hw(ctx, &ctx->res_list_ife_csid,
			ctx->base[i].idx, CAM_CSID_HALT_IMMEDIATELY, false, &csid_rst_status);
	}
...
static int cam_ife_mgr_stop_hw(void *hw_mgr_priv, void *stop_hw_args)
{
	...
	/* Stop the master CSID path first */
	cam_ife_mgr_csid_stop_hw(ctx, &ctx->res_list_ife_csid,
		master_base_idx, csid_halt_type, stop_isp->internal_trigger, &csid_rst_status);

	/* Use master CSID's reset status to decide if async reset is needed or not */
	stop_isp->reset_timed_out = csid_rst_status;
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		cam_ife_mgr_csid_stop_hw(ctx, &ctx->res_list_ife_csid,
			ctx->base[i].idx, csid_halt_type, stop_isp->internal_trigger, &csid_rst_status);
	}
...
static int cam_ife_mgr_start_hw(void *hw_mgr_priv, void *start_hw_args)
{
	...
	csid_debug_args.csid_debug = g_ife_hw_mgr.debug_cfg.csid_debug;
	csid_debug_args.csid_rx_capture_debug = g_ife_hw_mgr.debug_cfg.rx_capture_debug;
	csid_debug_args.rx_capture_debug_set = g_ife_hw_mgr.debug_cfg.rx_capture_debug_set;
...
start_only:
	...
	if ((ctx->flags.is_sfe_fs || ctx->flags.is_sfe_shdr) &&
		...
		rc = cam_ife_mgr_prog_default_settings(false, -1, ctx);
		if (rc)
			goto err;
		ctx->sfe_info.skip_scratch_cfg_streamon = false;
	}
	...
	struct cam_hw_prepare_update_args           *prepare)
{
	...
	for (i = 0; i < scratch_config->num_ports; i++) {
		...
		if (ife_scratch_config->num_ports >= CAM_IFE_SCRATCH_NUM_MAX) {
			CAM_ERR(CAM_ISP,
				"Max supported IFE scratch buffers: %u incoming exceeds max",
				CAM_IFE_SCRATCH_NUM_MAX);
			return -EINVAL;
		}

		port_info = &ife_scratch_config->buf_info[ife_scratch_config->num_ports++];
		rc = cam_isp_scratch_buf_update_util(buffer_info, port_info);
		if (rc)
			goto end;

		CAM_DBG(CAM_ISP,
			"IFE res_id: 0x%x w: 0x%x h: 0x%x s: 0x%x sh: 0x%x addr: 0x%x",
	...
	struct cam_hw_prepare_update_args           *prepare)
{
	...
	for (i = 0; i < scratch_config->num_ports; i++) {
		...
		CAM_INFO(CAM_ISP,
			"SFE res_id: 0x%x w: 0x%x h: 0x%x s: 0x%x sh: 0x%x addr: 0x%x ctx: %u",
			port_info->res_id, port_info->width,
			port_info->height, port_info->stride,
			port_info->slice_height, port_info->io_addr, ctx->ctx_index);
	}

	if (ctx->sfe_info.scratch_config->num_config != ctx->sfe_info.num_fetches)
		CAM_WARN(CAM_ISP, "Mismatch in number of scratch buffers provided: %u expected: %u",
	...
	uint32_t blob_type, uint32_t blob_size, uint8_t *blob_data)
{
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_SCRATCH_BUF_CFG: {
		...
		if (rc)
			CAM_ERR(CAM_ISP, "IFE scratch buffer update failed");
	}
		break;
	...
	struct cam_ife_hw_mgr_ctx            *ctx)
{
	...
	/* Update IFE WMs */
	for (i = 0; i < ctx->sfe_info.ife_scratch_config->num_ports; i++) {
...
static void cam_ife_hw_mgr_check_if_scratch_needed(
	struct cam_ife_hw_mgr_ctx *ctx,
	struct cam_isp_check_io_cfg_for_scratch *chk_for_scratch)
{
	/* scratch only for sHDR/FS use-cases */
	if (!((ctx->flags.is_sfe_fs) || (ctx->flags.is_sfe_shdr)))
		return;

	/* For SFE use number of fetches = number of scratch buffers needed */
	chk_for_scratch->sfe_scratch_res_info.num_active_fe_rdis = ctx->sfe_info.num_fetches;
	chk_for_scratch->check_for_sfe = true;

	/* Check if IFE has any scratch buffer */
	if (ctx->sfe_info.ife_scratch_config->num_ports) {
		int i;

		chk_for_scratch->check_for_ife = true;
		for (i = 0; i < ctx->sfe_info.ife_scratch_config->num_ports; i++) {
			chk_for_scratch->ife_scratch_res_info.ife_scratch_resources[i] =
				ctx->sfe_info.ife_scratch_config->buf_info[i].res_id;
			chk_for_scratch->ife_scratch_res_info.num_ports++;
			/* Update mask if there is config IO */
			chk_for_scratch->ife_scratch_res_info.ife_scratch_cfg_mask |= (1 << i);
			CAM_DBG(CAM_ISP,
				"Scratch for IFE res: 0x%x available - validate in IO config cfg: mask: 0x%x",
				ctx->sfe_info.ife_scratch_config->buf_info[i].res_id,
				chk_for_scratch->ife_scratch_res_info.ife_scratch_cfg_mask);
		}
	...
	void *prepare_hw_update_args)
{
	...
	struct cam_isp_check_io_cfg_for_scratch  chk_for_scratch = { 0 };
	...
	/* Update for scratch bufer check */
	cam_ife_hw_mgr_check_if_scratch_needed(ctx, &chk_for_scratch);
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		if ((chk_for_scratch.check_for_ife) &&
			(ctx->base[i].hw_type == CAM_ISP_HW_TYPE_VFE) && (fill_ife_fence)) {
			struct cam_isp_ife_scratch_buf_res_info *ife_res_info =
				&chk_for_scratch.ife_scratch_res_info;

			/* Config scratch for update packets only */
			if (ife_res_info->ife_scratch_cfg_mask) {
				if (prepare_hw_data->packet_opcode_type == CAM_ISP_PACKET_UPDATE_DEV) {
					CAM_DBG(CAM_ISP,
						"Adding IFE scratch buffer cfg_mask :0x%x",
						ife_res_info->ife_scratch_cfg_mask);
					rc = cam_isp_ife_add_scratch_buffer_cfg(
						ctx->base[i].idx, ife_res_info->ife_scratch_cfg_mask,
						prepare, &kmd_buf, ctx->res_list_ife_out, ctx);
					if (rc)
						goto end;
				}
			}
...
static int cam_ife_mgr_dump_bus_client_info(
	struct cam_ife_hw_mgr_ctx *ctx, uint32_t res_id)
{
	...
	if (cam_ife_hw_mgr_is_sfe_out_port(res_id))
		hw_mgr_res = &ctx->res_list_sfe_out[(res_id & 0xFF)];
	else if (cam_ife_hw_mgr_is_ife_out_port(res_id))
		hw_mgr_res = &ctx->res_list_ife_out[(res_id & 0xFF)];

	if (!hw_mgr_res) {
		CAM_WARN(CAM_ISP, "Unable to find IFE/SFE out rsrc: 0x%x in ctx: %u",
			res_id, ctx->ctx_index);
		return rc;
	}
...
static void __cam_ife_hw_mgr_dump_csid_path_config_regs(
	struct cam_ife_hw_mgr_ctx *ctx)
{
	int i;
	struct cam_hw_intf        *hw_intf;
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	__cam_ife_hw_mgr_dump_csid_path_config_regs(ctx);
	...
	if (!hw_mgr_res) {
		CAM_ERR(CAM_ISP, "No faulting bus WM res: 0x%x found", res_id);
		return;
	}

	cam_ife_mgr_dump_bus_client_info(ctx, res_id);
}
...
int cam_isp_config_csid_rup_aup(
	struct cam_ife_hw_mgr_ctx *ctx, int32_t use_mup)
{
	int rc = 0, i, j, hw_idx;
	struct cam_isp_hw_mgr_res       *hw_mgr_res;
	struct list_head                *res_list;
	...
	for (i = 0; i < CAM_IFE_CSID_HW_NUM_MAX; i++) {
		...
		if (use_mup < 0) {
			...
			rup_args[i].last_applied_mup = use_mup;
		}
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	for (i = 0; i < ife_buf_info->num_ports; i++) {
	...
	bool need_rup_aup, int32_t use_mup, struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	if (need_rup_aup) {
		rc = cam_isp_config_csid_rup_aup(ctx, use_mup);
		if (rc)
			CAM_ERR(CAM_ISP,
				"RUP/AUP update failed for scratch buffers");
	}

	CAM_INFO(CAM_ISP, "Applied default ctx: %u mup_found: %u last_mup: %u",
		ctx->ctx_index, use_mup, ctx->current_mup);
	return rc;
}

static int cam_ife_mgr_dump_irq_monitor(struct cam_ife_hw_mgr_ctx *ctx)
{
	struct cam_isp_hw_mgr_res *hw_mgr_res = NULL;
	struct cam_isp_resource_node *res;
	uint32_t dummy = 0;
	...
	hw_mgr_res = list_first_entry_or_null(&ctx->res_list_ife_src,
			struct cam_isp_hw_mgr_res, list);
	if (!hw_mgr_res || !hw_mgr_res->hw_res[0]) {
		CAM_ERR(CAM_ISP, "no vfe acquired!");
		return -ENODEV;
	}

	res = hw_mgr_res->hw_res[0];
	rc = res->hw_intf->hw_ops.process_cmd(res->hw_intf->hw_priv,
		CAM_ISP_HW_CMD_DUMP_IRQ_MONITOR, &dummy, sizeof(dummy));
	if (rc)
		CAM_ERR(CAM_ISP, "failed to dump irq monitor");

	return rc;
}

static int cam_ife_mgr_dump_aeb_debug_info(
	struct cam_ife_hw_mgr_ctx *ctx)
{
	int i, j;
	uint32_t res_id_out;
	...
	if (!((ctx->flags.is_aeb_mode) && (ctx->flags.is_sfe_shdr)))
		return 0;

	/* Dump IFE client info for the ones which are provided with scratch buffer */
	if (ctx->sfe_info.ife_scratch_config->num_ports) {
		for (i = 0; i < ctx->sfe_info.ife_scratch_config->num_ports; i++) {
			res_id_out = ctx->sfe_info.ife_scratch_config->buf_info[i].res_id & 0xFF;
			event_info.res_id = ctx->sfe_info.ife_scratch_config->buf_info[i].res_id;
			...
			CAM_INFO(CAM_ISP,
				"IFE scratch info for res: 0x%x scratch_buffer: 0x%x offset: 0x%x",
				ctx->sfe_info.ife_scratch_config->buf_info[i].res_id,
				ctx->sfe_info.ife_scratch_config->buf_info[i].io_addr,
				ctx->sfe_info.ife_scratch_config->buf_info[i].offset);
		}
	}

	notify_config.res_id = CAM_ISP_HW_VFE_IN_CAMIF;
	notify_config.dump_debug = false;
	for (i = 0; i < ctx->num_base; i++) {
		if (ctx->base[i].hw_type != CAM_ISP_HW_TYPE_VFE)
			continue;

		hw_intf = g_ife_hw_mgr.ife_devices[ctx->base[i].idx]->hw_intf;
		if (!hw_intf) {
			CAM_ERR(CAM_ISP, "Invalid hw_intf for HW type: %u idx: %u",
				ctx->base[i].hw_type, ctx->base[i].idx);
			continue;
		}
...
static int cam_ife_mgr_cmd(void *hw_mgr_priv, void *cmd_args)
{
	...
	case CAM_HW_MGR_CMD_INTERNAL:
		...
		case CAM_ISP_HW_MGR_CMD_PROG_DEFAULT_CFG:
			rc = cam_ife_mgr_prog_default_settings(true, isp_hw_cmd_args->u.use_mup, ctx);
			break;
		case CAM_ISP_HW_MGR_CMD_SET_BUBBLE_FLAG:
			ctx->process_bubble = isp_hw_cmd_args->u.process_bubble;
			break;
		case CAM_ISP_HW_MGR_CMD_DUMP_IRQ_MONITOR:
			rc = cam_ife_mgr_dump_irq_monitor(ctx);
			break;
		case CAM_ISP_HW_MGR_CMD_DUMP_AEB_DEBUG_INFO:
			rc = cam_ife_mgr_dump_aeb_debug_info(ctx);
			break;
		case CAM_ISP_HW_MGR_GET_SOF_TS:
			...
			break;
		case CAM_ISP_HW_MGR_CMD_DUMP_BUS_INFO:
			cam_ife_mgr_dump_bus_client_info(ctx, isp_hw_cmd_args->u.resource_handle);
			break;
		default:
	...
	uint32_t res_id, uint32_t last_consumed_addr)
{
	...
	for (i = 0; i < scratch_cfg->num_ports; i++) {
	...
	uint32_t res_id, uint32_t last_consumed_addr)
{
	...
	case CAM_ISP_SFE_OUT_RES_RDI_2:
		...
		if (cam_ife_hw_mgr_last_consumed_addr_check(last_consumed_addr, buf_info)) {
			CAM_DBG(CAM_ISP,
				"SFE RDI%u buf done for scratch - skip ctx notify",
	...
	struct cam_isp_hw_event_info     *event_info)
{
	...
	for (i = 0; i < compdone_evt_info->num_res; i++) {
		...
		if (ife_hw_mgr_ctx->flags.is_sfe_fs || ife_hw_mgr_ctx->flags.is_sfe_shdr) {
			...
			/* Check for IFE ports */
				if (ife_hw_mgr_ctx->sfe_info.ife_scratch_config->num_ports)
			...
			if (rc) {
				if (ife_hw_mgr_ctx->process_bubble)
					CAM_INFO(CAM_ISP,
						"scratch buffer bubble hw %d res_id %u addr 0x%x",
...
DEFINE_SIMPLE_ATTRIBUTE(cam_ife_csid_rx_capture_debug,
