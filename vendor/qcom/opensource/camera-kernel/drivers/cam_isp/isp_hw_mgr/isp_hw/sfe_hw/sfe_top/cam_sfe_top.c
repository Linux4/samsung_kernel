	...
	struct cam_cdm_utils_ops     *cdm_util_ops)
{
	...
	if (fcg_config->num_ch_ctx >= 0 &&
		fcg_config->num_ch_ctx > CAM_ISP_MAX_FCG_CH_CTXS) {
		CAM_ERR(CAM_SFE, "possible out of bounds access %d",
	...
	uint32_t                         arg_size)
{
	...
	if (arg_size != sizeof(struct cam_isp_hw_fcg_cmd)) {
		CAM_ERR(CAM_SFE, "Invalid cmd size, arg_size: %u, expected size: %d",
	...
	void *handler_priv, void *evt_payload_priv)
{
	...
	if (irq_status[0] & path_data->path_reg_data->subscribe_irq_mask) {
		if (irq_status[0] & path_data->path_reg_data->sof_irq_mask) {
			if (path_data->sof_count < 4) {
				CAM_INFO(CAM_SFE, "SFE:%d Received %s SOF %d",
					res->hw_intf->hw_idx,
					res->res_name,
				path_data->sof_count++);
			}
