	...
	struct cam_vfe_bus_ver3_wm_resource_data *wm_data)
{
	...
	cam_io_w_mb(wm_data->ubwc_meta_cfg, wm_data->common_data->mem_base +
		ubwc_regs->meta_cfg);
	CAM_DBG(CAM_ISP, "WM:%d meta stride 0x%X",
	...
	cam_io_w_mb(wm_data->ubwc_mode_cfg,  wm_data->common_data->mem_base +
		ubwc_regs->mode_cfg);
	CAM_DBG(CAM_ISP, "WM:%d ubwc_mode_cfg 0x%X",
		wm_data->index, wm_data->ubwc_mode_cfg);

	
	cam_io_w_mb(wm_data->ubwc_ctrl_2,  wm_data->common_data->mem_base +
		ubwc_regs->ctrl_2);
	CAM_DBG(CAM_ISP, "WM:%d ubwc_ctrl_2 0x%X",
		wm_data->index, wm_data->ubwc_ctrl_2);

	
	cam_io_w_mb(wm_data->ubwc_lossy_threshold_0, wm_data->common_data->mem_base +
		ubwc_regs->lossy_thresh0);
	CAM_DBG(CAM_ISP, "WM:%d lossy_thresh0 0x%X",
		wm_data->index,wm_data->ubwc_lossy_threshold_0);

	cam_io_w_mb(wm_data->ubwc_lossy_threshold_1, wm_data->common_data->mem_base +
		ubwc_regs->lossy_thresh1);
	CAM_DBG(CAM_ISP, "WM:%d lossy_thresh0 0x%X",
		wm_data->index,wm_data->ubwc_lossy_threshold_1);
	
	
	cam_io_w_mb(wm_data->ubwc_offset_lossy_variance, wm_data->common_data->mem_base +
		ubwc_regs->off_lossy_var);
	CAM_DBG(CAM_ISP, "WM:%d off_lossy_var 0x%X",
	...
	if (wm_data->ubwc_bandwidth_limit < 0xFFFF) {
		...
		cam_io_w_mb(wm_data->ubwc_bandwidth_limit, wm_data->common_data->mem_base +
			ubwc_regs->bw_limit);
		CAM_DBG(CAM_ISP, "WM:%d ubwc bw limit 0x%X",
			wm_data->index, wm_data->ubwc_bandwidth_limit);
	}
	...
	uint32_t arg_size)
{
	...
	struct cam_vfe_bus_ver3_reg_offset_ubwc_client *ubwc_regs;
	uint32_t i, val;
	uint32_t iova_addr, iova_offset, stride;
	dma_addr_t iova;
	...
	for (i = 0; i < vfe_out_data->num_wm; i++) {
		...
		/* WM Image address */
		iova = update_buf->wm_update->image_buf[0];
		if (cam_smmu_is_expanded_memory()) {
