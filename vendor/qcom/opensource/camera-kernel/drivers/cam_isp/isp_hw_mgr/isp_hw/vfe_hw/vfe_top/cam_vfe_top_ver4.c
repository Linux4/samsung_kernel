...
#define CAM_VFE_CAMIF_IRQ_SOF_DEBUG_CNT_MAX            3
	...
	struct cam_cdm_utils_ops        *cdm_util_ops)
{
	...
	if (fcg_config->num_ch_ctx >= 0 &&
		...
		CAM_ERR(CAM_SFE, "possible out of bounds %d",
	...
	struct cam_isp_hw_event_info *evt_info)
{
	if ((vfe_priv->enable_sof_irq_debug) &&
		...
		if (vfe_priv->irq_debug_cnt <=
			CAM_VFE_CAMIF_IRQ_SOF_DEBUG_CNT_MAX) {
			CAM_INFO(CAM_ISP, "VFE:%u is_mc:%s hw_ctxt:%d Received SOF",
				vfe_priv->hw_intf->hw_idx,
				CAM_BOOL_TO_YESNO(vfe_priv->reg_data->is_mc_path),
				hw_ctxt_id);
			vfe_priv->irq_debug_cnt++;
		}
