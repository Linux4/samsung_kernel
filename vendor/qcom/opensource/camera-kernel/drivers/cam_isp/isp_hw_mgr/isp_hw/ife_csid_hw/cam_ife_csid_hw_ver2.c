...
#define CAM_CSID_IRQ_SOF_DEBUG_CNT_MAX 9

/* Max CSI Rx irq error count threshold value */
#define CAM_IFE_CSID_MAX_IRQ_ERROR_COUNT               100

/* Max sensor switch out of sync threshold */
#define CAM_IFE_CSID_MAX_OUT_OF_SYNC_ERR_COUNT         30
	...
	struct cam_ife_csid_debug_cfg_args *debug_args)
{
	...
	//Enable SOF and EOF IRQs
	debug_val |= 0x1;
	CAM_INFO(CAM_ISP, "Enabling CSID Debug: %d", debug_val);
	while (debug_val) {
	...
	void *cmd_args)
{
	...
	CAM_INFO(CAM_ISP, "CSID:%u SOF irq %s, Notify CSIPHY: %d",
		csid_hw->hw_intf->hw_idx,
	...
	struct cam_ife_csid_ver2_evt_payload *evt_payload)
{
	...
	while (status) {
		if ((csid_reg->path_irq_desc[bit_pos].bitmask != 0) && (status & 0x1))
			CAM_DBG(CAM_ISP, "CSID[%u] IRQ %s %s timestamp:[%lld.%09lld]",
				csid_hw->hw_intf->hw_idx, irq_reg_tag[index],
				csid_reg->path_irq_desc[bit_pos].desc,
	...
	if (csid_hw->flags.sof_irq_triggered && csid_hw->flags.sfe_en) {
		if ((irq_status & IFE_CSID_VER2_PATH_INFO_INPUT_SOF))
			csid_hw->counters.irq_debug_cnt++;

		CAM_INFO(CAM_ISP, "CSID[%d]%s SOF IRQ DEBUG COUNT %d",
			csid_hw->hw_intf->hw_idx,
			irq_reg_tag[index],
			csid_hw->counters.irq_debug_cnt);
	...
	void *cmd_args, uint32_t arg_size)
{
	...
	rup_args->value = reg_val_pair[i + 1];
	return rc;
}
