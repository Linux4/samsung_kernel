...
static void __sysboot_turn_mcu_system (struct cam_ois_ctrl_t *o_ctrl)
{
	/* Assert NRST reset */
				...
				uint32_t data_size, unsigned long wait_time)
{
	...
	/* transmit Data */
	ret = i2c_master_send(o_ctrl->io_master_info.client, data, data_size);
	if (ret < 0)
	{
...
static int sysboot_i2c_receive_payload (struct cam_ois_ctrl_t *o_ctrl,
	uint8_t *recv, uint32_t data_size, unsigned long wait_time)
{
	int ret = 0;
	...
	uint8_t *cmd, uint32_t cmd_size, uint32_t data_size)
{
	...
	for (retry = 0; retry < BOOT_I2C_SYNC_RETRY_COUNT; ++retry)
	{
		...
		/* receive payload */
		ret = sysboot_i2c_receive_payload(o_ctrl, recv, data_size, BOOT_I2C_WAIT_RESP_TMOUT);
		if (ret < 0)
			continue;
...
int sysboot_i2c_read(struct cam_ois_ctrl_t *o_ctrl, uint32_t address, uint8_t *dst, size_t len)
{
	...
	*(uint32_t *)startaddr = HTONL(address);
	...
	for (retry = 0; retry < BOOT_I2C_SYNC_RETRY_COUNT; ++retry)
	{
		...
		/* receive payload */
		ret = sysboot_i2c_receive_payload(o_ctrl, dst, len, BOOT_I2C_WAIT_RESP_TMOUT);
		if (ret < 0)
			continue;
...
static void cam_ois_set_calib_addr (uint16_t subdev_id, struct cam_ois_cal_addr *cal_addr)
{
	switch (subdev_id) {
		case SEC_WIDE_SENSOR:
			cal_addr->x_write_addr = AKM_W_X_WRITE_UCLD;
			cal_addr->x_read_addr = AKM_W_X_READ_UCLD;
			cal_addr->y_write_addr = AKM_W_Y_WRITE_UCLD;
			cal_addr->y_read_addr = AKM_W_Y_READ_UCLD;
			break;

		case SEC_TELE_SENSOR:
			cal_addr->x_write_addr = AKM_T_X_WRITE_UCLD;
			cal_addr->x_read_addr = AKM_T_X_READ_UCLD;
			cal_addr->y_write_addr = AKM_T_Y_WRITE_UCLD;
			cal_addr->y_read_addr = AKM_T_Y_READ_UCLD;
			break;
...
static int cam_ois_read_stored_cal_data (struct cam_ois_ctrl_t *o_ctrl, struct cam_ois_cal_buf *cal_buf,
				struct cam_ois_cal_addr *cal_addr)
{
	uint16_t uiTemp = 0;

	/* Read stored calibration mark */
	cam_ois_bypass_mode1_i2c_read(o_ctrl, cal_addr->x_read_addr, 0xe4, cal_buf->rx_buf, 1);
	CAM_DBG(CAM_OIS, "Write Reg : 0xE4, Data : 0x%x", cal_buf->rx_buf[0]);
	if (cal_buf->rx_buf[0] != 1)
	{
	...
	cam_ois_bypass_mode1_i2c_read(o_ctrl, cal_addr->x_read_addr, 0xe5, cal_buf->rx_buf, 1);
	cal_buf->af_position = (uint16_t)cal_buf->rx_buf[0] << 4;
	CAM_DBG(CAM_OIS, "Write Reg : 0xE5, Data : 0x%x", cal_buf->af_position);

	/* Read stored PCAL and NCAL of X axis */
	cam_ois_bypass_mode1_i2c_read(o_ctrl, cal_addr->x_read_addr, 0x04, cal_buf->rx_buf, 4);
	uiTemp = ((uint16_t)cal_buf->rx_buf[0] << 8) & 0x8000;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[0] << 1) & 0x00fe;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[1] >> 7) & 0x0001;
	cal_buf->ideal_pCal[0] = (int16_t)uiTemp;
	uiTemp = ((uint16_t)cal_buf->rx_buf[2] << 8) & 0x8000;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[2] << 1) & 0x00fe;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[3] >> 7) & 0x0001;
	cal_buf->ideal_nCal[0] = (int16_t)uiTemp;
	CAM_DBG(CAM_OIS, "Read Reg : 0x04, Data : %d", cal_buf->ideal_pCal[0]);
	CAM_DBG(CAM_OIS, "Read Reg : 0x06, Data : %d", cal_buf->ideal_nCal[0]);

	/* Read stored PCAL and NCAL for Y axis */
	cam_ois_bypass_mode1_i2c_read(o_ctrl, cal_addr->y_read_addr, 0x04, cal_buf->rx_buf, 4);
	uiTemp = ((uint16_t)cal_buf->rx_buf[0] << 8) & 0x8000;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[0] << 1) & 0x00fe;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[1] >> 7) & 0x0001;
	cal_buf->ideal_pCal[1] = (int16_t)uiTemp;
	uiTemp = ((uint16_t)cal_buf->rx_buf[2] << 8) & 0x8000;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[2] << 1) & 0x00fe;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[3] >> 7) & 0x0001;
	cal_buf->ideal_nCal[1] = (int16_t)uiTemp;
	CAM_DBG(CAM_OIS, "Read Reg : 0x04, Data : %d", cal_buf->ideal_pCal[1]);
	CAM_DBG(CAM_OIS, "Read Reg : 0x06, Data : %d", cal_buf->ideal_nCal[1]);

	/* Move AF to best position which read from EEPROM */
	if (cal_buf->af_position >= NUM_AF_POSITION) {
		CAM_ERR(CAM_OIS, "af position error %u", cal_buf->af_position);
		return -1;
	}

	CAM_DBG(CAM_OIS, "ois read bypass1 af position %X", cal_buf->af_position);
	return 0;
}


static int cam_ois_write_cal_data (struct cam_ois_ctrl_t *o_ctrl, struct cam_ois_cal_buf *cal_buf,
				struct cam_ois_cal_addr *cal_addr)
{
	uint16_t uiTemp = 0;

	cal_buf->tx_buf[0] = 0x3b;
	cam_ois_bypass_mode1_i2c_write(o_ctrl, cal_addr->x_write_addr, 0xae, cal_buf->tx_buf, 1);
	cam_ois_bypass_mode1_i2c_write(o_ctrl, cal_addr->y_write_addr, 0xae, cal_buf->tx_buf, 1);
	CAM_DBG(CAM_OIS, "Write Reg : 0xae, Data : 0x%x", cal_buf->tx_buf[0]);

	/* Start hall calibration for X axis */
	cal_buf->tx_buf[0] = 0x01;
	cam_ois_bypass_mode1_i2c_write(o_ctrl, cal_addr->x_write_addr, 0x02, cal_buf->tx_buf, 1);
	msleep(150); // 150mSec

	/* Start hall calibration for Y axis */
	cam_ois_bypass_mode1_i2c_write(o_ctrl, cal_addr->y_write_addr, 0x02, cal_buf->tx_buf, 1);
	msleep(150); // 150mSec

	/* Clear setting Mode */
	cal_buf->tx_buf[0] = 0x00;
	cam_ois_bypass_mode1_i2c_write(o_ctrl, cal_addr->x_write_addr, 0xae, cal_buf->tx_buf, 1);
	cam_ois_bypass_mode1_i2c_write(o_ctrl, cal_addr->y_write_addr, 0xae, cal_buf->tx_buf, 1);
	CAM_DBG(CAM_OIS, "Write Reg : 0xae, Data : 0x%x", cal_buf->tx_buf[0]);
	...
	uiTemp |= ((uint16_t)cal_buf->rx_buf[0] << 1) & 0x00fe;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[1] >> 7) & 0x0001;
	cal_buf->current_pCal[0] = (int16_t)uiTemp;
	uiTemp = ((uint16_t)cal_buf->rx_buf[2] << 8) & 0x8000;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[2] << 1) & 0x00fe;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[3] >> 7) & 0x0001;
	cal_buf->current_nCal[0] = (int16_t)uiTemp;
	CAM_DBG(CAM_OIS, "Read Reg : 0x04, Data : %d", cal_buf->current_pCal[0]);
	CAM_DBG(CAM_OIS, "Read Reg : 0x06, Data : %d", cal_buf->current_nCal[0]);
	...
	uiTemp |= ((uint16_t)cal_buf->rx_buf[0] << 1) & 0x00fe;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[1] >> 7) & 0x0001;
	cal_buf->current_pCal[1] = (int16_t)uiTemp;
	uiTemp = ((uint16_t)cal_buf->rx_buf[2] << 8) & 0x8000;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[2] << 1) & 0x00fe;
	uiTemp |= ((uint16_t)cal_buf->rx_buf[3] >> 7) & 0x0001;
	cal_buf->current_nCal[1] = (int16_t)uiTemp;
	CAM_DBG(CAM_OIS, "Read Reg : 0x04, Data : %d", cal_buf->current_pCal[1]);
	CAM_DBG(CAM_OIS, "Read Reg : 0x06, Data : %d", cal_buf->current_nCal[1]);
...
{
	struct cam_ois_cal_addr cal_addr;
	struct cam_ois_cal_buf cal_buf;
	int rc = 0;
	...
	cam_ois_set_calib_addr(subdev_id, &cal_addr);

	/* Read stored calibration data */
	rc = cam_ois_read_stored_cal_data(o_ctrl, &cal_buf, &cal_addr);
	...
	if (g_a_ctrls[subdev_id] != NULL) {
		...
		if (!g_a_ctrls[subdev_id]->use_mcu) {
			cam_actuator_move_for_ois_read_hall_cal_test(g_a_ctrls[subdev_id], cal_buf.af_position);
			msleep(50);
		}
	}

	/* Change setting Mode for Hall cal */
	cam_ois_write_cal_data (o_ctrl, &cal_buf, &cal_addr);

	// Return the result
	result[0] = cal_buf.ideal_pCal[0];      // RESULT
	result[1] = cal_buf.ideal_nCal[0];      // RESULT
	result[2] = cal_buf.ideal_pCal[1];      // RESULT
	result[3] = cal_buf.ideal_nCal[1];      // RESULT
	result[4] = cal_buf.current_pCal[0];    // RESULT
	result[5] = cal_buf.current_nCal[0];    // RESULT
	result[6] = cal_buf.current_pCal[1];    // RESULT
	result[7] = cal_buf.current_nCal[1];    // RESULT
