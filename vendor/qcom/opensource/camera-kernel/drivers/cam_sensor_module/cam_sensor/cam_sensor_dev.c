		...
		struct cam_subdev_msg_payload *msg)
{
	...
	case CAM_SUBDEV_MESSAGE_IRQ_ERR:
		 ...
		 * by using is_full_recovery from ife/csid */
#if defined(CONFIG_SENSOR_RETENTION)
		 s_ctrl->soc_info.disable_sensor_retention = is_full_recovery;
#endif
		break;
	default:
		break;
...
static int cam_sensor_init_subdev_params(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	s_ctrl->v4l2_dev_str.msg_cb =
		cam_sensor_subdev_handle_message;
