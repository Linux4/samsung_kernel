	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_info("%s: threshold %u, frequency %d, aplitude %d\n",
		__func__, ois_autotest_threshold,
		ois_autotest_frequency, ois_autotest_amplitude);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (g_o_ctrl->cam_ois_state != CAM_OIS_START) {
		pr_err("%s: Not in right state to set ois mode %d",
			__func__, g_o_ctrl->cam_ois_state);
		goto error;
	}

	pr_info("%s: Configure OIS driver output 0x%x\n",
		__func__, g_o_ctrl->driver_output_mask);
