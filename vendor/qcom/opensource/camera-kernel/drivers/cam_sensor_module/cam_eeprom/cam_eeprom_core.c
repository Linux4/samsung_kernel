 ...
 */
/* M55 code for SR-QN6887A-01-90 by hanhaoran at 20230916 start */
	...
	struct cam_eeprom_memory_block_t *block)
{
	...
	for (j = 0; j < block->num_map; j++) {
		...
		if (emap[j].page.valid_size) {
			...
			/*M55 code for SR-QN6887A-837 by zhongquan at 20231116 begin */
			i2c_reg_settings.delay = emap[j].page.delay;
			/*M55 code for SR-QN6887A-837 by zhongquan at 20231116 end */
		...
		if (emap[j].mem.valid_size) {
            if(soc_private->i2c_info.slave_addr == 0x44){
...
/* M55 code for SR-QN6887A-01-90 by hanhaoran at 20230916 end */
		if (emap[j].pageen.valid_size) {
			i2c_reg_settings.addr_type = emap[j].pageen.addr_type;
			i2c_reg_settings.data_type = emap[j].pageen.data_type;
			i2c_reg_settings.size = 1;
