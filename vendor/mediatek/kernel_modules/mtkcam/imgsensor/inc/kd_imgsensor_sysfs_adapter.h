...
#define IMGSENSOR_READ_OTP_CAL(client, sensor_id, addr, data, size)                             imgsensor_read_otp_cal(client, sensor_id, addr, data, size)
...
#define IMGSENSOR_SYSFS_WRITE_CDR_RESULT(result, error_time)                                    imgsensor_sysfs_write_cdr_result(result, error_time)
...
static inline unsigned int IMGSENSOR_READ_OTP_CAL(struct i2c_client *client, unsigned int sensor_id,unsigned int addr, unsigned char *data, unsigned int size) {
	return -1;
}
...
static inline int IMGSENSOR_SYSFS_WRITE_CDR_RESULT(bool result, int error_time) {
	return 0;
}
