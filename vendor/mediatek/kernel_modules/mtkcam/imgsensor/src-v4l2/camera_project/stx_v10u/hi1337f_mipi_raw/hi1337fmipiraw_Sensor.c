...
static int hi1337f_close(struct subdrv_ctx *ctx);
...
static int hi1337f_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hi1337f_set_test_pattern_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static struct subdrv_feature_control feature_control_list[] = {
	...
	{SENSOR_FEATURE_SET_TEST_PATTERN, hi1337f_set_test_pattern},
	{SENSOR_FEATURE_SET_TEST_PATTERN_DATA, hi1337f_set_test_pattern_data},
...
static struct subdrv_mode_struct mode_struct[] = {
	...
	// frame_desc_vid
	{
		...
		.mode_setting_table = hi1337f_video_setting,
		.mode_setting_len = ARRAY_SIZE(hi1337f_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	// frame_desc_cus1
	{
		...
		.mode_setting_table = hi1337f_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(hi1337f_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static int hi1337f_close(struct subdrv_ctx *ctx)
{
	DRV_LOG_MUST(ctx, "hi1337f closed\n");
	return 0;
}
...
void hi1337f_sensor_wait_stream_off(struct subdrv_ctx *ctx)
{
	u32 i = 0;
	u32 timeout = 0;
	u16 pll_enable = 0;
	...
	mDELAY(init_delay);
	for (i = 0; i < timeout; i++) {
		pll_enable = subdrv_i2c_rd_u16(ctx, HI1337F_PLL_ENABLE_ADDR);
...
static void hi1337f_set_streaming_control(struct subdrv_ctx *ctx, bool enable)
{
	...
	if (enable) {
		...
		if (ctx->s_ctx.chk_s_off_end)
			hi1337f_sensor_wait_stream_off(ctx); /*Wait streamoff sensor specific logic*/
