...
static int sysfs_kernel_read(const char *name, unsigned char *data, unsigned int *size)
{
	...
	*size = fw->size;
	pr_info("[%s] file name:%s, size:%d Bytes", __func__, name, (int)fw->size);
...
enum IMGSENSOR_SENSOR_IDX imgsensor_get_sensor_idx(unsigned int sensor_id)
{
	...
	case SENSOR_POSITION_REAR4:
		return IMGSENSOR_SENSOR_IDX_SUB3;
...
unsigned int imgsensor_get_sensor_position(unsigned int sensor_id)
{
	int i = 0;
	int max_num_of_rom_info = ARRAY_SIZE(vendor_rom_info);
	enum sensor_position imgsensor_position = SENSOR_POSITION_NONE;
	...
	for (i = 0; i < max_num_of_rom_info; i++) {
		if (vendor_rom_info[i].sensor_id_with_rom == sensor_id) {
			imgsensor_position = vendor_rom_info[i].sensor_position;
			pr_info("[%s] sensor_id: %#06x, sensor position: %d", __func__, sensor_id, imgsensor_position);
			break;
		}
	}
	return (unsigned int)imgsensor_position;
}
...
#endif // FRONT_CAMERA||REAR_CAMERA2||FRONT_CAMERA2||REAR_CAMERA3||REAR_CAMERA4
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "c", 1)) {
		imgsensor_sec_get_hw_param(&ec_param, SENSOR_POSITION_FRONT2);
...
static DEVICE_ATTR(SVC_front2_module, S_IRUGO, camera_front2_moduleid_show, NULL);
...
#endif
#if defined(FRONT_CAMERA2)
	&dev_attr_SVC_front2_module.attr,
#endif
	NULL, // DO NOT REMOVE
};
...
#ifdef FRONT_CAMERA2
static int create_front2_sysfs(void)
{
	int ret = 0;
	if (camera_front_dev == NULL) {
		camera_front_dev = device_create(camera_class, NULL, 1, NULL, "front");
...
void imgsensor_destroy_front2_sysfs(void)
{
...
struct imgsensor_cdr_info cdr_info = {-1, {'\0'}, {'\0'}};

static ssize_t cam_cdr_value_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, sizeof(cdr_info.value), "%d\n", cdr_info.value);
}
...
{
	char *token = NULL;
	char *sbuf = kzalloc(sizeof(char) * (size + 1), GFP_KERNEL);
	char *s = sbuf;
	...
	if(!sbuf) {
		...
		goto ERR_CDR_VALUE_STORE;
	}
	memcpy(sbuf, buf, size);

	token = strsep(&s, ",");
	while (token != NULL && num_para < 10) {
		if (strlen(token)) {
			arg[num_para] = token;
			pr_debug("%s, arg[%d]:%s\n", __func__, num_para, arg[num_para]);
	...
	/* arg[1]: cdr value input, input is hex value */
	if(kstrtoint(arg[1], 16, &cdr_info.value)) {
		pr_err("%s, converting value failed\n", __func__);
		goto ERR_CDR_VALUE_STORE;
	}
	pr_info("[%s] cdr_value:%#x\n", __func__, cdr_info.value);
...
int get_cam_cdr_value(void)
{
	return cdr_info.value;
}

int set_cam_cdr_value(int data)
{
	cdr_info.value = data;
	return 0;
}
...
int imgsensor_sysfs_write_cdr_result(bool result, int error_time)
{
	if (result) {
		snprintf(cdr_info.result, sizeof(cdr_info.result), "%s", "\n");
		pr_info("[%s]: success\n", __func__);
	} else {
		snprintf(cdr_info.result, sizeof(cdr_info.result), "0,%d\n", error_time);
		pr_info("[%s]: fail\n", __func__);
	}
	return 0;
}
...
#if IS_ENABLED(CONFIG_LEDS_SM5714)
	return get_sm5714_rear_flash(buf);
#else
	return 0;
#endif
}
...
{
#if IS_ENABLED(CONFIG_LEDS_SM5714)
	return set_sm5714_rear_flash(buf, size);
#else
	return 0;
#endif
}

static DEVICE_ATTR(rear_flash, 0664, rear_flash_show, rear_flash_store);

static int create_flash_sysfs(void)
{
	int ret = 0;

	if (camera_flash_dev == NULL)
		camera_flash_dev = device_create(camera_class, NULL, 1, NULL, "flash");

	if (IS_ERR(camera_flash_dev)) {
		pr_err("imgsensor_sysfs_init: failed to create device(flash)\n");
		return -ENODEV;
	}

	if (device_create_file(camera_flash_dev, &dev_attr_rear_flash) < 0)
		printk(KERN_ERR "failed to create rear rear_flash device file, %s\n",
			dev_attr_rear_flash.attr.name);

	return ret;
}

void imgsensor_destroy_flash_sysfs(void)
{
	device_remove_file(camera_rear_dev, &dev_attr_rear_flash);
}
#endif
...
#endif //FRONT_CAMERA
#ifdef FRONT_CAMERA2
		imgsensor_destroy_front2_sysfs();
#endif //FRONT_CAMERA2
	}
...
MODULE_LICENSE("GPL");
EXPORT_SYMBOL_GPL(imgsensor_sysfs_write_cdr_result);
...
EXPORT_SYMBOL_GPL(get_cam_cdr_value);
EXPORT_SYMBOL_GPL(set_cam_cdr_value);
