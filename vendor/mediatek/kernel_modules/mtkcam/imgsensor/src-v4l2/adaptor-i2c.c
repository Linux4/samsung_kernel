...
int adaptor_i2c_wr_regs_u16_burst(struct i2c_client *i2c_client, u16 addr, u16 *list, u32 len)
{
	u8 *buf;
	struct i2c_msg msg;
	int ret, total;
	...
	while (IDX < total) {
		reg_addr = pdata[IDX];
		if (tosend == 0) {
			buf[tosend++] = (char)(reg_addr >> 8);
			buf[tosend++] = (char)(reg_addr & 0xFF);
		}

		data = pdata[IDX + 1];
		buf[tosend++] = (char)(data >> 8);
		buf[tosend++] = (char)(data & 0xFF);
		IDX += 2;

		msg.len = tosend;

		if ((IDX == total) || ((IDX < total) && (pdata[IDX] != (reg_addr + 2))) || (tosend >= MAX_VAL_NUM_U16)) {
			ret = i2c_transfer(i2c_client->adapter, &msg, 1);
			if (ret < 0) {
				adaptor_i2c_display_error_log(i2c_client, addr, reg_addr, ret);
				kfree(buf);
				return -EIO;
			}
			tosend = 0;
