	...
	for (i = 0; i < ctx->subdrv->pw_seq_cnt; i++) {
		...
		if (subctx->power_on_profile_en) {
			if (ent->id == HW_ID_MCLK_DRIVING_CURRENT)
				adaptor_logi(ctx, "[profile] power on[%s](%d mA), delay:%d ms\n", hw_id_names[ent->id], ent->val, ent->delay);
			else if (ent->id == HW_ID_MCLK)
				adaptor_logi(ctx, "[profile] power on[%s](%d MHz), delay:%d ms\n", hw_id_names[ent->id], ent->val, ent->delay);
			else
				adaptor_logi(ctx, "[profile] power on[%s], val:%d, delay:%d ms\n", hw_id_names[ent->id], ent->val, ent->delay);
		}
	}
...
int do_hw_power_off(struct adaptor_ctx *ctx)
{
	...
	for (i = ctx->subdrv->pw_seq_cnt - 1; i >= 0; i--) {
		...
		adaptor_logd(ctx, "[profile] power off[%s], delay:%d ms\n", hw_id_names[ent->id], ent->delay);
