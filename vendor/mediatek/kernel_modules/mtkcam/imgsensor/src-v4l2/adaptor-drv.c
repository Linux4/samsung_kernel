...
static int search_sensor_with_fixed_position(struct adaptor_ctx *ctx)
{
	...
	if (of_sensor_names_cnt == 0) {
		dev_info(ctx->dev,"[%s][ERR] of_sensor_names_cnt is 0\n", __func__);
		return -EIO;
	}
	...
	for (i = 0; i < of_sensor_names_cnt; i++) {
		for (j = 0; j < ARRAY_SIZE(imgsensor_subdrvs); j++) {
			...
			if (!strcmp(subdrv->name, of_sensor_names[i])) {
	...
	dev_info(ctx->dev, "[%s] default sensor:%s found\n", __func__, ctx->subdrv->name);
	return 0;
}
#else
...
static int imgsensor_probe(struct i2c_client *client)
{
	...
	if (!strcmp(dev->of_node->name, "sensor0")) {
		...
		if (!sysfs_node) {
			dev_info(dev, "compatiable node:(mediatek,imgsensor-sysfs) not found\n");
			return -EINVAL;
		}
