...
static struct regulator *g_vbb2_supply = NULL;
...
int connv3_plt_pmic_initial_setting_mt6989(struct platform_device *pdev, struct connv3_dev_cb* dev_cb)
{
	...
	g_vbb2_supply = devm_regulator_get(&pdev->dev, "conninfra_regulator");
	if (IS_ERR(g_vbb2_supply)) {
		pr_notice("Regulator_get conninfra_regulator fail\n");
		g_vbb2_supply = NULL;
	}
...
int connv3_plt_pmic_vsel_ctrl_mt6989(u32 enable)
{
	...
	if (enable) {
		if (g_vbb2_supply == NULL) {
			pr_err("[%s] vbb2 supply is null\n", __func__);
			return -1;
		}

		if (regulator_is_enabled(g_vbb2_supply) == 0) {
			ret = -1;
			for (i = 0; i < 3; i++) {
				if (regulator_enable(g_vbb2_supply))
					pr_notice("[%s] Failed to enable vbb2 supply, retry=[%d]\n",
						__func__, i);
				if (regulator_is_enabled(g_vbb2_supply)) {
					/* enabled as 3.84V */
					ret = 0;
					pr_info("[%s] vbb2 supply is enabled\n", __func__);
					break;
				} else
					msleep(100);
			}
		} else
			pr_info("[%s] vbb2 supply is already enabled\n", __func__);
