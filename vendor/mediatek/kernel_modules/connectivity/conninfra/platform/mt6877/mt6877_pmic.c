...
int consys_plt_pmic_get_from_dts_mt6877(struct platform_device *pdev, struct conninfra_dev_cb* dev_cb)
{
	...
	if (wifi_node) {
		ret = of_property_read_string(wifi_node, "flavor_bin", &fw_flavor);

		if (ret == 0 && strcmp(fw_flavor, "c") == 0)
			wifi_flavor = true;
	}

	if (wifi_flavor)
		reg_VCN33_2_WIFI = NULL;
	else
		reg_VCN33_2_WIFI = regulator_get(&pdev->dev, "vcn33_2_wifi");

	if (!reg_VCN33_2_WIFI)
		pr_notice("Regulator_get VCN33_2_WIFI fail, wifi_flavor=[%d]\n", wifi_flavor);
...
int consys_pmic_vcn33_2_power_ctl_mt6877(bool enable)
{
	...
	if (!reg_VCN33_2_WIFI) {
		pr_notice("There is no VCN33_2_WIFI, return fail\n");
		return -1;
	}
