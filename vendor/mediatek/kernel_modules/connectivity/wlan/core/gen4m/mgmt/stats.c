	...
	if (u2UdpDstPort == UDP_PORT_DHCPS || u2UdpDstPort == UDP_PORT_DHCPC) {
		...
		case EVENT_RX:
			...
			if (u4DhcpMagicCode == DHCP_MAGIC_NUMBER) {
				...
				if (g_u4LatestDHCPTid == u4TransID) {
		...
		struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	...
	case IP_PRO_UDP:
		statsParseUDPInfo(pvPacket, pucEthBody,
	...
	struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	...
	case ETH_P_IPV4:
	{
		...
		statsParseIPV4Info(pvPacket, pucEthBody,
	...
	case ETH_P_IPV6:
	{
		...
		case IPV6_PROTOCOL_ICMPV6:
...
#if 0
					/* TODO: Check RA packet else where */
			...
			case EVENT_TX:
				case ICMPV6_TYPE_NEIGHBOR_SOLICITATION:
					DBGLOG_LIMITED(RX, INFO,
						"<TX><IPv6> ICMPV6 Neighbor Solicitation\n");
					break;
				default:
					DBGLOG_LIMITED(TX, INFO,
	...
	struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	...
	statsParsePktInfo(pPkt, prSwRfb->pvPacket, 0, EVENT_RX, prAdapter, ucBssIndex);
	...
	struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	...
	statsParsePktInfo(pPktBuf, pvPacket, 0, EVENT_TX, prAdapter, ucBssIndex);
}
