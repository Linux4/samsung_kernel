...
nanHardStartXmit(struct sk_buff *prSkb, struct net_device *prDev)
{
	...
	struct sk_buff *prMcastSkb = NULL;
	...
	if (IS_BMCAST_MAC_ADDR(aucMacDestAddr)) {
		...
		if (is_ipv6_neighbor_soliciation(prMcastSkb,
						 aucUcastMacDestAddr)) {
			DBGLOG(NAN, LOUD,
		...
		__nanHardStartXmit(prGlueInfo, prDev, ucBssIndex, prMcastSkb);
	}
