...
static int btmtk_proc_show(struct seq_file *m, void *v)
{
	...
	if (pp_bdev[hci_idx] && pp_bdev[hci_idx]->rom_patch_bin_file_name)
		(void)seq_printf(m, "fw_file_name:%s\n", pp_bdev[hci_idx]->rom_patch_bin_file_name);
	else
		(void)seq_printf(m, "fw_file_name:null\n");
	return 0;
}
...
int btmtk_dispatch_fwlog_bluetooth_kpi(struct btmtk_dev *bdev, u8 *buf, int len, u8 type)
{
	...
	if (g_fwlog->btmtk_bluetooth_kpi &&
		...
		if (fwlog_blocking_warn == 0) {
			...
			BTMTK_INFO("btmtk_usb fwlog queue size is full(bluetooth_kpi)");
		}
	}
	return ret;
}
	...
	if ((bt_cb(skb)->pkt_type == HCI_ACLDATA_PKT) &&
		...
		if (skb_queue_len(&g_fwlog->fwlog_queue) < FWLOG_ASSERT_QUEUE_COUNT) {
			...
			if (fwlog_fwdump_blocking_warn == 0) {
				...
				BTMTK_INFO("btmtk fwlog queue size is full(coredump)");
			}
		}
...
coredump_fail_unlock:
		...
		/* picus or syslog */
		fwlog_count++;
		BTMTK_INFO_LIMITTED("fw log counter[%d], log_level[%d]",
				fwlog_count, bmain_info->fw_log_on);
		/* send to log handler, remove acl header (FF 05 LL LL) */
		bmain_info->hif_hook.log_handler(skb->data + 4, skb->len - 4);
		...
		if (!is_rx_queue_res_available(RING_BUFFER_SIZE >> 2)) {
			...
			/* FF 05 08 00 ... */
			skb->len = 12;
			BTMTK_DBG_RAW(skb->data, skb->len, "%s: FW log lost:", __func__);
		}

		/* Drop the firmware log during the suspend flow to avoid waking up the system. */
		if (unlikely(bmain_info->suspend_entry)) {
			BTMTK_INFO_RAW(skb->data, skb->len, "%s: drop FW log in suspend flow:", __func__);
			return 1;
		}
		...
		return 1;
				...
				skb->data[2] == 0x02 &&  skb->data[3] == 0x56 && bmain_info->hif_hook.fw_sleep_handler) {
		BTMTK_INFO_RAW(skb->data, skb->len, "%s: fw_sleep_event len[%d] %02x", __func__,
					skb->len + 1, hci_skb_pkt_type(skb));
		bmain_info->hif_hook.fw_sleep_handler(bdev);
		return 1;
