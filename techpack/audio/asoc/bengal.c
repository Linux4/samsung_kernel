...
static int bengal_amp_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_dai *codec_dai = rtd->codec_dai;

	pr_info("%s codec_dai.name %s\n", __func__, codec_dai->name);

	if (!strncmp((codec_dai->name), "tas", 3))
...
#if IS_ENABLED(CONFIG_SND_SOC_AMP_DUALIZATION)
#if defined(CONFIG_SND_SOC_TFA9878)
static struct snd_soc_dai_link amp_dualization_dai_link_list[] = {
	{
...
static void update_amp_dai_link(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "%s: interface name %s\n", __func__, interface_name);

	for (i = 0; i < ARRAY_SIZE(msm_mi2s_be_dai_links); i++) {
		if (!strcmp((msm_mi2s_be_dai_links[i].name), interface_name)) {
			memcpy(&(msm_mi2s_be_dai_links[i].codec_name),
				&(amp_dualization_dai_link_list[0].codec_name),
				sizeof(amp_dualization_dai_link_list[0].codec_name));
			memcpy(&(msm_mi2s_be_dai_links[i].codec_dai_name),
				&(amp_dualization_dai_link_list[0].codec_dai_name),
				sizeof(amp_dualization_dai_link_list[0].codec_dai_name));
			memcpy(&(msm_mi2s_be_dai_links[i+1].codec_name),
				&(amp_dualization_dai_link_list[0].codec_name),
				sizeof(amp_dualization_dai_link_list[0].codec_name));
			memcpy(&(msm_mi2s_be_dai_links[i+1].codec_dai_name),
				&(amp_dualization_dai_link_list[0].codec_dai_name),
				sizeof(amp_dualization_dai_link_list[0].codec_dai_name));

			dev_info(&pdev->dev, "codec_name %s codec_dai_name %s\n", __func__,
				msm_mi2s_be_dai_links[i].codec_name,
...
#if defined(CONFIG_SND_SOC_AMP_DUALIZATION)
