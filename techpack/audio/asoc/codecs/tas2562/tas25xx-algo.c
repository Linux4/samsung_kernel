...
void tas25xx_update_big_data(void)
{
	...
	for (iter = 0; iter < p_tas25xx_algo->spk_count; iter++) {
		...
		param_id = (TAS_SA_EXC_TEMP_STAT)|((uint32_t)(iter+1)<<24)|
			((uint32_t)(sizeof(data)/sizeof(uint32_t))<<16);
					...
					char *buf)
{
	...
	if (attr == &dev_attr_temp_max_persist_r)
		ret = snprintf(buf, MAX_STRING, "%d", p_tas25xx_algo->b_data[1].temp_max_persist);
	else
		ret = snprintf(buf, MAX_STRING, "%d", p_tas25xx_algo->b_data[0].temp_max_persist);
					...
					char *buf)
{
	...
	if (attr == &dev_attr_temp_over_count_r)
		ret = snprintf(buf, MAX_STRING, "%d", p_tas25xx_algo->b_data[1].temp_over_count);
	else
		ret = snprintf(buf, MAX_STRING, "%d", p_tas25xx_algo->b_data[0].temp_over_count);
