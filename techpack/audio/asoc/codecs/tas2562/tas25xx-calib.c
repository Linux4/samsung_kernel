...
static int smartamp_params_ctrl(uint8_t *input, u8 dir, u8 count)
{
	...
	if (ppacket->device_id == DEVICE1) {
		...
		pr_info("[TI-SmartPA:%s] Rcvd Device id for device 1, %x\n",
				__func__, ppacket->device_id);
	} else if (ppacket->device_id == DEVICE2) {
		paramid = (paramid | (index) | (length << 16) | (2 << 24));
		pr_info("[TI-SmartPA:%s] Rcvd Device id for device 2, %x\n",
				__func__, ppacket->device_id);
	} else {
	...
	if (paramid == CAPI_V2_TAS_TX_ENABLE ||
		...
		pr_err("[TI-SmartPA:%s] %s Device 0x%x params failed, paramid mismatch\n", __func__,
				dir == TAS_GET_PARAM ? "get" : "set", ppacket->device_id);
		kfree(ppacket);
		return -EFAULT;
	}
	#endif
	...
	if (ret)
		pr_err("[TI-SmartPA:%s] %s Device 0x%x params failed from afe, ret=%x\n", __func__,
				dir == TAS_GET_PARAM ? "get" : "set", ppacket->device_id, ret);
	else
