...
//+P86801AA1, zhangtao10.wt, ADD, 2023/05/04, aw smartpa compatibility
	if (aw882xx->aw_pa->chip_id == 0x2013) {
		ret = request_firmware(&cont, ACF_BIN_2013, aw882xx->dev);
		aw_dev_info(aw882xx->dev, "select [%s]!", ACF_BIN_2013);
	} else if (aw882xx->aw_pa->chip_id == 0x2113) {
		ret = request_firmware(&cont, ACF_BIN_2113, aw882xx->dev);
		aw_dev_info(aw882xx->dev, "select [%s]!", ACF_BIN_2113);
	} else {
 ...
 *****************************************************/
//+P86801AA1, zhangtao10.wt, ADD, 2023/05/04, aw smartpa compatibility
#define HW_ID0_GPIONUM (51+355)
#define HW_ID1_GPIONUM (52+355)
#define HW_ID2_GPIONUM (53+355)
static int read_gpio_status(struct aw882xx *aw882xx)
{
	int hw_id = 0;

	if(gpio_is_valid(HW_ID0_GPIONUM) && gpio_is_valid(HW_ID1_GPIONUM) && gpio_is_valid(HW_ID2_GPIONUM)) {
		hw_id = gpio_get_value(HW_ID0_GPIONUM) + gpio_get_value(HW_ID1_GPIONUM) + gpio_get_value(HW_ID2_GPIONUM);
		aw_dev_info(aw882xx->dev, "hw_id total is %d", hw_id);
	} else {
		aw_dev_info(aw882xx->dev, "has no hw_id gpio");
	}
...
//+P86801AA1, zhangtao10.wt, ADD, 2023/05/13, smartpa compatibility
	if (smartpa_type == FS16XX) {
		pr_info("%s fsm device has been registered\n",__func__);
		return EEXIST;
	}
//-P86801AA1, zhangtao10.wt, ADD, 2023/05/13, smartpa compatibility
	...
	//+P86801AA1, daisiqing.wt, ADD, 2022/04/28, add smartpa info for mmitest
	if (aw882xx->aw_pa->chip_id == 0x2013) {
		smartpa_type =snd_soc_set_smartpa_type("aw882xx_i2c_probe", AW88266);
	} else if (aw882xx->aw_pa->chip_id == 0x2113) {
		smartpa_type =snd_soc_set_smartpa_type("aw882xx_i2c_probe", AW88257);	//+P86801AA1, zhangtao10.wt, ADD, 2023/05/04, aw smartpa compatibility
	} else {
		hardwareinfo_set_prop(HARDWARE_SMARTPA, "unknown");
	}
	pr_info("%s smartpa_type is %d\n",__func__, smartpa_type);
	//-P86801AA1, daisiqing.wt, ADD, 2022/04/28, add smartpa info for mmitest
