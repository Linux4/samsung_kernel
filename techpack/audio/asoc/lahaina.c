...
static void update_amp_dai_link(struct platform_device *pdev)
{
	...
	for (i = SMA_CODEC; i < MAX_CODEC; i++) {
		if (combination_value & mask) {
			memcpy((msm_tdm_be_dai_links[interface_rx_index].codecs)+index,
				&hybrid_dai_link_list[i], sizeof(hybrid_dai_link_list[i]));
			if (!(i == TI_RCV_CODEC))
				memcpy((msm_tdm_be_dai_links[interface_tx_index].codecs)+index,
					&hybrid_dai_link_list[i], sizeof(hybrid_dai_link_list[i]));
	...
	if (index > 0 && index < MAX_AMP_COUNT) {
		msm_tdm_be_dai_links[interface_rx_index].num_codecs = index;
		msm_tdm_be_dai_links[interface_tx_index].num_codecs = index;
	} else {
		msm_tdm_be_dai_links[interface_rx_index].num_codecs = MAX_AMP_COUNT;
		msm_tdm_be_dai_links[interface_tx_index].num_codecs = MAX_AMP_COUNT;
	}
}
