...
static u32 _sde_rm_get_reserved_dsc_ids(struct sde_rm *rm,
		struct sde_rm_rsvp *rsvp_nxt,
		u8 *rsvp_dsc_ids,
		u32 rsvp_dsc_size)
{
		...
		u8 *_dsc_ids)
{
	...
	if (!_dsc_ids) {
		dsc_cnt = _sde_rm_get_reserved_dsc_ids(rm, rsvp, rsvp_dsc_ids,
						ARRAY_SIZE(rsvp_dsc_ids));
		if (dsc_cnt == num_dsc_enc)
