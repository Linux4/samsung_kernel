...
#if defined(CONFIG_DISPLAY_SAMSUNG)
static bool input_handler_registered; //Will be initialized to 0
#endif
...
#if defined(CONFIG_DISPLAY_SAMSUNG)
	//input_handler_registered = false; //To delete
		...
		void (*vbl_cb)(void *), void *vbl_data)
{
	...
	if (sde_encoder_in_clone_mode(drm_enc)) {
		...
		SDE_EVT32(DRMID(drm_enc), enable, SDE_EVTLOG_ERROR);
		return;
	}
		...
		struct sde_encoder_kickoff_params *params)
{
	 ...
	 */
	if (sde_encoder_is_dsc_enabled(sde_enc) && sde_enc->cur_master)
#else
	if (sde_enc->cur_master &&
