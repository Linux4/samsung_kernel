...
volatile int old_refresh_rate = 90;
extern int dsi_panel_fps90_cmd_set(struct dsi_panel *panel);
/*-P86801AA1, wangcong.wt, ADD, 2023/08/11, add register code change while fps change*/
...
/*+P86801AA1, wangcong.wt, ADD, 2023/08/11, add register code change while fps change*/
	if (display->panel->ft8203_fps_code_flag) {
		if (panel_initialized_flag) {
			if (sde_enc->mode_info.frame_rate == 90 && old_refresh_rate != sde_enc->mode_info.frame_rate) {
				rc = dsi_panel_fps90_cmd_set(display->panel);
				if (rc) {
					pr_err("fps90 [%s] failed to set cmd\n", display->name);
				} else {
					pr_info("fps90 [%s] success to set cmd fps old_fps=%d\n", display->name, old_refresh_rate);
