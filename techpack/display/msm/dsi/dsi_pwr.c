	...
	if (enable) {
		for (i = 0; i < regs->count; i++) {
			...
			if (panel && vdd && display) {
				...
				/* aot_tsp_reset_regulator */
				if ((vdd->aot_tsp_reset_regulator)
					&& !display->is_cont_splash_enabled
					&& (!strcmp(vreg->vreg_name, "tsp_rst"))) {
					DSI_INFO("aot_tsp_reset_regulator->dsi_tsp_reset_regulator\n");
					continue;
				}
			} else
		...
		for (i = (regs->count - 1); i >= 0; i--) {
			...
			if (panel && vdd && display) {
				...
				if ((vdd->aot_tsp_reset_regulator)
					&& !display->is_cont_splash_enabled
					&& (!strcmp(vreg->vreg_name, "tsp_rst"))) {
					DSI_INFO("aot_tsp_reset_regulator skip off here\n");
					continue;
				}				
