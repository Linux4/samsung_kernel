...
/* Need this for PBA? */
			...
			DSI_ERR("No reset and no disp_en\n");			
			rc = 0;
#endif			
			goto error;
		}
		...
		DSI_ERR("reset_gpio is not valid when TDDI\n");
		rc = 0;
		goto error;
	}
#endif
...
static int dsi_panel_reset(struct dsi_panel *panel)
{
	...
	if (vdd->aot_reset_regulator || vdd->aot_reset_regulator_late) {
		DSI_INFO("Not here, if reset_regulator enabled\n");
		goto exit;
	}
#endif
...
/* Touch reset regulater control when aot_reset_regulator enabled TDDI.
 * regulator name should be "tsp_rst"
 */
static int dsi_tsp_reset_regulator(struct dsi_panel *panel, bool enable)
{
	int rc = 0;
	int i;
	struct dsi_vreg *vreg;
	...
	/* Find number of "panel-reset" supply-name order among qcom,panel-supply-entries */
	for (i = 0; i < panel->power_info.count; i++) {
		if (!strcmp((panel->power_info.vregs + i)->vreg_name, "tsp_rst"))
			break;
	}

	if (i == panel->power_info.count) {
		DSI_ERR("Could not find tsp reset regulator name, i:%d\n", i);
		goto exit;
	} else {
...
#endif
{
	...
	/* 	AOT disable on factory binary  (for Tabs7 only) */
...
#endif
{
	...
	/* Turn off tsp_rst before reset off
	 * if aot_tsp_reset_regulator enabled */
	if (vdd->aot_tsp_reset_regulator) {
		rc = dsi_tsp_reset_regulator(panel, false);
		if (rc) {
			DSI_ERR("[%s] failed to off tsp_rst regulator, rc=%d\n",
				panel->name, rc);
		}
	}
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	if (!gpio_is_valid(panel->reset_config.lcd_mode_sel_gpio))
		DSI_ERR("mode gpio not specified\n");
...
int dsi_panel_update_pps(struct dsi_panel *panel)
{
	...
	if (vdd->no_qcom_pps == true) {
		for (rc = 0; rc  < 9/*(set->cmds->msg->tx_len)*/; rc++) {
			DSI_DEBUG("Qcom PPS [%2x %2x %2x %2x %2x %2x %2x %2x %2x %2x]\n",
...
int wait_tcon_ready(struct dsi_panel *panel)
{
	...
	for (i = 0; i < max_wait_cnt; i++) {
		if (gpio_get_value(vdd->dtsi_data.samsung_tcon_rdy_gpio)) {
			LCD_INFO(vdd, "tcon_rdy becomes level high!!!\n");
			break;
		}
	...
	if (vdd->dtsi_data.samsung_delay_after_tcon_rdy) {
		...
		if (vdd->dtsi_data.samsung_delay_after_tcon_rdy >= 100)
			...
			usleep_range(1000*vdd->dtsi_data.samsung_delay_after_tcon_rdy,
					1000*vdd->dtsi_data.samsung_delay_after_tcon_rdy);
	}
...
void tcon_prepare(void)
{
	...
	LCD_INFO(vdd, "++\n");
	/*	There is panel power on request. So panel reset executes here.
	 * panel power on -> LP11 -> RESET -> wait tcon ready
	 */
...
int dsi_panel_prepare(struct dsi_panel *panel)
{
	...
	if (vdd->aot_reset_regulator_late) {
		...
		/* TSP reset on if aot_tsp_reset_regulator enabled
		 * with aot_reset_regulator_late
		 */
		if (vdd->aot_tsp_reset_regulator)
			dsi_tsp_reset_regulator(panel, true);
	}
#endif
...
int dsi_panel_enable(struct dsi_panel *panel)
{
	...
	/* 3FA7 IC : display off, sleep in cmds should be sent before sleep out in case2 */
	if (vdd->poc_driver.read_case == READ_CASE2 && vdd->poc_driver.need_sleep_in) {
		LCD_INFO(vdd, "display off, sleep in cmds should be sent before sleep out in case2..\n");
