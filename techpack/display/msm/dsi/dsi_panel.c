...
/*F52 Lite code for SR-QN3979-01-144 by fengzhigang at 20210202 start*/
enum tp_module_used tp_is_used = UNKNOWN_TP;
#if defined(CONFIG_TOUCHSCREEN_ILI)
extern void ilitek_resume_by_ddi(void);
#endif
/*F52 Lite code for SR-QN3979-01-144 by fengzhigang at 20210202 end*/
...
/*F52 Lite code for SR-QN3979-01-144 by fengzhigang at 20210202 start*/
/*Compatibility needs to be done after other subsequent screens come*/
#if defined(CONFIG_TOUCHSCREEN_ILI)
	if(tp_is_used == ILITEK_ILI7807S) {
		DSI_INFO("tp_7807S TP_RESUME\n");
...
static int dsi_panel_parse_bl_config(struct dsi_panel *panel)
{
	...
	if (!data) {
		...
		/*F52 code for SR-QN3979-01-140 by yuanliding at 20210218 start*/
		rc = utils->read_u32(utils->data, "qcom,delay_first_frame_to_backlight_on", &val);
		if (rc) {
			panel->bl_config.first_frame_to_backlight_on = 0;
			DSI_DEBUG("[%s] delay_first_frame_to_backlight_on = 0\n",panel->name);
		}
		else
		{
			panel->bl_config.first_frame_to_backlight_on = val;
		}
		/*F52 code for SR-QN3979-01-140 by yuanliding at 20210218 end*/
	} else {
