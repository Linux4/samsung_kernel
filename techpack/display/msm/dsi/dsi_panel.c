...
#include <linux/hardware_info.h>//wangcong.wt,add,2023.8.28, add lcm_get_serialnum
#endif
/**
...
bool fts_gestrue_status;
EXPORT_SYMBOL(fts_gestrue_status);
bool himax_gestrue_status;
...
//bug761884, gudi@wt, add 20230616, GKI charger bring up
extern int set_jeita_lcd_on_off(bool lcdon);
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	{
		mdelay(3);
		rc= ocp2138_BiasPower_enable(LCM_LDO_VOL_5V6, LCM_LDO_VOL_5V6, 5);
		mdelay(3);
	}else if(!strcmp("focaltech_ft8203_wt_dsi_vdo_90hz_apf_boe",panel->name))
	{
		mdelay(3);
		rc= ocp2138_BiasPower_enable(LCM_LDO_VOL_5V6, LCM_LDO_VOL_5V6, 5);
		mdelay(3);
	}else if(!strcmp("lide_hx83102j_wt_dsi_vdo_90hz_hsd",panel->name))
	{
		mdelay(1);
	...
	{
		mdelay(3);
		rc= ocp2138_BiasPower_enable(LCM_LDO_VOL_6V0, LCM_LDO_VOL_6V0, 5);
		mdelay(3);
	}else if(!strcmp("chipone_icnl9951r_wt_dsi_vdo_90hz_boe",panel->name))
	{
		mdelay(1);
		rc= ocp2138_BiasPower_enable(LCM_LDO_VOL_6V0, LCM_LDO_VOL_6V0, 3);
		mdelay(10);
	}else if(!strcmp("txd_icnl9951r_wt_dsi_vdo_90hz_hkc",panel->name))
	{
		mdelay(1);
		rc= ocp2138_BiasPower_enable(LCM_LDO_VOL_6V0, LCM_LDO_VOL_6V0, 3);
		mdelay(10);
	}else if(!strcmp("hjr_icnl9951r_wt_dsi_vdo_90hz_hkc",panel->name))
	{
		mdelay(1);
		rc= ocp2138_BiasPower_enable(LCM_LDO_VOL_6V0, LCM_LDO_VOL_6V0, 3);
		mdelay(10);
	...
	if(esd_interval_time) {
		mdelay(15);
		pr_info("esd_interval_time mdelay(15)\n");
	}
...
// +P86801AA1 peiyuexiang.wt,add,20230511,add double_click
	...
	if (himax_gestrue_status || fts_gestrue_status || chipone_gestrue_status) {
		...
		if(rc)
			DSI_ERR("panel external bias ocp2138 power off\n");

		panel->power_info.refcount = 1;
	...
	u32 bl_lvl)
{
	...
	//+P86801AA1,liuyongliang.wt,add,2023.06.21, limit min value
	if(bl_lvl <= 0x20 && bl_lvl != 0)
		bl_lvl = 0x20;
	//-P86801AA1,liuyongliang.wt,add,2023.06.21, limit min value
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-off-no-deep-sleep-command",
	/*+P86801AA1, wangcong.wt, ADD, 2023/08/11, add register code change while fps change*/
	"qcom,mdss-dsi-fps60-command",
	"qcom,mdss-dsi-fps90-command",
	/*-P86801AA1, wangcong.wt, ADD, 2023/08/11, add register code change while fps change*/
};
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-off-no-deep-sleep-command-state",
	/*+P86801AA1, wangcong.wt, ADD, 2023/08/11, add register code change while fps change*/
	"qcom,mdss-dsi-fps60-command-state",
	"qcom,mdss-dsi-fps90-command-state",
	/*-P86801AA1, wangcong.wt, ADD, 2023/08/11, add register code change while fps change*/
};
...
static int dsi_panel_parse_misc_features(struct dsi_panel *panel)
{
	...
	/*+P86801AA1, wangcong.wt, ADD, 2023/08/11, add register code change while fps change*/
	panel->ft8203_fps_code_flag = utils->read_bool(utils->data,
			"qcom,mdss-dsi-ft8203-fps-code-flag");
	/*-P86801AA1, wangcong.wt, ADD, 2023/08/11, add register code change while fps change*/
	/*+P86801AA1, wangcong.wt, ADD, 2023/08/28, add lcm ic flag*/
	panel->hx83102_flag = utils->read_bool(utils->data,
			"qcom,mdss-dsi-hx83102-flag");
	panel->icnl9951r_flag = utils->read_bool(utils->data,
...
int dsi_panel_fps90_cmd_set(struct dsi_panel *panel)
{
       int rc = 0;
       if (!panel) {
               DSI_ERR("Invalid params\n");
       ...
       mutex_lock(&panel->panel_lock);
       rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_FPS90);
       if (rc)
           DSI_ERR("fps [%s] failed to send DSI_CMD_SET_FPS90 cmds, rc=%d\n",
                   panel->name, rc);
       mutex_unlock(&panel->panel_lock);
       return rc;
}
	...
	if (panel->ft8203_fps_code_flag) {
		...
		if (panel->cur_mode->timing.refresh_rate == 60) {
			...
			if (rc) {
				...
				pr_info("fps60 success to set cmd \n");
			}
		}
	} else {
...
int dsi_panel_disable(struct dsi_panel *panel)
{
	...
	if (!atomic_read(&panel->esd_recovery_pending)) {
		...
		if(himax_gestrue_status)
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_OFF_NO_DEEP_SLEEP);
		else
				...
				char *buf, size_t count)
{
	...
	pr_err("lcm_get_serialnum dsi_panel_mipi_reg_write buf:%s count:%u\n", buf, count);
	mutex_lock(&panel->panel_lock);
...
ssize_t dsi_panel_mipi_reg_read(struct dsi_panel *panel, char *buf)
{
	int i = 0;
	ssize_t count = 0;
	pr_err("lcm_get_serialnum dsi_panel_mipi_reg_read buf:%s \n", buf);
	mutex_lock(&panel->panel_lock);
	if (!panel) {
		mutex_unlock(&panel->panel_lock);
		return -EAGAIN;
...
//+,wangcong.wt,add,2023.8.28, add lcm_get_serialnum
#ifdef CONFIG_QGKI_BUILD
static void ft8203_lcm_get_serialnum(struct dsi_panel *panel)
{
	char id[21]="11111111111111111111";
	int i = 0;
	char value1[] = "00 00 39 01 00 00 00 00 02 00 00";
	char value2[] = "00 00 39 01 00 00 00 00 04 FF 82 01 01";
	char value3[] = "00 00 39 01 00 00 00 00 02 00 80";
	char value4[] = "00 00 39 01 00 00 00 00 03 FF 82 01";

	char value5[] = "00 00 39 01 00 00 00 00 02 00 88";
	char value6[] = "00 00 39 01 00 00 00 00 02 B0 07";
	//char value7[] = "00 00 39 01 00 00 00 00 02 00 00";
	char value8[] = "00 00 39 01 00 00 00 00 02 FA 02";

	char value9[] = "00 00 39 01 00 00 00 00 02 00 03";
	char value10[] = "00 00 39 01 00 00 00 00 02 00 0B";
	char value11[] = "00 00 39 01 00 00 00 00 02 00 10";

	//char value12[] = "00 00 39 01 00 00 00 00 02 00 00";
	char value13[] = "00 00 39 01 00 00 00 00 02 FA 5A";
	char value14[] = "00 00 39 01 00 00 00 00 02 00 88";
	char value15[] = "00 00 39 01 00 00 00 00 02 B0 01";

	//char value16[] = "00 00 39 01 00 00 00 00 02 00 00";
	char value17[] = "00 00 39 01 00 00 00 00 04 FF 00 00 00";
	char value18[] = "00 00 39 01 00 00 00 00 02 00 80";
	char value19[] = "00 00 39 01 00 00 00 00 03 FF 00 00";

	char value20[] = "01 08 06 01 00 01 00 00 01 F4";
	char value21[] = "01 05 06 01 00 01 00 00 01 F4";
	char value22[] = "01 07 06 01 00 01 00 00 01 F4";

	dsi_panel_mipi_reg_write(panel, value1, 33);//dcs_write_seq(dsi,0x00,0x00);
	dsi_panel_mipi_reg_write(panel, value2, 39);//dcs_write_seq(dsi,0xFF,0x82,0x01,0x01);
	dsi_panel_mipi_reg_write(panel, value3, 33);//dcs_write_seq(dsi,0x00,0x80);
	dsi_panel_mipi_reg_write(panel, value4, 36);//dcs_write_seq(dsi,0xFF,0x82,0x01);
	dsi_panel_mipi_reg_write(panel, value5, 33);//dcs_write_seq(dsi,0x00,0x88);
	dsi_panel_mipi_reg_write(panel, value6, 33);//dcs_write_seq(dsi,0xB0,0x07);
	dsi_panel_mipi_reg_write(panel, value1, 33);//dcs_write_seq(dsi,0x00,0x00);
	dsi_panel_mipi_reg_write(panel, value8, 33);//dcs_write_seq(dsi,0xFA,0x02);//0x02:ReadMasterRegister 0x01:ReadSlaveRegister

	dsi_panel_mipi_reg_write(panel, value9, 33);//dcs_write_seq(dsi,0x00,0x03);//GEN_WR(0x00,0x03);
	dsi_panel_mipi_reg_write(panel, value20, 30);//char value20[] = "01 08 06 01 00 01 00 00 01 F4";

	pr_err("lcm_get_serialnum :serialnum_temp20[0]%c,%c,%c,%c,%c,%c,%c,%c\n",serialnum_temp20[0],serialnum_temp20[1],serialnum_temp20[2],serialnum_temp20[3],serialnum_temp20[4],serialnum_temp20[5],serialnum_temp20[6],serialnum_temp20[7]);
	for (i = 0; i < 8; i++) {//debug
		id[i] = serialnum_temp20[i];
	}
	pr_err("lcm_get_serialnum : id[0]%c,%c,%c,%c,%c,%c,%c,%c\n",id[0],id[1],id[2],id[3],id[4],id[5],id[6],id[7]);

	dsi_panel_mipi_reg_write(panel, value10, 33);//char value10[] = "00 00 39 01 00 00 00 00 02 00 0B";
	dsi_panel_mipi_reg_write(panel, value21, 30);//char value21[] = "01 05 06 01 00 01 00 00 01 F4";
	for (i = 0; i < 5; i++) {//debug
		id[i+8] = serialnum_temp20[i];
	}
	pr_err("lcm_get_serialnum :id[8]%c,%c,%c,%c,%c,%c,%c,%c\n",id[8],id[9],id[10],id[11],id[12]);

	dsi_panel_mipi_reg_write(panel, value11, 33);//char value11[] = "00 00 39 01 00 00 00 00 02 00 10"; dcs_write_seq(dsi,0x00,0x10);//GEN_WR(0x00,0x10);
	dsi_panel_mipi_reg_write(panel, value22, 30);//char value22[] = "01 07 06 01 00 01 00 00 01 F4";
	for (i = 0; i < 7; i++) {//debug
		id[i+13] = serialnum_temp20[i];
	}
	pr_err("lcm_get_serialnum :id[13]%c,%c,%c,%c,%c,%c,%c\n",id[13],id[14],id[15],id[16],id[17],id[18],id[19]);

	dsi_panel_mipi_reg_write(panel, value1, 33);//dcs_write_seq(dsi,0x00,0x00);
	dsi_panel_mipi_reg_write(panel, value13, 33);//dcs_write_seq(dsi,0xFA,0x5A);
	dsi_panel_mipi_reg_write(panel, value14, 33);//dcs_write_seq(dsi,0x00,0x88);
	dsi_panel_mipi_reg_write(panel, value15, 33);//dcs_write_seq(dsi,0xB0,0x01);
	dsi_panel_mipi_reg_write(panel, value1, 33);//dcs_write_seq(dsi,0x00,0x00);
	dsi_panel_mipi_reg_write(panel, value17, 39);//dcs_write_seq(dsi,0xFF,0x00,0x00,0x00);
	dsi_panel_mipi_reg_write(panel, value18, 33);//dcs_write_seq(dsi,0x00,0x80);
	dsi_panel_mipi_reg_write(panel, value19, 36);//dcs_write_seq(dsi,0xFF,0x00,0x00);

	pr_err("lcm_get_serialnum id:%s\n", id);
	for (i = 0; i < 20; i++) {//debug
		lcm_serialnum[i] = id[i];
	}
	pr_err("ft8203 lcm_serialnum : lcm_serialnum%s \n", lcm_serialnum);
}

static void icnl9951_lcm_get_serialnum(struct dsi_panel *panel)
{
	int i = 0;
	char value1[] = "00 00 39 01 00 00 00 00 03 F0 5A 59";//36
	char value2[] = "00 00 39 01 00 00 00 00 03 F1 A5 A6";
	char value3[] = "01 20 06 01 00 01 00 00 01 91";      //30
	char value4[] = "00 00 39 01 00 00 00 00 03 F1 5A 59";
	char value5[] = "00 00 39 01 00 00 00 00 03 F0 A5 A6";

	dsi_panel_mipi_reg_write(panel, value1, 36);
	dsi_panel_mipi_reg_write(panel, value2, 36);
	pr_err("lcm_serialnum:%s\n", lcm_serialnum);
	dsi_panel_mipi_reg_write(panel, value3, 30);
	for (i = 0; i < 20; i++) {//debug
		lcm_serialnum[i] = serialnum_temp20[i];
	}
	dsi_panel_mipi_reg_write(panel, value4, 36);
	dsi_panel_mipi_reg_write(panel, value5, 36);
	pr_err("icnl9911 lcm_serialnum:%s\n", lcm_serialnum);
}

static void hx83102_lcm_get_serialnum(struct dsi_panel *panel)
{

	int i = 0;
	char value1[] = "00 00 39 01 00 00 00 00 06 B9 83 10 21 55 00";//45
	char value2[] = "00 00 39 01 00 00 00 00 02 BD 00";//33
	char value3[] = "00 00 39 01 00 00 00 00 04 BB 02 0F 18";//39
	char value4[] = "00 00 39 01 00 00 00 00 02 BB 0A";//33
	char value5[] = "00 00 39 01 00 00 00 00 02 BD 03";//33
	char value6[] = "01 20 06 01 00 01 00 00 01 B1";//30
	char value7[] = "00 00 39 01 00 00 00 00 02 BD 00";//33

	char value8[] = "00 00 39 01 00 00 00 00 04 BB 02 0F 00";//33

	dsi_panel_mipi_reg_write(panel, value1, 45);
	dsi_panel_mipi_reg_write(panel, value2, 33);

	dsi_panel_mipi_reg_write(panel, value3, 39);
	dsi_panel_mipi_reg_write(panel, value4, 33);
	usleep_range(50000, 51000);
	dsi_panel_mipi_reg_write(panel, value5, 33);
	dsi_panel_mipi_reg_write(panel, value6, 30);
	dsi_panel_mipi_reg_write(panel, value7, 33);
	pr_err("lcm_get_serialnum serialnum_temp20:%s\n", serialnum_temp20);
	if (serialnum_temp20[0] == 0xFF) {

		dsi_panel_mipi_reg_write(panel, value8, 39);
		dsi_panel_mipi_reg_write(panel, value4, 33);
		usleep_range(50000, 51000);
		dsi_panel_mipi_reg_write(panel, value5, 33);
		dsi_panel_mipi_reg_write(panel, value6, 30);
	...
	pr_err("hx83102 lcm_serialnum:%s\n", lcm_serialnum);
}

static void ft8203_dsbj_lcm_get_serialnum(struct dsi_panel *panel)
{
	char id[21]="11111111111111111111";
	int i = 0;
	char value1[] = "00 00 39 01 00 00 00 00 02 00 00";
	...
	char value9[] = "00 00 39 01 00 00 00 00 02 00 00";
	char value10[] = "00 00 39 01 00 00 00 00 02 00 08";
	char value11[] = "00 00 39 01 00 00 00 00 02 00 10";
	...
	char value20[] = "01 08 06 01 00 01 00 00 01 F4";
	char value21[] = "01 08 06 01 00 01 00 00 01 F4";
	char value22[] = "01 04 06 01 00 01 00 00 01 F4";
	...
	dsi_panel_mipi_reg_write(panel, value6, 33);//dcs_write_seq(dsi,0xB0,0x07);
	dsi_panel_mipi_reg_write(panel, value1, 33);//dcs_write_seq(dsi,0x00,0x00);
	dsi_panel_mipi_reg_write(panel, value8, 33);//dcs_write_seq(dsi,0xFA,0x02);

	dsi_panel_mipi_reg_write(panel, value9, 33);//dcs_write_seq(dsi,0x00,0x00);
	dsi_panel_mipi_reg_write(panel, value20, 30);//char value20[] = "01 08 06 01 00 01 00 00 01 F4";
	...
	dsi_panel_mipi_reg_write(panel, value10, 33);//char value10[] = "00 00 39 01 00 00 00 00 02 00 08";
	dsi_panel_mipi_reg_write(panel, value21, 30);//char value21[] = "01 08 06 01 00 01 00 00 01 F4";
	for (i = 0; i < 8; i++) {
		id[i+8] = serialnum_temp20[i];
	}
	pr_err("lcm_get_serialnum :id[8]%c,%c,%c,%c,%c,%c,%c,%c\n",id[8],id[9],id[10],id[11],id[12],id[13],id[14],id[15]);

	dsi_panel_mipi_reg_write(panel, value11, 33);//char value11[] = "00 00 39 01 00 00 00 00 02 00 10"
	dsi_panel_mipi_reg_write(panel, value22, 30);//char value22[] = "01 04 06 01 00 01 00 00 01 F4";
	for (i = 0; i < 4; i++) {
		id[i+16] = serialnum_temp20[i];
	}
	pr_err("lcm_get_serialnum :id[13]%c,%c,%c,%c\n",id[16],id[17],id[18],id[19]);
...
void lcm_serialnum_get(void)
{
	...
	pr_err("lcm_get_serialnum display->panel->name:%s\n",display->panel->name);
	mutex_lock(&display->display_lock);
	pr_err("lcm_get_serialnum in\n");
	if (display->panel->ft8203_fps_code_flag) {
		ft8203_lcm_get_serialnum(display->panel);
	}
	if (display->panel->hx83102_flag) {
		hx83102_lcm_get_serialnum(display->panel);
	}
	if (display->panel->icnl9951r_flag) {
		icnl9951_lcm_get_serialnum(display->panel);
	}
...
EXPORT_SYMBOL(lcm_serialnum_get);
#endif
