...
{
	struct dp_display_private *dp = g_secdp_priv;
	bool skip = dp->sec.debug.adapter_check_skip;
...
void secdp_debug_adapter_check_store(bool skip)
{
	...
	dp->sec.debug.adapter_check_skip = skip;
	DP_DEBUG("skip: %d\n", dp->sec.debug.adapter_check_skip);
}
		...
		void *data)
{
	...
	DP_DEBUG("link_conf:%d, hpd:%d\n", sec->link_conf,
		atomic_read(&sec->hpd.val));
	if ((sec->link_conf && atomic_read(&sec->hpd.val)) ||/*hpd high or hpd_irq*/
