...
static struct dsi_panel_cmd_set *ss_hbm_gamma(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*level_key = LEVEL1_KEY;
	br_interpolation_generate_event(vdd, GEN_HBM_INTERPOLATION_GAMMA, &hbm_gamma_cmds->cmds[0].ss_txbuf[1]);
...
static struct dsi_panel_cmd_set *ss_hbm_etc(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	br_interpolation_generate_event(vdd, GEN_HBM_INTERPOLATION_AOR, &hbm_etc_cmds->cmds[0].ss_txbuf[1]);

	/* 	0xB5 1th TSET
		BIT(7) is signed bit.
		BIT(6) ~ BIT(0) is real temperature.
	*/
	hbm_etc_cmds->cmds[1].ss_txbuf[1] = vdd->br_info.temperature > 0 ?
			vdd->br_info.temperature : (char)(BIT(7) | (-1*vdd->br_info.temperature));

	/* Elvss, MPS_CON : all HBM is 0xDC */
	hbm_etc_cmds->cmds[1].ss_txbuf[2] = 0xDC;

	br_interpolation_generate_event(vdd, GEN_HBM_INTERPOLATION_ELVSS, &hbm_etc_cmds->cmds[1].ss_txbuf[3]);

	/* read OTP B5h 58th -> write B5h 57h(Gram) */
	hbm_etc_cmds->cmds[1].ss_txbuf[57] = vdd->br_info.common_br.elvss_value[1];

	/* VINT */
	br_interpolation_generate_event(vdd, GEN_HBM_INTERPOLATION_VINT, &hbm_etc_cmds->cmds[2].ss_txbuf[2]);
	...
	if (vdd->br_info.gradual_acl_val) {
		hbm_etc_cmds->cmds[3].ss_txbuf[6] = 0x26;	/* 8% */
		hbm_etc_cmds->cmds[4].ss_txbuf[1] = 0x02;	/* ACL ON */
		hbm_etc_cmds->cmds[3].ss_txbuf[2] = 0x55;
	} else {
		hbm_etc_cmds->cmds[3].ss_txbuf[6] = 0x0;	/* 0% */
		hbm_etc_cmds->cmds[4].ss_txbuf[1] = 0x0;	/* ACL OFF */
		hbm_etc_cmds->cmds[3].ss_txbuf[2] = 0x44;
	}

	*level_key = LEVEL1_KEY;

	LCD_INFO(vdd, "elvss 3th: 0x%x 0x%x, vint 0x%x, acl (%d %x, per=%x, frame avg=%x)\n",
			hbm_etc_cmds->cmds[1].ss_txbuf[3],
			vdd->br_info.common_br.elvss_value[1],
			hbm_etc_cmds->cmds[2].ss_txbuf[2],
			vdd->br_info.gradual_acl_val,
			hbm_etc_cmds->cmds[4].ss_txbuf[1],
			hbm_etc_cmds->cmds[3].ss_txbuf[6],
			hbm_etc_cmds->cmds[3].ss_txbuf[2]);
...
static int ss_irc_read(struct samsung_display_driver_data *vdd)
{
	...
	if (!vdd->display_status_dsi.irc_otp) {
		...
		if (!vdd->display_status_dsi.irc_otp) {
			LCD_ERR(vdd, "fail to allocate irc_otp memory\n");
			return false;
		}
	}
	...
	memcpy(&set->cmds->ss_txbuf[2], vdd->display_status_dsi.irc_otp, NUM_IRC_OTP);
...
static int ss_hbm_read(struct samsung_display_driver_data *vdd)
{
	...
	memcpy(&hbm_gamma_cmds->cmds->ss_txbuf[1], hbm_buffer, 34);
		...
		struct brightness_table *br_tbl)
{
	...
	if (SS_IS_CMDS_NULL(pcmds)) {
		LCD_ERR(vdd, "No cmds for TX_HBM_GAMMA.. \n");
		return -EINVAL;
	}
	sconf->hbm_payload = &pcmds->cmds->ss_txbuf[1];
...
static struct dsi_panel_cmd_set *ss_aid(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	LCD_DEBUG(vdd, "[%d] level(%d), aid(%x %x)\n",
...
static struct dsi_panel_cmd_set *ss_acl_on(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*level_key = LEVEL1_KEY;
	...
	if (vdd->br_info.common_br.bl_level > 25500) { /* HBM */
		pcmds->cmds[0].ss_txbuf[3] = 0x80;	/* use hbm acl statr step 60% */
		pcmds->cmds[0].ss_txbuf[4] = 0x65;	/* use hbm acl statr step 60% */
		pcmds->cmds[0].ss_txbuf[6] = 0x26;	/* 8% */
	} else { /* Normal */
		pcmds->cmds[0].ss_txbuf[3] = 0x40;	/* use hbm acl statr step 60% */
		pcmds->cmds[0].ss_txbuf[4] = 0xFF;	/* use hbm acl statr step 60% */
		pcmds->cmds[0].ss_txbuf[6] = 0x50;	/* 15% */
	}
...
static struct dsi_panel_cmd_set *ss_elvss(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*/
	...
	elvss_cmds->cmds[0].ss_txbuf[57] = vdd->br_info.common_br.elvss_value[0];
...
static struct dsi_panel_cmd_set *ss_vint(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	br_interpolation_generate_event(vdd, GEN_NORMAL_INTERPOLATION_VINT, &vint_cmds->cmds->ss_txbuf[2]);

	if (vdd->xtalk_mode)
		vint_cmds->cmds->ss_txbuf[1] = 0x6B;	// VGH 6.2 V
	else
		vint_cmds->cmds->ss_txbuf[1] = 0xEB;	// VGH 7.0 V
...
static struct dsi_panel_cmd_set *ss_irc(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	br_interpolation_generate_event(vdd, GEN_NORMAL_INTERPOLATION_IRC, &irc_cmds->cmds->ss_txbuf[1]);

	/* set irc mode to moderato or flat gamma */
	if (vdd->br_info.common_br.irc_mode == IRC_MODERATO_MODE)
		irc_cmds->cmds->ss_txbuf[2] = IRC_MODERATO_MODE_VAL;
	else if (vdd->br_info.common_br.irc_mode == IRC_FLAT_GAMMA_MODE)
		irc_cmds->cmds->ss_txbuf[2] = IRC_FLAT_GAMMA_MODE_VAL;
	else
...
static struct dsi_panel_cmd_set *ss_hbm_irc(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	br_interpolation_generate_event(vdd, GEN_HBM_INTERPOLATION_IRC, &hbm_irc_cmds->cmds->ss_txbuf[1]);

	/* set irc mode to moderato or flat gamma */
	if (vdd->br_info.common_br.irc_mode == IRC_MODERATO_MODE)
		hbm_irc_cmds->cmds->ss_txbuf[2] = IRC_MODERATO_MODE_VAL;
	else if (vdd->br_info.common_br.irc_mode == IRC_FLAT_GAMMA_MODE)
		hbm_irc_cmds->cmds->ss_txbuf[2] = IRC_FLAT_GAMMA_MODE_VAL;
	else
		LCD_ERR(vdd, "invalid irc mode(%d)\n", vdd->br_info.common_br.irc_mode);
...
void S6E3HA9_AMF670UH01_FHD_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.convert_GAMMA_to_V = convert_GAMMA_to_V_S6E3HA9_AMF670UH01;
	vdd->panel_func.convert_V_to_GAMMA = convert_V_to_GAMMA_S6E3HA9_AMF670UH01;
