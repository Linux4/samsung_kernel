 ...
 *
*/
#include "XCP2_NT36672C_PM6585JB2_panel.h"
#include "XCP2_NT36672C_PM6585JB2_mdnie.h"
//#include "ss_dsi_reading_mode_XCP2_NT36672C_PM6585JB2.h"
...
static int samsung_panel_power_off_post(struct samsung_display_driver_data *vdd)
{
	struct dsi_vreg *dsi_vreg = NULL;
	...
	dsi_vreg = GET_DSI_VREG_NAME(vdd, "lcd_vsp");
	if (!IS_ERR_OR_NULL(dsi_vreg)) {
		if (regulator_is_enabled(dsi_vreg->vreg))
			LCD_INFO(vdd, "AOT enabled. Skip blic_ctrl\n");
		else
			ss_blic_ctrl(vdd, false); /* turn off blic */
	} else
		LCD_ERR(vdd, "dsi_vreg is NULL\n");
