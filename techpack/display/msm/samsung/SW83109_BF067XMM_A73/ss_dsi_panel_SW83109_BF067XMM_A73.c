...
*/
#include "ss_dsi_panel_SW83109_BF067XMM_A73.h"
#include "ss_dsi_mdnie_SW83109_BF067XMM_A73.h"
					...
					int *level_key, bool is_hbm, bool is_hmt)
{
	...
	if (cur_rr == 60) {
		vrr_cmds->cmds[1].ss_txbuf[6] = 0xAD; /* 60 HZ */
		vrr_cmds->cmds[3].ss_txbuf[1] = 0x01; /* 60 HZ */
	} else {
		vrr_cmds->cmds[1].ss_txbuf[6] = 0x8B; /* 120 HZ */
		vrr_cmds->cmds[3].ss_txbuf[1] = 0x03; /* 120 HZ */
	}
							...
							(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (vdd->vrr.cur_refresh_rate == 60) {
		pcmds->cmds[1].ss_txbuf[29] = 0x80;
		pcmds->cmds[1].post_wait_ms = 10;
	} else {
		pcmds->cmds[1].ss_txbuf[29] = 0x00;
		pcmds->cmds[1].post_wait_ms = 0;
	}

	if (vdd->finger_mask_updated) {
		LCD_INFO(vdd, "NORMAL - FINGER UPDTAED\n");
		if (vdd->vrr.cur_refresh_rate == 60)
			pcmds_smooth_off->cmds[0].post_wait_ms = FRAME_WAIT_60FPS;
		else
			pcmds_smooth_off->cmds[0].post_wait_ms = FRAME_WAIT_120FPS;

		ss_send_cmd(vdd, TX_SMOOTH_DIMMING_OFF);

		pcmds->cmds[1].post_wait_ms = 0;
		if (vdd->vrr.cur_refresh_rate == 60) {
			pcmds->cmds[2].post_wait_ms = HBM_NORMAL_DELAY_60FPS;
		} else {
			pcmds->cmds[2].post_wait_ms = HBM_NORMAL_DELAY_120FPS;
		}

		pcmds->cmds[2].last_command = 1;
		pcmds->cmds[3].ss_txbuf[1] = 0x00;
	} else {
		LCD_INFO(vdd, "NORMAL - NO FINGER\n");
		pcmds->cmds[2].post_wait_ms = 0;
		pcmds->cmds[2].last_command = 0;
		pcmds->cmds[3].ss_txbuf[1] = 0x20;
		pcmds->cmds[4].post_wait_ms = 0;
	}

	pcmds->cmds[4].ss_txbuf[1] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 8, 8);
	pcmds->cmds[4].ss_txbuf[2] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 0, 8);

	if (vdd->br_info.temperature > 1)
		pcmds->cmds[5].ss_txbuf[1] = 0x19;
	else if (vdd->br_info.temperature == 0)
		pcmds->cmds[5].ss_txbuf[1] = 0x00;
	else if (vdd->br_info.temperature > -15)
		pcmds->cmds[5].ss_txbuf[1] = 0x8E;
	else
		pcmds->cmds[5].ss_txbuf[1] = 0x94;
							...
							(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (vdd->finger_mask_updated) {
		...
		if (vdd->vrr.cur_refresh_rate == 60)
			pcmds->cmds[0].post_wait_ms = HBM_NORMAL_DELAY_60FPS;
		else
			pcmds->cmds[0].post_wait_ms = HBM_NORMAL_DELAY_120FPS;
		pcmds->cmds[0].last_command = 1;
		pcmds->cmds[1].ss_txbuf[1] = 0x00;
	} else {
		LCD_INFO(vdd, "HBM - NO FINGER\n");

		pcmds->cmds[0].post_wait_ms = 0;
		pcmds->cmds[0].last_command = 0;
		pcmds->cmds[1].ss_txbuf[1] = 0x20;
	}
	...
	if (vdd->br_info.temperature > 1)
		pcmds->cmds[3].ss_txbuf[1] = 0x19;
	else if (vdd->br_info.temperature == 0)
		pcmds->cmds[3].ss_txbuf[1] = 0x00;
	else if (vdd->br_info.temperature > -15)
		pcmds->cmds[3].ss_txbuf[1] = 0x8E;
	else
		pcmds->cmds[3].ss_txbuf[1] = 0x94;
...
static int ss_module_info_read(struct samsung_display_driver_data *vdd)
{
	...
	if (ss_get_cmds(vdd, RX_MODULE_INFO)->count) {
		...
		/* CELL ID (A9h 1~11th) */
		/* Manufacture Date */
		vdd->cell_id_dsi[0] = buf[0];
		vdd->cell_id_dsi[1] = buf[1];
		vdd->cell_id_dsi[2] = buf[2];
		vdd->cell_id_dsi[3] = buf[3];
		vdd->cell_id_dsi[4] = buf[4];
		vdd->cell_id_dsi[5] = buf[5];
		vdd->cell_id_dsi[6] = buf[6];
		vdd->cell_id_dsi[7] = buf[7];
		vdd->cell_id_dsi[8] = buf[8];
		vdd->cell_id_dsi[9] = buf[9];
		vdd->cell_id_dsi[10] = buf[10];
...
static int ss_ddi_id_read(struct samsung_display_driver_data *vdd)
{
	...
	/* Read mtp (EAh 1~10th) for ddi id */
	if (ss_get_cmds(vdd, RX_DDI_ID)->count) {
		ss_panel_data_read(vdd, RX_DDI_ID, ddi_id, LEVEL2_KEY);

		for (loop = 0; loop < 10; loop++)
			vdd->ddi_id_dsi[loop] = ddi_id[loop];

		LCD_INFO(vdd, "DSI%d : %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n", vdd->ndx,
			vdd->ddi_id_dsi[0], vdd->ddi_id_dsi[1],
			vdd->ddi_id_dsi[2], vdd->ddi_id_dsi[3],
			vdd->ddi_id_dsi[4], vdd->ddi_id_dsi[5],
			vdd->ddi_id_dsi[6], vdd->ddi_id_dsi[7],
			vdd->ddi_id_dsi[8], vdd->ddi_id_dsi[9]);
	} else {
...
static struct dsi_panel_cmd_set *ss_acl_on_hbm(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	/* ACL 8% in HBM */
	pcmds->cmds[3].ss_txbuf[7]  = 0x69;
	pcmds->cmds[3].ss_txbuf[8]  = 0xE4;
	pcmds->cmds[3].ss_txbuf[9]  = 0x60;
	pcmds->cmds[3].ss_txbuf[10] = 0xDC;
...
start_lpm_bl:
	...
	case LPM_60NIT:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_60NIT_CMD);
		set_lpm_off->cmds[1].ss_txbuf[1] = 0x04;
		set_lpm_off->cmds[1].ss_txbuf[2] = 0xE9;
		break;
	case LPM_30NIT:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_30NIT_CMD);
		set_lpm_off->cmds[1].ss_txbuf[1] = 0x03;
		set_lpm_off->cmds[1].ss_txbuf[2] = 0xAF;
		break;
	case LPM_10NIT:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_10NIT_CMD);
		set_lpm_off->cmds[1].ss_txbuf[1] = 0x02;
		set_lpm_off->cmds[1].ss_txbuf[2] = 0x85;
		break;
	case LPM_2NIT:
	default:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_2NIT_CMD);
		set_lpm_off->cmds[1].ss_txbuf[1] = 0x00;
		set_lpm_off->cmds[1].ss_txbuf[2] = 0x03;
		break;
	}
...
#define FFC_CMD_LEN (1)
#define FFC_START_IDX (1)
...
static int ss_self_display_data_init(struct samsung_display_driver_data *vdd)
{
	...
	make_self_dispaly_img_cmds_SW83109_A73(vdd, TX_SELF_MASK_IMAGE, FLAG_SELF_MASK);
	...
	make_self_dispaly_img_cmds_SW83109_A73(vdd, TX_SELF_MASK_IMAGE_CRC, FLAG_SELF_MASK_CRC);
...
void SW83109_BF067XMM_A73_FHD_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->self_disp.init = self_display_init_SW83109_A73;
