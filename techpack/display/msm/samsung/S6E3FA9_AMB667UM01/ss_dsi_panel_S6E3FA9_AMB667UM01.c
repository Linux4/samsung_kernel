...
static struct dsi_panel_cmd_set * mdss_brightness_gamma_mode2(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if(vdd->br_info.common_br.cd_idx <= MAX_BL_PF_LEVEL){
		...
		pcmds->cmds[2].msg.tx_buf[1] = get_bit(vdd->br_info.common_br.cd_level, 8, 2);
		pcmds->cmds[2].msg.tx_buf[2] = get_bit(vdd->br_info.common_br.cd_level, 0, 8);
	}
	else{
		LCD_INFO("HBM : cd_idx [%d] \n", vdd->br_info.common_br.cd_idx);
		pcmds->cmds[0].msg.tx_buf[1] = vdd->finger_mask_updated? 0xE0 : 0xE8;	/* HBM Smooth transition : 0xE8 */
		pcmds->cmds[1].msg.tx_buf[1] = vdd->br_info.temperature > 0 ? vdd->br_info.temperature : (char)(BIT(7) | (-1*vdd->br_info.temperature));
		pcmds->cmds[1].msg.tx_buf[3] = elvss_table[vdd->br_info.common_br.cd_level];		/* ELVSS Value for HBM brgihtness */
		pcmds->cmds[2].msg.tx_buf[1] = get_bit(vdd->br_info.common_br.cd_level, 8, 2);
		pcmds->cmds[2].msg.tx_buf[2] = get_bit(vdd->br_info.common_br.cd_level, 0, 8);
	}
...
static int poc_erase(struct samsung_display_driver_data *vdd, u32 erase_pos, u32 erase_size, u32 target_pos)
{
	...
	LCD_INFO("erase flash address [%02x] [%02x] [%02x]\n", poc_erase_sector_tx_cmds->cmds[2].msg.tx_buf[4],
													 poc_erase_sector_tx_cmds->cmds[2].msg.tx_buf[5],
													 poc_erase_sector_tx_cmds->cmds[2].msg.tx_buf[6]);
