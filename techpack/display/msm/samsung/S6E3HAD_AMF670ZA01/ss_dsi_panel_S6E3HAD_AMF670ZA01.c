...
*/
#include "ss_dsi_panel_S6E3HAD_AMF670ZA01.h"
#include "ss_dsi_mdnie_S6E3HAD_AMF670ZA01.h"

static char VBIAS_MTP[2];
static int samsung_panel_on_pre(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO(vdd, "+: ndx=%d\n", vdd->ndx);
	if(vdd->panel_revision < 3) {/* LESS than 0x45 revC */
		if (is_first) {
			ss_panel_data_read(vdd, RX_VBIAS_MTP, &VBIAS_MTP[0], LEVEL2_KEY);
			ss_panel_data_read(vdd, RX_VAINT_MTP, &VBIAS_MTP[1], LEVEL2_KEY);
			LCD_INFO(vdd, "is_first VBIAS_MTP = 0x%x 0x%x\n", (int)VBIAS_MTP[0], (int)VBIAS_MTP[1]);
			is_first = false;
		}
		pcmds = ss_get_cmds(vdd, TX_DISPLAY_ON);
		idx[0] = ss_get_cmd_idx(pcmds, 0x122, 0xC6);
		idx[1] = ss_get_cmd_idx(pcmds, 0x142, 0xC6);

		for(iter1 = 0; iter1 <= 1 ; iter1++) {
			for(iter2 = 1; iter2 <= 5 ; iter2++) {
				pcmds->cmds[idx[iter1]].ss_txbuf[iter2] = (int)VBIAS_MTP[iter1];
			}
		}
	}
...
HS
	29 00 00 00 00 00 04 B0 00 A6 C7			/* VRR_CMDID_AOR_GLOBAL */
	29 00 00 00 00 00 0B C7 09 A8 09 A8 01 AD 01 AD 01 AD	/* VRR_CMDID_AOR */
	29 00 00 00 00 00 04 B0 00 06 BD
	29 00 00 00 00 00 06 BD 08 00 14 00 01		/* VRR_CMDID_LFD_SET */
	29 00 00 00 00 00 02 BD 01
	29 00 00 00 00 00 04 B0 00 17 BD
	29 00 00 00 00 00 02 BD 40
	29 00 00 00 00 00 04 B0 00 03 F2
	29 00 00 00 00 00 02 F2 00					/* VRR_CMDID_OSC_SET1 */
	29 00 00 00 00 00 04 B0 00 06 F2
	29 00 00 00 00 00 03 F2 00 10				/* VRR_CMDID_VFP1 */
	29 00 00 00 00 00 04 B0 00 0D F2
	29 00 00 00 00 00 03 F2 00 10				/* VRR_CMDID_VFP2 */
	29 00 00 00 00 00 04 B0 00 44 F2
	29 00 00 00 00 00 02 F2 00					/* VRR_CMDID_OSC_SET2 */
	29 00 00 00 00 00 03 60 00 00				/* VRR_CMDID_FREQ_SET1 */
	/* 0x00, 0x00 : 120Hz */
	/* 0x00, 0x04 : 60Hz */
	/* 0x00, 0x08 : 40Hz */
	/* 0x00, 0x0C : 30Hz */
	...
	/* 0x00, 0x28 : 10.9 Hz */
	/* 0x00, 0x2C : 1 0Hz */
	29 00 00 00 00 00 02 B9 09	/* TE modulation
					 * 09: default,
					 * 19: 60HS
	...
	29 00 00 00 00 00 02 6A 00
	29 00 00 00 00 00 04 B0 03 E3 C6		/* Moved from gamma comp to meet timing */
	29 00 00 00 00 00 02 C6 00				/* Gamma enabler */
	29 00 00 00 00 00 04 B0 00 53 C7		/* Moved from gamma comp to meet timing */
	29 00 00 00 00 00 02 C7 00				/* AOR enabler */
	29 00 00 00 00 00 02 F7 0F
NS
	29 00 00 00 00 00 04 B0 00 86 C7			/* VRR_CMDID_AOR_GLOBAL */
	29 00 00 00 00 00 07 0B C7 09 A8 09 A8 01 AD 01 AD 01 AD 	/* VRR_CMDID_AOR */
	29 00 00 00 00 00 04 B0 00 06 BD
	29 00 00 00 00 00 06 BD 00 04 00 00 01		/* VRR_CMDID_LFD_SET */
	29 00 00 00 00 00 02 BD 01
	29 00 00 00 00 00 04 B0 00 17 BD
	29 00 00 00 00 00 02 BD 40
	29 00 00 00 00 00 04 B0 00 03 F2
	29 00 00 00 00 00 02 F2 50					/* VRR_CMDID_OSC_SET1 */
	29 00 00 00 00 00 04 B0 00 06 F2
	29 00 00 00 00 00 03 F2 00 10				/* VRR_CMDID_VFP1 */
	29 00 00 00 00 00 04 B0 00 0D F2
	29 00 00 00 00 00 03 F2 00 10				/* VRR_CMDID_VFP2 */
	29 00 00 00 00 00 04 B0 00 44 F2
	29 00 00 00 00 00 02 F2 80					/* VRR_CMDID_OSC_SET2 */
	29 00 00 00 00 00 03 60 00 00				/* VRR_CMDID_FREQ_SET1 */
	/* 0x00, 0x00: 60Hz */
	/* 0x00, 0x04: 30Hz */
	/* 0x00, 0x08: 20Hz */
...
u8 cmdid_aor_global[VRR_MAX][3] = {
	{0x00, 0x86, 0xC7},/*VRR_48NS*/
	{0x00, 0x86, 0xC7},/*VRR_60NS*/
	{0x00, 0xA6, 0xC7},/*VRR_48HS*/
	{0x00, 0xA6, 0xC7},/*VRR_60HS*/
	{0x00, 0xA6, 0xC7},/*VRR_96HS*/
	{0x00, 0xA6, 0xC7},/*VRR_120HS*/
};

u8 cmdid_aor_normal[10] = {
	0x09, 0xA8, 0x09, 0xA8, 0x01, 0xAD, 0x01, 0xAD, 0x01, 0xAD
};
					...
					int *level_key, bool is_hbm, bool is_hmt)
{
	...
	vrr_cmds->cmds[VRR_CMDID_LFD_SET].ss_txbuf[1] = (((min_div - 1) << 2) & 0xFF00) >> 8;
	vrr_cmds->cmds[VRR_CMDID_LFD_SET].ss_txbuf[2] = ((min_div - 1) << 2) & 0xFF;

	if (min_div == 1)
		vrr_cmds->cmds[VRR_CMDID_LFD_SET].ss_txbuf[1] = 0x08;
...
static struct dsi_panel_cmd_set *ss_acl_on(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	pcmds->cmds[6].ss_txbuf[1] = 0x01;	/* ACL 8% in HBM */

	if (vdd->br_info.gradual_acl_val == 2)
		pcmds->cmds[6].ss_txbuf[1] = 0x03;	/* 0x03 : ACL 30% */
...
static struct dsi_panel_cmd_set *ss_elvss_for_factory_test(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	cmd_idx = ss_get_cmd_idx(elvss_cmds, 0x11, 0xC2);
	elvss_cmds->cmds[cmd_idx].ss_txbuf[1] = vdd->set_factory_elvss? 0x07 : 0x16;
	cmd_idx = ss_get_cmd_idx(elvss_cmds, 0x16, 0xC2);
	elvss_cmds->cmds[cmd_idx].ss_txbuf[1] = vdd->set_factory_elvss? 0x00 : vdd->br_info.common_br.elvss_value[0];
...
{
	struct dsi_panel_cmd_set *irc_flat_cmds = ss_get_cmds(vdd, TX_IRC);
	struct dsi_panel_cmd_set *irc_moderate_cmds = ss_get_cmds(vdd, TX_IRC_OFF);
	...
	if (SS_IS_CMDS_NULL(irc_moderate_cmds)) {
		LCD_ERR(vdd, "No irc_moderate_cmds\n");
		return NULL;
	}
	if (SS_IS_CMDS_NULL(irc_flat_cmds)) {
		LCD_ERR(vdd, "No irc_flat_cmds\n");
		return NULL;
	}
...
static struct dsi_panel_cmd_set *ss_vint(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (vdd->xtalk_mode)
		vint_cmds->cmds[1].ss_txbuf[1] = 0x06;
	else
		vint_cmds->cmds[1].ss_txbuf[1] = 0x0E;
...
static u8 hlpm_aor[4][30] = {
	{0x01, 0xAD, 0x01, 0xAD, 0x01, 0xAD, 0x01, 0xAD, 0x01, 0xAD, 0x01, 0xAD, 0x01, 0xAD,
	 0x01, 0xAD, 0x01, 0xAD, 0x01, 0xAD, 0x01, 0xAD, 0x01, 0xAD, 0x01, 0xAD, 0x01, 0xAD, 0x01, 0xAD},
	{0x06, 0x08, 0x06, 0x08, 0x06, 0x08, 0x06, 0x08, 0x06, 0x08, 0x06, 0x08, 0x06, 0x08,
	 0x06, 0x08, 0x06, 0x08, 0x06, 0x08, 0x06, 0x08, 0x06, 0x08, 0x06, 0x08, 0x06, 0x08, 0x06, 0x08},
	{0x08, 0xF5, 0x08, 0xF5, 0x08, 0xF5, 0x08, 0xF5, 0x08, 0xF5, 0x08, 0xF5, 0x08, 0xF5,
	 0x08, 0xF5, 0x08, 0xF5, 0x08, 0xF5, 0x08, 0xF5, 0x08, 0xF5, 0x08, 0xF5, 0x08, 0xF5, 0x08, 0xF5},
	{0x0A, 0x30, 0x0A, 0x30, 0x0A, 0x30, 0x0A, 0x30, 0x0A, 0x30, 0x0A, 0x30, 0x0A, 0x30,
...
static void ss_set_panel_lpm_brightness(struct samsung_display_driver_data *vdd)
{
	...
	aor_idx = ss_get_cmd_idx(set, 0xE9, 0xC7);
	...
	case LPM_30NIT:
		...
		memcpy(&set->cmds[aor_idx].ss_txbuf[1], hlpm_aor[1], sizeof(hlpm_aor[1]));
		break;
	case LPM_10NIT:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_10NIT_CMD);
		memcpy(&set->cmds[aor_idx].ss_txbuf[1], hlpm_aor[2], sizeof(hlpm_aor[2]));
		break;
	case LPM_2NIT:
	default:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_2NIT_CMD);
		memcpy(&set->cmds[aor_idx].ss_txbuf[1], hlpm_aor[3], sizeof(hlpm_aor[3]));
		break;
	}
				...
				(struct samsung_display_driver_data *vdd, int enable)
{
	...
	aor_idx = ss_get_cmd_idx(set_lpm_on, 0xE9, 0xC7);
start_lpm_bl:
	/* LPM_ON: 3. HLPM brightness */
	...
	case LPM_60NIT:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_60NIT_CMD);
		memcpy(&set_lpm_on->cmds[aor_idx].ss_txbuf[1], hlpm_aor[0], sizeof(hlpm_aor[0]));
		gm2_wrdisbv = 248;
		break;
	case LPM_30NIT:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_30NIT_CMD);
		memcpy(&set_lpm_on->cmds[aor_idx].ss_txbuf[1], hlpm_aor[1], sizeof(hlpm_aor[1]));
		gm2_wrdisbv = 124;
		break;
	case LPM_10NIT:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_10NIT_CMD);
		memcpy(&set_lpm_on->cmds[aor_idx].ss_txbuf[1], hlpm_aor[2], sizeof(hlpm_aor[2]));
		gm2_wrdisbv = 43;
		break;
	case LPM_2NIT:
	default:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_2NIT_CMD);
		memcpy(&set_lpm_on->cmds[aor_idx].ss_txbuf[1], hlpm_aor[3], sizeof(hlpm_aor[3]));
		gm2_wrdisbv = 9;
		break;
	}
	...
	cmd_idx = ss_get_cmd_idx(set_lpm_on, 0x53, 0xC7);
...
static void ss_gray_spot(struct samsung_display_driver_data *vdd, int enable)
{
	...
	if (!enable) { /* grayspot off */
		pcmds->cmds[10].ss_txbuf[1] = vdd->br_info.common_br.elvss_value[0];
	}
}
...
{
	u8 valid_checksum[4] = {0xF5, 0xB5, 0xF5, 0xB5};
	int res;
...
static int ss_gct_write(struct samsung_display_driver_data *vdd)
{
	...
	u8 vddm_level_set[MAX_VDDM] = {0x0, 0x07, 0x28};
	...
	for (i = VDDM_LV; i < MAX_VDDM; i++) {
		...
		cmd_idx = ss_get_cmd_idx(set, 0x03, 0xD7);
		set->cmds[cmd_idx].ss_txbuf[1] = vddm_set[i];

		cmd_idx = ss_get_cmd_idx(set, 0x10, 0xF4);
		set->cmds[cmd_idx].ss_txbuf[1] = vddm_level_set[i];
...
static int ss_self_display_data_init(struct samsung_display_driver_data *vdd)
{
	...
	make_mass_self_display_img_cmds_HAD(vdd, TX_SELF_MASK_IMAGE, FLAG_SELF_MASK);
...
static int ss_ffc(struct samsung_display_driver_data *vdd, int idx)
{
	...
	cmd_idx = ss_get_cmd_idx(ffc_set, 0x16, 0xC5);
...
static int ss_gm2_gamma_comp_init(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO(vdd, "++\n");
	if (vdd->panel_revision >= 2) {/* asof revC */
		memcpy(MTP_OFFSET_96HS_VAL, MTP_OFFSET_96HS_VAL_revC, sizeof(MTP_OFFSET_96HS_VAL));
	} else {
...
struct dsi_panel_cmd_set *ss_brightness_gm2_hbm_gamma_comp(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (cur_rr == 96 || cur_rr == 48) {
		...
		gamma_comp_cmds->cmds[cmd_idx].ss_txbuf[3] = 0x18;
...
void S6E3HAD_AMF670ZA01_FHD_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.br_func[BR_FUNC_ELVSS] = ss_elvss_for_factory_test;
	...
	vdd->panel_func.br_func[BR_FUNC_HBM_IRC] = ss_irc;
...
/*	vdd->motto_info.vreg_ctrl_0 = 0x51;*/
}
