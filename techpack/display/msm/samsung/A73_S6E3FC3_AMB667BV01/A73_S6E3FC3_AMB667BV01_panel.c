...
*/
#include "A73_S6E3FC3_AMB667BV01_panel.h"
#include "A73_S6E3FC3_AMB667BV01_mdnie.h"
				...
				(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*/
	...
	if (vdd->finger_mask_updated) {
		...
		aor cmd -> mask image
		*/
		pcmds->cmds[4].last_command = 1;

		if (vdd->vrr.cur_refresh_rate > 60)
			pcmds->cmds[4].post_wait_ms = HBM_NORMAL_DELAY_120FPS;
		else
			pcmds->cmds[4].post_wait_ms = HBM_NORMAL_DELAY_60FPS;
	} else {
		pcmds->cmds[4].last_command = 0;
...
{
	char gray_spot_buf[2];
	struct dsi_panel_cmd_set *pcmds_off;
	...
	if (enable) {
		...
		LCD_INFO(vdd, "A73_3FC3 : gray_spot_buf[0] : 0x%x\n", gray_spot_buf[0]);
		LCD_INFO(vdd, "A73_3FC3 : gray_spot_buf[1] : 0x%x\n", gray_spot_buf[1]);

		/* TX_GRAY_SPOT_TEST_OFF : replace 0xXX, 0xXX */
		memcpy(&pcmds_off->cmds[3].ss_txbuf[1],
				gray_spot_buf,
				sizeof(char) * (pcmds_off->cmds[3].msg.tx_len - 1));
	} else {
		/* Nothing to do */
	}
}
...
static struct dsi_panel_cmd_set *ss_acl_on(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	pcmds->cmds[2].ss_txbuf[1] = 0x01;	/* ACL 8% in normal brightness */
...
static struct dsi_panel_cmd_set *ss_vint(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	idx = ss_get_cmd_idx(vint_cmds, 0x1E, 0xF4);

	if (vdd->xtalk_mode)
		vint_cmds->cmds[idx].ss_txbuf[1] = 0x0D; // ON
	else
		vint_cmds->cmds[idx].ss_txbuf[1] = 0x11; // OFF
...
static int ss_self_display_data_init(struct samsung_display_driver_data *vdd)
{
	...
	make_self_dispaly_img_cmds_A73_S6E3FC3_AMB667BV01(vdd, TX_SELF_MASK_IMAGE, FLAG_SELF_MASK);
	...
	make_self_dispaly_img_cmds_A73_S6E3FC3_AMB667BV01(vdd, TX_SELF_MASK_IMAGE_CRC, FLAG_SELF_MASK_CRC);
...
static int ss_ffc(struct samsung_display_driver_data *vdd, int idx)
{
	...
	memcpy(ffc_set->cmds[5].ss_txbuf,
			dyn_ffc_set->cmds[idx].ss_txbuf,
			ffc_set->cmds[5].msg.tx_len);
...
void A73_S6E3FC3_AMB667BV01_FHD_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->self_disp.init = self_display_init_A73_S6E3FC3_AMB667BV01;
