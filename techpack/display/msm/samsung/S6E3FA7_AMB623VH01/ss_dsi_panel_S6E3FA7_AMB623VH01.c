...
static struct dsi_panel_cmd_set * ss_brightness_gamma_mode2_normal(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	pcmds->cmds[0].ss_txbuf[1] = tset;

	pcmds->cmds[4].ss_txbuf[1] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 8, 3);
	pcmds->cmds[4].ss_txbuf[2] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 0, 8);
	LCD_INFO(vdd, "cd_idx: %d, cd_level: %d, WRDISBV: %x %x, elvss_dim_offset: %x, tset = 0x%x\n",
			vdd->br_info.common_br.cd_idx,
			vdd->br_info.common_br.cd_level,
			pcmds->cmds[4].ss_txbuf[1],
			pcmds->cmds[4].ss_txbuf[2],
			pcmds->cmds[0].ss_txbuf[3],
...
static struct dsi_panel_cmd_set * ss_brightness_gamma_mode2_hbm(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	pcmds->cmds[2].ss_txbuf[1] = tset;

	pcmds->cmds[2].ss_txbuf[3] = elvss_table_hbm_revA[vdd->br_info.common_br.cd_idx]; 	/* ELVSS Value for HBM brgihtness */

	/* 29 00 00 00 00 00 04 B1 11 70 7B			AOR interpolation */
	/* FA7_AMS623 uses same wrdisbv for HBM & interpolate aor */
	pcmds->cmds[1].ss_txbuf[1] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 8, 3) << 4 | 0x01;
	pcmds->cmds[1].ss_txbuf[3] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 0, 8);
	LCD_INFO(vdd, "cd_idx: %d, cd_level: %d, AOR 1st : %x, AOR 3rd : %x elvss_dim_offset: %x, tset = 0x%x\n",
			vdd->br_info.common_br.cd_idx,
			vdd->br_info.common_br.cd_level,
			pcmds->cmds[1].ss_txbuf[1],
			pcmds->cmds[1].ss_txbuf[3],
			pcmds->cmds[2].ss_txbuf[3],
...
static struct dsi_panel_cmd_set *ss_acl_on_hbm(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*level_key = LEVEL1_KEY;
	...
	pcmds->cmds[0].ss_txbuf[2] = 0x80;	/* start point 60% 80 64 */
	pcmds->cmds[0].ss_txbuf[3] = 0X64;
	pcmds->cmds[0].ss_txbuf[5] = 0X29;	/* ACL 8% */
...
static struct dsi_panel_cmd_set *ss_acl_on(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*level_key = LEVEL1_KEY;
	...
	pcmds->cmds[0].ss_txbuf[2] = 0x40;	/* start point 50% 40 FC */
	pcmds->cmds[0].ss_txbuf[3] = 0XFC;
	pcmds->cmds[0].ss_txbuf[5] = 0X4D;	/* ACL 15% */
...
static struct dsi_panel_cmd_set *ss_vint(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (vdd->xtalk_mode) {
		vint_cmds->cmds[1].ss_txbuf[1] = 0x6B;	/* VGH 6.2 V */
		LCD_INFO(vdd, "xtalk_mode on\n");
	}
	else {
		vint_cmds->cmds[1].ss_txbuf[1] = 0xCB;	/* VGH return V */
	}
...
static void ss_grayspot_etc(struct samsung_display_driver_data *vdd, int enable)
{
	...
	if(!enable)
	{
		...
		pcmds->cmds[7].ss_txbuf[1] = B5_23TH[0];
	}
	return;
}
...
static int poc_erase(struct samsung_display_driver_data *vdd, u32 erase_pos, u32 erase_size, u32 target_pos)
{
	...
	if (erase_size == POC_ERASE_64KB) {
		...
		poc_erase_sector_tx_cmds->cmds[2].ss_txbuf[2] = 0xD8;
	} else if (erase_size == POC_ERASE_32KB) {
		delay_us = 800000; /* 800ms */
		poc_erase_sector_tx_cmds->cmds[2].ss_txbuf[2] = 0x52;
	} else {
		delay_us = 400000; /* 400ms */
		poc_erase_sector_tx_cmds->cmds[2].ss_txbuf[2] = 0x20;
	}
...
static int poc_write(struct samsung_display_driver_data *vdd, u8 *data, u32 write_pos, u32 write_size)
{
	...
	if (SS_IS_CMDS_NULL(write_cmd)) {
		LCD_ERR(vdd, "no cmds for TX_POC_WRITE_LOOP_1BYTE..\n");
		return -EINVAL;
	}
	...
	LCD_INFO(vdd, "[WRITE] write_pos : %6d, write_size : %6d, last_pos %6d, poc_w_sise : %d delay %dus\n",
	...
	for (pos = write_pos; pos < last_pos; ) {
		...
		if (pos % loop_cnt == 0) {
			...
			LCD_DEBUG(vdd, "WRITE_LOOP_START pos : %d \n", pos);
			ss_send_cmd(vdd, TX_POC_WRITE_LOOP_START);

			usleep_range(delay_us, delay_us);

			/*	Multi Data Address */
			write_data_add->cmds[0].ss_txbuf[vdd->poc_driver.write_addr_idx[0]]
											= (pos & 0xFF0000) >> 16;
			write_data_add->cmds[0].ss_txbuf[vdd->poc_driver.write_addr_idx[1]]
											= (pos & 0x00FF00) >> 8;
			write_data_add->cmds[0].ss_txbuf[vdd->poc_driver.write_addr_idx[2]]
											= (pos & 0x0000FF);
		...
		/* data copy */
		write_cmd->cmds[0].ss_txbuf[1] = data[pos];
	...
	if (read_pos == 0) {
		if (vdd->poc_driver.check_read_case) {
			if (vdd->poc_driver.read_case == READ_CASE1) {
				...
				LCD_INFO(vdd, "WRITE [TX_POC_PRE_READ2]");
...
static int check_read_case(struct samsung_display_driver_data *vdd)
{
	...
	if (vdd->poc_driver.mca_data[4] == 0x00 &&
		...
		LCD_INFO(vdd, "FLASH READ_CASE1\n");
		return READ_CASE1;
	} else {
		LCD_INFO(vdd, "FLASH READ_CASE2\n");
		return READ_CASE2;
	}
}
