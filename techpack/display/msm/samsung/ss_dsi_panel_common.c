...
static int ss_check_te(struct samsung_display_driver_data *vdd)
{
		...
		struct samsung_display_driver_data *vdd)
{
	...
	for (panel_rev = 0; panel_rev < SUPPORT_PANEL_REVISION; panel_rev++) {
		...
		parse_dt_data(np, &info->candela_map_table[GAMMA_MODE2_NORMAL][panel_rev],
				sizeof(struct candela_map_table),
...
#if !defined(BRING_UP_TEMP_DEFINE)
...
static void set_normal_br_values(struct samsung_display_driver_data *vdd)
{
	...
	if (vdd->br_info.common_br.pac)
		...
		if (vdd->br_info.common_br.gamma_mode2_support)
			table = &vdd->br_info.candela_map_table[GAMMA_MODE2_NORMAL][vdd->panel_revision];
		else if(vdd->br_info.common_br.multi_to_one_support)
	...
	if (vdd->br_info.common_br.pac) {
		...
		if (vdd->br_info.common_br.gamma_mode2_support) {
			vdd->br_info.common_br.gamma_mode2_cd = table->gamma_mode2_cd[p];
			LCD_INFO("[%d] cd_idx (%d) cd (%d) panel_real_cd (%d)\n",
				p, vdd->br_info.common_br.cd_idx, vdd->br_info.common_br.cd_level, vdd->br_info.common_br.gamma_mode2_cd);
		} else {
			LCD_INFO("[%d] cd_idx (%d) cd (%d) \n", p, vdd->br_info.common_br.cd_idx, vdd->br_info.common_br.cd_level);
		}
	}
	return;
}
...
static void set_hmt_br_values(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO("cd_idx (%d) cd_level (%d) \n", vdd->br_info.hmt_stat.cmd_idx_hmt, vdd->br_info.hmt_stat.candela_level_hmt);
...
void ss_read_mtp(struct samsung_display_driver_data *vdd, int addr, int len, int pos, u8 *buf)
{
	...
	if (vdd->mipi_header_modi)
		ss_panel_data_read(vdd, RX_MTP_READ_SYSFS, buf, 0);
	else
...
void ss_panel_vrr_switch(struct vrr_info *vrr)
{
	...
	vdd->vrr.skip_dup_bl_update = false;
