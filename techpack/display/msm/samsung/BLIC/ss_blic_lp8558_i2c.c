...
int ss_blic_lp8558_control(struct samsung_display_driver_data *vdd)
{
	...
	*/
	...
	LCD_INFO("read values.. A0h:0x%x, 9Eh:0x%x\n", buf[0], buf[1]);
...
static int __init ss_blic_lp8558_init(void)
{
	...
	if (ret) {
		LCD_ERR("lp8558_init registration failed. ret= %d\n", ret);
	}

	return ret;
}

static void __exit ss_blic_lp8558_exit(void)
{
	i2c_del_driver(&ss_blic_lp8558_driver);
}

module_init(ss_blic_lp8558_init);
module_exit(ss_blic_lp8558_exit);

MODULE_DESCRIPTION("ss_blic_lp8558_i2c driver");
MODULE_LICENSE("GPL");
