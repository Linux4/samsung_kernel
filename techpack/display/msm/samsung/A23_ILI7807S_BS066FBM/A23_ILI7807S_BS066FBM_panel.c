 ...
 *
*/
#include "A23_ILI7807S_BS066FBM_mdnie.h"
#include "A23_ILI7807S_BS066FBM_panel.h"
...
static struct dsi_panel_cmd_set *tft_pwm(struct samsung_display_driver_data *vdd, int *level_key)
{
	 ...
	 * use 11:4 bits. so, for 0x7F, commands will be 077F.
	 * config_screenBrightnessSettingMaximum : bl_level = 255 : (pwm + 1) 
	 * 
	 */
	set->cmds->ss_txbuf[1] = (pwm >> 4) & 0xFF;
	set->cmds->ss_txbuf[2] = (pwm << 4) & 0xFF;

	LCD_INFO(vdd, "bl: %d, cd: %d, pwm: %02X, cmd: %02X %02X\n",
			vdd->br_info.common_br.bl_level,
			vdd->br_info.common_br.cd_level,
			pwm, set->cmds->ss_txbuf[1], set->cmds->ss_txbuf[2]);
