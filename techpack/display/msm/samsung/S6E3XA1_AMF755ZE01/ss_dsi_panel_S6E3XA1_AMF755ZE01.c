...
*/
#include "ss_dsi_panel_S6E3XA1_AMF755ZE01.h"
#include "ss_dsi_mdnie_S6E3XA1_AMF755ZE01.h"
...
static int samsung_panel_on_post(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO(vdd, "lpcharge = %d vdd->is_recovery_mode %d\n", lpcharge, vdd->is_recovery_mode);
	...
	if (cur_rr == 48 | cur_rr == 96) {
		...
		LCD_INFO(vdd, "compensate gamma for 48/96hz mode\n");
	}
...
static char ss_panel_revision(struct samsung_display_driver_data *vdd)
{
	...
	panel_rev = ss_panel_id0_get(vdd) & 0x03;
...
static struct dsi_panel_cmd_set * ss_brightness_gamma_mode2_normal(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	pcmds->cmds[0].ss_txbuf[1] = vdd->finger_mask_updated? 0x20 : 0x28;  /* Normal Smooth transition : 0x28 */
	pcmds->cmds[1].ss_txbuf[1] = tset;
	...
	if (vdd->br_info.common_br.irc_mode == IRC_MODERATO_MODE)
		pcmds->cmds[4].ss_txbuf[2] = 0xE1;
	else if (vdd->br_info.common_br.irc_mode == IRC_FLAT_GAMMA_MODE)
		pcmds->cmds[4].ss_txbuf[2] = 0xA1;
	else
		LCD_ERR(vdd, "invalid irc mode(%d)\n", vdd->br_info.common_br.irc_mode);

	LCD_INFO(vdd, "cd_idx: %d, cd_level: %d, WRDISBV: %x %x, tset: %x\n",
			vdd->br_info.common_br.cd_idx,
			vdd->br_info.common_br.cd_level,
			pcmds->cmds[2].ss_txbuf[1],
			pcmds->cmds[2].ss_txbuf[2],
			pcmds->cmds[1].ss_txbuf[1]);
...
static struct dsi_panel_cmd_set * ss_brightness_gamma_mode2_hbm(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	pcmds->cmds[1].ss_txbuf[3] = elvss_table_hbm_revA[vdd->br_info.common_br.cd_idx]; 	/* ELVSS Value for HBM brgihtness */
	...
	LCD_INFO(vdd, "cd_idx: %d, cd_level: %d, WRDISBV: %x %x, elvss: %x, tset: %x\n",
			vdd->br_info.common_br.cd_idx,
			vdd->br_info.common_br.cd_level,
			pcmds->cmds[2].ss_txbuf[1],
			pcmds->cmds[2].ss_txbuf[2],
			pcmds->cmds[1].ss_txbuf[3],
			pcmds->cmds[1].ss_txbuf[1]);
	*level_key = LEVEL1_KEY;
...
static int ss_gm2_ddi_flash_prepare_revA(struct samsung_display_driver_data *vdd)
{
	...
	if (checksum_tot_flash != checksum_tot_cal)
		...
		LCD_INFO(vdd, "total flash checksum SUCCESS\n");
...
static int ss_gm2_ddi_flash_prepare_revD(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO(vdd, "ddi_flash_raw_len: %d, start_addr %X, flash_raw_len : %d\n", ddi_flash_raw_len, start_addr, ddi_flash_raw_len);
...
{
	// revA is for old panel in TOP project
	if (0)
...
static int ss_test_ddi_flash_check(struct samsung_display_driver_data *vdd, char *buf)
{
	...
	gm2_table->is_flash_checksum_ok = 1;
	gm2_table->checksum_tot_flash = gm2_table->checksum_tot_cal;
...
static void convert_V_to_GAMMA_XA1(int *src, u8 *dst)
{
	...
	for (i = 0; i < GAMMA_V_SIZE; i++)
		if (dbg_org[i] != src[i])
			LCD_DEBUG(vdd, "fix: src[%d] %02X -> %02X\n", i, dbg_org[i],  src[i]);
#endif
...
static int ss_gm2_gamma_comp_init(struct samsung_display_driver_data *vdd)
{
	...
	if (SS_IS_CMDS_NULL(rx_cmds)) {
		LCD_ERR(vdd, "No cmds for RX_SMART_DIM_MTP.. \n");
		return -ENODEV;
	}
	 ...
	 * 0xB9 ~0xDD: HS, MODE3
	 */
	rx_cmds->cmds->ss_txbuf[0] =  0xC8;
	rx_cmds->cmds->msg.rx_len = GAMMA_SET_SIZE * 3; /* len */
	rx_cmds->read_startoffset = 0x6F; /* pos */
	 ...
	 * 0x82 ~0xA6: HS, MODE7
	 */
	rx_cmds->cmds->ss_txbuf[0] =  0xC7;
	rx_cmds->cmds->msg.rx_len = GAMMA_SET_SIZE * 4; /* len */
	rx_cmds->read_startoffset = 0x13; /* pos */
	ss_panel_data_read(vdd, RX_SMART_DIM_MTP, readbuf, LEVEL1_KEY);
	buf = readbuf;
	...
	for (i_mode = GAMMA_SET_MODE1; i_mode < GAMMA_SET_MODE_MAX; i_mode++) {
		LCD_INFO(vdd, "gamma_org GAMMA_SET_MODE%d\n", i_mode+1);
		...
		LCD_INFO(vdd, "gamma_comp GAMMA_SET_MODE%d\n", i_mode+1);
...
static struct dsi_panel_cmd_set * ss_brightness_gm2_gamma_comp(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (vrr->gm2_gamma != VRR_GM2_GAMMA_NO_ACTION) {
		LCD_INFO(vdd, "gm2_gamma compensation(%d)\n", vrr->gm2_gamma);
		...
		memcpy(&pcmds->cmds[GAMMA_COMP_CMDID_C8].ss_txbuf[1], buf,
		...
		memcpy(&pcmds->cmds[GAMMA_COMP_CMDID_C7].ss_txbuf[1], buf,
					...
					int *level_key, bool is_hbm, bool is_hmt)
{
	...
	/* 1. LFD setting */
	memcpy(&vrr_cmds->cmds[VRR_CMDID_LFD_SET].ss_txbuf[1],
			&cmd_lfd_set[vrr_id][0],
			vrr_cmds->cmds[VRR_CMDID_LFD_SET].msg.tx_len - 1);

	vrr_cmds->cmds[VRR_CMDID_LFD_SET].ss_txbuf[14] = (min_div - 1);

	/* 2. VRR_OSC_SETTING VRR_FREQ_SETTING */
	switch (cur_rr) {
		case 120 :
		case 96 :
			vrr_cmds->cmds[VRR_CMDID_FREQ_SETTING].ss_txbuf[2]= HS_120HZ;
			break;
		case 60 :
		case 48 :
			vrr_cmds->cmds[VRR_CMDID_FREQ_SETTING].ss_txbuf[2]= HS_60HZ;
			break;
	}
	vrr_cmds->cmds[VRR_CMDID_FREQ_SETTING].ss_txbuf[2] = max_div - 1;

	/* 3. VFP setting */
	memcpy(&vrr_cmds->cmds[VRR_CMDID_VFP_05].ss_txbuf[1],
			&cmd_vfp_change[vrr_id][0],
			vrr_cmds->cmds[VRR_CMDID_VFP_05].msg.tx_len - 1);
	memcpy(&vrr_cmds->cmds[VRR_CMDID_VFP_0C].ss_txbuf[1],
			&cmd_vfp_change[vrr_id][0],
	...
	/* ++ FOR DEBUGGING */
	LCD_DEBUG(vdd, "vbias value for mode = %d start \n", mode);

	for (i = 0 ; i < 22 ; i++) {
		LCD_DEBUG(vdd, "0x%x ", vbias[i]);
	}
	LCD_DEBUG(vdd, "vbias value for mode = %d end \n", mode);
	/* -- FOR DEBUGGING */

	if (cur_hs == 1) {
		vrr_cmds->cmds[VRR_CMDOFFSET_VAINT].ss_txbuf[1] = 0x47;
	}
	else {
		vrr_cmds->cmds[VRR_CMDOFFSET_VAINT].ss_txbuf[1] = 0x39;
	}

	if (!vbias) {
		LCD_ERR(vdd, "fail to get vbias..\n");
	} else {
		memcpy(&vrr_cmds->cmds[VRR_CMDID_VBIAS].ss_txbuf[1],
				&vbias[0],
				vrr_cmds->cmds[VRR_CMDID_VBIAS].msg.tx_len - 1);
		memcpy(&vrr_cmds->cmds[VRR_CMDID_VAINT].ss_txbuf[1],
				&vbias[vaint_offset],
				vrr_cmds->cmds[VRR_CMDID_VAINT].msg.tx_len - 1);
		vrr_cmds->cmds[VRR_CMDID_VAINT].ss_txbuf[vaint_last_value] = vbias[21];
	}
	...
	if (lpcharge || vdd->is_recovery_mode)
		vrr_cmds->cmds[VRR_CMDID_TE_MOD].ss_txbuf[1] = 0x01;
...
static int ss_gct_write(struct samsung_display_driver_data *vdd)
{
	...
	for (i = VDDM_LV; i < MAX_VDDM; i++) {
		...
		set->cmds[13].ss_txbuf[1] = vddm_set[i];
...
static struct dsi_panel_cmd_set *ss_acl_on_hbm(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*level_key = LEVEL1_KEY;
	...
	pcmds->cmds[0].ss_txbuf[3] = 0x80;	/* start point 60% 80 65 */
	pcmds->cmds[0].ss_txbuf[4] = 0X65;
	pcmds->cmds[0].ss_txbuf[6] = 0X26;	/* ACL 8% */

	LCD_INFO(vdd, "HBM: gradual_acl: %d, start point 0x%x0x%x acl per: 0x%x",
			vdd->br_info.gradual_acl_val, pcmds->cmds[0].ss_txbuf[3], pcmds->cmds[0].ss_txbuf[4], pcmds->cmds[0].ss_txbuf[6]);
...
static struct dsi_panel_cmd_set *ss_acl_on(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*level_key = LEVEL1_KEY;
	...
	pcmds->cmds[0].ss_txbuf[3] = 0x40;	/* start point 50% 40 FF */
	pcmds->cmds[0].ss_txbuf[4] = 0XFF;
	pcmds->cmds[0].ss_txbuf[6] = 0X50;	/* ACL 15% */

	LCD_INFO(vdd, "gradual_acl: %d, start point 0x%x0x%x acl per: 0x%x",
			vdd->br_info.gradual_acl_val, pcmds->cmds[0].ss_txbuf[3], pcmds->cmds[0].ss_txbuf[4], pcmds->cmds[0].ss_txbuf[6]);
...
static struct dsi_panel_cmd_set *ss_vint(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (vdd->xtalk_mode) {
		vint_cmds->cmds[2].ss_txbuf[1] = 0x06;	/* VGH 6.2 V */
		LCD_INFO(vdd, "xtalk_mode on\n");
	}
	else {
		vint_cmds->cmds[2].ss_txbuf[1] = 0x10;	/* VGH return V */
	}
...
static void ss_set_panel_lpm_brightness(struct samsung_display_driver_data *vdd)
{
	...
	if (is_first || unlikely(vdd->is_factory_mode)) {
		...
		LCD_INFO(vdd, "is_first BB_9TH = 0x%x\n", (int)BB_9TH[0]);
	...
	case LPM_60NIT:
		...
		set_lpm->cmds[LPMBL_CMDID_LPM_ON].ss_txbuf[1]= 0x22;
		break;
	case LPM_30NIT:
	...
	default:
		...
		set_lpm->cmds[LPMBL_CMDID_LPM_ON].ss_txbuf[1]= 0x23;
		break;
	}
	...
	memcpy(&set_lpm->cmds[LPMBL_CMDID_CTRL].ss_txbuf[1],
			&set_lpm_bl->cmds->ss_txbuf[1],
			sizeof(char) * (set_lpm->cmds[LPMBL_CMDID_CTRL].msg.tx_len - 1));

	set_lpm->cmds[LPMBL_CMDID_CTRL].ss_txbuf[9] =
	...
	vrr->lfd.max_div = 1;
	if (max_div != 1)
		LCD_ERR(vdd, "LPM max_div support only 1 (req: %d)\n", max_div);

	set_lpm->cmds[LPMBL_CMDID_LFD].ss_txbuf[12] = min_div - 1;

	/* send lpm bl cmd */
	ss_send_cmd(vdd, TX_LPM_BL_CMD);

	LCD_INFO(vdd, "[Panel LPM] bl_level : %s, LFD: div cmd: 0x%x, %dhz-%dhz (%d-%d)\n",
			...
			set_lpm->cmds[LPMBL_CMDID_LFD].ss_txbuf[12],
...
static void ss_update_panel_lpm_ctrl_cmd(struct samsung_display_driver_data *vdd, int enable)
{
	...
	case LPM_60NIT:
		set_lpm_bl = ss_get_cmds(vdd, TX_HLPM_60NIT_CMD);
		set_lpm_on->cmds[LPMON_CMDID_LPM_ON].ss_txbuf[1]= 0x22;
		break;
	case LPM_30NIT:
		set_lpm_bl = ss_get_cmds(vdd, TX_HLPM_30NIT_CMD);
		set_lpm_on->cmds[LPMON_CMDID_LPM_ON].ss_txbuf[1]= 0x22;
	...
	default:
		set_lpm_bl = ss_get_cmds(vdd, TX_HLPM_2NIT_CMD);
		set_lpm_on->cmds[LPMON_CMDID_LPM_ON].ss_txbuf[1]= 0x23;
		break;
	}
	...
	memcpy(&set_lpm_on->cmds[LPMON_CMDID_CTRL].ss_txbuf[1],
			&set_lpm_bl->cmds->ss_txbuf[1],
			sizeof(char) * set_lpm_on->cmds[LPMON_CMDID_CTRL].msg.tx_len - 1);

	set_lpm_on->cmds[LPMON_CMDID_CTRL].ss_txbuf[9] =
		(vdd->panel_lpm.lpm_bl_level > LPM_2NIT) ? (((int)BB_9TH[0] & 0x3F) | 0x40) : (((int)BB_9TH[0] & 0x3F) | 0x80);
	...
	set_lpm_on->cmds[LPMON_CMDID_LFD].ss_txbuf[12] = min_div - 1;
...
static void ss_grayspot_etc(struct samsung_display_driver_data *vdd, int enable)
{
	...
	if(!enable)
	{
		...
		pcmds->cmds[8].ss_txbuf[1] = B5_75TH[0];
	}
	return;
}
...
void S6E3XA1_AMF755ZE01_QXGA_init(struct samsung_display_driver_data *vdd)
{
	...
	/* Self display */
	vdd->self_disp.mask_crc_force_pass = true;
