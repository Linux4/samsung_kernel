 ...
 */
static void convert_GAMMA_to_V_XA1(struct samsung_display_driver_data *vdd, u8 *src, int *dst)
{
	/* i : V index
	 * j : RGB index
	 * k : extend V index
	...
	for (i = 0; i < GAMMA_SET_SIZE; i++)
		LCD_DEBUG(vdd, "[SDE] gamma src[%d] %02x", i, src[i]);
#if 0

	for (i = 0; i < GAMMA_V_SIZE; i++)
		LCD_INFO(vdd, "gammaV dst[%d] %02x", i, dst[i]);
#endif
 ...
 */
static void convert_V_to_GAMMA_XA1(struct samsung_display_driver_data *vdd, int *src, u8 *dst)
{
	/* i : gamma index
...
static int ss_gm2_gamma_comp_init(struct samsung_display_driver_data *vdd)
{
	...
	for (i_mode = GAMMA_SET_MODE1; i_mode < GAMMA_SET_MODE_MAX; i_mode++) {
		...
		gamma_org = get_gamma(gm2_mtp, GAMMA_SET_VRR_HS, i_mode, true);
		convert_GAMMA_to_V_XA1(vdd, gamma_org, gammaV);
		...
		convert_V_to_GAMMA_XA1(vdd, gammaV, gamma_comp);
	}
	return 0;
}
...
static void ss_set_panel_lpm_brightness(struct samsung_display_driver_data *vdd)
{
	...
	set_lpm->cmds[LPMBL_CMDID_CTRL].ss_txbuf[9] =
		(vdd->panel_lpm.lpm_bl_level > LPM_2NIT) ? 0x95 : 0x55;
