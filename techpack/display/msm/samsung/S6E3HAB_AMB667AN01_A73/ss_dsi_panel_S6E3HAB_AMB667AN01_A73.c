...
*/
#include "ss_dsi_panel_S6E3HAB_AMB667AN01_A73.h"
#include "ss_dsi_mdnie_S6E3HAB_AMB667AN01_A73.h"
					...
					int *level_key, bool is_hbm, bool is_hmt)
{
	...
	if (cur_rr == 60) {
		...
		vrr_cmds->cmds[6].ss_txbuf[1] = 0x41; /* SourceAmp Setting : 60 HZ */
	} else {
		vrr_cmds->cmds[0].ss_txbuf[1] = 0x20; /* Frequency Setting : 120 HZ */
		vrr_cmds->cmds[6].ss_txbuf[1] = 0x01; /* SourceAmp Setting : 120 HZ */
	}
...
#define HBM_NORMAL_DELAY_60FPS (17)
							...
							(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (vdd->finger_mask_updated) {
		LCD_INFO(vdd, "NORMAL - FINGER UPDTAED\n");
		pcmds->cmds[0].post_wait_ms = FRAME_WAIT_120FPS;
		pcmds->cmds[2].ss_txbuf[1] = 0x96;
	} else {
		LCD_INFO(vdd, "NORMAL - NO FINGER\n");
		pcmds->cmds[0].post_wait_ms = 0;
		pcmds->cmds[2].ss_txbuf[1] = 0x56;
	}
							...
							(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (vdd->finger_mask_updated) {
		LCD_INFO(vdd, "HBM - FINGER UPDTAED\n");
		if (last_br_hbm == false) { /* Normal -> HBM Case Only */
			/* Smooth Dimming Off First */
			if (vdd->vrr.cur_refresh_rate == 60)
				pcmds_smooth_off->cmds[1].post_wait_ms = 2 * FRAME_WAIT_60FPS;
			else
				pcmds_smooth_off->cmds[1].post_wait_ms = 2 * FRAME_WAIT_120FPS;

			ss_send_cmd(vdd, TX_SMOOTH_DIMMING_OFF);
		}
		pcmds->cmds[8].post_wait_ms = HBM_NORMAL_DELAY_120FPS;
	} else {
		LCD_INFO(vdd, "HBM - NO FINGER\n");
		pcmds->cmds[8].post_wait_ms = 0;
	}
...
static struct dsi_panel_cmd_set *ss_acl_on(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	pcmds->cmds[0].ss_txbuf[6] = 0x26;	/* ACL 8% in normal brightness */
...
static int ss_self_display_data_init(struct samsung_display_driver_data *vdd)
{
	...
	make_self_dispaly_img_cmds_HAB_AMB667AN01_A73(vdd, TX_SELF_MASK_IMAGE, FLAG_SELF_MASK);
	...
	make_self_dispaly_img_cmds_HAB_AMB667AN01_A73(vdd, TX_SELF_MASK_IMAGE_CRC, FLAG_SELF_MASK_CRC);
...
void S6E3HAB_AMB667AN01_A73_FHD_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->self_disp.init = self_display_init_HAB_AMB667AN01_A73;
