...
int ss_buck_i2c_read_ex(u8 addr,  u8 *value)
{
	...
	if (!buck_pinfo.client) {
		LCD_ERR("No isl98609 ic, not probed.\n");
	} else {
		struct i2c_msg msg[] = {
			{
...
static int __init ss_buck_isl98608_init(void)
{
	...
	if (ret)
		LCD_ERR("ss_buck_isl98608_init registration failed. ret= %d\n", ret);

	return ret;
}

static void __exit ss_buck_isl98608_exit(void)
{
