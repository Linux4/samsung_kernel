...
static u8 TE_FRAME_SEL[VRR_MAX][7] = {
	...
	{0x61, 0x00, 0x03, 0x05, 0x28, 0x00, 0x00},/*VRR_48HS*/
	{0x61, 0x00, 0x03, 0x05, 0x28, 0x00, 0x00},/*VRR_60HS*/
	{0x61, 0x00, 0x01, 0x05, 0x28, 0x00, 0x00},/*VRR_96HS*/
	...
	u8 *out_cmd, u32 base_rr, u32 min_div, u32 max_div)
{
	...
	LCD_INFO(vdd, "min_freq = %d, max_freq = %d, frame_inser_idx = %d\n",
...
static struct dsi_panel_cmd_set *ss_vint(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	idx = ss_get_cmd_idx(vint_cmds, 0x0C, 0xF4);

	if (vdd->xtalk_mode)
		vint_cmds->cmds[idx].ss_txbuf[1] = 0x16; // ON
	else
		vint_cmds->cmds[idx].ss_txbuf[1] = 0x1A; // OFF
...
static void ss_set_panel_lpm_brightness(struct samsung_display_driver_data *vdd)
{
	...
	lpm_ctrl_idx = ss_get_cmd_idx(set, 0x5F, 0xBD);
	if (min_fps == 30)
		set->cmds[lpm_ctrl_idx].ss_txbuf[2] = 0x00;
	else
		set->cmds[lpm_ctrl_idx].ss_txbuf[2] = 0x1D;

	lpm_ctrl_idx = ss_get_cmd_idx(set, 0x1A, 0xBD);
	if (min_fps == 30)
		set->cmds[lpm_ctrl_idx].ss_txbuf[2] = 0x00;
	else
		set->cmds[lpm_ctrl_idx].ss_txbuf[2] = 0x1D;
	...
	set->cmds[lpm_ctrl_idx].ss_txbuf[2] = aor_pload_0;
	set->cmds[lpm_ctrl_idx].ss_txbuf[3] = aor_pload_1;
			...
			(struct samsung_display_driver_data *vdd, int enable)
{
	...
	lpm_ctrl_idx = ss_get_cmd_idx(lpm_cmd, 0x5F, 0xBD);
	if (min_fps == 30)
