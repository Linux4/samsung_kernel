 ...
 */
int ss_mafpc_make_img_cmds_HAB(struct samsung_display_driver_data *vdd, char *data,
...
int ss_mafpc_make_img_mass_cmds_HAB(struct samsung_display_driver_data *vdd, char* data,
									u32 data_size, int cmd_type)
{
	struct dsi_cmd_desc *tcmds;
	struct dsi_panel_cmd_set *pcmds;
...
#define BUF_LEN 200
int ss_mafpc_update_enable_cmds_HAB(struct samsung_display_driver_data *vdd)
{
	struct dsi_panel_cmd_set *pcmds;
...
static int ss_mafpc_img_write(struct samsung_display_driver_data *vdd, bool is_instant)
{
	...
	if (is_instant) {
		...
		pcmds->cmds[0].ss_txbuf[1] = buf_1;
		pcmds->cmds[1].ss_txbuf[1] = buf_2;
		pcmds->cmds[2].ss_txbuf[1] = buf_3;
	}
...
int ss_mafpc_enable(struct samsung_display_driver_data *vdd, int enable)
{
	int ret = 0;
	 ...
	 */
	ret = ss_mafpc_update_enable_cmds_HAB(vdd);
	if (unlikely(ret < 0)) {
		LCD_ERR(vdd, "failed to ss_mafpc_update_enable_cmds\n");
		return ret;
	}
			 ...
			 size_t total_count, loff_t *ppos)
{
	...
	ret = ss_mafpc_make_img_mass_cmds_HAB(vdd, vdd->mafpc.img_buf, vdd->mafpc.img_size, TX_MAFPC_IMAGE);
	if (unlikely(ret < 0)) {
		LCD_ERR(vdd, "failed to ss_mafpc_make_img_mass_cmds_HAB\n");
		return ret;
	}
...
int ss_mafpc_init_HAB(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.br_func[BR_FUNC_MAFPC_SCALE] = ss_mafpc_brightness_scale_HAB;
