...
#include "self_display_HAB_AMB667AN01.h"
 ...
 */
void make_self_dispaly_img_cmds_HAB_AMB667AN01(struct samsung_display_driver_data *vdd,
		int cmd, u32 op)
{
	...
	if (IS_ERR_OR_NULL(pcmds->cmds)) {
		LCD_INFO(vdd, "pcmds->cmds is null!!\n");
...
void make_mass_self_display_img_cmds_HAB_AMB667AN01(struct samsung_display_driver_data *vdd,
		int cmd, u32 op)
{
	struct dsi_cmd_desc *tcmds;
	struct dsi_panel_cmd_set *pcmds;
	...
	LCD_INFO(vdd, "[HAB_AMB667AN01] [%s] total data size [%d], total cmd len[%d], cmd count [%d]\n",
	...
	LCD_INFO(vdd, "[HAB_AMB667AN01] Total Cmd Count(%d), Last Cmd Payload Len(%d)\n", c_cnt, tcmds[c_cnt-1].msg.tx_len);
...
static int self_display_debug(struct samsung_display_driver_data *vdd)
{
	...
	if (ss_get_cmds(vdd, RX_SELF_DISP_DEBUG)->count) {
		...
		LCD_INFO(vdd, "[HAB_AMB667AN01] SI_X_O(%u) SI_Y_O(%u) MEM_SUM_O(0x%X) SM_SUM_O(0x%X)\n",
		...
		if (vdd->self_disp.operation[FLAG_SELF_MASK].img_checksum !=
					vdd->self_disp.debug.SM_SUM_O) {
			LCD_ERR(vdd, "[HAB_AMB667AN01] self mask img checksum fail!!\n");
			return -1;
		}
	}
...
static void self_mask_img_write(struct samsung_display_driver_data *vdd)
{
	...
	usleep_range(1, 2); // delay 0.1 us

	LCD_INFO(vdd, "[HAB_AMB667AN01] TX_SELF_MASK_IMAGE\n");
...
static int self_mask_check(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO(vdd, "[HAB_AMB667AN01] mask_crc_read_data (%x)\n", vdd->self_disp.mask_crc_read_data);
...
static int self_display_open(struct inode *inode, struct file *file)
{
	...
	LCD_INFO(vdd, "[HAB_AMB667AN01] [open]\n");
...
static int self_display_release(struct inode *inode, struct file *file)
{
	...
	LCD_INFO(vdd, "[HAB_AMB667AN01] [release]\n");
...
int self_display_init_HAB_AMB667AN01(struct samsung_display_driver_data *vdd)
{
	int ret = 0;
	...
	LCD_INFO(vdd, "[HAB_AMB667AN01] Success to register self_disp device..(%d)\n", ret);
