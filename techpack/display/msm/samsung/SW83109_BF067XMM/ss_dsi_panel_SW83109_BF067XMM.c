...
*/
#include "ss_dsi_panel_SW83109_BF067XMM.h"
#include "ss_dsi_mdnie_SW83109_BF067XMM.h"
							...
							(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	pcmds->cmds[0].ss_txbuf[1] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 8, 8);
	pcmds->cmds[0].ss_txbuf[2] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 0, 8);

	if (vdd->br_info.temperature > 1)
		pcmds->cmds[1].ss_txbuf[1] = 0x19;
	else if (vdd->br_info.temperature == 0)
		pcmds->cmds[1].ss_txbuf[1] = 0x00;
	else if (vdd->br_info.temperature > -15)
		pcmds->cmds[1].ss_txbuf[1] = 0x8E;
	else
		pcmds->cmds[1].ss_txbuf[1] = 0x94;
...
static struct dsi_panel_cmd_set *ss_acl_on(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	/* ACL 15% in Normal Brightness */
	pcmds->cmds[3].ss_txbuf[7]  = 0x59;
	pcmds->cmds[3].ss_txbuf[8]  = 0xD3;
	pcmds->cmds[3].ss_txbuf[9]  = 0x46;
	pcmds->cmds[3].ss_txbuf[10] = 0xBE;
...
static int ss_self_display_data_init(struct samsung_display_driver_data *vdd)
{
	...
	make_self_dispaly_img_cmds_SW83109(vdd, TX_SELF_MASK_IMAGE, FLAG_SELF_MASK);
	...
	make_self_dispaly_img_cmds_SW83109(vdd, TX_SELF_MASK_IMAGE_CRC, FLAG_SELF_MASK_CRC);
...
void SW83109_BF067XMM_FHD_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->self_disp.init = self_display_init_SW83109;
