...
ssize_t flash_power_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	for (i = 0; i < 3; i++)
	{
		g_flash_data.led_current_ma[i] = 125;
	}
...
#if defined(CONFIG_SEC_A42XQ_PROJECT) || defined(CONFIG_SEC_A51XQ_PROJECT) || defined(CONFIG_SEC_A52XQ_PROJECT) || defined(CONFIG_SEC_A42XUQ_PROJECT) || defined(CONFIG_SEC_M62XQ_PROJECT) || defined(CONFIG_SEC_M52XQ_PROJECT)
...
static ssize_t ois_power_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	{
		pr_err("%s: OIS client Pointer is NULL", __func__);
		return size;
	}
...
#endif
#if defined(CONFIG_SEC_M52XQ_PROJECT) || defined(CONFIG_SEC_M23XQ_PROJECT)
	23, //DUAL_REAR_PORTRAIT U and UW
#endif
