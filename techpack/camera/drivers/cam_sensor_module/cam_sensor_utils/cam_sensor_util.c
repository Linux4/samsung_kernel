		...
		case SENSOR_VDIG:
			for (j = 0; j < num_vreg; j++) {
				if (!strcmp(soc_info->rgltr_name[j],
					...
					/*F52 code add by chengzhi for ov16a1q dvdd at 2021.03.09 begin*/
					if (1200000==power_setting[i].config_val || 1100000==power_setting[i].config_val)
					{
						soc_info->rgltr_min_volt[j]=soc_info->rgltr_max_volt[j]=power_setting[i].config_val;
						CAM_ERR(CAM_SENSOR,"[O5codedebug-dvdd] soc_info->rgltr_min_volt[j]= %d",soc_info->rgltr_min_volt[j]);
					}
					/*F52 code add by chengzhi for ov16a1q dvdd at 2021.03.09 end*/
					if (VALIDATE_VOLTAGE(
