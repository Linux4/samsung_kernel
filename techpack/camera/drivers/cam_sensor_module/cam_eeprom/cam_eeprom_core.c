	...
	struct cam_eeprom_memory_block_t *block)
{
	...
	CAM_ERR(CAM_EEPROM, "sensor_id=0x%x,block->num_map=%d",sensor_id,block->num_map);
	...
	for (j = 0; j < block->num_map; j++) {
		...
		if (emap[j].mem.valid_size) {
			if(0x8044==sensor_id && (0!=j))
			{
				for(gc_i = 0; gc_i < emap[j].mem.valid_size; gc_i++){
					...
					*memptr = (uint8_t)gc_read;
					//CAM_ERR(CAM_EEPROM, "id=0x%x,addrType=%d,dataType=%d,ADDRESS=0x%x,memptr[%d]=0x%02x",e_ctrl->io_master_info.cci_client->sid,emap[j].mem.addr_type,emap[j].mem.data_type,gc_base,emap[j].mem.addr,*memptr);
					memptr++;
					gc_base++;
				}
			}
			...
			if (rc < 0) {
				...
				gc_base=gc_base+emap[j].mem.valid_size;
