	...
	uint32_t is_supported, uint8_t *pMapData, uint32_t camera_normal_cal_crc, ModuleInfo_t *mInfo)
{
	...
	if (mInfo->type == SEC_TELE2_SENSOR) {
		...
		if (isValidIdx(ConfIdx, &ConfAddr) == 1)
		{
			memcpy(rear4_module_fw_ver, &pMapData[ConfAddr], FROM_MODULE_FW_INFO_SIZE);
		...
		if (isValidIdx(ConfIdx, &ConfAddr) == 1)
		{
			memcpy(rear4_sensor_id, &pMapData[ConfAddr], FROM_SENSOR_ID_SIZE);
...
#endif
	{
        ...
        CAM_DBG(CAM_EEPROM, "[CAL] index:%d valid(%d, %d)", e_ctrl->soc_info.index, isValidIdx(ADDR_M_AF, &ConfAddr), isValidIdx(ADDR_S0_AF, &ConfAddr));
		{
			...
			AfIdx_t rear3_idx[] = {
				{AF_CAL_NEAR_IDX, AF_CAL_NEAR_OFFSET_FROM_AF},
				{AF_CAL_FAR_IDX, AF_CAL_FAR_OFFSET_FROM_AF},
#if !defined(CONFIG_SEC_R9Q_PROJECT) && !defined(CONFIG_SEC_Q2Q_PROJECT) && !defined(CONFIG_SEC_V2Q_PROJECT)
				{AF_CAL_M1_IDX, AF_CAL_M1_OFFSET_FROM_AF}
#endif
			};
#endif
...
#if defined(CONFIG_SAMSUNG_REAR_DUAL)
	else if (idx == SEC_ULTRA_WIDE_SENSOR) {
		pEEPROM_fw_version = rear2_fw_ver;
		pPHONE_fw_version = "N";
		pLOAD_fw_version = rear2_load_fw_ver;
		pCAM_fw_version = rear2_fw_ver;
		pCAM_fw_full_version = rear2_fw_full_ver;
	}
#endif
#if defined(CONFIG_SAMSUNG_FRONT_TOP)
	else if (idx == SEC_FRONT_TOP_SENSOR) {
...
#endif
#if defined(CONFIG_SAMSUNG_REAR_TOF)
	else if (idx == SEC_REAR_TOF_SENSOR) {
...
#endif
#if defined(CONFIG_SAMSUNG_FRONT_TOF)
	else if (idx == SEC_FRONT_TOF_SENSOR) {
...
int32_t cam_eeprom_check_firmware_cal(uint32_t camera_cal_crc, uint32_t camera_normal_cal_crc, ModuleInfo_t *mInfo)
{
	...
	if (mInfo->type == SEC_WIDE_SENSOR)	{
		if (camera_fw_crc == OK) {
			...
			cnt = scnprintf(&final_cmd_ack[offset], SYSFS_FW_VER_SIZE-offset, "%s", "FW");
			offset += cnt;
	...
	if (version_isp != 'Q' && version_isp != 'U' && version_isp != 'A' && version_isp != 'X' && version_isp != 'E' && version_isp != 'L') {
...
/**
 * cam_otp_hi1336c_read() - read map data into buffer for hi1336c
 * @io_master_info:	otp io struct
...
/**
 * cam_otp_hi1336c_burst_read() - read map data into buffer with burst mode
 * @io_master_info:	otp io struct
...
/**
 * cam_otp_hi1336c_read_memory() - read map data into buffer
 * @e_ctrl:	otp control struct
 * @block:	block to be read
 *
 * This function iterates through blocks stored in block->map, reads each
