...
//P86801AA1-1797,lihuan1wingtech.com,add,20230512,add camera otp bring up code
#include "eeprom_Cal_Data/eepromWriteInitParmsBeforeReadCalData.h"
...
//+P86801AA1-1797,lihuan1wingtech.com,add,20230512,add camera otp bring up code
//+bug P86801AA1-1797, jiangzhongyuan.wt, add, 2023.05.18, add sc520cs otp bring up
		if (emap[j].mem.valid_size) {
			uint16_t sensor_id = 0;
			char gc_read[3]={0};
			if(eb_info->i2c_info.slave_addr == w11_sc520cs_front_txd_i2c_slave_addr){
				camera_io_dev_read_seq(&e_ctrl->io_master_info, w11_sc520cs_front_txd_sensor_id_addr, gc_read,
				CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE,2);
				sensor_id = (uint16_t)(gc_read[0]<<8|gc_read[1]);
			}
			if( sensor_id == w11_sc520cs_front_txd_sensor_id ){
				if(0 == eeprom_memory_map_read_data_sc520cs(eb_info->i2c_info.slave_addr,emap[j],e_ctrl,memptr)){
					CAM_DBG(CAM_EEPROM, "read otp data successfully");
				} else {
				...
				if(0 == eeprom_memory_map_read_data(eb_info->i2c_info.slave_addr,emap[j],e_ctrl,memptr)){
					CAM_DBG(CAM_EEPROM, "read otp data successfully");
				} else {
					rc = camera_io_dev_read_seq(&e_ctrl->io_master_info,
					 emap[j].mem.addr, memptr,
				...
				if(0 == eeprom_memory_map_unint_para(eb_info->i2c_info.slave_addr,e_ctrl)){
					CAM_ERR(CAM_EEPROM,"unint otp pata data successfully");
				}else{
					CAM_ERR(CAM_EEPROM,"unint otp pata data fail");
				}
			}
		}
//-bug P86801AA1-1797, jiangzhongyuan.wt, add, 2023.05.18, add sc520cs otp bring up
...
//+P86801AA1-1797,lihuan1wingtech.com,add,20230512,add camera otp bring up code
			#if EEPROM_DUMP_ENABLE
			CAM_ERR(CAM_EEPROM, "e_ctrl->cal_data.num_data: %d",
					e_ctrl->cal_data.num_data);
			for (print_i=0; print_i <e_ctrl->cal_data.num_data; print_i++)
				CAM_ERR(CAM_EEPROM, "eeprom cali data  read_buffer[%d]: 0x%x",
						print_i, read_buffer[print_i]);
			#endif
//-P86801AA1-1797,lihuan1wingtech.com,add,20230512,add camera otp bring up code
