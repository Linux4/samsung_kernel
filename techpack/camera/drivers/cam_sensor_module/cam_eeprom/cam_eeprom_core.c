...
#ifdef CONFIG_GC02M1_SENSOR
#include "gc02m1_otp.h"
static int cam_otp_gc02m1_read_memory(struct cam_eeprom_ctrl_t *e_ctrl,
                               struct cam_eeprom_memory_block_t *block);
struct cam_sensor_i2c_reg_setting gc02m1_otp_load = {
    gc02m1_otp_load_setfile_reg, sizeof(gc02m1_otp_load_setfile_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE, 50
};

struct cam_sensor_i2c_reg_setting gc02m1_otp_init = {
    gc02m1_otp_init_read_sensor_reg, sizeof(gc02m1_otp_init_read_sensor_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE, 10
};

struct cam_sensor_i2c_reg_setting gc02m1_otp_finish_read = {
    gc02m1_otp_finish_read_sensor_reg, sizeof(gc02m1_otp_finish_read_sensor_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE, 10
};

struct cam_sensor_i2c_reg_setting gc02m1_otp_init_write = {
    gc02m1_otp_init_write_sensor_reg, sizeof(gc02m1_otp_init_write_sensor_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE, 10
};

struct cam_sensor_i2c_reg_setting gc02m1_otp_finish_write = {
    gc02m1_otp_finish_write_sensor_reg, sizeof(gc02m1_otp_finish_write_sensor_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE, 10
};
#endif

#ifdef CONFIG_GC08A3_SENSOR
#include "gc08a3_otp.h"
static int cam_otp_gc08a3_read_memory(struct cam_eeprom_ctrl_t *e_ctrl,
                               struct cam_eeprom_memory_block_t *block);
struct cam_sensor_i2c_reg_setting gc08a3_otp_load = {
    gc08a3_otp_load_setfile_reg, sizeof(gc08a3_otp_load_setfile_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE, 50
};

struct cam_sensor_i2c_reg_setting gc08a3_otp_init = {
    gc08a3_otp_init_read_sensor_reg, sizeof(gc08a3_otp_init_read_sensor_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE, 10
};

struct cam_sensor_i2c_reg_setting gc08a3_otp_finish_read = {
    gc08a3_otp_finish_read_sensor_reg, sizeof(gc08a3_otp_finish_read_sensor_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE, 10
};

struct cam_sensor_i2c_reg_setting gc08a3_otp_init_write = {
    gc08a3_otp_init_write_sensor_reg, sizeof(gc08a3_otp_init_write_sensor_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE, 10
};

struct cam_sensor_i2c_reg_setting gc08a3_otp_finish_write = {
    gc08a3_otp_finish_write_sensor_reg, sizeof(gc08a3_otp_finish_write_sensor_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE, 10
};
#endif
...
static int cam_eeprom_update_module_info(struct cam_eeprom_ctrl_t *e_ctrl)
{
    ...
    if (e_ctrl->soc_info.index == CAM_EEPROM_IDX_FRONT) {// Front cam
        ...
        map_ver = 0; //e_ctrl->cal_data.mapdata[FRONT_CAM_MAP_VERSION_ADDR];
#else
        ...
        if (e_ctrl->soc_info.index == CAM_EEPROM_IDX_WIDE) {
            ...
            map_ver = 0; //e_ctrl->cal_data.mapdata[REAR2_CAM_MAP_VERSION_ADDR];
#else
    ...
    if (e_ctrl->soc_info.index == CAM_EEPROM_IDX_FRONT) { //Front Cam
       ...
       // The below changes has not been verified for front sensor with actutor enabled.
        ...
        if(1 == e_ctrl->dualization_id){
            CAM_ERR(CAM_EEPROM, " OLDBOY e_ctrl->soc_info.index: %d", e_ctrl->soc_info.index);
        ...
        /* rear sensor id */
            memcpy(rear_sensor_id, &e_ctrl->cal_data.mapdata[REAR_SENSOR_ID_ADDR], FROM_SENSOR_ID_SIZE);
        ...
        /* rear module id */
            memcpy(rear_module_id, &e_ctrl->cal_data.mapdata[REAR_MODULE_ID_ADDR+REAR_MODULE_ID_ADDR_OFFSET], FROM_MODULE_ID_SIZE-REAR_MODULE_ID_ADDR_OFFSET);
            rear_module_id[FROM_MODULE_ID_SIZE-REAR_MODULE_ID_ADDR_OFFSET] = '\0';
        ...
        /* rear3 tilt */
...
#endif
#if defined(CONFIG_SEC_A42XQ_PROJECT) || defined(CONFIG_SEC_A51XQ_PROJECT) || defined(CONFIG_SEC_A52XQ_PROJECT) || defined(CONFIG_SEC_M62XQ_PROJECT)
            ...
            ConfAddr -= OIS_CAL_MARK_START_OFFSET;
            CAM_INFO(CAM_EEPROM, "ois_m1_cal_mark = 0X%x", ois_m1_cal_mark);
            ...
            if (REAR3_MODULE_ID_ADDR == 0)
            {
                memset(rear3_module_id, 0x00, sizeof(rear3_module_id));
            }
            else
            {
        ...
        /* update EEPROM fw version on sysfs */
...
#endif
					/* Load FW */
#if defined(CONFIG_SAMSUNG_CAMERA_MULTI_CAL)
					{
						...
						fw_size = fw->size;
						if (fw_size > FROM_REAR3_DUAL_CAL_SIZE) {
							fw_size = FROM_REAR3_DUAL_CAL_SIZE;
						}
						CAM_INFO(CAM_EEPROM, "rear3_dual_cal size = %d", fw_size);
            ...
            /* rear2 sensor id */
            memcpy(rear2_sensor_id, &e_ctrl->cal_data.mapdata[REAR2_SENSOR_ID_ADDR], FROM_SENSOR_ID_SIZE);
            /* rear2 module id */
...
#ifdef CAM_EEPROM_DBG
            CAM_INFO(CAM_EEPROM,
                "rear2 sensor id = %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c",
                rear2_sensor_id[0], rear2_sensor_id[1], rear2_sensor_id[2], rear2_sensor_id[3],
                rear2_sensor_id[4], rear2_sensor_id[5], rear2_sensor_id[6], rear2_sensor_id[7],
                rear2_sensor_id[8], rear2_sensor_id[9], rear2_sensor_id[10], rear2_sensor_id[11],
                rear2_sensor_id[12], rear2_sensor_id[13], rear2_sensor_id[14], rear2_sensor_id[15]);
...
#ifdef CAM_EEPROM_DBG
            CAM_INFO(CAM_EEPROM, "rear2_module_id = %c %c %c %c %c %c %c %c %c %c",
                rear2_module_id[0], rear2_module_id[1], rear2_module_id[2], rear2_module_id[3], rear2_module_id[4],
                rear2_module_id[5], rear2_module_id[6], rear2_module_id[7], rear2_module_id[8], rear2_module_id[9]);
#endif
...
#ifdef CAM_EEPROM_DBG
           CAM_INFO(CAM_EEPROM,
                "rear2 manufacturer info = %c %c %c %c %c %c %c %c %c %c",
                rear2_fw_ver[0], rear2_fw_ver[1], rear2_fw_ver[2], rear2_fw_ver[3], rear2_fw_ver[4],
                rear2_fw_ver[5], rear2_fw_ver[6], rear2_fw_ver[7], rear2_fw_ver[8], rear2_fw_ver[9],
                rear2_fw_ver[10]);
#endif
        ...
        /* update EEPROM fw version on sysfs */
...
#if defined(CONFIG_SAMSUNG_CAMERA_OTP)
            memcpy(rear4_fw_ver, &e_ctrl->cal_data.mapdata[SENSOR_MACRO_OTP_SENSOR_MODULE_FW_OFFSET], FROM_MODULE_FW_INFO_SIZE);
            ...
            CAM_INFO(CAM_EEPROM,
                "rear rear4_phone_fw_ver info = %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
                rear4_phone_fw_ver[0], rear4_phone_fw_ver[1], rear4_phone_fw_ver[2], rear4_phone_fw_ver[3], rear4_phone_fw_ver[4],
                rear4_phone_fw_ver[5], rear4_phone_fw_ver[6], rear4_phone_fw_ver[7], rear4_phone_fw_ver[8], rear4_phone_fw_ver[9],
                rear4_phone_fw_ver[10]);

            /* temp rear4 load version */
            /* temp load version */
            if (strncmp(rear4_phone_fw_ver, rear4_fw_ver, HW_INFO_MAX_SIZE-1) == 0
                && strncmp(&rear4_phone_fw_ver[HW_INFO_MAX_SIZE-1], &rear4_fw_ver[HW_INFO_MAX_SIZE-1], SW_INFO_MAX_SIZE-1) >= 0) {
                CAM_INFO(CAM_EEPROM, "Load from phone");
                strcpy(rear4_load_fw_ver, rear4_phone_fw_ver);
                loadfrom = 'P';
            } else {
            ...
            sensor_ver[1] = 0;
            dll_ver[1] = 0;
            normal_cri_rev = CRITERION_REV_MACRO;
            ...
            CAM_INFO(CAM_EEPROM,
                ...
                rear4_sensor_id[12], rear4_sensor_id[13], rear4_sensor_id[14], rear4_sensor_id[15]);
#ifdef CAM_EEPROM_DBG
            CAM_INFO(CAM_EEPROM, "rear4 sensor id = %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c",
                rear4_sensor_id[0], rear4_sensor_id[1], rear4_sensor_id[2], rear4_sensor_id[3],
                rear4_sensor_id[4], rear4_sensor_id[5], rear4_sensor_id[6], rear4_sensor_id[7],
                rear4_sensor_id[8], rear4_sensor_id[9], rear4_sensor_id[10], rear4_sensor_id[11],
                rear4_sensor_id[12], rear4_sensor_id[13], rear4_sensor_id[14], rear4_sensor_id[15]);
            ...
            memcpy(rear4_module_id, &e_ctrl->cal_data.mapdata[REAR4_MODULE_ID_ADDR], FROM_MODULE_ID_SIZE);
            rear4_module_id[FROM_MODULE_ID_SIZE] = '\0';
            CAM_INFO(CAM_EEPROM, "rear4 module id = %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X",
                rear4_module_id[0], rear4_module_id[1], rear4_module_id[2], rear4_module_id[3], rear4_module_id[4],
                rear4_module_id[5], rear4_module_id[6], rear4_module_id[7], rear4_module_id[8], rear4_module_id[9]);
#ifdef CAM_EEPROM_DBG
           CAM_INFO(CAM_EEPROM, "rear4_module_id = %c %c %c %c %c %c %c %c %c %c %c",
                rear4_module_id[0], rear4_module_id[1], rear4_module_id[2], rear4_module_id[3], rear4_module_id[4],
                rear4_module_id[5], rear4_module_id[6], rear4_module_id[7], rear4_module_id[8], rear4_module_id[9]);
#endif
            ...
            rear4_fw_ver[FROM_MODULE_FW_INFO_SIZE] = '\0';
            CAM_INFO(CAM_EEPROM, "rear4 manufacturer info = %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
                rear4_fw_ver[0], rear4_fw_ver[1], rear4_fw_ver[2], rear4_fw_ver[3], rear4_fw_ver[4],
                rear4_fw_ver[5], rear4_fw_ver[6], rear4_fw_ver[7], rear4_fw_ver[8], rear4_fw_ver[9],
                rear4_fw_ver[10]);
#ifdef CAM_EEPROM_DBG
    ...
    const char *update_fw_ver, cam_eeprom_fw_version_idx update_fw_index, cam_eeprom_idx_type idx)
{
    ...
    pCAM_fw_version = NULL;
    pCAM_fw_full_version = NULL;
    pEEPROM_fw_version = NULL;
    pPHONE_fw_version = NULL;
    pLOAD_fw_version = NULL;
    ...
    if (idx == CAM_EEPROM_IDX_WIDE) {
        ...
        pPHONE_fw_version = rear4_phone_fw_ver;
        ...
        CAM_ERR(CAM_SENSOR,
                "camera_idx %d is not correct", idx);
        return;
    }
...
#ifdef CONFIG_GC08A3_SENSOR
       rc = cam_otp_gc08a3_read_memory(e_ctrl, block);
#endif
    } else if(e_ctrl->soc_info.index == CAM_EEPROM_IDX_BOKEH) {
#ifdef CONFIG_GC02M1_SENSOR
        rc = cam_otp_gc02m1_read_memory(e_ctrl, block);
#endif
    } else {
#ifdef CONFIG_GC5035_SENSOR
        rc = cam_otp_gc5035_read_memory(e_ctrl, block);
                               ...
                               struct cam_eeprom_memory_block_t *block)
{
    ...
    case 0: //BANK1
    case 1:
        offset = SENSOR_OTP_BANK1_START_REGISTER;
        break;

    case 3: //BANK2
        offset = SENSOR_OTP_BANK2_START_REGISTER;
        break;
...
#ifdef CONFIG_GC02M1_SENSOR
/**
  * cam_otp_gc02m1_read() - read map data into buffer for gc02m1
  * @e_ctrl:    otp control struct
  * @addr:    memory address to be read
  * @memptr    memory address to be stored
  *
  ...
  * region and concatenate them into the pre-allocated block->mapdata
  */
static int cam_otp_gc02m1_read(struct cam_eeprom_ctrl_t *e_ctrl, uint32_t addr,
                                uint8_t *memptr)
{
    int rc = 0;
    struct cam_sensor_i2c_reg_setting i2c_reg_settings;
    ...
    i2c_reg_array.reg_data = 0x02;
    i2c_reg_settings.reg_setting = &i2c_reg_array;

    rc = camera_io_dev_write(&(e_ctrl->io_master_info), &i2c_reg_settings);
    if (rc < 0)
    {
        CAM_ERR(CAM_EEPROM, "write addr failed");
        goto err;
    }

    // OTP addr
    i2c_reg_array.reg_addr = 0x17;
    i2c_reg_array.reg_data = addr * 8;
    i2c_reg_settings.reg_setting = &i2c_reg_array;
    ...
    // OTP read pulse , Do read OTP
    i2c_reg_array.reg_addr = 0xF3;
    i2c_reg_array.reg_data = 0x34;
    i2c_reg_settings.reg_setting = &i2c_reg_array;

    rc = camera_io_dev_write(&(e_ctrl->io_master_info), &i2c_reg_settings);
    if (rc < 0)
    {
        CAM_ERR(CAM_EEPROM,"write pluse failed");
        goto err;
    }

    // read one byte
    rc = camera_io_dev_read_seq(&e_ctrl->io_master_info, 0x19, memptr, addr_type, data_type, 1);
    if (rc < 0)
    {
        CAM_ERR(CAM_EEPROM, "read failed rc %d", rc);
    }
    CAM_DBG(CAM_EEPROM,"addr = 0x%02X memptr[0]=%c *memptr=%c", addr * 8, memptr[0], *memptr);
...
/**
  * cam_otp_gc02m1_read_memory() - read map data into buffer
  * @e_ctrl:    otp control struct
  * @block:    block to be read
  *
  * This function iterates through blocks stored in block->map, reads each
                                      ...
                                      struct cam_eeprom_memory_block_t *block)
{
    ...
    CAM_INFO(CAM_EEPROM,"load_otp_setfile=%d index=%d",
        sizeof(gc02m1_otp_load)/sizeof(struct cam_sensor_i2c_reg_array), e_ctrl->soc_info.index);

    //load otp global setfile
    rc = camera_io_dev_write(&e_ctrl->io_master_info, &gc02m1_otp_load);
    if (rc < 0)
    {
        pr_err("%s:(%d) load otp globle setfile failed\n", __func__, __LINE__);
        return rc;
    ...
    //load read init setfile
    rc = camera_io_dev_write(&e_ctrl->io_master_info, &gc02m1_otp_init);
    if (rc < 0)
    {
        CAM_ERR(CAM_EEPROM,"load read init setfile failed");
        return rc;
    }

    // check page
    rc = camera_io_dev_read(&(e_ctrl->io_master_info), 0xFE, &OTP_Page, addr_type,data_type);
    if (rc < 0)
    {
        CAM_ERR(CAM_EEPROM,"read failed");
        return rc;
    }
    CAM_INFO(CAM_EEPROM,"current page: %d", OTP_Page);
    ...
    for (j = 1; j < block->num_map; j++)
    {
        read_size = emap[j].mem.valid_size;
        memptr = block->mapdata + emap[j].mem.addr;
        addr = emap[j].mem.addr + GC02M1_SENSOR_OTP_PAGE_START_REGISTER;

        CAM_INFO(CAM_EEPROM, "[%d / %d] memptr = %pK, emap[j].mem.addr = 0x%X, size = 0x%X, subdev = %d read_size=%u device_type = %d",
                j, block->num_map, memptr, emap[j].mem.addr, emap[j].mem.valid_size, e_ctrl->soc_info.index, read_size, e_ctrl->eeprom_device_type);
        ...
        CAM_INFO(CAM_EEPROM, "emap[%d].mem.addr=0x%x block->mapdata=%pK memptr=%pK OTP addr=0x%x", j, emap[j].mem.addr, block->mapdata, memptr, addr);
        while(read_size > 0)
        {
            rc = cam_otp_gc02m1_read(e_ctrl, addr, memptr);
            if (rc < 0)
            {
                CAM_ERR(CAM_EEPROM, "camera otp read fail rc:%d", rc);
                goto err;
    ...
    memptr = block->mapdata;
    CAM_INFO(CAM_EEPROM,"read data done memptr=%pK VR:: End",memptr);

err:
    return rc;
}
#endif //CONFIG_GC02M1_SENSOR

#ifdef CONFIG_GC08A3_SENSOR
/**
  * cam_otp_gc08a3_read() - read map data into buffer for gc02m1
  * @e_ctrl:    otp control struct
  * @addr:    memory address to be read
  * @memptr    memory address to be stored
  *
  ...
  * region and concatenate them into the pre-allocated block->mapdata
  */
static int cam_otp_gc08a3_read(struct cam_eeprom_ctrl_t *e_ctrl, uint32_t addr,
                                uint8_t *memptr)
{
    int rc = 0;
    struct cam_sensor_i2c_reg_setting i2c_reg_settings;
    ...
    i2c_reg_array.reg_data = ((addr*8)&(0xFF00))>>8;
    i2c_reg_settings.reg_setting = &i2c_reg_array;
    //CAM_INFO(CAM_EEPROM,"high addr = 0x%02X ", i2c_reg_array.reg_data);
    rc = camera_io_dev_write(&(e_ctrl->io_master_info), &i2c_reg_settings);
    if (rc < 0)
    {
        CAM_ERR(CAM_EEPROM,"write addr failed");
    ...
    i2c_reg_array.reg_data = ((addr*8)&(0x00FF));
    i2c_reg_settings.reg_setting = &i2c_reg_array;
    //CAM_INFO(CAM_EEPROM,"low addr = 0x%02X ", i2c_reg_array.reg_data);
    ...
    // read one byte
    rc = camera_io_dev_read_seq(&e_ctrl->io_master_info, 0x0a6c, memptr, addr_type, data_type, 1);
    if (rc < 0)
    {
        CAM_ERR(CAM_EEPROM, "read failed rc %d", rc);
    }
    CAM_DBG(CAM_EEPROM,"addr = 0x%02X memptr[0]=%c *memptr=0x%x (%c)", addr * 8, memptr[0], *memptr, *memptr);
...
/**
  * cam_otp_gc08a3_read_memory() - read map data into buffer
  * @e_ctrl:    otp control struct
  * @block:    block to be read
  *
  * This function iterates through blocks stored in block->map, reads each
                                      ...
                                      struct cam_eeprom_memory_block_t *block)
{
    ...
    CAM_ERR(CAM_EEPROM,"load_otp_setfile=%d index=%d",
        gc08a3_otp_load.size, e_ctrl->soc_info.index);

    rc = camera_io_dev_read(&(e_ctrl->io_master_info), GC08A3_SENSOR_OTP_SLAVE_ID_REGISTER,&OTP_ID,addr_type,CAMERA_SENSOR_I2C_TYPE_WORD);

    if (rc < 0)
    {
        CAM_ERR(CAM_EEPROM, "OTP match ID failed, return");
        return rc;
    }
    else
    {
        CAM_ERR(CAM_EEPROM, "current OTP_ID: 0x%x\n", OTP_ID);
    }

    //load otp global setfile
    rc = camera_io_dev_write(&e_ctrl->io_master_info, &gc08a3_otp_load);
    if (rc < 0)
    {
        pr_err("%s:(%d) load otp globle setfile failed\n", __func__, __LINE__);
        return rc;
    ...
    //load read init setfile
    rc = camera_io_dev_write(&e_ctrl->io_master_info, &gc08a3_otp_init);
    if (rc < 0)
    {
        CAM_ERR(CAM_EEPROM,"load read init setfile failed");
        return rc;
    ...
    // check page
    rc = camera_io_dev_read(&(e_ctrl->io_master_info), GC08A3_SENSOR_OTP_PAGE_SELECT_REGISTER,&OTP_Page,addr_type,data_type);
    if (rc < 0)
    {
        pr_err("%s:(%d) read failed\n", __func__, __LINE__);
        return rc;
    ...
    // select bank
    rc = cam_otp_gc08a3_read(e_ctrl, GC08A3_SENSOR_OTP_BANK_SELECT_REGISTER, &OTP_Bank);
    if (rc < 0)
    {
        pr_err("%s:(%d) read failed\n", __func__, __LINE__);
    }
    ...
    case 0: //BANK1
    case 1:
        offset = GC08A3_SENSOR_OTP_BANK1_START_REGISTER;
        break;
    ...
    for (j = 1; j < block->num_map; j++)
    {
        ...
        while(read_size > 0)
        {
            rc = cam_otp_gc08a3_read(e_ctrl, addr, memptr);
            if (rc < 0)
            {
                CAM_ERR(CAM_EEPROM, "camera otp read fail rc:%d", rc);
                goto err;
