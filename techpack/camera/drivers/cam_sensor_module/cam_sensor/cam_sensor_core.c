...
#endif
#if defined(CONFIG_SEC_A51XQ_PROJECT)
		if (s_ctrl->soc_info.index == 0) { // check Rear IMX582
			sensor_id = s_ctrl->sensordata->slave_info.sensor_id;
			expected_version_id = s_ctrl->sensordata->slave_info.version_id;
			rc = camera_io_dev_read(
				&(s_ctrl->io_master_info),
				IMX582_OTP_CHIP_REVISION_ADDR, &version_id,
				CAMERA_SENSOR_I2C_TYPE_WORD,
				CAMERA_SENSOR_I2C_TYPE_WORD);
				version_id>>=8;
				CAM_INFO(CAM_SENSOR,
					"Read IMX582 chip id 0x%x,expected_version_id 0x%x", version_id, expected_version_id);
			switch (version_id) {
			...
			if (rc < 0) {
				...
				CAM_INFO(CAM_SENSOR,
					"Read version id 0x%x,expected_version_id 0x%x", version_id, expected_version_id);
					if (version_id == expected_version_id && version_id == 0X0)
						CAM_INFO(CAM_SENSOR, "Found IMX582 Non OTP sensor");
					else if (version_id == expected_version_id && version_id == 0X2)
						CAM_INFO(CAM_SENSOR, "Found IMX582 OTP Sensor");
					else {
						CAM_INFO(CAM_SENSOR, "Not matched");
						rc = -EINVAL;
						cam_sensor_power_down(s_ctrl);
...
#if defined(CONFIG_GC5035_MACRO_OTP_DD_AUTOLOAD)
			if (s_ctrl->soc_info.index == 4) // check Rear Macro GC5035
			{
