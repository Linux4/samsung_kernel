...
extern char *cameraModuleInfo[5];
#endif
	...
	struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	s_ctrl->pipeline_delay =
		probe_info->reserved;
	/*hmd code add for resolve gotodump info by chengzhi at 2020.06.09 start*/
	copy_from_user(s_ctrl->sensordata->slave_info.sensor_name,probe_info->psensor_name,O5_SENSORNAME_MAXLENGTH);
	/*hmd code add for resolve gotodump info by chengzhi at 2020.06.09 end*/		
...
/*F52 code add for compatible with depth1st_2rd by xutengtao at 2021.02.11 start*/
/*
@ parameter : *s_ctrl                       sensor_ctrl
@ parameter : *gc_write_setting             gc02b10_reg_setting
@ return    : vendorId
*/
uint32_t cam_sensor_readOTP(struct cam_sensor_ctrl_t *s_ctrl,struct cam_sensor_i2c_reg_array *gc_write_setting)
{
    ...
    int32_t  gc_rc = 0;
    uint32_t vendorid;
    struct cam_sensor_i2c_reg_setting *gc02b10_write_setting;

    gc02b10_write_setting = (struct cam_sensor_i2c_reg_setting *)vmalloc(sizeof(struct cam_sensor_i2c_reg_setting));

    for(gc_i = 0; gc_i <= 1; gc_i++)
    {
        gc02b10_write_setting->addr_type = 1;
        gc02b10_write_setting->data_type = 1;
        gc02b10_write_setting->size = 1;
        gc02b10_write_setting->delay = 0;
        gc02b10_write_setting->reg_setting = &gc_write_setting[gc_i];
        gc_rc = camera_io_dev_write_continuous(&(s_ctrl->io_master_info),gc02b10_write_setting,1);
    }
    ...
    vfree(gc02b10_write_setting);
    return vendorid;
}
/*F52 code add for compatible with depth1st_2rd by xutengtao at 2021.02.11 end*/
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
    ...
    char *gc_sensor_name;
    uint32_t gc8034_lens_id;
    uint32_t vendorid;
    char *module_sensor_name="NULL_NAME";
    struct cam_sensor_i2c_reg_setting *gc_write_setting;

    static struct cam_sensor_i2c_reg_array gc02m1b_reg[20] =
    {
        //otp init setting begin
        {0xfe,0x00,0,0},{0xfc,0x01,0,0},{0xf4,0x41,0,0},{0xf5,0xc0,0,0},
        {0xf6,0x44,0,0},{0xf8,0x38,0,0},{0xf9,0x82,0,0},{0xfa,0x00,0,0},
        {0xfd,0x80,0,0},{0xfc,0x81,0,0},{0xf7,0x01,0,0},{0xfc,0x80,0,0},
        {0xfc,0x80,0,0},{0xfc,0x80,0,0},{0xfc,0x80,0,0},{0xfc,0x8e,0,0},
        {0xf3,0x30,0,0},

        //read otp
        {0xfe,0x02,0,0},
        {0x17,0x78,0,0}, //flag_addr = 0x78
                         //data= read_cmos_sensor(0x19);
        {0xf3,0x34,0,0},
    };
    //group1 vendor id addr 0x80
    static struct cam_sensor_i2c_reg_array gc02m1b_reg_group1[2] =
    {
        {0x17,0x80,0,0},
        {0xf3,0x34,0,0},
    };
    //group2 vendor id addr 0xc0
    static struct cam_sensor_i2c_reg_array gc02m1b_reg_group2[2] =
    {
       {0x17,0xc0,0,0},
       {0xf3,0x34,0,0},
    };
    //must closeOTP
    static struct cam_sensor_i2c_reg_array gc02m1b_reg_closeOTP[2] =
    {
        {0xf7,0x01,0,0},
        {0xfe,0x00,0,0},
    };
    ...
    /*F52 code add for compatible with macro_1st&2rd/widely_4p&5p/depth_1st_2rd by xutengtao at 2021.02.08 start*/
    ...
    CAM_INFO(CAM_SENSOR,"sensor_name =%s",s_ctrl->sensordata->slave_info.sensor_name);
    //widely 4P 5P lensId
    if((0 == strcmp(gc_sensor_name, "o5001_gc8034_widely_4P"))||(0 == strcmp(gc_sensor_name, "o5001_gc8034_widely_5P")))
    {
        s_ctrl->io_master_info.cci_client->sid=0xA8>>1;         //0xA8 eeprom IIC address
        CAM_ERR(CAM_SENSOR,"gc8034 cci_client->sid =%d",s_ctrl->io_master_info.cci_client->sid);
        camera_io_dev_read(&(s_ctrl->io_master_info),0x0C,&gc8034_lens_id, CAMERA_SENSOR_I2C_TYPE_WORD,CAMERA_SENSOR_I2C_TYPE_BYTE);
        CAM_ERR(CAM_SENSOR,"gc8034_lens_id =%d",gc8034_lens_id);
        if(gc8034_lens_id==0x42)
        {
            module_sensor_name="o5001_gc8034_widely_4P";
        }
        else if(gc8034_lens_id==0x02)
        {
            module_sensor_name="o5001_gc8034_widely_5P";
        }
    ...
    //macro 1st 2rd vendorId
    if((0 == strcmp(gc_sensor_name, "o5001_gc02m1_macrojk"))||(0 == strcmp(gc_sensor_name, "o5002_gc02m1_macrocxt")))
    {
        s_ctrl->io_master_info.cci_client->sid= 0xA4>>1;        //0xA4 eeprom IIC address
        CAM_ERR(CAM_SENSOR,"gc02m1_macro cci_client->sid =%d",s_ctrl->io_master_info.cci_client->sid);
        camera_io_dev_read(&(s_ctrl->io_master_info),0x01,&vendorid, CAMERA_SENSOR_I2C_TYPE_WORD,CAMERA_SENSOR_I2C_TYPE_BYTE);
        CAM_ERR(CAM_SENSOR,"gc02m1_macro vendorid =%d",vendorid);
        if(vendorid==0x08)
        {
            module_sensor_name="o5001_gc02m1_macrojk";
        }
        else if(vendorid==0x54)
        {
            module_sensor_name="o5002_gc02m1_macrocxt";
        }
        s_ctrl->io_master_info.cci_client->sid = 0x6E >> 1;     //0x6E sensor IIC address
        if(0!=strcmp(gc_sensor_name, module_sensor_name))
        {
    ...
    //depth 1st 2rd vendorId
    if((0 == strcmp(gc_sensor_name, "o5001_gc02m1b_depthjk"))||(0 == strcmp(gc_sensor_name, "o5002_gc02m1b_depthcxt")))//is gc02m1b
    {
        uint32_t gc_i = 0;
        int32_t  gc_rc = 0;
        uint32_t gc02m1b_flag = 0;   //flag to distinguish group1 & group2

        //init otp open to get ready to read addr
        //just read addr = 0x19
        for(gc_i = 0; gc_i <= 19; gc_i++)
        {
            gc_write_setting->addr_type = 1;
            gc_write_setting->data_type = 1;
            gc_write_setting->size = 1;
            gc_write_setting->delay = 0;
            gc_write_setting->reg_setting = &gc02m1b_reg[gc_i];
        ...
        camera_io_dev_read(&(s_ctrl->io_master_info),
                           ...
                           CAMERA_SENSOR_I2C_TYPE_BYTE,
                           CAMERA_SENSOR_I2C_TYPE_BYTE);
        CAM_ERR(CAM_SENSOR,"flag_reg=%d",gc02m1b_flag);

        if(gc02m1b_flag&0x04)             //group1
        {
            vendorid = cam_sensor_readOTP(s_ctrl,gc02m1b_reg_group1);
            CAM_ERR(CAM_SENSOR,"gc02m1b_group1:vendorid=0x%x",vendorid);
            if(vendorid == 0x08)
            {
                module_sensor_name = "o5001_gc02m1b_depthjk";
            }
            else if(vendorid == 0x54)
            {
                module_sensor_name = "o5002_gc02m1b_depthcxt";
            }
        }
        else if(gc02m1b_flag&0x01)        //group2
        {
            vendorid = cam_sensor_readOTP(s_ctrl,gc02m1b_reg_group2);
            CAM_ERR(CAM_SENSOR,"gc02m1b_group2:vendorid=0x%x",vendorid);
            if(vendorid == 0x08)
            {
                module_sensor_name = "o5001_gc02m1b_depthjk";
            }
        ...
        for(gc_i = 0; gc_i <= 1; gc_i++)
        {
            ...
            gc_write_setting->size = 1;
            gc_write_setting->delay = 0;
            gc_write_setting->reg_setting = &gc02m1b_reg_closeOTP[gc_i];
            gc_rc = camera_io_dev_write_continuous(&(s_ctrl->io_master_info),gc_write_setting,1);
        }
    ...
    vfree(gc_write_setting);
    /*F52 code add for compatible with macro_1st&2rd/widely_4p&5p/depth1st_2rd by xutengtao at 2021.02.08 end*/
	#if CAM_MODULE_INFO_CONFIG
