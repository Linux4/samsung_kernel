	...
	struct cam_req_mgr_flush_request *flush_req)
{
	...
	if (flush_req->type == CAM_REQ_MGR_FLUSH_TYPE_ALL) {
		...
		CAM_INFO(CAM_ISP, "REG_DUMP_ON_FLUSH Done");
	...
	struct cam_context *ctx)
{
	...
	hw_cmd_args.ctxt_to_hw_map = ctx_isp->hw_ctx;
	hw_cmd_args.cmd_type = CAM_HW_MGR_CMD_INTERNAL;
	isp_hw_cmd_args.cmd_type = CAM_ISP_HW_MGR_CMD_FULL_REG_DUMP;
	isp_hw_cmd_args.u.sof_irq_enable = 1;
	hw_cmd_args.u.internal_args = (void *)&isp_hw_cmd_args;
...
static int cam_isp_dump_req_list(struct cam_context *ctx,
	struct list_head *dump_req_list, unsigned long iova,
	uint32_t buf_info)
{
	struct cam_ctx_request *req = NULL;
	...
	uint32_t buf_info)
{
	...
	CAM_INFO(CAM_ISP, "iommu fault handler for isp ctx %d state %d",
		ctx->ctx_id, ctx->state);
	rc0 = cam_isp_dump_reg(ctx);

	rc = cam_context_dump_pf_info_to_hw(ctx, NULL,
		iova, buf_info, &mem_found);
	if (mem_found)
		CAM_ERR(CAM_ISP, "Found page fault");

	if (!list_empty(&ctx->active_req_list)) {
		CAM_INFO(CAM_ISP, "Iterating over active_list");
		rc = cam_isp_dump_req_list(ctx, &ctx->active_req_list,
			iova, buf_info);
	}

	if (!list_empty(&ctx->wait_req_list)) {
		CAM_INFO(CAM_ISP, "Iterating over wait_list");
		rc = cam_isp_dump_req_list(ctx, &ctx->wait_req_list,
			iova, buf_info);
	}
	...
	if (!list_empty(&ctx->wait_req_list)) {
		CAM_INFO(CAM_ISP,
			"Iterating over pending req list");
		rc = cam_isp_dump_req_list(ctx, &ctx->pending_req_list,
			iova, buf_info);
	}
