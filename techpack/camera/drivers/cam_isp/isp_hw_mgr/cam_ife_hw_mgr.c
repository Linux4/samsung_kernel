...
extern struct cam_ife_mem_to_port req_io_cfg_data[CAM_ISP_MAX_REQ_DUMP];
...
static int cam_ife_mgr_csid_change_mode(
	struct cam_ife_hw_mgr_ctx *ctx, struct list_head  *stop_list,
		uint32_t  base_idx, uint32_t halt_mode)
{
	struct cam_ife_hw_mgr_res      *hw_mgr_res;
	struct cam_isp_resource_node   *isp_res;
	struct cam_isp_resource_node   *stop_res = NULL;
	...
	list_for_each_entry(hw_mgr_res, stop_list, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			CAM_DBG(CAM_ISP, "base_idx %d res_type %d res_id %d cnt %u",
				base_idx, isp_res->res_type, isp_res->res_id, cnt);
	...
	if (cnt && stop_res) {
		hw_intf               = stop_res->hw_intf;
		stop.num_res          = 1;
		stop.node_res         = &stop_res;
		stop.halt_mode        = halt_mode;
		stop.change_halt_mode = true;
...
static int cam_ife_mgr_stop_hw(void *hw_mgr_priv, void *stop_hw_args)
{
	...
	if (csid_halt_type == CAM_CSID_HALT_IMMEDIATELY) {
		for (i = 0; i < ctx->num_base; i++) {
			if (ctx->base[i].idx == master_base_idx)
				continue;
			cam_ife_mgr_csid_change_mode(ctx, &ctx->res_list_ife_csid,
					ctx->base[i].idx, 0);
		}
	}

	CAM_INFO(CAM_ISP, "Stopping master CSID idx %d Halt type %d",
	...
	CAM_INFO(CAM_ISP, "Stopping master CID idx %d", master_base_idx);
	...
	CAM_INFO(CAM_ISP, "Going to stop IFE BUS read");
	/* IFE bus rd resources */
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_in_rd, list) {
		cam_ife_hw_mgr_stop_hw_res(hw_mgr_res);
	}
	...
	CAM_INFO(CAM_ISP, "pause hw done");
...
static void cam_ife_mgr_dump_full_reg(struct cam_ife_hw_mgr_ctx *ctx)
{
	struct cam_ife_hw_mgr_res        *hw_mgr_res;
	struct cam_hw_intf               *hw_intf;
...
static void cam_isp_dump_req_cfg_info(uint32_t pf_buf_info,
		bool *mem_found)
{
	int req_idx, i, j, k;

	for (req_idx = 0; req_idx < CAM_ISP_MAX_REQ_DUMP; req_idx++) {
		for (i = 0; i < CAM_ISP_MAX_PORTS_DUMP; i++) {
			CAM_INFO(CAM_ISP, "ReqID: %ld", req_io_cfg_data[req_idx].request);
			for (j = 0; j < 2; j++) {
				if (!req_io_cfg_data[req_idx].is_dual && j)
					continue;
				if (req_io_cfg_data[req_idx].is_dual)
					CAM_INFO(CAM_ISP, "Dual IFE idx %d", j);

				for (k = 0; k < req_io_cfg_data[req_idx].ports[i].ife[j].num_planes; k++) {
					if (pf_buf_info &&
						(GET_FD_FROM_HANDLE(req_io_cfg_data[req_idx].ports[i].ife[j].io_cfg.mem_handle[k]) ==
						 GET_FD_FROM_HANDLE(pf_buf_info))) {
						CAM_INFO(CAM_ISP,
							"Found PF at port: 0x%x mem 0x%x fd: 0x%x",
							req_io_cfg_data[req_idx].ports[i].ife[j].io_cfg.resource_type,
							req_io_cfg_data[req_idx].ports[i].ife[j].io_cfg.mem_handle[k],
							pf_buf_info);
						if (mem_found)
							*mem_found = true;
					}
					...
					CAM_INFO(CAM_ISP,
						"vfe %d pln %d port 0x%x w %d h %d s %u size 0x%x addr 0x%x end_addr 0x%x offset %x memh %x",
						req_io_cfg_data[req_idx].ife_idx, k,
						req_io_cfg_data[req_idx].ports[i].ife[j].io_cfg.resource_type,
						req_io_cfg_data[req_idx].ports[i].ife[j].io_cfg.planes[k].width,
						req_io_cfg_data[req_idx].ports[i].ife[j].io_cfg.planes[k].height,
						req_io_cfg_data[req_idx].ports[i].ife[j].io_cfg.planes[k].plane_stride,
						req_io_cfg_data[req_idx].ports[i].ife[j].planes[k].size,
						req_io_cfg_data[req_idx].ports[i].ife[j].planes[k].io_addr,
						(req_io_cfg_data[req_idx].ports[i].ife[j].planes[k].io_addr +
						 req_io_cfg_data[req_idx].ports[i].ife[j].planes[k].size),
						req_io_cfg_data[req_idx].ports[i].ife[j].io_cfg.offsets[k],
						req_io_cfg_data[req_idx].ports[i].ife[j].io_cfg.mem_handle[k]);
				}
			}
		}
	...
	bool *mem_found)
{
	...
	if (!packet) {
		cam_isp_dump_req_cfg_info(pf_buf_info, mem_found);
		return;
	}
...
static int cam_ife_mgr_cmd(void *hw_mgr_priv, void *cmd_args)
{
	...
	case CAM_HW_MGR_CMD_INTERNAL:
		...
		case CAM_ISP_HW_MGR_GET_PACKET_OPCODE:
			...
			break;
		case CAM_ISP_HW_MGR_CMD_FULL_REG_DUMP:
			cam_ife_mgr_dump_full_reg(ctx);
			break;
		default:
