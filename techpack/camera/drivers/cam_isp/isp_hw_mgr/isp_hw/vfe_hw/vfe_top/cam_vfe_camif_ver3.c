	...
	void *cmd_args, uint32_t arg_size)
{
	...
	ife_base_phy = (void *)camif_priv->soc_info->mem_block[0]->start;
	...
	for (i = 0; i < 6656; i++) {
		...
		dump_ife[local_val][i].addr = ife_base_phy + (i*4);
		dump_ife[local_val][i].val  = val;
	}
