...
static int cam_vfe_full_reg_dump(
	struct cam_vfe_top_ver2_priv *top_priv,
	void *cmd_args, uint32_t arg_size)
{
	struct cam_isp_hw_get_cmd_update  *cmd_update = cmd_args;
	...
	if (cmd_update->res->process_cmd)
		...
		CAM_ISP_HW_CMD_GET_FULL_REG_DUMP, cmd_args, arg_size);
	...
	void *cmd_args, uint32_t arg_size)
{
	...
	case CAM_ISP_HW_CMD_DUMP_HW:
		rc = cam_vfe_hw_dump(top_priv,
			cmd_args, arg_size);
		break;
	case CAM_ISP_HW_CMD_GET_FULL_REG_DUMP:
		rc = cam_vfe_full_reg_dump(top_priv,
			cmd_args, arg_size);
		break;
	default:
		rc = -EINVAL;
