...
static int cam_vfe_camif_get_full_reg_dump(
	struct cam_isp_resource_node  *camif_res,
	void *cmd_args, uint32_t arg_size)
{
	struct cam_vfe_mux_camif_data *camif_priv;
	...
	ife_base_phy = (void *)camif_priv->soc_info->mem_block[i]->start;
	ife_base = camif_priv->soc_info->reg_map[0].mem_base;

	CAM_INFO(CAM_ISP, "Phy ife base addr 0x%x", ife_base_phy);

	for (i = 0; i < 5248; i++) {
		addr = ife_base+(i*4);
		val  = cam_io_r_mb(addr);
		dump_ife1[local_val][i].addr = ife_base_phy + (i*4);
		dump_ife1[local_val][i].val  = val;
	}
	...
	uint32_t cmd_type, void *cmd_args, uint32_t arg_size)
{
	...
	case CAM_ISP_HW_CMD_GET_FULL_REG_DUMP:
		rc = cam_vfe_camif_get_full_reg_dump(rsrc_node, cmd_args,
			arg_size);
		break;
	default:
		CAM_ERR(CAM_ISP,
