	...
	enum cam_ife_csid_halt_mode      halt_mode)
{
	...
	if (res->res_id == CAM_IFE_PIX_PATH_RES_IPP) {
		...
		CAM_ERR(CAM_ISP, "Cant change halt mode for CSID:%d PPP path Res:%d ",
			csid_hw->hw_intf->hw_idx, res->res_id);
		goto end;
	}
	...
	void *stop_args, uint32_t arg_size)
{
	...
	csid_hw_info = (struct cam_hw_info  *)hw_priv;
	csid_hw = (struct cam_ife_csid_hw   *)csid_hw_info->core_info;
	CAM_INFO(CAM_ISP, "CSID:%d num_res %d",
		csid_hw->hw_intf->hw_idx,
		csid_stop->num_res);

	if (csid_stop->change_halt_mode) {
		...
		for (i = 0; i < csid_stop->num_res; i++) {
			...
			case CAM_ISP_RESOURCE_PIX_PATH:
				if (res->res_id == CAM_IFE_PIX_PATH_RES_IPP) {
					cam_ife_csid_change_pxl_halt_mode(csid_hw,
						res, csid_stop->halt_mode);
				}
				break;
			default:
				CAM_ERR(CAM_ISP, "CSID:%d Invalid res type%d",
