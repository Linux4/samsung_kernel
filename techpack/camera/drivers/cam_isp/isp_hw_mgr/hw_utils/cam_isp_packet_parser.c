...
struct cam_ife_mem_to_port req_io_cfg_data[CAM_ISP_MAX_REQ_DUMP];
atomic_t req_idx = ATOMIC_INIT(-1);
	...
	struct cam_isp_frame_header_info     *frame_header_info)
{
	...
	req_idx_local = atomic_inc_return(&req_idx);
	req_idx_local = req_idx_local % CAM_ISP_MAX_REQ_DUMP;
	atomic_set(&req_idx, req_idx_local);
	memset(&req_io_cfg_data[req_idx_local], 0,
			sizeof(req_io_cfg_data[req_idx_local]));
	req_io_cfg_data[req_idx_local].request =
		prepare->packet->header.request_id;
	get_monotonic_boottime(&req_io_cfg_data[req_idx_local].ts);
	...
	for (i = 0; i < prepare->packet->num_io_configs; i++) {
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX &&
			...
			if (j) {
				req_io_cfg_data[req_idx_local].is_dual = true;
			}

			req_io_cfg_data[req_idx_local].ife_idx = base_idx;
			...
			for (plane_id = 0; plane_id < CAM_PACKET_MAX_PLANES;
				...
				io_addr[plane_id] += io_cfg[i].offsets[plane_id];

				req_io_cfg_data[req_idx_local].ports[i].ife[j].planes[plane_id].io_addr
					= io_addr[plane_id];
				req_io_cfg_data[req_idx_local].ports[i].ife[j].planes[plane_id].size
					= size;
			...
			req_io_cfg_data[req_idx_local].ports[i].
				ife[j].num_planes = plane_id;
			req_io_cfg_data[req_idx_local].ports[i].
				ife[j].io_cfg     = io_cfg[i];
