...
int cam_register_subdev(struct cam_subdev *csd)
{
	...
	CAM_INFO(CAM_CRM, "Register device %s", csd->name);
	...
	if (sd->flags & V4L2_SUBDEV_FL_HAS_DEVNODE) {
		...
		CAM_INFO(CAM_CRM, "created node :%s", sd->entity.name);
	}
...
static int cam_req_mgr_component_master_bind(struct device *dev)
{
	...
	CAM_INFO(CAM_CRM, "Master bind called");
	...
	CAM_INFO(CAM_CRM, "initialize cam_req_ordered_sd_list");
...
static int cam_req_mgr_probe(struct platform_device *pdev)
{
	...
	CAM_INFO(CAM_CRM, "req_mgr_probe");
