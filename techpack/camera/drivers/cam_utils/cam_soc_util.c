...
int cam_soc_util_get_dt_properties(struct cam_hw_soc_info *soc_info)
{
	...
	rc = of_property_read_u32(of_node, "sub-type", &soc_info->sub_type);
	if (rc) {
		CAM_INFO(CAM_UTIL, "device %s failed to read sub-type",
			soc_info->dev_name);
		soc_info->sub_type = -1;
	}
	else
		CAM_INFO(CAM_UTIL, "device %s sub-type: %d", soc_info->dev_name, soc_info->sub_type);
