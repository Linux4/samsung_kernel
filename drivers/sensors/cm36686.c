...
static int proximity_store_cancelation(struct device *dev, bool do_calib)
{
	...
	pr_info("%s: prox_cal = %d, ps_high_thresh = %d, ps_low_thresh = %d\n",
		__func__,
		ps_reg_init_setting[PS_CANCEL][CMD],
		ps_reg_init_setting[PS_THD_HIGH][CMD],
		ps_reg_init_setting[PS_THD_LOW][CMD]);
...
static int cm36686_leden_gpio_onoff(struct cm36686_data *cm36686, bool onoff)
{
	...
	if(!onoff && (cm36686->power_state & LIGHT_ENABLED || cm36686->power_state & PROXIMITY_ENABLED))
	{
...
static void cm36686_work_func_light(struct work_struct *work)
{
	...
	input_report_rel(cm36686->light_input_dev, REL_X, time_lo);
	...
	struct cm36686_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np, "cm36686,cal_skip", &pdata->cal_skip);
	if (ret < 0) {
		pr_err("%s - Cannot set cal_skip\n", __func__);
		pdata->cal_skip = CAL_SKIP_ADC;
	}

	ret = of_property_read_u32(np, "cm36686,cal_fail", &pdata->cal_fail);
	if (ret < 0) {
		pr_err("%s - Cannot set cal_fail\n", __func__);
		pdata->cal_fail = CAL_FAIL_ADC;
	}
