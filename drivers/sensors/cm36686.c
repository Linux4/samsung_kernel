...
#if defined(CONFIG_SENSORS_CM36686_USE_DUMMY_INITIAL)
#define DUMMY_INITIAL_VAL	3
#endif
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (new_value && !(cm36686->power_state & PROXIMITY_ENABLED)) {
		...
		input_report_abs(cm36686->proximity_input_dev,
			ABS_DISTANCE, DUMMY_INITIAL_VAL);
#endif
		input_report_abs(cm36686->proximity_input_dev,
			ABS_DISTANCE, 1);
...
#if defined(CONFIG_SENSORS_CM36686_USE_DUMMY_INITIAL)
	input_set_abs_params(cm36686->proximity_input_dev, ABS_DISTANCE, 0,
		DUMMY_INITIAL_VAL, 0, 0);
#else
