	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (new_value && !(cm36686->power_state & LIGHT_ENABLED)) {
		...
		if (cm36686->pdata->vdd_always_on == 0 && cm36686->power_state == 0)
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (new_value && !(cm36686->power_state & PROXIMITY_ENABLED)) {
		...
		if (cm36686->pdata->vdd_always_on == 0 && cm36686->power_state == 0)
			proximity_vdd_onoff(dev, ON);
		if(cm36686->pdata->vled_same_vdd == 0)
	...
	struct cm36686_platform_data *pdata)
{
	...
	pdata->vled_ldo = of_get_named_gpio_flags(np, "cm36686,vled_ldo",
		0, &flags);
	if (pdata->vled_ldo < 0) {
		SENSOR_INFO("can't get vled_ldo: means to use regulator as vLED\n");
		pdata->vled_ldo = 0;
	} else {
	...
	const struct i2c_device_id *id)
{
	...
	if (cm36686->pdata->vdd_always_on == 0)
