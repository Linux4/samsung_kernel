...
static void cm36686_work_func_light(struct work_struct *work)
{
	...
	input_report_rel(cm36686->light_input_dev,
		REL_DIAL, cm36686->als_data + 1);
	input_report_rel(cm36686->light_input_dev,
		REL_WHEEL, cm36686->white_data + 1);
				...
				const struct i2c_device_id *id)
{
	...
	input_set_capability(cm36686->proximity_input_dev, EV_ABS,
		ABS_DISTANCE);
	input_set_abs_params(cm36686->proximity_input_dev,
	...
	if (ret) {
		pr_err("%s: could not create proximity_dev ret is %d\n",
			__func__, ret);
	...
	if (ret) {
		pr_err("%s: could not create light_dev ret is %d\n",
			__func__, ret);
