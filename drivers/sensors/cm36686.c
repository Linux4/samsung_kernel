...
#if defined(CONFIG_SENSORS_PROXIMITY_FOR_TEST)
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (trim_value > -1) {
		cm36686->pdata->default_trim = trim_value;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cm36686_i2c_write_word(cm36686, regist, val);
...
{ 
	...
	for (i = 0; i < 10; i++) {
		cm36686_i2c_read_word(cm36686, i, &val[i]);
