...
#define BMA254_DEFAULT_DELAY            200000000LL
...
#define BMA254_TOP_UPPER_RIGHT          0
#define BMA254_TOP_LOWER_RIGHT          1
#define BMA254_TOP_LOWER_LEFT           2
#define BMA254_TOP_UPPER_LEFT           3
#define BMA254_BOTTOM_UPPER_RIGHT       4
#define BMA254_BOTTOM_LOWER_RIGHT       5
#define BMA254_BOTTOM_LOWER_LEFT        6
#define BMA254_BOTTOM_UPPER_LEFT        7
...
struct bma254_p {
	...
	struct bma254_v accdata;
	struct bma254_v caldata;
...
static int bma254_open_calibration(struct bma254_p *);

static int bma254_i2c_recovery(struct bma254_p *data)
{
	int ret, i;
...
static int bma254_i2c_read(struct bma254_p *data,
...
static int bma254_i2c_write(struct bma254_p *data,
...
static int bma254_set_mode(struct bma254_p *data, unsigned char mode)
{
	int ret = 0;
	...
	ret = bma254_i2c_read(data, BMA254_MODE_CTRL_REG, &buf1, 1);
	ret += bma254_i2c_read(data, BMA254_LOW_NOISE_CTRL_REG, &buf2, 1);

	switch (mode) {
	case BMA254_MODE_NORMAL:
		buf1  = BMA254_SET_BITSLICE(buf1, BMA254_MODE_CTRL, 0);
		buf2  = BMA254_SET_BITSLICE(buf2, BMA254_LOW_POWER_MODE, 0);
		ret += bma254_i2c_write(data, BMA254_MODE_CTRL_REG, buf1);
		mdelay(1);
		ret += bma254_i2c_write(data, BMA254_LOW_NOISE_CTRL_REG, buf2);
		break;
	case BMA254_MODE_LOWPOWER1:
		buf1  = BMA254_SET_BITSLICE(buf1, BMA254_MODE_CTRL, 2);
		buf2  = BMA254_SET_BITSLICE(buf2, BMA254_LOW_POWER_MODE, 0);
		ret += bma254_i2c_write(data, BMA254_MODE_CTRL_REG, buf1);
		mdelay(1);
		ret += bma254_i2c_write(data, BMA254_LOW_NOISE_CTRL_REG, buf2);
	...
	case BMA254_MODE_SUSPEND:
		buf1  = BMA254_SET_BITSLICE(buf1, BMA254_MODE_CTRL, 4);
		buf2  = BMA254_SET_BITSLICE(buf2, BMA254_LOW_POWER_MODE, 0);
		ret += bma254_i2c_write(data, BMA254_LOW_NOISE_CTRL_REG, buf2);
		mdelay(1);
		ret += bma254_i2c_write(data, BMA254_MODE_CTRL_REG, buf1);
		break;
	case BMA254_MODE_DEEP_SUSPEND:
		buf1  = BMA254_SET_BITSLICE(buf1, BMA254_MODE_CTRL, 1);
		buf2  = BMA254_SET_BITSLICE(buf2, BMA254_LOW_POWER_MODE, 1);
		ret += bma254_i2c_write(data, BMA254_MODE_CTRL_REG, buf1);
		mdelay(1);
		ret += bma254_i2c_write(data, BMA254_LOW_NOISE_CTRL_REG, buf2);
		break;
...
static int bma254_set_range(struct bma254_p *data, unsigned char range)
{
	int ret = 0 ;
	unsigned char buf;

	ret = bma254_i2c_read(data, BMA254_RANGE_SEL_REG, &buf, 1);

	switch (range) {
	case BMA254_RANGE_2G:
		buf = BMA254_SET_BITSLICE(buf, BMA254_RANGE_SEL, 3);
		break;
	case BMA254_RANGE_4G:
		buf = BMA254_SET_BITSLICE(buf, BMA254_RANGE_SEL, 5);
		break;
	case BMA254_RANGE_8G:
		buf = BMA254_SET_BITSLICE(buf, BMA254_RANGE_SEL, 8);
		break;
	case BMA254_RANGE_16G:
		buf = BMA254_SET_BITSLICE(buf, BMA254_RANGE_SEL, 12);
		break;
	default:
		buf = BMA254_SET_BITSLICE(buf, BMA254_RANGE_SEL, 3);
		break;
	...
	ret += bma254_i2c_write(data, BMA254_RANGE_SEL_REG, buf);

	return ret;
}

static int bma254_set_bandwidth(struct bma254_p *data,
		unsigned char bandwidth)
{
	...
	if (bandwidth <= 7 || bandwidth >= 16)
		bandwidth = BMA254_BW_250HZ;

	ret = bma254_i2c_read(data, BMA254_BANDWIDTH__REG, &buf, 1);
	buf = BMA254_SET_BITSLICE(buf, BMA254_BANDWIDTH, bandwidth);
	ret += bma254_i2c_write(data, BMA254_BANDWIDTH__REG, buf);

	pr_info("[SENSOR]: %s - change bandwidth %u\n", __func__, bandwidth);
	return ret;
}

static int bma254_read_accel_xyz(struct bma254_p *data,	struct bma254_v *acc)
{
	int ret = 0;
	unsigned char buf[READ_DATA_LENTH];

	ret = bma254_i2c_read(data,	BMA254_ACC_X12_LSB__REG, buf, READ_DATA_LENTH);
	if (ret < 0)
		goto exit;

	acc->x = BMA254_GET_BITSLICE(buf[0], BMA254_ACC_X12_LSB) |
			(BMA254_GET_BITSLICE(buf[1], BMA254_ACC_X_MSB) <<
			(BMA254_ACC_X12_LSB__LEN));
	acc->x = acc->x << (sizeof(short) * 8 - (BMA254_ACC_X12_LSB__LEN +
			BMA254_ACC_X_MSB__LEN));
	acc->x = acc->x >> (sizeof(short) * 8 - (BMA254_ACC_X12_LSB__LEN +
			BMA254_ACC_X_MSB__LEN));

	acc->y = BMA254_GET_BITSLICE(buf[2], BMA254_ACC_Y12_LSB) |
			(BMA254_GET_BITSLICE(buf[3],
			BMA254_ACC_Y_MSB) << (BMA254_ACC_Y12_LSB__LEN));
	acc->y = acc->y << (sizeof(short) * 8 - (BMA254_ACC_Y12_LSB__LEN +
			BMA254_ACC_Y_MSB__LEN));
	acc->y = acc->y >> (sizeof(short) * 8 - (BMA254_ACC_Y12_LSB__LEN +
			BMA254_ACC_Y_MSB__LEN));

	acc->z = BMA254_GET_BITSLICE(buf[4], BMA254_ACC_Z12_LSB) |
			(BMA254_GET_BITSLICE(buf[5],
			BMA254_ACC_Z_MSB) << (BMA254_ACC_Z12_LSB__LEN));
	acc->z = acc->z << (sizeof(short) * 8 - (BMA254_ACC_Z12_LSB__LEN +
...
static enum hrtimer_restart bma254_timer_func(struct hrtimer *timer)
{
	struct bma254_p *data = container_of(timer,
					struct bma254_p, accel_timer);
...
static void bma254_work_func(struct work_struct *work)
{
	int ret;
	struct bma254_v acc;
	struct bma254_p *data = container_of(work, struct bma254_p, work);

	ret = bma254_read_accel_xyz(data, &acc);
	if (ret < 0)
		goto exit;
...
static void bma254_set_enable(struct bma254_p *data, int enable)
{
	if (enable == ON) {
...
{
	struct bma254_p *data = dev_get_drvdata(dev);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (enable) {
		if (pre_enable == OFF) {
			bma254_open_calibration(data);
			bma254_set_mode(data, BMA254_MODE_NORMAL);
			atomic_set(&data->enable, ON);
			bma254_set_enable(data, ON);
		}
	} else {
		if (pre_enable == ON) {
			atomic_set(&data->enable, OFF);
			if (data->recog_flag == ON)
				bma254_set_mode(data, BMA254_MODE_LOWPOWER1);
			else
				bma254_set_mode(data, BMA254_MODE_SUSPEND);

			bma254_set_enable(data, OFF);
		}
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (delay <= 5000000LL)
		bma254_set_bandwidth(data, BMA254_BW_125HZ);
	else if (delay <= 10000000LL)
		bma254_set_bandwidth(data, BMA254_BW_62_50HZ);
	else if (delay <= 20000000LL)
		bma254_set_bandwidth(data, BMA254_BW_31_25HZ);
	else
		bma254_set_bandwidth(data, BMA254_BW_7_81HZ);
...
static int bma254_open_calibration(struct bma254_p *data)
{
	int ret = 0;
	mm_segment_t old_fs;
...
static int bma254_do_calibrate(struct bma254_p *data, int enable)
{
	int sum[3] = { 0, };
	int ret = 0, cnt;
	struct file *cal_filp = NULL;
	...
	if (enable) {
		...
		for (cnt = 0; cnt < CALIBRATION_DATA_AMOUNT; cnt++) {
			bma254_read_accel_xyz(data, &acc);
			sum[0] += acc.x;
			sum[1] += acc.y;
			sum[2] += acc.z;
			mdelay(10);
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = bma254_open_calibration(data);
	if (ret < 0)
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = bma254_do_calibrate(data, (int)dEnable);
	if (ret < 0)
...
static ssize_t bma254_raw_data_read(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct bma254_v acc;
	struct bma254_p *data = dev_get_drvdata(dev);
...
static void bma254_set_int_enable(struct bma254_p *data,
		unsigned char InterruptType , unsigned char value)
{
	unsigned char reg;

	bma254_i2c_read(data, BMA254_INT_ENABLE1_REG, &reg, 1);

	switch (InterruptType) {
	case SLOPE_X_INT:
		/* Slope X Interrupt */
		reg = BMA254_SET_BITSLICE(reg, BMA254_EN_SLOPE_X_INT, value);
		break;
	case SLOPE_Y_INT:
		/* Slope Y Interrupt */
		reg = BMA254_SET_BITSLICE(reg, BMA254_EN_SLOPE_Y_INT, value);
		break;
	case SLOPE_Z_INT:
		/* Slope Z Interrupt */
		reg = BMA254_SET_BITSLICE(reg, BMA254_EN_SLOPE_Z_INT, value);
		break;
	default:
		break;
	}

	bma254_i2c_write(data, BMA254_INT_ENABLE1_REG, reg);
}

static void bma254_slope_enable(struct bma254_p *data,
		int enable, int factory_mode)
{
	unsigned char reg;

	if (enable == ON) {
		bma254_i2c_read(data, BMA254_EN_INT1_PAD_SLOPE__REG, &reg, 1);
		reg = BMA254_SET_BITSLICE(reg, BMA254_EN_INT1_PAD_SLOPE, ON);
		bma254_i2c_write(data, BMA254_EN_INT1_PAD_SLOPE__REG, reg);

		bma254_i2c_read(data, BMA254_INT_MODE_SEL__REG, &reg, 1);
		reg = BMA254_SET_BITSLICE(reg, BMA254_INT_MODE_SEL, 0x01);
		bma254_i2c_write(data, BMA254_INT_MODE_SEL__REG, reg);

		if (factory_mode == OFF) {
			bma254_i2c_read(data, BMA254_SLOPE_DUR__REG, &reg, 1);
			reg = BMA254_SET_BITSLICE(reg, BMA254_SLOPE_DUR,
					SLOPE_DURATION_VALUE);
			bma254_i2c_write(data, BMA254_SLOPE_DUR__REG, reg);

			reg = SLOPE_THRESHOLD_VALUE;
			bma254_i2c_write(data, BMA254_SLOPE_THRES__REG, reg);

			bma254_set_int_enable(data, SLOPE_X_INT, ON);
			bma254_set_int_enable(data, SLOPE_Y_INT, ON);
			bma254_set_int_enable(data, SLOPE_Z_INT, ON);
		} else {
			bma254_i2c_read(data, BMA254_SLOPE_DUR__REG, &reg, 1);
			reg = BMA254_SET_BITSLICE(reg, BMA254_SLOPE_DUR, 0x01);
			bma254_i2c_write(data, BMA254_SLOPE_DUR__REG, reg);
			...
			bma254_set_int_enable(data, SLOPE_Z_INT, ON);
			bma254_set_bandwidth(data, BMA254_BW_250HZ);
		}
	} else if (enable == OFF) {
		bma254_i2c_read(data, BMA254_EN_INT1_PAD_SLOPE__REG, &reg, 1);
		reg = BMA254_SET_BITSLICE(reg, BMA254_EN_INT1_PAD_SLOPE, OFF);
		bma254_i2c_write(data, BMA254_EN_INT1_PAD_SLOPE__REG, reg);

		bma254_set_int_enable(data, SLOPE_X_INT, OFF);
		bma254_set_int_enable(data, SLOPE_Y_INT, OFF);
		bma254_set_int_enable(data, SLOPE_Z_INT, OFF);
	}
}

static ssize_t bma254_reactive_alert_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ((enable == ON) && (data->recog_flag == OFF)) {
		...
		bma254_slope_enable(data, ON, factory_mode);
		...
		bma254_slope_enable(data, OFF, factory_mode);
...
static ssize_t bma254_reactive_alert_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct bma254_p *data = dev_get_drvdata(dev);
							...
							size_t count)
{
	...
	if(value==1){
		err = bma254_i2c_write(data, BMA254_BW_SEL_REG, BMA254_BW_31_25HZ);
		if(err){
			pr_err("%s: Can't set bandwdith LPF = 1", __func__);
			goto done;
		}else{
			data->lowpassfilter=1;
		}
	}else if(value==0){
		err = bma254_i2c_write(data, BMA254_BW_SEL_REG, BMA254_BW_1000HZ);
		if(err){
			pr_err("%s: Can't set bandwdith LPF = 0\n", __func__);
			goto done;
		}else{
			data->lowpassfilter=0;
		}
	}
done:
	return count;
...
static DEVICE_ATTR(name, S_IRUGO, bma254_name_show, NULL);
static DEVICE_ATTR(vendor, S_IRUGO, bma254_vendor_show, NULL);
static DEVICE_ATTR(calibration, S_IRUGO | S_IWUSR | S_IWGRP,
	bma254_calibration_show, bma254_calibration_store);
static DEVICE_ATTR(raw_data, S_IRUGO, bma254_raw_data_read, NULL);
static DEVICE_ATTR(reactive_alert, S_IRUGO | S_IWUSR | S_IWGRP,
	bma254_reactive_alert_show, bma254_reactive_alert_store);
...
static void bma254_irq_work_func(struct work_struct *work)
{
	struct bma254_p *data = container_of((struct delayed_work *)work,
		struct bma254_p, irq_work);
...
static irqreturn_t bma254_irq_thread(int irq, void *bma254_data_p)
{
	struct bma254_p *data = bma254_data_p;
...
static int bma254_setup_pin(struct bma254_p *data)
{
	int ret;
...
static int bma254_input_init(struct bma254_p *data)
{
	int ret = 0;
	...
	/* sysfs node creation */
	ret = sysfs_create_group(&dev->dev.kobj, &bma254_attribute_group);
	if (ret < 0) {
...
static int bma254_parse_dt(struct bma254_p *data, struct device *dev)
{
...
{
	int ret;
	struct regulator *sensor_vdd, *sensor_vddio;

	sensor_vdd = devm_regulator_get(dev, "bma254-i2c,vdd");
	if (IS_ERR(sensor_vdd)) {
		pr_err("[SENSOR]: %s - cannot get vdd\n", __func__);
		return -ENOMEM;
	}

	sensor_vddio = devm_regulator_get(dev, "bma254-i2c,vio");
	if (IS_ERR(sensor_vddio)) {
		pr_err("[SENSOR]: %s - cannot get vio\n", __func__);
		devm_regulator_put(sensor_vdd);
		return -ENOMEM;
	}

	if (onoff) {
		ret = regulator_enable(sensor_vdd);
		if (ret)
			pr_err("[SENSOR]: %s - Failed to enable vdd.\n",
				__func__);

		ret = regulator_enable(sensor_vddio);
		if (ret)
			pr_err("[SENSOR]: %s - Failed to enable vdd-io.\n",
				__func__);

	} else {
		ret = regulator_disable(sensor_vdd);
		if (ret)
			pr_err("[SENSOR]: %s - Failed to disable vdd.\n",
				__func__);
		ret = regulator_disable(sensor_vddio);
		if (ret)
			pr_err("[SENSOR]: %s - Failed to disable vdd-io.\n",
				__func__);
	}

	devm_regulator_put(sensor_vdd);
	devm_regulator_put(sensor_vddio);
	mdelay(5);
		...
		const struct i2c_device_id *id)
{
	...
	data = kzalloc(sizeof(struct bma254_p), GFP_KERNEL);
	if (data == NULL) {
	...
	ret = bma254_parse_dt(data, &client->dev);
	if (ret < 0) {
	...
	for (i = 0; i < CHIP_ID_RETRIES; i++) {
		ret = i2c_smbus_read_word_data(client, BMA254_CHIP_ID_REG);
		if ((ret & 0x00ff) != BMA254_CHIP_ID) {
	...
	data->poll_delay = ns_to_ktime(BMA254_DEFAULT_DELAY);
	data->accel_timer.function = bma254_timer_func;
	...
	INIT_WORK(&data->work, bma254_work_func);
	INIT_DELAYED_WORK(&data->irq_work, bma254_irq_work_func);

	data->irq1 = gpio_to_irq(data->acc_int1);

	ret = request_threaded_irq(data->irq1, NULL, bma254_irq_thread,
		IRQF_TRIGGER_RISING | IRQF_ONESHOT, "bma254_accel", data);
	if (ret < 0) {
	...
	bma254_set_bandwidth(data, BMA254_BW_125HZ);
	bma254_set_range(data, BMA254_RANGE_4G);
	bma254_set_mode(data, BMA254_MODE_SUSPEND);
...
static void bma254_shutdown(struct i2c_client *client)
{
	struct bma254_p *data = (struct bma254_p *)i2c_get_clientdata(client);
...
static struct of_device_id bma254_match_table[] = {
	{ .compatible = "bma254-i2c",},
	{},
};

static const struct i2c_device_id bma254_id[] = {
	{ "bma254_match_table", 0 },
	{ }
};

static const struct dev_pm_ops bma254_pm_ops = {
...
MODULE_DESCRIPTION("bma254 accelerometer sensor driver");
