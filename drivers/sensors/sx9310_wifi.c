...
static void sx9310_initialize_chip(struct sx9310_p *data)
{
	...
	if (cnt >= 10)
		pr_err("[SX9310_WIFI]: %s - reset fail(%d)\n", __func__, cnt);
...
static void sx9310_get_data(struct sx9310_p *data)
{
	...
	pr_info("[SX9310_WIFI]: %s - Capmain: %d, Useful: %d, avg: %d, diff: %d, Offset: %u\n",
...
static void sx9310_set_enable(struct sx9310_p *data, int enable)
{
	...
	if (enable == ON) {
		...
		pr_info("[SX9310_WIFI]: %s - enable(status : 0x%x)\n", __func__, status);
		data->diff_avg = 0;
		data->diff_cnt = 0;
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_info("[SX9310_WIFI]: %s - new_value = %u old_value = %d\n",
...
static void sx9310_touch_process(struct sx9310_p *data, u8 flag)
{
	...
	sx9310_i2c_read(data, SX9310_STAT0_REG, &status);
	pr_info("[SX9310_WIFI]: %s - (status: 0x%x)\n", __func__, status);
	sx9310_get_data(data);
...
static void sx9310_debug_work_func(struct work_struct *work)
{
	...
	if (check_ta_state() > 1) {
		...
		data->check_usb = false;
		sx9310_set_offset_calibration(data);
		pr_info("[SX9310_WIFI]: %s - TA is removed\n", __func__);
	}
...
static int sx9310_setup_pin(struct sx9310_p *data)
{
	...
	if (ret < 0) {
		pr_err("[SX9310_WIFI]: %s - failed to set gpio %d(%d)\n",
			__func__, data->gpio_nirq, ret);
		...
		const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		pr_err("[SX9310_WIFI]: %s - reset failed %d\n", __func__, ret);
	...
	if (ret < 0) {
		pr_err("[SX9310_WIFI]: %s - failed to set request irq %d(%d)\n",
	...
	if (ret) {
		pr_err("[SX9310_WIFI] %s - cound not register sensor(%d).\n",
			__func__, ret);
