...
static int sx9360_check_hallic_state(char *file_path, char hall_ic_status[])
{
	...
	if (IS_ERR(filep)) {
		iRet = PTR_ERR(filep);
		pr_err("[SX9360_WIFI]: %s - file open fail %d\n", __func__, iRet);
		set_fs(old_fs);
		return iRet;
	}
	...
	if (iRet <= 0) {
		pr_err("[SX9360_WIFI]: %s - file read fail %d\n", __func__, iRet);
...
static int sx9360_i2c_write(struct sx9360_p *data, u8 reg_addr, u8 buf)
{
	...
	if (ret < 0)
		pr_err("[SX9360_WIFI]: %s - i2c write error %d\n", __func__, ret);
...
static int sx9360_i2c_read(struct sx9360_p *data, u8 reg_addr, u8 *buf)
{
	...
	if (ret < 0)
		pr_err("[SX9360_WIFI]: %s - i2c read error %d\n", __func__, ret);
...
static void sx9360_initialize_register(struct sx9360_p *data)
{
	...
	for (idx = 0; idx < (sizeof(setup_reg) >> 1); idx++) {		
		...
		pr_info("[SX9360_WIFI]: %s - Write Reg: 0x%x Value: 0x%x\n",
...
static void sx9360_initialize_chip(struct sx9360_p *data)
{
	...
	if (cnt >= 10)
		pr_err("[SX9360_WIFI]: %s - s/w reset fail(%d)\n", __func__, cnt);
...
static void sx9360_send_event(struct sx9360_p *data, u8 state)
{
	if (data->skip_data == true) {
		pr_info("[SX9360_WIFI]: %s - skip grip event\n", __func__);
		return;
	}

	if (state == ACTIVE) {
		data->state = ACTIVE;
		pr_info("[SX9360_WIFI]: %s - touched\n", __func__);
	} else {
		data->state = IDLE;
		pr_info("[SX9360_WIFI]: %s - released\n", __func__);
	}
...
static void sx9360_display_data_reg(struct sx9360_p *data)
{
	...
	pr_info("[SX9360_WIFI]: ############# %d reference #############\n", 0);
...
static void sx9360_get_data(struct sx9360_p *data)
{
	...
	pr_info("[SX9360_WIFI]: %s - capMain: %ld, useful: %ld, avg: %d, diff: %d, Offset: %u\n",
...
static void sx9360_check_status(struct sx9360_p *data)
{
	...
	pr_info("[SX9360_WIFI]: %s - (status: 0x%x)\n", __func__, status);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtoul(buf, 10, &val)) {
		pr_err("[SX9360_WIFI]: %s - Invalid Argument\n", __func__);
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret < 0) {
		pr_err("[SX9360_WIFI]: %s - kstrtou8 failed.(%d)\n", __func__, ret);
		return count;
	}
	...
	if (onoff == 0) {
		...
		pr_err("[SX9360_WIFI]: %s - unknown value %d\n", __func__, onoff);
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_info("[SX9360_WIFI]: %s - new_value = %u\n", __func__, enable);
...
static void sx9360_touch_process(struct sx9360_p *data)
{
	...
	if (data->state == IDLE) {
		if (status & CSX_STATUS_REG)
			...
			pr_info("[SX9360_WIFI]: %s - 0x%x already released.\n",
				__func__, status);
	} else { /* User released button */
		if (!(status & CSX_STATUS_REG)) {
			sx9360_send_event(data, IDLE);
		} else {
			pr_info("[SX9360_WIFI]: %s - 0x%x still touched\n",
				__func__, status);
		}
	}
}
...
static void sx9360_process_interrupt(struct sx9360_p *data)
{
	...
	pr_info("[SX9360_WIFI]: %s - status %d\n", __func__, status);
...
static void sx9360_irq_work_func(struct work_struct *work)
{
	...
	if (sx9360_get_nirq_state(data) == 0)
		...
		pr_err("[SX9360_WIFI]: %s - nirq read high %d\n",
...
static void sx9360_debug_work_func(struct work_struct *work)
{
	...
	if (data->hallic_detect) {
		...
		if (strcmp(data->hall_ic, str) == 0) {
			if (hall_flag) {
				pr_info("[SX9360_WIFI]: %s - hall IC is closed\n", __func__);
				sx9360_set_offset_calibration(data);
				hall_flag = 0;
			}
		} else {
	...
	if (data->hallic_cert_detect) {
		...
		if (strcmp(data->hall_ic, str) == 0) {
			if (hall_cert_flag) {
				pr_info("[SX9360_WIFI]: %s - cert hall IC is closed\n", __func__);
				sx9360_set_offset_calibration(data);
				hall_cert_flag = 0;
			}
		} else {
...
static int sx9360_setup_pin(struct sx9360_p *data)
{
	...
	if (ret < 0) {
		pr_err("[SX9360_WIFI]: %s - gpio %d request failed (%d)\n",
	...
	if (ret < 0) {
		pr_err("[SX9360_WIFI]: %s - failed to set gpio %d as input (%d)\n",
			__func__, data->gpio_nirq, ret);
...
static int sx9360_parse_dt(struct sx9360_p *data, struct device *dev)
{
	...
	data->gpio_nirq = of_get_named_gpio_flags(dNode,
		"sx9360_wifi,nirq-gpio", 0, &flags);
	if (data->gpio_nirq < 0) {
		pr_err("[SENSOR]: %s - get gpio_nirq error\n", __func__);
		return -ENODEV;
	}

	if (!sx9360_read_setupreg(dNode, "sx9360_wifi,reggnrlctrl2", &reggnrlctrl2))
		setup_reg[SX9360_GNRLCTRL_REG_IDX + 2].val = reggnrlctrl2;
	if (!sx9360_read_setupreg(dNode, "sx9360_wifi,regafeparam1phm", &regafeparam1phm))
	...
	if (!sx9360_read_setupreg(dNode, "sx9360_wifi,regproxctrl4", &regproxctrl4))
		setup_reg[SX9360_PROXCTRL_REG_IDX + 5].val = regproxctrl4;
	if (!sx9360_read_setupreg(dNode, "sx9360_wifi,regproxctrl3", &regproxctrl3))
	...
	ret = of_property_read_u32(dNode, "sx9360_wifi,hallic_cert_detect", &data->hallic_cert_detect);
	if (ret < 0)
		...
		unsigned long action, void *data)
{
	...
	struct sx9360_p *pdata =
		container_of(nb, struct sx9360_p, cpuidle_ccic_nb);
	...
	if (pdata->init_done == ON) {
		...
		case ATTACHED_DEV_JIG_USB_ON_MUIC:
			pr_info("[SX9360_WIFI]: %s skip cable = %d, attach = %d\n",
				__func__, usb_typec_info.cable_type, usb_typec_info.attach);
			break;
		default:
			pr_info("[SX9360_WIFI]: %s accept cable = %d, attach = %d\n",
				__func__, usb_typec_info.cable_type, usb_typec_info.attach);
...
static int sx9360_check_chip_id(struct sx9360_p *data)
{
	...
	if (ret < 0) {
		pr_err("[SX9360_WIFI]: whoami[0x%x] read failed %d\n", value, ret);
		return ret;
	}
	if(value != WHO_AM_I)
	{
		pr_err("[SX9360_WIFI]: invalid whoami(%x)\n", value);
		return -1;
	}
		...
		const struct i2c_device_id *id)
{
	...
	pr_info("[SX9360_WIFI]: %s - Probe Start!\n", __func__);
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		pr_err("[SX9360_WIFI]: %s - i2c_check_functionality error\n",
			__func__);
		goto exit;
	}
	...
	if (data == NULL) {
		pr_err("[SX9360_WIFI]: %s - kzalloc error\n", __func__);
		ret = -ENOMEM;
		goto exit_kzalloc;
	}
	...
	if (ret < 0) {
		pr_err("[SX9360_WIFI]: %s - of_node error\n", __func__);
		ret = -ENODEV;
		goto exit_of_node;
	}
	...
	if (ret) {
		pr_err("[SX9360_WIFI]: %s - could not setup pin\n", __func__);
		goto exit_setup_pin;
	}
	...
	if (ret < 0) {
		pr_err("[SX9360_WIFI]: %s - chip id check failed %d\n", __func__, ret);
		goto exit_chip_reset;
	}

	ret = sx9360_i2c_write(data, SX9360_SOFTRESET_REG, SX9360_SOFTRESET);
	if (ret < 0) {
		pr_err("[SX9360_WIFI]: %s - chip reset failed %d\n", __func__, ret);
		goto exit_chip_reset;
	}
	...
	ret = request_threaded_irq(data->irq, NULL, sx9360_interrupt_thread,
			...
			"sx9360_irq", data);
	if (ret < 0) {
		pr_err("[SX9360_WIFI]: %s - failed to set request_threaded_irq %d"
	...
	if (ret) {
		pr_err("[SX9360_WIFI] %s - cound not register sensor(%d).\n",
			__func__, ret);
	...
	pr_info("[SX9360_WIFI]: %s - Probe done!\n", __func__);
...
exit_kzalloc:
exit:
	pr_err("[SX9360_WIFI]: %s - Probe fail!\n", __func__);
	return ret;
}
