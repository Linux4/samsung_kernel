...
#ifdef CONFIG_MACH_J75LTE_KOR_KTT
#define CAL_SKIP_ADC	9 //(nondetect threshold *60% :  9)
#define CAL_FAIL_ADC	19 //(detect threshold : 19)
#else
#define CAL_SKIP_ADC	8 //(nondetect threshold *60% :  9)
#define CAL_FAIL_ADC	18 //(detect threshold : 19)
#endif
...
#ifndef CONFIG_MACH_J75LTE_KOR_KTT
...
static void cm36672p_pmic_leda_onoff(struct cm36672p_data *info, int onoff)
{
	...
	if (info->leden_3p0 == NULL) {
		...
		if (IS_ERR(info->leden_3p0)) {
			SENSOR_ERR("regulator_get failed for PROX_LEDA_3.0V\n");
				return;
		}
	}
	...
	if (onoff) {
		...
		if (ret)
			SENSOR_ERR("leda_3p3 enable failed (%d)\n", ret);
	} else {
		ret = regulator_disable(info->leden_3p0);
		if (ret)
			SENSOR_ERR("leden_3p0 disable failed (%d)\n", ret);
	}
	msleep(20);
	return;
}
