...
#define CAL_SKIP_ADC	57
#define CAL_FAIL_ADC	125
#elif defined(CONFIG_MACH_J3XPROLTE_CHN_CTC) || defined(CONFIG_MACH_J3XPROLTE_CHN_OPEN)
#define CAL_SKIP_ADC	15
#define CAL_FAIL_ADC	33
#else
...
#if defined(CONFIG_SENSORS_LEDA_EN_GPIO)
#if defined(CONFIG_SENSORS_CM36672P_RFGPIO)
		ret = gpio_direction_output(data->pdata->leden_gpio, 1);
		if (ret)
			pr_err("%s: unable to set_direction for prox_en [%d]\n",
				__func__, data->pdata->leden_gpio);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (trim_value > -1) {
		...
		err = cm36672p_i2c_write_word(data, REG_PS_CANC,
			ps_reg_init_setting[PS_CANCEL][CMD]);
		if (err < 0)
			SENSOR_ERR("cm36672_ps_canc is failed. %d\n", err);
