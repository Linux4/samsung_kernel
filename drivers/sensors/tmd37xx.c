...
#define CAL_SKIP_ADC	56
#define CAL_FAIL_ADC	100
...
#ifdef CONFIG_SENSORS_TMD3700
	hrtimer_start(&taos->timer, ns_to_ktime(Atime_ms*100000), HRTIMER_MODE_REL);
#endif
}
...
static void taos_work_func_light(struct work_struct *work)
{
	...
	if(taos->light_first_lux) {
		SENSOR_INFO("first-lux:%d, cct:%d\n",adc, cct);
		taos->light_first_lux = false;
	}
}
