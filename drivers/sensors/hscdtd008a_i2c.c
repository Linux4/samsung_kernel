...
struct hscd_power_data {
	...
	struct regulator *regulator_magi2c;
};
...
static int hscd_power_on(int onoff)
{
	...
	if (onoff == 1) {
		...
		if (hscd_power.regulator_magi2c) {
			err = regulator_enable(hscd_power.regulator_magi2c);
			if (err) {
				alps_errmsg("Couldn't enable magi2c %d\n", err);
				return err;
			}
		}
	} else if (onoff == 0) {
		...
		if (hscd_power.regulator_magi2c)
			regulator_disable(hscd_power.regulator_magi2c);
	}
	msleep(60);
	return err;
}
...
static DEVICE_ATTR(selftest, S_IRUGO | S_IRUSR | S_IRGRP, selftest_show, NULL);
static DEVICE_ATTR(status, S_IRUGO | S_IRUSR | S_IRGRP, status_show, NULL);
static DEVICE_ATTR(adc, S_IRUGO | S_IRUSR | S_IRGRP, adc_show, NULL);
static DEVICE_ATTR(raw_data, S_IRUGO | S_IRUSR | S_IRGRP, mag_raw_data_read,
	NULL);
static DEVICE_ATTR(name, S_IRUGO | S_IRUSR | S_IRGRP, name_show, NULL);
static DEVICE_ATTR(vendor, S_IRUGO | S_IRUSR | S_IRGRP, vendor_show, NULL);
...
static int hscd_parse_dt(struct device *dev, struct hscd_platform_data *pdata)
{
	...
	pdata->reg_magi2c = of_get_property(np, "HSCDTD008-i2c,reg_magi2c",
		NULL);
	return 0;
}
#else
...
static int hscd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	hscd_power.regulator_magi2c = NULL;
	hscd_power.regulator_vdd = regulator_get(&client->dev,
		platform_data->reg_vdd);
	...
	hscd_power.regulator_vio = regulator_get(&client->dev,
		platform_data->reg_vio);
	...
	hscd_power.regulator_magi2c = regulator_get(&client->dev,
		platform_data->reg_magi2c);
	if (IS_ERR(hscd_power.regulator_magi2c)) {
		ret = PTR_ERR(hscd_power.regulator_magi2c);
		hscd_power.regulator_magi2c = NULL;
		alps_errmsg("Failed to get regulator_magi2c %d\n", ret);
#if !defined(CONFIG_MACH_VASTA3G_EUR)
		goto err_setup_regulator;
#endif
	}
	/* turn on the power */
	...
	err = sensors_register(magnetic_device, NULL, magnetic_attrs,
		"magnetic_sensor");
	if (err < 0)
		return err;
...
err_setup_regulator:
	...
	if (hscd_power.regulator_magi2c) {
		regulator_disable(hscd_power.regulator_magi2c);
		regulator_put(hscd_power.regulator_magi2c);
	}
exit:
	this_client = NULL;
