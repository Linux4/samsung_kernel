...
#else
	{STK3X3X_PSCTRL_REG,            (STK3X3X_PS_PRS4 | STK3X3X_PS_GAIN8 | STK3X3X_PS_IT200),    0xFF},
#endif
...
static void stk_ps_report(struct stk3x3x_data *drv_data, int nf)
{
	...
	input_report_abs(drv_data->prox_input_dev, ABS_DISTANCE, nf);
	...
	if (nf == STK3X3X_PRX_NEAR_BY)
		reg_value = (STK3X3X_PS_PRS2 | STK3X3X_PS_GAIN8 | STK3X3X_PS_IT400);
	else if (nf == STK3X3X_PRX_FAR_AWAY) {
		reg_value = (STK3X3X_PS_PRS4 | STK3X3X_PS_GAIN8 | STK3X3X_PS_IT400);
	}
...
#ifdef STK_INTELLI_PERSIST
					| STK3X3X_STATE_EN_INTELL_PRST_MASK
#endif
				   ));
...
int32_t stk3x3x_get_data(struct stk3x3x_data *drv_data, uint16_t *raw_data)
{
	...
	if ((flag_value >> 6) & 0x1) {
		...
		*raw_data = (reg_value[0] << 8 | reg_value[1]);
		...
		if (drv_data->adc == 0) {
			SENSOR_ERR("Unavailable operation, adc=0\n");
			return STK3X3X_PS_DATA_UNAVAILABLE;
		} else {
...
static DEVICE_ATTR(prox_trim,         0664,   stk_ps_code_trim_show,      NULL);
static DEVICE_ATTR(prox_cal,          0664,   stk_ps_code_cal_show,       NULL);
