...
int gp2a_read(struct gp2a_data *gp2a, u8 reg, u8 *val)
{
	int err = 0;
...
int gp2a_write(struct gp2a_data *gp2a, u8 reg, u8 val)
{
	int err = -1;
...
#endif
		value = 0x18;
		gp2a_write(gp2a, REGS_CON, value);
		value = 0x08;
		gp2a_write(gp2a, REGS_GAIN, value);
		value = gp2a->nondetect;
		gp2a_write(gp2a, REGS_HYS, value);
		value = 0x04;
		gp2a_write(gp2a, REGS_CYCLE, value);
		value = 0x03;
		gp2a_write(gp2a, REGS_OPMOD, value);
...
static int gp2a_pmic_leda_onoff(struct device *dev, bool onoff)
{
	struct regulator *gp2a_pls;
	int ret;

	pr_info("%s %s\n", __func__, (onoff) ? "on" : "off");

	gp2a_pls = devm_regulator_get(dev, "gp2a-pls");
	if (IS_ERR(gp2a_pls)) {
		pr_err("[SENSOR]: %s - cannot get gp2a_pls\n", __func__);
		return -ENOMEM;
	}

	if (onoff) {
		ret = regulator_enable(gp2a_pls);
		if (ret) {
			pr_err("%s: enable gp2a-pls failed, rc=%d\n",
				__func__, ret);
			return ret;
		}

	} else {
		ret = regulator_disable(gp2a_pls);
		if (ret) {
			pr_err("%s: disable gp2a-pls failed, rc=%d\n",
				__func__, ret);
			return ret;
		}
	}

	devm_regulator_put(gp2a_pls);
	msleep(20);
...
#ifndef CONFIG_SENSORS_GP2A002_PMIC_LEDA
	pdata->power_en = of_get_named_gpio_flags(np, "gp2a,en-gpio",
				0, &flags);
	if (pdata->power_en < 0) {
...
#else
	gp2a_pmic_leda_onoff(&gp2a->i2c_client->dev, true);
#endif
