...
static void sx9500_get_data(struct sx9500_p *data)
{
	...
	pr_info("[SX9500_WIFI]: %s - cm=%d,uf=%d,av=%d,df=%d,Of=%u (%d)\n",
...
static void sx9500_set_enable(struct sx9500_p *data, u8 enable)
{
	...
	if (enable == ON) {
		...
		pr_info("[SX9500_WIFI]: %s - enable(0x%x)\n", __func__, status);
		data->diff_avg = 0;
		data->diff_cnt = 0;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("[SX9500_WIFI]: %s - Freq : 0x%x\n", __func__, data->freq);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_info("[SX9500_WIFI]: %s - new_value = %u old_value = %d\n",
...
static void sx9500_debug_work_func(struct work_struct *work)
{
	...
	if (check_ta_state() > 1) {
		...
		data->check_usb = false;
		sx9500_set_offset_calibration(data);
		pr_info("[SX9500_WIFI]: %s - TA is removed\n", __func__);
	}
...
static int sx9500_setup_pin(struct sx9500_p *data)
{
	...
	ret = gpio_request(data->gpio_nirq, "SX9500_WIFI_nIRQ");
	if (ret < 0) {
	...
	if (ret < 0) {
		pr_err("[SX9500_WIFI]: %s - failed to set gpio %d(%d)\n",
			__func__, data->gpio_nirq, ret);
...
static void sx9500_initialize_variable(struct sx9500_p *data)
{
	...
	data->normal_th = (u8)CONFIG_SENSORS_SX9500_WIFI_NORMAL_TOUCH_THRESHOLD;
...
static int sx9500_parse_dt(struct sx9500_p *data, struct device *dev)
{
	...
	if (data->gpio_nirq < 0) {
		pr_err("[SX9500_WIFI]: %s - get gpio_nirq error\n", __func__);
		return -ENODEV;
	}
		...
		const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		pr_err("[SX9500_WIFI]: %s - reset failed %d\n", __func__, ret);
	...
	if (ret < 0) {
		pr_err("[SX9500_WIFI]: %s - failed to set request irq %d(%d)\n",
	...
	if (ret) {
		pr_err("[SX9500_WIFI] %s - cound not register sensor(%d).\n",
			__func__, ret);
