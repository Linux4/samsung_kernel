...
{
	struct regulator *sensor_vcc, *sensor_lvs1, *acceli2c_pull;
	int ret=0;
	...
	acceli2c_pull = devm_regulator_get(dev, "bma255-i2c,acceli2c-pull");
	if (IS_ERR(acceli2c_pull)) {
		pr_err("[SENSOR]: %s - cannot get acceli2c-pull\n", __func__);
		return -ENOMEM;
	}
	if (onoff) {
	ret= regulator_enable(sensor_vcc);
	ret=regulator_enable(sensor_lvs1);
	ret=regulator_enable(acceli2c_pull);
	} else {
		ret=regulator_disable(sensor_vcc);
		ret=regulator_disable(sensor_lvs1);
		ret=regulator_disable(acceli2c_pull);
	}

	devm_regulator_put(sensor_vcc);
	devm_regulator_put(sensor_lvs1);
	devm_regulator_put(acceli2c_pull);
	mdelay(5);
