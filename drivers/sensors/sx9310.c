...
#define NORMAL_TOUCH_THRESHOLD   176
/* CS0, CS1, CS2, CS3 */
#ifdef CONFIG_SENSORS_SX9310_USE_2ND_CH
...
static void sx9310_get_data(struct sx9310_p *data)
{
	...
	SENSOR_INFO("Capmain: %d, Useful: %d, avg: %d, diff: %d, Offset: %u\n",
...
static void sx9310_initialize_variable(struct sx9310_p *data)
{
	...
	data->normal_th = (u8)NORMAL_TOUCH_THRESHOLD;
	data->normal_th_buf = data->normal_th;
	data->ch1_state = IDLE;
	data->init_done = OFF;

	SENSOR_INFO("normal_th[%u]\n", data->normal_th);
