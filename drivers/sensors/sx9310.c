...
static int check_hallic_state(char *file_path, unsigned char hall_ic_status[])
{
	...
	if (iRet != sizeof(hall_sysfs)) {
		SENSOR_ERR("Can't read hall ic status\n");
		iRet = -EIO;
	} else {
...
#ifdef CONFIG_SENSORS_SX9310_USE_2ND_CH
		sx9310_i2c_write(data,
				SX9310_CPS_CTRL8_REG, data->normal_th_ch2);
#endif
#else
		sx9310_i2c_write(data,
...
static void sx9310_get_data(struct sx9310_p *data)
{
	...
	SENSOR_INFO("Capmain: %d, Useful: %d, avg: %d, diff: %d, Offset: %u, skip_data: %d\n",
		data->capmain, data->useful, data->avg,
		data->diff, data->offset, data->skip_data);
#ifdef CONFIG_SENSORS_SX9310_USE_2ND_CH
	SENSOR_INFO("Capmain[2ch]: %d, Useful: %d, avg: %d, diff: %d, Offset: %u\n",
		data->capmain_ch2, data->useful_ch2, data->avg_ch2,
		...
		if (status & (CSX_STATUS_REG << MAIN_SENSOR)) {
			if (status & (CSX_STATUS_REG << CH2_SENSOR)) {
				if (status & (BODY_STATUS_REG << (MAIN_SENSOR+1)))
					...
					SENSOR_INFO("main D, ch2 D, but body no\n");
			} else {
				send_event(data, ACTIVE);
			}
		} else {
			if (status & (CSX_STATUS_REG << CH2_SENSOR))
				SENSOR_INFO("only CH2 D, no SAR\n");
		}
	} else {
		if (!(status & (CSX_STATUS_REG << MAIN_SENSOR)))
			send_event(data, IDLE);
...
static void sx9310_debug_work_func(struct work_struct *work)
{
	...
	if (ret < 0) {
		SENSOR_ERR("hallic 1 detect fail = %d\n",
			ret);
	}
#endif
	...
	if (ret < 0) {
		SENSOR_ERR("hallic 2 detect fail = %d\n",
			ret);
	}
...
#endif
		if (hall_flag) {
			SENSOR_INFO("hall IC1&2 is closed\n");
			sx9310_set_offset_calibration(data);
			hall_flag = 0;
		}
	} else
...
static int sx9310_check_chip_id(struct sx9310_p *data)
{
	int ret;
	unsigned char value = 0;

	ret = sx9310_i2c_read(data, SX9310_WHOAMI_REG, &value);
	if (ret < 0) {
		SENSOR_ERR("whoami read failed %d\n", ret);
		return ret;
	}

	SENSOR_INFO("WHO AM I[0x%x]\n",  value);

	switch (value) {
	case 0x01:
		return 0;
	default:
		SENSOR_ERR("invalid whoami(%#x)\n",  value);
		return -1;
	}
	return 0;
}
...
static int sx9310_parse_dt(struct sx9310_p *data, struct device *dev)
{
	...
	ret = of_property_read_u32(node, "sx9310-i2c,normal_th", &temp_val);
	if (!ret) {
...
#ifdef CONFIG_SENSORS_SX9310_USE_2ND_CH
	ret = of_property_read_u32(node, "sx9310-i2c,normal_th_ch2", &temp_val);
	if (!ret) {
...
static int sx9310_regulator_onoff(struct sx9310_p *data, bool onoff)
{
	...
	if (!data->reg_vdd) {
		...
		if (!regulator_get_voltage(data->reg_vdd))
			regulator_set_voltage(data->reg_vdd, 3000000, 3000000);
	}
		...
		const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		SENSOR_ERR("reset failed %d\n", ret);
		goto exit_chip_reset;
	}
	msleep(20);

	ret =  sx9310_check_chip_id(data);
	if (ret < 0) {
	...
	ret = request_threaded_irq(data->irq, NULL, sx9310_interrupt_thread,
			...
			"sx9310_irq", data);
	if (ret < 0) {
		SENSOR_ERR("failed to set request irq %d(%d)\n",
			data->irq, ret);
