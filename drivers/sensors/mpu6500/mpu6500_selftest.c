			 ...
			 int gyro_lsb_bias[3])
{
	...
	if (result) {
		pr_info("register backup error=%d", result);
		return result;
	}

	/* 0x40 : Sleep state, 0x20 : LP state */
	if (mpu6500_selftest.pwm_mgmt[0] & 0x60) {
		result = mpu6500_i2c_write_single_reg(client,
			MPUREG_PWR_MGMT_1, 0x00);
		if (result) {
	...
	result = mpu6500_i2c_write_single_reg(client,
			MPUREG_FIFO_EN, BIT_GYRO_FIFO_EN);
	if (result)
		return result;
	...
	for (i = 0; i < 3; i++) {
		...
		gyro_rms[i] = dps_rms[i] *
			DEF_SCALE_FOR_FLOAT / DEF_SQRT_SCALE_FOR_RMS;
	}
	...
	if (result) {
		pr_info("register recovering error=%d", result);
		return result;
	}
	return ret_val;
}
	...
	int *gyro_result, int *accel_result, int sensors)
{
	...
	/* setup parameters */
	result = mpu6500_i2c_write_single_reg(client,
				MPUREG_CONFIG, MPU_FILTER_184HZ);
	if (result)
		return result;
	...
	while (s < INIT_ST_SAMPLES) {
		...
		if (packet_count < (INIT_ST_SAMPLES - 3)) {
			pr_err("[SENSOR] %s, HW_SELF_TEST_PACKET_ERROR=%d",
