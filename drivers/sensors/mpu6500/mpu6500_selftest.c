...
static int mpu6500_accel_self_test(struct i2c_client *client,
			 ...
			 int gyro_lsb_bias[3])
{
	...
	if (mpu6500_selftest.pwm_mgmt[0] & 0x40
		|| mpu6500_selftest.pwm_mgmt[0] & 0x20) {
		result = mpu6500_i2c_write_single_reg(client, MPUREG_PWR_MGMT_1,
			0x00);
		if (result) {
			pr_info("init PWR_MGMT error=%d", result);
			return result;
		}
	}

	regs[0] = mpu6500_selftest.pwm_mgmt[1] &
		~(BIT_STBY_XG | BIT_STBY_YG | BIT_STBY_ZG);
	result = mpu6500_i2c_write_single_reg(client, MPUREG_PWR_MGMT_2,
		regs[0]);
	...
	result = mpu6500_i2c_write_single_reg(client, MPUREG_FIFO_EN,
		BIT_GYRO_FIFO_EN);
	if (result)
		return result;
	...
	/* Check packet count */
	if ((abs(packet_count - GYRO_PACKET_THRESH) > GYRO_THRESH) &&
		(packet_count < GYRO_PACKET_THRESH)) {
	...
	for (i = 0; i < packet_count; i++) {
		...
		result = mpu6500_i2c_read_reg(client,
			MPUREG_FIFO_R_W, FIFO_PACKET_SIZE, data);
		if (result)
			return result;
	...
	for (j = 0; j < 3; j++) {
		if (abs(avg[j]) > test_setup.bias_thresh) {
			pr_info("%s-Gyro bias (%ld) exceeded threshold (threshold = %d LSB)\n",
	...
	for (i = 0, rms[X] = 0, rms[Y] = 0, rms[Z] = 0; i < packet_count; i++) {
		rms[X] += (long)(gyro_data[0][i] - avg[X])
			* (gyro_data[0][i] - avg[X]);
		rms[Y] += (long)(gyro_data[1][i] - avg[Y])
			* (gyro_data[1][i] - avg[Y]);
		rms[Z] += (long)(gyro_data[2][i] - avg[Z])
			* (gyro_data[2][i] - avg[Z]);
	}
	...
	if (VERBOSE_OUT) {
		pr_info("RMS ^ 2 : %+8ld %+8ld %+8ld\n",
			...
			(long)rms[Y] / packet_count,
			(long)rms[Z] / packet_count);
	}

	{
		...
		for (j = 0; j < 3; j++) {
			if (rms[j] / packet_count > test_setup.rms_thresh) {
				pr_info("%s-Gyro rms (%ld) exceeded threshold (threshold = %d LSB)\n",
					a_name[j], rms[j] / packet_count,
	...
	int *gyro_result, int *accel_result, int sensors)
{
	...
	while (s < INIT_ST_SAMPLES) {
		...
		if (packet_count < (INIT_ST_SAMPLES - DEF_TOTAL_TIMING_TOL)) {
			pr_info("HW_SELF_TEST_PACKET_ERROR=%d",
				packet_count);
			return -1;
		}
		...
		while ((i < packet_count) && (s < INIT_ST_SAMPLES)) {
			...
			if (sensors & MPU6500_HWST_ACCEL) {
				for (j = 0; j < THREE_AXIS; j++)
					accel_result[j] += (short)be16_to_cpup(
						(__be16 *)(&data[2 * j]));
			...
			if (sensors & MPU6500_HWST_GYRO)
				for (j = 0; j < THREE_AXIS; j++)
					gyro_result[j] += (short)be16_to_cpup(
						(__be16 *)(&data[ind + 2 * j]));
