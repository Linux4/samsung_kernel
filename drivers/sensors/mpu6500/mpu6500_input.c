...
#define MPU6500_DEFAULT_DELAY	200000000LL	/* ns */
#define MPU6500_MAX_DELAY	255000000LL	/* ns */
					 ...
					 const struct i2c_device_id *id)
{
	...
	if ((whoami != MPU6500_ID) && (whoami != MPU6515_ID)) {
		pr_err("[SENSOR] %s: mpu6500 or mpu6515 chip ID failed\n", __func__);
		error = -EIO;
		goto err_checking_device;
	}
	...
	if (error) {
		pr_err("[SENSOR] %s: can not register accelerometer_sensor (%d).\n",
			__func__, error);
	...
	if (error) {
		pr_err("[SENSOR] %s: can not register gyro sensor device(%d).\n",
			__func__, error);
