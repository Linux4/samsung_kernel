...
static void mpu6500_input_report_gyro_xyz(struct mpu6500_input_data *data)
{
	...
	const struct mpu6k_input_platform_data *pdata = data->pdata;
...
static int __devinit mpu6500_input_initialize(struct mpu6500_input_data *data,
...
static void mpu6500_request_gpio(struct mpu6k_input_platform_data *pdata)
{
	int ret;
					 ...
					 const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "mpu6500_input_probe Start\n");
	...
	if (client-> dev.of_node) {
		pdata = devm_kzalloc (&client->dev ,
			sizeof (struct mpu6k_input_platform_data), GFP_KERNEL);
		if (!pdata) {
	...
	printk(KERN_INFO "mpu6500_input_probe Success\n");
	return 0;
...
module_i2c_driver(mpu6500_input_driver)
