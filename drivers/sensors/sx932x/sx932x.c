...
#define SUB_MODEL_NAME   "SX9330_SUB"
 ...
 */
/* F52 code for QN3979-110 by xiongxiaoliang at 20210208 start */
static int manual_offset_calibration(psx93XX_t this)
{
    ...
    /* F52 code for QN3979-700 by xiongxiaoliang at 20210303 start */
    msleep(500);
    ...
    msleep(1500);
    /* F52 code for QN3979-700 by xiongxiaoliang at 20210303 end */
    return returnValue;
}
/* F52 code for QN3979-110 by xiongxiaoliang at 20210208 end */
...
static int sar_charger_notifier_callback(struct notifier_block *nb,
    ...
    dev_info( sx932x_sar_usb->pdev, "sar_charger_notifier_callback\n");
    psy= power_supply_get_by_name("usb");
    if (!psy) {
        dev_info( sx932x_sar_usb->pdev, "Couldn't get usbpsy\n");
        return -EINVAL;
    }

    dev_info( sx932x_sar_usb->pdev, "sar_charger_notifier_callback val = %ld\n", val);
    if (!strcmp(psy->desc->name, "usb")) {
        if (psy && val == POWER_SUPPLY_PROP_STATUS) {
            ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_PRESENT, &prop);
            dev_info( sx932x_sar_usb->pdev, "power_supply_get_property prop.intval = %d\n", prop.intval);

            if (ret < 0) {
                dev_info( sx932x_sar_usb->pdev, "Couldn't get POWER_SUPPLY_PROP_ONLINE rc = %d\n", ret);
                return ret;
            } else {
                if(sx932x_sar_usb->usb_plug_status == 2)
                    sx932x_sar_usb->usb_plug_status = prop.intval;

                if(sx932x_sar_usb->usb_plug_status != prop.intval) {
                    dev_info( sx932x_sar_usb->pdev, "usb prop.intval = %d\n", prop.intval);
                    sx932x_sar_usb->usb_plug_status = prop.intval;

                    if(sx932x_sar_usb->charger_notify_wq != NULL)
                        queue_work(sx932x_sar_usb->charger_notify_wq, &sx932x_sar_usb->update_charger);
                }
            }
        }
    }
...
static void sar_update_charger(struct work_struct *work)
{
    ...
    ret = manual_offset_calibration(sx932x_sar_usb);

    if(ret != 2) {
        dev_info( sx932x_sar_usb->pdev, "manual_offset_calibration failed\n");
    }
}
...
void sar_plat_charger_init(void)
{
    ...
    sx932x_sar_usb->usb_plug_status = 2;
    sx932x_sar_usb->charger_notify_wq = create_singlethread_workqueue("sar_charger_wq");

    if (!sx932x_sar_usb->charger_notify_wq) {
        dev_info( sx932x_sar_usb->pdev, "allocate sar_charger_notify_wq failed\n");
        return;
    }

    INIT_WORK(&sx932x_sar_usb->update_charger, sar_update_charger);

    sx932x_sar_usb->notifier_charger.notifier_call = sar_charger_notifier_callback;
    ret = power_supply_reg_notifier(&sx932x_sar_usb->notifier_charger);

    if (ret < 0)
        dev_info( sx932x_sar_usb->pdev, "power_supply_reg_notifier failed\n");
}
/*F52 code for QN3979-1896 by xiongxiaoliang at 20210322 end*/
...
static void touchProcess(psx93XX_t this)
{
    ...
    struct _buttonInfo *pCurrentButton  = NULL;
    /*F52 code for QN3979-1085 by fanjun at 20210312 start*/
    /* F52 code for QN3979-700 by xiongxiaoliang at 20210303 start */
    //read_rawData(this);
    /* F52 code for QN3979-700 by xiongxiaoliang at 20210303 end */
    /*F52 code for QN3979-1085 by fanjun at 20210312 end*/
    if (this && (pDevice = this->pDevice))
    {
        //dev_info(this->pdev, "Inside touchProcess()\n");
        read_register(this, SX932x_STAT0_REG, &i);
        /* F52 code for QN3979-146 by xiongxiaoliang at 20210223 start */
        /*
         *just use PH2,PH1
         *reg 0x01 state-->[3:0]=[PH3,PH2,PH1,PH0]
         */
        i &= 0x06;
        /* F52 code for QN3979-146 by xiongxiaoliang at 20210223 end */
        buttons = pDevice->pbuttonInformation->buttons;
        /* F52  code for SR-QN3979-01-244 by xiongxiaoliang at 20210204 start */
        input_bottom_sar = pDevice->pbuttonInformation->input_bottom_sar;
        input_top_sar = pDevice->pbuttonInformation->input_top_sar;
...
static int sx932x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    * (1->NIRQ=0, 0->NIRQ=1) */
    ...
    if (err > 0) {
        dev_err(&client->dev, "sx9328 whoami check fail!\n");
    ...
    if (!input_bottom_sar) {
        dev_err(&client->dev, "input_allocate_device bottom failed \n");
        err = -ENOMEM;
    ...
    if(input_register_device(input_bottom_sar)){
        dev_err(&client->dev, "add sar sensor bottom unsuccess\n");
    ...
    if(input_register_device(input_top_sar)){
        dev_err(&client->dev, "add sar sensor top unsuccess\n");
    ...
    if (this->init){
        ...
        dev_err(&client->dev,"No init function!!!!\n");
        err = -ENOMEM;
        goto exit_no_init;
    }
...
exit_no_init:
#if defined(CONFIG_SENSORS)
    sysfs_remove_group(&input_bottom_sar->dev.kobj,&ss_sx932x_attributes_group);
    sysfs_remove_group(&input_top_sar->dev.kobj,&ss_sx932x_attributes_group);
exit_input_create:
...
#if 1//def CONFIG_PM
/*====================================================*/
/* F52  code for P210415-03197 by xiongxiaoliang at 20210420 start */
/***** Kernel Suspend *****/
...
/* F52  code for P210415-03197 by xiongxiaoliang at 20210420 end */
/***** Kernel Resume *****/
...
static void sx93XX_schedule_work(psx93XX_t this, unsigned long delay)
{
    unsigned long flags;
    if (this) {
        /* F52 code for QN3979-409 by xiongxiaoliang at 20210224 start */
        //dev_info(this->pdev, "sx93XX_schedule_work()\n");
        /* F52 code for QN3979-409 by xiongxiaoliang at 20210224 end */
