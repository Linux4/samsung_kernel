...
#include <linux/sensor/sensors_class.h>
...
int sensors_create_symlink(struct kobject *target, const char *name)
{
	...
	if (symlink_dev == NULL) {
		SENSOR_ERR("symlink_dev is NULL!!!\n");
		return err;
	}

	err = sysfs_create_link(&symlink_dev->kobj, target, name);
	if (err < 0) {
		SENSOR_ERR("%s failed!(%d)\n", name, err);
		return err;
	}
		...
		struct device_attribute *attributes[], char *name)
{
	...
	*dev = device_create(sensors_class, NULL, 0, drvdata, "%s", name);
	if (IS_ERR(*dev)) {
		ret = PTR_ERR(*dev);
		SENSOR_ERR("device_create failed![%d]\n", ret);
		return ret;
	}

	for (i = 0; attributes[i] != NULL; i++)
		if ((device_create_file(*dev, attributes[i])) < 0)
			SENSOR_ERR("fail device_create_file"\
				"(dev, attributes[%d])\n", i);
...
int sensors_input_init(void)
{
	...
	if (!meta_input_dev) {
		SENSOR_ERR("failed alloc meta dev\n");
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		SENSOR_ERR("failed register meta dev\n");
	...
	ret = sensors_create_symlink(&meta_input_dev->dev.kobj,
		meta_input_dev->name);
	if (ret < 0) {
		SENSOR_ERR("failed create meta symlink\n");
...
static int __init sensors_class_init(void)
{
	...
	sensors_class = class_create(THIS_MODULE, "sensors");
	if (IS_ERR(sensors_class)) {
		SENSOR_ERR("create sensors_class is failed.(err=%d)\n",
			IS_ERR(sensors_class));
		return PTR_ERR(sensors_class);
	}
	...
	if (IS_ERR(sensor_dev)) {
		SENSOR_ERR("sensor_dev create failed![%d]\n",
			IS_ERR(sensor_dev));
		...
		if ((device_create_file(sensor_dev, *sensor_attr)) < 0)
			pr_err("[SENSOR_CLASS] failed flush device_file\n");
	}
	...
	if (IS_ERR(symlink_dev)) {
		SENSOR_ERR("symlink_dev create failed![%d]\n",
			IS_ERR(symlink_dev));
