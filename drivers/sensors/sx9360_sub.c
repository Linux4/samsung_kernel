...
static void sx9360_enter_unknown_mode(struct sx9360_p *data, int type)
{
	if (data->noti_enable && !data->skip_data) {
		...
		if (data->is_unknown_mode == UNKNOWN_OFF) {
			...
			pr_info("[SX9360_SUB] : UNKNOWN Re-enter\n");
		} else {
			pr_info("[SX9360_SUB] : already UNKNOWN\n");
		}
...
static ssize_t sx9360_usefilter_data_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	u8 val = 0;
	int offset = 0, idx = 0;
	...
	for (idx = 0; idx < (int)(ARRAY_SIZE(usefilter_reg)); idx++) {
		sx9360_i2c_read(data, usefilter_reg[idx].reg, &val);
		pr_info("[SX9360_SUB]: %s - Read Reg: 0x%x Value: 0x%x\n\n",
			__func__, usefilter_reg[idx].reg, val);

		offset += snprintf(buf + offset, PAGE_SIZE - offset,
		"Reg: 0x%x Value: 0x%08x\n", usefilter_reg[idx].reg, val);
	}
	return offset;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val == 1)
		...
		pr_err("[SX9360_SUB] : %s -Invalid Argument(%u)\n", __func__, val);
...
static DEVICE_ATTR(usefilter_data, S_IRUGO, sx9360_usefilter_data_show, NULL);
...
static void sx9360_check_first_working(struct sx9360_p *data)
{
	if (data->noti_enable && data->motion) {
		if (data->detect_threshold < data->diff) {
			...
			pr_info("[SX9360_SUB] : first working detected %d\n", data->diff);
		} else {
			if (data->first_working) {
				data->is_unknown_mode = UNKNOWN_OFF;
				pr_info("[SX9360_SUB] : Release detected %d, unknown mode off\n", data->diff);
			}
		}
	}
}
...
static void sx9360_debug_work_func(struct work_struct *work)
{
	...
	if (data->hallic_detect) {
		...
		if (strcmp(data->hall_ic, str) == 0) {
			...
			if (!hall_flag) {
				pr_info("[SX9360_SUB]: %s - hall IC is opened\n", __func__);
				sx9360_enter_unknown_mode(data, TYPE_HALL);
			}
			hall_flag = 1;
		}
	...
	if (data->debug_zero_count >= ZERO_DETECT_TIME) {
		...
		if (ret < 0)
			pr_err("[SX9360_SUB]: fail to read PHEN :0x%02x (%d)\n", value, ret);
		else if (value == 0) {
			pr_info("[SX9360_SUB]: %s - detected all data zero!!!\n", __func__);
...
static int sx9360_input_init(struct sx9360_p *data)
{
	...
	if (!noti_input_dev) {
		pr_err("[SX9360_SUB] : input_allocate_device failed\n");
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		pr_err("[SX9360_SUB] : failed to register input dev for noti (%d)\n", ret);
		input_unregister_device(dev);
...
static int sx9360_parse_dt(struct sx9360_p *data, struct device *dev)
{
	...
	if (data->gpio_nirq < 0) {
		...
		pr_info("[SX9360_SUB]: %s - get gpio_nirq %d\n",
			__func__, data->gpio_nirq);
	}

	data->vdd_ldo = of_get_named_gpio_flags(dNode, "sx9360,vdd_en",
			0, &flags);
	if (data->vdd_ldo < 0) {
		pr_info("[SX9360_SUB] not using vdd en\n");
		data->vdd_ldo = 0;
	} else {
		ret = gpio_request(data->vdd_ldo, "grip_en");
		if (ret < 0) {
			pr_info("[SX9360_SUB] gpio %d request failed (%d)\n",
				data->vdd_ldo, ret);
		}
		...
		const struct i2c_device_id *id)
{
	...
	/* ldo control */
	if (data->vdd_ldo) {
		pr_info("[SX9360_SUB]: %s - vdd on !!\n", __func__);
		gpio_set_value(data->vdd_ldo, 1);
		usleep_range(10000, 11000);
	}
...
{
	pr_info("[SX9360_SUB] sx9360_init called\n");
