...
#ifdef CONFIG_SENSORS_GRIP_CHK_HALLIC
static int sx9306_sub_check_hallic_state(char *file_path,
...
static int sx9306_sub_get_nirq_state(struct sx9306_p *data)
{
	return gpio_get_value_cansleep(data->gpio_nirq);
}

static int sx9306_sub_i2c_write(struct sx9306_p *data, u8 reg_addr, u8 buf)
{
	int ret;
	struct i2c_msg msg;
...
static int sx9306_sub_i2c_read(struct sx9306_p *data, u8 reg_addr, u8 *buf)
{
	int ret;
...
static int sx9306_sub_i2c_read_block(struct sx9306_p *data, u8 reg_addr,
	u8 *buf, u8 buf_size)
{
	int ret;
	struct i2c_msg msg[2];
...
static u8 sx9306_sub_read_irqstate(struct sx9306_p *data)
{
	u8 val = 0;

	if (sx9306_sub_i2c_read(data, SX9306_IRQSTAT_REG, &val) >= 0)
		return val;

	return 0;
}

static void sx9306_sub_initialize_register(struct sx9306_p *data)
{
	u8 val = 0;
	int idx;

	for (idx = 0; idx < (int)(sizeof(setup_reg) >> 1); idx++) {
		sx9306_sub_i2c_write(data, setup_reg[idx].reg,
			setup_reg[idx].val);
		SENSOR_INFO("Write Reg: 0x%x Value: 0x%x\n",
			setup_reg[idx].reg, setup_reg[idx].val);

		sx9306_sub_i2c_read(data, setup_reg[idx].reg, &val);
...
static void sx9306_sub_initialize_chip(struct sx9306_p *data)
{
	int cnt = 0;

	while ((sx9306_sub_get_nirq_state(data) == 0) && (cnt++ < 10)) {
		sx9306_sub_read_irqstate(data);
		msleep(20);
	}
	...
	sx9306_sub_initialize_register(data);
}

static int sx9306_sub_set_offset_calibration(struct sx9306_p *data)
{
	int ret = 0;

	ret = sx9306_sub_i2c_write(data, SX9306_IRQSTAT_REG, 0xFF);
	SENSOR_INFO("done\n");

	return ret;
}

static void sx9306_sub_send_event(struct sx9306_p *data, u8 state)
{
	SENSOR_INFO("THD = %d\n", data->normal_th);

	if (state == ACTIVE) {
		data->state = ACTIVE;
		sx9306_sub_i2c_write(data, SX9306_CPS_CTRL6_REG,
			data->normal_th);
...
static void sx9306_sub_display_data_reg(struct sx9306_p *data)
{
	u8 val, reg;

	sx9306_sub_i2c_write(data, SX9306_REGSENSORSELECT, data->channel_main);
	for (reg = SX9306_REGUSEMSB; reg <= SX9306_REGOFFSETLSB; reg++) {
		sx9306_sub_i2c_read(data, reg, &val);
		SENSOR_INFO("Register(0x%2x) data(0x%2x)\n", reg, val);
	}
}

static void sx9306_sub_get_data(struct sx9306_p *data)
{
	u8 ms_byte = 0;
	u8 ls_byte = 0;
	...
	sx9306_sub_i2c_write(data, SX9306_REGSENSORSELECT, data->channel_main);
	sx9306_sub_i2c_read_block(data, SX9306_REGUSEMSB,
...
static int sx9306_sub_set_mode(struct sx9306_p *data, unsigned char mode)
{
	int ret = -EINVAL;

	if (mode == SX9306_MODE_SLEEP) {
		ret = sx9306_sub_i2c_write(data, SX9306_CPS_CTRL0_REG,
		...
		sx9306_sub_set_offset_calibration(data);
		msleep(400);
	}
...
static void sx9306_sub_set_enable(struct sx9306_p *data, u8 enable)
{
	u8 status = 0;

	if (enable == ON) {
		sx9306_sub_i2c_read(data, SX9306_TCHCMPSTAT_REG, &status);
		SENSOR_INFO("enable(0x%x)\n", status);
		data->diff_avg = 0;
		data->diff_cnt = 0;
		sx9306_sub_get_data(data);

		if (data->skip_data == true) {
			input_report_rel(data->input, REL_MISC, 2);
			input_sync(data->input);
		} else if (status & (CSX_STATUS_REG << data->channel_main)) {
			sx9306_sub_send_event(data, ACTIVE);
		} else {
			sx9306_sub_send_event(data, IDLE);
		}
		...
		/* enabling IRQ */
		sx9306_sub_i2c_write(data, SX9306_IRQ_ENABLE_REG,
						setup_reg[0].val);
		...
		/* disabling IRQ */
		sx9306_sub_i2c_write(data, SX9306_IRQ_ENABLE_REG, 0x00);
...
static void sx9306_sub_set_debug_work(struct sx9306_p *data, u8 enable)
{
	if (enable == ON) {
		data->debug_count = 0;
...
static ssize_t sx9306_sub_get_offset_calibration_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	u8 val = 0;
	struct sx9306_p *data = dev_get_drvdata(dev);

	sx9306_sub_i2c_read(data, SX9306_IRQSTAT_REG, &val);

	return snprintf(buf, PAGE_SIZE, "%d\n", val);
}

static ssize_t sx9306_sub_set_offset_calibration_store(struct device *dev,
...
static ssize_t sx9306_sub_register_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int regist = 0, val = 0;
	int idx;
	...
	sx9306_sub_i2c_write(data, (unsigned char)regist, (unsigned char)val);
...
static ssize_t sx9306_sub_register_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char val = 0;
	int offset = 0, idx = 0;
...
static ssize_t sx9306_sub_read_data_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9306_p *data = dev_get_drvdata(dev);

	sx9306_sub_display_data_reg(data);

	return snprintf(buf, PAGE_SIZE, "%d\n", 0);
}

static ssize_t sx9306_sub_sw_reset_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9306_sub_freq_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	sx9306_sub_i2c_write(data, SX9306_CPS_CTRL2_REG, reg);
...
static ssize_t sx9306_sub_freq_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9306_sub_vendor_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", VENDOR_NAME);
}

static ssize_t sx9306_sub_name_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", MODEL_NAME);
}

static ssize_t sx9306_sub_touch_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "1\n");
}

static ssize_t sx9306_sub_raw_data_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	static int sum;
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9306_sub_threshold_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	/* It's for init touch */
...
static ssize_t sx9306_sub_threshold_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}

static ssize_t sx9306_sub_normal_threshold_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9306_sub_normal_threshold_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long val;
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9306_sub_onoff_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9306_sub_onoff_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	u8 val;
	int ret;
...
static ssize_t sx9306_sub_calibration_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "2,0,0\n");
}

static ssize_t sx9306_sub_calibration_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}

static ssize_t sx9306_sub_gain_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int ret;
...
static ssize_t sx9306_sub_range_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int ret;
...
static ssize_t sx9306_sub_diff_avg_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9306_sub_irq_count_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9306_sub_irq_count_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static DEVICE_ATTR(menual_calibrate, S_IRUGO | S_IWUSR | S_IWGRP,
			sx9306_sub_get_offset_calibration_show,
			sx9306_sub_set_offset_calibration_store);
static DEVICE_ATTR(reg, S_IRUGO | S_IWUSR | S_IWGRP,
			sx9306_sub_register_show, sx9306_sub_register_store);
static DEVICE_ATTR(readback, S_IRUGO, sx9306_sub_read_data_show, NULL);
static DEVICE_ATTR(reset, S_IRUGO, sx9306_sub_sw_reset_show, NULL);

static DEVICE_ATTR(name, S_IRUGO, sx9306_sub_name_show, NULL);
static DEVICE_ATTR(vendor, S_IRUGO, sx9306_sub_vendor_show, NULL);
static DEVICE_ATTR(gain, S_IRUGO, sx9306_sub_gain_show, NULL);
static DEVICE_ATTR(range, S_IRUGO, sx9306_sub_range_show, NULL);
static DEVICE_ATTR(mode, S_IRUGO, sx9306_sub_touch_mode_show, NULL);
static DEVICE_ATTR(raw_data, S_IRUGO, sx9306_sub_raw_data_show, NULL);
static DEVICE_ATTR(diff_avg, S_IRUGO, sx9306_sub_diff_avg_show, NULL);
static DEVICE_ATTR(calibration, S_IRUGO | S_IWUSR | S_IWGRP,
			sx9306_sub_calibration_show,
			sx9306_sub_calibration_store);
static DEVICE_ATTR(onoff, S_IRUGO | S_IWUSR | S_IWGRP,
			sx9306_sub_onoff_show, sx9306_sub_onoff_store);
static DEVICE_ATTR(threshold, S_IRUGO | S_IWUSR | S_IWGRP,
			sx9306_sub_threshold_show,
			sx9306_sub_threshold_store);
static DEVICE_ATTR(normal_threshold, S_IRUGO | S_IWUSR | S_IWGRP,
			sx9306_sub_normal_threshold_show,
			sx9306_sub_normal_threshold_store);
static DEVICE_ATTR(freq, S_IRUGO | S_IWUSR | S_IWGRP,
			sx9306_sub_freq_show, sx9306_sub_freq_store);
static DEVICE_ATTR(irq_count, S_IRUGO | S_IWUSR | S_IWGRP,
			sx9306_sub_irq_count_show, sx9306_sub_irq_count_store);
...
/*****************************************************************************/
static ssize_t sx9306_sub_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	sx9306_sub_set_enable(data, enable);

	return size;
}

static ssize_t sx9306_sub_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9306_sub_flush_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	u8 enable;
	int ret;
...
static DEVICE_ATTR(enable, S_IRUGO | S_IWUSR | S_IWGRP,
		sx9306_sub_enable_show, sx9306_sub_enable_store);
static DEVICE_ATTR(flush, S_IWUSR | S_IWGRP,
		NULL, sx9306_sub_flush_store);
...
static void sx9306_sub_touch_process(struct sx9306_p *data, u8 flag)
{
	u8 status = 0;
...
static void sx9306_sub_process_interrupt(struct sx9306_p *data)
{
	u8 flag = 0;

	/* since we are not in an interrupt don't need to disable irq. */
	flag = sx9306_sub_read_irqstate(data);

	if (flag & IRQ_PROCESS_CONDITION)
		sx9306_sub_touch_process(data, flag);
}
#if defined(CONFIG_CCIC_NOTIFIER) && defined(CONFIG_USB_TYPEC_MANAGER_NOTIFIER)
static int sx9306_sub_ccic_handle_notification(struct notifier_block *nb,
		unsigned long action, void *data)
{
	...
	if (pdata->init_done == ON) {
		...
		case USB_STATUS_NOTIFY_DETACH:
			...
			sx9306_sub_i2c_write(pdata, SX9306_CPS_CTRL6_REG,
				pdata->normal_th);
			sx9306_sub_set_offset_calibration(pdata);
			break;
		default:
				...
				unsigned long action, void *data)
{
	...
	sx9306_sub_i2c_write(pdata, SX9306_CPS_CTRL6_REG, pdata->normal_th);
...
#endif
static void sx9306_sub_init_work_func(struct work_struct *work)
{
	struct sx9306_p *data = container_of((struct delayed_work *)work,
		struct sx9306_p, init_work);

	sx9306_sub_initialize_chip(data);
	sx9306_sub_i2c_write(data, SX9306_CPS_CTRL6_REG, data->normal_th);
	sx9306_sub_set_mode(data, SX9306_MODE_NORMAL);
...
static void sx9306_sub_irq_work_func(struct work_struct *work)
{
	struct sx9306_p *data = container_of((struct delayed_work *)work,
		struct sx9306_p, irq_work);

	sx9306_sub_process_interrupt(data);
}

static void sx9306_sub_debug_work_func(struct work_struct *work)
{
	struct sx9306_p *data = container_of((struct delayed_work *)work,
...
#ifdef CONFIG_SENSORS_GRIP_CHK_HALLIC
	sx9306_sub_check_hallic_state(HALLIC_PATH, data->hall_ic);
...
static irqreturn_t sx9306_sub_interrupt_thread(int irq, void *pdata)
{
	struct sx9306_p *data = pdata;

	if (sx9306_sub_get_nirq_state(data) == 1)
...
static int sx9306_sub_input_init(struct sx9306_p *data)
{
	int ret = 0;
...
static int sx9306_sub_setup_pin(struct sx9306_p *data)
{
	int ret;
...
static void sx9306_sub_initialize_variable(struct sx9306_p *data)
{
	data->state = IDLE;
	data->skip_data = false;
	data->init_done = OFF;
...
static void sx9306_sub_read_setupreg(struct device_node *dnode, char *str,
				u8 *val)
{
	u32 temp_val;
	int ret;
...
static int sx9306_sub_parse_dt(struct sx9306_p *data, struct device *dev)
{
	...
	data->gpio_nirq = of_get_named_gpio_flags(node,
		"sx9306_sub-i2c,nirq-gpio", 0, &flags);
	if (data->gpio_nirq < 0) {
		SENSOR_ERR("get gpio_nirq error\n");
		return -ENODEV;
	}

	sx9306_sub_read_setupreg(node, "sx9306_sub-i2c,ctrl0",
		&setup_reg[SX9306_CTRL0_IDX].val);
	sx9306_sub_read_setupreg(node, "sx9306_sub-i2c,ctrl1",
		&setup_reg[SX9306_CTRL1_IDX].val);
	sx9306_sub_read_setupreg(node, "sx9306_sub-i2c,ctrl2",
		&setup_reg[SX9306_CTRL2_IDX].val);
	sx9306_sub_read_setupreg(node, "sx9306_sub-i2c,ctrl3",
		&setup_reg[SX9306_CTRL3_IDX].val);
	sx9306_sub_read_setupreg(node, "sx9306_sub-i2c,ctrl4",
		&setup_reg[SX9306_CTRL4_IDX].val);
	sx9306_sub_read_setupreg(node, "sx9306_sub-i2c,ctrl5",
		&setup_reg[SX9306_CTRL5_IDX].val);
	sx9306_sub_read_setupreg(node, "sx9306_sub-i2c,ctrl6",
		&setup_reg[SX9306_CTRL6_IDX].val);
	sx9306_sub_read_setupreg(node, "sx9306_sub-i2c,ctrl7",
		&setup_reg[SX9306_CTRL7_IDX].val);
	sx9306_sub_read_setupreg(node, "sx9306_sub-i2c,ctrl8",
		&setup_reg[SX9306_CTRL8_IDX].val);
	sx9306_sub_read_setupreg(node, "sx9306_sub-i2c,ctrl9",
		&setup_reg[SX9306_CTRL9_IDX].val);
	sx9306_sub_read_setupreg(node, "sx9306_sub-i2c,ctrl10",
		&setup_reg[SX9306_CTRL10_IDX].val);

	ret = of_property_read_u32(node, "sx9306_sub-i2c,normal-thd",
				&temp_val);
	if (!ret)
	...
	ret = of_property_read_u32(node, "sx9306_sub-i2c,ta-thd", &temp_val);
	if (!ret)
	...
	ret = of_property_read_u32(node, "sx9306_sub-i2c,ch-main", &temp_val);
	if (!ret)
	...
	ret = of_property_read_u32(node, "sx9306_sub-i2c,ch-sub1", &temp_val);
	if (!ret)
...
static int sx9306_sub_probe(struct i2c_client *client,
			const struct i2c_device_id *id)
{
	...
	ret = sx9306_sub_parse_dt(data, &client->dev);
	if (ret < 0) {
		SENSOR_ERR("of_node error\n");
		goto exit_of_node;
	}

	ret = sx9306_sub_setup_pin(data);
	if (ret) {
	...
	/* read chip id */
	ret = sx9306_sub_i2c_write(data, SX9306_SOFTRESET_REG,
				SX9306_SOFTRESET);
	if (ret < 0) {
	...
	sx9306_sub_initialize_variable(data);

	INIT_DELAYED_WORK(&data->init_work, sx9306_sub_init_work_func);
	INIT_DELAYED_WORK(&data->irq_work, sx9306_sub_irq_work_func);
	INIT_DELAYED_WORK(&data->debug_work, sx9306_sub_debug_work_func);

	data->irq = gpio_to_irq(data->gpio_nirq);
	/* add IRQF_NO_SUSPEND option in case of Spreadtrum AP */
	ret = request_threaded_irq(data->irq, NULL, sx9306_sub_interrupt_thread,
	...
	ret = sx9306_sub_input_init(data);
	if (ret < 0)
...
static int sx9306_sub_remove(struct i2c_client *client)
{
	SENSOR_INFO("\n");
	return 0;
}

static int sx9306_sub_suspend(struct device *dev)
{
	struct sx9306_p *data = dev_get_drvdata(dev);

	SENSOR_INFO("\n");
	sx9306_sub_set_debug_work(data, OFF);

	return 0;
}

static int sx9306_sub_resume(struct device *dev)
{
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static void sx9306_sub_shutdown(struct i2c_client *client)
{
	...
	sx9306_sub_set_debug_work(data, OFF);
	if (atomic_read(&data->enable) == ON)
		sx9306_sub_set_enable(data, OFF);
	sx9306_sub_set_mode(data, SX9306_MODE_SLEEP);
}
...
static const struct i2c_device_id sx9306_sub_id[] = {
	{ "sx9306_match_table", 0 },
	{ }
};

static const struct dev_pm_ops sx9306_sub_pm_ops = {
	.suspend = sx9306_sub_suspend,
	.resume = sx9306_sub_resume,
};

static struct i2c_driver sx9306_sub_driver = {
	.driver = {
		.name	= MODEL_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sx9306_match_table,
		.pm = &sx9306_sub_pm_ops
	},
	.probe		= sx9306_sub_probe,
	.remove		= sx9306_sub_remove,
	.shutdown	= sx9306_sub_shutdown,
	.id_table	= sx9306_sub_id,
};

static int __init sx9306_sub_init(void)
{
	return i2c_add_driver(&sx9306_sub_driver);
}

static void __exit sx9306_sub_exit(void)
{
	i2c_del_driver(&sx9306_sub_driver);
}

module_init(sx9306_sub_init);
module_exit(sx9306_sub_exit);
