		...
		u8 reg, u8 *val, unsigned int len)
{
	...
	while (retry--) {
		...
		if (ret >= 0)
			break;
		SENSOR_ERR("sub fail(address[%d] write)(%d)(%d)\n", msg.addr, retry, ret);
		usleep_range(10000, 11000);
#ifdef FEATURE_I2C_CHANGE
		if (ret == -6) {
			if (client->addr == 0x20)
				client->addr = 0x21;
			else
				client->addr = 0x20;
			SENSOR_ERR("sub address changes %x -> %x", msg.addr, client->addr);
			msg.addr = client->addr;
		}
#endif
	}
	...
	while (retry--) {
		...
		SENSOR_ERR("sub fail(address[%d] read)(%d)(%d)\n", msg.addr, retry, ret);
		usleep_range(10000, 11000);
	}
	mutex_unlock(&data->lock);
	return ret;
...
static void a96t3x6_set_enable(struct a96t3x6_data *data, int enable, bool force)
{
	...
	if ((data->current_state == enable) ||
		...
		SENSOR_INFO("sub old = %d, new = %d, skip exception case\n",
...
static void a96t3x6_firmware_work_func(struct work_struct *work)
{
	...
	if (ret) {
		...
		SENSOR_ERR("sub final retry failed\n");
	} else {
		SENSOR_INFO("sub fw check success\n");
	}
}
#endif
				...
				struct device_attribute *attr, char *buf)
{
	...
	while (retry--) {
		...
		if (r_buf[0] == 0x20)
			break;
		if (ret < 0)
			SENSOR_ERR("sub failed(%d)\n", retry);
		msleep(100);
	}

	SENSOR_INFO("sub expect 0x20 read 0x%x\n", r_buf[0]);
	a96t3x6_check_diff_and_cap(data);
...
static int a96t3x6_fw_mode_enter(struct a96t3x6_data *data)
{
	...
	if (data->firmup_cmd != cmd) {
		SENSOR_ERR("sub cmd not matched, firmup fail (ret = %d, cmd = %d)\n", ret, cmd);
		return -2;
	}
...
static int a96t3x6_fw_update(struct a96t3x6_data *data, u8 cmd)
{
	...
	while(retry > 0)
	{
	    ...
	    if (ret < 0)
	    	SENSOR_ERR("sub a96t3x6_fw_mode_enter fail, retry : %d\n", i);
	    else
	    	break;
...
static void grip_always_active(struct a96t3x6_data *data, int on)
{
	...
	ret = a96t3x6_i2c_write(data->client, REG_GRIP_ALWAYS_ACTIVE, &cmd);
		if (ret < 0)
			SENSOR_INFO("sub failed to change grip always active mode\n");
	...
	while (retry--) {
		...
		if (ret < 0)
			SENSOR_INFO("sub i2c read fail(%d)\n", ret);
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (data->current_state) {
		...
		ret = a96t3x6_i2c_write(data->client, REG_SAR_ENABLE, &cmd);
		if (ret < 0)
			SENSOR_INFO("sub failed to enable grip irq\n");
...
static int a96t3x6_power_onoff(void *pdata, bool on)
{
	...
	if (data->ldo_en) {
		...
		ret = gpio_get_value(data->ldo_en);
		SENSOR_INFO("sub ldo_en power %d, %d\n", on, ret);
		ret = 0;
	}
	...
	if (data->dvdd_vreg_name) {
		if (data->dvdd_vreg == NULL) {
			...
			if (IS_ERR(data->dvdd_vreg)) {
				data->dvdd_vreg = NULL;
				SENSOR_ERR("sub failed to get dvdd_vreg %s\n", data->dvdd_vreg_name);
			}
		}
	}		
	...
	if (data->dvdd_vreg) {
		...
		SENSOR_INFO("sub dvdd_vreg reg_enabled=%d voltage=%d\n", reg_enabled, voltage);
	}
	...
	if (on) {
		if (data->dvdd_vreg) {
			if (reg_enabled == 0) {
				...
				SENSOR_INFO("sub dvdd_vreg turned on\n");
			}
		}
	} else {
		if (data->dvdd_vreg) {
			if (reg_enabled == 1) {
				...
				SENSOR_INFO("sub dvdd_vreg turned off\n");
			}
		}
	}
			...
			struct a96t3x6_data *data)
{
	...
	ret = gpio_request(data->grip_int, "a96t3x6_sub_IRQ");
	if (ret < 0) {
...
static int a96t3x6_parse_dt(struct a96t3x6_data *data, struct device *dev)
{
	...
	SENSOR_INFO("sub dvdd_vreg_name: %s\n", data->dvdd_vreg_name);
	...
	ret = of_property_read_u8(np, "a96t3x6,earjack_noise",
		&data->earjack_noise);
	if (ret < 0) {
		SENSOR_INFO("sub failed to get the earjack noise dt\n");
		data->earjack_noise = 0;
	} else {
		SENSOR_INFO("sub grip earjack noise block is adjusting %d \n", (int)data->earjack_noise);
				...
				unsigned long action, void *data)
{
	...
	case STATUS_VBUS_LOW:
		...
		break;
	default:
		SENSOR_INFO("sub vbus skip attach = %d\n", vbus_type);
		break;
	}
