...
static void a96t3x6_debug_work_func(struct work_struct *work)
{
	...
	} else if ((hall_state != HALL_CLOSE_STATE && hall_prev_state != hall_state)
		|| (cert_hall_state != HALL_CLOSE_STATE && cert_hall_prev_state != cert_hall_state)) {
...
{
	struct a96t3x6_data *data = container_of((struct work_struct *)work,
						struct a96t3x6_data, pdic_attach_reset_work);
	u8 cmd = CMD_OFF;
...
{
	struct a96t3x6_data *data = container_of((struct work_struct *)work,
						struct a96t3x6_data, pdic_detach_reset_work);
	u8 cmd = CMD_ON;
