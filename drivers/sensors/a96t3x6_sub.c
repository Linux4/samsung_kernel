...
static int a96t3x6_pdic_handle_notification(struct notifier_block *nb,
	...
	if (usb_typec_info.rprd == PDIC_NOTIFY_HOST) {
		...
		GRIP_INFO("otg detach - unknown, reset skip");
	} else {
...
static void a96t3x6_debug_work_func(struct work_struct *work)
{
	...
	if (ret < 0) {
		...
		ret = manager_notifier_register(&data->cpuidle_ccic_nb,
					a96t3x6_pdic_handle_notification,
...
#if defined(CONFIG_SENSORS_A96T3X6_LDO_SHARE)
	...
	struct device_attribute *attr, char *buf)
{
	...
	if ((checksum[0] != data->checksum_h_bin) ||
		...
		return snprintf(buf, PAGE_SIZE, "OK,%02x%02x\n",
			checksum[0], checksum[1]);
#endif
#else //CONFIG_SENSORS_A96T3X6_CRC_CHECK
	ret = a96t3x6_crc_check(data);
...
#endif
#ifdef CONFIG_SENSORS_A96T3X6_LDO_SHARE
...
static int a96t3x6_fw_check(struct a96t3x6_data *data)
{
	...
	if (data->crc_check == CRC_FAIL) {
		GRIP_ERR("crc failed. force update\n");
		fw_up |= 0x08;
	}
#endif
	if (fw_up) {
...
static int a96t3x6_power_onoff(void *pdata, bool on)
{
	...
	if (data->ldo_en) {
		ret = gpio_request(data->ldo_en, "a96t3x6_sub_ldo_en");
		if (ret < 0) {
...
err_reg_input_dev:
	...
	gpio_free(data->grip_int);
#ifndef CONFIG_SENSORS_A96T3X6_LDO_SHARE
	if (data->power)
		data->power(data, false);
#endif
pwr_config:
...
static int a96t3x6_remove(struct i2c_client *client)
{
	...
	data->power(data, false);
#ifdef CONFIG_SENSORS_A96T3X6_HALL_NOTIFIER
	hall_notifier_unregister(&data->hall_nb);
#endif
