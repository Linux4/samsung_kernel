...
static int a96t3x6_fw_update(struct a96t3x6_data *data, u8 cmd)
{
	...
	while (retry > 0) {
		a96t3x6_reset_for_bootmode(data);
		usleep_range(BOOT_DELAY, BOOT_DELAY + 100);
...
static int __init a96t3x6_sub_init(void)
{
	GRIP_INFO("%s\n", __func__);
...
static void __exit a96t3x6_sub_exit(void)
{
	i2c_del_driver(&a96t3x6_driver);
}

module_init(a96t3x6_sub_init);
module_exit(a96t3x6_sub_exit);
