			...
			struct  mag_platform_data *pdata)
{
	...
	/*scl,sda and irq */
	pdata->i2c_pull_up = of_property_read_bool(np, "yas532,i2c-pull-up");

	pdata->str_l19 = of_get_property(np,"yas532,reg_vdd", NULL);
	pdata->str_lvs1 = of_get_property(np,"yas532,reg_vio", NULL);
	pdata->used_chip = of_property_read_u32(np,"yas532,used_chip", &pdata->used_chip_flags);
	pdata->position = of_property_read_u32(np,"yas532,used_chip", &pdata->position_flags);
...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int rt =0, sysfs_created = 0, sysfs_raw_created = 0;
	...
	int position;
	int error;
#ifdef CONFIG_YAS_MAG_MULTI_SUPPORT
	struct mag_platform_data *platform_data;

	if (client-> dev.of_node) {
		platform_data = devm_kzalloc (&client->dev ,
			sizeof (struct mag_platform_data), GFP_KERNEL);
		if (!platform_data) {
	...
	error = yas_parse_dt(&client->dev, platform_data);
	if (error) {
		goto err;
	}
#endif
