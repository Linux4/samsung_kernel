	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (suspend) {
		...
		geomagnetic_suspend(this_client);
	} else {
			...
			struct  mag_platform_data *pdata)
{
	...
	if (of_property_read_u32(np, "ssp,rotation-direction", &pdata->position))
		pdata->position = 0;

	pr_err("yas_parse_dt complete, Position:%d\n",pdata->position);
...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client-> dev.of_node) {
		...
		iRet = yas_parse_dt(&client->dev, pdata);
		if (iRet)
			return iRet;
	} 
	else
