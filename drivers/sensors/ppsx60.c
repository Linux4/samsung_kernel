...
#define TMPLIER 10000 /*for float type which can not be used in kernel*/
#define TRIMBIT 0x01
#define COMPLEMENT_MAX 2097151
#define COMPLEMENT_MIN -2097151

#define EOL_1STEP 1
#define EOL_2STEP 2
#define EOL_LOW_CURRENT 0xd340
#define EOL_HIGH_CURRENT 0x32c80
#define EOL_TIA_GAIN 0x0005
#define EOL_AMB_GAIN 0x0005
#define EOL_OFFSET 0x0000

int32_t goffsetir = TMPLIER;
int32_t goffsetred = TMPLIER;

static int ppsX60_hrm_eol_test_enable(struct ppsX60_device_data *data, int current_step);

static unsigned int reg_defaults[NUM_OF_PPG_REG] = {
	0x0,			/* 0x00_00 AFE_CONTROL0 : REG_READ[0]=1 */
	27588,		/* 0x01_01 AFE_LED2STC : [15:0] */
	27988,		/* 0x02_02 AFE_LED2ENDC : [15:0] */
	3,			/* 0x03_03 AFE_LED1LEDSTC : [15:0] */
	483,			/* 0x04_04 AFE_LED1LEDENDC : [15:0] */
	567,			/* 0x05_05 AFE_ALED2STC_LED3STC : [15:0] */
	967,			/* 0x06_06 AFE_ALED2ENDC_LED3ENDC : [15:0] */
	83,			/* 0x07_07 AFE_LED1STC : [15:0] */
	483,			/* 0x08_08 AFE_LED1ENDC : [15:0] */
	27508,		/* 0x09_09 AFE_LED2LEDSTC : [15:0] */
	27988,		/* 0x0a_10 AFE_LED2LEDENDC : [15:0] */
	27104,		/* 0x0b_11 AFE_ALED1STC : [15:0] */
	27504,		/* 0x0c_12 AFE_ALED1ENDC : [15:0] */
	27999,		/* 0x0d_13 AFE_LED2CONVST : [15:0] */
	28991,		/* 0x0e_14 AFE_LED2CONVEND : [15:0] */
	29002,		/* 0x0f_15 AFE_ALED2CONVST_LED3CONVST : [15:0] */
	29994,		/* 0x10_16 AFE_ALED2CONVEND_LED3CONVEND : [15:0] */
	30005,		/* 0x11_17 AFE_LED1CONVST : [15:0] */
	30997,		/* 0x12_18 AFE_LED1CONVEND : [15:0] */
	31008,		/* 0x13_19 AFE_ALED1CONVST : [15:0] */
	32000,		/* 0x14_20 AFE_ALED1CONVEND : [15:0] */
	27992,		/* 0x15_21 AFE_ADCRSTSTCT0 : [15:0] */
	27998,		/* 0x16_22 AFE_ADCRSTENDCT0 : [15:0] */
	28995,		/* 0x17_23 AFE_ADCRSTSTCT1 : [15:0] */
	29001,		/* 0x18_24 AFE_ADCRSTENDCT1 : [15:0] */
	29998,		/* 0x19_25 AFE_ADCRSTSTCT2 : [15:0] */
	30004,		/* 0x1a_26 AFE_ADCRSTENDCT2 : [15:0] */
	31001,		/* 0x1b_27 AFE_ADCRSTSTCT3 : [15:0] */
	31007,		/* 0x1c_28 AFE_ADCRSTENDCT3 : [15:0] */
	...
	0x4,			/* 0x21_33 AFE_TIAAMBGAIN : TIA_CF[5:3]=5pF, TIA_GAIN[2:0]=50k */
	...
	0x000000,		/* 0x28_40 AFE_RESERVED2 : FIX_BITS1[23:22]=3 pre-release Silicon fixed */
	...
	1767,		/* 0x32_50 AFE_DPD1STC_PDNCYCLESTC : [15:0] */
	26224,		/* 0x33_51 AFE_DPD1ENDC_PDNCYCLEENDC : [15:0] */
	0,			/* 0x34_52 AFE_DPD2STC : */
	0,			/* 0x35_53 AFE_DPD2ENDC : */
	487,			/* 0x36_54 AFE_REFSTC_LED3LEDSTC : [15:0] */
	967,			/* 0x37_55 AFE_REFENDC_LED3LEDENDC : [15:0] */
...
static int ppsX60_regulator_onoff(struct ppsX60_device_data *data, int onoff)
{
	...
	if (!data->hrm_en) {
		data->vdd_3p3 = regulator_get(NULL, "VDD_HRM_3P3");
...
static int ppsX60_trim_check(struct ppsX60_device_data *device)
{
	uint32_t iroffset[] = {TMPLIER, 8544, 7573, 6667, 12222, 14966, 18333, 22449}; /*trim ratio * TMPLIER*/
	uint32_t redoffset[] = {TMPLIER, 8545, 7574, 6667, 12237, 14988, 18338, 22456}; /*trim ratio * TMPLIER*/
	uint32_t retval_trim = 0;
	uint32_t irbitcheck = 0;
	uint32_t redbitcheck = 0;
	int err;

	err = ppsX60_i2c_write(device, AFE_CONTROL0, I2C_WRITE_ENABLE);
	if (err != 0) {
		pr_err("%s - error AFE_CONTROL0\n", __func__);
		return -EIO;
	}

	err = ppsX60_i2c_write(device, AFE_CONTROL0, CTL0_SW_RESET);
	if (err != 0) {
		pr_err("%s - error AFE_CONTROL0\n", __func__);
		return -EIO;
	}

	err = ppsX60_i2c_write(device, AFE_CONTROL0, I2C_READ_ENABLE);
	if (err != 0) {
		pr_err("%s - error AFE_CONTROL0\n", __func__);
		return -EIO;
	}

	err = ppsX60_i2c_read(device, AFE_RESERVED3, &retval_trim);
	if (err != 0) {
		pr_err("%s - error AFE_RESERVED3\n", __func__);
		return -EIO;
	}

	retval_trim = retval_trim & 0xBF;

	if (retval_trim & 0x80) {
		irbitcheck = retval_trim & 0x07; /*valid IR trim bit*/
		redbitcheck = (retval_trim & 0x38)>>3; /*valid RED trim bit*/

		goffsetir = iroffset[irbitcheck];
		goffsetred = redoffset[redbitcheck];
	} else {
		goffsetred = TMPLIER;
		goffsetir = TMPLIER;
	}

	pr_info("%s retval_trim = 0x%x, goffsetir = %d, goffsetred = %d\n",
		__func__, retval_trim, goffsetir, goffsetred);
	return 0;
}
...
static int ppsX60_init_device(struct ppsX60_device_data *data)
{
	...
	err = ppsX60_trim_check(data);
	if (err != 0) {
		pr_err("%s - error ppsX60_trim_check\n", __func__);
		return -EIO;
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sysfs_streq(buf, "16")) /* ID_HRM_RAW */
		handle = 16;
	else if (sysfs_streq(buf, "17")) /* ID_HRM_ALC */
		handle = 17;
	else {
...
static void ppsX60_eol_test_onoff(struct ppsX60_device_data *data, int onoff)
{
	int err;
	pr_info("ppsX60_%s: %d\n", __func__, onoff);
	...
	if (onoff) {
		...
		data->eol_step = EOL_1STEP;
		err = ppsX60_hrm_eol_test_enable(data, data->eol_step);
		if (err != 0)
			pr_err("ppsX60_hrm_eol_test_enable err : %d\n", err);
...
static void ppsX60_eol_check_data(struct ppsX60_device_data *data, int current_step)
{
	int err, i;
	int eol_irdiff[101];
	int eol_reddiff[101];
	int64_t eol_smirdiff[101];
	int64_t eol_smreddiff[101];
	int64_t smir_sum = 0;
	long smir_avg = 0;
	int64_t smred_sum = 0;
	long smred_avg = 0;
	...
	pr_info("ppsX60_%s\n", __func__);
	if (data->eol_step == EOL_1STEP) {
		data->eol_lowir_result = data->eol_sum[0]/data->eol_data_count;
		data->eol_lowred_result = data->eol_sum[1]/data->eol_data_count;
		data->eol_freq_result = data->eol_data_count;

		ppsX60_mode_enable(data, HRM_LDO_ON);
		data->eol_step = EOL_2STEP;
		err = ppsX60_hrm_eol_test_enable(data, data->eol_step);
		if (err != 0)
			pr_err("ppsX60_hrm_eol_test_enable err : %d\n", err);
		data->eol_test_is_enable = 1;

	} else if (data->eol_step == EOL_2STEP) {
		data->eol_highir_result = data->eol_sum[0] / 100;
		data->eol_highred_result = data->eol_sum[1] / 100;

		data->eol_ir_ac_result = data->eol_highir_result - data->eol_lowir_result;
		data->eol_red_ac_result = data->eol_highred_result - data->eol_lowred_result;

		data->eol_ir_acdc_result = ((int64_t)data->eol_ir_ac_result*10000000)/data->eol_highir_result;
		data->eol_red_acdc_result = ((int64_t)data->eol_red_ac_result*10000000)/data->eol_highred_result;
		data->eol_absorb_result = ((int64_t)data->eol_red_acdc_result*1000)/data->eol_ir_acdc_result;

		for (i = 0; i < 100; i++) {
				eol_irdiff[i] = data->eol_highir_result - data->eol_irhigh[i];
				eol_reddiff[i] = data->eol_highred_result - data->eol_redhigh[i];
				eol_smirdiff[i] = (int64_t)(eol_irdiff[i]*eol_irdiff[i]);
				eol_smreddiff[i] = (int64_t)(eol_reddiff[i]*eol_reddiff[i]);
				smir_sum += eol_smirdiff[i];
				smred_sum += eol_smreddiff[i];
		}
		smir_avg = smir_sum/100;
		smred_avg = smred_sum/100;

		ir_stddev = int_sqrt(smir_avg);
		red_stddev = int_sqrt(smred_avg);

		data->eol_irnoise_result = (((int64_t)ir_stddev*10000000)/data->eol_highir_result)*100;
		data->eol_rednoise_result = (((int64_t)red_stddev*10000000)/data->eol_highred_result)*100;

		pr_info("%s %d,%d,%d,%d,%d,%lld,%d,%d,%d,%d,%lld,%d", __func__,
		data->eol_freq_result, data->eol_lowir_result, data->eol_highir_result, data->eol_ir_ac_result,
		data->eol_ir_acdc_result, data->eol_irnoise_result, data->eol_lowred_result,
		data->eol_highred_result, data->eol_red_ac_result, data->eol_red_acdc_result,
		data->eol_rednoise_result, data->eol_absorb_result);

		ppsX60_eol_test_onoff(data, 0);
		data->eol_test_status = 1;
	}
}

static void ppsX60_eol_work_func(struct work_struct *work)
{
	struct ppsX60_device_data *data = container_of(work, struct ppsX60_device_data,
						work_eol);
	data->eol_timer_count++;
	if (data->eol_step == EOL_1STEP) {
		if (data->eol_timer_count >= 200) {
			hrtimer_cancel(&data->eol_timer);
			ppsX60_eol_test_onoff(data, 0);
			ppsX60_eol_check_data(data, data->eol_step);
		}
	} else if (data->eol_step == EOL_2STEP) {
		if (data->eol_data_count >= 300) {
			hrtimer_cancel(&data->eol_timer);
			ppsX60_eol_test_onoff(data, 0);
			ppsX60_eol_check_data(data, data->eol_step);
		}
...
static enum hrtimer_restart ppsX60_eol_timer_func(struct hrtimer *timer)
{
	struct ppsX60_device_data *data = container_of(timer,
					struct ppsX60_device_data, eol_timer);

	queue_work(data->eol_wq, &data->work_eol);
	hrtimer_forward_now(&data->eol_timer, data->poll_delay);
...
{
	int err;
	pr_info("ppsX60_%s: %d\n", __func__, current_step);
	err = ppsX60_i2c_write(data, AFE_CONTROL0, I2C_WRITE_ENABLE);
	if (err != 0) {
		pr_err("%s - error AFE_CONTROL0\n", __func__);
		return -EIO;
	}
	if (current_step == EOL_1STEP) {
		err = ppsX60_i2c_write(data, AFE_LEDCNTRL,  EOL_LOW_CURRENT);
		if (err != 0) {
			pr_err("%s - error AFE_LEDCNTRL\n", __func__);
			return -EIO;
		}
	} else if (current_step == EOL_2STEP) {
		err = ppsX60_i2c_write(data, AFE_LEDCNTRL,  EOL_HIGH_CURRENT);
		if (err != 0) {
			pr_err("%s - error AFE_LEDCNTRL\n", __func__);
			return -EIO;
		}
	}
	err = ppsX60_i2c_write(data, AFE_TIAAMBGAIN, EOL_TIA_GAIN);
	if (err != 0) {
		pr_err("%s - error AFE_TIAAMBGAIN\n", __func__);
		return -EIO;
	}
	err = ppsX60_i2c_write(data, AFE_TIAGAIN, EOL_AMB_GAIN);
	if (err != 0) {
		pr_err("%s - error AFE_TIAGAIN\n", __func__);
		return -EIO;
	}
	err = ppsX60_i2c_write(data, AFE_DAC_SETTING_REG, EOL_OFFSET);
	if (err != 0) {
		pr_err("%s - error AFE_DAC_SETTING_REG\n", __func__);
		return -EIO;
	}
	...
	data->eol_data_count = 0;
	data->eol_timer_count = 0;
	data->eol_sum[0] = 0;
	data->eol_sum[1] = 0;
	hrtimer_start(&data->eol_timer, data->poll_delay, HRTIMER_MODE_REL);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sysfs_streq(buf, "1")) { /* eol_test start */
		...
		data->eol_timer_count = 0;
		data->eol_freq_result = 0;
		data->eol_lowir_result = 0;
		data->eol_lowred_result = 0;
		data->eol_highir_result = 0;
		data->eol_highred_result = 0;
		data->eol_ir_ac_result = 0;
		data->eol_red_ac_result = 0;
		data->eol_ir_acdc_result = 0;
		data->eol_red_acdc_result = 0;
		data->eol_absorb_result = 0;
		data->eol_irnoise_result = 0;
		data->eol_rednoise_result = 0;
	} else if (sysfs_streq(buf, "0")) { /* eol_test stop */
		test_onoff = 0;
	} else {
		pr_info("ppsX60_%s: invalid value %d\n", __func__, *buf);
		return -EINVAL;
	}
	pr_info("ppsX60_%s: %d\n", __func__, test_onoff);
	...
	ppsX60_eol_test_onoff(data, test_onoff);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (data->eol_test_status == 0) {
		pr_info("ppsX60_%s - data->eol_test_status is NULL\n",
		__func__);
		data->eol_test_status = 0;
		return snprintf(buf, PAGE_SIZE, "%s\n", "NO_EOL_TEST");
	}
	pr_info("ppsX60_%s - result = %d\n", __func__, data->eol_test_status);
	data->eol_test_status = 0;
	return snprintf(buf, PAGE_SIZE, "%d,%d,%d,%d,%d,%lld,%d,%d,%d,%d,%lld,%d\n",
	data->eol_freq_result, data->eol_lowir_result, data->eol_highir_result, data->eol_ir_ac_result,
	data->eol_ir_acdc_result, data->eol_irnoise_result, data->eol_lowred_result,
	data->eol_highred_result, data->eol_red_ac_result, data->eol_red_acdc_result,
	data->eol_rednoise_result, data->eol_absorb_result);
	...
	struct device_attribute *attr, char *buf)
{
	struct ppsX60_device_data *data = dev_get_drvdata(dev);
	int err = 0;
	int i2c_check_value = 0x0002;
	int gain_val[2] = {0,};
	int led_val[4] = {0,};

	data->selftest_result = false;

	ppsX60_mode_enable(data, 1);
/*s/w reset*/
	err = ppsX60_i2c_write(data, AFE_CONTROL0, I2C_WRITE_ENABLE);
	if (err != 0) {
		pr_err("%s - error AFE_CONTROL0\n", __func__);
		data->selftest_result = -EIO;
		goto self_test_end;
	}
/*step 1 : i2c check*/
	err = ppsX60_i2c_write(data, AFE_TIAGAIN, i2c_check_value);
	if (err != 0) {
		pr_err("%s - error AFE_TIAGAIN\n", __func__);
		data->selftest_result = false;
		goto self_test_end;
	}
	err = ppsX60_i2c_write(data, AFE_TIAAMBGAIN, i2c_check_value);
	if (err != 0) {
		pr_err("%s - error AFE_TIAGAIN\n", __func__);
		data->selftest_result = false;
		goto self_test_end;
	...
	mdelay(10);
	err = ppsX60_i2c_read(data, AFE_TIAGAIN, &gain_val[0]);
	if (err != 0) {
		pr_err("%s - error AFE_TIAGAIN\n", __func__);
		data->selftest_result = false;
		goto self_test_end;
	}
	err = ppsX60_i2c_read(data, AFE_TIAAMBGAIN, &gain_val[1]);
	if (err != 0) {
		pr_err("%s - error AFE_TIAAMBGAIN\n", __func__);
		data->selftest_result = false;
		goto self_test_end;
	}
	pr_info("%s GAIN:0x%x, AGAIN:0x%x -check value:0x%x\n",
		__func__, gain_val[0], gain_val[1], i2c_check_value);

	if ((i2c_check_value == gain_val[0]) &&
		(i2c_check_value == gain_val[1])) {
		data->selftest_result = true;
	} else {
		pr_err("%s - error gain check 0:0x%x,1:0x%x\n",
			__func__, gain_val[0], gain_val[1]);
		data->selftest_result = 0;
		goto self_test_end;
	}
/*step 2 : data check*/
	err = ppsX60_i2c_read(data, AFE_LED2VAL, &led_val[0]);
	if (err != 0) {
		pr_err("%s - error AFE_LED2VAL\n", __func__);
		data->selftest_result = false;
		goto self_test_end;
	}
	err = ppsX60_i2c_read(data, AFE_LED3VAL, &led_val[1]);
	if (err != 0) {
		pr_err("%s - error AFE_LED3VAL\n", __func__);
		data->selftest_result = false;
		goto self_test_end;
	}
	err = ppsX60_i2c_read(data, AFE_LED1VAL, &led_val[2]);
	if (err != 0) {
		pr_err("%s - error AFE_LED1VAL\n", __func__);
		data->selftest_result = false;
		goto self_test_end;
	}
	err = ppsX60_i2c_read(data, AFE_ALED1VAL, &led_val[3]);
	if (err != 0) {
		pr_err("%s - error AFE_ALED1VAL\n", __func__);
		data->selftest_result = false;
		goto self_test_end;
	}
	led_val[0] = afeConvP32((int)led_val[0]);
	led_val[1] = afeConvP32((int)led_val[1]);
	led_val[2] = afeConvP32((int)led_val[2]);
	led_val[3] = afeConvP32((int)led_val[3]);
pr_info("%s led after: [0];%d,[1];%d,[2];%d,[3];%d,\n",
		__func__, led_val[0], led_val[1], led_val[2], led_val[3]);
	if((led_val[0] >= COMPLEMENT_MIN) && (led_val[0] <= COMPLEMENT_MAX ) &&
		(led_val[1] >= COMPLEMENT_MIN) && (led_val[1] <= COMPLEMENT_MAX) &&
		(led_val[2] >= COMPLEMENT_MIN) && (led_val[2] <= COMPLEMENT_MAX) &&
		(led_val[3] >= COMPLEMENT_MIN) && (led_val[3] <= COMPLEMENT_MAX)) {
		if ((led_val[0] == 0) && (led_val[1] == 0)
			&& (led_val[2] == 0) && (led_val[3] == 0)) {
			data->selftest_result = false;
			goto self_test_end;
		} else {
			data->selftest_result = true;
...
self_test_end:
	ppsX60_mode_enable(data, 0);
	pr_info("ppsX60_%s - result = %d\n", __func__, data->selftest_result);
	return snprintf(buf, PAGE_SIZE, "%d\n",data->selftest_result);
}
...
static DEVICE_ATTR(eol_test_result, S_IRUGO, eol_test_result_show, NULL);
static DEVICE_ATTR(eol_test_status, S_IRUGO, eol_test_status_show, NULL);

static DEVICE_ATTR(selftest, S_IRUGO, selftest_show, NULL);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sysfs_streq(buf, "18")) /* HRM LED IR */
		handle = 18;
	else if (sysfs_streq(buf, "19")) /* HRM LED RED */
		handle = 19;
	else {
		pr_info("%s: invalid value %d\n", __func__, *buf);
		return -EINVAL;
...
irqreturn_t ppsX60_irq_handler(int irq, void *device)
{
	...
	int64_t tir, tred; /*to prevent overflow*/
...
/*	pr_info("%s: before IR(%d) Red(%d) Amb1(%d) Amb2(%d)\n", __func__,
		complement_data[0], complement_data[1], complement_data[2], complement_data[3]);*/


	tir = complement_data[0];

	if (complement_data[0] < 0)
		tir *= (2*TMPLIER) - goffsetir; /*Negative calc*/
	else
		tir *= goffsetir;

	complement_data[0] = tir / TMPLIER;
	if (complement_data[0] > COMPLEMENT_MAX)
		complement_data[0] = COMPLEMENT_MAX;
	else if (complement_data[0] < COMPLEMENT_MIN)
		complement_data[0] = COMPLEMENT_MIN;

	tred = complement_data[1];

	if (complement_data[1] < 0)
		tred *= (2*TMPLIER) - goffsetred; /*Negative calc*/
	else
		tred *= goffsetred;

	complement_data[1] = tred / TMPLIER;
	if (complement_data[1] > COMPLEMENT_MAX)
		complement_data[1] = COMPLEMENT_MAX;
	else if (complement_data[1] < COMPLEMENT_MIN)
		complement_data[1] = COMPLEMENT_MIN;

	if (data->eol_test_is_enable) {
		data->eol_data_count++;
		if (data->eol_step == EOL_1STEP) {
			data->eol_sum[0] = (int64_t)(data->eol_sum[0] + complement_data[0]);/* IR */
			data->eol_sum[1] = (int64_t)(data->eol_sum[1] + complement_data[1]);/* RED */
		} else if (data->eol_step == EOL_2STEP) {
			if (data->eol_data_count > 200 && data->eol_data_count <= 300) {
				data->eol_irhigh[data->eol_data_count-201] = complement_data[0];/* IR */
				data->eol_redhigh[data->eol_data_count-201] = complement_data[1];/* RED */
				data->eol_sum[0] = (int64_t)(data->eol_sum[0] + data->eol_irhigh[data->eol_data_count-201]);/* IR */
				data->eol_sum[1] = (int64_t)(data->eol_sum[1] + data->eol_redhigh[data->eol_data_count-201]);/* RED */
			}
		}
		if ((data->eol_data_count % 10) == 0) {
			pr_info("%s data_count:%d,timer_count:%d data 0:%d,1:%d \n",
				__func__, data->eol_data_count, data->eol_timer_count, complement_data[0], complement_data[1]);
		}
	} else {
		if (err == 0) {
			if (((timestamp_new - data->old_timestamp) > (ktime_to_ms(data->poll_delay)*2* 1800000LL))\
			&& (data->old_timestamp != 0)){
				...
				if (atomic_read(&data->isenable_led)) {
					...
					input_report_rel(data->hrmled_input_dev, REL_DIAL, time_hi);
					input_report_rel(data->hrmled_input_dev, REL_WHEEL, time_lo);
					...
					input_report_rel(data->hrm_input_dev, REL_DIAL, time_hi);
					input_report_rel(data->hrm_input_dev, REL_WHEEL, time_lo);
					input_sync(data->hrm_input_dev);
				}
				/*pr_info("%s_%lld gap: after IR(%d) Red(%d) goffsetir(%d) goffsetred(%d)\n", __func__,
					timestamp, complement_data[0], complement_data[1], goffsetir, goffsetred);*/
			}
			...
			/*pr_info("%s_%lld_n: after IR(%d) Red(%d) goffsetir(%d) goffsetred(%d)\n", __func__,
					timestamp_new, complement_data[0], complement_data[1], goffsetir, goffsetred);*/
		}
	}
	...
	struct device *dev)
{
	...
	data->hrm_en = of_get_named_gpio_flags(dnode,
		"ppsx60,vdd_3p3_en", 0, &flags);
	if (data->hrm_en < 0) {
		pr_err("%s - get hrm_en has no en gpio %d\n", __func__, data->hrm_en);
		data->hrm_en = 0;
	}
...
static int ppsX60_gpio_setup(struct ppsX60_device_data *data)
{
	...
	errorno = request_threaded_irq(data->irq, NULL,
		ppsX60_irq_handler, IRQF_TRIGGER_RISING|IRQF_ONESHOT,
		"hrm_sensor_irq", data);
...
int ppsX60_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	input_set_capability(data->hrm_input_dev, EV_REL, REL_DIAL);
	input_set_capability(data->hrm_input_dev, EV_REL, REL_WHEEL);
	...
	input_set_capability(data->hrmled_input_dev, EV_REL, REL_DIAL);
	input_set_capability(data->hrmled_input_dev, EV_REL, REL_WHEEL);
	...
	err = sensors_create_symlink(&data->hrmled_input_dev->dev.kobj, data->hrmled_input_dev->name);
	if (err < 0) {
		pr_err("%s - create_symlink error\n", __func__);
	...
	hrtimer_init(&data->eol_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	data->poll_delay = ns_to_ktime(EOL_DELAY);
	data->eol_timer.function = ppsX60_eol_timer_func;

	data->eol_wq = create_singlethread_workqueue("ppsx60_eol_wq");
	if (!data->eol_wq) {
		err = -ENOMEM;
		pr_err("%s: could not create eol_wq workqueue\n",
			__func__);
		goto err_create_eol_workqueue;
	}

	INIT_WORK(&data->work_eol, ppsX60_eol_work_func);
...
dev_set_drvdata_failed:
	destroy_workqueue(data->eol_wq);
err_create_eol_workqueue:
...
err_hrmled_sysfs_create_group:
	sensors_remove_symlink(&data->hrmled_input_dev->dev.kobj, data->hrmled_input_dev->name);
