...
static int taos_get_lux(struct taos_data *taos)
{
	...
	/* new calculate lux */
	CPL = (ATIME_ms * AGAIN *1000) / 82;
	lux1 = (1000*cleardata - 1780 * irdata) / CPL;
	lux2 = (790 * cleardata -1350 * irdata) / CPL;
	//calculated_lux = MAX(lux1, lux2, 0);
...
static void taos_work_func_light(struct work_struct *work)
{
	...
	if ((ktime_to_ns(taos->light_poll_delay) * (int64_t)taos->time_count)
		>= ((int64_t)LIGHT_LOG_TIME * NSEC_PER_SEC)) {
		printk("[TAOS]: %s c:%d, i:%d, a:%d\n",
			__func__, taos->cleardata, taos->irdata, adc);
		taos->time_count = 0;
	} else {
		taos->time_count++;
	}
...
/* set sysfs for proximity sensor and light sensor */
	...
	err =  sensors_initialize_symlink(taos->light_input_dev);
	if (err) {
...
static int taos_suspend(struct device *dev)
{
	...
	*/
	...
	if (!(taos->power_state & PROXIMITY_ENABLED)) {
		taos_chip_off(taos);
		gpio_direction_output(taos->pdata->en, 0);
		//taos->pdata->power(false);
	} else {
		disable_irq(taos->irq);
	}
...
static int taos_resume(struct device *dev)
{
	...
	if (!(taos->power_state & PROXIMITY_ENABLED)) {
		gpio_direction_output(taos->pdata->en, 1);
		msleep(20);
		taos_chip_on(taos);
	} else {
		enable_irq(taos->irq);
