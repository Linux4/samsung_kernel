		...
		case BMA2X2_MODE_NORMAL:
			...
			do{
				...
				comres += bma2x2_smbus_read_byte(client,
					BMA2X2_LOW_NOISE_CTRL_REG,
					&err2);
				if ((data1 == err1)&&(data2 == err2))
					break;
				else
					pr_info("[SENSOR] %s ###retry NORMAL data1: 0x%x, data2: 0x%x\n",__func__, err1, err2);
			}while(retry--);
		...
		case BMA2X2_MODE_LOWPOWER1:
			...
			do{
				...
				if ((data1 == err1)&&(data2 == err2))
					...
					pr_info("[SENSOR] %s ###retry LOWPOWER1 data1: 0x%x, data2: 0x%x\n",__func__, err1, err2);
			}while(retry--);
			break;
...
#endif
#if 0//def DETECT_XYZ_ERR
	struct bma2x2_v acc_old = *acc;
#endif
...
#ifdef DETECT_XYZ_ERR
	{
		int sum_all = abs(acc->x)+abs(acc->y)+abs(acc->z);
		if(sum_all < 150 || sum_all > 1500)
		{
			pr_err("%s [SENSOR] - i2cstd: sum error(%d)\n",__func__, sum_all);
			pr_err("[SENSOR]: %s - x = %d, y = %d, z = %d \n",
					__func__, acc->x, acc->y,
								acc->z);
			if (--retry > 0)
				goto read;
		}
