...
#define GM_ST_X_SPEC_MIN	-30
#define GM_ST_X_SPEC_MAX	30
#define GM_ST_Y_SPEC_MIN	-30
#define GM_ST_Y_SPEC_MAX	30
#define GM_ST_Z_SPEC_MIN	-400
#define GM_ST_Z_SPEC_MAX	-50
...
retry:
	...
	*dac_ret += ak09911c_i2c_read(data->client, AK09911C_REG_CNTL2, &reg);
	...
	pr_info("[SENSOR]: %s - self test x = %d, y = %d, z = %d\n",
		__func__, x, y, z);
	if ((x >= GM_ST_X_SPEC_MIN) && (x <= GM_ST_X_SPEC_MAX))
		pr_info("[SENSOR]: %s - x passed self test, %d<=x<=%d\n",
			__func__, GM_ST_X_SPEC_MIN, GM_ST_X_SPEC_MAX);
	else
		pr_info("[SENSOR]: %s - x failed self test, %d<=x<=%d\n",
			__func__, GM_ST_X_SPEC_MIN, GM_ST_X_SPEC_MAX);
	if ((y >= GM_ST_Y_SPEC_MIN) && (y <= GM_ST_Y_SPEC_MAX))
		pr_info("[SENSOR]: %s - y passed self test, %d<=y<=%d\n",
			__func__, GM_ST_Y_SPEC_MIN, GM_ST_Y_SPEC_MAX);
	else
		pr_info("[SENSOR]: %s - y failed self test, %d<=y<=%d\n",
			__func__, GM_ST_Y_SPEC_MIN, GM_ST_Y_SPEC_MAX);
	if ((z >= GM_ST_Z_SPEC_MIN) && (z <= GM_ST_Z_SPEC_MAX))
		pr_info("[SENSOR]: %s - z passed self test, %d<=z<=%d\n",
			__func__, GM_ST_Z_SPEC_MIN, GM_ST_Z_SPEC_MAX);
	else
		pr_info("[SENSOR]: %s - z failed self test, %d<=z<=%d\n",
			__func__, GM_ST_Z_SPEC_MIN, GM_ST_Z_SPEC_MAX);
	...
	if (((x >= GM_ST_X_SPEC_MIN) && (x <= GM_ST_X_SPEC_MAX)) &&
		((y >= GM_ST_Y_SPEC_MIN) && (y <= GM_ST_Y_SPEC_MAX)) &&
		((z >= GM_ST_Z_SPEC_MIN) && (z <= GM_ST_Z_SPEC_MAX))) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (retries = 0; retries < 5; retries++) {
		if (ak09911c_read_mag_xyz(data, &mag) == 0) {
			if ((mag.x < GM_DATA_SPEC_MAX)
				&& (mag.x > GM_DATA_SPEC_MIN)
				&& (mag.y < GM_DATA_SPEC_MAX)
				&& (mag.y > GM_DATA_SPEC_MIN)
				&& (mag.z < GM_DATA_SPEC_MAX)
				&& (mag.z > GM_DATA_SPEC_MIN))
				adc_ret = 0;
			else
