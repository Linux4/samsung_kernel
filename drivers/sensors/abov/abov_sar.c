            ...
            case IDLE: /* Button is being in far state! */
                if ((i & pCurrentButton->mask) == pCurrentButton->mask) {
                    ...
                    if (board->cap_channel_bottom == counter) {
                        ...
                        #else
                        input_report_abs(input_bottom_sar, ABS_DISTANCE, 0);
                        #endif
                        ...
                        #else
                        input_report_abs(input_top_sar, ABS_DISTANCE, 0);
                        #endif
...
static int abov_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ret == 0) {
        ...
        regulator_put(pplatData->vdd);
        LOG_INFO("free vdd/vddio regulator source!\n");
    ...
    if (this) {
        ...
        if (pDevice) {
            ...
            #else
            __set_bit(EV_ABS, input_bottom_sar->evbit);
            input_set_abs_params(input_bottom_sar, ABS_DISTANCE, -1, 100, 0, 0);
            #endif
            /* save the input pointer and finish initialization */
            pDevice->pbuttonInformation->input_bottom_sar = input_bottom_sar;
            /* save the input pointer and finish initialization */
            input_bottom_sar->name = "grip_bottom_sensor";
            if (input_register_device(input_bottom_sar)) {
                LOG_INFO("add abov sar sensor unsuccess\n");
                return -ENOMEM;
            }
            ...
            #else
            __set_bit(EV_ABS, input_top_sar->evbit);
            input_set_abs_params(input_top_sar, ABS_DISTANCE, -1, 100, 0, 0);
            #endif
            /* save the input pointer and finish initialization */
            pDevice->pbuttonInformation->input_top_sar = input_top_sar;
            /* save the input pointer and finish initialization */
            input_top_sar->name = "grip_top_sensor";
            if (input_register_device(input_top_sar)) {
            LOG_INFO("add abov top sar sensor unsuccess\n");
            return -ENOMEM;
            }
        }
