...
static void sx9360_reset_work_func(struct work_struct *work)
{
	struct sx9360_p *data = container_of((struct work_struct *)work,
						struct sx9360_p, reset_work);
	sx9360_set_offset_calibration(data);
}
/*****************************************************************************/
#ifdef CONFIG_SUPPORT_CAMERA_FREEFALL
					     ...
					     unsigned long action, void *data)
{
	...
	struct sx9360_p *pdata = container_of(nb, struct sx9360_p, cpuidle_ccic_nb);
	...
	schedule_work(&pdata->reset_work);
...
#if IS_ENABLED(CONFIG_CCIC_NOTIFIER) || IS_ENABLED(CONFIG_PDIC_NOTIFIER) || IS_ENABLED(CONFIG_MUIC_NOTIFIER)
	INIT_WORK(&data->reset_work, sx9360_reset_work_func);
#endif
