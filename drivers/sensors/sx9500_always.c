...
#if defined(CONFIG_MUIC_NOTIFIER)
static int sx9500_cpuidle_muic_notifier(struct notifier_block *nb,
				unsigned long action, void *data)
{
	struct sx9500_p *grip_data;
	muic_attached_dev_t attached_dev = *(muic_attached_dev_t *)data;

	grip_data = container_of(nb, struct sx9500_p, cpuidle_muic_nb);
	...
	case ATTACHED_DEV_AFC_CHARGER_9V_MUIC:
		...
		if (grip_data->init_done == ON)
			sx9500_set_offset_calibration(grip_data);
		else
		...
		const struct i2c_device_id *id)
{
	...
	muic_notifier_register(&data->cpuidle_muic_nb,
		sx9500_cpuidle_muic_notifier, MUIC_NOTIFY_DEV_CPUIDLE);
#endif
