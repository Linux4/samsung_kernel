...
#define REG_STATUS_ADDR		(0x27)	/** STATUS register */
...
static int k303c_mag_hw_init(struct k303c_mag_p *data)
{
	...
	if (err < 0) {
		pr_err("%s, failed to read WHO_AM_I\n", __func__);
		goto exit;
	} else
		data->hw_working = 1;

	if (buf[0] != dataus_registers.who_am_i.value) {
		pr_err("%s, unkown device. Expected: 0x%02x, was: 0x%02x\n",
			__func__, dataus_registers.who_am_i.value, buf[0]);
		err = -1;
		goto exit;
	}
	...
	if (err < 0) {
		pr_err("%s, Error initializing CLTR registers\n", __func__);
		goto exit;
	}
...
static int k303c_regulator_onoff(struct k303c_mag_p *data, bool onoff)
{
	int ret = -1;
	DBG_FUNC_NAME;
	...
	unsigned int poll_interval_ms)
{
	...
	new_odr = k303c_mag_odr_table[i].value;
	config[1] = ((ODR_MAG_MASK & new_odr) |
...
#if defined(CONFIG_SENSORS_DRV_DBG)
...
static int k303c_mag_get_data(struct k303c_mag_p *data, int *xyz)
{
	...
	xyz[0] = ((data->pdata->negate_x) ?
		(-hw_d[data->pdata->axis_map_x]) :
		(hw_d[data->pdata->axis_map_x]));
	xyz[1] = ((data->pdata->negate_y) ?
		(-hw_d[data->pdata->axis_map_y]) :
		(hw_d[data->pdata->axis_map_y]));
	xyz[2] = ((data->pdata->negate_z) ?
...
static int k303c_mag_input_init(struct k303c_mag_p *data)
{
	...
	if (err) {
		dev_err(&data->client->dev,
			...
			data->input->name);
		goto exit_failed_register_input;
	}
	...
	if (err < 0) {
		...
		goto exit_failed_create_symlink;
	}
	...
	if (err < 0) {
		...
		goto exit_failed_create_sysfs;
	}

	return 0;

exit_failed_create_sysfs:
	sensors_remove_symlink(&data->input->dev.kobj,
		data->input->name);
exit_failed_create_symlink:
	input_unregister_device(data->input);
exit_failed_register_input:
	...
	const struct i2c_device_id *id)
{
	...
	err = k303c_regulator_onoff(data, true);
	if (err < 0) {
	...
	if (err < 0) {
		...
		goto err_init_input_dev;
	}

	err = sensors_register(data->factory_device, data, sensor_attrs,
		MODULE_NAME);
	if (err < 0) {
