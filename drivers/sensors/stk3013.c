...
static void stk3013_proc_plat_data(struct stk3013_data *ps_data, struct stk3013_platform_data *plat_data)
{
	...
	if (ps_data->wait_reg < 2) {
		pr_warn("wait_reg should be larger than 2, force to write 2\n");
		ps_data->wait_reg = 2;
	} else if (ps_data->wait_reg > 0xFF) {
		pr_warn("wait_reg should be less than 0xFF, force to write 0xFF\n");
...
static int proximity_store_calibration(struct device *dev, bool do_calib)
{
	...
	if (do_calib) {
		...
		SENSOR_INFO(" ps_default_offset =  %d\n", offset_data);
...
#endif
	if (ret < 0) {
		pr_warn("request_any_context_irq(%d) failed for (%d)\n", irq, ret);
...
static int stk3013_suspend(struct device *dev)
{
	...
	if (ps_data->ps_enabled) {
		if (device_may_wakeup(&client->dev)) {
			...
			if (ret)
				pr_warn("set_irq_wake(%d) failed, err=(%d)\n", ps_data->irq, ret);
		} else {
...
static int stk3013_resume(struct device *dev)
{
	...
	if (ps_data->ps_enabled) {
		if (device_may_wakeup(&client->dev)) {
			...
			if (ret)
				pr_warn("disable_irq_wake(%d) failed, err=(%d)\n", ps_data->irq, ret);
		}
	}
...
static int check_calibration_offset(struct stk3013_data *ps_data)
{
	...
	SENSOR_INFO(" file_offset = %d, default_offset =%d\n", file_offset_data, ps_data->ps_default_offset);
