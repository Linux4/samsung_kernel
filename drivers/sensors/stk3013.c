...
#if defined(CONFIG_SENSORS_STK3013_SENSOR_EN_GPIO)
static int stk3013_setup_sensoren_gpio(struct stk3013_data *stk3013)
{
	int rc;
	struct stk3013_platform_data *pdata = stk3013->pdata;
...
static int stk3013_sensoren_gpio_onoff(struct stk3013_data *stk3013, bool onoff)
{
	struct stk3013_platform_data *pdata = stk3013->pdata;
...
#if defined(CONFIG_SENSORS_STK3013_SENSOR_EN_GPIO)
	pdata->sensoren_gpio = of_get_named_gpio_flags(np, "stk,leden_gpio"
		,0, &pdata->int_flags);
...
#if defined(CONFIG_SENSORS_STK3013_SENSOR_EN_GPIO)
	ret = stk3013_setup_sensoren_gpio(ps_data);
	if(ret < 0) {
		SENSOR_ERR("could not setup sensoren_gpio\n");
		goto err_setup_sensoren_gpio;
	}
	stk3013_sensoren_gpio_onoff(ps_data, 1);
#else
