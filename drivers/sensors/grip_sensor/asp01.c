...
struct asp01_data {
 ...
 static void grip_init_code_set(struct asp01_platform_data *pdata)
{
	pdata->cr_divsr = 10;
	pdata->cr_divnd = 12;
	pdata->cs_divsr = 10;
	...
	pdata->init_code[SET_UNLOCK] =	0x5a;
	pdata->init_code[SET_RST_ERR] =	0x33;
	pdata->init_code[SET_PROX_PER] =	0x34;
	pdata->init_code[SET_PAR_PER] =	0x34;
	pdata->init_code[SET_TOUCH_PER] =	0x3c;
	pdata->init_code[SET_HI_CAL_PER] =	0x08;
	pdata->init_code[SET_BSMFM_SET] =	0x31;
	pdata->init_code[SET_ERR_MFM_CYC] =0x33;
	pdata->init_code[SET_TOUCH_MFM_CYC] =0x25;
	pdata->init_code[SET_HI_CAL_SPD] =	0x19;
	pdata->init_code[SET_CAL_SPD] =	0x03;
	//pdata.init_code[SET_INIT_REF] =	0x00;
	pdata->init_code[SET_BFT_MOT] =	0x40;
	pdata->init_code[SET_TOU_RF_EXT] =	0x00;
	pdata->init_code[SET_SYS_FUNC] =	0x10;
	pdata->init_code[SET_OFF_TIME] =	0x50;
	pdata->init_code[SET_SENSE_TIME] =	0x50;
	pdata->init_code[SET_DUTY_TIME] =	0x50;
	pdata->init_code[SET_HW_CON1] =	0x78;
	pdata->init_code[SET_HW_CON2] =	0x27;
	pdata->init_code[SET_HW_CON3] =	0x20;
	pdata->init_code[SET_HW_CON4] =	0x27;
	pdata->init_code[SET_HW_CON5] =	0x83;
	pdata->init_code[SET_HW_CON6] =	0x3f;
	pdata->init_code[SET_HW_CON7] =	0x48;
	pdata->init_code[SET_HW_CON8] =	0x20;
	pdata->init_code[SET_HW_CON10] =	0x27;
	pdata->init_code[SET_HW_CON11] =	0x00;
}
...
static void sensor_power_on_vdd(struct asp01_data *info, int onoff)
{
	int ret;

	if (info->vdd_2p85 == NULL) {
		info->vdd_2p85 =regulator_get(&info->client->dev, "8226_l6");
		if (IS_ERR(info->vdd_2p85)){
			pr_err("%s: regulator_get failed for 8226_l6\n", __func__);
			return ;
		}
		ret = regulator_set_voltage(info->vdd_2p85, 1800000, 1800000);
		if (ret)
...
static int asp01_parse_dt(struct device *dev,
			struct  asp01_platform_data *pdata)
{
	struct device_node *np = dev->of_node;
	grip_init_code_set(pdata);
	/*irq */
    pdata->t_out = of_get_named_gpio_flags(np, "asp01,irq_gpio",
            0, &pdata->irq_gpio_flags);
	printk(KERN_INFO "%s irq is:%d \n", __func__, pdata->t_out);
	return 0;
}
#else
static int asp01_parse_dt(struct device *dev,
		       ...
		       const struct i2c_device_id *id)
{
	...
	if(client->dev.of_node) {
		pdata = devm_kzalloc (&client->dev ,
			sizeof(struct asp01_platform_data ), GFP_KERNEL);
		if(!pdata) {
		dev_err(&client->dev, "Failed to allocate memory\n");
		return -ENOMEM;
		}
		err = asp01_parse_dt(&client->dev, pdata);
		if(err)
			goto err_devicetree;
	} else
...
#ifdef CONFIG_OF
static struct of_device_id asp01_match_table[] = {
	{ .compatible = "ADS,asp01",},
	{},
};
#else
#define asp01_match_table NULL
#endif
...
static struct i2c_driver asp01_driver = {
	...
	.driver = {
		...
		.owner = THIS_MODULE,
		.name = "asp01",
		.of_match_table = asp01_match_table,
	},
