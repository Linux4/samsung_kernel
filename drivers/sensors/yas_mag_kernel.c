...
static void yas_work_func(struct work_struct *work_st)
{
	struct yas_data mag[1];
	struct yas_state *data = container_of((struct delayed_work *)work_st,
	...
	if((timestamp - data->old_timestamp > data->poll_delay * 1800000LL) &&
		(data->old_timestamp != 0)) {
		timestamp_half = (timestamp + data->old_timestamp) >> 1;
		time_hi = (int)((timestamp_half & TIME_HI_MASK) >> TIME_HI_SHIFT);
		time_lo = (int)(timestamp_half & TIME_LO_MASK);
