...
static void sensor_power_on_vdd(struct gp2a_data *info, int onoff)
{
	...
	if (info->vdd_2p85 == NULL) {
		info->vdd_2p85 = regulator_get(&info->client->dev, "8916_l17");
		if (IS_ERR(info->vdd_2p85)) {
			pr_err("%s: regulator_get failed for 8916_l17\n",
				__func__);
			return;
		}
		ret = regulator_set_voltage(info->vdd_2p85, 2850000, 2850000);
		if (ret)
			pr_err("%s: err vsensor_2p85 setting voltage ret=%d\n",
				__func__, ret);
	}
	if (info->vreg_2p8 == NULL) {
		info->vreg_2p8 = regulator_get(&info->client->dev, "8916_l6");
		if (IS_ERR(info->vreg_2p8)) {
			pr_err("%s: regulator_get failed for 8916_l6\n",
				__func__);
			return;
		}
		ret = regulator_set_voltage(info->vreg_2p8, 1800000, 1800000);
		if (ret)
			pr_err("%s: err vreg_2p8 setting voltage ret=%d\n",
				__func__, ret);
	}

	if (info->vreg5_1p8 == NULL) {
		info->vreg5_1p8 = regulator_get(&info->client->dev, "8916_l5");
		if (IS_ERR(info->vreg5_1p8)) {
			pr_err("%s: regulator_get failed for 8916_l5\n",
				__func__);
			return;
		}
		ret = regulator_set_voltage(info->vreg5_1p8, 1800000, 1800000);
		if (ret)
			pr_err("%s: err vreg_2p8 setting voltage ret=%d\n",
				__func__, ret);
	}
	...
	if (onoff == 1) {
		...
		if (ret)
			pr_err("%s: err enablinig info->vdd_2p85\n", __func__);

		ret = regulator_enable(info->vreg_2p8);
		if (ret)
			pr_err("%s: err enablinig info->vreg_2p8\n", __func__);
		ret = regulator_enable(info->vreg5_1p8);
		if (ret)
			pr_err("%s: err enablinig info->vreg5_1p8\n", __func__);
	} else if (onoff == 0) {
		if (regulator_is_enabled(info->vdd_2p85)) {
			ret = regulator_disable(info->vdd_2p85);
			if (ret)
				pr_err("%s: err vdd_2p85 disable\n", __func__);
		}
		if (regulator_is_enabled(info->vreg_2p8)) {
			ret = regulator_disable(info->vreg_2p8);
			if (ret)
				pr_err("%s: err vreg_2p8 disable\n", __func__);
		}
		if (regulator_is_enabled(info->vreg5_1p8)) {
			ret = regulator_disable(info->vreg5_1p8);
			if (ret)
				pr_err("%s: err vreg5_1p8 disable\n", __func__);
		}
	}
	msleep(30);
	return;
...
static struct attribute *gp2a_prox_attributes[] = {
	&dev_attr_prox_enable.attr, &dev_attr_prox_flush.attr, NULL
};
...
static void gp2a_work_func_prox(struct work_struct *work)
{
	...
	else if (!data->proximity_detection)
		input_report_abs(data->prox_input_dev, ABS_DISTANCE, 1);
	else
		input_report_abs(data->prox_input_dev, ABS_DISTANCE,
			data->proximity_detection);
...
static int gp2a_setup_irq(struct gp2a_data *data)
{
	...
	if (unlikely(rc < 0)) {
		pr_err("%s:  gpio_vled_en gpio %d request failed (%d)\n",
	...
	if (unlikely(rc < 0)) {
		pr_err("%s:  gpio_vled_en failed to set gpio %d as input (%d)\n",
			__func__, data->vled_gpio, rc);
