...
static void sx9320_send_event(struct sx9320_p *data, int ch, u8 state)
{
...
#ifdef CONFIG_SENSORS_SX9320_2CH
static void sx9320_get_data_ch2(struct sx9320_p *data)
{
	u8 ms_byte = 0;
	u8 ls_byte = 0;
	s16 avg = 0;
	...
	sx9320_i2c_write(data, SX9320_REGSENSORSELECT, REF_SENSOR);
	...
	capMain = (((s32)ms_byte * 21234) + ((s32)ls_byte * 496))
			+ ((useful * range) / ((gain_ch2 * again * 65536) / 1000));
	...
	data->useful_ch2 = useful;
	data->offset_ch2 = offset;
	data->capMain_ch2= capMain;
	data->avg_ch2 = avg;
	data->diff_ch2 = useful - avg;
...
#ifdef CONFIG_SENSORS_SX9320_2CH
		ret = sx9320_i2c_write(data, SX9320_GNRLCTRL1_REG,
			setup_reg[2].val | (1 << data->phen) | (1 << REF_SENSOR));		
#else
		ret = sx9320_i2c_write(data, SX9320_GNRLCTRL1_REG,
...
static void sx9320_check_status(struct sx9320_p *data, int enable)
{
	...
	if (status & (PHX_STATUS_REG << data->phen)) {
		sx9320_send_event(data, data->phen, ACTIVE);
	} else {
		sx9320_send_event(data, data->phen, IDLE);
	}

#ifdef CONFIG_SENSORS_SX9320
...
#ifdef CONFIG_SENSORS_SX9320_2CH
static ssize_t sx9320_ch_count_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
		...
		struct device_attribute *attr, char *buf)
{
	...
	case 0x03:
	case 0x04:
		ret = snprintf(buf, PAGE_SIZE, "x%d\n", 1 << (gain_ch2 - 1));
		break;
	default:
		ret = snprintf(buf, PAGE_SIZE, "Reserved\n");
		break;
...
static ssize_t sx9320_raw_data_ch2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	static int sum_diff;
	struct sx9320_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9320_normal_ch2_threshold_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9320_p *data = dev_get_drvdata(dev);
	u8 hysteresis = 0;
...
static ssize_t sx9320_diff_avg_ch2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9320_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9320_useful_avg_ch2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9320_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9320_sampling_freq_ch2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9320_p *data = dev_get_drvdata(dev);
	u8 sampling_freq_ch2;
	...
	sx9320_i2c_read(data, SX9320_AFECTRL4_REG, &sampling_freq_ch2);
	sampling_freq_ch2 = (sampling_freq_ch2 >> 3) & 0x1f;
...
#ifdef CONFIG_SENSORS_SX9320_2CH
static DEVICE_ATTR(ch_count, 0444, sx9320_ch_count_show, NULL);
static DEVICE_ATTR(gain_ch2, 0444, sx9320_gain_ch2_show, NULL);
static DEVICE_ATTR(raw_data_ch2, 0444, sx9320_raw_data_ch2_show, NULL);
static DEVICE_ATTR(normal_threshold_ch2, 0444,
		sx9320_normal_ch2_threshold_show, NULL);
...
static void sx9320_touch_process(struct sx9320_p *data, u8 flag)
{
	...
	if (data->state_ch1 == IDLE) {
		if (status & (PHX_STATUS_REG << data->phen))
			sx9320_send_event(data, MAIN_SENSOR, ACTIVE);
		else
			pr_info("[SX9320]: %s - 0x%x already released\n",
				__func__, status);
	} else {
		if (!(status & (PHX_STATUS_REG << data->phen)))
			sx9320_send_event(data, MAIN_SENSOR, IDLE);
		else
