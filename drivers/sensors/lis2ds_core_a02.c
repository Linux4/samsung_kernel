...
#define ACCEL_IDENTICAL_DATA_CHECK_TIME_MS   1000    /* 1000 ms */
#define ACCEL_POWER_OFF_DELAY_US             10000   /* 10 ms */
#define ACCEL_POWER_ON_DELAY_US              50000   /* 50 ms */
#define ACCEL_SW_RESET_DELAY_US              50000   /* 50 ms */
...
#define TILT_DEGREE_MAX_CNT	(21)
#define TILT_DEGREE_TB_BASE	(25)
#define TILT_DEGREE_25	(25)
#define TILT_DEGREE_45	(45)
u8 degree_tb[TILT_DEGREE_MAX_CNT] = {
	0x74, 0x73, 0x72, 0x71, 0x70,
	0x6F, 0x6D, 0x6C, 0x6B, 0x6A,
	0x69, 0x68, 0x66, 0x65, 0x64,
	0x62, 0x61, 0x5F, 0x5E, 0x5C, 0x5B
};

#define PROTOS_MOTION_MAGIC_CODE	7700000L
...
static int motor_state = MOTOR_OFF;

int accel_motor_notify(struct notifier_block *nb,
	unsigned long enable, void *v)
{
	struct vib_notifier_context *vib = (struct vib_notifier_context *)v;

	SENSOR_INFO("%s: %s, idx: %d timeout: %d\n",
		__func__, enable ? "ON" : "OFF", vib->index, vib->timeout);

	motor_state = (int)enable;

	return 0;
}
#endif
static int lis2ds_set_tilt_degree(struct lis2ds_data *cdata) {
	u8 value = 0x00;

	if (cdata->tilt_delay == PROTOS_MOTION_MAGIC_CODE) {
		SENSOR_INFO("tilt degree : protos motion(12)\n");
		// Set 12 degree
		value = 0x7D;
	} else if ((cdata->tilt_degree < TILT_DEGREE_25) || (cdata->tilt_degree > TILT_DEGREE_45)) {
		SENSOR_INFO("tilt degree : default(35)\n");
		// Set 35 degree
		value = 0x69;
	} else
		value = degree_tb[cdata->tilt_degree - TILT_DEGREE_TB_BASE];

	return value;
}

static int lis2ds_set_tilt_variant(struct lis2ds_data *cdata) {
	int err;
	u8 data = 0;

	// using hidden register
	err = cdata->tf->read(cdata, 0x21, 1, &data, true);
	//SENSOR_INFO("tilt varian(%d): read 0x21: 0x%02x\n", err, data);
	data |= 0x10;
	cdata->tf->write(cdata, 0x21, 1, &data, true);
	//SENSOR_INFO("tilt varian(%d): write 0x21: 0x%02x\n", err, data);

	// Minimum angle is 12 degree. value is 0x7D.
	err = cdata->tf->read(cdata, 0x36, 1, &data, true);
	//SENSOR_INFO("tilt varian(%d): read 0x36: 0x%02x\n", err, data);
	data = lis2ds_set_tilt_degree(cdata);
	cdata->tf->write(cdata, 0x36, 1, &data, true);
	//SENSOR_INFO("tilt varian(%d): write 0x36: 0x%02x\n", err, data);

	// latency set to 40ms
	err = cdata->tf->read(cdata, 0x37, 1, &data, true);
	//SENSOR_INFO("tilt varian(%d): read 0x37: 0x%02x\n", err, data);
	if (cdata->tilt_delay == PROTOS_MOTION_MAGIC_CODE)
		data = 0x00;
	else
		data = 0x32;
	cdata->tf->write(cdata, 0x37, 1, &data, true);
	//SENSOR_INFO("tilt varian(%d): write 0x37: 0x%02x\n", err, data);

	err = cdata->tf->read(cdata, 0x3F, 1, &data, true);
	//SENSOR_INFO("tilt varian(%d): read 0x3F: 0x%02x\n", err, data);
	data &= 0xEF;
	cdata->tf->write(cdata, 0x3F, 1, &data, true);
	//SENSOR_INFO("tilt varian(%d): write 0x3F: 0x%02x\n", err, data);
	SENSOR_INFO("tilt varian done\n");
...
static void lis2ds_check_chip_reset(struct lis2ds_data *cdata) {

	int delay_ms;
	
	delay_ms = (int)ktime_to_ms(cdata->acc_delay);

	if (cdata->accel_data[0] == cdata->accel_prev_data[0] &&
		cdata->accel_data[1] == cdata->accel_prev_data[1] &&
		cdata->accel_data[2] == cdata->accel_prev_data[2]) {

		cdata->accel_data_identical_count--;
		if (cdata->accel_data_identical_count < 0)
			cdata->accel_data_identical_count = -1;

		SENSOR_INFO("accel data identical count = %d\n", cdata->accel_data_identical_count);
		SENSOR_INFO("x = %d (%d), y = %d (%d), z = %d (%d)\n", cdata->accel_data[0], cdata->accel_prev_data[0], 
				cdata->accel_data[1], cdata->accel_prev_data[1], cdata->accel_data[2], cdata->accel_prev_data[2]);
	} else {
		cdata->accel_data_identical_count = ACCEL_IDENTICAL_DATA_CHECK_TIME_MS / delay_ms;
	}

	// For continuous 1 second, all of accel data is identical to previous data
	// Perform hardware and software reset to try to fix it
	if (cdata->accel_data_identical_count == 0) {
		SENSOR_INFO("Sensor data stuck. Triggering Chip Reset");
		schedule_work(&cdata->acc_chip_reset_work);
	}

	cdata->accel_prev_data[0] = cdata->accel_data[0];
	cdata->accel_prev_data[1] = cdata->accel_data[1];
	cdata->accel_prev_data[2] = cdata->accel_data[2];
}

// This function performs hardware and software reset
// Disable Sensor -> Hardware Reset -> Software Reset -> Init Register -> Enable Sensor
static void lis2ds_chip_reset_work_func(struct work_struct *work)
{
	struct lis2ds_data *cdata =
		container_of(work, struct lis2ds_data, acc_chip_reset_work);
	...
	for (i = 0; i < LIS2DS_SENSORS_NUMB; i++) {
		cdata->sensors[i].prev_enabled = cdata->sensors[i].enabled;
		if(cdata->sensors[i].enabled) {
			SENSOR_INFO("Disabled sensor = %d", i);
			lis2ds_disable_sensors(cdata, i);
		}
	}

	// Hardware Reset
	SENSOR_INFO("Hardware Reset start");
	lis2ds_vdd_onoff(cdata,OFF);
	lis2ds_vdd_onoff(cdata,ON);
	SENSOR_INFO("Hardware Reset done");

	// Software Reset and Init registers
	SENSOR_INFO("Software Reset start");
	lis2ds_init_sensors(cdata);
	SENSOR_INFO("Software Reset done");

	// Enable Sensors
	for (i = 0; i < LIS2DS_SENSORS_NUMB; i++) {
		if(cdata->sensors[i].prev_enabled) {
			SENSOR_INFO("Enabled sensor = %d", i);
			lis2ds_enable_sensors(cdata, i);
		}
	}
...
#ifdef CONFIG_SEC_VIB_NOTIFIER
	if (motor_state >= MOTOR_ON && cdata->lpf_on) {
		if (motor_state == MOTOR_VIB) {
			cdata->accel_data[0] = ((cdata->accel_prev_data[0] / 100) * 95) + ((cdata->accel_data[0] / 100) * 5);
			cdata->accel_data[1] = ((cdata->accel_prev_data[1] / 100) * 95) + ((cdata->accel_data[1] / 100) * 5);
			cdata->accel_data[2] = ((cdata->accel_prev_data[2] / 100) * 95) + ((cdata->accel_data[2] / 100) * 5);
		} else {
			motor_state = MOTOR_VIB;
		}
	}
#endif
	lis2ds_check_chip_reset(cdata);
...
static irqreturn_t lis2ds_thread_fn(int irq, void *private)
{
	...
	SENSOR_INFO("irq=%d, cdata->irq=%d\n", irq, cdata->irq);

	if (cdata->accel_data_identical_count == 0)
		SENSOR_INFO("Skip IRQ. Accel Chip Reset Ongoing");
	else
...
static int lis2ds_init_sensors(struct lis2ds_data *cdata)
{
	...
	usleep_range(ACCEL_SW_RESET_DELAY_US, ACCEL_SW_RESET_DELAY_US);

	// Check SW Reset done or not
	err = cdata->tf->read(cdata, LIS2DS_CTRL2_ADDR, 1, &val, true);
	if (val & 0xC0) {
		SENSOR_INFO("Software Reset Failed, val = %d", val);
	} else {
		SENSOR_INFO("Software Reset Success, val = %d", val);
	}
		...
		const char *buf, size_t count)
{
	...
	if (enable) {
		cdata->tilt_enable_count++;
		SENSOR_INFO("enable tilt count:(%d)\n", cdata->tilt_enable_count);
		if (cdata->tilt_enable_count == 1) {
			err = lis2ds_enable_sensors(cdata, LIS2DS_TILT);
			lis2ds_set_irq(cdata, 1);
		}
	} else {
		if (cdata->tilt_enable_count != 0) {
			cdata->tilt_enable_count--;
			if (cdata->tilt_enable_count == 0) {
				lis2ds_set_irq(cdata, 0);
				err = lis2ds_disable_sensors(cdata, LIS2DS_TILT);
			}
			SENSOR_INFO("disable tilt count:(%d)\n", cdata->tilt_enable_count);
		}
	}
	mutex_unlock(&cdata->mutex_enable);
	return count;
...
static ssize_t lis2ds_tilt_delay_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct lis2ds_data *cdata = input_get_drvdata(input);
...
static ssize_t lis2ds_tilt_delay_store(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	struct input_dev *input = to_input_dev(dev);
	...
	cdata->tilt_delay = (int)data;

	SENSOR_INFO("tilt delay = %d\n", cdata->tilt_delay);

	err = lis2ds_set_tilt_variant(cdata);
...
static struct device_attribute dev_attr_tilt_poll_delay =
	__ATTR(poll_delay, 0644,
	lis2ds_tilt_delay_show,
	lis2ds_tilt_delay_store);
...
/* Tilt Degree */
static ssize_t lis2ds_tilt_degree_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct lis2ds_data *cdata = input_get_drvdata(input);
...
static ssize_t lis2ds_tilt_degree_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct input_dev *input = to_input_dev(dev);
	struct lis2ds_data *cdata = input_get_drvdata(input);
	...
	cdata->tilt_degree = result;
	lis2ds_set_tilt_variant(cdata);
	SENSOR_INFO("tilt degree = %d\n", cdata->tilt_degree);

	return size;
}

static ssize_t lis2ds_power_control_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct lis2ds_data *cdata = input_get_drvdata(input);

	SENSOR_INFO("power_control = %d\n", cdata->power_control);

	return snprintf(buf, PAGE_SIZE, "%d\n", cdata->power_control);
}

static ssize_t lis2ds_power_control_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct input_dev *input = to_input_dev(dev);
	struct lis2ds_data *cdata = input_get_drvdata(input);
	...
	cdata->power_control = result;
	SENSOR_INFO("power_control = %d\n", cdata->power_control);

	if (cdata->power_control == 1) {
		lis2ds_vdd_onoff(cdata, OFF);
		lis2ds_vdd_onoff(cdata, ON);
	} else if (cdata->power_control == 2) {
		lis2ds_vdd_onoff(cdata, OFF);
	} else if (cdata->power_control == 3) {
		lis2ds_vdd_onoff(cdata, ON);
	}
...
static DEVICE_ATTR(tilt_degree, 0644,
				lis2ds_tilt_degree_show,
				lis2ds_tilt_degree_store);

static DEVICE_ATTR(power_control, 0644,
				lis2ds_power_control_show,
				lis2ds_power_control_store);
...
static u32 lis2ds_parse_dt(struct lis2ds_data *cdata)
{
	...
	if (cdata->lis2ds_ldo_pin < 0) {
		...
		SENSOR_ERR("Set vdd_ldo_pin through DTSI ldo_pin %d\n", cdata->lis2ds_ldo_pin);
...
static int lis2ds_vdd_onoff(struct lis2ds_data *cdata, int onoff)
{
	if (cdata->lis2ds_ldo_pin) {
		...
		SENSOR_INFO("Accel Power %d", onoff);
		if (onoff) {
			// 20 ms is ideal value but lets keep 50 ms for safer side
			usleep_range(ACCEL_POWER_ON_DELAY_US, ACCEL_POWER_ON_DELAY_US);
		} else {
			usleep_range(ACCEL_POWER_OFF_DELAY_US, ACCEL_POWER_OFF_DELAY_US);
		}
	}
...
int lis2ds_common_probe(struct lis2ds_data *cdata)
{
	u8 wai = 0;
	int retry = 5;
	int err = -ENODEV;
	...
	while (retry--) {
		...
		if (wai != LIS2DS_WHO_AM_I_DEF) {
			...
			SENSOR_ERR("Chip Found, WhomAmI = %d\n", wai);
			break;
		}
	}
	...
	cdata->accel_prev_data[0] = 0;
	cdata->accel_prev_data[1] = 0;
	cdata->accel_prev_data[2] = 0;
	cdata->lpf_on = ON;
#ifdef CONFIG_SEC_VIB_NOTIFIER
	motor_nb.notifier_call = accel_motor_notify;
	motor_nb.priority = 1;
	sec_vib_notifier_register(&motor_nb);
#endif
	...
	INIT_WORK(&cdata->acc_chip_reset_work, lis2ds_chip_reset_work_func);
	...
	if (cdata->irq > 0) {
		...
		err = request_threaded_irq(cdata->irq, lis2ds_thread_fn, NULL,
