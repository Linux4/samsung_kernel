...
#define SENSOR_ATTR_SIZE 61
#else
#define SENSOR_ATTR_SIZE 46
#endif
#endif
...
#ifdef CONFIG_SENSORS_GRIP_HAND_POSITION
...
static void isg6320_enter_error_mode(struct isg6320_data *data, enum grip_error_state err_state)
{
...
static int isg6320_i2c_write(struct isg6320_data *data, u8 cmd, u8 val)
{
	...
	if (ret < 0) {
		if (data->i2c_fail_count < 3)
			data->i2c_fail_count++;
		if (data->i2c_fail_count >= 3)
			isg6320_enter_error_mode(data, FAIL_I2C_READ_3_TIMES);
...
#if defined(CONFIG_USE_MULTI_CHANNEL) && defined(CONFIG_SENSORS_GRIP_HAND_POSITION)
static bool update_hand_pos(struct isg6320_data *data)
{
	struct hand_position *hand = data->hand;
	int prev_hand_pos = hand->hand_pos;
	bool is_changed = false;
	pr_info("[GRIP_%d] update_hand_pos\n", data->ic_num); //TODO delete
	mutex_lock(&hand->hand_lock);

	if ((data->state == FAR) && (data->mul_ch->state_b == FAR)) {
		hand->hand_pos = NONE;
	} else {
		if (data->state == CLOSE) {
			if (data->mul_ch->state_b == CLOSE) {
				if (hand->hand_detect_a) {
					if (hand->hand_detect_b)
						hand->hand_pos = hand->hand_priority;
					else
						hand->hand_pos = RIGHT;
				} else {
					if (hand->hand_detect_b)
	...
	if (prev_hand_pos != hand->hand_pos) {
		is_changed = true;
	} else {
		pr_info("[GRIP_%d] %s skip hand pos (not changed)\n", data->ic_num, __func__);//TODO delete
	}

	mutex_unlock(&hand->hand_lock);
	pr_info("[GRIP_%d] update_hand_pos end\n", data->ic_num); //TODO delete
	return is_changed;
}
#endif
	...
	if (state_a == CLOSE) {
		if (data->state == FAR) {
			...
			if (data->ic_num == MAIN_GRIP)
				if (data->diff >= hand->hand_thd_a)
					hand->hand_detect_a = true;
#endif
		}
	} else {
		if (data->state == CLOSE) {
...
#ifdef CONFIG_SENSORS_GRIP_HAND_POSITION
				if (data->ic_num == MAIN_GRIP) {
					if (data->mul_ch->diff_b >= hand->hand_thd_b) {
						hand->hand_detect_b = true;
					}
				}
#endif
			}
...
#ifdef CONFIG_SENSORS_GRIP_HAND_POSITION
	if (data->ic_num == MAIN_GRIP) {
		if (update_hand_pos(data)) {
			pr_info("[GRIP_%d] IRQ HAND POSITON : %s\n", data->ic_num, hand_name[hand->hand_pos]);
			input_report_rel(data->input_dev, REL_HWHEEL, NONE);
			input_sync(data->input_dev);
			input_report_rel(data->input_dev, REL_HWHEEL, hand->hand_pos);
		} else {
			pr_info("[GRIP_%d] IRQ HAND POSITON : SKIP (%s)\n", data->ic_num, hand_name[hand->hand_pos]);
			input_report_rel(data->input_dev, REL_HWHEEL, SKIP);
		}
	}
#endif
#endif
...
static void mcc_work_func(struct work_struct *work)
{
	...
	ret = isg6320_i2c_write(data, ISG6320_A_PROXCTL4_REG, threshold);
	if (ret < 0)
		goto exit_mcc_work_func;
	ret = isg6320_i2c_write(data, ISG6320_A_PROXCTL8_REG, hysteresis);
	if (ret < 0)
		goto exit_mcc_work_func;
...
exit_mcc_work_func:
	mutex_unlock(&data->lock);
	if (ret < 0) {
		isg6320_enter_error_mode(data, FAIL_MCC_RESET);
...
static void isg6320_initialize(struct isg6320_data *data)
{
	...
	if (data->setup_reg_exist) {
		...
		for (; i < data->reg_size ; i++) {
			...
			if (ret < 0 && retry == 0) {
				...
				isg6320_enter_error_mode(data, FAIL_SETUP_REGISTER);
				goto exit_initialize;
			}
		}
	}
	...
	if (data->normal_th > 0) {
		...
		ret = isg6320_i2c_write(data, ISG6320_A_PROXCTL4_REG, val);
		if (ret < 0) {
...
#if !defined(CONFIG_SEC_FACTORY) && defined(CONFIG_SUPPORT_MCC_THRESHOLD_CHANGE)
	if (data->ic_num == MAIN_GRIP) {
		ret = isg6320_i2c_read(data, ISG6320_A_PROXCTL4_REG, &val, 1);
		if (ret >= 0)
			data->default_threshold = val;
		if (data->mcc_threshold == 0)
			data->mcc_threshold = data->default_threshold;

		ret = isg6320_i2c_read(data, ISG6320_A_PROXCTL8_REG, &val, 1);
		if (ret >= 0)
	...
	if (data->multi_use) {
		if (data->mul_ch->normal_th_b > 0) {
			...
			ret = isg6320_i2c_write(data, ISG6320_B_PROXCTL4_REG, val);
			if (ret < 0) {
				pr_err("[GRIP_%d] %s PROXCTL write fail\n", data->ic_num, __func__);
				goto exit_initialize;
			}
...
static void isg6320_set_enable(struct isg6320_data *data, int enable)
{
	...
	if (data->check_abnormal_working == true) {
		...
		if (enable == ON) {
			...
			isg6320_enter_error_mode(data, FAIL_UPDATE_PREV_STATE);
		}
	...
	if (enable == ON) {
		...
		if (data->check_abnormal_working == true || data->initialized == OFF) {
			data->enable = enable;
			isg6320_enter_error_mode(data, FAIL_I2C_ENABLE);
...
#ifdef CONFIG_SENSORS_GRIP_HAND_POSITION
			if (data->ic_num == MAIN_GRIP) {
				if (data->diff >= data->hand->hand_thd_a)
					data->hand->hand_detect_a = true;
				else
					data->hand->hand_detect_a = false;
				if (data->mul_ch->diff_b >= data->hand->hand_thd_b)
					data->hand->hand_detect_b = true;
				else
					data->hand->hand_detect_b = false;

				update_hand_pos(data); //first data have to report regardless of the result.
				input_report_rel(data->input_dev, REL_HWHEEL, NONE);
				input_sync(data->input_dev);
				input_report_rel(data->input_dev, REL_HWHEEL, data->hand->hand_pos);
				pr_info("[GRIP_%d] FIRST HAND POSITON : %s\n", data->ic_num, hand_name[data->hand->hand_pos]);//TODO delete
			}
#endif
#endif
			input_report_rel(data->input_dev, REL_X, data->is_unknown_mode);
...
#ifdef CONFIG_USE_MULTI_CHANNEL
#ifdef CONFIG_SENSORS_GRIP_HAND_POSITION
static ssize_t isg6320_hand_thd_a_show(struct device *dev,
					struct device_attribute *attr, char *buf)
{
	struct isg6320_data *data = dev_get_drvdata(dev);
	if (data->ic_num == MAIN_GRIP) {
		return sprintf(buf, "%d\n", data->hand->hand_thd_a);
	}

	return sprintf(buf, "-1\n");
}

static ssize_t isg6320_hand_thd_b_show(struct device *dev,
					struct device_attribute *attr, char *buf)
{
	struct isg6320_data *data = dev_get_drvdata(dev);

	if (data->ic_num == MAIN_GRIP) {
		return sprintf(buf, "%d\n", data->hand->hand_thd_b);
	}

	return sprintf(buf, "-1\n");
}

static ssize_t isg6320_hand_thd_a_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	int val = 0;
	struct isg6320_data *data = dev_get_drvdata(dev);
	...
	sscanf(buf, "%d", &val);
	if (val < 0) {
		pr_err("[GRIP_%d] %s - invalid arg\n", data->ic_num, __func__);
		return size;
	}
	if (data->ic_num == MAIN_GRIP) {
		data->hand->hand_thd_a = val;
		if (update_hand_pos(data)) {
			pr_info("[GRIP_%d] IRQ HAND POSITON : %s\n", data->ic_num, hand_name[data->hand->hand_pos]);
			input_report_rel(data->input_dev, REL_HWHEEL, NONE);
			input_sync(data->input_dev);
			input_report_rel(data->input_dev, REL_HWHEEL, data->hand->hand_pos);
			input_sync(data->input_dev);
...
static ssize_t isg6320_hand_thd_b_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	int val = 0;
	struct isg6320_data *data = dev_get_drvdata(dev);
...
static ssize_t isg6320_hand_position_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct isg6320_data *data = dev_get_drvdata(dev);

	if (data->ic_num == MAIN_GRIP) {
		return sprintf(buf, "%s\n", hand_name[data->hand->hand_pos]);
	} else {
		return sprintf(buf, "%s\n", "NOT SUPPORT");
	}
}
...
#ifdef CONFIG_USE_MULTI_CHANNEL
#ifdef CONFIG_SENSORS_GRIP_HAND_POSITION
static DEVICE_ATTR(hand_thd_a, 0664, isg6320_hand_thd_a_show, isg6320_hand_thd_a_store);
static DEVICE_ATTR(hand_thd_b, 0664, isg6320_hand_thd_b_show, isg6320_hand_thd_b_store);
static DEVICE_ATTR(hand_position, 0444, isg6320_hand_position_show, NULL);
#endif
...
#endif
#if defined(CONFIG_USE_MULTI_CHANNEL) && defined(CONFIG_SENSORS_GRIP_HAND_POSITION)
static int debug_hand_pos(struct isg6320_data *data)
{
	struct hand_position *hand = data->hand;
	bool is_changed = false;
	...
	if ((data->state == CLOSE) && (data->mul_ch->state_b == CLOSE)) {
		bool detect_a = (data->diff > hand->hand_thd_a) ? true : false;
		bool detect_b = (data->mul_ch->diff_b > hand->hand_thd_b) ? true : false;

		if (hand->hand_detect_a && hand->hand_detect_b) { //HH?’HW/WH/WW
			if (detect_a && !detect_b) {
				if (hand->hand_priority != RIGHT) {
					is_changed = true;
					hand->hand_pos = RIGHT;
				}
			} else if (!detect_a && detect_b) {
				if (hand->hand_priority != LEFT) {
					is_changed = true;
					hand->hand_pos = LEFT;
				}
			} else if (!detect_a && !detect_b) {
				//if priority is same with HH and WW, not changed.(TBD)
			}
		} else if (hand->hand_detect_a && !hand->hand_detect_b) { //HW?’HH/WH/WW
			if (detect_a && detect_b) {
				is_changed = true;
				hand->hand_pos = hand->hand_priority;
			} else if (!detect_a && detect_b) {
		...
		} else if (!hand->hand_detect_a && hand->hand_detect_b) { //WH?’HH/HW/WW
			if (detect_a && detect_b) {
				is_changed = true;
				hand->hand_pos = hand->hand_priority;
			} else if (detect_a && !detect_b) {
		...
		hand->hand_detect_a = detect_a;
		hand->hand_detect_b = detect_b;
	} else if ((data->state == CLOSE) && (data->mul_ch->state_b == FAR)) {
		if (hand->hand_detect_a == true) {
			if (data->diff < hand->hand_thd_a)
				hand->hand_detect_a = false;
		} else {
			if (data->diff >= hand->hand_thd_a)
	...
	} else if ((data->state == FAR) && (data->mul_ch->state_b == CLOSE)) {
		if (hand->hand_detect_b == true) {
			if (data->mul_ch->diff_b < hand->hand_thd_b)
				hand->hand_detect_b = false;
		} else {
			if (data->mul_ch->diff_b >= hand->hand_thd_b)
...
static void invalid_cfcal_work_func(struct work_struct *work)
{
	...
	if (ret < 0) {
		if (data->reset_fail_cnt < 5)
			data->reset_fail_cnt++;
		if (data->reset_fail_cnt >= 2) {
			isg6320_enter_error_mode(data, FAIL_RESET);
...
static void isg6320_check_first_working(struct isg6320_data *data, int channel_num)
{
	...
	if (data->noti_enable && data->motion) {
		if (channel_num == 1) {
			if (data->normal_th != 0 && data->normal_th < data->diff) {
				if (!data->first_working) {
					data->first_working = true;
			...
			if (data->multi_use) {
				if (data->mul_ch->normal_th_b != 0 && data->mul_ch->normal_th_b < data->mul_ch->diff_b) {
					if (!data->mul_ch->first_working) {
						data->mul_ch->first_working = true;
...
#if defined(CONFIG_USE_MULTI_CHANNEL) && defined(CONFIG_SENSORS_GRIP_HAND_POSITION)
	if ((data->ic_num == MAIN_GRIP) && !data->skip_data) {
		if (debug_hand_pos(data)) {
			pr_info("[GRIP_%d] DEBUG HAND POSITON : %s\n", data->ic_num, hand_name[data->hand->hand_pos]);
			input_report_rel(data->input_dev, REL_HWHEEL, NONE);
			input_sync(data->input_dev);
			input_report_rel(data->input_dev, REL_HWHEEL, data->hand->hand_pos);
			input_sync(data->input_dev);
...
#ifdef CONFIG_SENSORS_GRIP_HAND_POSITION
	if (data->ic_num == MAIN_GRIP) {
		ret = of_property_read_u32(node, "isg6320,hand_thd_a", &data->hand->hand_thd_a);
		if (ret < 0)
			pr_err("[GRIP_%d] hand threshold A set err\n", data->ic_num);
		ret = of_property_read_u32(node, "isg6320,hand_thd_b", &data->hand->hand_thd_b);
		if (ret < 0)
			pr_err("[GRIP_%d] hand threshold B set err\n", data->ic_num);
		ret = of_property_read_u32(node, "isg6320,hand_priority", &data->hand->hand_priority);////1ch or 2ch (1 or 2)
		if (ret < 0)
			pr_err("[GRIP_%d] hand priority set err\n", data->ic_num);
		else
			data->hand->hand_priority += NONE;
	}
#endif
#else
	data->multi_use = 0;
...
#ifdef CONFIG_SENSORS_GRIP_HAND_POSITION
	if (data->ic_num == MAIN_GRIP) {
		data->hand = kzalloc(sizeof(struct hand_position), GFP_KERNEL);
		if (!data->hand) {
			pr_err("[GRIP_%d] hand position alloc failed", data->ic_num);
		} else {
			pr_info("[GRIP_%d] use hand position", data->ic_num);
		}
	}
#endif
#endif
