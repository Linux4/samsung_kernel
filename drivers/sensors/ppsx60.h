...
#define CTL0_SW_RESET			0x08
#define PPS_LIB_VER 20

#define EOL_DELAY            5000000LL
...
struct ppsX60_device_data {
	...
	u32 led_current;
	int hrm_threshold;
	struct hrtimer eol_timer;
	ktime_t poll_delay;
	struct workqueue_struct *eol_wq;
	struct work_struct work_eol;
	...
	int64_t eol_sum[2];
	int eol_freq_result;
	int eol_lowir_result;
	int eol_lowred_result;
	int eol_highir_result;
	int eol_highred_result;
	int eol_ir_ac_result;
	int eol_red_ac_result;
	int eol_ir_acdc_result;
	int eol_red_acdc_result;
	int eol_absorb_result;
	int selftest_result;
	int eol_irhigh[101];
	int eol_redhigh[101];
	int64_t eol_irnoise_result;
	int64_t eol_rednoise_result;
	u8 eol_test_status;
	u64 old_timestamp;
};
/*
