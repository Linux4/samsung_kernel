...
static void bma2x2_work_func(struct work_struct *work)
{
	...
	queue_delayed_work_on(get_low_pwr_cpu(), bma2x2->wq, &bma2x2->work, delay);
}
#endif
...
#ifndef CONFIG_BMA_ENABLE_NEWDATA_INT
		queue_delayed_work_on(get_low_pwr_cpu(), bma2x2->wq, &bma2x2->work,
...
static void bma2x2_irq_work_func(struct kthread_work *work)
{
	struct bma2x2_data *bma2x2 = container_of((struct kthread_work *)work,
...
static irqreturn_t bma2x2_irq_handler(int irq, void *handle)
{
	...
	queue_kthread_work(&data->irq_worker, &data->irq_work);
		...
		const struct i2c_device_id *id)
{
	...
	init_kthread_worker(&data->irq_worker);
	data->irq_worker_task = kthread_run(kthread_worker_fn, &data->irq_worker,
                                           "bma2x2_acce__irq_work");
	if (IS_ERR(data->irq_worker_task)) {
		pr_err("Could not create irq_work task\n");
		goto exit_kthread;
	}
	init_kthread_work(&data->irq_work, bma2x2_irq_work_func);
#endif

#ifndef CONFIG_BMA_ENABLE_NEWDATA_INT
	INIT_DELAYED_WORK(&data->work, bma2x2_work_func);
	data->wq = create_singlethread_workqueue("bma2x2_work_func_wq");
	if (!data->wq) {
		err = -ENOMEM;
...
exit_workqueue:
	 kthread_stop(data->irq_worker_task);
	 input_free_device(dev);
exit_kthread:
kfree_exit:
...
static int bma2x2_resume(struct device *dev)
{
	...
	if (atomic_read(&data->enable) == 1) {
		bma2x2_set_mode(data->bma2x2_client,
			BMA2X2_MODE_NORMAL, BMA_ENABLED_INPUT);
#ifndef CONFIG_BMA_ENABLE_NEWDATA_INT
		queue_delayed_work_on(get_low_pwr_cpu(), data->wq, &data->work,
