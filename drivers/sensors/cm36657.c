...
static int cm36657_control_and_report(struct cm36657_data *cm36657, uint8_t mode)
{
	...
	if (mode == CONTROL_PS) {
		...
		if (int_flag & INT_FLAG_PS_IF_CLOSE) {
			...
			cm36657_get_ps_adc_value(cm36657, &ps_data);
			mutex_unlock(&cm36657->read_lock);
			if (cm36657->rf_noise_defence)
				cm36657->prox_count = 0;
			...
			if (cm36657->rf_noise_defence) {
				if (ps_data == 0) {
					cm36657->prox_count++;
					if (cm36657->prox_count == 10) {
						for (reg = 0x03; reg <= 0x07; reg++) {
			...
			if (ps_data == 0 || ((cm36657->cur_lv == 2) && (ps_data < cm36657_thd_tbl[2][0])))
				cm36657_get_canc_ps(cm36657, 0);
		} else {
			mutex_lock(&cm36657->read_lock);
			cm36657_get_ps_adc_value(cm36657, &ps_data);
			mutex_unlock(&cm36657->read_lock);
			SENSOR_ERR("Unknown event, int_flag=0x%4x, ps_data = %d\n", int_flag, ps_data);
			if (cm36657->rf_noise_defence) {
				cm36657->prox_count++;
				if (cm36657->prox_count == 10) {
					for (reg = 0x03; reg <= 0x07; reg++) {
	...
	if ((direction && (ps_data < cm36657_thd_tbl[cm36657->cur_lv][0]))
		|| ((!direction) && (ps_data > cm36657_thd_tbl[cm36657->cur_lv][1]))
		|| (mode == CONTROL_PS))
#endif
	{
...
irqreturn_t cm36657_irq_handler(int irq, void *data)
{
	...
	if (cm36657->rf_noise_defence)
		SENSOR_INFO("noise reset count : %d\n", cm36657->reset_count);
	else
		SENSOR_INFO("!!\n");
...
static int cm36657_setup_irq(struct cm36657_data *cm36657)
{
	...
	rc = request_any_context_irq(cm36657->irq, cm36657_irq_handler,
			IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "cm36657", cm36657);
...
static int cm36657_parse_dt(struct cm36657_data *cm36657, struct device *dev)
{
	...
	cm36657->irq_gpio = of_get_named_gpio_flags(np, "cm36657,irq_gpio", 0, &flags);
	...
	if (of_property_read_u32(np, "cm36657,rf_noise_defence",
		&temp) < 0)
		SENSOR_INFO("Don't need rf_noise_defence_code\n");
	else
		cm36657->rf_noise_defence = temp;
	...
	if (of_property_read_u32(np, "cm36657,ps_conf1_reg",
		&temp) < 0) {
	...
	if (of_property_read_u32(np, "cm36657,ps_conf3_reg",
		&temp) < 0) {
	...
	const struct i2c_device_id *id)
{
	...
	cm36657->prox_avg_wq = create_singlethread_workqueue("cm36657_prox_avg_wq");
