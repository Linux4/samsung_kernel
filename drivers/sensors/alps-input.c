...
*/
	case ALPSIO_SET_DELAY:
		...
		if ( data->flgM ) {
			hscd_activate(1, data->flgM, data->delay);
			/*Start Workqueue with new delay*/
			cancel_delayed_work_sync(&data->alps_idev->work);
			queue_delayed_work(system_freezable_wq, &data->alps_idev->work, msecs_to_jiffies(data->delay));
		}
/*		if (data->flgA)
			accsns_activate(1, data->flgA, data->delay);*/
		mutex_unlock(&data->alps_lock);
