...
#define MIF_UTIL		65
#define INT_UTIL		70
#define DPP_SRC_W		1080
#define DPP_SRC_H		2340
#define MIF_FREQ_VM		1352000
...
static struct pm_qos_request exynos_mif_bts_vm_qos;
...
static unsigned int bts_update_decon_bw(struct bts_decon_info *decon)
{
	...
	for (i = 0; i < BTS_DPP_MAX; i++) {
		dpp = &decon->dpp[i];
		if(dpp->src_w == DPP_SRC_W && dpp->src_h == DPP_SRC_H) {
			pm_qos_update_request(&exynos_mif_bts_vm_qos, MIF_FREQ_VM);
		}
		if (!dpp->used)
			continue;
...
{
	pm_qos_update_request(&exynos_mif_bts_vm_qos, 0);
	return 0;
}
...
static int __init exynos_bts_init(void)
{
	...
	pm_qos_add_request(&exynos_mif_bts_vm_qos, PM_QOS_BUS_THROUGHPUT, 0);
