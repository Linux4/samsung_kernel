...
void bts_setqos_sysreg(BWL_QOS_SYSREG_IP qos_id, addr_u32 base, unsigned int *priority)
{
	...
	case BTS_SYSREG_DISPAUD:
		...
		tmp_reg &= ~((0xf << 20) | (0xf << 16) | (0xf << 12) | (0xf << 8) | (0xf << 4) | 0xf);
		Outp32(base + DISPAUD_QOS_CON, tmp_reg | (priority[0] << 20) | (priority[0] << 16) |
			(priority[0] << 12) | (priority[0] << 8) | (priority[0] << 4) | (priority[0]));
		break;
	case BTS_SYSREG_ISP0:
		Outp32(base + ISP_QOS_CON0, (priority[0] << 12) | (priority[0] << 8) |
			(priority[0] << 4) | (priority[0]));
		break;
	case BTS_SYSREG_ISP1:
		tmp_reg = Inp32(base + ISP_QOS_CON1);
		tmp_reg &= ~((0xf << 28) | (0xf << 24) | (0xf << 20) | (0xf << 16) | (0xf << 12) | (0xf << 8) | (0xf << 4) | 0xf);
		Outp32(base + ISP_QOS_CON1, tmp_reg | (priority[0] << 28) | (priority[0] << 24) | (priority[0] << 20) |
			(priority[0] << 16) | (priority[0] << 12) | (priority[0] << 8) |
			(priority[0] << 4) | (priority[0]));
		break;
	case BTS_SYSREG_MODAPIF_CP:
	...
	case BTS_SYSREG_MODAPIF_GNSS:
		...
		break;
	case BTS_SYSREG_MFCMSCL0:
		Outp32(base + MFCMSCL_QOS_CON_MFC, (priority[0] << 4) | (priority[0]));
		break;
	case BTS_SYSREG_MFCMSCL1:
		Outp32(base + MFCMSCL_QOS_CON_MSCL, (priority[0] << 20) | (priority[0] << 16) | (priority[0] << 12) |
			(priority[0] << 8) | (priority[0] << 4) | (priority[0]));
		break;
	case BTS_SYSREG_CCORE_CPU:
		Outp32(base + CCORE_CPU_QOS_CON, (priority[0] << 4) | (priority[0]));
		break;
	case BTS_SYSREG_CCORE_APL:
		Outp32(base + CCORE_APL_QOS_CON, (priority[0] << 4) | (priority[0]));
		break;
	default:
		break;
	}
