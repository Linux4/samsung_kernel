...
struct dentry *regdump_init_dentry(void)
{
	...
	if (!d_regdump && !once) {
		once = 1;
		pr_warn("Could not create debugfs directory 'regdump'\n");
		return NULL;
	}
...
int dump_reg_to_console(void)
{
	...
	list_for_each_entry_reverse(ops, &regdump_ops_list, node)
	{
		pr_emerg(" ======== dump %s start ========\n",
				ops->dev_name);
		if (ops->enable) {
			region = ops->regions;
			for (i = 0; i < ops->reg_nums; i++, region++) {
				val = regdump_read_region(ops, region);
				pr_err("%-26s: 0x%lx: 0x%08lx\n",
...
int register_regdump_ops(struct regdump_ops *ops)
{
	...
	if (!total_sz) {
		...
		if (!ops->buffer) {
			pr_err("REGDUMP: fail to alloc mem for %s\n",
					ops->dev_name);
	...
	if (!ops->dev_name) {
		pr_warn("Regdump: need specify name\n");
		return -EINVAL;
	}

	d_dev = debugfs_create_dir(ops->dev_name, d_regdump);
	if (!d_dev) {
		pr_warn("Could not create debugfs directory \'%s\'",
				ops->dev_name);
		return -EINVAL;
	}
