...
/* management frame tx status struct */
struct wlan_sipc_event_mgmt_tx_status {
	u64 cookie;		/* cookie */
	u8	ack;		/* status */
	u32	len;		/* frame len */
	u8	buf[0];		/* mgmt frame */
...
struct wlan_sipc_p2p_enable {
	/*used to enable cp2 p2p mode */
	u8  p2p_enable;
} __packed;
#endif /* CONFIG_SPRDWL_WIFI_DIRECT */
...
enum wlan_sipc_event_id {
	...
	WIFI_EVENT_INTERNAL_BSS_INFO,	/*15*/
	WIFI_EVENT_MAX
};
...
extern int wlan_sipc_cmd_send(struct wlan_sipc *sipc, u16 len, u8 type, u8 id);
extern int wlan_sipc_cmd_receive(struct wlan_sipc *sipc, u16 len, u8 id);
extern int sprdwl_cmd_send_recv(struct wlan_sipc *sipc, u8 type, u8 id);
extern int sprdwl_scan_cmd(struct wlan_sipc *sipc, const u8 *ssid, int len);
extern int sprdwl_set_wpa_version_cmd(struct wlan_sipc *sipc, u32 wpa_version);
extern int sprdwl_set_auth_type_cmd(struct wlan_sipc *sipc, u32 type);
extern int sprdwl_set_cipher_cmd(struct wlan_sipc *sipc, u32 cipher, u8 cmd_id);
extern int sprdwl_set_key_management_cmd(struct wlan_sipc *sipc, u8 key_mgmt);
extern int sprdwl_get_device_mode_cmd(struct wlan_sipc *sipc);
extern int sprdwl_pmksa_cmd(struct wlan_sipc *sipc,
			    const u8 *bssid, const u8 *pmkid, u8 type);
extern int sprdwl_set_psk_cmd(struct wlan_sipc *sipc,
			      const u8 *key, u32 key_len);
extern int sprdwl_set_channel_cmd(struct wlan_sipc *sipc, u32 channel);
extern int sprdwl_set_bssid_cmd(struct wlan_sipc *sipc, const u8 *addr);
extern int sprdwl_set_essid_cmd(struct wlan_sipc *sipc,
				const u8 *essid, int essid_len);
extern int sprdwl_disconnect_cmd(struct wlan_sipc *sipc, u16 reason_code);
extern int sprdwl_add_key_cmd(struct wlan_sipc *sipc, const u8 *key_data,
			      u8 key_len, u8 pairwise, u8 key_index,
			      const u8 *key_seq, u8 cypher_type,
			      const u8 *pmac);
extern int sprdwl_del_key_cmd(struct wlan_sipc *sipc,
			      u16 key_index, const u8 *mac_addr);
extern int sprdwl_set_key_cmd(struct wlan_sipc *sipc, u8 key_index);
extern int sprdwl_set_rts_cmd(struct wlan_sipc *sipc, u16 rts_threshold);
extern int sprdwl_set_frag_cmd(struct wlan_sipc *sipc, u16 frag_threshold);
