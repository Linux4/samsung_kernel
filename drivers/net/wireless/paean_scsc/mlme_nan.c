					   ...
					   u16 cluster_merge)
{
	...
	p = fapi_append_data_u16(req, 0x0011);
...
int slsi_mlme_nan_enable(struct slsi_dev *sdev, struct net_device *dev, struct slsi_hal_nan_enable_req *hal_req)
{
	...
	rx = slsi_mlme_req_cfm_ind(sdev, dev, req, MLME_NAN_START_CFM, MLME_NAN_EVENT_IND, slsi_nan_start_cfm_validate);
	if (!rx)
		return -EINVAL;
...
int slsi_mlme_nan_range_cancel_req(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	rx = slsi_mlme_req_cfm_ind(sdev, dev, req, MLME_NAN_RANGE_CANCEL_CFM, MLME_NAN_RANGE_CANCELLED_IND, slsi_nan_range_cancel_cfm_validate);
	if (!rx) {
