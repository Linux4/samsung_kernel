...
static void test_slsi_procfs_throughput_stats_read(struct kunit *test)
{
	...
	for (iface = 0; iface < CONFIG_SCSC_WLAN_MAX_INTERFACES; iface++)
		sdev->netdev[iface] = 0;

	sdev->netdev[SLSI_NET_INDEX_WLAN] = kunit_kzalloc(test, sizeof(struct net_device), GFP_KERNEL);
	dev = sdev->netdev[SLSI_NET_INDEX_WLAN];
	vif = netdev_priv(dev);
...
static void test_slsi_procfs_sta_bss_read(struct kunit *test)
{
	...
	dev = sdev->netdev[SLSI_NET_INDEX_WLAN];
	vif = netdev_priv(dev);
	SLSI_MUTEX_INIT(vif->vif_mutex);
	vif->sta.sta_bss = &sta_bss;
	vif->vif_type = FAPI_VIFTYPE_STATION;
	vif->sta.vif_status = SLSI_VIF_STATUS_CONNECTED;
...
static void test_slsi_procfs_big_data_read(struct kunit *test)
{
	...
	vif = netdev_priv(dev);
	SLSI_MUTEX_INIT(vif->vif_mutex);
	vif->ap.last_disconnected_sta.address[0] = 0xaa;
	vif->activated = true;
