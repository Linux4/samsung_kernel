...
{
	struct net_device *dev = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
	struct net_device *dev3 = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
	struct slsi_dev *sdev = kunit_kzalloc(test, sizeof(struct slsi_dev), GFP_KERNEL);
	struct netdev_vif *ndev_vif = netdev_priv(dev);
...
{
	struct net_device *dev = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
	struct net_device *sb_dev = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
	struct slsi_dev *sdev = kunit_kzalloc(test, sizeof(struct slsi_dev), GFP_KERNEL);
	struct sk_buff *skb = kunit_kzalloc(test, sizeof(struct sk_buff), GFP_KERNEL);
	...
	KUNIT_EXPECT_EQ(test, SLSI_NETIF_Q_PRIORITY, call_slsi_net_select_queue(dev, skb, sb_dev, NULL));

	ehdr->h_proto = cpu_to_be16(ETH_P_ARP);
	KUNIT_EXPECT_EQ(test, SLSI_NETIF_Q_ARP, call_slsi_net_select_queue(dev, skb, sb_dev, NULL));
...
static void test_slsi_net_hw_xmit(struct kunit *test)
{
	...
	dev->flags = 1;
	ndev_vif->vif_type = FAPI_VIFTYPE_AP;
	KUNIT_EXPECT_EQ(test, -ENOSPC, slsi_net_hw_xmit(skb, dev));
...
static void test_slsi_netif_dynamic_iface_add(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_netif_dynamic_iface_add(sdev, name));
}
...
static void test_slsi_netif_register_locked(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, 0, slsi_netif_register_locked(sdev, dev, 1));
	atomic_set(&ndev_vif->is_registered, 0);
	KUNIT_EXPECT_EQ(test, 1, slsi_netif_register_locked(sdev, dev, 0));
	SLSI_MUTEX_UNLOCK(sdev->netdev_add_remove_mutex);
}
...
{
	struct net_device *dev = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
	struct net_device *dev_skb = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
	struct slsi_dev *sdev = kunit_kzalloc(test, sizeof(struct slsi_dev), GFP_KERNEL);
	struct netdev_vif *ndev_vif = netdev_priv(dev);
