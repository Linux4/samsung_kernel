...
static void test_slsi_mlme_set_scan_mode_req(struct kunit *test)
{
	...
	sdev->netdev[SLSI_NET_INDEX_WLAN] = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif),
							  GFP_KERNEL);
	...
	sndev_vif->sig_wait.cfm = fapi_alloc(mlme_set_scan_mode_cfm, MLME_SET_SCAN_MODE_CFM, 0, 10);
	fapi_set_u16(sndev_vif->sig_wait.cfm, u.mlme_set_scan_mode_cfm.result_code, FAPI_RESULTCODE_SUCCESS);
	...
	sndev_vif->sig_wait.cfm = fapi_alloc(mlme_set_scan_mode_cfm, MLME_SET_SCAN_MODE_CFM, 0, 10);
	fapi_set_u16(sndev_vif->sig_wait.cfm, u.mlme_set_scan_mode_cfm.result_code, FAPI_RESULTCODE_UNSPECIFIED_FAILURE);
...
static void test_slsi_mlme_append_gscan_channel_list(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_mlme_append_gscan_channel_list(sdev, dev, req, nl_bucket));
...
static void test_slsi_mlme_append_channel_list(struct kunit *test)
{
	...
	sdev = kunit_kzalloc(test, sizeof(struct slsi_dev), GFP_KERNEL);
	dev = kunit_kzalloc(test, sizeof(struct net_device), GFP_KERNEL);
...
static void test_slsi_set_scan_params(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_set_scan_params(dev, scan_id, scan_type, report_mode,
							    num_ssids, ssids, req));
...
static void test_slsi_mlme_add_sched_scan(struct kunit *test)
{
	...
	sndev_vif->sig_wait.cfm = fapi_alloc(mlme_add_scan_cfm, MLME_ADD_SCAN_CFM, 0, 10);
	fapi_set_u16(sndev_vif->sig_wait.cfm, u.mlme_add_scan_cfm.result_code, FAPI_RESULTCODE_SUCCESS);
	sndev_vif->mgmt_tx_cookie = 0x1234;
	sdev->mlme_blocked = false;
	sdev->wlan_service_on = true;
	sdev->sig_wait_cfm_timeout = &timeout;
	...
	sndev_vif->sig_wait.cfm = fapi_alloc(mlme_add_scan_cfm, MLME_ADD_SCAN_CFM, 0, 10);
	fapi_set_u16(sndev_vif->sig_wait.cfm, u.mlme_add_scan_cfm.result_code, FAPI_RESULTCODE_UNSPECIFIED_FAILURE);
...
static void test_slsi_mlme_del_scan(struct kunit *test)
{
	...
	sndev_vif->sig_wait.cfm = fapi_alloc(mlme_del_scan_cfm, MLME_DEL_SCAN_CFM, 0, 10);
	fapi_set_u16(sndev_vif->sig_wait.cfm, u.mlme_del_scan_cfm.result_code, FAPI_RESULTCODE_SUCCESS);
	sndev_vif->mgmt_tx_cookie = 0x1234;
	sdev->mlme_blocked = false;
	sdev->wlan_service_on = true;
	sdev->sig_wait_cfm_timeout = &timeout;
	...
	sndev_vif->sig_wait.cfm = fapi_alloc(mlme_del_scan_cfm, MLME_DEL_SCAN_CFM, 0, 10);
	fapi_set_u16(sndev_vif->sig_wait.cfm, u.mlme_del_scan_cfm.result_code, FAPI_RESULTCODE_UNSPECIFIED_FAILURE);
...
static void test_slsi_mlme_start_prepare_ies(struct kunit *test)
{
	...
	ndev_vif = kunit_kzalloc(test, sizeof(struct netdev_vif), GFP_KERNEL);
	ndev_vif->sdev = kunit_kzalloc(test, sizeof(struct slsi_dev), GFP_KERNEL);
	ndev_vif->wdev.netdev = kunit_kzalloc(test, sizeof(struct net_device), GFP_KERNEL);
...
static void test_slsi_decode_fw_rate(struct kunit *test)
{
	...
	slsi_decode_fw_rate(fw_rate, rate, &data_rate_mbps);
...
static void test_slsi_mlme_register_action_frame(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EIO, slsi_mlme_register_action_frame(sdev, dev, af_bitmap_active, af_bitmap_suspended));
	...
	KUNIT_EXPECT_EQ(test, 0, slsi_mlme_register_action_frame(sdev, dev, af_bitmap_active, af_bitmap_suspended));

	sdev->sig_wait.cfm = fapi_alloc(mlme_register_action_frame_cfm, MLME_REGISTER_ACTION_FRAME_CFM, 0, 10);
	fapi_set_u16(sdev->sig_wait.cfm, u.mlme_register_action_frame_cfm.result_code, FAPI_RESULTCODE_UNSPECIFIED_FAILURE);

	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_mlme_register_action_frame(sdev, dev, af_bitmap_active, af_bitmap_suspended));
}
...
static void test_slsi_mlme_send_frame_data(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, NETDEV_TX_OK, slsi_mlme_send_frame_data(sdev, dev, skb, msg_type, host_tag, dwell_time, period));
	...
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_mlme_send_frame_data(sdev, dev, skb, msg_type, host_tag, dwell_time, period));
	kfree_skb(skb);			//skb is not cousumed

	skb = fapi_alloc(mlme_send_frame_req, MLME_SEND_FRAME_REQ, 0, 100);
	skb->next = alloc_skb(100, GFP_KERNEL);
	ndev_vif->mgmt_tx_cookie = 0x01;
	KUNIT_EXPECT_EQ(test, 0, slsi_mlme_send_frame_data(sdev, dev, skb, msg_type, host_tag, dwell_time, period));
...
static void test_slsi_mlme_wtc_mode_req(struct kunit *test)
{
	...
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	int wtc_mode = 2;
	int scan_mode = FAPI_SCANMODE_LOW_LATENCY_2;
	int rssi = -57;
	int rssi_th_2g = -27;
	int rssi_th_5g = -57;
	int rssi_th_6g = -97;
...
static void test_slsi_mlme_tdls_action(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EOPNOTSUPP, slsi_mlme_tdls_action(sdev, dev, peer, action));
...
static void test_slsi_mlme_add_range_req(struct kunit *test)
{
	...
	sndev_vif->sig_wait.cfm = fapi_alloc(mlme_add_range_cfm, MLME_ADD_RANGE_CFM, 0, 10);
	fapi_set_u16(sndev_vif->sig_wait.cfm, u.mlme_add_range_cfm.result_code, FAPI_RESULTCODE_SUCCESS);
	sndev_vif->mgmt_tx_cookie = 0x1234;
	sdev->mlme_blocked = false;
	sdev->wlan_service_on = true;
	sdev->sig_wait_cfm_timeout = &timeout;

	KUNIT_EXPECT_EQ(test, 0, slsi_mlme_add_range_req(sdev, dev, count, nl_rtt_params, rtt_id, source_addr));

	sndev_vif->sig_wait.cfm = fapi_alloc(mlme_add_range_cfm, MLME_ADD_RANGE_CFM, 0, 10);
	fapi_set_u16(sndev_vif->sig_wait.cfm, u.mlme_add_range_cfm.result_code, FAPI_RESULTCODE_UNSPECIFIED_FAILURE);
...
static void test_slsi_mlme_start_link_stats_req(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EIO, slsi_mlme_start_link_stats_req(sdev, mpdu_size_threshold, aggressive_stats_enabled));
	...
	KUNIT_EXPECT_EQ(test, 0, slsi_mlme_start_link_stats_req(sdev, mpdu_size_threshold, aggressive_stats_enabled));

	sndev_vif->sig_wait.cfm = fapi_alloc(mlme_start_link_statistics_cfm, MLME_START_LINK_STATISTICS_CFM, 0, 10);
	fapi_set_u16(sndev_vif->sig_wait.cfm, u.mlme_start_link_statistics_cfm.result_code, FAPI_RESULTCODE_UNSPECIFIED_FAILURE);

	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_mlme_start_link_stats_req(sdev, mpdu_size_threshold, aggressive_stats_enabled));
}
...
static void test_slsi_mlme_twt_setup(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_mlme_twt_setup(sdev, dev, tsetup));
}
...
static void test_slsi_mlme_twt_teardown(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_mlme_twt_teardown(sdev, dev, setup_id, negotiation_type));
}

static void test_slsi_mlme_twt_suspend(struct kunit *test)
{
	struct slsi_dev *sdev = kunit_kzalloc(test, sizeof(struct slsi_dev), GFP_KERNEL);
	struct net_device *dev = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	...
	KUNIT_EXPECT_EQ(test, -EIO, slsi_mlme_twt_suspend(sdev, dev, setup_id, negotiation_type));

	slsi_sig_send_init(&ndev_vif->sig_wait);
	slsi_sig_send_init(&sdev->sig_wait);
	sdev->sig_wait.cfm = fapi_alloc(mlme_twt_suspend_cfm, MLME_TWT_SUSPEND_CFM, 0, 10);
	fapi_set_u16(sdev->sig_wait.cfm, u.mlme_twt_suspend_cfm.result_code, FAPI_RESULTCODE_UNSPECIFIED_FAILURE);
	ndev_vif->mgmt_tx_cookie = 0x1234;
	sdev->mlme_blocked = false;
	sdev->wlan_service_on = true;
	sdev->sig_wait_cfm_timeout = &timeout;

	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_mlme_twt_suspend(sdev, dev, setup_id, negotiation_type));
}
...
static struct kunit_case mlme_test_cases[] = {
	...
	KUNIT_CASE(test_slsi_mlme_twt_suspend),
