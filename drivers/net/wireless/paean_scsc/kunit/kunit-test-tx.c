...
static void test_slsi_get_dwell_time_for_wps(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, SLSI_EAP_WPS_DWELL_TIME, slsi_get_dwell_time_for_wps(sdev, ndev_vif, eapol, eap_length));
...
static void test_slsi_tx_data(struct kunit *test)
{
	...
	vif->vif_type = FAPI_VIFTYPE_STATION;
	KUNIT_EXPECT_EQ(test, 0, slsi_tx_data(sdev, ndev, skb));

	vif->peer_sta_records = 0;
	vif->vif_type = FAPI_VIFTYPE_AP;
	KUNIT_EXPECT_NE(test, 0, slsi_tx_data(sdev, ndev, skb));
...
static void test_slsi_tx_control(struct kunit *test)
{
	...
	KUNIT_EXPECT_NE(test, 0, slsi_tx_control(sdev, ndev, skb));

	(((struct fapi_signal *)(skb)->data)->id) = FAPI_SAP_TYPE_MA;
	KUNIT_EXPECT_EQ(test, 0, slsi_tx_control(sdev, ndev, skb));
