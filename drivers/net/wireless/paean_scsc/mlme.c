...
void slsi_mlme_get_sta_dump(struct netdev_vif *ndev_vif, struct net_device *dev, int *dump_params)
{
	...
	if (values[mib_index].type != SLSI_MIB_TYPE_NONE) {
		...
		dump_params[SLSI_STA_DUMP_RSSI] = values[mib_index++].u.intValue;
	} else {
	...
	if (values[mib_index].type == SLSI_MIB_TYPE_UINT) {
		SLSI_DBG3(sdev, SLSI_MLME, "SLSI_PSID_UNIFI_FRAME_RX_COUNTERS = %d\n",
			  values[mib_index].u.intValue);
		dump_params[SLSI_STA_DUMP_RX_COUNTERS] = values[mib_index++].u.intValue;
	} else {
		SLSI_ERR(sdev, "Invalid type: PSID = 0x%x\n", get_values[mib_index++].psid);
	}
	/*tx good count*/
	if (values[mib_index].type == SLSI_MIB_TYPE_UINT) {
		SLSI_DBG3(sdev, SLSI_MLME, "SLSI_PSID_UNIFI_FRAME_TX_COUNTERS = %d\n",
			  values[mib_index].u.intValue);
		dump_params[SLSI_STA_DUMP_TX_COUNTERS] = values[mib_index++].u.intValue;
	} else {
		SLSI_ERR(sdev, "Invalid type: PSID = 0x%x\n", get_values[mib_index++].psid);
	}
	/*rx data rate*/
	if (values[mib_index].type != SLSI_MIB_TYPE_NONE) {
		...
		slsi_decode_fw_rate((u32)values[mib_index++].u.uintValue, NULL,
				    (unsigned long *)&dump_params[SLSI_STA_DUMP_RXBITRATE],
				    &dump_params[SLSI_STA_DUMP_RXNSS],
				    &dump_params[SLSI_STA_DUMP_RXBANDWIDTH],
				    &dump_params[SLSI_STA_DUMP_RXMCS]);
	} else {
	...
	if (values[mib_index].type != SLSI_MIB_TYPE_NONE) {
		...
		SLSI_DBG3(sdev, SLSI_MLME, "SLSI_PSID_UNIFI_TX_DATA_RATE = %d\n",
			  values[mib_index].u.intValue);
		slsi_decode_fw_rate((u32)values[mib_index].u.uintValue, NULL,
				    (unsigned long *)&dump_params[SLSI_STA_DUMP_TXBITRATE],
				    &dump_params[SLSI_STA_DUMP_TXNSS],
				    &dump_params[SLSI_STA_DUMP_TXBANDWIDTH],
				    &dump_params[SLSI_STA_DUMP_TXMCS]);
	} else {
		SLSI_DBG3(sdev, SLSI_MLME, "Invalid type: PSID = 0x%x\n",
			  get_values[mib_index].psid);
	}
	dump_params[SLSI_STA_DUMP_MODE] = slsi_sta_ieee80211_mode(dev, ndev_vif->chan->center_freq);
	dump_params[SLSI_STA_DUMP_RXBYTES] = peer->sinfo.rx_bytes;
	dump_params[SLSI_STA_DUMP_TXBYTES] = peer->sinfo.tx_bytes;
				    ...
				    struct sk_buff *req)
{
	...
	if (((reg_rule->freq_range->start_freq / 1000) == 5) &&
		...
		bulk_data->max_bw = reg_rule->freq_range->max_bandwidth & 0xFF;
				    ...
				    struct sk_buff *req)
{
	...
	if (((reg_rule->freq_range->start_freq / 1000) == 5) &&
		...
		desc_ie->max_bw = reg_rule->freq_range->max_bandwidth & 0xFF;
	...
	if (reg_rule->freq_range->start_freq >= 5925 &&
		...
		if (reg_rule->flags & SLSI_REGULATORY_6GHZ_SP) {
			...
			desc_ie->max_power_lpi = (reg_rule->flags & SLSI_REGULATORY_6GHZ_LPI) ?
				slsi_calculate_lpi_power(desc_ie->max_bw) : 0;
...
static int slsi_get_twt_error_code(int result_code)
{
	switch (result_code) {
	case FAPI_RESULTCODE_INVALID_PARAMETERS:
	case FAPI_RESULTCODE_INVALID_VIRTUAL_INTERFACE_INDEX:
		result_code = SLSI_TWT_WIFI_ERROR_INVALID_ARGUMENT;
		break;
	case FAPI_RESULTCODE_NOT_SUPPORTED:
		result_code = SLSI_TWT_WIFI_ERROR_NOT_SUPPORTED;
		break;
	case FAPI_RESULTCODE_INVALID_STATE:
		result_code = SLSI_TWT_WIFI_ERROR_BUSY;
		break;
	case FAPI_RESULTCODE_NOT_PRESENT:
		result_code = SLSI_TWT_WIFI_ERROR_NOT_AVAILABLE;
		break;
	default:
		result_code = SLSI_TWT_WIFI_ERROR_TEMPORAL;
	}
	return result_code;
}
...
int slsi_mlme_twt_setup(struct slsi_dev *sdev, struct net_device *dev, struct twt_setup *tsetup)
{
	...
	if (fapi_get_u16(cfm, u.mlme_twt_setup_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		r = slsi_get_twt_error_code(fapi_get_u16(cfm, u.mlme_twt_setup_cfm.result_code));
...
int slsi_mlme_twt_teardown(struct slsi_dev *sdev, struct net_device *dev, u16 setup_id, u16 negotiation_type)
{
	...
	if (fapi_get_u16(cfm, u.mlme_twt_teardown_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		r = slsi_get_twt_error_code(fapi_get_u16(cfm, u.mlme_twt_teardown_cfm.result_code));
...
int slsi_mlme_twt_suspend(struct slsi_dev *sdev, struct net_device *dev, u16 setup_id, u32 suspend_duration)
{
	...
	if (fapi_get_u16(cfm, u.mlme_twt_suspend_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		r = slsi_get_twt_error_code(fapi_get_u16(cfm, u.mlme_twt_suspend_cfm.result_code));
			       ...
			       int command_len, int *command_pos, u16 setup_id)
{
	...
	if (fapi_get_u16(cfm, u.mlme_twt_status_query_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		r = slsi_get_twt_error_code(fapi_get_u16(cfm, u.mlme_twt_status_query_cfm.result_code));
