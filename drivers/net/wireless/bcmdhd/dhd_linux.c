 ...
 *
 * $Id: dhd_linux.c 566698 2015-06-25 15:17:47Z $
 */
...
#ifdef NVPATH_FROMDT
{
	...
	dt_node = of_find_node_by_name(NULL, "bcmdhd-wlan");

	if(!dt_node){
		DHD_ERROR(("[NVPATH_FROMDT] Can not find node\n"));
	} else {
		for_each_property_of_node(dt_node,prop){
			if ((strstr(fw, "_sta") != NULL) || (strstr(fw, "_apsta") != NULL)){
				if(!strncmp(prop->name, "nv_path", strlen("nv_path"))){
					if(!of_property_read_string(dt_node, prop->name, &str_data)){
						nv = str_data;
						DHD_ERROR(("[NVPATH_FROMDT] Update sta nv : [%s]\n", nv));
					}
				}
			}
			else if (strstr(fw, "_mfg") != NULL){
				if(!strncmp(prop->name, "nv_mfg_path", strlen("nv_mfg_path"))){
					if(!of_property_read_string(dt_node, prop->name, &str_data)){
						nv = str_data;
						DHD_ERROR(("[NVPATH_FROMDT]Update mfg nv : [%s]\n", nv));
					}
				}
			}
		}
