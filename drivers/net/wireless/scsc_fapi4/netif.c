...
static int tcp_ack_suppression_max = 16;
module_param(tcp_ack_suppression_max, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tcp_ack_suppression_max, "Maximum number of TCP acks suppressed before latest flushed to tx");

static bool tcp_ack_suppression_adaptive_disable = true;
module_param(tcp_ack_suppression_adaptive_disable, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tcp_ack_suppression_adaptive_disable, "Disable TCP ack suppression adaptive max algorithm");

static int tcp_ack_suppression_adaptive_timeout_max = 64;
module_param(tcp_ack_suppression_adaptive_timeout_max, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tcp_ack_suppression_adaptive_timeout_max, "Timeout (in ms) before cached TCP ack is flushed to tx in adaptive mode");

static int tcp_ack_suppression_adaptive_acks_max = 32;
module_param(tcp_ack_suppression_adaptive_acks_max, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tcp_ack_suppression_adaptive_acks_max, "Maximum number of TCP acks suppressed before latest flushed to tx in adaptive mode");

static int tcp_ack_suppression_adaptive_consecutive_timeouts = 4;
module_param(tcp_ack_suppression_adaptive_consecutive_timeouts, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tcp_ack_suppression_adaptive_consecutive_timeouts, "Maximum number of consecutive timeouts before max is reduced");

static int tcp_ack_suppression_adaptive_consecutive_max_acks = 8;
module_param(tcp_ack_suppression_adaptive_consecutive_max_acks, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tcp_ack_suppression_adaptive_consecutive_max_acks, "Maximum number of consecutive maximums before max is increased");
...
static int slsi_netif_tcp_ack_suppression_start(struct net_device *dev)
{
	...
	for (index = 0; index < MAX_ACK_SUPPRESSION_RECORDS; index++) {
		...
		tcp_ack->ack_seq = 0;
		tcp_ack->count = 0;
		tcp_ack->consecutive_timeouts = 0;
	...
	ndev_vif->tack_acks = 0;
	ndev_vif->tack_suppressed = 0;
	ndev_vif->tack_sent = 0;
	ndev_vif->tack_max = 0;
	...
	ndev_vif->tack_dropped = 0;
	ndev_vif->tack_ktime = 0;
	ndev_vif->tack_lastrecord = 0;
	ndev_vif->tack_searchrecord = 0;
	ndev_vif->tack_ece = 0;
...
static void slsi_netif_tcp_ack_suppression_timeout(unsigned long data)
{
	...
	while ((skb = skb_dequeue(&tcp_ack->list)) != 0) {
		...
		/* Special mark to exclude from suppression algorithm. */
		if (!skb->dev) {
			kfree_skb(skb);
			slsi_spinlock_unlock(&tcp_ack->lock);
			return;
		...
		ndev_vif->tack_timeout++;
		tcp_ack->consecutive_timeouts++;
		tcp_ack->consecutive_max = 0;
		if (!tcp_ack_suppression_adaptive_disable && (tcp_ack->consecutive_timeouts >= tcp_ack_suppression_adaptive_consecutive_timeouts)) {
			tcp_ack->consecutive_timeouts = 0;
			/* reduce the max Acks to suppress and reduce timer also */
			tcp_ack->max = (tcp_ack->max >> 1) ? (tcp_ack->max >> 1) : 1;
			tcp_ack->age = (tcp_ack->age >> 1) ? (tcp_ack->age >> 1) : 1;
		}
		/* Mark skb to prevent it being cached again. */
		skb->cb[20] = (unsigned long)skb & 0xff;
		skb->cb[21] = ((unsigned long)skb >> 8) & 0xff;

		r = slsi_net_hw_xmit(skb, skb->dev);
		if (r == NETDEV_TX_OK) {
			tcp_ack->last_sent = ktime_get();
		} else if (r == NETDEV_TX_BUSY) {
			ndev_vif->tack_dropped++;
			kfree_skb(skb);
		} else {
			ndev_vif->tack_dropped++;
		}
...
static void slsi_netif_tcp_ack_suppression_syn(struct net_device *dev, struct sk_buff *skb)
{
	...
	for (index = 0; index < MAX_ACK_SUPPRESSION_RECORDS; index++) {
		...
		if (tcp_ack->daddr) {
			if (ktime_to_ms(ktime_sub(ktime_get(), tcp_ack->last_sent)) >= 10*1000) {
		...
		if (tcp_ack->daddr == 0) {
			...
			if (tcp_ack_suppression_adaptive_disable) {
				tcp_ack->max = tcp_ack_suppression_max;
...
static struct sk_buff *slsi_netif_tcp_ack_suppression_pkt(struct net_device *dev, struct sk_buff *skb)
{
	...
	/* cb indices selected as not changed by scheduling kernel code */
	if ((skb->cb[20] == ((unsigned long)skb & 0xff)) && (skb->cb[21] == (((unsigned long)skb >> 8) & 0xff))) {
		ndev_vif->tack_sent++;
		skb->cb[20] = 0;
		skb->cb[21] = 0;
	...
	/* Increment count here to avoid double counting acks re-inserted into the transmit path
	 * by the timeout code.
	 */
	ndev_vif->tack_acks++;
	...
	/* When the TCP connection is newly made, wait until
	 * a number of Acks are sent before applying the
	 * suppression rules. It is to allow the cwnd to grow
	 * at a normal rate at the TCP sender.
	 */
	if (tcp_ack->tcp_slow_start) {
		tcp_ack->slow_start_count++;
		if (tcp_ack->slow_start_count >= tcp_ack_suppression_adaptive_slow_start) {
...
_forward_now:
	...
	if (forward_now) {
		...
		if (tcp_ack->count >= tcp_ack->max) {
			...
			if (tcp_ack_robustness)
				slsi_skb_cb_get(skb)->frame_format = SLSI_NETIF_FRAME_TCP_ACK;
			if (!tcp_ack_suppression_adaptive_disable && (tcp_ack->consecutive_max >= tcp_ack_suppression_adaptive_consecutive_max_acks)) {
				tcp_ack->consecutive_max = 0;
				/* increase the max Acks to suppress and timer also */
				tcp_ack->max = ((tcp_ack->max << 1) < tcp_ack_suppression_adaptive_acks_max) ? (tcp_ack->max << 1) : tcp_ack_suppression_adaptive_acks_max;
				tcp_ack->age = ((tcp_ack->age << 1) < tcp_ack_suppression_adaptive_timeout_max) ? (tcp_ack->age << 1) : tcp_ack_suppression_adaptive_timeout_max;
			}
		}
	}
	if (!flush) {
