...
{
	struct netdev_vif *ndev_vif = container_of((struct delayed_work *)work,
						   struct netdev_vif, ap.del_softap_vif_work);
	struct net_device *dev;
	struct slsi_dev     *sdev;
	...
	dev = slsi_get_netdev(sdev, SLSI_NET_INDEX_P2PX_SWLAN);
	if (!dev) {
		SLSI_NET_DBG1(dev, SLSI_INIT_DEINIT, "Dev is NULL\n");
		return;
	}
