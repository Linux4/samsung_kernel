		 ...
		 struct cfg80211_connect_params *sme)
{
	...
	if (ndev_vif->sta.sta_bss)
		...
		SLSI_NET_ERR(dev, "MG : slsi_connect : Checking all the values.\n");
		SLSI_NET_ERR(dev, "MG : slsi_connect : sme->ssid is %.*s\n", (int)sme->ssid_len, sme->ssid);
		SLSI_NET_ERR(dev, "MG : slsi_connect : sme->channel is %p\n", sme->channel);
		SLSI_NET_ERR(dev, "MG : slsi_connect : ndev_vif->sta.vif_status is %d\n", ndev_vif->sta.vif_status);
		SLSI_NET_ERR(dev, "MG : slsi_connect : peer_address is %pM\n", peer_address);
		SLSI_NET_ERR(dev, "MG : slsi_connect : sme->bssid is %pM\n", sme->bssid);
		SLSI_NET_ERR(dev, "MG : slsi_connect : sdev->device_config.qos_info is %d\n", sdev->device_config.qos_info);
		  ...
		  struct cfg80211_ap_settings *settings)
{
	...
	INIT_DELAYED_WORK(&ndev_vif->ap.del_softap_vif_work, slsi_del_ap_netif_work);
...
#endif
exit:
	if ((memcmp(dev->name, "wlan1", 5) == 0) || (memcmp(dev->name, "swlan0", 6) == 0)) {
		if (ndev_vif->ap.rm_net_dev_on_ap_stop)
			if (!queue_delayed_work(sdev->device_wq, &ndev_vif->ap.del_softap_vif_work, 0))
				SLSI_NET_ERR(dev, "Failed to queue work\n");
	}
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
