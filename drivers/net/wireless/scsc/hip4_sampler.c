...
void hip4_sampler_create(struct scsc_mx *mx)
{
	...
	if (kstrtol(dev_uid, 10, &uid)) {
		SLSI_ERR_NODEV("Invalid device uid default to zero\n");
		uid = 0;
	}
#endif
	...
	for (i = 0; i < SCSC_HIP4_STREAM_CH; i++) {
		...
		snprintf(dev_name, sizeof(dev_name), "%s_%d_%s", "hip4", (int)uid, "sam_str");
		...
		if (hip4_sampler.devs[minor].dev == NULL) {
		...
		snprintf(dev_name, sizeof(dev_name), "%s_%d_%s", "hip4", (int)uid, "sam_off");
