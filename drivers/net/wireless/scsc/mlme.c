...
#ifdef CONFIG_SCSC_WLAN_P2P_GC_EASY_CONNECT
	if (ndev_vif->iftype == NL80211_IFTYPE_STATION && ndev_vif->ifnum == SLSI_NET_INDEX_P2PX_SWLAN) {
		SLSI_NET_INFO(dev, "P2P_GC_EASY_CONNECT(iftype:%d, ifnum:%d)\n", ndev_vif->iftype, ndev_vif->ifnum);
		fapi_set_u32(req, u.mlme_add_vif_req.spare_1, 1);
	}
#endif
...
void slsi_calc_max_data_rate(struct net_device *dev, u8 bandwidth, u8 antenna_mode)
{
	...
	SLSI_NET_INFO(dev, "sta_mode : %u, freq : %u, bandwidth : %u, antenna_mode : %u, max_rate_mbps : %lu\n",
