...
static int slsi_create_softap_interface(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int len)
{
	struct net_device *dev = wdev->netdev;
	struct slsi_dev   *sdev = SDEV_FROM_WIPHY(wiphy);
	...
	nla_for_each_attr(attr, data, len, temp) {
		type = nla_type(attr);
		switch (type) {
		case SLSI_NL_ATTRIBUTE_AP_IFACE_NAME:
		{
			intf_name = (char *)nla_data(attr);
			break;
		}
		default:
			SLSI_ERR(sdev, "Invalid type : %d\n", type);
	...
	if (swlan_dev && (memcmp(swlan_dev->name, intf_name, strlen(intf_name)) == 0)) {
		SLSI_DBG3(sdev, SLSI_GSCAN, "softap interface already present, performing cleanup\n");
		slsi_vif_cleanup(sdev, swlan_dev, true);
		return 0;
	}

	iface = slsi_netif_add(sdev,  intf_name);
	if (iface < 0)
		goto exit;

	swlan_dev = slsi_get_netdev(sdev, iface);
	if (!swlan_dev)
		goto exit;

	ndev_swlan_vif = netdev_priv(swlan_dev);

	err = slsi_netif_register_rtlnl_locked(sdev, swlan_dev);
	if (err)
		goto exit;

	SLSI_MUTEX_LOCK(ndev_swlan_vif->vif_mutex);
	ndev_swlan_vif->iftype = type;
	swlan_dev->ieee80211_ptr->iftype = type;
	SLSI_MUTEX_UNLOCK(ndev_swlan_vif->vif_mutex);

	SLSI_DBG3(sdev, SLSI_GSCAN, "create interface %s\n", intf_name);
...
static int slsi_delete_softap_interface(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int len)
{
	struct net_device *dev = wdev->netdev;
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct slsi_dev   *sdev = SDEV_FROM_WIPHY(wiphy);
	...
	SLSI_DBG3(sdev, SLSI_GSCAN, "Received delete_softap_interface command for %s\n", intf_name);

	if ((memcmp(intf_name, "wlan1", 5) == 0) || (memcmp(intf_name, "swlan0", 6) == 0))
		dev = sdev->netdev[SLSI_NET_INDEX_P2PX_SWLAN];

	if (!dev) {
		SLSI_NET_ERR(dev, "Dev not present\n");
		goto exit;
	}

	if (memcmp(intf_name, dev->name, strlen(intf_name)) == 0) {
		ndev_vif = netdev_priv(dev);
#ifdef CONFIG_SCSC_WLAN_WIFI_SHARING
		SLSI_MUTEX_LOCK(ndev_vif->vif_mutex);
		...
		if (ndev_vif->activated) {
			ndev_vif->ap.rm_net_dev_on_ap_stop = true;
		} else {
			ndev_vif->ap.rm_net_dev_on_ap_stop = false;
...
static const struct wiphy_vendor_command     slsi_vendor_cmd[] = {
	...
	},
	{
		{
			.vendor_id = OUI_GOOGLE,
			.subcmd = SLSI_NL80211_VENDOR_SUBCMD_CREATE_SOFTAP_INTERFACE
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = slsi_create_softap_interface
	},
	{
		{
			.vendor_id = OUI_GOOGLE,
			.subcmd = SLSI_NL80211_VENDOR_SUBCMD_DELETE_SOFTAP_INTERFACE
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = slsi_delete_softap_interface
	},
#ifdef CONFIG_SCSC_WLAN_ENHANCED_LOGGING
	{
		{
