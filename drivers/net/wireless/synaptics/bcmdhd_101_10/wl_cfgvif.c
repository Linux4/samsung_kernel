	...
	struct cfg80211_ap_settings *info)
{
...
#ifdef WL_HOSTAPD_CFG
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	/* Check for primary iface connected interfaces before wl down */
	err = wl_get_drv_status(cfg, CONNECTED, primary_ndev);
	if (err == 0) {
		WL_ERR(("Concurrent i/f operational. can do wl down"));
		err = wldev_ioctl_set(dev, WLC_DOWN, &downup, sizeof(downup));
		if (err < 0) {
			WL_ERR(("WLC_DOWN error %d\n", err));
		}
	}
	...
	if (err) {
		WL_ERR(("set nmode %d failed: %d\n", nmode, err));
	} else {
		WL_ERR(("set nmode %d success\n", nmode));
	}
...
#ifdef SUPPORT_2G_VHT
	if (dhd_2g_vht) {
		struct ieee80211_channel *chan = dev->ieee80211_ptr->preset_chandef.chan;
	...
	err = wldev_iovar_setint(dev, "vhtmode", vhtmode);
	if (err) {
		WL_ERR(("set vhtmode %d failed: %d\n", vhtmode, err));
	} else {
		WL_ERR(("set vhtmode %d success\n", vhtmode));
	}
	...
	err = wl_cfg80211_set_he_enab(dev, cfg, bssidx, WL_IF_TYPE_AP, hemode);
	if (err) {
		WL_ERR(("set he enab %d failed: %d\n", hemode, err));
	} else {
		WL_ERR(("set he enab %d success\n", hemode));
	}
	...
	err = wldev_iovar_setint(dev, "mode_reqd", only_mode);
	if (err) {
		WL_ERR(("set only_mode %d failed: %d\n", only_mode, err));
	} else {
		WL_ERR(("set only_mode %d success\n", only_mode));
	}
	err = wldev_ioctl_set(dev, WLC_UP, &downup, sizeof(downup));
	if (err < 0) {
		WL_ERR(("WLC_UP error %d\n", err));
	}
...
#if defined(READ_CONFIG_FROM_FILE) || defined(WL_HOSTAPD_CFG)
int wl_cfg80211_set_he_enab(struct net_device *dev, struct bcm_cfg80211 *cfg,
		s32 bssidx, u32 interface_type, uint enab)
{
	uint8 se_he_xtlv[32];
	int se_he_xtlv_len = sizeof(se_he_xtlv);
	he_xtlv_v32 v32;
	...
	WL_INFORM(("Set he enab[%d] done\n", enab));

	return err;
}
#endif /* defined(READ_CONFIG_FROM_FILE) || defined(WL_HOSTAPD_CFG) */
