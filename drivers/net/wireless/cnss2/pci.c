...
struct exynos_pm_qos_request cpu_qos; /* CPU */
struct exynos_pm_qos_request int_qos; /* SNOC */
struct exynos_pm_qos_request mif_qos; /* DDR */

/* CPU 1920000 + SNOC 533000 + DDR 2730000 for 11AC per test */
#define CPU_VOTE_CLOCK_11AC 1920000
#define INT_VOTE_CLOCK_11AC 533000
#define MIF_VOTE_CLOCK_11AC 2730000

/* CPU 0 + SNOC 533000 + DDR 0 for 11G per test */
#define CPU_VOTE_CLOCK_11G 0
#define INT_VOTE_CLOCK_11G 533000
#define MIF_VOTE_CLOCK_11G 1539000

#define CPU_VOTE_CLOCK_DEF 0
#define INT_VOTE_CLOCK_DEF 0
#define MIF_VOTE_CLOCK_DEF 0
				    ...
				    u32 bw, bool save)
{
	switch (bw) {
		...
		case CNSS_BUS_WIDTH_VERY_HIGH:
			/*11ac/11ax: CPU(4/5/6) mid core 1920000 + SNOC 533000 + DDR 2730000 per test */
		...
		case CNSS_BUS_WIDTH_LOW_LATENCY:
			/*11g : CPU 0 + SNOC 533000 + DDR 0 per test */
			exynos_pm_qos_update_request(&cpu_qos, CPU_VOTE_CLOCK_11G);
			exynos_pm_qos_update_request(&int_qos, INT_VOTE_CLOCK_11G);
			exynos_pm_qos_update_request(&mif_qos, MIF_VOTE_CLOCK_11G);
			break;
		default:
...
void exynos_pm_qos_init()
{
	...
	exynos_pm_qos_add_request(&cpu_qos, PM_QOS_CLUSTER1_FREQ_MIN, CPU_VOTE_CLOCK_DEF);
...
static irqreturn_t cnss_pci_wake_handler(int irq, void *data)
{
	...
	if (jiff != jiffies) {
		...
		if (wake_count > 200) {
			if (plat_priv->fw_init_ready) {
				panic("Wlan wake storm after fw ready\n");
			} else {
				cnss_pr_err("Wlan wake storm before fw ready\n");
				disable_irq_wake(plat_priv->wake_irq);
				disable_irq_nosync(plat_priv->wake_irq);
				plat_priv->wake_irq_disable = true;
			}
		}
	}
	wake_count++;
...
static int cnss_pci_wake_gpio_init(struct cnss_pci_data *pci_priv)
{
	...
	plat_priv->wake_irq = pci_priv->wake_irq;
