...
static void rtl8156_up(struct r8152 *tp)
{
	...
	case RTL_VER_11:
		...
		ocp_write_word(tp, MCU_TYPE_PLA, 0xc0a6,
			       (ocp_data + 0x800) / 8);
		ocp_write_word(tp, MCU_TYPE_PLA, 0xc0aa,
			       (ocp_data + 0x1800) / 8);
	...
	default:
		...
		ocp_write_dword(tp, MCU_TYPE_PLA, 0xe81a,
				(ocp_data + 0x800) / 16);
		break;
	}
...
static inline void __rtl_work_func(struct r8152 *tp)
{
	...
	if (test_bit(RTL8152_UNPLUG, &tp->flags) || !netif_running(tp->netdev)) {
		pr_err("%s:unplug or netif not runniung \n", __func__);
		return;
	}

	if (usb_autopm_get_interface(tp->intf) < 0) {
		pr_err("%s:cannot get interface\n", __func__);
		return;
	}

	if (!test_bit(WORK_ENABLE, &tp->flags)) {
		pr_err("%: not set WORK_ENABLE \n", __func__);
		goto out1;
	}
...
ocp_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	case RTL_VER_13:
		...
		strcat(buf, "\n");
		strcat(buf, "\n");
		strcat(buf, "uc2_patch_191014_usb\n");
		strcat(buf, "\n");
		strcat(buf, "\n");
		strcat(buf, "PLA_patch_code_20191121_v1\n");
		break;
	default:
		strcat(buf, "\n\n\n\n\n\n\n");
		break;
...
static const struct usb_device_id rtl8156_table[] = {
	/* Realtek */
	{REALTEK_USB_DEVICE(VENDOR_ID_REALTEK, 0x8156)},
	{}
};

MODULE_DEVICE_TABLE(usb, rtl8156_table);

static struct usb_driver rtl8156_driver = {
	.name =		MODULENAME,
	.id_table =	rtl8156_table,
	.probe =	rtl8152_probe,
...
module_usb_driver(rtl8156_driver);
