...
static size_t glink_smem_rx_avail(struct qcom_glink_pipe *np)
{
	...
	GLINK_INFO(smem_ilc, "Enter: RX READ:0x%x, WRITE:0x%x\n", readl(pipe->tail), readl(pipe->head));
				  ...
				  size_t count)
{
	...
	*pipe->tail = cpu_to_le32(tail);
	...
	GLINK_INFO(smem_ilc, "Exit: RX READ:0x%x, WRITE:0x%x\n", readl(pipe->tail), readl(pipe->head));
}
...
*name, void *ilc, unsigned int cmd, int locate) {
	...
	if (!strcmp(name, "adsp"))
	{
		tail = le32_to_cpu(*pipe->tail);
		head = le32_to_cpu(*pipe->head);
		LOG_IDX(ilc, "[%d] cmd:%d h:%x t:%x\n", locate, cmd, head, tail);
	}
}
					    ...
					    struct device_node *node)
{
	...
	*tx_pipe->head = 0;
	...
	smem_ilc = ipc_log_context_create(GLINK_LOG_PAGE_CNT, "smem_node", 0);
