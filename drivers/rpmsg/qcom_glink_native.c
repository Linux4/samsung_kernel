...
static irqreturn_t qcom_glink_native_intr(int irq, void *data)
{
	...
	for (;;) {
		...
		adsp_idx_logging(glink->rx_pipe, glink->name, glink->ilc, 0xFF, 1);
		...
		adsp_idx_logging(glink->rx_pipe, glink->name, glink->ilc, cmd, 2);
		...
		case RPM_CMD_RX_DONE_W_REUSE:
			...
			qcom_glink_rx_advance(glink, ALIGN(sizeof(msg), 8));
			if (!strcmp(glink->name,"adsp"))
			{
				qcom_glink_rx_peak(glink, &msg1, 0, sizeof(msg1));
				cmd1 = le16_to_cpu(msg1.cmd);
				adsp_idx_logging(glink->rx_pipe, glink->name, 
				glink->ilc, cmd1,4);
			}
			break;
		...
		case RPM_CMD_SIGNALS:
			...
			break;
		default:
			adsp_idx_logging(glink->rx_pipe, glink->name, glink->ilc, cmd, 3);
			if (!strcmp(glink->name,"adsp") && cmd == 48)
			BUG();
