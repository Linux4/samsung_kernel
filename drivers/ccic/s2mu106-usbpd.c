...
int s2mu106_set_water_lpm_mode(struct s2mu106_usbpd_data *pdic_data)
{
	u8 data, data_lpm;
	int ret = 0;
	struct i2c_client *i2c = pdic_data->i2c;
...
#ifdef CONFIG_SEC_FACTORY
	s2mu106_usbpd_write_reg(i2c, S2MU106_REG_PLUG_CTRL_SET_RD, S2MU106_THRESHOLD_385MV);
#else
...
static void s2mu106_usbpd_init_configure(struct s2mu106_usbpd_data *_data)
{
	...
	if (rid) {
		...
		s2mu106_usbpd_set_rp_scr_sel(_data, PLUG_CTRL_RP180);
		if (factory_mode) {
			if (rid != REG_RID_523K) {
