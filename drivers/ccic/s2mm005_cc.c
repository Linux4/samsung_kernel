...
void process_cc_attach(void * data,u8 *plug_attach_done)
{
	...
	if(of_find_property(usbpd_data->dev->of_node, "ccic,disable_water_det", NULL))
		usbpd_data->water_det = 0;	//add temporary to disable water det	
...
#if !defined(CONFIG_CCIC_S2MM005_PD_DISABLE)
...
#ifdef CONFIG_MUIC_SM5705_SWITCH_CONTROL_GPIO
			if ( (rid == RID_000K) || (rid == RID_001K) || (rid == RID_255K) 
			|| (rid == RID_301K) || (rid == RID_523K) || (rid == RID_619K) )            
					muic_GPIO_control(1);
