...
int s2mm005_ver_check(void *data)
{
	struct s2mm005_data *usbpd_data = (struct s2mm005_data *)data;
	...
	if (usbpd_data->firm_ver[2] == 0xff) {
		...
		s2mm005_get_chip_hwversion(usbpd_data, &chip_swver);
		pr_err("%s: CHIP SWversion %2x %2x %2x %2x\n", __func__,
				chip_swver.main[2], chip_swver.main[1],
				chip_swver.main[0], chip_swver.boot);

		if (chip_swver.main[0] == 0xff) {
			pr_err("%s Invalid FW version\n", __func__);
			return CCIC_VERSION_INVALID;
		}
...
static irqreturn_t s2mm005_init_detect_irq(struct s2mm005_data *usbpd_data)
{
	...
	ret = s2mm005_ver_check(usbpd_data);
	if (ret == CCIC_VERSION_INVALID) {
		dev_err(&i2c->dev, "%s: get invalid CCIC version\n", __func__);
		goto ver_err_init;
	}
			       ...
			       const struct i2c_device_id *id)
{
	...
	if (system_rev < 2)
	{
		dev_err(&i2c->dev, "Before REV02, don't support CCIC\n");
		return 0;
	}
	#endif
