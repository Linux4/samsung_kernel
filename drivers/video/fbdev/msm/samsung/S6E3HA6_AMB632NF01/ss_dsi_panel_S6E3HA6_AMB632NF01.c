...
static struct dsi_panel_cmds *mdss_pre_poc_burn_in(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	struct samsung_display_driver_data *vdd = check_valid_ctrl(ctrl);
	...
	*level_key = LEVEL_KEY_NONE;
	...
	return get_panel_tx_cmds(ctrl, TX_POC_BURN_IN_PRE);
}

static struct dsi_panel_cmds poc_burn_in_cmd;
static struct dsi_panel_cmds *mdss_poc_burn_in(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	struct samsung_display_driver_data *vdd = check_valid_ctrl(ctrl);
	struct dsi_panel_cmds *poc_burn_in_cmds = get_panel_tx_cmds(ctrl, TX_POC_BURN_IN);
	int cd_index = 0;
	int cmd_idx = 0;

	LCD_DEBUG("++ \n");

	if (IS_ERR_OR_NULL(vdd) || IS_ERR_OR_NULL(poc_burn_in_cmds)) {
		LCD_ERR("Invalid data ctrl : 0x%zx vdd : 0x%zx cmds : 0x%zx", (size_t)ctrl, (size_t)vdd, (size_t)poc_burn_in_cmds);
		return NULL;
	}

	cd_index  = vdd->cd_idx;
	cmd_idx = vdd->dtsi_data[ctrl->ndx].poc_burn_in_map_table[vdd->panel_revision].cmd_idx[cd_index];

	LCD_DEBUG("cd_index (%d) : cmd_idx(%d)\n", cd_index, cmd_idx);

	poc_burn_in_cmd.cmds = &(poc_burn_in_cmds->cmds[cmd_idx]);
	poc_burn_in_cmd.cmd_cnt = 1;
	*level_key = LEVEL1_KEY;

	LCD_DEBUG("-- \n");
	return &poc_burn_in_cmd;
}
...
static struct dsi_panel_cmds *mdss_hbm_poc_burn_in(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	struct samsung_display_driver_data *vdd = check_valid_ctrl(ctrl);
	struct dsi_panel_cmds * hbm_poc_burn_in_cmds = get_panel_tx_cmds(ctrl, TX_POC_BURN_IN_HBM);
	int para_idx = 0;

	LCD_DEBUG("++\n");

	if (IS_ERR_OR_NULL(vdd) || IS_ERR_OR_NULL(hbm_poc_burn_in_cmds)) {
		LCD_ERR("Invalid data ctrl : 0x%zx vdd : 0x%zx cmds : 0x%zx", (size_t)ctrl, (size_t)vdd, (size_t)hbm_poc_burn_in_cmds);
		return NULL;
	}

	if (IS_ERR_OR_NULL(hbm_poc_burn_in_cmds->cmds)) {
		LCD_ERR("No HBM_POC_BURN_IN_tx_cmds\n");
		return NULL;
	}
...
static void  mdss_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_brightness_pre_poc_burn_in = mdss_pre_poc_burn_in;
	vdd->panel_func.samsung_brightness_poc_burn_in = mdss_poc_burn_in;
	...
	vdd->panel_func.samsung_hbm_poc_burn_in = mdss_hbm_poc_burn_in;
