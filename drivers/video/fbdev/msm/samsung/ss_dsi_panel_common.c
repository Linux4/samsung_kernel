			...
			struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	for (panel_rev = 0; panel_rev < SUPPORT_PANEL_REVISION; panel_rev++) {
		...
		/* PoC */
		if (vdd->poc_driver.is_support_common) {
			...
			struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	vdd->poc_driver.is_support_common = of_property_read_bool(np, "samsung,support_poc_driver_common");
	LCD_INFO("vdd->poc_drvier.is_support_common = %d\n", vdd->poc_driver.is_support_common);
