 ...
 * 3. M_GRAY[VT], M_GRAY[V0] 's value in TP_M_GRAY_calc() // these are fixed like 0 or 1..
 */

#include "ss_dsi_smart_dimming_S6E3FA3_AMS598NJ01.h"
...
static int generate_gray_scale(struct SMART_DIM *pSmart)
{
	...
	*/
	V_idx = V0; // start V of gray scale
...
static void TP_M_GRAY_calc(struct SMART_DIM *pSmart, long long *L, int *M_GRAY, int table_index)
{
	...
	/* 2.2.6 - add offset for Candela Gamma Compensation (V255~V3) */
	for (i = V255; i >= V3; i--) {
		if (table_index == -1) {
...
struct smartdim_conf *smart_get_conf_S6E3FA3_AMS598NJ01(void) {
