	...
	*/
	for (wait_cnt = 0; wait_cnt < MAX_POC_SHOW_WAIT; wait_cnt++) {
		...
		if ((vdd->vdd_blank_mode[ndx] ==  FB_BLANK_UNBLANK) &&
			...
			pr_err("vdd_blank_mode (%d) vdd_blank_mode_done(%d) \n", vdd->vdd_blank_mode[ndx], vdd->vdd_blank_mode_done[ndx]);
			break;
		}
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (vdd->gct.is_support && vdd->panel_func.samsung_gct_write) {
		...
		/* In SDM660, esd interrupt is not generated. So add force recovery using panel_dead*/
		mdss_fb_report_panel_dead(pstatus_data->mfd);
	} else {
		LCD_ERR("gct is not supported.. %d\n", vdd->gct.is_support);
	}
	LCD_INFO("-\n");
