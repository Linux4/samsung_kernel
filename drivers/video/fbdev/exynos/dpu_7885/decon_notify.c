	...
	unsigned long val, void *v)
{
	...
	early_delta = max(EVENT_TIME[FB_EARLY_EVENT_BLANK], EVENT_TIME[DECON_EARLY_EVENT_DOZE]);
	early_delta = (EVENT_TIME[val] - early_delta) / NSEC_PER_MSEC;

	after_delta = max(EVENT_TIME[FB_EVENT_BLANK], EVENT_TIME[DECON_EVENT_DOZE]);
	after_delta = (EVENT_TIME[val] - after_delta) / NSEC_PER_MSEC;

	frame_delta = (EVENT_TIME[val] - EVENT_TIME[DECON_EVENT_FRAME]) / NSEC_PER_MSEC;
	...
	else if (val == DECON_EVENT_FRAME_DONE)
		pr_info("decon: decon_notifier: blank_mode: %d, %02lx, * %-*s, %-*s, %lld(%lld)\n",
