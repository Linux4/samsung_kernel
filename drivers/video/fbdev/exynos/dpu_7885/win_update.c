		...
		struct decon_reg_data *regs)
{
	...
	if(decon->lcd_info->dsc_enabled){
		...
		if (decon->lcd_info->xres - r2.left < MIN_WIN_BLOCK_WIDTH)
			r2.left = ((r1.left - MIN_WIN_BLOCK_WIDTH) / decon->win_up.rect_w) *
			decon->win_up.rect_w;
		if (decon->lcd_info->yres - r2.top < MIN_WIN_BLOCK_HEIGHT)
			r2.top = ((r1.top - MIN_WIN_BLOCK_HEIGHT) / decon->win_up.rect_h) *
			decon->win_up.rect_h;
