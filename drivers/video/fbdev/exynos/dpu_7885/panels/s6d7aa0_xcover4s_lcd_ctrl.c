...
#include "s6d7aa0_xcover4s_param.h"
#include "dd.h"
...
#define S6D7AA0_ID_REG			0x04	/* LCD ID1,ID2,ID3 */
...
static int s6d7aa0_read_id(struct lcd_info *lcd)
{
	...
	ret = dsim_read_hl_data(lcd, S6D7AA0_ID_REG, S6D7AA0_ID_LEN, lcd->id_info.id);
...
static int s6d7aa0_init(struct lcd_info *lcd)
{
	...
	DSI_WRITE(SEQ_TEST_KEY_ON_F1, ARRAY_SIZE(SEQ_TEST_KEY_ON_F1));
	...
	DSI_WRITE(SEQ_S6D7AA0_BL_LEVEL, ARRAY_SIZE(SEQ_S6D7AA0_BL_LEVEL));
	...
	DSI_WRITE(SEQ_TEST_KEY_OFF_F1, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F1));
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value == 1) {
		...
		DSI_WRITE(SEQ_TEST_KEY_ON_FC, ARRAY_SIZE(SEQ_TEST_KEY_ON_FC));
		DSI_WRITE(SEQ_TEST_KEY_ON_F1, ARRAY_SIZE(SEQ_TEST_KEY_ON_F1));
		DSI_WRITE(SEQ_S6D7AA0_CABC_ON, ARRAY_SIZE(SEQ_S6D7AA0_CABC_ON));
		DSI_WRITE(SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
		DSI_WRITE(SEQ_TEST_KEY_OFF_FC, ARRAY_SIZE(SEQ_TEST_KEY_OFF_FC));
		DSI_WRITE(SEQ_TEST_KEY_OFF_F1, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F1));
	} else {
...
struct dsim_lcd_driver s6d7aa0_mipi_lcd_driver = {
	.name		= "s6d7aa0",
