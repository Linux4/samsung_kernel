...
{
	int ret = 0, opr_status = ACL_STATUS_15P, acl_status = ACL_STATUS_15P;
	union acl_info acl_value = {0, };
	...
	acl_value.percent = lcd->acl_table[opr_status][LDI_OFFSET_ACL];
...
static int dsim_panel_set_hmt_brightness(struct lcd_info *lcd, int force)
{
	...
	dev_info(&lcd->ld->dev, "%s: hmt_brightness: %d, hmt bl: %d, %x %x, lx: %d\n", __func__, lcd->hmt_brightness,
		hmt_brightness_table[lcd->hmt_brightness], bl_reg[1], bl_reg[2], lcd->lux);
