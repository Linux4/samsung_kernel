...
static int dsim_panel_set_brightness(struct lcd_info *lcd, int force)
{
	...
	if (!!lcd->brightness != !!lcd->current_brightness) {
		...
		i2c_smbus_write_byte_data(lcd->backlight_client, 0x24, !lcd->brightness ? 1 : 0);
	}
	...
	dev_info(&lcd->ld->dev, "%s: brightness: %d, %d(%2x %2x), lx: %d\n", __func__,
				...
				unsigned long event, void *data)
{
	...
	if (fb_blank == FB_BLANK_UNBLANK)
		...
		s2dps01_array_write(lcd->backlight_client, s2dps01_exit_param, ARRAY_SIZE(s2dps01_exit_param));

		ret = gpio_request_one(lcd->gpio_lcd_1p8, GPIOF_OUT_INIT_LOW, "gpio_lcd_1p8");
		if (ret < 0)
			dev_err(&lcd->ld->dev, "%s: failed to set BL GPIO\n", __func__);
		else {
			dev_info(&lcd->ld->dev, "%s: Turn off Power 1p8\n", __func__);
			gpio_free(lcd->gpio_lcd_1p8);
		}
	}
...
static int hx83102d_probe(struct lcd_info *lcd)
{
	...
	lcd->gpio_lcd_1p8 = of_get_gpio_with_name("gpio_lcd_1p8");
...
static void lcd_init_sysfs(struct lcd_info *lcd)
{
	...
	init_debugfs_param("blic_init", &s2dps01_init_param, 8 * sizeof(u8), ARRAY_SIZE(s2dps01_init_param), 3);
	init_debugfs_param("blic_exit", &s2dps01_exit_param, 8 * sizeof(u8), ARRAY_SIZE(s2dps01_exit_param), 3);
}
...
static int dsim_panel_resume_early(struct dsim_device *dsim)
{
	...
	s2dps01_array_write(lcd->backlight_client, s2dps01_init_param, ARRAY_SIZE(s2dps01_init_param));
