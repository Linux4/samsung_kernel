...
static int s6e8fc0_read_coordinate(struct lcd_info *lcd)
{
	...
	ret = s6e8fc0_read_info(lcd, LDI_REG_COORDINATE, LDI_LEN_COORDINATE + LDI_LEN_DATE, &buf[LDI_GPARA_COORDINATE]);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);

	ret = s6e8fc0_read_info(lcd, LDI_REG_MANUFACTURE_INFO, LDI_LEN_MANUFACTURE_INFO, &buf[LDI_GPARA_MANUFACTURE_INFO]);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
