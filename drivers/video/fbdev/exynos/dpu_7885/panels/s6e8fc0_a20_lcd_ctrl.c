...
static int dsim_panel_set_acl(struct lcd_info *lcd, int force)
{
	...
	unsigned int acl_value = 0, opr_value = 0;

	opr_status = brightness_opr_table[!!lcd->adaptive_control][lcd->brightness];
	acl_status = !!opr_status;

	acl_value = lcd->acl_table[acl_status][LDI_OFFSET_ACL];
	opr_value = lcd->opr_table[opr_status][LDI_OFFSET_ACL];

	if (force)
		goto update;
	else if (lcd->current_acl != acl_value)
		goto update;
	else if (lcd->current_opr != opr_value)
		goto update;
	else
		goto exit;
