...
{
	struct exynos_md *md;
	struct decon_lcd *actual_lcd_info;
	...
	actual_lcd_info = (struct decon_lcd *)v4l2_get_subdev_hostdata(decon->output_sd);
	if (decon->pdata->dsi_mode == DSI_MODE_DUAL_DSI) {
		memcpy(&decon->dual_lcd_info, actual_lcd_info, sizeof(struct decon_lcd));
		decon->dual_lcd_info.xres = actual_lcd_info->xres * 2;
		actual_lcd_info = &decon->dual_lcd_info;
	}
	decon->lcd_info = actual_lcd_info;
...
int decon_config_eint_for_te(struct platform_device *pdev, struct decon_device *decon)
{
	...
	int gpio, gpio1;
#endif
#ifdef CONFIG_EXYNOS7880_DISPLAY_TE_IRQ_GIC
	struct resource *res;
#endif
	int ret = 0;
...
#ifdef CONFIG_EXYNOS7880_DISPLAY_TE_IRQ_GIC
	/* TE SIGNAL */
	res = platform_get_resource(pdev, IORESOURCE_IRQ, 6);
	if (res) {
...
int decon_lcd_off(struct decon_device *decon)
{
	...
	if (decon->pdata->dsi_mode == DSI_MODE_DUAL_DSI) {
		ret = v4l2_subdev_call(decon->output_sd1, core, ioctl, DSIM_IOC_LCD_OFF, NULL);
		if (ret < 0)
			decon_err("failed to turn off LCD\n");
	}
