...
static int s6e3fc2_read_info(struct lcd_info *lcd, u8 reg, u32 len, u8 *buf)
{
	int ret = 0, i;
...
static int s6e3fc2_read_id(struct lcd_info *lcd)
{
	...
	ret = s6e3fc2_read_info(lcd, LDI_REG_ID, LDI_LEN_ID, lcd->id_info.id);
...
static int s6e3fc2_read_panel_info(struct lcd_info *lcd)
{
	...
	ret = s6e3fc2_read_info(lcd, LDI_REG_COORDINATE, ARRAY_SIZE(buf), buf);
	if (ret < 0)
...
static int s6e3fc2_read_chip_id(struct lcd_info *lcd)
{
	...
	ret = s6e3fc2_read_info(lcd, LDI_REG_CHIP_ID, LDI_LEN_CHIP_ID, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
...
#ifdef CONFIG_DISPLAY_USE_INFO
static int s6e3fc2_inc_dpui_u32_field(struct lcd_info *lcd, enum dpui_key key, u32 value)
{
	if (lcd->connected) {
...
static int s6e3fc2_read_rdnumpe(struct lcd_info *lcd)
{
	int ret = 0;
	unsigned char buf[LDI_LEN_RDNUMPE] = {0, };

	ret = s6e3fc2_read_info(lcd, LDI_REG_RDNUMPE, LDI_LEN_RDNUMPE, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
	else
		s6e3fc2_inc_dpui_u32_field(lcd, DPUI_KEY_PNDSIE, (buf[0] & LDI_PNDSIE_MASK));
...
static int s6e3fc2_read_esderr(struct lcd_info *lcd)
{
	...
	ret = s6e3fc2_read_info(lcd, LDI_REG_ESDERR, LDI_LEN_ESDERR, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
...
static int s6e3fc2_read_rddsdr(struct lcd_info *lcd)
{
	...
	ret = s6e3fc2_read_info(lcd, LDI_REG_RDDSDR, LDI_LEN_RDDSDR, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
	else
		s6e3fc2_inc_dpui_u32_field(lcd, DPUI_KEY_PNSDRE, !(buf[0] & LDI_PNSDRE_MASK));
...
static int s6e3fc2_read_rddpm(struct lcd_info *lcd)
{
	...
	ret = s6e3fc2_read_info(lcd, LDI_REG_RDDPM, LDI_LEN_RDDPM, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
	else
...
static int s6e3fc2_read_rddsm(struct lcd_info *lcd)
{
	...
	ret = s6e3fc2_read_info(lcd, LDI_REG_RDDSM, LDI_LEN_RDDSM, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
	else
...
#if defined(CONFIG_SUPPORT_MASK_LAYER)
	lcd->mask_brightness = 319; /* 525 nit */
#endif
...
#if defined(CONFIG_SEC_FACTORY)
static DEVICE_ATTR(enable_fd, 0220, NULL, fast_discharge_store);
#endif
...
static int dsim_panel_mask_brightness(struct dsim_device *dsim)
{
	...
	if (regs->mask_layer == true && decon->current_mask_layer == false) {
		...
		dsim_panel_set_brightness(lcd, 1);
		decon_abd_save_log_event(&decon->abd, "mask_brightness");

		decon_wait_for_vsync(decon, VSYNC_TIMEOUT_MSEC);
		decon_info("%s: MASK_LAYER TE 2\n", __func__);
		decon_abd_save_log_event(&decon->abd, "mask_te_2");
		decon_wait_for_vsync(decon, VSYNC_TIMEOUT_MSEC);
		decon_info("%s: MASK_LAYER TE 3\n", __func__);
		decon_abd_save_log_event(&decon->abd, "mask_te_3");

		decon_reg_start(decon->id, &psr);

		decon_info("%s: MASK_LAYER trigger done.\n", __func__);
		decon_abd_save_log_event(&decon->abd, "mask_trigger_done");
	...
	if (regs->mask_layer == false && decon->current_mask_layer == true) {
		...
		dsim_panel_set_brightness(lcd, 1);
		decon_abd_save_log_event(&decon->abd, "prev_brightness");
