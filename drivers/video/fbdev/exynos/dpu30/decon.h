...
struct decon_reg_data {
	...
	struct decon_dma_buf_data dma_hdr_buf_data[MAX_DECON_WIN];
	struct decon_dma_buf_data dma_dqe_buf_data;
	struct dma_fence *retire_fence;
	struct dpp_hdr_lut hdr_lut[MAX_DECON_WIN];
	struct decon_dqe_lut dqe_lut;
	bool is_dqe_regs_for_hdr;
...
#if defined(CONFIG_EXYNOS_DECON_DQE)
static inline bool decon_is_dqe_for_hdr(struct decon_device *decon)
{
	return atomic_read(&decon->is_dqe_for_hdr);
}

static inline void decon_set_dqe_for_hdr(struct decon_device *decon, bool on)
{
	atomic_set(&decon->is_dqe_for_hdr, !!on);
}
#endif
