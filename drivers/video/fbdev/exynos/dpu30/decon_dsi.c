...
#if defined(CONFIG_EXYNOS_DECON_DQE)
		if (decon_is_dqe_for_hdr(decon))
			decon_dqe_lpd_data_read_for_hdr(decon);
		else
...
#ifdef CONFIG_DYNAMIC_FREQ
		ret_df = v4l2_subdev_call(decon->panel_sd, core, ioctl,
...
int decon_set_par(struct fb_info *info)
{
	...
	decon_reg_set_window_control(decon->id, win_no, &win_regs, false, 0);
...
#if defined(CONFIG_EXYNOS_DECON_DQE)
	if (decon_is_dqe_for_hdr(decon)) {
		decon_dqe_restore_context_for_hdr(decon);
		decon_dqe_lpd_data_write_for_hdr(decon);
	} else {
...
static int dpu_set_pre_df_dsim(struct decon_device *decon)
{
	...
	if (status->context)
		decon_info("[DYN_FREQ]:INFO:%s:target hs : %d, m : %d, k:%d ctx:%d\n",
