...
static void decon_reg_set_bpc_10bit(u32 id, u32 en)
{
	u32 val = 0, mask;
...
static void decon_reg_set_10bit_blend_dither(u32 id, u32 en)
{
	decon_write_mask(id, DECON_10BIT_BLEND_DITHER_EN,
			en ? ~0 : 0, DECON_10BIT_BLEND_DITHER_EN_F);
}
...
static void decon_reg_set_win_bnd_function(u32 id, u32 win_idx,
		struct decon_window_regs *regs, u32 hdr_en)
{
	...
	if (hdr_en && (blend == DECON_BLENDING_PREMULT))
		blend = DECON_BLENDING_COVERAGE;
		...
		struct decon_window_regs *regs, u32 winmap_en, u32 hdr_en)
{
	...
	if (win_en) {
		...
		decon_reg_set_win_bnd_function(0, win_idx, regs, hdr_en);
...
{
	decon_reg_set_10bit_blend_dither(id, en);
	decon_reg_set_bpc_10bit(id, en);
}
