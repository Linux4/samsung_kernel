...
int dpp_hdr_tune_en = 0;
module_param(dpp_hdr_tune_en, int, 0644);

struct dpp_device *dpp_drvdata[SOC_DPP_CNT];

struct hdr_device *hdr_drvdata[SOC_DPP_CNT];
struct class *hdr_class[SOC_DPP_CNT];
char hdr_result[2048];
...
static int dpp_check_format(struct dpp_device *dpp, struct dpp_params_info *p)
{
	...
	/* TODO : seperate HDR and WCG, the enum of transfer should be changed */
	if (!(test_bit(DPP_ATTR_HDR, &dpp->attr)
			|| test_bit(DPP_ATTR_C_WCG, &dpp->attr)
			|| test_bit(DPP_ATTR_HDR10P, &dpp->attr))
					&& (p->hdr > DPP_HDR_OFF)) {
		dpp_err("Not support WCG/HDR/HDR10P in DPP%d - No H/W!\n",
				dpp->id);
		return -EINVAL;
	}
#if 0
	/* TODO : HOW to check WCG, there is no config for WCG */
	/* TODO : HOW to check HDR10P, there is no config for HDR10P yet */
	if (!test_bit(DPP_ATTR_HDR10P, &dpp->attr)
					&& (p->hdr == DPP_HDR_ST2096)) {
		dpp_err("Not support HDR10P in DPP%d - No H/W!\n",
				dpp->id);
		return -EINVAL;
	}
#endif
	if ((p->hdr < DPP_HDR_OFF) || (p->hdr >= DPP_HDR_MAX)) {
...
void dpp_get_hdr_params(struct dpp_device *dpp, struct dpp_params_info *p)
{
	struct hdr_device *hdr = hdr_drvdata[dpp->id];
	struct dpp_hdr_params *hdr_lut = NULL;
	hdr_lut = &p->hdr_params;
	...
	if (dpp_hdr_tune_en) {
		...
		mutex_lock(&hdr->lock);
		p->is_hdr_tune = true;
		memcpy(&dpp->dpp_config->hdr_lut, &dpp->hdr10_tune, sizeof(struct dpp_hdr_lut));
		hdr_lut->hdr_en 	= dpp->dpp_config->hdr_lut.hdr_en;
		hdr_lut->eotf_en 	= dpp->dpp_config->hdr_lut.eotf_en;
		hdr_lut->gm_en 		= dpp->dpp_config->hdr_lut.gm_en;
		hdr_lut->tm_en 		= dpp->dpp_config->hdr_lut.tm_en;
		hdr_lut->oetf_en 	= dpp->dpp_config->hdr_lut.oetf_en;

		hdr_lut->eotf_x 	= dpp->dpp_config->hdr_lut.eotf_x;
		hdr_lut->eotf_y 	= dpp->dpp_config->hdr_lut.eotf_y;
		hdr_lut->gm_coef 	= dpp->dpp_config->hdr_lut.gm_coef;
		hdr_lut->tm_coef	= dpp->dpp_config->hdr_lut.tm_coef;
		hdr_lut->tm_rngx 	= dpp->dpp_config->hdr_lut.tm_rngx;
		hdr_lut->tm_rngy 	= dpp->dpp_config->hdr_lut.tm_rngy;
		hdr_lut->tm_x 		= dpp->dpp_config->hdr_lut.tm_x;
		hdr_lut->tm_y 		= dpp->dpp_config->hdr_lut.tm_y;
		hdr_lut->oetf_x 	= dpp->dpp_config->hdr_lut.oetf_x;
		hdr_lut->oetf_y 	= dpp->dpp_config->hdr_lut.oetf_y;
...
static int dpp_init_resources(struct dpp_device *dpp, struct platform_device *pdev)
{
	...
	if (dpp->id == 0) {
		...
		if (test_bit(DPP_ATTR_C_WCG, &dpp->attr)) {
...
#if defined(CONFIG_EXYNOS_HDR_TUNE)
static ssize_t dpp_hdr_param_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t count = 0;
	struct hdr_device *hdr = dev_get_drvdata(dev);
	...
	sprintf(str_temp,"hdr_en(%d), tm_en(%d), gm_en(%d), oetf_en(%d), eotf_en(%d)\n",
			dpp->hdr10_tune.hdr_en,
			dpp->hdr10_tune.tm_en, dpp->hdr10_tune.gm_en,
			dpp->hdr10_tune.oetf_en, dpp->hdr10_tune.eotf_en);
	strcat(hdr_result, str_temp);

	sprintf(str_temp,"gm_coef : ");
	strcat(hdr_result, str_temp);
	for (i = 0; i < 9; i++) {
		sprintf(str_temp, "%d ", dpp->hdr10_tune.gm_coef[i]);
		strcat(hdr_result, str_temp);
	}
	...
	count = sprintf(buf, "%s", hdr_result);

	return count;
}

static ssize_t dpp_hdr_param_store(struct device *dev, struct device_attribute *attr,
		const char *buffer, size_t count)
{
	...
	mutex_lock(&hdr->lock);
	memcpy(&dpp->hdr10_tune, buffer, sizeof(struct dpp_hdr_lut));
	mutex_unlock(&hdr->lock);

	return count;
}

static DEVICE_ATTR(hdr, 0660,
	dpp_hdr_param_show,
	dpp_hdr_param_store);

static struct attribute *hdr_attrs[] = {
	&dev_attr_hdr.attr,
	NULL,
};
ATTRIBUTE_GROUPS(hdr);

int dpp_hdr_create_interface(struct dpp_device *dpp)
{
	int ret = 0;
	struct hdr_device *hdr;
	char class_name[32] = "\0";

	if (!test_bit(DPP_ATTR_HDR, &dpp->attr)
			&& !test_bit(DPP_ATTR_C_WCG, &dpp->attr)) {
		dpp_info("dpp%d doesn't need hdr interface\n", dpp->id);
		return 0;
	}

	hdr = kzalloc(sizeof(struct hdr_device), GFP_KERNEL);
	if (!hdr) {
		ret = -ENOMEM;
		goto exit0;
	}

	hdr_drvdata[dpp->id] = hdr;
	hdr->dpp = dpp;

	if (IS_ERR_OR_NULL(hdr_class[dpp->id])) {
		sprintf(class_name, "hdr%d", dpp->id);
		hdr_class[dpp->id] = class_create(THIS_MODULE, class_name);
		if (IS_ERR_OR_NULL(hdr_class[dpp->id])) {
			dpp_err("failed to create hdr class\n");
			ret = -EINVAL;
			goto exit1;
		}

		hdr_class[dpp->id]->dev_groups = hdr_groups;
	}

	hdr->dev = device_create(hdr_class[dpp->id], dpp->dev, 0,
			&hdr, "hdr%d", dpp->id);
	if (IS_ERR_OR_NULL(hdr->dev)) {
		pr_err("failed to create hdr device\n");
		ret = -EINVAL;
		goto exit2;
	}
	...
	dpp_info("dpp%d_hdr_create_interface done.\n", dpp->id);

	return ret;
exit2:
	class_destroy(hdr_class[dpp->id]);
exit1:
	kfree(hdr);
exit0:
	return ret;
...
#if defined(CONFIG_EXYNOS_HDR_TUNE)
	ret = dpp_hdr_create_interface(dpp);
	if (ret)
		goto err_clk;
#endif
	dpp->state = DPP_STATE_OFF;
