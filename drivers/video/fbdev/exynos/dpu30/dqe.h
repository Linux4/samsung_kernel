...
struct dqe_device {
	...
	struct dqe_ctx ctx;
	struct dqe_ctx_for_hdr ctx_for_hdr;
	enum dqe_state state;
	u32 lpd_data[LPD_DATA_SIZE];
	u32 lpd_data_for_hdr[LPD_DATA_SIZE];
};
...
void decon_dqe_lpd_data_read_for_hdr(struct decon_device *decon);
void decon_dqe_lpd_data_write_for_hdr(struct decon_device *decon);
void decon_dqe_restore_context_for_hdr(struct decon_device *decon);
void decon_dqe_set_for_hdr(struct decon_device *decon, struct decon_reg_data *regs);
void decon_dqe_set_ctx_need_update(struct decon_device *decon, bool on);
bool decon_dqe_check_ctx_need_update(struct decon_device *decon);
