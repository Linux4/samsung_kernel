...
#if defined(CONFIG_EXYNOS_COMMON_PANEL) && !defined(CONFIG_NO_LCD)
...
static void decon_unmap_ion_handle(struct decon_device *decon,
		struct decon_dma_buf_data *dma)
{
	/* This is DVA(Device Virtual Address) for setting base address SFR */
	if (!IS_ERR_OR_NULL(dma->attachment) && !IS_ERR_VALUE(dma->dma_addr)) {
...
#if defined(CONFIG_EXYNOS_DECON_DQE)
static int decon_import_dqe_buffer(struct decon_device *decon,
		struct decon_win_config_data *win_data,
		struct decon_reg_data *regs)
{
	struct dma_buf *buf = NULL;
	struct decon_dma_buf_data *dma_dqe_buf_data = NULL;
	struct device *dev = NULL;
	struct dsim_device *dsim;
#if defined(CONFIG_EXYNOS_DISPLAYPORT)
	struct displayport_device *displayport;
#endif
	struct decon_dqe_lut *dqe_lut = NULL;
	size_t buf_size = 0;

	decon_dbg("%s +\n", __func__);

	regs->is_dqe_regs_for_hdr = false;

	memset(&regs->dqe_lut, 0, sizeof(struct decon_dqe_lut));
	if (win_data->fd_dqe < 0)
		return 0;

	if (win_data->fd_dqe < 3) {
		decon_err("invalid buffer fd_dqe(%d)\n", win_data->fd_dqe);
		return 0;
	}

	memset(&regs->dma_dqe_buf_data, 0,
			sizeof(struct decon_dma_buf_data));

	dma_dqe_buf_data = &regs->dma_dqe_buf_data;
	buf = dma_buf_get(win_data->fd_dqe);
	...
	buf_size = decon_map_ion_handle(decon, dev, dma_dqe_buf_data, buf, 0);
	if (!buf_size) {
		decon_err("failed to map buffer\n");
		goto err_map;
	}
#if 0
	if (buf_size != sizeof(struct decon_dqe_lut)) {
		decon_err("dqe_lut buf size if wrong, get size(%d) != origin size(%d)\n");
		goto err_map;
	}
#endif
	...
	memcpy(&regs->dqe_lut, dqe_lut, sizeof(struct decon_dqe_lut));

	decon_unmap_ion_handle(decon, dma_dqe_buf_data);

	dma_buf_vunmap(dma_dqe_buf_data->dma_buf, dqe_lut);

	dma_buf_put(buf);

	regs->is_dqe_regs_for_hdr = true;
...
static int decon_import_hdr_buffer(struct decon_device *decon, int idx,
		struct decon_win_config *config,
		struct decon_reg_data *regs)
{
	struct dma_buf *buf = NULL;
	struct decon_dma_buf_data *dma_hdr_buf_data = NULL;
	struct device *dev = NULL;
	struct dsim_device *dsim;
#if defined(CONFIG_EXYNOS_DISPLAYPORT)
	struct displayport_device *displayport;
#endif
	struct dpp_hdr_lut *hdr_lut = NULL;
	size_t buf_size = 0;

	decon_dbg("%s +\n", __func__);

	/* check whether it is wcg/hdr10/hdr10+ case or not
	 * before decision with HWC below case is basic
	 * hdr10+ case : hdr_std == st2096
	 * after decision with HWC add below case
	 * wcg case : input colorspace != target colorspace
	 * hdr10 case : hdr_std == st2084
	 */

	memset(&regs->hdr_lut[idx], 0, sizeof(struct dpp_hdr_lut));
	if (config->fd_hdr < 0)
		return 0;

	if (config->fd_hdr < 3) {
		decon_err("invalid buffer fd_hdr(%d)\n", config->fd_hdr);
		return 0;
	}

	memset(&regs->dma_hdr_buf_data[idx], 0,
			sizeof(struct decon_dma_buf_data));

	dma_hdr_buf_data = &regs->dma_hdr_buf_data[idx];
	buf = dma_buf_get(config->fd_hdr);
	if (IS_ERR_OR_NULL(buf)) {
		decon_err("failed to get dma_buf:%ld\n", PTR_ERR(buf));
		goto err_get;
	}
	...
	buf_size = decon_map_ion_handle(decon, dev, dma_hdr_buf_data, buf, idx);
	if (!buf_size) {
		decon_err("failed to map buffer\n");
		goto err_map;
	}
#if 0
	if (buf_size != sizeof(struct dpp_hdr_lut)) {
		decon_err("hdr_lut buf size if wrong, get size(%d) != origin size(%d)\n");
		goto err_map;
	}
#endif
	/* KVA is passed to DPP parameters structure */
	...
	memcpy(&regs->hdr_lut[idx], hdr_lut, sizeof(struct dpp_hdr_lut));

	decon_unmap_ion_handle(decon, dma_hdr_buf_data);

	dma_buf_vunmap(dma_hdr_buf_data->dma_buf, hdr_lut);
		...
		struct decon_reg_data *regs, int idx)
{
	...
	ret = decon_import_hdr_buffer(decon, idx, config, regs);
	if (ret)
		goto err;
		...
		struct decon_reg_data *regs)
{
	...
	for (i = 0; i < decon->dt.max_win; i++) {
		...
		memcpy(&dpp_config.hdr_lut, &regs->hdr_lut[i],
				sizeof(struct dpp_hdr_lut));
...
static int __decon_update_regs(struct decon_device *decon, struct decon_reg_data *regs)
{
	...
	if (dpp_hdr_tune_en || regs->is_dqe_regs_for_hdr)
		decon_reg_set_10bit_global_control(decon->id, has_10bit_fmt);
	else
		decon_reg_set_10bit_global_control(decon->id, 0);
...
#if defined(CONFIG_EXYNOS_DECON_DQE)
	if (!dpp_hdr_tune_en && regs->is_dqe_regs_for_hdr) {
		decon_set_dqe_for_hdr(decon, 1);
		decon_dqe_set_for_hdr(decon, regs);
	} else {
		decon_set_dqe_for_hdr(decon, 0);
		if (decon_dqe_check_ctx_need_update(decon))
...
#if defined(CONFIG_EXYNOS_DECON_DQE)
	if (!dpp_hdr_tune_en && regs->is_dqe_regs_for_hdr)
		decon_dqe_set_ctx_need_update(decon, 1);
#endif
...
#if defined(CONFIG_EXYNOS_DECON_DQE)
	if (decon_import_dqe_buffer(decon, win_data, regs) < 0)
		decon_err("failed to import dqe buffer\n");
#endif
...
static int decon_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "%s NO_LCD bypass on\n", __func__);
#endif
#if defined(CONFIG_EXYNOS_COMMON_PANEL) || \
		defined(CONFIG_EXYNOS_READ_ESD_SOLUTION)
