...
static int getidx_fps_table(struct maptbl *tbl)
{
	...
	index = search_table_u32(S6E3FA9_VRR_FPS,
			ARRAY_SIZE(S6E3FA9_VRR_FPS), props->vrr.mode);
...
static void copy_grayspot_otp_maptbl(struct maptbl *tbl, u8 *dst)
{
	struct panel_device *panel;
	struct panel_info *panel_data;
	...
	resource_copy_by_name(panel_data, dst, "grayspot");
}
#endif
...
static int getidx_hmd_brt_table(struct maptbl *tbl)
{
	int row = 0;
	struct panel_info *panel_data;
	struct panel_bl_device *panel_bl;
