		...
		u8 *buf, u32 addr, u32 len)
{
...
#ifdef CONFIG_SUPPORT_POC_FLASH_CHKSUM
	...
	for (i = 0; i < len;) {
		...
		ret = resource_copy_by_name(panel_data, copy_buf, "poc_data");

		read_len = len - i;
		if (read_len > poc_info->rdata_len)
			read_len = poc_info->rdata_len;
		memcpy(&buf[i], copy_buf, read_len);
...
static int poc_get_octa_poc(struct panel_device *panel)
{
	...
	ret = resource_copy_by_name(panel_data, manufacture_info, "manufacture_info");
	if (unlikely(ret < 0)) {
		pr_err("%s failed to copy resource(manufacture_info) (ret %d)\n", __func__, ret);
		return ret;
	}
	poc_info->poc = manufacture_info[1] & 0x0F;
...
int set_panel_poc(struct panel_poc_device *poc_dev, u32 cmd, void *arg)
{
	...
	case POC_OP_DIM_READ:
		...
		if (index < POC_DIM_PARTITION || index > POC_DIM_PARTITION_END) {
			pr_err("%s, invalid index(%d) of dim partition\n",
					__func__, index);
			return -EINVAL;
		}
	...
	case POC_OP_MTP_READ:
		...
		if (index < POC_MTP_PARTITION || index > POC_MTP_PARTITION_END) {
			pr_err("%s, invalid index(%d) of mtp partition\n",
					__func__, index);
			return -EINVAL;
		}
...
int panel_poc_probe(struct panel_device *panel, struct panel_poc_data *poc_data)
{
	...
	poc_info->rdata_len = poc_data->rdata_len;
