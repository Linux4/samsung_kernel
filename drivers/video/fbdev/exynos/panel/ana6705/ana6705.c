/*
 * linux/drivers/video/fbdev/exynos/panel/ana6705/ana6705.c
 *
 * ana6705 Dimming Driver
 *
...
static int getidx_fps_table(struct maptbl *tbl)
{
	...
	index = search_table_u32(ANA6705_VRR_FPS,
			ARRAY_SIZE(ANA6705_VRR_FPS), props->vrr.mode);
...
static int init_mdnie_night_mode_table(struct maptbl *tbl)
{
	...
	if (sizeof_maptbl(tbl) < (ANA6705_NIGHT_MODE_OFS +
	...
	maptbl_copy(night_maptbl, &tbl->arr[ANA6705_NIGHT_MODE_OFS]);
...
static int init_mdnie_color_lens_table(struct maptbl *tbl)
{
	...
	if (sizeof_maptbl(tbl) < (ANA6705_COLOR_LENS_OFS +
	...
	if (IS_COLOR_LENS_MODE(mdnie))
		maptbl_copy(color_lens_maptbl, &tbl->arr[ANA6705_COLOR_LENS_OFS]);
...
static void show_err(struct dumpinfo *info)
{
	int ret;
	struct resinfo *res = info->res;
	u8 err[ANA6705_ERR_LEN] = { 0, }, err_15_8, err_7_0;
...
{
	int ret;
	u8 err_fg[ANA6705_ERR_FG_LEN] = { 0, };
	struct resinfo *res = info->res;
...
static void show_dsi_err(struct dumpinfo *info)
{
	int ret;
	struct resinfo *res = info->res;
	u8 dsi_err[ANA6705_DSI_ERR_LEN] = { 0, };
