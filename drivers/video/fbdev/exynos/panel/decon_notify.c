...
u32 EVENT_NAME_LEN;
u32 STATE_NAME_LEN;
u64 STAMP_TIME[SMCDSD_STAMP_MAX][CHAIN_MAX];

static u32 EVENT_TO_STAMP[SMCDSD_EVENT_MAX] = {
...
int decon_notifier_call_chain(unsigned long val, void *v)
{
	...
	if (val >= SMCDSD_EVENT_MAX || state >= SMCDSD_STATE_MAX) {
...
static BLOCKING_NOTIFIER_HEAD(decon_fb_notifier_list);

int decon_fb_register_client(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&decon_fb_notifier_list, nb);
}
EXPORT_SYMBOL(decon_fb_register_client);

int decon_fb_unregister_client(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&decon_fb_notifier_list, nb);
}
EXPORT_SYMBOL(decon_fb_unregister_client);

int decon_fb_notifier_call_chain(unsigned long val, void *v)
{
	return blocking_notifier_call_chain(&decon_fb_notifier_list, val, v);
}
EXPORT_SYMBOL_GPL(decon_fb_notifier_call_chain);

int decon_fb_simple_notifier_call_chain(unsigned long val, int blank)
{
	struct fb_info *fbinfo = registered_fb[0];
	struct fb_event v = {0, };
	int fb_blank = blank;
	...
	return decon_fb_notifier_call_chain(val, &v);
}
EXPORT_SYMBOL(decon_fb_simple_notifier_call_chain);
#endif

int __init decon_notifier_init(void)
{
...
#if defined(CONFIG_DRM_MEDIATEK)
	decon_fb_register_client(&decon_fb_notifier_blank_min);
	decon_fb_register_client(&decon_fb_notifier);
	decon_fb_register_client(&decon_fb_notifier_blank_max);
#else
