...
enum {	EVENT_LIST	SMCDSD_EVENT_MAX	};
enum {	STATE_LIST	SMCDSD_STATE_MAX	};
enum {	STAMP_LIST	SMCDSD_STAMP_MAX	};
#undef __XX
...
extern int decon_fb_register_client(struct notifier_block *nb);
extern int decon_fb_unregister_client(struct notifier_block *nb);
extern int decon_fb_notifier_call_chain(unsigned long val, void *v);
extern int decon_fb_simple_notifier_call_chain(unsigned long val, int blank);
#endif
