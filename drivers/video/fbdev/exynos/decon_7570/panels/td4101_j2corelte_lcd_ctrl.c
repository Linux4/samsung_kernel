...
static int td4101_init(struct lcd_info *lcd)
{
	...
	if (lcd->dsi_control) {
		DSI_WRITE(SEQ_TD4101_B6_836, ARRAY_SIZE(SEQ_TD4101_B6_836));
		dev_info(&lcd->ld->dev, "%s: dsi_control: %d\n", __func__, lcd->dsi_control);
	} else
...
static int td4101_get_dsi_control(struct lcd_info *lcd)
{
	struct device_node *node = NULL;
	char *dts_name = "lcd_info";
	unsigned int hs_clk = 0;

	node = of_find_node_with_property(NULL, dts_name);
	if (!node) {
		dev_info(&lcd->ld->dev, "%s: of_find_node_with_property fail\n", __func__);
		goto exit;
	}

	node = of_parse_phandle(node, dts_name, 0);
	if (!node) {
		dev_info(&lcd->ld->dev, "%s: of_parse_phandle fail\n", __func__);
		goto exit;
	}

	of_property_read_u32(node, "timing,dsi-hs-clk", &hs_clk);

	if (hs_clk == 836)
		lcd->dsi_control = 1;

	dev_info(&lcd->ld->dev, "%s: hs_clk: %d, dsi_control: %d\n", __func__, hs_clk, lcd->dsi_control);
exit:
	return 0;
}
