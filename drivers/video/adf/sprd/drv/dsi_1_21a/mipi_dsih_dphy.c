 ...
 */
#ifdef GEN_2
dsih_error_t mipi_dsih_dphy_configure(dphy_t * phy, uint8_t no_of_lanes,
				      uint32_t output_freq)
{
	...
	} loop_bandwidth[] = {	/* gen 2 associates the charge pump current and LPF resistor with the
	...
	for (input_divider = 1 + (phy->reference_freq / DPHY_DIV_UPPER_LIMIT);
		...
		if ((tmp_loop_divider % 2) == 0) {	/* if even */
			if (output_freq == (tmp_loop_divider * (phy->reference_freq / input_divider))) {	/* exact values found */
				...
				break;
			} else if ((output_freq - (tmp_loop_divider * (phy->reference_freq / input_divider))) < delta) {	/* values found with smaller delta */
				loop_divider = tmp_loop_divider;
				delta =
				    output_freq -
				    (tmp_loop_divider *
			...
			if (output_freq == (tmp_loop_divider * (phy->reference_freq / input_divider))) {	/* exact values found */
				...
				delta =
				    ...
				    output_freq;
				break;
			} else if (((tmp_loop_divider * (phy->reference_freq / input_divider)) - output_freq) < delta) {	/* values found with smaller delta */
				loop_divider = tmp_loop_divider;
				delta =
				    (tmp_loop_divider *
				     (phy->reference_freq / input_divider)) -
				      ...
				      uint32_t output_freq)
{
	...
	if ((!flag) || ((phy->reference_freq / input_divider) < DPHY_DIV_LOWER_LIMIT)) {	/* no exact value found in previous for loop */
	...
	for (i = 0;
	     (i < (sizeof(loop_bandwidth) / sizeof(loop_bandwidth[0])))
...
{
	mipi_dsih_dphy_write_part(instance, R_DPHY_IF_CFG, no_of_byte_cycles, 8,
				  8);
}
...
{
	mipi_dsih_dphy_write_part(instance, R_DPHY_IF_CFG, no_of_lanes - 1, 0,
				  2);
}
						...
						uint8_t trigger_request)
{
	...
	if (sum == 1) {		/* clear old trigger */
		mipi_dsih_dphy_write_part(instance, R_DPHY_TX_TRIGGERS, 0x00, 0,
					  4);
...
dsih_error_t mipi_dsih_dphy_ulps_data_lanes(dphy_t * instance, int enable)
{
	...
	if (enable) {
		...
		for (timeout = 0; timeout < DSIH_PHY_ACTIVE_WAIT; timeout++) {	/* verify that the DPHY has left ULPM */
...
void mipi_dsih_dphy_ulps_data_lanes(dphy_t * instance, int enable)
{
	int timeout;
	if (enable) {
		mipi_dsih_dphy_write_part(instance, R_DSI_HOST_PHY_IF_CTRL, 1,
					  3, 1);
	} else {
		mipi_dsih_dphy_write_part(instance, R_DSI_HOST_PHY_IF_CTRL, 1,
					  4, 1);
		for (timeout = 0; timeout < DSIH_PHY_ACTIVE_WAIT; timeout++) {	/* verify that the DPHY has left ULPM */
			/* mask 1010100100000 */
			if (mipi_dsih_dphy_status(instance, 0x1520) == 0) {	/* wait at least 1ms */
				for (timeout = 0; timeout < ONE_MS_ACTIVE_WAIT;
				     timeout++) {
					;
				}
		...
		mipi_dsih_dphy_write_part(instance, R_DSI_HOST_PHY_IF_CTRL, 0,
					  3, 1);
		mipi_dsih_dphy_write_part(instance, R_DSI_HOST_PHY_IF_CTRL, 0,
					  4, 1);
	}
...
dsih_error_t mipi_dsih_dphy_ulps_clk_lane(dphy_t * instance, int enable)
{
	...
	if (enable) {
		...
		for (timeout = 0; timeout < DSIH_PHY_ACTIVE_WAIT; timeout++) {	/* verify that the DPHY has left ULPM */
			/* mask 1010100100000 */
			if (mipi_dsih_dphy_status(instance, clk_lane_mask) == clk_lane_mask) {	/* wait at least 1ms */
				instance->log_info(" stat %x, mask %x",
						   mipi_dsih_dphy_status
						   (instance, clk_lane_mask),
						   clk_lane_mask);
				break;
			}
			/* wait at least 1ms */
			for (timeout = 0; timeout < ONE_MS_ACTIVE_WAIT; timeout++) {	/* dummy operation for the loop not to be optimised */
				enable =
				    mipi_dsih_dphy_status(instance,
							  clk_lane_mask);
			}
...
void mipi_dsih_dphy_ulps_clk_lane(dphy_t * instance, int enable)
{
	...
	if (enable) {
		...
		for (timeout = 0; timeout < DSIH_PHY_ACTIVE_WAIT; timeout++) {	/* verify that the DPHY has left ULPM */
			/* mask 1010100100000 */
			if (mipi_dsih_dphy_status(instance, 0x0004) == 0) {	/* wait at least 1ms */
				for (timeout = 0; timeout < ONE_MS_ACTIVE_WAIT;
				     timeout++) {
					;
				}
...
{
	mipi_dsih_dphy_write_part(instance, R_DPHY_TST_CRTL1, on_falling_edge,
				  16, 1);
}
 ...
 */
void mipi_dsih_dphy_write(dphy_t * instance, uint8_t address, uint8_t * data,
			  uint8_t data_length)
{
	unsigned i = 0;
	if (data != 0) {
 ...
 */
void mipi_dsih_dphy_write_word(dphy_t * instance, uint32_t reg_address,
			       uint32_t data)
{
 ...
 */
void mipi_dsih_dphy_write_part(dphy_t * instance, uint32_t reg_address,
 ...
 */
uint32_t mipi_dsih_dphy_read_part(dphy_t * instance, uint32_t reg_address,
