...
struct mipi_dsim_device *g_dsim = NULL;

extern void s5p_dynamic_clk_set(unsigned int mode)
{
	int hs_clk = 0;

	if (mode) { /*Another mode*/	/*432Mbps*/
		g_dsim->dsim_config->p = 4;
		g_dsim->dsim_config->m = 72;
		g_dsim->dsim_config->s = 1;
	} else {	/*Normal mode*/	/*492Mbps*/
		g_dsim->dsim_config->p = 4;
		g_dsim->dsim_config->m = 82;
		g_dsim->dsim_config->s = 1;
	}

	hs_clk = g_dsim->dsim_config->m * FIN_HZ / (g_dsim->dsim_config->p * (1 << g_dsim->dsim_config->s));

	if (!dphy_support_upto_1GHz())
		hs_clk *= 2;

	dev_info(g_dsim->dev, "%s : When Next wake up, HS clk will be changed to %d (%s). \n",
				__func__, hs_clk, mode ? "Another mode" : "Normal mode");
}
...
static int mipi_dsi_get_dphy_timing(struct mipi_dsim_device *dsim)
{
	bool loop = true;
	...
	switch ( (dsim->dsim_config->esc_clk ) / MHZ ) {
	/* Mhz */
	case 20:
...
static int mipi_dsi_set_dphy_timing_value(struct mipi_dsim_device *dsim)
{
		...
		if (byte_clk_sel == DSIM_PLL_OUT_DIV8) {
			...
			if (!soc_is_exynos5250() && !soc_is_exynos3250() && !soc_is_exynos3470() && !soc_is_exynos3472()) {
				...
				if (soc_is_exynos4415())
					s5p_mipi_dsi_set_b_dphyctrl(dsim, 0x1ed);
				else
				...
				s5p_mipi_dsi_set_timing_register2(dsim, 0x0c, 0x11,
									0x0d);
#else
				s5p_mipi_dsi_set_b_dphyctrl(dsim, 0x0FA);
				s5p_mipi_dsi_set_timing_register0(dsim, 0x03, 0x05);
#ifdef CONFIG_LCD_MIPI_D6EA8061
				s5p_mipi_dsi_set_timing_register1(dsim, 0x04, 0x16, 0x0A, 0x05);
#else
				s5p_mipi_dsi_set_timing_register1(dsim, 0x03, 0x12, 0x09, 0x04);
#endif
				s5p_mipi_dsi_set_timing_register2(dsim, 0x04, 0x05, 0x06);
#endif
#else
				dsim->hs_clk = hs_clk / MHZ;
				dsim->timing.bps = 0;
				mipi_dsi_get_dphy_timing(dsim);

				mipi_dsi_set_dphy_timing_value(dsim);

				dev_info(dsim->dev, "PHY timing: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x \n",
					dsim->timing.b_dphyctl, dsim->timing.lpx, dsim->timing.hs_exit,
					dsim->timing.clk_prepare, dsim->timing.clk_zero, dsim->timing.clk_post,
					dsim->timing.hs_prepare, dsim->timing.hs_zero, dsim->timing.hs_trail, dsim->timing.clk_trail);
#endif
			}
...
static int s5p_mipi_dsi_enable(struct mipi_dsim_device *dsim)
{
	...
	/* enable interrupts */
#if defined CONFIG_BACKLIGHT_POLLING_AMOLED
	s5p_mipi_dsi_set_interrupt_mask(dsim, RX_DAT_DONE, SFR_PH_FIFO_EMPTY, 0);
#else
...
static int s5p_mipi_dsi_probe(struct platform_device *pdev)
{
	...
	/* enable interrupts */
#if defined CONFIG_BACKLIGHT_POLLING_AMOLED
		s5p_mipi_dsi_set_interrupt_mask(dsim, RX_DAT_DONE, SFR_PL_FIFO_EMPTY, 0);
#else
	s5p_mipi_dsi_set_interrupt_mask(dsim, SFR_PH_FIFO_EMPTY | SFR_PL_FIFO_EMPTY | RX_DAT_DONE, 0);
#endif
