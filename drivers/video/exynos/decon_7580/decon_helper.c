...
static struct clk_list_t clk_list[disp_clks_max] = {
	{ .c_name = "disp_pll", },
	{ .c_name = "bus_pll", },
	{ .c_name = "mout_sclk_disp_decon_int_eclk_a", },
	{ .c_name = "mout_sclk_disp_decon_int_eclk_b", },
	{ .c_name = "mout_aclk_disp_333_user", },
	{ .c_name = "mout_sclk_decon_int_eclk_user", },
	{ .c_name = "sclk_disp_decon_int_eclk", },
	{ .c_name = "dout_sclk_disp_decon_int_eclk", },
	{ .c_name = "m_decon0_eclk", },
	{ .c_name = "d_decon0_eclk", },
	{ .c_name = "m_decon0_vclk", },
	{ .c_name = "d_decon0_vclk", },
	{ .c_name = "mout_aclk_disp_200", },
	{ .c_name = "dout_aclk_disp_200", },
	{ .c_name = "mout_aclk_disp_200_user", },
	{ .c_name = "d_pclk_disp", },
};

static int decon_clk_get(struct device *dev, enum disp_clks idx, const char *parent)
{
	struct clk *p;
	struct clk *c;
	int ret = 0;
...
int decon_clk_set_parent(struct device *dev, enum disp_clks idx, const char *parent)
{
	struct clk *p;
	struct clk *c;
	int ret = 0;
	...
	if (unlikely(IS_ERR_OR_NULL(clk_list[idx].c))) {
		ret = decon_clk_get(dev, idx, parent);
		if (ret < 0) {
			pr_err("%s: can't get clock: %s\n", __func__, conid);
			return ret;
		}
...
int decon_clk_set_rate(struct device *dev,  enum disp_clks idx, unsigned int rate)
{
	...
	if (unlikely(IS_ERR_OR_NULL(clk_list[idx].c))) {
		ret = decon_clk_get(dev, idx, NULL);
		if (ret < 0) {
			pr_err("%s: can't get clock: %s\n", __func__, conid);
			return ret;
		}
