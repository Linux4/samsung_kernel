...
int _ana38401_write(struct lcd_info *lcd, const u8 *seq, u32 len)
{
	int ret = 0;
	int retry;
...
int _ana38401_read(struct lcd_info *lcd, u8 addr, u8 *buf, u32 len)
{
	int ret = 0;
	u8 cmd;
	int retry;
...
static int ana38401_read_coordinate(struct lcd_info *lcd)
{
	...
	ret = ana38401_read(lcd, LDI_COORDINATE_REG, buf, LDI_COORDINATE_LEN);
...
static int ana38401_read_id(struct lcd_info *lcd, u8 *buf)
{
	...
	ret = _ana38401_read(lcd, LDI_ID_REG, buf, LDI_ID_LEN);
...
static int ana38401_read_code(struct lcd_info *lcd, u8 *buf)
{
	int ret;

	ret = ana38401_read(lcd, LDI_CODE_REG, buf, LDI_CODE_LEN);
...
static int ana38401_read_mtp(struct lcd_info *lcd, u8 *buf)
{
	...
	ret = ana38401_read(lcd, LDI_MTP_REG, buf, LDI_MTP_LEN);
...
static int ana38401_read_elvss(struct lcd_info *lcd, u8 *buf)
{
	int ret, i;

	ret = ana38401_read(lcd, LDI_ELVSS_REG, buf, LDI_ELVSS_LEN);
...
static int ana38401_read_tset(struct lcd_info *lcd)
{
	int ret, i;

	ret = ana38401_read(lcd, LDI_TSET_REG, &lcd->tset_table[1], LDI_TSET_LEN);
...
acl_update:
	if (force || lcd->current_acl != ACL_CUTOFF_TABLE[level][1]) {
		ret = ana38401_write(lcd, ACL_CUTOFF_TABLE[level], ACL_PARAM_SIZE);
		ret += ana38401_write(lcd, ACL_OPR_TABLE[level], OPR_PARAM_SIZE);
...
elvss_update:
	ret = ana38401_write(lcd, lcd->elvss_table[acl][elvss_level], ELVSS_PARAM_SIZE);
...
static int ana38401_set_tset(struct lcd_info *lcd, u8 force)
{
	int ret = 0;
	u8 tset;
	...
	if (force || lcd->tset_table[LDI_TSET_LEN] != tset) {
		...
		ret = ana38401_write(lcd, lcd->tset_table, TSET_PARAM_SIZE);
...
static int update_brightness(struct lcd_info *lcd, u8 force)
{
	...
	if (force || (lcd->ldi_enable && (lcd->current_bl != lcd->bl))) {
		ana38401_write(lcd, SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
		ana38401_gamma_ctl(lcd);
		ana38401_aid_parameter_ctl(lcd, force);
		ana38401_set_elvss(lcd, force);
		ana38401_set_acl(lcd, force);
		ana38401_set_tset(lcd, force);
		ana38401_write(lcd, SEQ_GAMMA_UPDATE, ARRAY_SIZE(SEQ_GAMMA_UPDATE));
		ana38401_write(lcd, SEQ_GAMMA_UPDATE_L, ARRAY_SIZE(SEQ_GAMMA_UPDATE_L));
		ana38401_write(lcd, SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
...
static int ana38401_ldi_init(struct lcd_info *lcd)
{
	...
	pr_info("%s: Gamma Direct Wirte\n", __func__);

	_ana38401_write(lcd, SEQ_GAMMA_CONTROL, ARRAY_SIZE(SEQ_GAMMA_CONTROL));
	_ana38401_write(lcd, SEQ_AID_SET, ARRAY_SIZE(SEQ_AID_SET));
	_ana38401_write(lcd, SEQ_ELVSS_SET_1, ARRAY_SIZE(SEQ_ELVSS_SET_1));
	_ana38401_write(lcd, SEQ_ELVSS_SET_2, ARRAY_SIZE(SEQ_ELVSS_SET_2));
	_ana38401_write(lcd, SEQ_GAMMA_UPDATE_1, ARRAY_SIZE(SEQ_GAMMA_UPDATE_1));
	_ana38401_write(lcd, SEQ_GAMMA_UPDATE_2, ARRAY_SIZE(SEQ_GAMMA_UPDATE_2));
...
static int ana38401_ldi_enable(struct lcd_info *lcd)
{
	...
	_ana38401_write(lcd, SEQ_DISPLAY_ON, ARRAY_SIZE(SEQ_DISPLAY_ON));
...
static int ana38401_ldi_disable(struct lcd_info *lcd)
{
	...
	/* 2. Display Off (28h) */
	_ana38401_write(lcd, SEQ_DISPLAY_OFF, ARRAY_SIZE(SEQ_DISPLAY_OFF));
...
static int ana38401_probe(struct mipi_dsim_device *dsim)
{
	...
	ana38401_write(lcd, SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
	ana38401_read_id(lcd, lcd->id);
	ana38401_read_code(lcd, lcd->code);
	ana38401_read_coordinate(lcd);
	ana38401_read_mtp(lcd, mtp_data);
	ana38401_read_elvss(lcd, elvss_data);
	ana38401_read_tset(lcd);
	ana38401_write(lcd, SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
...
#if defined(CONFIG_DECON_MDNIE_LITE)
	mdnie_register(&lcd->ld->dev, lcd, (mdnie_w)ana38401_write_set, (mdnie_r)ana38401_read);
#endif
