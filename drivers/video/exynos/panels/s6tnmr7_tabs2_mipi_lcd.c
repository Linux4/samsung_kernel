...
static const unsigned char SEQ_RESOLUTION[] = {
	0xB0,	0x79,	0xB2,	0xC0,
	0xB0,	0x7D,	0xB2,	0x03,
	0xB0,	0x7F,	0xB2,	0x01,
	0xB0,	0x80,	0xB2,	0x53,
...
static void s6tnmr7_change_resolution(struct lcd_info *lcd)
{
	unsigned int i;

	for (i = 0; i + 1 < ARRAY_SIZE(SEQ_RESOLUTION); i += 2) {
		s6tnmr7_write(lcd, &SEQ_RESOLUTION[i], 2);
		/* printf("%2x, %2x\n", SEQ_RESOLUTION[i], SEQ_RESOLUTION[i + 1]); */
	}
}
...
#if defined(CONFIG_DECON_MDNIE_LITE)
	mdnie_register(&lcd->ld->dev, lcd, (mdnie_w)s6tnmr7_write_set, (mdnie_r)s6tnmr7_read);
#endif
