...
struct lcd_info {
	...
	union {
		u32			value;
		unsigned char		id[S6E88A0_ID_LEN];
	} id_info;
...
static void dsim_panel_set_elvss(struct lcd_info *lcd)
{
	...
	if (level) {
		...
		if (dsim_write_hl_data(lcd, B6_W, ARRAY_SIZE(B6_W)) < 0)
...
static struct SmtDimInfo daisy_dimming_info[MAX_BR_INFO] = {				/* add hbm array */
	...
	{.br = 350, .cTbl = ctbl350nit, .aid = aid183, .elvAcl = elv_0B, .elv = elv_0B, .m_gray = m_gray_350, .way = W2},
	{.br = 500, .cTbl = ctbl350nit, .aid = aid183, .elvAcl = elv_0B, .elv = elv_0B, .m_gray = m_gray_350, .way = W2}
};
...
static int s6e88a0_read_init_info(struct lcd_info *lcd, unsigned char *mtp)
{
	...
	/* id */
	ret = dsim_read_hl_data(lcd, S6E88A0_ID_REG, S6E88A0_ID_LEN, lcd->id_info.id);
	...
	/* mtp */
	ret = dsim_read_hl_data(lcd, S6E88A0_MTP_ADDR, S6E88A0_MTP_DATE_SIZE, buf);
	...
	/* date */
	ret = dsim_read_hl_data(lcd, S6E88A0_DATE_REG, S6E88A0_DATE_LEN, bufForDate);
	...
	/* coordinate */
	ret = dsim_read_hl_data(lcd, S6E88A0_COORDINATE_REG, S6E88A0_COORDINATE_LEN, bufForCoordi);
	...
	/* Read HBM from B5h */
	ret = dsim_read_hl_data(lcd, HBM_GAMMA_READ_ADD_1, HBM_GAMMA_READ_SIZE_1, buf_HBM_1);
	...
	/* Read HBM from B6h */
	ret = dsim_read_hl_data(lcd, HBM_GAMMA_READ_ADD_2, HBM_GAMMA_READ_SIZE_2, buf_HBM_2);
...
static int s6e88a0_init(struct lcd_info *lcd)
{
	...
	if (lcd->id_info.id[2] == 0x00) {
		ret = dsim_write_hl_data(lcd, SEQ_ACL_MTP, ARRAY_SIZE(SEQ_ACL_MTP));
		if (ret < 0) {
	...
	ret = dsim_write_hl_data(lcd, SEQ_ELVSS_SET_S6E88A0, ARRAY_SIZE(SEQ_ELVSS_SET_S6E88A0));
	if (ret < 0) {
	...
	ret = dsim_write_hl_data(lcd, SEQ_ACL_SET_S6E88A0, ARRAY_SIZE(SEQ_ACL_SET_S6E88A0));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_ACL_SET\n", __func__);
		goto init_exit;
	}
	...
	ret = dsim_write_hl_data(lcd, SEQ_MDNIE_1, ARRAY_SIZE(SEQ_MDNIE_1));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_MDNIE_1\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_MDNIE_2, ARRAY_SIZE(SEQ_MDNIE_2));
	if (ret < 0) {
