...
static int dsim_panel_set_brightness(struct dsim_device *dsim, int force)
{
	...
	dsim_info("%s: platform BL : %d panel BL reg : [%3d] %02x%02x\n",
		__func__, panel->bd->props.brightness, bl, bl_reg[1], bl_reg[2]);
...
static int dsi_write_table(struct dsim_device *dsim, const struct mipi_cmd *table, int size)
{
	...
	for (i = 0; i < table_size; i++) {
		ret = dsim_write_hl_data(dsim, table_ptr[i].cmd, ARRAY_SIZE(table_ptr[i].cmd));
...
static int ili9881c_read_init_info(struct dsim_device *dsim)
{
	int i = 0;
	struct panel_private *panel = &dsim->priv;
...
static int ili9881c_displayon(struct dsim_device *dsim)
{
	int ret = 0;
...
static int ili9881c_exit(struct dsim_device *dsim)
{
	int ret = 0;
	dsim_info("MDD : %s was called\n", __func__);
...
static int ili9881c_init(struct dsim_device *dsim)
{
	int ret;

	dsim_info("MDD : %s was called\n", __func__);

	ret = dsim_write_hl_data(dsim, SEQ_PAGE_3, ARRAY_SIZE(SEQ_PAGE_3));
	if (ret < 0) {
		dsim_err("%s : fail to write CMD : SEQ_PAGE_3\n", __func__);
		goto init_exit;
	}

	ret = dsi_write_table(dsim, SEQ_CMD_TABLE_3, ARRAY_SIZE(SEQ_CMD_TABLE_3));
	if (ret < 0) {
		dsim_err("%s : fail to write CMD : SEQ_CMD_TABLE_3\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(dsim, SEQ_PAGE_4, ARRAY_SIZE(SEQ_PAGE_4));
	if (ret < 0) {
		dsim_err("%s : fail to write CMD : SEQ_PAGE_4\n", __func__);
		goto init_exit;
	}

	ret = dsi_write_table(dsim, SEQ_CMD_TABLE_4, ARRAY_SIZE(SEQ_CMD_TABLE_4));
	if (ret < 0) {
		dsim_err("%s : fail to write CMD : SEQ_CMD_TABLE_4\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(dsim, SEQ_PAGE_1, ARRAY_SIZE(SEQ_PAGE_1));
	if (ret < 0) {
		dsim_err("%s : fail to write CMD : SEQ_PAGE_3\n", __func__);
		goto init_exit;
	}

	ret = dsi_write_table(dsim, SEQ_CMD_TABLE_1, ARRAY_SIZE(SEQ_CMD_TABLE_1));
	if (ret < 0) {
		dsim_err("%s : fail to write CMD : SEQ_CMD_TABLE_1\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(dsim, SEQ_PAGE_2, ARRAY_SIZE(SEQ_PAGE_2));
	if (ret < 0) {
		dsim_err("%s : fail to write CMD : SEQ_PAGE_2\n", __func__);
		goto init_exit;
	}

	ret = dsi_write_table(dsim, SEQ_CMD_TABLE_2, ARRAY_SIZE(SEQ_CMD_TABLE_2));
	if (ret < 0) {
		dsim_err("%s : fail to write CMD : SEQ_CMD_TABLE_2\n", __func__);
		goto init_exit;
	}
	...
	ret = dsim_write_hl_data(dsim, SEQ_IE_LOW, ARRAY_SIZE(SEQ_IE_LOW));
	if (ret < 0) {
		dsim_err("%s : fail to write CMD : SEQ_IE_LOW\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(dsim, SEQ_BR_TR_TIME, ARRAY_SIZE(SEQ_BR_TR_TIME));
	if (ret < 0) {
		dsim_err("%s : fail to write CMD : SEQ_BR_TR_TIME\n", __func__);
		goto init_exit;
	}
...
struct dsim_panel_ops ili9881c_panel_ops = {
	.early_probe = NULL,
	.probe		= ili9881c_probe,
	.displayon	= ili9881c_displayon,
	.exit		= ili9881c_exit,
	.init		= ili9881c_init,
};
