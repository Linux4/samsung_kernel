...
static int dsi_write_table(struct lcd_info *lcd, const struct mipi_cmd *table, int size)
{
	...
	for (i = 0; i < table_size; i++) {
		ret = dsim_write_hl_data(lcd->dsim, table_ptr[i].cmd, ARRAY_SIZE(table_ptr[i].cmd));
...
static int ili9881c_init(struct lcd_info *lcd)
{
	...
	ret = dsim_write_hl_data(lcd->dsim, SEQ_PAGE_3, ARRAY_SIZE(SEQ_PAGE_3));
	if (ret < 0) {
	...
	ret = dsim_write_hl_data(lcd->dsim, SEQ_PAGE_4, ARRAY_SIZE(SEQ_PAGE_4));
	if (ret < 0) {
	...
	ret = dsim_write_hl_data(lcd->dsim, SEQ_PAGE_1, ARRAY_SIZE(SEQ_PAGE_1));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s : failed to write CMD : SEQ_PAGE_3\n", __func__);
		goto init_exit;
	}
	...
	ret = dsim_write_hl_data(lcd->dsim, SEQ_PAGE_2, ARRAY_SIZE(SEQ_PAGE_2));
	if (ret < 0) {
	...
	ret = dsim_write_hl_data(lcd->dsim, SEQ_PAGE_0, ARRAY_SIZE(SEQ_PAGE_0));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s : failed to write CMD : SEQ_PAGE_0\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd->dsim, SEQ_IE_LOW, ARRAY_SIZE(SEQ_IE_LOW));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s : failed to write CMD : SEQ_IE_LOW\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd->dsim, SEQ_BR_TR_TIME, ARRAY_SIZE(SEQ_BR_TR_TIME));
	if (ret < 0) {
