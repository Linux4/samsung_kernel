...
static int dsim_panel_set_hbm(struct dsim_device *dsim, int force)
{
	...
	if(panel->br_tbl[brightness] == MAX_BRIGHTNESS && dsim->priv.weakness_hbm_comp == 2 && level == 0) {
		opr = ACL_OPR_16_FRAME;
	}

	if (force || dsim->priv.current_acl_opr != opr) {
		dsim->priv.current_acl_opr = opr;
		if (dsim_write_hl_data(dsim, panel->acl_opr_tbl[opr], ARRAY_SIZE(SEQ_ACL_ON_OPR_AVR)) < 0) {
			dsim_err("fail to write SEQ_ACL_ON/OFF_OPR_AVR command.\n");
			ret = -EPERM;
		}
		dsim_info("acl_opr: %d, weakness_hbm_comp: %d\n", dsim->priv.current_acl_opr, dsim->priv.weakness_hbm_comp);
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	sprintf(buf, "%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X\n",
		priv->date[0], priv->date[1], priv->date[2],
		priv->date[3], priv->date[4], 0x00, 0x00,
		(priv->coordinate[0]&0xFF00)>>8, priv->coordinate[0]&0x00FF,
...
// 73~87th : HBM gamma
	...
	dsim->priv.date[0] = buf[40]; /*year*/ /*month*/
	dsim->priv.date[1] = buf[41]; /*day*/
	dsim->priv.date[2] = buf[42]; /*hour*/
	dsim->priv.date[3] = buf[43]; /*minute*/
	dsim->priv.date[4] = buf[44]; /*second*/
...
static int dsim_panel_probe(struct dsim_device *dsim)
{
	...
	panel->siop_enable = 0;
	panel->current_hbm = 0;
	panel->current_acl_opr = ACL_OPR_16_FRAME;
