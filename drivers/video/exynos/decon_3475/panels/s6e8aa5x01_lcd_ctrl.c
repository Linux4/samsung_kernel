...
static struct SmtDimInfo dimming_info[MAX_BR_INFO + 1] = {				/* add hbm array */
	...
	{.br = 360, .cTbl = ctbl360nit, .aid = aid360, .elvAcl = elv_360, .elv = elv_360, .m_gray = m_gray_360},
	{.br = 360, .cTbl = ctbl360nit, .aid = aid360, .elvAcl = elv_HBM, .elv = elv_HBM, .m_gray = m_gray_360},
};
...
static int dsim_panel_set_hbm(struct lcd_info *lcd, int force)
{
	...
	int brightness, opr = ACL_OPR_32_FRAME;

	brightness = lcd->bd->props.brightness;

	if (lcd->br_tbl[brightness] == MAX_BRIGHTNESS && lcd->weakness_hbm_comp == 2 && level == 0) {
		opr = ACL_OPR_16_FRAME;
	}

	if (force || lcd->current_acl_opr != opr) {
		lcd->current_acl_opr = opr;
		if (dsim_write_hl_data(lcd->dsim, lcd->acl_opr_tbl[opr], ARRAY_SIZE(SEQ_ACL_ON_OPR_AVR)) < 0) {
			dev_err(&lcd->ld->dev, "failed to write SEQ_ACL_ON/OFF_OPR_AVR command.\n");
			ret = -EPERM;
		}
		dev_info(&lcd->ld->dev, "acl_opr: %d, weakness_hbm_comp: %d\n", lcd->current_acl_opr, lcd->weakness_hbm_comp);
	}
	...
	if (force || lcd->current_hbm != lcd->hbm_tbl[level][1]) {
		...
		if (level) {
			if (dsim_write_hl_data(lcd->dsim, lcd->hbm_gamma, HBM_GAMMA_CMD_CNT) < 0) {
...
static int dsim_panel_set_tset(struct lcd_info *lcd, int force)
{
	...
	if (force || (lcd->tset[0] != tset)) {
		...
		if (dsim_write_hl_data(lcd->dsim, SEQ_TSET_GP, ARRAY_SIZE(SEQ_TSET_GP)) < 0) {
...
static int low_level_set_brightness(struct lcd_info *lcd, int force)
{
	...
	if (dsim_write_hl_data(lcd->dsim, SEQ_GAMMA_UPDATE, ARRAY_SIZE(SEQ_GAMMA_UPDATE)) < 0)
		dev_err(&lcd->ld->dev, "%s : failed to write SEQ_GAMMA_UPDATE on command.\n", __func__);

	if (dsim_write_hl_data(lcd->dsim, SEQ_GAMMA_UPDATE_L, ARRAY_SIZE(SEQ_GAMMA_UPDATE_L)) < 0)
...
static int s6e8aa5x01_read_init_info(struct lcd_info *lcd, unsigned char *mtp)
{
	...
	/* id */
	msleep(1);
	ret = dsim_read_hl_data(lcd->dsim, S6E8AA5X01_ID_REG, S6E8AA5X01_ID_LEN, lcd->id);
	...
	/* chip id */
	msleep(1);
	ret = dsim_read_hl_data(lcd->dsim, S6E8AA5X01_CHIP_ID_REG, S6E8AA5X01_CHIP_ID_LEN, lcd->chip_id);
...
// 73~87th : HBM gamma
...
#if 0
	/* mtp */
	ret = dsim_read_hl_data(lcd->dsim, S6E8AA5X01_MTP_ADDR, S6E8AA5X01_MTP_DATE_SIZE, buf);
	...
	/* mtp */
	msleep(1);
	ret = dsim_read_hl_data(lcd->dsim, S6E8AA5X01_MTP_ADDR, S6E8AA5X01_MTP_SIZE_GAMMA, buf);
	if (ret != S6E8AA5X01_MTP_SIZE_GAMMA) {
		dev_err(&lcd->ld->dev, "failed to read mtp, check panel connection\n");
		goto read_fail;
	}

	ret = dsim_write_hl_data(lcd->dsim, SEQ_MTP_READ_DATE_GP, ARRAY_SIZE(SEQ_MTP_READ_DATE_GP));
	if (ret < 0) {
	...
	/* mtp */
	msleep(1);
	ret = dsim_read_hl_data(lcd->dsim, S6E8AA5X01_MTP_ADDR, S6E8AA5X01_MTP_SIZE_DATE, buf + S6E8AA5X01_MTP_OFFSET_DATE);
	if (ret != S6E8AA5X01_MTP_SIZE_DATE) {
		dev_err(&lcd->ld->dev, "failed to read mtp, check panel connection\n");
		goto read_fail;
	}

	ret = dsim_write_hl_data(lcd->dsim, SEQ_MTP_READ_HBM_GP, ARRAY_SIZE(SEQ_MTP_READ_HBM_GP));
	if (ret < 0) {
	...
	/* mtp */
	msleep(1);
	ret = dsim_read_hl_data(lcd->dsim, S6E8AA5X01_MTP_ADDR, S6E8AA5X01_MTP_SIZE_HBM, buf + S6E8AA5X01_MTP_OFFSET_HBM);
	...
	/* elvss */
	msleep(1);
	ret = dsim_read_hl_data(lcd->dsim, ELVSS_REG, ELVSS_MTP_LEN, buf);
	...
	/* coordinate */
	msleep(1);
	ret = dsim_read_hl_data(lcd->dsim, S6E8AA5X01_COORDINATE_REG, S6E8AA5X01_COORDINATE_LEN, bufForCoordi);
	...
	/* chip id */
	msleep(1);
	ret = dsim_read_hl_data(lcd->dsim, S6E8AA5X01_CODE_REG, S6E8AA5X01_CODE_LEN, lcd->code);
...
static int s6e8aa5x01_init(struct lcd_info *lcd)
{
	...
	ret = dsim_write_hl_data(lcd->dsim, SEQ_AID_360NIT, ARRAY_SIZE(SEQ_AID_360NIT));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s : failed to write CMD : SEQ_AID_360NIT\n", __func__);
		goto init_exit;
	}
	ret = dsim_write_hl_data(lcd->dsim, SEQ_ELVSS_360NIT, ARRAY_SIZE(SEQ_ELVSS_360NIT));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s : failed to write CMD : SEQ_ELVSS_360NIT\n", __func__);
		goto init_exit;
	}
	ret = dsim_write_hl_data(lcd->dsim, SEQ_ACL_OFF_OPR, ARRAY_SIZE(SEQ_ACL_OFF_OPR));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s : failed to write CMD : SEQ_ACL_OFF_OPR\n", __func__);
		goto init_exit;
	}
	ret = dsim_write_hl_data(lcd->dsim, SEQ_ACL_OFF, ARRAY_SIZE(SEQ_ACL_OFF));
	if (ret < 0) {
	...
	ret = dsim_write_hl_data(lcd->dsim, SEQ_TSET_GP, ARRAY_SIZE(SEQ_TSET_GP));
	if (ret < 0) {
	...
	/* 3. Common Setting */
	ret = dsim_write_hl_data(lcd->dsim, SEQ_PENTILE_SETTING, ARRAY_SIZE(SEQ_PENTILE_SETTING));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s : failed to write CMD : SEQ_PENTILE_SETTING\n", __func__);
		goto init_exit;
	}
	ret = dsim_write_hl_data(lcd->dsim, SEQ_DE_DIM_GP, ARRAY_SIZE(SEQ_DE_DIM_GP));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s : failed to write CMD : SEQ_DE_DIM_GP\n", __func__);
		goto init_exit;
	}
	ret = dsim_write_hl_data(lcd->dsim, SEQ_DE_DIM_SETTING, ARRAY_SIZE(SEQ_DE_DIM_SETTING));
	if (ret < 0) {
