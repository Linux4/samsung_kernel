...
struct lcd_info {
	...
	union {
		u32			value;
		unsigned char		id[SC7798D_ID_LEN];
	} id_info;
...
static int dsim_panel_set_cabc(struct lcd_info *lcd, int force)
{
	...
	if (lcd->lux < 0 || LEVEL_IS_HBM(lcd->brightness))
		lcd->acl_enable = 0;
	else
		lcd->acl_enable = 1;
	...
	if (lcd->acl_enable) { /* cabc on */
		if (dsim_write_hl_data(lcd, SEQ_INIT1_1, ARRAY_SIZE(SEQ_INIT1_1)) < 0)
			dev_err(&lcd->ld->dev, "%s: failed to write SEQ_INIT1_1\n", __func__);
		if (dsim_write_hl_data(lcd, SEQ_CABC_ON, ARRAY_SIZE(SEQ_CABC_ON)) < 0)
		...
		if (dsim_write_hl_data(lcd, SEQ_CABC_OFF, ARRAY_SIZE(SEQ_CABC_OFF)) < 0)
			dev_err(&lcd->ld->dev, "%s: failed to write SEQ_CD_ON\n", __func__);
	}

	lcd->current_acl = lcd->acl_enable;
	dev_info(&lcd->ld->dev, "cabc: %d, brightness: %d, lux: %d\n", lcd->current_acl, lcd->brightness, lcd->lux);
...
static int dsim_panel_set_brightness(struct lcd_info *lcd, int force)
{
	...
	lcd->brightness = bl = lcd->bd->props.brightness;
...
static int sc7798d_read_init_info(struct lcd_info *lcd)
{
	...
	/* id */
	ret = dsim_read_hl_data(lcd, SC7798D_ID_REG, SC7798D_ID_LEN, lcd->id_info.id);
...
static int sc7798d_displayon_late(struct lcd_info *lcd)
{
	...
	ret = dsim_write_hl_data(lcd, SEQ_INIT4_4, ARRAY_SIZE(SEQ_INIT4_4));
...
static int sc7798d_init(struct lcd_info *lcd)
{
	...
	ret = dsim_write_hl_data(lcd, SEQ_INIT1_1, ARRAY_SIZE(SEQ_INIT1_1));
	ret = dsim_write_hl_data(lcd, SEQ_INIT1_2, ARRAY_SIZE(SEQ_INIT1_2));
	ret = dsim_write_hl_data(lcd, SEQ_INIT1_3, ARRAY_SIZE(SEQ_INIT1_3));
	ret = dsim_write_hl_data(lcd, SEQ_INIT1_4, ARRAY_SIZE(SEQ_INIT1_4));
	ret = dsim_write_hl_data(lcd, SEQ_INIT1_5, ARRAY_SIZE(SEQ_INIT1_5));

	msleep(12);

	ret = dsim_write_hl_data(lcd, SEQ_INIT2_1, ARRAY_SIZE(SEQ_INIT2_1));
	ret = dsim_write_hl_data(lcd, SEQ_INIT2_2, ARRAY_SIZE(SEQ_INIT2_2));
	ret = dsim_write_hl_data(lcd, SEQ_INIT2_3, ARRAY_SIZE(SEQ_INIT2_3));
	ret = dsim_write_hl_data(lcd, SEQ_INIT2_4, ARRAY_SIZE(SEQ_INIT2_4));
	ret = dsim_write_hl_data(lcd, SEQ_INIT2_5, ARRAY_SIZE(SEQ_INIT2_5));
	ret = dsim_write_hl_data(lcd, SEQ_INIT2_6, ARRAY_SIZE(SEQ_INIT2_6));
	ret = dsim_write_hl_data(lcd, SEQ_INIT2_7, ARRAY_SIZE(SEQ_INIT2_7));
	ret = dsim_write_hl_data(lcd, SEQ_INIT2_8, ARRAY_SIZE(SEQ_INIT2_8));
	ret = dsim_write_hl_data(lcd, SEQ_INIT2_9, ARRAY_SIZE(SEQ_INIT2_9));

	ret = dsim_write_hl_data(lcd, SEQ_INIT3_1, ARRAY_SIZE(SEQ_INIT3_1));

	ret = dsim_write_hl_data(lcd, SEQ_INIT4_1, ARRAY_SIZE(SEQ_INIT4_1));
	ret = dsim_write_hl_data(lcd, SEQ_INIT4_2, ARRAY_SIZE(SEQ_INIT4_2));
	ret = dsim_write_hl_data(lcd, SEQ_INIT4_3, ARRAY_SIZE(SEQ_INIT4_3));
