...
struct lcd_info {
	...
	union {
		u32			value;
		unsigned char		id[EA8061S_ID_LEN];
	} id_info;
...
static void dsim_panel_aid_ctrl_ea8061s(struct lcd_info *lcd)
{
	...
	if (dsim_write_hl_data(lcd, aid, AID_CMD_CNT_EA8061S) < 0)
		dev_err(&lcd->ld->dev, "%s: failed to write gamma\n", __func__);
}
...
static void dsim_panel_set_elvss_ea8061s(struct lcd_info *lcd)
{
	...
	if (dsim_write_hl_data(lcd, SEQ_ELVSS_GLOBAL_EA8061S, ARRAY_SIZE(SEQ_ELVSS_GLOBAL_EA8061S)) < 0)
...
static int dsim_panel_set_hbm_ea8061s(struct lcd_info *lcd, int force)
{
	...
	if (force || lcd->current_hbm != lcd->hbm_tbl[level][1]) {
		...
		if (dsim_write_hl_data(lcd, SEQ_GAMMA_UPDATE_S6E88A0, ARRAY_SIZE(SEQ_GAMMA_UPDATE_S6E88A0)) < 0) {
...
static struct SmtDimInfo daisy_dimming_info_ea8061s[MAX_BR_INFO] = {				/* add hbm array */
	...
	{.br = 350, .cTbl = ctbl350nit_ea, .aid = aid183_ea, .elvAcl = elv_ea_8B, .elv = elv_ea_8B, .m_gray = m_gray_350_ea, .way = W2},
	{.br = 500, .cTbl = ctbl350nit_ea, .aid = aid183_ea, .elvAcl = elv_ea_8B, .elv = elv_ea_8B, .m_gray = m_gray_350_ea, .way = W2}
};
...
static int ea8061s_read_init_info(struct lcd_info *lcd, unsigned char *mtp)
{
	...
	/* id */
	ret = dsim_read_hl_data(lcd, EA8061S_ID_REG, EA8061S_ID_LEN, lcd->id_info.id);
	...
	/* mtp */
	ret = dsim_read_hl_data(lcd, EA8061S_MTP_ADDR, EA8061S_MTP_DATE_SIZE, buf);
	...
	/* date & coordinate */
	ret = dsim_read_hl_data(lcd, EA8061S_DATE_REG, EA8061S_DATE_LEN, bufForDate);
	...
	/* Read HBM from B6h */
	ret = dsim_read_hl_data(lcd, HBM_GAMMA_READ_ADD_EA8061S, HBM_GAMMA_READ_SIZE_EA8061S, buf_HBM);
...
static int ea8061s_init(struct lcd_info *lcd)
{
	...
	ret = dsim_write_hl_data(lcd, SEQ_SOURCE_SLEW_EA8061S, ARRAY_SIZE(SEQ_SOURCE_SLEW_EA8061S));
	if (ret < 0) {
	...
	ret = dsim_write_hl_data(lcd, SEQ_AID_SET_EA8061S, ARRAY_SIZE(SEQ_AID_SET_EA8061S));
	if (ret < 0) {
	...
	ret = dsim_write_hl_data(lcd, SEQ_GAMMA_UPDATE_S6E88A0, ARRAY_SIZE(SEQ_GAMMA_UPDATE_S6E88A0));
	if (ret < 0) {
	...
	ret = dsim_write_hl_data(lcd, SEQ_S_WIRE_EA8061S, ARRAY_SIZE(SEQ_S_WIRE_EA8061S));
	if (ret < 0) {
	...
	ret = dsim_write_hl_data(lcd, SEQ_ELVSS_SET_EA8061S, ARRAY_SIZE(SEQ_ELVSS_SET_EA8061S));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_ELVSS_SET_EA8061S\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_ACL_SET_EA8061S, ARRAY_SIZE(SEQ_ACL_SET_EA8061S));
	if (ret < 0) {
	...
	ret = dsim_write_hl_data(lcd, SEQ_TSET_EA8061S, ARRAY_SIZE(SEQ_TSET_EA8061S));
	if (ret < 0) {
	...
	ret = dsim_write_hl_data(lcd, SEQ_MDNIE_1_EA8061S, ARRAY_SIZE(SEQ_MDNIE_1_EA8061S));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_MDNIE_1_EA8061S\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_MDNIE_2_EA8061S, ARRAY_SIZE(SEQ_MDNIE_2_EA8061S));
	if (ret < 0) {
