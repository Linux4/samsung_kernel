...
struct lcd_info {
	...
	union {
		u32			value;
		unsigned char		id[S6D7AA0X62_ID_LEN];
	} id_info;
...
static int s6d7aa0x62_check_id(struct lcd_info *lcd)
{
	int ret = 0;
	u8 *dump = NULL;

	dump = kcalloc(S6D7AA0X62_ID_LEN, sizeof(u8), GFP_KERNEL);

	ret = dsim_read_hl_data(lcd, 0xDA, 1, dump);
	if (ret < 1)
		dev_err(&lcd->ld->dev, "%s: 0xDA read fail\n", __func__);
	ret = dsim_read_hl_data(lcd, 0xDB, 1, dump+1);
	if (ret < 1)
		dev_err(&lcd->ld->dev, "%s: 0xDB read fail\n", __func__);
	ret = dsim_read_hl_data(lcd, 0xDC, 1, dump+2);
	if (ret < 1)
		dev_err(&lcd->ld->dev, "%s: 0xDC read fail\n", __func__);

	lcdtype = ((dump[0] << 16)|(dump[1] << 8)|(dump[2] << 0));

	dev_info(&lcd->ld->dev, "0x%06x, ", lcdtype);
...
static int s6d7aa0x62_displayon(struct lcd_info *lcd)
{
	...
	ret = dsim_write_hl_data(lcd, SEQ_BR_DEF, ARRAY_SIZE(SEQ_BR_DEF));
	if (ret < 0) {
...
#if defined(CONFIG_SEC_FACTORY)
	if(lcdtype != S6D7AA0_ID) {
		ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_REVERSE, ARRAY_SIZE(SEQ_S6D7AA0X62_REVERSE));
		if (ret < 0) {
			dev_err(&lcd->ld->dev, "%s : failed to write CMD : SEQ_S6D7AA0X62_REVERSE\n", __func__);
			goto init_exit;
		}
	}
#endif

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_1, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_1));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_1\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_2, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_2));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_2\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_3, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_3));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_3\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_4, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_4));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_4\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_5, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_5));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_5\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_6, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_6));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_6\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_7, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_7));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_7\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_8, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_8));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_8\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_9, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_9));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_9\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_10, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_10));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_10\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_11, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_11));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_11\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_12, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_12));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_12\n", __func__);
		goto init_exit;
	}
	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_13, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_13));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_13\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_14, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_14));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_14\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_15, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_15));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_15\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_16, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_16));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_16\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_17, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_17));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_17\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_18, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_18));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_18\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_19, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_19));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_19\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_20, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_20));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_20\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_21, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_21));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_21\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_22, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_22));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_22\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_S6D7AA0X62_INIT_23, ARRAY_SIZE(SEQ_S6D7AA0X62_INIT_23));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_S6D7AA0X62_INIT_23\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_GAMMA_FA, ARRAY_SIZE(SEQ_GAMMA_FA));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_GAMMA_FA\n", __func__);
		goto init_exit;
	}

	ret = dsim_write_hl_data(lcd, SEQ_GAMMA_FB, ARRAY_SIZE(SEQ_GAMMA_FB));
	if (ret < 0) {
