...
void dsim_clocks_info(struct dsim_device *dsim)
{
...
int dsim_get_clocks(struct dsim_device *dsim)
{
...
static int dsim_parse_lcd_info(struct dsim_device *dsim)
{
	...
	if (!node && !first_node) {
		dsim_err("Panel information is not provided.\n");
		return -EINVAL;
	}

	/* no matching panel. select first panel */
	if (!node && first_node)
		node = first_node;

	dsim_info("%s is found at index %d.\n", of_node_full_name(node), 0);
	...
	octa_pin_gpio = of_get_gpio(node, 0);
	if (octa_pin_gpio > 0) {
		gpio_request_one(octa_pin_gpio, GPIOF_IN, "octa_pin");
		dsim->octa_id = gpio_get_value(octa_pin_gpio);
		dsim_info("dsim board OCTA pin (%s) \n", dsim->octa_id ? "high" : "low" );
		gpio_free(octa_pin_gpio);
	} else {
...
static int dsim_probe(struct platform_device *pdev)
{
	...
	if (dsim_reg_is_hs_clk_ready(dsim->id)) {
		/* DPHY on: required to maintain ref_count of regulators */
		dsim_set_panel_power(dsim, 1);
