...
int decon_int_get_clocks(struct decon_device *decon)
{
	...
	decon->res.aclk = clk_get(decon->dev, "aclk_disp");
	if (IS_ERR_OR_NULL(decon->res.aclk)) {
		decon_err("failed to get aclk_disp\n");
		return -ENODEV;
	}

	decon->res.aclk_dispaud_d = clk_get(decon->dev, "aclk_dispaud_d");
	if (IS_ERR_OR_NULL(decon->res.aclk_dispaud_d)) {
		decon_err("failed to get aclk_dispaud_d\n");
		return -ENODEV;
	}

	decon->res.eclk = clk_get(decon->dev, "decon0_eclk");
	if (IS_ERR_OR_NULL(decon->res.eclk)) {
		decon_err("failed to get eclk\n");
		return -ENODEV;
	}

	decon->res.vclk = clk_get(decon->dev, "decon0_vclk");
	if (IS_ERR_OR_NULL(decon->res.vclk)) {
		decon_err("failed to get vclk\n");
		return -ENODEV;
	}

	decon->res.d_decon0_vclk = clk_get(decon->dev, "d_decon0_vclk");
	if (IS_ERR_OR_NULL(decon->res.d_decon0_vclk)) {
		decon_err("failed to get d_decon0_vclk\n");
		return -ENODEV;
	}

	decon->res.d_decon0_eclk = clk_get(decon->dev, "d_decon0_eclk");
	if (IS_ERR_OR_NULL(decon->res.d_decon0_eclk)) {
		decon_err("failed to get d_decon0_eclk\n");
		return -ENODEV;
	}

	decon->res.m_sclk_decon_int_vclk_user = clk_get(decon->dev, "m_sclk_decon_int_vclk_user");
	if (IS_ERR_OR_NULL(decon->res.m_sclk_decon_int_vclk_user)) {
		decon_err("failed to get m_sclk_decon_int_vclk_user\n");
		return -ENODEV;
	}

	decon->res.m_sclk_decon_int_eclk_user = clk_get(decon->dev, "m_sclk_decon_int_eclk_user");
	if (IS_ERR_OR_NULL(decon->res.m_sclk_decon_int_eclk_user)) {
		decon_err("failed to get m_sclk_decon_int_eclk_user\n");
		return -ENODEV;
	}

	decon->res.m_sclk_disp_decon0_vclk = clk_get(decon->dev, "m_sclk_disp_decon0_vclk");
	if (IS_ERR_OR_NULL(decon->res.m_sclk_disp_decon0_vclk)) {
		decon_err("failed to get m_sclk_disp_decon0_vclk\n");
		return -ENODEV;
	}

	decon->res.d_sclk_disp_decon0_vclk = clk_get(decon->dev, "d_sclk_disp_decon0_vclk");
	if (IS_ERR_OR_NULL(decon->res.d_sclk_disp_decon0_vclk)) {
		decon_err("failed to get d_sclk_disp_decon0_vclk\n");
		return -ENODEV;
	}

	decon->res.m_sclk_disp_decon0_eclk = clk_get(decon->dev, "m_sclk_disp_decon0_eclk");
	if (IS_ERR_OR_NULL(decon->res.m_sclk_disp_decon0_eclk)) {
		decon_err("failed to get m_sclk_disp_decon0_eclk\n");
		return -ENODEV;
	}

	decon->res.d_sclk_disp_decon0_eclk = clk_get(decon->dev, "d_sclk_disp_decon0_eclk");
	if (IS_ERR_OR_NULL(decon->res.d_sclk_disp_decon0_eclk)) {
		decon_err("failed to get d_sclk_disp_decon0_eclk\n");
		return -ENODEV;
	}
...
void decon_int_set_clocks(struct decon_device *decon)
{
	...
	if (IS_ENABLED(CONFIG_EXYNOS3475_DECON_LCD_S6E8AA0)) {
		clk_set_parent(decon->res.d_sclk_disp_decon0_vclk, decon->res.m_sclk_disp_decon0_vclk);
		clk_set_parent(decon->res.d_sclk_disp_decon0_eclk, decon->res.m_sclk_disp_decon0_eclk);

		decon_clk_set_rate(dev, decon->res.d_sclk_disp_decon0_vclk, 62 * MHZ);
		decon_clk_set_rate(dev, decon->res.d_sclk_disp_decon0_eclk, 76 * MHZ);
	} else if (IS_ENABLED(CONFIG_EXYNOS3475_DECON_LCD_S6D78A)) {
		clk_set_parent(decon->res.d_sclk_disp_decon0_eclk, decon->res.m_sclk_disp_decon0_eclk);
		...
		decon_clk_set_rate(dev, decon->res.d_decon0_vclk, 69 * MHZ);
		decon_clk_set_rate(dev, decon->res.d_sclk_disp_decon0_eclk, 92 * MHZ);
		decon_clk_set_rate(dev, decon->res.d_decon0_eclk, 92* MHZ);
	} else if (IS_ENABLED(CONFIG_EXYNOS3475_DECON_LCD_S6E88A0)) {
		clk_set_parent(decon->res.d_sclk_disp_decon0_eclk, decon->res.m_sclk_disp_decon0_eclk);

		decon_clk_set_rate(dev, decon->res.d_decon0_vclk, 38 * MHZ);
		decon_clk_set_rate(dev, decon->res.d_decon0_eclk, 46* MHZ);
	} else if (IS_ENABLED(CONFIG_EXYNOS3475_DECON_LCD_ILI9881C)) {
		clk_set_parent(decon->res.d_sclk_disp_decon0_eclk, decon->res.m_sclk_disp_decon0_eclk);
		...
		decon_clk_set_rate(dev, decon->res.d_decon0_vclk, 35 * MHZ);
		decon_clk_set_rate(dev, decon->res.d_sclk_disp_decon0_eclk, 138 * MHZ);
...
#if 0
	// It cause critical delay in decon_display_bootloader_fb
