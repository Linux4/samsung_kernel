...
#define DSIM_STOP_STATE_CNT_VIDEO_MODE		0x1
...
const u32 dphy_timing[][10] = {
	/*
	 * bps, clk_prepare, clk_zero, clk_post, clk_trail, hs_prepare,
	 * hs_zero, hs_trail, lpx, hs_exit
	 */
...
void dsim_reg_set_stop_state_cnt(u32 id, struct decon_lcd *lcd_info)
{
	u32 val;

	if (lcd_info->mode == DECON_VIDEO_MODE)
		val = DSIM_ESCMODE_STOP_STATE_CNT(DSIM_STOP_STATE_CNT_VIDEO_MODE);
	else
		val = DSIM_ESCMODE_STOP_STATE_CNT(DSIM_STOP_STATE_CNT);
...
void dsim_reg_set_config(u32 id, u32 mode, u32 data_lane_cnt)
{
	...
	if (mode == DECON_VIDEO_MODE) {
		...
		/* In Command mode, Clklane_Stop/Start must be disabled */
	...
	val |= DSIM_CONFIG_BURST_MODE | DSIM_CONFIG_EOT_R03_DISABLE |
			DSIM_CONFIG_HFP_DISABLE |
		DSIM_CONFIG_NUM_OF_DATA_LANE(data_lane_cnt - 1) |
		DSIM_CONFIG_PIXEL_FORMAT(DSIM_PIXEL_FORMAT_RGB24);

	mask = 	DSIM_CONFIG_NONCONTINUOUS_CLOCK_LANE |
		DSIM_CONFIG_CLKLANE_STOP_START |
		DSIM_CONFIG_EOT_R03_DISABLE |
		DSIM_CONFIG_BURST_MODE | DSIM_CONFIG_VIDEO_MODE |
		DSIM_CONFIG_HFP_DISABLE | DSIM_CONFIG_PIXEL_FORMAT_MASK |
		DSIM_CONFIG_NUM_OF_DATA_LANE_MASK;

	dsim_write_mask(id, DSIM_CONFIG, val, mask);
}
void dsim_reg_enable_noncontinuousclockmode(u32 id, u32 en)
{
	u32 val = en ? ~0 : 0;
	dsim_write_mask(id, DSIM_CONFIG, val,
				DSIM_CONFIG_NONCONTINUOUS_CLOCK_LANE);
}
...
int dsim_reg_wait_hs_clk_ready(u32 id)
{
	...
	val &= DSIM_CONFIG_NONCONTINUOUS_CLOCK_LANE_MASK;
...
void dsim_reg_wr_tx_header(u32 id, u32 data_id,
		unsigned long data0, u32 data1)
{
			...
			struct dphy_timing_value *t)
{
	...
	if (!i) {
		dsim_err(
		"%u Mhz hs clock can't find proper dphy timing values\n",
			hs_clk);
		return -EINVAL;
	}
...
void dsim_reg_set_mpll_ctrl(u32 id)
{
	u32 val = DSIM_PLLCTRL1_M_PLL0_DEFAULT;

	dsim_write_mask(id, DSIM_PLLCTRL1, val,
				DSIM_PLLCTRL1_M_PLL0_DEFAULT_MASK);
}

void dsim_reg_set_mdphy_ctrl(u32 id)
{
	dsim_write(id, DSIM_PHYCTRL_M1, DSIM_PHYCTRL_M1_PLL_LOCK);
}
...
void dsim_reg_init(u32 id, struct decon_lcd *lcd_info, u32 data_lane_cnt)
{
	...
	/* set counter */
	dsim_reg_set_stop_state_cnt(id, lcd_info);
	...
	if (lcd_info->mode == DECON_VIDEO_MODE)
		dsim_reg_enable_noncontinuousclockmode(id, 0);
	else
		dsim_reg_enable_noncontinuousclockmode(id, 1);
}
...
int dsim_reg_set_clocks(u32 id, struct dsim_clks *clks, u32 lane, u32 en)
{
	...
	if (en) {
		...
		dsim_reg_set_mpll_ctrl(id);

		dsim_reg_set_mdphy_ctrl(id);
...
int dsim_reg_set_hs_clock(u32 id, struct decon_lcd *lcd, u32 en)
{
	...
	if (en) {
		is_non = dsim_reg_is_noncontinuous_clk_enabled(id);
