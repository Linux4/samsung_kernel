...
#define SAVE_LOG_INFO(...)	dct_save_log(false, __func__, __VA_ARGS__)
#define SAVE_LOG_ERR(...)	dct_save_log(true, __func__, __VA_ARGS__)
...
bool blog_full;
void dct_save_log(bool berror, const char *func_name, const char *fmt, ...)
{
	va_list args;
	int remain;
...
static int dct_setData(DCT_DATA_INFO *dinfo, char *line)
{
	...
	case DCT_TYPE_U8:
		if (temp_value != (u64)(u8)temp_value) {
			...
			SAVE_LOG_ERR("Enable Max Value: %llu\n", 0xffLL);
			ret = -ERANGE;
			goto out;
		}
		break;
