...
static irqreturn_t lcdc_isr(int irq, void *data)
{
	...
	if (reg_val & 1) { /* lcdc done isr */
		...
		pr_debug(KERN_INFO "[LCD] sprdfb: [%s]: Done INT !\n", __FUNCTION__);
	}
...
static int32_t lcdc_sync(struct sprdfb_device *dev)
{
	...
	if (dev->enable == 0) {
		printk("[LCD] sprdfb: lcdc_sync fb suspeneded already!!\n");
		return -1;
	}
	...
	if (!ret) { /* time out */
		...
		printk(KERN_ERR "[LCD] sprdfb: lcdc_sync time out!!!!!\n");
		return -1;
	}
	return 0;
}
...
static int32_t sprdfb_lcdc_early_init(struct sprdfb_device *dev)
{
	...
	if(lcdc_ctx.is_inited){
		printk(KERN_WARNING "[LCD] sprdfb: lcdc early init warning!(has been inited)");
		return 0;
	}

	lcdc_ctx.clk_lcdc = clk_get(NULL, "clk_lcd");
	if (IS_ERR(lcdc_ctx.clk_lcdc)) {
		printk(KERN_WARNING "[LCD] sprdfb: get clk_lcd fail!\n");
		return 0;
	} else {
		pr_debug(KERN_INFO "[LCD] sprdfb: get clk_lcd ok!\n");
	}
	...
	if(!dev->panel_ready){
		...
		if (ret) {
			printk(KERN_WARNING "[LCD] sprdfb: enable clk_lcdc fail!\n");
			return 0;
		} else {
			pr_debug(KERN_INFO "[LCD] sprdfb: get clk_lcdc ok!\n");
		}
	...
	if (ret) {
		printk(KERN_ERR "[LCD] sprdfb: lcdc failed to request irq!\n");
...
{
	printk(KERN_INFO "[LCD] sprdfb: [%s], dev->enable = %d\n",__FUNCTION__, dev->enable);

	if (0 != dev->enable){
		/* must wait ,lcdc_sync() */
		lcdc_ctx.vsync_waiter ++;
		lcdc_sync(dev);
		printk(KERN_INFO "[LCD] sprdfb: [%s] got sync\n",__FUNCTION__);
		...
		printk(KERN_ERR "[LCD] sprdfb: [%s]: Invalid device status %d\n", __FUNCTION__, dev->enable);
	}
	return 0;
}
