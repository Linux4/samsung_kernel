...
static irqreturn_t dsi_isr0(int irq, void *data)
{
	...
	printk(KERN_ERR "[LCD] sprdfb: [%s](0x%x)!\n", __FUNCTION__, reg_val);
	/*
...
static irqreturn_t dsi_isr1(int irq, void *data)
{
	...
	if(BIT(7) == (reg_val & BIT(7))){
		...
		/*need delay 1us*/
		printk("[LCD] sprdfb: reset dsi host!\n");
		printk("[LCD] sprdfb: mipi->lan_number:%d ,mipi->phy_feq:%d \n",mipi->lan_number,mipi->phy_feq);
		if(NULL == phy){
			printk("[LCD] sprdfb: the phy is null \n");
...
static int32_t dsi_edpi_setbuswidth(struct info_mipi * mipi)
{
	...
	case 24:
		...
		break;
	default:
		printk(KERN_ERR "[LCD] sprdfb: [%s] fail, invalid video_bus_width\n", __FUNCTION__);
		return 0;
	}
...
int32_t dsi_dpi_init(struct sprdfb_device *dev)
{
	...
	if(result != OK){
		printk(KERN_ERR "[LCD] sprdfb: [%s] mipi_dsih_dpi_video fail (%d)!\n", __FUNCTION__, result);
		return -1;
	}
...
static int32_t dsi_module_init(struct sprdfb_device *dev)
{
	...
	if(dsi_ctx.is_inited){
		printk(KERN_INFO "[LCD] sprdfb: dsi_module_init. is_inited==true!");
		return 0;
	}
	else{
		printk(KERN_INFO "[LCD] sprdfb: dsi_module_init. call only once!");
	}
...
//		clk_disable(dsi_ctx.clk_dsi);
		...
		printk(KERN_ERR "[LCD] sprdfb: dsi request irq int0 OK!\n");
	}
...
//		clk_disable(dsi_ctx.clk_dsi);
		...
		printk(KERN_ERR "[LCD] sprdfb: dsi request irq int1 OK!\n");
	}
...
void dsi_print_global_config(void)
{
	...
	printk("[LCD] sprdfb: dsi: 0x20E00040 = 0x%x, 0x20E00000 = 0x%x, 0x402E0024 = 0x%x\n",
...
//	dsi_core_write_function(SPRD_MIPI_DSIC_BASE,  R_DSI_HOST_ERROR_MSK1, 0x3ffff);
	...
	if(OK != result){
		printk(KERN_ERR "[LCD] sprdfb: [%s]: mipi_dsih_open fail (%d)!\n", __FUNCTION__, result);
		dsi_ctx.status = 1;
		return -1;
	}
	...
	if(OK != result){
		printk(KERN_ERR "[LCD] sprdfb: [%s]: mipi_dsih_dphy_configure fail (%d)!\n", __FUNCTION__, result);
		dsi_ctx.status = 1;
		return -1;
	}
	...
	while(5 != (dsi_core_read_function(SPRD_MIPI_DSIC_BASE, R_DSI_HOST_PHY_STATUS) & 5)){
		if(0x0 == ++i%500000){
			...
			printk("[LCD] sprdfb: [%s] warning: busy waiting!\n", __FUNCTION__);
			break;
		}
	}
	...
	if(OK != result){
		printk(KERN_ERR "[LCD] sprdfb: [%s]: mipi_dsih_enable_rx fail (%d)!\n", __FUNCTION__, result);
		dsi_ctx.status = 1;
		return -1;
	}
	...
	if(OK != result){
		printk(KERN_ERR "[LCD] sprdfb: [%s]: mipi_dsih_ecc_rx fail (%d)!\n", __FUNCTION__, result);
		dsi_ctx.status = 1;
		return -1;
	}
	...
	if(OK != result){
		printk(KERN_ERR "[LCD] sprdfb: [%s]: mipi_dsih_eotp_rx fail (%d)!\n", __FUNCTION__, result);
		dsi_ctx.status = 1;
		return -1;
	}
	...
	if(OK != result){
		printk(KERN_ERR "[LCD] sprdfb: [%s]: mipi_dsih_eotp_tx fail (%d)!\n", __FUNCTION__, result);
		dsi_ctx.status = 1;
		return -1;
	}
...
int32_t sprdfb_dsi_init(struct sprdfb_device *dev)
{
	...
	if(0 != of_address_to_resource(dev->of_dev->of_node, 1, &r)){
		printk(KERN_ERR "[LCD] sprdfb: sprdfb_dsi_init fail. (can't get register base address)\n");
		return -1;
	}
	g_dsi_base_addr = r.start;
	printk("[LCD] sprdfb: set g_dsi_base_addr = 0x%x\n", g_dsi_base_addr);
#endif
	...
	if(!dsi_ctx.is_inited){
		...
		if(dev->panel_ready){
			//panel ready
			printk(KERN_INFO "[LCD] sprdfb: [%s]: dsi has alread initialized\n", __FUNCTION__);
			...
			//panel not ready
			printk(KERN_INFO "[LCD] sprdfb: [%s]: dsi is not initialized\n", __FUNCTION__);
			dsi_enable();
			dsi_reset();
			dsi_module_init(dev);
		...
		//resume
		printk(KERN_INFO "[LCD] sprdfb: [%s]: resume\n", __FUNCTION__);
		dsi_enable();
		dsi_reset();
		result=sprdfb_dsih_init(dev);
	}
...
int32_t sprdfb_dsi_uninit(struct sprdfb_device *dev)
{
	dsih_error_t result;
	dsih_ctrl_t* dsi_instance = &(dsi_ctx.dsi_inst);
	printk(KERN_INFO "[LCD] sprdfb: [%s], dev_id = %d\n",__FUNCTION__, dev->dev_id);
#ifdef FB_DSIH_VERSION_1P21A
	...
	if(OK != result){
		printk(KERN_ERR "[LCD] sprdfb: [%s]: sprdfb_dsi_uninit fail (%d)!\n", __FUNCTION__, result);
		return -1;
	}
...
static int32_t sprdfb_dsi_gen_write(uint8_t *param, uint16_t param_length)
{
	...
	if(OK != result){
		printk(KERN_ERR "[LCD] sprdfb: [%s] error (%d)\n", __FUNCTION__, result);
		return -1;
	}
	return 0;
}
...
#endif
	if(0 != (reg_val & 0x2)){
		printk("[LCD] sprdfb: [%s] mipi read hang (0x%x)!\n", __FUNCTION__, reg_val);
		dsi_ctx.status = 2;
		result = 0;
	}

	if(0 != (reg_val_1 & 0x701)){
		printk("[LCD] sprdfb: [%s] mipi read status error!(0x%x, 0x%x)\n", __FUNCTION__, reg_val_1, reg_val_2);
		result = 0;
	}
...
static int32_t sprdfb_dsi_dcs_read(uint8_t command, uint8_t bytes_to_read, uint8_t *read_buffer)
{
	...
	if(0 == result){
		printk(KERN_ERR "[LCD] sprdfb: [%s] return error (%d)\n", __FUNCTION__, result);
		return -1;
	}
	return 0;
}
