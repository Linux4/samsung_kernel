	...
	if (sprd_panel_spec.rst_mode) {
		if (val) {
			panel_rstpin_map[0].val =
				sci_glb_read(CTL_PIN_BASE+REG_PIN_LCD_RSTN, 0xffffffff);
			i = 1;
		} else
			i = 0;
...
{
	pr_info("[LCD][gen_panel] hfp %d\n", sprd_panel_timing.hfp);
	pr_info("[LCD][gen_panel] hbp %d\n", sprd_panel_timing.hbp);
	pr_info("[LCD][gen_panel] hsync %d\n", sprd_panel_timing.hsync);
	pr_info("[LCD][gen_panel] vfp %d\n", sprd_panel_timing.vfp);
	pr_info("[LCD][gen_panel] vbp %d\n", sprd_panel_timing.vbp);
	pr_info("[LCD][gen_panel] vsync %d\n", sprd_panel_timing.vsync);

	pr_info("[LCD][gen_panel] work_mode %d\n", sprd_panel_mipi_info.work_mode);
	pr_info("[LCD][gen_panel] video_bus_width %d\n",
			sprd_panel_mipi_info.video_bus_width);
	pr_info("[LCD][gen_panel] lan_number %d\n", sprd_panel_mipi_info.lan_number);
	pr_info("[LCD][gen_panel] phy_feq %d\n", sprd_panel_mipi_info.phy_feq);
	pr_info("[LCD][gen_panel] h_sync_pol %d\n", sprd_panel_mipi_info.h_sync_pol);
	pr_info("[LCD][gen_panel] v_sync_pol %d\n", sprd_panel_mipi_info.v_sync_pol);
	pr_info("[LCD][gen_panel] de_pol %d\n", sprd_panel_mipi_info.de_pol);
	pr_info("[LCD][gen_panel] te_pol %d\n", sprd_panel_mipi_info.te_pol);
	pr_info("[LCD][gen_panel] color_mode_pol %d\n",
			sprd_panel_mipi_info.color_mode_pol);
	pr_info("[LCD][gen_panel] shut_down_pol %d\n",
			sprd_panel_mipi_info.shut_down_pol);

	pr_info("[LCD][gen_panel] width %d\n", sprd_panel_spec.width);
	pr_info("[LCD][gen_panel] height %d\n", sprd_panel_spec.height);
	pr_info("[LCD][gen_panel] fps %d\n", sprd_panel_spec.fps);
	pr_info("[LCD][gen_panel] type %d\n", sprd_panel_spec.type);
	pr_info("[LCD][gen_panel] direction %d\n", sprd_panel_spec.direction);
	pr_info("[LCD][gen_panel] suspend_mode %d\n",
			sprd_panel_spec.suspend_mode);

	pr_info("[LCD][gen_panel] dev_id %d\n", sprd_panel_cfg.dev_id);
	pr_info("[LCD][gen_panel] lcd_id %x\n", sprd_panel_cfg.lcd_id);
	pr_info("[LCD][gen_panel] name %s\n", sprd_panel_cfg.lcd_name);
	pr_info("[LCD][gen_panel] rst_mode %d\n", sprd_panel_spec.rst_mode);
		...
		const int count)
{
	...
	mipi_force_write(desc[0].data_type, desc[0].data, desc[0].length);
	ret = mipi_force_read(desc[1].data[0], desc[0].data[0], buf);
...
int sprd_panel_parse_dt(const struct device_node *np)
{
	...
	/*
	   ...
	   */
	sprd_panel_spec.rst_mode =
		of_property_read_bool(np, "gen-panel-rst-high-in-sleep");
