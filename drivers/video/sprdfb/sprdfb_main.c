...
static int sprdfb_mmap(struct fb_info *info, struct vm_area_struct *vma)
{
	...
	if (NULL == info) {
			pr_info(KERN_ERR "sprdfb: sprdfb_ioctl error. (Invalid Parameter)");
			return -1;
	}
...
static int setup_fb_mem(struct sprdfb_device *dev, struct platform_device *pdev)
{
	...
	if (use_reserve_mem) {
		ret = of_property_read_u32_array(pdev->dev.of_node, "sprd,fb_mem",
			reserve_mem, 2);
		if (0 != ret)
			pr_info(KERN_ERR "sprdfb: Failed to got framebuffer memory from dt file\n");
	}
#else
	...
	if (!use_reserve_mem) {
		...
		if (NULL == addr) {
			pr_info(KERN_ERR "sprdfb: Failed to allocate framebuffer memory\n");
			return -ENOMEM;
		}
		pr_info(KERN_INFO "sprdfb: got %d bytes mem at 0x%p\n", len, addr);
		...
		if (NULL == addr) {
			pr_info(KERN_ERR "sprdfb: Unable to map framebuffer base: 0x%p\n", addr);
			return -ENOMEM;
		}
		dev->fb->fix.smem_len = len;
		dev->fb->screen_base = (char *)addr;
...
static int sprdfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *fb)
{
	...
	if (0 == dev->enable) {
		pr_info(KERN_ERR "sprdfb: [%s]: Invalid Device status %d", __func__, dev->enable);
		return -1;
	}

	ret = dev->ctrl->refresh(dev);
	if (ret) {
		pr_info(KERN_ERR "sprdfb: failed to refresh !!!!\n");
		return -1;
	}
			...
			unsigned long arg)
{
	...
	case SPRD_FB_SET_OVERLAY:
		...
		if (copy_from_user(&local_overlay_info, argp, sizeof(local_overlay_info))) {
			pr_info("sprdfb: SET_OVERLAY copy failed!\n");
			return -EFAULT;
		}
	...
	case SPRD_FB_DISPLAY_OVERLAY:
		...
		if (copy_from_user(&local_overlay_display, argp, sizeof(local_overlay_display))) {
			pr_info("sprdfb: DISPLAY_OVERLAY copy failed!\n");
			return -EFAULT;
		}
	...
	case SPRD_FB_SET_POWER_MODE:
		...
		pr_info("sprdfb: [%s] : SPRD_FB_SET_POWER_MODE (%d)\n", __func__, power_mode);
		break;

	default:
		pr_info(KERN_INFO "sprdfb: [%s]: unknown cmd(%d)\n", __func__, cmd);
		break;
	}
...
static void ESD_work_func(struct work_struct *work)
{
	...
	if (0 != dev->enable) {
		...
		pr_info("sprdfb: DON't reschedule ESD workqueue since device not avialbe!!\n");
...
uint32_t sprdfb_get_chip_id(void)
{
	...
	if ((adie_chip_id & 0xffff0000) < 0x50000000) {
		pr_info("sprdfb: [%s], chip id 0x%08x is invalidate, try to get it by reading reg directly!\n", __func__, adie_chip_id);

		adie_chip_id = dispc_glb_read(SPRD_AONAPB_BASE+0xFC);
		if ((adie_chip_id & 0xffff0000) < 0x50000000)
			pr_info("sprdfb: [%s], chip id 0x%08x from reg is invalidate too!\n", __func__, adie_chip_id);
	}

	pr_info("sprdfb: return chip id 0x%08x\n", adie_chip_id);
	return adie_chip_id;
}
...
static int sprdfb_probe(struct platform_device *pdev)
{
	...
	if (!fb) {
		pr_info(KERN_ERR "sprdfb: sprdfb_probe allocate buffer fail.\n");
		ret = -ENOMEM;
		goto err0;
	}
	...
	pr_info("sprdfb: [%s] id = %d\n", __func__, dev->dev_id);
#else
	dev->dev_id = pdev->id;
#endif
	if ((SPRDFB_MAINLCD_ID != dev->dev_id) && (SPRDFB_SUBLCD_ID != dev->dev_id)) {
		pr_info(KERN_ERR "sprdfb: sprdfb_probe fail. (unsupported device id)\n");
		goto err0;
	}
	...
	if (SPRDFB_MAINLCD_ID == dev->dev_id) {
		...
		if (0 != of_address_to_resource(pdev->dev.of_node, 0, &r)) {
			pr_info(KERN_ERR "sprdfb: sprdfb_probe fail. (can't get register base address)\n");
			goto err0;
		}
		...
		if (!g_dispc_base_addr)
			BUG();
		pr_info("sprdfb: set g_dispc_base_addr = %ld\n", g_dispc_base_addr);
#endif
	}
	...
	if (sprdfb_panel_get(dev)) {
		...
		if (0 != ret) {
			pr_info("sprdfb: read display_size from dts fail (%d)\n", ret);
	...
	if (ret) {
		pr_info(KERN_ERR "sprdfb: sprdfb_probe register framebuffer fail.\n");
		goto cleanup;
	}
