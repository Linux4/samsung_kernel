...
int dispc_notifier_register(struct dispc_dbs *handler)
{
	...
	struct dispc_dbs *e;
		printk( KERN_ERR "dispc_notifier_register \n");
...
static unsigned int dispc_change_notification(int state, unsigned int type)
{
	...
	list_for_each_entry(pos, &dispc_dev_handlers, link) {
		if (pos->dispc_notifier != NULL) {
			...
			printk(KERN_ERR "%s: calling %pf done \n", __func__, pos->dispc_notifier );
		}
	}
...
static ssize_t __sprd_set_fps(struct sprd_notifier* dev, const char* buf, size_t count)
{
	...
	printk(KERN_ERR "__sprdfb_fps_set_val %d\n", val);
...
static int __init sprd_notifier_init(void)
{
	...
	printk(KERN_ERR"Initializing sprd dispc device.\n");
	...
	if(!sprd_notifier_obj) {
		err = -ENOMEM;
		printk(KERN_ERR"Failed to alloc sprd_notifier.\n");
		goto unregister;
	}

	err = __sprd_dispc_setup_dev(sprd_notifier_obj);
	if(err) {
		printk(KERN_ERR"Failed to setup dev: %d.\n", err);
		goto cleanup;
	}
	...
	if(IS_ERR(sprd_dispc_class)) {
		...
		printk(KERN_ERR"Failed to create sprd dispc class.\n");
		goto destroy_cdev;
   }
...
//	sprd_dispc_class->dev_attrs = dispc_device_attributes;
	...
	if(IS_ERR(temp)) {
		err = PTR_ERR(temp);
		printk(KERN_ERR"Failed to create sprd dispc device.");
		goto destroy_class;
	}

	err = device_create_file(temp, &dev_attr_fps);
	if(err < 0) {
		printk(KERN_ERR"Failed to create attribute val.");
		goto destroy_device;
	}

	dev_set_drvdata(temp, sprd_notifier_obj);

	printk(KERN_ERR"Succedded to initialize sprd dispc device.\n");
	return 0;
...
static void __exit sprd_notifier_exit(void)
{
	...
	printk(KERN_ERR"Destroy sprd dispc device.\n");
