		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!fb_dev) {
		pr_err("[LCD] fb_dev can't be found\n");
		return -ENXIO;
	}
			...
			const char *buf, size_t count)
{
	...
	if (divider < 1 || divider > 0xff || dpi_clk_src < 1) {
		pr_err("[LCD] divider:[%d], clk_src:[%d] is invalid\n",
				divider, dpi_clk_src);
		return count;
	}
...
DIRECT_GO:
	if (new_pclk == fb_dev->dpi_clock) {
		/* Do nothing */
		pr_warn("[LCD] new pclk is the same as current pclk\n");
		return count;
	}

	ret = sprdfb_chg_clk_intf(fb_dev, SPRDFB_DYNAMIC_PCLK, new_pclk);
	if (ret) {
		pr_err("[LCD] %s: failed to change dpi clock. ret=%d\n",
				__func__, ret);
		return ret;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!panel) {
		pr_err("[LCD] panel doesn't exist.");
		return -ENXIO;
	}
	ret = snprintf(buf, PAGE_SIZE,
			"current fps: %u\nnew fps: %u\norigin fps: %u\n",
			...
			const char *buf, size_t count)
{
	...
	if (panel->fps == fps) {
		/* Do nothing */
		pr_warn("[LCD] new fps is the same as current fps\n");
		return count;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (panel->type != LCD_MODE_DSI) {
		pr_err("[LCD] Current panel is not mipi dsi\n");
		ret = snprintf(buf, PAGE_SIZE,
			...
			const char *buf, size_t count)
{
	...
	if (panel->type != LCD_MODE_DSI) {
		pr_err("[LCD] sys write failure. Current panel is not mipi dsi\n");
		return count;
	}
	...
	pr_debug("[LCD] input dphy_freq is %d\n", dphy_freq);

	if (dphy_freq == attr_info->curr_mipi_clk) {
		/* Do nothing */
		pr_warn("[LCD] new dphy_freq is the same as current freq\n");
		return count;
	}
	...
	if (dphy_freq <= 1500000 && dphy_freq >= 90000) {
		...
		if (ret) {
			pr_err("[LCD] sprdfb_chg_clk_intf change d-phy freq fail.\n");
			return count;
		}
		attr_info->curr_mipi_clk = dphy_freq;
	} else {
		pr_warn("[LCD] input mipi frequency:%d is out of range.\n",
				dphy_freq);
	}
...
int sprdfb_create_sysfs(struct sprdfb_device *fb_dev)
{
	...
	if (rc)
		pr_err("[LCD] sysfs group creation failed, rc=%d\n", rc);
