...
static int __init lcd_id_get(char *str)
{
	...
	pr_info(KERN_INFO "sprdfb: [%s]LCD Panel ID from uboot: 0x%x\n", __func__, lcd_id_from_uboot);
	return 1;
}
...
static int __init lcd_base_get(char *str)
{
	...
	pr_info(KERN_INFO "sprdfb: [%s]LCD Panel Base from uboot: 0x%lx\n", __func__, lcd_base_from_uboot);
	return 1;
}
...
#endif
	default:
		pr_info("sprdfb: [%s]: erro panel type.(%d,%d, %d)", __func__, cfg->dev_id, cfg->lcd_id, cfg->panel->type);
...
int panel_init(struct sprdfb_device *dev)
{
	...
	pr_debug("sprdfb: [%s], dev_id= %d, type = %d\n", __func__, dev->dev_id, dev->panel->type);

	if (!dev->panel->if_ctrl->panel_if_init(dev)) {
		pr_info(KERN_ERR "sprdfb: [%s]: panel_if_init fail!\n", __func__);
		return -1;
	}
...
static struct panel_spec *adapt_panel_from_uboot(uint16_t dev_id)
{
	...
	list_for_each_entry(cfg, panel_list, list) {
		if (lcd_id_from_uboot == cfg->lcd_id) {
			pr_info(KERN_INFO "sprdfb: [%s]: LCD Panel 0x%x is attached!\n", __func__, cfg->lcd_id);
			return cfg->panel;
		}
	}
	pr_info(KERN_ERR "sprdfb: [%s]: Failed to match LCD Panel from uboot!\n", __func__);
...
static struct panel_spec *adapt_panel_from_readid(struct sprdfb_device *dev)
{
	...
	if (dummy_cfg != NULL) {
		pr_info("sprdfb: [%s]: Can't find read panel, Use dummy panel!\n", __func__);
	...
	pr_info(KERN_ERR "sprdfb:  [%s]: failed to attach LCD Panel!\n", __func__);
	return NULL;
}
...
bool sprdfb_panel_get(struct sprdfb_device *dev)
{
	...
	if (NULL == dev) {
		pr_info("sprdfb: [%s]: Invalid param\n", __func__);
		return false;
	}
...
uint32_t sprdfb_panel_esd_check(struct sprdfb_device *dev)
{
	...
	if (SPRDFB_PANEL_IF_EDPI == dev->panel_if_type) {
		...
		if (!result) { /*time out*/
			pr_info("sprdfb: [%s] esd check  not got te signal!!!!\n", __func__);
			dev->esd_te_waiter = 0;
			result = 0;
		} else {
	...
	if (result == 0) {
		...
		if (0 == if_status) {
			pr_info("sprdfb: [%s] fail! Need reset panel.(%d,%d,%d)\n", __func__,
			...
			pr_info("sprdfb: [%s] fail! Need reset panel and panel if!!!!(%d,%d,%d)\n", __func__,
							dev->check_esd_time, dev->panel_reset_time, dev->reset_dsi_time);
			dev->reset_dsi_time++;
...
int sprdfb_panel_register(struct panel_cfg *cfg)
{
	...
	if (!panel_check(cfg)) {
		pr_info("[LCD] %s : panel check fail!id = %d\n", __func__, cfg->dev_id);
		return -1;
	}
