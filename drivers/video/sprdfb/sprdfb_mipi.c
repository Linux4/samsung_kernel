...
static void mipi_dispc_init_config(struct panel_spec *panel)
{
	...
	if(NULL == panel){
		printk(KERN_ERR "[LCD] sprdfb: [%s] fail.(Invalid Param)\n", __FUNCTION__);
		return;
	}

	if(SPRDFB_PANEL_TYPE_MIPI != panel->type){
		printk(KERN_ERR "[LCD] sprdfb: [%s] fail.(not  mcu panel)\n", __FUNCTION__);
		return;
	}
	...
	if(SPRDFB_MIPI_MODE_VIDEO == panel->info.mipi->work_mode){
		...
		if(!(panel->cap & PANEL_CAP_NOT_TEAR_SYNC)){
			printk("[LCD] sprdfb: mipi_dispc_init_config not support TE\n");
			/*enable te*/
			reg_val |= (1<<8);
		}
		/*te pol*/
	...
	pr_debug("[LCD] sprdfb: [%s] DISPC_DPI_CTRL = %d\n", __FUNCTION__, dispc_read(DISPC_DPI_CTRL));
}
...
{
	if(NULL == panel){
		printk("[LCD] sprdfb: [%s] fail. (Invalid param)\n", __FUNCTION__);
		return false;
	}

	if(SPRDFB_PANEL_TYPE_MIPI != panel->type){
		printk("[LCD] sprdfb: [%s] fail. (not mipi param)\n", __FUNCTION__);
		return false;
	}
...
static void sprdfb_mipi_panel_mount(struct sprdfb_device *dev)
{
	if((NULL == dev) || (NULL == dev->panel)){
		printk(KERN_ERR "[LCD] sprdfb: [%s]: Invalid Param\n", __FUNCTION__);
		return;
	}

	pr_debug(KERN_INFO "[LCD] sprdfb: [%s], dev_id = %d\n",__FUNCTION__, dev->dev_id);
