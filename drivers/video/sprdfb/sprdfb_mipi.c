...
static void mipi_dispc_init_config(struct panel_spec *panel)
{
	...
	if (NULL == panel) {
		pr_info(KERN_ERR "sprdfb: [%s] fail.(Invalid Param)\n", __func__);
		return;
	}

	if (SPRDFB_PANEL_TYPE_MIPI != panel->type) {
		pr_info(KERN_ERR "sprdfb: [%s] fail.(not  mcu panel)\n", __func__);
		return;
	}
	...
	if (SPRDFB_MIPI_MODE_VIDEO == panel->info.mipi->work_mode) {
		...
		if (!(panel->cap & PANEL_CAP_NOT_TEAR_SYNC)) {
			pr_info("sprdfb: mipi_dispc_init_config not support TE\n");
			/*enable te*/
			reg_val |= (1<<8);
		}
		/*te pol*/
	...
	pr_debug("sprdfb: [%s] DISPC_DPI_CTRL = %d\n", __func__, dispc_read(DISPC_DPI_CTRL));
}
...
{
	if (NULL == panel) {
		pr_info("sprdfb: [%s] fail. (Invalid param)\n", __func__);
		return false;
	}

	if (SPRDFB_PANEL_TYPE_MIPI != panel->type) {
		pr_info("sprdfb: [%s] fail. (not mipi param)\n", __func__);
		return false;
	}
...
static void sprdfb_mipi_panel_mount(struct sprdfb_device *dev)
{
	if ((NULL == dev) || (NULL == dev->panel)) {
		pr_info(KERN_ERR "sprdfb: [%s]: Invalid Param\n", __func__);
		return;
	}
...
{
	pr_info(KERN_INFO "sprdfb: [%s], dev_id = %d\n", __func__, dev->dev_id);
	sprdfb_dsi_uninit(dev);
