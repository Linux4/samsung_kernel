	...
	if ((reg_val & DISPC_INT_TE_MASK)
		&& (SPRDFB_PANEL_IF_DPI == dev->panel_if_type)) {
			...
			if (0 != dev->esd_te_waiter) {
				pr_info("sprdfb: dispc_isr esd_te_done!");
				dev->esd_te_done = 1;
...
static int32_t dispc_sync(struct sprdfb_device *dev)
{
	...
	if (dev->enable == 0) {
		pr_info("sprdfb: dispc_sync fb suspeneded already!!\n");
		return -1;
	}
	...
	if (!ret) { /* time out */
		...
		pr_info(KERN_ERR "sprdfb: dispc_sync time out!!!!!\n");
		{/*for debug*/
			int32_t i = 0;
			for (i = 0; i < 256; i += 16)
...
static int32_t dispc_clk_init(struct sprdfb_device *dev)
{
	...
	if (0 != ret) {
		pr_info("sprdfb: read clock-src fail (%d)\n", ret);
		return -1;
	}

	ret = of_property_read_u32(dev->of_dev->of_node, "dpi_clk_div", &def_dpi_clk_div);
	if (0 != ret) {
		pr_info("sprdfb: read dpi_clk_div fail (%d)\n", ret);
		return -1;
	}
	...
	if (IS_ERR(clk_parent1)) {
		pr_info(KERN_WARNING "sprdfb: get clk_parent1 fail!\n");
		return -1;
	} else {
	...
	if (IS_ERR(clk_parent2)) {
		pr_info(KERN_WARNING "sprdfb: get clk_parent2 fail!\n");
		return -1;
	} else {
	...
	if (IS_ERR(clk_parent3)) {
		pr_info(KERN_WARNING "sprdfb: get clk_parent3 fail!\n");
		return -1;
	} else {
...
#endif
	if (IS_ERR(clk_parent3)) {
		pr_info(KERN_WARNING "sprdfb: get clk_parent4 fail!\n");
		return -1;
	} else {
	...
	if (IS_ERR(dispc_ctx.clk_dispc)) {
		pr_info(KERN_WARNING "sprdfb: get clk_dispc fail!\n");
		return -1;
	} else {
	...
	if (IS_ERR(dispc_ctx.clk_dispc_dbi)) {
		pr_info(KERN_WARNING "sprdfb: get clk_dispc_dbi fail!\n");
		return -1;
	} else {
	...
	if (IS_ERR(dispc_ctx.clk_dispc_dpi)) {
		pr_info(KERN_WARNING "sprdfb: get clk_dispc_dpi fail!\n");
		return -1;
	} else {
	...
	if (ret)
		pr_info(KERN_ERR "sprdfb: dispc set clk parent fail\n");
	...
	if (ret)
		pr_info(KERN_ERR "sprdfb: dispc set dbi clk parent fail\n");
	...
	if (ret)
		pr_info(KERN_ERR "sprdfb: dispc set dpi clk parent fail\n");


	ret = clk_set_parent(dispc_ctx.clk_dispc_emc, clk_parent4);
	if (ret)
		pr_info(KERN_ERR "sprdfb: dispc set emc clk parent fail\n");
	...
	if (ret)
		pr_info("sprdfb: enable clk_dispc_emc error!!!\n");


	ret = sprdfb_dispc_clk_enable(&dispc_ctx, SPRDFB_DYNAMIC_CLK_FORCE);
	if (ret) {
		pr_info(KERN_WARNING "sprdfb: [%s] enable dispc_clk fail!\n", __func__);
		return -1;
	} else
...
static int32_t sprdfb_dispc_module_init(struct sprdfb_device *dev)
{
	...
	if (dispc_ctx.is_inited) {
		pr_info(KERN_WARNING "sprdfb: dispc_module has already initialized! warning!!");
		return 0;
	} else
		pr_info(KERN_INFO "sprdfb: dispc_module_init. call only once!");
...
#endif
	pr_info("sprdfb: dispc irq_num = %d\n", irq_num);
	...
	if (ret) {
		pr_info(KERN_ERR "sprdfb: dispc failed to request irq!\n");
...
static int32_t sprdfb_dispc_early_init(struct sprdfb_device *dev)
{
	...
	if (ret) {
		pr_info(KERN_WARNING "sprdfb: dispc_clk_init fail!\n");
		return -1;
	}
	...
	if (!dispc_ctx.is_inited) {
		...
		if (dev->panel_ready) {
			/*panel ready*/
			pr_info(KERN_INFO "sprdfb: [%s]: dispc has alread initialized\n", __func__);
			dispc_ctx.is_first_frame = false;
		} else {
			/*panel not ready*/
			pr_info(KERN_INFO "sprdfb: [%s]: dispc is not initialized\n", __func__);
			dispc_reset();
			dispc_module_enable();
			dispc_ctx.is_first_frame = true;
		}
		...
		/*resume*/
		pr_info(KERN_INFO "sprdfb: [%s]: sprdfb_dispc_early_init resume\n", __func__);
		dispc_reset();
		dispc_module_enable();
		dispc_ctx.is_first_frame = true;
	}
...
static int sprdfb_dispc_clk_enable(struct sprdfb_dispc_context *dispc_ctx_ptr, SPRDFB_DYNAMIC_CLK_SWITCH_E clock_switch_type)
{
	...
	if (!dispc_ctx_ptr->clk_is_open) {
		...
		if (ret) {
			pr_info("sprdfb: enable clk_dispc error!!!\n");
			ret = -1;
		...
		if (ret) {
			pr_info("sprdfb: enable clk_dispc_dpi error!!!\n");
			ret = -1;
			goto ERROR_CLK_ENABLE;
		}
		...
		if (ret) {
			pr_info("sprdfb: enable clk_dispc_dbi error!!!\n");
			ret = -1;
			goto ERROR_CLK_ENABLE;
		}
...
ERROR_CLK_ENABLE:
	...
	pr_info("sprdfb: sprdfb_dispc_clk_enable error!!!!!!\n");
	return ret;
}
...
static int32_t sprdfb_dispc_init(struct sprdfb_device *dev)
{
	...
	if (NULL == dev) {
		pr_info("sprdfb: [%s] Invalid parameter!\n", __func__);
		return -1;
	}
...
static int32_t sprdfb_dispc_refresh(struct sprdfb_device *dev)
{
	...
	down(&dev->refresh_lock);
	if (0 == dev->enable) {
		pr_info("sprdfb: [%s]: do not refresh in suspend!!!\n", __func__);
		goto ERROR_REFRESH;
	}
	...
	if (SPRDFB_PANEL_IF_DPI != dev->panel_if_type) {
		...
		if (sprdfb_dispc_clk_enable(&dispc_ctx, SPRDFB_DYNAMIC_CLK_REFRESH)) {
			pr_info(KERN_WARNING "sprdfb: enable dispc_clk fail in refresh!\n");
			goto ERROR_REFRESH;
		}
#endif
	}
	...
	if (!dev->ESD_work_start) {
		pr_info("sprdfb: schedule ESD work queue!\n");
...
{
	pr_info(KERN_INFO "sprdfb: [%s], dev->enable = %d\n", __func__, dev->enable);
	...
	if (0 != dev->enable) {
		...
		if (SPRDFB_PANEL_IF_DPI != dev->panel_if_type) {
			...
			if (sprdfb_dispc_clk_enable(&dispc_ctx, SPRDFB_DYNAMIC_CLK_COUNT))
				pr_info(KERN_WARNING "sprdfb: [%s] clk enable fail!!!\n", __func__);
#endif
			pr_info(KERN_INFO "sprdfb: [%s] got sync\n", __func__);
		}
		...
		if (dev->ESD_work_start == true) {
			pr_info("sprdfb: cancel ESD work queue\n");
		...
		pr_info(KERN_ERR "sprdfb: [%s]: Invalid device status %d\n", __func__, dev->enable);
...
static int32_t sprdfb_dispc_resume(struct sprdfb_device *dev)
{
	...
	if (dev->enable == 0) {
		...
		if (1) {/*(dispc_read(DISPC_SIZE_XY) == 0 ) {  resume from deep sleep */
			pr_info(KERN_INFO "sprdfb: [%s] from deep sleep\n", __func__);
			sprdfb_dispc_early_init(dev);
			sprdfb_panel_resume(dev, true);
			sprdfb_dispc_init(dev);
		} else {
			pr_info(KERN_INFO "sprdfb: [%s]  not from deep sleep\n", __func__);
	...
	pr_info(KERN_INFO "sprdfb: [%s], leave dev->enable= %d\n", __func__, dev->enable);
...
static int32_t sprdfb_dispc_check_esd(struct sprdfb_device *dev)
{
	...
	if (SPRDFB_PANEL_IF_DBI == dev->panel_if_type) {
		pr_info("sprdfb: [%s] leave (not support dbi mode now)!\n", __func__);
		ret = -1;
	...
	if (0 == (dev->check_esd_time % 30))
	    pr_info("sprdfb: [%s] (%d, %d, %d)\n",
			__func__, dev->check_esd_time, dev->panel_reset_time, dev->reset_dsi_time);
	else
...
/*
	if (SPRD_OVERLAY_STATUS_OFF  != dispc_ctx.overlay_state) {
		pr_info(KERN_ERR "sprdfb: Overlay open fail (has been opened)");
		return -1;
	}
*/
...
static int overlay_start(struct sprdfb_device *dev, uint32_t layer_index)
{
	pr_debug("sprdfb: [%s] : %d, %d\n", __func__, dispc_ctx.overlay_state, layer_index);


	if (SPRD_OVERLAY_STATUS_ON  != dispc_ctx.overlay_state) {
		pr_info(KERN_ERR "sprdfb: overlay start fail. (not opened)");
		return -1;
	}

	if ((0 == dispc_read(DISPC_IMG_Y_BASE_ADDR)) && (0 == dispc_read(DISPC_OSD_BASE_ADDR))) {
		pr_info(KERN_ERR "sprdfb: overlay start fail. (not configged)");
		return -1;
	}

/*
	if (0 != dispc_sync(dev)) {
		pr_info(KERN_ERR "sprdfb: overlay start fail. (wait done fail)");
		return -1;
	}
*/
	dispc_set_bg_color(0x0);
...
static int overlay_img_configure(struct sprdfb_device *dev, int type, overlay_rect *rect, unsigned char *buffer, int y_endian, int uv_endian, bool rb_switch)
{
	...
	pr_debug("sprdfb: [%s] : %d, (%d, %d,%d,%d), 0x%x\n", __func__, type, rect->x, rect->y, rect->h, rect->w, (unsigned int)buffer);


	if (SPRD_OVERLAY_STATUS_ON  != dispc_ctx.overlay_state) {
		pr_info(KERN_ERR "sprdfb: Overlay config fail (not opened)");
		return -1;
	}

	if (type >= SPRD_DATA_TYPE_LIMIT) {
		pr_info(KERN_ERR "sprdfb: Overlay config fail (type error)");
		return -1;
	}

	if ((y_endian >= SPRD_IMG_DATA_ENDIAN_LIMIT) || (uv_endian >= SPRD_IMG_DATA_ENDIAN_LIMIT)) {
		pr_info(KERN_ERR "sprdfb: Overlay config fail (y, uv endian error)");
		return -1;
	}
...
static int overlay_osd_configure(struct sprdfb_device *dev, int type, overlay_rect *rect, unsigned char *buffer, int y_endian, int uv_endian, bool rb_switch)
{
	...
	if (y_endian >= SPRD_IMG_DATA_ENDIAN_LIMIT) {
		pr_info(KERN_ERR "sprdfb: Overlay config fail (rgb endian error)");
		return -1;
	}
...
{
	if (SPRD_OVERLAY_STATUS_OFF  == dispc_ctx.overlay_state) {
		pr_info(KERN_ERR "sprdfb: overlay close fail. (has been closed)");
		return 0;
	}

/*
	if (0 != sprd_lcdc_sync(dev)) {
		pr_info(KERN_ERR "sprdfb: overlay close fail. (wait done fail)\n");
		return -1;
	}
*/
	/*dispc_set_bg_color(0xFFFFFFFF);*/
...
static int32_t sprdfb_dispc_enable_overlay(struct sprdfb_device *dev, struct overlay_info *info, int enable)
{
	...
	if (enable) {  /*enable*/
		if (NULL == info) {
			pr_info(KERN_ERR "sprdfb: sprdfb_dispc_enable_overlay fail (Invalid parameter)\n");
		...
		if (0 == dev->enable) {
			pr_info(KERN_ERR "sprdfb: sprdfb_dispc_enable_overlay fail. (dev not enable)\n");
			goto ERROR_ENABLE_OVERLAY;
		}

		if (0 != dispc_sync(dev)) {
			pr_info(KERN_ERR "sprdfb: sprdfb_dispc_enable_overlay fail. (wait done fail)\n");
			goto ERROR_ENABLE_OVERLAY;
		}
		...
		if (SPRD_LAYER_IMG == info->layer_index)
			...
			pr_info(KERN_ERR "sprdfb: sprdfb_dispc_enable_overlay fail. (invalid layer index)\n");
...
static int32_t sprdfb_dispc_display_overlay(struct sprdfb_device *dev, struct overlay_display *setting)
{
	...
	if (SPRD_OVERLAY_STATUS_ON == dispc_ctx.overlay_state) {
		if (overlay_start(dev, setting->layer_index) != 0) {
			pr_info("sprdfb: %s[%d] overlay_start() err, return without run dispc!\n", __func__, __LINE__);
			goto ERROR_DISPLAY_OVERLAY;
		}
	}
	...
	if ((SPRD_OVERLAY_DISPLAY_SYNC == setting->display_mode)
		...
		if (dispc_sync(dev) != 0) /* time out??? disable ?? */
			pr_info("sprdfb: sprdfb  do sprd_lcdc_display_overlay  time out!\n");
		/*dispc_ctx.vsync_done = 0;*/
	}
...
static int32_t spdfb_dispc_wait_for_vsync(struct sprdfb_device *dev)
{
	...
	if (SPRDFB_PANEL_IF_DPI == dev->panel_if_type) {
		...
		if (!ret) { /* time out */
				...
				pr_info(KERN_ERR "sprdfb: vsync time out!!!!!(0x%x, 0x%x)\n",
					reg_val0, reg_val1);
				{/*for debug*/
					int32_t i = 0;
					for (i = 0; i < 256; i += 16) {
		...
		if (!ret) { /* time out */
			dispc_ctx.waitfor_vsync_done = 1;
			pr_info(KERN_ERR "sprdfb: vsync time out!!!!!\n");
			{/*for debug*/
				int32_t i = 0;
				for (i = 0; i < 256; i += 16) {
					pr_info("sprdfb: %x: 0x%x, 0x%x, 0x%x, 0x%x\n", i,
...
static void dispc_stop_for_feature(struct sprdfb_device *dev)
{
	...
	if (SPRDFB_PANEL_IF_DPI == dev->panel_if_type) {
		...
		if (wait_count >= 100)
			pr_info("sprdfb:[%s] can't wait till dispc stop!!!\n", __func__);
...
static unsigned int sprdfb_dispc_change_threshold(struct devfreq_dbs *h, unsigned int state)
{
	...
	if (NULL == dev || 0 == dev->enable) {
		/*pr_info(KERN_ERR "sprdfb: sprdfb_dispc_change_threshold fail.(dev not enable)\n");*/
		return 0;
	}
	pr_info(KERN_ERR "sprdfb: DMC change freq(%u)\n", state);
	mdelay(10);
	if (SPRDFB_PANEL_IF_DPI == dev->panel_if_type) {
...
static int32_t sprdfb_dispc_refresh_logo(struct sprdfb_device *dev)
{
	...
	if (SPRDFB_PANEL_IF_DPI != dev->panel_if_type) {
		pr_info(KERN_ERR "sprdfb: sprdfb_dispc_refresh_logo if type error\n!");
		return 0;
	}
...
/* wait for update- done interruption */
	...
	if (i >= 500) {
		pr_info("sprdfb: [%s] wait dispc done int time out!! (0x%x)\n",
...
void sprdfb_dispc_logo_proc(struct sprdfb_device *dev)
{
	...
	if (dev == NULL) {
		pr_info("sprdfb: %s[%d]: dev == NULL, return without process logo!!\n",
					__func__, __LINE__);
		return;
	}

	if (lcd_base_from_uboot == 0L) {
		pr_info("sprdfb: %s[%d]: lcd_base_from_uboot == 0, return without process logo!!\n",
					__func__, __LINE__);
		return;
	}
	if (SPRDFB_PANEL_IF_DPI != dev->panel_if_type)
...
/*#define USE_OVERLAY_BUFF*/
	...
	if (!dev->logo_buffer_addr_v) {
		pr_info(KERN_ERR "sprdfb: %s Failed to allocate logo proc buffer\n", __func__);
		return;
	}
	pr_info(KERN_INFO "sprdfb:got %d bytes logo proc buffer at 0x%p\n",
	...
	if (!logo_src_v || !logo_dst_v) {
		pr_info(KERN_ERR "sprdfb: %s[%d]: Unable to map boot logo memory: src-0x%p, dst-0x%p\n",
				__func__, __LINE__, logo_src_v, logo_dst_v);
		return;
	}

	pr_info("sprdfb: %s[%d]: lcd_base_from_uboot: 0x%lx, logo_src_v:0x%p\n",
						__func__, __LINE__, lcd_base_from_uboot, logo_src_v);
	pr_info("sprdfb: %s[%d]: logo_dst_p: 0x%lx, logo_dst_v:0x%p\n",
						__func__, __LINE__, logo_dst_p, logo_dst_v);
	memcpy(logo_dst_v, logo_src_v, logo_size);
	dma_sync_single_for_device(dev->fb->dev,
			logo_dst_p, logo_size, DMA_TO_DEVICE);
...
{
	pr_info("sprdfb: sprdfb_is_refresh_done vsync_done=%d",
