	...
	for (volt = VOLT_V255, cnt = 0; volt >= VOLT_VT; volt--) {
		for (offset = R_OFFSET; offset <= B_OFFSET; offset++) {
			if (volt == VOLT_V255) {
				...
				dst[volt][offset] = char_to_int(src[cnt++]);
			}
		}
	}
...
static void gamma_init(struct SMART_DIM *pSmart, char *dest, int size)
{
	...
	int bl_level, cnt, volt, offset, brightness, candela_index;
	int *gamma_curve;
	...
	if (brightness < 350)
		gamma_curve = candela_coeff_2p15;
	else
		gamma_curve = candela_coeff_2p2;

	for (volt = VOLT_VT; volt < VOLT_MAX; volt++)
		candela_level[volt] =
			((long long)(gamma_curve[VT_CURVE[volt]])) *
			((long long)(bl_level));
	...
	for (volt = VOLT_VT; volt < VOLT_MAX; volt++)
		if (searching_function(candela_level[volt],
					&(bl_index[volt]), GAMMA_CURVE_2P2))
	...
	for (volt = VOLT_V3; volt < VOLT_MAX; volt++)
		for (offset = R_OFFSET; offset <= B_OFFSET; offset++)
			{
				if((gamma[volt][offset] - MTP[volt][offset]) < 0)
					gamma[volt][offset] = 0;
				else if((gamma[volt][offset] - MTP[volt][offset]) > 255)
					gamma[volt][offset] = 255;
				else
