...
static inline int tbl_value(struct tbl_s32 *table, int idx)
{
	if (unlikely(!table || !table->tbl || !table->sz)) {
		pr_err("%s, invalid table\n", __func__);
		return 0;
	}

	if (unlikely(idx < 0 || idx >= table->sz)) {
		pr_err("%s, out of table size(sz %d, idx %d)\n",
				__func__, table->sz, idx);
		return 0;
	}

	return table->tbl[idx];
}

#ifdef CONFIG_GEN_PANEL_OCTA
#ifdef USE_DIMMING_CORE
static unsigned int nr_tp;
static struct tbl_s32 tp_table;
static struct tbl_s32 mtp_base_table;
static struct tbl_s32 v_coeff_table;
static struct tbl_s32 vt_coeff_table;
static struct tbl_s32 gray_scale_offset_order_table;
static struct tbl_s32 rgb_color_offset_order_table;
#endif	/* USE_DIMMING_CORE */
static struct tbl_s32 aid_table;
static struct tbl_s32 elvss_table;
static struct tbl_s32 mps_table;
static struct tbl_s32 elvss_temp_table;
static struct tbl_s32 acl_table;
static struct tbl_s32 rgb_color_offset_table;
static struct tbl_s32 gray_scale_offset_table;
static struct tbl_s32 base_lux_table;
static struct tbl_s32 gamma_curve_table;
static struct tbl_s32 luminance_table;
static unsigned int nr_luminance;
...
#ifdef CONFIG_GEN_PANEL_DYNAMIC_AID
#ifdef USE_DIMMING_CORE
static bool dim_info_initialized = false;
static struct dimming_info dim_info;
#else
static struct smartdim_conf *sdimconf;
#endif	/* USE_DIMMING_CORE */
#endif	/* CONFIG_GEN_PANEL_DYNAMIC_AID */
#endif	/* CONFIG_GEN_PANEL_OCTA */
#ifdef CONFIG_DIMMING_DEBUG
extern int mtp_debug_attach(struct lcd *lcd, struct smartdim_conf *dim_conf);
extern void mtp_debug_detach(struct lcd *lcd);
#endif
                ...
                u8 *buf, u8 addr, u8 pos, u8 len)
{
	...
	while (tlen > 0) {
		read_len[0] = ((tlen < MAX_READ_BYTES) ? tlen : MAX_READ_BYTES);
		ret = gen_panel_rx_cmds(lcd, buf, read_desc, ARRAY_SIZE(read_desc));
		if (ret != read_len[0]) {
			pr_err("%s, failed to read addr:%2Xh, pos:%d, len:%u, ret %d\n",
...
static int gen_panel_set_elvss(struct lcd *lcd, int candela_index)
{
	...
	desc = find_op_cmd_desc(lcd, PANEL_TSET_CMD, 0xB6);
	if (desc) {
		...
		*elvss_temp_data =
			elvss_temp_table.tbl[itemp * nr_luminance + candela_index];
		pr_info("%s, elvss_temp 0x%02X\n", __func__, *elvss_temp_data);
	}
	...
	if (desc) {
		if (mps_table.tbl) {
			...
			*mps_data = mps_table.tbl[candela_index];
		...
		if (elvss_table.tbl) {
			...
			*elvss_data = elvss_table.tbl[candela_index];
		...
		if (elvss_temp_table) {
			...
			*elvss_temp_data =
				elvss_temp_table[itemp * nr_luminance + candela_index];
...
static int gen_panel_set_aid(struct lcd *lcd, int candela_index)
{
	...
	if (aid_table.tbl) {
		...
		*aid_data = htons((u16)(aid_table.tbl)[candela_index]);
...
static int gen_panel_set_acl(struct lcd *lcd, int candela_index)
{
	...
	if (!lcd->acl || ((candela_index == (nr_luminance - 1)) &&
	...
	if (acl_table.tbl) {
		...
		*acl_data = acl_table.tbl[candela_index];
	}
...
static int gen_panel_set_brightness(void *bd_data, int intensity)
{
	...
	if (IS_HBM(bl_info->auto_brightness) &&
		...
		index = find_candela_index_from_table(intensity,
				luminance_table.tbl, nr_luminance);
...
size_t gen_panel_read_ldi_info(struct lcd *lcd)
{
	...
	if (read_done_from_boot) {
		...
		/* set ELVSS cmd */
		if (elvss_temp_table.tbl)
			for (i = 0; i < nr_luminance * 3; i++)
				if (elvss_temp_table.tbl[i] == 0)
					elvss_temp_table.tbl[i] = boot_panel_elvss[0];
		...
		if (elvss_temp_table.tbl) {
			...
			pr_info("%s, temp_data %d\n", __func__, temp_data);
			for (i = 0; i < nr_luminance * 3; i++)
				if (!elvss_temp_table.tbl[i])
					elvss_temp_table.tbl[i] = temp_data;
		}
		gen_panel_write_op_cmds(lcd, PANEL_NV_DISABLE_CMD);
...
size_t gen_panel_read_mtp(struct lcd *lcd, u8 *mtp_data)
{
	...
	if (read_done_from_boot) {
		...
		panel_usleep(20000);
		len = gen_panel_read_reg1(lcd, out_buf, rd_reg, 0, rd_len);
		if (len == rd_len) {
			memcpy(mtp_data, out_buf, rd_len);
			pr_info("%s, MTP DATA\n", __func__);
			print_data(mtp_data, rd_len);
		} else
...
static int gen_panel_mdnie_calibration(struct lcd *lcd)
{
	struct mdnie_cal *cal = &lcd->mdnie.cal;
	struct gen_cmd_desc *desc;
	int area, rgb, i, scenario, index, mdnie_mode;
	unsigned char (*tune_tbl)[3];
	...
	for (i = 0; i < cal->nr_mode; i++) {
		mdnie_mode = cal->modes[i][0];
		if (cal->modes[i][1] >= cal->nr_tune) {
			pr_warn("%s, out of range (nr_tune %d)\n",
					__func__, cal->nr_tune);
			break;
		}
		tune_tbl = cal->tune[cal->modes[i][1]];
		for (scenario = 0; scenario < MDNIE_MODE_SIZE(mdnie_mode);
				scenario++) {
			if (scenario == PANEL_MDNIE_SCENARIO_EBOOK ||
					scenario == PANEL_MDNIE_SCENARIO_EMAIL)
				continue;
			index = MDNIE_MODE_OP_INDEX(mdnie_mode) + scenario;
			...
			for (rgb = OFFSET_R; rgb < RGB_OFFSET_MAX; rgb++) {
				...
				if (data) {
					...
					OP_CMD_DESC_NDATA_COPY(desc, cal->offset[rgb],
							&tune_tbl[area][rgb], 1);
					pr_info("%s, tune-table %d, offset 0x%02X, old 0x%02X, new 0x%02X\n",
							op_cmd_names[index], cal->modes[i][1],
...
#ifdef USE_DIMMING_CORE
static int mtp_8btoi(unsigned char val)
{
	int sign = (val & 0x80) ? -1 : 1;

	return sign * (val & 0x7F);
}

static int mtp_9btoi(unsigned char msb, unsigned char val)
{
	int sign = (msb & 0x1) ? -1 : 1;

	return sign * val;
}

static void mtp_ctoi(s32 (*dst)[MAX_COLOR], s8 *src, int nr_tp)
{
	int i, c, cnt;

	for (i = nr_tp - 1, cnt = 0; i >= 0; i--) {
		for (c = 0; c < MAX_COLOR; c++) {
			if (i == nr_tp - 1) {
				dst[i][c] = mtp_9btoi(src[cnt], src[cnt + 1]);
				cnt += 2;
			} else {
				dst[i][c] = mtp_8btoi(src[cnt++]);
			}
		}
	}
	...
	for (i = 0; i < nr_tp; i++)
		pr_info("[%d] %4d %4d %4d\n", i,
				dst[i][RED], dst[i][GREEN], dst[i][BLUE]);
}

static int gen_panel_dimming_info_init(struct dimming_info *dim_info)
{
	int i, ilum, c;

	if (unlikely(!dim_info)) {
		pr_err("%s, dim_info is null\n", __func__);
		return -EINVAL;
	}

	pr_info("%s, parse dimming info\n", __func__);
	for (i = 0; i < dim_info->nr_tp; i++)
		dim_info->tp[i] = tp_table.tbl[i];

	for (i = 0; i < dim_info->nr_tp; i++) {
		if (i == 0)
			sprintf(dim_info->tp_names[i], "VT");
		else
			sprintf(dim_info->tp_names[i], "V%d", dim_info->tp[i]);
	}

	dim_info->mtp.vregout = vregout_voltage;

	for (i = 0; i < dim_info->nr_tp; i++)
		for (c = 0; c < MAX_COLOR; c++)
			dim_info->mtp.mtp_base[i][c] =
				((int (*)[MAX_COLOR])mtp_base_table.tbl)[i][c];

	dim_info->mtp.g_curve_degree =
		gamma_value_to_degree(gamma_curve_table.tbl[dim_info->nr_luminance - 1]);
	if (unlikely(dim_info->mtp.g_curve_degree == GAMMA_NONE)) {
		pr_err("g_curve_degree %d\n", (int)dim_info->mtp.g_curve_degree);
		return -EINVAL;
	}

	for (i = 0; i < dim_info->nr_tp; i++) {
		dim_info->mtp.v_coeff[i].numerator = ((int (*)[2])v_coeff_table.tbl)[i][0];
		dim_info->mtp.v_coeff[i].denominator = ((int (*)[2])v_coeff_table.tbl)[i][1];
	}

	for (i = 0; i < vt_coeff_table.sz; i++)
		dim_info->mtp.vt_coeff[i].numerator = vt_coeff_table.tbl[i];

	for (ilum = 0; ilum < dim_info->nr_luminance; ilum++) {
		struct gamma_correction *dst = &dim_info->gamma_correction_lut[ilum];

		dst->luminance = luminance_table.tbl[ilum];
		dst->base_luminamce = base_lux_table.tbl[ilum];
		dst->g_curve_degree = gamma_value_to_degree(gamma_curve_table.tbl[ilum]);
		if (dst->g_curve_degree == GAMMA_NONE) {
			pr_err("g_curve_degree %d\n", (int)dst->g_curve_degree);
			return -EINVAL;
		}

		for (i = 0; i < gray_scale_offset_order_table.sz; i++)
			dst->gray_scale_offset[gray_scale_offset_order_table.tbl[i]] =
				gray_scale_offset_table.tbl[ilum * gray_scale_offset_order_table.sz + i];

		for (i = 0; i < rgb_color_offset_order_table.sz; i++)
			for (c = 0; c < MAX_COLOR; c++)
				dst->rgb_color_offset[rgb_color_offset_order_table.tbl[i]][c] =
					rgb_color_offset_table.tbl[(ilum * rgb_color_offset_order_table.sz * MAX_COLOR) + (i * MAX_COLOR) + c];
	}

	dim_info->target_luminance =
		dim_info->gamma_correction_lut[dim_info->nr_luminance - 1].luminance;
...
static int gen_panel_dimming_init(struct lcd *lcd)
{
	...
	pr_info("%s, alloc dimming info\n", __func__);
	alloc_dimming_info(&dim_info, nr_tp, nr_luminance);

	pr_info("%s, fill dimming info\n", __func__);
	gen_panel_dimming_info_init(&dim_info);

	ret = gen_panel_read_mtp(lcd, mtp_buf);
	if (ret < 0) {
		pr_err("%s, failed to read mtp\n", __func__);
		return -EINVAL;
	}
	mtp_ctoi(dim_info.mtp.mtp_offset, mtp_buf, nr_tp);

	pr_info("%s, process mtp\n", __func__);
	process_mtp(&dim_info);

	if (!gamma_table)
		gamma_table = kzalloc(sizeof(u8) * dim_info.nr_luminance * 33, GFP_KERNEL);
	for (i = 0; i < dim_info.nr_luminance; i++) {
		copy_mtp_output(dim_info.gamma_correction_lut,
				dim_info.gamma_correction_lut[i].luminance,
				&gamma_table[i * 33]);
	}
...
static int gen_panel_dimming_init(struct lcd *lcd)
{
	...
	sdimconf = get_conf();
	sdimconf->lux_tab = luminance_table.tbl;
	sdimconf->lux_tabsize = nr_luminance;
	sdimconf->rgb_offset = rgb_color_offset_table.tbl;
	sdimconf->candela_offset = gray_scale_offset_table.tbl;
	sdimconf->base_lux_table = base_lux_table.tbl;
	sdimconf->gamma_curve_table = gamma_curve_table.tbl;
	sdimconf->vregout_voltage = vregout_voltage;

#ifdef CONFIG_DIMMING_DEBUG
	mtp_debug_attach(lcd, sdimconf);
#endif
	...
	gamma_table = kzalloc(sizeof(u8) * nr_luminance * 33, GFP_KERNEL);
	for (i = 0; i < nr_luminance; i++) {
		sdimconf->generate_gamma(luminance_table.tbl[i],
				&gamma_table[i * 33]);
	}
 ...
 * @nr_row:		number of rows to read
 *
 */
static int gen_panel_parse_dt_table(struct device_node *np,
		const char *propname, struct tbl_s32 *table, int nr_row)
{
	int ret, sz;

	if (unlikely(!table || !propname || !nr_row)) {
		pr_err("%s, invalid parameter(%p %p %d)\n",
				__func__, propname, table, nr_row);
		return -EINVAL;
	}
	...
	if (sz % (nr_row)) {
		pr_err("%s: %s wrong table size(%d) nr_row(%d)\n",
				__func__, propname, sz, nr_row);
		return -EINVAL;
	}

	table->tbl = kzalloc(sz, GFP_KERNEL);
	if (unlikely(!table->tbl)) {
		pr_err("[LCD] %s: %s memory allocation failed\n",
				__func__, propname);
		return -ENOMEM;
	}
	table->sz = sz / sizeof(u32);

	ret = of_property_read_u32_array(np, propname,
			table->tbl, table->sz);
	if (unlikely(ret)) {
		pr_err("%s, read array property failed\n", __func__);
		kfree(table->tbl);
		table->sz = 0;
		return ret;
	}
		...
		struct manipulate_table *mani)
{
	...
	for (op_index = 0; op_index < PANEL_OP_CMD_MAX; op_index++)
		if (!strcmp(mani->cmd.name, op_cmd_names[op_index]))
			break;
		...
		struct device_node *np, struct lcd *lcd)
{
	...
	cal->nr_mode = (sz / (2 * sizeof(u32)));
	if (unlikely((sz % (2 * sizeof(u32))) || (!cal->nr_mode))) {
		pr_err("%s: wrong cal-modes size(%d)\n",
				__func__, sz);
		return -EINVAL;
	}

	cal->modes = (u32 (*)[2])kzalloc(sz, GFP_KERNEL);
	if (unlikely(!cal->modes)) {
		pr_err("%s: memory allocation failed (size:%d)\n",
				__func__, sz);
		return -ENOMEM;
	}
	of_property_read_u32_array(np, "cal-modes",
			(int *)cal->modes, cal->nr_mode * 2);

	of_property_read_u32_array(np, "cal-linear-equation", (u32 *)cal->line,
			sizeof(cal->line) / sizeof(int));

	for (index = 0; ; index++) {
		/* Retrieve the cal-tune-value-* property */
		sprintf(propname, "cal-tune-value-%d", index);
		if (!of_find_property(np, propname, &sz)) {
			pr_debug("end of cal-tune-value-%d not exist\n", index);
			break;
		}
	}
	...
	cal->nr_tune = index;
	if (cal->nr_tune) {
		cal->tune = (unsigned char (*)[9][3])kmalloc(cal->nr_tune *
				sizeof(u8) * 9 * 3, GFP_KERNEL);
		for (index = 0; index < cal->nr_tune; index++) {
			sprintf(propname, "cal-tune-value-%d", index);
			of_property_read_u8_array(np, propname,
					(u8 *)cal->tune[index], sizeof(u8) * 9 * 3);

			for (i = 0; i < 9; i++)
				pr_info("0x%02X 0x%02X 0x%02X\n",
						(u32)cal->tune[index][i][0],
						(u32)cal->tune[index][i][1],
						(u32)cal->tune[index][i][2]);
		...
		struct device_node *np, struct lcd *lcd)
{
	...
	of_property_read_u32(np, "gen-panel-octa-hbm-gamma-param-offset",
			&lcd->hbm_gamma_param_offset);
	ret = gen_panel_parse_dt_table(np, "gen-panel-candela-table", 
			&luminance_table, 1);
	if (unlikely(ret < 0)) {
		pr_err("%s, failed to parse candela-table(%d)\n",
				__func__, ret);
		return -EINVAL;
	}
	nr_luminance = luminance_table.sz;

#ifdef USE_DIMMING_CORE
	ret = gen_panel_parse_dt_table(np, "gen-panel-tp-table",
			&tp_table, 1);
	if (unlikely(ret < 0)) {
		pr_err("%s, failed to parse tp table(%d)\n",
				__func__, ret);
		return -EINVAL;
	}
	nr_tp = tp_table.sz;

	gen_panel_parse_dt_table(np, "gen-panel-mtp-base-table",
			&mtp_base_table, nr_tp);
	gen_panel_parse_dt_table(np, "gen-panel-v-coeff-table",
			&v_coeff_table, nr_tp);
	gen_panel_parse_dt_table(np, "gen-panel-vt-coeff-table",
			&vt_coeff_table, 1);
	gen_panel_parse_dt_table(np, "gen-panel-gray-scale-offset-order",
			&gray_scale_offset_order_table, 1);
	gen_panel_parse_dt_table(np, "gen-panel-rgb-color-offset-order",
			&rgb_color_offset_order_table, 1);
#endif

	gen_panel_parse_dt_table(np, "gen-panel-aid-table",
			&aid_table, nr_luminance);
	gen_panel_parse_dt_table(np, "gen-panel-acl-table",
			&acl_table, nr_luminance);
	gen_panel_parse_dt_table(np, "gen-panel-mps-table",
			&mps_table, nr_luminance);
	gen_panel_parse_dt_table(np, "gen-panel-elvss-table",
			&elvss_table, nr_luminance);
	gen_panel_parse_dt_table(np, "gen-panel-elvss-temperature-table",
			&elvss_temp_table, nr_luminance);
	gen_panel_parse_dt_table(np, "gen-panel-candela-compensation-table",
			&gray_scale_offset_table, nr_luminance);
	gen_panel_parse_dt_table(np, "gen-panel-rgb-compensation-table",
			&rgb_color_offset_table, nr_luminance);
	gen_panel_parse_dt_table(np, "gen-panel-base-lux-table",
			&base_lux_table, nr_luminance);
	gen_panel_parse_dt_table(np, "gen-panel-gamma-curve-table",
			&gamma_curve_table, nr_luminance);
...
static DEVICE_ATTR(manufacture_code, S_IRUGO, manufacture_code_show, NULL);
...
int gen_panel_remove(struct lcd *lcd)
{
	...
	kfree(acl_table.tbl);
	kfree(mps_table.tbl);
	kfree(elvss_table.tbl);
	kfree(elvss_temp_table.tbl);
	kfree(base_lux_table.tbl);
	kfree(gamma_curve_table.tbl);
	kfree(rgb_color_offset_table.tbl);
	kfree(gray_scale_offset_table.tbl);
	kfree(luminance_table.tbl);
	kfree(gamma_table);
#endif
	/* free command tables */
