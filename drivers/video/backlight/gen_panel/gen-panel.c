...
static int gen_panel_set_elvss(struct lcd *lcd, int candela_index)
{
	...
	if (op_cmd_valid(lcd, PANEL_ELVSS_TEMPERATURE_CMD)) {
		...
		if (desc_index <= 17) { /* >= 30cd*/
			gen_panel_write_reg(lcd, 0xB0, 0x10);
			gen_panel_write_reg(lcd, 0xB6, lcd->temperature_elvss_value);
		} else {
			...
			gen_panel_write_reg(lcd, 0xB0, 0x10);
			gen_panel_write_op_cmd_desc(lcd, PANEL_ELVSS_TEMPERATURE_CMD,
					desc_index);
		}
	}
...
void gen_panel_start(struct lcd *lcd, int status)
{
	...
	if (status) {
		...
		/* after video on, add delay of 2 frames
			before doing mDNIE update */
