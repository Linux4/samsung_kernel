...
static int gen_panel_set_elvss(struct lcd *lcd, int candela_index)
{
	...
	if (elvss_temp_table) {
		elvss_temp_data = &desc->data[data_offset + lcd->elvss_param_offset];
...
size_t gen_panel_read_ldi_info(struct lcd *lcd)
{
	...
	if (len != 29) {
		pr_err("%s, failed to read B6h\n", __func__);
		gen_panel_write_op_cmds(lcd, PANEL_NV_DISABLE_CMD);
		return -EINVAL;
	}
	print_data(reg_B6h, ARRAY_SIZE(reg_B6h));
	if (strcmp("ea8061s",lcd->panel_name)) {
		panel_usleep(20000);
		...
		panel_usleep(20000);
		len = gen_panel_read_reg(lcd, reg_A1h, 0xA1, 13);
		if (len != 13) {
			pr_err("%s, failed to read A1h\n", __func__);
			gen_panel_write_op_cmds(lcd, PANEL_NV_DISABLE_CMD);
			return -EINVAL;
		}
		print_data(reg_A1h, ARRAY_SIZE(reg_A1h));

		coordinate[0] = reg_A1h[0] << 8 | reg_A1h[1];	/* X */
		coordinate[1] = reg_A1h[2] << 8 | reg_A1h[3];	/* Y */
		date[0] = reg_A1h[4];
		date[1] = reg_A1h[5];
		gen_panel_write_op_cmds(lcd, PANEL_NV_DISABLE_CMD);
		if (elvss_temp_table)
...
#ifdef CONFIG_GEN_PANEL_OCTA
			gen_panel_set_brightness(bl_info->prvinfo,
							prev_intensity);
#endif
			gen_panel_write_op_cmds(lcd,PANEL_POST_ENABLE_1_CMD);
		}
