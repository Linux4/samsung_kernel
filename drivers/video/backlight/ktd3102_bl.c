		...
		int brightness)
{
	...
	if (IS_HBM(ginfo->auto_brightness) &&
		...
		return ginfo->range[BRT_VALUE_HBM].tune_level;
...
static int ktd3102_backlight_update_status(struct backlight_device *bd)
{
	...
	if (bd->props.power != FB_BLANK_UNBLANK ||
			...
			!atomic_read(&bl_info->ginfo.enable))
		brightness = 0;
...
static int ktd3102_backlight_probe(struct platform_device *pdev)
{
	...
	if (IS_ENABLED(CONFIG_OF)) {
		...
		ginfo->hbm_en = of_property_read_bool(brt_node,
				"gen-panel-bl-hbm-en");
	...
	atomic_set(&bl_info->ginfo.enable, 1);
...
static int ktd3102_backlight_runtime_suspend(struct device *dev)
{
	...
	atomic_set(&bl_info->ginfo.enable, 0);
	backlight_update_status(bd);
