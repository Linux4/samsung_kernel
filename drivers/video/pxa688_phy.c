		...
		u8 count, u8 lp)
{
	...
	u32 send_data = 0, waddr, i, timeout, turnaround;
	...
	for (i = 0; i < count; i++) {
		...
		if (!((i + 1) % 4)) {
			...
			while ((readl(&dsi->cmd3) &
				...
				timeout--;
			if (!timeout)
				pr_err("DSI write data to the packet "
	...
	timeout = 100000;
	while ((readl(&dsi->cmd0) &
		DSI_CFG_CPU_CMD_REQ_MASK) && timeout)
		timeout--;
	if (!timeout)
		pr_err("DSI send out packet maybe failed.\n");
}
...
void dsi_set_dphy(struct pxa168fb_info *fbi)
{
	...
	hs_trail = hs_trail > 3 ? hs_trail - 3 : 0;
