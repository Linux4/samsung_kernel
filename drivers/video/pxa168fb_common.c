		...
		int count)
{
	...
	if (buffer && packet_len) {
		...
		if (fbi->video_on) {
			pr_debug("%s: send command in vsync\n", __func__);
			txcmd_data.fbi = fbi;
			...
			if (!ret) {
				pr_err("%s: fail to send command in vsync\n", __func__);
				for (loop = 0; loop < count; loop++)
					dsi_send_prepared_cmd_tx(fbi, cmds[loop],
							buffer + DSI_MAX_DATA_BYTES * loop, packet_len[loop]);
			}
		} else {
			for (loop = 0; loop < count; loop++)
				dsi_send_prepared_cmd_tx(fbi, cmds[loop],
