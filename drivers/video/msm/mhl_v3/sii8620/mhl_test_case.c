...
extern int sii8620_detection_callback(unsigned long event);
...
static int mhl_test_case_hpd(struct sii8620_platform_data *pdata,
		unsigned long *muic_state, uint32_t delay, uint32_t count, int random)
{
	int i;
	unsigned int wait_sec;
	...
	if (!pdata) {
		pr_err("mhl_test_case_hpd pdata is NULL\n");
		return -ENXIO;
	}
	...
	for (i = 0; i < count && *muic_state; i++) {
		...
		msleep(wait_sec);
		pdata->hdmi_mhl_ops->set_upstream_hpd(pdata->hdmi_pdev, 1);
		msleep(50);
		mhl_tx_write_reg((void *)NULL, REG_HPD_CTRL,
...
void mhl_test_case_main(struct sii8620_platform_data *pdata, unsigned long *muic_state, int *val)
{
	...
	case MHL_TEST_MUIC:
		...
		break;
	case MHL_TEST_HPD:
		mhl_test_case_hpd(pdata, muic_state, val[1], val[2], val[3]);
		break;
