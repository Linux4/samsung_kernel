...
static void sii8620_setup_charging(struct mhl_dev_context *dev_context)
{
	...
	if ((version & 0xf0) >= 0x20) {
		...
		if (((version & 0xf0) < 0x30) && plim == 0x03)
			plim = 0x00;
		MHL_TX_DBG_ERR(": version: 0x%02x, plim(%d)\n", version, plim);
		...
		MHL_TX_DBG_ERR(": version: 0x%02x, id:%d, reserved:%d\n", version, adopter_id,
		...
		MHL_TX_DBG_ERR("MHL version error - 0x%02x\n", version);
	}
}
				...
				uint8_t data1)
{
	...
	if (req->status.flags.cancel == true) {
		...
		if (force_ocbus_for_ects == 2) {
			dev_context->dev_cap_cache.mdc.vid_link_mode
				&= ~((uint8_t)(MHL_DEV_VID_LINK_SUPP_16BPP |
		...
		MHL_TX_DBG_ERR("devcat: 0x%02X, devcat_changes: 0x%02X\n",
				 dev_context->dev_cap_cache.mdc.deviceCategory,
				devcap_changes.mdc.deviceCategory);
#ifdef CONFIG_MHL3_SEC_FEATURE
		if (MHL_DEV_CATEGORY_POW_BIT & dev_context->dev_cap_cache.mdc.
				deviceCategory)
