...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	gpio_set_value(ctrl->bl_on_gpio, 1);
	mdelay(1);
...
static int mdss_dsi_set_brightness_level(int level)
{
	int tune_level = 0;
...
static void mdss_dsi_panel_bklt_gpio_swing(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	int tune_level = 0;
	...
	{
		if (!tune_level) {
			gpio_set_value(ctrl->bl_on_gpio, 0);
			mdelay(3);
		} else {
			unsigned long irq_flags;
		       int pulse;

			 if(lcd_brightness<=tune_level){
				pulse = tune_level - lcd_brightness;
			}else{
				pulse = MAX_BRIGHTNESS_IN_BLU - (lcd_brightness - tune_level);
			}

			printk("[KTD3102] tune_level: %d, pulse : %d, lcd_brightness : %d \n",tune_level,pulse,lcd_brightness); 
...
#endif
int mdss_dsi_himax_panel_reset(struct mdss_panel_data *pdata, int enable)
{
...
#if defined(CONFIG_FB_MSM_MDSS_KTD3102_BACKLIGHT)
	case BL_GPIO_SWING:
		mdss_dsi_panel_bklt_gpio_swing(ctrl_pdata, mdss_dsi_set_brightness_level(bl_level));
		break;
#endif
	default:
