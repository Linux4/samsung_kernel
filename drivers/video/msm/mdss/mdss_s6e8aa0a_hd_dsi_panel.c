...
static int  aid_operation(int lux)
{
	...
	int no_change = 0;
		...
		printk("%s brightness_level : %d adi_status:%d\n", __func__,lux, aid_status);
...
static int set_gamma_level(int bl_level, unsigned char gamma_mode)
{
	...
	if (mpd.lcd_current_cd_idx == cd) {
		...
		printk("backlight is not change\n");
		return -1;
	} else
	...
	if (gamma_mode == GAMMA_SMART) {
		...
		printk("SD: %03d %s\n", gamma_lux, pBuffer);
		printk("bl_level:%d,cd:%d:Candela:%d aid_change:%d\n",bl_level, cd, gamma_lux, aid_change);
	}
	return aid_change;
}
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if (!msd.dstat.is_smart_dim_loaded) {
		...
		if (mpd.gamma_initial && boot_on == 0) {
			mpd.smart_s6e8aa0x01.brightness_level = 180;
			generate_gamma(&mpd.smart_s6e8aa0x01,
				&(mpd.gamma_initial[2]), GAMMA_SET_MAX);
