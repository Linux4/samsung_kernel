...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if(ctrl_pdata->panel_external_power) {
			ret = ctrl_pdata->panel_external_power(1);
			if (ret) {
				pr_err("%s: Panel external power failed. rc=%d\n",
						__func__, ret);
				goto error;
			}
		}
		...
		if(ctrl_pdata->panel_external_power) {
			ret = ctrl_pdata->panel_external_power(0);
			if (ret) {
				pr_err("%s: Panel external power failed. rc=%d\n",
						__func__, ret);
				goto error;
...
int mdss_dsi_ulps_config(struct mdss_dsi_ctrl_pdata *ctrl_pdata, int enable)
{
	...
	if (!mdss_dsi_ulps_feature_enabled(pdata)) {
		pr_info("%s: ULPS feature not supported. enable=%d\n",
			__func__, enable);
		return -ENOTSUPP;
	}
	...
	if (pinfo->cont_splash_enabled) {
		pr_info("%s: skip ULPS config with splash screen enabled\n",
			__func__);
		return 0;
	}
	...
	pr_info("%s: DSI lane status = 0x%08x. Ulps %s\n", __func__,
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_FB_REGISTERED:
		if (ctrl_pdata->registered) {
			pr_info("%s:event=%d, calling panel registered callback \n",
				 __func__, event);
