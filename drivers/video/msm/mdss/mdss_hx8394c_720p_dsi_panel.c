...
#if defined(CONFIG_CABC_TUNING_HX8394C)
#if defined(CONFIG_MACH_VASTALTE_CHN_CMCC_DUOS)
	if(msd.dstat.siop_status == 1 && msd.dstat.auto_brightness == 0){
#else
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
...
#if defined(CONFIG_CABC_TUNING_HX8394C)
#if defined(CONFIG_MACH_VASTALTE_CHN_CMCC_DUOS)
	if(first_boot != 1 && msd.dstat.siop_status == 1 && msd.dstat.auto_brightness == 0){
#else
	...
	if( gpio_is_valid ( lcd_blic_on ) )	{
		...
		rc = gpio_tlmm_config ( GPIO_CFG ( lcd_blic_on, 0,
			...
			GPIO_CFG_ENABLE );
		if (rc) {
			pr_err("%s : gpio config for lcd_blic_on failed. :ret=%d", __func__, rc);
		}
#endif
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
...
#if defined(CONFIG_MACH_VASTALTE_CHN_CMCC_DUOS) && defined(CONFIG_CABC_TUNING_HX8394C)
	struct mdss_panel_data *pdata = msd.mpd;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
#endif
	...
	if(msd.dstat.auto_brightness)
		mdss_dsi_panel_cmds_send(ctrl_pdata, &ctrl_pdata->cabc_off_cmds);
	else if(msd.dstat.auto_brightness == 0 && msd.dstat.siop_status)
		mdss_dsi_panel_cmds_send(ctrl_pdata, &ctrl_pdata->cabc_on_cmds);
#endif
