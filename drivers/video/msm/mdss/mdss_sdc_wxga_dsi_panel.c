...
#define START_BYTE       (0x18)
#define CONTROL_BYTE_GEN       (0x09)
...
static void GPIO_I2C_ACK(uint32_t delay)
{
	...
	/* SDA -> Input */
	gpio_tlmm_config(GPIO_CFG(gpio_i2c.sda, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
	...
	/* SDA -> Onput Low */
	gpio_tlmm_config(GPIO_CFG(gpio_i2c.sda, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
					GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	...
	if (ack)
		pr_err("BL I2C  -> No ACK\n");
}

void lp8556_i2c_init(void)
{
	gpio_tlmm_config(GPIO_CFG(gpio_i2c.scl, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
	...
	pr_err( "i2c_init: .scl:[%d]  .sda[%d]\n",gpio_i2c.scl,gpio_i2c.sda);
}
...
void lvds_i2c_burst_write_nonend(uint8_t *Data, uint8_t length)
{
	uint32_t i, j;
...
static int WriteRegister(u8 addr, u8 w_data)
{
	char buf[] = GEN_QL_CSR_WRITE;

	buf[0] = 0x58;
	buf[1] = addr;/* Address LS */
	buf[2] = w_data;/* Data*/

	lvds_i2c_burst_write((char *)(&buf[0]), (uint8_t)3);
	return 0;
	
}

void send_i2c_lp8556_data(void)
{
	int ret = 0;
	lp8556_i2c_init();
	ret = WriteRegister(0x01, 0x80);
	if (ret != 0)
		pr_err("[BL:ERROR]:bl_I2cWrite16 failed:01: %d\n",ret);
	ret = WriteRegister(0xA7, 0xFA);
	if (ret != 0)
		pr_err("[BL:ERROR]:bl_I2cWrite16 failed:A3: %d\n",ret);
	ret = WriteRegister(0xA3, 0x5E);
	if (ret != 0)
		pr_err("[BL:ERROR]:bl_I2cWrite16 failed:01: %d\n",ret);
	ret = WriteRegister(0xA5, 0x34);
	if (ret != 0)
		pr_err("[BL:ERROR]:bl_I2cWrite16 failed:A5: %d\n",ret);
	udelay(1000);
}
...
#endif
int mdss_dsi_sdc_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		int rc;
		mdelay(20);
		gpio_direction_output(ctrl_pdata->bl_sda, 0);
		mdelay(5);
		gpio_direction_output(ctrl_pdata->bl_scl, 0);
		mdelay(5);
		gpio_set_value_cansleep(ctrl_pdata->bl_on_gpio,(1<<1));//PMIC GPIO
		gpio_direction_output(ctrl_pdata->bl_rst_gpio, 1);
		...
		rc = gpio_tlmm_config(GPIO_CFG(ctrl_pdata->bl_ldi_en, 0,
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	is_negative_on();
#endif
	gpio_set_value(ctrl->bl_ap_pwm, 1);
#if defined(CONFIG_ESD_ERR_FG_RECOVERY)
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	ctrl = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
	gpio_set_value(ctrl->bl_ap_pwm, 0);
	gpio_direction_output(ctrl->bl_rst_gpio, 0);
	mdelay(2);
	gpio_set_value_cansleep(ctrl->bl_on_gpio,0);
#if defined(CONFIG_ESD_ERR_FG_RECOVERY)
	if (!err_fg_working) {
