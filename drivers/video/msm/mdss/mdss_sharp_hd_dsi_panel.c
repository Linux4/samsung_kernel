...
unsigned char mdss_dsi_panel_pwm_scaling(int level)
{
	...
	if (is_sharp_panel) {
		...
		if (level >= MAX_BRIGHTNESS_LEVEL)
			scaled_level  = BL_MAX_BRIGHTNESS_LEVEL_JDI;
		else if (level >= MID_BRIGHTNESS_LEVEL) {
			scaled_level  = (level - MID_BRIGHTNESS_LEVEL) *
			(BL_MAX_BRIGHTNESS_LEVEL_JDI - BL_MID_BRIGHTNESS_LEVEL_JDI) / (MAX_BRIGHTNESS_LEVEL-MID_BRIGHTNESS_LEVEL) + BL_MID_BRIGHTNESS_LEVEL_JDI;
		} else if (level >= DIM_BRIGHTNESS_LEVEL) {
			scaled_level  = (level - DIM_BRIGHTNESS_LEVEL) *
			(BL_MID_BRIGHTNESS_LEVEL_JDI - BL_DIM_BRIGHTNESS_LEVEL_JDI) / (MID_BRIGHTNESS_LEVEL-DIM_BRIGHTNESS_LEVEL) + BL_DIM_BRIGHTNESS_LEVEL_JDI;
		} else if (level >= LOW_BRIGHTNESS_LEVEL) {
			scaled_level  = (level - LOW_BRIGHTNESS_LEVEL) *
			(BL_DIM_BRIGHTNESS_LEVEL_JDI - BL_LOW_BRIGHTNESS_LEVEL_JDI) / (DIM_BRIGHTNESS_LEVEL-LOW_BRIGHTNESS_LEVEL) + BL_LOW_BRIGHTNESS_LEVEL_JDI;
		}  else{
			if(poweroff_charging == 1)
				scaled_level  = level*BL_LOW_BRIGHTNESS_LEVEL_JDI/LOW_BRIGHTNESS_LEVEL;
			else
				scaled_level  = BL_MIN_BRIGHTNESS_JDI;
		}
	}
#else
	if (level >= MAX_BRIGHTNESS_LEVEL)
...
#endif
int mdss_dsi_sharp_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (is_sharp_panel) {
		if (gpio_is_valid(ctrl_pdata->disp_supply_sda)) {
			gpio_set_value(ctrl_pdata->disp_supply_sda, 1);
		}
		if (gpio_is_valid(ctrl_pdata->disp_supply_scl)) {
			gpio_set_value(ctrl_pdata->disp_supply_scl, 1);
		}
