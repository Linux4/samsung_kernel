...
static unsigned int resume_timeout_ctrl = 0;
static unsigned int update_cnt_log = 0;
#endif
...
#if defined(CONFIG_FB_MSM_MIPI_HX8394C_PANEL)
		if(resume_timeout_ctrl)
		{
			resume_timeout_ctrl++;
			ret = sync_fence_wait(fences[i], (1 * MSEC_PER_SEC));
			if (ret == -ETIME) {
				update_cnt_log++;
				printk("# 1 sec for the first frame update_cnt_log=%d\n", update_cnt_log);
			}
			if(resume_timeout_ctrl == 3)
				resume_timeout_ctrl = 0;
		}
		else
		{
#endif
