 ...
 *
*/
#include "ss_dsi_smart_dimming_S6E8AA5X01_AMS604RM01.h"
...
static int vt_adjustment(struct SMART_DIM *pSmart)
{
	...
	LSB = char_to_int(pSmart->MTP.R_OFFSET.OFFSET_0);
	...
	LSB = char_to_int(pSmart->MTP.G_OFFSET.OFFSET_0);
	...
	LSB = char_to_int(pSmart->MTP.B_OFFSET.OFFSET_0);
...
static int hbm_interpolation_table[HBM_INTERPOLATION_STEP] = {378, 395, 413, 430, 448, 465, 483, 500};
static int hbm_gamma[33];
static int max_gamma[33];
static void hbm_interpolation_init(struct SMART_DIM *pSmart)
{
	...
	char *hbm_payload;
	//char log_buf[256];
	int hbm_interpolation_gamma[HBM_INTERPOLATION_STEP][33];
	int cnt;
	...
	for (i = 0; i < HBM_INTERPOLATION_STEP; i++) {
		rate = ((hbm_interpolation_table[i] - 360) * 4194304) / (500 - 360);
		for (j = 0; j < 30; j++)
...
static int smart_dimming_init(struct SMART_DIM *psmart)
{
	...
	psmart->vregout_voltage = VREG0_REF_6P3;
...
struct smartdim_conf *smart_get_conf_S6E8AA5X01_AMS604RM01(void) {
