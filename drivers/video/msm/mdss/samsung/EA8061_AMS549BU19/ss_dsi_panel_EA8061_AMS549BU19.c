 ...
 *
*/
#include "ss_dsi_panel_EA8061_AMS549BU19.h"
#include "ss_dsi_mdnie_EA8061_AMS549BU19.h"
...
static struct dsi_panel_cmds * mdss_elvss(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (id3 >= 0x04) {
		...
		if (vdd->temperature > 0)
			...
			vdd->dtsi_data[ctrl->ndx].gamma_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x94;
		/* mps */
		if (vdd->temperature > 0)
...
static int __init samsung_panel_init(void)
{
	...
	char panel_string[] = "ss_dsi_panel_EA8061_HD";
