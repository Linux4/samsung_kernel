...
enum {
	...
	REGTYPE_TCON_RDY, /* TCON ready pin controlled by display */
...
struct ssreg_rdata_tconrdy {
	int tconrdy_gpio;
	u32 gpio_tconrdy_flags;
};
...
struct ssreg_regulator_data {
	...
	union {
		...
		struct ssreg_rdata_tconrdy *r_tconrdy; /* REGTYPE_PANEL_RESET */
...
static int panelrst_enable(struct regulator_dev *rdev)
{
	...
	dev_info(&rdev->dev, "enable reset pin(%d), id=%d\n", r_panelrst->rst_gpio, id);
...
static int panelrst_disable(struct regulator_dev *rdev)
{
	...
	dev_info(&rdev->dev, "disable reset pin(%d), id=%d\n", r_panelrst->rst_gpio, id);
	WARN(rdata->reg_type != REGTYPE_PANEL_RESET,
			"reg_type=%d", rdata->reg_type);
...
/* wait for TCON READY signal from display */
static int tconrdy_enable(struct regulator_dev *rdev)
{
	struct ssreg_pmic *pmic = rdev_get_drvdata(rdev);
	struct ssreg_pdata *pdata = pmic->pdata;
	int id = rdev_get_id(rdev);
	...
	while (!gpio_get_value(r_tconrdy->tconrdy_gpio) && cnt++ < MAX_WAITTIME_TCONRDY)
		usleep_range(1000, 1000);

	if (cnt > MAX_WAITTIME_TCONRDY || cnt < MIN_WAITTIME_TCONRDY)
		dev_err(&rdev->dev, "err: tcon_rdy time=%d[ms], gpio=%d\n",
				cnt, r_tconrdy->tconrdy_gpio);

	dev_info(&rdev->dev, "tcon_rdy: %d[ms], gpio=%d, id=%d\n",
			cnt, r_tconrdy->tconrdy_gpio, id);

	return 0;
}

static int tconrdy_disable(struct regulator_dev *rdev)
{
	return 0;
}
...
static struct regulator_ops tconrdy_ops = {
	.enable		= tconrdy_enable,
	.disable	= tconrdy_disable,
	.is_enabled	= tconrdy_is_enabled,
};
...
static int parse_tconrdy(struct device *dev, struct device_node *np,
			struct ssreg_regulator_data *rdata)
{
	struct ssreg_rdata_tconrdy *r_tconrdy;
	int ret = 0;
	...
	r_tconrdy = devm_kzalloc(dev,
			sizeof(struct ssreg_rdata_tconrdy), GFP_KERNEL);
	if (!r_tconrdy)
		return -ENOMEM;

	r_tconrdy->tconrdy_gpio = of_get_named_gpio_flags(np, "tconrdy-gpio", 0,
				&r_tconrdy->gpio_tconrdy_flags);

	ret = gpio_request_one(r_tconrdy->tconrdy_gpio,
			r_tconrdy->gpio_tconrdy_flags, "tcon-rdy");
	if (ret) {
		dev_err(dev, "fail to request tconrdy gpio(%d, f=%x), ret=%d\n",
			r_tconrdy->tconrdy_gpio, r_tconrdy->gpio_tconrdy_flags, ret);
		return ret;
	}

	rdata->r_tconrdy = r_tconrdy;

	dev_info(dev, "tconrdy_gpio=%d, 0x%x, boot_on=%d\n",
			r_tconrdy->tconrdy_gpio,
			r_tconrdy->gpio_tconrdy_flags,
			rdata->initdata->constraints.boot_on);
...
static struct ssreg_pdata *ssreg_parse_dt(struct device *dev)
{
	...
	for_each_child_of_node(regulators_np, reg_np) {
		...
		else if (rdata->reg_type == REGTYPE_TCON_RDY)
			ret = parse_tconrdy(dev, reg_np, rdata);
