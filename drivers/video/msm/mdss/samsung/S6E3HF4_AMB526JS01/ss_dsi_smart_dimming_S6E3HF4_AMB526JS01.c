 ...
 *
*/
#include "ss_dsi_smart_dimming_S6E3HF4_AMB526JS01.h"
...
{
	if (id3 >= 0x04 || (mdss_get_hero2_fab_type() == S6E3HF4_AMB526JS01_A2_LINE) || hero2_a2_revM)
		return VREG0_REF_6P5;
	else
		return VREG0_REF_6P4;
}

static unsigned int get_base_luminance(struct SMART_DIM *pSmart)
{
	int cnt;
	...
	if (hero2) {
		if (mdss_get_hero2_fab_type() == S6E3HF4_AMB526JS01_A2_LINE ){
			memcpy(base_luminance, base_luminance_hero2_a2_line, sizeof(base_luminance_hero2_a2_line));
		} else {
			if (hero2_a2_revM)
				memcpy(base_luminance, base_luminance_hero2_revM, sizeof(base_luminance_hero2_revM));
			else if (id3 == 0x04 || id3 == 0x05 || id3 == 0x06)
				memcpy(base_luminance, base_luminance_hero2_revF, sizeof(base_luminance_hero2_revF));
			else if (id3 >= 0x07)
				memcpy(base_luminance, base_luminance_hero2_revL, sizeof(base_luminance_hero2_revL));
			else
				memcpy(base_luminance, base_luminance_hero2_revA, sizeof(base_luminance_hero2_revA));
		}
	} else {
		if (id3 >= 0x02)
...
static int get_gradation_offset(int table_index, int index)
{
	if (hero2) {
		if (mdss_get_hero2_fab_type() == S6E3HF4_AMB526JS01_A2_LINE ){
			return gradation_offset_hero2_a2_line[table_index][index];
		} else {
			if (hero2_a2_revM)
				return gradation_offset_hero2_revM[table_index][index];
			else if (id3 == 0x04 || id3 == 0x05 || id3 == 0x06)
				return gradation_offset_hero2_revF[table_index][index];
			else if (id3 >= 0x07)
				return gradation_offset_hero2_revL[table_index][index];
			else
				return gradation_offset_hero2_revA[table_index][index];
		}
	} else {
		if (id3 >= 0x02)
...
static int get_rgb_offset(int table_index, int index)
{
	if (hero2) {
		if (mdss_get_hero2_fab_type() == S6E3HF4_AMB526JS01_A2_LINE ){
			return rgb_offset_hero2_a2_line[table_index][index];
		} else {
			if (hero2_a2_revM)
				return rgb_offset_hero2_revM[table_index][index];
			else if (id3 == 0x04 || id3 == 0x05 || id3 == 0x06)
				return rgb_offset_hero2_revF[table_index][index];
			else if (id3 >= 0x07)
				return rgb_offset_hero2_revL[table_index][index];
			else
				return rgb_offset_hero2_revA[table_index][index];
		}
	} else {
		if (id3 >= 0x02)
...
static void TP_L_calc(struct SMART_DIM *pSmart, long long *L, unsigned int base_level)
{
...
static void set_project_type(void)
{
	hero2_a2_revM = 0;

	if (id2 & 0x10) {
		hero2 = 1;
		if (((id1 & 0x40) == 0x00) && (id3 == 0x08)) {
			hero2_a2_revM = 1;
			pr_err("%s : projcet is hero2 a2 revM\n", __func__);
		}
	} else
		hero = 1;

	pr_err("%s : projcet is %s\n", __func__, hero2 ? "hero2" : "hero");
}

void set_otherline_dimming_data(void)
{
	memcpy(base_luminance_hero2_a2_line, a2_line_data.base_luminance, sizeof(a2_line_data.base_luminance));
	memcpy(gradation_offset_hero2_a2_line, a2_line_data.gradation_offset, sizeof(a2_line_data.gradation_offset));
	memcpy(rgb_offset_hero2_a2_line, a2_line_data.rgb_offset, sizeof(a2_line_data.rgb_offset));

	memcpy(base_luminance_reverse_hmt_single_a2_line, a2_line_data.hmt_base_luminance, sizeof(a2_line_data.hmt_base_luminance));
	memcpy(gradation_offset_reverse_hmt_single_a2_line, a2_line_data.hmt_gradation_offset, sizeof(a2_line_data.hmt_gradation_offset));
	memcpy(rgb_offset_reverse_hmt_single_a2_line, a2_line_data.hmt_rgb_offset, sizeof(a2_line_data.hmt_rgb_offset));
...
static int smart_dimming_init(struct SMART_DIM *psmart)
{
	...
	if (id3 >= 0x02 || hero2)
		...
		memcpy(&(psmart->gen_table[65-1].gamma_setting),
				max_lux_table, GAMMA_SET_MAX);
...
static unsigned int get_base_luminance_hmt(int brightness_level) {

	int cnt;
	unsigned int base_luminance[HMT_LUMINANCE_MAX][2];

	if (mdss_get_hero2_fab_type() == S6E3HF4_AMB526JS01_A2_LINE)
		memcpy(base_luminance, base_luminance_reverse_hmt_single_a2_line, sizeof(base_luminance_reverse_hmt_single_a2_line));
	else {
		if (hero2_a2_revM)
			memcpy(base_luminance, base_luminance_reverse_hmt_single_revM, sizeof(base_luminance_reverse_hmt_single_revM));
		else if (id3 == 0x04 || id3 == 0x05 || id3 == 0x06)
			memcpy(base_luminance, base_luminance_reverse_hmt_single_revF, sizeof(base_luminance_reverse_hmt_single_revF));
		else if (id3 >= 0x07)
			memcpy(base_luminance, base_luminance_reverse_hmt_single_revL, sizeof(base_luminance_reverse_hmt_single_revL));
		else
...
static int get_gradation_offset_hmt(int table_index, int index) {
	...
	if (mdss_get_hero2_fab_type() == S6E3HF4_AMB526JS01_A2_LINE)
		gradation_offset = gradation_offset_reverse_hmt_single_a2_line[table_index][index];
	else{
		if (hero2_a2_revM)
			gradation_offset = gradation_offset_reverse_hmt_single_revM[table_index][index];
		else if (id3 == 0x04 || id3 == 0x05 || id3 == 0x06)
			gradation_offset = gradation_offset_reverse_hmt_single_revF[table_index][index];
		else if (id3 >= 0x07)
			gradation_offset = gradation_offset_reverse_hmt_single_revL[table_index][index];
		else
...
static int get_rgb_offset_hmt(int table_index, int index) {
	...
	if (mdss_get_hero2_fab_type() == S6E3HF4_AMB526JS01_A2_LINE)
		rgb_offset = rgb_offset_reverse_hmt_single_a2_line[table_index][index];
	else{
		if (hero2_a2_revM)
			rgb_offset = rgb_offset_reverse_hmt_single_revM[table_index][index];
		else if (id3 == 0x04 || id3 == 0x05 || id3 == 0x06)
			rgb_offset = rgb_offset_reverse_hmt_single_revF[table_index][index];
		else if (id3 >= 0x07)
			rgb_offset = rgb_offset_reverse_hmt_single_revL[table_index][index];
		else
...
static void TP_L_calc_hmt(struct SMART_DIM *pSmart, long long *L, unsigned int base_level)
{
	long long temp_cal_data = 0;
	int point_index;
	int cnt;
