...
void make_self_dispaly_img_cmds(enum mipi_samsung_tx_cmd_list cmd, u32 op)
{
	...
	/* cmd size */
	cmd_size = data_size / payload_size + 2;
	...
	for (i = 0; i < pcmds->cmd_cnt; i++) {
		...
		if (i == 0) {
			/*for test key enable*/
			tcmds[i].payload = kzalloc(3, GFP_KERNEL);
			if (tcmds[i].payload == NULL) {
				LCD_ERR("fail to kzalloc for self_mask cmds first payload: enable key\n");
				return;
			}
			tcmds[i].payload[0] = 0xF0;
			tcmds[i].payload[1] = 0x5A;
			tcmds[i].payload[2] = 0x5A;
			j = 3;
		} else if (i == (pcmds->cmd_cnt - 1)) {
			/*for test key disable*/
			tcmds[i].payload = kzalloc(3, GFP_KERNEL);
			if (tcmds[i].payload == NULL) {
				LCD_ERR("fail to kzalloc for self_mask cmds last payload: disable key\n");
				return;
			}
			tcmds[i].payload[0] = 0xF0;
			tcmds[i].payload[1] = 0xA5;
			tcmds[i].payload[2] = 0xA5;
			j = 3;
		} else {
		/* fill image data */
		...
		if (tcmds[i].payload == NULL) {
			...
			tcmds[i].payload[0] = (i == 1) ? 0x4C : 0x5C;
