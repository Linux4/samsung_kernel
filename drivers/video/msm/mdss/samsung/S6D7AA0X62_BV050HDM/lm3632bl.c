...
#ifdef CONFIG_MACH_J7_CHN_CMCC	
		if(vdd->manufacture_id_dsi[0]==0x0BC050)
		{
			msleep(3); //only J7008 CMCC 2nd. LCD (ILI9881C) is OK (because Timing issue)
		}
#endif		
		if (gpio_is_valid(info->pdata->gpio_backlight_panel_enn))
...
#ifdef CONFIG_MACH_J7_CHN_CMCC
		if(vdd->manufacture_id_dsi[0]==0x0BC050) //only J7008 CMCC 2nd. LCD (ILI9881C) is OK (because Bightness)
		{
			mdss_samsung_parse_backlight_settings(&pdev->dev,&info->bl_ic_outdoor_settings,"backlight-ic-tuning-outdoor-ili9881c");
		}
		else
#endif
