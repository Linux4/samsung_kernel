 ...
 *
*/
#include "ss_dsi_smart_dimming_S6E3HA5_AMB567MK01.h"
...
static int get_base_luminance(struct SMART_DIM *pSmart)
{
	...
	if (id3 == 0x80)
		memcpy(base_luminance, base_luminance_grace_revA, sizeof(base_luminance_grace_revA));
	else
		memcpy(base_luminance, base_luminance_grace_revA, sizeof(base_luminance_grace_revA));
...
{
	if (id3 == 0x80)
		return gradation_offset_grace_revA[table_index][index];
	else
		return gradation_offset_grace_revA[table_index][index];
}
...
{
	if (id3 == 0x80)
		return rgb_offset_grace_revA[table_index][index];
	else
		return rgb_offset_grace_revA[table_index][index];
}
