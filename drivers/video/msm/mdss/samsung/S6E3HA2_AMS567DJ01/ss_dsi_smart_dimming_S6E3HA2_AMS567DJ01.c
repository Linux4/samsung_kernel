...
static void gamma_init_revF(struct SMART_DIM *pSmart, char *str, int size)
{
	long long candela_level[TABLE_MAX] = {-1, };
	int bl_index[TABLE_MAX] = {-1, };
	int gamma_setting[GAMMA_SET_MAX];
...
//			if (searching_function(candela_level[cnt],
	...
	*/
	for (cnt = 1; cnt < TABLE_MAX; cnt++) {
		...
		bl_index[TABLE_MAX - cnt] +=
			gradation_offset_revF[table_index][cnt - 1];
	...
	*/
	for (cnt = 0; cnt < RGB_COMPENSATION; cnt++) {
		...
		if (cnt < 3) {
			level_V255 = gamma_setting[cnt * 2] << 8 | gamma_setting[(cnt * 2) + 1];
			level_V255 +=
				rgb_offset_revF[table_index][cnt];
			level_255_temp_MSB = level_V255 / 256;

			gamma_setting[cnt * 2] = level_255_temp_MSB & 0xff;
			gamma_setting[(cnt * 2) + 1] = level_V255 & 0xff;
		} else {
			gamma_setting[cnt+3] += rgb_offset_revF[table_index][cnt];
		}
	}
	/*subtration MTP_OFFSET value from generated gamma table*/
	mtp_offset_substraction(pSmart, gamma_setting);
...
static void gamma_init_revH(struct SMART_DIM *pSmart, char *str, int size)
{
	long long candela_level[TABLE_MAX] = {-1, };
	int bl_index[TABLE_MAX] = {-1, };
	int gamma_setting[GAMMA_SET_MAX];
	...
	*/
	for (cnt = 1; cnt < TABLE_MAX; cnt++) {
		...
		bl_index[TABLE_MAX - cnt] +=
			gradation_offset_revH[table_index][cnt - 1];
	...
	*/
	for (cnt = 0; cnt < RGB_COMPENSATION; cnt++) {
		...
		if (cnt < 3) {
			level_V255 = gamma_setting[cnt * 2] << 8 | gamma_setting[(cnt * 2) + 1];
			level_V255 +=
				rgb_offset_revH[table_index][cnt];
			level_255_temp_MSB = level_V255 / 256;

			gamma_setting[cnt * 2] = level_255_temp_MSB & 0xff;
			gamma_setting[(cnt * 2) + 1] = level_V255 & 0xff;
		} else {
			gamma_setting[cnt+3] += rgb_offset_revH[table_index][cnt];
		}
	}
	/*subtration MTP_OFFSET value from generated gamma table*/
	mtp_offset_substraction(pSmart, gamma_setting);
...
static void gamma_init_revC(struct SMART_DIM *pSmart, char *str, int size)
{
	long long candela_level[TABLE_MAX] = {-1, };
	int bl_index[TABLE_MAX] = {-1, };
	int gamma_setting[GAMMA_SET_MAX];
...
#if defined(AID_OPERATION)
		if(id3 == PANEL_REVC || id3 == PANEL_REVD) {
			gamma_init_revC(psmart,
				(char *)(&(psmart->gen_table[lux_loop].gamma_setting)),
				GAMMA_SET_MAX);
		} else if (id3 == PANEL_REVF || id3 == PANEL_REVG) {
			gamma_init_revF(psmart,
			(char *)(&(psmart->gen_table[lux_loop].gamma_setting)),
			GAMMA_SET_MAX);
		} else if (id3 == PANEL_REVH || id3 == PANEL_REVI || id3 == PANEL_REVJ) {
			gamma_init_revH(psmart,
			(char *)(&(psmart->gen_table[lux_loop].gamma_setting)),
			GAMMA_SET_MAX);
		} else {
...
static void set_150cd_lux_table(char *para, int size)
{
	int i;
