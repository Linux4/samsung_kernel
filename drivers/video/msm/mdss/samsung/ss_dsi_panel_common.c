			...
			struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	/*TDDI from Synaptic requir reset signal should low after vsp line of DCDC during sleep*/
	vdd->dtsi_data[ctrl->ndx].samsung_power_off_reset_after_vsp = of_property_read_bool(np, "samsung,mdss-power-off-reset-after-vsp");
	LCD_ERR("samsung,mdss-power-off-reset-after-vsp  %s\n",
		vdd->dtsi_data[ctrl->ndx].samsung_power_off_reset_after_vsp ? "enabled" : "disabled");
