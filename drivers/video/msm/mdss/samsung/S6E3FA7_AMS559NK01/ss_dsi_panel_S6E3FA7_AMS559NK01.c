...
#define GRADUAL_ACL_STEP_JP1 6 //refer config_display.xml
static struct dsi_panel_cmds *mdss_acl_on(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	struct samsung_display_driver_data *vdd = check_valid_ctrl(ctrl);

	u8 acl_value[GRADUAL_ACL_STEP_JP1] = {0x00, 0x0F, 0x1F, 0x2E, 0x3E, 0x4D};
	int step;
	...
	*level_key = LEVEL1_KEY;
	...
	if (vdd->gradual_acl_val
			&& vdd->gradual_acl_val < GRADUAL_ACL_STEP_JP1)
		step = vdd->gradual_acl_val;
	else
		step = GRADUAL_ACL_STEP_JP1 - 1; /* default value */
