 ...
 *
*/
#include "ss_dsi_smart_dimming_S6E3HA3_AMS567JA01.h"
...
static void print_RGB_offset(struct SMART_DIM *pSmart)
{
	...
	for (i=0; i<V_MAX; i++) {
		...
		/* To escape build error */
		pr_debug("%s MTP_OFFSET %4s %3d %3d %3d \n", __func__, V_LIST_HMT[i],
				pSmart->MTP[i][R],
				pSmart->MTP[i][G],
				pSmart->MTP[i][B]);
	}
...
static void TP_gamma_voltage_calc_hmt(struct SMART_DIM *pSmart)
{
	int i,j;
	int OutputGamma;
	int MTP_OFFSET;
	...
	for (j = 0; j < RGB_MAX; j++) {
		MTP_OFFSET = pSmart->MTP[VT][j];
		fraction_hmt[VT_HMT][0] = vt_coefficient[MTP_OFFSET];
		pSmart->RGB_OUTPUT_HMT[VT_HMT][j] =
			v255_TP_gamma_voltage_calc(pSmart->vregout_voltage,
						0,
						fraction_hmt[VT_HMT]);
	}
	...
	for (j = 0; j < RGB_MAX; j++) {
		MTP_OFFSET = pSmart->MTP[V255][j];
		OutputGamma = MTP_OFFSET + center_gamma[V255][j];
		pSmart->RGB_OUTPUT_HMT[V255_HMT][j] =
			v255_TP_gamma_voltage_calc(pSmart->vregout_voltage,
						OutputGamma,
						fraction_hmt[V255_HMT]);
	}

	/* V203 ~ V11 */
	for (i = V203_HMT; i >= V11_HMT; i--) {
		for (j = 0; j < RGB_MAX; j++) {
			MTP_OFFSET = pSmart->MTP[i - 1][j];
			OutputGamma = MTP_OFFSET + center_gamma[i - 1][j];
			pSmart->RGB_OUTPUT_HMT[i][j] =
				other_TP_gamma_voltage_calc(pSmart->RGB_OUTPUT_HMT[VT_HMT][j],
						pSmart->RGB_OUTPUT_HMT[i + 1][j],
						OutputGamma,
						fraction_hmt[i]);
		}
	}
...
#ifdef SMART_DIMMING_DEBUG
	for (i = 0; i < V_MAX_HMT; i++) {
		pr_err("%16s %4s %d %d %d\n", __func__,V_LIST_HMT[i],
				pSmart->RGB_OUTPUT_HMT[i][R],
				pSmart->RGB_OUTPUT_HMT[i][G],
				pSmart->RGB_OUTPUT_HMT[i][B]);
	}
#endif
...
// 3. V3 : (VREG1 - V_TP)* denominator_TP /(VREG1 - V_nextTP) - numerator_TP
...
static void TP_gamma_code_calc_hmt(struct SMART_DIM *pSmart, int* M_GRAY, int *str)
{
	unsigned long long val;
	int TP, nextTP;
	int i,j;
	...
	for (i = V203; i >= V11; i--) {
		for (j = 0; j < RGB_MAX; j++) {
			TP = M_GRAY[i];
			nextTP = M_GRAY[i+1];
			val = other_TP_gamma_code_calc(pSmart->RGB_OUTPUT_HMT[VT_HMT][j],
							pSmart->GRAY[TP][j],
							pSmart->GRAY[nextTP][j],
							fraction[i]);
			str[cnt++] = val;
...
static int generate_gray_scale(struct SMART_DIM *pSmart)
{
	...
	int den = 0;
	/*
		GRAY OUTPUT VOLTAGE of TP's (V0,V3,V11,V23,V35,V51,V87,V151,V203,V255)
		(V0 is VREG1)
	*/
	for (i = 0; i < RGB_MAX; i++)
...
static int generate_gray_scale_hmt(struct SMART_DIM *pSmart)
{
	int i, V_idx = 0;
	int cnt = 0, cal_cnt = 0;
	int den = 0;
	...
	*/
	for (i = 0; i < RGB_MAX; i++)
		pSmart->GRAY[0][i] = pSmart->RGB_OUTPUT_HMT[V0_HMT][i];

	for (cnt = 1; cnt < V_MAX; cnt++) {
		for (i = 0; i < RGB_MAX; i++) {
			pSmart->GRAY[INFLECTION_VOLTAGE_ARRAY[cnt]][i] =
				pSmart->RGB_OUTPUT_HMT[cnt + 1][i];
		}
	}
...
static unsigned int get_base_luminance(struct SMART_DIM *pSmart)
{
	...
	if (panel_rev >= S6E3HA3_AMS567JA01_REV_C)
...
static void hbm_interpolation_init(struct SMART_DIM *pSmart)
{
	...
	/* Set V0 & VT RGB gamma value to 0 */
	for (loop = 0; loop < HBM_INTERPOLATION_STEP; loop++) {
		pSmart->hbm_interpolation_table[loop].gamma_setting[GAMMA_SET_MAX - 1] = 0; /*VT B*/
		pSmart->hbm_interpolation_table[loop].gamma_setting[GAMMA_SET_MAX - 2] = 0; /*VT R/G*/

		pSmart->hbm_interpolation_table[loop].gamma_setting[GAMMA_SET_MAX - 3] = 0; /*V0 B*/
		pSmart->hbm_interpolation_table[loop].gamma_setting[GAMMA_SET_MAX - 4] = 0; /*V0 G*/
		pSmart->hbm_interpolation_table[loop].gamma_setting[GAMMA_SET_MAX - 5] = 0; /*V0 R*/
	}
...
static int smart_dimming_init(struct SMART_DIM *psmart)
{
	...
	panel_rev = psmart->ldi_revision & 0x0F;
	...
	/* Set V0 & VT RGB gamma value to 0 */
	for (lux_loop = 0; lux_loop < LUMINANCE_MAX; lux_loop++) {
		psmart->gen_table[lux_loop].gamma_setting[GAMMA_SET_MAX - 1] = 0; /*VT B*/
		psmart->gen_table[lux_loop].gamma_setting[GAMMA_SET_MAX - 2] = 0; /*VT R/G*/

		psmart->gen_table[lux_loop].gamma_setting[GAMMA_SET_MAX - 3] = 0; /*V0 B*/
		psmart->gen_table[lux_loop].gamma_setting[GAMMA_SET_MAX - 4] = 0; /*V0 G*/
		psmart->gen_table[lux_loop].gamma_setting[GAMMA_SET_MAX - 5] = 0; /*V0 R*/
	}
...
static unsigned int get_base_luminance_hmt(int brightness_level) {
	...
	if (panel_rev >= S6E3HA3_AMS567JA01_REV_C)
		memcpy(base_luminance, base_luminance_reverse_hmt_single_revC, sizeof(base_luminance_reverse_hmt_single_revC));
	else
...
static int get_gradation_offset_hmt(int table_index, int index) {
	...
	if (panel_rev >= S6E3HA3_AMS567JA01_REV_C)
		gradation_offset = gradation_offset_reverse_hmt_single_revC[table_index][index];
	else
		gradation_offset = gradation_offset_reverse_hmt_single_revA[table_index][index];
...
static int get_rgb_offset_hmt(int table_index, int index) {
	...
	if (panel_rev >= S6E3HA3_AMS567JA01_REV_C)
		rgb_offset = rgb_offset_reverse_hmt_single_revC[table_index][index];
	else
		rgb_offset = rgb_offset_reverse_hmt_single_revA[table_index][index];
...
static void gamma_init_hmt(struct SMART_DIM *pSmart, char *str, int size, int table_index)
{
	...
	int i;
#endif
	long long L[V_MAX_HMT] = {0, };
	int M_GRAY[V_MAX_HMT] = {0, };
	int gamma_setting[GAMMA_SET_MAX];
	unsigned int base_level = 0;
	int cnt;
	...
	/* 3.7.1 - Generate Gamma code */
	TP_gamma_code_calc_hmt(pSmart, M_GRAY, gamma_setting);
...
static int smart_dimming_init_hmt(struct SMART_DIM *psmart)
{
	...
	pr_err("%s : ++ id3(%d) rev(%d)\n",__func__, id3, panel_rev);
	...
	if (panel_rev >= S6E3HA3_AMS567JA01_REV_C)
		psmart->vregout_voltage = VREG0_REF_6P6;
	else
		psmart->vregout_voltage = VREG0_REF_6P4;
	...
	/* 3. other VT : VT - (VT - V_nextTP) * (OutputGamma + numerator_TP)/(denominator_TP)		*/
	/********************************************************************************************/
	TP_gamma_voltage_calc_hmt(psmart);

	/* 2.3.5 D - Gray Output Voltage */
	if (generate_gray_scale_hmt(psmart)) {
		pr_err(KERN_ERR "lcd smart dimming fail generate_gray_scale\n");
		return -EINVAL;
	}
	...
	/* Set V0 & VT RGB gamma value to 0 */
	for (lux_loop = 0; lux_loop < psmart->lux_table_max; lux_loop++) {
		psmart->hmt_gen_table[lux_loop].gamma_setting[GAMMA_SET_MAX - 1] = 0; /*VT B*/
		psmart->hmt_gen_table[lux_loop].gamma_setting[GAMMA_SET_MAX - 2] = 0; /*VT R/G*/

		psmart->hmt_gen_table[lux_loop].gamma_setting[GAMMA_SET_MAX - 3] = 0; /*V0 B*/
		psmart->hmt_gen_table[lux_loop].gamma_setting[GAMMA_SET_MAX - 4] = 0; /*V0 G*/
		psmart->hmt_gen_table[lux_loop].gamma_setting[GAMMA_SET_MAX - 5] = 0; /*V0 R*/
	}
