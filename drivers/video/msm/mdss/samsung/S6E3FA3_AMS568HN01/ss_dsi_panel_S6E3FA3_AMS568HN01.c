 ...
 *
*/
#include "ss_dsi_panel_S6E3FA3_AMS568HN01.h"
#include "ss_dsi_mdnie_S6E3FA3_AMS568HN01.h"
...
static struct dsi_panel_cmds * mdss_elvss_temperature2(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	...
	if (vdd->temperature > 0)
		...
		if (candela_value == 2)
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
			= vdd->display_ststus_dsi[ctrl->ndx].elvss_value + 0x04;
		else if (candela_value == 3)
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
			= vdd->display_ststus_dsi[ctrl->ndx].elvss_value + 0x03;
		else if (candela_value == 4)
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
			= vdd->display_ststus_dsi[ctrl->ndx].elvss_value + 0x02;
		else if (candela_value == 5)
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
			= vdd->display_ststus_dsi[ctrl->ndx].elvss_value + 0x01;
		else
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
			= vdd->display_ststus_dsi[ctrl->ndx].elvss_value;
	} else {
...
static void mdss_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_get_conf = smart_get_conf_S6E3FA3_AMS568HN01;
...
static int __init samsung_panel_init(void)
{
	...
	char panel_string[] = "ss_dsi_panel_S6E3FA3_AMS568HN01_FHD";
