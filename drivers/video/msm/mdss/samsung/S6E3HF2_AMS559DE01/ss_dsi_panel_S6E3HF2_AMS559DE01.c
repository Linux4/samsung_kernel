 ...
 *
*/
#include "ss_dsi_panel_S6E3HF2_AMS559DE01.h"
#include "ss_dsi_mdnie_S6E3HF2_AMS559DE01.h"
...
static struct dsi_panel_cmds * mdss_elvss_temperature2(struct mdss_dsi_ctrl_pdata *ctrl)
{
	struct samsung_display_driver_data *vdd = check_valid_ctrl(ctrl);
...
static int mdss_samsung_change_ldi_fps(struct mdss_dsi_ctrl_pdata *ctrl, unsigned int input_fps)
{
	...
	pr_info("%s :current comp value(0x%x),offset(%d)\n", __func__,
		vdd->dtsi_data[ctrl->ndx].ldi_fps_change_tx_cmds[vdd->panel_revision].cmds[1].payload[3], offset);

	dest_cal_val = vdd->dtsi_data[ctrl->ndx].ldi_fps_change_tx_cmds[vdd->panel_revision].cmds[1].payload[3] + offset;
	...
	vdd->dtsi_data[ctrl->ndx].ldi_fps_change_tx_cmds[vdd->panel_revision].cmds[1].payload[3] = dest_cal_val;
...
static int init_ldi_fps(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	vdd->dtsi_data[ctrl->ndx].ldi_fps_change_tx_cmds[vdd->panel_revision].cmds[1].payload[3] = 0xC5;

	pr_err("%s : (0x%x)\n", __func__, vdd->dtsi_data[ctrl->ndx].ldi_fps_change_tx_cmds[vdd->panel_revision].cmds[1].payload[3]);
...
static void  mdss_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_get_conf = smart_get_conf_S6E3HF2_AMS559DE01;
...
static int __init samsung_panel_init(void)
{
	...
	char panel_string[] = "ss_dsi_panel_S6E3HF2_AMS559DE01_WQXGA";
