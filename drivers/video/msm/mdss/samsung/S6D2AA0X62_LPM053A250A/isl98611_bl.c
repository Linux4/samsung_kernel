/*
 * ISL98611-backlight.c - Platform data for backlight driver
 *
...
#include "ss_dsi_panel_S6D7AA0X62_hd_video.h"
...
void mdss_backlight_ic_power_on(int enable)
{
	...
	if(enable) {
		...
		/*for (i = 0; i <info->bl_ic_settings.table_length;i=i+2)
			isl98611_write_byte(info->isl98611, info->bl_ic_settings.table[i], info->bl_ic_settings.table[i+1]);*/
		/* In case of PBA booting, turn bl_out off */
		/*if (!mdss_panel_attach_get(vdd->ctrl_dsi[DISPLAY_1]))
			isl98611_write_byte(info->isl98611, 0x0A, 0x00);*/
...
void pwm_backlight_control_i2c(int scaled_level)
{
   ...
   /* Following taken from isl98611_bl_set_brightness provided by TI for i2c based control*/
	data = scaled_level & LM3632_BRT_LSB_MASK;
	ret = isl98611_update_bits(info->isl98611, info->bl_control.table[0],
	...
	isl98611_write_byte(info->isl98611, info->bl_control.table[2], data);
}
...
static void pwm_backlight_outdoor_control(int enable)
{
	...
	if(enable) {
		for (i = 0; i <info->bl_ic_outdoor_settings.table_length;i=i+2)
			isl98611_write_byte(info->isl98611, info->bl_ic_outdoor_settings.table[i], info->bl_ic_outdoor_settings.table[i+1]);
	} else {
		for (i = 0; i <info->bl_ic_normal_settings.table_length;i=i+2)
			isl98611_write_byte(info->isl98611, info->bl_ic_normal_settings.table[i], info->bl_ic_normal_settings.table[i+1]);
	}
}
#endif
...
int backlight_i2c_read_sys(u8 reg, u8* val)
{
	int err = 0;
	struct backlight_info *info = bl_info;
    err = isl98611_read_byte(info->isl98611, reg, val);
	return err;
}
int backlight_i2c_write_sys(u8 reg,  u8 val)
{
	int err = 0;
	struct backlight_info *info = bl_info;
    err = isl98611_write_byte(info->isl98611, reg, val);
	return err;
}
...
static int isl98611_bl_probe(struct platform_device *pdev)
{
	struct isl98611 *isl98611 = dev_get_drvdata(pdev->dev.parent);
	struct isl98611_backlight_platform_data *pdata = isl98611->pdata->bl_pdata;
	struct isl98611_bl *isl98611_bl;
	...
	isl98611_bl = devm_kzalloc(&pdev->dev, sizeof(*isl98611_bl), GFP_KERNEL);
	if (!isl98611_bl)
		return -ENOMEM;

	isl98611_bl->dev = &pdev->dev;
	isl98611_bl->isl98611 = isl98611;
	isl98611_bl->pdata = pdata;

	if (!isl98611_bl->pdata) {
		pdata = devm_kzalloc(&pdev->dev,
			sizeof(struct isl98611_backlight_platform_data), GFP_KERNEL);
		if (!pdata) {
...
static int isl98611_bl_remove(struct platform_device *pdev)
{
	return 0;
}

#ifdef CONFIG_OF
static const struct of_device_id isl98611_bl_of_match[] = {
	{ .compatible = "intersil,isl98611-backlight", },
	{ }
};
MODULE_DEVICE_TABLE(of, isl98611_bl_of_match);
#endif

static struct platform_driver isl98611_bl_driver = {
	.probe = isl98611_bl_probe,
	.remove = isl98611_bl_remove,
	.driver = {
		.name = "isl98611-backlight",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(isl98611_bl_of_match),
	},
};
module_platform_driver(isl98611_bl_driver);

MODULE_DESCRIPTION("Intersil ISL98611 Backlight Driver");
MODULE_AUTHOR("ravikant.s2@samsung.com");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:isl98611-backlight");
