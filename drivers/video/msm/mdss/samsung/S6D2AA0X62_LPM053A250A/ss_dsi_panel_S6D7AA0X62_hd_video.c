...
#include "ss_dsi_mdnie_S6D7AA0X62_hd_video.h"
...
static void backlight_tft_late_on(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if(vdd->auto_brightness==6 && vdd->panel_func.samsung_bl_ic_outdoor)
...
static void dsi_update_mdnie_data(void)
{
	...
	mdnie_data.DSI0_GRAYSCALE_MDNIE = NULL;
	mdnie_data.DSI0_GRAYSCALE_NEGATIVE_MDNIE = NULL;
...
static void mdss_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->support_panel_max = S6D7AA0X62_SUPPORT_PANEL_COUNT;
...
static int __init samsung_panel_init(void)
{
	...
	char panel_string[] = "qcom,mdss_dsi_s6d2aa0x_hd_video";

	pr_info("%s : mdss_mdp_panel = %s\n", __func__, mdss_mdp_panel);
	...
	if (!strncmp(vdd->panel_name, panel_string, strlen(panel_string)))
		...
		pr_info("%s : %s  %s\n", __func__, vdd->panel_name , panel_string);		
