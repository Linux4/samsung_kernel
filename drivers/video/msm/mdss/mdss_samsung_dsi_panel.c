...
#define DT_CMD_HDR 6
	...
	|| defined(CONFIG_FB_MSM_MIPI_EA8061V_PANEL)
#define HBM_RE
#endif
...
static struct dsi_cmd elvss_temperature_cmds;
#endif
static struct dsi_cmd elvss_caps_setting_cmds;
/*
...
static struct  panel_hrev panel_supp_cdp[]= {
	...
	{"Samsung EA8061 HD video mode panel",PANEL_EA8061V_HD_VIDEO},
	{NULL}
};
...
static struct dsi_cmd get_elvss_temprature_set(void)
{
	...
	if (msd.dstat.temper_need_update) {
		...
		elvss_temperature_cmds.cmd_desc[1].payload[1] =
				msd.dstat.temperature_value;
	}
	elvss_temperature.cmd_desc = &(elvss_temperature_cmds.cmd_desc[0]);
	elvss_temperature.num_of_cmds = elvss_temperature_cmds.num_of_cmds;

	return elvss_temperature;
}
#endif
static struct dsi_cmd get_elvss_caps_set(void)
{
	struct dsi_cmd elvss_caps = {0, };

	if (msd.dstat.acl_on) {
		elvss_caps_setting_cmds.cmd_desc[0].payload[1] = 0x4c;
		elvss_caps_setting_cmds.cmd_desc[1].payload[1] = 0x29;
	} else {
		elvss_caps_setting_cmds.cmd_desc[0].payload[1] = 0x5c;
		elvss_caps_setting_cmds.cmd_desc[1].payload[1] = 0x21;
	}
	elvss_caps.cmd_desc = &(elvss_caps_setting_cmds.cmd_desc[0]);
	elvss_caps.num_of_cmds = elvss_caps_setting_cmds.num_of_cmds;
...
static struct dsi_cmd get_elvss_control_set(int cd_idx)
{
	...
	/* Get the command desc */
	if (msd.dstat.acl_on)
		elvss_cmds_list.cmd_desc[cmd_idx].payload[1] = 0x4c;
	else
...
static int make_brightcontrol_set(int bl_level)
{
...
#endif
	struct dsi_cmd elvss_caps_set = {0, 0, 0, 0, 0};
	...
	elvss_caps_set = get_elvss_caps_set();
	cmd_count = update_bright_packet(cmd_count, &elvss_caps_set);
	/* acl */
...
static int mdss_dsi_panel_dimming_init(struct mdss_panel_data *pdata)
{
...
#else
	memcpy(&hbm_etc_cmds_list.cmd_desc[1].payload[17], hbm_buffer+6, 1);
	mipi_samsung_read_nv_mem(pdata, &nv_mtp_hbm2_read_cmds, hbm_buffer);
	...
	memcpy(&hbm_etc_cmds_list.cmd_desc[1].payload[2], hbm_buffer+2, 15);
	memcpy(&hbm_off_cmds.cmd_desc[0].payload[2], hbm_buffer+2, 16);
...
#if 0 //defined(CONFIG_MDNIE_LITE_TUNING)
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	MIPI_OUTP(ctrl->ctrl_base + 0x3C, 0x14000000);
#endif
...
#if defined(TEMPERATURE_ELVSS)
	mdss_samsung_parse_panel_cmd(np, &elvss_temperature_cmds,
		"samsung,panel-elvss-cmds-tempcompen-list");
#endif
	mdss_samsung_parse_panel_cmd(np, &elvss_caps_setting_cmds,
		"samsung,panel-elvss-cmds-caps-setting-list");
