...
#endif
int mdss_dsi_boe_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		rc = gpio_tlmm_config(GPIO_CFG(ctrl_pdata->disp_en_gpio_3v, 0,
					...
					GPIO_CFG_ENABLE);
		if (rc)
		pr_err("enabling disp_en_gpio_3v failed, rc=%d\n",rc);
		...
		mdelay(20);
		gpio_set_value(ctrl_pdata->disp_en_gpio_3v, 1);
		mdelay(5);
		...
		if (gpio_is_valid(ctrl_pdata->disp_en_gpio_3v))
			gpio_set_value((ctrl_pdata->disp_en_gpio_3v), 1);
		...
		rc = gpio_tlmm_config(GPIO_CFG(ctrl_pdata->disp_en_gpio_3v, 0,
					...
					GPIO_CFG_ENABLE);
		if (rc)
		pr_err("disabling disp_en_gpio_3v failed, rc=%d\n",rc);
		rc = gpio_tlmm_config(GPIO_CFG(ctrl_pdata->rst_gpio, 0,
					GPIO_CFG_OUTPUT,GPIO_CFG_PULL_DOWN,GPIO_CFG_2MA),
					GPIO_CFG_ENABLE);
		if (rc)
		...
		gpio_set_value(ctrl_pdata->disp_en_gpio_3v, 0);
...
static ssize_t mdss_boe_disp_get_power(struct device *dev,
			struct device_attribute *attr, char *buf)
{
...
static ssize_t mdss_boe_disp_set_power(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned int power;
...
static DEVICE_ATTR(lcd_power, S_IRUGO | S_IWUSR | S_IWGRP,
			mdss_boe_disp_get_power,
			mdss_boe_disp_set_power);
...
static struct lcd_ops mdss_boe_disp_props = {
...
static ssize_t mdss_boe_auto_brightness_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	int rc;
...
static ssize_t mdss_boe_auto_brightness_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
...
static DEVICE_ATTR(auto_brightness, S_IRUGO | S_IWUSR | S_IWGRP,
			mdss_boe_auto_brightness_show,
			mdss_boe_auto_brightness_store);
	...
	bool cmd_cfg_cont_splash)
{
	...
	ctrl_pdata->panel_reset = mdss_dsi_boe_panel_reset;
