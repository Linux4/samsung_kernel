		 ...
		 */
			...
			if(pipe->play_cnt == 0) {
				pr_info("1: play_cnt(%d), num(%d), ndx(%d), pre_csc_value0(%d), pre_csc_value1(%d), csc_update(%d), mixer_num(%d)\n",
						pipe->play_cnt, pipe->num, pipe->ndx,pre_csc_value[0], pre_csc_value[1], csc_update, mixer_num);
				mdss_mdp_csc_setup(MDSS_MDP_BLOCK_SSPP, pipe->num, 1,
										   MDSS_MDP_CSC_YUV2RGB);
				pre_csc_value[mixer_num] = csc_update;
			} else if ((pre_csc_value[mixer_num] != csc_update) && (csc_update != 2)) {
				pr_info("2: play_cnt(%d), num(%d), ndx(%d), pre_csc_value0(%d), pre_csc_value1(%d), csc_update(%d), mixer_num(%d)\n",
						pipe->play_cnt, pipe->num, pipe->ndx,pre_csc_value[0], pre_csc_value[1], csc_update, mixer_num);	
				mdss_mdp_csc_setup(MDSS_MDP_BLOCK_SSPP, pipe->num, 1,
										   MDSS_MDP_CSC_YUV2RGB);
				pre_csc_value[mixer_num] = csc_update;
