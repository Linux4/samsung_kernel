...
#define CONFIG_ESD_ERR_FG_RECOVERY
...
int mdss_nt35510_panel_reset(struct mdss_panel_data *pdata, int enable)
{
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (!gpio_is_valid(msd.lcd_esd_det_gpio)) {
		...
		if (rc) {
			pr_err("request lcd_esd_det gpio failed, rc=%d\n",
				rc);
	...
	if (!gpio_is_valid(msd.lcd_flm_gpio)) {
		...
		rc = gpio_request(msd.lcd_flm_gpio, "lcd_esd_det");
		if (rc) {
	...
	bool cmd_cfg_cont_splash)
{
	...
	ctrl_pdata->panel_reset = mdss_nt35510_panel_reset;
	...
	err_fg_gpio = gpio_to_irq(msd.lcd_esd_det_gpio);
