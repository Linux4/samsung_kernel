...
static char ACL_COND_SET_20[] = {
	...
	0x00, 0x01, 0x03, 0x08, 0x0C,
	0x0E, 0x12, 0x15, 0x16, 0x17,
	0x18, 0x19
};
...
static char ACL_COND_SET_33[] = {
	...
	0x01, 0xDF, 0x00, 0x00, 0x03,
	0x1F, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x01, 0x05, 0x10, 0x16,
	0x1E, 0x20, 0x22, 0x26, 0x28,
	0x2A, 0x2C
};
...
static char MTP_enable1[] = {
	0xF0,
	0x5A, 0x5A
};

static char MTP_enable2[] = {
	0xFC,
	0x5A, 0x5A
};

static struct dsi_cmd_desc samsung_panel_MTP_cmds[] = {
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 0,
	sizeof(MTP_enable1)}, MTP_enable1},
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 0,
	sizeof(MTP_enable2)}, MTP_enable2},
};
...
static struct dsi_cmd_desc DSI_CMD_ACL_20 = {
	{DTYPE_DCS_LWRITE, 1, 0, 0, 0,
	sizeof(ACL_COND_SET_20)}, ACL_COND_SET_20 };
static struct dsi_cmd_desc DSI_CMD_ACL_33 = {
	{DTYPE_DCS_LWRITE, 1, 0, 0, 0,
	sizeof(ACL_COND_SET_33)}, ACL_COND_SET_33 };
...
static int get_candela_index(int bl_level)
{
	...
	case 60 ... 69:
		...
		break;
	case 70 ... 79:
		backlightlevel = GAMMA_90CD; /* 7*/
		break;
	case 80 ... 89:
		backlightlevel = GAMMA_100CD; /* 8*/
		break;
	case 90 ... 99:
		backlightlevel = GAMMA_110CD; /* 9 */
		break;
	case 100 ... 109:
		backlightlevel = GAMMA_120CD; /* 10*/
		break;
	case 110 ... 119:
	...
	case 120 ... 129:
		...
		break;
	case 130 ... 139:
		backlightlevel = GAMMA_150CD; /* 13 */
		break;
	case 140 ... 149:
		backlightlevel = GAMMA_160CD; /* 14 */
		break;
	case 150 ... 159:
		backlightlevel = GAMMA_170CD; /* 15 */
		break;
	case 160 ... 169:
		backlightlevel = GAMMA_180CD; /* 16 */
		break;
	case 170 ... 179:
		backlightlevel = GAMMA_190CD; /* 17 */
		break;
	case 180 ... 189:
		backlightlevel = GAMMA_200CD; /* 18*/
		break;
	case 190 ... 199:
		backlightlevel = GAMMA_210CD; /* 19 */
		break;
	case 200 ... 209:
		backlightlevel = GAMMA_220CD; /* 20*/
		break;
	case 210 ... 219:
		backlightlevel = GAMMA_230CD; /* 21*/
		break;
	case 220 ... 224:
		backlightlevel = GAMMA_240CD; /* 22 */
		break;
	case 225 ... 229:
		backlightlevel = GAMMA_250CD; /* 23 */
		break;
	case 230 ... 239:
		backlightlevel = GAMMA_270CD; /* 24*/
		break;
	case 240 ... 249:
		backlightlevel = GAMMA_280CD; /* 25*/
		break;
	case 250 ... 254:
		backlightlevel = GAMMA_290CD; /* 26*/
		break;
	case 255:
		backlightlevel = GAMMA_300CD; /* 28 */
		break;
	default:
		backlightlevel = GAMMA_40CD; /* 2 */
		break;
...
static int set_gamma_level(int bl_level, int gamma_mode)
{
	...
	if (gamma_mode == GAMMA_SMART) {
		...
		pr_debug("lcd :current lux(%d) g_gamma_lux(%d)\n",
...
int mdss_dsi_magna_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		rc = gpio_tlmm_config(GPIO_CFG(ctrl_pdata->disp_en_gpio2_1_8v, 0,
					...
					GPIO_CFG_ENABLE);
		if (rc)
			pr_err("enabling disp_en_gpio2_1_8v[%d] failed, rc=%d\n", ctrl_pdata->disp_en_gpio2_1_8v, rc);
		...
		gpio_set_value(ctrl_pdata->disp_en_gpio2_1_8v, 1);
		mdelay(5);
		...
		rc = gpio_tlmm_config(GPIO_CFG(ctrl_pdata->disp_en_gpio2_1_8v, 0,
					...
					GPIO_CFG_ENABLE);
		if (rc)
			pr_err("disabling disp_en_gpio2_1_8v failed, rc=%d\n",rc);
		...
		gpio_set_value(ctrl_pdata->disp_en_gpio2_1_8v, 0);
		gpio_set_value((ctrl_pdata->rst_gpio), 0);
	}
	return 0;
}
							...
							u32 bl_level)
{
	...
	if(first_boot == 0)
	{
		...
		if (!msd.dstat.is_smart_dim_loaded) {
			...
			char *mtp_data = (char *)&(mpd.smart_ea8868.MTP);
			...
			pr_err("%s MTP is determined:%d err_cnt:%d",
							__func__, mtp_cnt, err_cnt);

			for (i = 0; i < MTP_DATA_SIZE_EA8868; i++) {
					pr_err("%s MTP DATA[%d] : %02x\n",
		...
		pr_err("mdss_dsi_panel_bl_ctrl6 \n");
	      ...
	      const int isUseMutex, struct msm_fb_data_type *pMFD)
{
	...
	*/
	...
	static char packet_size[] = { 0x04, 0x00 };
	...
	for (read_addr_cnt = 0; read_addr_cnt < sizeof(ea8868_read_address); read_addr_cnt++) {
		readed_size = 0;
...
static int find_mtp(struct msm_fb_data_type *mfd, char * mtp)
{
	char first_mtp[MTP_DATA_SIZE_EA8868]={0,};
	char second_mtp[MTP_DATA_SIZE_EA8868]={0,};
	char third_mtp[MTP_DATA_SIZE_EA8868]={0,};
	int mtp_size = MTP_DATA_SIZE_EA8868;

	int correct_mtp;

	mdss_dsi_cmds_send(msd.ctrl_pdata, samsung_panel_MTP_cmds, ARRAY_SIZE(samsung_panel_MTP_cmds), 0);
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if(!mpd.gamma_initial)
		return 0;

	if (0){//!msd.dstat.is_smart_dim_loaded) {
		...
		pr_debug("%s MTP is determined:%d err_cnt:%d",
						__func__, mtp_cnt, err_cnt);

		for (i = 0; i < MTP_DATA_SIZE_EA8868; i++) {
				pr_debug("%s MTP DATA[%d] : %02x\n",
						__func__, i, mtp_data[i]);
		}
