...
#if IS_BUILTIN(CONFIG_SEC_DEBUG)
static u64 __arch_irq_stat_cpu(unsigned int cpu)
{
	return arch_irq_stat_cpu(cpu);
}
#else
/* FIXME: smp_irq_stat_cpu is not exported for vendor modules in linux-5.4.y */
static u64 __arch_irq_stat_cpu(unsigned int cpu)
{
	u64 sum = 0;
	int i;
