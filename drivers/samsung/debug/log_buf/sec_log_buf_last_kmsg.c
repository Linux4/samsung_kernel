...
static const struct file_operations last_kmsg_buf_fops = {
	.owner = THIS_MODULE,
	.open = sec_last_kmsg_buf_open,
	.read = sec_last_kmsg_buf_read,
	.llseek = sec_last_kmsg_buf_lseek,
	.release = sec_last_kmsg_buf_release,
};
...
int __last_kmsg_procfs_create(struct builder *bd)
{
	...
	last_kmsg->proc = proc_create_data(LAST_LOG_BUF_NODE, 0444,
			NULL, &last_kmsg_buf_fops, last_kmsg);
	if (!last_kmsg->proc) {
