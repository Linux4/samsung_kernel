...
static int sec_fiemap_check_ranges(struct super_block *sb,
					   ...
					   const char *buf, size_t size)
{
	...
	pfiemap = (struct fiemap *)vzalloc(SEC_PARAM_EXTRA_MAX);
	if (!pfiemap) {
	...
	error = sec_fiemap_check_ranges(inode->i_sb, pfiemap->fm_start, pfiemap->fm_length, &len);
	if (error)
		goto alloc_err;
