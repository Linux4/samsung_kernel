...
static int acm_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	case ((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		if(!acm->port_open_state){
			printk(KERN_DEBUG "usb: %s modem port open\n", __func__);
			acm->port_open_state = true;
		}
		break;
	...
	case ((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		if(w_value == 0){
			printk(KERN_DEBUG "usb: %s modem port close\n", __func__);
			acm->port_open_state = false;
		}
		value = 0;
