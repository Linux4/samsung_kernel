...
#ifdef CONFIG_SM5502_MUIC
static int vbusout_old;
static int vbusout_new;
static int reset_oneshot;

static int muic_vbus_cb(struct notifier_block *self, unsigned long action, void *dev)
{
	switch (action) {
	...
	case CABLE_TYPE3_NONSTD_SDP_MUIC :
		printk(KERN_INFO "sm5502 VBUSOUT ON - notified \n");
		vbusout_old = vbusout_new;
		vbusout_new = VBUS_HIGH;
		break;
	...
	case CABLE_TYPE2_JIG_UART_ON_MUIC:
		printk(KERN_INFO "sm5502 VBUSOUT OFF - notified \n");
		vbusout_old = vbusout_new;
		vbusout_new = VBUS_LOW;
		break;
...
static struct notifier_block muic_vbus_nb = {
  .notifier_call = muic_vbus_cb,
};
#endif
...
static void mv_udc_vbus_work(struct work_struct *work)
{
	...
	if (vbus == VBUS_LOW && vbusout_new == VBUS_HIGH) {
		dev_info(&udc->dev->dev, "MUIC(sm5502) VBUS Detach BUG Case\n");
		dev_info(&udc->dev->dev, "vbusout_old : %d, vbusout_new : %d\n", vbusout_old, vbusout_new);
		if (reset_oneshot != 1) {
			sm5502_ic_reset();
			reset_oneshot = 1;
		} else {
			dev_info(&udc->dev->dev, "No Reset\n");
		}
	} else if(vbus == VBUS_HIGH && vbusout_new == VBUS_LOW) {
		dev_info(&udc->dev->dev, "MUIC(sm5502) VBUS Attach BUG Case\n");
		dev_info(&udc->dev->dev, "vbusout_old : %d, vbusout_new : %d\n", vbusout_old, vbusout_new);
		if (reset_oneshot != 1) {
			sm5502_ic_reset();
			reset_oneshot = 1;
...
#ifdef CONFIG_SM5502_MUIC
	usb_switch_register_notify(&muic_vbus_nb);
