...
#define EXYNOS_SS_UDC_CABLE_CONNECT(udc, connect)	\
	exynos_ss_udc_cable_connect(udc, connect)
#define WAKE_LOCK_INIT(udc)\
	wake_lock_init(&udc->wake_lock, WAKE_LOCK_SUSPEND, "exynos_udc")
#define WAKE_LOCK_ACQUIRE(udc) \
...
static void exynos_ss_udc_irq_ulstchng(struct exynos_ss_udc *udc, u32 event)
{
	...
	if (!soc_is_exynos5260()) {
		...
		/* WORKAROUND : LFPS Corruption */
		if (event & EXYNOS_USB3_DEVT_EvtInfo_SS) {
			if (link_state == EXYNOS_USB3_DEVT_EvtInfo_RX_DET) {
				/* TX_CM_EN_OVRD(bit 9):1, TX_CM_EN(bit 8):1 */
				if (udc->core->ops->phy_crport_ctrl)
					udc->core->ops->phy_crport_ctrl(udc->core,
							0x1000, 0x300);
			} else if (link_state == EXYNOS_USB3_DEVT_EvtInfo_SS_DIS) {
				/* TX_CM_EN_OVRD(bit 9):0, TX_CM_EN(bit 8):1 */
				if (udc->core->ops->phy_crport_ctrl)
					udc->core->ops->phy_crport_ctrl(udc->core,
							0x1000, 0x100);
			} else if (link_state == EXYNOS_USB3_DEVT_EvtInfo_U3) {
				/* TX_CM_EN_OVRD(bit 9):0, TX_CM_EN(bit 8):1 */
				if (udc->core->ops->phy_crport_ctrl)
					udc->core->ops->phy_crport_ctrl(udc->core,
							0x1000, 0x100);
			} else if (link_state == EXYNOS_USB3_DEVT_EvtInfo_Resume_Rst) {
				/* TX_CM_EN_OVRD(bit 9):1, TX_CM_EN(bit 8):1 */
				if (udc->core->ops->phy_crport_ctrl)
					udc->core->ops->phy_crport_ctrl(udc->core,
							0x1000, 0x300);
			...
			if (link_state == EXYNOS_USB3_DEVT_EvtInfo_SS_DIS) {
				/* TX_CM_EN_OVRD(bit 9):0, TX_CM_EN(bit 8):1 */
				if (udc->core->ops->phy_crport_ctrl)
					udc->core->ops->phy_crport_ctrl(udc->core,
							0x1000, 0x100);
...
static void exynos_ss_udc_handle_devt(struct exynos_ss_udc *udc, u32 event)
{
	...
	case EXYNOS_USB3_DEVT_EVENT_WkUpEvt:
		dev_vdbg(udc->dev, "Wakeup Detected Event");
		/* TX_CM_EN_OVRD(bit 9):1, TX_CM_EN(bit 8):1 */
		if (udc->core->ops->phy_crport_ctrl)
			udc->core->ops->phy_crport_ctrl(udc->core,
					0x1000, 0x300);
