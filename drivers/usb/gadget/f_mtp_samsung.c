		...
		struct device_attribute *attr, char *buf)
{
	...
	printk(KERN_DEBUG "usb: %s guid = %s\n", __func__, guid_info);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	printk(KERN_DEBUG "usb: %s guid_info=%s, %s\n", __func__, buf, guid_info);
	return value;
}
...
static long  mtpg_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
	...
	case MTP_VBUS_DISABLE:
		printk(KERN_DEBUG "[%s] line=[%d] \n",
							__func__, __LINE__);
		if (dev->cdev && dev->cdev->gadget) {
#if 0    //160104 add below codes for disable mtp function.
			usb_gadget_disconnect(cdev->gadget);
#else
				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	if (value >= 0) {
		...
		{
			printk("usb: [%s] RECEIVE PC GUID / line[%d]\n",
						__func__, __LINE__);
			value = w_length;
		...
		printk(KERN_DEBUG "usb: mtp_ctrlrequest "\
...
static int mtp_setup(void)
{
	...
	if (err) {
		printk(KERN_DEBUG "usb: %s failed to create guid attr\n",
				__func__);
		return err;
	}
	else
	printk(KERN_DEBUG "usb: %s success to create guid attr\n",
				__func__);
	return 0;
err_work:
err_misc_register:
