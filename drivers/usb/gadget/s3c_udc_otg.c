...
static inline const struct exynos_udc_drvdata
*exynos_udc_get_driver_data(struct platform_device *pdev);

static void exynos_udc_clk_disable_unprepare(struct s3c_udc *dev)
{
	int i;
	...
	if (dev->clk) {
		...
		for (i = 0; dev->clocks[i] != NULL; i++)
			clk_disable_unprepare(dev->clocks[i]);
	}
}

static int exynos_udc_clk_prepare_enable(struct s3c_udc *dev)
{
	int i;
	int ret;
	...
	if (dev->clk) {
		...
		for (i = 0; dev->clocks[i] != NULL; i++) {
			ret = clk_prepare_enable(dev->clocks[i]);
			if (ret)
				goto err;
		}
	}
...
static int exynos_udc_clk_get(struct s3c_udc *dev)
{
	const char *clk_id;
	...
	/* already set in probe */
	pdev = dev->dev;

	clk_id = "otg";
	clk = devm_clk_get(&pdev->dev, clk_id);
	if (IS_ERR_OR_NULL(clk)) {
		clk_names = dev->drvdata->clk_names;
		size = dev->drvdata->clk_size;
		dev->clocks = (struct clk **) kmalloc(size * sizeof(struct clk *),
				GFP_KERNEL);
		if (!dev->clocks)
			return -ENOMEM;

		for (i = 0; clk_names[i] != NULL; i++) {
			clk_id = clk_names[i];
			clk = devm_clk_get(&pdev->dev, clk_id);
			if (IS_ERR_OR_NULL(clk))
				goto err;
			...
			dev->clocks[i] = clk;
			dev_info(&pdev->dev, "get %dth clock %s\n", i, clk_id);
		}
		dev->clocks[i] = NULL;
	} else {
		dev->clk = clk;
...
static int udc_enable(struct s3c_udc *dev)
{
	...
	if (err) {
		pr_info(KERN_ERR "%s: can't enable clk \n", driver_name);
		return err;
	}
...
static int s3c_udc_probe(struct platform_device *pdev)
{
	...
	/* get drvdata from dt */
	drvdata = exynos_udc_get_driver_data(pdev);
	if (!drvdata)
		return -EINVAL;
...
static const char *exynos7580_otg_clk_names[] = {"otg_aclk", "otg_hclk",
	"upsizer_otg", "xiu_d_fsys1", "upsizer_fsys1", "upsizer_ahb_usbhs", NULL};

static const char *exynos3475_otg_clk_names[] = {"otg_aclk", "otg_hclk",
	"upsizer_otg", "xiu_d_fsys1", "upsizer_fsys1", "upsizer_ahb_usbhs",
	"ahb_usbhs", "ahb2axi_usbhs", NULL};

static struct exynos_udc_drvdata exynos7580_udc_drvdata = {
	.clk_names = exynos7580_otg_clk_names,
	.clk_size = ARRAY_SIZE(exynos7580_otg_clk_names),
};

static struct exynos_udc_drvdata exynos3475_udc_drvdata = {
	.clk_names = exynos3475_otg_clk_names,
	.clk_size = ARRAY_SIZE(exynos3475_otg_clk_names),
};

#ifdef CONFIG_OF
static const struct of_device_id exynos_udc_match[] = {
	{ .compatible = "samsung,exynos_udc",
	  .data = &exynos7580_udc_drvdata,
	},
	{ .compatible = "samsung,exynos3475_udc",
	  .data = &exynos3475_udc_drvdata,
	},
	{},
};
...
*exynos_udc_get_driver_data(struct platform_device *pdev)
{
	if (pdev->dev.of_node) {
		...
		match = of_match_node(exynos_udc_match,
				pdev->dev.of_node);
		return match->data;
	}
