...
#include <linux/usb/exynos_ohci.h>
#include <linux/usb/phy.h>
#include <linux/usb/samsung_usb_phy.h>
#include <mach/exynos-pm.h>
#include <linux/slab.h>

static int exynos_ohci_clk_prepare_enable(struct exynos_ohci_hcd *exynos_ohci)
{
	int i;
	int ret;
	...
	if (exynos_ohci->clk) {
		...
		for (i = 0; exynos_ohci->clocks[i] != NULL; i++) {
			ret = clk_prepare_enable(exynos_ohci->clocks[i]);
			if (ret)
				goto err;
		}
	}
...
err:
	/* roll back */
	for (i = i - 1; i >= 0; i--)
		clk_disable_unprepare(exynos_ohci->clocks[i]);

	return ret;
}

static void exynos_ohci_clk_disable_unprepare(struct exynos_ohci_hcd *exynos_ohci)
{
	int i;
...
static int exynos_ohci_clk_get(struct exynos_ohci_hcd *exynos_ohci, struct device *dev)
{
	const char *clk_id;
	const char **exynos_ohci_clk_names;
	struct clk *clk;
	int size;
	int i;
	...
	if (IS_ERR_OR_NULL(clk)) {
		...
		exynos_ohci->clk = NULL;
		/* fallback to separate clock control */
		exynos_ohci_clk_names = exynos_ohci->drvdata->clk_names;
		size = exynos_ohci->drvdata->clk_size;
		exynos_ohci->clocks = (struct clk **) kmalloc(size * sizeof(struct clk *),
				GFP_KERNEL);
		if (!exynos_ohci->clocks)
			return -ENOMEM;

		for (i = 0; exynos_ohci_clk_names[i] != NULL; i++) {
			clk_id = exynos_ohci_clk_names[i];

			clk = devm_clk_get(dev, clk_id);
			if (IS_ERR_OR_NULL(clk))
				goto err;

			exynos_ohci->clocks[i] = clk;
			dev_dbg(dev, "get %dth clock %s\n", i, clk_id);
		}

		exynos_ohci->clocks[i] = NULL;
...
static int exynos_ohci_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
	struct usb_phy *phy;
	const struct exynos_ohci_drvdata *drv_data;
	int irq;
	int err;
...
skip_phy:
	...
	drv_data = exynos_ohci_get_driver_data(pdev);
	exynos_ohci->drvdata = drv_data;

	exynos_ohci->hcd = hcd;
	err = exynos_ohci_clk_get(exynos_ohci, &pdev->dev);
	...
	err = exynos_ohci_clk_prepare_enable(exynos_ohci);
	if (err)
		goto fail_clk;
	...
	if (exynos_ohci->drvdata->pm_enable) {
...
fail_add_hcd:
	...
	exynos_ohci_phy_disable(exynos_ohci);
fail_io:
	exynos_ohci_clk_disable_unprepare(exynos_ohci);
fail_clk:
	usb_put_hcd(hcd);
	return err;
}
...
static int exynos_ohci_suspend(struct device *dev)
{
	...
	/* Check whether Host Controller use power management or not */
	if (!exynos_ohci->drvdata->pm_enable)
		return 0;
...
static int exynos_ohci_resume(struct device *dev)
{
	...
	exynos_ohci_clk_prepare_enable(exynos_ohci);
...
static const char *exynos7580_ohci_clk_names[] = {"otg_aclk", "xiu_d_fsys1", "ahb_usbhs",
		"ahb2axi_usbhs", "upsizer_fsys1", "upsizer_ahb_usbhs", "usbhost20", NULL};

static struct exynos_ohci_drvdata exynos4210_ohci_drvdata = {
	.pm_enable = 1,
};
static struct exynos_ohci_drvdata exynos5440_ohci_drvdata = {
	.pm_enable = 1,
};
static struct exynos_ohci_drvdata exynos5_ohci_drvdata = {
	.pm_enable = 1,
};
static struct exynos_ohci_drvdata exynos7580_ohci_drvdata = {
	.clk_names = exynos7580_ohci_clk_names,
	.clk_size = ARRAY_SIZE(exynos7580_ohci_clk_names),
	.pm_enable = 0,
};
...
static const struct of_device_id exynos_ohci_match[] = {
	{
		...
		.data = &exynos4210_ohci_drvdata,
	}, {
		.compatible = "samsung,exynos5440-ohci",
		.data = &exynos5440_ohci_drvdata,
	}, {
		.compatible = "samsung,exynos5-ohci",
		.data = &exynos5_ohci_drvdata,
	}, {
		.compatible = "samsung,exynos7580-ohci",
		.data = &exynos7580_ohci_drvdata,
	},
	{},
};
