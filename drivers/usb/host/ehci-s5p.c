...
static void __iomem *hcd_regs;
void s5p_ehci_set_ohci_susp_lgcy(void)
{
	int delay_count = 0;
	...
	if (!(readl(INSNREG00(hcd_regs)) & OHCI_SUSP_LGCY))
		writel(readl(INSNREG00(hcd_regs)) | OHCI_SUSP_LGCY,
			INSNREG00(hcd_regs));
}
#endif

static int s5p_ehci_port_power_off(struct platform_device *pdev, int port)
{
...
static int s5p_ehci_port_power_on(struct platform_device *pdev, int port)
{
	struct s5p_ehci_hcd *s5p_ehci = platform_get_drvdata(pdev);
	struct usb_hcd *hcd = s5p_ehci->hcd;
	struct ehci_hcd *ehci = hcd_to_ehci(hcd);
...
#endif
		s5p_ehci_port_power_on(pdev, port);
	} else
		s5p_ehci_port_power_off(pdev, port);

	device_unlock(dev);
	return count;
}
static DEVICE_ATTR(port_power, S_IWUSR | S_IWGRP | S_IRUSR | S_IRGRP,
		NULL, store_port_power);
...
static inline int create_ehci_sys_file(struct ehci_hcd *ehci)
{
	...
	ret = device_create_file(ehci_to_hcd(ehci)->self.controller,
			&dev_attr_port_power);
	if (ret) {
		pr_err("port power device_created_file fail(%d)\n", ret);
		return ret;
	}
	...
	ret = device_create_file(ehci_to_hcd(ehci)->self.controller,
			&dev_attr_ehci_power);
	if (ret) {
		pr_err("ehci power device_created_file fail(%d)\n", ret);
		return ret;
	}
