...
#define ADDRESS_START 0x1200
#define ADDRESS_END 0x19FC
...
static ssize_t ssphy_addr_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%x (%d/%d)\n", tune_addr, tune_buf_cnt, TUNE_BUF_SIZE);
}
static ssize_t ssphy_addr_store(struct device *dev,
...
static DEVICE_ATTR(ssphy_addr, 0664,
	ssphy_addr_show, ssphy_addr_store);

static ssize_t ssphy_val_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct msm_ssphy_qmp *phy = dev_get_drvdata(dev);
	char str[15 * (TUNE_BUF_SIZE + 1)] = {0, };
	int i;
	...
	if (tune_addr) {
		return sprintf(buf, "0x%x 0x%x (%d/%d)\n", tune_addr,
			(readl_relaxed(phy->base + tune_addr) & 0xff), tune_buf_cnt, TUNE_BUF_SIZE);
	}
	else {
		sprintf(str, "Table(%d/%d)\n", tune_buf_cnt, TUNE_BUF_SIZE);
		for (i = 0; i < tune_buf_cnt; i++) {
			sprintf(str, "%s0x%x 0x%x\n", str, tune_buf[i][0], tune_buf[i][1]);
		}
		return sprintf(buf, "%s\n", str);
	}
}
static ssize_t ssphy_val_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct msm_ssphy_qmp *phy = dev_get_drvdata(dev);
	u32 val;
	...
	if (tune_addr) {
		for (i = 0; i < tune_buf_cnt; i++) {
			if (tune_buf[i][0] == tune_addr) {
				writel_relaxed(val, phy->base + tune_addr);
				tune_buf[i][1] = val;
				usleep_range(1, 2);
				pr_info("%s(): [%d] 0x%x 0x%x (%d/%d)\n", __func__, i, tune_addr,
					(readl_relaxed(phy->base + tune_addr) & 0xff), tune_buf_cnt, TUNE_BUF_SIZE);
				return size;
			}
		}
		...
		pr_info("%s(): tuning address is not set\n", __func__);

	return size;
}
static DEVICE_ATTR(ssphy_val, 0664,
	ssphy_val_show, ssphy_val_store);

static struct attribute *ssphy_attrs[] = {
	&dev_attr_ssphy_addr.attr,
	&dev_attr_ssphy_val.attr,
	NULL,
};
