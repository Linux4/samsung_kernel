...
static void qusb_phy_host_init(struct usb_phy *phy)
{
	...
	/*F52 code for P210304-02433 by wangzikang at 2020/04/08 start*/
	pr_info("TUNE: Programming TUNE1 parameter as:0x%x\n", qphy->tune_val);
	/*F52 code for P210304-02433 by wangzikang at 2020/04/08 end*/
}
	...
	if (size) {
		...
		if (qphy->qusb_phy_init_seq) {
			...
			of_property_read_u32_array(dev->of_node,
				...
				qphy->init_seq_len);
			/*F52 code for P210304-02433 by wangzikang at 2020/04/08 start*/
			for (count=0; count<(qphy->init_seq_len/2); count++){
				pr_info("TUNE SEQ: [0x%x]=0x%x\n",qphy->qusb_phy_init_seq[2*count+1],qphy->qusb_phy_init_seq[2*count]);
			}
			/*F52 code for P210304-02433 by wangzikang at 2020/04/08 end*/
		} else {
			dev_err(dev,
