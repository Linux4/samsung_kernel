...
static int dwc3_otg_statemachine(struct otg_fsm *fsm)
{
	...
	struct usb_phy *phy3	= fsm->otg->phy3;
	...
	if (fsm->reset) {
		...
		phy3->state = OTG_STATE_UNDEFINED;
		goto exit;
	}
	...
	case OTG_STATE_UNDEFINED:
		if (fsm->id) {
			...
			phy3->state = OTG_STATE_B_IDLE;
		} else {
			phy->state = OTG_STATE_A_IDLE;
			phy3->state = OTG_STATE_A_IDLE;
		}
		break;
...
has_ext_otg:
	...
	dotg->otg.phy3 = dwc->usb3_phy;
	dotg->otg.phy3->otg = &dotg->otg;
	dotg->otg.phy3->state = OTG_STATE_UNDEFINED;
