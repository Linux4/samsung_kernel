		...
		u8 *buf, size_t len)
{
	...
	/* F52 code for P210317-02835 Fix the PD disconnect by gaochao at 2021/04/02 start */
	if (!strcmp(msg_to_string(msg_type, num_objs, PD_MSG_HDR_IS_EXTENDED(header)), "Alert")) {
		usbpd_err(&pd->dev, "Alert message receied, ignore and return\n");
		return;
	}
	/* F52 code for P210317-02835 Fix the PD disconnect by gaochao at 2021/04/02 end */
...
#define HV_DISABLE_ENABLE_PD_TA_VOLTAGE_LIMIT	10000000
#define HV_DISABLE_ENABLE_PD_TA_CURRENT_LIMIT	1000000
#endif
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* F52 code for SR-QN3979-01-805 SEC_FLOATING_FEATURE_BATTERY_SUPPORT_HV_DURING_CHARGING by gaochao at 2021/04/05 start */
	#if defined(CONFIG_AFC)
	ret = power_supply_get_property(pd->bat_psy, POWER_SUPPLY_PROP_HV_DISABLE,
		&pval);
	if (ret < 0) {
		pr_err("Failed to get HV_DISABLE, rc=%d\n", ret);
	}

	if (!pval.intval) {
		// handle_hard_reset(pd);
		usbpd_dbg(&pd->dev, "hv_disable is false\n");
	} else {
		if (uv >= HV_DISABLE_ENABLE_PD_TA_VOLTAGE_LIMIT) {
			uv = HV_DISABLE_ENABLE_PD_TA_VOLTAGE_LIMIT;
		}

		if (ua > HV_DISABLE_ENABLE_PD_TA_CURRENT_LIMIT) {
			ua = HV_DISABLE_ENABLE_PD_TA_CURRENT_LIMIT;
		}
	}

	usbpd_dbg(&pd->dev, "requested_current=%d, pdo=%d, uv=%d, ua=%d, hv_disable=%d\n",
		pd->requested_current, pdo, uv, ua, pval.intval);
	#endif
	/* F52 code for SR-QN3979-01-805 SEC_FLOATING_FEATURE_BATTERY_SUPPORT_HV_DURING_CHARGING by gaochao at 2021/04/05 end */
