...
#if defined(CONFIG_PDIC_NOTIFIER) || defined(CONFIG_IFCONN_NOTIFIER)
...
{
#if !defined(CONFIG_CCIC_S2MM005) && !defined(CONFIG_CCIC_S2MU106) && !defined(CONFIG_CCIC_S2MU107)
	u8 on = (u8)!!enable;
...
static int usb_notifier_probe(struct platform_device *pdev)
{
	...
	ifconn_notifier_register(&pdata->muic_usb_nb,
					...
					IFCONN_NOTIFY_USB,
					IFCONN_NOTIFY_MUIC);
	ifconn_notifier_register(&pdata->pdic_usb_nb,
					pdic_usb_handle_notification,
					IFCONN_NOTIFY_USB,
					IFCONN_NOTIFY_PDIC);
...
{
#if defined(CONFIG_IFCONN_NOTIFIER)
	ifconn_notifier_unregister(IFCONN_NOTIFY_USB, IFCONN_NOTIFY_PDIC);
