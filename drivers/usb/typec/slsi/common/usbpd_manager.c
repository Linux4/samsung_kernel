...
int usbpd_manager_evaluate_capability(struct usbpd_data *pd_data)
{
	...
	for (i = 0; i < policy->rx_msg_header.num_data_objs; i++) {
		...
		case POWER_TYPE_VARIABLE:
			...
			break;
		case POWER_TYPE_PPS:
			max_volt = pd_obj->power_data_obj_pps.max_voltage * USBPD_PPS_VOLT_UNIT;
			min_volt = pd_obj->power_data_obj_pps.min_voltage * USBPD_PPS_VOLT_UNIT;
			max_current = pd_obj->power_data_obj_pps.max_current * USBPD_PPS_CURRENT_UNIT;
