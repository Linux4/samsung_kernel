...
static irqreturn_t sc27xx_typec_interrupt(int irq, void *data)
{
	...
	if (ret){
		pr_emerg("%s:line%d pmic zhanrui, ret = %d\n", ret);
		return ret;
	}
...
void Set_Typec_Type(enum typec_port_type mode){
	dev_err(typec_sc->dev, "now typec works in mode %d\n",mode);
	typec_sc->typec_cap.type = mode;
	sc27xx_typec_enable(typec_sc);
}
EXPORT_SYMBOL_GPL(Set_Typec_Type);
