...
#if defined(CONFIG_USB_TYPEC_MANAGER_NOTIFIER) && !defined(CONFIG_USB_CCIC_NOTIFIER_USING_QC)
...
static void ccic_uevent_work(int id, int state)
{
...
#if defined(CONFIG_USB_CCIC_NOTIFIER_USING_QC)
	char *cable[2] = { "attached_muic_cable", NULL };
#endif
#endif
	struct device *ccic_device = get_ccic_device();
	...
	case CCIC_NOTIFY_ID_CC_PIN_STATUS:
		...
		break;
#if defined(CONFIG_USB_CCIC_NOTIFIER_USING_QC)
	case CCIC_NOTIFY_ID_CABLE:
		kobject_uevent_env(&ccic_device->kobj, KOBJ_CHANGE, cable);
		break;
...
int ccic_notifier_notify(CC_NOTI_TYPEDEF *p_noti, void *pd, int pdic_attach)
{
	...
	case CCIC_NOTIFY_ID_CABLE:
		pr_info("%s: src:%01x dest:%01x id:%02x attach:%02x\n", __func__,
			p_noti->src, p_noti->dest, p_noti->id, p_noti->sub1);
		ccic_uevent_work(CCIC_NOTIFY_ID_CABLE, p_noti->sub1);
		return 0;
#endif
#endif
