...
int sm5713_select_pps(int num, int ppsVol, int ppsCur)
{
	struct sm5713_usbpd_data *psubpd = pd_noti.pusbpd;
	int timeout = 0;
	...
	pd_noti.sink_status.power_list[num].pps_voltage = ppsVol;
	pd_noti.sink_status.power_list[num].pps_current = ppsCur;
	if (pd_noti.sink_status.power_list[num].apdo) {
		pd_noti.sink_status.request_apdo = true;
	} else {
...
int sm5713_get_apdo_max_current(unsigned int *pdo_pos, unsigned int taMaxVol, unsigned int *taMaxCur)
{
	int i;
	int ret = 0;
	int max_current = 0;
	...
	if (!pd_noti.sink_status.has_apdo) {
		pr_info("%s: pd don't have apdo(rev:%d)\n",
			__func__, pd_noti.sink_status.pd_rev);
		return -1;
	}
...
int sm5713_get_apdo_max_power(unsigned int *pdo_pos, unsigned int *taMaxVol, unsigned int *taMaxCur, unsigned int *taMaxPwr)
{
	int i;
	int ret = 0;
...
data_obj_type sm5713_usbpd_select_capability(struct sm5713_usbpd_data *pd_data)
{
	...
	if (pd_noti.sink_status.request_apdo) {
		output_vol = (pd_noti.sink_status.power_list[pdo_num].pps_voltage / USBPD_OUT_VOLT_UNIT);
		op_curr = (pd_noti.sink_status.power_list[pdo_num].pps_current / USBPD_PPS_CURRENT_UNIT);
...
int sm5713_usbpd_evaluate_capability(struct sm5713_usbpd_data *pd_data)
{
	...
	pdic_sink_status->pd_rev = PDIC_PD_REV_20;
	...
	for (i = 0; i < policy->rx_msg_header.num_data_objs; i++) {
		...
		case POWER_TYPE_APDO:
...
#if defined(CONFIG_PDIC_PD30)
			pdic_sink_status->pd_rev = PDIC_PD_REV_30;
			pdic_sink_status->has_apdo = true;
#endif
			dev_info(pd_data->dev, "[%d] Augmented min_volt(%d)mV, max_volt (%d)mV, max_current(%d)mA\n",
					i+1, min_volt * USBPD_PPS_VOLT_UNIT, max_volt * USBPD_PPS_VOLT_UNIT, max_current * USBPD_PPS_CURRENT_UNIT);
#ifdef CONFIG_USB_TYPEC_MANAGER_NOTIFIER
			if (max_volt * USBPD_PPS_VOLT_UNIT <= MAX_CHARGING_VOLT)
				available_pdo_num = i + 1;
			...
			pdic_sink_status->power_list[i + 1].pps_voltage = pdic_sink_status->power_list[i + 1].max_voltage;
			pdic_sink_status->power_list[i + 1].pps_current = pdic_sink_status->power_list[i + 1].max_current;
#endif			
			break;
		default:
		...
		data_obj_type *obj)
{
	...
	if (pd_noti.sink_status.pd_rev == PDIC_PD_REV_30)
		header->spec_revision = USBPD_REV_30;
	else
		...
		if (ext_msg && pd_data->policy.rx_msg_header.spec_revision == 0) {
			...
			case USBPD_Source_Capabilities:
				...
				if (pd_data->policy.rx_msg_header.spec_revision == USBPD_REV_30)
					sm5713_set_enable_pd_30_mode(pd_data);			
				break;
			case USBPD_Request:
...
static int sm5713_usbpd_manager_init(struct sm5713_usbpd_data *pd_data)
{
	...
	fp_sec_pd_select_pps = sm5713_select_pps;
	fp_sec_pd_get_apdo_max_current = sm5713_get_apdo_max_current;
	fp_sec_pd_get_apdo_max_power = sm5713_get_apdo_max_power;
#endif
