...
#if IS_ENABLED(CONFIG_SEC_PD)
	tcpc_pd_update_src_cap(pd_port->tcpc_dev, &pd_port->remote_src_cap);
#else
...
#if IS_ENABLED(CONFIG_SEC_PD)
	pd_port->tcpc_dev->pd_noti.sink_status.selected_pdo_num = req_info.pos;
	pd_port->tcpc_dev->pd_noti.sink_status.current_pdo_num = req_info.pos;
#endif
...
#if IS_ENABLED(CONFIG_SEC_PD)
	pd_port->tcpc_dev->pd_noti.sink_status.current_pdo_num =
		pd_port->tcpc_dev->pd_noti.sink_status.selected_pdo_num;
#endif
}
...
#if IS_ENABLED(CONFIG_PDIC_POLICY)
	pdic_policy_send_msg(pd_port->tcpc_dev->pp_data, MSG_EX_CNT, 0, 0);
#endif /* CONFIG_PDIC_POLICY */
}
