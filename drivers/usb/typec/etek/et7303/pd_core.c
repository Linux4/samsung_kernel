...
int pd_set_data_role(struct pd_port *pd_port, uint8_t dr)
{
	...
	if (pd_port->data_role == PD_ROLE_UFP)
		pdic_policy_send_msg(pd_port->tcpc_dev->pp_data, MSG_UFP, 0, 0);
	else if (pd_port->data_role == PD_ROLE_DFP)
		pdic_policy_send_msg(pd_port->tcpc_dev->pp_data, MSG_DFP, 0, 0);
#endif /* CONFIG_PDIC_POLICY */
...
int pd_set_power_role(struct pd_port *pd_port, uint8_t pr)
{
	...
	if (pd_port->power_role == PD_ROLE_SINK) {
		pdic_policy_send_msg(pd_port->tcpc_dev->pp_data, MSG_SNK, 0, 0);

		switch (pd_port->tcpc_dev->typec_remote_rp_level) {
		case TYPEC_CC_VOLT_SNK_1_5:
			pdic_policy_send_msg(pd_port->tcpc_dev->pp_data,
					MSG_RP22K, 0, 0);
			break;
	...
	} else if (pd_port->power_role == PD_ROLE_SOURCE)
		pdic_policy_send_msg(pd_port->tcpc_dev->pp_data, MSG_SRC, 0, 0);

	if (pd_port->tcpc_dev->typec_polarity)
		pdic_policy_send_msg(pd_port->tcpc_dev->pp_data, MSG_CC2, 0, 0);
	else
		pdic_policy_send_msg(pd_port->tcpc_dev->pp_data, MSG_CC1, 0, 0);
