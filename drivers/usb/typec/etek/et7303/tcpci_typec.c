...
static inline void typec_source_attached_entry(struct tcpc_device *tcpc_dev)
{
	...
	pdic_policy_send_msg(tcpc_dev->pp_data, MSG_SRC, 0, 0);
	pdic_policy_send_msg(tcpc_dev->pp_data, MSG_DFP, 0, 0);

	if (tcpc_dev->typec_polarity)
		pdic_policy_send_msg(tcpc_dev->pp_data, MSG_CC2, 0, 0);
	else
		pdic_policy_send_msg(tcpc_dev->pp_data, MSG_CC1, 0, 0);
...
#if IS_ENABLED(CONFIG_PDIC_POLICY)
	pdic_policy_send_msg(tcpc_dev->pp_data, MSG_SNK, 0, 0);
	pdic_policy_send_msg(tcpc_dev->pp_data, MSG_UFP, 0, 0);

	switch (tcpc_dev->typec_remote_rp_level) {
	case TYPEC_CC_VOLT_SNK_1_5:
		pdic_policy_send_msg(tcpc_dev->pp_data, MSG_RP22K, 0, 0);
		break;
	case TYPEC_CC_VOLT_SNK_3_0:
		pdic_policy_send_msg(tcpc_dev->pp_data, MSG_RP10K, 0, 0);
		break;
	case TYPEC_CC_VOLT_SNK_DFT:
		pdic_policy_send_msg(tcpc_dev->pp_data, MSG_RP56K, 0, 0);
		break;
	default:
		break;
	}
...
#if IS_ENABLED(CONFIG_PDIC_POLICY)
	pdic_policy_send_msg(tcpc_dev->pp_data, MSG_DEBUG, 0, 0);
#endif /* CONFIG_PDIC_POLICY */
	return true;
}
...
#if IS_ENABLED(CONFIG_PDIC_POLICY)
	pdic_policy_send_msg(tcpc_dev->pp_data, MSG_AUDIO, 0, 0);
#endif /* CONFIG_PDIC_POLICY */
