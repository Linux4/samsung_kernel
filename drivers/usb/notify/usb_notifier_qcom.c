...
static int otg_accessory_power(bool enable)
{
	...
	if (psy_otg) {
...
#ifdef CONFIG_BATTERY_SAMSUNG_USING_QC
		while (count--) {
			ret = power_supply_get_property(psy_otg,
					POWER_SUPPLY_PROP_VOLTAGE_NOW, &val);
			if (ret){
				pr_info("%s %d, get POWER_SUPPLY_PROP_VOLTAGE_NOW fail count=%d\n", __func__, __LINE__, count);
				break;
			}
			/*
			 ...
			 * before proceeding.
			 */
			if ( enable && val.intval >= 4750000) /*vsafe5Vmin*/
			{
				pr_info("%s %d, enable vbus succeed count=%d\n", __func__, __LINE__, count);
			   	vbus_notifier_handle(STATUS_VBUS_HIGH);
				break;
			}
			/*
	             * Check to make sure VBUS voltage reaches below  VSafe0V (0.8V) 
			 * before proceeding.
			 */
			 if (!enable && val.intval <= 800000) /*VSafe0V( 0.8v)*/
			{
			    pr_info("%s %d, disable vbus succeed count=%d\n", __func__, __LINE__, count);
		...
		if (count < 0)
			pr_info("%s %d, get POWER_SUPPLY_PROP_VOLTAGE_NOW fail timeout=%d\n", __func__, __LINE__, count);
#endif
	}else if (psy_battery) {
		if (enable)
...
#ifdef CONFIG_BATTERY_SAMSUNG_USING_QC
	if (set)
		sec_bat_set_usb_configuration(USB_CURRENT_HIGH_SPEED);
	else
		sec_bat_set_usb_configuration(USB_CURRENT_UNCONFIGURED);
#else
	union power_supply_propval val;
