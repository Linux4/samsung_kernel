...
#ifdef CONFIG_MUIC_SM5705_SWITCH_CONTROL_GPIO
			/*There is some timing issue when connect with iphone by OTG connector*/
			/*set USB_ID pin high to let MUIC doesn't do BC1.2 charging when DFP connected*/
			muic_GPIO_control(1);
#endif
...
static int otg_accessory_power(bool enable)
{
	...
	if (psy_otg) {
		...
		if (enable) {
			o_notify->hw_param[USB_CCIC_OTG_USE_COUNT]++;	// OTG Count	
