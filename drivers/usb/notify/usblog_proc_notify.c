	...
	unsigned long long ts, unsigned long rem_nsec, int type, int enable)
{
	...
	case NOTIFY_PCM_PLAYBACK:
		seq_printf(m, "[%5lu.%06lu][%02d-%02d %02d:%02d:%02d] USB PCM PLAYBACK %s\n",
			(unsigned long)ts, rem_nsec / 1000,
			rt->tm_mon, rt->tm_mday, rt->tm_hour, rt->tm_min, rt->tm_sec,
				enable ? "OPEN" : "CLOSE");
		break;
	case NOTIFY_PCM_CAPTURE:
		seq_printf(m, "[%5lu.%06lu][%02d-%02d %02d:%02d:%02d] USB PCM CAPTURE %s\n",
			(unsigned long)ts, rem_nsec / 1000,
			rt->tm_mon, rt->tm_mday, rt->tm_hour, rt->tm_min, rt->tm_sec,
				enable ? "OPEN" : "CLOSE");
		break;
	default:
		seq_printf(m, "[%5lu.%06lu][%02d-%02d %02d:%02d:%02d] undefined event\n",
			(unsigned long)ts, rem_nsec / 1000,
			rt->tm_mon, rt->tm_mday, rt->tm_hour, rt->tm_min, rt->tm_sec);
		break;
	}
