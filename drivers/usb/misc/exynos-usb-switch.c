...
#ifdef CONFIG_MACH_XYREF4415
	/* When USB host cable is disconnected, OTG_BOOST5V_EN pin goes to LOW to stop providing VBUS.
	   But even if OTG_BOOST5V_EN pin went to LOW, the VBUS is still provided for 100ms.
	   This is caused by 20uF capacity.
	   So we should control second path for provding VBUS as set XUHOSTPWREN pin to LOW.
	   But XUHOSTPWREN pin control seems to be very complicated.
	   The below is workaround to ignore the interrupt which is generated by 20uF capacity.
	 */
	if (detection_diff == -1) {
		time_end = ktime_get();

		detection_diff = ktime_to_ms(ktime_sub(time_end, time_start));
		if (detection_diff < 500) {
			detection_diff = -1;
			return IRQ_HANDLED;
		}
	}
...
static int exynos_usb_status_init(struct exynos_usb_switch *usb_switch)
{
	...
	/* Now we are ready to accept interrupts*/
	usb_switch->ready = 1;
...
static void exynos_usbswitch_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct exynos_usb_switch *usb_switch = platform_get_drvdata(pdev);
...
static const struct dev_pm_ops exynos_usbswitch_pm_ops = {
	...
	.complete		= exynos_usbswitch_resume,
};
