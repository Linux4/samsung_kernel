				...
				const struct i2c_device_id *dev_id)
{
	...
	for(reg = 0; reg < 0xC; reg++) {
		ret = s2mpb03_read_reg(i2c, reg, &val);
        pr_err("[s2mpb03] dump : reg=0x%x, val=0x%x\n", reg, val);        
	}

#if 0 	/* test */
	{
        struct regulator *reg_id1, *reg_id2, *reg_id3;
        reg_id1 = regulator_get(NULL, "s2mpb03-ldo1");
		ret = regulator_enable(reg_id1);
		if (ret < 0) {
			pr_err("regulator_enable s2mpb03-ldo1 failed: %d\n", ret);
			regulator_put(reg_id1);
		}

        reg_id2 = regulator_get(NULL, "s2mpb03-ldo2");
		ret = regulator_enable(reg_id2);
		if (ret < 0) {
			pr_err("regulator_enable s2mpb03-ldo2 failed: %d\n", ret);
			regulator_put(reg_id2);
		}

        reg_id3 = regulator_get(NULL, "s2mpb03-ldo3");
		ret = regulator_enable(reg_id3);
		if (ret < 0) {
			pr_err("regulator_enable s2mpb03-ldo3 failed: %d\n", ret);
			regulator_put(reg_id3);
		}
