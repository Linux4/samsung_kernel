...
static int ldo_turn_on(struct regulator_dev *rdev)
{
	...
	if (regs->pd_set == ANA_REG_GLB_POWER_PD_SW)
...
static int set_regu_offset(struct regulator_dev *rdev)
{
	...
	efuse_data = 0;//sci_otp_get_offset(regu_name);
...
#define REGS_ANA_GLB_PHYS			(SPRD_ADI_PHYS + 0x8C00)
#define REGS_ANA_GLB_BASE			(SPRD_ADI_BASE + 0x8C00)
#define REGS_ANA_GLB_SIZE			(SZ_2K)
#endif
 ...
 * which named sc2731-regulators.dtsi, BUT register all in regulator driver probe func,
...
static int sci_regulator_probe(struct platform_device *pdev)
{
	...
	pr_info
	    ("sc2731 ana chipid:(0x%08x), ana_mixed_ctl:(0x%08x), otp_sel:(0x%08x)\n",
