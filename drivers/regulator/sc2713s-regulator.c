...
static int dcdc_get_trimming_step(struct regulator_dev *rdev, int to_vol)
{
	...
	if ((0 == strcmp(desc->desc.name, "vddmem")) ||
		(0 == strcmp(desc->desc.name, "vddwrf"))) { /* FIXME: dcdcmem/dcdcwrf step 200/32mV */
...
static int __init regu_driver_init(void)
{
	...
	{/* dcdcarm/dcdccore/dcdcmem common debugfs interface */
		...
		debugfs_create_symlink("vddarm", vol_root, str);
		sprintf(str, "../%s/vddcore/voltage", sci_regulator_driver.driver.name);
		debugfs_create_symlink("vddcore", vol_root, str);
		sprintf(str, "../%s/vddmem/voltage", sci_regulator_driver.driver.name);
		debugfs_create_symlink("vddmem", vol_root, str);
	}
#endif
