...
static int s2mpu15_pmic_probe(struct platform_device *pdev)
{
	...
	ret = s2mpu15_bulk_read(s2mpu15->i2c, S2MPU15_PM1_OFFSRC1_CUR, 2,
			pmic_offsrc);
	if (ret)
		dev_err(&pdev->dev, "failed to read OFFSRC\n");

	/* Clear OFFSRC1, OFFSRC2 register */
	ret = s2mpu15_bulk_write(s2mpu15->i2c, S2MPU15_PM1_OFFSRC1_CUR, 2,
			offsrc_val);
	if (ret)
...
static void s2mpu15_pmic_shutdown(struct platform_device *pdev)
{
	struct s2mpu15_info *s2mpu15 = platform_get_drvdata(pdev);

	pr_info("%s: Set INST_ACOK_EN bit\n", __func__);
	s2mpu15_update_reg(s2mpu15->i2c, S2MPU15_PM1_CFG_PM, (1 << 2), (1 << 2));
}
#endif /* CONFIG_SEC_PM */
...
#if IS_ENABLED(CONFIG_SEC_PM)
	.shutdown = s2mpu15_pmic_shutdown,
#endif
};
