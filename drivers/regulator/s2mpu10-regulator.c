...
u8 s2mpu10_get_id(void)
{
	return static_info->iodev->pmic_rev;
}
EXPORT_SYMBOL_GPL(s2mpu10_get_id);

int s2mpu10_wa_write(struct s2mpu10_info *s2mpu10)
{
	...
	if (chip_id1 >= 1 && chip_id2 >= 1) {
		...
		ret = s2mpu10_write_reg(s2mpu10->i2c, 0x84, 0xF5);
		if (ret < 0) {
			s2mpu10_write_reg(s2mpu10->i2c, 0x84, 0xF5);
			pr_err("%s: 0x184 write error\n", __func__);
			return ret;
		}
		s2mpu10_read_reg(s2mpu10->i2c, 0x84, &val);
		if(val != 0xF5) {
			s2mpu10_write_reg(s2mpu10->i2c, 0x84, 0xF5);
			pr_err("%s: 0x184 write error!\n", __func__);
			return -1;
		...
		ret = s2mpu10_write_reg(s2mpu10->i2c, 0x98, 0x06);
		if (ret < 0) {
			s2mpu10_write_reg(s2mpu10->i2c, 0x98, 0x06);
			pr_err("%s: 0x198 write error\n", __func__);
			return ret;
		}
		s2mpu10_read_reg(s2mpu10->i2c, 0x98, &val);
		if(val != 0x06) {
			s2mpu10_write_reg(s2mpu10->i2c, 0x98, 0x06);
			pr_err("%s: 0x198 write error!\n", __func__);
			return -1;
...
static void s2mpu10_pmic_shutdown(struct platform_device *pdev)
{
	struct s2mpu10_info *s2mpu10 = platform_get_drvdata(pdev);

	s2mpu10_wa_write(s2mpu10);
...
static struct platform_driver s2mpu10_pmic_driver = {
	...
	.shutdown = s2mpu10_pmic_shutdown,
