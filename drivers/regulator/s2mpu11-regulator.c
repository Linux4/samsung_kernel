...
void s2mpu11_oi_function(struct s2mpu11_info *s2mpu11)
{
	...
	s2mpu11_update_reg(i2c, S2MPU11_PMIC_REG_BUCK_OI_PD_EN, 0x00, 0x1F);
...
u8 s2mpu11_get_id(void)
{
	return static_info->iodev->pmic_rev;
}
EXPORT_SYMBOL_GPL(s2mpu11_get_id);

int s2mpu11_wa_write(struct s2mpu11_info *s2mpu11)
{
	u8 val = 0, chip_id1 = 0, chip_id2 = 0;
	int ret = 0;

	chip_id1 = s2mpu10_get_id();
	chip_id2 = s2mpu11_get_id();

	if (chip_id1 >= 1 && chip_id2 >= 1) {
		/* Version : hard coding and write & verify */
		ret = s2mpu11_write_reg(s2mpu11->i2c, 0x42, 0xB5);
		if (ret < 0) {
			s2mpu11_write_reg(s2mpu11->i2c, 0x42, 0xB5);
			pr_err("%s: 0x142 write error\n", __func__);
			return ret;
		}
		s2mpu11_read_reg(s2mpu11->i2c, 0x42, &val);
		if(val != 0xB5) {
			s2mpu11_write_reg(s2mpu11->i2c, 0x42, 0xB5);
			pr_err("%s: 0x142 write error!\n", __func__);
			return -1;
		}
		/* Init cnt : hard coding and write & verify */
		ret = s2mpu11_write_reg(s2mpu11->i2c, 0x56, 0x06);
		if (ret < 0) {
			s2mpu11_write_reg(s2mpu11->i2c, 0x56, 0x06);
			pr_err("%s: 0x156 write error\n", __func__);
			return ret;
		}
		s2mpu11_read_reg(s2mpu11->i2c, 0x56, &val);
		if(val != 0x06) {
			s2mpu11_write_reg(s2mpu11->i2c, 0x56, 0x06);
			pr_err("%s: 0x156 write error!\n", __func__);
			return -1;
