...
static int s5m8767_get_voltage_sel(struct regulator_dev *rdev)
{
	...
	if (ret) {
		pr_err("%s: failed get_voltage_reg(%d)\n", __func__, ret);
		return ret;
	}
		...
		int min_vol, int max_vol)
{
	...
	if (max_vol < desc->min || min_vol > desc->max) {
		pr_err("%s: invalid voltage %d-%d, %d-%d\n", __func__,
			min_vol, max_vol, desc->min, desc->max);
		return -EINVAL;
	}
	...
	if (desc->min + desc->step * selector > max_vol) {
		pr_err("%s: invalid voltage: desc->min(%d), step(%d), selector(%d), \
				max_vol(%d)\n", __func__, desc->min, desc->step,
				selector, max_vol);
		return -EINVAL;
	}
				...
				int min_uV, int max_uV, unsigned *selector)
{
	...
	if (sel < 0) {
		pr_err("%s: failed convert_voltage(%d)\n", __func__, sel);
		return sel;
	}
	ret = s5m8767_get_voltage_register(rdev, &reg);
	if (ret) {
