...
static void max77705_process_pd(struct max77705_usbc_platform_data *usbc_data)
{
	...
	if (pd_data->cc_sbu_short) {
		pd_data->sink_status.available_pdo_num = 1;
		pd_data->sink_status.power_list[1].max_current =
			pd_data->sink_status.power_list[1].max_current > 1800 ?
			1800 : pd_data->sink_status.power_list[1].max_current;
		pd_data->sink_status.has_apdo = false;
	}

	pr_info("%s : current_pdo_num(%d), available_pdo_num(%d), has_apdo(%d)\n", __func__,
		pd_data->sink_status.current_pdo_num, pd_data->sink_status.available_pdo_num, pd_data->sink_status.has_apdo);
...
void max77705_select_pdo(int num)
{
	...
	pd_data->sink_status.selected_pdo_num = num;

	if (pd_data->sink_status.event != CCIC_NOTIFY_PD_EVENT_SINK)
		pd_data->sink_status.event = CCIC_NOTIFY_PD_EVENT_SINK;

	if (pd_data->sink_status.current_pdo_num == pd_data->sink_status.selected_pdo_num) {
...
int max77705_select_pps(int num, int ppsVol, int ppsCur)
{
	...
	if (num > pd_data->sink_status.available_pdo_num) {
		pr_info("%s: request pdo num(%d) is higher taht available pdo.\n", __func__, num);
		return -EINVAL;
	}

	if (!pd_data->sink_status.power_list[num].apdo) {
		pr_info("%s: request pdo num(%d) is not apdo.\n", __func__, num);
		return -EINVAL;
	} else
		pd_data->sink_status.selected_pdo_num = num;

	if (ppsVol > pd_data->sink_status.power_list[num].max_voltage) {
		pr_info("%s: ppsVol is over(%d, max:%d)\n",
			__func__, ppsVol, pd_data->sink_status.power_list[num].max_voltage);
		ppsVol = pd_data->sink_status.power_list[num].max_voltage;
	} else if (ppsVol < pd_data->sink_status.power_list[num].min_voltage) {
		pr_info("%s: ppsVol is under(%d, min:%d)\n",
			__func__, ppsVol, pd_data->sink_status.power_list[num].min_voltage);
		ppsVol = pd_data->sink_status.power_list[num].min_voltage;
	}

	if (ppsCur > pd_data->sink_status.power_list[num].max_current) {
		pr_info("%s: ppsCur is over(%d, max:%d)\n",
			__func__, ppsCur, pd_data->sink_status.power_list[num].max_current);
		ppsCur = pd_data->sink_status.power_list[num].max_current;
	} else if (ppsCur < 0) {
	...
	pd_data->sink_status.pps_voltage = ppsVol;
	pd_data->sink_status.pps_current = ppsCur;

	pr_info(" %s : PPS PDO(%d), voltage(%d), current(%d) is selected to change\n",
		__func__, pd_data->sink_status.selected_pdo_num, ppsVol, ppsCur);
...
void max77705_pd_retry_work(struct work_struct *work)
{
	...
	if (pd_data->sink_status.event == CCIC_NOTIFY_PD_EVENT_DETACH)
		return;

	init_usbc_cmd_data(&value);
	num = pd_data->sink_status.selected_pdo_num;
	...
	if (pd_data->sink_status.power_list[num].apdo) {
		value.opcode = OPCODE_APDO_SRCCAP_REQUEST;
		value.write_data[0] = (num & 0xFF); /* APDO Position */
		value.write_data[1] = (pd_data->sink_status.pps_voltage / 20) & 0xFF; /* Output Voltage(Low) */
		value.write_data[2] = ((pd_data->sink_status.pps_voltage / 20) >> 8) & 0xFF; /* Output Voltage(High) */
		value.write_data[3] = (pd_data->sink_status.pps_current / 50) & 0x7F; /* Operating Current */
		value.write_length = 4;
		value.read_length = 1; /* Result */
		max77705_usbc_opcode_write(g_usbc_data, &value);
	} else {
...
void max77705_pdo_list(struct max77705_usbc_platform_data *usbc_data, unsigned char *data)
{
	...
	pd_data->sink_status.available_pdo_num = temp;
	pr_info("%s : Temp[0x%02x] Data[0x%02x] available_pdo_num[%d]\n",
		__func__, temp, data[1], pd_data->sink_status.available_pdo_num);
	...
	for (i = 0; i < temp; i++) {
		...
		if (!(do_power_nego) &&
			(pd_data->sink_status.power_list[i + 1].max_current != max_current * UNIT_FOR_CURRENT ||
			pd_data->sink_status.power_list[i + 1].max_voltage != max_voltage * UNIT_FOR_VOLTAGE))
			do_power_nego = true;
		...
		pr_info("%s : PDO_Num[%d] MAX_CURR(%d) MAX_VOLT(%d), AVAILABLE_PDO_Num(%d)\n", __func__,
				i, pd_data->sink_status.power_list[i + 1].max_current,
				pd_data->sink_status.power_list[i + 1].max_voltage,
				pd_data->sink_status.available_pdo_num);
	}

	if (usbc_data->pd_data->pdo_list && do_power_nego) {
		pr_info("%s : PDO list is changed, so power negotiation is need\n",
			__func__);
		pd_data->sink_status.selected_pdo_num = 0;
		pd_data->sink_status.event = CCIC_NOTIFY_PD_EVENT_SINK_CAP;
	}

	if (pd_data->sink_status.current_pdo_num != pd_data->sink_status.selected_pdo_num) {
		if (pd_data->sink_status.selected_pdo_num == 0)
...
void max77705_current_pdo(struct max77705_usbc_platform_data *usbc_data, unsigned char *data)
{
	...
	if (!pd_data->sink_status.available_pdo_num)
		do_power_nego = true;

	sel_pdo_pos = ((data[1] >> 3) & 0x07);
	pd_data->sink_status.current_pdo_num = sel_pdo_pos;
	...
	for (i = 0; i < num_of_pdo; ++i) {
		pPower_list = &pd_data->sink_status.power_list[available_pdo_num + 1];
		...
		if (!do_power_nego)
			prev_power_list = pd_data->sink_status.power_list[available_pdo_num + 1];
		...
		case PDO_TYPE_FIXED:
			...
			available_pdo_num++;
 			break;
		case PDO_TYPE_APDO:
			pd_data->sink_status.has_apdo = true;
			available_pdo_num++;
	...
	if (!do_power_nego && (pd_data->sink_status.available_pdo_num != available_pdo_num))
		do_power_nego = true;

	pd_data->sink_status.available_pdo_num = available_pdo_num;
	pr_info("%s : current_pdo_num(%d), available_pdo_num(%d/%d)\n", __func__,
		pd_data->sink_status.current_pdo_num, pd_data->sink_status.available_pdo_num, num_of_pdo);
	...
	if (do_power_nego || pd_data->sink_status.selected_pdo_num == 0) {
		for (i = 0; i < num_of_pdo; ++i) {
			pdo_obj.data = (data[2 + (i * 4)]
				| (data[3 + (i * 4)] << 8)
				| (data[4 + (i * 4)] << 16)
		...
		for (i = 0; i < pd_data->sink_status.available_pdo_num; ++i) {
			pPower_list = &pd_data->sink_status.power_list[i + 1];
...
void max77705_detach_pd(struct max77705_usbc_platform_data *usbc_data)
{
	...
	if (pd_data->sink_status.event != CCIC_NOTIFY_PD_EVENT_DETACH) {
		cancel_delayed_work(&usbc_data->pd_data->retry_work);
		if (pd_data->sink_status.available_pdo_num)
			memset(pd_data->sink_status.power_list, 0, (sizeof(POWER_LIST) * (MAX_PDO_NUM + 1)));
		pd_data->sink_status.rp_currentlvl = RP_CURRENT_LEVEL_NONE;
		pd_data->sink_status.selected_pdo_num = 0;
		pd_data->sink_status.available_pdo_num = 0;
		pd_data->sink_status.current_pdo_num = 0;
		pd_data->sink_status.pps_voltage = 0;
		pd_data->sink_status.pps_current = 0;
 		pd_data->sink_status.has_apdo = false;
		max77705_set_enable_pps(false, 0, 0);
		pd_data->sink_status.event = CCIC_NOTIFY_PD_EVENT_DETACH;
...
static void max77705_notify_prswap(struct max77705_usbc_platform_data *usbc_data, u8 pd_msg)
{
	...
	case PRSWAP_SNKTOSWAP:
		pd_data->sink_status.event = CCIC_NOTIFY_PD_EVENT_PRSWAP_SNKTOSRC;
		pd_data->sink_status.selected_pdo_num = 0;
		pd_data->sink_status.available_pdo_num = 0;
		pd_data->sink_status.current_pdo_num = 0;
		usbc_data->pd_data->psrdy_received = false;
...
void max77705_notify_rp_current_level(struct max77705_usbc_platform_data *usbc_data)
{
	...
	if (usbc_data->plug_attach_done && !usbc_data->pd_data->psrdy_received &&
		...
		rp_currentlvl != pd_data->sink_status.rp_currentlvl &&
		rp_currentlvl >= RP_CURRENT_LEVEL_DEFAULT) {
		pd_data->sink_status.rp_currentlvl = rp_currentlvl;
		pd_data->sink_status.event = CCIC_NOTIFY_PD_EVENT_CCIC_ATTACH;
		pr_info("%s : rp_currentlvl(%d)\n", __func__, pd_data->sink_status.rp_currentlvl);
		max77705_ccic_event_work(usbc_data, CCIC_NOTIFY_DEV_BATTERY,
			CCIC_NOTIFY_ID_POWER_STATUS, 0/*attach*/, 0, 0);
	}
}
...
int max77705_pd_init(struct max77705_usbc_platform_data *usbc_data)
{
	...
	msg_maxim(" IN(%d)", pd_data->sink_status.rp_currentlvl);
	...
	pd_data->sink_status.pps_current = 0;
	pd_data->sink_status.has_apdo = false;
	pd_data->sink_status.fp_sec_pd_select_pdo = max77705_select_pdo;
	pd_data->sink_status.fp_sec_pd_select_pps = max77705_select_pps;
	...
	msg_maxim(" OUT(%d)", pd_data->sink_status.rp_currentlvl);
	return 0;
