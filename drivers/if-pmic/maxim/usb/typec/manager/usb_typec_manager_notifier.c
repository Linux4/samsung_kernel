...
static void manager_muic_event_notifier(struct work_struct *data)
{
...
static void manager_event_notifier(struct work_struct *data)
{
	struct typec_manager_event_work *event_work =
		container_of(data, struct typec_manager_event_work, typec_manager_work);
	MANAGER_NOTI_TYPEDEF manager_event_noti;
...
#ifndef CONFIG_GKI_USB // please modify rtc_time, do_gettimeofday
...
static void water_dry_time_update(int mode)
{
#ifndef CONFIG_GKI_USB // please modify rtc_time, do_gettimeofday
	struct timeval time;
...
static void wVbus_time_update(int mode)
{
#ifndef CONFIG_GKI_USB // please modify rtc_time, do_gettimeofday
	struct timeval time;
				...
				unsigned long action, void *data)
{
	...
	case CCIC_NOTIFY_ID_POWER_STATUS:
		if(p_noti.sub1) /*attach*/
			typec_manager.pd_con_state = 1;	// CCIC_NOTIFY_PD_EVENT_SINK
...
static int __init manager_notifier_init(void)
{
	int ret = 0;
	int notifier_result = 0;
	pccic_data_t pccic_data;
