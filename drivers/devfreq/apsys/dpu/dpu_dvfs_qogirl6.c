...
static void get_dpu_dvfs_status(struct ip_dvfs_status *dvfs_status)
{
	...
	dvfs_status->apsys_cur_volt =
		qogirl6_apsys_val_to_volt(reg->ap_dvfs_voltage_dbg >> 12 & 0x7);
	dvfs_status->vsp_vote_volt =
		qogirl6_apsys_val_to_volt(reg->ap_dvfs_voltage_dbg >> 6 & 0x7);
	dvfs_status->dpu_vote_volt =
		qogirl6_apsys_val_to_volt(reg->ap_dvfs_voltage_dbg >> 3 & 0x7);
	dvfs_status->vdsp_vote_volt = "N/A";

	dvfs_status->vsp_cur_freq =
		qogirl6_vsp_val_to_freq(reg->ap_dvfs_cgm_cfg_dbg >> 3);
	dvfs_status->dpu_cur_freq =
		qogirl6_dpu_val_to_freq(reg->ap_dvfs_cgm_cfg_dbg & 0x7);
