...
void dfs_register_save(void)
{
	...
	if (g_dfs_data->socdump_flag) {
		...
		//ExtB P220714-05401,wenhui.wt, MODIFY, 20220723,socdump debug which module cause the ap cpu hang
		mdelay(1000);
...
static int send_scene_request(unsigned int magic)
{
	...
	if ((g_dfs_data->request_freq > target_freq) &&
		(g_dfs_data->request_freq != g_dfs_data->backdoor_freq))
		target_freq = g_dfs_data->request_freq;
...
int send_freq_request(unsigned int freq)
{
	...
	(g_dfs_data->request_freq == g_dfs_data->backdoor_freq)) {
		err = reset_backdoor();
		if (!err)
			g_dfs_data->request_freq = freq;
		return err;
	}

	for (i = 0; i < g_dfs_data->freq_num; i++) {
		if (freq == g_dfs_data->freq_table[i])
			break;
	}
	if ((g_dfs_data->freq_num == i) && (freq != 0)) {
		dev_err(dev, "request freq %d no support!!\n", freq);
		return -1;
	}
	if (unlikely(freq > g_dfs_data->freq_table[5])) {
		err = set_backdoor();
		if (!err)
			g_dfs_data->request_freq = g_dfs_data->backdoor_freq;
		return err;
...
EXPORT_SYMBOL(send_freq_request);
