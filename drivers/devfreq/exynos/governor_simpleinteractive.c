					...
					unsigned long *freq)
{
	...
	*freq = exynos_pm_qos_min;
	...
	if (data->prev_freq != exynos_df->old_freq) {
	...
	if (exynos_pm_qos_max > exynos_df->old_freq && *freq < exynos_df->old_freq &&
			data->freq_timer.expires > jiffies) {
		*freq = exynos_df->old_freq;
...
out:
	 ...
	 */
	data->prev_freq = exynos_df->old_freq;
