/* linux/drivers/devfreq/exynos/exynos7880_bus_cam.c
 *
 * Copyright (c) 2015 Samsung Electronics Co., Ltd.
 *              http://www.samsung.com
 *
 * Samsung EXYNOS7880 SoC CAM devfreq driver
 *
...
#define DEVFREQ_CAM_REBOOT_FREQ	(200000)

static struct exynos_devfreq_data *cam_data;

static int exynos7880_devfreq_cam_cmu_dump(struct device *dev,
...
static int exynos7880_devfreq_cam_reboot(struct device *dev,
					struct exynos_devfreq_data *data)
{
...
static u32 exynos7880_devfreq_cam_get_target_freq(char *name, u32 freq)
{
	return cal_dfs_get_rate_by_member(dvfs_cam, name, freq);
}

static int exynos7880_devfreq_cam_get_freq(struct device *dev, u32 *cur_freq,
				struct exynos_devfreq_data *data)
{
...
static int exynos7880_devfreq_cam_set_freq(struct device *dev,
...
static int exynos7880_devfreq_cam_init_freq_table(struct device *dev,
					struct exynos_devfreq_data *data)
{
...
static int exynos7880_devfreq_cam_get_volt_table(struct device *dev,
...
static int exynos7880_devfreq_cam_init(struct device *dev,
				struct exynos_devfreq_data *data)
{
...
static int exynos7880_devfreq_cam_exit(struct device *dev,
				struct exynos_devfreq_data *data)
{
	clk_put(data->clk);
...
static int __init exynos7880_devfreq_cam_init_prepare(struct exynos_devfreq_data *data)
{
	data->ops.init = exynos7880_devfreq_cam_init;
	data->ops.exit = exynos7880_devfreq_cam_exit;
	data->ops.get_volt_table = exynos7880_devfreq_cam_get_volt_table;
	data->ops.get_freq = exynos7880_devfreq_cam_get_freq;
	data->ops.set_freq = exynos7880_devfreq_cam_set_freq;
	data->ops.init_freq_table = exynos7880_devfreq_cam_init_freq_table;
	data->ops.get_target_freq = exynos7880_devfreq_cam_get_target_freq;
	data->ops.reboot = exynos7880_devfreq_cam_reboot;
	data->ops.cmu_dump = exynos7880_devfreq_cam_cmu_dump;
...
static int __init exynos7880_devfreq_cam_initcall(void)
{
	if (register_exynos_devfreq_init_prepare(DEVFREQ_CAM,
				exynos7880_devfreq_cam_init_prepare))
		return -EINVAL;

	return 0;
}
fs_initcall(exynos7880_devfreq_cam_initcall);
