				...
				unsigned int *ccnt, unsigned long *pmcnt)
{
	...
	if (filter == PPMU_SET_DDR) {
		max_ccnt = handle->ppmu[PPMU_DMC_0].ccnt + handle->ppmu[PPMU_DMC_1].ccnt;
		max_pmcnt = handle->ppmu[PPMU_DMC_0].count[PPMU_PMNCNT3] +
				handle->ppmu[PPMU_DMC_1].count[PPMU_PMNCNT3];
	} else {
		for (i = start; i <= end; i++) {
