	...
	*target_freq = opp_get_freq(target_opp);
	...
	if (old_freq < *target_freq) {
		if (data->isp_set_volt) {
			...
			register_match_abb(ID_ISP, *target_freq);
			set_match_abb(ID_ISP, get_match_abb(ID_ISP, *target_freq));
		}
...
static int exynos7_devfreq_isp_probe(struct platform_device *pdev)
{
	...
	data = devm_kzalloc(&pdev->dev, sizeof(struct devfreq_data_isp), GFP_KERNEL);
	if (!data) {
	...
	exynos7_devfreq_isp_profile.freq_table = devm_kzalloc(&pdev->dev, sizeof(int) * data->max_state, GFP_KERNEL);
	...
	data->vdd_isp_cam0 = devm_regulator_get(&pdev->dev, "vdd_intmif");

	rcu_read_lock();
	freq = exynos7_devfreq_isp_governor_data.cal_qos_max;
	target_opp = devfreq_recommended_opp(data->dev, &freq, 0);
	if (IS_ERR(target_opp)) {
		rcu_read_unlock();
		dev_err(data->dev, "DEVFREQ(ISP) : Invalid OPP to set voltagen");
		ret = PTR_ERR(target_opp);
		goto err_data;
	}
	...
	regulator_set_voltage(data->vdd_isp_cam0, data->old_volt, REGULATOR_MAX_MICROVOLT);
...
static int exynos7_devfreq_isp_resume(struct device *dev)
{
	...
	pm_qos_update_request(&exynos7_isp_qos, data->default_qos);

	return 0;
}

static const struct dev_pm_ops exynos7_devfreq_isp_pm = {
