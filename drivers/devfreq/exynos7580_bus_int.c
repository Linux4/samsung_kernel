	...
	*target_freq = opp_get_freq(target_opp);
	...
	if (old_freq < *target_freq) {
		if (data->int_set_volt) {
			...
			register_match_abb(ID_INT, *target_freq);
			set_match_abb(ID_INT, get_match_abb(ID_INT, *target_freq));
		}
...
static int exynos7_devfreq_int_probe(struct platform_device *pdev)
{
	...
	exynos7_devfreq_int_profile.freq_table = devm_kzalloc(&pdev->dev, sizeof(int) * data->max_state, GFP_KERNEL);
	if (!exynos7_devfreq_int_profile.freq_table) {
	...
	data->vdd_int = devm_regulator_get(&pdev->dev, "vdd_intmif");

	rcu_read_lock();
	freq = exynos7_devfreq_int_governor_data.cal_qos_max;
	...
	regulator_set_voltage(data->vdd_int, data->old_volt, REGULATOR_MAX_MICROVOLT);
...
static int exynos7_devfreq_int_resume(struct device *dev)
{
	...
	pm_qos_update_request(&exynos7_int_qos, data->default_qos);

	return 0;
}

static const struct dev_pm_ops exynos7_devfreq_int_pm = {
