...
#ifndef CONFIG_USB_PORT_POWER_OPTIMIZATION
int is_rewa_enable;
#endif
							...
							int is_enable, int is_cancel)
{
	...
	if (is_cancel == 0) {
		...
		if (gpio_val) {
			...
			dev_info(phy_drd->dev, "%s: gpio value: %d, airplane mode\n",
					__func__, gpio_val);
#ifdef CONFIG_USB_SS_REMOTE_WAKEUP
			phy_exynos_usb3p1_u3_rewa_enable(&phy_drd->usbphy_info, 0);
#endif
...
static int exynos_usbdrd_phy_probe(struct platform_device *pdev)
{
	...
	phy_drd->vdd085_usb = regulator_get(dev, "vdd_ldo8");
	...
	phy_drd->vdd18_usb = regulator_get(dev, "vdd_ldo9");
	...
	phy_drd->vdd33_usb = regulator_get(dev, "vdd_ldo10");
