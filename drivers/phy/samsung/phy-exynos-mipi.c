...
static int __set_phy_cfg_0503_0008_dphy(void __iomem *regs, int option, u32 *cfg)
{
	...
	if (cfg[SPEED] >= PHY_REF_SPEED) {
		...
		if (cfg[SPEED] >= 3000 && cfg[SPEED] <= 4500)
			...
			skew_delay_sel = 2;
		else if (cfg[SPEED] >= 0 && cfg[SPEED] < 2000)
			skew_delay_sel = 3;
		else
			skew_delay_sel = 0;
	}
	...
	/* clock */
	writel(0x00000080, regs + 0x100C); /* DPHY_ACTRL_SC_03 */
	writel(0x00000038, regs + 0x1010); /* DPHY_ACTRL_SC_04 */
	writel(0x000000F8, regs + 0x1014); /* DPHY_ACTRL_SC_05 */
	
	writel(0x00000002, regs + 0x1018); /* DPHY_ACTRL_SC_06 */

	/* data */
	for (i = 0; i <= cfg[LANES]; i++) {
		writel(0x00000080, regs + 0x1428 + (i * 0x400)); /* DPHY_ACTRL_SD_03 */
		writel(0x00000038, regs + 0x142C + (i * 0x400)); /* DPHY_ACTRL_SD_04 */
		writel(0x000000F8, regs + 0x1430 + (i * 0x400)); /* DPHY_ACTRL_SD_05 */
