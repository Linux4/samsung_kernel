...
static int additional_cr_reg_update(struct exynos_usbphy_info *info)
{
	...
	/* unused lane off @ power down mode */
#if 0
	if (info->used_phy_port == 1) {
		phy_exynos_snps_usbdp_cr_write(info, 0x402f, 0xf);
		phy_exynos_snps_usbdp_cr_write(info, 0x412f, 0xf);
		phy_exynos_snps_usbdp_cr_write(info, 0x3018, 0xf);
		phy_exynos_snps_usbdp_cr_write(info, 0x3118, 0xf);
		/* disable sigdet for lane 1 RX */
		phy_exynos_snps_usbdp_cr_write(info, 0x4135, 0x2a);

	} else {
		phy_exynos_snps_usbdp_cr_write(info, 0x422f, 0xf);
		phy_exynos_snps_usbdp_cr_write(info, 0x432f, 0xf);
		phy_exynos_snps_usbdp_cr_write(info, 0x3218, 0xf);
		phy_exynos_snps_usbdp_cr_write(info, 0x3318, 0xf);
		/* disable sigdet for lane 2 RX */
		phy_exynos_snps_usbdp_cr_write(info, 0x4235, 0x2a);
	}
#endif
	/* set tune parameter */
