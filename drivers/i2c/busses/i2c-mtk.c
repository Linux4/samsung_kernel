...
static int mt_i2c_do_transfer(struct mt_i2c *i2c)
{
    ...
    //begin Bug 527462 determine if there is a host transport error,dulinyu.wt, 20200107
    if (i2c->dev_comp->control_irq_sel == 1)
		control_reg |= I2C_CONTROL_IRQ_SEL;
	i2c_writew(control_reg, i2c, OFFSET_CONTROL);
    //end   Bug 527462 determine if there is a host transport error,dulinyu.wt, 20200107
	/* set start condition */
	if (speed_hz <= 100000)
	...
	if (i2c->ch_offset || (i2c->dev_comp->control_irq_sel == 1))//Bug 527462,check the host transport error,dulinyu.wt, 20200107
		int_reg &= ~(I2C_HS_NACKERR | I2C_ACKERR);
	/* Clear interrupt status */
...
int mt_i2c_parse_comp_data(void)
{
    ...
    of_property_read_u8(comp_node, "control_irq_sel",
		(u8 *)&i2c_common_compat.control_irq_sel);//Bug 527462,read property "control_irq_sel",dulinyu.wt, 20200107
	return 0;
}
