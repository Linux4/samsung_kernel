...
static int exynos5_i2c_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(dev->of_node, "samsung,weak-pull-up-mask", &i2c->ps_cfg);
	if (!ret) {
		ret = of_property_read_u32(dev->of_node, "samsung,weak-pud-val", &i2c->weak_pud_val);
		if (!ret) {
			i2c->ps_reg = syscon_regmap_lookup_by_phandle(dev->of_node,
					"samsung,ps-phandle");
			if (IS_ERR(i2c->ps_reg)) {
				dev_err(dev, "failed to get sysreg phandle\n");
			} else {
				i2c->weak_pud_en = 1;
				regmap_update_bits(i2c->ps_reg, 0x0, PS_MASK << i2c->ps_cfg,
						i2c->weak_pud_val << i2c->ps_cfg);
			}
		}
	}
