...
qup_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
    ...
    #if defined(CONFIG_MACH_KS01EUR)
		dev->clk_ctl = QUP_I2C_SCL_NOISE_REJECTION(dev->clk_ctl, dev->pdata->noise_rjct_scl);
		dev->clk_ctl = QUP_I2C_SDA_NOISE_REJECTION(dev->clk_ctl, dev->pdata->noise_rjct_sda);
    #endif
