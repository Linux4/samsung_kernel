...
static void bma254_work_func(struct work_struct *work)
{
	...
	input_report_rel(bma254->input, REL_X, acc.x);
	input_report_rel(bma254->input, REL_Y, acc.y);
	input_report_rel(bma254->input, REL_Z, acc.z);
...
static int bma254_get_motion_interrupt(struct i2c_client *client)
{
	...
	pr_info("%s: ACC_INT_STATUS %x comres %d\n", __func__, data,comres);
	if (data & 0x04)
		result = 1;
		else
...
static void bma254_set_motion_interrupt(struct i2c_client *client, bool enable, bool factorytest)
{
	...
	if (enable) {
		...
		data = 0x04;
		bma254_smbus_write_byte(client, BMA254_EN_INT1_PAD_SLOPE__REG,
                                                    &data);
		mdelay(3);

		data =  0x02; /*0x02: Count3(Ignore 2 interrupts), 0x03:Count4*/
		bma254_smbus_write_byte(client, BMA254_SLOPE_DUR__REG,
		                                        &data);
		mdelay(3);
		if (factorytest){
			data = 0x00;
			bma254_smbus_write_byte(client, BMA254_SLOPE_THRES__REG,
								&data);
		}
		else{
		#if defined(CONFIG_MACH_CS02) || defined(CONFIG_MACH_CS05)
			data = 0x0c;
		#else
			data = 0x0a; //12->a
		#endif
		...
		mdelay(3);
		data = 0x07;
		bma254_smbus_write_byte(client, BMA254_INT_ENABLE1_REG,
		                                        &data);
	}
	else {
		data = 0x00;
							...
							size_t count)
{
	...
	default:
		err = -EINVAL;
		pr_err("%s: invalid value %d and err %d\n", __func__, *buf,err);
		return count;
	}
...
static int bma254_setup_irq(struct bma254_data *bma254)
{
	...
	rc = gpio_request(gpio,"gpio_acc_int");
    if (rc < 0) {
