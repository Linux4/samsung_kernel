...
static void isa1000_work_func(struct work_struct *work)
{
	...
	struct hrtimer *timer = &ddata->timer;
        ...
        if (ddata->timeout) {
            ...
            if (ddata->pdata->ldo_type == INTERNAL_LDO) {
			...
			struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "period: %u\n", drvdata->pdata->period);
}
static DEVICE_ATTR(period, 0660, period_show, period_store);
	...
	isa1000_get_devtree_pdata(struct device *dev)
{
	...
	of_property_read_u32(child_node, "isa1000,max_timeout", &pdata->max_timeout);
         of_property_read_u32(child_node, "isa1000,ldo_type", &pdata->ldo_type);
...
static int isa1000_probe(struct platform_device *pdev)
{
	...
	struct isa1000_ddata *ddata;
         struct pinctrl *isa1000_default = NULL;
	int ret = 0;
         printk(KERN_ERR "[VIB] isa1000_probe\n");
	...
	if (!pdata) {
		...
		if (IS_ERR(pdata)) {
			...
			goto err_pdata;
                ...
                isa1000_default = devm_pinctrl_get_select_default(&pdev->dev);
                if (IS_ERR(isa1000_default)) {
                    if (PTR_ERR(isa1000_default) == -EPROBE_DEFER){
                    		ret = -EPROBE_DEFER;
            	        goto err_pdata;
                    }
	...
	ddata->pwm = pwm_request(ddata->pdata->pwm_id, "mediatek-disp-pwm");
	if (IS_ERR(ddata->pwm)) {
	...
	platform_set_drvdata(pdev, ddata);
         ...
         printk(KERN_ERR "[VIB] isa1000_probe end\n");
