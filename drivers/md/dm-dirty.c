...
//		DMERR("Not page size IO: size:%u, sector:%llu", bio->bi_size, (u64)bio->bi_sector);
		DMERR("Not page size IO\n");
		return -EIO;
	}

	if(sector_to_block(v, bio->bi_sector) + bytes_to_block(v, bio->bi_size) > v->data_blocks) {
//		DMERR("Out of range. sector:%llu, size:%u", bio->bi_sector, (u64)bio->bi_size);
		DMERR("Out of range \n");
		return -EIO;
	}
...
static int finish_verity_tree(struct dm_verity *v)
{
	...
	if(dirty_update_hash(v, hash_block, desc, result)) {
		...
		DMERR("Failed to calculate root hash\n");
		goto free_desc;
	}
