...
void print_blks_cnt(char *dev_name)
{
	...
	pr_err("dev_name = %s, total_blks = %llu, skipped_blks = %llu, corrupted_blks = %llu, fec_correct_blks = %llu",
			dev_name, get_total_blks(), get_skipped_blks(), get_corrupted_blks(), get_fec_correct_blks());
...
void print_fc_blks_list(void)
{
	...
	if (get_fec_correct_blks() >= (long long) MAX_FC_BLKS_LIST) {
		pr_err("%s %llu", b_info->dev_name[b_info->list_idx],
				(unsigned long long)b_info->fc_blks_list[b_info->list_idx]);
