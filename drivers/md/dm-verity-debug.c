...
struct blks_info * get_b_info(char* dev_name){
    printk(KERN_ERR "dm-verity-debug : dev_name = %s\n",dev_name);

    if(empty_b_info())
        b_info = create_b_info();
    else{
        printk(KERN_INFO "dm-verity-debug : b_info already exists\n");
        return b_info;
    }

    if(!b_info){
        printk(KERN_ERR "dm-verity-debug : Can't get b_info !\n");
        return 0;
    }

    printk(KERN_INFO "dm-verity-debug : get b_info successfully\n");
    return b_info;
}
...
void add_fc_blks_entry(sector_t cur_blk, char* dev_name){
    if(empty_b_info()){
        printk(KERN_ERR "dm-verity-debug : b_info is empty !\n");
        return;
    }
...
void print_blks_cnt(char* dev_name){
    ...
    printk(KERN_ERR "dev_name = %s,total_blks = %llu,skipped_blks = %llu,corrupted_blks = %llu,fec_correct_blks = %llu",dev_name,get_total_blks(),get_skipped_blks(),get_corrupted_blks(),get_fec_correct_blks());

    if(foc > 0){
        printk(KERN_ERR "fec_off_cnt = %d",foc);
        for(i = 0 ; i < foc; i++)
            printk(KERN_ERR "fec_off_dev = %s ",b_info->fec_off_list[i]);
    }
}
