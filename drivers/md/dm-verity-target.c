...
int verity_map(struct dm_target *ti, struct bio *bio)
{
    ...
    if ((total_blks - prev_total_blks) > 0x1000) {
        ...
        DMERR_LIMIT("total_blks=%lu skipped_blks=%lu delta=%lu, device=%s", (long unsigned int)total_blks, (long unsigned int)skipped_blks, (long unsigned int)(total_blks-skipped_blks), v->data_dev->name);
    } 
#endif
