				...
				const char *buf, size_t len)
{
	...
	int ret = 0;
	ret = lpm_set_mode(0xF0, 0x77770000, 0);
	return len;
#if 0
	int ret = 0, i = 0;
	int cpu;
	...
	printk("[LPM] %s Cluster LPM Enable [%s] :\n",power_cluster->cluster_name, buf);
	for (i = 0; i < power_cluster->nlevels; i++) {
		kp.arg = &power_cluster->levels[i].available.idle_enabled;
		ret = param_set_bool(buf, &kp);
	}
	for_each_cpu(cpu, &power_cluster->child_cpus) {
		level_list = cpu_level_available[cpu];
		kp.arg = &level_list[0].idle_enabled; /* WFI  */
		ret = param_set_bool(buf, &kp);
		if (!ret)
			return -EINVAL;
		kp.arg = &level_list[2].idle_enabled; /* PC   */
		ret = param_set_bool(buf, &kp);
		if (!ret)
			return -EINVAL;
		kp.arg = &level_list[3].idle_enabled; /* SPC  */
		ret = param_set_bool(buf, &kp);
		if (!ret)
			return -EINVAL;
	}
