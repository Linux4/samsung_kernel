		...
		const struct cpumask *cpu, int child_idx, bool from_idle)
{
	...
	if (level->notify_rpm) {
		...
		if (poweroff_charging !=1)
		{
			//	pr_err("%s: schedule work for dummy votes\n", __func__);
			schedule_work(&dummy_vote_work);
		}
#endif
#ifdef CONFIG_RPM_WAIT_FOR_ACK_DEBUG_PATCH
		rpm_smd_int_sts = 0x2;
