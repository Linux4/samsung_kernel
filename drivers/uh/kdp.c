...
struct task_security_struct_kdp init_sec __kdp_ro;
...
void __init kdp_init(void)
{
	...
	cred.sp_size		= sizeof(struct task_security_struct_kdp);
	...
	cred.bp_cred_secptr	= offsetof(struct task_security_struct_kdp, bp_cred);
...
void __init kdp_cred_init(void)
{
	if (kdp_enable) {
		...
		tsec_jar = kmem_cache_create("tsec_jar", sizeof(struct task_security_struct_kdp),
...
void *cred_init_security_kdp(struct cred *cred)
{
	struct task_security_struct_kdp *tsec = &init_sec;
	tsec->bp_cred = cred;
	cred->security = tsec;
...
{
	struct task_security_struct_kdp *tsec = (struct task_security_struct_kdp *)sec_ptr;
	...
	if (!is_kdp_protect_addr(cred) ||
		...
		printk(KERN_ERR, "[KDP] cred: %d, sp: %d",
