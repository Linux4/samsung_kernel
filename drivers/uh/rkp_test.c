...
static void hyp_check_range_rw(u64 va, u64 count, u64 *ro, u64 *rw)
{
	unsigned long  flags;
	raw_spin_lock_irqsave(&par_lock, flags);
	uh_call(UH_APP_RKP, CMD_ID_TEST_GET_RO, (unsigned long)va, (unsigned long)count, (unsigned long) ro, (unsigned long)rw);
	*ro = *ha1;
	*rw = *ha2;
	raw_spin_unlock_irqrestore(&par_lock, flags);
}
...
#else
	u64 fixmap_va = __fix_to_virt(FIX_TEXT_POKE0);
#endif
...
#else
		{((u64) __end_rodata),	((u64)fixmap_va)-((u64) __end_rodata),	"ERODATA - FIXMAP", false, true},
		{((u64)fixmap_va),		((u64) PAGE_SIZE),			"     FIXMAP     ", true, false},
		{((u64)fixmap_va+PAGE_SIZE),	((u64) MEM_END-((u64)fixmap_va+PAGE_SIZE)),	"FIXMAP - MEM_END", false, true},
#endif 
