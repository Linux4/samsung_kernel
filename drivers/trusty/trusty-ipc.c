...
static struct tipc_msg_buf *_get_txbuf_locked(struct tipc_virtio_dev *vds)
{
	...
	if (vds->free_msg_buf_cnt) {
		...
		if (vds->msg_buf_cnt >= vds->msg_buf_max_cnt) {
			pr_emerg("%s: vds->msg_buf_cnt %d\n", __func__, vds->msg_buf_cnt);
		...
		if (!mb) {
			pr_emerg("%s: alloc_msg_buff no memory\n", __func__);
...
int tipc_chan_queue_msg(struct tipc_chan *chan, struct tipc_msg_buf *mb)
{
	...
	case TIPC_CONNECTED:
		...
		if (!msg_buf_list) {
			...
			pr_emerg("%s: failed to allocate msg_buf_list(%d)\n",
			       __func__, err);
			return err;
		}
...
static ssize_t tipc_write_iter(struct kiocb *iocb, struct iov_iter *iter)
{
	...
	while (copyed_len < total_len) {
		...
		if (IS_ERR(txbuf)) {
			if (copyed_len == 0) {
				pr_emerg("get txbuf failed! no memory\n");
				ret = -ENOMEM;
				goto err_out;
			} else {
				break;
...
static int tipc_virtio_probe(struct virtio_device *vdev)
{
	...
	for (i = 0; i < rxvq_num; i++) {
		...
		if (!rxbuf) {
			...
			pr_emerg("%s: alloc msg buff o memory\n", __func__);
			err = -ENOMEM;
