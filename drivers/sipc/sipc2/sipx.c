...
inline uint16_t SIPX_GEN_DESC(uint32_t length, uint16_t offset)
{
	return ((length & 0x7ff) | ((offset & 0x1F) << 11));
}

inline void SIPX_PARSE_DESC(uint16_t desc, uint32_t *length, uint16_t *offset)
{
	*length = (desc & 0x7ff);
	*offset = (desc >> 11) & 0x1F;
}
...
int sipx_get_item_from_pool(struct sipx_pool *pool, struct sblock *blk)
{
	unsigned long flags;
	int pos;
	int ret = 0;
...
int sipx_put_item_back_to_pool(struct sipx_pool *pool, struct sblock *blk)
{
	unsigned long flags;
	int pos;
	int ret = 0;
...
int sipx_put_item_into_pool(struct sipx_pool *pool, struct sblock *blk)
{
	unsigned long flags;
	int pos;
	int ret = 0;
	...
	if ((wr - rd) < pool->fifo_size) {
		...
		/*smp_wmb();
		pool->fifo_buf[pos].index = blk->index;
		(*(uint32_t*)(&pool->fifo_buf[pos])) = *((uint32_t*)&item); */
		(*(uint32_t *)(&pool->fifo_buf[pos])) = item;
		pool->fifo_info->fifo_wrptr = wr + 1;
	} else {
		SIPX_ERR("sipx_put_item_into_pool fail: wr:0x%x, rd:0x%x, size:%d\n",
...
int sipx_get_pool_free_count(struct sipx_pool *pool)
{
	unsigned long flags;
	int ret = 0;
...
int sipx_put_item_to_ring(struct sipx_ring *ring, struct sblock *blk,
		int *yell, int *full)
{
	unsigned long flags;
	int pos;
	...
	(*(uint32_t *)(&ring->fifo_buf[pos])) = (*(uint32_t *)&item);
...
int sipx_get_item_from_ring(struct sipx_ring *ring, struct sblock *blk)
{
	unsigned long flags;
	int pos;
	int ret = 0;
	...
	if (wr == rd)
		...
		/* force compiler do one 4-bytes operation */
		/* item = ring->fifo_buf[pos]; */
		(*(uint32_t *)&item) = (*(uint32_t *)(&ring->fifo_buf[pos]));
...
int sipx_get_ring_item_cnt(struct sipx_ring *ring)
{
	unsigned long flags;
	int ret = 0;
		...
		struct sipx_channel *sipx_chan)
{
	...
	if (sipx_chan->dl_record_flag) {
		...
		SIPX_INFO("sipx_recover_channel dl_record_flag = 1\n");
	...
	while (fifo_info->fifo_wrptr != fifo_info->fifo_rdptr) {
		...
		SIPX_INFO("sipx_recover_channel dl_ring fifo_info: 0x%p, wrptr: 0x%x, rdptr:0x%x\n",
				(void *)fifo_info,
	...
	while (fifo_info->fifo_wrptr != fifo_info->fifo_rdptr) {
		...
		SIPX_INFO("sipx_recover_channel dl_ack_ring fifo_info:0x%p, wrptr:0x%x, rdptr:0x%x\n",
				(void *)fifo_info,
				(unsigned int)fifo_info->fifo_wrptr,
				(unsigned int)fifo_info->fifo_rdptr);
	...
	while (fifo_info->fifo_wrptr != fifo_info->fifo_rdptr) {
		...
		SIPX_INFO("sipx_recover_channel ul_ring fifo_info:0x%p, wrptr:0x%x, rdptr:0x%x\n",
				(void *)fifo_info,
				(unsigned int)fifo_info->fifo_wrptr,
				(unsigned int)fifo_info->fifo_rdptr);
	...
	while (fifo_info->fifo_wrptr != fifo_info->fifo_rdptr) {
		...
		SIPX_INFO("sipx_recover_channel ul_ack_ring fifo_info:0x%p, wrptr:0x%x, rdptr:0x%x\n",
				(void *)fifo_info,
				(unsigned int)fifo_info->fifo_wrptr,
				(unsigned int)fifo_info->fifo_rdptr);
	...
	if (sipx_chan->ul_record_flag) {
		...
		SIPX_INFO("sipx_recover_channel ul_record_flag = 1\n");
...
static int sipx_recover(uint8_t dst)
{
	...
	for (i = 0; i < SMSG_VALID_CH_NR; i++) {
		...
		if (sipx_chan && (SBLOCK_STATE_READY == sipx_chan->state))
...
static int sipx_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (rval) {
			SIPX_INFO("non-handled sipx_chan msg: %d-%d, %d, %d, %d\n",
					sipx_chan->dst, sipx_chan->channel,
					mrecv.type, mrecv.flag, mrecv.value);
			rval = 0;
		}
...
int create_sipx_pool_ctrl(struct sipx_pool **out,
		volatile struct sipx_fifo_info *fifo_info,
		void *fifo_virt,
		void *blk_virt,
		struct sipx_mgr *sipx)
{
	...
	pool = kzalloc(sizeof(struct sipx_pool), GFP_KERNEL);
	if (!pool) {
...
int destroy_spix_mgr(struct sipx_mgr *sipx)
{
	kfree(sipx->dl_pool);
...
int create_sipx_mgr(struct sipx_mgr **out, struct sipx_init_data *pdata)
{
	struct sipx_mgr *sipx = NULL;
	volatile struct sipx_fifo_info *fifo_info;
	volatile uint32_t *chan_cnt;
	uint32_t offset_chan_cnt;
	uint32_t offset_dl_blk;
	uint32_t offset_dl_fifo;
	uint32_t offset_dl_ack_blk;
	uint32_t offset_dl_ack_fifo;
	uint32_t offset_ul_blk;
	uint32_t offset_ul_fifo;
	uint32_t offset_ul_ack_blk;
	uint32_t offset_ul_ack_fifo;
	uint32_t total_size = 0;
	void *p = NULL;
	int ret = 0;


	sipx = kzalloc(sizeof(struct sipx_mgr) , GFP_KERNEL);
	if (!sipx) {
	...
	sipx->blk_size = SBLOCKSZ_ALIGN(NORMAL_BLOCK_SIZE + SIPX_BLOCK_PENDING,
			SBLOCK_ALIGN_BYTES);
	sipx->ack_blk_size = SBLOCKSZ_ALIGN(ACK_BLOCK_SIZE + SIPX_BLOCK_PENDING,
			SBLOCK_ALIGN_BYTES);
	...
	/* channels smem ptr */
	total_size += sizeof(uint32_t) * (SMSG_VALID_CH_NR + 1);
	...
	sipx->smem_size = SBLOCKSZ_ALIGN(total_size, SBLOCK_ALIGN_BYTES);
	...
	fifo_info->blks_addr = sipx->smem_addr + offset_dl_ack_blk;
	fifo_info->fifo_size = sipx->dl_ack_pool_size;
	fifo_info->blk_size = sipx->ack_blk_size;
	fifo_info->fifo_rdptr = 0;
	fifo_info->fifo_wrptr = 0;
	fifo_info->fifo_addr = sipx->smem_addr + offset_dl_ack_fifo;
#ifdef CONFIG_SIPCMEM_CACHE_EN
	...
	/* offset of smem_addr */
	fifo_info->blks_addr = sipx->smem_addr + offset_ul_blk;
	...
	/* offset of smem_addr */
	fifo_info->fifo_addr = sipx->smem_addr + offset_ul_fifo;
#ifdef CONFIG_SIPCMEM_CACHE_EN
	...
	fifo_info->blks_addr = sipx->smem_addr + offset_ul_ack_blk;
	fifo_info->fifo_size = sipx->ul_ack_pool_size;
	fifo_info->blk_size = sipx->ack_blk_size;
	fifo_info->fifo_rdptr = 0;
	fifo_info->fifo_wrptr = 0;
	fifo_info->fifo_addr = sipx->smem_addr + offset_ul_ack_fifo;
	...
	/* channel count */
	chan_cnt = (volatile uint32_t *)((sipx->smem_virt) +
			offset_chan_cnt);
	*chan_cnt = SMSG_VALID_CH_NR;
#ifdef CONFIG_SIPCMEM_CACHE_EN
	/* save pool start pos */
...
static int sipx_parse_dt(struct sipx_init_data **init, struct device *dev)
{
	...
	pdata = devm_kzalloc(dev, sizeof(struct sipx_init_data), GFP_KERNEL);
	if (!pdata)
		return -ENOMEM;


	ret = of_property_read_string(np,
		"sprd,name", (const char **)&pdata->name);
	if (ret)
		goto error;

	ret = of_property_read_u32(np, "sprd,dst",
		(uint32_t *)&data);
...
static inline void sipx_destroy_pdata(struct sipx_init_data **init, struct device *dev)
{
...
EXPORT_SYMBOL(sipx_get_ack_blk_len);
...
int sipx_get(uint8_t dst, uint8_t channel, struct sblock *blk, int is_ack)
{
	...
	if (ch_index == INVALID_CHANEL_INDEX) {
		SIPX_ERR("%s:channel %d is not be configed!\n", __func__, channel);
		return -EINVAL;
	}

	if (!sipx) {
		SIPX_ERR("sipx-%d-%d sipx_get sipx not exist!\n",
			dst, channel);
		return -ENODEV;
	}
	...
	if (!sipx_chan) {
		SIPX_ERR("sipx-%d-%d sipx_get sipx_get not exist\n",
			dst, channel);
		return -ENODEV;
	}

	if (sipx_chan->state != SBLOCK_STATE_READY) {
		SIPX_ERR("sipx-%d-%d sipx_get not ready!\n",
			dst, channel);
		return -EIO;
	}
	...
	if (is_ack) {
		...
		if (ret) {
			SIPX_INFO("sipx_get %d-%d ack pool is empty!\n",
					dst, channel);
	...
	if (ret) {
		SIPX_INFO("sipx_get %d-%d ack:%d is empty!\n",
				dst, channel, is_ack);
		return -EAGAIN;
	}
...
int sipx_put(uint8_t dst, uint8_t channel, struct sblock *blk)
{
	...
	if (!sipx) {
		SIPX_ERR("sipx-%d-%d sipx_put sipx not exist!\n",
			dst, channel);
		return -ENODEV;
	}
	...
	if (!sipx_chan) {
		SIPX_ERR("sipx-%d-%d sipx_put sipx_get not exist\n",
			dst, channel);
		return -ENODEV;
	}

	if (sipx_chan->state != SBLOCK_STATE_READY) {
		SIPX_ERR("sipx-%d-%d sipx_put not ready!\n",
			dst, channel);
		return -EIO;
	}
...
EXPORT_SYMBOL(sipx_put);

enum hrtimer_restart sipx_ul_trigger_timer_handler(struct hrtimer *timer)
{
	struct sipx_channel *sipx_chan = container_of(timer,
			struct sipx_channel,
			ul_timer);
	struct smsg mevt;
	unsigned long flags;
...
void sipx_force_ul_trigger(struct sipx_channel *sipx_chan)
{
	struct smsg mevt;
	unsigned long flags;
...
void sipx_start_trigger_timer(struct sipx_channel *sipx_chan)
{
...
void sipx_try_trigger_cp(struct sipx_channel *sipx_chan)
{
	unsigned long flags;
...
int sipx_flush(uint8_t dst, uint8_t channel)
{
	...
	if (!sipx) {
		SIPX_ERR("sipx-%d-%d sipx_flush sipx not exist!\n",
			dst, channel);
		return -ENODEV;
	}
	...
	if (!sipx_chan) {
		SIPX_ERR("sipx-%d-%d sipx_flush sipx_get not exist\n",
			dst, channel);
		return -ENODEV;
	}

	if (sipx_chan->state != SBLOCK_STATE_READY) {
		SIPX_ERR("sipx-%d-%d sipx_flush not ready!\n",
			dst, channel);
		return -EIO;
	}
...
int sipx_send(uint8_t dst, uint8_t channel, struct sblock *blk)
{
	...
	if (!sipx) {
		SIPX_ERR("sipx-%d-%d sipx_send sipx not exist!\n",
			dst, channel);
		return -ENODEV;
	}
	...
	if (!sipx_chan) {
		SIPX_ERR("sipx-%d-%d sipx_send sipx_get not exist\n",
			dst, channel);
		return -ENODEV;
	}

	if (sipx_chan->state != SBLOCK_STATE_READY) {
		SIPX_ERR("sipx-%d-%d sipx_send not ready!\n",
			dst, channel);
		return -EIO;
	}
...
int sipx_receive(uint8_t dst, uint8_t channel, struct sblock *blk)
{
	...
	if (!sipx) {
		SIPX_ERR("sipx-%d-%d sipx_receive sipx not exist!\n",
			dst, channel);
		return -ENODEV;
	}
	...
	if (!sipx_chan) {
		SIPX_ERR("sipx-%d-%d sipx_receive sipx_get not exist\n",
			dst, channel);
		return -ENODEV;
	}

	if (sipx_chan->state != SBLOCK_STATE_READY) {
		SIPX_ERR("sipx-%d-%d sipx_receive not ready!\n",
			dst, channel);
		return -EIO;
	}
...
int sipx_release(uint8_t dst, uint8_t channel, struct sblock *blk)
{
	...
	if (!sipx) {
		SIPX_ERR("sipx-%d-%d seblock_release sipx not exist!\n",
			dst, channel);
		return -ENODEV;
	}
	...
	if (!sipx_chan) {
		SIPX_ERR("sipx-%d-%d seblock_release sipx_get not exist\n",
			dst, channel);
		return -ENODEV;
	}

	if (sipx_chan->state != SBLOCK_STATE_READY) {
		SIPX_ERR("sipx-%d-%d seblock_release not ready!\n",
			dst, channel);
		return -EIO;
	}

	SIPX_DEBUG("seblock_release: dst=%d, channel=%d, addr=%p, len=%d, offset=%d\n",
			dst, channel, blk->addr, blk->length, blk->offset);
...
int sipx_get_arrived_count(uint8_t dst, uint8_t channel)
{
	...
	if (!sipx) {
		SIPX_ERR("sipx-%d-%d sipx_get_arrived_count sipx not exist!\n",
			dst, channel);
		return -ENODEV;
	}
	...
	if (!sipx_chan) {
		SIPX_ERR("sipx-%d-%d sipx_get_arrived_count sipx_get not exist\n",
			dst, channel);
		return -ENODEV;
	}

	if (sipx_chan->state != SBLOCK_STATE_READY) {
		SIPX_ERR("sipx-%d-%d sipx_get_arrived_count not ready!\n",
			dst, channel);
		return -EIO;
	}
...
EXPORT_SYMBOL(sipx_get_arrived_count);
...
int sipx_get_free_count(uint8_t dst, uint8_t channel)
{
	...
	if (!sipx) {
		SIPX_ERR("sipx-%d-%d sipx_get_free_count sipx not exist!\n",
			dst, channel);
		return -ENODEV;
	}
	...
	if (!sipx_chan) {
		SIPX_ERR("sipx-%d-%d sipx_get_free_count sipx_get not exist\n",
			dst, channel);
		return -ENODEV;
	}

	if (sipx_chan->state != SBLOCK_STATE_READY) {
		SIPX_ERR("sipx-%d-%d sipx_get_free_count not ready!\n",
			dst, channel);
		return -EIO;
	}
...
EXPORT_SYMBOL(sipx_get_free_count);

int create_sipx_ring_ctrl(struct sipx_ring **out,
		volatile struct sipx_fifo_info *fifo_info,
		void *fifo_virt,
		void *blk_virt,
		struct sipx_mgr *sipx)
{
	...
	ring = kzalloc(sizeof(struct sipx_ring), GFP_KERNEL);
	if (!ring) {
...
int init_sipx_channel_fifos(struct sipx_mgr *sipx,
		struct sipx_channel *sipx_chan)
{
	volatile struct sipx_fifo_info *fifo_info;
	uint32_t offset;
	...
	fifo_info->fifo_addr = sipx_chan->smem_addr + offset;
...
int destroy_sipx_channel_ctrl(struct sipx_channel *sipx_chan)
{
...
int update_sipx_channel_ptr(struct sipx_mgr *sipx,
		struct sipx_channel *sipx_chan)
{
	uint32_t offset;
	...
	offset += sizeof(uint32_t) * sipx_chan->channel;

	chan_ptr_mem_ptr = (volatile uint32_t *)(sipx->smem_virt + offset);
	*chan_ptr_mem_ptr = sipx_chan->smem_addr;

	return 0;
}

int create_sipx_channel_ctrl(struct sipx_mgr *sipx, uint8_t channel,
		struct sipx_channel **out)
{
	...
	/* create channel ctrl */
	sipx_chan = kzalloc(sizeof(struct sipx_channel) , GFP_KERNEL);
	if (!sipx_chan)
		return -ENOMEM;
...
EXPORT_SYMBOL(sipx_chan_destroy);
		...
		void (*handler)(int event, void *data), void *data)
{
	...
	if (!sipx) {
		SIPX_ERR("sipx-%d-%d sipx_chan_register_notifier sipx is not exist!\n",
			dst, channel);
		return -ENODEV;
	}
	...
	if (!sipx_chan) {
		SIPX_ERR("sipx-%d-%d sipx_chan_register_notifier se_chanl is not exist!\n",
			dst, channel);
		return -ENODEV;
	}
...
EXPORT_SYMBOL(sipx_chan_register_notifier);
...
EXPORT_SYMBOL(sipx_chan_create);
...
static int sipx_probe(struct platform_device *pdev)
{
	...
	SIPX_INFO("sipx device tree, name=%s, dst=%u, pool size=%d, %d, %d, %d\n",
...
static int sipx_debug_show(struct seq_file *m, void *private)
{
	...
	for (i = 0; i < SIPC_ID_NR; i++) {
		...
		seq_printf(m, "sipx dst %d, state: 0x%0x, recovery: %d,"
				" smem_virt: 0x%p, smem_cached_virt: 0x%p,"
				"smem_addr: 0x%0x, smem_size: 0x%0x\n",
...
#else
		seq_printf(m, "sipx dst %d, state: 0x%0x, recovery: %d,"
				"smem_virt: 0x%p, smem_addr: 0x%0x,"
				" smem_size: 0x%0x\n",
				sipx->dst, sipx->state,
				sipx->recovery,
				sipx->smem_virt,
		...
		seq_printf(m, "dl_pool_size: %d, dl_ack_pool_size: %d,"
				"ul_pool_size: %d, ul_ack_pool_size: %d,"
				"blk_size: %d, ack_blk_size: %d\n",
				sipx->dl_pool_size,
		...
		seq_printf(m, "dl_pool: blks_addr :0x%0x, blk_size :%d,"
				"fifo_addr :0x%0x, fifo_size :%d, fifo_rdptr"
				" :0x%0x, fifo_wrptr: 0x%0x\n",
				(uint32_t)fifo_info->blks_addr,
				(uint32_t)fifo_info->blk_size,
				(uint32_t)fifo_info->fifo_addr,
				(uint32_t)fifo_info->fifo_size,
				(uint32_t)fifo_info->fifo_rdptr,
				(uint32_t)fifo_info->fifo_wrptr);
		fifo_info = sipx->dl_ack_pool->fifo_info;

		seq_printf(m, "dl_ack_pool: blks_addr :0x%0x, blk_size :%d,"
				"fifo_addr :0x%0x, fifo_size :%d,"
				" fifo_rdptr :0x%0x, fifo_wrptr: 0x%0x\n",
				(uint32_t)fifo_info->blks_addr,
				(uint32_t)fifo_info->blk_size,
				(uint32_t)fifo_info->fifo_addr,
				(uint32_t)fifo_info->fifo_size,
		...
		for (j = 0; j < SMSG_VALID_CH_NR; j++) {
			...
			/* sipx channel */
			seq_printf(m, "sipx_chan dst %d, channel: %3d,"
					"state: %d, smem_virt: 0x%p, smem_addr: 0x%0x,"
					" smem_size: 0x%0x, dl_record: 0x%0x,"
					"ul_record: 0x%0x\n",
					sipx_chan->dst,
					sipx_chan->channel,
					sipx_chan->state,
