...
int sblock_get(uint8_t dst, uint8_t channel, struct sblock *blk, int timeout)
{
	...
	if (poolhd->txblk_rdptr != poolhd->txblk_wrptr &&
		...
		blk->addr = sblock->smem_virt +
			    (ring->p_txblks[txpos].addr - sblock->smem_addr);
...
int sblock_receive(uint8_t dst, uint8_t channel,
...
static int sblock_debug_show(struct seq_file *m, void *private)
{
	...
	for (i = 0; i < SIPC_ID_NR; i++) {
		for (j = 0;  j < SMSG_VALID_CH_NR; j++) {
			...
			seq_puts(m, "***************************************************************************************************************************\n");
			seq_printf(m, "sblock dst %d, channel: %3d, state: %d, smem_virt: 0x%lx, smem_addr: 0x%0x, smem_size: 0x%0x, txblksz: %d, rxblksz: %d\n",
				   sblock->dst,
				   sblock->channel,
				   sblock->state,
