...
static int spipe_parse_dt(struct spipe_init_data **init, struct device *dev)
{
	...
	ret = of_property_read_u32(np,
				   "sprd,ringnr",
				   (uint32_t *)&pdata->ringnr);
	if (ret)
		goto error;

	ret = of_property_read_u32(np,
				   "sprd,size-rxbuf",
				   (uint32_t *)&pdata->rxbuf_size);
	if (ret)
		goto error;

	ret = of_property_read_u32(np,
				   "sprd,size-txbuf",
				   (uint32_t *)&pdata->txbuf_size);
	if (ret)
		goto error;
...
static inline void spipe_destroy_pdata(struct spipe_init_data **init, struct device *dev)
{
...
static int spipe_probe(struct platform_device *pdev)
{
	...
	pr_info("spipe: after parse device tree, name=%s, dst=%u, channel=%u, ringnr=%u,  rxbuf_size=%u, txbuf_size=%u, sipc_name=%s\n",
		init->name,
		init->dst,
		init->channel,
		init->ringnr,
	...
	rval = sbuf_create_ex(init->dst, init->channel, init->ringnr,
		init->txbuf_size, init->rxbuf_size, init->sipc_name);
	if (rval != 0) {
