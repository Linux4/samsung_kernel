...
ssize_t set_barometer_lps22h_sea_level_pressure(struct ssp_data *data, const char *buf)
{
        int ret = 0;
...
ssize_t get_barometer_lps22h_selftest(struct ssp_data *data, char *buf)
{
        ...
        ret = ssp_send_command(data, CMD_GETVALUE, SENSOR_TYPE_PRESSURE, SENSOR_FACTORY, 3000, NULL, 0, &buffer, &buffer_length);
