...
#if defined(CONFIG_SENSORS_SSP_LIGHT_TCS3701) || defined(CONFIG_SENSORS_SSP_LIGHT_STK3X6X)
#define CONFIG_SENSORS_SSP_LIGHT_UNDERPANEL
#endif
...
#ifdef CONFIG_SENSORS_SSP_LIGHT_UNDERPANEL
...
void select_light_ops(struct ssp_data *data, char *name)
{
	...
	count++;
#endif
#if defined(CONFIG_SENSORS_SSP_LIGHT_STK3A5X)
	count++;
#endif
#if defined(CONFIG_SENSORS_SSP_LIGHT_STK3328)
	count++;
#endif
	if (count == 0) {
...
#endif
#if defined(CONFIG_SENSORS_SSP_LIGHT_STK3A5X)
	light_ops_ary[i++] = get_light_stk3a5x_function_pointer(data);
#endif
#if defined(CONFIG_SENSORS_SSP_LIGHT_STK3328)
	light_ops_ary[i++] = get_light_stk3328_function_pointer(data);
#endif
	if (count > 1) {
		for (i = 0; i < count ; i++) {
