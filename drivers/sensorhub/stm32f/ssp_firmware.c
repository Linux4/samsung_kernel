...
{
	if(data->is_mcu_ldo_rev)
	return SSP_FIRMWARE_REVISION_STM;
	else
		return SSP_FIRMWARE_REVISION_STM_NOLDO;
}
...
static int update_mcu_bin(struct ssp_data *data, int iBinType)
{
	...
	/* HW request: I2C line is reversed */
		...
		ret = load_kernel_fw_bootmode(data->spi, BL_FW_NAME);
			...
			ret = load_kernel_fw_bootmode(data->spi, BL_FW_NAME_NOLDO);
		break;
...
int check_fwbl(struct ssp_data *data)
{
	...
	fw_revision = SSP_FIRMWARE_REVISION_STM;
		...
		fw_revision = SSP_FIRMWARE_REVISION_STM_NOLDO;
