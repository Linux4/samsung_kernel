...
void report_meta_data(struct ssp_data *data, struct sensor_value *s)
{
	...
	memset(meta_event, META_EVENT, data->info[s->meta_data.sensor].report_data_len);
	ssp_iio_push_buffers(data->indio_devs[s->meta_data.sensor],	META_TIMESTAMP, meta_event,
...
void report_scontext_notice_data(struct ssp_data *data, char notice)
{
	...
	if (notice == MSG2SSP_AP_STATUS_RESET) {
		len = 4;
		if (data->is_reset_from_kernel == true) {
