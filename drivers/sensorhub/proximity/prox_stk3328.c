...
u16 get_proximity_stk3328_raw_data(struct ssp_data *data)
{
	u16 uRowdata = 0;
	char chTempbuf[8] = { 0, };
...
ssize_t get_proximity_stk3328_name(char *buf)
{
	return sprintf(buf, "%s\n", "STK3328");
}

ssize_t get_proximity_stk3328_vendor(char *buf)
{
	return sprintf(buf, "%s\n", "Sitronix");
}

ssize_t get_proximity_stk3328_probe_status(struct ssp_data *data, char *buf)
{
...
ssize_t get_stk3328_threshold_high(struct ssp_data *data, char *buf)
{
...
ssize_t set_stk3328_threshold_high(struct ssp_data *data, const char *buf)
{
	u16 uNewThresh;
	int ret, i = 0;
...
ssize_t get_stk3328_threshold_low(struct ssp_data *data, char *buf)
{
...
ssize_t set_stk3328_threshold_low(struct ssp_data *data, const char *buf)
{
	u16 uNewThresh;
	int ret, i = 0;
	u16 prox_bits_mask = 0, prox_non_bits_mask = 0;
...
ssize_t get_proximity_stk3328_trim_value(struct ssp_data *data, char *buf)
{
	int ret = 0;
	char *buffer = NULL;
	int buffer_length = 0;
...
ssize_t get_proximity_stk3328_avg_raw_data(struct ssp_data *data, char *buf)
{
 ...
 */
ssize_t set_proximity_stk3328_avg_raw_data(struct ssp_data *data, const char *buf)
{
	char chTempbuf[8] = { 0, };
	int ret;
	int64_t dEnable;
...
struct proximity_sensor_operations prox_stk3328_ops = {
	...
	.get_proximity_probe_status = get_proximity_stk3328_probe_status,
	.get_threshold_high = get_stk3328_threshold_high,
	.set_threshold_high = set_stk3328_threshold_high,
	.get_threshold_low = get_stk3328_threshold_low,
	.set_threshold_low = set_stk3328_threshold_low,
	.get_proximity_avg_raw_data = get_proximity_stk3328_avg_raw_data,
