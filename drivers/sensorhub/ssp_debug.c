	...
	if (sensor_type < SENSOR_TYPE_MAX) {
		...
		case SENSOR_TYPE_LIGHT_AUTOBRIGHTNESS:
			ssp_info("%s(%u) : %u, %u (%lld) (%ums, %dms)", data->info[sensor_type].name, sensor_type,
				 ...
				 data->delay[sensor_type].max_report_latency);
			break;
		case SENSOR_TYPE_FLIP_COVER_DETECTOR:
			ssp_info("%s(%u) : %d %d %d / %d %d %d %d (%lld) (%ums, %dms)",
				 data->info[sensor_type].name, sensor_type,
				 data->buf[sensor_type].value,
				 data->buf[sensor_type].magX,
				 data->buf[sensor_type].stable_min_max,
				 data->buf[sensor_type].uncal_mag_x,
				 data->buf[sensor_type].uncal_mag_y,
				 data->buf[sensor_type].uncal_mag_z,
				 data->buf[sensor_type].saturation,
				 data->buf[sensor_type].timestamp,
				 data->delay[sensor_type].sampling_period,
				 data->delay[sensor_type].max_report_latency);
			break;
