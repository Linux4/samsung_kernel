		...
		case SSP2AP_GYRO_CAL:
			{
				...
				ssp_wake_lock(&data->ssp_wakelock);
				//save_gyro_cal_data(data, caldata);
				ssp_wake_unlock(&data->ssp_wakelock);
			...
			case SSP2AP_MAG_CAL:
			{
				...
				ssp_wake_lock(&data->ssp_wakelock);
				index += save_mag_cal_data(data, dataframe + index);
				ssp_wake_unlock(&data->ssp_wakelock);
			}
			break;
#endif
...
int set_pdc_matrix(struct ssp_data *data)
{
	...
	ret = ssp_send_command(data, CMD_SETVALUE, SENSOR_TYPE_GEOMAGNETIC_FIELD,
	                       MAGNETIC_STATIC_MATRIX, 0, (char*)data->pdc_matrix, sizeof(data->pdc_matrix), NULL,
	                       NULL);
