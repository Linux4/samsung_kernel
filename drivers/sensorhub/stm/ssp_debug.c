...
*/
	...
	if (data->bMcuDumpMode == true)	{
		...
		if (data->realtime_dump_file == NULL) {
			...
			snprintf(strFilePath, PAGE_SIZE, "%s%d.dump",
...
void ssp_dump_task(struct work_struct *work)
{
	...
	if (big->data->bMcuDumpMode == true) {
		...
		snprintf(strFilePath, PAGE_SIZE, "%s%d.dump", DUMP_FILE_PATH,
			(int)cur_time.tv_sec);
		dump_file = filp_open(strFilePath,
				O_RDWR | O_CREAT | O_APPEND, 0666);
#else
		snprintf(strFilePath, PAGE_SIZE, "%s.dump", DUMP_FILE_PATH);
		dump_file = filp_open(strFilePath,
		...
		int iRcvDataFrameLength)
{
	...
	*pDataIdx += 1;
	...
	if (length > iRcvDataFrameLength - *pDataIdx || length <= 0) {
		ssp_infof("MSG From MCU - invalid debug length(%u/%d/%d)",
			length, iRcvDataFrameLength, cur);
		return length ? length : ERROR;
	}

	ssp_info("MSG From MCU - %s", &pchRcvDataFrame[*pDataIdx]);
	*pDataIdx += length;
	return 0;
}
