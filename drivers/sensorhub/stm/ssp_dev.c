	...
	struct ssp_data *data)
{
	...
	data->psns_vreg = devm_regulator_get(dev, "psns_vreg");  
	if (IS_ERR(data->psns_vreg)) {                              
		errorno = PTR_ERR(data->psns_vreg);
		ssp_err("failed get regulator %d", errorno);
		goto dt_exit;                                      
	}                                                                                                     
	                                                       
	errorno = regulator_enable(data->psns_vreg);                    
	if (errorno){
		ssp_err("failed enable regulator %d", errorno);
		goto dt_exit;                                      
	}
