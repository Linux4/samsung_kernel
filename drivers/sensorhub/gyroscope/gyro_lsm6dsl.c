...
ssize_t get_gyro_lsm6dsl_temperature(struct ssp_data *data, char *buf)
{
        ...
        ret = ssp_send_command(data, CMD_GETVALUE, SENSOR_TYPE_GYROSCOPE, GYROSCOPE_TEMPERATURE_FACTORY, 3000, NULL, 0, &buffer, &buffer_length);
...
ssize_t get_gyro_lsm6dsl_selftest(struct ssp_data *data, char *buf)
{
        ...
        ret = ssp_send_command(data, CMD_GETVALUE, SENSOR_TYPE_GYROSCOPE, SENSOR_FACTORY, 7000, NULL, 0, &temp_buf, &temp_buf_length);
        ...
        pr_err("[SSP]%d %d %d %d %d %d %d %d %d %d %d %d\n", temp_buf[0], temp_buf[1], temp_buf[2], temp_buf[3], temp_buf[4], temp_buf[5], temp_buf[6], temp_buf[7], temp_buf[8], temp_buf[9],
               temp_buf[10], temp_buf[11]);
        ...
        if (hw_result < 0) {
                ...
                if (shift_ratio[0] == GYRO_LIB_DL_FAIL && shift_ratio[1] == GYRO_LIB_DL_FAIL && shift_ratio[2] == GYRO_LIB_DL_FAIL) {
        ...
        if (VERBOSE_OUT) {
                pr_info("[SSP] abs bias : %+8d.%03d %+8d.%03d %+8d.%03d (dps)\n", (int) abs(gyro_bias[0]) / DEF_SCALE_FOR_FLOAT, (int) abs(gyro_bias[0]) % DEF_SCALE_FOR_FLOAT,
                        (int) abs(gyro_bias[1]) / DEF_SCALE_FOR_FLOAT, (int) abs(gyro_bias[1]) % DEF_SCALE_FOR_FLOAT, (int) abs(gyro_bias[2]) / DEF_SCALE_FOR_FLOAT,
        ...
        pr_info("[SSP] RMS : %+8d.%03d	 %+8d.%03d  %+8d.%03d (dps)\n", (int) abs(gyro_rms[0]) / DEF_SCALE_FOR_FLOAT, (int) abs(gyro_rms[0]) % DEF_SCALE_FOR_FLOAT,
                (int) abs(gyro_rms[1]) / DEF_SCALE_FOR_FLOAT, (int) abs(gyro_rms[1]) % DEF_SCALE_FOR_FLOAT, (int) abs(gyro_rms[2]) / DEF_SCALE_FOR_FLOAT,
...
exit:
        ssp_dbg("[SSP]: %s - " "%d,%d,%d," "%d,%d,%d," "%d,%d,%d," "%d,%d,%d,%d,%d\n", __func__, gyro_fifo_avg[0], gyro_fifo_avg[1], gyro_fifo_avg[2], gyro_self_zro[0], gyro_self_zro[1],
                gyro_self_zro[2], gyro_self_bias[0], gyro_self_bias[1], gyro_self_bias[2], gyro_self_diff[0], gyro_self_diff[1], gyro_self_diff[2], fifo_ret, cal_ret);
        ...
        ret = sprintf(buf, "%d,%d,%d,"
                      "%d,%d,%d,"
                      "%d,%d,%d,"
                      "%d,%d,%d,%d,%d\n", gyro_fifo_avg[0], gyro_fifo_avg[1], gyro_fifo_avg[2], gyro_self_zro[0], gyro_self_zro[1], gyro_self_zro[2], gyro_self_bias[0], gyro_self_bias[1],
                      gyro_self_bias[2], gyro_self_diff[0], gyro_self_diff[1], gyro_self_diff[2], fifo_ret, cal_ret);
