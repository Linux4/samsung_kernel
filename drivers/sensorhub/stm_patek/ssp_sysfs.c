	...
	int iSensorType, int64_t dNewDelay)
{
	...
	case ADD_SENSOR_STATE:
		...
		if (iSensorType == TEMPERATURE_HUMIDITY_SENSOR &&
			...
			pr_info("[SSP] Skip Add Temphumidity Sensor\n");
				...
				if (iSensorType == LIGHT_SENSOR) {
					if(data->folder_state == 1){
						data->folder_state = 0;
						pr_info("[SSP] folder open and light sensor enable it need delay\n");
						msleep(300);
					}
				}
		...
		if (iSensorType == PROXIMITY_SENSOR) {
			...
			|| (iSensorType == ORIENTATION_SENSOR)\
			|| (iSensorType == ROTATION_VECTOR)) {
			/*flip_status 0:open, 1:close*/
			set_magnetic_cal_with_folder_state(data, ssp_get_flip_status());
			pr_info("[SSP] %s : Folder is %sed now.\n", __func__,
						ssp_get_flip_status() ? "clos":"open");
		}
#endif
		break;
