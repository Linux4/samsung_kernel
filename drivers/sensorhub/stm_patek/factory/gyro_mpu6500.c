...
#define VENDOR_MAXIM	"MAXIM"
#define CHIP_ID_MAX21103		"MAX21103"
...
#define DEF_GYRO_SENS_MAXIM	(30000 / DEF_GYRO_FULLSCALE)
...
#define DEF_BIAS_LSB_THRESH_SELF_MAXIM	(20 * DEF_GYRO_SENS_MAXIM)
#define DEF_BIAS_LSB_THRESH_SELF_MAX21103_MAXIM	(30 * DEF_GYRO_SENS_MAXIM)
#define DEF_RMS_LSB_TH_SELF_MAXIM (5 * DEF_GYRO_SENS_MAXIM)

#define DEF_RMS_THRESH	((DEF_RMS_LSB_TH_SELF) * (DEF_RMS_LSB_TH_SELF))
#define DEF_RMS_THRESH_MAXIM	((DEF_RMS_LSB_TH_SELF_MAXIM) * (DEF_RMS_LSB_TH_SELF_MAXIM))
...
ssize_t mpu6500_gyro_selftest(char *buf, struct ssp_data *data)
{
	...
	if (data->ap_rev >= MAXIM_MAX21103_REV){
		...
		avg[1] = avg[1]/8;
		avg[2] = avg[2]/8;
		pr_info("[SSP] new avg %+8ld %+8ld %+8ld (LSB)\n", avg[0], avg[1], avg[2]);
	}
#endif
