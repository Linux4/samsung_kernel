...
#define SSP_FIRMWARE_REVISION_MAXIM_PATEK	14092900
...
#define BL_FW_NAME_MAXIM_PATEK   "ssp_stm_maxim_patek.fw"
...
unsigned int get_module_rev(struct ssp_data *data)
{
	...
	if (data->ap_rev >= MAXIM_MAX21103_REV)
		return SSP_FIRMWARE_REVISION_MAXIM_PATEK;
	else
...
#elif defined(CONFIG_SEC_PATEK_PROJECT)
		if (data->ap_rev >= MAXIM_MAX21103_REV)
			iRet = load_kernel_fw_bootmode(data->spi, BL_FW_NAME_MAXIM_PATEK);
		else
...
#elif defined(CONFIG_SEC_PATEK_PROJECT)
	if (data->ap_rev >= MAXIM_MAX21103_REV)
		fw_revision = SSP_FIRMWARE_REVISION_MAXIM_PATEK;
	else
