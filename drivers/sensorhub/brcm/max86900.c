...
static int max86900_regulator_onoff(struct max86900_device_data *data, int onoff)
{
	...
	if (!data->vdd_1p8_gpio) {
		regulator_vdd_1p8 = regulator_get(NULL, "HRM_1.8V_AP");
		if (PTR_ERR(regulator_vdd_1p8) || regulator_vdd_1p8 == NULL) {
	...
	if (!data->led_3p3_gpio) {
		regulator_led_3p3 = regulator_get(NULL, "HRM_3.3V_AP");
		if (PTR_ERR(regulator_led_3p3) || regulator_led_3p3 == NULL) {
...
int max86900_probe(struct i2c_client *client, const struct i2c_device_id *id )
{
	...
	if (err < 0) {
		pr_err("%s max86900_regulator_onoff fail(%d, %d)\n", __func__,
			err, HRM_LDO_OFF);
		goto err_set_regulator_failed;
	}

	pr_info("%s success\n", __func__);
	goto done;

err_set_regulator_failed:
