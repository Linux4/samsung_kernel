...
static void get_seq_step_data(char *pchRcvDataFrame, int *iDataIdx,
	struct sensor_value *sensorsdata)
{
	memcpy(sensorsdata, pchRcvDataFrame + *iDataIdx, 4);
	*iDataIdx += 4;
...
void initialize_function_pointer(struct ssp_data *data)
{
	...
	data->get_sensor_data[SEQUENTIAL_STEP] = get_seq_step_data;
	...
	data->report_sensor_data[SEQUENTIAL_STEP] = report_seq_step_data;
