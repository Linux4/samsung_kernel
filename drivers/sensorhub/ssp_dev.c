...
static int ssp_parse_dt(struct device *dev, struct ssp_data *data)
{
	...
	if (of_property_read_u16_array(np, "ssp-mag-array",
	                              data->pdc_matrix, sizeof(data->pdc_matrix)/sizeof(data->pdc_matrix[0]))) {
...
static int ssp_probe(struct platform_device *pdev)
{
	...
	ssp_wake_lock_init(&data->ssp_wakelock, "ssp_wake_lock");
...
err_input_register_device:
	ssp_wake_lock_destroy(&data->ssp_wakelock);
...
{
	struct ssp_data *data = dev_get_drvdata(&pdev->dev);
...
static void ssp_resume(struct device *dev)
{
	struct ssp_data *data = dev_get_drvdata(dev);
	ssp_infof();
...
{
	int ret = 0;
	pr_info("[SSP] ssp_init called init\n");
	ret = platform_driver_register(&ssp_driver);

	pr_info("[SSP] ssp_init ret %d\n", ret);
	return ret;
