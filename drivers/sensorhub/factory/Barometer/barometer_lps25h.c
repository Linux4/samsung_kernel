...
ssize_t get_barometer_lps25h_name(char *buf)
{
	return sprintf(buf, "%s\n", "LPS25H");
}


ssize_t get_barometer_lps25h_vendor(char *buf)
{
	return sprintf(buf, "%s\n", "STM");
}

ssize_t set_barometer_lps25h_sea_level_pressure(struct ssp_data *data,
                                                const char *buf)
{
	int ret = 0;
...
ssize_t get_barometer_lps25h_calibration(struct ssp_data *data, char *buf)
{
...
ssize_t set_barometer_lps25h_calibration(struct ssp_data *data, const char *buf)
{
	int pressure_cal = 0;
	int ret = 0;;
...
ssize_t get_barometer_lps25h_temperature(struct ssp_data *data, char *buf)
{
	s32 temperature = 0;
	s32 float_temperature = 0;
	s32 temp = 0;
	temp = (s32)(data->buf[SENSOR_TYPE_PRESSURE].temperature);
	temperature = (4250) + ((temp / (120 * 4)) *
	                        100); //(42.5f) + (temperature/(120 * 4));
	float_temperature = ((temperature % 100) > 0 ? (temperature % 100) : -
	                     (temperature % 100));

	return sprintf(buf, "%d.%02d\n", (temperature / 100), float_temperature);
}

struct barometer_sensor_operations barometer_lps25h_ops = {
	.get_barometer_name = get_barometer_lps25h_name,
	.get_barometer_vendor = get_barometer_lps25h_vendor,
	.get_barometer_calibration = get_barometer_lps25h_calibration,
