...
void initialize_accelerometer_sysfs(void)
{
	...
	if (ret < 0) {
		shub_errf("fail to create %s sysfs device", sensor->name);
		return;
	}
...
void initialize_accelerometer_sub_sysfs(void)
{
	struct shub_sensor *sensor = get_sensor(SENSOR_TYPE_ACCELEROMETER_SUB);
	int ret;

	ret = sensor_device_create(&accel_sub_sysfs_device, NULL, "accelerometer_sub_sensor");
	if (ret < 0) {
		shub_errf("fail to create %s sysfs device", sensor->name);
		return;
	}

	ret = add_sensor_device_attr(accel_sub_sysfs_device, acc_sub_attrs);
	if (ret < 0) {
		shub_errf("fail to add %s sysfs device attr", sensor->name);
		return;
	}
...
void remove_accelerometer_sub_sysfs(void)
{
	remove_sensor_device_attr(accel_sub_sysfs_device, acc_sub_attrs);
	sensor_device_destroy(accel_sub_sysfs_device);
	accel_sub_sysfs_device = NULL;
}

void initialize_accelerometer_sub_factory(bool en)
{
	if (!get_sensor(SENSOR_TYPE_ACCELEROMETER_SUB))
		return;

	if (en)
		initialize_accelerometer_sub_sysfs();
	else
		remove_accelerometer_sub_sysfs();
}
