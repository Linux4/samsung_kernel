...
int enable_accel_lsm6dsl_for_cal(struct ssp_data *data)
{
	u8 buf[8] = { 0, };
...
void disable_accel_lsm6dsl_for_cal(struct ssp_data *data, int ret)
{
	u8 buf[8] = { 0, };
	s32 dMsDelay = data->delay[SENSOR_TYPE_ACCELEROMETER].sampling_period;
	memcpy(&buf[0], &dMsDelay, 4);
...
int accel_lsm6dsl_do_calibrate(struct ssp_data *data, int enable)
{
	int iSum[3] = { 0, };
	int ret = 0;
	...
	if (enable) {
		...
		ret = enable_accel_lsm6dsl_for_cal(data);
		msleep(300);
		...
		disable_accel_lsm6dsl_for_cal(data, ret);
...
ssize_t set_accel_lsm6dsl_calibration(struct ssp_data *data, const char *buf)
{
	...
	ret = accel_lsm6dsl_do_calibrate(data, (int)enable);
	if (ret < 0) {
		pr_err("[SSP]: %s - accel_lsm6dsl_do_calibrate() failed\n", __func__);
	}
...
struct accelometer_sensor_operations* get_accelometer_lsm6dsl_function_pointer(struct ssp_data *data)
{
