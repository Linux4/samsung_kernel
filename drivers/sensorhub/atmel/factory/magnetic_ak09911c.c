...
static int set_pdc_matrix(struct ssp_data *data)
{
	...
	msg = kzalloc(sizeof(*msg), GFP_KERNEL);
	if (msg == NULL) {
		pr_err("[SSP] %s, failed to alloc memory for "
				"ssp_msg\n", __func__);
		return -ENOMEM;
	}
	...
	msg->buffer = (char*) kzalloc(27, GFP_KERNEL);
	if (msg->buffer == NULL) {
	...
	msg->free_buffer = 1;
	memcpy(msg->buffer, pdc_matrix, 27);
