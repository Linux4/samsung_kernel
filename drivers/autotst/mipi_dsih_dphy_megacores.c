...
static int mipi_dsih_dphy_calc_pll_param(dphy_t *phy, struct dphy_pll *pll)
{
	...
	if (!pll || !pll->freq)
		goto FAIL;
	printk("autotst:billy mipi_dsih_dphy_calc_pll_param pll->freq = %d \n",pll->freq);
...
static int mipi_dsih_dphy_set_pll_reg(dphy_t *phy, struct dphy_pll *pll)
{
	...
	val = (u8 *)&regs;
	printk("autotst:billy regs._09.bits.kint_h=%d",regs._09.bits.kint_h);
	printk("autotst:billy regs._0a.bits.kint_m=%d",regs._0a.bits.kint_m);
	printk("autotst:billy regs._0b.bits.out_sel=%d",regs._0b.bits.out_sel);
	printk("autotst:billy regs._0b.bits.kint_l=%d",regs._0b.bits.kint_l);
	printk("autotst:billy regs._0c.bits.kstep_h=%d",regs._0c.bits.kstep_h);
	printk("autotst:billy regs._0d.bits.kstep_m=%d",regs._0d.bits.kstep_m);
	printk("autotst:billy regs._0e.bits.kstep_l=%d",regs._0e.bits.kstep_l);
...
static int mipi_dsih_dphy_config_pll(dphy_t *phy, u32 freq)
{
	...
	pll.freq = freq;
	printk("autotst:billy mipi_dsih_dphy_config_pll in \n");
