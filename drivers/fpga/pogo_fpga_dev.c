...
int pogo_fpga_config(struct pogo_fpga *pogo_fpga)
{
	...
	*/
	image_data = (uint8_t *)kzalloc(imageSize + TX_HEADER_100_DUMMY_CLOCK_SIZE, GFP_KERNEL);
	if (image_data == NULL) {
		return -ENOMEM;
	}

	memcpy(image_data, sensorHubImage, imageSize);
	memset(image_data + imageSize, 0, TX_HEADER_100_DUMMY_CLOCK_SIZE); /* 100 clocks ~= 13 dummy bytes transfered */
	...
	pogo_fpga_tx_data(pogo_fpga, image_data, imageSize + TX_HEADER_100_DUMMY_CLOCK_SIZE);
