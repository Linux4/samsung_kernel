					  ...
					  int reading)
{
	...
	dev_info(arizona->dev, "arizona_cs47l15_tune_headphone  reading =%d\n",reading);
...
int arizona_micd_button_start(struct arizona_extcon_info *info)
{
	int ret;
...
EXPORT_SYMBOL_GPL(arizona_micd_button_start);
...
static int arizona_antenna_button_start(struct arizona_extcon_info *info)
{
	...
	/* check if impedance level is supported */
	micd_lvl = arizona_antenna_get_micd_level(
				HOHM_TO_OHM(arizona->hp_impedance_x100), 0);
	...
	if (pdata->antenna_hp_imp_range_hi)
		hp_imp_range_hi =
			arizona_antenna_get_micd_level(
...
const struct arizona_jd_state arizona_micd_button = {
	...
	.start = arizona_micd_button_start,
