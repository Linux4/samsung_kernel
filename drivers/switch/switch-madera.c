...
#ifdef CONFIG_SEC_EARJACK_35_USB_SWITCH
#include <linux/sec_mux_sel.h>
#endif /* CONFIG_SEC_EARJACK_35_USB_SWITCH */
...
static void madera_hs_mic_control(struct madera_extcon_info *info, int state)
{
	...
	case MADERA_MIC_MUTE:
		dev_dbg(info->madera->dev, "Mute headset mic: 0x%04x\n",
			addr);
		val = MADERA_MIC_MUTE;
		break;
	case MADERA_MIC_UNMUTE:
		dev_dbg(info->madera->dev, "Unmute headset mic: 0x%04x\n",
			addr);
		val = MADERA_MIC_UNMUTE;
		break;
	default:
		dev_err(info->madera->dev,
...
void madera_set_headphone_imp(struct madera_extcon_info *info, int imp)
{
	...
	if (madera->pdata.hpdet_cb)
		madera->pdata.hpdet_cb(HOHM_TO_OHM(imp));
...
#ifdef CONFIG_SEC_EARJACK_35_USB_SWITCH
		sec_mpp_mux_control(EAR_MIC_35MM_MUX_SEL_NUM,
			SEC_MUX_SEL_EAR_MIC_35MM, 1);
		sec_mpp_mux_control(EAR_MIC_35MM_MUX_SEL_NUM,
			SEC_MUX_SEL_EAR_MIC_35MM, 0);
		
		sec_mpp_mux_control(EAR_OUT_35MM_MUX_SEL_NUM,
			SEC_MUX_SEL_EAR_OUT_35MM, 1);
		sec_mpp_mux_control(EAR_OUT_35MM_MUX_SEL_NUM,
			SEC_MUX_SEL_EAR_OUT_35MM, 0);
#endif /* CONFIG_SEC_EARJACK_35_USB_SWITCH */
...
#include "../../sound/soc/samsung/jack_madera.c"
