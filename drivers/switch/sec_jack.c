...
//KSND
static void pm822_jack_key_handle(struct pm822_headset_info *info, int voltage)
{
	...
	if(voltage < pdata->press_release_th) {
		...
		if(info->psw_data_headset->state != SEC_JACK_NO_DEVICE){
			input_report_key(info->idev, info->pressed_code, state);
			input_sync(info->idev);
		} else {
			pr_info("[jack] headset is not present so key event will be not dealed with\n");
		}
...
#ifdef COM_DET_CONTROL
	com_value = gpio_get_value(info->gpio_com); /* get com detecting status - ffkaka */
...
static void pm822_jack_buttons_work(struct work_struct *work)
{
	...
	if (value)
		regmap_update_bits(info->map, PM822_INT_ENA_3, PM822_MIC_INT_ENA3, PM822_MIC_INT_ENA3);
...
#ifdef COM_DET_CONTROL 
	info->gpio_com = mfp_to_gpio(info->pdata->com_det_gpio);
