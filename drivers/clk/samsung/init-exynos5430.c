...
#ifdef CONFIG_EXYNOS5430_HD
	exynos_set_parent("mout_mem0_pll", "fin_pll");

	fout_mem0_pll = clk_get(NULL, "fout_mem0_pll");
	if (IS_ERR(fout_mem0_pll)) {
		pr_err("failed get fout_mem0_pll clock\n");
		goto out;
	}

	clk_prepare(fout_mem0_pll);
	clk_enable(fout_mem0_pll);
	clk_disable(fout_mem0_pll);
	clk_unprepare(fout_mem0_pll);
	clk_put(fout_mem0_pll);
out:
#endif
