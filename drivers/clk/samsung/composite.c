...
static int samsung_composite_pll_enable_onchange(struct clk_hw *hw)
{
	...
	do {
		cpu_relax();
		reg = readl(pll->stat_reg);
	} while (reg & BIT(pll->stat_bit)); /* check MUX_BUSY 1 bit of PLL MUX */
...
static int samsung_mux_set_parent(struct clk_hw *hw, u8 index)
{
	...
	if (mux->stat_reg)
		...
		} while (val & BIT(mux->stat_bit + mux->stat_width - 1)); /* check MUX_BUSY 1 bit */
				...
				unsigned long parent_rate)
{
	...
	if (divider->stat_reg)
		...
		} while (val & BIT(divider->stat_bit + divider->stat_width - 1)); /* check DIV_BUSY 1 bit */
...
static int samsung_usermux_enable(struct clk_hw *hw)
{
	...
	if (usermux->stat_reg)
		...
		} while (val & BIT(usermux->stat_bit));	/* check MUX_BUSY 1 bit of USERMUX */
