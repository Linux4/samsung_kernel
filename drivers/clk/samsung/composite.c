					...
					unsigned long parent_rate)
{
	...
	u32 pll_con0, pll_con1;
	const struct samsung_pll_rate_table *rate;
	unsigned int lock_time, lock_time_latency;
	...
	/* Set PLL lock time */
	lock_time = rate->pdiv * PLL2650X_LOCK_FACTOR;
	writel(lock_time, pll->lock_reg);
	 ...
	 */
	lock_time_latency = lock_time / PLL_FIN;		/* us */
	mdelay(lock_time_latency / 1000);			/* us to ms */
