...
#if defined(CONFIG_SEC_GTS6L_PROJECT) || defined(CONFIG_SEC_GTS6LWIFI_PROJECT)
	struct delayed_work	irq_dump_work;
#endif
};
...
#if defined(CONFIG_SEC_GTS6L_PROJECT) || defined(CONFIG_SEC_GTS6LWIFI_PROJECT)
#define IRQ_DUMP_DELAY	1000
static int panic_count = 0;
static void irq_dump_work_fn(struct work_struct *work)
{
	struct spmi_pmic_arb *pmic_arb = container_of(work, struct spmi_pmic_arb, irq_dump_work.work);
	struct spmi_controller *ctrl = the_ctrl;
	u8 val;

	pmic_arb_read_cmd(ctrl, SPMI_CMD_EXT_READL, 0, 0x818, &val, 1);
	
	if ((val & 0x1) == 1) {
		pr_info("%s: PMIC latch is set(0x%x)\n", __func__, val);
		panic_count++;
	} else {
		panic_count = 0;
	}
	...
	if (panic_count > 4) {
		debug_pmic_dump();
		panic("%s: latch is not cleared for 5s(0x%x)\n", val);
	}

	schedule_delayed_work(&pmic_arb->irq_dump_work,
				msecs_to_jiffies(IRQ_DUMP_DELAY));
}
#endif
...
#if defined(CONFIG_SEC_GTS6L_PROJECT) || defined(CONFIG_SEC_GTS6LWIFI_PROJECT)
	INIT_DELAYED_WORK(&pmic_arb->irq_dump_work, irq_dump_work_fn);
	schedule_delayed_work(&pmic_arb->irq_dump_work,	msecs_to_jiffies(10000));
	the_ctrl = ctrl;
#endif
