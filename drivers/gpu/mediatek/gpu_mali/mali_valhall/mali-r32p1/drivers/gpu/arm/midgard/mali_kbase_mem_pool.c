				  ...
				  const bool alloc_from_kthread)
{
	...
	if ((page_owner->flags & PF_EXITING) || fatal_signal_pending(page_owner)) {
		dev_info(pool->kbdev->dev, "%s : Process %s/%d exiting (exit:%d, fatal:%d)", __func__, page_owner->comm,
			 task_pid_nr(page_owner), page_owner->flags & PF_EXITING, fatal_signal_pending(page_owner));
		return false;
	}
