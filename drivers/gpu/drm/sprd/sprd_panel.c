...
#include <linux/sprd_dfs_drv.h>	//bug P220705-06311 ,pengzhenhua1.wt,modify,20220725,DPU underflow
#include <video/mipi_display.h>
#include <video/of_display_timing.h>
#include <video/videomode.h>
#include <linux/hardware_info.h>	//Bug760817, pengzhenhua1.wt, add, 20220527, lcd bringup
#include "sprd_dpu.h"
#include "sprd_panel.h"
#include "dsi/sprd_dsi_api.h"
#include "sysfs/sysfs_display.h"
//+Bug760817, pengzhenhua1.wt, add, 20220525, lcd bringup
...
extern char Lcm_name[HARDWARE_MAX_ITEM_LONGTH];	//Bug760817, pengzhenhua1.wt, add, 20220527, lcd bringup
...
static int __init lcd_name_get(char *str)
{
	...
	strncpy(Lcm_name, lcd_name, strlen(lcd_name)+1);	//Bug760817, pengzhenhua1.wt, add, 20220527, lcd bringup
	return 0;
}
...
//+bug P220705-06311 ,pengzhenhua1.wt,modify,20220725,DPU underflow
static int sprd_panel_dfs_control(int lcdon_en)
{
	char *LCDON  = "lcdon";
	char *LCDOFF = "lcdoff";
	int err;
	...
	{
		DRM_INFO("%s() lcdon\n", __func__);
		err = scene_dfs_request(LCDON);
		if (err)
			DRM_ERROR("scene exit fail: %d\n", err);
		err = scene_exit(LCDOFF); 
		if (err)
			DRM_ERROR("scene exit fail: %d\n", err);
	}
	else
	{
		DRM_INFO("%s() lcdoff\n", __func__);
		err = scene_dfs_request(LCDOFF);
		if (err)
			DRM_ERROR("scene exit fail: %d\n", err);
		err = scene_exit(LCDON); 
		if (err)
			DRM_ERROR("scene exit fail: %d\n", err);
	}
	return 0;
...
//+Bug760817, pengzhenhua1.wt, del, 20220525, lcd bringup
/*	if (panel->info.avee_gpio) {
		gpiod_direction_output(panel->info.avee_gpio, 0);
...
static int sprd_panel_prepare(struct drm_panel *p)
{
	...
	struct gpio_timing *timing;
	int items, i, ret;
	unsigned char read_byte = 0;	//Bug760817, pengzhenhua1.wt, add, 20220525, lcd bringup

	DRM_INFO("%s()\n", __func__);
	
	sprd_panel_dfs_control(1);	//bug P220705-06311 ,pengzhenhua1.wt,modify,20220725,DPU underflow
	...
	if (panel->info.avdd_gpio) {
		...
		lcm_i2c_write_bytes(POSCNTL, 0x14);	//Bug760817, pengzhenhua1.wt, add, 20220525, lcd bringup
		mdelay(2);
	}

	if (panel->info.avee_gpio) {
		gpiod_direction_output(panel->info.avee_gpio, 1);
		lcm_i2c_write_bytes(NEGCNTL, 0x14);	//Bug760817, pengzhenhua1.wt, add, 20220525, lcd bringup
		mdelay(12);
	}
//+Bug760817, pengzhenhua1.wt, add, 20220525, lcd bringup
	/*
	*Fixed hX83102E water ripple issue. Modify the value of
	*register 0x03, change the current mode to 90mA, and
	*increase the biasing chip register configuration
	*to change the chip load capacity.
	*/
	if(!strcmp(Lcm_name, "lcd_hx83102e_lide_al_hsd_video_fhdp"))
	{	
		lcm_i2c_read_bytes(0x04, &read_byte);

		if (read_byte == 0x03) {
			lcm_i2c_write_bytes(CONTROL, 0x43);
		} else {
			lcm_i2c_write_bytes(CONTROL, 0x03);
			lcm_i2c_write_bytes(0x61, 0x2C);
			lcm_i2c_write_bytes(0x04, (read_byte | 0x08));

			lcm_i2c_read_bytes(0x46, &read_byte);
			lcm_i2c_write_bytes(0x46, (read_byte | 0x40));

			lcm_i2c_write_bytes(0x61, 0x00);
		}
	}
//-Bug760817, pengzhenhua1.wt, add, 20220525, lcd bringup
	if (panel->info.reset_gpio) {
...
static int sprd_panel_disable(struct drm_panel *p)
{
	...
	if ((panel->info.cmds_len[CMD_CODE_DEEP_SLEEP_IN] != 0)
			&& (tp_gesture_flag == 0)) {
		sprd_panel_send_cmds(panel->slave,
	...
	mutex_unlock(&panel_lock);
	sprd_panel_dfs_control(0);	//bug P220705-06311 ,pengzhenhua1.wt,modify,20220725,DPU underflow
//-Bug760817, pengzhenhua1.wt, add, 20220525, lcd bringup
	return 0;
}
	...
	for (i = 0; i < info->esd_conf.esd_check_reg_count; i++) {
		...
		for (j = 0; j < info->esd_conf.val_len_array[i]; j++) {
			if (read_val[j] != info->esd_conf.val_seq[j + last_rd_count]) {
				DRM_ERROR("esd check failed,read_reg = %02x read value = 0x%02x read_rd_count = %02x\n",info->esd_conf.reg_seq[i],
						read_val,info->esd_conf.val_seq[j + last_rd_count]);
				mutex_unlock(&panel_lock);
				return -EINVAL;
			}
		}
...
//+Bug760817, pengzhenhua1.wt, add, 20220525, lcd bringup
static void sprd_panel_switch_master(struct sprd_panel *panel)
{
	int rc = 0 ;
	mutex_lock(&panel_lock);
	rc = sprd_panel_send_cmds(panel->slave,
			     panel->info.cmds[CMD_CODE_ESD_MASTER],
			     panel->info.cmds_len[CMD_CODE_ESD_MASTER]);
	if (rc) {
		DRM_ERROR("failed to send sprd_panel_master cmds, rc=%d\n",rc);
	}
	mutex_unlock(&panel_lock);
}
static void sprd_panel_switch_slave(struct sprd_panel *panel)
{
	int rc = 0;
	mutex_lock(&panel_lock);
	rc = sprd_panel_send_cmds(panel->slave,
		panel->info.cmds[CMD_CODE_ESD_SLAVE],
		panel->info.cmds_len[CMD_CODE_ESD_SLAVE]);
	if (rc) {
		DRM_ERROR("failed to send sprd_panel_slave cmds,rc=%d\n", rc);
	}
	mutex_unlock(&panel_lock);
}
//-Bug760817, pengzhenhua1.wt, add, 20220525, lcd bringup
...
//+Bug760817, pengzhenhua1.wt, add, 20220525, lcd bringup
	if (info->esd_conf.esd_check_mode == ESD_MODE_REG_CHECK){
		if(!strcmp(Lcm_name, "lcd_nt36523b_lce_panda_video_fhdp"))	
		{
			sprd_panel_switch_master(panel);
		}
		ret = sprd_panel_esd_check(panel);
		if(!strcmp(Lcm_name, "lcd_nt36523b_lce_panda_video_fhdp"))	
		{	
			sprd_panel_switch_slave(panel);
			ret |= sprd_panel_esd_check(panel);
		}
//-Bug760817, pengzhenhua1.wt, add, 20220525, lcd bringup		
	...
	if (ret && panel->base.connector && panel->base.connector->encoder) {
		...
		//+Bug760817, pengzhenhua1.wt, add, 20220718, lcd bringup
		if(!strcmp(Lcm_name, "lcd_hx83102e_lide_al_hsd_video_fhdp"))
		{
			DRM_INFO("====== esd recovery lcd_hx83102e reset&vddio down========\n");
			if (panel->info.reset_gpio) {
				gpiod_direction_output(panel->info.reset_gpio,0);
				mdelay(5);
				DRM_INFO("====== esd recovery lcd_hx83102e reset down success ========\n");
			}
			if (panel->info.iovdd_en_gpio) {
				gpiod_direction_output(panel->info.iovdd_en_gpio, 0);
				mdelay(50);
				DRM_INFO("====== esd recovery lcd_hx83102e iovdd down success ========\n");
				gpiod_direction_output(panel->info.iovdd_en_gpio, 1);
				mdelay(1);
				DRM_INFO("====== esd recovery lcd_hx83102e iovdd up success ========\n");
			}
		}
		//-Bug760817, pengzhenhua1.wt, add, 20220718, lcd bringup
...
//+Bug760817, pengzhenhua1.wt, add, 20220718, lcd bringup	
	panel->info.iovdd_en_gpio = devm_gpiod_get_optional(dev,
					"iovdd", GPIOD_ASIS);
	if (IS_ERR_OR_NULL(panel->info.iovdd_en_gpio))
		DRM_WARN("can't get panel iovdd gpio: %ld\n",
				 PTR_ERR(panel->info.iovdd_en_gpio));
//-Bug760817, pengzhenhua1.wt, add, 20220718, lcd bringup				 
	return 0;
}
	...
	struct sprd_panel *panel)
{
	...
	p = of_get_property(lcd_node, "sprd,esd-master-command", &bytes);
	if (p) {
		info->cmds[CMD_CODE_ESD_MASTER] = p;
		info->cmds_len[CMD_CODE_ESD_MASTER] = bytes;
	} else
		DRM_ERROR("can't find sprd,esd-master-command property\n");

	p = of_get_property(lcd_node, "sprd,esd-slave-command", &bytes);
	if (p) {
		info->cmds[CMD_CODE_ESD_SLAVE] = p;
		info->cmds_len[CMD_CODE_ESD_SLAVE] = bytes;
	} else
		DRM_ERROR("can't find sprd,esd-slave-command property\n");
