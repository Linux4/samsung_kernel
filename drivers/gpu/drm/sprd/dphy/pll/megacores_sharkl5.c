...
/* hs03 code for SL6215DEV-3924  by wenghailong at 2021/12/29 end */
{
	...
	case ZERO_TIME:
		...
		break;
	case TRAIL_TIME:
		regmap_write(regmap, 0x34, val[CLK] - 1);
		regmap_write(regmap, 0x44, val[DATA] - 3);
	...
	case EXIT_TIME:
		...
		regmap_write(regmap, 0x46, val[DATA] - 3);
		regmap_write(regmap, 0x56, val[DATA] - 1);
		regmap_write(regmap, 0x66, val[DATA] - 1);
		regmap_write(regmap, 0x76, val[DATA] - 1);
