...
//bug 766261,cjd.wt,modify for check RAM fail,2022/6/8
...
//bug 766261,cjd.wt,modify for check RAM fail,2022/6/8,start
	if (calibration_mode) {
		struct device_node *node;
		struct resource r;
		int ret;
		...
		if (unlikely(atomic_inc_not_zero(&logo2animation)) && r.start) {
			DRM_INFO("free logo memory addr:0x%lx size:0x%lx\n",
				r.start, resource_size(&r));
			free_reserved_area(phys_to_virt(r.start),
				phys_to_virt(r.start + resource_size(&r)),
				-1, "logo");
		}
		return -ENODEV;
	}
//bug 766261,cjd.wt,modify for check RAM fail,2022/6/8,end
