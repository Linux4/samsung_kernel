				...
				const char *buf, size_t count)
{
	...
	dphy->glb->dphy_global_disable(&dphy->ctx);
...
static ssize_t hop_store(struct device *dev,
			...
			const char *buf, size_t count)
{
/*Tab A8 code for AX6300DEV-207 by chenpengbo at 2021/09/18 start*/
#ifndef CONFIG_TARGET_UMS512_1H10
	int ret;
	int delta;
	struct sprd_dphy *dphy = dev_get_drvdata(dev);
	struct dphy_context *ctx = &dphy->ctx;
	...
	mutex_unlock(&dphy->ctx.lock);
#endif
/*Tab A8 code for AX6300DEV-207 by chenpengbo at 2021/09/18 end*/
	return count;
}
