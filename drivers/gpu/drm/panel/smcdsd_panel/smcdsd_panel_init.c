...
int smcdsd_panel_get_params_from_dt(struct drm_display_mode *drm,
	struct mtk_panel_params *ext, struct mipi_dsi_device *dsi)
{
	...
	plcd->lcm_params = params = kzalloc(sizeof(struct LCM_PARAMS), GFP_KERNEL);
	...
	drm->hdisplay = params->width;
	drm->hsync_start = params->width + params->dsi.horizontal_frontporch;
	drm->hsync_end = params->width + params->dsi.horizontal_frontporch + params->dsi.horizontal_sync_active;
	drm->htotal = params->width + params->dsi.horizontal_frontporch + params->dsi.horizontal_sync_active + params->dsi.horizontal_backporch;

	drm->vdisplay = params->height;
	drm->vsync_start = params->height + params->dsi.vertical_frontporch;
	drm->vsync_end = params->height + params->dsi.vertical_frontporch + params->dsi.vertical_sync_active;
	drm->vtotal = params->height + params->dsi.vertical_frontporch + params->dsi.vertical_sync_active + params->dsi.vertical_backporch;

	drm->vrefresh = 60;	/* todo but useless */
	...
	ext->ssc_disable = params->dsi.ssc_disable;
	ext->data_rate = params->dsi.data_rate;
	ext->pll_clk = params->dsi.PLL_CLOCK;

	dsi->lanes = params->dsi.LANE_NUM;
	dsi->format = MIPI_DSI_FMT_RGB888;	/* todo but useless */
	...
	if (params->dsi.mode == BURST_VDO_MODE)
		dsi->mode_flags |= (MIPI_DSI_MODE_VIDEO | MIPI_DSI_MODE_VIDEO_BURST);
	else if (params->dsi.mode == SYNC_PULSE_VDO_MODE)
		dsi->mode_flags |= (MIPI_DSI_MODE_VIDEO | MIPI_DSI_MODE_VIDEO_SYNC_PULSE);
	else if (params->dsi.mode == SYNC_EVENT_VDO_MODE)
		dsi->mode_flags |= (MIPI_DSI_MODE_VIDEO);

	if (params->dsi.noncont_clock)
