...
static int common_lcd_get_modes(struct drm_panel *panel)
{
	...
	mode = drm_mode_duplicate(panel->drm, &plcd->default_mode);
	if (!mode) {
		dev_err(panel->drm->dev, "failed to add mode %ux%ux@%u\n",
			plcd->default_mode.hdisplay, plcd->default_mode.vdisplay,
			plcd->default_mode.vrefresh);
		return -ENOMEM;
	}
	...
	panel->connector->display_info.width_mm = plcd->lcm_params->physical_width;
	panel->connector->display_info.height_mm = plcd->lcm_params->physical_height;
...
static int common_lcd_probe(struct mipi_dsi_device *dsi)
{
	...
	lcd_driver_init();
	smcdsd_panel_get_params_from_dt(&plcd->default_mode, &plcd->ext_params, dsi);
...
#if defined(CONFIG_MTK_PANEL_EXT)
	mtk_panel_tch_handle_reg(&plcd->panel);
	ret = mtk_panel_ext_create(dev, &plcd->ext_params, &ext_funcs, &plcd->panel);
	if (ret < 0)
		return ret;
#endif

	smcdsd_match[0].data = (void *)plcd;
