...
static int smcdsd_panel_send_msg(struct lcd_info *lcd, int force)
{
	...
	/* KEY OPEN */
	package[count++] = &PACKAGE_LIST[MSG_IDX_BASE][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_KEY_ENABLE)];

	/* ACL */
	package[count++] = &PACKAGE_LIST[MSG_IDX_BASE][acl_table[lcd->adaptive_control]];

	/* BRIGHTNESS */
	package[count++] = &PACKAGE_LIST[MSG_IDX_BRIGHTNESS][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_BRIGHTNESS) + lcd->brightness];
	package[count++] = &PACKAGE_LIST[MSG_IDX_BASE][wrcrl_table[lcd->brightness]];

	/* VRR */
	if (lcd->vrefresh == 60)
		package[count++] = &PACKAGE_LIST[MSG_IDX_BASE][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_FPS_60HZ)];
	else if (lcd->vrefresh == 90)
		package[count++] = &PACKAGE_LIST[MSG_IDX_BASE][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_FPS_90HZ)];

	/* UPDATE */
	package[count++] = &PACKAGE_LIST[MSG_IDX_BASE][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_PANEL_UPDATE)];

	/* KEY CLOSE */
	package[count++] = &PACKAGE_LIST[MSG_IDX_BASE][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_KEY_DISABLE)];
...
#if defined(CONFIG_SMCDSD_DYNAMIC_MIPI)
static int s6e8fc3_sdc_change_ffc(struct lcd_info *lcd, unsigned int ffc_on)
{
	int ret = 0;
	// ffc cmd
	if (ffc_on) {
 ...
 *			smcdsd_dsi_tx_package(lcd, &PACKAGE_LIST[MSG_IDX_BASE][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_MIPI_822)]);
 *		else if (lcd->df_index_req == DYNAMIC_MIPI_INDEX_2)
 *			smcdsd_dsi_tx_package(lcd, &PACKAGE_LIST[MSG_IDX_BASE][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_MIPI_824)]);
 *		else
 *			smcdsd_dsi_tx_package(lcd, &PACKAGE_LIST[MSG_IDX_BASE][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_MIPI_806)]);
 */
				...
				unsigned long size, void *buf)
{
	...
	if (msg->dev_id == IPC_SYSTEM_CP_CHANNEL_INFO &&
		...
		mutex_lock(&lcd->lock);
		if (lcd->state)
			s6e8fc3_sdc_change_ffc(lcd, FFC_OFF);
		mutex_unlock(&lcd->lock);
		smcdsd_commit_lock(lcd, false);
...
static int smcdsd_panel_framedone_dynamic_mipi(struct platform_device *p)
{
	...
	if (lcd) {
		...
		if (lcd->df_index_req != lcd->df_index_current)
			if (lcd->state)
				s6e8fc3_sdc_change_ffc(lcd, FFC_UPDATE);
		mutex_unlock(&lcd->lock);
		smcdsd_commit_lock(lcd, false);
	}
	return 0;
...
static int s6e8fc3_sdc_read_id(struct lcd_info *lcd)
{
	int i = 0, ret = 0;
	struct mipi_dsi_lcd_common *pdata = lcd->pdata;
...
static int s6e8fc3_sdc_read_date(struct lcd_info *lcd)
{
	int ret = 0;
	unsigned char buf[LDI_LEN_DATE] = {0, };

	smcdsd_dsi_tx_package(lcd, &PACKAGE_LIST[MSG_IDX_BASE][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_KEY_ENABLE)]);
	...
	smcdsd_dsi_tx_package(lcd, &PACKAGE_LIST[MSG_IDX_BASE][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_KEY_DISABLE)]);

	return ret;
}

static int s6e8fc3_sdc_read_chip_id(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e8fc3_sdc_read_init_info(struct lcd_info *lcd)
{
	int ret = 0;
	struct mipi_dsi_lcd_common *pdata = lcd->pdata;
	...
	s6e8fc3_sdc_read_id(lcd);

	if (!lcd->fac_info)
		return ret;

	s6e8fc3_sdc_read_date(lcd);
	s6e8fc3_sdc_read_chip_id(lcd);

	return ret;
}

static int s6e8fc3_sdc_exit(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	smcdsd_dsi_tx_package(lcd, &PACKAGE_LIST[MSG_IDX_BASE][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_EXIT)]);
...
static int s6e8fc3_sdc_init(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	s6e8fc3_sdc_read_init_info(lcd);

	smcdsd_dsi_tx_package(lcd, &PACKAGE_LIST[MSG_IDX_BASE][GET_ENUM_WITH_NAME(MSG_S6E8FC3_SDC_00_INIT)]);
#if defined(CONFIG_SMCDSD_DYNAMIC_MIPI)
	s6e8fc3_sdc_change_ffc(lcd, FFC_UPDATE);
#endif
...
static int s6e8fc3_sdc_displayon(struct lcd_info *lcd)
{
	int ret = 0;
	...
	/* 12. Display On(29h) */
	//send_cmd(S6E8FC3_SDC_00_DISPLAY_ON, ARRAY_SIZE(S6E8FC3_SDC_00_DISPLAY_ON));
...
static int s6e8fc3_sdc_register_notifier(struct lcd_info *lcd)
{
...
static int s6e8fc3_sdc_probe(struct lcd_info *lcd)
{
	dev_info(&lcd->ld->dev, "+ %s\n", __func__);
	...
	lcd->vrefresh = 90;	/* temp if vrr is working, then back it to 60 */
...
static void lcd_init_sysfs(struct lcd_info *lcd)
{
	...
	for (i = 0; i < (u16)ARRAY_SIZE(MSG_S6E8FC3_SDC_00_INIT); i++)
		init_debugfs_param("lcd_init", (void *)MSG_S6E8FC3_SDC_00_INIT[i].dsi_msg.tx_buf, U8_MAX, MSG_S6E8FC3_SDC_00_INIT[i].dsi_msg.tx_len, 0);
	for (i = 0; i < (u16)ARRAY_SIZE(MSG_S6E8FC3_SDC_00_EXIT); i++)
		init_debugfs_param("lcd_exit", (void *)MSG_S6E8FC3_SDC_00_EXIT[i].dsi_msg.tx_buf, U8_MAX, MSG_S6E8FC3_SDC_00_EXIT[i].dsi_msg.tx_len, 0);
}
...
static int smcdsd_panel_enable(struct platform_device *p)
{
	...
	s6e8fc3_sdc_displayon(lcd);	/* OFF,DOZE_SUSPEND-> ON, DOZE */
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	s6e8fc3_sdc_probe(lcd);

	s6e8fc3_sdc_register_notifier(lcd);
...
struct mipi_dsi_lcd_driver s6e8fc3_sdc_mipi_lcd_driver = {
	.driver = {
		.name = "s6e8fc3_sdc",
	},
...
__XX_ADD_LCD_DRIVER(s6e8fc3_sdc_mipi_lcd_driver);
