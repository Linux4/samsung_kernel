...
#include "a13x_td4160_boe_param.h"
#include "dd.h"
...
static int td4160_boe_read_id(struct lcd_info *lcd)
{
	int i = 0, ret = 0;
...
static int td4160_boe_read_init_info(struct lcd_info *lcd)
{
	int ret = 0;
	...
	td4160_boe_read_id(lcd);

	return ret;
}

static int td4160_boe_exit(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	DSI_WRITE(SEQ_TD4160_BOE_DISPLAY_OFF, ARRAY_SIZE(SEQ_TD4160_BOE_DISPLAY_OFF));

	msleep(20);

	DSI_WRITE(SEQ_TD4160_BOE_SLEEP_IN, ARRAY_SIZE(SEQ_TD4160_BOE_SLEEP_IN));
...
static int td4160_boe_init(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	td4160_boe_read_init_info(lcd);

	smcdsd_dsi_tx_set(lcd, LCD_SEQ_INIT_1, ARRAY_SIZE(LCD_SEQ_INIT_1));

	DSI_WRITE(SEQ_TD4160_BOE_BRIGHTNESS, ARRAY_SIZE(SEQ_TD4160_BOE_BRIGHTNESS));

	DSI_WRITE(SEQ_TD4160_BOE_SLEEP_OUT, ARRAY_SIZE(SEQ_TD4160_BOE_SLEEP_OUT));
	msleep(100);
	DSI_WRITE(SEQ_TD4160_BOE_DISPLAY_ON, ARRAY_SIZE(SEQ_TD4160_BOE_DISPLAY_ON));
...
static int td4160_boe_register_notifier(struct lcd_info *lcd)
{
...
static int td4160_boe_probe(struct lcd_info *lcd)
{
	dev_info(&lcd->ld->dev, "+ %s\n", __func__);
...
static void lcd_init_sysfs(struct lcd_info *lcd)
{
	...
	for (i = 0; i < ARRAY_SIZE(LCD_SEQ_INIT_1); i++)
		init_debugfs_param("lcd_init_01", LCD_SEQ_INIT_1[i].cmd, U8_MAX, LCD_SEQ_INIT_1[i].len, 50);
}
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	td4160_boe_probe(lcd);

	td4160_boe_register_notifier(lcd);
...
struct mipi_dsi_lcd_driver td4160_boe_mipi_lcd_driver = {
	.driver = {
		.name = "td4160_boe",
	},
...
__XX_ADD_LCD_DRIVER(td4160_boe_mipi_lcd_driver);
