...
#include "a13x_nt36525b_sharp_param.h"
#include "dd.h"
...
static int nt36525b_sharp_read_id(struct lcd_info *lcd)
{
	int i = 0, ret = 0;
...
static int nt36525b_sharp_read_init_info(struct lcd_info *lcd)
{
	int ret = 0;
	...
	nt36525b_sharp_read_id(lcd);

	return ret;
}

static int nt36525b_sharp_exit(struct lcd_info *lcd)
{
	int ret = 0;
	...
	DSI_WRITE(SEQ_NT36525B_SHARP_BACK_TO_PAGE_1_A, ARRAY_SIZE(SEQ_NT36525B_SHARP_BACK_TO_PAGE_1_A));
	DSI_WRITE(SEQ_NT36525B_SHARP_BACK_TO_PAGE_1_B, ARRAY_SIZE(SEQ_NT36525B_SHARP_BACK_TO_PAGE_1_B));

	DSI_WRITE(SEQ_NT36525B_SHARP_DISPLAY_OFF, ARRAY_SIZE(SEQ_NT36525B_SHARP_DISPLAY_OFF));

	msleep(20);

	DSI_WRITE(SEQ_NT36525B_SHARP_SLEEP_IN, ARRAY_SIZE(SEQ_NT36525B_SHARP_SLEEP_IN));

	return ret;
}

static int nt36525b_sharp_init(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	nt36525b_sharp_read_init_info(lcd);
	...
	DSI_WRITE(SEQ_NT36525B_SHARP_BRIGHTNESS, ARRAY_SIZE(SEQ_NT36525B_SHARP_BRIGHTNESS));

	DSI_WRITE(SEQ_NT36525B_SHARP_SLEEP_OUT, ARRAY_SIZE(SEQ_NT36525B_SHARP_SLEEP_OUT));
	msleep(100);
	DSI_WRITE(SEQ_NT36525B_SHARP_DISPLAY_ON, ARRAY_SIZE(SEQ_NT36525B_SHARP_DISPLAY_ON));
...
static int nt36525b_sharp_register_notifier(struct lcd_info *lcd)
{
...
static int nt36525b_sharp_probe(struct lcd_info *lcd)
{
	dev_info(&lcd->ld->dev, "+ %s\n", __func__);
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	nt36525b_sharp_probe(lcd);

	nt36525b_sharp_register_notifier(lcd);
...
struct mipi_dsi_lcd_driver nt36525b_sharp_mipi_lcd_driver = {
	.driver = {
		.name = "nt36525b_sharp",
	},
...
__XX_ADD_LCD_DRIVER(nt36525b_sharp_mipi_lcd_driver);
