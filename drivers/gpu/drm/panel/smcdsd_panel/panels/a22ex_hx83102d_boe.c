...
#include "a22ex_hx83102d_boe_param.h"
#include "dd.h"
...
static int hx83102d_boe_read_id(struct lcd_info *lcd)
{
	int i = 0, ret = 0;
...
static int hx83102d_boe_read_init_info(struct lcd_info *lcd)
{
	int ret = 0;
	...
	hx83102d_boe_read_id(lcd);

	return ret;
}

static int hx83102d_boe_exit(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	DSI_WRITE(SEQ_HX83102D_BOE_DISPLAY_OFF, ARRAY_SIZE(SEQ_HX83102D_BOE_DISPLAY_OFF));

	msleep(20);

	DSI_WRITE(SEQ_HX83102D_BOE_SLEEP_IN, ARRAY_SIZE(SEQ_HX83102D_BOE_SLEEP_IN));

	return ret;
}

static int hx83102d_boe_init(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	hx83102d_boe_read_init_info(lcd);

	if (cpu_to_be32(lcd->id_info.value) == 0x9A6234)
		smcdsd_dsi_tx_set(lcd, LCD_SEQ_INIT_AL_1, ARRAY_SIZE(LCD_SEQ_INIT_AL_1));
	else
		smcdsd_dsi_tx_set(lcd, LCD_SEQ_INIT_CU_1, ARRAY_SIZE(LCD_SEQ_INIT_CU_1));

	DSI_WRITE(SEQ_HX83102D_BOE_SLEEP_OUT, ARRAY_SIZE(SEQ_HX83102D_BOE_SLEEP_OUT));
	msleep(50);
	DSI_WRITE(SEQ_HX83102D_BOE_DISPLAY_ON, ARRAY_SIZE(SEQ_HX83102D_BOE_DISPLAY_ON));
...
static int hx83102d_boe_register_notifier(struct lcd_info *lcd)
{
...
static int hx83102d_boe_probe(struct lcd_info *lcd)
{
	dev_info(&lcd->ld->dev, "+ %s\n", __func__);
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	hx83102d_boe_probe(lcd);

	hx83102d_boe_register_notifier(lcd);
...
struct mipi_dsi_lcd_driver hx83102d_boe_mipi_lcd_driver = {
	.driver = {
		.name = "hx83102d_boe",
	},
...
__XX_ADD_LCD_DRIVER(hx83102d_boe_mipi_lcd_driver);
