...
#include "a13x_nt36525c_boe_param.h"
#include "dd.h"
...
static int nt36525c_boe_read_id(struct lcd_info *lcd)
{
	int i = 0, ret = 0;
...
static int nt36525c_boe_read_init_info(struct lcd_info *lcd)
{
	int ret = 0;
	...
	nt36525c_boe_read_id(lcd);

	return ret;
}

static int nt36525c_boe_exit(struct lcd_info *lcd)
{
	int ret = 0;
	...
	DSI_WRITE(SEQ_NT36525C_BOE_BACK_TO_PAGE_1_A, ARRAY_SIZE(SEQ_NT36525C_BOE_BACK_TO_PAGE_1_A));
	DSI_WRITE(SEQ_NT36525C_BOE_BACK_TO_PAGE_1_B, ARRAY_SIZE(SEQ_NT36525C_BOE_BACK_TO_PAGE_1_B));

	DSI_WRITE(SEQ_NT36525C_BOE_DISPLAY_OFF, ARRAY_SIZE(SEQ_NT36525C_BOE_DISPLAY_OFF));

	msleep(20);

	DSI_WRITE(SEQ_NT36525C_BOE_SLEEP_IN, ARRAY_SIZE(SEQ_NT36525C_BOE_SLEEP_IN));

	return ret;
}

static int nt36525c_boe_init(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	nt36525c_boe_read_init_info(lcd);
	...
	DSI_WRITE(SEQ_NT36525C_BOE_BRIGHTNESS, ARRAY_SIZE(SEQ_NT36525C_BOE_BRIGHTNESS));

	DSI_WRITE(SEQ_NT36525C_BOE_SLEEP_OUT, ARRAY_SIZE(SEQ_NT36525C_BOE_SLEEP_OUT));
	msleep(100);
	DSI_WRITE(SEQ_NT36525C_BOE_DISPLAY_ON, ARRAY_SIZE(SEQ_NT36525C_BOE_DISPLAY_ON));
...
static int nt36525c_boe_register_notifier(struct lcd_info *lcd)
{
...
static int nt36525c_boe_probe(struct lcd_info *lcd)
{
	dev_info(&lcd->ld->dev, "+ %s\n", __func__);
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	nt36525c_boe_probe(lcd);

	nt36525c_boe_register_notifier(lcd);
...
struct mipi_dsi_lcd_driver nt36525c_boe_mipi_lcd_driver = {
	.driver = {
		.name = "nt36525c_boe",
	},
...
__XX_ADD_LCD_DRIVER(nt36525c_boe_mipi_lcd_driver);
