...
static int mdnie_write_table(struct mdnie_info *mdnie, struct mdnie_table *table)
{
	...
	for (i = 0; table->seq[i].dsi_msg.tx_len; i++) {
		if (IS_ERR_OR_NULL(table->seq[i].dsi_msg.tx_buf)) {
...
static struct mdnie_table *mdnie_find_table(struct mdnie_info *mdnie)
{
	...
	if (IS_LIGHT_NOTIFICATION(mdnie->light_notification)) {
		...
		goto exit;
	} else if (IS_HMD(mdnie->hmd_mode)) {
		table = mdnie->tune->hmd_table ? &mdnie->tune->hmd_table[mdnie->hmd_mode] : NULL;
		goto exit;
...
int mdnie_force_update(struct device *dev, void *data)
{
	...
	ret = write(mdnie->data, table->seq, i);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (value != mdnie->hmd_mode && value < HMD_MDNIE_MAX) {
		mutex_lock(&mdnie->lock);
		mdnie->hmd_mode = value;
		mutex_unlock(&mdnie->lock);
		mdnie_update(mdnie);
	}
...
static DEVICE_ATTR_RW(hdr);
static DEVICE_ATTR_RW(mdnie_ldu);
static DEVICE_ATTR_RW(light_notification);
static DEVICE_ATTR_RW(color_lens);
#ifdef CONFIG_SMCDSD_HMD
static DEVICE_ATTR(hmt_color_temperature, 644, hmd_color_temperature_show, hmd_color_temperature_store);
#endif
...
static int mdnie_register_fb(struct mdnie_info *mdnie)
{
	...
	return smcdsd_register_notifier(&mdnie->fb_notif);
}
