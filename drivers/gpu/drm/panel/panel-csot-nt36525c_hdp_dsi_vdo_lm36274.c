...
#define LCM_I2C_ID_NAME 	"nt36525c_lm36274"
 ...
 * Function Prototype
 *****************************************************************************/
static int _nt36525c_lcm_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id);
static int _nt36525c_lcm_i2c_remove(struct i2c_client *client);
...
static struct i2c_driver _lcm_i2c_driver = {
	...
	.probe = _nt36525c_lcm_i2c_probe,
	.remove = _nt36525c_lcm_i2c_remove,
	.driver = {
		.owner = THIS_MODULE,
 ...
 *****************************************************************************/
#define LCD_CSOT_NT36525C_ID      (0x3)
static int _nt36525c_lcm_i2c_probe(struct i2c_client *client,
			  const struct i2c_device_id *id)
{
	struct device_node *lcd_comp;
	int ret;

	pr_err("[LCM][I2C][nt36525c] %s\n", __func__);
	pr_debug("[LCM][I2C][nt36525c] NT: info==>name=%s addr=0x%x\n", client->name,
		 client->addr);
	...
	if (ret != LCD_CSOT_NT36525C_ID && ret != 0) {
		pr_info("nt36525c : %s : LCD HW ID doesn't match\n", __func__);
		return -1;
	}
#endif
	...
	"k6833v1,nt36525c,vdo");
	if (lcd_comp) {
		pr_info("nt36525c : %s : panel compatible match\n", __func__);
		return 0;
	}

	pr_info("nt36525c : %s : panel compatible doesn't match\n", __func__);
	return -EPERM;
#else
	return 0;
#endif
...
static int _lcm_i2c_write_bytes(unsigned char addr, unsigned char value)
{
	...
	if (client == NULL) {
		pr_err("ERROR!![nt36525c] _lcm_i2c_client is null\n");
		return 0;
	}
	...
	if (ret < 0)
		pr_err("[LCM][ERROR][nt36525c] _lcm_i2c write data fail !!\n");
 ...
 */
static int __init _nt36525c_lcm_i2c_init(void)
{
	pr_info("[LCM][I2C][nt36525c] %s\n", __func__);
	i2c_add_driver(&_lcm_i2c_driver);
	pr_info("[LCM][I2C][nt36525c] %s success\n", __func__);
	return 0;
}

static void __exit _nt36525c_lcm_i2c_exit(void)
{
	pr_info("[LCM][I2C][nt36525c] %s\n", __func__);
	i2c_del_driver(&_lcm_i2c_driver);
}

module_init(_nt36525c_lcm_i2c_init);
module_exit(_nt36525c_lcm_i2c_exit);
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	lcm_dcs_write_seq_static(ctx, 0xB0, 0x00, 0x08, 0x00, 0x09, 0x00, 0x3B, 0x00, 0x54, 0x00, 0x6B,
		0x00, 0x7F, 0x00, 0x91, 0x00, 0xA2);
	lcm_dcs_write_seq_static(ctx, 0xB1, 0x00, 0xB2, 0x00, 0xE7, 0x01, 0x0F, 0x01, 0x4F, 0x01, 0x7F,
		0x01, 0xCC, 0x01, 0xF9, 0x01, 0xFB);
	lcm_dcs_write_seq_static(ctx, 0xB2, 0x02, 0x46, 0x02, 0x89, 0x02, 0xB6, 0x02, 0xEB, 0x03, 0x12,
		0x03, 0x3D, 0x03, 0x4E, 0x03, 0x5D);
	lcm_dcs_write_seq_static(ctx, 0xB3, 0x03, 0x6F, 0x03, 0x86, 0x03, 0xA3, 0x03, 0xC3, 0x03, 0xDB,
		0x03, 0xDF);
	lcm_dcs_write_seq_static(ctx, 0xB4, 0x00, 0x08, 0x00, 0x09, 0x00, 0x3B, 0x00, 0x54, 0x00, 0x6B,
		0x00, 0x7F, 0x00, 0x91, 0x00, 0xA2);
	lcm_dcs_write_seq_static(ctx, 0xB5, 0x00, 0xB2, 0x00, 0xE7, 0x01, 0x0F, 0x01, 0x4F, 0x01, 0x7F,
		0x01, 0xCC, 0x01, 0xF9, 0x01, 0xFB);
	lcm_dcs_write_seq_static(ctx, 0xB6, 0x02, 0x46, 0x02, 0x89, 0x02, 0xB6, 0x02, 0xEB, 0x03, 0x12,
		0x03, 0x3D, 0x03, 0x4E, 0x03, 0x5D);
	lcm_dcs_write_seq_static(ctx, 0xB7, 0x03, 0x6F, 0x03, 0x86, 0x03, 0xA3, 0x03, 0xC3, 0x03, 0xDB,
		0x03, 0xDF);
	lcm_dcs_write_seq_static(ctx, 0xB8, 0x00, 0x08, 0x00, 0x09, 0x00, 0x3B, 0x00, 0x54, 0x00, 0x6B,
		0x00, 0x7F, 0x00, 0x91, 0x00, 0xA2);
	lcm_dcs_write_seq_static(ctx, 0xB9, 0x00, 0xB2, 0x00, 0xE7, 0x01, 0x0F, 0x01, 0x4F, 0x01, 0x7F,
		0x01, 0xCC, 0x01, 0xF9, 0x01, 0xFB);
	lcm_dcs_write_seq_static(ctx, 0xBA, 0x02, 0x46, 0x02, 0x89, 0x02, 0xB6, 0x02, 0xEB, 0x03, 0x12,
		0x03, 0x3D, 0x03, 0x4E, 0x03, 0x5D);
	lcm_dcs_write_seq_static(ctx, 0xBB, 0x03, 0x6F, 0x03, 0x86, 0x03, 0xA3, 0x03, 0xC3, 0x03, 0xDB,
		0x03, 0xDF);
	  
	lcm_dcs_write_seq_static(ctx, 0xFF, 0x21);
	lcm_dcs_write_seq_static(ctx, 0xFB, 0x01);

	lcm_dcs_write_seq_static(ctx, 0xB0, 0x00, 0x00, 0x00, 0x01, 0x00, 0x33, 0x00, 0x4C, 0x00, 0x63,
		0x00, 0x77, 0x00, 0x89, 0x00, 0x9A);
	lcm_dcs_write_seq_static(ctx, 0xB1, 0x00, 0xAA, 0x00, 0xDF, 0x01, 0x07, 0x01, 0x47, 0x01, 0x77,
		0x01, 0xC4, 0x02, 0x11, 0x02, 0x13);
	lcm_dcs_write_seq_static(ctx, 0xB2, 0x02, 0x3E, 0x02, 0x81, 0x02, 0xAE, 0x02, 0xE3, 0x03, 0x0A,
		0x03, 0x35, 0x03, 0x46, 0x03, 0x55);
	lcm_dcs_write_seq_static(ctx, 0xB3, 0x03, 0x67, 0x03, 0x7E, 0x03, 0x9B, 0x03, 0xBB, 0x03, 0xD3,
		0x03, 0xD7);
	lcm_dcs_write_seq_static(ctx, 0xB4, 0x00, 0x00, 0x00, 0x01, 0x00, 0x33, 0x00, 0x4C, 0x00, 0x63,
		0x00, 0x77, 0x00, 0x89, 0x00, 0x9A);
	lcm_dcs_write_seq_static(ctx, 0xB5, 0x00, 0xAA, 0x00, 0xDF, 0x01, 0x07, 0x01, 0x47, 0x01, 0x77,
		0x01, 0xC4, 0x02, 0x11, 0x02, 0x13);
	lcm_dcs_write_seq_static(ctx, 0xB6, 0x02, 0x3E, 0x02, 0x81, 0x02, 0xAE, 0x02, 0xE3, 0x03, 0x0A,
		0x03, 0x35, 0x03, 0x46, 0x03, 0x55);
	lcm_dcs_write_seq_static(ctx, 0xB7, 0x03, 0x67, 0x03, 0x7E, 0x03, 0x9B, 0x03, 0xBB, 0x03, 0xD3,
		0x03, 0xD7);
	lcm_dcs_write_seq_static(ctx, 0xB8, 0x00, 0x00, 0x00, 0x01, 0x00, 0x33, 0x00, 0x4C, 0x00, 0x63,
		0x00, 0x77, 0x00, 0x89, 0x00, 0x9A);
	lcm_dcs_write_seq_static(ctx, 0xB9, 0x00, 0xAA, 0x00, 0xDF, 0x01, 0x07, 0x01, 0x47, 0x01, 0x77,
		0x01, 0xC4, 0x02, 0x11, 0x02, 0x13);
	lcm_dcs_write_seq_static(ctx, 0xBA, 0x02, 0x3E, 0x02, 0x81, 0x02, 0xAE, 0x02, 0xE3, 0x03, 0x0A,
		0x03, 0x35, 0x03, 0x46, 0x03, 0x55);
	lcm_dcs_write_seq_static(ctx, 0xBB, 0x03, 0x67, 0x03, 0x7E, 0x03, 0x9B, 0x03, 0xBB, 0x03, 0xD3,
		0x03, 0xD7);
	...
	lcm_dcs_write_seq_static(ctx, 0xFB, 0x01);
	lcm_dcs_write_seq_static(ctx, 0x93, 0x1C);
	...
	lcm_dcs_write_seq_static(ctx, 0x06, 0x1C);
	...
	lcm_dcs_write_seq_static(ctx, 0x35, 0x0D);
	...
	lcm_dcs_write_seq_static(ctx, 0x3B, 0x66);
	...
	lcm_dcs_write_seq_static(ctx, 0x48, 0x22);
	lcm_dcs_write_seq_static(ctx, 0x49, 0x00);
	lcm_dcs_write_seq_static(ctx, 0x4A, 0x10);
	lcm_dcs_write_seq_static(ctx, 0x4B, 0x66);
	...
	lcm_dcs_write_seq_static(ctx, 0x5A, 0x1C);
	lcm_dcs_write_seq_static(ctx, 0x5B, 0x72);
	...
	lcm_dcs_write_seq_static(ctx, 0x80, 0x0D);
	lcm_dcs_write_seq_static(ctx, 0x81, 0x08);
	lcm_dcs_write_seq_static(ctx, 0x82, 0x0D);
	...
	lcm_dcs_write_seq_static(ctx, 0x92, 0x8F);
	...
	lcm_dcs_write_seq_static(ctx, 0xB1, 0x8B);
	...
	lcm_dcs_write_seq_static(ctx, 0x0B, 0xB7);
	...
	lcm_dcs_write_seq_static(ctx, 0xC1, 0x3E);
	...
	lcm_dcs_write_seq_static(ctx, 0x11);	/* Exit sleep mode */
	msleep(150);
	lcm_dcs_write_seq_static(ctx, 0x29);	/* Set display on */
}

static int nt36525c_lcm_disable(struct drm_panel *panel)
{
...
static int nt36525c_lcm_unprepare(struct drm_panel *panel)
{
	struct lcm *ctx = panel_to_lcm(panel);
...
static int nt36525c_lcm_prepare(struct drm_panel *panel)
{
	struct lcm *ctx = panel_to_lcm(panel);
	int ret;
...
static int nt36525c_lcm_enable(struct drm_panel *panel)
{
	struct lcm *ctx = panel_to_lcm(panel);
...
#if defined(CONFIG_MTK_PANEL_EXT)
static int nt36525c_panel_ext_reset(struct drm_panel *panel, int on)
{
	struct lcm *ctx = panel_to_lcm(panel);
...
static int nt36525c_panel_ata_check(struct drm_panel *panel)
{
	struct lcm *ctx = panel_to_lcm(panel);
	struct mipi_dsi_device *dsi = to_mipi_dsi_device(ctx->dev);
...
static int nt36525c_lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb,
...
static int nt36525c_lcm_get_virtual_heigh(void)
{
	return VAC;
}

static int nt36525c_lcm_get_virtual_width(void)
{
	return HAC;
}
...
static struct mtk_panel_funcs ext_funcs = {
	.reset = nt36525c_panel_ext_reset,
	.set_backlight_cmdq = nt36525c_lcm_setbacklight_cmdq,
	.ata_check = nt36525c_panel_ata_check,
	.get_virtual_heigh = nt36525c_lcm_get_virtual_heigh,
	.get_virtual_width = nt36525c_lcm_get_virtual_width,
};
#endif
...
static int nt36525c_lcm_get_modes(struct drm_panel *panel)
{
...
static const struct drm_panel_funcs lcm_drm_funcs = {
	.disable = nt36525c_lcm_disable,
	.unprepare = nt36525c_lcm_unprepare,
	.prepare = nt36525c_lcm_prepare,
	.enable = nt36525c_lcm_enable,
	.get_modes = nt36525c_lcm_get_modes,
};
...
static int nt36525c_lcm_probe(struct mipi_dsi_device *dsi)
{
...
static int nt36525c_lcm_remove(struct mipi_dsi_device *dsi)
{
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "k6833v1,nt36525c,vdo", },
	{ }
};

MODULE_DEVICE_TABLE(of, lcm_of_match);

static struct mipi_dsi_driver nt36525c_lcm_driver = {
	.probe = nt36525c_lcm_probe,
	.remove = nt36525c_lcm_remove,
	.driver = {
		.name = "panel-k6833v1_64-nt36525c-vdo",
		.owner = THIS_MODULE,
		.of_match_table = lcm_of_match,
	},
};

module_mipi_dsi_driver(nt36525c_lcm_driver);

MODULE_AUTHOR("Rio Moon <rio.moon@mediatek.com>");
MODULE_DESCRIPTION("Novatek nt36525c VDO LCD Panel Driver");
