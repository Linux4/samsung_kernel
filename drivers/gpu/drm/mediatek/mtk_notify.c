...
int noti_uevent_user(struct mtk_uevent_dev *udev, int state)
{
	...
	n = snprintf(name_buf, sizeof(name_buf), "%s", udev->name);

	if (n < 0 || n >= sizeof(name_buf))
		strcpy(name_buf, "unknown error");

	envp[0] = name_buf;
	n = snprintf(state_buf, sizeof(state_buf), "SWITCH_STATE=%d", udev->state);

	if (n < 0 || n >= sizeof(state_buf))
		strcpy(state_buf, "unknown error");
...
int mtk_notifier_callback(struct notifier_block *p, unsigned long event, void *data)
{
	...
	if (event == MTK_FPS_CHANGE) {
		unsigned int fps = *((unsigned int *)data);
	...
	if (event == MTK_POWER_MODE_CHANGE) {
		unsigned int power_mode = *((unsigned int *)data);
...
int mtk_check_powermode(struct drm_atomic_state *state, int mode)
{
	...
	for_each_oldnew_crtc_in_state(state, crtc, old_crtc_state, new_crtc_state, i) {
		...
		if (powerMode != DISP_NONE && pre_powerMode != DISP_NONE) {
			DDPINFO("%s : %s -> %s\n", __func__,
