...
*/
#include "ss_dsi_panel_S6E3FC3_AMS667YM01.h"
#include "ss_dsi_mdnie_S6E3FC3_AMS667YM01.h"
					...
					int *level_key, bool is_hbm, bool is_hmt)
{
	...
	if (cur_rr == 60) {
		...
		if (ss_panel_revision(vdd) <= 'B') {
			LCD_INFO("Old Panel Support 96Hz\n");
			vrr_cmds->cmds[0].ss_txbuf[1] = 0x00; /* 96 HZ */
			vrr_cmds->cmds[0].ss_txbuf[2] = 0x04; /* 96 HZ */
		} else {
			LCD_INFO("New Panel Support 90Hz\n");
							...
							(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (ss_panel_revision(vdd) >= 'G') {
		finger_mask_update_delay = vdd->vrr.cur_refresh_rate > 60 ? 11000 : 17000;
		...
		*/
	...
	} else	if (ss_panel_revision(vdd) >= 'F') {
		/* Smooth transition : 0x28 */
		pcmds->cmds[1].ss_txbuf[1] = vdd->finger_mask_updated ? 0x20 : 0x28;
		...
		pcmds->cmds[3].ss_txbuf[1] = vdd->br_info.temperature > 0 ?
				vdd->br_info.temperature : (char)(BIT(7) | (-1 * vdd->br_info.temperature));
		/* AOR */
		pcmds->cmds[5].ss_txbuf[1] = normal_aor_manual[cd_index].aor_63h_119;
		pcmds->cmds[5].ss_txbuf[2] = normal_aor_manual[cd_index].aor_63h_120;
		pcmds->cmds[5].ss_txbuf[3] = normal_aor_manual[cd_index].aor_63h_121;
	} else {
		/* Smooth transition : 0x28 */
							...
							(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (ss_panel_revision(vdd) >= 'G') {
		...
		pcmds->cmds[3].ss_txbuf[1] = vdd->br_info.temperature > 0 ?
				vdd->br_info.temperature : (char)(BIT(7) | (-1 * vdd->br_info.temperature));
		/* AOR */
		pcmds->cmds[5].ss_txbuf[1] = hbm_aor_manual[cd_index].aor_63h_119;
		pcmds->cmds[5].ss_txbuf[2] = hbm_aor_manual[cd_index].aor_63h_120;
		pcmds->cmds[5].ss_txbuf[3] = hbm_aor_manual[cd_index].aor_63h_121;
	} else {
		pcmds->cmds[3].ss_txbuf[1] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 8, 8);
		pcmds->cmds[3].ss_txbuf[2] = get_bit(vdd->br_info.common_br.gm2_wrdisbv, 0, 8);
...
void S6E3FC3_AMS667YM01_FHD_init(struct samsung_display_driver_data *vdd)
{
	...
	/* SAMSUNG_FINGERPRINT */
	/* te high -> 70us (90fps) -> te low -> tx start */
	vdd->panel_hbm_entry_after_te = 80;
