...
static int self_aclock_set(struct samsung_display_driver_data *vdd)
{
	...
	if ((sa_info.pos_x - (AC_HH_MEM_REUSE_W/2)) >= 0)
		mem_reuse_x = sa_info.pos_x - (AC_HH_MEM_REUSE_W/2);
	else
		mem_reuse_x = 0;

	if ((sa_info.pos_y - (AC_HH_MEM_REUSE_H/2)) >= 0)
		mem_reuse_y = sa_info.pos_x - (AC_HH_MEM_REUSE_H/2);
	else
		mem_reuse_y = 0;
...
static int self_dclock_set(struct samsung_display_driver_data *vdd)
{
	...
	if (vdd->self_disp.st_info.disp_24h) {
		cmd_pload[4] |= vdd->self_disp.st_info.disp_24h & 0x30;
	} else {
		cmd_pload[4] &= ~(BIT(4));
		cmd_pload[4] &= ~(BIT(5));
	}
	...
	/* Color */
	cmd_pload[25] = (sd_info.color & 0xF000) >> 12; /* A */
	cmd_pload[26] = (sd_info.color & 0xF00) >> 8; /* R */
	cmd_pload[27] = (sd_info.color & 0xF0) >> 4; /* G */
	cmd_pload[28] = (sd_info.color & 0xF); /* B */
...
static int self_partial_hlpm_scan_set(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO("Self Partial HLPM/Scan hlpm_En(%d), hlpm_mode_sel(0x%x), hlpm_a1(%d), hlpm_a2(%d),\
				hlpm_a3(%d), hlpm_a4(%d) / scan_en(%d), scan_line(%d_%d)\n",
...
int self_display_init_HA9(struct samsung_display_driver_data *vdd)
{
	...
	if (vdd->ndx == PRIMARY_DISPLAY_NDX)
		sprintf(devname, "self_display");
	else
		sprintf(devname, "self_display%d", vdd->ndx);
