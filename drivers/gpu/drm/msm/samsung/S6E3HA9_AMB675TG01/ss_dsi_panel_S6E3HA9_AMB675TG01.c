...
static struct dsi_panel_cmd_set *ss_acl_on(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*level_key = LEVEL1_KEY;
	...
	if (vdd->br.bl_level > 25500) {
		...
		pcmds->cmds[0].msg.tx_buf[6] = 0x26;	/* use hbm acl percentage 8% */
	} else {
		pcmds->cmds[0].msg.tx_buf[3] = 0x40;	/* use normal acl statr step 50% */
		pcmds->cmds[0].msg.tx_buf[4] = 0xFF;	/* use normal acl statr step 50% */
		
		pcmds->cmds[0].msg.tx_buf[6] = 0x55;	/* use normal acl percentage 15% */
	}
#endif
