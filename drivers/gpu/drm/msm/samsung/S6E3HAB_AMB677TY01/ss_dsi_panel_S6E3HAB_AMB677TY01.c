...
//#include "ss_dsi_interpolation_S6E3HAB_AMB677TY01.h" // JUN_TEMP
...
static int ss_smart_dimming_init(struct samsung_display_driver_data *vdd)
{
	...
	// JUN_TEMP br_tbl->smart_dimming_dsi = sconf;
...
static struct dsi_panel_cmd_set *ss_vint(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	// JUN_TEMP int cur_rr = vdd->vrr.cur_refresh_rate;
	...
	/* 48/60NM/60HS: 5th (off=4), 96/120hz: 29th (off=28) */
	// JUN_TEMP  vint_cmds->cmds[0].msg.tx_buf[1] = (cur_rr <= 60) ? 4 : 28;
...
static struct dsi_panel_cmd_set *ss_gamma(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	//JUN_TEMP struct brightness_table *br_tbl = ss_get_cur_br_tbl(vdd);
...
static int ss_gct_write(struct samsung_display_driver_data *vdd)
{
	...
	// JUN_TEMP vdd->mafpc.force_delay = true;
...
static void samsung_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_dimming_init = NULL;	// ss_smart_dimming_init;
	vdd->panel_func.samsung_smart_get_conf     = NULL; 	// smart_get_conf_S6E3HAB_AMB677TY01;
	...
	// JUN_TEMP vdd->panel_func.samsung_brightness_dbv = NULL; // ss_dbv;
	...
	// JUN_TEMP vdd->panel_func.samsung_brightness_ltps = NULL; // ss_ltps;
	// JUN_TEMP vdd->panel_func.samsung_brightness_vrr  = NULL; // ss_vrr;
	...
	// JUN_TEMP vdd->panel_func.samsung_brightness_vrr_hbm = NULL; // ss_vrr_hbm;

	/* Total level key in brightness */
	// JUN_TEMP vdd->panel_func.samsung_brightness_tot_level = NULL; // ss_brightness_tot_level;

	/* Event */

	/* HMT */
	vdd->panel_func.samsung_brightness_gamma_hmt	= NULL; // ss_gamma_hmt;
	vdd->panel_func.samsung_brightness_aid_hmt 	= NULL; // ss_aid_hmt;
	vdd->panel_func.samsung_brightness_elvss_hmt	= NULL; // ss_elvss_hmt;
	// JUN_TEMP vdd->panel_func.samsung_brightness_vrr_hmt	= NULL; // ss_vrr_hmt;
	vdd->panel_func.samsung_smart_dimming_hmt_init	= NULL; //ss_smart_dimming_init_hmt;
	vdd->panel_func.samsung_smart_get_conf_hmt	= NULL; // smart_get_conf_S6E3HAB_AMB677TY01_hmt;
	...
	/* Set IRC init value */
	vdd->br.irc_mode = IRC_FLAT_GAMMA_MODE; /* default value */
	...
	/* VRR */
	// JUN_TEMP ss_vrr_init(&vdd->vrr);

	// JUN_TEMP vdd->panel_func.samsung_check_support_mode = ss_check_support_mode;
}
