...
void ss_copr_set_cmd(struct samsung_display_driver_data *vdd, struct COPR_CMD *cmd)
{
	...
	for (i = 0; i < roi_max; i++) {
		if (vdd->copr.ver <= COPR_VER_2P0) {
			cmd_pload[roi_start++] |= ((cmd->roi[i].ROI_X_S & 0x700) >> 8);
			...
			cmd_pload[roi_start++] = ((cmd->roi[i].ROI_X_E & 0x700) >> 8);
...
int ss_get_copr_orig_cmd(struct samsung_display_driver_data *vdd)
{
	...
	for (i = 0; i < roi_max; i++) {
		if (vdd->copr.ver <= COPR_VER_2P0) {
			cmd->roi[i].ROI_X_S = ((cmd_pload[roi_start++] & 0x07) << 8);
			...
			cmd->roi[i].ROI_X_E = ((cmd_pload[roi_start++] & 0x07) << 8);
