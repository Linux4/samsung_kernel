...
static bool isl98611_is_fault(struct blic_message *message)
{
	int init_loop;
	...
	for (init_loop = 0; init_loop < message->size; init_loop++) {
		...
		if (address == 0x01 && value != 0) {
			is_fault = true;
			LCD_ERR("BLIC fault found: 0x%x\n", value);
		}
	}
...
void isl98611_bl_ic_en(int enable)
{
	...
	char address, data;
	int init_loop;
	int retry = 2; // recover blic once if fault found
	...
	if (enable) {
		while (retry--) {
			...
			if (!isl98611_is_fault(&info->pdata->init_data) || !retry)
				break;

			LCD_ERR("BLIC fault error: reset blic, retry=%d\n", retry);
			backlight_i2c_write(info->client, 0x01, 0x3F, 1);
			msleep(100);
		}
