...
__visible_for_testing int get_current_ddi_osc_index(struct panel_device *panel)
{
...
__visible_for_testing int find_s6e3fab_vrr(struct panel_vrr *vrr)
{
	size_t i;
...
__visible_for_testing int getidx_s6e3fab_vrr_mode(int mode)
{
...
__visible_for_testing int init_elvss_cal_offset_table(struct maptbl *tbl)
{
	...
	maptbl_for_each(tbl, index)
		tbl->arr[index] = elvss_cal_offset_otp_value;
...
__visible_for_testing int getidx_vrr_aor_fq_brt_table(struct maptbl *tbl)
{
	...
	if (row < 0 || row >= maptbl_get_countof_row(tbl)) {
		panel_err("aor_fq_brt table is invalid, brightness %d row %d nrow %d\n",
				panel_bl->bd->props.brightness, row,
...
__visible_for_testing int s6e3fab_getidx_vddm_table(struct maptbl *tbl)
{
	struct panel_device *panel = (struct panel_device *)tbl->pdata;
...
__visible_for_testing int init_color_blind_table(struct maptbl *tbl)
{
	...
	if (S6E3FAB_SCR_CR_OFS + mdnie->props.sz_scr > maptbl_get_sizeof_maptbl(tbl)) {
...
__visible_for_testing int init_mdnie_night_mode_table(struct maptbl *tbl)
{
	...
	if (maptbl_get_sizeof_maptbl(tbl) < (S6E3FAB_NIGHT_MODE_OFS +
...
__visible_for_testing int init_mdnie_color_lens_table(struct maptbl *tbl)
{
	...
	if (maptbl_get_sizeof_maptbl(tbl) < (S6E3FAB_COLOR_LENS_OFS +
...
__visible_for_testing int gamma_to_reg_map_table(struct panel_device *panel,
			u8 (*new_gamma)[MAX_S6E3FAB_FQ_SET][S6E3FAB_GAMMA_MTP_LEN], int vrr_index, int br_index)
{
	...
	__visible_for_testing int copy_order_mtp[MAX_S6E3FAB_FQ_SET] = {
		S6E3FAB_FQ_LOW_7,
		S6E3FAB_FQ_LOW_6,
		S6E3FAB_FQ_LOW_5,
		S6E3FAB_FQ_LOW_4,
...
__visible_for_testing void gamma_itoc(u8 *dst, s32(*src)[MAX_COLOR], int nr_tp)
{
...
__visible_for_testing int gamma_ctoi(s32 (*dst)[MAX_COLOR], u8 *src, int nr_tp)
{
...
__visible_for_testing int do_gamma_update(struct panel_device *panel, int vrr_index, int br_index)
{
	int ret;
	struct panel_info *panel_data;
...
__visible_for_testing int __init s6e3fab_panel_init(void)
{
...
__visible_for_testing void __exit s6e3fab_panel_exit(void)
{
