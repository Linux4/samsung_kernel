...
__visible_for_testing DEFINE_REDIRECT_MOCKABLE(s6e3had_get_vrr_lfd_min_freq, RETURNS(int), PARAMS(struct panel_info *));
__visible_for_testing int REAL_ID(s6e3had_get_vrr_lfd_min_freq)(struct panel_info *panel_data)
{
	struct panel_device *panel = to_panel_device(panel_data);
...
__visible_for_testing DEFINE_REDIRECT_MOCKABLE(s6e3had_get_vrr_lfd_max_freq, RETURNS(int), PARAMS(struct panel_info *));
__visible_for_testing int REAL_ID(s6e3had_get_vrr_lfd_max_freq)(struct panel_info *panel_data)
{
	struct panel_device *panel = to_panel_device(panel_data);
	int vrr_fps, div_count;
...
__visible_for_testing int gamma_sum(s32 (*dst)[MAX_COLOR], s32 (*src)[MAX_COLOR],
...
__visible_for_testing int getidx_gm2_elvss_table(struct maptbl *tbl)
{
	...
	/* TODO: range check must be maptbl's common function */
	if (row < 0)
		return -EINVAL;
...
__visible_for_testing int getidx_sync_control_table(struct maptbl *tbl)
{
	...
	/*
	 * TODO: need to check whether if it is same with
	 *       row = (is_hbm_brightness(panel_bl, prev_brightness) ?
	 *					SMOOTH_TRANS_ON : SMOOTH_TRANS_OFF);
	 *
	 * prev_brightness -> brightness variable
	 * normal -> hbm : off
	 * hbm -> normal : on
	 * normal -> normal : off
	 * hbm -> hbm : on
...
__visible_for_testing int getidx_lpm_mode_table(struct maptbl *tbl)
{
	int row = 0;
	struct panel_device *panel;
	struct panel_bl_device *panel_bl;
...
__visible_for_testing int s6e3had_getidx_vddm_table(struct maptbl *tbl)
{
	struct panel_device *panel;
	struct panel_properties *props;
...
__visible_for_testing void copy_vaint_maptbl(struct maptbl *tbl, u8 *dst)
{
	struct panel_device *panel;
	struct panel_info *panel_data;
	struct panel_bl_device *panel_bl;
...
__visible_for_testing void copy_copr_maptbl(struct maptbl *tbl, u8 *dst)
{
	...
	dst[offset++] = ((reg->copr_mask & 0x1) << 4) |
		((reg->copr_pwr & 0x1) << 1) |
		(reg->copr_en & 0x1);
	dst[offset++] = (reg->roi_on & 0x1F);
	dst[offset++] = (reg->copr_gamma & 0x1F);
	dst[offset++] = (reg->copr_frame_count >> 8) & 0x3;
...
__visible_for_testing int init_color_blind_table(struct maptbl *tbl)
{
	...
	if (S6E3HAD_SCR_CR_OFS + mdnie->props.sz_scr > maptbl_get_sizeof_maptbl(tbl)) {
...
__visible_for_testing int init_mdnie_night_mode_table(struct maptbl *tbl)
{
	...
	if (maptbl_get_sizeof_maptbl(tbl) < (S6E3HAD_NIGHT_MODE_OFS +
...
__visible_for_testing int init_mdnie_color_lens_table(struct maptbl *tbl)
{
	...
	if (maptbl_get_sizeof_maptbl(tbl) < (S6E3HAD_COLOR_LENS_OFS +
...
__visible_for_testing int getidx_mafpc_vbp_maptbl(struct maptbl *tbl)
{
	...
	return maptbl_index(tbl, 0, is_id_gte_e2(panel), 0);
}
...
__visible_for_testing bool is_vrr_96hs_mode(struct panel_device *panel)
{
	...
	panel_data = &panel->panel_data;
	if (unlikely(!(panel_data->maptbl) ||
				!(panel_data->nr_maptbl > GAMMA_WRITE_0_MAPTBL) ||
