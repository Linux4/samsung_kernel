/*
 * linux/drivers/video/fbdev/exynos/panel/sw83109c/sw83109c.c
 *
 * SW83109C Dimming Driver
 *
...
int getidx_sw83109c_vrr_fps(int vrr_fps)
{
	int fps_index = SW83109C_VRR_FPS_60;

	switch (vrr_fps) {
	case 120:
		fps_index = SW83109C_VRR_FPS_120;
		break;
	case 90:
		fps_index = SW83109C_VRR_FPS_90;
		break;
	case 60:
		fps_index = SW83109C_VRR_FPS_60;
		break;
	default:
		fps_index = SW83109C_VRR_FPS_60;
...
int getidx_sw83109c_current_vrr_fps(struct panel_device *panel)
{
	int vrr_fps;
...
int getidx_vrr_fps_table(struct maptbl *tbl)
{
	...
	index = getidx_sw83109c_current_vrr_fps(panel);
	if (index < 0)
		row = 0;
	else
		row = index;
...
int sw83109c_getidx_ffc_table(struct maptbl *tbl)
{
	...
	switch (dsi_clk) {
	case 1443000:
		idx = SW83109C_HS_CLK_1443;
		break;
	case 1462000:
		idx = SW83109C_HS_CLK_1462;
		break;
	case 1471000:
		idx = SW83109C_HS_CLK_1471;
		break;
	default:
...
int sw83109c_getidx_acl_opr_table(struct maptbl *tbl)
{
	...
	if (row == 0)
		row = SW83109C_ACL_OPR_0;
	else
		row = SW83109C_ACL_OPR_2;
	if (is_hbm_brightness(panel_bl, panel_bl->props.brightness))
		row = SW83109C_ACL_OPR_1;

	if (row >= MAX_SW83109C_ACL_OPR) {
