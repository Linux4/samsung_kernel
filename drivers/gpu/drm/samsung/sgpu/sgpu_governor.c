...
static int sgpu_dvfs_governor_amigo_get_target(struct devfreq *df, uint32_t *level)
{
	...
	profiler_stc_set_cur_freq(MIGOV_GPU, df->profile->freq_table[*level]);
...
ssize_t sgpu_governor_current_info_show(struct devfreq *df, char *buf)
{
	...
	return scnprintf(buf, DEVFREQ_NAME_LEN, "%s", data->governor->name);
}
...
#define DEFAULT_HIGHSPEED_FREQ			545000
...
#if IS_ENABLED(CONFIG_CAL_IF) && IS_ENABLED(CONFIG_DEBUG_SNAPSHOT)
#if IS_ENABLED(CONFIG_EXYNOS_ESCA_DVFS_MANAGER)
...
#endif /*CONFIG_CAL_IF &&  CONFIG_DEBUG_SNAPSHOT*/
