...
#include "gc/gc_10_4_0_offset_m1.h"
#include "gc/gc_10_4_0_sh_mask_m1.h"
...
#ifdef CONFIG_DRM_SGPU_EXYNOS
	uint32_t ret, addr = (uint32_t)adev->rmmio_base + GL2ACEM_RST_OFFSET;

	exynos_smc_readsfr(addr, &val);
	val |= (0x1 << GL2ACEM_RST__SRT_ST__SHIFT);
	exynos_smc(SMC_CMD_REG, SMC_REG_ID_SFR_W(addr), val, 0);
#else /* CONFIG_DRM_SGPU_EXYNOS */
	val = RREG32_SOC15(GC, 0, mmGL2ACEM_RST);
	val = REG_SET_FIELD(val, GL2ACEM_RST, SRT_ST, 0x1);
...
#ifdef CONFIG_DRM_SGPU_EXYNOS
			ret = exynos_smc_readsfr(addr, &val);
			if (ret)
				pr_err("reading ADDR 0x%x is fail\n", addr);
#else
			val = RREG32_SOC15(GC, 0, mmGL2ACEM_RST);
#endif /* CONFIG_DRM_SGPU_EXYNOS */
