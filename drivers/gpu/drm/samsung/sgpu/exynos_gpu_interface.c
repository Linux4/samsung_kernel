...
int exynos_interface_init(struct devfreq *df)
{
	...
	migov_pid_list_readout = 0;
	migov_pid_list_top = 0;
	memset(&migov_pid_list[0], 0, sizeof(u16) * NUM_OF_PID);
...
void exynos_stats_set_vsync(ktime_t time_us)
{
	...
	if (atomic_read(&rtp_vsync_swapcall_counter) > 0)
		rtp_vsync_frame_counter++;
	atomic_set(&rtp_vsync_swapcall_counter, 0);
	rtp_vsync_counter++;
	atomic_inc(&migov_vsync_counter);
}
EXPORT_SYMBOL(exynos_stats_set_vsync);

void exynos_stats_get_frame_info(s32 *nrframe, u64 *nrvsync, u64 *delta_ms)
{
	*nrframe = (s32)atomic_read(&migov_frame_counter);
	atomic_set(&migov_frame_counter, 0);
	*nrvsync = (u64)atomic_read(&migov_vsync_counter);
	atomic_set(&migov_vsync_counter, 0);
	*delta_ms = (u64)(rtp_vsync_prev - (ktime_t)atomic64_read(&migov_last_updated_vsync_time)) / 1000;
	atomic64_set(&migov_last_updated_vsync_time, (u64)rtp_vsync_prev);
}
...
void exynos_stats_get_run_times(u64 *times)
{
	...
	if (rtp_readout != 0) {
		memset(&last_frame, 0, sizeof(struct amigo_interframe_data));
		cur_nrq = rtp_nrq;

		if (rtp_prev_minlock_cpu_freq > 0 || rtp_prev_minlock_gpu_freq > 0) {
			if (freq_qos_request_active(&cl1_min_pm_qos))
				freq_qos_update_request(&cl1_min_pm_qos, 0);
			if (freq_qos_request_active(&cl2_min_pm_qos))
				freq_qos_update_request(&cl2_min_pm_qos, 0);
...
void exynos_stats_get_pid_list(u16 *list)
{
	int i;
	int prev_top;

	if (migov_pid_list_readout != 0)
		memset(list, 0, sizeof(u16) * NUM_OF_PID);
	else {
	/* Transaction to transfer PID list to migov */
		do {
			prev_top = migov_pid_list_top;
			for (i = 0; i < NUM_OF_PID; i++)
				list[i] = migov_pid_list[i];
		} while (prev_top != migov_pid_list_top);
	}
	migov_pid_list_readout = 1;
}
...
unsigned long exynos_amigo_shortterm(struct amigo_interframe_data *dst)
{
	...
	if (exynos_amigo_is_joint_gov(df) && rtp_target_frametime > 0) {
		...
		if (rtp_prev_minlock_cpu_freq >= 0 && cpufreq_pm_qos_added > 0) {
...
int exynos_amigo_interframe_sw_update(ktime_t start, ktime_t end, ktime_t total)
{
	...
	atomic_inc(&rtp_vsync_swapcall_counter);
	atomic_inc(&migov_frame_counter);
	...
	if (migov_pid_list_top < NUM_OF_PID) {
		u64 pid = (u64)current->tgid;
		u64 hash = ((pid >> 16) ^ pid) & 0xffff;

		if (hash == 0)
			hash = 1;
		migov_pid_list[migov_pid_list_top++] = (u16)hash;
	}
...
void exynos_sdp_set_cur_freq(int id, int freq)
{
	int i = rtp_cur_freqlv[id];

	for (i = rtp_cur_freqlv[id]; i > 0; i--) {
		if (i < rtp_powertable_size[id] && freq <= rtp_powertable[id][i].freq)
			break;
	}

	while (i < rtp_powertable_size[id]) {
		if (freq >= rtp_powertable[id][i].freq) {
			rtp_cur_freqlv[id] = i;
			return;
		}
		i++;
	...
	if (rtp_powertable_size[id] > 0)
		rtp_cur_freqlv[id] = rtp_powertable_size[id] - 1;
}
EXPORT_SYMBOL(exynos_sdp_set_cur_freq);
