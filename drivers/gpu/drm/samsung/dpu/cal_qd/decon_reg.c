...
static void decon_reg_set_sram_enable(u32 id)
{
	if (id == 0)
		...
		/* DP and SWB share SRAM7 because they are exclusively used */
		decon_write_mask(id, SRAM_EN_OF_PRI_0, ~0,
				SRAM5_EN_F | SRAM6_EN_F | SRAM7_EN_F);
	else if (id == 3) {
...
void decon_reg_set_pll_wakeup(u32 id, u32 en)
{
	...
	if (en && dsim) {
		 ...
		 * - delay time(based on OSC) : 1/76.8Mhz * 500cycles = 6.5usec
		 */
		pms = &dsim->config.dphy_pms;
		delay_val = pms->p * 65 / 10;
...
int decon_reg_chk_rsc_status_by_decon_id(u32 id)
{
	...
	cal_log_info(id, "[RSC CH][0x%016X]\n", rsc_ch);
