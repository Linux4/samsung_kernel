...
static void decon_reg_set_data_path(u32 id, struct decon_config *cfg)
{
	...
	switch (out_type) {
	case DECON_OUT_DSI0:
		/* s5e8535 has DECON0/OUT0 connected only to DSIMIF0 */
		val = OUTIF_DSI0;
...
{
	u32 mask = SHD_REG_UP_REQ_DQE;

	if (decon_read_mask(id, DATA_PATH_CON, ENHANCE_DQE_ON))
		decon_write_mask(id, SHD_REG_UP_REQ, ~0, mask);
}
