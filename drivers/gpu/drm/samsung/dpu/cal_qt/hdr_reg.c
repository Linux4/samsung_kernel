...
#define hdr_regs_desc(id)			(&regs_hdr[REGS_HDR_BASE][id])
...
bool hdr_reg_verify(u32 id, u32 hw_type, u32 start_offset, u32 length)
{
	...
	if (hw_type == HDR_HW_TYPE_1) {	// S.LSI
		...
		//case HDR_LSI_H_OETF_POSX(0):
			if (length != HDR_OETF_L_POSX_LUT_REG_CNT
                                && length != HDR_OETF_H_POSX_LUT_REG_CNT)
				goto error;
			break;
		...
		case HDR_LSI_L_GM_OFFS(0):
			if (length != HDR_GM_OFFS_REG_CNT)
				goto error;
			break;
		case HDR_LSI_H_OETF_POSY(0):
			if (length != HDR_OETF_H_POSY_LUT_REG_CNT)
				goto error;
			break;
		case HDR_LSI_H_EOTF_POSX(0):
			if (length != HDR_EOTF_POSX_LUT_REG_CNT)
				goto error;
			break;
		case HDR_LSI_H_EOTF_POSY(0):
			if (length != HDR_EOTF_POSY_LUT_REG_CNT)
				goto error;
			break;
		case HDR_LSI_H_GM_COEF(0):
			if (length != HDR_GM_COEF_REG_CNT)
				goto error;
			break;
		case HDR_LSI_H_GM_OFFS(0):
			if (length != HDR_GM_OFFS_REG_CNT)
				goto error;
			break;
		case HDR_LSI_H_TM_POSX(0):
			if (length != HDR_TM_POSX_LUT_REG_CNT)
				goto error;
			break;
		case HDR_LSI_H_TM_POSY(0):
			if (length != HDR_TM_POSY_LUT_REG_CNT)
				goto error;
			break;
		case HDR_LSI_H_TM_COEF:
			if (length != 1)
				goto error;
			break;
		case HDR_LSI_H_TM_RNGX:
			if (length != 1)
				goto error;
			break;
		case HDR_LSI_H_TM_RNGY:
			if (length != 1)
				goto error;
			break;
		case HDR_LSI_H_EOTF_NORMAL_COEF:
			if (length != 1)
				goto error;
			break;
		case HDR_LSI_H_EOTF_NORMAL_SHIFT:
			if (length != 1)
				goto error;
			break;
		default:
