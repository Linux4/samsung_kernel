...
{
	if ((id != REGS_DPP10_ID) && (id > REGS_DPP7_ID))
		return;
...
{
	dpp_write_mask(id, DPP_COM_HDR_CON, en, DPP_MULT_EN);
}
...
void dpp_reg_init(u32 id, const unsigned long attr)
{
	if (test_bit(DPP_ATTR_IDMA, &attr)) {
		...
		idma_reg_set_ic_max(id, 0x28);
		idma_reg_set_assigned_mo(id, 0x28);
...
{
        if (test_bit(DPP_ATTR_IDMA, &attr)) {
		cal_log_err(id, "dpp_irq_clear_mask: clear & mask");
...
{
	if (test_bit(DPP_ATTR_IDMA, &attr)) {
		cal_log_err(id, "dpp_irq_enable_unmask: en & unmask");
		idma_reg_clear_irq(id, IDMA_ALL_IRQ_CLEAR);
		idma_reg_set_irq_mask_all(id, 0);
		idma_reg_set_irq_enable(id);
        }
...

{
	...
	if (!test_bit(DPP_ATTR_AFBC, &attr) && (p->comp_type == COMP_TYPE_AFBC)) {
		cal_log_err(id, "AFBC is not supported\n");
		return -ENOTSUPP;
	}

	if (p->comp_type == COMP_TYPE_AFBC) {
		if (fmt->fmt == DRM_FORMAT_RGBA1010102 ||
				fmt->fmt == DRM_FORMAT_BGRA1010102) {
			cal_log_err(id, "AFBC + RGBA1010102, BGRA1010102 is not supported\n");
			return -EINVAL;
		}
	}
