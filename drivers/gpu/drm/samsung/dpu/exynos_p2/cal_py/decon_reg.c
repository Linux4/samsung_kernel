...
static void decon_reg_set_outfifo_size_ctl2(u32 id, u32 width, u32 height)
{
	...
	/* In Papaya, OF_SIZE_2 is available only in decon0 */
	if (id != 0)
		return;
...
static void decon_reg_set_data_path(u32 id, struct decon_config *cfg)
{
	...
	switch (out_type) {
	case DECON_OUT_DSI0:
		/* s5e8825 has DECON0/OUT0 connected only to DSIMIF0 */
		val = OUTIF_DSI0;
...
void decon_reg_set_cwb_enable(u32 id, u32 en)
{
	...
	// TBD: Since below settings are not valid for s5e8825,
	// do we need other changes ??
	if (en) {
#if 0 //Pamir-9925 changes
		decon_write_mask(id, DATA_PATH_CON_1,
				CWB_SRC_F(CWB_SRC_ENH_OUT) |
				CWB_OF_IDX_F(CWB_OF_IDX_OF1),
				CWB_SRC_MASK | CWB_OF_IDX_MASK);
#else // Olympus-2100
#if 0 // Not available in Papaya
		...
		bool in_slice[4], u32 ds_en[2], u32 sm_ch[2])
{
	...
	case 4:
		...
		/* Revisit to check if ENC1 needs to be updated */
	...
	struct decon_dsc *dsc_enc)
{
	...
	if (config->vendor_pps.scale_increment_interval)
		dsc_enc->scale_increment_interval = config->vendor_pps.scale_increment_interval;
		...
		struct decon_dsc *dsc_enc, u32 chk_en)
{
	...
	if (id == 1) {
		...
		cal_log_debug(id, "Decon%d (%d) &  DP not supported\n", id);
	} else {
