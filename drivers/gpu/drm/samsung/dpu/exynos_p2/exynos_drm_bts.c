...
module_param_unsafe(dpu_bts_log_level, int, 0600);
...
static void dpu_bts_printk(const char *function_name, const char *format, ...)
{
...
static u32 dpu_bts_get_vblank_time_ns(struct decon_device *decon)
{
	...
	if (v_blank_t_ns < (3 * line_t_ns)) {
		...
		DPU_DEBUG_BTS("\t-WARN: v_blank_t_ns is abnormal!(-> %d%)\n",
				v_blank_t_ns);
	}
		...
		u32 *module_cycle, u32 *basic_cycle)
{
	...
	DPU_DEBUG_BTS("BEFORE latency check\n");
	DPU_DEBUG_BTS("\tACLK: %d KHz\n", aclk_base);
	...
	DPU_DEBUG_BTS("AFTER latency check\n");
	DPU_DEBUG_BTS("\tACLK: %d KHz\n", dfs_aclk);
	DPU_DEBUG_BTS("\tMODULE: module_cycle(%d)\n", temp_cycle);
	DPU_DEBUG_BTS("\tBASIC: basic_cycle(%d)\n", base_cycle);
		...
		u32 module_cycle, u32 basic_cycle, u32 dst_y, u32 *dpu_lat_t)
{
	...
	if (max_lat_t_ns > dpu_lat_t_ns) {
		...
		DPU_DEBUG_BTS("\tWARN: latency calc result is over tx_allow_t_ns!\n");
	}
	tx_allow_t_ns = tx_allow_t_ns * decon->bts.rot_util / 100;
	DPU_DEBUG_BTS("\t-dpu_cycle(%d) aclk_x_1k_ns(%d)\n",
			dpu_cycle, aclk_x_1k_ns);
	DPU_DEBUG_BTS("\t-dpu_lat_t_ns(%d) tx_allow_t_ns(%d)\n",
...
retry_hi_freq:
	...
	DPU_DEBUG_BTS("\t-rot_init_bw(%d) rot_need_clk(%d)\n",
				...
				const struct dpu_bts_win_config *config)
{
	...
	DPU_DEBUG_BTS("\tDPP%d : bpp(%d) src w(%d) h(%d) rot(%d) comp(%d) fmt(%s)\n",
				...
				struct bts_decon_info *bts_info, u32 format, int idx)
{
	...
	if (dpp->rotation) {
		...
		rot_bw = (u64)src_w * rot_read_line * dpp->bpp / 8 * 1000000U /
				tx_allow_ns;
		DPU_DEBUG_BTS("\tDPP%d ch_bw(%d), rot_bw(%d)\n", idx, ch_bw, rot_bw);
		if (rot_bw > ch_bw)
			ch_bw = rot_bw;
	}
...
void dpu_bts_calc_bw(struct exynos_drm_crtc *exynos_crtc)
{
	...
	for (i = 0; i < decon->win_cnt; ++i) {
		...
		idx = config[i].dpp_ch;
		dpu_bts_convert_config_to_info(&bts_info.dpp[idx], &config[i]);
	...
	if (config->state == DPU_WIN_STATE_BUFFER) {
		dpu_bts_convert_config_to_info(&bts_info.dpp[wb_idx], config);
...
void dpu_bts_update_bw(struct exynos_drm_crtc *exynos_crtc, bool shadow_updated)
{
	...
	DPU_DEBUG_BTS("\t(%s shadow_update)", (shadow_updated ? "after" : "before"));
	...
	if (shadow_updated) {
		...
		if (decon->bts.max_disp_freq < decon->bts.prev_max_disp_freq) {
			exynos_pm_qos_update_request(&decon->bts.disp_qos,
					decon->bts.max_disp_freq);
			DPU_DEBUG_BTS("disp_qos_update_request(disp_qos=%d)\n",
						decon->bts.max_disp_freq);
		}
...
void dpu_bts_release_bw(struct exynos_drm_crtc *exynos_crtc)
{
	...
	if ((decon->config.out_type & DECON_OUT_DSI) ||
		...
		if (exynos_pm_qos_request_active(&decon->bts.disp_qos))
			...
			DPU_ERR_BTS("%s disp qos setting error\n", __func__);
...
void dpu_bts_init(struct exynos_drm_crtc *exynos_crtc)
{
	...
	if (!dpu_bts_is_normal_boot() && (decon->config.out_type & DECON_OUT_DSI))
		DPU_INFO_BTS("decon%d boot_mode:0x%02X. MIF minlock(%d) will work.\n",
...
void dpu_bts_print_info(const struct exynos_drm_crtc *exynos_crtc)
{
	...
	DPU_INFO_BTS("DECON%d: bw(prev:%u curr:%u), disp(prev:%u curr:%u), peak(%u)\n",
			decon->id, decon->bts.prev_total_bw,
			decon->bts.total_bw, decon->bts.prev_max_disp_freq,
			decon->bts.max_disp_freq, decon->bts.peak);

	DPU_INFO_BTS("MIF(%lu), INT(%lu), DISP(%lu)\n",
	...
	for (i = 0; i < MAX_DPP_CNT; ++i) {
		...
		DPU_INFO_BTS("DPP[%d] bpp(%d) src(%d %d) dst(%d %d %d %d)\
				rot(%d) comp(%d) fmt(%s)\n", i, info->dpp[i].bpp,
