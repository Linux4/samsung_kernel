...
{
	static void __iomem *pwr_regs = NULL;
	int ret = 1;
...
struct dp_supported_preset supported_videos[] = {
	...
	{V2560X1440P120,   V4L2_DV_BT_CVT_2560X1440P120_ADDED, 120, SYNC_POSITIVE, SYNC_POSITIVE, 0, "V2560X1440P120"},
	...
	{V5120X1440P60,    V4L2_DV_BT_CVT_5120X1440P60_ADDED, 60, SYNC_POSITIVE, SYNC_POSITIVE,   0, "V5120X1440P60"},
...
#if IS_ENABLED(CONFIG_PHY_SAMSUNG_USB_CAL)
...
void dp_reg_phy_mode_setting(struct dp_cal_res *cal_res)
{
	...
	cal_log_info(0, "CONFIG_USB_TYPEC_MANAGER_NOTIFIER\n");
	switch (cal_res->ccic_notify_dp_conf) {
	case CCIC_NOTIFY_DP_PIN_UNKNOWN:
		cal_log_debug(0, "CCIC_NOTIFY_DP_PIN_UNKNOWN\n");
		break;

	case CCIC_NOTIFY_DP_PIN_A:
		cal_log_info(0, "CCIC_NOTIFY_DP_PIN_A\n");
		break;
	case CCIC_NOTIFY_DP_PIN_B:
		cal_log_info(0, "CCIC_NOTIFY_DP_PIN_B\n");
		break;
	case CCIC_NOTIFY_DP_PIN_C:
		cal_log_info(0, "CCIC_NOTIFY_DP_PIN_C\n");
	case CCIC_NOTIFY_DP_PIN_E:
		cal_log_info(0, "CCIC_NOTIFY_DP_PIN_E\n");
#if IS_ENABLED(CONFIG_PHY_SAMSUNG_USB_CAL)
		xhci_exynos_inform_dp_use(1, 4);
#endif
		tcpc_mux_con = 0x2;
	...
	case CCIC_NOTIFY_DP_PIN_D:
		cal_log_info(0, "CCIC_NOTIFY_DP_PIN_D\n");
	case CCIC_NOTIFY_DP_PIN_F:
		cal_log_info(0, "CCIC_NOTIFY_DP_PIN_F\n");
#if IS_ENABLED(CONFIG_PHY_SAMSUNG_USB_CAL)
		xhci_exynos_inform_dp_use(1, 2);
#endif
		tcpc_mux_con = 0x3;
		if (cal_res->dp_sw_sel) {
			tcpc_orientation = 0;
	...
	cal_log_info(0, "direction_%s\n", tcpc_orientation ? "flipped":"origin");
	cal_log_info(0, "Start USB to DP2/4\n");
...
void dp_reg_set_phy_tune(struct dp_cal_res *cal_res, u32 phy_lane_num, u32 amp_lvl, u32 pre_emp_lvl)
{
	...
	cal_log_info(0, "phy_lane_num(%d), amp_lvl(%d), pre_emp_lvl(%d)\n", phy_lane_num, amp_lvl, pre_emp_lvl);
	for (i = EQ_MAIN; i <= RBOOST; i++) {
		val = mask = 0;
		if (link_rate <=  LINK_RATE_2_7Gbps)
			val = phy_eq_hbr0_1[amp_lvl][pre_emp_lvl][i];
		else
			val = phy_eq_hbr2_3[amp_lvl][pre_emp_lvl][i];
		if (val == -1)
			continue;
...
#ifdef DP_DUMP
	{
		cal_log_info(0, "DP link rate(%d),
				DP PHY EQ SFR DP_CONFIG8/9/10/17 DUMP ===\n", link_rate);
...
u64 dp_reg_get_video_clk(void)
{
	u64 pixelclock;
	struct dp_device *dp = get_dp_drvdata();

#ifdef FEATURE_USE_DRM_EDID_PARSER
	pixelclock = dp->cur_mode.clock * 1000;
#else
	pixelclock = supported_videos[dp->cur_video].dv_timings.bt.pixelclock;
#endif
...
void dp_reg_set_active_symbol(struct dp_cal_res *cal_res)
{
	...
	cal_log_debug(0, "bpp:%u, clk: %u, bandwidth: %u, lanecount: %u\n",
			...
			for (i = 0; i < length; i += AUX_DATA_BUF_COUNT) {
				...
				if (ret == 0) {
					dp_reg_aux_ch_received_buf(
						data+((i/AUX_DATA_BUF_COUNT)*AUX_DATA_BUF_COUNT),
						buf_length);
...
int dp_reg_edid_read(struct dp_cal_res *cal_res, u8 edid_addr_offset, u32 length, u8 *data)
{
	...
	while(retry_cnt > 0) {
		...
		ret = dp_reg_set_aux_ch_operation_enable();
		dp_reg_set_aux_ch_address_only_command(0);
		cal_log_debug(0, "1st address only request in EDID read\n");
...
#if IS_ENABLED(CONFIG_PHY_SAMSUNG_USB_CAL)
	xhci_exynos_inform_dp_use(0, dp_reg_get_lane_count());
...
u16 dp_reg_cr_read(u16 addr, int *err)
{
	...
	if (clk_cycle == 10) {
		cal_log_info(0, "CR read failed to (0x%04x)\n", addr);
		*err = 1;
		return -1;
	} else {
		dp_reg_set_cr_clk_low();
