...
struct dp_device {
	...
	struct exynos_drm_connector connector;
...
#else
static inline struct dp_device *get_dp_drvdata(void) { return NULL; }
#endif

static inline bool IS_DP_ON_STATE(void)
{
	struct dp_device *dp = get_dp_drvdata();

	return dp && dp->state == DP_STATE_ON;
}
...
{
	return (dp_get_hpd_state(dp) == EXYNOS_HPD_UNPLUG) ? false : true;
}
