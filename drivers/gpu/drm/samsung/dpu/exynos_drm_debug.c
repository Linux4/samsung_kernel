...
size_t dpu_dsi_packet_to_string(void *src, void *buf, size_t remained)
{
	...
	if (mipi_dsi_packet_format_is_long(type))
		n += scnprintf(buf + n, remained - n, "PL size(%zu) PL addr(%#x)",
...
int dpu_init_debug(struct exynos_drm_crtc *exynos_crtc)
{
	...
	exynos_crtc->d.df_mif_idx = exynos_crtc->bts->df_mif_idx;
	exynos_crtc->d.df_int_idx = exynos_crtc->bts->df_int_idx;
	exynos_crtc->d.df_disp_idx = exynos_crtc->bts->df_disp_idx;
