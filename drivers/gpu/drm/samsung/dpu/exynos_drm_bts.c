...
static bool check_crtc_get_en_bts_fps(struct decon_device *decon)
{
	...
	return crtc_get_en_bts_fps(crtc);
}
...
static u64 dpu_bts_get_resol_clock(struct decon_device *decon)
{
	...
	if (decon->config.mode.op_mode == DECON_VIDEO_MODE &&
			!check_crtc_get_en_bts_fps(decon))
...
static void
dpu_bts_perframe_bw_control(struct decon_device *decon, enum bts_perframe_dir dir)
{
	static int int_qos_org = 0;
	static bool prev_lock = false;
	bool require_lock = false;
	bool update = false;
	int int_qos_target;

	if (is_version(&decon->config.version, 6, 30)
			&& (decon->config.mode.op_mode == DECON_VIDEO_MODE)
			&& (decon->config.out_type & DECON_OUT_DSI0)) {
		int_qos_target = 666 * 1000;

		if ((decon->bts.total_layer_cnt >= 6) && decon->bts.has_yuv
				&& (decon->bts.resol_clk >= 190382))
			require_lock = true;
		update = ((prev_lock != require_lock) ? true : false);

		if (update && require_lock && (dir == DPU_BTS_PERFRAME_ACQUIRE)) {
			int_qos_org = (exynos_pm_qos_read_req_value(PM_QOS_BUS_THROUGHPUT,
					&decon->bts.int_qos) > 0 ? : 0);
			exynos_pm_qos_update_request(&decon->bts.int_qos, int_qos_target);
			prev_lock = require_lock;
		} else if (update && !require_lock && (dir == DPU_BTS_PERFRAME_RELEASE)) {
			if (int_qos_org != int_qos_target)
				exynos_pm_qos_update_request(&decon->bts.int_qos, int_qos_org);
			int_qos_org = 0;
			prev_lock = require_lock;
		}
	}
...
void dpu_bts_set_bus_qos(const struct exynos_drm_crtc *exynos_crtc)
{
	...
	if (is_version(&decon->config.version, 7, 0)) {
		...
		if ((decon->config.out_type & DECON_OUT_DSI) &&
		    ...
		    (decon->bts.fps >= 63)) {
			decon->bts.int_default = 400000;
			decon->bts.mif_default = 546000;
		}
	}
...
static bool dpu_bts_check_max_perf(struct decon_device *decon)
{
	if (is_version(&decon->config.version, 6, 20)) {
		if ((decon->config.mode.op_mode == DECON_COMMAND_MODE) &&
		    (decon->config.out_type & DECON_OUT_DSI0) &&
		    (decon->bts.total_layer_cnt >= 6) &&
		    (decon->bts.resol_clk >= 361428)) {
...
static void dpu_bts_find_max_disp_freq(struct decon_device *decon)
{
	...
	decon->bts.total_layer_cnt = 0;
	decon->bts.has_yuv = false;

	for (i = 0; i < decon->win_cnt; ++i) {
		if (!is_version_below(&decon->config.version, 6, 30))
			continue;
		...
		fmt_info = dpu_find_fmt_info(config[i].format);
		decon->bts.has_yuv |= IS_YUV(fmt_info);
		decon->bts.total_layer_cnt++;
	}
...
void dpu_bts_calc_bw(struct exynos_drm_crtc *exynos_crtc)
{
	...
	dpu_bts_perframe_bw_control(decon, DPU_BTS_PERFRAME_RELEASE);
	...
	dpu_bts_perframe_bw_control(decon, DPU_BTS_PERFRAME_ACQUIRE);
...
void dpu_bts_update_bw(struct exynos_drm_crtc *exynos_crtc, bool shadow_updated)
{
	...
	if (shadow_updated) {
		...
		if (!is_max_perf && (decon->bts.max_disp_freq < decon->bts.prev_max_disp_freq)) {
		...
		is_max_perf = dpu_bts_check_max_perf(decon);
		if (!is_max_perf) {
