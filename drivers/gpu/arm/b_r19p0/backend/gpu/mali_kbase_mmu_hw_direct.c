...
static int as_n_bak = 0;
static u32 nr_bak = 0;
static u32 op_bak = 0;
static unsigned int irq_bak = 0;

static char mmu_op[6][10] = {
	"NOP",
	"UPDATE",
	"LOCK",
	"UNLOCK",
		...
		unsigned int as_nr)
{
	...
	if (max_loops == 0) {
		...
		dev_err(kbdev->dev, "step: %s\n", (step==1) ? "LOCK or UNLOCK" : (step==2) ? "Run MMU operation" : "Wait for flush");
		dev_err(kbdev->dev, "as_n: %d\n", as_n_bak);
		dev_err(kbdev->dev, "nr: %u\n", nr_bak);
		dev_err(kbdev->dev, "op: %s\n", mmu_op[op_bak]);
		dev_err(kbdev->dev, "irq: %u\n", irq_bak);
		return -1;
	}
