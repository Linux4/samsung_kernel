...
static int gpu_validate_attrib_data(struct exynos_context *platform)
{
	...
	platform->gpu_min_clock = data == 0 ? 350 : (u32) data;
	...
	platform->gpu_dvfs_start_clock = data == 0 ? 350 : (u32) data;
	data = gpu_get_attrib_data(attrib, GPU_DVFS_BL_CONFIG_CLOCK);
	platform->gpu_dvfs_config_clock = data == 0 ? 350 : (u32) data;
	...
	gpu_dvfs_update_start_clk(G3D_DVFS_GOVERNOR_DEFAULT, data == 0 ? 350 : (u32) data);
	...
	gpu_dvfs_update_start_clk(G3D_DVFS_GOVERNOR_STATIC, data == 0 ? 350 : (u32) data);
	...
	gpu_dvfs_update_start_clk(G3D_DVFS_GOVERNOR_BOOSTER, data == 0 ? 350 : (u32) data);
	...
	gpu_dvfs_update_start_clk(G3D_DVFS_GOVERNOR_INTERACTIVE, data == 0 ? 350 : (u32) data);
	...
	platform->interactive.highspeed_clock = data == 0 ? 553 : (u32) data;
	...
	platform->tmu_lock_clk[THROTTLING1] = data == 0 ? 553 : (u32) data;
	data = gpu_get_attrib_data(attrib, GPU_TEMP_THROTTLING2);
	platform->tmu_lock_clk[THROTTLING2] = data == 0 ? 553 : (u32) data;
	data = gpu_get_attrib_data(attrib, GPU_TEMP_THROTTLING3);
	platform->tmu_lock_clk[THROTTLING3] = data == 0 ? 553 : (u32) data;
	data = gpu_get_attrib_data(attrib, GPU_TEMP_THROTTLING4);
	platform->tmu_lock_clk[THROTTLING4] = data == 0 ? 553 : (u32) data;
	data = gpu_get_attrib_data(attrib, GPU_TEMP_THROTTLING5);
	platform->tmu_lock_clk[THROTTLING5] = data == 0 ? 553 : (u32) data;
	data = gpu_get_attrib_data(attrib, GPU_TEMP_TRIPPING);
	platform->tmu_lock_clk[TRIPPING] = data == 0 ? 553 : (u32) data;
