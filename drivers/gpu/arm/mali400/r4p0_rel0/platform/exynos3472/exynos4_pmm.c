/* drivers/gpu/mali400/mali/platform/exynos3472/exynos4_pmm.c
 *
...
#define SCLEPLLCLK_NAME  "sclk_epll_muxed"
...
mali_bool mali_clk_get(void)
{
	...
	if (sclk_epll_clock == NULL) {
		sclk_epll_clock = clk_get(NULL, SCLEPLLCLK_NAME);
	...
	if (bts_g3d_clock == NULL) {
		bts_g3d_clock = clk_get(NULL, "qe.g3d");
		if (IS_ERR(bts_g3d_clock)) {
			MALI_PRINT(("G3D BTS Error : failed to get source mali clock\n"));
			return MALI_FALSE;
		}
	}
...
void mali_clk_set_rate(unsigned int clk, unsigned int mhz)
{
	...
	if (atomic_read(&clk_active) == 0) {
		...
		err = clk_enable(bts_g3d_clock);
		MALI_DEBUG_PRINT(3, ("mali_clk_set_rate g3d_bts_clock %p error %d\n", bts_g3d_clock, err));
...
static _mali_osk_errcode_t enable_mali_clocks(void)
{
	...
	if (atomic_read(&clk_active) == 0) {
		...
		err = clk_enable(bts_g3d_clock);
		MALI_DEBUG_PRINT(3, ("enable_mali_clocks g3d_bts_clock %p error %d\n", bts_g3d_clock, err));
		atomic_set(&clk_active, 1);
	}
...
static _mali_osk_errcode_t disable_mali_clocks(void)
{
	...
	if (atomic_read(&clk_active)) {
		...
		clk_disable(bts_g3d_clock);
		MALI_DEBUG_PRINT(3, ("disable_g3d_bts_clocks mali_clock %p\n", bts_g3d_clock));
