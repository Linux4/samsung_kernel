...
#else
#define MALI_COHERENCY_ADDR		0x0
#define MALI_ARUSER_HINT_ADDR	0x0
#define MALI_AWUSER_HINT_ADDR	0x0
#define MALI_USER_REG4_ADDR		0x0
#endif
	if (MALI_COHERENCY_ADDR) {
		kbdev->g3d_coherency_features = ioremap(MALI_COHERENCY_ADDR, 4);

		if (!kbdev->g3d_coherency_features) {
			dev_err(kbdev->g3d_coherency_features, "Can't remap coherency register\n");
			err = -EINVAL;
		} else {
			GPU_LOG(DVFS_DEBUG, DUMMY, 0u, 0u, "ioremap succeeded to access coherency register\n");
		}
	} else {
		kbdev->g3d_coherency_features = 0;
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u,
				"Coherency register addr is 0x0. Coherency will be disabled\n");
	}
	if (MALI_ARUSER_HINT_ADDR) {
		kbdev->aruser_hint = ioremap(MALI_ARUSER_HINT_ADDR, 4);

		if (!kbdev->aruser_hint) {
			dev_err(kbdev->aruser_hint, "Can't remap aruser hint register\n");
			err = -EINVAL;
		} else {
			GPU_LOG(DVFS_DEBUG, DUMMY, 0u, 0u, "ioremap succeeded to access aruser hint register\n");
		}
	} else {
		kbdev->aruser_hint = 0;
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u,
				"Aruser hint register addr is 0x0. Aruser hint will be disabled\n");
	}
	if (MALI_AWUSER_HINT_ADDR) {
		kbdev->awuser_hint = ioremap(MALI_AWUSER_HINT_ADDR, 4);

		if (!kbdev->awuser_hint) {
			dev_err(kbdev->awuser_hint, "Can't remap awuser hint register\n");
			err = -EINVAL;
		} else {
			GPU_LOG(DVFS_DEBUG, DUMMY, 0u, 0u, "ioremap succeeded to access awuser hint register\n");
		}
	} else {
		kbdev->awuser_hint = 0;
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u,
				"Awuser hint register addr is 0x0. Awuser hint will be disabled\n");
	}
	if (MALI_USER_REG4_ADDR) {
		kbdev->user_reg4 = ioremap(MALI_USER_REG4_ADDR, 4);

		if (!kbdev->user_reg4) {
			dev_err(kbdev->user_reg4, "Can't remap user_reg4 register\n");
			err = -EINVAL;
		} else {
			GPU_LOG(DVFS_DEBUG, DUMMY, 0u, 0u, "ioremap succeeded to access user_reg4 register\n");
		}
	} else {
		kbdev->user_reg4 = 0;
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u,
				"user_reg4 register addr is 0x0. user_reg4 will be disabled\n");
	}
...
#if IS_ENABLED(CONFIG_MALI_COHERENCY)
	if (kbdev->g3d_coherency_features) {
		iounmap(kbdev->g3d_coherency_features);
		GPU_LOG(DVFS_DEBUG, DUMMY, 0u, 0u, "coherency register iounmap succeeded\n");
	}
	if (kbdev->aruser_hint) {
		iounmap(kbdev->aruser_hint);
		GPU_LOG(DVFS_DEBUG, DUMMY, 0u, 0u, "aruser hint register iounmap succeeded\n");
	}
	if (kbdev->awuser_hint) {
		iounmap(kbdev->awuser_hint);
		GPU_LOG(DVFS_DEBUG, DUMMY, 0u, 0u, "awuser hint register iounmap succeeded\n");
	}
	if (kbdev->user_reg4) {
		iounmap(kbdev->user_reg4);
		GPU_LOG(DVFS_DEBUG, DUMMY, 0u, 0u, "user_reg4 register iounmap succeeded\n");
	}
#endif
...
static int kbase_device_suspend(struct device *dev)
{
	...
	/* MALI_SEC_INTEGRATION */
	KBASE_KTRACE_ADD_EXYNOS(kbdev, KBASE_DEVICE_SUSPEND, NULL, NULL, \
...
static int kbase_device_resume(struct device *dev)
{
	...
	/* MALI_SEC_INTEGRATION */
	KBASE_KTRACE_ADD_EXYNOS(kbdev, KBASE_DEVICE_RESUME, NULL, NULL, \
			platform->power_runtime_suspend_ret, platform->power_runtime_resume_ret);
