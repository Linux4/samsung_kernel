...
int gpu_pm_qos_command(struct exynos_context *platform, gpu_pmqos_state state)
{
	...
	case GPU_CONTROL_PM_QOS_INIT:
		...
		pm_qos_add_request(&exynos5_g3d_cpu_cluster_max_qos, PM_QOS_CPU_FREQ_MAX, PM_QOS_CPU_FREQ_MAX_DEFAULT_VALUE);
		pm_qos_add_request(&exynos5_g3d_cpu_cluster_min_qos, PM_QOS_CPU_FREQ_MIN, 0);
		break;
	case GPU_CONTROL_PM_QOS_DEINIT:
		...
		pm_qos_remove_request(&exynos5_g3d_cpu_cluster_max_qos);
		pm_qos_remove_request(&exynos5_g3d_cpu_cluster_min_qos);
		break;
	case GPU_CONTROL_PM_QOS_SET:
		...
		pm_qos_update_request(&exynos5_g3d_cpu_cluster_min_qos, platform->table[platform->step].cpu_freq);
		if (!platform->boost_is_enabled)
			pm_qos_update_request(&exynos5_g3d_cpu_cluster_max_qos, platform->table[platform->step].cpu_max_freq);
	...
	case GPU_CONTROL_PM_QOS_RESET:
		...
		pm_qos_update_request(&exynos5_g3d_cpu_cluster_min_qos, 0);
		pm_qos_update_request(&exynos5_g3d_cpu_cluster_max_qos, PM_QOS_CPU_FREQ_MAX_DEFAULT_VALUE);
		break;
	case GPU_CONTROL_PM_QOS_EGL_SET:
		pm_qos_update_request(&exynos5_g3d_cpu_cluster_min_qos, platform->boost_egl_min_lock);
		break;
