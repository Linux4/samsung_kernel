...
/* MALI_SEC_INTEGRATION */
static void crawl_page_table_level(struct kbase_context *kctx, mali_addr64 vaddr, phys_addr_t pgd, int level)
{
	u64 *pgd_page;
	int i;
	...
	if ((pgd_page[index] & 3) == ENTRY_IS_PTE) {
		...
		crawl_page_table_level(kctx, vaddr, target_pgd, level + 1);
...
void crawl_page_table(struct kbase_context *kctx, mali_addr64 vaddr)
{
	dev_err(kctx->kbdev->dev, "Looking up virtual GPU address: 0x%016llX\n", vaddr);
	crawl_page_table_level(kctx, vaddr, kctx->pgd, 0);
}
...
/* MALI_SEC_INTEGRATION */
	crawl_page_table(kctx, as->fault_addr);
