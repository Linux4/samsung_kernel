...
int gpu_set_target_clk_vol(int clk)
{
	...
	if (!platform->clk_g3d_status) {
		...
		GPU_LOG(DVFS_INFO, DUMMY, 0u, 0u, "%s: can't set clock and voltage in the clock-off state!\n", __func__);
		return -1;
	}
	...
	target_vol = MAX(gpu_dvfs_get_voltage(target_clk) + platform->voltage_margin,
									platform->voltage_margin ? platform->cold_min_vol : 0);
