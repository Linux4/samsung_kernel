...
void sprd_iommu_gsp_clk_enable(struct sprd_iommu_dev *dev)
{
	pr_debug("%s\n", __func__);
...
void sprd_iommu_gsp_clk_disable(struct sprd_iommu_dev *dev)
{
	pr_debug("%s\n", __func__);
#ifndef CONFIG_SC_FPGA
...
int sprd_iommu_gsp_init(struct sprd_iommu_dev *dev,
			struct sprd_iommu_init_data *data)
{
	...
	sprd_iommu_gsp_clk_enable(dev);
	err = sprd_iommu_init(dev, data);
	sprd_iommu_gsp_clk_disable(dev);
...
unsigned long sprd_iommu_gsp_iova_alloc(struct sprd_iommu_dev *dev,
					size_t iova_length)
{
	return sprd_iommu_iova_alloc(dev, iova_length);
}

void sprd_iommu_gsp_iova_free(struct sprd_iommu_dev *dev, unsigned long iova,
				size_t iova_length)
{
	sprd_iommu_iova_free(dev, iova, iova_length);
}

int sprd_iommu_gsp_iova_map(struct sprd_iommu_dev *dev, unsigned long iova,
...
int sprd_iommu_gsp_iova_unmap(struct sprd_iommu_dev *dev, unsigned long iova,
				size_t iova_length)
{
	int err = -1;
...
int sprd_iommu_gsp_dump(struct sprd_iommu_dev *dev, void *data)
{
	return 0;
}

void sprd_iommu_gsp_pgt_show(struct sprd_iommu_dev *dev)
{
...
struct sprd_iommu_ops iommu_gsp_ops = {
	...
	.enable = sprd_iommu_gsp_clk_enable,
	.disable = sprd_iommu_gsp_clk_disable,
	.dump = sprd_iommu_gsp_dump,
	.open = sprd_iommu_gsp_open,
	.release = sprd_iommu_gsp_release,
	.pgt_show = sprd_iommu_gsp_pgt_show,
};
