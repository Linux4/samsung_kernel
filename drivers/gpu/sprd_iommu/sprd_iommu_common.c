...
static inline void sprd_iommu_update_pgt(struct sprd_iommu_dev *dev, unsigned long iova, uint32_t paddr, size_t size)
{
	unsigned long pgt_base = dev->init_data->pgt_base;
	...
	for(;iova<end_iova;iova+=SPRD_IOMMU_PAGE_SIZE,paddr+=SPRD_IOMMU_PAGE_SIZE)
	{
		...
		*((unsigned int *)(pgt_base)+SPRD_IOMMU_PTE_ENTRY(iova)) = paddr;
...
static inline void sprd_iommu_clear_pgt(struct sprd_iommu_dev *dev, unsigned long iova, size_t size)
{
	unsigned long pgt_base = dev->init_data->pgt_base;
	unsigned long end_iova = iova + size;
	unsigned int *pa_fault_page = (__pa(dev->fault_page) | 0xFFF);
	...
	for(; iova < end_iova; iova += SPRD_IOMMU_PAGE_SIZE) {
		*((unsigned int *)(pgt_base) + SPRD_IOMMU_PTE_ENTRY(iova)) = pa_fault_page;
	}
}
...
int sprd_iommu_init(struct sprd_iommu_dev *dev, struct sprd_iommu_init_data *data)
{
	...
	pr_info("sprd_iommu %s iova_base:0x%lx, iova_size:0x%x, pgt_base:0x%lx, pgt_size:0x%x",
	...
	//1. get one free page(fault_page) and use its physical address as fault pte instead of 0xFFFF_FFFF
	dev->fault_page = __get_free_page(GFP_KERNEL);

	if(!dev->fault_page)
	{
		free_pages((unsigned long)dev->pgt, get_order(data->pgt_size));
		pr_err("sprd_iommu %s get_free_page fail\n",dev->init_data->name);
		return -1;
	}

	//2. fill fault_page with a dead pattern 0xDEADDEAD,
	for(i = 0; i < PAGE_SIZE/sizeof(unsigned long); i++)
		((unsigned long *)dev->fault_page)[i] = 0xDEADDEAD;

	pr_info("sprd_iommu %s fault_pa = 0x%x\n", dev->init_data->name, dev->fault_page);

	//3. fill pgt with the address of fault_page instead of 0xFFFF_FFFF
	{
		int i = 0;
		unsigned long *pa_fault_page = (__pa(dev->fault_page) | 0xFFF);
		for(i = 0; i < PAGE_ALIGN(data->pgt_size)/sizeof(unsigned long); i++)
			((unsigned long *)data->pgt_base)[i] = pa_fault_page;
	}

	dev->pool = gen_pool_create(12, -1);
	if (!dev->pool) {
		free_pages((unsigned long)dev->pgt, get_order(data->pgt_size));
		free_page((unsigned long)dev->fault_page);
...
int sprd_iommu_iova_map(struct sprd_iommu_dev *dev, unsigned long iova, size_t iova_length, struct ion_buffer *handle)
{
	...
	for_each_sg(table->sgl, sg, table->nents, i) {
		sprd_iommu_update_pgt(dev, iova_cur,get_phys_addr(sg),PAGE_ALIGN(sg_dma_len(sg)));
...
int sprd_iommu_iova_unmap(struct sprd_iommu_dev *dev, unsigned long iova, size_t iova_length, struct ion_buffer *handle)
{
	...
	mmu_reg_write(dev->init_data->ctrl_reg,MMU_TLB_EN(0),MMU_TLB_EN_MASK);
#endif
	sprd_iommu_clear_pgt(dev, iova, iova_length);
	//memcpy((unsigned long*)dev->pgt,(unsigned long*)dev->init_data->pgt_base,PAGE_ALIGN(dev->init_data->pgt_size));
#ifdef GSP_IOMMU_WORKAROUND1
	if(dev->ops == &iommu_mm_ops){
		mmu_reg_write(dev->init_data->ctrl_reg,MMU_TLB_EN(1),MMU_TLB_EN_MASK);
