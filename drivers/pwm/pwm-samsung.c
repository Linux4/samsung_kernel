...
static int s3c_pwm_enable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	if (!pwm_enable_cnt && s3c->clk_ctrl && !s3c->need_hw_init) {
		exynos_update_ip_idle_status(s3c->idle_ip_index, 0);
...
static void s3c_pwm_disable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	if (!pwm_enable_cnt && s3c->clk_ctrl && !s3c->need_hw_init) {
		...
		exynos_update_ip_idle_status(s3c->idle_ip_index, 1);
	}
				...
				unsigned long cmd, void *v)
{
	...
	case LPA_ENTER:
	case LPD_ENTER:
		if (!g_s3c->need_hw_init && !pwm_enable_cnt)
			s3c_pwm_save(g_s3c);
		break;
	case LPA_EXIT:
	case LPD_EXIT:
...
static int s3c_pwm_probe(struct platform_device *pdev)
{
	...
	s3c->idle_ip_index = exynos_get_idle_ip_index(dev_name(&pdev->dev));
	g_s3c = s3c;
