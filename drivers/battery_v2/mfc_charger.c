...
bool mfc_tx_function_check(struct mfc_charger_data *charger)
{
	...
	mfc_reg_read(charger->client, MFC_RX_DEVICE_INFO1_REG, &reg_f2);
	mfc_reg_read(charger->client, MFC_RX_DEVICE_INFO2_REG, &reg_f4);
...
static void mfc_tx_mode_control(struct mfc_charger_data *charger)
{
	u8 reg_data;
	union power_supply_propval value = {0, };

	mfc_reg_read(charger->client, MFC_STATUS_H_REG, &reg_data);

	if (reg_data & MFC_STAT_H_TX_CON_DISCON_MASK) {
		if (mfc_tx_function_check(charger)) {
			pr_info("%s : Rx(samsung device) connected\n", __func__);
			...
			pr_info("%s : Not Samsung Device\n", __func__);
			return;
		}
	} else {
		pr_info("%s : Rx disconnected\n", __func__);
		charger->wc_rx_connected = false;
		charger->wc_tx_vout = WC_TX_VOUT_5_0V;

		value.intval = charger->wc_tx_vout;
	...
	psy_do_property("wireless", set, POWER_SUPPLY_EXT_PROP_WIRELESS_RX_CONNECTED,
			value);
}
...
static void mfc_wpc_rx_det_work(struct work_struct *work)
{
	...
	mfc_reg_read(charger->client, MFC_RX_CMFET_CTRL_REG, &reg_data);

	if (reg_data >= 2) {
		pr_info("%s : Samsung Gear Connected\n", __func__);
		value.intval = charger->wc_tx_vout = WC_TX_VOUT_5_0V;
		charger->wc_rx_type = SS_GEAR;
		psy_do_property(charger->pdata->wired_charger_name, set,
				POWER_SUPPLY_EXT_PROP_WIRELESS_TX_VOUT, value);
	} else if (reg_data < 2) {
		pr_info("%s : Samsung Phone Connected\n", __func__);
		value.intval = charger->wc_tx_vout = WC_TX_VOUT_7_5V;
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		...
		pr_info("%s Firm Ver = %x RX_VOUT = %dmV, RX_VRECT = %dmV, RX_IOUT = %dmA, OP_FREQ = %dKHz, IC Rev = 0x%x, IC Font = 0x%x, cable=%d\n",
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_WIRELESS_TX_ENABLE:
			if (val->intval) {
				pr_info("%s: Turn on TX\n", __func__);
				charger->wc_tx_enable = true;
				mfc_uno_on(charger, true);
			} else {
				pr_info("%s: Turn off TX, and reset UNO config\n", __func__);
				charger->wc_tx_enable = false;
				charger->wc_rx_connected = false;
				mfc_uno_on(charger, false);

				value.intval = WC_TX_VOUT_OFF;
				psy_do_property(charger->pdata->wired_charger_name, set,
					POWER_SUPPLY_EXT_PROP_WIRELESS_TX_VOUT, value);
...
void mfc_mst_routine(struct mfc_charger_data *charger, u8 *irq_src)
{
	...
	if (charger->is_mst_on == MST_MODE_2) {
		...
		pr_info("%s: 0x21 Register AC Missing(%d)\n", __func__, data);
		if (data) {
			/* clear intterupt */
			mfc_reg_write(charger->client, MFC_INT_A_CLEAR_L_REG, irq_src[0]); // clear int
			mfc_reg_write(charger->client, MFC_INT_A_CLEAR_H_REG, irq_src[1]); // clear int
