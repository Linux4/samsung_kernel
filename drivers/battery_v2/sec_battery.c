...
/* set 1400mA due to peak current to pass the PD authentication */
	...
	if (battery->pdic_info.sink_status.power_list[pdo_num].max_current == 1500)
		battery->pdic_info.sink_status.power_list[pdo_num].max_current = 1400;
#endif
...
static int sec_bat_set_charging_current(struct sec_battery_info *battery)
{
	...
	int swelling_current = charging_current;
#if defined(CONFIG_MULTI_PORT_CHARGING)
	int sub_charging_current = battery->pdata->charging_current[SEC_BATTERY_CABLE_POGO_WCIN].fast_charging_current;
	/* charging_current is total current */
	int main_charging_current = charging_current;
	get_prev_sub_charging_current(battery, &charging_current, &main_charging_current, &sub_charging_current);
#endif
...
#if defined(CONFIG_MULTI_PORT_CHARGING)
	get_sub_charging_current(battery, &charging_current, &main_charging_current, &sub_charging_current, &input_current);
	charging_current = main_charging_current;
#endif
				...
				struct sec_battery_info *battery)
{
	...
	case SEC_BATTERY_THERMAL_SOURCE_ADC:
...
#if defined(CONFIG_USE_IFPMIC_NTC_PIN_FOR_BLKT)
			psy_do_property(battery->pdata->fuelgauge_name, get,
				POWER_SUPPLY_EXT_PROP_BLKT_TEMP, value);
#else
			sec_bat_get_value_by_adc(battery,
		...
		break;
	case SEC_BATTERY_THERMAL_FAKE:
		battery->temperature = 260;
		battery->usb_temp = 260;
		battery->chg_temp = 260;
		battery->wpc_temp = 260;
		battery->coil_temp = 260;
		battery->blkt_temp = 260;
		pr_info("%s : fixed temperature (bat: %d, usb: %d, wpc: %d, chg: %d, blkt: %d)\n",
			__func__, battery->temperature, battery->usb_temp, battery->wpc_temp,
			battery->chg_temp, battery->blkt_temp);
		break;
	default:
		break;
	}
				...
				struct sec_battery_info *battery)
{
	...
	//battery_last_dcvs(battery->capacity, battery->voltage_avg, battery->temperature, battery->current_avg);  // check build error
}
...
static void sec_bat_calculate_safety_time(struct sec_battery_info *battery)
{
	...
	if (battery->pogo_status)
		curr += battery->sub_charging_current;
#endif
				...
				struct work_struct *work)
{
	...
	if (!battery->wc_enable && battery->pdata->wpc_en) {
...
static void sec_bat_cable_work(struct work_struct *work)
{
	...
	mutex_lock(&battery->cable_worklock);
	...
	if (current_cable_type == battery->cable_type && !battery->no_skip_cable_work) {
		dev_dbg(battery->dev,
...
skip_update_battery_status:
	...
	queue_delayed_work(battery->monitor_wqueue, &battery->monitor_work, 0);
end_of_cable_work:
	mutex_unlock(&battery->cable_worklock);
...
#if defined(CONFIG_FUELGAUGE_SM5705) || defined(CONFIG_FUELGAUGE_SM5708) || defined(CONFIG_INBATVOLT_FG)
			psy_do_property(battery->pdata->fuelgauge_name, get,
				POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
			ret = value.intval;
#else
		...
		if (sscanf(buf, "%10d\n", &x) == 1) {
			if (battery->pdata->wpc_en) {
				if (x == 0) {
					...
					battery->wc_enable_cnt = 0;
					pr_info("%s: WC ENABLE: Disable", __func__);
				} else if (x == 1) {
					battery->wc_enable = true;
					battery->wc_enable_cnt = 0;
					pr_info("%s: WC ENABLE: Enable", __func__);
				} else {
					dev_info(battery->dev,
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SCOPE:
		pr_info("%s: water event block for USER %d \n", __func__, val->intval);
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		...
		pr_info("%s cable type = %d max charger power = %d\n", __func__, battery->cable_type, battery->max_charge_power);
		mutex_unlock(&battery->cable_worklock);		
		break;
	case POWER_SUPPLY_PROP_HEALTH:
				...
				muic_attached_dev_t attached_dev)
{
...
#if defined(CONFIG_S2MU005_CHARGER_BUCK_OFF)
		val.intval = SEC_BAT_CHG_MODE_BUCK_OFF;
		psy_do_property(battery->pdata->charger_name, set,
			POWER_SUPPLY_PROP_CHARGING_ENABLED, val);
#endif
	...
	case ATTACHED_DEV_JIG_UART_OFF_VB_FG_MUIC:
		if (!battery->pdata->water_proof && battery->pdata->detect_moisture) {
			...
			current_cable_type = factory_mode ? SEC_BATTERY_CABLE_NONE : SEC_BATTERY_CABLE_UARTOFF;
		}
		break;
		...
		case MUIC_NOTIFY_CMD_VBUS_OFF:
			{
				...
				if (is_nocharge_type(battery->cable_type)) {
					...
					value.intval = 100;
					psy_do_property("s2mu004-charger", set,
						POWER_SUPPLY_PROP_CURRENT_MAX, value);
				}
...
#if !defined(CONFIG_ENG_BATTERY_CONCEPT) && !defined(CONFIG_SEC_FACTORY)
	pr_info("%s : water_proof(%d), detect_moisture(%d), block_water_event(%d), muic_cable_type(%d)\n", 
			__func__, battery->pdata->water_proof, battery->pdata->detect_moisture, 
			battery->block_water_event, battery->muic_cable_type);
			
	if (!battery->pdata->water_proof && battery->pdata->detect_moisture) {
		/* Block water event UI popup is required only for 301k & 523k cable */ 
		block_water_event =
		...
		/* Block water event UI popup is required for 301k , 523k & 619k cable*/ 
		block_water_event =
			(!battery->pdata->water_proof) ||
			(battery->block_water_event) ||
		...
		struct sec_battery_info *battery)
{
...
#endif
#if defined(CONFIG_MUIC_S2MU004_SUPPORT_BC1P2_CERTI)
	/* the reason for 101 is that the value may not be updated in 'sec_bat_set_charging_current' */
	pdata->charging_current[SEC_BATTERY_CABLE_NONE].input_current_limit = 101;
#endif
	...
	if (ret) {
		...
		/* do not use vbat ovp algorithm */
		pdata->max_voltage_thr = 5000;
	}
	...
	pdata->fake_temperature = of_property_read_bool(np, "battery,fake_temperature");

	pdata->water_proof = of_property_read_bool(np, "battery,water_proof");
	pr_info("%s : water_proof = %d \n", __func__, pdata->water_proof);
	...
	if (pdata->fake_temperature)
		pdata->thermal_source = SEC_BATTERY_THERMAL_FAKE;
...
#ifndef CONFIG_USE_IFPMIC_NTC_PIN_FOR_BLKT
...
static int sec_battery_probe(struct platform_device *pdev)
{
	...
	mutex_init(&battery->cable_worklock);
...
#if !defined(CONFIG_SEC_FACTORY) && defined(CONFIG_CHARGING_VZWCONCEPT)
	dev_info(battery->dev, "%s: Sales is VZW\n", __func__);
...
err_irq:
	...
	mutex_destroy(&battery->cable_worklock);
	kfree(pdata);
err_bat_free:
	kfree(battery);
