...
{
	int i, j, min, temp_voltage, temp_current, temp_index, max_index;
	...
	for(i = 0, max_index=0; i < pd_list_index; i++) {
		...
		if( max_index != i)
			if(battery->pd_list.pd_info[max_index].input_voltage *  battery->pd_list.pd_info[max_index].input_current
				< battery->pd_list.pd_info[i].input_voltage *  battery->pd_list.pd_info[i].input_current)
				max_index = i;
	}
	battery->pd_list.max_pd_count = pd_list_index;
	battery->max_charge_power = battery->pdic_info.sink_status.power_list[ \
		battery->pd_list.pd_info[max_index].pdo_index].max_voltage * \
		battery->pdic_info.sink_status.power_list[battery->pd_list.pd_info[ \
		max_index].pdo_index].max_current / 1000;
	battery->pd_max_charge_power = battery->max_charge_power;
	battery->pdic_ps_rdy = false;
	select_pdo(battery->pd_list.pd_info[max_index].pdo_index);
}
