...
//#include <linux/sec_param.h>			temp
...
#endif
// static struct qpnp_vadc_chip *adc_client;	temp
...
static void sec_bat_swelling_check(struct sec_battery_info *battery, int temperature)
{
	...
	bool do_charge_on = false;
	/* do_charge_on flag : for 5~15 degree additional charging
   	                       with over 4.2V battery */
	...
	if (battery->swelling_mode) {
		...
		if ((temperature <= battery->pdata->swelling_high_temp_recov) &&
			...
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_VOLTAGE_MAX, val);
		} else if ((battery->voltage_now < swelling_rechg_voltage && battery->charging_block) || 
				do_charge_on) {
			pr_info("%s: swelling mode recharging start. Vbatt(%d), charge on Flag(%d)\n",
				__func__, battery->voltage_now, do_charge_on);
			battery->charging_mode = SEC_BATTERY_CHARGING_1ST;
...
static void sec_bat_cable_work(struct work_struct *work)
{
	...
	if (battery->cable_type == POWER_SUPPLY_TYPE_BATTERY ||
		...
		sec_bat_set_current_event(battery, SEC_BAT_CURRENT_EVENT_CHARGE_DISABLE, 1);
#if defined(CONFIG_CALC_TIME_TO_FULL)
...
//	case ATTACHED_DEV_UNDEFINED_RANGE_MUIC:		temp
	...
	case ATTACHED_DEV_UNOFFICIAL_ID_USB_MUIC:
		...
		break;
//	case ATTACHED_DEV_JIG_UART_ON_VB_MUIC:		temp
	case ATTACHED_DEV_JIG_UART_OFF_VB_MUIC:
	case ATTACHED_DEV_JIG_UART_OFF_VB_FG_MUIC:
		current_cable_type = POWER_SUPPLY_TYPE_UARTOFF;
		break;
//	case ATTACHED_DEV_RDU_TA_MUIC:			temp
//		battery->store_mode = true;
...
//			battery->muic_cable_type == ATTACHED_DEV_AFC_CHARGER_9V_DUPLI_MUIC ||		temp
...
//		(battery->muic_cable_type != ATTACHED_DEV_JIG_UART_ON_VB_MUIC) &&		temp
...
//		(battery->muic_cable_type != ATTACHED_DEV_UNDEFINED_RANGE_MUIC));		temp
