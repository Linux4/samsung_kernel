...
long long __attribute__((weak)) div_64(long long a, long long b)
{
...
{
#if defined(CONFIG_HV_MUIC_VOLTAGE_CTRL) || defined(CONFIG_SUPPORT_QC30) || defined(CONFIG_MUIC_SM5705_AFC_18W_TA_SUPPORT)
	if (battery->siop_level >= 100) {
...
#elif defined(CONFIG_MUIC_SM5705_AFC_18W_TA_SUPPORT)
				muic_18W_TA_voltage_control(SEC_INPUT_VOLTAGE_12V);
#endif
			} else {
#if defined(CONFIG_HV_MUIC_VOLTAGE_CTRL)
...
#elif defined(CONFIG_MUIC_SM5705_AFC_18W_TA_SUPPORT)
			muic_18W_TA_voltage_control(SEC_INPUT_VOLTAGE_5V);
#endif
...
static bool sec_bat_check_recharge(struct sec_battery_info *battery)
{
	...
	if (battery->status == POWER_SUPPLY_STATUS_FULL &&
		...
		if (battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP) {
			recharging_voltage = battery->pdata->swelling_low_rechg_voltage_1st; // float voltage - 150mV
...
static void sec_bat_swelling_check(struct sec_battery_info *battery)
{
	...
	if (battery->swelling_mode) {
		if (battery->temperature <= battery->pdata->swelling_low_temp_recov_2nd) { 
			swelling_rechg_voltage = battery->pdata->swelling_low_rechg_voltage_2nd;
		}
...
static void sec_bat_calculate_safety_time(struct sec_battery_info *battery)
{
	...
	expired_time = div_64((expired_time * battery->pdata->standard_curr), curr);
	...
	expired_time = div_64((expired_time * curr),battery->pdata->standard_curr);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case BATT_INBAT_VOLTAGE_OCV:
		if (battery->pdata->support_fgsrc_change == true) {
			...
			psy_do_property(battery->pdata->fgsrc_switch_name, set,
					(int)POWER_SUPPLY_EXT_PROP_INBAT_VOLTAGE_FGSRC_SWITCHING, value);
			for (j = 0; j < 5; j++) {
				psy_do_property(battery->pdata->fuelgauge_name, get,
					POWER_SUPPLY_PROP_VOLTAGE_NOW, value);
				ocv_data[j] = value.intval;
	...
	case CHECK_SLAVE_CHG:
		psy_do_property(battery->pdata->charger_name, get,
			(int)POWER_SUPPLY_EXT_PROP_CHECK_SLAVE_I2C, value);
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
				value.intval);
	...
	case WC_OP_FREQ:
		psy_do_property(battery->pdata->wireless_charger_name, get,
			(int)POWER_SUPPLY_EXT_PROP_WIRELESS_OP_FREQ, value);
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n", value.intval);
		break;
	case WC_CMD_INFO:
		psy_do_property(battery->pdata->wireless_charger_name, get,
			(int)POWER_SUPPLY_EXT_PROP_WIRELESS_TX_CMD, value);
		i += scnprintf(buf + i, PAGE_SIZE - i, "0x%02x ",
			value.intval);

		psy_do_property(battery->pdata->wireless_charger_name, get,
			(int)POWER_SUPPLY_EXT_PROP_WIRELESS_TX_VAL, value);
		i += scnprintf(buf + i, PAGE_SIZE - i, "0x%02x ",
			value.intval);	
		break;
	...
	case MODE:
		value.strval = NULL;
		psy_do_property(battery->pdata->charger_name, get,
			(int)POWER_SUPPLY_EXT_PROP_MULTI_CHARGER_MODE, value);
		i += scnprintf(buf + i, PAGE_SIZE - i, "%s\n",
	...
	case BATT_CHIP_ID:
		psy_do_property(battery->pdata->charger_name, get,
			(int)POWER_SUPPLY_EXT_PROP_CHIP_ID, value);
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
				value.intval);
		break;
				...
				const union power_supply_propval *val)
{
	...
	int full_check_type = SEC_BATTERY_FULLCHARGED_NONE;
	union power_supply_propval value = {0, };
	enum power_supply_ext_property ext_psp = (int)psp;
...
#if defined(CONFIG_MUIC_S2MU005_DISCHARGING_WA)
		...
		unsigned long action, void *data)
{
	...
	pr_err("batt_handle_notification(%s)\n", cmd);
		...
		struct sec_battery_info *battery)
{
	...
	if (ret) {
		...
		pr_info("%s: swelling wireless high temp recovery is swelling high temp recovery\n", __func__);
	}
	...
	ret = of_property_read_u32(np, "battery,swelling_low_rechg_voltage_1st",
		(unsigned int *)&pdata->swelling_low_rechg_voltage_1st);
	if (ret) {
		pr_info("%s: swelling_low_rechg_voltage_1st is Empty\n", __func__);
				pdata->swelling_low_rechg_voltage_1st = 4200;
	}

	ret = of_property_read_u32(np, "battery,swelling_low_rechg_voltage_2nd",
		(unsigned int *)&pdata->swelling_low_rechg_voltage_2nd);
	if (ret) {
		pr_info("%s: swelling_low_rechg_voltage_2nd is Empty\n", __func__);
				pdata->swelling_low_rechg_voltage_2nd = 4000;
	}
	...
	pr_info("%s : SWELLING_HIGH_TEMP(%d) SWELLING_HIGH_TEMP_RECOVERY(%d)\n"
		...
		"SWELLING_LOW_RCHG_1st(%d), SWELLING_LOW_RCHG_2nd(%d)\n",
		...
		pdata->swelling_low_rechg_voltage_1st, pdata->swelling_low_rechg_voltage_2nd);
#endif
...
static int sec_battery_probe(struct platform_device *pdev)
{
	...
	pr_err("%s: SEC Battery Driver Loading\n", __func__);
...
#else
#if defined(CONFIG_MUIC_NOTIFIER)
	pr_err("%s: batt_handle_notification(batt_handle_notification),MUIC_NOTIFY_DEV_CHARGER\n", __func__);
