...
static void s2mu005_charger_initialize(struct s2mu005_charger_data *charger)
{
	...
	s2mu005_read_reg(charger->client, 0x17, &temp);
	temp &= ~0x30;
	s2mu005_write_reg(charger->client, 0x17, temp);
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		pr_err("[DEBUG]%s:[BATT] cable_type(%d), input_current(%d)mA\n",
