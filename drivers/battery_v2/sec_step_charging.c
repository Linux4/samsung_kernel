...
void sec_step_charging_init(struct sec_battery_info *battery, struct device *dev)
{
	...
	if (!p) {
		...
		if (ret) {
			...
			unsigned int *qc_step_charging_current;
			...
			qc_step_charging_current = kzalloc(sizeof(u32) * len, GFP_KERNEL);
			ret = of_property_read_u32_array(np, "battery,qc_step_charging_current",
					qc_step_charging_current, len);
			if (ret) {
				pr_info("%s : QC step_charging_current read fail\n", __func__);
			} else {
				if (pdata->step_charging_current)
					kfree(pdata->step_charging_current);

				pdata->step_charging_current = qc_step_charging_current;
				for (i = 0; i < len; i++) {
					pr_info("%s : QC current(%d)\n",
						__func__, qc_step_charging_current[i]);
				}
				pr_info("%s : max charging current : %d\n", __func__, pdata->max_charging_current);
			}
