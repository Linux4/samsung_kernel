...
// #include <linux/battery/sec_charger.h>	temp
// #include "include/charger/sm5701_charger.h"	temp
...
static u8 SM5701_otg_control(struct SM5701_charger_data *charger, int enable)
{
	...
	mask = 0x02;
	otg_enable_flag_0 = enable;
/*	temp ( enable after led bringup )  
	if (enable) {
		otg_en |= mask;
		otg_en &= ~0x04;
...
static u8 SM5701_toggle_charger(struct SM5701_charger_data *charger, int enable)
{
	...
	pr_info("%s enable : %d \n", __func__, enable);
	pr_info("%s init SM5701_CNTL = 0x%x  \n", __func__, chg_en);
	...
	if (charger->dev_id == 4) {
		...
		if (led_state_charger == LED_DISABLE) {
			...
			SM5701_reg_read(charger->SM5701->i2c, SM5701_CNTL, &chg_en);
			pr_info("%s : CNTL register to (0x09) : 0x%02x\n", __func__, chg_en);

		} else {
			if (otg_enable_flag_0)
...
*/
	...
	case POWER_SUPPLY_PROP_CHARGE_OTG_CONTROL:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		pr_info("%s : POWER_SUPPLY_PROP_CHARGING_ENABLED, charger->is_charging = %d\n", __func__, val->intval);
		charger->is_charging = !val->intval;
...
int SM5701_chg_dt_init(struct device_node *np, struct device *dev, sec_battery_platform_data_t *pdata)
{
	int ret = 0, len = 0;
	...
	if (chg_gpio_en < 0) {
		pr_err("%s: chgen-gpio failed: %d\n", __func__,
							chg_gpio_en);
		return chg_gpio_en;
	} else 
		pr_err("%s: chgen-gpio done: %d\n", __func__,
							chg_gpio_en);
...
*/
	...
	if (chg_irq_gpio < 0)
		...
		pr_err("%s: chgirq-gpio get done: %d\n", __func__, chg_irq_gpio);
...
//	pdata->chg_gpio_init = true;	temp for bringup
...
// temp		ret = sec_chg_dt_init(pdev->dev.of_node, &pdev->dev, charger->pdata);
		ret = SM5701_chg_dt_init(pdev->dev.of_node, &pdev->dev, charger->pdata);
	...
	charger->psy_chg.name           = "sm5701-charger";
