...
static void s2mu005_reset_fg(struct s2mu005_fuelgauge_data *fuelgauge)
{
	...
	/* After FG reset current battery data version get reset to default value 1, causing mismatch in bootloader and kernel FG data version.
	 Below code restores the FG data version in 0x48 register to it's initialized value. */
...
static void s2mu005_fg_reset_capacity_by_jig_connection(struct s2mu005_fuelgauge_data *fuelgauge, int meas_mode)
{
	u8 data = 0;
	...
	if (meas_mode == SEC_BAT_INBAT_FGSRC_SWITCHING_ON) {
		...
		s2mu005_read_reg_byte(fuelgauge->i2c, 0x25, &data);
		data &= 0xCF;
		data |= 0x10;
		s2mu005_write_and_verify_reg_byte(fuelgauge->i2c, 0x25, data);
		mdelay(1000);
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		{
			...
			/* Blanket Temperature */
			case POWER_SUPPLY_EXT_PROP_BLKT_TEMP:
				val->intval = s2mu005_get_temperature(fuelgauge);
				break;
#endif
			default:
                            ...
                            const union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_TEMP_AMBIENT:
			...
			break;
		case POWER_SUPPLY_PROP_ENERGY_NOW:
			s2mu005_fg_reset_capacity_by_jig_connection(fuelgauge, val->intval);
			break;
		case POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN:
			dev_info(&fuelgauge->i2c->dev,
		...
		case POWER_SUPPLY_EXT_PROP_FUELGAUGE_FACTORY:
			...
			s2mu005_fg_reset_capacity_by_jig_connection(fuelgauge,0);
			break;
#if defined(CONFIG_BATTERY_AGE_FORECAST)
