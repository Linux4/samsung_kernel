	...
	if((por_state != 0x00) ||
	   (fuelgauge->probe_done == true &&
		...
		if (fuelgauge->reg_OTP_52 != reg_OTP_52 || fuelgauge->reg_OTP_53 != reg_OTP_53) {
			psy_do_property("s2mu005-charger", set, (int)POWER_SUPPLY_EXT_PROP_FUELGAUGE_RESET, value);
...
static int s2mu005_fuelgauge_parse_dt(struct s2mu005_fuelgauge_data *fuelgauge)
{
	...
	if (np == NULL) {
		...
		fuelgauge->pdata->fg_irq = gpio_request(fuelgauge->pdata->fg_irq, "gpio-fuel_int");
		...
		ret = of_property_read_u32(np, "fuelgauge,dual_battery",
				&fuelgauge->pdata->dual_battery);
		if (ret < 0) {
			pr_err("%s error reading pdata->dual_battery %d\n",
					__func__, ret);
			fuelgauge->pdata->dual_battery = 0;
		}
		...
		if (!np) {
			...
			/* Check if dual_battery is defined for battery dualization checking */
			if (fuelgauge->pdata->dual_battery) {
				/* Check HW revision */
				batt_id = gpio_get_value(BATT_ID_GPIO);
				pr_info("%s BATTERY ID : %d\n", __func__, batt_id);
			}

			if (batt_id == 0) {
				pr_info("%s Using data for 2850mA battery\n", __func__);
				ret = of_property_read_u32_array(np, "battery,battery_table3_2850", fuelgauge->info.battery_table3, 88);
				if (ret < 0) {
					pr_err("%s error reading battery,battery_table3_2850\n", __func__);
				}

				ret = of_property_read_u32_array(np, "battery,battery_table4_2850", fuelgauge->info.battery_table4, 22);
				if (ret < 0) {
					pr_err("%s error reading battery,battery_table4_2850\n", __func__);
				}

				ret = of_property_read_u32_array(np, "battery,batcap_2850", fuelgauge->info.batcap, 4);
				if (ret < 0) {
					pr_err("%s error reading battery,batcap_2850\n", __func__);
				}

				ret = of_property_read_u32_array(np, "battery,soc_arr_evt2_2850", fuelgauge->info.soc_arr_evt2, 22);
				if (ret < 0) {
					pr_err("%s error reading battery,soc_arr_evt2_2850\n", __func__);
				}

				ret = of_property_read_u32_array(np, "battery,ocv_arr_evt2_2850", fuelgauge->info.ocv_arr_evt2, 22);
				if (ret < 0) {
					pr_err("%s error reading battery,ocv_arr_evt2_2850\n", __func__);
				}
				ret = of_property_read_u32_array(np, "battery,FG_Accumulative_rate_evt2_2850", fuelgauge->info.fg_accumulative_rate_evt2, 2);
				if (ret < 0) {

					fuelgauge->info.fg_accumulative_rate_evt2[0]=0x00;    // REG 0x44
					fuelgauge->info.fg_accumulative_rate_evt2[1]=0x08;    // REG 0x45
					pr_err("%s There is no FG_Accumulative_rate_2850 value in DT. set to the default value(0x800) \n", __func__);
				}
			} else {
				pr_info("%s Using default battery data\n", __func__);
