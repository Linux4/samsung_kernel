	...
	int enable)
{
	...
	if (enable){
		...
		current_setting = charger->charging_current;
		da9155_set_charge_current(charger, 500);
		msleep(400);//before Sub_charger On, add deblay to wait the Main_charger current drop down, avoid Ibatt Peak.
...
static int da9155_check_cable_type(unsigned int type)
{
	...
	if (type == POWER_SUPPLY_TYPE_HV_MAINS
			|| type == POWER_SUPPLY_TYPE_HV_MAINS_18W
	...
	if(charger->i2c) {
		...
		if (enable == ENABLE) {
			...
			if((charger->prev_cable_type == POWER_SUPPLY_TYPE_HV_MAINS ||
				charger->prev_cable_type == POWER_SUPPLY_TYPE_HV_MAINS_18W ||
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_HEALTH:
		if (charger->cable_type == POWER_SUPPLY_TYPE_HV_MAINS ||
			charger->cable_type == POWER_SUPPLY_TYPE_HV_MAINS_18W ||
