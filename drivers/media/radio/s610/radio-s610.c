...
{
	struct device_node *np = NULL;
	struct i2c_client *client13;
	struct i2c_client *client14;
	struct s2mpu13_dev *s2mpu13;
	struct s2mpu14_dev *s2mpu14;

	np = of_find_compatible_node(NULL, NULL, "samsung,s2mpu13mfd");
	if (!np) {
		dev_err(radio->dev, "PMIC s2mpu13mfd device is not available\n");
		return NULL;
	}
	client13 = of_find_i2c_device_by_node(np);
	if (!client13) {
		dev_err(radio->dev, "PMIC s2mpu13mfd i2s is not available\n");
		return NULL;
	}
	s2mpu13 = i2c_get_clientdata(client13);
	radio->i2c_main = s2mpu13->pmic;

	np = of_find_compatible_node(NULL, NULL, "samsung,s2mpu14mfd");
	if (!np) {
		dev_err(radio->dev, "PMIC s2mpu14mfd device is not available\n");
		return NULL;
	}
	client14 = of_find_i2c_device_by_node(np);
	if (!client14) {
		dev_err(radio->dev, "PMIC s2mpu14mfd i2s is not available\n");
		return NULL;
	}

	s2mpu14 = i2c_get_clientdata(client14);
	radio->i2c_sub = s2mpu14->pmic;
...
int fm_ldo_enable(struct s610_radio *radio)
{
	...
	/* LDO16M 0xEC */
	ret = s2mpu13_write_reg(radio->i2c_main, 0x58, 0xEC);
	if (ret < 0) {
	...
	/* LDO19S 0xDE */
	ret = s2mpu14_write_reg(radio->i2c_sub, 0x57, 0xDE);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC sub register\n", __func__);
		return -1;
	}

	s2mpu13_read_reg(radio->i2c_main, 0x58, &reg16m);
	dev_info(radio->dev, "%s: read S2MPU13_PM_LDO16_CTRL: %xh\n", __func__, reg16m);

	s2mpu14_read_reg(radio->i2c_sub, 0x57, &reg19s);
	dev_info(radio->dev, "%s: read S2MPU14_PM_LDO19_CTRL: %xh\n", __func__, reg19s);
...
int fm_ldo_disable(struct s610_radio *radio)
{
	...
	/* LDO16M 0x6C */
	ret = s2mpu13_write_reg(radio->i2c_main, 0x58, 0x6C);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}
	...
	/* LDO19S 0x5E */
	ret = s2mpu14_write_reg(radio->i2c_sub, 0x57, 0x5E);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC sub register\n", __func__);
		return -1;
	}
