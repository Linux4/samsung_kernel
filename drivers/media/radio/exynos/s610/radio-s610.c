...
int fm_ldo_enable(struct s610_radio *radio)
{
	...
	s2mpu15_read_reg(radio->i2c_main1, ADDR_LDO26M_CTRL, &radio->ldo26m);
	dev_info(radio->dev, "%s:1) read S2MPU15_PM_LDO26_CTRL: %xh\n", __func__, radio->ldo26m);
	...
	/* ADDR_LDO26M_CTRL Always On 0xD8 */
	ret = s2mpu15_update_reg(radio->i2c_main1, ADDR_LDO26M_CTRL, 0xC0, 0xC0);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}

	/* ADDR_LDO27M_CTRL Always On 0xD2 */
	ret = s2mpu15_update_reg(radio->i2c_main1, ADDR_LDO27M_CTRL, 0xC0, 0xC0);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}

	s2mpu15_read_reg(radio->i2c_main1, ADDR_LDO26M_CTRL, &reg26m);
	dev_info(radio->dev, "%s:2) read S2MPU15_PM_LDO26_CTRL: %xh\n", __func__, reg26m);
...
int fm_ldo_disable(struct s610_radio *radio)
{
	...
	/* ADDR_LDO26M_CTRL On/off by SEL_VGPIO 0x58 */
	ret = s2mpu15_update_reg(radio->i2c_main1, ADDR_LDO26M_CTRL, 0x40, 0xC0);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}

	/* ADDR_LDO27M_CTRL On/off by SEL_VGPIO 0x52 */
	ret = s2mpu15_update_reg(radio->i2c_main1, ADDR_LDO27M_CTRL, 0x40, 0xC0);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}
	...
	s2mpu15_read_reg(radio->i2c_main1, ADDR_LDO26M_CTRL, &reg26m);
	dev_info(radio->dev, "%s: read S2MPU15_PM_LDO26_CTRL: %xh\n", __func__, reg26m);
