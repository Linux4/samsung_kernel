...
enum fm_rds_state_enum fm_rds_get_state(struct s610_radio *radio)
{
...
static enum fm_host_rds_errors_enum fm_rds_process_block(
		struct s610_radio *radio,
		u16 data, enum fm_rds_block_type_enum block_type,
		u16 err_count)
{
	enum fm_host_rds_errors_enum error_type;
...
void store_rt_data(struct fm_rds_parser_info *pi, u16 info, u8 blk_type, u8 err_cnt)
{
	...
	switch (blk_type) {
		case RDS_BLKTYPE_C:
			...
			RDSEBUG(gradio,
				"%s(), [RDS] RT_A(C): [%c][%c] [%d][%d], modulo idx=%d, seg=%d\n",
				__func__, a, b, (int)a, (int)b, i, seg);
			break;
		case RDS_BLKTYPE_D:
			if (pi->grp == RDS_GRPTYPE_2A) {
				...
				RDSEBUG(gradio,
					"%s(), [RDS] RT_A(D): [%c][%c] [%d][%d], modulo idx=%d, seg=%d\n",
					__func__, a, b, (int)a, (int)b, i, seg);
				...
				RDSEBUG(gradio,
					"%s(), [RDS] RT_B(C): [%c][%c] [%d][%d], modulo idx=%d, seg=%d\n",
					__func__, a, b, (int)a, (int)b, i, seg);
			}
			break;
		default:
	...
	struct fm_rds_parser_info *pi)
{
	...
	if (ringbuf_bytes_free(&radio->rds_rb) < RDS_MEM_MAX_THRESH_PARSER)	{
		RDSEBUG(radio, "[RDS] ringbuff not free, wait..");
		/*ringbuf_reset(&radio->rds_rb);*/
		return;
	}
	...
	while (pi->rds_event) {
		...
		if (!pi->rds_event && total_size) {
			if (!ringbuf_memcpy_into(&radio->rds_rb, buf_ptr, total_size)) {
				usage = ringbuf_bytes_used(&radio->rds_rb);
				RDSEBUG(radio,
					"%s(), warning. Wait until it is possible to enter ringbuff! usage:%04d",
					__func__, ringbuf_bytes_used(&radio->rds_rb));
				if (!usage)
					return;
			}
...
u8 find_code_word(u32 *data, enum fm_rds_block_type_enum b_type, bool seq_lock)
{
	u16 first13;
	u16 second13;
	u16 syndrome;
...
void fm_process_rds_data(struct s610_radio *radio)
{
	...
	u32 fifo_data;
	u32 data;
	enum fm_host_rds_errors_enum err_cnt;
	u8 min_pos = 0;
	u8 min_blk = 0;
	u8 min_blk_tmp = 0;
			...
			case RDS_STATE_FOUND_BL_D:
				if ((end_pos / BUF_LEN != (end_pos + BLK_LEN) / BUF_LEN) && !fetch_data && !remains) {
					...
					RDSEBUG(radio,"%s(), data:0x%x, errcnt:%d, gcount:%d",
						...
						err_cnt,
						radio->rds_gcnt++);
					if (fm_rds_process_block(radio, data >> 10, i, err_cnt)
							 == HOST_RDS_ERRS_UNCORR)
						fm_rds_update_error_status(radio,
							 radio->low->fm_state.rds_unsync_uncorr_weight);
					else
