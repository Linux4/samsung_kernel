...
#ifdef USE_PMIC_RW
static struct device_node *exynos_pmic_parse_dt(struct s621_radio *radio);
...
#ifndef RDS_POLLING_ENABLE
static irqreturn_t s621_hw_irq_handle(int irq, void *devid)
{
	struct s621_radio *radio = devid;
	u32 int_stat;
...
static struct device_node *exynos_pmic_parse_dt(struct s621_radio *radio)
{
	...
	np = of_find_compatible_node(NULL, NULL, "samsung,s2mpu16mfd");
	if (!np) {
		dev_err(radio->dev, "PMIC s2mpu16mfd device is not available\n");
		return NULL;
	}
	client16 = of_find_i2c_device_by_node(np);
	if (!client16) {
		dev_err(radio->dev, "PMIC s2mpu16mfd i2s is not available\n");
		return NULL;
	}
	s2mpu16 = i2c_get_clientdata(client16);
	radio->i2c_sub1 = s2mpu16->pmic1;
	radio->i2c_sub2 = s2mpu16->pmic2;
...
int fm_ldo_enable(struct s621_radio *radio)
{
	...
	s2mpu16_read_reg(radio->i2c_sub1, ADDR_LDO8S_CTRL, &radio->ldo8s);
	dev_info(radio->dev, "%s:1) read S2MPU16_PM1_LDO8S_CTRL: %xh\n", __func__, radio->ldo8s);
	...
	/* ADDR_LDO8S_CTRL Always On 0xD8 */
	ret = s2mpu16_update_reg(radio->i2c_sub1, ADDR_LDO8S_CTRL, 0xC0, 0xC0);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}
	...
	s2mpu16_read_reg(radio->i2c_sub1, ADDR_LDO8S_CTRL, &reg8s);
	dev_info(radio->dev, "%s:1) read S2MPU16_PM1_LDO8S_CTRL: %xh\n", __func__, reg8s);
...
int fm_ldo_disable(struct s621_radio *radio)
{
	...
	/* ADDR_LDO8S_CTRL On/off by SEL_VGPIO 0x58 */
	ret = s2mpu16_update_reg(radio->i2c_sub1, ADDR_LDO8S_CTRL, 0x40, 0xC0);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}
	...
	s2mpu16_read_reg(radio->i2c_sub1, ADDR_LDO8S_CTRL, &reg8s);
	dev_info(radio->dev, "%s: read S2MPU16_PM1_LDO8S_CTRL: %xh\n", __func__, reg8s);
...
#ifndef	RDS_POLLING_ENABLE
#ifdef	ENABLE_IF_WORK_QUEUE
	INIT_WORK(&radio->if_work, s621_if_work);
#endif	/*ENABLE_IF_WORK_QUEUE*/
#endif	/* RDS_POLLING_ENABLE */
...
MODULE_DESCRIPTION("Driver for S621 FM Radio in Exynos8835");
MODULE_LICENSE("GPL");
