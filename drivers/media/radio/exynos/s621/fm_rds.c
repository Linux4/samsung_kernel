...
/*
 * drivers/media/radio/exynos/s621/fm_rds.c
 *
 * FM Radio RDS driver for SAMSUNG S621 chip
 *
...
void fm_rds_write_data(struct s621_radio *radio,
...
void fm_rds_write_data_remove(struct s621_radio *radio,
...
int fm_read_rds_data(struct s621_radio *radio, u8 *buffer, int size,
		u16 *blocks)
{
	u16 rsize = size;
...
void fm_rds_change_state(struct s621_radio *radio,
		enum fm_rds_state_enum new_state)
{
	radio->low->fm_rds_state.current_state = new_state;
}

enum fm_rds_state_enum fm_rds_get_state(struct s621_radio *radio)
{
	return radio->low->fm_rds_state.current_state;
}

void fm_rds_update_error_status(struct s621_radio *radio, u16 errors)
{
	if (errors == 0) {
		...
		if (radio->low->fm_rds_state.error_bits
			...
			if (!radio->rds_sync_loss_cnt) {
				...
				fm_rds_write_data_remove(radio,
					(enum fm_rds_rm_align_enum) fm_rds_get_state(radio));
			}
...
void fm_rds_write_data_pi(struct s621_radio *radio,
	struct fm_rds_parser_info *pi)
{
...
void fm_process_rds_data(struct s621_radio *radio)
{
	u16 i, j, k, l;
	u16 avail_blocks;
	u32 fifo_data;
