...
static int iris_fops_open(struct file *file)
{
	...
	if (gpio_is_valid(radio->sel_gpio)) {
		gpio_set_value(radio->sel_gpio, 1);
	}
...
END:
	...
	if (gpio_is_valid(radio->sel_gpio)) {
		gpio_set_value(radio->sel_gpio, 0);
	}
...
static int iris_probe(struct platform_device *pdev)
{
	...
	if (radio->dev->of_node) {
		radio->sel_gpio = of_get_named_gpio(radio->dev->of_node,
					"qcom,radio-select-sw", 0);
	}

	if (gpio_is_valid(radio->sel_gpio)) {
		retval = gpio_request(radio->sel_gpio, "FM_SEL_SW");
		if (retval)
			FMDERR(": Could not request gpio: %d\n", radio->sel_gpio);

		gpio_direction_output(radio->sel_gpio, 0);
	}
