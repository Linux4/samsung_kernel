...
int dvb_dmx_init(struct dvb_demux *dvbdemux)
{
	...
	dvbdemux->users = 0;
	dvbdemux->filter = vzalloc(dvbdemux->filternum * sizeof(struct dvb_demux_filter));

	if (!dvbdemux->filter)
		return -ENOMEM;

	dvbdemux->feed = vzalloc(dvbdemux->feednum * sizeof(struct dvb_demux_feed));
	if (!dvbdemux->feed) {
	...
	dvbdemux->rec_info_pool = vzalloc(dvbdemux->feednum *
