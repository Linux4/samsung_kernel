...
*/
static int __must_check GSPN_ClocksCheckPhase0(GSPN_CONTEXT_T *gspnCtx, uint32_t core_id)
{
    int ret = 0;
    GSPN_CORE_MNG_T *core_mng = &gspnCtx->core_mng;
    ...
    if(core_mng->module_sel_reg_base && gspnCtx->capability.version == 0x10) {
...
int GSPN_TryToRecover(GSPN_CONTEXT_T *gspnCtx, uint32_t core_id)
{
    ...
    if(core_id < 0 || core_id >= GSPN_CORE_MAX || gspnCtx == NULL) {
        return -1;
    }
    reg_base = gspnCtx->core_mng.cores[core_id].ctl_reg_base;

    //on sharkLT8, check GSP/GSPN module select
    base = gspnCtx->core_mng.module_sel_reg_base;
    mask = 0x1;
    if(base && (gspnCtx->capability.version == 0x10) && (mask != (REG_GET(base)&mask))) {
    ...
    base = gspnCtx->core_mng.cores[core_id].clk_select_reg_base;
    mask = 0x3;
    ...
    //check GSPN module enable
    base = gspnCtx->core_mng.enable_reg_base;
    mask = gspnCtx->core_mng.cores[core_id].gspn_en_rst_bit;
    if(mask != (REG_GET(base)&mask)) {
    ...
    base = gspnCtx->core_mng.reset_reg_base;
    mask = gspnCtx->core_mng.cores[core_id].gspn_en_rst_bit;
    if(mask == (REG_GET(base)&mask)) {
    ...
    //check GSPN AUTO_GATE clock
    base = gspnCtx->core_mng.cores[core_id].auto_gate_reg_base;
    mask = gspnCtx->core_mng.cores[core_id].auto_gate_bit;
    if(mask != (REG_GET(base)&mask)) {
        GSPN_LOGE("core%d auto gate is disabled! 0x%08x & 0x%08x\n",
                  core_id, REG_GET(base), mask);
        REG_OR(base,mask);
    ...
    //check GSPN EMC clock
    base = gspnCtx->core_mng.cores[core_id].emc_reg_base;
    mask = gspnCtx->core_mng.cores[core_id].emc_en_bit;
    if(mask != (REG_GET(base)&mask)) {
    ...
    //check iommu clock
    base = gspnCtx->core_mng.enable_reg_base;
    mask = gspnCtx->core_mng.cores[core_id].mmu_en_rst_bit;
    if(mask != (REG_GET(base)&mask)) {
    ...
    base = gspnCtx->core_mng.cores[core_id].iommu_ctl_reg_base;
    mask = 0x1;
    if(mask != (REG_GET(base)&mask)) {
...
{
    int ret = 0;
    if(!CORE_ID_VALID(core_id) || gspnCtx == NULL) {
        return -1;
    }

    ret = GSPN_CoreClockSwitch(&gspnCtx->core_mng.cores[core_id],1);
    if(ret) {
    ...
    ret = GSPN_ClocksCheckPhase0(gspnCtx,core_id);
    return ret;
}
...
{
    int32_t sub_idx = core->current_kcmd_sub_cmd_index;
    GSPN_KCMD_INFO_T *kcmd = core->current_kcmd;
...
{
    GSPN_CONTEXT_T *gspnCtx = (GSPN_CONTEXT_T *)dev_id;
