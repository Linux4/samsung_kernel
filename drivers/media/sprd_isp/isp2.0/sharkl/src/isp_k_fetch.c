...
static int32_t isp_k_fetch_block(struct isp_io_param *param)
{
	...
	if (0 != ret) {
		printk("isp_k_fetch_block: copy error, ret=0x%x\n", (uint32_t)ret);
		return -1;
	}

	if (fetch_info.substract) {
		REG_OWR(ISP_FETCH_PARAM, BIT_7);
	} else {
		REG_MWR(ISP_FETCH_PARAM, BIT_7, 0);
	}

	/*afifo_wr_mode: it is not used by app, default value: 0x00*/
	REG_MWR(ISP_FETCH_PARAM, BIT_8, 0);
...
static int32_t isp_k_fetch_slice_size(struct isp_io_param *param)
{
	...
	if (0 != ret) {
		printk("isp_k_fetch_slice_size: copy error, ret=0x%x\n", (uint32_t)ret);
		return -1;
	}
