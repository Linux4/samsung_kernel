									...
									uint32_t *count, int fd)
{
	...
	if (fence != NULL) {
		...
		GSPN_LOGE("acquire fence get from fd error: %d\n", fd);
									...
									struct gspn_sync_timeline *timeline)
{
	int fd = 0;
	int ret = 0;
	uint32_t *acq_cnt = NULL;
	struct sync_fence **acq_fen_arr = NULL;
	int layer_enable = -1;

	if (layer_info == NULL
		|| list_data == NULL
		|| timeline ==NULL) {
		GSPN_LOGE("image fence process with null pointer\n");
		return -1;	
	}

	acq_cnt = list_data->acq_cnt;
	acq_fen_arr = list_data->acq_fen_arr;
	layer_enable = layer_info->layer_en;
									...
									struct gspn_sync_timeline *timeline)
{
	...
	if (layer_info == NULL
		|| list_data == NULL
		|| timeline ==NULL) {
		GSPN_LOGE("osd fence process with null pointer\n");
		return -1;	
	}
									...
									unsigned int record)
{
	...
	if (layer_enable) {
		data.name[sizeof(data.name) - 1] = '\0';
		strlcpy(data.name, "gspn_rls_fen", sizeof(data.name));
...
void gspn_layer_list_fence_free(
		struct gspn_layer_list_fence_data* list_data)
{
	struct sync_fence **fen_arr = NULL;
	struct sync_fence *fen = NULL;
	uint32_t *count = NULL;

	fen_arr = list_data->acq_fen_arr;;
	fen = list_data->rls_fen;
									...
									struct gspn_sync_timeline *timeline)
{
	...
	GSPN_LAYER0_INFO_T *img_layer = NULL;
	GSPN_LAYER1_INFO_T *osd_layers[3];
	GSPN_LAYER_DES1_INFO_T *des1_layer = NULL;
	GSPN_LAYER_DES2_INFO_T *des2_layer = NULL;

	struct gspn_layer_list_fence_data list_data;
	int32_t __user *des1_rls_fd;
	int32_t __user *des2_rls_fd;
	if (kcmd == NULL || timeline == NULL) {
		GSPN_LOGE("fence process with null kcmd or timeline\n");
		return ret;
	}

	cmd = &kcmd->src_cmd;
	img_layer = &cmd->l0_info;
	osd_layers[0] = &cmd->l1_info;
	osd_layers[1] = &cmd->l2_info;
	osd_layers[2] = &cmd->l3_info;
	des1_layer = &cmd->des1_info;
	des2_layer = &cmd->des2_info;
	...
	*list_data.acq_cnt = 0;
	...
	ret |= gspn_osd_layer_fence_process(&osd_layers, &list_data,
										timeline);
	if (ret) {
