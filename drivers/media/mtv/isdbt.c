...
#elif defined(CONFIG_MTV_MEDIATEK)
...
static void isdbt_gpio_on(void)
{
	...
	isdbt_pinctrl = devm_pinctrl_get_select(isdbt_device, "isdbt_power_en_high");
	if (IS_ERR(isdbt_pinctrl)) {
		DPRINTK("Target does not use pinctrl\n");
	}
	usleep_range(10000, 10000);
...
//		gpio_set_value(dt_pdata->isdbt_ant_sel, 1);
		isdbt_pinctrl = devm_pinctrl_get_select(isdbt_device, "isdbt_ant_sel_low");
		if (IS_ERR(isdbt_pinctrl)) {
			DPRINTK("Target does not use pinctrl\n");
		}
		usleep_range(10000, 10000);
	...
	if (gpio_is_valid(dt_pdata->isdbt_rst)) {
		...
		isdbt_pinctrl = devm_pinctrl_get_select(isdbt_device, "isdbt_rst_high");
		if (IS_ERR(isdbt_pinctrl)) {
			DPRINTK("Target does not use pinctrl\n");
		}
		usleep_range(10000, 10000);
...
static void isdbt_gpio_off(void)
{
	...
	isdbt_pinctrl = devm_pinctrl_get_select(isdbt_device, "isdbt_power_en_low");
	if (IS_ERR(isdbt_pinctrl)) {
		DPRINTK("Target does not use pinctrl\n");
	}
...
/*
	if (gpio_is_valid(dt_pdata->isdbt_ant_sel)) {
		...
		isdbt_pinctrl = devm_pinctrl_get_select(isdbt_device, "isdbt_ant_sel_high");
		if (IS_ERR(isdbt_pinctrl)) {
			DPRINTK("Target does not use pinctrl\n");
		}
		usleep_range(10000, 10000);
...
*/
	if (gpio_is_valid(dt_pdata->isdbt_rst)) {
		...
		isdbt_pinctrl = devm_pinctrl_get_select(isdbt_device, "isdbt_rst_low");
		if (IS_ERR(isdbt_pinctrl)) {
			DPRINTK("Target does not use pinctrl\n");
		}
	}
...
#ifdef CONFIG_MTV_MEDIATEK
static int isdbt_vcore_high(void)
{
	int ret = 0;
	ret = vcorefs_request_dvfs_opp(KIR_DTV, OPP_0);
	pr_info("%s : ret(%d)\n", __func__, ret);
	return ret;
}

static int isdbt_vcore_low(void)
{
	int ret = 0;
	ret = vcorefs_request_dvfs_opp(KIR_DTV, OPP_OFF);
	pr_info("%s : ret(%d)\n", __func__, ret);
	return ret;
}
#endif
...
int isdbt_power_on(unsigned long arg)
{
	...
	DPRINTK("%s: main_xtal_freq(%d)\n", __func__, dt_pdata->isdbt_xtal_freq);
	ret = (*isdbtdrv_func->power_on)(dt_pdata->isdbt_xtal_freq);
...
static bool get_isdbt_dt_pdata(struct device *dev)
{
	...
	if(dev->of_node)
	{
		...
		of_property_read_u32(dev->of_node, "isdbt_xtal_freq", &dt_pdata->isdbt_xtal_freq);
		if (dt_pdata->isdbt_xtal_freq < 0) {
			DPRINTK("can not find the isdbt_xtal_freq\n");
			goto alloc_err;
		}
		DPRINTK("device tree read complete\n");
#if defined(CONFIG_MTV_SMS3230)
		...
		dt_pdata->isdbt_xtal_freq = 24000;
	}
#if 0
	...
	DPRINTK("[daromy] isdbt_xtal_freq : %d\n", dt_pdata->isdbt_xtal_freq);
#endif
	return true;
...
static int isdbt_probe(struct platform_device *pdev)
{
	...
	isdbt_pinctrl = devm_pinctrl_get_select(isdbt_device, "isdbt_irq_init");
	if (IS_ERR(isdbt_pinctrl)) {
		DPRINTK("Target does not use pinctrl\n");
	}
#endif
	...
	ret = isdbt_spi_init();
	pr_info("%s : isdbt_spi_init() ret:%d\n", __func__, ret);
...
#elif defined(CONFIG_MTV_MEDIATEK)
	/* set drive-strength
		pin -> gpio65~68 is grouped as one SPI port -> just call it once (MTK)
		drv_param -> 0: 2mA, 1: 4mA, 2: 6mA, 3: 8mA */
	mt_set_spi_gpio_drv(65, 3);
#endif
