...
static struct spi_pin_desc spi_pin_group[] = {
	{"SPI_MISO", SPI_PIN_FUNC_DEF, REG_PIN_SPI0_DI + CTL_PIN_BASE, ISDBT_SPI_MISO},
	{"SPI_CLK", SPI_PIN_FUNC_DEF, REG_PIN_SPI0_CLK + CTL_PIN_BASE, ISDBT_SPI_CLK},
	{"SPI_MOSI", SPI_PIN_FUNC_DEF, REG_PIN_SPI0_DO + CTL_PIN_BASE, ISDBT_SPI_MOSI},
	{"SPI_CS0", SPI_PIN_FUNC_DEF, REG_PIN_SPI0_CSN + CTL_PIN_BASE, ISDBT_SPI_CS}
};

static struct spi_pin_desc isdbt_pwr_en_pin[1] = {
	{"SPI_PWR_EN", SPI_PIN_FUNC_GPIO, REG_PIN_IIS0MCK  + CTL_PIN_BASE, ISDBT_PWR_EN}
};
...
//			gpio_free(gpio);
	...
	__raw_writel(value, reg);
	local_irq_restore(flags);
	DPRINTK("[daromy] spi pin ctrl : %s - %x\n", isdbt_pwr_en_pin[0].name, value);
}
#endif
#endif
