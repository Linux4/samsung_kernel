...
static u32 fc8300_get_current_clk(HANDLE handle, DEVICEID devid)
{
	...
	return ((main_xtal_freq >> pre_sel) * multi) >> post_sel;
}
...
static s32 fc8300_set_tsif_clk(HANDLE handle, DEVICEID devid)
{
	...
	if (main_xtal_freq <= 10000)
		pre_sel = 0;
	else if (main_xtal_freq <= 20000)
		pre_sel = 1;
	else if (main_xtal_freq <= 40000)
		pre_sel = 2;
	else
		pre_sel = 3;

	input_freq = main_xtal_freq >> pre_sel;
...
s32 fc8300_probe(HANDLE handle, DEVICEID devid)
{
	...
	print_log(0, "fc8300_probe: ver(0x%x)\n", ver);
...
#endif
//	bbm_byte_write(handle, DIV_BROADCAST, BBM_HANGING_ENABLE, 0x00);
