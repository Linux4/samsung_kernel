		...
		enum BROADCAST_TYPE broadcast)
{
	...
	if (main_xtal_freq < 32000) {
		cal_temp = main_xtal_freq * 72 / 9000;
		filter_cal_09 = (u8) cal_temp;
	} else {
		filter_cal_09 = 0x00;
	}
	cal_temp = main_xtal_freq * 72 / 18000;
	filter_cal_18 = (u8) cal_temp;

#if (BBM_BAND_WIDTH == 6)
	cal_temp = main_xtal_freq * 72 / 30000;
#elif (BBM_BAND_WIDTH == 7)
	cal_temp = main_xtal_freq * 72 / 35000;
#else /* BBM_BAND_WIDTH == 8 */
	cal_temp = main_xtal_freq * 72 / 40000;
	...
	fc8300_write(handle, devid, 0xfd, (u8)(main_xtal_freq / 1000));
