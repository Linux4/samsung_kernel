...
#include <mach/samsung_camera_cs05.h>
...
#if defined(CONFIG_MACH_CS05)
static u16 s5k4ecgx_vendor_id(struct i2c_client *client)
{
	Cam_Printk(KERN_NOTICE "%s is called\n", __func__);

	s5k4ecgx_write(client, 0xFCFC, 0xD000); // Default page address setting
	s5k4ecgx_write(client, 0x0004, 0x0000); // Enable Address Auto-Increase

	s5k4ecgx_write(client, 0x0012, 0x0001);
	s5k4ecgx_write(client, 0x007A, 0x0000);
	s5k4ecgx_write(client, 0xA000, 0x0004);

	s5k4ecgx_write(client, 0xA002, 0x0006); // OTP PAGE 6
	s5k4ecgx_write(client, 0xA000, 0x0001);
	s5k4ecgx_write(client, 0xFCFC, 0xD000);
	s5k4ecgx_write(client, 0x0004, 0x0001);

	s5k4ecgx_read(client, 0xA006, &VENDOR_ID);

	Cam_Printk(S5K4ECGX_MOD_NAME"   [5MEGA CAM] vendor_id(Module) ID : 0x%04X\n", VENDOR_ID);
...
static void s5k4ecgx_init_af_position(struct i2c_client *client)
{
	int err=0;
	int i=0;

	Cam_Printk(KERN_NOTICE "s5k4ecgx_init_af_position is called!\n");

	tagCamReg32_t S5K4ECGX_INIT_AF_POSITION[] =
	{
		// AF Window Settings
		{0x0028, 0x7000},
		{0x002A, 0x0294},
	...
	for (i=0 ; i <ARRAY_SIZE(S5K4ECGX_INIT_AF_POSITION); i++) {
		err = s5k4ecgx_write(client, S5K4ECGX_INIT_AF_POSITION[i].addr, S5K4ECGX_INIT_AF_POSITION[i].value);
	}

	if(err < 0){
		Cam_Printk(KERN_ERR"%s: Failed: I2C write for S5K4ECGX_INIT_AF_POSITION\n", __func__);
	}else{
		Cam_Printk(KERN_NOTICE "%s: Success: Write S5K4ECGX_INIT_AF_POSITION\n", __func__);
	}
}
...
static int s5k4ecgx_set_focus_mode(struct i2c_client *client, s32 value, int step)
{
	...
	switch( step )
	{
		case SET_FOCUS_STEP1:
			s5k4ecgx_init_af_position(client); // Init AF x, y position
