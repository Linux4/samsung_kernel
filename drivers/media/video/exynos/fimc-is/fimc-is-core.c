...
#if defined(CONFIG_CAMERA_SENSOR_SR261)
	strcpy(type, "SFILE_SR261_FIMC_IS");
#endif
...
#if defined(CONFIG_CAMERA_SENSOR_SR261_OBJ)
	ret = sensor_sr261_probe(NULL, NULL);
	if (ret) {
		err("sensor_sr261_probe is fail(%d)", ret);
		goto p_err3;
	}
#endif
