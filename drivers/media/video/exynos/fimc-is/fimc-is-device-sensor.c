...
#include "sensor/fimc-is-device-sr261.h"
...
#ifdef CONFIG_USE_VENDER_FEATURE
	/* To indentify sensor later, using sensor_id.*/
	...
	u32 scenario)
{
	...
	if (scenario) {
		...
		if ((scenario == SENSOR_SCENARIO_VISION)
		    && (!device->pdata->vision)) {
			merr("vision mode not supported", device);
			ret = -EPERM;
			goto p_err;
		}
	...
	if (module->position == SENSOR_POSITION_REAR)
		fimc_is_sec_set_camid(CAMERA_SINGLE_REAR);
	else
