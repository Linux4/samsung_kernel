...
int mxr_sysmmu_fault_handler(struct device *dev, const char *mmuname,
		enum exynos_sysmmu_inttype itype, unsigned long pgtable_base,
		unsigned long fault_addr)
{
	struct mxr_device *mdev = to_mdev(dev);

	pr_err("MIXER PAGE FAULT occurred at 0x%lx (Page table base: 0x%lx)\n",
			fault_addr, pgtable_base);

	mxr_dump_registers(mdev);
...
static int __devinit mxr_probe(struct platform_device *pdev)
{
	...
	exynos_sysmmu_set_fault_handler(&s5p_device_mixer.dev,
			mxr_sysmmu_fault_handler);
