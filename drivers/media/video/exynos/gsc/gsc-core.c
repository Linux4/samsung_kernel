...
static char *gsc_clocks[GSC_MAX_CLOCKS] = {
	"gscl", "aclk_gscaler_333", "aclk_gscl_333"
};
#endif
int gsc_dbg = 6;
...
void gsc_set_prefbuf(struct gsc_dev *gsc, struct gsc_frame frm)
{
	...
	if (frm.fmt->num_planes == 2) {
		...
		u32 low_addr = 0, low_plane = 0, mid_addr = 0, mid_plane = 0, high_addr = 0, high_plane = 0;
		u32 t_min, t_max;
...
static int gsc_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(gsc->clock[CLK_GATE])) {
		dev_err(&pdev->dev, "failed to get gscaler.%d clock",
				gsc->id);
		goto err_regs_unmap;
	}
