...
#define SC_SMC_PROTECTION_ID 5
#define SC_SMC_PROTECTION_ON 1
#define SC_SMC_PROTECTION_OFF 0
#define SMC_PROTECTION_SET  0x81000000

#ifdef CONFIG_EXYNOS_CONTENT_PATH_PROTECTION
static inline int sc_set_contents_protection(struct sc_ctx *ctx, bool on)
{
	/*
	 * NOTE: racing while cheching dev->m2m.in_use and updating
	 *       dev->state may occur logically. But it does not
	 *       occur in reallity because user-land takes care.
	 */
	if (atomic_read(&ctx->sc_dev->m2m.in_use) > 1) {
		dev_err(ctx->sc_dev->dev,
				"Unabled to set contents protection due to multictx\n");
		return -EBUSY;
	}

	if (on && test_bit(DEV_CP, &ctx->sc_dev->state)) {
		dev_err(ctx->sc_dev->dev,
				"Contents protection is already set\n");
		return -EBUSY;
	}
	...
	vb2_ion_set_protected(ctx->sc_dev->alloc_ctx, on);

	return 0;
}

static inline void sc_clear_contents_protection(struct sc_ctx *ctx)
{
	/* ctx->state_for_cp := &sc->state by V4L2_CID_CONTENT_PROTECTION */
	if (ctx->state_for_cp) {
...
static inline int sc_set_contents_protection(struct sc_ctx *ctx, bool on)
{
	dev_err(ctx->sc_dev->dev, "Contents protection is not configured!\n");
	return -ENOSYS;
}
#define sc_clear_contents_protection(x) do { } while (0)
#define exynos_smc(a, b, c, d) do { } while (0)
#endif
...
static int sc_open(struct file *file)
{
	...
	if (test_bit(DEV_CP, &sc->state)) {
		dev_err(sc->dev,
			"Multi-ctx is not allowed while contents protection\n");
		return -EINVAL;
	}
...
static irqreturn_t sc_irq_handler(int irq, void *priv)
{
	...
	exynos_smc(SMC_PROTECTION_SET, 0, SC_SMC_PROTECTION_ID,
			test_bit(DEV_CP, &sc->state) ?
				SC_SMC_PROTECTION_OFF : SC_SMC_PROTECTION_ON);
...
static void sc_m2m_device_run(void *priv)
{
	...
	exynos_smc(SMC_PROTECTION_SET, 0, SC_SMC_PROTECTION_ID,
			test_bit(DEV_CP, &sc->state) ?
				SC_SMC_PROTECTION_ON : SC_SMC_PROTECTION_OFF);
