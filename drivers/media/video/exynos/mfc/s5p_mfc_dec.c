...
static int s5p_mfc_buf_init(struct vb2_buffer *vb)
{
	...
	if (vq->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		for (i = 0; i < ctx->dst_fmt->num_planes; i++) {
			...
			raw = &ctx->raw_buf;
			if (is_out_of_bound(dev, buf->planes.raw[i],
							raw->plane_size[i])) {
				mfc_err("Out of bound, base = 0x%x\n", dev->port_a);
				mfc_err("buf[%d] = 0x%x(size = 0x%x)\n",
					i, buf->planes.raw[i], raw->plane_size[i]);
				return -EINVAL;
			}
		}
		...
		if (is_out_of_bound(dev, buf->planes.stream, dec->src_buf_size)) {
			mfc_err("Out of bound, base = 0x%x", dev->port_a);
			mfc_err("buf = 0x%x(size = 0x%x)\n",
					buf->planes.stream, dec->src_buf_size);
			return -EINVAL;
		}
