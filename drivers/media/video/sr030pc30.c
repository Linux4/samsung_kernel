...
static int sr030pc30_init_state;
#endif
...
#if defined(CONFIG_MACH_CS05)
	Cam_Printk(KERN_NOTICE "===sr030pc30_init===[%s  %d]====== : Skip sr030pc30_Init_Reg setting\n", __FUNCTION__, __LINE__);
#else
...
#if defined(CONFIG_MACH_CS05)
	if(value == VT_MODE){
		sr030pc30_write_regs(client,sr030pc30_VT_Init_Reg,ARRAY_SIZE(sr030pc30_VT_Init_Reg),"sr030pc30_VT_Init_Reg");
		sr030pc30_cam_state = SR030PC30_STATE_PREVIEW;
		sr030pc30_init_state = FALSE;
		Cam_Printk(KERN_NOTICE "sr030pc30_set_preview_status: VT mode\n");
	}
	else if(value == SMART_STAY_MODE){
		sr030pc30_cam_state = SR030PC30_STATE_SMART_STAY;
		Cam_Printk(KERN_NOTICE "sr030pc30_set_preview_status: Smart stay mode\n");
	}
	else{
		if(sr030pc30_init_state == FALSE){
			sr030pc30_write_regs(client,sr030pc30_Init_Reg,ARRAY_SIZE(sr030pc30_Init_Reg),"sr030pc30_Init_Reg");
			sr030pc30_t_brightness(client, sensor->ev); // Rewrite brightness setting to sensor (Sleep on and Wake up issue)
			sr030pc30_cam_state = SR030PC30_STATE_PREVIEW;
			sr030pc30_init_state = TRUE;
