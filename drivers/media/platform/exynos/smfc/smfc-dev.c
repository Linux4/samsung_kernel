...
static int exynos_smfc_probe(struct platform_device *pdev)
{
	...
	if (exynos_soc_info.main_rev == 0) {
		...
		if (smfc->idle_ip_index < 0)
			return smfc->idle_ip_index;
	} else {
		smfc->idle_ip_index = -1;
	}
#endif
...
#if defined(CONFIG_SOC_S5E9935)
	if (exynos_soc_info.main_rev == 0)
		exynos_update_ip_idle_status(smfc->idle_ip_index, 0);
#endif
...
#if defined(CONFIG_SOC_S5E9935)
	if (exynos_soc_info.main_rev == 0)
		exynos_update_ip_idle_status(smfc->idle_ip_index, 1);
#endif
