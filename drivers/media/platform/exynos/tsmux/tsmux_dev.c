...
#define MAX_OTF_JOB_DONE_WAIT_TIME	100000
#define MAX_VIDEO_FRAME_SKIP_TIME	10000000
...
irqreturn_t tsmux_irq(int irq, void *priv)
{
	...
	if (tsmux_is_partial_done_id_0(tsmux_dev)) {
		job_id = 0;
		print_tsmux(TSMUX_COMMON, "Job ID %d is partial done\n", job_id);
...
static int tsmux_open(struct inode *inode, struct file *filp)
{
	...
	ktime = ktime_get();
	ctx->tsmux_start_stamp = ktime_to_us(ktime);
...
int tsmux_ioctl_m2m_map_buf(struct tsmux_context *ctx, struct tsmux_m2m_cmd_queue *arg)
{
	...
	if (copy_from_user(&temp_m2m_cmd_queue, arg,
		...
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_m2m_map_buf: fail copy_from_user\n");
		return -EFAULT;
	}
	...
	if (copy_to_user(arg, &ctx->m2m_cmd_queue,
				sizeof(struct tsmux_m2m_cmd_queue))) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_m2m_map_buf: fail copy_to_user\n");
		return  -EFAULT;
	}
	...
	struct tsmux_buffer_info *buf_info, int dma_direction)
{
	...
	if (ctx == NULL || ctx->tsmux_dev == NULL) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_m2m_unmap_buf: ctx is null\n");
		return -ENOMEM;
	}
	...
	if (!IS_ERR_VALUE(temp_dma_addr) && temp_dma_addr) {
		print_tsmux(TSMUX_M2M, "dma_buf_unmap_attachment(%pK, %llx)\n",
...
int tsmux_ioctl_asb_test(struct tsmux_context *ctx, struct tsmux_asb_job *arg)
{
	...
	if (copy_from_user(&temp_asb_job, arg, sizeof(struct tsmux_asb_job))) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_asb_test: fail copy_from_user\n");
		return -EFAULT;
	}
...
int tsmux_ioctl_m2m_run(struct tsmux_context *ctx, struct tsmux_m2m_cmd_queue *arg)
{
	...
	if (ctx == NULL || ctx->tsmux_dev == NULL) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_m2m_run: ctx is null\n");
		return -ENOMEM;
	}

	if (copy_from_user(&temp_m2m_cmd_queue, arg, sizeof(struct tsmux_m2m_cmd_queue))) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_m2m_run: fail copy_from_user\n");
		return -EFAULT;
	}

	tsmux_dev = ctx->tsmux_dev;

	print_tsmux(TSMUX_M2M, "line %d\n", __LINE__);
	...
	print_tsmux(TSMUX_M2M, "m2m job %d, dst_len_reg %d", 1, dst_len);

	if (copy_to_user(arg, &ctx->m2m_cmd_queue, sizeof(struct tsmux_m2m_cmd_queue))) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_m2m_run: fail to copy_to_user\n");
		return -EFAULT;
	}
...
void tsmux_set_es_size(unsigned int size)
{
	...
	if (count == 1) {
		...
		print_tsmux(TSMUX_OTF, "index %d, es_size %d\n", index, size);
	} else {
		print_tsmux(TSMUX_ERR, "BUF_DONE count %d\n", count);
	}
	spin_unlock_irqrestore(&g_tsmux_dev->device_spinlock, flags);
}
...
int tsmux_ioctl_set_psi_info(struct tsmux_context *ctx, struct tsmux_psi_info *arg)
{
	...
	if (ctx == NULL || ctx->tsmux_dev == NULL) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_set_psi_info: ctx is null\n");
		return -ENOMEM;
	}

	if (copy_from_user(&temp_psi_info, arg, sizeof(struct tsmux_psi_info))) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_set_psi_info: fail copy_from_user\n");
		return -EFAULT;
	}
...
static bool tsmux_ioctl_otf_dq_buf(struct tsmux_context *ctx, struct tsmux_otf_cmd_queue *arg)
{
	long wait_time = 0;
	struct tsmux_device *tsmux_dev = ctx->tsmux_dev;
	unsigned long flags;
	...
	while ((index = get_job_done_buf(ctx)) == -1) {
		wait_time = wait_event_interruptible_timeout(ctx->otf_wait_queue,
				is_otf_job_done(ctx), usecs_to_jiffies(MAX_OTF_JOB_DONE_WAIT_TIME));
		ktime = ktime_get();
		timestamp = ktime_to_us(ktime);
		spin_lock_irqsave(&tsmux_dev->device_spinlock, flags);
		timestamp = timestamp - ctx->tsmux_start_stamp;
		if (wait_time <= 0) {
			if ((ctx->otf_job_queued && timestamp >= MAX_OTF_JOB_DONE_WAIT_TIME) ||
				timestamp >= (MAX_VIDEO_FRAME_SKIP_TIME + MAX_OTF_JOB_DONE_WAIT_TIME))
				time_out = true;
		}
		spin_unlock_irqrestore(&tsmux_dev->device_spinlock, flags);
	...
	if (index != -1) {
		...
		print_tsmux(TSMUX_OTF, "wait %d for es_size done\n", i);
	}
	...
	if (wait_time > 0 || index != -1) {
		...
		print_tsmux(TSMUX_COMMON, "mfc_encoding_index %d, es_size %d, g2d %lld %lld\n",
		...
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_otf_dq_buf: time out (wait_time: %ld)\n", wait_time);
		return -EFAULT;
	}
...
static bool tsmux_ioctl_otf_q_buf(struct tsmux_context *ctx, int32_t *arg)
{
	struct tsmux_device *tsmux_dev = ctx->tsmux_dev;
	unsigned long flags;
	int32_t index;
	...
	if (copy_from_user(&temp_cur_buf_num, arg, sizeof(int32_t))) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_otf_q_buf: fail copy_from_user\n");
		return -EFAULT;
	}
...
static int tsmux_ioctl_otf_set_config(struct tsmux_context *ctx, struct tsmux_otf_config *arg)
{
	...
	if (copy_from_user(&temp_otf_config, arg, sizeof(struct tsmux_otf_config))) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_otf_set_config: fail copy_from_user\n");
		return -EFAULT;
	}
...
static int tsmux_ioctl_set_rtp_ts_info(struct tsmux_context *ctx, struct tsmux_rtp_ts_info *arg)
{
	...
	if (copy_from_user(&temp_rtp_ts_info, arg, sizeof(struct tsmux_rtp_ts_info))) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_set_rtp_ts_info: fail copy_from_user\n");
		return -EFAULT;
	}
	spin_lock_irqsave(&tsmux_dev->device_spinlock, flags);
...
static int tsmux_ioctl_otf_map_buf(struct tsmux_context *ctx, struct tsmux_otf_cmd_queue *arg)
{
	...
	struct dma_buf_attachment *temp_dmabuf_att;
	dma_addr_t temp_dma_addr;
	struct dma_buf_map temp_dambuf_map = DMA_BUF_MAP_INIT_VADDR(NULL);
	void *temp_vaddr = NULL;
	struct tsmux_otf_cmd_queue temp_otf_cmd_queue;

	if (ctx == NULL || ctx->tsmux_dev == NULL) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_otf_map_buf: ctx is null\n");
		return -ENOMEM;
	}
	...
	if (copy_from_user(&temp_otf_cmd_queue, arg, sizeof(struct tsmux_otf_cmd_queue))) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_otf_map_buf: fail copy_from_user\n");
		return -EFAULT;
	}
	...
	for (i = 0; i < TSMUX_OUT_BUF_CNT; i++) {
		...
		if (IS_ERR(temp_dmabuf_att)) {
			...
			temp_dma_addr = sg_dma_address(temp_sgt->sgl);
			print_tsmux(TSMUX_OTF, "dma_buf_map_attachment() ret dma_addr_t 0x%llx\n",
				temp_dma_addr);
		}
		...
		if (IS_ERR_VALUE(temp_dma_addr) || temp_dma_addr == 0) {
			...
			ret = dma_buf_vmap(temp_dmabuf, &temp_dambuf_map);
			if (ret) {
				print_tsmux(TSMUX_ERR, "otf dma_buf_vmap() error\n");
				ret = -ENOMEM;
			} else {
				temp_vaddr = temp_dambuf_map.vaddr;
		...
		memcpy(&out_buf_info->dmabuf_map, &temp_dambuf_map, sizeof(struct dma_buf_map));
...
int tsmux_ioctl_otf_unmap_buf(struct tsmux_context *ctx)
{
	...
	if (ctx == NULL || ctx->tsmux_dev == NULL) {
		print_tsmux(TSMUX_ERR, "tsmux_ioctl_otf_unmap_buf: ctx is null\n");
		return -ENOMEM;
	}
	...
	for (i = 0; i < TSMUX_OUT_BUF_CNT; i++) {
		...
		memcpy(&temp_dambuf_map, &out_buf_info->dmabuf_map, sizeof(struct dma_buf_map));
		...
		if (temp_vaddr) {
			print_tsmux(TSMUX_OTF, "dma_buf_vunmap(%pK, %pK)\n",
					temp_dmabuf, temp_vaddr);
			dma_buf_vunmap(temp_dmabuf, &temp_dambuf_map);
		}

		if (!IS_ERR_VALUE(temp_dma_addr) && temp_dma_addr) {
			print_tsmux(TSMUX_OTF, "dma_buf_unmap_attachment(%pK, %llx)\n",
					temp_dmabuf_att, temp_dma_addr);
			dma_buf_unmap_attachment(temp_dmabuf_att, temp_sgt, DMA_FROM_DEVICE);
		}

		print_tsmux(TSMUX_OTF, "dma_buf_unmap_attachment() ok\n");
			    ...
			    unsigned int cmd, unsigned long arg)
{
	...
	case TSMUX_IOCTL_SET_RTP_TS_INFO:
		print_tsmux(TSMUX_COMMON, "TSMUX_IOCTL_SET_RTP_TS_INFO\n");
	...
	case TSMUX_IOCTL_ASB_TEST:
		print_tsmux(TSMUX_OTF, "TSMUX_IOCTL_ASB_TEST\n");
#ifdef ASB_TEST
	...
	default:
		print_tsmux(TSMUX_ERR, "tsmux_ioctl: invalid ioctl\n");
		ret = -ENOTTY;
	}
...
#ifdef ASB_TEST
	g_tsmux_debug_level = TSMUX_ERR | TSMUX_SFR | TSMUX_COMMON | TSMUX_OTF | TSMUX_M2M | TSMUX_DBG_SFR;
#else
