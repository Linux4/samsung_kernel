		...
		int num_planes)
{
	...
	for (i = 0; i < num_planes; i++) {
		...
		MFC_CORE_DMA_WRITEL(buf_addr->otf_daddr[index][i],
		...
		unsigned int strm_size)
{
	...
	mfc_ctx_debug(2, "[STREAM] strm_size, %#x, %d, need_buf_size, %zu, buf_size, %zu, offset: %u\n",
...
void mfc_core_set_dec_metadata_buffer(struct mfc_core *core, struct mfc_ctx *ctx)
{
	...
	mfc_ctx_debug(2, "[MEMINFO] metadata buf 0x%llx size: %d\n",
