...
static int s5p_mfc_set_slice_mode(struct s5p_mfc_ctx *ctx)
{
	...
	/* multi-slice control */
	if (enc->slice_mode == V4L2_MPEG_VIDEO_MULTI_SLICE_MODE_MAX_MB_ROW)
...
void s5p_mfc_set_default_params(struct s5p_mfc_ctx *ctx)
{
	...
	WRITEL(0x40, S5P_FIMV_E_MV_HOR_RANGE);
	WRITEL(0x20, S5P_FIMV_E_MV_VER_RANGE);
	...
	/* Hierarchical Coding */
	WRITEL(0x8, S5P_FIMV_E_H264_NUM_T_LAYER);
	WRITEL(0x0, S5P_FIMV_E_VP8_NUM_T_LAYER);
	...
	WRITEL(0x1E0001, S5P_FIMV_E_RC_FRAME_RATE); /* framerate: 30fps */
	...
	/* H264 */
	WRITEL(0x1, S5P_FIMV_E_H264_OPTIONS);
	...
	/* Additional initialization: NAL_start only */
...
static int s5p_mfc_set_enc_params(struct s5p_mfc_ctx *ctx)
{
	...
	if (p->intra_refresh_mb)
		reg |= (0x1 << 4);
	/* memory structure src tile. frame 0: Linear, 1: 2D tiled */
	if (ctx->src_fmt->fourcc == V4L2_PIX_FMT_NV12MT_16X16)
		reg |= (0x1 << 7);
	/* memory structure recon tile. frame 0: Linear, 1: 2D tiled */
	reg |= (0x1 << 8);
...
static int s5p_mfc_set_enc_params_h264(struct s5p_mfc_ctx *ctx)
{
	...
	mfc_debug(2, "set Temporal SVC : p_264->hier_qp_enable %d, "
		"num_hier_layer: %d, hier_ref_type : %d, NUM_T_LAYER: 0x%x\n",
		p_264->hier_qp_enable, p_264->num_hier_layer, p_264->hier_ref_type, reg);
...
static int s5p_mfc_set_enc_params_vp8(struct s5p_mfc_ctx *ctx)
{
	...
	reg = READL(S5P_FIMV_E_VP8_NUM_T_LAYER);
	reg &= ~(0x7);
