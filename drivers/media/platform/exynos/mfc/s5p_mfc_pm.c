...
#elif defined(CONFIG_ARCH_EXYNOS5) || defined(CONFIG_ARCH_EXYNOS3)
...
int s5p_mfc_init_pm(struct s5p_mfc_dev *dev)
{
	...
	dev->pm.clock = clk_get(dev->device, "aclk_mfcmscl_200_aclk_mfc");
#endif
...
int s5p_mfc_set_clock_parent(struct s5p_mfc_dev *dev)
{
	...
	clk_child = clk_get(dev->device, "aclk_mfcmscl_333_aclk_mfcmscl_d");
...
int s5p_mfc_power_off(struct s5p_mfc_dev *dev)
{
	...
	if (IS_ERR_OR_NULL(clk_child)) {
		pr_err("failed to get %s clock\n", "aclk_mfcmscl_333_aclk_mfcmscl_d");
		return PTR_ERR(clk_child);
	}
