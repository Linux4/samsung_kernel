			...
			int drm_switch)
{
		...
		if (mfc_core_nal_q_check_enable(core) == 0) {
			...
			if (drm_switch)
				mfc_core_run_cache_flush(core, ctx->is_drm,
						MFC_CACHEFLUSH, drm_switch, 0);

			if (IS_NO_INFOLOG(ctx))
				mfc_debug(2, "[NALQ] start NAL QUEUE\n");
			else
				mfc_info("[NALQ] start NAL QUEUE\n");
	...
	case NAL_Q_STATE_STARTED:
		if (mfc_core_nal_q_check_enable(core) == 0 ||
			...
			if (ctx->dec_priv && ctx->dec_priv->defer_dec) {
				for (i = 0; i < (MFC_DEC_DEFER_FRAME - 1); i++) {
					if (mfc_core_nal_q_enqueue_in_buf(core, core_ctx,
							nal_q_handle->nal_q_in_handle)) {
						mfc_debug(2, "[NALQ][DEFER] Failed to enqueue input data (%d)\n", i);
						break;
					}
				}
			}
		...
		break;
	default:
		mfc_info("[NALQ] can't try command, nal_q_state : %d\n",
...
int mfc_core_just_run(struct mfc_core *core, int new_ctx_index)
{
	...
	if (!MFC_FEATURE_SUPPORT(dev, dev->pdata->drm_switch_predict)
		...
		if (drm_switch)
			mfc_core_run_cache_flush(core, ctx->is_drm, MFC_CACHEFLUSH, drm_switch, 0);
	} else {
		 ...
		 */
		next_ctx_index = core->sched->get_next_work(core);
		if (next_ctx_index >= 0) {
