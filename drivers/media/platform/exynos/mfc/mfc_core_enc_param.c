...
static void __mfc_set_enc_params(struct mfc_core *core, struct mfc_ctx *ctx)
{
	...
	if (dev->pdata->support_sbwcl) {
		if (ctx->is_sbwc_lossy == MFC_SBWCL_NOHEADER) {
			...
			mfc_clear_set_bits(reg, 0x3, 28, 0);
		}
	}
				...
				struct mfc_ctx *ctx)
{
	...
	if (IS_8K_PERF(ctx) || (OVER_UHD_RES(ctx) && p->rc_framerate > 45)) {
		p->num_refs_for_p = 1;
		mfc_ctx_debug(2, "forcely use 1-ref frame for 8K or 4K %d fps\n", p->rc_framerate);
	}
...
static void __mfc_set_enc_params_bpg(struct mfc_core *core ,struct mfc_ctx *ctx)
{
	...
	mfc_ctx_debug(3, "main image extension size %d (thumbnail: %d, exif: %d)\n",
			reg, p_bpg->thumb_size, p_bpg->exif_size);
