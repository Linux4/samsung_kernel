					...
					struct s2mpu_notifier_info *nb_info)
{
	...
	if ((strncmp("MFC0", nb_block->subsystem, strlen("MFC0")) == 0) ||
			(strncmp("MFC1", nb_block->subsystem, strlen("MFC1")) == 0)) {
		core = container_of(nb_block, struct mfc_core, mfc_s2mpu_nb);
	} else if ((strncmp("TZMP2_MFC0", nb_block->subsystem, strlen("TZMP2_MFC0")) == 0) ||
			(strncmp("TZMP2_MFC1", nb_block->subsystem, strlen("TZMP2_MFC1")) == 0)) {
		core = container_of(nb_block, struct mfc_core, mfc_s2mpu_nb_drm);
	} else {
		pr_err("It is not MFC block: %s\n", nb_block->subsystem);
