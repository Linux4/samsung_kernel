		...
		unsigned int pixelformat)
{
	...
	if (fmt && !dev->pdata->support_sbwclh && (fmt->type & MFC_FMT_SBWCLH)) {
		mfc_ctx_err("[FRAME] SBWC lossy v2.7 is not supported\n");
		fmt = NULL;
	}
							...
							struct v4l2_format *f)
{
	...
	mfc_info("[FRAME] resolution w: %d, h: %d, Y stride: %d, C stride: %d (mb: %lld)\n",
...
static int mfc_enc_qbuf(struct file *file, void *priv, struct v4l2_buffer *buf)
{
	...
	if (buf->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		if (ctx->src_fmt->mem_planes != buf->length) {
			mfc_err("number of memory container miss-match between Src planes(%d) and buffer length(%d)\n",
...
static int __mfc_enc_set_param(struct mfc_ctx *ctx, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_MPEG_VIDEO_GDC_VOTF:
		...
		if (mfc_core_votf_init(core, ctx))
			return -EFAULT;
