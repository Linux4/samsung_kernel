...
unsigned long mfc_qos_get_weighted_mb_fps(struct mfc_ctx *ctx, unsigned long fps)
{
	unsigned long mb_width, mb_height, frame_mb, mb, weighted_mb;

	mb_width = (ctx->img_width + 15) / 16;
	mb_height = (ctx->img_height + 15) / 16;
	frame_mb = mb_width * mb_height;

	mb = frame_mb * (fps / 1000);
	weighted_mb = __mfc_qos_get_weighted_mb(ctx, mb);
...
void __mfc_qos_calculate(struct mfc_core *core, struct mfc_ctx *ctx, int delete)
{
	...
	mfc_ctx_debug(3, "[QoS] core-%d total hw mb: %lld\n", core->id, core->total_mb);
