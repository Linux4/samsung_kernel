				...
				struct mfc_fmt *fmt)
{
	...
	case V4L2_PIX_FMT_NV12N_SBWC_256_8B:
		buf->addr[0][0] = start_raw;
		buf->addr[0][1] = SBWC_256_8B_CBCR_BASE(start_raw, ctx->img_width, ctx->img_height);
		break;
	case V4L2_PIX_FMT_NV12N_SBWC_10B:
		buf->addr[0][0] = start_raw;
	...
	case V4L2_PIX_FMT_NV12N_SBWC_256_10B:
		buf->addr[0][0] = start_raw;
		buf->addr[0][1] = SBWC_256_10B_CBCR_BASE(start_raw, ctx->img_width, ctx->img_height);
		break;
	case V4L2_PIX_FMT_NV12N_SBWCL_8B:
		buf->addr[0][0] = start_raw;
	...
	case V4L2_PIX_FMT_NV12N_SBWCL_32_10B:
		buf->addr[0][0] = start_raw;
		buf->addr[0][1] = SBWCL_32_CBCR_BASE(start_raw, ctx->img_width, ctx->img_height);
		break;
	case V4L2_PIX_FMT_NV12N_SBWCL_64_8B:
	case V4L2_PIX_FMT_NV12N_SBWCL_64_10B:
		buf->addr[0][0] = start_raw;
		buf->addr[0][1] = SBWCL_64_CBCR_BASE(start_raw, ctx->img_width, ctx->img_height);
		break;
	...
	for (i = 0; i < fmt->num_planes; i++)
		mfc_ctx_debug(2, "[MEMINFO] plane[%d] addr %#x\n", i, buf->addr[0][i]);
}
...
void mfc_dec_defer_src_update_timer(struct mfc_ctx *ctx)
{
	...
	mfc_ctx_debug(4, "[DEFER] src buffer: mod timer\n");
...
void mfc_dec_defer_dst_update_timer(struct mfc_ctx *ctx)
{
	...
	mfc_ctx_debug(4, "[DEFER] dst buffer: mod timer\n");
...
void mfc_dec_defer_disable_raw(struct mfc_ctx *ctx, int del_timer)
{
	...
	if (dec->defer_dec) {
		mfc_ctx_debug(2, "[DEFER] disable, del_timer %d\n", del_timer);
...
void mfc_dec_defer_update(struct mfc_core_ctx *core_ctx)
{
	...
	if ((num_inst == 1) && IS_SINGLE_MODE(ctx) && !ctx->boosting_time &&
		...
		UNDER_8K_RES(ctx) &&
		(mfc_rate_get_op_framerate(ctx) <= MFC_DEC_DEFER_MAX_FPS)) {
