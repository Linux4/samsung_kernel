...
static int __mfc_rate_get_bps_section_by_bps(struct mfc_dev *dev, int Kbps)
{
	int i;
	...
	for (i = 0; i < dev->pdata->num_mfc_freq; i++) {
		if (Kbps <= dev->bitrate_table[i].bps_interval) {
			mfc_dev_debug(3, "[BPS] MFC freq lv%d, %dKHz is needed\n",
		...
		struct mfc_ts_control *ts, struct timespec64 *time, int type)
{
	...
	if (ctx->dev->debugfs.debug_ts == 1) {
		...
		mfc_ctx_info("=================[%s%s%s][TS]===================\n",
				(type & 0x1) ? "SRC" : "DST", (type & 0x2) ? "_Q" : "",
				(type & 0x4) ? "_DQ" : "");
		mfc_ctx_info("[%s%s%s][TS] New timestamp = %ld.%06ld, count = %d\n",
			(type & 0x1) ? "SRC" : "DST",
			(type & 0x2) ? "_Q" : "",
			(type & 0x4) ? "_DQ" : "",
...
static int __mfc_rate_get_bps_section(struct mfc_ctx *ctx, u32 bytesused)
{
	...
	mfc_ctx_debug(3, "[BPS] %d Kbps, average %ld Kbits per frame\n", ctx->Kbps, avg_Kbits);

	return __mfc_rate_get_bps_section_by_bps(dev, ctx->Kbps);
}
...
static bool __mfc_rate_update_boost_mode(struct mfc_ctx *ctx, unsigned long framerate)
{
	...
	if (ctx->boosting_time && (ktime < ctx->boosting_time)) {
		mfc_ctx_debug(4, "[BOOST] seeking booster on-going %ld.%06ld until %ld.%06ld\n",
			(ktime / NSEC_PER_SEC),
	...
	if (!ctx->dst_q_ts.ts_is_full && !ctx->boosting_time) {
		...
		mfc_ctx_debug(2, "[BOOST] seeking booster start %ld.%06ld ~ %ld.%06ld (during %dms)\n",
			(ktime / NSEC_PER_SEC),
			(ktime - ((ktime / NSEC_PER_SEC) * NSEC_PER_SEC)) / NSEC_PER_USEC,
			(ctx->boosting_time / NSEC_PER_SEC),
			(ctx->boosting_time - ((ctx->boosting_time / NSEC_PER_SEC) * NSEC_PER_SEC))
	...
	if (ctx->boosting_time)
		mfc_ctx_debug(2, "[BOOST] seeking booster terminated %ld.%06ld\n",
			(ktime / NSEC_PER_SEC),
			(ktime - ((ktime / NSEC_PER_SEC) * NSEC_PER_SEC)) / NSEC_PER_USEC);
	...
	if (!ctx->src_ts.ts_is_full) {
		...
		if (ctx->rt == MFC_NON_RT) {
			if (framerate < DEC_DEFAULT_FPS) {
				...
				if (framerate != ctx->framerate)
					mfc_ctx_debug(2, "[QoS] max operating fps %ld\n", framerate);
			}
		}
...
int mfc_rate_check_perf_ctx(struct mfc_ctx *ctx, int max_runtime)
{
	...
	mfc_ctx_debug(2, "[PRIO][rt %d] st %ld.%06ld, curr %ld.%06ld, interval %lld, fps %d\n",
