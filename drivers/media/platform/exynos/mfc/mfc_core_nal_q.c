					...
					DecoderOutputStr *pOutStr, unsigned int index)
{
	...
	mfc_debug(2, "[NALQ][HDR+][META] copy metadata offset %#llx size: %d / %d\n",
		...
		EncoderInputStr *pInStr, unsigned int index)
{
	...
	if (enc->hdr10_plus_stat_gen) {
		...
		pInStr->SourceStatAddr = stat_info_buf_raw;
		mfc_debug(2, "[NALQ][MEMINFO][HDR+] stat info buf 0x%llx\n", stat_info_buf_raw);
	} else {
		pInStr->HevcNalControl &= ~(0x1 << 7);
			...
			DecoderInputStr *pInStr)
{
	...
	unsigned int need_cpb_buf_size;
	...
	mfc_debug(2, "[NALQ][STREAM] strm_size, %#x,%d, need_buf_size, %u, sg_size, %zu\n",
			strm_size, strm_size, need_cpb_buf_size, sg_size);
	...
	pInStr->CpbBufferSize = need_cpb_buf_size;
		...
		EncoderOutputStr *pOutStr, int index)
{
	...
	if (is_hdr10_plus_stat_info) {
		...
		mfc_debug(2, "[HDR+] stat info SEI size: %#x, SEI offset: %#x\n",
			...
			DecoderOutputStr *pOutStr)
{
	...
	mfc_core_qos_get_disp_ratio(ctx, pOutStr->DecodedFrameCnt, pOutStr->DisplayFrameCnt);
	qos_update = mfc_core_qos_mb_calculate(core, core_ctx, pOutStr->MfcProcessingCycle,
