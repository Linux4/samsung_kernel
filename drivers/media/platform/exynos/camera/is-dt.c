		...
		struct exynos_platform_is_module *pdata, struct is_sensor_cfg *cfg)
{
	...
	cfg->stat_sensor_mode = VC_SENSOR_MODE_INVALID;
	...
	if (opt_np) {
		...
		if (of_find_property(opt_np, "img_vc", NULL)) {
			idx_dt = 0;
			of_property_read_u32_index(opt_np, "img_vc", idx_dt++, &cfg->img_vc[CSI_OTF_OUT_LONG]);
			of_property_read_u32_index(opt_np, "img_vc", idx_dt++, &cfg->img_vc[CSI_OTF_OUT_SHORT]);
			cfg->set_vc = true;
		}

		if (of_find_property(opt_np, "af_vc", NULL)) {
			idx_dt = 0;
			of_property_read_u32_index(opt_np, "af_vc", idx_dt++, &cfg->af_vc[CSI_OTF_OUT_LONG]);
			of_property_read_u32_index(opt_np, "af_vc", idx_dt++, &cfg->af_vc[CSI_OTF_OUT_SHORT]);
		}

		if (of_find_property(opt_np, "stat_sensor_mode", NULL))
			of_property_read_u32(opt_np, "stat_sensor_mode", &cfg->stat_sensor_mode);
	}
}
