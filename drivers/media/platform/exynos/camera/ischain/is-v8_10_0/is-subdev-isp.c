...
void is_ischain_isp_in_calc_stripe_start_pos(struct is_crop *incrop,
	...
	u32 region_num, u32 h_pix_num[])
{
	...
	for (region_id = 0; region_id < region_num; ++region_id) {
		if (!region_id) {
			...
			/* Middle region */
			stripe_w = ALIGN((incrop->w * (region_id + 1) / region_num) - h_pix_num[region_id - 1], 4);
			stripe_w = ALIGN_UPDOWN_STRIPE_WIDTH(stripe_w);
		} else {
			/* Right region */
		...
		u32 bitwidth)
{
	...
	if (!region_id) {
		/* Left region */
		is_ischain_isp_in_calc_stripe_start_pos(incrop, stream,
			frame->stripe_info.region_num, frame->stripe_info.in.h_pix_num);
	...
	IS_DECLARE_PMAP(pmap))
{
	...
	if (!test_bit(IS_GROUP_OTF_INPUT, &group->state)) {
		...
		if ((hw_format == DMA_INOUT_FORMAT_BAYER_PACKED) && (flag_extra == COMP || flag_extra == COMP_LOSS)) {
			if (exynos_soc_info.main_rev >= 1) {
				u32 comp_format =
				(flag_extra == COMP) ? DMA_INOUT_FORMAT_BAYER_COMP : DMA_INOUT_FORMAT_BAYER_COMP_LOSSY;
