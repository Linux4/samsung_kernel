...
int blk_csis_qactive_control(bool on_off)
{
	void __iomem *csis_sys_regs;
	u32 csis_val = 0;
	int i = 0, ret = 0;
	...
	info("SYSREG_R_CSIS_LH_QACTIVE_CON:(0x%08X)\n", csis_val);
	is_hw_set_reg(csis_sys_regs,
...
int blk_ipp_mux_control(u32 value)
{
	...
	info("SYSREG_R_IPP_USER_CON1:(0x%08X)\n", ipp_val);
...
int blk_ipp_qactive_control(bool on_off)
{
	void __iomem *ipp_sys_regs;
	u32 ipp_val = 0;
	int i = 0, ret = 0;
...
int blk_tnr_lh_control(bool on_off)
{
...
int blk_dns_otfenable_control(u32 value)
{
	void __iomem *dns_sys_regs;
	u32 dns_val = 0;
	int ret = 0;
	...
	info("SYSREG_R_DNS_USER_CON1:(0x%08X)\n", dns_val);
...
int blk_itp_otfenable_control(u32 value)
{
	...
	info("SYSREG_R_ITP_USER_CON:(0x%08X)\n", itp_val);
...
int blk_mcsc_otfenable_control(u32 value)
{
	...
	info("SYSREG_R_MCSC_USER_CON2:(0x%08X)\n", mcsc_val);
...
int is_hw_ischain_enable(struct is_core *core)
{
	...
	ret = blk_csis_qactive_control(1);
	if (!ret)
		err("blk_csis_qactive_control is failed (%d)\n", ret);

	ret = blk_tnr_lh_control(1);
	if (!ret)
		err("blk_tnr_lh_control is failed (%d)\n", ret);
...
int is_hw_ischain_disable(struct is_core *core)
{
	...
	ret = blk_csis_qactive_control(0);
	if (!ret)
		err("blk_csis_qactive_control is failed (%d)\n", ret);

	ret = blk_ipp_qactive_control(0);
	if (!ret)
		err("blk_ipp_qactive_control is failed (%d)\n", ret);

	ret = blk_tnr_lh_control(0);
	if (!ret)
		err("blk_tnr_lh_control is failed (%d)\n", ret);
