...
static struct is_clk is_clk_list[] = {
	/* CIS_CLK (MCLK) */
	REGISTER_CLK("GATE_U_CLKINV_MCLK0_QCH"),
	REGISTER_CLK("GATE_U_CLKINV_MCLK1_QCH"),
	REGISTER_CLK("GATE_U_CLKINV_MCLK2_QCH"),
	REGISTER_CLK("GATE_U_CLKINV_MCLK3_QCH"),
	REGISTER_CLK("GATE_U_CLKINV_MCLK4_QCH"),
	REGISTER_CLK("GATE_U_CLKINV_MCLK5_QCH"),
	...
	REGISTER_CLK("GATE_TNR_CMU_TNR_QCH"), /* TNR */
...
static void exynos9815_is_print_clk_reg(void)
{

}

static int exynos9815_is_print_clk(struct device *dev)
{
	...
	is_get_rate_dt(dev, "GATE_TNR_CMU_TNR_QCH"); /* TNR */
...
int exynos9815_is_dump_clk(struct device *dev)
{
	...
	is_dump_rate_dt(dev, "GATE_TNR_CMU_TNR_QCH"); /* TNR */
...
int exynos9815_is_clk_gate(u32 clk_gate_id, bool is_on)
{
	return 0;
}

int exynos9815_is_uart_gate(struct device *dev, bool mask)
{
	return 0;
}

int exynos9815_is_get_clk(struct device *dev)
{
	struct clk *clk;
	size_t i;
...
int exynos9815_is_cfg_clk(struct device *dev)
{
	return 0;
}

int exynos9815_is_clk_on(struct device *dev)
{
	int ret = 0;
	...
	is_enable(dev, "GATE_TNR_CMU_TNR_QCH"); /* TNR */
	...
	is_enable(dev, "GATE_CSISX6_QCH_DMA"); /* CSISX6 */

	if (is_get_debug_param(IS_DEBUG_PARAM_CLK) > 1)
		exynos9815_is_print_clk(dev);
...
int exynos9815_is_clk_off(struct device *dev)
{
	int ret = 0;
	struct exynos_platform_is *pdata;
	...
	is_disable(dev, "GATE_TNR_CMU_TNR_QCH"); /* TNR */
	...
	is_disable(dev, "GATE_CSISX6_QCH_DMA"); /* CSISX6 */
...
{
	exynos9815_is_clk_gate(clk_gate_id, is_on);
	return 0;
}
