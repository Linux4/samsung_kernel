...
static int exynos9815_is_csi_gate(struct device *dev, u32 instance, bool mask)
{
	int ret = 0;
...
int exynos9815_is_sensor_iclk_cfg(struct device *dev,
	u32 scenario,
	u32 channel)
{
	is_enable(dev, "GATE_MIPI_PHY_LINK_WRAP_QCH_CSIS0");
...
int exynos9815_is_sensor_iclk_on(struct device *dev,
	u32 scenario,
	u32 channel)
{
	int ret = 0;
	...
	switch (channel) {
	case 0:
		exynos9815_is_csi_gate(dev, 1, true);
		exynos9815_is_csi_gate(dev, 2, true);
		exynos9815_is_csi_gate(dev, 3, true);
		exynos9815_is_csi_gate(dev, 4, true);
		exynos9815_is_csi_gate(dev, 5, true);
		break;
	case 1:
		exynos9815_is_csi_gate(dev, 0, true);
		exynos9815_is_csi_gate(dev, 2, true);
		exynos9815_is_csi_gate(dev, 3, true);
		exynos9815_is_csi_gate(dev, 4, true);
		exynos9815_is_csi_gate(dev, 5, true);
...
int exynos9815_is_sensor_iclk_off(struct device *dev,
	u32 scenario,
	u32 channel)
{
	exynos9815_is_csi_gate(dev, channel, true);
...
int exynos9815_is_sensor_mclk_on(struct device *dev,
	u32 scenario,
	u32 channel,
	u32 freq)
{
	...
	pr_debug("%s(scenario : %d / ch : %d / freq : %d)\n", __func__, scenario, channel, freq);

	snprintf(clk_name, sizeof(clk_name), "DOUT_CLKCMU_CIS_CLK%d", channel);
	is_enable(dev, clk_name);
	is_set_rate(dev, clk_name, freq * 1000); /* freq unit: khz */

	snprintf(clk_name, sizeof(clk_name), "GATE_U_CLKINV_MCLK%d_QCH", channel);
	is_enable(dev, clk_name);

	return 0;
}

int exynos9815_is_sensor_mclk_off(struct device *dev,
		u32 scenario,
		u32 channel)
{
	char clk_name[30];
...
int exynos_is_sensor_iclk_cfg(struct device *dev,
	...
	u32 channel)
{
	exynos9815_is_sensor_iclk_cfg(dev, scenario, channel);
	return 0;
}
...
int exynos_is_sensor_iclk_on(struct device *dev,
	...
	u32 channel)
{
	exynos9815_is_sensor_iclk_on(dev, scenario, channel);
	return 0;
}
...
int exynos_is_sensor_iclk_off(struct device *dev,
	...
	u32 channel)
{
	exynos9815_is_sensor_iclk_off(dev, scenario, channel);
	return 0;
}
...
int exynos_is_sensor_mclk_on(struct device *dev,
	...
	u32 freq)
{
	exynos9815_is_sensor_mclk_on(dev, scenario, channel, freq);
	return 0;
}
...
int exynos_is_sensor_mclk_off(struct device *dev,
	...
	u32 channel)
{
	exynos9815_is_sensor_mclk_off(dev, scenario, channel);
	return 0;
}
