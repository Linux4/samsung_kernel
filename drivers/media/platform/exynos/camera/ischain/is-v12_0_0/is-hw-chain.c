...
int is_hw_get_capture_slot(struct is_frame *frame, dma_addr_t **taddr, u64 **taddr_k, u32 vid)
{
	...
	case IS_LVN_BYRP_HDR1:
		*taddr = frame->dva_byrp_hdr1;
		break;
	case IS_LVN_BYRP_RGBY_HIST:
		*taddr = frame->dva_byrp_rgby_hist;
		break;
	case IS_LVN_BYRP_THSTAT_AWB:
		*taddr = frame->dva_byrp_thstat_awb;
		break;
	/* RGBP */
	case IS_LVN_RGBP_HF:
	...
	/* SHRP */
	case IS_LVN_SHRP_HF:
		*taddr = frame->dva_shrp_hf;
		break;
	case IS_LVN_SHRP_SEG:
		*taddr = frame->dva_shrp_seg;
		break;
	/* MCSC */
	case IS_LVN_MCSC_P0:
	...
	case IS_LVN_SHRP_RTA_INFO:
		*taddr_k = frame->kva_shrp_rta_info;
		break;
	...
	bool reset)
{
	...
	case DEV_HW_MCFP:
		...
		break;
	case DEV_HW_DLFE:
		TADDR_COPY(dst, src, kva_dlfe_rta_info, reset);
		break;
	case DEV_HW_MCSC0:
...
void is_hw_chain_probe(void *core)
{
	...
	/* SHRP */
	is_shrp_video_probe(&i_video[PABLO_VIDEO_PROBE_SHRP], core, IS_VIDEO_SHRP, 0);
...
void is_hw_print_target_dva(struct is_frame *leader_frame, u32 instance)
{
	...
	for (i = 0; i < IS_MAX_PLANES; i++) {
		...
		IS_PRINT_TARGET_DVA(dva_cstat_sat[0]);
		IS_PRINT_TARGET_DVA(dva_cstat_sat[1]);
		IS_PRINT_TARGET_DVA(dva_cstat_cav[0]);
		IS_PRINT_TARGET_DVA(dva_cstat_cav[1]);
