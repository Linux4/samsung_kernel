	...
	*use_out_crop = (incrop->x == 0) ? 1 : 0;
	...
	if (frame) {
		...
		if (!region_id) {
			...
			temp_stripe_w = ldr_frame->stripe_info.in.h_pix_num[region_id] -
				frame->stripe_info.in.h_pix_num[region_id - 1] - incrop->x;

			/* Stripe x when crop offset x start in middle region */
			if (!frame->stripe_info.in.h_pix_num[region_id - 1] &&
				incrop->x < ldr_frame->stripe_info.in.h_pix_num[region_id]) {
			...
			frame->stripe_info.in.h_pix_num[region_id] =
				frame->stripe_info.in.h_pix_num[region_id - 1] + stripe_w;
			frame->stripe_info.in.h_pix_ratio = stripe_w * STRIPE_RATIO_PRECISION / incrop->w;
			if (*use_out_crop) {
		...
		*stripe_roi_end_pos_x = stripe_roi_end_x;
		...
		if (!region_id) {
			...
			if (*use_out_crop) {
				...
				/* Add horizontal DMA offset */
				if (x_flip)
					dma_offset = (otcrop->w - frame->stripe_info.out.h_pix_num[region_id - 1]
									- stripe_w) * fmt->bitsperpixel[0] / BITS_PER_BYTE;
				else
					dma_offset = frame->stripe_info.out.h_pix_num[region_id - 1]
								* fmt->bitsperpixel[0] / BITS_PER_BYTE;
			}
			frame->stripe_info.out.h_pix_num[region_id] =
				frame->stripe_info.out.h_pix_num[region_id - 1] + stripe_w;
		} else {
			/* Right region */
			if (*use_out_crop) {
				stripe_w = ALIGN_DOWN(GET_SCALED_SIZE(stripe_w, h_ratio), MCSC_WIDTH_ALIGN);
