	...
	int flag_capture, struct is_dvfs_scenario_param *param)
{
	...
	dbg_dvfs(1, "[DVFS] dual info (Rear: %dfps, Rear2: %dfps, Rear3: %dfps, Rear4: %dfps, Rear_TOF: %dfsp)\n",
		device,
	...
	dbg_dvfs(1, "[DVFS] (Front: %dfps, Front2: %dfps, Front3: %dfps, Front4: %dfps, Front_TOF: %dfsp, mode: %d)\n",
		device,
			...
			struct exynos_platform_is *pdata)
{
	...
	if (core->dual_info.mode == IS_DUAL_MODE_NOTHING ||
	    core->dual_info.mode == IS_DUAL_MODE_BYPASS) { /* SINGLE */
		u32 cphy = ids->pdata->use_cphy;
		u32 mipi_speed = cfg->mipi_speed;
		u32 lanes = cfg->lanes + 1;
		u32 votf = cfg->votf;
		u32 width = cfg->width;
		u32 height = cfg->height;
		u32 max_fps = cfg->max_fps;
		u32 ppc = IS_DVFS_OTF_PPC;
		u32 potf_bit = IS_DVFS_POTF_BIT;
		u32 bit = 10;
		u32 clk, clk_csis, clk_cstat;
		u32 lv, lv_csis = cfg->dvfs_lv_csis;
		...
		if (votf) {
			if (cphy)
				clk_csis = mipi_speed * 16 / 7 * lanes / potf_bit;
			else
				clk_csis = mipi_speed * lanes / potf_bit;

			clk_cstat = width * height * max_fps / ppc / MHZ;
			clk_csis = clk_csis >= clk_cstat ? clk_csis : clk_cstat;
		} else {
			if (cphy)
		...
		dbg_dvfs(1, "[DVFS] clk(csis:%d, cstat:%d) lv(%d)\n", device, clk_csis, clk_cstat, lv_csis);

		return lv_csis;
	}

	mwarn("[DVFS] csis lv is not detected.", device);
