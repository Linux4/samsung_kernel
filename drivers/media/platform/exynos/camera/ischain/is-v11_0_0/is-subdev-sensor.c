...
#define SENSOR_2EXP_MODE_SHORT_IMG_VC_MASK	1
#define SENSOR_2EXP_MODE_SHORT_PD_VC_MASK	3
		...
		struct is_subdev *subdev, int vc)
{
	...
	if (sensor->cfg->set_vc)
		mask = BIT_MASK(sensor->cfg->img_vc[CSI_OTF_OUT_SHORT])
			| BIT_MASK(sensor->cfg->af_vc[CSI_OTF_OUT_SHORT]);
	else
		mask = BIT_MASK(SENSOR_2EXP_MODE_SHORT_IMG_VC_MASK)
			| BIT_MASK(SENSOR_2EXP_MODE_SHORT_PD_VC_MASK);
