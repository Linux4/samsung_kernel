		...
		struct is_fmt *fmt)
{
	...
	if (!frame->stripe_info.region_id) {
		...
		frame->stripe_info.h_pix_num[region_id] = stripe_w;
	} else {
		/* Right region */
		stripe_w = incrop->w - frame->stripe_info.h_pix_num[region_id - 1];

		/* Add horizontal DMA offset */
		dma_offset = frame->stripe_info.h_pix_num[region_id - 1] - STRIPE_MARGIN_WIDTH;
