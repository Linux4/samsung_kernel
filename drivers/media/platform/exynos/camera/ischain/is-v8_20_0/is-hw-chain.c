...
int is_hw_camif_cfg(void *sensor_data)
{
	...
	if (sensor_open_cnt == 1) {
		if (ischain->group_3aa.id == GROUP_ID_3AA0)
			sel_ipp0_input = otf_info->csi_ch;
		else if (ischain->group_3aa.id == GROUP_ID_3AA1)
			sel_ipp1_input = otf_info->csi_ch;
		info("USE path CSIS(%d) - IPP(%d)\n", otf_info->csi_ch, ischain->group_3aa.id);
	}
	...
	if (IS_ENABLED(SECURE_CAMERA_FACE) && IS_ENABLED(PROTECT_SYSREG_IS)
		...
		if (!sensor_cfg) {
			merr("[CSI%d] sensor cfg is null", csi, otf_info->csi_ch);
			ret = -EINVAL;
			goto p_err;
		}
		...
		if (sensor_cfg->mipi_speed) {
			...
			merr("[CSI%d] mipi_speed is invalid", csi, otf_info->csi_ch);
			ret = -EINVAL;
			goto p_err;
		}

		ret = csi_hw_s_phy_set(csi->phy, sensor_cfg->lanes, sensor_cfg->mipi_speed, settle, otf_info->csi_ch,
				csi->use_cphy);
	}
p_err:
