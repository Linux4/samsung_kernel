...
static struct is_clk is_clk_list[] = {
	...
	REGISTER_CLK("GATE_DFTMUX_CMU_QCH_CIS_CLK7"),
	...
	REGISTER_CLK("DOUT_DIV_CLKCMU_CIS_CLK7"),
	...
	REGISTER_CLK("GATE_CSIS_PDP_QCH_PDP"), /* PDP */
	REGISTER_CLK("GATE_LH_AST_SI_OTF0_CSISCSTAT_QCH"), /* CSTAT0 */
	REGISTER_CLK("GATE_LH_AST_SI_OTF1_CSISCSTAT_QCH"), /* CSTAT1 */
	REGISTER_CLK("GATE_LH_AST_SI_OTF2_CSISCSTAT_QCH"), /* CSTAT2 */
	REGISTER_CLK("GATE_LH_AST_SI_OTF3_CSISCSTAT_QCH"), /* CSTAT3 */
	REGISTER_CLK("GATE_LME_QCH_0"), /* LME0 */
	REGISTER_CLK("GATE_LME_QCH_1"), /* LME1 */
	REGISTER_CLK("GATE_MCFP_QCH"), /* MCFP */
	REGISTER_CLK("GATE_YUVP_QCH"), /* YUVP */
...
#if 0//def CONFIG_CAMERA_VENDER_MCD //pamir bringup
		for (i = 0; i < 200; i++) {
...
static void exynos9925_is_print_clk_reg(void)
{

}

static int exynos9925_is_print_clk(struct device *dev)
{
	pr_info("PABLO-IS CLOCK DUMP\n");

	/* CIS_CLK (MCLK) */
	is_get_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK0");
	is_get_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK1");
	is_get_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK2");
	is_get_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK3");
	is_get_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK4");
	is_get_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK5");
	is_get_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK6");
	is_get_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK7");
	...
	is_get_rate_dt(dev, "GATE_MIPI_PHY_LINK_WRAP_QCH_CSIS6");

	is_get_rate_dt(dev, "GATE_CSIS_PDP_QCH_PDP"); /* PDP */
	/* is_get_rate_dt(dev, "GATE_ADD_TAA_QCH");  3AA */
	is_get_rate_dt(dev, "GATE_LME_QCH_0"); /* LME0 */
	is_get_rate_dt(dev, "GATE_LME_QCH_1"); /* LME0 */
	is_get_rate_dt(dev, "GATE_MCFP_QCH"); /* MCFP */
	is_get_rate_dt(dev, "GATE_YUVP_QCH"); /* YUVP */
	is_get_rate_dt(dev, "GATE_MCSC_QCH"); /* MCSC */
	/* is_get_rate_dt(dev, "GATE_GDC_QCH");  GDC */

	exynos9925_is_print_clk_reg();
...
int exynos9925_is_dump_clk(struct device *dev)
{
	cinfo("### PABLO-IS CLOCK DUMP ###\n");

	/* CIS_CLK (MCLK) */
	is_dump_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK0");
	is_dump_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK1");
	is_dump_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK2");
	is_dump_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK3");
	is_dump_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK4");
	is_dump_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK5");
	is_dump_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK6");
	is_dump_rate_dt(dev, "DOUT_DIV_CLKCMU_CIS_CLK7");
	...
	is_dump_rate_dt(dev, "GATE_MIPI_PHY_LINK_WRAP_QCH_CSIS6");

	is_dump_rate_dt(dev, "GATE_CSIS_PDP_QCH_PDP"); /* PDP */
	/* is_dump_rate_dt(dev, "GATE_ADD_TAA_QCH");  3AA */
	is_dump_rate_dt(dev, "GATE_LME_QCH_0"); /* LME0 */
	is_dump_rate_dt(dev, "GATE_LME_QCH_1"); /* LME1 */
	is_dump_rate_dt(dev, "GATE_MCFP_QCH"); /* MCFP */
	is_dump_rate_dt(dev, "GATE_YUVP_QCH"); /* YUVP */
...
int exynos9925_is_clk_gate(u32 clk_gate_id, bool is_on)
{
	return 0;
}

int exynos9925_is_uart_gate(struct device *dev, bool mask)
{
	return 0;
}

int exynos9925_is_get_clk(struct device *dev)
{
	struct clk *clk;
	size_t i;
...
int exynos9925_is_cfg_clk(struct device *dev)
{
	return 0;
}

int exynos9925_is_clk_on(struct device *dev)
{
	int ret = 0;
	...
	is_enable(dev, "GATE_CSIS_PDP_QCH_PDP"); /* PDP */
	/* is_enable(dev, "GATE_ADD_TAA_QCH");  3AA */
	is_enable(dev, "GATE_LME_QCH_0"); /* LME0 */
	is_enable(dev, "GATE_LME_QCH_1"); /* LME1 */
	is_enable(dev, "GATE_MCFP_QCH"); /* MCFP */
	is_enable(dev, "GATE_YUVP_QCH"); /* YUVP */
	is_enable(dev, "GATE_MCSC_QCH"); /* MCSC */
	/* is_enable(dev, "GATE_GDC_QCH");  GDC */

	if (is_get_debug_param(IS_DEBUG_PARAM_CLK) > 1)
		exynos9925_is_print_clk(dev);
...
int exynos9925_is_clk_off(struct device *dev)
{
	int ret = 0;
	struct exynos_platform_is *pdata;
	...
	is_disable(dev, "GATE_CSIS_PDP_QCH_PDP"); /* PDP */
	/* is_disable(dev, "GATE_ADD_TAA_QCH");  3AA */
	is_disable(dev, "GATE_LME_QCH_0"); /* LME0 */
	is_disable(dev, "GATE_LME_QCH_1"); /* LME1 */
	is_disable(dev, "GATE_MCFP_QCH"); /* MCFP */
	is_disable(dev, "GATE_YUVP_QCH"); /* YUVP */
 ...
 */
int exynos9925_is_clk_get_rate(struct device *dev)
{
	size_t index;
...
{
	exynos9925_is_clk_gate(clk_gate_id, is_on);
	return 0;
}
...
void is_get_clk_ops(struct exynos_platform_is *pdata)
{
	...
	pdata->clk_on_off = exynos_is_clk_gate;
}
