	...
	struct camera2_node *node)
{
	...
	otparm.x = 0;
	otparm.y = 0;
	otparm.w = taa_param->lmeds_output.width;
	otparm.h = taa_param->lmeds_output.height;
	...
	if (node->request) {
		...
		ldr_frame->txlTargetAddress[0] = 0;
		ldr_frame->txlTargetAddress[1] = 0;
		ldr_frame->txlTargetAddress[2] = 0;
		node->request = 0;
	}
