	...
	struct camera2_node *node)
{
	...
	otparm.x = 0;
	otparm.y = 0;
	otparm.w = taa_param->orbds_output.width;
	otparm.h = taa_param->orbds_output.height;
	...
	if (node->request) {
		...
		ldr_frame->txoTargetAddress[0] = 0;
		ldr_frame->txoTargetAddress[1] = 0;
		ldr_frame->txoTargetAddress[2] = 0;
		node->request = 0;
	}
