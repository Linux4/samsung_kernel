	...
	int hw_id, struct platform_device *pdev, struct is_hardware *hardware)
{
	...
	lib_support = is_get_lib_support();
	...
	if (!lib_support->minfo) {
		...
		info_itfc("[ID:%2d] kvaddr for taaisp: 0x%lx\n", hw_id,
			CALL_BUFOP(lib_support->minfo->pb_taaisp, kvaddr,
					lib_support->minfo->pb_taaisp));
#endif
	}
	...
	int hw_id, struct platform_device *pdev, struct is_hardware *hardware)
{
	...
	lib_support = is_get_lib_support();
	for (i = 0; i < INTR_HWIP_MAX; i++) {
		itf_byrp->handler[i].valid = false;

		/* TODO: this is not cool */
		lib_support->intr_handler_taaisp[handler_id][i]
			= (struct hwip_intr_handler *)&itf_byrp->handler[i];
	}
	...
	set_bit(IS_CHAIN_IF_STATE_INIT, &itf_byrp->state);
	...
	int hw_id, struct platform_device *pdev, struct is_hardware *hardware)
{
	...
	lib_support = is_get_lib_support();
	for (i = 0; i < INTR_HWIP_MAX; i++) {
		itf_rgbp->handler[i].valid = false;

		/* TODO: this is not cool */
		lib_support->intr_handler_taaisp[handler_id][i]
			= (struct hwip_intr_handler *)&itf_rgbp->handler[i];
	}
	...
	set_bit(IS_CHAIN_IF_STATE_INIT, &itf_rgbp->state);
	...
	int hw_id, struct platform_device *pdev, struct is_hardware *hardware)
{
	...
	for (i = 0; i < INTR_HWIP_MAX; i++)
		itf_mcfp->handler[i].valid = false;
	...
	set_bit(IS_CHAIN_IF_STATE_INIT, &itf_mcfp->state);
	...
	int hw_id, struct platform_device *pdev, struct is_hardware *hardware)
{
	...
	lib_support = is_get_lib_support();
	for (i = 0; i < INTR_HWIP_MAX; i++) {
		itf_orbmch->handler[i].valid = false;

		lib_support->intr_handler_taaisp[handler_id][i]
			= (struct hwip_intr_handler *)&itf_orbmch->handler[i];
	}
	if (!lib_support->minfo) {
		lib_support->itfc		= itfc;
		lib_support->minfo		= itfc->minfo;
	...
	set_bit(IS_CHAIN_IF_STATE_INIT, &itf_orbmch->state);
