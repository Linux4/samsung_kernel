...
skip_get_module:
	...
	if (!im->loaded_setfile[position]) {
		...
		if (module->setfile_preload) {
			...
			minfo("preloaded(%ldus) %s is ready to use, size: 0x%zx\n", device,
			...
			ret = is_hardware_parse_setfile(device->hardware, (ulong)setfile->data, position);
			if (ret) {
			...
			minfo("reloaded(%ldus) %s is ready to use, size: 0x%zx\n", device,
							PABLO_KTIME_US_DELTA_NOW(time_to_ready),
		...
		minfo("pinned(%ldus) %s is ready to use\n", device,
					PABLO_KTIME_US_DELTA_NOW(time_to_ready),
					im->name_setfile[position]);
	}
	...
	struct is_vender *vender)
{
	...
	ret = is_itf_setfile_wrap(itf, device);
	if (ret)
		goto p_err;
...
static int is_itf_init_process_start(struct is_device_ischain *device)
{
	...
	gnext = get_ischain_leader_group(device);

	while (gnext) {
		ret = is_itf_process_on_wrap(device, gnext);
		if (ret)
			return ret;
...
{
	int ret;
	struct is_group *leader, *gnext;
	u64 grp_map;

	FIMC_BUG(!device);

	leader = gnext = get_ischain_leader_group(device);
	grp_map = is_itf_g_logical_group_id(device);

	while (gnext) {
		ret = is_itf_process_stop(device, gnext, 0);
		if (ret) {
	...
	ret = is_itf_a_param(device, grp_map);
	if (ret) {
	...
	gnext = leader;
	while (gnext) {
		ret = is_itf_process_start(device, gnext);
		if (ret) {
