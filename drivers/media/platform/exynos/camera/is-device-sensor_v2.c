...
static inline bool is_sensor_cmp_mode(struct is_sensor_cfg *ss_cfg, struct is_sensor_mode *mode)
{
	...
	if (mode->hw_format && ss_cfg->output[CSI_VIRTUAL_CH_0].hwformat != mode->hw_format)
		return false;
...
KUNIT_EXPORT_SYMBOL(is_sensor_get_mono_mode);
	...
	struct camera2_node *ldr_node)
{
	...
	if (test_bit(IS_SENSOR_SWITCHING, &device->seamless_state)) {
		u32 otf_out_num;

		if (test_bit(IS_SENSOR_SINGLE_MODE, &device->seamless_state))
			otf_out_num = 2;
		else if (test_bit(IS_SENSOR_2EXP_MODE, &device->seamless_state))
			otf_out_num = 1;
...
static int is_sensor_back_start(void *device, struct is_queue *iq)
{
	...
	for (vc = CSI_VIRTUAL_CH_0; vc < CSI_VIRTUAL_CH_MAX; vc++) {
		...
		if (test_bit(IS_SUBDEV_INTERNAL_USE, &dma_subdev->state)
			...
			if (ret) {
				merr("[CSI%d] VC%d subdev internal start is fail(%d)", csi,
			...
			if (icpu_dev) {
				...
				for (frame_idx = 0; frame_idx < framemgr->num_frames; frame_idx++) {
					...
					if (ret) {
						merr("failed to attach_ext(%d)", dma_subdev, ret);
						goto err_attach;
					}
				}
			}
