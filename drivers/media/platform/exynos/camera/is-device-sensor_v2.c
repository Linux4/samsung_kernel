	...
	u32 position, struct is_module_enum **module)
{
	...
	*module = NULL;
	...
	if (position >= SENSOR_POSITION_MAX) {
		merr("invalid position (%d)", device, position);
		ret = -EINVAL;
		goto p_err;
	}
...
struct is_sensor_cfg *is_sensor_g_mode(struct is_device_sensor *device)
{
	...
	minfo("try to find sensor mode(%dx%d@%d) ex_mode(%d:%d) hw_format(%x)\n", device,
		width,
		height,
		framerate,
		ex_mode,
...
#ifdef ENABLE_DTP
	if (device->dtp_check) {
#if defined(CAMERA_DTP_TIMER_MS)
		mod_timer(&device->dtp_timer, jiffies + msecs_to_jiffies(CAMERA_DTP_TIMER_MS));
#else
...
static int is_sensor_s_fmt(void *device, struct is_queue *iq)
{
	...
	if (ids->cfg->votf && !test_bit(IS_SENSOR_STAND_ALONE, &ids->state)) {
	...
	u32 nonblock)
{
	...
	if (IS_ENABLED(ENABLE_DVFS) &&
	    test_bit(IS_SENSOR_STAND_ALONE, &device->state)) {
