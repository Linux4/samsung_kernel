				 ...
				 struct v4l2_ext_controls *ctrls)
{
	...
	ret = is_vender_ssx_video_s_ext_ctrl(ctrls, device);
	if (ret) {
		merr("is_vender_ssx_video_s_ext_ctrl is fail(%d)", device, ret);
		goto p_err;
	}
				    ...
				    struct v4l2_ext_controls *ctrls)
{
	...
	ret = is_vender_ssx_video_g_ext_ctrl(ctrls, device);
	if (!ret)
		return 0;
