...
#include "hardware/sfr/is-sfr-pdp-v6_0.h"
#include "is-config.h"
	...
	u32 hwformat)
{
	...
	val = PDP_GET_F_COREX(ctx->test_addr, PDP_R_Y_REORDER_LINEBUF_SIZE,
			PDP_F_Y_REORDER0_LINEBUF_SIZE); /* 0 fix */
	KUNIT_EXPECT_EQ(test, val, (u32)0);
}

static void pablo_pdp_check_reorder_cfg(struct kunit *test, bool enable, u32 hwformat)
{
	struct pablo_hw_api_pdp_test_ctx *ctx = test->priv;
	u32 val1, val2;
	u32 ppc_val, gain_shift;
	...
	val1 = PDP_GET_F_COREX(ctx->test_addr, PDP_R_Y_REORDER_BLC_GAIN_SHIFT, PDP_F_Y_REORDER0_BLC_GAIN_SHIFT);
	KUNIT_EXPECT_EQ(test, val1, gain_shift);
}
...
static void pablo_hw_api_pdp_hw_s_core_kunit_test(struct kunit *test)
{
	...
	num_buffers = 8;
	enable = true;
	cfg->input[pdp->af_vc].extformat = POTF_EN(pd_hwformat);
	...
	pablo_pdp_check_reorder_cfg(test, enable, pd_hwformat);
