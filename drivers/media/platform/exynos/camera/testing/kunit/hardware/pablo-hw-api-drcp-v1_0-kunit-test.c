...
static void pablo_hw_api_drcp_hw_is_occurred_kunit_test(struct kunit *test)
{
	unsigned int occurred;
	unsigned int status = 0;
	enum yuvp_event_type type = INTR_FRAME_START;

	occurred = drcp_hw_is_occurred(status, type);
	KUNIT_EXPECT_EQ(test, occurred, (unsigned int)0);
}

static void pablo_hw_api_drcp_hw_is_occurred1_kunit_test(struct kunit *test)
{
	unsigned int occurred;
	unsigned int status = 0;
	enum yuvp_event_type type = INTR_FRAME_START;

	occurred = drcp_hw_is_occurred1(status, type);
	KUNIT_EXPECT_EQ(test, occurred, (unsigned int)0);
}
...
static struct kunit_case pablo_hw_api_drcp_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_api_drcp_hw_is_occurred_kunit_test),
	KUNIT_CASE(pablo_hw_api_drcp_hw_is_occurred1_kunit_test),
