...
static void pablo_hw_rgbp_handle_interrupt0_kunit_test(struct kunit *test)
{
	...
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, itfc->itf_ip[12].handler[INTR_HWIP1].handler);

	/* not opened */
	ret = itfc->itf_ip[12].handler[INTR_HWIP1].handler(0, hw_ip);
	KUNIT_EXPECT_EQ(test, ret, 0);
...
static void pablo_hw_rgbp_handle_interrupt1_kunit_test(struct kunit *test)
{
	...
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, itfc->itf_ip[12].handler[INTR_HWIP2].handler);

	/* not opened */
	ret = itfc->itf_ip[12].handler[INTR_HWIP2].handler(0, hw_ip);
	KUNIT_EXPECT_EQ(test, ret, 0);
