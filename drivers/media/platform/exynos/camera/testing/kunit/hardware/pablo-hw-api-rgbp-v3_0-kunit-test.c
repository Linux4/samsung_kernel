...
static void pablo_hw_api_rgbp_hw_s_core_kunit_test(struct kunit *test)
{
	...
	/* rgbp_hw_s_cin_fifo */
	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_IP_USE_OTF_PATH_01, RGBP_F_IP_USE_OTF_IN_FOR_PATH_0);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
...
{
	struct param_otf_output param;
	u32 set_val, expected_val;
	u32 set_id = COREX_DIRECT;

	param.cmd = 1;
	param.destination= 1;
	rgbp_hw_s_cout_fifo(test_ctx.pmio, set_id, &param);

	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_IP_USE_OTF_PATH_01, RGBP_F_IP_USE_OTF_OUT_FOR_PATH_0);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
...
static void pablo_hw_api_rgbp_hw_s_wdma_addr_kunit_test(struct kunit *test)
{
	...
	ret = rgbp_hw_s_wdma_addr(&test_ctx.dma, &addr, plane,
			num_buffers, buf_idx, comp_sbwc_en, payload_size, out_yuv);
	KUNIT_EXPECT_EQ(test, ret, 0);
}
...
static void pablo_hw_api_rgbp_hw_s_block_crc_kunit_test(struct kunit *test)
{
	...
	for (idx = 0; idx < ARRAY_SIZE(crc_reg_list); idx++) {
		val = RGBP_GET_F(test_ctx.pmio, crc_reg_list[idx], crc_field_list[idx]);
		KUNIT_EXPECT_EQ(test, val, seed);
	}
}
...
{
	u32 in_width = 320, out_width = 320;
	u32 in_height = 240, out_height = 240;
	u32 val;

	rgbp_hw_s_crop(test_ctx.pmio, in_width, in_height, out_width, out_height);

	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_RGB_DMSCCROP_SIZE, RGBP_F_RGB_DMSCCROP_SIZE_X);
	KUNIT_EXPECT_EQ(test, val, in_width);
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_RGB_DMSCCROP_SIZE, RGBP_F_RGB_DMSCCROP_SIZE_Y);
	KUNIT_EXPECT_EQ(test, val, in_height);
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_YUV_SCCROP_SIZE, RGBP_F_YUV_SCCROP_SIZE_X);
	KUNIT_EXPECT_EQ(test, val, out_width);
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_YUV_SCCROP_SIZE, RGBP_F_YUV_SCCROP_SIZE_Y);
	KUNIT_EXPECT_EQ(test, val, out_height);
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_STAT_SATFLGCROP_SIZE, RGBP_F_STAT_SATFLGCROP_SIZE_X);
	KUNIT_EXPECT_EQ(test, val, out_width);
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_STAT_SATFLGCROP_SIZE, RGBP_F_STAT_SATFLGCROP_SIZE_Y);
	KUNIT_EXPECT_EQ(test, val, out_height);
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_STAT_DECOMPCROP_SIZE, RGBP_F_STAT_DECOMPCROP_SIZE_X);
	KUNIT_EXPECT_EQ(test, val, out_width);
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_STAT_DECOMPCROP_SIZE, RGBP_F_STAT_DECOMPCROP_SIZE_Y);
	KUNIT_EXPECT_EQ(test, val, out_height);
}
...
static void pablo_hw_api_rgbp_hw_s_dma_init_kunit_test(struct kunit *test)
{
	...
	ret = rgbp_hw_s_wdma_init(hw_ip, &test_ctx.dma, &test_ctx.param_set,
			instance, enable, in_crop_size_x, cache_hint, &sbwc_en, &payload_size, out_yuv);
	KUNIT_EXPECT_EQ(test, ret, 1);
}
