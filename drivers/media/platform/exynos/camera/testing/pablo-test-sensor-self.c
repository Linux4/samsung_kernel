...
static int pablo_test_set_phy_tune(const char *val, const struct kernel_param *kp)
{
	...
	u32 num_crc_code, crc_test_time;
	...
	if (pre_arg_num == 1) {
		...
		num_crc_code = 48;
		crc_test_time = 100;
	} else if (pre_arg_num == 3) {
		ret = kstrtouint(argv[arg_idx++], 0, &num_crc_code);
		if (ret) {
			err("Invalid num_crc_code %d ret %d", num_crc_code, ret);
			goto func_exit;
		}

		ret = kstrtouint(argv[arg_idx++], 0, &crc_test_time);
		if (ret) {
			err("Invalid crc_test_time %d ret %d", crc_test_time, ret);
			goto func_exit;
		}
	} else {
	...
	if (!num_crc_code) {
		...
		crc_code_sweep(num_crc_code, crc_test_time, csi, sd_csi, sd_mod, regs);
	}
...
int test_phy_margin_check(char *buffer, int cam_index)
{
	...
	strcpy(phy_tune_args, "48 100 0"); // default phy tune command
