...
{
	return votfdev[dev_id];
}
KUNIT_EXPORT_SYMBOL(get_votf_dev);

void set_votf_dev(u32 dev_id, struct votf_dev *dev)
{
	votfdev[dev_id] = dev;
}
KUNIT_EXPORT_SYMBOL(set_votf_dev);
#endif
...
static struct votf_dev *get_votf_dev_by_ip(struct votf_info *vinfo)
{
	...
	for (dev_id = 0; dev_id < VOTF_DEV_NUM && votfdev[dev_id] && !votf; dev_id++) {
...
static u32 get_offset(struct votf_info *vinfo,
...
static void _votfitf_init(struct votf_dev *votf)
{
	int ip, id;
...
int votfitf_create_link(int src_ip, int dst_ip)
{
	...
	if (!camerapp_check_votf_ring(votf->votf_addr[dst_ip_idx], dst_module)) {
...
static void parse_votf_table(struct votf_dev *votf, struct device_node *np)
{
	...
	for_each_child_of_node(np, table_np) {
		of_property_read_u32_index(table_np, "info", idx++, &temp.use);
		...
		for (id_num = 0; id_num < id_cnt; id_num++) {
			...
			votf->votf_table[service][ip_num][id_num].use = temp.use;
