...
static void pablo_mem_sync(struct is_priv_buf *pbuf, off_t offset, size_t size,
		struct device *dev, enum dma_data_direction dir, enum dma_sync_target target, unsigned int secure)
{
	int i;
	...
	if (secure) {
		device_initialize(&dummy_dev);
		paddr = (dma_addr_t *)sg_phys(pbuf->sgt->sgl);
	}
		...
		off_t offset, size_t size, enum dma_data_direction dir)
{
	...
	if (pbuf) {
		...
		pablo_mem_sync(pbuf, offset, size, iic->dev, dir, SYNC_FOR_DEVICE, 0);
	}
}
		...
		off_t offset, size_t size, enum dma_data_direction dir)
{
	...
	if (pbuf) {
		iic = (struct is_ion_ctx *)(pbuf->ctx);
		pablo_mem_sync(pbuf, offset, size, iic->dev, dir, SYNC_FOR_CPU, 0);
	}
}
		...
		off_t offset, size_t size, enum dma_data_direction dir)
{
	...
	if (pbuf) {
		...
		pablo_mem_sync(pbuf, offset, size, pdhc->dev, dir, SYNC_FOR_DEVICE, 0);
	}
}
		...
		off_t offset, size_t size, enum dma_data_direction dir)
{
	...
	if (pbuf) {
		pdhc = (struct pablo_dmabuf_heap_ctx *)(pbuf->ctx);
		pablo_mem_sync(pbuf, offset, size, pdhc->dev, dir, SYNC_FOR_CPU, 0);
	}
}

static void pablo_dmabuf_heap_sync_for_secure(struct is_priv_buf *pbuf,
		off_t offset, size_t size, enum dma_data_direction dir)
{
	struct pablo_dmabuf_heap_ctx *pdhc;
	...
	if (pbuf) {
		pdhc = (struct pablo_dmabuf_heap_ctx *)(pbuf->ctx);
		pablo_mem_sync(pbuf, offset, size, pdhc->dev, dir, SYNC_FOR_CPU, 1);
	}
}
...
const struct is_priv_buf_ops pablo_priv_buf_ops_dmabuf_heap = {
	...
	.sync_for_secure	= pablo_dmabuf_heap_sync_for_secure,
};
...
static void *pablo_dmabuf_heap_init(struct platform_device *pdev)
{
	...
	pdhc->dh_secure_camera_contig = dma_heap_find(SECURE_CAMERA_CONTIG_NAME);
	if (!pdhc->dh_secure_camera_contig)
		probe_warn("There's no secure camera contig reserved memory");
	pdhc->dh_secure_camera_heap = dma_heap_find(SECURE_CAMERA_HEAP_NAME);
	if (!pdhc->dh_secure_camera_heap)
		probe_warn("There's no secure camera heap reserved memory");
...
static void pablo_dmabuf_heap_cleanup(void *ctx)
{
	...
	if (pdhc->dh_secure_camera_contig)
		dma_heap_put(pdhc->dh_secure_camera_contig);
	if (pdhc->dh_secure_camera_heap)
		dma_heap_put(pdhc->dh_secure_camera_heap);
...
static struct dma_heap *pablo_dmabuf_heap_find(void *ctx, const char *heapname)
{
	...
	else if (!strncmp(heapname, SECURE_CAMERA_CONTIG_NAME, strlen(SECURE_CAMERA_CONTIG_NAME)))
		return pdhc->dh_secure_camera_contig;
	else if (!strncmp(heapname, SECURE_CAMERA_HEAP_NAME, strlen(SECURE_CAMERA_HEAP_NAME)))
		return pdhc->dh_secure_camera_heap;
	else
		return NULL;
}
