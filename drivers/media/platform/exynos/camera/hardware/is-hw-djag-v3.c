...
/* Read the djag_en from setfile */
	...
	if (!djag_en || param->input.width > out_width || param->input.height > out_height) {
		...
		is_scaler_set_djag_strip_config(get_base(hw_ip), output_id, 0, 0);
#endif
		return ret;
	}
	...
	is_scaler_set_djag_input_size(get_base(hw_ip), in_width, in_height);
	is_scaler_set_djag_src_size(get_base(hw_ip), src_crop.x, src_crop.y, src_crop.w, src_crop.h);
	is_scaler_set_djag_scaling_ratio(get_base(hw_ip), hratio, vratio);
	is_scaler_set_djag_init_phase_offset(get_base(hw_ip), 0, 0);
	is_scaler_set_djag_round_mode(get_base(hw_ip), 1);
...
#endif
	is_scaler_set_djag_tuning_param(get_base(hw_ip), djag_tuneset);
#if defined(USE_YUV_RANGE_BY_ISP)
...
#endif
	is_scaler_set_djag_dither_wb(get_base(hw_ip), djag_wb, white, black);
	is_scaler_set_djag_enable(get_base(hw_ip), djag_en);

#ifdef USE_MCSC_STRIP_OUT_CROP
	if (djag_en && use_out_crop) {
		is_scaler_set_djag_strip_enable(get_base(hw_ip), output_id, 1);
		is_scaler_set_djag_strip_config(get_base(hw_ip), output_id, temp_stripe_pre_dst_x, temp_stripe_start_pos_x);
	...
	is_scaler_set_djag_dst_size(get_base(hw_ip), out_width, out_height);
