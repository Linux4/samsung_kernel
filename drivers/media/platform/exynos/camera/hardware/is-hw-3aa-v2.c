	...
	struct is_group *group)
{
	...
	if (!atomic_read(&hardware->slot_rsccount[group->slot])) {
	...
	u32 instance)
{
	...
	head = GET_HEAD_GROUP_IN_DEVICE(IS_DEVICE_ISCHAIN, hw_ip->group[instance]);
	if (test_bit(IS_GROUP_OTF_INPUT, &head->state))
		config.int_row_cord = param_set->otf_input.height / 2;
	else
	...
	u32 next_id, struct is_hardware *hardware)
{
	...
	msinfo_hw("change_chain done (state: curr(0x%lx) next(0x%lx))\n", instance, hw_ip,
