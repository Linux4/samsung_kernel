	...
	if (path == OTF || en_votf) {
		...
		if (pdp->vc_mux_base) {
			if (sensor_cfg->set_vc) {
				pdp->img_vc = sensor_cfg->img_vc[pdp->mux_offset] % MAX_NUM_INPUT_OTF_VC;
				pdp->af_vc = sensor_cfg->af_vc[pdp->mux_offset] % MAX_NUM_INPUT_OTF_VC;
				pdp_hw_g_input_mux_val(&input_mux_val, pdp->img_vc, pdp->af_vc);
			} else {
				vc_mux_id = (pdp->vc_mux_elems * pdp->mux_offset) + pdp->csi_ch;
				input_mux_val = pdp->vc_mux_val[vc_mux_id];
				pdp_hw_g_input_vc(input_mux_val, &pdp->img_vc, &pdp->af_vc);
			}
			...
			bool flag, u32 f_type)
{
	...
	if (!test_bit(IS_ISCHAIN_REPROCESSING, &device->state)) {
		...
		if (hw_ip->frame_type == LIB_FRAME_HDR_SHORT && pdp->ctx_num > 1)
		...
		vc_mux_id = (pdp->vc_mux_elems * pdp->mux_offset) + pdp->csi_ch;
		pdp_hw_g_input_vc(pdp->vc_mux_val[vc_mux_id], &pdp->img_vc, &pdp->af_vc);
