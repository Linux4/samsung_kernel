...
int lme_hw_s_wdma_addr(void __iomem *base, dma_addr_t *addr, u32 id, u32 set_id)
{
	...
	case LME_WDMA_MV_OUT:
		/* TOOD: Need to fix support corex */
		LME_SET_R(base + GET_LME_COREX_OFFSET(set_id),
				LME_R_DMACLIENT_SPS_MV_OUT_GEOM_BASE_ADDR_0, addr[0]);
		break;
	case LME_WDMA_MBMV_OUT:
		/* TOOD: Need to fix support corex */
		LME_SET_R(base + GET_LME_COREX_OFFSET(set_id),
				LME_R_DMACLIENT_MBMV_OUT_GEOM_BASE_ADDR_0, addr[0]);
		...
		LME_SET_R(base + GET_LME_COREX_OFFSET(set_id),
				LME_R_DMACLIENT_MBMV_OUT_GEOM_BASE_ADDR_1, addr[1]);
		break;
	case LME_WDMA_SAD_OUT:
		/* TOOD: Need to fix support corex */
		LME_SET_R(base + GET_LME_COREX_OFFSET(set_id),
				LME_R_DMACLIENT_SAD_OUT_GEOM_BASE_ADDR_0, addr[0]);
		break;
	default:
		err_hw("[LME] invalid dma_id[%d]", id);
		ret = SET_ERROR;
