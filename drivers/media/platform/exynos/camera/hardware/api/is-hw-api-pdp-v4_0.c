...
int pdp_hw_get_bpc_reorder_sram_size(u32 width, int vc_sensor_mode)
{
	...
	case VC_SENSOR_MODE_IMX_2X2OCL_1_FULL_VPD:
		size = width;
		break;
	...
	case VC_SENSOR_MODE_IMX_2X2OCL_1_HYBRID_VPD:
		size = width / 2;
		break;
	default:
...
{
	*mux_val = (img_vc << csis_top_fields[CSIS_TOP_F_MUX_IMG_VC_PDP0].bit_start) |
		   (af_vc << csis_top_fields[CSIS_TOP_F_MUX_AF0_VC_PDP0].bit_start);
}
