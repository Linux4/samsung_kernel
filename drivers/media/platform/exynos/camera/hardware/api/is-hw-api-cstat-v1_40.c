...
#define CSTAT_MIN_CLK			200000000UL
...
void cstat_hw_s_dma_cfg(struct cstat_param_set *p_set, struct is_cstat_config *conf)
{
	...
	if (p_set->pre.cmd) {
		if(p_set->mono_mode)
			p_set->pre.width = conf->pre_grid_w * 8;
		else
			p_set->pre.width = conf->pre_grid_w * 3 * 8;
	...
	if (p_set->awb.cmd) {
		if(p_set->mono_mode)
			p_set->awb.width = conf->awb_grid_w * 8;
		else
	...
	if (p_set->ae.cmd) {
		if(p_set->mono_mode)
			p_set->ae.width = conf->ae_grid_w * 8;
		else
...
{
	CSTAT_SET_F(base, CSTAT_R_BYR_SBPC_CONFIG0, CSTAT_F_BYR_SBPC_MONO_MODE_EN, enable);
	if (enable)
		CSTAT_SET_F(base, CSTAT_R_BYR_SBPC_CONFIG1, CSTAT_F_BYR_SBPC_COLOR_NORM_EN, 0);
	CSTAT_SET_F(base, CSTAT_R_BYR_BLC_CONFIG, CSTAT_F_BYR_BLC_MONO_MODE_EN, enable);
	CSTAT_SET_F(base, CSTAT_R_BYR_THSTATPRE_ENABLE_CONFIG, CSTAT_F_BYR_THSTATPRE_MONO_MODE_EN, enable);
	CSTAT_SET_F(base, CSTAT_R_BYR_CGRAS_CONFIG, CSTAT_F_BYR_CGRAS_MONO_MODE_EN, enable);
	CSTAT_SET_F(base, CSTAT_R_BYR_THSTATAWB_ENABLE_CONFIG, CSTAT_F_BYR_THSTATAWB_MONO_MODE_EN, enable);
	CSTAT_SET_F(base, CSTAT_R_BYR_THSTATAE_ENABLE_CONFIG, CSTAT_F_BYR_THSTATAE_MONO_MODE_EN, enable);
	CSTAT_SET_F(base, CSTAT_R_BYR_RGBYHIST_MONO_MODE_EN, CSTAT_F_BYR_RGBYHIST_MONO_MODE_EN, enable);
	CSTAT_SET_F(base, CSTAT_R_BYR_CDAF_AUTO_FOCUS_STATISTICS_CONFIG, CSTAT_F_BYR_CDAF_MONO_MODE_EN, enable);
}
