...
static int is_hw_dma_set_format(struct is_common_dma *dma, u32 format, u32 format_type)
{
	...
	case DMA_FMT_BAYER:
		if (test_bit(format, &dma->available_bayer_format_map)) {
			...
			err_hw("invalid format_type[%d] fomat:[%d] available:[%lx]",
...
static int is_hw_dma_set_cache_32b_pa(struct is_common_dma *dma, u32 en)
{
	void __iomem *base = __is_hw_dma_get_base_addr(dma);

	SET_F(base, DMA_R_CACHE_CONTROL, DMA_F_32B_PARTIAL_ALLOC_CONTROL, en);
...
static const struct is_common_dma_ops is_hw_dma_ops = {
	...
	.dma_set_cache_32b_pa		= is_hw_dma_set_cache_32b_pa,
...
static int pmio_dma_set_cache_32b_pa(struct is_common_dma *dma, u32 en)
{
	void __iomem *base = __pmio_dma_get_base_addr(dma);
	u32 val;
	unsigned int corex_offset = GET_COREX_OFFSET(dma->set_id);

	val = PMIO_GET_R(base, corex_offset + dma->reg_ofs + dma_regs[DMA_R_CACHE_CONTROL].sfr_offset);
	val = SET_V(val, DMA_F_32B_PARTIAL_ALLOC_CONTROL, en);
	PMIO_SET_R(base, corex_offset + dma->reg_ofs + dma_regs[DMA_R_CACHE_CONTROL].sfr_offset, val);
	...
	u32 block_width, u32 block_height)
{
	...
	if (comp_sbwc_en == COMP) {
		...
		if (block_width == 256 && block_height == 1) {
			...
			err_hw("Invalid sbwc block width(%d) height(%d)", block_width, block_height);
			return 0;
		}
