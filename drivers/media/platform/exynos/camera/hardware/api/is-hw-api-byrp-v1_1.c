...
static void byrp_hw_s_path(void __iomem *base, u32 set_id, bool mpc_en)
{
	u32 val = 0;
...
int byrp_hw_wait_corex_idle(void __iomem *base)
{
	u32 busy;
	u32 try_cnt = 0;
...
int byrp_hw_s_corex_init(void __iomem *base, bool enable)
{
	u32 i;
	u32 reset_count = 0;
 ...
 * CR type: No Corex
 */
int byrp_hw_s_corex_start(void __iomem *base, bool enable)
{
	if (!enable)
		return 0;
...
void byrp_hw_s_disparity_size(void __iomem *base, u32 set_id, struct is_hw_size_config *size_config)
{
...
void byrp_hw_s_strgen(void __iomem *base, u32 set_id)
{
	/* RDMA input off*/
	...
	BYRP_SET_F(base + GET_COREX_OFFSET(set_id), BYRP_R_IP_USE_CINFIFO_NEW_FRAME_IN,
		BYRP_F_IP_USE_CINFIFO_NEW_FRAME_IN, 0);
