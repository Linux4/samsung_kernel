...
unsigned int ypp_hw_is_occurred(unsigned int status, enum ypp_event_type type)
{
	u32 mask;
...
int ypp_hw_wait_idle(void __iomem *base)
{
	...
	info_hw("[YUVPP] idle status after disable (idle:%d, int1:0x%X)\n", idle, int_all);
	...
	u32 *sbwc_en, u32 *payload_size)
{
	...
	*sbwc_en = comp_sbwc_en = is_hw_dma_get_comp_sbwc_en(sbwc_type, &comp_64b_align);
	...
	if (comp_sbwc_en == 0)
		stride_1p = (dma->id == YUVPP_RDMA_HIST) ? 2048 : is_hw_dma_get_img_stride(
			memory_bitwidth, pixelsize, hwformat, width, 16, true);
		...
		stride_1p = is_hw_dma_get_payload_stride(comp_sbwc_en, pixelsize, width,
							...
							YPP_COMP_BLOCK_WIDTH,
							YPP_COMP_BLOCK_HEIGHT);
		header_stride_1p = (comp_sbwc_en == 1) ?
			is_hw_dma_get_header_stride(width, YPP_COMP_BLOCK_WIDTH, 16) : 0;
	} else {
		return SET_ERROR;
	}
	...
	*payload_size = 0;
	...
	case 1:
		...
		*payload_size = ((height + YPP_COMP_BLOCK_HEIGHT - 1) / YPP_COMP_BLOCK_HEIGHT) * stride_1p;
		break;
	case 2:
		ret |= CALL_DMA_OPS(dma, dma_set_comp_64b_align, comp_64b_align);
