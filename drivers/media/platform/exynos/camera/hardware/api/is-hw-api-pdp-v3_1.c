...
#define PDP_VERSION_V3_1		(0x3001000) /* v3.1 */
	...
	u32 pixelsize, u32 sensor_type)
{
	...
	if (pixelsize == OTF_INPUT_BIT_WIDTH_14BIT)
		bit_mode = 2;
	else if (pixelsize == OTF_INPUT_BIT_WIDTH_12BIT)
		bit_mode = 1;
	else
		bit_mode = 0;
	...
	PDP_SET_R(base, PDP_R_LIC_VIRTUAL_LINE_CONFIG_COMMON, val);
	...
	PDP_SET_R(base, PDP_R_LIC_VIRTUAL_LINE_CONFIG_DYNAMIC, val);
...
static void _pdp_hw_s_int_mask(void __iomem *base, u32 sensor_type, u32 path)
{
	...
	PDP_SET_F(base, PDP_R_CONTINT_INT1_ENABLE, PDP_F_CONTINT_INT1_ENABLE, INT1_EN_MASK);
	...
	u32 rmo, u32 en_sdc, u32 en_votf, u32 en_dma, ulong freq, u32 ex_mode)
{
	...
	/* TODO: set SDC at EVT1 */

	/* BAYER */
	PDP_SET_R(base, PDP_R_RDMA_BAYER_VOTF_EN, en_votf);
...
void pdp_hw_s_rdma_addr(void __iomem *base, dma_addr_t *address, u32 num_buffers)
{
	...
	do {
		PDP_SET_F(base, PDP_R_RDMA_BAYER_IMG_BASE_ADDR_1P_FRO0 + i, PDP_F_RDMA_BAYER_IMG_BASE_ADDR_1P_FRO0, (u32)address[i]);
	} while (++i < num_buffers);
}
