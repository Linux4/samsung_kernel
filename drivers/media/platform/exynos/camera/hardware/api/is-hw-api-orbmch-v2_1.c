...
unsigned int orbmch_hw_is_occurred(unsigned int status, enum orbmch_event_type type)
{
	...
	case ORBMCH_INTR_FRAME_START:
		mask = 1 << INTR_ORBMCH_ASET_FRAME_START;
		break;
	case ORBMCH_INTR_FRAME_END:
		mask = 1 << INTR_ORBMCH_ASET_FRAME_END;
		break;
...
int orbmch_hw_s_reset(void __iomem *base)
{
	...
	while (1) {
		...
		if (temp == 0x3)
			break;
		if (reset_count > ORBMCH_TRY_COUNT)
			return reset_count;
		reset_count++;
	}
...
int orbmch_hw_s_wdma_init(struct is_common_dma *dma, struct orbmch_param_set *param_set, u32 enable, u32 set_id)
{
	...
	case ORBMCH_WDMA_MCH_RESULT:
		/* Set mch wdma stride */
		if (ORBMCH_GET_R(dma->base, ORBMCH_R_ORBMCH_MCH_OUTPUT_MODE_A) == 0)
...
int orbmch_hw_s_rdma_addr(struct is_common_dma *dma, u32 *addr, u32 set_id)
{
	int ret;
	...
	switch (dma->id) {
	case ORBMCH_RDMA_ORB_L1:
		dbg_orbmch(4, "[API][%s] dma->id: %d, L1 address[0]: 0x%#x, L2 address[1]: 0x%#x\n", __func__,
			dma->id, addr[0], addr[1]);
		ORBMCH_SET_R(dma->base, ORBMCH_R_ORBMCH_ORB_RDMA0_BASE_ADDR0_A, addr[0]);
		ORBMCH_SET_R(dma->base, ORBMCH_R_ORBMCH_ORB_RDMA1_BASE_ADDR0_A, addr[0]);
		break;
	case ORBMCH_RDMA_ORB_L2:
		ORBMCH_SET_R(dma->base, ORBMCH_R_ORBMCH_ORB_RDMA0_BASE_ADDR1_A, addr[1]);
		ORBMCH_SET_R(dma->base, ORBMCH_R_ORBMCH_ORB_RDMA1_BASE_ADDR1_A, addr[1]);
		break;
	case ORBMCH_RDMA_MCH_PREV_DESC:
		dbg_orbmch(4, "[API][%s] dma->id: %d, prev descriptor addr: %#x\n", __func__,
			dma->id, addr[0]);
		ORBMCH_SET_R(dma->base, ORBMCH_R_ORBMCH_MCH_RDMA0_BASE_ADDR0_A, (dma_addr_t)addr[0]);
		break;
	case ORBMCH_RDMA_MCH_CUR_DESC:
		dbg_orbmch(4, "[API][%s] dma->id: %d, cur descriptor addr: %#x\n", __func__,
			dma->id, (addr[0] + ORB_KEY_BUF_SIZE));
		ORBMCH_SET_R(dma->base, ORBMCH_R_ORBMCH_MCH_RDMA0_BASE_ADDR1_A,
				((dma_addr_t)addr[0] + (dma_addr_t)ORB_KEY_BUF_SIZE));
		break;
	default:
		err_hw("[ORBMCH] invalid dma_id[%d]", dma->id);
		ret = SET_ERROR;
...
int orbmch_hw_s_wdma_addr(struct is_common_dma *dma, u32 *addr, u32 set_id)
{
	int ret = SET_SUCCESS;

	switch (dma->id) {
	case ORBMCH_WDMA_ORB_KEY:
		dbg_orbmch(4, "[API][%s] dma->id: %d, key addr: %#x desc addr: %#x, mch out addr: %#x\n",
			__func__, dma->id, addr[0], addr[0] + ORB_KEY_BUF_SIZE, addr[1]);
		ORBMCH_SET_R(dma->base, ORBMCH_R_ORBMCH_ORB_WDMA0_BASE_ADDR0_A, (dma_addr_t)addr[0]);
		break;
	case ORBMCH_WDMA_ORB_DESC:
		ORBMCH_SET_R(dma->base, ORBMCH_R_ORBMCH_ORB_WDMA0_BASE_ADDR1_A,
				((dma_addr_t)addr[0] + (dma_addr_t)ORB_KEY_BUF_SIZE));
		break;
	case ORBMCH_WDMA_MCH_RESULT:
		ORBMCH_SET_R(dma->base, ORBMCH_R_ORBMCH_MCH_WDMA0_BASE_ADDR0_A, (dma_addr_t)(addr[1]));
		break;
	default:
		err_hw("[ORBMCH] invalid dma_id[%d]", dma->id);
		ret = SET_ERROR;
...
unsigned int orbmch_hw_g_int_status(void __iomem *base, bool clear, u32 num_buffers, u32 *irq_state)
{
	...
	if (src_all & 1 << INTR_ORBMCH_SETTING_ERROR)
		ORBMCH_SET_R(base, ORBMCH_R_ORBMCH_INTERRUPT_STATUS, 1 << INTR_ORBMCH_SETTING_ERROR);
	if (clear)
	...
	struct orbmch_param_set *param_set, struct orbmch_data *data)
{
	...
	/* Keep current descriptor data for previous descriptor buffer on a next frame */
	if (param_set->output_kva_motion_pure[0] && param_set->prev_output_kva[0])
		memcpy((void *)param_set->prev_output_kva[0],
			(void *)(param_set->output_kva_motion_pure[0] + ORB_KEY_BUF_SIZE),
			ORB_DESC_BUF_SIZE);
	else
		err_hw("[%d][F:%d][ORBMCH] Descriptor buffer or Pure motion buffer[0] is NULL (prev desc:%#p, motion buffer[0]:%#p)",
			instance,
			fcount,
			param_set->prev_output_kva[0],
