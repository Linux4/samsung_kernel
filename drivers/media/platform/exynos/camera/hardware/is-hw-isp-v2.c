	...
	u32 instance, struct is_frame *frame)
{
	...
	for (buf_id = 0; buf_id < ISP_INTERNAL_BUF_MAX; buf_id++) {
		...
		if (isp_hw_get_buf_info(buf_id, sd, param_set, scenario))
			continue;
	...
	u32 instance, struct is_frame *frame)
{
	...
	CALL_HW_OPS(hw_ip, dma_cfg, "tnr_prev_in", hw_ip,
	...
	CALL_HW_OPS(hw_ip, dma_cfg, "tnr_wgt_in", hw_ip,
	...
	CALL_HW_OPS(hw_ip, dma_cfg, "tnr_prev_out", hw_ip,
		frame, cur_idx, frame->num_buffers,
	...
	CALL_HW_OPS(hw_ip, dma_cfg, "tnr_wgt_out", hw_ip,
		frame, 0, frame->num_buffers,
...
static int is_hw_isp_shot(struct is_hw_ip *hw_ip, struct is_frame *frame, ulong hw_map)
{
	...
	CALL_HW_OPS(hw_ip, dma_cfg, "ixs", hw_ip,
		frame, cur_idx, frame->num_buffers,
...
#if IS_ENABLED(SOC_TNR_MERGER)
#if IS_ENABLED(USE_MCFP_MOTION_INTERFACE)
	cmd_mv_in = CALL_HW_OPS(hw_ip, dma_cfg, "motion", hw_ip,
		frame, cur_idx, frame->num_buffers,
	...
	CALL_HW_OPS(hw_ip, dma_cfg, "drcgrid", hw_ip,
		frame, cur_idx, frame->num_buffers,
