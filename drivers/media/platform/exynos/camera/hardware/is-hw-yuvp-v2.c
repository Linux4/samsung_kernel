...
static int is_hw_yuvp_handle_interrupt0(u32 id, void *context)
{
	...
	if (yuvp_hw_is_occured0(status, INTR_FRAME_START)) {
	...
	if (yuvp_hw_is_occured0(status, INTR_FRAME_END)) {
...
static int __is_hw_yuvp_get_input_dva(u32 id, u32 *cmd, u32 **input_dva,
...
static int __is_hw_yuvp_bypass(struct is_hw_ip *hw_ip, u32 set_id)
{
	...
	if (IS_ENABLED(YPP_UNIT_TEST)) {
		yuvp_hw_s_int_mask0(hw_ip->regs[REG_SETA], 0x1FE1FFFF);
		yuvp_hw_s_strgen(hw_ip->regs[REG_SETA], 0);
	}
	...
	struct is_frame *frame, u32 fcount, u32 instance)
{
	...
	cmd_input_scmap = is_hardware_dma_cfg_32bit("scmap", hw_ip,
			frame, cur_idx, frame->num_buffers,
