...
#define WDMA_IDX(vc)			vc / DMA_VIRTUAL_CH_MAX
#define WDMA_VC(vc) 			vc % DMA_VIRTUAL_CH_MAX
#define WDMA_MAX(cfg)			((cfg) ? DIV_ROUND_UP(cfg->max_vc + 1, DMA_VIRTUAL_CH_MAX) : CSI_OTF_OUT_CH_NUM)
		...
		int otf_ch)
{
	...
	for (idx_wdma = 0; idx_wdma < WDMA_MAX(csi->sensor_cfg); idx_wdma++) {
...
static int csi_dma_attach(struct is_device_csi *csi)
{
	...
	for (idx_wdma = 0; idx_wdma < WDMA_MAX(csi->sensor_cfg); idx_wdma++) {
		...
		for (vc = DMA_VIRTUAL_CH_0; vc < DMA_VIRTUAL_CH_MAX; vc++) {
			csi_hw_dma_reset(wdma->regs_vc[WDMA_VC(vc)]);
		...
		enum vb2_buffer_state state)
{
	...
	/* buffer done for several virtual ch 0 ~ 7, internal vc is skipped */
			...
			if (likely(framemgr)) {
				...
				if (frame) {
					if (!csi->pre_dma_enable[vc] && !csi->cur_dma_enable[vc]) {
						...
						if (!csi_hw_g_output_cur_dma_enable(wdma->regs_vc[WDMA_VC(vc)], vc))
		...
		struct is_frame *frame, u32 index, u32 vc)
{
	...
	csi_hw_s_multibuf_dma_addr(wdma->regs_ctl, wdma->regs_vc[WDMA_VC(vc)],
				WDMA_VC(vc), index, frame->dvaddr_buffer[0]);
}
		...
		u32 vc, bool enable)
{
	...
	csi_hw_s_output_dma(wdma->regs_vc[WDMA_VC(vc)], WDMA_VC(vc), _enable);
}
		...
		u32 vc, bool clear)
{
	...
	csi_hw_s_frameptr(wdma->regs_vc[WDMA_VC(vc)], WDMA_VC(vc), frameptr, clear);
}
...
static void csis_s_vc_dma_frobuf(struct is_device_csi *csi, u32 vc)
{
	...
	for (b_i = 0; b_i < csi->dma_batch_num; b_i++) {
		...
		csi_hw_s_dma_addr(wdma->regs_ctl, wdma->regs_vc[WDMA_VC(vc)],
				WDMA_VC(vc), b_i + frameptr, dva);
		...
		if (vc == CSI_VIRTUAL_CH_0 && (dma_subdev->sbwc_type & COMP)) {
			...
			dva += p_size;
			csi_hw_s_dma_header_addr(wdma->regs_ctl, wdma->regs_vc[WDMA_VC(vc)],
					b_i + frameptr, dva);
		}
	}
...
static inline void csi_frame_start_inline(struct is_device_csi *csi)
{
	u32 inc = 1;
...
void csi_set_using_vc_buffer(void *ctrl)
{
	...
	spin_lock_irqsave(&csi->dma_rta_lock, flags);
	if (!csi->wdma[WDMA_IDX(vc)]) {
		err("csi->wdma was already detached");
		goto p_err;
	}

	if (csi->wdma[WDMA_IDX(vc)]->regs_vc[WDMA_VC(vc)])
		csi_hw_s_multibuf_dma_fcntseq(csi->wdma[WDMA_IDX(vc)]->regs_vc[WDMA_VC(vc)], buffer_total_num, addr, use);
	else {
...
static inline int csi_g_otf_ch(struct is_device_csi *csi, struct is_group *group)
{
	...
	csi->otf_out_ch[CSI_OTF_OUT_SINGLE] = otf_ch;
	csi->otf_out_num = 1;

	/* Short bayer out on AEB mode */
	child = group->pnext;
	if (child && child->slot == GROUP_SLOT_PAF) {
		otf_ch = child->id - GROUP_ID_PAF0;
		if (otf_ch >= MAX_NUM_CSIS_OTF_CH)
		...
		csi->otf_out_ch[CSI_OTF_OUT_SHORT] = otf_ch;
		csi->otf_out_num++;
	}

	if (csi->otf_out_num < WDMA_MAX(csi->sensor_cfg)) {
		err("invalid setting. otf_out_num(%d) attached wdma num(%d)",
			csi->otf_out_num, WDMA_MAX(csi->sensor_cfg));
		goto err;
	}
...
static inline void csi_s_config_dma(struct is_device_csi *csi, struct is_vci_config *vci_config)
{
	...
	for (idx_wdma = 0; idx_wdma < WDMA_MAX(csi->sensor_cfg); idx_wdma++) {
		wdma = csi->wdma[idx_wdma];
		otf_ch = csi->otf_out_ch[idx_wdma];
		if (wdma->regs_mux) {
			if (csi->bns) {
				bns_en = csi->bns->en;
			...
			csi_hw_s_dma_input_mux(wdma->regs_mux, wdma->index,
				bns_en, bns_mux_val, csi->ch, otf_ch);
		}
	}

	otf_ch = csi->otf_out_ch[CSI_OTF_OUT_SINGLE];
	if (csi->stat_wdma)
		csi_hw_s_dma_input_mux(csi->stat_wdma->regs_mux, csi->stat_wdma->index,
			false, 0, csi->ch, otf_ch);
	...
	for (vc = CSI_VIRTUAL_CH_0; vc < CSI_VIRTUAL_CH_MAX; vc++) {
		...
		if (vc != CSI_VIRTUAL_CH_0 && csi->stat_wdma) {
			...
			wdma = csi->wdma[WDMA_IDX(vc)];
			wdma_ch = csi->wdma_ch[WDMA_IDX(vc)];
		}

		if (!wdma)
			continue;

		wdma_mod = csi->wdma_mod[WDMA_IDX(vc)];
		if (!wdma_mod) {
		...
		csi_hw_s_config_dma_cmn(wdma->regs_ctl, WDMA_VC(vc),
	...
	struct is_framemgr *framemgr, u32 vc)
{
	...
	if (!test_bit(IS_SUBDEV_INTERNAL_USE, &dma_subdev->state) ||
		...
		if ((csi->sensor_cfg->output[vc].type == VC_EMBEDDED) ||
			...
			u32 frameptr = csi_hw_g_frameptr(wdma->regs_vc[WDMA_VC(vc)], WDMA_VC(vc));
			...
			case CSIS_ERR_DMA_DMAFIFO_FULL:
				...
				if (!(csi->crc_flag ||
					...
					is_debug_s2d(true, "[DMA%d][VC%d] CSIS error!! %s",
						csi->wdma_ch[WDMA_IDX(vc)], vc, err_str);
#endif
				break;
...
void csi_hw_cdump_all(struct is_device_csi *csi)
{
	...
	for (vc = CSI_VIRTUAL_CH_0; vc < CSI_VIRTUAL_CH_MAX; vc++) {
		wdma = csi->wdma[WDMA_IDX(vc)];
		if (wdma)
			csi_hw_vcdma_cdump(wdma->regs_vc[WDMA_VC(vc)]);
	}
	for (idx_wdma = 0; idx_wdma < WDMA_MAX(csi->sensor_cfg); idx_wdma++) {
		wdma = csi->wdma[idx_wdma];
		wdma_mod = csi->wdma_mod[idx_wdma];
	...
	if (csi->stat_wdma) {
		for (vc = CSI_VIRTUAL_CH_0; vc < CSI_VIRTUAL_CH_MAX; vc++)
			csi_hw_vcdma_cdump(csi->stat_wdma->regs_vc[WDMA_VC(vc)]);
...
void csi_hw_dump_all(struct is_device_csi *csi)
{
	...
	if (csi->stat_wdma) {
		...
		for (vc = CSI_VIRTUAL_CH_0; vc < CSI_VIRTUAL_CH_MAX; vc++) {
			...
			csi_hw_vcdma_dump(csi->stat_wdma->regs_vc[WDMA_VC(vc)]);
		}
	}
...
static void wq_ebuf_reset(struct work_struct *data)
{
	...
	if (!test_bit(IS_SENSOR_FRONT_START, &sensor->state) || !csi->otf_out_num)
		return;
...
static irqreturn_t is_isr_csi_dma(int irq, void *data)
{
	...
	for (vc = CSI_VIRTUAL_CH_0; vc < CSI_VIRTUAL_CH_MAX; vc++) {
		...
		csi_hw_s_dma_dbg_cnt(wdma->regs_vc[WDMA_VC(vc)], &irq_src, WDMA_VC(vc));

		if (dma_frame_end & (1 << (WDMA_VC(vc)))) {
			/*
			...
			if (csi->f_id_dec && vc == CSI_VIRTUAL_CH_0) {
				if (csi_hw_get_version(csi->base_reg) <
					...
					struct is_camif_wdma_module *wdma_mod = csi->wdma_mod[WDMA_IDX(vc)];
...
static irqreturn_t is_isr_csi_ebuf(int irq, void *data)
{
	...
	ebuf_ch = csi->otf_out_ch[CSI_OTF_OUT_SINGLE];
...
static int csi_g_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	...
	case SENSOR_IOCTL_G_VC7_FRAMEPTR:
		...
		ctrl->value = csi_hw_g_frameptr(wdma->regs_vc[WDMA_VC(vc)], vc);
		break;
	default:
		break;
	}
	...
	case SENSOR_IOCTL_DISABLE_VC1_VOTF:
		{
			...
			wdma_mod = csi->wdma_mod[WDMA_IDX(CSI_VIRTUAL_CH_1)];
			if (!wdma_mod) {
				merr("[CSI%d] wdma_mod is NULL", csi, csi->ch);
				return -ENODEV;
			}
			...
			csi_hw_s_dma_common_votf_cfg(wdma_mod->regs,
					...
					csi->wdma_ch[WDMA_IDX(CSI_VIRTUAL_CH_1)],
					CSI_VIRTUAL_CH_1,
					false);
	...
	int enable)
{
	...
	if (csi_g_otf_ch(csi, group)) {
		merr("[CSI%d] Failed to get OTF_OUT CH", csi, csi->ch);
	...
	/* BNS configuration */
	if (csi->bns && csi->otf_out_num)
		pablo_camif_bns_cfg(csi->bns, sensor_cfg, otf_ch);

	/* CSIS OTF_OUT MUX configuration */
	if (csi->otf_out_mux) {
		for (otf_id = CSI_OTF_OUT_SINGLE; otf_id < csi->otf_out_num; otf_id++)
			csi_hw_s_otf_out_mux(csi->otf_out_mux, csi->ch,
					     csi->otf_out_ch[otf_id],
					     (sensor_cfg->set_vc) ? sensor_cfg->img_vc[otf_id] : 0,
					     true);
	}
	...
	struct is_device_csi *csi)
{
	...
	if (test_bit(IS_GROUP_OTF_OUTPUT, &group->state) &&
		...
		struct pablo_camif_ebuf *ebuf = csi->ebuf;
		u32 otf_ch = (u32)csi->otf_out_ch[CSI_OTF_OUT_SINGLE];
	...
	if (test_bit(CSIS_DMA_ENABLE, &csi->state)) {
		...
		for (vc = CSI_VIRTUAL_CH_0; vc < CSI_VIRTUAL_CH_MAX; vc++) {
			...
			if (csi->stat_wdma)
				csi_hw_dma_reset(csi->stat_wdma->regs_vc[WDMA_VC(vc)]);
		...
		for (vc = CSI_VIRTUAL_CH_0; vc < CSI_VIRTUAL_CH_MAX; vc++) {
			...
			wdma_mod = is_camif_wdma_module_get(csi->wdma_ch[WDMA_IDX(vc)]);
			csi_hw_s_dma_common_votf_cfg(wdma_mod->regs,
					csi->dma_subdev[vc]->output.width,
					csi->wdma_ch[WDMA_IDX(vc)],
					vc,
					false);
	...
	if (csi->otf_out_mux) {
		for (otf_id = CSI_OTF_OUT_SINGLE; otf_id < csi->otf_out_num; otf_id++)
			csi_hw_s_otf_out_mux(csi->otf_out_mux,
					csi->ch, csi->otf_out_ch[otf_id], otf_id * DMA_VIRTUAL_CH_MAX, false);
	}
	...
	csi->otf_batch_num = 1;
	csi->dma_batch_num = 1;
	memset(csi->otf_out_ch, 0, sizeof(csi->otf_out_ch));
...
static int csi_s_buffer(struct v4l2_subdev *subdev, void *buf, unsigned int *vcp)
{
	...
	if (!test_bit(IS_SUBDEV_VOTF_USE, &dma_subdev->state) &&
			!test_bit(IS_SUBDEV_INTERNAL_USE, &dma_subdev->state)) {
		if (csi_hw_g_output_dma_enable(wdma->regs_vc[WDMA_VC(vc)], vc)) {
