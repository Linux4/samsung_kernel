...
static void is_hotjoin_chk_dwork_fn(struct work_struct *data)
{
	int ret;
	struct is_module_enum *module
		= container_of((void *)data, struct is_module_enum, hotjoin_chk_dwork);
	if (test_bit(IS_MODULE_GPIO_ON, &module->state)) {
		ret = module->pdata->gpio_cfg(module, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF);
		if (ret) {
			probe_err("gpio_cfg is fail(%d)", ret);
			return;
		}
...
static int sensor_module_probe_i2c(struct platform_device *pdev)
{
	...
	if (setfile) {
		carve_binary_version(IS_BIN_SETFILE, position, (void *)setfile->data, setfile->size);

		ret = is_hardware_parse_setfile(&core->hardware, (ulong)setfile->data, position);
		if (ret) {
			probe_err("failed to parse setfile for %s, size: 0x%zx",
				setfile_name, setfile->size);
			module->setfile_preload = false;
			module->setfile_size = 0;
		} else {
			module->setfile_preload = true;
			module->setfile_size = setfile->size;
			probe_info("%s is preloaded after trying %d time(s), size: 0x%zx\n",
	...
	module->stm = module->sensor_name = device->sensor_name = pdata->sensor_name;
	module->setfile_name = pdata->setfile_name;
	module->cfgs = pdata->cfgs;
	module->stat_vc = pdata->stat_vc;
	module->cfg = pdata->cfg;
	module->vpd_sensor_mode = pdata->vpd_sensor_mode;
	...
	/* used for hotjoin */
	if (pdata->cis_ixc_type == I3C_TYPE) {
		ret = module->pdata->gpio_cfg(module, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON);
		if (ret) {
			probe_err("gpio_cfg is fail(%d)", ret);
			goto p_err;
		}
		...
		INIT_DELAYED_WORK(&module->hotjoin_chk_dwork, is_hotjoin_chk_dwork_fn);
		schedule_delayed_work(&module->hotjoin_chk_dwork, msecs_to_jiffies(IS_HOTJOIN_CHK_DTIME));
	}
