...
int sensor_imx754_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	if ((target_exposure->long_val <= 0) || (target_exposure->short_val <= 0)) {
		err("[%d]invalid target exposure(%d, %d)", cis->id, target_exposure->long_val, target_exposure->short_val);
		ret = -EINVAL;
		goto p_err;
	}
...
int sensor_imx754_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	...
	if (min_fps > cis_data->max_fps) {
		err("[%d] request FPS is too high(%d), set to max(%d)",
...
static int sensor_imx754_cis_check_rev_on_init(struct v4l2_subdev *subdev)
{
	...
	ret = is_sensor_write8(client, 0xA02, 0x00);
	if (ret < 0) {
		ret = -EAGAIN;
		goto p_err;
	}
...
p_err:
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
	ret = sensor_cis_check_rev_on_init(subdev);
	return ret;
}
