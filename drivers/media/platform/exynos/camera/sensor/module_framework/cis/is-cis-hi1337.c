...
int sensor_hi1337_cis_set_analog_digital_gain(struct v4l2_subdev *subdev, struct ae_param *again, struct ae_param *dgain)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = NULL;
...
int sensor_hi1337_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	IXC_MUTEX_LOCK(cis->ixc_lock);
	ret = cis->ixc_ops->read8(cis->client, SENSOR_HI1337_ANALOG_GAIN_ADDR, &analog_gain);
	if (ret < 0)
		goto p_err_unlock;
...
int sensor_hi1337_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	dgains[0] = dgains[1] = dgains[2] = dgains[3] = dgain_code;
	ret = cis->ixc_ops->write16_array(cis->client, SENSOR_HI1337_DIG_GAIN_ADDR, dgains, 4);
	if (ret < 0) {
		goto p_err_unlock;
	}
...
static struct is_cis_ops cis_ops_hi1337 = {
	...
	.cis_get_analog_gain = sensor_hi1337_cis_get_analog_gain,
...
#ifdef MODULE
builtin_i2c_driver(sensor_cis_hi1337_driver);
#else
