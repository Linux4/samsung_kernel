...
int sensor_hm1_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read16(client, 0x0704, &data16);
	...
	else goto i2c_err;
	{
		int i = 0;
		for (i = 0; i < 96; i += 2) {
			ret = cis->ixc_ops->read16(client, 0xE580 + i, &data16);
...
int sensor_hm1_cis_wait_streamon(struct v4l2_subdev *subdev)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	cis->ixc_ops->write16(client, 0x602C, 0x2001);
	pr_info("[%s] 0x2001 page\n", __func__);

	cis->ixc_ops->write16(client, 0x602E, 0x20FC);
	cis->ixc_ops->read16(client, 0x6F12, &data16);
	pr_info("[%s] 0x20FC(0x%x)\n", __func__, data16);

	cis->ixc_ops->write16(client, 0x602E, 0x20FE);
	cis->ixc_ops->read16(client, 0x6F12, &data16);
	pr_info("[%s] 0x20FE(0x%x)\n", __func__, data16);

	cis->ixc_ops->write16(client, 0x602E, 0x2100);
	cis->ixc_ops->read16(client, 0x6F12, &data16);
	pr_info("[%s] 0x2100(0x%x)\n", __func__, data16);

	cis->ixc_ops->write16(client, 0x602C, 0x4000);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
...
int sensor_hm1_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->read16(client, 0x0D08, &data16);
	pr_info("[%s] 0x0D08(0x%x)\n", __func__, data16);

	cis->ixc_ops->read16(client, 0x0D0A, &data16);
