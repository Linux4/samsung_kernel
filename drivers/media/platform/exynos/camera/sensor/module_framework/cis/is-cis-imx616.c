...
static int sensor_imx616_set_ebd(struct is_cis *cis)
{
	...
	ret = cis->ixc_ops->read8(client, SENSOR_IMX616_EBD_CONTROL_ADDR, &enabled_ebd);
	if (ret < 0) {
	...
	ret = cis->ixc_ops->write8(client, SENSOR_IMX616_EBD_CONTROL_ADDR, enabled_ebd);
	if (ret < 0) {
		err("is_sensor_read is fail, (ret %d)", ret);
		ret = -EAGAIN;
		goto p_err;
...
int sensor_imx616_cis_get_fineIntegTime(struct is_cis *cis)
{
	...
	ret = cis->ixc_ops->read16(client, REG(FINE_INTEG_TIME), &fine_integ_time);
	if (ret < 0) {
...
int sensor_imx616_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (IS_3HDR_MODE(cis)) {
		...
		ret = cis->ixc_ops->write8(client, REG(AEHIST_LN_AUTO_THRETH), 0x0);
		CHECK_GOTO(ret < 0, p_i2c_err);
		ret = cis->ixc_ops->write16_array(client, REG(AEHIST_LN_THRETH_START),
					aehist_linear_threth, ARRAY_SIZE(aehist_linear_threth));
		CHECK_ERR_GOTO(ret < 0, p_i2c_err, "failed to set linear");

		ret = cis->ixc_ops->write8(client, REG(AEHIST_LOG_AUTO_THRETH), 0x0);
		CHECK_GOTO(ret < 0, p_i2c_err);
		ret = cis->ixc_ops->write16_array(client, REG(AEHIST_LOG_THRETH_START),
...
int sensor_imx616_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if (IS_3HDR_MODE(cis)) {
		...
		ret |= cis->ixc_ops->write8(client, 0x3021, 0x01);
		if (ret)
...
int sensor_imx616_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	if (IS_3HDR_MODE(cis)) {
		ret = cis->ixc_ops->write8(client, 0x3020, 0x00);
		ret |= cis->ixc_ops->write8(client, 0x3021, 0x00);
		if (ret)
...
int sensor_imx616_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	ret = cis->ixc_ops->write16(client, REG(FRAME_LENGTH_LINE), frame_length_lines);
	CHECK_GOTO(ret < 0, p_i2c_err);
...
int sensor_imx616_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	ret = cis->ixc_ops->write16(client, REG(COARSE_INTEG_TIME), long_coarse_int);
	CHECK_GOTO(ret < 0, p_i2c_err);
	...
	if (IS_3HDR_MODE(cis)) {
		...
		ret = cis->ixc_ops->write16(client, REG(MID_COARSE_INTEG_TIME), middle_coarse_int);
		CHECK_GOTO(ret < 0, p_i2c_err);

		ret = cis->ixc_ops->write16(client, REG(ST_COARSE_INTEG_TIME), short_coarse_int);
		CHECK_GOTO(ret < 0, p_i2c_err);
	}
...
int sensor_imx616_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	ret = cis->ixc_ops->write16(client, SENSOR_IMX616_AGAIN_ADDR, long_gain);
	CHECK_GOTO(ret < 0, p_i2c_err);
	...
	if (IS_3HDR_MODE(cis)) {
		...
		ret = cis->ixc_ops->write16(client, REG(MID_AGAIN), middle_gain);
		CHECK_GOTO(ret < 0, p_i2c_err);

		ret = cis->ixc_ops->write16(client, REG(ST_AGAIN), short_gain);
		CHECK_GOTO(ret < 0, p_i2c_err);
	}
...
int sensor_imx616_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	ret = cis->ixc_ops->read16(client, REG(AGAIN), &analog_gain);
	CHECK_GOTO(ret < 0, p_i2c_err);
...
int sensor_imx616_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	ret = cis->ixc_ops->write16(client, REG(DGAIN), long_gain);
	CHECK_GOTO(ret < 0, p_i2c_err);
	...
	if (IS_3HDR_MODE(cis)) {
		...
		ret = cis->ixc_ops->write16(client, REG(MID_DGAIN), middle_gain);
		CHECK_GOTO(ret < 0, p_i2c_err);

		ret = cis->ixc_ops->write16(client, REG(ST_DGAIN), short_gain);
		CHECK_GOTO(ret < 0, p_i2c_err);
	}
...
int sensor_imx616_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	...
	ret = cis->ixc_ops->read16(client, REG(DGAIN), &digital_gain);
	CHECK_GOTO(ret < 0, p_i2c_err);
...
int sensor_imx616_cis_set_wb_gain(struct v4l2_subdev *subdev, struct wb_gains wb_gains)
{
	...
	ret = cis->ixc_ops->write16_array(client, REG(ABS_GAIN_GR_SET), abs_gains, 4);
