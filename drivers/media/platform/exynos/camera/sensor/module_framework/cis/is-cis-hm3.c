...
void sensor_hm3_cis_set_mode_group(u32 mode)
{
	...
	sensor_hm3_mode_groups[SENSOR_HM3_MODE_LN] = MODE_GROUP_NONE;
...
int sensor_hm3_cis_update_seamless_mode(struct v4l2_subdev *subdev)
{
	...
	struct is_device_sensor_peri *sensor_peri = NULL;
	struct sensor_open_extended *ext_info;
	unsigned int next_mode = sensor_hm3_mode_groups[SENSOR_HM3_MODE_DEFAULT];
	u16 fast_change_idx = SENSOR_HM3_FAST_CHANGE_IDX_NONE;
	u32 dummy_value = 0;
	...
	if (sensor_hm3_cis_get_lownoise_supported(subdev)) {
		...
		case IS_CIS_LN2:
		case IS_CIS_LN4:
			next_mode = sensor_hm3_mode_groups[SENSOR_HM3_MODE_LN];
			break;
		}
	}
	...
	if (next_mode != MODE_GROUP_NONE && mode != next_mode) {
		...
		fast_change_idx = sensor_hm3_spec_mode_retention_attr[next_mode].fast_change_idx;
		ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
...
int sensor_hm3_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	...
	if (cis_data->cur_12bit_mode == SENSOR_12BIT_STATE_REAL_12BIT
		...
		cis_data->max_analog_gain[0] =
			sensor_hm3_common_mode_attr[sensor_hm3_mode_groups[SENSOR_HM3_MODE_IDCG]].max_analog_gain;
	} else if (cis_data->cur_12bit_mode == SENSOR_12BIT_STATE_PSEUDO_12BIT
		&& mode == sensor_hm3_mode_groups[SENSOR_HM3_MODE_IDCG]) {
		if (cis_data->cur_lownoise_mode != IS_CIS_LNOFF)
			cis_data->max_analog_gain[0] =
				sensor_hm3_common_mode_attr[sensor_hm3_mode_groups[SENSOR_HM3_MODE_LN]].max_analog_gain;
		else
			cis_data->max_analog_gain[0] =
			sensor_hm3_common_mode_attr[sensor_hm3_mode_groups[SENSOR_HM3_MODE_DEFAULT]].max_analog_gain;
	} else
#endif
	{
...
int sensor_hm3_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (ext_info->use_retention_mode == SENSOR_RETENTION_ACTIVATED) {
		...
		if (fast_change_idx != SENSOR_HM3_FAST_CHANGE_IDX_NONE) {
			load_sram_idx = sensor_hm3_spec_mode_retention_attr[mode].load_sram_idx;

			if (sensor_hm3_mode_groups[SENSOR_HM3_MODE_LN] != MODE_GROUP_NONE)
				load_sram_idx_ln =
					sensor_hm3_spec_mode_retention_attr[sensor_hm3_mode_groups[SENSOR_HM3_MODE_LN]].load_sram_idx;
			else
				load_sram_idx_ln = SENSOR_HM3_LOAD_SRAM_IDX_NONE;
			...
			if (load_sram_idx_ln != SENSOR_HM3_LOAD_SRAM_IDX_NONE) {
				ret = sensor_cis_set_registers(subdev,
	...
	cis->cis_data->remosaic_mode = sensor_hm3_spec_mode_attr[mode].remosaic_mode;
	...
	const struct i2c_device_id *id)
{
	...
	if (mclk_freq_khz == 19200) {
		if (strcmp(setfile, "default") == 0 ||
			...
			sensor_hm3_load_sram = sensor_hm3_setfile_A_19p2_load_sram;
