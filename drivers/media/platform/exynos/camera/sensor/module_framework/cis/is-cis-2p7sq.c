...
int sensor_2p7sq_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if (is_vender_wdr_mode_on(cis_data))
		cis->ixc_ops->write8(client, 0x0216, 0x01);
	else
		cis->ixc_ops->write8(client, 0x0216, 0x00);
...
int sensor_2p7sq_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* Long digital gain */
	if (is_vender_wdr_mode_on(cis_data)) {
		ret = cis->ixc_ops->write16(client, 0x305C, long_gain);
		if (ret < 0)
			goto p_err;
	}
...
static int cis_2p7sq_probe_i2c(struct i2c_client *client,
