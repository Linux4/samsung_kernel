...
u32 sensor_imx586_cis_get_fineIntegTime(struct is_cis *cis)
{
	...
	ret = cis->ixc_ops->read16(client, SENSOR_IMX586_FINE_INTEG_TIME_ADDR, &fine_integ_time);
	if (ret < 0) {
...
int sensor_imx586_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->write8(client, 0x3040, 0x01);
	cis->ixc_ops->write8(client, 0x3F71, 0x01);

	cis->ixc_ops->write8(client, 0x0101, 0x02);//TEMP_2020 flip setting(Only z3 project)
...
int sensor_imx586_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->write16(client, SENSOR_IMX586_COARSE_INTEG_TIME_ADDR, coarse_int);
		if (ret < 0)
			goto p_i2c_err;
...
int sensor_imx586_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	// Analog gain
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->write16(client, SENSOR_IMX586_ANALOG_GAIN_ADDR, analog_gain);
	if (ret < 0)
		goto p_i2c_err;
...
int sensor_imx586_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->read16(client, SENSOR_IMX586_ANALOG_GAIN_ADDR, &analog_gain);
	if (ret < 0)
		goto p_i2c_err;
...
int sensor_imx586_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->read16(client, SENSOR_IMX586_DIG_GAIN_ADDR, &digital_gain);
	if (ret < 0)
		goto p_i2c_err;
...
int sensor_imx586_cis_set_wb_gain(struct v4l2_subdev *subdev, struct wb_gains wb_gains)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->write16_array(client, SENSOR_IMX586_ABS_GAIN_GR_SET_ADDR, abs_gains, 4);
	if (ret < 0)
		goto p_i2c_err;
	...
	const struct i2c_device_id *id)
{
	...
	if (strcmp(setfile, "default") == 0 || strcmp(setfile, "setA") == 0) {
		...
		cis->mipi_sensor_mode = sensor_IMX586_setfile_A_mipi_sensor_mode;
		cis->mipi_sensor_mode_size = ARRAY_SIZE(sensor_IMX586_setfile_A_mipi_sensor_mode);
