...
int sensor_4ha_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read8(client, 0x3C02, &data8);
	if (unlikely(!ret)) pr_info("0x3C02(0x%x)\n", data8);
	else goto i2c_err;
	ret = cis->ixc_ops->read8(client, 0x3C03, &data8);
	if (unlikely(!ret)) pr_info("0x3C03(0x%x)\n", data8);
	else goto i2c_err;
	ret = cis->ixc_ops->read8(client, 0x3C05, &data8);
	if (unlikely(!ret)) pr_info("0x3C05(0x%x)\n", data8);
	else goto i2c_err;
	ret = cis->ixc_ops->read8(client, 0x3500, &data8);
