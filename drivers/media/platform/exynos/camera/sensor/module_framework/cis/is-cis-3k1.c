...
int sensor_3k1_cis_update_crop_region(struct v4l2_subdev *subdev)
{
	...
	ret |= cis->ixc_ops->read16(client, 0x0344, &x_start);
	ret |= cis->ixc_ops->read16(client, 0x0346, &y_start);
	ret |= cis->ixc_ops->read16(client, 0x0348, &x_end);
	ret |= cis->ixc_ops->read16(client, 0x034A, &y_end);
	...
	ret |= cis->ixc_ops->write16(client, 0x0344, x_start);
	ret |= cis->ixc_ops->write16(client, 0x0346, y_start);
	ret |= cis->ixc_ops->write16(client, 0x0348, x_end);
	ret |= cis->ixc_ops->write16(client, 0x034A, y_end);
