...
exit:
	info("OTP Off\n");
	ret = cis->ixc_ops->write8(cis->client, 0x0ACE, 0x00);
	ret |= cis->ixc_ops->write8(cis->client, GC08A3_OTP_MODE_SEL_ADDR, 0x00);
	ret |= cis->ixc_ops->write8(cis->client, 0x0A67, 0x00);
	ret |= cis->ixc_ops->write8(cis->client, 0x0316, 0x00);

	ret |= cis->ixc_ops->write8(cis->client, 0x0315, 0x00);
	ret |= cis->ixc_ops->write8(cis->client, 0x031C, 0x00);
	if (ret < 0) {
		err("OTP Off: failed!!");
		ret = -EINVAL;
	}
	info("%s-X\n", __func__);
	return 0; //for retry, in case of i2c fail
}
