...
int sensor_2l3_cis_check_rev(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read16(client, 0x0002, &rev);
	if (ret < 0) {
		cis->rev_flag = true;
		ret = -EAGAIN;
	} else {
