...
#include "is-cis-sc501-setA.h"
#include "is-cis-sc501-setB.h"

#include "is-helper-i2c.h"
#include "is-vender-specific.h"

#define SENSOR_NAME "SC501"
...
static const u32 *sensor_sc501_setfile_sizes;
static const struct sensor_pll_info_compact **sensor_sc501_pllinfos;
static u32 sensor_sc501_max_setfile_num;
static const u32 *sensor_sc501_fsync_master;
static u32 sensor_sc501_fsync_master_size;
...
static void sensor_sc501_cis_data_calculation(const struct sensor_pll_info_compact *pll_info, cis_shared_data *cis_data)
{
	u32 vt_pix_clk_hz = 0;
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_SC501_FINE_INTEGRATION_TIME_MIN;
	cis_data->max_fine_integration_time = SENSOR_SC501_FINE_INTEGRATION_TIME_MAX;
	cis_data->min_coarse_integration_time = SENSOR_SC501_COARSE_INTEGRATION_TIME_MIN;
	cis_data->max_margin_coarse_integration_time = SENSOR_SC501_COARSE_INTEGRATION_TIME_MAX_MARGIN;
	info("%s: done", __func__);
}

static int sensor_sc501_wait_stream_off_status(cis_shared_data *cis_data)
{
	int ret = 0;
	u32 timeout = 0;
...
int sensor_sc501_cis_check_rev(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u8 rev = 0;
	...
	probe_info("sc501 cis_check_rev start\n");
...
/* CIS OPS */
int sensor_sc501_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	u32 setfile_index = 0;
	...
	cis->cis_data->cur_width = SENSOR_SC501_MAX_WIDTH;
	cis->cis_data->cur_height = SENSOR_SC501_MAX_HEIGHT;
	cis->cis_data->low_expo_start = 33000;
	cis->need_mode_change = false;

	sensor_sc501_cis_data_calculation(sensor_sc501_pllinfos[setfile_index], cis->cis_data);
...
int sensor_sc501_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client = NULL;
	...
	pr_info("[%s] *******************************\n", __func__);
	I2C_MUTEX_LOCK(cis->i2c_lock);
	sensor_cis_dump_registers(subdev, sensor_sc501_setfiles[0], sensor_sc501_setfile_sizes[0]);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
	pr_info("[%s] *******************************\n", __func__);
...
int sensor_sc501_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = sensor_cis_set_registers(subdev, sensor_sc501_global, sensor_sc501_global_size);
	if (ret < 0) {
...
int sensor_sc501_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct is_cis *cis = NULL;
...
int sensor_sc501_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	int ret = 0;
	bool binning = false;
	...
	ret = sensor_sc501_wait_stream_off_status(cis_data);
	if (ret) {
		err("Must stream off\n");
		ret = -EINVAL;
		goto p_err;
	...
	if (binning) {
		ratio_w = (SENSOR_SC501_MAX_WIDTH / cis_data->cur_width);
		ratio_h = (SENSOR_SC501_MAX_HEIGHT / cis_data->cur_height);
	} else {
		ratio_w = 1;
		ratio_h = 1;
	}

	if (((cis_data->cur_width * ratio_w) > SENSOR_SC501_MAX_WIDTH) ||
		((cis_data->cur_height * ratio_h) > SENSOR_SC501_MAX_HEIGHT)) {
	...
	start_x = ((SENSOR_SC501_MAX_WIDTH - cis_data->cur_width * ratio_w) / 2) & (~0x1);
	start_y = ((SENSOR_SC501_MAX_HEIGHT - cis_data->cur_height * ratio_h) / 2) & (~0x1);
...
int sensor_sc501_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
...
int sensor_sc501_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_sc501_cis_set_exposure_time(struct v4l2_subdev *subdev, u16 multiple_exp)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	if (multiple_exp > ( 2 * cis_data->frame_length_lines - cis_data->max_margin_coarse_integration_time))
		frame_length_lines = (multiple_exp + cis_data->max_margin_coarse_integration_time + 1) / 2;
	else
		frame_length_lines = cis_data->frame_length_lines;

	if (frame_length_lines > SENSOR_SC501_MAX_FRAME_LENGTH_LINE_VALUE)
		frame_length_lines = SENSOR_SC501_MAX_FRAME_LENGTH_LINE_VALUE;

	multiple_exp = (multiple_exp < cis_data->min_coarse_integration_time) ? cis_data->min_coarse_integration_time : multiple_exp;
	multiple_exp = (multiple_exp > (2 * SENSOR_SC501_MAX_FRAME_LENGTH_LINE_VALUE - cis_data->max_margin_coarse_integration_time)) ? (2 * SENSOR_SC501_MAX_FRAME_LENGTH_LINE_VALUE - cis_data->max_margin_coarse_integration_time) : multiple_exp;

	I2C_MUTEX_LOCK(cis->i2c_lock);

	ret = is_sensor_write8(client, 0x320e, (frame_length_lines >> 8) & 0xFF);
	if (ret < 0)
		goto p_err;
	...
	ret = is_sensor_write8(client, 0x3e00, (multiple_exp >> 12) & 0xFF);
	if (ret < 0)
		goto p_err;
...
int sensor_sc501_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	cis_shared_data *cis_data = NULL;
	...
	min_integration_time = (u32)((u64)((line_length_pck * min_coarse / 2) + min_fine) * 1000 / vt_pix_clk_freq_khz);
...
int sensor_sc501_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_sc501_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_sc501_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	if (sensor_sc501_check_master_stream_off(core)) {
...
int sensor_sc501_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
	...
	ret = sensor_sc501_cis_set_frame_duration(subdev, frame_duration);
	if (ret < 0) {
...
int sensor_sc501_cis_adjust_analog_gain(struct v4l2_subdev *subdev, u32 input_again, u32 *target_permile)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
u32 sensor_sc501_cis_calc_again_closest(u32 permile)
{
	int i;

	if (permile <= sensor_sc501_analog_gain[CODE_GAIN_INDEX][PERMILE_GAIN_INDEX])
		return sensor_sc501_analog_gain[CODE_GAIN_INDEX][PERMILE_GAIN_INDEX];
	if (permile >= sensor_sc501_analog_gain[MAX_GAIN_INDEX][PERMILE_GAIN_INDEX])
		return sensor_sc501_analog_gain[MAX_GAIN_INDEX][PERMILE_GAIN_INDEX];

	for (i = 0; i <= MAX_GAIN_INDEX; i++)
	{
		if (sensor_sc501_analog_gain[i][PERMILE_GAIN_INDEX] == permile)
			return sensor_sc501_analog_gain[i][PERMILE_GAIN_INDEX];

		if ((int)(permile - sensor_sc501_analog_gain[i][PERMILE_GAIN_INDEX]) < 0)
			return sensor_sc501_analog_gain[i-1][PERMILE_GAIN_INDEX];
	}
...
u32 sensor_sc501_cis_calc_again_permile(u8 code)
{
	u32 ret = 0;
	int i;

	for (i = 0; i <= MAX_GAIN_INDEX; i++) {
		if (sensor_sc501_analog_gain[i][0] == code) {
			ret = sensor_sc501_analog_gain[i][1];
			break;
		}
	}
...
u32 sensor_sc501_cis_calc_again_code(u32 permile)
{
	u32 ret = 0, nearest_val = 0;
	int i;

	dbg_sensor(2, "[%s] permile %d\n", __func__, permile);
	nearest_val = sensor_sc501_cis_calc_again_closest(permile);
	dbg_sensor(2, "[%s] nearest_val %d\n", __func__, nearest_val);

	for (i = 0; i <= MAX_GAIN_INDEX; i++) {
		if (sensor_sc501_analog_gain[i][1] == nearest_val) {
			ret = sensor_sc501_analog_gain[i][0];
			break;
		}
	}
...
u32 sensor_sc501_cis_calc_dgain_code(u32 input_gain, u32 permile)
{
	u32 calc_value = 0;
	u32 digital_gain = 0;
...
u32 sensor_sc501_cis_calc_dgain_permile(u32 code, u32 fine_gain)
{
	return (code + 1) * 1000 + ((code + 1) * (fine_gain - 128) * 1000) / 128;
}

int sensor_sc501_cis_set_analog_digital_gain(struct v4l2_subdev *subdev, u32 input_gain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	if (input_gain < BASEGAIN)
		input_gain = BASEGAIN;
	else if (input_gain > 511 * BASEGAIN)
		input_gain = 511 * BASEGAIN;

	if((input_gain >= BASEGAIN) && (input_gain < 2 * BASEGAIN)) {
		total_again = 0x00;
		total_dgain = 0x00;
		ana_real_gain = 1;
	}
	...
	else if((input_gain >= 128 * BASEGAIN) && (input_gain < 256 * BASEGAIN)) {
		total_again = 0x0f;
		total_dgain = 0x07;
		ana_real_gain = 128;
	}
	else if((input_gain >= 256 * BASEGAIN) && (input_gain < 512 * BASEGAIN)) {
		total_again = 0x0f;
		total_dgain = 0x0f;
		ana_real_gain = 256;
	}
...
int sensor_sc501_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	*again = sensor_sc501_cis_calc_again_permile(analog_gain);
...
int sensor_sc501_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	read_value = SENSOR_SC501_MIN_ANALOG_GAIN_SET_VALUE;

	cis_data->min_analog_gain[0] = read_value;

	cis_data->min_analog_gain[1] = sensor_sc501_cis_calc_again_permile(read_value);
...
int sensor_sc501_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	read_value = SENSOR_SC501_MAX_ANALOG_GAIN_SET_VALUE;

	cis_data->max_analog_gain[0] = read_value;

	cis_data->max_analog_gain[1] = sensor_sc501_cis_calc_again_permile(read_value);
...
int sensor_sc501_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_sc501_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	cis_data->min_digital_gain[0] = SENSOR_SC501_MIN_DIGITAL_GAIN_SET_VALUE;
	cis_data->min_digital_gain[1] = sensor_sc501_cis_calc_dgain_permile(cis_data->min_digital_gain[0], SENSOR_SC501_MIN_DIGITAL_FINE_GAIN_SET_VALUE);
...
int sensor_sc501_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	cis_data->max_digital_gain[0] = SENSOR_SC501_MAX_DIGITAL_GAIN_SET_VALUE;
	cis_data->max_digital_gain[1] = sensor_sc501_cis_calc_dgain_permile(cis_data->max_digital_gain[0], SENSOR_SC501_MAX_DIGITAL_FINE_GAIN_SET_VALUE);
...
int sensor_sc501_cis_set_totalgain(struct v4l2_subdev *subdev, struct ae_param *target_exposure,
	struct ae_param *again, struct ae_param *dgain)
{
	...
	origin_exp = (u16)((((target_exposure->long_val * vt_pix_clk_freq_khz) - min_fine_int) / (line_length_pck * 1000))) * 2;
	...
	/* Set Exposure Time */
	ret = sensor_sc501_cis_set_exposure_time(subdev, origin_exp);
	if (ret < 0) {
		err("[%s] sensor_sc501_cis_set_exposure_time fail\n", __func__);
		goto p_err;
	}

	/* Set Analog & Digital gains */
	input_gain = again->val * dgain->val;
	ret = sensor_sc501_cis_set_analog_digital_gain(subdev, input_gain);
	if (ret < 0) {
		err("[%s] sensor_sc501_cis_set_analog_digital_gain fail\n", __func__);
		goto p_err;
	}
...
int sensor_sc501_cis_wait_streamon(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u32 poll_time_ms = 0;
	struct is_cis *cis;
...
int sensor_sc501_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u32 poll_time_ms = 0;
	struct is_cis *cis;
	...
	do {
		...
		*/
		...
		dbg_sensor(1, "[MOD:D:%d] %s, cur_frame_value(%d), (poll_time_ms(%d) < STREAM_OFF_POLL_TIME_MS(%d))\n",
				cis->id, __func__, cur_frame_value, poll_time_ms, STREAM_OFF_POLL_TIME_MS);
...
static struct is_cis_ops cis_ops = {
	.cis_init = sensor_sc501_cis_init,
	.cis_log_status = sensor_sc501_cis_log_status,
	.cis_set_global_setting = sensor_sc501_cis_set_global_setting,
	.cis_mode_change = sensor_sc501_cis_mode_change,
	.cis_set_size = sensor_sc501_cis_set_size,
	.cis_stream_on = sensor_sc501_cis_stream_on,
	.cis_stream_off = sensor_sc501_cis_stream_off,
	// .cis_set_exposure_time = NULL,
	.cis_get_min_exposure_time = sensor_sc501_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_sc501_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_sc501_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_sc501_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_sc501_cis_set_frame_rate,
	// .cis_adjust_analog_gain = sensor_sc501_cis_adjust_analog_gain,
	// .cis_set_analog_gain = NULL,
	// .cis_get_analog_gain = sensor_sc501_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_sc501_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_sc501_cis_get_max_analog_gain,
	...
	.cis_set_totalgain = sensor_sc501_cis_set_totalgain,
};

int cis_sc501_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	sensor_peri = find_peri_by_cis_id(device, SENSOR_NAME_SC501);
	if (!sensor_peri) {
		...
		if (ret) {
			...
			if (cis->id == specific->sensor_id[rom_position]) {
				...
				if (vender_rom_addr[rom_position]) {
					...
					probe_info("%s: SC501 OTP address not defined!\n", __func__);
				}
			}
#ifdef SUPPORT_SENSOR_DUALIZATION
				...
				if (vender_rom_addr_dualized[rom_position]) {
					...
					probe_info("%s: [Dualization] SC501 OTP address not defined!\n", __func__);
				}
			}
#endif
			else {
	...
	if (strcmp(setfile, "default") == 0 || strcmp(setfile, "setA") == 0) {
		...
		sensor_sc501_global = sensor_sc501_setfile_A_Global;
		sensor_sc501_global_size = ARRAY_SIZE(sensor_sc501_setfile_A_Global);
		sensor_sc501_setfiles = sensor_sc501_setfiles_A;
		sensor_sc501_setfile_sizes = sensor_sc501_setfile_A_sizes;
		sensor_sc501_pllinfos = sensor_sc501_pllinfos_A;
		sensor_sc501_max_setfile_num = ARRAY_SIZE(sensor_sc501_setfiles_A);
		sensor_sc501_fsync_master = sensor_sc501_setfile_A_Fsync_Master;
		sensor_sc501_fsync_master_size = ARRAY_SIZE(sensor_sc501_setfile_A_Fsync_Master);
		sensor_sc501_fsync_slave = sensor_sc501_setfile_A_Fsync_Slave;
		sensor_sc501_fsync_slave_size = ARRAY_SIZE(sensor_sc501_setfile_A_Fsync_Slave);
	} else if (strcmp(setfile, "setB") == 0) {
		probe_info("%s setfile_B\n", __func__);
		sensor_sc501_global = sensor_sc501_setfile_B_Global;
		sensor_sc501_global_size = ARRAY_SIZE(sensor_sc501_setfile_B_Global);
		sensor_sc501_setfiles = sensor_sc501_setfiles_B;
		sensor_sc501_setfile_sizes = sensor_sc501_setfile_B_sizes;
		sensor_sc501_pllinfos = sensor_sc501_pllinfos_B;
		sensor_sc501_max_setfile_num = ARRAY_SIZE(sensor_sc501_setfiles_B);
		sensor_sc501_fsync_master = sensor_sc501_setfile_B_Fsync_Master;
		sensor_sc501_fsync_master_size = ARRAY_SIZE(sensor_sc501_setfile_B_Fsync_Master);
		sensor_sc501_fsync_slave = sensor_sc501_setfile_B_Fsync_Slave;
		sensor_sc501_fsync_slave_size = ARRAY_SIZE(sensor_sc501_setfile_B_Fsync_Slave);
	} else {
...
static const struct of_device_id sensor_cis_sc501_match[] = {
	{
		.compatible = "samsung,exynos-is-cis-sc501",
	},
	{
		.compatible = "samsung,exynos-is-cis-sc501-macro",
	},
};
MODULE_DEVICE_TABLE(of, sensor_cis_sc501_match);

static const struct i2c_device_id sensor_cis_sc501_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_cis_sc501_driver = {
	.probe	= cis_sc501_probe,
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_cis_sc501_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_cis_sc501_idt,
};

static int __init sensor_cis_sc501_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_sc501_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_sc501_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_cis_sc501_init);
