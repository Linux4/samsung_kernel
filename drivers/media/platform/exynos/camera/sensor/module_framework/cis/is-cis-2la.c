...
int sensor_2la_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if (cis->cis_data->dual_slave) {
		msleep(40);
		ret = cis->ixc_ops->write16(client, 0x0B30, 0x0100);
		if (ret < 0)
...
int sensor_2la_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->write16(client, 0x0100, 0x0000);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
	...
	const struct i2c_device_id *id)
{
	...
	if (mclk_freq_khz == 19200) {
		...
		sensor_2la_dualsync_slave = sensor_2la_setfile_A_19p2_dual_slave;
		sensor_2la_dualsync_slave_size = ARRAY_SIZE(sensor_2la_setfile_A_19p2_dual_slave);
		sensor_2la_dualsync_single = sensor_2la_setfile_A_19p2_dual_single;
		sensor_2la_dualsync_single_size = ARRAY_SIZE(sensor_2la_setfile_A_19p2_dual_single);
		...
		sensor_2la_dualsync_slave = sensor_2la_setfile_A_dual_slave;
		sensor_2la_dualsync_slave_size = ARRAY_SIZE(sensor_2la_setfile_A_dual_slave);
		sensor_2la_dualsync_single = sensor_2la_setfile_A_dual_single;
		sensor_2la_dualsync_single_size = ARRAY_SIZE(sensor_2la_setfile_A_dual_single);
