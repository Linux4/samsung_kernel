...
int sensor_imx516_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 duration)
{
	...
	switch (duration) {
		case SENSOR_FPS_5:
			cur_fps = fps_list[5];
			fps = cur_fps & 0xFF;
			cis->ixc_ops->write8(client, SENOSR_FPS_REGISTER_2, fps);
			fps = cur_fps >> 8;
			cis->ixc_ops->write8(client, SENOSR_FPS_REGISTER_1, fps);
			break;
		case SENSOR_FPS_10:
			cur_fps = fps_list[4];
			fps = cur_fps & 0xFF;
...
int sensor_imx516_cis_set_vcsel_current(struct v4l2_subdev *subdev, u32 value)
{
	...
	cis->ixc_ops->write8(client, 0x0403, 0x20);
	cis->ixc_ops->write8(client, 0x0405, 0x00);
	cis->ixc_ops->write8(client, 0x0407, 0x00);
	cis->ixc_ops->write8(client, 0x0500, 0x02);
	cis->ixc_ops->write8(client, 0x0501, 0x0e);
	cis->ixc_ops->write8(client, 0x0502, 0x80);
	cis->ixc_ops->write8(client, 0x0401, 0x01);
	cis->ixc_ops->write8(client, 0x0400, 0x01);
	cis->ixc_ops->write8(client, 0x0401, 0x00);
	...
	cis->ixc_ops->write8(client, 0x0407, 0x00);
	cis->ixc_ops->write8(client, 0x0500, 0x02);
	cis->ixc_ops->write8(client, 0x0501, 0x14);
	cis->ixc_ops->write8(client, 0x0502, 0x00);
	cis->ixc_ops->write8(client, 0x0401, 0x01);
	cis->ixc_ops->write8(client, 0x0400, 0x01);
	cis->ixc_ops->write8(client, 0x0401, 0x00);
...
int sensor_imx516_cis_set_laser_error(struct v4l2_subdev *subdev, u32 mode)
{
	...
	for(i=0; i < sizeof(sensor_imx516_laser_testcases)/sizeof(sensor_imx516_laser_testcases[0]); i++){
		...
		if(test.error == mode){
			...
			cis->ixc_ops->write8(client, 0x0403, 0x20);
			cis->ixc_ops->write8(client, 0x0405, 0x00);
			cis->ixc_ops->write8(client, 0x0407, 0x00);		/*send buf CXA4026 config */
			cis->ixc_ops->write8(client, 0x0500, 0x02);
			cis->ixc_ops->write8(client, 0x0501, test.addr);		/* CXA4026 addr */
			cis->ixc_ops->write8(client, 0x0502, test.data);		/* Data to send */
			cis->ixc_ops->write8(client, 0x0401, 0x01);		/* Send start*/
			cis->ixc_ops->write8(client, 0x0400, 0x01);
			cis->ixc_ops->write8(client, 0x0401, 0x00);		/* Send end */
...
int sensor_imx516_cis_get_laser_error_flag(struct v4l2_subdev *subdev, int *value)
{
	...
	cis->ixc_ops->read8(client, 0x0590, &err_flag[0]);	/* 0x0590 for CSA4026 error flag(0x27) */
	cis->ixc_ops->read8(client, 0x0591, &err_flag[1]);	/* 0x0591 for CSA4026 error flag(0x28) */
...
int sensor_imx516_cis_get_vcsel_photo_diode(struct v4l2_subdev *subdev, u16 *value)
{
	...
	cis->ixc_ops->read8(client, 0x0588, &value8_1) ;	/* 0x058B for PD_TARGET_DATA[9:8] */
	cis->ixc_ops->read8(client, 0x058F, &value8_0);	/* 0x0582 for PD_TARGET_DATA[7:0] */
	cis->ixc_ops->read8(client, 0x0590, &err_flag[0]);	/* 0x0590 for CSA4026 error flag(0x27) */
	cis->ixc_ops->read8(client, 0x0591, &err_flag[1]);	/* 0x0591 for CSA4026 error flag(0x28) */
