...
static const u32 **sensor_gd2_setfiles;
static const u32 *sensor_gd2_setfile_sizes;
static const struct sensor_pll_info_compact **sensor_gd2_pllinfos;
static u32 sensor_gd2_max_setfile_num;

static void sensor_gd2_set_integration_max_margin(u32 mode, cis_shared_data *cis_data)
{
	FIMC_BUG_VOID(!cis_data);

	cis_data->max_margin_coarse_integration_time = SENSOR_GD2_COARSE_INTEGRATION_TIME_MAX_MARGIN_FULL;
	if (mode == SENSOR_GD2_4SUM_CROP_3264X1836_120FPS)
		cis_data->max_margin_coarse_integration_time = SENSOR_GD2_COARSE_INTEGRATION_TIME_MAX_MARGIN_4SUM;
	dbg_sensor(1, "max_margin_coarse_integration_time(%d)\n",
		cis_data->max_margin_coarse_integration_time);
}

static void sensor_gd2_set_integration_min(u32 mode, cis_shared_data *cis_data)
{
	FIMC_BUG_VOID(!cis_data);

	switch (mode) {
	case SENSOR_GD2_REMOSAIC_FULL_6528x4896_24FPS:
	case SENSOR_GD2_2SUM_FULL_3264X2448_30FPS:
	case SENSOR_GD2_2SUM_CROP_3264X1836_30FPS:
	case SENSOR_GD2_2SUM_CROP_2880X1980_30FPS:
	case SENSOR_GD2_2SUM_CROP_2640X1980_30FPS:
	case SENSOR_GD2_2SUM_CROP_1632X1224_120FPS:
	case SENSOR_GD2_2SUM_CROP_3264X1836_60FPS:
		cis_data->min_coarse_integration_time = SENSOR_GD2_COARSE_INTEGRATION_TIME_MIN_FULL_2SUM;
		dbg_sensor(1, "FULL or 2SUM min_coarse_integration_time(%d)\n",
			cis_data->min_coarse_integration_time);
		break;
	case SENSOR_GD2_4SUM_CROP_3264X1836_120FPS:
		cis_data->min_coarse_integration_time = SENSOR_GD2_COARSE_INTEGRATION_TIME_MIN_4SUM;
		dbg_sensor(1, "4SUM min_coarse_integration_time(%d)\n",
			cis_data->min_coarse_integration_time);
		break;
	default:
		err("[%s] Unsupported gd2 sensor mode\n", __func__);
		cis_data->min_coarse_integration_time = SENSOR_GD2_COARSE_INTEGRATION_TIME_MIN_FULL_2SUM;
...
static void sensor_gd2_cis_data_calculation(const struct sensor_pll_info_compact *pll_info_compact, cis_shared_data *cis_data)
{
	u64 vt_pix_clk_hz;
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_GD2_FINE_INTEGRATION_TIME;
	cis_data->max_fine_integration_time = SENSOR_GD2_FINE_INTEGRATION_TIME;
}

int sensor_gd2_cis_check_rev(struct is_cis *cis)
{
	int ret = 0;
	u8 status = 0;
	u8 rev = 0;
	...
	is_sensor_write16(client, SENSOR_GD2_OTP_PAGE_SETUP_ADDR, 0x0001);

	/* Turn ON OTP Read MODE - Read Start */
	is_sensor_write16(client, SENSOR_GD2_OTP_READ_TRANSFER_MODE_ADDR,  0x0100);

	/* Check status - 0x00 : read ready*/
	is_sensor_read8(client, SENSOR_GD2_OTP_STATUS_REGISTER_ADDR,  &status);
	if (status != 0x0) {
		err("status fail, (%d)", status);
		msleep(40);
	}

	ret = is_sensor_read8(client, SENSOR_GD2_OTP_CHIP_REVISION_ADDR, &rev);
	if (ret < 0) {
	...
	probe_info("S5KGD2 Rev:%#x  (%#x, %#x)", rev, data1, data2);
	return 0;
}
...
int sensor_gd2_cis_init(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_gd2_cis_check_rev(cis);
	if (ret < 0) {
		warn("sensor_gd2_check_rev is fail when cis init");
		cis->rev_flag = true;
		ret = 0;
	}

	cis->cis_data->cur_width = SENSOR_GD2_MAX_WIDTH;
	cis->cis_data->cur_height = SENSOR_GD2_MAX_HEIGHT;
	cis->cis_data->low_expo_start = 33000;
	cis->need_mode_change = false;

	sensor_gd2_cis_data_calculation(sensor_gd2_pllinfos[setfile_index], cis->cis_data);
	sensor_gd2_set_integration_max_margin(setfile_index, cis->cis_data);
	sensor_gd2_set_integration_min(setfile_index, cis->cis_data);
...
int sensor_gd2_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	sensor_cis_dump_registers(subdev, sensor_gd2_setfiles[0], sensor_gd2_setfile_sizes[0]);
...
static int sensor_gd2_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	int ret = 0;
...
#else
static inline int sensor_gd2_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{ return 0; }
#endif
 ...
 */
int sensor_gd2_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	ret = sensor_gd2_cis_group_param_hold_func(subdev, hold);
	if (ret < 0)
		goto p_err;
...
int sensor_gd2_apply_cross_talk_settings(struct is_cis *cis)
{
	...
	is_sensor_write16(cis->client, 0xFCFC, 0x2002);

	for (i = 0; i < xtc_size; i += 2) {
		u16 val = (cal_data[i + 1] & 0xFF) << 8;
		val |= (cal_data[i] & 0xFF);
		ret = is_sensor_write16(cis->client, 0x3800 + i, val);
		if (ret < 0) {
	...
	info("[%s] cross talk settings done", __func__);
p_err:
	return ret;
}
...
int sensor_gd2_apply_lsc_settings(struct is_cis *cis)
{
	...
	is_sensor_write16(cis->client, 0xFCFC, 0x4000);
	is_sensor_write16(cis->client, 0x6000, 0x0085);
	...
	info("[%s] lsc settings done", __func__);
p_err:
	return ret;
}
...
int sensor_gd2_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	...
	//initial settings
	ret = sensor_cis_set_registers(subdev, sensor_gd2_initial_settings_setfile_B, ARRAY_SIZE(sensor_gd2_initial_settings_setfile_B));
	if (ret < 0) {
		err("sensor_gd2_set_registers fail!!");
		goto p_err;
	}

	//tnp settings
	ret = sensor_cis_set_registers(subdev, sensor_gd2_tnp_settings_setfile_B, ARRAY_SIZE(sensor_gd2_tnp_settings_setfile_B));
	if (ret < 0) {
		err("sensor_gd2_set_registers fail!!");
		goto p_err;
	}
	...
	ret = sensor_cis_set_registers(subdev, sensor_gd2_global_calibration_settings_setfile_B, ARRAY_SIZE(sensor_gd2_global_calibration_settings_setfile_B));
	if (ret < 0) {
		err("sensor_gd2_set_registers fail!!");
		goto p_err;
	}
	...
	//lsc
	ret = sensor_gd2_apply_lsc_settings(cis);
	if (ret < 0) {
		err("sensor_gd2_pdaf_off_setfiles fail!!");
		goto p_err;
	}
...
int sensor_gd2_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (mode > sensor_gd2_max_setfile_num) {
		err("invalid mode(%d)!!", mode);
		return -EINVAL;
	}

	sensor_gd2_cis_data_calculation(sensor_gd2_pllinfos[mode], cis->cis_data);
	sensor_gd2_set_integration_max_margin(mode, cis->cis_data);
	sensor_gd2_set_integration_min(mode, cis->cis_data);

	I2C_MUTEX_LOCK(cis->i2c_lock);

	ret = sensor_cis_set_registers(subdev, sensor_gd2_setfiles[mode], sensor_gd2_setfile_sizes[mode]);
	if (ret < 0) {
		err("sensor_gd2_setfiles fail!!");
		goto p_err;
	}
...
/* Deprecated */
int sensor_gd2_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	return 0;
}
...
int sensor_gd2_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_gd2_cis_group_param_hold_func(subdev, 0x00);
	if (ret < 0)
...
int sensor_gd2_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	numerator = (u64)vt_pic_clk_freq_khz * target_exposure->long_val;
	long_coarse_int = ((target_exposure->long_val * vt_pic_clk_freq_khz) / 1000 - min_fine_int) / line_length_pck;
	numerator = (u64)vt_pic_clk_freq_khz * target_exposure->short_val;
	...
	hold = sensor_gd2_cis_group_param_hold_func(subdev, 0x01);
	if (hold < 0) {
		ret = hold;
		goto p_err;
	}
	...
	/* Short exposure */
	ret = is_sensor_write16(client, SENSOR_GD2_COARSE_INTEG_TIME_ADDR, short_coarse_int);
	if (ret < 0)
		goto p_err;

	/* Long exposure */
	if (is_vender_wdr_mode_on(cis_data)) {
		ret = is_sensor_write16(client, SENSOR_GD2_LONG_COARSE_INTEG_TIME_ADDR, long_coarse_int);
		if (ret < 0)
			goto p_err;
	}
...
p_err:
	if (hold > 0) {
		hold = sensor_gd2_cis_group_param_hold_func(subdev, 0x00);
		if (hold < 0)
			ret = hold;
	}
...
int sensor_gd2_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct is_cis *cis = NULL;
...
int sensor_gd2_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_gd2_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_gd2_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	ret = is_sensor_write16(client, SENSOR_GD2_FRAME_LENGTH_LINE_ADDR, frame_length_lines);
	if (ret < 0)
		goto p_err;
...
int sensor_gd2_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
	...
	ret = sensor_gd2_cis_set_frame_duration(subdev, frame_duration);
	if (ret < 0) {
...
int sensor_gd2_cis_adjust_analog_gain(struct v4l2_subdev *subdev, u32 input_again, u32 *target_permile)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_gd2_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	ret = is_sensor_write16(client, SENSOR_GD2_ANALOG_GAIN_ADDR, analog_gain);
	if (ret < 0)
		goto p_err;
...
int sensor_gd2_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	ret = is_sensor_read16(client, SENSOR_GD2_ANALOG_GAIN_ADDR, &analog_gain);
	if (ret < 0)
		goto p_err;
...
int sensor_gd2_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_gd2_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_gd2_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	/* Short digital gain */
	ret = is_sensor_write16_array(client, SENSOR_GD2_DIG_GAIN_ADDR, dgains, 4);
	if (ret < 0)
		goto p_err;
...
int sensor_gd2_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	ret = is_sensor_read16(client, SENSOR_GD2_DIG_GAIN_ADDR, &digital_gain);
	if (ret < 0)
		goto p_err;
...
int sensor_gd2_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_gd2_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
...
int sensor_gd2_cis_long_term_exposure(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
...
static int sensor_gd2_cis_set_dual_master_setting(struct is_cis *cis)
{
	int ret = 0;
	struct i2c_client *client;
...
int sensor_gd2_cis_set_dual_setting(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct is_cis *cis;
	...
	case DUAL_SYNC_MASTER:
		ret = sensor_gd2_cis_set_dual_master_setting(cis);
		if (ret)
			err("gd2 dual master setting fail");
		break;
...
void sensor_gd2_cis_data_calc(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct is_cis *cis = NULL;
	...
	if (cis->rev_flag == true) {
		...
		ret = sensor_gd2_cis_check_rev(cis);
		if (ret < 0) {
			err("sensor_gd2_check_rev is fail: ret(%d)", ret);
			return;
		}
	}
...
static struct is_cis_ops cis_ops = {
	...
	.cis_group_param_hold = sensor_gd2_cis_group_param_hold,
	.cis_set_global_setting = sensor_gd2_cis_set_global_setting,
	.cis_mode_change = sensor_gd2_cis_mode_change,
	.cis_set_size = sensor_gd2_cis_set_size,
	.cis_stream_on = sensor_gd2_cis_stream_on,
	.cis_stream_off = sensor_gd2_cis_stream_off,
	.cis_set_exposure_time = sensor_gd2_cis_set_exposure_time,
	.cis_get_min_exposure_time = sensor_gd2_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_gd2_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_gd2_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_gd2_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_gd2_cis_set_frame_rate,
	.cis_adjust_analog_gain = sensor_gd2_cis_adjust_analog_gain,
	.cis_set_analog_gain = sensor_gd2_cis_set_analog_gain,
	.cis_get_analog_gain = sensor_gd2_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_gd2_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_gd2_cis_get_max_analog_gain,
	.cis_set_digital_gain = sensor_gd2_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_gd2_cis_get_digital_gain,
	.cis_get_min_digital_gain = sensor_gd2_cis_get_min_digital_gain,
	.cis_get_max_digital_gain = sensor_gd2_cis_get_max_digital_gain,
	...
	.cis_set_long_term_exposure = sensor_gd2_cis_long_term_exposure,
	.cis_set_dual_setting = sensor_gd2_cis_set_dual_setting,
	.cis_data_calculation = sensor_gd2_cis_data_calc,
};
	...
	const struct i2c_device_id *id)
{
	...
	if (ret) {
		...
		sensor_gd2_setfiles = sensor_gd2_setfiles_A;
		sensor_gd2_setfile_sizes = sensor_gd2_setfile_A_sizes;
		sensor_gd2_pllinfos = sensor_gd2_pllinfos_A;
		sensor_gd2_max_setfile_num = ARRAY_SIZE(sensor_gd2_setfiles_A);
        } else if (strcmp(setfile, "setB") == 0) {
                probe_info("%s setfile_B\n", __func__);
		sensor_gd2_setfiles = sensor_gd2_setfiles_B;
		sensor_gd2_setfile_sizes = sensor_gd2_setfile_B_sizes;
		sensor_gd2_pllinfos = sensor_gd2_pllinfos_B;
		sensor_gd2_max_setfile_num = ARRAY_SIZE(sensor_gd2_setfiles_B);
        } else {
