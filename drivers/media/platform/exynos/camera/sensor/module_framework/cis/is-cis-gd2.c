...
static const u32 *sensor_gd2_global;
static u32 sensor_gd2_global_size;
...
static void sensor_gd2_set_integration_max_margin(u32 mode, cis_shared_data *cis_data)
{
	...
	cis_data->max_margin_coarse_integration_time = SENSOR_GD2_COARSE_INTEGRATION_TIME_MAX_MARGIN;
...
static void sensor_gd2_set_integration_min(u32 mode, cis_shared_data *cis_data)
{
	...
	switch (mode) {
	case SENSOR_GD2_REMOSAIC_FULL_8000x6000_10FPS:
		cis_data->min_coarse_integration_time = SENSOR_GD2_COARSE_INTEGRATION_TIME_MIN_FULL;
		dbg_sensor(1, "FULL min_coarse_integration_time(%d)\n",
			cis_data->min_coarse_integration_time);
		break;
	case SENSOR_GD2_2X2BIN_FULL_4000X3000_30FPS:
	case SENSOR_GD2_2X2BIN_CROP_4000X2256_30FPS:
	case SENSOR_GD2_2X2BIN_CROP_4000X1844_30FPS:
	case SENSOR_GD2_2X2BIN_CROP_3664X3000_30FPS:
	case SENSOR_GD2_2X2BIN_CROP_3000X3000_30FPS:
		cis_data->min_coarse_integration_time = SENSOR_GD2_COARSE_INTEGRATION_TIME_MIN_2SUM;
		dbg_sensor(1, "2SUM2BIN min_coarse_integration_time(%d)\n",
			cis_data->min_coarse_integration_time);
		break;
	case SENSOR_GD2_4X4BIN_FULL_2000X1500_94FPS:
	case SENSOR_GD2_4X4BIN_CROP_2000X1128_120FPS:
		...
		dbg_sensor(1, "4SUM4BIN min_coarse_integration_time(%d)\n",
			cis_data->min_coarse_integration_time);
		break;
	case SENSOR_GD2_4X4BIN_CROP_1920X1080_240FPS:
		cis_data->min_coarse_integration_time = SENSOR_GD2_COARSE_INTEGRATION_TIME_MIN_4SUM_240;
		dbg_sensor(1, "4SUM4BIN 240 min_coarse_integration_time(%d)\n",
			cis_data->min_coarse_integration_time);
		break;
	default:
		err("[%s] Unsupport gd2 sensor mode\n", __func__);
		cis_data->min_coarse_integration_time = SENSOR_GD2_COARSE_INTEGRATION_TIME_MIN_2SUM;
...
int sensor_gd2_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_cis_set_registers(subdev, sensor_gd2_global, sensor_gd2_global_size);
	...
	const struct i2c_device_id *id)
{
	...
	if (ret) {
		...
		sensor_gd2_global = sensor_gd2_setfile_A_Global;
		sensor_gd2_global_size = ARRAY_SIZE(sensor_gd2_setfile_A_Global);
		...
		sensor_gd2_global = sensor_gd2_setfile_B_Global;
		sensor_gd2_global_size = ARRAY_SIZE(sensor_gd2_setfile_B_Global);
