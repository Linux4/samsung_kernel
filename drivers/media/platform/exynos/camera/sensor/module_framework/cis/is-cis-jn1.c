...
/*	*rom_cal_buf = cal_buf[SENSOR_POSITION_REAR];

	if (*rom_cal_buf == NULL) {
		err("cal buf is null. position %d", SENSOR_POSITION_REAR);
		goto p_err;
	}*/
	...
	if (rom_cal_buf[0] == 0xFF && rom_cal_buf[1] == 0x00) {
		...
		for (i = 0; i < (data_size/2) ; i++) {
			...
			//err("write data[%d]: %#06x", i, write_data);
		}
		is_sensor_write16(client, 0x6028, 0x2400);
...
#ifdef USE_AP_PDAF
static void sensor_jn1_cis_set_paf_stat_enable(u32 mode, cis_shared_data *cis_data)
{
	WARN_ON(!cis_data);
...
int sensor_jn1_cis_check_rev_on_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	...
	pr_info("%s : Sensor version. Rev. 0x%X (%#x, %#x)", __func__, rev, data1, data2);
...
int sensor_jn1_cis_init(struct v4l2_subdev *subdev)
{
	...
	//force use setA only for bringup
	if (cis->cis_data->cis_rev == 0xA1) {
		is_info("%s setfile_B\n", __func__);
#if SENSOR_JN1_BURST_WRITE
		sensor_jn1_global = sensor_jn1_setfile_A_Global_Burst;
		sensor_jn1_global_size = ARRAY_SIZE(sensor_jn1_setfile_A_Global_Burst);
#else
...
int sensor_jn1_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
...
#ifdef USE_AP_PDAF
	sensor_jn1_cis_set_paf_stat_enable(mode, cis->cis_data);
#endif
	...
	ret = is_sensor_write8(client, 0x0101, 0x03);
	if (ret < 0)
		 goto p_err;
#endif
...
p_err:
	err("[%s] sen_strm_enable(%d), cit_lshift_count (%d), target_exp(%d)", __func__,
...
static struct is_cis_ops cis_ops = {
	...
	.cis_check_rev_on_init = sensor_jn1_cis_check_rev_on_init,
	...
	const struct i2c_device_id *id)
{
	...
	sensor_peri = find_peri_by_cis_id(device, SENSOR_NAME_S5KJN1);
	if (!sensor_peri) {
