...
int sensor_2x5_cis_write_cal(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->write16(client, SENSOR_2X5_W_DIR_PAGE, cal_page_lsc);
	...
	for (i = 0; i < SENSOR_2X5_CAL_LSC_SIZE / 2; i++) {
		ret = cis->ixc_ops->write16(client, cal_offset_lsc + (i * 2), cpu_to_be16(lsc_buf[i]));
	...
	for (i = 0; i < SENSOR_2X5_CAL_XTALK_SIZE / 2; i++) {
		ret = cis->ixc_ops->write16(client, cal_offset_xtalk + (i * 2), cpu_to_be16(xtalk_buf[i]));
...
int sensor_2x5_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (cal_done == true) {
		ret = cis->ixc_ops->write16(client, SENSOR_2X5_W_DIR_PAGE, 0x4000);
		if (ret < 0) {
			err("i2c fail addr(%x, 0x4000) ,ret = %d\n", SENSOR_2X5_W_DIR_PAGE, ret);
			goto p_err_i2c;
		}
		ret = cis->ixc_ops->write16(client, 0x6000, 0x0005);
		if (ret < 0) {
	...
	if (cal_done == true) {
		...
		ret = cis->ixc_ops->write16(client, 0x6000, 0x0085);
		if (ret < 0) {
...
int sensor_2x5_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	{
	u16 pll;
	ret = cis->ixc_ops->read16(client, 0x0300, &pll);
	dbg_sensor(1, "______ vt_pix_clk_div(%x)\n", pll);
	ret = cis->ixc_ops->read16(client, 0x0302, &pll);
	dbg_sensor(1, "______ vt_sys_clk_div(%x)\n", pll);
	ret = cis->ixc_ops->read16(client, 0x0304, &pll);
	dbg_sensor(1, "______ pre_pll_clk_div(%x)\n", pll);
	ret = cis->ixc_ops->read16(client, 0x0306, &pll);
	dbg_sensor(1, "______ pll_multiplier(%x)\n", pll);
	ret = cis->ixc_ops->read16(client, 0x0308, &pll);
	dbg_sensor(1, "______ op_pix_clk_div(%x)\n", pll);
	ret = cis->ixc_ops->read16(client, 0x030a, &pll);
	dbg_sensor(1, "______ op_sys_clk_div(%x)\n", pll);

	ret = cis->ixc_ops->read16(client, 0x030c, &pll);
	dbg_sensor(1, "______ secnd_pre_pll_clk_div(%x)\n", pll);
	ret = cis->ixc_ops->read16(client, 0x030e, &pll);
	...
	/* Sensor stream on */
	ret = cis->ixc_ops->write16(client, 0x0100, 0x0100);
	if (ret < 0)
...
int sensor_2x5_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	/* Sensor stream off */
	ret |= cis->ixc_ops->write16(client, 0x0100, 0x0000);
	if (ret < 0)
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0100, 0x00, ret);
...
int sensor_2x5_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	if (is_vender_wdr_mode_on(cis_data)) {
		ret = cis->ixc_ops->write16(client, 0x021E, long_coarse_int);
		if (ret < 0)
			goto p_err;
	}
#endif
...
int sensor_2x5_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->read16(client, 0x0084, &read_value);
	if (ret < 0)
...
int sensor_2x5_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->read16(client, 0x0086, &read_value);
	if (ret < 0)
...
int sensor_2x5_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	if (false /* is_vender_wdr_mode_on(cis_data)*/) {
		...
		ret = cis->ixc_ops->write16(client, 0x0C82, long_gain);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_2x5_cis_set_wb_gain(struct v4l2_subdev *subdev, struct wb_gains wb_gains)
{
	...
	ret = cis->ixc_ops->write16(client, SENSOR_2X5_W_DIR_PAGE, addr_page);
	if (ret < 0) {
		err("i2c fail addr(%x, %x) ,ret = %d\n", SENSOR_2X5_W_DIR_PAGE, addr_page, ret);
		goto p_err;
	}
	for (i = 0; i < ARRAY_SIZE(abs_gains); i++) {
		ret = cis->ixc_ops->write16(client, addr_offset + (i * 2), abs_gains[i]);
