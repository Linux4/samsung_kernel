...
int sensor_imx576_cis_check_rev(struct is_cis *cis)
{
	...
	cis->ixc_ops->write8(client, 0x0A02,  0x3F);

	/* Turn ON OTP Read MODE */
	cis->ixc_ops->write8(client, 0x0A00,  0x01);

	/* Check status - 0x01 : read ready*/
	cis->ixc_ops->read8(client, 0x0A01,  &status);
	if ((status & 0x1) == false)
	 ...
	 */
	ret = cis->ixc_ops->read8(client, 0x0018, &rev);
	if (ret < 0) {
...
int sensor_imx576_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	ret = cis->ixc_ops->read16(client, SENSOR_IMX576_ANALOG_GAIN_ADDR, &analog_gain);
	if (ret < 0)
		goto p_err;
...
int sensor_imx576_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	...
	ret = cis->ixc_ops->read16(client, SENSOR_IMX576_DIG_GAIN_ADDR, &digital_gain);
	if (ret < 0)
		goto p_err;
...
int sensor_imx576_cis_set_wb_gain(struct v4l2_subdev *subdev, struct wb_gains wb_gains)
{
	...
	ret = cis->ixc_ops->write16_array(client, SENSOR_IMX576_ABS_GAIN_GR_SET_ADDR, abs_gains, 4);
	if (ret < 0)
		goto p_err;
