...
int sensor_hi1336_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if (ret < 0) {
		err("sensor_hi1336_cis_group_param_hold fail ret = %d\n", ret);
		goto p_err;
	}
...
int sensor_hi1336_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	/********************************************************************************
	*	2020.02.17 - remove condtion for checking max integration time.
	*	In case of 'Variable Frame Time', coarse_integration_time is allowed over max coarse_integration_time.
