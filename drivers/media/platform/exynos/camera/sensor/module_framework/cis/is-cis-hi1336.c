...
int sensor_hi1336_cis_check_rev(struct is_cis *cis)
{
	...
	ret = cis->ixc_ops->read16(client, SENSOR_HI1336_MODEL_ID_ADDR, &data16);
	if (ret < 0) {
...
int sensor_hi1336_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read16(client, SENSOR_HI1336_STREAM_ONOFF_ADDR, &data16);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
...
int sensor_hi1336_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	/* Sensor stream on */
	ret = cis->ixc_ops->write16(client, SENSOR_HI1336_STREAM_ONOFF_ADDR,
			0x0001);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
...
int sensor_hi1336_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->write16(client, SENSOR_HI1336_STREAM_ONOFF_ADDR, 0x0000);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
				SENSOR_HI1336_STREAM_ONOFF_ADDR, ret);
...
int sensor_hi1336_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read16(client, SENSOR_HI1336_ISP_FRAME_CNT_ADDR,
			&cur_frame_value);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
...
int sensor_hi1336_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->write16(client, SENSOR_HI1336_FRAME_LENGTH_LINE_ADDR, frame_length_lines);
	if (ret < 0) {
		goto p_i2c_err;
	}
...
int sensor_hi1336_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->write16(client, SENSOR_HI1336_COARSE_INTEG_TIME_ADDR, coarse_int);
		if (ret < 0)
			goto p_i2c_err;
...
int sensor_hi1336_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	// Analog gain
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->write8(client, SENSOR_HI1336_ANALOG_GAIN_ADDR, analog_gain);
	if (ret < 0)
		goto p_i2c_err;
...
int sensor_hi1336_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->read8(client, SENSOR_HI1336_ANALOG_GAIN_ADDR, &analog_gain);
	if (ret < 0)
		goto p_i2c_err;
...
int sensor_hi1336_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	ret = cis->ixc_ops->write16_array(client, SENSOR_HI1336_DIG_GAIN_ADDR, dgains, 4);
	if (ret < 0) {
		goto p_i2c_err;
	}

	ret = cis->ixc_ops->read16(client, SENSOR_HI1336_ISP_ENABLE_ADDR, &read_val);
	if (ret < 0) {
		goto p_i2c_err;
	}

	enable_dgain = read_val | (0x1 << 4); // [4]: D gain enable
	ret = cis->ixc_ops->write16(client, SENSOR_HI1336_ISP_ENABLE_ADDR, enable_dgain);
	if (ret < 0) {
		goto p_i2c_err;
	}
...
int sensor_hi1336_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->read16(client, SENSOR_HI1336_DIG_GAIN_ADDR, &digital_gain);
	if (ret < 0)
		goto p_i2c_err;
