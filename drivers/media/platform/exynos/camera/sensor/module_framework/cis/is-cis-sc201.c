...
int sensor_sc201_cis_check_rev(struct v4l2_subdev *subdev)
{
	...
	u8 sensor_id_msb = 0, sensor_id_lsb = 0;
	...
	ret = cis->ixc_ops->read8(client, 0x3107, &sensor_id_msb);
	if (ret < 0)
		goto p_err;

	ret = cis->ixc_ops->read8(client, 0x3108, &sensor_id_lsb);
	if (ret < 0)
		goto p_err;

	cis->cis_data->cis_rev = (sensor_id_msb << 8) | sensor_id_lsb;
...
int sensor_sc201_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = sensor_cis_dump_registers(subdev, sensor_sc201_setfiles[0], sensor_sc201_setfile_sizes[0]);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
...
int sensor_sc201_cis_set_exposure_time(struct v4l2_subdev *subdev, u16 multiple_exp)
{
	...
	ret = cis->ixc_ops->write8(client, 0x320e, (frame_length_lines >> 8) & 0x7F);
	if (ret < 0)
		goto p_err;

	ret = cis->ixc_ops->write8(client, 0x320f, (frame_length_lines & 0xFF));
	if (ret < 0)
		goto p_err;

	ret = cis->ixc_ops->write8(client, 0x3e00, (multiple_exp >> 12) & 0x0F);
	if (ret < 0)
		goto p_err;

	ret = cis->ixc_ops->write8(client, 0x3e01, (multiple_exp >> 4) & 0xFF);
	if (ret < 0)
		goto p_err;

	ret = cis->ixc_ops->write8(client, 0x3e02, (multiple_exp << 4) & 0xF0);
	if (ret < 0)
		goto p_err;
...
int sensor_sc201_cis_set_analog_digital_gain(struct v4l2_subdev *subdev, u32 input_gain)
{
	...
	ret = cis->ixc_ops->write8(client, 0x3e09, total_again);
	if (ret < 0)
		goto p_err;

	ret = cis->ixc_ops->write8(client, 0x3e06, total_dgain);
	if (ret < 0)
		goto p_err;

	ret = cis->ixc_ops->write8(client, 0x3e07, total_fine_dgain);
	if (ret < 0)
		goto p_err;
...
int sensor_sc201_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	/* page_select */
	ret = cis->ixc_ops->write8(client, 0xfe, 0x00);
	if (ret < 0)
		goto p_err;

	ret = cis->ixc_ops->read8(client, 0xb6, &analog_gain);
	if (ret < 0)
		goto p_err;
...
int sensor_sc201_cis_compensate_gain_for_extremely_br(struct v4l2_subdev *subdev, u32 expo, u32 *again, u32 *dgain)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_sc201_cis_wait_streamon(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read8(client, 0x320e, &sensor_frame_length_lines_msb);
	if (ret < 0) {
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x320e, sensor_frame_length_lines_msb, ret);
		goto p_err;
	}

	ret = cis->ixc_ops->read8(client, 0x320f, &sensor_frame_length_lines_lsb);
	if (ret < 0) {
	...
	ret = cis->ixc_ops->read8(client, 0x320c, &sensor_line_length_pck_msb);
	if (ret < 0) {
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x320c, sensor_line_length_pck_msb, ret);
		goto p_err;
	}

	ret = cis->ixc_ops->read8(client, 0x320d, &sensor_line_length_pck_lsb);
	if (ret < 0) {
	...
	ret = cis->ixc_ops->read8(client, 0x3976, &sensor_fcount_lsb);
	if (ret < 0) {
...
int sensor_sc201_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read8(client, 0x0100, &streamoff);
	if (ret < 0) {
	...
	/* Checking stream off */
	do {
		ret = cis->ixc_ops->read8(client, 0x3974, &sensor_blc_data_high);
		if (ret < 0) {
		...
		ret = cis->ixc_ops->read8(client, 0x3975, &sensor_blc_data_mid);
		if (ret < 0) {
			err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x3975, sensor_blc_data_mid, ret);
			goto p_err;
		}

		ret = cis->ixc_ops->read8(client, 0x3976, &sensor_blc_data_low);
		if (ret < 0) {
...
static struct is_cis_ops cis_ops = {
	...
	.cis_compensate_gain_for_extremely_br = sensor_sc201_cis_compensate_gain_for_extremely_br,
...
#ifdef MODULE
builtin_i2c_driver(sensor_cis_sc201_driver);
#else
