...
int sensor_imx258_cis_init(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_imx258_cis_check_rev_on_init(subdev);
	if (ret < 0) {
		warn("Checking imx258 rev is failed during cis init");
		ret = -EINVAL;
		goto p_err;
	}
#endif
...
int sensor_imx258_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	/* Enable/Disable Embedded Data 0: Disable, 2: Enable */
	ret = is_sensor_write8(client, 0x4041, 0x00);
	if (ret < 0) {
