...
int sensor_2l4_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	pr_info("[%s] *******************************\n", __func__);
	ret = cis->ixc_ops->write16(client, 0xFCFC, 0x2000);
	if (unlikely(!ret)) pr_info("0x2000 page\n");
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0xEA2A, &data16);
	if (unlikely(!ret)) pr_info("0xEA2A(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0xEA2C, &data16);
	...
	ret = cis->ixc_ops->read16(client, 0x0002, &data16);
	...
	ret = cis->ixc_ops->read16(client, 0x0136, &data16);
	...
	ret = cis->ixc_ops->read16(client, 0x020E, &data16);
	if (unlikely(!ret)) pr_info("0x020E(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x021E, &data16);
	if (unlikely(!ret)) pr_info("0x021E(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x0226, &data16);
	if (unlikely(!ret)) pr_info("0x0226(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x0340, &data16);
	if (unlikely(!ret)) pr_info("0x0340(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x0700, &data16);
	if (unlikely(!ret)) pr_info("0x0700(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x0702, &data16);
	if (unlikely(!ret)) pr_info("0x0702(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x0A70, &data16);
	if (unlikely(!ret)) pr_info("0x0A70(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x0A72, &data16);
	if (unlikely(!ret)) pr_info("0x0A72(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x0A78, &data16);
	if (unlikely(!ret)) pr_info("0x0A78(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x0A7A, &data16);
	if (unlikely(!ret)) pr_info("0x0A7A(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x0A7C, &data16);
	if (unlikely(!ret)) pr_info("0x0A7C(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x0C80, &data16);
	if (unlikely(!ret)) pr_info("0x0C80(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read8(client, 0x3000, &data8);
...
int sensor_2l4_cis_retention_crc_enable(struct v4l2_subdev *subdev, u32 mode)
{
	...
	default:
		...
		cis->ixc_ops->write8(client, 0x010E, 0x01);
...
int sensor_2l4_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	cis->ixc_ops->write16(cis->client, 0x602A, 0x1090);
...
int sensor_2l4_cis_set_lownoise_mode_change(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->read8(cis->client, 0x0005, &sensor_2l4_ln_mode_frame_count);
	...
	case IS_CIS_LNOFF:
		...
		ret |= cis->ixc_ops->write16(cis->client, 0x602A, 0xFF22);
		ret |= cis->ixc_ops->write16(cis->client, 0x6F12, 0x0001);
		ret |= cis->ixc_ops->write16(cis->client, 0x3012, 0x0101);
		...
		case SENSOR_2L4_4032X1908_30FPS:
		case SENSOR_2L4_3024X3024_30FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x0FE0);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x00D0);
			break;
		case SENSOR_2L4_4032X3024_24FPS:
		case SENSOR_2L4_4032X2268_24FPS:
		case SENSOR_2L4_4032X1908_24FPS:
		case SENSOR_2L4_3024X3024_24FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x1660);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x00B0);
			break;
		}
#endif
	...
	case IS_CIS_LN2:
		...
		ret |= cis->ixc_ops->write16(cis->client, 0x602A, 0xFF22);
		ret |= cis->ixc_ops->write16(cis->client, 0x6F12, 0x0001);
		ret |= cis->ixc_ops->write16(cis->client, 0x3012, 0x0201);
		...
		case SENSOR_2L4_4032X1908_30FPS:
		case SENSOR_2L4_3024X3024_30FPS:
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7A, 0x0000);
			ret |= cis->ixc_ops->write16(cis->client, 0x0A7C, 0x00D0);
			break;
		case SENSOR_2L4_4032X3024_24FPS:
		case SENSOR_2L4_4032X2268_24FPS:
	...
	case IS_CIS_LN4:
		...
		ret |= cis->ixc_ops->write16(cis->client, 0x602A, 0xFF22);
		ret |= cis->ixc_ops->write16(cis->client, 0x6F12, 0x0001);
		ret |= cis->ixc_ops->write16(cis->client, 0x3012, 0x0401);
	...
	case IS_CIS_LN2_PEDESTAL128:
		...
		ret |= cis->ixc_ops->write16(cis->client, 0x0B30, 0x0200);
		ret |= cis->ixc_ops->write16(cis->client, 0x0BC0, 0x0080); /* pedestal 128 */

#ifdef CAMERA_REAR2
		ret |= cis->ixc_ops->write16(cis->client, 0x30D0, 0x0000);
		ret |= cis->ixc_ops->write16(cis->client, 0x30D2, 0x0500);
#endif
		ext_info->use_retention_mode = SENSOR_RETENTION_INACTIVE;
		break;
	case IS_CIS_LN4_PEDESTAL128:
		dbg_sensor(1, "[%s] IS_CIS_LN4_PEDESTAL128\n", __func__);
		ret |= cis->ixc_ops->write16(cis->client, 0x0B30, 0x0300);
		ret |= cis->ixc_ops->write16(cis->client, 0x0BC0, 0x0080); /* pedestal 128 */
...
int sensor_2l4_cis_stream_on(struct v4l2_subdev *subdev)
{
...
#ifdef DEBUG_2L4_PLL
	{
		...
		cis->ixc_ops->read16(client, 0x0314, &pll);
		dbg_sensor(1, "______ DRAM_pre_pll_clk_div(0x%x)\n", pll);
		cis->ixc_ops->read16(client, 0x0316, &pll);
		dbg_sensor(1, "______ DRAM_pll_multiplier(0x%x)\n", pll);
		cis->ixc_ops->read16(client, 0x0318, &pll);
...
int sensor_2l4_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	/* CIT shifter */
	if (cis->long_term_mode.sen_strm_off_on_enable == false) {
		ret = cis->ixc_ops->write16(client, 0x0702, coarse_integration_time_shifter);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_2l4_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	if (cis->long_term_mode.sen_strm_off_on_enable == false) {
		ret = cis->ixc_ops->write8(client, 0x0701, frame_length_lines_shifter);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_2l4_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* Long digital gain */
	if (sensor_2l4_cis_is_wdr_mode_on(cis_data)) {
		ret = cis->ixc_ops->write16(client, 0x0C80, long_gain);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_2l4_cis_set_frs_control(struct v4l2_subdev *subdev, u32 command)
{
	...
	case FRS_SSM_STOP:
		...
		ret |= cis->ixc_ops->write8(cis->client, 0x0A53, 0x01); /* stop_user_record */
		break;
	...
	case FRS_SSM_MODE_FACTORY_TEST:
		...
		ret |= cis->ixc_ops->write16(cis->client, 0x0A5A, 0x0010); /* before_q_frames = 16 */
		ret |= cis->ixc_ops->write8(cis->client, 0x0A52, 0x01); /* start_user_record */
		ret |= cis->ixc_ops->write16(cis->client, 0xF40A, 0x0009); /* test */
		ret |= cis->ixc_ops->write16(cis->client, 0xF404, 0xFFF7); /* test */
...
int sensor_2l4_cis_get_super_slow_motion_gmc(struct v4l2_subdev *subdev, u32 *gmc)
{
	...
	ret |= cis->ixc_ops->write16(cis->client, 0x602C, 0x2001);
	ret |= cis->ixc_ops->write16(cis->client, 0x602E, 0x0E34);
...
int sensor_2l4_cis_get_super_slow_motion_frame_id(struct v4l2_subdev *subdev, u32 *frameid)
{
	...
	ret |= cis->ixc_ops->read8(cis->client, 0x1A0C, &frame_id);
	if (ret < 0) {
...
int sensor_2l4_cis_set_factory_control(struct v4l2_subdev *subdev, u32 command)
{
	...
	case FAC_CTRL_BIT_TEST:
		...
		ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
		ret |= cis->ixc_ops->write16(cis->client, 0xF44A, 0x0009); // TG 2.55v -> 2.45v
		ret |= cis->ixc_ops->write16(cis->client, 0xF44C, 0x0009); // TG 2.55v -> 2.45v
		msleep(50);
		ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x2000);
		ret |= cis->ixc_ops->write16(cis->client, 0xAE16, 0x0010); // RG 3.70v -> 3.30v
		ext_info->use_retention_mode = SENSOR_RETENTION_INACTIVE;
		break;
	default:
		pr_info("[%s] not support command(%d)\n", __func__, command);
...
static int cis_2l4_probe_i2c(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	if (strcmp(setfile, "default") == 0 ||
		...
		sensor_2l4_load_sram_size = sensor_2l4_setfile_A_sizes_load_sram;
#endif
		cis->mipi_sensor_mode = sensor_2l4_setfile_A_mipi_sensor_mode;
		cis->mipi_sensor_mode_size = ARRAY_SIZE(sensor_2l4_setfile_A_mipi_sensor_mode);
		verify_sensor_mode = sensor_2l4_setfile_A_verify_sensor_mode;
		verify_sensor_mode_size = ARRAY_SIZE(sensor_2l4_setfile_A_verify_sensor_mode);
	} else {
...
#ifdef MODULE
builtin_i2c_driver(sensor_cis_2l4_driver);
#else
