...
int sensor_2l2_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->write16(client, 0x6028, 0x2000);
	cis->ixc_ops->write16(client, 0x602A, 0x0ACD);
	cis->ixc_ops->write8(client, 0x6F12, 0x01);
...
int sensor_2l2_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* Long digital gain */
	if (is_vender_wdr_mode_on(cis_data)) {
		ret = cis->ixc_ops->write16(client, 0x3072, long_gain);
		if (ret < 0)
			goto p_err;
	}
