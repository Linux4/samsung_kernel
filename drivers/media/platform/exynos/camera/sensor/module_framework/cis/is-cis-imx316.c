...
int sensor_imx316_cis_check_rev(struct is_cis *cis)
{
	...
	ret = cis->ixc_ops->read8(client, 0x0001, &rev);
	...
	ret = cis->ixc_ops->read8(client, 0x0003, &rev);
	info("sensor_imx316_cis_check_rev 0x0003 %x ", rev);
	ret = cis->ixc_ops->read8(client, 0x0019,&rev);
...
int sensor_imx316_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->read16(client, 0x0001, &data16);
	pr_err("[SEN:DUMP] manufacturer ID(%x)\n", data16);
	cis->ixc_ops->read8(client, SENSOR_DEPTH_MAP_NUMBER, &data8);
	pr_err("[SEN:DUMP] frame counter (%x)\n", data8);
	cis->ixc_ops->read8(client, 0x1001, &data8);
...
int sensor_imx316_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->read8(client, SENSOR_DEPTH_MAP_NUMBER, &sensor_fcount);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
...
#endif
	cis->ixc_ops->write8(client, 0x1001, 0x01);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
...
int sensor_imx316_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->write8(client, 0x1001, 0x00);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
...
int sensor_imx316_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	value = value_32 & 0xFF;
	cis->ixc_ops->write8(client, SENSOR_EXTAREA_INTG_0_A_3, value);
	cis->ixc_ops->write8(client, SENSOR_EXTAREA_INTG_0_B_3, value);
	value = (value_32 & 0xFF00) >> 8;
	cis->ixc_ops->write8(client, SENSOR_EXTAREA_INTG_0_A_2, value);
	cis->ixc_ops->write8(client, SENSOR_EXTAREA_INTG_0_B_2, value);
	value = (value_32 & 0xFF0000) >> 16;
	cis->ixc_ops->write8(client, SENSOR_EXTAREA_INTG_0_A_1, value);
	cis->ixc_ops->write8(client, SENSOR_EXTAREA_INTG_0_B_1, value);
	value = (value_32 & 0xFF000000) >> 24;
	cis->ixc_ops->write8(client, SENSOR_EXTAREA_INTG_0_A_0, value);
	cis->ixc_ops->write8(client, SENSOR_EXTAREA_INTG_0_B_0, value);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
...
int sensor_imx316_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 duration)
{
	...
	sensor_imx316_frame_duration = duration;
	I2C_MUTEX_LOCK(cis->i2c_lock);
	cis->ixc_ops->write8(client, 0x0102, 0x01);

	info("%s : set %d", __func__, duration);
	switch (duration) {
		case SENSOR_FPS_5:
			cis->ixc_ops->write8(client, 0x2154, 0x36);
			cis->ixc_ops->write8(client, 0x2155, 0x3C);
			break;
		case SENSOR_FPS_10:
			cis->ixc_ops->write8(client, 0x2154, 0x17);
			cis->ixc_ops->write8(client, 0x2155, 0xBC);
			break;
		case SENSOR_FPS_15:
			cis->ixc_ops->write8(client, 0x2154, 0x0D);
			cis->ixc_ops->write8(client, 0x2155, 0x92);
			break;
		case SENSOR_FPS_20:
			cis->ixc_ops->write8(client, 0x2154, 0x08);
			cis->ixc_ops->write8(client, 0x2155, 0x7E);
			break;
		case SENSOR_FPS_30:
			cis->ixc_ops->write8(client, 0x2154, 0x03);
			cis->ixc_ops->write8(client, 0x2155, 0x68);
			break;
		default:
	...
	cis->ixc_ops->write8(client, 0x0102, 0x00);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
...
int sensor_imx316_cis_set_laser_control(struct v4l2_subdev *subdev, u32 onoff)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	if (onoff) {
		cis->ixc_ops->write8(client, SENSOR_LASER_CONTROL_REG_1, SENSOR_LASER_ON);
		cis->ixc_ops->write8(client, SENSOR_LASER_CONTROL_REG_2, SENSOR_LASER_ON);
	} else {
		cis->ixc_ops->write8(client, SENSOR_LASER_CONTROL_REG_1, SENSOR_LASER_OFF);
		cis->ixc_ops->write8(client, SENSOR_LASER_CONTROL_REG_2, SENSOR_LASER_OFF);
	}
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
...
int sensor_imx316_cis_set_vcsel_current(struct v4l2_subdev *subdev, u32 value)
{
	...
	cis->ixc_ops->write8(client, 0x0436, 0x00);
	cis->ixc_ops->write8(client, 0x0437, 0x00);
	cis->ixc_ops->write8(client, 0x043A, 0x02);
	cis->ixc_ops->write8(client, 0x0500, 0x02);
	cis->ixc_ops->write8(client, 0x0501, 0x08);
	cis->ixc_ops->write8(client, 0x0502, value8);

	cis->ixc_ops->write8(client, 0x0431, 0x01);
	cis->ixc_ops->write8(client, 0x0430, 0x01);
	cis->ixc_ops->write8(client, 0x0431, 0x00);
	...
	if (module->position == SENSOR_POSITION_REAR_TOF) {  /* for setting IPD_OFFSET */
		...
		cis->ixc_ops->write8(client, 0x043A, 0x02);
		cis->ixc_ops->write8(client, 0x0500, 0x02);
		cis->ixc_ops->write8(client, 0x0501, 0x0D);
		cis->ixc_ops->write8(client, 0x0502, 0xC3);
		cis->ixc_ops->write8(client, 0x0431, 0x01);
		cis->ixc_ops->write8(client, 0x0430, 0x01);
		cis->ixc_ops->write8(client, 0x0431, 0x00);
	}
...
int sensor_imx316_cis_get_vcsel_photo_diode(struct v4l2_subdev *subdev, u16 *value)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	cis->ixc_ops->write8(client, 0x0421, 0x00);
	cis->ixc_ops->write8(client, 0x0436, 0x00);
	cis->ixc_ops->write8(client, 0x0437, 0x00);
	cis->ixc_ops->write8(client, 0x043A, 0x01);
	cis->ixc_ops->write8(client, 0x0500, 0x11);
	cis->ixc_ops->write8(client, 0x0501, 0x90);
	cis->ixc_ops->write8(client, 0x0431, 0x01);
	cis->ixc_ops->write8(client, 0x0430, 0x01);
	cis->ixc_ops->write8(client, 0x0431, 0x00);

	cis->ixc_ops->read8(client, 0x058B, &value8_1); /*0x058B for APC2_CHECK_DATA[9:8]*/

	cis->ixc_ops->write8(client, 0x0423, 0xA0);
	cis->ixc_ops->write8(client, 0x0436, 0x00);
	cis->ixc_ops->write8(client, 0x0437, 0x00);
	cis->ixc_ops->write8(client, 0x043A, 0x01);
	cis->ixc_ops->write8(client, 0x0500, 0x07);
	cis->ixc_ops->write8(client, 0x0501, 0xA0);
	cis->ixc_ops->write8(client, 0x0431, 0x01);
	cis->ixc_ops->write8(client, 0x0430, 0x01);
	cis->ixc_ops->write8(client, 0x0431, 0x00);
	cis->ixc_ops->read8(client, 0x0582, &value8_0); /* 0x0582 for APC2_CHECK_DATA[7:0] */
