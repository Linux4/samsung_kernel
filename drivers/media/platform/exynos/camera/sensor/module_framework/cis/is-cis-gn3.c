...
static u32 sensor_gn3_max_load_sram_num;
...
int sensor_gn3_cis_select_setfile(struct v4l2_subdev *subdev)
{
	...
	info("[%s] dual sync pin - gpio%d\n", __func__, (sensor_gn3_select_gpio1 ? 1 : 2));
#endif

#ifdef USE_SENSOR_TEST_SETTING
	if (is_vener_ts_get_sensor_test_flag(SENSOR_NAME_S5KGN3)) {
		is_vender_ts_get_setting_for_test(SENSOR_NAME_S5KGN3,
			&sensor_gn3_global,
			&sensor_gn3_global_size,
			&sensor_gn3_setfiles,
			&sensor_gn3_setfile_sizes,
			&sensor_gn3_pllinfos);
#if IS_ENABLED(USE_CAMERA_SENSOR_RETENTION)
		is_vender_test_sensor_get_retention_setting_for_test(SENSOR_NAME_S5KGN3,
			&sensor_gn3_global_retention,
			&sensor_gn3_global_retention_size,
			&sensor_gn3_retention,
			&sensor_gn3_retention_size,
			&sensor_gn3_load_sram,
			&sensor_gn3_load_sram_size);
#endif
	}
#endif
...
int sensor_gn3_cis_update_seamless_mode(struct v4l2_subdev *subdev)
{
	...
	if (sensor_gn3_seamless_delay_flag == true) {
...
int sensor_gn3_cis_retention_prepare(struct v4l2_subdev *subdev)
{
	...
	ret |= is_sensor_write16(cis->client, 0x0F9A, 0x5FC4);
	ret |= is_sensor_write16(cis->client, 0x0F9C, 0x6668);
	ret |= is_sensor_write16(cis->client, 0x0F9E, 0x6D0C);
	ret |= is_sensor_write16(cis->client, 0x0FA0, 0x73B0);
	ret |= is_sensor_write16(cis->client, 0x0FA2, 0x7A54);
	ret |= is_sensor_write16(cis->client, 0x0FA4, 0xA3D0);
	ret |= is_sensor_write16(cis->client, 0x0FA6, 0xAA74);
	ret |= is_sensor_write16(cis->client, 0x0FA8, 0xB118);
	ret |= is_sensor_write16(cis->client, 0xFCFC, 0x4000);
...
int sensor_gn3_cis_retention_crc_check(struct v4l2_subdev *subdev)
{
	...
	if (crc_check == 0x0100) {
		...
		is_sensor_write16(cis->client, 0x0620, 0x0000);
	} else {
...
int sensor_gn3_cis_wait_seamless_update_delay(struct v4l2_subdev *subdev)
{
	...
	if (sensor_gn3_seamless_delay_count > 0) {
		sensor_gn3_seamless_delay_flag = true;
		info("%s: sensor_gn3_seamless_delay_count : %d , cis_data->cur_frame_us_time(%d)\n",
			__func__, sensor_gn3_seamless_delay_count, cis_data->cur_frame_us_time);

		if (cis_data->cur_frame_us_time > 300000 && cis_data->cur_frame_us_time < 2000000)
			// for Hyperlapse night mode
			msleep(cis_data->cur_frame_us_time * sensor_gn3_seamless_delay_count / 1000);
		else
...
int sensor_gn3_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	...
	if (vt_pic_clk_freq_khz == 0) {
		err("[MOD:D:%d] Invalid vt_pic_clk_freq_khz(%d)", cis->id, vt_pic_clk_freq_khz/1000);
		goto p_err;
	}
	line_length_pck = cis_data->line_length_pck;
...
int sensor_gn3_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	if (cis->long_term_mode.sen_strm_off_on_enable == false) {
		...
		if (frame_duration < 33333
			&& sensor_gn3_mode_groups[SENSOR_GN3_MODE_LN2] != MODE_GROUP_NONE
			&& cis_data->sens_config_index_cur == sensor_gn3_mode_groups[SENSOR_GN3_MODE_LN2]) {
...
int sensor_gn3_cis_set_test_pattern(struct v4l2_subdev *subdev, camera2_sensor_ctl_t *sensor_ctl)
{
	...
	if (cis->cis_data->cur_pattern_mode != sensor_ctl->testPatternMode) {
		if (sensor_ctl->testPatternMode == SENSOR_TEST_PATTERN_MODE_OFF) {
			...
			is_sensor_write16(client, 0x0620, 0x0000);
			I2C_MUTEX_UNLOCK(cis->i2c_lock);
			...
			is_sensor_write16(client, 0x0620, 0x0001);
			I2C_MUTEX_UNLOCK(cis->i2c_lock);
	...
	const struct i2c_device_id *id)
{
	...
	if (mclk_freq_khz == 19200) {
		if (strcmp(setfile, "default") == 0 ||
			...
			sensor_gn3_max_load_sram_num = ARRAY_SIZE(sensor_gn3_setfile_A_19p2_sizes_load_sram);
#endif
		} else {
		...
		if (strcmp(setfile, "default") == 0 ||
			...
			sensor_gn3_max_load_sram_num = ARRAY_SIZE(sensor_gn3_setfile_A_sizes_load_sram);
#endif
		} else {
...
#ifdef USE_SENSOR_TEST_SETTING
	is_vender_ts_set_default_setting_for_test(SENSOR_NAME_S5KGN3,
		sensor_gn3_global,
		sensor_gn3_global_size,
		sensor_gn3_setfiles,
		sensor_gn3_setfile_sizes,
		(void *)sensor_gn3_pllinfos,
		sensor_gn3_max_setfile_num);

#if IS_ENABLED(USE_CAMERA_SENSOR_RETENTION)
	is_vender_ts_set_default_retention_setting_for_test(SENSOR_NAME_S5KGN3,
		sensor_gn3_global_retention,
		sensor_gn3_global_retention_size,
		sensor_gn3_retention,
		sensor_gn3_retention_size,
		sensor_gn3_max_retention_num,
		sensor_gn3_load_sram,
		sensor_gn3_load_sram_size,
		sensor_gn3_max_load_sram_num);
#endif
#endif
