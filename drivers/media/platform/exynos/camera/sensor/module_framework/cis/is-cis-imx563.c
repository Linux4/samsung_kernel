...
int sensor_imx563_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	if (sensor_imx563_cis_is_wdr_mode_on(cis_data)) {
	 ...
	 if (sensor_imx563_cis_is_wdr_mode_on(cis_data)) {
		/* Short exposure */
		ret = cis->ixc_ops->write16(client, 0x0224, short_coarse_int);
		if (ret < 0)
			goto p_err_i2c_unlock;
		/* Long exposure */
		ret = cis->ixc_ops->write16(client, 0x0202, long_coarse_int);
		if (ret < 0)
			goto p_err_i2c_unlock;
	} else {
		/* Short exposure */
		...
		if (cis_data->sens_config_index_cur == SENSOR_IMX563_2016X1136_480FPS
			...
			ret |= cis->ixc_ops->write16(client, 0x0E10, short_coarse_int);
			ret |= cis->ixc_ops->write16(client, 0x0E28, short_coarse_int);
		}
#endif
	}
	...
	/* CIT shifter */
	if (cis->long_term_mode.sen_strm_off_on_enable == false) {
		ret = cis->ixc_ops->write8(client, 0x3100, coarse_integration_time_shifter);
		if (ret < 0)
			goto p_err_i2c_unlock;
	}
...
int sensor_imx563_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	if (cis->long_term_mode.sen_strm_off_on_enable == false) {
		ret = cis->ixc_ops->write8(client, 0x3101, frame_length_lines_shifter);
		if (ret < 0)
			goto p_err_i2c_unlock;
	}
...
int sensor_imx563_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	if (cis->cis_data->sens_config_index_cur == SENSOR_IMX563_2016X1136_480FPS
		...
		ret |= cis->ixc_ops->write16(client, 0x0E12, analog_gain);
		ret |= cis->ixc_ops->write16(client, 0x0E2A, analog_gain);
	}
#endif
...
int sensor_imx563_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	if (cis_data->sens_config_index_cur == SENSOR_IMX563_2016X1136_480FPS
		|| cis_data->sens_config_index_cur == SENSOR_IMX563_1280X720_480FPS) {
		ret |= cis->ixc_ops->write16(client, 0x0E14, short_gain);
		ret |= cis->ixc_ops->write16(client, 0x0E2C, short_gain);
	}
#endif
