...
int sensor_gc02m2_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	return 0;
}
#else
	...
	const struct i2c_device_id *id)
{
	...
	sensor_peri = find_peri_by_cis_id(device, SENSOR_NAME_GC02M2);
	if (!sensor_peri) {
	...
	if (of_property_read_bool(dnode, "use_sensor_otp")) {
		...
		if (ret) {
			...
			if (vender_rom_addr[rom_position]) {
				...
				probe_info("%s: GC02M2 OTP addrress not defined!\n", __func__);
			}
		}
	}
#else
