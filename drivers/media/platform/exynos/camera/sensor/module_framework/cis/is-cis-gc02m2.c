...
static u16 sensor_gc02m2_fcount;
...
int sensor_gc02m2_cis_check_rev(struct v4l2_subdev *subdev)
{
	...
	if (ret < 0) {
		err("set sensor_gc02m2_otp_read fail!!");
		goto p_err;
	}
...
u16 sensor_gc02m2_cis_get_framecount(struct is_cis *cis)
{
	struct i2c_client *client;
	u16 fcount = 0xffff;
	int ret = 0;
...
int sensor_gc02m2_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = sensor_cis_dump_registers(subdev, sensor_gc02m2_setfiles[0], sensor_gc02m2_setfile_sizes[0]);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
...
int sensor_gc02m2_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (ret < 0) {
		err("sensor_gc02m2_setfiles fail!!");
		goto p_i2c_err;
	}
...
int sensor_gc02m2_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	sensor_gc02m2_fcount = sensor_gc02m2_cis_get_framecount(cis);
	...
	info("%s done, frame_count(%d) cur_frame_ms_time(%d)\n",
		__func__, sensor_gc02m2_fcount, cis_data->cur_frame_us_time/1000);
...
int sensor_gc02m2_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	...
	do {
		sensor_fcount = sensor_gc02m2_cis_get_framecount(cis);
...
int sensor_gc02m2_cis_wait_streamon(struct v4l2_subdev *subdev)
{
	...
	cur_frame_count = sensor_gc02m2_cis_get_framecount(cis);

	/* Checking stream on */
	do {
		next_frame_count = sensor_gc02m2_cis_get_framecount(cis);
...
static const struct of_device_id sensor_cis_gc02m2_match[] = {
	{
		.compatible = "samsung,exynos-is-cis-gc02m2-macro",
	},
	{},
};
...
#ifdef MODULE
builtin_i2c_driver(sensor_cis_gc02m2_driver);
#else
