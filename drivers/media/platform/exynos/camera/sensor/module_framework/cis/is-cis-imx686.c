...
static int sensor_imx686_get_fine_integration_value(struct is_cis *cis, u16 *fine_integ)
{
	...
	ret = cis->ixc_ops->read16(client, SENSOR_IMX686_FINE_INTEG_TIME_ADDR, &fine_integ_time);
	if (ret < 0) {
...
static int sensor_imx686_set_ebd(struct is_cis *cis)
{
	...
	ret = cis->ixc_ops->read8(client, SENSOR_IMX686_EBD_CONTROL_ADDR, &enabled_ebd);
	if (ret < 0) {
		err("is_sensor_read is fail, (ret %d)", ret);
		ret = -EAGAIN;
		goto p_err;
	...
	ret = cis->ixc_ops->write8(client, SENSOR_IMX686_EBD_CONTROL_ADDR, enabled_ebd);
	if (ret < 0) {
		err("is_sensor_read is fail, (ret %d)", ret);
		ret = -EAGAIN;
		goto p_err;
...
int sensor_imx686_cis_check_rev(struct is_cis *cis)
{
	...
	ret = cis->ixc_ops->write8(client, SENSOR_IMX686_OTP_PAGE_SETUP_ADDR, 0x7F);
	if (ret < 0) {
	...
	ret = cis->ixc_ops->write8(client, SENSOR_IMX686_OTP_READ_TRANSFER_MODE_ADDR,  0x01);
	if (ret < 0) {
		err("is_sensor_read is fail, (ret %d)", ret);
		ret = -EAGAIN;
		goto i2c_err;
	...
	ret = cis->ixc_ops->read8(client, SENSOR_IMX686_OTP_STATUS_REGISTER_ADDR,  &status);
	if (ret < 0) {
		err("is_sensor_read is fail, (ret %d)", ret);
		ret = -EAGAIN;
		goto i2c_err;
	...
	/* CHIP REV 0x0018 */
	ret = cis->ixc_ops->read8(client, SENSOR_IMX686_OTP_CHIP_REVISION_ADDR, &rev);
	if (ret < 0) {
		err("is_sensor_read is fail, (ret %d)", ret);
		ret = -EAGAIN;
		goto i2c_err;
...
int sensor_imx686_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read16(client, SENSOR_IMX686_MODEL_ID_ADDR, &data16);
	if (unlikely(!ret)) pr_info("model_id(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read8(client, SENSOR_IMX686_REVISION_NUM_ADDR, &data8);
	if (unlikely(!ret)) pr_info("revision_number(0x%x)\n", data8);
	else goto i2c_err;
	ret = cis->ixc_ops->read8(client, SENSOR_IMX686_FRAME_COUNT_ADDR, &data8);
	if (unlikely(!ret)) pr_info("frame_count(0x%x)\n", data8);
	else goto i2c_err;
	ret = cis->ixc_ops->read8(client, SENSOR_IMX686_SETUP_MODE_SELECT_ADDR, &data8);
	if (unlikely(!ret)) pr_info("setup_mode(streaming)(0x%x)\n", data8);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, SENSOR_IMX686_COARSE_INTEG_TIME_ADDR, &data16);
	if (unlikely(!ret)) pr_info("coarse_integration_time(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, SENSOR_IMX686_ANALOG_GAIN_ADDR, &data16);
	if (unlikely(!ret)) pr_info("gain_code_global(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, SENSOR_IMX686_FRAME_LENGTH_LINE_ADDR, &data16);
	if (unlikely(!ret)) pr_info("frame_length_line(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read8(client, SENSOR_IMX686_LINE_LENGTH_PCK_ADDR, &data8);
...
static int sensor_imx686_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->write8(client, SENSOR_IMX686_GROUP_PARAM_HOLD_ADDR, hold);
	if (ret < 0)
		goto i2c_err;
...
int sensor_imx686_cis_stream_on(struct v4l2_subdev *subdev)
{
...

	{
		...
		cis->ixc_ops->read8(client, 0x0110, &value);
		info("IMX686: CSI_CH_ID(%x)\n", value);
		cis->ixc_ops->read8(client, 0x3076, &value);
		info("IMX686: PDAF_L_VCID(%x)\n", value);
		cis->ixc_ops->read16(client, 0x0112, &data_format);
		info("IMX686: CSI_DT_FMT(%x)\n", data_format);
		cis->ixc_ops->read8(client, 0x3077, &value);
		info("IMX686: PD_L_DT(%x)\n", value);
		cis->ixc_ops->read8(client, 0x3400, &value);
		info("IMX686: PDAF_TYPE_SEL(%x)\n", value);
		cis->ixc_ops->read8(client, 0x3092, &value);
		info("IMX686: PDAF_OUT_EN(%x)\n", value);
		cis->ixc_ops->read8(client, 0x401B, &value);
		info("IMX686: PDAF_Y_OUT_EN(%x)\n", value);
		cis->ixc_ops->read8(client, 0x4018, &value);
		info("IMX686: PDAF_X_OUT_SIZE_H(%x)\n", value);
		cis->ixc_ops->read8(client, 0x4019, &value);
		info("IMX686: PDAF_X_OUT_SIZE_L(%x)\n", value);
		cis->ixc_ops->read8(client, 0xBCF1, &value);
		info("IMX686: EBD_SIZE_V(%x)\n", value);
		cis->ixc_ops->read8(client, 0x4107, &value);
	...
	/* Sensor stream on */
	ret = cis->ixc_ops->write8(client, SENSOR_IMX686_SETUP_MODE_SELECT_ADDR, 0x01);
	if (ret < 0)
...
int sensor_imx686_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	/* Sensor stream off */
	ret = cis->ixc_ops->write8(client, SENSOR_IMX686_SETUP_MODE_SELECT_ADDR, 0x00);
	if (ret < 0)
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0100, 0x0000, ret);
...
int sensor_imx686_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	ret = cis->ixc_ops->write16(client, SENSOR_IMX686_FRAME_LENGTH_LINE_ADDR, frame_length_lines);
	if (ret < 0)
		goto i2c_err;
...
int sensor_imx686_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	/* Short exposure */
	ret = cis->ixc_ops->write16(client, SENSOR_IMX686_COARSE_INTEG_TIME_ADDR, short_coarse_int);
	if (ret < 0)
		goto i2c_err;
...
int sensor_imx686_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	ret = cis->ixc_ops->write16(client, SENSOR_IMX686_ANALOG_GAIN_ADDR, analog_gain);
	if (ret < 0)
		goto i2c_err;
...
int sensor_imx686_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	ret = cis->ixc_ops->read16(client, SENSOR_IMX686_ANALOG_GAIN_ADDR, &analog_gain);
	if (ret < 0)
		goto i2c_err;
...
int sensor_imx686_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	...
	ret = cis->ixc_ops->read16(client, SENSOR_IMX686_DIG_GAIN_ADDR, &digital_gain);
	if (ret < 0)
		goto i2c_err;
...
#define ABS_WB_GAIN_NUM	4
	...
	for (index=0; index < ABS_WB_GAIN_NUM; index++) {
		ret = cis->ixc_ops->write16(client, (SENSOR_IMX686_ABS_GAIN_GR_SET_ADDR + (index * 2)), abs_gains[index]);
		if (ret < 0) {
