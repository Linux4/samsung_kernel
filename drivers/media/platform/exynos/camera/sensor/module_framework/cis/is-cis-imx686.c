...
static void sensor_imx686_set_coarse_integration_min(u32 mode, cis_shared_data *cis_data)
{
	...
	switch (mode) {
		...
		case SENSOR_IMX686_2X2BIN_4000X3000_30FPS:
		...
		case SENSOR_IMX686_V2H2_2120X1188_240FPS:
...
#define STREAM_OFF_WAIT_TIME 500
	...
	if (timeout == STREAM_OFF_WAIT_TIME) {
		pr_err("ERR[%s]: actual stream off wait timeout\n", __func__);
		ret = -1;
	}
...
int sensor_imx686_cis_check_rev(struct is_cis *cis)
{
	...
	/* Specify OTP Page Address for READ - Page159(dec) */
	ret = is_sensor_write8(client, SENSOR_IMX686_OTP_PAGE_SETUP_ADDR, 0x9F);
	if (ret < 0) {
	...
	if ((status & 0x1) == false)
		err("OTP read status check fail, (%d)", status);
...
***********************************************************************/
	...
	if (SENSOR_IMX686_MP_VERSION(cis)) {
		probe_info("%s: CHIP REV is MP version !!! global setting is changed\n", __func__);
		sensor_imx686_global = sensor_imx686_setfile_B_global;
		sensor_imx686_global_size = ARRAY_SIZE(sensor_imx686_setfile_B_global);
	} else {
		probe_info("%s: CHIP REV is ES version !!!\n", __func__);
	}
...
int sensor_imx686_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	ret = is_sensor_read16(client, SENSOR_IMX686_LINE_LENGTH_PCK_ADDR, &data16);
...
i2c_err:
	...
	msleep(10);		//for prevent CSIS_ERR during mode change
...
int sensor_imx686_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	cis_data->max_coarse_integration_time =
		SENSOR_IMX686_MAX_COARSE_INTEG_WITH_FRM_LENGTH_CTRL - cis_data->max_margin_coarse_integration_time;
...
int sensor_imx686_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	...
	max_coarse = SENSOR_IMX686_MAX_COARSE_INTEG_WITH_FRM_LENGTH_CTRL - max_coarse_margin;
...
int sensor_imx686_cis_long_term_exposure(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
	struct is_long_term_expo_mode *lte_mode;
	...
	if (lte_mode->sen_strm_off_on_enable) {
		...
		if (target_exp >= 125000 ) {
			...
			ret = is_sensor_write8(cis->client, SENSOR_IMX686_CIT_LSHIFT_ADDR, cit_lshift_count);
		}
	} else {
		cit_lshift_count = 0;
		ret = is_sensor_write8(cis->client, SENSOR_IMX686_CIT_LSHIFT_ADDR, cit_lshift_count);
...
static struct is_cis_ops cis_ops_imx686 = {
	...
	.cis_set_long_term_exposure = sensor_imx686_cis_long_term_exposure,
	...
	const struct i2c_device_id *id)
{
	...
	if (strcmp(setfile, "default") == 0 || strcmp(setfile, "setA") == 0) {
		...
		sensor_imx686_global = sensor_imx686_setfile_ES_A_global;
		sensor_imx686_global_size = ARRAY_SIZE(sensor_imx686_setfile_ES_A_global);
		...
		sensor_imx686_global = sensor_imx686_setfile_ES_B_global;
		sensor_imx686_global_size = ARRAY_SIZE(sensor_imx686_setfile_ES_B_global);
		...
		err("%s setfile index out of bound, take setfile_B", __func__);
		sensor_imx686_global = sensor_imx686_setfile_ES_B_global;
		sensor_imx686_global_size = ARRAY_SIZE(sensor_imx686_setfile_ES_B_global);
		sensor_imx686_setfiles = sensor_imx686_setfiles_A;
		sensor_imx686_setfile_sizes = sensor_imx686_setfile_A_sizes;
