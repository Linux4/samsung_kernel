...
int sensor_imx582_cis_check_rev(struct is_cis *cis)
{
	...
	cis->ixc_ops->write8(client, SENSOR_IMX582_OTP_PAGE_SETUP_ADDR, 0x7F);

	/* Turn ON OTP Read MODE */
	cis->ixc_ops->write8(client, SENSOR_IMX582_OTP_READ_TRANSFER_MODE_ADDR,  0x01);

	/* Check status - 0x01 : read ready*/
	cis->ixc_ops->read8(client, SENSOR_IMX582_OTP_STATUS_REGISTER_ADDR,  &status);
	if ((status & 0x1) == false)
		err("status fail, (%d)", status);

	/* CHIP REV 0x0018 */
	ret = cis->ixc_ops->read8(client, SENSOR_IMX582_OTP_CHIP_REVISION_ADDR, &rev);
	if (ret < 0) {
...
u32 sensor_imx582_cis_get_fineIntegTime(struct is_cis *cis)
{
	...
	ret = cis->ixc_ops->read16(client, SENSOR_IMX582_FINE_INTEG_TIME_ADDR, &fine_integ_time);
	if (ret < 0) {
...
int sensor_imx582_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->write16(client, SENSOR_IMX582_FRAME_LENGTH_LINE_ADDR, frame_length_lines);
	if (ret < 0) {
		goto p_i2c_err;
	}
...
int sensor_imx582_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->write16(client, SENSOR_IMX582_COARSE_INTEG_TIME_ADDR, coarse_int);
		if (ret < 0)
			goto p_i2c_err;
...
int sensor_imx582_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	// Analog gain
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->write16(client, SENSOR_IMX582_ANALOG_GAIN_ADDR, analog_gain);
	if (ret < 0)
		goto p_i2c_err;
...
int sensor_imx582_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->read16(client, SENSOR_IMX582_ANALOG_GAIN_ADDR, &analog_gain);
	if (ret < 0)
		goto p_i2c_err;
...
int sensor_imx582_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->read16(client, SENSOR_IMX582_DIG_GAIN_ADDR, &digital_gain);
	if (ret < 0)
		goto p_i2c_err;
...
int sensor_imx582_cis_set_wb_gain(struct v4l2_subdev *subdev, struct wb_gains wb_gains)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = cis->ixc_ops->write16_array(client, SENSOR_IMX582_ABS_GAIN_GR_SET_ADDR, abs_gains, 4);
	if (ret < 0)
		goto p_i2c_err;
