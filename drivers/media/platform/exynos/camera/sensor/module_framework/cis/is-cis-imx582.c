...
***********************************************************************/
	...
	switch (cis->cis_data->cis_rev) {
		case SENSOR_IMX582_CHIP_ID_VER_6_0_0x0D:
						...
						u32 *target_duration)
{
	...
	dbg_sensor(1, "[%s](vsync cnt = %d) input exp(%d), calcurated frame duraion(%d), min_frame_us(%d), max_frame_us_time(%d)\n",
...
int sensor_imx582_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	dbg_sensor(1, "[MOD:D:%d] %s, pixelrate(%llu), llp(%d), fll(%d), cit(%d)\n",  cis->id, __func__,
