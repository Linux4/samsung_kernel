...
int sensor_fp5529_init(struct i2c_client *client, struct is_caldata_list_fp5529 *cal_data)
{
	...
	if (!cal_data) {
		...
		/* wait 5ms after power-on for ff5529 */
		...
		struct is_actuator *actuator)
{
	...
	if (init_step > 0) {
		for (i = 0; i < init_step; i++) {
			ret = sensor_fp5529_write_position(client, sysfs_actuator.init_positions[i]);
			if (ret < 0)
				goto p_err;
		...
		ret = sensor_fp5529_write_position(client, DEF_FP5529_FIRST_POSITION);
		if (ret < 0)
			goto p_err;

		msleep(DEF_FP5529_FIRST_DELAY);

		ret = sensor_fp5529_write_position(client, DEF_FP5529_SECOND_POSITION);
		if (ret < 0)
			goto p_err;
...
int sensor_fp5529_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	ret = sensor_fp5529_init(client, cal_data);
	if (ret < 0){
#ifdef USE_CAMERA_HW_BIG_DATA
...
int sensor_fp5529_actuator_soft_landing(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_actuator *actuator;
...
int sensor_fp5529_actuator_set_position(struct v4l2_subdev *subdev, u32 *info)
{
	...
	/* position Set */
	ret = sensor_fp5529_write_position(client, position);
	if (ret <0)
		goto p_err;
	actuator->position = position;
...
int sensor_fp5529_actuator_probe(struct i2c_client *client,
...
static struct i2c_driver actuator_fp5529_driver = {
	...
	.probe	= sensor_fp5529_actuator_probe,
