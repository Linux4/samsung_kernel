...
int sensor_gt9778_init(struct v4l2_subdev *subdev, struct is_caldata_sac_gt9778 *cal_data)
{
	...
	info("[%s]AF Cal data: sac_mode=0x%2x\n", __func__, sac_mode);
	info("[%s]AF Cal data: sac_time=0x%2x\n", __func__, sac_time);
...
int sensor_gt9778_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	/*if (lib->binary_load_flg) {
		...
		cal_addr = minfo->kvaddr_cal[SENSOR_POSITION_REAR] + REAR_CAL_AF_PAN_ADDR;
...
int sensor_gt9778_actuator_get_status(struct v4l2_subdev *subdev, u32 *info)
{
	...
	I2C_MUTEX_UNLOCK(actuator->i2c_lock);
	if (ret < 0) {
		err("%s failed to read REF_STATUS %d", __func__, ret);
		goto p_err;
	}
...
long sensor_gt9778_actuator_ioctl(struct v4l2_subdev *subdev, unsigned int cmd, void *arg)
{
	int ret = 0;
	...
	case SENSOR_IOCTL_ACT_S_CTRL:
		ret = sensor_gt9778_actuator_s_ctrl(subdev, ctrl);
		if (ret) {
	...
	case SENSOR_IOCTL_ACT_G_CTRL:
		ret = sensor_gt9778_actuator_g_ctrl(subdev, ctrl);
		if (ret) {
...
static const struct v4l2_subdev_core_ops core_ops = {
	...
	.ioctl = sensor_gt9778_actuator_ioctl,
};
