...
static int sensor_ak7348_write_position(struct i2c_client *client, u32 val)
{
	...
	ret = actuator->ixc_ops->addr8_write8(client, AK7348_REG_POS_HIGH, val_high);
	if (ret < 0)
		goto p_err;
	ret = actuator->ixc_ops->addr8_write8(client, AK7348_REG_POS_LOW, val_low);
...
long sensor_ak7348_actuator_ioctl(struct v4l2_subdev *subdev, unsigned int cmd, void *arg)
{
	int ret = 0;
	...
	case SENSOR_IOCTL_ACT_S_CTRL:
		ret = sensor_ak7348_actuator_s_ctrl(subdev, ctrl);
		if (ret) {
	...
	case SENSOR_IOCTL_ACT_G_CTRL:
		ret = sensor_ak7348_actuator_g_ctrl(subdev, ctrl);
		if (ret) {
...
static const struct v4l2_subdev_core_ops core_ops = {
	...
	.ioctl = sensor_ak7348_actuator_ioctl,
};
...
static int sensor_ak7348_actuator_probe_i2c(struct i2c_client *client,
...
static struct i2c_driver sensor_actuator_ak7348_driver = {
	.probe  = sensor_ak7348_actuator_probe_i2c,
	.driver = {
		.name	= ACTUATOR_NAME,
		.owner	= THIS_MODULE,
