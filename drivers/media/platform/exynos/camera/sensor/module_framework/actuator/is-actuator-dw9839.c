...
static int sensor_dw9839_init(struct is_actuator *actuator)
{
	...
	/* =============  PID initial code ===================*/
	ret = actuator->ixc_ops->addr8_write8(client, 0x40, 0xf1);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x41, 0x31);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x42, 0x2B);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x47, 0x00); /* PCAL_ADJ */
	ret |= actuator->ixc_ops->addr8_write8(client, 0x48, 0x00); /* NCAL_ADJ */
	ret |= actuator->ixc_ops->addr8_write8(client, 0x49, 0x0A); /* PGAIN */
	ret |= actuator->ixc_ops->addr8_write8(client, 0x4A, 0xF0);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x4B, 0x00); /* IGAIN */
	ret |= actuator->ixc_ops->addr8_write8(client, 0x4C, 0x21);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x4D, 0x07); /* DGAIN */
	ret |= actuator->ixc_ops->addr8_write8(client, 0x4E, 0x3A);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x4F, 0x23); /* DLPF */
	ret |= actuator->ixc_ops->addr8_write8(client, 0x52, 0x80);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x53, 0x00);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x54, 0x00);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x55, 0x01);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x57, 0x8F);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x58, 0x80);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x59, 0x05);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x5F, 0x00);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x60, 0x00);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x62, 0x0D);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x63, 0x85);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x64, 0x02);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x65, 0x7A);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x66, 0x00);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x67, 0x00);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x68, 0x80);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x69, 0x80);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x6A, 0x80);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x6B, 0x00);
	/* =============  Calibration ===================*/
	ret |= actuator->ixc_ops->addr8_write8(client, 0x03, 0x01);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x04, 0x04);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x03, 0x11);
	msleep(500);
	/* ===========  data store ===================*/
	ret |= actuator->ixc_ops->addr8_write8(client, 0x03, 0x01);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x78, 0x01);
	msleep(400);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x05, 0x01);

	/* goto standby mode */
	usleep_range(1500, 1510);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x03, 0x01);
	ret |= actuator->ixc_ops->addr8_write8(client, 0x04, 0x00);
...
skip_cal:
	/* set Active Mode */
	ret = actuator->ixc_ops->addr8_write8(client, REG_ACTIVE_EN, 0x01);
	ret = actuator->ixc_ops->addr8_write8(client, 0x40, 0xF1);
	ret = actuator->ixc_ops->addr8_write8(client, 0x54, 0x0E);
	ret = actuator->ixc_ops->addr8_write8(client, REG_ACTIVE_EN, 0x11);
	if (ret < 0)
		goto p_err;

	/* delay after active mode */
	usleep_range(3000, 3010);
	ret = actuator->ixc_ops->addr8_write8(client, REG_ACT_MODE, 0x00);
	if (ret < 0)
		goto p_err;
	/* read pcal, ncal */
	actuator_info = (struct dw9839_actuator_info *)actuator->priv_info;
	ret = actuator->ixc_ops->addr8_read8(client, REG_PCAL_MSB, &pcal_msb);
	if (ret < 0)
		goto p_err;
	ret = actuator->ixc_ops->addr8_read8(client, REG_PCAL_LSB, &pcal_lsb);
	if (ret < 0)
		goto p_err;
	ret = actuator->ixc_ops->addr8_read8(client, REG_NCAL_MSB, &ncal_msb);
	if (ret < 0)
		goto p_err;
	ret = actuator->ixc_ops->addr8_read8(client, REG_NCAL_LSB, &ncal_lsb);
	if (ret < 0)
		goto p_err;
...
static int sensor_dw9839_write_position(struct i2c_client *client, u32 val)
{
	...
	ret = actuator->ixc_ops->addr8_write8(client, REG_TARGET_MSB, val_msb);
	if (ret < 0)
		goto p_err;
	ret = actuator->ixc_ops->addr8_write8(client, REG_TARGET_LSB, val_lsb);
	if (ret < 0)
		goto p_err;
		...
		struct is_actuator *actuator)
{
	...
	if (init_step > 0) {
		...
		ret = actuator->ixc_ops->addr8_write8(client, REG_INIT_POS, DEF_DW9839_INIT_POSITION >> 2);
		if (ret < 0)
			goto p_err;
...
int sensor_dw9839_actuator_get_actual_position(struct v4l2_subdev *subdev, u32 *info)
{
	...
	ret = actuator->ixc_ops->addr8_write8(client, REG_ADC_R_EN, 1);
	if (ret < 0)
		goto p_err;
	ret = actuator->ixc_ops->addr8_read8(client, REG_ADC_R_MSB, &pos_msb);
	if (ret < 0)
		goto p_err;
	ret = actuator->ixc_ops->addr8_read8(client, REG_ADC_R_LSB, &pos_lsb);
	if (ret < 0)
		goto p_err;
...
static int sensor_dw9839_actuator_probe_i2c(struct i2c_client *client,
...
static struct i2c_driver sensor_actuator_dw9839_driver = {
	.probe  = sensor_dw9839_actuator_probe_i2c,
	.driver = {
		.name	= ACTUATOR_NAME,
		.owner	= THIS_MODULE,
