...
int sensor_dw9800_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	ret = actuator->ixc_ops->addr8_write8(client, 0x02, 0x02);
	if (ret < 0)
		goto p_err_mutex;
	...
	ret = actuator->ixc_ops->addr8_write8(client, 0x06, 0x40);
	if (ret < 0)
		goto p_err_mutex;

	ret = actuator->ixc_ops->addr8_write8(client, 0x07, 0x7A);
	if (ret < 0)
		goto p_err_mutex;

	ret = actuator->ixc_ops->addr8_read8(client, 0x00, &product_id);
...
int sensor_dw9800_actuator_get_status(struct v4l2_subdev *subdev, u32 *info)
{
	 ...
	 */
	//ret = actuator->ixc_ops->addr8_read8(client, 0x05, &val);
...
static int sensor_dw9800_actuator_probe_i2c(struct i2c_client *client,
...
static struct i2c_driver sensor_actuator_dw9800_driver = {
	.probe  = sensor_dw9800_actuator_probe_i2c,
	.driver = {
		.name	= ACTUATOR_NAME,
		.owner	= THIS_MODULE,
