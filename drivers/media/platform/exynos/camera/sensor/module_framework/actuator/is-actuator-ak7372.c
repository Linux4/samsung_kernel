...
static int sensor_ak7372_write_position(struct i2c_client *client, u32 val)
{
	...
	ret = actuator->ixc_ops->addr8_write8(client, AK7372_REG_POS_HIGH, val_high);

	if (ret < 0)
		goto p_err;
	ret = actuator->ixc_ops->addr8_write8(client, AK7372_REG_POS_LOW, val_low);
	if (ret < 0)
		goto p_err;
...
static int sensor_ak7372_actuator_probe_i2c(struct i2c_client *client,
...
static struct i2c_driver sensor_actuator_ak7372_driver = {
	.probe  = sensor_ak7372_actuator_probe_i2c,
	.driver = {
		.name	= ACTUATOR_NAME,
		.owner	= THIS_MODULE,
