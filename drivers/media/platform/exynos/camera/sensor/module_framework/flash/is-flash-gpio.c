...
long flash_gpio_ioctl(struct v4l2_subdev *subdev, unsigned int cmd, void *arg)
{
	int ret = 0;
	...
	case SENSOR_IOCTL_FLS_S_CTRL:
		ret = flash_gpio_s_ctrl(subdev, ctrl);
		if (ret) {
...
static int __init flash_gpio_platform_probe_i2c(struct platform_device *pdev)
{
	int ret = 0;
	struct device *dev;
...
static int __init is_sensor_flash_gpio_init_i2c(void)
{
	int ret;

	ret = platform_driver_probe(&sensor_flash_gpio_platform_driver,
				flash_gpio_platform_probe_i2c);
	if (ret)
...
late_initcall_sync(is_sensor_flash_gpio_init_i2c);
