		...
		size_t buf_len, loff_t *ppos)
{
	...
	ICPU_DEBUG("overflow(%d), offset(%zu), read_offset(%lld)", overflow_bit, offset, read_offset);
	...
	if (ret || (offset < read_offset && buf_len)) {
		ICPU_ERR("unknown err(ret=%d), offset(%zu), read_offset(%lld), overflow(%d)",
...
int pablo_icpu_debug_config(void *kva, size_t fw_mem_size)
{
	...
	ICPU_DEBUG("log offset addr(0x%llx), base(0x%llx)", (u64) debug.log_offset, (u64) debug.log_base_kva);
