...
static void _csi_hw_s_dma_input_mux(void __iomem *mux_regs, u32 wdma_index,
		int otf_ch, bool mux_en, bool bns_en, u32 bns_mux_val, int otf_out_id,
		u32* link_vc_list, struct is_wdma_info* wdma_info)
{
	u32 mux_val, link_vc, wdma_vc, lc, lc_start, lc_end, mux_val_base;
	...
	if (!mux_en) {
		...
		for (lc = lc_start; lc <= lc_end; lc++) {
			...
			wdma_info->wdma_idx[link_vc] = otf_out_id;
...
{
	_csi_hw_s_dma_input_mux(mux_regs, index, 0, false, false, 0, 0, 0, 0);
}
