...
int is_sec_set_registers(struct i2c_client *client, const u32 *regs, const u32 size, int position)
{
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < size; i += I2C_WRITE) {
		if (regs[i + I2C_BYTE] == I2C_WRITE_FORMAT_ADDR8_DATA8) {
			ret = cis->ixc_ops->addr8_write8(client, regs[i + I2C_ADDR], regs[i + I2C_DATA]);
			if (ret < 0) {
				err("cis->ixc_ops->addr8_write8 fail, ret(%d), addr(%#x), data(%#x)",
					ret, regs[i + I2C_ADDR], regs[i + I2C_DATA]);
				break;
			}
		} else if (regs[i + I2C_BYTE] == I2C_WRITE_FORMAT_ADDR16_DATA8) {
			ret = cis->ixc_ops->write8(client, regs[i + I2C_ADDR], regs[i + I2C_DATA]);
			if (ret < 0) {
			...
			ret = cis->ixc_ops->write16(client, regs[i + I2C_ADDR], regs[i + I2C_DATA]);
			if (ret < 0) {
				err("is->ixc_ops->write16 fail, ret(%d), addr(%#x), data(%#x)",
					ret, regs[i + I2C_ADDR], regs[i + I2C_DATA]);
				break;
			}
		}
...
i2c_write_retry_global:
	if (camera_running == false) {
		ret = is_sec_set_registers(client, sensor_otp_4ha_global, sensor_otp_4ha_global_size, position);
		if (unlikely(ret)) {
...
int is_sec_readcal_otprom_hi1336(int rom_id, struct is_rom_info *finfo, char *buf, int position)
{
	int ret = 0;
	int retry = IS_CAL_RETRY_CNT;
	struct is_core *core = is_get_is_core();
...
i2c_write_retry_global:
	...
	read_addr = 0x0308;
crc_retry:
	ret = cis->ixc_ops->write8(client, 0x0808, 0x01); /* stream on */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0808, 0x0001);
		return ret;
	}
	ret = cis->ixc_ops->write8(client, 0x0B02, 0x01); /* fast standby on */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0B02, 0x01);
		return ret;
	}
	ret = cis->ixc_ops->write8(client, 0x0809, 0x00); /* stream off */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0809, 0x00);
		return ret;
	}
	ret = cis->ixc_ops->write8(client, 0x0B00, 0x00); /* stream off */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0B00, 0x00);
		return ret;
	...
	ret = cis->ixc_ops->write8(client, 0x0260, 0x10); /* OTP test mode enable */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0260, 0x10);
		return ret;
	}
	ret = cis->ixc_ops->write8(client, 0x0809, 0x01); /* stream on */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0809, 0x01);
		return ret;
	}
	ret = cis->ixc_ops->write8(client, 0x0b00, 0x01); /* stream on */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0b00, 0x01);
		return ret;
	...
	/* read otp bank */
	ret = cis->ixc_ops->write8(client, 0x030A,  ((0x400) >> 8) & 0xFF); /* upper 8bit */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x030A,  ((0x400) >> 8) & 0xFF);
		return ret;
	}
	ret = cis->ixc_ops->write8(client, 0x030B, 0x400 & 0xFF); /* lower 8bit */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x030B, 0x400 & 0xFF);
		return ret;
	}
	ret = cis->ixc_ops->write8(client, 0x0302, 0x01); /* read mode */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0302, 0x01);
		return ret;
	...
	ret = cis->ixc_ops->read8(client, read_addr, &bank);
	if (unlikely(ret)) {
	...
	/* OTP burst read */
	ret = cis->ixc_ops->write8(client, 0x030A, ((start_addr) >> 8) & 0xFF); /* upper 8bit */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x030A, ((start_addr) >> 8) & 0xFF);
		return ret;
	}
	ret = cis->ixc_ops->write8(client, 0x030B, start_addr & 0xFF); /* lower 8bit */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
	...
	ret = cis->ixc_ops->write8(client, 0x0712, 0x01); /* burst read register on */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0712, 0x01);
		return ret;
	...
	ret = cis->ixc_ops->read8_size(client, &buf[0], read_addr, IS_READ_MAX_HI1336_OTP_CAL_SIZE);
	...
	ret = cis->ixc_ops->write8(client, 0x0712, 0x00); /* burst read register off */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0712, 0x00);
		return ret;
...
exit:
	...
	usleep_range(10000, 10000); /* sleep 10msec */
	ret = cis->ixc_ops->write8(client, 0x0260, 0x00); /* OTP mode display */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0260, 0x00);
		return ret;
...
int is_sec_readcal_otprom_3l6(int rom_id, struct is_rom_info *finfo, char *buf, int position)
{
	int ret = 0;
...
crc_retry:
	ret = cis->ixc_ops->write8(client, 0x0100, 0x00); /* stream on */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0100, 0x00);
		return ret;
	...
	ret = cis->ixc_ops->write8(client, 0x0101, 0x01); /* stream on */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0101, 0x01);
		return ret;
	...
	ret = cis->ixc_ops->write8(client, 0x0A00, 0x04); /* make initial state */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0A00, 0x04);
		goto exit;
	}
	ret = cis->ixc_ops->write8(client, 0x0A02, S5K3L6_OTP_BANK_READ_PAGE); /* select page */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
	...
	ret = cis->ixc_ops->write8(client, 0x0A00, 0x01); /* read mode */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0A00, 0x01);
		goto exit;
	...
	ret = cis->ixc_ops->read8(client, S5K3L6_OTP_PAGE_START_ADDR, &bank);
	if (unlikely(ret)) {
	...
	ret = cis->ixc_ops->write8(client, 0x0A00, 0x00); /* disable NVM controller */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0A00, 0x00);
		goto exit;
	...
	while (num_otp_byte_read < IS_READ_MAX_S5K3L6_OTP_CAL_SIZE) {
		...
		ret = cis->ixc_ops->write8(client, 0x0A02, page_number); /* select page */
		if (ret < 0) {
			err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
		...
		while (page_byte_index <= S5K3L6_OTP_PAGE_SIZE) {
			ret = cis->ixc_ops->read8(client, read_addr, &buf[num_otp_byte_read]);
			if (unlikely(ret)) {
				err("failed to read OTP bank address(%d) ret(%d)\n", read_addr, ret);
				goto exit;
			}
...
#endif
	ret = cis->ixc_ops->write16(client, 0x0100, 0x0000); /* stream off */
	if (ret < 0) {
		err("is->ixc_ops->write8 fail, ret(%d), addr(%#x), data(%#x)\n",
			ret, 0x0100, 0x0000);
		return ret;
...
i2c_write_retry_global:
	if (camera_running == false) {
		...
		ret = is_sec_set_registers(client, sensor_gc02m1_setfile_otp_global, ARRAY_SIZE(sensor_gc02m1_setfile_otp_global), position);
		if (unlikely(ret)) {
			err("failed to apply otprom global settings (%d)\n", ret);
			if (retry >= 0) {
				retry--;
	...
	ret = is_sec_set_registers(client, sensor_gc02m1_setfile_otp_mode, ARRAY_SIZE(sensor_gc02m1_setfile_otp_mode), position);
	if (unlikely(ret)) {
	...
	ret = is_sec_set_registers(client, sensor_gc02m1_setfile_otp_init, ARRAY_SIZE(sensor_gc02m1_setfile_otp_init), position);
	if (unlikely(ret)) {
		err("failed to apply otprom init setting (%d)\n", ret);
		ret = -EINVAL;
		goto exit;
...
int is_sec_readcal_otprom(int rom_id)
{
	...
	switch(sensor_id) {
		...
		case SENSOR_NAME_HI1336:
			ret = is_sec_readcal_otprom_hi1336(rom_id, finfo, buf, position);
			break;
