...
int is_i2c_config(struct i2c_client *client, bool onoff)
{
...
int is_i2c_read_burst(struct i2c_client *client, void *buf, u32 addr, size_t size)
{
	...
	is_sensor_write8( client, OTP_READ_START_ADDR_HIGH, addr_buf[0]);
	is_sensor_write8( client, OTP_READ_START_ADDR_LOW, addr_buf[1]);
	is_sensor_write8(client, OTP_READ_MODE_ADDR, 0x01);
	for(index = 0; index <=size; index++){
		is_sensor_read8( client, OTP_READ_ADDR, buf+index);
	}
...
ssize_t write_data_to_file_append(char *name, char *buf, size_t count, loff_t *pos)
{
	...
	if (force_caldata_dump) {
		ksys_rmdir(name);
		fd = ksys_open(name, O_WRONLY | O_CREAT | O_APPEND | O_SYNC, 0666);
	} else {
		fd = ksys_open(name, O_WRONLY | O_CREAT | O_APPEND | O_SYNC, 0664);
	}
	if (fd < 0) {
		err("open file error: %s", name);
		/* sys_umask(old_mask); */ /*Fix Me: 4.19 version */
