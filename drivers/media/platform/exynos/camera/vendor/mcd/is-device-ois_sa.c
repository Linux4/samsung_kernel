...
bool is_ois_diff_test_impl(struct is_core *core, int *x_diff, int *y_diff)
{
	...
	ret = is_ois_i2c_write(client, 0x0034, 0x64);
	...
	ret = is_ois_i2c_read(client, 0x0034, &val);
	...
	ret = is_ois_i2c_write(client, 0x020E, 0x00);
	ret |= is_ois_i2c_write(client, 0x020F, 0x00);
	ret |= is_ois_i2c_write(client, 0x0210, 0x50);
	ret |= is_ois_i2c_write(client, 0x0211, 0x50);
	if (ret) {
		err("i2c write fail\n");
	}
...
bool is_ois_fw_version(struct is_core *core)
{
	...
	ret = is_ois_i2c_read(client, 0x00F9, &version[0]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}

	ret = is_ois_i2c_read(client, 0x00F8, &version[1]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}

	ret = is_ois_i2c_read(client, 0x007C, &version[2]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}

	ret = is_ois_i2c_read(client, 0x007D, &version[3]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}

	ret = is_ois_i2c_read(client, 0x007E, &version[4]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}

	ret = is_ois_i2c_read(client, 0x007F, &version[5]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}
...
bool is_ois_read_userdata(struct is_core *core)
{
	...
	is_ois_i2c_read(client, 0x000E, &RcvData[0]);
	if (RcvData[0] == 0x14) {
		is_ois_i2c_read_multi(client, 0x0100, RcvData, FW_TRANS_SIZE);
...
bool is_ois_check_fw_impl(struct is_core *core)
{
	...
	ret = is_ois_fw_version(core);
	if (!ret) {
		err("Failed to read ois fw version.");
		return false;
	}

	is_ois_read_userdata(core);
	...
	if (ois_minfo.header_ver[FW_GYRO_SENSOR] == '6') {
		if (ois_minfo.header_ver[FW_CORE_VERSION] == 'C' ||
			...
			ret = is_ois_open_fw(core, FIMC_OIS_FW_NAME_DOM, &buf);
		...
		ois_minfo.header_ver[FW_CORE_VERSION] == 'F'
			...
			ret = is_ois_open_fw(core, FIMC_OIS_FW_NAME_SEC, &buf);
		}
	} else if (ois_minfo.header_ver[FW_GYRO_SENSOR] == '8') {
		if (ois_minfo.header_ver[FW_CORE_VERSION] == 'H' ||
			ois_minfo.header_ver[FW_CORE_VERSION] == 'J') {
			ret = is_ois_open_fw(core, FIMC_OIS_FW_NAME_SEC_2, &buf);
		}
	} else {
...
void is_ois_fw_update_impl(struct is_core *core)
{
	...
	/* OIS Status Check */
	ois_status = is_ois_read_status(core);
	if (ois_status == 0x01) {
		forced_update = true;
	}
	...
	if (ret) {
		err("is_ois_open_fw failed(%d)", ret);
	}
	...
	if (!fw_sdcard && !forced_update) {
		module_ver = is_ois_fw_revision(ois_minfo.header_ver);
		binary_ver = is_ois_fw_revision(ois_pinfo.header_ver);
...
retry:
	...
	is_ois_i2c_read(client, 0x01, &RcvData); /* OISSTS Read */
	...
	/* SET FWUP_CTRL REG */
	is_ois_i2c_write(client ,0x000C, 0x0B);
	msleep(20);
	...
	for (block = 4; block < 8; block++) {       /* 0x1000 - 0x1FFF */
		for (i = 0; i < 4; i++) {
			...
			is_ois_i2c_write_multi(client, 0x0100, SendData, FW_TRANS_SIZE + 2);
	...
	checkSum = is_ois_calc_checksum(&bootCode[0], sum_size);
	SendData[0] = (checkSum & 0x00FF);
	SendData[1] = (checkSum & 0xFF00) >> 8;
	SendData[2] = 0x00;
	SendData[3] = 0x00;
	is_ois_i2c_write_multi(client, 0x08, SendData, 6);
	msleep(10);			 /* (RUMBA Self Reset) */

	is_ois_i2c_read_multi(client, 0x06, data, 2); /* Error Status read */
	...
	if (data[1] == 0x00 && data[0] == 0x01) {					/* Boot F/W Update Error check */
		...
		is_ois_i2c_write(client, 0x0C, SendData[0]);		/* FWUPCTRL REG(0x000C) 1Byte Send */
		...
		for (block = 4; block <= 27; block++) {		/* 0x1000 -0x 6FFF (RUMBA-SA)*/
			for (i = 0; i < 4; i++) {
				...
				is_ois_i2c_write_multi(client, 0x100, SendData, FW_TRANS_SIZE + 2); /* FLS_DATA REG(0x0100) 256Byte Send */
				position += FW_TRANS_SIZE;
				if (i ==0 ) {
