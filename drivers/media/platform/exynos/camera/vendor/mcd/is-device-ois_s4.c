...
void is_ois_enable_s4(struct is_core *core)
{
	int ret = 0;
...
void is_ois_version_s4(struct is_core *core)
{
	int ret = 0;
	u8 val_c = 0, val_d = 0;
	u16 version = 0;
	...
	ret = is_ois_i2c_read(client, 0x00FC, &val_c);
	if (ret) {
		err("i2c write fail\n");
	}

	ret = is_ois_i2c_read(client, 0x00FD, &val_d);
	if (ret) {
		err("i2c write fail\n");
	}
...
bool is_ois_diff_test_s4(struct is_core *core, int *x_diff, int *y_diff)
{
	...
	ret = is_ois_i2c_read_multi(client, 0x021A, read_x, 2);
	ret |= is_ois_i2c_read_multi(client, 0x021C, read_y, 2);
	if (ret) {
		err("i2c read fail\n");
	}

	ret = is_ois_i2c_write_multi(client, 0x0022, read_x, 4);
	ret |= is_ois_i2c_write_multi(client, 0x0024, read_y, 4);
	ret |= is_ois_i2c_write(client, 0x0002, 0x02);
	ret |= is_ois_i2c_write(client, 0x0000, 0x01);
	if (ret) {
		err("i2c write fail\n");
	}
	msleep(400);
	...
	ret = is_ois_i2c_write(client, 0x0230, 0x64);
	ret |= is_ois_i2c_write(client, 0x0231, 0x00);
	ret |= is_ois_i2c_write(client, 0x0232, 0x64);
	ret |= is_ois_i2c_write(client, 0x0233, 0x00);
	if (ret) {
		err("i2c write fail\n");
	}

	ret = is_ois_i2c_read(client, 0x0230, &val);
	dbg_ois("OIS[read_val_0x0230::0x%04x]\n", val);
	ret |= is_ois_i2c_read(client, 0x0231, &val);
	dbg_ois("OIS[read_val_0x0231::0x%04x]\n", val);
	ret |= is_ois_i2c_read(client, 0x0232, &val);
	dbg_ois("OIS[read_val_0x0232::0x%04x]\n", val);
	ret |= is_ois_i2c_read(client, 0x0233, &val);
	dbg_ois("OIS[read_val_0x0233::0x%04x]\n", val);
	if (ret) {
		err("i2c read fail\n");
	}

	ret = is_ois_i2c_write(client, 0x020E, 0x6E);
	ret |= is_ois_i2c_write(client, 0x020F, 0x6E);
	ret |= is_ois_i2c_write(client, 0x0210, 0x1E);
	ret |= is_ois_i2c_write(client, 0x0211, 0x1E);
	if (ret) {
		err("i2c write fail\n");
	}

	ret = is_ois_i2c_write(client, 0x0013, 0x01);
	if (ret) {
		err("i2c write fail\n");
	}

#if 0
	ret = is_ois_i2c_write(client, 0x0012, 0x01);
	if (ret) {
		err("i2c write fail\n");
	}
	...
	retries = 30;
	do { //polarity check
		ret = is_ois_i2c_read(client, 0x0012, &val);
		if (ret != 0) {
			break;
		}
		msleep(100);
	...
	is_ois_i2c_read(client, 0x0200, &val);
	err("OIS[read_val_0x0200::0x%04x]\n", val);
#endif

	retries = 120;
	do {
		ret = is_ois_i2c_read(client, 0x0013, &val);
		if (ret != 0) {
			break;
		}
		msleep(100);
...
bool is_ois_sine_wavecheck_s4(struct is_core *core,
		            ...
		            int threshold, bool *x_result, bool *y_result, int *sin_x, int *sin_y)
{
	...
	is_ois_i2c_write(client, 0x0002, 0x05); /* ois centering mode */
	ret = is_ois_i2c_write(client, 0x00, 0x01); /* ois servo on */
	if (ret) {
		err("i2c write fail\n");
	}
	msleep(100);
	...
	ret = is_ois_i2c_write(client, 0x00, 0x00); /* ois servo off */
	if (ret) {
		err("i2c write fail\n");
	}
	msleep(100);

	value = is_ois_sine_wavecheck_s4(core, threshold, sin_x, sin_y, &result);
	if (*sin_x == -1 && *sin_y == -1) {
...
int is_ois_read_manual_cal_s4(struct is_core *core)
{
	int ret = 0;
	u8 version[20] = {0, };
	u8 read_ver[3] = {0, };
	...
	ret = is_ois_i2c_write_multi(client, 0x0100, version, 0x16);
	msleep(5);
	ret |= is_ois_i2c_read(client, 0x0118, &read_ver[0]);
	ret |= is_ois_i2c_read(client, 0x0119, &read_ver[1]);
	ret |= is_ois_i2c_read(client, 0x011A, &read_ver[2]);
	if (ret) {
		err("i2c cmd fail\n");
		ret = -EINVAL;
		goto exit;
...
bool is_ois_fw_version_s4(struct is_core *core)
{
	...
	ret = is_ois_i2c_read(client, 0x00FB, &version[0]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}

	ret = is_ois_i2c_read(client, 0x00F9, &version[1]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}

	ret = is_ois_i2c_read(client, 0x00F8, &version[2]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}

	ret = is_ois_i2c_read(client, 0x007C, &version[3]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}

	ret = is_ois_i2c_read(client, 0x007D, &version[4]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}

	ret = is_ois_i2c_read(client, 0x007E, &version[5]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}

	ret = is_ois_i2c_read(client, 0x007F, &version[6]);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}
...
bool is_ois_read_userdata_s4(struct is_core *core)
{
	...
	/* OIS servo OFF */
	is_ois_i2c_write(client ,0x0000, 0x00);
	retries = 50;
	do {
		ret = is_ois_i2c_read(client, 0x0000, &val);
		if (ret != 0) {
			break;
		}
		msleep(100);
	...
	is_ois_i2c_write(client ,0x000F, 0x12);
	SendData[0] = 0x00;
	SendData[1] = 0x00;
	is_ois_i2c_write_multi(client, 0x0010, SendData, 4);
	is_ois_i2c_write(client ,0x000E, 0x04);

	retries = 50;
	do {
		ret = is_ois_i2c_read(client, 0x000E, &val);
		if (ret != 0) {
			break;
		}
		msleep(100);
	...
	is_ois_i2c_read_multi(client, 0x0100, Read_data, USER_FW_SIZE);
	...
	is_ois_i2c_write(client ,0x000F, 0x0E);
	SendData[0] = 0x00;
	SendData[1] = 0x04;
	is_ois_i2c_write_multi(client, 0x0010, SendData, 4);
	is_ois_i2c_write(client ,0x000E, 0x04);
	...
	is_ois_i2c_read(client, 0x0100, &ois_shift_info);
	...
	if (ois_shift_info == 0x11) {
		...
		calData[0] = 0;
		calData[1] = 0;
		is_ois_i2c_read_multi(client, 0x0102, calData, 2);
		...
		calData[0] = 0;
		calData[1] = 0;
		is_ois_i2c_read_multi(client, 0x0104, calData, 2);
		...
		calData[0] = 0;
		calData[1] = 0;
		is_ois_i2c_read_multi(client, 0x0106, calData, 2);
		...
		for (i = 0; i < 9; i++) {
			calData[0] = 0;
			calData[1] = 0;
			is_ois_i2c_read_multi(client, 0x0108 + i*2, calData, 2);
			ois_shift_x_cal = (calData[1] << 8) | (calData[0]);

			calData[0] = 0;
			calData[1] = 0;
			is_ois_i2c_read_multi(client, 0x0120 + i*2, calData, 2);
...
bool is_ois_check_fw_s4(struct is_core *core)
{
	...
	case 'M':
	case 'O':
		ret = is_ois_open_fw_s4(core, FIMC_OIS_FW_NAME_DOM, &buf);
		break;
	case 'B':
	case 'D':
	case 'F':
	...
	case 'N':
	case 'P':
		ret = is_ois_open_fw_s4(core, FIMC_OIS_FW_NAME_SEC, &buf);
		break;
	default:
		ret = -EINVAL;
		break;
	...
	if (ret) {
		err("is_ois_open_fw_s4 failed(%d)\n", ret);
	}
...
void is_ois_check_extclk_s4(struct is_core *core)
{
	u8 cur_clk[4] = {0, };
	u8 new_clk[4] = {0, };
	u32 cur_clk_32 = 0;
	...
	ret = is_ois_i2c_read_multi(client, 0x03F0, cur_clk, 4);
	if (ret) {
		err("i2c read fail\n");
		goto exit;
	}
	...
	if (cur_clk_32 != CAMERA_OIS_EXT_CLK) {
		...
		/* write EXTCLK */
		ret = is_ois_i2c_write_multi(client, 0x03F0, new_clk, 6);
		if (ret) {
			err("i2c write fail\n");
			goto exit;
		}
		...
		/* write PLLMULTIPLE */
		ret = is_ois_i2c_write(client, 0x03F4, pll_multi);
		if (ret) {
			err("i2c write fail\n");
			goto exit;
		}
		...
		/* write PLLDIVIDE */
		ret = is_ois_i2c_write(client, 0x03F5, pll_divide);
		if (ret) {
			err("i2c write fail\n");
			goto exit;
		}
		...
		/* FLASH TO OIS MODULE */
		ret = is_ois_i2c_write(client, 0x0003, 0x01);
		if (ret) {
			err("i2c write fail\n");
			msleep(200);
			goto exit;
		...
		/* S/W Reset */
		ret = is_ois_i2c_write(client, 0x000D, 0x01);
		ret |= is_ois_i2c_write(client, 0x000E, 0x06);
		if (ret) {
			err("i2c write fail\n");
			msleep(50);
			goto exit;
...
void is_ois_fw_update_s4(struct is_core *core)
{
	...
	/* OIS Status Check */
	ret = is_ois_i2c_read_multi(client, 0x00FC, ois_status, 4);
	if (ret) {
		err("i2c read fail\n");
		goto p_err;
	}
	...
	if (ois_status_32 == OIS_FW_UPDATE_ERROR_STATUS) {
		forced_update = true;
		is_ois_read_manual_cal_s4(core);
	} else {
#ifdef CAMERA_USE_OIS_EXT_CLK
		/* Check EXTCLK value */
		is_ois_check_extclk_s4(core);
#endif
	}
	...
	if (ret) {
		err("is_ois_open_fw_s4 failed(%d)", ret);
	}
	...
	if (!forced_update) {
		if (!is_ois_version_compare(ois_minfo.header_ver, ois_pinfo.header_ver,
			ois_uinfo.header_ver)) {
		...
		if (!is_ois_version_compare_default(ois_minfo.header_ver, ois_pinfo.header_ver)) {
	...
	crc_result = is_ois_crc_check(core, buf);
	if (crc_result == false) {
		err("OIS CRC32 error.\n");
		goto p_err;
	}
	...
	if (!fw_sdcard && !forced_update) {
		module_ver = is_ois_fw_revision_s4(ois_minfo.header_ver);
		binary_ver = is_ois_fw_revision_s4(ois_pinfo.header_ver);
...
retry:
	...
	is_ois_i2c_read(client, 0x0001, &RcvData); /* OISSTS Read */
	...
	is_ois_i2c_write(client, 0x000C, SendData[0]);		/* FWUPCTRL REG(0x000C) 1Byte Send */
	msleep(55);
	...
	for (block = 0; block < 112; block++) {		/* 0x1000 -0x 6FFF (RUMBA-SA)*/
		...
		is_ois_i2c_write_multi(client, 0x0100, SendData, FW_TRANS_SIZE + 2); /* FLS_DATA REG(0x0100) 256Byte Send */
	...
	checkSum = is_ois_calc_checksum(&progCode[0], sum_size);
	SendData[0] = (checkSum & 0x00FF);
	SendData[1] = (checkSum & 0xFF00) >> 8;
	SendData[2] = 0;		/* Don't Care */
	SendData[3] = 0x80;		/* Self Reset Request */

	is_ois_i2c_write_multi(client, 0x08, SendData, 6);  /* FWUP_CHKSUM REG(0x0008) 4Byte Send */
	msleep(190);		/* (Wait RUMBA Self Reset) */
	is_ois_i2c_read(client, 0x6, (u8*)&RcvDataShort); /* Error Status read */
	...
	/* S/W Reset */
	is_ois_i2c_write(client ,0x000D, 0x01);
	is_ois_i2c_write(client ,0x000E, 0x06);
	msleep(50);

	/* Param init - Flash to Rumba */
	is_ois_i2c_write(client ,0x0036, 0x03);
	msleep(200);
...
int is_ois_shift_compensation_s4(struct v4l2_subdev *subdev, int position, int resolution)
{
	...
	*/
	...
	ret = is_ois_i2c_write_multi(ois->client, 0x004C, write_data, 6);
	if (ret < 0)
...
u8 is_read_ois_mode_s4(struct v4l2_subdev *subdev)
{
	int ret = 0;
...
int is_set_ois_mode_s4(struct v4l2_subdev *subdev, int mode)
{
	...
	if (ois->fadeupdown == false) {
		is_ois_set_ggfadeupdown_s4(subdev, 1000, 1000);
		ois->fadeupdown = true;
	}
...
void is_status_check_s4(struct i2c_client *client)
{
...
int is_ois_init_s4(struct v4l2_subdev *subdev)
{
	...
	I2C_MUTEX_LOCK(ois->i2c_lock);
	/* use user data */
	ret |= is_ois_i2c_write(ois->client, 0x000F, 0x0E);
	cal_data[0] = 0x00;
	cal_data[1] = 0x04;
	ret |= is_ois_i2c_write_multi(ois->client, 0x0010, cal_data, 4);
	ret |= is_ois_i2c_write(ois->client, 0x000E, 0x04);
	if (ret < 0) {
#ifdef USE_CAMERA_HW_BIG_DATA
	...
	is_status_check_s4(ois->client);

	is_ois_i2c_read(ois->client, 0x0100, &shift_available);
	...
	for (i = 0; i < 9; i++) {
		cal_data[0] = 0;
		cal_data[1] = 0;
		is_ois_i2c_read_multi(ois->client, 0x0108 + (i * 2), cal_data, 2);
		ois_shift_x_cal = (cal_data[1] << 8) | (cal_data[0]);

		cal_data[0] = 0;
		cal_data[1] = 0;
		is_ois_i2c_read_multi(ois->client, 0x0120 + (i * 2), cal_data, 2);
	...
	/* CACTRL */
	ret = is_ois_i2c_write(ois->client, 0x0039, 0x01);
	if (ret < 0) {
	...
	ret = is_ois_i2c_read_multi(ois->client, 0x021A, data, 2);
	ois_center_x = (data[1] << 8) | (data[0]);
	ret |= is_ois_i2c_read_multi(ois->client, 0x021C, data, 2);
...
int is_ois_shift_s4(struct v4l2_subdev *subdev)
{
	struct is_ois *ois = NULL;
	struct i2c_client *client = NULL;
	u8 data[2];
	...
	ret = is_ois_i2c_write_multi(ois->client, 0x0022, data, 4);
	if (ret < 0)
		err("i2c write multi is fail\n");

	data[0] = (ois_center_y & 0xFF);
	data[1] = (ois_center_y & 0xFF00) >> 8;
	ret = is_ois_i2c_write_multi(ois->client, 0x0024, data, 4);
	if (ret < 0)
		err("i2c write multi is fail\n");
...
int is_ois_set_still_s4(struct is_core *core)
{
	...
	client = is_ois_i2c_get_client(core);

	ret = is_ois_i2c_write(client, 0x0002, 0x00);
	if (ret) {
		err("i2c write fail\n");
		return ret;
	}
...
static struct is_ois_ops ois_ops_s4 = {
	.ois_init = is_ois_init_s4,
	.ois_set_mode = is_set_ois_mode_s4,
	.ois_shift_compensation = is_ois_shift_compensation_s4,
	.ois_fw_update = is_ois_fw_update_s4,
	.ois_self_test = is_ois_self_test_s4,
	.ois_diff_test = is_ois_diff_test_s4,
	.ois_auto_test = is_ois_auto_test_s4,
	.ois_check_fw = is_ois_check_fw_s4,
	.ois_enable = is_ois_enable_s4,
	.ois_offset_test = is_ois_offset_test_s4,
	.ois_get_offset_data = is_ois_get_offset_data_s4,
	.ois_gyro_sleep = is_ois_gyro_sleep_s4,
	.ois_exif_data = is_ois_exif_data_s4,
	.ois_read_status = is_ois_read_status_s4,
	.ois_read_cal_checksum = is_ois_read_cal_checksum_s4,
	.ois_set_coef = is_ois_set_coef_s4,
	.ois_center_shift = is_ois_shift_s4,
	.ois_set_center = is_ois_set_centering_s4,
	.ois_read_mode = is_read_ois_mode_s4,
};
...
static const struct of_device_id exynos_is_ois_rumbaS4_match[] = {
	{
		.compatible = "samsung,exynos-is-ois-rumbaS4",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_ois_rumbaS4_match);
...
static struct i2c_driver ois_rumbaS4_driver = {
	.driver = {
		.name	= OIS_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_is_ois_rumbaS4_match
	},
