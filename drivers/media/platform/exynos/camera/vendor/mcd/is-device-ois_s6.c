...
void is_ois_enable_s6(struct is_core *core)
{
	int ret = 0;
...
void is_ois_version_s6(struct is_core *core)
{
	int ret = 0;
	u8 val_c = 0, val_d = 0;
	u16 version = 0;
...
void is_ois_offset_test_s6(struct is_core *core, long *raw_data_x, long *raw_data_y)
{
	int ret = 0, i = 0;
	u8 val = 0, x = 0, y = 0;
	...
	*raw_data_y = sum * 1000 / scale_factor / 10;
	...
	is_ois_version_s6(core);
	dbg_ois("%s : X\n", __func__);
	return;
}

void is_ois_get_offset_data_s6(struct is_core *core, long *raw_data_x, long *raw_data_y)
{
	int i = 0;
	u8 val = 0, x = 0, y = 0;
	int x_sum = 0, y_sum = 0, sum = 0;
...
int is_ois_self_test_s6(struct is_core *core)
{
	int ret = 0;
	u8 val = 0;
...
bool is_ois_diff_test_s6(struct is_core *core, int *x_diff, int *y_diff)
{
	int ret = 0;
	u8 val = 0, x = 0, y = 0;
...
bool is_ois_sine_wavecheck_s6(struct is_core *core,
...
bool is_ois_auto_test_s6(struct is_core *core,
					int threshold, bool *x_result, bool *y_result, int *sin_x, int *sin_y)
{
	...
	value = is_ois_sine_wavecheck_s6(core, threshold, sin_x, sin_y, &result);
	if (*sin_x == -1 && *sin_y == -1) {
...
bool is_ois_sine_wavecheck_rear2_s6(struct is_core *core,
					int threshold, int *sinx, int *siny, int *result,
...
bool is_ois_auto_test_rear2_s6(struct is_core *core,
					int threshold, bool *x_result, bool *y_result, int *sin_x, int *sin_y,
					bool *x_result_2nd, bool *y_result_2nd, int *sin_x_2nd, int *sin_y_2nd)
{
	...
	value = is_ois_sine_wavecheck_rear2_s6(core, threshold, sin_x, sin_y, &result,
				sin_x_2nd, sin_y_2nd);
...
void is_ois_gyro_sleep_s6(struct is_core *core)
{
	int ret = 0;
	u8 val = 0;
	int retries = 20;
...
void is_ois_exif_data_s6(struct is_core *core)
{
	int ret = 0;
...
u8 is_ois_read_status_s6(struct is_core *core)
{
	int ret = 0;
	u8 status = 0;
	struct i2c_client *client = is_ois_i2c_get_client(core);
...
u8 is_ois_read_cal_checksum_s6(struct is_core *core)
{
	int ret = 0;
	u8 status = 0;
	struct i2c_client *client = is_ois_i2c_get_client(core);
...
int is_ois_read_manual_cal_s6(struct is_core *core)
{
	int ret = 0;
	u8 version[20] = {0, };
	u8 read_ver[3] = {0, };
...
bool is_ois_fw_version_s6(struct is_core *core)
{
	int ret = 0;
	char version[7] = {0, };
...
int is_ois_fw_revision_s6(char *fw_ver)
{
	int revision = 0;
...
bool is_ois_read_userdata_s6(struct is_core *core)
{
	u8 SendData[2];
	...
	is_ois_i2c_write(client ,0x000F, 0x40);
	SendData[0] = 0x00;
	SendData[1] = 0x00;
	...
	is_ois_i2c_read_multi(client, 0x0100, Read_data + 0x100, USER_FW_SIZE);
	...
	if (ois_shift_info == 0x11) {
		...
		for (i = 0; i < 9; i++) {
			calData[0] = 0;
			calData[1] = 0;
			is_ois_i2c_read_multi(client, 0x0110 + i*2, calData, 2);
			ois_shift_x_cal = (calData[1] << 8) | (calData[0]);

			calData[0] = 0;
			calData[1] = 0;
			is_ois_i2c_read_multi(client, 0x0122 + i*2, calData, 2);
	...
	ois_shift_info = 0;
	is_ois_i2c_read(client, 0x0101, &ois_shift_info);
	info("OIS Shift Info : 0x%x\n", ois_shift_info);
	...
	if (ois_shift_info == 0x11) {
		...
		calData[0] = 0;
		calData[1] = 0;
		is_ois_i2c_read_multi(client, 0x0108, calData, 2);
		ois_shift_x_diff = (calData[1] << 8) | (calData[0]);
		...
		calData[0] = 0;
		calData[1] = 0;
		is_ois_i2c_read_multi(client, 0x010A, calData, 2);
		ois_shift_y_diff = (calData[1] << 8) | (calData[0]);
		...
		for (i = 0; i < 9; i++) {
			calData[0] = 0;
			calData[1] = 0;
			is_ois_i2c_read_multi(client, 0x0140 + i*2, calData, 2);
			ois_shift_x_cal = (calData[1] << 8) | (calData[0]);

			calData[0] = 0;
			calData[1] = 0;
			is_ois_i2c_read_multi(client, 0x0152 + i*2, calData, 2);
			ois_shift_y_cal = (calData[1] << 8) | (calData[0]);
...
int is_ois_open_fw_s6(struct is_core *core, char *name, u8 **buf)
{
	int ret = 0;
	ulong size = 0;
...
bool is_ois_check_fw_s6(struct is_core *core)
{
	u8 *buf = NULL;
	int ret = 0;

	ret = is_ois_fw_version_s6(core);
	...
	case 'M':
	case 'O':
		ret = is_ois_open_fw_s6(core, FIMC_OIS_FW_NAME_DOM, &buf);
		break;
	case 'B':
	case 'D':
	case 'F':
	...
	case 'N':
	case 'P':
		ret = is_ois_open_fw_s6(core, FIMC_OIS_FW_NAME_SEC, &buf);
		break;
	default:
		ret = -EINVAL;
		break;
	...
	if (ret) {
		err("is_ois_open_fw_s6 failed(%d)\n", ret);
	}
...
int is_ois_read_fw_ver_s6(char *name, char *ver)
{
	int ret = 0;
	ulong size = 0;
	char buf[100] = {0, };
...
void is_ois_check_extclk_s6(struct is_core *core)
{
	u8 cur_clk[4] = {0, };
	u8 new_clk[4] = {0, };
	u32 cur_clk_32 = 0;
...
void is_ois_fw_update_s6(struct is_core *core)
{
	...
	if (ois_status_32 == OIS_FW_UPDATE_ERROR_STATUS) {
		forced_update = true;
		is_ois_read_manual_cal_s6(core);
	} else {
#ifdef CAMERA_USE_OIS_EXT_CLK
		/* Check EXTCLK value */
		is_ois_check_extclk_s6(core);
#endif
	}
	...
	if (ret) {
		err("is_ois_open_fw_s6 failed(%d)", ret);
	}
	...
	if (!fw_sdcard && !forced_update) {
		module_ver = is_ois_fw_revision_s6(ois_minfo.header_ver);
		binary_ver = is_ois_fw_revision_s6(ois_pinfo.header_ver);
...
int is_ois_shift_compensation_s6(struct v4l2_subdev *subdev, int position, int resolution)
{
	int ret = 0;
	struct is_ois *ois;
	u32 af_value;
	...
	device = (struct is_device_sensor *)v4l2_get_subdev_hostdata(ois->sensor_peri->subdev_cis);
	if (unlikely(!device)) {
	...
	sub_scene = (setfile & IS_SETFILE_MASK);
	scene = (setfile & IS_SCENARIO_MASK) >> IS_SCENARIO_SHIFT;
#ifdef OIS_CENTERING_SHIFT_ENABLE
	if ((sub_scene == ISS_SUB_SCENARIO_FHD_60FPS) || (scene == IS_SCENARIO_SWVDIS)
		|| (scene == IS_SCENARIO_AUTO_DUAL)) /* scene == IS_SCENARIO_SWVDIS : UHD */
	...
	*/
	...
	if (ois->device == SENSOR_POSITION_REAR)
		...
		ret = is_ois_i2c_write_multi(ois->client, 0x0098, write_data, 6);
...
u8 is_read_ois_mode_s6(struct v4l2_subdev *subdev)
{
	int ret = 0;
...
int is_set_ois_mode_s6(struct v4l2_subdev *subdev, int mode)
{
	int ret = 0;
	struct is_ois *ois = NULL;
	struct i2c_client *client = NULL;
	...
	if (ois->fadeupdown == false) {
		if (ois_fadeupdown == false) {
			ois_fadeupdown = true;
			is_ois_set_ggfadeupdown_s6(subdev, 1000, 1000);
		}
		ois->fadeupdown = true;
	}
...
void is_status_check_s6(struct i2c_client *client)
{
...
int fimc_calculate_shift_value(struct v4l2_subdev *subdev)
{
	...
	is_status_check_s6(ois->client);
#ifdef OIS_CENTERING_SHIFT_ENABLE
	is_ois_i2c_read_multi(ois->client, 0x0254, read_multi, 4);
	Wide_XGG_Hex = (read_multi[3] << 24) | (read_multi[2] << 16) | (read_multi[1] << 8) | (read_multi[0]);

	is_ois_i2c_read_multi(ois->client, 0x0554, read_multi, 4);
	Tele_XGG_Hex = (read_multi[3] << 24) | (read_multi[2] << 16) | (read_multi[1] << 8) | (read_multi[0]);

	is_ois_i2c_read_multi(ois->client, 0x0258, read_multi, 4);
	Wide_YGG_Hex = (read_multi[3] << 24) | (read_multi[2] << 16) | (read_multi[1] << 8) | (read_multi[0]);

	is_ois_i2c_read_multi(ois->client, 0x0558, read_multi, 4);
	...
	if (shift_available != 0x11) {
		...
		for (i = 0; i < 9; i++) {
			cal_data[0] = 0;
			cal_data[1] = 0;
			is_ois_i2c_read_multi(ois->client, 0x0110 + (i * 2), cal_data, 2);
			ois_shift_x_cal = (cal_data[1] << 8) | (cal_data[0]);

			cal_data[0] = 0;
			cal_data[1] = 0;
			is_ois_i2c_read_multi(ois->client, 0x0122 + (i * 2), cal_data, 2);
	...
	shift_available = 0;
	is_ois_i2c_read(ois->client, 0x0101, &shift_available);
	if (shift_available != 0x11) {
		...
		for (i = 0; i < 9; i++) {
			cal_data[0] = 0;
			cal_data[1] = 0;
			is_ois_i2c_read_multi(ois->client, 0x0140 + (i * 2), cal_data, 2);
			ois_shift_x_cal = (cal_data[1] << 8) | (cal_data[0]);

			cal_data[0] = 0;
			cal_data[1] = 0;
			is_ois_i2c_read_multi(ois->client, 0x0152 + (i * 2), cal_data, 2);
			ois_shift_y_cal = (cal_data[1] << 8) | (cal_data[0]);
...
int is_ois_init_s6(struct v4l2_subdev *subdev)
{
	int ret = 0;
#ifdef USE_OIS_SLEEP_MODE
	...
	if (module->position == SENSOR_POSITION_REAR2) {
		ret = is_ois_i2c_write(ois->client, 0x00BF, 0x02);
		if (ret < 0)
			err("ois 0x00BF write is fail");
		ret = is_ois_i2c_write(ois->client, 0x00BE, 0x02);
		if (ret < 0)
			err("ois 0x00BE write is fail");
		else
			is_ois_set_oissel_info(0x02);

	} else if (module->position == SENSOR_POSITION_REAR) {
		ret = is_ois_i2c_write(ois->client, 0x00BF, 0x01);
		if (ret < 0)
			err("ois 0x00BF write is fail");
		ret = is_ois_i2c_write(ois->client, 0x00BE, 0x01);
		if (ret < 0)
			err("ois 0x00BE write is fail");
		else
			is_ois_set_oissel_info(0x01);
	...
	ret = is_ois_i2c_read_multi(client, 0x021A, data, 2);
	ois_center_x = (data[1] << 8) | (data[0]);
	ret |= is_ois_i2c_read_multi(client, 0x021C, data, 2);
...
int is_ois_deinit_s6(struct v4l2_subdev *subdev)
{
	int ret = 0;
...
#ifdef USE_OIS_SLEEP_MODE
int is_ois_start_s6(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_ois *ois = NULL;
	struct i2c_client *client = NULL;
	...
	if ((ois_wide_start == true && module->position == SENSOR_POSITION_REAR2) || //tele already enabled
		...
		ret = is_ois_i2c_write(ois->client, 0x00BE, 0x03);
		if (ret < 0)
			err("ois 0x00BE write is fail");
		else
			is_ois_set_oissel_info(0x03);
...
int is_ois_stop_s6(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_ois *ois = NULL;
	struct i2c_client *client = NULL;
	...
	read_oissel = is_ois_get_oissel_info();
...
int is_ois_set_coef_s6(struct v4l2_subdev *subdev, u8 coef)
{
	int ret = 0;
	struct is_ois *ois = NULL;
	struct i2c_client *client = NULL;
...
int is_ois_set_centering_s6(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_ois *ois = NULL;
	struct i2c_client *client = NULL;
...
int is_ois_set_still_s6(struct is_core *core)
{
	int ret = 0;
	struct i2c_client *client = NULL;
...
int is_ois_shift_s6(struct v4l2_subdev *subdev)
{
	struct is_ois *ois = NULL;
	struct i2c_client *client = NULL;
	u8 data[2];
...
#ifdef CAMERA_REAR2_OIS
int is_ois_set_power_mode(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_ois *ois = NULL;
	struct i2c_client *client = NULL;
...
int is_ois_set_ggfadeupdown_s6(struct v4l2_subdev *subdev, int up, int down)
{
	int ret = 0;
	struct is_ois *ois = NULL;
	struct i2c_client *client = NULL;
	...
	write_data[2] = 0x55;
	write_data[3] = 0x3F;
	is_ois_i2c_write_multi(ois->client, 0x0348, write_data, 6);

	write_data[0] = 0x06;
	write_data[1] = 0x81;
	write_data[2] = 0x55;
	write_data[3] = 0x3F;
	is_ois_i2c_write_multi(ois->client, 0x03D8, write_data, 6);

#ifdef USE_OIS_SLEEP_MODE
	/* if camera is already started, skip VDIS setting */
	is_ois_i2c_read(ois->client, 0x00BF, &read_sensorStart);
...
static struct is_ois_ops ois_ops_s6 = {
	.ois_init = is_ois_init_s6,
	.ois_deinit = is_ois_deinit_s6,
#ifdef USE_OIS_SLEEP_MODE
	.ois_start = is_ois_start_s6,
	.ois_stop = is_ois_stop_s6,
#endif
	.ois_set_mode = is_set_ois_mode_s6,
	.ois_shift_compensation = is_ois_shift_compensation_s6,
	.ois_fw_update = is_ois_fw_update_s6,
	.ois_self_test = is_ois_self_test_s6,
	.ois_diff_test = is_ois_diff_test_s6,
	.ois_auto_test = is_ois_auto_test_s6,
#ifdef CAMERA_REAR2_OIS
	.ois_auto_test_rear2 = is_ois_auto_test_rear2_s6,
	.ois_set_power_mode = is_ois_set_power_mode,
#endif
	.ois_check_fw = is_ois_check_fw_s6,
	.ois_enable = is_ois_enable_s6,
	.ois_offset_test = is_ois_offset_test_s6,
	.ois_get_offset_data = is_ois_get_offset_data_s6,
	.ois_gyro_sleep = is_ois_gyro_sleep_s6,
	.ois_exif_data = is_ois_exif_data_s6,
	.ois_read_status = is_ois_read_status_s6,
	.ois_read_cal_checksum = is_ois_read_cal_checksum_s6,
	.ois_set_coef = is_ois_set_coef_s6,
	.ois_read_fw_ver = is_ois_read_fw_ver_s6,
	.ois_center_shift = is_ois_shift_s6,
	.ois_set_center = is_ois_set_centering_s6,
	.ois_read_mode = is_read_ois_mode_s6,
};
...
static const struct of_device_id exynos_is_ois_rumbaS6_match[] = {
	{
		.compatible = "samsung,exynos-is-ois-rumbaS6",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_ois_rumbaS6_match);
...
static struct i2c_driver ois_rumbaS6_driver = {
	.driver = {
		.name	= OIS_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_is_ois_rumbaS6_match
	},
