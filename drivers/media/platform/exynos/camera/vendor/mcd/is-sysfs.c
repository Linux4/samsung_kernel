...
static ssize_t camera_af_hall_show(char *buf, enum is_cam_info_index cam_index)
{
	int ret;
	int position;
	struct is_cam_info *cam_info;
	...
	if (ret) {
		err("[%s] actuator read af hall fail\n", sensor_id);
		goto p_err;
	}
...
static ssize_t camera_rear_af_hall_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
#ifdef CAMERA_REAR3_AFHALL
static ssize_t camera_rear3_af_hall_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
#ifdef CAMERA_REAR3_AFHALL
static DEVICE_ATTR(rear3_af_hall, S_IRUSR|S_IRGRP, camera_rear3_af_hall_show, NULL);
#endif
#ifdef CAMERA_REAR3_AFCAL
...
int is_create_sysfs(struct is_core *core)
{
	...
	if (IS_ERR(camera_rear_dev)) {
		...
		if (device_create_file(camera_rear_dev, &dev_attr_rear_af_hall) < 0) {
			pr_err("failed to create rear device file, %s\n",
				dev_attr_rear_af_hall.attr.name);
		}
#ifdef CAMERA_REAR_DUAL_CAL
...
#ifdef CAMERA_REAR3_AFHALL
		if (device_create_file(camera_rear_dev, &dev_attr_rear3_af_hall) < 0) {
			pr_err("failed to create rear device file, %s\n",
...
int is_destroy_sysfs(struct is_core *core)
{
	...
	if (camera_rear_dev) {
		...
		device_remove_file(camera_rear_dev, &dev_attr_rear_af_hall);
#ifdef CAMERA_REAR_DUAL_CAL
...
#ifdef CAMERA_REAR3_AFHALL
		device_remove_file(camera_rear_dev, &dev_attr_rear3_af_hall);
#endif
#ifdef CAMERA_REAR3_AFCAL
