...
enum is_csi_state {
	...
	CSIS_SET_MULTIBUF_VC4,
	CSIS_SET_MULTIBUF_VC5,
	CSIS_SET_MULTIBUF_VC6,
	CSIS_SET_MULTIBUF_VC7,
...
struct is_device_csi {
	...
	u32 ch;					/* CSIS link input channel */
	u32 otf_out_ch[CSI_OTF_OUT_CH_NUM];	/* CSIS link output channel after MUX */
	u32 otf_out_num;
	...
	struct is_camif_wdma		*wdma[CSI_OTF_OUT_CH_NUM];
	int				wdma_ch[CSI_OTF_OUT_CH_NUM];
	struct is_camif_wdma_module	*wdma_mod[CSI_OTF_OUT_CH_NUM];
...
struct pablo_csi_wq_set_func {
	void (*wq_csis_dma_vc[CSI_VIRTUAL_CH_MAX])(struct work_struct *data);
};
