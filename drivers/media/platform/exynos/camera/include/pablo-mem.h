...
#define SECURE_CAMERA_CONTIG_NAME	"camera_heap-secure"
#if defined(SECURE_CAMERA_USE_EXT_HEAP)
#define SECURE_CAMERA_HEAP_NAME		"secure_camera_heap_drc-secure"
#else
#define SECURE_CAMERA_HEAP_NAME		"secure_camera-secure"
#endif
...
struct is_priv_buf_ops {
	...
	void (*sync_for_cpu)(struct is_priv_buf *pbuf,
							off_t offset, size_t size,
							enum dma_data_direction dir);
	void (*sync_for_secure)(struct is_priv_buf *pbuf,
							off_t offset, size_t size,
							enum dma_data_direction dir);
};
...
struct pablo_dmabuf_heap_ctx {
	...
	struct dma_heap	*dh_secure_camera_contig;
	struct dma_heap	*dh_secure_camera_heap;
	struct mutex	lock;
};
