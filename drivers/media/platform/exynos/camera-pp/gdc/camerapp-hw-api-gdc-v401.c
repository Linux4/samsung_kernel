 ...
 */
	...
	if (d_frame->extra == COMP) {
		...
		/* Assume that lossy compression rate is 8bit 50%, 10bit 40% */
		if (d_frame->pixel_size == CAMERAPP_PIXEL_SIZE_8BIT) {
			output_stride_lum_w = (u32)(SBWCL_8B_STRIDE(out_dma_width, 50));
			output_stride_chroma_w = (u32)(SBWCL_8B_STRIDE(out_dma_width, 50));
		} else if (d_frame->pixel_size == CAMERAPP_PIXEL_SIZE_10BIT) {
			output_stride_lum_w = (u32)(SBWCL_10B_STRIDE(out_dma_width, 40));
			output_stride_chroma_w = (u32)(SBWCL_10B_STRIDE(out_dma_width, 40));
		}
...
void camerapp_hw_gdc_votf_enable(void __iomem *base_addr, u8 rw)
{
	if (rw) {
		...
		camerapp_sfr_set_reg(base_addr, &gdc_regs[GDC_R_GDC_BUS_USER_INFO_WDMA], 0x20);
	}
}
