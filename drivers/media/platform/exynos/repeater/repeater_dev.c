...
int repeater_get_valid_buffer(int *buf_idx)
{
	...
	if (ctx->ctx_status == REPEATER_CTX_INIT || ctx->repeater_encode_cb == NULL) {
		print_repeater_debug(RPT_ERROR, "%s, ctx_status %d, %s\n", __func__,
			ctx->ctx_status, ctx->repeater_encode_cb == NULL ? "cb is NULL" : "cb is not NULL");
		spin_unlock_irqrestore(&repeater_spinlock, flags);
		return -EFAULT;
	}
