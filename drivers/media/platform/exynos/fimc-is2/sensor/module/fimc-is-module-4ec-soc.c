...
#include "fimc-is-module-4ec-soc.h"

#if defined(CONFIG_CAMERA_GTESVELTE)
#include "fimc-is-module-4ec-soc-reg_gtesvelte.h" /* 4ECGA : setfile for GTESVELTE */
#elif defined(CONFIG_CAMERA_GTAXSWIFI)
#include "fimc-is-module-4ec-soc-reg_gtaxswifi.h"
#else
#include "fimc-is-module-4ec-soc-reg.h" /* 4ECGX */
#endif
...
static struct fimc_is_sensor_cfg settle_4ec[] = {
	/* 3264x2448@30fps */
#if 1
	FIMC_IS_SENSOR_CFG(2576, 1932, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(2560, 1920, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(2560, 1536, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(2560, 1440, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(2048, 1536, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(2048, 1152, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(1920, 1920, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(1920, 1080, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(1600, 1200, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(1536, 1536, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(1280, 960, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(1280, 720, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(1024, 768, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(720, 720, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(720, 480, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(640, 480, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(352, 288, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(320, 240, 30, 7, 0, CSI_DATA_LANES_2),
	FIMC_IS_SENSOR_CFG(176, 144, 30, 7, 0, CSI_DATA_LANES_2),
#else
	FIMC_IS_SENSOR_CFG_SOC(3264, 2448, 30, 7, 0),
	FIMC_IS_SENSOR_CFG_SOC(1024, 768, 30, 7, 1),
#endif
};
	...
	const struct s5k4ecgx_regset_table *regset, u16 *data, u32 count)
{
	...
	for (i = 0; i < regset->array_size[set_idx]; i++) {
		...
		if (unlikely(ret < 0)) {
			//err("regs set(addr 0x%08X, size %d) apply is fail(%d)",
			//	regset->reg[set_idx], regset->array_size[set_idx], ret);
			goto p_err;
		}
	}
...
static int sensor_4ec_s_frame_rate(struct v4l2_subdev *subdev, int frame_rate)
{
	...
	cam_info("%s( %d ) : E\n",__func__, frame_rate);
	if (module_4ec->fps == frame_rate) {
		cam_info("%s: skipped frame rate set due to same set FPS\n", __func__);
		return ret;
	}
...
static int sensor_4ec_init(struct v4l2_subdev *subdev, u32 val)
{
...
#else
	{
		...
		end_regset = regset;
		cam_info("%s : first regset = %p, last regset = %p, count = %ld\n",
		...
		struct exynos_platform_fimc_is_module *pdata)
{
	...
	int gpio_reset = 0;
	int gpio_standby = 0;
#if defined (VDD_CAM_EN_GPIO_CONTROL)
	int gpio_core_en = 0;
#endif
	int gpio_none = 0;
//	int gpio_mclk = 0;
...
/*	gpio_mclk = of_get_named_gpio(dnode, "gpio_mclk", 0);
...
#else
	SET_PIN(pdata, SENSOR_SCENARIO_EXTERNAL, GPIO_SCENARIO_ON, gpio_none, "VDD28_CAM_AF", PIN_REGULATOR, 1, 2000);
#endif
...
#else
	SET_PIN(pdata, SENSOR_SCENARIO_EXTERNAL, GPIO_SCENARIO_ON, gpio_none, "VDD23_CAM_CORE_1P2", PIN_REGULATOR, 1, 1000);
#endif
	...
	SET_PIN(pdata, SENSOR_SCENARIO_EXTERNAL, GPIO_SCENARIO_OFF, gpio_none, "VDD18_CAM_SENSOR_IO", PIN_REGULATOR, 0, 150);
#if defined (VDD_CAM_SENSOR_A2P8_GPIO_CONTROL)
...
#else
	SET_PIN(pdata, SENSOR_SCENARIO_EXTERNAL, GPIO_SCENARIO_OFF, gpio_none, "VDD28_CAM_AF", PIN_REGULATOR, 0, 0);
#endif
#if defined (VDDD_1P2_CAM_GPIO_CONTROL)
	SET_PIN(pdata, SENSOR_SCENARIO_EXTERNAL, GPIO_SCENARIO_OFF, gpio_core_en, NULL, PIN_OUTPUT, 0, 0);
#else
	SET_PIN(pdata, SENSOR_SCENARIO_EXTERNAL, GPIO_SCENARIO_OFF, gpio_none, "VDD23_CAM_CORE_1P2", PIN_REGULATOR, 0, 0);
#endif
