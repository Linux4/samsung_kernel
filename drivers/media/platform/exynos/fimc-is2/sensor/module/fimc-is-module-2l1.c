...
static struct fimc_is_vci vci_2l1[] = {
	{
...
static int sensor_2l1_init(struct v4l2_subdev *subdev, u32 val)
{
	int ret = 0;
		...
		struct exynos_platform_fimc_is_module *pdata)
{
	...
	SET_PIN_VOLTAGE(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_none, "VDDA_2.9V_CAM", PIN_REGULATOR, 1, 0, 2950000); /* 2.875V */
#ifdef CONFIG_OIS_USE
	...
	SET_PIN_VOLTAGE(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_STANDBY_OFF_SENSOR, gpio_none, "VDDA_2.9V_CAM", PIN_REGULATOR, 1, 0, 2950000); /* 2.875V */
#ifdef CONFIG_OIS_USE
#ifdef CAMERA_USE_OIS_VDD_1_8V
	...
	SET_PIN_VOLTAGE(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_STANDBY_OFF, gpio_none, "VDDA_2.9V_CAM", PIN_REGULATOR, 1, 0, 2950000); /* 2.875V */
#ifdef CONFIG_OIS_USE
#ifdef CAMERA_USE_OIS_VDD_1_8V
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_STANDBY_ON, gpio_none, "VDDIO_1.8V_CAM", PIN_REGULATOR, 1, 0);
#else
...
int sensor_2l1_probe(struct platform_device *pdev)
{
	int ret = 0;
	...
	module->bitwidth = 10;
	module->vcis = ARRAY_SIZE(vci_2l1);
	module->vci = vci_2l1;
	module->sensor_maker = "SLSI";
	module->sensor_name = "S5K2L1";
	module->setfile_name = "setfile_2l1.bin";
	module->cfgs = ARRAY_SIZE(config_2l1);
	module->cfg = config_2l1;
	module->ops = NULL;
	...
	ext->sensor_con.product_name = SENSOR_NAME_S5K2L1;
...
static int sensor_2l1_remove(struct platform_device *pdev)
{
	int ret = 0;
...
static const struct of_device_id exynos_fimc_is_sensor_2l1_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-sensor-2l1",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_sensor_2l1_match);

static struct platform_driver sensor_2l1_driver = {
	.probe  = sensor_2l1_probe,
	.remove = sensor_2l1_remove,
	.driver = {
		.name   = SENSOR_NAME,
		.owner  = THIS_MODULE,
		.of_match_table = exynos_fimc_is_sensor_2l1_match,
	}
};

static int __init fimc_is_sensor_module_init(void)
{
	int ret = platform_driver_register(&sensor_2l1_driver);
	if (ret)
...
{
	platform_driver_unregister(&sensor_2l1_driver);
}
