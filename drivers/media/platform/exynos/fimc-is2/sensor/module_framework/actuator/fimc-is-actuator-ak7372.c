...
#define DEF_AK7372_FIRST_POSITION		120
#define DEF_AK7372_FIRST_DELAY			30

#define AK7372_PRODUCT_ID_ADDR  0x03
#define AK7372_POSITION1_ADDR  0x0 // AK7372 reg POSITION1
#define AK7372_POSITION2_ADDR  0x1 // AK7372 reg POSITION2
#define AK7372_ACTIVEMODE_ADDR  0x2 // AK7372 reg CONT1
#define AK7372_ACTIVEMODE_OUTDIS 0x0
...
static int sensor_ak7372_write_position(struct i2c_client *client, u32 val)
{
	...
	dbg_sensor("AK7372 val_high : 0x%04X\n", val_high);
	dbg_sensor("AK7372 val_low : 0x%04X\n", val_low);

	ret = fimc_is_sensor_addr8_write8(client, AK7372_POSITION1_ADDR, val_high);
	if (ret < 0)
		goto p_err;

	ret = fimc_is_sensor_addr8_write8(client, AK7372_POSITION2_ADDR, val_low);
	if (ret < 0)
		goto p_err;
		...
		struct fimc_is_actuator *actuator)
{
	...
	if (init_step > 0) {
		...
		ret = sensor_ak7372_write_position(client, DEF_AK7372_FIRST_POSITION);
		if (ret < 0)
			goto p_err;

		mdelay(DEF_AK7372_FIRST_DELAY);

		actuator->position = DEF_AK7372_FIRST_POSITION;

		dbg_sensor("initial position %d setting\n", DEF_AK7372_FIRST_POSITION);
	}
...
int sensor_ak7372_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	int first_position = DEF_AK7372_FIRST_POSITION;
	...
	// CheckProductID
	ret = fimc_is_sensor_addr8_read8(client, AK7372_PRODUCT_ID_ADDR, &product_id);
	if (ret < 0) {
#ifdef USE_CAMERA_HW_BIG_DATA
	...
	dbg_sensor("AK7372 product_id : 0x%04X\n", product_id);

	if (product_id != AK7372_PRODUCT_ID) {
		err("AK7372 is not detected(0x%04X)\n", product_id);
		goto p_err;
	}
	...
	ret = sensor_ak7372_write_position(client, first_position);
	if (ret <0)
		goto p_err;
	actuator->position = first_position;

	/* Go active mode */
	ret = fimc_is_sensor_addr8_write8(client, AK7372_ACTIVEMODE_ADDR, AK7372_ACTIVEMODE_OUTDIS);
	if (ret <0)
		goto p_err;
	...
	for(addr=0x00; addr<=0x25; addr++) {
		...
		err("AK7372 Register : addr = 0x%04X, cal_value = 0x%04X\n", addr, cal_value);
	}
#endif
...
int sensor_ak7372_actuator_probe(struct i2c_client *client,
		const struct i2c_device_id *id)
{
	...
	sensor_peri = find_peri_by_act_id(device, ACTUATOR_NAME_AK7372);
	if (!sensor_peri) {
...
static int sensor_ak7372_actuator_remove(struct i2c_client *client)
{
	int ret = 0;
...
static const struct of_device_id exynos_fimc_is_ak7372_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-actuator-ak7372",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_ak7372_match);

static const struct i2c_device_id actuator_ak7372_idt[] = {
	{ ACTUATOR_NAME, 0 },
	{},
};

static struct i2c_driver actuator_ak7372_driver = {
	.driver = {
		.name	= ACTUATOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_fimc_is_ak7372_match
	},
	.probe	= sensor_ak7372_actuator_probe,
	.remove	= sensor_ak7372_actuator_remove,
	.id_table = actuator_ak7372_idt
};
module_i2c_driver(actuator_ak7372_driver);
