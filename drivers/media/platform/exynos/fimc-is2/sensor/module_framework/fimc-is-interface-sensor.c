...
u32 get_min_exposure_time(struct fimc_is_sensor_interface *itf)
{
	...
	dbg_sen_itf("%s:(%d:%d) min exp(%d)\n", __func__,
		get_vsync_count(itf), get_frame_count(itf), exposure);
...
u32 get_min_analog_gain(struct fimc_is_sensor_interface *itf)
{
	...
	dbg_sen_itf("%s:(%d:%d) min analog gain(%d)\n", __func__,
		get_vsync_count(itf), get_frame_count(itf), again);
...
u32 get_min_digital_gain(struct fimc_is_sensor_interface *itf)
{
	...
	dbg_sen_itf("%s:(%d:%d) min digital gain(%d)\n", __func__,
		get_vsync_count(itf), get_frame_count(itf), dgain);
			...
			u32 short_exposure)
{
	...
	for (; i <= end_index; i++) {
		ret =  set_interface_param(itf, itf->cis_mode, ITF_CIS_PARAM_EXPOSURE,
				i, long_exposure, short_exposure);
		if (ret < 0) {
			pr_err("[%s] set_interface_param EXPOSURE fail(%d)\n", __func__, ret);
			goto p_err;
		}
			...
			u64 *line_period)
{
	...
	ret =  get_interface_param(itf, itf->cis_mode, ITF_CIS_PARAM_EXPOSURE,
			CURRENT_FRAME, long_exposure, short_exposure);
	if (ret < 0)
		pr_err("[%s] get_interface_param EXPOSURE fail(%d)\n", __func__, ret);
			...
			u32 short_analog_gain)
{
	...
	i = (itf->vsync_flag == false ? 0 : end_index);
	for (; i <= end_index; i++) {
		ret =  set_interface_param(itf, itf->cis_mode, ITF_CIS_PARAM_ANALOG_GAIN,
				i, long_analog_gain, short_analog_gain);
		if (ret < 0) {
			pr_err("[%s] set_interface_param EXPOSURE fail(%d)\n", __func__, ret);
			goto p_err;
		}
			...
			u32 *short_analog_gain)
{
	...
	ret =  get_interface_param(itf, itf->cis_mode, ITF_CIS_PARAM_ANALOG_GAIN,
			NEXT_FRAME, long_analog_gain, short_analog_gain);
	if (ret < 0)
			...
			u32 *short_analog_gain)
{
	...
	ret =  get_interface_param(itf, itf->cis_mode, ITF_CIS_PARAM_ANALOG_GAIN,
			CURRENT_FRAME, long_analog_gain, short_analog_gain);
	if (ret < 0)
		pr_err("[%s] get_interface_param ANALOG_GAIN fail(%d)\n", __func__, ret);
				...
				u32 *short_digital_gain)
{
	...
	ret =  get_interface_param(itf, itf->cis_mode, ITF_CIS_PARAM_DIGITAL_GAIN,
			NEXT_FRAME, long_digital_gain, short_digital_gain);
	if (ret < 0)
			...
			u32 *short_digital_gain)
{
	...
	ret =  get_interface_param(itf, itf->cis_mode, ITF_CIS_PARAM_DIGITAL_GAIN,
			CURRENT_FRAME, long_digital_gain, short_digital_gain);
	if (ret < 0)
		pr_err("[%s] get_interface_param DIGITAL_GAIN fail(%d)\n", __func__, ret);
			...
			u32 *frame_length_lines)
{
	...
	*frame_length_lines = sensor_peri->cis.cis_data->frame_length_lines;
	...
	dbg_sen_itf("[%s](%d:%d) pclk(%d), line_length_pck(%d), frame_length_lines(%d)\n",
		__func__, get_vsync_count(itf), get_frame_count(itf),
		*pclk, *line_length_pck, *frame_length_lines);
