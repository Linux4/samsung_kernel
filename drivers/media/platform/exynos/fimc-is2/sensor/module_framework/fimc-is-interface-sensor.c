...
int sensor_adjust_ctrl(struct fimc_is_sensor_interface *itf,
			u32 input,
			struct v4l2_control *ctrl)
{
	int ret = 0;
	...
	ret = fimc_is_sensor_peri_adj_ctrl(device, input, ctrl);
	if (ret < 0) {
		err("err!!! ret(%d)", ret);
		ret = -EINVAL;
		goto p_err;
...
u32 get_frame_count(struct fimc_is_sensor_interface *itf)
{
	...
	/* TODO: How can we get frame count at DMA mode */
...
u32 get_min_exposure_time(struct fimc_is_sensor_interface *itf)
{
	...
	dbg_sen_itf("%s:(%d) min exp(%d)\n", __func__, get_vsync_count(itf), exposure);
...
u32 get_min_analog_gain(struct fimc_is_sensor_interface *itf)
{
	...
	dbg_sen_itf("%s:(%d) min analog gain(%d)\n", __func__, get_vsync_count(itf), again);
...
u32 get_max_analog_gain(struct fimc_is_sensor_interface *itf)
{
	...
	dbg_sen_itf("%s:(%d) max analog gain(%d)\n", __func__, get_vsync_count(itf), again);
...
u32 get_min_digital_gain(struct fimc_is_sensor_interface *itf)
{
	...
	dbg_sen_itf("%s:(%d) min digital gain(%d)\n", __func__, get_vsync_count(itf), dgain);
...
u32 get_max_digital_gain(struct fimc_is_sensor_interface *itf)
{
	...
	dbg_sen_itf("%s:(%d) max digital gain(%d)\n", __func__, get_vsync_count(itf), dgain);
...
u32 get_vsync_count(struct fimc_is_sensor_interface *itf)
{
	...
	return atomic_read(&csi->vsync_count);
}
			...
			u32 short_exposure)
{
	...
	dbg_sen_itf("[%s](%d) long_exposure(%d), short_exposure(%d)\n", __func__, get_vsync_count(itf), long_exposure, short_exposure);
			...
			u64 *line_period)
{
	...
	*line_period = sensor_peri->cis.cis_data->line_readOut_time;
	...
	dbg_sen_itf("%s:(%d) exp(%d, %d), frame_period %d, line_period %lld\n", __func__, get_vsync_count(itf), *long_exposure, *short_exposure, *frame_period, *line_period);
			...
			u32 short_analog_gain)
{
	...
	dbg_sen_itf("[%s](%d) long_analog_gain(%d), short_analog_gain(%d)\n", __func__, get_vsync_count(itf), long_analog_gain, short_analog_gain);
		...
		u32 short_digital_gain)
{
	...
	dbg_sen_itf("[%s](%d) long_total_gain(%d), short_total_gain(%d)\n", __func__, get_vsync_count(itf), long_total_gain, short_total_gain);
	dbg_sen_itf("[%s](%d) long_analog_gain(%d), short_analog_gain(%d)\n", __func__, get_vsync_count(itf), long_analog_gain, short_analog_gain);
	dbg_sen_itf("[%s](%d) long_digital_gain(%d), short_digital_gain(%d)\n", __func__, get_vsync_count(itf), long_digital_gain, short_digital_gain);
			...
			u32 *frame_length_lines)
{
	...
	*frame_length_lines = sensor_peri->cis.cis_data->frame_length_lines;
	...
	dbg_sen_itf("[%s](%d) pclk(%d), line_length_pck(%d), frame_length_lines(%d)\n", __func__, get_vsync_count(itf), *pclk, *line_length_pck, *frame_length_lines);
			...
			bool executed)
{
	...
	sensor_ctl = get_sensor_ctl_from_module(itf, get_vsync_count(itf));
...
int end_of_frame(struct fimc_is_sensor_interface *itf)
{
	...
	if (itf->vsync_flag == true) {
		...
		if (itf->otf_flag_3aa == false) {
			...
			if (ret < 0)
				pr_err("[%s] get EXPOSURE_GAIN fail(%d)\n", __func__, ret);
