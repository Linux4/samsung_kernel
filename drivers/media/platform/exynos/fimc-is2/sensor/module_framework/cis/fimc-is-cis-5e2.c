...
#include "fimc-is-cis-5e2-setA.h"
...
int sensor_5e2_cis_init(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_cis_set_registers(subdev, sensor_5e2_setfiles[setfile_index], sensor_5e2_setfile_sizes[setfile_index]);
	if (ret < 0) {
...
int sensor_5e2_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	sensor_5e2_cis_group_param_hold_func(subdev, 0x00);
...
int sensor_5e2_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	dbg_sensor("[MOD:D:%d] %s, vt_pic_clk_freq_mhz (%d), line_length_pck(%d), min_fine_int (%d)\n", cis->id, __func__,
			vt_pic_clk_freq_mhz, line_length_pck, min_fine_int);
	...
	if (long_coarse_int > cis_data->max_coarse_integration_time) {
		...
		dbg_sensor("[MOD:D:%d] %s, vsync_cnt(%d), long coarse(%d) max\n", cis->id, __func__,
			cis_data->sen_vsync_count, long_coarse_int);
	}

	if (short_coarse_int > cis_data->max_coarse_integration_time) {
		short_coarse_int = cis_data->max_coarse_integration_time;
		dbg_sensor("[MOD:D:%d] %s, vsync_cnt(%d), short coarse(%d) max\n", cis->id, __func__,
			cis_data->sen_vsync_count, short_coarse_int);
	}

	if (long_coarse_int < cis_data->min_coarse_integration_time) {
		long_coarse_int = cis_data->min_coarse_integration_time;
		dbg_sensor("[MOD:D:%d] %s, vsync_cnt(%d), long coarse(%d) min\n", cis->id, __func__,
			cis_data->sen_vsync_count, long_coarse_int);
	}

	if (short_coarse_int < cis_data->min_coarse_integration_time) {
		short_coarse_int = cis_data->min_coarse_integration_time;
		dbg_sensor("[MOD:D:%d] %s, vsync_cnt(%d), short coarse(%d) min\n", cis->id, __func__,
			cis_data->sen_vsync_count, short_coarse_int);
	}

	dbg_sensor("[MOD:D:%d] %s, frame_length_lines(%#x), long_coarse_int %#x, short_coarse_int %#x\n", cis->id, __func__,
						...
						u32 *target_duration)
{
	...
	dbg_sensor("[%s] input exp(%d), adj duration, frame duraion(%d), min_frame_us(%d)\n",
			__func__, input_exposure_time, frame_duration, cis_data->min_frame_us_time);
	dbg_sensor("[%s] adj duration, frame duraion(%d), min_frame_us(%d)\n",
...
int sensor_5e2_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	dbg_sensor("[MOD:D:%d] %s, input_again = %d us, analog_gain(%#x)\n",
...
int sensor_5e2_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	dbg_sensor("[MOD:D:%d] %s, input_dgain = %d/%d us, long_gain(%#x), short_gain(%#x)\n",
...
int cis_5e2_probe(struct i2c_client *client,
...
static int cis_5e2_remove(struct i2c_client *client)
{
	int ret = 0;
	return ret;
}

static const struct of_device_id exynos_fimc_is_cis_5e2_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-5e2",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_cis_5e2_match);

static const struct i2c_device_id cis_5e2_idt[] = {
	{ SENSOR_NAME, 0 },
};

static struct i2c_driver cis_5e2_driver = {
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_fimc_is_cis_5e2_match
	},
	.probe	= cis_5e2_probe,
	.remove	= cis_5e2_remove,
	.id_table = cis_5e2_idt
};
module_i2c_driver(cis_5e2_driver);
