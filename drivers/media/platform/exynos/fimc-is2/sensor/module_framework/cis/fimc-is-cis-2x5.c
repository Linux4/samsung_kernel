...
int sensor_2x5_cis_write_cal(struct v4l2_subdev *subdev)
{
	...
	u8 *xtalk_buf = NULL, *lsc_buf = NULL;
	char *cal_buf = NULL;
#ifdef CONFIG_VENDER_MCD_V2
...
#endif
	xtalk_buf = &cal_buf[finfo->sensor_cal_data_start_addr];
	lsc_buf = &cal_buf[finfo->sensor_cal_data_start_addr + SENSOR_2X5_CAL_XTALK_SIZE];
	...
	ret = fimc_is_sensor_write16(client, 0x6028, SENSOR_2X5_XTALK_ADDR_PAGE);
	ret |= fimc_is_sensor_write16(client, 0x602A, SENSOR_2X5_XTALK_ADDR_OFFSET);
	if (ret < 0) {
		err("[xtalk] i2c fail addr(%x, %x) ,ret = %d\n", 
			SENSOR_2X5_XTALK_ADDR_PAGE, SENSOR_2X5_LSC_ADDR_OFFSET, ret);
		goto p_err;
	}
	for (i = 0; i < SENSOR_2X5_CAL_XTALK_SIZE; i++) {
		ret = fimc_is_sensor_write16(client, 0x6F12, xtalk_buf[i]);
		if (ret < 0) {
			err("[xtalk] i2c fail addr(%x), val(%x), ret = %d\n",
				SENSOR_2X5_XTALK_ADDR_OFFSET + i, xtalk_buf[i], ret);
			goto p_err;
		}
	}
	...
	ret = fimc_is_sensor_write16(client, 0x6028, SENSOR_2X5_LSC_ADDR_PAGE);
	ret |= fimc_is_sensor_write16(client, 0x602A, SENSOR_2X5_LSC_ADDR_OFFSET);
	if (ret < 0) {
		err("[lsc] i2c fail addr(%x, %x) ,ret = %d\n", SENSOR_2X5_LSC_ADDR_PAGE, SENSOR_2X5_LSC_ADDR_OFFSET, ret);
		goto p_err;
	}
	for (i = 0; i < SENSOR_2X5_CAL_LSC_SIZE; i++) {
		ret = fimc_is_sensor_write16(client, 0x6F12, xtalk_buf[i]);
		if (ret < 0) {
			err("[lsc] i2c fail addr(%x), val(%x), ret = %d\n",
				SENSOR_2X5_LSC_ADDR_OFFSET + i, lsc_buf[i], ret);
			goto p_err;
		}
	}
...
int sensor_2x5_cis_init(struct v4l2_subdev *subdev)
{
	...
	if (version_id == SENSOR_2X5_VER_SP13) {
		...
		last_version = true;
		err("%s : invalid version ID 0x%02X\n", __func__, version_id);
		info("%s : EVT1 setfile_B\n", __func__);
		sensor_2x5_global = sensor_2x5_setfile_B_Global;
		sensor_2x5_global_size = ARRAY_SIZE(sensor_2x5_setfile_B_Global);
		sensor_2x5_setfiles = sensor_2x5_setfiles_B;
...
int sensor_2x5_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (last_version == false) {
		dbg_sensor(1, "%s: write Tetra ISP\n", __func__);
...
#endif
	/* Be careful for i2c lock position */
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
	...
	info("%s: mode changed(%d)%s\n", __func__, mode, last_version ? " " : ((isp_mode == TETRA_ISP_24MP) ? " Remosaic" : " Tetra"));
...
static struct fimc_is_cis_ops cis_ops = {
	...
	.cis_mode_change = sensor_2x5_cis_mode_change,
