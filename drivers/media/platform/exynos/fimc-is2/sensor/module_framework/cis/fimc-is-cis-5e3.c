...
int sensor_5e3_cis_init(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_cis_set_registers(subdev, sensor_5e3_setfiles[setfile_index], sensor_5e3_setfile_sizes[setfile_index]);
	if (ret < 0) {
	...
	fimc_is_sensor_write8(client, 0xA02, 0x02);
	fimc_is_sensor_write8(client, 0xA00, 0x01);
	...
	if (selected_page == 0x3) {
		...
		fimc_is_sensor_write8(client, 0xA00, 0x04);
		fimc_is_sensor_write8(client, 0xA00, 0x00);

		msleep(1);

		fimc_is_sensor_write8(client, 0xA00, 0x04);
		fimc_is_sensor_write8(client, 0xA02, 0x03);
		fimc_is_sensor_write8(client, 0xA00, 0x01);
	}
	fimc_is_sensor_read16(client, 0x0A04, &data16[0]);
	fimc_is_sensor_read16(client, 0x0A06, &data16[1]);
	fimc_is_sensor_read16(client, 0x0A08, &data16[2]);
	fimc_is_sensor_read16(client, 0x0A0A, &data16[3]);
...
int sensor_5e3_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	sensor_5e3_cis_group_param_hold_func(subdev, 0x00);
