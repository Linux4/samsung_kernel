...
#define SENSOR_NAME "SR556"
/* #define DEBUG_SR556_PLL */
...
static const u32 *sensor_sr556_global;
static u32 sensor_sr556_global_size;
static const u32 **sensor_sr556_setfiles;
static const u32 *sensor_sr556_setfile_sizes;
static const struct sensor_pll_info **sensor_sr556_pllinfos;
static u32 sensor_sr556_max_setfile_num;

static void sensor_sr556_cis_data_calculation(const struct sensor_pll_info *pll_info, cis_shared_data *cis_data)
{
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_SR556_FINE_INTEGRATION_TIME_MIN;
	cis_data->max_fine_integration_time = SENSOR_SR556_FINE_INTEGRATION_TIME_MAX;
	cis_data->min_coarse_integration_time = SENSOR_SR556_COARSE_INTEGRATION_TIME_MIN;
	cis_data->max_margin_coarse_integration_time = SENSOR_SR556_COARSE_INTEGRATION_TIME_MAX_MARGIN;
}

static int sensor_sr556_wait_stream_off_status(cis_shared_data *cis_data)
{
	int ret = 0;
	u32 timeout = 0;
...
/* CIS OPS */
int sensor_sr556_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
...
#endif
		warn("sensor_sr556_check_rev is fail when cis init");
		cis->rev_flag = true;
		goto p_err;
	}

	cis->cis_data->cur_width = SENSOR_SR556_MAX_WIDTH;
	cis->cis_data->cur_height = SENSOR_SR556_MAX_HEIGHT;
	cis->cis_data->low_expo_start = 33000;
	cis->need_mode_change = false;

	sensor_sr556_cis_data_calculation(sensor_sr556_pllinfos[setfile_index], cis->cis_data);
...
int sensor_sr556_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
...
static int sensor_sr556_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	int ret = 0;
...
#else
static inline int sensor_sr556_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	return 0;
}
#endif
 ...
 */
int sensor_sr556_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
	...
	ret = sensor_sr556_cis_group_param_hold_func(subdev, hold);
	if (ret < 0)
		goto p_err;
...
int sensor_sr556_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
	...
	ret = sensor_cis_set_registers(subdev, sensor_sr556_global, sensor_sr556_global_size);

	if (ret < 0) {
		err("sensor_sr556_set_registers fail!!");
		goto p_err;
	}
...
int sensor_sr556_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	...
	max_setfile_num = sensor_sr556_max_setfile_num;
	...
	if (cis->rev_flag == true) {
		...
		ret = sensor_cis_check_rev(cis);
		if (ret < 0) {
			err("sensor_sr556_check_rev is fail");
			goto p_err;
		}
	}

	sensor_sr556_cis_data_calculation(sensor_sr556_pllinfos[mode], cis->cis_data);

	ret = sensor_cis_set_registers(subdev, sensor_sr556_setfiles[mode], sensor_sr556_setfile_sizes[mode]);
	if (ret < 0) {
		err("sensor_sr556_set_registers fail!!");
		goto p_err;
	}
...
int sensor_sr556_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	int ret = 0;
	bool binning = false;
	...
	ret = sensor_sr556_wait_stream_off_status(cis_data);
	if (ret) {
		err("Must stream off\n");
		ret = -EINVAL;
		goto p_err;
	...
	if (binning) {
		ratio_w = (SENSOR_SR556_MAX_WIDTH / cis_data->cur_width);
		ratio_h = (SENSOR_SR556_MAX_HEIGHT / cis_data->cur_height);
	} else {
		ratio_w = 1;
		ratio_h = 1;
	}

	if (((cis_data->cur_width * ratio_w) > SENSOR_SR556_MAX_WIDTH) ||
		((cis_data->cur_height * ratio_h) > SENSOR_SR556_MAX_HEIGHT)) {
		err("Config max sensor size over~!!\n");
		ret = -EINVAL;
		goto p_err;
	}

	start_x = ((SENSOR_SR556_MAX_WIDTH - cis_data->cur_width * ratio_w) / 2) & (~0x1);
	start_y = ((SENSOR_SR556_MAX_WIDTH - cis_data->cur_height * ratio_h) / 2) & (~0x1);
...
int sensor_sr556_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	...
	ret = sensor_sr556_cis_group_param_hold_func(subdev, 0x00);
	if (ret < 0)
		err("[%s] sensor_sr556_cis_group_param_hold_func fail\n", __func__);

#ifdef DEBUG_SR556_PLL
	{
	u8 pll_8;
	u16 pll;
	dbg_sensor("______ vt_pix_clk_div(%x)\n", 0x03);
	ret = fimc_is_sensor_read8(client, 0x0F34, &pll_8);
	dbg_sensor("______ vt_sys_clk_div1(%x)vt_sys_clk_div2(%x)\n", (pll_8 & 0x07), (pll_8 & 0x18));
...
#endif
	ret = fimc_is_sensor_write16(client, 0x0040, 0x0000);
	if (ret < 0)
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0114, 0x0100, ret);
	ret = fimc_is_sensor_write16(client, 0x0042, 0x0100);
	if (ret < 0)
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0114, 0x0100, ret);
	ret = fimc_is_sensor_write16(client, 0x003e, 0x0000);
	if (ret < 0)
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0114, 0x0100, ret);


	/* Sensor stream on */
	ret = fimc_is_sensor_write16(client, 0x0114, 0x0100);
	if (ret < 0)
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0114, 0x0100, ret);
...
int sensor_sr556_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
	...
	/* Sensor stream off */
	ret = fimc_is_sensor_write16(client, 0x0114, 0x0000);
	if (ret < 0)
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0114, 0x00, ret);
...
int sensor_sr556_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	hold = sensor_sr556_cis_group_param_hold_func(subdev, 0x01);
	if (hold < 0) {
		ret = hold;
		goto p_err;
	}
	...
	dbg_sensor("[MOD:D:%d] %s, vsync_cnt(%d), vt_pic_clk_freq_mhz (%d), line_length_pck(%d)\n", cis->id, __func__,
...
p_err:
	if (hold > 0) {
		hold = sensor_sr556_cis_group_param_hold_func(subdev, 0x00);
		if (hold < 0)
			ret = hold;
	}
...
int sensor_sr556_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
...
int sensor_sr556_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_sr556_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_sr556_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_sr556_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
	...
	ret = sensor_sr556_cis_set_frame_duration(subdev, frame_duration);
	if (ret < 0) {
...
/* AG = ( (1/16) * code ) + 1 */
u32 sensor_sr556_cis_calc_again_code(u32 permile)
{
	return ((permile - 1000) * 16 / 1000);
}

u32 sensor_sr556_cis_calc_again_permile(u32 code)
{
	return ((code * 1000 / 16 + 1000));
}

u32 sensor_sr556_cis_calc_dgain_code(u32 permile)
{
	u16 buf[2] = {0, 0};
...
int sensor_sr556_cis_adjust_analog_gain(struct v4l2_subdev *subdev, u32 input_again, u32 *target_permile)
{
	...
	again_code = sensor_sr556_cis_calc_again_code(input_again);
	...
	again_permile = sensor_sr556_cis_calc_again_permile(again_code);
...
int sensor_sr556_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	analog_gain = (u16)sensor_sr556_cis_calc_again_code(again->val);
...
int sensor_sr556_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	*again = sensor_sr556_cis_calc_again_permile(analog_gain);
...
int sensor_sr556_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
	...
	cis_data->min_analog_gain[1] = sensor_sr556_cis_calc_again_permile(read_value);
...
int sensor_sr556_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
	...
	cis_data->max_analog_gain[1] = sensor_sr556_cis_calc_again_permile(read_value);
...
int sensor_sr556_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	long_gain = (u16)sensor_sr556_cis_calc_dgain_code(dgain->long_val);
	short_gain = (u16)sensor_sr556_cis_calc_dgain_code(dgain->short_val);
...
int sensor_sr556_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	*dgain = sensor_sr556_cis_calc_dgain_permile(digital_gain);
...
int sensor_sr556_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
	...
	cis_data->min_digital_gain[1] = sensor_sr556_cis_calc_dgain_permile(read_value);
...
int sensor_sr556_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
	...
	/* sr556 cannot read min/max digital gain */
	read_value = 0x1FFF;

	cis_data->max_digital_gain[0] = read_value;

	cis_data->max_digital_gain[1] = sensor_sr556_cis_calc_dgain_permile(read_value);
...
int sensor_sr556_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u32 poll_time_ms = 0;
	struct fimc_is_cis *cis;
...
int sensor_sr556_cis_wait_streamon(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u32 poll_time_ms = 0;
	struct fimc_is_cis *cis;
...
static struct fimc_is_cis_ops cis_ops = {
	.cis_init = sensor_sr556_cis_init,
	.cis_log_status = sensor_sr556_cis_log_status,
	.cis_group_param_hold = sensor_sr556_cis_group_param_hold,
	.cis_set_global_setting = sensor_sr556_cis_set_global_setting,
	.cis_mode_change = sensor_sr556_cis_mode_change,
	.cis_set_size = sensor_sr556_cis_set_size,
	.cis_stream_on = sensor_sr556_cis_stream_on,
	.cis_stream_off = sensor_sr556_cis_stream_off,
	.cis_set_exposure_time = sensor_sr556_cis_set_exposure_time,
	.cis_get_min_exposure_time = sensor_sr556_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_sr556_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_sr556_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_sr556_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_sr556_cis_set_frame_rate,
	.cis_adjust_analog_gain = sensor_sr556_cis_adjust_analog_gain,
	.cis_set_analog_gain = sensor_sr556_cis_set_analog_gain,
	.cis_get_analog_gain = sensor_sr556_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_sr556_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_sr556_cis_get_max_analog_gain,
	.cis_set_digital_gain = sensor_sr556_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_sr556_cis_get_digital_gain,
	.cis_get_min_digital_gain = sensor_sr556_cis_get_min_digital_gain,
	.cis_get_max_digital_gain = sensor_sr556_cis_get_max_digital_gain,
	.cis_compensate_gain_for_extremely_br = sensor_cis_compensate_gain_for_extremely_br,
	.cis_wait_streamoff = sensor_sr556_cis_wait_streamoff,
};

int cis_sr556_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	sensor_peri = find_peri_by_cis_id(device, SENSOR_NAME_SR556);
	if (!sensor_peri) {
	...
	if (strcmp(setfile, "default") == 0 ||
		...
		sensor_sr556_global_size = sizeof(sensor_sr556_setfile_A_Global) / sizeof(sensor_sr556_setfile_A_Global[0]);
		sensor_sr556_setfiles = sensor_sr556_setfiles_A;
		sensor_sr556_setfile_sizes = sensor_sr556_setfile_A_sizes;
		sensor_sr556_pllinfos = sensor_sr556_pllinfos_A;
		sensor_sr556_max_setfile_num = sizeof(sensor_sr556_setfiles_A) / sizeof(sensor_sr556_setfiles_A[0]);
	} else if (strcmp(setfile, "setB") == 0) {
		probe_info("%s : setfile_B\n", __func__);
		sensor_sr556_global = sensor_sr556_setfile_B_Global;
		sensor_sr556_global_size = sizeof(sensor_sr556_setfile_B_Global) / sizeof(sensor_sr556_setfile_B_Global[0]);
		sensor_sr556_setfiles = sensor_sr556_setfiles_B;
		sensor_sr556_setfile_sizes = sensor_sr556_setfile_B_sizes;
		sensor_sr556_pllinfos = sensor_sr556_pllinfos_B;
		sensor_sr556_max_setfile_num = sizeof(sensor_sr556_setfiles_B) / sizeof(sensor_sr556_setfiles_B[0]);
	} else {
...
static int cis_sr556_remove(struct i2c_client *client)
{
	int ret = 0;
	return ret;
}

static const struct of_device_id exynos_fimc_is_cis_sr556_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-sr556",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_cis_sr556_match);

static const struct i2c_device_id cis_sr556_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver cis_sr556_driver = {
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_fimc_is_cis_sr556_match
	},
	.probe	= cis_sr556_probe,
	.remove	= cis_sr556_remove,
	.id_table = cis_sr556_idt
};
module_i2c_driver(cis_sr556_driver);
