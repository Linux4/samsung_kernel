...
int sensor_4h5yc_cis_init(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_cis_set_registers(subdev, sensor_4h5yc_setfiles[setfile_index], sensor_4h5yc_setfile_sizes[setfile_index]);
	if (ret < 0) {
...
int sensor_4h5yc_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	dbg_sensor("[MOD:D:%d] %s, vsync_cnt(%d), vt_pic_clk_freq_mhz (%d), line_length_pck(%d), fine_int (%d)\n",
		cis->id, __func__, cis_data->sen_vsync_count, vt_pic_clk_freq_mhz, line_length_pck, fine_int);
	dbg_sensor("[MOD:D:%d] %s, vsync_cnt(%d), frame_length_lines(%#x), long_coarse_int %#x, short_coarse_int %#x\n",
		cis->id, __func__, cis_data->sen_vsync_count, cis_data->frame_length_lines, long_coarse_int, short_coarse_int);
...
int sensor_4h5yc_cis_compensate_gain_for_extremely_br(struct v4l2_subdev *subdev,
