...
static int sensor_module_sr556_power_setpin(struct device *dev,
	struct exynos_platform_fimc_is_module *pdata)
{
	...
	gpio_sensor_a2p8_en = of_get_named_gpio(dnode, "gpio_cam_2p8_en", 0);
	...
	gpio_1p2_en = of_get_named_gpio(dnode, "gpio_cam_1p2_en", 0);
	...
	gpio_1p8_a2p8_en = of_get_named_gpio(dnode, "gpio_camio_1p8_cam_a2p8_en", 0);
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_none, "delay", PIN_NONE, 0, 11000);
	...
	if (gpio_is_valid(gpio_1p8_a2p8_en)) {
		SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, gpio_1p8_a2p8_en, "1p8_a2p8_en", PIN_OUTPUT, 0, 0);
	}
	if (gpio_is_valid(gpio_1p8_en)) {
	...
	SET_PIN(pdata, SENSOR_SCENARIO_READ_ROM, GPIO_SCENARIO_ON, gpio_none, "delay", PIN_NONE, 0, 11000);
#else
	if (gpio_is_valid(gpio_1p8_en)) {
	...
	SET_PIN(pdata, SENSOR_SCENARIO_READ_ROM, GPIO_SCENARIO_OFF, gpio_none, "pin", PIN_FUNCTION, 1, 1000);
	if (gpio_is_valid(gpio_1p8_a2p8_en)) {
		SET_PIN(pdata, SENSOR_SCENARIO_READ_ROM, GPIO_SCENARIO_OFF, gpio_1p8_a2p8_en, "1p8_a2p8_en", PIN_OUTPUT, 0, 0);
	}
	if (gpio_is_valid(gpio_1p8_en)) {
...
int sensor_module_sr556_probe(struct platform_device *pdev)
{
	int ret = 0;
	...
	fimc_is_module_parse_dt(dev, sensor_module_sr556_power_setpin);
	...
	module->bitwidth = 10;
	module->vcis = ARRAY_SIZE(vci_module_sr556);
	module->vci = vci_module_sr556;
	module->sensor_maker = "skhynix";
	module->sensor_name = "SR556";
	module->setfile_name = "setfile_sr556.bin";
	module->cfgs = ARRAY_SIZE(config_module_sr556);
	module->cfg = config_module_sr556;
	module->ops = NULL;
	/* Sensor peri */
...
static int sensor_module_sr556_remove(struct platform_device *pdev)
{
	int ret = 0;
...
static const struct of_device_id exynos_fimc_is_sensor_module_sr556_match[] = {
	{
		.compatible = "samsung,sensor-module-sr556",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_sensor_module_sr556_match);

static struct platform_driver sensor_module_sr556_driver = {
	.probe  = sensor_module_sr556_probe,
	.remove = sensor_module_sr556_remove,
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-SR556",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_fimc_is_sensor_module_sr556_match,
	}
};

module_platform_driver(sensor_module_sr556_driver);
