...
static struct fimc_is_sensor_cfg config_module_2p6[] = {
	...
	/* 1152x864@120fps */
	FIMC_IS_SENSOR_CFG_EXT(1152, 864, 120, 30, 11, CSI_DATA_LANES_4, 1352, 0, 0, 0),
	/* 1152x656@120fps */
	FIMC_IS_SENSOR_CFG_EXT(1152, 656, 120, 30, 12, CSI_DATA_LANES_4, 1352, 0, 0, 0),
};
	...
	struct exynos_platform_fimc_is_module *pdata)
{
	...
	/* READ_ROM - POWER ON */
	if (gpio_is_valid(gpio_cam_io_en)) {
		SET_PIN(pdata, SENSOR_SCENARIO_READ_ROM, GPIO_SCENARIO_ON, gpio_cam_io_en, "pdaf gpio_cam_io_en", PIN_OUTPUT, 1, 0);
	} else {
		SET_PIN(pdata, SENSOR_SCENARIO_READ_ROM, GPIO_SCENARIO_ON, gpio_none, "CAM_VDDIO_1P8", PIN_REGULATOR, 1, 0);
	}

	if (gpio_is_valid(gpio_cam_af_en)) {
		SET_PIN(pdata, SENSOR_SCENARIO_READ_ROM, GPIO_SCENARIO_ON, gpio_cam_af_en, "pdaf gpio_cam_af_en", PIN_OUTPUT, 1, 1000);
	} else {
		SET_PIN(pdata, SENSOR_SCENARIO_READ_ROM, GPIO_SCENARIO_ON, gpio_none, "VDD_CAM_AF_2P8", PIN_REGULATOR, 1, 1000);
	}
