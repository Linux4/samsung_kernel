...
#ifndef CONFIG_ENABLE_HAL3_2_META_INTERFACE
			sensor_peri->cis.expecting_sensor_dm[dm_index[0]].analogGain = 0;
			sensor_peri->cis.expecting_sensor_dm[dm_index[0]].digitalGain = 0;
#endif
		} else {
...
#ifndef CONFIG_ENABLE_HAL3_2_META_INTERFACE
			sensor_peri->cis.expecting_sensor_dm[dm_index[0]].analogGain = adj_gain_setting->long_again;
			sensor_peri->cis.expecting_sensor_dm[dm_index[0]].digitalGain = adj_gain_setting->long_dgain;
#endif
		}
	} else {
		sensor_peri->cis.expecting_sensor_dm[dm_index[0]].sensitivity = sensor_peri->cis.expecting_sensor_dm[dm_index[1]].sensitivity;
#ifndef CONFIG_ENABLE_HAL3_2_META_INTERFACE
		sensor_peri->cis.expecting_sensor_dm[dm_index[0]].analogGain = sensor_peri->cis.expecting_sensor_dm[dm_index[1]].analogGain;
		sensor_peri->cis.expecting_sensor_dm[dm_index[0]].digitalGain = sensor_peri->cis.expecting_sensor_dm[dm_index[1]].digitalGain;
#endif
	}
...
void fimc_is_sensor_ctl_frame_evt(struct fimc_is_device_sensor *device)
{
	...
	if(vsync_count < DIFF_BET_SEN_ISP + 1) {
		err("%s: vsync_count(%d) < DIFF_BET_SEN_ISP + 1(%d)\n", __func__, vsync_count, DIFF_BET_SEN_ISP + 1);
		goto p_err;
	}
	...
	applied_frame_number = vsync_count - DIFF_BET_SEN_ISP;
	...
	if (module_ctl->sensor_frame_number == applied_frame_number && module_ctl->alg_reset_flag == true) {
		...
		if (module_ctl->sensor_frame_number == applied_frame_number) {
			err("[%s] frame number(%d) Shot process of AE is too slow\n", __func__, applied_frame_number);
		} else {
			dbg_sensor("module->sen_framenum(%d), applied_frame_num(%d), alg_reset_flag (%d)\n",
