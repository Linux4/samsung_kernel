...
#endif
#ifdef ENABLE_FD_SW
	fw_size += SIZE_FD_INTERNEL_BUF * NUM_FD_INTERNAL_BUF;
#endif
...
static int fimc_is_resourcemgr_initmem(struct fimc_is_resourcemgr *resourcemgr)
{
	...
	resourcemgr->minfo.dvaddr_dshared = resourcemgr->minfo.dvaddr + offset;
	resourcemgr->minfo.kvaddr_dshared = resourcemgr->minfo.kvaddr + offset;
...
static int fimc_is_resourcemgr_allocmem(struct fimc_is_resourcemgr *resourcemgr)
{
	...
	fw_size += FIMC_IS_LHFD_MAP_SIZE + FIMC_IS_GUARD_SIZE;
	fw_size += FIMC_IS_RESERVE_SIZE + FIMC_IS_GUARD_SIZE;
#endif

	probe_info("[RSC] Internal memory size : %08x\n", fw_size);
	fw_size = PAGE_ALIGN(fw_size);
	probe_info("[RSC] Internal memory size (aligned) : %08x\n", fw_size);
...
#ifdef CONFIG_ARGOS
	/* dm hotlplug disable as camera on */
...
#ifdef CONFIG_ARGOS
	/* dm hotlplug enable as camera off */
	if (atomic_read(&core->rsccount) == 1)
