...
static const struct scp_param init_scp_param = {
	...
	.otf_input = {
		...
		.width = DEFAULT_CAPTURE_VIDEO_WIDTH,
		.height = DEFAULT_CAPTURE_VIDEO_HEIGHT,
#if defined(CONFIG_SOC_EXYNOS3475) /* FIXME */
		.format = OTF_INPUT_FORMAT_YUV422,
		.bitwidth = OTF_INPUT_BIT_WIDTH_12BIT,
#else
		.format = OTF_INPUT_FORMAT_YUV444,
...
void fimc_is_ischain_version(enum fimc_is_bin_type type, const char *load_bin, u32 size)
{
	...
	case FIMC_IS_BIN_LIBRARY:
		...
		info("Library version : %s\n", version_str);
		break;
	default:
		info("Invaid FW type (%d)\n", (int)type);
		break;
	}
}
	...
	struct fimc_is_module_enum *active_sensor, int position)
{
	...
	info("start_addr : 0x%x, cal_size : 0x%x, cal_ptr : 0x%p\n",
		start_addr, cal_size, cal_ptr);
	...
	struct fimc_is_module_enum *active_sensor, int position)
{
	...
	if (ret)
		...
		mwarn("calibration loading is success", device);
#else /* #ifdef CONFIG_USE_VENDER_FEATURE */
	...
	if (active_sensor->position == SENSOR_POSITION_REAR) {
		ret = request_binary(&cal_bin, FIMC_IS_REAR_CAL_SDCARD_PATH,
								FIMC_IS_REAR_CAL, NULL);
		cal_addr = FIMC_IS_REAR_CALDATA_OFFSET;
	} else if (active_sensor->position == SENSOR_POSITION_FRONT) {
		ret = request_binary(&cal_bin, FIMC_IS_FRONT_CAL_SDCARD_PATH,
								FIMC_IS_FRONT_CAL, NULL);
		cal_addr = FIMC_IS_FRONT_CALDATA_OFFSET;
	} else {
		merr("Invalid sensor position: %d", device, active_sensor->position);
		return -EINVAL;
	}

	if (ret) {
		merr("[%d]File loading of cal data is fail!! (%d)", device,
							active_sensor->position, ret);
		goto exit;
	}

#ifndef ENABLE_IS_CORE
	for (i = 0; i < HW_SLOT_MAX; i++)
		device->hardware->hw_ip[i].sensor_position = active_sensor->position;
#endif

	memcpy((void *)(device->imemory.kvaddr + cal_addr), (void *)cal_bin.data, cal_bin.size);

exit:
	release_binary(&cal_bin);

	if (ret)
		err("CAL data loading is fail");
	else
		info("CAL data loading is complete\n");
#endif
	return ret;
}
#endif
...
int fimc_is_ischain_loadfd(struct fimc_is_device_ischain *device)
{
	...
	fd_bin = FD_LIB_BASE;
#endif
	set_memory_xn((unsigned long)fd_bin, PFN_UP(SZ_512K));
	set_memory_rw((unsigned long)fd_bin, PFN_UP(SZ_512K));

	setup_binary_loader(&bin, 4, 0, NULL, NULL);
	ret = request_binary(&bin, FD_SW_SDCARD_PATH, FD_SW_BIN_NAME,
					&device->pdev->dev);
	if (ret) {
		err_lib("failed to load fd library (%d)", ret);
		return ret;
	}
	info("binary info - type: F, addr: %#x, from: %s\n", (u32)fd_bin,
		was_loaded_by(&bin) ? "built-in" : "user-provided");
	memcpy((void *)fd_bin, bin.data, bin.size);
	set_memory_ro((unsigned long)fd_bin, PFN_UP(bin.size));
	set_memory_x((unsigned long)fd_bin, PFN_UP(bin.size));
	release_binary(&bin);
...
#if defined(CONFIG_ARM_TRUSTZONE) && !defined(CONFIG_SOC_EXYNOS3475)
	u32 i;
	...
	u32 indexes)
{
	...
	if (frame) {
		...
		ret = fimc_is_itf_s_param_wrap(device, lindex, hindex, indexes);
		if (ret)
			merr("fimc_is_itf_s_param_wrap is fail(%d)", device, ret);
	...
	u32 group)
{
	...
	if (ret)
		merr("fimc_is_itf_a_param_wrap is fail(%d)", device, ret);
...
static int fimc_is_itf_f_param(struct fimc_is_device_ischain *device)
{
	...
	if (ret)
		merr("fimc_is_itf_f_param_wrap is fail(%d)", device, ret);
...
static int fimc_is_itf_enum(struct fimc_is_device_ischain *device)
{
	...
	if (ret)
		merr("fimc_is_itf_enum_wrap is fail(%d)", device, ret);
...
int fimc_is_itf_set_fwboot(struct fimc_is_device_ischain *device, u32 val)
{
	...
	if (ret)
		merr("fimc_is_itf_set_fwboot_wrap is fail(%d)", device, ret);
	...
	struct sensor_open_extended *ext_info)
{
	...
	ret = fimc_is_itf_open_wrap(device,
		...
		offset_path);
	if (ret) {
		merr("fimc_is_itf_open_wrap is fail", device);
		ret = -EINVAL;
		goto p_err;
	}
...
static int fimc_is_itf_close(struct fimc_is_device_ischain *device)
{
	...
	if (ret)
		merr("fimc_is_itf_close_wrap is fail", device);
	...
	char *setfile_name)
{
	...
	if (ret) {
		merr("fimc_is_itf_saddr_wrap is fail(%d)", device, ret);
		goto p_err;
	}
...
#endif
	setfile_addr += device->imemory.kvaddr;
	ret = fimc_is_itf_setfile_wrap(itf, setfile_addr, device);
	if (ret) {
		merr("fimc_is_itf_setfile_wrap is fail(%d)", device, ret);
		goto p_err;
	}
...
#ifdef ENABLE_FD_SW
	{
		...
		if (!unlikely(device->fd_lib)) {
			ret = -EINVAL;
			merr("fd_library is null(%d)\n", device, ret);
			goto p_err;
		}
		fd_lib = device->fd_lib;

		ret = fimc_is_lib_fd_load_setfile(fd_lib, setfile_addr);
		if (ret) {
			merr("fimc_is_lib_fd_setfile_load is fail(%d)\n", device, ret);
			goto p_err;
		}
	}
#endif
	...
	u32 group, u32 shot_addr, u32 shot_size)
{
	...
	if (ret)
		merr("fimc_is_itf_map_wrap is fail(%d)", device, ret);
	...
	u32 group)
{
	...
	if (ret)
		merr("fimc_is_itf_unmap_wrap is fail(%d)", device, ret);
...
int fimc_is_itf_stream_on(struct fimc_is_device_ischain *device)
{
	...
	if (ret)
		merr("fimc_is_itf_stream_on_wrap is fail(%d)", device, ret);
	...
	u32 group)
{
	...
	if (ret)
		merr("fimc_is_itf_process_on_wrap is fail(%d)", device, ret);
	...
	u32 group)
{
	...
	if (ret)
		merr("fimc_is_itf_process_off_wrap is fail(%d)", device, ret);
...
int fimc_is_itf_power_down(struct fimc_is_interface *interface)
{
	...
	ret = fimc_is_itf_power_down_wrap(interface);
	if (ret)
		err("fimc_is_itf_power_down_wrap is fail(%d)", ret);
			...
			int cmd, int val)
{
	...
	if (ret)
		merr("fimc_is_itf_sys_ctl_wrap is fail(%d)", this, ret);
...
{
	return fimc_is_itf_sensor_mode_wrap(ischain);
}
	...
	struct fimc_is_frame *frame)
{
	...
	if (ret)
		merr("fimc_is_itf_shot_wrap is fail(%d)", device, ret);
...
#ifdef CONFIG_USE_VENDER_FEATURE
		ret = fimc_is_sec_run_fw_sel(device, core->current_position);
		if (ret) {
...
#if defined(ENABLE_IS_CORE) || defined(ENABLE_FD_SW)
p_err:
#endif
	info("%s(%d):%d\n", __func__, on, ret);
	return ret;
...
static int fimc_is_ischain_open(struct fimc_is_device_ischain *device)
{
	...
	imemory->dvaddr_dshared	= minfo->dvaddr_dshared;
	imemory->kvaddr_dshared	= minfo->kvaddr_dshared;
#endif
...
#else
			ret = fimc_is_ischain_loadcalb(device, module, SENSOR_POSITION_REAR);
#endif
			if (ret) {
	...
	u32 *indexes)
{
	...
	*hindex |= HIGHBIT_OF(PARAM_FD_OTF_INPUT);
	...
	fimc_is_lib_fd_size_check(device->fd_lib, fd_config, &subdev->output,
					otf_input->width, otf_input->height);
		...
		struct fimc_is_frame *ldr_frame)
{
	...
	ret = fimc_is_lib_fd_map_init(device->fd_lib,
		...
		device->imemory.kvaddr_dshared, vra_param);
	if (ret) {
	...
	ret = fimc_is_lib_fd_select_buf(lib_data, &uctl->fdUd,
			device->imemory.kvaddr_dshared, device->imemory.dvaddr_dshared);
