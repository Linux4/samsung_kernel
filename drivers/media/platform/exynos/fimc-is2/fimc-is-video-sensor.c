...
static int fimc_is_ssx_video_g_ext_ctrls(struct file *file, void *priv,
	struct v4l2_ext_controls *ctrls)
{
	...
	ret = fimc_is_sensor_g_ext_ctrls(device, ctrls);
	if (ret) {
		err("invalid ioctl(0x%08X) is requested", ctrl->id);
		ret = -EINVAL;
		goto p_err;
...
const struct v4l2_ioctl_ops fimc_is_ssx_video_ioctl_ops = {
	...
	.vidioc_g_ext_ctrls		= fimc_is_ssx_video_g_ext_ctrls,
