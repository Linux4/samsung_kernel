	...
	struct v4l2_buffer *buf)
{
	...
	if ((pipe->dst) && (pipe->dst->leader.vid == video->id)) {
		if (index >=  FIMC_IS_MAX_BUFS) {
			mverr("The leader index(%d) is bigger than array size(%d) or index is invalid",
				vctx, video, index, FIMC_IS_MAX_BUFS);
			ret = -EINVAL;
			goto p_err;
		}
		...
		if (index >=  FIMC_IS_MAX_BUFS) {
			mverr("The junction index(%d) is bigger than array size(%d) or index is invalid",
				vctx, video, index, FIMC_IS_MAX_BUFS);
			ret = -EINVAL;
			goto p_err;
		}
