...
#define CPY_SHOT_CTL_DM(type, item)		\
	do {								\
		memset(&dm->type.item, 0x00, sizeof(typeof(dm->type.item)));		\
		dm->type.item = ctl->type.item;	\
	} while (0)
...
#define CPY_SHOT_UCTL_UDM(type, item)			\
	do {										\
		memset(&udm->type.item, 0x00, sizeof(typeof(udm->type.item)));		\
		udm->type.item = uctl->type##Ud.item;	\
	} while (0)
...
static inline void copy_lens_dm(camera2_ctl_t *ctl, camera2_dm_t *dm)
{
	...
	if ((int)tmp >= 0) {
		...
		dm->lens.focusDistance = -1;
	}

	lastfocusd = ctl->lens.focusDistance;
}
