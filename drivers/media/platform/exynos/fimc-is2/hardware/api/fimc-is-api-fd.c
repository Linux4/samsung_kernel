...
int fimc_is_lib_fd_load_setfile(struct fimc_is_lib *fd_lib, u32 setfile_addr)
{
	...
	setfile_base = setfile_addr + setfile_offset;
	setfile_table = scenario_table + subip_num * scenario_num * sizeof(u32);
	setfile_index_table = (struct fd_setfile_element *)(setfile_table + \
				(ulong)(subip_num * sizeof(u32)));

	designed_bit = header_v3->designed_bit;

	for (i = 0; i < subip_num; i++) {
		for (mask = 0; mask < sizeof(designed_bit) * 8; mask++) {
			if (designed_bit & (1 << mask)) {
				if (designed_bit == SETFILE_BIT_FD)
					detect_fd_setfile = true;
				designed_bit &= ~(1 << mask);
				break;
			}
		}
		...
		for (j = 0; j < scenario_num; j++) {
			/* scenario index */
			lib_data->setfile.index[j] = *(u32 *)scenario_table;
			scenario_table += sizeof(u32);
		}

		lib_data->setfile.num = *(u32 *)setfile_table; /* set file num */
		setfile_table += sizeof(u32);

		for (j = 0; j < lib_data->setfile.num; j++) {
			if (detect_fd_setfile) {
				lib_data->setfile.table[j].addr =
					setfile_index_table->addr + setfile_base;
				lib_data->setfile.table[j].size = setfile_index_table->size;
				detect_fd_setfile = false;

				fd_setfile = (struct fd_setfile *)lib_data->setfile.table[j].addr;
				dbg_lib("FD setfile_version(%#x), flag(%#x)\n",
						fd_setfile->preview.setfile_version, fd_setfile->preview.flag);
				dbg_lib("min_face(%d), max_face(%d)\n",
				...
				dbg_lib("frame_per_lock(%d), smooth(%d), max_face_cnt(%d)\n",
						fd_setfile->preview.frame_per_lock, fd_setfile->preview.smooth,
						fd_setfile->preview.max_face_cnt);
				dbg_lib("keep_faces_over_frame(%d), boost_fd_vs_fd(%d), boost_fd_vs_speed(%d)\n",
						fd_setfile->preview.keep_faces_over_frame,
						fd_setfile->preview.boost_fd_vs_fd,
						fd_setfile->preview.boost_fd_vs_speed);
				dbg_lib("frame_per_lock_no_face(%d), keep_limit_no_face(%d)\n",
						fd_setfile->preview.frame_per_lock_no_face,
						fd_setfile->preview.keep_limit_no_face);
				dbg_lib("min_face_size_feature_detect(%d), max_face_size_feature_detect(%d)\n",
