	...
	struct fimc_is_interface_ischain *itfc)
{
	...
	hardware->func_3aa = NULL;
	hardware->func_isp = NULL;
...
exit:
	dbg_hw("START TASK [ID:%d][F:%d]\n", hw_ip->id, frame->fcount);
	...
	bool rep_flag)
{
	...
	ret = CALL_HW_OPS(hw_ip, init, group, rep_flag);
	if (ret) {
	...
	u32 instance)
{
	...
	case DEV_HW_3AA0:
#if defined(ENABLE_3AAISP)
		if (atomic_read(&hw_ip->ref_count) == 0)
			hardware->func_3aa = NULL;
#else
		hw_slot = fimc_is_hw_slot_id(DEV_HW_3AA1);
		if (valid_hw_slot_id(hw_slot)) {
			if ((atomic_read(&hw_ip->ref_count) == 0) &&
				(atomic_read(&hardware->hw_ip[hw_slot].ref_count) == 0))
				hardware->func_3aa = NULL;
		} else {
			if (atomic_read(&hw_ip->ref_count) == 0)
				hardware->func_3aa = NULL;
...
int _set_setfile_table(struct fimc_is_hardware *hardware,
	u32 hw_id, u32 addr, u32 size, int index)
{
	int ret = 0;
	int hw_slot = -1;
...
int _get_hw_id_by_settfile(u32 id, u32 num)
{
	int ret = 0;

	/* the number of sub ip*/
	switch (num) {
	case 3:
		/* ISP, DRC, FD */
		switch (id) {
	...
	case 5:
		...
		break;
	default:
		err_hw("invalid sub ip number[%d]", num);
		ret = -EINVAL;
		break;
	}
	...
	u32 addr, u32 instance, unsigned long hw_path)
{
	...
	u32 *scenario_table = NULL;
	u32 *setfile_table = NULL;
	struct fimc_is_setfile_element *setfile_index_table = NULL;
	u32 setfile_num, setfile_index, setfile_base = 0;
	int hw_id = 0;
	...
	dbg_hw("[I:%d]%s: [addr:0x%08x]\n", instance, __func__, addr);
	...
	if (header_v3->magic_number < SET_FILE_MAGIC_NUMBER) {
		...
		scenario_table = (u32 *)(addr + sizeof(struct fimc_is_setfile_header_v2));
		setfile_base = setfile_offset + addr;
		setfile_table = (u32 *)((u32)scenario_table +
				(subip_num * scenario_num * sizeof(u32)));
		setfile_index_table = (struct fimc_is_setfile_element *)((u32)setfile_table +
				(header_v2->subip_num * sizeof(u32)));
		ver = SETFILE_V2;
		dbg_hw("%s: version: %d\n", __func__, ver);
	} else {
		...
		scenario_table = (u32 *)(addr + sizeof(struct fimc_is_setfile_header_v3));
		setfile_base = setfile_offset + addr;
		setfile_table = (u32 *)((u32)scenario_table +
				(subip_num * scenario_num * sizeof(u32)));
		setfile_index_table = (struct fimc_is_setfile_element *)((u32)setfile_table +
				(subip_num * sizeof(u32)));
		ver = SETFILE_V3;
		dbg_hw("%s: version: %d\n", __func__, ver);
	...
	dbg_hw("%s: setfile base: 0x%08x\n", __func__, setfile_base);
	dbg_hw("%s: setfile_table: 0x%p\n", __func__, setfile_table);
	dbg_hw("%s: setfile_index_table: 0x%p\n", __func__, setfile_index_table);
	...
	for (i = 0; i < subip_num; i++) {
		hw_id = _get_hw_id_by_settfile(i, subip_num);
		if (hw_id < 0) {
			err_hw("invalid hw (%d)", hw_id);
			ret = -EINVAL;
			goto exit;
		...
		for (j = 0; j < setfile_num; j++) {
			...
			/* load setfile */
			_load_setfile(hardware, hw_id, j, instance, hw_path);
			setfile_index_table++;
		}
	}
