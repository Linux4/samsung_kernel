...
typedef unsigned int (*register_lib_isp)(void **func_3aa, void **func_isp);
...
struct fimc_is_hw_ip {
	...
	void __iomem				*base_addr_b;	/* for isp */
	void					*priv_info;
...
struct fimc_is_hw_ip_ops {
	...
	int (*init)(struct fimc_is_hw_ip *hw_ip, struct fimc_is_group *group, bool flag);
	int (*close)(struct fimc_is_hw_ip *hw_ip, u32 instance);
	int (*enable)(struct fimc_is_hw_ip *hw_ip, u32 instance, unsigned long hw_map);
	int (*disable)(struct fimc_is_hw_ip *hw_ip, u32 instance, unsigned long hw_map);
	int (*shot)(struct fimc_is_hw_ip *hw_ip, struct fimc_is_frame *frame, unsigned long hw_map);
	int (*set_param)(struct fimc_is_hw_ip *hw_ip,
		struct is_region *region,
		u32 lindex,
		u32 hindex,
		u32 instance,
		unsigned long hw_map);
	int (*get_meta)(struct fimc_is_hw_ip *hw_ip, struct fimc_is_frame *frame, unsigned long hw_map);
	int (*frame_ndone)(struct fimc_is_hw_ip *hw_ip, struct fimc_is_frame *frame, u32 instance, bool late_flag);
	int (*load_setfile)(struct fimc_is_hw_ip *hw_ip, int index, u32 instance, unsigned long hw_map);
	int (*apply_setfile)(struct fimc_is_hw_ip *hw_ip, int index, u32 instance, unsigned long hw_map);
	int (*delete_setfile)(struct fimc_is_hw_ip *hw_ip, u32 instance, unsigned long hw_map);
 ...
 * @func_3aa : pointer to 3aa library
 * @func_isp : pointer to isp library
 */
struct fimc_is_hardware {
	...
	struct lib_interface_func	*func_3aa;
	struct lib_interface_func	*func_isp;

	/* for dynamic DMA allocation */
	ulong				kvaddr_fd[3];
	/* keep last configuration */
	unsigned long			hw_path[FIMC_IS_MAX_NODES];
...
int fimc_is_hardware_open(struct fimc_is_hardware *hardware,
	u32 hw_id,
	struct fimc_is_group *group,
	u32 instance,
	bool rep_flag);
...
int fimc_is_hardware_load_setfile(struct fimc_is_hardware *hardware,
	u32 addr, u32 instance, unsigned long hw_path);
