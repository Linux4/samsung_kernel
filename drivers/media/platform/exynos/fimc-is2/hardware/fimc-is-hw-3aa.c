...
int fimc_is_hw_3aa_open(struct fimc_is_hw_ip *hw_ip, u32 instance, u32 *size)
{
	...
	*size = sizeof(struct fimc_is_hw_3aa);
	...
	if ((hw_ip->hardware->func_3aa == NULL) || (hw_ip->hardware->func_isp == NULL)) {
		ret = ((register_lib_isp)((char *)LIB_ISP_ADDR))((void **)&hw_ip->hardware->func_3aa, (void **)&hw_ip->hardware->func_isp);
		dbg_hw("lib_interface_func is set (%d)\n", hw_ip->id);
	}

	if (hw_ip->hardware->func_3aa == NULL) {
...
int fimc_is_hw_3aa_init(struct fimc_is_hw_ip *hw_ip, struct fimc_is_group *group, bool flag)
{
	int ret = 0;
	int i = 0;
	u32 instance = 0;
	...
	for (i = 0; i < FIMC_IS_MAX_NODES; i++) {
		...
		hw_3aa->lib[i].func = hw_ip->hardware->func_3aa;
	}
	...
	if (hw_3aa->lib[instance].object != NULL) {
		...
		ret = fimc_is_lib_isp_object_create(hw_ip, &hw_3aa->lib[instance], (u32)instance);
		if (ret) {
