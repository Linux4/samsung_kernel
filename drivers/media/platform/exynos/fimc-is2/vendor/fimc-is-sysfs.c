...
{
	struct fimc_is_device_ischain *device = &(sysfs_core->ischain[0]);
	int ret = 0;
	...
	if (((position == SENSOR_POSITION_REAR) && (!finfo->is_caldata_read))
		...
		ret = fimc_is_sec_run_fw_sel(device, position);
		if (ret) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (force_caldata_dump == false) {
		ret = fimc_is_sec_run_fw_sel(device, SENSOR_POSITION_REAR);
		if (ret) {
			err("fimc_is_sec_run_fw_sel is fail(%d)", ret);
		}
	}
