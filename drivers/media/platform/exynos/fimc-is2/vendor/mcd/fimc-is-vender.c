...
struct fimc_is_module_enum *active_sensor, int position)
{
	...
	if(cal_buf != NULL){
#if defined(CONFIG_CAMERA_EEPROM_SUPPORT_REAR) || defined(CONFIG_CAMERA_EEPROM_SUPPORT_FRONT)
		if( cal_buf[EEP_HEADER_CAL_MAP_VER_START_ADDR]==0 && 
			cal_buf[EEP_HEADER_CAL_MAP_VER_START_ADDR+1]==0 &&
			cal_buf[EEP_HEADER_CAL_MAP_VER_START_ADDR+2]==0 &&
			cal_buf[EEP_HEADER_CAL_MAP_VER_START_ADDR+3]==0){
			info("CAL DATA:MAP ver: None \n");
		}
		else{
...
#else
		if( cal_buf[OTP_HEADER_CAL_MAP_VER_START_ADDR_FRONT]==0 && 
			cal_buf[OTP_HEADER_CAL_MAP_VER_START_ADDR_FRONT+1]==0 &&
			cal_buf[OTP_HEADER_CAL_MAP_VER_START_ADDR_FRONT+2]==0 &&
			cal_buf[OTP_HEADER_CAL_MAP_VER_START_ADDR_FRONT+3]==0){
			info("CAL DATA:MAP ver: None \n");
		}
		else{
			info("CAL DATA:MAP ver: %c%c%c%c \n",
