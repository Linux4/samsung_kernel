...
int fimc_is_af_power(struct fimc_is_device_af *af_device, bool onoff)
{
	...
	/*VDDIO_1.8V_CAM*/
	ret = fimc_is_af_ldo_enable("VDD18_CAM_SENSOR_IO", onoff);
	if (ret) {
		err("failed to power control VDD18_CAM_SENSOR_IO, onoff = %d", onoff);
		return -EINVAL;
	}
...
int16_t fimc_is_af_enable(void *device, bool onoff)
{
	...
	if (!specific->running_rear_camera) {
		...
		if (onoff) {
			...
			io_regulator = fimc_is_check_regulator_status("VDD18_CAM_SENSOR_IO");
