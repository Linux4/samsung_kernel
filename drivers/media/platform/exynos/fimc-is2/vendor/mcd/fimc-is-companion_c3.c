...
#include "fimc-is-companion_address_c3.h"
...
#define FIMC_IS_COMPANION_LSC_ADDR			(0x000CE810)
#define FIMC_IS_COMPANION_LSC_SIZE			5880
#define FIMC_IS_COMPANION_LSC_PARAMETER_ADDR		(0x000CFF0C)
#define FIMC_IS_COMPANION_LSC_PARAMETER_SIZE		228
#define FIMC_IS_COMPANION_LSC_ADDR_FRONT		(0x000CD020)
#define FIMC_IS_COMPANION_LSC_SIZE_FRONT		5880
#define FIMC_IS_COMPANION_LSC_PARAMETER_ADDR_FRONT	(0x000CE71C)
#define FIMC_IS_COMPANION_LSC_PARAMETER_SIZE_FRONT	228
#define FIMC_IS_COMPANION_PDAF_SHAD_ADDR		(0x000CCBDC)
...
static int fimc_is_comp_load_i2c_cal(struct fimc_is_core *core, u32 addr, int position)
{
	...
	if ((companion_lsc_isvalid && position == SENSOR_POSITION_REAR)
		...
		for (i = 0; i < data_size; i += 2) {
			...
			if (ret) {
				err("fimc_is_comp_i2c_write() fail");
				break;
			}
		}
	}
