...
static int fimc_is_mcs_video_open(struct file *file)
{
	...
	ret = open_vctx(file, video, &vctx, device->instance, FRAMEMGR_ID_MXS);
	if (ret) {
	...
	unsigned int input)
{
	...
	ret = fimc_is_ischain_mcs_s_input(device, stream, module, vindex, intype, leader);
	if (ret) {
...
static void fimc_is_mcs_buffer_queue(struct vb2_buffer *vb)
{
	...
	ret = fimc_is_ischain_mcs_buffer_queue(device, queue, vb->v4l2_buf.index);
	if (ret) {
...
static void fimc_is_mcs_buffer_finish(struct vb2_buffer *vb)
{
	...
	ret = fimc_is_ischain_mcs_buffer_finish(device, vb->v4l2_buf.index);
	if (ret) {
