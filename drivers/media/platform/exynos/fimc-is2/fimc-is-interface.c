...
int fimc_is_hw_logdump(struct fimc_is_interface *this)
{
	...
	if (fimc_is_debug.read_vptr > debug_cnt)
		count = (DEBUG_CNT - fimc_is_debug.read_vptr) + debug_cnt;
	else
		count = debug_cnt - fimc_is_debug.read_vptr;

	if (count) {
		printk(KERN_DEBUG "start(%d %d)\n", debug_cnt, count);
		for (i = fimc_is_debug.read_vptr; count > 0; count--) {
			letter = debug[i];
			if (letter) {
		...
		fimc_is_debug.read_vptr = debug_cnt;
