...
static void fimc_is_gframe_info_to_frame(struct fimc_is_frame *frame,
	struct fimc_is_group_frame *gframe, u32 slot)
{
	BUG_ON(!gframe);
	BUG_ON(!frame);
	BUG_ON(!frame->shot_ext);
	...
	memcpy(&frame->shot_ext->node_group, &gframe->group_cfg[slot],
		sizeof(struct camera2_node_group));
}
	...
	if (test_bit(FIMC_IS_ISCHAIN_REPROCESSING, &device->state)) {
		...
		if (test_bit(FIMC_IS_GROUP_OTF_INPUT, &group->state)) {
			...
			group->sync_shots = max_t(int, MIN_OF_SYNC_SHOTS,
					framemgr->num_frames - group->async_shots);
	...
	struct fimc_is_group_frame **result)
{
	...
	if (gprev && !gnext) {
		...
		fimc_is_gframe_info_to_frame(frame, gframe, group->slot);
	} else if (!gprev && gnext) {
		/* leader */
		group->fcount++;
	...
	u32 done_state)
{
	...
	if ((device->vra.leader == &group->leader) &&
			test_bit(FIMC_IS_SUBDEV_RUN, &device->vra.state)) {
		fimc_is_lib_fd_trigger(device->fd_lib);
		fimc_is_lib_fd_update_meta(device->fd_lib,
				&frame->shot->dm.stats,
				&frame->shot->udm.fd);
	}
#endif
