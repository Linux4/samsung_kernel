...
#define FLITE_REG_LINEINT_RATIO(x)			((x) << 0)
...
#if defined(CONFIG_SOC_EXYNOS7420) || defined(CONFIG_SOC_EXYNOS3475)
...
#ifdef USE_FLITE_LINEIRQ
	/* for checking line interrupt */
...
#ifdef USE_FLITE_LINEIRQ
static void flite_hw_set_line_interrupt_source(u32 __iomem *base_reg)
{
	u32 cfg = 0;
	cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));
...
static void flite_hw_clr_line_interrupt_source(u32 __iomem *base_reg)
{
	u32 cfg = 0;
	cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));
...
static int flite_hw_check_line_interrupt_source(u32 __iomem *base_reg)
{
	u32 cfg = 0;
	cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));
...
static void flite_hw_set_line_interrupt_ratio(u32 __iomem *base_reg, u32 ratio)
{
	u32 cfg = 0;
	cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_LINEINT));

	/* set line interrupt ratio
	 * 0x0 - interrupt is not generated
	 * 0x1 ~ 0x13 - (each value * 5) %
	 * else - interrupt is not generated */
	cfg |= FLITE_REG_LINEINT_RATIO(ratio);

	writel(cfg, base_reg + TO_WORD_OFFSET(FLITE_REG_LINEINT));
}
#endif
	...
	if (test_bit(bdone, &flite->state)) {
		...
		if (frame) {
			...
			if (!flite_s_use_buffer(flite, bdone)) {
				...
				swap_head_frame(framemgr, FS_PROCESS);
			}
		} else {
#ifdef TASKLET_MSG
...
static void tasklet_flite_end1(unsigned long data)
{
	...
	if (flite_hw_get_output_dma(flite->base_reg)) {
		if (framemgr->queued_count[FS_PROCESS] == 2) {
			...
			if (frame) {
				frame_done = frame;
				trans_frame(framemgr, frame_done, FS_COMPLETE);
			} else {
	...
	struct fimc_is_framemgr *framemgr)
{
	...
	case FLITE_ID_A:
		...
		flite->irq[FLITE_ID_A] = platform_get_irq(device->pdev, 0);
		if (flite->irq[FLITE_ID_A] < 0) {
			dev_err(&device->pdev->dev, "Failed to get irq\n");
			goto p_err;
		}

		ret = request_irq(flite->irq[FLITE_ID_A],
	...
	case FLITE_ID_B:
		...
		flite->irq[FLITE_ID_B] = platform_get_irq(device->pdev, 0);
		if (flite->irq[FLITE_ID_B] < 0) {
			dev_err(&device->pdev->dev, "Failed to get irq\n");
			goto p_err;
		}

		ret = request_irq(flite->irq[FLITE_ID_B],
#ifdef ENABLE_ONE_SLOT
			fimc_is_flite_isr_1slot,
#else
			fimc_is_flite_isr_2slot,
	...
	case FLITE_ID_C:
		...
		flite->irq[FLITE_ID_C] = platform_get_irq(device->pdev, 0);
		if (flite->irq[FLITE_ID_C] < 0) {
			dev_err(&device->pdev->dev, "Failed to get irq\n");
			goto p_err;
		}

		ret = request_irq(flite->irq[FLITE_ID_C],
#ifdef ENABLE_ONE_SLOT
			fimc_is_flite_isr_1slot,
#else
			fimc_is_flite_isr_2slot,
...
int fimc_is_flite_close(struct v4l2_subdev *subdev)
{
	...
	switch(flite->instance) {
	case FLITE_ID_A:
		free_irq(flite->irq[FLITE_ID_A], flite);
		break;
	case FLITE_ID_B:
		free_irq(flite->irq[FLITE_ID_B], flite);
		break;
#ifdef IRQ_FIMC_LITE2
	case FLITE_ID_C:
		free_irq(flite->irq[FLITE_ID_C], flite);
		break;
#endif
	default:
		err("instance is invalid(%d)", flite->instance);
