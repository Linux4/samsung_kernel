	...
	u32 nonblock)
{
	...
	struct fimc_is_module_enum *module;
#ifndef ENABLE_A5
	struct fimc_is_device_sensor_peri *sensor_device;
#endif
	...
	sensor_device = (struct fimc_is_device_sensor_peri *)module->private_data;

	if (test_bit(FIMC_IS_SENSOR_ACTUATOR_AVAILABLE, &sensor_device->peri_state) &&
			(device->actuator_init)) {
		device->actuator_init = false;
		ret = v4l2_subdev_call(sensor_device->subdev_actuator, core, init, 0);
		if (ret)
