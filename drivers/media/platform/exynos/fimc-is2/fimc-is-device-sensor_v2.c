	...
	struct fimc_is_frame *ldr_frame)
{
	...
	BUG_ON(!GET_SUBDEV_FRAMEMGR(f_subdev));
	...
	unsigned int notification)
{
	...
	if (frame) {
		...
		ldr_framemgr = GET_SUBDEV_FRAMEMGR(subdev->leader);
		BUG_ON(!ldr_framemgr);
...
static int fimc_is_sensor_probe(struct platform_device *pdev)
{
	...
	fimc_is_subdev_probe(&device->ssvc0, instance, ENTRY_SSVC0, "SSXVC0", &fimc_is_subdev_ssvc0_ops);
	fimc_is_subdev_probe(&device->ssvc1, instance, ENTRY_SSVC1, "SSXVC1", &fimc_is_subdev_ssvc1_ops);
	fimc_is_subdev_probe(&device->ssvc2, instance, ENTRY_SSVC2, "SSXVC2", &fimc_is_subdev_ssvc2_ops);
	fimc_is_subdev_probe(&device->ssvc3, instance, ENTRY_SSVC3, "SSXVC3", &fimc_is_subdev_ssvc3_ops);
	...
	struct fimc_is_frame *check_frame)
{
	...
	if (!test_bit(FIMC_IS_SENSOR_OTF_OUTPUT, &sensor->state) &&
		...
		merr("[G%d] late shot (sensor:%d, line:%d, frame:%d)", ischain, group->id,
				sensor->fcount, sensor->line_fcount, frame->fcount);
		ret = -EINVAL;
		goto p_err;
	}
