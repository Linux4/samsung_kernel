...
static irqreturn_t fimc_is_csi_isr(int irq, void *data)
{
	...
	if (status & S5PCSIS_INTSRC_FRAME_END) {
		...
		/* CSI end frame time for M2M AF */
		if (device->ischain &&
				!test_bit(FIMC_IS_GROUP_OTF_INPUT, &device->ischain->group_3aa.state)) {

			if (instant_cnt == 0 && csi->valid_flag == 1) {
				do_gettimeofday(&interval_time);
				csi->valid_flag = 0;

				csi->end_sec = interval_time.tv_sec;
				csi->end_usec = interval_time.tv_usec;

				csi->valid_time = (csi->end_sec - csi->start_sec) + (csi->end_usec - csi->start_usec);
			}
		}
	...
	if (status & S5PCSIS_INTSRC_FRAME_START) {
		...
		/* CSI start frame time for M2M AF */
		if (device->ischain &&
				!test_bit(FIMC_IS_GROUP_OTF_INPUT, &device->ischain->group_3aa.state)) {
			if (instant_cnt == 0 && csi->valid_flag == 1) {
				do_gettimeofday(&interval_time);
				csi->start_sec = interval_time.tv_sec;
				csi->start_usec = interval_time.tv_usec;
			}

			/* start hrtimer for M2M AF setting */
			v4l2_subdev_notify(*csi->subdev, CSI_NOTIFY_M2M_ACTUATOR, device);
		}
	}
