...
static int sc_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_2D_SRC_BLEND_SET_FMT:
		if ((ctx->sc_dev->version != SCALER_VERSION(2, 2, 0))) {
			dev_err(ctx->sc_dev->dev,
			"%s: blending is not supported other than v2.2.0\n",
			__func__);
			return -EINVAL;
		}
	...
	case 3:
		if (frame->sc_fmt->num_planes == 1) {
			...
			if (frame->sc_fmt->is_alphablend_fmt) {
				BUG_ON((sc->version != SCALER_VERSION(2, 2, 0)));
...
static void sc_m2m_device_run(void *priv)
{
	...
	if ((sc->version == SCALER_VERSION(2, 2, 0)) && ctx->bl_op) {
		src_blend_frame = &ctx->src_blend_frame;
