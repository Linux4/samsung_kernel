...
static int jpeg_clk_get(struct jpeg_dev *jpeg)
{
	...
	jpeg->clocks[JPEG_GATE_CLK] = devm_clk_get(dev, "gate");
	if (!IS_ERR(jpeg->clocks[JPEG_GATE_CLK])) {
		clk_prepare(jpeg->clocks[JPEG_GATE_CLK]);
	} else if (PTR_ERR(jpeg->clocks[JPEG_GATE_CLK]) != -ENOENT) {
		dev_err(dev, "Failed to get 'gate' clock\n");
		return PTR_ERR(jpeg->clocks[JPEG_GATE_CLK]);
	}

	jpeg->clocks[JPEG_CHLD1_CLK] = devm_clk_get(dev, "child1");
	if (!IS_ERR(jpeg->clocks[JPEG_CHLD1_CLK])) {
		jpeg->clocks[JPEG_PARN1_CLK] = devm_clk_get(dev, "parent1");
		if (IS_ERR(jpeg->clocks[JPEG_PARN1_CLK])) {
			dev_err(dev, "Failed to get 'parent1' clock\n");
			clk_put(jpeg->clocks[JPEG_CHLD1_CLK]);
			return PTR_ERR(jpeg->clocks[JPEG_PARN1_CLK]);
		}
	} else if (PTR_ERR(jpeg->clocks[JPEG_CHLD1_CLK]) != -ENOENT) {
		dev_err(dev, "Failed to get 'child1' clock\n");
		return PTR_ERR(jpeg->clocks[JPEG_CHLD1_CLK]);
	} else {
		jpeg->clocks[JPEG_PARN1_CLK] = ERR_PTR(-ENOENT);
	}

	jpeg->clocks[JPEG_CHLD2_CLK] = devm_clk_get(dev, "child2");
	if (!IS_ERR(jpeg->clocks[JPEG_CHLD2_CLK])) {
		jpeg->clocks[JPEG_PARN2_CLK] = devm_clk_get(dev, "parent2");
		if (IS_ERR(jpeg->clocks[JPEG_PARN2_CLK])) {
			dev_err(dev, "Failed to get 'parent2' clock\n");
			clk_put(jpeg->clocks[JPEG_CHLD2_CLK]);
			return PTR_ERR(jpeg->clocks[JPEG_PARN2_CLK]);
		}
	} else if (PTR_ERR(jpeg->clocks[JPEG_CHLD2_CLK]) != -ENOENT) {
		dev_err(dev, "Failed to get 'child2' clock\n");
		return PTR_ERR(jpeg->clocks[JPEG_CHLD2_CLK]);
	} else {
		jpeg->clocks[JPEG_PARN2_CLK] = ERR_PTR(-ENOENT);
	}

	jpeg->clocks[JPEG_CHLD3_CLK] = devm_clk_get(dev, "child3");
	if (!IS_ERR(jpeg->clocks[JPEG_CHLD3_CLK])) {
		jpeg->clocks[JPEG_PARN3_CLK] = devm_clk_get(dev, "parent3");
		if (IS_ERR(jpeg->clocks[JPEG_PARN3_CLK])) {
			dev_err(dev, "Failed to get 'parent3' clock\n");
			clk_put(jpeg->clocks[JPEG_CHLD3_CLK]);
			return PTR_ERR(jpeg->clocks[JPEG_PARN3_CLK]);
		}
	} else if (PTR_ERR(jpeg->clocks[JPEG_CHLD3_CLK]) != -ENOENT) {
		dev_err(dev, "Failed to get 'child3' clock\n");
		return PTR_ERR(jpeg->clocks[JPEG_CHLD3_CLK]);
	} else {
		jpeg->clocks[JPEG_PARN3_CLK] = ERR_PTR(-ENOENT);
	}
