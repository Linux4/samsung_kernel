...
#define MAX_SECURE_DEC_INST_COUNT 2
...
static bool msm_vidc_check_for_secure_dec_overload(struct msm_vidc_core *core)
{
	u32 secure_dec_instance_count = 0;
	struct msm_vidc_inst *inst = NULL;
	bool overload = false;
	mutex_lock(&core->lock);
	list_for_each_entry(inst, &core->instances, list) {
		if (inst->session_type == MSM_VIDC_DECODER && (inst->flags & VIDC_SECURE))
			secure_dec_instance_count++;
	}
	mutex_unlock(&core->lock);
	/* Instance count includes current instance as well. */
	if (secure_dec_instance_count > MAX_SECURE_DEC_INST_COUNT)
		overload = true;
	return overload;
}
...
int msm_vdec_s_fmt(struct msm_vidc_inst *inst, struct v4l2_format *f)
{
	...
	if (f->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		// SAMSUNG LIMITS MAX SECURE-DECODER-INTANCE TO 2
		if (inst->flags & VIDC_SECURE) {
			if (msm_vidc_check_for_secure_dec_overload(inst->core)) {
				dprintk(VIDC_ERR, "secure decode overload\n");
				rc = -ENOTSUPP;
				goto err_invalid_fmt;
			}
		}
