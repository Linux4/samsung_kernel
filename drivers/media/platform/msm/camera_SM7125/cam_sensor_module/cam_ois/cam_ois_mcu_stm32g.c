...
static int oisfw_force_update= 0;//0644;
...
#if defined(CONFIG_SEC_C2Q_PROJECT) || defined(CONFIG_SEC_R8Q_PROJECT) || defined(CONFIG_SEC_A52Q_PROJECT) || defined(CONFIG_SEC_A72Q_PROJECT)
...
const uint16_t sysboot_i2c_slave_address = 0xC4;
...
FW_UPDATE_RETRY:
	...
	for (i = 0; i < (int)(sizeof(ois_dev_core)/sizeof(char)); i++) {
	   ...
	   CAM_INFO(CAM_OIS, "[OIS FW] force update(module %s, phone %s)", o_ctrl->module_ver, o_ctrl->phone_ver);
        ...
        //Below code checks whether sensor module OIS FW version is from A52, if so, then load right FW from phone
        if ((strncmp(o_ctrl->module_ver, "PSC1", OIS_MCU_VERSION_SIZE) == '\0') &&  //A52 OIS FW
	    (strncmp(o_ctrl->phone_ver, "PBC1", OIS_MCU_VERSION_SIZE) == '\0')) { //A72 OIS FW
	   CAM_INFO(CAM_OIS, "[OIS FW] force update(module %s, phone %s)", o_ctrl->module_ver, o_ctrl->phone_ver);
  	   is_mcu_nack = true;
	}
#endif
...
#endif
#if defined(CONFIG_SEC_A72Q_PROJECT)
        rc = cam_ois_i2c_write(o_ctrl, 0x0440, 0x00,
		CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE);
#else
