...
/*Initial setting for AF driver IC*/    
    rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_write(
	   &a_ctrl->i2c_client,
	   0x02,
	...
	struct msm_actuator_move_params_t *move_params)
{
    ...
    uint8_t lsb, msb;
/*Damping_params0 : for Soft landing when single AF triggered*/    
...
/*Damping_params1 : for Soft landing up to default position when camera starting */    
    ...
    if((curr_step_pos == 0)&&(is_damping_first_time1 == 0)){
        ...
        if (curr_lens_pos != target_lens_pos) {
            //if((dest_step_pos > damp_threshold_index0)||(dest_step_pos > curr_step_pos)
