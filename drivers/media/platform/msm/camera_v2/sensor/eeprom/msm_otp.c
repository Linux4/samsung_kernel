...
{0x0916, 0x0b0a}, //09 mipi_data_zero
...
static int prepare_read_write_data(struct msm_eeprom_ctrl_t *e_ctrl, struct msm_eeprom_cfg_data *cdata,
	uint8_t addr_h, uint8_t addr_l, enum msm_camera_i2c_data_type data_type, int isWrite)
{
	int rc=0;
				   ...
				   struct msm_eeprom_cfg_data *cdata)
{
	...
	rc = prepare_read_write_data(e_ctrl, cdata, addr_h, addr_l, MSM_CAMERA_I2C_BYTE_DATA, 0);
	if (rc < 0) {
	...
	pr_err("%s:  read data, rc addr = 0x%p %d %d\n", __func__, (void*)cdata->cfg.read_data.addr, cdata->cfg.read_data.num_bytes, rc);
	if (rc < 0) {
		pr_err("%s: failed to read data, rc addr = 0x%p %d %d\n", __func__, (void*)cdata->cfg.read_data.addr, cdata->cfg.read_data.num_bytes, rc);
		goto POWER_DOWN;
	}
				    ...
				    struct msm_eeprom_cfg_data *cdata)
{
	...
	if (cdata->cfg.write_data.compress) {
		...
		rc = prepare_read_write_data(e_ctrl, cdata, addr_h, addr_l, MSM_CAMERA_I2C_BYTE_DATA, 1);
		if (rc < 0) {
			pr_err("%s: failed to prepare read/write on otp\n", __func__);
			goto POWER_DOWN;
		}
