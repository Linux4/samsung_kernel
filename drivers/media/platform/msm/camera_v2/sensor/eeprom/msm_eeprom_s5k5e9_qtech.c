...
#define BYTES_FOR_PAGE_QTECH 42
#define START_ADDR_OF_PAGE_QTECH 0x0A04
#define NUM_OF_PAGE_QTECH 1
#define FIRST_PAGE_QTECH 17
...
int s5k5e9_qtech_insensor_read_otp_info(struct msm_eeprom_ctrl_t *e_ctrl, struct msm_eeprom_memory_block_t *block)
{
    ...
    for(i = 0; i < NUM_OF_PAGE_QTECH; i++){
        rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0x0A02, FIRST_PAGE_QTECH+i, 1);
        ...
        if(0 == complete)
            return -1;
        rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(&(e_ctrl->i2c_client), START_ADDR_OF_PAGE_QTECH, memptr, BYTES_FOR_PAGE_QTECH);
