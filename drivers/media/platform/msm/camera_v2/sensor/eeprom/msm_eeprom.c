				...
				struct msm_eeprom_memory_block_t *block)
{
	...
	uint8_t page;
#if defined MSM_OTP_DEBUG || !defined(CONFIG_S5K5E9YX)
    int i = 0;
#endif
	int j = 0;
#if defined CONFIG_S5K5E3YX || !defined(CONFIG_S5K5E9YX)
	uint16_t OTP_Data=0;
#endif
#ifdef CONFIG_S5K5E9YX
	int read_bytes = 0;
	...
	rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
		&(e_ctrl->i2c_client), 0x0A02, 41, data_type);
	if (rc < 0) {
		pr_err("%s:(%d) switch to page 41 failed\n", __func__, __LINE__);
		return rc;
	}

	rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_read(
		&(e_ctrl->i2c_client), S5K5E9_OTP_BANK_SELECT_REGISTER,
		&OTP_Bank, data_type);
#else
	rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_read(
	...
	for (i = 0; i < block->num_data; i += 16) {
		pr_err("%s memptr[%03X]: %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c\n", __func__,
		    i, memptr[i],memptr[i+1],memptr[i+2],memptr[i+3],memptr[i+4],memptr[i+5],memptr[i+6],memptr[i+7],
		    memptr[i+8],memptr[i+9],memptr[i+10],memptr[i+11],memptr[i+12],memptr[i+13],memptr[i+14],memptr[i+15]);
	}
#endif
