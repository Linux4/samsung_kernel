...
extern void msm_sensor_set_info(int event,void* buf);//bug515614 yinjie1.wt, add,2019/12/02,86118 project camera kernel code for bring up(add for set sensor information)
	...
	struct msm_eeprom_memory_map_array *eeprom_map_array)
{
	...
	for (j = 0; j < eeprom_map_array->msm_size_of_max_mappings; j++) {
		...
		for (i = 0; i < eeprom_map->memory_map_size; i++) {
			...
			break;
			case MSM_CAM_READ: {
			//bug515614 yinjie1.wt, add,2019/12/02,86118 project camera kernel code for bring up(for hi556 read otp)
				uint16_t m;
				uint16_t read_val = 0;
				e_ctrl->i2c_client.addr_type =
				eeprom_map->mem_settings[i].addr_type;
			...
			break;
			//-bug515614 yinjie1.wt, add,2019/12/02,86118 project camera kernel code for bring up(for hi556 read otp)
			//+bug515614 yinjie1.wt, add,2019/12/02,86118 project camera kernel code for bring up(add loop read ops)
	...
	memptr = e_ctrl->cal_data.mapdata;
	msm_sensor_set_info((e_ctrl->subdev_id+0x10),(void*)memptr);//bug515614 yinjie1.wt, add,2019/12/02,86118 project camera kernel code for bring up(add forset sensor information)
