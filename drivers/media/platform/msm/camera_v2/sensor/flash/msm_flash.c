	...
	void __user *argp)
{
	...
	case CFG_FLASH_TORCH:
		if ((flash_ctrl->flash_state == MSM_CAMERA_FLASH_OFF) ||
			(flash_ctrl->flash_state == MSM_CAMERA_FLASH_INIT)) {
			for (i = 0; i < MAX_LED_TRIGGERS; i++) {
				if(flash_data->flash_current[0] != 9999) {   //Change the value of current to -1 only if the current value has not been set to 9999 from HAL (9999 has been set from HAL to signal TORCH from QuickPanel
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	if (flash_ctrl->ext_pmic_func_tbl.ext_pmic_pre_flash_on)    //FLASH_LOW is set in case of Pre-flash, set pre-flash current in case Pre-flash is enabled
	{
		//when we have more PMIC source, flash_ctrl will have index, flash_ctrl[0, 1, 2 ...]
		DECIPHER_FLASH(flash_ctrl, ext_pmic_pre_flash_on, flash_data);
	}
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	if(flash_data->flash_current[0] == 9999) {    //Dummy current value set at HAL as 9999 to notify TORCH from QuickPanel is ON

		flash_data->flash_current[0] = -1;    //Reset the current to -1 again, since it notifies LED driver to set default current which comes from DTSI
		flash_data->flash_current[1] = -1;
		if (flash_ctrl->ext_pmic_func_tbl.ext_pmic_torch_on)    //TORCH from Quick Panel case
		{
	  ...
	  if (flash_ctrl->ext_pmic_func_tbl.ext_pmic_pre_flash_on)   //Set the same current as in Pre-flash case in MOVIE mode also
	    {
		//when we have more PMIC source, flash_ctrl will have index, flash_ctrl[0, 1, 2 ...]
		DECIPHER_FLASH(flash_ctrl, ext_pmic_pre_flash_on, flash_data);
		}
