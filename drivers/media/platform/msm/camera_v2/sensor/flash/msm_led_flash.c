...
#if defined(CONFIG_FRONT_FLASH)
	fctrl->front_led_irq_gpio = of_get_named_gpio(fctrl->pdev->dev.of_node, "qcom,front-led1-gpio", 0);
	if (fctrl->front_led_irq_gpio < 0) {
		/* It is prefered to use the gpio obtained locally in the specific driver file. */
		pr_err("Fail(Non-Fatal) : can't get front-led1-gpio.\n");
	}
#endif
