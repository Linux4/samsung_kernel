...
extern struct msm_aperture_ctrl_t *aperture_ctrl;
extern int msm_aperture_iris_init(struct msm_aperture_ctrl_t *a_ctrl);
#endif
...
extern int32_t msm_ois_set_mode(struct msm_ois_ctrl_t *a_ctrl,
							uint16_t mode)
{
#if defined(CONFIG_SAMSUNG_APERTURE)
...
#if defined(CONFIG_SAMSUNG_APERTURE)
	case OIS_MODE_FIXED:
		CDBG_I("SET :: OIS_MODE_FIXED\n");
		if(msm_ois_i2c_seq_write(a_ctrl, 0x0022, a_ctrl->Xcenter, 2) < 0)
			return -1;
		ycenter_shift =((a_ctrl->Ycenter[1] << 8) | a_ctrl->Ycenter[0]) + 1000;
		reg_val[0] = ycenter_shift & 0xFF;
		reg_val[1] = (ycenter_shift >> 8) & 0xFF;
		if(msm_ois_i2c_seq_write(a_ctrl, 0x0024, reg_val, 2) < 0) /* OIS ctrl reg set (shift+1000)*/
			return -1;
		if(msm_ois_i2c_byte_write(a_ctrl, 0x02, 0x02) < 0)
			return -1;
		break;
#endif
	default:
...
#if defined(CONFIG_SEC_KELLYLTE_PROJECT)
	ret |= msm_ois_i2c_byte_write(g_msm_ois_t, 0x0055, 0x34); /* amplitude level for measurement. */
#else
...
#if defined(CONFIG_SEC_KELLYLTE_PROJECT)
    //OIS centering mode
    msm_ois_i2c_byte_write(g_msm_ois_t, 0x0002, 0x05);
    //OIS serv On
    msm_ois_i2c_byte_write(g_msm_ois_t, 0x0000, 0x01);

	msleep(100);

    //aperture init
    msm_aperture_iris_init(aperture_ctrl);
    //OIS serv off
    msm_ois_i2c_byte_write(g_msm_ois_t, 0x0000, 0x00);
#endif
	msleep(100);
