...
{
    if (!info || (info->core_ver != 'A' && info->core_ver != 'B' && info->core_ver != 'C'))
        sprintf(buf, "NULL");
    else
...
int msm_ois_load_phone_fw(struct msm_ois_ctrl_t *a_ctrl)
{
    ...
    if (a_ctrl->fw_info.phone_ver.core_ver != 'A' &&
        ...
        a_ctrl->fw_info.phone_ver.core_ver != 'C') {

        pr_err("%s:%d OIS phone firmware core version missmatch expected [A/B/C] got[%c]\n",
...
void msm_ois_load_module_fw(struct msm_ois_ctrl_t *a_ctrl)
{
    ...
    if (a_ctrl->fw_info.module_ver.core_ver != 'A' &&
        ...
        a_ctrl->fw_info.module_ver.core_ver != 'C') {

        pr_err("%s:%d OIS module firmware core version missmatch expected [A/B/C] got[%c]\n",
                __func__, __LINE__, a_ctrl->fw_info.phone_ver.core_ver);
