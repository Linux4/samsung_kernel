	...
	uint8_t *sensor_name)
{
    ...
    CDBG("[syscamera][%s::%d][E]\n", __FUNCTION__, __LINE__);
		...
		if(companion_dev->eeprom_fw_ver[1] == '1' &&
			...
			((companion_dev->eeprom_fw_ver[3] == 'Q') || (companion_dev->eeprom_fw_ver[3] == 'U') || (companion_dev->eeprom_fw_ver[3] == 'A'))) {
			for(i = 0; i < 5; i++) {
		...
		// Get sensor name
		pr_err("[syscamera][%s::%d] sensor_name : %s", __FUNCTION__, __LINE__, sensor_name);
...
static int msm_companion_pll_init(struct companion_device *companion_dev)
{
	...
	CDBG("[syscamera][%s::%d][BIN_INFO:: voltage %s]\n", __FUNCTION__, __LINE__, isp_core);
