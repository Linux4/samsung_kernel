	...
	uint32_t *irq_status0, uint32_t *irq_status1)
{
	...
	if (vfe_dev->vfe_base == NULL || vfe_dev->vfe_open_cnt == 0) { 
		pr_err("isp_dbg: %s: VFE%d open cnt = %d, device closed(base = %pK)\n", 
			__func__, vfe_dev->pdev->id, vfe_dev->vfe_open_cnt,vfe_dev->vfe_base);
		return;
	}
	...
	if (overflow_mask) {
		...
		dual_vfe_res = vfe_dev->common_data->dual_vfe_res;
		if (vfe_dev->is_split) {
			pr_err("isp_dbg: %s: is_split %d", __func__, vfe_dev->is_split);
			for (vfe_id = 0; vfe_id < MAX_VFE; vfe_id++) {
				if (vfe_dev->common_data->dual_vfe_res->vfe_dev[vfe_id]->vfe_base == NULL) {
					 pr_err("isp_dbg: %s: vfe_dev[%d] base is NULL", __func__, vfe_id);
					 return;
				}
