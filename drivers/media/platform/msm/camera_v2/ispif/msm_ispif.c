...
static int msm_ispif_reset_hw(struct ispif_device *ispif, int release)
{
	...
	if(ispif->isp_fatal_err == true){
		pr_err("%s: skip msm_ispif_reset_hw() when isp_fatal_err is true", __func__);
		ispif->isp_fatal_err = false;
		return rc;
	}
	...
	void *data)
{
	...
	if (fatal_err == true) {
		pr_err("ispif_dbg: %s: fatal error, stop ispif immediately\n",
				__func__);
		ispif->isp_fatal_err = true;
		for (i = 0; i < ispif->vfe_info.num_vfe; i++) {
			msm_camera_io_w(0x0, 
...
static void msm_ispif_release(struct ispif_device *ispif)
{
	...
	rc = msm_ispif_reset_hw(ispif, 1);
	if (rc < 0) {
		pr_err("ispif_dbg: msm_ispif_reset_hw returns negative values: %d", rc);
		msm_ispif_clk_ahb_enable(ispif, 0);
	} else {
		pr_err("ispif_dbg: msm_ispif_reset_hw returns positive values: %d", rc);
		msm_ispif_clk_ahb_enable(ispif, 0);
	}
