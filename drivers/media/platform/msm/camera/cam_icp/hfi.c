...
#define HFI_POLL_TRY_SLEEP_MIN 2000
#define HFI_POLL_TRY_SLEEP_MAX 2500
...
void cam_hfi_disable_cpu(void __iomem *icp_base)
{
	...
	while (try < HFI_MAX_PC_POLL_TRY) {
		...
		/* Instead of QC Patch : msleep_interruptible(2);
		 * use usleep_range for more precise sleep time. 
		 * there is 10~12ms advantage than QC patch.
		 */
		usleep_range(HFI_POLL_TRY_SLEEP_MIN, HFI_POLL_TRY_SLEEP_MAX);
		try++;
	}
