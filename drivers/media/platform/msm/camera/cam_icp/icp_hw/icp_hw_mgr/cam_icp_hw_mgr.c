...
static int cam_icp_mgr_process_msg_create_handle(uint32_t *msg_ptr)
{
	...
	if (ctx_data->state == CAM_ICP_CTX_STATE_IN_USE) {
        ...
        CAM_ERR(CAM_ICP, "fw_handle = %x, id = %u", ctx_data->fw_handle, ctx_data->ctx_id);
	} else {
        CAM_ERR(CAM_ICP,
            "This ctx is no longer in use current state: %d, id = %u",
            ctx_data->state, ctx_data->ctx_id);
	...
	struct cam_icp_hw_ctx_data *ctx_data)
{
	...
	if (!rem_jiffies) {
		rc = -ETIMEDOUT;
        CAM_ERR(CAM_ICP, "FW timeout/err in abort handle command, fw_handle = %x, id = %u",
            ctx_data->fw_handle, ctx_data->ctx_id);
		cam_icp_mgr_process_dbg_buf(icp_hw_mgr.a5_dbg_lvl);
		cam_hfi_queue_dump();
	}
...
static int cam_icp_mgr_release_ctx(struct cam_icp_hw_mgr *hw_mgr, int ctx_id)
{
    ...
    CAM_ERR(CAM_ICP, "E: ctx_id = %d recovery = %d fw_handle = %x",
        ctx_id, atomic_read(&hw_mgr->recovery), hw_mgr->ctx_data[ctx_id].fw_handle);
    ...
    CAM_ERR(CAM_ICP, "fw_handle = %x fw_status = %d",
        hw_mgr->ctx_data[ctx_id].fw_handle,
        hw_mgr->fw_download);
...
static int cam_icp_mgr_hw_open(void *hw_mgr_priv, void *download_fw_args)
{
    ...
    if (rc) {
        CAM_ERR(CAM_ICP, "FW download failed with err %d\n", __func__);
		goto fw_download_failed;
        }
	...
	struct cam_icp_hw_ctx_data *ctx_data)
{
	...
	if (ctx_data->fw_handle == 0) {
        CAM_ERR(CAM_ICP, "Invalid handle created for %u", ctx_data->ctx_id);
		rc = -EINVAL;
	}
...
static int cam_icp_mgr_acquire_hw(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	rc = cam_icp_mgr_create_handle(icp_dev_acquire_info->dev_type,
		ctx_data);
	if (rc) {
        CAM_ERR(CAM_ICP, "create handle failed, id = %u fw_status = %d rc = %d",
                ctx_data->ctx_id, hw_mgr->fw_download, rc);
