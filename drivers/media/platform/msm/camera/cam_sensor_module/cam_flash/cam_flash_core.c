...
int cam_flash_off(struct cam_flash_ctrl *flash_ctrl)
{
	...
	rc = sm5705_fled_led_off(led_ctrl.index);
	...
	struct cam_flash_frame_setting *flash_data)
{
	...
	led_ctrl.index = 0;
	led_ctrl.flash_current_mA = flash_data->led_current_ma[led_ctrl.index];
	CAM_INFO(CAM_FLASH, "CAM Low Flash ON");
	rc = sm5705_fled_pre_flash_on(led_ctrl.index,led_ctrl.flash_current_mA);
	...
	struct cam_flash_frame_setting *flash_data)
{
	...
	rc = sm5705_fled_flash_on(led_ctrl.index);
	...
	struct cam_flash_frame_setting *flash_data)
{
	...
	led_ctrl.index = 0;	
   	rc = sm5705_fled_torch_on(led_ctrl.index);/* torch, on */
	...
	uint64_t req_id)
{
	...
	if (req_id == 0) {
		...
		if ((flash_data->opcode == CAMERA_SENSOR_FLASH_OP_FIREHIGH) &&
			...
			CAM_DBG(CAM_FLASH, "NOP opcode: req_id: %llu", req_id);
		}
	}
...
int cam_flash_parser(struct cam_flash_ctrl *fctrl, void *arg)
{
	...
	if (config.offset > len_of_buffer) {
		CAM_ERR(CAM_FLASH,
			"offset is out of bounds: offset: %llu len: %zu",
