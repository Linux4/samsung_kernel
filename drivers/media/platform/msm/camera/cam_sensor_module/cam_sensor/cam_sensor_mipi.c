...
#if !defined(CONFIG_SEC_GTS5L_PROJECT) && !defined(CONFIG_SEC_GTS6X_PROJECT) && !defined(CONFIG_SEC_GTS5LWIFI_PROJECT) && !defined(CONFIG_SEC_GTS6L_PROJECT) && !defined(CONFIG_SEC_GTS6LWIFI_PROJECT)
...
void cam_mipi_init_setting(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->sensordata->slave_info.sensor_id == FRONT_SENSOR_ID_IMX374) {
		...
		if (s_ctrl->sensor_mode == 0) {
			...
			CAM_ERR(CAM_SENSOR, "[dynamic_mipi] Invalid sensor mode.");
			s_ctrl->mipi_info = sensor_sak2l4sx_setfile_tx_sensor_full_mode;
		}
		cur_mipi_sensor_mode = &(s_ctrl->mipi_info[0]);
...
#elif defined(CONFIG_SEC_D2XQ_PROJECT) || defined(CONFIG_SEC_D2Q_PROJECT)
