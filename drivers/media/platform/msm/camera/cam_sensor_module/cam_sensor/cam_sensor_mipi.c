...
#include "cam_sensor_adaptive_mipi_gw1p.h"
	...
	if (rear_frs_test_mode == 0) {
		...
		if (s_ctrl->sensordata->slave_info.sensor_id == SENSOR_ID_S5KGW1P) {
			...
			if (s_ctrl->sensor_mode == 0) {
				s_ctrl->mipi_info = sensor_gw1p_setfile_A_mipi_sensor_mode;
			} else if (s_ctrl->sensor_mode == 1) {
				s_ctrl->mipi_info = sensor_gw1p_setfile_B_mipi_sensor_mode;
			} else if (s_ctrl->sensor_mode == 2) {
				s_ctrl->mipi_info = sensor_gw1p_setfile_C_mipi_sensor_mode;
			} else {
				CAM_ERR(CAM_SENSOR, "[adaptive_mipi] sensor mode %d not supported");
				s_ctrl->mipi_info = NULL;
			}
...
void cam_mipi_update_info(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (NULL == cur_mipi_sensor_mode) {
		CAM_INFO(CAM_SENSOR, "[adaptive_mipi] Unsupported index");
		return;
	}
