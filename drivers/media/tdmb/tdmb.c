...
static void tdmb_set_config_poweron(void)
{
	...
	if (dt_pdata->tdmb_ldo_en > 0) {
		rc = gpio_request(dt_pdata->tdmb_ldo_en, "gpio_ldo_en");
		if (rc < 0) {
			DPRINTK("%s: tdmb_ldo_en gpio %d request failed (%d)\n",
					__func__, dt_pdata->tdmb_ldo_en, rc);
		}
	}

	rc = gpio_request(dt_pdata->tdmb_en, "gpio_tdmb_en");
	if (rc < 0) {
		DPRINTK("%s: tdmb_en gpio %d request failed (%d)\n",
			__func__, dt_pdata->tdmb_en, rc);
	}

	if (dt_pdata->tdmb_lna_en > 0) {
		dt_pdata->tdmb_lna_en_req = gpio_request(dt_pdata->tdmb_lna_en, "gpio_tdmb_lna_en");
		if (dt_pdata->tdmb_lna_en_req < 0) {
			DPRINTK("%s: tdmb_lna_en gpio %d request failed (%d)\n",
				__func__, dt_pdata->tdmb_lna_en, dt_pdata->tdmb_lna_en_req);
		}
	}
	...
	if (dt_pdata->tdmb_use_irq) {
		...
		if (rc < 0) {
			DPRINTK("%s: tdmb_irq gpio %d request failed (%d)\n",
...
static void tdmb_set_config_poweroff(void)
{
	...
	if (dt_pdata->tdmb_lna_en > 0 && dt_pdata->tdmb_lna_en_req >= 0) {
	...
	if (dt_pdata->tdmb_ldo_en > 0) {
		gpio_free(dt_pdata->tdmb_ldo_en);
	}
}
...
static void tdmb_gpio_on(void)
{
	...
	if (dt_pdata->tdmb_ldo_en > 0) {
		gpio_direction_output(dt_pdata->tdmb_ldo_en, 1);
...
#endif
	if (dt_pdata->tdmb_ldo_en > 0)
		gpio_direction_output(dt_pdata->tdmb_ldo_en, 0);
...
static int tdmb_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	*tdmb_ts_tail = 0;
	...
	DPRINTK("head : %pK, tail : %pK, buffer : %pK, size : %x\n",
	...
	*cmd_tail = 0;
	...
	DPRINTK("cmd head : %pK, tail : %pK, buffer : %pK, size : %x\n",
			cmd_head, cmd_tail,
			cmd_buffer, cmd_size);
...
static struct tdmb_dt_platform_data *get_tdmb_dt_pdata(struct device *dev)
{
	...
	pdata->tdmb_ldo_en = of_get_named_gpio(dev->of_node, "tdmb_ldo_en", 0);
	if (!gpio_is_valid(pdata->tdmb_ldo_en)) {
		DPRINTK("Failed to get is valid tdmb_ldo_en\n");
	}
