...
static void tdmb_set_config_poweron(void)
{
	...
	int ret;
	ret = gpio_request(dt_pdata->tdmb_fm_ant_sel, "tdmb_fm_ant_sel");
	if (ret) {
		DPRINTK("Unable to request tdmb_fm_ant_sel [%d]\n", dt_pdata->tdmb_fm_ant_sel);
		return;
	}

	ret = regulator_enable(dt_pdata->reg_ldo);
	if (ret)
		DPRINTK("%s: regulator enable fail!!\n", __func__);
	else
		DPRINTK("%s: regulator enabled\n", __func__);
#endif
...
#ifdef CONFIG_TDMB_FM_ANT_SEL
	gpio_free(dt_pdata->tdmb_fm_ant_sel);

	regulator_disable(dt_pdata->reg_ldo);
#endif
#elif defined(CONFIG_TDMB_SLSI)
	struct pinctrl *pinctrl;
...
#ifdef CONFIG_TDMB_FM_ANT_SEL
	gpio_direction_output(dt_pdata->tdmb_fm_ant_sel, 1);
#endif
...
static bool tdmb_power_on(void)
{
	...
	if (tdmbdrv_func->power_on(dt_pdata->tdmb_xtal_freq) == false) {
#else
...
static struct tdmb_dt_platform_data *get_tdmb_dt_pdata(struct device *dev)
{
	...
	of_property_read_string(dev->of_node, "tdmb_fm_ant_ldo", &ldo_str);

	pdata->reg_ldo = regulator_get(NULL, ldo_str);
 	if (IS_ERR(pdata->reg_ldo)) {
 		pr_err("%s : could not get vreg\n", __func__);
		goto alloc_err;
	}
#endif
	...
	if(IS_ERR(pdata->gpio_init)) {
		DPRINTK("%s : could not get pins tdmb_gpio_init state (%li)\n",
			__func__, PTR_ERR(pdata->gpio_init));
		goto err_pinctrl_lookup_state;
	}
...
alloc_err:
	if(pdata->tdmb_en)
		gpio_free(pdata->tdmb_en);
	if(pdata->tdmb_rst)
		gpio_free(pdata->tdmb_rst);
	if(pdata->tdmb_irq)
		gpio_free(pdata->tdmb_irq);
