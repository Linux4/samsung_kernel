...
static struct tdmb_dt_platform_data *get_tdmb_dt_pdata(struct device *dev)
{
	...
	if(of_property_read_string(dev->of_node,
		...
		pdata->tdmb_vreg_supply = false;
		DPRINTK("Unable to find tdmb_vreg_supply\n");
	} else {
		pdata->tdmb_vreg_supply = true;
		DPRINTK("use tdmb_vreg_supply\n");
	}
	return pdata;
