...
#define FC8080_FIC_BUFFER_SIZE 516
#define FC8080_MSC_BUFFER_SIZE 8196

/* static u8 fic_buffer[512+4] __cacheline_aligned; */
static u8 *fic_buffer;
/* static u8 msc_buffer[8192+4] __cacheline_aligned; */
static u8 *msc_buffer;
...
void fc8080_buffer_init(void)
{
	fic_buffer = kzalloc(FC8080_FIC_BUFFER_SIZE, GFP_KERNEL);
	if (!fic_buffer)
		pr_err("%s : failed to allocate fic_buffer\n", __func__);

	msc_buffer = kzalloc(FC8080_MSC_BUFFER_SIZE, GFP_KERNEL);
	if (!msc_buffer)
		pr_err("%s : failed to allocate msc_buffer\n", __func__);
}
