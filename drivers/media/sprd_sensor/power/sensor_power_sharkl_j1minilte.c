...
static int sensor_sr544_poweron(uint32_t *fd_handle, struct sensor_power *dev0, struct sensor_power *dev1, struct sensor_power *dev2)
{
	int ret = 0;

	//printk("sensor_sr544_poweron start\n");
	...
	sensor_k_sensor_sel(fd_handle, SENSOR_DEV_0);//select main sensor(sensor sr544);
	sensor_k_set_pd_level(fd_handle, 0);//power down valid for sr544
	sensor_k_set_rst_level(fd_handle, 0);//reset valid for sr544
	...
	sensor_k_set_voltage_cammot(fd_handle, SENSOR_VDD_1800MV);
	...
	printk("sensor_sr544_poweron OK \n");

	return ret;
}

static int sensor_sr544_poweroff(uint32_t *fd_handle, struct sensor_power *dev0, struct sensor_power *dev1, struct sensor_power *dev2)
{
	int ret = 0;

	//printk("sensor_sr544_poweroff start\n");

	/*select main sensor(sensor sr544)*/
	...
	printk("sensor_sr544_poweroff OK\n");
...
int sensor_power_init(uint32_t *fd_handle, uint32_t sensor_id, struct sensor_power *dev0, struct sensor_power *dev1, struct sensor_power *dev2)
{
	...
	mdelay(1);
	sensor_k_sensor_sel(fd_handle, SENSOR_DEV_0);
	sensor_k_set_pd_level(fd_handle, 0);//reset valid for sr544
	sensor_k_sensor_sel(fd_handle, SENSOR_DEV_1);//select sub sensor(sensor sr030);
	sensor_k_set_pd_level(fd_handle, 0);//power down valid for sr030
	mdelay(1);
	sensor_k_set_voltage_cammot(fd_handle, SENSOR_VDD_CLOSED);
...
int sensor_power_on(uint32_t *fd_handle, uint32_t sensor_id, struct sensor_power *dev0, struct sensor_power *dev1, struct sensor_power *dev2)
{
	...
	if (SENSOR_DEV_0 == sensor_id) {
		ret = sensor_sr544_poweron(fd_handle, dev0, dev1, dev2);
...
int sensor_power_off(uint32_t *fd_handle, uint32_t sensor_id, struct sensor_power *dev0, struct sensor_power *dev1, struct sensor_power *dev2)
{
	...
	if (SENSOR_DEV_0 == sensor_id) {
		ret = sensor_sr544_poweroff(fd_handle, dev0, dev1, dev2);
	} else if (SENSOR_DEV_1 == sensor_id) {
		if (GC0310_I2C_SLAVE_ADDR == i2c_slave_addr) {
