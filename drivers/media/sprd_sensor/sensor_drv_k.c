...
#if defined(CONFIG_MACH_J3XLTE) || defined(CONFIG_MACH_J3XNLTE)|| defined(CONFIG_MACH_J1ACEVELTE) || defined(CONFIG_GTEXSLTE_DS)
...
{
#if defined(CONFIG_MACH_J3XLTE) || defined(CONFIG_MACH_J3XNLTE) || defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_GTEXSLTE_DS)
...
#elif defined(CONFIG_GTEXSLTE_DS)
	if(fw_version[3]=='P'){// If ISP is not Spreadtrum's one : 'P' Spreadtrum, 'Q' Qualcomm, 'L' LSI
			printk("%s : ISP = %c\n", __func__, fw_version[3]);
			if(fw_version[10]=='M')
				sprintf(cam_checkfw_factory, "OK");
			else
...
static ssize_t Rear_camera_checkfw_user(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(fw_version[3]=='P'){// If ISP is not Spreadtrum's one : 'P' Spreadtrum, 'Q' Qualcomm, 'L' LSI
			...
			if(fw_version[10]=='M')
				...
				sprintf(cam_checkfw_user, "NG_VER");
	}
	else
		sprintf(cam_checkfw_user, "NG_VER");
