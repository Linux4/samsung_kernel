...
static void __dma_glb_state_check(struct sprd_dma_chn *mchan)
{
	...
	pr_warn("AP global debug reg:pause=0x%x,raw_sts=0x%x,msk_sts=0x%x,"
			"req_sts=0x%x,en_sts=0x%x,debug sts=0x%x,arb_sel_sts=0x%x!\n",
			ap_dma_glb_reg->pause, ap_dma_glb_reg->int_raw_sts,
			ap_dma_glb_reg->int_msk_sts, ap_dma_glb_reg->req_sts,
			ap_dma_glb_reg->en_sts, ap_dma_glb_reg->debug_sts,
			ap_dma_glb_reg->arb_sel_sts);
}

/* dma debug function */
static void __dma_cfg_check_chn_register(struct dma_chan *chan)
{
	struct sprd_dma_chn *mchan = to_sprd_dma_chan(chan);
	struct sprd_dma_chn_reg *dma_reg = NULL;

	if (mchan->dma_chn_base == NULL)
		return ;
...
static void __dma_check_int(struct sprd_dma_dev *sdev, int type)
{
	...
	while(irq_status || aon_irq_status) {
		...
		/* dma config error. */
		if (dma_reg->intc & 0x100000)
			__dma_cfg_check_chn_register(&mchan->chan);
		dma_reg->intc |= 0x1f << 24;
...
/* shutdown method */
static int sprd_dma_shutdown(struct platform_device *pdev)
{
	...
	for (i = 0; i < sdev->dma_dev.chancnt; i++) {
		if (sdev->channels[i].chan_status == USED)
			sprd_dma_free_chan_resources(&sdev->channels[i].chan);
	}
	__ap_dma_softreset();
	__ap_dma_disable();
	...
	__aon_dma_reset();
	__aon_dma_disable();
	pr_notice("SPRD DMA engine driver shutdown OK!\n");
#endif
	return 0;
}
