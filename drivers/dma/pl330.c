...
static int pl330_add(struct pl330_info *pi)
{
	...
	pl330->state = INIT;
	pl330->usage_count = 0;
	pl330->idle_ip_index = exynos_get_idle_ip_index(dev_name(pi->dev));
...
static int pl330_alloc_chan_resources(struct dma_chan *chan)
{
	...
	unsigned long flags;
	struct pl330_dmac *pl330 = pdmac->pif.pl330_data;

	if (pl330->usage_count == 0)
		exynos_update_ip_idle_status(pl330->idle_ip_index, 0);
...
static void pl330_free_chan_resources(struct dma_chan *chan)
{
	struct dma_pl330_chan *pch = to_pchan(chan);
	unsigned long flags;
	struct pl330_dmac *pl330 = pch->dmac->pif.pl330_data;
	...
	if (pl330->usage_count == 0)
		exynos_update_ip_idle_status(pl330->idle_ip_index, 1);
}
