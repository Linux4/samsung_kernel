...
static int gp2a_regulator_onoff(struct device *dev, bool onoff)
{
	...
	if (IS_ERR(gp2a_vio)) {
		...
		return -ENOMEM;
	} else if (!regulator_get_voltage(gp2a_vio)) {
		ret = regulator_set_voltage(gp2a_vio, 1800000, 1800000);
	}
	...
	if (onoff) {
		...
		if (regulator_is_enabled(gp2a_vio)) {
...
static void gp2a_work_func_light(struct work_struct *work)
{
	...
	val = iio_get_time_ns();
	val = (val & 0x00000000ffffffff) | ((s64)(data->lux) << 32);
...
static const struct iio_chan_spec gp2a_light_channels[] = {
	{
		...
		.indexed = 1,
		.channel = 0,
		.processed_val = IIO_CHAN_INFO_PROCESSED,
			...
			if (val == SENSOR_DISABLE || val == SENSOR_ENABLE) {
				if (prox_data->data->prox_enable!= (u8)val) {
					...
					if (prox_data->data->prox_enable == SENSOR_ENABLE) {
						...
						gp2a_prox_raw(iio_priv_to_dev(prox_data), NULL, &val2, NULL, 0);
						val3 = iio_get_time_ns();
						val3 = (val3 & 0x00000000ffffffff) | ((s64)val2 << 32);
...
static irqreturn_t gp2a_irq_handler(int irq, void *dev_id)
{
	...
	if (irq == data->irq) {
		gp2a_prox_raw(iio_priv_to_dev(data->prox_data), NULL, &val, NULL, 0);
		val2 = iio_get_time_ns();
		val2 = (val2 & 0x00000000ffffffff) | ((s64)val << 32);
...
static int gp2a_light_probe_buffer(struct iio_dev *indio_dev)
{
	...
	pr_err("[light]%s, light_prox_buffer called =1 \n", __func__);
	...
	pr_err("[light]%s, light_prox_buffer called = 2\n", __func__);
	buffer->scan_timestamp = true;
	indio_dev->buffer = buffer;
	indio_dev->setup_ops = &gp2a_light_buffer_setup_ops;
	indio_dev->modes |= INDIO_BUFFER_TRIGGERED;
	pr_err("[light]%s, light_prox_buffer called = 3\n", __func__);
	...
	const struct i2c_device_id *id)
{
	...
	light_dev = iio_device_alloc(sizeof(struct gp2a_light_data));
	if (!light_dev) {
		pr_err("%s, iio_device_alloc error\n", __func__);
		err = -ENOMEM;
		goto iio_allocate_light_err;
	}

	prox_dev = iio_device_alloc(sizeof(struct gp2a_prox_data));
	if (!prox_dev) {
		pr_err("%s, iio_device_alloc error\n", __func__);
		err = -ENOMEM;
	...
	err = gp2a_light_probe_buffer(light_dev);
	if (err) {
		pr_err("[light]%s, prox probe buffer failed\n", __func__);
...
static void gp2a_shutdown(struct i2c_client *client)
{
	...
	gpio_free(data->gpio);
	iio_device_free(data->prox_data->indio_prox);
	iio_device_free(data->light_data->indio_light);
	kfree(data);
	gp2a_regulator_onoff(&client->dev, false);
}
