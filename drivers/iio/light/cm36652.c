...
{
	struct cm36652_data *st = iio_priv(indio_dev);
	unsigned long flags;
	spin_lock_irqsave(&st->spin_lock, flags);
	iio_trigger_poll(st->trig, cm36652_iio_get_boottime_ns());
...
static void cm36652_led_onoff(struct cm36652_data *cm36652, bool onoff)
{
	int ret = 0;

	pr_info("%s onoff : %d\n", __func__,onoff);

	cm36652->vled_2p8 = regulator_get(NULL, "VLED_2.8V");
	if (IS_ERR(cm36652->vled_2p8)) {
		pr_err("%s : regulator 2.8 is not available", __func__);
		return;
	}

	if (onoff) {
		ret = regulator_enable(cm36652->vled_2p8);
		if (ret < 0) {
		...
		regulator_disable(cm36652->vled_2p8);
	}

	regulator_put(cm36652->vled_2p8);
	return;
}
...
static void cm36652_prox_enable(struct cm36652_data *cm36652)
{
	u8 val = 1;
	int i;
	int err = 0;
...
static void cm36652_prox_disable(struct cm36652_data *cm36652)
{
	pr_info("%s\n", __func__);
	cm36652->power_state &= ~PROXIMITY_ENABLED;
	cm36652_led_onoff(cm36652,0);
		...
		const char *buf, size_t count)
{
	...
	st->light_poll_delay = ns_to_ktime(delay*NSEC_PER_MSEC);
	mutex_unlock(&st->lock);
	return count;
}
...
irqreturn_t cm36652_irq_thread_fn(int irq, void *data)
{
	...
	struct iio_dev *indio_dev = iio_priv_to_dev(cm36652);
	...
	val = gpio_get_value(cm36652->pdata->irq);
	cm36652_i2c_read_word(cm36652, REG_PS_DATA, &ps_data);
	cm36652->proximity_detection = val;

	if (cm36652->power_state & PROXIMITY_ENABLED) {
		/* 0 is close, 1 is far */
		cm36652_prox_data_rdy_trig_poll(indio_dev);
	}
...
static void cm36652_work_func_light(struct work_struct *work)
{
	...
	cm36652->color[0] = cm36652->als_red_data;
	cm36652->color[1] = cm36652->als_green_data;
	cm36652->color[2] = cm36652->als_blue_data;
	cm36652->color[3] = cm36652->white_data;

	cm36652->als_data = cm36652->als_green_data;

	cm36652_light_data_rdy_trig_poll(indio_dev);
...
static int cm36652_prox_probe_trigger(struct iio_dev *indio_dev)
{
	...
	st->trig->ops = &cm36652_prox_trigger_ops;
...
static int cm36652_light_probe_trigger(struct iio_dev *indio_dev)
{
	...
	st->trig->dev.parent = &st->i2c_client->dev;
	st->trig->ops = &cm36652_light_trigger_ops;
	iio_trigger_set_drvdata(st->trig, indio_dev);
	ret = iio_trigger_register(st->trig);
	if (ret)
		goto error_free_trig;
	...
	const struct i2c_device_id *id)
{
	...
	indio_dev_light = iio_device_alloc(sizeof(*cm36652_iio));
	if (!indio_dev_light) {
	...
	indio_dev_light->name = CM36652_LIGHT_NAME;
	indio_dev_light->dev.parent = &client->dev;
	indio_dev_light->info = &cm36652_light_info;
	indio_dev_light->channels = cm36652_light_channels;
	indio_dev_light->num_channels = ARRAY_SIZE(cm36652_light_channels);
	indio_dev_light->modes = INDIO_DIRECT_MODE;

	cm36652_iio = iio_priv(indio_dev_light);
	...
	err = cm36652_light_probe_buffer(indio_dev_light);
	if (err) {
		pr_err("%s, light probe buffer failed\n", __func__);
		goto error_free_light_dev;
	}

	err = cm36652_light_probe_trigger(indio_dev_light);
	if (err) {
	...
	indio_dev_prox = iio_device_alloc(sizeof(*cm36652_iio));
	if (!indio_dev_prox) {
	...
	indio_dev_prox->name = CM36652_PROX_NAME;
	indio_dev_prox->dev.parent = &client->dev;
	indio_dev_prox->info = &cm36652_prox_info;
	indio_dev_prox->channels = cm36652_prox_channels;
	indio_dev_prox->num_channels = ARRAY_SIZE(cm36652_prox_channels);
	indio_dev_prox->modes = INDIO_DIRECT_MODE;

	cm36652_iio = iio_priv(indio_dev_prox);
	cm36652_iio->i2c_client = client;
	...
	/* probe buffer */
	err = cm36652_prox_probe_buffer(indio_dev_prox);
	if (err) {
	...
	/* probe trigger */
	err = cm36652_prox_probe_trigger(indio_dev_prox);
	if (err) {
...
error_remove_prox_trigger:
	cm36652_prox_remove_trigger(indio_dev_prox);
error_remove_prox_buffer:
	cm36652_prox_remove_buffer(indio_dev_prox);
error_free_prox_dev:
	iio_device_free(indio_dev_prox);
err_create_light_workqueue:
	iio_device_unregister(indio_dev_light);
error_remove_light_trigger:
	cm36652_light_remove_trigger(indio_dev_light);
error_remove_light_buffer:
	cm36652_light_remove_buffer(indio_dev_light);
