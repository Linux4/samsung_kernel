...
static int check_ta_state(void)
{
	...
	if (psy == NULL) {
		...
		if (psy == NULL) {
			SENSOR_ERR("[SX9310  ] failed to get ps battery\n");
			return -EINVAL;
		}
	}
...
static int check_hallic_state(char *file_path, unsigned char hall_ic_status[])
{
	...
	if (IS_ERR(filep)) {
		iRet = PTR_ERR(filep);
		if (iRet != -ENOENT)
			SENSOR_ERR("[SX9310  ] file open fail [%s] %d\n",
				file_path, iRet);
		set_fs(old_fs);
		goto exit;
		}
	...
	if (!iRet) {
		SENSOR_ERR("[SX9310  ] failed : iRet(%d), size(%d), hall= %s\n",
...
static int sx9310_i2c_write(struct sx9310_p *data, u8 reg_addr, u8 buf)
{
	...
	if (ret < 0)
		SENSOR_ERR("[SX9310  ] i2c write error %d\n", ret);
...
static int sx9310_i2c_read(struct sx9310_p *data, u8 reg_addr, u8 *buf)
{
	...
	if (ret < 0)
		SENSOR_ERR("[SX9310  ] i2c read error %d\n", ret);
...
static void sx9310_initialize_register(struct sx9310_p *data)
{
	...
	for (idx = 0; idx < (sizeof(setup_reg) / 2); idx++) {
		...
		SENSOR_INFO("[SX9310  ] Write Reg: 0x%x Value: 0x%x\n",
			setup_reg[idx].reg, setup_reg[idx].val);

		sx9310_i2c_read(data, setup_reg[idx].reg, &val);
		SENSOR_INFO("[SX9310  ] Read Reg: 0x%x Value: 0x%x\n\n",
...
static void sx9310_initialize_chip(struct sx9310_p *data)
{
	...
	if (cnt >= 10)
		SENSOR_ERR("[SX9310  ] s/w reset fail(%d)\n", cnt);
...
static void send_event(struct sx9310_p *data, u8 state)
{
	...
	if (check_ta_state() > 1) {
		...
		SENSOR_INFO("[SX9310  ] TA cable connected\n");
	}
#endif
	if (state == ACTIVE) {
		data->state = ACTIVE;
...
#endif
		SENSOR_INFO("[SX9310  ] button touched\n");
		...
		sx9310_i2c_write(data, SX9310_CPS_CTRL8_REG, data->normal_th);
#endif
		SENSOR_INFO("[SX9310  ] button released\n");
...
static void sx9310_display_data_reg(struct sx9310_p *data)
{
	...
	for (reg = SX9310_REGUSEMSB; reg <= SX9310_REGOFFSETLSB; reg++) {
		...
		SENSOR_INFO("[SX9310  ] Register(0x%2x) data(0x%2x)\n", reg, val);
	}
}
...
static void sx9310_get_data(struct sx9310_p *data)
{
	...
	SENSOR_INFO("[SX9310  ] Capmain: %d, Useful: %d, avg: %d, diff: %d, Offset: %u\n",
...
static int sx9310_set_mode(struct sx9310_p *data, unsigned char mode)
{
	...
	SENSOR_INFO("[SX9310  ] change the mode : %u\n", mode);
...
static void sx9310_ch_interrupt_read(struct sx9310_p *data, u8 status)
{
	...
	SENSOR_INFO("[SX9310  ] ch1:%d, ch2:%d\n",
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtoul(buf, 10, &val)) {
		SENSOR_ERR("[SX9310  ] Invalid Argument\n");
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d,%d", &regist, &val) != 2) {
		SENSOR_ERR("[SX9310  ] The number of data are wrong\n");
		return -EINVAL;
	}
...
{
	struct sx9310_p *data = dev_get_drvdata(dev);
	SENSOR_INFO("[SX9310  ] sw reset start\n");
	...
	sx9310_get_data(data);
	SENSOR_INFO("[SX9310  ] sw reset end\n");
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	SENSOR_INFO("[SX9310  ] normal threshold %lu\n", val);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	SENSOR_INFO("[SX9310  ] body threshold %lu\n", val);
	data->normal_th_buf = data->normal_th = (u8)(val);
...
static void sx9310_touch_process(struct sx9310_p *data, u8 flag)
{
	...
	if (data->state == IDLE) {
		if (status & (CSX_STATUS_REG << MAIN_SENSOR))
			...
			SENSOR_INFO("[SX9310  ] already released\n");
	} else {
		if (!(status & (CSX_STATUS_REG << MAIN_SENSOR)))
			send_event(data, IDLE);
		else
			SENSOR_INFO("[SX9310  ] still touched\n");
	}
}
...
static void sx9310_irq_work_func(struct work_struct *work)
{
	...
	if (sx9310_get_nirq_state(data) == 0)
		...
		SENSOR_ERR("[SX9310  ] nirq read high %d\n",
...
static void sx9310_debug_work_func(struct work_struct *work)
{
	...
	if (ret < 0) {
		SENSOR_ERR("[SX9310  ] hallic 1 detect fail = %d\n",
			ret);
	}
#endif
	...
	if (ret < 0) {
		SENSOR_ERR("[SX9310  ] hallic 2 detect fail = %d\n",
			ret);
	}
...
#endif
		if (hall_flag) {
			SENSOR_INFO("[SX9310  ] hall IC1&2 is closed\n");
		sx9310_set_offset_calibration(data);
			hall_flag = 0;
			SENSOR_INFO("[SX9310  ] TA is removed\n");
		}
	} else
		hall_flag = 1;
...
static irqreturn_t sx9310_interrupt_thread(int irq, void *pdata)
{
	...
	if (sx9310_get_nirq_state(data) == 1) {
		SENSOR_ERR("[SX9310  ] nirq read high\n");
	} else {
...
static int sx9310_setup_pin(struct sx9310_p *data)
{
	...
	if (ret < 0) {
		SENSOR_ERR("[SX9310  ] gpio %d request failed (%d)\n",
			data->gpio_nirq, ret);
		return ret;
	}
	...
	if (ret < 0) {
		SENSOR_ERR("[SX9310  ] failed to set gpio %d(%d)\n",
			data->gpio_nirq, ret);
...
static int sx9310_parse_dt(struct sx9310_p *data, struct device *dev)
{
	...
	if (data->gpio_nirq < 0) {
		SENSOR_ERR("[SX9310  ] get gpio_nirq error\n");
		return -ENODEV;
	}
	...
	if (!ret) {
		...
		SENSOR_INFO("[SX9310  ] Normal Touch Threshold : %u\n",
			data->normal_th);
	} else {
		data->normal_th = 176;
		SENSOR_INFO("[SX9310  ] Can't get normal_th\n");
	}
#ifdef CONFIG_SENSORS_SX9310_DEFENCE_CODE_FOR_TA_NOISE
	ret = of_property_read_u32(dnode, "sx9310,cable_norm_th", &temp_val);
	if (!ret) {
		data->cable_norm_th = (u8)temp_val;
		SENSOR_INFO("[SX9310  ] Cable Normal Touch Threshold : %u\n",
			data->cable_norm_th);
	} else {
		data->cable_norm_th = 0;
		SENSOR_INFO("[SX9310  ] Can't get cable normal_th\n");
	}
#endif
	return 0;
}
				...
				unsigned long action, void *data)
{
	...
	if (grip_data->init_done == OFF) {
		SENSOR_INFO("[SX9310  ] not initialized\n");
		goto skip_noti;
	}
	...
	case ATTACHED_DEV_AFC_CHARGER_9V_MUIC:
		if (action == MUIC_NOTIFY_CMD_ATTACH) {
			...
			SENSOR_INFO("[SX9310  ] TA/USB is inserted\n");
		}
		else {
			grip_data->normal_th = grip_data->normal_th_buf;
			SENSOR_INFO("[SX9310  ] TA/USB is removed\n");
		}
...
skip_noti:
	SENSOR_INFO("[SX9310  ] dev=%d, action=%lu\n", attached_dev, action);
		...
		const struct i2c_device_id *id)
{
	...
	SENSOR_INFO("[SX9310  ] Probe Start!\n");
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		SENSOR_ERR("[SX9310  ] i2c_check_functionality error\n");
		goto exit;
	}
	...
	if (data == NULL) {
		SENSOR_ERR("[SX9310  ] kzalloc error\n");
		ret = -ENOMEM;
		goto exit_kzalloc;
	}
	...
	if (ret < 0) {
		SENSOR_ERR("[SX9310  ] of_node error\n");
		ret = -ENODEV;
		goto exit_of_node;
	}
	...
	if (ret) {
		SENSOR_ERR("[SX9310  ] could not setup pin\n");
		goto exit_setup_pin;
	}
	...
	if (ret < 0) {
		SENSOR_ERR("[SX9310  ] chip reset failed %d\n", ret);
	...
	ret = request_threaded_irq(data->irq, NULL, sx9310_interrupt_thread,
			...
			"sx9310_irq", data);
	if (ret < 0) {
		SENSOR_ERR("[SX9310  ] fail to set request irq %d(%d)"
			, data->irq, ret);
	...
	if (ret) {
		SENSOR_ERR("[SX9310  ] can not register grip_sensor(%d).\n", ret);
