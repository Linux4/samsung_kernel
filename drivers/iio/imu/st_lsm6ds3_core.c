...
static const struct iio_event_spec st_lsm6ds3_event = {
...
static int st_lsm6ds3_set_lpf(struct lsm6ds3_data *cdata, int onoff)
{
	...
	ret = cdata->tf->read(&cdata->tb, cdata->dev, ST_LSM6DS3_ACCEL_ODR_ADDR, 1, &buf);
	buf = (ST_LSM6DS3_ACCEL_ODR_MASK & (odr << 4)) | \
		((~ST_LSM6DS3_ACCEL_ODR_MASK) & buf);
	ret += cdata->tf->write(&cdata->tb, cdata->dev, ST_LSM6DS3_ACCEL_ODR_ADDR, 1, &buf);

	ret = cdata->tf->read(&cdata->tb, cdata->dev, ST_LSM6DS3_ACCEL_BW_ADDR, 1, &buf);
	buf = (ST_LSM6DS3_ACCEL_BW_MASK & bw) | \
		((~ST_LSM6DS3_ACCEL_BW_MASK) & buf);
	ret += cdata->tf->write(&cdata->tb, cdata->dev, ST_LSM6DS3_ACCEL_BW_ADDR, 1, &buf);
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = cdata->tf->read(&cdata->tb, cdata->dev, ST_LSM6DS3_OUT_TEMP_L_ADDR, 2, data);
...
//	st_lsm6ds3_vdd_on(cdata, 1);
	...
	sensors_create_symlink(&cdata->indio_dev[ST_INDIO_DEV_ACCEL]->dev.kobj,
				cdata->indio_dev[ST_INDIO_DEV_ACCEL]->name);
	sensors_create_symlink(&cdata->indio_dev[ST_INDIO_DEV_GYRO]->dev.kobj,
				cdata->indio_dev[ST_INDIO_DEV_GYRO]->name);
	sensors_create_symlink(&cdata->indio_dev[ST_INDIO_DEV_SIGN_MOTION]->dev.kobj,
				cdata->indio_dev[ST_INDIO_DEV_SIGN_MOTION]->name);
	sensors_create_symlink(&cdata->indio_dev[ST_INDIO_DEV_STEP_COUNTER]->dev.kobj,
				cdata->indio_dev[ST_INDIO_DEV_STEP_COUNTER]->name);
	sensors_create_symlink(&cdata->indio_dev[ST_INDIO_DEV_STEP_DETECTOR]->dev.kobj,
				cdata->indio_dev[ST_INDIO_DEV_STEP_DETECTOR]->name);
	sensors_create_symlink(&cdata->indio_dev[ST_INDIO_DEV_TILT]->dev.kobj,
				cdata->indio_dev[ST_INDIO_DEV_TILT]->name);
