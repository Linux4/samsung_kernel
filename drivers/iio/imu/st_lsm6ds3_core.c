...
#define ST_LSM6DS3_GYR_MIN_ST			((int)(250/0.07f))
...
#define ST_LSM6DS3_AD_FEATURES_ADDRESS		0x56
#define ST_LSM6DS3_AD_FEATURES_MASK		0x03
#define ST_LSM6DS3_FULL_AD_FEATURES		0x00
#define ST_LSM6DS3_MICRO_AD_FEATURES		0x01
#define ST_LSM6DS3_SELFTEST_AD_FEATURES		0x02
#define ST_LSM6DS3_NO_AD_FEATURES		0x03
...
#define ST_LSM6DS3_STEP_C_OUT_L_ADDR		0x4b
...
#define ST_LSM6DS3_STEP_COUNTER_THS_VALUE	0x0d
...
static struct st_lsm6ds3_odr_table {
	...
	.odr_avl[5] = { .hz = 833, .value = ST_LSM6DS3_ODR_833HZ_VAL },
	.odr_avl[6] = { .hz = 1660, .value = ST_LSM6DS3_ODR_1660HZ_VAL },
	.odr_avl[7] = { .hz = 6660, .value = ST_LSM6DS3_ODR_6660HZ_VAL },
};
...
static const struct iio_chan_spec st_lsm6ds3_step_c_ch[] = {
	{
		...
		.address = ST_LSM6DS3_STEP_C_OUT_L_ADDR,
		.scan_type = {
			.sign = 'u',
			.realbits = 16,
			.storagebits = 16,
...
static int st_acc_open_calibration(struct lsm6ds3_data *cdata)
{
	int ret = 0;
	mm_segment_t old_fs;
...
static int st_gyro_open_calibration(struct lsm6ds3_data *cdata)
{
	int ret = 0;
	mm_segment_t old_fs;
	struct file *cal_filp = NULL;
...
int st_lsm6ds3_reg_write(struct lsm6ds3_sensor_data *sdata, u8 reg_addr, int len, u8 *data)
{
	int err, i;
	...
	for (i = 0; i < len; i++) {
		err = sdata->cdata->tf->write(&sdata->cdata->tb, sdata->cdata->dev,
						reg_addr + i, 1, &data[i]);
		if (err < 0) {
			dev_err(sdata->cdata->dev, "failed to write reg (%d).\n", *data);
			return err;
		}
	}
...
int st_lsm6ds3_reg_read(struct lsm6ds3_sensor_data *sdata, u8 reg_addr, int len, u8 *data)
{
	int err, i;
	...
	for (i = 0; i < len; i++) {
		err = sdata->cdata->tf->read(&sdata->cdata->tb, sdata->cdata->dev,
						reg_addr + i, 1, &data[i]);
		if (err < 0) {
			dev_err(sdata->cdata->dev, "failed to read reg (%d).\n", reg_addr + i);
			return err;
		}
	}
...
void st_lsm6ds3_reg_dump(struct lsm6ds3_sensor_data *sdata)
{
	int err, i;
	u8 backup_regs1[5] = {0,}, backup_regs2[10] = {0,};
	u8 backup_regs3[2] = {0,}, backup_regs4[2] = {0,};

	err = st_lsm6ds3_reg_read(sdata, ST_LSM6DS3_INT1_ADDR, 2, backup_regs4);
	if (err < 0) {
		dev_err(sdata->cdata->dev, "failed to read int registers.\n");
	}
	err = st_lsm6ds3_reg_read(sdata, ST_LSM6DS3_MD1_ADDR, 2, backup_regs3);
	if (err < 0) {
		dev_err(sdata->cdata->dev, "failed to read md registers.\n");
	}
	err = st_lsm6ds3_reg_read(sdata, ST_LSM6DS3_CTRL1_ADDR, 10, backup_regs2);
	if (err < 0) {
		dev_err(sdata->cdata->dev, "failed to read ctrl registers.\n");
	}
	err = st_lsm6ds3_reg_read(sdata, ST_LSM6DS3_FIFO_CTRL1_ADDR, 5, backup_regs1);
	if (err < 0) {
		dev_err(sdata->cdata->dev, "failed to read fifo registers.\n");
	}
	for (i = 0; i < 5; i++) {
		dev_warn(sdata->cdata->dev, "FIFO [%d]: %x\n", i, backup_regs1[i]);
	}
	for (i = 0; i < 10; i++) {
		dev_warn(sdata->cdata->dev, "CTRL [%d]: %x\n", i, backup_regs2[i]);
	}
	for (i = 0; i < 2; i++) {
		dev_warn(sdata->cdata->dev, "MD [%d]: %x\n", i, backup_regs3[i]);
	}
	for (i = 0; i < 2; i++) {
		dev_warn(sdata->cdata->dev, "INT [%d]: %x\n", i, backup_regs4[i]);
	}
}
...
static int st_lsm6ds3_read_ad_feature_version(struct lsm6ds3_data *cdata)
{
	...
	switch (patch_mode) {
	case ST_LSM6DS3_FULL_AD_FEATURES:
		cdata->micro_ad_feature = true;
		cdata->selftest_ad_feature = true;
		break;
	case ST_LSM6DS3_MICRO_AD_FEATURES:
		cdata->micro_ad_feature = true;
		cdata->selftest_ad_feature = false;
		break;
	case ST_LSM6DS3_SELFTEST_AD_FEATURES:
		cdata->micro_ad_feature = false;
		cdata->selftest_ad_feature = true;
		break;
	case ST_LSM6DS3_NO_AD_FEATURES:
		cdata->micro_ad_feature = false;
		cdata->selftest_ad_feature = false;
		break;
	default:
...
static int st_lsm6ds3_set_selftest(struct lsm6ds3_sensor_data *sdata, int index)
{
	...
	case ST_INDIO_DEV_GYRO:
		...
		if (sdata->cdata->selftest_ad_feature)
...
read_zero_rate_again:
	...
	*fifo_pass = 1;
	...
	buf[3] = 0x00;
	buf[4] = 0x00;
	err = st_lsm6ds3_reg_write(sdata, ST_LSM6DS3_FIFO_CTRL5_ADDR, 1, buf);
	if (err < 0)
		return err;
...
static int st_lsm6ds3_selftest_run(struct lsm6ds3_sensor_data *sdata,	int test, char *out_str)
{
	...
	pr_info("%s, start\n", __func__);
	/* check ID */
	err = st_lsm6ds3_reg_read(sdata, ST_LSM6DS3_WAI_ADDRESS, 1, &init_status);
	if (err < 0) {
	...
	/* disable int */
	err = st_lsm6ds3_reg_write(sdata, ST_LSM6DS3_MD1_ADDR, 2, zero);
	if (err < 0) {
		dev_err(sdata->cdata->dev, "failed to write 0 into MD registers.\n");
		goto restore_regs;
	}
	err = st_lsm6ds3_reg_write(sdata, ST_LSM6DS3_INT1_ADDR, 2, zero);
	if (err < 0) {
		dev_err(sdata->cdata->dev, "failed to write 0 into INT registers.\n");
		goto restore_regs;
	}
	err = st_lsm6ds3_reg_write(sdata, ST_LSM6DS3_FIFO_CTRL1_ADDR, 5, zero);
	if (err < 0) {
	...
	zero[8] = 0x38;
	zero[9] = 0x38;
	err = st_lsm6ds3_reg_read(sdata, ST_LSM6DS3_CTRL1_ADDR, 10, zero);
	if (err < 0) {
		dev_err(sdata->cdata->dev, "failed to write 0 into CTRL registers.\n");
		goto restore_regs;
	}
...
static DEVICE_ATTR(lowpassfilter, S_IRUGO, st_lsm6ds3_lowpassfilter_show,
	st_lsm6ds3_lowpassfilter_store);
...
int st_lsm6ds3_common_probe(struct lsm6ds3_data *cdata, int irq)
{
	...
	for (i = 0; i < ST_INDIO_DEV_NUM; i++) {
		...
		if ((i == ST_INDIO_DEV_ACCEL) || (i == ST_INDIO_DEV_GYRO)) {
			...
			sdata->c_gain = st_lsm6ds3_fs_table[i].fs_avl[0].gain;
		}
...
int st_lsm6ds3_common_resume(struct lsm6ds3_data *cdata)
{
	...
	for (i = 0; i < ST_INDIO_DEV_NUM; i++) {
		...
		if ((1 << sdata->sindex) & sdata->cdata->sensors_enabled) {
			err = st_lsm6ds3_set_enable(sdata, true);
			if (err < 0)
				return err;
		}
