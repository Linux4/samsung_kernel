...
static int inv_set_flip_pickup_gesture_params_V3(struct inv_mpu_state *st)
{
	...
	INVLOG(FUNC_INT_ENTRY, "Enter\n");
        ...
        still_consistency_count_threshold   = 30;
...
static int inv_load_firmware(struct inv_mpu_state *st)
{
	...
	while(size > 0)
	{
		...
		if (result) {
			INVLOG(ERR, "error writing firmware:%d. Retrying...\n", bank);
			if (!bank)
			{
				data -= DMP_OFFSET;
			}
			...
			if (retry > 5)
			{
				INVLOG(ERR, "DMP Firmware download failed.\n");
				return result;
			}
			msleep(10);
			continue;
		}
		INVLOG(IL4, "bank %d size %d\n", bank, size);
		data += write_size;
		bank++;
		size -= write_size;
	}
...
static int inv_verify_firmware(struct inv_mpu_state *st)
{
	...
	for (bank = 0; size > 0; bank++, size -= write_size) {
		...
		if (0 != memcmp(firmware, data, write_size)) {
			INVLOG(ERR, "load data error, bank=%d\n", bank);
			return -EINVAL;
		}
		data += write_size;
	}
...
int inv_firmware_load(struct inv_mpu_state *st)
{
	...
	if (result) {
		INVLOG(ERR,"Fail to LP mode off\n");
