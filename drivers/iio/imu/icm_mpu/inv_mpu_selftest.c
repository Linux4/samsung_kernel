...
#define ACCEL_ST_AL_MAX ((DEF_ACCEL_ST_AL_MAX * DEF_ST_SCALE \
				 / DEF_ST_ACCEL_FS_MG) * DEF_ST_PRECISION)
#ifdef CONFIG_SENSORS
	pr_info("[SENSOR] reg_bias : %d, %d, %d \n", reg_avg[0], reg_avg[1], reg_avg[2]);
	pr_info("[SENSOR]  st_avg : %d, %d, %d \n", st_avg[0], st_avg[1], st_avg[2]);
#endif
	ret_val = 0;
	...
	if (!otp_value_zero) {
		...
		for (i = 0; i < 3; i++) {
			...
			if (st_shift_cust[i] < DEF_ACCEL_ST_SHIFT_DELTA_MIN
				...
				ret_val = 1;
#ifdef CONFIG_SENSORS
				pr_info("[SENSOR] error : st_shift_cust %d is smaller than %d\n", st_shift_cust[0], DEF_ACCEL_ST_SHIFT_DELTA_MIN
							* st_shift_prod[i]);
#endif
			}
			if (st_shift_cust[i] > DEF_ACCEL_ST_SHIFT_DELTA_MAX
				...
				ret_val = 1;
#ifdef CONFIG_SENSORS
				pr_info("[SENSOR] error : st_shift_cust %d is bigger than %d\n", st_shift_cust[0], DEF_ACCEL_ST_SHIFT_DELTA_MIN
							* st_shift_prod[i]);
#endif
			}
		}
	...
	for( i = 0; i < 3; i++)
	{
		st_shift_cust[i] = abs(st_avg[i] - reg_avg[i]);
#ifdef CONFIG_SENSORS
		ratio[i] = abs(st_shift_cust[i] / st_shift_prod[i] - DEF_ST_PRECISION);
#endif
	}

	if(ratio[i] > DEF_ACCEL_ST_SHIFT_DELTA_MIN)
	{
		ret_val = 1;
#ifdef CONFIG_SENSORS
		pr_info("[SENSOR] error : ratio[%d] = %d is smaller than %d\n", i,ratio[i],DEF_ACCEL_ST_SHIFT_DELTA_MIN);
#endif
	}

#ifdef CONFIG_SENSORS
	INVLOG(IL2,"self_test accel ratio - %+d %+d %+d\n",ratio[0], ratio[1], ratio[2]);
#endif
...
static int inv_setup_selftest(struct inv_mpu_state *st)
{
	...
	if (result)
	{
		INVLOG(ERR, "REG_INT_ENABLE write error.\n");
		return result;
	}
	result = inv_plat_single_write(st, REG_INT_ENABLE_1,0);
	if (result)
	{
		INVLOG(ERR, "REG_INT_ENABLE_1 write error.\n");
		return result;
	}
...
int inv_accel_self_test(struct inv_mpu_state *st, int *accel_ratio)
{
	...
	if (result)
		goto test_fail;
	pr_debug("[SENSOR] self_test accel bias_regular - %+d %+d %+d\n",
						accel_bias_regular[0],
						accel_bias_regular[1],
						accel_bias_regular[2]);
	pr_debug("[SENSOR] self_test accel bias_st - %+d %+d %+d\n",
						accel_bias_st[0],
						accel_bias_st[1],
						accel_bias_st[2]);
