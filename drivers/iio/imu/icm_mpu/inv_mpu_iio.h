...
#define INV_CHECKER_BANK
#define INV_CHECKER_LPMODE
#define INV_CHECKER_MUTEX
...
#define FACTORY_ACCEL_CAL_PATH	"/efs/FactoryApp/calibration_data"
#define MODEL_NAME	"ICM10320"
...
#define BIT_WOM_INT			0x08	// WOM HKSONG.2014.11.21
#define BIT_DMP_INT			0x02	// WOM HKSONG.2014.11.21
...
#define REG_ACCEL_INTEL_CTRL	0x12			// WOM HKSONG.2014.11.21
#define BIT_ACCEL_INTEL_EN			2			// WOM HKSONG.2014.11.21
#define BIT_ACCEL_INTEL_MODE_INT		1		// WOM HKSONG.2014.11.21

#define REG_ACCEL_WOM_THR	0x13				// WOM HKSONG.2014.11.21

#define REG_ACCEL_CONFIG        0x14
#define SHIFT_ACCEL_FS                  1
#define ACCEL_FS_SEL			0x01
#define ACCEL_DLPCFG			0x38
#define ACCEL_FCHOICE			0x01

#define REG_ACCEL_CONFIG_2       0x15
#define BIT_ACCEL_CTEN                  0x1C
#define DEC3_CFG			0x02
...
/* Pedo logging mode parameter */
#define PEDLOG_CADENCE_LEN	20
#define PEDLOG_INT_CADENCE 		0x01
#define PEDLOG_INT_STEP_COUNTER 	0x02
#define PEDLOG_INT_START_WALKING	0x04
#define PEDLOG_INT_STOP_WALKING		0x08

#define PEDLOG_STAT_STOP 0x0
#define PEDLOG_STAT_WALK 0x1
...
struct inv_pedlog {
	s32 cadence[PEDLOG_CADENCE_LEN];
...
struct inv_mpu_state {
	...
	struct inv_chip_config_s chip_config_backup;
	struct inv_chip_info_s chip_info;
	struct inv_smd smd;
	struct inv_ped ped;
	u8     wom_enable;		// WOM HKSONG.2014.11.21
/*****************************************************************************/
/*	Pedo logging mode structure                                          */
	struct inv_pedlog pedlog;
/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
...
enum MPU_IIO_ATTR_ADDR {
	...
	ATTR_DEBUG_REG_ADDR,
	ATTR_DEBUG_REG_ADDR_1,
	ATTR_DEBUG_REG_WRITE_1,
	ATTR_DEBUG_LPF_ENABLE,
	ATTR_DEBUG_SUSPEND_RESUME,
/*****************************************************************************/
/*	Pedo logging mode features					     */
	ATTR_DMP_PEDLOG_CADENCE,
	ATTR_DMP_PEDLOG_ENABLE,
	ATTR_DMP_PEDLOG_INTERRUPT_PERIOD,
	ATTR_DMP_PEDLOG_INSTANT_CADENCE,
	ATTR_DMP_PEDLOG_FLUSH_CADENCE,
	ATTR_DMP_PEDLOG_TIMER,
/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
	ATTR_WOM_THLD,
	ATTR_WOM_ENABLE,
...
#define INVLOG(a, fmt, ...) \
	{ if(a == ERR) \
		pr_err("[INVN:ERROR %s] "fmt, __func__, ##__VA_ARGS__); \
	  else if(a & INVLOG_LEVEL) \
	  	pr_info("[INVN %s:%d] "fmt, __func__, \
	  		__LINE__, ##__VA_ARGS__); \
	}
#else
#define INVLOG(a, ...) pr_info(__VA_ARGS__)
#endif
