...
static int inv_setup_compass_akm(struct inv_mpu_state *st)
{
	...
	INVLOG(IL4, "addr : 0x%x\n", addr);

	result = inv_execute_read_secondary(st, 0, addr, REG_AKM_ID, 1, data);
	if (result)
	{
		INVLOG(ERR, "Failed to read secondary.\n");
		return result;
	}

	INVLOG(IL3, "REG_AKM_ID %x\n", data[0]);
	if (data[0] != DATA_AKM_ID)
	{
		INVLOG(ERR, "data[0](%d) is not matched with DATA_AKM_ID(%d).\n",data[0], DATA_AKM_ID);
		return -ENXIO;
	}
	...
	if (result)
	{
		INVLOG(ERR, "Failed to write secondary.\n");
		return result;
	}
	...
	if (COMPASS_ID_AK09912 == st->plat_data.sec_slave_id) {
		...
		if (result)
		{
			INVLOG(ERR, "Fail to write secondary REG_AK09912_CNTL1.\n");
			return result;
		}
	...
	if (result)
	{
		INVLOG(ERR, "Failed to write REG_USER_CTRL.\n");
		return result;
	}
	...
	if (result)
	{
		INVLOG(ERR, "Failed to read REG_EXT_SLV_SENS_DATA_00.\n");
		return result;
	}

	INVLOG(IL4, "%s senx=%d, seny=%d, senz=%d\n",
		 st->hw->name,
	...
	if (result)
	{
		pr_info("%s Fail to write DATA_AKM_MODE_PD\n", __func__);
		return result;
	}
	...
	if (COMPASS_ID_AK8975 == st->plat_data.sec_slave_id) {
		...
		pr_info("%s Cannot find AKM\n", __func__);
		return -EINVAL;
	}
	result = inv_set_bank(st, BANK_SEL_3);
	if (result)
	{
		INVLOG(ERR, "Fail to set bank 3\n");
		return result;
	}
	...
	if (result)
	{
		INVLOG(ERR, "Failed to write REG_I2C_SLV2_CTRL.\n");
		return result;
	}
	...
	if (result)
	{
		INVLOG(ERR, "Failed to set bank 0.\n");
		return result;
	}
...
static int inv_akm_read_data(struct inv_mpu_state *st, short *o)
{
	...
	if (st->chip_config.dmp_on) {
		...
		result = inv_plat_read(st, REG_EXT_SLV_SENS_DATA_00,
...
static int inv_check_akm_self_test(struct inv_mpu_state *st)
{
	...
	if ((COMPASS_ID_AK09911 != st->plat_data.sec_slave_id) &&
		...
		result = inv_execute_write_secondary(st, 0, addr,
				REG_AKM_ST_CTRL, DATA_AKM_SELF_TEST);
		if (result)
			goto AKM_fail;
	}
	/* set self test mode */
	...
	while (counter > 0) {
		...
		if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id)
			result = inv_execute_read_secondary(st, 0, addr,
					REG_AK09911_STATUS1, 1, data);
		else if (COMPASS_ID_AK09912 == st->plat_data.sec_slave_id)
			result = inv_execute_read_secondary(st, 0, addr,
					REG_AK09912_STATUS1, 1, data);
		else
			result = inv_execute_read_secondary(st, 0, addr,
					REG_AKM_STATUS, 1, data);
		if (result)
			goto AKM_fail;
...
{
	INVLOG(IL4, "sec_slave_id : %d\n", (enum ext_slave_id)st->plat_data.sec_slave_id);
	INVLOG(IL4, "COMPASS_ID_AK8975: %d\n", COMPASS_ID_AK8975);
	INVLOG(IL4, "COMPASS_ID_AK8972: %d\n", COMPASS_ID_AK8972);
	INVLOG(IL4, "COMPASS_ID_AK8963: %d\n", COMPASS_ID_AK8963);
	INVLOG(IL4, "COMPASS_ID_AK09911: %d\n", COMPASS_ID_AK09911);
	INVLOG(IL4, "COMPASS_ID_AK09912: %d\n", COMPASS_ID_AK09912);
