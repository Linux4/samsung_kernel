...
int inv_mpu_power_on(struct mpu_platform_data *pdata)
{
	...
	INVLOG(IL4, "mpu_power_on 1 %d\n", err);
	err = regulator_enable(pdata->vdd_i2c);
	INVLOG(IL4, "mpu_power_on 2 %d\n", err);
...
int inv_mpu_power_off(struct mpu_platform_data *pdata)
{
	...
	INVLOG(IL4, "inv_mpu_power_off call\n");
...
int inv_parse_orientation_matrix(struct device *dev, s8 *orient)
{
	...
	if (rc) {
		INVLOG(ERR, "Unable to read axis_map_x\n");
		return rc;
	}
	rc = of_property_read_u32(np, "negate_x", &temp_val2);
	if (rc) {
		INVLOG(ERR, "Unable to read negate_x\n");
		return rc;
	}
	if (temp_val2)
		orient[temp_val] = -1;
	...
	if (rc) {
		INVLOG(ERR, "Unable to read axis_map_y\n");
		return rc;
	}
	rc = of_property_read_u32(np, "negate_y", &temp_val2);
	if (rc) {
		INVLOG(ERR, "Unable to read negate_y\n");
		return rc;
	}
	if (temp_val2)
		orient[3 + temp_val] = -1;
	...
	if (rc) {
		INVLOG(ERR, "Unable to read axis_map_z\n");
		return rc;
	}
	rc = of_property_read_u32(np, "negate_z", &temp_val2);
	if (rc) {
		INVLOG(ERR, "Unable to read negate_z\n");
		return rc;
	}
	if (temp_val2)
		orient[6 + temp_val] = -1;
							...
							s8 *orient)
{
	...
	if (rc) {
		INVLOG(ERR, "Unable to read secondary axis_map_x\n");
		return rc;
	}
	rc = of_property_read_u32(np, "inven,secondary_negate_x", &temp_val2);
	if (rc) {
		INVLOG(ERR, "Unable to read secondary negate_x\n");
		return rc;
	}
	if (temp_val2)
		orient[temp_val] = -1;
	...
	if (rc) {
		INVLOG(ERR, "Unable to read secondary axis_map_y\n");
		return rc;
	}
	rc = of_property_read_u32(np, "inven,secondary_negate_y", &temp_val2);
	if (rc) {
		INVLOG(ERR, "Unable to read secondary negate_y\n");
		return rc;
	}
	if (temp_val2)
		orient[3 + temp_val] = -1;
	...
	if (rc) {
		INVLOG(ERR, "Unable to read secondary axis_map_z\n");
		return rc;
	}
	rc = of_property_read_u32(np, "inven,secondary_negate_z", &temp_val2);
	if (rc) {
		INVLOG(ERR, "Unable to read secondary negate_z\n");
		return rc;
	}
	if (temp_val2)
		orient[6 + temp_val] = -1;
...
int inv_parse_secondary(struct device *dev, struct mpu_platform_data *pdata)
{
	...
	if (of_property_read_string(np, "inven,secondary_type", &name)) {
		INVLOG(ERR, "Missing secondary type.\n");
		return -EINVAL;
	}
	...
	if (of_property_read_string(np, "inven,secondary_name", &name)) {
		INVLOG(ERR, "Missing secondary name.\n");
		return -EINVAL;
	}

	INVLOG(IL4, "Secondary_name %s\n", name);
	...
	if (rc) {
		INVLOG(ERR, "Unable to read secondary register\n");
		return rc;
	}
	...
	INVLOG(IL4, "finished successfully.\n");
...
int inv_parse_aux(struct device *dev, struct mpu_platform_data *pdata)
{
	...
	if (rc) {
		INVLOG(ERR, "Unable to read aux register\n");
		return rc;
	}
...
int inv_parse_readonly_secondary(struct device *dev, struct mpu_platform_data *pdata)
{
	int rc;
	struct device_node *np = dev->of_node;
	u32 temp_val;
	...
	if (of_property_read_string(np, "inven,read_only_slave_type", &name)) {
		INVLOG(ERR, "Missing read only slave type type.\n");
		return -EINVAL;
	}
	...
	if (of_property_read_string(np, "inven,read_only_slave_name", &name)) {
		INVLOG(ERR, "Missing read only slave type name.\n");
		return -EINVAL;
	}
	...
	if (rc) {
		INVLOG(ERR, "Unable to read read only slave reg register\n");
		return rc;
	}
...
int invensense_mpu_parse_dt(struct device *dev, struct mpu_platform_data *pdata)
{
	...
	pr_info("%s: Invensense MPU parse_dt started 2.\n", __func__);
	...
	pr_info("%s: Invensense MPU parse_dt started 4.\n", __func__);
