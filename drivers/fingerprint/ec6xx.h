...
#ifndef _EC6XX_LINUX_DRIVER_H_
#define _EC6XX_LINUX_DRIVER_H_
...
#define CHIP_ID						"EC6XX"

/* assigned */
#define EC6XX_MAJOR					152
/* ... up to 256 */
...
#define OP_OTP_R						0x80
#define OP_OTP_W						0x82
...
#define EC6XX_INT_DETECTION_PERIOD			10
#define EC6XX_DETECTION_THRESHOLD			10
...
#define FP_CLB_READ						0X44
#define FP_CLB_WRITE					0x45
#define FP_OTP_READ						0X46
#define FP_OTP_WRITE					0x47
#define FP_OTP_OFF						0x48
...
/* Sensor Registers */
#define FDATA_EC6XX_ADDR				0x00
#define FSTATUS_EC6XX_ADDR				0x01
/* Detect Define */
...
int ec6xx_io_burst_write_register_backward(struct ec6xx_data *etspi,
		struct egis_ioc_transfer *ioc);
int ec6xx_io_read_register(struct ec6xx_data *etspi,
		u8 *addr, u8 *buf);
int ec6xx_io_write_register(struct ec6xx_data *etspi,
		u8 *buf);
int ec6xx_read_register(struct ec6xx_data *etspi,
		u8 addr, u8 *buf);
int ec6xx_write_register(struct ec6xx_data *etspi,
		u8 addr, u8 buf);
int ec6xx_io_nvm_read(struct ec6xx_data *etspi,
		struct egis_ioc_transfer *ioc);
int ec6xx_io_nvm_write(struct ec6xx_data *etspi,
		struct egis_ioc_transfer *ioc);
int ec6xx_io_nvm_off(struct ec6xx_data *etspi,
		struct egis_ioc_transfer *ioc);
int ec6xx_io_vdm_read(struct ec6xx_data *etspi,
		struct egis_ioc_transfer *ioc);
int ec6xx_io_vdm_write(struct ec6xx_data *etspi,
		struct egis_ioc_transfer *ioc);
int ec6xx_io_get_frame(struct ec6xx_data *etspi, u8 *frame, u32 size);
int ec6xx_io_otp_read(struct ec6xx_data *etspi,
		struct egis_ioc_transfer *ioc);
int ec6xx_io_otp_write(struct ec6xx_data *etspi,
		struct egis_ioc_transfer *ioc);
int ec6xx_io_otp_off(struct ec6xx_data *etspi,
		struct egis_ioc_transfer *ioc);
int ec6xx_io_clb_read(struct ec6xx_data *etspi,
		struct egis_ioc_transfer *ioc);
int ec6xx_io_clb_write(struct ec6xx_data *etspi,
		struct egis_ioc_transfer *ioc);
