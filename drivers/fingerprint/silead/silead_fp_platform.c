...
/* F52 code for finger detect LCD by wurui at 20220310 start */
...
#endif
/* F52 code for finger detect LCD by wurui at 20220310 end */
...
/* F52  code for P210325-02995 by Shibinbin at 20210329 start */
...
/* F52  code for P210325-02995 by Shibinbin at 20210329 end */
                             ...
                             unsigned long event, void *data)
{
    ...
    LOG_MSG_DEBUG(DBG_LOG, "silfp_fb_callback enter\n");
    ...
    default:
        LOG_MSG_DEBUG(INFO_LOG, "[%s] DRM BLANK(%d) do not need process\n", __func__, blank);
        break;
    }
    return retval;
}
...
static irqreturn_t silfp_irq_handler(int irq, void *dev_id)
{
	...
	//__pm_wakeup_event(fp_dev->wakelock, 10*HZ);
...
static void silfp_wakelock_ctl(struct silfp_data *fp_dev, unsigned char lock)
{
	...
	//__pm_wakeup_event(fp_dev->wakelock_hal, 10*HZ);
    } else {
        wake_unlock(&fp_dev->wakelock_hal);
	//__pm_relax(fp_dev->wakelock_hal);
    }
}
/*-------------------------------------------------------------------------*/
...
int  silfp_touch_event_handler(struct fp_dev_touch_info* tp_info)
{
	...
	//__pm_wakeup_event(g_fp_dev->wakelock, 10*HZ);
    }
    return 0;
}
...
static int silfp_probe(struct platform_device *spi)
{
    ...
    //fp_dev->wakelock=wakeup_source_register(fp_dev->dev,"silfp_wakelock");
    wake_lock_init(&fp_dev->wakelock_hal,WAKE_LOCK_SUSPEND,"silfp_wakelock_hal");
    //fp_dev->wakelock_hal=wakeup_source_register(fp_dev->dev,"silfp_wakelock_hal");
...
static int silfp_remove(struct platform_device *spi)
{
    ...
    //wakeup_source_unregister(fp_dev->wakelock);
    wake_lock_destroy(&fp_dev->wakelock_hal);
    //wakeup_source_unregister(fp_dev->wakelock_hal);
