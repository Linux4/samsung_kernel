...
#define SILEDA_WAKELOCK_HOLD_TIME 3000  /* in ms */
...
/* -------------------------------------------------------------------- */
 ...
 static void notification_work(struct work_struct *work)
{
	printk("ttt mdss_prim_panel_fb_unblank begin\n");
	//mdss_prim_panel_fb_unblank(1500);
	printk("ttt mdss_prim_panel_fb_unblank end\n");
}
...
static irqreturn_t silfp_irq_handler(int irq, void *dev_id)
{
    ...
    if (true) {
        ...
        wake_lock_timeout(&fp_dev->wakelock_hal, msecs_to_jiffies(SILEDA_WAKELOCK_HOLD_TIME));//wake_lock_timeout(&fp_dev->wakelock, 10*HZ); /* set a little long for a poor MCU */
        ...
        printk("ttt schedule notification_work\n");
        schedule_work(&fp_dev->cust_work);
    }
...
static int silfp_init(struct silfp_data *fp_dev)
{
    ...
    fp_dev->wait_finger_down = false;
    INIT_WORK(&fp_dev->cust_work, notification_work);
...
{
    if (lock) {
       wake_lock_timeout(&fp_dev->wakelock_hal, msecs_to_jiffies(SILEDA_WAKELOCK_HOLD_TIME));// wake_lock_timeout(&fp_dev->wakelock_hal, 10*HZ);
    } else {
