...
static void silfp_hw_poweron(struct silfp_data *fp_dev)
{
    ...
    /* F52 code for finger power on by wurui at 20210729 start */
    if ( fp_dev->avdd_ldo ) {
        err = regulator_set_voltage(fp_dev->avdd_ldo, AVDD_MIN, AVDD_MAX);	/*set 2.8v*/
        if (err) {
            LOG_MSG_DEBUG(ERR_LOG, "%s: Failed to regulator_set_voltage ret:%d \n", __func__, err);
        }
        err = regulator_set_load(fp_dev->avdd_ldo, 0);
        if (err) {
            LOG_MSG_DEBUG(ERR_LOG, "%s: Failed to regulator_set_load ret:%d \n", __func__, err);
        }
        err = regulator_enable(fp_dev->avdd_ldo);	/*enable regulator*/
        if (err) {
            LOG_MSG_DEBUG(ERR_LOG, "%s: Failed to regulator disable ret:%d \n", __func__, err);
        }
        //pmic_set_register_value(PMIC_RG_VCAMA_CAL,0x0A);
    }
    /* F52 code for finger power on by wurui at 20210729 end */
    if ( fp_dev->vddio_ldo ) {
...
static void silfp_hw_poweroff(struct silfp_data *fp_dev)
{
...
#ifdef BSP_SIL_POWER_SUPPLY_REGULATOR
    /* Power control by Regulators(LDO) */
    /* F52 code for finger power off by wurui at 20210729 start */
    if ( fp_dev->avdd_ldo && (regulator_is_enabled(fp_dev->avdd_ldo) > 0)) {
        err = regulator_disable(fp_dev->avdd_ldo);    /*disable regulator*/
        if (err) {
            LOG_MSG_DEBUG(ERR_LOG, "%s: Failed to regulator disable ret:%d \n", __func__, err);
        }
        err = regulator_set_voltage(fp_dev->avdd_ldo, 0, AVDD_MAX);
        if (err) {
            LOG_MSG_DEBUG(ERR_LOG, "%s: Failed to regulator_set_voltage ret:%d \n", __func__, err);
        }
        err = regulator_set_load(fp_dev->avdd_ldo, 0);
    ...
    /* F52 code for finger power off by wurui at 20210729 end */
...
static void silfp_power_deinit(struct silfp_data *fp_dev)
{
    ...
    if ( fp_dev->avdd_ldo != NULL ) {
        if( regulator_is_enabled(fp_dev->avdd_ldo) > 0 ) {
            ...
            LOG_MSG_DEBUG(INFO_LOG, "[%s] disable regulator avdd ldo.\n", __func__);
...
static int silfp_resource_init(struct silfp_data *fp_dev, struct fp_dev_init_t *dev_info)
{
    ...
    int vio18 = 0;
    int ret;
    LOG_MSG_DEBUG(ERR_LOG, "silfp_resource_init enter \n");
    ...
    //ldo33 = of_get_named_gpio(fp_dev->spi->dev.of_node, "ldoen-gpios", 0);
    vio18 = of_get_named_gpio(fp_dev->spi->dev.of_node, "vioen-gpios", 0);
    LOG_MSG_DEBUG(ERR_LOG, "[%s] int_port %d, rst_port %d.\n",__func__,fp_dev->int_port,fp_dev->rst_port);
    if (fp_dev->int_port >= 0 ) {
        gpio_free(fp_dev->int_port);
    }
    ...
    if (ldo33 > 0 ) {
	gpio_free(ldo33);
        ret = gpio_request(ldo33, "SILFP_LDO_PIN");
        if (ret < 0) {
            LOG_MSG_DEBUG(ERR_LOG, "[%s] Failed to request GPIO=%d, ret=%d",__func__,(s32)ldo33, ret);
        } else {
            gpio_direction_output(ldo33, 1);
            LOG_MSG_DEBUG(ERR_LOG, "silfp_resource_init enter  ldo33\n");
        }
    }
    */
    if (vio18 > 0 ) {
	gpio_free(vio18);
        ret = gpio_request(vio18, "SILFP_VIO_PIN");
        if (ret < 0) {
            LOG_MSG_DEBUG(ERR_LOG, "[%s] Failed to request GPIO=%d, ret=%d",__func__,(s32)vio18, ret);
        } else {
            gpio_direction_output(vio18, 1);
            LOG_MSG_DEBUG(ERR_LOG, "silfp_resource_init enter  vio18\n");
        }
    }
	...
	LOG_MSG_DEBUG(ERR_LOG, "[%s] gpio_direction_input start",__func__);
	...
	LOG_MSG_DEBUG(ERR_LOG, "[%s] gpio_direction_input end",__func__);
        if ( ret < 0 ) {
