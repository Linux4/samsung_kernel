...
static void silfp_hw_poweron(struct silfp_data *fp_dev)
{
    ...
    if ( fp_dev->avdd_ldo ) {
		...
		err = regulator_set_load(fp_dev->avdd_ldo,100000);
...
//    LOG_MSG_DEBUG(INFO_LOG, "%s: power supply ret:%d  avdd_port = %d\n", __func__, err,fp_dev->avdd_port);
}
...
static int silfp_parse_dts(struct silfp_data* fp_dev)
{
    ...
    fp_dev->avdd_ldo = regulator_get(&fp_dev->spi->dev, "silead-avdd");
...
#ifdef BSP_SIL_POWER_SUPPLY_GPIO
    fp_dev->avdd_port = of_get_named_gpio(fp_dev->spi->dev.of_node, "power-gpios", 0);
    LOG_MSG_DEBUG(ERR_LOG,"power-gpios = %d\n",fp_dev->avdd_port);
