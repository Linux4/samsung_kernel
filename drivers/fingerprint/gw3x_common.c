...
static int gw3x_probe_common(struct device *dev, struct gf_device *gf_dev)
{
	...
	// 4.19 R
	wakeup_source_init(gf_dev->wake_lock, "gw3x_wake_lock");
	// 4.19 Q
	if (!(gf_dev->wake_lock)) {
		gf_dev->wake_lock = wakeup_source_create("gw3x_wake_lock");
		if (gf_dev->wake_lock)
			wakeup_source_add(gf_dev->wake_lock);
	}
#else
	// 5.4 R
	gf_dev->wake_lock = wakeup_source_register(&gf_dev->dev, "gw3x_wake_lock");
#endif
