...
#define FP_NOTIFY                         1
#define FP_NOTIFY_TYPE                    FPSENSOR_WAKEUP_OLD
...
/* F52  code for SR-QN3979-01-243 by Shibinbin at 20210205 start */
extern int finger_sysfs;
/* F52  code for SR-QN3979-01-243 by Shibinbin at 20210205 end */
...
#endif
    if ( fpsensor->avdd_ldo && (regulator_is_enabled(fpsensor->avdd_ldo) > 0)) {
        regulator_disable(fpsensor->avdd_ldo);    /*disable regulator*/
    }
}
...
#endif
    fpsensor->avdd_ldo = regulator_get(&fpsensor->spi->dev, "avdd");
        if ( fpsensor->avdd_ldo ) {
        ret = regulator_set_voltage(fpsensor->avdd_ldo, AVDD_MIN, AVDD_MAX);	/*set 3.3v*/
        ret = regulator_enable(fpsensor->avdd_ldo);	/*enable regulator*/
    }
...
static irqreturn_t fpsensor_irq(int irq, void *handle)
{
    ...
    ** since this is interrupt context (other thread...) */
    smp_rmb();
#if FPSENSOR_WAKEUP_TYPE == FPSENSOR_WAKEUP_SOURCE
    __pm_wakeup_event(fpsensor_dev->g_ttw_wl, 1000);
#else
    wake_lock_timeout(fpsensor_dev->g_ttw_wl, msecs_to_jiffies(1000));
#endif
    setRcvIRQ(1);
...
static int fpsensor_release(struct inode *inode, struct file *filp)
{
    ...
    if ( fpsensor_dev->avdd_ldo != NULL ) {
        if ( regulator_is_enabled(fpsensor_dev->avdd_ldo) > 0 ) {
            regulator_disable(fpsensor_dev->avdd_ldo);	/*disable regulator*/
            fpsensor_debug(INFO_LOG, "%s disable regulator avdd ldo\n", __func__);
            regulator_put(fpsensor_dev->avdd_ldo);
            fpsensor_dev->avdd_ldo = NULL;
        }
    }
    /*last close??*/
    fpsensor_dev->users--;
...
static int fpsensor_probe(struct platform_device *pdev)
{
    ...
    fpsensor_dev->g_ttw_wl=wakeup_source_register(fpsensor_dev->dev,"fpsensor_ttw_wl");
#else
...
static int fpsensor_remove(struct platform_device *pdev)
{
    ...
    wakeup_source_unregister(fpsensor_dev->g_ttw_wl);
#else
