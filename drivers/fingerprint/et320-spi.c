		...
		struct poll_table_struct *wait)
{
	...
	pr_info("%s FPS fps_interrupt_poll, finger_on(%d), int_count(%d)\n", __func__, etspi->finger_on, etspi->int_count);

	if (!etspi->finger_on) poll_wait(file, &interrupt_waitq, wait);
...
static int etspi_type_check(struct etspi_data *etspi)
{
	...
	pr_info("%s sensor type is EGIS ET3xx sensor, ** skip **\n", __func__);
	return 0;
#else
	/*
