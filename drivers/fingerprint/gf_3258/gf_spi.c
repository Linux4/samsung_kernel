...
#define GF_INPUT_NAME    "uinput-goodixfp"// "qwerty" /*"goodix_fp" */
...
static irqreturn_t gf_irq(int irq, void *handle)
{
 ...
 if ((gf_dev->wait_finger_down == true) && (gf_dev->device_available == 1) && (gf_dev->fb_black == 1)) {
  ...
  printk("ttt begin shecule work unblank");
 }
...
static int gf_open(struct inode *inode, struct file *filp)
{
 ...
 if (status == 0) {
  if (status == 0) {
   ...
   pr_info("Succeed to open device. irq = %d\n",
     gf_dev->irq);
    printk("gf_ gf_open users : %d\n",gf_dev->users);
   if (gf_dev->users == 1) {
    status = gf_parse_dts(gf_dev);
    printk("gf_ gf_parse_dts :%d\n",status);
    if (status)
     goto err_parse_dt;

    status = irq_setup(gf_dev);
    printk("gf_ irq_setup :%d\n",status);
    if (status)
     goto err_irq;
   }
   msleep(50);
 ...
 printk("gf_ gf_open :%d\n",status);

 return status;
err_irq:
 gf_cleanup(gf_dev);
err_parse_dt:
 printk("gf_ err_parse_dt :%d\n",status);
 return status;
}
