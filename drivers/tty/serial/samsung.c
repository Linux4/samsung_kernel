...
#if defined(CONFIG_BT_BCM4339) || defined(CONFIG_BT_BCM4334)
struct pm_qos_request exynos5_bt_cpu_qos;
...
{
	struct s3c24xx_uart_port *ourport = container_of(work, struct s3c24xx_uart_port, qos_work.work);
	struct uart_port *port = &ourport->port;

	pm_qos_update_request_timeout(&exynos5_bt_cpu_qos, 1500000, 1000000);
...
static irqreturn_t s3c64xx_serial_handle_irq(int irq, void *id)
{
	...
	if (port->line == 0)
		...
		schedule_delayed_work(&ourport->qos_work, msecs_to_jiffies(100));
#endif
...
#if defined(CONFIG_BT_BCM4339) || defined(CONFIG_BT_BCM4334)
	if (pdev->id == 0){
		INIT_DELAYED_WORK(&ourport->qos_work, bt_qos_func);
		pm_qos_add_request(&exynos5_bt_cpu_qos, PM_QOS_CPU_FREQ_MIN, 0);
...
#if defined(CONFIG_BT_BCM4339) || defined(CONFIG_BT_BCM4334)
	if (dev->id == 0) {
		pm_qos_remove_request(&exynos5_bt_cpu_qos);
...
static struct s3c24xx_serial_drv_data exynos4210_serial_drv_data = {
	...
	defined(CONFIG_SOC_EXYNOS5410) || defined(CONFIG_SOC_EXYNOS5420) || \
	defined(CONFIG_SOC_EXYNOS3250) || defined(CONFIG_SOC_EXYNOS3472)
		.ulcon		= 0x3,
#endif
		.ucon		= S5PV210_UCON_DEFAULT,
		.ufcon		= S5PV210_UFCON_DEFAULT,
...
#elif defined(CONFIG_SOC_EXYNOS3250) || defined(CONFIG_SOC_EXYNOS3472)
