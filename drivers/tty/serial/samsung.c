...
static int s3c24xx_serial_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(dev->of_node, "samsung,weak-pull-up-mask", &ourport->ps_cfg);
	if (!ret) {
		ret = of_property_read_u32(dev->of_node, "samsung,weak-pud-val", &ourport->weak_pud_val);
		if (!ret) {
			ourport->ps_reg = syscon_regmap_lookup_by_phandle(dev->of_node,
					"samsung,ps-phandle");
			if (IS_ERR(ourport->ps_reg)) {
				dev_err(dev, "failed to get sysreg phandle\n");
			} else {
				ourport->weak_pud_en = 1;
				regmap_update_bits(ourport->ps_reg, 0x0, PS_MASK << ourport->ps_cfg,
						ourport->weak_pud_val << ourport->ps_cfg);
			}
		}
	}
