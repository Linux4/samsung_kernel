...
#define SP_RX_UART1_FIFO 16 /* For samsung uart calibration on /dev/ttyS1 */
/*UART IEN*/
...
static int serial_sprd_startup(struct uart_port *port)
{
	...
	serial_out(port, ARM_UART_CTL2, ((SP_TX_FIFO << 8) | (port->line == 1 ? SP_RX_UART1_FIFO : SP_RX_FIFO)));
	/* clear rx fifo */
	...
	if (chip_info->dma_enable) {
		...
		ctrl1 |= 0x3e00 | (port->line == 1 ? SP_RX_UART1_FIFO : SP_RX_FIFO);
	}
	serial_out(port, ARM_UART_CTL1, ctrl1);
				    ...
				    struct ktermios *old)
{
	...
	//fc |= 0x3e00 | (port->line == 1 ? SP_RX_UART1_FIFO : SP_RX_FIFO);
