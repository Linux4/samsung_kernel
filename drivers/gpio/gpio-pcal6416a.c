...
/* gpios : select gpios to set by bit */
/* value : gpio status to set according to gpios */
void pcal6416a_set_multiple_gpio(uint16_t gpios, uint16_t value)
{
	struct pcal6416a_chip *chip
		= container_of(&g_dev->gpio_chip, struct pcal6416a_chip, gpio_chip);
	int num_gpios = g_dev->pdata->ngpio;
	int offset = 0;

	pr_info("%s : selected gpios - 0x%04x\n", __func__, gpios);
	pr_info("%s : value - 0x%04x\n", __func__, value);

	mutex_lock(&chip->lock);
	while (offset < num_gpios) {
		if ( gpios & (uint16_t)(1u << offset) ) {
			pr_info("%s : offset = %d\n", __func__, offset);
			pr_info("%s : value = %d\n", __func__, 0x0001 & (value>>offset));
			if ( 0x0001 & (value>>offset))
				chip->reg_output |= (1u << offset);
			else
				chip->reg_output &= ~(1u << offset);
		}
		offset++;
...
EXPORT_SYMBOL(pcal6416a_set_multiple_gpio);
		...
		struct device_attribute *devattr, char *buf)
{
	...
	/* TEST
	pcal6416a_set_multiple_gpio(0x0110, 0x0110);
	msleep(1000);
	pcal6416a_set_multiple_gpio(0x0110, 0x0000);
*/
	return strlen(buf);
}
