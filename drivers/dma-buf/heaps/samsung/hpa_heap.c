...
static int hpa_secure_unprotect(struct buffer_prot_info *protdesc, struct device *dev)
{
	...
	if (ret) {
		pr_err("%s: UNPROT %#x (err=%#lx,dva=%#x,size=%#lx,cnt=%u,flg=%u)\n",
		       __func__, HVC_DRM_TZMP2_UNPROT, ret, protdesc->dma_addr,
		       size, protdesc->chunk_count, protdesc->flags);
		return -EACCES;
	}
	ida_free(&hpa_unique_id, unique_id);
...
static int hpa_heap_vmap(struct dma_buf *dmabuf, struct dma_buf_map *map)
{
