...
static void ufs_get_health_info(struct ufs_hba *hba)
{
...
static DEVICE_ATTR(un, 0440, ufshcd_unique_number_show, NULL);
...
static DEVICE_ATTR(man_id, 0444, ufshcd_manufacturer_id_show, NULL);

static ssize_t ufshcd_lt_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	int curr_len;
	...
	curr_len = snprintf(buf, PAGE_SIZE, "%01x\n", lt);

	return curr_len;
}
static DEVICE_ATTR(lt, 0444, ufshcd_lt_show, NULL);

static ssize_t ufshcd_lc_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	int curr_len;

	curr_len = sprintf(buf, "%u\n", hba->lc_info);

	return curr_len;
}

static ssize_t ufshcd_lc_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	unsigned int value;
...
static DEVICE_ATTR(lc, 0664, ufshcd_lc_show, ufshcd_lc_store);
...
static inline int ufshcd_create_vendor_sysfs_nodes(struct ufs_hba *hba)
{
	int err  = -ENOMEM;
...
static inline void ufshcd_add_sysfs_nodes(struct ufs_hba *hba)
{
	...
	ufshcd_create_vendor_sysfs_nodes(hba);
}
