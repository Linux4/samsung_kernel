...
#define DL_DBG_RX_INFO_CONTROL_DIRECT 0x4818
...
static struct exynos_ufs_sfr_log ufs_log_sfr[] = {
	...
	{"SMU_RD_ABORT_MATCH_INFO"	,	HCI_SMU_RD_ABORT_MATCH_INFO,	0},
	{"SMU_WR_ABORT_MATCH_INFO"	,	HCI_SMU_WR_ABORT_MATCH_INFO,	0},
	{"DBR DUPLICATION INFO"		,	HCI_DBR_DUPLICATION_INFO,	0},
	{"DBR TIMER CONFIG"		,	HCI_DBR_TIMER_CONFIG,		0},
	{"DBR TIMER ENABLE"		,	HCI_UTRL_DBR_TIMER_ENABLE,		0},
	{"DBR TIMER STATUS"		,	HCI_UTRL_DBR_TIMER_STATUS,		0},
...
static struct exynos_ufs_sfr_log ufs_show_sfr[] = {
	...
	{"UFS ACG DISABLE"	 	,	HCI_UFS_ACG_DISABLE,		0},
	{"MPHY REFCLK SEL"		,	HCI_MPHY_REFCLK_SEL,		0},
	{"SMU RD_ABORT MATCH INFO"	,	HCI_SMU_RD_ABORT_MATCH_INFO,	0},
	{"SMU WR_ABORT MATCH INFO"	,	HCI_SMU_WR_ABORT_MATCH_INFO,	0},
	{"DBR DUPLICATION INFO"		,	HCI_DBR_DUPLICATION_INFO,	0},
	{"DBR TIMER CONFIG"		,	HCI_DBR_TIMER_CONFIG,		0},
	{"DBR TIMER ENABLE"		,	HCI_UTRL_DBR_TIMER_ENABLE,	0},
	{"DBR TIMER STATUS"		,	HCI_UTRL_DBR_TIMER_STATUS,	0},
 ...
 */
#ifdef CONFIG_DEBUG_FS
static int ufs_debug_show(struct seq_file *s, void *unused)
{
...
{
	return single_open(file, ufs_debug_show, inode->i_private);
}

static ssize_t ufs_debug_write(struct file *file,
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	if (value & UFSHCD_DEBUG_LEVEL1){
		...
		writel(0x1F<<24 | 0x14<<18, ufs->reg_unipro + DL_DBG_RX_INFO_CONTROL_DIRECT);
...
static const struct file_operations ufs_debug_fops = {
	.open			= ufs_debug_open,
	.write			= ufs_debug_write,
	.read			= seq_read,
	.llseek			= seq_lseek,
	.release		= single_release,
};

static void ufs_debugfs_init(struct exynos_ufs *ufs)
{
	struct dentry *file;
	...
	file = debugfs_create_file("monitor", S_IRUGO | S_IWUSR,
			ufs->debug.root, ufs, &ufs_debug_fops);
	if (!file) {
...
static void ufs_debugfs_exit(struct exynos_ufs *ufs)
{
	if (ufs->debug.root) {
		debugfs_remove_recursive(ufs->debug.root);
		ufs->debug.root = NULL;
...
void exynos_ufs_exit_dbg(struct ufs_hba *hba)
{
	struct exynos_ufs *ufs = to_exynos_ufs(hba);

	ufs_debugfs_exit(ufs);
}
