...
SPRD_CPU_SHOW(chip_id);

static struct device_attribute sprd_cpu_attr[] = {
	SPRD_CPU_ATTR(cur_ctrl_param),
	SPRD_CPU_ATTR_RO(chip_id),
};
...
static int sprd_cpu_creat_attr(struct device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(sprd_cpu_attr); i++) {
		rc = device_create_file(dev, &sprd_cpu_attr[i]);
		if (rc)
...
sprd_attrs_failed:
	while (i--)
		device_remove_file(dev, &sprd_cpu_attr[i]);
...
static ssize_t sprd_cpu_show_chip_id(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	 ...
	 SPRDCPU_DEBUG("%s: 0x%x\n", __func__, chip_id);
	 len = sprintf(buf, "%x\n", chip_id);
	 SPRDCPU_DEBUG("len=%d, buf=%s\n", len, buf);
...
#define BUF_LEN	64
static int sprd_cpu_core_limit(int cluster, int max_core);
					...
					const char *buf, size_t count)
{
	...
	sprd_cpu_core_limit(info->cluster, info->param.max_core);
#if defined(CONFIG_ARCH_SCX35L) && !defined(CONFIG_ARCH_SCX35LT8)
	if (info->binning == 0 && info->param.low_freq > 0 && info->param.low_vol > 0){
			 ...
			 unsigned long state)
{
	...
	sprd_cpu_core_limit(c_info->cluster, max_core);
...
static int sprd_cpu_cooling_probe(struct platform_device *pdev)
{
	...
	sprd_cpu_creat_attr(&info->cdev->device);
...
static int sprd_cpu_cooling_remove(struct platform_device *pdev)
{
	...
	sprd_cpu_remove_attr(&info->cdev->device);
