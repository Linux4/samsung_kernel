...
thermal_add_hwmon_sysfs(struct thermal_zone_device *tz)
{
 ...
 register_sys_interface:
	...
	snprintf(temp->temp_input.name, PAGE_SIZE,
	...
	if (tz->ops->get_crit_temp) {
		...
		if (!tz->ops->get_crit_temp(tz, &temperature)) {
			snprintf(temp->temp_crit.name, PAGE_SIZE,
				     ...
				     struct thermal_cooling_device *cdev)
{
	...
	snprintf(dev->name, THERMAL_NAME_LENGTH, "cdev%d", dev->id);
	result =
				...
				const struct thermal_cooling_device_ops *ops)
{
	...
	strncpy(cdev->type, type ? type : "", THERMAL_NAME_LENGTH-1);
	cdev->ops = ops;
...
static int create_trip_attrs(struct thermal_zone_device *tz, int mask)
{
	...
	for (indx = 0; indx < tz->trips; indx++) {
		...
		error = device_create_file(&tz->device,
				   &tz->trip_type_attrs[indx].attr);
		if (error)
			pr_info("failed to create type attrs.\n");
		...
		if (error)
			pr_info("failed to create temp attrs.\n");
		...
		if (error)
			pr_info("failed to create freq attrs.\n");
	}
	return 0;
}
	...
	int tc1, int tc2, int passive_delay, int polling_delay)
{
	...
	strncpy(tz->type, type ? type : "", THERMAL_NAME_LENGTH-1);
	tz->ops = ops;
