				...
				unsigned long temp)
{
	...
	for (i = 0; i < EXYNOS_TMU_COUNT; i++) {
		...
		writel(EXYNOS4_TMU_CLEAR_RISE_INT|EXYNOS4_TMU_CLEAR_FALL_INT,
				data->base[i] + EXYNOS_TMU_REG_INTCLEAR);

		/* Change the trigger levels  */
		rising_threshold = readl(data->base[i] + EXYNOS_THD_TEMP_RISE);
		falling_threshold = readl(data->base[i] + EXYNOS_THD_TEMP_FALL);
		...
		writel(rising_threshold, data->base[i] + EXYNOS_THD_TEMP_RISE);
		writel(falling_threshold, data->base[i] + EXYNOS_THD_TEMP_FALL);
...
static int exynos_get_throttling_count(struct thermal_zone_device *thermal,
		enum thermal_trip_type type)
{
	struct exynos_tmu_platform_data *pdata = NULL;
	struct exynos_tmu_data *data;
...
static int exynos_tmu_read(struct exynos_tmu_data *data)
{
	...
	exynos_check_gpu_noti_state(max);
...
out:
	...
	if (EXYNOS_TMU_COUNT > 3)
		pr_debug("TEMP[0] = %d, TEMP[1] = %d, TEMP[2] = %d, TEMP[3] = %d, TEMP[4] = %d      CPU = %d, GPU = %d\n",
			alltemp[0], alltemp[1], alltemp[2], alltemp[3], alltemp[4], max, gpu_temp);

	sec_debug_aux_log(SEC_DEBUG_AUXLOG_THERMAL_CHANGE,
		"[TMU] %d", max);
...
static void exynos_check_tmu_noti_state(int temp)
{
	...
	if (temp > HOT_CRITICAL_TEMP)
		...
		cur_state = TMU_HOT;
	else if (temp > HOT_85 && temp <= HOT_NORMAL_TEMP)
		cur_state = TMU_95;
	else if (temp > COLD_TEMP && temp <= HOT_85)
...
/* sysfs interface : /sys/devices/platform/exynos4-tmu/temp */
static ssize_t
...
#ifdef CONFIG_SEC_PM
/* sysfs interface : /sys/devices/platform/exynos4-tmu/curr_temp */
static ssize_t
                ...
                struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < EXYNOS_TMU_COUNT-1; i++){
		len += snprintf(&buf[len], PAGE_SIZE, "%ld,", temp[i]);
        }
        len += snprintf(&buf[len], PAGE_SIZE, "%ld", temp[i]);
	return len;
}
...
#ifndef CONFIG_ARM_EXYNOS_CPUFREQ
	ret = exynos_register_thermal(&exynos_sensor_conf);
	if (ret) {
