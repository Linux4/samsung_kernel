...
static DEFINE_SPINLOCK(cooling_lock);
static bool cooling_apply = false;
				 ...
				 unsigned long cooling_state)
{
	...
	spin_lock(&cooling_lock);
	if (!cooling_apply && cpufreq_device->cpufreq_state == cooling_state) {
		cooling_apply = false;
		spin_unlock(&cooling_lock);
		return 0;
	}
	spin_unlock(&cooling_lock);
