...
const long valid_min_temp = 10;
const long valid_max_temp = 125;
			...
			unsigned long *temp)
{
	...
	*temp = th_zone->sensor_conf->read_temperature(data);
	...
	if ((long)*temp < valid_min_temp)
		*temp = valid_min_temp;
	if ((long)*temp > valid_max_temp)
		*temp = valid_max_temp;
	...
	*temp = *temp * MCELSIUS;
	...
	if (th_zone->sensor_conf->d_type == CPU)
		cpu_max_temp[th_zone->sensor_conf->id] = *temp;
