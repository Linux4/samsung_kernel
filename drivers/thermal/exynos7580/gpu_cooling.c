...
static struct cpufreq_frequency_table gpu_freq_table_800[] = {
	{0, 800 * 1000},
	{1, 734 * 1000},
	{2, 668 * 1000},
	{3, 534 * 1000},
...
static struct cpufreq_frequency_table gpu_freq_table_668[] = {
	{0, 668 * 1000},
	{1, 534 * 1000},
	{2, 440 * 1000},
	{3, 350 * 1000},
...
int gpufreq_set_cur_temp(bool suspended, unsigned long temp)
{
	...
	exynos_ss_printk("[TMU] GPU COLD event = %d\n", tstate);
...
gpufreq_cooling_register(const struct cpumask *clip_gpus)
{
	if (soc_is_exynos7580_v1())
		gpu_freq_table = gpu_freq_table_668;
	else
		gpu_freq_table = gpu_freq_table_800;
