...
#endif
#include "../../drivers/misc/mediatek/auxadc/mtk_auxadc.h"
...
static int sec_therm_get_adc_data(struct sec_therm_info *info)
{
	...
	for (i = 0; i < ADC_SAMPLING_CNT; i++) {
		...
		IMM_GetOneChannelValue(adc_ch, data, &adc_data);
		adc_value_mv = adc_data;
		if (i != 0) {
            if (adc_value_mv > adc_max)
                adc_max = adc_value_mv;
            else if (adc_value_mv < adc_min)
                adc_min = adc_value_mv;
		} else {
            adc_max = adc_value_mv;
            adc_min = adc_value_mv;
		}
        adc_total += adc_value_mv;
	}
