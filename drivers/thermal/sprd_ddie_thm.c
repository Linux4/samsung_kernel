...
static  void sensor_rdy_wait_clear(unsigned long reg){
	...
	if (cnt == -1){
		__thm_reg_write((reg), 0x30, 0x38);
		udelay(200);
		printk("thm sensor timeout 0x%x\n",__thm_reg_read(reg));
		__thm_reg_write((reg), 0x38, 0x38);
		WARN_ON(1);
	}
}
...
//		raw_temp = sprd_thm_temp2rawdata(CRITIC_TEMP - cal_offset);
	...
	__thm_reg_write((local_sensor_addr + SENSOR_CTRL), 0x31, 0x31);
	sensor_rdy_status(local_sensor_addr + SENSOR_CTRL);
	__thm_reg_write((local_sensor_addr + SENSOR_CTRL), 0x38, 0x38);
...
int sprd_thm_hw_disable_sensor(struct sprd_thermal_zone *pzone)
{
	...
	if (SPRD_ARM_SENSOR == pzone->sensor_id) {
		...
		__thm_reg_write((pzone->reg_base + SENSOR_CTRL), 0x00, 0x01);
		sensor_rdy_status(pzone->reg_base + SENSOR_CTRL);
		__thm_reg_write((pzone->reg_base + SENSOR_CTRL), 0x38, 0x38);
		sensor_rdy_wait_clear(pzone->reg_base + SENSOR_CTRL);
	} else if (SPRD_GPU_SENSOR == pzone->sensor_id) {
		/*__thm_reg_write((u32)(pzone->reg_base + SENSOR_CTRL+ LOCAL_SENSOR_ADDR_OFF), 0x00, 0x01);*/
		__thm_reg_write((pzone->reg_base + SENSOR_CTRL + LOCAL_SENSOR_ADDR_OFF), 0x00, 0x01);
		sensor_rdy_status(pzone->reg_base + SENSOR_CTRL + LOCAL_SENSOR_ADDR_OFF);
		__thm_reg_write((pzone->reg_base + SENSOR_CTRL + LOCAL_SENSOR_ADDR_OFF), 0x38, 0x38);
		sensor_rdy_wait_clear(pzone->reg_base + SENSOR_CTRL + LOCAL_SENSOR_ADDR_OFF);
	} else {
                         ...
                         struct device *dev)
{
	...
	strcpy(pdata->thm_name, sensor_child->name);
...
static int sprd_ddie_thm_probe(struct platform_device *pdev)
{
	...
	thm_dev = kzalloc(sizeof(struct thm_control), GFP_KERNEL);
		if (!thm_dev)
			return -ENOMEM;
	thm_dev->nsensor = pthm->nsensor;
