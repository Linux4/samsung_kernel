				...
				struct kernel_crypt_op *kcop)
{
	...
	if (ses_ptr->cdata.init != 0) {
		...
		kwmode = dmam_alloc_coherent(fmp->dev, sizeof(uint64_t), &kwmode_pa, GFP_KERNEL);
		if (!kwmode) {
			dev_err(fmp->dev, "%s: Fail to alloc memory for get kwmode\n", __func__);
			ret = -ENOMEM;
			goto out_unlock;
		}
		...
		if (smc_ret) {
			dev_err(fmp->dev, "%s: failed to get FMP KW_MODE\n", __func__);
			ret = smc_ret;
			goto dma_free;
		}
...
recover_kwmode:
	if (ses_ptr->cdata.init != 0) {
		...
		if (smc_ret) {
			dev_err(fmp->dev, "%s: failed to recover FMP KW_MODE state\n", __func__);
			ret = smc_ret;
		}
	}
