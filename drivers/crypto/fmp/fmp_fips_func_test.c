...
#else
static int fmp_func_test_zeroization(int slot, char *str)
{
	if (!str) {
		pr_err("%s: invalid fmp table address or string\n", __func__);
		return -EINVAL;
	...
	if (!strcmp("zeroization", get_fmp_fips_functest_mode()))
		pr_info("FIPS FMP %d keyslot zeroize (%s) ", slot, str);
...
int exynos_fmp_func_test_KAT_case(struct exynos_fmp *fmp)
{
	int i, ret;
#ifdef CONFIG_KEYS_IN_PRDT
	struct exynos_fmp_crypt_info fmp_crypt_info;
	struct fmp_table_setting table;
#else
	struct exynos_fmp_crypt_info fmp_crypt_info;
	struct fmp_handle handle;
#endif
	struct device *dev;
...
#ifdef CONFIG_KEYS_IN_PRDT
		memset(&fmp_crypt_info, 0, sizeof(struct exynos_fmp_crypt_info));
		memset(&table, 0, sizeof(struct fmp_table_setting));
#else
		memset(&fmp_crypt_info, 0, sizeof(struct exynos_fmp_crypt_info));
		memset(&handle, 0, sizeof(struct fmp_handle));
#endif
		dev_info(dev, "FIPS FUNC : (%d-2) Try to set config\n", i + 1);
#ifdef CONFIG_KEYS_IN_PRDT
		ret = exynos_fmp_crypt(&fmp_crypt_info, &table);
#else
		ret = exynos_fmp_crypt(&fmp_crypt_info, &handle);
#endif
		if (ret)
			dev_info(dev,
