...
#define IPA_BAN_MAX 2
static struct ipa_ban ipa_banned_netdev[IPA_BAN_MAX] = {
	{"wlan", 0}, { "bt-pan", 0},
};
int sfp_rand(void)
{
...
bool is_banned_ipa_netdev(struct net_device *dev)
{
	...
	for (i = 0; i < IPA_BAN_MAX; i++) {
		if (ipa_banned_netdev[i].state == 0 &&
		    strncasecmp(dev->name, ipa_banned_netdev[i].dev_name,
				strlen(ipa_banned_netdev[i].dev_name)) == 0) {
			dev_put(dev);
			return true;
		}
	}
...
static int sfp_parse_dt(struct device *dev)
{
	...
	ret = of_property_read_s32(np, "sprd,enable-pamwifi", &sdata);
	if (ret) {
		dev_err(dev,
                        "read sprd,enable_pamwifi fail ret %d\n", ret);
		return ret;
	}

	pamwifi_enable = sdata;
	if (pamwifi_enable)
		ipa_banned_netdev[SFP_BAN_DEV_WLAN].state = 1;
...
static const struct of_device_id sfp_match_table[] = {
	{ .compatible = "sprd,sfp"},
	{ }
};
...
static struct platform_driver sfp_driver = {
	...
	.driver = {
		.owner = THIS_MODULE,
		.name = SFP_IFACE_PREF,
		.of_match_table = sfp_match_table
	}
};
