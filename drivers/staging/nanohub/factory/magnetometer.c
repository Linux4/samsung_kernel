...
struct magnetometer_t mag_default = {
	...
	.get_fcd_magnetic_matrix = sensor_default_show,
	.set_fcd_magnetic_matrix = sensor_default_store,
};
...
static ssize_t fcd_matrix_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return get_mag(dev_get_drvdata(dev))->get_fcd_magnetic_matrix(dev, buf);
}

static ssize_t fcd_matrix_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	return get_mag(dev_get_drvdata(dev))->set_fcd_magnetic_matrix(dev, buf, size);
}
...
int set_fcd_matrix(struct ssp_data *data)
{
	...
	msg = kzalloc(sizeof(*msg), GFP_KERNEL);
	msg->cmd = MSG2SSP_AP_SET_FCD_MATRIX;
	msg->length = sizeof(data->fcd_matrix);
	msg->options = AP2HUB_WRITE;
	msg->buffer = kzalloc(sizeof(data->fcd_matrix), GFP_KERNEL);
	msg->free_buffer = 1;
	memcpy(msg->buffer, data->fcd_matrix, sizeof(data->fcd_matrix));
...
static DEVICE_ATTR(matrix2, 0660, fcd_matrix_show, fcd_matrix_store);
...
int initialize_magnetic_sensor(struct ssp_data *data)
{
	...
	ret = set_fcd_matrix(data);

	if (ret < 0)
		pr_err("[SSP]: %s - set_fcd_matrix failed %d\n", __func__, ret);
