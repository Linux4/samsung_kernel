...
#define MODEL_NAME	"S5E9925_CHUB"
	...
	struct device_attribute *attr, char *buf)
{
	...
	pr_info("CHUB01%u,CHUB1%u\n", get_module_rev(data), data->uCurFirmRev);
	return sprintf(buf, "CHUB01%u,CHUB01%u\n", get_module_rev(data), data->uCurFirmRev);
}
...
bool mcu_reset_by_msg(struct ssp_data *data)
{
	...
	pr_info("[SSP] %s, mcu reset by mcureset msg\n", __func__);
	reset_mcu(data);
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (iDataIdx = 2; iDataIdx < chLength + 2;) {
		...
		get_sensor_data(iSensorData, (char *)buffer, &iDataIdx, &(fsb[iSensorData]));
	}
