...
struct app_info{
	char name[LMK_NETLINK_MAX_NAME_LENGTH];
	int pid;
	int adj;
};

static int get_killing_process_full_name(struct task_struct *task, char *buffer, int buflen)
{
	int res = 0;
	unsigned int len;
...
static void send_killing_app_info_to_user(char* name,int pid,int adj){
	struct sk_buff* skb;
	struct nlmsghdr* nlh;
	struct app_info app;
...
static void lmk_nl_recv_msg(struct sk_buff* skb){
	char* msg="com.app.test";
	send_killing_app_info_to_user(msg,100,100);
}
...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	char  selected_process_full_name[LMK_NETLINK_MAX_NAME_LENGTH];
	...
	if (selected && (selected_oom_score_adj || is_need_lmk_kill)) {
			...
			get_killing_process_full_name(selected,selected_process_full_name,sizeof(selected_process_full_name));
			...
			rcu_read_unlock();
			send_killing_app_info_to_user(selected_process_full_name,selected_process_pid,selected_process_adj);
#ifdef CONFIG_SEC_DEBUG_LMK_MEMINFO
