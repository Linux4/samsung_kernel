...
static long number_of_reclaim_pages = NR_TO_RECLAIM_PAGES;
static uint32_t minimum_freeswap_pages = MIN_FREESWAP_PAGES;
static long minimum_reclaim_pages = MIN_RECLAIM_PAGES;
static unsigned long minimum_interval_time = MIN_CSWAP_INTERVAL;
#else
#include <linux/fs.h>
#include <linux/device.h>
#include <linux/err.h>
...
#ifdef CONFIG_ZRAM_FOR_RTCC2
module_param_named(nr_reclaim, number_of_reclaim_pages, long, S_IRUSR | S_IWUSR);
module_param_named(min_reclaim, minimum_reclaim_pages, long, S_IRUSR | S_IWUSR);
#else
module_param_named(min_reclaim, minimun_reclaim_pages, long, S_IRUSR | S_IWUSR);
#endif /* CONFIG_ZRAM_FOR_RTCC2 */
module_param_named(min_interval, minimum_interval_time, ulong, S_IRUSR | S_IWUSR);
#endif /* CONFIG_ZRAM_FOR_ANDROID */
