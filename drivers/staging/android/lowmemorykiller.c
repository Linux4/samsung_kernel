...
extern u64 zram_compressed_size;
extern u64 zram_pages_stored;
#endif
...
static unsigned long lowmem_scan(struct shrinker *s, struct shrink_control *sc)
{
...
#endif
#if defined(CONFIG_RUNTIME_COMPCACHE)
		if (zram_pages_stored) {
			lowmem_print(3, "shown tasksize : %d\n", tasksize);
			tasksize += zram_compressed_size
	...
	if (selected) {
		...
		/*
		 * LMK reclaimed pages need to be considered in
		 * shrink slab logic to quit memory reclaim.
		 */
		if (reclaim_state)
