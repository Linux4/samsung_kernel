					  ...
					  unsigned long flags)
{
	...
	return buffer_info->priv_phys == ION_CARVEOUT_ALLOCATE_FAIL ?
		-ENOMEM : 0;
}
...
struct sg_table *ion_dma_reserved_heap_map_dma(struct ion_heap *heap,
					       struct ion_buffer *buffer)
{
...
void ion_dma_reserved_heap_unmap_dma(struct ion_heap *heap,
				     struct ion_buffer *buffer)
{
...
static struct ion_heap_ops dma_reserved_heap_ops = {
		...
		.map_dma = ion_dma_reserved_heap_map_dma,
		.unmap_dma = ion_dma_reserved_heap_unmap_dma,
