					      ...
					      *heap_data)
{
	...
	/*
	 * need to use GFP_DMA32 instead of GFP_DMA
	 *
	 * for arm64 uses ZONE_DMA for allocations below 32-bits.  These days we
	 * name the zone for that ZONE_DMA32, which will allow to use the
	 * dma-direct and generic swiotlb code as-is, so rename it.
	 */
	page = alloc_pages(GFP_DMA32 | __GFP_ZERO, order);
	if (!page) {
