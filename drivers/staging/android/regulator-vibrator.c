...
static void vibrator_enable(struct timed_output_dev *dev, int duration)
{
	unsigned long flags;
	unsigned int dur = duration;
	struct reg_vibr *vibr = dev_get_drvdata(dev->dev->parent);
...
static int vibrator_get_time(struct timed_output_dev *dev)
{
	...
	if (hrtimer_active(&vibr->vibr_timer)) {
		ktime_t r = hrtimer_get_remaining(&vibr->vibr_timer);
...
*/
	timed_output_dev_register(&pdev->dev, &mtk_vibrator);
