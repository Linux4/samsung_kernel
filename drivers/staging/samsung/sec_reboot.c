...
static void sec_power_off(void)
{
	...
	for_each_child_of_node(np, pp) {
		...
		if (keycode == KEY_END_CALL || keycode == KEY_POWER) {
			pr_info("%s: <%u>\n", __func__,  keycode);
...
static void sec_reboot(char str, const char *cmd)
{
	...
	/* Check valid INFORM3 or not */
	inform3 = readl(EXYNOS_PMU_INFORM3);
	if ((inform3 == UPLOAD_CAUSE_KERNEL_PANIC) ||
		(inform3 == UPLOAD_CAUSE_FORCED_UPLOAD) ||
		(inform3 == UPLOAD_CAUSE_CP_ERROR_FATAL) ||
		(inform3 == UPLOAD_CAUSE_USER_FAULT) ||
		(inform3 == UPLOAD_CAUSE_HSIC_DISCONNECTED))
		goto flush_cache;
