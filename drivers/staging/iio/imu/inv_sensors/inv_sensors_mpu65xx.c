...
#define DEF_SW_ST_GYRO_FULLSCALE	2000	/*gyro full scale dps*/
#define DEF_SW_SELFTEST_SENSITIVITY(x)	\
	((DEF_SW_ST_GYRO_FULLSCALE*x)/32768)
...
#define DEF_ST_6500_STABLE_TIME		200
#define BYTES_PER_SENSOR		(6)
#define DEF_SELFTEST_SAMPLE_RATE	0
#define DEF_GYRO_WAIT_TIME		70
#define THREE_AXIS			(3)
#define INIT_ST_SAMPLES			70
...
struct mpu65xx_ctrl_t {
	...
	bool is_accel_active;
	bool is_gyro_active;
	bool is_slave_active;
};
	...
	int *pkcount)
{
	...
	int gyro_data[DEF_SW_SELFTEST_SAMPLE_COUNT][THREE_AXIS];
	unsigned int sleep;
	...
	/*set pll*/
	tmp = regs[0] | CLKSEL_PLL;
	i2c_write_single(dctl, MPU65XX_PWR_MGMT_1, tmp, res, exit_i2c_error);
	/*set gyro work*/
	tmp = (BIT_ACCEL_STBY);
	...
	i2c_write_single(dctl, MPU65XX_USER_CTRL, BIT_FIFO_RST, res,
		exit_i2c_error);
	/*disable gyro lpf */
	i2c_write_single(dctl, MPU65XX_CONFIG, 0x0, res, exit_i2c_error);
	/*disable accel lpf */
	i2c_write_single(dctl, MPU65XX_ACCEL_CONFIG2, 0x0, res,
		exit_i2c_error);
	...
	/*-------------------------------------------------*/
	sleep = DEF_SW_SELFTEST_SAMPLE_TIME + DEF_SW_SKIP_COUNT;
	do {
		msleep(sleep);

		i2c_read(dctl, MPU65XX_FIFO_COUNTH, 2, regs, res,
			exit_i2c_error);
		...
		/* Check packet count */
		if (packet_count == 0 && retry_cnt > 9) {
			INVSENS_LOGE("Gyro Packet Count ERR, Retry done %d\n",
				retry_cnt);
			return -EPERM;
		}
		...
		sleep = 20;
		retry_cnt++;
	} while (packet_count < (DEF_SW_SELFTEST_SAMPLE_TIME + DEF_SW_SKIP_COUNT));
	...
	if (packet_count > DEF_SW_SELFTEST_SAMPLE_COUNT)
		packet_count = DEF_SW_SELFTEST_SAMPLE_COUNT;

	pr_info("[INV] Gyro Packet counter : %d\n", packet_count);
	...
	for (i = 0; i < packet_count; i++) {
		...
		gyro_data[i][0] = (int)(sensors[0] * DEF_ST_PRECISION);
		gyro_data[i][1] = (int)(sensors[1] * DEF_ST_PRECISION);
		gyro_data[i][2] = (int)(sensors[2] * DEF_ST_PRECISION);

		gyro_bias[0] += gyro_data[i][0];
		gyro_bias[1] += gyro_data[i][1];
		gyro_bias[2] += gyro_data[i][2];
	}
	...
	scaled_bias[0] = DEF_SW_SELFTEST_SENSITIVITY(gyro_bias[0]);
	scaled_bias[1] = DEF_SW_SELFTEST_SENSITIVITY(gyro_bias[1]);
	scaled_bias[2] = DEF_SW_SELFTEST_SENSITIVITY(gyro_bias[2]);
	...
	scaled_rms[0] = DEF_SW_SELFTEST_SENSITIVITY(gyro_rms[0]);
	scaled_rms[1] = DEF_SW_SELFTEST_SENSITIVITY(gyro_rms[1]);
	scaled_rms[2] = DEF_SW_SELFTEST_SENSITIVITY(gyro_rms[2]);
	...
	for (i = 0; i < 3; i++) {
		...
		if (!otp_value_zero) {
			...
			if (st_shift_cust[i] < DEF_6500_GYRO_CT_SHIFT_DELTA
						* st_shift_prod[i]) {
				ret_val = 1;
				INVSENS_LOGE("gyro st_shift_cust under A\n");
			}
		} else {
			...
			if (st_shift_cust[i] < DEF_GYRO_ST_AL *
						...
						DEF_ST_PRECISION) {
				ret_val = 1;
				INVSENS_LOGE("gyro st_shift_cust under B\n");
			}
		}
	...
	if (ret_val == 0) {
		...
		for (i = 0; i < 3; i++)
			if (abs(reg_avg[i]) > DEF_GYRO_OFFSET_MAX *
						...
						DEF_ST_PRECISION) {
				ret_val = 1;
				INVSENS_LOGE("gyro st_shift_cust exceed C\n");
			}
	}

	if (ret_val)
		INVSENS_LOGE("Gyro hw selftest failed\n");
...
#define ACCEL_ST_AL_MAX ((DEF_ACCEL_ST_AL_MAX * DEF_ST_SCALE \
	...
	if (!otp_value_zero) {
		...
		for (i = 0; i < 3; i++) {
			...
			if (st_shift_ratio[i] > DEF_6500_ACCEL_ST_SHIFT_DELTA) {
				ret_val = 1;
				INVSENS_LOGE("accel st_shift_ratio exceed\n");
			}
		...
		for (i = 0; i < 3; i++) {
			...
			if (st_shift_cust[i] < ACCEL_ST_AL_MIN ||
				...
				ret_val = 1;
				INVSENS_LOGE("accel st_shift_cust exceed\n");
			}
	...
	if (ret_val)
		INVSENS_LOGE("Accel hw selftest failed\n");
...
{
	int res = 0;
	u8 reg = 0, user_ctrl = 0, fifo_en = 0;
	...
	if (dctl->enabled_mask) {
		...
		if (!dctl->is_lpa_enabled) {
			if (dctl->is_accel_active | dctl->is_gyro_active)
				user_ctrl = BIT_FIFO_EN;

			if (dctl->is_slave_active)
...
static int mpu65xx_set_motion_int(struct mpu65xx_ctrl_t *dctl)
{
	...
	if (dctl->is_mi_enabled) {
		wom = dctl->reg.wom_thres;
		i2c_write_single(dctl, MPU65XX_WOM_THRESH, wom, res,
			exit_i2c_error);
	}
				 ...
				 u32 func_mask, bool enable)
{
	...
	dctl->is_accel_active = is_accel_active;
	dctl->is_gyro_active = is_gyro_active;
	dctl->is_slave_active = is_slave_active;
	...
	if (dctl->sm_ctrl.is_dmp_on) {
		...
		*/
		...
		if (count > 0) {
			...
			/*reset packet number
			*to consider the buffer size of data list*/
...
static int mpu65xx_if_init(struct invsens_driver_t *drv)
{
	...
	dctl->is_accel_active = false;
	dctl->is_gyro_active = false;
	dctl->is_slave_active = false;
	...
	case INV_IOCTL_S_GYRO_SELFTEST:
		{
			...
			if (scaled_bias[0] > 30*DEF_ST_PRECISION ||
				...
				scaled_bias[2] > 30*DEF_ST_PRECISION) {
				st_result = 0; /*fail*/
				INVSENS_LOGE("Gyro bias exceeded threshold\n");
			} else {
				st_result = 1; /*success*/
			}
			...
			if (scaled_rms[0] == 0 ||
				...
				scaled_rms[2] == 0) {
				st_result = 0; /*fail*/
				INVSENS_LOGE("Gyro rms should not be 0\n");
			} else if (scaled_rms[0] > 5*DEF_ST_PRECISION ||
				scaled_rms[1] > 5*DEF_ST_PRECISION ||
				scaled_rms[2] > 5*DEF_ST_PRECISION) {
				st_result = 0; /*fail*/
				INVSENS_LOGE("Gyro rms exceeded threshold\n");
			} else {
				st_result = 1; /*success*/
			}
			...
			if (st_result) {
				do {
					hwst_result = mpu65xx_hw_selftest(dctl,
						accel_st_ratio, gyro_st_ratio);
					pr_info("[INV] Gyro selftest: try_count = %d\n", try_count);
					try_count -- ;
				} while(try_count > 0 && !(hwst_result & 0x1));

			} else
				INVSENS_LOGE("Skip Gyro hw selftest\n");

			pr_info("[INV] Gyro selftest result: sw = %d, hw = %d",
				st_result, hwst_result);
	...
	case INV_IOCTL_S_ACCEL_SELFTEST:
		{
			...
			do {
				res = mpu65xx_hw_selftest(dctl,
					accel_st_ratio, gyro_st_ratio);
				pr_info("[INV] Accel selftest: try_count = %d\n", try_count);
				try_count -- ;
			} while(try_count > 0 && !((res >> 1) & 0x1));
