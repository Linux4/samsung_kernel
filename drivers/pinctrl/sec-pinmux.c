...
#define AP_GPIO_COUNT   134
#else
#define AP_GPIO_COUNT   145
#endif
/****************************************************************/
...
static void msm8916_check_gpio_status(unsigned char phonestate)
{
	...
	for (i = 0; i < AP_GPIO_COUNT; i++) {
		...
		if (i >= CONFIG_ESE_SPI_GPIO_START && i <= CONFIG_ESE_SPI_GPIO_END)
			continue;
#endif
...
#endif
#if defined(CONFIG_ESE_SECURE)
		if (gpio >= CONFIG_ESE_SPI_GPIO_START && gpio <= CONFIG_ESE_SPI_GPIO_END)
			continue;
#endif
...
static int __init msm_gpiomux_init(void)
{
	...
	msm_gpiomux_ngpio = msm_tlmm_get_gp_num();
