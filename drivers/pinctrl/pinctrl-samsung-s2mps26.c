...
#define S2MPS26_REG_GPIO_STATUS1		(0x0E)
#define S2MPS26_PMIC_GPIO_SET0			(0x12)
#define GPIO_RANGE				(8)
#define GPIO_DEFAULT_NUM			(26)
...
static const char *const pmic_gpio_groups[] = {
	...
	"gpio_s6", "gpio_s7", "gpio_s8", "gpio_s9", "gpio_s10", "gpio_s11",
	"gpio_s12", "gpio_s13", "gpio_s14", "gpio_s15", "gpio_s16", "gpio_s17",
	"gpio_s18", "gpio_s19", "gpio_s20", "gpio_s21", "gpio_s22", "gpio_s23",
	"gpio_s24", "gpio_shv",
};
...
static int pmic_gpio_read(struct pmic_gpio_state *state, unsigned int addr)
{
	...
	ret = s2mps26_read_reg(state->i2c, addr, &val);
	if (ret < 0)
		dev_err(state->dev, "[SUB_PMIC] read 0x%x failed\n", addr);
	else
		ret = val;
			   ...
			   struct pmic_gpio_pad *pad, unsigned int val)
{
	...
	reg |= 1 << ((pad->base % 4) + SYSREG_ENABLE);
...
static int pmic_gpio_write(struct pmic_gpio_state *state, unsigned int addr, unsigned int val)
{
	...
	ret = s2mps26_write_reg(state->i2c, addr, val);
	if (ret < 0)
		dev_err(state->dev, "[SUB_PMIC] write 0x%x failed\n", addr);
				...
				struct seq_file *s, unsigned pin)
{
	...
	uint8_t reg_set = S2MPS26_PMIC_GPIO_SET0 + pin;
	uint8_t reg_status = (pin / GPIO_RANGE) + S2MPS26_REG_GPIO_STATUS1;
					...
					unsigned pin, unsigned long *config)
{
	...
	case PMIC_GPIO_CONF_PULL_UP:
		...
		dev_info(state->dev, "[SUB_PMIC] %s: pin%d: %s(%#x)\n",
	...
	case PMIC_GPIO_CONF_INPUT_ENABLE:
		...
		dev_info(state->dev, "[SUB_PMIC] %s: pin%d: MODE(%s)\n",
	...
	case PMIC_GPIO_CONF_OUTPUT:
		...
		dev_info(state->dev, "[SUB_PMIC] %s: pin%d: DAT(%s_%#x)\n",
	...
	case PMIC_GPIO_CONF_DRIVE_STRENGTH:
		...
		dev_info(state->dev, "[SUB_PMIC] %s: pin%d: DRV_STR(%s_%#x)\n",
				__func__, pin, strengths[pad->strength], pad->strength);
		break;
	default:
		dev_err(state->dev, "[SUB_PMIC] %s: pmic_gpio_pin_config_get fail(param: %#x)\n",
				__func__, param);
		return -EINVAL;
	}
					...
					unsigned long *configs, unsigned num_configs)
{
	...
	for (i = 0; i < num_configs; i++) {
		...
		case PMIC_GPIO_CONF_DRIVE_STRENGTH:
			...
			break;
		default:
			dev_err(state->dev, "[SUB_PMIC] %s: pmic_gpio_pin_config_set fail(param: %#x)\n",
					__func__, param);
			return -ENOTSUPP;
		}
	}
	...
	dev_info(state->dev, "[SUB_PMIC] %s: pin%d: reg(%#02hhx%02hhx:%#02hhx), "
...
static int pmic_gpio_parse_dt(struct s2mps26_dev *iodev, struct pmic_gpio_state *state)
{
	...
	gpio_np = of_find_node_by_name(mfd_np, "s2mps26-gpio");
	if (!gpio_np) {
...
static int pmic_gpio_probe(struct platform_device *pdev)
{
	...
	/* VGPIO setting by AP */
	state->sysreg_base = ioremap(SYSREG_ALIVE + VGPIO_RX_R11, 0x24);
	...
	if (ret < 0) {
		dev_err(state->dev, "[SUB_PMIC] %s: pmic_gpio_parse_dt fail\n", __func__);
		ret = -ENODEV;
...
static void pmic_gpio_shutdown(struct platform_device *pdev)
{
	struct pmic_gpio_state *state = platform_get_drvdata(pdev);
	unsigned int i = 0;
	int ret = 0;

	dev_info(state->dev, "[SUB_PMIC] %s: Configure GPIOs as input, pull-down\n", __func__);

	for (i = 0; i < 23; i++) {
		/* GPIO0~22 pull_down enable, output drive strength control = 1mA */
		ret = pmic_gpio_write(state, S2MPS26_PMIC_GPIO_SET0 + i, 0x09);
		if (ret < 0)
			dev_err(state->dev, "%s: Failed to write PMIC GPIO(%d: %d)\n",
					__func__, i, ret);
	}
}

static const struct of_device_id pmic_gpio_of_match[] = {
	{ .compatible = "s2mps26-gpio" },
	{ },
};
...
static struct platform_driver pmic_gpio_driver = {
	...
	.shutdown = pmic_gpio_shutdown,
};

module_platform_driver(pmic_gpio_driver);

MODULE_AUTHOR("Myungsu Cha <myung-su.cha@samsung.com>");
