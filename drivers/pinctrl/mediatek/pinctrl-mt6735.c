...
bool mtk_pctrl_init_gpio_default(struct platform_device *pdev)
{
	...
	struct pinctrl_state *pin_init[ARRAY_SIZE(mtk_pins_mt6735)];
	int i = 0, ret;
	const char* pinname;

	pr_warn("mt6735 titan pinctrl probe init gpio default value\n");
	...
	for (i = 0; i < ARRAY_SIZE(mtk_pins_mt6735); i++) {
		pinname = kasprintf(GFP_KERNEL, "gpio%d", i);
		pin_init[i] = pinctrl_lookup_state(pinctrl_gpio, pinname);
		if (!IS_ERR(pin_init[i])) 
			pinctrl_select_state(pinctrl_gpio, pin_init[i]);
	}
...
static int mt6735_pinctrl_probe(struct platform_device *pdev)
{
	...
	mtk_pctrl_init(pdev, &mt6735_pinctrl_data, NULL);
