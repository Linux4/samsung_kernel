...
//Modify by shengzhong for SR-AL7160A-01-748 on 20240507 begin
...
ssize_t mtk_pctrl_show_pin(struct mtk_pinctrl *hw,
	unsigned int gpio, char *buf, unsigned int bufLen, int *pHexOut)
{
	const struct mtk_pin_desc *desc;
	...
	if (pHexOut){
		...
		// 1bit pull_en
		// ibit pull_sel
		int dir = mtk_pctrl_get_direction(hw, gpio);
		int dout = mtk_pctrl_get_out(hw, gpio);
		int din = mtk_pctrl_get_in(hw, gpio);
		int driver = mtk_pctrl_get_driving(hw, gpio);
		int smt = mtk_pctrl_get_smt(hw, gpio);
		int ies = mtk_pctrl_get_ies(hw, gpio);
		(*pHexOut) = (pinmux & 0xF) << 12 |
					 (dir & 0x1) << 11 |
					 (dout & 0x1) << 10 |
					 (din  & 0x1) << 9 |
					 (driver & 0xF) << 5 |
...
//Modify by shengzhong for SR-AL7160A-01-748 on 20240507 end
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < chip->ngpio; i++) {
		...
		//Modify by shengzhong for SR-AL7160A-01-748 on 20240507 begin
		len += mtk_pctrl_show_pin(hw, i, buf + len, bufLen - len, NULL);
		//Modify by shengzhong for SR-AL7160A-01-748 on 20240507 end
	}
...
void gpio_dump_regs_range(int start, int end)
{
	...
	for (i = start; i < end; i++) {
		//Modify by shengzhong for SR-AL7160A-01-748 on 20240507 begin
		(void)mtk_pctrl_show_pin(hw, i, buf, 96, &combined);
		snprintf(gpioall+(i*4),sizeof(gpioall),"%04X", (combined & 0xFFFF));
		//Modify by shengzhong for SR-AL7160A-01-748 on 20240507 end
	}
	//Modify by shengzhong for SR-AL7160A-01-748 on 20240507 begin
	pr_notice("gpio:%s", gpioall);
	//Modify by shengzhong for SR-AL7160A-01-748 on 20240507 end
}
