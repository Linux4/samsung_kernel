		...
		unsigned int pin, bool enable, bool isup, unsigned int arg)
{
	...
	if (arg != 0 && arg != 1) {
		dev_info(pctl->dev, "invalid pull-up argument %d on pin %d .\n",
			arg, pin);
		return -EINVAL;
	}
		...
		struct regmap *regmap)
{
	...
	if (node) {
		...
		dev_info(&pdev->dev, "Pinctrl node has not register regmap.\n");
		return -EINVAL;
	}
	...
	*pctl->chip = mtk_gpio_chip;
	...
	if (IS_ERR(pctl->pctl_dev)) {
		dev_info(&pdev->dev, "couldn't register pinctrl driver\n");
