...
bool mtk_pctrl_init_gpio_default(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(mtk_pins_mt6757); i++) {
		...
		if (IS_ERR(pin_init)) {
			...
			dev_info(&pdev->dev, "titan no need to set gpio in kernel gpio name = %s!\n", pin_name[i]);
			pass_gpio[i] = 1;
		}
		if (pass_gpio[i] != 1) {
			dev_info(&pdev->dev, "set setting of %s!\n", pin_name[i]);
