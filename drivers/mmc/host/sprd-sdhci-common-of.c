...
static void sdhci_dumpregs(struct sdhci_host *host)
{
	...
	for (i = 0; i < 0x08; i++) {
		printk(KERN_ERR DRIVER_NAME
		       ...
		       sdhci_readl(host, (regAddr + 16 * i)), sdhci_readl(host,
									  (regAddr
									   + 4 +
									   16 *
									   i)),
		       sdhci_readl(host, (regAddr + 8 + 16 * i)),
		       sdhci_readl(host, (regAddr + 12 + 16 * i)));
	}
						...
						struct mmc_ios *ios)
{
	...
	case MMC_SIGNAL_VOLTAGE_330:
		...
		if (host->vqmmc) {
			ret =
			    regulator_set_voltage(host->vqmmc, 3000000,
						  3000000);
			if (ret) {
				pr_warning
	...
	case MMC_SIGNAL_VOLTAGE_120:
		if (host->vqmmc) {
			ret =
			    regulator_set_voltage(host->vqmmc, 1100000,
						  1300000);
			if (ret) {
				pr_warning
...
static void sdhci_cmd_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (host->cmd->error) {
		if (host->mmc->card) {
			pr_err
			    ("%s: !!!!! error in sending cmd:%d, int:0x%x, err:%d \n",
			     mmc_hostname(host->mmc), host->cmd->opcode,
			     intmask, host->cmd->error);
			sdhci_dumpregs(host);
		}
		tasklet_schedule(&host->finish_tasklet);
		return;
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	if ((host->version >= SDHCI_SPEC_200) && (caps[0] & SDHCI_CAN_DO_ADMA2))
		host->flags |= SDHCI_USE_ADMA;
