		...
		struct device_attribute *attr, char *buf)
{
	...
	if (card) {
		...
		sprintf(buf, "\"MANID\":\"0x%02X\",\"PNM\":\"%s\","\
				...
				(card->ext_csd.cmdq_mode_en ? true : false),
...
static struct attribute *mmccard_attributes[] = {
	&dev_attr_hwrst.attr,
	NULL,
};

static struct attribute_group mmccard_attr_group = {
	.attrs = mmccard_attributes,
};
#endif
...
#ifdef CONFIG_SEC_FACTORY
		retval = sysfs_create_group(&mmccard_dev->kobj, &mmccard_attr_group);
		if (retval) {
			pr_err("%s:Failed to create sdcard sysfs group\n", __func__);
		}
#endif
