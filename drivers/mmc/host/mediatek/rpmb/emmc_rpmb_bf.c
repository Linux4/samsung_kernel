...
struct task_struct *open_th_bf;
struct task_struct *rpmbDci_th_bf;
...
int hmac_sha256_bf(const char *key, u32 klen, const char *str, u32 len, u8 *hmac)
{
 ...
 */
int emmc_rpmb_switch_bf(struct mmc_card *card, struct emmc_rpmb_blk_data *md)
{
	int ret;
...
static void emmc_rpmb_dump_frame_bf(u8 *data_frame)
{
...
static int emmc_rpmb_send_command_bf(
...
int emmc_rpmb_req_start_bf(struct mmc_card *card, struct emmc_rpmb_req *req)
{
	int err = 0;
	...
	*/
	if (type == RPMB_WRITE_DATA)
		err = emmc_rpmb_send_command_bf(card, data_frame, blks, type, RPMB_REQ);
	else
		err = emmc_rpmb_send_command_bf(card, data_frame, 1, type, RPMB_REQ);
	...
	*/
	...
	if (type == RPMB_WRITE_DATA || type == RPMB_PROGRAM_KEY) {
		...
		err = emmc_rpmb_send_command_bf(card, data_frame, 1, RPMB_RESULT_READ, RPMB_REQ);
		if (err) {
	...
	if (type == RPMB_READ_DATA)
		err = emmc_rpmb_send_command_bf(card, data_frame, blks, type, RPMB_RESP);
	else
		err = emmc_rpmb_send_command_bf(card, data_frame, 1, type, RPMB_RESP);
...
int emmc_rpmb_req_handle_bf(struct mmc_card *card, struct emmc_rpmb_req *rpmb_req)
{
	 ...
	 */
	ret = emmc_rpmb_switch_bf(card, part_md);
	if (ret) {
	 ...
	 */
	ret = emmc_rpmb_req_start_bf(card, rpmb_req);
	if (ret) {
...
error:
	...
	emmc_rpmb_dump_frame_bf(rpmb_req->data_frame);
 ...
 *
 **********************************************************************************/
int emmc_rpmb_req_set_key_bf(struct mmc_card *card, u8 *key)
{
	...
	ret = emmc_rpmb_req_handle_bf(card, &rpmb_req);
	if (ret) {
...
int emmc_rpmb_req_get_wc_bf(struct mmc_card *card, u8 *key, u32 *wc)
{
	struct emmc_rpmb_req rpmb_req;
	struct s_rpmb *rpmb_frame;
	...
	do {
		 ...
		 */
		hmac_sha256_bf(key, 32, rpmb_frame->data, 284, hmac);
...
int emmc_rpmb_req_write_data_bf(struct mmc_card *card, struct rpmb_ioc_param *param)
{
	struct emmc_rpmb_req rpmb_req;
	struct s_rpmb *rpmb_frame;
	...
	while (left_blkcnt) {
		...
		ret = emmc_rpmb_req_get_wc_bf(card, param->key, &wc);
		if (ret) {
		...
		hmac_sha256_bf(param->key, 32, dataBuf_start, 284 * tran_blkcnt, rpmb_frame[iCnt].mac);
		 ...
		 */
		hmac_sha256_bf(param->key, 32, rpmb_frame->data, 284, hmac);
	...
	for (iCnt = 0; iCnt < total_blkcnt; iCnt++) {
		...
		hmac_sha256_bf(param->key, 32, rpmb_frame->data, 284, rpmb_frame->mac);
...
int emmc_rpmb_req_read_data_bf(struct mmc_card *card, struct rpmb_ioc_param *param)
{
	struct emmc_rpmb_req rpmb_req;
	...
	*/
	...
	while (left_blkcnt) {
		 ...
		 */
		hmac_sha256_bf(param->key, 32, dataBuf_start, 284 * tran_blkcnt, hmac);
...
static int emmc_rpmb_execute_bf(u32 cmdId)
{
	int ret;
...
{
	/* do nothing but wake up emmc_rpmb_listenDci_bf */
...
int emmc_rpmb_listenDci_bf(void *data)
{
	...
	for (;;) {
		...
		msg.result = emmc_rpmb_execute_bf(cmdId);
...
#define BLOWFISH_OPEN_SESSION_TMO       600
static int emmc_rpmb_open_session_bf(void)
{
	int cnt = 0, ret = 0;
	struct tdriver_msg msg;
	...
	while (!tzdev_is_up()) {
		/* tzdev_print(0, "blowfish NOT ready!, sleep 2s\n"); */
		msleep(2000);
		cnt++;
...
retry:
	...
	rpmb_dci = (struct dciMessage_t *) bf_shmem;
	rpmbDci_th_bf = kthread_run(emmc_rpmb_listenDci_bf, NULL, "rpmb_Dci");
	if (IS_ERR(rpmbDci_th_bf)) {
...
static int emmc_rpmb_thread_bf(void *context)
{
	int ret;

	MSG(INFO, "%s start\n", __func__);

	ret = emmc_rpmb_open_session_bf();
	MSG(INFO, "%s emmc_rpmb_open_session_bf, ret = %x\n", __func__, ret);

	return 0;
}

static int emmc_rpmb_open_bf(struct inode *inode, struct file *file)
{
	MSG(INFO, "%s, !!!!!!!!!!!!\n", __func__);
...
static long emmc_rpmb_ioctl_bf(struct file *file, unsigned int cmd, unsigned long arg)
{
	int err = 0;
	struct mmc_card *card = mtk_msdc_host[0]->mmc->card;
	...
	case RPMB_IOCTL_PROGRAM_KEY:
		...
		ret = emmc_rpmb_req_set_key_bf(card, param.key);
	...
	case RPMB_IOCTL_READ_DATA:
		...
		ret = emmc_rpmb_req_read_data_bf(card, &param);
	...
	case RPMB_IOCTL_WRITE_DATA:
		...
		ret = emmc_rpmb_req_write_data_bf(card, &param);
...
static int emmc_rpmb_close_bf(struct inode *inode, struct file *file)
{
	int ret = 0;
...
static const struct file_operations emmc_rpmb_fops = {
	.owner = THIS_MODULE,
	.open = emmc_rpmb_open_bf,
	.release = emmc_rpmb_close_bf,
	.unlocked_ioctl = emmc_rpmb_ioctl_bf,
	.write = NULL,
	.read = NULL,
};

static int __init emmc_rpmb_init_bf(void)
{
	int alloc_ret = -1;
	int cdev_ret = -1;
	int major;
	...
	open_th_bf = kthread_run(emmc_rpmb_thread_bf, NULL, "rpmb_open");
	if (IS_ERR(open_th_bf))
		MSG(ERR, "%s, init kthread_run failed!\n", __func__);
...
late_initcall(emmc_rpmb_init_bf);
