...
/* if use external ldo please enable it, else disable it.*/
#define SDCARD_USE_EXTERNAL_LDO

#ifdef SDCARD_USE_EXTERNAL_LDO
struct pinctrl *sd_ptl;
struct pinctrl_state *sd_ext_ldo_low = NULL;
struct pinctrl_state *sd_ext_ldo_high = NULL;
#endif
...
#ifdef SDCARD_USE_EXTERNAL_LDO
static void msdc_sd_ext_ldo(int vol)
{
	if (vol)
	{
		pinctrl_select_state(sd_ptl, sd_ext_ldo_high);
	}
	else
	{
		pinctrl_select_state(sd_ptl, sd_ext_ldo_low);
	}
}
#endif
...
int msdc_hwPowerOn(unsigned int powerId, int powerVolt, char *mode_name)
{
	...
	if (powerId == POWER_LDO_VMCH)
		...
		reg = reg_vemc_3v3;
#ifdef SDCARD_USE_EXTERNAL_LDO
	if (powerId == POWER_LDO_VMCH && board_id >=2) {
		msdc_sd_ext_ldo(powerVolt);
		return 0;
	}
#endif
	if (reg == NULL) {
...
bool msdc_hwPowerDown(unsigned int powerId, char *mode_name)
{
	...
	/*--Disable OCFB_EN--*/
	if (powerId == POWER_LDO_VMCH)
		pmic_set_register_value(PMIC_RG_VMCH_OCFB_EN, 0);
...
static void msdc_pin_pud(struct msdc_host *host, u32 mode)
{
	...
	case 0:
		...
		sdr_set_field(MSDC0_GPIO_PUPD0_G5_ADDR,
			MSDC0_PUPD_DSL_MASK, 0x4);
		sdr_set_field(MSDC0_GPIO_PUPD1_G5_ADDR,
			MSDC0_PUPD_RSTB_MASK, 0x4);
...
/* supply clock(400KHz) at sd card initialization */
void msdc_sd_clock_run(struct msdc_host *host)
{
	void __iomem *base = host->base;

	msdc_set_mclk(host, MMC_TIMING_LEGACY, 400000);
...
#ifdef SDCARD_USE_EXTERNAL_LDO
static int msdc_sd_ldo_init(struct platform_device *pdev)
{
	sd_ptl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(sd_ptl)) {
		pr_err("[MMC]: SD card get pinctrl fail\n");
		return -1;
	}


	sd_ext_ldo_low = pinctrl_lookup_state(sd_ptl, "sd_ext_ldo_low");
	if (IS_ERR(sd_ext_ldo_low)) {
		pr_err("[MMC]: SD card get sd_ldo_low fail\n");
	}

 
	sd_ext_ldo_high = pinctrl_lookup_state(sd_ptl, "sd_ext_ldo_high");
	if (IS_ERR(sd_ext_ldo_high)) {
		pr_err("[MMC]: SD card get sd_ldo_high fail\n");
	}
	pr_err("[MMC]%s done\n",__func__);
	return 0;
}
...
#ifdef SDCARD_USE_EXTERNAL_LDO
	if (pdev->id == 1 && board_id >= 2)
		msdc_sd_ldo_init(pdev);
#endif
