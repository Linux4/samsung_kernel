...
void msdc_pin_config_by_id(u32 id, u32 mode)
{
	if (id == 0) {
		...
		if (mode == MSDC_PIN_PULL_NONE) {
			...
			/* Switch MSDC0_CMD/DSL/DAT* to 50K ohm PD
			 * Switch MSDC0_CLK to High_Z
			 */
			MSDC_SET_FIELD(MSDC0_GPIO_PUPD_ADDR, MSDC0_PUPD_ALL_MASK, 0x7FE);
			MSDC_SET_FIELD(MSDC0_GPIO_R0_ADDR, MSDC0_R0_ALL_MASK, 0x0);
			MSDC_SET_FIELD(MSDC0_GPIO_R1_ADDR, MSDC0_R1_ALL_MASK, 0x7FE);
		} else if (mode == MSDC_PIN_PULL_UP) {
			/* Switch MSDC0_CLK to High_Z,
			 * MSDC0_CMD/MSDC0_DAT* to 10K ohm PU,
			 * MSDC0_DSL to 50K ohm PD
			 */
			MSDC_SET_FIELD(MSDC0_GPIO_PUPD_ADDR, MSDC0_PUPD_ALL_MASK, 0x400);
			MSDC_SET_FIELD(MSDC0_GPIO_R0_ADDR, MSDC0_R0_ALL_MASK, 0x3FE);
			MSDC_SET_FIELD(MSDC0_GPIO_R1_ADDR, MSDC0_R1_ALL_MASK, 0x400);
		}
	} else if (id == 1) {
		if (mode == MSDC_PIN_PULL_NONE) {
