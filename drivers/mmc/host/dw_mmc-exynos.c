...
static int dw_mci_exynos_priv_init(struct dw_mci *host)
{
	...
	host->idle_status = exynos_get_idle_ip_index(dev_name(host->dev));
	exynos_update_ip_idle_status(host->idle_status, STATUS_IDLE);
#endif
					...
					unsigned long cmd, void *v)
{
	...
	case LPA_ENTER:
	case LPD_ENTER:
		if( host->req_state == DW_MMC_REQ_IDLE )
			exynos_sfr_save(0);
		break;
	case LPA_ENTER_FAIL:
	case LPD_ENTER_FAIL:
...
void dw_mci_exynos_cfg_smu(struct dw_mci *host)
{
...
#ifdef CONFIG_MMC_DW_FMP_DM_CRYPT
	if (!(host->pdata->quirks & DW_MCI_QUIRK_BYPASS_SMU) && !(host->pdata->quirks & DW_MCI_QUIRK_USE_SMU))
		return;
#else
	...
	if (host->pdata->quirks & DW_MCI_QUIRK_USE_SMU) {
		reg &= ~DWMCI_MPSECURITY_MMC_SFR_PROT_ON;
		reg &= ~DWMCI_MPSECURITY_USEOTPMASK;
		mci_writel(host, MPSECURITY, (reg | DWMCI_MPSECURITY_PROTBYTZPC | DWMCI_MPSECURITY_FMP_ENC_ON | \
							DWMCI_MPSECURITY_NSKEYREG | DWMCI_MPSECURITY_DESCTYPE(3)));
	}
#else
	if (host->pdata->quirks & DW_MCI_QUIRK_BYPASS_SMU) {
		mci_writel(host, MPSBEGIN0, 0);
...
static void dw_mci_exynos_set_ios(struct dw_mci *host, unsigned int tuning, struct mmc_ios *ios)
{
	...
	if (timing == MMC_TIMING_MMC_HS200_DDR) {
		...
		if (priv->ctrl_flag & DW_MMC_EXYNOS_ULP_MUX_SEL_HS400)
			clksel |= BIT(30);
...
static int dw_mci_exynos_parse_dt(struct dw_mci *host)
{
	...
	*/
	...
	if (of_find_property(np, "ulp-mux-sel-hs400", NULL))
		priv->ctrl_flag |= DW_MMC_EXYNOS_ULP_MUX_SEL_HS400;
	...
	case 0:
		...
		if (priv->ctrl_flag & DW_MMC_EXYNOS_ULP_MUX_SEL_HS400) {
			/* Rx Delay Line for ULP mode */
			of_property_read_u32(np,
				"samsung,dw-mshc-ulp-ddr200-delay-line",
					&priv->delay_line);

			/* Tx Delay Line for ULP mode */
			of_property_read_u32(np,
				"samsung,dw-mshc-ulp-ddr200-tx-delay-line",
					&priv->tx_delay_line);

			/* The fast RXCRC packet arrival time for ULP mode */
			of_property_read_u32(np,
				"samsung,dw-mshc-ulp-txdt-crc-timer-fastlimit",
					&priv->ddr200_tx_t_fastlimit);

			/* Initial value of the timeout down counter for
			 * RXCRC packet for ULP mode
			 */
			of_property_read_u32(np,
				"samsung,dw-mshc-ulp-txdt-crc-timer-initval",
					&priv->ddr200_tx_t_initval);
		} else {

			/* Rx Delay Line */
			of_property_read_u32(np,
				"samsung,dw-mshc-ddr200-delay-line",
					&priv->delay_line);

			/* Tx Delay Line */
			of_property_read_u32(np,
				"samsung,dw-mshc-ddr200-tx-delay-line",
					&priv->tx_delay_line);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (host->cur_slot && host->cur_slot->mmc && host->cur_slot->mmc->card) {
		if (!gpio_is_valid(priv->cd_gpio) && priv->sec_sd_slot_type > 0)
			goto gpio_error;
...
static int dw_mci_exynos_idle_control(struct dw_mci *host, u32 flag)
{
	int ret = 0;
	...
	case NOTIFY_IDLE:
		...
		break;
	case NOTIFY_BUSY:
		exynos_update_ip_idle_status(host->idle_status, STATUS_BUSY);
		break;
	}
#endif
	return ret;
		...
		enum dw_mci_misc_control control, void *priv)
{
	...
	case CTRL_SET_DEF_CAPS:
		...
		break;
	case CTRL_NOTIFY_IDLE:	/* Power mode change notifier */
		ret = dw_mci_exynos_idle_control(host, *((u32*)priv));
		break;
	default:
