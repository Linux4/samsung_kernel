...
static int dw_mci_card_int_hwacg_ctrl(struct dw_mci *host, bool mode)
{
	int ret = 0;
	u32 reg;
...
static void dw_mci_exynos_set_ios(struct dw_mci *host, struct mmc_ios *ios)
{
	...
	if (host->pdata->quirks & DW_MCI_QUIRK_HWACG_CTRL) {
		if (host->current_speed > 400*1000)
			dw_mci_card_int_hwacg_ctrl(host, false);
		else
			dw_mci_card_int_hwacg_ctrl(host, true);
	}
