...
#include <linux/proc_fs.h>//bug 760817,cjd.wt,Add sd_tray_gpio_value,2022/05/26
...
//bug 729041,huangxiaotian,modify for code scan,2022/01/28
	...
	if (on_off) {
		...
		if (bht_target_host(host)) {
			...
			{
				pr_err("%s: no det_gpio provided %d\n", mmc_hostname(host->mmc),__LINE__);
			}
			...
			if (gpio_is_valid(host->pwr_gpio)) {
				...
				pr_err("%s: pwr_gpio pin %d status is %d LINE %d\n", mmc_hostname(host->mmc), host->pwr_gpio, \
					gpio_get_value(host->pwr_gpio),__LINE__);
			}
			else
			{
				pr_err("%s: no pwr_gpio provided line is %d\n", mmc_hostname(host->mmc),__LINE__);
			}
		...
		if (bht_target_host(host)) {
			...
			{
				pr_err("%s: chenxu no det_gpio provided\n", mmc_hostname(host->mmc));
			}
			...
			{
				pr_err("%s: clear tuning result for power off and card removed\n", mmc_hostname(host->mmc));
			...
			if (gpio_is_valid(host->pwr_gpio)) {
				...
				pr_err("%s: pwr_gpio pin %d status is %d\n", mmc_hostname(host->mmc), host->pwr_gpio, \
...
#ifdef CONFIG_MMC_SDHCI_BH201
                if ((strcmp(host->device_name, "sdio_sd") != 0) && (ios->clock <= 400000 || ios->timing == MMC_TIMING_SD_HS ||
		...
		struct device_attribute *attr, char *buf)
{
	...
	// card_insert_or_not: SDcard Tray status (in: 1, out: 0)
	if (card_insert_or_not && card) {
		pr_err("SD card inserted.\n");
		return sprintf(buf, "Insert\n");
	} else if (card_insert_or_not && !card) {
...
static DEVICE_ATTR(status, 0444, sd_status_show, NULL);
		...
		struct sprd_sdhc_host *host)
{
...
#ifdef CONFIG_MMC_SDHCI_BH201
	host->det_gpio = host->detect_gpio;
#endif
	if (!gpio_is_valid(host->detect_gpio)) {
		...
		sd_gpio = host->detect_gpio;//bug 760817,cjd.wt,Add sd_tray_gpio_value,2022/05/26
	}
...
//bug 760817,cjd.wt,Add sd_tray_gpio_value,2022/05/26
	...
	if ((strcmp(host->device_name, "sdio_emmc") == 0)) {
		mmc_card_dev = sec_device_create(NULL, "mmc");
		...
		if (IS_ERR(mmc_card_dev)) {
			...
			if (device_create_file(mmc_card_dev,
				...
				dev_set_drvdata(mmc_card_dev, mmc);
		}
	}
	...
	if ((strcmp(host->device_name, "sdio_sd") == 0)) {
		...
		sd_card_dev = sec_device_create(NULL, "sdcard");

		if (IS_ERR(sd_card_dev))
			pr_err("%s : Failed to create device!\n", __func__);
		else {
			if (device_create_file(sd_card_dev, &dev_attr_status) < 0)
				pr_err("%s : Failed to create device file(%s)!\n",
						__func__, dev_attr_data.attr.name);

			dev_set_drvdata(sd_card_dev, mmc);
		}
	}
