...
#define SPRD_CMD_MASK	0x3F
#define SPRD_CMD_INDEX	24
...
static void sdhci_irq_dumpregs(struct sdhci_host *host)
{
	int cmd_index;
	cmd_index = (sdhci_readl(host, SDHCI_TRANSFER_MODE) >> SPRD_CMD_INDEX) & SPRD_CMD_MASK;
	printk("[%s]: CMD = %d, RESPONSE = 0x%08x\n",
			mmc_hostname(host->mmc), cmd_index, sdhci_readl(host, SDHCI_RESPONSE));
}
...
static void sdhci_cmd_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (host->cmd->error) {
		pr_err("[%s] %s error = %d, INT_ST =0x%08x\n",
				__func__, mmc_hostname(host->mmc),
				host->cmd->error, intmask);
		tasklet_schedule(&host->finish_tasklet);
		sdhci_irq_dumpregs(host);
		return;
	}
