...
void dw_mci_reg_dump(struct dw_mci *host)
{
	...
	dev_err(&host->dev, ": THRCTL:	0x%08x\n", mci_readl(host, DATA(0x100)));
	dev_err(&host->dev, ": DDRREG:	0x%08x\n", mci_readl(host, DATA(0x10C)));
	dev_err(&host->dev, ": EnSHIFT:	0x%08x\n", mci_readl(host, DATA(0x110)));
	dev_err(&host->dev, ": RDDQS:	0x%08x\n", mci_readl(host, DATA(0x180)));
	dev_err(&host->dev, ": AFIFO:	0x%08x\n", mci_readl(host, DATA(0x184)));
	dev_err(&host->dev, ": dline:	0x%08x\n", mci_readl(host, DATA(0x188)));
				   ...
				   struct mmc_command *cmd)
{
	...
	if (mrq->cmd->opcode == MMC_SEND_TUNING_BLOCK ||
		...
		mod_timer(&host->timer, jiffies + msecs_to_jiffies(300));
...
int dw_mci_suspend(struct dw_mci *host)
{
	...
	if (host->pdata->ch_num == 0 && host->pdata->cfg_gpio)
...
#if defined(CONFIG_MACH_UNIVERSAL4415)
...
#endif /* CONFIG_MACH_UNIVERSAL4415 */
