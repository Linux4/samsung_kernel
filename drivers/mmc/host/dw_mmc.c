				 ...
				 struct mmc_command *cmd, u32 cmd_flags)
{
	...
	if (host->quirks & DW_MCI_QUIRK_HWACG_CTRL) {
		if (drv_data && drv_data->hwacg_control)
			drv_data->hwacg_control(host, true);
	}
...
static void dw_mci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_OFF:
		if (!(slot->host->quirks & DW_MMC_QUIRK_FIXED_VOLTAGE)) {
			...
			if (host->quirks & DW_MCI_QUIRK_HWACG_CTRL) {
				if (drv_data && drv_data->hwacg_control)
					drv_data->hwacg_control(host, false);
			}
		}
		break;
	default:
...
static int dw_mci_cmdq_hwacg_control(struct mmc_host *mmc, bool set)
{
	int ret = 0;
	struct dw_mci_slot *slot = mmc_priv(mmc);
	struct dw_mci *host = slot->host;
	...
	if (set == true) {
		if (host->quirks & DW_MCI_QUIRK_HWACG_CTRL)
			if (drv_data && drv_data->hwacg_control)
				ret = drv_data->hwacg_control(host, true);
	} else {
		if (host->quirks & DW_MCI_QUIRK_HWACG_CTRL)
			if (drv_data && drv_data->hwacg_control)
				ret = drv_data->hwacg_control(host, false);
	}
	return ret;
}
