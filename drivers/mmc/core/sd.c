...
void bht_load_hw_inject(struct mmc_host* mmc_host, struct mmc_card* card, u32* cfg_data, int data_len, u32 sel200, u32 sel100)
{
	...
	//enable hw inject function
	gg_hw_inj[1] = 0x7364032;
	//setting SDR104 selb inject phase
	gg_hw_inj[11] = 0x77316200;
	//setting SDR50 selb inject phase
...
static void mmc_sd_detect(struct mmc_host *host)
{
...
#endif
#if IFLYTEK
        struct sprd_sdhc_host* vendor_host;
	...
	if (err) {
		...
		#if IFLYTEK
        	vendor_host = mmc_priv(host);
        	sdhost = sprd_priv(vendor_host);
...
int mmc_attach_sd(struct mmc_host *host)
{
	...
	retry : if (bht_target_host(mmc_priv(host))) {
		...
		while (retries) {
			...
			/* the card initialization successful */
			if(!err)
			{
				host->degrade = 0;
				pr_info("card init successfully!\n");
				break;
			}
			/* the card is failed at identify stage, it's bad card */
			if (host->cur_sd_bus_speed < 2) {
				host->degrade = 0;
				pr_err("%s: mmc_sd_init_card() failure (err = %d)\n",
					mmc_hostname(host), err);
				goto err;
			}/* the card is failed at tuning stage, need to retry */
			else {
				retries--;
		...
		/* the card is failed at tuning stage and retry time use up, need degrade */
		if (err) {
			host->degrade = 1;
