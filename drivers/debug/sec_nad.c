...
static ssize_t show_nad_support(struct device *dev,
				struct device_attribute *attr, char *buf)
{
#if defined(CONFIG_ARCH_MSM8998) || defined(CONFIG_ARCH_MSM8996) || defined(CONFIG_ARCH_SDM845) || defined(CONFIG_ARCH_SDM660) || defined(CONFIG_ARCH_SDM450)
				...
				const char *buf, size_t count)
{
	...
	if (TEST_CRYPTO(test_name))
		...
		item = QMESACACHE;
	else if (TEST_STORAGE(test_name))
		item = STORAGE;
...
{
	#if 0
		//QNAD_1.0.0_SS_29012018_SDM660 - initial version
		return snprintf(buf, BUFF_SZ, "S660.0101.01.Port\n");

  		//QNAD_1.0.0_SS_29012018_SDM660 - change distribution of time 
  		return snprintf(buf, BUFF_SZ, "S660.0101.02.DisT\n"); 

		//QNAD_1.0.1_SS_09022018_SDM660 - version up
		return snprintf(buf, BUFF_SZ, "S660.0102.03.VerUp\n"); 
	#else
		//QNAD_1.0.1_SS_09022018_SDM660 - version up
		return snprintf(buf, BUFF_SZ, "S660.0102.04.SR\n");
	#endif
}
