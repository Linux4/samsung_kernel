			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	failed_cnt = get_smd_item_result(param_quest_data, strResult, ITEM_CNT);
	if( failed_cnt==-1 ) {
	...
	// 2.2. in case of SMD, let's save result to param
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	// 2.1. The uefi_enhancement will make EnhanceLog.txt if it was passed. 
	// so, let's determin pass/fail with checking existence of EnhanceLog.txt
	{
		int fd = 0;
		mm_segment_t old_fs = get_fs();
		set_fs(KERNEL_DS);
		if ( (fd = sys_open(UEFI_ENHANCEMENT_RESULT_FAILE, O_RDONLY, 0))>=0 ) {
			QUEST_PRINT("%s : the EnhanceLog.txt exists (PASS)", __func__);
		}else {
			QUEST_PRINT("%s : no EnhanceLog.txt (FAIL)", __func__);
			uefi_result = TOTALTEST_FAIL;
		}
		set_fs(old_fs);
	}
