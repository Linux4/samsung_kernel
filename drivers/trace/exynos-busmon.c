...
static const struct list_head *timeout_list_p;
...
static irqreturn_t busmon_logging_irq(int irq, void *data)
{
	...
	if (bits) {
		...
		if (init_desc && !strncmp(init_desc, "MIF_CPU", strlen("MIF_CPU")))
			return IRQ_HANDLED;
		...
		if (init_desc && strncmp(init_desc, "P_CCI", strlen("P_CCI"))) {
			...
			} else if (!strncmp(init_desc, "CP", strlen("CP"))
				|| !strncmp(init_desc, "APB_CORE_CP", strlen("APB_CORE_CP"))
				|| !strncmp(init_desc, "MIF_CP", strlen("MIF_CP"))) {
				/* Don't raise panic here. waiting for CP crash */			
				bits = busmon_get_bits(pdata->faulten_bits, 0);
...
void busmon_timeout_pd_init(const char *pd_name)
{
	struct busmon_timeout *timeout;
	struct list_head *entry;
	u32 val;

	if (list_empty(timeout_list_p))
		return;

	list_for_each(entry, timeout_list_p) {
		timeout = list_entry(entry, struct busmon_timeout, list);
		if (timeout && timeout->runtime_pm && timeout->enabled &&
				!strncmp(pd_name, timeout->pd_name, strlen(pd_name))) {
			val = __raw_readl(timeout->regs);
			val |= (0x1) << timeout->enable_bit;
			__raw_writel(val, timeout->regs);

			pr_debug("%s: Exynos Bus Monitor timeout enabled(%s, bit:%d)\n",
				pd_name, timeout->name, timeout->enable_bit);
		}
	}
}
...
static void busmon_timeout_init(struct busmon_dev *busmon)
{
	...
	list_for_each(entry, &busmon->pdata->timeout_list) {
		timeout = list_entry(entry, struct busmon_timeout, list);
		if (timeout && timeout->enabled && !timeout->runtime_pm) {
			val = __raw_readl(timeout->regs);
			val |= (0x1) << timeout->enable_bit;
			__raw_writel(val, timeout->regs);
...
static void busmon_logging_init(struct busmon_dev *busmon)
{
	...
	dev_info(busmon->dev, "Exynos BUS Monitor logging %s\n",
				...
				struct busmon_dev *busmon)
{
	...
	while ((time_child_np = of_get_next_child(time_np, time_child_np)) != NULL) {
		...
		of_property_read_string(time_child_np, "nickname",
		...
		of_property_read_u32(time_child_np, "runtime-pm",
					&timeout->runtime_pm);
		if (timeout->runtime_pm)
			of_property_read_string(time_child_np, "pd-name",
						(const char **)&timeout->pd_name);
		list_add(&timeout->list, &pdata->timeout_list);
	}
	timeout_list_p = &pdata->timeout_list;
	of_node_put(time_np);
out:
	return ret;
}
...
static int busmon_probe(struct platform_device *pdev)
{
	...
	busmon->regs = devm_request_and_ioremap(&pdev->dev, res);
	if (busmon->regs == NULL) {
