...
#ifndef CONFIG_EXYNOS_SNAPSHOT_WITHOUT_KEVENT
	struct task_log {
		unsigned long long time;
		unsigned long sp;
...
#else
	{"log_kevents",	{SZ_32K,	0, 0, false, true, true}, NULL, NULL, 0},
#endif
#ifndef CONFIG_EXYNOS_SNAPSHOT_WITHOUT_KERNEL_LOG
#ifdef CONFIG_EXYNOS_SNAPSHOT_KERNEL_LOG_480K
	{"log_kernel",	{SZ_1K * 480,	0, 0, false, true, true}, NULL, NULL, 0},
#else
	{"log_kernel",	{SZ_2M,		0, 0, false, true, true}, NULL, NULL, 0},
#endif
#endif
...
static struct exynos_ss_base ess_base_sparsed;
static struct exynos_ss_log_idx __maybe_unused ess_idx;
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(3,5,00)
static inline void __maybe_unused exynos_ss_hook_logbuf(const char buf)
{
...
#else
static inline void __maybe_unused exynos_ss_hook_logbuf(const char *buf, size_t size)
{
				...
				char *line, size_t size, size_t *len)
{
	...
	unsigned long __maybe_unused rem_nsec;
	u64 __maybe_unused ts;
...
#endif /* CONFIG_EXYNOS_SNAPSHOT_WITHOUT_KEVENT */
#ifdef CONFIG_EXYNOS_SNAPSHOT_IRQ_EXIT
...
static int __init exynos_ss_vm_remap(struct vm_struct *vm)
{
	struct page *page;
	struct page **pages;
	unsigned long i;
	...
	page_size = vm->size / PAGE_SIZE;
	pages = kzalloc(sizeof(struct page*) * page_size, GFP_KERNEL);
	page = phys_to_page(vm->phys_addr);

	for (i = 0; i < page_size; i++)
		pages[i] = page++;

	ret = map_vm_area(vm, prot, pages);
	kfree(pages);
...
static size_t __init exynos_ss_remap(void)
{
	...
	if (!!ess_desc.vm_sparsed.size)
		ret = !!exynos_ss_vm_remap(&ess_desc.vm) ||
			!!exynos_ss_vm_remap(&ess_desc.vm_sparsed);
	else
		ret = exynos_ss_vm_remap(&ess_desc.vm);
	...
	for (i = 0; i < ARRAY_SIZE(ess_items); i++) {
		...
		if (i == ess_desc.kevents_num ||
			...
			/* For Next */
			if ((i == ess_desc.kevents_num) && !!ess_desc.vm_sparsed.size) {
				pre_vaddr = (size_t)ess_base_sparsed.vaddr;
				pre_paddr = (size_t)ess_base_sparsed.paddr;
			} else {
...
#define exynos_ss_reserve_mem_kernel_api(base, size) \
			(memblock_is_region_reserved(base, size) ? \
			-1 : memblock_reserve(base, size))
#else
#define exynos_ss_reserve_mem_kernel_api(base, size) reserve_bootmem(base, size, BOOTMEM_EXCLUSIVE)
#endif

#ifdef CONFIG_EXYNOS_SNAPSHOT_MINIMIZED_MODE
#define exynos_ss_reserve_mem(base) exynos_ss_reserve_mem_minimized(base)
#else
#define exynos_ss_reserve_mem(base) exynos_ss_reserve_mem_normal(base)
#endif

static size_t __init __maybe_unused exynos_ss_reserve_mem_minimized(size_t base)
{
	unsigned long i;
	size_t base_sparsed = base + SZ_8M;
	size_t size = 0;
	size_t size_sparsed = 0;

	size = ess_items[ess_desc.kevents_num].entry.size;
	for (i = ess_desc.kevents_num + 1; i < ARRAY_SIZE(ess_items); i++) {
		if (ess_items[i].entry.enabled_init)
			size_sparsed += ess_items[i].entry.size;
	}

	if (!exynos_ss_reserve_mem_kernel_api(base, size)) {
		ess_base.paddr = base;
		...
		pr_err("exynos-snapshot: fail to reserve : 0x%zx, 0x%zx\n", base, size);
		return 0;
	}

	if (!size_sparsed)
		return ess_base.size;

	if (!exynos_ss_reserve_mem_kernel_api(base_sparsed, size_sparsed)) {
		ess_base_sparsed.paddr = base_sparsed;
		ess_base_sparsed.vaddr = (size_t)(ESS_FIXED_VIRT_BASE + SZ_8M);
		ess_base_sparsed.size = size_sparsed;
		ess_base_sparsed.enabled = false;

		/* Reserved fixed virtual memory within VMALLOC region */
		ess_desc.vm_sparsed.phys_addr = base_sparsed;
		ess_desc.vm_sparsed.addr = (void *)(ESS_FIXED_VIRT_BASE + SZ_8M);
		ess_desc.vm_sparsed.size = size_sparsed;
		ess_desc.vm_sparsed.flags = VM_NO_GUARD;

		vm_area_add_early(&ess_desc.vm_sparsed);

		pr_info("exynos-snapshot: mem sparsed reserved complete : 0x%zx, 0x%zx, 0x%zx\n",
			base_sparsed, (size_t)(ESS_FIXED_VIRT_BASE + SZ_8M), size_sparsed);
	} else {
		pr_err("exynos-snapshot: fail to reserve : 0x%zx, 0x%zx\n",
								base_sparsed, size_sparsed);
		return 0;
	}

	return ess_base.size + ess_base_sparsed.size;
}

static size_t __init __maybe_unused exynos_ss_reserve_mem_normal(size_t base)
{
	unsigned long i;
	size_t size = 0;
...
static int __init exynos_ss_setup(char *str)
{
	...
	exynos_ss_init_desc();
	size = exynos_ss_reserve_mem(base);
