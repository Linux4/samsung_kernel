...
/* time in seconds when SODI3 began for persistent clock */
static struct timespec64 tk_soidle3_time;
...
int mtk_timer_compensate_pre_handler(void)
{
	struct timespec64 delta, delta_delta;
	static struct timespec64 old_delta;
	static struct timespec64 old_system;

	read_persistent_clock64(&tk_soidle3_time);
	if (!tk_soidle3_time.tv_sec && !tk_soidle3_time.tv_nsec) {
		pr_notice("read_persistent_clock is not registered\n");
		return -ENODEV;
	}

	getnstimeofday64(&old_system); /* CLOCK_REALTIME */
	delta = timespec64_sub(old_system, tk_soidle3_time);
	delta_delta = timespec64_sub(delta, old_delta);
	if (abs(delta_delta.tv_sec) >= 2)
		old_delta = delta;
	else
		tk_soidle3_time = timespec64_add(tk_soidle3_time, delta_delta);

	return 0;
}
EXPORT_SYMBOL(mtk_timer_compensate_pre_handler);

int mtk_timer_compensate_post_handler(void)
{
	struct timespec64 new_system;
	struct timespec64 ts_new, ts_delta;
	bool sleeptime_injected;

	sleeptime_injected = false;
	read_persistent_clock64(&ts_new);
	/* snapshot the current rtc and system time after SODI3 */
	getnstimeofday64(&new_system);

	if (timespec64_compare(&ts_new, &tk_soidle3_time) > 0) {
		ts_delta = timespec64_sub(ts_new, tk_soidle3_time);
		sleeptime_injected = true;
	}

	if (sleeptime_injected)
		timekeeping_inject_offset(&ts_delta);

	return 0;
}
EXPORT_SYMBOL(mtk_timer_compensate_post_handler);
