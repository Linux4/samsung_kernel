...
struct t_ib_info* create_ib_instance(struct t_ib_trigger* p_IbTrigger, int uniqId)
{
	...
	for (i=0; i<MAX_DEVICE_TYPE_NUM; i++) {
		if (ib_device_trees[i].type == dev_type)
			ib->ib_dt = &ib_device_trees[i];
	}
...
int parse_dtsi_str(struct device_node *np, const char *target_node, void *target_arr, int isIntType)
{
	...
	while (token != NULL) {
		pr_info(ITAG" %s %d's Type Value(%s)", target_node, iter, token);
		...
		if (isIntType) {
			...
			pr_info(ITAG" Target_arr[%d] : %d", iter, int_target_arr_ptr[iter]);
		} else {
...
//Init Resource Release Values
	...
	for_each_child_of_node(np, cnp) {
		...
		for_each_child_of_node(resource_node, child_resource_node) {
			...
			/*
			if (strcmp(ib_dt->res[allowed_resources[resource_node_index]].label, "CPU") == 0) {
				ib_dt->res[allowed_resources[resource_node_index]].res_id = CPUFREQ;
			} else if (strcmp(ib_dt->res[allowed_resources[resource_node_index]].label, "DDR") == 0) {
				ib_dt->res[allowed_resources[resource_node_index]].res_id = DDRFREQ;
			}*/
	...
	// ********** Initialize Sysfs **********
	{
		...
		if (ib_init_succeed) {
			...
			for (ib_type = 0; ib_type < MAX_DEVICE_TYPE_NUM; ib_type++) {
				pr_info(ITAG" Label(%s) ibType(%d) Head_Val(%d) Tail_Val(%d)",
					ib_device_trees[ib_type].label, ib_device_trees[ib_type].type, ib_device_trees[ib_type].head_time, ib_device_trees[ib_type].tail_time);
