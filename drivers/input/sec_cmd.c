...
{
	char delim = ':';
	memset(data->cmd_result, 0x00, data->cmd_buffer_size);
	memcpy(data->cmd_result, data->cmd, strnlen(data->cmd, SEC_CMD_STR_LEN));
				 ...
				 struct device_attribute *devattr, char *buf)
{
	...
	size = snprintf(buf, SEC_CMD_BUF_SIZE, "%s\n", data->cmd_result);
			...
			int len, int devt)
{
	...
	data->cmd_result = kzalloc(data->cmd_buffer_size, GFP_KERNEL);
	if (!data->cmd_result) {
		pr_err("%s: failed to allocate cmd_result\n", __func__);
