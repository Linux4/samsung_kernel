...
#define STM32_MASTER_APPSS_PROC		1
#define STM32_SLAVE			512
#define STM32_AB_DATA			0
#define STM32_IB_MIN			0
#define STM32_IB_MAX			4068000
#endif
					...
					char *data, int len)
{
	...
	send_data.firm_state = stm32->firm_state;
	stm32->firm_state = false;
...
static void stm32_get_fw_info_of_bin(struct stm32_dev *stm32)
{
	...
	memcpy(stm32->fw_header, &stm32->fw->data[stm32->dtdata->magic_word_offset],
		...
		struct stm32_dev *device_data)
{
	...
	ret = of_property_read_string(np, "stm32,fota_fw_path", &device_data->dtdata->fota_fw_path);
	if (ret < 0) {
	...
	if (of_property_read_u32(np, "stm32,magic_word_offset", &device_data->dtdata->magic_word_offset) < 0) {
		input_err(false, dev, "%s: Failed to get magic-word-offset property\n", __func__);
		device_data->dtdata->magic_word_offset = STM32_FW_APP_CFG_OFFSET;
	}

	device_data->dtdata->support_open_close = of_property_read_bool(np, "support_open_close");

	input_info(true, dev, "%s: scl: %d, sda: %d, int:%d, conn: %d, mcu_swclk: %d, mcu_nrst: %d model_name1:%s model_name2:%s model_name3:%s mcu_fw_name:%s i2c-burstmax:%d count:%d "
					"magic_word_offset:0x%x, open_close:%d\n",
			...
			device_data->dtdata->model_name[0], device_data->dtdata->model_name[1], device_data->dtdata->model_name[2], 
			device_data->dtdata->mcu_fw_name, device_data->dtdata->i2c_burstmax, count, device_data->dtdata->magic_word_offset, device_data->dtdata->support_open_close);
	return 0;
}
#else
static int stm32_parse_dt(struct device *dev,
...
static void stm32_keyboard_connect(struct stm32_dev *data)
{
	...
	if (!data->resume_done.done)
		__pm_wakeup_event(data->stm32_ws, 3 * MSEC_PER_SEC);
#endif
		...
		const struct i2c_device_id *id)
{
	...
	device_data->client_boot = i2c_new_dummy_device(client->adapter, boot_addr); 
#else
	...
	device_data->stm32_ws = wakeup_source_register(&client->dev, "stm_key wake lock");
...
#elif IS_ENABLED(CONFIG_INTERCONNECT)
	device_data->stm32_icc_data = icc_get(&client->dev, STM32_MASTER_APPSS_PROC, STM32_SLAVE);
	if (IS_ERR_OR_NULL(device_data->stm32_icc_data)) {
		input_err(true, &client->dev, "%s icc_get_failed\n", __func__);
	} else {
		device_data->stm32_register_ddr = 1;
	}
#endif
...
#elif IS_ENABLED(CONFIG_INTERCONNECT)
	if (device_data->stm32_register_ddr)
		INIT_DELAYED_WORK(&device_data->bus_voting_work, stm32_bus_voting_work);
...
#if IS_ENABLED(CONFIG_INPUT_SEC_NOTIFIER)
	sec_input_register_notify(&device_data->keyboard_input_nb, stm32_keyboard_notify_call, 1);
#endif
...
#elif IS_ENABLED(CONFIG_INTERCONNECT)
	if (device_data->stm32_register_ddr)
		icc_put(device_data->stm32_icc_data);
#endif
interrupt_err:
err_firmware_update:
...
err_config:
	...
	wakeup_source_unregister(device_data->stm32_ws);
