...
static int stm32_dev_calchecksum(unsigned char *data, size_t size);
...
static int stm32_dev_firmware_update(struct stm32_dev *data, const u8 *fw_data, size_t size, int retry)
{
	...
	u8 readdata[STM32_DEV_FW_UPDATE_PACKET_SIZE] = "";
	u8 *tmp_data = NULL;
	u8 *ic_data = NULL;
	...
	/* verify checkSum */
	checksum_bin = stm32_dev_calchecksum(tmp_data, size);
	...
	checksum_ic = stm32_dev_calchecksum(ic_data, size);
...
//	int ret;
	...
	if (data->mdata.phone_ver[3] > data->mdata.ic_ver[3])
		...
		return 1;
	else if ((data->mdata.phone_ver[3] != 0xff) && (data->mdata.ic_ver[3] >= 0xA0))
		return 1;
...
static int stm32_dev_checksum(struct stm32_dev *data, const u8 *fw_data, size_t size)
{
	...
	/* verify checkSum */
	checksum_bin = stm32_dev_calchecksum(&tmp_data2[stm32_memory_map.pages->size  * last_sector_number], check_size);
	...
	checksum_ic = stm32_dev_calchecksum(tmp_data, check_size);
...
static int stm32_dev_firmware_update_on_probe(struct stm32_dev *data)
{
	...
	ret = stm32_dev_checksum(data, fw_entry->data, fw_entry->size);
	if (ret < 0) {
		force_firm = true;
