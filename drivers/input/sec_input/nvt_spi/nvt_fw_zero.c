...
static int32_t nvt_get_fw_need_write_size(struct nvt_ts_data *ts, struct nvt_firmware *fw_entry)
{
	int32_t i = 0;
...
return:
	n.a.
*******************************************************/
static int32_t nvt_download_init(struct nvt_ts_data *ts)
{
...
static int32_t nvt_bin_header_parser(struct nvt_ts_data *ts, u8 *fwdata, size_t fwsize)
{
	uint32_t list = 0;
	uint32_t pos = 0x00;
	uint32_t end = 0x00;
...
*******************************************************/
static int32_t update_firmware_request(struct nvt_ts_data *ts, const char *filename)
{
	uint8_t retry = 0;
	int32_t ret = 0;
	...
	while (1) {
		...
		if (nvt_get_fw_need_write_size(ts, ts->cur_fw)) {
		...
		/* BIN Header Parser */
		ret = nvt_bin_header_parser(ts, ts->cur_fw->data, ts->cur_fw->size);
		if (ret) {
			input_err(true, &ts->client->dev,"bin header parser failed\n");
			goto invalid;
		} else {
			ts->fw_ver_bin[0] = ts->cur_fw->data[FW_BIN_IC_ID];
			ts->fw_ver_bin[1] = ts->cur_fw->data[FW_BIN_PROJECT_ID];
			ts->fw_ver_bin[2] = ts->cur_fw->data[FW_BIN_PANEL_ID];
			ts->fw_ver_bin[3] = ts->cur_fw->data[FW_BIN_VER_OFFSET];
			ts->fw_ver_bin_bar = ts->cur_fw->data[FW_BIN_VER_BAR_OFFSET];

			input_dbg(false, &ts->client->dev, "%s: fw_ver_bin = %02X%02X%02X%02X\n", __func__,
...
*******************************************************/
static int32_t nvt_write_sram(struct nvt_ts_data *ts, const u8 *fwdata,
		uint32_t SRAM_addr, uint32_t size, uint32_t BIN_addr)
{
	...
	for (i = 0 ; i < count ; i++) {
		...
		ret = ts->nvt_ts_write(ts, fwbuf, 1, &fwbuf[1], len);
		if (ret) {
...
return:
	n.a.
*******************************************************/
static int32_t nvt_write_firmware(struct nvt_ts_data *ts, const u8 *fwdata, size_t fwsize)
{
	uint32_t list = 0;
	char *name;
...
return:
	n.a.
*******************************************************/
static int32_t nvt_check_fw_checksum(struct nvt_ts_data *ts)
{
	...
	ret = ts->nvt_ts_read(ts, &fwbuf[0], 1, &fwbuf[1], len);
	if (ret) {
...
return:
	n.a.
*******************************************************/
static void nvt_set_bld_crc_bank(struct nvt_ts_data *ts, uint32_t DES_ADDR, uint32_t SRAM_ADDR,
		...
		uint32_t G_CHECKSUM_ADDR, uint32_t crc)
{
	...
	ts->nvt_ts_write(ts, fwbuf, 1, &fwbuf[1], 3);
	...
	if (ts->hw_crc == 1) {
		ts->nvt_ts_write(ts, fwbuf, 1, &fwbuf[1], 2);
	} else if (ts->hw_crc > 1) {
		ts->nvt_ts_write(ts, fwbuf, 1, &fwbuf[1], 3);
	}
	...
	fwbuf[3] = (crc >> 16) & 0xFF;
	fwbuf[4] = (crc >> 24) & 0xFF;
	ts->nvt_ts_write(ts, fwbuf, 1, &fwbuf[1], 4);;
...
void nvt_bld_crc_enable(struct nvt_ts_data *ts)
{
	...
	buf[1] = 0xFF;
	ts->nvt_ts_read(ts, buf, 1, &buf[1], 1);
...
void nvt_boot_ready(struct nvt_ts_data *ts)
{
	...
	nvt_ts_write_addr(ts, ts->mmap->BOOT_RDY_ADDR, 1);

	sec_delay(5);

	if (!ts->hw_crc) {
		//---write BOOT_RDY status cmds---
		nvt_ts_write_addr(ts, ts->mmap->BOOT_RDY_ADDR, 0);

		//---write POR_CD cmds---
		nvt_ts_write_addr(ts, ts->mmap->POR_CD_ADDR, 0xA0);
	}
}
...
static void nvt_read_bld_hw_crc(struct nvt_ts_data *ts)
{
	...
	buf[3] = 0x00;
	buf[4] = 0x00;
	ts->nvt_ts_read(ts, &buf[0], 1, &buf[1], 4);
...
return:
	Executive outcomes. 0---succeed. else---fail.
*******************************************************/
static int32_t nvt_download_firmware_hw_crc(struct nvt_ts_data *ts)
{
	uint8_t retry = 0;
	int32_t ret = 0;
	...
	while (1) {
		...
		ret = nvt_write_firmware(ts, ts->cur_fw->data, ts->cur_fw->size);
		if (ret) {
...
return:
	n.a.
*******************************************************/
static int32_t nvt_download_firmware(struct nvt_ts_data *ts)
{
	uint8_t retry = 0;
	int32_t ret = 0;
	...
	while (1) {
		...
		/* clear fw reset status */
		nvt_ts_write_addr(ts, ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_RESET_COMPLETE, 0x00);
...
int nvt_ts_fw_update_from_bin(struct nvt_ts_data *ts)
{
	int ret = 0;
	mutex_lock(&ts->lock);
	ret = nvt_update_firmware(ts, ts->plat_data->firmware_name);
	mutex_unlock(&ts->lock);
...
int nvt_ts_fw_update_from_mp_bin(struct nvt_ts_data *ts, bool is_start)
{
	...
	if (is_start) {
		...
		ret = nvt_update_firmware(ts, ts->nplat_data->firmware_name_mp);
...
int nvt_ts_fw_update_on_probe(struct nvt_ts_data *ts)
{
	...
	if(of_property_read_bool(ts->client->dev.of_node, "novatek,mp-support-dt")) {
		...
		ts->nvt_ts_read(ts, &mpcriteria[0], 1, &mpcriteria[1], 2);
