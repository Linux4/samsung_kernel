...
int synaptics_ts_start_device(void *data)
{
	...
	ret = ts->synaptics_ts_get_baredata(ts, buf, 4);
	if (ret < 0) {
...
int synaptics_ts_init(struct synaptics_ts_data *ts)
{
	...
	device_init_wakeup(ts->sec.fac_dev, true);
...
int synaptics_ts_probe(struct synaptics_ts_data *ts)
{
	int ret = 0;
	...
	ret = request_threaded_irq(ts->irq, NULL, synaptics_ts_irq_thread,
