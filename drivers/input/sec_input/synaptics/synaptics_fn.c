	...
	unsigned int delay_ms_resp)
{
	...
	if (command == CMD_READ_SEC_SPONGE_REG)
...
int synaptics_ts_set_up_rawdata_report_type(struct synaptics_ts_data *ts)
{
	...
	if (ts->raw_mode) {
		retval = synaptics_ts_enable_report(ts,
				REPORT_SEC_COORDINATE_RAWDATA_EVENT, true);
		if (retval < 0) {
			input_err(true, ts->dev, "%s: Fail to enable sec coordinate and rawdata report\n", __func__);
			return -EIO;
		}
		retval = synaptics_ts_set_dynamic_config(ts, DC_ENABLE_EXTENDED_TOUCH_REPORT_CONTROL,
			0x03);
		if (retval < 0) {
			input_err(true, ts->dev, "%s: Fail to enable dynamic config\n", __func__);
			return -EIO;
		}
		retval = synaptics_ts_enable_report(ts,
				REPORT_SEC_COORDINATE_EVENT, false);
		if (retval < 0) {
		...
		retval = synaptics_ts_enable_report(ts,
				REPORT_SEC_COORDINATE_RAWDATA_EVENT, false);
		if (retval < 0) {
			input_err(true, ts->dev, "%s: Fail to enable sec coordinate and rawdata report\n", __func__);
			return -EIO;
		}
		...
		unsigned int size)
{
	...
	if ((!data) || (size < SYNAPTICS_TS_MESSAGE_HEADER_SIZE)) {
		input_info(true, ts->dev, "%s:Invalid parameters\n", __func__);
		return -EINVAL;
	}
	...
	if (header->code == REPORT_IDENTIFY) {
		...
		if (retval < 0) {
			...
			retval = synaptics_ts_v1_write_message(ts,
					...
					RESET_DELAY_MS);
			if (retval < 0) {
				input_info(true, ts->dev, "%s: Fail to identify the device\n", __func__);
				return -ENODEV;
			}
		}
...
int synaptics_ts_setup(struct synaptics_ts_data *ts)
{
	...
	ret = ts->synaptics_ts_get_baredata(ts, data, 4);
	if (ret < 0) {
		input_err(true, ts->dev, "%s: failed to get bare data\n", __func__);
		goto out;
	}
...
int synaptics_ts_write_grip_data(struct synaptics_ts_data *ts, unsigned char *payload, int payload_size)
{
	...
	ret = synaptics_ts_send_command(ts,
		...
		FORCE_ATTN_DRIVEN);
	if (ret < 0)
		input_err(true, ts->dev, "%s: Fail to write grip data, %d", __func__, ret);
