/* drivers/input/sec_input/melfas/melfas_dump.c
 *
...
{
	struct melfas_ts_data *ts = container_of(work, struct melfas_ts_data, check_rawdata.work);
	...
	melfas_ts_run_rawdata(ts, false);
}
...
void melfas_ts_dump_tsp_log(struct device *dev)
{
	...
	pr_info("%s: %s %s: start\n", MELFAS_TS_I2C_NAME, SECLOG, __func__);
...
void melfas_ts_sponge_dump_flush(struct melfas_ts_data *ts, int dump_area)
{
	...
	ret = ts->melfas_ts_read_sponge(ts, sec_spg_dat, ts->sponge_dump_event * ts->sponge_dump_format);
	if (ret < 0) {
...
EXPORT_SYMBOL(melfas_ts_sponge_dump_flush);
#endif
