...
int ff_ctl_free_pins(void)
{
	...
	{
		regulator_disable(g_context->ff_vdd);
...
int ff_ctl_enable_power(bool on)
{
	...
	if (on)
	{
		...
		if (g_context->ff_vdd && regulator_is_enabled(g_context->ff_vdd) > 0) 
		{
			err = regulator_disable(g_context->ff_vdd);
			if (err < 0)
			{
				FF_LOGE("Can't disable voltage");
				return err;
			}
		}
	}
