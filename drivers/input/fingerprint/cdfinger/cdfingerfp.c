...
#endif
#ifdef POWER_REGULATOR
    ret = regulator_set_load(pdata->vdd,10000);
	if(ret)
	{
		CDFINGER_ERR("load current fail\n");
		return ret;
	}
...
{
    int i;
    pdata->cdfinger_input = input_allocate_device();
    if(!pdata->cdfinger_input) {
        CDFINGER_ERR("cdfinger->cdfinger_inputdev create faile!\n");
        return -ENOMEM;
    }
    for (i = 0; i < ARRAY_SIZE(maps); i++)
        input_set_capability(pdata->cdfinger_input, maps[i].type, maps[i].code);
    pdata->cdfinger_input->id.bustype = BUS_HOST;
    pdata->cdfinger_input->name=INPUT_DEVICE_NAME;
    if(input_register_device(pdata->cdfinger_input))
    {
        CDFINGER_ERR("register inputdev failed\n");
        input_free_device(pdata->cdfinger_input);
        pdata->cdfinger_input = NULL;
        return -1;
    }
	return 0;
}
static int cdfinger_init_irq(struct cdfingerfp_data *pdata)
{
	...
	{
		error = cdfinger_create_inputdev(pdata);
		if (error < 0)
		{
		    CDFINGER_ERR("cdfinger create inputdev err\n");
		    return error;
		}
	}
	...
	switch (cmd) {
		...
		case CDFINGER_RELEASE_DEVICE:
			...
			if(1 == isInit) {
				...
				input_free_device(cdfinger->cdfinger_input);
				cdfinger->cdfinger_input = NULL;
			}
			isInit = 0;
...
static int cdfinger_support_id(struct cdfingerfp_data *cdfinger)
{
	...
	id_value = gpio_get_value(cdfinger->id_num);
...
static int cdfinger_probe(struct platform_device *pdev)
{
	...
	if (status < 0) {
		CDFINGER_ERR("%s: fb_register_client failed, returned with status=%d\n",
                     __func__, status);
...
static int cdfinger_remove(struct platform_device *pdev)
{
    ...
    cdfinger_power_off(cdfingerdev);
