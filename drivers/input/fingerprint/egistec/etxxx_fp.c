...
#include <linux/fb.h>
/* HS03S code added for P210622-02293 by wurui at 20210624 start */
#include <linux/wakelock.h>
/* HS03S code added for P210622-02293 by wurui at 20210624 end */
#include "etxxx_fp.h"
...
irqreturn_t fp_eint_func(int irq, void *dev_id)
{
	...
	/* HS03S code added for P210622-02293 by wurui at 20210624 start */
	wake_lock_timeout(&egis_dev->wakeup_lock_fp, 2000);
	/* HS03S code added for P210622-02293 by wurui at 20210624 end */
	return IRQ_HANDLED;
}
...
{
	gpio_set_value(egis_dev->rstPin, 0);
	mdelay(Rst_off_delay);
	gpio_set_value(egis_dev->rstPin, 1);
	mdelay(Rst_on_delay);
...
int do_egisfp_power_onoff(struct egisfp_dev_t *egis_dev, struct egisfp_ioctl_cmd_t *ioctl_data)
{
	...
	if (power_onoff)
	{
		if (egis_dev->pwr_by_gpio)
			gpio_set_value(egis_dev->vcc_33v_Pin, 1);
		else
	...
	{
		if (egis_dev->pwr_by_gpio)
			gpio_set_value(egis_dev->vcc_33v_Pin, 0);
		else
...
long egisfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FP_WAKELOCK_ENABLE:
		...
		/* HS03S code added for P210622-02293 by wurui at 20210624 start */
		wake_lock(&egis_dev->wakeup_lock_fp);
 		/* HS03S code added for P210622-02293 by wurui at 20210624 end */
		break;
	case FP_WAKELOCK_DISABLE:
		DEBUG_PRINT(" %s : FP_WAKELOCK_DISABLE \n", __func__);
		wake_unlock(&egis_dev->wakeup_lock_fp);
		break;
	case GET_IO_STUS:
...
/* HS03s code added for SR-AL5625-01-201 by wurui at 20210521 start */
...
/* HS03s code added for SR-AL5625-01-201 by wurui at 20210521 end */
...
int egisfp_remove(struct platform_device *pdev)
{
	...
	/* HS03S code added for P210622-02293 by wurui at 20210624 start */
	wake_lock_destroy(&egis_dev->wakeup_lock_fp);
	/* HS03S code added for P210622-02293 by wurui at 20210624 end */
...
/* Initialize the driver data */
	...
	/* HS03S code added for P210622-02293 by wurui at 20210624 start */
	wake_lock_init(&egis_dev->wakeup_lock_fp, WAKE_LOCK_SUSPEND, "egisfp wakelock");
	/* HS03S code added for P210622-02293 by wurui at 20210624 end */
