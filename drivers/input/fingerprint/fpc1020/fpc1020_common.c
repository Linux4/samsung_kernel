...
int fpc1020_gpio_reset(fpc1020_data_t *fpc1020)
{
	...
	while (counter) {
		...
		udelay(FPC1020_RESET_HIGH1_US);
 		...
 		udelay(FPC1020_RESET_LOW_US + 10);
...
int fpc1020_reset(fpc1020_data_t *fpc1020)
{
	...
	if(fpc1020->spi->master->bus_num != 1){
...
int fpc1020_wait_for_irq(fpc1020_data_t *fpc1020, int timeout)
{
	...
	if (result < 0) {
	printk(KERN_ERR "[SPRD_FINGERPRINT_ERR] wait_event_interruptible interrupted by signal.\n");
...
#if 1//for debug only
	printk(KERN_INFO 
		"[SPRD_FINGERPRINT_INFO] %s %s 0x%x/%dd (%d bytes) %x %x %x %x : %x %x %x %x\n",
		 __func__,
		      ...
		      fpc1020_reg_access_t *reg_data)
{
	...
	u8 temp_buffer[FPC1020_REG_MAX_SIZE] = {0};
	...
	if (reg_data->reg_size > sizeof(temp_buffer)) {
		printk(KERN_ERR 
			"[SPRD_FINGERPRINT_ERR] %s: illegal register size\n",
			__func__);
	...
	if (error)
    	printk(KERN_ERR "[SPRD_FINGERPRINT_ERR] %s: spi_sync failed.\n", __func__);
	...
	if (!reg_data->write) {
		...
		printk(KERN_INFO "[SPRD_FINGERPRINT_INFO] %s: target little endian\n",__func__);
		...
		printk(KERN_INFO "[SPRD_FINGERPRINT_INFO] %s: target big endian\n",__func__);
			memcpy(reg_data->dataptr,
...
#ifdef DEBUG_LOG
	printk(KERN_INFO 
		"[SPRD_FINGERPRINT_INFO] %s %s 0x%x/%dd (%d bytes) %x %x %x %x : %x %x %x %x..rx_len=%d\n",
		 __func__,
		(reg_data->write) ? "WRITE" : "READ",
		reg_data->reg,
		reg_data->reg,
				...
				size_t buff_size)
{
	...
	printk(KERN_INFO "[SPRD_FINGERPRINT_INFO] func: %s line: %d\n", __func__,__LINE__);
	printk(KERN_INFO "[SPRD_FINGERPRINT_INFO] image_size_bytes = %d, buff_size = %d\n",
			(int)image_size_bytes,(int)buff_size);

	if ((offset + (int)image_size_bytes +2) > buff_size) {
	...
	if (!error) {
		...
		printk(KERN_INFO "[SPRD_FINGERPRINT_INFO] \
			%s buffer length %d, %x %x %x %x: %x %x %x %x\n ",
			__func__, length+rest_length,
			buffer[0], buffer[1], buffer[2], buffer[3],
			buffer[image_size_bytes-4], buffer[image_size_bytes-3],
			buffer[image_size_bytes-2], buffer[image_size_bytes-1]);
...
static int fpc1020_set_finger_drive(fpc1020_data_t *fpc1020, bool enable)
{
	...
	printk(KERN_INFO "[SPRD_FINGERPRINT_INFO] func: %s line: %d\n", __func__,__LINE__);
	printk(KERN_INFO "[SPRD_FINGERPRINT_INFO] %s: %s\n", __func__, (enable)? "ON":"OFF");
...
int fpc1020_calc_finger_detect_threshold_min(fpc1020_data_t *fpc1020)
{
	...
	printk(KERN_INFO "[SPRD_FINGERPRINT_INFO] %s : %s %d\n",
			__func__,
