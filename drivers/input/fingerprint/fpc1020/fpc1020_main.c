...
static int fpc1020_probe(struct spi_device *spi)
{
	...
	if(spi->master->bus_num != 1){
		printk(KERN_INFO "[SPRD_FINGERPRINT_INFO] %s: spi bus num != 1\n", __func__);
		return -EINVAL;
	}
	printk(KERN_INFO "[SPRD_FINGERPRINT_INFO] %s: spi bus num == 1\n", __func__);
	...
	fpc1020->reset_gpio = FINGERPRINT_FPC1020_RESET;
	fpc1020->irq_gpio   = FINGERPRINT_FPC1020_IRQ;
	fpc1020->cs_gpio    = FINGERPRINT_FPC1020_CS;
...
*/
	...
	printk(KERN_INFO "[SPRD_FINGERPRINT_INFO] fpc1020 probe done!\n");
	
	return 0;

err_cdev:
	printk(KERN_ERR "[SPRD_FINGERPRINT_ERR] err_cdev: func:%s line:%d\n", __func__,__LINE__);
	cdev_del(&fpc1020->cdev);

err_chrdev:
	printk(KERN_ERR "[SPRD_FINGERPRINT_ERR] err_chrdev: func:%s line:%d\n", __func__,__LINE__);
	unregister_chrdev_region(fpc1020->devno, 1);

	fpc1020_manage_sysfs(fpc1020, spi, false);

err:
	printk(KERN_ERR "[SPRD_FINGERPRINT_ERR] err: func:%s line:%d\n", __func__,__LINE__);
					...
					struct fpc1020_platform_data *pdata)
{
	...
	if (gpio_is_valid(pdata->cs_gpio)) {
		...
		printk(KERN_INFO "[SPRD_FINGERPRINT_INFO] 
			Assign SPI.CS -> GPIO%d\n",
			pdata->cs_gpio);

		error = gpio_request(pdata->cs_gpio, "fpc1020_cs");
		if (error) {
		printk(KERN_ERR "[SPRD_FINGERPRINT_ERR] 
					...
					struct fpc1020_platform_data *pdata)
{
	...
	pdata->reset_gpio = FINGERPRINT_FPC1020_RESET;
	pdata->irq_gpio   = FINGERPRINT_FPC1020_IRQ;
	pdata->cs_gpio    = FINGERPRINT_FPC1020_CS;
	pdata->force_hwid = 0;
