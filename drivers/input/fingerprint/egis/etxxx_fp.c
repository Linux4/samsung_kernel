...
irqreturn_t fp_eint_func_ll(int irq, void *dev_id)
{
	...
	if (egis_dev->fps_ints.drdy_irq_flag == DRDY_IRQ_ENABLE)
	{
		...
		DEBUG_PRINT(" %s : DRDY_IRQ_DISABLE \n", __func__);
...
int egisfp_interrupt_init(struct egisfp_dev_t *egis_dev, int int_mode, int detect_period, int detect_threshold)
{
	...
	if (egis_dev->fps_ints.drdy_irq_flag == DRDY_IRQ_DISABLE)
	{
		...
		DEBUG_PRINT(" %s : enable irq with mode = %d \n", __func__, int_mode);
...
unsigned int egisfp_interrupt_poll(struct file *file, struct poll_table_struct *wait)
{
	...
	if (egis_dev->fps_ints.finger_on)
	{
		...
		INFO_PRINT(" egisfp_interrupt_poll_mask = %d \n", mask);
	}
	else if (egis_dev->fps_ints.drdy_irq_abort == 1)
	{
		mask |= 0x0400 | POLLRDNORM;
		egis_dev->fps_ints.drdy_irq_abort = 0;
	}
	...
	if (egis_dev != NULL)
	{
		if (!egis_dev->platforminit_done)
		{
			...
			/* if (egis_dev->ctrl_power)
			{
				...
				{
					egis_dev->vcc = devm_regulator_get(&egis_dev->dd->dev, "fp_vdd");
			...
			if (egis_dev->dd)
			{
				...
				egis_dev->reset_high = pinctrl_lookup_state(egis_dev->pinctrl, "reset-high");
				...
				egis_dev->reset_low = pinctrl_lookup_state(egis_dev->pinctrl, "reset-low");
				...
				if (pinctrl_select_state(egis_dev->pinctrl, egis_dev->reset_low))
					goto egisfp_pinctrl_fail;
				egis_dev->irq_active = pinctrl_lookup_state(egis_dev->pinctrl, "irq-init");
			...
			if (egis_dev->dd)
			{
				...
				egis_dev->spi_active = pinctrl_lookup_state(egis_dev->pinctrl, "spi-default");
				...
				/* egis_dev->spi_default = pinctrl_lookup_state(egis_dev->pinctrl, "gpio-default");
				...
				// 2020.12.29 set spi active for Sumsung N22
				if (pinctrl_select_state(egis_dev->pinctrl, egis_dev->spi_active))
					goto egisfp_pinctrl_fail;
			}
...
// Register FB notifier +++
	...
	fb_register_client(&egis_dev->notifier);
// Register FB notifier ---
...
//+Bug 612359,zjj.wt,add,2021/01/18, add fp adm node
	...
	{
		ERROR_PRINT(" %s : fingerprint_adm_class error \n", __func__);
	...
	if(device_create_file(fingerprint_adm_dev,&adm_on_off_attr) < 0){
		ERROR_PRINT(" %s : fail to creat fingerprint_adm_device  %d \n", __func__,adm_on_off_attr.attr.name);
	}
//-Bug 612359,zjj.wt,add,2021/01/18, add fp adm node
