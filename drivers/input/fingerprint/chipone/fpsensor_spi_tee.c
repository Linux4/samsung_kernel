...
static long fpsensor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FPSENSOR_IOC_INIT:
       ...
       */
       retval = devm_request_threaded_irq(&g_fpsensor->platform->dev, g_fpsensor->irq,NULL, 
...
static int fpsensor_dev_setup(fpsensor_data_t *fpsensor)
{
 ...
 //   }
    ...
    dev = device_create(fpsensor->class, &fpsensor->platform->dev, dev_no, fpsensor, FPSENSOR_DEV_NAME);
    if (IS_ERR(dev)) {
