...
static int mt_spi_power_flag =  0;
...
static void fpsensor_spi_power_enable(u8 bonoff)
{
	int retval = 0;
	int status = 0;
	int is_enable = 0;
	if (g_fpsensor->fp_regulator){
	  is_enable = regulator_is_enabled(g_fpsensor->fp_regulator);  
	  fpsensor_debug(ERR_LOG, "regulaor_is_enabled status = %d\n", is_enable);
	}
	fpsensor_debug(ERR_LOG, "fpsensor_spi_power_enable bonoff = %d\n", bonoff);
    if ((bonoff == 0) && (mt_spi_power_flag == 1)) {
        if (g_fpsensor->fp_regulator){
    ...
    } else if((bonoff == 1) && (mt_spi_power_flag == 0)) {
       if (g_fpsensor->fp_regulator)
	   {
		    if(is_enable <= 0)
