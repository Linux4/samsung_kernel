...
static unsigned int tsp_dumpkey_used_event[] = {
	KEY_VOLUMEDOWN,
	KEY_POWER,
};

static size_t tsp_dumpkey_nr_used_event =
	ARRAY_SIZE(tsp_dumpkey_used_event);
...
//	if (unlikely(!sec_debug_is_enabled()))
		sec_kn_register_notifier(&nb_gpio_keys,
				tsp_dumpkey_used_event,
				tsp_dumpkey_nr_used_event);
	return 0;
}
