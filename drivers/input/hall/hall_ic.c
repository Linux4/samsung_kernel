	...
	struct device_attribute *attr, char *buf)
{
	...
	pr_err("HALL:gpio value is %d, hall state is %d.\n", ddata->pdata->hall->state, !ddata->pdata->hall->state);
	sprintf(buf, "%d\n", !ddata->pdata->hall->state);

	return strlen(buf);
}

static DEVICE_ATTR_RO(hall_state);
...
static void hall_ic_work(struct work_struct *work)
{
	...
	hall->state = gpiod_get_value(hall->gpiod);
	pr_err("HALL:gpio value is %d, hall state is %d.\n", hall->state, !hall->state);

	input_report_switch(hall->input, hall->event, !hall->state);
	input_sync(hall->input);
	pr_err("HALL:END input_sync");
}
#endif
...
static irqreturn_t hall_ic_detect(int irq, void *dev_id)
{
	...
	int state = !!gpiod_get_value(hall->gpiod);
	//pr_err("HALL:interrupt hall_gpiod = %lx.\n", hall->gpiod);
	//pr_info("HALL:%s %s(%d)\n", __func__,
	//	hall->name, state);
...
static int hall_ic_input_dev_register(struct hall_ic_data *hall)
{
	...
	if (!input) {
		pr_err("HALL:failed to allocate state\n");
		return -ENOMEM;
	}
	...
	if (ret) {
		pr_err("HALL:failed to register input device\n");
		return ret;
	}
...
static int hall_ic_setup_halls(struct hall_ic_drvdata *ddata)
{
	...
	list_for_each_entry(hall, &hall_ic_list, list) {
		...
		ret = request_threaded_irq(hall->irq, NULL, hall_ic_detect,
				...
				hall->name, hall);
		if (ret < 0)
			pr_err("HALL:failed to request irq %d(%d)\n",
				hall->irq, ret);
...
static struct hall_ic_pdata *hall_ic_parsing_dt(struct device *dev)
{
	...
	pr_err("HALL:ENTER hall_ic_parsing_dt.\n");
	if (!node)
	...
	for_each_child_of_node(node, pp) {
		...
		pr_err("HALL:hall gpio is %d.\n", hall->gpio);
		pr_err("HALL:hall flags is %d.\n", flags);
		hall->gpiod = gpio_to_desc(hall->gpio);
		pr_err("HALL:true hall_gpiod = %lx.\n", hall->gpiod);
		ret = gpiod_request(hall->gpiod, "int-gpios");
		if (ret < 0 && ret != -EBUSY) {
			pr_err("gpio request failed.\n");
			return ERR_PTR(ret);
		}
		gpiod_direction_input(hall->gpiod);
		hall->state = gpiod_get_value(hall->gpiod);
		pr_err("HALL:init gpio state is %d.\n", hall->state);
		if (hall->gpio < 0) {
			ret = hall->gpio;
			if (ret) {
				pr_err("HALL:Failed to get gpio flags %d\n", ret);
				return ERR_PTR(ret);
			}
		}
		hall->active_low = flags & OF_GPIO_ACTIVE_LOW;
		pr_err("HALL:hall active_low is %d.\n", hall->active_low);
		hall->irq = gpio_to_irq(hall->gpio);
		pr_err("HALL:hall irq number is %d.\n", hall->irq);
		hall->name = of_get_property(pp, "name", NULL);

		pr_info("HALL:%s %s\n", __func__, hall->name);

		if (of_property_read_u32(pp, "event", &hall->event)) {
			pr_err("HALL:failed to get event: 0x%x\n", hall->event);
			return ERR_PTR(-EINVAL);
		}

		pr_info("HALL:%s event is %d\n", __func__, hall->event);

		list_add(&hall->list, &hall_ic_list);
	}
	pr_err("HALL:END hall_ic_parsing_dt.\n");
	return pdata;
}
...
static int hall_ic_probe(struct platform_device *pdev)
{
	...
	if (!pdata) {
		...
		if (IS_ERR(pdata)) {
			pr_err("HALL:%s : fail to get the DT\n", __func__);
			goto fail1;
		}
	}
	...
	if (!ddata) {
		pr_err("HALL:failed to allocate drvdata\n");
		ret = -ENOMEM;
		goto fail1;
	}
	...
	list_for_each_entry(hall, &hall_ic_list, list) {
		...
		if (ret) {
			pr_err("HALL:hall_ic_input_dev_register failed %d\n", ret);
			goto fail2;
		}
	...
	if (ret) {
		pr_err("HALL:failed to set up hall : %d\n", ret);
		goto fail2;
	}

	hardwareinfo_set_prop(HARDWARE_HALL, "HALL IC");

	ret = device_create_file(&pdev->dev, &dev_attr_hall_state);
	if(ret < 0) {
		pr_err("Failed to create attribute hall_state!\n");
	} else {
		pr_info("%s: line %d: Device create hall_state file for device of platform device success!\n", __func__, __LINE__);
	}
...
static int hall_ic_resume(struct device *dev)
{
	...
	list_for_each_entry(hall, &hall_ic_list, list) {
		int state = !!gpiod_get_value(hall->gpiod);
		pr_err("HALL:resume hall_gpiod = %lx.\n", hall->gpiod);
		state ^= hall->active_low;
		pr_info("HALL:%s %s %s(%d)\n", __func__, hall->name,
