...
#define HX9031A_DRIVER_VER "Change-Id 99b3d5"

//#define USE_USB_CHANGE

#ifdef ANFR_TEST
#define MAX_INT_COUNT	20
#define MAX_CH_COUNT  3
#endif
...
#ifdef ANFR_TEST
static bool anfr_flg = true;
static int anfr_diff = 0;
#endif
...
static void hx9031a_ch_cfg(uint8_t chip_select)
{
    ...
    if(HX9023E_ON_BOARD == chip_select) {
        ...
        CH0_POS = CS0;  //grip_sensor
        CH0_NEG = CS1;  //grip_sensor ref
...
static void hx9031a_input_report(void)
{
    ...
    {
        if(anfr_flg == true){
            LOG_INFO("anfr_diff = %d", anfr_diff);
            if(anfr_diff >= 1){
                anfr_flg = false;
            }
            if((data_diff[0] < 0) || (data_diff[1] < 0) || (data_diff[2] < 0)){
                anfr_diff++;
            }
        }
        LOG_INFO("interrupt_count is %d", this->interrupt_count);
        if ((this->interrupt_count <= MAX_INT_COUNT) && (anfr_flg == true)){
            LOG_INFO("force all channel State=Near");
            for (ii = 0; ii < 3; ii++) {
                if(hx9031a_channels[ii].enabled){
    ...
    if(!strncmp(temp_input_dev->name,"grip_sensor_sub",sizeof("grip_sensor_sub"))){
        if (!strncmp(buf, "1", 1)){
            ...
            if(hx9031a_pdata.hx_first_boot){
                ...
                LOG_INFO("anfr:channels[1] enable input near\n");
            }else{
...
#ifdef ANFR_TEST
static void hx9031a_irq_counter(void)
{
	int i = 0;
	uint8_t value = 0;
	uint8_t temp = hx9031a_pdata.prox_state_reg;
	uint8_t curr_status = 0;
	static uint8_t last_status[HX9031A_CH_NUM] = {0};
	for (i = 0; i < HX9031A_CH_NUM; i++) {
        curr_status = (temp >> i) & 0x1;
        LOG_INFO("curr_status[%d] = 0x%x", i, curr_status);
        LOG_INFO("last_status[%d] = 0x%x", i, last_status[i]);
        if ((curr_status != last_status[i]) && hx9031a_channels[i].enabled) {
            value = 1;
        ...
        char *buf)
{
    ...
    uint32_t cap_reg = 0;
    uint32_t cap_val = 0;
    uint32_t cap_val_int = 0;
    uint32_t cap_val_dec = 0;
    ...
    cap_val = cap_reg * 586;    //cap_reg * 58.6ff
    cap_val_int = cap_val / 10000;
    cap_val_dec = cap_val % 10000;
	snprintf(temp_data, sizeof(temp_data), "%d.%d", cap_val_int,
							cap_val_dec);
    PRINT_ERR("CH0_background_cap=%s\n", temp_data);
    ...
    hx9031a_read(RA_OFFSET_DAC0_7_0_0x15, rx_buf, 2);
    cap_reg = ((rx_buf[1] << 8) | (rx_buf[0]));
    cap_reg = cap_reg & 0xFFF;//12bit
    cap_val = cap_reg * 586;    //cap_reg * 58.6ff
    cap_val_int = cap_val / 10000;
    cap_val_dec = cap_val % 10000;
	snprintf(temp_data, sizeof(temp_data), "%d.%d", cap_val_int,
							cap_val_dec);
    PRINT_ERR("CH0_refer_channel_cap=%s\n", temp_data);
	len += snprintf(buf+len, PAGE_SIZE-len,
        ...
        char *buf)
{
	...
	snprintf(temp_data, sizeof(temp_data), "%d.%d", cap_val_int,
							cap_val_dec);
    PRINT_ERR("CH2_background_cap=%s\n", temp_data);
	len += snprintf(buf+len, PAGE_SIZE-len,
	...
	snprintf(temp_data, sizeof(temp_data), "%d.%d", cap_val_int,
							cap_val_dec);
    PRINT_ERR("CH2_refer_channel_cap=%s\n", temp_data);
	len += snprintf(buf+len, PAGE_SIZE-len,
        ...
        char *buf)
{
	...
	snprintf(temp_data, sizeof(temp_data), "%d.%d", cap_val_int,
							cap_val_dec);
    PRINT_ERR("CH3_background_cap=%s\n", temp_data);
	len += snprintf(buf+len, PAGE_SIZE-len,
	...
	snprintf(temp_data, sizeof(temp_data), "%d.%d", cap_val_int,
							cap_val_dec);
    PRINT_ERR("CH3_refer_channel_cap=%s\n", temp_data);
	len += snprintf(buf+len, PAGE_SIZE-len,
...
//}end =============================================================
    ...
    hx9031a_pdata.interrupt_count = 0;
#endif
    ...
    hardwareinfo_set_prop(HARDWARE_SAR,"HX9031A,FW:V0.3");  // add hardware info for MMI
