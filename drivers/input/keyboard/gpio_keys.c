...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	if (error) {
		input_err(true, global_dev, "%s: Failed to create device file in sysfs entries(%s)!\n",
				__func__, dev_attr_sec_key_pressed.attr.name);
	}

	error = device_create_file(sec_key, &dev_attr_wakeup_keys);
	if (error < 0) {
		input_err(true, global_dev, "%s: Failed to create device file(%s), error: %d\n",
				__func__, dev_attr_wakeup_keys.attr.name, error);
	}
