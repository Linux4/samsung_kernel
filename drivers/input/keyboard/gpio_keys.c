...
#if defined(CONFIG_KEYBOARD_S2MPU06)
		...
		struct device_attribute *attr, char *buf)
{
	...
	char tmp[7] = {0};
	ssize_t count;
	int len = (ddata->pdata->nbuttons + 1) * 7 + 2;
...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	if ((button->code == KEY_VOLUMEUP) && !!state) {
		printk(KERN_INFO "volup key is pressed\n");
	}

	if ((button->code == KEY_VOLUMEDOWN) && !!state) {
		printk(KERN_INFO "voldn key is pressed\n");
	}

	if ((button->code == KEY_RECENT) && !!state) {
		printk(KERN_INFO "recent key is pressed\n");
	}

	if ((button->code == KEY_BACK) && !!state) {
		printk(KERN_INFO "back key is pressed\n");
	}

	if ((button->code ==  KEY_DUMMY_HOME) && !!state) {
		printk(KERN_INFO "multi key is pressed\n");
	}
