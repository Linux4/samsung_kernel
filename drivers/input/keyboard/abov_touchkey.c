...
#define TK_FW_PATH_BIN "abov/abov_tk_a7.fw"
	...
	if (on) {
		if (pdata->vdd_io_vreg) {
			ret = regulator_enable(pdata->vdd_io_vreg);
				if(ret)
					pr_info("[TKEY] 1.8v %s: iovdd reg enable fail\n", __func__);
				else
					pr_info("[TKEY] 1.8v %s enable \n", __func__);
		}
		if (pdata->gpio_en >= 0) {
	            gpio_direction_output(pdata->gpio_en, on);
	            pr_info("[TKEY] 2.8v %s: %s: %d\n", __func__, on ? "on" : "off",
	                    gpio_get_value(pdata->gpio_en));
		}

		if (pdata->gpio_tkey_led_en >= 0) {
	            gpio_direction_output(pdata->gpio_tkey_led_en, on);
	            pr_info("[TKEY] 3.3v %s: %s: %d\n", __func__, on ? "on" : "off",
		...
		if (pdata->vdd_io_vreg) {
			ret = regulator_disable(pdata->vdd_io_vreg);
				if(ret)
					pr_info("[TKEY] 1.8v %s: iovdd reg disable fail\n", __func__);
				else
					pr_info("[TKEY] 1.8v %s disable \n", __func__);
		}
		if (pdata->gpio_tkey_led_en >= 0) {
	            gpio_direction_output(pdata->gpio_tkey_led_en, on);
	            pr_info("[TKEY] 3.3v %s: %s: %d\n", __func__, on ? "on" : "off",
