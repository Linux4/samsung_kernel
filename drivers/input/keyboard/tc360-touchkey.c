...
void tc360_power(int gpio_en, int onoff)
{
	int ret = 0;
	static struct regulator *reg_l5;

	pr_info("[TKEY] power %s\n", onoff ? "on" : "off");

	if (!reg_l5) {
		reg_l5 = regulator_get(NULL, "8916_l5");
		if (IS_ERR(reg_l5)) {
			pr_err("%s: could not get 8916_l5, rc = %ld\n",
				__func__, PTR_ERR(reg_l5));
			return;
		}
	}
	ret = regulator_set_voltage(reg_l5, 1800000, 1800000);
		if (ret) {
			pr_err("%s: unable to set reg_l5 voltage to 3.3V\n",
				__func__);
			return;
		}
	if (onoff) {
		if (!regulator_is_enabled(reg_l5)) {
			ret = regulator_enable(reg_l5);
			if (ret) {
				pr_err("%s: enable lvs6 failed, rc=%d\n",
					__func__, ret);
				return;
			}
			pr_info("%s: tsp 1.8V[8916_l5] on is finished.\n", __func__);
		} else
			pr_info("%s: tsp 1.8V[8916_l5] is already on.\n", __func__);
	} else {
		if (regulator_is_enabled(reg_l5)) {
			ret = regulator_disable(reg_l5);
			if (ret) {
				pr_err("%s: enable 8916_l5 failed, rc=%d\n",
					__func__, ret);
				return;
			}
			pr_info("%s: tsp 1.8V[8916_l5] off is finished.\n", __func__);
		} else
			pr_info("%s: tsp 1.8V[8916_l5] is already off.\n", __func__);
	}
	//enable TKEY IC
	ret = gpio_direction_output(gpio_en, onoff);
		if (ret) {
			printk(" Error in setting GPIOS\n");
			return ;
		}
			...
			struct tc360_platform_data *pdata)
{
	...
	pdata->gpio_en = of_get_named_gpio(np, "TC360,tkey_en-gpio", 0);
	if(pdata->gpio_en < 0){
		dev_err(dev, "unable to get gpio_sda\n");
...
{
	data->pdata->power(data->pdata->gpio_en, false);
	...
	data->pdata->power(data->pdata->gpio_en, true);
