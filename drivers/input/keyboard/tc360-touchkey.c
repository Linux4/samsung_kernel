...
#ifdef CONFIG_OF
void tc360_power(struct tc360_platform_data *pdata, int onoff)
{
	int ret = 0;

	pr_info("[TKEY] power %s\n", onoff ? "on" : "off");

	if(onoff){
		ret = regulator_enable(pdata->vddo_supply);
		if (ret) {
		...
		ret = regulator_disable(pdata->vddo_supply);
		if (ret) {
			pr_err("%s: disable vddo failed, rc=%d\n",
				__func__, ret);
			return;
		}
			...
			struct tc360_platform_data *pdata)
{
	...
	pdata->gpio_int = of_get_named_gpio(np, "TC360,irq-gpio", 0);
	if(pdata->gpio_int < 0){
		dev_err(dev, "unable to get gpio_int\n");
		return pdata->gpio_int;
	}

	pdata->gpio_scl = of_get_named_gpio(np, "TC360,scl-gpio", 0);
	if(pdata->gpio_scl < 0){
		dev_err(dev, "unable to get gpio_scl\n");
		return pdata->gpio_scl;
	}

	pdata->gpio_sda = of_get_named_gpio(np, "TC360,sda-gpio", 0);
	if(pdata->gpio_sda < 0){
		dev_err(dev, "unable to get gpio_sda\n");
		return pdata->gpio_sda;
	}

	pdata->gpio_tkey_led_en = of_get_named_gpio(np, "TC360,tkey_led_en-gpio", 0);
	if(pdata->gpio_tkey_led_en < 0){
		dev_err(dev, "unable to get gpio_tkey_led_en...ignoring\n");
	}

	pdata->vddo_supply = regulator_get(dev, "vddo");
	if(IS_ERR(pdata->vddo_supply)){
		dev_err(dev, "unable to get vddo regulator\n");
	}

	pdata->vdd_supply = regulator_get(dev, "vdd");
	if(IS_ERR(pdata->vdd_supply)){
		dev_err(dev, "unable to get vdd regulator\n");
	}

	//printk("Touchkey gpios are: TOUCHKEY_I2C_SCL = %d, TOUCHKEY_I2C_SDA = %d, TOUCHKEY_INT = %d \n", pdata->gpio_scl, pdata->gpio_sda, pdata->gpio_int);
	pdata->keycodes = touchkey_keycodes;
	pdata->enable = 1;
	pdata->suspend_type = TC360_SUSPEND_WITH_POWER_OFF ;
	pdata->power = tc360_power ;
	pdata->udelay = 6;
	return 0;
}
#else
...
static void tc360_request_gpio(struct tc360_platform_data *pdata)
{
	int ret;
	...
	if (ret) {
		printk(KERN_ERR " unable to request touchkey_scl [%d]\n",
				pdata->gpio_scl);
		return;
	}
	...
	if (ret) {
		printk(KERN_ERR " unable to request touchkey_sda [%d]\n", pdata->gpio_sda);
		return;
	}
	...
	if (ret) {
		printk(KERN_ERR " unable to request touchkey_irq [%d]\n", pdata->gpio_int);
		return;
	}
#if defined(CONFIG_GPIO_MSM_V3)
	...
	if(ret < 0){
		printk(KERN_ERR "unable to request gpio_tkey_led_en..Tkey led will not work...ignoring\n");
	}

	ret = regulator_set_voltage(pdata->vddo_supply, 1800000, 1800000);
	if (ret) {
		pr_err("%s: unable to set vddo voltage to 1.8V\n",
			__func__);
	}
}
...
static irqreturn_t tc360_interrupt(int irq, void *dev_id)
{
	...
	case 1 ... 2:
		...
		dev_info(&client->dev, "key[%3d] is %s\n",
			data->pdata->keycodes[key_index - 1],
		...
		input_report_key(data->input_dev,
				data->pdata->keycodes[key_index - 1], press);
...
{
	data->pdata->power(data->pdata, false);
	...
	data->pdata->power(data->pdata, true);
...
static void tc360_led_worker(struct work_struct *work)
{
	...
	if(data->pdata->gpio_tkey_led_en >= 0)
		gpio_direction_output(data->pdata->gpio_tkey_led_en,enable);
			  ...
			  enum led_brightness value)
{
	...
	if(!data->enabled){
		dev_info(&client->dev, "TKEY is disabled.\n");
		return;
	}
...
static int tc360_pinctrl_configure(struct tc360_data *data, 
							bool active)
{
...
static int tc360_init_interface(struct tc360_data *data)
{
	...
	ret = sysfs_create_link(&data->fdata->dummy_dev->kobj,
				 ...
				 const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		error = tc360_parse_dt(&client->dev, pdata);
		if (error)
			return error;
	} else
...
/* Get pinctrl if target uses pinctrl */
	...
	if (data->pinctrl) {
		error = tc360_pinctrl_configure(data, true);
		if (error)
			pr_err("%s: cannot set ts pinctrl active state\n", __func__);
	}
	...
	data->num_key = 2;//data->pdata->num_key;
	...
	input_dev->keycode = data->pdata->keycodes;
	input_dev->keycodesize = sizeof(data->pdata->keycodes[0]);
	...
	for (i = 0; i < data->num_key; i++) {
		input_set_capability(input_dev, EV_KEY, data->pdata->keycodes[i]);
		set_bit(data->pdata->keycodes[i], input_dev->keybit);
	}
...
//	int ret;
	...
	data->pdata->power(data->pdata, false);
	for (i = 0; i < data->num_key; i++)
		input_report_key(data->input_dev, data->pdata->keycodes[i], 0);
	input_sync(data->input_dev);
	data->enabled = false;
	return 0;
#else
...
#endif
#ifdef CONFIG_OF
static struct of_device_id tc360_match_table[] = {
	{ .compatible = "TC360,TOUCHKEY",},
	{ },
};
#else
#define tc360_match_table	NULL
#endif
...
static struct i2c_driver tc360_driver = {
	...
	.driver = {
		.name	= TC360_DEVICE,
	#ifdef CONFIG_OF
		.of_match_table = tc360_match_table,
	#endif
#if defined(CONFIG_PM) 
		.pm	= &tc360_pm_ops,
#endif
