				 ...
				 size_t size)
{
	...
	if (device_data->fac_key_led_en) {
		printk(KERN_ERR "[KEY] Skip key_led_off(%d) by factory app\n", data);
		return size;
	}
...
static ssize_t key_led_onoff_fac(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t size)
{
	int data;
	...
	if (data) {
		device_data->fac_key_led_en = 1;
		gpio_direction_output(device_data->dtdata->gpio_tkey_led_en, 1);
		printk(KERN_ERR "[KEY] key_led_on\n");
	} else {
		device_data->fac_key_led_en = 0;
		gpio_direction_output(device_data->dtdata->gpio_tkey_led_en, 0);
		printk(KERN_ERR "[KEY] key_led_off\n");
	}
...
static DEVICE_ATTR(brightness_fac, S_IRUGO | S_IWUSR | S_IWGRP,
		NULL, key_led_onoff_fac);

static struct attribute *key_attributes[] = {
	&dev_attr_brightness.attr,
	&dev_attr_brightness_fac.attr,
	NULL,
};
