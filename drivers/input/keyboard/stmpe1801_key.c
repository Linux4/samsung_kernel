...
#define keypad_debug_dbg(mode, dev, fmt, ...)	\
({								\
	if (mode) {					\
...
#define keypad_debug_info(mode, dev, fmt, ...)	\
({								\
	if (mode) {							\
...
#define keypad_debug_err(mode, dev, fmt, ...)	\
({								\
	if (mode) {					\
...
#else
#define keypad_debug_dbg(mode, dev, fmt, ...)	dev_dbg(dev, fmt, ## __VA_ARGS__)
#define keypad_debug_info(mode, dev, fmt, ...)	dev_info(dev, fmt, ## __VA_ARGS__)
#define keypad_debug_err(mode, dev, fmt, ...)	dev_err(dev, fmt, ## __VA_ARGS__)
#endif
...
static int __stmpe1801_get_keycode_entries(struct stmpe1801_dev *stmpe1801, u32 mask_row, u32 mask_col)
{
	...
	for (i = 8; i > 0; i--) {
		comp = 1 << (i - 1);
		keypad_debug_info(true, &stmpe1801->client->dev, "row: %d, comp: %d\n", row, comp);
		if (row & comp) break;
	}

	for (j = 10; j > 0; j--) {
		comp = 1 << (j - 1);
		keypad_debug_info(true, &stmpe1801->client->dev, "col: %d, comp: %d\n", col, comp);
		if (col & comp) break;
	}
#ifdef DBG_PRN
	keypad_debug_info(true, &stmpe1801->client->dev, "row: %d, col: %d\n", i, j);
#endif
...
static int __i2c_block_read(struct i2c_client *client, u8 regaddr, u8 length, u8 *values)
{
	...
	while (retry--) {
		...
		if (ret != 2) {
			keypad_debug_err(true, &client->dev, "failed to read reg %#x: %d\n", regaddr, ret);
			ret = -EIO;
...
static int __i2c_block_write(struct i2c_client *client, u8 regaddr, u8 length, const u8 *values)
{
	...
	while (retry--) {
		...
		if (ret != 1) {
			keypad_debug_err(true, &client->dev, "failed to write reg %#x: %d\n", regaddr, ret);
			ret = -EIO;
				usleep_range(10000, 10000);
				continue;
		}
...
#ifdef DBG_PRN
	keypad_debug_info(true, &stmpe1801->client->dev, "dev lock @ %s\n", __func__);
#endif
...
#ifdef DBG_PRN
	keypad_debug_info(true, &stmpe1801->client->dev, "dev unlock @ %s\n", __func__);
#endif
...
static int stmpe1801_hw_init(struct stmpe1801_dev *stmpe1801)
{
...
#ifdef DBG_PRN
	keypad_debug_info(true, &stmpe1801->client->dev, "Chip ID = %x %x\n" ,
	...
	if (stmpe1801->dtdata->block_type & STMPE1801_BLOCK_KEY) {
		...
		if (ret < 0) goto err_hw_init;
#ifdef DBG_PRN
		keypad_debug_info(true, &stmpe1801->client->dev, "Keypad setting done.\n");
#endif
	}
	...
	if (stmpe1801->dtdata->block_type & STMPE1801_BLOCK_GPIO) {
		for (i = 0 ; i < stmpe1801_int_data_size ; i++){
			...
			if (ret < 0)
				keypad_debug_err(true, &stmpe1801->client->dev, "%s : STMPE1801_REG_INT_GPIO_MASK_LOW ERROR.\n", __func__);
			...
			if (ret < 0)
				keypad_debug_err(true, &stmpe1801->client->dev, "%s : STMPE1801_REG_GPIO_SET_DIR_yyy ERROR.\n", __func__);

			ret = __i2c_set_bits(stmpe1801->client, STMPE1801_REG_GPIO_FE_yyy + stmpe1801_int_data[i].offset_addr,
											stmpe1801_int_data[i].offset_bit, 1);
			if (ret < 0)
				keypad_debug_err(true, &stmpe1801->client->dev, "%s : STMPE1801_REG_GPIO_FE_yyy ERROR.\n", __func__);

#ifdef DBG_PRN
			keypad_debug_info(true, &stmpe1801->client->dev, "GPIO [%d] setting done.\n", stmpe1801_int_data[i].gpio_num);
#endif
		}
	}
...
{
#ifdef DBG_PRN
	keypad_debug_info(true, &stmpe1801->client->dev, "%s\n", __func__);
#endif
...
#ifdef DBG_PRN
	keypad_debug_info(true, &stmpe1801->client->dev, "return ok @ %s\n", __func__);
#endif
...
err_dis:
#ifdef DBG_PRN
	keypad_debug_err(true, &stmpe1801->client->dev, "err: dev unlock @ %s\n", __func__);
#endif
	mutex_unlock(&stmpe1801->dev_lock);
...
static void stmpe1801_dev_int_proc(struct stmpe1801_dev *stmpe1801)
{
	...
	keypad_debug_info(true, &stmpe1801->client->dev,
		...
		if (ret & STMPE1801_INT_MASK_GPIO) {	// GPIO mode
			...
			for (i=0 ; i < stmpe1801_int_data_size ; i++){
				if ((1 << stmpe1801_int_data[i].offset_bit) & ret) {
					keypad_debug_info(true, &stmpe1801->client->dev, "%s : gpio[%d] %s\n",
				...
				if (ret > 0) {
					if (key_values.value != 0xffff8f8f8LL) {
						for (j = 0; j < 3; j++) {
							...
							//keypad_debug_info(true, &stmpe1801->client->dev, "%s code(%x) R:C(%d:%d)\n",
							//				!up ? "P" : "R", stmpe1801->keycode[code], row, col);
...
#ifdef DBG_PRN
	keypad_debug_info(true, &stmpe1801->client->dev, "%s -- dev_int_proc --\n",__func__);
#endif
...
static irqreturn_t stmpe1801_dev_isr(int irq, void *dev_id)
{
	...
	while(stmpe1801->dtdata->sleep_state == true){
		keypad_debug_info(true, &stmpe1801->client->dev, "%s sleep_state retry=%d\n", __func__, retry);
			...
			struct stmpe1801_dev *device_data)
{
	...
	if(device_data->dtdata->gpio_int < 0){
		keypad_debug_err(true, dev, "unable to get gpio_int\n");
		return device_data->dtdata->gpio_int;
	}

	ret = of_property_read_u32(np, "stmpe,block_type", &temp);
	if(ret){
		keypad_debug_err(true, dev, "unable to get block_type\n");
		return ret;
	}
	...
	if(ret){
		keypad_debug_err(true, dev, "unable to get debounce\n");
		return ret;
	}
	...
	if(ret){
		keypad_debug_err(true, dev, "unable to get scan_cnt\n");
		return ret;
	}
	...
	if(ret){
		keypad_debug_err(true, dev, "unable to get repeat\n");
		return ret;
	}
	...
	if(device_data->dtdata->gpio_tkey_led_en < 0){
		keypad_debug_err(true, dev, "unable to get gpio_tkey_led_en...ignoring\n");
	}

	ret = of_property_read_u32(np, "keypad,num-rows", &temp);
	if(ret){
		keypad_debug_err(true, dev, "unable to get num-rows\n");
		return ret;
	}
	...
	if(ret){
		keypad_debug_err(true, dev, "unable to get num-columns\n");
		return ret;
	}
	...
	if (!map) {
		keypad_debug_err(true, dev, "Keymap not specified\n");
		return -EINVAL;
	}
	...
	if (!keymap_data) {
		keypad_debug_err(true, dev, "Unable to allocate memory\n");
		return -ENOMEM;
	}
	...
	if (!keymap) {
		keypad_debug_err(true, dev, "could not allocate memory for keymap\n");
		return -ENOMEM;
	}
...
#endif
	keypad_debug_info(true, dev, "%s: gpio_int:%d, gpio_led_en:%d, block_type:%d\n",
				 ...
				 size_t size)
{
	...
	if (data) {
		...
		keypad_debug_err(true, dev, "[KEY] key_led_on\n");
	} else {
		gpio_direction_output(device_data->dtdata->gpio_tkey_led_en, 0);
		keypad_debug_err(true, dev, "[KEY] key_led_off\n");
	}
				 ...
				 size_t size)
{
	...
	if (data) {
		...
		keypad_debug_err(true, dev, "[KEY] %s expander_gpio %d\n",__func__, data);
	}
	else {
...
void stmpe1801_gpio_enable(bool en)
{
	...
	keypad_debug_info(true, &data->client->dev, "%s - MP:%x\n",__func__, ret);
}
...
int stmpe_find_irq(unsigned int irq)
{
	...
	keypad_debug_info(true, &data->client->dev, "%s : Can not find supported gpio[%d]\n",__func__, irq);
	return -1;
}
...
int stmpe_enable_irq(unsigned int irq)
{
	...
	if (ret < 0)
		keypad_debug_err(true, &data->client->dev, "%s fail[%d].\n", __func__, irq);

	keypad_debug_info(true, &data->client->dev, "%s Done[%d].\n", __func__, irq);
	return 0;
}
int stmpe_disable_irq(unsigned int irq)
{
	...
	if (ret < 0)
		keypad_debug_info(true, &data->client->dev, "%s fail[%d].\n", __func__, irq);
							...
							unsigned long irqflags, char *devname, void *dev_id)
{
	...
	if (gpio_index < 0) {
		keypad_debug_err(true, &data->client->dev, "%s Can not find supported gpio[%d]\n",__func__, irq);
		return -1;
	}

	if (stmpe1801_int_data[gpio_index].irq_enable){
		keypad_debug_err(true, &data->client->dev, "%s Already use gpio pin(%d)\n",__func__, irq);
		return -1;
	}
	...
	keypad_debug_info(true, &data->client->dev, "%s : gpio_index[%d] gpio[%d] offset_bit[%d] device_name[%s] irq_type[%x].\n",
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		keypad_debug_err(true, &client->dev,
	...
	if (!device_data) {
		keypad_debug_err(true, &client->dev, "Failed to allocate memory\n");
		return -ENOMEM;
	}
	...
	if (client->dev.of_node) {
		...
		if (ret) {
			keypad_debug_err(true, &client->dev, "Failed to use device tree\n");
			ret = -EIO;
			goto err_config;
		}

	} else {
		keypad_debug_err(true, &client->dev, "No use device tree\n");
		device_data->dtdata = client->dev.platform_data;
	}

	if (device_data->dtdata == NULL) {
		keypad_debug_err(true, &client->dev, "failed to get platform data\n");
		goto err_config;
	}
	...
	if (IS_ERR(device_data->pinctrl)) {
		...
		keypad_debug_err(true, &client->dev, "%s: Target does not use pinctrl\n", __func__);
	...
	device_data->keycode_entries = __stmpe1801_get_keycode_entries(device_data, ((1 << device_data->dtdata->num_row) - 1), ((1 << device_data->dtdata->num_column)-1));
#ifdef DBG_PRN
	keypad_debug_info(true, &client->dev, "%s keycode entries (%d)\n",__func__, device_data->keycode_entries);
	keypad_debug_info(true, &client->dev, "%s keymap size (%d)\n",__func__, device_data->keymap_data->keymap_size);
#endif
	device_data->keycode = kzalloc(device_data->keycode_entries * sizeof(unsigned short), GFP_KERNEL);
	if (device_data == NULL) {
		keypad_debug_err(true, &client->dev, "kzalloc memory error\n");
		goto err_keycode;
	}
#ifdef DBG_PRN
	keypad_debug_info(true, &client->dev, "%s keycode addr (%p)\n", __func__, device_data->keycode);
#endif
	...
	if (ret < 0)
	{
		keypad_debug_err(true, &client->dev, "stmpe1801_dev_power_on error\n");
		goto err_power;
	}
	ret = stmpe1801_dev_power_off(device_data);
	if (ret < 0)
	{
		keypad_debug_err(true, &client->dev, "stmpe1801_dev_power_off error\n");
		goto err_power;
	}
...
// request irq
...
#ifdef DBG_PRN
		keypad_debug_info(true, &client->dev, "%s INT mode (%d)\n", __func__, device_data->dev_irq);
#endif
...
#ifdef DBG_PRN
		keypad_debug_info(true, &client->dev, "%s poll mode\n", __func__);
#endif
	...
	if (IS_ERR(device_data->sec_keypad))
		keypad_debug_err(true, &client->dev, "Failed to create sec_key device\n");
#ifdef SUPPORTED_KEYPAD_LED
	ret = sysfs_create_group(&device_data->sec_keypad->kobj, &key_attr_group);
	if (ret) {
		keypad_debug_err(true, &client->dev, "Failed to create the test sysfs: %d\n",
			ret);
	}
#endif
#ifdef SUPPORTED_GPIO_SYSFS
...
#ifdef DBG_PRN
	keypad_debug_info(true, &client->dev, "Error at stmpe1801_dev_probe\n");
#endif
...
#ifdef DBG_PRN
	keypad_debug_info(true, &device_data->client->dev, "%s\n", __func__);
#endif
