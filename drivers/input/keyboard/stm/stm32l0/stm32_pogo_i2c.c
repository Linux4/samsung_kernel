...
static void stm32_print_info(struct stm32_dev *stm32)
{
	...
	input_info(true, &stm32->client->dev, "%s_v%d.%d.%d.%d, con:%d,%d, int:%d, depth:%d\n",
...
{
	struct irq_desc *desc = irq_to_desc(stm32->dev_irq);
	bool int_control = (desc->depth == 0);

	input_err(true, &stm32->client->dev, "%s\n", __func__);
...
static int stm32_i2c_reg_read(struct i2c_client *client, u8 ep_id, u8 regaddr, u8 *values)
{
	struct stm32_dev *stm32 = i2c_get_clientdata(client);
	int ret;
	u16 payload_size = 0;
...
static int stm32_read_crc(struct stm32_dev *stm32)
{
	...
	ret = stm32_i2c_reg_read(stm32->client, ID_MCU, STM32_CMD_CHECK_CRC, rbuf);
	if (ret < 0)
		return ret;
...
static int stm32_read_version(struct stm32_dev *stm32)
{
	...
	ret = stm32_i2c_reg_read(stm32->client, ID_MCU, STM32_CMD_CHECK_VERSION, rbuf);
	if (ret < 0)
		return ret;
...
static void stm32_check_ic_work(struct work_struct *work)
{
	...
	ret = stm32_i2c_reg_read(stm32->client, ID_MCU, STM32_CMD_GET_MODE, &buff);
	if (ret < 0) {
		input_err(true, &stm32->client->dev,
...
static int stm32_fw_update(struct stm32_dev *stm32, const struct firmware *fw)
{
	int ret, i, page_index = 0, page_num, retry = 3;
	u8 buff[4] = { 0 };
	u16 page_size;
	...
	ret = stm32_i2c_reg_write(stm32->client, ID_MCU, STM32_CMD_ENTER_DFU_MODE);
	if (ret < 0)
		return ret;
	...
	for (i = 0; i < 50; i++) {
		stm32_delay(100);
		if (gpio_get_value(stm32->dtdata->gpio_int) == 0)
			break;
	}

	if (gpio_get_value(stm32->dtdata->gpio_int) != 0) {
		input_err(true, &stm32->client->dev,
				"%s: INT is stil high. MCU is not ready\n", __func__);
		goto out;
	}

	ret = stm32_i2c_reg_read(stm32->client, ID_MCU, STM32_CMD_GET_PROTOCOL_VERSION, buff);
	if (ret < 0)
		goto out;
	input_info(true, &stm32->client->dev,
			"%s: protocol ver: 0x%02X%02X\n", __func__, buff[1], buff[0]);

	ret = stm32_i2c_reg_read(stm32->client, ID_MCU, STM32_CMD_GET_PAGE_SIZE, buff);
	if (ret < 0)
		goto out;
	...
	input_info(true, &stm32->client->dev,
			"%s: fw size: %d, page size: %d, page num:%d, packet size: %d\n",
			__func__, fw->size, page_size, page_num, STM32_CFG_PACKET_SIZE);

	/* disable write protection of target bank & erase the target bank */
	ret = stm32_i2c_reg_read(stm32->client, ID_MCU, STM32_CMD_START_FW_UPGRADE, buff);
	...
	while (page_index <= page_num) {
		...
		ret = stm32_i2c_header_write(stm32->client, ID_MCU, page_size + 5);
		if (ret < 0)
			goto out;
	...
	ret = stm32_i2c_reg_read(stm32->client, ID_MCU, STM32_CMD_GET_TARGET_FW_VERSION, buff);
	if (ret < 0)
		goto out;
	...
	ret = stm32_i2c_reg_read(stm32->client, ID_MCU, STM32_CMD_GET_TARGET_FW_CRC32, buff);
	if (ret < 0)
		goto out;
		...
		size_t size)
{
	...
	ret = stm32_i2c_reg_read(stm32->client, ID_MCU, reg, buff);
