...
static void stm32_print_info(struct stm32_dev *stm32)
{
	...
	input_info(true, &stm32->client->dev, "mcu_fw(bin):%x, mcu_fw(ic):%x, %s_v%d.%d.%d.%d, TC_v%02X%02X.%X, con:%d, int:%d, depth:%d, rst:%d, hall:%d\n",
			...
			stm32->connect_state, gpio_get_value(stm32->dtdata->gpio_int),
			desc->depth, stm32->reset_count, stm32->hall_closed);
}
...
int stm32_i2c_read_bulk(struct i2c_client *client, u8 *data, u8 length)
{
	...
	if ((!device_data->connect_state) && (client->addr != 0x51)) {
...
static void stm32_keyboard_connect(struct stm32_dev *data)
{
	...
	snprintf(str_conn, 16, "CONNECT=%d", data->connect_state);
...
static irqreturn_t stm32_conn_isr(int irq, void *dev_id)
{
	...
	data->connect_state = gpio_get_value(data->dtdata->gpio_conn);
