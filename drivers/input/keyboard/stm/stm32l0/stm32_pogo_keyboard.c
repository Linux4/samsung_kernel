...
struct stm32_keypad_dev {
	...
	char					key_name[707][10];
};

static int stm32_get_keycode_entries(struct stm32_keypad_dev *stm32,
		u32 mask_row, u32 mask_col)
{
	...
	input_info(true, &stm32->pdev->dev, "row: %d, col: %d\n", i, j);
...
static void stm32_pogo_kpd_event(struct stm32_keypad_dev *stm32, char *key_values, int len)
{
	int i, event_count;
		...
		struct stm32_keypad_dev *device_data)
{
	...
	snprintf(device_data->key_name[KEY_SEARCH], 10, "Search");
	...
	snprintf(device_data->key_name[KEY_HOME], 10, "HOME");
		...
		unsigned long action, void *data)
{
	...
	case POGO_NOTIFIER_EVENTID_KEYPAD:
		stm32_pogo_kpd_event(stm32, pogo_data.data, pogo_data.size);
		break;
	};
