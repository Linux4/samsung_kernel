...
struct stm32_touchpad_dev {
	...
	int					touch_count;
	u8					button[STM32_MAX_BUTTON_NUM];
};

u32 STM32_BUTTON_MAPPING_KEY[STM32_MAX_BUTTON_NUM] = { BTN_LEFT };
...
static void stm32_release_all_finger(struct stm32_touchpad_dev *stm32)
{
	...
	for (i = 0; i < STM32_MAX_BUTTON_NUM; i++) {
		if (stm32->button[i] == ICON_BUTTON_DOWN) {
			stm32->button[i] = ICON_BUTTON_UP;
			input_report_key(stm32->input_dev, STM32_BUTTON_MAPPING_KEY[i], 0);
			input_info(true, &stm32->pdev->dev, "[BRA] bID:%d, 0x%02X\n",
					i, STM32_BUTTON_MAPPING_KEY[i]);
		}
	}
...
static void stm32_pogo_touchpad_event(struct stm32_touchpad_dev *stm32, char *event, int len)
{
	...
	if (stm32_bit_test(touch_info.status, BIT_ICON_EVENT)) {
		for (i = 0; i < STM32_MAX_BUTTON_NUM; i++) {
			if (stm32_bit_test(touch_info.button_info, BIT_O_ICON0_DOWN + i))
				stm32->button[i] = ICON_BUTTON_DOWN;
			else
				stm32->button[i] = ICON_BUTTON_UP;

			input_report_key(stm32->input_dev, STM32_BUTTON_MAPPING_KEY[i],
						stm32->button[i] == ICON_BUTTON_DOWN);
			input_info(true, &stm32->pdev->dev,
					"[B%s] bID:%d, 0x%02X\n",
					stm32->button[i] == ICON_BUTTON_DOWN ? "P" : "R",
					i, STM32_BUTTON_MAPPING_KEY[i]);
		}
	}
...
static int stm32_touchpad_set_input_dev(struct stm32_touchpad_dev *device_data)
{
	...
	for (i = 0; i < STM32_MAX_BUTTON_NUM; i++)
		set_bit(STM32_BUTTON_MAPPING_KEY[i], input_dev->keybit);
