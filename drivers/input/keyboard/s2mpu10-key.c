/*
 * Driver for Power keys on s2mpu10 IC by PWRON rising, falling interrupts.
 *
 * drivers/input/keyboard/s2mpu10_keys.c
 * S2MPU10 Keyboard Driver
 *
...
extern bool (*pmic_key_get_pwrkey)(void);
#endif
...
static struct platform_device *s2mpu10_key_pdev;
...
struct power_keys_drvdata {
	struct device *dev;
	struct s2mpu10_platform_data *s2mpu10_pdata;
...
bool s2mpu10_is_pwron(void)
{
	struct power_keys_drvdata *ddata = platform_get_drvdata(s2mpu10_key_pdev);
	int i;
	int keystate = 0;
...
EXPORT_SYMBOL_GPL(s2mpu10_is_pwron);

bool s2mpu10_get_pwrkey_value(void)
{
	int ret;
	u8 val;
	struct s2mpu10_dev *s2mpu10 = dev_get_drvdata(s2mpu10_key_pdev->dev.parent);

	ret = s2mpu10_read_reg_nomutex(s2mpu10->i2c,
...
EXPORT_SYMBOL_GPL(s2mpu10_get_pwrkey_value);
...
static void s2mpu10_keys_work_func(struct work_struct *work)
{
...
static struct power_keys_platform_data *power_keys_get_devtree_pdata(
		struct s2mpu10_dev *mfd_dev)
{
	/* to do */
	#define S2MPU10_SUPPORT_KEY_NUM	(1)
	...
	key_node = of_find_node_by_name(p_node, "s2mpu10-keys");
	if (!key_node) {
		dev_err(dev, "could not find s2mpu10-keys sub-node\n");
		error = -ENOENT;
		goto err_out;
	}

	nbuttons = of_get_child_count(key_node);
	if (nbuttons > S2MPU10_SUPPORT_KEY_NUM)
		dev_warn(dev, "s2mpu10-keys support only one button.\n");
...
power_keys_get_devtree_pdata(struct s2mpu10_dev *mfd_dev)
{
...
static int power_keys_probe(struct platform_device *pdev)
{
	...
	struct s2mpu10_dev *s2mpu10 = dev_get_drvdata(pdev->dev.parent);
	struct s2mpu10_platform_data *mpdata = s2mpu10->pdata;
	...
	if (!pdata) {
		pdata = power_keys_get_devtree_pdata(s2mpu10);
		if (IS_ERR(pdata))
	...
	for (i = 0; i < ddata->pdata->nbuttons; i++) {
		...
		bdata->irq_wqueue = create_singlethread_workqueue("s2mpu10-key-wqueue");
		if (!bdata->irq_wqueue) {
			pr_err("%s: fail to create workqueue\n", __func__);
			goto fail1;
		}
		INIT_DELAYED_WORK(&bdata->key_work, s2mpu10_keys_work_func);

		input_set_capability(input, button->type ?: EV_KEY, button->code);
	}

	ddata->irq_pwronr = mpdata->irq_base + S2MPU10_PMIC_IRQ_PWRONR_INT1;
	ddata->irq_pwronf = mpdata->irq_base + S2MPU10_PMIC_IRQ_PWRONF_INT1;
#ifndef CONFIG_M2_BOARD
...
#ifdef CONFIG_SEC_DEBUG
	pmic_key_is_pwron = s2mpu10_is_pwron;
#endif
#ifdef CONFIG_SEC_REBOOT
...
MODULE_DESCRIPTION("Keyboard driver for s2mpu10");
MODULE_ALIAS("platform:s2mpu10 Power key");
