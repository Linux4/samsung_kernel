...
static int of_hall_data_parsing_dt(struct device *dev,struct hall_drvdata *ddata)
{
	...
	gpio = of_get_named_gpio_flags(np_hall, "hall,gpio_flip_cover", 0, &flags);
	ddata->gpio_flip_cover = gpio;

	gpio = gpio_to_irq(gpio);
	ddata->irq_flip_cover = gpio;

	ddata->folder_type = of_property_read_bool(np_hall, "hall,folder_type");
	printk("[keys] phone type : (%s)\n", ddata->folder_type ? "Folder" : "Bar");

	return 0;
}
#endif

static void hall_power(struct hall_drvdata *ddata, struct device *dev)
{
	int error;

	ddata->vdd_supply = regulator_get(dev, "hall_vdd");

	if(IS_ERR(ddata->vdd_supply)){
		dev_err(dev, "unable to get vddo regulator, ignoring\n");
		return;
	}

	error = regulator_set_voltage(ddata->vdd_supply, 1800000, 1800000);
	if (error) {
		pr_err("%s: unable to set vdd voltage to 1.8V, ignoring\n",
			__func__);
	}

	/*hall ic regulator should be disabled only at driver unload*/
	error = regulator_enable(ddata->vdd_supply);
	if (error) {
		pr_err("%s: enable vdd failed, rc=%d, ignoring\n",
			__func__, error);
		goto err;
	}
	...
	return;
err:
	regulator_put(ddata->vdd_supply);
	ddata->vdd_supply = NULL;
}
...
static int hall_remove(struct platform_device *pdev)
{
	...
	if(ddata->vdd_supply){
		regulator_disable(ddata->vdd_supply);
		regulator_put(ddata->vdd_supply);
	}
