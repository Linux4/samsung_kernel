...
{
#ifdef PSOC3_5
	if (gpio_get_value(g_pdata->gpio_sda))
		return 1;
	else
		return 0;
...
void SCLKHigh(void)
{
	gpio_direction_output(g_pdata->gpio_scl, 1);
}
...
void SCLKLow(void)
{
	gpio_direction_output(g_pdata->gpio_scl, 0);
}
...
{
	gpio_direction_input(g_pdata->gpio_scl);
}
#endif
...
{
	gpio_direction_output(g_pdata->gpio_sda, 1);
}
...
{
	 gpio_direction_output(g_pdata->gpio_sda, 0);
}
...
{
	 gpio_direction_input(g_pdata->gpio_sda);
}
