...
static int tkey_i2c_check(struct cypress_touchkey_info *info)
{
	...
	/* CY device bit 6 (0x20) = MBR3145 */
	if(info->device_ver == 0x20)
		dev_info(&client->dev, "%s: CY device(0x%02x) = CY8CMBR3145\n"
	...
	dev_info(&client->dev, "%s: CY CRC(0x%04x)\n",__func__,info->crc);
#endif
	return ret;
}
...
static int cypress_power_onoff(struct device *dev, bool onoff)
{
	...
	if (info->pdata->vcc_en > 0) {
		...
		if (ret) {
			dev_err(&info->client->dev,
					"%s: unable to set_direction for vcc_en [%d], ret=%d\n",
					__func__, info->pdata->vcc_en, ret);
			return ret;
		}
	}
	...
	if (info->pdata->vdd_led > 0) {
		...
		if (ret) {
			dev_err(&info->client->dev,
					"%s: unable to set_direction for vdd_led [%d], ret=%d\n",
					__func__, info->pdata->vdd_led, ret);
			return ret;
		}
	}
...
static irqreturn_t cypress_touchkey_interrupt(int irq, void *dev_id)
{
	...
	if (code < 0 || code >= info->pdata->keycodes_size) {
		dev_err(&info->client->dev,
...
//		info->support_fw_update = false;
	...
	dev_info(&client->dev, "%s : [IC] device_ver:%#x\n",
			 __func__ , info->device_ver);
