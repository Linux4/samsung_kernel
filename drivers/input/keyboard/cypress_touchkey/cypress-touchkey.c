		...
		u8 reg, u8 *val, unsigned int len)
{
	...
	while (retry--) {
		...
		dev_info(&client->dev, "%s: i2c transfer error. ret=%d\n", 
			__func__, err);
		msleep(20);
	}
	return err;
...
struct cypress_touchkey_platform_data *g_pdata;
				  ...
				  const struct i2c_device_id *id)
{
...
#endif
	dev_info(&info->client->dev, "gpio_to_irq GPIO %d IRQ %d\n",
			pdata->gpio_int, client->irq);
	...
	if (ret < 0) {
		dev_err(&info->client->dev,
			"%s: Failed to create input symbolic link, ret=%d\n",
				__func__, ret);
	}
