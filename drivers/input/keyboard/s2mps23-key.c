/*
 * Driver for Power keys on s2mps23 IC by PWRON rising, falling interrupts.
 *
 * drivers/input/keyboard/s2mps23_keys.c
 * S2MPS23 Keyboard Driver
 *
...
struct power_keys_drvdata {
	struct device *dev;
	struct s2mps23_platform_data *s2mps23_pdata;
...
static void s2mps23_keys_work_func(struct work_struct *work)
{
...
power_keys_get_devtree_pdata(struct s2mps23_dev *iodev)
{
	#define S2MPS23_SUPPORT_KEY_NUM	(1)
	...
	key_np = of_find_node_by_name(mfd_np, "s2mps23-keys");
	if (!key_np) {
	...
	if (nbuttons > S2MPS23_SUPPORT_KEY_NUM || nbuttons == 0) {
		dev_warn(dev, "%s: s2mps23-keys support only one button(%d)\n",
			__func__, nbuttons);
		error = -ENODEV;
		goto err_out;
	}
...
static int power_keys_set_interrupt(struct power_keys_drvdata *ddata, int irq_base)
{
	...
	ddata->irq_pwronr = irq_base + S2MPS23_PMIC_IRQ_PWRONR_INT1;
	ddata->irq_pwronf = irq_base + S2MPS23_PMIC_IRQ_PWRONF_INT1;
				     ...
				     struct input_dev *input, int *wakeup)
{
	...
	for (cnt = 0; cnt < ddata->pdata->nbuttons; cnt++) {
		...
		INIT_DELAYED_WORK(&bdata->key_work, s2mps23_keys_work_func);
...
power_keys_set_drvdata(struct platform_device *pdev,
			struct power_keys_platform_data *pdata,
			struct input_dev *input, struct s2mps23_dev *iodev)
{
...
static int power_keys_probe(struct platform_device *pdev)
{
	...
	input->phys		= "s2mps23-keys/input0";
...
MODULE_DESCRIPTION("Keyboard driver for s2mps23");
MODULE_ALIAS("platform:s2mps23 Power key");
