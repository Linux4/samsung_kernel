 ...
 *****************************************************************************/
struct swd_data {
	struct cypress_touchkey_info *swd_tkey_info;
	struct device *dev;
...
static void HEX_ReadSiliconId(u32 *hexSiliconId)
{
	...
	{
		*hexSiliconId = *hexSiliconId | ((u32)(deviceSiliconId_HexFile[i]) << (8*i));
	}
}
...
static enum Transition_mode {OPEN_XXX, VIRGIN_OPEN, PROT_XXX, WRONG_TRANSITION } flow;
...
static void SetIMO48MHz(void)
{
	...
	parameter1 = (u32)(((u32)SROM_KEY1 << 0) +	//
				(((u32)SROM_KEY2 + (u32)SROM_CMD_SET_IMO_48MHZ) << 8));
...
static u8 GetChipProtectionVal(void)
{
	...
	/* Load the Parameter1 with the SROM command to read silicon ID */
	parameter1 = (u32)(((u32)SROM_KEY1 << 0) +	//
				(((u32)SROM_KEY2 + (u32)SROM_CMD_GET_SILICON_ID) << 8));
...
static u8 GetTransitionMode(void)
{
	...
	dev_info(g_sd->dev, "%s: chipProtectionData_Chip=%s\n", __func__,
		chipProtectionData_Chip == CHIP_PROT_VIRGIN ? "CHIP_PROT_VIRGIN" :
		chipProtectionData_Chip == CHIP_PROT_OPEN ? "CHIP_PROT_OPEN" :
		chipProtectionData_Chip == CHIP_PROT_PROTECTED ? "CHIP_PROT_PROTECTED" :
		"ELSE");
...
#define SetXresHigh()		g_sd->power(g_sd->dev, 1)
#define SetXresLow()		g_sd->power(g_sd->dev, 0)
static u8 DeviceAcquire(void)
{
	...
	/* Set XRES of PSoC 4 low for 100us with SWDCK and SWDIO low (min delay
	   required is 5us) */
	SetXresLow();
	usleep(10 * 1000);//msleep(10);//100);//DelayHundredUs();
	SetXresHigh();

	usleep_range(19 * 1000, 20 * 1000);
	//preempt_disable();
    ...
    }while((swd_PacketAck != SWD_OK_ACK)&& time_before_eq(jiffies, end_time));
    ...
    dev_info(g_sd->dev, "%s:%d: chip_DAP_Id=0x%08x\n",__func__,__LINE__,chip_DAP_Id);
    dev_info(g_sd->dev, "%s:%d: chip_DAP_Id=0x%08x\n",__func__,__LINE__,chip_DAP_Id);
    dev_info(g_sd->dev, "%s:%d: chip_DAP_Id=0x%08x\n",__func__,__LINE__,chip_DAP_Id);
		goto testmode_fail;//return(FAILURE);
	}
	...
	if( swd_PacketAck != SWD_OK_ACK )
    {
	dev_info(g_sd->dev, "%s:%d: swd_PacketAck=0x%02x\n",__func__,__LINE__,swd_PacketAck);
	goto testmode_fail;//return(FAILURE);
    }
    ...
    {
	dev_info(g_sd->dev, "%s:%d: status=0x%x\n",__func__,__LINE__,status);
        goto testmode_fail;//return (FAILURE);
    }
	...
	if (result != SROM_SUCCESS)
	{
		dev_info(g_sd->dev, "%s:%d: result=0x%x\n",__func__,__LINE__,result);
		goto testmode_fail;//return(FAILURE);
	}
	...
	if (result != SROM_SUCCESS)
	{
		dev_err(g_sd->dev, "%s: fail result=%x\n", __func__, result);
		return(FAILURE);		
	}
	#endif
...
static u8 VerifySiliconId(void)
{
	...
	*/
	...
	dev_info(g_sd->dev, "%s: deviceSiliconID=0x%08x\n", __func__, deviceSiliconID);
	dev_info(g_sd->dev, "%s: hexSiliconId=0x%08x\n", __func__, hexSiliconId);
...
static u8 EraseAllFlash(void)
{
    ...
    {
		parameter1 = (u32)(((u32)SROM_KEY1 << 0) + //
					(((u32)SROM_KEY2 + (u32)SROM_CMD_ERASE_ALL) << 8));
...
{  
	...
	dev_info(g_sd->dev, "%s: checksum_Privileged=0x%08x\n", __func__, checksum_Privileged);
	dev_info(g_sd->dev, "%s: success\n", __func__);
	return(SUCCESS);
}
...
{
	u8 arrayID			= 0;
	u8 rowData[BYTES_PER_FLASH_ROW] = {0, };
	...
	dev_info(g_sd->dev, "%s: numOfFlashRows=%d\n", __func__,
		numOfFlashRows);
	...
	for ( rowCount = 0; rowCount < numOfFlashRows; rowCount++)
	{
	...
	#endif
		if (rowCount == 0)
			dev_info(g_sd->dev, "%s: row%d file=0x%02x 0x%02x 0x%02x 0x%02x\n", __func__,
				rowCount, rowData[0], rowData[1], rowData[2], rowData[3]);
		else
		if (rowCount == (numOfFlashRows-1))
			dev_info(g_sd->dev, "%s: row%d file=0x%02x 0x%02x 0x%02x 0x%02x\n", __func__,
				rowCount, rowData[0], rowData[1], rowData[2], rowData[3]);
	    ...
	    parameter1 = (u32)(((u32)SROM_KEY1 << 0) +
					(((u32)SROM_KEY2 + (u32)SROM_CMD_PROGRAM_ROW) << 8) +
...
static u8 VerifyFlash(void)
{
	...
	u8  i;
	u8  rowData[BYTES_PER_FLASH_ROW] = {0, };
	u8  chipData[BYTES_PER_FLASH_ROW] = {0, };
	...
	for ( rowCount = 0; rowCount < numOfFlashRows; rowCount++)
	{
		...
		if (rowCount == 0)
			dev_info(g_sd->dev, "%s: row%d chip=0x%02x 0x%02x 0x%02x 0x%02x\n", __func__,
				rowCount, chipData[0], chipData[1], chipData[2], chipData[3]);
		else
		if (rowCount == (numOfFlashRows-1))
			dev_info(g_sd->dev, "%s: row%d chip=0x%02x 0x%02x 0x%02x 0x%02x\n", __func__,
				rowCount, chipData[0], chipData[1], chipData[2], chipData[3]);
...
static u8 ProgramProtectionSettings(void)
{
	...
	HEX_ReadChipProtectionData(&chipProtectionData_Hex);
	dev_dbg(g_sd->dev, "%s: chipProtectionData_Hex=%d\n", __func__,
	...
	dev_dbg(g_sd->dev, "%s: rowProtectionData0~2:0x%02x 0x%02x 0x%02x \n", __func__,
		rowProtectionData[0], rowProtectionData[1], rowProtectionData[2]);

	/* Load protection setting of current macro into volatile latch using
	   LoadLatch API */
	result = LoadLatch_protect(arrayID, &rowProtectionData[0], rowProtectionByteSize);
...
static u8 VerifyProtectionSettings(void)
{
	...
	rowProtectionByteSize = numOfFlashRows/8;
	dev_info(g_sd->dev, "%s: rowProtectionByteSize=%d\n", __func__,
		rowProtectionByteSize);
	...
	for (i = 0; i < rowProtectionByteSize; i += 4)
	{
		...
		if (i == 0)
		    dev_info(g_sd->dev, "%s: rowProtectionFlashData0~2:0x%02x 0x%02x 0x%02x \n", __func__,
				rowProtectionData[0], rowProtectionData[1], rowProtectionData[2]);
	}
	...
	HEX_ReadChipProtectionData(&chipProtectionData_Hex);
	dev_info(g_sd->dev, "%s: chipProtectionData_Hex=%d\n", __func__,
		chipProtectionData_Hex);
...
static u8 VerifyChecksum(void)
{
	...
	if (chip_Checksum != g_sd->swd_tkey_info->fw_img->checksum)
    {
        swd_PacketAck = swd_PacketAck | VERIFICATION_ERROR;
 ...
 * probe
 *****************************************************************************/
int cy8cmbr_swd_program(struct cypress_touchkey_info *info)
{
	struct swd_data *sd;
	int rc;
/*
...
*/
	...
	sd = &_swd_data;
	sd->swd_tkey_info = info;
	sd->power = info->power;
	sd->swdio_gpio = info->pdata->gpio_sda;
	sd->swdck_gpio = info->pdata->gpio_scl;
	sd->fw_img = info->fw_img->data;
	sd->fw_img_size = info->fw_img->fw_len;
	sd->dev = &info->client->dev;
	g_sd = sd;
	...
	dev_info(&info->client->dev, "%s: end, rc=%d\n", __func__, rc);
	return rc;
