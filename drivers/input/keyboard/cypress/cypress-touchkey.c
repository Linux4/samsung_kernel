		...
		size_t size)
{
	...
	if(tkey_i2c->dtdata->touchkey_led != true){
		input_err(true, &tkey_i2c->client->dev, "%s: touchkey_led not use \n", __func__);
		return size;
	}
...
static struct touchkey_devicetree_data *cypress_parse_dt(struct i2c_client *client)
{
	...
	dtdata->touchkey_led = of_property_read_bool(np, "cypress,touchkey-led");	
		...
		const struct i2c_device_id *id)
{
	...
	if( tkey_i2c->dtdata->touchkey_led )
