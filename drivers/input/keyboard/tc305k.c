			...
			struct tc300k_devicetree_data *dtdata)
{
	...
	retval = of_property_read_string(np, "coreriver,bus-reg-name", &name);
	if (retval < 0)
		dtdata->bus_reg_name = NULL;
	else
		dtdata->bus_reg_name = name;
	...
	dtdata->use_tkey_led = of_property_read_bool(np, "coreriver,use_tkey_led");
	...
	pr_info("[TK] %s : %s, scl:%d, sda:%d, irq:%d, gpio-en:%d, sub-det:%d, "
			...
			dtdata->gpio_sub_det, dtdata->fw_name, dtdata->sensing_ch_num, dtdata->use_tkey_led,
			dtdata->tsk_ic_num,
...
int tc300k_touchkey_power(struct tc300k_data *data, bool on)
{
	...
	if (data->dtdata->bus_reg_name) {
		regulator = regulator_get(&data->client->dev, "iovcc");
		if (IS_ERR(regulator)){
	 ...
	 struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = tc300k_fw_update(data, fw_path, true);
		...
		const struct i2c_device_id *id)
{
	...
	if (data->dtdata->use_tkey_led) {
		...
		if (ret)
			dev_err(&client->dev, "[TK] Failed to create brightness sysfs\n");
	}
...
static struct of_device_id coreriver_match_table[] = {
	{ .compatible = "coreriver,tc305k-keypad",},
	{ },
};
#else
