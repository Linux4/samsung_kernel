...
struct cypress_touchkey_info *copy_info;
...
*/			
	...
	//pr_err("[TKEY]%s: vdd_en g:%d, r:%d\n", __func__, info->pdata->vdd_en, ret);
	if (ret) {
								...
								bool flag)
{
	...
	if (info->pdata->vdd_led > 0){
		...
		dev_notice(&info->client->dev, "%s vdd_led:%d, on:%d", __func__, info->pdata->vdd_led, flag);
	}
}
...
static int cypress_touchkey_led_on(struct cypress_touchkey_info *info)
{
	...
	ret = i2c_smbus_write_byte_data(info->client, CYPRESS_GEN, buf);
	if (ret < 0){
		dev_info(&info->client->dev,"%s: i2c write error retry[%d]\n", __func__, ret);
		msleep(5);
		ret = i2c_smbus_write_byte_data(info->client, CYPRESS_GEN, buf);
		if(ret < 0){
			dev_info(&info->client->dev,"%s: i2c write error [%d]\n", __func__, ret);
		}
	}
	
	ret = gpio_direction_output( KEYLED_GPIO, 1);
	if (ret) {
		dev_info(&info->client->dev, "[TKEY]%s: can't control gpio [%d]\n",	__func__,KEYLED_GPIO );
	}
...
static int cypress_touchkey_led_off(struct cypress_touchkey_info *info)
{
	...
	ret = gpio_direction_output( KEYLED_GPIO, 0);
	if (ret) {
		dev_info(&info->client->dev, "[TKEY]%s: can't control gpio [%d]\n",	__func__,KEYLED_GPIO );
	}
				 ...
				 size_t size)
{
	...
	if (buf != NULL) {
		...
		dev_info(&info->client->dev, "[TouchKey] touch_led_control : %d\n", buf[0]);
	} else
			...
			struct cypress_touchkey_platform_data *pdata)
{
	...
	pdata->vdd_led = of_get_named_gpio(np, "cypress,vdd_led-gpio", 0);

	pdata->vdd_en = of_get_named_gpio(np, "cypress,vdd_en-gpio", 0);
	...
	dev_info(dev, "%s: pu:%d, led:%d, en:%d, scl:%d, sda:%d, int:%d\n", __func__, pdata->i2c_pull_up,pdata->vdd_led, pdata->vdd_en, pdata->gpio_scl, pdata->gpio_sda, pdata->gpio_int);
...
*/
	...
	ic_fw_ver = data[1];
	info->fw_id = data[5];
	dev_info(&info->client->dev, "%s: IC Version FW:0x%02x, ID:0x%02x \n", __func__, ic_fw_ver, info->fw_id);

#ifdef TKEY_FIRMUP_TRY

	if ((ic_fw_ver < BIN_FW_VERSION)||(ic_fw_ver==0x04)) {
		int retry = 2;
...
//		cypress_config_gpio_i2c(pdata, 0);
		while (retry--) {
			...
			dev_info(&client->dev, "[TouchKey] Touchkey_update failed... retry...\n");
		}
//		cypress_config_gpio_i2c(pdata, 1);
		...
		ic_fw_ver = i2c_smbus_read_byte_data(info->client, CYPRESS_FW_VER);
		dev_info(&client->dev, "[TouchKey] %s : FW Ver 0x%02x\n", __func__, ic_fw_ver);
	}	
	else {
		dev_info(&client->dev, "[TouchKey] FW update does not need!\n");
	}
#else
	dev_info(&client->dev, "[TouchKey] FW update does not try!\n");
#endif
	cypress_touchkey_auto_cal(info);
