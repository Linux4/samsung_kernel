...
#define TC350K_1KEY_DATA	0x10
#define TC350K_2KEY_DATA	0x18
#define TC350K_3KEY_DATA	0x20
#define TC350K_4KEY_DATA	0x18
#define TC350K_DATA_SIZE	0x08

#define TC350K_THR_H_OFFSET		0x00
#define TC350K_THR_L_OFFSET		0x01
#define TC350K_CH_PCK_H_OFFSET	0x02
#define TC350K_CH_PCK_L_OFFSET	0x03
#define TC350K_DIFF_H_OFFSET	0x04
#define TC350K_DIFF_L_OFFSET	0x05
#define TC350K_RAW_H_OFFSET		0x06
#define TC350K_RAW_L_OFFSET		0x07

#define TC350K_1KEY_THRES_H		0x10
#define TC350K_1KEY_THRES_L		0x11
			...
			struct tc300k_platform_data *pdata)
{
	...
	printk(KERN_DEBUG "[TK] %s: tkey_scl= %d, tkey_sda= %d, tkey_int= %d, tkey_sub_det= %d, ic = %s,led =%s,f/w=%s",
				__func__, pdata->gpio_scl, pdata->gpio_sda, pdata->gpio_int, pdata->gpio_sub_det,
				pdata->regulator_ic, pdata->regulator_led, pdata->fw_name);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if(data->pdata->tsk_ic_num == TC350K_TSK_IC) {
		ret = i2c_smbus_read_byte_data(client, TC350K_1KEY_THRES_H);
		if (ret < 0) {
		...
		ret = i2c_smbus_read_byte_data(client, TC350K_1KEY_THRES_L);
		if (ret < 0) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	if(data->pdata->tsk_ic_num == TC350K_TSK_IC) {
		ret = i2c_smbus_read_i2c_block_data(client, TC350K_1KEY_DATA, TC350K_DATA_SIZE, buff);
		if (ret != TC350K_DATA_SIZE) {
			dev_err(&client->dev, "[TK] %s read fail(%d)\n", __func__, ret);
			return -1;
		}
		value = (buff[TC350K_CH_PCK_H_OFFSET] << 8) |
			buff[TC350K_CH_PCK_L_OFFSET];
	} else {
		...
		struct device_attribute *attr, char *buf)
{
	...
	if(data->pdata->tsk_ic_num == TC350K_TSK_IC) {
		ret = i2c_smbus_read_i2c_block_data(client, TC350K_3KEY_DATA, TC350K_DATA_SIZE, buff);
		if (ret != TC350K_DATA_SIZE) {
			dev_err(&client->dev, "[TK] %s read fail(%d)\n", __func__, ret);
			return -1;
		}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if(data->pdata->tsk_ic_num == TC350K_TSK_IC) {
		ret = i2c_smbus_read_i2c_block_data(client, TC350K_2KEY_DATA, TC350K_DATA_SIZE, buff);
		if (ret != TC350K_DATA_SIZE) {
			dev_err(&client->dev, "[TK] %s read fail(%d)\n", __func__, ret);
			return -1;
		}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if(data->pdata->tsk_ic_num == TC350K_TSK_IC) {
		ret = i2c_smbus_read_i2c_block_data(client, TC350K_4KEY_DATA, TC350K_DATA_SIZE, buff);
		if (ret != TC350K_DATA_SIZE) {
			dev_err(&client->dev, "[TK] %s read fail(%d)\n", __func__, ret);
			return -1;
		}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if(data->pdata->tsk_ic_num == TC350K_TSK_IC) {
		...
		value = (buff[TC350K_RAW_H_OFFSET] << 8) |
			buff[TC350K_RAW_L_OFFSET];
	} else {
		ret = i2c_smbus_read_i2c_block_data(client, TC300K_2KEY_DATA, 6, buff);
...
static int tc300k_fw_check(struct tc300k_data *data)
{
	...
	if (ret < 0) {
		if (data->pdata->gpio_sub_det != 0 && !gpio_get_value(data->pdata->gpio_sub_det)) {
			dev_err(&client->dev,
				"[TK] %s: i2c fail, But TSK conneter is connected[%d/%d]!\n",
				__func__, data->pdata->gpio_sub_det, gpio_get_value(data->pdata->gpio_sub_det));
			data->fw_ver = 0xFF;
		} else {
			dev_err(&client->dev,
