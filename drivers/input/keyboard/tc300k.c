...
#define TC300K_FW_NAME "j1acelte.fw"
...
void tc300k_power(struct tc300k_data *data, int on)
{
	...
	regulator = regulator_get(&client->dev, "tk_vdd");
	if (IS_ERR(regulator)){
...
static int load_fw_built_in(struct tc300k_data *data)
{
	...
	ret = request_firmware(&data->fw, "coreriver/j1acelte.fw", &client->dev);
	if (ret) {
...
static int tc300k_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "[KWON] Enter probe!\n");
...
#ifdef CONFIG_PM_RUNTIME
	SET_RUNTIME_PM_OPS(tc300k_suspend, tc300k_resume, NULL)
#else
