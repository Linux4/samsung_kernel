...
#include <linux/sec_class.h>	//remove in 2016/3/15
#include <linux/wakelock.h>
...
//#define KEY_RECENT		KEY_MENU	/*add in 2016/3/15*/
...
{
	input_info(true, &data->client->dev, "%s: %s", __func__, on ? "on" : "off");
...
/*#ifdef CONFIG_SAMSUNG_PRODUCT_SHIP
			input_info(true, &client->dev, "[TK] key %s\n",
		...
		const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		dtdata = devm_kzalloc(&client->dev, sizeof(struct tc300k_devicetree_data), GFP_KERNEL);
		if (!dtdata) {
	...
	data->sec_touchkey = sec_device_create(data, "sec_touchkey");
	...
	input_err(true, &client->dev, "[TK] %s done\n", __func__);
	return 0;
