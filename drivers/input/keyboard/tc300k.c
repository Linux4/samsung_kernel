...
/*#define KEY_RECENT		254	//old keycode */
...
/*add for fb notify*/
#if defined(CONFIG_TC300K_USE_FB_NOTIFY)
#define TC300K_FB_NOTIFY
#endif
#if defined(TC300K_FB_NOTIFY)
#include <linux/fb.h>

static struct tc300k_data *local_data;
static struct work_struct tc300k_resume_work;
static struct workqueue_struct *tc300k_resume_workqueue;
static struct notifier_block tc300k_fb_notifier;
...
/* use fb_notifier */
static void tc300k_resume_workqueue_callback(struct work_struct *work)
{
	if ((local_data == NULL) || (local_data->client == NULL)) {
		pr_err("%s, local_data or local_data->client is null!\n", __func__);
		return;
	}
#ifndef FEATURE_GRIP_FOR_SAR
	tc300k_start(&(local_data->client->dev));
#endif
}
static int tc300k_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	pr_info("%s, tc300k_fb_notifier(blank=%d)\n", __func__, blank);

	switch (blank) {
	case FB_BLANK_UNBLANK:
			err = queue_work(tc300k_resume_workqueue, &tc300k_resume_work);
			if (!err) {
				pr_err("%s, queue tc300k_resume_workqueue failed err = %d!\n", __func__, err);
				return err;
			}
		break;
	case FB_BLANK_POWERDOWN:
			err = cancel_work_sync(&tc300k_resume_work);
			if (err)
				pr_err("%s, cancel tc300k_resume_workqueue failed ,err = %d!\n", __func__, err);
#ifndef FEATURE_GRIP_FOR_SAR
			tc300k_stop(&(local_data->client->dev));
#endif
		break;
	default:
		break;
...
#if defined(CONFIG_DRV_SAMSUNG) | defined(FEATURE_GRIP_FOR_SAR)
...
#if defined(CONFIG_DRV_SAMSUNG)
	data->sec_touchkey = sec_device_create(0, data, "sec_touchkey");
...
#if defined(TC300K_FB_NOTIFY)
	local_data = data;
	tc300k_resume_workqueue = create_singlethread_workqueue("tc300k_resume");
	INIT_WORK(&tc300k_resume_work, tc300k_resume_workqueue_callback);
	/* use fb_notifier */
	tc300k_fb_notifier.notifier_call = tc300k_fb_notifier_callback;
	if (fb_register_client(&tc300k_fb_notifier))
		dev_err(&client->dev, "register fb_notifier fail!\n");
#endif
...
#ifdef CONFIG_OF
static const struct of_device_id coreriver_match_table[] = {
