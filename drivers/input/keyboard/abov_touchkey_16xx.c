...
#define LIGHT_VOLTAGE_MIN_VAL		2800
...
static int touchkey_led_set(struct abov_ft1604_info *info, int data)
{
	...
	if (info->dtdata->vdd_led > 0) {
		if(data == 1) {
			...
			dev_err(&info->client->dev, "%s data param is wrong value: %d", __func__, data);
		if(ret < 0)
			dev_err(&info->client->dev, "%s fail to set vdd_led(%d), ret(%d)", __func__, data, ret);
	}
			...
			struct abov_ft1604_devicetree_data *dtdata)
{
	...
	if (IS_ERR_OR_NULL(dtdata->vdd_led)) {
		dtdata->vdd_led = NULL;
		dev_err(dev, "vdd_led is not used for PMIC LDO.\n");
	} else {
