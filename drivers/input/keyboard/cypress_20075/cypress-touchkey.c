...
static int touchkey_power_on(void *data, bool on)
{
	...
	regulator_core = regulator_get(dev, TK_CORE_REGULATOR_NAME);
	if (IS_ERR_OR_NULL(regulator_core)) {
		ret = PTR_ERR(regulator_core);
		tk_debug_err(true, dev,
			"%s : TK regulator_get failed(%d)\n", __func__, ret);
		goto out;
	}
...
static void cypress_request_gpio(struct i2c_client *client, struct touchkey_platform_data *pdata)
{
	...
	if (!pdata->i2c_gpio) {
		...
		if (ret) {
			tk_debug_err(true, &client->dev,
				"%s: unable to request touchkey_scl [%d] (%d)\n",
					__func__, pdata->gpio_scl, ret);
			return;
		}

		ret = gpio_request(pdata->gpio_sda, "touchkey_sda");
		if (ret) {
			tk_debug_err(true, &client->dev,"%s: unable to request touchkey_sda [%d] (%d)\n",
					__func__, pdata->gpio_sda, ret);
			return;
		}
	}
	...
	if (ret) {
		tk_debug_err(true, &client->dev,"%s: unable to request touchkey_irq [%d] (%d)\n",
				__func__, pdata->gpio_int, ret);
		return;
	}
}
