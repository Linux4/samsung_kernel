...
#elif defined(CONFIG_MACH_HLLTE) ||	defined(CONFIG_MACH_HL3G) || defined(CONFIG_KEYBOARD_CYPRESS_TKEY_HL)
...
static int touchkey_power_on(bool on)
{
	...
	if (on) {
		gpio_direction_output(tkey_gpio_int, 1);
...
static int cypress_ldo_onoff(bool on)
{
	...
	if(!tkey_regulator){
		tkey_regulator = regulator_get(NULL, TK_REGULATOR_NAME);
...
static int touchkey_led_power_on(bool on)
{
	...
	if(!tkey_led_regulator){
		tkey_led_regulator = regulator_get(NULL, TK_LED_REGULATOR_NAME);
...
#endif
/*	if (pdata->vdd_led > 0) {
		ret = gpio_request(pdata->vdd_led, "vdd_led");
		if (ret) {
			printk(KERN_ERR "%s: unable to request vdd_led [%d]\n", __func__, pdata->vdd_led);
		}
	} */
/*
	if (pdata->gpio_scl > 0) {
		ret = gpio_request(pdata->gpio_scl, "gpio_scl");
		if (ret) {
			printk(KERN_ERR "%s: unable to request gpio_scl [%d]\n", __func__, pdata->gpio_scl);
		}
	}

	if (pdata->gpio_sda > 0) {
		ret = gpio_request(pdata->gpio_sda, "gpio_sda");
		if (ret) {
			printk(KERN_ERR "%s: unable to request gpio_sda [%d]\n", __func__, pdata->gpio_sda);
		}
	}
*/	
	...
	const struct i2c_device_id *id)
{
	...
	struct touchkey_i2c *tkey_i2c;
	#if defined(TK_USE_LCDTYPE_CHECK) || defined(CONFIG_KEYBOARD_CYPRESS_TKEY_HL)
	bool bforced = false;
	#endif
...
/*	pdata->register_cb = touchkey_register_callback; */
#ifdef GPIO_2TOUCH_RST
	//pdata->gpio_rst = GPIO_2TOUCH_RST;
