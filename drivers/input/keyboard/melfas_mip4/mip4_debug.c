...
int mip4_tk_dev_create(struct mip4_tk_info *info)
{
	...
	if (alloc_chrdev_region(&info->mip4_tk_dev, 0, 1, MIP_DEVICE_NAME)) {
...
* Sysfs run test
*/
static ssize_t mip4_tk_sys_test(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mip4_tk_info *info = dev_get_drvdata(dev);
	int ret;
	u8 test_type;
...
* Sysfs functions
*/
static DEVICE_ATTR(fw_version, 0666, mip4_tk_sys_fw_version, NULL);
static DEVICE_ATTR(fw_path, 0666, mip4_tk_sys_fw_path_show, mip4_tk_sys_fw_path_store);
static DEVICE_ATTR(info, 0666, mip4_tk_sys_info, NULL);
#ifdef DEBUG
static DEVICE_ATTR(power_on, 0666, mip4_tk_sys_power_on, NULL);
static DEVICE_ATTR(power_off, 0666, mip4_tk_sys_power_off, NULL);
static DEVICE_ATTR(reboot, 0666, mip4_tk_sys_reboot, NULL);
#endif
static DEVICE_ATTR(mode_glove, 0666, mip4_tk_sys_mode_show, mip4_tk_sys_mode_store);
static DEVICE_ATTR(mode_charger, 0666, mip4_tk_sys_mode_show, mip4_tk_sys_mode_store);
static DEVICE_ATTR(image_intensity, 0666, mip4_tk_sys_image, NULL);
static DEVICE_ATTR(image_rawdata, 0666, mip4_tk_sys_image, NULL);
static DEVICE_ATTR(test_cm, 0666, mip4_tk_sys_test, NULL);
static DEVICE_ATTR(test_cm_jitter, 0666, mip4_tk_sys_test, NULL);
