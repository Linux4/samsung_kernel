...
static bool flash_write_w9019(struct wacom_i2c *wac_i2c,
			      ...
			      unsigned long *max_address)
{
	...
	for (ulAddress = start_address; ulAddress < *max_address;
		...
		bRet =
		    flash_write_block_w9019(wac_i2c, flash_data, ulAddress,
					    &command_id, &ECH);
		if (!bRet)
			return false;
		if (ECH_len == 3)
...
int wacom_i2c_flash_w9019(struct wacom_i2c *wac_i2c, unsigned char *fw_data)
{
	...
	unsigned long max_address = W9019_END_ADDR;			/* Max.address of Load data */
	unsigned long start_address = W9019_START_ADDR;	        /* Start.address of Load data */
	...
	/*Erase the old program*/
			input_err(true, &client->dev, "%s erasing the current firmware \n", __func__);
#ifdef PARTIAL_ERASE
	bRet = flash_erase_w9019(wac_i2c, eraseBlock,  eraseBlockNum);
	if (!bRet) {
			input_err(true, &client->dev, "%s failed to erase the user program \n", __func__);
	...
	input_info(true, &client->dev, "%s writing new firmware\n",
		   __func__);
	bRet =
	    flash_write_w9019(wac_i2c, wac_i2c->fw_data, start_address,
			      &max_address);
	if (!bRet) {
		input_err(true, &client->dev,
