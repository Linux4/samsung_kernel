...
*/
	input_info(true, &wac_i2c->client->dev, "%s: QUERY SUCCEEDED\n", __func__);
...
static int wacom_i2c_flash_w9020(struct wacom_i2c *wac_i2c, unsigned char *fw_data)
{
	...
	input_info(true, &client->dev, "%s: BL version: %x\n", __func__, iBLVer);
	...
	input_info(true, &client->dev, "%s: MPU type: %x\n", __func__, iMpuType);
