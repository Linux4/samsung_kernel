...
#define MPS_FILEPATH		"efs/imei/mps_code.dat"
...
static void aw9610x_get_calidata(struct aw9610x *aw9610x)
{
	...
	for (i = AW_CLA1_SPE_REG_NUM; i < AW_SPE_REG_NUM; i++) {
		...
		AWLOGE(aw9610x->dev, "spedata[%d] = %d\n", i, aw9610x->spedata[i]);
	}
...
#ifdef ANFR_FLAG 
static void aw9610x_irq_counter(struct aw9610x *aw9610x)
{
	int i = 0;
	int value = 0;
	uint32_t temp = 0;
	uint32_t curr_status = 0;
	static uint32_t last_status[AW_CHANNEL_MAX] = {0};
	aw9610x_i2c_read(aw9610x, REG_STAT0, &temp);
	for (i = 0; i < AW_CHANNEL_MAX; i++) {
        if(aw9610x->channel_func & (1 << i)){
			curr_status = (((uint8_t)(temp >> (24 + i)) & 0x1)) |
				(((uint8_t)(temp >> (16 + i)) & 0x1) << 1) |
				(((uint8_t)(temp >> (8 + i)) & 0x1) << 2) |
				(((uint8_t)(temp >> (i)) & 0x1) << 3);
			AWLOGI(aw9610x->dev, "curr_status[%d] = 0x%x", i, curr_status);
			if ((curr_status != last_status[i]) && (aw9610x->enable_flag & (1 << i))) {
				value = 1;
				last_status[i] = curr_status;
				break;
			}
			last_status[i] = curr_status;
		}
...
static void aw9610x_diff_read(struct aw9610x *aw9610x)
{
	int i = 0;
	int32_t diff_reg_val[AW_CHANNEL_MAX] = {0};
	uint32_t data_en = 0;
	for(i = 0; i < AW_CHANNEL_MAX; i++){
		aw9610x_i2c_read(aw9610x, REG_DIFF_CH0 + i * 4, &diff_reg_val[i]);
		diff_reg_val[i] /= AW_DATA_PROCESS_FACTOR;
	}
	if(diff_reg_val[0] < -5000 || diff_reg_val[2] < -5000 || diff_reg_val[3] < -5000){
		aw9610x_i2c_read(aw9610x, REG_SCANCTRL0, &data_en);
		aw9610x_i2c_write_bits(aw9610x, REG_SCANCTRL0, ~(0x3f << 8),
					(data_en & 0x3f) << 8);
	}
}
int aw9610x_first_report(struct aw9610x *aw9610x)
{
	uint8_t i = 0;
	uint8_t j = aw9610x->sar_num;
	uint32_t reg_val = 0;
	static uint16_t ch_result[AW_CHANNEL_MAX] = {0};
	if(aw9610x->aw_first_boot){
		for(i = 0; i < AW_CHANNEL_MAX; i++){
			if(aw9610x->channel_func & (1 << i)){
				aw9610x_diff_read(aw9610x);
				aw9610x_i2c_read(aw9610x, REG_AFECFG1_CH0 + i * 20, &reg_val);
				ch_result[i] = reg_val >> 16;
				AWLOGI(aw9610x->dev, "ch_result[%d] = %d", i, ch_result[i]);
				AWLOGI(aw9610x->dev, "back_cap[%d] = %d", i, 
								aw9610x->back_cap[i]);
#ifdef USE_USB_CHANGE
				AWLOGI(aw9610x->dev, "ps_is_present = %d", i,
						aw9610x->ps_is_present);
#endif
			}
			
		}
#ifdef USE_USB_CHANGE
	if(!aw9610x->ps_is_present){
#endif
		if((aw9610x->interrupt_count <= MAX_INT_COUNT) &&
			(ch_result[0]+10 >= aw9610x->back_cap[0]) &&
			(ch_result[2]+10 >= aw9610x->back_cap[2]) &&
			(ch_result[3]+10 >= aw9610x->back_cap[3])){
			AWLOGI(aw9610x->dev, "channel State=Near");
			AWLOGI(aw9610x->dev, "interrupt_count = %d",
						aw9610x->interrupt_count);
			for(i = 0; i < AW_CHANNEL_MAX; i++){
				if((aw9610x->channel_func & (1 << i)) &&
...
#ifdef ANFR_FLAG 
	aw9610x_irq_counter(aw9610x);
	ret = aw9610x_first_report(aw9610x);
	if(ret){
		AWLOGE(aw9610x->dev, "first report is near\n");
		return;
	}
#endif
...
static int aw9610x_ps_get_state(struct power_supply *psy, bool *present)
{
		...
		unsigned long event, void *p)
{
	...
	if (event == PSY_EVENT_PROP_CHANGED
		...
		retval = aw9610x_ps_get_state(psy, &present);
		if (retval) {
		...
		if (this->aw_first_boot && this->ps_is_present && this->channel_func)
        {
            aw9610x_irq_handle(this);
        }
	}
	return 0;
}
    ...
    if(!strncmp(temp_input_dev->name,"grip_sensor",sizeof("grip_sensor")))
    {
        if (!strncmp(buf, "1", 1))
        {
			...
			if(this->aw_first_boot){
				...
				schedule_delayed_work(&this->report_ch0_work, msecs_to_jiffies(50));
				LOG_INFO("anfr:channels[0] enable input near\n");
			}else{
				input_report_rel(this->aw_pad[0].input, REL_MISC, 2);
			}
#else
    ...
    if(!strncmp(temp_input_dev->name,"grip_sensor_sub",sizeof("grip_sensor_sub")))
    {
        if (!strncmp(buf, "1", 1))
        {
			...
			if(this->aw_first_boot){
				...
				schedule_delayed_work(&this->report_ch2_work, msecs_to_jiffies(50));
				LOG_INFO("anfr:channels[2] enable input near\n");
			}else{
				input_report_rel(this->aw_pad[2].input, REL_MISC, 2);
			}
#else
    ...
    if(!strncmp(temp_input_dev->name,"grip_sensor_wifi",sizeof("grip_sensor_wifi")))
    {
        if (!strncmp(buf, "1", 1))
        {
			...
			if(this->aw_first_boot){
				...
				schedule_delayed_work(&this->report_ch3_work, msecs_to_jiffies(50));
				LOG_INFO("anfr:channels[3] enable input near\n");
			}else{
				input_report_rel(this->aw_pad[3].input, REL_MISC, 2);
			}
#else
...
/* ---for factory mode use,libo7.wt */
#ifdef ANFR_FLAG
static void aw9610x_report_ch0_work_func(struct work_struct *work)
{
	uint32_t reg_val = 0;
	aw9610x_t this = aw9610x_ptr;
	//ch0
	aw9610x_i2c_read(this, REG_AFECFG1_CH0, &reg_val);
	this->back_cap[0] = reg_val>>16;
	input_report_rel(this->aw_pad[0].input, REL_MISC, 1);
	input_sync(this->aw_pad[0].input);
	AWLOGE(this->dev,"back_cap = %d\n", this->back_cap[0]);
    return;
}
static void aw9610x_report_ch2_work_func(struct work_struct *work)
{
	uint32_t reg_val = 0;
	aw9610x_t this = aw9610x_ptr;
	//ch2
	aw9610x_i2c_read(this, REG_AFECFG1_CH0 + 40, &reg_val);
	this->back_cap[2] = reg_val>>16;
	input_report_rel(this->aw_pad[2].input, REL_MISC, 1);
	input_sync(this->aw_pad[2].input);
	AWLOGE(this->dev,"back_cap = %d\n",this->back_cap[2]);
    return;
}
static void aw9610x_report_ch3_work_func(struct work_struct *work)
{
	uint32_t reg_val = 0;
	aw9610x_t this = aw9610x_ptr;
	//ch3
	aw9610x_i2c_read(this, REG_AFECFG1_CH0 + 60, &reg_val);
	this->back_cap[3] = reg_val>>16;
	input_report_rel(this->aw_pad[3].input, REL_MISC, 1);
	input_sync(this->aw_pad[3].input);
	AWLOGE(this->dev,"back_cap = %d\n", this->back_cap[3]);
	AWLOGE(this->dev,"reg:back_cap = 0x%X\n", reg_val);
    return;
}
#endif
static int32_t
...
#ifdef ANFR_FLAG 
	INIT_DELAYED_WORK(&aw9610x->report_ch0_work, aw9610x_report_ch0_work_func);
	INIT_DELAYED_WORK(&aw9610x->report_ch2_work, aw9610x_report_ch2_work_func);
	INIT_DELAYED_WORK(&aw9610x->report_ch3_work, aw9610x_report_ch3_work_func);
	aw9610x->aw_first_boot = true;
	aw9610x->interrupt_count = 0;
#endif
#ifdef USE_USB_CHANGE
...
#ifdef USE_USB_CHANGE
free_ps_notifier:
    AWLOGE(aw9610x->dev, "%s free ps notifier:.\n", __func__);
