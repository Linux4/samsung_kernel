...
*/
	...
	if (!strncmp(pdata->project_name, "J7max", 5)) {
		if (rmi4_data->opcode_changed) {
			input_info(true, &rmi4_data->i2c_client->dev, "%s: OPCODE changed firmware\n",
				__func__);
			if (rmi4_data->fw_version_of_bin != rmi4_data->fw_version_of_ic) {
				input_info(true, &rmi4_data->i2c_client->dev, "%s: OPCODE: need to update\n",
					__func__);
				goto out;;
			} else {
				input_info(true, &rmi4_data->i2c_client->dev, "%s: OPCODE: do not need to update\n",
					__func__);
				return true;
			}
		}
...
static void fw_update(void *dev_data)
{
	...
	if (sec->cmd_param[0] == 1) {
		...
		input_info(true, &rmi4_data->i2c_client->dev, "%s: user_ship, success [%d]\n", __func__, retval);
		return;
	}
#endif
