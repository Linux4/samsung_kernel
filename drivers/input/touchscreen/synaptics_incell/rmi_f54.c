...
static int tddi_ratio_calculation(struct synaptics_rmi4_data *rmi4_data, signed short *p_image)
{
	...
	int tx_num = f54->tx_assigned;
	int rx_num = f54->rx_assigned;
	unsigned char left_size = 8;
	unsigned char right_size = 8;
	signed short *p_data_16;
	signed short *p_left_median = NULL;
	signed short *p_right_median = NULL;
	signed short *p_left_column_buf = NULL;
...
static void  run_elec_open_test(void *dev_data)
{
	...
	unsigned char ctrl182_data[4] = {0x00, 0x00, 0x00, 0x00};
	...
	retval = rmi4_data->i2c_read(rmi4_data,
			0x013B, //control.reg_182->address,
	...
	ctrl182_data[0] = ELEC_OPEN_TEST_TX_ON_COUNT;
	ctrl182_data[1] = (ELEC_OPEN_TEST_TX_ON_COUNT >> 8) & 0xff;
	ctrl182_data[2] = ELEC_OPEN_TEST_RX_ON_COUNT;
	ctrl182_data[3] = (ELEC_OPEN_TEST_RX_ON_COUNT >> 8) & 0xff;
	...
	input_err(true, &rmi4_data->i2c_client->dev, "delta ratio cal average = %d\n", average2);
#endif

	input_err(true, &rmi4_data->i2c_client->dev, "min = %d, max = %d\n", min, max);
	input_err(true, &rmi4_data->i2c_client->dev, "delta average = %d\n", average);

	snprintf(data->cmd_buff, CMD_RESULT_STR_LEN, "%d", average);
	cmd_state = CMD_STATUS_OK;
