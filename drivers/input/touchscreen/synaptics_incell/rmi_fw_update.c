...
void fwu_parse_image_header_0506_simple(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmi4_data->opcode_changed = (fwu->img_data.ui_config.data[1] >> 5);

	input_err(true, &rmi4_data->i2c_client->dev, "%s: month:%02X, date:%02X, hwrev:%02X, fwrev:%02X \n",
			__func__, fwu->img_data.ui_config.data[0], fwu->img_data.ui_config.data[1],
			fwu->img_data.ui_config.data[2], fwu->img_data.ui_config.data[3]);
