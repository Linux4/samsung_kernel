...
#define SHORT_TEST_CSV_FILE "/data/tpdata/ShortTest.csv"
#define OPEN_TEST_CSV_FILE "/data/tpdata/OpenTest.csv"
#define FW_RAWDATA_CSV_FILE "/data/tpdata/FWMutualTest.csv"
#define FW_CC_CSV_FILE "/data/tpdata/FWCCTest.csv"
#define NOISE_TEST_CSV_FILE "/data/tpdata/NoiseTest.csv"
...
static int32_t nvt_selftest_for_hq(void)
{
	struct device_node *np = ts->client->dev.of_node;
	unsigned char mpcriteria[32] = {0};	//novatek-mp-criteria-default
	int8_t nvt_selftest_result = -1;
...
static ssize_t nvt_test_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	int count = 0;
	int ret = 0;
...
static ssize_t nvt_test_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	return count;
}

static DEVICE_ATTR(factory_check, 0644, nvt_test_show, nvt_test_store);

static struct attribute *nvt_test_attributes[] ={
	&dev_attr_factory_check.attr,
	NULL
};

static struct attribute_group nvt_test_attribute_group = {
.attrs = nvt_test_attributes
};

int nvt_test_node_init(struct platform_device *tpinfo_device)
{
    int err=0;
    err = sysfs_create_group(&tpinfo_device->dev.kobj, &nvt_test_attribute_group);
    if (0 != err)
    {
        NVT_ERR( "ERROR: ITO node create failed.");
        sysfs_remove_group(&tpinfo_device->dev.kobj, &nvt_test_attribute_group);
        return -EIO;
    }
    else
    {
        NVT_LOG("ITO node create_succeeded.");
    }
    return err;
}
