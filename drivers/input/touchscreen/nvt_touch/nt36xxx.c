...
static int nvt_parse_dt(struct device *dev)
{
	...
	ts->reset_gpio = of_get_named_gpio_flags(np, "rst-gpio", 0, &ts->reset_flags);
	input_info(true, dev, "%s: rst-gpio=%d\n", __func__, ts->reset_gpio);
#endif
	platdata->irq_gpio = of_get_named_gpio_flags(np, "irq-gpio", 0, &platdata->irq_flags);
	...
	input_info(true, dev, "%s: irq-gpio=%d\n", __func__, platdata->irq_gpio);
