		...
		struct device_node *core_node)
{
	...
	pdata->pwr_1p8 = of_get_named_gpio(core_node,
				"cy,pwr_1p8", 0);

	rc = of_property_read_u32(core_node,
				"cy,vdd_volt_3p3", &pdata->vdd_volt_3p3);
	if (rc) {
		pr_err("Failed to get property vdd_volt_3p3 from node (%d)\n", rc);
		goto fail_free;
	}
...
int cyttsp5_devtree_create_and_get_pdata(struct device *adap_dev)
{
	...
	for_each_child_of_node(adap_dev->of_node, core_node) {
		...
		if(system_rev > 4)
			_cyttsp5_loader_platform_data.fw = &cyttsp5_firmware_hw03;
		else
			_cyttsp5_loader_platform_data.fw = &cyttsp5_firmware;
