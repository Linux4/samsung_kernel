...
//		return retval;
	...
	if (on) {
		...
		usleep(500);
		gpio_direction_output(pdata->io_en, 1);
		pinctrl_state = pinctrl_lookup_state(rmi4_data->pinctrl, "on_state");
	} else {
		gpio_direction_output(pdata->io_en, 0);
...
static int synaptics_parse_dt(struct i2c_client *client)
{
	...
	if (gpio_is_valid(pdata->gpio)) {
		retval = gpio_request(pdata->gpio, "synaptics,tsp_int");
		if (retval) {
	...
	pdata->vdd_en = of_get_named_gpio(np, "synaptics,vdd_en", 0);
	if (gpio_is_valid(pdata->vdd_en)) {
		retval = gpio_request(pdata->vdd_en, "synaptics,vdd_en");
		if (retval) {
			tsp_debug_err(true, dev, "Unable to request vdd_en [%d]\n", pdata->vdd_en);
			return -EINVAL;
		} 
	} else {
	...
	pdata->io_en = of_get_named_gpio(np, "synaptics,io_en", 0);
	if (gpio_is_valid(pdata->io_en)) {
		retval = gpio_request(pdata->io_en, "synaptics,io_en");
		if (retval) {
			tsp_debug_err(true, dev, "Unable to request io_en [%d]\n", pdata->io_en);
			return -EINVAL;
		} 
	} else {
		tsp_debug_err(true, dev, "Failed to get io_en gpio\n");
		return -EINVAL;
	}
	...
	if (of_property_read_string(np, "synaptics,factory_firmware_name", &pdata->factory_firmware_name))
		tsp_debug_err(true, dev, "Failed to get factory firmware name\n");
	...
	tsp_debug_info(true, dev, "num_of[rx,tx]: [%d,%d], max[x,y]: [%d,%d], vdd,io[%d,%d] max_width: %d, flip[x,y,x_y,x_offset]:[%d,%d,%d %d]",
			pdata->num_of_rx, pdata->num_of_tx, pdata->sensor_max_x, pdata->sensor_max_y, pdata->vdd_en, pdata->io_en,
			pdata->max_touch_width, pdata->x_flip, pdata->y_flip, pdata->x_y_chnage, pdata->x_offset);
...
//	synaptics_rmi4_stop_device(rmi4_data);
	...
	if(rmi4_data->save_input_open){
		tsp_debug_err(true, &rmi4_data->i2c_client->dev,"%s, re input open, %d", __func__, rmi4_data->save_input_open);
...
static int synaptics_rmi4_input_open(struct input_dev *dev)
{
	...
	if (!rmi4_data->init_done.done) {
		tsp_debug_err(true, &rmi4_data->i2c_client->dev,"%s, not done probe, %d", __func__, rmi4_data->init_done.done);
	...
	tsp_debug_info(false, &rmi4_data->i2c_client->dev, "%s\n", __func__);
