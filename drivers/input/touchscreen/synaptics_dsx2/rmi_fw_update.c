...
static int fwu_read_f34_v7_partition_table(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = fwu_wait_for_idle(rmi4_data, WRITE_WAIT_MS);//fwu_wait_for_idle(WRITE_WAIT_MS, true);
...
//	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
	...
	do {
		...
		retval = fwu_write_f34_command_v7(rmi4_data, command);//fwu_write_f34_command(command);
		if (retval < 0) {
			tsp_debug_err(true, &rmi4_data->i2c_client->dev,
...
#else
	if (flash_area != NONE) {
		retval = fwu_enter_flash_prog_v7(rmi4_data);
		if (retval < 0) {
//			rmi4_data->reset_device(rmi4_data, false);
			tsp_debug_err(false, &rmi4_data->i2c_client->dev,
						"%s: Failed to enter fwu_enter_flash_prog_v7()\n",
						__func__);
			goto exit;
		}
	}
