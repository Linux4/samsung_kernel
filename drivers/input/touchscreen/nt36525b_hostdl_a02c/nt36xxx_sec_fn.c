...
static void set_game_mode(void *device_data)
{
	...
	if (reti) {
		input_err(true, &ts->client->dev, "failed to switch game mode - 0x%02X\n", buf[3]);
		goto out;
	}
...
static void prox_lp_scan_mode(void *device_data)
{
	...
	if (sec->cmd_param[0] < 0 || sec->cmd_param[0] > 1) {
		...
		mode = sec->cmd_param[0] ? SLEEP_IN: SLEEP_OUT;
	}
	...
	input_info(true, &ts->client->dev, "%s,%s: %s\n", __func__, (sec->cmd_param[0])?"SLEEP_IN":"SLEEP_OUT", buff);
...
*/
	...
	input_info(true, &ts->client->dev, "%s: %s %s\n", __func__, (ts->aot_enable)?"ON":"OFF", buff);
...
static void run_prox_intensity_read_all(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "SUM_X:%d THD_X:%d", sum, 0);
