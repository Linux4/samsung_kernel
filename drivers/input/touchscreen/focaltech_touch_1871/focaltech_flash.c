...
/* HS70 add for HS70-407 by gaozhengwei at 2019/10/29 start */
    if ((chip_id->rom_idh == id[0]) && (chip_id->rom_idl == id[1])) {
        return 0;
    }
    FTS_INFO("chip_id:0x%02x 0x%02x", chip_id->rom_idh, chip_id->rom_idl);
/* HS70 add for HS70-407 by gaozhengwei at 2019/10/29 end */
    return -EIO;
}
...
static int fts_fwupg_get_module_info(struct fts_upgrade *upg)
{
    ...
    if (NULL != strstr(fts_data->pdata->panel_name, "mdss_dsi_boe_ft8165_720p_video")) {
        upg->module_id = 0x10;
        strcpy(fts_data->pdata->module_name,"ft8615_hlt");
        FTS_INFO("the specified display module is HLT-BOE\n");
    }
    else if(NULL != strstr(fts_data->pdata->panel_name, "mdss_dsi_qc_ft8165_hd_video")) {
        upg->module_id = 0x40;
    ...
    /*HS70 code for HLT-CDH bringup by zhouzichun at 2020/3/20 end*/
        ...
        upg->module_id = 0x40;
        strcpy(fts_data->pdata->module_name,"ft8615_inx");
        FTS_ERROR("can't find the specified display module .\n");
    }
    #ifdef CONFIG_HW_INFO
    set_tp_module_name(fts_data->pdata->module_name);
    #endif
