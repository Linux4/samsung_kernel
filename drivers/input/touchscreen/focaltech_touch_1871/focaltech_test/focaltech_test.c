...
/* HS70 add for HS70-363 by gaozhengwei at 2019/11/21 start */
#include <linux/time.h>
#include <linux/rtc.h>
/* HS70 add for HS70-363 by gaozhengwei at 2019/11/21 end */
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    if ( fwupgrade != NULL && fwupgrade->module_info != NULL) {
        sprintf(filepath, "%s%s_%s%s", FTS_INI_FILE_PATH, get_date_time_str(), fwupgrade->module_info->vendor_name, file_name);
    } else {
...
static void fts_test_save_data_csv(struct fts_test *tdata)
{
    ...
    /* HS70 add for HS70-363 by gaozhengwei at 2019/11/21 start */
    switch (qc_ito_TestResult_fts) {
    case 0:
        fts_test_save_test_data(FTS_CSV_FAIL_FILE_NAME, csv_buffer, csv_length);
        break;
    case 1:
        fts_test_save_test_data(FTS_CSV_PASS_FILE_NAME, csv_buffer, csv_length);
        break;
    default:
        break;
    }
...
static void fts_test_save_result_txt(struct fts_test *tdata)
{
    ...
    switch (qc_ito_TestResult_fts) {
    case 0:
        fts_test_save_test_data(FTS_TXT_FAIL_FILE_NAME, tdata->testresult,
				tdata->testresult_len);
        break;
    case 1:
        fts_test_save_test_data(FTS_TXT_PASS_FILE_NAME, tdata->testresult,
				tdata->testresult_len);
        break;
    default:
        break;
...
/*
 * qc_fts_test_entry - test main entry
 *
...
int qc_fts_test_entry(char *ini_file_name)
{
    int ret = 0;
   ...
   /* Start testing according to the test configuration */
    if (true == fts_test_start()) {
        qc_ito_TestResult_fts = 1;
        FTS_TEST_SAVE_INFO("\n\n=======Tp test pass.\n");
    } else {
        qc_ito_TestResult_fts = 0;
        FTS_TEST_SAVE_INFO("\n\n=======Tp test failure.\n");
    }
    FTS_INFO("Ito test result = %d \n",qc_ito_TestResult_fts);
...
int qc_fts_test_init(struct fts_ts_data *ts_data)
{
    int ret = 0;
...
int qc_fts_test_exit(struct fts_ts_data *ts_data)
{
    FTS_TEST_FUNC_ENTER();
