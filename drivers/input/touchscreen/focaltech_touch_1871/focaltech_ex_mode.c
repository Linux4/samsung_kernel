...
int fts_ex_mode_switch(enum _ex_mode mode, u8 value)
{
    ...
    case MODE_HIGH_SENSITIVITY:
        ret = fts_write_reg(FTS_REG_HIGH_SENSITIVITY_MODE_EN, m_val);
        if (ret < 0) {
            FTS_ERROR("MODE_HIGH_SENSITIVITY switch to %d fail", m_val);
        }
        break;
...
/* HS70 add for HS70-1379 by gaozhengwei at 2019/11/24 start */
static ssize_t fts_high_sensitivity_mode_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    u8 val = 0;
    ...
    mutex_lock(&input_dev->mutex);
    fts_read_reg(FTS_REG_HIGH_SENSITIVITY_MODE_EN, &val);
    count = snprintf(buf + count, PAGE_SIZE, "High Sensitivity Mode:%s\n",
                     ts_data->high_sensitivity_mode ? "On" : "Off");
    count += snprintf(buf + count, PAGE_SIZE, "High Sensitivity Reg(0xC8):%d\n", val);
    mutex_unlock(&input_dev->mutex);

    return count;
}

static ssize_t fts_high_sensitivity_mode_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = 0;
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        ...
        if (ts_data->high_sensitivity_mode) {
            ...
            if (ret >= 0) {
                ts_data->high_sensitivity_mode = DISABLE;
            }
        }
    }

    FTS_DEBUG("high sensitivity mode:%d", ts_data->high_sensitivity_mode);
    return count;
}
/* HS70 add for HS70-1379 by gaozhengwei at 2019/11/24 end */
...
/* HS70 add for HS70-1379 by gaozhengwei at 2019/11/24 start */
static DEVICE_ATTR(fts_high_sensitivity_mode, S_IRUGO | S_IWUSR,
                   fts_high_sensitivity_mode_show, fts_high_sensitivity_mode_store);
/* HS70 add for HS70-1379 by gaozhengwei at 2019/11/24 end */
static struct attribute *fts_touch_mode_attrs[] = {
    ...
    /* HS70 add for HS70-1379 by gaozhengwei at 2019/11/24 start */
    &dev_attr_fts_high_sensitivity_mode.attr,
    /* HS70 add for HS70-1379 by gaozhengwei at 2019/11/24 end */
    NULL,
};
...
int fts_ex_mode_recovery(struct fts_ts_data *ts_data)
{
    ...
    /* HS70 add for HS70-1379 by gaozhengwei at 2019/11/24 start */
    if (ts_data->high_sensitivity_mode) {
        fts_ex_mode_switch(MODE_HIGH_SENSITIVITY, ENABLE);
    }
    /* HS70 add for HS70-1379 by gaozhengwei at 2019/11/24 end */
