...
static int fts_enter_into_boot(void)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		sec_delay(upg->setting_nf->delay_init);
...
int fts_enter_test_environment(bool test_state)
{
	...
	if(upg->ts_data->set_test_fw == test_state) {
		FTS_ERROR("enter test environment value is same(%d,%d) skip.",upg->ts_data->set_test_fw, test_state);
		return -EINVAL;
	}
	...
	upg->ts_data->set_test_fw = test_state;
...
int fts_download_ramtest_bin(void)
{
	...
	snprintf(fwname, FILE_NAME_LENGTH, "%s", upg->ts_data->pdata->ramtest_name);
	...
	if (i >= 3) {
		FTS_ERROR("ramtest FW download fail");
		release_firmware(fw);
		return -EIO;
	}
...
static int fts_get_fw_file_from_sdcard(struct fts_upgrade *upg)
{
	int ret = 0;
	char fwname[FILE_NAME_LENGTH] = { 0 };
...
static int fts_fwupg_get_fw_file(struct fts_upgrade *upg, int update_type)
{
	...
	if (FTS_FW_REQUEST_SUPPORT) {
		...
		if (update_type == TSP_BUILT_IN)
			...
			ret = fts_get_fw_file_from_sdcard(upg);
	}
...
int fts_fwupg_func(int update_type, bool force)
{
	...
	FTS_INFO("update_type = %s, force = %d", update_type == TSP_BUILT_IN ? "built-in" : "sdcard", force);
	if (!upg || !upg->ts_data) {
		FTS_ERROR("upg/ts_data is null");
		return -ENOMEM;
	}
...
{
	fts_fwupg_func(TSP_BUILT_IN, true);
}


int fts_fwupg_init_download(struct fts_upgrade *upg)
{
	int ret = 0;
	int i = 0;
	u8 chip_id = 0;
	...
	/* get fw */
	ret = fts_fwupg_get_fw_file(upg, TSP_BUILT_IN);
	if (ret < 0) {
		FTS_ERROR("get file fail, can't upgrade");
		return ret;
	}
	...
	for (i = 0; i < 3; i++) {
		FTS_INFO("fw download times:%d", i + 1);
		ret = fts_fw_write_start(upg->fw, upg->fw_length, true);
		if (0 == ret)
		break;
	}
	upg->ts_data->fw_loading = 0;
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
	...
	return fts_fwupg_init_download(fwupgrade);
}
