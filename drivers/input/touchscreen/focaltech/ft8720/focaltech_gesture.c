	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&ts_data->input_dev->mutex);
	fts_read_reg(0xC4, &val);
	count = snprintf(buf, PAGE_SIZE, "SPAY Mode:%s\n",
					 val ? "On" : "Off");
	mutex_unlock(&ts_data->input_dev->mutex);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&ts_data->input_dev->mutex);
	if (FTS_SYSFS_ECHO_ON(buf)) {
		FTS_DEBUG("enable SPAY");
		//fts_write_reg(0xC4, 0x01);
		ts_data->gesture_mode |= GESTURE_SPAY_EN;
		ts_data->spay_enable = ENABLE;
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		FTS_DEBUG("disable SPAY");
		//fts_write_reg(0xC4, 0x0);
		ts_data->gesture_mode &= ~GESTURE_SPAY_EN;
		ts_data->spay_enable = DISABLE;
	}
	mutex_unlock(&ts_data->input_dev->mutex);
...
static ssize_t fts_double_click_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	u8 val = 0;
	...
	mutex_lock(&ts_data->input_dev->mutex);
	fts_read_reg(0xD1, &val);
	count = snprintf(buf, PAGE_SIZE, "double click Mode:%s\n",
					 val ? "On" : "Off");
	mutex_unlock(&ts_data->input_dev->mutex);
...
static ssize_t fts_double_click_store(
	struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct fts_ts_data *ts_data = fts_data;
	...
	mutex_lock(&ts_data->input_dev->mutex);
	if (FTS_SYSFS_ECHO_ON(buf)) {
		FTS_DEBUG("enable double click");
		ts_data->gesture_mode |= GESTURE_DOUBLECLICK_EN;
		ts_data->aot_enable = ENABLE;
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		FTS_DEBUG("disable double click");
		ts_data->gesture_mode &= ~GESTURE_DOUBLECLICK_EN;
		ts_data->aot_enable = DISABLE;
	}
	mutex_unlock(&ts_data->input_dev->mutex);
...
static DEVICE_ATTR(fts_spay_mode, S_IRUGO | S_IWUSR,
				  fts_spay_show, fts_spay_store);

static DEVICE_ATTR(fts_doubleclick_mode, S_IRUGO | S_IWUSR,
...
static struct attribute *fts_gesture_mode_attrs[] = {
	...
	&dev_attr_fts_spay_mode.attr,
	&dev_attr_fts_doubleclick_mode.attr,
	NULL,
};
...
static void fts_gesture_report(struct fts_ts_data *ts_data, int gesture_id)
{
	...
	case GESTURE_DOUBLECLICK:
		FTS_INFO("Double Tap to wake up");
	...
	case GESTURE_SWIPE:
		FTS_INFO("SPAY");
		input_report_key(ts_data->input_dev, KEY_BLACK_UI_GESTURE, 1);
		input_sync(ts_data->input_dev);
		input_report_key(ts_data->input_dev, KEY_BLACK_UI_GESTURE, 0);
		input_sync(ts_data->input_dev);
		break;
	default:
		break;
...
int fts_gesture_readdata(struct fts_ts_data *ts_data, u8 *data)
{
	...
	if (gesture->gesture_id == 0x0 || gesture->gesture_id == 0xFE)
		return 1;
