...
static int fts_get_fw_file_from_sdcard(struct fts_upgrade *upg)
{
...
#ifdef SUPPORT_SIGNED_FW
	snprintf(fwname, FILE_NAME_LENGTH, "%s", TSP_PATH_EXTERNAL_FW_SIGNED);
#else
	snprintf(fwname, FILE_NAME_LENGTH, "%s", TSP_PATH_EXTERNAL_FW);
#endif

	oldfs = get_fs();
	set_fs(KERNEL_DS);
	filp = filp_open(fwname, O_RDONLY, S_IRUSR);
	if (IS_ERR(filp)) {
		FTS_ERROR("open firmware file failed \"%s\"", fwname);
		set_fs(oldfs);
		return -ENOENT;
	}
	...
	if (fsize > 0) {
		...
		if (!spu_fw_data) {
			FTS_ERROR("failed to alloc spu_fw_data");
			vfree(upgrade_fw);
			ret = -ENOMEM;
			goto out;
		}
		...
		FTS_INFO("path [%s] size [%zu]", fwname, spu_fw_size);
		if (nread != spu_fw_size) {
			FTS_ERROR("[ERROR] vfs_read - size[%zu] read[%zu]", fsize, nread);
			vfree(spu_fw_data);
			vfree(upgrade_fw);
			ret = -ENOENT;
			goto out;
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
	...
	return fts_fwupg_func(TSP_BUILT_IN, true);
}
