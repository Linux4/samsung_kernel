...
static void get_delta_data(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct fts_ts_data *ts_data = container_of(sec, struct fts_ts_data, sec);
	int ret = 0;
	...
	ret = fts_write_reg(FACTORY_REG_DATA_SELECT, 0x00);
	if (ret < 0)
		FTS_ERROR("failed to write data select type 0");
...
static void run_delta_data_all(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;

	FTS_FUNC_ENTER();
	run_allnode_data(sec, ALL_NODE_TYPE_DELTADATA);
}
...
static struct sec_cmd sec_cmds[] = {
	...
	{SEC_CMD("run_cs_delta_read_all", run_delta_data_all),},
