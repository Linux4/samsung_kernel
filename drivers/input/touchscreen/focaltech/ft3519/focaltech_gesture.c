...
static void fts_gesture_report(struct fts_ts_data *ts_data, struct fts_gesture_st *gesture)
{
	...
	case GESTURE_SINGLETAP:
		sec_input_gesture_report(ts_data->dev, SPONGE_EVENT_TYPE_SINGLE_TAP, gesture->coordinate_x[0], gesture->coordinate_y[0]);
		break;
...
int fts_gesture_readdata(struct fts_ts_data *ts_data, u8 *data)
{
	...
	if (!ts_data->suspended || (ts_data->pdata->power_state != SEC_INPUT_STATE_LPM)) {
		return 1;
	}
/*
