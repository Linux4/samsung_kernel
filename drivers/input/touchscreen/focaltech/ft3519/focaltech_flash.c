...
static int fts_fwupg_reset_to_boot(struct fts_upgrade *upg)
{
	...
	sec_delay(FTS_DELAY_UPGRADE_AA);
	...
	sec_delay(FTS_DELAY_UPGRADE_RESET);
	return 0;
}
...
int ft3519_pram_test_ecc_cal(u32 saddr, u32 len)
{
	return fts_pram_ecc_cal(fwupgrade, saddr, len);
}
...
int ft3519_pram_test_write_buf(u8 *buf, u32 len)
{
	return fts_pram_write_buf(fwupgrade, buf, len);
}
...
static int fts_pram_start(void)
{
	...
	sec_delay(FTS_DELAY_PRAMBOOT_START);
    ...
    u32 delay)
{
	...
	for (i = 0; i < packet_number; i++) {
		...
		for (j = 0; j < FTS_RETRIES_WRITE; j++) {
			...
			ret = fts_read(&cmd , 1, val, FTS_CMD_FLASH_STATUS_LEN);
			if (ret < 0) {
				FTS_ERROR("failed to read flash status");
				return ret;
			}
			read_status = (((u16)val[0]) << 8) + val[1];
			...
			sec_delay(FTS_RETRIES_DELAY_WRITE);
		}
	}
...
int fts_flash_read_buf(u32 saddr, u8 *buf, u32 len)
{
	...
	for (i = 0; i < packet_number; i++) {
		...
		if (upg->ts_data->bus_type == BUS_TYPE_I2C) {
			...
			sec_delay(FTS_CMD_READ_DELAY); /* must wait, otherwise read wrong data */
...
static int fts_param_ide_in_host(struct fts_upgrade *upg)
{
	...
	if (!memcmp(&upg->fw[off], FTS_FW_IDE_SIG, FTS_FW_IDE_SIG_LEN)) {
...
static bool fts_fwupg_need_upgrade(struct fts_upgrade *upg, bool sec, bool force)
{
	int ret = 0;
	bool fwvalid = false;
	u8 fw_ver_in_host = 0;
	...
	if (fwvalid) {
		if (sec) {
			ret = fts_get_fw_ver(upg->ts_data);
			if (ret < 0) {
				FTS_ERROR("failed to read version, need to update fw %d", ret);
				return true;
			}
			...
			if ((upg->ts_data->ic_fw_ver.ic_name == 0xFF) &&
				(upg->ts_data->ic_fw_ver.project_name == 0xFF) &&
				(upg->ts_data->ic_fw_ver.module_id == 0xFF))
				ret = 1;
			else if (upg->ts_data->ic_fw_ver.project_name != upg->ts_data->bin_fw_ver.project_name)
				ret = 1;
			else if (upg->ts_data->ic_fw_ver.fw_ver < upg->ts_data->bin_fw_ver.fw_ver)
				ret = 1;
			else if (upg->ts_data->ic_fw_ver.module_id != upg->ts_data->bin_fw_ver.module_id)
				ret = 0;
			else if (upg->ts_data->ic_fw_ver.ic_name != upg->ts_data->bin_fw_ver.ic_name)
				ret = 0;
			else
				ret = 0;

			if ((upg->ts_data->pdata->bringup == 3) &&
					(upg->ts_data->ic_fw_ver.fw_ver != upg->ts_data->bin_fw_ver.fw_ver)) {
				FTS_INFO("bring up 3, force update because version is different");
				return true;
			}

			if (ret) {
				FTS_INFO("need to update fw");
				return true;
			} else {
				FTS_INFO("skip fw update");
				return false;
			}
		} else {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_fwupg_upgrade(struct fts_upgrade *upg, bool force)
{
	int ret = 0;
	...
	upgrade_flag = fts_fwupg_need_upgrade(upg, true, force);
				...
				if (ret <= 0) {
					...
					if (upg->func->upgrade) {
						...
						if (ret < 0) {
							...
							fts_fwupg_get_ver_in_tp(&ver);
							FTS_INFO("success force upgrade to fw version %02x", ver);
							ret = 1;
							break;
						}
					}
 ...
 ***********************************************************************/
static int fts_fwupg_auto_upgrade(struct fts_upgrade *upg, bool force)
{
	int ret = -1;

	FTS_INFO("********************FTS enter upgrade********************");

	ret = fts_fwupg_upgrade(upg, force);
	if (ret < 0)
		FTS_ERROR("**********tp fw(app/param) upgrade failed**********");
	else if (ret == 1)
		FTS_INFO("**********tp fw(app/param) upgrade success**********");
	else
		FTS_INFO("**********tp fw(app/param) no upgrade success**********");
...
static int fts_get_fw_file_from_sdcard(struct fts_upgrade *upg, int update_type)
{
	int ret = 0;
	...
	long fsize;
	bool signing = false;
#ifdef SUPPORT_SIGNED_FW
	long spu_ret, spu_fw_size;
#endif

	switch (update_type) {
	case TSP_SDCARD:
		snprintf(fwname, FILE_NAME_LENGTH, "%s", TSP_EXTERNAL_FW);
		break;
	case TSP_SIGNED_SDCARD:
		snprintf(fwname, FILE_NAME_LENGTH, "%s", TSP_EXTERNAL_FW_SIGNED);
		signing = true;
		break;
	case TSP_SPU:
	case TSP_VERIFICATION:
		snprintf(fwname, FILE_NAME_LENGTH, "%s", TSP_SPU_FW_SIGNED);
		signing = true;
		break;
	default:
		FTS_ERROR("update_type %d is invalid", update_type);
		return -EINVAL;
	}

	ret = request_firmware(&fw_entry, fwname, upg->ts_data->dev);
	if (ret) {
		FTS_ERROR("fw is not available %d, %s", ret, fwname);
		return -ENOENT;
	}
	...
	if (fsize > 0) {
		...
		if (!upgrade_fw) {
			FTS_ERROR("failed to alloc upgrade_fw");
			ret = -ENOMEM;
			goto out;
		}
		if (signing) {
#ifdef SUPPORT_SIGNED_FW
			spu_ret = spu_firmware_signature_verify("TSP", fw_entry->data, spu_fw_size);
			FTS_INFO("spu_ret : %ld, spu_fw_size:%ld", spu_ret, spu_fw_size);
			if (spu_ret != fsize) {
				FTS_ERROR("signature verify failed, %ld", spu_ret);
				vfree(upgrade_fw);
				ret = -ENOENT;
				goto out;
			} else {
				FTS_INFO("signature verify succeeded");
				ret = 0;
			}
#endif
		}
		...
		if (update_type == TSP_VERIFICATION) {
			FTS_INFO("just verify signing, do not fw update");
			vfree(upgrade_fw);
			goto out;
		}

		memcpy(upgrade_fw, fw_entry->data, fsize);
	} else {
		FTS_ERROR("[ERROR] fsize [%zu]", fsize);
		ret = -ENOENT;
		goto out;
	}

	if (upg->fw) {
		FTS_INFO("free exist fw data");
		vfree(upg->fw);
		upg->fw = NULL;
	}
	upg->fw = upgrade_fw;
...
static int fts_get_fw_file_via_request_firmware(struct fts_upgrade *upg)
{
	...
	snprintf(fwname, FILE_NAME_LENGTH, "%s", upg->ts_data->pdata->firmware_name);
	...
	tmpbuf = vzalloc(fw->size);
	if (tmpbuf == NULL) {
		FTS_ERROR("fw buffer vzalloc fail");
		release_firmware(fw);
		return -ENOMEM;
	} else {
		...
		upg->fw_from_request = 1;
		memcpy((u8 *)&upg->ts_data->bin_fw_ver, upg->fw + FTS_APP_VER_INFO_OFFSET, 4);
		FTS_INFO("[BIN FW VER] FT%02X%02X%02X%02X",
				upg->ts_data->bin_fw_ver.ic_name,
				upg->ts_data->bin_fw_ver.project_name,
				upg->ts_data->bin_fw_ver.module_id,
				upg->ts_data->bin_fw_ver.fw_ver);
	}
 ...
 *  Return: return 0 if success, otherwise return error code
 *****************************************************************************/
static int fts_fwupg_get_fw_file(struct fts_upgrade *upg, int update_type)
{
	int ret = 0;
	...
	if (FTS_FW_REQUEST_SUPPORT) {
		if (update_type == TSP_BUILT_IN)
			...
			ret = fts_get_fw_file_from_sdcard(upg, update_type);
...
int fts_get_fw_ver(struct fts_ts_data *ts_data)
{
	...
	ret = fts_read_reg(FTS_REG_SEC_IC_NAME, &ts_data->ic_fw_ver.ic_name);
	if (ret < 0) {
		FTS_ERROR("failed to read ic name");
		return ret;
	}

	ret = fts_read_reg(FTS_REG_SEC_PROJECT_NAME, &ts_data->ic_fw_ver.project_name);
	if (ret < 0) {
		FTS_ERROR("failed to read ic name");
		return ret;
	}

	ret = fts_read_reg(FTS_REG_SEC_MODULE_ID, &ts_data->ic_fw_ver.module_id);
	if (ret < 0) {
		FTS_ERROR("failed to read ic name");
		return ret;
	}

	ret = fts_read_reg(FTS_REG_SEC_FW_VERSION, &ts_data->ic_fw_ver.fw_ver);
	if (ret < 0) {
		FTS_ERROR("failed to read ic name");
		return ret;
	}

	FTS_INFO("[IC FW VER] FT%02X%02X%02X%02X",
...
int fts_fwupg_func(int update_type, bool force)
{
	...
	FTS_INFO("update_type = %d, force = %d", update_type, force);
	if (!upg || !upg->ts_data) {
		FTS_ERROR("upg/ts_data is null");
		return -ENOMEM;
	}

	if ((upg->ts_data->pdata->bringup == 1) && !force) {
		FTS_INFO("bring up. do not update");
		return 0;
	}

	if (force || !upg->fw || upg->fw_length < FTS_MIN_LEN) {
		/* get fw */
		ret = fts_fwupg_get_fw_file(upg, update_type);
		if (ret < 0) {
			FTS_ERROR("get file fail, can't upgrade");
			return ret;
		}
	} else {
		FTS_INFO("use allocated fw data");
	}

	if (update_type == TSP_VERIFICATION)
		return ret;

	if ((upg->ts_data->pdata->bringup == 2) && !force) {
		FTS_INFO("bring up. do not update");
		return ret;
	}
	...
	/* run auto upgrade */
	ret = fts_fwupg_auto_upgrade(upg, force);
	if (ret < 0) {
