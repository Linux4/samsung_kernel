...
static void set_pretest_enable(void);
static void set_deadcheck_enable(void);
...
struct ft_cmd ft_cmds[] = {
	...
	{FT_CMD("set_pretest_enable", set_pretest_enable),},
	{FT_CMD("deadcheck", set_deadcheck_enable),},
		...
		const struct firmware *fw_entry)
{
	...
	/* B020, BF20 fw must be downgrade. This must be erased next fw update or several weeks later. */
	if (rmi4_data->fw_version_of_ic == 0x20) {
		dev_info(&rmi4_data->i2c_client->dev, "%s: 0x20 firmware need to be downgrade.\n",
			__func__);
		return false;
	}
...
static void set_pretest_enable(void)
{
	...
	pretest_enable = data->cmd_param[0];
	if (!(pretest_enable == 0 || pretest_enable == 1)) {
		dev_err(&rmi4_data->i2c_client->dev, "%s : wrong param %d\n",
			__func__, pretest_enable);
		snprintf(data->cmd_buff, sizeof(data->cmd_buff), "NG");
		data->cmd_state = CMD_STATUS_FAIL;
	}
	else {
		dev_info(&rmi4_data->i2c_client->dev, "%s : %s pretest\n",
			__func__, pretest_enable ? "enter" : "exit");
		sprintf(data->cmd_buff, "%s : %s pretest",
			__func__, pretest_enable ? "enter" : "exit");
		rmi4_data->in_pretest = pretest_enable;
		snprintf(data->cmd_buff, sizeof(data->cmd_buff), "OK");
		data->cmd_state = CMD_STATUS_OK;
	}
...
static void set_deadcheck_enable(void)
{
	...
	deadcheck_enable = data->cmd_param[0];
	if (!(deadcheck_enable == 0 || deadcheck_enable == 1)) {
		dev_err(&rmi4_data->i2c_client->dev, "%s : wrong param %d\n",
			__func__, deadcheck_enable);
		snprintf(data->cmd_buff, sizeof(data->cmd_buff), "NG");
		data->cmd_state = CMD_STATUS_FAIL;
	}
	else {
		dev_info(&rmi4_data->i2c_client->dev, "%s : %s deadcheck\n",
			__func__, deadcheck_enable ? "enable" : "disable");
		sprintf(data->cmd_buff, "%s : %s deadcheck",
			__func__, deadcheck_enable ? "enable" : "disable");
		rmi4_data->enable_deadcheck = deadcheck_enable;
		snprintf(data->cmd_buff, sizeof(data->cmd_buff), "OK");
		data->cmd_state = CMD_STATUS_OK;
	}
