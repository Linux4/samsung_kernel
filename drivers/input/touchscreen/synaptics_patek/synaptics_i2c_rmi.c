		...
		unsigned int address)
{
	...
	if (page != rmi4_data->current_page) {
		...
		if (retry == SYN_I2C_RETRY_TIMES && rmi4_data->tsp_probe)
			schedule_delayed_work(&rmi4_data->reset_work,
				msecs_to_jiffies(10));
	} else {
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	do {
		...
		if (count++ > 100) {
			dev_err(&rmi4_data->i2c_client->dev, "%s: count %d\n",
				__func__, count);
			rmi4_data->irq_is_low = true;
			goto out;
		}
	} while (!gpio_get_value(rmi4_data->dt_data->tsp_int));

out:
	rmi4_data->is_in_irq = false;
	return IRQ_HANDLED;
}

 /* clear interrupt register */
static int synaptics_rmi4_read_intr_reg(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
		...
		bool enable)
{
	...
	if (enable) {
		...
		retval = synaptics_rmi4_read_intr_reg(rmi4_data);
		if (retval < 0)
			return retval;
...
static int synaptics_rmi4_input_open(struct input_dev *dev)
{
	...
	if (retval < 0)
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to start device\n", __func__);
#endif
	if (rmi4_data->enable_deadcheck)
		schedule_delayed_work(&rmi4_data->check_work,
...
static void synaptics_rmi4_input_close(struct input_dev *dev)
{
	...
	if (rmi4_data->enable_deadcheck)
		cancel_delayed_work(&rmi4_data->check_work);

#ifdef USE_OPEN_DWORK
	cancel_delayed_work(&rmi4_data->open_work);
#endif
	cancel_delayed_work(&rmi4_data->reset_work);
#ifdef TSP_PATTERN_TRACKING_METHOD
	cancel_delayed_work(&rmi4_data->reboot_work);
#endif
...
static int synaptics_rmi4_reinit_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if ((rmi4_data->intr_mask[0] & 0x01) == 1) {
		retval = synaptics_rmi4_query_device(rmi4_data);
		if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev,
					"%s: Failed to query device\n",
		...
		for (i = 0; i < rmi4_data->dt_data->num_of_supply; i++) {
			if (regulator_is_enabled(rmi4_data->dt_data->supplies[i].consumer)) {
				...
				dev_err(&rmi4_data->i2c_client->dev,
					"%s: %s is enabled[OK]\n", __func__,
					rmi4_data->dt_data->supplies[i].supply);
				if (i < rmi4_data->dt_data->num_of_supply - 1)
					msleep(20);
			}
		}
	} else {
...
static void synaptics_rmi4_reset_work(struct work_struct *work)
{
	...
	dev_err(&rmi4_data->i2c_client->dev,
			"%s, cpld cdone = %d\n",
			__func__, gpio_get_value(rmi4_data->dt_data->cdone));
...
#ifdef CONFIG_DUAL_LCD
	mutex_lock(&rmi4_data->rmi4_switching_mutex);
	gpio_set_value(rmi4_data->dt_data->tsp_sel, !(rmi4_data->flip_status));
	synaptics_rmi4_read_intr_reg(rmi4_data);
	synaptics_rmi4_sleep_mode(rmi4_data, 1);
	gpio_set_value(rmi4_data->dt_data->tsp_sel, rmi4_data->flip_status);
	mutex_unlock(&rmi4_data->rmi4_switching_mutex);
#endif
	retval = synaptics_rmi4_f54_set_control(rmi4_data);
	if (retval < 0)
		dev_err(&rmi4_data->i2c_client->dev,
...
static void synaptics_rmi4_reboot_work(struct work_struct *work)
{
	...
	schedule_delayed_work(&rmi4_data->reset_work, msecs_to_jiffies(1));
}
#endif

static void synaptics_rmi4_dead_check(struct work_struct *work)
{
	int retval;
	unsigned char data;
	struct synaptics_rmi4_data *rmi4_data =
	...
	if (rmi4_data->in_fw_update)
		goto out;

	if (rmi4_data->in_switching)
		goto out;

	if (!rmi4_data->irq_is_low && rmi4_data->is_in_irq)
		goto out;
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s: Failed to read Interrupt enable mask\n",
			__func__);
		return;
	}
	...
	dev_err(&rmi4_data->i2c_client->dev,
		"%s: Interrupt Enable = %x\n", __func__, data);

	rmi4_data->irq_is_low = false;
	schedule_delayed_work(&rmi4_data->reset_work,
		msecs_to_jiffies(10));
	return;
out:
...
static int fpga_configuration(struct synaptics_rmi4_data *rmi4_data)
{
	int retry = 3;
	int retval;
	...
	while(retry > 0){
		...
		dev_err(&rmi4_data->i2c_client->dev, "FPGA configuration %s. CDONE : %d\n",
	...
	if (retry == 0){
		dev_err(&rmi4_data->i2c_client->dev, "failed to configurate FPGA\n");
		return retry;
	}
		...
		const struct i2c_device_id *dev_id)
{
	...
	*/
	...
	rmi4_data->in_pretest = false;
	rmi4_data->enable_deadcheck = true;
	rmi4_data->irq_is_low = false;
#ifdef CONFIG_DUAL_LCD
	...
	*/
	...
	retval = fpga_configuration(rmi4_data);
	if (!retval){
		dev_err(&rmi4_data->i2c_client->dev,
			"failed to configurate FPGA\n");
...
#endif
	INIT_DELAYED_WORK(&rmi4_data->reset_work, synaptics_rmi4_reset_work);
#ifdef TSP_PATTERN_TRACKING_METHOD
	INIT_DELAYED_WORK(&rmi4_data->reboot_work, synaptics_rmi4_reboot_work);
#endif
	INIT_DELAYED_WORK(&rmi4_data->check_work, synaptics_rmi4_dead_check);
...
err_subTSP_reboot:
	...
	if (retval < 0) {
		...
		if ((retval == TSP_NEEDTO_REBOOT)&&(rmi4_data->rebootcount< MAX_TSP_REBOOT)){
			...
			rmi4_data->rebootcount++;
			dev_err(&client->dev,
					"%s: %d reboot sequence by i2c fail(TSP_SUB)\n",
					__func__, rmi4_data->rebootcount);
			goto err_subTSP_reboot;
		}
		else goto err_subTSP_query_dev;
	}
	retval = synaptics_rmi4_fw_update_on_probe(rmi4_data);
	if (retval < 0) {
...
static int synaptics_rmi4_stop_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->in_pretest) {
		fpga_power_ctrl(rmi4_data, 0);
		regulator_put(rmi4_data->vreg_2p95);
		regulator_put(rmi4_data->vreg_2p5);
		rmi4_data->vreg_2p95 = NULL;
		rmi4_data->vreg_2p5 = NULL;
	}
...
static int synaptics_rmi4_start_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	dev_info(&rmi4_data->i2c_client->dev, "%s, cpld cdone = %d\n",
		__func__, gpio_get_value(rmi4_data->dt_data->cdone));
