...
#define LOG_FORMAT "%s: %s, tmp_data[0]=%02X,tmp_data[1]=%02X,tmp_data[2]=%02X,tmp_data[3]=%02X\n"
...
static void himax_mcu_read_FW_ver(void)
{
	...
	if (pfw_op->addr_ver_ic_name[3] != 0x78) {
		...
		ic_data->_ver_ic_name = data[0];
		I("IC name = 0x%02X\n", ic_data->_ver_ic_name);
	}
...
{
	I("%s: Now reset the Touch chip. gpio %d\n", __func__, private_ts->rst_gpio);
	himax_ctrl_tsp_reset(private_ts, false);
	usleep_range(5000, 5001);
	himax_ctrl_tsp_reset(private_ts, true);
#if !defined(HX_ZERO_FLASH)
...
static void himax_mcu_ic_reset(uint8_t loadconfig, uint8_t int_off)
{
	...
	if ((ts->rst_gpio >= 0) || private_ts->pdata->name_tsp_rst) {
		if (int_off)
...
int hx_parse_bin_cfg_data(u8 *data, struct zf_info *zf_info_arr, uint8_t *FW_buf, uint8_t *sram_min, uint8_t *ovl_sidx)
{
	...
	for (i = 1; i < part_num; i++) {
		...
		if(flag_1k_header == true)
			memcpy(buf, &data[i * 0x10 + HX64K + HX1K], 16);
		else
			memcpy(buf, &data[i * 0x10 + HX64K], 16);
...
int himax_zf_part_info(u8 *data)
{
	...
	/*CFG*/
	cfg_sz = hx_parse_bin_cfg_data(data, zf_info_arr, FW_buf, sram_min, &ovl_sidx);
	I("Now cfg_sz=%d\n", cfg_sz);
	if (cfg_sz < 0) {
		ret = LENGTH_FAIL;
		goto END_FUNC;
	...
	/*Fw Entity*/
	if(flag_1k_header == true)
		memcpy(buf, &data[0 * 0x10 + HX64K + HX1K], 16);
	else
		memcpy(buf, &data[0 * 0x10 + HX64K], 16);
