...
void himax_get_arraydata_edge(uint32_t *RAW)
{
	...
	ArrayData = kzalloc(len * sizeof(uint32_t), GFP_KERNEL);
	if (!ArrayData) {
...
//					 0660);
	...
	if (raw_file == NULL) {
		raw_file = filp_open(g_file_path, O_TRUNC|O_CREAT|O_RDWR, 0660);
...
static void hx_debug(void)
{
	...
	I(TEMP_LOG, __func__, "0x900000A8", tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

	himax_in_parse_assign_cmd(fw_addr_flag_reset_event, tmp_addr, sizeof(tmp_addr));
	g_core_fp.fp_register_read(tmp_addr, 4, tmp_data, false);
	I(TEMP_LOG, __func__, "0x900000E4", tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

	himax_in_parse_assign_cmd(0x900000E8, tmp_addr, sizeof(tmp_addr));
	g_core_fp.fp_register_read(tmp_addr, 4, tmp_data, false);
	I(TEMP_LOG, __func__, "0x900000E8", tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

	himax_in_parse_assign_cmd(0x90000048, tmp_addr, sizeof(tmp_addr));
	g_core_fp.fp_register_read(tmp_addr, 4, tmp_data, false);
	I(TEMP_LOG, __func__, "0x90000048", tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

	himax_in_parse_assign_cmd(fw_addr_fw_dbg_msg_addr, tmp_addr, sizeof(tmp_addr));
	g_core_fp.fp_register_read(tmp_addr, 4, tmp_data, false);
	I(TEMP_LOG, __func__, "0x10007F40", tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
}

static uint32_t himax_get_rawdata(int32_t RAW[], uint32_t datalen, uint8_t checktype)
{
	uint8_t *tmp_rawdata;
	bool get_raw_rlst;
	uint8_t retry = 0;
	...
	/* Copy Data*/
	for (i = 0; i < ic_data->HX_TX_NUM*ic_data->HX_RX_NUM; i++) {
		tmp_data =  (((int8_t)tmp_rawdata[(i * 2) + 1] << 8) | tmp_rawdata[(i * 2)]);
		RAW[i] = tmp_data;
...
static void himax_neg_noise_sup(uint8_t *data)
{
	...
	I("%s: write 0x%02X%02X%02X%02X\n", __func__, data[3], data[2], data[1], data[0]);
}
...
static void himax_get_noise_base(uint8_t checktype)/*Normal Threshold*/
{
	...
	himax_in_parse_assign_cmd(addr_noise_scale, tmp_addr2, sizeof(tmp_addr2));
...
static void hx_findout_limit(int32_t *RAW, int *limit_val, int sz_mutual)
{
	int i = 0;
	int tmp = 0;
...
static void himax_osr_ctrl(bool enable)
{
	...
	do {
		...
		w_byte = tmp_data[idx_wr_byte];

		g_core_fp.fp_register_write(tmp_addr, 4, tmp_data, 0);
		g_core_fp.fp_register_read(tmp_addr, 4, tmp_data, false);
		back_byte = tmp_data[idx_wr_byte];
		if (w_byte != back_byte)
...
//	g_dc_max = g_core_fp.fp_get_max_dc();
	...
	if (checktype == HIMAX_ABS_NOISE || checktype == HIMAX_WEIGHT_NOISE) {
		...
		if (tmp_data[3] == tmp_addr[1] && tmp_data[2] == tmp_addr[0])
			g_weight_val = (tmp_data[1] << 8) | tmp_data[0];
		else
			I("%s: FW does not support weight test\n", __func__);
		RI("%s: weight = %d, %02X, %02X\n", __func__, g_weight_val, tmp_data[2], tmp_data[3]);
		/*********************************/
	}
...
static int himax_fw_update_kernel(struct himax_ts_data *ts)
{
	...
	if (private_ts->fw_data_ums) {
		input_err(true, private_ts->dev, "%s: private_ts->fw_data_ums is not NULL(%d)\n",
					__func__, private_ts->fw_size_ums);
		vfree(private_ts->fw_data_ums);
		private_ts->fw_size_ums = 0;
		private_ts->fw_data_ums = NULL;
	}
	...
	hx_mcu_bin_desc_get((unsigned char *) firmware->data, HX1K);
	ret = himax_zf_part_info((unsigned char *) firmware->data);
#else
...
static int himax_fw_update_from_storage(bool signing, const char *file_path)
{
	...
	fsize = firmware->size;
	input_err(true, private_ts->dev, "%s %s: fsize:%d\n", HIMAX_LOG_TAG, __func__, fsize);
	if (signing) {
		spu_fw_size = fsize;
	...
	if (fsize > 0) {
		if (signing) {
#ifdef SUPPORT_FW_SIGNED
			spu_ret = spu_firmware_signature_verify("TSP", firmware->data, spu_fw_size);
#endif
		...
		memcpy(private_ts->fw_data_ums, firmware->data, fsize);
		private_ts->fw_size_ums = fsize;
	} else {
		I("%s [ERROR] fsize [%zu]\n", __func__, fsize);
		release_firmware(firmware);
	...
	case FW_SIZE_32k:
		ret = g_core_fp.fp_fts_ctpm_fw_upgrade_with_sys_fs_32k(
					(unsigned char *) firmware->data, fsize, false);
		break;
	case FW_SIZE_60k:
		ret = g_core_fp.fp_fts_ctpm_fw_upgrade_with_sys_fs_60k(
					(unsigned char *) firmware->data, fsize, false);
	...
	input_err(true, private_ts->dev, "%s: ret\n", __func__, ret);
...
static void fw_update(void *dev_data)
{
	...
	case UMS:
		...
		ret = himax_fw_update_from_storage(true, TSP_EXTERNAL_FW_SIGNED);
#else
		ret = himax_fw_update_from_storage(false, TSP_EXTERNAL_FW);
#endif
		break;
	case SPU:
		ret = himax_fw_update_from_storage(true, TSP_SPU_FW_SIGNED);
		break;
	default:
		ret = -EINVAL;
		I("not support param %d", sec->cmd_param[0]);
		break;
	}
	...
	if (ret) {
		...
		if (strcmp(private_ts->pdata->proj_name, "M20") == 0) {
			...
			if (strcmp(HX_83112A_SERIES_PWON, private_ts->chip_name) == 0)
				...
				bin_ver_ic_name = 0x04;
			else if (strcmp(HX_83108A_SERIES_PWON, private_ts->chip_name) == 0)
				bin_ver_ic_name = 0x05;
			else
				bin_ver_ic_name = firmware->data[ADDR_VER_IC_NAME];
...
static void get_fw_ver_ic(void *dev_data)
{
	...
	if (strcmp(private_ts->pdata->proj_name, "M20") == 0) {
		...
		if (strcmp(HX_83112A_SERIES_PWON, private_ts->chip_name) == 0)
			...
			ic_ver_ic_name = ic_data->_ver_ic_name;
...
static void hx_print_frame(struct himax_ts_data *ts, uint32_t *frame)
{
	...
	int msg_len = 15;
	char *buf = NULL;
	int line_size = ic_data->HX_TX_NUM * 15;

	buf = kzalloc(line_size, GFP_KERNEL);
	if (!buf)
		return;
	for (i = 0; i < ic_data->HX_TX_NUM; i++) {
		if (i == 0)
			snprintf(msg, msg_len, "        TX%2d", i);
		else
			snprintf(msg, msg_len, "  TX%2d", i);

		strlcat(buf, msg, line_size);
	}
	RI("%s\n", buf);
	...
	for (i = 0; i < ic_data->HX_RX_NUM; i++) {
		...
		for (j = 0; j < ic_data->HX_TX_NUM; j++) {
			snprintf(msg, msg_len, "%5d ", frame[(j * ic_data->HX_RX_NUM) + i]);
			strlcat(buf, msg, line_size);
		}
		RI("RX%2d %s\n", i, buf);
	}
...
static void get_rawcap(void *dev_data)
{
	...
	/* save to flash */
	/* 1. data size*/
	rawdata_size = sizeof(int32_t) * datalen;
...
static void get_rawcap_all(void *dev_data)
{
	...
	int buff_size = ic_data->HX_TX_NUM * ic_data->HX_RX_NUM * 10;
	struct sec_cmd_data *sec = (struct sec_cmd_data *)dev_data;
	/*struct himax_ts_data *data =
		container_of(sec, struct himax_ts_data, sec);*/
	...
	for (i = 0; i < ic_data->HX_RX_NUM; i++) {
		for (j = 0; j < ic_data->HX_TX_NUM; j++) {
			snprintf(msg, SEC_CMD_STR_LEN, "%d,",
					(int)g_sec_raw_buff->_rawdata[(j * ic_data->HX_RX_NUM) + i]);
			strlcat(buf, msg, buff_size);
		}
	}

	sec->cmd_state = SEC_CMD_STATUS_OK;
	sec_cmd_set_cmd_result(sec, buf, strnlen(buf, buff_size));
...
static void get_open_all(void *dev_data)
{
	...
	for (i = 0; i < ic_data->HX_RX_NUM; i++) {
		for (j = 0; j < ic_data->HX_TX_NUM; j++) {
			snprintf(msg, SEC_CMD_STR_LEN, "%d,",
					(int)g_sec_raw_buff->_open[(j * ic_data->HX_RX_NUM) + i]);
			strlcat(buf, msg, buff_size);
		}
	}
...
static void get_short_all(void *dev_data)
{
	...
	for (i = 0; i < ic_data->HX_RX_NUM; i++) {
		for (j = 0; j < ic_data->HX_TX_NUM; j++) {
			snprintf(msg, SEC_CMD_STR_LEN, "%d,",
					(int)g_sec_raw_buff->_short[(j * ic_data->HX_RX_NUM) + i]);
			strlcat(buf, msg, buff_size);
		}
	}
...
static void get_mic_open_all(void *dev_data)
{
	...
	for (i = 0; i < ic_data->HX_RX_NUM; i++) {
		for (j = 0; j < ic_data->HX_TX_NUM; j++) {
			snprintf(msg, SEC_CMD_STR_LEN, "%d,",
					(int)g_sec_raw_buff->_mopen[(j * ic_data->HX_RX_NUM) + i]);
			strlcat(buf, msg, buff_size);
		}
	}
...
END_OUPUT:
	...
	if (sec->cmd_all_factory_state == SEC_CMD_STATUS_RUNNING) {
		...
		snprintf(buf, sizeof(buf), "%d,%d", 0, g_weight_val);
...
static void get_noise_all(void *dev_data)
{
	...
	for (i = 0; i < ic_data->HX_RX_NUM; i++) {
		for (j = 0; j < ic_data->HX_TX_NUM; j++) {
			snprintf(msg, SEC_CMD_STR_LEN, "%d,",
					(int)g_sec_raw_buff->_noise[(j * ic_data->HX_RX_NUM) + i]);
			strlcat(buf, msg, buff_size);
		}
	}
...
static void run_snr_non_touched(void *dev_data)
{
	int ret = 1;
	char buf[LEN_RSLT] = { 0 };
	uint8_t tmp_addr[4] = {0x30, 0x7F, 0x00, 0x10};
	uint8_t send_data[4] = {0x00, 0x00, 0x5A, 0xA5};
	...
	if (sec->cmd_param[0] <= 0 || sec->cmd_param[0] > 1000) {
		E("%s: frame is out of range[%d]\n", __func__, sec->cmd_param[0]);
		ret = 0;
		goto err_frame_data;
	}

	send_data[1] = (sec->cmd_param[0] >> 8) & 0xFF;
	send_data[0] = sec->cmd_param[0] & 0xFF;
	...
	if (retry_cnt >= HIMAX_REG_RETRY_TIMES) {
		E("%s: write cmd fail %02X%02X%02X%02X\n", __func__,
			recv_data[3], recv_data[2], recv_data[1], recv_data[0]);
		ret = 0;
		goto err_frame_data;
	}
	...
	I("%s: retry cnt = %d, %02X, %02X\n", __func__, retry_cnt, recv_data[3], recv_data[2]);

	if (recv_data[3] == 0xA7 && recv_data[2] == 0x7A) {
		I("%s: get data success\n", __func__);
		ret = 1;
	} else if (recv_data[3] == 0xA9 && recv_data[2] == 0x9A) {
		I("%s: get data fail\n", __func__);
		ret = 0;
	} else {
		I("%s: unknow status\n", __func__);
...
static void run_snr_touched(void *dev_data)
{
	int ret = 1;
	char buf[LEN_RSLT] = { 0 };
	uint8_t tmp_addr[4] = {0x30, 0x7F, 0x00, 0x10};
	...
	while (retry_cnt++ < 100) {
		...
		if (result[0] == 0xA77A || result[0] == 0xA99A)
			break;
	}

	I("%s: retry cnt = %d, %08X\n", __func__, retry_cnt, result[0]);

	if (result[0] == 0xA77A) {
		I("%s: get data success\n", __func__);
		ret = 1;
	} else if (result[0] == 0xA99A) {
		E("%s: get data fail\n", __func__);
		ret = 0;
	} else {
		E("%s: unknow status\n", __func__);
...
err_frame_data:
	if (ret == 1) {
		sec->cmd_state = SEC_CMD_STATUS_OK;
		snprintf(buf, sizeof(buf), "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
			result[1], (result[2] >> 8) & 0xFF, (result[2]) & 0xFF,
			result[3], (result[4] >> 8) & 0xFF, (result[4]) & 0xFF,
			result[5], (result[6] >> 8) & 0xFF, (result[6]) & 0xFF,
			result[7], (result[8] >> 8) & 0xFF, (result[8]) & 0xFF,
			result[9], (result[10] >> 8) & 0xFF, (result[10]) & 0xFF,
			result[11], (result[12] >> 8) & 0xFF, (result[12]) & 0xFF,
			result[13], (result[14] >> 8) & 0xFF, (result[14]) & 0xFF,
			result[15], (result[16] >> 8) & 0xFF, (result[16]) & 0xFF,
			result[17], (result[18] >> 8) & 0xFF, (result[18]) & 0xFF);
	} else {
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		snprintf(buf, sizeof(buf), "%s", "NG");
	}
...
static void glove_mode(void *dev_data)
{
	...
	switch (sec->cmd_param[0]) {
	case 0:
		data->glove_enabled = 0;
		I("%s: Unset High Sensitivity Mode\n", __func__);
		ret = himax_set_ap_change_mode(HIGH_SENSITIVITY_MODE, 0);
		break;
	case 1:
		data->glove_enabled = 1;
		I("%s: Set High Sensitivity Mode\n", __func__);
		ret = himax_set_ap_change_mode(HIGH_SENSITIVITY_MODE, 1);
		break;
	default:
...
void psensor_enable(struct himax_ts_data *data, unsigned int mode)
{
	...
	case 3:
		...
		break;
	default:
		input_err(true, data->dev, "%s: invalid parameter %d\n", __func__, mode);
		break;
	}
}
...
static void ear_detect_enable(void *device_data)
{
	...
	input_info(true, data->dev, "%s\n", __func__);

	sec_cmd_set_default_result(sec);

	if (!data->pdata->support_ear_detect) {
		input_err(true, data->dev, "%s: Not support EarDetection!\n", __func__);
		ret = -1;
		goto out;
	}
	...
	if (!(sec->cmd_param[0] == 0 || sec->cmd_param[0] == 1 || sec->cmd_param[0] == 3)) {
		...
		data->ear_detect_mode = sec->cmd_param[0];
		if (sec->cmd_param[0] != 0) {
			data->proxy_current_thd = 0;
			data->proxy_current_sum = 0;
		}
	}

	if (data->suspended) {
		input_err(true, data->dev, "%s: now screen off stae, it'll setting after screen on(%d)!\n",
					__func__, data->ear_detect_mode);
		goto out;
	}

	mutex_lock(&data->device_lock);

	psensor_enable(data, data->ear_detect_mode);
...
out:
	if (ret < 0) {
		...
		sec->cmd_state = SEC_CMD_STATUS_OK;
		input_info(true, data->dev, "%s,%d: %s\n", __func__, sec->cmd_param[0], buff);
	}
...
static void hx_proxy_dbg(struct himax_ts_data *ts)
{
	uint8_t tmp_addr[] = {0x18, 0x7F, 0x00, 0x10};
	uint8_t tmp_data[8] = {0};

	g_core_fp.fp_register_read(tmp_addr, 8, tmp_data, false);
	ts->proxy_current_thd = tmp_data[1] << 8 | tmp_data[0];
	ts->proxy_current_sum = tmp_data[7] << 24
				| tmp_data[6] << 16
				| tmp_data[5] << 8
				| tmp_data[4];
}
...
static void run_prox_intensity_read_all(void *device_data)
{
	...
	if (data->ear_detect_mode == 0) {
		input_err(true, data->dev, "%s: Please enable ear detect mode first!\n", __func__);
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		goto CMD_STATE;
	}
...
CMD_STATE:
	if (sec->cmd_state == SEC_CMD_STATUS_FAIL) {
		snprintf(buff, sizeof(buff), "NG, please check command");
	} else {
		snprintf(buff, sizeof(buff), "SUM_X:%d THD_X:%d",
			data->proxy_current_sum, data->proxy_current_thd);
		sec->cmd_state = SEC_CMD_STATUS_OK;
	}
	...
	input_info(true, data->dev, "%s: buff : %s sum : %d thd : %d\n", __func__, buff, data->proxy_current_sum, data->proxy_current_thd);
}
...
static void prox_lp_scan_mode(void *device_data)
{
	...
	if (!data->pdata->prox_lp_scan_enabled) {
		input_err(true, data->dev, "%s: Not support LPSCAN!\n", __func__);
		ret = -1;
		goto out;
	}
	...
	if (sec->cmd_param[0] < 0 || sec->cmd_param[0] > 1) {
		...
		data->prox_lp_scan_mode_enabled = mode;
	}

	if (!data->late_suspended) {
		input_info(true, data->dev, "%s  now screen on state, it'll setting after screen off\n", __func__);
		goto out;
	}
	...
	if (mode) {
		g_core_fp._proximity_sleep_out();
	} else {
		g_core_fp._proximity_sleep_in();
	}
...
out:
	if (ret < 0) {
		snprintf(buff, sizeof(buff), "NG");
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		input_info(true, data->dev, "%s : failed to switch %d mode\n", __func__, mode);
	} else {
		snprintf(buff, sizeof(buff), "OK");
		sec->cmd_state = SEC_CMD_STATUS_OK;
		input_info(true, data->dev, "%s : switch to %s mode OK\n",
				__func__, mode ? "SLEEP_OUT" : "SLEEP_IN");
	}
...
int himax_set_ap_change_mode(int mode, int enable)
{
	...
	I("%s: mode:%d %s %s\n", __func__, mode, enable ? "enable" : "disable",
		ret ? "failed" : "succeed");

	return ret;
}
EXPORT_SYMBOL(himax_set_ap_change_mode);
...
static void set_game_mode(void *dev_data)
{
	...
	case 0:
		I("%s: Unset Game Mode\n", __func__);
		ret = himax_set_ap_change_mode(GAME_MODE, 0);
		break;
	case 1:
		I("%s: Set Game Mode\n", __func__);
		ret = himax_set_ap_change_mode(GAME_MODE, 1);
		break;
	default:
		I("%s: Invalid Argument\n", __func__);
		break;
...
static void set_note_mode(void *dev_data)
{
	...
	case 0:
		I("%s: Unset Note Mode\n", __func__);
		ret = himax_set_ap_change_mode(NOTE_MODE, 0);
		break;
	case 1:
		I("%s: Set Note Mode\n", __func__);
		ret = himax_set_ap_change_mode(NOTE_MODE, 1);
		break;
	default:
		I("%s: Invalid Argument\n", __func__);
		break;
...
static void set_sip_mode(void *dev_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)dev_data;
	char buf[SEC_CMD_STR_LEN] = { 0 };
	int ret = -EINVAL;
	...
	case 0:
		I("%s: Unset SIP Mode\n", __func__);
		ret = himax_set_ap_change_mode(SIP_MODE, 0);
		break;
	case 1:
		I("%s: Set SIP Mode\n", __func__);
		ret = himax_set_ap_change_mode(SIP_MODE, 1);
		break;
	default:
		I("%s: Invalid Argument\n", __func__);
		break;
...
static void scan_block(void *dev_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)dev_data;
	char buf[SEC_CMD_STR_LEN] = {0};
...
static int ram_test(int type)
{
	...
	if (type == 1) {
		size = ic_data->dsram_size;
		addr = ic_data->dsram_addr;
	} else {
		size = ic_data->isram_size;
		addr = ic_data->isram_addr;
	}
	...
	g_core_fp.fp_register_write(tmp_addr, size, write_buf, 0);
	crc = g_core_fp.fp_check_CRC(tmp_addr, size);
	I("0x55_crc=0x%08X\n", crc);

	crc_sw = g_core_fp.fp_Calculate_CRC_with_AP(write_buf, 0, size);
	I("0x55_crc_sw=0x%08X\n", crc_sw);
	...
	result = SEC_CMD_STATUS_OK;

FAIL_END:
	kfree(write_buf);
#if defined(HX_ZERO_FLASH)
	I("Back to Normal:update fw!\n");
...
static void run_sram_test(void *dev_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)dev_data;
	struct himax_ts_data *ts = container_of(sec, struct himax_ts_data, sec);
	char buff[SEC_CMD_STR_LEN] = { 0 };
	...
	if (ts->suspended) {
		input_err(true, ts->dev, "%s: now power off state!\n", __func__);
		goto out;
	}

	I("%s:testing  dsram\n", __func__);
	sec->cmd_state = ram_test(1);
	if (sec->cmd_state != SEC_CMD_STATUS_OK) {
		input_err(true, ts->dev, "%s: dsram test failed!\n", __func__);
		goto out;
	}
	I("%s:testing isram\n", __func__);
	sec->cmd_state = ram_test(2);
	if (sec->cmd_state != SEC_CMD_STATUS_OK) {
		input_err(true, ts->dev, "%s: isram test failed!\n", __func__);
		goto out;
	}
...
//	{SEC_CMD("get_crc_check", get_checksum_data),},
...
{
	int mode;
	uint8_t addr_keep_act[4] = {0xD4, 0x7F, 0x00, 0x10};
	uint8_t send_keep_act[4] = {0x00, 0x00, 0x00, 0x00};
	uint8_t recv_keep_act[4] = {0x00, 0x00, 0x00, 0x00};
	int retry_cnt = 0;
	...
	if (strcmp(HX_83108A_SERIES_PWON, private_ts->chip_name) == 0
		...
		if (mode == 1) {
			...
			send_keep_act[3] = 0xCD;
			send_keep_act[2] = 0xCD;
			send_keep_act[1] = 0xCD;
			send_keep_act[0] = 0xCD;
		} else {
			I("%s: Back to normal!\n", __func__);
			send_keep_act[3] = 0x00;
			send_keep_act[2] = 0x00;
			send_keep_act[1] = 0x00;
			send_keep_act[0] = 0x00;
		}
		do {
			g_core_fp.fp_register_write(addr_keep_act, DATA_LEN_4, send_keep_act, 0);
			usleep_range(1000, 1100);
			g_core_fp.fp_register_read(addr_keep_act, DATA_LEN_4, recv_keep_act, 0);
			retry_cnt++;
		...
		struct device_attribute *attr, char *buf)
{
	...
	input_info(true, ts->dev, "%s: %d\n", __func__, ts->ear_detect_val);

	return snprintf(buf, SEC_CMD_BUF_SIZE, "%d", ts->ear_detect_val != 3 ? 0 : 3);
}
		...
		const char *buf, size_t count)
{
	...
	if (!ts->pdata->support_ear_detect) {
		input_err(true, ts->dev, "%s: Not support EarDetection!\n", __func__);
		ret = -1;
		goto out;
	}

	if (!(sec->cmd_param[0] == 0 || sec->cmd_param[0] == 1 || sec->cmd_param[0] == 3)) {
		input_err(true, ts->dev, "%s: invalid parameter %d\n", __func__, sec->cmd_param[0]);
		ret = -1;
		goto out;
	} else {
	...
	if (ts->suspended) {
		input_err(true, ts->dev, "%s: now screen off stae, it'll setting after screen on(%d)!\n",
					...
					char *buf)
{
	...
	return snprintf(buf, SEC_CMD_BUF_SIZE, "%d", !ts->suspended);
}
					...
					const char *buf, size_t count)
{
	...
	if (!ts->pdata->early_suspend || !ts->pdata->late_suspend || !ts->pdata->early_resume || !ts->pdata->late_resume) {
		input_err(true, ts->dev, "%s: suspend/resume function is not connected\n", __func__);
		return count;
	}
	...
	if (buff[0] == LCD_ON || buff[0] == LCD_DOZE || buff[0] == LCD_DOZE_SUSPEND) {
		if (buff[1] == LCD_EARLY_EVENT) {
			...
			ts->pdata->late_resume(ts->dev);
		}
	} else if (buff[0] == LCD_OFF) {
		if (buff[1] == LCD_EARLY_EVENT) {
			ts->pdata->early_suspend(ts->dev);
		} else if (buff[1] == LCD_LATE_EVENT) {
...
//	data->sec.wait_cmd_result_done = true;
	...
	g_sec_raw_buff->_rawdata =
		kzalloc(sizeof(int32_t) * ic_data->HX_TX_NUM * ic_data->HX_RX_NUM,
			GFP_KERNEL);
