...
static uint32_t himax_get_rawdata(int32_t RAW[], uint32_t datalen, uint8_t checktype)
{
	...
	for (i = 0; i < ic_data->HX_TX_NUM*ic_data->HX_RX_NUM; i++) {
		...
		if (ic_data->notch_sz > 0) {
			if (i == ic_data->notch_arr[notch_idx]
				&& notch_idx < ic_data->notch_sz) {
				if (g_ts_dbg != 0) {
					I("%s: Now notch idx=%d, assigned val[%d]=%d, org val=%d\n",
						__func__, i, i - 1, RAW[i - 1], RAW[i]);
				}
				/* It's notch, assign as value of neighbor*/
				RAW[i] = RAW[i - 1];
				notch_idx++;
			}
		}
...
//	g_dc_max = g_core_fp.fp_get_max_dc();
	...
	if (checktype == HIMAX_ABS_NOISE || checktype == HIMAX_WEIGHT_NOISE) {
		...
		if (tmp_data[3] == tmp_addr[1] && tmp_data[2] == tmp_addr[0])
			...
			input_info(true, private_ts->dev, "%s: FW does not support weight test\n", __func__);

		RI("%s: weight = %d, tmp_data [0]:%02X, [1]:%02X, [2]:%02X, [3]:%02X\n",
				__func__, g_weight_val, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
		/*********************************/
	}
...
static int himax_fw_update_from_storage(bool signing, const char *file_path)
{
	...
	if (fsize > 0) {
		...
		if (!upgrade_fw) {
			input_err(true, private_ts->dev, "%s %s: upgrade_fw vzalloc fail\n", HIMAX_LOG_TAG, __func__);
		...
		if (signing) {
			...
			if (!spu_fw_data) {
				input_err(true, private_ts->dev, "%s %s: spu_fw_data vzalloc fail\n", HIMAX_LOG_TAG, __func__);
				ret = -HX_INSPECT_MEMALLCTFAIL;
				if (upgrade_fw)
					vfree(upgrade_fw);
				goto err_data_alloc;
			...
			if (nread != spu_fw_size) {
				input_err(true, private_ts->dev, "%s: failed to read signed firmware file, nread %ld Bytes\n",
					__func__, nread);
...
#endif
			input_info(true, private_ts->dev, "%s: spu_ret : %ld, spu_fw_size:%ld\n", __func__, spu_ret, spu_fw_size);

			if (spu_ret != fsize) {
				input_err(true, private_ts->dev, "%s: signature verify failed, %ld\n", __func__, spu_ret);
			...
			if (nread != fsize) {
				input_err(true, private_ts->dev, "%s: failed to read firmware file, nread %ld Bytes\n",
					__func__, nread);
				ret = -FW_ERR_FILE_READ;
				goto err_vfs_read;
			}
		...
		memcpy(private_ts->fw_data_ums, upgrade_fw, fsize);
		private_ts->fw_size_ums = fsize;
	} else {
		input_err(true, private_ts->dev, "%s [ERROR] fsize [%zu]\n", __func__, fsize);
		ret = FW_ERR_FILE_READ;
		goto err_data_alloc;
	}
	...
	hx_mcu_bin_desc_get(upgrade_fw, HX1K);
	ret = himax_zf_part_info(upgrade_fw);
#else
	switch (fsize) {
	case FW_SIZE_32k:
		ret = g_core_fp.fp_fts_ctpm_fw_upgrade_with_sys_fs_32k(
