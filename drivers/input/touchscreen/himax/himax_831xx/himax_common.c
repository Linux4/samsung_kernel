...
static void himax_print_info(struct himax_ts_data *ts)
{
	...
	input_info(true, ts->dev,
		...
		ts->touch_count, ts->SMWP_enable, ts->USB_detect_flag,
		ts->noise_mode, ts->lamp_noise_mode,
		ic_data->_ver_ic_name, ic_data->vendor_cid_maj_ver, ic_data->vendor_panel_ver, ic_data->vendor_cid_min_ver,
		ts->print_info_cnt_open, ts->print_info_cnt_release);
#else
	input_info(true, ts->dev,
		"tc:%d lp:%x noise:%d lamp:%d // v:HX%02X%02X%02X%02X // #%d %d\n",
		ts->touch_count, ts->SMWP_enable,
		ts->noise_mode, ts->lamp_noise_mode,
		ic_data->_ver_ic_name, ic_data->vendor_cid_maj_ver, ic_data->vendor_panel_ver, ic_data->vendor_cid_min_ver,
		ts->print_info_cnt_open, ts->print_info_cnt_release);
#endif
...
int himax_chip_common_init(void)
{
	...
	if (lpm_mode) {
		input_info(true, ts->dev, "%s: Do not load driver in lpm_mode %d\n", __func__, lpm_mode);
...
#ifdef HX_RST_PIN_FUNC
	if (ts->pdata->name_tsp_rst)
		regulator_put(ts->regulator_tsp_rst);
#endif
...
#ifdef HX_RST_PIN_FUNC
	if (ts->pdata->name_tsp_rst) {
		ts->regulator_tsp_rst = regulator_get(NULL, ts->pdata->name_tsp_rst);
		if (IS_ERR(ts->regulator_tsp_rst)) {
			input_err(true, ts->dev, "%s: Failed to get regulator_tsp_rst regulator.", __func__);
		}
	}
#endif
	if (ts->pdata->name_lcd_rst) {
...
int himax_ctrl_tsp_reset_regulator(struct himax_ts_data *ts, bool on)
{
	...
	if (!ts->pdata->name_tsp_rst) {
		input_err(true, ts->dev, "%s: name_tsp_rst is null\n", __func__);
		return 0;
	}

	input_info(true, ts->dev, "%s: called! %s, regulator_is_enabled(%d)\n",
				__func__, on ? "on" : "off", regulator_is_enabled(ts->regulator_tsp_rst));

	if (regulator_is_enabled(ts->regulator_tsp_rst) == on)
		return ret;

	if (on) {
		ret = regulator_enable(ts->regulator_tsp_rst);
		if (ret) {
			input_err(true, ts->dev, "%s: Failed to enable regulator_tsp_rst: %d\n", __func__, ret);
			goto out;
		}
	} else {
		regulator_disable(ts->regulator_tsp_rst);
	}

out:
	input_info(true, ts->dev, "%s: %s: done. regulator_is_enabled(%d)\n",
			__func__, on ? "on" : "off", regulator_is_enabled(ts->regulator_tsp_rst));
...
int himax_ctrl_tsp_reset(struct himax_ts_data *ts, bool on)
{
	...
	if (gpio_is_valid(ts->pdata->gpio_reset)) {
		ret = gpio_direction_output(ts->pdata->gpio_reset, on);
		if (ret)
			E("unable to set direction for gpio_reset [%d]\n", ts->pdata->gpio_reset);
	} else if (ts->pdata->name_tsp_rst)
		ret = himax_ctrl_tsp_reset_regulator(ts, on);
