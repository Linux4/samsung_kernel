...
#ifdef HX_SMART_WAKEUP || defined(HX_TOUCH_PROXIMITY)
#ifdef HX_GESTURE_TRACK
...
//				HIMAX_LOG_TAG, ts->ear_detect_mode, ts->ear_detect_val);
		if (ts->ear_detect_mode >= 1 && ts->ear_detect_val >= 0) {
...
static void himax_print_info(struct himax_ts_data *ts)
{
	...
	input_info(true, ts->dev,
		...
		ts->SMWP_enable, ts->USB_detect_flag,
		ts->noise_mode, ts->lamp_noise_mode,
		ts->print_info_cnt_open, ts->print_info_cnt_release);
#else
	input_info(true, ts->dev,
		"tc:%d v:%02X%02X lp:%x noise:%d lamp:%d // #%d %d\n",
		ts->touch_count, ic_data->vendor_cid_maj_ver, ic_data->vendor_cid_min_ver,
		ts->SMWP_enable, ts->noise_mode, ts->lamp_noise_mode,
		ts->print_info_cnt_open, ts->print_info_cnt_release);
#endif
}
...
int himax_chip_common_init(void)
{
	...
	if (lpcharge) {
		input_info(true, ts->dev, "%s: Do not load driver in lpcharge %d\n", __func__, lpcharge);
...
{
	static struct regulator *regulator_lcd_reset;

	static bool enabled;
	int ret = 0;

	if (!ts->pdata->regulator_lcd_reset)
		return 0;
	...
	if (IS_ERR_OR_NULL(regulator_lcd_reset)) {
		regulator_lcd_reset = devm_regulator_get(ts->dev, ts->pdata->regulator_lcd_reset);
		if (IS_ERR_OR_NULL(regulator_lcd_reset)) {
			input_err(true, ts->dev, "%s: Failed to get regulator_lcd_reset(%s) regulator.\n",
...
{
	static struct regulator *regulator_lcd_vdd;
	static struct regulator *regulator_lcd_bl;

	static bool enabled;
	int ret = 0;

	if (!ts->pdata->regulator_lcd_vdd || !ts->pdata->regulator_lcd_bl)
		return 0;
	...
	if (IS_ERR_OR_NULL(regulator_lcd_vdd)) {
		regulator_lcd_vdd = devm_regulator_get(ts->dev, ts->pdata->regulator_lcd_vdd);
		if (IS_ERR_OR_NULL(regulator_lcd_vdd)) {
			input_err(true, ts->dev, "%s: Failed to get regulator_lcd_vdd(%s) regulator.\n",
					__func__, ts->pdata->regulator_lcd_vdd);
			ret = PTR_ERR(regulator_lcd_vdd);
			goto error;
		}
	}

	if (IS_ERR_OR_NULL(regulator_lcd_bl)) {
		regulator_lcd_bl = devm_regulator_get(ts->dev, ts->pdata->regulator_lcd_bl);
		if (IS_ERR_OR_NULL(regulator_lcd_bl)) {
			input_err(true, ts->dev, "%s: Failed to get regulator_lcd_bl(%s) regulator.\n",
					__func__, ts->pdata->regulator_lcd_bl);
			ret = PTR_ERR(regulator_lcd_bl);
			goto error;
		}
	}
	...
	input_info(true, ts->dev, "%s: %s: [BEFORE] lcd_pwr1:%d,lcd_pwr2:%d,lcd_ldo:%d\n",
			__func__, on ? "on" : "off",
			regulator_is_enabled(regulator_lcd_vdd),
			regulator_is_enabled(regulator_lcd_bl));

	if (on) {
		ret = regulator_enable(regulator_lcd_bl);
		if (ret) {
			input_err(true, ts->dev, "%s: Failed to enable regulator_lcd_bl: %d\n", __func__, ret);
			goto out;
		}
		ret = regulator_enable(regulator_lcd_vdd);
		if (ret) {
			input_err(true, ts->dev, "%s: Failed to enable regulator_lcd_vdd: %d\n", __func__, ret);
			goto out;
		}
	} else {
		regulator_disable(regulator_lcd_vdd);
		regulator_disable(regulator_lcd_bl);
	}
...
int himax_chip_common_early_suspend(struct himax_ts_data *ts)
{
	...
	input_info(true, ts->dev, "%s %s: START lp:%d cover:%d, ear_mode:%d, ear_val:%d, suspend_mode:%d\n",
			HIMAX_LOG_TAG, __func__, ts->SMWP_enable, ts->cover_closed, ts->ear_detect_mode, ts->ear_detect_val,
			atomic_read(&ts->suspend_mode));
