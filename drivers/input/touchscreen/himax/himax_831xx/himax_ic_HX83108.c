...
static void himax_hx83108a_system_reset(void)
{
	int ret = 0;
	uint8_t tmp_data[DATA_LEN_4];
	int retry = 0;
...
static void hx83108a_dd_en_read(int en)
{
	...
	if (en) {
		...
		usleep_range(1000, 1001);
		g_core_fp.fp_register_write(addr_pwd1, DATA_LEN_4, en_pwd1, false);
		usleep_range(1000, 1001);
...
static void hx83108a_proximity_display_off(void)
{
...
static void hx83108a_proximity_sleep_in(void)
{
...
static void hx83108a_proximity_display_on(void)
{
...
static void hx83108a_proximity_sleep_out(void)
{
...
static void hx83108_func_re_init(void)
{
	...
	g_core_fp._dd_en_read = hx83108a_dd_en_read;
	g_core_fp._proximity_display_off = hx83108a_proximity_display_off;
	g_core_fp._proximity_sleep_in = hx83108a_proximity_sleep_in;
	g_core_fp._proximity_display_on =  hx83108a_proximity_display_on;
	g_core_fp._proximity_sleep_out = hx83108a_proximity_sleep_out;
#if defined(HX_ZERO_FLASH)
	g_core_fp.fp_system_reset = himax_hx83108a_system_reset;
#endif
}

static void hx83108_reg_re_init(void)
{
	// (*kp_private_ts)->ic_notouch_frame = hx83108_notouch_frame;
}
...
{
	kp_himax_in_parse_assign_cmd(hx83108a_fw_addr_raw_out_sel,
		(*kp_pfw_op)->addr_raw_out_sel,
		sizeof((*kp_pfw_op)->addr_raw_out_sel));
	// (*kp_private_ts)->ic_notouch_frame = hx83108a_notouch_frame;
}
...
static bool hx83108_chip_detect(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		&& ((tmp_data[1] == 0x8a))) {
			...
			(*kp_ic_data)->dsram_size = hx83108a_dsram_size;
			(*kp_ic_data)->isram_size = hx83108a_isram_size;
			(*kp_ic_data)->dsram_addr = hx83108a_dsram_addr;
			(*kp_ic_data)->isram_addr = hx83108a_isram_addr;
...
DECLARE(HX_MOD_KSYM_HX83108);

static int himax_hx83108_probe(void)
{
	I("%s:Enter\n", __func__);
	himax_add_chip_dt(hx83108_chip_detect);

	return 0;
}

static int himax_hx83108_remove(void)
{
	free_chip_dt_table();
	return 0;
}

static int __init himax_hx83108_init(void)
{
	int ret = 0;

	I("%s\n", __func__);
	ret = himax_hx83108_probe();
	return 0;
}

static void __exit himax_hx83108_exit(void)
{
	himax_hx83108_remove();
}
