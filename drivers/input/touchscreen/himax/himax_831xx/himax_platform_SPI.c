					...
					struct himax_i2c_platform_data *pdata)
{
	...
	u32 data = 0;
	int lcdtype = 0;
	int fw_name_cnt, select_lcd_count = 0;
	int lcd_id_num = 1;
	...
	if (of_property_read_string(dt, "himax,name_tsp_rst", &pdata->name_tsp_rst)) {
		input_err(true, ts->dev, "%s: Failed to get name_tsp_rst property\n", __func__);
		pdata->name_tsp_rst = NULL;
	}
#endif
	/* lcd regulator */
...
int himax_gpio_power_config(struct himax_i2c_platform_data *pdata)
{
	...
	if (gpio_is_valid(pdata->gpio_reset)) {
		...
		if (error) {
			E("unable to request gpio_reset [%d] %d\n", pdata->gpio_reset, __LINE__);
			goto err_regulator_on;
		}
	}

	error = himax_ctrl_tsp_reset(private_ts, false);
	if (error)
	...
	if (gpio_is_valid(pdata->gpio_irq)) {
		...
		if (error) {
			E("unable to request gpio_irq [%d] %d\n", pdata->gpio_irq, __LINE__);
...
#ifdef HX_RST_PIN_FUNC
	error = himax_ctrl_tsp_reset(private_ts, true);
	if (error)
		goto err_set_gpio_irq;
		...
		unsigned long cmd, void *data)
{
	...
	case STATUS_VBUS_HIGH:
		...
		{
			private_ts->USB_detect_flag = true;
	...
	case STATUS_VBUS_LOW:
		I("%s : detach\n", __func__);
#if defined(HX_USB_DETECT_GLOBAL)
		private_ts->USB_detect_flag = false;
...
static int tsp_panel_notification(struct notifier_block *n, unsigned long data, void *v)
{
	input_info(true, private_ts->dev, "%s: data = %ld\n", __func__, data);

	if (data == PANEL_EVENT_UB_CON_CHANGED) {
		int i = *((char *)v);

		input_info(true, private_ts->dev, "%s: data = %ld, i = %d\n", __func__, data, i);

		if (i == PANEL_EVENT_UB_CON_DISCONNECTED) {
			input_info(true, private_ts->dev, "%s: UB_CON_DISCONNECTED : disable irq & pin control\n", __func__);
			himax_int_enable(0);
			himax_pinctrl_configure(private_ts, false);
			msleep(private_ts->pdata->one_frame_delay);
#ifdef HX_RST_PIN_FUNC
			himax_ctrl_tsp_reset(private_ts, false);
#endif
		}
	}
	return 0;
...
#endif
#if (IS_ENABLED(CONFIG_EXYNOS_DPU30) || IS_ENABLED(CONFIG_DRM_SAMSUNG_DPU)) && IS_ENABLED(CONFIG_PANEL_NOTIFY)
	ts->panel_nb.priority = 1;
	ts->panel_nb.notifier_call = tsp_panel_notification;
 	ret = panel_notifier_register(&ts->panel_nb);
...
#else
#if IS_ENABLED(CONFIG_MUIC_NOTIFIER)
	muic_notifier_unregister(&ts->muic_nb);
#endif
#endif
#if IS_ENABLED(CONFIG_VBUS_NOTIFIER)
