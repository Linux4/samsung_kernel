...
*/
	...
	if (of_property_read_string(dt, "himax,regulator_lcd_vdd", &pdata->regulator_lcd_vdd)) {
		input_err(true, ts->dev, "%s: Failed to get regulator_dvdd name property\n", __func__);
		return -EINVAL;
	}

	if (of_property_read_string(dt, "himax,regulator_lcd_reset", &pdata->regulator_lcd_reset)) {
		input_err(true, ts->dev, "%s: Failed to get regulator_dvdd name property\n", __func__);
		return -EINVAL;
	}

	if (of_property_read_string(dt, "himax,regulator_lcd_bl", &pdata->regulator_lcd_bl)) {
		input_err(true, ts->dev, "%s: Failed to get regulator_dvdd name property\n", __func__);
		return -EINVAL;
	}
