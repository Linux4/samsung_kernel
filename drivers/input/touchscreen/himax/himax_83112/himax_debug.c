				...
				size_t len, loff_t * pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (g_ts->chip_cell_type == CHIP_IS_ON_CELL) {
			ret +=
		...
		if (g_ts->chip_cell_type == CHIP_IS_IN_CELL) {
			ret += snprintf(temp_buf + ret, len - ret,
					"Cusomer = %s \n",
					...
					size_t len, loff_t * pos)
{
	...
	if (k == 4) {
		...
		input_info(true, ts->dev, "%s %s: %d, %d, %d, %d\n",
						...
						size_t len, loff_t * pos)
{
	...
	input_info(true, ts->dev, "%s %s: Now debug level value=%d\n",
					...
					size_t len, loff_t * pos)
{
	...
	input_info(true, g_ts->dev, "%s himax %s \n", buf,
			HIMAX_LOG_TAG);

	if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' && buf[2] == 'x') {
		length = strlen(buf);

/* input_info(true, ts->dev, "%s: length = %d.\n", __func__,length); */
		...
		input_info(true, g_ts->dev, "%s %s: %s.\n",
		...
		if (buf[0] == 'r') {
			...
			if (strcmp(HX_85XX_H_SERIES_PWON, g_ts->chip_name)
				== 0 && cfg_flag == 0)
				cfg_flag = 2;
		} else if (buf[0] == 'w') {
			if (buf[3] == 'F' && buf[4] == 'E') {
...
void himax_log_touch_data(int start)
{
	...
	if (hx_touch_data->diag_cmd > 0) {
	...
	else if (g_ts->SMWP_enable > 0 && g_ts->suspended) {
		...
		input_err(true, g_ts->dev, "%s %s:cmd fault\n",
				HIMAX_LOG_TAG, __func__);
	}
...
void himax_log_touch_event(struct himax_ts_data *ts, int start)
{
	...
	if (g_target_report_data->finger_on > 0
		...
		for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
			...
			&& g_target_report_data->finger_num == 0) {
		input_info(true, ts->dev, "%s All Finger leave\n",
				HIMAX_LOG_TAG);
	} else {
		input_info(true, ts->dev, "%s %s : wrong input!\n",
				HIMAX_LOG_TAG, __func__);
	}
}
...
void himax_log_touch_int_devation(int touched)
{
	if (touched == HX_FINGER_ON) {
		...
		/*  input_info(true, ts->dev, " Irq start time = %ld.%06ld s\n",
...
/*  input_info(true, ts->dev, "Irq finish time = %ld.%06ld s\n",
...
/* input_info(true, ts->dev, "dbg hx_point_num=%d,old_finger=0x%02X,pre_finger_mask=0x%02X\n",ts->hx_point_num,ts->old_finger,ts->pre_finger_mask);*/
			}
		}
	}
}
...
/* input_info(true, ts->dev, "debug level 1\n"); */
...
/* 1:common dsram,2:100 frame Max,3:N-(N-1)frame */
	...
	dsram_type = ts->diag_cmd / 10;
	input_info(true, ts->dev, "%s %s:Entering ts->diag_cmd=%d!\n",
	...
	if (dsram_type >= 1 && dsram_type <= 3) {
		queue_delayed_work(g_ts->himax_diag_wq,
					&g_ts->himax_diag_delay_wrok,
					1 / 10 * HZ);
...
/* 1:IIR,2:DC,3:Bank,4:IIR2,5:IIR2_N,6:FIR2,7:Baseline,8:dump coord */
	...
	input_info(true, ts->dev, "%s %s:g_switch_mode = %d\n",
	...
	if (storage_type == 0 && rawdata_type > 0 && rawdata_type < 8) {
		input_info(true, ts->dev, "%s %s,common\n",
				HIMAX_LOG_TAG, __func__);
		...
		if (DSRAM_Flag) {
			...
			DSRAM_Flag = false;
			/* 2. Stop DSRAM thread */
			cancel_delayed_work(&g_ts->himax_diag_delay_wrok);
			/* 3. Enable ISR */
			himax_int_enable(1);
		...
		g_core_fp.fp_diag_register_set(command[0], storage_type);
			...
			&& rawdata_type < 8) {
		input_info(true, ts->dev, "%s %s,dsram\n",
				HIMAX_LOG_TAG, __func__);
		diag_max_cnt = 0;
		...
		input_info(true, ts->dev, "%s Soritng mode!\n",
				HIMAX_LOG_TAG);
				...
				size_t len, loff_t * pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (debug_level_cmd == 't') {
			...
			if (g_ts->protocol_type == PROTOCOL_TYPE_A) {
				ret += snprintf(temp_buf + ret, len - ret,
						"Protocol : TYPE_A\n");
			} else {
				ret += snprintf(temp_buf + ret, len - ret,
...
/* parse the file name */
		...
		input_info(true, g_ts->dev,
				"%s %s: upgrade from file(%s) start!\n",
				HIMAX_LOG_TAG, __func__, fileName);
		result = request_firmware(&fw, fileName, g_ts->dev);
...
/*	start to upgrade */
		...
		input_info(true, g_ts->dev,
				"%s NOW debug echo r!\n", HIMAX_LOG_TAG);
/* himax_program_sram(); */
		g_ts->himax_0f_update_wq =
			create_singlethread_workqueue("HMX_update_0f_reuqest_write");

		if (!g_ts->himax_0f_update_wq)
			input_err(true, g_ts->dev,
					"%s allocate syn_update_wq failed\n",
					HIMAX_LOG_TAG);

		INIT_DELAYED_WORK(&g_ts->work_0f_update,
					g_core_fp.fp_0f_operation);
		queue_delayed_work(g_ts->himax_0f_update_wq,
					&g_ts->work_0f_update,
...
static int himax_proc_flash_read(struct seq_file *s, void *v)
{
	...
	input_info(true, g_ts->dev, "%s flash_progress = %d \n",
					...
					loff_t * data)
{
	...
	memset(buf_tmp, 0x0, sizeof(buf_tmp));
	input_info(true, g_ts->dev, "%s %s: buf = %s\n",
	...
	if (buf[0] == '1') {	/* 1_32,1_60,1_64,1_24,1_28 for flash size 32k,60k,64k,124k,128k */
		...
		queue_work(g_ts->flash_wq, &g_ts->flash_work);
					...
					size_t len, loff_t * pos)
{
	...
	if (buf[0] == '0') {
		...
		input_info(true, g_ts->dev, "%s Do nothing \n",
				HIMAX_LOG_TAG);
	}
					...
					size_t len, loff_t * pos)
{
	...
	input_info(true, g_ts->dev, "%s Clear ESD Flag \n",
			HIMAX_LOG_TAG);
					...
					size_t len, loff_t * pos)
{
	...
	input_info(true, g_ts->dev, "%s guest info progress\n",
			HIMAX_LOG_TAG);
	if (!HX_PROC_SEND_FLAG) {
		temp_buf = kzalloc(len, GFP_KERNEL);
...
// int shift = 0;
	...
	input_info(true, g_ts->dev, "%s %s: buf = %s\n",
			HIMAX_LOG_TAG, __func__, buf);
	if (buf[0] == 'r') {
		input_info(true, g_ts->dev, "%s %s,Test to get",
				HIMAX_LOG_TAG, __func__);
		queue_work(g_ts->guest_info_wq,
				&g_ts->guest_info_work);
	}
...
int hx_set_sram_raw(int set_val)
{
	...
	if (set_val) {
		if (DSRAM_Flag) {
			...
			g_ts->diag_cmd = HX_NOISE_SELCT + HX_DSRAM_EN;
		...
		if (DSRAM_Flag) {
			...
			DSRAM_Flag = false;
			/* (2) Stop DSRAM thread */
			g_ts->diag_cmd = HX_DSRAM_DIS + HX_NORMAL_SELCT;
...
int hx_set_stack_raw(int set_val)
{
	...
	g_core_fp.fp_diag_register_set(g_ts->diag_cmd, 0);
