...
static int himax_mcu_flash_write_burst_lenth(uint8_t *reg_byte, uint8_t *write_data, uint32_t length)
{
	...
	/* if (length + ADDR_LEN_4 > FLASH_RW_MAX_LEN) {
		input_err(true, g_ts->dev, "%s %s: write len over %d!\n", HIMAX_LOG_TAG, __func__, FLASH_RW_MAX_LEN);
		return;
	} */
...
/*input_info(true, g_ts->dev, "%s,Entering \n", __func__);*/
	if (cfg_flag == 0) {
				...
				uint8_t hb, uint8_t lb)
{
	...
	do {
		...
		/* input_info(true, g_ts->dev, "%s:Now tmp_data[0]=0x%02X,[1]=0x%02X,[2]=0x%02X,[3]=0x%02X\n",
...
static void himax_mcu_sense_on(uint8_t FlashMode)
{
	uint8_t tmp_data[DATA_LEN_4];
	int retry = 0;
	input_info(true, g_ts->dev, "%s Enter %s \n",
			HIMAX_LOG_TAG, __func__);
	...
	/* only for a21s factory jig */
	msleep(100);
	g_core_fp.set_bending_algo_for_factory(FW_SET_FACTORY_ON);
#endif
	input_info(true, g_ts->dev, "%s End %s\n", HIMAX_LOG_TAG, __func__);
...
{
	input_info(true, g_ts->dev, "%s %s: resetting glove_mode,%d\n",
		HIMAX_LOG_TAG, __func__, g_ts->glove_enabled);
	g_core_fp.fp_set_HSEN_enable(g_ts->glove_enabled, false);
}
...
static uint32_t himax_mcu_check_CRC(uint8_t * start_addr, int reload_length)
{
	...
	if (ret < NO_ERR) {
		input_err(true, g_ts->dev, "%s: i2c access fail!\n", __func__);
		return HW_CRC_FAIL;
	}
...
static int himax_mcu_black_gest_ctrl(bool enable)
{
	...
	input_info(true, g_ts->dev,
			...
			__func__, enable, g_ts->suspended);
...
static void himax_mcu_set_HSEN_enable(uint8_t HSEN_enable, bool suspended)
{
	...
	do {
		...
		g_core_fp.fp_register_read(pfw_op->addr_hsen_enable, DATA_LEN_4,
					   tmp_data, 0);
/*input_info(true, g_ts->dev, "%s: tmp_data[0]=%d, HSEN_enable=%d, retry_cnt=%d \n", __func__, tmp_data[0],HSEN_enable,retry_cnt);*/
		retry_cnt++;
	} while ((tmp_data[3] != back_data[3] || tmp_data[2] != back_data[2]
			|| tmp_data[1] != back_data[1] || tmp_data[0] != back_data[0])
			&& retry_cnt < HIMAX_REG_RETRY_TIMES);
...
static void himax_mcu_usb_detect_set(uint8_t * cable_config)
{
	...
	do {
		...
		g_core_fp.fp_register_read(pfw_op->addr_usb_detect, DATA_LEN_4,
						tmp_data, 0);
/*input_info(true, g_ts->dev, "%s: tmp_data[0]=%d, USB detect=%d, retry_cnt=%d \n", __func__, tmp_data[0],cable_config[1] ,retry_cnt);*/
		retry_cnt++;
	} while ((tmp_data[3] != back_data[3] || tmp_data[2] != back_data[2]
			|| tmp_data[1] != back_data[1] || tmp_data[0] != back_data[0])
			&& retry_cnt < HIMAX_REG_RETRY_TIMES);
...
static void himax_mcu_idle_mode(int disable)
{
	...
	input_info(true, g_ts->dev, "%s: setting OK!\n",
			__func__);
}
...
static void himax_mcu_reload_disable(int disable)
{
	...
	input_info(true, g_ts->dev, "%s %s: setting OK!\n",
			HIMAX_LOG_TAG, __func__);
}
...
static bool himax_mcu_check_chip_version(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x10)
			&& (tmp_data[1] == 0x2a)) {
			strlcpy(g_ts->chip_name, HX_83102A_SERIES_PWON,
				30);
			ret_data = true;
			break;
		} else {
...
static void himax_mcu_read_FW_ver(void)
{
	...
	while (reload_status == 0) {
		...
		if ((data[1] == 0x9A && data[0] == 0xA9)
			...
			himax_in_parse_assign_cmd(0x10007F40, tmp_addr, sizeof(tmp_addr));
			g_core_fp.fp_register_read(tmp_addr, DATA_LEN_4, tmp, 0);
			input_err(true, g_ts->dev,"%s %s: 0x10007F40 : 0~3 [0x%X][0x%X][0x%X][0x%X]\n",
						HIMAX_LOG_TAG, __func__, tmp[0],tmp[1],tmp[2],tmp[3]);

			himax_in_parse_assign_cmd(0x900000A8, tmp_addr, sizeof(tmp_addr));
			g_core_fp.fp_register_read(tmp_addr, DATA_LEN_4, tmp, 0);
			input_err(true, g_ts->dev,"%s %s: 0x900000A8 : 0~3 [0x%X][0x%X][0x%X][0x%X]\n",
						HIMAX_LOG_TAG, __func__, tmp[0],tmp[1],tmp[2],tmp[3]);

			himax_in_parse_assign_cmd(0x900000E8, tmp_addr, sizeof(tmp_addr));
			g_core_fp.fp_register_read(tmp_addr, DATA_LEN_4, tmp, 0);
			input_err(true, g_ts->dev,"%s %s: 0x900000E8 : 0~3 [0x%X][0x%X][0x%X][0x%X]\n",
						HIMAX_LOG_TAG, __func__, tmp[0],tmp[1],tmp[2],tmp[3]);
	...
	input_info(true, g_ts->dev,
			...
			data_2[1]);
	input_info(true, g_ts->dev, "reload_status=%d\n",
			reload_status);
	/*=====================================
	Read FW version
	=====================================*/
	...
	input_info(true, g_ts->dev, "%s PANEL_VER : %X \n",
			HIMAX_LOG_TAG, ic_data->vendor_panel_ver);
	input_info(true, g_ts->dev, "%s FW_VER : %X \n",
	...
	input_info(true, g_ts->dev, "%s CID_VER : %X \n",
			HIMAX_LOG_TAG,
	...
	input_info(true, g_ts->dev, "%s Cusomer ID = %s \n",
	...
	input_info(true, g_ts->dev, "%s Project ID = %s \n",
...
static void himax_mcu_return_event_stack(void)
{
	...
	do {
		input_info(true, g_ts->dev, "%s now %d times!\n",
				HIMAX_LOG_TAG, retry);
	...
	input_info(true, g_ts->dev, "%s %s: End of setting!\n",
			HIMAX_LOG_TAG, __func__);
}
...
static bool himax_mcu_calculateChecksum(bool change_iref)
{
	...
	if (CRC_result != 0) {
		input_info(true, g_ts->dev, "%s: CRC Fail=%d\n", __func__, CRC_result);
	}
...
static void himax_mcu_irq_switch(int switch_on)
{
	if (switch_on) {
		if (g_ts->use_irq) {
			...
			hrtimer_start(&g_ts->timer, ktime_set(1, 0),
					HRTIMER_MODE_REL);
		}
	} else {
		if (g_ts->use_irq) {
			...
			hrtimer_cancel(&g_ts->timer);
			cancel_work_sync(&g_ts->work);
		}
	}
}
...
static int himax_mcu_switch_mode(int mode)
{
	...
	while (retry != 0) {
		input_info(true, g_ts->dev, "%s %s: Read [%d]\n",
				HIMAX_LOG_TAG, __func__, retry);
...
static int himax_mcu_get_max_dc(void)
{
	...
	input_info(true, g_ts->dev, "%s %s: dc max = %d\n",
...
int hx_mcu_set_edge_border(int set_val)
{
	...
	input_info(true, g_ts->dev, "%s %s:Now set_val=%d\n",
...
int hx_mcu_set_bending_algo_for_factory(int set_val)
{
	int ret = NO_ERR;
	int retry = 10;
	uint8_t tmp_data[DATA_LEN_4] = { 0 };

	input_info(true, g_ts->dev, "%s %s:Now set_val=%d, reg=0x%02X%02X%02X%02X\n",
			HIMAX_LOG_TAG, __func__, set_val,
			pfw_op->addr_bending_factory[0], pfw_op->addr_bending_factory[1],
			pfw_op->addr_bending_factory[2], pfw_op->addr_bending_factory[3]);
	do {
		g_core_fp.fp_register_read(pfw_op->addr_bending_factory, DATA_LEN_4,
						tmp_data, 0);
		input_info(true, g_ts->dev,
				"%s:Now [0]=0x%02X,[1]=0x%02X,[2]=0x%02X,[3]=0x%02X\n",
				__func__, tmp_data[0], tmp_data[1], tmp_data[2],
		...
		tmp_data[0] = set_val;
		g_core_fp.fp_register_write(pfw_op->addr_bending_factory,
						DATA_LEN_4, tmp_data, false);
		input_info(true, g_ts->dev,
				"%s 1. After write value\n", HIMAX_LOG_TAG);
		g_core_fp.fp_register_read(pfw_op->addr_bending_factory, DATA_LEN_4,
...
int hx_mcu_set_cal_switch(int set_val)
{
	...
	do {
		...
		input_info(true, g_ts->dev,
				"%s Now write value\n", HIMAX_LOG_TAG);
		input_info(true, g_ts->dev, "%s Sense off\n",
				HIMAX_LOG_TAG);
		g_core_fp.fp_sense_off(true);
		input_info(true, g_ts->dev,
				"%s Disable reload!\n", HIMAX_LOG_TAG);
...
/*clear bit*/
		...
		input_info(true, g_ts->dev, "%s Sense on\n",
				HIMAX_LOG_TAG);
		g_core_fp.fp_sense_on(0x00);
				...
				uint32_t write_len)
{
	...
	input_info(true, g_ts->dev, "%s %s: Now write_len=%d\n",
...
static bool himax_mcu_flash_lastdata_check(void)
{
	...
	for (temp_addr = start_addr; temp_addr < (start_addr + flash_page_len);
		...
		/*input_info(true, g_ts->dev, "temp_addr=%d,tmp_addr[0]=0x%2X, tmp_addr[1]=0x%2X,tmp_addr[2]=0x%2X,tmp_addr[3]=0x%2X\n", temp_addr,tmp_addr[0], tmp_addr[1], tmp_addr[2],tmp_addr[3]);*/
...
static bool himax_mcu_get_DSRAM_data(uint8_t * info_data, bool DSRAM_Flag)
{
	...
	input_info(true, g_ts->dev, "%s,m_key_num=%d\n",__func__ ,m_key_num);
	...
	for (i = 0; i < total_read_times; i++) {
		...
		/*input_info(true, g_ts->dev, "%s address = %08X \n", __func__, address);*/
		...
		if (total_size_temp >= max_i2c_size) {
			...
			/*input_info(true, g_ts->dev, "last total_size_temp=%d\n",total_size_temp);*/
	...
	/* 4. FW stop outputing */
	/*input_info(true, g_ts->dev, "DSRAM_Flag=%d\n",DSRAM_Flag);*/
	if (DSRAM_Flag == false || g_ts->diag_cmd == 0) {
	/*input_info(true, g_ts->dev, "Return to Event Stack!\n");*/
		g_core_fp.fp_register_write(psram_op->addr_rawdata_addr,
						DATA_LEN_4, psram_op->data_fin, 0);
	} else {
	/*input_info(true, g_ts->dev, "Continue to SRAM!\n");*/
		g_core_fp.fp_register_write(psram_op->addr_rawdata_addr,
	...
	if (check_sum_cal % 0x10000 != 0) {
		...
		/*input_info(true, g_ts->dev, "%s checksum PASS \n", __func__);*/
	}
...
{
	input_info(true, g_ts->dev, "%s: use default incell init.\n", __func__);
}
...
static void himax_mcu_pin_reset(void)
{
	...
	error = gpio_request(g_ts->rst_gpio, "himax-reset");
	...
	himax_rst_gpio_set(g_ts->rst_gpio, 0);
	msleep(20);
	himax_rst_gpio_set(g_ts->rst_gpio, 1);
	msleep(50);

	if (gpio_is_valid(g_ts->rst_gpio))
		gpio_free(g_ts->rst_gpio);
}
...
static void himax_mcu_ic_reset(uint8_t loadconfig, uint8_t int_off)
{
	...
	input_info(true, g_ts->dev, "%s %s,status: loadconfig=%d,int_off=%d\n",
		HIMAX_LOG_TAG, __func__, loadconfig, int_off);
...
static void himax_mcu_touch_information(void)
{
	...
	/*input_info(true, g_ts->dev, "%s : HX_RX_NUM=%d,ic_data->HX_TX_NUM=%d,ic_data->HX_MAX_PT=%d\n",__func__,ic_data->HX_RX_NUM,ic_data->HX_TX_NUM,ic_data->HX_MAX_PT);*/
	g_core_fp.fp_register_read(pdriver_op->addr_fw_define_xy_res_enable,
					DATA_LEN_4, data, 0);

	/*input_info(true, g_ts->dev, "%s : c_data->HX_XY_REVERSE=0x%2.2X\n",__func__,data[1]);*/
	if ((data[1] & 0x04) == 0x04) {
	...
	input_info(true, g_ts->dev, "%s : ic_data->HX_Y_RES=%d, ic_data->HX_X_RES=%d\n",
	...
	/*input_info(true, g_ts->dev, "%s : data[0]=0x%2.2X,data[1]=0x%2.2X,data[2]=0x%2.2X,data[3]=0x%2.2X\n",__func__,data[0],data[1],data[2],data[3]);
	input_info(true, g_ts->dev, "data[0] & 0x01 = %d\n",(data[0] & 0x01));*/
	if ((data[1] & 0x01) == 1) {
	...
	/* input_info(true, g_ts->dev, "%s: tmp_data[0] = 0x%02X,tmp_data[1] = 0x%02X,tmp_data[2] = 0x%02X,tmp_data[3] = 0x%02X\n",
	__func__, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);*/
...
#endif
	input_info(true, g_ts->dev, "%s:HX_RX_NUM =%d,HX_TX_NUM =%d,HX_MAX_PT=%d\n",
				__func__, ic_data->HX_RX_NUM, ic_data->HX_TX_NUM, ic_data->HX_MAX_PT);
	input_info(true, g_ts->dev, "%s:HX_XY_REVERSE =%d,HX_Y_RES =%d,HX_X_RES=%d\n",
				__func__, ic_data->HX_XY_REVERSE, ic_data->HX_Y_RES, ic_data->HX_X_RES);
	input_info(true, g_ts->dev, "%s:HX_INT_IS_EDGE =%d \n",
...
/* Check 128th byte CRC */
	...
	if (check_sum_cal % HX64K != 0) {
		input_info(true, g_ts->dev, "%s %s fail=%2X \n",
...
/* uint32_t checksum = 0x00; */
	...
	input_info(true, g_ts->dev, "%s %s: Checksum = 0x%8X\n",
	...
	for (custom_info_temp = 0; custom_info_temp < HX_GUEST_INFO_SIZE;
		...
		/*if(custom_info_temp == 0)
		{
			for( i = 0; i< 256 ; i++)
			{
				if(i % 16 == 0 && i > 0)
					input_info(true, g_ts->dev, "\n");
				input_info(true, g_ts->dev, "%s g_guest_info_data->g_guest_str[%d][%d] = 0x%02X", HIMAX_LOG_TAG, custom_info_temp,i,g_guest_info_data->g_guest_str[custom_info_temp][i]);
			}
		}*/
	}
	/* himax_burst_enable(g_ts->client, 0); */
...
void himax_mcu_clean_sram_0f(uint8_t * addr, int write_len, int type)
{
	...
	input_info(true, g_ts->dev, "%s %s, total size=%d\n",
...
void himax_mcu_write_sram_0f(u8 *data, uint8_t * addr, int start_index, uint32_t write_len)
{
	int total_read_times = 0;
	int max_bus_size = MAX_I2C_TRANS_SZ;
	int total_size_temp = 0;
	...
	for(i = 0;i<10;i++)
	{
		input_info(true, g_ts->dev, "[%d] 0x%2.2X", i, tmp_data[i]);
	}
	input_info(true, g_ts->dev, "\n");
	*/
	if (total_size_temp % max_bus_size == 0) {
	...
	for (i = 0; i < (total_read_times); i++) {
		...
		if (total_size_temp >= max_bus_size) {
			memcpy(tmp_data,
					&data[start_index + i * max_bus_size],
					max_bus_size);
			g_core_fp.fp_register_write(tmp_addr, max_bus_size,
							tmp_data, 0);
			total_size_temp = total_size_temp - max_bus_size;
		...
		/*input_info(true, g_ts->dev, "[log]write %d time end!\n", i);*/
		address = ((i + 1) * max_bus_size);
		tmp_addr[0] = addr[0] + (uint8_t) ((address) & 0x00FF);
...
int himax_sram_write_crc_check(u8 *data, uint8_t * addr, int strt_idx, uint32_t len)
{
	int retry = 0;
	int crc = -1;
...
int himax_zf_part_info(u8 *data)
{
	int offset = 0;
	int part_num = 0;
	int ret = 0;
...
/*0. check 1k header*/
	...
	for (i = 0; i < part_num; i++) {
		...
		zf_info_arr[i].write_size = buf[6] << 16 | buf[5] << 8 | buf[4];
...
void himax_mcu_firmware_update_0f(u8 *data, int size)
{
	int ret = 0;
	uint8_t tmp_data[DATA_LEN_4] = {0x01, 0x00, 0x00, 0x00};

	input_info(true, g_ts->dev, "%s: Entering - total FW size=%d\n", __func__, size);
	...
	if (size > HX64K) {
		ret = himax_zf_part_info(data);
	} else {
		/* first 48K */
		if (himax_sram_write_crc_check(data, pzf_op->data_sram_start_addr, 0, HX_48K_SZ) != 0)
			input_err(true, g_ts->dev,
...
/*config info*/
		if (G_POWERONOF == 1) {
			if (himax_sram_write_crc_check(data, pzf_op->data_cfg_info, 0xC000, 128) != 0)
				input_err(true, g_ts->dev,
		...
		if (G_POWERONOF == 1) {
			if (himax_sram_write_crc_check(data, pzf_op->data_fw_cfg_1, 0xC0FE, 528) != 0)
				input_err(true, g_ts->dev,
		...
		if (G_POWERONOF == 1) {
			if (himax_sram_write_crc_check(data, pzf_op->data_fw_cfg_3, 0xCA00, 128) != 0)
				input_err(true, g_ts->dev,
		...
		/*ADC config*/
		if (G_POWERONOF == 1) {
			if (himax_sram_write_crc_check(data, pzf_op->data_adc_cfg_1, 0xD640, 1200) != 0)
				input_err(true, g_ts->dev,
		...
		if (G_POWERONOF == 1) {
			if (himax_sram_write_crc_check(data, pzf_op->data_adc_cfg_2, 0xD320, 800) != 0)
				input_err(true, g_ts->dev,
		...
		/*mapping table*/
		if (G_POWERONOF == 1) {
			if (himax_sram_write_crc_check(data, pzf_op->data_map_table, 0xE000, 1536) != 0)
				input_err(true, g_ts->dev,
	...
	/* set n frame=0*/
/*	if (G_POWERONOF == 1) {
		g_core_fp.fp_write_sram_0f(data, pzf_op->data_mode_switch, 0xC30C, 4);
	} else {
...
*/
	...
	input_info(true, g_ts->dev, "%s: End \n", __func__);
}
...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	err = request_firmware(&fw_entry, file_name, g_ts->dev);
	if (err < 0) {
		input_err(true, g_ts->dev,
	...
	data = kzalloc(fw_entry->size, GFP_KERNEL);
	if (!data) {
		input_err(true, g_ts->dev, "%s: failed to alloc g_ts->fw_data_ums mem\n", __func__);
		err = -1;
		goto err_kzalloc_fail;
	}
	memcpy(data, fw_entry->data, fw_entry->size);

	if (!data)
		g_core_fp.fp_firmware_update_0f(data, (int)fw_entry->size);
...
err_out:
	...
	input_info(true, g_ts->dev, "%s: END(%d)\n", __func__, err);
	return err;
}
...
int himax_mcu_0f_operation_dirly(void)
{
	...
	input_info(true, g_ts->dev, "%s: fw_data_ums %s\n",
			__func__, g_ts->fw_data_ums == NULL ? "is NULL" : "use");

	himax_int_enable(0);

	if (g_f_0f_updat == 1) {
		input_info(true, g_ts->dev, "%s:[Warning]Other thread is updating now!\n", __func__);
		err = -1;
		return err;
	} else {
		input_info(true, g_ts->dev, "%s:Entering Update Flow!\n", __func__);
		g_f_0f_updat = 1;
	}

	if (g_ts->fw_data_ums == NULL) {
		input_info(true, g_ts->dev, "%s: fw_size_kernel = %d\n", __func__, g_ts->fw_size_kernel);
		g_core_fp.fp_firmware_update_0f(g_ts->fw_data_kernel, g_ts->fw_size_kernel);
	
	} else {
		input_info(true, g_ts->dev, "%s: fw_size_ums = %d\n", __func__, g_ts->fw_size_ums);
		g_core_fp.fp_firmware_update_0f(g_ts->fw_data_ums, g_ts->fw_size_ums);
	}
...
void himax_mcu_0f_operation(struct work_struct *work)
{
	...
	input_raw_info(true, g_ts->dev, "%s: file name = %s\n", __func__, g_ts->pdata->i_CTPM_firmware_name);

	err = request_firmware(&fw_entry, g_ts->pdata->i_CTPM_firmware_name, g_ts->dev);
	if (err < 0) {
		input_err(true, g_ts->dev, "%s: request_firmware fail(%d)\n", __func__, err);
		return;
	}

	g_ts->fw_data_kernel = kzalloc(fw_entry->size, GFP_KERNEL);
	if (!g_ts->fw_data_kernel) {
		input_err(true, g_ts->dev, "%s: failed to kzalloc g_ts->fw_data_kernel mem\n", __func__);
		goto err_out;
	}

	memcpy(g_ts->fw_data_kernel, fw_entry->data, fw_entry->size);
	g_ts->fw_size_kernel = (int)fw_entry->size;
	input_info(true, g_ts->dev, "%s: g_ts->fw_size_kernel = %d\n", __func__, g_ts->fw_size_kernel);
...
err_out:
	...
	input_raw_info(true, g_ts->dev, "%s: Read raw data!!!\n", __func__);
	input_raw_data_clear();
	himax_run_rawdata_all(g_ts);
				...
				int start_index, int read_len)
{
	...
	input_info(true, g_ts->dev, "%s %s, READ Start \n",
			HIMAX_LOG_TAG, __func__);
	input_info(true, g_ts->dev, "%s %s, start_index = %d \n",
	...
	for (i = 0; i < read_len; i++, j++) {
		...
		input_info(true, g_ts->dev, "0x%2.2X, ",
				temp_info_data[i]);
	...
	input_info(true, g_ts->dev, "%s %s, READ END \n",
			HIMAX_LOG_TAG, __func__);
	input_info(true, g_ts->dev, "%s %s, Not Same count=%d\n",
...
void himax_mcu_read_all_sram(uint8_t * addr, int read_len)
{
	...
	*/
	...
	{
		input_info(true, g_ts->dev, "0x%2.2X, ", temp_info_data[i]);
...
void himax_mcu_0f_operation_check(int type)
{
	...
	input_info(true, g_ts->dev, "%s %s:file name = %s\n",
			HIMAX_LOG_TAG, __func__, g_ts->pdata->i_CTPM_firmware_name);

	err = request_firmware(&fw_entry, g_ts->pdata->i_CTPM_firmware_name, g_ts->dev);
	if (err < 0) {
		input_err(true, g_ts->dev, "%s %s, fail in line%d error code=%d\n",
				HIMAX_LOG_TAG, __func__, __LINE__, err);
		return;
	}
...
static void himax_mcu_fp_init(void)
{
	...
	g_core_fp.set_bending_algo_for_factory = hx_mcu_set_bending_algo_for_factory;
...
void himax_in_parse_assign_cmd(uint32_t addr, uint8_t * cmd, int len)
{
	...
	case 1:
		cmd[0] = addr;
		/*input_info(true, g_ts->dev, "%s: cmd[0] = 0x%02X\n", __func__, cmd[0]);*/
		break;

	case 2:
		cmd[0] = addr % 0x100;
		cmd[1] = (addr >> 8) % 0x100;
		/*input_info(true, g_ts->dev, "%s: cmd[0] = 0x%02X,cmd[1] = 0x%02X\n", __func__, cmd[0], cmd[1]);*/
		break;
	...
	case 4:
		...
		/*  input_info(true, g_ts->dev, "%s: cmd[0] = 0x%02X,cmd[1] = 0x%02X,cmd[2] = 0x%02X,cmd[3] = 0x%02X\n",
...
#endif
	himax_in_parse_assign_cmd(FW_SET_BENDING_FOR_FAC, pfw_op->addr_bending_factory,
					sizeof(pfw_op->addr_bending_factory));
	himax_in_parse_assign_cmd(FW_EDGE_SHIFT_ADDR, pfw_op->addr_edge_shift,
				sizeof(pfw_op->addr_edge_shift));
