					...
					size_t len, loff_t * pos)
{
	...
	size_t ret = 0;
	char *temp_buf;
	input_info(true, g_ts->dev, "%s %s: enter, %d \n",
					...
					size_t len, loff_t * pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (copy_to_user(buf, temp_buf, len))
			input_info(true, ts->dev, "%s %s,here:%d\n",
					HIMAX_LOG_TAG, __func__, __LINE__);
				...
				size_t len, loff_t * pos)
{
	...
	input_info(true, ts->dev, "%s %s: HSEN_enable = %d.\n",
				...
				size_t len, loff_t * pos)
{
	...
	input_info(true, ts->dev, "%s %s: SMART_WAKEUP_enable = %d.\n",
					...
					size_t len, loff_t * pos)
{
	...
	input_info(true, ts->dev, "%s %s: psensor_flag = %d.\n",
...
{
	struct himax_ts_data *ts_data = g_ts;
...
static int i_get_FW(void)
{
	...
	input_info(true, g_ts->dev, "%s file name = %s\n",
			HIMAX_LOG_TAG, g_ts->pdata->i_CTPM_firmware_name);
	ret =
		request_firmware(&image, g_ts->pdata->i_CTPM_firmware_name,
				g_ts->dev);
	if (ret < 0) {
		input_err(true, g_ts->dev,
...
static int himax_wake_event_parse(struct himax_ts_data *ts, int ts_status)
{
	...
	for (i = 0; i < GEST_PTLG_ID_LEN; i++) {
		...
		input_dbg(false, ts->dev, "0x%2.2X ", buf[i]);
	...
	if (buf[GEST_PTLG_ID_LEN] == GEST_PTLG_HDR_ID1 &&
		...
		input_info(true, ts->dev, "%s gest_len = %d \n",
...
int himax_report_data_init(void)
{
	...
	input_info(true, g_ts->dev, "%s: rawdata_frame_size = %d, rawdata_size = %d\n",
				__func__, hx_touch_data->rawdata_frame_size, rawdata_size);
	input_info(true, g_ts->dev, "%s: ic_data->HX_MAX_PT:%d, hx_raw_cnt_max:%d, hx_raw_cnt_rmd:%d, g_hx_rawdata_size:%d, hx_touch_data->touch_info_size:%d\n",
	...
	if (HX_SP_INFO_SZ < (hx_touch_data->touch_all_size - hx_touch_data->touch_info_size)) {
		...
		input_err(true, g_ts->dev, "%s:Now Special data size is %d, it over than rule...\n", __func__, HX_SP_INFO_SZ);
		goto mem_alloc_fail;
	}
#endif
...
mem_alloc_fail_hx_coor_buf:
	input_info(true, g_ts->dev, "%s %s: Memory allocate fail!\n", HIMAX_LOG_TAG, __func__);
...
/* else */
/* 	input_info(true, ts->dev, "%s: Cable status is the same as previous one, ignore.\n", __func__); */
	}
}
#endif
...
void hx_log_touch_event(struct himax_ts_data *ts)
{
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		if ((((ts->old_finger >> loop_i & 1) == 0)
			...
			input_info(true, ts->dev,
				"[P] tId:%d.%d x:%d y:%d loc:%s ma:%d mi:%d tc:%d\n",
				loop_i, (ts->input_dev->mt->trkid - 1) & TRKID_MAX,
				x, y, loc, ma, mi, ts->touch_count);
#else
			input_info(true, ts->dev,
				"[P] tId:%d.%d loc:%s ma:%d mi:%d tc:%d\n",
				loop_i, (ts->input_dev->mt->trkid - 1) & TRKID_MAX,
				loc, ma, mi, ts->touch_count);
#endif
		} else
...
#ifndef CONFIG_SAMSUNG_PRODUCT_SHIP
			input_info(true, ts->dev,
				"[R] tId:%d x:%d y:%d loc:%s dd:%d,%d mc:%d tc:%d\n",
...
#endif
/* input_info(true, ts->dev, "Now Status is %d\n", result); */
	return result;
}
...
END_FUNCTION:
	if (g_ts_dbg != 0)
		input_info(true, ts->dev, "%s %s: END, ret_val=%d! \n",
						...
						int ts_status)
{
	...
	if (ts_path == HX_REPORT_COORD) {
		...
		input_err(true, g_ts->dev, "%s %s, Fail Path!\n",
				HIMAX_LOG_TAG, __func__);
					...
					int ts_status)
{
	...
	/*input_info(true, ts->dev, "%s: hx_point_num = 0!\n", __func__);*/
		for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
...
/* input_info(true, ts->dev, "EN_NoiseFilter=%d\n", EN_NoiseFilter); */
	EN_NoiseFilter = EN_NoiseFilter & 0x01;
/* input_info(true, ts->dev, "EN_NoiseFilter2=%d\n", EN_NoiseFilter); */
#if defined(HX_EN_SEL_BUTTON) || defined(HX_EN_MUT_BUTTON)
	tpd_key =
...
#endif
	default:
		input_err(true, ts->dev, "%s %s:Fail Path!\n",
			HIMAX_LOG_TAG, __func__);
		ts_status = HX_PATH_FAIL;
		break;
	}
					...
					struct himax_ts_data *ts)
{
	...
	if (g_ts_dbg != 0)
		input_info(true, ts->dev, "%s %s: Entering \n",
				HIMAX_LOG_TAG, __func__);

	if (tp_key_index != 0x00) {
		input_info(true, ts->dev, "%s virtual key index =%x\n",
		...
		input_info(true, ts->dev, "%s virtual key released\n",
				HIMAX_LOG_TAG);
		vk_press = 0;
#ifndef	HX_PROTOCOL_A
		input_mt_slot(ts->input_dev, 0);
...
static void himax_finger_report(struct himax_ts_data *ts)
{
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		if (g_target_report_data->x[i] >= 0
			...
			valid = false;
		if (g_ts_dbg != 0)
			input_info(true, ts->dev, "%s valid=%d\n",
					HIMAX_LOG_TAG, valid);
		if (valid) {
			if (g_ts_dbg != 0)
				input_info(true, ts->dev,
...
static void himax_finger_leave(struct himax_ts_data *ts)
{
	...
	if (ts->first_pressed == 1) {
		ts->first_pressed = 2;
		input_info(true, ts->dev, "%s: E1@%d, %d\n",
...
GET_TOUCH_FAIL:
	input_info(true, ts->dev, "%s %s: Now reset the Touch chip.\n",
			HIMAX_LOG_TAG, __func__);
#ifdef HX_RST_PIN_FUNC
...
static void himax_cable_tp_status_handler_func(int connect_status)
{
	...
	input_info(true, ts->dev, "%s Touch: cable change to %d\n",
...
{
	input_info(true, g_ts->dev, "%s %s: in\n", HIMAX_LOG_TAG,
			__func__);
...
static int himax_pinctrl_configure(struct himax_ts_data *ts, bool enable)
{
	struct pinctrl_state *state;

	input_dbg(true, ts->dev, "%s: %s\n", __func__, enable ? "ACTIVE" : "SUSPEND");

	if (enable) {
		state = pinctrl_lookup_state(ts->pdata->pinctrl, "on_state");
		if (IS_ERR(ts->pdata->pinctrl))
			input_err(true, ts->dev, "%s: could not get active pinstate\n", __func__);
	} else {
		state = pinctrl_lookup_state(ts->pdata->pinctrl, "off_state");
		if (IS_ERR(ts->pdata->pinctrl))
			input_err(true, ts->dev, "%s: could not get suspend pinstate\n", __func__);
	}
...
int himax_chip_common_init(void)
{
	...
	input_info(true, ts->dev, "%s: PDATA START\n", HIMAX_LOG_TAG);
	...
	input_info(true, ts->dev, "%s ic_data START\n", HIMAX_LOG_TAG);
...
/* allocate report data */
	...
	if (himax_parse_dt(ts, pdata) < 0) {
		input_err(true, ts->dev, "%s pdata is NULL for DT\n", HIMAX_LOG_TAG);
		err = -EACCES;
	...
	if (pdata->power) {
		...
		if (ret < 0) {
			input_err(true, ts->dev, "%s: power on failed\n", __func__);
	...
	if (i == g_hx_ic_dt_num) {
		input_err(true, ts->dev, "%s %s: chip detect failed!\n", HIMAX_LOG_TAG, __func__);
		err = -ENXIO;
...
found_hx_chip:
	if (g_core_fp.fp_chip_init != NULL) {
		...
		input_err(true, ts->dev, "%s: function point of chip_init is NULL!\n", __func__);
		err = -ENXIO;
		goto error_ic_detect_failed;
	}
...
/*calculate the i2c data size*/
	calcDataSize();
	input_info(true, ts->dev, "%s: calcDataSize complete\n", __func__);
#ifdef CONFIG_OF
...
#endif
	input_info(true, ts->dev, "%s: Use Protocol Type %c\n",
	...
	if (!ts->himax_att_wq) {
		input_err(true, ts->dev, "%s: allocate syn_att_wq failed\n", __func__);
		err = -ENOMEM;
