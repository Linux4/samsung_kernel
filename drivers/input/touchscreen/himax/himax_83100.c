...
#ifdef CONFIG_SEC_INCELL
	rc = of_property_read_u32(dt, "himax,lcd-id", &data);
	if (rc < 0)
		pdata->lcd_id = 0;
	else
		pdata->lcd_id = data;
	
	pr_err("%s: LCD module 0x%x\n", __func__, pdata->lcd_id);
#endif
...
static int himax_83100_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef CONFIG_FB_MSM_MDSS_SAMSUNG
	{
		...
		if (lcd_id == 0xFFFFFF) {
			pr_err("%s: LCD module was taken off\n",
					__func__);
			return -ENODEV;
		}
#ifdef CONFIG_SEC_INCELL
		if (lcd_id != pdata->lcd_id) {
			pr_err("%s: Failed to indentify LCD module(0x%x)\n",
					__func__, lcd_id);
			return -ENODEV;
		}
	#endif
	...
	if (pdata->gpio_irq>= 0) {
			ret = gpio_request(pdata->gpio_irq, "himax,irq-gpio");
			if (ret < 0)
				E("%s: request irq pin failed\n", __func__);
	}
