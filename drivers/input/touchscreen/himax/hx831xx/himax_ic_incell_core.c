...
#ifdef HX_ZERO_FLASH
		max_bus_size = (write_length > HX_MAX_WRITE_SZ) ? HX_MAX_WRITE_SZ : write_length;
#endif
...
static bool himax_mcu_dd_reg_write(uint8_t addr, uint8_t pa_num, int len, uint8_t *data, uint8_t bank)
{
	...
	w_data = kzalloc(data_len * sizeof(uint8_t), GFP_KERNEL);
	if (!w_data) {
		E("%s: allocate w_data failed\n", __func__);
		return retval;
	}

	chk_data = kzalloc(data_len * sizeof(bool), GFP_KERNEL);
	if (!chk_data) {
		E("%s: allocate chk_data failed\n", __func__);
		kfree(w_data);
		return retval;
	}
	...
	retval = g_core_fp.fp_register_write(tmp_addr, data_len, w_data, 0) == NO_ERR;
...
static void himax_mcu_set_SMWP_enable(uint8_t SMWP_enable, bool suspended)
{
	...
	I("%s: enable:%d, suspended=%d\n", __func__, SMWP_enable, suspended);
...
static void himax_mcu_set_HSEN_enable(uint8_t HSEN_enable, bool suspended)
{
	...
	I("%s: enable:%d, suspended=%d\n", __func__, HSEN_enable, suspended);
...
static void himax_mcu_read_FW_ver(void)
{
	...
	while (reload_status == 0) {
		...
		if (data_2[3] == 0x00 && data_2[2] == 0x00
	...
	I("%s : data_2[3]=0x%2.2X,data_2[2]=0x%2.2X,data_2[1]=0x%2.2X,data_2[0]=0x%2.2X\n", __func__, data_2[3], data_2[2], data_2[1], data_2[0]);
	I("reload_status=%d\n", reload_status);
	/* =====================================
	 * Read FW version
	 * =====================================
...
{
	uint8_t tmp_addr[4];
	uint32_t start_addr = 0xFFFFFFFF; /* 64K - 0x80, which is the address of the last 128bytes in 64K, default value*/
	uint32_t temp_addr = 0;
	uint32_t flash_page_len = 0x80;
	uint8_t flash_tmp_buffer[256];
...
static void himax_mcu_touch_information(void)
{
	...
	g_core_fp.fp_register_read(pdriver_op->addr_fw_define_rxnum_txnum_maxpt, DATA_LEN_4, data, 0);
	...
	ic_data->HX_MAX_PT				= SEC_FIX_MAX_PT;
	...
	ic_data->HX_INT_IS_EDGE = SEC_FIX_INT_EDGE;
...
#if IS_ENABLED(CONFIG_INPUT_SEC_NOTIFIER_LEGO)
...
void himax_mcu_clean_sram_0f(uint8_t *addr, int write_len, int type)
{
	...
	max_bus_size = (write_len > HX_MAX_WRITE_SZ) ? HX_MAX_WRITE_SZ : write_len;
...
int himax_mcu_in_cmd_struct_init(void)
{
	...
	g_internal_buffer = kzalloc(sizeof(uint8_t) * (HX_MAX_WRITE_SZ + 6), GFP_KERNEL);
