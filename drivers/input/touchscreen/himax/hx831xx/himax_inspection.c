...
#define HIMAX_UMS_FW_PATH "/sdcard/Firmware/TSP/tsp.bin"
...
static int himax_fw_update_from_storage(bool signing, const char *file_path)
{
	...
	ret = request_firmware(&fw_entry, file_path, private_ts->dev);
	if (ret) {
		E("%s: fw is not available %d, %s\n", __func__, ret, file_path);
		himax_int_enable(1);
	...
	case FW_SIZE_32k:
		ret = g_core_fp.fp_fts_ctpm_fw_upgrade_with_sys_fs_32k(
					(unsigned char *) fw_entry->data, fsize, false);
		break;
	case FW_SIZE_60k:
		ret = g_core_fp.fp_fts_ctpm_fw_upgrade_with_sys_fs_60k(
					(unsigned char *) fw_entry->data, fsize, false);
...
static void get_noise(void *dev_data)
{
	...
	RI("%s: weight = %d, %02X, %02X\n", __func__, weight, tmp_data[2], tmp_data[3]);
	/*********************************/
...
int himax_set_ap_change_mode(int mode, int enable)
{
	...
	if (strcmp(HX_83102E_SERIES_PWON, ts->chip_name) == 0) {
...
static void himax_set_cover_mode(struct himax_ts_data *ts, bool closed)
{
	u8 tmp_addr[4] = { 0 };
	u8 sdata[4] = { 0 };
	int ret;
					...
					const char *buf, size_t count)
{
	...
	if (!ts->pdata->suspend || !ts->pdata->resume) {
