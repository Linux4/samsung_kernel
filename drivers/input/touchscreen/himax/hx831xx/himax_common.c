...
void hx_log_touch_event(struct himax_ts_data *ts)
{
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		if ((((ts->old_finger >> loop_i & 1) == 0)
			...
			I("[P] tID:%d.%d x:%d y:%d major:%d minor:%d loc:%s tc:%d\n",
				loop_i,
				(ts->input_dev->mt->trkid - 1) & TRKID_MAX, x, y, ma, mi, loc,
				ts->touch_count);
#else
			I("[P] tID:%d.%d major:%d minor:%d loc:%s tc:%d\n",
				loop_i,
				(ts->input_dev->mt->trkid - 1) & TRKID_MAX, ma, mi, loc,
				ts->touch_count);
#endif
		} else
			...
			location_detect(ts, loc, x, y);
#ifndef CONFIG_SAMSUNG_PRODUCT_SHIP
			I("[R] tID:%d loc:%s dd:%d,%d mc:%d tc:%d lx:%d ly:%d\n",
				loop_i, loc, x - ts->p_x[loop_i], y - ts->p_y[loop_i],
				mc, ts->touch_count, x, y);
#else
			I("[R] tID:%d loc:%s dd:%d,%d mc:%d tc:%d\n",
				loop_i, loc, x - ts->p_x[loop_i], y - ts->p_y[loop_i],
				mc, ts->touch_count);
#endif
		}
...
static void himax_report_all_leave_event(struct himax_ts_data *ts)
{
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		...
		input_report_abs(ts->input_dev, ABS_MT_PALM, 0);
#else
		input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, 0);
#endif
...
static void himax_finger_report(struct himax_ts_data *ts)
{
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		...
		if (valid) {
			...
			input_report_abs(ts->input_dev, ABS_MT_PALM, g_target_report_data->palm[i]);
#else
...
static void himax_fb_register(struct work_struct *work)
{
	...
	ret = msm_drm_register_notifier_client(&ts->fb_notif);

	if (ret)
		E("Unable to register drm_notifier: %d\n", ret);
}
...
static void himax_print_info(struct himax_ts_data *ts)
{
	...
	input_info(true, ts->dev,
		"tc:%d v:%02X%02X lp:%x ta:%d noise:%d lamp:%d // #%d %d\n",
		ts->touch_count, ic_data->vendor_cid_maj_ver, ic_data->vendor_cid_min_ver,
		ts->SMWP_enable, USB_detect_flag,
...
found_hx_chip:
	...
	ts->initialized = true;
#if defined(CONFIG_FB) || defined(CONFIG_DISPLAY_SAMSUNG)
...
void himax_chip_common_deinit(void)
{
	...
	if (!ts->pdata->enable_sysinput_enabled) {
#if defined(CONFIG_DISPLAY_SAMSUNG)
		if (msm_drm_unregister_notifier_client(&ts->fb_notif))
...
int himax_ctrl_lcd_regulators(struct himax_ts_data *ts, bool on)
{
	...
	input_info(true, ts->dev, "%s: %s: [BEFORE] lcd_pwr1:%d,lcd_pwr2:%d,lcd_ldo:%d\n",
			__func__, on ? "on" : "off",
...
out:
	input_info(true, ts->dev, "%s: %s: [AFTER] lcd_pwr1:%d,lcd_pwr2:%d,lcd_ldo:%d\n",
		__func__, on ? "on" : "off",
			regulator_is_enabled(panel_buck_en),
			regulator_is_enabled(panel_buck_en2),
			regulator_is_enabled(panel_ldo_en));
