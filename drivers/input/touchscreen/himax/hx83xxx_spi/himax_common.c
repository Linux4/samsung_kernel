...
/*gest_event mapping to gest_key_def*/
	uint16_t gest_key_def[GEST_SUP_NUM] = {KEY_WAKEUP, KEY_BLACK_UI_GESTURE};
...
int himax_input_register(struct himax_ts_data *ts, struct input_dev *input_dev, u8 propbit)
{
...
#if defined(HX_SMART_WAKEUP)
	for (i = 0; i < GEST_SUP_NUM; i++) {
		if ((gest_key_def[i] == KEY_BLACK_UI_GESTURE) && !ts->pdata->support_spay_gesture)
			continue;
	...
	if (himax_input_register_device(input_dev) == 0)
		...
		E("%s, input_dev register_device failed. %d\n", __func__, ret);
		return ret;
	}
	...
	if (himax_input_register_device(ts->input_dev_proximity) == 0) {
		ret = NO_ERR;
	} else {
		ret = PROX_INPUT_REGISTER_FAIL;
		E("%s, input_dev_proximity register_device failed. %d\n", __func__, ret);
...
static void himax_wake_event_report(void)
{
	...
	if (KEY_EVENT) {
		if ((KEY_EVENT == KEY_BLACK_UI_GESTURE) && private_ts->spay_enabled) {
			I("%s: SPAY event!\n", __func__);
			private_ts->pdata->scrub_id = SPONGE_EVENT_TYPE_SPAY;
			input_report_key(private_ts->input_dev, KEY_BLACK_UI_GESTURE, 1);
			input_sync(private_ts->input_dev);
			input_report_key(private_ts->input_dev, KEY_BLACK_UI_GESTURE, 0);
			input_sync(private_ts->input_dev);
		}  else {
...
#endif
#ifdef HX_TOUCH_PROXIMITY
	if (ts->ear_detect_mode && ts->suspended) {
		if (g_ts_dbg)
			input_info(true, ts->dev,
...
//				HIMAX_LOG_TAG, ts->ear_detect_mode, ts->ear_detect_val);
		if (ts->ear_detect_mode >= 1) {
...
static void himax_print_info(struct himax_ts_data *ts)
{
	...
	input_info(true, ts->dev,
		"tc:%d lp:%x(%d/%d) ta:%d noise:%d lamp:%d // v:HX%02X%02X%02X%02X // #%d %d\n",
		ts->touch_count,
		ts->SMWP_enable, ts->aot_enabled, ts->spay_enabled,
		ts->USB_detect_flag, ts->noise_mode, ts->lamp_noise_mode,
		ic_data->ver_ic_name, ic_data->vendor_cid_maj_ver, ic_data->vendor_panel_ver, ic_data->vendor_cid_min_ver,
		ts->print_info_cnt_open, ts->print_info_cnt_release);
#else
	input_info(true, ts->dev,
		"tc:%d lp:%x(%d/%d) noise:%d lamp:%d // v:HX%02X%02X%02X%02X // #%d %d\n",
		ts->touch_count,
		ts->SMWP_enable, ts->aot_enabled, ts->spay_enabled,
...
int himax_chip_common_init(void)
{
	...
	himax_regulator_init(ts);

#ifdef HX_RST_PIN_FUNC
	ts->rst_gpio = pdata->gpio_reset;
#endif
	ts->cs_gpio = pdata->gpio_cs;
...
{
	if (ts->pdata->name_lcd_rst) {
		ts->regulator_lcd_rst = regulator_get(NULL, ts->pdata->name_lcd_rst);
	...
	if (ts->pdata->name_lcd_vddi) {
		ts->regulator_lcd_vddi = regulator_get(NULL, ts->pdata->name_lcd_vddi);
		if (IS_ERR(ts->regulator_lcd_vddi)) {
			input_err(true, ts->dev, "%s: Failed to get regulator_lcd_vddi regulator.", __func__);
		}
	}

	if (ts->pdata->name_lcd_bl_en) {
		ts->regulator_lcd_bl_en = regulator_get(NULL, ts->pdata->name_lcd_bl_en);
		if (IS_ERR(ts->regulator_lcd_bl_en)) {
			input_err(true, ts->dev, "%s: Failed to get regulator_lcd_bl_en regulator.", __func__);
		}
	}

	if (ts->pdata->name_lcd_vsp) {
		ts->regulator_lcd_vsp = regulator_get(NULL, ts->pdata->name_lcd_vsp);
		if (IS_ERR(ts->regulator_lcd_vsp)) {
			input_err(true, ts->dev, "%s: Failed to get regulator_lcd_vsp regulator.", __func__);
		}
	}

	if (ts->pdata->name_lcd_vsn) {
		ts->regulator_lcd_vsn = regulator_get(NULL, ts->pdata->name_lcd_vsn);
		if (IS_ERR(ts->regulator_lcd_vsn)) {
			input_err(true, ts->dev, "%s: Failed to get regulator_lcd_vsn regulator.", __func__);
		}
	}
...
int himax_ctrl_lcd_regulators(struct himax_ts_data *ts, bool on)
{
	...
	if (on) {
		if (ts->pdata->name_lcd_vddi) {
			ret = regulator_enable(ts->regulator_lcd_vddi);
			if (ret) {
				input_err(true, ts->dev, "%s: Failed to enable regulator_lcd_vddi: %d\n", __func__, ret);
				return ret;
			}
		}
		...
		if (ts->pdata->name_lcd_bl_en) {
			...
			if (ret) {
				input_err(true, ts->dev, "%s: Failed to enable regulator_lcd_bl_en: %d\n", __func__, ret);
				return ret;
			}
		}
		...
		if (ts->pdata->name_lcd_vsp) {
			...
			if (ret) {
				input_err(true, ts->dev, "%s: Failed to enable regulator_lcd_vsp: %d\n", __func__, ret);
				return ret;
			}
		}
		if (ts->pdata->name_lcd_vsn) {
			...
			if (ret) {
				input_err(true, ts->dev, "%s: Failed to enable regulator_lcd_vsn: %d\n", __func__, ret);
				return ret;
			}
		}
	} else {
	...
	input_info(true, ts->dev, "%s: %s: done.\n", __func__, on ? "on" : "off");
...
int himax_chip_common_early_suspend(struct himax_ts_data *ts)
{
	...
	input_info(true, ts->dev, "%s %s: START lp:%d(%d/%d) cover:%d, ear_mode:%d, ear_val:%d, suspend_mode:%d\n",
			HIMAX_LOG_TAG, __func__, ts->SMWP_enable, ts->aot_enabled, ts->spay_enabled, ts->cover_closed,
			ts->ear_detect_mode, ts->ear_detect_val, atomic_read(&ts->suspend_mode));
	...
	if (ts->SMWP_enable || ts->ear_detect_mode) {
...
int himax_chip_common_late_resume(struct himax_ts_data *ts)
{
	...
	input_info(true, ts->dev, "%s %s: START lp:%d(%d/%d) cover:%d, ear_mode:%d, ear_val:%d, suspend_mode:%d\n",
			HIMAX_LOG_TAG, __func__, ts->SMWP_enable, ts->aot_enabled, ts->spay_enabled,
			ts->cover_closed, ts->ear_detect_mode, ts->ear_detect_val, atomic_read(&ts->suspend_mode));
