...
static void himax_osr_ctrl(bool enable)
{
	...
	if (strcmp(HX_83108A_SERIES_PWON, private_ts->chip_name) == 0
		...
		if (strcmp(HX_83122A_SERIES_PWON, private_ts->chip_name) == 0) {
			tmp_addr[3] = pfw_op->addr_osr_ctrl[3];
			tmp_addr[2] = pfw_op->addr_osr_ctrl[2];
			tmp_addr[1] = pfw_op->addr_osr_ctrl[1];
			tmp_addr[0] = pfw_op->addr_osr_ctrl[0];
...
static int himax_fw_update_from_storage(bool signing, const char *file_path)
{
	...
	fsize = firmware->size;
	input_err(true, private_ts->dev, "%s %s: fsize:%ld\n", HIMAX_LOG_TAG, __func__, fsize);
	if (signing) {
		spu_fw_size = fsize;
	...
	case FW_SIZE_255k:
		...
		break;
	default:
		E("%s %s: does not support fw size %ld\n",
				HIMAX_LOG_TAG, __func__, fsize);
	}
	...
	input_err(true, private_ts->dev, "%s: ret %d\n", __func__, ret);
...
static void aot_enable(void *dev_data)
{
	...
	ts->SMWP_enable = ts->aot_enabled | ts->spay_enabled;
...
static void spay_enable(void *dev_data)
{
	...
	if (!ts->pdata->support_spay_gesture) {
		E("%s: Not support SPAY!\n", __func__);
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		goto out;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	input_info(true, ts->dev, "%s: %d%s%s%s\n",
			__func__, feature,
		...
		struct device_attribute *attr, char *buf)
{
	...
	snprintf(buff, sizeof(buff), "%d %d %d", ts->pdata->scrub_id, 0, 0);
	input_info(true, ts->dev, "%s: scrub_id: %s\n", __func__, buff);
