...
static void himax_hopping_ctrl(bool enable)
{
	uint8_t tmp_addr[4] = {0};
	uint8_t tmp_data[4] = {0};
	uint8_t w_byte = 0;
	...
	do {
		...
		back_byte = tmp_data[0];
		if (w_byte != back_byte)
			input_info(true, &private_ts->client->dev, "%s %s: Write hopping_ctrl failed, w_byte = %02X, back_byte = %02X\n",
				HIMAX_LOG_TAG, __func__, w_byte, back_byte);
		else {
			input_info(true, &private_ts->client->dev, "%s %s: Write hopping_ctrl correctly, w_byte = %02X, back_byte = %02X\n",
				HIMAX_LOG_TAG, __func__, w_byte, back_byte);
			break;
		}
	} while (--retry > 0);
...
static int hx_get_one_raw_diff(int32_t	*RAW, uint32_t datalen, int data_type)
{
	int ret = 0;
	uint8_t tmp_addr[4] = {0};
	...
	input_info(true, &private_ts->client->dev, "%s Need Change Mode ,target=%s\n",
		HIMAX_LOG_TAG, g_himax_inspection_mode[HIMAX_INSPECTION_RAWDATA]);
	g_core_fp.fp_sense_off(true);
	hx_turn_on_mp_func(1);
#ifndef HX_ZERO_FLASH
	if (g_core_fp.fp_reload_disable != NULL)
	...
	himax_set_N_frame(2, HIMAX_INSPECTION_RAWDATA);

	himax_hopping_ctrl(0);
	...
	do {
		g_core_fp.fp_register_write(tmp_addr, DATA_LEN_4, tmp_write, false);
		msleep(10);
		g_core_fp.fp_register_read(tmp_addr, DATA_LEN_4, tmp_read, false);
		input_err(true, &private_ts->client->dev, "%s %s:R%02X%02X%02X%02XH= 0x%02X%02X%02X%02X\n",
					HIMAX_LOG_TAG, __func__,
					tmp_addr[3], tmp_addr[2], tmp_addr[1], tmp_addr[0],
					tmp_read[3], tmp_read[2], tmp_read[1], tmp_read[0]);
		if (tmp_read[3] == tmp_write[3] && tmp_read[2] == tmp_write[2]
			&& tmp_read[1] == tmp_write[1] && tmp_read[0] == tmp_write[0]) {
				chk_write = 1;
		} else {
			retry--;
	...
	} while (retry > 0 && chk_write == 0);

	ret = himax_wait_sorting_mode(HIMAX_INSPECTION_RAWDATA);
	if (ret) {
		input_err(true, &private_ts->client->dev, "%s %s: himax_wait_sorting_mode FAIL\n",
			HIMAX_LOG_TAG, __func__);
		goto END_FUNC;
	...
	ret = himax_get_rawdata(RAW, datalen, HIMAX_INSPECTION_RAWDATA);
	if (ret) {
		input_err(true, &private_ts->client->dev, "%s %s: himax_get_rawdata FAIL\n",
			HIMAX_LOG_TAG, __func__);
		goto END_FUNC;
...
static void fw_update(void *dev_data)
{
	...
	case UMS:
		...
		upgrade_fw = kzalloc(fsize, GFP_KERNEL);
		/* read the latest firmware binary file */
		ret = vfs_read(filp, (char __user *)upgrade_fw,
				fsize, &filp->f_pos);
		if (ret < 0) {
			input_err(true, &data->client->dev,
		...
		spu_ret = spu_firmware_signature_verify("TSP", upgrade_fw, fsize);
		if (spu_ret != ori_size) {
			input_err(true, &data->client->dev, "%s: signature verify failed, spu_ret:%ld, ori_size:%ld\n",
		...
		fsize = ori_size;
		input_info(true, &data->client->dev, "%s: signature verify success, spu_ret:%ld, ori_size:%ld\n",
			__func__, spu_ret, ori_size);
#endif
...
static void get_diff_set_raw(void *dev_data)
{
	uint32_t *RAW1st = NULL;
	uint32_t *RAW2nd = NULL;
	uint32_t *RAWdiff = NULL;
	int datalen = 0;
	int val = 0;
	int i = 0;
	char buf[LEN_RSLT] = { 0 };
	...
	struct himax_ts_data *data =
		container_of(sec, struct himax_ts_data, sec);
	int count = 0;
	int count_criteria =  (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) / 4;
	int val_criteria = 300;

	input_info(true, &private_ts->client->dev, "%s,start \n", __func__);
	...
	datalen =
		(ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) + ic_data->HX_TX_NUM +
		ic_data->HX_RX_NUM;
	RAW1st = kzalloc(sizeof(uint32_t) * datalen, GFP_KERNEL);
	if (!RAW1st) {
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		snprintf(buf, sizeof(buf), "%s:failed to allocate memory", __func__);
		sec_cmd_set_cmd_result(sec, buf, strnlen(buf, sizeof(buf)));
	...
	RAW2nd = kzalloc(sizeof(uint32_t) * datalen, GFP_KERNEL);
	if (!RAW2nd) {
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		snprintf(buf, sizeof(buf), "%s:failed to allocate memory", __func__);
		sec_cmd_set_cmd_result(sec, buf, strnlen(buf, sizeof(buf)));
	...
	RAWdiff = kzalloc(sizeof(uint32_t) * datalen, GFP_KERNEL);
	if (!RAWdiff) {
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		snprintf(buf, sizeof(buf), "%s:failed to allocate memory", __func__);
		sec_cmd_set_cmd_result(sec, buf, strnlen(buf, sizeof(buf)));
	...
	himax_int_enable(0);
	val = hx_get_one_raw_diff(RAW1st, datalen, 1);
	if (val < 0) {
		input_err(true, &data->client->dev,
				"%s %s: get rawdata fail!\n", HIMAX_LOG_TAG,
				__func__);
		snprintf(buf, sizeof(buf), "%s:get fail", __func__);
		goto err_out_irq_enable;
	}
	hx_print_frame(data, RAW1st);

	val = hx_get_one_raw_diff(RAW2nd, datalen, 2);
	if (val < 0) {
		input_err(true, &data->client->dev,
				"%s %s: get rawdata fail!\n", HIMAX_LOG_TAG,
				__func__);
	...
	hx_print_frame(data, RAW2nd);

	for (i = 0; i < sz_mutual; i++) { // without self
		RAWdiff[i] = HX_ABS(((int)(RAW1st[i] - RAW2nd[i])));
		if (RAWdiff[i] <= val_criteria)
			count++;
	}
	hx_print_frame(data, RAWdiff);
	I("%s: Count = %d, Criteria=%d\n", __func__, count, count_criteria);

	if (count >= count_criteria) {
		g_sec_raw_buff->f_ready_rawdata = HX_RAWDATA_NOT_READY;
		input_err(true, &data->client->dev, "%s %s:  fail!\n",
				HIMAX_LOG_TAG, __func__);
		goto err_out_irq_enable;
	} else {
		g_sec_raw_buff->f_ready_rawdata = HX_RAWDATA_READY;
		input_info(true, &data->client->dev, "%s %s:  ok!\n",
				HIMAX_LOG_TAG, __func__);
	}

	snprintf(buf, sizeof(buf), "%d", TEST_RESULT_PASS);
	sec_cmd_set_cmd_result(sec, buf, strnlen(buf, sizeof(buf)));
	sec->cmd_state = SEC_CMD_STATUS_OK;

	if (sec->cmd_all_factory_state == SEC_CMD_STATUS_RUNNING)
		sec_cmd_set_cmd_result_all(sec, buf, strnlen(buf, sizeof(buf)), "DIFFRAW");

	snprintf(result, sizeof(result), "RESULT=PASS");
	sec_cmd_send_event_to_user(&data->sec, test, result);
...
static void hx_dd_en_read(int en)
{
	uint8_t addr_osc[] = {0xA8, 0x80, 0x08, 0x90};
	uint8_t addr_osc_pw[] = {0xE0, 0x80, 0x08, 0x90};
	uint8_t addr_pwd1[] = {0x00, 0x90, 0x0B, 0x30};

	uint8_t en_val_osc[] = {0x01, 0x00, 0x00, 0x00};
	uint8_t en_pwd_val_osc[] = {0xA5, 0x00, 0x00, 0x00};
	uint8_t en_pwd1[] = {0x00, 0x83, 0x10, 0x2d};
...
static void change_vdd(int vdd_idx)
{
	uint32_t vdd_val[] = {0x00000100, 0x00000200, 0x00000300, 0x00000400, 0x00000500, 0x00000600, 0x00000700};
	uint8_t tmp_addr[] = {0x00, 0xC0, 0x0B, 0x30};
	uint8_t tmp_data[4] = {0};
	uint8_t tmp_read[4] = {0};
	...
	tmp_data[3] = (uint8_t)((vdd_val[vdd_idx] >> 24) & 0x00FF);
	tmp_data[2] = (uint8_t)((vdd_val[vdd_idx]  >> 16) & 0x00FF);
	tmp_data[1] = (uint8_t)((vdd_val[vdd_idx]  >> 8) & 0x00FF);
	tmp_data[0] = (uint8_t)((vdd_val[vdd_idx]) & 0x00FF);
	g_core_fp.fp_register_write(tmp_addr, DATA_LEN_4, tmp_data, false);
	usleep_range(10000, 10001);
	tmp_addr[0] = 0x01;
	g_core_fp.fp_register_read(tmp_addr, DATA_LEN_4, tmp_read, false);
	I("%s: Now value=0x%02X%02X%02X%02X!\n",
		__func__,
		tmp_read[3], tmp_read[2], tmp_read[1], tmp_read[0]);
	hx_dd_en_read(0);
}
...
static int ram_test(int type)
{
	...
	uint32_t crc_sw = 0;
	int times = 0;
	uint32_t tmp_addr_32 = 0x00;
	int write_sz = 128;
	int one_write_sz = 128;
	input_info(true, &private_ts->client->dev, "%s,start \n", __func__);
	if (type == 1) {
	...
	tmp_addr_32 = addr;
	times = size / one_write_sz;
	times = size % one_write_sz > 1 ? times + 1 : times;

	for (i = 0; i < times; i++) {
		if (i == (times - 1) &&
			(size % one_write_sz > 0)) {
			write_sz = size % one_write_sz;
		} else {
			write_sz = one_write_sz;
		}
		tmp_addr[3] = (uint8_t)((tmp_addr_32 >> 24) & 0x00FF);
		tmp_addr[2] = (uint8_t)((tmp_addr_32 >> 16) & 0x00FF);
		tmp_addr[1] = (uint8_t)((tmp_addr_32 >> 8) & 0x00FF);
		tmp_addr[0] = (uint8_t)((tmp_addr_32) & 0x00FF);
		tmp_addr_32 += one_write_sz;
		g_core_fp.fp_register_write(tmp_addr, write_sz, write_buf, 0);
	}
...
END:
	input_info(true, &private_ts->client->dev, "%s,end \n", __func__);
	return result;
}

static void check_sram_test_done(void)
{
	uint8_t data[12];
...
static void run_sram_test(void *dev_data)
{
	...
	input_info(true, ts->dev, "%s start dsram\n", __func__);
	...
	input_info(true, ts->dev, "%s start isram\n", __func__);
...
struct sec_cmd sec_cmds[] = {
	...
	{SEC_CMD("get_diff_set_raw", get_diff_set_raw),},
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (private_ts->pdata->enable_sysinput_enabled)
		feature |= INPUT_FEATURE_ENABLE_SYSINPUT_ENABLED;

	input_info(true, &private_ts->client->dev, "%s %s: %d%s\n",
		HIMAX_LOG_TAG, __func__, feature, feature & INPUT_FEATURE_ENABLE_SYSINPUT_ENABLED ? " SE" : "");
...
{
	input_info(true, &private_ts->client->dev, "%s %s : %d\n",
			HIMAX_LOG_TAG, __func__, private_ts->suspended);

	return snprintf(buf, SEC_CMD_BUF_SIZE, "%d", private_ts->suspended);
}
		...
		const char *buf, size_t count)
{
	...
	if (ret != 2) {
		input_err(true, &private_ts->client->dev,
				"%s %s failed read params [%d]\n", HIMAX_LOG_TAG, __func__, ret);
		return -EINVAL;
	}

	input_info(true, &private_ts->client->dev, "%s %s : %d %d\n",
				HIMAX_LOG_TAG, __func__, buff[0], buff[1]);
