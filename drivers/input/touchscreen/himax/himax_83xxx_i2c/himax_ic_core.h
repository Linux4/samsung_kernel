...
#define fw_addr_hsen_sts					0x10007ff4
...
/*void (*fp_flash_write_burst_lenth)(uint8_t *reg_byte, uint8_t *write_data, uint32_t length);*/
	...
	void (*fp_set_reload_cmd) (uint8_t * write_data, int idx,
	...
	void (*fp_flash_programming_sz) (uint32_t start_addr,
	...
	void (*fp_diag_parse_raw_data) (struct himax_report_data *
					hx_touch_data, int mul_num,
					int self_num, uint8_t diag_cmd,
					int32_t * mutual_data,
					int32_t * self_data);
	void (*fp_ic_reset) (uint8_t loadconfig, uint8_t int_off);
	int (*fp_ic_esd_recovery) (int hx_esd_event, int hx_zero_event,
				int length);
	...
	void (*fp_firmware_read_0f) (const struct firmware * fw_entry,
				int type);
