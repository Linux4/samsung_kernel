...
int g_i_PANEL_VER = 0;		/*VER for GUEST */
#endif
#ifdef HX_ZERO_FLASH
int g_f_0f_updat = 0;
#endif
...
static int himax_auto_update_check(void)
{
	...
	if (g_core_fp.fp_fw_ver_bin() == 0) {
		input_info(true, &private_ts->client->dev,
				"%s %s: fw_ver:0x%x,config_ver:0x%x,maj_ver:0x%x,min_ver:0x%x,vendor_panel_ver:0x%x\n",
					HIMAX_LOG_TAG, __func__, ic_data->vendor_fw_ver, ic_data->vendor_config_ver,
					ic_data->vendor_cid_maj_ver, ic_data->vendor_cid_min_ver, ic_data->vendor_panel_ver);
		input_info(true, &private_ts->client->dev,
				"%s %s: g_i_FW_VER:0x%x,g_i_CFG_VER:0x%x,g_i_CID_MAJ:0x%x,g_i_CID_MIN:0x%x,g_i_PANEL_VER:0x%x\n",
				HIMAX_LOG_TAG, __func__, g_i_FW_VER, g_i_CFG_VER, g_i_CID_MAJ, g_i_CID_MIN, g_i_PANEL_VER);

		if (private_ts->pdata->separated_factory_fw && (ic_data->vendor_cid_maj_ver != g_i_CID_MAJ)) {
			input_info(true, &private_ts->client->dev,
				"%s %s: Need to update!(different g_i_CID_MAJ)\n", HIMAX_LOG_TAG, __func__);
			return NO_ERR;
		}

		if((ic_data->vendor_cid_maj_ver == g_i_CID_MAJ) && (ic_data->vendor_panel_ver != g_i_PANEL_VER)){
			input_info(true, &private_ts->client->dev,
				"%s %s: Need to g_i_PANEL_VER update!(different g_i_PANEL_VER)\n", HIMAX_LOG_TAG, __func__);
			return NO_ERR;
		}
...
void himax_cable_detect_func(bool force_renew)
{
	...
	connect_status = private_ts->USB_detect_flag;	/* upmu_is_chr_det(); */
	ts = private_ts;
...
int himax_chip_common_suspend(struct himax_ts_data *ts)
{
	...
	if (ts->suspended) {
		input_info(true, &ts->client->dev,
				"%s %s: Already suspended. Skipped.(shutdown:%d) \n",
				HIMAX_LOG_TAG, __func__, ts->shutdown);
		return 0;
	} else {
		himax_int_enable(0);
		ts->suspended = true;
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
	...
	if (ts->suspended == false || ts->shutdown) {
		input_info(true, &ts->client->dev,
				"%s %s: It had entered resume, skip this step(%d, %d) \n",
				HIMAX_LOG_TAG, __func__, ts->suspended, ts->shutdown);
		return 0;
	} else {
		ts->suspended = false;
	}
