...
#if defined(CONFIG_MTK_ENABLE_LCDID_CHECK_FOR_TOUCH)
	if (of_property_read_u32(dt, "himax,lcd_id_count", &data) == 0)
		ts->ssdev_lcd_id_count = data;
	pr_info("%s : lcd_id_count(0x%x)\n", __func__, data);

	rc = of_property_read_u32_array(dt, "himax,lcd_id",
			tmp_lcd_id, ts->ssdev_lcd_id_count);
	if (rc && (rc != -EINVAL)) {
		D(" %s:Fail to read lcd_id %d\n", __func__, rc);
		return rc;
	}

	for (i = 0; i < (int)(ts->ssdev_lcd_id_count); i++) {
		ts->ssdev_lcd_id[i] = tmp_lcd_id[i];
		pr_info("%s : lcd_id[%d] = 0x%x\n", __func__, i, ts->ssdev_lcd_id[i]);
	}
#endif
...
static int _lcm_lcd_id_dt(void)
{
	...
	pr_info("[TOUCH][lcm_hx83108a] lcd_id 0x%x\n", _lcd_id);
	return _lcd_id;
}
#endif
...
int himax_chip_common_probe(struct spi_device *spi)
{
	...
	for (i = 0; i < (int)(ts->ssdev_lcd_id_count); i++) {
		if (lcd_id == ts->ssdev_lcd_id[i])
			break;
	}

	pr_info("//// %s: i= %d\n", __func__, i);
	if (i == (int)ts->ssdev_lcd_id_count) {
		ret = -1;
