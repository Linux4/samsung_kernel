...
static int cyttsp5_hw_power(struct cyttsp5_core_platform_data *pdata, int on)
{
	...
	if (gpio_is_valid(pdata->pwr_3p3)) {
		...
		if (ret) {
			pr_err("%s: unable to set %d for ic en %d\n",
	...
	if (!reg_1p8) {
		...
		if (IS_ERR_OR_NULL(reg_1p8)) {
			pr_err("%s: could not get vdd_io, rc = %ld,%ld\n",
				__func__, PTR_ERR(reg_1p8), IS_ERR(reg_1p8));
			reg_1p8 = NULL;
...
out:
	...
	if (reg_1p8)
		pr_cont(" 1.8v = %d", regulator_is_enabled(reg_1p8));
	pr_cont("\n");
