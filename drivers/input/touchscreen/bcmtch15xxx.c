...
/* -- driver version -- */
#define	BCMTCH_DRIVER_VERSION		"1.4.3.07"
...
#define BCMTCH_STATE_RAW_FRAME	1
...
#if BCMTCH_STATE_RAW_FRAME
#pragma pack(push, 1)
struct tofe_afe_cfg {
	uint32_t _pad32_16_0[16];
	uint8_t afe_rx_preamp_q;
	uint8_t afe_rx_preamp_gain;
	uint8_t _pad8_2_0[2];
};
#pragma pack(pop)

#pragma pack(push, 1)
struct tofe_sem_dsp_cfg {
	int32_t _pad32_13_0[13];
	uint8_t _pad8_2_0[2];
	uint8_t sd_shift;
	uint8_t _pad8_1_0;
};
...
struct tofe_tch_cfg {
	...
	uint8_t cols;
	uint8_t _pad8_3_0[3];
	uint8_t asc_magnitude_truncation;
	uint16_t _pad16_5_0[5];
	struct tofe_afe_cfg afe;
	struct tofe_sem_dsp_cfg dsp_cfg;
};
#pragma pack(pop)
#endif /* BCMTCH_STATE_RAW_FRAME */
...
#if BCMTCH_STATE_RAW_FRAME
/* ------------------------------------ */
/* - BCM Touch Controller Record Info - */
/* ------------------------------------ */

#define BCMTCH_RECORD_SIZE_UNIT 32
#define BCMTCH_MAX_FRAME_SIZE (48*32)
#define BCMTCH_RECORD_DROP_BITS	2
#define BCMTCH_MAX_RAW_FRAME_NUM (1 << 10)

#pragma pack(push, 1)
struct bcmtch_record_data {
	uint32_t timestamp;
	uint32_t ftx;
	uint8_t size;
	uint8_t sample_counter;
	uint8_t data_in[BCMTCH_MAX_FRAME_SIZE*2];
};
#pragma pack(pop)
...
#if BCMTCH_STATE_RAW_FRAME
#define BCMTCH_BF_STATE_RECORD_MODE  0x00000400
#endif
...
static const struct firmware_load_info_t BCMTCH_BINARIES[] = {
	...
	{BCMTCHWC, BCMTCHWC, "bcm_tsp/bcmtchfw_bin.fw", 0, BCMTCH_FIRMWARE_FLAGS_COMBI},
...
#define RECORD_SIZE_UNIT 32
#define TOFE_HOST_SLOT_TOTAL_SIZE (15*12)

#define TOFE_HOST_RSP_BUF_SIZE \
			((BCMTCH_MAX_TOUCH \
			* TOFE_HOST_TOUCH_SLOT_BIG_SIZE + 1) \
			+ (BCMTCH_MAX_FRAME_SIZE + 10))
#else /* BCMTCH_STATE_RAW_FRAME */
...
#if BCMTCH_STATE_RAW_FRAME
/*- Recording scan command/response format. -*/
union bcmtch_state_record_param_1 {
	struct {
		uint8_t bit_drop_num:4;
		uint8_t type:3;			/* 0 - heatmap, 1- deltas. */
		uint8_t disable_touch:1;
	};
	uint8_t reg;
};

struct bcmtch_state_record_parameters {
	union bcmtch_state_cmd_start_scan	p0;
	union bcmtch_state_record_param_1	p1;
};

union bcmtch_state_record_status {
	struct {
		uint8_t data_size:7;
		uint8_t done:1;
	};
...
#if BCMTCH_STATE_RAW_FRAME
	struct bcmtch_state_record_parameters rec_params;
	union bcmtch_state_record_status rec_status;
#endif
#if BCMTCH_STATE_RAW_FRAME
	bool bcmtch_raw_frame_pending;
	int32_t sensor_rows;
	int32_t sensor_cols;
	int32_t sensor_total;
	int32_t afe_rx_preamp_q;
	int32_t afe_rx_preamp_gain;
	int32_t asc_magnitude_truncation;
	int32_t sd_shift;
	uint16_t data_raw_frame[BCMTCH_MAX_FRAME_SIZE];
	uint16_t data_capacitances[BCMTCH_MAX_FRAME_SIZE];
	uint32_t data_raw_frame_avg[BCMTCH_MAX_FRAME_SIZE];
	uint32_t data_cap_frame_avg[BCMTCH_MAX_FRAME_SIZE];
#endif
...
static int32_t bcmtch_state_stop_scan(
	struct bcmtch_data *);
#if BCMTCH_STATE_RAW_FRAME
int32_t bcmtch_dev_raw_frame_get(
	struct bcmtch_data *, uint16_t *, int16_t *, int16_t *);
#endif
#if BCMTCH_STATE_RAW_FRAME
static void	bcmtch_dev_process_state_record(
	struct bcmtch_data *);
static int32_t bcmtch_state_record_scan(
	struct bcmtch_data *);
#endif
...
struct tsp_cmd tsp_cmds_bcm[] = {
	{TSP_CMD("fw_update", fw_update),},
	{TSP_CMD("get_fw_ver_bin", get_fw_ver_bin),},
	{TSP_CMD("get_fw_ver_ic", get_fw_ver_bin),},
...
/* -- Firmware version ABI -- */
static ssize_t bcmtch_os_abi_fw_version(
					struct device *dev,
					struct device_attribute *devattr,
					char *buf)
{
...
DEVICE_ATTR(fw_version, 0444, bcmtch_os_abi_fw_version, NULL);
...
#if BCMTCH_STATE_RAW_FRAME
/* -- Raw frame ABI -- */
static ssize_t bcmtch_os_abi_raw_frame(
					struct device *dev,
					struct device_attribute *devattr,
					char *buf)
{
	...
	ret_val = bcmtch_dev_raw_frame_get(
				bcmtch_data_ptr,
				&frame_size,
				bcmtch_data_ptr->data_raw_frame,
				bcmtch_data_ptr->data_capacitances);

	if (!ret_val && ((frame_size << 1) + frame_size) < PAGE_SIZE) {
		for (i = 0; i < frame_size; i++) {
			count +=
				snprintf(buf + count, PAGE_SIZE - count,
					"%04X ",
					bcmtch_data_ptr->data_raw_frame[i]);
		}
		count += snprintf(buf + count,
				PAGE_SIZE - count, "\n");
	}
	...
	if (!ret_val && ((frame_size << 1) + frame_size) < PAGE_SIZE) {
		for (i = 0; i < frame_size; i++) {
			count +=
				snprintf(buf + count, PAGE_SIZE - count,
					"%04X ",
					bcmtch_data_ptr->data_capacitances[i]);
		}
		count += snprintf(buf + count,
				PAGE_SIZE - count, "\n");
	}
...
DEVICE_ATTR(raw_frame, 0444, bcmtch_os_abi_raw_frame, NULL);
#endif /* BCMTCH_STATE_RAW_FRAME */
...
/* ----------------------------------------- */
/*  Added "enabled" sysfs for suspend/resume   */
/* ----------------------------------------- */

static ssize_t bcmtsp_enabled_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
	struct bcmtch_data *bcmtch_data_ptr = dev_get_drvdata(dev);
...
static void raw2capacitance(
		struct bcmtch_data *bcmtch_data_ptr,
		uint16_t *raw,
		uint16_t *capacitance,
		unsigned data_size)
{
	int cap_scale_factor;
	#define CAP_SCALE 0x100

	int preamp_q = bcmtch_data_ptr->afe_rx_preamp_q;
	int preamp_gain = bcmtch_data_ptr->afe_rx_preamp_gain>>2;
	int magn_trunc = bcmtch_data_ptr->asc_magnitude_truncation;
	int sd_shift = bcmtch_data_ptr->sd_shift;
	unsigned int i;

	/* Numbers derived from Napa AMS */
	#define GAIN_SCALE_FACTOR 100
	const int gain_scale[4] = {2312, 1892, 1576, 1366};

	cap_scale_factor =
			(int)(((CAP_SCALE << (2-magn_trunc)) >> sd_shift) *
			(1<<preamp_q) * (1 << (preamp_gain>>2)) *
			GAIN_SCALE_FACTOR /
			gain_scale[preamp_gain % 4]);

	for (i = 0; i < data_size; i++) {
		int v = raw[i] * cap_scale_factor / CAP_SCALE;
		capacitance[i] = (uint16_t)v;
	}
}

static void bcmtch_os_cli_raw_frame_get(
					struct bcmtch_data *bcmtch_data_ptr,
					uint32_t frame_num)
{
	int32_t ret_val = 0;
	...
	if (frame_num > BCMTCH_MAX_RAW_FRAME_NUM) {
		BCMTCH_ERR(
			"RF: raw fame number %d exceed the limit %d.\n",
			frame_num,
			BCMTCH_MAX_RAW_FRAME_NUM);
		return;
	}

	/* Init frame array */
	memset(bcmtch_data_ptr->data_raw_frame_avg,
			0,
			BCMTCH_MAX_FRAME_SIZE * sizeof(int32_t));
	memset(bcmtch_data_ptr->data_cap_frame_avg,
			0,
			BCMTCH_MAX_FRAME_SIZE * sizeof(int32_t));

	/* Request multiple data frames and accumulate the results. */
	for (i = 0, j = 0; i < frame_num && j < (frame_num << 1); j++) {
		...
		ret_val = bcmtch_dev_raw_frame_get(
					...
					bcmtch_data_ptr->data_capacitances);
		if (!ret_val) {
			BCMTCH_INFO("RF: raw frame %d\n", i);
			for (k = 0; k < frame_size; k++) {
				bcmtch_data_ptr->data_raw_frame_avg[k] +=
					bcmtch_data_ptr->data_raw_frame[k];
				bcmtch_data_ptr->data_cap_frame_avg[k] +=
					bcmtch_data_ptr->data_capacitances[k];
			}
			i++;
		}
	}
	...
	if (i < frame_num) {
		BCMTCH_ERR("raw frames query timeout!\n");
		return;
	}

	/* calculate averages */
	for (i = 0; i < frame_size; i++) {
		bcmtch_data_ptr->data_raw_frame_avg[i] /= frame_num;
		bcmtch_data_ptr->data_cap_frame_avg[i] /= frame_num;
	}

	/* Dump the average of the raw data frame. */
	BCMTCH_INFO(
		"RF: heatmap size=%d, Raw Frame Average Data\n",
		frame_size);
	for (i = 0; i < frame_size; i += 8) {
		for (j = 0; j < 8; j++) {
			dump[j] = (i + j) < frame_size ?
				(bcmtch_data_ptr->
						data_raw_frame_avg[i + j]) : 0;
		}
		BCMTCH_INFO(
			"RF: 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x\n",
			dump[0], dump[1], dump[2], dump[3],
			dump[4], dump[5], dump[6], dump[7]);
	}

	/* Dump the capacitance frame average data. */
	BCMTCH_INFO(
		"RF: heatmap size=%d, Capacitance Frame Average Data\n",
		frame_size);
	BCMTCH_INFO(
		"RF: rx_preamp_q=%d rx_preamp_gain=%d mag_trunc=%d sd_shift=%d\n",
		bcmtch_data_ptr->afe_rx_preamp_q,
		bcmtch_data_ptr->afe_rx_preamp_gain,
		bcmtch_data_ptr->asc_magnitude_truncation,
		bcmtch_data_ptr->sd_shift);

	for (i = 0; i < frame_size; i += 8) {
		for (j = 0; j < 8; j++) {
			dump[j] = (i + j) < frame_size ?
				(bcmtch_data_ptr->
						data_cap_frame_avg[i + j]) : 0;
		}
		BCMTCH_INFO(
			"RF: 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x\n",
			dump[0], dump[1], dump[2], dump[3],
					...
					size_t count)
{
...
#if BCMTCH_STATE_RAW_FRAME
	else if ((count > strlen("get raw frame ")) &&
			sscanf(buf, "get raw frame %d",
				&in_value_count) == 1) {
		bcmtch_os_cli_raw_frame_get(bcmtch_data_ptr,
			in_value_count);
	}
#endif
	...
	} else if (strncmp(buf, "fw version", strlen("fw version")) == 0) {
		BCMTCH_INFO("F/W version= %s%02x%04x\n",
			"BR", 0x5,
			bcmtch_data_ptr->fw_signature.cust_release_num);
	} else {
		BCMTCH_INFO("Usage:\n");
...
#if BCMTCH_STATE_RAW_FRAME
		BCMTCH_INFO("get raw frame <# of frame>\n");
#endif
#if defined(CONFIG_PM)
		BCMTCH_INFO("suspend\n");
		BCMTCH_INFO("resume\n");
...
/* ------------------------------------------------------ */
/* - BCM Touch Controller Raw Frame Data Implementation - */
/* ------------------------------------------------------ */

static inline
void bcmtch_frame_decode(
			unsigned char *data_in,
			unsigned short *data_last,
			int data_last_size,
			int counter,
			int scale)
{
	...
	if (counter == 0) {
		for (i = 0; i < data_last_size; i++)
			data_last[i] = ((unsigned short *)data_in)[i];
	} else {
		for (i = 0; i < data_last_size; i++) {
			int d = data_in[j>>1];
			if (j & 1)
...
int32_t bcmtch_dev_raw_frame_get(
		struct bcmtch_data *bcmtch_data_ptr,
		uint16_t *frame_size,
		int16_t *out_raw_frame,
		int16_t *out_capacitance)
{
	int i;
	int32_t ret_val = 0;
	struct bcmtch_record_data *rec_info = NULL;
	uint8_t *resp_buff =
				&bcmtch_data_ptr->bcmtch_state_resp_buffer[1];
	uint8_t worker_index;

	if (!out_capacitance) {
		BCMTCH_ERR("RF: NULL input array.\n");
		return -EINVAL;
	}

	if (bcmtch_data_ptr->work_process_index < BCMTCH_WP_STATE) {
		BCMTCH_ERR("RF: Not state protocol mode.\n");
		return -EINVAL;
	}
	...
	if (ret_val) {
		BCMTCH_ERR("%s: stop scan failed! (%d)\n",
			__func__,
			ret_val);
		return -EIO;
	}
	...
	if (!bcmtch_data_ptr->rec_params.p0.reg) {
		bcmtch_data_ptr->rec_params.p0.reg =
			bcmtch_data_ptr->scan_data.cmd.reg;
		bcmtch_data_ptr->rec_params.p1.bit_drop_num =
				BCMTCH_RECORD_DROP_BITS + 1;
		bcmtch_data_ptr->rec_params.p1.type = 0;
		bcmtch_data_ptr->rec_params.p1.disable_touch = 0;
	}

	/* Set up raw frame worker process */
	worker_index = bcmtch_data_ptr->work_process_index;
	bcmtch_data_ptr->work_process_index = BCMTCH_WP_STATE_RECORD;
	bcmtch_data_ptr->bcmtch_raw_frame_pending = true;

	/* Single scan */
	ret_val = bcmtch_state_record_scan(bcmtch_data_ptr);
	if (ret_val) {
		BCMTCH_ERR("%s: single scan failed! (%d)\n",
			__func__,
			ret_val);
		bcmtch_data_ptr->bcmtch_raw_frame_pending = false;
		ret_val = -EIO;
		goto bcmtch_raw_frame_exit;
	}

	mutex_unlock(&bcmtch_data_ptr->mutex_work);
	for (i = 0; bcmtch_data_ptr->bcmtch_raw_frame_pending
						&& (i < 25); i++) {
	...
	bcmtch_data_ptr->work_process_index = worker_index;

	if (bcmtch_data_ptr->bcmtch_raw_frame_pending) {
		BCMTCH_ERR("%s: heatmap reading timeout!\n",
				__func__);
		bcmtch_data_ptr->bcmtch_raw_frame_pending = false;
	...
	/* decode data */
	rec_info = (struct bcmtch_record_data *)resp_buff;
	BCMTCH_DBG(
		BCMTCH_DF_RF,
		"RF:%s: ts=0x%08x ftx=0x%08x sc=%d size=%d\n",
		__func__,
		rec_info->timestamp,
		rec_info->ftx,
		rec_info->sample_counter,
		rec_info->size);

	bcmtch_frame_decode(
		rec_info->data_in,
		bcmtch_data_ptr->data_raw_frame,
		bcmtch_data_ptr->sensor_total,
		rec_info->sample_counter,
		BCMTCH_RECORD_DROP_BITS);

	/* Converting to capacitance */
	raw2capacitance(
		bcmtch_data_ptr,
		bcmtch_data_ptr->data_raw_frame,
		out_capacitance,
	...
	*frame_size = bcmtch_data_ptr->sensor_total;

bcmtch_raw_frame_exit:
	/* start scan */
			...
			uint8_t slots)
{
	...
	ret_val =
		bcmtch_com_read_dma(
			bcmtch_data_ptr,
#if BCMTCH_STATE_RAW_FRAME
			/* in recording case, no status byte to read */
			data_size +
			((bcmtch_boot_flag & BCMTCH_BF_STATE_RECORD_MODE) ?
				0 : 1),
#else
			data_size + 1,
#endif
...
#if BCMTCH_STATE_RAW_FRAME
	struct tofe_tch_cfg *p_tch_cfg = NULL;
#endif
		...
		case BCMTCH_FIRMWARE_FLAGS_POST_BOOT_CONFIGS:
			...
			if (!p_cfg[TOFE_TOC_INDEX_CHANNEL]) {
				...
				p_tch_cfg =
					(struct tofe_tch_cfg *)
					((uint32_t)mem_data
						+ p_cfg[TOFE_TOC_INDEX_TCH]
						- BCMTCH_ADDR_TOC_BASE);
				if (p_tch_cfg) {
					bcmtch_data_ptr->sensor_rows =
						p_tch_cfg->rows;
					bcmtch_data_ptr->sensor_cols =
						p_tch_cfg->cols;
					bcmtch_data_ptr->sensor_total =
						p_tch_cfg->rows *
						p_tch_cfg->cols;
					...
					BCMTCH_DBG(
						BCMTCH_DF_RF,
						"RF: rows=%d cols=%d sensor_total=%d\n",
						bcmtch_data_ptr->sensor_rows,
						bcmtch_data_ptr->sensor_cols,
						bcmtch_data_ptr->sensor_total);

					bcmtch_data_ptr->afe_rx_preamp_q =
						p_tch_cfg->afe.afe_rx_preamp_q;
					bcmtch_data_ptr->afe_rx_preamp_gain =
						p_tch_cfg
						->afe.afe_rx_preamp_gain;
					bcmtch_data_ptr
					->asc_magnitude_truncation =
						p_tch_cfg
						->asc_magnitude_truncation;
					bcmtch_data_ptr->sd_shift =
						p_tch_cfg->dsp_cfg.sd_shift;
					BCMTCH_DBG(
						BCMTCH_DF_RF,
						"RF: rx_preamp_q=%d rx_preamp_gain=%d mag_trunc=%d sd_shift=%d\n",
						bcmtch_data_ptr
		...
		struct bcmtch_data *bcmtch_data_ptr)
{
	...
	pr_info("[TSP] BCMTCH chip_id = 0x%06X rev = 0x%2X\n",
		...
		struct bcmtch_data *bcmtch_data_ptr)
{
	...
	bcmtch_data_ptr->bcmtch_dev_process_table
			[BCMTCH_WP_STATE_RECORD] =
				bcmtch_dev_process_state_record;
#endif
#endif /* BCMTCH_STATE_PROTOCOL */

	/* The process function during the post boot patch
	 * swith (boot from ROM to boot from RAM). */
	bcmtch_data_ptr->bcmtch_dev_process_table
			[BCMTCH_WP_PATCH_INIT] =
...
#if BCMTCH_STATE_RAW_FRAME
static inline int32_t bcmtch_state_record_scan(
					struct bcmtch_data *bcmtch_data_ptr)
{
	int32_t ret_val = 0;
	uint8_t parameters[TOFE_HOST_CMD_SHORT_SIZE];

	parameters[0] = bcmtch_data_ptr->rec_params.p0.reg;
	parameters[1] = bcmtch_data_ptr->rec_params.p1.reg;

	ret_val = bcmtch_state_i2c_send_command(
					bcmtch_data_ptr,
					BCMTCH_CMD_SINGLE_SCAN,
					parameters,
					2);
	BCMTCH_DBG(
		BCMTCH_DF_RC,
		"RC: RECORD SCAN: (p0=0x%0x, p1=0x%0x) ret=%d\n",
		parameters[0],
		parameters[1],
		ret_val);
	...
	struct bcmtch_data *bcmtch_data_ptr)
{
	...
	uint32_t tch_report_size;
	uint8_t *dma_buff = bcmtch_data_ptr->bcmtch_state_resp_buffer;
	uint8_t *resp_buff = &dma_buff[1];
	...
	/* Read long response */
	bcmtch_data_ptr->rec_status.reg =
		bcmtch_data_ptr->bcmtch_state_resp_buffer[0];

	/* record data size */
	record_size = bcmtch_data_ptr
					->rec_status.data_size
						* BCMTCH_RECORD_SIZE_UNIT;

	/* Touch report size */
	tch_report_size =
		(uint32_t)
		(bcmtch_data_ptr->rec_params.p0.max_slot_number *
			bcmtch_data_ptr->scan_data.touch_slot_size);

	dma_len = record_size + 1
			+ (bcmtch_data_ptr->rec_params.p1.disable_touch ?
				0 : tch_report_size);

	BCMTCH_DBG(
		BCMTCH_DF_RC,
		"RC: rec_size=%d tch_size=%d dma_read=%d\n",
		record_size,
		tch_report_size,
		dma_len);

	if (dma_len > TOFE_HOST_RSP_BUF_SIZE) {
		BCMTCH_ERR(
			"RC: record/touch size %d overflow resp buffer %d.\n",
			dma_len,
			TOFE_HOST_RSP_BUF_SIZE);
		goto err_record_scan;
	}
	...
	ret_val = bcmtch_com_read_dma(
				 ...
				 dma_buff);
	if (ret_val) {
		BCMTCH_ERR("RC: %s DMA read error! (%d)\n",
				__func__,
				ret_val);
		goto err_record_scan;
	}
	...
	BCMTCH_DBG(
		BCMTCH_DF_RC,
		"RC:%s: ts=0x%08x ftx=0x%08x sc=%d size=%d\n",
		__func__,
		rec_info->timestamp,
		rec_info->ftx,
		rec_info->sample_counter,
	...
	if (bcmtch_data_ptr->rec_ftx != rec_info->ftx) {
		BCMTCH_DBG(
			BCMTCH_DF_RC,
			"RC:%s: last ftx=0x%08x new ftx=0x%08x\n",
			__func__,
			bcmtch_data_ptr->rec_ftx,
			rec_info->ftx);

		bcmtch_data_ptr->rec_ftx = rec_info->ftx;
	}

	/* Process touch report if touch is enabled. */
	if (!bcmtch_data_ptr->rec_params.p1.disable_touch) {
		touch_report = &resp_buff[record_size];

		bcmtch_data_ptr->scan_data.status.slots =
				bcmtch_data_ptr->rec_params.p0.max_slot_number;

		BCMTCH_DBG(BCMTCH_DF_RC, "RC: touch report:");
		for (i = 0; i < bcmtch_data_ptr->scan_data.touch_slot_size; i++)
			BCMTCH_DBG(
				BCMTCH_DF_RC,
				"RC: 0x%02x",
				touch_report[i]);
		BCMTCH_DBG(BCMTCH_DF_RC, "RC: end.]");
...
#endif
		bcmtch_state_record_scan(bcmtch_data_ptr);

	return;

err_record_scan:
#if BCMTCH_STATE_RAW_FRAME
	/* Set the raw frame reading done */
	bcmtch_data_ptr->bcmtch_raw_frame_pending = false;
#endif
	return;
}
...
#ifdef SEC_TSP_FACTORY_TEST
static void set_cmd_result(struct bcmtch_data *info, char *buff, int len)
{
	strncat(info->cmd_result, buff, len);
}
static void set_default_result(struct bcmtch_data *info)
{
	char delim = ':';
...
static void run_cm_delta_read(void *device_data)
{
	struct bcmtch_data *info = (struct bcmtch_data *)device_data;
...
static void fw_update(void *device_data)
{
	...
	set_default_result(info);
	set_cmd_result(info, "OK", 16);
	//strncat(cmd_result, "OK", 16);
	info->cmd_state = 2;
}
...
static void get_fw_ver_bin(void *device_data)
{
	...
	snprintf(buff , sizeof(buff),
				"%s%02x%04x\n",
				"BR", 0x5,
				info->fw_signature.cust_release_num);

	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	//strncat(cmd_result, FW_VERSION,16);
}

static ssize_t show_cmd_result(struct device *dev, struct device_attribute
				    *devattr, char *buf)
{
	struct bcmtch_data *info = dev_get_drvdata(dev);
...
static void get_chip_vendor(void *device_data)
{
	...
	set_default_result(info);
	snprintf(buff, sizeof(buff), "%s", "BROADCOM");
	set_cmd_result(info, buff, 16);
	info->cmd_state = 2;
}
...
static void get_chip_name(void *device_data)
{
	...
	char buff[16] = {0};
	set_default_result(info);
	snprintf(buff, sizeof(buff), "%s", "BCM15300");
	set_cmd_result(info, buff, 16);
	info->cmd_state = 2;
}
...
static void get_threshold(void *device_data)
{
	...
	set_default_result(info);
	if (info->threshold_gate_finger < 0) {
		snprintf(buff, sizeof(buff), "%s", "NG");
		set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
		info->cmd_state = 3;
		return;
	}
	snprintf(buff, sizeof(buff), "%d", info->threshold_gate_finger);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	//dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
	//		buff, strnlen(buff, sizeof(buff)));
				  ...
				  *devattr, const char *buf, size_t count)
{
	...
	if (info->cmd_is_running == true) {
		printk(KERN_INFO "tsp_cmd: other cmd is running.\n");
		goto err_out;
	}
	mutex_lock(&info->cmd_lock);
...
static ssize_t touchkey_menu_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	static u16 menu_sensitivity=100;
	return snprintf(buf, 20, "%d\n", menu_sensitivity);
}

static ssize_t touchkey_back_show(struct device *dev,
				struct device_attribute *attr, char *buf){
	static u16 back_sensitivity =100;
	return snprintf(buf, 20, "%d\n", back_sensitivity);
}

static ssize_t touchkey_version_read(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct bcmtch_data *info = dev_get_drvdata(dev);
...
static DEVICE_ATTR(touchkey_firm_version_panel, S_IRUGO,	touchkey_version_read, NULL);
static DEVICE_ATTR(touchkey_firm_version_phone, S_IRUGO,touchkey_version_read, NULL);
	...
	struct bcmtch_data *bcmtch_data_ptr)
{
	...
	if(!bcmtch_data_ptr->device_enabled)
		return 0;

	bcmtch_data_ptr->device_enabled = 0;
	...
	struct bcmtch_data *bcmtch_data_ptr)
{
	...
	bcmtch_data_ptr->device_enabled = 1;
		...
		bool enable)
{
	...
	gpio_request(31, "bcomtsp");

	ret_val = gpio_direction_output(31, enable);
		...
		struct bcmtch_data *bcmtch_data_ptr)
{
	...
	if (bcmtch_data_ptr) {
		...
		if (bcmtch_data_ptr->p_input_device) {
			...
			input_set_drvdata(bcmtch_data_ptr->p_input_device, bcmtch_data_ptr);

			set_bit(EV_SYN, bcmtch_data_ptr->p_input_device->evbit);
			set_bit(EV_ABS, bcmtch_data_ptr->p_input_device->evbit);
			set_bit(EV_LED, bcmtch_data_ptr->p_input_device->evbit);
	        set_bit(LED_MISC, bcmtch_data_ptr->p_input_device->ledbit);
			__set_bit(
				INPUT_PROP_DIRECT,
...
static int bcm_ts_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	pm_message_t msg = { PM_EVENT_SUSPEND };
	pr_info("[TSP] %s\n", __func__);
	return bcmtch_suspend(client, msg);
}

static int bcm_ts_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	pr_info("[TSP] %s\n", __func__);
	return bcmtch_resume(client);
}

static const struct dev_pm_ops bcm_ts_pm_ops = {
	.suspend = bcm_ts_suspend,
	.resume = bcm_ts_resume,
};
#endif
...
#ifdef SEC_TSP_FACTORY_TEST
	INIT_LIST_HEAD(&bcmtch_data_ptr->cmd_list_head);
	for (i = 0; i < ARRAY_SIZE(tsp_cmds_bcm); i++)
		list_add_tail(&tsp_cmds_bcm[i].list, &bcmtch_data_ptr->cmd_list_head);
	mutex_init(&bcmtch_data_ptr->cmd_lock);
	bcmtch_data_ptr->cmd_is_running = false;
	fac_dev_ts = device_create(sec_class,
			NULL, 0, p_i2c_client, "tsp");
	if (IS_ERR(fac_dev_ts))
		dev_err(&p_i2c_client->dev, "tsp: Failed to create device for the sysfs\n");
	dev_set_drvdata(fac_dev_ts, bcmtch_data_ptr);
	ret_val = sysfs_create_group(&fac_dev_ts->kobj,
				 &sec_touch_factory_attr_group);
	if (ret_val)
		dev_err(&p_i2c_client->dev, "TSP: Failed to create sysfs group\n");
#endif

#ifdef TOUCHKEY
	touchkey_dev = device_create(sec_class,
		NULL, 0, p_i2c_client, "sec_touchkey");

	if (IS_ERR(touchkey_dev))
		dev_err(&p_i2c_client->dev,
		"Failed to create device for the touchkey sysfs\n");
	dev_set_drvdata(touchkey_dev, bcmtch_data_ptr);
	ret_val = sysfs_create_group(&touchkey_dev->kobj,
		&sec_touchkey_attr_group);
	if (ret_val)
		dev_err(&p_i2c_client->dev, "Failed to create sysfs group\n");
#endif
	ret_val = sysfs_create_file(&bcmtch_data_ptr->p_input_device->dev.kobj,
			&attrs[0].attr);
	if (ret_val < 0) {
		dev_err(&p_i2c_client->dev,
...
static int32_t bcmtch_i2c_remove(struct i2c_client *p_i2c_client)
{
	...
	bcmtch_dev_free(p_i2c_client);
	 ...
	 sysfs_remove_group(&fac_dev_ts->kobj,
				 &sec_touch_factory_attr_group);
	device_unregister(fac_dev_ts);
#endif
#ifdef TOUCHKEY
	 sysfs_remove_group(&touchkey_dev->kobj,
				 &sec_touchkey_attr_group);
	device_unregister(touchkey_dev);
#endif
	return 0;
}
