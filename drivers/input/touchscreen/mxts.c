...
static int mxts_power_onoff(struct mxt_data *data, int onoff)
{
	int ret = 0;
	...
	if(onoff){
		ret = regulator_enable(data->pdata->vddo_supply);
		if (ret) {
		...
		ret = regulator_enable(data->pdata->vdd_supply);
		if (ret) {
		...
		ret = regulator_disable(data->pdata->vddo_supply);
		if (ret) {
		...
		ret = regulator_disable(data->pdata->vdd_supply);
		if (ret) {
			...
			struct mxt_platform_data *pdata)
{
	...
	pr_err("tsp_int= %d, ", pdata->tsp_int);
...
static int mxt_request_gpio(struct mxt_data *data)
{
	...
	gpio_tlmm_config(GPIO_CFG(data->pdata->tsp_int, 0,
			GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
#endif

	ret = regulator_set_voltage(data->pdata->vddo_supply, 1800000, 1800000);
	if (ret) {
...
#ifdef TSP_BOOSTER
static void set_dvfs_lock(struct mxt_data *data, int32_t on);
#endif
...
*/
	error = mxts_power_onoff(data, 0); //data->pdata->power_off();
	if (error) {
		...
		const struct i2c_device_id *id)
{
	...
	if (data->pdata->boot_address) {
	 ...
	 printk("the boot_address is 0x%04x and data->pdata->boot_address is 0x%04x and clien adress is 0x%04x \n",boot_address,data->pdata->boot_address,client->addr);
