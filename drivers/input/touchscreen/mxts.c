...
static int mxts_power_onoff(struct mxt_data *data, int enable)
{
	int ret = 0;
	static struct regulator *reg_l5;

	printk("TSP POWER ON OFF FUNCTION\n");
			...
			struct mxt_platform_data *pdata)
{
	...
	pr_err("tsp_en= %d, tsp_int= %d, ",pdata->tsp_en, pdata->tsp_int);
