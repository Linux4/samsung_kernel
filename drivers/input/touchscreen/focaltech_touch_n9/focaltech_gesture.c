...
static int gesture_get_capability(struct device *dev)
{
    ...
    FTS_INFO("ATO version close gesture.");
    return 0;
#else
    FTS_INFO("none-ATO version close gesture.");
    return 0; 
    //return BIT(GS_KEY_DOUBLE_TAP); //to open.
#endif
}
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
    ...
    ts_data->tid->tid_ops->gesture_set_capability = tid_gesture_set_capability;
    ts_data->tid->tid_ops->gesture_get_capability = gesture_get_capability;
