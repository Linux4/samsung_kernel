...
static int fts_fwupg_get_ver_in_host(struct fts_upgrade *upg, u8 *ver)
{
    ...
    if (upg->fw_length < 0x010E) {
        FTS_ERROR("fw len(0x%0x) < 0x010E",
                  upg->fw_length);
        return -EINVAL;
    }
    *ver = upg->fw[0x010E];
...
static void fts_fwupg_work(struct work_struct *work)
{
	...
	fts_fwupg_get_ver_in_host(upg, &ver);
		ts_data->tid->phone_fw_version = (unsigned int)ver;
		FTS_INFO("fw in host is : 0x%02x" , ver);
    if (upg->ts_data->fw_loading) {
        FTS_INFO("fw is loading, not download again");
        return ;
    }
...
static int tid_product_selector(struct device *dev)
{
	...
	if (ret <  0) {
		FTS_DEBUG("read boot id fail,retry:%d", ret);
	}
	FTS_INFO("chip id_focal:%04x %04x", chip_id[0], chip_id[0]);
...
{
	out_values[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = 0x46;
	return 0;
}
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
    ...
    //tid_ops->firmware_upgrade = NULL;//tid_firmware_upgrade;
#if FTS_ESDCHECK_EN
    fts_esdcheck_switch(DISABLE);
#endif
