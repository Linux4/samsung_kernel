...
#if 0
        fts_read_reg(0xC8, &val_C8);
        if (val_C8 == 0x01){
            FTS_INFO("fts disable avdd!!!");

            ret = fts_power_source_ctrl(ts_data, DISABLE);
            FTS_INFO("disable vdd regulator first ,ret=%d", ret);
            if (ret) {
                FTS_ERROR("disable vdd regulator first failed,ret=%d", ret);
            }
            ts_data->power_disabled = false;
            ret = fts_power_source_ctrl(ts_data, DISABLE);
            FTS_INFO("disable vdd regulator second ,ret=%d", ret);       
            if (ret) {
                FTS_ERROR("disable vdd regulator second failed,ret=%d", ret);
            }
            msleep(6);
            ret = fts_power_source_ctrl(ts_data, ENABLE);
            FTS_INFO("enable vdd regulator first ,ret=%d", ret);
            if (ret) {
                FTS_ERROR("enable vdd regulator first failed,ret=%d", ret);
            }
             ts_data->power_disabled = true;
            ret = fts_power_source_ctrl(ts_data, ENABLE);
            FTS_INFO("enable vdd regulator second ,ret=%d", ret);
            if (ret) {
                FTS_ERROR("enable vdd regulator second failed,ret=%d", ret);
            }

            FTS_INFO("fts esdcheck end avdd!!!");
            fts_write_reg(0xC9, 1);
        }

#endif 
#if 1
		/*add by machengcheng esd 2019-6-14 start */
		fts_read_reg(0xC8, &val_C8);
        if (val_C8 == 0x01){
            FTS_INFO("fts disable avdd!!!");
            ret = regulator_disable(ts_data->vddn);
            if (ret) {
                FTS_ERROR("disable vddn regulator failed,ret=%d", ret);
            }
            ret = regulator_disable(ts_data->vddp);
            if (ret) {
                FTS_ERROR("disable vddp regulator failed,ret=%d", ret);
            }
            msleep(6);
            FTS_INFO("fts enable avdd!!!");
            ret = regulator_enable(ts_data->vddp);
            if (ret) {
            	FTS_ERROR("enable vddp regulator failed,ret=%d", ret);
            }
            ret = regulator_enable(ts_data->vddn);
            if (ret) {
            	FTS_ERROR("enable vddn regulator failed,ret=%d", ret);
            }
            FTS_INFO("fts esdcheck end avdd!!!");
            fts_write_reg(0xC9, 1);
        }
		/*add by machengcheng esd 2019-6-14 end */
#endif        
