...
struct ilitek_sec_info *ilitek_sec_info= NULL;
...
struct platform_device ilitek_hwinfo_device= {
	.name = HWINFO_NAME,
	.id = -1,
};
...
extern unsigned char g_user_buf[];
...
void ili_node_init(void)
{
	int i = 0, ret = 0;
/* HS70 add for SR-ZQL1871-01-172 by zhouzichun at 20191016 start */
	struct ilitek_sec_info *sec_info = NULL;
	sec_info = (struct ilitek_sec_info *)kzalloc(sizeof(*ilitek_sec_info), GFP_KERNEL);
	if (!sec_info) {
		ILI_ERR("allocate memory for sec_info fail");
		return ;
	}
	ilitek_sec_info = sec_info;
...
/* HS70 add for SR-ZQL1871-01-172 by zhouzichun at 20191016 end */
	...
	netlink_init();
	platform_device_register(&ilitek_hwinfo_device);
    	ilitek_test_node_init(&ilitek_hwinfo_device);
	ilitek_extra_proc_init();//creat /proc/tp_info
/* HS70 add for SR-ZQL1871-01-172 by zhouzichun at 20191016 start */
	ret = sec_cmd_init(&sec_info->sec, ilitek_commands, ilitek_get_array_size(), SEC_CLASS_DEVT_TSP);
    	if (ret < 0) {
