...
static int mp_show_result(bool lcm_on)
{
	...
	if (pass_item_count == 0) {
		...
		if(core_mp.lost_parameter){
			...
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s/%s_%s_%s.csv", ret_path_name,ilits->md_name,get_date_time_str(), ret_fail_name);
	} else {
		...
		snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s/%s_%s_%s.csv", ret_path_name, ilits->md_name,get_date_time_str(), ret_pass_name);
	}
