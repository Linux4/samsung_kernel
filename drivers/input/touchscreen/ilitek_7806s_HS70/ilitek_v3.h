...
/* HS70 modify for HS70-147 by zhouzichun at 20191010 start */     //need_check
extern struct platform_device ilitek_hwinfo_device;
int ilitek_test_node_exit(struct platform_device *tpinfo_device);
/* HS70 modify for HS70-147 by zhouzichun at 20191010 end */
/* HS70 modify for SR-ZQL1871-01-172 by zhouzichun at 20191020 start */
extern struct sec_cmd ilitek_commands[];
extern int ilitek_get_array_size(void);
...
#define RESUME_BY_DDI				ENABLE    //need_check
...
#define ILITEK_USE_ENABLE_NODE  ENABLE
...
#define CSV_LCM_OFF_PATH		"/sdcard/tpdata"
...
({									\
	pr_info("ILI7806S:[INFO] (%s, %d): " fmt, __func__, __LINE__, ##arg);	\
})									\

#define ILI_ERR(fmt, arg...)						\
({									\
	pr_err("ILI7806S:[ERR] (%s, %d): " fmt, __func__, __LINE__, ##arg);	\
})									\
...
do {									\
	if (debug_en)						\
	pr_info("ILI7806S: [DEBUG](%s, %d): " fmt, __func__, __LINE__, ##arg);	\
} while (0)
...
/* HS70 code for HS70-133 by liufurong at 2019/10/31 end */
	...
	bool glove; //add for touch sensitivity
...
struct ilitek_sec_info {
	struct sec_cmd_data sec;
	u32 fw_version;
	u8 cfg_version;
	u32 fw_version_bin;
};
extern struct ilitek_sec_info *ilitek_sec_info;
...
/* HS70 add for SR-ZQL1871-01-177 by gaozhengwei at 2019/11/02 start */
extern int ili_lcm_bias_power_init(struct ilitek_ts_data *data);
extern int ili_lcm_bias_power_deinit(struct ilitek_ts_data *data);
extern int ili_lcm_power_source_ctrl(struct ilitek_ts_data *data, int enable);
/* HS70 add for SR-ZQL1871-01-177 by gaozhengwei at 2019/11/02 end */
