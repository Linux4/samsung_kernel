...
static int ilitek_get_tp_module(void)
{
	...
	if (NULL != strstr(panel_name, "mdss_dsi_tm19_ili7806s_hd_video")){
		fw_num = MODEL_TM;
...
void ili_dev_remove(void)
{
	...
	ilitek_test_node_exit(&ilitek_hwinfo_device);
	kfree(ilits->tr_buf);
	kfree(ilits->gcoord);
	ili_interface_dev_exit(ilits);
	/* HS70 add for SR-ZQL1871-01-172 by zhouzichun at 20191021 start */
	if (ilitek_sec_info)
		kfree(ilitek_sec_info);
	/* HS70 add for SR-ZQL1871-01-172 by zhouzichun at 20191021 end */
}
