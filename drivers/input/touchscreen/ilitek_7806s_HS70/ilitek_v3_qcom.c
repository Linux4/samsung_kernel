...
void ili_input_register(void)
{
	...
	ilits->input->close = ili_input_close;
#endif
...
#if ILITEK_USE_ENABLE_NODE
    ret = sysfs_create_link(&ilitek_sec_info->sec.fac_dev->kobj,&ilits->input->dev.kobj, "input");
    if (ret < 0) {
        ILI_ERR("%s: Failed to sysfs_create_link\n", __func__);
    }
#endif
}
...
static int ilitek_plat_notifier_fb(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (!(event == FB_EARLY_EVENT_BLANK || event == FB_EVENT_BLANK)) {
		ILI_DBG("event(%lu) do not need process\n", event);
		return 0;
	}
...
static int ilitek_pinctrl_init(void)
{
	...
	if (IS_ERR_OR_NULL(ilits->ts_pinctrl)) {
		ILI_ERR("Failed to get pinctrl");
		ret = PTR_ERR(ilits->ts_pinctrl);
		ilits->ts_pinctrl = NULL;
		return ret;
	}

	ilits->int_default = pinctrl_lookup_state(ilits->ts_pinctrl,
		"default");
	if (IS_ERR_OR_NULL(ilits->int_default)) {
		ILI_ERR("Pin state[default] not found");
		ret = PTR_ERR(ilits->int_default);
		goto exit_put;
	}

	ilits->int_out_high = pinctrl_lookup_state(ilits->ts_pinctrl,
		"int-output-high");
	if (IS_ERR_OR_NULL(ilits->int_out_high)) {
		ILI_ERR("Pin state[int-output-high] not found");
		ret = PTR_ERR(ilits->int_out_high);
	}

	ilits->int_out_low = pinctrl_lookup_state(ilits->ts_pinctrl,
		"int-output-low");
	if (IS_ERR_OR_NULL(ilits->int_out_low)) {
		ILI_ERR("Pin state[int-output-low] not found");
		ret = PTR_ERR(ilits->int_out_low);
		goto exit_put;
	}

	ilits->int_input = pinctrl_lookup_state(ilits->ts_pinctrl,
		"int-input");
	if (IS_ERR_OR_NULL(ilits->int_input)) {
		ILI_ERR("Pin state[int-input] not found");
		ret = PTR_ERR(ilits->int_input);
		goto exit_put;
	}
...
exit_put:
	...
	ilits->ts_pinctrl = NULL;
	ilits->int_default = NULL;
	ilits->int_out_high = NULL;
	ilits->int_out_low = NULL;
	ilits->int_input = NULL;
	return ret;
}
/* HS70 code for HS70-133 by liufurong at 2019/10/31 end */
...
/* HS70 add for SR-ZQL1871-01-177 by gaozhengwei at 2019/11/02 end */
	...
	if (!ret && ilits->ts_pinctrl) {
		/*
		 * Pinctrl handle is optional. If pinctrl handle is found
		 * let pins to be configured in active state. If not
		 * found continue further without error.
		 */
		ret = pinctrl_select_state(ilits->ts_pinctrl,ilits->int_input);
		if (ret < 0) {
			ILI_ERR("failed to select pin to active state\n");
		}
	}
