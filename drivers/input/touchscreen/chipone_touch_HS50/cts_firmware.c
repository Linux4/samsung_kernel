        ...
        const struct cts_firmware *firmware, bool to_flash)
{
    ...
    if (!to_flash || !cts_dev->rtdata.has_flash) {
        cts_info("Write firmware to sram size %zu",
            firmware_info.firmware_sect_size);

	/* Legacy fw will copy CRC and len info from SRAM to
	* hardware register when boot but new fw NOT do this:
	...
	*/
       if ((ret = cts_sram_writel_retry(cts_dev, 0x0FFFC,
           firmware_info.firmware_sect_crc, 3, 1)) != 0) {
           cts_err("Write CRC to 0x0FFFC failed %d", ret);
           goto out;
       }
       if ((ret = cts_sram_writel_retry(cts_dev, 0x0FFF8,
