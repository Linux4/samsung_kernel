...
static void himax_mcu_power_on_init(void)
{
	...
	/*Tab A8 code for SR-AX6300-01-131 by chenpengbo at 2021/09/22 start*/
       ...
       himax_parse_assign_cmd(fw_data_hand_blade_on_off, tmp_data, 4);
       g_core_fp.fp_register_write(pfw_op->addr_hand_blade_addr,
                            tmp_data, DATA_LEN_4);
#endif
...
#endif
/*Tab A8 code for SR-AX6300-01-126 by chenpengbo at 2021/09/02 start*/
...
/*Tab A8 code for SR-AX6300-01-126 by chenpengbo at 2021/09/02 end*/
...
static void hx_ap_notify_fw_sus(int suspend)
{
	...
	/*Tab A8 code for SR-AX6300-01-131 by chenpengbo at 2021/09/22 start*/
#if defined(SEC_PALM_FUNC)
	himax_parse_assign_cmd(fw_data_hand_blade_on_off,
				data_tmp, 4);
	g_core_fp.fp_register_write(pfw_op->addr_hand_blade_addr,
				data_tmp, DATA_LEN_4);
#endif
...
static bool himax_mcu_get_DSRAM_data(uint8_t *info_data, bool DSRAM_Flag)
{
	...
	if (private_ts->in_self_test == HX_FAC_TEST) {
		g_core_fp.fp_sense_off(true);
	}
...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	if (ret < 0) {
		if (private_ts->in_self_test) {
			...
			private_ts->using_kernel_fw = 1;
#elif defined(FW_GET_FROM_I_FILE)
			if (private_ts->firmware_filp_open.data != NULL) {
				tmp_fw_entry->data = private_ts->firmware_filp_open.data;
				tmp_fw_entry->size = private_ts->firmware_filp_open.size;
				fw = tmp_fw_entry;
				private_ts->using_kernel_fw = 1;
...
/* CORE_FW */
	...
	/*Tab A8 code for SR-AX6300-01-131 by chenpengbo at 2021/09/22 start*/
#if defined(SEC_PALM_FUNC)
	himax_parse_assign_cmd(fw_addr_hand_blade_addr,
		pfw_op->addr_hand_blade_addr,
		sizeof(pfw_op->addr_hand_blade_addr));
#endif
	/*Tab A8 code for SR-AX6300-01-131 by chenpengbo at 2021/09/22 end*/
#if defined(HX_ULTRA_LOW_POWER)
