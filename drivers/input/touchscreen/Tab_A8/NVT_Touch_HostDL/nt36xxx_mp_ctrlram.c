...
#define FILE_INFOR "INX_NT36523B_"
...
static int32_t nvt_selftest_for_hq(void)
{
	...
	sprintf(csv_file_path, "%s%s%s%s.csv", FW_RAWDATA_CSV_FILE, FILE_INFOR, get_date_time_str(), TestResult_FWMutual == 0 ? "_PASS" : "_FAIL");
	...
	/*Tab A8 code for SR-AX6300-01-373 by luxinjun at 2021/09/10 start*/
	memset(csv_file_path,0,64 * sizeof(char));
	sprintf(csv_file_path, "%s%s%s%s.csv", FW_CC_CSV_FILE, FILE_INFOR, get_date_time_str(), TestResult_FW_CC == 0 ? "_PASS" : "_FAIL");
	// Save Rawdata to CSV file
	...
	/*Tab A8 code for SR-AX6300-01-373 by luxinjun at 2021/09/10 start*/
	memset(csv_file_path,0,64 * sizeof(char));
	sprintf(csv_file_path, "%s%s%s%s.csv", NOISE_TEST_CSV_FILE, FILE_INFOR, get_date_time_str(), TestResult_Noise == 0 ? "_PASS" : "_FAIL");
	// Save Rawdata to CSV file
	...
	/*Tab A8 code for SR-AX6300-01-373 by luxinjun at 2021/09/10 start*/
	memset(csv_file_path,0,64 * sizeof(char));
	sprintf(csv_file_path, "%s%s%s%s.csv", SHORT_TEST_CSV_FILE, FILE_INFOR, get_date_time_str(), TestResult_Short == 0 ? "_PASS" : "_FAIL");
	...
	/*Tab A8 code for SR-AX6300-01-373 by luxinjun at 2021/09/10 start*/
	memset(csv_file_path,0,64 * sizeof(char));
	sprintf(csv_file_path, "%s%s%s%s.csv", OPEN_TEST_CSV_FILE, FILE_INFOR, get_date_time_str(), TestResult_Open == 0 ? "_PASS" : "_FAIL");
	// Save RawData to CSV file
