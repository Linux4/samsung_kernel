...
static const char *goodix_ts_name = "Goodix GT9XX TouchScreen";
...
static void gtp_int_sync(struct i2c_client *client, s32 ms)
{
	...
	GTP_GPIO_OUTPUT(GTP_INT_PORT(gtp_data->irq), 0);
	GTP_MSLEEP(ms);
	GTP_GPIO_AS_INT(GTP_INT_PORT(gtp_data->irq));
}
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	GTP_GPIO_OUTPUT(GTP_RST_PORT(gtp_data->reset), 0);
	GTP_MSLEEP(ms);
	GTP_GPIO_OUTPUT(GTP_INT_PORT(gtp_data->irq), client->addr == 0x14);

	GTP_MSLEEP(2);
	GTP_GPIO_OUTPUT(GTP_RST_PORT(gtp_data->reset), 1);

	/* must > 3ms */
	GTP_MSLEEP(6);
	GTP_GPIO_AS_INPUT(GTP_RST_PORT(gtp_data->reset));
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
	...
	while (retry++ < 5) {
		...
		if (ret > 0) {
			GTP_INFO("GTP CONFIG VERSION: 0x%2x", test[2]);
			return ret;
		}
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	ret = GTP_GPIO_REQUEST(GTP_INT_PORT(ts->data->irq), "GTP_INT_IRQ");
	if (ret < 0) {
		GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d", \
				(s32)GTP_INT_PORT(ts->data->irq), ret);
		ret = -ENODEV;
	} else
		GTP_GPIO_AS_INT(GTP_INT_PORT(ts->data->irq));

	ret = GTP_GPIO_REQUEST(GTP_RST_PORT(ts->data->reset), "GTP_RST_PORT");
	if (ret < 0) {
		GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d", \
				(s32)GTP_RST_PORT(ts->data->reset), ret);
		ret = -ENODEV;
	}

	GTP_GPIO_AS_INPUT(GTP_RST_PORT(ts->data->reset));
	gtp_reset_guitar(ts->client, 20);

	if (ret < 0) {
		GTP_GPIO_FREE(GTP_RST_PORT(ts->data->reset));
		GTP_GPIO_FREE(GTP_INT_PORT(ts->data->irq));
	}
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
	...
	if (ret) {
		...
		GTP_GPIO_AS_INPUT(GTP_INT_PORT(ts->data->irq));
		GTP_GPIO_FREE(GTP_INT_PORT(ts->data->irq));
			...
			const struct i2c_device_id *id)
{
	...
	if (ret == 0) {
		GTP_ERROR("GTP create WR node failed");
		ret = -ENODEV;
		goto out_irq;
	}
#endif
...
static int gtp_enter_sleep(struct goodix_ts_data *ts)
{
	...
	GTP_GPIO_OUTPUT(GTP_INT_PORT(ts->data->irq), 0);
	GTP_MSLEEP(5);
	while (retry++ < 5) {
...
#else
	while (retry++ < 10) {
		GTP_GPIO_OUTPUT(GTP_INT_PORT(ts->data->irq), 1);
		GTP_MSLEEP(5);
