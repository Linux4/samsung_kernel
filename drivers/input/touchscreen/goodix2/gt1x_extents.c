...
s32 gesture_event_handler(struct input_dev * dev)
{
	...
	if (DOZE_ENABLED == gesture_doze_status) {
		...
		if (ret == 0) {
			if (doze_buf[0] == 0 || !QUERYBIT(gestures_flag, doze_buf[0])) {
			...
			mutex_unlock(&gesture_data_mutex);
            key_code = doze_buf[0] < 16? KEY_GES_CUSTOM : KEY_GES_REGULAR;

            gesture_doze_status = DOZE_WAKEUP;
...
#define IO_VERSION                   "V1.2-20150420"
