...
#if GTP_SMART_COVER || GTP_PROXIMITY
...
s32 gt1x_wakeup_sleep(void)
{
	...
	while (retry++ < 2) {
...
#endif
		{
			...
			/* test i2c */
			ret = gt1x_i2c_test();
			if (!ret) {  /* i2c test succeed, init externl watchdog */
#if GTP_ESD_PROTECT
				ret = gt1x_init_ext_watchdog();
				if(!ret) {
					break;
	...
	if (ret) {		/* wakeup failed , try waking up by reset */
		while (retry--) {
			ret = gt1x_reset_guitar();
			if(!ret) {
				break;
...
s32 gt1x_touch_event_handler(u8 * data, struct input_dev * dev, struct input_dev * pen_dev)
{
    ...
    if (check_sum) {
        GTP_ERROR("Checksum error, discard this package!");
        return ERROR_VALUE;
    }
/* 
