...
static s32 i2c_dma_read_mtk(struct mms_ts_info *info,char *write_buf, unsigned int wlen, u8 *buffer, s32 len);
static s32 i2c_dma_write_mtk(struct mms_ts_info *info, u8 *buffer, s32 len);
#if TPD_SUPPORT_I2C_DMA
static u8 *gpDMABuf_va;
static dma_addr_t gpDMABuf_pa;
struct mutex dma_mutex;
struct mms_ts_info *localinfo;//add by cassy
...
static s32 i2c_dma_read_mtk(struct mms_ts_info *info,char *write_buf, unsigned int wlen, u8 *buffer, s32 len)
{
	...
	while (pos != len) {
		...
		ret = i2c_transfer(info->client->adapter, msgs, 2);
		if (ret != 2) {
			printk("test by cassy for I2C Transfer error!");
			ret = ERROR_IIC;
			break;
		}
		ret = 0;
				...
				char *read_buf, unsigned int read_len)
{
	...
	if(read_len>8)
	{
	   i2c_dma_read_mtk(info, write_buf,write_len, read_buf, read_len);
	   goto DONE;
	}
	else
	{
				...
				unsigned int read_len)
{
	...
	while (retry--) {
// bobule porting i2c dma
		if (read_len < 8) {
			info->client->addr = info->client->addr;
			info->client->timing = I2C_MASTER_CLOCK;
			res = i2c_master_recv(info->client, rbuf, read_len);
		}
		else {
			info->client->addr = info->client->addr & I2C_MASK_FLAG;
			info->client->ext_flag = (info->client->ext_flag | I2C_ENEXT_FLAG | I2C_DMA_FLAG),
			info->client->timing = I2C_MASTER_CLOCK;
			res = i2c_master_recv(info->client, (u8 *)(uintptr_t)gpDMABuf_pa, read_len);
			if (res > 0)
				memcpy(rbuf, gpDMABuf_va, read_len);
			else
      			dev_dbg(&info->client->dev, "%s [DMA] fail! res:%d\n", __func__, res);
		}
// bobule porting i2c dma end
		if (res == read_len) {
			goto DONE;
		} else if (res < 0) {
			dev_err(&info->client->dev,
...
//add by cassy begin
	if(write_len>8)
	{
	   i2c_dma_write_mtk(info, write_buf, write_len);
	   goto DONE;
	}
	else
	{
...
static int mms_test_resume(struct device *dev)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
...
static int mms_test_suspend(struct device *dev)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
...
static const struct dev_pm_ops mms_dev_pm_ops = {
	.suspend = mms_test_suspend,
	.resume = mms_test_resume,
};
#endif
