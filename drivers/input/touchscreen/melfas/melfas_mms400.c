...
static DEVICE_ATTR(fw_update, S_IRUGO, mms_sys_fw_update, NULL);
...
static void mms_switching_tsp_work(struct work_struct *work)
{
	...
	if (mms_data->flip_status != mms_data->flip_status_current)
	{
		...
		gpio_direction_output(mms_data->dtdata->tsp_sel,
			mms_data->flip_status_current);
...
ERROR:
	...
	pr_err("MELFAS " CHIP_NAME " Touchscreen initialization failed %d\n", ret);
	return ret;
}
