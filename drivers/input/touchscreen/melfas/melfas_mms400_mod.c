...
int mms_parse_devicetree(struct device *dev, struct mms_ts_info *info)
{
	...
	if (IS_ERR(info->dtdata->vreg_vio))
		dev_err(dev, "%s [ERROR] vdd-io get\n", __func__);
...
void mms_set_cover_type(struct mms_ts_info *info)
{
	...
	dev_info(&info->client->dev, "%s: %d, type %d\n",
			__func__, info->flip_enable, info->cover_type);

	if (!info->enabled) {
		dev_err(&info->client->dev, "%s: dev is not enabled\n", __func__);
		return;
	}
	...
	if (ret) {
		dev_err(&info->client->dev, "%s [ERROR] mms_i2c_write %x %x %x, ret %d\n",
...
int mms_lowpower_mode(struct mms_ts_info *info, int on)
{
	...
	if (!info->dtdata->support_lpm) {
		dev_err(&info->client->dev, "%s not supported\n", __func__);
		return -EINVAL;
	}
	...
	if (mms_i2c_write(info, wbuf, 3)) {
		dev_err(&info->client->dev, "%s [ERROR] write %x %x %x\n",
	...
	if (mms_i2c_read(info, wbuf, 2, rbuf, 1)) {
		dev_err(&info->client->dev, "%s [ERROR] read %x %x, rbuf %x\n",
				__func__, wbuf[0], wbuf[1], rbuf[0]);
		return -EIO;
	}

	if (rbuf[0] != on) {
		dev_err(&info->client->dev, "%s [ERROR] not changed to %s mode, rbuf %x\n",
				__func__, on ? "LPM" : "normal", rbuf[0]);
		return -EIO;
	}

	dev_info(&info->client->dev, "%s: %s mode", __func__, on ? "LPM" : "normal");
	return 0;
}
