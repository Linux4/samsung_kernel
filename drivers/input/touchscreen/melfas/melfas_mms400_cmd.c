...
static void cmd_get_fw_ver_bin(void *device_data)
{
	...
	dev_info(&info->client->dev, "%s: OCTA ID 0x%06x\n", __func__, info->octa_id);

	sprintf(buf, "ME%02X%02X%02X\n", info->octa_id & 0xFF, ver_file[3],ver_file[5]);
	sec->cmd_state = SEC_CMD_STATUS_OK;
...
static void cmd_get_fw_ver_ic(void *device_data)
{
	...
	sprintf(buf, "ME%02X%02X%02X\n",
		info->octa_id & 0xFF, info->core_ver_ic, info->config_ver_ic);
	sec->cmd_state = SEC_CMD_STATUS_OK;
...
static void spay_enable(void *device_data)
{
	...
	dev_info(&client->dev, "%s: %s mode, %x\n",
...
void mms_sysfs_cmd_remove(struct mms_ts_info *info)
{
	...
	sysfs_delete_link(&info->sec.fac_dev->kobj,
			&info->input_dev->dev.kobj, "input");
