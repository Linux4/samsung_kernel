...
int mms_run_test(struct mms_ts_info *info, u8 test_type)
{
	...
	dev_dbg(&info->client->dev, "%s [START]\n", __func__);
	dev_info(&info->client->dev, "%s - test_type[%d]\n", __func__, test_type);
	...
	dev_info(&info->client->dev, "%s - set control mode\n", __func__);
	...
	dev_info(&info->client->dev, "%s - set test type\n", __func__);
...
//		msleep(10);
	...
	buf_addr_l = rbuf[0];
	buf_addr_h = rbuf[1];
	dev_info(&info->client->dev, "%s - buf_addr[0x%02X 0x%02X]\n",
	...
	dev_info(&info->client->dev, "%s - set normal mode\n", __func__);
	ret = 0;
...
int mms_get_image(struct mms_ts_info *info, u8 image_type)
{
	...
	dev_dbg(&info->client->dev, "%s [START]\n", __func__);
	dev_info(&info->client->dev, "%s - image_type[%d]\n", __func__, image_type);
 ...
 * Sysfs functions
 */
static DEVICE_ATTR(fw_version, 0644, mms_sys_fw_version, NULL);
static DEVICE_ATTR(info, 0644, mms_sys_info, NULL);
static DEVICE_ATTR(device_enable, 0644, mms_sys_device_enable, NULL);
static DEVICE_ATTR(device_disable, 0644, mms_sys_device_disable, NULL);
static DEVICE_ATTR(irq_enable, 0644, mms_sys_irq_enable, NULL);
static DEVICE_ATTR(irq_disable, 0644, mms_sys_irq_disable, NULL);
static DEVICE_ATTR(power_on, 0644, mms_sys_power_on, NULL);
static DEVICE_ATTR(power_off, 0644, mms_sys_power_off, NULL);
static DEVICE_ATTR(reboot, 0644, mms_sys_reboot, NULL);
static DEVICE_ATTR(mode_glove, 0644, mms_sys_glove_mode_show, mms_sys_glove_mode_store);
static DEVICE_ATTR(mode_charger, 0644,
			mms_sys_charger_mode_show, mms_sys_charger_mode_store);
static DEVICE_ATTR(mode_cover_window, 0644,
			mms_sys_window_mode_show, mms_sys_window_mode_store);
static DEVICE_ATTR(mode_palm_rejection, 0644,
			mms_sys_palm_rejection_mode_show, mms_sys_palm_rejection_mode_store);
static DEVICE_ATTR(image_intensity, 0644, mms_sys_intensity, NULL);
static DEVICE_ATTR(image_rawdata, 0644, mms_sys_rawdata, NULL);
static DEVICE_ATTR(test_cm_delta, 0644, mms_sys_test_cm_delta, NULL);
static DEVICE_ATTR(test_cm_abs, 0644, mms_sys_test_cm_abs, NULL);
static DEVICE_ATTR(test_cm_jitter, 0644, mms_sys_test_cm_jitter, NULL);
static DEVICE_ATTR(test_short, 0644, mms_sys_test_short, NULL);
