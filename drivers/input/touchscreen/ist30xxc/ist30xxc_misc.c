...
int parse_tsp_node(struct ist30xx_data *data, u8 flag,
        struct TSP_NODE_BUF *node, s16 *buf16, bool only_screen)
{
	int i, j;
	s16 val = 0;
	TSP_INFO *tsp = &data->tsp_info;
...
reg_access_end:
	ist30xx_cmd_hold(data->client, 0);
...
extern void ist30xx_set_ta_mode(int charging);
			    ...
			    const char *buf, size_t size)
{
	...
	if (direct->cmd == DIRECT_CMD_WRITE) {
		ist30xx_cmd_hold(data->client, 1);
		ist30xx_write_cmd(data->client, DIRECT_ADDR(direct->addr),
                direct->val);
		ret = ist30xx_read_reg(data->client, DIRECT_ADDR(direct->addr),
			&direct->val);
		if (ret)
...
int ist30xx_init_misc_sysfs(struct ist30xx_data *data)
{
	...
	int frame_buf_size = pdata->max_node * IST30XX_DATA_LEN;
	...
	data->tsp_info.node = node;

	data->tsp_info.node->raw =
		kzalloc(pdata->max_node * sizeof(u16), GFP_KERNEL);
	data->tsp_info.node->base =
		kzalloc(pdata->max_node * sizeof(u16), GFP_KERNEL);
	data->tsp_info.node->filter =
		kzalloc(pdata->max_node * sizeof(u16), GFP_KERNEL);
