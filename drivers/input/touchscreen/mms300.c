...
#define FW_IMAGE_NAME	"tsp_melfas/mms345L.fw";
...
static void get_config_ver(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "G510F_Me_0730_XX");
...
out:
	...
	dev_info(&client->dev, "reboot mms\n");
...
static int mms_ts_fw_check(struct mms_ts_info *info)
{
	...
	if (ret < 0) {		/* tsp connect check */
		dev_err(&client->dev, "%s: i2c fail...[%d], addr[%x]\n",
	...
	if (!update) {
#if defined(CONFIG_SEC_KLEOS_PROJECT) || defined(TEMP_USE_FOR_DEV)
		if ((info->fw_boot_ver < BOOT_VERSION) || (info->fw_boot_ver > 0xf0) ||
			(info->fw_core_ver < CORE_VERSION) || (info->fw_core_ver > 0xf0) ||
			(info->fw_ic_ver < FW_VERSION) || (info->fw_ic_ver > 0xf0)) {
			if (info->pdata->gpio_seperated > 0) {
				dev_err(&client->dev,
...
static int mms_ts_input_open(struct input_dev *dev)
{
	...
	ret = gpio_get_value(info->pdata->vdd_en);
	if (!ret)
		ret = gpio_direction_output(info->pdata->vdd_en, 1);
	msleep(150);
