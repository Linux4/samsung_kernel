...
static void get_scr_y_num(void *dev_data)
{
	...
	if (val >= 0) {
		...
		snprintf(buf, sizeof(buf), "%s", "NG");
		sec->cmd_state = CMD_STATE_FAIL;
		tsp_err("%s: fail to read num of y (%d).\n", __func__, val);
	}
	set_cmd_result(sec, buf, strnlen(buf, sizeof(buf)));
	tsp_info("%s: %s(%d)\n", __func__,
			buf, strnlen(buf, sizeof(buf)));
...
static void get_all_x_num(void *dev_data)
{
	...
	if (val >= 0) {
		...
		snprintf(buf, sizeof(buf), "%s", "NG");
		sec->cmd_state = CMD_STATE_FAIL;
		tsp_err("%s: fail to read all num of x (%d).\n", __func__, val);
	}
	set_cmd_result(sec, buf, strnlen(buf, sizeof(buf)));
	tsp_info("%s: %s(%d)\n", __func__,
			buf, strnlen(buf, sizeof(buf)));
...
static void get_all_y_num(void *dev_data)
{
	...
	if (val >= 0) {
		...
		snprintf(buf, sizeof(buf), "%s", "NG");
		sec->cmd_state = CMD_STATE_FAIL;
		tsp_err("%s: fail to read all num of y (%d).\n", __func__, val);
	}
	set_cmd_result(sec, buf, strnlen(buf, sizeof(buf)));
	tsp_info("%s: %s(%d)\n", __func__,
			buf, strnlen(buf, sizeof(buf)));
...
//	sec_touchkey = sec_device_create(data, "sec_touchkey");
	sec_touchkey = sec_device_create(10, data, "sec_touchkey");
...
//	sec_fac_dev = sec_device_create(data, "tsp");
	sec_fac_dev = sec_device_create(11, data, "tsp");
...
err_sec_fac_dev_attr:
	sec_device_destroy(11);
err_sec_fac_dev:
#if IST30XX_USE_KEY
err_sec_touchkey_attr:
	sec_device_destroy(10);
err_sec_touchkey:
#endif
	return -ENODEV;
}
