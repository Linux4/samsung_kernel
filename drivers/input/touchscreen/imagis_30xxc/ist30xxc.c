...
void tsp_printk(int level, const char *fmt, ...)
{
	...
	printk("%s %s %pV", SECLOG, IST30XX_DEBUG_TAG, &vaf);
...
int tsp_touched[IST30XX_MAX_MT_FINGERS];
void print_tsp_event(struct ist30xx_data *data, finger_info *finger)
{
	...
	if (press) {
		...
		if (tsp_touched[idx] == 0) {
			/* touch down */
			data->touch_pressed_num++;
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
			tsp_noti("%s%d (%d, %d)(%d) M:%d fw:0x%x\n",
					TOUCH_DOWN_MESSAGE, finger->bit_field.id,
					finger->bit_field.x, finger->bit_field.y,
					data->z_values[idx], finger->bit_field.area, data->fw.cur.fw_ver);
#else
			tsp_noti("%s%d M:%d fw:0x%x\n", TOUCH_DOWN_MESSAGE, finger->bit_field.id,
				finger->bit_field.area, data->fw.cur.fw_ver);
#endif
...
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
			tsp_noti("%s%d (%d, %d) mc:%d\n", TOUCH_UP_MESSAGE, finger->bit_field.id, data->lx, data->ly, tsp_touched[idx]);
#else
			tsp_noti("%s%d mc:%d\n", TOUCH_UP_MESSAGE, finger->bit_field.id, tsp_touched[idx]);
...
static int tsp_ta_notification(struct notifier_block *nb,
		unsigned long action, void *data)
{
	...
	tsp_info("%s: cmd=%s, attached_dev=%d, cable_type=%d\n",
...
static int __init ist30xx_notifier_init(void)
{
	if (ts_data != NULL) {
		tsp_info("%s\n", __func__);
		muic_notifier_register(&ts_data->muic_nb,
				       tsp_ta_notification,
				       MUIC_NOTIFY_DEV_TSP);
	}
	return 0;
}
device_initcall_sync(ist30xx_notifier_init);
#endif
...
static void debug_work_func(struct work_struct *work)
{
	...
	if (!buf32) {
		tsp_err("%s: failed to alloc memory\n", __func__);
		return;
	}
...
#endif
#ifdef CONFIG_MUIC_NOTIFIER
	muic_notifier_unregister(&data->muic_nb);
#endif
	ist30xx_disable_irq(data);
	free_irq(client->irq, data);
	ist30xx_power_off(data);
...
static int __init ist30xx_init(void)
{
	...
	if (poweroff_charging) {
		tsp_noti("%s LPM Charging Mode!!\n", __func__);
		return 0;
	}
#endif

	if (get_lcd_attached("GET") == 0xFFFFFF) {
		tsp_noti("%s: LCD is not attached\n", __func__);
		return 0;
	}
