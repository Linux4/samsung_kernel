...
static void get_intensity_all_data(void *device_data)
{
	...
	struct mms_ts_info *info = container_of(sec, struct mms_ts_info, sec);
	int ret;
	int length = (int)strlen(info->print_buf);
...
static void spay_enable(void *device_data)
{
	...
	*/
	...
	if (sec->cmd_param[0]) {
		...
		info->lowpower_flag =
		    info->lowpower_flag & ~(MMS_LPM_FLAG_SPAY);
...
static void get_aod_rect(void *device_data)
{
	...
	for (i = 0; i < 4; i++)
		rect_data[i] =
		    (rbuf[i * 2 + 1] & 0xFF) << 8 | (rbuf[i * 2] & 0xFF);
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	snprintf(buff, sizeof(buff), "%d %d %d", info->scrub_id, info->scrub_x,
		 info->scrub_y);
