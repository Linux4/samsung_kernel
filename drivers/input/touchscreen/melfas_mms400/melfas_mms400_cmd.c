					...
					const char *buf, size_t count)
{
	...
	if (count >= CMD_LEN) { 
		tsp_debug_err(true, &info->client->dev, "%s: overflow command length\n", __func__); 
		return -EINVAL; 
	} 
...
static DEVICE_ATTR(cmd, 0660, NULL, mms_sys_cmd);
...
static DEVICE_ATTR(cmd_status, 0660, mms_sys_cmd_status, NULL);
...
static DEVICE_ATTR(cmd_result, 0660, mms_sys_cmd_result, NULL);
...
static DEVICE_ATTR(cmd_list, 0660, mms_sys_cmd_list, NULL);
