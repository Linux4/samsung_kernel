...
/**
 * Check connection for ito test
 */
static int check_connection_ito(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct mms_ts_info *info = container_of(sec, struct mms_ts_info, sec);
	char buf[64] = { 0 };
...
int mms_panel_ito_test(void *device_data) {
	...
	//code to check HW panel is defective or not and update the ito_test values
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct mms_ts_info *info = container_of(sec, struct mms_ts_info, sec);
	int bOpen;
	
	// open
	bOpen = check_connection_ito(sec);
	input_info(true, &info->client->dev, "%s - bOpen %d\n", __func__, bOpen);
	printk("result: %d\n", bOpen);
...
void mms_read_info_work(struct work_struct *work)
{
	...
	struct sec_cmd_data *sec = 	container_of(work, struct sec_cmd_data,
						work_read_info.work);
						
	cmd_run_test_cm_abs(sec);
	cmd_run_test_cm_delta(sec);
	ret = mms_panel_ito_test(sec);
	if (ret == 1)
		printk("mms_panel_ito_test: %s\n", "OK");
	else
		printk("mms_panel_ito_test: %s\n", "FAIL");
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, SEC_CMD_BUF_SIZE, "%02X",
		info->ito_test[0]);
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	snprintf(buff, sizeof(buff), "%d", info->wet_count);
	info->wet_count = 0;
