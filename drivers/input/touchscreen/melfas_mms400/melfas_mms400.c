...
#if 0 //defined(CONFIG_BATTERY_SAMSUNG)
	if (lpcharge == 1) {
...
static int mms_init_config(struct mms_ts_info *info)
{
	...
	temp_x = (rbuf[0]) | (rbuf[1] << 8);
	temp_y = (rbuf[2]) | (rbuf[3] << 8);
	info->max_x = info->dtdata->max_x;
	info->max_y = info->dtdata->max_y;
	tsp_debug_info(true, &info->client->dev, "%s max x[%d] y[%d], read x[%d] y[%d]\n",
		__func__, info->max_x, info->max_y, temp_x, temp_y);
...
static int mms_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	info->dtdata->vdd_io_vreg = regulator_get(&client->dev, "vddio");
	if (IS_ERR(info->dtdata->vdd_io_vreg)){
		info->dtdata->vdd_io_vreg = NULL;
		tsp_debug_err(true, &client->dev, "info->dtdata->vdd_io_vreg get error, ignoring\n");
	} else {
		regulator_set_voltage(info->dtdata->vdd_io_vreg, 1800000, 1800000);

		ret = regulator_enable(info->dtdata->vdd_io_vreg);	// always on
		if (ret) {
