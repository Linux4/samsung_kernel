...
/*end*/
#ifdef MMS_TOUCH_COMPATIBLE
#include <linux/string.h>
#define MMS_PRODUCT_NAME "NEWJ7_FHD"
static unsigned int force_load_mms_driver;
#endif
...
static void secure_touch_clk_unprepare_disable(struct mms_ts_info *info)
{
	clk_disable_unprepare(info->core_clk);
...
void mms_reboot(struct mms_ts_info *info)
{
	...
	usleep_range(9000, 10000);	/*replace msleep(10)*/
...
void dump_tsp_log(void)
{
	...
	if (poweroff_charging) {
		pr_info("%s, ignored ## lpm charging Mode!!\n", __func__);
		return;
	}
#endif
	if (p_ghost_check == NULL) {
		pr_info("%s, ignored ## tsp probe fail!!\n", __func__);
		return;
	}
 ...
 */
#ifdef MMS_TOUCH_COMPATIBLE
static int mms_read_chip_information(struct mms_ts_info *info)
{
	u8 wbuf[8];
	u8 rbuf[32];
	int ret = 0;
	...
	for (i = 0; i < sizeof(rbuf); i++)
		pr_info("%s rbuf[%d]=0x%x\n", __func__, i, rbuf[i]);
	memcpy(info->product_name, rbuf, 16);
	pr_info("%s - product_name[%s],MMS_PRODUCT_NAME[%s]\n",
		__func__, info->product_name, MMS_PRODUCT_NAME);
	if (strncmp(info->product_name, MMS_PRODUCT_NAME, 16)) {
		pr_info("%s - product_name check failed![%s]!=[%s]\n",
			__func__, info->product_name, MMS_PRODUCT_NAME);
		return -EFAULT;
	}
	return 0;
...
#ifdef MMS_TOUCH_COMPATIBLE
	/*add for compatible*/
	unsigned int i2c_real_address = client->addr;
	/*end*/
#endif
...
/*add for touchscreen compatible by wxj in 2017.3.21*/
	if (touchscreen_load_status_check() > 0) {
		pr_info("%s have another touchscreen load success!\n", __func__);
		return -EEXIST;
	}
/*end*/
...
#ifdef MMS_TOUCH_COMPATIBLE
	/*add for compatible*/
		if (!of_property_read_u32(client->dev.of_node, "i2c_real_address",
			&i2c_real_address)) {
			pr_info("%s i2c_real_address = 0x%x\n", __func__, i2c_real_address);
			if ((i2c_real_address > 0) && (i2c_real_address <= 0x7f))
				client->addr = i2c_real_address;
		}
		ret = of_property_read_u32(client->dev.of_node, "force_load_mms_driver",
			&force_load_mms_driver);
		pr_info("%s force_load_mms_driver = %u\n", __func__, force_load_mms_driver);
	/*end*/
#endif
	} else
#endif
...
#ifdef MMS_TOUCH_COMPATIBLE
	if (force_load_mms_driver == 0) {
		ret = mms_read_chip_information(info);
		if (ret < 0)
