...
void mms_input_event_handler(struct mms_ts_info *info, u8 sz, u8 *buf)
{
	...
	for (i = 1; i < sz; i += info->event_size) {
		...
		/* add defend code to avoid finger array break mms_ts_info,
		*at this case info->event_size will become 0 and for loop cause watchdog upload.
		*/
		if (id < 0) {
			input_err(true, &client->dev,
...
static struct mms_ts_info *tsp_driver;
