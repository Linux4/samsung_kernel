...
int mms_power_control(struct mms_ts_info *info, int enable)
{
	...
	} else if (!IS_ERR_OR_NULL(info->dtdata->vreg_vdd)) {
		/* using regulator */
		ret = regulator_set_voltage(info->dtdata->vreg_vdd, 3000000, 3000000);
		
		if (enable) {
			ret = regulator_enable(info->dtdata->vreg_vdd);
			if (ret)
				dev_err(&client->dev,
						"%s [ERROR] regulator_enable [%d]\n",
						__func__, ret);
		} else {
			ret = regulator_disable(info->dtdata->vreg_vdd);
			if (ret)
				dev_err(&client->dev,
	...
	if (gpio_is_valid(info->dtdata->gpio_vdd))
		...
		pr_cont(" info->dtdata->gpio_vdd:%d", regulator_is_enabled(info->dtdata->vreg_vdd));
	if (gpio_is_valid(info->dtdata->gpio_vio))
...
int mms_parse_devicetree(struct device *dev, struct mms_ts_info *info)
{
	...
	if (gpio_is_valid(info->dtdata->gpio_vdd)) {
		gpio_request(info->dtdata->gpio_vdd, "melfas_vdd_gpio");
	 	...
	 	if (of_property_read_string(np, "melfas,vdd_en", &vdd)) {
			dev_err(dev,  "Failed to get regulator_dvdd name property\n");
		} else {
			info->dtdata->vreg_vdd = regulator_get(NULL, vdd);
			if (IS_ERR(info->dtdata->vreg_vdd))
