/*
 *
 * Zinitix bt531 touchscreen driver
 *
...
#define SUPPORTED_REGULATOR_POWER_CONTROL
...
#include "zinitix_touch_bt531_firmware.h"
...
#define MAX_RAW_DATA_SZ				448 /* 28x16*///576 /* 32x18 */
...
/* Register Map*/
#define BT531_SWRESET_CMD					0x0000
#define BT531_WAKEUP_CMD					0x0001

#define BT531_IDLE_CMD						0x0004
#define BT531_SLEEP_CMD						0x0005

#define BT531_CLEAR_INT_STATUS_CMD			0x0003
#define BT531_CALIBRATE_CMD					0x0006
#define BT531_SAVE_STATUS_CMD				0x0007
#define BT531_SAVE_CALIBRATION_CMD			0x0008
#define BT531_RECALL_FACTORY_CMD			0x000f

#define BT531_THRESHOLD						0x0020

#define BT531_DEBUG_REG						0x0115 /* 0~7 */

#define BT531_TOUCH_MODE					0x0010
#define BT531_CHIP_REVISION					0x0011
#define BT531_FIRMWARE_VERSION				0x0012

#define BT531_MINOR_FW_VERSION				0x0121

#define BT531_VENDOR_ID						0x001C
#define BT531_HW_ID							0x0014

#define BT531_DATA_VERSION_REG				0x0013
#define BT531_SUPPORTED_FINGER_NUM			0x0015
#define BT531_EEPROM_INFO					0x0018
#define BT531_INITIAL_TOUCH_MODE			0x0019

#define BT531_TOTAL_NUMBER_OF_X				0x0060
#define BT531_TOTAL_NUMBER_OF_Y				0x0061

#define BT531_DELAY_RAW_FOR_HOST			0x007f

#define BT531_BUTTON_SUPPORTED_NUM			0x00B0
#define BT531_BUTTON_SENSITIVITY			0x00B2
#define BT531_DUMMY_BUTTON_SENSITIVITY		0X00C8

#define BT531_X_RESOLUTION					0x00C0
#define BT531_Y_RESOLUTION					0x00C1

#define BT531_POINT_STATUS_REG				0x0080
#define BT531_ICON_STATUS_REG				0x00AA

#define BT531_AFE_FREQUENCY					0x0100
#define BT531_DND_N_COUNT					0x0122
#define BT531_DND_U_COUNT					0x0135

#define BT531_RAWDATA_REG					0x0200

#define BT531_EEPROM_INFO_REG				0x0018

#define BT531_INT_ENABLE_FLAG				0x00f0
#define BT531_PERIODICAL_INTERRUPT_INTERVAL	0x00f1

#define BT531_BTN_WIDTH						0x016d

#define BT531_CHECKSUM_RESULT				0x012c

#define BT531_INIT_FLASH					0x01d0
#define BT531_WRITE_FLASH					0x01d1
#define BT531_READ_FLASH					0x01d2
...
struct bt531_ts_info {
	struct i2c_client				*client;
	struct input_dev				*input_dev;
	struct bt531_ts_platform_data	*pdata;
	char							phys[32];
...
u8* m_firmware_data;
#ifdef SUPPORTED_REGULATOR_POWER_CONTROL
static struct regulator *touch_regulator =  NULL;
#if defined(CONFIG_MACH_CS02)
#define TOUCH_REGULATOR_NAME "v_tsp_3v3"
#define TOUCH_REGULATOR_VOLTAGE 3300000
#else
#define TOUCH_REGULATOR_NAME NULL
#define TOUCH_REGULATOR_VOLTAGE 0
#endif
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void bt531_ts_early_suspend(struct early_suspend *h);
static void bt531_ts_late_resume(struct early_suspend *h);
#endif

static bool bt531_power_control(struct bt531_ts_info *info, u8 ctl);

static bool init_touch(struct bt531_ts_info *info, bool force);
static bool mini_init_touch(struct bt531_ts_info *info);
static void clear_report_data(struct bt531_ts_info *info);
static void esd_timer_start(u16 sec, struct bt531_ts_info *info);
static void esd_timer_stop(struct bt531_ts_info *info);
static void esd_timer_init(struct bt531_ts_info *info);
...
struct bt531_ts_info *misc_info;

static bool get_raw_data(struct bt531_ts_info *info, u8 *buff, int skip_cnt)
{
	struct i2c_client *client = info->client;
	struct bt531_ts_platform_data *pdata = info->pdata;
	...
	for(i = 0; i < skip_cnt; i++) {
		...
		write_cmd(client, BT531_CLEAR_INT_STATUS_CMD);
		msleep(1);
	}
	...
	if (read_raw_data(client, BT531_RAWDATA_REG, (char *)buff, sz) < 0) {
...
static bool ts_get_raw_data(struct bt531_ts_info *info)
{
	struct i2c_client *client = info->client;
	u32 total_node = info->cap_info.total_node_num;
	u32 sz;
	...
	if (read_raw_data(info->client, BT531_RAWDATA_REG,
...
static bool i2c_checksum(struct bt531_ts_info *info, s16 *pChecksum, u16 wlength)
{
	s16 checksum_result;
	s16 checksum_cur;
	int i;
...
#endif
static bool ts_read_coord(struct bt531_ts_info *info)
{
	struct bt531_ts_platform_data *pdata = info->pdata;
	struct i2c_client *client = info->client;
#if (TOUCH_POINT_MODE == 1)
	...
	if(info->cap_info.i2s_checksum)
		if (write_reg(info->client,
			...
			return false;
#endif
	if (read_data(info->client, BT531_POINT_STATUS_REG,
	...
	for (i = 0; i < info->cap_info.multi_fingers; i++) {
		if (zinitix_bit_test(info->touch_info.event_flag, i)) {
			...
			if (read_data(info->client, BT531_POINT_STATUS_REG + 2 + ( i * 4),
...
out:
	...
	write_cmd(info->client, BT531_CLEAR_INT_STATUS_CMD);
...
static void esd_timeout_handler(unsigned long data)
{
	struct bt531_ts_info *info = (struct bt531_ts_info *)data;
...
{
	struct bt531_ts_info *info =
				container_of(work, struct bt531_ts_info, tmr_work);
	struct i2c_client *client = info->client;
	...
	disable_irq(info->irq);
	bt531_power_control(info, POWER_OFF);
	bt531_power_control(info, POWER_ON_SEQUENCE);
...
static bool bt531_power_sequence(struct bt531_ts_info *info)
{
	struct i2c_client *client = info->client;
	int retry = 0;
...
static bool bt531_power_control(struct bt531_ts_info *info, u8 ctl)
{
#if !defined(SUPPORTED_REGULATOR_POWER_CONTROL)
	struct bt531_ts_platform_data *pdata = info->pdata;
	...
	if (ctl == POWER_OFF) {
		...
		if (ctl == POWER_ON_SEQUENCE) {
			...
			return bt531_power_sequence(info);
		}
	}
	...
	if(touch_regulator == NULL) {
		touch_regulator = regulator_get(NULL,TOUCH_REGULATOR_NAME);
		if (IS_ERR(touch_regulator)) {
			touch_regulator = NULL;
			printk("get touch_regulator regulator error\n");
			return false;
		}
	}
	if (ctl == POWER_OFF) {
		if (is_power_on) {
			...
			if (ret) {
				is_power_on = 1;
				printk(KERN_ERR "%s: touch_regulator disable failed (%d)\n",__func__, ret);
			}
		}
	}
	else
	{
		if (!is_power_on) {
			is_power_on = 1;
			regulator_set_voltage(touch_regulator, TOUCH_REGULATOR_VOLTAGE, TOUCH_REGULATOR_VOLTAGE);
			ret = regulator_enable(touch_regulator);
			if (ret) {
				is_power_on = 0;
				printk(KERN_ERR "%s: touch_regulator enable failed (%d)\n",__func__, ret);
			}
		}
		pr_info("[TSP] %s, expected power[%d], actural power[%d]\n", __func__, ctl, is_power_on);
		if (ctl == POWER_ON_SEQUENCE) {
			msleep(CHIP_ON_DELAY);
...
static bool ts_check_need_upgrade(struct bt531_ts_info *info,
...
static u8 ts_upgrade_firmware(struct bt531_ts_info *info,
...
retry_upgrade:
	bt531_power_control(info, POWER_OFF);
	bt531_power_control(info, POWER_ON);
	mdelay(100);
	...
	if (write_cmd(client, BT531_INIT_FLASH) != I2C_SUCCESS) {
	...
	for (flash_addr = 0; flash_addr < size; ) {
		for (i = 0; i < pdata->page_size / TC_SECTOR_SZ; i++) {
			...
			if (write_data(client, BT531_WRITE_FLASH,
	...
	for (flash_addr = 0; flash_addr < size; ) {
		for (i = 0; i < pdata->page_size / TC_SECTOR_SZ; i++) {
			/*zinitix_debug_msg("read :addr=%04x, len=%d\n",
								flash_addr, TC_SECTOR_SZ);*/
			if (read_firmware_data(client, BT531_READ_FLASH,
...
static bool ts_hw_calibration(struct bt531_ts_info *info)
{
	struct i2c_client *client = info->client;
	u16	chip_eeprom_info;
	int time_out = 0;

	if (write_reg(client, BT531_TOUCH_MODE, 0x07) != I2C_SUCCESS)
		return false;
	...
	if (write_cmd(client, BT531_CALIBRATE_CMD) != I2C_SUCCESS)
		return false;

	if (write_cmd(client, BT531_CLEAR_INT_STATUS_CMD) != I2C_SUCCESS)
		return false;
	...
	do {
		...
		if (read_data(client, BT531_EEPROM_INFO_REG,
		...
		if(time_out++ == 4){
			write_cmd(client, BT531_CALIBRATE_CMD);
			mdelay(10);
			write_cmd(client, BT531_CLEAR_INT_STATUS_CMD);
	...
	if (write_reg(client, BT531_TOUCH_MODE, TOUCH_POINT_MODE) != I2C_SUCCESS)
		return false;

	if (info->cap_info.ic_int_mask != 0) {
		if (write_reg(client, BT531_INT_ENABLE_FLAG,
	...
	udelay(100);
	if (write_cmd(client, BT531_SAVE_CALIBRATION_CMD) != I2C_SUCCESS)
		return false;
...
retry_init:
	...
	for (i = 0; i < INIT_RETRY_CNT; i++) {
		if (read_data(client, BT531_CHECKSUM_RESULT,
	...
	if (write_cmd(client, BT531_SWRESET_CMD) != I2C_SUCCESS)
		goto fail_init;
	...
	if (write_reg(client, BT531_INT_ENABLE_FLAG, 0x0) != I2C_SUCCESS)
		goto fail_init;
	...
	if (read_data(client, BT531_VENDOR_ID, (u8 *)&cap->vendor_id, 2) < 0)
		goto fail_init;


	if (read_data(client, BT531_CHIP_REVISION,
...
//	cap->ic_fw_size = 32 * 1024;
	...
	if (read_data(client, BT531_HW_ID, (u8 *)&cap->hw_id, 2) < 0)
		goto fail_init;

	if (read_data(client, BT531_THRESHOLD, (u8 *)&cap->threshold, 2) < 0)
		goto fail_init;

	if (read_data(client, BT531_BUTTON_SENSITIVITY,
					(u8 *)&cap->key_threshold, 2) < 0)
		goto fail_init;

//	if (read_data(client, BT531_DUMMY_BUTTON_SENSITIVITY,
//					(u8 *)&cap->dummy_threshold, 2) < 0)
//		goto fail_init;

	if (read_data(client, BT531_TOTAL_NUMBER_OF_X,
					(u8 *)&cap->x_node_num, 2) < 0)
		goto fail_init;

	if (read_data(client, BT531_TOTAL_NUMBER_OF_Y,
					(u8 *)&cap->y_node_num, 2) < 0)
		goto fail_init;

	cap->total_node_num = cap->x_node_num * cap->y_node_num;

	if (read_data(client, BT531_AFE_FREQUENCY,
	...
	if (read_data(client, BT531_FIRMWARE_VERSION,
					(u8 *)&cap->fw_version, 2) < 0)
		goto fail_init;

	if (read_data(client, BT531_MINOR_FW_VERSION,
					(u8 *)&cap->fw_minor_version, 2) < 0)
		goto fail_init;

	if (read_data(client, BT531_DATA_VERSION_REG,
	...
	if (!force) {
		if (ts_check_need_upgrade(info, cap->fw_version,
			...
			if (write_reg(client, BT531_INT_ENABLE_FLAG, 0) != I2C_SUCCESS)
				goto fail_init;
	...
	/* initialize */
	if (write_reg(client, BT531_X_RESOLUTION,
					(u16)pdata->x_resolution) != I2C_SUCCESS)
		goto fail_init;

	if (write_reg(client, BT531_Y_RESOLUTION,
	...
	if (write_reg(client, BT531_BUTTON_SUPPORTED_NUM,
					(u16)cap->button_num) != I2C_SUCCESS)
		goto fail_init;

	if (write_reg(client, BT531_SUPPORTED_FINGER_NUM,
	...
	if (write_reg(client, BT531_INITIAL_TOUCH_MODE,
					TOUCH_POINT_MODE) != I2C_SUCCESS)
		goto fail_init;

	if (write_reg(client, BT531_TOUCH_MODE, info->touch_mode) != I2C_SUCCESS)
		goto fail_init;
	...
	if (info->touch_mode != TOUCH_POINT_MODE) { /* Test Mode */
		if (write_reg(client, BT531_DELAY_RAW_FOR_HOST,
...
#if ESD_TIMER_INTERVAL
	if (write_reg(client, BT531_PERIODICAL_INTERRUPT_INTERVAL,
...
static irqreturn_t bt531_touch_irq_handler(int irq, void *data)
{
	struct bt531_ts_info* info = (struct bt531_ts_info*)data;
	struct bt531_ts_platform_data *pdata = info->pdata;
	struct i2c_client *client = info->client;
	...
	if (zinitix_bit_test(info->touch_info.status, BIT_ICON_EVENT)) {
		if (read_data(info->client, BT531_ICON_STATUS_REG,
...
static void bt531_ts_late_resume(struct early_suspend *h)
{
	struct bt531_ts_info *info = misc_info;
	//info = container_of(h, struct bt531_ts_info, early_suspend);
...
#ifdef CONFIG_PM
	write_cmd(info->client, BT531_WAKEUP_CMD);
	mdelay(1);
#else
	bt531_power_control(info, POWER_ON_SEQUENCE);
#endif
...
#if ESD_TIMER_INTERVAL
	/*write_reg(info->client, BT531_PERIODICAL_INTERRUPT_INTERVAL, 0);*/
	esd_timer_stop(info);
#endif

#ifdef CONIFG_PM
	write_reg(info->client, BT531_INT_ENABLE_FLAG, 0x0);

	udelay(100);
	if (write_cmd(info->client, BT531_SLEEP_CMD) != I2C_SUCCESS) {
...
static int bt531_ts_resume(struct device *dev)
{
	struct bt531_ts_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
...
static int bt531_ts_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bt531_ts_info *info = i2c_get_clientdata(client);
...
static bool ts_set_touchmode(u16 value){
	...
	if (value == TOUCH_DND_MODE) {
		if (write_reg(misc_info->client, BT531_DND_N_COUNT,
						SEC_DND_N_COUNT) != I2C_SUCCESS)
			dev_err(&misc_info->client->dev, "Failed to set DND_N_COUNT\n");

		if (write_reg(misc_info->client, BT531_AFE_FREQUENCY,
		...
		if (write_reg(misc_info->client, BT531_DND_U_COUNT,
	...
	if(misc_info->touch_mode != TOUCH_POINT_MODE) {
		if (write_reg(misc_info->client, BT531_DELAY_RAW_FOR_HOST,
						RAWDATA_DELAY_FOR_HOST) != I2C_SUCCESS)
			dev_err(&misc_info->client->dev, "%s: Failed to set"
						" DELAY_RAW_FOR_HOST\n", __func__);
	}

	if (write_reg(misc_info->client, BT531_TOUCH_MODE,
	...
	for(i=0; i < 10; i++) {
		mdelay(20);
		write_cmd(misc_info->client, BT531_CLEAR_INT_STATUS_CMD);
	}
...
#ifdef SEC_FACTORY_TEST
static inline void set_cmd_result(struct bt531_ts_info *info, char *buff, int len)
{
	strncat(info->factory_info->cmd_result, buff, len);
}

static inline void set_default_result(struct bt531_ts_info *info)
{
	char delim = ':';
...
static void fw_update(void *device_data)
{
	struct bt531_ts_info *info = (struct bt531_ts_info *)device_data;
	struct i2c_client *client = info->client;
...
#define BT531_VENDOR_NAME "ZINITIX"
...
static void get_chip_name(void *device_data)
{
	...
	snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", BT531_CHIP_NAME);
	set_cmd_result(info, finfo->cmd_buff,
					strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	finfo->cmd_state = OK;
...
static void get_tkey_delta(void *device_data)
{
	...
	ret = read_data(misc_info->client, BT531_BTN_WIDTH + btn_node, (u8*)&val, 2);
...
static void get_key_threshold(void *device_data)
{
	...
	ret = read_data(misc_info->client, BT531_BUTTON_SENSITIVITY, (u8*)&threshold, 2);
										 ...
										 char *buf)
{
	...
	ret = read_data(client, BT531_BTN_WIDTH + i, (u8*)&val, 2);
	if (ret < 0) {
...
static int init_sec_factory(struct bt531_ts_info *info)
{
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case TOUCH_IOCTL_HW_CALIBRAION:
		...
		if (write_reg(misc_info->client,
			BT531_TOUCH_MODE, mode) != I2C_SUCCESS) {
...
fail_hw_cal:
	...
	case TOUCH_IOCTL_DONOT_TOUCH_EVENT:
		...
		misc_info->work_state = SET_MODE;
		if (write_reg(misc_info->client,
			BT531_INT_ENABLE_FLAG, 0) != I2C_SUCCESS)
			ret = -1;
		zinitix_debug_msg("write : reg addr = 0x%x, val = 0x0\r\n",
			BT531_INT_ENABLE_FLAG);
	...
	case TOUCH_IOCTL_SEND_SAVE_STATUS:
		...
		if (write_cmd(misc_info->client,
			BT531_SAVE_STATUS_CMD) != I2C_SUCCESS)
			ret =  -1;
...
static int bt531_ts_probe(struct i2c_client *client,
		const struct i2c_device_id *i2c_id)
{
	struct i2c_adapter *adapter = to_i2c_adapter(client->dev.parent);
	struct bt531_ts_platform_data *pdata = client->dev.platform_data;
	struct bt531_ts_info *info;
	struct input_dev *input_dev;
	int ret = 0;
	int i;
	...
	info = kzalloc(sizeof(struct bt531_ts_info), GFP_KERNEL);
	if (!info) {
		dev_err(&client->dev, "%s: Failed to allocate memory\n", __func__);
	...
	ret = gpio_request(pdata->gpio_int, "bt531_int");
	if (ret < 0) {
		pr_err("bt531: Failed to obtain gpio for int\n");
		goto err_power_sequence;
	}
	gpio_direction_input(pdata->gpio_int);
	pr_info("bt531: initialize pins\n");

	// power on
	if (bt531_power_control(info, POWER_ON_SEQUENCE) == false) {
		ret = -EPERM;

		goto err_power_sequence;
	}

	if (read_data(info->client,0xcc00, (u8 *)&info->cap_info.chip_code, 2) < 0) {
		pr_info("fail to read chip code\n");
		goto err_power_sequence;
	}
	pr_info("chip code = 0x%x\n", info->cap_info.chip_code);
	if (info->cap_info.chip_code == 0xf400) {
		m_firmware_data = m_firmware_data_531;
		pdata->page_size = 128;
...
/*	input_set_abs_params(info->input_dev, ABS_MT_WIDTH_MINOR,
	...
	ret = request_threaded_irq(info->irq, NULL, bt531_touch_irq_handler,
		IRQF_TRIGGER_FALLING | IRQF_ONESHOT , BT531_TS_DEVICE, info);
	...
	info->early_suspend.suspend = bt531_ts_early_suspend;
	info->early_suspend.resume = bt531_ts_late_resume;
...
static int bt531_ts_remove(struct i2c_client *client)
{
	struct bt531_ts_info *info = i2c_get_clientdata(client);
	struct bt531_ts_platform_data *pdata = info->pdata;
...
static struct i2c_device_id bt531_idtable[] = {
	{BT531_TS_DEVICE, 0},
	{ }
};

#if defined(CONFIG_PM) && !defined(CONFIG_HAS_EARLYSUSPEND)
static const struct dev_pm_ops bt531_ts_pm_ops = {
#if defined(CONFIG_PM_RUNTIME)
	SET_RUNTIME_PM_OPS(bt531_ts_suspend, bt531_ts_resume, NULL)
#else
	SET_SYSTEM_SLEEP_PM_OPS(bt531_ts_suspend, bt531_ts_resume)
#endif
};
#endif

static struct i2c_driver bt531_ts_driver = {
	.probe	= bt531_ts_probe,
	.remove	= bt531_ts_remove,
	.id_table	= bt531_idtable,
	.driver		= {
		.owner	= THIS_MODULE,
		.name	= BT531_TS_DEVICE,
#if defined(CONFIG_PM) && !defined(CONFIG_HAS_EARLYSUSPEND)
		.pm		= &bt531_ts_pm_ops,
#endif
	},
...
static int __devinit bt531_ts_init(void)
{
	printk("[TSP]%s\n",__func__);
#if defined(CONFIG_SPA) || defined(CONFIG_SPA_LPM_MODE)
	...
	if (!lpcharge)
#endif
		return i2c_add_driver(&bt531_ts_driver);
	else
		return 0;
}

static void __exit bt531_ts_exit(void)
{
	i2c_del_driver(&bt531_ts_driver);
}

module_init(bt531_ts_init);
module_exit(bt531_ts_exit);
