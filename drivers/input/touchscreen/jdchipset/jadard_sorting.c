...
extern char* cpt_lcdname;
extern char* hlt_lcdname;
...
static int jadard_parse_sorting_threshold_file(void)
{
	...
	if(lcd_name){
		if (strncmp(cpt_lcdname, lcd_name, strlen(cpt_lcdname)) == 0){
			file_name = "cpt_jd_sorting_threshold.txt";
		}
		else if(strncmp(hlt_lcdname, lcd_name, strlen(hlt_lcdname)) == 0){
			file_name = "hlt_jd_sorting_threshold.txt";
		}
	}
	/*HS03 code for SR-SL6215-01-90 by chenyihong at 20210817 end*/
...
static int jadard_read_sorting_threshold(void)
{
	...
	/*HS03 code for SR-SL6215-01-86 by chenyihong at 20210813 start*/
	if(lcd_name){
		if (strncmp(cpt_lcdname, lcd_name, strlen(cpt_lcdname)) == 0){
			sprintf(jd_g_file_path, "%s%s.txt", JD_RSLT_OUT_PATH, CPT_JD_RSLT_OUT_FILE);
		}
		else if(strncmp(hlt_lcdname, lcd_name, strlen(hlt_lcdname)) == 0){
			sprintf(jd_g_file_path, "%s%s.txt", JD_RSLT_OUT_PATH, HLT_JD_RSLT_OUT_FILE);
		}
	}
	/*HS03 code for SR-SL6215-01-86 by chenyihong at 20210813 end*/
