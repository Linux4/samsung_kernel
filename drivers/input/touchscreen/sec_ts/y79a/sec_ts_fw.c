...
static int sec_ts_load_fw(struct sec_ts_data *ts, int update_type, const char *file_path)
{
	fw_header *fw_hd;
	struct file *fp;
	mm_segment_t old_fs;
...
int sec_ts_firmware_update_on_hidden_menu(struct sec_ts_data *ts, int update_type)
{
	...
	case TSP_SDCARD:
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
		ret = sec_ts_load_fw(ts, update_type, TSP_PATH_EXTERNAL_FW);
#else
		ret = sec_ts_load_fw(ts, update_type, TSP_PATH_EXTERNAL_FW_SIGNED);
#endif
		break;
	case TSP_SPU:
	case TSP_VERIFICATION:
		ret = sec_ts_load_fw(ts, update_type, TSP_PATH_SPU_FW_SIGNED);
		break;
	default:
