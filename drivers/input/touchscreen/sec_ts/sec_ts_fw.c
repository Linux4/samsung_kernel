...
static int sec_ts_check_firmware_version(struct sec_ts_data *ts, const u8 *fw_info)
{
	...
	tsp_debug_info(true, &ts->client->dev,
				...
				__func__, fw_ver[0], fw_ver[1], fw_ver[2], fw_ver[3], fw_hd->img_ver);
	...
	ts->plat_data->img_version_of_bin[0] = (fw_hd->img_ver && 0xFF);
	...
	tsp_debug_info(true, &ts->client->dev,
				"%s: [IC] firmware version info : %x.%x.%x.%x // [BIN] %08X\n",
				__func__, fw_ver[0], fw_ver[1], fw_ver[2], fw_ver[3], fw_hd->fw_ver);

#if 0 // jjlee for bring-up
	if (((fw_hd->fw_ver) & 0xff) != fw_ver[0]) {
		tsp_debug_err(true, &ts->client->dev,
					"%s: F/W product 0 is not equal: %x\n ",
					__func__, fw_ver[0]);
		return -1;
	}
	if (((fw_hd->fw_ver >> 8) & 0xff) != fw_ver[1]) {
		tsp_debug_err(true, &ts->client->dev,
					"%s: F/W project 1 is not equal : %x\n ",
					__func__, fw_ver[1]);
		return -1;
	}
#endif

	if ((((fw_hd->fw_ver >> 24) & 0xff) > fw_ver[3]) || (((fw_hd->fw_ver >> 16) & 0xff) > fw_ver[2]))
		return 1;

	ret = ts->sec_ts_i2c_read(ts, SEC_TS_READ_PARA_VERSION, para_ver, 4);
	if (ret < 0) {
		tsp_debug_info(true, &ts->client->dev,
					"%s: parameter version read error\n ", __func__);
		return -1;
	}

	tsp_debug_info(true, &ts->client->dev,
				"%s: [IC] parameter version info : %x.%x.%x.%x // [BIN] %08X\n ",
				__func__, para_ver[0], para_ver[1], para_ver[2], para_ver[3], fw_hd->para_ver);
	...
	if (((fw_hd->para_ver) & 0xff) != para_ver[0]) {
		tsp_debug_err(true, &ts->client->dev,
					"%s: Parameter product 0 is not equal: %x\n ",
					__func__, para_ver[0]);
		return -1;
	}
	...
	if (((fw_hd->para_ver >> 8) & 0xff) != para_ver[1]) {
		tsp_debug_err(true, &ts->client->dev,
					"%s: Parameter project 1 is not equal: %x\n ",
					__func__, para_ver[1]);
		return -1;
	}
#endif
	if ((((fw_hd->para_ver >> 24) & 0xff) > para_ver[3]) || (((fw_hd->para_ver >> 16) & 0xff) > para_ver[2]))
		return 1;
...
static int sec_ts_chunk_update(struct sec_ts_data *ts, u32 addr, u32 size, u8 *data)
{
	...
	mem_data = kzalloc(fw_size4, GFP_KERNEL);
	if (!mem_data) {
...
/*
 * I2C read buffer is 256 byte. do not support this verify routine
 * below code is read 1024 byte.
	...
	tsp_debug_info(true, &ts->client->dev, "%s verify done\n", __func__);
*/

//TEST
//	sec_ts_memoryread(ts, addr, mem_rb, fw_size4);
//TEST
...
int sec_ts_firmware_update_on_hidden_menu(struct sec_ts_data *ts, int update_type)
{
	...
	case UMS:
		...
		break;
	case FFU:
		tsp_debug_err(true, ts->dev, "%s: Not support yet\n", __func__);
		break;
	default:
