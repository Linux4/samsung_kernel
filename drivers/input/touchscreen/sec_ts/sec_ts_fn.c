...
/*
	...
	if (data->cmd_is_running == true) {
		tsp_debug_err(true, &ts->client->dev, "%s: Still servicing previous command. Skip cmd :%s\n", __func__, buf);
		return count;
	}
*/
/*
		...
		short *max)
{
	...
	/* Select rawdata type */
	/* SEC_TS_CMD_MUTU_RAWDATA_TYPE */
	ret = ts->sec_ts_i2c_write(ts, 0xf4, &type, 1);
	if (ret < 0) {
...
static void get_fw_ver_bin(void *device_data)
{
	...
	if (strncmp(ts->plat_data->model_name, "A7100", 5) == 0)
...
void set_smartcover_mode(struct sec_ts_data *ts, bool on)
{
	...
	if (on == 1) {
		ret = ts->sec_ts_write_reg(ts, regMon, 2);
		if (ret < 0)
			tsp_debug_err(true, &ts->client->dev, "%s mode on failed. ret: %d\n", __func__, ret);

	} else {
		ret = ts->sec_ts_write_reg(ts, regMoff, 2);
		if (ret < 0)
...
void set_smartcover_clear(struct sec_ts_ts *ts)
{
	...
	ret = ts->sec_ts_write_reg(ts, regClr, 6);
	if (ret < 0)
...
void set_smartcover_data(struct sec_ts_data *ts)
{
	...
	for (i = 0; i < MAX_TX; i++) {
		...
		/* data write */
		ret = ts->sec_ts_write_reg(info, regData, 6);
		if (ret < 0)
