...
int sec_ts_wait_for_ready(struct sec_ts_data *ts)
{
	int rc = -1;
	int retry = 0;
	...
	while(sec_ts_i2c_read(ts, SEC_TS_READ_ONE_EVENT, tBuff, SEC_TS_EVENT_BUFF_SIZE)) {
		if (tBuff[0] == 0x01) {
			rc = 0;
			tsp_debug_info(true, &ts->client->dev, "%s: tBuff=%02X, rc=%d\n", __func__, tBuff[0],rc);
			break;
	}
...
#ifdef SEC_TS_SUPPORT_STRINGLIB
			if((p_event_status->eid == 0x00) && (p_event_status->tchsta == 0x01)&& (p_event_status->sid == SEC_TS_SID_GESTURE)) {
				struct sec_ts_gesture_status *p_gesture_status = (struct sec_ts_gesture_status *)read_event_buff;
				int x, y, w, h;

				x = p_gesture_status->x;
				y = (p_gesture_status->y_4_2 << 2) | p_gesture_status->y_1_0;
				w = p_gesture_status->w;
				h = (p_gesture_status->h_4 << 4) | p_gesture_status->h_3_0;

				tsp_debug_info(true, &ts->client->dev, "%s: %s, X:%d, Y:%d, W:%d, H:%d\n", __func__,
				...
				ts->scrub_id = 0x08;//-> temp value : knock knock event value is 8
...
//TEST]
		if (coordinate->action == SEC_TS_COORDINATE_ACTION_PRESS) {
				...
				tsp_debug_info(true, &ts->client->dev,
					"%s: [P] tID:%d, x:%d, y:%d, w:%d, z:%d, tc:%d\n",
					__func__, t_id, coordinate->x, coordinate->y, coordinate->touch_width, p_event_coord->z, ts->touch_count);
...
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
				tsp_debug_info(true, &ts->client->dev,
					"%s: [R] tID:%d mc: %d tc:%d lx:%d ly:%d ver:%02X.%02X\n",
					__func__, t_id, coordinate->mcount, ts->touch_count, coordinate->x, coordinate->y,
					 ts->plat_data->img_version_of_ic[2],ts->plat_data->img_version_of_ic[3]);
#else
			tsp_debug_info(true, &ts->client->dev,
					"%s: [R] tID:%d mc: %d tc:%d ver:%02X.%02X\n",
