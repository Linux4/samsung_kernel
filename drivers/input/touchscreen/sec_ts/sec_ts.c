...
static DEVICE_ATTR(sec_ts_bulk,(S_IWUSR | S_IWGRP), NULL, sec_ts_bulk_store);
static DEVICE_ATTR(sec_ts_fw, (S_IWUSR | S_IWGRP), NULL, sec_ts_fw_store);
static DEVICE_ATTR(sec_ts_setreadsize, (S_IWUSR | S_IWGRP), NULL, sec_ts_setreadsize_store);
static DEVICE_ATTR(sec_ts_setlv1cmd, (S_IWUSR | S_IWGRP), NULL, sec_ts_setlv1cmd_store);
static DEVICE_ATTR(sec_ts_enter_recovery, (S_IWUSR | S_IWGRP), NULL, sec_ts_enter_recovery_store);
static DEVICE_ATTR(sec_ts_id, S_IRUGO, sec_ts_id_show, NULL);
static DEVICE_ATTR(sec_ts_fw_ver, S_IRUGO, sec_ts_fw_ver_show, NULL);
static DEVICE_ATTR(sec_ts_status, S_IRUGO, sec_ts_status_show, NULL);
static DEVICE_ATTR(sec_ts_raw, S_IRUGO, sec_ts_raw_show, NULL);
static DEVICE_ATTR(sec_ts_selfraw, S_IRUGO, sec_ts_selfraw_show, NULL);
static DEVICE_ATTR(sec_ts_read_lv3, S_IRUGO, sec_ts_read_lv3_show, NULL);
static DEVICE_ATTR(sec_ts_raw_channel, S_IRUGO, sec_ts_raw_channel_show, NULL);
static DEVICE_ATTR(sec_ts_regread, S_IRUGO, sec_ts_regread_show, NULL);
static DEVICE_ATTR(sec_ts_touchtype, S_IRUGO, sec_ts_touchtype_show, NULL);
static DEVICE_ATTR(sec_ts_gesture_status, S_IRUGO, sec_ts_gesture_status_show, NULL);
static DEVICE_ATTR(sec_ts_dd_version, S_IRUGO, sec_ts_dd_version_show, NULL);
static DEVICE_ATTR(sec_ts_read_bulk, S_IRUGO, sec_ts_read_bulk_show, NULL);
static DEVICE_ATTR(sec_ts_read_eventtime, S_IRUGO, sec_ts_read_eventtime_show, NULL);
...
int sec_ts_i2c_read(struct sec_ts_data * ts, u8 reg, u8 * data, int len)
{
	...
	for (retry = 0; retry < 3; retry++) {
		...
		tsp_debug_err(true, &ts->client->dev, "%s: I2C retry %d, ret:%d\n", __func__, retry + 1, ret);
		...
		unsigned short *reg, unsigned char *data, int length)
{
	...
	if (ts->digital_rev == FTS_DIGITAL_REV_1) {
		return sec_ts_i2c_read(ts, string_reg, 3, data, length);
	} else {
		int rtn;
		...
		rtn = sec_ts_i2c_read(ts, string_reg, 3, buf, length + 1);
		if (rtn >= 0)
...
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
				tsp_debug_info(true, &ts->client->dev,
						"%s: [R] tID:%d %02X%02X, mc: %d tc:%d lx:%d ly:%d\n",
						__func__, t_id, ts->plat_data->img_version_of_ic[2], ts->plat_data->img_version_of_ic[3], 
						ts->coord[t_id].mcount, ts->touch_count, ts->coord[t_id].x, ts->coord[t_id].y);
#else
				tsp_debug_info(true, &ts->client->dev,
						"%s: [R] tID:%d %02X%02X, mc: %d tc:%d %d\n",
						__func__, t_id, ts->plat_data->img_version_of_ic[2], ts->plat_data->img_version_of_ic[3], 
...
#if 1//def CONFIG_TOUCHSCREEN_SEC_TS_GLOVEMODE
...
static ssize_t sec_ts_raw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	read_raw_buff = kzalloc(sizeof(u8) * READ_RAW_DATA_SIZE, GFP_KERNEL);
	if (!read_raw_buff) {
		tsp_debug_err(true, &ts->client->dev, "sec_ts_raw_show kzalloc failed\n");
		return 0;
	}
...
static ssize_t sec_ts_selfraw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	read_raw_buff = kzalloc(sizeof(u8) * READ_SELF_RAW_DATA_SIZE, GFP_KERNEL);
	if (!read_raw_buff) {
		tsp_debug_err(true, &ts->client->dev, "sec_ts_selfraw_show kzalloc failed\n");
		return 0;
	}
	disable_irq(ts->client->irq);
...
#if 0
	if (lv1_readremain == 0) {
		read_lv1_buff = kzalloc(sizeof(u8) * lv1_readsize, GFP_KERNEL);
		if (!read_lv1_buff) {
...
static int sec_ts_power_ctrl(struct sec_ts_data *ts, bool enable)
{
	...
	int retval;
	static struct regulator *vddo; //*avdd;

	dev_err(dev, "%s: %d\n", __func__, enable);

#if 0	// not use
	if (!avdd) {
		avdd = devm_regulator_get(&ts->client->dev, "avdd");
	...
	if (!vddo) {
		vddo = devm_regulator_get(&ts->client->dev, "vddo");
...
/* QCOM */
	...
	tsp_debug_err(true, &client->dev, "%s: i2c buffer limit: %d\n", __func__, pdata->i2c_burstmax);
...
static int sec_ts_read_information(struct sec_ts_data *ts)
{
	...
	memset(data, 0x0, 9);
	ret = sec_ts_i2c_read(ts, SEC_TS_READ_SUB_ID, data, 3);
	if (ret < 0) {
		tsp_debug_err(true, &ts->client->dev,
...
/*
	if (((data[5] << 8) | data[6]) > 0)
		ts->plat_data->max_y = ((data[5] << 8) | data[6]);

	if (((data[7] << 8) | data[8]) > 0)
		ts->plat_data->max_x = ((data[7] << 8) | data[8]);
*/
	memset(data, 0x0, 1);
