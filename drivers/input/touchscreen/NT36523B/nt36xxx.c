 ...
 *
 * $Revision: 88000 $
 * $Date: 2021-09-08 14:26:46 +0800 (周三, 08 9月 2021) $
 *
...
//+bug752760,fangzhihua.wt,mod,20220517,fix doubleclick fail to wakeup sometimes
...
uint32_t SWRST_N8_ADDR = 0x03F0FE; //read from dtsi
uint32_t SPI_RD_FAST_ADDR = 0x03F310;  //read from dtsi
...
info_retry:
	...
	sprintf(Ctp_name,"LCE,NT36523B,FW:0x%02X",buf[1]);
	NVT_LOG("nvt_get_fw_ver Ctp_name is : %s\n",Ctp_name);
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	if (ret) {
		NVT_LOG("error reading novatek,swrst-n8-addr. ret=%d\n", ret);
		return ret;
	} else {
...
static void get_fw_ver(void *device_data)
{
	...
	ret = nvt_get_fw_info();
	if (ret) {
		NVT_ERR("nvt_get_fw_info failed.\n");
	}

	snprintf(buff, sizeof(buff), "%s,%s,FW:0x%02X","LCE","NT36523B",ts->fw_ver);
...
static void aot_enable(void *device_data)
{
	...
	sec_cmd_set_cmd_result(sec, buff, strnlen(buff, sizeof(buff)));
	sec_cmd_set_cmd_exit(sec);
	NVT_LOG( "%s: tp_gesture_flag =%d \n",__func__,tp_gesture_flag);
	NVT_LOG( "%s: %s,sec->cmd_state == %d\n", __func__, buff, sec->cmd_state);

}
static struct sec_cmd nvt_commands[] = {
...
//#if defined(CONFIG_DRM_PANEL)
	...
	panel_notifier_chain_register(&ts->panel_notif);
#endif
	...
	ret = disp_notifier_register(&ts->drm_panel_notif);
	if(ret < 0){
		//input_err(true, &ts->client->dev, "Failed to register drm notifier\n");
		NVT_LOG("Failed to register drm notifier\n");
...
#elif defined(CONFIG_DRM_PANEL)
	if (disp_notifier_unregister(&ts->drm_panel_notif))
...
//+extB P220411-00928,fangzhihua.wt, mod, 20220509,for reboot issue
	//nvt_irq_enable(false);
	if (ts->irq_enabled) {
...
#ifdef CONFIG_PANEL_NOTIFIER
static int nvt_panel_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
	...
	if (event == PANEL_EVENT_PWR_STATE) {
		if (*evdata == PANEL_STATE_DISABLE) {
                       nvt_ts_suspend(&ts->client->dev);
		} else if (*evdata == PANEL_STATE_ENABLE) {
...
static int nvt_drm_panel_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	NVT_LOG("event = %d DISPC_POWER_OFF = %d DISPC_POWER_ON = %d\n", event, DISPC_POWER_OFF, DISPC_POWER_ON);
