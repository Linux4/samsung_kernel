 ...
 *
 * $Revision: 85416 $
 * $Date: 2021-07-20 15:08:07 +0800 (周二, 20 7月 2021) $
 *
...
//+bug696971,fangzhihua.wt,add, 20220311,for TP openshort test result save
...
extern char touch_test_time[40];//bug696971,fangzhihua.wt,add, 20220311,for TP openshort test result save
...
static int32_t nvt_save_rawdata_to_csv(int32_t *rawdata, uint8_t x_ch, uint8_t y_ch, const char *file_path, uint32_t offset)
{
	...
	char file_path_with_time[128] = {0};//bug696971,fangzhihua.wt,add, 20220311,for TP openshort test result save
	printk("%s:++\n", __func__);
	...
	set_fs(KERNEL_DS);
	//+bug696971,fangzhihua.wt,add, 20220311,for TP openshort test result save
	snprintf(file_path_with_time, sizeof(file_path_with_time), "%s_%s.csv", file_path,touch_test_time);
	if (offset == 0)
		fp = filp_open(file_path_with_time, O_RDWR | O_CREAT | O_TRUNC, 0644);
	else
		fp = filp_open(file_path_with_time, O_RDWR | O_CREAT, 0644);
...
static void nvt_enable_open_test(void)
{
	...
	//read fw history
	if (buf[2] != 0x00) {
		NVT_LOG("Read back 2 bytes from offset EVENT_MAP_HOST_CMD NG: 0x%02X, 0x%02X\n", buf[1], buf[2]);
		nvt_read_fw_history(ts->mmap->MMAP_HISTORY_EVENT0);
		nvt_read_fw_history(ts->mmap->MMAP_HISTORY_EVENT1);
...
int nvt_factory_test(void)
{
	...
	NVT_LOG("--\n");
	if ((TestResult_FW_Rawdata == 0 ) && (TestResult_Noise == 0) && (TestResult_Short == 0) &&
				(TestResult_Open == 0)&& (TestResult_FW_CC == 0)&& (TestResult_FW_DiffMax== 0)&&(TestResult_FW_DiffMin== 0)){
				NVT_LOG("************All test pass!!!************");
				return	0;
			} else {
				NVT_LOG("************Test FAIL!!!************");
				NVT_LOG(" TestResult_FW_Rawdata = %d  TestResult_Noise = %d  TestResult_Short = %d  TestResult_Open = %d  TestResult_FW_CC = %d  TestResult_FW_DiffMax = %d  TestResult_FW_DiffMin = %d\n",TestResult_FW_Rawdata,TestResult_Noise,TestResult_Short,TestResult_Open,TestResult_FW_CC,TestResult_FW_DiffMax,TestResult_FW_DiffMin);
					return	-1;
			}
