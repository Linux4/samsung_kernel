...
char touch_test_time[40];  //bug696971,fangzhihua.wt,add, 20220311,for TP openshort test result save
static int openshort_test_proc_show(struct seq_file *m, void *v)
{
	...
	memset(touch_test_time,'\0',sizeof(touch_test_time));

	do_gettimeofday(&ktv);
	ktv.tv_sec -= sys_tz.tz_minuteswest * 60;
	rtc_time_to_tm(ktv.tv_sec, &rtc_tm);

	snprintf(touch_test_time, sizeof(touch_test_time),
...
/*proc init part*/
static int proc_node_init(void)
{
	if (!strstr(lcd_name,"lcd_nt36523b_lce_panda_video_fhdp"))
		return 0;
	proc_touchscreen_dir = proc_mkdir(PROC_TOUCHSCREEN_FOLDER , NULL);
	if (proc_touchscreen_dir == NULL)  {
	NVT_LOG(KERN_ERR "%s: proc_touchpanel_dir file create failed!\n", __func__);
	}else{
	...
	if(proc_ctp_openshort_test_file ==NULL){
		NVT_LOG(KERN_ERR "%s: proc_ctp_openshort_test_file create failed!\n", __func__);
		remove_proc_entry(PROC_TOUCHSCREEN_FOLDER, NULL);
	}else{
		NVT_LOG("%s:  proc_create PROC_CTP_OPENSHORT_TEST_FILE success",__func__);
		}
