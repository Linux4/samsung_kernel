 ...
 *
 * $Revision: 46615 $
 * $Date: 2019-06-26 09:30:40 +0800 (週三, 26 六月 2019) $
 *
...
#if NVT_TOUCH_MP_SEC
	if ((TestResult_Short == 0)	&& (TestResult_Open == 0) && (TestResult_FW_Rawdata == 0)&& (TestResult_Noise == 0)) {
		seq_printf(m, "1\n");
	} else {
		seq_printf(m, "0\n");
	}
					   ...
					   const struct firmware *fw)
{
	...
	mutex_unlock(&ts->lock);
	NVT_LOG("open_shaort_test: TestResult_Short = %x TestResult_Open = %x TestResult_FW_Rawdata = %x TestResult_Noise = %x --\n", TestResult_Short, TestResult_Open, TestResult_FW_Rawdata, TestResult_Noise);
	NVT_LOG("--\n");
...
{
	struct touch_info_dev_operations *tid_ops = ts->tid->tid_ops;
