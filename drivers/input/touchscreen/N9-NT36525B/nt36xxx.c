...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	ts->reset_gpio = of_get_named_gpio_flags(np, DTS_RESET_GPIO, 0, &ts->reset_flags);
	NVT_LOG("novatek,reset-gpio=%d\n", ts->reset_gpio);
#endif
	ts->irq_gpio = of_get_named_gpio_flags(np, DTS_INT_GPIO, 0, &ts->irq_flags);
...
#if NOVATEK_POWER_SOURCE_CUST_EN
int  novatek_plat_regulator_power_on(bool status)
{
    int rc = 0;

    if (IS_ERR_OR_NULL(ts->vddp) || IS_ERR_OR_NULL(ts->vddn)) {
        NVT_ERR("vdd is invalid");
        return -EINVAL;
    }
    ...
    if (status) {
        if (ts->power_disabled) {
            NVT_LOG("regulator enable !");
            rc = regulator_enable(ts->vddp);
            if ( rc != 0 ) {
                NVT_ERR("enable vsp fail\n");
            }
            rc = regulator_enable(ts->vddn);
            if ( rc != 0 ) {
                NVT_ERR("enable vsn fail\n");
            }

            ts->power_disabled = false;
        }
    } else {
        if (!ts->power_disabled) {
            NVT_LOG("regulator disable !");
            rc = regulator_disable(ts->vddp);
            if ( rc != 0 ) {
                NVT_ERR("enable vsp fail\n");
            }
            rc = regulator_disable(ts->vddn);
            if ( rc != 0 ) {
                NVT_ERR("enable vsn fail\n");
            }
            ts->power_disabled = true;
        }
    }
    return rc;   
}

int novatek_plat_regulator_power_init(bool status)
{
    int rc = 0;
    NVT_LOG("%s\n", status ? "POWER ENABLE" : "POWER DISABLE");
    if ( !status ) {
		NVT_LOG("power_init is deny\n");
        goto pwr_deny;
	}
    ts->vddp = regulator_get(&ts->client->dev, "vddp");
	if ( IS_ERR(ts->vddp) ) {
        rc = PTR_ERR(ts->vddp);
		NVT_ERR("Regulator get failed ts->vddp rc=%d\n",rc);
		goto lab_pwr_err;
    }
    ts->vddn = regulator_get(&ts->client->dev, "vddn");
	if ( IS_ERR(ts->vddn) ) {
        rc = PTR_ERR(ts->vddn);
		NVT_ERR("Regulator get failed ts->vddn rc=%d\n",rc);
		goto ibb_pwr_err;
    }
    NVT_LOG("power init end\n");
    return rc;

pwr_deny:
ibb_pwr_err:
	if (ts->vddn) {
		regulator_put(ts->vddn);
		ts->vddn = NULL;
	}
lab_pwr_err:
	if (ts->vddp) {
		regulator_put(ts->vddp);
		ts->vddp = NULL;
	}
    NVT_LOG("power exit end\n");
    return rc;

}

static void fts_power_source_exit(void)
{
    novatek_plat_regulator_power_on(DISABLE);
    novatek_plat_regulator_power_init(DISABLE);
}
#endif
...
static int gesture_get_capability(struct device *dev)
{
    ...
    NVT_LOG("ATO version close gesture.");
    return 0;
#else
    NVT_LOG("none-ATO version close gesture.");
...
static void get_fw_ver_bin(void *device_data)
{
	...
	struct nvt_ts_data *ts_data = container_of(sec, struct nvt_ts_data, sec);
	char buff[18] = { 0 };
	struct touch_info_dev *tid = ts->tid;
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
    ...
    if(ret == 1){
        NVT_LOG("offcharge mode,do not load tp probe ");
        return -1;
    }

	cmdline_nova_tp = strstr(saved_command_line,"nt36525b_");
	NVT_LOG("cmdline_fts_tp= %s",cmdline_nova_tp);
	if (cmdline_nova_tp == NULL){
		NVT_ERR("get nova,nt36525b_fail,this is not novatek tp");
		return -ENOMEM;
	}
	module_name = cmdline_nova_tp + strlen("nt36525b_");
	NVT_LOG("module_name= %s",module_name);

	if(strncmp(module_name,"txd_hdp_video",strlen("txd_hdp_video")) != 0){
		NVT_LOG("it is not novatek TP !\n");
		return -ENOMEM;
	}
    ...
    if (unlikely(!tid)) {
		ret = -ENOMEM;
        goto err_ts_data_alloc_failed;
    }
    tid_ops = tid->tid_ops;
    ts->tid = tid;
    tid->vendor = "novatek";
    tid->product = "nt36525b";
	tid->open_short_not_use_fw = 1;
...
#if NOVATEK_POWER_SOURCE_CUST_EN
    ret = novatek_plat_regulator_power_init(ENABLE);
    if (ret) {
        NVT_ERR("fail to get power(regulator)");
        goto err_power_init;
    }
    ts->power_disabled = true;
    ret = novatek_plat_regulator_power_on(ENABLE);
    if (ret) {
        NVT_ERR("fail to enable power(regulator)");
        goto err_power_init;
    }
#endif
    ...
    ts->tid->tid_ops->gesture_set_capability = tid_gesture_set_capability;
    ts->tid->tid_ops->gesture_get_capability = gesture_get_capability;
#endif
    ts->gesture = DISABLE;
    ...
    if (unlikely(ret)){
	    NVT_ERR("tid register failed");
        goto err_tid_and_ops_alloc_failed;
    }

#ifdef SEC_TSP_FACTORY_TEST
    ret = sec_cmd_init(&ts->sec, nova_commands,
        ARRAY_SIZE(nova_commands), SEC_CLASS_DEVT_TSP);
    if (ret < 0) {
        input_err(true, &client->dev,"%s: Failed to sec_cmd_init\n", __func__);
        ret = -ENODEV;
        goto err_tid_register_failed;
    }
#endif
	...
	return 0;
err_tid_register_failed:
    devm_tid_unregister(&client->dev, tid);
err_tid_and_ops_alloc_failed:
...
#if NOVATEK_POWER_SOURCE_CUST_EN
err_power_init:
    fts_power_source_exit();
#endif
err_spi_setup:
err_ckeck_full_duplex:
err_ts_data_alloc_failed:
...
#if NOVATEK_POWER_SOURCE_CUST_EN
    if(ts->gesture == DISABLE){
        novatek_plat_regulator_power_on(ENABLE);
    }
#endif
