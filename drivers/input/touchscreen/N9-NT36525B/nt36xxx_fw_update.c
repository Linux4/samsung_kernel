...
#include "nt36xxx.h"
#include "NVT_firmware_MANTIX_TXD.h"
...
static int32_t update_firmware_request(char *filename)
{
	...
	while (1) {
		...
		if (ret) {
			...
			request_fw_headfile->size = sizeof(FIRMWARE_DATA_MANTIX_TXD);
			request_fw_headfile->data = FIRMWARE_DATA_MANTIX_TXD;
        ...
        ts->tid->phone_fw_version = (unsigned int)*(fw_entry->data+FW_BIN_VER_OFFSET);
		/* BIN Header Parser */
