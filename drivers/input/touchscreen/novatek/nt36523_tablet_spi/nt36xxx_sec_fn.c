...
static void clear_cover_mode(void *device_data)
{
	...
	if (ts->platdata->scanoff_cover_close) {
		if (ts->flip_enable) {
			...
			wbuf[1] = 0x00;
			ret = CTP_SPI_WRITE(ts->client, wbuf, 2);
			...
			if (ts->power_status == LP_MODE_STATUS) {
				...
				wbuf[1] = LPWG_ENTER;
				wbuf[2] = 0x80;
				ret = CTP_SPI_WRITE(ts->client, wbuf, 3);
...
static void nvt_ts_set_grip_exception_zone(struct nvt_ts_data *ts, int *cmd_param)
{
	...
	switch (cmd_param[1]) {
		case 0:	//disable
			...
			buf[6] = 0;
			buf[7] = 0;
			nvt_ts_mode_switch_extended(ts, buf, 8, true);
			goto out;
		case 1:	//enable left
...
static void nvt_ts_set_grip_landscape_mode(struct nvt_ts_data *ts, int *cmd_param)
{
	...
	switch (cmd_param[1]) {
		case 0: //use previous portrait setting
			...
			buf[3] = (u8)cmd_param[1];
			nvt_ts_mode_switch_extended(ts, buf, 4, true);
			goto out;
		case 1: //enable landscape mode
			...
			nvt_ts_mode_switch_extended(ts, buf, 10, true);
			goto out;
		default:
			input_err(true, &ts->client->dev, "%s: not support parameter, cmd_param[1]=%d\n", __func__, cmd_param[1]);
			goto err;
