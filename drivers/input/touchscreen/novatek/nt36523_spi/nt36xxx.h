...
#define NVTLOG				"nt36xxx"
#define input_dbg(mode, dev, fmt, ...)						\
({										\
	dev_dbg(dev, NVTLOG " " fmt, ## __VA_ARGS__);				\
})
#define input_info(mode, dev, fmt, ...)						\
({										\
	dev_info(dev, NVTLOG " " fmt, ## __VA_ARGS__);				\
})
#define input_err(mode, dev, fmt, ...)						\
({										\
	dev_err(dev, NVTLOG " " fmt, ## __VA_ARGS__);				\
})
...
#define SEC_LPWG_DUMP 0
#define SEC_FW_STATUS 0
