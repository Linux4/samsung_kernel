...
int nvt_ts_mode_restore(struct nvt_ts_data *ts)
{
	...
	mode_read = nvt_ts_mode_read(ts);
	func_need_switch = ts->sec_function ^ mode_read;

	input_info(true, &ts->client->dev, "%s: sec_function:0x%X, mode_read:0x%X, func_need_switch:0x%X\n",
				__func__, ts->sec_function, mode_read, func_need_switch);
