...
static int pinctrl_configure(struct nvt_ts_data *ts, bool enable)
{
	...
	if (enable) {
		...
		if (IS_ERR(ts->platdata->pinctrl))
			NVT_LOG("%s: could not get active pinstate\n", __func__);
	} else {
		state = pinctrl_lookup_state(ts->platdata->pinctrl, "off_state");
		if (IS_ERR(ts->platdata->pinctrl))
			NVT_LOG("%s: could not get suspend pinstate\n", __func__);
	}
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	if (IS_ERR(ts->platdata->pinctrl))
		NVT_ERR("%s: could not get pinctrl\n", __func__);
...
/*
	...
	*/
	...
	if (ts->input_dev == NULL) {
		NVT_ERR("allocate input proximity device failed\n");
	...
	if (ret) {
		NVT_ERR("register input proximity device (%s) failed. ret=%d\n", ts->input_dev_proximity->name, ret);
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	if (ts->ear_detect_enable) {
		NVT_LOG("%s ear_detect_enable write start : ear_detect_enable = %d\n", __func__,ts->ear_detect_enable);
		buf[0] = EVENT_MAP_HOST_CMD;
		buf[1] = 0x85;
		CTP_SPI_WRITE(ts->client, buf, 2);
		NVT_LOG("%s ear_detect_enable write done\n", __func__);
	}
