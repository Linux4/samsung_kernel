...
void nvt_print_info(void)
{
	...
	if (ts->touch_count == 0)
		...
		input_info(true, &ts->client->dev,
				...
				ts->sec_function, ts->noise_mode, ts->lowpower_mode, ts->ear_detect_enable,
...
*/
   ...
   if (POINT_DATA_LEN >= POINT_DATA_CHECKSUM_LEN) {
	...
	if (ts->power_status == LP_MODE_STATUS && !ts->ear_detect_mode) {
...
int32_t nvt_ts_suspend(struct device *dev)
{
	...
	input_info(true, &ts->client->dev, "%s : start %d, %d\n", __func__, ts->ear_detect_enable, ts->lowpower_mode);

	if ((ts->lowpower_mode && (ts->prox_power_off || ts->ear_detect_enable)) ||
		(!ts->lowpower_mode && ts->ear_detect_enable))
		ts->ear_detect_mode = true;

	if ((ts->lowpower_mode || ts->lcdoff_test) && !ts->ear_detect_mode) {
		nvt_irq_enable(false);
