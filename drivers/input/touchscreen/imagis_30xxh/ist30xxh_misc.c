...
int dump_touch_node(struct ist30xx_data *data, u8 flag,
        struct TSP_NODE_BUF *node)
{
	int i, j;
	int count = 0;
...
#if defined(CONFIG_TOUCHSCREEN_DUMP_MODE)
int ist30xx_display_dump_log(struct ist30xx_data *data)
{
	int ret = 0;
	TSP_INFO *tsp = &data->tsp_info;
	u8 flag = NODE_FLAG_CDC | NODE_FLAG_BASE;
			       ...
			       size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		if (buf_value == NULL) {
			tsp_err("%s buf_value is NULL error\n", __func__);;
			return size;
		} else if (kstrtou32(buffer, 10, &start_ch_rx) < 0) {
			tsp_err("%s kstrto32 fail\n", __func__);
			return size;
		}
		...
		if (buf_value == NULL) {
			tsp_err("%s buf_value is NULL error\n", __func__);
			return size;
		} else if (kstrtou32(buffer, 10, &stop_ch_rx) < 0) {
			tsp_err("%s kstrto32 fail\n", __func__);
			return size;
		}
		...
		if (buf_value == NULL) {
			tsp_err("%s buf_value is NULL error\n", __func__);
			return size;
		} else if (kstrtou32(buffer, 10, &start_ch_tx) < 0) {
			tsp_err("%s kstrto32 fail\n", __func__);
			return size;
		}
		...
		if (buf_value == NULL) {
			tsp_err("%s buf_value is NULL error\n", __func__);
			return size;
		} else if (kstrtou32(buffer, 10, &stop_ch_tx) < 0) {
			tsp_err("%s kstrto32 fail\n", __func__);
			return size;
		}
	} else {
