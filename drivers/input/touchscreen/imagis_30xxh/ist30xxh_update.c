...
int ist30xx_isp_erase(struct ist30xx_data *data, u32 addr, u32 size)
{
	...
	for (i = 0; i < page_cnt; i++) {
		ret =
		    ist30xx_isp_aspgm(data, addr + (i * ISP_PAGE_SIZE),
				      ISP_MAIN);
		if (unlikely(ret))
			return ret;
			   ...
			   int len)
{
	...
	ret = ist30xx_write_cmd(data, IST30XX_HIB_CMD,
				(eHCOM_FW_HOLD << 16) | (IST30XX_ENABLE &
							 0xFFFF));
	if (ret)
	...
	for (i = 0; i < len; i++) {
		ret = ist30xx_write_buf(data->client,
					IST30XX_DA_ADDR(data->sec_info_addr),
					buf32 + i, 1);
		if (ret)
			goto err_write_sec_info;
	...
	ret = ist30xx_write_cmd(data, IST30XX_HIB_CMD,
				(eHCOM_FW_HOLD << 16) | (IST30XX_DISABLE &
							 0xFFFF));
	if (ret)
		goto err_write_sec_info;
...
int ist30xx_isp_fw_update(struct ist30xx_data *data, const u8 * buf)
{
	...
	ret =
	    ist30xx_isp_program(data, addr, (u32 *) buf,
				IST30XX_ROM_TOTAL_SIZE);
	if (unlikely(ret))
		goto isp_fw_update_end;
...
int ist30xx_tsp_update_info(struct ist30xx_data *data)
{
	...
	ret =
	    ist30xx_burst_read(data->client, IST30XX_DA_ADDR(eHCOM_GET_CHIP_ID),
...
int ist30xx_auto_bin_update(struct ist30xx_data *data)
{
	...
	u32 buff;
#endif
	if (!data->dt_data->fw_path) {
		tsp_err("do not exist fw_path is NULL \n");
		return false;
	}
			 ...
			 const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		if (buf_value == NULL) {
			...
			return size;
		} else if (kstrtouint(buf_value, 10, &mode) < 0) {
		...
		if (buf_value == NULL) {
			tsp_err("%s buf_value is NULL error\n", __func__);
			return size;
		} else if (kstrtouint(buf_value, 10, &calib) < 0) {
			tsp_err("%s kstrtoint fail\n", __func__);
			return size;
		}
	} else {
...
ssize_t ist30xx_ium_read_show(struct device * dev,
			      struct device_attribute * attr, char *buf)
{
	int i;
	int ret;
