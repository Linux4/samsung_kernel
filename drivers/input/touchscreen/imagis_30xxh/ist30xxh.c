...
void print_tsp_event(struct ist30xx_data *data, int id, finger_info * finger)
{
	...
	if (press) {
		...
		if (data->tsp_touched[id] == false) {
			...
			if (PARSE_PALM_STATUS(data->t_status) && (data->palm_flag == 0)) {
				tsp_noti("%s : ID:%d Palm(%x) major : %d minor : %d\n", __func__,
					id + 1, PARSE_PALM_STATUS(data->t_status),
					finger->bit_field.ma, finger->bit_field.mi);
				data->palm_flag++;
			}
			/* touch move */
		...
		if (data->tsp_touched[id] == true) {
			/* touch up */
			data->touch_pressed_num--;
			tsp_noti("%s%d mc:%d (0x%02x) ",
				 TOUCH_UP_MESSAGE, id + 1, data->move_count[id],
				 data->fw.cur.fw_ver);
#ifdef TCLM_CONCEPT
			printk("(test_result data :%x) (C%02XT%04X.%4s%s)\n", data->test_result.data[0],
	...
	if ((data->touch_pressed_num > 5) && (data->check_multi == 0)) {
		data->check_multi = 1;
		data->multi_count++;
	}
...
void recording_data(struct ist30xx_data *data, bool idle)
{
	...
	ret = ist30xx_read_reg(data->client, IST30XX_HIB_TOUCH_STATUS, &scancnt);
	if (ret) {
...
static irqreturn_t ist30xx_irq_thread(int irq, void *ptr)
{
	...
	if (data->status.sys_mode == STATE_LPM) {
		wake_lock_timeout(&data->wakelock, msecs_to_jiffies(3 * MSEC_PER_SEC));
		/* waiting for blsp block resuming, if not occurs i2c error */
		ret = wait_for_completion_interruptible_timeout(&data->resume_done, msecs_to_jiffies(3 * MSEC_PER_SEC));
		if (ret == 0) {
			tsp_err("%s: LPM: pm resume is not handled\n", __func__);
			return IRQ_HANDLED;
		}

		if (ret < 0) {
			tsp_err("%s: LPM: -ERESTARTSYS if interrupted, %d\n", __func__, ret);
			return IRQ_HANDLED;
		}

		tsp_info("%s: run LPM interrupt handler, %d\n", __func__, ret);
	}
...
static int ist30xx_ts_open(struct input_dev *dev)
{
	...
	if (!data->initialized) {
		tsp_warn("IC initialize is not complete\n");
		return 0;
	}
...
#ifdef CONFIG_PM
static int ist30xx_pm_suspend(struct device *dev)
{
	...
	if (data->input_dev) {
		retval = mutex_lock_interruptible(&data->input_dev->mutex);
		if (retval) {
		...
		if (!data->input_dev->disabled) {
			data->input_dev->disabled = true;
			if (data->input_dev->users && data->input_dev->close) {
				tsp_err("%s called without input_close\n",
						__func__);
				data->input_dev->close(data->input_dev);
			}
			data->input_dev->users = 0;
		}
...
static int ist30xx_pm_resume(struct device *dev)
{
	struct ist30xx_data *data = dev_get_drvdata(dev);
...
void timer_handler(unsigned long timer_data)
{
	...
	if (status->event_mode) {
		if (likely((status->sys_mode != STATE_POWER_OFF) && (status->update != 1) &&
...
static int ist30xx_parse_dt(struct device *dev, struct ist30xx_data *data)
{
	...
	if (of_property_read_string(np, "imagis,fw-path", &data->dt_data->fw_path) >= 0)
		tsp_info("%s() fw_path: %s\n", __func__,
			 data->dt_data->fw_path);
	else
		data->dt_data->fw_path = NULL;

#ifdef IST30XX_USE_CMCS
	if (of_property_read_string(np, "imagis,cmcs-path", &data->dt_data->cmcs_path) >= 0)
		tsp_info("%s() cmcs_path: %s\n", __func__,
			 data->dt_data->cmcs_path);
	else
		data->dt_data->cmcs_path = NULL;
#endif
	...
	if (of_property_read_u32
	    (np, "imagis,tclm_level", &data->dt_data->tclm_level) >= 0)
		tsp_info("%s() tclm_level: %d\n", __func__,
...
static void ist30xx_check_rawdata(struct work_struct *work)
{
	...
	if (data->tsp_dump_lock == 1) {
		tsp_info("%s: ignored ## already checking..\n", __func__);
		return;
	}
	if (data->status.sys_mode == STATE_POWER_OFF) {
		tsp_info("%s: ignored ## IC is power off\n", __func__);
		return;
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	wake_lock_init(&data->wakelock, WAKE_LOCK_SUSPEND, "tsp_wakelock");
	...
	while (retry-- > 0) {
		...
		if (unlikely(ret == 0)) {
			...
			/*
			 * if the default chip id and chip id are different, use to include the following syntax. 
...
static const struct dev_pm_ops ist30xx_pm_ops = {
	.suspend = ist30xx_pm_suspend,
	.resume = ist30xx_pm_resume,
};
#endif
