...
static int mms_ts_config(struct mms_ts_info *info, int fw_location)
{
	...
	while (retries--){
		if (!get_fw_version2(client, ver)){
			...
			printk("%s, get version fail, reset, r:%d", __func__, retries);
			mms_reboot(info);
		}
	} 
...
/* __devinit */
int mms_ts_probe(struct i2c_client *client,
				  const struct i2c_device_id *id)
{
	...
	melfas_vdd_on(info, 1);
	printk("%s --- vdden:%d, irq:%d, i2cpu:%d, vcci2c:%d\n",__func__,info->pdata->vdd_en, info->pdata->gpio_int, info->pdata->i2c_pull_up,!!(info->vcc_i2c));
	msleep(100);
