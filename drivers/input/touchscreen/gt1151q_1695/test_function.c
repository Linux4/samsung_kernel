...
/*----------------------------------- other define-----------------------------------*/
char save_result_dir[250] = "/sdcard/tpdata/";
...
static void _get_channel_average_value(void)
{
	...
	if (current_data_index == 0) {
		...
		memset(channel_square_average, 0, sizeof(*channel_square_average) / sizeof(channel_square_average[0]));
	}
	...
	if ((test_types & _ACCORD_CHECK) != 0) {
		...
		for (i = 0; i < sys.sc_sensor_num; i++) {
			...
			for (j = 0; j < sys.sc_driver_num; j++) {
				...
				{
					bytes += (s32) snprintf((char *)&data[bytes], 10, "0.00000%ld,",
								channel_max_accord[i + j * sys.sc_sensor_num]);
				}
...
static s32 _init_test_paramters(char *inipath)
{
	...
	int ret = -1;
	/* HS60 add for HS60-1486 by zhouzichun at 20190910 start */
	int retry = 0;
	/* HS60 add for HS60-1486 by zhouzichun at 20190910 end */
	u8 *s_nc_buf;
	u8 *key_nc_buf;
#if !GTP_TEST_PARAMS_FROM_INI
	const u8 nc_tmp[] = NC;
	...
	/* HS60 add for HS60-1486 by zhouzichun at 20190910 start */
	while (retry++ < 3) {
		ret = _alloc_test_memory(b_size);
		if (ret >= 0) {
			GTP_INFO("_alloc_test_memory successfully");
			break;
		}
		msleep(200);
	}
	if (ret < 0) {
		GTP_ERROR("_alloc_test_memory failed");
		return MEMORY_ERR;
	}
	GTP_INFO("begin _get_test_parameters");
	/* HS60 add for HS60-1486 by zhouzichun at 20190910 end */
