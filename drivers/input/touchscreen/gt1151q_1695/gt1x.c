...
//static u8 gt1x_wakeup_level1;
u8 gt1x_glove_type = 0;
unsigned int tp_glove_state;
int gt1x_glove_cfg(int sensor_id);
int gtp_send_glove_cfg(bool glove_enable);
int glove_tpd_halt = 0;
int gtp_glove_mode_status = 1;
struct kobject *goodix_glove_kobj;
//static u8 gtp_glove_config[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
//	= {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
extern int gt1x_glove_update_state(void);
...
#ifdef CONFIG_GTP_GLOVE_MODE
static ssize_t gtp_glove_enable_show(struct kobject *kobj, struct kobj_attribute *attr,char *buf)
{
...
static ssize_t gtp_glove_enable_store(struct kobject *kobj, struct kobj_attribute *attr,const char *buf, size_t count)
{
		struct glove_device *dev = gt1x_glove_dev;
		int s = (buf[0] - '0');
		...
		dev->state = s;
		gt1x_glove_update_state();
		return count;
}

static struct kobj_attribute glove_enable_status = {
	.attr = {
		 .name = "glove_enable",
		 .mode = S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP,
		 },
	.show = &gtp_glove_enable_show,
	.store = &gtp_glove_enable_store,
};
...
int gt1x_glove_cfg(int sensor_id)
{
	...
	u8 *cfg;
	int size;
	gt1x_glove_dev= kmalloc(sizeof(struct glove_device), GFP_KERNEL);
	cfg = gt1x_glove_dev->config;
	size = gt1x_find_tp_config(dev, "glove-config",
			sensor_id, cfg);
	if (size < 0) {
		GTP_ERROR("Failed to obtain glove config");
		return size;
	}

	cfg[0] &= 0x7F;
	set_reg_bit(cfg[TRIGGER_LOC], 0, gt1x_glove_type);//zkl
	set_reg_bit(cfg[MODULE_SWITCH3_LOC], 5, !gt1x_wakeup_level);//zkl
	gt1x_glove_dev->cfg_len = size;
	return 0;
}
...
/* HS60 modify for HS60-230 by liufurong at 20190730 start */
	GTP_GPIO_OUTPUT(GTP_RST_PORT,0);
/* HS60 modify for HS60-230 by liufurong at 20190730 end */
	return ret;
}
...
/* Huaqin add for AR-ZQL1695-01000000051 ito test by liufurong at 2019/07/17 start */
#define GTP_INFO_PROC_FILE "tp_info"
static struct proc_dir_entry *gtp_info_proc_entry;

static ssize_t gtp_proc_getinfo_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	int ret = 0;
	u8 cfg_ver_info = 0;
	char buf[150] = {0};
...
static const struct file_operations gtp_info_proc_fops = {
	.owner = THIS_MODULE,
	.read = gtp_proc_getinfo_read,
};

/*******************************************************
Description:
	gtp touchscreen extra function proc. file node
	initial function.

return:
	Executive outcomes. 0---succeed. -12---failed.
*******************************************************/
int32_t gtp_extra_proc_init(void)
{
	gtp_info_proc_entry = proc_create(GTP_INFO_PROC_FILE, 0664, NULL, &gtp_info_proc_fops);
	if (NULL == gtp_info_proc_entry)
	{
...
/* HS60 add for SR-ZQL1695-01000000583 by liufurong at 20190724 start */
	...
	if (fts_probe_ok == 1) {
		GTP_INFO("FTS has probe ok ,gtp dont probe\n");
		return -1;
	}
	...
	if (!sec_info) {
		GTP_ERROR("allocate memory for sec_info fail");
		return -ENOMEM;
	}
	gtp_sec_info = sec_info;
...
//add by likang for pinctrl @20171130/
	...
	gt1x_init();
		...
		gt1x_glove_cfg(gt1x_version.sensor_id);
		goodix_glove_kobj = kobject_create_and_add("goodix_glove", NULL) ;
		if (goodix_glove_kobj == NULL)
		{
			GTP_ERROR("%s: subsystem_register failed\n", __func__);
		}

		ret = sysfs_create_file(goodix_glove_kobj, &glove_enable_status.attr);
		if (ret)
		{
...
/* Huaqin add for AR-ZQL1695-01000000051 ito test by liufurong at 2019/07/17 start */
	...
	gtp_test_node_init(&hwinfo_device);
	gtp_extra_proc_init();
/* Huaqin add for AR-ZQL1695-01000000051 ito test by liufurong at 2019/07/17 end */
	//gtp_hw_info();
/* HS60 add for SR-ZQL1695-01000000583 by liufurong at 20190724 start */
	ret = sec_cmd_init(&sec_info->sec, gtp_commands, gtp_get_array_size(), SEC_CLASS_DEVT_TSP);
	if (ret < 0) {
...
static int gt1x_ts_remove(struct i2c_client *client)
{
	...
	gt1x_release_resource();
	/* Huaqin add for AR-ZQL1695-01000000051 ito test by liufurong at 2019/07/17 start */
	gtp_test_node_exit(&hwinfo_device);
	/* Huaqin add for AR-ZQL1695-01000000051 ito test by liufurong at 2019/07/17 end */
