...
extern struct sec_info *gtp_sec_info;
extern int gesture_enabled;
extern struct sec_cmd gtp_commands[];
extern int gtp_get_array_size(void);
...
extern int gtp_test_node_init(struct platform_device *tpinfo_device);
extern int gtp_test_node_exit(struct platform_device *tpinfo_device);
...
extern int gt1x_glove_cfg(int sensor_id);
struct glove_device{
		int enabled;
		int state;
		int suspended;
...
extern struct glove_device *gt1x_glove_dev;
...
int gt1x_find_tp_config(struct device *dev,char *cfg_name, u8 sensor_id, u8 *config);
