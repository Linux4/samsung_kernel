...
{
		int ret = 0;
		struct glove_device *dev = gt1x_glove_dev;
		if (!dev) {
			return -ENODEV;
		}
		if (!dev->suspended) {
			...
			#ifdef CONFIG_GTP_CHARGER_SWITCH
				gt1x_charger_config(1);// charger detector module check
			#else
				ret = gt1x_send_cfg(gt1x_config, gt1x_cfg_length);
			#endif
			}
		GTP_DEBUG("Update glove state %s.", dev->state ? "On" : "Off");
		} else {
...
#ifdef CONFIG_GTP_GLOVE_MODE
		if (gt1x_glove_dev && gt1x_glove_dev->enabled
			&& gt1x_glove_dev->state) {
			return;
		}
#endif
