...
#define UPDATE_FILE_PATH_1			"/vendor/firmware/_goodix_update_.bin"
...
int gt1x_update_prepare(char *filename)
{
	...
	if (filename == NULL) {
		...
		vts_name = getname_kernel(update_info.fw_name);
		update_info.fw_file = file_open_name(vts_name, O_RDONLY, 0);
...
_reset:
	...
	gtp_sec_info->fw_version = ver_info.patch_id;
	gtp_sec_info->fw_version_bin = fw_ver_info.patch_id;
...
int gt1x_burn_subsystem(struct fw_subsystem_info *subsystem)
{
	...
	while (length > 0 && retry > 0) {
		...
		while (wait_time-- > 0) {
			...
			msleep(5);
			gt1x_i2c_read(0x8022, &confirm, 1);
			if (ret < 0) {
			continue;
			}
			if (buffer[0] != confirm) {
...
void dump_to_file(u16 addr, int length, char *filepath)
{
	...
	flp = file_open_name(vts_name, O_RDWR | O_CREAT, 0666);
	if (IS_ERR(flp)) {
