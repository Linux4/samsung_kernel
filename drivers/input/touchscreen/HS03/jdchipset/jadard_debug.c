...
static ssize_t jadard_tp_info_read(struct file *file, char *buf,
								size_t len, loff_t *pos)
{
	size_t ret = 0;
	char *buf_tmp = NULL;
...
static struct file_operations jadard_proc_debug_tp_info_ops = {
	.owner = THIS_MODULE,
	.read = jadard_tp_info_read,
	//.write = jadard_tp_info_write,
};
...
int jadard_touch_proc_init(void)
{
	...
	/*HS03 code for SR-SL6215-01-86 by chenyihong at 20210805 start*/
	jadard_proc_tpinfo_file = proc_create(JADARD_PROC_TPINFO_FILE, (S_IWUSR | S_IRUGO),
										NULL, &jadard_proc_debug_tp_info_ops);
	if (jadard_proc_tpinfo_file == NULL) {
		JD_E(" %s: proc debug file create failed!\n", __func__);
		goto fail_C;
	}
	/*HS03 code for SR-SL6215-01-86 by chenyihong at 20210805 end*/
...
/*HS03 code for SR-SL6215-01-86 by chenyihong at 20210805 start*/
fail_C : remove_proc_entry(JADARD_PROC_DEBUG_FILE, pjadard_touch_proc_dir);
/*HS03 code for SR-SL6215-01-86 by chenyihong at 20210805 end*/
