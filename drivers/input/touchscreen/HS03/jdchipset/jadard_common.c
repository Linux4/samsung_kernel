								...
								unsigned long val, void *v)
{
	...
	if (!psy) {
		JD_E("Couldn't get usbpsy\n");
		return -EINVAL;
	}
	if (!strcmp(psy->desc->name, "usb")) {
		if (psy && val == POWER_SUPPLY_PROP_STATUS) {
			ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_ONLINE, &prop);
			JD_I("jadard_charger_notifier_callback, prop.intval=%d,jd_usb_detect_flag=%d!\n",prop.intval,jd_usb_detect_flag);
			if (ret < 0) {
				JD_E("Couldn't get POWER_SUPPLY_PROP_ONLINE rc=%d\n", ret);
				return ret;
			} else {
				if(prop.intval != jd_usb_detect_flag) {
					JD_I("usb_plug_status = %d\n", prop.intval);
					jd_usb_detect_flag = prop.intval;
				}
			}
		}
	}
