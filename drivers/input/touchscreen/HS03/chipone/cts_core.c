        ...
        struct cts_device_touch_info *touch_info)
{
    ...
    if (!chipone_ts_data_global->cts_tp_on_off){
        cts_dbg("cts_tp_on_off = 0");
        return -ECANCELED;
    }
...
static void cts_charger_notify_work(struct work_struct *work)
{
    ...
    cts_info("noti_value = %d,Set dev charger %s",
        chipone_ts_data_global->usb_plug_status, chipone_ts_data_global->usb_plug_status ? "ATTACHED" : "DETATCHED");
    ret = cts_send_command(&chipone_ts_data_global->cts_dev,
        chipone_ts_data_global->usb_plug_status ? CTS_CMD_CHARGER_ATTACHED : CTS_CMD_CHARGER_DETACHED);
    if (ret) {
        cts_err("Send CMD_CHARGER_%s failed %d",
            chipone_ts_data_global->usb_plug_status ? "ATTACHED" : "DETACHED", ret);
    }
}

static int cts_charger_notifier_callback(struct notifier_block *nb, unsigned long val, void *v)
{
    int ret = 0;
    struct power_supply *psy = NULL;
    union power_supply_propval prop;

    cts_info("cts_charger_notifier_callback!");
    if(chipone_ts_data_global->fw_update_stat != 1)
        return 0;

    psy= power_supply_get_by_name("usb");
    if (!psy) {
        cts_err("Couldn't get usbpsy");
        return -EINVAL;
    }
    if (!strcmp(psy->desc->name, "usb")) {
        if (psy && val == POWER_SUPPLY_PROP_STATUS) {
            ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_ONLINE, &prop);
            cts_info("cts_charger_notifier_callback, prop.intval=%d,usb_plug_status=%d!",prop.intval,chipone_ts_data_global->usb_plug_status);
            if (ret < 0) {
                cts_err("Couldn't get POWER_SUPPLY_PROP_ONLINE rc=%d", ret);
                return ret;
            } else {
                if(prop.intval != chipone_ts_data_global->usb_plug_status) {
                    cts_info("usb_plug_status = %d", prop.intval);
                    chipone_ts_data_global->usb_plug_status = prop.intval;
                    if((chipone_ts_data_global->cts_dev.rtdata.suspended == false) && (chipone_ts_data_global->cts_charger_notify_wq != NULL)){
                        queue_work(chipone_ts_data_global->cts_charger_notify_wq, &chipone_ts_data_global->charger_notify_work);
                    }
                }
            }
        }
...
void cts_charger_init(void)
{
    int ret = 0;
    struct power_supply *psy = NULL;
    union power_supply_propval prop;
    chipone_ts_data_global->cts_charger_notify_wq = create_singlethread_workqueue("cts_charger_wq");
    if (!chipone_ts_data_global->cts_charger_notify_wq) {
        cts_err("allocate cts_charger_notify_wq failed");
        return;
    }
    INIT_WORK(&chipone_ts_data_global->charger_notify_work, cts_charger_notify_work);
    chipone_ts_data_global->charger_notif.notifier_call = cts_charger_notifier_callback;
    ret = power_supply_reg_notifier(&chipone_ts_data_global->charger_notif);
    if (ret) {
    ...
    */
    ...
    if (psy) {
        ...
        if (ret < 0) {
            ...
            chipone_ts_data_global->usb_plug_status = prop.intval;
            cts_dbg("boot check usb_plug_status = %d", prop.intval);
        }
    }
}
...
int cts_resume_device(struct cts_device *cts_dev)
{
    ...
    /*HS03 code for SR-SL6215-01-254  by LiShuai at 20210909 start*/
    if(chipone_ts_data_global->cts_charger_notify_wq != NULL)
        queue_work(chipone_ts_data_global->cts_charger_notify_wq, &chipone_ts_data_global->charger_notify_work);
    /*HS03 code for SR-SL6215-01-254  by LiShuai at 20210909 end*/
