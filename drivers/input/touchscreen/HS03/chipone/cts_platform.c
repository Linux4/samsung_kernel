...
int cts_spi_send_recv(struct cts_platform_data *pdata, size_t len , u8 *tx_buffer, u8 *rx_buffer)
{
    ...
    struct spi_transfer cmd = {
        .cs_change = 0,
        .delay_usecs = 0,
        /* HS03 code for SR-SL6215-01-32 by yuanliding at 20210902 start */
        .speed_hz = pdata->spi_speed,
        //.speed_hz = cts_spi_speed * 1000u,
        /* HS03 code for SR-SL6215-01-32 by yuanliding at 20210902 end */
        .tx_buf = tx_buffer,
        .rx_buf = rx_buffer,
        .len    = len,
        //.tx_dma = 0,
        ...
        struct device_node *dev_node)
{
    ...
    /* HS03 code for SR-SL6215-01-32 by yuanliding at 20210902 start */
    ret = of_property_read_u32(dev_node, CFG_CTS_OF_SPI_FREQUENCY,
            &pdata->spi_speed);
    if (ret) {
        cts_warn("Parse spi frequency from dt failed %d", ret);
        //return ret;
    }
    cts_info("  %-12s: %d", "spi frequency", pdata->spi_speed);
    /* HS03 code for SR-SL6215-01-32 by yuanliding at 20210902 end */
...
int cts_input_open(struct input_dev *dev)
{
    chipone_ts_data_global->cts_tp_on_off = 1;
    cts_info("cts_input_open:input enable");

    return 0;
}

void cts_input_close(struct input_dev *dev)
{
    chipone_ts_data_global->cts_tp_on_off = 0;
    cts_info("cts_input_close:input disable");
}
