...
int gcore_parse_mp_test_ini(struct gcore_mp_data *mp_data)
{
    ...
    /*HS03 code for SR-SL6215-01-1189 by duanyaoming at 20220407 start*/
    strncpy(mp_ini_name, fn_data.gdev->gc_mp_ini_name, sizeof(mp_ini_name));
    GTP_DEBUG("open mp ini file:%s", mp_ini_name);

    f = filp_open(mp_ini_name, O_RDONLY, 644);
     /*HS03 code for SR-SL6215-01-1189 by duanyaoming at 20220407  end*/
    if (IS_ERR_OR_NULL(f)) {
...
int gcore_mp_test_item_open(struct gcore_mp_data *mp_data)
{
    ...
    /*HS03 code for SR-SL6215-01-1191 by duanyaoming at 20220412 start*/
    ...
    /*HS03 code for SR-SL6215-01-1191 by duanyaoming at 20220412 end*/
...
int dump_mp_info_to_csv_file(const char *filepath, int flags)
{
    ...
    u8 speed[10];
#endif
    strncpy(icname, fn_data.gdev->gc_ic_name, sizeof(icname));
    pos = 0;
    file = filp_open(filepath, flags, 0666);
    if (IS_ERR(file)) {
        GTP_ERROR("Open file %s failed %ld", filepath, PTR_ERR(file));
...
int gcore_mp_bin_update(void)
{
    ...
    /*HS03 code for SR-SL6215-01-1189 by duanyaoming at 20220407 start*/
    strncpy(mp_bin_name, fn_data.gdev->gc_mp_name, sizeof(mp_bin_name));
    GTP_DEBUG("open mp bin file:%s", mp_bin_name);
