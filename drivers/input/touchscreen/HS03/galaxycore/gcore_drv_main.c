...
/* HS03 code for SL6215DEV- by zhoulingyun at 20210909 start */
#include <linux/sprd_drm_notifier.h>

static struct gcore_dev *gcore_ts_data = NULL;
/* HS03 code for SL6215DEV- by zhoulingyun at 20210909 end */
...
s32 gcore_touch_event_handler(struct gcore_dev *gdev)
{
	...
	|| defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) \
	|| defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
	int tpd_res_x = TOUCH_REPORT_X_MAX;
	int tpd_res_y = TOUCH_REPORT_Y_MAX;
...
static irqreturn_t tpd_event_handler(int irq, void *dev_id)
{
	...
	GTP_DEBUG("tpd_event_handler enter.");
...
{
	GTP_DEBUG("event = %d,gcore_ts_data->tp_suspend=%d\n", event, gcore_ts_data->tp_suspend);

        if ((event == DISPC_POWER_OFF) && (gcore_ts_data->tp_suspend == false))
                gcore_suspend();
        else if ((event == DISPC_POWER_ON) && (gcore_ts_data->tp_suspend == true))
                gcore_resume();
	return 0;
}
/*HS03 code for SL6215DEV-872 by zhoulingyun at 20210909 end*/
...
void gcore_spi_config(void)
{
	...
	gcore_spi_slave->max_speed_hz = 5000000;
...
static int gcore_spi_remove(struct spi_device *slave)
{
	...
	/*HS03 code for SL6215DEV-872 by zhoulingyun at 20210909 start*/
	disp_notifier_unregister(&gdev->drm_notifier);
	/*HS03 code for SL6215DEV-872 by zhoulingyun at 20210909 end*/
	gcore_deinit(gdev);
