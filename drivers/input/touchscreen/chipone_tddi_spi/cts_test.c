...
static int wait_fw_to_curr_mode(struct cts_device *cts_dev)
{
	...
	do {
		...
		if (ret) {
			...
			if (work_mode == CTS_TEST_OPEN_SHORT_DET_MODE)
				return 0;
		}
		mdelay(10);
	} while (++i < 100);
...
int wait_to_cfg_mode(struct cts_device *cts_dev)
{
    int i = 0;
    int ret;
    uint8_t work_mode;
    ...
    do {
        cts_info("Wait firmware to CTS_CFG_MODE work, times:%d", i);
        ret = cts_tcs_get_krang_current_workmode(cts_dev, &work_mode);
        if (ret == 0 && work_mode == CTS_CFG_MODE) {
            return 0;
        } else {
            cts_info("Get firmware work mode failed, curr_mode: %d", work_mode);
            ret = -1;
            mdelay(10);
            continue;
        }
...
static int prepare_test(struct cts_device *cts_dev)
{
    ...
    ret = cts_tcs_set_krang_workmode(cts_dev, CTS_CFG_MODE);
    if (ret) {
    ...
    ret = wait_to_cfg_mode(cts_dev);
    if (ret) {
        cts_err("Wait cfg mode failed %d", ret);
        return ret;
    }
    ...
    ret = cts_tcs_set_krang_workmode(cts_dev, CTS_FIRMWARE_WORK_MODE_NORMAL);
    if (ret) {
