		...
		u8 *dst, u8 *src, enum int_data_type type)
{
	...
	if (type == INT_DATA_TYPE_CNEGDATA) {
		for (i = 0; i < cols; i++) {
			for (j = 0; j < rows; j++) {
				*dst++ = src[j * cols + i];
			}
...
int cts_tcs_get_krang_current_workmode(const struct cts_device *cts_dev, u8 *workmode)
{
    u8 buf = 0;
    int ret = 0;
...
int cts_tcs_set_krang_workmode(const struct cts_device *cts_dev, u8 workmode)
{
    return cts_tcs_write(cts_dev, CMD_SYS_STS_KRANG_WORK_MODE_RW, &workmode,
            sizeof(workmode));
}

int cts_tcs_set_krang_stop(const struct cts_device *cts_dev)
{
	uint8_t stop = 1;

    return cts_tcs_write(cts_dev, CMD_TP_STD_CMD_SET_KRANG_STOP, &stop, 1);
}
...
int cts_tcs_set_high_sensitivity_mode(struct cts_device *cts_dev, u8 enable)
{
    struct cts_firmware_status *status = (struct cts_firmware_status *)
            &cts_dev->rtdata.firmware_status;
    int ret;

    cts_info("Set high sensitivity enable:%d", enable);

    status->high_sensitivity = enable;

    ret = cts_tcs_write(cts_dev, CMD_SYS_STS_HI_SENSE_EN_RW,
            &enable, sizeof(enable));
    if (ret) {
        cts_err("Set high sensitivity failed!");
    }

    return ret;
}

int cts_tcs_get_high_sensitivity_mode(const struct cts_device *cts_dev, u8 *enabled)
{
    u8 buf[1] = { 0 };
    int ret;

    ret = cts_tcs_read(cts_dev, CMD_SYS_STS_HI_SENSE_EN_RW, buf, sizeof(buf));
    if (ret == 0) {
        *enabled = buf[0];
    }
...
void cts_tcs_reinit_fw_status(struct cts_device *cts_dev)
{
    ...
    cts_tcs_set_high_sensitivity_mode(cts_dev, status->high_sensitivity);
}
