        ...
        size_t len, u32 crc, int retry)
{
    ...
    if (cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9922C) {
        ...
        addr[1] = 0x02bff8;
        addr[2] = 0x02bffc;
    } else if (cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9951 ||
			cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9951R) {
        addr[0] = 0x037ff0;
        addr[1] = 0x037ff8;
        addr[2] = 0x037ffc;
    } else {
        ...
        struct cts_device_touch_info *touch_info)
{
	...
	cts_dbg("TOUCH INFO SIZE: %d", sizeof(struct cts_device_touch_info));
...
int cts_irq_handler(struct cts_device *cts_dev)
{
    ...
    if (unlikely(cts_dev->rtdata.suspended)) {
        ...
        ret = cts_plat_process_touch_msg(cts_dev->pdata, touch_info);
        if (ret) {
...
int cts_resume_device(struct cts_device *cts_dev)
{
    ...
    if (cts_dev->rtdata.gesture_wakeup_enabled) {
        ...
        ret = cts_wait_current_mode(cts_dev, 0);
        if (ret) {
...
void cts_show_touch_debug_msg(void *debug)
{
    ...
    cts_info("High_Sens   : %d", status->high_sensitivity);
...
//+P86801AA1 peiyuexiang.wt 20230625, add, hardware_info
#define CTS_DEV_HWID_ICNL9951R   0x991510u

static u32 tp_ic_type = 0;
static u16 firmware_version_from_ic = 0;

static void fts_read_tp_hardware_info(struct cts_device *ctps_dev)
{
    struct cts_device *cts_dev = ctps_dev;
    pr_err("CTS-Core:%s entry\n", __func__);
    tp_ic_type = cts_dev->hwdata->hwid;
    firmware_version_from_ic = cts_dev->fwdata.version;

    cts_err("factory read ic_type :0x%08x, firmware_version_from_ic: 0x%04x\n", tp_ic_type, firmware_version_from_ic);
}

static void fts_hardwareinfo_set(void)
{
    char ic_name[HARDWARE_MAX_ITEM_LONGTH];
    pr_err("CTS-Core:%s entry\n", __func__);
    if(tp_ic_type == CTS_DEV_HWID_ICNL9951R)
    {
        snprintf(ic_name,HARDWARE_MAX_ITEM_LONGTH,"ICNL9951R");
    }else{
        snprintf(ic_name,HARDWARE_MAX_ITEM_LONGTH,"Other IC");
    }

    if (strstr(Lcm_name_tp,"chipone_icnl9951r_wt_dsi_vdo_90hz_boe")) {
        snprintf(TP_name,HARDWARE_MAX_ITEM_LONGTH,"BOE, %s, FW:0x%04X",  \
                    ic_name, firmware_version_from_ic);
	}
    if (strstr(Lcm_name_tp,"hjr_icnl9951r_wt_dsi_vdo_90hz_hkc")) {
        snprintf(TP_name,HARDWARE_MAX_ITEM_LONGTH,"HJR, %s, FW:0x%04X",  \
                    ic_name, firmware_version_from_ic);
    }
    if (strstr(Lcm_name_tp,"txd_icnl9951r_wt_dsi_vdo_90hz_hkc")) {
        snprintf(TP_name,HARDWARE_MAX_ITEM_LONGTH,"TXD, %s, FW:0x%04X",  \
                    ic_name, firmware_version_from_ic);
    }
// +P86801AA1 peiyuexiang.wt,add,20230905,compatible XINXIAN
    if (strstr(Lcm_name_tp,"xinxian_icnl9951r_wt_dsi_vdo_90hz_mdt")) {
        snprintf(TP_name,HARDWARE_MAX_ITEM_LONGTH,"XINXIAN,%s, FW:0x%04X",  \
                    ic_name, firmware_version_from_ic);
    }
// -P86801AA1 peiyuexiang.wt,add,20230905,compatible XINXIAN
...
//+P86801AA1, peiyuexiang.wt, 20230625, add, hardware_info
	fts_read_tp_hardware_info(cts_dev);
