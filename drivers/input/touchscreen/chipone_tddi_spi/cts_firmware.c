...
// +P86801AA1 peiyuexiang.wt,add,20230803,compatible HJR
...
#if !defined(CONFIG_CTS_ICTYPE_ICNL9951) || !defined(CONFIG_CTS_ICTYPE_ICNL9951R)
...
extern int cts_tcs_set_krang_stop(const struct cts_device *cts_dev);
        ...
        u32 hwid, u16 fwid, u16 curr_firmware_ver)
{
    ...
    firmware_builtin = cts_request_newer_driver_builtin_firmware(
        hwid, fwid, curr_firmware_ver);
	if (firmware_builtin) {
		curr_firmware_ver = FIRMWARE_VERSION(firmware_builtin);
	}
#endif
...
#ifdef CFG_CTS_FIRMWARE_IN_FS
#ifndef CFG_CTS_FOR_GKI
    if (is_filesystem_mounted(CFG_CTS_FIRMWARE_FILEPATH_HJR)) {
#endif
        firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,
#ifdef CFG_CTS_FW_UPDATE_FILE_LOAD
        cts_dev->config_fw_name[0] ? cts_dev->config_fw_name : CFG_CTS_FIRMWARE_FILENAME_HJR,
#else
        CFG_CTS_FIRMWARE_FILENAME_HJR,
#endif
        curr_firmware_ver);
	}
#endif /* CFG_CTS_FIRMWARE_IN_FS */
...
#ifdef CFG_CTS_FIRMWARE_IN_FS
#ifndef CFG_CTS_FOR_GKI
    if (is_filesystem_mounted(CFG_CTS_FIRMWARE_FILEPATH_TXD)) {
#endif
        firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,
#ifdef CFG_CTS_FW_UPDATE_FILE_LOAD
        cts_dev->config_fw_name[0] ? cts_dev->config_fw_name : CFG_CTS_FIRMWARE_FILENAME_TXD,
#else
        CFG_CTS_FIRMWARE_FILENAME_TXD,
#endif
        curr_firmware_ver);
	}
#endif /* CFG_CTS_FIRMWARE_IN_FS */
...
#ifdef CFG_CTS_FIRMWARE_IN_FS
#ifndef CFG_CTS_FOR_GKI
    if (is_filesystem_mounted(CFG_CTS_FIRMWARE_FILEPATH_XINXIAN)) {
#endif
        firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,
#ifdef CFG_CTS_FW_UPDATE_FILE_LOAD
        cts_dev->config_fw_name[0] ? cts_dev->config_fw_name : CFG_CTS_FIRMWARE_FILENAME_XINXIAN,
#else
        CFG_CTS_FIRMWARE_FILENAME_XINXIAN,
#endif
        curr_firmware_ver);
	}
#endif /* CFG_CTS_FIRMWARE_IN_FS */
        ...
        const struct cts_firmware *firmware, bool to_flash)
{
	...
	if ((ret = cts_switch_to_master(cts_dev))) {
		cts_err("Switch to master faild %d", ret);
		goto out;
	}

    ret = cts_tcs_set_krang_stop(cts_dev);// stop master and slave
    if (ret < 0) {
        cts_warn("Set krang stop failed");
    }
	...
	if ((ret = cts_switch_to_slave(cts_dev))) {
