	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			if (finger_status == F12_PALM_STATUS)
				input_report_key(rmi4_data->input_dev, KEY_PALM_MODE, 1);
			...
			touch_count++;
			break;
/*	//remove for report finger in palm
		case F12_PALM_STATUS:
			dev_dbg(rmi4_data->pdev->dev.parent,
...
#endif
		input_report_key(rmi4_data->input_dev, KEY_PALM_MODE, 0);
		input_report_key(rmi4_data->input_dev,
				BTN_TOUCH, 0);
		input_report_key(rmi4_data->input_dev,
				BTN_TOOL_FINGER, 0);
...
static int synaptics_reset_device(struct device *dev)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);

	return synaptics_rmi4_reset_device(rmi4_data, false);
}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	rmi4_data->tid = tid;
	tid->tid_ops->reset = synaptics_reset_device;
...
static void synaptics_rmi4_late_resume(struct early_suspend *h)
{
	...
	struct synaptics_rmi4_data *rmi4_data =
			container_of(h, struct synaptics_rmi4_data,
			early_suspend);
        dev_err(rmi4_data->pdev->dev.parent,"lqg===>%s: start\n",__func__); //lqg add 0819
	if (rmi4_data->stay_awake)
		return;
...
exit:
	...
	rmi4_data->suspend = false;
        dev_err(rmi4_data->pdev->dev.parent,"lqg===>%s: end\n",__func__); //lqg add 0819
	return;
}
#endif
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (!rmi4_data->suspend) {
		...
		synaptics_rmi4_wakeup_gesture(rmi4_data, true);
		usleep_range(TDDI_LPWG_WAIT_US, TDDI_LPWG_WAIT_US + 20);
#endif
		synaptics_rmi4_irq_enable(rmi4_data, false, false);
		synaptics_rmi4_sleep_enable(rmi4_data, true);
		synaptics_rmi4_free_fingers(rmi4_data);
