...
static int raw_limit_save(const u8 *data, short *raw_limit_lower, short *raw_limit_upper)
{
	char temp[5] = {'\0'};
	int i, j, k;
	unsigned long val;
	...
	if( !data || !raw_limit_lower || ! raw_limit_upper )
		return -EINVAL;
	...
	if (!strncmp(temp, "min", 3)){
		for ( i = 0; i < numberOfRows*numberOfColums; ++i){
			for ( j = 0; j < 4; ++j,++k){
				temp[j] = data[k];
			}
			if (kstrtoul(temp, 10, &val))
				return -EINVAL;
			raw_limit_lower[i] = (short)val;
			k += 2;
		}
	}
...
static int synaptics_open_short_test(struct device *dev, struct seq_file *s, const struct firmware *fw)
{
	int retval = -ENOMEM;
	int i, j, k;
	unsigned short numberOfRows = f54->tx_assigned;
	...
	short *p_data;
	short *report_data;
	short *raw_limit_lower;
	short *raw_limit_upper;

	raw_limit_lower = kmalloc(numberOfRows * numberOfColums * 2, GFP_KERNEL);
	if (!raw_limit_lower)
		return -ENOMEM;
	memset(raw_limit_lower, 0, sizeof(*raw_limit_lower));
	raw_limit_upper = kmalloc(numberOfRows * numberOfColums * 2, GFP_KERNEL);
	if (!raw_limit_upper) {
		goto free_lower;
	}
	memset(raw_limit_upper, 0, sizeof(*raw_limit_upper));

	retval = test_sysfs_read_report(NULL, NULL, "92", 2, true, true);
	if(retval < 0) {
		dev_err(dev, "%s: open_short_test failed with errno=%d\n", __func__,retval);
		goto free_upper;
	}

	p_data = kzalloc(numberOfRows * numberOfColums * 2,  GFP_KERNEL);
	if (unlikely(!p_data)){
		retval = -ENOMEM;
		goto free_upper;
	}
	...
	retval = raw_limit_save(fw->data, raw_limit_lower, raw_limit_upper);
	if(retval){
		dev_err(dev, "%s: save raw limit value failed \n", __func__);
		goto free_pdata;
	}
	...
	for (i = 0; i < numberOfRows; i++) {
		for (j = 0; j < numberOfColums; j++) {
			if ((p_data[i * numberOfColums + j] <  raw_limit_lower[k]) ||
						(p_data[i * numberOfColums + j] > raw_limit_upper[k])) {
				retval = 0;
				dev_err(dev, "%s: point(%d, %d) = %d\n",
						__func__, i, j, p_data[i * numberOfColums + j]);
				seq_printf(s, "fail point(%d, %d) = %d\n", i, j, p_data[i * numberOfColums + j]);
			}
			k++;
		}
	}
...
free_upper:
	if (raw_limit_upper)
		kfree(raw_limit_upper);
free_lower:
	if (raw_limit_lower)
		kfree(raw_limit_lower);
...
static int synaptics_rmi4_test_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmi4_data->tid->tid_ops->open_short_test = synaptics_open_short_test;
