...
static int fwu_get_version(struct device *dev, int *major, int *minor)
{
	int retval;
	unsigned char config_id_size;
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
	...
	retval = synaptics_rmi4_reg_read(rmi4_data, fwu->f34_fd.ctrl_base_addr,
									 fwu->config_id, config_id_size);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent, "%s: Get config id fail\n",
				__func__);
		return retval;
	}
	if (minor)
		*minor = fwu->config_id[config_id_size - 1];
	if (major)
		*major = fwu->bootloader_id[0];
...
static int get_lockdown_info(struct device *dev, char *buf)
{
	...
	lockdown_data[12] = '\0';
	dev_info(rmi4_data->pdev->dev.parent, "%s: lockdown data = %s\n",
				__func__, lockdown_data);

	buf[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = lockdown_data[4];
	buf[LOCKDOWN_INFO_DISPLAY_MAKER_INDEX] = lockdown_data[5];
	buf[LOCKDOWN_INFO_PANEL_COLOR_INDEX] = lockdown_data[7];
	buf[LOCKDOWN_INFO_CG_MAKER_INDEX] = lockdown_data[6];
	buf[LOCKDOWN_INFO_HW_VERSION_INDEX] = lockdown_data[8];

	dev_info(rmi4_data->pdev->dev.parent, "%s: lockdown info = %s\n",
				__func__, buf);
	kfree(lockdown_data);
	mutex_unlock(&fwu_sysfs_mutex);

/*  read lockdown_info in lcd_id way
*	struct device_node *np = dev_of_node(rmi4_data->pdev->dev.parent);
*	int gpio = of_get_named_gpio(np, "synaptics,lcdid-gpio", 0);
*	int ret;
*
*	if (gpio < 0) {
*		dev_err(rmi4_data->pdev->dev.parent, "%s: Get Lcd id node fail\n", __func__);
*		return gpio;
*	}
*	ret = gpio_request(gpio, "lcdid");
*	if (ret) {
*		dev_err(rmi4_data->pdev->dev.parent, "%s: Get Lcd id fail\n", __func__);
*		return ret;
*	}
*	gpio_direction_input(gpio);
*	if (gpio_get_value(gpio))
*		buf[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = 0x51;
*	else
*		buf[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = 0x50;
*	gpio_free(gpio);
*/
...
static int fwu_recovery_write_chunk(void)
{
	...
	for (chunk = 0; chunk < chunk_total; chunk++) {
		...
		if( retval < 0){
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to copy data, error_code = %d\n",
					__func__, retval);
			return retval;			
		}
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	if (tid_upgrade_firmware_nowait(rmi4_data->tid))
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to invoke firmware loader\n",
				__func__);
...
static int firmware_upgrade(struct device *dev, const struct firmware *fw,
							bool is_force)
{
	int ret;

	fwu->force_update = is_force;
	ret = synaptics_fw_updater(fw->data);
	if (ret < 0) {
		dev_err(dev->parent, "%s: Failed to do reflash\n",
				__func__);
		return ret;
	}
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmi4_data->tid->tid_ops->get_version = fwu_get_version;
	rmi4_data->tid->tid_ops->firmware_upgrade = firmware_upgrade;
	rmi4_data->tid->tid_ops->get_lockdown_info = get_lockdown_info;

	retval = devm_tid_register(&rmi4_data->pdev->dev, rmi4_data->tid);
