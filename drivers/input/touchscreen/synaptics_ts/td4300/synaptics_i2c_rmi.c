		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		if (finger_status != OBJECT_NOT_PRESENT) {
			...
			input_report_abs(rmi4_data->input_dev, ABS_MT_CUSTOM,
					...
					struct i2c_client *client)
{
	...
	input_info(true, dev, "%s: firmware: %s, recovery_fw: %s", __func__, pdata->firmware_name, pdata->firmware_name_bl);
}
...
*/
	input_info(false, &rmi4_data->i2c_client->dev, "%s\n", __func__);
