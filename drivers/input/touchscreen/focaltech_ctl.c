...
static int ft_rw_iic_drv_mywrite(struct i2c_client *client,
					u8 *buf, int length)
{
	int ret = 0;
...
static int ft_rw_iic_drv_RDWR(struct i2c_client *client, unsigned long arg)
{
	...
	if (!access_ok(VERIFY_READ,
		(struct ft_rw_i2c_queue *)arg, sizeof(struct ft_rw_i2c_queue)))
		return -EFAULT;
	...
	data_ptrs = kmalloc(i2c_rw_queue.queuenum * sizeof(u8 __user *),
				GFP_KERNEL);
	if (data_ptrs == NULL) {
		kfree(i2c_rw_msg);
		return -ENOMEM;
...
static long ft_rw_iic_drv_ioctl(struct file *filp,
