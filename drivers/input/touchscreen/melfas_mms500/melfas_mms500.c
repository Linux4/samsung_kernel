...
void create_flash_count(struct work_struct *work);
void write_flash_count(struct work_struct *work);
...
static int mms_input_open(struct input_dev *dev)
{
	...
	if (!info->init) {
		...
		input_info(true, &info->client->dev, "%s TSP irq is enabled(depth: %d, wakeup: %d, count: %d)", __func__, desc->depth, desc->wake_depth, desc->irq_count);
	}
 ...
 * Read chip name 
 ...
 mms549 IC Name : "M5H0"
 */
int mms_get_chip_name(struct mms_ts_info *info)
{
	u8 rbuf[4];
	u8 wbuf[2];
	int i;
	...
	for (i = 0; i < 4; i++) {
		info->chip_name[i] = rbuf[i];
	}

	info->chip_name[4] = 0;

	input_info(true, &info->client->dev,
			"%s: chip_name:%s, %c%c%c%c\n",
			__func__, info->chip_name, rbuf[0], rbuf[1], rbuf[2], rbuf[3]);
...
/*
* Update target pages of firmware from kernel built-in binary
*/
int mip_ts_fw_page_update_from_kernel(struct mms_ts_info *info, int addr, int length, int retry)
{
	...
	dev_dbg(&info->client->dev, "%s - request_firmware\n", __func__);
	...
	//Update firmware
	while (retry > 0) {
		ret = mip_ts_flash_fw_page(info, fw->data, fw->size, addr, length);
		dev_err(&info->client->dev, "%s - mip4_ts_flash_fw_page[%d]\n", __func__, ret);
		if (ret >= fw_err_none) {
			break;
		}
...
/*
* Alert event handler - Flash 
*/
static int mip_ts_alert_handler_flash(struct mms_ts_info *info, u8 *rbuf)
{
	int ret = 0;
	int retry = 5;
	int addr;
	...
	addr_kb = rbuf[2] | (rbuf[3] << 8);
	length_kb = rbuf[4] | (rbuf[5] << 8);
	dev_info(&info->client->dev, "%s - addr[%dK] length[%dK]\n", __func__, addr_kb, length_kb);

	addr = addr_kb * 1024;
	length = length_kb * 1024;

	ret = mip_ts_fw_page_update_from_kernel(info, addr, length, retry);
	if (ret < fw_err_none) {
		dev_err(&info->client->dev, "%s - mip4_ts_fw_page_update_from_kernel[%d]\n", __func__, ret);
		goto error;
	}

	dev_info(&info->client->dev, "%s [DONE]\n", __func__);

	info->flash_alert_cnt++;
	if (info->create_file == 1)
		schedule_delayed_work(&info->work_write_flash_count, msecs_to_jiffies(1));
...
static irqreturn_t mms_interrupt(int irq, void *dev_id)
{
	...
	if(info->lowpower_mode && info->ic_status >= LPM_RESUME)
	{
		...
		if(alert_type & MMS_LPM_FLAG_SPAY) {
			...
			if (category == 0) {
				...
				//Alert event
				if (rbuf[0] == MIP_ALERT_FLASH) {
					//Flash
					input_info(true, &client->dev, "%s: [Error Page]\n", __func__);
					if (mip_ts_alert_handler_flash(info, rbuf)) {
						goto ERROR;
					}
	...
	if (category == 0) {
		...
		input_info(true, &client->dev, "%s - alert type [%d]\n", __func__, alert_type);
		...
		} else if (alert_type == MIP_ALERT_FLASH) {
			//Flash
			input_info(true, &client->dev, "%s: [Error Page]\n", __func__);
			if (mip_ts_alert_handler_flash(info, rbuf)) {
				goto ERROR;
...
#define PATH_COUNT	"/efs/FactoryApp/tsp_count"
#define FILE_SIZE	32
...
{
	struct mms_ts_info *info = container_of(work, struct mms_ts_info,
						work_flash_count.work);

	struct file *file_count = NULL;
	mm_segment_t old_fs;
	int ret = 0;
	int c_data = 0;
	char read_count[FILE_SIZE] = { 0 };

	wake_lock(&info->wakelock_write);

	old_fs = get_fs();
	set_fs(KERNEL_DS);

	file_count = filp_open(PATH_COUNT, O_RDWR | O_CREAT, 0660);
	if (IS_ERR(file_count)) {
		set_fs(old_fs);
		ret = PTR_ERR(file_count);
		input_err(true, &info->client->dev, "%s: open fail :%d\n", __func__, ret);
		schedule_delayed_work(&info->work_flash_count, msecs_to_jiffies(2000));

		wake_unlock(&info->wakelock_write);

		return;
	}

	file_count->f_op->read(file_count, read_count, sizeof(read_count), &file_count->f_pos);

	c_data = atoi(read_count);

	input_info(true, &info->client->dev, "%s: read data: %s, copy value:%d\n", __func__, read_count, c_data);

	info->flash_alert_cnt += c_data;

	filp_close(file_count, current->files);
	set_fs(old_fs);
...
{
	struct mms_ts_info *info = container_of(work, struct mms_ts_info,
						work_write_flash_count.work);

	struct file *file_count = NULL;
	mm_segment_t old_fs;
	int ret = 0;
	char write_count[FILE_SIZE] = { 0 };

	input_info(true, &info->client->dev, "%s: start, write:%d\n", __func__, info->flash_alert_cnt);
	...
	file_count = filp_open(PATH_COUNT, O_TRUNC | O_RDWR | O_CREAT, 0660);
	if (IS_ERR(file_count)) {
		set_fs(old_fs);
		ret = PTR_ERR(file_count);
		input_err(true, &info->client->dev, "%s: open fail :%d\n", __func__, ret);
	...
	snprintf(write_count, FILE_SIZE, "%d", info->flash_alert_cnt);

	file_count->f_op->write(file_count, write_count, sizeof(write_count), &file_count->f_pos);
	...
	input_info(true, &info->client->dev, "%s: end, write:%d\n", __func__, info->flash_alert_cnt);
}
...
static int mms_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&info->work_flash_count, create_flash_count);
	INIT_DELAYED_WORK(&info->work_write_flash_count, write_flash_count);
	wake_lock_init(&info->wakelock_write, WAKE_LOCK_SUSPEND, "mms_wakelock_write_flash");
	schedule_delayed_work(&info->work_flash_count, msecs_to_jiffies(2000));
...
err_mem_alloc:
ERROR:
	input_err(true, &client->dev, " Touchscreen initialization failed\n");
	return ret;
}
...
static int mms_remove(struct i2c_client *client)
{
	...
	cancel_delayed_work(&info->work_flash_count);
	...
	wake_lock_destroy(&info->wakelock);
	wake_lock_destroy(&info->wakelock_write);
...
MODULE_DESCRIPTION("MELFAS MMS500 Touchscreen");
