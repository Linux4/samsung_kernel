...
#ifdef CONFIG_SAMSUNG_PRODUCT_SHIP
				input_info(true, &client->dev,
					"P[%d] loc:%c%c z:%d p:%d m:%d,%d tc:%d fc:%d\n",
					id, location_detect(info, y, 1), location_detect(info, x, 0),
					pressure, palm, touch_major, touch_minor, info->touch_count, info->flash_alert_cnt);
#else
				input_err(true, &client->dev,
					"P[%d] (%d, %d) z:%d p:%d m:%d,%d tc:%d fc:%d\n",
					id, x, y, pressure, palm, touch_major, touch_minor, info->touch_count, info->flash_alert_cnt);
#endif
			}
		}
	}
