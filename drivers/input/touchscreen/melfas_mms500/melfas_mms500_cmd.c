...
/**
 * Command : Get result of cp short test
 */
static void cmd_get_cp_short(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct mms_ts_info *info = container_of(sec, struct mms_ts_info, sec);
	char buf[SEC_CMD_STR_LEN] = { 0 };
	...
	if(x > 0 && x <= info->node_x)
		idx = x + info->node_y;
	else if(y > 0 && y <= info->node_y)
		idx = y;
...
/**
 * Command : Get result of cp lpm test
 */
static void cmd_get_cp_lpm(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct mms_ts_info *info = container_of(sec, struct mms_ts_info, sec);
	char buf[SEC_CMD_STR_LEN] = { 0 };
...
static void get_self_cp_short_all_data(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct mms_ts_info *info = container_of(sec, struct mms_ts_info, sec);
	int ret;
...
static void get_self_cp_lpm_all_data(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct mms_ts_info *info = container_of(sec, struct mms_ts_info, sec);
	int ret;
...
/**
 * Command : Test flash memory
 */
static void cmd_test_flash(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct mms_ts_info *info = container_of(sec, struct mms_ts_info, sec);
	char buf[SEC_CMD_STR_LEN] = { 0 };
...
EXIT:
	...
	//Download normal firmware
	for (i = 0; i < 3; i++) {
		mms_reboot(info);
		ret_update = mms_fw_update_from_kernel(info, true);
		if (ret_update >= 0) {
			break;
		} else {
			sprintf(buf, "%s", "NG");
...
static struct sec_cmd mms_commands[] = {
	...
	{SEC_CMD("get_cp_short", cmd_get_cp_short),},
	{SEC_CMD("run_cp_lpm_read", cmd_run_test_cp_lpm),},
	{SEC_CMD("get_cp_lpm", cmd_get_cp_lpm),},
	...
	{SEC_CMD("get_self_cp_short_all_data", get_self_cp_short_all_data),},
	{SEC_CMD("get_self_cp_lpm_all_data", get_self_cp_lpm_all_data),},	
	...
	{SEC_CMD("test_flash", cmd_test_flash),},
