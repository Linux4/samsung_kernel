...
static void delayed_start_cpufreq(struct work_struct *work)
{
			start_cpufreq();
			printk(KERN_INFO "%s(): start_cpufreq\n", __func__);
			dfs_started = 1;
}

static void setting_cpufreq(struct cyttsp4_mt_data *data,uint32_t on)
{
	if ( on == 0 )
	{
		if (!dfs_started)
			schedule_delayed_work(&data->delayed_cpufreq_work, msecs_to_jiffies(1000));
	}
	else if ( on == 1 )
	{
		cancel_delayed_work_sync(&data->delayed_cpufreq_work);
		if (dfs_started) {
			int ret = 0;
			ret = stop_cpufreq();
			printk(KERN_INFO "%s(): stop_cpufreq\n", __func__);
			if (ret)
				printk(KERN_INFO "%s(): error<%d>! stop_cpufreq\n", __func__, ret);
			else
				dfs_started = 0;
		}
	}
	else
		printk(KERN_INFO "%s(): error! setting_cpufreq\n", __func__);
}
#endif
...
#if defined(TOUCH_BOOSTER_SHMOBILE)	
	INIT_DELAYED_WORK(&md->delayed_cpufreq_work, delayed_start_cpufreq);
#endif
