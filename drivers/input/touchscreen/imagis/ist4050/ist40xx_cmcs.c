...
int ist40xx_set_cmcs_fw(struct ist40xx_data *data, CMCS_PARAM param, u32 *buf32)
{
	...
	ret = ist40xx_write_buf(data->client, waddr, (u32 *)&val, 1);
	if (ret)
		return ret;
		...
		u32 *buf32, u32 mode)
{
    ...
    ret = ist40xx_write_buf(data->client, waddr, (u32 *)&mode, 1);
	if (ret)
		return -EPERM;
...
int ist40xx_cmcs_test(struct ist40xx_data *data, u32 mode)
{
	...
	write_data = IST40XX_ENABLE;
	ret = ist40xx_write_buf(data->client, eHCOM_RUN_RAMCODE, (u32 *)&write_data, 1);
	cmcs_next_step(ret);
