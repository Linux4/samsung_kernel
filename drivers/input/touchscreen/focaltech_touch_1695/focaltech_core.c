...
#endif
/* Huaqin add for AR-ZQL1695-01000000051 ito test by liufurong at 2019/07/15 end */
...
static ssize_t fts_proc_getinfo_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	/* HS60 modify for HS60-86 by liufurong at 20190808 start */
	 if ( fwupgrade != NULL && fwupgrade->module_info != NULL) {
		snprintf(buf, 150, "IC=FT5446 module=%s fw_ver= %x \n",fwupgrade->module_info->vendor_name,fwver);
	 } else {
		snprintf(buf, 150, "IC=FT5446 module=NULL fw_ver= %x \n",fwver);
	 }
	 /* HS60 modify for HS60-86 by liufurong at 20190808 end */
...
#if FTS_TEST_EN
static ssize_t zh_fts_test_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	char fwname[128] = {0};
	int count = 0;
...
#endif
	fts_test_entry( fwname);//ito test
...
static ssize_t zh_fts_test_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	return count;
}

static DEVICE_ATTR(factory_check, 0644, zh_fts_test_show, zh_fts_test_store);

static struct attribute *zh_test_attributes[] ={
	&dev_attr_factory_check.attr,
	NULL
};

static struct attribute_group zh_test_attribute_group = {
.attrs = zh_test_attributes
};

int fts_test_node_init(struct platform_device *tpinfo_device)
{
    int err=0;
    err = sysfs_create_group(&tpinfo_device->dev.kobj, &zh_test_attribute_group);
    if (0 != err)
    {
        FTS_ERROR( "ERROR: ITO node create failed.");
        sysfs_remove_group(&tpinfo_device->dev.kobj, &zh_test_attribute_group);
        return -EIO;
    }
    else
    {
...
static int fts_input_report_b(struct fts_ts_data *data)
{
    ...
    for (i = 0; i < data->touch_point; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            ...
            input_report_abs(data->input_dev, ABS_MT_CUSTOM, events[i].palm);
            ...
            if ((data->log_level >= 2) ||
                ...
                FTS_DEBUG("[B]P%d(%d, %d)[p:%d,mj:%d,mn:%d,palm:%d] DOWN!",
                          events[i].id,
                          events[i].x, events[i].y,
                          events[i].p, events[i].major,events[i].minor,events[i].palm);
#else
		FTS_DEBUG("[B]P%d [p:%d,mj:%d,mn:%d,palm:%d] DOWN!",
                          events[i].id, events[i].p, events[i].major,events[i].minor,events[i].palm);
#endif
            }
        } else {
            uppoint++;
...
static int fts_read_parse_touchdata(struct fts_ts_data *data)
{
    ...
    data->palm_flag = buf[FTS_TOUCH_POINT_NUM] & 0xF0;
    ...
    for (i = 0; i < max_touch_num; i++) {
       ...
       if(data->palm_flag == 96)//Capture Screen
        {
            events[i].palm = 1;
            events[i].major = 300;
            events[i].minor =  100;
        }
        else if(data->palm_flag == 64)//Palm Touch
        {
            events[i].palm = 1;
            events[i].major = 300;
            events[i].minor =  300;
...
static int fts_power_source_init(struct fts_ts_data *ts_data)
{
    ...
    ret = regulator_set_load(ts_data->vdd,15000);
        if (ret < 0) {
            FTS_ERROR("Failed to request vdd current:%d\n",ret);
    }
...
#endif
/* HS60 modiify for HS60-3634 by liufurong at 20191103 start */
#if (!FTS_POWER_SOURCE_ALWAYSON)
    ret = fts_power_source_ctrl(ts_data, DISABLE);
    if (ret < 0) {
        FTS_ERROR("power off fail, ret=%d", ret);
    }
#endif
/* HS60 modiify for HS60-3634 by liufurong at 20191103 end */
    return ret;
}
...
/* Huaqin add for AR-ZQL1695-01000000051 ito test by liufurong at 2019/07/15 start */
    ...
    fts_test_node_init(&hwinfo_device);
...
{
    FTS_FUNC_ENTER();
    if (ts_data == NULL || ts_data->pdata == NULL)
        return 0;
#if FTS_POINT_REPORT_CHECK_EN
...
static int fts_ts_resume(struct device *dev)
{
    ...
    if (fts_gesture_resume(ts_data) == 0) {
        ...
        /* HS60 modify for HS60-3422 by liufurong at 2019/10/30 start */
        fts_write_reg(0x90,1);
        /* HS60 modify for HS60-3422 by liufurong at 2019/10/30 end */
        return 0;
    }
#endif
