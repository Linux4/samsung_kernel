...
static struct stm_ts_data *proc_ts = NULL;
...
//	u32 funcToTest[((count + 1) / 3)];
	...
	if (count > MESSAGE_MIN_HEADER_SIZE - 1 && p[0] == MESSAGE_START_BYTE) {
		...
		// pr_info("%s functionToTest[0] = %02X cmd[0]= %02X\n", tag, funcToTest[0], cmd[0]);
		for (; numberParam < (count + 1) / 3; numberParam++) {
			if (sscanf(p, "%02X ", &funcToTest[numberParam]) == 1) {
				p += 3;
				cmd[numberParam] = (u8)funcToTest[numberParam];
				// pr_info("%s functionToTest[%d] = %02X cmd[%d]= %02X\n", tag, numberParam, funcToTest[numberParam], numberParam, cmd[numberParam]);
			}				
		}
	}			
		...
		switch (funcToTest[0]) {
			...
			case CMD_WRITEREAD_BYTE:
				if (numberParam >= 5) {	/* need to pass: cmd[0]  cmd[1] … cmd[cmdLength-1] byteToRead1 byteToRead0 dummyByte */
					...
					// pr_info("%s bytesToRead = %d, dummy = %d\n", tag, byteToRead + mess.dummy, mess.dummy);
					...
					res = proc_ts->stm_ts_spi_read(proc_ts, &cmd[1], temp, &readData[1], byteToRead + mess.dummy);
			...
			case CMD_WRITE_BYTE:
				if (numberParam >= 2) {	/* need to pass: cmd[0]  cmd[1] … cmd[cmdLength-1] */
					temp = numberParam - 1;
					res = proc_ts->stm_ts_spi_write(proc_ts, &cmd[1], temp, NULL, 0);
				} else {
					pr_info("%s Wrong number of parameters!\n", tag);
					res = ERROR_OP_NOT_ALLOW;
				}
...
END:	/* here start the reporting phase, assembling the data to send in the
	...
	// pr_info("%s Size = %d byte_to_read = %d dummy = %d\n", tag, size, byteToRead, mess.dummy);

	driver_test_buff = (u8 *)kzalloc(size, GFP_KERNEL);

	// pr_info("%s Finish to allocate memory!\n", tag);
