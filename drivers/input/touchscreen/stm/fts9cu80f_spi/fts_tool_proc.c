...
static struct fts_ts_info *proc_info = NULL;
		...
		switch (funcToTest[0]) {
			...
			case CMD_WRITEREAD_BYTE:
				if (numberParam >= 5) {	/* need to pass: cmd[0]  cmd[1] … cmd[cmdLength-1] byteToRead1 byteToRead0 dummyByte */
					...
					res = proc_info->fts_read_reg(proc_info, &cmd[1], temp, &readData[1], byteToRead + mess.dummy);
			...
			case CMD_WRITE_BYTE:
				if (numberParam >= 2) {	/* need to pass: cmd[0]  cmd[1] … cmd[cmdLength-1] */
					temp = numberParam - 1;
					res = proc_info->fts_write_reg(proc_info, &cmd[1], temp);
				} else {
					pr_info("%s Wrong number of parameters!\n", tag);
					res = ERROR_OP_NOT_ALLOW;
				}
...
int fts_tool_proc_init(struct fts_ts_info *info)
{
	...
	proc_info = (struct fts_ts_info *) info;
