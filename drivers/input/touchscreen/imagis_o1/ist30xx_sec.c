...
static void boost_level(void *dev_data)
{
	...
	if (data->tsp_booster->dvfs_boost_mode == DVFS_STAGE_NONE) {
		retval = data->tsp_booster->dvfs_off(data->tsp_booster);
		if (retval < 0) {
			dev_err(&data->client->dev,
...
int sec_touch_sysfs(struct ist30xx_data *data)
{
	...
	sec_touchkey = device_create(sec_class, NULL, SEC_CLASS_DEVT_TKEY, NULL, "sec_touchkey");
	...
	/* /sys/class/sec/tsp */
	sec_fac_dev = device_create(sec_class, NULL, SEC_CLASS_DEVT_TSP, data, "tsp");
...
int sec_fac_cmd_exit(struct ist30xx_data *data)
{
	struct sec_factory *sec = (struct sec_factory *)&data->sec;
...
EXPORT_SYMBOL(sec_fac_cmd_exit);
