...
extern int sec_fac_cmd_exit(struct ist30xx_data *data);
#endif
			      ...
			      int key_counts)
{
	...
	for (id = 0; id < IST30XX_MAX_MT_FINGERS; id++) {
		...
		if (data->tsp_booster->dvfs_set)
			data->tsp_booster->dvfs_set(data->tsp_booster, finger_counts);
#endif
	}
...
static int ist30xx_parse_dt(struct device *dev, struct ist30xx_data *data)
{
	...
	data->dt_data->firm_flag = of_property_read_bool(np, "imagis,firm-flag");
	...
	tsp_info("%s() irq:%d, touch_en:%d, keyled_en:%d, firm_flag:%d, project:%s\n",
		 __func__, data->dt_data->irq_gpio, data->dt_data->touch_en_gpio,
		 data->dt_data->keyled_en_gpio, data->dt_data->firm_flag, data->dt_data->pname);
