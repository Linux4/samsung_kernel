...
int ist30xx_fw_update(struct i2c_client *client, const u8 *buf, int size)
{
	...
	if(data->dt_data->firm_flag == 0) {
		tsp_info("Do not binary firmware update with old HW\n");
		goto end;
	}
...
int ist30xx_check_auto_update(struct ist30xx_data *data)
{
	...
	if(data->dt_data->firm_flag == 0) {
		tsp_info("Do not firmware update with old HW\n");
		return 0;
	}
...
static DEVICE_ATTR(firmware, UPDATE_DEFAULT_ATTR, ist30xx_fw_status_show,
		   ist30xx_fw_store);
static DEVICE_ATTR(fw_sdcard, UPDATE_DEFAULT_ATTR, ist30xx_fw_sdcard_show, NULL);
