...
#if !defined(CONFIG_DRV_SAMSUNG) && !defined(CONFIG_SEC_SYSFS)
...
static ssize_t sec_cmd_show_support_feature(struct device *dev,
				 struct device_attribute *devattr, char *buf)
{
	struct sec_cmd_data *data = dev_get_drvdata(dev);
	...
	buf[0]= INPUT_FEATURE_ENABLE_SETTINGS_AOT;
	buf[1] = '\n';
...
static DEVICE_ATTR(support_feature, 0444, sec_cmd_show_support_feature, NULL);
