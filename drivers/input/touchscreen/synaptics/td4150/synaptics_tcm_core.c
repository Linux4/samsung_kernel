...
int syna_tcm_wait_hdl(struct syna_tcm_hcd *tcm_hcd)
{
	int retval;
...
#if 0
void syna_tcm_debug_lpwg_doubletap(struct syna_tcm_hcd *tcm_hcd)
{
	int retval;
	...
	input_info(true, tcm_hcd->pdev->dev.parent, "double tap information:\n");
	for (i = 0; i < resp_length; i++) {
		input_info(true, tcm_hcd->pdev->dev.parent, "0x%02x,  ", resp_buf[i]);
	}
	input_info(true, tcm_hcd->pdev->dev.parent, "double tap information end\n");
exit:
	if (resp_buf)
		kfree(resp_buf);
	return;
...
#endif
int syna_tcm_early_resume(struct device *dev)
{
	int retval = 0;
...
//		syna_tcm_debug_lpwg_doubletap(tcm_hcd);
...
/*	tcm_hcd->wakeup_gesture_enabled = WAKEUP_GESTURE; */
	...
	if (lpcharge) {
		input_info(true, &pdev->dev, "%s: Do not load driver in lpcharge %d\n", __func__, lpcharge);
		pinctrl_configure(tcm_hcd, false);
		retval = -ENODEV;
		goto err_get_lcd_regulator;
	}
