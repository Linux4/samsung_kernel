		...
		bool config, int dir, int state)
{
	...
	if (config) {
		...
		// //gpio_mode = gpio_get_mode(gpio);
		// //LOGE(tcm_hcd->pdev->dev.parent,
		// 			"GZW gpio_mode = %d\n", gpio_mode);
		// if (mode == 1) {
		// 	gpio_set_mode(gpio, 1);
		// 	gpio_mode = gpio_get_mode(gpio);
		// 	LOGE(tcm_hcd->pdev->dev.parent,
		// 			"GZW1 gpio_mode = %d\n", gpio_mode);
		// }
...
static int syna_tcm_config_gpio(struct syna_tcm_hcd *tcm_hcd)
{
	...
	// if (bdata->i2c_scl_gpio >= 0) {
	// 	retval = syna_tcm_set_gpio(tcm_hcd, bdata->i2c_scl_gpio,
	// 			true, 0, 0, 1);
	// 	if (retval < 0) {
	// 		LOGE(tcm_hcd->pdev->dev.parent,
	// 				"Failed to configure interrupt GPIO\n");
	...
	// if (bdata->i2c_sda_gpio >= 0) {
	// 	retval = syna_tcm_set_gpio(tcm_hcd, bdata->i2c_sda_gpio,
	// 			true, 0, 0,1);
	// 	if (retval < 0) {
	// 		LOGE(tcm_hcd->pdev->dev.parent,
	// 				"Failed to configure interrupt GPIO\n");
	...
	if (bdata->reset_gpio >= 0) {
		...
		// retval = gpio_get_value(bdata->reset_gpio);
		// LOGE(tcm_hcd->pdev->dev.parent,
		// 			"gzw reset status = %d\n", retval);
	}
