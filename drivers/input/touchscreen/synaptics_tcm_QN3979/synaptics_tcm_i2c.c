...
static int parse_dt(struct device *dev, struct syna_tcm_board_data *bdata)
{
	...
	prop = of_find_property(np, "synaptics,i2c-sda-gpio", NULL);
	if (prop && prop->length) {
		bdata->i2c_sda_gpio = of_get_named_gpio_flags(np,
				"synaptics,i2c-sda-gpio", 0, NULL);
	} else {
		bdata->i2c_sda_gpio = -1;
	}

	prop = of_find_property(np, "synaptics,i2c-scl-gpio", NULL);
	if (prop && prop->length) {
		bdata->i2c_scl_gpio = of_get_named_gpio_flags(np,
				"synaptics,i2c-scl-gpio", 0, NULL);
	} else {
		bdata->i2c_scl_gpio = -1;
	}
