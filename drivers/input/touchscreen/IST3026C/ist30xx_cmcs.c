...
int ist30xx_parse_cmcs_bin(const u8 *buf, const u32 size)
{
	...
	if (!strncmp(ts_cmcs->magic1, IST30XX_CMCS_MAGIC, sizeof(ts_cmcs->magic1))
		...
		ts_cmcs->buf_cm_min = (u16 *)&buf[idx];

        idx += (cmcs->ch.rx_num * cmcs->ch.tx_num) * sizeof(s16);
        ts_cmcs->buf_cm_max = (u16 *)&buf[idx];
    ...
    tsp_verb(" key num: %d,ch1(%d,%d),ch2(%d,%d),ch3(%d,%d),ch4(%d,%d),ch5(%d,%d)\n", 
         cmcs->ch.key_use_num, 
	...
	tsp_verb(" cm_min: %x, %x, %x, %x\n",
		 ts_cmcs->buf_cm_min[0], ts_cmcs->buf_cm_min[1],
		 ts_cmcs->buf_cm_min[2], ts_cmcs->buf_cm_min[3]);
	tsp_verb(" cm_max: %x, %x, %x, %x\n",
		 ts_cmcs->buf_cm_max[0], ts_cmcs->buf_cm_max[1],
		 ts_cmcs->buf_cm_max[2], ts_cmcs->buf_cm_max[3]);
			    ...
			    u32 *buf32)
{
	...
	waddr = IST30XX_DA_ADDR(cmcs->addr.sensor1);
	len = (cmcs->sensor1_size / IST30XX_DATA_LEN) - 2;
    buf32 += 2;
    ...
    if (len > 0) {
        ...
        tsp_info("sensor reg1 loaded!\n");
    }

    buf32 += len;
	waddr = IST30XX_DA_ADDR(cmcs->addr.sensor2);
    len = cmcs->sensor2_size / IST30XX_DATA_LEN;
    tsp_verb("%08x %08x %08x %08x\n", buf32[0], buf32[1], buf32[2], buf32[3]);
    tsp_verb("%08x(%d)\n", waddr, len);
    ...
    if (len > 0) {
        ...
        tsp_info("sensor reg2 loaded!\n");
    }

    buf32 += len;
	waddr = IST30XX_DA_ADDR(cmcs->addr.sensor3);
...
int ist30xx_set_cmcs_cmd(struct i2c_client *client, CMCS_INFO *cmcs)
{
    ...
    waddr = IST30XX_DA_ADDR(cmcs->addr.base_screen);
    ...
    waddr = IST30XX_DA_ADDR(cmcs->addr.base_key);
    ...
    waddr = IST30XX_DA_ADDR(cmcs->addr.start_cp);
    ...
    waddr = IST30XX_DA_ADDR(cmcs->addr.vcmp);
    ...
    waddr = IST30XX_DA_ADDR(cmcs->addr.cmcs_size);
    val = cmcs->cmd.cmcs_size - 4;
	ret = ist30xx_write_cmd(client, waddr, val);
	if (ret)
		return ret;
...
int ist30xx_apply_cmcs_slope(CMCS_INFO *cmcs, CMCS_BUF *cmcs_buf)
{
	...
	s16 *pcm_spec_min = (s16 *)cmcs_buf->cm_spec_min;
    s16 *pcm_spec_max = (s16 *)cmcs_buf->cm_spec_max;
	...
	memcpy(cmcs_buf->cm_spec_min, ts_cmcs->buf_cm_min, (ch_num * sizeof(u16)));
    memcpy(cmcs_buf->cm_spec_max, ts_cmcs->buf_cm_max, (ch_num * sizeof(u16)));
	...
	for (i = 0; i < cmcs->ch.tx_num; i++) {
		tsp_info(" ");
		for (j = 0; j < cmcs->ch.rx_num; j++)
			printk("%5d ", cmcs_buf->cm_spec_min[idx1++]);
		printk("\n");
	}
    ...
    for (i = 0; i < cmcs->ch.tx_num; i++) {
		tsp_info(" ");
		for (j = 0; j < cmcs->ch.rx_num; j++)
			printk("%5d ", cmcs_buf->cm_spec_max[idx1++]);
		printk("\n");
	}
#endif
	...
	for (i = 0; i < height; i++) {
		for (j = 0; j < width - 1; j++) {
			...
			pslope0[idx1] += 
                (((pcm_spec_min[idx1] + pcm_spec_max[idx1]) / 2) - 
                ((pcm_spec_min[idx2] + pcm_spec_max[idx2]) / 2));
		}
	}
...
#define IST30XX_CMCS_CM     (0)
#define IST30XX_CMCS_CS0    (1)
#define IST30XX_CMCS_CS1    (2)
int ist30xx_get_cmcs_buf(struct i2c_client *client, int mode, CMCS_INFO *cmcs, 
        s16 *buf)
{
	int ret;
	u16 len = (IST30XX_NODE_TOTAL_NUM * sizeof(buf[0])) / IST30XX_DATA_LEN;
    
	ret = ist30xx_burst_read(client, 
            IST30XX_CMCS_RESULT + (IST30XX_CMCS_RESULT_SIZE * mode), 
            (u32 *)buf, len, true);
	if (unlikely(ret))
		return ret;
...
int ist30xx_cmcs_test(struct ist30xx_data *data, const u8 *buf, int size)
{
	...
	ret = ist30xx_set_cmcs_sensor(data->client, cmcs, buf32);
	cmcs_next_step(ret);

    /* Set command */
	ret = ist30xx_set_cmcs_cmd(data->client, cmcs);
	cmcs_next_step(ret);

	/* Load cmcs test code */
	buf32 = (u32 *)ts_cmcs->buf_cmcs;
	len = cmcs->cmd.cmcs_size / IST30XX_DATA_LEN - 1;
	tsp_verb("%08x %08x %08x %08x\n", buf32[0], buf32[1], buf32[2], buf32[3]);
	...
	if (ENABLE_CM_MODE(cmcs->cmd.mode)) {
		...
		ret = ist30xx_get_cmcs_buf(data->client, 
                IST30XX_CMCS_CM, cmcs, ts_cmcs_buf->cm);
		cmcs_next_step(ret);
	...
	if (ENABLE_CS_MODE(cmcs->cmd.mode)) {
		...
		ret = ist30xx_get_cmcs_buf(data->client, 
                IST30XX_CMCS_CS0, cmcs, ts_cmcs_buf->cs0);
		cmcs_next_step(ret);

		/* Read CS1 data */
		ret = ist30xx_get_cmcs_buf(data->client, 
                IST30XX_CMCS_CS1, cmcs, ts_cmcs_buf->cs1);
		cmcs_next_step(ret);
	}
            ...
            struct device_attribute *attr, char *buf)
{
	...
	return print_cmcs(ts_cmcs_buf->cm_spec_min, buf);
}
            ...
            struct device_attribute *attr, char *buf)
{
	...
	return print_cmcs(ts_cmcs_buf->cm_spec_max, buf);
}
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < cmcs->ch.tx_num; i++) {
		for (j = 0; j < cmcs->ch.rx_num; j++) {
			...
			min_spec = ts_cmcs_buf->cm_spec_min[idx];
            max_spec = ts_cmcs_buf->cm_spec_max[idx];
