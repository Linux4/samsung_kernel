...
#if IST30XX_USE_KEY
    tkey->enable = (bool)(cfg_buf[0x39] & 1);
	tkey->key_num = (u8)cfg_buf[0x3A];
...
int ist30xx_fw_update(struct ist30xx_data *data, const u8 *buf, int size)
{
	...
	tsp_info(" core: %x, fw: %x, test: %x (addr: 0x%x ~ 0x%x)\n",
...
int ist30xx_auto_bin_update(struct ist30xx_data *data)
{
    ...
    tsp_info("IC: %x, Binary ver core: %x, fw: %x, test: %x\n",
...
update_bin:   // TSP is not ready / FW update
	tsp_info("Update version. fw(core, test): %x(%x, %x) -> %x(%x, %x)\n",
