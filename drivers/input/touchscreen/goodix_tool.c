 ...
 *	V1.5: 2013/02/21, update format and style
...
#define CMD_HEAD_LENGTH		(sizeof(cmd_head) - sizeof(u8 *))
...
struct st_cmd_head {
	...
	u8 flag_relation;	/* flag_val: 0:!=; 1:=; 2:>; 3:<; */
...
static u8 goodix_relation(u8 src, u8 dst, u8 rlt)
{
	u8 ret = 0;
...
static u8 goodix_confirm(void)
{
	...
	for (i = 0; i < cmd_head.times; i++) {
		...
		if (true == goodix_relation(buf[GTP_ADDR_LENGTH],
	...
	if (1 == cmd_head.wr) {
		...
		if (1 == cmd_head.flag) {
			if (FAIL == goodix_confirm()) {
				GTP_ERROR("[WRITE]goodix_confirm fail!");
				return FAIL;
			}
		} else if (2 == cmd_head.flag)
			GTP_DEBUG("[WRITE]need interrupt.");
				...
				int count, int *eof, void *data)
{
	...
	if (cmd_head.wr % 2)
		...
		/* Read error code */
		GTP_DEBUG("[READ]Read error code!");
