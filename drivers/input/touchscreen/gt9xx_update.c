...
static struct i2c_client *guitar_client;
static u8 got_file_flag;
u16 show_len;
u16 total_len;
				...
				struct st_fw_head *fw_head, u8 *path)
{
	...
	if (path) {
		 ...
		 * the config file & firmware file must be in the same path
		 * single or double.
		 */
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
	...
	while (retry++ < 5) {
		...
		if (fw_dsp_isp == NULL)
			...
			GTP_INFO("[burn_dsp_isp]Alloc %dk byte success", \
...
static u8 gup_burn_fw_ss51(struct i2c_client *client)
{
	...
	while (retry++ < 5) {
		...
		if (fw_ss51 == NULL)
			...
			GTP_INFO("[burn_fw_ss51]Alloc %dk byte success", \
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
	...
	while (retry++ < 5) {
		...
		if (fw_dsp == NULL)
			...
			GTP_INFO("[burn_fw_dsp]Alloc %dk byte success", \
						(FW_SECTION_LENGTH/1024));
			break;
		}
	}
	...
	GTP_DEBUG("[burn_fw_dsp]step8:send burn cmd move data to flash");
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
	...
	while (retry++ < 5) {
		...
		if (fw_boot == NULL)
			...
			GTP_INFO("[burn_fw_boot]Alloc %dk byte success", \
						(FW_BOOT_LENGTH / 1024));
			break;
		}
	}
	...
	/* step7:send burn cmd to move data to flash from sram */
	GTP_DEBUG("[burn_fw_boot]step7:send burn cmd to move data to flash");
...
s32 gup_update_proc(void *dir)
{
	...
	gup_leave_update_mode(guitar_client);
	GTP_MSLEEP(100);
