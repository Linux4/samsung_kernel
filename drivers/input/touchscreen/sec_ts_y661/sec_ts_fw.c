...
static void sec_ts_save_version_of_bin(struct sec_ts_data *ts, const struct fw_header *fw_hd)
{
...
static int sec_ts_flashwrite(struct sec_ts_data *ts, u32 mem_addr, u8 *mem_data, u32 mem_size, int retry)
{
	...
	for (page_idx = page_num - 1;; page_idx--) {
		...
		if (page_idx == 0) /* end condition (page_idx >= 0)   page_idx type unsigned int */
			break;
	}
...
static int sec_ts_firmware_update(struct sec_ts_data *ts, const u8 *data, size_t size,
		int bl_update, int restore_cal, int retry)
{
	int i;
	int ret;
	struct fw_header *fw_hd;
	struct fw_chunk *fw_ch;
	u8 fw_status = 0;
	u8 *fd = (u8 *)data;
	u8 tBuff[3];
#ifdef PAT_CONTROL
	...
	input_info(true, &ts->client->dev, "%s: cal_count(0x%02X) pat_function dt(%d) "
			"restore_cal(%d) magic_cal(%d)\n",
