...
#ifndef MEDIATEK_LOCAL_BRANCH
#include <linux/sec_class.h>
#else
static struct class *sec_class;
static atomic_t sec_dev;

static struct device *sec_device_create(void *drvdata, const char *fmt)
{
	struct device *dev;

	if (IS_ERR_OR_NULL(sec_class)) {
		sec_class = class_create(THIS_MODULE, "sec_ts_y661");
		if (IS_ERR_OR_NULL(sec_class)) {
...
static void sec_device_destroy(dev_t devt)
{
	pr_info("%s : %d\n", __func__, devt);
...
MODULE_DESCRIPTION("factory command");
MODULE_LICENSE("GPL");
