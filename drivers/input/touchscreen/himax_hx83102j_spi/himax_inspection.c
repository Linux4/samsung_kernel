...
static void himax_set_N_frame(uint16_t Nframe, uint8_t checktype)
{
	...
	hx_s_core_fp._register_write(ADDR_SET_FRAME, tmp_data, 4);
...
static uint32_t himax_wait_sorting_mode(uint8_t checktype)
{
	...
	do {
		...
		if (hx_s_ts->debug_log_level & BIT(4)) {
			hx_s_core_fp._register_read(
				hx_s_ic_setup._addr_chk_fw_status, tmp_data, 4);
			I(TEMP_LOG, __func__, hx_s_ic_setup._addr_chk_fw_status,
				tmp_data[0], tmp_data[1],
				tmp_data[2], tmp_data[3]);

			hx_s_core_fp._register_read(
				ADDR_FLAG_RESET_EVENT, tmp_data, 4);
			I(TEMP_LOG, __func__, ADDR_FLAG_RESET_EVENT,
				tmp_data[0], tmp_data[1],
				tmp_data[2], tmp_data[3]);

			hx_s_core_fp._register_read(
				hx_s_ic_setup._addr_fw_dbg_msg, tmp_data, 4);
			I(TEMP_LOG, __func__, hx_s_ic_setup._addr_fw_dbg_msg,
				tmp_data[0], tmp_data[1],
				tmp_data[2], tmp_data[3]);
...
static void hx_print_fw_info(void)
{
	...
	hx_s_core_fp._register_read(ADDR_FW_CFG, data, DATA_LEN_4);
	config_ver = data[2] << 8 | data[3];
	touch_cfg_ver = data[2];
	display_cfg_ver = data[3];

	hx_s_core_fp._register_read(ADDR_FW_VENDOR, data, DATA_LEN_4);
	cid_maj_ver = data[2];
	cid_min_ver = data[3];

	hx_s_core_fp._register_read(ADDR_CUS_INFO, data, 12);
	memcpy(cus_info, data, 12);

	hx_s_core_fp._register_read(ADDR_PROJ_INFO, data, 12);
...
// -P86801AA1 peiyuexiang.wt,add,20230704,compatible with lide
	...
	for (i = 0; i < HX_CRITERIA_SIZE; i++) {
		g_inspection_criteria[i] = kcalloc((hx_s_ic_data->tx_num
				* hx_s_ic_data->rx_num),
				sizeof(int), GFP_KERNEL);
...
#if defined(HX_ZERO_FLASH)
	ret = hx_s_core_fp._0f_op_file_dirly(g_fw_mp_upgrade_name);
	if (ret) {
...
#if defined(HX_ZERO_FLASH)
UPDATE_MPFW_FAIL:
	hx_s_core_fp._0f_op_file_dirly(g_fw_boot_upgrade_name);
#else
	if (hx_s_ts->debug_log_level & BIT(4))
		I("%s:start sense off!\n", __func__);
	hx_s_core_fp._sense_off(true);
	...
	hx_s_core_fp._register_write(0x10007294, tmp_data, 4);
	if (hx_s_core_fp._reload_disable != NULL)
		hx_s_core_fp._reload_disable(0);
	...
	if (rslt == HX_INSP_OK) {
		I("Self_Test Pass:\n");
		seq_printf(s, "result=1\n");
	} else {
		I("Self_Test Fail:\n");
		seq_printf(s, "result=0\n");
	}
...
#if defined(HX_ZERO_FLASH)
	if (hx_s_ts->debug_log_level & BIT(4))
		I("%s:MP FW CID : 0x%04X\n", __func__, g_mp_fw_ver);
#endif
