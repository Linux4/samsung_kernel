...
struct ic_setup_collect {
	...
	uint32_t _addr_hand_blade;
	uint32_t _data_hand_blade_on_off;
#endif
...
#if defined(SEC_PALM_FUNC)
	#define addr_hand_blade			 		0x10007F28
	#define data_hand_blade_on_off	 	 	0xa55aa55a
#endif
#if defined(EARPHONE_PREVENT)
...
/* CORE_FLASH */
	...
	bool (*_bin_desc_data_get)(uint32_t addr, uint8_t *flash_buf);
...
/* CORE_DRIVER */
	...
	void (*_write_sram_0f)(uint8_t *addr, const uint8_t *data,
		uint32_t len);
	...
	void (*_read_sram_0f)(const struct firmware *fw_entry,
			...
			int start_index,
			int read_len);
	void (*_read_all_sram)(uint8_t *addr, int read_len);
...
extern bool hx_bin_desc_data_get(uint32_t addr, uint8_t *flash_buf);
...
extern void himax_mcu_write_sram_0f(uint8_t *addr,
...
extern void himax_mcu_read_all_sram(uint8_t *addr, int read_len);
