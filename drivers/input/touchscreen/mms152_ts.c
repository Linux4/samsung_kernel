...
#define MSM_GPIO_TO_INT(n) (NR_MSM_IRQS + (n))	/*HASH 1106*/
...
static int melfas_mux_fw_flash(struct melfas_tsi_platform_data *pdata,
				bool to_gpios)
{
	if (to_gpios) {
...
static int mms_flash_boot(struct mms_ts_info *info)
{
	...
	ret = request_firmware(&tsp_fw,
		"tsp_melfas/mms_boot.fw", &(client->dev));
	if (ret) {
...
/* this code is temporary code for prevent ISP FW update routine */
	dev_info(&client->dev, "%s: check FW VERSION\n", __func__);
	if (get_fw_version(info, SEC_CORE) == 0x53 &&
		get_fw_version(info, SEC_CONFIG) == fw_bin_ver) {
		dev_info(&client->dev, "%s: don't need update\n", __func__);
		return 1;
	}
/* this code is temporary code for prevent ISP FW update routine */
	...
	ret = request_firmware(&tsp_fw,
		"tsp_melfas/melfas.fw", &(client->dev));
	if (ret) {
		dev_err(&client->dev, "request firmware error!!\n");
		return 1;
	}
...
*/
	...
	fw_boot = i2c_smbus_read_byte_data(info->client, 0xE1);
	fw_core = i2c_smbus_read_byte_data(info->client, 0xE2);
	fw_config = i2c_smbus_read_byte_data(info->client, 0xE3);

	val1 = i2c_smbus_read_byte_data(info->client, 0x0B);
	val2 = i2c_smbus_read_byte_data(info->client, 0x0C);

	printk(KERN_ERR "%s: x_num : %d, y_num : %d\n", __func__, val1, val2);
	printk(KERN_ERR "%s: core: 0x%.2x, boot: 0x%.2x, config: 0x%.2x\n",
			__func__, fw_core, fw_boot, fw_config);
...
*/
	...
	if (IS_ERR(sec_touchscreen)) {
		dev_err(&client->dev,
			"tsp: Failed to create device for the sysfs1\n");
		ret = -ENODEV;
	}
	...
	if (IS_ERR(fac_dev_ts))
		dev_err(&client->dev, "tsp: Failed to create device for the sysfs\n");

	ret = sysfs_create_group(&fac_dev_ts->kobj,
				 &sec_touch_factory_attr_group);
	if (ret)
		dev_err(&client->dev, "tsp: Failed to create sysfs group\n");
#endif
	return 0;
