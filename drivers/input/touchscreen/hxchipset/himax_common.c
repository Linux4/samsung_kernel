...
static int himax_self_test_seq_read(struct seq_file *s, void *v)
{
	...
	mutex_lock(&(private_ts->self_test_lock));
	if (chip_test_r_flag) {
	...
	mutex_unlock(&(private_ts->self_test_lock));
 ...
 */
#if defined(HX_EXCP_RECOVERY)
void himax_fw_status_check(void)
{
...
/*start ts_work*/
#if defined(HX_USB_DETECT_GLOBAL)
extern int smblib_get_usb_mode(void);
void himax_cable_detect_func(bool force_renew)
{
	...
	connect_status = smblib_get_usb_mode();
	ts = private_ts;
 ...
 */
	...
	/* drop 1st interrupts after chip reset */
		...
		HX_EXCP_RESET_ACTIVATE = 0;
		I("%s: Back from reset[HX_EXCP_RESET_ACTIVATE], \
			ready to serve.\n", __func__);
...
static void himax_fb_register(struct work_struct *work)
{
	...
	ts->fb_notif.notifier_call = drm_notifier_callback;
	if (himax_panel) {
		ret = drm_panel_notifier_register(himax_panel, &ts->fb_notif);
	}
#endif
#endif
	if (ret)
 ...
 */
	...
	ts->ts_SMWP_wake_lock = wakeup_source_register(ts->dev,
						     HIMAX_common_NAME);	
#endif
#if defined(HX_HIGH_SENSE)
	ts->HSEN_enable = 0;
...
#else
	if (himax_panel && drm_panel_notifier_unregister(himax_panel, &ts->fb_notif))
		E("Error occurred while unregistering drm_notifier.\n");
#endif
	cancel_delayed_work_sync(&ts->work_att);
	destroy_workqueue(ts->himax_att_wq);
...
{
#if defined(HX_CONTAINER_SPEED_UP)
        I("%s:flush_delayed_work.\n",__func__);
        flush_delayed_work(&ts->ts_int_work);
#endif
...
void himax_clr_reload_status(void)
{
	uint8_t data[4] = {0};
...
void himax_chk_reload_status(void)
{
	uint8_t data[4] = {0};
	uint8_t retry = 0;
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
	...
	if (g_core_fp.fp_sense_on != NULL) {
			himax_clr_reload_status();
			g_core_fp.fp_sense_on(0x00);
			himax_chk_reload_status();
		}
#endif
#if defined(HX_SMART_WAKEUP)\
	|| defined(HX_HIGH_SENSE)\
