...
static void himax_fb_register(struct work_struct *work)
{
	...
	I("%s in\n", __func__);
	ts->fb_notif.notifier_call = drm_hx_notifier_callback;
...
char fw_name[64] = "Himax_firmware";

static int assigned_fw_name(char *fw_name)
{
	
	//char *pos = NULL;
	//char *underline_pos4 = NULL;
	//char *buf = saved_command_line;
	//char *target_str = "msm_drm.dsi_display0=";
	unsigned char str_len = 0;
	//unsigned char loop_i = 0;
	/*
	pos = strstr(buf,target_str);

	if( NULL == pos ){
		E( "can't find the specified display module .\n");
		return 0;
	}

	for( loop_i = 1 ; loop_i < 6 ; loop_i++ )
	{
		pos = strchr( pos+1 ,CHAR_UNDERLINE );

		if( NULL == pos ) {
			E( "can't find the specified char at cycle %d .\n",loop_i);
			return 0;
		}
		switch (loop_i) {
		case 4:
	...
	*/
	//if ( underline_pos4 && str_len != 0 ) {
		//strncat( fw_name,underline_pos4,str_len );//assgin module info
		strncat( fw_name,"_inx.bin",8 );//assgin module info
		//strncat( fw_name,FW_SUFFIX,strlen(FW_SUFFIX) );//assgin file suffix.
		fw_name[strlen(fw_name)] = '\0';//assgin eof.
	//}
	str_len=22;
	I( "zzc_assigned_fw_name:fw_name%s\n",fw_name);
	return str_len;
}
...
int himax_chip_common_init(void)
{
	...
	if (assigned_fw_name(fw_name)){
		i_CTPM_firmware_name = fw_name;
