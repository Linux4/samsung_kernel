...
#endif
	hx_lcm_bias_power_init(ts);
	hx_lcm_power_source_ctrl(ts, 1);
#if defined(CONFIG_OF)
...
error_ic_detect_failed:
	hx_lcm_power_source_ctrl(ts, 0);
	hx_lcm_bias_power_deinit(ts);
...
#endif
	himax_rst_gpio_set(private_ts->rst_gpio, 0);//modify by FAE at 2020/08/05
	himax_int_enable(0);
