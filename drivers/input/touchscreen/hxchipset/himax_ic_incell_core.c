...
static void himax_mcu_read_FW_ver(void)
{
	...
	while (reload_status == 0) {
		...
		|| (data_2[1] == 0x72 && data_2[0] == 0xC0)) {
			...
			retry++;
			usleep_range(10000, 11000);
			I("%s: waiting for reload done %d times\n", __func__, retry);
...
static void himax_mcu_flash_programming(uint8_t *FW_content, int FW_Size)
{
       ...
       uint8_t tmp_data[DATA_LEN_4];
       int ret = 0;
       uint8_t * buring_data = NULL;
       buring_data = kcalloc(FLASH_RW_MAX_LEN, sizeof(uint8_t), GFP_KERNEL);
              if (buring_data == NULL) {
                     E("%s, Allocate FW_buf array failed!\n", __func__);
                     ret = MEM_ALLOC_FAIL;
                     goto ALOC_buring_data_FAIL;
              }
...
flash_programming_end:
       kfree(buring_data);
       buring_data = NULL;
ALOC_buring_data_FAIL:
              return;
}
...
static bool himax_mcu_flash_lastdata_check(uint32_t size)
{
       ...
       */
       ...
       flash_tmp_buffer = kcalloc(128, sizeof(uint8_t), GFP_KERNEL);
              if (flash_tmp_buffer == NULL) {
                     E("%s, Allocate FW_buf array failed!\n", __func__);
                     ret = 1;
                     goto ALOC_flash_tmp_buffer_FAIL;
              }
...
static bool hx_bin_desc_data_get(uint32_t addr, uint8_t *flash_buf)
{
	...
	for (i = 0; i < FW_PAGE_SZ; i = i + data_sz) {
		...
		if (!chk_end) { /*1. Check all zero*/
			...
			flash_addr = (unsigned long)(flash_buf[i + 4] + (flash_buf[i + 5] << 8)
...
static bool hx_mcu_bin_desc_get(unsigned char *fw, uint32_t max_sz)
{
	...
	do {
		...
		if (!g_bin_desc_flag) {
			...
			I("%s: Bin don't have description table\n",	__func__);
			break;
		}
...
extern uint8_t himax_get_panel_info(void);
		...
		uint32_t hx_zero_event, uint32_t length)
{
	...
	if (hx_excp_event == length) {
		...
		if (g_zero_event_count > 5) {
			...
			g_zero_event_count++;
			I("[HIMAX TP MSG]: ALL 0xED event is %d times.\n",
					g_zero_event_count);
