...
#endif
/*Tab A8 code for SR-AX6300-01-130 by chenpengbo at 2021/08/19 start*/
//extern struct sec_cmd himax_commands[];
extern int himax_get_array_size(void);
/*Tab A8 code for SR-AX6300-01-130 by chenpengbo at 2021/08/19 end*/
/*Tab A8 code for SR-AX6300-01-52 by chenpengbo at 2021/08/31 start*/
static int himax_get_tp_module(void)
{
    int fw_num = 0;
    char *command_line = saved_command_line;

    if (NULL != strstr(command_line, "lcd_hx83102e_inx_mipi_hdp")){
        fw_num = MODEL_INX;
	} else if(NULL != strstr(command_line, "lcd_hx83102e_xy_hsd_mipi_hdp")){
		fw_num = MODEL_XY_HSD;
	} else if(NULL != strstr(command_line, "lcd_hx83102e_ls_hsd_mipi_hdp")){
		fw_num = MODEL_LS_HSD;
 	} else
...
static void himax_update_module_info(void)
{
    ...
    {
    case MODEL_INX:
            strcpy(ts->himax_name,"hx83102e_inx");
            strcpy(ts->himax_nomalfw_rq_name, "hx83102e_inx_firmware.bin");
            strcpy(ts->himax_mpfw_rq_name, "hx83102e_inx_mpfw.bin");
            strcpy(ts->himax_csv_name, "hx_inx_criteria.csv");
            break;
    case MODEL_XY_HSD:
            strcpy(ts->himax_name,"hx83102e_xy_hsd");
            strcpy(ts->himax_nomalfw_rq_name, "hx83102e_xy_hsd_firmware.bin");
            strcpy(ts->himax_mpfw_rq_name, "hx83102e_xy_hsd_mpfw.bin");
            strcpy(ts->himax_csv_name, "hx_xy_hsd_criteria.csv");
            break;
	case MODEL_LS_HSD:
            strcpy(ts->himax_name,"hx83102e_ls_hsd");
            strcpy(ts->himax_nomalfw_rq_name, "hx83102e_ls_hsd_firmware.bin");
            strcpy(ts->himax_mpfw_rq_name, "hx83102e_ls_hsd_mpfw.bin");
            strcpy(ts->himax_csv_name, "hx_ls_hsd_criteria.csv");
            break;
    default:
...
/*Tab A8 code for SR-AX6300-01-52 by chenpengbo at 2021/08/31 end*/
...
static ssize_t himax_ts_suspend_show(struct device  *dev,
...
/* reset chip */
static ssize_t himax_ts_suspend_store(struct device *dev,
...
static ssize_t himax_ts_resume_show(struct device  *dev,
		struct device_attribute *attr, char *buf)
{
	struct himax_ts_data *ts = dev_get_drvdata(dev);
	int cnt = -EINVAL;
...
/* reset chip */
static ssize_t himax_ts_resume_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf,
				     size_t count)
{
...
static ssize_t himax_firmware_version_show(struct device  *dev,
		struct device_attribute *attr, char *buf)
{
	int cnt = -EINVAL;
	cnt = snprintf(buf, PAGE_SIZE, "Firmware version:0x%x\n", ic_data->vendor_fw_ver);
	return cnt;
}
static ssize_t himax_chip_id_show(struct device  *dev,
		struct device_attribute *attr, char *buf)
{
	int cnt = -EINVAL;
	struct himax_ts_data *ts = private_ts;
...
static DEVICE_ATTR(ts_suspend, S_IRUGO | S_IWUSR | S_IWGRP, himax_ts_suspend_show, himax_ts_suspend_store);
static DEVICE_ATTR(ts_resume, S_IRUGO | S_IWUSR | S_IWGRP, himax_ts_resume_show, himax_ts_resume_store);
static DEVICE_ATTR(firmware_version, S_IRUGO, himax_firmware_version_show, NULL);
static DEVICE_ATTR(chip_id, S_IRUGO, himax_chip_id_show, NULL);

static struct attribute *sysfs_attrs[] = {
	&dev_attr_ts_suspend.attr,
	&dev_attr_ts_resume.attr,
...
static int himax_sysfs_init(struct himax_ts_data *ts)
{
	int ret;

	ret = sysfs_create_group(&ts->dev->kobj, &sysfs_group);
	if (ret) {
		E("failed create core sysfs group");
		return ret;
	}

	ret = sysfs_create_link(NULL, &ts->dev->kobj, "touchscreen");
	if (ret) {
		E("failed to link display sysfs");
		return ret;
	}
	return ret;
}
