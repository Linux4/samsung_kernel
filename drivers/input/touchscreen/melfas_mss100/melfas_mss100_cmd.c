...
EXIT:
	sec_cmd_set_cmd_result(sec, buf, strnlen(buf, sizeof(buf)));
	if (sec->cmd_all_factory_state == SEC_CMD_STATUS_RUNNING)
		sec_cmd_set_cmd_result_all(sec, buf, strnlen(buf, sizeof(buf)), "CP");
}
...
EXIT:
	sec_cmd_set_cmd_result(sec, buf, strnlen(buf, sizeof(buf)));
	if (sec->cmd_all_factory_state == SEC_CMD_STATUS_RUNNING)
		sec_cmd_set_cmd_result_all(sec, buf, strnlen(buf, sizeof(buf)), "CP_SHORT");
}
...
EXIT:
	sec_cmd_set_cmd_result(sec, buf, strnlen(buf, sizeof(buf)));
	if (sec->cmd_all_factory_state == SEC_CMD_STATUS_RUNNING)
		sec_cmd_set_cmd_result_all(sec, buf, strnlen(buf, sizeof(buf)), "CP_LPM");
}
...
static void cmd_run_prox_intensity_read_all(void *device_data)
{
	...
	snprintf(data, sizeof(data), "%d", rbuf[0]);
...
static void run_trx_short_test(void *device_data)
{
	...
	if (sec->cmd_param[0] == OPEN_SHORT_TEST && sec->cmd_param[1] == 0) {
		...
		snprintf(info->print_buf, PAGE_SIZE, "%s", "CONT");
		sec_cmd_set_cmd_result(sec, info->print_buf, strlen(info->print_buf));
		sec->cmd_state = SEC_CMD_STATUS_OK;
		return;
	}
...
#if 0 /* #ifdef CONFIG_TRUSTONIC_TRUSTED_UI */
...
static void set_aod_rect(void *device_data)
{
	...
	mms_set_custom_library(info, SPONGE_TOUCHBOX_W_OFFSET, data, 8);
