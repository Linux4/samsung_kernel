		...
		u16 *vector_elem_num, int table_size)
{
	...
	for (i_line = 0; i_line < (key_exist + vector_num); i_line++) {
		if ((i_line == 0) && (key_exist == 1)) {
			elem_len = key_num;
			input_info(true, &info->client->dev, "[Key]\n");
		} else {
			...
			case VECTOR_ID_SCREEN_RX:
				input_info(true, &info->client->dev, "[Screen Rx]\n");
				break;
			case VECTOR_ID_SCREEN_TX:
				input_info(true, &info->client->dev, "[Screen Tx]\n");
				break;
			case VECTOR_ID_KEY_RX:
				input_info(true, &info->client->dev, "[Key Rx]\n");
				break;
			case VECTOR_ID_KEY_TX:
				input_info(true, &info->client->dev, "[Key Tx]\n");
				break;
			case VECTOR_ID_PRESSURE:
				input_info(true, &info->client->dev, "[Pressure]\n");
				break;
			case VECTOR_ID_OPEN_RESULT:
				input_info(true, &info->client->dev, "[Open Result]\n");
				break;
			case VECTOR_ID_OPEN_RX:
				input_info(true, &info->client->dev, "[Open Rx]\n");
				break;
			case VECTOR_ID_OPEN_TX:
				input_info(true, &info->client->dev, "[Open Tx\n");
				break;
			case VECTOR_ID_SHORT_RESULT:
				input_info(true, &info->client->dev, "[Short Result]\n");
				break;
			case VECTOR_ID_SHORT_RX:
				input_info(true, &info->client->dev, "[Short Rx]\n");
				break;
			case VECTOR_ID_SHORT_TX:
				input_info(true, &info->client->dev, "[Short Tx]\n");
				break;
			default:
				input_info(true, &info->client->dev, "[%d]\n", i_vector);
				break;
			}
			i_vector++;
		}
...
int mms_run_test(struct mms_ts_info *info, u8 test_type)
{
	...
	case MIP_TEST_TYPE_CM:
		input_info(true, &info->client->dev, "==== CM Test ===\n");
		break;
	case MIP_TEST_TYPE_CM_ABS:
		input_info(true, &info->client->dev, "==== CM ABS Test ===\n");
		break;
	case MIP_TEST_TYPE_CM_JITTER:
		input_info(true, &info->client->dev, "==== CM JITTER Test ===\n");
		break;
	case MIP_TEST_TYPE_SHORT:
		input_info(true, &info->client->dev, "==== SHORT Test ===\n");
		break;
	case MIP_TEST_TYPE_GPIO_LOW:
		input_info(true, &info->client->dev, "==== GPIO LOW Test ===\n");
		break;
	case MIP_TEST_TYPE_GPIO_HIGH:
		input_info(true, &info->client->dev, "==== GPIO HIGH Test ===\n");
		break;
	case MIP_TEST_TYPE_CM_DIFF_HOR:
		input_info(true, &info->client->dev, "==== CM DIFF HOR Test ===\n");
		break;
	case MIP_TEST_TYPE_CM_DIFF_VER:
		input_info(true, &info->client->dev, "==== CM DIFF VER Test ===\n");
		break;
	case MIP_TEST_TYPE_CP:
		input_info(true, &info->client->dev, "==== CP Test ===\n");
		break;
	case MIP_TEST_TYPE_CP_SHORT:
		input_info(true, &info->client->dev, "==== CP SHORT Test ===\n");
		break;		
	case MIP_TEST_TYPE_CP_LPM:
		input_info(true, &info->client->dev, "==== CP LPM Test ===\n");
		break;
	case MIP_TEST_TYPE_PANEL_CONN:
		input_info(true, &info->client->dev, "==== CONNECTION Test ===\n");
		break;
	case MIP_TEST_TYPE_OPEN_SHORT:
		input_info(true, &info->client->dev, "==== OPEN SHORT Test ===\n");
		break;
	default:
		input_info(true, &info->client->dev, "%s [ERROR] Unknown test type\n", __func__);
