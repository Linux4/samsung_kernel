...
u32 BUTTON_MAPPING_KEY_WITH_DUMMY[MAX_SUPPORTED_BUTTON_NUM_WITH_DUMMY]
...
static void cover_set(struct zt7548_ts_info *info)
{
...
int zt7548_notification(struct notifier_block *nb,
...
static bool get_raw_data(struct zt7548_ts_info *info, u8 *buff, int skip_cnt)
{
	...
	for (i = 0; sz > 0; i++) {
		...
		if (sz < I2C_BUFFER_SIZE)
			temp_sz = sz;
		if (read_raw_data(client, ZT7548_RAWDATA_REG + i,
...
static bool i2c_checksum(struct zt7548_ts_info *info, s16 *pChecksum, u16 wlength)
{
	s16 checksum_result;
	s16 checksum_cur;
	int i;
...
static bool ts_check_need_upgrade(struct zt7548_ts_info *info,
...
static bool ts_upgrade_firmware(struct zt7548_ts_info *info, const u8 *firmware_data, u32 size)
{
	...
	zt7548_power_control(info, POWER_OFF);
	zt7548_power_control(info, POWER_ON);
...
static bool init_touch(struct zt7548_ts_info *info, bool fw_skip)
{
	...
	if (!info->fw_data) {
		snprintf(fw_path, ZINITIX_MAX_FW_PATH, "%s", info->pdata->fw_name);
...
retry_init:
	...
	checksum_err = false;
	for (i = 0; i < INIT_RETRY_CNT; i++) {
		if (read_data(client, ZT7548_CHECKSUM_RESULT,
	...
	cap->button_num = info->max_button_num;
	...
	if (!pdata->calculate_dnd_from_fw) {
	...
	dev_err(&client->dev, "%s: fw ver:0x%x, minor ver:0x%x, reg ver:0x%x, hw id:0x%x\n",
			__func__, cap->fw_version, cap->fw_minor_version,
	...
	if (!fw_skip && ts_check_need_upgrade(info, cap->fw_version,
		...
		if (read_data(client, ZT7548_CHECKSUM_RESULT, (u8 *)&chip_check_sum, 2) < 0)
			goto fail_init;
...
static void clear_report_data(struct zt7548_ts_info *info)
{
	...
	for (i = 0; i < info->cap_info.button_num; i++) {
		if (info->button[i] == ICON_BUTTON_DOWN) {
			...
			input_report_key(info->input_dev, info->button_mapping_key[i], 0);
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
			dev_info(&info->client->dev, "key %d up\n", info->button_mapping_key[i]);
#else
	...
	if (zinitix_bit_test(info->touch_info.status, BIT_ICON_EVENT)) {
		...
		for (i = 0; i < info->cap_info.button_num; i++) {
			if (zinitix_bit_test(info->icon_event_reg, (BIT_O_ICON0_DOWN + i))) {
				input_report_key(info->input_dev, info->button_mapping_key[i], 1);
				if (info->button[i] != ICON_BUTTON_DOWN &&
						(info->button_mapping_key[i] == KEY_RECENT ||
						info->button_mapping_key[i] == KEY_BACK))
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
			...
			if (zinitix_bit_test(info->icon_event_reg, (BIT_O_ICON0_UP + i))) {
				info->button[i] = ICON_BUTTON_UP;
				input_report_key(info->input_dev, info->button_mapping_key[i], 0);
				if (info->button_mapping_key[i] == KEY_RECENT ||
						info->button_mapping_key[i] == KEY_BACK)
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
...
#if 0
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
				dev_info(&client->dev, "[%d][M] x = %d, y = %d,"
						" w = %d, p = %d\n",i, x, y, w, palm);
#else
				dev_info(&client->dev,
						"[%d][M] w = %d, p = %d\n", i, w, palm);
...
static bool ts_set_touchmode(u16 value)
{
	...
	write_cmd(misc_info->client, ZT7548_WAKEUP_CMD);
	zinitix_delay(50);

	if (misc_info->pdata->calculate_dnd_from_fw) {
		misc_info->work_state = SET_MODE;
	} else {
		if (misc_info->touch_mode == TOUCH_POINT_MODE) {
			/* factory data */
			read_data(misc_info->client, ZT7548_MUTUAL_AMP_V_SEL,
					(u8 *)&misc_info->cap_info.mutual_amp_v_sel, 2);
			read_data(misc_info->client, ZT7548_AFE_FREQUENCY,
					(u8 *)&misc_info->cap_info.afe_frequency, 2);
			read_data(misc_info->client, ZT7548_DND_SHIFT_VALUE,
		...
		if (value == TOUCH_DND_MODE) {
			...
			if (write_reg(misc_info->client, ZT7548_MUTUAL_AMP_V_SEL,
				...
				dev_info(&misc_info->client->dev, "[zinitix_touch] TEST Mode : "
						"Fail to set ZT7548_MUTUAL_AMP_V_SEL %x.\n", SEC_MUTUAL_AMP_V_SEL);
#endif
	...
	dev_info(&misc_info->client->dev, "[zinitix_touch] %s : touch_mode = %d\r\n",
...
static bool ts_set_touchmode2(u16 value)
{
	...
	if(value == TOUCH_DND_MODE) {
		...
		if (write_reg(misc_info->client, ZT7548_DND_N_COUNT,
			...
			dev_info(&misc_info->client->dev, "[zinitix_touch] TEST Mode : "
					"Fail to set ZT7548_HFDND_N_COUNT %d.\n", SEC_HFDND_N_COUNT);
		if (write_reg(misc_info->client, ZT7548_DND_U_COUNT,
				SEC_HFDND_U_COUNT)!=I2C_SUCCESS)
			dev_info(&misc_info->client->dev, "[zinitix_touch] TEST Mode : "
					"Fail to set ZT7548_HFDND_U_COUNT %d.\n", SEC_HFDND_U_COUNT);
		if (write_reg(misc_info->client, ZT7548_AFE_FREQUENCY,
...
/* Read firmware version from IC */
	down(&info->work_lock);
	write_cmd(client, ZT7548_WAKEUP_CMD);
	zinitix_delay(50);

	if (read_data(client, ZT7548_FIRMWARE_VERSION, (u8 *)&fw_version, 2) < 0)
		dev_err(&client->dev, "Failed to read firmware version\n");
	if (read_data(client, ZT7548_MINOR_FW_VERSION, (u8 *)&fw_minor_version, 2) < 0)
		dev_err(&client->dev, "Failed to read minor version\n");
	if (read_data(client, ZT7548_DATA_VERSION_REG, (u8 *)&reg_version, 2) < 0)
		dev_err(&client->dev, "Failed to read register version\n");
	if (read_data(client, ZT7548_HW_ID, (u8 *)&hw_id, 2) < 0)
...
#define ZT7548_VENDOR_NAME "ZINITIX"
...
static void get_chip_vendor(void *device_data)
{
	...
	snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", ZT7548_VENDOR_NAME);
	set_cmd_result(finfo, finfo->cmd_buff,
			strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	finfo->cmd_state = OK;
...
#define ZT7548_CHIP_NAME "ZT7548"
...
static void get_chip_name(void *device_data)
{
	...
	if (info->chip_code == ZT7548_IC_CHIP_CODE)
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", ZT7548_CHIP_NAME);
	else if (info->chip_code == ZT7538_IC_CHIP_CODE)
...
static void get_all_data(struct zt7548_ts_info *info,
	...
	for (i = 0; i < x_num - 1; i++) {
		...
		for (j = 0; j < y_num; j++) {
			...
			if ((i >= x_num - 2) && !next_val) {	/* touchkey node */
				...
				pr_cont(" %d", raw_data->vgap_data[offset]);
				continue;
			}
	...
	for (i = 0; i < x_num ; i++) {
		...
		for (j = 0; j < y_num - 1; j++) {
			...
			if ((i >= x_num - 1) && !cur_val) {	/* touchkey node */
				...
				pr_cont(" %d", raw_data->hgap_data[offset]);
				continue;
			}
...
static void run_hfdnd_read(void *device_data)
{
	...
	if (info->pdata->calculate_dnd_from_fw)
	...
	for (i = 0; i < x_num - 1; i++) {
		...
		for (j = 0; j < y_num; j++) {
			...
			if ((i >= x_num - 2) && !next_val) {	/* touchkey node */
				...
				pr_cont(" %d",raw_data->hfvgap_data[offset]);
				continue;
			}
	...
	for (i = 0; i < x_num ; i++) {
		...
		for (j = 0; j < y_num - 1; j++) {
			...
			if ((i >= x_num - 1) && !cur_val) {	/* touchkey node */
				...
				pr_cont(" %d", raw_data->hfhgap_data[offset]);
				continue;
			}
...
static void run_rxshort_read(void *device_data)
{
	...
	for (i = 0; i < y_num; i++) {
			pr_info("%s: rxshort[%d] : [%2d]\n", client->name, i, raw_data->rxshort_data[i]);
...
static void run_txshort_read(void *device_data)
{
	...
	for (i = 0; i < x_num - 1; i++) {
			pr_info("%s: txshort[%d] : [%2d]\n", client->name, i, raw_data->txshort_data[i]);
...
#if ESD_TIMER_INTERVAL
	esd_timer_stop(info);
	write_reg(client, ZT7548_PERIODICAL_INTERRUPT_INTERVAL, 0);
#endif

	write_reg(client, ZT7548_EEPROM_INFO_REG, 0xffff);

	write_reg(client, 0xc003, 0x0001);
	write_reg(client, 0xc104, 0x0001);
	usleep_range(100, 100);
	if (write_cmd(client, ZT7548_SAVE_STATUS_CMD) != I2C_SUCCESS) {
...
#if ESD_TIMER_INTERVAL
	write_reg(client, ZT7548_PERIODICAL_INTERRUPT_INTERVAL,
...
{
	struct zt7548_ts_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	struct tsp_factory_info *finfo = info->factory_info;
	char *cur, *start, *end;
...
static ssize_t cmd_list_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct zt7548_ts_info *info = dev_get_drvdata(dev);
	struct tsp_factory_info *finfo = info->factory_info;
	struct tsp_cmd *tsp_cmd_ptr = NULL;
	char buffer[finfo->cmd_buffer_size + 30];
	char buffer_name[TSP_CMD_STR_LEN];

	snprintf(buffer, 30, "++factory command list++\n");

	list_for_each_entry(tsp_cmd_ptr, &finfo->cmd_list_head, list) {
		if (strncmp(tsp_cmd_ptr->cmd_name, "not_support_cmd", 15)) {
			snprintf(buffer_name, TSP_CMD_STR_LEN, "%s\n", tsp_cmd_ptr->cmd_name);
			strcat(buffer, buffer_name);
		}
	}

	dev_info(&info->client->dev, "%s: length : %u / %d\n", __func__,
			(unsigned int)strlen(buffer), finfo->cmd_buffer_size);

	return snprintf(buf, TSP_CMD_RESULT_STR_LEN - 1, "%s\n", buffer);
}
					...
					struct device_attribute *attr, char *buf)
{
	...
	if (info->pdata->use_dummy_key) {
		if (!strcmp(attr->attr.name, "touchkey_dummy_btn1"))
			...
			i = 2;
		else if (!strcmp(attr->attr.name, "touchkey_dummy_btn2"))
			i = 3;
		else {
	...
	down(&info->work_lock);
	ret = read_data(client, ZT7548_BTN_WIDTH + i, (u8 *)&val, 2);
	up(&info->work_lock);
	if (ret < 0) {
...
static DEVICE_ATTR(touchkey_dummy_btn2, S_IRUGO, show_touchkey_sensitivity, NULL);
...
static struct attribute *touchkey_attributes[] = {
	...
	&dev_attr_touchkey_dummy_btn2.attr,
#ifdef SUPPORTED_KEY_LED
...
static int init_sec_factory(struct zt7548_ts_info *info)
{
	...
	factory_info->cmd_buffer_size = 0;
	...
	for (i = 0; i < ARRAY_SIZE(tsp_cmds); i++) {
		...
		if (tsp_cmds[i].cmd_name)
			factory_info->cmd_buffer_size += strlen(tsp_cmds[i].cmd_name) + 1;
	}
					...
					struct zt7548_ts_dt_data *pdata)
{
	...
	pdata->reg_boot_on = of_property_read_bool(np, "zt7548,reg_boot_on");
	pr_cont("reg_boot_on:%d, ", pdata->reg_boot_on);

	pdata->use_dummy_key = of_property_read_bool(np, "zt7548,use_dummy_key");
	pr_cont("use_dummy_key:%d, ", pdata->use_dummy_key);

	pdata->calculate_dnd_from_fw = of_property_read_bool(np, "zt7548,calculate_dnd_from_fw");
	pr_cont("calculate_dnd_from_fw:%d, ", pdata->calculate_dnd_from_fw);
...
static int zt7548_ts_probe(struct i2c_client *client, const struct i2c_device_id *i2c_id)
{
	...
	if (info->pdata->use_dummy_key) {
		info->max_button_num = MAX_SUPPORTED_BUTTON_NUM_WITH_DUMMY;
		info->button_mapping_key = BUTTON_MAPPING_KEY_WITH_DUMMY;
	} else {
		info->max_button_num = MAX_SUPPORTED_BUTTON_NUM;
		info->button_mapping_key = BUTTON_MAPPING_KEY;
	}
	info->button = devm_kzalloc(&client->dev, info->max_button_num, GFP_KERNEL);
	for (i = 0; i < info->max_button_num; i++)
		info->button[i] = ICON_BUTTON_UNCHANGE;
	dev_info(&client->dev, "%s: %d buttons\n", __func__, info->max_button_num);
	...
	/* Do not report dummy key */
	if (info->pdata->use_dummy_key) {
		for (i = 0; i < MAX_SUPPORTED_BUTTON_NUM_WITH_DUMMY; i++)
			set_bit(BUTTON_MAPPING_KEY_WITH_DUMMY[i], info->input_dev->keybit);
	} else {
	...
	muic_notifier_register(&info->charger_nb,
		zt7548_notification, MUIC_NOTIFY_DEV_TSP);
#endif
