...
static struct tsp_cmd tsp_cmds[] = {
	...
	{TSP_CMD("run_hfdnd_h_gap_jitter_read", run_hfdnd_h_gap_jitter_read),},	//add zinitix  160718
	{TSP_CMD("get_hfdnd_h_gap_jitter_read", get_hfdnd_h_gap_jitter_read),},			//add zinitix  160718
...
static void hfdnd_spec_adjust(void *device_data)
{
	...
	if (test) {	/* set : assy */
		...
		info->hfdnd_h_gap_jitter_spec = HF_DND_H_GAP_Jitter_Ref_data;
...
static void run_dnd_read(void *device_data)
{
	...
	get_raw_data(info, (u8 *)raw_data->dnd_data, 10);
...
static void run_dnd_v_gap_read(void *device_data)
{
	...
	memset(raw_data->vgap_data, 0x00, ARRAY_SIZE(raw_data->vgap_data));
...
static void run_dnd_h_gap_read(void *device_data)
{
	...
	memset(raw_data->hgap_data, 0x00, ARRAY_SIZE(raw_data->hgap_data));
...
static void run_hfdnd_v_gap_read(void *device_data)
{
	...
	memset(raw_data->hfvgap_data, 0x00, ARRAY_SIZE(raw_data->vgap_data));
...
//add  hfdnd_h_gap_jiiter zinitix 160718
static void run_hfdnd_h_gap_jitter_read(void *device_data)
{
	struct zt7548_ts_info *info = (struct zt7548_ts_info *)device_data;
	struct i2c_client *client = info->client;
	struct tsp_factory_info *finfo = info->factory_info;
	...
	memset(raw_data->hfhgap_jitter_data, 0x00, ARRAY_SIZE(raw_data->hfhgap_jitter_data));

	dev_info(&client->dev, "HFDND H Gap Jitter start\n");
	...
	for (i = 0; i < x_num-1 ; i++){
		...
		for (k = 0; k < y_num-1; k++){
			offset = (i * y_num) + k;			
			Tx_SUM = Tx_SUM + raw_data->hfhgap_data[offset] ;
			}

			Tx_AVG = ((Tx_SUM*100)+5) / ((y_num - 1)*100);

		
		for (j = 0; j < y_num-1; j++){
			offset = (i * y_num) + j;			

			if(raw_data->hfhgap_data[offset] > Tx_AVG)
				raw_data->hfhgap_jitter_data[offset]  = raw_data->hfhgap_data[offset] - Tx_AVG;
			else
				raw_data->hfhgap_jitter_data[offset]  = Tx_AVG - raw_data->hfhgap_data[offset];

			val = raw_data->hfhgap_jitter_data[offset];
			
			pr_cont("%d ", val);
		
#if defined(CONFIG_TOUCHSCREEN_ZT7538_TS)
			if (result && offset < ZT7538_H_GAP_JITTER_DATA_SIZE &&
				raw_data->hfhgap_jitter_data[offset] > info->hfdnd_h_gap_jitter_spec[offset]) {
				result = false;
				fx = i;
				fy = j;
				pr_cont("(E)");
	...
	dev_info(&client->dev, "DND H Gap Jitter screen_max %d, touchkey_max %d\n", screen_max, touchkey_max);
	...
	if (result) {
		...
		offset = (fx * y_num) + fy;
		sprintf(finfo->cmd_buff, "%d,%d,%d,%d,%d", fx, fy,
					0, info->hfdnd_h_gap_jitter_spec[offset],
					raw_data->hfhgap_jitter_data[offset]);
		pr_info("%s: hfdnd h gap jitter view fail: [%d][%d]\n", info->client->name, fx, fy);
	}
#else
	snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%d,%d", screen_max, touchkey_max);
#endif
...
//add  hfdnd_h_gap_jiiter zinitix 160718
static void get_hfdnd_h_gap_jitter_read(void *device_data)
{
	struct zt7548_ts_info *info = (struct zt7548_ts_info *)device_data;
	struct tsp_factory_info *finfo = info->factory_info;
	struct tsp_raw_data *raw_data = info->raw_data;
	...
	sprintf(finfo->cmd_buff, "%d", raw_data->hfhgap_jitter_data[node_num]);
	set_cmd_result(finfo, finfo->cmd_buff, strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	finfo->cmd_state = OK;
