...
int gcore_get_gpio_info(struct gcore_dev *gdev, struct device_node *node)
{
	...
	gdev->irq_gpio = of_get_named_gpio_flags(node, "irq-gpio", 0, (enum of_gpio_flags *)&flag);
	if (!gpio_is_valid(gdev->irq_gpio)) {
		GTP_ERROR("invalid irq-gpio:%d", gdev->irq_gpio);
		return -EPERM;
	}

	if (gpio_request(gdev->irq_gpio, "irq-gpio")) {
		GTP_ERROR("gpio %d request failed!", gdev->irq_gpio);
		return -EPERM;
	}

	gdev->rst_gpio = of_get_named_gpio_flags(node, "rst-gpio", 0, (enum of_gpio_flags *)&flag);
	if (!gpio_is_valid(gdev->rst_gpio)) {
		GTP_ERROR("invalid gcore,reset-gpio:%d", gdev->rst_gpio);
		return -EPERM;
	}

	if (gpio_request(gdev->rst_gpio, "rst-gpio")) {
