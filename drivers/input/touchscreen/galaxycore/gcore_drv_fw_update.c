...
#define FW_BIN_NAME "V750_1.1.bin"
...
EXPORT_SYMBOL(gcore_fw_read_rawdata);
...
EXPORT_SYMBOL(gcore_fw_read_diffdata);
...
#define DUMP_FW_FILE_NAME  "/data/tpdata/gcore_dump_fw.bin"
...
int gcore_idm_open_tp(void)
{
	...
	/* open tp */
	gcore_idm_write_reg(0xD0001CA0, &open, 1);
...
/* gcore_reset();  //reset RESX pin */
	...
	if (!wait_for_completion_interruptible_timeout(&fw_update_complete, 1 * HZ)) {
		...
		GTP_ERROR("hostdownload must go on.");
		gcore_auto_update_hostdownload(fw_buf);
		return 0;
/*		return -EPERM; */
	}
