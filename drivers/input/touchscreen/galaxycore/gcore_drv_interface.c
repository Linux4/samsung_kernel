...
#define GCORE_TP_INFO_FILE   "tp_info"
...
ssize_t gcore_tp_info_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos);
struct proc_dir_entry *gcore_tp_info_entry;
struct file_operations gcore_tp_info_fops = {
	.owner = THIS_MODULE,
...
ssize_t gcore_tp_info_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	snprintf(buf, 150, "TP_module=JZ_PANDA_GC7202 V750_%d%d.%d%d",read_data[1],read_data[0],read_data[3],read_data[2]);
	rc = simple_read_from_buffer(buffer, count, ppos, buf, strlen(buf));
...
*/
	ret = copy_to_user(buffer, touch_data_p, DEMO_RAWDATA_SIZE);

	gdev->usr_read = false;
	gdev->data_ready = false;

	return DEMO_RAWDATA_SIZE;
...
#define MP_TEST_PATH "/data/tpdata"
static ssize_t gcore_test_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	GTP_DEBUG("JZ GC7202 start mp test");
	/* Create the directory for mp_test result */
	if ((dev_mkdir(MP_TEST_PATH, S_IRUGO | S_IWUSR)) != 0){
		GTP_ERROR("Failed to create directory for mp_test\n");

	}
	if (gcore_start_mp_test()) {
		GTP_ERROR("JZ GC7202 mp test fail!");
		return 0;
	}
...
static ssize_t gcore_test_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	return 0;
}

static DEVICE_ATTR(factory_check, 0644, gcore_test_show, gcore_test_store);

static struct attribute *gcore_test_attributes[] ={
	&dev_attr_factory_check.attr,
	NULL
};

static struct attribute_group gcore_test_attribute_group = {
	.attrs = gcore_test_attributes
};

int gcore_test_node_init(struct platform_device *tpinfo_device)
{
    int err=0;
    err = sysfs_create_group(&tpinfo_device->dev.kobj, &gcore_test_attribute_group);
    if (0 != err)
    {
        GTP_DEBUG( "ERROR: ITO node create failed.");
        sysfs_remove_group(&tpinfo_device->dev.kobj, &gcore_test_attribute_group);
        return -EIO;
    }
    else
    {
        GTP_DEBUG("ITO node create_succeeded.");
    }
    return err;
}
#endif
...
int gcore_app_node_init(void)
{
	...
	gcore_test_node_init(&hwinfo_device);
#endif
	gcore_tp_info_entry = proc_create(GCORE_TP_INFO_FILE, 0666, NULL, &gcore_tp_info_fops);
	if (gcore_tp_info_entry == NULL) {
		GTP_ERROR("create proc entry tp_info failed");
		return -EPERM;
	}
#endif
	return 0;
...
#if TP_INFO
	if (gcore_tp_info_entry != NULL) {
		remove_proc_entry(GCORE_TP_INFO_FILE, NULL);
	}
#endif
}
