...
static irqreturn_t mms_ts_interrupt(int irq, void *dev_id)
{
...
#endif
	if (buf[0] == 0x0F) {	/* ESD */
		dev_err(&client->dev, "ESD DETECT.... reset!!\n");
		reset_mms_ts(info);
		goto out;
	}

	if (buf[0] == 0x0E) { /* NOISE MODE */
		dev_err(&client->dev, "[TSP] noise mode enter!!\n");
		info->noise_mode = 1;
		mms_set_noise_mode(info);
		goto out;
	}
	...
	if (buf[0] == 0x0B) { /* THREEWAVE MODE */
		if (buf[1] == 0x01) {
			dev_err(&client->dev, "[TSP] three-wave mode enter!!\n");
			info->threewave_mode = 1;
		} else {
			dev_err(&client->dev, "[TSP] three-wave mode exit!!\n");
