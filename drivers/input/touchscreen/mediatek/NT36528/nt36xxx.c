...
#endif
//+S96818AA1-1936,daijun1.wt,modify,2023/05/10,nt36528 modify SPI timing
static struct mtk_chip_config nvt_spi_ctrdata = {
	.cs_holdtime = 0,
	.cs_idletime = 0,
	.sample_sel = 0,
	.tick_delay = 0,
...
//-S96818AA1-1936,daijun1.wt,modify,2023/06/08,Fix the issue of gesture failure when the system enters deep sleep
	...
	ts->client->controller_data = (void*)&nvt_spi_ctrdata;	//S96818AA1-1936,daijun1.wt,modify,2023/05/10,nt36528 modify SPI timing
...
static void nvt_update_charger(struct work_struct *work)
{
    ...
    if(ts->usb_plug_status){
        nvt_set_charger(1);
        input_info(true, &ts->client->dev, "nvt usb_online usb_plug_status=%d\n",ts->usb_plug_status);
    }else{
        nvt_set_charger(0);
        input_info(true, &ts->client->dev, "nvt usb_offline usb_plug_status=%d\n",ts->usb_plug_status);
    }
...
void nvt_plat_charger_init(void)
{
    ...
    if (!ts->charger_notify_wq) {
        input_err(true, &ts->client->dev, "%s allocate nvt_charger_notify_wq failed\n", __func__);
        return;
    }
