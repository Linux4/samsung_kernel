...
bool ili_gesture_status = DISABLE;
...
void ili_input_register(void)
{
	...
	input_set_capability(ilits->input, EV_KEY, KEY_GESTURE_DOUBLE_TAP);
	...
	__set_bit(KEY_GESTURE_DOUBLE_TAP, ilits->input->keybit);
...
static void aot_enable(void *device_data)
{
	...
	if (sec->cmd_param[0] < 0 || sec->cmd_param[0] > 1) {
		...
		sec_cmd_set_cmd_result(sec, buff, strnlen(buff, sizeof(buff)));
		ILI_ERR("sec->cmd_param[0] = %d, buff = %s, sec->cmd_state = %d\n",
				sec->cmd_param[0], buff, sec->cmd_state);
	...
	if (buf[0] == 1) {
		...
		ili_gesture_status = ENABLE;
	} else {
		ilits->gesture = DISABLE;
		ili_gesture_status = DISABLE;
	}
	...
	ILI_INFO("%s, buff == %s,sec->cmd_state == %d\n",
			sec->cmd_param[0] ? "on" : "off", buff, sec->cmd_state);
	sec_cmd_set_cmd_exit(sec);
}
...
#ifdef SEC_TSP_FACTORY_TEST
	ret = sec_cmd_init(&ilits->sec, ft2_commands, ARRAY_SIZE(ft2_commands), SEC_CLASS_DEVT_TSP);
	if (ret < 0) {
...
static int __init ilitek_plat_dev_init(void)
{
	...
	ret = ilitek_get_tp_module();
	if (ret < 0) {
		ILI_ERR("no compatable device found\n");
		return -ENODEV;
	}
