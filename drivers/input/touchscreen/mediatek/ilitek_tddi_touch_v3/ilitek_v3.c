...
static int __parse_tag_videolfb(struct device_node *node)
{
	...
	if (videolfb_tag) {
		...
		ILI_ERR("parse lcm name from dts failed\n");
		return -EINVAL;
	}
...
int ilitek_get_tp_module(void)
{
	int error;
	int module_id = 0;

	error = _parse_tag_videolfb();
	if (error) {
		ILI_ERR("parse lcam name fail\n");
		return -EINVAL;
	}

	if (0 == strcmp(mtkfb_lcm_name, "ili7806s_hd_plus_vdo_txd_na")) {
		module_id = MODEL_TXD_7806S_NA;
	} else {
		module_id = -1;
	}
...
static void ili_update_tp_module_info(void)
{
	...
	switch (module) {
	case MODEL_TXD_7806S_NA:
		ilits->ic_name = "ILI7806S";
		ilits->md_name = "TXD_AUO";
		ilits->md_fw_filp_path = TXD_7806S_NA_FW_FILP_PATH;
		ilits->md_fw_rq_path = TXD_7806S_NA_FW_REQUEST_PATH;
		ilits->md_ini_path = TXD_7806S_NA_INI_NAME_PATH;
		ilits->md_ini_rq_path = TXD_7806S_NA_INI_REQUEST_PATH;
		ilits->md_fw_ili = CTPM_FW_TXD_7806S_NA;
		ilits->md_fw_ili_size = sizeof(CTPM_FW_TXD_7806S_NA);
		break;
	default:
		break;
	}
	...
	ILI_INFO("Found %s-%s module: ini path = %s, fw path = (%s, %s, %d)\n",
			ilits->ic_name,
			ilits->md_name,
			ilits->md_ini_path,
