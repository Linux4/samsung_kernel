...
static void dma_trigger_reg_setting(u32 reg_dest_addr, u32 flash_start_addr, u32 copy_size)
{
	...
	if (ili_ice_mode_bit_mask_write(DMA52_ADDR, DMA52_reg_dma_ch0_src2_en, (0ul << 31)) < 0)
...
void ili_report_gesture_mode(u8 *buf, int len)
{
	...
	case GESTURE_DOUBLECLICK:
		...
		input_report_key(input, KEY_GESTURE_DOUBLE_TAP, 1);
		input_sync(input);
		input_report_key(input, KEY_GESTURE_DOUBLE_TAP, 0);
		input_sync(input);
