...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	if (strncmp(cmd, "hwreset", strlen(cmd)) == 0) {
		...
		ili_gesture_status = ilits->gesture;
		ILI_INFO("gesture = %d\n", ilits->gesture);
...
//+Bug623240, shenwenbin.wt, add, 20210510, TP add double tap function node
...
static ssize_t ilitek_proc_double_tap_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	...
	if (buff != NULL) {
		...
		if (res < 0) {
			ILI_ERR("copy data from user space, failed\n");
			return -1;
		}
	}
	ret = sscanf(cmd, "%d", &temp);
	if(ret < 0){
		ILI_ERR("invalid cmd parameter\n");
		return -EINVAL;
	}
	...
	ILI_INFO("double tap = %d\n", temp);
	if(temp) {
		ilits->gesture = true;
		ili_gesture_status = true;
	}
	else {
		ilits->gesture =false;
		ili_gesture_status = false;
	}
	return size;
}
//-Bug623240, shenwenbin.wt, add, 20210510, TP add double tap function node
...
static ssize_t ilitek_proc_glove_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	...
	temp = ili_katoi(cmd);
	mutex_lock(&ilits->touch_mutex); //+ add by songbinbo.wt for High sensitivity mode contrl 20191125
	if (temp > 0) {
		if (ili_ic_func_ctrl("glove", ENABLE) < 0) {
			ILI_ERR("Write glove start cmd failed\n");
			goto out;
		}
		ilits->glove = true;
	} else {
		if (ili_ic_func_ctrl("glove", DISABLE) < 0) {
			ILI_ERR("Write glove stop cmd failed\n");
			goto out;
		}
		ilits->glove = false;
	}
	ILI_INFO("Write glove cmd success.glove = %d\n", temp);
out:
	mutex_unlock(&ilits->touch_mutex); //- add by songbinbo.wt for High sensitivity mode contrl 20191125
...
//+Bug623240, shenwenbin.wt, add, 20210510, TP add double tap function node
static struct file_operations proc_double_tap_enable_fops = {
	.read = ilitek_proc_double_tap_read,
	.write = ilitek_proc_double_tap_write,
};
//-Bug623240, shenwenbin.wt, add, 20210510, TP add double tap function node

static struct file_operations proc_glove_enable_fops = {
...
proc_node iliproc[] = {
	...
	//-Bug623240, shenwenbin.wt, add, 20210510, TP add double tap function node
	// +add by linminyi1.wt for ilitek tp touch sensitivity mode 20200910
...
#define CTP_PROC_DIR_NAME  "touchscreen"
#define CTP_OPENSHORT_PROC_NAME "ctp_openshort_test"
static struct proc_dir_entry *ctp_test_proc = NULL;

static ssize_t wt_ctp_test_proc_write(struct file *file,const char __user *userbuf,size_t count,loff_t *ppos)
{
	return -1;
}

static ssize_t wt_ctp_test_proc_read(struct file *file, char __user *buf,
	size_t count, loff_t *ppos)
{
	...
	*ppos += count;
	...
	if (ilits->wt_result < 0) {
		if (copy_to_user((char *)buf, "result=0\n", len)) {
			ILI_ERR("Failed to copy data to user space\n");
			return -1;
		}
	} else {
		if (copy_to_user((char *)buf, "result=1\n", len)) {
			ILI_ERR("Failed to copy data to user space\n");
			return -1;
		}
	}
...
static struct file_operations ctp_open_procs_fops = {
	.write = wt_ctp_test_proc_write,
	.read = wt_ctp_test_proc_read,
	.owner = THIS_MODULE,
};
#endif

#ifdef ILITEK_TOUCHSCREEN_LOCKDOWN_INFO
static struct proc_dir_entry *ilitek_proc_lock_down_info_wt;

static ssize_t ctp_lockdown_proc_read(struct file *file,
	char __user *buf, size_t count, loff_t *ppos)
{
	...
	*ppos += count;
	...
	ptr = kzalloc(ILITEK_LOCK_DOWN_INFO_LEN + 1, GFP_KERNEL);
	if (ptr == NULL) {
		ILI_ERR("failed to alloc memory for lockdown info\n");
		return -ENOMEM;
	}
	...
	* get lock down info
	*/
	lockdown_info[2] = 0x32; // default black
...
static struct proc_dir_entry *wt_ctp_proc_dir = NULL;
void wt_ctp_node_init(void)
{
	ILI_INFO("openshort create_ctp_proc \n");
	if(wt_ctp_proc_dir == NULL){
		wt_ctp_proc_dir = proc_mkdir(CTP_PROC_DIR_NAME,NULL);
		if(wt_ctp_proc_dir == NULL){
...
#if WINGTECH_TP_OPENSHORT_EN
	ctp_test_proc = proc_create(CTP_OPENSHORT_PROC_NAME,0777, wt_ctp_proc_dir,
		&ctp_open_procs_fops);
	if(ctp_test_proc == NULL){
		ILI_ERR("create open_proc fail\n");
	}
#endif

#ifdef ILITEK_TOUCHSCREEN_LOCKDOWN_INFO
	ilitek_proc_lock_down_info_wt = proc_create(ILITEK_PROC_LOCK_DOWN_INFO_FILE_WT,
		0777, wt_ctp_proc_dir, &ilitek_proc_lockdown_info_ops);
