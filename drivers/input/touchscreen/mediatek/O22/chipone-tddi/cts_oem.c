...
#define ITO_TEST_OUT_FILEPATH_SIZE    128
...
static char *get_date_time_str(void)
{
    ...
    static char time_data_buf[TIMESTAMP_BUF_SIZE] = {0};
...
static int save_selftest_data_to_file(struct cts_oem_data *oem_data)
{
    ...
    int ret;
    char filepath[ITO_TEST_OUT_FILEPATH_SIZE] = {0};
    char timestamp[TIMESTAMP_BUF_SIZE] = {0};

    strcpy(timestamp , get_date_time_str());
    ...
    snprintf(filepath,sizeof(filepath)-1,"%s%s_%s.csv",OEM_RAWDATA_TEST_DATA_FILEPATH,timestamp,
    oem_data->rawdata_test_result == 0 ? "PASS" : "FAIL");
    if (oem_data->test_rawdata) {
        ret = dump_tsdata_to_csv_file(filepath,
    ...
    memset(filepath,0,sizeof(filepath));
    snprintf(filepath,sizeof(filepath)-1,"%s%s_%s.csv",OEM_NOISE_TEST_DATA_FILEPATH,timestamp,
    oem_data->noise_test_result == 0 ? "PASS" : "FAIL");
    if (oem_data->test_noise) {
        ret = dump_tsdata_to_csv_file(filepath,
    ...
    memset(filepath,0,sizeof(filepath));
    snprintf(filepath,sizeof(filepath)-1,"%s%s_%s.csv",OEM_OPEN_TEST_DATA_FILEPATH,timestamp,
    oem_data->open_test_result == 0 ? "PASS" : "FAIL");
    if (oem_data->test_open) {
        ret = dump_tsdata_to_csv_file(filepath,
    ...
    memset(filepath,0,sizeof(filepath));
    snprintf(filepath,sizeof(filepath)-1,"%s%s_%s.csv",OEM_SHORT_TEST_DATA_FILEPATH,timestamp,
    oem_data->short_test_result == 0 ? "PASS" : "FAIL");
    if (oem_data->test_short) {
        ret = dump_tsdata_to_csv_file(filepath,
    ...
    memset(filepath,0,sizeof(filepath));
    snprintf(filepath,sizeof(filepath)-1,"%s%s_%s.csv",OEM_COMP_CAP_TEST_DATA_FILEPATH,timestamp,
    oem_data->comp_cap_test_result == 0 ? "PASS" : "FAIL");
    if (oem_data->test_comp_cap) {
        ret = dump_comp_cap_to_csv_file(filepath,
...
static int selftest_seq_show(struct seq_file *m, void *v)
{
    ...
    if (oem_data->test_rawdata) {
        ...
        if (oem_data->rawdata_test_result == 0) {
            ...
            for(i = 0; i < oem_data->rawdata_test_frames; i++) {
                dump_tsdata_to_seq_file(m, oem_data->rawdata_test_data + i * hw_rows * hw_cols,
                    rows, cols, hw_cols);
                seq_putc(m, '\n');
            }
        } else {
    ...
    if (oem_data->test_noise) {
        ...
        if (oem_data->noise_test_result == 0) {
            ...
            for(i = 0; i < oem_data->noise_test_frames; i++) {
                dump_tsdata_to_seq_file(m, oem_data->noise_test_data + i * hw_rows * hw_cols,
                    rows, cols, hw_cols);
                seq_putc(m, '\n');
            }
        } else {
    ...
    if (oem_data->test_short) {
        ...
        if (oem_data->short_test_result == 0) {
            ...
            seq_printf(m, " FAIL!\n");
            for (i = 0; i < 10; i++) {
                dump_tsdata_to_seq_file(m, oem_data->short_test_data + i * hw_rows * hw_cols,
                    rows, cols, hw_cols);
                seq_putc(m, '\n');
            }
        } else {
    ...
    if (oem_data->test_comp_cap) {
        ...
        if (oem_data->comp_cap_test_result == 0) {
            ...
            seq_printf(m, " FAIL!\n");
            dump_comp_cap_to_seq_file(m, oem_data->comp_cap_test_data, rows, cols, hw_cols);
        } else {
            seq_printf(m, " ERROR(%d)!\n\n", oem_data->comp_cap_test_result);
        }
    }
...
int cts_ito_test(void){
    ...
    if (tp_cts_data == NULL) {
        cts_err("Open selftest & factory_check with cts_data = NULL");
        return -EFAULT;
    }

    cts_data=tp_cts_data;
    ...
    oem_data = cts_data->oem_data;
    if (oem_data == NULL) {
        cts_err("Open selftest & factory_check with oem_data = NULL");
        return -EFAULT;
    }
    ...
    if (!oem_data->test_config_from_dt_has_parsed) {
        ret = parse_selftest_dt(oem_data, cts_data->pdata->spi_client->dev.of_node);
        if (ret) {
            cts_err("Parse selftest & factory_check dt failed %d", ret);
            return ret;
        }
    }
    ...
    ret = save_selftest_data_to_file(oem_data);
    if (ret) {
        cts_err("Save selftest & factory_check data to file failed %d", ret);
    }

    cts_info("----Show  function seq selftest-----");
    ...
    if (oem_data->reset_pin_test_result || oem_data->rawdata_test_result || oem_data->noise_test_result ||
         oem_data->open_test_result || oem_data->short_test_result || oem_data->comp_cap_test_result) {
        return 0;       //selftest failed
    } else {
        return 1;       //selftest passed
    }
}
/*hs14 code for SR-AL6528A-01-526 by hehaoran5 at 20220919 end*/