...
static int wait_sfctrl_xfer_comp(const struct cts_device *cts_dev)
{
    ...
    do {
        sfctrl_read_reg_check_ret(readb_relaxed, rSFCTRLv2_SF_BUSY,
                      &status);
        if (status == 0)
            return 0;
        mdelay(20);
               ...
               u32 flash_addr, const void *src, size_t size)
{
    return sfctrlv2_transfer(cts_dev, SFCTRLv2_CMD_PP, (void *)src,
                 flash_addr,
                 cts_dev->hwdata->sfctrl->xchg_sram_base, size,
               ...
               u32 sram_addr, size_t size, u32 *crc)
{
    ...
    sfctrl_write_reg_check_ret(writel_relaxed, rSFCTRLv2_SRAM_ADDR,
                   sram_addr);
    mdelay(1);
    sfctrl_write_reg_check_ret(writel_relaxed, rSFCTRLv2_DATA_LENGTH,
                   (u32) size);
    mdelay(1);
                ...
                u32 flash_addr, size_t size, u32 *crc)
{
    ...
    sfctrl_write_reg_check_ret(writel_relaxed, rSFCTRLv2_DATA_LENGTH,
                   (u32) size);
    sfctrl_write_reg_check_ret(writeb_relaxed, rSFCTRLv2_FLASH_CRC_START,
                   1);
