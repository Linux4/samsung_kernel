...
static struct proc_dir_entry *ovt_tcm_info_proc_entry;

static ssize_t ovt_tcm_proc_getinfo_read(struct file *flip,char __user *buff,size_t size,loff_t *pPos)
{
    char buf[50] = {0};
    int rc = 0;
    struct ovt_tcm_hcd *tcm_hcd;
    ...
    /*hs14 code for SR-AL6528A-01-467 by hehaoran5 at 20220909 start*/
    snprintf(buf,sizeof(buf),"IC_NAME:%s TOUCH_VER:%02x \n",tcm_hcd->tp_module_name,tcm_hcd->app_info.customer_config_id[FW_VER] & 0xFF);
    /*hs14 code for SR-AL6528A-01-467 by hehaoran5 at 20220909 end*/
    rc = simple_read_from_buffer(buff,size,pPos,buf,strlen(buf));
    return rc;
}

static const struct file_operations ovt_tcm_info_proc_fops = {
    .owner = THIS_MODULE,
    .read = ovt_tcm_proc_getinfo_read,
};

static int32_t ovt_tcm_extra_proc_init(void)
{
    ovt_tcm_info_proc_entry = proc_create(OVT_TCM_INFO_PROC_FILE,0666,NULL,&ovt_tcm_info_proc_fops);
    if (NULL == ovt_tcm_info_proc_entry) {
...
/*hs14 code for SR-AL6528A-01-458 by hehaoran5 at 20220913 end*/
    ...
    /*hs14 code for AL6528A-7 by hehaoran5 at 20220831 start*/
    LOGI(&pdev->dev,"%s(): start init tp_info\n",__func__);
    ovt_tcm_extra_proc_init();
    LOGI(&pdev->dev,"%s():  init tp_info success\n",__func__);
    /*hs14 code for AL6528A-7 by hehaoran5 at 20220831 end*/

    /*hs14 code for AL6528A-8 by hehaoran5 at 20220831 start*/
    LOGI(&pdev->dev,"%s(): start init ito_test",__func__);
    platform_device_register(&tdinfo_device);
    td_test_node_init(&tdinfo_device);
    LOGI(&pdev->dev,"%s(): ito_test init success",__func__);
    /*hs14 code for AL6528A-8 by hehaoran5 at 20220831 end*/
    /*hs14 code for AL6528A-13 by hehaoran5 at 20220901 start*/
    retval = sec_cmd_init(&tcm_hcd->sec, ovt_tcm_commands, ovt_tcm_get_array_size(), SEC_CLASS_DEVT_TSP);
    if (retval < 0) {
        LOGI(&pdev->dev,"%s: Failed to sec_cmd_init\n", __func__);
    } else {
        LOGI(&pdev->dev,"%s: sec_cmd_init success!\n", __func__);
    }
    ...
    /*hs14 code for AL6528A-213 by hehaoran5 at 20221002 start*/
    tcm_hcd->esd_recovery_notifier.notifier_call = ovt_tcm_esd_recovery_notifier_cb;
    retval = register_esd_tp_recovery_notifier(&tcm_hcd->esd_recovery_notifier);
    if (retval < 0) {
        LOGE(tcm_hcd->pdev->dev.parent,
                "Failed to register FB notifier client\n");
    }
