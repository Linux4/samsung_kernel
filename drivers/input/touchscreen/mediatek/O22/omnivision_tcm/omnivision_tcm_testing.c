...
#define LIMIT_CSV_FILE_PATH "/vendor/firmware/td4375_txd_jdi_mp_test.csv"
...
static int testing_do_testing(void)
{
    ...
    if (error_count) {
        //test fail result
        sprintf(file_path, "/data/tpdata/TP_%s_TEST_DATA_%02d%02d%02d-%02d%02d%02d-FAIL.csv", tcm_hcd->tp_module_name,
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
    } else {
        //test pass result
        sprintf(file_path, "/data/tpdata/TP_%s_TEST_DATA_%02d%02d%02d-%02d%02d%02d-PASS.csv", tcm_hcd->tp_module_name,
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
    }
...
/* O22 code for AL6528A-5 by hehaoran5 at 20220830 start */
...
int td_ito_test(void)
{
    ...
    if ((dev_mkdir(TD_RANGE_TEST_OUT_PATH, 0777)) != 0) {
        printk("Failed to create directory for mp_test\n");
    }

    val = testing_do_testing();
    if (val < 0) {
        printk("%s():ito_test fail",__func__);
        return 0;//fail
    } else {
        printk("%s():ito_test success",__func__);
        return 1;//success
    }
}
/* O22 code for AL6528A-5 by hehaoran5 at 20220830 end */
