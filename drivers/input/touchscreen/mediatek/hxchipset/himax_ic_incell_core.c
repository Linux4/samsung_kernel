...
static void himax_mcu_read_FW_ver(void)
{
	...
	I("Project ID = %s\n", ic_data->vendor_proj_info);
        ...
        snprintf(firmware_ver,HARDWARE_MAX_ITEM_LONGTH,"HX83104-A,VND:Innolux,FW:0x%02x", ic_data->vendor_touch_cfg_ver);
		...
		uint8_t *addr, int start_index, int read_len)
{
	...
	I("%s, total size=%d, bus size=%d, read time=%d\n",  __func__, total_size, max_bus_size, total_read_times);
