...
uint16_t gest_key_def[GEST_SUP_NUM] = {
        KEY_HOMEPAGE, 251, 252, 253, 254, 255, 256, 257,  //KEY_POWER
	258, 259, 260, 261, 262, 263, 264, 265,
	266, 267, 268, 269, 270, 271, 272, 273,
	274, 275};

uint8_t *wake_event_buffer;
uint8_t himax_gesture_status = 0;
#endif
...
char *i_CTPM_firmware_name = "Himax_fw.bin";
 ...
 */
	...
	if ((val & HX_INSP_EFILE) == HX_INSP_EFILE)
		seq_puts(s, "  Get criteria File Fail\n");
	if ((val & HX_INSP_MEMALLCTFAIL) == HX_INSP_MEMALLCTFAIL)
		seq_puts(s, "  Allocate memory Fail\n");
	if ((val & HX_INSP_EGETRAW) == HX_INSP_EGETRAW)
		seq_puts(s, "  Get raw data Fail\n");
	if ((val & HX_INSP_ESWITCHMODE) == HX_INSP_ESWITCHMODE)
...
static void get_fw_ver_bin(void *device_data)
{
	...
	get_FW_version_from_bin();
	 ...
	 snprintf(buff, sizeof(buff) ,"HX83104-A,VND:Innolux,FW:0x%02x", g_i_FW_VER_BIN);
...
static void get_fw_ver_ic(void *device_data)
{
	...
	I("himax Fw version from IC  is  0x%02x\n", ic_data->vendor_touch_cfg_ver);
        ...
        snprintf(buff, sizeof(buff) ,"HX83104-A,VND:Innolux,FW:0x%02x", ic_data->vendor_touch_cfg_ver);
...
static void aot_enable(void *device_data)
{
    ...
    if (sec->cmd_param[0] < 0 || sec->cmd_param[0] > 1) {
        ...
        sec_cmd_set_cmd_result(sec, buff, strnlen(buff, sizeof(buff)));
        E( "%s: sec->cmd_param[0] == %s, buff == %s,sec->cmd_state == %d\n", __func__, sec->cmd_param[0], buff, sec->cmd_state);
