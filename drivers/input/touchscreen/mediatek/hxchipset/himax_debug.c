...
/* moved from debug.h */
struct proc_dir_entry *himax_proc_debug_level_file;
struct proc_dir_entry *himax_proc_vendor_file;
struct proc_dir_entry *himax_proc_attn_file;
struct proc_dir_entry *himax_proc_int_en_file;
struct proc_dir_entry *himax_proc_layout_file;
struct proc_dir_entry *himax_proc_CRC_test_file;
...
struct proc_dir_entry *himax_proc_SENSE_ON_OFF_file;
	...
	struct proc_dir_entry *himax_proc_guest_info_file;
#endif
...
static int himax_diag_seq_read(struct seq_file *s, void *v)
{
	...
	if ((ts->diag_cmd >= 1 && ts->diag_cmd <= 3) || (ts->diag_cmd == 7)) {
		...
		for (loop_i = 0; loop_i < ic_data->HX_BT_NUM; loop_i++)
			seq_printf(s, "%4d", diag_self[ic_data->HX_RX_NUM_2 +
...
bool himax_ts_diag_func(void)
{
	...
	unsigned int temp_buf_len = 0;//add by songbinbo.wt for Himax tp driver modify bug 2019.05.14
	...
	if (dsram_type >= 1 && dsram_type <= 3) {
		...
		for (i = 0; i < x_channel * y_channel; i++) {
			...
			temp_buf_len =  strlen(temp_buf);//add by songbinbo.wt for Himax tp driver modify bug 2019.05.14
			strlcat(&write_buf[i*temp_buf_len], temp_buf, temp_buf_len);//add by songbinbo.wt for Himax tp driver modify bug 2019.05.14
		...
		for (i = 0; i < x_channel; i++) {
			...
			temp_buf_len =  strlen(temp_buf);//add by songbinbo.wt for Himax tp driver modify bug 2019.05.14
			strlcat(&write_buf[(i+x_channel * y_channel) *temp_buf_len], temp_buf, temp_buf_len);//add by songbinbo.wt for Himax tp driver modify bug 2019.05.14
		}
...
#define DIAG_NOT_SUPP "[Himax]ts->diag_cmd=0x%x ,storage_type=%d,"\
			" rawdata_type=%d! Maybe no support!\n"
#define DIAG_ERR_RESET "[Himax]ts->diag_cmd error!diag_command=0x%x so reset\n"
			...
			size_t len, loff_t *data)
{
	...
	if (ts->diag_cmd > 0 && rawdata_type == 0) {
		I(DIAG_NOT_SUPP, ts->diag_cmd, storage_type, rawdata_type);
		ts->diag_cmd = 0x00;
	} else {
	...
	memset(diag_mutual, 0x00,
	  ic_data->HX_RX_NUM * ic_data->HX_TX_NUM * sizeof(int32_t));
	/* Set data 0 */
	...
	if (storage_type == 0 && rawdata_type > 0 && rawdata_type < 8) {
		...
		if (storage_type == 4) {
			switch (rawdata_type) {
			case 1:
			vts_name = kp_getname_kernel(IIR_DUMP_FILE);
			diag_sram_fn = kp_file_open_name(vts_name,
						O_CREAT | O_WRONLY, 0);
			break;

			case 2:
			vts_name = kp_getname_kernel(DC_DUMP_FILE);
			diag_sram_fn = kp_file_open_name(vts_name,
						O_CREAT | O_WRONLY, 0);
			break;

			case 3:
			vts_name = kp_getname_kernel(BANK_DUMP_FILE);
			diag_sram_fn = kp_file_open_name(vts_name,
						O_CREAT | O_WRONLY, 0);
			break;
			  ...
			  NO_ERR) {
				for (loop_i = 0; loop_i < cmd_set[0];
				  loop_i++) {
					if ((loop_i % 8) == 0)
						ret += snprintf(temp_buf + ret,
						len - ret, "0x%02X : ", loop_i);

					ret += snprintf(temp_buf + ret,
					len - ret, "0x%02X ", tmp_data[loop_i]);
					...
					size_t len, loff_t *pos)
{
	...
	uint8_t **p_guest_str_fmt =
		(uint8_t **)g_guest_info_data->g_guest_str_in_format;
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (g_core_fp.guest_info_get_status()) {
			...
			if (info->g_guest_info_type == 1) {
				for (j = 0; j < 3; j++) {
					ret = printUnit(temp_buf, len, max_size,
					 g_guest_info_item[j],
				...
				for (j = 0; j < 10; j++) {
					...
					if (info->g_guest_data_type[j] == 0)
						...
						ret = printMat(temp_buf, len,
						 info->g_guest_data_len[j],
						 p_guest_str_fmt[j], ret);
...
int himax_touch_proc_init(void)
{
	...
	himax_proc_reset_file = proc_create(HIMAX_PROC_RESET_FILE,
				0200, himax_touch_proc_dir,
				&himax_proc_reset_ops);
	...
	himax_proc_fw_debug_file = proc_create(HIMAX_PROC_FW_DEBUG_FILE,
				0644, himax_touch_proc_dir,
	...
	himax_proc_dd_debug_file = proc_create(HIMAX_PROC_DD_DEBUG_FILE,
				0644, himax_touch_proc_dir,
...
#ifdef HX_ESD_RECOVERY
	himax_proc_ESD_cnt_file = proc_create(HIMAX_PROC_ESD_CNT_FILE,
				0666,
...
#endif
	himax_proc_CRC_test_file = proc_create(HIMAX_PROC_CRC_TEST_FILE,
				0666,
...
#ifdef HX_TP_PROC_GUEST_INFO
	himax_proc_guest_info_file = proc_create(HIMAX_PROC_GUEST_INFO_FILE,
			0666, himax_touch_proc_dir, &himax_proc_guest_info_ops);
