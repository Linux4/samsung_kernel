...
static void ilitek_resume_by_ddi_work(struct work_struct *work)
{
	...
	if ((ilits->gesture) ||(ilits->spay_gesture))
...
int ili_sleep_handler(int mode)
{
	...
	case TP_SUSPEND:
		...
		if ((ilits->gesture) ||(ilits->spay_gesture) || (ilits->prox_face_mode == true)) {
...
int ili_set_tp_data_len(int format, bool send, u8 *data)
{
	...
	case DATA_FORMAT_GESTURE_DEBUG:
		...
		/*add for gesture mode position customer type 20221019*/
		if (ilits->rib.nCustomerType != POSITION_CUSTOMER_TYPE_OFF) {
			len += P5_X_CUSTOMER_LENGTH;
		}
...
static void ili_update_tp_module_info(void)
{
	...
	case MODEL_CW:
		ilits->md_name = "CW";
		ilits->md_fw_filp_path = CW_FW_FILP_PATH;
		ilits->md_fw_rq_path = CW_FW_REQUEST_PATH;
		ilits->md_ini_path = CW_INI_NAME_PATH;
		ilits->md_ini_rq_path = CW_INI_REQUEST_PATH;
		ilits->md_fw_ili = CTPM_FW_CW;
		ilits->md_fw_ili_size = sizeof(CTPM_FW_CW);
		break;
	default:
		break;
	}
