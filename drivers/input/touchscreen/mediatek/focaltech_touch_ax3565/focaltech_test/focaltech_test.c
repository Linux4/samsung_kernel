...
*****************************************************************************/
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 start*/
void fts_ito_test_work(struct work_struct *work)
{
    struct file *pfile = NULL;
    loff_t pos;
    mm_segment_t old_fs;
    struct fts_test *tdata = fts_ftest;
    FTS_TEST_INFO("write ito data start\n");
    mutex_lock(&tdata->ito_lock);

    if (NULL == pfile) {
        pfile = filp_open(tdata->data_filepath, O_TRUNC | O_CREAT | O_RDWR, 0);
    }
    if (IS_ERR(pfile)) {
        FTS_TEST_ERROR("error occured while opening file %s.",  tdata->data_filepath);
        mutex_unlock(&tdata->ito_lock);
        return;
    }
    ...
    set_fs(KERNEL_DS);
    pos = 0;
    vfs_write(pfile, tdata->data_buffer, tdata->data_buffer_len, &pos);
    filp_close(pfile, NULL);
    pfile = NULL;
    set_fs(old_fs);
    if (tdata->data_buffer) {
        vfree(tdata->data_buffer);
        tdata->data_buffer = NULL;
    }

    if (NULL == pfile) {
        pfile = filp_open(tdata->result_filepath, O_TRUNC | O_CREAT | O_RDWR, 0);
    }
    if (IS_ERR(pfile)) {
        FTS_TEST_ERROR("error occured while opening file %s.",  tdata->result_filepath);
        mutex_unlock(&tdata->ito_lock);
        return;
    }
    ...
    set_fs(KERNEL_DS);
    pos = 0;
    vfs_write(pfile, tdata->testresult, tdata->testresult_len, &pos);
    filp_close(pfile, NULL);
    set_fs(old_fs);
    fts_test_malloc_free_data_txt(tdata, false);
    mutex_unlock(&tdata->ito_lock);
    FTS_TEST_INFO("write ito data end\n");
    return;
}
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 end*/
...
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 start*/
    struct fts_test *tdata = fts_ftest;
    FTS_TEST_FUNC_ENTER();
    FTS_TEST_INFO("file_name = %s,len = %d , data_filepath size = %d",file_name,len, sizeof(tdata->data_filepath));
    if (NULL != strstr(file_name, "testdata")) {
        memset(tdata->data_filepath, 0, sizeof(tdata->data_filepath));
        if ( fwupgrade != NULL && fwupgrade->module_info != NULL) {
            sprintf(tdata->data_filepath, "%s%s_%s%s", FTS_INI_FILE_PATH, get_date_time_str(), fwupgrade->module_info->vendor_name, file_name);
        } else {
            sprintf(tdata->data_filepath, "%s%s", FTS_INI_FILE_PATH, file_name);
        }
        if(tdata->data_buffer == NULL) {
            tdata->data_buffer = vmalloc(CSV_BUFFER_LEN);
            if (!tdata->data_buffer) {
                FTS_TEST_ERROR("tdata->data_buffer malloc fail\n");
                return 0;
            }
        }
        memset(tdata->data_buffer, 0, CSV_BUFFER_LEN);
        memcpy(tdata->data_buffer, data_buf, len);
        tdata->data_buffer_len = len;
    } else if (NULL != strstr(file_name, "testresult")) {
        memset(tdata->result_filepath, 0, sizeof(tdata->result_filepath));
        if ( fwupgrade != NULL && fwupgrade->module_info != NULL) {
            sprintf(tdata->result_filepath, "%s%s_%s%s", FTS_INI_FILE_PATH, get_date_time_str(), fwupgrade->module_info->vendor_name, file_name);
        } else {
            sprintf(tdata->result_filepath, "%s%s", FTS_INI_FILE_PATH, file_name);
        }
...
test_err:
    ...
    enter_work_mode();
    /*TabA7 Lite code for OT8-2503 by liufurong at 20210210 start*/
    mutex_unlock(&fts_ftest->ito_lock);
    queue_delayed_work(fts_ftest->ito_wq, &fts_ftest->ito_test_wq, msecs_to_jiffies(1));
    /*TabA7 Lite code for OT8-2503 by liufurong at 20210210 end*/
    return ret;
}
/* Huaqin add for SR-AX3565-01-700 ito test by fengzhigang at 2021/01/11 end */
...
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 start*/
    fts_ftest->ito_wq = alloc_workqueue("fts_ito_wq", WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
    INIT_DELAYED_WORK(&fts_ftest->ito_test_wq, fts_ito_test_work);
    mutex_init(&fts_ftest->ito_lock);
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 end*/
