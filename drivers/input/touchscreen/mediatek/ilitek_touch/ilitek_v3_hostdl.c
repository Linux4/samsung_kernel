...
static int ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	ILI_INFO("report_info_block : nIsI2C = %d, nReserved00 = %d, nReportResolutionMode = %d, nReserved02 = %x,  nReserved03 = %x\n",
		ilits->rib.nIsI2C, ilits->rib.nReserved00, ilits->rib.nReportResolutionMode, ilits->rib.nReserved02, ilits->rib.nReserved03);
...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
	...
	case REQUEST_FIRMWARE:
		while (request_firmware(&fw, ilits->md_fw_rq_path, ilits->dev) < 0) {
			ILI_ERR("Request firmware failed, try again,retry_count is %d\n",retry_count);
			mdelay(100);
			retry_count++;
			if (retry_count >= 40)
				goto out;
