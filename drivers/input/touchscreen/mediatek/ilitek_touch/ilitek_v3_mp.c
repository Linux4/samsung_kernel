...
static char *g_ito_data_csv = NULL;
static int g_ito_data_len = 0;
...
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 start*/
void ilitek_ito_test_work(struct work_struct *work)
{
	struct file *f = NULL;
	mm_segment_t fs;
	loff_t pos;

	ILI_INFO("write ito data start Open CSV : %s\n", csv_name);
	mutex_lock(&ilits->ito_lock);
	if (f == NULL)
		f = filp_open(csv_name, O_WRONLY | O_CREAT | O_TRUNC, 644);

	if (ERR_ALLOC_MEM(f)) {
		ILI_ERR("Failed to open CSV file");
		mutex_unlock(&ilits->ito_lock);
		return;
	}
	...
	set_fs(KERNEL_DS);
	pos = 0;
	vfs_write(f, g_ito_data_csv, g_ito_data_len, &pos);
	set_fs(fs);
	filp_close(f, NULL);
	if (g_ito_data_csv) {
		ipio_vfree((void **)&g_ito_data_csv);
		g_ito_data_csv = NULL;
	}
	mutex_unlock(&ilits->ito_lock);
	ILI_INFO("write ito data end\n");
	return;
}
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 end*/
...
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 start*/
	mutex_lock(&ilits->ito_lock);
	if (g_ito_data_csv == NULL) {
		g_ito_data_csv = vmalloc(CSV_FILE_SIZE);
		if (ERR_ALLOC_MEM(csv)) {
			ILI_ERR("Failed to allocate CSV mem\n");
			ret = -EMP_NOMEM;
			goto fail_open;
	...
	memset(g_ito_data_csv, 0, CSV_FILE_SIZE);
	mutex_unlock(&ilits->ito_lock);
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 end*/
	max_threshold = kcalloc(core_mp.frame_len, sizeof(s32), GFP_KERNEL);
	min_threshold = kcalloc(core_mp.frame_len, sizeof(s32), GFP_KERNEL);
...
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 start*/
	memcpy(g_ito_data_csv, csv, csv_len);
	g_ito_data_len = csv_len;
	queue_delayed_work(ilits->ilitek_ito_wq, &ilits->ilitek_ito_test_wq, msecs_to_jiffies(10));
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 end*/
