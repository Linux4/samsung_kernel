...
static ssize_t ili_test_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	int count = 0;
	int ret = 0;
...
static ssize_t ili_test_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	return count;
}

static DEVICE_ATTR(factory_check, 0644, ili_test_show, ili_test_store);
...
static ssize_t ili_proc_getinfo_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	int rc = 0,temp = 0;
	char *buf = NULL;

	buf = kzalloc(150, GFP_KERNEL);
	if (ERR_ALLOC_MEM(buf)) {
		ILI_ERR("Failed to allocate tp_info_buf\n");
		rc = -ENOMEM;
		goto out;
	}
	mutex_lock(&ilits->touch_mutex);
	...
	snprintf(buf, 150, "tp_module=%s fw_ver=%d.%d.%d.%d\n",ilits->md_name, (temp >> 24) & CHAR_MASK, (temp >> 16) & CHAR_MASK, (temp >> 8) & CHAR_MASK, temp & CHAR_MASK);
...
static const struct file_operations ili_info_proc_fops = {
	.owner = THIS_MODULE,
	.read = ili_proc_getinfo_read,
};

int32_t ili_extra_proc_init(void)
{
	ilitek_info_proc_entry = proc_create(ilitek_INFO_PROC_FILE, 0777, NULL, &ili_info_proc_fops);
...
void ili_node_init(void)
{
	...
	if (!sec_info) {
		ILI_ERR("allocate memory for sec_info fail");
		return ;
	}
	ilits->sec_info = sec_info;
	...
	ilits->ilitek_ito_wq = alloc_workqueue("ilitek_ito_wq", WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
	INIT_DELAYED_WORK(&ilits->ilitek_ito_test_wq, ilitek_ito_test_work);
	mutex_init(&ilits->ito_lock);
