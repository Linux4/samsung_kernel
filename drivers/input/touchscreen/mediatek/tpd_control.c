			...
			unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		...
		if (blank == FB_BLANK_UNBLANK) {
			TPD_DMESG("LCD ON Notify, event=%lu, blank=%d\n", event, blank);
		...
		if (blank == FB_BLANK_POWERDOWN) {
			TPD_DMESG("LCD OFF Notify, event=%lu, blank=%d\n", event, blank);
			if (g_tpd_drv && !tpd_suspend_flag) {
...
#define MTK_TPD_INFO_PROC_FILE "tp_info"
static struct proc_dir_entry *mtk_tpd_info_proc_entry;
char *mtp_chip_name = "NULL";
int mtp_fw_ver = 0;

static ssize_t mtk_tpd_proc_getinfo_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
        int rc = 0;
...
static const struct file_operations mtk_tpd_info_proc_fops = {
        .owner = THIS_MODULE,
        .read = mtk_tpd_proc_getinfo_read,
};

static int32_t mtk_tpd_proc_init(void)
{
        mtk_tpd_info_proc_entry = proc_create(MTK_TPD_INFO_PROC_FILE, 0777, NULL, &mtk_tpd_info_proc_fops);
        if (NULL == mtk_tpd_info_proc_entry)
        {
