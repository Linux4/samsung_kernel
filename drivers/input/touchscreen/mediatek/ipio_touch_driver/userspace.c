...
{
	int ret = 0;
	char cmd[10] = { '\0' };	//+modified by songbinbo.wt for ILITEK tp Coverity scan  CID 77139 20190509
...
static ssize_t ilitek_proc_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	...
	data = kmalloc(512 * sizeof(uint8_t), GFP_KERNEL);
        if (ERR_ALLOC_MEM(data)) {
		ipio_err("Failed to allocate a data buffer, %ld\n", PTR_ERR(data));
		return -ENOMEM;
	}
	memset(data, 0, 512);
	...
	if (strcmp(cmd, "reset") == 0) {
		...
		temp[0] = 0xFA;
		temp[1] = 0x08;
		val = core_write(core_config->slave_i2c_addr, temp, 2);
		if(val < 0){
                      ipio_err("test baseline raw write cmd failed.\n");
		}
		ilitek_platform_enable_irq();
		...
		val = core_write(core_config->slave_i2c_addr, temp, 2);
                if(val < 0){
                    ipio_err("test get delac write cmd failed.\n");
                 }
		ilitek_platform_enable_irq();
		...
		val = core_write(core_config->slave_i2c_addr, temp, w_len);
		if(val < 0){
                    ipio_err("write i2c_w cmd failed.\n");
		}
		...
		val = core_write(core_config->slave_i2c_addr, temp, w_len);
		if(val < 0){
                       ipio_err("write i2c_w_r cmd failed.\n");
		}
...
#ifdef CONFIG_COMPAT
static long ilitek_proc_ioctl_compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
...
#ifdef CONFIG_COMPAT
    .compat_ioctl   = ilitek_proc_ioctl_compat,
#endif
	...
	for (; i < ARRAY_SIZE(proc_table); i++) {
		...
		if (proc_table[i].node == NULL) {
			...
			proc_table[i].isCreated = true;
		        ipio_wt_info("Succeed to create %s under /proc\n", proc_table[i].name);
		}
	}
