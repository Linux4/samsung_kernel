...
//	dma_txbuf = kmalloc(SPI_WRITE_BUFF_MAXSIZE, GFP_KERNEL);

	if (!dma_txbuf) {		
		ipio_info("alloc dma_txbuf error");
		status = -ENOMEM;
		goto out;
	}
	dma_rxbuf = kzalloc(SPI_READ_BUFF_MAXSIZE, GFP_KERNEL);
//	dma_rxbuf = kmalloc(SPI_READ_BUFF_MAXSIZE, GFP_KERNEL);
	if (!dma_rxbuf) {
		ipio_info("alloc dma_rxbuf error");
		status = -ENOMEM;
		goto out;
	}
	if(!mutex_trylock(&lock)){
		ipio_info("mutex trylock error");
	};
...
int core_tx_unlock_check(void)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (CHECK_EQUAL(status, unlock) == 0) {
			ipio_wt_info("Tx check unlock free!\n");
			return 0;
		}
...
{
	int ret = 0;
	ret = core_spi_ice_mode_unlock_read(pBuf, nSize);
	if ( ret < 0) {
		ipio_err("spi ice mode unlock write failed\n");
		goto out;
	}
...
{
	int ret = 0, size = 0, count = 3;
	bool ice_modem_ok = false;
...
#else
	core_spi->spi_write_then_read = core_mtk_spi_write_then_read;//spi_write_then_read;
#endif
