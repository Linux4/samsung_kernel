	...
	switch(core_fr->actual_fw_mode) {
		...
		case P5_0_FIRMWARE_TEST_MODE:
			...
			if (INTERFACE == I2C_INTERFACE) {
				...
				val = core_write(core_config->slave_i2c_addr, cmd, 1);
				if(val < 0){
                                    ipio_err("read mp test info write cmd failed.\n");
				}
				mdelay(10);
...
//-add by songbinbo.wt for ILITEK tp reset sequence 20190417
	...
	core_config->isSuspending = false;
...
int core_config_ice_mode_enable(bool stop_mcu)
{
	...
	do {
		...
		if (ret < 0){
			...
			continue;//add by songbinbo.wt for ILITEK tp enter ice_mode optimize 20190419
		}
	...
	core_config->icemodeenable = false;//add by songbinbo.wt for ILITEK tp enter ice_mode optimize 20190419
...
int core_config_check_cdc_busy(int conut, int delay)
{
	...
	while (timer > 0) {
		val = core_write(core_config->slave_i2c_addr, cmd, 2);
		if(val < 0){
                     ipio_err("Failed to write read ctrl cmd\n");
		}
		val = core_write(core_config->slave_i2c_addr, &cmd[1], 1);
		if(val < 0){
                     ipio_err("Failed to write cdc busy cmd\n");
		}
		val = core_read(core_config->slave_i2c_addr, &busy, 1);
		if (val < 0){
                    ipio_err("Failed to write cdc check busy cmd\n");
		}
...
int core_config_get_protocol_ver(void)
{
	...
	ipio_wt_info("Procotol Version = %d.%d.%d\n",
...
int core_config_get_core_ver(void)
{
	...
	/* in protocol v5, ignore the first btye because of a header. */
	ipio_wt_info("Core Version = %d.%d.%d.%d\n",
...
int core_config_get_chip_id(void)
{
	...
	ipio_wt_info("Chip ID = 0x%x\n", core_config->chip_id);
	ipio_wt_info("Chip Type = 0x%x\n", core_config->chip_type);
	ipio_wt_info("Chip Core id = 0x%x\n", core_config->core_type);
	ipio_wt_info("OTP ID = 0x%x\n", core_config->chip_otp_id);
	ipio_wt_info("ANA ID = 0x%x\n", core_config->chip_ana_id);
	if(!is_first_in_ice)
		core_config_ice_mode_disable();
	return ret;
}
