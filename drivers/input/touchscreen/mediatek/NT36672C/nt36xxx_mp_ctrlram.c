	...
	unsigned char mpcriteria[32] = {0}; //novatek-mp-criteria-default
		...
		if(of_property_read_bool(np, "novatek,mp-support-dt")) {
			...
			if (nvt_mp_parse_dt(np, mpcriteria)) {
				...
				nvt_update_firmware(ts->platdata->firmware_name);
				mutex_unlock(&ts->lock);
				printk("mp parse device tree failed!\n");
				return -EINVAL;
			}
		} else {
			printk("Not found novatek,mp-support-dt, use default setting\n");
			//---Print Test Criteria---
			nvt_print_criteria();
		}
		...
		if (nvt_switch_FreqHopEnDis(FREQ_HOP_DISABLE)) {
			mutex_unlock(&ts->lock);
			printk("switch frequency hopping disable failed!\n");
			return -EAGAIN;
		}

		if (nvt_check_fw_reset_state(RESET_STATE_NORMAL_RUN)) {
			mutex_unlock(&ts->lock);
			printk("check fw reset state failed!\n");
			return -EAGAIN;
		}
