...
#define FW_BIN_NAME_HKC "gcore_tp_fw_hkc.bin"
#define FW_BIN_NAME_TXD "gcore_tp_fw_txd.bin"
...
int gcore_send_mipi_reset(void)
{
	...
	|| defined(CONFIG_ENABLE_CHIP_TYPE_GC7202H_GC7272)
...
static void aot_enable(void *device_data)
{
    ...
    if(buf[0] == 1){
        ...
        gc_gestrue_status = gdev_fwu->gesture_wakeup_en;
    }else{
...
#if 1	//S96818AA1-9394,wangtao14.wt,modify,2023/08/08,gc7272 Add support_feature node
//modify 7/4
static ssize_t gc_support_feature(struct device *dev,
...
static DEVICE_ATTR(support_feature, 0444, gc_support_feature, NULL);

static struct attribute *gc_cmd_attributes[] = {
    &dev_attr_support_feature.attr,
    NULL,
};

static struct attribute_group gc_cmd_attr_group = {
    .attrs = gc_cmd_attributes,
};
#endif
...
#if defined(CONFIG_ENABLE_CHIP_TYPE_GC7202H_GC7272) \
			|| defined(CONFIG_ENABLE_CHIP_TYPE_GC7302)
	u8 ckucr0[4] = { 0 };
#endif
...
//+S96818AA1-1936,wangtao14.wt,add,2023/07/10,gc7272 modify Let the TP mp test process complete, and then notify the event
...
//+S96818AA1-1936,wangtao14.wt,add,2023/07/10,gc7272 No command is sent when the screen is off
...
//+S96818AA1-1936,wangtao14.wt,modify,2023/07/06,gc7272 add download times
int notify_contin;
...
//+S96818AA1-1936,wangtao14.wt,modify,2023/07/06,gc7272 gcore CB value download
...
//+S96818AA1-1936,wangtao14.wt,modify,2023/07/06,gc7272 Software reset changed to hardware reset
	gcore_reset();
#if 0
...
//-S96818AA1-1936,wangtao14.wt,add,2023/07/10,gc7272 ESD
    ...
    if (strstr(saved_command_line,"n28_gc7272_dsi_vdo_hdp_xinxian_hkc")){
        if (request_firmware(&fw, FW_BIN_NAME_HKC, &gdev_fwu->bus_device->dev)) {
...
//-S96818AA1-1936,daijun1.wt,add,2023/09/18,n28-tp add firmware upgrade path switching function
	...
	gcore_read_fw_version(read_data, sizeof(read_data));
    GTP_DEBUG("gc_tpfwver_show fw_version is : 0x0%d%d%d%d\n",read_data[1],read_data[0],read_data[3],read_data[2]);
    if (strstr(saved_command_line,"n28_gc7272_dsi_vdo_hdp_xinxian_hkc")){
        sprintf(Ctp_name,"HKC,GC7272,FW:0x0%d\n",read_data[2]);
    }else if(strstr(saved_command_line,"n28_gc7272_dsi_vdo_hdp_txd_sharp")){
	...
	gcore_tp_esd_fail = false;
#endif
//+S96818AA1-1936,wangtao14.wt,add,2023/07/10,gc7272 creat resume notify workqueue
#ifdef	CONFIG_SAVE_CB_CHECK_VALUE
		queue_delayed_work(fn_data.gdev->fwu_workqueue,
							   &fn_data.gdev->cb_work,
							   msecs_to_jiffies(20));
...
{
//+S96818AA1-1936,wangtao14.wt,add,2023/07/10,gc7272 add command_line  to distinguish lcm
	if (((!strstr(saved_command_line,"n28_gc7272_dsi_vdo_hdp_xinxian_hkc"))&&(!strstr(saved_command_line,"n28_gc7272_dsi_vdo_hdp_txd_sharp"))) || (touch_bootmode == 8)){
		return;
	}
//-S96818AA1-1936,wangtao14.wt,add,2023/07/10,gc7272 add command_line  to distinguish lcm
...
//+S96818AA1-1936,wangtao14.wt,modify,2023/07/06,gc7272 set GPIO-reset value
...
static struct device_attribute *gcore_attribute[] = {
	...
	&dev_attr_notify_fw,
//+S96818AA1-1936,wangtao14.wt,modify,2023/07/06,gc7272 gpio-reset
	&dev_attr_rst_gpio,
//-S96818AA1-1936,wangtao14.wt,modify,2023/07/06,gc7272 gpio-reset
#ifdef CONFIG_GCORE_AUTO_UPDATE_FW_FLASHDOWNLOAD
...
//+S96818AA1-9394,wangtao14.wt,modify,2023/08/08,gc7272 Add support_feature node
#if 1
//modify 7/4
    ret = sysfs_create_group(&gdev->sec.fac_dev->kobj, &gc_cmd_attr_group);
    if (ret) {
        GTP_ERROR( "%s: failed to create sysfs attributes\n",__func__);
           goto out;
	}
#endif
	return 0;
...
//+S96818AA1-1936,wangtao14.wt,modify,2023/07/06,exit sec
#ifdef SEC_TSP_FACTORY_TEST
    sec_cmd_exit(&gdev->sec, SEC_CLASS_DEVT_TSP);
#endif
//+S96818AA1-1936,wangtao14.wt,modify,2023/07/06,exit sec
#if 0
//modify 7/4
	sysfs_remove_group(&gdev->sec.fac_dev->kobj, &gc_cmd_attr_group);
#endif
