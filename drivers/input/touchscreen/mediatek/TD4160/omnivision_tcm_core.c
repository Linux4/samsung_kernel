...
int ovt_tcm_set_func_##c_name##_en_state(bool en) \
{ \
	int retval = 0; \
...
\
		...
		retval = tcm_hcd->set_dynamic_config(tcm_hcd, id, en); \
		if (retval != 0) { \
			LOGE(tcm_hcd->pdev->dev.parent,"Failed to set #c_name command\n"); \
		} \
	} \
	tcm_hcd->func_##c_name##_en = en; \
	return retval; \
} \
...
static ssize_t ovt_support_feature(struct device *dev,
    struct device_attribute *attr, char *buf)
{
    u32 feature = 0;
...
//+S96818AA1-1936,daijun1.wt,modify,2023/05/23,n28-tp add SS func node and double click to wakeup
    ...
    retval = sysfs_create_group(&tcm_hcd->sec.fac_dev->kobj, &ovt_cmd_attr_group);
    if (retval){
        LOGE(tcm_hcd->pdev->dev.parent,"%s: failed to create sysfs attributes\n",__func__);
        goto err_creste_sec_file;
    }
//-S96818AA1-1936,daijun1.wt,modify,2023/05/23,n28-tp add SS func node and double click to wakeup
#if 0
...
#endif
//+S96818AA1-1936,daijun1.wt,modify,2023/05/23,n28-tp add SS func node and double click to wakeup
        sysfs_remove_group(&tcm_hcd->sec.fac_dev->kobj, &ovt_cmd_attr_group);
        sec_cmd_exit(&tcm_hcd->sec, SEC_CLASS_DEVT_TSP);
//-S96818AA1-1936,daijun1.wt,modify,2023/05/23,n28-tp add SS func node and double click to wakeup
	for (idx = 0; idx < ARRAY_SIZE(dynamic_config_attrs); idx++) {
		sysfs_remove_file(tcm_hcd->dynamnic_config_sysfs_dir,
...
static int tpd_local_init(void)
{
    ...
    printk("ovt tcm driver tpd_local_init\n");
