...
#ifdef SAMSUNG_PROCTORL
		input_report_key(ilits->input, BTN_PLAM, ilits->palmflag);
#endif
...
#ifdef SAMSUNG_PROCTORL
	if(ilits->rib.nReportResolutionMode == POSITION_LOW_RESOLUTION) {
		ilits->palmflag = ((buf[P5_X_DEMO_MODE_PACKET_LEN+P5_X_SAMSUNG_AXIS_LEN+1] & 0x08) >> 3);
	}
	else {
		ilits->palmflag = ((buf[P5_X_DEMO_MODE_PACKET_LEN_HIGH_RESOLUTION+P5_X_SAMSUNG_AXIS_LEN+1] & 0x08) >> 3);
	}
	ILI_DBG("palmflag = %d\n", ilits->palmflag);
#endif
...
void ili_debug_mode_report_point(u8 *buf, int len)
{
	...
	ilits->palmflag = ((buf[axis_addr + P5_X_SAMSUNG_AXIS_LEN + 2] & 0x08) >> 3);
	ILI_DBG("palmflag = %d\n", ilits->palmflag);
#endif
