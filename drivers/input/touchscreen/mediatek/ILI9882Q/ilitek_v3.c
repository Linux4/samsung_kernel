...
static void ilitek_resume_by_ddi_work(struct work_struct *work)
{
	...
	if (ilits->fw_upgrade_mode == UPGRADE_IRAM) {
		if (ili_fw_upgrade_handler(NULL) < 0)
			ILI_ERR("ili_fw_upgrade_handler fail\n");
	} else {
...
//	demo_mode_packet_len += P5_X_SAMSUNG_PACKET_INFO_LEN + P5_X_SAMSUNG_AXIS_LEN +P5_X_SAMSUNG_STATE_INFO;
//#endif
	switch (format) {
...
#ifdef SAMSUNG_PROCTORL
		len += (2 * ilits->xch_num * ilits->ych_num) + (ilits->stx * 2) + (ilits->srx * 2) + (2 * self_key);
		len += P5_X_INFO_CHECKSUM_LENGTH;
		len += P5_X_SAMSUNG_AXIS_LEN + P5_X_SAMSUNG_STATE_INFO;
#else
	...
	case DATA_FORMAT_GESTURE_SPECIAL_DEMO:
		...
		ret = ili_ic_func_ctrl("gesture_demo_en", ilits->gesture_demo_ctrl);
		if (ret < 0)
			ILI_ERR("gesture_demo_en fail\n");

		ILI_INFO("knock_en setting\n");
		ret = ili_ic_func_ctrl("knock_en", 0x8);
		if (ret < 0)
			ILI_ERR("knock_en fail\n");
...
int ili_report_handler(void)
{
	...
	case P5_X_DEBUG_PACKET_ID:
		...
		break;
#ifdef SAMSUNG_PROCTORL
	case P5_X_DEMO_SAMSUNG_PACKET_ID:
		...
		break;
	case P5_X_DEBUG_SAMSUNG_PACKET_ID:
		ili_report_debug_mode(trdata, rlen);
		break;
#else
