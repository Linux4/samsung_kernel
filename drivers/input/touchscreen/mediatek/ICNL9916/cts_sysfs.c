...
static void get_fw_ver_bin(void *device_data)
{
	...
	cts_tcs_get_fw_ver(cts_dev, &cts_dev->fwdata.version);
    if (g_lcm_name == 25) {
		snprintf(buff, sizeof(buff), "xinxain,icnl9916 fw:%04x\n",cts_dev->fwdata.version);
	}else {
		snprintf(buff, sizeof(buff), "dsbj,icnl9916 fw:%04x\n",cts_dev->fwdata.version);
	}
...
static void get_fw_ver_ic(void *device_data)
{
	...
	cts_tcs_get_fw_ver(cts_dev, &cts_dev->fwdata.version);
    if (g_lcm_name == 25) {
		snprintf(buff, sizeof(buff), "xinxian,icnl9916 fw:%04x\n",cts_dev->fwdata.version);
	}else {
		snprintf(buff, sizeof(buff), "dsbj,icnl9916 fw:%04x\n",cts_dev->fwdata.version);
	}
...
static void aot_enable(void *device_data)
{
    ...
    sec_cmd_set_cmd_result(sec, buff, strnlen(buff, sizeof(buff)));
    cts_info( "%s: %s,sec->cmd_state == %d", __func__, buff, sec->cmd_state);
    sec_cmd_set_cmd_exit(sec);
}
...
/* -S96818AA1-1936,daijun1.wt,modify,2023/08/24,icnl9916 add glove_mode and earjack_detect */
static ssize_t cts_support_feature(struct device *dev,
    struct device_attribute *attr, char *buf)
{
    u32 feature = 0;
...
static DEVICE_ATTR(support_feature, 0444, cts_support_feature, NULL);
...
//modify 7/4
#ifdef SEC_TSP_FACTORY_TEST
    ret = sec_cmd_init(&data->sec, cts_commands, ARRAY_SIZE(cts_commands), SEC_CLASS_DEVT_TSP);
    if (ret < 0) {
        cts_err("%s: Failed to sec_cmd_init\n", __func__);
        return -ENODEV;
    }
//+S96818AA1-9394,wangtao14.wt,modify,2023/08/08,gc7272 Add support_feature node
    ret = sysfs_create_group(&data->sec.fac_dev->kobj, &cts_cmd_attr_group);
    if (ret) {
        cts_err( "%s: failed to create sysfs attributes\n",__func__);
           goto out;
	}
#endif
    return 0;
