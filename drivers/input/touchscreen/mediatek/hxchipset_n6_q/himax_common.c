...
static ssize_t  ctp_open_proc_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	int val = 0x00;
	int len = count;
	int result = -1;
    ...
    *pos+=count;
	...
	if(g_f_0f_updat == 1){
		E("FW is upgrading, please wait a moment.\n");
		return 0;
	}
	if(private_ts->is_openshort_test_going||private_ts->suspended){
		E("last openshort test is going,or is suspended.\n");
		return 0;
	}

	private_ts->is_openshort_test_going = true;
	himax_int_enable(0);/* disable irq */
	private_ts->in_self_test = 1;
	val = g_core_fp.fp_chip_self_test();
#ifdef HX_ESD_RECOVERY
...
copy_to_user_error:
	...
	private_ts->is_openshort_test_going = false;
