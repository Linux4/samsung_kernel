...
static void himax_mcu_read_FW_ver(void)
{
	...
	snprintf(firmware_ver,HARDWARE_MAX_ITEM_LONGTH,"HX83112A,VND:Lide,FW:0x%02x", ic_data->vendor_touch_cfg_ver);
...
static int himax_mcu_fw_ver_bin(void)
{
	...
	if (i_CTPM_FW != NULL) {
		...
		g_i_FW_VER_BIN = (i_CTPM_FW[CFG_VER_MAJ_FLASH_ADDR]);
...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	if (g_f_0f_updat == 1) {
		release_firmware (fw_entry); //add by songbinbo.wt for Coverity Scan CID 79501  20190521
...
int himax_mcu_0f_operation_dirly(void)
{
	...
	if (g_f_0f_updat == 1) {
		release_firmware (fw_entry); //add by songbinbo.wt for Coverity Scan CID 79508  20190521
		I("%s:[Warning]Other thread is updating now!\n", __func__);
		err = -1;
		return err;
	} else {
...
//	mutex_lock(&(private_ts->fw_lock));
	g_core_fp.fp_firmware_update_0f (fw_entry);
//	mutex_unlock(&(private_ts->fw_lock));
	release_firmware (fw_entry);
...
void himax_mcu_0f_operation(struct work_struct *work)
{
	...
	if (g_f_0f_updat == 1) {
		release_firmware (fw_entry); //add by songbinbo.wt for Coverity Scan CID 79498  20190521
		I("%s:[Warning]Other thread is updating now!\n", __func__);
		return ;
	} else {
		I("%s:Entering Update Flow!\n", __func__);
