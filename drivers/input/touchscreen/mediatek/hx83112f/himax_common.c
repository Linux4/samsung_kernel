...
#include "Himax_83112f_FW_DSBJ.h"
#include "Himax_83112f_FW_DJN.h"
extern struct firmware *tmpfw_entry;
#endif
...
static int i_get_FW(void)
{
	...
	if (ret < 0) {
		...
		if(private_ts->firmware_headfile.data != NULL){
			tmpfw_entry->data = private_ts->firmware_headfile.data;
			tmpfw_entry->size = private_ts->firmware_headfile.size;
			hxfw = tmpfw_entry;
			I("%s: Not find FW in userspace, use i file",__func__);
			result = HX_EMBEDDED_FW;
		} else {
			I("%s: Not find h file",__func__);
...
#if defined(FW_GET_FROM_I_FILE)	
	if(g_lcm_name == 3) {
		ts->firmware_headfile.data = himax_bin_hx83112_jdi;
		ts->firmware_headfile.size = sizeof(himax_bin_hx83112_jdi);
	}else if(g_lcm_name == 4) {
		ts->firmware_headfile.data = himax_bin_hx83112_boe;
		ts->firmware_headfile.size = sizeof(himax_bin_hx83112_boe);
	}
#endif
...
#if !defined(HX_SWU_RESUME_SET_FW)
