...
#if defined(FW_GET_FROM_I_FILE)   
    tmpfw_entry = kzalloc(sizeof(struct firmware), GFP_KERNEL);
    if(tmpfw_entry == NULL) {
        I("%s kzalloc tmp_fw_entry is failed!\n", __func__);
		ret = -ENOMEM;
		goto err_spi_setup;
        }
#endif
...
{
	if (!strstr(saved_command_line,"hx83112f_fhdp_wt_dsi_vdo_cphy_90hz_djn")){
                if (!strstr(saved_command_line,"hx83112f_fhdp_wt_dsi_vdo_cphy_90hz_dsbj")){
                        return  -1;
                }
        }
