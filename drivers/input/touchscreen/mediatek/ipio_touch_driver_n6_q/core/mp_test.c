...
extern int ili_open_short_test_result;
...
int core_mp_start_test(bool lcm_on)
{
	...
	ipio_info("core_mp_start_test before lock\n"); //ritchie add
//	mutex_lock(&ipd->plat_mutex); //ritchie add
	mutex_lock(&ipd->touch_mutex);
	ipio_info("core_mp_start_test after lock\n"); //ritchie add

	ipd->isOpenShortTestGoing = true;
	...
	ret = core_parser_path(ipd->ini_name);
	if (ret < 0) {
	...
	if (protocol->major >= 5 && protocol->mid >= 4) {
		if (lcm_on) {
            ...
            if((tItems[35].item_result == MP_PASS) && (tItems[34].item_result == MP_PASS) && (tItems[24].item_result == MP_PASS) \
                && (tItems[45].item_result == MP_PASS) && (tItems[40].item_result == MP_PASS) && (tItems[3].item_result == MP_PASS) \
                && (tItems[0].item_result == MP_PASS) && (tItems[41].item_result == MP_PASS) && (tItems[42].item_result == MP_PASS) \
                && (tItems[46].item_result == MP_PASS) )
               ret = 1;
            else 
               ret = -1;
		} else {
            ...
            if((tItems[39].item_result == MP_PASS) && (tItems[38].item_result == MP_PASS) && (tItems[37].item_result == MP_PASS) \
                && (tItems[36].item_result == MP_PASS) && (tItems[43].item_result == MP_PASS) && (tItems[44].item_result == MP_PASS) )
               ret = 1;
            else 
               ret = -1;
		}
        ...
        if((tItems[29].item_result == MP_PASS) && (tItems[32].item_result == MP_PASS) && (tItems[33].item_result == MP_PASS) \
            && (tItems[26].item_result == MP_PASS) && (tItems[30].item_result == MP_PASS) && (tItems[31].item_result == MP_PASS) )
           ret = 1;
        else 
           ret = -1;
	}
...
//	mutex_unlock(&ipd->plat_mutex); //ritchie add
