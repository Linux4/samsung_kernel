...
#if GCORE_WDT_RECOVERY_ENABLE
static u8 wdt_valid_values[] = { 0xFF,0xFF,0x73,0x72,0x00,0x01,0xFF,0xFF };
#endif
...
#if 0
		for (i = 1; i <= 10; i++) {
			GTP_REPORT("demo %d:%x %x %x %x %x %x", i, coor_data[6*i-6], \
				coor_data[6*i-5], coor_data[6*i-4], coor_data[6*i-3], \
				coor_data[6*i-2], coor_data[6*i-1]);
		}
		GTP_REPORT("demo:%x %x %x %x", coor_data[60], coor_data[61], \
...
int gcore_probe_device(void)
{
	...
	u16 chip_type = 0x8503;
	u16 chip_lc_type = 0x8403;
#endif
	u8 buf[2] = { 0 };
	u16 id = 0;
	int retries = 0;
	...
	while (retries++ < 3) {
		...
		if (id == chip_type || id == chip_lc_type) {
