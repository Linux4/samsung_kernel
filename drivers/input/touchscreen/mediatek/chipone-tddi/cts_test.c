...
static int set_display_state(struct cts_device *cts_dev, bool active)
{
    ...
    if (ret) {
        cts_err("Read display access flag failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
    ...
    if (ret) {
        cts_err("Write display access flag %02x failed %d(%s)",
            access_flag, ret, cts_strerror(ret));
        return ret;
    }
    ...
    if (active) {
        ...
        if (ret) {
            cts_err("Write DCS-CMD11 failed %d(%s)",
                ret, cts_strerror(ret));
            return ret;
        }
        ...
        if (ret) {
            cts_err("Write DCS-CMD29 failed %d(%s)",
                ret, cts_strerror(ret));
            return ret;
        }
        ...
        if (ret) {
            cts_err("Write DCS-CMD28 failed %d(%s)",
                ret, cts_strerror(ret));
            return ret;
        }
        ...
        if (ret) {
            cts_err("Write DCS-CMD10 failed %d(%s)",
                ret, cts_strerror(ret));
            return ret;
        }
    ...
    if (ret) {
        cts_err("Restore display access flag %02x failed %d(%s)",
            access_flag, ret, cts_strerror(ret));
        return ret;
    }
    ...
    struct cts_test_param *param)
{
    ...
    if (ret) {
        cts_err("Read need display on register failed %d(%s)",
            ret, cts_strerror(ret));
        goto stop_dump_test_data_to_file;
    }
    ...
    if (need_display_on == 0) {
        ...
        if (ret) {
            cts_err("Set display state to SLEEP failed %d(%s)",
                ret, cts_strerror(ret));
            goto stop_dump_test_data_to_file;
        }
    ...
    if (need_display_on == 0) {
        ...
        ret = get_test_result(cts_dev, test_result);
        if (ret) {
            cts_err("Read skip test result failed %d(%s)",
            ret, cts_strerror(ret));
            goto recovery_display_state;
        }
...
recovery_display_state:
    if (recovery_display_state) {
        ...
        if (r) {
            cts_err("Set display state to ACTIVE failed %d(%s)",
                r, cts_strerror(r));
        }
    }
...
show_test_result:
    if (ret > 0) {
        cts_info("Short test has %d nodes FAIL, ELAPSED TIME: %lldms",
            ret, ktime_ms_delta(ktime_get(), start_time));
    } else if (ret < 0) {
