        ...
        char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    case CTS_TOOL_CMD_GET_PANEL_PARAM:
        ...
        if (ret) {
            cts_err("Get panel param len: %u failed %d(%s)",
                cmd->data_len, ret, cts_strerror(ret));
        }
        break;
    ...
    case CTS_TOOL_CMD_GET_DIFF_DATA:
        ...
        if (ret) {
            cts_err("Enable read raw/diff data failed %d(%s)",
        ...
        if (ret) {
            cts_err("Disable read raw/diff data failed %d(%s)",
                ret, cts_strerror(ret));
            break;
        }
    ...
    case CTS_TOOL_CMD_READ_HOSTCOMM:
        ...
        if (ret) {
            cts_err("Read firmware reg addr 0x%04x failed %d(%s)",
                get_unaligned_le16(cmd->addr), ret, cts_strerror(ret));
        } else {
    ...
    case CTS_TOOL_CMD_READ_GESTURE_INFO:
        ...
        if (ret) {
            cts_err("Get gesture info failed %d(%s)",
                ret, cts_strerror(ret));
        }
        break;
    ...
    case CTS_TOOL_CMD_READ_HOSTCOMM_MULTIBYTE:
        ...
        if (ret) {
            cts_err("Read firmware reg addr 0x%04x len %u failed %d(%s)",
    ...
    case CTS_TOOL_CMD_READ_PROGRAM_MODE_MULTIBYTE:
        ...
        if (ret) {
            cts_err("Read under program mode I2C xfer failed %d(%s)",
                ret, cts_strerror(ret));
            //break;
        }
    ...
    case CTS_TOOL_CMD_I2C_DIRECT_READ:
        {
            ...
            while (left_size) {
                ...
                if (ret) {
                    cts_err("Direct read i2c_addr 0x%02x addr 0x%06x len %zu failed %d(%s)",
                        cmd->addr[0], cts_tool_direct_access_addr,
                        xfer_size, ret, cts_strerror(ret));
                    break;
                }
        ...
        const char __user * buffer, size_t count, loff_t * ppos)
{
    ...
    if (ret) {
        cts_err("Copy command header from user buffer failed %d(%s)",
            ret, cts_strerror(ret));
        return -EIO;
    } else {
    ...
    if(cmd->cmd & BIT(0)) {
        if(cmd->data_len) {
            ...
            if (ret) {
                cts_err("Copy command payload from user buffer len %u failed %d(%s)",
                    cmd->data_len, ret, cts_strerror(ret));
                return -EIO;
            }
        }
    ...
    case CTS_TOOL_CMD_UPDATE_PANEL_PARAM_IN_SRAM:
        ...
        ret = cts_fw_reg_writesb(cts_dev, CTS_DEVICE_FW_REG_PANEL_PARAM,
                cmd->data, cmd->data_len);
        if (ret) {
            cts_err("Write panel param failed %d(%s)",
                ret, cts_strerror(ret));
            break;
        }
    ...
    case CTS_TOOL_CMD_DOWNLOAD_FIRMWARE:
        ...
        ret = cts_update_firmware_from_file(cts_dev, cts_tool_firmware_filepath, false);
        if (ret) {
            cts_err("Updata firmware from file failed %d(%s)",
                ret, cts_strerror(ret));
            //break;
        }
    ...
    case CTS_TOOL_CMD_WRITE_HOSTCOMM:
        ...
        ret = cts_fw_reg_writeb(cts_dev,
                get_unaligned_le16(cmd->addr), cmd->data[0]);
        if (ret) {
            cts_err("Write firmware reg addr: 0x%04x val=0x%02x failed %d(%s)",
                get_unaligned_le16(cmd->addr), cmd->data[0],
                ret, cts_strerror(ret));
        }
        break;
    ...
    case CTS_TOOL_CMD_WRITE_HOSTCOMM_MULTIBYTE:
        ...
        ret = cts_fw_reg_writesb(cts_dev, get_unaligned_le16(cmd->addr),
                cmd->data, cmd->data_len);
        if (ret) {
            cts_err("Write firmare reg addr: 0x%04x len %u failed %d(%s)",
                get_unaligned_le16(cmd->addr), cmd->data_len,
                ret, cts_strerror(ret));
        }
        break;
    ...
    case CTS_TOOL_CMD_WRITE_PROGRAM_MODE_MULTIBYTE:
        ...
        ret = cts_sram_writesb(cts_dev,
                ...
                cmd->data, cmd->data_len);
        if (ret) {
            cts_err("Write program mode multibyte failed %d(%s)",
                ret, cts_strerror(ret));
            //break;
        }
    ...
    case CTS_TOOL_CMD_I2C_DIRECT_WRITE:
        {
            ...
            do {
                ...
                if (ret) {
                    cts_err("Direct write i2c_addr 0x%02x addr 0x%06x len %zu failed %d(%s)",
                        cmd->addr[0], cts_tool_direct_access_addr,
                        xfer_len, ret, cts_strerror(ret));
                    break;
                }
    ...
    u32 ntests, struct cts_test_param *tests)
{
    ...
    for (i = 0; i < ntests; i++) {
        ...
        if (validate_data) {
            ...
            if (validate_min) {
                ...
                if (IS_ERR(tests[i].min)) {
                    ...
                    tests[i].min = NULL;
                    cts_err("Memdup min threshold from user failed %d(%s)",
                        ret, cts_strerror(ret));
                    goto store_result;
                }
            } else {
            ...
            if (validate_max) {
                ...
                if (IS_ERR(tests[i].max)) {
                    ...
                    tests[i].max = NULL;
                    cts_err("Memdup max threshold from user failed %d(%s)",
                        ret, cts_strerror(ret));
                    goto store_result;
                }
            } else {
            ...
            if (skip_invalid_node) {
                ...
                if (IS_ERR(tests[i].invalid_nodes)) {
                    ...
                    cts_err("Memdup invalid node from user failed %d(%s)",
                        ret, cts_strerror(ret));
                    goto store_result;
                }
            }
        ...
        if (driver_log_to_file || driver_log_to_user) {
            ...
            if (ret) {
                cts_err("Start driver log redirect failed %d(%s)",
                    ret, cts_strerror(ret));
                goto store_result;
            }
        }
        ...
        if (tests[i].priv_param_size && tests[i].priv_param) {
            ...
            if (IS_ERR(tests[i].priv_param)) {
                ...
                cts_err("Memdup priv param from user failed %d(%s)",
                    ret, cts_strerror(ret));
                goto store_result;
            }
        }
    ...
    u8 reg_type, u32 nregs, struct cts_rdwr_reg *regs)
{
    ...
    if (reg_type == CTS_IOCTL_RDWR_REG_TYPE_DDI) {
        ...
        if (ret) {
            cts_err("Get fw esd protection failed %d(%s)",
                ret, cts_strerror(ret));
            goto unlock_device;
        }
        ...
        if (fw_esd_protect) {
            ...
            if (ret) {
                cts_err("Set fw esd protection failed %d(%s)",
                    ret, cts_strerror(ret));
                goto unlock_device;
            }
        }
        ...
        if (ret) {
            cts_err("Enable access ddi reg failed %d(%s)",
                ret, cts_strerror(ret));
    ...
    for (i = 0; i < nregs; i++) {
        ...
        if (reg->flags & CTS_IOCTL_RDWR_REG_FLAG_RD) {
            ...
            if (ret) {
                kfree(data);
                cts_err("Read reg from addr: 0x%06x len: %u failed %d(%s)",
                    reg->addr, reg->len, ret, cts_strerror(ret));
                goto disable_access_ddi_reg;
            }
            ...
            kfree(data);
            if (ret) {
                cts_err("Write reg from addr 0x%06x len %u failed %d(%s)",
                    reg->addr, reg->len, ret, cts_strerror(ret));
                goto disable_access_ddi_reg;
            }
        }
...
disable_access_ddi_reg:
    if (reg_type == CTS_IOCTL_RDWR_REG_TYPE_DDI) {
        ...
        if (r) {
            cts_err("Disable access ddi reg failed %d(%s)",
                r, cts_strerror(r));
        }
    }
...
recovery_fw_esd_protect:
    if (reg_type == CTS_IOCTL_RDWR_REG_TYPE_DDI && fw_esd_protect) {
        ...
        if (r) {
            cts_err("Re-Enable fw esd protection failed %d(%s)",
                r, cts_strerror(r));
        }
    }
    ...
    struct cts_upgrade_fw_ioctl_data *ioctl_data)
{
    ...
    if (!!(ioctl_data->flags & CTS_IOCTL_UPGRADE_FW_FLAG_FILE)) {
        ...
        if (IS_ERR(filepath)) {
            ret = PTR_ERR(filepath);
            cts_err("Memdump filepath to kernel failed %d(%s)",
                ret, cts_strerror(ret));
            return ret;
        }
        ...
        if (ret) {
            cts_err("Upgrade fw from file '%s' failed %d(%s)",
    ...
    if (!!(ioctl_data->flags & CTS_IOCTL_UPGRADE_FW_FLAG_FW_DATA)) {
        ...
        if (IS_ERR(firmware.data)) {
            ret = PTR_ERR(firmware.data);
            cts_err("Memdump fw data to kernel failed %d(%s)",
                ret, cts_strerror(ret));
            return ret;
        }
        ...
        if (ret) {
            cts_err("Upgrade firmware data failed %d(%s)",
                ret, cts_strerror(ret));
        }
        kfree(firmware.data);
        ...
        unsigned long arg)
{
    ...
    case CTS_TOOL_IOCTL_TEST:{
        ...
        if (IS_ERR(tests_pa)) {
            ...
            cts_err("Memdump test param to kernel failed %d(%s)",
                ret, cts_strerror(ret));
            return ret;
        }
    ...
    case CTS_TOOL_IOCTL_RDWR_REG:{
        ...
        if (IS_ERR(regs_pa)) {
            ...
            cts_err("Memdump cts_rdwr_reg to kernel failed %d(%s)",
                ret, cts_strerror(ret));
            return ret;
        }
