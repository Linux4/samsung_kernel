       ...
       const struct cts_device *cts_dev, const char *filepath, u16 curr_version)
{
    ...
    if (firmware == NULL) {
        cts_err("Request from file alloc cts_firmware failed");
        return NULL;
    }
	...
	if (strchr(filepath, '/') != NULL) {
		...
		if (!is_firmware_size_valid(firmware)) {
			cts_info("File '%s' size: %zu invalid",
				filepath, firmware->size);
			goto err_close_file;
		}
    ...
    read_size = kernel_read(file, pos, firmware->data, firmware->size);
		...
		if (read_size < 0 || read_size != firmware->size) {
			cts_err("Request from fs read whole file failed %d(%s)",
		...
		ret = request_firmware(&firmware->fw, filepath, &cts_dev->pdata->ts_input_dev->dev);
		if (ret) {
...
const struct cts_firmware *cts_request_firmware_from_fs(const struct cts_device *cts_dev,const char *filepath)
{
        ...
        const char *filepath, bool to_flash)
{
    ...
    if (ret) {
        cts_err("Update to %s from file failed %d(%s)",
            to_flash ? "flash" : "sram", ret, cts_strerror(ret));
         ...
         u8 *buf, bool calc_crc, u32 crc)
{
    ...
    if (calc_crc) {
        ...
        if (ret) {
            cts_err("Calc data in flash from 0x%06x size %zu crc "
                    "failed %d(%s), try to direct readback",
                flash_addr, size, ret, cts_strerror(ret));
            // FALL through by direct compare data read back
        } else {
            if (crc_flash != crc) {
                cts_err("Crc data in flash from 0x%06x size %zu crc "
    ...
    if (ret) {
        cts_err("Read flash from 0x%06x size %zu failed %d(%s)",
            flash_addr, size, ret, cts_strerror(ret));
        goto err_free_buf;
    }
        ...
        const struct cts_firmware_sect_info *firmware_info)
{
    ...
    if (ret) {
        cts_err("Program firmware section from sram failed %d(%s)",
    ...
    if (ret) {
        cts_err("Program firmware crc section failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
    ...
    if (ret) {
        cts_err("Validate Firmware-CRC section failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
    ...
    if (ret) {
        cts_err("Program firmware section fist 4bytes failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
    ...
    if (ret) {
        cts_err("Validate firmware section failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
        ...
        const struct cts_firmware_sect_info *firmware_info)
{
    ...
    ret = cts_program_flash(cts_dev, CTS_DDIPARAM_SECTION_OFFSET,
        ...
        firmware_info->ddiparam_sect_size);
    if (ret) {
        cts_err("Program DDIParam section failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
    ...
    if (ret) {
        cts_err("Validate DDIParam section failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
    ...
    if (ret) {
        cts_err("Program DDIParam-CRC section failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
    ...
    if (ret) {
        cts_err("Validate DDIParam-CRC section failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
        ...
        const struct cts_firmware *firmware, bool to_flash)
{
    ...
    if (ret) {
        cts_err("Device enter program mode failed %d(%s)",
            ret, cts_strerror(ret));
        goto out;
    }
    ...
    if (ret) {
        cts_err("Write firmware to sram failed %d(%s)",
            ret, cts_strerror(ret));
    }

    if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x015FF0,
            0xCC33555A, 3, 1)) != 0) {
        cts_err("Write VALID flag to 0x015FF0 failed %d(%s)",
            ret, cts_strerror(ret));
        goto out;
    }
    if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x015FF8,
            firmware_info.firmware_sect_crc, 3, 1)) != 0) {
        cts_err("Write CRC to 0x015FF8 failed %d(%s)",
            ret, cts_strerror(ret));
        goto out;
    }
    if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x015FFC,
            firmware_info.firmware_sect_size, 3, 1)) != 0) {
        cts_err("Write LEN to 0x015FFC failed %d(%s)",
            ret, cts_strerror(ret));
        goto out;
    }
    ...
    if (ret) {
        cts_warn("Prepare flash operation failed %d(%s)",
            ret, cts_strerror(ret));
        // Go through and try
    }
    ...
    do {
        ...
        ret = cts_erase_flash(cts_dev, CTS_FIRMWARE_CRC_SECTION_OFFSET,
            firmware_info.firmware_crc_sect_size);
        if (ret) {
            cts_err("Erase firmware crc section failed %d(%s) retries %d",
                ret, cts_strerror(ret), retries);
            continue;
        }

        ret = cts_erase_flash(cts_dev,
            0, firmware_info.firmware_sect_size);
        if (ret) {
            cts_err("Erase firmware section failed %d(%s) retries %d",
                ret, cts_strerror(ret), retries);
            continue;
        }
        ...
        if (ret) {
            cts_err("Program firmware & crc section failed %d(%s) retries %d",
                ret, cts_strerror(ret), retries);
        }
    } while (ret && retries < 3);
    ...
    if (ret == 0 && firmware_info.ddiparam_sect_size != 0) {
        ...
        do {
            ...
            ret = cts_erase_flash(cts_dev, CTS_DDIPARAM_CRC_SECTION_OFFSET,
                    firmware_info.ddiparam_crc_sect_size);
            if (ret) {
                cts_err("Erase DDIParam crc secction failed %d(%s), retries %d",
                    ret, cts_strerror(ret), retries);
                continue;
            }
            ...
            ret = cts_erase_flash(cts_dev, CTS_DDIPARAM_SECTION_OFFSET,
                    firmware_info.ddiparam_sect_size);
            if (ret) {
                cts_err("Erase DDIParam section failed %d(%s), retries %d",
                    ret, cts_strerror(ret), retries);
                continue;
            }
            ...
            if (ret) {
                cts_err("Program DDIParam & crc section failed %d(%s) retries %d",
                    ret, cts_strerror(ret), retries);
            }
        } while (ret && retries < 3);
    }
