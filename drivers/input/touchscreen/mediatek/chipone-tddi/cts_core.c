...
static int icnl9911_set_access_ddi_reg(struct cts_device *cts_dev, bool enable)
{
    ...
    ret = cts_hw_reg_readb(cts_dev,
        CTS_DEV_HW_REG_DDI_REG_CTRL, &access_flag);
    if (ret) {
        cts_err("Read HW_REG_DDI_REG_CTRL failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }

    access_flag = enable ? (access_flag | 0x01) : (access_flag & (~0x01));
    ret = cts_hw_reg_writeb(cts_dev,
        CTS_DEV_HW_REG_DDI_REG_CTRL, access_flag);
    if (ret) {
        cts_err("Write HW_REG_DDI_REG_CTRL %02x failed %d(%s)",
    ...
    if (ret) {
        cts_err("Write password failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
...
static int icnl9911s_set_access_ddi_reg(struct cts_device *cts_dev, bool enable)
{
    ...
    if (ret) {
        cts_err("Write 0x30074 failed %d(%s)",
            access_flag, ret, cts_strerror(ret));
        return ret;
    }
    ...
    if (ret) {
        cts_err("Write password to F0 failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
    ret = cts_hw_reg_writew(cts_dev, 0x3DFF4, enable ? 0xA6A5 : 0x5A5A);
    if (ret) {
        cts_err("Write password to F1 failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
