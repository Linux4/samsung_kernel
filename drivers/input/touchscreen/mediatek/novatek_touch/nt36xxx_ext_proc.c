...
void nvt_get_mdata(int32_t *, uint8_t *, uint8_t *);
void nvt_read_get_num_mdata(uint32_t , int32_t *, uint32_t );
...
static int32_t nvt_pen_diff_open(struct inode *inode, struct file *file)
{
	...
	nvt_read_get_num_mdata(ts->mmap->PEN_1D_DIFF_TIP_X_ADDR, xdata_pen_tip_x, ts->platdata->x_num);
	nvt_read_get_num_mdata(ts->mmap->PEN_1D_DIFF_TIP_Y_ADDR, xdata_pen_tip_y, ts->platdata->y_num);
	nvt_read_get_num_mdata(ts->mmap->PEN_1D_DIFF_RING_X_ADDR, xdata_pen_ring_x, ts->platdata->x_num);
	nvt_read_get_num_mdata(ts->mmap->PEN_1D_DIFF_RING_Y_ADDR, xdata_pen_ring_y, ts->platdata->y_num);
...
int32_t nvt_extra_proc_init(void)
{
	...
	if (ts->pen_support) {
		...
		if (NVT_proc_pen_diff_entry == NULL) {
			input_err(true, &ts->client->dev, "create proc/%s Failed!\n", NVT_PEN_DIFF);
			return -ENOMEM;
		} else {
			input_info(true, &ts->client->dev, "create proc/%s Succeeded!\n", NVT_PEN_DIFF);
		}
	}
...
void nvt_extra_proc_deinit(void)
{
	...
	if (ts->pen_support) {
		if (NVT_proc_pen_diff_entry != NULL) {
			...
			input_info(true, &ts->client->dev, "Removed /proc/%s\n", NVT_PEN_DIFF);
		}
	}
}
#endif	// end of #if NVT_TOUCH_EXT_PROC
