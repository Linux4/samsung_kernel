...
static struct nvt_ito_data g_ito_data[8];
static int32_t g_ito_item_index = 0;
...
static int nvt_mp_buffer_init(void)
{
	...
	size_t Pen_RecordResult_BufSize = max(ts->platdata->x_num * ts->y_gang_num, ts->x_gang_num * ts->platdata->y_num);
	size_t Pen_RawData_BufSize = max(ts->platdata->x_num * ts->y_gang_num, ts->x_gang_num * ts->platdata->y_num) * sizeof(int32_t);
...
static void nvt_print_criteria(void)
{
	...
	if (ts->pen_support) {
		...
		nvt_print_lmt_array(PS_Config_Lmt_PenTipX_FW_Raw_P, ts->platdata->x_num, ts->y_gang_num);
		printk("PS_Config_Lmt_PenTipX_FW_Raw_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenTipX_FW_Raw_N, ts->platdata->x_num, ts->y_gang_num);

		//---PS_Config_Lmt_PenTipY_FW_Raw---
		printk("PS_Config_Lmt_PenTipY_FW_Raw_P:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenTipY_FW_Raw_P, ts->x_gang_num, ts->platdata->y_num);
		printk("PS_Config_Lmt_PenTipY_FW_Raw_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenTipY_FW_Raw_N, ts->x_gang_num, ts->platdata->y_num);

		//---PS_Config_Lmt_PenRingX_FW_Raw---
		printk("PS_Config_Lmt_PenRingX_FW_Raw_P:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenRingX_FW_Raw_P, ts->platdata->x_num, ts->y_gang_num);
		printk("PS_Config_Lmt_PenRingX_FW_Raw_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenRingX_FW_Raw_N, ts->platdata->x_num, ts->y_gang_num);

		//---PS_Config_Lmt_PenRingY_FW_Raw---
		printk("PS_Config_Lmt_PenRingY_FW_Raw_P:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenRingY_FW_Raw_P, ts->x_gang_num, ts->platdata->y_num);
		printk("PS_Config_Lmt_PenRingY_FW_Raw_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenRingY_FW_Raw_N, ts->x_gang_num, ts->platdata->y_num);

		//---PS_Config_Lmt_PenTipX_FW_Diff---
		printk("PS_Config_Lmt_PenTipX_FW_Diff_P:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenTipX_FW_Diff_P, ts->platdata->x_num, ts->y_gang_num);
		printk("PS_Config_Lmt_PenTipX_FW_Diff_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenTipX_FW_Diff_N, ts->platdata->x_num, ts->y_gang_num);

		//---PS_Config_Lmt_PenTipY_FW_Diff---
		printk("PS_Config_Lmt_PenTipY_FW_Diff_P:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenTipY_FW_Diff_P, ts->x_gang_num, ts->platdata->y_num);
		printk("PS_Config_Lmt_PenTipY_FW_Diff_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenTipY_FW_Diff_N, ts->x_gang_num, ts->platdata->y_num);

		//---PS_Config_Lmt_PenRingX_FW_Diff---
		printk("PS_Config_Lmt_PenRingX_FW_Diff_P:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenRingX_FW_Diff_P, ts->platdata->x_num, ts->y_gang_num);
		printk("PS_Config_Lmt_PenRingX_FW_Diff_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenRingX_FW_Diff_N, ts->platdata->x_num, ts->y_gang_num);

		//---PS_Config_Lmt_PenRingY_FW_Diff---
		printk("PS_Config_Lmt_PenRingY_FW_Diff_P:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenRingY_FW_Diff_P, ts->x_gang_num, ts->platdata->y_num);
		printk("PS_Config_Lmt_PenRingY_FW_Diff_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_PenRingY_FW_Diff_N, ts->x_gang_num, ts->platdata->y_num);
	} /* if (ts->pen_support) */
...
#endif /* #if TOUCH_KEY_NUM > 0 */
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 start*/
	if(g_ito_data[g_ito_item_index].ito_data == NULL) {
		g_ito_data[g_ito_item_index].ito_data = (char *)kzalloc(output_len, GFP_KERNEL);
		if (!g_ito_data[g_ito_item_index].ito_data) {
			input_err(true, &ts->client->dev, "%s: kzalloc for fbufp failed!\n", __func__);
			return -ENOMEM;
		}
	}
	memset(g_ito_data[g_ito_item_index].ito_data, 0, output_len);
	memcpy(g_ito_data[g_ito_item_index].ito_data, fbufp, output_len);
	g_ito_data[g_ito_item_index].data_len = output_len;
	g_ito_data[g_ito_item_index].offset = offset;
	sprintf(g_ito_data[g_ito_item_index].filepath, "%s_%s.csv", file_path, get_date_time_str());
	//g_ito_data[g_ito_item_index].filepath = file_path;
	g_ito_item_index = g_ito_item_index + 1;
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 end*/
	if (fbufp) {
		kfree(fbufp);
		fbufp = NULL;
...
static int32_t nvt_read_pen_baseline(void)
{
	...
	nvt_read_get_num_mdata(ts->mmap->PEN_2D_BL_TIP_X_ADDR, RawData_PenTipX_Raw, ts->platdata->x_num * ts->y_gang_num);
	nvt_read_get_num_mdata(ts->mmap->PEN_2D_BL_TIP_Y_ADDR, RawData_PenTipY_Raw, ts->x_gang_num * ts->platdata->y_num);
	nvt_read_get_num_mdata(ts->mmap->PEN_2D_BL_RING_X_ADDR, RawData_PenRingX_Raw, ts->platdata->x_num * ts->y_gang_num);
	nvt_read_get_num_mdata(ts->mmap->PEN_2D_BL_RING_Y_ADDR, RawData_PenRingY_Raw, ts->x_gang_num * ts->platdata->y_num);

	// Save Rawdata to CSV file
	printk("%s:RawData_PenTipX_Raw\n", __func__);
	if (nvt_save_rawdata_to_csv(RawData_PenTipX_Raw, ts->platdata->x_num, ts->y_gang_num, PEN_FW_RAW_TEST_CSV_FILE, csv_output_offset) < 0) {
		input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	csv_output_offset += ts->y_gang_num * ts->platdata->x_num * 7 + ts->y_gang_num * 2;
	printk("%s:RawData_PenTipY_Raw\n", __func__);
	if (nvt_save_rawdata_to_csv(RawData_PenTipY_Raw, ts->x_gang_num, ts->platdata->y_num, PEN_FW_RAW_TEST_CSV_FILE, csv_output_offset) < 0) {
		input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	csv_output_offset += ts->platdata->y_num * ts->x_gang_num * 7 + ts->platdata->y_num * 2;
	printk("%s:RawData_PenRingX_Raw\n", __func__);
	if (nvt_save_rawdata_to_csv(RawData_PenRingX_Raw, ts->platdata->x_num, ts->y_gang_num, PEN_FW_RAW_TEST_CSV_FILE, csv_output_offset) < 0) {
		input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	csv_output_offset += ts->y_gang_num * ts->platdata->x_num * 7 + ts->y_gang_num * 2;
	printk("%s:RawData_PenRingY_Raw\n", __func__);
	if (nvt_save_rawdata_to_csv(RawData_PenRingY_Raw, ts->x_gang_num, ts->platdata->y_num, PEN_FW_RAW_TEST_CSV_FILE, csv_output_offset) < 0) {
		input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
...
static int32_t nvt_read_fw_noise(int32_t *xdata)
{
	...
	if (ts->pen_support) {
		// get pen noise data
		nvt_read_get_num_mdata(ts->mmap->PEN_2D_DIFF_TIP_X_ADDR, RawData_PenTipX_DiffMax, ts->platdata->x_num * ts->y_gang_num);
		nvt_read_get_num_mdata(ts->mmap->PEN_2D_RAW_TIP_X_ADDR, RawData_PenTipX_DiffMin, ts->platdata->x_num * ts->y_gang_num);
		nvt_read_get_num_mdata(ts->mmap->PEN_2D_DIFF_TIP_Y_ADDR, RawData_PenTipY_DiffMax, ts->x_gang_num * ts->platdata->y_num);
		nvt_read_get_num_mdata(ts->mmap->PEN_2D_RAW_TIP_Y_ADDR, RawData_PenTipY_DiffMin, ts->x_gang_num * ts->platdata->y_num);
		nvt_read_get_num_mdata(ts->mmap->PEN_2D_DIFF_RING_X_ADDR, RawData_PenRingX_DiffMax, ts->platdata->x_num * ts->y_gang_num);
		nvt_read_get_num_mdata(ts->mmap->PEN_2D_RAW_RING_X_ADDR, RawData_PenRingX_DiffMin, ts->platdata->x_num * ts->y_gang_num);
		nvt_read_get_num_mdata(ts->mmap->PEN_2D_DIFF_RING_Y_ADDR, RawData_PenRingY_DiffMax, ts->x_gang_num * ts->platdata->y_num);
		nvt_read_get_num_mdata(ts->mmap->PEN_2D_RAW_RING_Y_ADDR, RawData_PenRingY_DiffMin, ts->x_gang_num * ts->platdata->y_num);
	} /* if (ts->pen_support) */
	...
	if (ts->pen_support) {
		...
		if (nvt_save_rawdata_to_csv(RawData_PenTipX_DiffMax, ts->platdata->x_num, ts->y_gang_num, PEN_NOISE_TEST_CSV_FILE, csv_pen_noise_offset) < 0) {
			input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_gang_num * ts->platdata->x_num * 7 + ts->y_gang_num * 2;
		printk("%s:RawData_PenTipX_DiffMin:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenTipX_DiffMin, ts->platdata->x_num, ts->y_gang_num, PEN_NOISE_TEST_CSV_FILE, csv_pen_noise_offset) < 0) {
			input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_gang_num * ts->platdata->x_num * 7 + ts->y_gang_num * 2;
		printk("%s:RawData_PenTipY_DiffMax:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenTipY_DiffMax, ts->x_gang_num, ts->platdata->y_num, PEN_NOISE_TEST_CSV_FILE, csv_pen_noise_offset) < 0) {
			input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->platdata->y_num * ts->x_gang_num * 7 + ts->platdata->y_num * 2;
		printk("%s:RawData_PenTipY_DiffMin:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenTipY_DiffMin, ts->x_gang_num, ts->platdata->y_num, PEN_NOISE_TEST_CSV_FILE, csv_pen_noise_offset) < 0) {
			input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->platdata->y_num * ts->x_gang_num * 7 + ts->platdata->y_num * 2;
		printk("%s:RawData_PenRingX_DiffMax:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenRingX_DiffMax, ts->platdata->x_num, ts->y_gang_num, PEN_NOISE_TEST_CSV_FILE, csv_pen_noise_offset) < 0) {
			input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_gang_num * ts->platdata->x_num * 7 + ts->y_gang_num * 2;
		printk("%s:RawData_PenRingX_DiffMin:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenRingX_DiffMin, ts->platdata->x_num, ts->y_gang_num, PEN_NOISE_TEST_CSV_FILE, csv_pen_noise_offset) < 0) {
			input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_gang_num * ts->platdata->x_num * 7 + ts->y_gang_num * 2;
		printk("%s:RawData_PenRingY_DiffMax:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenRingY_DiffMax, ts->x_gang_num, ts->platdata->y_num, PEN_NOISE_TEST_CSV_FILE, csv_pen_noise_offset) < 0) {
			input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->platdata->y_num * ts->x_gang_num * 7 + ts->platdata->y_num * 2;
		printk("%s:RawData_PenRingY_DiffMin:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenRingY_DiffMin, ts->x_gang_num, ts->platdata->y_num, PEN_NOISE_TEST_CSV_FILE, csv_pen_noise_offset) < 0) {
			input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
	} /* if (ts->pen_support) */
...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	if (ts->pen_support) {
		...
		if ((TestResult_Pen_FW_Raw == 0) || (TestResult_Pen_FW_Raw == 1)) {
			print_selftest_result(m, TestResult_Pen_FW_Raw, RecordResult_PenTipX_Raw, RawData_PenTipX_Raw, ts->platdata->x_num, ts->y_gang_num);
		} else { // TestResult_Pen_FW_Raw is -1
			nvt_mp_seq_printf(m, " FAIL!\n");
			if (TestResult_PenTipX_Raw == -1) {
				nvt_mp_seq_printf(m, "Pen Tip X Raw");
				print_selftest_result(m, TestResult_PenTipX_Raw, RecordResult_PenTipX_Raw, RawData_PenTipX_Raw, ts->platdata->x_num, ts->y_gang_num);
			}
			if (TestResult_PenTipY_Raw == -1) {
				nvt_mp_seq_printf(m, "Pen Tip Y Raw");
				print_selftest_result(m, TestResult_PenTipY_Raw, RecordResult_PenTipY_Raw, RawData_PenTipY_Raw, ts->x_gang_num, ts->platdata->y_num);
			}
			if (TestResult_PenRingX_Raw == -1) {
				nvt_mp_seq_printf(m, "Pen Ring X Raw");
				print_selftest_result(m, TestResult_PenRingX_Raw, RecordResult_PenRingX_Raw, RawData_PenRingX_Raw, ts->platdata->x_num, ts->y_gang_num);
			}
			if (TestResult_PenRingY_Raw == -1) {
				nvt_mp_seq_printf(m, "Pen Ring Y Raw");
				print_selftest_result(m, TestResult_PenRingY_Raw, RecordResult_PenRingY_Raw, RawData_PenRingY_Raw, ts->x_gang_num, ts->platdata->y_num);
			}
		}

		nvt_mp_seq_printf(m, "Pen Noise Test");
		if ((TestResult_Pen_Noise == 0) || (TestResult_Pen_Noise == 1)) {
			print_selftest_result(m, TestResult_Pen_Noise, RecordResult_PenTipX_DiffMax, RawData_PenTipX_DiffMax, ts->platdata->x_num, ts->y_gang_num);
		} else { // TestResult_Pen_Noise is -1
			nvt_mp_seq_printf(m, " FAIL!\n");
			if (TestResult_PenTipX_DiffMax == -1) {
				nvt_mp_seq_printf(m, "Pen Tip X Diff Max");
				print_selftest_result(m, TestResult_PenTipX_DiffMax, RecordResult_PenTipX_DiffMax, RawData_PenTipX_DiffMax, ts->platdata->x_num, ts->y_gang_num);
			}
			if (TestResult_PenTipX_DiffMin == -1) {
				nvt_mp_seq_printf(m, "Pen Tip X Diff Min");
				print_selftest_result(m, TestResult_PenTipX_DiffMin, RecordResult_PenTipX_DiffMin, RawData_PenTipX_DiffMin, ts->platdata->x_num, ts->y_gang_num);
			}
			if (TestResult_PenTipY_DiffMax == -1) {
				nvt_mp_seq_printf(m, "Pen Tip Y Diff Max");
				print_selftest_result(m, TestResult_PenTipY_DiffMax, RecordResult_PenTipY_DiffMax, RawData_PenTipY_DiffMax, ts->x_gang_num, ts->platdata->y_num);
			}
			if (TestResult_PenTipY_DiffMin == -1) {
				nvt_mp_seq_printf(m, "Pen Tip Y Diff Min");
				print_selftest_result(m, TestResult_PenTipY_DiffMin, RecordResult_PenTipY_DiffMin, RawData_PenTipY_DiffMin, ts->x_gang_num, ts->platdata->y_num);
			}
			if (TestResult_PenRingX_DiffMax == -1) {
				nvt_mp_seq_printf(m, "Pen Ring X Diff Max");
				print_selftest_result(m, TestResult_PenRingX_DiffMax, RecordResult_PenRingX_DiffMax, RawData_PenRingX_DiffMax, ts->platdata->x_num, ts->y_gang_num);
			}
			if (TestResult_PenRingX_DiffMin == -1) {
				nvt_mp_seq_printf(m, "Pen Ring X Diff Min");
				print_selftest_result(m, TestResult_PenRingX_DiffMin, RecordResult_PenRingX_DiffMin, RawData_PenRingX_DiffMin, ts->platdata->x_num, ts->y_gang_num);
			}
			if (TestResult_PenRingY_DiffMax == -1) {
				nvt_mp_seq_printf(m, "Pen Ring Y Diff Max");
				print_selftest_result(m, TestResult_PenRingY_DiffMax, RecordResult_PenRingY_DiffMax, RawData_PenRingY_DiffMax, ts->x_gang_num, ts->platdata->y_num);
			}
			if (TestResult_PenRingY_DiffMin == -1) {
				nvt_mp_seq_printf(m, "Pen Ring Y Diff Min");
				print_selftest_result(m, TestResult_PenRingY_DiffMin, RecordResult_PenRingY_DiffMin, RawData_PenRingY_DiffMin, ts->x_gang_num, ts->platdata->y_num);
			}
		}
	} /* if (ts->pen_support) */
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	if (ts->pen_support) {
		...
		if (nvt_read_pen_baseline() != 0) {
			...
			TestResult_PenTipX_Raw = RawDataTest_SinglePoint_Sub(RawData_PenTipX_Raw, RecordResult_PenTipX_Raw, ts->platdata->x_num, ts->y_gang_num,
											PS_Config_Lmt_PenTipX_FW_Raw_P, PS_Config_Lmt_PenTipX_FW_Raw_N);
			TestResult_PenTipY_Raw = RawDataTest_SinglePoint_Sub(RawData_PenTipY_Raw, RecordResult_PenTipY_Raw, ts->x_gang_num, ts->platdata->y_num,
											PS_Config_Lmt_PenTipY_FW_Raw_P, PS_Config_Lmt_PenTipY_FW_Raw_N);
			TestResult_PenRingX_Raw = RawDataTest_SinglePoint_Sub(RawData_PenRingX_Raw, RecordResult_PenRingX_Raw, ts->platdata->x_num, ts->y_gang_num,
											PS_Config_Lmt_PenRingX_FW_Raw_P, PS_Config_Lmt_PenRingX_FW_Raw_N);
			TestResult_PenRingY_Raw = RawDataTest_SinglePoint_Sub(RawData_PenRingY_Raw, RecordResult_PenRingY_Raw, ts->x_gang_num, ts->platdata->y_num,
	...
	if (nvt_read_fw_noise(RawData_Diff) != 0) {
		...
		if (ts->pen_support) {
			TestResult_PenTipX_DiffMax = RawDataTest_SinglePoint_Sub(RawData_PenTipX_DiffMax, RecordResult_PenTipX_DiffMax, ts->platdata->x_num, ts->y_gang_num,
											PS_Config_Lmt_PenTipX_FW_Diff_P, PS_Config_Lmt_PenTipX_FW_Diff_N);

			TestResult_PenTipX_DiffMin = RawDataTest_SinglePoint_Sub(RawData_PenTipX_DiffMin, RecordResult_PenTipX_DiffMin, ts->platdata->x_num, ts->y_gang_num,
											PS_Config_Lmt_PenTipX_FW_Diff_P, PS_Config_Lmt_PenTipX_FW_Diff_N);

			TestResult_PenTipY_DiffMax = RawDataTest_SinglePoint_Sub(RawData_PenTipY_DiffMax, RecordResult_PenTipY_DiffMax, ts->x_gang_num, ts->platdata->y_num,
											PS_Config_Lmt_PenTipY_FW_Diff_P, PS_Config_Lmt_PenTipY_FW_Diff_N);

			TestResult_PenTipY_DiffMin = RawDataTest_SinglePoint_Sub(RawData_PenTipY_DiffMin, RecordResult_PenTipY_DiffMin, ts->x_gang_num, ts->platdata->y_num,
											PS_Config_Lmt_PenTipY_FW_Diff_P, PS_Config_Lmt_PenTipY_FW_Diff_N);

			TestResult_PenRingX_DiffMax = RawDataTest_SinglePoint_Sub(RawData_PenRingX_DiffMax, RecordResult_PenRingX_DiffMax, ts->platdata->x_num, ts->y_gang_num,
											PS_Config_Lmt_PenRingX_FW_Diff_P, PS_Config_Lmt_PenRingX_FW_Diff_N);

			TestResult_PenRingX_DiffMin = RawDataTest_SinglePoint_Sub(RawData_PenRingX_DiffMin, RecordResult_PenRingX_DiffMin, ts->platdata->x_num, ts->y_gang_num,
											PS_Config_Lmt_PenRingX_FW_Diff_P, PS_Config_Lmt_PenRingX_FW_Diff_N);

			TestResult_PenRingY_DiffMax = RawDataTest_SinglePoint_Sub(RawData_PenRingY_DiffMax, RecordResult_PenRingY_DiffMax, ts->x_gang_num, ts->platdata->y_num,
											PS_Config_Lmt_PenRingY_FW_Diff_P, PS_Config_Lmt_PenRingY_FW_Diff_N);

			TestResult_PenRingY_DiffMin = RawDataTest_SinglePoint_Sub(RawData_PenRingY_DiffMin, RecordResult_PenRingY_DiffMin, ts->x_gang_num, ts->platdata->y_num,
											PS_Config_Lmt_PenRingY_FW_Diff_P, PS_Config_Lmt_PenRingY_FW_Diff_N);
		...
		uint32_t x_num, uint32_t y_num)
{
	...
	len /= sizeof(u32);
	if ((!data) || (!len) || (len < size)) {
		input_err(true, &ts->client->dev, "error find %s. len=%d\n", name, len);
		return -1;
	} else {
		input_info(true, &ts->client->dev, "%s. len=%d\n", name, len);
		ret = of_property_read_u32_array(np, name, array, len);
		if (ret) {
			input_err(true, &ts->client->dev, "error reading %s. ret=%d\n", name, ret);
			return -1;
		}
...
int32_t nvt_mp_parse_dt(struct device_node *root, const char *node_compatible)
{
	...
	if (ts->pen_support) {
		if (nvt_mp_parse_pen_array(np, "PS_Config_Lmt_PenTipX_FW_Raw_P", PS_Config_Lmt_PenTipX_FW_Raw_P,
				ts->platdata->x_num, ts->y_gang_num))
			return -1;
		...
		if (nvt_mp_parse_pen_array(np, "PS_Config_Lmt_PenTipY_FW_Raw_P", PS_Config_Lmt_PenTipY_FW_Raw_P,
				ts->x_gang_num, ts->platdata->y_num))
			return -1;
...
/*TabA7 Lite code for OT8-2503 by liufurong at 20210210 start*/
static void nvt_ito_test_work(struct work_struct *work)
{
	...
	{
		if (NULL == pfile) {
			pfile = filp_open(g_ito_data[i].filepath, O_RDWR | O_CREAT, 0644);
		}
		if (IS_ERR(pfile)) {
		...
		set_fs(KERNEL_DS);
		pos = g_ito_data[i].offset;
		data_len = g_ito_data[i].data_len;
		//input_info(true, &ts->client->dev, "len = %d ;HQ add forg_ito_data[i].data_len %d suscess\n", strlen(g_ito_data[i].ito_data), g_ito_data[i].data_len);
		vfs_write(pfile, g_ito_data[i].ito_data, data_len, &pos);
		filp_close(pfile, NULL);
		pfile = NULL;
		if (g_ito_data[i].ito_data) {
			kfree(g_ito_data[i].ito_data);
			g_ito_data[i].ito_data = NULL;
		}
	}
	set_fs(old_fs);
	mutex_unlock(&ts->ito_lock);
