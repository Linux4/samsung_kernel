...
#define PRINT_ROW_SIZE          (16)
    ...
    cts_info("Read fw reg add: 0x%04x size: %u", addr, size);
...
#define PRINT_ROW_SIZE          (16)
    ...
    cts_info("Read hw reg addr: 0x%08x size: %u", addr, size);
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (ret > 0) {
        return scnprintf(buf, PAGE_SIZE,
            ...
            ret, min, ktime_ms_delta(ktime_get(), start_time));
    } else if (ret < 0) {
        return scnprintf(buf, PAGE_SIZE,
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (ret > 0) {
        return scnprintf(buf, PAGE_SIZE,
            ...
            ret, min, max, ktime_ms_delta(ktime_get(), start_time));
    } else if (ret < 0) {
        return scnprintf(buf, PAGE_SIZE,
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (ret > 0) {
        return scnprintf(buf, PAGE_SIZE,
            ...
            ret, max, ktime_ms_delta(ktime_get(), start_time));
    } else if (ret < 0) {
        return scnprintf(buf, PAGE_SIZE,
...
#define COMP_CAP_TEST_DATA_FILENAME     "comp-cap-test-data.txt"
   ...
   /* struct cts_test_param int_pin_test_param = {
    ...
    int rawdata_min = 900;
    int rawdata_max = 2100;
    int noise_max = 70;
    int open_min = 1800;
    int short_min = 200;
    int comp_cap_min = 1;
    int comp_cap_max = 126;
    ...
    dir_result = cts_mkdir_for_file(rawdata_test_param.test_data_filepath, 0777);
     if (dir_result) {
         cts_err("Create dir for test data failed %d(%s)",
             dir_result, cts_strerror(dir_result));
     }
    ...
    if (comp_cap_test_result > 0) {
        ...
        count += scnprintf(buf + count, PAGE_SIZE,
            "%-10s test: FAIL %d(%s)\n", "Comp-Cap",
            comp_cap_test_result,
            cts_strerror(comp_cap_test_result));
    } else {
        count += scnprintf(buf + count, PAGE_SIZE,
            "%-10s test: PASS\n", "Comp-Cap");
    }
...
static ssize_t up_time_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    cts_info("Read UP-TIME");

    return scnprintf(buf, 30,  "up time: %d\n",  cts_up_time);
}

static ssize_t up_time_store(struct device *dev,
        struct device_attribute *attr,const char *buf, size_t count)
{
    int ret;
    ret = kstrtoint(buf, 0, &cts_up_time);
    if (ret != 0){
        cts_err("kstrtoint error:%d", ret);
    }
    cts_info("Write UP-TIME :%d", cts_up_time);
    return count;

}

static DEVICE_ATTR(up_time, S_IRUSR | S_IWUSR, up_time_show, up_time_store);
