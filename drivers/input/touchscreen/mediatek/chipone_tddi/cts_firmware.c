        ...
        const struct cts_firmware_sect_info *firmware_info)
{
    ...
    if (firmware_info->firmware_sect_size < 4) {
	cts_err("Program firmware size %d", firmware_info->firmware_sect_size - 4);
	return 0;
    }else{
	sect_size_fw = firmware_info->firmware_sect_size - 4;
    }

    ret = cts_program_flash_from_sram(cts_dev, 4, 4, sect_size_fw);
    if (ret) {
