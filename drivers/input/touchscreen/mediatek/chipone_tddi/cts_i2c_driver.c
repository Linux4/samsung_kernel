...
bool chipone_gestrue_status = DISABLE;
...
static void aot_enable(void *device_data)
{
    ...
    if(buf[0] == 1){
	 ...
	 cts_enable_gesture_wakeup(&cts_data->cts_dev);
        chipone_gestrue_status = ENABLE;
#endif
    }else{
#ifdef CFG_CTS_GESTURE
        cts_disable_gesture_wakeup(&cts_data->cts_dev);
        chipone_gestrue_status = DISABLE;
#endif
    }
...
#ifdef SEC_TSP_FACTORY_TEST
      ret = sec_cmd_init(&cts_data->sec, chipone_commands, ARRAY_SIZE(chipone_commands), SEC_CLASS_DEVT_TSP);
	if (ret < 0) {
...
err_free_pdata:
	if(cts_data->pdata->irq_thread){
		kthread_stop(cts_data->pdata->irq_thread);
		cts_data->pdata->irq_thread = NULL;
	}
...
int cts_driver_init(void)
{
	...
	cts_info("Init return init_flag "CTS_ERR_FMT_STR, CTS_ERR_ARG(init_flag));
	if(init_flag)
		return init_flag;
	else	
		return 0;
