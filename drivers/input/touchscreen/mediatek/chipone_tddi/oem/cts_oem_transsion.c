...
#define OEM_OF_DEF_PROPVAL_RAWDATA_FRAMES   16
#define OEM_OF_DEF_PROPVAL_RAWDATA_MIN      1200
#define OEM_OF_DEF_PROPVAL_RAWDATA_MAX      2900
#define OEM_OF_DEF_PROPVAL_NOISE_FRAMES     16
#define OEM_OF_DEF_PROPVAL_NOISE_MAX        70
#define OEM_OF_DEF_PROPVAL_OPEN_MIN         2600
...
#define OEM_TEST_DATA_DIR                   "/data/vendor/fac_sources/"
#define OEM_CHIPONE_TEST_DATA_DIR           "/data/vendor/fac_sources/chipone"
...
static int tp_openshort_result;
static struct cts_oem_data *tp_cts_data = NULL;

struct cts_oem_data {
    struct proc_dir_entry *selftest_proc_entry;
	struct proc_dir_entry *proc_touchscreen_dir;
	struct proc_dir_entry *proc_ctp_openshort_test_file;
...
{
    //int offset = strlen(OEM_OF_PROPNAME_PREFIX);
...
static int save_selftest_data_to_file(struct cts_oem_data *oem_data)
{
    ...
    if ((dev_mkdir(OEM_TEST_DATA_DIR, S_IRUGO | S_IWUSR)) != 0)
        cts_err("Failed to create directory for mp_test\n");

    ktime_get_real_ts64(&ts);
    ts.tv_sec -= sys_tz.tz_minuteswest * 60;
    rtc_time_to_tm(ts.tv_sec, &rtc_tm);
    snprintf(strtime, sizeof(strtime), "_%04d%02d%02d_%02d%02d%02d/",
        rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
        rtc_tm.tm_hour, rtc_tm.tm_min, rtc_tm.tm_sec);

    snprintf(dir, sizeof(OEM_CHIPONE_TEST_DATA_DIR) + sizeof(strtime),
        OEM_CHIPONE_TEST_DATA_DIR"%s", strtime);
    if ((dev_mkdir(dir, S_IRUGO | S_IWUSR)) != 0)
        cts_err("Failed to create directory for chipone_test\n");

    snprintf(test_file_path, sizeof(OEM_CHIPONE_TEST_DATA_DIR)
        + sizeof(strtime) + sizeof(OEM_RAWDATA_TEST_DATA_FILEPATH),
        OEM_CHIPONE_TEST_DATA_DIR"%s"OEM_RAWDATA_TEST_DATA_FILEPATH, strtime);
    cts_info("test_file_path:%s", test_file_path);
    if (oem_data->test_rawdata) {
            ret = dump_tsdata_to_csv_file(test_file_path,
    ...
    snprintf(test_file_path, sizeof(OEM_CHIPONE_TEST_DATA_DIR)
        + sizeof(strtime) + sizeof(OEM_NOISE_TEST_DATA_FILEPATH),
        OEM_CHIPONE_TEST_DATA_DIR"%s"OEM_NOISE_TEST_DATA_FILEPATH, strtime);
    if (oem_data->test_noise) {
            ret = dump_tsdata_to_csv_file(test_file_path,
    ...
    snprintf(test_file_path, sizeof(OEM_CHIPONE_TEST_DATA_DIR)
        + sizeof(strtime) + sizeof(OEM_OPEN_TEST_DATA_FILEPATH),
        OEM_CHIPONE_TEST_DATA_DIR"%s"OEM_OPEN_TEST_DATA_FILEPATH, strtime);
    if (oem_data->test_open) {
        ret = dump_tsdata_to_csv_file(test_file_path,
    ...
    snprintf(test_file_path, sizeof(OEM_CHIPONE_TEST_DATA_DIR)
        + sizeof(strtime) + sizeof(OEM_SHORT_TEST_DATA_FILEPATH),
        OEM_CHIPONE_TEST_DATA_DIR"%s"OEM_SHORT_TEST_DATA_FILEPATH, strtime);
    if (oem_data->test_short) {
            ret = dump_tsdata_to_csv_file(test_file_path,
    ...
    snprintf(test_file_path, sizeof(OEM_CHIPONE_TEST_DATA_DIR)
        + sizeof(strtime) + sizeof(OEM_COMP_CAP_TEST_DATA_FILEPATH),
        OEM_CHIPONE_TEST_DATA_DIR"%s"OEM_COMP_CAP_TEST_DATA_FILEPATH, strtime);
    if (oem_data->test_comp_cap) {
        ret = dump_comp_cap_to_csv_file(test_file_path,
...
int cts_oem_init(struct chipone_ts_data *cts_data)
{
	...
	oem_data->proc_touchscreen_dir = proc_mkdir(PROC_TOUCHSCREEN_FOLDER , NULL);
    if (oem_data->proc_touchscreen_dir == NULL)  {
        cts_err("%s: proc_touchpanel_dir file create failed!\n", __func__);
        ret = -EFAULT;
        goto free_oem_data;
    }else{
#ifdef PROC_NODE
        oem_data->proc_ctp_openshort_test_file = proc_create_data(PROC_CTP_OPENSHORT_TEST_FILE,
		(S_IWUSR | S_IRUGO), oem_data->proc_touchscreen_dir, &proc_ctp_openshort_test_fops, cts_data);
        if((oem_data->proc_ctp_openshort_test_file)==NULL){
            cts_err("%s: proc_ctp_openshort_test_file create failed!\n", __func__);
            remove_proc_entry(PROC_TOUCHSCREEN_FOLDER, NULL);
            ret = -EFAULT;
            goto free_oem_data;
        }else{
            cts_err("%s:  proc_create PROC_CTP_OPENSHORT_TEST_FILE success",__func__);
        }
...
int proc_node_init(struct chipone_ts_data *cts_data)
{
	struct cts_oem_data *oem_data = NULL;
...
int proc_node_deinit(struct chipone_ts_data *cts_data)
{
    struct cts_oem_data *oem_data = NULL;
    ...
    if (oem_data->proc_ctp_openshort_test_file) {
        cts_info("  Remove '/proc/"PROC_TOUCHSCREEN_FOLDER"'");
        remove_proc_entry(PROC_TOUCHSCREEN_FOLDER, NULL);
    }
