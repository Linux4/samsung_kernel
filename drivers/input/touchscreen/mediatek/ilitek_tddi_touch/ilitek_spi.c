...
static int ilitek_spi_probe(struct spi_device *spi)
{
	...
	idev->fw_open = REQUEST_FIRMWARE;//FILP_OPEN;
...
#if ENABLE_GESTURE
	idev->gesture = DISABLE; // gesture mode  default is disable
       gestrue_status = idev->gesture;
#endif
         idev->glove = DISABLE;// touch sensitivity default is disable
...
int ilitek_tddi_interface_dev_init(struct ilitek_hwif_info *hwif)
{
	...
	ipio_err("info->bus_driver.driver.name == %s, hwif->name == %s,\n",info->bus_driver.driver.name,  hwif->name);
