...
static int ilitek_tddi_fw_iram_upgrade(u8 *pfw)
{
	...
	if (!idev->info_from_hex || (idev->chip->core_ver < CORE_VER_1410)){
		...
		mdelay(20); //ritchie add for wait firstcmd
	}
...
static int ilitek_tddi_fw_ili_convert(u8 *pfw)
{
	...
	ipio_info("Enter %s.\n", __func__);
	if (size_ili < ILI_FILE_HEADER) {
		ipio_err("size of ILI file is invalid, size_ili = %d.\n", size_ili);
		return -EINVAL;
	}
...
out:
	...
	tfd.end_addr = (size_ili - ILI_FILE_HEADER);
	ipio_info("Exit %s.\n", __func__);
	return 0;
}
...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
	...
	ipio_err("Enter %s.\n", __func__);
	ipio_err("Open file method = %s, path = %s\n",
...
out:
	...
	schedule_work(&idev->glove_work_queue);//+ add by songbinbo.wt for High sensitivity mode contrl after suspend resume 20191125
	ipio_err("Exit %s.\n", __func__);
	return ret;
}
