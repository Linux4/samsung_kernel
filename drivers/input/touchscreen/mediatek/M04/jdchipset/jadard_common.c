...
#ifdef JD_EARPHONE_DETECT
/**
*Name: <jadard_earphone_read>
*Author: <huangzhongjie>
*Date: <2022/08/08>
*Param: <struct file *file, char *buf, size_t len, loff_t *pos>
...
{
	struct jadard_ts_data *ts = pjadard_ts_data;
	char buf_tmp[EARPHONE_WRITE_LEN] = {0};
...
static int jd_get_tp_module(void)
{
    ...
    JD_I("command_line = %s\n", command_line);
	/*hs04 code for DEAL6398A-1724 by tangsumian1 at 20220915 start*/
	if (NULL != strstr(command_line, "lcd_jd9365t_txd_ctc_mipi_hdp_video")){
		ts->firmware_name = "txd_ctc_jadard_firmware.bin";
		mtp_chip_name = "TXD_CTC_JD9365T";
		JD_I("txd ctc panel has been found\n");
	} else if (NULL != strstr(command_line, "lcd_jd9365t_txd_boe_mipi_hdp_video")){
		ts->firmware_name = "txd_boe_jadard_firmware.bin";
		mtp_chip_name = "TXD_BOE_JD9365T";
		JD_I("txd boe panel has been found\n");
	} else if (NULL != strstr(command_line, "lcd_jd9365t_txd_ctc_jbpd_mipi_hdp_video")){
		ts->firmware_name = "txd_ctc_jbpd_jadard_firmware.bin";
		mtp_chip_name = "TXD_CTC_JBPD_JD9365T";
		JD_I("txd ctc jbpd panel has been found\n");
	} else {
		JD_I("can't find lcd!");
	}
	/*hs04 code for DEAL6398A-1724 by tangsumian1 at 20220915 end*/
...
/*hs04 code for DEAL6398A-522 by huangzhongjie at 20220808 start*/
/**
*Name: <jadard_earphone_notifier_callback>
*Author: <huangzhongjie>
*Date: <2022/08/08>
...
static int jadard_earphone_notifier_callback(struct notifier_block *nb, unsigned long event, void *ptr)
{
	...
	if (earphone_status != pjadard_ts_data->earphone_enable) {
		...
		/*hs04 code for DEAL6398A-184 by huangzhongjie at 20220809 start*/
		g_module_fp.fp_set_earphone_enable(earphone_status);
		/*hs04 code for DEAL6398A-184 by huangzhongjie at 20220809 end*/
	}

	return 0;
}
/**
*Name: <jadard_earphone_init>
*Author: <huangzhongjie>
*Date: <2022/08/08>
*Param: <NA>
*Return: <NA>
