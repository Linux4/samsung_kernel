...
/**
*Name: <cts_charger_notify_work>
*Author: <tangsumian1>
*Date: <2022/08/27>
*Param: <struct work_struct *work>
*Return: <NA>
*Purpose: <charger notify work>
*/
...
/**
*Name: <cts_charger_notifier_callback>
*Author: <tangsumian1>
*Date: <2022/08/27>
*Param: <struct notifier_block *nb, unsigned long val, void *v>
*Return: <charger result>
*Purpose: <charger notifier callback>
*/
static int cts_charger_notifier_callback(struct notifier_block *nb, unsigned long val, void *v)
{
    ...
    if (chipone_ts_data->fw_update_stat != 1) {
        return 0;
    }
    ...
    if (!strcmp(psy->desc->name, "usb")) {
        if (psy && val == POWER_SUPPLY_PROP_STATUS) {
            ...
            cts_info("cts_charger_notifier_callback, prop.intval=%d,usb_plug_status=%d!",prop.intval,chipone_ts_data->usb_plug_status);
            if (ret < 0) {
                cts_err("Couldn't get POWER_SUPPLY_PROP_ONLINE rc=%d", ret);
                return ret;
            } else {
                if (prop.intval != chipone_ts_data->usb_plug_status) {
                    cts_info("usb_plug_status = %d", prop.intval);
                    chipone_ts_data->usb_plug_status = prop.intval;
                    if ((chipone_ts_data->cts_dev.rtdata.suspended == false) && (chipone_ts_data->cts_charger_notify_wq != NULL)) {
                        queue_work(chipone_ts_data->cts_charger_notify_wq, &chipone_ts_data->charger_notify_work);
                    }
                }
            }
        }
...
/**
*Name: <cts_charger_init>
*Author: <tangsumian1>
*Date: <2022/08/27>
*Param: <NA>
*Return: <NA>
*Purpose: <charger_init>
*/
void cts_charger_init(void)
{
    ...
    struct power_supply *psy = NULL;
    union power_supply_propval prop;
    chipone_ts_data->cts_charger_notify_wq = create_singlethread_workqueue("cts_charger_wq");
    if (!chipone_ts_data->cts_charger_notify_wq) {
        cts_err("allocate cts_charger_notify_wq failed");
        return;
    }
    INIT_WORK(&chipone_ts_data->charger_notify_work, cts_charger_notify_work);
    chipone_ts_data->charger_notif.notifier_call = cts_charger_notifier_callback;
    ret = power_supply_reg_notifier(&chipone_ts_data->charger_notif);
    if (ret) {
...
/**
*Name: <cts_earphone_notifier_callback>
*Author: <tangsumian1>
*Date: <2022/08/27>
...
*/
static int cts_earphone_notifier_callback(struct notifier_block *nb, unsigned long event, void *ptr)
{
    int ret = 0;
    ...
    if (event == EARPHONE_PLUGIN_STATE) {
        chipone_ts_data->earphone_status = 1;
        cts_info("earphone in");
    } else if (event == EARPHONE_PLUGOUT_STATE) {
        chipone_ts_data->earphone_status = 0;
    ...
    if (earphone_pre_status != chipone_ts_data->earphone_status) {
        ...
        earphone_pre_status = chipone_ts_data->earphone_status;
    }

    return 0;
}
/**
*Name: <cts_earphone_init>
*Author: <tangsumian1>
*Date: <2022/08/27>
*Param: <NA>
*Return: <NA>
...
*/
void cts_earphone_init(void)
{
    int ret = 0;
    chipone_ts_data->earphone_notif.notifier_call = cts_earphone_notifier_callback;
    ret = headset_notifier_register(&chipone_ts_data->earphone_notif);
    if (ret) {
...
int cts_suspend_device(struct cts_device *cts_dev)
{
    ...
    cts_info("Set suspend mode:%s", cts_dev->rtdata.gesture_wakeup_enabled ? "gesture" : "sleep");
...
static void cts_update_module_info(void)
{
    ...
    {
    case MODEL_HLT_HKC:
        chipone_ts_data->firmware_filepath = "/vendor/firmware/hlt_hkc_firmware_nl9911c.bin";
        mtp_chip_name = "HKC_NL9911C";
        break;
/*hs04 code for SR-AL6398A-01-676 by tangsumian1 at 20220722 start*/
    case MODEL_HR_HR:
        chipone_ts_data->firmware_filepath = "/vendor/firmware/hr_hr_firmware_nl9911c.bin";
        mtp_chip_name = "HR_NL9911C";
        break;
