...
#define MIP_BIN_TAIL_MARK		{0x4D, 0x42, 0x54, 0x01}
...
*/
static int mip_isc_read_cal_data(struct mip_ts_info *info, u8 *data)
{
...
int mip_flash_fw(struct mip_ts_info *info, const u8 *fw_data, size_t fw_size, bool force, bool section)
{
	...
	if (fw_size != (48 * 1024 - cal_data_size)) {
		dev_err(&client->dev, "%s [ERROR] wrong bin size [%d]\n", __func__, fw_size);
	...
	image_size = bin_size + cal_data_size;
	...
	if (mip_isc_read_cal_data(info, rbuf)) {
		dev_err(&client->dev, "%s [ERROR] mip_isc_read_clock_data\n", __func__);
		ret = fw_err_download;
		goto ERROR_UPDATE;
	}
#if MIP_FW_UPDATE_DEBUG
	print_hex_dump(KERN_ERR, MIP_DEVICE_NAME " Cal Data : ", DUMP_PREFIX_OFFSET, 16, 1, rbuf, cal_data_size, false);
#endif

	/* Prepare image */
	memcpy(&image_data[bin_size], rbuf, cal_data_size);
#if MIP_FW_UPDATE_DEBUG
	print_hex_dump(KERN_ERR, MIP_DEVICE_NAME " Image : ", DUMP_PREFIX_OFFSET, 16, 1, image_data, image_size, false);
#endif
	...
	while (offset >= offset_start) {
		/* Program page */
		if (mip_isc_program_page(info, offset, &image_data[offset], ISC_PAGE_SIZE)) {
...
#if MIP_FW_UPDATE_DEBUG
		print_hex_dump(KERN_ERR, MIP_DEVICE_NAME " F/W File : ", DUMP_PREFIX_OFFSET, 16, 1, &image_data[offset], ISC_PAGE_SIZE, false);
