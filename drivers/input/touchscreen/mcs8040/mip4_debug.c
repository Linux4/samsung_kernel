...
static ssize_t mip_sys_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	} else if (!strcmp(attr->attr.name, "mode_edge")) {
...
static ssize_t mip_sys_test(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	} else if (!strcmp(attr->attr.name, "test_hybrid_cp")) {
		test_type = MIP_TEST_TYPE_HSELF_CP;
	} else if (!strcmp(attr->attr.name, "test_hybrid_jitter")) {
...
static DEVICE_ATTR(mode_edge, S_IRUGO | S_IWUSR | S_IWGRP, mip_sys_mode_show, mip_sys_mode_store);
...
#endif
#if ((CHIP_MODEL == CHIP_MMS500) || (CHIP_MODEL == CHIP_MFS10))
static DEVICE_ATTR(test_hybrid_cp, S_IRUGO, mip_sys_test, NULL);
static DEVICE_ATTR(test_hybrid_jitter, S_IRUGO, mip_sys_test, NULL);
#endif
#if MIP_USE_WAKEUP_GESTURE
...
#endif
#if ((CHIP_MODEL == CHIP_MMS500) || (CHIP_MODEL == CHIP_MFS10))
	&dev_attr_test_hybrid_cp.attr,
	&dev_attr_test_hybrid_jitter.attr,
#endif
#if MIP_USE_WAKEUP_GESTURE
