...
#if 0 //def CONFIG_BATTERY_SAMSUNG
...
static int bt532_power_ctrl(void *data, bool on)
{
	...
	struct bt532_ts_platform_data *pdata = info->pdata;
	struct device *dev = &info->client->dev;
	static struct regulator *regulator_dvdd = NULL;
...
#if 0
	if (!pdata->gpio_ldo_en && regulator_dvdd == NULL) {
	...
	if(regulator_avdd == NULL){
		...
		retval = regulator_set_voltage(regulator_avdd, 3300000, 3300000);
		if (retval < 0)
			tsp_debug_err(true, dev, "%s: set voltage error(%d)\n", 
					__func__, retval);		
	}
