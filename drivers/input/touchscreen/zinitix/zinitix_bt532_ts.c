...
const u16 dnd_v_diff_00[29][18]={
	...
	{4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500},
	{4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500,	4500},
	{	0,	     0,	    0,		      0,	4500,	0	,	0	,	0	,	0	,	0	,	0	,	0	,	0	,	4500,	0	,	0,		0	,	0	}
};
...
static void bt532_firmware_check(void)
{
	...
	ret = gpio_request_one(TSP_VENDOR_1, GPIOF_DIR_IN, "TSP_ID_CHECK1");

	if (ret < 0) {
		zinitix_printk("%s: Request GPIO failed, gpio %d (%d)\n", BT532_TS_DEVICE,
			TSP_VENDOR_1, ret);
		return;
	}
	ret = gpio_request_one(TSP_VENDOR_2, GPIOF_DIR_IN, "TSP_ID_CHECK2");
	if (ret < 0) {
		zinitix_printk("%s: Request GPIO failed, gpio %d (%d)\n", BT532_TS_DEVICE,
			TSP_VENDOR_2, ret);
		return;
	...
	printk("gpio_get_value(TSP_ID_1:gpio%d)=%d, gpio_get_value(TSP_ID_2:gpio%d)=%d\n",\
		TSP_VENDOR_1, gpio_get_value(TSP_VENDOR_1), TSP_VENDOR_2, gpio_get_value(TSP_VENDOR_2));

	if(!gpio_get_value(TSP_VENDOR_1))
		zinitix_bit_clr(misc_info->tsp_type_hw,0);		//HW_ID 2
	if(!gpio_get_value(TSP_VENDOR_2))
		zinitix_bit_clr(misc_info->tsp_type_hw,1);		//HW_ID 0

	gpio_free(TSP_VENDOR_1);
	gpio_free(TSP_VENDOR_2);
		...
		if (zinitix_bit_test(sub_status, SUB_BIT_EXIST)) {
			...
			if (zinitix_bit_test(sub_status, SUB_BIT_DOWN)){
				...
				tsp_debug_info(true, &client->dev, "Finger [%02d] x = %d, y = %d,"
							" w = %d fw=0x%x \n", i, x, y, w, info->cap_info.reg_data_version);
#else
...
static bool ts_set_touchmode(u16 value){
	...
	if(value == TOUCH_DND_MODE) {
		read_data(misc_info->client, ZINITIX_INTERNAL_FLAG_02, (u8 *)&reg_val, 2);
		zinitix_bit_clr(reg_val, 1);
		write_reg(misc_info->client, ZINITIX_INTERNAL_FLAG_02, reg_val);
