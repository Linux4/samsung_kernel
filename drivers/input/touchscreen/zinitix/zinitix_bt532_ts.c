...
#include "zinitix_touch_bt532_firmware_mega2_01.h"
#include "zinitix_touch_bt532_firmware_mega2_02.h"
#define TSP_TYPE_COUNT	2
u8 *m_pFirmware [TSP_TYPE_COUNT] = {(u8*)m_firmware_data_02,(u8*)m_firmware_data_01,};
u8 m_FirmwareIdx = 0;
...
static void ts_select_type_hw(struct bt532_ts_info *info) {
	...
	zinitix_printk(KERN_INFO "touch tsp type = %d, %d\n", i, m_FirmwareIdx);
...
static bool mini_init_touch(struct bt532_ts_info *info)
{
	...
	tsp_debug_info(true, &client->dev, "touch max x = %d\r\n", pdata->x_resolution);
	tsp_debug_info(true, &client->dev, "touch max y = %d\r\n", pdata->y_resolution);
...
static bool  bt532_touch_key_force_release(struct bt532_ts_info *info)
{
	...
	for(i=0; i<info->cap_info.button_num; i++) {
		if (info->button[i] == ICON_BUTTON_DOWN) {
			...
			if (ret < 0) {
				tsp_debug_info(true, &client->dev, ": Failed to read %d's key sensitivity %s\n", i, __func__);
			}
			if(val < info->cap_info.key_threshold) {
				info->button[i] = ICON_BUTTON_UP;
				input_report_key(info->input_dev, BUTTON_MAPPING_KEY[i], 0);
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
				tsp_debug_info(true, &client->dev, "Button up = %d (%s)\n", i, __func__);
#else
				tsp_debug_info(true, &client->dev, "Button up (%s)\n", __func__);
#endif
				flag = true;
			}
		}
...
#if defined(CONFIG_MACH_MEGA2)
		if(bt532_touch_key_force_release(info))
			input_sync(info->input_dev);
#endif
		goto out;
	}
...
#if defined(CONFIG_MACH_MEGA2)
	bt532_touch_key_force_release(info);
#endif
...
static bool run_reference_PDiff_read(void *device_data, const u16 h_diff[][17], const u16 v_diff[][18], int bHidden1, int bMenu, int bBack, int bHidden2 ) //DND
{
	...
	tsp_debug_info(true, &client->dev, "%s : ++++++ DND SPEC +++++++++\n",__func__);
	for(i = 0; i < x_num; i++)
	{
		tsp_debug_info(true, &client->dev, "%s : ",__func__);
		for(j = 0; j < y_num; j++)
		{
			tsp_debug_info(true, &client->dev, "%5d ", info->dnd_data[i*y_num+j]);
		}
	tsp_debug_info(true, &client->dev, "\n");
	}
	tsp_debug_info(true, &client->dev, "%s : ------- DND SPEC ----------\n",__func__);
	tsp_debug_info(true, &client->dev, "%s : TSP Diff test scale factor = %d\n", __func__, info->ref_scale_factor);
	tsp_debug_info(true, &client->dev, "%s : H Diff start\n",__func__);
	//H DIff
...
#if PDIFF_DEBUG
	tsp_debug_info(true, &client->dev, "%s : ++++++ h_diff SPEC +++++++++\n",__func__);
	for(i = 0; i < x_num-1; i++){
		tsp_debug_info(true, &client->dev, "%s : ",__func__);
		for(j = 0; j < y_num-1; j++)
			tsp_debug_info(true, &client->dev, "%5d ",(u16)h_diff[i][j]);
		tsp_debug_info(true, &client->dev, "\n");
	}
	tsp_debug_info(true, &client->dev, "%s : ------- h_diff SPEC ----------\n",__func__);
	tsp_debug_info(true, &client->dev, "%s : ++++++ calculated h_diff SPEC +++++++++\n",__func__);
	for(i = 0; i < x_num-1; i++){
		tsp_debug_info(true, &client->dev, "%s : ",__func__);
		for(j = 0; j < y_num-1; j++)
			tsp_debug_info(true, &client->dev, "%5d ",print_diff[i*y_num+j]);
		tsp_debug_info(true, &client->dev, "\n");
	}
	tsp_debug_info(true, &client->dev, "%s : ------- calculated h_diff SPEC ----------\n",__func__);
#endif
	tsp_debug_info(true, &client->dev, "%s :",__func__);
	if(pass)
		tsp_debug_info(true, &client->dev, "H Diff pass\n");
	else
		tsp_debug_info(true, &client->dev, "H Diff fail\n");

	tsp_debug_info(true, &client->dev, "%s : V Diff start\n",__func__);
	info->hdiff_max_x = info->hdiff_max_y = info->hdiff_min_x = info->hdiff_min_y = 0;
	...
	tsp_debug_info(true, &client->dev, "%s : ",__func__);
	if(pass)
		tsp_debug_info(true, &client->dev, "V Diff view pass\n");
	else
		tsp_debug_info(true, &client->dev, "V Diff view fail\n");

#if PDIFF_DEBUG
	tsp_debug_info(true, &client->dev, "%s : ++++++ v_diff SPEC +++++++++\n",__func__);
	for(i=0; i < x_num-2; i++){
		tsp_debug_info(true, &client->dev, "%s : ",__func__);
		for(j=0; j < y_num; j++)
			tsp_debug_info(true, &client->dev, "%5d ",(u16)v_diff[i][j]);
		tsp_debug_info(true, &client->dev, "\n");
	}
	tsp_debug_info(true, &client->dev, "%s : ------- v_diff SPEC ----------\n",__func__);
	tsp_debug_info(true, &client->dev, "%s : ++++++ calculated v_diff SPEC +++++++++\n",__func__);
	for(i=0; i < x_num-2; i++){
		tsp_debug_info(true, &client->dev, "%s : ",__func__);
		for(j=0; j < y_num; j++)
			tsp_debug_info(true, &client->dev, "%5d ",print_diff[i*y_num+j]);
	...
	{
		tsp_debug_info(true, &client->dev, "%s : TSP Button scale = %d\n", __func__, info->ref_scale_factor);
		tsp_debug_info(true, &client->dev, "%s : TSP Button Diff Spec. = %d %d %d %d\n", __func__,
...
#if PDIFF_DEBUG
			tsp_debug_info(true, &client->dev, "%s : ",__func__);
			tsp_debug_info(true, &client->dev, "buttons[%d]'s diff_val is %d\n", i, diff_val);
#endif
			}
			}
	tsp_debug_info(true, &client->dev, "%s : ",__func__);
	if(pass)
		tsp_debug_info(true, &client->dev, "Button Diff pass\n");
	else
		tsp_debug_info(true, &client->dev, "Button Diff fail\n");
...
static void run_reference_PDiff(void *device_data)
{
	...
	tsp_debug_info(true, &misc_info->client->dev, "run_ref_diff check hw_id = 0x%04X\n", info->cap_info.hw_id);
	...
	*/
	if(check) {
		tsp_debug_info(true, &misc_info->client->dev, "diff pass\n");
		sprintf(buff, "%s\n", "pass");
		set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
		finfo->cmd_state = 2;
	}
	else {
		tsp_debug_info(true, &misc_info->client->dev, "diff fail\n");
		sprintf(buff, "%s\n", "fail");
		set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
		finfo->cmd_state = 3;
	}
...
static void boost_level(void *device_data)
{
	...
	if (finfo->cmd_param[0] < 0 || finfo->cmd_param[0] >= max_level) {
		snprintf(buff, sizeof(buff), "NG");
		finfo->cmd_state = FAIL;
	} else {
		change_boost_level(finfo->cmd_param[0], BOOSTER_DEVICE_TOUCH);
		...
		const struct i2c_device_id *i2c_id)
{
	...
	if(defer_probe == true){
		defer_probe = false;
		tsp_debug_err(true, &client->dev, "defer_probe\n");
		return -EPROBE_DEFER;
	}
