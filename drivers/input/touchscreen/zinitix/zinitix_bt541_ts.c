	...
	if (reported == true ||
		...
		for (i = 0; i < info->cap_info.multi_fingers; i++) {
			...
			if (zinitix_bit_test(prev_sub_status, SUB_BIT_EXIST)) {
				dev_info(&client->dev, "Finger [%02d] up ver0x%02x hw0x%02x\n", 
					i, info->cap_info.reg_data_version, info->cap_info.hw_id);
				info->finger_cnt1--;
...
#if defined(CONFIG_SAMSUNG_LPM_MODE) && !defined(TEMP_USE_FOR_DEV)
