...
static irqreturn_t secure_filter_interrupt(struct bt532_ts_info *info)
{
	if (atomic_read(&info->secure_enabled) == SECURE_TOUCH_ENABLED) {
...
static int secure_touch_clk_prepare_enable(struct bt532_ts_info *info)
{
	int ret;
...
static void secure_touch_clk_unprepare_disable(struct bt532_ts_info *info)
{
	if (!info->core_clk || !info->iface_clk) {
		input_err(true, &info->client->dev, "%s: error clk\n", __func__);
		return;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (data == 1) {
		...
		if (pm_runtime_get_sync(info->client->adapter->dev.parent) < 0) {
			...
			write_reg(info->client, BT532_PERIODICAL_INTERRUPT_INTERVAL, SCAN_RATE_HZ * ESD_TIMER_INTERVAL);
		...
		clear_report_data(info);
		bt532_touch_work(info->client->irq, info);
...
static int secure_touch_init(struct bt532_ts_info *info)
{
...
static void secure_touch_remove(struct bt532_ts_info *info)
{
	if (!IS_ERR_OR_NULL(info->core_clk))
		clk_put(info->core_clk);
...
static void secure_touch_stop(struct bt532_ts_info *info, bool stop)
{
	...
	for (flash_addr = 0; flash_addr < size; ) {
		for (i = 0; i < page_sz / TC_SECTOR_SZ_READ; i++) {
			if (read_firmware_data(client, BT532_READ_FLASH,
				...
				input_err(true, &client->dev, "%s: Failed to read firmare\n", __func__);
				goto fail_upgrade;
			}
...
static bool ts_hw_calibration(struct bt532_ts_info *info)
{
	...
	do {
		...
		if (read_data(client, BT532_EEPROM_INFO, (u8 *)&chip_eeprom_info, 2) < 0)
			return false;
...
static int fw_update_work(struct bt532_ts_info *info, bool force_update)
{
	...
	if (need_update == true || force_update == true) {
		...
		ret = read_data(info->client, BT532_EEPROM_INFO, (u8 *)&chip_eeprom_info, 2);
		if (ret < 0) {
		...
		if (ret < 0) {
			input_err(true, &info->client->dev, "%s: sec_tclm_get_nvm_all error \n", __func__);
			ret = 0;
			goto fw_request_fail;
		}
...
static bool init_touch(struct bt532_ts_info *info)
{
	...
	if (read_data(info->client, BT532_SUPPORTED_FINGER_NUM, data, 1) < 0)
...
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
				input_info(true, &client->dev, "Finger [%02d] x = %d, y = %d, %s"
							" w=%d p=%d g=%d fw=0x%02x%02x (0x%x)\n", i, x, y, location, w, palm, info->glove_touch,
							info->cap_info.hw_id, info->cap_info.reg_data_version, ic_status);
#else
				input_info(true, &client->dev, "Finger [%02d] %s w=%d p=%d g=%d fw=0x%02x%02x (0x%x)\n"
							, i, location, w, palm, info->glove_touch, info->cap_info.hw_id, info->cap_info.reg_data_version, ic_status);
#endif
...
static void run_ssr_read(void *device_data)
{
	...
	input_info(true,&client->dev, "SELF SATURATION RX start\n");
	...
	input_info(true, &client->dev, "SELF SATURATION RX Pass\n");
...
int bt532_tclm_data_read(struct i2c_client *client, int address)
{
	...
	case SEC_TCLM_NVM_OFFSET_IC_FIRMWARE_VER:
		...
		ret = read_data(client, BT532_CHIP_REVISION, (u8*)&buff[2], 8);
		if (ret < 0) {
			input_err(true, &info->client->dev,"%s: fail chip_revision\n", __func__);
			return ret;
		}
...
static void check_connection(void *device_data)
{
	...
	if (conn_check_val != 1) {
		input_err(true, &info->client->dev,"%s: TSP connection value is %X\n", __func__, conn_check_val);
		goto err_conn_check;
	}
...
#ifndef CONFIG_EXYNOS_DECON_MDNIE_LITE
	int lcdtype = 0;
#endif
