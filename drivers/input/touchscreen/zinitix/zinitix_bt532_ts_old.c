...
#ifdef CONFIG_TOUCHSCREEN_ZINITIX_ZT75XX_OLD
...
#else
				input_info(true, &client->dev, "Finger [%02d] w=%d p=%d fw=0x%02x%02x \n"
...
static int  bt532_ts_open(struct input_dev *dev)
{
	...
	input_dbg(true, &misc_info->client->dev, "%s, %d \n", __func__, __LINE__);
...
static bool ts_set_touchmode3(u16 value)
{
	...
	if(value == TOUCH_RXSHORT_MODE) {
		if (write_reg(misc_info->client, 0xc000, 0x0001) != I2C_SUCCESS) 
			input_info(true, &misc_info->client->dev, "vendor cmd enable \n");
		...
		if (write_reg(misc_info->client, 0xc000, 0x0000) != I2C_SUCCESS) 
			input_info(true, &misc_info->client->dev, "vendor cmd disable \n");    
...
static void clear_cover_mode(void *device_data)
{
	...
	input_info(true, &info->client->dev, "COVER state = %d\n", g_cover_state);
	set_cmd_result(info, finfo->cmd_buff,
					strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	mutex_lock(&finfo->cmd_lock);
	finfo->cmd_is_running = false;
