...
static int32_t nvt_download_init(void)
{
	...
	//input_info(true, &ts->client->dev, "NVT_TRANSFER_LEN = 0x%06X\n", NVT_TRANSFER_LEN);
...
static int32_t nvt_bin_header_parser(const u8 *fwdata, size_t fwsize)
{
	...
	input_info(true, &ts->client->dev, "cascade_2nd_header_info = %d\n", cascade_2nd_header_info);
...
static int32_t nvt_download_firmware_hw_crc(uint8_t full)
{
	...
	while (1) {
		...
		if (cascade_2nd_header_info) {
			...
			if (ret) {
				input_err(true, &ts->client->dev, "spi dma tx info failed. (%d)\n", ret);
				goto fail;
			}
		} else {
			ret = nvt_write_firmware(fw_entry->data, fw_entry->size, full);
