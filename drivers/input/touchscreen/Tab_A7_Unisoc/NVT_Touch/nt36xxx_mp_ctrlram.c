...
/*Tab A8 code for SR-AX6300-01-373 by luxinjun at 2021/09/10 start*/
#define SHORT_TEST_CSV_FILE "/data/tpdata/ShortTest_"
#define OPEN_TEST_CSV_FILE "/data/tpdata/OpenTest_"
#define FW_RAWDATA_CSV_FILE "/data/tpdata/FWMutualTest_"
#define FW_CC_CSV_FILE "/data/tpdata/FWCCTest_"
#define NOISE_TEST_CSV_FILE "/data/tpdata/NoiseTest_"
...
static char *get_date_time_str(void)
{
	...
	/*Tab A7 Lite T618 code for AX6189DEV-414 by suyurui at 2022/1/12 start*/
	snprintf(time_data_buf, sizeof(time_data_buf), "_%04d%02d%02d-%02d%02d%02d",
		(rtc_now_time.tm_year + 1900), rtc_now_time.tm_mon + 1,
		rtc_now_time.tm_mday, rtc_now_time.tm_hour, rtc_now_time.tm_min,
		rtc_now_time.tm_sec);
	/*Tab A7 Lite T618 code for AX6189DEV-414 by suyurui at 2022/1/12 end*/
...
static void nvt_print_criteria(void)
{
	...
	nvt_print_lmt_array(PS_Config_Lmt_FDM_Rawdata_P, FDM_X_Channel, Y_Channel);
	printk("PS_Config_Lmt_FDM_Rawdata_N:\n");
	nvt_print_lmt_array(PS_Config_Lmt_FDM_Rawdata_N, FDM_X_Channel, Y_Channel);
	//---PS_Config_Lmt_FDM_Diff---
	printk("PS_Config_Lmt_FDM_Diff_P:\n");
	nvt_print_lmt_array(PS_Config_Lmt_FDM_Diff_P, FDM_X_Channel, Y_Channel);
	printk("PS_Config_Lmt_FDM_Diff_N:\n");
	nvt_print_lmt_array(PS_Config_Lmt_FDM_Diff_N, FDM_X_Channel, Y_Channel);
...
static int32_t nvt_selftest_for_hq(void)
{
	...
	/*Tab A8 code for SR-AX6300-01-373 by luxinjun at 2021/09/10 start*/
	csv_file_path = kzalloc(64 * sizeof(char), GFP_KERNEL);
	/*Tab A8 code for SR-AX6300-01-373 by luxinjun at 2021/09/10 end*/
	...
	/*Tab A8 code for SR-AX6300-01-373 by luxinjun at 2021/09/10 start*/
	memset(csv_file_path,0,64 * sizeof(char));
	/*Tab A7 Lite T618 code for AX6189DEV-414 by suyurui at 2022/1/12 start*/
	sprintf(csv_file_path, "%s%s%s%s.csv", FW_RAWDATA_CSV_FILE, ts->platdata->md_name, get_date_time_str(), TestResult_FWMutual == 0 ? "_PASS" : "_FAIL");
	/*Tab A7 Lite T618 code for AX6189DEV-414 by suyurui at 2022/1/12 end*/
	input_info(true, &ts->client->dev, "[sec_input] %s:%s\n", __func__, csv_file_path);
	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(RawData_FWMutual, X_Channel, Y_Channel, csv_file_path, 0) < 0) {
		input_err(true, &ts->client->dev, "%s: save rawdata to CSV file failed\n", __func__);
		return -EAGAIN;
	}
	/*Tab A8 code for SR-AX6300-01-373 by luxinjun at 2021/09/10 end*/
	...
	memset(csv_file_path,0,64 * sizeof(char));
	/*Tab A7 Lite T618 code for AX6189DEV-414 by suyurui at 2022/1/12 start*/
	sprintf(csv_file_path, "%s%s%s%s.csv", FW_CC_CSV_FILE, ts->platdata->md_name, get_date_time_str(), TestResult_FW_CC == 0 ? "_PASS" : "_FAIL");
	/*Tab A7 Lite T618 code for AX6189DEV-414 by suyurui at 2022/1/12 end*/
	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(RawData_FW_CC, X_Channel, Y_Channel, csv_file_path, 0) < 0) {
	...
	memset(csv_file_path,0,64 * sizeof(char));
	/*Tab A7 Lite T618 code for AX6189DEV-414 by suyurui at 2022/1/12 start*/
	sprintf(csv_file_path, "%s%s%s%s.csv", NOISE_TEST_CSV_FILE, ts->platdata->md_name, get_date_time_str(), TestResult_Noise == 0 ? "_PASS" : "_FAIL");
	/*Tab A7 Lite T618 code for AX6189DEV-414 by suyurui at 2022/1/12 end*/
	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(RawData_Diff_Max, X_Channel, Y_Channel, csv_file_path, 0) < 0) {
		input_err(true, &ts->client->dev, "save rawdata to CSV file failed\n");
		return -EAGAIN;
	}

	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(RawData_Diff_Min, X_Channel, Y_Channel, csv_file_path, rawdata_diff_min_offset_noise) < 0) {
		input_err(true, &ts->client->dev, "%s: save rawdata to CSV file failed\n", __func__);
		return -EAGAIN;
	}
	/*Tab A8 code for SR-AX6300-01-373 by luxinjun at 2021/09/10 end*/
	...
	memset(csv_file_path,0,64 * sizeof(char));
	/*Tab A7 Lite T618 code for AX6189DEV-414 by suyurui at 2022/1/12 start*/
	sprintf(csv_file_path, "%s%s%s%s.csv", SHORT_TEST_CSV_FILE, ts->platdata->md_name, get_date_time_str(), TestResult_Short == 0 ? "_PASS" : "_FAIL");
	/*Tab A7 Lite T618 code for AX6189DEV-414 by suyurui at 2022/1/12 end*/

	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(RawData_Short, X_Channel, Y_Channel, csv_file_path, 0) < 0) {
	...
	memset(csv_file_path,0,64 * sizeof(char));
	/*Tab A7 Lite T618 code for AX6189DEV-414 by suyurui at 2022/1/12 start*/
	sprintf(csv_file_path, "%s%s%s%s.csv", OPEN_TEST_CSV_FILE, ts->platdata->md_name, get_date_time_str(), TestResult_Open == 0 ? "_PASS" : "_FAIL");
	/*Tab A7 Lite T618 code for AX6189DEV-414 by suyurui at 2022/1/12 end*/
	// Save RawData to CSV file
	if (nvt_save_rawdata_to_csv(RawData_Open, X_Channel, Y_Channel, csv_file_path, 0) < 0) {
		input_err(true, &ts->client->dev, "%s: save rawdata to CSV file failed\n", __func__);
		return -EAGAIN;
	}
	/*Tab A8 code for SR-AX6300-01-373 by luxinjun at 2021/09/10 end*/
