...
static ssize_t nvt_proc_getinfo_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	char buf[150] = {0};
	int rc = 0;
	/*Tab A7 T618 code for SR-AX6189A-01-250 by suyurui at 2021/12/28 start*/
	snprintf(buf, 150, "IC_module:%s; Touch_FW_ver: 0x%X\n",ts->platdata->md_name,ts->fw_ver);
	/*Tab A7 T618 code for SR-AX6189A-01-250 by suyurui at 2021/12/28 end*/
...
*/
	...
	/*Tab A7 Lite T618 code for P220118-05996 by suyurui at 2022/1/27 start*/
	if (ts->tp_is_enabled) {
		input_report_key(ts->input_dev, BTN_TOUCH, (finger_cnt > 0));
	} else {
		input_info(true, &ts->client->dev, "%s : tp_is_enabled:%d\n", __func__, ts->tp_is_enabled);
	}
	/*Tab A7 Lite T618 code for P220118-05996 by suyurui at 2022/1/27 end*/
#else /* MT_PROTOCOL_B */
	if (finger_cnt == 0) {
/*Tab A8 code for AX6300DEV-1956 by huangzhongjie at 2021/11/04 start*/
#if NVT_TOUCH_ESD_PROTECT
...
nvt_upgrade_module nvt_module_list[] = {
	...
	{
		.lcd_name = "lcd_nt36523b_txd_mdt_mipi_hdp",
		.fw_num = MODEL_TXD_MDT,
	},
	{
		.lcd_name = "lcd_nt36523b_ls_hsd_mipi_hdp",
		.fw_num = MODEL_LS_HSD,
	},
...
static void nvt_update_module_info(void)
{
	...
	/*Tab A7 Lite T618 code for SR-AX6189A-01-148|SR-AX6189A-01-847 by suyurui at 2022/1/13 start*/
	case MODEL_TXD_MDT:
		strcpy(ts->platdata->md_name,"nt36523b_txd_mdt");
		strcpy(ts->platdata->firmware_name, "nt36523b_txd_mdt_fw.bin");
		strcpy(ts->platdata->firmware_name_mp, "nt36523b_txd_mdt_mp.bin");
		break;
	case MODEL_LS_HSD:
		strcpy(ts->platdata->md_name,"nt36523b_ls_hsd");
		strcpy(ts->platdata->firmware_name, "nt36523b_ls_hsd_fw.bin");
		strcpy(ts->platdata->firmware_name_mp, "nt36523b_ls_hsd_mp.bin");
		break;
	/*Tab A7 Lite T618 code for SR-AX6189A-01-148|SR-AX6189A-01-847 by suyurui at 2022/1/13 end*/
	default:
...
static struct of_device_id nvt_match_table[] = {
	{ .compatible = "novatek,NVT-ts-spi-ot9",},
	{ },
};
#endif
