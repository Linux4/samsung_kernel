...
hx_upgrade_module module_list[] = {
	...
	{
		.lcd_name = "lcd_hx83102e_hlt_hsd_mipi_hdp",
		.fw_num = MODEL_HLT_HSD,
	},
	{
		.lcd_name = "lcd_hx83102e_gx_hsd_mipi_hdp",
		.fw_num = MODEL_GX_HSD,
	},
	{
		.lcd_name = "lcd_hx83102e_hy_mdt_mipi_hdp",
		.fw_num = MODEL_HY_MDT,
	},
	/*Tab A7 Lite T618 code for AX6189DEV-20|AX6189DEV-26|SR-AX6189A-01-840 by suyurui at 2022/1/11 end*/
};
...
static void himax_update_module_info(void)
{
	...
	case MODEL_HLT_HSD:
            ...
            strcpy(ts->himax_nomalfw_rq_name, "hx83102e_hlt_hsd_firmware.bin");
            strcpy(ts->himax_mpfw_rq_name, "hx83102e_hlt_hsd_mpfw.bin");
            strcpy(ts->himax_csv_name, "hx_hlt_hsd_criteria.csv");
#if defined(FW_GET_FROM_I_FILE)
            ts->firmware_filp_open.data = CTPM_FW_HLT_HSD;
            ts->firmware_filp_open.size = sizeof(CTPM_FW_HLT_HSD);
#endif
            break;
	case MODEL_GX_HSD:
...
#if defined(FW_GET_FROM_I_FILE)
            ts->firmware_filp_open.data = CTPM_FW_GX_HSD;
            ts->firmware_filp_open.size = sizeof(CTPM_FW_GX_HSD);
#endif
            break;
	case MODEL_HY_MDT:
            strcpy(ts->himax_name, "hx83102e_hy_mdt");
            strcpy(ts->himax_nomalfw_rq_name, "hx83102e_hy_mdt_firmware.bin");
            strcpy(ts->himax_mpfw_rq_name, "hx83102e_hy_mdt_mpfw.bin");
            strcpy(ts->himax_csv_name, "hx_hy_mdt_criteria.csv");
#if defined(FW_GET_FROM_I_FILE)
            ts->firmware_filp_open.data = CTPM_FW_HY_MDT;
            ts->firmware_filp_open.size = sizeof(CTPM_FW_HY_MDT);
#endif
            break;
	/*Tab A7 Lite T618 code for AX6189DEV-20|AX6189DEV-26|SR-AX6189A-01-840 by suyurui at 2022/1/11 end*/
    default:
