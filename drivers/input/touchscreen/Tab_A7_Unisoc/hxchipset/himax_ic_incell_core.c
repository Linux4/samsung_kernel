...
/*Tab A8 code for AX6300DEV-3406 by chenpengbo at 2021/11/24 start*/
static int himax_mcu_assign_sorting_mode(uint8_t *tmp_data)
{
	...
	do {
		g_core_fp.fp_register_write(pfw_op->addr_sorting_mode_en, tmp_data, DATA_LEN_4);
		usleep_range(1000, 1100);
		g_core_fp.fp_register_read(pfw_op->addr_sorting_mode_en, data, DATA_LEN_4);
		retry_cnt++;
	} while ((data[1] != tmp_data[1] || data[0] != tmp_data[0])
		&& retry_cnt < HIMAX_REG_RETRY_TIMES);

	if ( retry_cnt >= HIMAX_REG_RETRY_TIMES) {
		E ("%s:addr: 0x%02X%02X%02X%02X, write Fail，and the data of read :0x%02X%02X%02X%02X\n ",
		__func__,
		pfw_op->addr_sorting_mode_en[3],
		pfw_op->addr_sorting_mode_en[2],
		pfw_op->addr_sorting_mode_en[1],
...
/*Tab A8 code for AX6300DEV-3406 by chenpengbo at 2021/11/24 end*/
