...
/*Tab A8 code for SR-AX6300-01-126 by chenpengbo at 2021/09/10 start*/
...
/*Tab A8 code for SR-AX6300-01-267 by chenpengbo at 2021/08/18 start*/
#define HWINFO_NAME "tp_wake_switch"
static struct platform_device hwinfo_device = {
	.name = HWINFO_NAME,
	.id = -1,
};
/*Tab A8 code for SR-AX6300-01-267 by chenpengbo at 2021/08/18 end*/
...
/*Tab A8 code for AX6300DEV-1125 by gaozhengwei at 20211003 start*/
#if defined(FW_GET_FROM_I_FILE)
struct firmware *hx_fw_entry = NULL;
#endif
/*Tab A8 code for AX6300DEV-1125 by gaozhengwei at 20211003 end*/
...
{
    private_ts->tp_is_enabled = 1;
        return 0;
}

static void himax_input_close(struct input_dev *dev)
{
    private_ts->tp_is_enabled = 0;
}
...
#if defined(FW_GET_FROM_I_FILE)
	if (hx_fw_entry == NULL) {
		hx_fw_entry = kzalloc(sizeof(struct firmware), GFP_KERNEL);
		if (hx_fw_entry == NULL) {
...
/*Tab A8 code for AX6300DEV-1125 by gaozhengwei at 20211003 start*/
#elif defined(FW_GET_FROM_I_FILE)
		if (private_ts->firmware_filp_open.data != NULL) {
			hx_fw_entry->data = private_ts->firmware_filp_open.data;
			hx_fw_entry->size = private_ts->firmware_filp_open.size;
			hxfw = hx_fw_entry;
			I("%s: Not find FW in userspace, use i file",__func__);
			result = HX_EMBEDDED_FW;
		} else {
			I("%s: Not find i file",__func__);
...
/*Tab A8 code for P211118-02935  by chenpengbo at 2021/11/19 start*/
...
/*Tab A8 code for P211118-02935  by chenpengbo at 2021/11/19 end*/
...
void himax_cable_detect_func(bool force_renew)
{
	...
	/*#Tab A8 code for SR-AX6300-01-62 by chenpengbo at 2021/08/23 start*/
	USB_detect_flag = sprd_touch_get_usb_status();
	/*#Tab A8 code for SR-AX6300-01-62 by chenpengbo at 2021/08/23 end*/
...
/*Tab A8 code for AX6300DEV-805 by suyurui at 2021/10/13 start*/
#if defined(SEC_PALM_FUNC)
...
skip_stylus_operation:
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		...
		&& y <= ts->pdata->abs_y_max) {
			...
			g_target_report_data->p[loop_i].x = x;
			g_target_report_data->p[loop_i].y = y;
			g_target_report_data->p[loop_i].w = w;
...
#endif
			g_target_report_data->p[loop_i].id = 1;
...
/*Tab A8 code for AX6300DEV-871 by suyurui at 2021/10/8 start*/
/*
#ifdef SEC_PALM_FUNC
...
#endif
*/
/*Tab A8 code for AX6300DEV-871 by suyurui at 2021/10/8 end*/
			...
			g_target_report_data->min[loop_i] = min;
#endif
			g_target_report_data->p[loop_i].id = 0;
	...
	if (g_ts_dbg != 0) {
		for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++)
			D("DBG X=%d  Y=%d ID=%d\n",
				g_target_report_data->p[loop_i].x,
				g_target_report_data->p[loop_i].y,
				g_target_report_data->p[loop_i].id);
...
void himax_ts_work(struct himax_ts_data *ts)
{
	...
	/*Tab A8 code for AX6300DEV-2726 by chenpengbo at 2021/11/04 start*/
	if (!ts->initialized) {
		I("probe not finished\n");
		return;
	}
	/*Tab A8 code for AX6300DEV-2726 by chenpengbo at 2021/11/04 end*/
	if (debug_data != NULL) {
...
/*Tab A8 code for SR-AX6300-01-126 by chenpengbo at 2021/09/10 start*/
static int himax_drm_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	I("event = %d,private_ts->suspended=%d", event, private_ts->suspended);

        if ((event == DISPC_POWER_OFF) && !private_ts->suspended)
                himax_chip_common_suspend(private_ts);
/*Tab A8 code for AX6300DEV-805 by suyurui at 2021/10/12 start*/
#if !defined(HX_RESUME_BY_DDI)
        else if ((event == DISPC_POWER_ON) && private_ts->suspended)
                himax_chip_common_resume(private_ts);
#endif
/*Tab A8 code for AX6300DEV-805 by suyurui at 2021/10/12 end*/
	return 0;
}
/*Tab A8 code for SR-AX6300-01-126 by chenpengbo at 2021/09/10 end*/

/*Tab A8 code for SR-AX6300-01-267 by chenpengbo at 2021/08/18 start*/
static ssize_t himax_ts_factory_check_show(struct device  *dev,
...
static DEVICE_ATTR(factory_check, 0644, himax_ts_factory_check_show, NULL);
...
/*Tab A8 code for AX6300DEV-1218 by hehaoran at 20211003 start*/
/**
*support_feature
*Author：hehaoran
*Date：2021/10/03
...
static ssize_t hx_read_support_feature(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	u32 feature = 0;
...
static DEVICE_ATTR(support_feature, 0444, hx_read_support_feature, NULL);
...
static int hx_sec_fn_init(struct himax_ts_data *ts)
{
	int ret;

	ret = sec_cmd_init(&ts->sec, himax_commands, himax_get_array_size(), SEC_CLASS_DEVT_TSP);
	if (ret < 0) {
		E("%s: Failed to sec_cmd_init\n", __func__);
		return ret;
	} else {
		I("%s: sec_cmd_init success!\n", __func__);
	}

	ret = sysfs_create_group(&ts->sec.fac_dev->kobj, &hx_cmd_attr_group);
	if (ret) {
	...
	if (ret < 0) {
		E("%s: Failed to sysfs_create_link\n", __func__);
		goto out;
	}
	I("%s: sysfs_create_link success!\n", __func__);
	return 0;
out:
...
static void hx_init_sec_failed(struct himax_ts_data *ts)
{
	sysfs_delete_link(&ts->sec.fac_dev->kobj, &ts->input_dev->dev.kobj, "input");
	sysfs_remove_group(&ts->sec.fac_dev->kobj, &hx_cmd_attr_group);
	sec_cmd_exit(&ts->sec, SEC_CLASS_DEVT_TSP);
}
/*Tab A8 code for AX6300DEV-1218 by hehaoran at 20211003 end*/
...
int himax_chip_common_init(void)
{
	...
	queue_delayed_work(ts->himax_boot_upgrade_wq, &ts->work_boot_upgrade,
			/*Tab A8 code for AX6300DEV-1125 by suyurui at 2021/09/30 start*/
			msecs_to_jiffies(400));
			/*Tab A8 code for AX6300DEV-1125 by suyurui at 2021/09/30 end*/
...
/*Tab A8 code for SR-AX6300-01-126 by chenpengbo at 2021/09/10 start*/
	...
	ts->SMWP_enable = 0;
#endif
	ts->drm_notify.notifier_call = himax_drm_notifier_callback;
...
/*Tab A8 code for AX6300DEV-805 by suyurui at 2021/10/9 end*/
	...
	ret = platform_device_register(&ts->platform_device);
    	if (ret) {
        	E("platform_device_register failed");
        	return -ENODEV;
    	}

    	ret = sysfs_create_group(&ts->platform_device.dev.kobj, &factory_group);
    	if (ret) {
        	E("Add device attr failed %d", ret);
        	return ret;
    	}
/*Tab A8 code for AX6300DEV-1218 by hehaoran at 20211003 start*/
	ret = hx_sec_fn_init(ts);
	if (ret) {
		E("failed to init for factory function");
		goto err_hx_sec_fn_init_failed;
	}
/*Tab A8 code for AX6300DEV-1218 by hehaoran at 20211003 end*/
	/*Tab A8 code for AX6300DEV-805 by suyurui at 2021/10/9 start*/
	tp_is_used = HXCHIPSET;
	/*Tab A8 code for AX6300DEV-805 by suyurui at 2021/10/9 end*/
	return 0;
	/*Tab A8 code for SR-AX6300-01-267 by chenpengbo at 2021/08/18 end*/

/*Tab A8 code for AX6300DEV-1218 by hehaoran at 20211003 start*/
err_hx_sec_fn_init_failed:
	hx_init_sec_failed(ts);
/*Tab A8 code for AX6300DEV-1218 by hehaoran at 20211003 end*/
...
#endif
/*Tab A8 code for AX6300DEV-805 by suyurui at 2021/10/6 start*/
#if defined(HX_CONTAINER_SPEED_UP) || defined(HX_RESUME_BY_DDI)
/*Tab A8 code for AX6300DEV-805 by suyurui at 2021/10/6 end*/
