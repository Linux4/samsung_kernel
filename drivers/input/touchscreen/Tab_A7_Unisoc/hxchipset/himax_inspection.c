...
/*Tab A8 code for SR-AX6300-01-54 by suyurui at 2021/09/06 start*/
static char g_file_name[256];
/*Tab A8 code for SR-AX6300-01-54 by suyurui at 2021/09/06 end*/
...
static uint32_t mpTestFunc(uint8_t checktype, uint32_t datalen)
{
	...
	if (HX_FAC_TEST) {
		...
		if (private_ts->debug_log_level & BIT(4))
			I("%s:start sense on!\n", __func__);
		/*Tab A8 code for AX6300DEV-140 by chenpengbo at 2021/09/14 start*/
		g_core_fp.fp_sense_on(0);
		/*Tab A8 code for AX6300DEV-140 by chenpengbo at 2021/09/14 end*/
		if (private_ts->debug_log_level & BIT(4))
			I("%s:end sense on!\n", __func__);
...
/*Tab A8 code for SR-AX6300-01-54 by chenpengbo at 2021/09/06 start*/
...
/*Tab A8 code for SR-AX6300-01-54 by chenpengbo at 2021/09/06 end*/
...
static int himax_self_test_data_init(void)
{
	...
	/*Tab A8 code for AX6300DEV-871 by suyurui at 2021/09/29 start*/
	err = request_firmware(&file_entry, file_name_1, ts->dev);
	/*Tab A8 code for AX6300DEV-871 by suyurui at 2021/09/29 end*/
	if (err < 0) {
...
static int himax_chip_self_test_for_hq(void)
{
	...
	ret = himax_self_test_data_init();
	 /*Tab A8 code for AX6300DEV-140 by chenpengbo at 2021/09/09 start*/
	if (ret > 0) {
		ret = himax_self_test_data_init();
		if (ret > 0) {
			E("%s: initialize self test failed\n", __func__);
	...
	/*Tab A8 code for AX6300DEV-140 by chenpengbo at 2021/09/09 end*/
...
SELF_TEST_FAIL:
	if (ret) {
		sprintf(g_file_name, "Fail");
	} else {
		sprintf(g_file_name, "OK");
	}
	snprintf(g_file_path, (int)(strlen(HX_RSLT_OUT_PATH)
			+ strlen(g_file_name)+37),
			"%s%s_%s_%s.txt", HX_RSLT_OUT_PATH, ts->himax_name, get_date_time_str(), g_file_name);
