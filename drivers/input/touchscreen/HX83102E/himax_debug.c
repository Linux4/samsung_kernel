			...
			const char __user *buff, size_t len, loff_t *data)
{
	...
	//+bug 747189,fangzhihua.wt,20220412,Function "himax_flash_dump_store" overrun
	if (len > 0)
		str_len = len;
	else
		return -EFAULT;
	...
	for (i = 0; i < 4; i++) {
		if (input_str[i] == NULL) {
			...
			case 2:
				...
				break;
			default:
				addr = hx_parse_hexstr2int(input_str[i]);
				I("addr=%d\n", addr);
				break;
			}
		}
...
int himax_debug_init(void)
{
	...
	snprintf(g_file_path, (int)(strlen(HX_RSLT_OUT_PATH)
			+ strlen(HX_RSLT_OUT_FILE)+strlen(hx_touch_test_time)+1),
