...
static void get_fw_ver_bin(void *device_data)
{
	...
	if (si->ready && si->si_ptrs.samsung_data) {
		struct cyttsp4_touch_firmware *fws = cd->pdata->fw;
		struct cyttsp4_touch_firmware *fw;
		u16 siliconID;

		siliconID = get_unaligned_be16(&si->si_ptrs.cydata->jtag_si_id3);
		if (siliconID < 0x0250)
			fw = &fws[CYTTSP4_SILICON_REV_B];
		else
			fw = &fws[CYTTSP4_SILICON_REV_D];

		sprintf(buff, "%c%c%02X%04X", 
				si->si_ptrs.samsung_data->ic_vendorh, 
				si->si_ptrs.samsung_data->ic_vendorl,
				fw->hw_version,
				fw->fw_version);
	}
...
{
	struct cyttsp4_core_data *ts = (struct cyttsp4_core_data *)device_data;
	struct cyttsp4_sysinfo *si = &ts->sysinfo;
	u16 siliconID;
	char buff[16] = {0};
	...
	siliconID = get_unaligned_be16(&si->si_ptrs.cydata->jtag_si_id3);
	snprintf(buff, sizeof(buff), "%s", siliconID < 0x0250 ? "TMA445-B" : "TMA445-D");
	set_cmd_result(ts, buff, strnlen(buff, sizeof(buff)));
	ts->cmd_state = 2;
	dev_info(ts->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
static void fw_update(void *device_data) //item2
{
	...
	{
		if (cd->cmd_param[0])
			dev_info(cd->dev, "%s: fw update forced\n", __func__);
		ret = loader_func(loader, cd->cmd_param[0]);
	}
	if(ret)
	{
		cd->cmd_state = 4;
...
static int cyttsp4_si_get_test_data(struct cyttsp4_core_data *cd)
{
	...
	dev_vdbg(cd->dev, "%s: SCANNING is %s codel=%02X\n",
...
static int cyttsp4_core_sleep_(struct cyttsp4_core_data *cd)
{
	...
	mutex_lock(&cd->irq_depth_lock);
	dev_err(cd->dev, "%s: ---- disable_irq depth= %d\n", __func__, tsp_irq_depth);	
	if (tsp_irq_depth == 1)
	{
	disable_irq(cd->irq);
		tsp_irq_depth = 0;
	...
	mutex_unlock(&cd->irq_depth_lock);
#if 0
...
error:
	ss = SS_SLEEP_OFF;
	mutex_lock(&cd->irq_depth_lock);
	dev_err(cd->dev, "%s: ++++ enable_irq: depth= %d\n", __func__, tsp_irq_depth );	
	if (tsp_irq_depth == 0)
	{
	enable_irq(cd->irq);
		tsp_irq_depth = 1;
...
static int cyttsp4_core_wake_(struct cyttsp4_core_data *cd)
{
	...
	/* Wait for heartbeat */
	mutex_lock(&cd->irq_depth_lock);
	dev_err(dev, "%s: ++++ enable_irq depth= %d\n", __func__, tsp_irq_depth );
	if (tsp_irq_depth == 0)
	{
	enable_irq(cd->irq);
		tsp_irq_depth = 1;
...
//		cyttsp4_configure_ram(cd);			// KEVI added +
	...
	dev_vdbg(cd->dev, "%s: cyttsp4_exit startup r=%d...\n", __func__, rc);
...
static int cyttsp4_core_shutdown(struct cyttsp4_core *core)
{
	struct device *dev = &core->dev;
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	int rc;
	...
	rc = cyttsp4_core_sleep(cd);
	if (rc < 0) {
		dev_err(dev, "%s: Error on shutdown\n", __func__);
		return -EAGAIN;
	}
...
struct cyttsp4_core_driver cyttsp4_core_driver = {
	...
	.read = cyttsp4_read_,
	.shutdown	= cyttsp4_core_shutdown,
	.driver = {
