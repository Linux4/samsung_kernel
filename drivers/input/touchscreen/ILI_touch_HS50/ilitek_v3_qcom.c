...
#if ILI_USE_ENABLE_NODE
    ret = sysfs_create_link(&ilits->sec_info->sec.fac_dev->kobj,&ilits->input->dev.kobj, "input");
    if (ret < 0) {
...
static int ilitek_plat_notifier_fb(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if ((event != MSM_DRM_EARLY_EVENT_BLANK) && (event != MSM_DRM_EVENT_BLANK)){
		ILI_ERR("do not care this event");
		return NOTIFY_DONE;
	}
#else
...
static int ilitek_plat_probe(void)
{
	...
	strcpy(ilits->hw_info_name, ilits->md_name);
	set_tp_module_name(ilits->hw_info_name);
	#endif
