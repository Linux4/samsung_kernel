...
static bool ts_set_touchmode(u16 value){
	...
	if (value == TOUCH_DND_MODE) {
		...
		if (write_reg(misc_info->client, BT432_DND_N_COUNT,
			...
			dev_err(&misc_info->client->dev, "Failed to set N_COUNT\n");
	}
...
static void get_tkey_delta(void *device_data)
{
	...
	ret = read_data(misc_info->client, BT432_BTN_WIDTH + btn_node, (u8*)&val, 2);
		...
		const struct i2c_device_id *i2c_id)
{
	...
	if (ret < 0) {
		pr_err("bt432: Failed to obtain gpio for int\n");
		goto err_power_sequence;
	}
	gpio_direction_input(pdata->gpio_int);
	pr_info("bt432: initialize pins\n");
...
/*	input_set_abs_params(info->input_dev, ABS_MT_WIDTH_MINOR,
	...
	ret = request_threaded_irq(info->irq, NULL, bt432_touch_irq_handler,
		IRQF_TRIGGER_FALLING | IRQF_ONESHOT , info->input_dev->name, info);
...
static int __devinit bt432_ts_init(void)
{
	printk("[TSP]%s\n",__func__);
#if defined(CONFIG_SPA) || defined(CONFIG_SPA_LPM_MODE)
