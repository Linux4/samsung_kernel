 ...
 *
 *
 * mip4_fw_mms600.c : Firmware update functions for MMS600
 *
 * Version : 2017.11.25
 */
...
int mip4_ts_flash_fw(struct mip4_ts_info *info, const u8 *fw_data, size_t fw_size, bool force, bool section)
{
	...
	ret = isc_enter(info);
	if (ret != 0) {
		dev_err(&client->dev,"%s [ERROR] isc_enter\n", __func__);
		ret = FW_ERR_DOWNLOAD;
		goto ERROR_UPDATE;
	}
...
#if FW_UPDATE_DEBUG
		print_hex_dump(KERN_ERR, MIP4_TS_DEVICE_NAME " F/W File : ", DUMP_PREFIX_OFFSET, 16, 1, &bin_data[offset], ISC_PAGE_SIZE, false);
