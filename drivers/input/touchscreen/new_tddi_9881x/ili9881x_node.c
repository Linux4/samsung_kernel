...
static ssize_t ilitek_proc_get_customer_info(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	ILI_INFO(" %s%s\n",  ilits->fw_customer_info, ilits->fw_customer_info+27);

	size = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "%s%s\n",  ilits->fw_customer_info, ilits->fw_customer_info+27);
...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	}  else if (strncmp(cmd, "getcustomerinfo", strlen(cmd)) == 0) {
		ILI_INFO("%s%s\n", ilits->fw_customer_info, ilits->fw_customer_info+27);
		memset(g_user_buf, 0, USER_STR_BUFF * sizeof(unsigned char));
		len = snprintf(g_user_buf, PAGE_SIZE, "%s%s\n", ilits->fw_customer_info, ilits->fw_customer_info+27);
		if (copy_to_user((u8 *)buff, g_user_buf, len))
			ILI_ERR("Failed to copy data to user space\n");
	}
