...
#else
	ilits->tp_int = tpd_dts_data.eint_gpio_num;//MTK_INT_GPIO;
	ilits->tp_rst = tpd_dts_data.rst_gpio_num;//MTK_RST_GPIO;
#endif
...
#if 1
		ilits->irq_gpio = tpd_dts_data.eint_gpio_num;
		ilits->irq_num = gpio_to_irq(ilits->irq_gpio);

		pr_err("%s irq=%d, irq->gpio=%d\n", __func__,
					ilits->irq_num, ilits->irq_gpio);
					
		ret = gpio_request(ilits->irq_gpio, "ili_tsp_irq");
		if (ret) {
			dev_err(ilits->dev, "%s: failed requesting gpio %d\n",
					__func__, ilits->irq_gpio);
			return ret;
		}
		gpio_direction_input(ilits->irq_gpio);
		gpio_free(ilits->irq_gpio);
#else
