...
static void ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	fw_customer_info_addr = fbi[MP].end - INFO_CUSTOMER_INFO_HEX_ADDR;
	ILI_INFO("Parsing hex customer info addr = 0x%x\n", fw_customer_info_addr);
	ipio_memcpy(ilits->fw_customer_info, pfw + fw_customer_info_addr, sizeof(ilits->fw_customer_info), sizeof(ilits->fw_customer_info));
