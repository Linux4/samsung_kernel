...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    snprintf(filepath, FILE_NAME_LENGTH, "%s%s", FTS_INI_OUT_FILE_PATH, file_name);//guoyan1
    FTS_INFO("save test data to %s", filepath);

    if (NULL == pfile) {
        // pfile = filp_open(filepath, O_TRUNC | O_CREAT | O_RDWR, 0);
        pfile = filp_open(filepath, O_RDWR | O_CREAT, 0644);//guoyan1 for openshort data save
    }
    if (IS_ERR(pfile)) {
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    //                  FTS_INI_FILE_PATH);
    size += snprintf(buf + size, PAGE_SIZE, "FTS_INI_OUT_FILE_PATH:%s\n",
                     FTS_INI_OUT_FILE_PATH);//guoyan1 0105
...
ssize_t fts_test_store_for_tip(void)
{
    int ret = 0;
    struct fts_ts_data *ts_data = fts_data;
    struct input_dev *input_dev;
    ...
    if (ret < 0) {
        ...
        fts_test_entry(MP_UPDATE_FIRMWARE_NAME);
        //fts_test_entry("focal_ts_mp_lide_hsd.ini");
    }
    ret = fts_enter_test_environment(0);
    if (ret < 0) {
        FTS_ERROR("enter normal environment fail");
