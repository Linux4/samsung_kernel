...
//+bug621390, sheqihao.wt, add, 20210120,  CID163807 fix function  fts_test_read_reg no retuen value
    ret = fts_test_read_reg(REG_FW_VERSION, &val);
    if(ret < 0){
            FTS_TEST_ERROR("read REG_FW_VERSION register fail");
            return ret;
    }
    ...
    if (IC_HW_INCELL == tdata->func->hwtype) {
        ...
        if(ret < 0){
            FTS_TEST_ERROR("read REG_VA_TOUCH_THR register fail");
            return ret;
    }
        tdata->va_touch_thr = val;
        fts_test_read_reg(REG_VKEY_TOUCH_THR, &val);
        if(ret < 0){
            FTS_TEST_ERROR("read REG_VKEY_TOUCH_THR register fail");
            return ret;
    }
    ...
    if (IC_HW_MC_SC == tdata->func->hwtype) {
        ...
        if(ret < 0){
            FTS_TEST_ERROR("read FACTORY_REG_PATTERN register fail");
            return ret;
    }
        tdata->v3_pattern = (1 == val) ? true : false;
        fts_test_read_reg(FACTORY_REG_NOMAPPING, &val);
        if(ret < 0){
            FTS_TEST_ERROR("read FACTORY_REG_NOMAPPING register fail");
            return ret;
    }
    //-bug621390, sheqihao.wt, add, 20210120,  CID163807 fix function  fts_test_read_reg no retuen value
        tdata->mapping = val;
...
static int fts_test_start(void)
{
    ...
    if ((tdata->func != NULL) && (tdata->func->start_test != NULL)) {
