...
#if 0//guoyan1 defined(CONFIG_FB)
#include <linux/notifier.h>
#include <linux/fb.h>
#elif defined(CONFIG_DRM)
#if 1 // guoyan1 defined(CONFIG_DRM_PANEL)
                         ...
                         u16 type, u8 id_h, u8 id_l, bool force)
{
 ...
 /* guoyan1 */
#ifdef CONFIG_TOUCHSCREEN_TID_OPENSHORT_TEST    /* guoyan1 */
extern ssize_t fts_test_store_for_tip(void);
static int tid_open_short_test(struct device *dev, struct seq_file *seq,
                   const struct firmware *fw)
{
	return fts_test_store_for_tip();
}
#endif
...
#ifdef SEC_FTS_TSP_FACTORY_TEST
static void get_fw_ver(void *device_data)
{
	...
	sec_ret = fts_read_reg(FTS_REG_FW_VER, &sec_ver);
...
struct panel_info{
	uint8_t panel_id;
	char * panel_dsc;
} fts_panel_info[] = {{0,"NULL"}, {1,"lide_hsd"}, {2,"tianma_tianma"}};
//+bug616968, guoyan1.wt, add, 2021/03/09, TP ft8201ab tianma_tianma bringup
//-bug616968, guoyan1.wt, add, 2021/03/09, TP ft8201ab tianma_tianma bringup

#define NO_PANEL 0
uint8_t fts_get_panel_info(void)
{
	...
	char *buf = saved_command_line;
 	...
 	panel_num = sizeof(fts_panel_info) /sizeof(struct panel_info);

	for( loop_i = 0; loop_i < panel_num; loop_i++ ){
		pos = strstr(buf, fts_panel_info[loop_i].panel_dsc);
		if( NULL != pos ){
			panel_found = true;
			FTS_INFO("panel id : %d,%s panel exist.\n",
				fts_panel_info[loop_i].panel_id, fts_panel_info[loop_i].panel_dsc);
			break;
		}
	}

	if (panel_found)
		return fts_panel_info[loop_i].panel_id;
	else{
		FTS_ERROR("Panel info can't match,pls check and retry.\n");
		return NO_PANEL;
	}
...
#define MP_SUFFIX ".ini"
char fts_fw_name[64] = "focal_ts_fw_";
char fts_mp_name[64] = "focal_ts_mp_";
static bool fts_assigned_fw_name(void)
{
	uint8_t panel_id = 0;

	panel_id = fts_get_panel_info();

	if(panel_id != NO_PANEL) {
		strncat(fts_fw_name, fts_panel_info[panel_id].panel_dsc,
				strlen(fts_panel_info[panel_id].panel_dsc));
		strncat(fts_fw_name, FW_SUFFIX,strlen(FW_SUFFIX));
		FTS_INFO("fw name is %s.\n",fts_fw_name);
		strncat(fts_mp_name, fts_panel_info[panel_id].panel_dsc,
				strlen(fts_panel_info[panel_id].panel_dsc));
		strncat(fts_mp_name, MP_SUFFIX,strlen(MP_SUFFIX));
		FTS_INFO("mp fw name is %s.\n",fts_mp_name);
		return 1;
	}else{
		FTS_ERROR("fw name dose not assign, pls check panel info.\n");
	}
	return 0;
}
#endif
...
//+bug614711, guoyan1.wt, add, 2021/01/03, TP ft8012ab lide gesture patch
//guoyan1
...
static int drm_notifier_callback(struct notifier_block *self,
                                 unsigned long event, void *data)
{
    //struct msm_drm_notifier *evdata = data;//guoyan1
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    struct touch_info_dev_operations *tid_ops;  /* guoyan1 */
    int pdata_size = sizeof(struct fts_ts_platform_data);
    struct device_node *dp = ts_data->spi->dev.of_node;     //guoyan1
...
/* guoyan1 */
        ...
        if (check_dt(dp)) {
		if (!check_default_tp(dp, "qcom,spi-touch3-active"))
			ret = -EPROBE_DEFER;
		else
			ret = -ENODEV;
...
//-bug614711, guoyan1.wt, add, 2021/01/03, TP ft8012ab lide gesture patch
    ...
    //guoyan1 0105
    tid = devm_tid_and_ops_allocate(ts_data->dev);
    if (unlikely(!tid)) {
	ret = -ENOMEM;
        goto ts_data_alloc_failed;
    }

#if defined(FTS_VENDOR_UPGRADE)
	if (!fts_assigned_fw_name()) {
		memcpy(fts_fw_name, "focal_ts_fw.bin", sizeof("focal_ts_fw.bin"));
		memcpy(fts_mp_name, "focal_ts_mp.ini", sizeof("focal_ts_mp.ini"));
	}
#endif
	...
	tid->panel_maker = fts_panel_info[fts_get_panel_info()].panel_dsc;
...
//guoyan1 0105
    ret = devm_tid_register(ts_data->dev, tid);
    if (unlikely(ret)){
...
//module_init(fts_ts_init);     //guoyan1 for check dt twice 20210111
//late_initcall(fts_ts_init);   //guoyan1 for check dt twice follow himax & nvt 20210119
