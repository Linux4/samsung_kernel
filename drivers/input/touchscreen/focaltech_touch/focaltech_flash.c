...
struct upgrade_func *upgrade_func_list[] = {
	...
	&upgrade_func_ft3617_003,/*OA6859903, liuchunyang.wt, add, 20201021, add FT3617-003 TP IC for N8*/
};
...
static bool fts_fwupg_need_upgrade(struct fts_upgrade *upg)
{
    ...
    upg->phone_fw_version = (unsigned int)fw_ver_in_host;
        ...
        if (fw_ver_in_tp < fw_ver_in_host) { //Bug601075, shenwenbin.wt, modify, 20201230 mv N8_Q code to N8_R
            return true;
        }
    } else {
 ...
 *  Return: return 0 if success, otherwise return error code
 *****************************************************************************/
static int fts_fwupg_get_fw_file(struct fts_upgrade *upg, const struct firmware *fw)
{
    int ret = 0;
    ...
    if (FTS_FW_REQUEST_SUPPORT) {
        ret = fts_get_fw_file_via_request_firmware(upg, fw);
        if (ret != 0) {
            get_fw_i_flag = true;
        }
    } else {
...
static int tid_product_selector(struct device *dev)
{
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
    struct fts_upgrade *upg = fwupgrade;
	bool fwvalid = false;//OA6859903, liuchunyang.wt, add, 20201021, add FT3617-003 TP IC for N8
    int ret = 0;
        u8 chip_id[2] = { 0 };
		
//+OA6859903, liuchunyang.wt, add, 20201021, add FT3617-003 TP IC for N8
	fts_irq_disable();
#if FTS_ESDCHECK_EN
	fts_esdcheck_switch(0);
#endif
	
	mdelay(200);
	fwvalid = fts_fwupg_check_fw_valid();
	if(!fwvalid)
    	FTS_INFO("FTS:=====fw_valid fail");
//-OA6859903, liuchunyang.wt, add, 20201021, add FT3617-003 TP IC for N8
    ret = fts_fwupg_reset_to_boot(upg);
    if (ret < 0) {
        FTS_ERROR("enter into romboot/bootloader fail");
...
//+OA6859903, liuchunyang.wt, add, 20201021, add FT3617-003 TP IC for N8
#if FTS_ESDCHECK_EN
	fts_esdcheck_switch(1);
#endif
	fts_irq_enable();

    FTS_INFO("boot_id0:chip id:%04x", chip_id[0]);
    FTS_INFO("boot_id1:chip id:%04x", chip_id[1]);

    if ((0x5C == chip_id[1])&&(0x54 == chip_id[0]))//FT3528
        return 0;
    else if ((0x2C == chip_id[1])&&(0x58 == chip_id[0]))//FT3617
        return 1;
    else if ((0xB6 == chip_id[1])&&(0x80 == chip_id[0]))//FT8201
        return 2;
    else if ((0x2E == chip_id[1])&&(0x58 == chip_id[0]))//FT3617-003
        return 3;
//-OA6859903, liuchunyang.wt, add, 20201021, add FT3617-003 TP IC for N8
    else
        return ret;
...
static int tid_get_lockdown_info(struct device *dev, char *out_values)
{
    ...
    vid = (int)((module_id << 8) + vendor_id);
        ...
        FTS_INFO("vendor id:%04x", vid);
    //+Bug604581, shenwenbin.wt, add, 20201203, modify TP compatible method
    if (0x60 == vid)
        out_values[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = 0x62;//Bug601075, shenwenbin.wt, modify, 20201230 mv N8_Q code to N8_R
    else if (0x61 == vid)		
        out_values[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = 0x57;
    else if (0x62 == vid)		
        out_values[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = 0x58;
    else if (0x63 == vid)		
        out_values[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = 0x59;
    else if (0x64 == vid)		
        out_values[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = 0x60;
    //-Bug604581, shenwenbin.wt, add, 20201203, modify TP compatible method
    else if (0xf0 == vid  || 0x34 == vid)
        out_values[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = 0x61;//Bug601075, shenwenbin.wt, modify, 20201230 mv N8_Q code to N8_R
    else if (0xda == vid)
        out_values[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = 0x47;
    else {
            FTS_ERROR("vendor id is readed from flash is wrong, the No. is %04x", vid);
            return -100;
        }
               ...
               unsigned int *minor)
{
    ...
    ret = fts_fwupg_get_ver_in_tp(&ver);
    if(ret)
    {
                FTS_ERROR("get firware version error\n");
                return ret;
    }
    *minor = ver;
    *major = 0x1000;
...
static int tid_firmware_upgrade(struct device *dev, const struct firmware *fw,
                bool force)
{
        int ret = 0;
        struct fts_upgrade *upg = fwupgrade;
        ...
        /* get fw */
        ret = fts_fwupg_get_fw_file(upg, fw);
        if (ret < 0) {
            FTS_ERROR("get file fail, can't upgrade");
        } else {
            /* ic init if have */
        ...
        ts_data->tid->phone_fw_version = upg->phone_fw_version;
...
{
    struct touch_info_dev_operations *tid_ops = ts_data->tid->tid_ops;
    int i = 0;
    int j = 0;
    int ic_stype = 0;
    ...
    //if (0 == func_count) {//Condition 0==func_count is always false
    ...
    //if (1 == func_count) {//Condition '1==func_count' is always false
    //    fwupgrade->func = func;
    //} else {
    ...
    tid_ops->get_lockdown_info = tid_get_lockdown_info;//bug601074,sheqihao.wt, modify, 2020.11.18, P81080-R Project tp customized
