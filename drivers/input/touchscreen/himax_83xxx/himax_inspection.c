...
static int g_gap_horizontal_partial = 2;
...
static int hx_findout_limit(uint32_t * RAW, int *limit_val)
{
	int i = 0;
	int tmp = 0;
...
static void himax_osr_ctrl(bool enable)
{
	...
	do {
		...
		if (w_byte != back_byte)
			input_info(true, &private_ts->client->dev, "%s %s: Write osr_ctrl failed, w_byte = %2X, back_byte = %2X\n",
				HIMAX_LOG_TAG, __func__, w_byte, back_byte);
		else {
			input_info(true, &private_ts->client->dev, "%s %s: Write osr_ctrl correctly, w_byte = %2X, back_byte = %2X\n",
				HIMAX_LOG_TAG, __func__, w_byte, back_byte);
			break;
		}
	} while (--retry > 0);
...
FAIL_END:
	if (sec->cmd_state == SEC_CMD_STATUS_OK)
		snprintf(buf, sizeof(buf), "%s:%s", __func__, "OK");
	else
		snprintf(buf, sizeof(buf), "%s:%s", __func__, "NG");
	sec_cmd_set_cmd_result(sec, buf, strnlen(buf, sizeof(buf)));
	himax_int_enable(1);
	input_info(true, &data->client->dev, "%s %s: %s(%d)\n", HIMAX_LOG_TAG,
			__func__, buf, (int)strnlen(buf, sizeof(buf)));
...
static void get_fw_ver_bin(void *dev_data)
{
	...
	if (ret) {
		...
		bin_cfg_ver =
			(firmware->data[CFG_VER_MAJ_FLASH_ADDR] << 8) |
			firmware->data[CFG_VER_MIN_FLASH_ADDR];
		snprintf(buf, sizeof(buf), "HX%04X%04X%04X", bin_fw_ver,
			bin_cid_ver, bin_cfg_ver);
	}
	if (firmware) {
		release_firmware(firmware);
	}
...
static void get_fw_ver_ic(void *dev_data)
{
	...
	snprintf(buf, sizeof(buf), "HX%04X%04X%04X", ic_data->vendor_fw_ver,
...
static void get_rawcap(void *dev_data)
{
	...
	val = hx_findout_limit(RAW, limit_val);

	if (val < 0) {
		E("%s: get rawdata max/min fail!\n", __func__);
		snprintf(buf, sizeof(buf), "%s:get max/min fail", __func__);
		goto END_OUPUT;
	}
	...
	if (g_ts_dbg != 0) {
		for (i = 0; i < ic_data->HX_TX_NUM * ic_data->HX_RX_NUM; i++) {
			if (i % ic_data->HX_RX_NUM == (ic_data->HX_RX_NUM - 1))
				I("%d\n", g_sec_raw_buff->_rawdata[i]);
			else
				I("%d, ", g_sec_raw_buff->_rawdata[i]);
		}
	}
	/* save to flash */
	/* 1. data size*/
...
static void get_open(void *dev_data)
{
	...
	if (val < 0) {
		E("%s: get rawdata max/min fail!\n", __func__);
		snprintf(buf, sizeof(buf), "%s:get max/min fail\n", __func__);
		goto END_OUPUT;
	}
	...
	if (g_ts_dbg != 0) {
		for (i = 0; i < ic_data->HX_TX_NUM * ic_data->HX_RX_NUM; i++) {
			if (i % ic_data->HX_RX_NUM == (ic_data->HX_RX_NUM - 1))
				I("%d\n", g_sec_raw_buff->_open[i]);
			else
				I("%d, ", g_sec_raw_buff->_open[i]);
		}
	}
END_OUPUT:
	if (val >= 0) {
...
static void get_short(void *dev_data)
{
	...
	if (g_ts_dbg != 0) {
		for (i = 0; i < ic_data->HX_TX_NUM * ic_data->HX_RX_NUM; i++) {
			if (i % ic_data->HX_RX_NUM == (ic_data->HX_RX_NUM - 1))
				I("%d\n", g_sec_raw_buff->_short[i]);
			else
				I("%d, ", g_sec_raw_buff->_short[i]);
		}
	}
END_OUPUT:
	if (val >= 0) {
...
static void get_mic_open(void *dev_data)
{
	...
	if (g_ts_dbg != 0) {
		for (i = 0; i < ic_data->HX_TX_NUM * ic_data->HX_RX_NUM; i++) {
			if (i % ic_data->HX_RX_NUM == (ic_data->HX_RX_NUM - 1))
				I("%d\n", g_sec_raw_buff->_mopen[i]);
			else
				I("%d, ", g_sec_raw_buff->_mopen[i]);
		}
	}
END_OUPUT:
	if (val >= 0) {
...
static void get_noise(void *dev_data)
{
	...
	if (g_ts_dbg != 0) {
		for (i = 0; i < ic_data->HX_TX_NUM * ic_data->HX_RX_NUM; i++) {
			if (i % ic_data->HX_RX_NUM == (ic_data->HX_RX_NUM - 1))
				I("%d\n", g_sec_raw_buff->_noise[i]);
			else
				I("%d, ", g_sec_raw_buff->_noise[i]);
		}
	}
END_OUPUT:
	if (val >= 0) {
...
static void get_lp_rawcap(void *dev_data)
{
	...
	if (g_ts_dbg != 0) {
		for (i = 0; i < ic_data->HX_TX_NUM * ic_data->HX_RX_NUM; i++) {
			if (i % ic_data->HX_RX_NUM == (ic_data->HX_RX_NUM - 1))
				I("%d\n", g_sec_raw_buff->_lp_rawdata[i]);
			else
				I("%d, ", g_sec_raw_buff->_lp_rawdata[i]);
		}
	}
END_OUPUT:
	if (val >= 0) {
...
static void get_lp_noise(void *dev_data)
{
	...
	if (g_ts_dbg != 0) {
		for (i = 0; i < ic_data->HX_TX_NUM * ic_data->HX_RX_NUM; i++) {
			if (i % ic_data->HX_RX_NUM == (ic_data->HX_RX_NUM - 1))
				I("%d\n", g_sec_raw_buff->_lp_noise[i]);
			else
				I("%d, ", g_sec_raw_buff->_lp_noise[i]);
		}
	}
END_OUPUT:
	if (val >= 0) {
...
static void get_gap_data_x(void *dev_data)
{
	...
	hx_gap_hor_raw(HIMAX_INSPECTION_GAPTEST_RAW, RAW,
				g_sec_raw_buff->_gap_hor);

	limit_val[0] = -9999;	/* max */
	limit_val[1] = 9999;	/* min */
	val = hx_findout_limit(g_sec_raw_buff->_gap_hor, limit_val);
	if (val < 0) {
		E("%s: get gap_ver max/min fail!\n", __func__);
		snprintf(buf, sizeof(buf), "%s:get max/min fail\n", __func__);
		goto END_OUPUT;
	}
	if (g_ts_dbg != 0) {
		for (i = 0; i < ic_data->HX_TX_NUM * ic_data->HX_RX_NUM; i++) {
			if (i % ic_data->HX_RX_NUM == (ic_data->HX_RX_NUM - 1))
				I("%4d\n", g_sec_raw_buff->_gap_hor[i]);
			else
				I("%4d, ", g_sec_raw_buff->_gap_hor[i]);
		}
	}
END_OUPUT:
	if (val >= 0) {
...
static void get_gap_data_y(void *dev_data)
{
	...
	hx_gap_ver_raw(HIMAX_INSPECTION_GAPTEST_RAW, RAW,
				g_sec_raw_buff->_gap_ver);

	limit_val[0] = -9999;	/* max */
	limit_val[1] = 9999;	/* min */
	val = hx_findout_limit(g_sec_raw_buff->_gap_ver, limit_val);
	if (val < 0) {
		E("%s: get gap_ver max/min fail!\n", __func__);
		snprintf(buf, sizeof(buf), "%s:get max/min fail\n", __func__);
		goto END_OUPUT;
	...
	if (g_ts_dbg != 0) {
		for (i = 0; i < ic_data->HX_TX_NUM * ic_data->HX_RX_NUM; i++) {
			if (i % ic_data->HX_RX_NUM == (ic_data->HX_RX_NUM - 1))
				I("%4d\n", g_sec_raw_buff->_gap_ver[i]);
			else
				I("%4d, ", g_sec_raw_buff->_gap_ver[i]);
		}
	}
END_OUPUT:
	if (val >= 0) {
