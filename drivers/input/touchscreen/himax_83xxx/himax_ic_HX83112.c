...
/* check fw status */
		...
		if (tmp_data[0] != 0x05) {
			input_info(true, &private_ts->client->dev,
					"%s %s: Do not need wait FW, Status = 0x%02X!\n",
...
static void hx83112_dynamic_fw_name(uint8_t ic_name)
{
	...
	if (private_ts->pdata->i_CTPM_firmware_name != NULL) {
		kfree(private_ts->pdata->i_CTPM_firmware_name);
		private_ts->pdata->i_CTPM_firmware_name = NULL;
	}
	...
	if (private_ts->pdata->i_CTPM_firmware_name != NULL) {
		memcpy(private_ts->pdata->i_CTPM_firmware_name, firmware_name,
				(sizeof(char) * (strlen(firmware_name) + 1)));
	}
	input_info(true, &private_ts->client->dev,
			"%s %s: i_CTPM_firmware_name : %s \n", HIMAX_LOG_TAG,
			__func__, private_ts->pdata->i_CTPM_firmware_name);
}
#endif
#endif
	...
	if (g_core_chip_dt == NULL) {
		g_core_chip_dt =
			kzalloc(sizeof(struct himax_chip_detect) *
				HX_DRIVER_MAX_IC_NUM, GFP_KERNEL);
		I("%s:1st alloc g_core_chip_dt \n", __func__);
	}
	if (g_hx_ic_dt_num < HX_DRIVER_MAX_IC_NUM) {
		g_core_chip_dt[g_hx_ic_dt_num].fp_chip_detect =
			hx83112_chip_detect;
		g_hx_ic_dt_num++;
	}
