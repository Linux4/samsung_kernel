	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		if ((((ts->old_finger >> loop_i & 1) == 0)
			...
			input_info(true, &ts->client->dev,
				...
				(ts->input_dev->mt->trkid - 1) & TRKID_MAX, x, y, loc, w, w,
				ts->touch_count);
#else
			input_info(true, &ts->client->dev,
				"%s: p%d.%d loc:%s ma:%d mi:%d tc:%d\n",
				HIMAX_LOG_TAG, loop_i,
				(ts->input_dev->mt->trkid - 1) & TRKID_MAX, loc, w, w,
				ts->touch_count);
#endif
		} else
...
static void himax_finger_report(struct himax_ts_data *ts)
{
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		...
		if (valid) {
			...
			g_target_report_data->major[i] = ic_data->HX_Y_RES / ic_data->HX_RX_NUM *
							hx_touch_data->hx_sp_info_buf[HX_EVENT_STCK_HEADR + i * 2];
			g_target_report_data->minor[i] = ic_data->HX_X_RES / ic_data->HX_TX_NUM *
							hx_touch_data->hx_sp_info_buf[HX_EVENT_STCK_HEADR + i * 2 + 1];
...
void himax_cb_suspend(void)
{
#ifdef CONFIG_BATTERY_SAMSUNG
	if (lpcharge == 1)
		return;
#endif
	if (lcdtype == 0 || hx_bootmode == 2)
		return;
