...
/* HS70 add for SR-ZQL1871-01-172 by zhouzichun at 20191016 start */
struct sec_info *ili_sec_info= NULL;
/* HS70 add for SR-ZQL1871-01-172 by zhouzichun at 20191016 end */
/* HS70 modify for HS70-147 by zhouzichun at 20191010 start */
#define HWINFO_NAME		"tp_wake_switch"
struct platform_device hwinfo_device= {
	.name = HWINFO_NAME,
	.id = -1,
};
static int dev_mkdir(char *name, umode_t mode);
/* HS70 modify for HS70-147 by zhouzichun at 20191010 end */
/* HS70 modify for HS70-147 by zhouzichun at 20191010 start */
#define ilitek_INFO_PROC_FILE "tp_info"
static struct proc_dir_entry *ilitek_info_proc_entry;
...
static ssize_t ilitek_proc_getinfo_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	/* HS70 add for HS70-1717 by zhanghao at 20191129 start */
		snprintf(buf, 150, "IC=ILI7807G module=%s fw_ver=%d.%d.%d.%d \n",idev->module_name,ch[1], ch[2], ch[3], ch[4]);
	}
	else
	{
		snprintf(buf, 150, "IC=ILI7807G module=%s fw_ver=NA\n",idev->module_name);
...
static const struct file_operations ilitek_info_proc_fops = {
	.owner = THIS_MODULE,
...
*******************************************************/
int32_t ilitek_extra_proc_init(void)
{
	ilitek_info_proc_entry = proc_create(ilitek_INFO_PROC_FILE, 0777, NULL, &ilitek_info_proc_fops);
	if (NULL == ilitek_info_proc_entry)
	{
		ipio_err( "Couldn't create proc entry!");
		return -ENOMEM;
	}
	else
	{
		ipio_info( "Create proc entry success!");
	}
	return 0;
}
static ssize_t zh_ili_test_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	int count = 0;
	int ret = 0;
	char apk_ret[100] = {0};
	...
	mutex_unlock(&idev->touch_mutex);
	count = sprintf(buf, "%s\n", (ret < 0) ? "0" : "1");//return test  result
	return count;
}

static ssize_t zh_ili_test_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	return count;
}

static DEVICE_ATTR(factory_check, 0644, zh_ili_test_show, zh_ili_test_store);

static struct attribute *ili_test_attributes[] ={
	&dev_attr_factory_check.attr,
	NULL
};

static struct attribute_group ili_test_attribute_group = {
.attrs = ili_test_attributes
};

int ili_test_node_init(struct platform_device *tpinfo_device)
{
    int err=0;
    err = sysfs_create_group(&tpinfo_device->dev.kobj, &ili_test_attribute_group);
    if (0 != err)
    {
        ipio_err( "ERROR: ITO node create failed.");
        sysfs_remove_group(&tpinfo_device->dev.kobj, &ili_test_attribute_group);
        return -EIO;
    }
    else
    {
        ipio_info("ITO node create_succeeded.");
    }
    return err;
}

int ili_test_node_exit(struct platform_device *tpinfo_device)
{
	sysfs_remove_group(&tpinfo_device->dev.kobj, &ili_test_attribute_group);
	return 0;
}
...
/* HS70 add for SR-ZQL1871-01-172 by zhouzichun at 20191016 start */
	...
	sec_info = (struct sec_info *)kzalloc(sizeof(*sec_info), GFP_KERNEL);
	if (!sec_info) {
		ipio_err("allocate memory for sec_info fail");
		return ;
	}
	ili_sec_info = sec_info;
	sec_info->fw_version = 0;
	sec_info->fw_version_bin = 0;
	sec_info->cfg_version = 0;
/* HS70 add for SR-ZQL1871-01-172 by zhouzichun at 20191016 end */
	...
	platform_device_register(&hwinfo_device);
    	ili_test_node_init(&hwinfo_device);
	ilitek_extra_proc_init();//creat /proc/tp_info
/* HS70 add for SR-ZQL1871-01-172 by zhouzichun at 20191016 start */
	ret = sec_cmd_init(&sec_info->sec, ili_commands, ili_get_array_size(), SEC_CLASS_DEVT_TSP);
    	if (ret < 0) {
        	ipio_err("Failed to sec_cmd_init\n");
    	}
/* HS70 add for SR-ZQL1871-01-172 by zhouzichun at 20191016 end */
}
