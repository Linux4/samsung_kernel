...
/* HS70 add for HS70-993 by gaozhengwei at 2019/11/11 start */
...
#else
	if (request_firmware(&fw, path, idev->dev) < 0) {
		ipio_err("Request ini failed\n");
		goto out;
	}
	...
	ipio_memcpy((u8 *)tmp, fw->data, fsize * sizeof(*fw->data), fsize);
	tmp[fsize] = 0x00;
	release_firmware(fw);
#endif
...
int ilitek_tddi_mp_test_main(char *apk, bool lcm_on, char *single)
{
	...
	/* HS70 add for HS70-1717 by zhanghao at 20191129 start */
	ret = ilitek_tddi_mp_ini_parser(idev->ini_name);
