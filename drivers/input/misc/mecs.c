...
static long ecs_ctrl_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ECOMPASS_IOC_SET_ACC_DATA:
		if (copy_from_user(acc, pa, sizeof(acc)))
			return -EFAULT;

//        printk("memsic set acc data x: %d y: %d z: %d\n", acc[0], acc[1], acc[2]);
        break;
	case ECOMPASS_IOC_GET_ACC_DATA:
		if (copy_to_user(pa, acc, sizeof(acc)))
			return -EFAULT;
//        printk("memsic get acc data x: %d y: %d z: %d\n", acc[0], acc[1], acc[2]);
        break;
