...
#endif
	/*if i2c clk is 400K, not set here*/
	np = np->parent;
        tempvalue = 0;
        of_property_read_u32(np,"clock-frequency", &tempvalue);
        if (400000 != tempvalue) {
                sprd_i2c_ctl_chg_clk(client->adapter->nr, 400000);
                LOG_INFO("set i2c clk 400KHZ by acc\n");
        } else {
                LOG_INFO("No need to set 400KHz,Have set by i2c\n");
        }
        np = np->child;
