...
#include <linux/input/abov_sar_2ch.h> /* main struct, interrupt,init,pointers */
...
static void touchProcess(pabovXX_t this)
{
    ...
    if (this && pDevice) {
        ...
        if (unlikely((buttons == NULL) || (input_bottom == NULL) || (input_top == NULL))) {
...
static CLASS_ATTR(calibrate, 0660, calibrate_show, calibrate_store);
...
static CLASS_ATTR(enable, 0660, enable_show, enable_store);
...
static CLASS_ATTR(reg, 0660, reg_show, reg_store);
...
static int capsensor_set_enable_bottom(struct sensors_classdev *sensors_cdev, unsigned int enable) //ch0
{
    return enable_bottom(enable);
}

static int capsensor_set_enable_top(struct sensors_classdev *sensors_cdev, unsigned int enable) //ch0
{
    return enable_top(enable);
}
#endif
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (val == 0) {
        ...
        if (mEnabled) {
            ...
            input_report_rel(pDevice->pbuttonInformation->input_top, REL_MISC, 2);
            input_sync(pDevice->pbuttonInformation->input_top);
        }
    } else {
...
static int abovxx_grip_wifi_flush(struct sensors_classdev *sensors_cdev,unsigned char val)
{
    ...
    input_report_rel(pDevice->pbuttonInformation->input_top, REL_MAX, val);
    input_sync(pDevice->pbuttonInformation->input_top);
...
static CLASS_ATTR(update_fw, 0660, update_fw_show, update_fw_store);
...
static CLASS_ATTR(force_update_fw, 0660, force_update_fw_show, force_update_fw_store);
//Mtr 2856,20191029,gaojingxuan.wt,add sar-sensor factory test node,begin
static ssize_t reg_dump_show_ch0(struct class *class,
                struct class_attribute *attr,
                char *buf)
{
    u8 reg_value = 0;
    ...
    lsb_ch0 = reg_value;
    reg_result = (msb_ch0 << 8) | lsb_ch0;
    p += snprintf(p, PAGE_SIZE, "CS7_background_cap=%d;", reg_result);
#endif
    //CH0 diff
...
static ssize_t reg_dump_store_ch0(struct class *class,
        struct class_attribute *attr,
        const char *buf, size_t count)
{
    return count;
...
static CLASS_ATTR(reg_ch0, 0660, reg_dump_show_ch0, reg_dump_store_ch0);

static ssize_t reg_dump_show_ch1(struct class *class,
        struct class_attribute *attr,
        char *buf)
{
    u8 reg_value = 0;
...
static ssize_t reg_dump_store_ch1(struct class *class,
        struct class_attribute *attr,
        const char *buf, size_t count)
{
    return count;
...
static CLASS_ATTR(reg_ch1, 0660, reg_dump_show_ch1, reg_dump_store_ch1);
...
{
    pabovXX_t this = abov_sar_ptr;
    int status = -1;//bug 518784,gjx.wt,modify.20191202,initialize the value
    char *p = buf;
...
static int abov_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (this) {
        ...
        if (pDevice) {
            ...
            /* Create the input device */
            input_top = input_allocate_device(); //ch1-->wifi->top
            if (!input_top) {
                ret = -ENOMEM;
                goto err_ch1_alloc;
            }
            /* Set all the keycodes */
            input_set_capability(input_top, EV_REL, REL_MISC);
            input_set_capability(input_top, EV_REL, REL_MAX);
            /* save the input pointer and finish initialization */
            pDevice->pbuttonInformation->input_top = input_top;
            /* save the input pointer and finish initialization */
            input_top->name = "grip_sensor_wifi";
        ...
        sensors_capsensor_top_cdev.sensors_enable = capsensor_set_enable_top;
        sensors_capsensor_top_cdev.sensors_poll_delay = NULL;
        sensors_capsensor_top_cdev.sensors_flush = abovxx_grip_wifi_flush;
        ret = sensors_classdev_register(&input_top->dev, &sensors_capsensor_top_cdev);
        if (ret < 0)
             LOG_DBG("create chi cap sensor_class  file failed (%d)\n", ret);
#endif
//+bug 492320,20191119,gaojingxuan.wt,add,use SS hal
//+bug 518737,20191202,gjx.wt,modify,judge the return value
        ret = device_create_file(&input_bottom->dev, &dev_attr_enable);
        if(ret != 0)
            LOG_DBG("input_bottom device_create_file failed (%d)\n", ret);
        ret = device_create_file(&input_top->dev, &dev_attr_enable);
        if(ret != 0)
            LOG_DBG("input_top device_create_file failed (%d)\n", ret);
//-bug 518737,20191202,gjx.wt,modify,judge the return value
//-bug 492320,20191119,gaojingxuan.wt,add,use SS hal
#if defined(CONFIG_SENSORS)
...
static void abovXX_process_interrupt(pabovXX_t this, u8 nirqlow)
{
    ...
    if (unlikely(this->useIrqTimer && nirqlow)) {
        ...
        LOG_INFO("abovXX Schedule Irq timer");
    }
}
