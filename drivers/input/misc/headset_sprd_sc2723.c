...
/* is_set = 0, gnd_det to AUXADC */
static void set_adc_to_gnd_det(unsigned is_set)
{
    headset_adc_en(1);
    ...
    if (is_set) {
        ...
        headset_reg_set_val(HEADMIC_DETECT_REG(ANA_HDT0), AUDIO_HEAD2ADC_SEL_GND_DET, AUDIO_HEAD2ADC_SEL_MASK, AUDIO_HEAD2ADC_SEL_SHIFT);
    }
}
...
static SPRD_HEADSET_TYPE headset_type_detect(int last_gpio_detect_value)
{
    ...
    int adc_mic_ideal = 0;
    int adc_gnd_average = 0 ;
    int readadctime = 9 ;
    int readleftadctime = 0 ;
...
no_mic_retry:
    ...
    //PRINT_INFO("headmicbias on\n");

    set_adc_to_headmic(1);
    //PRINT_INFO("begin to read adc_mic\n");
...
static void adpgar_byp_select_func(struct work_struct *work)
{
    if ((0x2723 == (sci_get_ana_chip_id() >> 16))
            && (SC2723ES == sci_get_ana_chip_ver() || SC2723TS == sci_get_ana_chip_ver())) {
        return;
    }
