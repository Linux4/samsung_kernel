...
#if defined (SEC_USE_HS_SYSFS)
static struct jack_device_priv headset_priv;
#endif
...
static void headset_button_work_func(struct work_struct *work)
{
    ...
    if(1 == button_state_current) {//pressed!
        ...
        if(0 == button_state_last) {
                ...
                button_state_last = 1;
#if defined (SEC_USE_HS_SYSFS)
                headset_priv.jack.button_det = true;
                headset_priv.jack.adc_val = adc_mic_average;
#endif
        ...
        if(1 == button_state_last) {
                ...
                button_state_last = 0;
#if defined (SEC_USE_HS_SYSFS)
                headset_priv.jack.button_det = false;
                headset_priv.jack.adc_val = 0;
#endif
...
#endif
#if defined (SEC_USE_HS_SYSFS)
    headset_priv.jack.jack_det = ht->sdev.state;
#endif
...
#if defined (SEC_USE_HS_SYSFS)
    struct jack_device_priv *pHset_priv = &headset_priv;
#endif
...
#if defined (SEC_USE_HS_SYSFS)
    pHset_priv->jack_buttons_zones = buttons_data;
    pHset_priv->mic_adc_range = pdata->adc_threshold_3pole_detect;
#endif
...
static int headset_detect_probe(struct platform_device *pdev)
{
    ...
    for (i = 0; i < pdata->nbuttons; i++) {
        ...
        input_set_capability(input_dev, EV_KEY, buttons->code);
    }
...
#if defined (SEC_USE_HS_SYSFS)
    pHset_priv->sdev = ht->sdev;
    create_jack_factory_devices(pHset_priv);
#endif
