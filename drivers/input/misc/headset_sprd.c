...
//DFMS testing adding estate sys attribute
...
static int headset_detecting = 0; //false button interrupt
#endif
...
int headset_gpio_2_button_state(int gpio_button_value_current)
{
        struct sprd_headset *ht = &headset;
        struct sprd_headset_platform_data *pdata = ht->platform_data;
...
adc_mic_retry:
        ...
        if(0 != pdata->gpio_switch) {
                ...
                msleep(50);
                adc_left_average = adc_get_average(pdata->gpio_detect, last_gpio_detect_value);
...
static void headset_button_work_func(struct work_struct *work)
{
        ...
        if(1 == button_state_current) {//pressed!
...
#ifdef CONFIG_MACH_SHARK
				if(headset_detecting == 1){
						PRINT_INFO("False button interrupt ( Headset_detecting) \n");	
						goto out;
				}
#endif
                        ...
                        input_sync(ht->input_dev);
                        button_state_last = 1;
						ret_estate = PUSH; //DFMS testing sysfs estate update
                ...
                if(1 == button_state_last) {
                        ...
                        input_sync(ht->input_dev);
                        button_state_last = 0;
						ret_estate = RELEASE;	//DFMS testing sysfs estate release
...
//DFMS testing estate show function added
static ssize_t estate_show(struct device *dev, struct device_attribute *attr, char *buf)
 {
...
static int headset_detect_probe(struct platform_device *pdev)
{
	...
	if (ret)
		...
		//DFMS testing, created estate attribute
