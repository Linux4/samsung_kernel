...
static s32 sysfs_head_time, head_cpu_freq;
static s32 sysfs_tail_time, tail_cpu_freq;
...
static void input_booster_sysfs_freqs(struct booster_dvfs *dvfs)
{
	...
	dvfs->freqs[BOOSTER_LEVEL2].chg_cpu_freq = tail_cpu_freq;
}
...
DECLARE_DVFS_DELAYED_WORK_FUNC(CHG, TOUCH)
{
	...
	break;
	case BOOSTER_LEVEL2:
		if (dvfs->freqs[BOOSTER_LEVEL2].chg_cpu_freq == 0) {
#ifndef CONFIG_ARM_MMP_BL_CPUFREQ
			remove_qos(&dvfs->cpu_qos);
#else
			remove_qos(&dvfs->cpu_qos);
...
#ifndef CONFIG_ARM_MMP_BL_CPUFREQ
			set_qos(&dvfs->cpu_qos, PM_QOS_CPUFREQ_MIN, dvfs->freqs[BOOSTER_LEVEL2].chg_cpu_freq);
#else
			set_qos(&dvfs->cpu_qos, PM_QOS_CPUFREQ_L_MIN, dvfs->freqs[BOOSTER_LEVEL2].chg_cpu_freq);
			set_qos(&dvfs->cpu_big_qos, PM_QOS_CPUFREQ_B_MIN, dvfs->freqs[BOOSTER_LEVEL2].chg_cpu_freq);
#endif
		}
...
DECLARE_DVFS_WORK_FUNC(SET, TOUCH)
{
	...
	case BOOSTER_MODE_ON:
		...
		switch (dvfs->level) {
		case BOOSTER_LEVEL1:
			if (dvfs->freqs[BOOSTER_LEVEL1].cpu_freq == 0) {
#ifndef CONFIG_ARM_MMP_BL_CPUFREQ
				remove_qos(&dvfs->cpu_qos);
#else
				remove_qos(&dvfs->cpu_qos);
...
#ifndef CONFIG_ARM_MMP_BL_CPUFREQ
				set_qos(&dvfs->cpu_qos, PM_QOS_CPUFREQ_MIN, dvfs->freqs[BOOSTER_LEVEL1].cpu_freq);
#else
				set_qos(&dvfs->cpu_qos, PM_QOS_CPUFREQ_L_MIN, dvfs->freqs[BOOSTER_LEVEL1].cpu_freq);
				set_qos(&dvfs->cpu_big_qos, PM_QOS_CPUFREQ_B_MIN, dvfs->freqs[BOOSTER_LEVEL1].cpu_freq);
#endif
			}
		break;
		case BOOSTER_LEVEL2:
...
#ifndef CONFIG_ARM_MMP_BL_CPUFREQ
				set_qos(&dvfs->cpu_qos, PM_QOS_CPUFREQ_MIN, dvfs->freqs[BOOSTER_LEVEL2].cpu_freq);
#else
				set_qos(&dvfs->cpu_qos, PM_QOS_CPUFREQ_L_MIN, dvfs->freqs[BOOSTER_LEVEL2].cpu_freq);
				set_qos(&dvfs->cpu_big_qos, PM_QOS_CPUFREQ_B_MIN, dvfs->freqs[BOOSTER_LEVEL2].cpu_freq);
#endif
			}
		break;
		case BOOSTER_LEVEL3:
			if (dvfs->freqs[BOOSTER_LEVEL3].cpu_freq == 0) {
#ifndef CONFIG_ARM_MMP_BL_CPUFREQ
				remove_qos(&dvfs->cpu_qos);
#else
				remove_qos(&dvfs->cpu_qos);
...
#ifndef CONFIG_ARM_MMP_BL_CPUFREQ
				set_qos(&dvfs->cpu_qos, PM_QOS_CPUFREQ_MIN, dvfs->freqs[BOOSTER_LEVEL3].cpu_freq);
#else
				set_qos(&dvfs->cpu_qos, PM_QOS_CPUFREQ_L_MIN, dvfs->freqs[BOOSTER_LEVEL3].cpu_freq);
				set_qos(&dvfs->cpu_big_qos, PM_QOS_CPUFREQ_B_MIN, dvfs->freqs[BOOSTER_LEVEL3].cpu_freq);
#endif
			}
		break;
		default:
...
void change_booster_level_for_tsp(unsigned char level)
{
	...
	if (!dvfs || !dvfs->initialized) {
		dev_err(g_data->dev, "%s: Dvfs is not initialized\n",
			__func__);
		return;
	}
...
static void input_booster_lookup_freqs(struct booster_dvfs *dvfs)
{
	...
	for (i = 0; i < BOOSTER_LEVEL_MAX; i++) {
		...
		dev_info(data->dev, "%s: %s's LEVEL%d cpu %d\n",
			__func__, dvfs->name, i,
			dvfs->freqs[i].cpu_freq);
	}
	dev_info(data->dev, "#############################################################\n");
}

static void input_booster_lookup_chg_freqs(struct booster_dvfs *dvfs)
{
	struct input_booster *data = dev_get_drvdata(dvfs->parent_dev);
	int i;
	...
	dev_info(data->dev, "#############################################################\n");
	for (i = 0; i < BOOSTER_LEVEL_MAX; i++) {
		if (!dvfs->freqs[i].chg_cpu_freq)
			continue;

		dev_info(data->dev, "%s: %s's LEVEL%d chg_cpu %d\n",
			__func__, dvfs->name, i,
			dvfs->freqs[i].chg_cpu_freq);
	}
	dev_info(data->dev, "#############################################################\n");
}
		...
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%lu%lu", &head_time, &cpu_freq) != 2) {
		printk("### Keep this format : [head_time cpu_freq] (Ex: 0 1100000 ###\n");
		goto out;
	}
		...
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%lu%lu", &tail_time, &cpu_freq) != 2) {
		printk("### Keep this format : [tail_time cpu_freq] (Ex: 0 1100000 ###\n");
		goto out;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d%lu", &level, &cpu_freq) != 2) {
		dev_err(data->dev, "### Keep this format : [level cpu_freq] (Ex: 1 1100000 ###\n");
		goto out;
	}
...
#ifdef BOOSTER_DEBUG
	input_booster_lookup_freqs(dvfs);
	input_booster_lookup_chg_freqs(dvfs);
	input_booster_lookup_times(dvfs);
#endif
out:
	return ret;
...
static int input_booster_parse_dt(struct device *dev)
{
	...
	for_each_child_of_node(np, cnp) {
		...
		booster_device->desc = of_get_property(cnp, "label", NULL);
		if (of_property_read_u32(cnp, "type", &booster_device->type)) {
		...
		plevels = of_get_property(cnp, "levels", &num_level);
		...
		for (i = 0; i < num_level; i++) {
			if (of_property_read_u32_index(cnp, "levels", i, &level)) {
				dev_err(dev, "Failed to get levels property\n");
				retval = -EINVAL;
				goto err_out;
			}

			retval = of_property_read_u32_index(cnp, "cpu_freqs", i,
			...
			retval = of_property_read_u32_index(cnp, "chg_cpu_freqs", i,
				 &pdata->freq_tables[device_type][level].chg_cpu_freq);
			if (retval) {
				dev_err(dev, "Failed to get xxx_freqs property\n");
				retval = -EINVAL;
				goto err_out;
			...
			retval = of_property_read_u32_index(cnp, "head_times", i,
				 &pdata->time_tables[device_type][level].head_time);
			retval |= of_property_read_u32_index(cnp, "tail_times", i,
				 &pdata->time_tables[device_type][level].tail_time);
			retval |= of_property_read_u32_index(cnp, "phase_times", i,
		...
		if(device_type == BOOSTER_DEVICE_TOUCH){
			...
			tail_cpu_freq = booster_device->freq_table[BOOSTER_LEVEL2].chg_cpu_freq;
		}
		...
		for (i = 0; i < BOOSTER_LEVEL_MAX; i++) {
			dev_info(dev, "Level %d : frequency[%d]\n", i, booster_device->freq_table[i].cpu_freq);
			dev_info(dev, "Level %d : chg_frequency[%d]\n", i, booster_device->freq_table[i].chg_cpu_freq);
			dev_info(dev, "Level %d : times[%d,%d,%d]\n", i,booster_device->time_table[i].head_time, booster_device->time_table[i].tail_time, booster_device->time_table[i].phase_time);
		}
#endif
		device_count++;
	}
...
static int input_booster_probe(struct platform_device *pdev)
{
	...
	if (dev->of_node) {
		...
		ret = input_booster_parse_dt(dev);
		if (ret) {
	...
	if (!pdata) {
		dev_err(dev, "There are no platform data\n");
		return -EINVAL;
	}

	if (!pdata->ndevices || !pdata->devices) {
		dev_err(dev, "There are no devices for booster\n");
		return -EINVAL;
	}
	...
	if (!data) {
		dev_err(dev, "Fail to allocate booster data\n");
		return -ENOMEM;
	}
	...
	for (i = 0; i < data->ndevices; i++) {
		...
		if (ret) {
			dev_err(dev, "Failed to init dvfs for %s\n",
				data->devices[i].desc);
			goto err_init_dvfs;
		}
