...
static void touchProcess(struct abovXX * this)
{
	...
	if (this && pDevice) {
		...
		LOG_INFO("Inside touchProcess() i = 0x%x\n",i);
...
//+bug 612420,leiying.wt,add,20210427,SAR FW first update failed
#if WT_ADD_SAR_HARDWARE_INFO
...
//+bug 492320,20191119,gaojingxuan.wt,add,use SS hal
        ...
        //device_create_file(&input_top->dev, &dev_attr_enable);
...
static void abovXX_process_interrupt(pabovXX_t this, u8 nirqlow)
{
	...
	printk("Worker - Refresh Status %d ,nirqlow 0x%x ,this->useIrqTimer 0x%x\n", status,nirqlow,this->useIrqTimer);
...
static void abovXX_worker_func(struct work_struct *work)
{
	...
	if (work) {
		...
		if ((!this->get_nirq_low) || (!this->get_nirq_low(this->board->irq_gpio))) {
			...
			printk("abovXX_worker_func error \n");
		}
	} else {
