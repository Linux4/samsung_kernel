...
extern char sales_code_from_cmdline[SALE_CODE_STR_LENGTH + 1];
...
#endif
 printk("sx9375 sales_code_from_cmdline=%s,is_anfr_sar = %d\n",sales_code_from_cmdline,(uint8_t)is_anfr_sar);
//-P86801AA1,ludandan2  2023/06/08, add sar power reduction control switch
#if SAR_IN_FRANCE
            if (self->sar_first_boot) {
                if ((self->interrupt_count < MAX_INT_COUNT)  && is_anfr_sar && ((ch0_result - self->ch0_backgrand_cap) >= -8) && \
