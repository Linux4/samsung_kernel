...
static void flip_cover_work(struct work_struct *work)
{
	...
	if(first == second) {
		flip_cover = first;
		/* SW_LID scancode : open=0, close=1 */
		input_report_switch(ddata->input, SW_LID, flip_cover^1);
...
static int hall_probe(struct platform_device *pdev)
{
	...
	dev = sec_dev_get_by_name(SEC_KEYPAD_PATH);
	if (!dev) {
		printk("[hall] failed to find %s path\n", SEC_KEYPAD_PATH);
	}
	else {
		error = sysfs_create_group(&dev->kobj, &hall_attr_group);
		if (error) {
