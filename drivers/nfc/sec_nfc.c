...
static irqreturn_t sec_nfc_clk_irq_thread(int irq, void *dev_id)
{
	...
	pr_debug("[NFC]Clock Interrupt is occurred!\n");
							...
							unsigned long arg)
{
	...
	case SEC_NFC_SET_MODE:
		dev_info(info->dev, "%s: SEC_NFC_SET_MODE\n", __func__);
	...
	struct sec_nfc_platform_data *pdata)
{
	...
	if(pdata->ven < 0)
		of_property_read_u32(np, "sec-nfc,ven-gpio",
			&pdata->ven);
	if(pdata->pvdd < 0)
		of_property_read_u32(np, "sec-nfc,pvdd-gpio",
			&pdata->pvdd);
	...
	if (of_get_property(dev->of_node, "sec-nfc,nfc_pm_clk", NULL)) {
		...
		if (IS_ERR(pdata->clk)) {
			pr_err("[NFC] %s: Couldn't get rf_clk)\n", __func__);
		} else {
	...
	pr_info("%s: irq : %d, ven : %d, firm : %d, pvdd : %d\n",
			__func__, pdata->irq, pdata->ven, pdata->firm, pdata->pvdd);
	return 0;
}
...
static int __sec_nfc_probe(struct device *dev)
{
	...
	if (of_get_property(dev->of_node, "sec-nfc,nfc_pm_clk", NULL)) {
		if (pdata->clk_req) {
			...
			if (ret) {
				dev_err(dev, "[NFC] failed to get gpio clk-req-gpio\n");
