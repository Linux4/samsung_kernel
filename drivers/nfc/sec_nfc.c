...
#include <linux/clk.h>
	...
	defined(CONFIG_SEC_NFC_USE_8994_BBCLK2)
#include <linux/clk.h>
#endif
...
#ifdef CONFIG_SEC_NFC_LDO_CONTROL
static int sec_nfc_regulator_onoff(struct i2c_client *client, int onoff)
{
	int rc = 0;
	struct regulator *regulator_i2c_1p8;

	regulator_i2c_1p8 = regulator_get(&client->dev, "sec-nfc,i2c_1p8");
...
#endif
#else
	ret = sec_nfc_regulator_onoff(client, NFC_I2C_LDO_ON);
		if (ret < 0)
			pr_err("%s max86900_regulator_on fail err = %d\n",
				__func__, ret);
		usleep_range(1000, 1100);
...
#ifdef CONFIG_SEC_NFC_USE_8994_BBCLK2
/* PMIC */
...
void sec_nfc_clk_ctl_enable(struct sec_nfc_info *info)
{
	...
	if (pdata->clk_req)
		...
		dev_err(info->dev, "failed to get clk_irq\n");
		return;
	}
					...
					enum sec_nfc_mode mode)
{
	...
	if (info->mode == mode) {
		pr_info("Power mode is already %d", mode);
		return;
	}
...
static int sec_nfc_pinctrl(struct device *dev)
{
	...
	if (IS_ERR(nfc_pinctrl)) {
		pr_debug("[NFC] Target does not use pinctrl\n");
		nfc_pinctrl = NULL;
	} else {
