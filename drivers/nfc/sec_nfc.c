...
#if defined(LSI_AP) && defined(CONFIG_ESE_SECURE)
...
static int sec_nfc_regulator_onoff(struct sec_nfc_platform_data *data, int onoff)
{
	...
	regulator_nfc_pvdd = data->nfc_pvdd;
	...
	struct sec_nfc_platform_data *pdata)
{
	...
	nfc_det_gpio = of_get_named_gpio(np, "sec-nfc,nfc-det-gpio", 0);
	if (!gpio_is_valid(nfc_det_gpio)) {
		pr_info("%s : nfc-det-gpio is not set", __func__);
	} else {
		gpio_request(nfc_det_gpio, "nfc_det_gpio");
		gpio_direction_input(nfc_det_gpio);
		if (!gpio_get_value(nfc_det_gpio)) {
			pr_info("%s : nfc is not supported", __func__);
			return -ENODEV;
		}
		pr_info("%s : nfc is supported", __func__);
	}
	...
	if (of_get_property(dev->of_node, "sec-nfc,ldo_control", NULL)) {
		...
		if (of_find_property(np, "sec-nfc,pvdd_en", NULL))
			pdata->pvdd = of_get_named_gpio(np, "sec-nfc,pvdd_en", 0);
	}
...
static int __sec_nfc_probe(struct device *dev)
{
	...
	if (pdata->clk_req_wake) {
		...
		NFC_LOG_ERR("probe() gpio to irq :%d\n", pdata->clk_irq);

		ret = request_threaded_irq(pdata->clk_irq, NULL, sec_nfc_clk_irq_thread,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT, SEC_NFC_DRIVER_NAME, info);
		if (ret < 0)
...
{
	NFC_LOG_INFO("Loading sec_nfc driver\n");
	if (lpcharge) {
