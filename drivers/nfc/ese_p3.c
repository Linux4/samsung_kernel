...
// static int chipselect = 0;
 // static int spimode = SPI_MODE_3;//SPI_MODE_3;
 // static int maxspeed = 10000000; // 10 Mhz
#endif
...
static int p3_disable_clk(struct spip3_data *spip3)
{
	...
	gpio_set_value(spip3->nfc_ese_pwr_req, 0);
...
static int spip3_open(struct inode *inode, struct file *filp)
{
	...
	/*setup OT chip*/
	gpio_set_value(g_spip3->nfc_ese_pwr_req, 1);
	usleep_range(2900,3000);
#if 0
	if(spip3 == NULL){
		printk("spip3 pointer is null return -1");
		return -1;
	}

	printk("%s\n", __func__);
	mutex_lock(&device_list_lock);
	list_for_each_entry(spip3, &device_list, device_entry) {
		if (spip3->devt == inode->i_rdev) {
			status = 0;
			break;
		}
	}
  ...
  //              }
  //       } else
   //              pr_info("spip3: nothing for minor %d\n", iminor(inode));
...
//		spin_unlock_irq(&spip3->spi_lock);
	...
	// In case of only spip3 open and release
	gpio_direction_output(spip3->nfc_ese_pwr_req, 0);
...
static int spip3_probe(struct spi_device *spi)
{
	...
	spip3->spi = spi;
		...
		spip3->devt = MKDEV(P3_SPI_MAJOR, P3_SPI_MINORS);
#if 0
		dev = device_create(spip3_class, &spi->dev, spip3->devt,
					 spip3, "p3");
		status = IS_ERR(dev) ? PTR_ERR(dev) : 0;

		if (status =misc_register= 0) {
		 list_add(&spip3->device_entry, &device_list);
		}
#else
		ret = (&p3_dev->p3_device);
		if (ret < 0)
		{
...
static int __init spip3_init(void)
{
	...
	status = register_chrdev(P3_SPI_MAJOR, "spip3chardev", &spip3_fops);

	if (status < 0) {
		return status;
	}

	spip3_class = class_create(THIS_MODULE, "spip3class");
	if (IS_ERR(spip3_class)) {
		unregister_chrdev(P3_SPI_MAJOR, spip3_spi.driver.name);
		return PTR_ERR(spip3_class);
	}

#if 1
	status = spi_register_driver(&spip3_spi);
	if (status < 0) {
		class_destroy(spip3_class);
		unregister_chrdev(P3_SPI_MAJOR, spip3_spi.driver.name);
	}
#else
	status = spi_register_driver(&spip3_spi);
...
/* We create a virtual device that will sit on the bus*/
	...
	printk("smpark : busnum=%d cs=%d bufsiz=%d maxspeed=%d",
		0, chipselect, bufsiz, maxspeed);

	return spip3_probe(spi);
