...
#define DRIVER_VERSION "2.2.0.16"
...
#ifdef CONFIG_NFC_ST21NFC
	/* GPIO for i2c switch (output) */
	struct gpio_desc *gpiod_switch_en;
#endif
...
static int st21nfc_clock_select(struct st21nfc_device *st21nfc_dev)
{
	...
	if (st21nfc_dev->clk_run == false) {
		...
		NFC_LOG_INFO("%s:%d clock enabled\n", __FILE__, __LINE__);
	}
	return ret;
...
static irqreturn_t st21nfc_clk_irq_handler(int irq, void *dev_id)
{
	struct st21nfc_device *st21nfc_dev = dev_id;
	int req_val;

	mutex_lock(&st21nfc_dev->clkreq_mutex);
	if (st21nfc_dev->irq_all_trigger) {
		req_val = gpiod_get_value(st21nfc_dev->gpiod_clkreq);
		...
		if (req_val) {
			...
			if (!st21nfc_dev->clk_enabled && !IS_ERR_OR_NULL(st21nfc_dev->s_clk) &&
					!clk_prepare_enable(st21nfc_dev->s_clk))
				st21nfc_dev->clk_enabled = true;
		} else {
			if (st21nfc_dev->clk_enabled && !IS_ERR_OR_NULL(st21nfc_dev->s_clk)) {
				clk_disable_unprepare(st21nfc_dev->s_clk);
				st21nfc_dev->clk_enabled = false;
			}
		}
	} else {
		NFC_LOG_REC("clk_req\n");
	...
	mutex_unlock(&st21nfc_dev->clkreq_mutex);
					...
					int mode)
{
	...
	if (ret) {
		NFC_LOG_ERR("%s : set_irq_type failed\n", __func__);
		return -ENODEV;
	}
	 ...
	 */
	if (enable_debug_log)
		NFC_LOG_DBG("%s : requesting IRQ %d\n", __func__, client->irq);
	st21nfc_dev->irq_enabled = true;
	...
	if (ret) {
		NFC_LOG_ERR("%s : devm_request_irq failed\n", __func__);
		return -ENODEV;
	}
				       ...
				       bool is_ntf)
{
	...
	if (new_state == old_state) {
		if ((st21nfc_dev->pw_states[ST21NFC_IDLE].last_entry != 0) ||
			...
			NFC_LOG_ERR("%s Error: Switched from %s to %s!: %llx, ntf=%d\n",
	...
	NFC_LOG_DBG("%s Switching from %s to %s: %llx, ntf=%d\n", __func__,
				       ...
				       char *buf, size_t count)
{
	...
	if (count != HEADER_LENGTH) {
		NFC_LOG_ERR("%s Warning: expect previous one was idle data\n");
				...
				size_t count, loff_t *offset)
{
	...
	if (gpiod_get_value(st21nfc_dev->gpiod_irq) == 0) {
		NFC_LOG_INFO("%s : read called but no IRQ.\n", __func__);
...
#ifdef ST54J_PWRSTATS
	if (ret < 0) {
		NFC_LOG_ERR("%s: i2c_master_recv returned %d\n", __func__, ret);
	...
	if (ret > count) {
		NFC_LOG_ERR("%s: received too many bytes from i2c (%d)\n", __func__,
		       ret);
		return -EIO;
	}
	...
	if (idle < HEADER_LENGTH) {
		...
		if ((st21nfc_dev->r_state_current == ST21NFC_HEADER) &&
			...
			if (enable_debug_log)
				NFC_LOG_DBG("%s : new state = ST21NFC_PAYLOAD\n",
					 __func__);
		} else {
			st21nfc_dev->r_state_current = ST21NFC_HEADER;
			if (enable_debug_log)
				NFC_LOG_DBG("%s : new state = ST21NFC_HEADER\n",
					 __func__);
		}
	}
#endif // ST21NFCD_MTK
				 ...
				 size_t count, loff_t *offset)
{
	...
	if (IS_ERR_OR_NULL(tmp)) {
		NFC_LOG_ERR("%s : memdup_user failed\n", __func__);
		return -EFAULT;
	}
	...
	if (ret != count) {
		NFC_LOG_ERR("%s : i2c_master_send returned %d\n", __func__, ret);
		ret = -EIO;
	}
	kfree(tmp);
...
static int st21nfc_dev_open(struct inode *inode, struct file *filp)
{
	...
	NFC_LOG_INFO("%s:%d dev_open\n", __FILE__, __LINE__);

	if (st21nfc_dev->device_open) {
		ret = -EBUSY;
		NFC_LOG_ERR("%s : device already opened ret= %d\n", __func__, ret);
	} else {
...
static int st21nfc_release(struct inode *inode, struct file *file)
{
	...
	NFC_LOG_INFO("%s : device_open  = false\n", __func__);
			      ...
			      unsigned long arg)
{
	...
	case ST21NFC_LEGACY_SET_POLARITY_RISING:
		NFC_LOG_INFO(" ### ST21NFC_SET_POLARITY_RISING ###\n");
	...
	case ST21NFC_LEGACY_RECOVERY:
		...
		if (!IS_ERR_OR_NULL(st21nfc_dev->gpiod_reset)) {
			...
			if (ret) {
				NFC_LOG_ERR("%s : gpiod_direction_output failed\n",
				       __func__);
				ret = -ENODEV;
	...
	case ST21NFC_USE_ESE:
		...
		if (enable_debug_log)
			NFC_LOG_DBG("%s use ESE %d : %d\n", __func__, ret, tmp);
		break;
	default:
		NFC_LOG_ERR("%s bad ioctl %u\n", __func__, cmd);
		ret = -EINVAL;
		break;
	}
	return ret;
...
static int st21nfc_ping(struct st21nfc_device *st21nfc_dev)
{
	...
	while ((loops-- > 0) && gpiod_get_value(st21nfc_dev->gpiod_irq)) {
		...
		// Read next message.
		len = i2c_master_recv(st21nfc_dev->client, st21nfc_dev->buffer, 4);
		if (len != 4) {
			NFC_LOG_ERR("%s: Could not read header: %d\n", __func__, len);
			break;
		}

		if (st21nfc_dev->buffer[0] == IDLE_CHARACTER) {
			if (st21nfc_dev->buffer[1] == IDLE_CHARACTER) {
				NFC_LOG_ERR("%s: Read 7E7E... header, IRQ always high ? Stop\n", __func__);
				break;
			} else {
				// 4bytes header, shift
				...
				if (len != (int)st21nfc_dev->buffer[2]) {
					NFC_LOG_ERR("%s: Could not read payload: %d\n", __func__, len);
					break;
				}
			}
		} else {
		...
		NFC_LOG_INFO("%s Read message (%d bytes): %02x %02x ...\n", __func__,
...
#ifdef CONFIG_NFC_ST21NFC
static const struct acpi_gpio_params switch_en_gpios = { 4, 0, false }; /*i2c switch*/
#endif
...
#ifdef CONFIG_NFC_ST21NFC
	{ "switch_en-gpios", &switch_en_gpios, 1 },
#endif
};
...
static ssize_t pvdd_store(struct class *class, struct class_attribute *attr,
	...
	const char *buf, size_t size)
{
	...
	NFC_LOG_INFO("%s debug log\n", enable_debug_log ? "enable" : "disable");

	return size;
}
static CLASS_ATTR_RW(check);

static int secnfc_regulator_onoff(struct st21nfc_device *st21nfc_dev, bool on)
{
	...
	if (on)
		rc = regulator_enable(st21nfc_dev->nfc_pvdd);
	else
		rc = regulator_disable(st21nfc_dev->nfc_pvdd);

	if (rc)
		NFC_LOG_ERR("%s: %s failed %d\n", __func__, (on ? "enable" : "disable"), rc);
	else
...
static int secnfc_init(struct i2c_client *client)
{
	int ret = 0;
	...
	st21nfc_dev->nfc_class = class_create(THIS_MODULE, CLASS_NAME);
	if (IS_ERR_OR_NULL(st21nfc_dev->nfc_class)) {
		ret = PTR_ERR(st21nfc_dev->nfc_class);
		NFC_LOG_ERR("%s: failed to register device class %d\n", __func__, ret);
		goto err_class;
	}
	ret = class_create_file(st21nfc_dev->nfc_class, &class_attr_nfc_support);
	if (ret) {
		NFC_LOG_ERR("%s: failed to create nfc_support %d\n", __func__, ret);
		goto err_class_support;
	}
	ret = class_create_file(st21nfc_dev->nfc_class, &class_attr_pvdd);
	if (ret) {
		NFC_LOG_ERR("%s: failed to create pvdd %d\n", __func__, ret);
		goto err_class_pvdd;
	}
	ret = class_create_file(st21nfc_dev->nfc_class, &class_attr_check);
	if (ret) {
		NFC_LOG_ERR("%s: failed to create check %d\n", __func__, ret);
		goto err_class_check;
	}

	return 0;

err_class_check:
	class_remove_file(st21nfc_dev->nfc_class, &class_attr_pvdd);
err_class_pvdd:
	class_remove_file(st21nfc_dev->nfc_class, &class_attr_nfc_support);
err_class_support:
	class_destroy(st21nfc_dev->nfc_class);
err_class:
	secnfc_regulator_onoff(st21nfc_dev, false);
exit:
	return ret;
}

static void secnfc_deinit(struct i2c_client *client)
{
	struct st21nfc_device *st21nfc_dev = i2c_get_clientdata(client);
	...
	class_remove_file(st21nfc_dev->nfc_class, &class_attr_check);
	class_remove_file(st21nfc_dev->nfc_class, &class_attr_pvdd);
	class_remove_file(st21nfc_dev->nfc_class, &class_attr_nfc_support);
	class_destroy(st21nfc_dev->nfc_class);
...
// QCOM and MTK54 use standard GPIO definition
	ret = acpi_dev_add_driver_gpios(ACPI_COMPANION(dev),
					acpi_st21nfc_gpios);
	if (ret)
		NFC_LOG_DBG("Unable to add GPIO mapping table\n");

	if (!of_property_read_string(np, "st21nfc,ap_vendor", &ap_str)) {
		if (!strcmp(ap_str, "slsi"))
			st21nfc_dev->ap_vendor = AP_VENDOR_SLSI;
		else if (!strcmp(ap_str, "qct") || !strcmp(ap_str, "qualcomm"))
			st21nfc_dev->ap_vendor = AP_VENDOR_QCT;
		else if (!strcmp(ap_str, "mtk"))
			st21nfc_dev->ap_vendor = AP_VENDOR_MTK;
	} else {
...
// QCOM and MTK54 use standard GPIO definition
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_pidle)) {
		NFC_LOG_ERR("[OPTIONAL] Unable to request pidle-gpio\n");
		ret = 0;
	} else {
		...
		if (ret) {
			NFC_LOG_ERR("%s : devm_request_irq for power stats idle failed\n",
			       __func__);
			return ret;
		}
		...
		if (ret) {
			NFC_LOG_ERR("%s : sysfs_create_file for power stats failed\n",
			       __func__);
			return ret;
		}
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_clkreq)) {
		...
		if (ret < 0) {
			NFC_LOG_ERR("%s : st21nfc_clock_select failed\n", __func__);
			goto err_sysfs_power_stats;
		}

		st21nfc_dev->clk_req_wake = of_property_read_bool(np, "st21nfc,clk_req_wake");

		if (st21nfc_dev->clk_req_wake) {
			unsigned long irq_flag = IRQF_TRIGGER_RISING | IRQF_ONESHOT;

			NFC_LOG_INFO("register CLK REQ IRQ handler\n");

			if (device_property_read_bool(dev, "st21nfc,irq_all_trigger")) {
				st21nfc_dev->irq_all_trigger = true;
				irq_flag |=  IRQF_TRIGGER_FALLING;
				NFC_LOG_INFO("clk_req all triggeer\n");
			}

			st21nfc_dev->clk_irq = gpiod_to_irq(st21nfc_dev->gpiod_clkreq);
			ret = devm_request_threaded_irq(dev, st21nfc_dev->clk_irq, NULL,
					st21nfc_clk_irq_handler, irq_flag, "sec-nfc_clk", st21nfc_dev);
			if (ret < 0)
				NFC_LOG_ERR("failed to register CLK REQ IRQ handler\n");
			else
				enable_irq_wake(st21nfc_dev->clk_irq);
		}
	}
	...
	NFC_LOG_DBG("debug irq_gpio = %d, client-irq =  %d, pidle_gpio = %d\n",
	...
	if (ret) {
		NFC_LOG_ERR("%s : misc_register failed\n", __func__);
	...
	if (ret) {
		NFC_LOG_ERR("%s : secnfc_init failed\n", __func__);
...
//if regulator is turned on late, it takes time for rst pin to turn on after the power.
	...
	if (st21nfc_dev->ap_vendor == AP_VENDOR_MTK) {
		...
		pinctrl = devm_pinctrl_get_select(&client->dev, "nfc_on");
		if (IS_ERR_OR_NULL(pinctrl))
			NFC_LOG_ERR("Failed to pinctrl default\n");
		else
			devm_pinctrl_put(pinctrl);
		NFC_LOG_ERR("i2c off pinctrl called\n");
	}
	...
	if (ret) {
		NFC_LOG_ERR("%s : sysfs_create_group failed\n", __func__);
		goto err_sysfs_create_group_failed;
	}
	...
	if ((ret = st21nfc_ping(st21nfc_dev))) {
		NFC_LOG_ERR("%s: Did not get CORE_RESET_NTF, hardware issue? %d\n", __func__, ret);
		return ret;
	}
#endif // WITH_PING_DURING_PROBE
...
err_sysfs_power_stats:
	...
	mutex_destroy(&st21nfc_dev->clkreq_mutex);
	return ret;
}
...
static int st21nfc_remove(struct i2c_client *client)
{
...
#ifdef CONFIG_NFC_ST21NFC
	if (!st21nfc_dev) {
		NFC_LOG_ERR("%s : unable to get nfc device\n", __func__);
		return -ENOMEM;
	}

	secnfc_deinit(client);
#endif
	...
	devm_free_irq(&client->dev, st21nfc_dev->clk_irq, st21nfc_dev);
	mutex_destroy(&st21nfc_dev->clkreq_mutex);
	mutex_destroy(&st21nfc_dev->read_mutex);
	mutex_destroy(&st21nfc_dev->irq_dir_mutex);
...
static int st21nfc_resume(struct device *device)
{
	...
	/* if nfc is off, then disable clock for some SLSI AP */
	if (st21nfc_dev->ap_vendor == AP_VENDOR_SLSI && st21nfc_dev->irq_all_trigger &&
			!st21nfc_dev->device_open) {
		mutex_lock(&st21nfc_dev->clkreq_mutex);
		if (!IS_ERR_OR_NULL(st21nfc_dev->s_clk)) {
			if (!st21nfc_dev->clk_enabled)
				clk_prepare_enable(st21nfc_dev->s_clk);
			clk_disable_unprepare(st21nfc_dev->s_clk);
			st21nfc_dev->clk_enabled = false;
		}
		mutex_unlock(&st21nfc_dev->clkreq_mutex);
		NFC_LOG_INFO("clk disabled\n");
	}
...
static int __init st21nfc_dev_init(void)
{
	...
	nfc_logger_init();
#endif
	NFC_LOG_INFO("Loading st21nfc driver\n");
#ifdef CONFIG_NFC_ST21NFC
	enable_debug_log = false;
#endif
