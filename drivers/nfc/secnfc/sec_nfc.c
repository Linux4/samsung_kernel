...
static void sec_nfc_uart_suspend(void)
{
	...
	ret = gpio_tlmm_config (GPIO_CFG(27, GPIOMUX_FUNC_GPIO,
					GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
					GPIO_CFG_ENABLE);
	if (ret) {

		pr_err( "failed to configure GPIO_27. ret %d\n", ret);
	}

	ret = gpio_tlmm_config (GPIO_CFG(28, GPIOMUX_FUNC_GPIO,
					GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
					GPIO_CFG_ENABLE);
	if (ret) {

		pr_err( "failed to configure GPIO_28. ret %d\n", ret);
	}

	ret = gpio_tlmm_config (GPIO_CFG(29, GPIOMUX_FUNC_GPIO,
					GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
					GPIO_CFG_ENABLE);
	if (ret) {

		pr_err( "failed to configure GPIO_29. ret %d\n", ret);
	}

	ret = gpio_tlmm_config (GPIO_CFG(30, GPIOMUX_FUNC_GPIO,
					GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
					GPIO_CFG_ENABLE);
	if (ret) {

		pr_err( "failed to configure GPIO_30. ret %d\n", ret);
	}
	pr_info("%s: exit\n", __func__);
}
...
static int sec_nfc_probe(struct platform_device *pdev)
{
	...
	if(system_rev >= 11) {
		...
		ret = gpio_tlmm_config (GPIO_CFG(77, GPIOMUX_FUNC_GPIO,
					GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
					GPIO_CFG_ENABLE);
		if (ret) {

			dev_err(dev, "failed to configure GPIO_77. ret %d \n", ret);
		} else {

			if (poweroff_charging) {
				pr_info("%s: [poweroff_charging] Setting the GPIO_77 pin LOW\n",__func__);
				gpio_set_value(77, 0);
				sec_nfc_uart_suspend();
			} else {
				pr_info("%s: [Normal case] Setting the GPIO_77 pin HIGH \n",__func__);
				gpio_set_value(77, 1);
			}
			pr_info("%s: Set the GPIO_77 (%d) to HIGH. \n", __func__, pdata->tvdd);
		}
	}
