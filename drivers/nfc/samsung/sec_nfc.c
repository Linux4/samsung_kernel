...
#include "sec_nfc.h"
#include "wakelock.h"
/*Huaqin modify for SR-QL3095-01-46 by zhangshuli at 2020/09/02 start */
#include <soc/qcom/smem.h>
/*Huaqin modify for SR-QL3095-01-46 by zhangshuli at 2020/09/02 end */
...
enum {
	HQ_PCBA_HS50_EUR = 0x260,
	HQ_PCBA_HS50_EUR_UW = 0x2E0,
};
/*Huaqin modify for SR-QL3095-01-46 by zhangshuli at 2020/09/02 end */
...
/*Huaqin modify for SR-QL3095-01-46 by zhangshuli at 2020/08/25 start */
static long sec_nfc_compat_ioctl(struct file *file, unsigned int cmd,
                            unsigned long arg) {
    long ret = 0;
    ret = sec_nfc_ioctl(file, cmd, (unsigned long)compat_ptr(arg));
    return ret;
}
/*Huaqin modify for SR-QL3095-01-46 by zhangshuli at 2020/08/25 end */
...
uint32_t get_nfc_board_id(void) {
    uint32_t  huaqin_pcba_config = 0xff;
    uint32_t *pcba_config_addr = NULL;
    uint32_t smem_pcba_config_size = 0;
    pcba_config_addr = (uint32_t *) smem_get_entry(HUAQIN_PCBA_CONFIG_SMEM_ITEM,
            &smem_pcba_config_size, 0, SMEM_ANY_HOST_FLAG);
    if (pcba_config_addr == NULL) {
        pr_info("%s: smem_get_entry faile \n", __func__);
        return -1;
    }
    huaqin_pcba_config = (*pcba_config_addr);
    pr_info("%s: board id is 0x%x\n", __func__, huaqin_pcba_config);
...
static int __sec_nfc_probe(struct device *dev)
{
    ...
    int ret = 0;
    /*Huaqin modify for SR-QL3095-01-46 by zhangshuli at 2020/09/02 start */
    uint32_t sub_type = PCBA_IDS_MASK & get_nfc_board_id();
    pr_info("%s: pcba is %x\n", __func__, sub_type);
    if ((sub_type != HQ_PCBA_HS50_EUR) && (sub_type != HQ_PCBA_HS50_EUR_UW)){
        pr_info("%s: pcba is not EU ,we ignore it \n", __func__);
        return -ENOMEM;
    }
    /*Huaqin modify for SR-QL3095-01-46 by zhangshuli at 2020/09/02 end */
