	...
	struct sec_nfc_platform_data *pdata)
{
	...
	/*check if NFC support*/
	if (of_find_property(np, "sec-nfc,check_nfc", NULL)) {
		int nfc_support = of_get_named_gpio(np, "sec-nfc,check_nfc", 0);

		if (!gpio_is_valid(nfc_support)) {
			NFC_LOG_ERR("check gpio not valid: %d\n", nfc_support);
			return -ENXIO;
		}

		if (gpio_get_value(nfc_support) > 0) {
			NFC_LOG_INFO("nfc support\n");
		} else {
			struct pinctrl *pinctrl = devm_pinctrl_get_select(dev, "nfc_nc");

			if (IS_ERR_OR_NULL(pinctrl))
				NFC_LOG_ERR("Failed to configure NC pinctrl\n");
			else
