...
/* Can Enable this only when there's no eSE*/
#undef FEATURE_NFC_IRQ_LEVEL
/* CLK_REQ is used only for awake AP.(no real clk control)*/
#define FEATURE_CLK_REQ_WAKE
...
#if NFC_DEBUG
		pr_err("%s, irq_gpio = %d  no real!!!\n", __func__,
			gpio_get_value(pn547_dev->irq_gpio));
#endif
		return IRQ_HANDLED;
	}

#ifdef FEATURE_NFC_IRQ_LEVEL
...
#if NFC_DEBUG
	pr_info("pn547_dev_irq_handler : call\n");
#endif
...
#ifdef FEATURE_CLK_REQ_WAKE
static irqreturn_t pn547_clk_irq_handler(int irq, void *dev_id)
{
	struct pn547_dev *pn547_dev = dev_id;
...
#if NFC_DEBUG
		pr_err("%s, irq_gpio = %d  no real!!!\n", __func__,
			gpio_get_value(pn547_dev->clk_req_gpio));
#endif
		return IRQ_HANDLED;
	}
	pr_info("pn547_clk_irq_handler : called\n");

	wake_lock_timeout(&pn547_dev->nfc_clk_wake_lock, 1*HZ);
	if (atomic_read(&pn547_dev->clk_irq_enabled) == 1) {
		disable_irq_nosync(pn547_dev->clk_req_irq);
		atomic_set(&pn547_dev->clk_irq_enabled, 0);
	}
...
wait_irq:
	...
	if (!gpio_get_value(pn547_dev->irq_gpio)) {
		...
		if (atomic_read(&pn547_dev->irq_enabled) == 1) {
			...
			disable_irq_nosync(pn547_dev->client->irq);
			atomic_set(&pn547_dev->irq_enabled, 0);
		} /* You can change irq flag not to toggle irq enable or disable.*/
		...
		if (!gpio_get_value(pn547_dev->irq_gpio))
			ret = wait_event_interruptible(pn547_dev->read_wq,
				atomic_read(&pn547_dev->read_flag));
#endif /*FEATURE_NFC_IRQ_LEVEL*/

#if NFC_DEBUG
		pr_info("pn547_dev_read : -------  h\n");
#endif
		if (ret)
			goto fail;
...
#endif
			pn547_dev->nfc_ven_enabled = true;
			pr_info("%s power on with firmware, irq_en=%d\n", __func__,
				atomic_read(&pn547_dev->irq_enabled));
		} else if (arg == 1) {
			/* power on */
			if (pn547_dev->conf_gpio)
	...
	struct pn547_i2c_platform_data *pdata)
{
	...
	pdata->clk_req_gpio = of_get_named_gpio_flags(np, "pn547,clk_req-gpio",
		0, &gpio_flags);
	pdata->clk_req_irq = gpio_to_irq(pdata->clk_req_gpio);
	pr_info("%s CLK_REQ gpio:%d, irq:%d \n", __func__, pdata->clk_req_gpio,
		pdata->clk_req_irq);
#endif
#ifdef CONFIG_NFC_PN547_ESE_SUPPORT
	...
	if (of_get_property(dev->of_node, "pn547,gpio_power", NULL)) {
		pdata->pvdd_en = of_get_named_gpio_flags(np, "pn547,pvdd_en",
			0, &pdata->pvdd_en_flags);
		if (pdata->pvdd_en < 0)
		{
			pr_err("%s - get nfc_pvdd error [%d]\n", __func__,pdata->pvdd_en);
			pdata->pvdd_en = 0;
		}
		else
			pr_info("%s gpio_power pvdd_en:%d\n", __func__, pdata->pvdd_en);
	}

	if (of_property_read_string(np, "pn547,i2c_1p8",
			&pdata->i2c_1p8) < 0) {
			pr_err("%s - no LDO. gpio_power?\n", __func__);
			pdata->i2c_1p8 = NULL;
	}
		       ...
		       const struct i2c_device_id *id)
{
	...
	/* To set I2C clk speed 400KHz only for Spreadtrum AP */
	...
	if (platform_data->pvdd_en != 0) {
		ret = gpio_request(platform_data->pvdd_en, "nfc_pvdd_en");
		if (ret) {
			pr_err("%s failed to request pvdd_en pin\n", __func__);
			return -ENODEV;
		}
	}
	...
	} else if (platform_data->pvdd_en != 0) {
		gpio_direction_output(platform_data->pvdd_en, 1);
		pr_info("%s pvdd_en enabled.\n", __func__);
	}
	...
	pr_info("%s : IRQ num %d, irq_gpio:%d\n", __func__, client->irq,
	...
	ret = request_irq(client->irq, pn547_dev_irq_handler,
			  IRQF_TRIGGER_HIGH | IRQF_NO_SUSPEND, "pn547", pn547_dev);
#else
	ret = request_irq(client->irq, pn547_dev_irq_handler,
...
#ifdef FEATURE_CLK_REQ_WAKE
	atomic_set(&pn547_dev->clk_irq_enabled, 0);
	ret = request_irq(pn547_dev->clk_req_irq, pn547_clk_irq_handler,
		IRQF_TRIGGER_HIGH | IRQF_ONESHOT | IRQF_NO_SUSPEND, "pn547_clk_req", pn547_dev);
	if (ret)
		dev_err(&client->dev, "%s request clk_req_irq failed\n", __func__);
	disable_irq(pn547_dev->clk_req_irq);
#endif
...
static int pn547_suspend(struct device *dev)
{
	...
	struct pn547_dev *data = i2c_get_clientdata(client);

	if (atomic_cmpxchg(&data->clk_irq_enabled, 0, 1) == 0) {
		/*atomic_set(&pn547_dev->clk_irq_enabled, 1);*/
		enable_irq(data->clk_req_irq);
	}

	pr_info("%s [%d]\n", __func__, 
		atomic_read(&data->clk_irq_enabled));
#endif
	return 0;
}
...
static int pn547_resume(struct device *dev)
{
	...
	if (atomic_read(&data->clk_irq_enabled) == 1) {
		disable_irq(data->clk_req_irq);
		atomic_set(&pn547_dev->clk_irq_enabled, 0);
	}
#endif
	return 0;
