	...
	struct pn547_i2c_platform_data *pdata)
{
	...
	pdata->firm_gpio = of_get_named_gpio_flags(np, "pn547,firm-gpio", 0,
											&pdata->firm_gpio_flags);
	if (pdata->firm_gpio < 0) {
		pr_err("%s: no firm_gpio at dt file\n", __func__);
		return -ENOENT;
	}
...
#endif
#ifdef CONFIG_NFC_PN547_LDO_CONTROL
	if (of_property_read_string(np, "pn547,i2c_1p8", &pdata->i2c_1p8) < 0) {
	...
	pr_info("%s: irq : %d, ven : %d, firm : %d\n", __func__, pdata->irq_gpio,
										pdata->ven_gpio, pdata->firm_gpio);
