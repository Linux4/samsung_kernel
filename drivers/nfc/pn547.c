...
wait_irq:
	...
	if (!gpio_get_value(pn547_dev->irq_gpio)) {
		...
		while (1) 
		{
			...
			/* You can change irq flag not to toggle irq enable/disable. */
		       ...
		       const struct i2c_device_id *id)
{
	...
	} else if (platform_data->pvdd_en != NULL) {
...
static int pn547_suspend(struct device *dev)
{
	...
	if (atomic_read(&data->clk_irq_enabled) == 0) {
