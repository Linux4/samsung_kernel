...
struct cpufreq_limit_parameter param = {
	...
	.big_min_freq		= 0,
	.big_max_freq		= 0,
	.ltl_min_lock		= 1248000 / 2, /* devide value is ltl_divider */
		...
		unsigned long val, void *data)
{
	...
	pr_debug("++ CPUFREQ(%d): %s: user(%d ~ %d), pol(%lu ~ %lu), set(%lu(%s) ~ %lu), over(%d)\n",
		policy->cpu, __func__,
		policy->user_policy.min, policy->user_policy.max, policy->min, policy->max, min, max,
		cpufreq_limit_get_handle(min_id)->name, param.over_limit);
	...
	pr_debug("-- CPUFREQ(%d): %s: user(%d ~ %d), pol(%lu ~ %lu), set(%lu ~ %lu), over(%d)\n",
		policy->cpu, __func__,
