...
struct cpufreq_limit_hmp hmp_param = {
	...
	.big_cpu_end			= 7,
	.big_min_freq			= 910000,
	.big_max_freq			= 2392000,
	.big_off_freq			= 910000,
	.little_min_freq		= 123500, //247000 Khz
	.little_max_freq		= 845000, //1690000 Khz
	.little_min_lock		= 123500, //247000 Khz
...
ssize_t cpufreq_limit_get_table(char *buf)
{
	...
	for (i = 0, k = 0; i < count_b && k < count_l; ) {
		if(cpuftbl_b[i].frequency > cpuftbl_L[k].frequency)
			len += sprintf(buf + len,"%u ", cpuftbl_b[i++].frequency);
		else if(cpuftbl_b[i].frequency <  cpuftbl_L[k].frequency)
			len += sprintf(buf + len,"%u ", cpuftbl_L[k++].frequency);
		else{
			len += sprintf(buf + len,"%u ", cpuftbl_b[i].frequency);
			i++;
			k++;
