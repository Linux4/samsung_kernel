...
static const u32 EXYNOS9630_MCT_PHYS = 0x10040000;
static const u32 EXYNOS9630_MCT_IOVA = 0x50540000;
static const u32 EXYNOS9630_MCT_SIZE = 0x1000;
...
static int enable_mct_fw_access(struct npu_system *system)
{
	...
	ret = iommu_map(
		domain, EXYNOS9630_MCT_IOVA, EXYNOS9630_MCT_PHYS,
		EXYNOS9630_MCT_SIZE, IOMMU_READ);
	if (ret) {
...
static int disable_mct_fw_access(struct npu_system *system)
{
	...
	if (system->pdev) {
		domain = iommu_get_domain_for_dev(&(system->pdev->dev));
		iommu_unmap(domain, EXYNOS9630_MCT_IOVA, EXYNOS9630_MCT_SIZE);
	}
