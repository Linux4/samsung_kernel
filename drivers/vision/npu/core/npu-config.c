...
static const char * const cfg_name[] = {
	...
	"npu0_llc_boost_buffer_size",
	"npu1_llc_boost_buffer_size",
	"npu2_llc_boost_buffer_size",
	"npu_llc_chunk_size",
#endif
};
...
int npu_create_config(struct device *dev)
{
	...
	configs[DSP_WAIT_MODE] = TMP_DSP_WAIT_MODE;
	configs[DSP_WAIT_RESET] = TMP_DSP_WAIT_RESET;
	configs[DSP_WAIT_BOOT] = TMP_DSP_WAIT_BOOT;
#if IS_ENABLED(CONFIG_NPU_IMB_THRESHOLD)
	configs[NPU_IMB_THRESHOLD_SIZE] = TMP_NPU_IMB_THRESHOLD_SIZE;
#endif
#if IS_ENABLED(CONFIG_NPU_USE_LLC)
	configs[NPU0_LLC_BOOST_BUFFER_SIZE] = TMP_NPU0_LLC_BOOST_BUFFER_SIZE;
	configs[NPU1_LLC_BOOST_BUFFER_SIZE] = TMP_NPU1_LLC_BOOST_BUFFER_SIZE;
	configs[NPU2_LLC_BOOST_BUFFER_SIZE] = TMP_NPU2_LLC_BOOST_BUFFER_SIZE;
...
void npu_print_config(void)
{
	...
	probe_info("%s : %u, %s : %u, %s : %u\n"
		, cfg_name[DSP_WAIT_MODE], configs[DSP_WAIT_MODE]
		, cfg_name[DSP_WAIT_RESET], configs[DSP_WAIT_RESET]
		, cfg_name[DSP_WAIT_BOOT], configs[DSP_WAIT_BOOT]);
#endif
#if IS_ENABLED(CONFIG_NPU_IMB_THRESHOLD)
	probe_info("%s : %u\n",
		cfg_name[NPU_IMB_THRESHOLD_SIZE], configs[NPU_IMB_THRESHOLD_SIZE]);
#endif
#if IS_ENABLED(CONFIG_NPU_USE_LLC)
	probe_info("%s : %u, %s : %u, %s : %u, %s : %u\n"
		, cfg_name[NPU0_LLC_BOOST_BUFFER_SIZE], configs[NPU0_LLC_BOOST_BUFFER_SIZE]
		, cfg_name[NPU1_LLC_BOOST_BUFFER_SIZE], configs[NPU1_LLC_BOOST_BUFFER_SIZE]
		, cfg_name[NPU2_LLC_BOOST_BUFFER_SIZE], configs[NPU2_LLC_BOOST_BUFFER_SIZE]
