...
static void destroy_npu_golden(void)
{
	...
	npu_dbg("removed %d file entries.\n", cnt);
...
static int npu_golden_set_open(struct inode *inode, struct file *file)
{
	...
	npu_dbg("start, npu_golden_ctx = @%pK\n", &npu_golden_ctx);

	/* Destroy last golden setting */
	if (npu_golden_ctx.data.flags.valid) {
		npu_dbg("Purge existing golden data....\n");
		destroy_npu_golden();
	}
	/* Clear old data */
...
static int set_frame_id(struct parse_buf *parse_buf)
{
	...
	if (ret) {
		npu_err("invalid frame id (%s) specified on line (%d), ret = %d\n",
			parse_buf->token_char,
			npu_golden_ctx.data.parse_pos.line, ret);
		return ret;
	}
...
static int set_net_id(struct parse_buf *parse_buf)
{
	...
	if (ret) {
		npu_err("invalid net id [%s] specified on line %d, ret = %d\n",
			parse_buf->token_char,
			npu_golden_ctx.data.parse_pos.line, ret);
		return ret;
	}
...
static int set_hdr_param(const char ch, struct parse_buf *parse_buf)
{
	...
	if (ret) {
		npu_err("value setter failed on line (%d), ret = %d\n", npu_golden_ctx.data.parse_pos.line, ret);
		return ret;
	}
...
static int __npu_golden_compare(const struct npu_frame *frame)
{
	...
	npu_dbg("starting with frame @ %pK, result at %pK\n [cur=%zu len=%zu cop=%zu]",
...
int register_golden_matcher(struct device *dev)
{
	...
	probe_info("initialize statekeeper in %s\n", __func__);
	npu_statekeeper_initialize(&npu_golden_ctx.statekeeper,
				   NPU_GOLDEN_STATE_INVALID,
				   state_names, transition_map);

	probe_info("%s: Register " SET_GOLDEN_DESC_DEBUGFS_NAME " to debugfs\n", __func__);
	...
	probe_info("%s: Register " RESULT_GOLDEN_MATCH_DEBUGFS_NAME " to debugfs\n", __func__);
