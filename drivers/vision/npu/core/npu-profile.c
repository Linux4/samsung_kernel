 ...
 *****************************************************************************/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)
static void npu_profile_print_time(const u32 point_id, const u32 uid,
		const u32 frame_id, const u32 p0, const u32 p1, const u32 p2)
{
	struct timespec64 ts = ns_to_timespec64(ktime_get_real());

	npu_dbg(S_PROFILE_PREFIX "[%08lu]s[%09lu]ns id:%u uid:%u fid:%u p[] = [%u %u %u]\n",
		ts.tv_sec, ts.tv_nsec, point_id, uid, frame_id, p0, p1, p2);
}
#else
static void npu_profile_print_time(const u32 point_id, const u32 uid,
		const u32 frame_id, const u32 p0, const u32 p1, const u32 p2)
{
	...
	npu_dbg("S-PROFILE: [%08lu]s[%06lu]us id:%u uid:%u fid:%u p[] = [%u %u %u]\n",
		tv.tv_sec, tv.tv_usec, point_id, uid, frame_id, p0, p1, p2);
}
#endif

/* Set initial value of profile_data */
static void init_profile_data(struct npu_memory_buffer *buf, struct npu_profile *profile_data)
{
	...
	/* Cache flush */
	dma_sync_sg_for_device(buf->attachment->dev,
		buf->sgt->sgl, buf->sgt->orig_nents, DMA_TO_DEVICE);
...
static int alloc_profile_data(struct npu_profile_control *profile_ctl)
{
	...
	ret = npu_memory_alloc(&system->memory, &profile_ctl->buf, configs[NPU_PBHA_HINT_00]);
	if (ret) {
	...
	init_profile_data(&profile_ctl->buf, profile_ctl->profile_data);
	ret = 0;
	...
	nw_cmd_e cmd, struct npu_memory_buffer *buf_handle)
{
	...
	case NPU_NW_CMD_PROFILE_START:
		if (buf_handle) {
			...
			probe_info("prepare profile start command: cmd(%u) v(" NPU_KPTR_LOG ") d(%pad) size(%zu)\n",
				nw.cmd, nw.ncp_addr.vaddr, &nw.ncp_addr.daddr, nw.ncp_addr.size);
		} else {
			probe_err("buf_handle is null on profile start command.\n");
			ret = -EINVAL;
			goto err_exit;
		}
		break;
	...
	case NPU_NW_CMD_PROFILE_STOP:
		probe_info("prepare profile stop command: cmd(%u)\n", nw.cmd);
		break;
	default:
		probe_err("invalid cmd(%d)\n", cmd);
		ret = -EINVAL;
		goto err_exit;
	}
	...
	while ((ret = npu_ncp_mgmt_put(&nw)) <= 0) {
		probe_info("queue full when insearting profile control message. Retrying...");
		if (retry_cnt++ >= PROFILE_CMD_POST_RETRY_CNT) {
			probe_err("timeout exceeded.\n");
			ret = -EWOULDBLOCK;
			goto err_exit;
		}
	...
	/* Success */
	probe_info("profile control message has posted\n");
	...
	if (ret < 0) {
		probe_err("wait_event_timeout error(%d)\n", ret);
		goto err_exit;
	}
	if (!atomic_read(&profile_ctl->result_available)) {
		probe_err("timeout waiting H/W response\n");
		ret = -ETIMEDOUT;
		goto err_exit;
	}
	if (profile_ctl->result_code != NPU_ERR_NO_ERROR) {
		probe_err("hardware reply with NDONE(%d)\n", profile_ctl->result_code);
		ret = -EFAULT;
		goto err_exit;
	}
	ret = 0;
...
static int npu_profile_stop(struct npu_profile_control *profile_ctl)
{
	...
	probe_info("start in npu_profile_stop.\n");
	...
	if (SKEEPER_COMPARE_STATE(&profile_ctl->statekeeper, NPU_PROFILE_STATE_PWRON_WAITING)) {
		...
		probe_info("postponed starting is canceled by stop request.\n");
		npu_statekeeper_transition(&profile_ctl->statekeeper, NPU_PROFILE_STATE_NOT_INITIALIZED);
		ret = 0;
		goto ok_exit;
	}
	...
	probe_info("sending NPU_NW_CMD_PROFILE_STOP to NPU H/W.\n");
	ret = request_profile_ctl_to_hw(profile_ctl, NPU_NW_CMD_PROFILE_STOP, NULL);
	if (ret) {
		probe_err("fail(%d) in request_profile_ctl_to_hw\n", ret);
		goto err_exit;
	}
	...
	dma_sync_sg_for_cpu(profile_ctl->buf.attachment->dev,
			profile_ctl->buf.sgt->sgl,
			profile_ctl->buf.sgt->orig_nents, DMA_FROM_DEVICE);

	probe_info("fw profile id(%u), start(%u) total(%u) valid(%u)\n",
	...
	probe_info("fw profile invalidate addr(" NPU_KPTR_LOG "), size(%ld)\n",
...
err_exit:
	/* Stop failed */
	probe_err("stopping failed. clean-up profile data.\n");
	npu_profile_clear(profile_ctl);

ok_exit:
	update_current_profile_func();
	probe_info("complete in npu_profile_stop\n");
	return ret;
}
...
int npu_profile_probe(struct npu_system *system)
{
	...
	profile_ctl->pwm = npu_get_io_area(system, "sfrpwm")->vaddr + configs[TCNTO0_OFF];
...
int npu_profile_release(void)
{
	...
	probe_info("start in npu_profile_release\n");
	...
	if (SKEEPER_COMPARE_STATE(&profile_ctl->statekeeper, NPU_PROFILE_STATE_EXPORT_READY)) {
		probe_info("profiling data is remaining. cleaning it first.");
		npu_profile_clear(profile_ctl);
	}
	...
	const u32 p0, const u32 p1, const u32 p2)
{
	...
	if (is_s_profile_enabled())
		npu_profile_print_time(point_id, uid, frame_id, p0, p1, p2);
