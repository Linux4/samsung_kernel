...
static int alloc_profile_data(struct npu_profile_control *profile_ctl)
{
	...
	ret = npu_memory_alloc(&system->memory, &profile_ctl->buf, npu_get_configs(NPU_PBHA_HINT_00));
	if (ret) {
		npu_err("npu_memory_alloc for profile buffer memory, ret(%d)\n", ret);
		goto err_exit;
	}
	npu_dbg("profiling buffer allocated, size(%lu) / dv(%pad) / kv(" NPU_KPTR_LOG ")",
...
int npu_profile_probe(struct npu_system *system)
{
	...
	profile_ctl->pwm = npu_get_io_area(system, "sfrpwm")->vaddr + npu_get_configs(TCNTO0_OFF);
