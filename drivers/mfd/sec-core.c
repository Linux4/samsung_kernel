...
static struct mfd_cell s2mpu01_devs[] = {
	{
		.name = "s2mpu01-pmic",
	}, {
		.name = "s2m-rtc",
...
static struct mfd_cell s2mpu01a_devs[] = {
	{
		.name = "s2mpu01a-pmic",
	}, {
		.name = "s2m-rtc",
...
static struct mfd_cell s2mpa01_devs[] = {
	{
		.name = "s2mpa01-pmic",
	}, {
		.name = "s2m-rtc",
...
static struct mfd_cell s2mpu02x_devs[] = {
	{
		.name = "s2mpu02x-pmic",
	}, {
		.name = "s2m-rtc",
			    ...
			    const struct i2c_device_id *id)
{
	...
	case S2MPS11X:
		...
		break;
	case S2MPU01X:
		ret = mfd_add_devices(sec_pmic->dev, -1, s2mpu01_devs,
					ARRAY_SIZE(s2mpu01_devs), NULL, 0);
		break;
	case S2MPU01A:
		ret = mfd_add_devices(sec_pmic->dev, -1, s2mpu01a_devs,
					ARRAY_SIZE(s2mpu01a_devs), NULL, 0);
		break;
	case S2MPA01:
		ret = mfd_add_devices(sec_pmic->dev, -1, s2mpa01_devs,
					ARRAY_SIZE(s2mpa01_devs), NULL, 0);
		break;
	case S2MPS14:
		ret = mfd_add_devices(sec_pmic->dev, -1, s2mps14_devs,
					ARRAY_SIZE(s2mps14_devs), NULL, 0);
		break;
	case S2MPU02X:
		ret = mfd_add_devices(sec_pmic->dev, -1, s2mpu02x_devs,
					ARRAY_SIZE(s2mpu02x_devs), NULL, 0);
		break;
	default:
