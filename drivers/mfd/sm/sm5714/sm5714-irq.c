...
static irqreturn_t sm5714_irq_thread(int irq, void *data)
{
	...
	if (irq_src & SM5714_IRQSRC_CHG) {
		ret = sm5714_bulk_read(sm5714->charger, SM5714_CHG_REG_INT1,
								SM5714_NUM_IRQ_CHG_REGS, &irq_reg[CHG_INT1]);
		if (ret) {
	...
	if (irq_src & SM5714_IRQSRC_MUIC) {
		ret = sm5714_bulk_read(sm5714->muic, SM5714_MUIC_REG_INT1,
								SM5714_NUM_IRQ_MUIC_REGS, &irq_reg[MUIC_INT1]);
