			    ...
			    unsigned int mask, unsigned int target)
{
	...
	for (i = 0; i < timeout; i++) {
		...
		printk("arizona_poll_reg()!reg = %d, val = %d \n", reg, val);
		if (ret != 0) {
...
static int arizona_of_get_core_pdata(struct arizona *arizona)
{
	...
	ear_switch_gpio = of_get_named_gpio(arizona->dev->of_node,
                 "wlf,ear_switch_ena", 0);

	if (gpio_is_valid(ear_switch_gpio)) {
	    if (gpio_request(ear_switch_gpio, "wlf,ear_switch_ena"))
	        printk("Failed to request ear_switch_gpio\n");
	}
	gpio_direction_output(ear_switch_gpio, 1);
	__gpio_set_value(ear_switch_gpio, 1);
...
static struct mfd_cell cs47l15_devs[] = {
	{ .name = "arizona-extcon" },  //wangqin 0727 temp
	{ .name = "arizona-gpio" },
	{ .name = "arizona-haptics" },
	{ .name = "arizona-pwm" },
	{ .name = "cs47l15-codec" },
...
int arizona_dev_init(struct arizona *arizona)
{
	...
	printk("arizona_dev_init()2222  arizona->type = %d  \n", arizona->type);
