...
int s2mpa05_read_reg(struct i2c_client *i2c, uint8_t reg, uint8_t *dest)
{
	...
	ret = exynos_acpm_read_reg(acpm_mfd_node, EXTRA_CHANNEL, i2c->addr, reg, dest);
...
int s2mpa05_bulk_read(struct i2c_client *i2c, uint8_t reg, int count, uint8_t *buf)
{
	...
	mutex_lock(&s2mpa05->i2c_lock);
	ret = exynos_acpm_bulk_read(acpm_mfd_node, EXTRA_CHANNEL, i2c->addr, reg, count, buf);
	mutex_unlock(&s2mpa05->i2c_lock);
	if (ret) {
		pr_err("[%s] acpm ipc fail!\n", __func__);
		return ret;
...
int s2mpa05_write_reg(struct i2c_client *i2c, uint8_t reg, uint8_t value)
{
	...
	mutex_lock(&s2mpa05->i2c_lock);
	ret = exynos_acpm_write_reg(acpm_mfd_node, EXTRA_CHANNEL, i2c->addr, reg, value);
	mutex_unlock(&s2mpa05->i2c_lock);
	if (ret) {
		pr_err("[%s] acpm ipc fail!\n", __func__);
		return ret;
...
int s2mpa05_bulk_write(struct i2c_client *i2c, uint8_t reg, int count, uint8_t *buf)
{
	...
	mutex_lock(&s2mpa05->i2c_lock);
	ret = exynos_acpm_bulk_write(acpm_mfd_node, EXTRA_CHANNEL, i2c->addr, reg, count, buf);
	mutex_unlock(&s2mpa05->i2c_lock);
	if (ret) {
		pr_err("[%s] acpm ipc fail!\n", __func__);
		return ret;
...
int s2mpa05_update_reg(struct i2c_client *i2c, uint8_t reg, uint8_t val, uint8_t mask)
{
	...
	mutex_lock(&s2mpa05->i2c_lock);
	ret = exynos_acpm_update_reg(acpm_mfd_node, EXTRA_CHANNEL, i2c->addr, reg, val, mask);
	mutex_unlock(&s2mpa05->i2c_lock);
	if (ret) {
		pr_err("[%s] acpm ipc fail!\n", __func__);
		return ret;
...
static int s2mpa05_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *dev_id)
{
	...
	if (i2c->dev.of_node) {
		pdata = devm_kzalloc(&i2c->dev,
				     sizeof(struct s2mpa05_platform_data),
				     GFP_KERNEL);
		if (!pdata)
			return -ENOMEM;
	...
	ret = s2mpa05_read_reg(i2c, S2MPA05_REG_COMMON_CHIP_ID, &reg_data);
	if (ret < 0) {
		dev_err(s2mpa05->dev,
	...
	s2mpa05->vgpio = devm_i2c_new_dummy_device(s2mpa05->dev, i2c->adapter, I2C_ADDR_TEMP);
	s2mpa05->vgpio->addr = I2C_ADDR_VGPIO;
	s2mpa05->pmic = devm_i2c_new_dummy_device(s2mpa05->dev, i2c->adapter, I2C_ADDR_PMIC);
	s2mpa05->close = devm_i2c_new_dummy_device(s2mpa05->dev, i2c->adapter, I2C_ADDR_CLOSE);
	s2mpa05->gpio = devm_i2c_new_dummy_device(s2mpa05->dev, i2c->adapter, I2C_ADDR_GPIO);
	...
	ret = devm_mfd_add_devices(s2mpa05->dev, -1, s2mpa05_devs,
			      ARRAY_SIZE(s2mpa05_devs), NULL, 0, NULL);
	if (ret < 0)
		goto err_w_lock;
