...
static int pmic_irq_suspend(struct platform_device *pdev, pm_message_t state)
{
	suspended = true;
	return 0;
}

static int pmic_irq_resume(struct platform_device *pdev)
{
	suspended = false;
	return 0;
}
...
#ifdef CONFIG_SEC_PM
	.suspend = pmic_irq_suspend,
	.resume = pmic_irq_resume,
#endif
};
