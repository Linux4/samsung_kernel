...
static bool uart_enable_rustproof;
#endif
...
#if defined(CONFIG_MUIC_SUPPORT_RUSTPROOF)
static int sm5504_bat_check(void)
{
	int vbat_on = 1;
#ifdef CONFIG_OF
	int rustproof_on;
	...
	np = of_find_node_by_name(NULL, "sprd-rustproof");

	if (np) {
		ret = of_property_read_u32(np, "rustproof_por_src_flag",
					&rustproof_on);
		if (ret) {
		...
		pr_info("%s rustproof_on: %d\n", __func__, rustproof_on);

		if (rustproof_on)
		{
			if(BITS_POR_SRC_FLAG(sci_adi_read(ANA_REG_GLB_POR_SRC_FLAG)) & 0x08)
			{
				vbat_on = 0; 
				pr_info("JIG ON check for rustproof vbat_on:%d\n", vbat_on);
			}
		}
		return vbat_on;
	}
				       ...
				       unsigned int new_status)
{
    ...
    if (sec_vf_adc_current_check() && !enable_muic_uart && !uart_enable_rustproof)
#else
    if (sm5504_bat_check() && !enable_muic_uart && !uart_enable_rustproof)
#endif
	{
		sm5504_reg_write(chip, SM5504_REG_MANUAL_SW1, 0x00);
		sm5504_clr_bits(chip, SM5504_REG_CONTROL, 1 << 2);
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct sm5504_chip *chip = dev_get_drvdata(dev);
	
	/* is_rustproof is false, UART enabled.*/
	return sprintf(buf, "%d\n", uart_enable_rustproof);
}
					...
					const char *buf, size_t size)
{
	...
	if (!strncmp (buf, "1", 1)) {
		/*  UART enabled */
		uart_enable_rustproof = true;
	} else {
		/* UART disabled */
		uart_enable_rustproof = false;
	}
	return size;
}
