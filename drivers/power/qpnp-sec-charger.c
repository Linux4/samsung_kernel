...
#endif
#ifdef CONFIG_CMCC_MTBF_CHG_BLOCK
	bool batt_chg_on;
#endif
};
...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	if ((usb_ov_sts & 0x80) == 0x80) {
		...
		#ifdef CONFIG_CMCC_MTBF_CHG_BLOCK
		if (chip->batt_chg_on && chip->cable_type == CABLE_TYPE_USB)
...
#if defined(CONFIG_MACH_CS03_SGLTE)
	chip->batt_pdata->temp_low_recover_event = 0 - chip->batt_pdata->temp_low_recover_event;
#endif
...
#if defined(CONFIG_MACH_CS03_SGLTE)
	chip->batt_pdata->temp_low_recover_normal = 0 - chip->batt_pdata->temp_low_recover_normal;
#endif
...
#if defined(CONFIG_MACH_CS03_SGLTE)
	chip->batt_pdata->temp_low_recover_lpm = 0 - chip->batt_pdata->temp_low_recover_lpm;
#endif
#endif
        if (rc)
...
#ifdef CONFIG_CMCC_MTBF_CHG_BLOCK
        case BATT_CHG_ON:
		pr_info("%s : BATT_CHG_ON %s\n", __func__, buf);
		if (sscanf(buf, "%2d\n", &x) == 1) {
			if (x == 1) {
				chip->batt_chg_on = true;
				chip->cable_type = CABLE_TYPE_NONE;
				pr_debug("batt_chg_on, charging stop \n");
				sec_handle_cable_insertion_removal(chip);
			} else if (x == 0) {
				chip->batt_chg_on = false;
				chip->cable_type = msm8930_get_cable_status();
				pr_debug("batt_chg_on, charging start \n");
				sec_handle_cable_insertion_removal(chip);
			} else {
				pr_info("%s:Invalid\n", __func__);
			}
...
static void sec_handle_cable_insertion_removal(struct qpnp_chg_chip *chip)
{
	...
	if (chip->cable_type != CABLE_TYPE_NONE && !chip->batt_present) {
		sec_pm8226_stop_charging(chip);
		chip->batt_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
...
static void sec_bat_monitor(struct work_struct *work)
{
	...
	if ((chip->cable_type == CABLE_TYPE_USB ||
		...
		(chip->batt_health == POWER_SUPPLY_HEALTH_GOOD)) {
			...
			#else
#if defined(CONFIG_MACH_MS01_LTE) || defined(CONFIG_MACH_MS01_CHN_SGLTE)
			if (batt_voltage < 4310)
#elif defined(CONFIG_MACH_CS03_SGLTE)
			if (batt_voltage < 4310 ||batt_capacity <= 99)
#else
