...
static void s2mu003_set_mivr_level(struct s2mu003_charger_data *charger)
{
	...
	mutex_lock(&charger->io_lock);
	s2mu003_assign_bits(charger->i2c_client, S2MU003_CHG_CTRL4,
					...
					bool enable)
{
	...
	if (!enable) {
		...
		if (charger->rev_id == 0x02 || charger->rev_id == 0x0a)
			s2mu003_clr_bits(charger->client, 0x8d, 0x1 << 7);
	}
}
...
static void s2mu003_configure_charger(struct s2mu003_charger_data *charger)
{
	...
	pr_debug("%s : Set config charging\n", __func__);
	...
	pr_debug("%s : termination current (%dmA)\n",
			__func__, eoc);
...
static bool s2mu003_chg_init(struct s2mu003_charger_data *charger)
{
	...
	ret = s2mu003_reg_read(charger->client, 0xa5);
	if (ret < 0)
		pr_err("%s : failed to read revision ID 0xa5\n", __func__);

	rev_id = (ret & 0xc0) >> 4;

	ret = s2mu003_reg_read(charger->client, 0xaf);
	if (ret < 0)
		pr_err("%s : failed to read revision ID 0xaf\n", __func__);
	...
	if (charger->rev_id == 0x02 || charger->rev_id == 0x0a) {
		ret = s2mu003_set_bits(charger->client, S2MU003_CHG_CTRL1,
						 S2MU003_SEL_SWFREQ_MASK);
		ret = s2mu003_clr_bits(charger->client, 0x8d, 0x1 << 7);
		ret = s2mu003_clr_bits(charger->client, 0x64, 0x1 << 3);
		ret = s2mu003_clr_bits(charger->client, 0x75, 0x1);
	}
	...
	/* s2mu003_assign_bits(charger->s2mu003->i2c_client,
					0x18, 0x3C, 0x30); */
	...
	charger->battery_valid = (ret & S2MU003_CHG_BATP) ? false : true;
#if !(ENABLE_MIVR)
	s2mu003_assign_bits(charger->client,
		...
		enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (!charger->battery_valid)
			...
			if (s2mu003_get_charging_status(charger)
		...
		unsigned long action, void *data)
{
	...
	if ((cable_type >= 0) &&
		...
		if (cable_type != charger->battery_cable_type) {
			...
			pr_debug("%s: Cable is Not Changed(%d)\n",
...
static void sec_bat_get_battery_info(struct work_struct *work)
{
	...
	pr_debug("%s, battery_info: voltage_now: (%d), "
		"voltage_avg: (%d), voltage_ocv: (%d), capacity: (%d)\n",
	...
	if (!charger->battery_valid) {
		...
		if (charger->rev_id == 0x02 || charger->rev_id == 0x0a) {
			if (charger->battery_valid) {
				s2mu003_clr_bits(charger->client, 0x64,
							      0x1 << 3);
...
#endif
	charger->psy_battery.name = "battery";
