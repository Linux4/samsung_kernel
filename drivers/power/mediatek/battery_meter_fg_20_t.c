...
signed int battery_meter_get_charger_voltage(void)
{
	...
	/* FIXME: workaround for TC10SP bring up */
	battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_CHARGER, &val);
#endif
	return val;
}
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	if (g_use_mtk_fg) {
...
#if defined(FG_BAT_ZCV_INT)
		bm_notice("[FG_BAT_ZCV_INT]register\n");
		...
		bm_notice("[BAT_probe] power_supply_register\n");
		mtk_fuelgauge.psy = power_supply_register(&(dev->dev), &mtk_fuelgauge.psd, NULL);

		if (IS_ERR(mtk_fuelgauge.psy)) {
			bm_err("[BAT_probe] power_supply_register AC Fail !!\n");
			ret = PTR_ERR(mtk_fuelgauge.psy);
			return ret;
		}
	}
