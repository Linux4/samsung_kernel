...
int __batt_meter_init_cust_data_from_dt(void)
{
	...
	__batt_meter_parse_node(np, "batt_temperature_table_size", &num);
	...
	__batt_meter_parse_node(np, "r_profile_t1_size", &num);
	...
	__batt_meter_parse_node(np, "r_profile_t2_size", &num);
	...
	__batt_meter_parse_node(np, "r_profile_t3_size", &num);
...
void dod_init(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[FGADC] get_hw_ocv=%d, HW_SOC=%d, SW_SOC = %d\n",
	...
	gFG_capacity_by_v = gFG_capacity_by_v_init;
	battery_log(BAT_LOG_CRTI, "[FGADC] HW_FG_FORCE_USE_SW_OCV : HW_SOC=%d, SW_SOC = %d\n",
		 gFG_capacity_by_v, gFG_capacity_by_v_init);
#endif
	/* ------------------------------------------------------------------------------- */
#endif
	...
	if (is_battery_remove_pmic() == 0 && (g_rtc_fg_soc != 0)) {
		battery_log(BAT_LOG_CRTI, "[FGADC]is_battery_remove()==0 , use rtc_fg_soc%d\n",
			 g_rtc_fg_soc);
		gFG_capacity_by_v = g_rtc_fg_soc;
	} else {
...
if (((g_rtc_fg_soc != 0)
	...
	battery_log(BAT_LOG_CRTI, "[FGADC] g_rtc_fg_soc=%d, gFG_capacity_by_v=%d\n",
		 g_rtc_fg_soc, gFG_capacity_by_v);

	if (gFG_capacity_by_v == 0 && bat_is_charger_exist() == KAL_TRUE) {
		gFG_capacity_by_v = 1;

		battery_log(BAT_LOG_CRTI, "[FGADC] gFG_capacity_by_v=%d\n", gFG_capacity_by_v);
	}
	...
	battery_log(BAT_LOG_CRTI, "[FGADC] gFG_DOD0=%d\n", gFG_DOD0);
	...
	if (batt_meter_cust_data.change_tracking_point) {
		...
		battery_log(BAT_LOG_CRTI, "[FGADC] gFG_15_vlot = %dmV\n", gFG_15_vlot);
	} else {
		/* gFG_15_vlot = fgauge_read_v_by_capacity(86); //14% */
		gFG_15_vlot = fgauge_read_v_by_capacity((100 - g_tracking_point));
		battery_log(BAT_LOG_CRTI, "[FGADC] gFG_15_vlot = %dmV\n", gFG_15_vlot);
		if ((gFG_15_vlot > 3800) || (gFG_15_vlot < 3600)) {
			battery_log(BAT_LOG_CRTI, "[FGADC] gFG_15_vlot(%d) over range, reset to 3700\n",
				 gFG_15_vlot);
			gFG_15_vlot = 3700;
		}
	}
