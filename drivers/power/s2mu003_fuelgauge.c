...
static int s2mu003_read_block_reg(struct i2c_client *client, int reg, u8 *buf)
{
	int ret;
...
static int s2mu003_write_block_reg(struct i2c_client *client, int reg, u8 *buf)
{
	int ret;
...
#if 0
static int s2mu003_read_reg(struct i2c_client *client, int reg)
{
	int ret;
...
static int s2mu003_init_regs(struct s2mu003_fuelgauge_data *fuelgauge)
{
	...
	ret = s2mu003_write_reg(fuelgauge->i2c,
			       S2MU003_REG_START, 0x03);
...
static void s2mu003_alert_init(struct s2mu003_fuelgauge_data *fuelgauge)
{
	...
	data[1] = 0x00;
	s2mu003_write_block_reg(fuelgauge->i2c,
			S2MU003_REG_IRQ_LVL, data);
}
...
static bool s2mu003_check_status(struct i2c_client *client)
{
	...
	if (s2mu003_read_block_reg(client, S2MU003_REG_STATUS, data) < 0)
		return ret;
			...
			int temperature)
{
	...
	data[0] = val;
	data[1] = 0x00;
	s2mu003_write_block_reg(fuelgauge->i2c, S2MU003_REG_RTEMP, data);
...
static int s2mu003_get_temperature(struct s2mu003_fuelgauge_data *fuelgauge)
{
	...
	if (s2mu003_read_block_reg(fuelgauge->i2c,
			S2MU003_REG_RTEMP, data) < 0)
		return -ERANGE;
...
static int s2mu003_get_soc(struct s2mu003_fuelgauge_data *fuelgauge)
{
	...
	for (i = 0; i < 50; i++) {
		if (s2mu003_read_block_reg(fuelgauge->i2c,
				S2MU003_REG_RSOC, data) < 0)
			return -EINVAL;
		if (s2mu003_read_block_reg(fuelgauge->i2c,
				S2MU003_REG_RSOC, check_data) < 0)
			return -EINVAL;
	...
	dev_dbg(&fuelgauge->i2c->dev, "%s: raw capacity (0x%x:%d)\n", __func__,
		compliment, rsoc);
...
bool s2mu003_fuelgauge_fuelalert_init(struct i2c_client *client, int soc)
{
	...
	if (s2mu003_read_block_reg(client, S2MU003_REG_IRQ, data) < 0)
		return -1;
	...
	s2mu003_write_block_reg(client, S2MU003_REG_IRQ, data);
...
bool s2mu003_hal_fg_fuelalert_process(void *irq_data, bool is_fuel_alerted)
{
	...
	ret = s2mu003_write_reg(fuelgauge->i2c, S2MU003_REG_IRQ, 0x00);
	if (ret < 0)
		dev_err(&fuelgauge->i2c->dev, "%s: Error(%d)\n", __func__, ret);
...
#ifdef CONFIG_OF
static int s2mu003_fuelgauge_parse_dt(struct device *dev,
		struct s2mu003_fuelgauge_data *fuelgauge)
{
...
MODULE_DEVICE_TABLE(of, s2mu003_fuelgauge_match_table);
#else
...
static int __devinit s2mu003_fuelgauge_probe(struct i2c_client *i2c,
			    const struct i2c_device_id *id)
{
...
#ifdef CONFIG_OF
	fuelgauge->pdata = devm_kzalloc(&i2c->dev,
	...
	ret = s2mu003_fuelgauge_parse_dt(&i2c->dev, fuelgauge);
	if (ret < 0)
		goto err_parse_dt;
#else
	goto err_parse_dt;
	...
	i2c_set_clientdata(i2c, fuelgauge);
	...
	s2mu003_write_block_reg(fuelgauge->i2c, S2MU003_REG_RTEMP, temp);
	...
	ret = power_supply_register(&i2c->dev, &fuelgauge->psy_fg);
	if (ret) {
...
static struct i2c_driver s2mu003_fuelgauge_driver = {
	...
	.remove = __devexit_p(s2mu003_fuelgauge_remove),
