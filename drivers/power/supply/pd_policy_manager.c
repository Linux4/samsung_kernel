...
/* +churui1.wt, ADD, 20230602, CP charging control */
		pr_notice("g_cp_charging.cp_stop %d, set_cp_stop %d\n", g_cp_charging.cp_stop, set_cp_stop);
		if (g_cp_charging.cp_stop && set_cp_stop == 0) {
			set_cp_stop = 1;
...
/* -churui1.wt, ADD, 20230602, CP charging control */
	...
	case PD_PM_STATE_FC2_EXIT:
		...
		pr_err(">>>leave CP charging %d\n", recover);

		if (recover) {
			usbpd_pm_move_state(pdpm, PD_PM_STATE_ENTRY);
		} else {
			g_cp_charging.cp_leave = 1; //leave cp charging
			rc = 1;
		}
...
/* +churui1.wt, ADD, 20230602, CP charging control */
	case PD_PM_STATE_STOP_CHARGING:
		if (!g_cp_charging.cp_stop && set_cp_stop) {
			set_cp_stop = 0;
			usbpd_pm_move_state(pdpm, PD_PM_STATE_ENTRY);
		}
		break;
...
static void usbpd_pd_contact(struct usbpd_pm *pdpm, bool connected)
{
	...
	if (connected) {
		...
		pr_err("start cp charging pps support %d\n",
			pdpm->pps_supported);
		if (pdpm->pps_supported) {
			g_cp_charging.cp_leave = 0; //start cp charging
