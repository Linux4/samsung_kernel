...
static bool s2mu107_sc_get_batt_present(struct s2mu107_sc_data *sw_charger)
{
	...
	ret = s2mu107_read_reg(sw_charger->i2c, S2MU107_SC_STATUS5, &reg_data);
	if (ret < 0)
		return true;

	return (ret & 0x80) ? false : true;
}
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		val->intval = s2mu107_sc_get_batt_present(sw_charger);
		break;
...
static irqreturn_t s2mu107_sc_fault_isr(int irq, void *data)
{
	...
	*/
	...
	case CHG_STATUS_WD_RST:
		/* 1,3 */
		pr_info("%s: wdt AP reset, USB PD reset\n", __func__);
		psy = power_supply_get_by_name("s2mu107-usbpd");
		if (!psy)
			pr_err("%s, fail to set topoff current to FG\n", __func__);
		else {
