...
int power_supply_uevent(struct device *dev, struct kobj_uevent_env *env)
{
	...
	dev_info(dev, "STANLEY1 : POWER_SUPPLY_NAME=%s : %d \n", psy->desc->name, psy->desc->num_properties);

	ret = add_uevent_var(env, "POWER_SUPPLY_NAME=%s", psy->desc->name);
	if (ret)
		return ret;

	dev_info(dev, "STANLEY2 : POWER_SUPPLY_NAME=%s : %d \n", psy->desc->name, psy->desc->num_properties);
	...
	for (j = 0; j < psy->desc->num_properties; j++) {
		...
		dev_info(dev, "STANLEY2.5 : attr->attr.name %d %s\n", j, attr->attr.name);
		...
		dev_info(dev, "STANLEY3 : POWER_SUPPLY_%s=%s\n", attrname, prop_buf);
		kfree(attrname);
		if (ret)
			goto out;
	}
