...
//- SS_charging, add battery_cycle node
//bug528143,zhaosidong.wt,ADD,20200109,skip soc limit
...
static enum power_supply_property battery_props[] = {
	...
	//+Bug492301,lili5.wt,ADD,20191021,online charger report
...
//-Bug492299,lili5.wt,ADD,20191021,battery Current event and slate mode
//Bug492298,lili5.wt,ADD,20191119,hv charger status
	...
	union power_supply_propval *val)
{
	...
	int fgcurrent = 0;
	bool b_ischarging = 0;
	int chr_type;//Bug492299,lili5.wt,ADD,20191021,battery Current event and slate mode
	struct charger_manager *cm = gm.pbat_consumer->cm; //Bug492298,lili5.wt,ADD,20191119,hv charger status
...
//EXTB P200401-03210,zhaosidong.wt,ADD,20200423,hv disable funtion
		if (batt_hv_disable) {
			val->intval = 0;
		} else {
			chr_type = mt_get_charger_type();
...
#if defined(CONFIG_BATTERY_N6)
		pr_notice_ratelimited("N6 doesn't support hv charger (%d)\n", val->intval);
		val->intval = 0;
#endif
		}
		break;
	...
	case POWER_SUPPLY_PROP_BATTERY_CYCLE:
		...
		break;
	//- SS_charging, add battery_cycle node
	//Bug522914,zhaosidong.wt,ADD,20191231, property charge type
...
{
	switch (psp) {
	case POWER_SUPPLY_PROP_BATT_SLATE_MODE://+Bug492299,lili5.wt,ADD,20191021,battery Current event and slate mode
	case POWER_SUPPLY_PROP_STORE_MODE:
	//+ SS_charging, add battery_cycle node
	case POWER_SUPPLY_PROP_BATTERY_CYCLE:
	//- SS_charging, add battery_cycle node
...
struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret_value = mtk_battery_get_id();

	bm_err("[EM] battery_id : %d\n", ret_value);
	return sprintf(buf, "%d\n", ret_value);
}

static ssize_t store_battery_id(
	struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	bm_err("[EM] Not Support Write Function\n");
...
static DEVICE_ATTR(
	battery_id, 0664,
	show_battery_id, store_battery_id);

extern bool g_ignore_swap;
...
/* STOP CHARGING */
			fg_sw_bat_cycle_accu();
//EXTB P200422-05185,zhaosidong.wt,ADD,20200424,DRS make phone discharging
			if (g_ignore_swap) {
				bm_err("ignore_swap keep charging\n");
			} else {
