...
void fgauge_get_profile_id(void)
{
	...
	_battery_id_gpio = of_get_named_gpio(batterty_node, "BatteryID_GPIO", 0);
	if (_battery_id_gpio < 0) {
		gm.battery_id = 0;
		bm_err("[%s] not find BatteryID-gpio. Defaul battery ID = %d\n", gm.battery_id);
		return;
	}

	/* gpio_direction_input(_battery_id_gpio); */
	/* A22e-5G >> 1(high):SDI battery, 0(low):ATL:battery */
	_battery_id = gpio_get_value(_battery_id_gpio);
	bm_err("[%s]GPIO Battery id (%d) (battery_id_gpio=%d)\n",
		__func__,
		_battery_id, _battery_id_gpio);

	gm.battery_id = _battery_id;
}
#else
void fgauge_get_profile_id(void)
{
