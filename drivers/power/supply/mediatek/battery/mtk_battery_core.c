...
#endif
/* A06 code for SR-AL7160A-01-233 by zhangziyi at 20240409 start */
...
void fgauge_get_profile_id(void)
{
	...
	if (id_volt >= 513 && id_volt <= 627) {
		gm.battery_id = SS_BATTERY_SDI;
	} else if (id_volt >= 1080 && id_volt <= 1320) {
		gm.battery_id = SS_BATTERY_BYD;
	} else if (id_volt >= 810 && id_volt <= 990) {
		gm.battery_id = SS_BATTERY_ATL;
	} else if (id_volt >= 250 && id_volt <= 400){
		gm.battery_id = SS_BATTERY_GF;
	} else {
		bm_err("bat_id get fail, load default id");
		gm.battery_id = SS_BATTERY_SDI;
	}
...
/* A06 code for SR-AL7160A-01-233|AL7160A-2224 by zhangziyi at 20240601 end */
#endif
