...
static int gpio_afc_recv_data(struct gpio_afc_ddata *ddata)
{
	...
	return gpio_afc_check_sping(ddata);
#else
	 bool ack = false;
	 int gpio = ddata->pdata->gpio_afc_data;
	 int i = 0, retry = 0;
...
static void gpio_afc_dwork(struct work_struct *work)
{
	struct gpio_afc_ddata *ddata =
	    container_of(work, struct gpio_afc_ddata, afc_check.work);
	...
	for (retry = 0; retry < AFC_RETRY_CNT; retry++) {
		for (i = 0; i < AFC_RETRY_CNT ; i++) {
			...
			if (ret >= vol - 2 && ret <= vol + 2) {
				ret = 0;
				ddata->curr_voltage = vol;
				pr_info("%s success\n", __func__);
				break;
			} else if (ret == 0) {
...
int set_afc_voltage(int voltage)
{
	...
	cancel_delayed_work(&ddata->afc_check);
	...
	schedule_delayed_work(&ddata->afc_check, msecs_to_jiffies(delayed));
...
static int gpio_afc_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&ddata->afc_check, gpio_afc_dwork);
	ddata->pdata = pdata;
	ddata->gpio_input = false;
