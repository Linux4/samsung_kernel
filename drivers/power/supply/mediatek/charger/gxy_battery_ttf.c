...
/*A06 code for P240605-02536 by shixuanxuan at 20240606 start*/
#ifdef CONFIG_HQ_PROJECT_O8
        if ((!(info->hv_disable)) && (battery->afc_result == 1 ||
            ...
            /*A06 code for P240606-02607 by xiongxiaoliang at 20240611 start*/
            if (battery->usb_vol_now >= USB_HV_VLOTAGE_MIN) {
                charge = battery->ttf_d->ttf_hv_charge_current;
            } else {
                charge = battery->ttf_d->ttf_dcp_charge_current;
            }
            /*A06 code for P240606-02607 by xiongxiaoliang at 20240611 end*/
...
#endif
/*A06 code for P240605-02536 by shixuanxuan at 20240606 end*/
        ...
        /*A06 code for AL7160A-3661 by xiongxiaoliang at 20240627 start*/
        //force display 1 min when UISOC < 100%.
        if (battery->ttf_d->timetofull <= 60) {
            battery->ttf_d->timetofull = 60;
        }
        /*A06 code for AL7160A-3661 by xiongxiaoliang at 20240627 end*/
...
static void gxy_bat_time_to_full_work(struct work_struct *work)
{
    ...
    if (gxy_ttf_init_psy(gxy_battery) == false) {
        ...
        if (check_ttf_state(gxy_battery->capacity, gxy_battery->bat_sts)) {
            ...
            /*A06 code for AL7160A-3650 by xiongxiaoliang at 20240627 start*/
            if ((gxy_battery->capacity >= 80) && (gxy_battery->chg_info != NULL) &&
                (gxy_battery->chg_info->cust_batt_cap > 0 && gxy_battery->chg_info->cust_batt_cap != 100)) {
                queue_delayed_work(gxy_battery->monitor_wqueue,
                    &gxy_battery->ttf_d->timetofull_work, msecs_to_jiffies(2000));
                pr_err("%s: switch %d protect, Continue working\n", __func__, gxy_battery->chg_info->cust_batt_cap);
            } else {
                pr_err("%s: cancel work\n", __func__);
                cancel_delayed_work(&gxy_battery->ttf_d->timetofull_work);
            }
            /*A06 code for AL7160A-3650 by xiongxiaoliang at 20240627 end*/
        }
    }
}
/* hs14_u code for AL6528AU-252 by liufurong at 2024/02/19 start */
