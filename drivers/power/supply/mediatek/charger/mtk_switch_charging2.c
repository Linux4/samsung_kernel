...
#if 1//ndef WT_COMPILE_FACTORY_VERSION
		if(info->lcmoff) {
			if(info->ap_thermal_lcmoff.cc < pdata->charging_current_limit)
				pdata->charging_current_limit = info->ap_thermal_lcmoff.cc;
		} else {
...
static int mtk_switch_charging_run(struct charger_manager *info)
{
	...
	chr_err("mtk_pdc_check_charger(info)=%d,mtk_is_TA_support_pd_pps(info)=%d\n",mtk_pdc_check_charger(info),mtk_is_TA_support_pd_pps(info));
