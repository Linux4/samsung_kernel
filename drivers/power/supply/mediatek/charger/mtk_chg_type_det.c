...
/*A06 code for P240709-06253 by jiashixian at 20240709 start*/
#if defined(CONFIG_HQ_PROJECT_O8)
#define PD_CONNECT_HARD_RESET_STATE 9
#endif
/*A06 code for P240709-06253 by jiashixian at 20240709 end*/
...
/* hs14 code for  SR-AL6528A-01-339 by shanxinkai at 2022/09/30 end*/
	/* A06 code for P240528-06609 by shixuanxuan at 20240615 start*/
	#if defined(CONFIG_HQ_PROJECT_O8)
	int pps_retry_count = 5;
	#endif
	/* A06 code for P240528-06609 by shixuanxuan at 20240615 end*/
...
/*A06 code for SR-AL7160A-01-230 by shixuanxuan at 20240416 start*/
	...
	case POWER_SUPPLY_PROP_HV_CHARGER_STATUS:
		...
		if (info->hv_disable) {
			...
			/* A06 code for P240528-06609 by shixuanxuan at 20240531 start*/
 			do {
				ret = ss_fast_charger_status(info);
				msleep(100);
				pps_retry_count--;
				pr_info("g_pd_online: %d, get fast charge sratus:%d, retry:%d\n",g_pd_online, ret, pps_retry_count);
			} while (g_pd_online && (ret == NORMALL_CHARGING_STS) && pps_retry_count);
			/*A06 code for P240625-06985 by xiongxiaoliang at 20240626 start*/
			if ((info->pd_type == MTK_PD_CONNECT_PE_READY_SNK_PD30) && (info->chr_type == STANDARD_HOST)) {
				val->intval = NORMALL_CHARGING_STS;
			} else {
				val->intval = ss_fast_charger_status(info);
			}
			/*A06 code for P240625-06985 by xiongxiaoliang at 20240626 end*/
			/* A06 code for P240528-06609 by shixuanxuan at 20240531 end*/
		}
		break;
	/*A06 code for P240614-04356 by xiongxiaoliang at 20240626 start*/
	case POWER_SUPPLY_PROP_DIRECT_CHARGING_STATUS:
		...
		if (info->hv_disable) {
			...
			if ((info->chr_type == STANDARD_CHARGER) || (info->chr_type == NONSTANDARD_CHARGER)) {
	...
	case POWER_SUPPLY_PROP_CHARGING_TYPE:
		...
		if ((info->chr_type == STANDARD_CHARGER) || (info->chr_type == NONSTANDARD_CHARGER)) {
			if (mtk_is_pep_series_connect(info) || mtk_pdc_check_charger(info) || afc_get_is_connect(info)) {
				val->strval = "9V_TA";
...
/* A06 code for AL7160A-11 by shixuanxuan at 20240321 start */
#if defined(CONFIG_HQ_PROJECT_O22)
extern int fusb302_send_5v_source_caps(int ma);
#endif
/* A06 code for AL7160A-11 by shixuanxuan at 20240321 end */
...
/*A06 code for AL7160A-1232 by jiashixian at 20240518 start*/
	...
	union power_supply_propval adc_val = {0,};
#endif
...
/* hs14 code for SR-AL6528A-01-242 by shanxinkai at 2022/10/12 end */
	...
	case POWER_SUPPLY_PROP_SHIPMODE:
		...
		if (IS_ERR_OR_NULL(cp_psy)) {
			pr_err("%s TCPC OTG get cp_psy fail !\n", __func__);
		} else {
			adc_val.intval = 0;
			power_supply_set_property(cp_psy,
                        POWER_SUPPLY_PROP_CP_ADC_ENABLE, &adc_val);
		}
		#endif
		/*A06 code for AL7160A-1232 by jiashixian at 20240518 end*/
	...
	case TCP_NOTIFY_TYPEC_STATE:
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			if (cti->tcpc_kpoc) {
				...
				pr_err("%s cti->tcpc->pd_port.pd_connect_state =  %d\n",__func__,state);
				vbus_value = battery_get_vbus();
				pr_err("%s error_recovery_once = %d\n", __func__,
					cti->tcpc->pd_port.error_recovery_once);
				if ((cti->tcpc->pd_port.error_recovery_once == 1)
				|| (state == PD_CONNECT_HARD_RESET_STATE)
				|| (vbus_value > 4000)) {
					pr_err("%s KPOC error recovery once vbus_value=%d\n",
					__func__,vbus_value);
					vbus_value = 0;
					plug_in_out_handler(cti, false, false);
					msleep(200);
					vbus_value = battery_get_vbus();
					pr_err("%s twice KPOC error recovery once vbus_value=%d\n",
					__func__,vbus_value);
					if(vbus_value > 4000)
					{
						vbus_value = 0;
