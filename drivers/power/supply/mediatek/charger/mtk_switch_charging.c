...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	/* AICL */
    /*yuanjian.wt modift for AFC*/
	if (!mtk_pe20_get_is_connect(info) && !mtk_pe_get_is_connect(info) && !mtk_is_TA_support_pd_pps(info) && 
	    !mtk_pdc_check_charger(info) && !afc_get_is_connect(info)) {
	...
	if (mtk_is_TA_support_pd_pps(info)) {
		...
		if (KERNEL_POWER_OFF_CHARGING_BOOT == get_boot_mode()
			|| LOW_POWER_OFF_CHARGING_BOOT == get_boot_mode()) {
				...
				pdata->charging_current_limit =
					info->data.usb_charger_current;
		} else if (info->usb_state == USB_SUSPEND) {
			charger_dev_enable_hz(info->chg1_dev, true);
			pdata->input_current_limit = 0;
			pdata->charging_current_limit = 0;
			charger_manager_notifier(info, CHARGER_NOTIFY_STOP_CHARGING);
...
#endif /*CONFIG_MT6370_PMU_CHARGER*/
//-Bug516174,zhaosidong.wt,ADD,20191126,charge current limit for AP overheat
...
//Bug506950,zhaosidong.wt,ADD,20191121,remove aicl of PD charger
	...
	/* If AICR < 300mA, stop AFC/PE+/PE+20 */
...
//EXTB P200318-01624,zhaosidong.wt,ADD,20200324, SOC is mantained for a long time
	 ...
	 * enable AFC/PE+/PE+20, if it is disabled
	 */
