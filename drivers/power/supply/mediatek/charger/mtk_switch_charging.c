...
#define HV_DISABLE_CURRENT_UA 1500000
#endif
...
/*A06 code for P240514-06911 by shixuanxuan at 20240515 start*/
#ifdef CONFIG_HQ_PROJECT_O8
	if (info->hv_disable == HV_DISABLE) {
		pdata->charging_current_limit =
			min(pdata->charging_current_limit, HV_DISABLE_CURRENT_UA);
	}
#endif
/*A06 code for P240514-06911 by shixuanxuan at 20240515 end*/
...
static int ss_battery_aging_update(struct charger_manager *info)
{
	...
	g_cp_charging_lmt.cp_battery_cv = info->data.battery_cv;
	#endif
	/* A06 code for AL7160A-2776 by xiongxiaoliang at 20240606 end*/
...
static void swchg_select_cv(struct charger_manager *info)
{
	...
	if (info->enable_sw_jeita){
		if (info->sw_jeita.cv != 0) {
			/* A06 code for AL7160A-3543 by xiongxiaoliang at 20240624 start*/
			if (info->data.ss_batt_aging_enable){
				info->data.setting_cv = min(info->data.battery_cv, info->sw_jeita.cv);
			} else {
				info->data.setting_cv = info->sw_jeita.cv;
			}
			chr_err("%s sw_jeita_cv=%d, setting_cv=%d\n", __func__, info->sw_jeita.cv, info->data.setting_cv);
			/* A06 code for AL7160A-3543 by xiongxiaoliang at 20240624 end*/
