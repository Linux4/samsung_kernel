...
struct afc_dev *g_cx2560_afc = NULL;
#endif
...
/* A06 code for P240514-07707 by xiongxiaoliang at 20240524 start */
#if defined(CONFIG_HQ_PROJECT_O8)
void cx2560_afc_switch_ctrl(bool level)
{
	int afc_switch_gpio_value = 0;

	if (g_cx2560_afc != NULL) {
		afc_switch_gpio_value = gpio_get_value(g_cx2560_afc->afc_switch_gpio);
		pr_info("%s switch_value[%d]\n", __func__, afc_switch_gpio_value);
		if (afc_switch_gpio_value != level) {
			gpio_direction_output(g_cx2560_afc->afc_switch_gpio, level);
			afc_switch_gpio_value = gpio_get_value(g_cx2560_afc->afc_switch_gpio);
			pr_info("%s switch_value[%d], level[%d]\n", __func__, afc_switch_gpio_value, level);
		} else {
			pr_info("%s switch_value[%d]==level[%d]\n", __func__, afc_switch_gpio_value, level);
		}
	} else {
		pr_err("%s g_cx2560_afc is NULL", __func__);
	}
}
EXPORT_SYMBOL_GPL(cx2560_afc_switch_ctrl);
#endif
/* A06 code for P240514-07707 by xiongxiaoliang at 20240524 end */
