...
struct sgm41513 {
    ...
    struct delayed_work set_icl_work;
    int icl_ma;
    #endif
...
#define SGM41513_MIVR_MAX  5400000
#define SGM41513_MIVR_DEFAULT  4600000
static int sgm41513_enable_power_path(struct charger_device *chg_dev, bool en)
{
	u32 mivr = (en ? SGM41513_MIVR_DEFAULT : SGM41513_MIVR_MAX);

    pr_info("%s: en = %d\n", __func__, en);
    return sgm41513_set_ivl(chg_dev, mivr);
}
#endif
...
int sgm41513_set_chargevolt(struct sgm41513 *sgm, int volt)
{
    ...
    /*A06 code for P240529-05030 by jiashixian at 20240612 start*/
    #ifdef CONFIG_HQ_PROJECT_O8
    ret = sgm41513_update_bits(sgm, SGM41513_REG_04, REG04_VREG_MASK,
                    val << REG04_VREG_SHIFT);
    if (ret < 0) {
        pr_err("write SGM41513_REG_04 fail, ret:%d\n", ret);
        return ret;
    }

    val1 = ((volt - REG04_VREG_BASE) % REG04_VREG_LSB) / REG0F_VREG_FT_LSB;
    if (val1 >= 1) {
        val1 = 1;
    } else {
        val1 = 0;
    }
    ret = sgm41513_update_bits(sgm, SGM41513_REG_0F, REG0F_VREG_FT_MASK,
                   val1 << REG0F_VREG_FT_SHIFT);
    if (ret < 0) {
        pr_err("write SGM41513_REG_0F fail, ret:%d\n", ret);
    }
...
int sgm41513_set_input_current_limit(struct sgm41513 *sgm, int curr)
{
    ...
    if (curr < REG00_IINLIM_BASE) {
        ...
        sgm41513_enable_power_path(sgm->chg_dev, false);
    } else {
        sgm41513_enable_power_path(sgm->chg_dev, true);
    }
    #else
    if (curr < REG00_IINLIM_BASE)
        curr = REG00_IINLIM_BASE;
...
/*A06 code for P240610-03562|AL7160A-1630 by shixuanxuan at 20240614 start*/
#ifdef CONFIG_HQ_PROJECT_O8
static void sgm41513_set_icl_work(struct work_struct *work)
{
    struct sgm41513 *sgm = container_of(work, struct sgm41513,
                            set_icl_work.work);
    ...
    sgm41513_set_input_current_limit(sgm, sgm->icl_ma);
}
#endif
/*A06 code for P240610-03562|AL7160A-1630 by shixuanxuan at 20240614 end*/
...
static irqreturn_t sgm41513_irq_handler(int irq, void *data)
{
    ...
    /* A06 code for P240610-03562|AL7160A-1630 by shixuanxuan at 20240614 start */
    #ifdef CONFIG_HQ_PROJECT_O8
    schedule_delayed_work(&sgm->set_icl_work, 0);

    /* A06 code for AL7160A-3569 by shixuanxuan at 20240614 start */
    ...
    pr_info("%s enter. bypass_chgdet_en = %d, g_pr_swap = %d\n",
        __func__, sgm->bypass_chgdet_en, g_pr_swap);

    if ((sgm->bypass_chgdet_en == true) || g_pr_swap) {
        pr_err("%s:bypass_chgdet_en=%d, g_pr_swap=%d, skip bc12\n", __func__,
            sgm->bypass_chgdet_en, g_pr_swap);
        return IRQ_HANDLED;
    }
    #else
    /* hs14 code for P221116-03489 by wenyaqi at 2022/11/23 start */
    ...
    sgm->power_good = !!(reg_val & REG08_PG_STAT_MASK);
    #endif
    /* A06 code for AL7160A-3569 by shixuanxuan at 20240614 end */
                 ...
                 const struct i2c_device_id *id)
{
    ...
    /*A06 code for AL7160A-1630 by shixuanxuan at 20240615 start*/
    #ifdef CONFIG_HQ_PROJECT_O8
    INIT_DELAYED_WORK(&sgm->set_icl_work, sgm41513_set_icl_work);
    sgm->icl_ma = 0;
    #endif
    /*A06 code for AL7160A-1630 by shixuanxuan at 20240615 end*/
