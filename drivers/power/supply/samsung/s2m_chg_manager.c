...
static int get_battery_info(struct s2m_chg_manager_info *battery)
{
	...
	if (!psy) {
		pr_info("%s, fail to get muic-manager psy\n", __func__);
	} else {
		ret = power_supply_get_property(psy, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_MUIC_PATH, &value);
		battery->path_status = value.intval;
	}
...
continue_monitor:
	pr_err("%s: Status(%s), Health(%s), Cable(%d, %s), Recharging(%d))\n", __func__,
		bat_status_str[battery->status], health_str[battery->health],
		battery->cable_type, path_status_to_str[battery->path_status], battery->is_recharging);
