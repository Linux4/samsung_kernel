				  ...
				  struct power_supply_battery_info *info, int num)
{
	...
	#ifdef CONFIG_AFC
	info->cur.afc_cur = -EINVAL;
	info->cur.afc_limit = -EINVAL;
	#endif
	...
	/* Tab A8 code for SR-AX6300-01-5 by wenyaqi at 20210824 start */
	#ifdef CONFIG_AFC
	of_property_read_u32_index(battery_np, "charge-afc-current-microamp", 0,
				   &info->cur.afc_cur);
	of_property_read_u32_index(battery_np, "charge-afc-current-microamp", 1,
				   &info->cur.afc_limit);
	#endif
	/* Tab A8 code for SR-AX6300-01-5 by wenyaqi at 20210824 end */
