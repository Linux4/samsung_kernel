...
void hq_update_charing_count(struct mtk_charger *info)
{
	...
	if(info->interval_time == 0)// if the usb connected and the phone restart interval_time=0;
		info->base_time = info->current_time -1;
				...
				int chr_type)
{
	...
	#if !defined(HQ_FACTORY_BUILD)
	info->interval_time = 0;
	pr_err("base_time = %d", info->base_time);
        ...
        unsigned long status, void *data)
{
    ...
    switch ((u8)status) {
    case USB_SUSPEND:
        pr_info("%s: usb_state SUSPEND, set input current <=2.5mA\n", __func__);
        //charger_dev_set_input_current(info->chg1_dev, 2000);
		charger_dev_enable_powerpath(info->chg1_dev, 0);
        break;
    case USB_UNCONFIGURED:
        pr_info("%s: usb_state UNCONFIGURED, set input current <=100mA\n", __func__);
		charger_dev_enable_powerpath(info->chg1_dev, 1);
        charger_dev_set_input_current(info->chg1_dev, 100000);
        break;
    case USB_CONFIGURED:
        if (info->chr_type == POWER_SUPPLY_TYPE_USB) {
            pr_info("%s: usb_state CONFIGURED, set input current 500mA(SDP)\n", __func__);
            charger_dev_set_input_current(info->chg1_dev, 500000);
        } else if (info->chr_type == POWER_SUPPLY_TYPE_USB_CDP) {
            pr_info("%s: usb_state CONFIGURED, set input current 1500mA(CDP)\n", __func__);
            charger_dev_set_input_current(info->chg1_dev, 1500000);
        }
        break;
    }
    charger_dev_dump_registers(info->chg1_dev);
    charger_dev_get_ibus(info->chg1_dev, &ibus);
    return 0;
}
#endif
