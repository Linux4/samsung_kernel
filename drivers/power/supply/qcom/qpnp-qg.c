...
//+Bug 439935 gudi.wt,MODIFIY,20190726,S86119 batter for recharge
//+Extb 1547,xujianbang.wt,Modify,20190326,recharge when bootup at almost full battery.
...
static int qg_charge_full_update(struct qpnp_qg *chip)
{
	...
	if (chip->charge_done && !chip->charge_full) {
		if (chip->msoc >= 99 && health == POWER_SUPPLY_HEALTH_GOOD) {
			...
			//+Bug 445529 caijiaqi.wt,ADD,20190514,modifiy low probability quick recharger
			chip->msoc = 100;
