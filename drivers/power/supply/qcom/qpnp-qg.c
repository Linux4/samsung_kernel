...
static int qg_get_battery_capacity(struct qpnp_qg *chip, int *soc)
{
	...
	if (chip->dt.linearize_soc && chip->maint_soc > 0)
		...
		*soc = chip->msoc;
	/* F52 for QN3979-765 by wenyaqi at 20210311 start */
	#ifdef HQ_D85_BUILD
	*soc = BATT_MISSING_SOC;
	#endif
	/* F52 for QN3979-765 by wenyaqi at 20210311 end */
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_RESISTANCE_ID:
		...
		break;
	/* QN3979 for SR-QN3979-01-794 Add default battery id by shixuanxuan at 2021/03/03 start */
	case POWER_SUPPLY_PROP_BATT_ID_ERROR:
		pval->intval = chip->batt_id_error;
		break;
	/* QN3979 for SR-QN3979-01-794 Add default battery id by shixuanxuan at 2021/03/03 end */
...
#define CUSTOM_BAT_HIGH_TEMP_VBATT_FLOAT_THRESHOLD	4200000
static int qg_battery_aging_update(struct qpnp_qg *chip)
{
	int rc, cycle_count = 0, vbat_float = 0, vbat_recharge = 0;
	union power_supply_propval prop = {0, };
	...
	rc = get_val(chip->vfloat_data, cycle_count, &vbat_float);
	...
	rc = get_val(chip->vbat_rechg_data, cycle_count, &vbat_recharge);
	if (rc < 0) {
	...
	{
		chip->bp.float_volt_uv = CUSTOM_BAT_HIGH_TEMP_VBATT_FLOAT_THRESHOLD;
		chip->bp.recharge_vbat_mv = CUSTOM_BAT_HIGH_TEMP_RECHARGE_THRESHOLD;
	} else
	{
		chip->bp.float_volt_uv = vbat_float;
		chip->bp.recharge_vbat_mv = vbat_recharge;
	}
	...
	prop.intval = chip->bp.recharge_vbat_mv;
	rc = power_supply_set_property(chip->batt_psy,
...
#define QN3979_DEFAULT_BATT_ID 100000
/* QN3979 for SR-QN3979-01-794 Add default battery id by shixuanxuan at 2021/03/03 end */
static int get_batt_id_ohm(struct qpnp_qg *chip, u32 *batt_id_ohm)
{
	...
	if (rc < 0 ) {
		...
		while ((re_detect_cycle < WORK_AROUND_DETECT_TIMES_ID) && (rc < 0))
		{
			rc = iio_read_channel_processed(chip->batt_id_chan, &batt_id_mv);
			pr_err("[%s]Re-detect BATT_ID \n");
			if (rc < 0)
			{
		...
		if (rc < 0) {
			chip->batt_id_ohm_default = QN3979_DEFAULT_BATT_ID;
			pr_err("[%s]chip->batt_id_ohm_default = %d , rc=%d, PCBA = 0x%x!",
						__func__ , chip->batt_id_ohm_default, rc);
		}
...
/* F52 for QN3979-765 by wenyaqi at 20210311 start */
#ifdef HQ_D85_BUILD
#define DEFAULT_VBATT_CUTOFF_MV		3100
#define DEFAULT_VBATT_LOW_MV		3200
#else
