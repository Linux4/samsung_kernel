...
/* F52 code for QN3979-60 Add log for battery charging by gaochao at 2021/02/20 start */
static void smblib_battery_chg_info_update(struct smb_charger *chg)
{
	union power_supply_propval info_val = {0, };
	struct smbchg_info chg_info = {0, };

	if (!chg || !chg->batt_psy || !chg->bms_psy || !chg->usb_psy)
	{
	...
	chg_info.chg_stat = info_val.intval;

	power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_VOLTAGE_OCV, &info_val);
	chg_info.ocv = info_val.intval;

	/* F52 code for P210317-02835 Battery charging and diacharging by gaochao at 2021/03/24 start */
	power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_VOLTAGE_AVG, &info_val);
	chg_info.vbat_avg = info_val.intval;
	/* F52 code for P210317-02835 Battery charging and diacharging by gaochao at 2021/03/24 end */
	...
	chg_info.usb_current = info_val.intval;
	...
	power_supply_get_property(chg->cp_psy, POWER_SUPPLY_PROP_CP_ILIM, &info_val);
	chg_info.cp_ilim = info_val.intval;

	power_supply_get_property(chg->cp_psy, POWER_SUPPLY_PROP_CP_ENABLE, &info_val);
	chg_info.cp_enable = info_val.intval;

	/* F52 code for P210317-02835 Battery charging and diacharging by gaochao at 2021/03/24 start */
	smblib_err(chg, "cap=%d, vbat=%d, ocv=%d, vbat_avg=%d, FCC=%d, tbat=%d, status=%d, Vbus=%d, usb_cur=%d, ICL_set=%d, chg_type=%d, cp_ilim=%d, smb_en=%d\n",
					chg_info.cap, chg_info.vbat, chg_info.ocv, chg_info.vbat_avg, chg_info.ibat, chg_info.bat_t, chg_info.chg_stat,
					chg_info.vbus, chg_info.usb_current,chg_info.icl_settled, chg_info.chg_type, chg_info.cp_ilim, chg_info.cp_enable);
	/* F52 code for P210317-02835 Battery charging and diacharging by gaochao at 2021/03/24 end */
}

static void smblib_battery_chg_info_work_init(struct smb_charger *chg)
{
	if (!chg)
	{
		pr_err("[ss]line=%d: ichg is null\n", __LINE__);
	...
	pr_debug("line=%d: start battery charging log\n", __LINE__);
	schedule_delayed_work(&chg->battery_chg_info_work,
					msecs_to_jiffies(BATTERY_CHARGING_LOG_START_TIME));
}

static void smblib_battery_chg_info_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work,
					struct smb_charger, battery_chg_info_work.work);
	...
	static int cycle_set_hv_disable = 0;
	const union power_supply_propval pval_disable = {1, };
	const union power_supply_propval pval_enable = {0, };

	if (chg->custom_fine_tune_hv_resume_debug && chg->vbus_rising)
	{
		cycle_set_hv_disable++;
		if (!(cycle_set_hv_disable % 2))
		{
			chg->hv_disable = true;
			smblib_hv_resume(chg, &pval_disable);

		}
		else
		{
			chg->hv_disable = false;
			smblib_hv_resume(chg, &pval_enable);
		}

		pr_info("[%s]line=%d: pd_active=%d, ok_to_pd=%d, hv_disable=%d\n",
			__FUNCTION__, __LINE__, chg->pd_active, chg->ok_to_pd, chg->hv_disable);
	}
	#endif
	/* F52 code for SR-QN3979-01-805 SEC_FLOATING_FEATURE_BATTERY_SUPPORT_HV_DURING_CHARGING by gaochao at 2021/04/05 end */

	smblib_battery_chg_info_update(chg);
	schedule_delayed_work(&chg->battery_chg_info_work,
					msecs_to_jiffies(BATTERY_CHARGING_LOG_SCHEDULE_TIME));
}
/* F52 code for QN3979-60 Add log for battery charging by gaochao at 2021/02/20 end */
...
/* F52 add for SR-QN3979-01-226 by wenyaqi at 2021/02/10 start */
#ifndef HQ_FACTORY_BUILD //ss version
static int ss_set_icl_suspend(struct smb_charger *chg, bool suspend)
{
	int rc = 0;
	/* F52 for P210511-06251 Modify store mode function by shixuanxuan at 20210512 start */
	const union power_supply_propval val={1,}, pval={0,};

	if (suspend)
		smblib_set_prop_input_suspend(chg, &val);
	else
		smblib_set_prop_input_suspend(chg, &pval);
	/* F52 for P210511-06251 Modify store mode function by shixuanxuan at 20210512 end */
	return rc;
}
...
static void ss_retail_app_rule(struct smb_charger *chg)
{
	union power_supply_propval prop = {0, };
	int bat_capacity = 0;
	int rc = 0;
	int retail_app_dischg_thres = 0;
	int retail_app_chg_thres = 0;

	/* obtain battery capacity */
	rc = power_supply_get_property(chg->batt_psy, POWER_SUPPLY_PROP_CAPACITY, &prop);
	if (rc < 0)
		pr_err("[%s] Failed to get battery capacity, rc=%d\n", __FUNCTION__, rc);

	bat_capacity = prop.intval;
	if (sales_code_is("VZW") || sales_code_is("VPP")) {
		pr_err("%s: Sales is VZW or VPP\n", __func__);
		retail_app_dischg_thres = SS_RETAIL_APP_DISCHG_THRES_VZW;
		retail_app_chg_thres = SS_RETAIL_APP_CHG_THRES_VZW;
	} else {
		retail_app_dischg_thres = SS_RETAIL_APP_DISCHG_THRES;
		retail_app_chg_thres = SS_RETAIL_APP_CHG_THRES;
	}

	/* discharge when capacity > RETAIL_APP_DISCHG_THRESHOLD*/
	if (bat_capacity >= retail_app_dischg_thres) {
		rc = ss_set_icl_suspend(chg, true);
		if (rc < 0)
			pr_err("Couldn't suspend input rc=%d\n", rc);
		pr_info("[%s] bat_capacity %d >= %d% stop usb charging\n", __func__,
			bat_capacity, retail_app_dischg_thres);
	} else if (bat_capacity < retail_app_chg_thres) {
		rc = ss_set_icl_suspend(chg, false);
		if (rc < 0)
			pr_err("Couldn't resume input rc=%d\n", rc);
		pr_info("[%s] bat_capacity %d < %d% resume usb charging\n", __FUNCTION__,
			bat_capacity, retail_app_chg_thres);
	} else {
		pr_info("[%s] keep previous state\n", __FUNCTION__);
	}
}

static void ss_retail_app_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work,
		struct smb_charger, retail_app_work.work);
	int rc = 0;

	pr_debug("[%s] run every per %d s, ss_store_mode=%d\n",
			__FUNCTION__, RETAIL_APP_DETECT_TIMER / 1000, chg->ss_store_mode);

	/* when store_mode set to 1, check battery level to charge or discharge */
	if (chg->ss_store_mode == SS_STORE_MODE_ENABLE) {
		ss_retail_app_rule(chg);
		rc = vote(chg->fcc_votable, STORE_MODE_VOTER, true, STORE_MODE_FCC);
		if(rc < 0)
			pr_err("[%s] Fail to vote STORE_MODE_VOTER, rc=%d\n", __FUNCTION__, rc);
	} else {
	...
	schedule_delayed_work(&chg->retail_app_work,
		msecs_to_jiffies(RETAIL_APP_DETECT_TIMER));
}

void smblib_get_prop_ss_store_mode(struct smb_charger *chg,
	union power_supply_propval *val)
{
	if (chg)
	{
		val->intval = chg->ss_store_mode;
		pr_err("[%s] chg->ss_store_mode=%d\n", __FUNCTION__, chg->ss_store_mode);
	} else
		pr_err("[%s] chg is NULL\n", __FUNCTION__);
}

void smblib_set_prop_ss_store_mode(struct smb_charger *chg,
	const union power_supply_propval *val)
{
	...
	if (chg) {
		...
		chg->ss_store_mode = val->intval;
		pr_err("[%s] vbus_rising=%d, ss_store_mode=%d\n", __FUNCTION__, vbus_rising, chg->ss_store_mode);
		if (vbus_rising) {
			if (chg->ss_store_mode == SS_STORE_MODE_ENABLE) {
				pr_err("Set store mode while usb is present!");
				rc = vote(chg->fcc_votable, STORE_MODE_VOTER, true, STORE_MODE_FCC);
				if(rc < 0)
					pr_err("[%s] Fail to vote STORE_MODE_VOTER, rc=%d\n", __FUNCTION__, rc);
				schedule_delayed_work(&chg->retail_app_work,
				...
				pr_err("Clear store mode while usb is present!");
				rc = vote(chg->fcc_votable, STORE_MODE_VOTER, false, STORE_MODE_FCC);
				if(rc < 0)
					pr_err("[%s] Fail to vote STORE_MODE_VOTER, rc=%d\n", __FUNCTION__, rc);
				cancel_delayed_work_sync(&chg->retail_app_work);
			}
		}
	} else
		pr_err("[%s] chg is NULL\n", __FUNCTION__);
...
#endif
/* F52 add for SR-QN3979-01-226 by wenyaqi at 2021/02/10 end */
...
int smblib_set_icl_current(struct smb_charger *chg, int icl_ua)
{
	...
	if (chg->chg_param.smb_version == PMI632_SUBTYPE)
		schgm_flash_torch_priority(chg, suspend ? TORCH_BOOST_MODE :
					TORCH_BUCK_MODE);
	/* F52 add for QN3979-301 Enable Huawei cable charging by shixuanxuan at 2020/03/08 start */
	...
	if (smblib_get_prop_typec_mode(chg) ==
		...
		return 0;
	*/
	/* F52 add for QN3979-301 Enable Huawei cable charging by shixuanxuan at 2020/03/08 end */
	if (suspend)
				...
				union power_supply_propval *val)
{
	...
	/* F52 code for P220406-06201 by shixuanxuan at 2022/04/07 start */
	union power_supply_propval cap_val = {0, };
	union power_supply_propval tbat_val = {0, };
	u8 chg_stat = 0;
	/* F52 code for P220406-06201 by shixuanxuan at 2022/04/07 end */

	/* F52 code for P220406-01840 by shixuanxuan at 2022/04/07 start */
	val->intval = POWER_SUPPLY_STATUS_UNKNOWN;
	/* F52 code for P220406-01840 by shixuanxuan at 2022/04/07 end */
	...
	if (pval.intval == POWER_SUPPLY_HEALTH_OVERVOLTAGE) {
		val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
                //SXX
		pr_err("lk0 Charge Enable state, ignore POWER_SUPPLY_STATUS_NOT_CHARGING\n");
		return 0;
	}
	...
	/* F52 code for P220406-06201 by shixuanxuan at 2022/04/07 start */
	rc = smblib_read(chg, CHARGING_ENABLE_CMD_REG, &chg_stat);
	if (rc < 0) {
		smblib_err(chg, "Couldn't read CHARGING_ENABLE_CMD_REG rc=%d\n",
			rc);
		return rc;
	}
	chg_stat = chg_stat & CHARGING_ENABLE_CMD_BIT;
	/* F52 code for P220406-06201 by shixuanxuan at 2022/04/07 end */
	...
	//pr_err("ltk stat = %d chg_status = %d\n", stat, chg_stat);
	/* F52 code for P220406-06201 by shixuanxuan at 2022/04/07 start */
	rc = power_supply_get_property(chg->batt_psy, POWER_SUPPLY_PROP_CAPACITY, &cap_val);
	if (rc < 0) {
		smblib_err(chg, "Couldn't read BATTERY_CHARGER_CAPACITY rc=%d\n",rc);
		return rc;
	}

	rc = power_supply_get_property(chg->batt_psy, POWER_SUPPLY_PROP_TEMP, &tbat_val);
	if (rc < 0) {
	...
	//pr_err("ltk cap_val = %d tbat_val = %d\n", cap_val, tbat_val);
	...
	case INHIBIT_CHARGE:
		/* F52 code for P220406-06201 by shixuanxuan at 2022/04/07 start */
		if ((cap_val.intval <= 99) && (tbat_val.intval <= 450)) {
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
			pr_err("charge_done1! soc=%d, tbat=%d\n", cap_val.intval, tbat_val.intval);
		} else {
			val->intval = POWER_SUPPLY_STATUS_FULL;
		}
		/* F52 code for P220406-06201 by shixuanxuan at 2022/04/07 end */
	...
	case PAUSE_CHARGE:
		/* F52 code for P220406-06201 by shixuanxuan at 2022/04/07 start */
		if (chg_stat || chg->re_chg == 0) {
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
			pr_err("Charge Enable state, ignore POWER_SUPPLY_STATUS_NOT_CHARGING\n");
		} else {
			val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
			pr_err("lk1 Charge Enable state, ignore POWER_SUPPLY_STATUS_NOT_CHARGING\n");
		}
		/* F52 code for P220406-06201 by shixuanxuan at 2022/04/07 end */
		break;
	default:
	...
	//SXX
	if (!stat) {
		if ((chg_stat || chg->re_chg == 0) && usb_online) {
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
			pr_err("lk2 Charge Enable state, ignore POWER_SUPPLY_STATUS_NOT_CHARGING\n");
		} else {
			val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
			pr_err("lk3 Charge Enable state, ignore POWER_SUPPLY_STATUS_NOT_CHARGING\n");
		}
	}

	return 0;
}
/* F52 add for SR-QN3979-01-258 Add slow charge sysFs node by shixuanxuan at 20210518 start */
#if !defined(HQ_FACTORY_BUILD)	//ss version
#define SLOW_CHARGING_CURRENT_STANDARD 400000
/* F52 code for QN3979-4256 modify slow charge of PD by shixuanxuan at 20210901 start */
#define SLOW_CHARGING_COUNT  20
#define SLOW_CHARGING_COUNT_POWERON  20
/* F52 code for QN3979-4256 modify slow charge of PD by shixuanxuan at 20210901 end */
...
#endif
/* F52 add for SR-QN3979-01-258 Add slow charge sysFs node by shixuanxuan at 20210518 end */
				...
				union power_supply_propval *val)
{
	...
	/* F52 code for QN3979-4256 modify slow charge of PD by shixuanxuan at 20210901 start */
	if (chg->pd_active) {
		pr_debug("PD type, exit_slow_charge detection!\n");
		return rc;
	}
	/* F52 code for QN3979-4256 modify slow charge of PD by shixuanxuan at 20210901 end */
	...
	if ((settled_icl < SLOW_CHARGING_CURRENT_STANDARD) && (chg->real_charger_type != POWER_SUPPLY_TYPE_UNKNOWN))
	{
				...
				union power_supply_propval *val)
{
	...
	if (stat & CHARGER_ERROR_STATUS_BAT_OV_BIT) {
		...
		if (!rc) {
			...
			if (pval.intval >= effective_fv_uv + 95000) {
			/* F52 code for P210317-02835 Battery charging and diacharging by gaochao at 2021/03/24 end */
					...
					union power_supply_propval *val)
{
	...
	if ((val->intval == 1) && (cap_val.intval <= 99) && (tbat_val.intval <= 450))
	{
		pr_err(" Charge_done! SOC = %d, tbat = %d \n", cap_val.intval, tbat_val.intval);
		...
		if (chg->re_chg == 1)
		{
			...
			if (rc < 0)
			{
				...
				pr_err(" Charge_done before soc=100, disable battery charge success! \n");
			}
			chg->re_chg = 0;
                        msleep(1000);
		}
		...
		if (chg->re_chg == 0)
		{
			...
			if (rc < 0)
			{
				...
				pr_err(" Clear charge_done status, resume battery charge success! \n");
			}
			chg->re_chg =1;
			msleep(1000);
		}
...
#if !defined(HQ_FACTORY_BUILD)
int smblib_get_prop_usb_otg(struct smb_charger *chg,
				  union power_supply_propval *val)
{
	int rc;
	u8 is_otg = 0;

	rc = smblib_read( chg , DCDC_CMD_OTG_REG , &is_otg );
	if (rc < 0 )
	{
		smblib_err(chg, "Failed to read DCDC_CMD_OTG_REG, rc=%d\n", rc);
	}
	val->intval = is_otg;
			...
			const union power_supply_propval *val)
{
	...
	} else if (val->intval == 0)
	{
		...
		chg->charging_enable_flag = 0;
		pr_info( "Disable charging\n");
	} else
	{
		chg->charging_enable_flag = 1;
		smblib_err(chg, "Couldn't disable charging\n");
	}
				  ...
				  const union power_supply_propval *val)
{
	...
	/* F52 code for QN3979-60 Add log for battery charging by gaochao at 2021/02/20 start */
	pr_info("[%s]line=%d: input_suspend=%d\n", __FUNCTION__, __LINE__, val->intval);
	/* F52 code for QN3979-60 Add log for battery charging by gaochao at 2021/02/20 end */
...
int smblib_dp_dm(struct smb_charger *chg, int val)
{
	...
	case POWER_SUPPLY_DP_DM_DP_PULSE:
		 ...
		 */
		/* F52 code for SR-QN3979-01-805 Disabling hv_disable during charging by gaochao at 2021/04/20 start */
		#if defined(CONFIG_AFC)
		if (chg->hv_disable) {
			chg->pulse_cnt = 0;

			pr_err("[%s]line=%d: QC3.0 increase Vbus per 200mV but hv_disable=%d\n",
				__FUNCTION__, __LINE__, chg->hv_disable);
				return -EINVAL;
		}
		#endif
		/* F52 code for SR-QN3979-01-805 Disabling hv_disable during charging by gaochao at 2021/04/20 end */
	...
	case POWER_SUPPLY_DP_DM_FORCE_9V:
		...
		if (stat & QC_5V_BIT) {
			...
			/* F52 code for QN3979-4140 QC2.0 is compatible with AFC by gaochao at 2021/05/12 start */
			#if defined(HQ_FACTORY_BUILD)
			rc = vote(chg->usb_icl_votable, HVDCP2_ICL_VOTER, true, DCP_CURRENT_UA);
			if (rc < 0) {
				pr_err("line=%d: Couldn't vote HVDCP2_ICL_VOTER rc=%d\n", __LINE__, rc);
			}
			#else
			vote(chg->usb_icl_votable, HVDCP2_ICL_VOTER,
					true, 1000000);
			#endif
			/* F52 code for QN3979-4140 QC2.0 is compatible with AFC by gaochao at 2021/05/12 end */
			smblib_hvdcp_set_fsw(chg, QC_9V_BIT);
		}

		/* F52 code for QN3979-4140 QC2.0 is compatible with AFC by gaochao at 2021/05/12 start */
		#if defined(HQ_FACTORY_BUILD)
		if (chg->afc_sts == AFC_FAIL || chg->afc_sts == AFC_INIT) {
			rc = smblib_force_vbus_voltage(chg, FORCE_5V_BIT);
		} else {
			rc = smblib_force_vbus_voltage(chg, FORCE_9V_BIT);
		}
		#else
		if (!chg->afc_qc2_ta_check) {
			rc = smblib_force_vbus_voltage(chg, FORCE_9V_BIT);
		}
		#endif
		/* F52 code for QN3979-4140 QC2.0 is compatible with AFC by gaochao at 2021/05/12 end */
		...
		/* F52 code for QN3979-302 Add AFC feature by gaochao at 2021/02/26 start */
		#if defined(CONFIG_AFC)
		/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 start */
		#if !defined(HQ_FACTORY_BUILD)	// SEP Binary
		schedule_delayed_work(&chg->compliant_check_work,
				msecs_to_jiffies(2000));
		#endif
		/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 end */
		#endif
		/* F52 code for QN3979-302 Add AFC feature by gaochao at 2021/02/26 end */
		break;
	case POWER_SUPPLY_DP_DM_FORCE_12V:
		/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/12 start */
		if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP ||
			chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3) {
			pr_debug("[%s]line=%d: QC2.0 QC3.0 doesn't increase to 12V, HW OVP is 11.5V\n", __FUNCTION__, __LINE__);

			return rc;
		}
		/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/12 end */
...
int smblib_disable_hw_jeita(struct smb_charger *chg, bool disable)
{
	 ...
	 */
	/* F52 for QN3979-765 by wenyaqi at 20210415 start */
	#ifdef HQ_D85_BUILD
	mask = 0xFF;
	#else
	...
	#endif
	/* F52 for QN3979-765 by wenyaqi at 20210415 end */
...
static int smblib_process_thermal_readings(struct smb_charger *chg)
{
	...
	if (chg->connector_temp > CONNECTOR_TEMP_SHDN_THRESH ||
		...
		/* F52 code for QN3979-710 TA keep power path by gaochao at 2021/03/12 start */
		// suspend_input = true;
		/* F52 code for QN3979-710 TA keep power path by gaochao at 2021/03/12 end */
		disable_smb = true;
		goto out;
	}
...
int hq_enable_battery_charging(struct smb_charger *chg,
                const union power_supply_propval *val)
{
	int rc;
	...
	if (1 == val->intval)
	{
        ...
        pr_info("[%s] Enable battery charging! \n",	__FUNCTION__);
		...
		pr_info("[%s] Disable battery charging! \n",	__FUNCTION__);
	} else
		smblib_err(chg, "Enable/Disable battery charging error \n");

	return 0;
}

void hq_update_charing_count(struct smb_charger *chg)
{
	struct timex txc;
	/* F52 add for HQ00001 modify battery long time protect function by shixuanxuan at 2021/06/22 start */
	int temp_time;

	temp_time = chg->current_time;
	/* F52 add for HQ00001 modify battery long time protect function by shixuanxuan at 2021/06/22 end */
	do_gettimeofday(&(txc.time));
	chg->current_time = txc.time.tv_sec;

	if(!chg->vbus_rising)
	{
		chg->base_time = chg->current_time;
		chg->interval_time = 0;

	} else {
		if(chg->base_time == 0)// if the usb connected and the phone restart base_time=0;
			chg->base_time = chg->current_time;
		/* F52 add for HQ00001 modify battery long time protect function by shixuanxuan at 2021/06/22 start */
		if ((chg->current_time - temp_time > 3600) || (chg->current_time - temp_time < 0))
		{
			chg->base_time = chg->current_time;
			chg->interval_time = 0;
		} else {
			chg->interval_time = chg->current_time-chg->base_time;
		}
	}
	/* F52 add for HQ00001 modify battery long time protect function by shixuanxuan at 2021/06/22 end */

	pr_info("[%s] current interval_time = %d (min)\n",
			__FUNCTION__, chg->interval_time/60);
}

void hq_batt_recharging(struct smb_charger *chg)
{
	const union power_supply_propval val={1,}, pval={0,};

	chg->en_batt_protect = DISABLE_BATT_PROTECT;

	if (chg->recharge_count < 3)
	{
		smblib_set_prop_input_suspend(chg, &pval);
		pr_debug("[%s] long charging battery protection power-path recharging!! \n",
			__FUNCTION__);
		hq_enable_battery_charging(chg, &val);
		pr_debug("[%s] long charging battery protection battery recharging!! \n",
			__FUNCTION__);
	}
...
void hq_batt_over_capacity(struct smb_charger *chg)
{
	union power_supply_propval val = {1,};

	if (chg->over_cap_count < 3)
	{
		smblib_set_prop_input_suspend(chg, &val);
	}
...
void hq_batt_capticity_hold(struct smb_charger *chg)
{
	const union power_supply_propval pval={0,};

	if (chg->cap_hold_count < 3)
	{
		smblib_set_prop_input_suspend(chg, &pval);
		hq_enable_battery_charging(chg, &pval);
	}
...
int hq_update_charge_state(struct smb_charger *chg)
{
	union power_supply_propval capa_val;
	int rc = 0;

	hq_update_charing_count(chg);

	if (chg->batt_protect_flag == ENABLE_BATT_PROTECT)
	{
		if (chg->interval_time > TSUSPEND )
		{
			rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_CAPACITY, &capa_val); // get bms capacity
			...
			if (capa_val.intval > HIGHT_CPPICITY)
			{
				hq_batt_over_capacity(chg);
				pr_info("The battery capacity is %d over ! Charging disabled!\n",capa_val.intval);
				chg->en_batt_protect = ENABLE_BATT_PROTECT;
			} else if(capa_val.intval == HIGHT_CPPICITY) {
				hq_batt_capticity_hold(chg);
				pr_info("The battery capacity is %d equal to STOP_CHARGING_CAPACITY! Battery charging disabled!\n",capa_val.intval);
				chg->en_batt_protect = ENABLE_BATT_PROTECT;
			} else if(capa_val.intval < LOW_CPPICITY){
				hq_batt_recharging(chg);
			}
		}
	} else {
		hq_batt_recharging(chg);
...
int hq_smblib_get_conn_temp(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	...
	if (chg->iio.connector_temp_chan) {
		rc = iio_read_channel_processed(chg->iio.connector_temp_chan,
				&val->intval);
		if (rc<0) {
			pr_err("Error in reading usb_connector_temp_chan, rc=%d\n", rc);
			return rc;
		}
	}
...
void hq_usb_conn_gpio_update(struct smb_charger *chg)
{
	int rc = 0;
	int temp;
	...
	if (temp > CHG_ALERT_HOT) {
		if (chg->vbus_conn_flag != 0) {
			/* F52 for QN3979-765 by wenyaqi at 20210311 start */
			#ifdef HQ_D85_BUILD
			pr_info("USB_connector hot! there is no operation in D85!\n");
			#else
			rc = gpio_direction_output(chg->vbus_control_gpio, DRAW_VBUS_GPIO11_HIGH);

			if (rc)	{
				pr_err("line=%d: failed to pull low vbus_control_gpio\n", __LINE__);
			}
			else {
				chg->vbus_conn_flag = 0;
				pr_info("USB_connector hot! Connect Vbus to GND!\n");
			}
			#endif
			/* F52 for QN3979-765 by wenyaqi at 20210311 end */
		}
	} else if (temp < CHG_ALERT_WARM) {
		if (chg->vbus_conn_flag != 1) {
...
static void smblib_update_usb_conn_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work,
			struct smb_charger, usb_conn_work.work);
	/* QN3979 for SR-QN3979-01-742 add battery protect function by shixuanxuan at 2021/04/25 start */
	#if !defined(HQ_FACTORY_BUILD)
	int rc;
	#endif
	...
	/* QN3979 for SR-QN3979-01-742 add battery protect function by shixuanxuan at 2021/04/25 start */
	#if !defined(HQ_FACTORY_BUILD)
	rc = hq_update_charge_state(chg);
	if (rc < 0)
		pr_err("%s: Fail to update charge state %d\n",__func__,rc);
	#endif
	/* QN3979 for SR-QN3979-01-742 add battery protect function by shixuanxuan at 2021/04/25 end */
	hq_usb_conn_gpio_update(chg);
	schedule_delayed_work(&chg->usb_conn_work, msecs_to_jiffies(USB_CONN_TIME));
}
...
/* F52 code for SR-QN3979-01-805 SEC_FLOATING_FEATURE_BATTERY_SUPPORT_HV_DURING_CHARGING by gaochao at 2021/04/05 start */
#if defined(CONFIG_AFC)
void smblib_hv_resume(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc = 0;
	const char *usb_icl_voter = get_effective_client(chg->usb_icl_votable);

	if (!chg || !val) {
		pr_err("chg or val is null\n");
	}

	// schedule_delayed_work(&chg->hv_resume_work, msecs_to_jiffies(0));
    /* F52 code for OLYMPICO5-766 by zhanglinhan at 2022/02/16 start */
	if (val->intval == HV_DISABLE) {
		...
		if (chg->vbus_rising) {
			if (chg->ok_to_pd) {
				...
				// hard reset but don't report battery status
			} else {
				if ((chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP ||
					chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3)
					&& (chg->quick_charge_detect)) {
    /* F52 code for OLYMPICO5-766 by zhanglinhan at 2022/02/16 end */
					/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/29 start */
					smblib_hvdcp_detect_try_enable(chg, false);
					smblib_hvdcp_set_fsw(chg, QC_5V_BIT);
					rc = smblib_force_vbus_voltage(chg, FORCE_5V_BIT);
					if (rc < 0)
						pr_err("Failed to force 5V\n");
					/* F52 code for P210508-01207 by shixuanxuan at 2021/05/08 start */
					//rc = vote(chg->cp_disable_votable, SEC_BATTERY_DISABLE_HV_VOTER, true, 0);
					//if (rc < 0)
					//	pr_err("line=%d: Couldn't vote cp_disable_votable rc=%d\n",
					//			__LINE__, rc);
					/* F52 code for P210508-01207 by shixuanxuan at 2021/05/08 end */
					rc = vote(chg->usb_icl_votable, SEC_BATTERY_DISABLE_HV_VOTER, true, DCP_CURRENT_UA);
					if (rc < 0)
						pr_err("line=%d: Couldn't vote usb_icl_votable rc=%d\n",
								__LINE__, rc);
					/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/29 end */
				} else {
					// AFC TA
					// detach_afc_hv_charge();
		...
		if (chg->vbus_rising) {
			...
			if (chg->ok_to_pd) {
				...
				if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
					// DCP TA or QC2.0 TA, QC3.0 TA, AFC TA with hv_disable true
					/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/20 start */
					chg->afc_detect_flag = true;
					cancel_delayed_work_sync(&chg->afc_start_work);
					detach_afc();  // related with afc driver
					smblib_hvdcp_detect_try_enable(chg, true);
					/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/20 end */
                    /* F52 code for OLYMPICO5-766 by zhanglinhan at 2022/02/16 start */
				} else {
					if ((chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP ||
						chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3)
                    ...
                    /* F52 code for OLYMPICO5-766 by zhanglinhan at 2022/02/16 end */
						...
						if (rc < 0)
							pr_err("Failed to force 9V\n");
						/* F52 code for P210508-01207 by shixuanxuan at 2021/05/08 start */
						//rc = vote(chg->cp_disable_votable, SEC_BATTERY_DISABLE_HV_VOTER, false, 0);
						//if (rc < 0)
						//	pr_err("line=%d: Couldn't vote cp_disable_votable rc=%d\n",
						//			__LINE__, rc);
						/* F52 code for P210508-01207 by shixuanxuan at 2021/05/08 end */
						rc = vote(chg->usb_icl_votable, SEC_BATTERY_DISABLE_HV_VOTER, false, 0);
						if (rc < 0)
							pr_err("line=%d: Couldn't vote usb_icl_votable rc=%d\n",
									__LINE__, rc);
						/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/29 end */
	...
	pr_info("[%s]line=%d: pd_active=%d, ok_to_pd=%d, chr_type=%d, afc_sts=%d, vbus_rising=%d, hv=%d, usb_icl_votable(%s): %d\n",
		__FUNCTION__, __LINE__, chg->pd_active, chg->ok_to_pd, chg->real_charger_type, chg->afc_sts, chg->vbus_rising, chg->hv_disable,
		usb_icl_voter ? usb_icl_voter : "None", get_effective_result(chg->usb_icl_votable));
}
/* F52 code for P211221-04056 by gaochao at 2022/01/04 end */
static void smblib_hv_resume_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work,
					struct smb_charger, hv_resume_work.work);
				    ...
				    const union power_supply_propval *val)
{
	...
	/* F52 code for SR-QN3979-01-805 SEC_FLOATING_FEATURE_BATTERY_SUPPORT_HV_DURING_CHARGING by gaochao at 2021/04/05 start */
	#if defined(CONFIG_AFC)
	// pr_debug("[%s]line=%d: pd_active=%d, ok_to_pd=%d, hv_disable=%d, val=%d\n",
	//	__FUNCTION__, __LINE__, chg->pd_active, chg->ok_to_pd, chg->hv_disable, val->intval);
	#endif
	/* F52 code for SR-QN3979-01-805 SEC_FLOATING_FEATURE_BATTERY_SUPPORT_HV_DURING_CHARGING by gaochao at 2021/04/05 end */
...
unsuspend_input:
	...
	if (!chg->qc2_unsupported_voltage &&
		...
		#if !defined(HQ_FACTORY_BUILD) // SEP Binary
		if (chg->vbus_rising) {
			// chg->qc2_max_pulses = 0;	// 00=5V; 01=9V; 10=12V
			chg->afc_qc2_ta_check = true;
		}
		#endif
		#endif
		/* F52 code for QN3979-4140 QC2.0 is compatible with AFC by gaochao at 2021/05/12 end */
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		/* F52 code for QN3979-1129 Plug in AFC TA to detect only once by gaochao at 2021/03/09 start */
		#if defined(CONFIG_AFC)
		chg->afc_detect_flag = true;
		#endif
		/* F52 code for QN3979-1129 Plug in AFC TA to detect only once by gaochao at 2021/03/09 end */
	} else {
		...
		#if defined(CONFIG_AFC)
		/* F52 code for QN3979-1129 Plug in AFC TA to detect only once by gaochao at 2021/03/09 start */
		chg->afc_detect_flag = false;
		/* F52 code for QN3979-1129 Plug in AFC TA to detect only once by gaochao at 2021/03/09 end */
		cancel_delayed_work_sync(&chg->afc_start_work);
		detach_afc();  //related with afc driver
		/* F52 code for QN3979-4140 QC2.0 is compatible with AFC by gaochao at 2021/05/12 start */
		#if !defined(HQ_FACTORY_BUILD) // SEP Binary
		cancel_delayed_work_sync(&chg->qc_monitor_work);
		if (chg->real_charger_type != POWER_SUPPLY_TYPE_AFC) {
			chg->afc_qc2_ta_check = false;
		}
		#endif
		/* F52 code for QN3979-4140 QC2.0 is compatible with AFC by gaochao at 2021/05/12 end */
		#endif
	...
	#if defined(CONFIG_AFC)
	chg->qc_check++;
	if (chg->qc_check >= QC_CHECK_COUNT) {
		chg->quick_charge_detect = true;
		chg->qc_check = QC_CHECK_COUNT;
	}
	#endif
	/* F52 code for P211221-04056 by gaochao at 2022/01/04 end */
	...
	if (chg->ss_store_mode == SS_STORE_MODE_ENABLE) {
		if (vbus_rising) {
			pr_err("[%s] start retail_app_work after %d ms\n",
					__FUNCTION__, RETAIL_APP_DETECT_TIMER);
			schedule_delayed_work(&chg->retail_app_work,
				msecs_to_jiffies(RETAIL_APP_DETECT_TIMER));
			rc = vote(chg->fcc_votable, STORE_MODE_VOTER, true, STORE_MODE_FCC);
			if(rc < 0)
				pr_err("[%s] Fail to vote STORE_MODE_VOTER true, rc=%d\n", __FUNCTION__, rc);
		} else {
			pr_err("[%s]cancel retail_app_work\n",
					__FUNCTION__);
			cancel_delayed_work_sync(&chg->retail_app_work);
			rc = vote(chg->fcc_votable, STORE_MODE_VOTER, false, STORE_MODE_FCC);
			if (rc < 0)
				pr_err("[%s] Fail to vote STORE_MODE_VOTER false, rc=%d\n", __FUNCTION__, rc);
		}
	}
	#endif
	/* F52 add for SR-QN3979-01-226 by wenyaqi at 2021/02/10 end */
...
irqreturn_t usb_plugin_irq_handler(int irq, void *data)
{
	...
	/* F52 code for QN3979-60 Add log for battery charging by gaochao at 2021/02/20 start */
	pr_info("[%s]line=%d: pd_hard_reset=%d\n", __FUNCTION__, __LINE__, chg->pd_hard_reset);
	/* F52 code for QN3979-60 Add log for battery charging by gaochao at 2021/02/20 end */
...
static void update_sw_icl_max(struct smb_charger *chg, int pst)
{
	...
	case POWER_SUPPLY_TYPE_USB:
		...
		if (!is_client_vote_enabled(chg->usb_icl_votable,
						USB_PSY_VOTER)) {
			/* if flash is active force 500mA */
			/* F52 code for QN3979-3813 Fix the bug that 100mA not 500mA connected with pc when the phone powered off by qiaodan at 2021/05/08 start */
			/*
			...
			*/
			...
			vote(chg->usb_icl_votable, USB_PSY_VOTER, true,
					SDP_CURRENT_UA);
		#endif
			/* F52 code for QN3979-3813 Fix the bug that 100mA not 500mA connected with pc when the phone powered off by qiaodan at 2021/05/08 end */
		}
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, false, 0);
		break;
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	#if defined(CONFIG_AFC)
	/* F52 code for QN3979-1129 Plug in AFC TA to detect only once by gaochao at 2021/03/09 start */
	if (chg->afc_detect_flag == true) {
		/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 start */
		#if defined(HQ_FACTORY_BUILD)	// factory Binary
		pr_info("[%s]AFC: start AFC work after=%d\n", __FUNCTION__, AFC_START_TIME);
		chg->afc_detect_flag = false;
		schedule_delayed_work(&chg->afc_start_work,
						msecs_to_jiffies(AFC_START_TIME));
		#else
		pr_info("[%s]AFC: start qc_monitor_work after=%d\n",
				__FUNCTION__, QC_MONITOR_TIME);
		chg->afc_detect_flag = false;
		// cancel_delayed_work_sync(&chg->afc_start_work);
		// detach_afc();  // related with afc driver
		// cancel_delayed_work_sync(&chg->qc_monitor_work);
		schedule_delayed_work(&chg->qc_monitor_work,
						msecs_to_jiffies(QC_MONITOR_TIME));
		#endif
		/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 end */
	}
	/* F52 code for QN3979-1129 Plug in AFC TA to detect only once by gaochao at 2021/03/09 end */
...
static void typec_src_insertion(struct smb_charger *chg)
{
	...
	if (!chg->ok_to_pd) {
		...
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't enable APSD rc=%d\n", rc);
			return;
		}
		*/
		// smblib_rerun_apsd_if_required(chg);
		...
		if (chg->hv_disable) {
			smblib_hvdcp_detect_try_enable(chg, false);
			pr_info("[%s]line=%d: enable hv_disable\n",
				__FUNCTION__, __LINE__);
		} else {
			smblib_hvdcp_detect_try_enable(chg, true);
			pr_info("[%s]line=%d: disable hv_disable\n",
				__FUNCTION__, __LINE__);
		}
		#endif
		/* F52 code for SR-QN3979-01-805 Disabling hv_disable during charging by gaochao at 2021/04/20 end */
...
static void smblib_afc_start_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
							afc_start_work.work);
	...
	apsd_result = smblib_update_usb_type(chg);
	typec_mode = smblib_get_prop_typec_mode(chg);
	pr_info("[%s]AFC: pst=%d, bit=%d, typec_mode=%d\n", __FUNCTION__,
			apsd_result->pst, apsd_result->bit, typec_mode);

	/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 start */
	#if defined(HQ_FACTORY_BUILD)	// factory Binary
	if ((apsd_result->bit == DCP_CHARGER_BIT) && (typec_mode == POWER_SUPPLY_TYPEC_SOURCE_DEFAULT)) {
	#else
	if ((apsd_result->pst == POWER_SUPPLY_TYPE_USB_DCP || apsd_result->pst == POWER_SUPPLY_TYPE_USB_HVDCP ||
	...
	*/
		if ((chg->afc_sts == AFC_INIT) && (!chg->hv_disable)) {
			...
			rc = vote(chg->usb_icl_votable, SEC_BATTERY_AFC_VOTER, true, 500000);   /* 500mA for AFC communication  */
			if (rc < 0)
			{
				pr_err("line=%d: Couldn't vote SEC_BATTERY_AFC_VOTER rc=%d\n", __LINE__, rc);
			}

			rc = smblib_set_adapter_allowance(chg, USBIN_ADAPTER_ALLOW_5V_TO_9V);
			// rc = smblib_usb_pd_adapter_allowance_override(chg, FORCE_9V);
			if (rc < 0)
...
/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 start */
#if !defined(HQ_FACTORY_BUILD) // SEP Binary
static void smblib_qc_monitor_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
							qc_monitor_work.work);
	int rc = 0;
	const struct apsd_result *apsd_result = NULL;
	union power_supply_propval pval = {0, };
	...
	if (rc < 0)
		pr_err("Get vbus voltage failed, rc=%d\n", rc);

	pr_info("[%s]AFC: pst=%d, Vbus=%d\n", __FUNCTION__,
		apsd_result->pst, pval.intval);

	/* F52 code for QN3979-4140 QC2.0 is compatible with AFC by gaochao at 2021/05/12 start */
	if ((apsd_result->pst != POWER_SUPPLY_TYPE_USB_HVDCP_3) &&
		(apsd_result->pst == POWER_SUPPLY_TYPE_USB_HVDCP ||
		apsd_result->pst == POWER_SUPPLY_TYPE_USB_DCP ||
		apsd_result->pst == POWER_SUPPLY_TYPE_UNKNOWN) &&
		(pval.intval < HVDCP_INCREASE_THRESHOLD)) {
		pr_info("[%s]AFC: start AFC work after=%d\n", __FUNCTION__, AFC_SHCEDULE_TIME);
	/* F52 code for QN3979-4140 QC2.0 is compatible with AFC by gaochao at 2021/05/12 end */
		smblib_hvdcp_detect_try_enable(chg, false);
		smblib_rerun_apsd(chg);
		/*
		...
		*/
		schedule_delayed_work(&chg->afc_start_work,
						msecs_to_jiffies(AFC_SHCEDULE_TIME));
	}
}
#endif
/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 end */
...
int smblib_init(struct smb_charger *chg)
{
	...
	/* QN3979 add for SR-QN3979-01-58 add usb connector thermal by shixuanxuan at 2021/02/12 start */
	INIT_DELAYED_WORK(&chg->usb_conn_work,smblib_update_usb_conn_work);

	schedule_delayed_work(&chg->usb_conn_work, msecs_to_jiffies(USB_CONN_TIME));
	/* QN3979 add for SR-QN3979-01-58 add usb connector thermal by shixuanxuan at 2021/02/12 end */
	/* F52 add for SR-QN3979-01-226 by wenyaqi at 2021/02/10 start */
	#ifndef HQ_FACTORY_BUILD //ss version
	INIT_DELAYED_WORK(&chg->retail_app_work, ss_retail_app_work);
	#endif
	/* F52 add for SR-QN3979-01-226 by wenyaqi at 2021/02/10 end */
	/* F52 code for QN3979-60 Add log for battery charging by gaochao at 2021/02/20 start */
	INIT_DELAYED_WORK(&chg->battery_chg_info_work,
					smblib_battery_chg_info_work);
	smblib_battery_chg_info_work_init(chg);
	/* F52 code for QN3979-60 Add log for battery charging by gaochao at 2021/02/20 end */
	/* F52 code for QN3979-302 Add AFC feature by gaochao at 2021/02/26 start */
	#if defined(CONFIG_AFC)
	INIT_DELAYED_WORK(&chg->compliant_check_work,
					smblib_compliant_check_work);
	INIT_DELAYED_WORK(&chg->afc_start_work,
					smblib_afc_start_work);
	/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 start */
	#if !defined(HQ_FACTORY_BUILD) // SEP Binary
	INIT_DELAYED_WORK(&chg->qc_monitor_work,
					smblib_qc_monitor_work);
	#endif
	/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 end */
	#endif
	/* F52 code for QN3979-302 Add AFC feature by gaochao at 2021/02/26 end */
	/* F52 code for SR-QN3979-01-805 SEC_FLOATING_FEATURE_BATTERY_SUPPORT_HV_DURING_CHARGING by gaochao at 2021/04/05 start */
	#if defined(CONFIG_AFC)
	INIT_DELAYED_WORK(&chg->hv_resume_work,
					smblib_hv_resume_work);
	#endif
	/* F52 code for SR-QN3979-01-805 SEC_FLOATING_FEATURE_BATTERY_SUPPORT_HV_DURING_CHARGING by gaochao at 2021/04/05 end */
	...
	/* F52 add for SR-QN3979-01-226 by wenyaqi at 2021/02/10 start */
	#ifndef HQ_FACTORY_BUILD //ss version
	chg->ss_store_mode = SS_STORE_MODE_DISABLE;
	#endif
	/* F52 add for SR-QN3979-01-226 by wenyaqi at 2021/02/10 end */
	/* F52 code for QN3979-1129 Plug in AFC TA to detect only once by gaochao at 2021/03/09 start */
	#if defined(CONFIG_AFC)
	...
	/* F52 code for SR-QN3979-01-805 SEC_FLOATING_FEATURE_BATTERY_SUPPORT_HV_DURING_CHARGING by gaochao at 2021/04/05 start */
	#if defined(CONFIG_AFC)
	chg->custom_fine_tune_hv_resume_debug = false;
	chg->vbus_rising = false;
	/* F52 code for QN3979-4140 QC2.0 is compatible with AFC by gaochao at 2021/05/12 start */
	#if !defined(HQ_FACTORY_BUILD) // SEP Binary
	chg->afc_qc2_ta_check = false;
	...
	#endif /* CONFIG_AFC */
	/* F52 code for QN3979-4140 QC2.0 is compatible with AFC by gaochao at 2021/05/12 end */
	/* F52 code for P210508-01207 by qiaodan at 2021/05/10 start */
	#if defined(CONFIG_AFC)
	/* F52 code for P211221-04056 by gaochao at 2022/01/04 start */
	chg->qc_check = 0;
	/* F52 code for P211221-04056 by gaochao at 2022/01/04 end */
	chg->quick_charge_detect = false;
	#endif
	/* F52 code for P210508-01207 by qiaodan at 2021/05/10 end */
	switch (chg->mode) {
...
int smblib_deinit(struct smb_charger *chg)
{
	...
	case PARALLEL_MASTER:
		...
		/* F52 code for QN3979-60 Add log for battery charging by gaochao at 2021/02/20 start */
		cancel_delayed_work_sync(&chg->battery_chg_info_work);
		/* F52 code for QN3979-60 Add log for battery charging by gaochao at 2021/02/20 end */
		/* F52 code for QN3979-302 Add AFC feature by gaochao at 2021/02/26 start */
		#if defined(CONFIG_AFC)
		cancel_delayed_work_sync(&chg->compliant_check_work);
		...
		/* F52 code for SR-QN3979-01-805 SEC_FLOATING_FEATURE_BATTERY_SUPPORT_HV_DURING_CHARGING by gaochao at 2021/04/05 start */
		#if defined(CONFIG_AFC)
		cancel_delayed_work_sync(&chg->hv_resume_work);
		#endif
		/* F52 code for SR-QN3979-01-805 SEC_FLOATING_FEATURE_BATTERY_SUPPORT_HV_DURING_CHARGING by gaochao at 2021/04/05 end */
...
int is_afc_result(struct smb_charger *chg, int result)
{
	...
	if (chg->real_charger_type != POWER_SUPPLY_TYPE_USB_DCP
			&& chg->real_charger_type != POWER_SUPPLY_TYPE_AFC) {
		/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 start */
		smblib_err(chg, "cable is not DCP OR AFC %d, chg_type=%d\n", result, chg->real_charger_type);
		/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 end */
		rc = vote(chg->usb_icl_votable, SEC_BATTERY_AFC_VOTER, false, 0);
		if (rc < 0)
		{
			pr_err("line=%d: Couldn't vote SEC_BATTERY_AFC_VOTER rc=%d\n", __LINE__, rc);
		}
	...
	/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 start */
	smblib_err(chg, "is_afc_result = %d, before afc_sts(%d), chg_type=%d\n", result, chg->afc_sts, chg->real_charger_type);
	/* F52 code for SR-QN3979-01-805 QC2.0 and QC3.0 for disabling hv_disable during charging by gaochao at 2021/04/19 end */
	chg->afc_sts = result;
	...
	if ((result == NOT_AFC) || (result == AFC_FAIL))  {
		...
		smblib_hvdcp_set_fsw(chg, QC_5V_BIT);
		rc = vote(chg->usb_icl_votable, SEC_BATTERY_AFC_VOTER, true, DCP_CURRENT_UA);
		if (rc < 0)
		{
			pr_err("line=%d: Couldn't vote SEC_BATTERY_AFC_VOTER rc=%d\n", __LINE__, rc);
		}
		...
		smblib_hvdcp_set_fsw(chg, QC_9V_BIT);
		// vote(chg->usb_icl_votable, HVDCP2_ICL_VOTER, false, 0);
		rc = vote(chg->usb_icl_votable, SEC_BATTERY_AFC_VOTER, true, AFC_CURRENT_UA);
		if (rc < 0)
		{
			pr_err("line=%d: Couldn't vote SEC_BATTERY_AFC_VOTER rc=%d\n", __LINE__, rc);
		}

		rc = vote(chg->fcc_votable, SEC_BATTERY_AFC_VOTER, true, 2700000);
		if (rc < 0)
		{
			pr_err("line=%d: Couldn't vote SEC_BATTERY_AFC_VOTER rc=%d\n", __LINE__, rc);
		}
