...
static void set_hard_jeita_dynamically(struct step_chg_info *chip)
{
	...
	if (bat_temperature.intval <= BAT_TEMPERATURE_NEGATIVE_0)
	{
	...
	else if ((bat_temperature.intval >= BAT_TEMPERATURE_POSITIVE_3) && (bat_health_status.intval == POWER_SUPPLY_HEALTH_COOL))
	{
	...
	else if ((bat_temperature.intval >= BAT_TEMPERATURE_POSITIVE_50)
	...
	else if ((bat_temperature.intval <= BAT_TEMPERATURE_POSITIVE_47) && (bat_health_status.intval == POWER_SUPPLY_HEALTH_WARM))
	{
