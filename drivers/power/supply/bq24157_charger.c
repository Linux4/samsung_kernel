...
enum bq2415x_charging_status {
	BQ2415X_CHG_STATUS_READY = 0,
	BQ2415X_CHG_STATUS_PROGRESS,
	BQ2415X_CHG_STATUS_DONE,
	BQ2415X_CHG_STATUS_FAULT,
	BQ2415X_CHG_STATUS_MAX,
};
...
struct bq2415x {
	...
	enum bq2415x_charging_status chg_state;
};
...
static int bq2415x_parse_dt(struct device *dev, struct bq2415x *bq)
{
	...
	ret = of_get_named_gpio(np, "bq24157,intr_gpio", 0);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np,
		"bq24157,intr_gpio_num", &bq->irq_gpio);
	if (ret < 0)
		pr_err("%s no intr_gpio info\n", __func__);
#endif
	return 0;
...
static int bq2415x_get_charging_status(struct bq2415x *bq,
	enum bq2415x_charging_status *chg_stat)
{
	int ret = 0;
	u8 val;
	...
	*chg_stat = (val & BQ2415X_STAT_MASK) >> BQ2415X_STAT_SHIFT;
...
{
	struct bq2415x *bq = power_supply_get_drvdata(psy);
	enum bq2415x_charging_status chg_stat = BQ2415X_CHG_STATUS_READY;
	int ret = 0;
	...
	case POWER_SUPPLY_PROP_STATUS:
		ret = bq2415x_get_charging_status(bq, &chg_stat);
		switch (chg_stat) {
		case BQ2415X_CHG_STATUS_READY:
			val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
			break;
		case BQ2415X_CHG_STATUS_PROGRESS:
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
			break;
		case BQ2415X_CHG_STATUS_DONE:
			val->intval = POWER_SUPPLY_STATUS_FULL;
			break;
		case BQ2415X_CHG_STATUS_FAULT:
...
static enum power_supply_property bq2415x_charger_properties[] = {
	POWER_SUPPLY_PROP_STATUS,
	POWER_SUPPLY_PROP_ONLINE,
};

static const struct power_supply_desc bq2415x_charger_desc = {
	.type			= POWER_SUPPLY_TYPE_UNKNOWN,
	.properties		= bq2415x_charger_properties,
	.num_properties		= ARRAY_SIZE(bq2415x_charger_properties),
	.get_property		= bq2415x_charger_get_property,
//	.usb_types		= bq2415x_charger_usb_types,
//	.num_usb_types		= ARRAY_SIZE(bq2415x_charger_usb_types),
};

static char *bq2415x_charger_supplied_to[] = {
	"battery",
	"mtk-master-charger"
};

static irqreturn_t bq24157_intr_handler(int irq, void *data)
{
	struct bq2415x *bq = data;
	enum bq2415x_charging_status chg_stat = BQ2415X_CHG_STATUS_READY;

	bq2415x_get_charging_status(bq, &chg_stat);
	if (chg_stat != bq->chg_state) {
		bq->chg_state = chg_stat;
		power_supply_changed(bq->psy);
	}
pr_err("intr handler\n");
	return IRQ_HANDLED;
...
static int bq2415x_init_alert(struct bq2415x *bq)
{
	int ret;

	ret = devm_gpio_request(bq->dev, bq->irq_gpio, "bq24157-irq");
	if (ret < 0) {
...
//	ret = request_irq(bq->irq, bq24157_intr_handler,
//		IRQF_TRIGGER_FALLING | IRQF_NO_THREAD, "bq24157-irq", bq);
	ret = devm_request_threaded_irq(bq->dev, bq->irq, NULL,
					bq24157_intr_handler,
					IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
					"bq24157-irq", bq);
	if (ret < 0) {
		pr_err("Error: failed to request irq%d (gpio = %d, ret = %d)\n",
			bq->irq, bq->irq_gpio, ret);
		return -EINVAL;
	}
...
//-bug 436809  modify getian.wt 20190403 Add charger IC model information in factory mode
	...
	if(!ret && (is_eta6937(bq) || is_hl7057(bq))){
		...
		bq->eta_charger_lock = wakeup_source_register(NULL, "ChargerIc suspend wakelock");
	...
	memcpy(&bq->psy_desc,
		&bq2415x_charger_desc, sizeof(bq->psy_desc));
	bq->psy_desc.name = "bq24157";
	charger_cfg.drv_data = bq;
	charger_cfg.of_node = client->dev.of_node;
	charger_cfg.supplied_to = bq2415x_charger_supplied_to;
	charger_cfg.num_supplicants = ARRAY_SIZE(bq2415x_charger_supplied_to);
	...
	ret = bq2415x_init_alert(bq);
	if (ret) {
		pr_err("bq2415x_init_alert failure: %d\n", ret);
	}
