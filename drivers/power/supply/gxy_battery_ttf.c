...
extern struct charger_manager *ttf_cm;

static bool gxy_ttf_init_psy(struct gxy_battery_ttf *battery) {
    bool value = true;
    int i = 0;
    struct charger_manager *cm = ttf_cm;
    ...
    if (IS_ERR_OR_NULL(battery->psy_chg)) {
        ...
        for (i = 0; cm->desc->psy_charger_stat[i]; i++) {
            battery->psy_chg = power_supply_get_by_name(cm->desc->psy_charger_stat[i]);
            if (IS_ERR_OR_NULL(battery->psy_chg)) {
...
static int get_fast_charger_type(void) {
    struct charger_desc *desc = ttf_cm->desc;
    int fast_charger_type = 0;
    if (!desc) {
        pr_err("%s:line%d: NULL pointer!!!\n", __func__, __LINE__);
        return -EINVAL;
    }
    fast_charger_type = desc->charger_type;
...
static void gxy_bat_calc_time_to_full(struct gxy_battery_ttf *battery)
{
    ...
    if (delayed_work_pending(&battery->ttf_d->timetofull_work)) {
        ...
        fast_charger_type = get_fast_charger_type();
        battery->max_charge_power = (battery->usb_vol_now) * (battery->batt_curr_now / 1000) / 1000;
        pr_err("%s: usb_vol_now[%d], batt_curr_now[%d], usb_type[%d],fast_charger_type[%d], max_charge_power[%d]\n", __func__,
            battery->usb_vol_now, battery->batt_curr_now, battery->usb_type, fast_charger_type, battery->max_charge_power);
        // USB TYPE judge
        if (fast_charger_type == POWER_SUPPLY_USB_CHARGER_TYPE_AFC ||
            fast_charger_type == POWER_SUPPLY_USB_CHARGER_TYPE_PD) { /* high voltage charging */
