...
#define RT9471_DISABLE_PIN_MASK		BIT(0)
...
static const struct regulator_desc rt9471_charger_multi_vbus_desc = {
...
static int rt9471_charger_register_vbus_regulator(struct rt9471_chip *chip)
{
	...
	if (!chip->enable_multi_charger_adapt) {
		reg = devm_regulator_register(chip->dev,
					      &rt9471_charger_vbus_desc, &cfg);
		dev_info(chip->dev, "use rt9471_charger_vbus_desc\n");
	} else {
		reg = devm_regulator_register(chip->dev,
					      &rt9471_charger_multi_vbus_desc, &cfg);
		dev_info(chip->dev, "use rt9471_charger_multi_vbus_desc\n");
	}
	if (IS_ERR(reg)) {
		ret = PTR_ERR(reg);
			...
			const struct i2c_device_id *id)
{
	...
	chip->enable_multi_charger_adapt =
	...
	if (regmap_np) {
		if (of_device_is_compatible(regmap_np->parent, "sprd,sc2721"))
			chip->charger_pd_mask = RT9471_DISABLE_PIN_MASK_2721;
		else
			chip->charger_pd_mask = RT9471_DISABLE_PIN_MASK;
	} else {
	...
	ret = of_property_read_u32_index(regmap_np, "reg", 2,
						 &chip->charger_pd);
	if (ret) {
		dev_err(dev, "failed to get charger_pd reg, ret = %d\n", ret);
		return ret;
	}
