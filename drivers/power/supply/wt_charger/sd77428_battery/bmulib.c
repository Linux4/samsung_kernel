...
void bmu_init_chip(parameter_data_t *paramter_temp)
{
	...
	gas_gauge->batt_ri = 120; //120 // charging. It is related to voltage when full charging condition.
...
void wait_ocv_flag_fun(void)
{
	...
	bmt_dbg("wait_ocv_flag_fun:poweron_flag: %d ,fRC %d \n",poweron_flag,batt_info->fRC);
	if(poweron_flag < 2)
	{
		afe_read_ocv_volt(&batt_info->fOCVVolt);
...
{
	int32_t sleep_ocv_soc = 0;
	int32_t sleep_ocv_mah = 0;

	uint32_t diff = one_percent_rc * 20;
...
void bmu_polling_loop(void)
{
	...
	*/
	...
	time_sec = oz8806_get_system_boot_time();
		...
		"[bmt-fg] adapter_status: %d, soh: %d, cycle: %d\n"
		"[bmt-fg] ----------------------------------------------------\n",
		...
		adapter_status,age_soh,batt_info->cycle_count);
...
static int32_t oz8806_cell_voltage_read(int32_t *voltage)
{
	...
	ret = sd77428_read_addr(the_oz8806,0x40004114,&reg);
	if(ret)
		return -1;
...
static int32_t oz8806_current_read(int32_t *data)
{
	...
	ret = sd77428_read_addr(the_oz8806,0x400041A8,&reg);			//250ms
	// ret = sd77428_read_addr(the_oz8806,0x400041AC,&reg);			//1s
	if(ret)
		return -1;
...
int32_t oz8806_temp_read(int32_t *voltage)
{
	...
	ret = sd77428_read_addr(the_oz8806,0x40004118,&reg);
	if(ret)
		return -1;
...
static int32_t oz8806_car_read(int32_t *car)
{
	...
	ret = sd77428_read_addr(the_oz8806,0x400041B8,&reg);
	if(ret)
		return -1;
...
static int32_t update_ntc_temp(int32_t* ntc_temp)
{
    int8_t i;
    uint32_t reg_value = 0;
    uint32_t ts_6ua;
    int32_t ntc_mohm, temp_ntc;
	int32_t ret = 0;
	int32_t Vts6 = 0;
	int32_t iTs6parrel = 0;

	//6ua
	if(sd77428_write_addr(the_oz8806,0x40004164,0x0000) < 0)
	{
		bmt_dbg("Set 0x40004164 output failed.\n");
		goto error;
	}
	usleep_range(10000, 11000);

	if(sd77428_write_addr(the_oz8806,0x40004360,0x2000) < 0)
	{
		bmt_dbg("Set 0x40004360 output failed.\n");
		goto error;
    }
	usleep_range(10000, 11000);
	...
	{
		ret = sd77428_read_addr(the_oz8806,0x40004360,&reg_value);
		if(ret >= 0)
			break;
		msleep(5);
	}
	if(i >= RETRY_CNT)
	{
	    bmt_dbg("read 0x360 failed.\n");
		goto error;
	}

	if(sd77428_write_addr(the_oz8806,0x40004170,0x0103) < 0)
    {
        bmt_dbg("Set 0x170 output failed.\n");
		goto error;
    }
	...
	for(i = 0; i < RETRY_CNT; i++)
	{
		ret = sd77428_read_addr(the_oz8806,0x40004118,&reg_value);
		if (ret >= 0)
			break;
		msleep(5);
	}
	...
	if(i >= RETRY_CNT)
	{
	    bmt_dbg("read 0x118 failed.\n");
		goto error;
	}

    for(i = 0; i < RETRY_CNT; i++)
	{
		ret = sd77428_read_addr(the_oz8806,0x40004170,&reg_value);
		if (ret >= 0)
			break;
		msleep(5);
	}
	if(i >= RETRY_CNT)
	{
		bmt_dbg("read 0x170 failed.\n");
		goto error;
	}

	Vts6 = (ts_6ua*354 - SERIES_RESISTANCE*6*10)/10;//uv
	iTs6parrel = (config_data->temp_ref_voltage * 1000-Vts6)/config_data->temp_pull_up + 6;//ua
	ntc_mohm = Vts6 / iTs6parrel;

	temp_ntc = one_latitude_table(parameter->cell_temp_num,parameter->temperature,ntc_mohm);
	*ntc_temp = temp_ntc;

    pr_info("ts_6ua:0x%04x, ntc_mohm:%d, ntc_temp:%d Vts6=%d iTs6parrel=%d\n",ts_6ua, ntc_mohm, *ntc_temp,Vts6,iTs6parrel);
 ...
 *		it's acceptable if one or more times reading fail
 *****************************************************************************/
static int32_t bat_temp = 0;
static int32_t afe_read_cell_temp(int32_t *data)
{
	int32_t ret = 0;
	int32_t ntc_temp1 = 0, ntc_temp2 = 0;
	int32_t ntc_temp = 25;

	bat_temp = ntc_temp;
	ret = update_ntc_temp(&ntc_temp1);
    ret = update_ntc_temp(&ntc_temp2);
	ntc_temp = ntc_temp2;
	bmt_dbg("yyyy. ntc_temp1:%d, ntc_temp2: %d,bat_temp: %d\n",ntc_temp1,ntc_temp2,bat_temp);

    if(ABS(ntc_temp1,ntc_temp2) > 3)
		ntc_temp = bat_temp;
	if(ret < 0)
	{
		*data = num_25;
	...
	ntc_temp = (8 * bat_temp + 2 * ntc_temp)/10;
	*data = ntc_temp;
	bmt_dbg("yyyy. *data:%d\n",ntc_temp);
