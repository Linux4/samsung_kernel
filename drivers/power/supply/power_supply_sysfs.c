...
static ssize_t power_supply_show_property(struct device *dev,
	...
	if (psp == POWER_SUPPLY_PROP_TYPE) {
		...
		if(psy->desc->type == POWER_SUPPLY_TYPE_USB_PD) {
			ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_PD_ACTIVE, &value);

			if((value.intval == POWER_SUPPLY_PD_ACTIVE) && (ret >= 0)) { //powered by another phone
	...
	/* F52 for 210330-04518 modify by qiaodan at 2021/04/20 start */
		ret = sprintf(buf, "%s\n",
			      power_supply_type_text[value.intval]);
		break;
	/* F52 for 210330-04518 modify by qiaodan at 2021/04/20 end */
...
static struct device_attribute power_supply_attrs[] = {
	...
	#if defined(CONFIG_AFC)
	POWER_SUPPLY_ATTR(custom_fine_tune_hv_resume_debug),
	#endif
