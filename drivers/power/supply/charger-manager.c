...
//+ExtB[P220318-04675] yuecong.wt.20220406 add afc_disable sys node
...
#define CM_CAP_CYCLE_TRACK_TIME			10 // fix track time by guoyanjun
#define CM_UVLO_OFFSET				50000
#define CM_FORCE_SET_FUEL_CAP_FULL		1000
#define CM_LOW_TEMP_REGION			100
#define CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD	3450000 // set shutdown check voltage is 3450 by guoyanjun
#define CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD_LOW	3250000 // set shutdown check voltage is 3320 in low temp by liyiying
#define CM_UVLO_CALIBRATION_TEMP_THRESHOLD_LOW	50 // set low temp by liyiying
#define CM_UVLO_CALIBRATION_CNT_THRESHOLD	5
#define CM_LOW_TEMP_SHUTDOWN_VALTAGE		3100000
...
#define CM_FAST_CHARGE_ENABLE_CURRENT		800000 // change enter fast charge current threshold by guoyanjun
#define CM_FAST_CHARGE_ENABLE_THERMAL_CURRENT	1000000
#define CM_FAST_CHARGE_DISABLE_BATTERY_VOLTAGE	3400000
#define CM_FAST_CHARGE_DISABLE_CURRENT		600000 // change exit fast charge current threshold by guoyanjun
...
#define CM_FAST_CHARGE_DISABLE_COUNT		4
...
#define CM_SET_IBAT_0_MA			0

#define CM_IR_COMPENSATION_TIME			3
#define CM_POWEROFF_TMP 		880
#define CM_POWEROFF_CNT_THRESHOLD	5

//+ExtB[P220411-01778] tankaikun.wt, add, 20220406. Add charger current step control.
#define DEFAULT_FCC_STEP_UPDATE_DELAY_MS 	1000	// 1s
#define DEFAULT_FCC_STEP_SIZE_UA		200000  // 200mA
#define DEFAULT_FCC_MAIN_UA			500000  // 500mA

#define STEP_UP 1
#define STEP_DOWN -1
//-ExtB[P220411-01778] tankaikun.wt, add, 20220406. Add charger current step control.
#define CM_CP_WORK_TIME_MS			500
#define printb(fmt, args...) do { printk("[wt_set][charger_manager][%s] "fmt, __FUNCTION__, ##args); } while (0)
#define printc(fmt, args...) do { printk("[wt_usb][charger_manager][%s:%d] "fmt, __FUNCTION__,__LINE__, ##args); } while (0)
...
struct charger_manager *wt_cm = NULL;
...
static struct charger_type charger_type[20] = {
	...
	{POWER_SUPPLY_USB_TYPE_AFC, POWER_SUPPLY_USB_CHARGER_TYPE_AFC}, // add afc type by guoyanjun 
...
//+ add charger ic dump reg by guoyanjun
static int dump_charger_reg(struct charger_manager *cm)
{
	union power_supply_propval val;
	struct power_supply *psy;
	int i, ret = -ENODEV;
	...
	for (i = 0; cm->desc->psy_charger_stat[i]; i++) {
		...
		ret = power_supply_get_property(psy,
						POWER_SUPPLY_PROP_DUMP_CHARGER_REG,
						&val);
		power_supply_put(psy);
	}
...
static bool is_full_charged(struct charger_manager *cm)
{
	...
	if (desc->fullbatt_uV > 0 && desc->fullbatt_uA > 0) {
		...
		if (uV >= desc->fullbatt_uV && uA <= desc->fullbatt_uA && uA >= 0) {
			printc("#batfull: uV&uA up to fullbat requirements(%d)!\n",desc->trigger_cnt);
...
static void cm_update_charge_info(struct charger_manager *cm, int cmd)
{
	...
	case POWER_SUPPLY_USB_CHARGER_TYPE_SDP:
		...
		thm_info->adapter_default_charge_vol = 5;
		cm->desc->time_to_full_report = 1; //ExtB[P220425-01299] tankaikun.wt, add, 20220426. report time to full after fcc step up complete
		if (desc->jeita_tab_size)
	...
	case POWER_SUPPLY_USB_CHARGER_TYPE_AFC: // add afc type by guoyanjun
	...
	printc("chgr type = %d, fchg_en = %d, cp_running = %d, cp_recovery = %d"
		 ...
		 desc->charger_type, desc->enable_fast_charge, desc->cp.cp_running,
...
//+ExtB[P220411-01778] tankaikun.wt, add, 20220406. Add charger current step control.
static void get_fcc_stepper_params(struct charger_manager *cm, int main_fcc_ua){
	printc("get_fcc_stepper_params fcc_ua = %d fcc_ua_now = %d \n",main_fcc_ua, cm->desc->main_fcc_ua);

	cm->desc->main_step_fcc_dir =
			(main_fcc_ua > cm->desc->main_fcc_ua) ?
				STEP_UP : STEP_DOWN;
	cm->desc->main_step_fcc_count =
			abs(main_fcc_ua - cm->desc->main_fcc_ua) /
				DEFAULT_FCC_STEP_SIZE_UA;
	cm->desc->main_step_fcc_residual =
			abs(main_fcc_ua - cm->desc->main_fcc_ua) %
				DEFAULT_FCC_STEP_SIZE_UA;
	printc("get_fcc_stepper_params dir=%d fcc_count=%d fcc_residual=%d\n",cm->desc->main_step_fcc_dir, cm->desc->main_step_fcc_count,
																				cm->desc->main_step_fcc_residual);
}
...
static void fcc_stepper_work(struct work_struct *work)
{
	...
	struct charger_manager *cm = container_of(dwork,
			struct charger_manager, fcc_stepper_work);
	const char **psy_charger_name;
	int reschedule_ms = 0;
	int main_fcc = cm->desc->main_fcc_ua;

	psy_charger_name = cm->desc->psy_charger_stat;

	printc("main_fcc=%d \n", main_fcc);
	if (cm->charger_enabled){
		if (cm->desc->main_step_fcc_count) {
			main_fcc += (DEFAULT_FCC_STEP_SIZE_UA * cm->desc->main_step_fcc_dir);
			cm->desc->main_step_fcc_count--;
			reschedule_ms = DEFAULT_FCC_STEP_UPDATE_DELAY_MS;
		}else if (cm->desc->main_step_fcc_residual) {
			main_fcc += cm->desc->main_step_fcc_residual;
			cm->desc->main_step_fcc_residual = 0;
		}
	}else{
		reschedule_ms = DEFAULT_FCC_STEP_UPDATE_DELAY_MS;
	}

	//+ExtB[P220425-01299] tankaikun.wt, add, 20220426. report time to full after fcc step up complete
	if(cm->desc->main_step_fcc_count == 0){
		cm->desc->time_to_full_report = 1;
	}
	//-ExtB[P220425-01299] tankaikun.wt, add, 20220426. report time to full after fcc step up complete

	cm->desc->main_fcc_ua = main_fcc;
	printc("main_step_fcc_count = %d new_main_fcc=%d \n", cm->desc->main_step_fcc_count, main_fcc);

	cm_vote_property(cm, main_fcc, psy_charger_name,
			 POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT);

	if (reschedule_ms) {
		schedule_delayed_work(&cm->fcc_stepper_work,
				msecs_to_jiffies(reschedule_ms));
		printc("Rescheduling FCC_STEPPER work\n");
		return;
	}
				  ...
				  int value, void *data)
{
	...
	printc("%s[%d]\n", vote_type_names[vote_type], value);
	...
	case SPRD_VOTE_TYPE_IBAT:
		...
		if(value > DEFAULT_FCC_MAIN_UA){
			get_fcc_stepper_params(cm, value);
			cancel_delayed_work_sync(&cm->fcc_stepper_work);
			schedule_delayed_work(&cm->fcc_stepper_work,0);
		}else {
			cancel_delayed_work(&cm->fcc_stepper_work);
			cm_vote_property(cm, value, psy_charger_name,
					 POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT);
			cm->desc->main_fcc_ua = value;
		}
		//-ExtB[P220411-01778] tankaikun.wt, add, 20220413. Add charger current step control.
		break;
	...
	case SPRD_VOTE_TYPE_CCCV:
		//+ Chk 1803,guoyanjun.wt,ADD,2021/12/10,If remove the temperature control,set CV to maximum safe voltage
		#ifdef CONFIG_DISABLE_TEMP_PROTECT
		printc("## Disable temp protect! set cv is 4.1V !\n");
		value = 4100000;
		#endif
		//- Chk 1803,guoyanjun.wt,ADD,2021/12/10,If remove the temperature control,set CV to maximum safe voltage
		//- Bug 704565,guoyanjun.wt,MODIFY,2021/11/07,bringup,don't set cv when vbat>cv_value

		//+bug782586, liyiying.wt, 2022/8/18, del, do not stop charging at temp-55
		/*
		get_vbat_now_uV(cm, &batt_uV);
		if(batt_uV >= value){
			printb("## %d > %d, batovp ! stop charge!\n",batt_uV,value);
			cm->wt_charger_enabled = false;
			try_charger_enable(cm, false);

			power_supply_changed(cm->charger_psy);
			cm->wt_charging_status |= WT_CHARGE_BATOVP_DISCHG;

			break;
		}else if((batt_uV < value-50000) && (cm->wt_charging_status & WT_CHARGE_BATOVP_DISCHG))
		{
			if(!(cm->wt_charging_status & ~WT_CHARGE_BATOVP_DISCHG)){
				printb("## %d < %d, not batovp,restore charge\n",batt_uV,value-50000);
				cm->wt_charger_enabled = true;
				try_charger_enable(cm, true);
				power_supply_changed(cm->charger_psy);
			}else{
				printb("Other ctrl discharge(0x%x)!\n",cm->wt_charging_status);
			}
			cm->wt_charging_status &= ~WT_CHARGE_BATOVP_DISCHG;
		}
		*/
...
//+ExtB[P220318-04675] yuecong.wt.20220406 add hv_disable sys node
extern bool hv_disable;
//-ExtB[P220318-04675] yuecong.wt.20220406 add hv_disable sys node
...
static int cm_fast_charge_enable_check(struct charger_manager *cm)
{
	...
	//+ExtB[P220318-04675] yuecong.wt.20220406 add hv_disable sys node
	if (hv_disable){
		dev_err(cm->dev, "## disable fast charging!!!\n");
		return 0;
	}
	//-ExtB[P220318-04675] yuecong.wt.20220406 add hv_disable sys node
	/*
...
static int cm_fast_charge_disable(struct charger_manager *cm)
{
	...
	dev_err(cm->dev, "cm_fast_charge_disable()\n");
	if (!desc->enable_fast_charge)
		return 0;
...
static int cm_fast_charge_disable_check(struct charger_manager *cm)
{
	...
	dev_err(cm->dev, "cm_fast_charge_disable_check(): disable fast charge\n");
	cm->desc->fast_charge_disable_count = 0;
...
{
	int err = 0;
	printc("fast_charger_type = %d,enable = %d\n",cm->desc->fast_charger_type,enable);
	if (cm->desc->fast_charger_type != POWER_SUPPLY_USB_CHARGER_TYPE_PD &&
	    ...
	    cm->desc->fast_charger_type != POWER_SUPPLY_USB_CHARGER_TYPE_AFC){ // add afc type by guoyanjun
		return 0;
	}
	//+Bug 699489,guoyanjun.wt,add,2021/11/02,bringup,ato version control capacity range of 30%~80%
	if(!cm->wt_charger_enabled){
		enable = cm->wt_charger_enabled;
	}
	//-Bug 699489,guoyanjun.wt,add,2021/11/02,bringup,ato version control capacity range of 30%~80%
	printc("enable = %d\n",enable);
	if (enable) {
...
static int try_charger_enable_by_psy(struct charger_manager *cm, bool enable)
{
	...
	for (i = 0; desc->psy_charger_stat[i]; i++) {
		...
		//+Bug 699489,guoyanjun.wt,add,2021/11/02,bringup,ato version control capacity range of 30%~80%	
		if(!cm->wt_charger_enabled)
			val.intval = cm->wt_charger_enabled;
		//-Bug 699489,guoyanjun.wt,add,2021/11/02,bringup,ato version control capacity range of 30%~80%
...
static int try_charger_enable(struct charger_manager *cm, bool enable)
{
	...
	if (enable) {
		...
		cm->desc->main_fcc_ua = DEFAULT_FCC_MAIN_UA - DEFAULT_FCC_STEP_SIZE_UA; //ExtB[P220411-01778] tankaikun.wt, add, 20220413. Add charger current step control.
		cm->charging_end_time = ktime_to_ms(ktime_get());
		cm_check_cp_start_condition(cm, enable);
		cm_ir_compensation_enable(cm, enable);
		err = try_charger_enable_by_psy(cm, enable);
		cancel_delayed_work(&cm->fcc_stepper_work); //ExtB[P220425-01299] tankaikun.wt, add, 20220426. report time to full after fcc step up complete
	}
	if (!err) {
...
static bool cm_force_set_hiz_mode(struct charger_manager *cm, bool enable)
{
	union power_supply_propval val;
	struct power_supply *psy;
	int i, ret = -ENODEV;
	...
	for (i = 0; cm->desc->psy_charger_stat[i]; i++) {
		...
		val.intval = enable;
		ret = power_supply_set_property(psy, POWER_SUPPLY_PROP_FORCE_HIZ_MODE, &val);
		power_supply_put(psy);
		if (ret) {
			dev_err(cm->dev, "failed to %s hiz mode, ret = %d\n",
				enable ? "enable" : "disable", ret);
			return false;
		}
	}
...
static bool dishiz_flag = false;
module_param_named(disable_hiz,dishiz_flag,bool, S_IRUGO | S_IWUSR);
static void wt_batt_works(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct charger_manager *cm = container_of(dwork,
				struct charger_manager, wt_update_work);
	int batt_uV = 0;
	get_vbat_now_uV(cm, &batt_uV);
	if(batt_uV < 3500000){
		dishiz_flag = true;
		printc("#hiz_mode: vbat < 3450mv, dishiz_flag changed: %d\n",dishiz_flag);
	}
	if(dishiz_flag){
		printc("#hiz_mode: dishiz_flag changed: %d\n",dishiz_flag);
		if(dishiz_flag && cm->desc->force_hiz){
			printc("#hiz_mode: disable hiz mode!\n");
			cm->desc->force_hiz = false;
			cm_force_set_hiz_mode(cm,false);
		}
	}else{
		printc("#hiz_mode: dishiz_flag = %d\n",dishiz_flag);
		queue_delayed_work(system_power_efficient_wq,&cm->wt_update_work,1 * HZ);
	}
}
//- Bug 702456,guoyanjun.wt,ADD,2021/12/13,enable hiz mode at ATO version
...
static void check_charging_duration(struct charger_manager *cm)
{
	...
	if (cm->charger_enabled) {
		...
		printc("#timeout: Charging time %ums\n",duration);
		
		if (duration > desc->charging_max_duration_ms &&
		    diff < desc->fullbatt_vchkdrop_uV) {
			printc("#timeout: Charging duration exceed %ums\n",
		...
		if (duration > desc->discharging_max_duration_ms) {
			printc("#timeout: Discharging duration exceed %ums\n",
				 desc->discharging_max_duration_ms);
			cm->charging_status &= ~CM_CHARGE_DURATION_ABNORMAL;
		}
	}
...
static int cm_check_thermal_status(struct charger_manager *cm)
{
	...
	printc("temp=%d,upper_limit=%d,lower_limit=%d,ret=%d\n",temp,upper_limit,lower_limit,ret);
...
static bool cm_manager_adjust_current(struct charger_manager *cm, int jeita_status)
{
	...
	if (jeita_status == 0 || jeita_status == desc->jeita_tab_size) {
		printc("stop charging due to battery overheat or cold(%d)\n",jeita_status);
		try_charger_enable(cm, false);
	...
	if (cm->desc->cp.cp_running && !cm_check_primary_charger_enabled(cm)) {
		printc("cp target terminate voltage = %d, target current = %d\n",
			 term_volt, target_cur);
		cm->desc->cp.cp_target_ibat = target_cur;
		goto exit;
	}

	printc("target terminate voltage = %d, target current = %d, batovp_trigger = %d\n",
		 term_volt, target_cur, batovp_trigger);

	//+bug782586, liyiying.wt, 2022/8/18, mod, do not stop charging at temp-55
	get_vbat_now_uV(cm, &batt_uV);
	if (batt_uV > term_volt) {
		batovp_trigger = 2;
		cm->desc->bat_ovp_flag = 1;
		printb("## %d > %d, batovp ! stop charge!\n", batt_uV, term_volt);
		cm->cm_charge_vote->vote(cm->cm_charge_vote, true,
				 SPRD_VOTE_TYPE_IBAT,
...
out:
	last_temp = cur_temp;
	printc("jeita status:(%d) %d %d, temperature:%d, jeita_size:%d\n",
		 jeita_status, temp_status, recovery_temp_status,
...
static int cm_manager_jeita_current_monitor(struct charger_manager *cm)
{
	...
	printc("current-last jeita status: %d-%d, current temperature: %d\n",
	...
	if (cur_jeita_status > last_jeita_status) {
		...
		temp_up_trigger = 0;
		temp_down_trigger = 0;
		//+bug782586, liyiying.wt, 2022/8/18, add, do not stop charging at temp-55
		if (cm->desc->bat_ovp_flag == 1) {
			printb("##batovp ! judge hoha!\n");
			is_normal = cm_manager_adjust_current(cm,
							cur_jeita_status);
			last_jeita_status = cur_jeita_status;
		}
...
#define ATO_HIGH_CAP 79
#define ATO_LOW_CAP 30
static int cm_charge_check_ato(struct charger_manager *cm)
{
	int fuel_cap = -1, ret;
	fuel_cap = DIV_ROUND_CLOSEST(cm->desc->cap, 10);
	printc("fuel_cap = %d, is_ato_discharge = %d\n",fuel_cap,(cm->wt_charging_status & WT_CHARGE_ATO_DISCHG));
	if(fuel_cap >= ATO_HIGH_CAP && !(cm->wt_charging_status & WT_CHARGE_ATO_DISCHG)){
		cm->wt_charger_enabled = false;
		try_charger_enable(cm, false);
		power_supply_changed(cm->charger_psy);
		cm->wt_charging_status |= WT_CHARGE_ATO_DISCHG;
		printb("ATO version stop charger cap = %d\n", fuel_cap);
	}

	if(fuel_cap <= ATO_LOW_CAP && (cm->wt_charging_status & WT_CHARGE_ATO_DISCHG)){
		if(!(cm->wt_charging_status & ~WT_CHARGE_ATO_DISCHG)){
			printb("ATO version enable charging cap = %d, restore charge!\n", fuel_cap);
			cm->wt_charger_enabled = true;
			try_charger_enable(cm, true);
			power_supply_changed(cm->charger_psy);
		}else{
...
//+ Bug 699035,guoyanjun.wt,add,2021/11/26,add demo note
#define USA_HIGH_CAP 35
#define USA_LOW_CAP 30
#define EUR_HIGH_CAP 70
#define EUR_LOW_CAP 60
static int cm_charge_check_demo(struct charger_manager *cm)
{
	int fuel_cap = -1, ret,demo_high_cap,demo_low_cap;
	fuel_cap = DIV_ROUND_CLOSEST(cm->desc->cap, 10);
	if(cm->desc->damo_flag == 1)
	{
		demo_high_cap = USA_HIGH_CAP;
		demo_low_cap = USA_LOW_CAP;
	}else{
		demo_high_cap = EUR_HIGH_CAP;
		demo_low_cap = EUR_LOW_CAP;
	}
	printc("fuel_cap = %d, is_demo_discharge = %d\n",fuel_cap,(cm->wt_charging_status & WT_CHARGE_DEMO_DISCHG));
	if(fuel_cap >= demo_high_cap && !(cm->wt_charging_status & WT_CHARGE_DEMO_DISCHG)){
		cm->wt_charger_enabled = false;
		try_charger_enable(cm, false);
		power_supply_changed(cm->charger_psy);
		cm->wt_charging_status |= WT_CHARGE_DEMO_DISCHG;
		printb("demo mode stop charger cap = %d\n", fuel_cap);
	}

	if(fuel_cap <= demo_low_cap && (cm->wt_charging_status & WT_CHARGE_DEMO_DISCHG)){
		if(!(cm->wt_charging_status & ~WT_CHARGE_DEMO_DISCHG)){
			printb("demo mode enable charging cap = %d, restore charge!\n", fuel_cap);
			cm->wt_charger_enabled = true;
			try_charger_enable(cm, true);
			power_supply_changed(cm->charger_psy);
		}else{
...
//+  add afc type handler by guoyanjun
extern bool is_afc_charger;
static void afc_charge_handler(struct charger_manager *cm)
{
	struct charger_desc *desc = cm->desc;
	union power_supply_propval val;
	printc("in\n");
	...
	if (cm->desc->is_fast_charge && !cm->desc->enable_fast_charge)
		cm_update_charge_info(cm, (CM_CHARGE_INFO_CHARGE_LIMIT |
					   CM_CHARGE_INFO_INPUT_LIMIT));
	//+ yuecong.wt.20220401	afc boost time too long
	if (cm->charger_enabled)
		_cm_monitor(cm);
	//- yuecong.wt.20220401 afc boost time too long
...
static void fast_charge_handler(struct charger_manager *cm)
{
	...
	//+ExtB[P220318-04043] yuecong.wt.20220413 modify pd charger trigger charging VI animation
	if((cm->desc->fast_charger_type == POWER_SUPPLY_USB_CHARGER_TYPE_PD ||
		cm->desc->fast_charger_type == POWER_SUPPLY_USB_CHARGER_TYPE_PD_PPS) && !cm->charger_enabled){
		cm->is_pd_charger = true;
 ...
 */
extern int cc_polarity; // set cc_polarity is 0 when no charger . add by guoyanjun
//+bug752760,fangzhihua.wt,open,20220617,for USB detect
...
static void misc_event_handler(struct charger_manager *cm, enum cm_event_types type)
{
	...
	if (is_ext_pwr_online(cm)) {
		...
		//cm->desc->bat_ovp_flag = 0;
		cc_polarity = 0; // set cc_polarity is 0 when no charger . add by guoyanjun
		if(!(cm->wt_charging_status & WT_CHARGE_FULL_CAP_DISCHG)){ //bug761719,wujishuai.wt,20220609,add full capacity node
			if(cm->desc->batt_slate_mode != 1){
				cm->wt_charger_enabled = true; // set wt_charger_enabled is true when no charger. add by guoyanjun
				cm->wt_charging_status = 0; // Bug 699489,guoyanjun.wt,add,2021/12/06,bringup,set wt_charging_status 0 when no charger
			}
		}
		is_afc_charger = false;//set is_afc_charger false when no charger . add by guoyanjun
		cm->desc->time_to_full_report = 0; //ExtB[P220425-01299] tankaikun.wt, add, 20220426. report time to full after fcc step up complete

		//+ExtB[P220318-04043] yuecong.wt.20220413 modify pd charger trigger charging VI animation
		cm->is_pd_charger = false;
		dev_warn(cm->dev, "clear cm->is_pd_charger ======= false\n");
		//-ExtB[P220318-04043] yuecong.wt.20220413 modify pd charger trigger charging VI animation
	}
			    ...
			    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		break;
	//+ add usb sys node by guoyanjun
	case POWER_SUPPLY_PROP_TYPEC_CC_ORIENTATION:
		val->intval = cc_polarity;
		break;
	case POWER_SUPPLY_PROP_CHARGE_REAL_TYPE:
		val->intval = wt_cm->desc->charger_type;
		break;
	case POWER_SUPPLY_PROP_AFC_FLAG:
		val->intval = is_afc_charger;
		break;
	//- add usb sys node by guoyanjun
	default:
		ret = -EINVAL;
...
//+bug761719,wujishuai.wt,20220609,add full capacity node
u32 batt_full_capacity = 100;
#define BATT_MISC_EVENT_FULL_CAPACITY  0x01000000

static void cm_batt_full_capacity_check(struct charger_manager *cm)
{
	int fuel_cap = -1;
	if(batt_full_capacity < 100 && (is_ext_pwr_online(cm))){
		fuel_cap = DIV_ROUND_CLOSEST(cm->desc->cap, 10);
		printc("fuel_cap = %d, batt_full_capacity = %d:%d\n",fuel_cap,batt_full_capacity,(cm->wt_charging_status & WT_CHARGE_FULL_CAP_DISCHG));
		if(fuel_cap >= batt_full_capacity && !(cm->wt_charging_status & WT_CHARGE_FULL_CAP_DISCHG)){
			cm->wt_charging_status |= WT_CHARGE_FULL_CAP_DISCHG;
			cm->wt_charger_enabled = false;
			try_charger_enable(cm, false);
			cm->desc->charger_status = cm->battery_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
			power_supply_changed(cm->charger_psy);
			printb("cap = %d, WT_CHARGE_FULL_CAP_DISCHG!\n", fuel_cap);
		}
		if(fuel_cap <= (batt_full_capacity - 2) && (cm->wt_charging_status & WT_CHARGE_FULL_CAP_DISCHG)){
			cm->wt_charging_status &= ~WT_CHARGE_FULL_CAP_DISCHG;
			if(!(cm->wt_charging_status & ~WT_CHARGE_FULL_CAP_DISCHG)){
				printb("WT_CHARGE_FULL_CAP_DISCHG: cap = %d, restore charge!\n", fuel_cap);
				cm->wt_charger_enabled = true;
				try_charger_enable(cm, true);
				cm->desc->charger_status = cm->battery_status = POWER_SUPPLY_STATUS_CHARGING;
				power_supply_changed(cm->charger_psy);
			}else{
				printb("Other ctrl discharge(0x%x)!\n",cm->wt_charging_status);
			}
	...
	else if(batt_full_capacity == 100){
		if(cm->wt_charging_status & WT_CHARGE_FULL_CAP_DISCHG){
			cm->wt_charging_status &= ~WT_CHARGE_FULL_CAP_DISCHG;
			if(is_ext_pwr_online(cm)){
				if(!(cm->wt_charging_status & ~WT_CHARGE_FULL_CAP_DISCHG)){
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY_LEVEL:
		...
		break;
	case POWER_SUPPLY_PROP_ONLINE:
	//+ Bug 699041,guoyanjun.wt,MODIFY,2021/12/08,modify ONLINE as required
	#if 0
		if (is_ext_pwr_online(cm))
			val->intval = 1;
		else
	...
	#else
		if (is_ext_pwr_online(cm)){
			switch(wt_cm->desc->charger_type){
				case POWER_SUPPLY_USB_CHARGER_TYPE_SDP:
				case POWER_SUPPLY_USB_CHARGER_TYPE_CDP:
					val->intval = 4;
					break;
				...
				case POWER_SUPPLY_USB_CHARGER_TYPE_DCP:
					val->intval = 3;
					break;
				//+ yuecong.wt.2022.04.01 modify charger type return info
				default:
	...
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
		//+ExtB[P220425-01299] tankaikun.wt, add, 20220426. report time to full after fcc step up complete
		if(!cm->desc->time_to_full_report){
			val->intval = 0;
			break;
		}
		//-ExtB[P220425-01299] tankaikun.wt, add, 20220426. report time to full after fcc step up complete
		...
		break;
	//+ add charger sys node by guoyanjun
	case POWER_SUPPLY_PROP_BAT_INTERNAL_RESISTANCE:
		val->intval = cm->desc->internal_resist;
		break;
	case POWER_SUPPLY_PROP_HV_CHARGER_STATUS:
		val->intval = cm->desc->enable_fast_charge;
		break;
	case POWER_SUPPLY_PROP_DEMO_FLAG:
		val->intval = cm->desc->damo_flag;
		break;
	case POWER_SUPPLY_PROP_FORCE_CAP:
		val->intval = cm->desc->force_cap;
		break;
	//- add charger sys node by guoyanjun
	//+ Bug 699037,guoyanjun.wt,ADD,2021/12/06,Add charging information node
	case POWER_SUPPLY_PROP_NEW_CHARGE_TYPE:
		ret = get_ibat_now_uA(cm, &chg_cur);
		if(chg_cur > 1000000)
			val->intval = 2;
		else if(chg_cur > 0)
			val->intval = 1;
		...
		break;
	//- Bug 699037,guoyanjun.wt,ADD,2021/12/06,Add charging information node
	//+ Bug 699039 ,guoyanjun.wt,ADD,2021/12/06,Add sys need node
	case POWER_SUPPLY_PROP_BATT_SLATE_MODE:
		val->intval = cm->desc->batt_slate_mode;
		break;
	case POWER_SUPPLY_PROP_BATT_CURRENT_EVENT:
		if (!is_ext_pwr_online(cm)){
			val->intval = 0;
		}else{
			//+bug761704,wujishuai.wt,20220622,modify batt_curent_slate node
			if(cm->desc->batt_slate_mode == 1){
				val->intval = 2048;
			}else if(cm->desc->temperature < -100){
				val->intval = 16;
			}else if(cm->desc->temperature > 450){
				val->intval = 32;
			}
			else{
			//-bug761704,wujishuai.wt,20220622,modify batt_curent_slate node
				if(cm->charging_status || cm->wt_charging_status)
					val->intval = 2;
				else
					val->intval = 1;
			}
	...
	case POWER_SUPPLY_PROP_OTG_TYPE:
		for (i = 0; cm->desc->psy_charger_stat[i]; i++) {
			...
			power_supply_put(psy);
			if (ret) {
				dev_err(cm->dev, "get otg failed\n");
				continue;
			}
		}
		break;
	//- Bug  699041 ,guoyanjun.wt,ADD,2021/12/08,Add sys need node
	//+ Bug  699043 ,guoyanjun.wt,ADD,2021/12/08,Add sys need node
	case POWER_SUPPLY_PROP_BATT_CURRENT_UA_NOW:
		ret = get_ibat_now_uA(cm, &val->intval);
		break;
	//- Bug  699043 ,guoyanjun.wt,ADD,2021/12/08,Add sys need node
	//+ Bug  699042 ,guoyanjun.wt,ADD,2021/12/08,Add sys need node
	case POWER_SUPPLY_PROP_BATT_MISC_EVENT:
		if (is_ext_pwr_online(cm) && (wt_cm->desc->charger_type == POWER_SUPPLY_CHARGER_TYPE_UNKNOWN))
			val->intval = 4;
		else
			val->intval = 0;
		//+bug761719,wujishuai.wt,20220609,add full capacity node
	...
	case POWER_SUPPLY_PROP_SHIPMODE_WHEN_SHUTDOWN:
		val->intval = cm->shipmode_when_shutdown;
		break;
	//-bug781105,wujishuai.wt,20220712,add shipmode node
	default:
		return -EINVAL;
		     ...
		     const union power_supply_propval *val)
{
	...
	if (!is_ext_pwr_online(cm))
		if(POWER_SUPPLY_PROP_BATT_SLATE_MODE != psp && POWER_SUPPLY_PROP_HV_DISABLE != psp && POWER_SUPPLY_PROP_BATT_FULL_CAPACITY != psp){
			dev_err(cm->dev, "no chager, can not set property\n");
			return -ENODEV;
		}
	...
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		...
		break;
	//+ Bug 704565,guoyanjun.wt,ADD,2021/12/02,add force cap for debug
	case POWER_SUPPLY_PROP_DEMO_FLAG:
		cm->desc->damo_flag = val->intval;
		break;
	case POWER_SUPPLY_PROP_FORCE_CAP:
		cm->desc->force_cap = val->intval;
		if(cm->desc->force_cap >=0 && cm->desc->force_cap <=1000){
			printc("## force_cap = %d !\n",cm->desc->force_cap);
			if(cm->desc->cap != cm->desc->force_cap){
				cm->desc->cap = cm->desc->force_cap;
				power_supply_changed(cm->charger_psy);
			}
		}else{
			int fuel_cap;
			ret = get_batt_cap(cm, &fuel_cap);
			cm->desc->cap = cm_capacity_remap(cm, fuel_cap);
			printc("## recover fuel_cap = %d !\n",cm->desc->cap);
			power_supply_changed(cm->charger_psy);
		}
		break;
	//- Bug 704565,guoyanjun.wt,ADD,2021/12/02,add force cap for debug
	//+ Bug 699039 ,guoyanjun.wt,ADD,2021/12/06,Add sys need node
	case POWER_SUPPLY_PROP_BATT_SLATE_MODE:
		cm->desc->batt_slate_mode = val->intval;
		if((cm->desc->batt_slate_mode == 1 || cm->desc->batt_slate_mode == 2) && !(cm->wt_charging_status & WT_CHARGE_SLATE_DISCHG)){
			cm->wt_charger_enabled = false;
			try_charger_enable(cm, false);
			power_supply_changed(cm->charger_psy);
			cm->wt_charging_status |= WT_CHARGE_SLATE_DISCHG;
			printb("Slate mode stop charger!\n");
		}else if(cm->desc->batt_slate_mode == 0 && (cm->wt_charging_status & WT_CHARGE_SLATE_DISCHG)){
			if(!(cm->wt_charging_status & ~WT_CHARGE_SLATE_DISCHG)){
				printb("Exit slate mode, restore charge!\n");
				cm->wt_charger_enabled = true;
				try_charger_enable(cm, true);
				power_supply_changed(cm->charger_psy);
			}else{
	...
	case POWER_SUPPLY_PROP_HV_DISABLE:
		...
		//+ExtB[P220318-04675] yuecong.wt.20220406 add afc_disable node to control for hv_disable node
		if(wt_cm != NULL){
			if(hv_disable){
				cm_fast_charge_disable(wt_cm);
			}else{
				ret = cm_fast_charge_enable_check(wt_cm);
				if (ret) {
					dev_err(wt_cm->dev,
						"failed to check fast charge enable\n");
				}
	...
	case POWER_SUPPLY_PROP_BATT_FULL_CAPACITY:
		...
		cm_batt_full_capacity_check(cm);
		break;
	//-bug761719,wujishuai.wt,20220609,add full capacity node
	//+bug781105,wujishuai.wt,20220712,add shipmode node
	case POWER_SUPPLY_PROP_SHIPMODE_WHEN_SHUTDOWN:
		if(val->intval == 1)
			cm->shipmode_when_shutdown = true;
		if(val->intval == 0)
			cm->shipmode_when_shutdown = false;
		dev_err(wt_cm->dev,"shipmode_when_shutdown = %d \n",cm->shipmode_when_shutdown);
		break;
	//bug781105,wujishuai.wt,20220712,add shipmode node
	default:
		ret = -EINVAL;
...
static enum power_supply_property usb_props[] = {
	...
	POWER_SUPPLY_PROP_CHARGE_REAL_TYPE,
...
static enum power_supply_property default_charger_props[] = {
	...
	//+ add charger sys node by guoyanjun
	POWER_SUPPLY_PROP_BAT_INTERNAL_RESISTANCE,
	POWER_SUPPLY_PROP_HV_CHARGER_STATUS,
	POWER_SUPPLY_PROP_DEMO_FLAG,
	POWER_SUPPLY_PROP_FORCE_CAP,
	...
	//- yuecong.wt.20220401 add hv_disable sys node
	POWER_SUPPLY_PROP_BATT_FULL_CAPACITY,//bug761719,wujishuai.wt,20220609,add full capacity node
	POWER_SUPPLY_PROP_SHIPMODE_WHEN_SHUTDOWN, //bug781105,wujishuai.wt,20220712,add shipmode node
	/*
...
static int cm_get_bat_info(struct charger_manager *cm)
{
	...
	//ExtB[P220411-01778] tankaikun.wt, add, 20220413. Add charger current step control.
	cm->desc->main_fcc_ua = DEFAULT_FCC_MAIN_UA - DEFAULT_FCC_STEP_SIZE_UA;
...
static void cm_uvlo_check_work(struct work_struct *work)
{
	...
	if(cm->desc->cap >= 5 && batt_uA < 0){
		cm->desc->cap--;
		if (DIV_ROUND_CLOSEST(old_cap, 10) != DIV_ROUND_CLOSEST(cm->desc->cap, 10)) {
			dev_err(cm->dev, "old_cap = %d, cm->desc->cap = %d\n",old_cap,cm->desc->cap);
			power_supply_changed(cm->charger_psy);
		}
	}

//+bug 741389, liyiying.wt, mod, 2022/4/27, the uisoc is nonlinearity when discharging
	if (cm->desc->temperature >= CM_UVLO_CALIBRATION_TEMP_THRESHOLD_LOW) {
		cm->desc->shutdown_voltage = 3400000;
	} else {
		cm->desc->shutdown_voltage = 3200000;
	}
//-bug 741389, liyiying.wt, mod, 2022/4/27, the uisoc is nonlinearity when discharging
	...
	if (cm->desc->uvlo_trigger_cnt >= CM_UVLO_CALIBRATION_CNT_THRESHOLD && cm->desc->cap < 5 && batt_uA < 0) {
...
//+bug 741389, liyiying.wt, mod, 2022/4/27, the uisoc is nonlinearity when discharging
	dev_err(cm->dev, " the temp is %d \n ", cm->desc->temperature);
	if (cm->desc->temperature >= CM_UVLO_CALIBRATION_TEMP_THRESHOLD_LOW) {
		dev_err(cm->dev, "the temp hight than 0 when uvlo\n");
		if (batt_uV < CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD)
			schedule_delayed_work(&cm->uvlo_work, msecs_to_jiffies(800));
	} else {
		dev_err(cm->dev, "the temp low than 0 when uvlo\n ");
		if (batt_uV < CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD_LOW)
			schedule_delayed_work(&cm->uvlo_work, msecs_to_jiffies(800));
	}
//-bug 741389, liyiying.wt, mod, 2022/4/27, the uisoc is nonlinearity when discharging
}
static void cm_tmp_check(struct charger_manager *cm,int cur_temp)
{
	static int tmp_trigger_cnt = 0;

	if (cur_temp >= CM_POWEROFF_TMP)
		tmp_trigger_cnt++;
	else
		tmp_trigger_cnt = 0;

	if (tmp_trigger_cnt >= CM_POWEROFF_CNT_THRESHOLD) {
		dev_err(cm->dev, "WARN: over hot!! tmp over %d, will shutdown!!\n",cur_temp);
		kernel_power_off();
	}
}
...
static void cm_batt_works(struct work_struct *work)
{
	...
	if (cur_temp <= CM_LOW_TEMP_REGION &&
		...
		if (cm->desc->low_temp_trigger_cnt++ > 3){
			fuel_cap = 0;
			dev_warn(cm->dev, "batt_uV lower than shutdown voltage in low temp, change msoc to 0\n");
		}
	...
	//+Bug 699489,guoyanjun.wt,add,2021/11/02,bringup,ato version control capacity range of 30%~80%
#ifdef WT_COMPILE_FACTORY_VERSION	
		cm_charge_check_ato(cm);
#endif
	//-Bug 699489,guoyanjun.wt,add,2021/11/02,bringup,ato version control capacity range of 30%~80%
//+ Bug 699035,guoyanjun.wt,add,2021/11/26,add demo note
	if(cm->desc->damo_flag)
		cm_charge_check_demo(cm);
	else{
		if(cm->wt_charging_status & WT_CHARGE_DEMO_DISCHG){
			if(!(cm->wt_charging_status & ~WT_CHARGE_DEMO_DISCHG)){
				printb("exit demo, restore charge!\n");
				cm->wt_charger_enabled = true;
				try_charger_enable(cm, true);
				power_supply_changed(cm->charger_psy);
			}else{
...
//- Bug 699035,guoyanjun.wt,add,2021/11/26,add demo note
	...
	cm_batt_full_capacity_check(cm);//bug761719,wujishuai.wt,20220609,add full capacity node

	printc("vbat: %d, vbat_avg: %d, OCV: %d, ibat: %d, ibat_avg: %d, ibus: %d "
		 " vbus: %d, msoc: %d, chg_sts: %d, frce_full: %d, chg_lmt_cur: %d "
		 " inpt_lmt_cur: %d, chgr_type: %d, Tboard: %d, Tbatt:%d, vbat_ovp_flag:%d\n",
		 ...
		 cm->desc->charger_type, board_temp, cur_temp, cm->desc->bat_ovp_flag);
	printc("thm_cur: %d thm_pwr: %d, is_fchg: %d, fchg_en: %d, tflush: %d, tperiod: %d "
		 "wt_charger_enabled = %d, wt_charging_status = 0x%x, batt_pro_mode: %d\n",
		 cm->desc->thm_info.thm_adjust_cur, cm->desc->thm_info.thm_pwr,cm->desc->is_fast_charge, cm->desc->enable_fast_charge, 
		 flush_time, period_time, cm->wt_charger_enabled, cm->wt_charging_status);
	dump_charger_reg(cm);// add charger ic dump reg by guoyanjun
...
//bug 741389, liyiying.wt, mod, 2022/4/27, the uisoc is nonlinearity when discharging
	if ((batt_uV < CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD) && (batt_uA < 0) && (cur_temp >= CM_UVLO_CALIBRATION_TEMP_THRESHOLD_LOW)) {
		dev_info(cm->dev, "batt_uV is less than UVLO calib volt\n");
		if ((cm->desc->cap > 25)){
//+bug 696970, liyiying.wt, 2022/3/24, mod, prevent the soc too hight to shutdown when arrive shutdown voltage
			fuel_cap = cm->desc->cap - 5*(1-batt_uA/1000000);
			printc("%s,update ui_soc is %d\n",__func__,fuel_cap);
//-bug 696970, liyiying.wt, 2022/3/24, mod, prevent the soc too hight to shutdown when arrive shutdown voltage
			if(fuel_cap < 0)
				fuel_cap = 0;
			printc("batt_uV is less than UVLO,cap:%d->%d\n",cm->desc->cap,fuel_cap);
		}else
			schedule_delayed_work(&cm->uvlo_work, msecs_to_jiffies(100));
	}

//+bug 741389, liyiying.wt, mod, 2022/4/27, the uisoc is nonlinearity when discharging
	if ((batt_uV < CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD_LOW) && (batt_uA < 0) && (cur_temp < CM_UVLO_CALIBRATION_TEMP_THRESHOLD_LOW)) {
		schedule_delayed_work(&cm->uvlo_work, msecs_to_jiffies(100));
	}
//-bug 741389, liyiying.wt, mod, 2022/4/27, the uisoc is nonlinearity when discharging

	cm_tmp_check(cm,cur_temp);

	printc("new_uisoc = %d, old_uisoc = %d\n", fuel_cap, cm->desc->cap);
	//+Bug 704565,guoyanjun.wt,ADD,2021/12/02,add force cap for debug
	if(cm->desc->force_cap >=0 && cm->desc->force_cap <=1000){
		printc("## already foce cap = %d\n",cm->desc->force_cap);
		cm->desc->cap = cm->desc->force_cap;
		power_supply_changed(cm->charger_psy);
		goto schedule_cap_update_work;
	}
...
//+ add sys/wingtech/charge_ctrl/StartCharging_Test and sys/wingtech/charge_ctrl/StopCharging_Test
static ssize_t start_charge(struct kobject *kobj, struct kobj_attribute *attr,char *buf)
{
	if(wt_cm){
		if(!(wt_cm->wt_charging_status & ~WT_CHARGE_SYS_DISCHG)){
			printb("sys set start charge, restore charge!\n");
//+ Bug 702456,guoyanjun.wt,ADD,2021/12/13,enable hiz mode at ATO version
#ifdef WT_COMPILE_FACTORY_VERSION
			if(wt_cm->desc->force_hiz){
				wt_cm->desc->force_hiz = false;
				cm_force_set_hiz_mode(wt_cm,false);
			}
#endif
//- Bug 702456,guoyanjun.wt,ADD,2021/12/13,enable hiz mode at ATO version

			wt_cm->wt_charger_enabled = true;
			try_charger_enable(wt_cm, true);
			power_supply_changed(wt_cm->charger_psy);
		}else{
			printb("Other ctrl discharge(0x%x)!\n",wt_cm->wt_charging_status);
		}
		wt_cm->wt_charging_status &= ~WT_CHARGE_SYS_DISCHG;
	}
	return sprintf(buf, "start charge\n");
}
static ssize_t stop_charge(struct kobject *kobj, struct kobj_attribute *attr,char *buf)
{
	if(wt_cm){
		printb("sys stop charge!\n");
		wt_cm->wt_charger_enabled = false;
		try_charger_enable(wt_cm, false);
		power_supply_changed(wt_cm->charger_psy);
		wt_cm->wt_charging_status |= WT_CHARGE_SYS_DISCHG;
	}
...
static struct kobj_attribute start_attr = {
    .attr = {
         .name = "StartCharging_Test",
         .mode = S_IRUGO,//S_IRUGO |S_IWUGO
         },
    .show = &start_charge,
    .store = NULL,
};
static struct kobj_attribute stop_attr = {
    .attr = {
         .name = "StopCharging_Test",
         .mode = S_IRUGO,//S_IRUGO |S_IWUGO
         },
    .show = &stop_charge,
    .store = NULL,
...
static struct attribute_group charge_ctrl_group = {
    .attrs = sys_attrs,
};

static void wt_create_chg_ctrl_sysfs(void)
{
	struct kobject *parent_kobj = kobject_create_and_add("wingtech",NULL);
	struct kobject *charge_ctrl_kobj = kobject_create_and_add("charge_ctrl",parent_kobj); 
    if (charge_ctrl_kobj)
		sysfs_create_group(charge_ctrl_kobj, &charge_ctrl_group );
				   ...
				   const char *buf, size_t size)
{
	...
	if (!strncasecmp(buf, "1", 1)) {
		...
		ret = sec_set_param(CM_OFFSET + 1, hv_disable ? '1' : '0');
		if (ret < 0)
...
//+ Bug 702456,guoyanjun.wt,ADD,2021/12/13,enable hiz mode at ATO version
#ifdef WT_COMPILE_FACTORY_VERSION	
	cm->desc->force_hiz = true;
	INIT_DELAYED_WORK(&cm->wt_update_work, wt_batt_works);	
#endif
//- Bug 702456,guoyanjun.wt,ADD,2021/12/13,enable hiz mode at ATO version
	...
	INIT_DELAYED_WORK(&cm->fcc_stepper_work, fcc_stepper_work);
	...
	cm->wt_charger_enabled = true;	//Bug 699489,guoyanjun.wt,add,2021/11/02,bringup,ato version control capacity range of 30%~80%
	cm->desc->force_cap = -1; // init force_cap is -1
...
#ifdef WT_COMPILE_FACTORY_VERSION	
	queue_delayed_work(system_power_efficient_wq, &cm->wt_update_work, 10 * HZ);
#endif
	wt_create_chg_ctrl_sysfs();//add sys/wingtech/charge_ctrl/StartCharging_Test and sys/wingtech/charge_ctrl/StopCharging_Test
	//+ExtB[P220318-04675] yuecong.wt.20220406 add afc_disable sys node
	cm->switch_dev = sec_device_create(cm, "switch");
	if (IS_ERR(cm->switch_dev)) {
		dev_err(&pdev->dev, "Failed to create switch_dev\n");
	}else{
//+bug-747179, liyiying.wt, mod, 2022/4/12, fix the no check retuen value
		ret = device_create_file(cm->switch_dev,&dev_attr_afc_disable);
		if (ret) {
			dev_err(&pdev->dev, "Failed to device create file, ret = %d\n", ret);
		}
//-bug-747179, liyiying.wt, mod, 2022/4/12, fix the no check retuen value
	}
	//-ExtB[P220318-04675] yuecong.wt.20220406 add afc_disable sys node
...
static int charger_manager_remove(struct platform_device *pdev)
{
	...
	cancel_delayed_work_sync(&cm->fcc_stepper_work); //ExtB[P220411-01778] tankaikun.wt, add, 20220413. Add charger current step control.
...
{
	struct charger_manager *cm = platform_get_drvdata(pdev);
	struct power_supply *psy; //bug781105,wujishuai.wt,20220712,add shipmode node
	int ret; //bug781105,wujishuai.wt,20220712,add shipmode node
	union power_supply_propval val;//bug781105,wujishuai.wt,20220712,add shipmode node

	dev_err(&pdev->dev, "charger_manager_shutdown\n");

	if (cm->desc->uvlo_trigger_cnt < CM_UVLO_CALIBRATION_CNT_THRESHOLD)
		set_batt_cap(cm, cm->desc->cap);

	//+bug781105,wujishuai.wt,20220712,add shipmode node
	if(cm->shipmode_when_shutdown){
		printk( "shutdown shipmode\n");

		val.intval = true;

		psy = power_supply_get_by_name(cm->desc->psy_charger_stat[0]);
		if (!psy) 
			printk( "Cannot find power supply \n");

		ret = power_supply_set_property(psy, POWER_SUPPLY_PROP_SHIPMODE, &val);
		if (ret)
			printk("fail to set shipmode");
...
static void cm_notify_type_handle(struct charger_manager *cm, enum cm_event_types type, char *msg)
{
	...
	case CM_EVENT_FAST_CHARGE:
		...
		break;
//+  add afc type handler by guoyanjun
	case CM_EVENT_AFC_CHARGE:
		afc_charge_handler(cm);
		break;
//-  add afc type handler by guoyanjun
	case CM_EVENT_INT:
