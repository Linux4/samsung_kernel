...
/*TabA7 Lite code for USBIF Current-Measurement by qiaodan at 2020/05/18 start*/
        ...
        if (usb_state == 0) {
            pr_info("%s:%d usb_state USB_SUSPEND, set input current 2.5mA\n", __func__, __LINE__);
            uA1 = 2400;
        } else if (usb_state == 1) {
            pr_info("%s:%d usb_state USB_UNCONFIGURED, set input current 100mA\n", __func__, __LINE__);
            uA1 = 99000;
        } else if (usb_state == 2) {
            pr_info("%s:%d usb_state USB_CONFIGURED, set input current %dmA\n", __func__, __LINE__,
