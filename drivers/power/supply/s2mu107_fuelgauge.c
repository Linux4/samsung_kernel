...
#include <linux/power/s2mu107_fuelgauge.h>
#include <linux/of_gpio.h>
...
static void s2mu107_reset_fg(struct s2mu107_fuelgauge_data *fuelgauge)
{
	...
	for (i = 0x92; i <= 0xe9; i++)
		s2mu107_write_and_verify_reg_byte_no_en(fuelgauge->i2c, i,
			fuelgauge->info.battery_table3[i - 0x92]);
	for (i = 0xea; i <= 0xff; i++)
		s2mu107_write_and_verify_reg_byte_no_en(fuelgauge->i2c, i,
			fuelgauge->info.battery_table4[i - 0xea]);
...
static int s2mu107_get_soc(struct s2mu107_fuelgauge_data *fuelgauge)
{
	...
	psy = power_supply_get_by_name("battery");
	if (!psy) {
		pr_err("%s: battery driver is not probed. "
				"Skip get_property for battery driver.\n", __func__);
	} else {
		ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_CAPACITY, &value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_AVG:
		if (val->intval == S2M_BATTERY_VOLTAGE_OCV)
	...
	case POWER_SUPPLY_PROP_CURRENT_AVG:
		val->intval = s2mu107_get_avgcurrent(fuelgauge);
		break;
...
static int s2mu107_fuelgauge_parse_dt(struct s2mu107_fuelgauge_data *fuelgauge)
{
	...
	if (np == NULL) {
		...
		if (!np) {
			...
			if (ret < 0) {
				...
				pr_err("%s There is no accumulative rate value in DT."
						" set to the default value(0x800)\n", __func__);
			}
	...
	/* TODO: Print parse data for debug */
	pr_info("%s: batcap: %02x %02x %02x %02x, table3: %02x, table4: %02x\n",
			__func__, fuelgauge->info.batcap[0], fuelgauge->info.batcap[1],
			fuelgauge->info.batcap[2], fuelgauge->info.batcap[3],
				...
				const struct i2c_device_id *id)
{
	...
	raw_soc_val = s2mu107_get_soc(fuelgauge);
	...
	fuelgauge->cable_type = POWER_SUPPLY_TYPE_BATTERY;
	fuelgauge->probe_done = true;
