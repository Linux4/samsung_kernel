...
static void exynos_reboot_parse(void *cmd)
{
	if (cmd) {
		...
		if (!strcmp((char *)cmd, "mte_enable"))
			regmap_update_bits(pmureg, reboot_cmd_offset, REBOOT_MTE_FLAG, REBOOT_MTE_FLAG);
		else if (!strcmp((char *)cmd, "mte_disable"))
			regmap_update_bits(pmureg, reboot_cmd_offset, REBOOT_MTE_FLAG, 0);
