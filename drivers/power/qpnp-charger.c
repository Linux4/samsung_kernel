...
qpnp_chg_ibatmax_set(struct qpnp_chg_chip *chip, int chg_current)
{
	...
	#ifdef CONFIG_BATTERY_SAMSUNG
	if (chg_current > QPNP_CHG_IBATMAX_MAX) {
		pr_err("bad mA=%d asked to set\n", chg_current);
		return -EINVAL;
	}
	#else
				...
				struct spmi_resource *spmi_resource)
{
	...
	case SMBCL_USB_CHGPTH_SUBTYPE:
		...
		rc = qpnp_chg_masked_write(chip,
			chip->usb_chgpth_base + USB_OVP_CTL,
			USB_VALID_DEB_20MS | USB_OVP_THR,
			USB_VALID_DEB_20MS | USB_OVP_THR, 1);
...
static void
sec_qpnp_test_read(struct qpnp_chg_chip *chip)
{
	u8 val_reg;
	char str[128] = {0,};

	if(qpnp_chg_read(chip, &val_reg, 0x1309, 1))
		goto print;
	sprintf(str+strlen(str), "0x1309=0x%02x, ", val_reg);

	if (qpnp_chg_read(chip, &val_reg, 0x1344, 1))
		goto print;
	sprintf(str+strlen(str), "0x1344=0x%02x, ", val_reg);

	if (qpnp_chg_read(chip, &val_reg, 0x1347, 1))
		goto print;
	sprintf(str+strlen(str), "0x1347=0x%02x, ", val_reg);

	if (qpnp_chg_read(chip, &val_reg, 0x1208, 1))
		goto print;
	sprintf(str+strlen(str), "0x1208=0x%02x, ", val_reg);

	if (qpnp_chg_read(chip, &val_reg, 0x1209, 1))
		goto print;
	sprintf(str+strlen(str), "0x1209=0x%02x, ", val_reg);

	if (qpnp_chg_read(chip, &val_reg, 0x100B, 1))
		goto print;
	sprintf(str+strlen(str), "0x100B=0x%02x, ", val_reg);

	if (qpnp_chg_read(chip, &val_reg, 0x1049, 1))
		goto print;
	sprintf(str+strlen(str), "0x1049=0x%02x, ", val_reg);

	if (qpnp_chg_read(chip, &val_reg, 0x1642, 1))
		goto print;
	sprintf(str+strlen(str), "0x1642=0x%02x, ", val_reg);

	if (qpnp_chg_read(chip, &val_reg, 0x1293, 1))
		goto print;
	sprintf(str+strlen(str), "0x1293=0x%02x, ", val_reg);
