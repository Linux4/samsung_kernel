...
static int s2mu005_get_rawsoc(struct s2mu005_fuelgauge_data *fuelgauge)
{
	...
	if (fuelgauge->revision >= 2) {
		...
		psy_do_property("s2mu005-battery", get, POWER_SUPPLY_PROP_CAPACITY, value);
...
static int s2mu005_get_current_average(struct s2mu005_fuelgauge_data *fuelgauge)
{
	...
	psy_do_property("s2mu005-battery", get,
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		if (val->intval == SEC_FUELGAUGE_CAPACITY_TYPE_RAW) {
			...
			if (fuelgauge->revision < 2) {
				psy_do_property("s2mu005-battery", get, POWER_SUPPLY_PROP_CAPACITY, ui_soc_val);
				psy_do_property("s2mu005-battery", get, POWER_SUPPLY_PROP_ONLINE, cable_type_val);
