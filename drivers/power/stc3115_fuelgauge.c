...
static struct wake_lock spa_stc311x_wakeup;
int spa_stc311x_wakeup_enabled;
#endif
...
static int read_voltage(int *vbat)
{
	...
	if (chip->pdata)
	{
		...
		for(Loop=0;Loop<16;Loop++)
	  	...
	  	GasGaugeData.ExternalTemperature = chip->pdata->ExternalTemperature(); /*External temperature fonction, return C*/
#endif
	  	GasGaugeData.ForceExternalTemperature = 0; /* 1=External temperature, 0=STC3115 temperature */
...
static int read_soc(int *soc)
{
	...
	if (*soc < 3)
	{
		if (!spa_stc311x_wakeup_enabled)
		{	
			printk("spa_stc311x_wakelock enable\n");
			spa_stc311x_wakeup_enabled = 1;
			wake_lock(&spa_stc311x_wakeup);
		}	
	}
	else
	{
		if (spa_stc311x_wakeup_enabled)
		{
			printk("spa_stc311x_wakelock disable\n");
			spa_stc311x_wakeup_enabled = 0;
			wake_unlock(&spa_stc311x_wakeup);
		}
	}
...
int read_soc_of_st_chip(int *st_soc) 
{
	return read_soc(st_soc);
}
...
static irqreturn_t stc311x_alert_irq(int irq, void *dev_id)
{
#if defined(CONFIG_SPA)
	printk("stc311x_alert_irq\n");
...
printk("stc311x_probe\n");
  ...
  dev_info(&client->dev, "power supply register,%d\n",ret);
	...
	spa_stc311x_wakeup_enabled = 0;
	wake_lock_init(&spa_stc311x_wakeup, WAKE_LOCK_SUSPEND, "spa_stc311x_wakeups");
	...
	if(client->irq)
	{
		ret = gpio_get_value(pxa_irq_to_gpio(client->irq));
		printk("  %s irq : %d, gpio value : %d\n", __func__, client->irq, ret);
			
            ret = gpio_request(pxa_irq_to_gpio(client->irq), "stc311x_alert");
            if(ret)
                    printk("  %s gpio_request failed\n", __func__);

            gpio_direction_input(pxa_irq_to_gpio(client->irq));
            ret = request_irq(client->irq, stc311x_alert_irq, IRQF_TRIGGER_FALLING, "stc311x_alert", chip->pdata);
            if(ret)
