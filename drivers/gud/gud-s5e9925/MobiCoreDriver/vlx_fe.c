...
static void vlx_fe_start(void *cookie)
{
	...
	/* Update the local pfe header with BE version from the be2fe_data */
	vlx_fe->pfe.hdr.be_version = vlx_fe->pfe.be2fe_data->be_version;

	/* Get the BE version */
	mc_dev_info("%s Connected to BE version 0x%x",
		    __func__, get_be_driver_version(&vlx_fe->pfe.hdr));
...
/* Called when the Frontend is initialized 1st time */
static int vlx_fe_probe(void)
{
	...
	set_fe_driver_version(&vlx_fe->pfe.hdr, MC_PV_VERSION);

	/* Create FE server for the future BE client/callbacks (be2fe comm) */
	...
	/* Create FE client for the BE server (fe2be comm)
	 * Backend VM/server supposed to be already started at this stage
	 */
	...
	/* Store in local vlx_fe the BE vRPC link and BE vRPC shared buffer */
	...
	/* Update the fe2be_data with FE versions for the BE */
	vlx_fe->pfe.fe2be_data->hdr_version = vlx_fe->pfe.hdr.hdr_version;
	vlx_fe->pfe.fe2be_data->fe_version = vlx_fe->pfe.hdr.fe_version;
