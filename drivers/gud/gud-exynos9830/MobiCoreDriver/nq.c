...
int nq_start(void)
{
	...
	l_ctx.irq_bh_thread = kthread_create(irq_bh_worker, NULL, "tee_irq_bh");
	if (IS_ERR(l_ctx.irq_bh_thread)) {
		ret = PTR_ERR(l_ctx.irq_bh_thread);
		mc_dev_err(ret, "irq_bh_worker thread creation failed");
		return ret;
	}
	set_cpus_allowed_ptr(l_ctx.irq_bh_thread, cpu_coregroup_mask(0));
	wake_up_process(l_ctx.irq_bh_thread);
	...
	for (cnt = 0; cnt < NQ_TEE_WORKER_THREADS; cnt++) {
		...
		set_cpus_allowed_ptr(l_ctx.tee_worker[cnt], cpu_coregroup_mask(0));
