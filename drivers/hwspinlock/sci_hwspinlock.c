...
static int sci_hwspinlock_probe(struct platform_device *pdev)
{
	...
	pdev->id = of_alias_get_id(pdev->dev.of_node, "hwspinlock");
	if(pdev->id == 0)
		return 0;
	__hwspinlock_init();
	...
	num_locks = 32;
	hwspinlock_base = (void __iomem *)(pdev->id == 0 ? SPRD_HWSPINLOCK0_BASE : SPRD_HWSPINLOCK1_BASE);
	...
	for (i = 0, hwlock = &bank->lock[0]; i < num_locks; i++, hwlock++) {
		if (hwspinlock_vid == 0x100 || hwspinlock_vid == 0x200)
			hwlock->priv = (void __iomem *)(hwspinlock_base + HWSPINLOCK_TOKEN_V1(i));
		else
			hwlock->priv = (void __iomem *)(hwspinlock_base + HWSPINLOCK_TOKEN_V0(i));
	}
	/*
	...
	printk("sci_hwspinlock_probe ok: hwspinlock name = %s, hwspinlock_vid = 0x%x, add num_locks = %d\n",
	       pdev->name,hwspinlock_vid,num_locks);
