...
static int parse_dt_baaw_pdata(struct device *dev, struct device_node *np,
					struct gnss_pdata *pdata)
{
	struct device_node *bn = NULL;
	struct device_node *child = NULL;
	struct gnss_baaw *window;
	int i = 0;

	bn = of_get_child_by_name(dev->of_node, "baaw_configuration");
	if (!bn) {
		gif_err("DT error: failed to get baaw_configuration\n");
		return -EINVAL;
	}

	gif_dt_read_u32(bn, "window_cnt", pdata->num_wnd);
	if (!pdata->num_wnd) {
		gif_err("DT error: baaw window count: %d\n", pdata->num_wnd);
		return -EINVAL;
	}

	pdata->wnd_set = devm_kzalloc(dev, sizeof(struct gnss_baaw) * pdata->num_wnd,
			GFP_KERNEL);
	if (!pdata->wnd_set)
		return -ENOMEM;

	for_each_child_of_node(bn, child) {
		window = &pdata->wnd_set[i];
		gif_dt_read_u32(child, "baaw,sfr_addr", window->sfr_addr);
		gif_dt_read_u32(child, "baaw,start", window->gnss_start);
		gif_dt_read_u32(child, "baaw,end", window->gnss_end);
		gif_dt_read_u32(child, "baaw,ap_start", window->ap_start);
		gif_dt_read_u32(child, "baaw,authority", window->authority);
		window->baaw_reg = devm_ioremap(dev, window->sfr_addr, SZ_16);
		if (!window->baaw_reg) {
			gif_err("baaw_reg ioremap failed\n");
			return -EIO;
		}
		i++;
	}
...
static struct gnss_pdata *gnss_if_parse_dt_pdata(struct device *dev)
{
	...
	ret = parse_dt_baaw_pdata(dev, dev->of_node, pdata);
	if (ret != 0) {
