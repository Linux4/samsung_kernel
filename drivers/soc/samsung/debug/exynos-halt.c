...
struct exynos_halt_data {
	...
	void __iomem *cs_gpr;
	unsigned int cs_gpr_dbgack_mask;
	struct device *dev;
};
...
void set_stop_cpu(void)
{
	...
	if (exynos_halt_data->cs_gpr) {
		__raw_writel(OSLOCK_MAGIC,
				exynos_halt_data->cs_gpr + DBGLAR);
		isb();
		__raw_writel(exynos_halt_data->cs_gpr_dbgack_mask,
				exynos_halt_data->cs_gpr);
		isb();
	}
...
static int exynos_halt_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(np, "gpr-offset", &offset);
	if (ret) {
		dev_info(&pdev->dev, "no coresight gpr offset\n");
		data->cs_gpr = NULL;
	} else {
		dev_info(&pdev->dev, "coresight dbg offset = [0x%lx]\n", offset);
		data->cs_gpr = ioremap(base + offset, SZ_4K);
		if (!data->cs_gpr) {
			dev_err(&pdev->dev, "cs_gpr ioremap fail\n");
			ret = -ENOMEM;
			goto err_free;
		}

		ret = of_property_read_u32(np, "gpr-dbgack-mask", &data->cs_gpr_dbgack_mask);
		if (ret) {
			dev_info(&pdev->dev, "no cs dbgack mask\n");
			data->cs_gpr_dbgack_mask = 0;
		}
		dev_info(&pdev->dev,
			"coresight dbgack gen mask = [0x%x]\n",
			data->cs_gpr_dbgack_mask);
	}
