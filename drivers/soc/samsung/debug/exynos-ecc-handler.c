...
static int exynos_ecc_handler_probe(struct platform_device *pdev)
{
	...
	ecc_desc.ecc_irqs = kzalloc(sizeof(struct ecc_irq_desc) * irq_num, GFP_KERNEL);
	...
	of_property_for_each_string(np, "interrupt-names", prop, name) {
		...
		err = request_irq(irq, exynos_ecc_handler, IRQF_NOBALANCING,
				name, NULL);
		if (err) {
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("EXYNOS ECC HANDLER");
