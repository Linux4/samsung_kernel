...
static void link_device_dbg(struct mem_link_device *mld)
{
	struct sbd_link_device *sl = &mld->sbd_link_dev;
	int i, j;

	mif_err("last_irq_time = %lu, current_time = %lu\n",
			mld->irq_jiffies, jiffies);
	mif_err("napi_prep = %d, count=(%d, %d)\n",
			napi_schedule_prep(&mld->mld_napi),
			mld->rx_int_count, mld->rx_poll_count);

	for (j = 0; j < 2; j++) {
		for (i = 0; i < sl->num_channels ; i++) {
			struct sbd_ring_buffer *rb = sbd_id2rb(sl, i, j);

			if (!rb || !rb->rp || !rb->wp)
				continue;

			mif_err("rb[%d][%d]: rp=%d, wp=%d, empty=%d, space=%d, usage=%d\n",
					j, i, *rb->rp, *rb->wp,
					rb_empty(rb),
					rb_space(rb),
					rb_usage(rb));
...
set_type:
	...
	/* For debugging P221114-00667 */
	if (crash_type == CRASH_REASON_RIL_MNR)
		link_device_dbg(mld);
