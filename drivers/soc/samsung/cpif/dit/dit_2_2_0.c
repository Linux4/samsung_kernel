...
static struct io_device *upstream_iod;
...
static int dit_set_reg_upstream(struct net_device *netdev, bool restore)
{
	if (restore)
		netdev = upstream_iod ? upstream_iod->ndev : NULL;
...
static int dit_set_desc_pos(enum dit_direction dir, struct dit_desc_info *desc_info,
			    unsigned int tail)
{
	phys_addr_t p_desc;
	...
	/* Reset HEAD of DST */
	cpif_set_bit(mask, TX_DST_DESC_RESET_BIT);
	cpif_set_bit(mask, RX_DST_DESC_RESET_BIT);
	WRITE_REG_VALUE(dc, mask, DIT_REG_DST_DESC_RESET);
...
static int dit_do_init_hw(void)
{
	...
	dit_set_reg_upstream(NULL, false);
...
int dit_ver_create(struct dit_ctrl_t *dc_ptr)
{
	...
	dc->set_desc_pos = dit_set_desc_pos;
