...
int exynos_acpm_pasr_mask_read(void)
{
	int ret = 0;
	ret = __raw_readl(flexpmu_dbg_base + (DATA_LINE * DID_PASR) + DATA_IDX);
	return ret;
}

int exynos_acpm_pasr_size_read(void)
{
	int ret = 0;
	ret = __raw_readl(flexpmu_dbg_base + (DATA_LINE * DID_PASR) + DATA_IDX + 4);
	return ret;
}

int exynos_acpm_pasr_write(u32 mask, u32 size)
{
	__raw_writel(mask, flexpmu_dbg_base + (DATA_LINE * DID_PASR) + DATA_IDX);
	__raw_writel(size, flexpmu_dbg_base + (DATA_LINE * DID_PASR) + DATA_IDX + 4);
