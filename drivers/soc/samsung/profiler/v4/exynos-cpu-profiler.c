...
static void cpupro_hook_freq_change(void *data, struct cpufreq_policy *policy)
{
	...
	if (dompro->migov_id == MIGOV_CL1 || dompro->migov_id == MIGOV_CL2)
		exynos_profiler_stc_set_cur_freqlv(dompro->migov_id, new_freq_idx);
...
static int init_domain(struct domain_profiler *dompro, struct device_node *dn)
{
	...
	if (dompro->migov_id <= MIGOV_CL2)
		exynos_profiler_stc_set_powertable(dompro->migov_id, dompro->table_cnt, dompro->table);
...
static int exynos_cpu_profiler_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node(profiler.root, dn) {
		...
		if (dompro->migov_id != -1)
			exynos_migov_register_domain(dompro->migov_id, &cpu_fn, &cpu_pd_fn);
	}
