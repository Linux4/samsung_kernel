...
{
	exynos_profiler_set_freq_margin(margin);
	return;
}
...
static void gpupro_reset_profiler(int user)
{
	...
	profiler.fc.time[ACTIVE] = exynos_profiler_get_time_in_state();
...
u16 gpupro_get_pidinfo(int idx)
{
	return profiler.result[MIGOV].pid_list[idx];
}
...
{
	exynos_profiler_set_targettime_margin(us);
	return;
}

void gpupro_set_util_margin(int percentage)
{
	exynos_profiler_set_util_margin(percentage);
	return;
}

void gpupro_set_decon_time(int us)
{
	exynos_profiler_set_decon_time(us);
	return;
}

void gpupro_set_comb_ctrl(int enable)
{
	exynos_profiler_set_comb_ctrl(enable);
	return;
}
...
u32 gpupro_get_target_cpuid(u64 *info)
{
	return exynos_profiler_get_target_cpuid(info);
}

void gpupro_set_cputracer_size(int sz)
{
	exynos_profiler_set_cputracer_size(sz);
	return;
}
struct private_fn_gpu gpu_pd_fn = {
	...
	.get_target_cpuid       = &gpupro_get_target_cpuid,
	.set_cputracer_size		= &gpupro_set_cputracer_size,
};
...
static u32 gpupro_update_profile(int user)
{
	...
	/* Private Data */
	exynos_profiler_get_run_times(&result->rtimes[0]);
	exynos_profiler_get_pid_list(&result->pid_list[0]);
...
static int exynos_gpu_profiler_probe(struct platform_device *pdev)
{
	...
	exynos_profiler_stc_set_powertable(profiler.migov_id, profiler.table_cnt, profiler.table);
