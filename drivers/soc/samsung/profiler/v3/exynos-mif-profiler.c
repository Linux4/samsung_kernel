...
u32 mifpro_update_profile(int user)
{
	...
	for (i = 0; i < profiler.table_cnt; i++) {
		...
		result->freq_stats[1][i] = (profile_in_state[i].transfer_data_mem - profiler.prev_profile[i].transfer_data_mem) >> 20;
...
static int set_mode(void *data, u64 val)
{
	int ret;
	...
	case 0:
	case 1:
		ret = mif_fn.update_mode(SYSFS, val);
		break;
	default:
		ret = -EINVAL;
		break;
...
static int get_mode(void *data, u64 *val)
{
	*val = profiler.enabled;
	return 0;
}
DEFINE_DEBUGFS_ATTRIBUTE(mode_fops, get_mode, set_mode, "%llu\n");

static int get_stats0_sum(void *data, u64 *val)
{
	*val = mif_pd_fn.get_stats0_sum();
	return 0;
}
DEFINE_DEBUGFS_ATTRIBUTE(stats0_sum_fops, get_stats0_sum, NULL, "%llu\n");

static int get_stats0_avg(void *data, u64 *val)
{
	*val = mif_pd_fn.get_stats0_avg();
	return 0;
}
DEFINE_DEBUGFS_ATTRIBUTE(stats0_avg_fops, get_stats0_avg, NULL, "%llu\n");

static int get_active_pct(void *data, u64 *val)
{
	*val = mif_fn.get_active_pct(SYSFS);
	return 0;
}
DEFINE_DEBUGFS_ATTRIBUTE(active_pct_fops, get_active_pct, NULL, "%llu\n");

static int exynos_mif_profiler_init_debugfs(void)
{
	static struct dentry *rootdir;
	rootdir = debugfs_create_dir("mif-profiler", NULL);
	if (IS_ERR(rootdir))
		return -ENOMEM;

	debugfs_create_file("mode", 0666, rootdir, NULL, &mode_fops);
	debugfs_create_file("stats0_sum", 0444, rootdir, NULL, &stats0_sum_fops);
	debugfs_create_file("stats0_avg", 0444, rootdir, NULL, &stats0_avg_fops);
	debugfs_create_file("active_pct", 0444, rootdir, NULL, &active_pct_fops);
...
static int exynos_mif_profiler_probe(struct platform_device *pdev)
{
	...
	ret = exynos_mif_profiler_init_debugfs();
	if(ret) {
		pr_err("mifpro: failed to init debugfs\n", ret);
	}
#ifdef CONFIG_EXYNOS_DEBUG_INFO
	show_profiler_info();
#endif
