...
static int exynos_wow_ip_init(struct exynos_wow_data *data);
...
static int exynos_wow_accumulator(struct exynos_wow_data *data)
{
	...
	for (i = 0; i < data->nr_ip; i++) {
		...
		if (!ip_data->monitor_only) {
			...
			data->profile.transfer_data_mem += (rw_request * ip_data->nr_ppc * 4 * ip_data->bus_width);
		}
	}
