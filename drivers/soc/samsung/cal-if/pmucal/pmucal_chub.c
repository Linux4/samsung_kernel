...
int pmucal_chub_on(void)
{
	...
	if (!pmucal_chub_list.on) {
		pr_err("%s there is no sequence element for chub-on.\n",
				PMUCAL_PREFIX);
		return -ENOENT;
	}
	...
	if (ret) {
		pr_err("%s %s: error on handling chub-on.\n",
				PMUCAL_PREFIX, __func__);
		return ret;
	}
...
//investigating virtual address of assrtion
	...
	if (ret) {
		pr_err("%s %s: error on PA2VA conversion. aborting on...\n",
				PMUCAL_PREFIX, __func__);
		goto out;
	}
