#ifndef __ASV_EXYNOS8835_H__
#define __ASV_EXYNOS8835_H__

#ifdef CONFIG_ARM64_BTI_KERNEL
#define IS_KERNEL
#elif CONFIG_LK_TEXT_BASE
#undef IS_KERNEL
#else
#error
#endif
...
#define SMC_READ(addr, var) exynos_smc_readsfr(addr, &var)
#define IOREMAP(base, size) (unsigned int *)ioremap(base, size)
#define PR_INFO(fmt, ...) \
	printk(KERN_INFO pr_fmt(fmt), ##__VA_ARGS__)
#else
#define SMC_READ(addr, var) var = exynos_smc_read(SMC_CMD_REG, SMC_REG_ID_SFR_R(addr))
#define IOREMAP(base, size) VM_SFR(base)
#define PR_INFO(fmt, ...) \
...
{
#if defined(CONFIG_ACPM_DVFS) || defined(CONFIG_ACPM_DVFS_MODULE) || !defined(IS_KERNEL)
...
int asv_table_init(void)
{
	...
	for (i = 0; i < ASV_INFO_ADDR_CNT; i++) {
		SMC_READ((unsigned long)(ASV_TABLE_BASE + 0x4 * i), tmp);
