...
#include "pmucal/flexpmu_cal_cpu_s5e8835.h"
#include "pmucal/flexpmu_cal_local_s5e8835.h"
#include "pmucal/flexpmu_cal_p2vmap_s5e8835.h"
#include "pmucal/flexpmu_cal_system_s5e8835.h"
#include "pmucal/flexpmu_cal_define_s5e8835.h"

#if IS_ENABLED(CONFIG_CP_PMUCAL)
#include "../pmucal/include/pmucal_cp.h"
#include "pmucal/pmucal_cp_s5e8835.h"
#endif

#if IS_ENABLED(CONFIG_GNSS_PMUCAL)
#include "../pmucal/include/pmucal_gnss.h"
#include "pmucal/pmucal_gnss_s5e8835.h"
#endif

#if IS_ENABLED(CONFIG_CHUB_PMUCAL)
#include "../pmucal/include/pmucal_chub.h"
#include "pmucal/pmucal_chub_s5e8835.h"
#endif
...
#include "cmucal/clkout_s5e8835.c"
#include "acpm_dvfs_s5e8835.h"
#include "asv_s5e8835.h"
#include "../ra.h"
#include <linux/smc.h>
...
#define S5E8835_CMU_TOP_BASE		(0x12900000)
#define PLL_CON0_PLL_MMC	(0x200)
#define PLL_CON1_PLL_MMC	(0x204)
#define PLL_CON2_PLL_MMC	(0x208)
#define PLL_CON3_PLL_MMC	(0x20c)
#define PLL_CON4_PLL_MMC	(0x210)
#define PLL_CON5_PLL_MMC	(0x214)
...
unsigned int s5e8835_frac_rpll_list[] = {
	TOP_PLL_SHARED0,
	TOP_PLL_SHARED1,
	TOP_PLL_G3D,
	TOP_PLL_MMC,
...
void s5e8835_cal_data_init(void)
{
	int i;
...
//	pmucal_sys_powermode[SYS_SLEEP_HSI1ON] = CPU_INFORM_SLEEP_HSI1ON;
	...
	cmu_top = ioremap(S5E8835_CMU_TOP_BASE, SZ_4K);
	if (!cmu_top)
		pr_err("%s: cmu_top ioremap failed\n", __func__);

	frac_rpll_size = ARRAY_SIZE(s5e8835_frac_rpll_list);
	for (i = 0; i < frac_rpll_size; i++)
		frac_rpll_list[i] = s5e8835_frac_rpll_list[i];

	skip_cmu_sfr[0] = 0x12900134;
	skip_cmu_sfr[1] = 0x12900138;
	skip_cmu_sfr[2] = 0x1290013c;
}

void (*cal_data_init)(void) = s5e8835_cal_data_init;
...
bool s5e8835_is_ignore_cmu_dbg_type(u32 type)
{
	dbg_offset = (type == MUX_TYPE) ? 0x3000 : 0x4000;
...
bool (*is_ignore_cmu_dbg_type)(u32 type) = s5e8835_is_ignore_cmu_dbg_type;
