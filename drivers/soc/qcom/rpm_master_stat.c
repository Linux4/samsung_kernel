...
#ifdef CONFIG_SEC_PM
struct msm_rpm_master_stats_platform_data *debug_master_pdata;
...
{
	void __iomem *reg;
	struct msm_rpm_master_stats data;
	int i;
	...
	reg = ioremap(debug_master_pdata->phys_addr_base,
						debug_master_pdata->phys_size);
	if (!reg) {
		pr_err("%s: ERROR could not ioremap start=%pa, len=%u\n",
			__func__, &debug_master_pdata->phys_addr_base,
			debug_master_pdata->phys_size);
		return;
	}

	buf_ptr += sprintf(buf_ptr, "PM: %s:", annotation);
	for (i = 0; i < debug_master_pdata->num_masters; i++) {
		master_name = debug_master_pdata->masters[i];
		data.xo_count = readl_relaxed(reg +
				(i * debug_master_pdata->master_offset +
				offsetof(struct msm_rpm_master_stats,
				xo_count)));
		data.xo_last_entered_at = readq_relaxed(reg +
				(i * debug_master_pdata->master_offset +
				offsetof(struct msm_rpm_master_stats,
				xo_last_entered_at)));
		data.xo_last_exited_at = readq_relaxed(reg +
				(i * debug_master_pdata->master_offset +
				offsetof(struct msm_rpm_master_stats,
				xo_last_exited_at)));
		data.xo_accumulated_duration = readq_relaxed(reg +
				(i * debug_master_pdata->master_offset +
				offsetof(struct msm_rpm_master_stats,
				xo_accumulated_duration)));

		actual_last_sleep = get_time_in_sec(data.xo_accumulated_duration);
		if (data.xo_last_entered_at > data.xo_last_exited_at)
		actual_last_sleep +=
				get_time_in_sec((arch_counter_get_cntvct()
				- data.xo_last_entered_at));
		

		buf_ptr += sprintf(buf_ptr, " %s(%d, %llu, %llu, %llu, %llu)",
			master_name, data.xo_count, actual_last_sleep, get_time_in_sec(data.xo_last_entered_at), get_time_in_sec(data.xo_last_exited_at), get_time_in_sec(data.xo_accumulated_duration));

		if (i < (debug_master_pdata->num_masters - 1))
