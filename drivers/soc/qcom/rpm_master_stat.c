		...
		struct msm_rpm_master_stats_private_data *prvdata, int master_cnt)
{
	...
	if (prvdata->platform_data->version == 2) {
		...
		record.count = readl_relaxed(prvdata->reg_base +
			(master_cnt * pdata->master_offset +
			offsetof(struct msm_rpm_master_stats, count)));

		record.last_entered_at = readq_relaxed(prvdata->reg_base +
			(master_cnt * pdata->master_offset +
			offsetof(struct msm_rpm_master_stats, last_entered_at)));

		record.last_exited_at = readq_relaxed(prvdata->reg_base +
			(master_cnt * pdata->master_offset +
			offsetof(struct msm_rpm_master_stats, last_exited_at)));

		record.accumulated_duration = readq_relaxed(prvdata->reg_base +
			(master_cnt * pdata->master_offset +
			offsetof(struct msm_rpm_master_stats, accumulated_duration)));
		...
		pr_info("%s:-\n"
			...
			"\tBringup_req:0x%llX\n"
			"\tBringup_ack:0x%llX\n"
			"\tLast_sleep_transition_duration:0x%x\n"
			"\tLast_wake_transition_duration:0x%x\n"
			"\tCount:%d\n"
			"\tLast_entered_at:0x%llX\n"
			"\tLast_exited_at:0x%llX\n"
			"\tAccumulated_duration:0x%llX\n"
			"\tWakeup_reason:0x%x\n"
			"\tNumshutdowns:0x%x\n"
			"\tActive_cores:0x%x\n",
			GET_MASTER_NAME(master_cnt, prvdata),
...
{
	if(enable_master_info == 1 && is_init_done == 1) {
		int i = 0;
		pr_info("============ RPM MASTERS STATS ============");
