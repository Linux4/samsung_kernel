			...
			struct qos_info *qos)
{
	...
	list_for_each_entry(itm, &qos->flow_head, list) {
		if (itm->bearer_id == bearer->bearer_id) {
			...
			net_log("m=%d b=%u gr=%u f=%u q=%d %s",
				qos->mux_id, itm->bearer_id, bearer->grant_size,
...
#define DFC_SVC_RTNL_LOCK_RETRY 1000
static void dfc_svc_init(struct work_struct *work)
{
	int rc = 0;
	int retry = 0;
	...
	while(!rtnl_trylock()) {
		 if (data->restart_state == 1 || retry++ > DFC_SVC_RTNL_LOCK_RETRY)
			return;
		msleep(20);
	}
