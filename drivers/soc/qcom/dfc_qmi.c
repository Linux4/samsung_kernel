			...
			struct qos_info *qos)
{
	...
	list_for_each_entry(itm, &qos->flow_head, list) {
		if (itm->bearer_id == bearer->bearer_id) {
			...
			net_log("%s: m=%d b=%u gr=%u f=%u q=%d %s\n",
				__func__, qos->mux_id, itm->bearer_id, bearer->grant_size,
				...
				struct dfc_flow_status_info_type_v01 *fc_info)
{
	...
	list_for_each_entry(flow_itm, &qos->flow_head, list) {
		...
		net_log("A: m=%d b=%u gr=%u f=%u q=%d %s",
			fc_info->mux_id, flow_itm->bearer_id, fc_info->num_bytes,
			flow_itm->flow_id, flow_itm->tcm_handle,
			enable ? "en" : "dis");
	}
			     ...
			     struct dfc_flow_status_info_type_v01 *fc_info)
{
	...
	if (itm) {
		...
		if (action) {
			net_log("%s: m=%d b=%d gr=%d s=%d a=%d\n",
				__func__, fc_info->mux_id, fc_info->bearer_id,
				      ...
				      struct dfc_svc_ind *svc_ind)
{
	...
	for (i = 0; i < ind->flow_status_len; i++) {
		...
		if (unlikely(flow_status->bearer_id == 0xFF)) {
			net_log("%s: m=%d b=%d gr=%d s=%d a=%d\n",
				__func__, flow_status->mux_id, flow_status->bearer_id,
				      ...
				      struct dfc_bearer_info_type_v01 *binfo)
{
	...
	net_log("%s: %s, b=%d, g=%d, sw=%d, status %d\n", 
		__func__, dev->name, binfo->bearer_id, 
		itm->grant_size, itm->rat_switch, tx_status);
