...
int qmi_rmnet_flow_control(struct net_device *dev, u32 tcm_handle, int enable)
{
	...
	net_log("%d-%02d-%02d %02d:%02d:%02d.%04lu, %s[%d] %s_queue\n", 
			      ...
			      struct qmi_info *qmi)
{
	...
	if (itm) {
		...
		trace_dfc_qmi_tc(dev->name, itm->bearer_id, itm->flow_id,
				 ...
				 bearer->grant_size > 0 ? 1 : 0);
		net_log("%s: %s m=%d b=%u gr=%u f=%u q=%d en",
			__func__, dev->name, qos_info->mux_id, itm->bearer_id,
			bearer->grant_size,
		   ...
		   struct qmi_info *qmi)
{
	...
	if (itm) {
		...
		net_log("%s: %s m=%d b=%u gr=%u f=%u q=%d en",
			__func__, dev->name, qos_info->mux_id, itm->bearer_id, 0,
			itm->flow_id, itm->tcm_handle);
...
static void qmi_rmnet_check_stats(struct work_struct *work)
{
	...
	if (qmi->ps_enabled) {
		...
		net_log("QMI_WDA_SET_POWERSAVE_MODE_REQ_V01> off\n");
	...
	if (!rxd && !txd) {
		...
		net_log("QMI_WDA_SET_POWERSAVE_MODE_REQ_V01> on, enable queue\n");
