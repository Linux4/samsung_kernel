...
#ifdef CONFIG_CX_VOTE_TURBO
void sig_rsi_set(int on)
{
	if (on)
		smd_riset(msm_rpm_data.ch_info, 1, 0);
	else
		smd_riset(msm_rpm_data.ch_info, 0, 1);

}
EXPORT_SYMBOL(sig_rsi_set);
#endif
...
#ifdef CONFIG_CX_VOTE_TURBO
int msm_rpm_send_message_next_active(enum msm_rpm_set set, uint32_t rsc_type,
		uint32_t rsc_id, struct msm_rpm_kvp *kvp, int nelems)
{
	int i, rc;
	struct msm_rpm_request *req =
...
//	struct msm_rpm_wait_data *rc1;
	...
	for (i = 0; i < nelems; i++) {
 	...
 	msg_id = msm_rpm_send_request_noirq(req);
	next_active_msg_id = msg_id;
	//pr_err("ANIL %s %d\n", __func__, msg_id);
	//rc1 = msm_rpm_get_entry_from_msg_id(msg_id);
	//if (rc1)
	//	msm_rpm_free_list_entry(rc1);
...
EXPORT_SYMBOL(msm_rpm_send_message_next_active);
#endif
...
#ifdef CONFIG_CX_VOTE_TURBO
#define RPM_RESOURCE_TYPE_CX	0x61706D73
#define RPM_CX_KEY		0x6E726F63
#define RPM_CX_RESOURCE_ID	0x2
#define RPM_CX_VALUE		0x6
static void send_next_active_cx_vote(void)
{
	int rc =0;
	uint32_t val = RPM_CX_VALUE;
	struct msm_rpm_kvp kvp_next_active[] = {
		{
			.key = RPM_CX_KEY,
			.data = (void *)&val,
			.length = sizeof(val),
	...
	rc = msm_rpm_send_message_next_active(MSM_RPM_CTX_NEXT_ACTIVE_SET,
		RPM_RESOURCE_TYPE_CX, RPM_CX_RESOURCE_ID,
		kvp_next_active, ARRAY_SIZE(kvp_next_active));
	WARN(rc < 0, "CX Turbo next active set is not applied!\n");
}
#endif
