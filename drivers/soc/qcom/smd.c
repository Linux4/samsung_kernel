...
int smd_riset(smd_channel_t *ch, unsigned int set, unsigned int clear)
{
	unsigned long flags;
	...
	SMD_POWER_INFO("SMD RI_set on ch%d set[%d]clear[%d]\n",
					ch->n, set, clear);
	spin_lock_irqsave(&smd_lock, flags);
	if (set)
		ch->half_ch->set_fRI(ch->send, 1);
	if (clear)
		ch->half_ch->set_fRI(ch->send, 0);
	spin_unlock_irqrestore(&smd_lock, flags);

	return 0;
}
EXPORT_SYMBOL(smd_riset);
#endif
