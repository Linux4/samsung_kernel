...
#define LOCATOR_SERVICE_TIMEOUT_AVS			500000
...
static int init_service_locator_avs(void)
{
	int rc = 0;
	...
	rc = wait_for_completion_interruptible_timeout(
				...
				msecs_to_jiffies(LOCATOR_SERVICE_TIMEOUT_AVS));
	if (rc < 0) {
...
static void pd_locator_work(struct work_struct *work)
{
	...
	if (!strncmp(adsp, data->service_name, sizeof(adsp)))
		rc = init_service_locator_avs();
	else
