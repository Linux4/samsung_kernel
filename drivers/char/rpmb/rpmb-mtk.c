...
static void release_rpmb_sock(struct sock_desc *rpmb_conn)
{
	tz_iwsock_release(rpmb_conn);
}

static int rpmb_execute_emmc_teegris(struct rpmb_ctx *ctx)
{
	int ret = -1, cnt = 0;
	struct mmc_card *card = mtk_msdc_host[0]->mmc->card;
	struct emmc_rpmb_req rpmb_req;
...
static void destroy_rpmb_ctx(struct rpmb_ctx *ctx)
{
	kfree(ctx);
}
...
{
	ctx->wsm_vaddr = dma_alloc_coherent(NULL,
			sizeof(struct rpmb_req) + MAX_RPMB_REQUEST_SIZE,
			&ctx->wsm_paddr, GFP_KERNEL);
	if (!ctx->wsm_vaddr || !ctx->wsm_paddr) {
		MSG(ERR, "%s failed to alloc rpmb wsm\n", __func__);
		return -ENOMEM;
	}

	MSG(INFO, "%s rpmb dma ddr : virt(%pK), phy(%pK)\n",
			__func__, (uint64_t)ctx->wsm_vaddr, (uint64_t)ctx->wsm_paddr);

	ctx->req = (struct rpmb_req *)ctx->wsm_vaddr;
	return 0;
}

static void release_rpmb_wsm(struct rpmb_ctx *ctx)
{
	dma_free_coherent(NULL,
			sizeof(struct rpmb_req) + MAX_RPMB_REQUEST_SIZE,
			ctx->wsm_vaddr, ctx->wsm_paddr);
}
...
static int rpmb_thread(void *context)
{
	...
	while (!kthread_should_stop()) {
		...
		if (ret) {
			MSG(ERR, "rpmb weait request error : %d\n", ret);
			goto release_sock;
		}

		if (rpmb_execute_emmc_teegris(ctx))
			MSG(ERR, "rpmb execute emmc error : %d\n", ret);
...
#endif
#else
	open_th = kthread_run(rpmb_thread, NULL, "rpmb_worker");
	if (IS_ERR(open_th))
		MSG(ERR, "%s, init kthread_run failed!\n", __func__);
#endif
