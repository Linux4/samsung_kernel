...
static void rpmb_execute_emmc_teegris(struct rpmb_ctx *ctx)
{
	int ret, cnt = 0;
	struct mmc_card *card = mtk_msdc_host[0]->mmc->card;
	struct emmc_rpmb_req rpmb_req;
...
#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT) || defined(CONFIG_TZDEV)
	MSG(INFO, "%s rpmb thread start\n", __func__);
