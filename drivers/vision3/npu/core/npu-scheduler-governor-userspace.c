...
static void npu_sgu_pm_qos_update_request(struct exynos_pm_qos_request *req,
							s32 new_value)
{
	exynos_pm_qos_update_request(req, new_value);
}
#else
static void npu_sgu_pm_qos_update_request(struct pm_qos_request *req,
							s32 new_value)
{
...
void npu_governor_userspace_start(struct npu_scheduler_dvfs_info *d)
{
	...
	npu_sgu_pm_qos_update_request(&d->qos_req_min, p->init_freq);
...
void npu_governor_userspace_stop(struct npu_scheduler_dvfs_info *d)
{
	...
	npu_sgu_pm_qos_update_request(&d->qos_req_min, 0);
...
void npu_governor_userspace_register(struct npu_scheduler_info *info)
{
	...
	probe_info("register userspace governor\n");
