...
static int npu_enable_stm_sfr(struct npu_system *system)
{
	return __enable_npu_stm_sfr(system, 0xCAFE);
}
#else
static int __npu_stm_sync(struct npu_system *system)
{
...
static int npu_enable_stm_sfr(struct npu_system *system)
{
	return __enable_npu_stm_sfr(system, get_stm_trace_set());
}
#endif
...
static int __enable_npu_stm_sfr(struct npu_system *system, u32 stm_sel)
{
	...
	if (stm_sel != 0xCAFE)
		npu_write_hw_reg(npu_get_io_area(system, "sfrnpus"), 0x0078, stm_sel, 0x0000007F);
