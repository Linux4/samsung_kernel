...
void dsp_graph_manager_stop(struct dsp_graph_manager *gmgr, unsigned int id)
{
	...
	list_for_each_entry_safe(graph, temp, &gmgr->list, list) {
		...
		graph->loaded = false;
		pool = dsp_mailbox_alloc_pool(&sys->mailbox, sizeof(*gid));
		if (IS_ERR(pool)) {
			dsp_err("Failed to alloc pool(%x/%ld)\n",
					graph->global_id, PTR_ERR(pool));
			pool = graph->pool;
		} else {
			dsp_mailbox_free_pool(graph->pool);
		}
...
int dsp_graph_manager_probe(struct dsp_graph_manager *gmgr, void *core)
{
	int ret;
