...
ssize_t show_iq_buf_status(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
	...
	return snprintf(buf, PAGE_SIZE,"g_iq.head_1->WR_RD_FLAG: 0x%x\ng_iq.head_1->data_addr: 0x%x\ng_iq.head_1->data_len: 0x%x\n",
		g_iq.head_1->WR_RD_FLAG, g_iq.head_1->data_addr, g_iq.head_1->data_len);
}
