...
/* FIXME: check window_color feature */
#ifdef WINDOW_COLOR_LED
static unsigned int window_color;
...
static void s2mu005_rgb_set_state(struct led_classdev *led_cdev,
				unsigned int brightness, unsigned int led_state, int is_low_powermode)
{
	const struct device *parent = led_cdev->dev->parent;
	struct s2mu005_rgb *s2mu005_rgb = dev_get_drvdata(parent);
	struct device *dev;
	int n;
	int ret;
	unsigned int br_ratio = 0;

	pr_info("leds-s2mu005-rgb: %s: %u: %u: %d\n", __func__, brightness, led_state, is_low_powermode);
	...
	if (brightness != 0) {
		...
		switch (n) {
		case RED:
			br_ratio = is_low_powermode ? low_br_ratio_r : brightness_ratio_r;
			break;
		case GREEN:
			br_ratio = is_low_powermode ? low_br_ratio_g : brightness_ratio_g;
			break;
		case BLUE:
			br_ratio = is_low_powermode ? low_br_ratio_b : brightness_ratio_b;
			break;
		}

		brightness = brightness * br_ratio / 100;
		/* NOTE: 
		 * There is possibility that low_powermode_current is 0.  ex)
		 * low_powermode_current is 1 & brightness_ratio_r is 90
		 * brightness = 1 * 90 / 100 = 0.9 brightness is inteager, so
		 * brightness is 0.  In this case, it is need to assign 1 of
		 * value.  */
		if (brightness == 0)
			brightness = 1;
	}
			...
			*s2mu005_rgb_parse_dt(struct device *dev)
{
	...
	/* bright ratio for low power mode */
	char lb_ratio_r[23] = "lb_ratio_r";
	char lb_ratio_g[23] = "lb_ratio_g";
	char lb_ratio_b[23] = "lb_ratio_b";
	...
	strcat(lb_ratio_r, window);
	strcat(lb_ratio_g, window);
	strcat(lb_ratio_b, window);
#endif
	...
	/* get low brightness ratio */
	ret = of_property_read_u32(np, lb_ratio_r, &temp);
	if (IS_ERR_VALUE(ret))
		pr_info("leds-s2mu005-rgb: %s, can't parsing low_br_ratio_r in dt\n",
				__func__);
	else
		low_br_ratio_r = (int)temp;

	pr_info("leds-s2mu005-rgb: %s, low_br_ratio_r = %x\n", __func__, low_br_ratio_r);

	/* get led green brightness ratio */
	ret = of_property_read_u32(np, lb_ratio_g, &temp);
	if (IS_ERR_VALUE(ret))
		pr_info("leds-s2mu005-rgb: %s, can't parsing low_br_ratio_g in dt\n",
				__func__);
	else
		low_br_ratio_g = (int)temp;

	pr_info("leds-s2mu005-rgb: %s, low_br_ratio_g = %x\n", __func__, low_br_ratio_g);

	/* get led blue brightness ratio */
	ret = of_property_read_u32(np, lb_ratio_b, &temp);
	if (IS_ERR_VALUE(ret))
		pr_info("leds-s2mu005-rgb: %s, can't parsing low_br_ratio_b in dt\n",
				__func__);
	else
		low_br_ratio_b = (int)temp;

	pr_info("leds-s2mu005-rgb: %s, low_br_ratio_b = %x\n", __func__, low_br_ratio_b);
					...
					const char *buf, size_t count)
{
	...
	case CHARGING:
		s2mu005_rgb_set_state(&s2mu005_rgb->led[RED], led_dynamic_current,
			RGBLED_ALWAYS_ON, led_lowpower_mode);
		break;
	case CHARGING_ERR:
		s2mu005_rgb_blink(&s2mu005_rgb->led[RED], 500, 500);
		s2mu005_rgb_set_state(&s2mu005_rgb->led[RED], led_dynamic_current,
			RGBLED_BLINK, led_lowpower_mode);
		break;
	case MISSED_NOTI:
		s2mu005_rgb_blink(&s2mu005_rgb->led[BLUE], 500, 5000);
		s2mu005_rgb_set_state(&s2mu005_rgb->led[BLUE], led_dynamic_current,
			RGBLED_BLINK, led_lowpower_mode);
		break;
	case LOW_BATTERY:
		...
		break;
	case FULLY_CHARGED:
		s2mu005_rgb_set_state(&s2mu005_rgb->led[GREEN], led_dynamic_current,
			RGBLED_ALWAYS_ON, led_lowpower_mode);
		break;
	case VOICE_REC:
		s2mu005_rgb_blink(&s2mu005_rgb->led[BLUE], 500, 5000);
					...
					const char *buf, size_t count)
{
	...
	if (delay_off_time == 0) {
		if (led_r_brightness)
			s2mu005_rgb_set_state(&s2mu005_rgb->led[RED], led_r_brightness,
					RGBLED_ALWAYS_ON, led_lowpower_mode);
		if (led_g_brightness)
			s2mu005_rgb_set_state(&s2mu005_rgb->led[GREEN], led_g_brightness,
					RGBLED_ALWAYS_ON, led_lowpower_mode);
		if (led_b_brightness)
			s2mu005_rgb_set_state(&s2mu005_rgb->led[BLUE], led_b_brightness,
					RGBLED_ALWAYS_ON, led_lowpower_mode);
	} else {
		if (led_r_brightness)
			s2mu005_rgb_set_state(&s2mu005_rgb->led[RED], led_r_brightness,
		...
		/* set 2/5 of delay_on_time for ramp up/down time each and 1/5
		 * for stable_on_time */
				...
				const char *buf, size_t count)
{
	...
	if (brightness != 0)
		s2mu005_rgb_set_state(&s2mu005_rgb->led[RED], brightness, RGBLED_ALWAYS_ON, 0);
	else
		s2mu005_rgb_set_state(&s2mu005_rgb->led[RED], LED_OFF, RGBLED_DISABLE, 0);
			...
			const char *buf, size_t count)
{
	...
	if (brightness != 0)
		s2mu005_rgb_set_state(&s2mu005_rgb->led[GREEN], brightness, RGBLED_ALWAYS_ON, 0);
	else
		s2mu005_rgb_set_state(&s2mu005_rgb->led[GREEN], LED_OFF, RGBLED_DISABLE, 0);
		...
		const char *buf, size_t count)
{
	...
	if (brightness != 0)
		s2mu005_rgb_set_state(&s2mu005_rgb->led[BLUE], brightness, RGBLED_ALWAYS_ON, 0);
	else
		s2mu005_rgb_set_state(&s2mu005_rgb->led[BLUE], LED_OFF, RGBLED_DISABLE, 0);
			...
			const char *buf, size_t count)
{
	...
	s2mu005_rgb_set_state(&s2mu005_rgb_num->led[n], led_dynamic_current, RGBLED_BLINK, 0);
...
#ifdef WINDOW_COLOR_LED
	pr_info("leds-s2mu005-rgb: %s : window_color=0x%x\n", __func__, window_color);
#endif
...
static void s2mu005_rgb_shutdown(struct platform_device *pdev)
{
	struct s2mu005_rgb *s2mu005_rgb = platform_get_drvdata(pdev);
	int i;
	...
	devm_kfree(&pdev->dev, s2mu005_rgb);
}
