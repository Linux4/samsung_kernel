...
static void ktd2026_sw_reset(struct ktd2026_drvdata *ddata)
{
	 ...
	 */
#if 1
	ddata->client->flags |= I2C_M_IGNORE_NAK;
	i2c_smbus_write_byte_data(ddata->client, KTD2026_REG_EN_RST, KTD2026_RESET);
	ddata->client->flags &= ~(I2C_M_IGNORE_NAK);
#else
	...
	enum ktd2026_pattern mode)
{
	...
	case POWERING:
		...
		ddata->led_dynamic_current = ddata->led_lowpower_mode ? LED_LOW_CURRENT :
			get_led_max_current(ddata, LED_G);
		ktd2026_leds_on(ddata, LED_G, LED_EN_PWM1, ddata->led_dynamic_current/2);
		break;
	...
	ktd2026_get_devtree_pdata(struct i2c_client *client)
{
	...
	for_each_child_of_node(node, child_node) {
		...
		of_property_read_u8(child_node, "max_brightness", &led_conf[i].max_brightness);
...
#ifdef CONFIG_OF
static struct of_device_id ktd2026_dt_ids[] = {
	{ .compatible = "ktd2026" },
	{},
};
MODULE_DEVICE_TABLE(of, ktd2026_dt_ids);
#endif
...
static struct i2c_driver ktd2026_i2c_driver = {
	.driver = {
		...
		.name	= "ktd2026",
#ifdef CONFIG_OF
		.of_match_table = of_match_ptr(ktd2026_dt_ids),
#endif
	},
	.id_table = ktd2026_id,
	.probe = ktd2026_probe,
