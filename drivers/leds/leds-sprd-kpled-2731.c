...
#endif
#include <linux/of.h>
#include <linux/leds-sprd-kpled-2731.h>
...
#endif
#define KPLED_CTL               (ANA_REGS_GLB_BASE + 0x2c4)
...
static inline uint32_t kpled_read(unsigned long reg)
{
...
#ifdef CONFIG_OF
static struct sprd_kpled_2731_platform_data *sprd_kpled_2731_parse_dt(struct platform_device *pdev)
{
	int ret;
	struct device_node *np = pdev->dev.of_node;
	struct sprd_kpled_2731_platform_data *pdata = NULL;
...
static int sprd_kpled_probe(struct platform_device *dev)
{
        ...
        if(np) {
                pdata = sprd_kpled_2731_parse_dt(dev);
                if (pdata == NULL) {
...
static const struct of_device_id kpled_2731_of_match[] = {
        { .compatible = "sprd,sprd-kpled-2731", },
        { }
};

static struct platform_driver sprd_kpled_2731_driver = {
        .driver = {
                .name  = "sprd-kpled-2731",
                .owner = THIS_MODULE,
                .of_match_table = kpled_2731_of_match,
        },
...
static int __init sprd_kpled_2731_init(void)
{
		PRINT_INFO("Locate in sprd_kpled_2731_init!\n");
        return platform_driver_register(&sprd_kpled_2731_driver);
}

static void sprd_kpled_2731_exit(void)
{
        platform_driver_unregister(&sprd_kpled_2731_driver);
}

module_init(sprd_kpled_2731_init);
module_exit(sprd_kpled_2731_exit);

MODULE_AUTHOR("ya huang <ya.huang@spreadtrum.com>");
MODULE_DESCRIPTION("Sprd Keyboard backlight driver for 2731s");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:sprd_kpled_2731");
