...
void sm5703_fled_iris_on(void)
{
	...
	pr_err("Iris led ON\n");
	sel = sm5703_fled_set_flash_current_sel(fled_info, 2);
	if(sel < 0){
		pr_err("SM5703 fled current set fail \n");
	}
...
EXPORT_SYMBOL(sm5703_fled_iris_on);

void sm5703_fled_iris_off(void)
{
	int sel = 0;
	sm_fled_info_t *fled_info = sm_fled_get_info_by_name(NULL);
	...
	pr_err("Iris led OFF\n");
	sel = sm5703_fled_set_flash_current_sel(fled_info, 0);
	if(sel < 0){
		pr_err("SM5703 fled current restore fail \n");
	}
	sm5703_fled_set_mode(fled_info,FLASHLIGHT_MODE_OFF);
...
EXPORT_SYMBOL(sm5703_fled_iris_off);

static ssize_t led_control(struct device *dev, struct device_attribute *attr,
                         const char *buf, size_t count)
{
	int rc;
	int selector = 0;
	...
	rc = sm5703_fled_set_flash_current_sel(fled_info, selector);
	if(rc < 0){
		pr_err("SM5703 fled current set fail \n");
	}
...
static DEVICE_ATTR(front_ir_led, S_IWUSR|S_IWGRP|S_IROTH, NULL, led_control);

int create_goyaflash_sysfs(void)
{
	if (!IS_ERR(camera_class)) {
		ir_led_dev = device_create(camera_class, NULL, 0, NULL/*led_datas[0]*/, "ir_led");
		if (ir_led_dev < 0)
			pr_err("Failed to create device(flash)!\n");
		if (device_create_file(ir_led_dev, &dev_attr_front_ir_led) < 0) {
			pr_err("failed to create device file, %s\n",
				   dev_attr_front_ir_led.attr.name);
		}

#ifdef CONFIG_LEDS_SEPERATE_MOVIE_FLASH
		if (device_create_file(ir_led_dev, &dev_attr_movie_brightness) < 0) {
			pr_err("failed to create device file, %s\n",
