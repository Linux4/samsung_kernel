		...
		const char *buf, size_t count)
{
	...
	switch(nValue) {
		...
		case 100:
			...
			sel = sm5703_fled_set_movie_current_sel(fled_info, info->pdata->fled_factory_current);
			if(sel < 0){
				pr_err("SM5703 fled current set fail \n");
			}
...
static int sm5703_fled_init(struct sm_fled_info *fled_info)
{
	...
	info->base.flashlight_dev->props.torch_brightness =
		info->pdata->fled_preflash_current;
	sm5703_assign_bits(info->i2c_client, SM5703_FLEDCNTL4,
			SM5703_IMLED_MASK, info->pdata->fled_preflash_current);
...
static sm5703_fled_platform_data_t sm5703_default_fled_pdata = {
	...
	.fled_preflash_current = SM5703_MOVIE_CURRENT(120),
	.fled_movie_current = SM5703_MOVIE_CURRENT(180),
	.fled_torch_current = SM5703_MOVIE_CURRENT(60),
	.fled_factory_current = SM5703_MOVIE_CURRENT(240),
};
		...
		struct sm5703_fled_platform_data *pdata)
{
	...
	*pdata = sm5703_default_fled_pdata;
	...
	if (of_property_read_u32(np, "flash_current", &bufer) == 0) {
		dev_info(dev, "flash_current = <%d>\n", bufer);
		pdata->fled_flash_current = SM5703_FLASH_CURRENT(bufer);
	}

	if (of_property_read_u32(np, "preflash_current", &bufer) == 0) {
		dev_info(dev, "preflash_current = <%d>\n", bufer);
		pdata->fled_preflash_current = SM5703_MOVIE_CURRENT(bufer);
	}

	if (of_property_read_u32(np, "movie_current", &bufer) == 0) {
		dev_info(dev, "movie_current = <%d>\n", bufer);
		pdata->fled_movie_current = SM5703_MOVIE_CURRENT(bufer);
	}

	if (of_property_read_u32(np, "torch_current", &bufer) == 0) {
		dev_info(dev, "torch_current = <%d>\n", bufer);
		pdata->fled_torch_current = SM5703_MOVIE_CURRENT(bufer);
	}

	if (of_property_read_u32(np, "factory_current", &bufer) == 0) {
		dev_info(dev, "factory_current = <%d>\n", bufer);
		pdata->fled_factory_current = SM5703_MOVIE_CURRENT(bufer);
	}
