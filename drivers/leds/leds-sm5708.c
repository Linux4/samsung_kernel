...
struct mutex g_sm5708_led_mutex;
...
int sm5708_fled_turn_on_only_torch(unsigned char index)
{
    if (assistive_light[index] == false)
    {
       if(NULL != g_sm5708_fled)    
         _fled_turn_on_torch(g_sm5708_fled, index);
    }
    return 0;
}
...
{
    int ret = 0;
    mutex_lock(&g_sm5708_led_mutex);
    if(flash_ctrl->index == 0)
    {
       ret=sm5708_fled_torch_on(flash_ctrl->index); //rear torch use default current
    }
    else if(flash_ctrl->index == 1){//front torch	
       if(flash_ctrl->flash_current_mA == -1)
          //just switch on
         ret = sm5708_fled_turn_on_only_torch(flash_ctrl->index);
       else
          //just apply current only
         ret = sm5708_fled_torch_set_current(flash_ctrl->index, flash_ctrl->flash_current_mA);
    }
    mutex_unlock(&g_sm5708_led_mutex);
    return ret;
}
...
int msm_fled_led_off_sm5708(ext_pmic_flash_ctrl_t *flash_ctrl)
{
    int ret;
    mutex_lock(&g_sm5708_led_mutex);
    ret = sm5708_fled_led_off(flash_ctrl->index);
    mutex_unlock(&g_sm5708_led_mutex);
    return ret;
}
