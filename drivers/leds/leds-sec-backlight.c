...
static int led_gpio_bkl_probe(struct platform_device *pdev)
{
	int rc = 0, i;
	u32 tmp;
	const char *temp_str;
	...
	key_pinctrl = devm_pinctrl_get_select(&pdev->dev,"backlight_active");
...
static int led_gpio_bkl_remove(struct platform_device *pdev)
{
	struct led_gpio_bkl_data *bkl_led =
...
#ifdef CONFIG_PM
static int led_gpio_bkl_suspend(struct device *dev)
{
	struct pinctrl *key_pinctrl;

	key_pinctrl = devm_pinctrl_get_select(dev,"backlight_suspend");
	if (IS_ERR(key_pinctrl)){
		if(PTR_ERR(key_pinctrl) == -EPROBE_DEFER)
				return -EPROBE_DEFER;
		pr_info("Target does not use pinctrl\n");
...
static int led_gpio_bkl_resume(struct device *dev)
{
	struct pinctrl *key_pinctrl;

	key_pinctrl = devm_pinctrl_get_select(dev,"backlight_active");
	if (IS_ERR(key_pinctrl)){
		if(PTR_ERR(key_pinctrl) == -EPROBE_DEFER)
				return -EPROBE_DEFER;
		pr_info("Target does not use pinctrl\n");
...
static SIMPLE_DEV_PM_OPS(led_gpio_bkl_pm_ops, led_gpio_bkl_suspend, led_gpio_bkl_resume);
#endif

static struct platform_driver led_gpio_bkl_driver = {
	.probe = led_gpio_bkl_probe,
	.remove =led_gpio_bkl_remove,
	.driver = {
		   .name = LED_GPIO_BACKLIGHT_DRIVER_NAME,
		   .owner = THIS_MODULE,
	   #ifdef CONFIG_PM
		   .pm = &led_gpio_bkl_pm_ops,
#endif
