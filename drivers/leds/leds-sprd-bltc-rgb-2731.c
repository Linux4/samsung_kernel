...
{
        sci_adi_set(brgb->sprd_arm_module_en_addr + 0x08, (0x1<<9));//ARM_MODULE_EN-enable pclk
        sci_adi_set(brgb->sprd_arm_module_en_addr + 0x18, (0x1<<7));//RTC_CLK_EN-enable rtc
        sci_adi_clr(brgb->sprd_arm_module_en_addr + 0x2bc, (0x1<<0));//SW POWERDOWN DISABLE
        sci_adi_clr(brgb->sprd_arm_module_en_addr + 0x2bc, (0x1f<<4));//CURRENT CONTROL DEFAULT
	sci_adi_set(brgb->sprd_arm_module_en_addr + 0x2c0, (0x1<<8));//WHTLED_SERIES_EN=1
	sci_adi_set(brgb->sprd_arm_module_en_addr + 0x2c0, (0x1<<0));//WHTLED POWERDOWN ENABLE
}
...
static void sprd_leds_bltc_rgb_disable(struct sprd_leds_bltc_rgb *brgb)
{
        ...
        PRINT_INFO("reg:0x%08X set_val:0x%08X brightness:%1d\n", brgb->sprd_bltc_base_addr + BLTC_CTRL, \
                   sprd_leds_bltc_rgb_read(brgb->sprd_bltc_base_addr + BLTC_CTRL),brgb->on_off);
}
...
//        PRINT_INFO("base_addr1:0x%1lx\n",base_addr1);	/***0x40038800***/
        ...
        for (i = 0; i < SPRD_LED_TYPE_TOTAL; i++) {
                ...
                brgb->enable = 0;
#if CONFIG_OF
                brgb->sprd_bltc_base_addr = SPRD_ADI_BASE + 0x8200;		/***0x40038200***/
                brgb->sprd_arm_module_en_addr = SPRD_ADI_BASE + 0x8c00;	/***0x40038c00***/
#else
                brgb->sprd_bltc_base_addr = pdata->sprd_base_addr + 0x200;		/***0x40038200***/
                brgb->sprd_arm_module_en_addr = pdata->sprd_base_addr + 0xc00;	/***0x40038c00***/
#endif
