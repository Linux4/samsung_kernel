...
static inline unsigned long kpled_read(unsigned long reg)
{
...
static void sprd_kpled_set_brightness( unsigned long  brightness)
{
	...
	// Set Output Current
	sci_adi_write((unsigned long)KPLED_CTL, ((brightness_level << KPLED_V_SHIFT) & KPLED_V_MSK), KPLED_V_MSK);
	PRINT_INFO("reg:0x%08X set_val:0x%08X  brightness:%ld  brightness_level:%ld(0~15)\n", \
		KPLED_CTL, kpled_read((unsigned long)KPLED_CTL), brightness, brightness_level);
}

static void sprd_kpled_enable(struct sprd_kpled *led)
{
#ifdef CONFIG_ARCH_SCX35
	sci_adi_clr((unsigned long)KPLED_CTL, KPLED_PD_SET);
	#ifdef CONFIG_ARCH_SCX30G
	sci_adi_set((unsigned long)CA_CTRL2, LDO_KPLED_PD);
	#endif
#else
	sci_adi_clr((unsigned long)KPLED_CTL, KPLED_PD_SET|KPLED_PD_RST);
	sci_adi_set((unsigned long)KPLED_CTL, KPLED_PD_RST);
#endif
...
{
#ifdef CONFIG_ARCH_SCX35
	sci_adi_set((unsigned long)KPLED_CTL, KPLED_PD_SET);
	#ifdef CONFIG_ARCH_SCX30G
	sci_adi_clr((unsigned long)CA_CTRL2, LDO_KPLED_PD);
	#endif
#else
	sci_adi_clr((unsigned long)KPLED_CTL, KPLED_PD_SET|KPLED_PD_RST);
	sci_adi_set((unsigned long)KPLED_CTL, KPLED_PD_SET);
...
#ifdef CONFIG_ARCH_SCX30G
	sci_adi_clr((unsigned long)CA_CTRL2, SLP_LDOKPLED_PD_EN);
#endif
