...
//		else {/* If enabled led is already master do nothing */}
...
static void an30259a_smartglow_animate_work(struct work_struct *work)
{
	...
	if((g_smartglow_data->in_ledaccess & 0xF0) == 0)
	{
		...
		if(!g_smartglow_data->IsResetMode) {	
			smartglow_reset();
		...
		if(!g_smartglow_data->in_ledaccess)
		{
			if(g_smartglow_data->IsResetMode) {
				...
				g_smartglow_data->IsResetMode  = 1;
			}
	...
	g_smartglow_data->IsResetMode  = 0;
...
MODULE_DESCRIPTION("AN30259A LED driver for Samsung SmartGlow");
