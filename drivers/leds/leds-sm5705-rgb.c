...
static int sm5705_rgb_set_LEDx_current(struct sm5705_rgb *sm5705_rgb, int index, unsigned char cur_value)
{
	...
	dev_info(sm5705_rgb->dev, "%s: LED[%d] Current Set: 0x%x\n",
						__func__, index, cur_value);
...
static int store_led_color(struct sm5705_rgb *sm5705_rgb, int idx, int brightness)
{
	...
	ret = sm5705_rgb_set_LEDx_enable(sm5705_rgb, idx, 0, (bool)brightness);
	if (IS_ERR_VALUE(ret))
		return -1;
	...
	for (i=0; i < LED_MAX; ++i) {
		if (led_br[i]) {
			...
			sm5705_rgb_set_LEDx_enable(sm5705_rgb, i,
					sm5705_rgb->delay_off_times_ms ? 1: 0, 1);
		}
	}
...
static DEVICE_ATTR(led_pattern, S_IRUSR| S_IWUSR| S_IRGRP| S_IWGRP, NULL, store_sm5705_rgb_pattern);
static DEVICE_ATTR(led_blink, S_IRUSR| S_IWUSR| S_IRGRP| S_IWGRP, NULL,  store_sm5705_rgb_blink);
static DEVICE_ATTR(led_brightness, S_IRUSR| S_IWUSR| S_IRGRP| S_IWGRP, NULL, store_sm5705_rgb_brightness);
...
static int sm5705_rgb_parse_dt(struct device *dev)
{
