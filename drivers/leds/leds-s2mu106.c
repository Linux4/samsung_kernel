...
int s2mu106_led_mode_ctrl(int state)
{
	...
	switch(state) {
		...
		case S2MU106_FLED_MODE_TORCH:
			...
			break;
		case S2MU106_FLED_MODE_FLASH:
			s2mu106_fled_operating_mode(fled, BOOST_MODE);
...
/* flashlight enable/disable function */
static int s2mu106_enable(int value)
{
	int mode = -1;
	int flash_current = 0;
	int torch_current = 0;

	pr_info("%s: enable start\n", __func__);
	...
	if (value <= 0) {
		...
		mode = S2MU106_FLED_MODE_TORCH;
		torch_current = s2mu106_fled_get_torch_curr(g_fled_data, 1);
	} else if (value == 2) {
		mode = S2MU106_FLED_MODE_FLASH;
		flash_current = s2mu106_fled_get_flash_curr(g_fled_data, 1);
	} else if (value == 100) {
		/* Factory Torch*/
	...
	if (g_fled_data->control_mode == CONTROL_I2C) {
		...
		if (mode == S2MU106_FLED_MODE_FLASH) {
			...
			s2mu106_led_mode_ctrl(S2MU106_FLED_MODE_FLASH);
			fd_use = true;
		} else if (mode == S2MU106_FLED_MODE_TORCH) {
...
int s2mu106_enable_external(int state, int curr) {
	if(state) {
		s2mu106_fled_set_curr(1, CAM_FLASH_MODE_SINGLE, curr);
		s2mu106_fled_set_curr(1, CAM_FLASH_MODE_TORCH, curr);
	}
	s2mu106_enable(state);
	return 0;
}
EXPORT_SYMBOL(s2mu106_enable_external);

bool s2mu106_is_fd_in_use() {
	return fd_use;
}
EXPORT_SYMBOL(s2mu106_is_fd_in_use);
