...
{
	if(mode == TA_MODE) {
		s2mu106_update_reg(g_fled_data->i2c, S2MU106_FLED_CTRL0, 0x80, 0xC0);
		pr_info("%s: TA only mode set\n", __func__);
	}
	else if(mode == AUTO_MODE) {
		s2mu106_update_reg(g_fled_data->i2c, S2MU106_FLED_CTRL0, 0x00, 0xC0);
...
static void s2mu106_fled_init(struct s2mu106_fled_data *fled)
{
	...
	if (gpio_is_valid(fled->pdata->flash_gpio) &&
		...
		fled->flash_gpio = fled->pdata->flash_gpio;
		fled->torch_gpio = fled->pdata->torch_gpio;
	...
	if(factory_mode)
		s2mu106_fled_set_operation_mode(TA_MODE);
	...
	/* flash current set */
	s2mu106_fled_set_flash_curr(fled, 1, fled->pdata->flash_current);

	/* torch current set */
	s2mu106_fled_set_torch_curr(fled, 1, fled->pdata->torch_current);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (g_fled_data->control_mode == CONTROL_I2C) {
		...
		if (mode == S2MU106_FLED_MODE_TORCH) {
			...
			/* flase, torch current set for default current */
			flash_current = g_fled_data->pdata->flash_current;
			torch_current = g_fled_data->pdata->torch_current;
...
static int s2mu106_led_probe(struct platform_device *pdev)
{
	...
	g_fled_data->flash_current = fled_data->flash_current;
	g_fled_data->torch_current = fled_data->torch_current;
