		...
		unsigned long action, void *data)
{
	...
	case MUIC_NOTIFY_CMD_LOGICALLY_DETACH:
		...
		if (!factory_mode) {
			/* CHGIN_ENGH = 0 */
        pr_err("%s : Disabling factory mode.\n", __func__);
			ret = s2mu005_update_reg(led_data->i2c,
	...
	case MUIC_NOTIFY_CMD_LOGICALLY_ATTACH:
		...
		attach_cable_check(attached_dev, &ta_attached,
						&sdp_attached);
		pr_err("%s : ta_attached[%d] sdp_attached[%d]\n", __func__, ta_attached, sdp_attached);
		if (ta_attached) {
			s2mu005_read_reg(led_data->i2c,
...
static void led_set(struct s2mu005_led_data *led_data)
{
	...
	if (led_data->test_brightness == LED_OFF && !assistive_light) {
		...
		if (ta_attached) {
			...
			if (ret < 0)
				goto error_set_bits;
			pr_err("%s torch mode setting complete. 0x37[7] = 1\n", __func__);
...
static int s2mu005_led_setup(struct s2mu005_led_data *led_data)
{
	...
	pr_err("%s Main Flash Current=%d\n", __func__,led_data->flash_brightness);
	...
	pr_err("%s Pre Flash Current=%d\n", __func__,led_data->preflash_brightness);
...
pr_err("%s, factory_mode=%d",__func__,factory_mode);
	if (factory_mode) {
...
int ss_rear_torch_set_flashlight(bool isFlashlight)
{
	...
	if (assistive_light == false) {
		...
		client = g_led_datas[S2MU005_FLASH_LED]->i2c;
		if(isFlashlight){
			CDBG("%s g_led_datas[S2MU005_FLASH_LED]->torch_brightness = %d \n", __func__,g_led_datas[S2MU005_FLASH_LED]->torch_brightness);
			/* flashright brightness set */
			return s2mu005_update_reg(client, S2MU005_REG_FLED_CH1_CTRL1,g_led_datas[S2MU005_FLASH_LED]->torch_brightness, S2MU005_TORCH_IOUT_MASK);
		}
		else{
...
int ss_rear_flash_set_brightness(int brightness)
{
	struct s2mu005_led_data *led_data = g_led_datas[S2MU005_FLASH_LED];

	pr_info("%s Input Current=%d\n", __func__, brightness);
	int level = S2MU005_FLASH_BRIGHTNESS(brightness);
	/* flash brightness set */
	s2mu005_update_reg(led_data->i2c, S2MU005_REG_FLED_CH1_CTRL0,
		level, S2MU005_FLASH_IOUT_MASK);

	/* torch brightness set */
	if (brightness <= 400) {
		level = S2MU005_TORCH_BRIGHTNESS(brightness);
		s2mu005_update_reg(led_data->i2c, S2MU005_REG_FLED_CH1_CTRL1,
		level, S2MU005_TORCH_IOUT_MASK);
	}
	return 0;
} EXPORT_SYMBOL_GPL(ss_rear_flash_set_brightness);
				...
				const char *buf, size_t size)
{
	...
	switch(value) {
		...
		case 200:
			...
			led_data = g_led_datas[S2MU005_FLASH_LED];
			led_cdev = &led_data->cdev;
			brightness = led_data->flash_brightness;
			assistive_light = true;
		...
		case 1004:
			...
			break;
		case 1006:
			brightness = S2MU005_TORCH_OUT_I_175MA;
			assistive_light = true;
			break;
		case 1009:
			brightness = S2MU005_TORCH_OUT_I_225MA;
			assistive_light = true;
			break;
		default:
				...
				struct s2mu005_fled_platform_data *pdata)
{
	...
	np = of_find_node_by_name(led_np, "s2mu005_fled");
	if (!np) {
	...
	ret = of_property_read_u32(np, "main_flash_current", &temp);
	if (ret < 0)
		goto dt_err;
	pdata->flash_brightness = S2MU005_FLASH_BRIGHTNESS(temp);
	dev_info(dev, "main_flash_current = <%d>, brightness = %x\n", temp, pdata->flash_brightness);
	...
	dev_info(dev, "movie_brightness = <%d>, brightness = %x\n", temp, pdata->movie_brightness);	
...
#if 0 //Alok not required for this project	
