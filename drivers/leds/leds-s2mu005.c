...
void ss_rear_torch_set_flashlight(bool isFlashlight);
...
//	s2mu005_update_reg(client,S2MU005_REG_FLED_CH1_CTRL1,0x0F,S2MU005_TORCH_IOUT_MASK);
//	s2mu005_update_reg(client,S2MU005_REG_FLED_CH1_CTRL1,0x03,S2MU005_TORCH_IOUT_MASK);
	s2mu005_update_reg(client,CH_FLASH_TORCH_EN,S2MU005_TORCH_ON_I2C,S2MU005_CH1_FLASH_TORCH_OFF_MASK);
	pr_info("%s\n", __func__);
...
void ss_front_flash_led_turn_on()
{
	...
	//struct s2mu005_dev *s2mu005 = dev_get_drvdata(pdev->dev.parent);
	struct i2c_client * client;
	 client = g_led_datas[S2MU005_TORCH_LED]->i2c;

	s2mu005_update_reg(client,CH_FLASH_TORCH_EN,S2MU005_FLASH_TORCH_OFF,S2MU005_CH2_FLASH_TORCH_OFF_MASK);
	s2mu005_update_reg(client,S2MU005_REG_FLED_CH2_CTRL1,0x00,S2MU005_TORCH_IOUT_MASK);
	s2mu005_update_reg(client,CH_FLASH_TORCH_EN,S2MU005_CH2_TORCH_ON_I2C,S2MU005_CH2_FLASH_TORCH_OFF_MASK);
