...
/*F52  code for QN3979-387 by xuxianwei at 20210224 start*/
...
/*F52  code for QN3979-387 by xuxianwei at 20210224 end*/
...
/*F52  code for QN3979-387 by xuxianwei at 20210224 start*/
	...
	{
		pr_err("xuxianwei add led:torch_0 is ok\n");
...
/*F52  code for QN3979-387 by xuxianwei at 20210224 start*/
	...
	{
		pr_err("xuxianwei add led:switch_0 is ok\n");
					...
					const char *buf, size_t len)
{
	...
	{
		pr_err("chengzhi add the torch stength error");
		return -EINVAL;
	}
	...
	#endif
	pr_err("chengzhi add get torch no initial value:%d",get_torch_strength);
	if(0x00==get_torch_strength)
	{
		qpnp_flash_led_brightness_set(led_cdev_samsung_switch,0);
		qpnp_flash_led_brightness_set(led_cdev_samsung_torch,100);
		qpnp_flash_led_brightness_set(led_cdev_samsung_switch,1);
		pr_err("chengzhi add set the torch strength inital!:%d",100);
	}
	pr_err("chengzhi add set torch value:%d",set_torch_strength);
