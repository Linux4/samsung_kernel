...
static void aw36518_set_flash_time_out_duration(int duration)
{
	...
	aw36518_i2c_read(aw36518_i2c_client, AW36518_REG_TIMING_CONF, &read_val);
...
static void aw36518_set_torch_cur(int cur)
{
	...
	aw36518_i2c_write(aw36518_i2c_client, AW36518_REG_TORCH_LEVEL_LED1, val);
...
static int aw36518_set_flash_cur(int cur)
{
	...
	ret = aw36518_i2c_write(aw36518_i2c_client, AW36518_REG_FLASH_LEVEL_LED1, val);
...
int aw36518_get_dts_info(struct i2c_client *client, struct aw36518_chip_data *pdata)
{
	int ret = 0;
	int i = 0;
	unsigned int temp = 0;
	...
	/* debug info */
	for (i = 0; i < 5; i++) {
		pr_info("%s: torch_level_%d_cur = %d", __func__, i, pdata->torch_level[i]);
	}

	pdata->gpio_request = -1;
	pdata->fen_pin = -1;
	ret = pdata->fen_pin = of_get_named_gpio(node, "strobe-torch-gpio", 0);
	pr_info("pdata->fen_pin = %d", pdata->fen_pin);
	if (ret < 0) {
		pr_err("%s : can't get flash-en-gpio\n", __func__);
	} else {
		pdata->gpio_request =  gpio_request(pdata->fen_pin, "aw36518_fled");
		if (pdata->gpio_request < 0) {
...
static ssize_t aw36518_rear_flash_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	u32 store_value;
	...
	switch (store_value) {
		...
		case 1: /*  1  set default brightness of torch and turn on torch */
			aw36518_i2c_write(aw36518_i2c_client, AW36518_REG_TORCH_LEVEL_LED1, AW36518_TORCH_DEFAULT_CUR_REG_VAL);
			aw36518_enable_torch(true);
			break;
		case 100: /* 100 :  set higtest brightness of torch and turn on torch */
			aw36518_i2c_write(aw36518_i2c_client, AW36518_REG_TORCH_LEVEL_LED1, AW36518_TORCH_HIGTEST_CUR_REG_VAL);
			aw36518_enable_torch(true);
			break;
		case 200:  /* 200 : factory flash test */
			aw36518_i2c_write(aw36518_i2c_client, AW36518_REG_FLASH_LEVEL_LED1, AW36518_FLASH_FACTORY_CUR_REG_VAL);
...
static DEVICE_ATTR(rear_flash, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH, aw36518_rear_flash_show, aw36518_rear_flash_store);
