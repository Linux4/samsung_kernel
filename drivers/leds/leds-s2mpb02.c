			...
			size_t count)
{
	...
	if (!gpio_is_valid(flash1_gpio))
		pr_info("[LED]%s, gpio state torch:%d", __func__, gpio_get_value(torch1_gpio));
	else
		pr_info("[LED]%s, gpio state flash:%d, torch:%d", __func__,
			gpio_get_value(flash1_gpio), gpio_get_value(torch1_gpio));
