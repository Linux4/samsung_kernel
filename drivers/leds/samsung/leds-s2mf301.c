...
{
	u8 dest = 0, bit = 0, mask = 0, status = 0, reg_mode = 0;
	...
	if (mode == S2MF301_FLED_MODE_OFF) {
		...
		s2mf301_read_reg(fled->chg, 0x18, &reg_mode);
		/* flash -> off, TA exist */
		if ((status & S2MF301_CH1_FLASH_ON) && (reg_mode == 0x01 || reg_mode == 0x03)) {
			/* charger off */
			s2mf301_update_reg(fled->chg, 0x18, 0x0, 0x0F);
			mdelay(1);
			/* charger mode */
			s2mf301_update_reg(fled->chg, 0x18, reg_mode, 0x0F);
		}
	}
	return 0;
}
