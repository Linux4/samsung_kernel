...
static int sm5714_fled_control(u8 fled_mode)
{
	...
	if (fled_mode == FLED_MODE_FLASH) {
		...
		if (ret < 0) {
			...
			msleep(5); //it need to delay change, becuase it may affect the captured image.
		...
		if (fled->pdata->led.en_mled == true) {
			...
			if (fled->torch_on_cnt == 0) {
				sm5714_charger_oper_push_event(SM5714_CHARGER_OP_EVENT_TORCH, 0);
		...
		if (fled->pdata->led.en_fled == true) {
			...
			if (fled->flash_on_cnt == 0) {
				sm5714_charger_oper_push_event(SM5714_CHARGER_OP_EVENT_FLASH, 0);
...
static int sm5714_fled_torch_on(u8 brightness)
{
	...
	if (fled->pdata->led.en_mled == false) {
		if (fled->torch_on_cnt == 0) {
			sm5714_charger_oper_push_event(SM5714_CHARGER_OP_EVENT_TORCH, 1);
		}
...
static int sm5714_fled_flash_on(u8 brightness)
{
	...
	if (fled->pdata->led.en_fled == false) {
		if (fled->flash_on_cnt == 0) {
			sm5714_charger_oper_push_event(SM5714_CHARGER_OP_EVENT_FLASH, 1);
		}
