...
#if defined(CONFIG_MACH_BAFFINQ) || defined(CONFIG_MACH_CS05)
	rt8547_send_data(0x01, 0x00);
#else
...
#if defined(CONFIG_MACH_BAFFINQ) || defined(CONFIG_MACH_CS05)
int rt8547_set_torch_current(int value)
{
	struct rt8547_platform_data *pdata = g_chip_ptr->dev->platform_data;
	int reg;
	...
	switch(value)
	{
		case 0:
			printk("%s: Torch Off\n", __func__);
			printk("%s: Set default torch current\n", __func__);
			reg = 0x02; // Timeout current
			reg_value[reg-1] = ((reg_value[reg-1]&~TIMEOUT_CURRENT_MASK)|(pdata->timeout_current<<5));
			reg = 0x03; // torch current
		...
		case 1:
			...
			break;
		case 2:
			printk("%s: Set 200mA torch current\n", __func__);
			reg = 0x02; // Timeout current
			reg_value[reg-1] = ((reg_value[reg-1]&~TIMEOUT_CURRENT_MASK)|(TIMEOUT_CURRENT_200MA<<5));
			reg = 0x03; // torch current
			reg_value[reg-1] = ((reg_value[reg-1]&~TORCH_CURRENT_MASK)|TORCH_CURRENT_200MA);
			break;
		case 3:
			printk("%s: Set 300mA torch current\n", __func__);
			reg = 0x02; // Timeout current
			reg_value[reg-1] = ((reg_value[reg-1]&~TIMEOUT_CURRENT_MASK)|(TIMEOUT_CURRENT_300MA<<5));
			reg = 0x03; // torch current
			reg_value[reg-1] = ((reg_value[reg-1]&~TORCH_CURRENT_MASK)|TORCH_CURRENT_300MA);
			break;
		case 4:
			printk("%s: Set 400mA torch current\n", __func__);
			reg = 0x02; // Timeout current
			reg_value[reg-1] = ((reg_value[reg-1]&~TIMEOUT_CURRENT_MASK)|(TIMEOUT_CURRENT_400MA<<5));
			reg = 0x03; // torch current
			reg_value[reg-1] = ((reg_value[reg-1]&~TORCH_CURRENT_MASK)|TORCH_CURRENT_400MA);
			break;
		default:
			printk("%s: Undefined value(%d), Set default torch current\n", __func__, value);
			reg = 0x02; // Timeout current
			reg_value[reg-1] = ((reg_value[reg-1]&~TIMEOUT_CURRENT_MASK)|(pdata->timeout_current<<5));
			reg = 0x03; // torch current
			reg_value[reg-1] = ((reg_value[reg-1]&~TORCH_CURRENT_MASK)|pdata->torch_current);
...
EXPORT_SYMBOL(rt8547_set_torch_current);
#endif
