...
#include "../misc/mediatek/flashlight/flashlight.h"

//#define SG_REMOVE_FOR_BRINGUP
#ifdef SG_REMOVE_FOR_BRINGUP
...
ssize_t front_flash_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	...
	if (value == 0) {
		...
		switch(value)
		{
			...
			case 1006:
				...
				break;
			case 1009:
				brightness +=2;//level-5
				break;
			/*
			case 1010:
				brightness = 5;//level-5
				break;
			case 1011:
				brightness = 6;//level-5
				break;
			case 1012:
				brightness = 7;//level-5	
				break;
				*/
			default:
				break;
...
ssize_t front_flash_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
 ...
 *****************************************************************************/
int s2mu005_led_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	//verify channel 
	/*
	if (channel < 0 || channel >= s2mu005_led_CHANNEL_NUM) {
	...
	case FLASH_IOC_SET_TIME_OUT_TIME_MS:
		...
		s2mu005_led_timeout_ms[channel] = fl_arg->arg;
		break;
		      */
	...
	case FLASH_IOC_SET_DUTY:
		...
		if(channel == 0)
		{
			...
			else if(gduty == 2) //preflash
			{
				ss_rear_torch_set_flashlight(0);
			}
...
int s2mu005_led_open(void *pArg)
{
	return 0;
}
int s2mu005_led_release(void *pArg)
{
	return 0;
}
int s2mu005_led_set_driver(void)
{
	return 0;
}
ssize_t s2mu005_led_strobe_store(struct flashlight_arg arg)
{
	return 0;
}

struct flashlight_operations s2mu005_ops = {
	s2mu005_led_open,
	s2mu005_led_release,
	s2mu005_led_ioctl,
	s2mu005_led_strobe_store,
	s2mu005_led_set_driver
};
...
static int s2mu005_led_probe(struct platform_device *pdev)
{
...
#ifndef CONFIG_OF
	struct s2mu005_mfd_platform_data *s2mu005_pdata;
#endif
	struct s2mu005_fled_platform_data *pdata;
	struct s2mu005_led_data *led_data;
	struct s2mu005_led *data;
	...
	if (flashlight_dev_register("flashlights-s2mu005", &s2mu005_ops)) {
			pr_err("ERROR: Failed to register flashlight device.\n");
			
	}
	
	pr_err("s2mu005_led_probe start");
	probe_flag = false;
	s2mu005 = dev_get_drvdata(pdev->dev.parent);
#ifndef CONFIG_OF
