...
_AllocateContiguousMemory(
    ...
    IN gctSIZE_T ContiguousRequested,
    ...
    )
{
        ...
        while (device->contiguousSize > 0)
        {
            ...
            if (gcmIS_SUCCESS(status))
            {
                ...
                if (kernel->hardware->mmuVersion == 0)
                    ...
                    physAddr = ((PLINUX_MDL)device->contiguousPhysical)->dmaHandle;
    ...
    else
    {
        ...
        {
            if (ContiguousRequested)
            {
                struct resource* mem_region = gcvNULL;
            ...
            device->contiguousRequested      = ContiguousRequested;
