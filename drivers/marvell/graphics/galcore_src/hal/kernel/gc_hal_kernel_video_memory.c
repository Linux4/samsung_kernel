    ...
    )
{
    ...
    /* Before free vidmem node, clean up its memory, then set the magic value to mark node has been freed. */
    gckOS_ZeroMemory((gctPOINTER)node, gcmSIZEOF(gcuVIDMEM_NODE));

    node->VidMem.magic = 0xDEADDEAD;
    ...
    )
{
    ...
    node->Virtual.magic = 0;
    node->Virtual.line = 0;
    gckOS_Sprintf(node->Virtual.funcName, "null");
	trace_printk("Vir Node Construct node:%p, kernel:%p\n",node,node->Virtual.kernel);
    ...
    if(IsPmem)
    {
        ...
        status = gckOS_AllocatePmemMemory(os,
                                    node->Virtual.bytes = Bytes,
                                    &node->Virtual.physical);

        if (gcmIS_ERROR(status))
        {
            database->ionAllocFail++;
            gcmkTRACE_ZONE(gcvLEVEL_WARNING, gcvZONE_VIDMEM,
                        "Cannot allocate pmem for size 0x%x\n", Bytes);
            goto OnError;
        }
        else
        {
            gckOS_GetTicks(&endTime);
            database->ionAllocTime += endTime - startTime;
            database->ionAllocSucc++;
        }
    }
    else
#endif
    {
        ...
        {
            gckOS_GetTicks(&endTime);
            database->conAllocTime += endTime - startTime;
            database->conAllocSucc++;
        }

        if (gcmIS_ERROR(status) && !node->Virtual.contiguous)
        {
            database->virtAllocFail++;
        ...
        {
            gckOS_GetTicks(&endTime);
            database->virtAllocTime += endTime - startTime;
            database->virtAllocSucc++;
        }
    }
    ...
    )
{
    ...
    )
    {
        ...
        status = gcvSTATUS_OUT_OF_MEMORY;
        gcmkTRACE_ZONE(gcvLEVEL_WARNING, gcvZONE_KERNEL, "Cannot allocate from reserved memory due to small memory limitation\n");
        goto OnError;
    ...
    node->VidMem.memory    = Memory;
    node->VidMem.line      = 0;
    gckOS_Sprintf(node->VidMem.funcName, "null");
    ...
    *Node = node;
    ...
    database->resAllocSucc++;
    database->resAllocTime += endTime - startTime;
    ...
    )
{
    ...
    else
    {
        ...
        if(Node->Virtual.bPmem)
        {
            ...
            gcmkVERIFY_OK(
                gckOS_FreePmemMemory(kernel->os,
                                    Node->Virtual.physical,
                                    Node->Virtual.bytes));
        }
        else
...
)
{
    ...
    if (Node->Virtual.contiguous)
    {
...
#endif
        {
            ...
            gcmkONERROR(gckHARDWARE_ConvertLogicalMdl(Kernel->hardware,
    ...
    )
{
    ...
    if (Node->VidMem.memory->object.type == gcvOBJ_VIDMEM)
    {
        if ((Node->VidMem.magic == 0xDEADDEAD)||(Node->VidMem.magic != 0))
        {
			gcmkPRINT("VidMem Node:%p lock Invalid magic : %x\n",Node, Node->VidMem.magic);
            gcmkONERROR(gcvSTATUS_INVALID_OBJECT);
        }
...
#endif
#if ALLDYNAMIC
        trace_printk("%d@Paddr0x%x,type:%s:%s,%d\n",(gctUINT32)Node->VidMem.bytes,*Address,"Resv",Node->VidMem.funcName,Node->VidMem.line);
#endif
    ...
    else
    {
        ...
        if((Node->Virtual.magic == 0xDEADDEAD) || (Node->Virtual.magic != 0) || (Node->Virtual.kernel != Kernel))
        {
			gcmkPRINT("Virtual Node:%p lock Invalid magic :%x, Mkernel:%p, Kernel:%p\n"
                    ,Node, Node->Virtual.magic, Node->Virtual.kernel, Kernel);
            gcmkONERROR(gcvSTATUS_INVALID_OBJECT);
        }
        ...
        if (Node->Virtual.lockeds[Kernel->core] ++ == 0)
        {
            ...
            {
                gcmkONERROR(
                    gckOS_GetPmemPhysicalAddress(os,
                                                Node->Virtual.physical,
                                                &Node->Virtual.addresses[Kernel->core]));
            }
            else if(needMapping == gcvFALSE)
            {
                /* Get physical address directly */
                gcmkONERROR(gckOS_GetPhysicalAddressByHandle(os,
                            Node->Virtual.logical,
                            Node->Virtual.physical,
                            &Node->Virtual.addresses[Kernel->core]));
            }
...
#endif
#if !ALLDYNAMIC || (MRVL_VIDEO_MEMORY_USE_TYPE == gcdMEM_TYPE_NONE)
            else
#endif
            {
#if gcdENABLE_VG
...
#if ALLDYNAMIC
        trace_printk("%d@Vaddr0x%x,type:%s,%s,%d\n",(gctUINT32)Node->Virtual.bytes,Node->Virtual.addresses[Kernel->core],(Node->Virtual.bPmem)?"ion":(Node->Virtual.contiguous)?"cont":"N-cont",Node->Virtual.funcName,Node->Virtual.line);
#endif
        /* Return hardware address. */
        *Address = Node->Virtual.addresses[Kernel->core];
        gcmkONERROR(gckOS_SetGCPhysByMdl(os, Node->Virtual.physical, *Address));
