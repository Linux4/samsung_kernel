...
struct _pulseEater_frequency_table majorCoreFreq_table[] =
{
    [0] = {
            .index     = 0,
            .hwPeriod  = 0xD,
...
struct _pulseEater_frequency_table TwoDCoreFreq_table[] =
{
    [0] = {
            .index     = 0,
            .hwPeriod  = 0xD,
    ...
    )
{
    ...
    */
...
#else
            /* Chip doesn't has minor features register 3. */
            Identity->chipMinorFeatures3 = 0;
...
#else
            /* Chip doesn't has minor features register 4. */
            Identity->chipMinorFeatures4 = 0;
    ...
    if(printingChipInfo)
    {
        /* Print out current chip informations. */
        ...
        gckOS_Print("  chipMinorFeatures1     = 0x%08X\n", Identity->chipMinorFeatures1);
        gckOS_Print("  chipMinorFeatures2     = 0x%08X\n", Identity->chipMinorFeatures2);
        gckOS_Print("  chipMinorFeatures3     = 0x%08X\n", Identity->chipMinorFeatures3);
        gckOS_Print("  chipMinorFeatures4     = 0x%08X\n", Identity->chipMinorFeatures4);
        gckOS_Print("  chipMinorFeatures5     = 0x%08X\n", Identity->chipMinorFeatures5);
    }
    ...
    */
	...
	streamCount        = (((((gctUINT32)(specs)) >> (0 ? 3:0)) &((gctUINT32) ((((1 ? 3:0) - (0 ? 3:0) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 3:0) - (0 ? 3:0) + 1)))))));
        ...
        if(printingChipInfo)
        {
            gckOS_Print("  gcChipSpecs            = 0x%08x\n", specs);
            gckOS_Print("  gcChipSpecs2           = 0x%08x\n", specs2);
            gckOS_Print("  gcChipSpecs3           = 0x%08x\n", specs3);
        }
    }
    ...
    if(printingChipInfo)
    {
        gckOS_Print("  streamCount            = %d\n", Identity->streamCount);
        gckOS_Print("  registerMax            = %d\n", Identity->registerMax);
        gckOS_Print("  threadCount            = %d\n", Identity->threadCount);
        gckOS_Print("  shaderCoreCount        = %d\n", Identity->shaderCoreCount);
        gckOS_Print("  vertexCacheSize        = %d\n", Identity->vertexCacheSize);
        gckOS_Print("  vertexOutputBufferSize = %d\n", Identity->vertexOutputBufferSize);
        gckOS_Print("  pixelPipes             = %d\n", Identity->pixelPipes);
        gckOS_Print("  instructionCount       = %d\n", Identity->instructionCount);
        gckOS_Print("  numConstants           = %d\n", Identity->numConstants);
        gckOS_Print("  bufferSize             = %d\n", Identity->bufferSize);
        gckOS_Print("  varyingsCount          = %d\n", Identity->varyingsCount);
        if (Core == gcvCORE_MAJOR)
        {
            printedChipInfo3D = gcvTRUE;
        }
        else
        {
            if (Core == gcvCORE_2D)
            {
                printedChipInfo2D = gcvTRUE;
            }
            else
            {
                printedChipInfoVG = gcvTRUE;
            }
        }
    }
...
gceSTATUS
_fetchPulsedata(gckHARDWARE Hardware)
{
    gckPulseEaterDB db = gcvNULL;
    gctUINT32 index = 0;
    gctUINT32 curTime, pulseCount[2], i = 0;
    gcePulseEaterDomain domain = gcvPulse_Core;
    gctUINT64 tick;
    gceSTATUS status = gcvSTATUS_OK;

    db = Hardware->pulseEaterDB[domain];
    ...
    for(; domain < 2; domain++)
    {
        ...
        /*start polling*/
        if(db->startTime != 0 && db->startIndex == ~0)
        {
            db->startIndex = index;
        }
        /*Count all pulse after one cycle
            ([0,PULSE_EATER_COUNT-1] get busyRatio in ahead records*/
        else if(db->startIndex == index)
        {
            gctUINT32 tmp;

            if(db->moreRound)
            {
                tmp = db->timeStamp - db->timeStampInLastRound;
            }
            else
            {
                tmp = db->timeStamp- db->startTime;
            }

            gckHARDWARE_QueryPulseEaterIdleProfile(Hardware, 0, &tmp, domain);
            db->moreRound = gcvTRUE;
            db->timeStampInLastRound = db->timeStamp;
        }

        /*record the time stamp && record 0-3 pulse counter*/
        for(i = 0; i < gcmCOUNTOF(db->data[index].pulseCount); i++)
        {
            gctUINT8 tmp = 0;

            tmp = (pulseCount[domain] & (0xFF000000 >> i*8))>>(gcmCOUNTOF(db->data[index].pulseCount) - i - 1)*8;

            if ((gctINT8)tmp <= 64 && (gctINT8)tmp >= 0)
            {
                db->data[index].pulseCount[i] = tmp;
            }
            else
            {
                db->data[index].pulseCount[i] = 0;
            }
        }
        db->data[index].freq     = Hardware->freq;
        db->data[index].sfPeriod = Hardware->sfPeriod;
        db->data[index].hwMs     = Hardware->hwMs;
        db->data[index].time     = curTime;
        db->timeStamp            = curTime;

        /*update index*/
        db->lastIndex = gckMATH_ModuloInt((index + 1), gcmCOUNTOF(db->data));

        //gcmkPRINT("domain: %d, index: %d, sfPeriod: %d, PulseC: 0x%08x", domain, index, db->data[index].sfPeriod, pulseCount[domain]);
    }
...
_CountPulseRecord(
    IN gceCORE Core,
    IN gckPulseEaterDB DB,
    IN gctUINT32 Start,
    IN gctUINT32 End,
    OUT gctUINT32_PTR sfPeriod,
    OUT gctUINT32_PTR totalTime,
    OUT gctUINT32_PTR pulseCount)
{
    gctINT i, j = 0;
    pulseEaterFTL freq_table =gcvNULL;
    gctUINT32 indexFreq = 0;

    for(i = Start; i < End; i++)
    {
        if(Core == gcvCORE_MAJOR)
        {
            freq_table = majorCoreFreq_table;
        }
        else
        {
            freq_table = TwoDCoreFreq_table;
        }

        /* Count frequency index 0->156M; 1->312M; 2->416M; 3->624M*/
        while(gcvTRUE)
        {
            if(freq_table[j].frequency != ~0)
            {
                if(DB->data[i].freq == freq_table[j].frequency)
                {
                    break;
                }
                j++;
        ...
        indexFreq = j;
        sfPeriod[indexFreq] = DB->data[i].sfPeriod;

        /* count total time*/
        totalTime[indexFreq] += DB->data[i].sfPeriod;

        /* Count all pulse*/
        {
            gctINT k = 0;

            while(k < gcmCOUNTOF(DB->data[i].pulseCount))
            {
                /* Get All pulseCounts in one record*/
                pulseCount[indexFreq] += DB->data[i].pulseCount[k];
                k++;
            }
        }
    }
    ...
    IN gceCORE Core)
{
    gceSTATUS status = gcvSTATUS_OK;
    gctINT32 i = 0;
    gctUINT32 pulseCount[4], totalTime[4], busyRatio[4], sfPeriod[4];

    /* Initialize variables*/
    while(i < gcmCOUNTOF(pulseCount))
    {
        pulseCount[i] = 0;
        totalTime[i]  = 0;
        busyRatio[i]  = 0;
    ...
    if(DB->startIndex == DB->lastIndex)
    {
        _CountPulseRecord(Core, DB, 0, PULSE_EATER_COUNT - 1,
                          sfPeriod, totalTime, pulseCount);
    }
    else if(DB->startIndex < DB->lastIndex)
    {
        _CountPulseRecord(Core, DB, DB->startIndex, DB->lastIndex - 1,
                          sfPeriod, totalTime, pulseCount);
    }
    else
    {
        _CountPulseRecord(Core, DB, 0, DB->lastIndex - 1,
                          sfPeriod, totalTime, pulseCount);
        _CountPulseRecord(Core, DB, DB->startIndex, PULSE_EATER_COUNT - 1,
                          sfPeriod, totalTime, pulseCount);
    }

    //gcmkPRINT("Domain: 0x%08x , StartTime: %d, StartIndex: %d, EndIndex: %d, ", DB, DB->startTime, DB->startIndex, DB->lastIndex);

    i = 0;
    while(i < gcmCOUNTOF(DB->totalTime))
    {
        /*add it to global variable*/
        DB->totalTime[i] += totalTime[i];

        //gcmkPRINT("%d, TotalTime: %d, pTotal: %d, PulseCount: %d", i, DB->totalTime[i], totalTime[i], pulseCount[i]);

        if(pulseCount[i] != 0 && totalTime[i] > 0)
        {
            /*        busyRatio = (100 * pulseCountAll/(64*4*sampleTotal))%TimeGap + [busyRatio before ahead(200) records]  */
            /*        sampleTotal = TimeGap/samplingPeriod*/
            busyRatio[i] = (100 * pulseCount[i] /(64 * gcmCOUNTOF(DB->data[i].pulseCount) * (totalTime[i]/sfPeriod[i]))) * (totalTime[i]/100);
            DB->busyRatio[i] += busyRatio[i];

            //gcmkPRINT("[Liang] %d: PtotalT:%d, TotalTime: %d, PbusyRatio: %d, busyRatio:%d\n", i, totalTime[i], DB->totalTime[i], busyRatio[i], DB->busyRatio[i]);
        }
        i++;
    }
    ...
    else
    {
        ...
        ** for GC400 family.  Next time - TELL ME!!! */
        if (((chipModel & 0xFF00) == 0x0400)
          && (chipModel != 0x0420))
        {
            chipModel = (gceCHIPMODEL) (chipModel & 0x0400);
        }
#else
        gcmkONERROR(gcvSTATUS_NOT_SUPPORTED);
    ...
    )
{
    ...
    gcmkONERROR(_ReadChipModel(Hardware->os,
                               Hardware->core,
                               &chipModel));
    ...
    )
{
    ...
    gcmkONERROR(gckOS_Allocate(Os,
                               ...
                               &pointer));
    gcmkONERROR(gckOS_ZeroMemory(pointer,
                                 gcmSIZEOF(struct _gckHARDWARE)));
	...
	#else
    hardware->extraEventStates = (chipModel >= gcv800);
    ...
    hardware->lastPowerState  = gcvPOWER_ON;

    gcmkONERROR(gckOS_AtomConstruct(Os, &hardware->clockState));
    gcmkONERROR(gckOS_AtomSet(Os, hardware->clockState, gcvTRUE));
    gcmkONERROR(gckOS_AtomConstruct(Os, &hardware->powerState));
    ...
    hardware->enablePowerOffTimeout = gcvTRUE;
    ...
    hardware->devObj.id       = (gctINT32)Core;
    hardware->devObj.kobj     = gcvNULL;
    gckOS_GetFreqTablePointer(Os, &(hardware->devObj.freqt), Core);
# if MRVL_CONFIG_SHADER_CLK_CONTROL
    if(has_feat_shader_indept_dfc() && Core == gcvCORE_MAJOR)
    {
        hardware->devShObj.id   = (gctINT32)gcvCORE_SH;
        hardware->devShObj.kobj = gcvNULL;
        gckOS_GetFreqTablePointer(Os, &(hardware->devShObj.freqt), gcvCORE_SH);
    }
# endif
    ...
    {
        gctUINT i = 0;
        for(; i< gcmCOUNTOF(hardware->pulseEaterDB); i++)
        {
            gctINT j = 0;
            gcmkONERROR(gckOS_Allocate(Os,
                                       gcmSIZEOF(struct _gckPulseEaterDB),
                                       &pointer));
            hardware->pulseEaterDB[i] = (gckPulseEaterDB)pointer;

            hardware->pulseEaterDB[i]->lastIndex = 0;
            hardware->pulseEaterDB[i]->timeStamp = 0;
            hardware->pulseEaterDB[i]->startTime = 0;
            hardware->pulseEaterDB[i]->moreRound = gcvFALSE;
            hardware->pulseEaterDB[i]->timeStampInLastRound = 0;
            hardware->pulseEaterDB[i]->startIndex = ~0;
            while (j < gcmCOUNTOF(hardware->pulseEaterDB[i]->busyRatio))
            {
                hardware->pulseEaterDB[i]->busyRatio[j] = 0;
                hardware->pulseEaterDB[i]->totalTime[j] = 0;
                j++;
            }
        }
...
OnError:
    ...
    if (hardware != gcvNULL)
    {
        ...
        if (hardware->clockState != gcvNULL)
        {
            gcmkVERIFY_OK(gckOS_AtomDestroy(Os, hardware->clockState));
        }

        if(has_feat_pulse_eater_profiler())
        {
            if (hardware->pulseEaterMutex != gcvNULL)
            {
                /*destroy the PulseEater mutex*/
                gcmkVERIFY_OK(gckOS_DeleteMutex(Os, hardware->pulseEaterMutex));
                hardware->pulseEaterMutex = gcvNULL;
            }

            if (hardware->pulseEaterTimer != gcvNULL)
            {
                /* Destroy the PulseEater timer */
                gcmkVERIFY_OK(gckOS_StopTimer(Os, hardware->pulseEaterTimer));
                gcmkVERIFY_OK(gckOS_DestroyTimer(Os, hardware->pulseEaterTimer));
                hardware->pulseEaterTimer = gcvNULL;
            }
        }

        if (hardware->powerState != gcvNULL)
        {
            gcmkVERIFY_OK(gckOS_AtomDestroy(Os, hardware->powerState));
        }
    ...
    )
{
    ...
    if(has_feat_pulse_eater_profiler())
    {
        gcmkVERIFY_OK(gckOS_DeleteMutex(Hardware->os, Hardware->pulseEaterMutex));
        gcmkVERIFY_OK(gckOS_StopTimer(Hardware->os, Hardware->pulseEaterTimer));
        gcmkVERIFY_OK(gckOS_DestroyTimer(Hardware->os, Hardware->pulseEaterTimer));

        {
            ...
            for(; i<2; i++)
            {
                gcmkONERROR(gcmkOS_SAFE_FREE(Hardware->os, Hardware->pulseEaterDB[i]));
            }
        }
    }

    gcmkVERIFY_OK(gckOS_AtomDestroy(Hardware->os, Hardware->clockState));
    gcmkVERIFY_OK(gckOS_AtomDestroy(Hardware->os, Hardware->powerState));
    ...
    )
{
    ...
    if (Hardware->identity.chipModel >= gcv400
        ...
        && !(Hardware->identity.chipModel == 0x2200 && Hardware->identity.chipRevision == 0x5244))
    {
        if (regPMC == 0)
        {
        gcmkONERROR(
    ...
    )
{
    ...
    {
        if(has_workaround_fe_read_cmd_end())
        {
    ...
    )
{
    ...
    if (InterruptValid)
    {
        ...
        {
            gcmkONERROR(
                gckOS_DirectReadRegisterByCoreId(Hardware->os,
                                           Hardware->core,
                                           CoreId,
                                           0x00010,
                                           &data));
    ...
    )
{
    ...
    gcmCONFIGUREUNIFORMS(Hardware->identity.chipModel,
                         Hardware->identity.chipRevision,
                         Hardware->identity.numConstants,
                         unifiedConst,
                         vsConstBase,
                         psConstBase,
                         vsConstMax,
    ...
    )
{
    ...
    gceCHIPPOWERSTATE tmpState = State;
...
#endif
        case gcvPOWER_ON_HINT:
        State = gcvPOWER_ON;
        hint  = gcvTRUE;
        break;

        case gcvPOWER_OFF_HINT:
        State = gcvPOWER_OFF;
        hint  = gcvTRUE;
        break;
    ...
    if (broadcast)
    {
        ...
        status = gckOS_AcquireRecMutex(os, Hardware->recMutexPower, 0);
        ...
        if (status == gcvSTATUS_TIMEOUT)
        {
            ...
            else
            {
                ...
                gcmkONERROR(gckOS_AcquireRecMutex(os,
    ...
    /* ## Specific patch ##
        By pass power status transition from OFF to SUSPEND/IDLE.
        galcore deamon and thread rountine may not be synced in some cases.
        Such as that galcore deamon puts GPU into OFF status while thread
        routine is still running and then try to SUSPEND GPU after its
        completion, which is meaningless but will cause GPU hang due to
        reading register when GPU's internal clock is disabled.
        It also suits for IDLE case which casue GPU running at low frequency.
    */
    if(Hardware->chipPowerState == gcvPOWER_OFF &&
        (State == gcvPOWER_SUSPEND || State == gcvPOWER_IDLE))
    {
        State = gcvPOWER_OFF;
    }

    /*
        The power state transition comes from
        suspend/resume callback
    */
    if(tmpState == gcvPOWER_ON_HINT)
    {
        /*
            Skip power on in resume callback if previouly power
            has already turned off before entering suspend.
        */
        if(Hardware->lastPowerState == gcvPOWER_OFF)
        {
            /* Release the power mutex. */
            gcmkONERROR(gckOS_ReleaseRecMutex(os, Hardware->recMutexPower));
    ...
    else if(tmpState == gcvPOWER_OFF_HINT)
    {
        /* Should be suspend callback, store power state */
        Hardware->lastPowerState = Hardware->chipPowerState;
    }
    /* ================================= */
    ...
    if(has_feat_pulse_eater_profiler() &&
       ...
       Hardware->core == gcvCORE_MAJOR)
    {
        /*polling register before clkoff to avoid
                no data in Sample Window*/
        _fetchPulsedata(Hardware);
        gcmkONERROR(
            gckOS_StopTimer(Hardware->os,
            Hardware->pulseEaterTimer));
    }

#if MRVL_CONFIG_ENABLE_GPUFREQ
    /* suspend GPUFreq governor. */
    if(flag & gcvPOWER_FLAG_POWER_OFF)
    {
        gcmkONERROR(gckOS_GPUFreqNotifierCallChain(
                        os,
                        GPUFREQ_GPU_EVENT_POWER_OFF,
                        (gctPOINTER) &Hardware->devObj));

#if MRVL_CONFIG_SHADER_CLK_CONTROL
        if(has_feat_shader_indept_dfc() && Hardware->core == gcvCORE_MAJOR)
        {
            gcmkONERROR(gckOS_GPUFreqNotifierCallChain(
                            os,
                            GPUFREQ_GPU_EVENT_POWER_OFF,
...
#if MRVL_ENABLE_GC_POWER_CLOCK
                gcmkONERROR(gckOS_SetGPUPowerOffMRVL(os, Hardware->core, gcvTRUE, gcvTRUE, hint));
#else
...
#if MRVL_ENABLE_GC_POWER_CLOCK
                gcmkONERROR(gckOS_SetGPUPowerOnMRVL(os, Hardware->core, gcvTRUE, gcvTRUE, hint));
#else
                gcmkONERROR(gckOS_SetGPUPower(os, Hardware->core, gcvTRUE, gcvTRUE));
    ...
    if (flag & gcvPOWER_FLAG_POWER_OFF)
    {
        gcmkONERROR(gckOS_AtomGet(os, Hardware->clockState, &clockStateValue));
        if (clockStateValue == gcvFALSE)
        {
            /* Turn on the GPU clock. */
#if MRVL_ENABLE_GC_POWER_CLOCK
            gcmkONERROR(
                    gckOS_SetGPUPowerOnMRVL(os,
                        Hardware->core,
                        gcvTRUE,
                        gcvFALSE,
                        hint));
    ...
    gcmkONERROR(gckOS_AtomGet(os, Hardware->clockState, &clockStateValue));
    gcmkONERROR(gckOS_AtomGet(os, Hardware->powerState, &powerStateValue));
    if (clockStateValue && powerStateValue)
    {
        /* Soft reset GPU after power ON. */
        ...
        if(gckHARDWARE_IsFeatureAvailable(Hardware, gcvFEATURE_DYNAMIC_FREQUENCY_SCALING) != gcvTRUE)
        {
            ...
            /*
                Register access will cause system hang when clock disabled by
                CLK3D_DIS/CLK2D_DIS bit. According to 4x driver, Since clock
                gating internal 2D/3D clocks only save little more power on the
                basis of scaling internal frequency to 1/64, 2D_DIS/3D_DIS bits
                would be kept untouched.
            */
            if (State == gcvPOWER_SUSPEND || State == gcvPOWER_OFF)
            {
                 clock &= ~3U;
            }
    ...
    if (flag & gcvPOWER_FLAG_DELAY)
    {
        ...
        ** power-off or suspend state. */
        gcmkONERROR(gckOS_Udelay(os, gcdPOWER_CONTROL_DELAY));
    }
    ...
    if (flag & (gcvPOWER_FLAG_POWER_OFF | gcvPOWER_FLAG_CLOCK_OFF))
    {
        ...
        clockStateValue = (flag & gcvPOWER_FLAG_CLOCK_OFF) ? gcvFALSE
                                                           : gcvTRUE;
        gcmkONERROR(gckOS_AtomSet(os, Hardware->clockState, clockStateValue));
...
#if MRVL_CONFIG_ENABLE_GPUFREQ
    /* resume GPUFreq governor. */
    if(flag & gcvPOWER_FLAG_INITIALIZE)
    {
        gcmkONERROR(gckOS_GPUFreqNotifierCallChain(
                        os,
                        GPUFREQ_GPU_EVENT_POWER_ON,
                        (gctPOINTER) &Hardware->devObj));
    ...
    if(has_feat_pulse_eater_profiler() &&
        (flag & gcvPOWER_FLAG_CLOCK_ON) &&
        Hardware->core == gcvCORE_MAJOR)
    {
        gcmkONERROR(
            gckOS_StartTimer(Hardware->os,
            Hardware->pulseEaterTimer,
            Hardware->sfPeriod));
    }
    ...
    )
{
    ...
    gcmkVERIFY_OK(
        gckOS_AcquireRecMutex(Hardware->os, Hardware->recMutexPower, gcvINFINITE));

    Hardware->powerManagement = PowerManagement;

    gcmkVERIFY_OK(gckOS_ReleaseRecMutex(Hardware->os, Hardware->recMutexPower));
    ...
    )
{
    ...
    gcmkVERIFY(gckOS_ReleaseRecMutex(Hardware->os, Hardware->recMutexPower));
...
)
{
    ...
    currentPwrState = Hardware->chipPowerState;
    ...
    if (currentPwrState == gcvPOWER_IDLE || currentPwrState == gcvPOWER_SUSPEND)
    {
...
gceSTATUS
gckHARDWARE_SetPowerOffTimeoutEnable(
    IN gckHARDWARE  Hardware,
    IN gctBOOL      Enable
)
{
    ...
    Hardware->enablePowerOffTimeout = Enable;

    if(Enable == gcvTRUE)
    {
        _TryToPowerOffGPU(Hardware);
    }
    ...
    )
{
    ...
    /* Verify the arguments. */
    gcmkVERIFY_OBJECT(Hardware, gcvOBJ_HARDWARE);
    gcmkVERIFY_ARGUMENT(RegData != gcvNULL);
    gcmkVERIFY_ARGUMENT(IsIdle != gcvNULL);

    /* We are idle when the power is OFF. */
    if (Hardware->chipPowerState == gcvPOWER_OFF)
    {
        *RegData = 0x0;
        *RegData3D1 = 0x0;
...
#define gcmkREAD_DEBUG_REGISTER(control, block, index, data) \
	...
	((((gctUINT32)(0)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? \GC_DEBUG_CONTROL##control_\block) - (0 ? \GC_DEBUG_CONTROL##control_\block) + 1) == 32) ? ~0 : (~(~0 << ((1 ? \GC_DEBUG_CONTROL##control_\block) - (0 ? \GC_DEBUG_CONTROL##control_\block) + 1))))))) << (0 ? \GC_DEBUG_CONTROL##control_\block))) | (((gctUINT32)((gctUINT32)(\index)&((gctUINT32) ((((1 ? \GC_DEBUG_CONTROL##control_\block) - (0 ? \GC_DEBUG_CONTROL##control_\block) + 1) == 32) ? ~0 : (~(~0 << ((1 ? \GC_DEBUG_CONTROL##control_\block) - (0 ? \GC_DEBUG_CONTROL##control_\block) + 1))))))) << (0 ? \GC_DEBUG_CONTROL##control_\block))))); \
	    gcmkONERROR( \
        gckOS_ReadRegisterEx(Hardware->os, \
                             Hardware->core, \
...
#define gcmkRESET_DEBUG_REGISTER(control, block) \
    gcmkONERROR( \
        gckOS_WriteRegisterEx(Hardware->os, \
                              Hardware->core, \
                              GC_DEBUG_CONTROL##control##_Address, \
	((((gctUINT32)(0)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? \GC_DEBUG_CONTROL##control_\block) - (0 ? \GC_DEBUG_CONTROL##control_\block) + 1) == 32) ? ~0 : (~(~0 << ((1 ? \GC_DEBUG_CONTROL##control_\block) - (0 ? \GC_DEBUG_CONTROL##control_\block) + 1))))))) << (0 ? \GC_DEBUG_CONTROL##control_\block))) | (((gctUINT32)((gctUINT32)(\15)&((gctUINT32) ((((1 ? \GC_DEBUG_CONTROL##control_\block) - (0 ? \GC_DEBUG_CONTROL##control_\block) + 1) == 32) ? ~0 : (~(~0 << ((1 ? \GC_DEBUG_CONTROL##control_\block) - (0 ? \GC_DEBUG_CONTROL##control_\block) + 1))))))) << (0 ? \GC_DEBUG_CONTROL##control_\block))))); \
	    gcmkONERROR( \
        gckOS_WriteRegisterEx(Hardware->os, \
                              Hardware->core, \
                              GC_DEBUG_CONTROL##control##_Address, \
	((((gctUINT32)(0)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? \GC_DEBUG_CONTROL##control_\block) - (0 ? \GC_DEBUG_CONTROL##control_\block) + 1) == 32) ? ~0 : (~(~0 << ((1 ? \GC_DEBUG_CONTROL##control_\block) - (0 ? \GC_DEBUG_CONTROL##control_\block) + 1))))))) << (0 ? \GC_DEBUG_CONTROL##control_\block))) | (((gctUINT32)((gctUINT32)(\0)&((gctUINT32) ((((1 ? \GC_DEBUG_CONTROL##control_\block) - (0 ? \GC_DEBUG_CONTROL##control_\block) + 1) == 32) ? ~0 : (~(~0 << ((1 ? \GC_DEBUG_CONTROL##control_\block) - (0 ? \GC_DEBUG_CONTROL##control_\block) + 1))))))) << (0 ? \GC_DEBUG_CONTROL##control_\block)))))
    ...
    )
{
	...
	if (((((((gctUINT32)(control)) >> (0 ? 16:16)) &((gctUINT32) ((((1 ? 16:16) - (0 ? 16:16) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 16:16) - (0 ? 16:16) + 1))))))) == 0)||  ((((((gctUINT32)(control)) >> (0 ? 17:17)) &((gctUINT32) ((((1 ? 17:17) - (0 ? 17:17) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 17:17) - (0 ? 17:17) + 1))))))) == 0))
	        {
            continue;
        }
	...
	data1 = ((((gctUINT32)(0x01590880)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? 18:18) - (0 ? 18:18) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 18:18) - (0 ? 18:18) + 1))))))) << (0 ? 18:18))) | (((gctUINT32)((gctUINT32)(1)&((gctUINT32) ((((1 ? 18:18) - (0 ? 18:18) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 18:18) - (0 ? 18:18) + 1))))))) << (0 ? 18:18)));
	
            /*Disable PE Stall*/
	data1 = ((((gctUINT32)(data1)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? 19:19) - (0 ? 19:19) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 19:19) - (0 ? 19:19) + 1))))))) << (0 ? 19:19))) | (((gctUINT32)((gctUINT32)(0)&((gctUINT32) ((((1 ? 19:19) - (0 ? 19:19) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 19:19) - (0 ? 19:19) + 1))))))) << (0 ? 19:19)));
	
            /*Disable fast start*/
	data1 = ((((gctUINT32)(data1)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? 22:22) - (0 ? 22:22) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 22:22) - (0 ? 22:22) + 1))))))) << (0 ? 22:22))) | (((gctUINT32)((gctUINT32)(0)&((gctUINT32) ((((1 ? 22:22) - (0 ? 22:22) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 22:22) - (0 ? 22:22) + 1))))))) << (0 ? 22:22)));
	
            /*Enable only idle*/
	data1 = ((((gctUINT32)(data1)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? 23:23) - (0 ? 23:23) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 23:23) - (0 ? 23:23) + 1))))))) << (0 ? 23:23))) | (((gctUINT32)((gctUINT32)(1)&((gctUINT32) ((((1 ? 23:23) - (0 ? 23:23) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 23:23) - (0 ? 23:23) + 1))))))) << (0 ? 23:23)));
	
            /* Update period*/
	data = ((((gctUINT32)(data1)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? 15:8) - (0 ? 15:8) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 15:8) - (0 ? 15:8) + 1))))))) << (0 ? 15:8))) | (((gctUINT32)((gctUINT32)(Hardware->hwPeriod)&((gctUINT32) ((((1 ? 15:8) - (0 ? 15:8) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 15:8) - (0 ? 15:8) + 1))))))) << (0 ? 15:8)));
	
            /* Enable shader pulse eater*/
	data = ((((gctUINT32)(data)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? 16:16) - (0 ? 16:16) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 16:16) - (0 ? 16:16) + 1))))))) << (0 ? 16:16))) | (((gctUINT32)((gctUINT32)(1)&((gctUINT32) ((((1 ? 16:16) - (0 ? 16:16) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 16:16) - (0 ? 16:16) + 1))))))) << (0 ? 16:16)));
    ...
    )
{
    ...
    gcmkONERROR(gckHARDWARE_Execute(Hardware, Hardware->kernel->restoreAddress, 16, gcvFALSE));
...
#if 1 
	/*available = ((((gctUINT32) (Hardware->identity.chipMinorFeatures2)) >> (0 ? 9:9) & ((gctUINT32) ((((1 ? 9:9) - (0 ? 9:9) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 9:9) - (0 ? 9:9) + 1)))))) == (0x1 & ((gctUINT32) ((((1 ? 9:9) - (0 ? 9:9) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 9:9) - (0 ? 9:9) + 1)))))));*/
	        available = gcvFALSE;
#else
        available = gcvFALSE;
    ...
    )
{
    ...
    gcmkPRINT_N(24, "GPU[%d](ChipModel=0x%x ChipRevision=0x%x ChipPowerStates=%d, ClkRefCntEx: %d, IntClk: %d):\n",
                core,
                Hardware->identity.chipModel,
                Hardware->identity.chipRevision,
                Hardware->chipPowerState,
                Hardware->refExtClock,
                Hardware->clk2D3D_Enable);
    ...
    IN gcePulseEaterDomain Domain)
{
    if(has_feat_pulse_eater_profiler())
    {
        ...
        gckPulseEaterDB db;
        gctUINT32 currentTick, timeGap, lastIndex;
        gceSTATUS status = gcvSTATUS_OK;

        gcmkHEADER_ARG("Hardware=0x%x TimeGap=0x%x", Hardware, TimeGap);
        ...
        /*Case A
            ...
            |---------lastRecord---------currTick
                                             |-timeGap--|*/
        if((gctINT)(currentTick - db->timeStamp) >= (gctINT)timeGap)
        {
            status = gcvSTATUS_OK;
        }
        /* Case B
        ...
        /* Case C
            ...
            |--------------------------|-|
                                                       lastRecord
         ...|----------- timeGap   --------|*/
        else if(((gctINT)(currentTick -db->timeStamp) >= 0) || /*Case B*/
                ((gctINT)(currentTick -db->timeStamp) < 0 || currentTick == 0)) /*Case C*/
        {
            status = _GetPulseEaterIdleTime(currentTick, timeGap, db, Hardware->core);
        }
    ...
    )
{
    if(has_feat_pulse_eater_profiler())
    {
        gctINT32 swPeriod = 0, hwPeriod = 0, i = 0;
        gctUINT8 hwMs = 0;
        gctUINT32 data;
        pulseEaterFTL freq_table = gcvNULL;
        gceSTATUS status = gcvSTATUS_OK;
        ...
        while(gcvTRUE)
        {
            if(freq_table[i].frequency != ~0)
            {
                if(Freq == freq_table[i].frequency)
                {
                    hwPeriod = freq_table[i].hwPeriod;
                    swPeriod = freq_table[i].swPeriod;
                    hwMs     = freq_table[i].hwMs;
                    break;
                }
                i++;
            }
        ...
        Hardware->hwPeriod  = hwPeriod;
        Hardware->sfPeriod  = swPeriod;
        Hardware->hwMs      = hwMs;
        Hardware->freq      = Freq;
	...
	data = ((((gctUINT32)(data)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? 15:8) - (0 ? 15:8) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 15:8) - (0 ? 15:8) + 1))))))) << (0 ? 15:8))) | (((gctUINT32)((gctUINT32)(Hardware->hwPeriod)&((gctUINT32) ((((1 ? 15:8) - (0 ? 15:8) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 15:8) - (0 ? 15:8) + 1))))))) << (0 ? 15:8)));
