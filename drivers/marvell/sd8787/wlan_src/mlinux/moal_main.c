...
/** Driver version */
char driver_version[] =
	"SD8787-%s-M" KERN_VERSION "14" MLAN_RELEASE_VERSION
	"-GPL" "-(" "FP" FPNUM ")"
#ifdef	DEBUG_LEVEL2
	"-dbg"
#endif
...
#endif /* DEBUG_LEVEL2 */
#ifdef ENABLE_FW_MEM_DUMP
t_u32 drvdbg = DEFAULT_DEBUG_MASK | MFW_D;
#else
		      ...
		      mlan_buffer * pmbuf, struct sk_buff *skb)
{
	...
	ENTER();
	/* skb->priority values from 256->263 are magic values to * directly
	   indicate a specific 802.1d priority.  This is used * to allow 802.1d
	   priority to be passed directly in from VLAN * tags, etc. */
...
#ifdef CONFIG_MACH_LT02LGT /* For LGU DSCP WMM mapping */
			if ((SKB_TOS(skb) >> 2) == 0x2e)
				tid = 7;
#endif
			PRINTM(MDAT_D,
...
woal_dump_drv_info(moal_handle * phandle)
{
	...
	pfile = filp_open("/data/log/file_drv_info", O_CREAT | O_RDWR, 0644);
	if (IS_ERR(pfile)) {
...
woal_dump_firmware_info(moal_handle * phandle)
{
	...
	if (IS_ERR(pfileITCM)) {
		...
		PRINTM(MMSG,
		       "Now Start to save all the outputs to the /data/log directory: %u.%06u, please wait...\n",
		       sec, usec);
		pfileDTCM =
