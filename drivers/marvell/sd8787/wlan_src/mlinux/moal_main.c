...
/** Driver version */
char driver_version[] =
	"SD8777-%s-C" KERN_VERSION "14" MLAN_RELEASE_VERSION
	"-GPL" "-(" "FP" FPNUM ")"
#ifdef	DEBUG_LEVEL2
	"-dbg"
#endif
		      ...
		      mlan_buffer * pmbuf, struct sk_buff *skb)
{
	...
	/* Record the current time the packet was queued; used to determine the
	   amount of time the packet was queued in the driver before it was sent
	   to the firmware.  The delay is then sent along with the packet to the
			...
			if (priv->media_connected == MFALSE)
				...
				if (priv->set_asynced_essid_flag == MTRUE) {
					...
					if (MLAN_STATUS_SUCCESS !=
						...
						memcpy(&priv->
						       prev_ssid_bssid.ssid,
						       &bss_info.ssid,
						       sizeof
						       (mlan_802_11_ssid));
						memcpy(&priv->
						       prev_ssid_bssid.bssid,
						       &bss_info.bssid,
						       MLAN_MAC_ADDR_LENGTH);
					}
					priv->set_asynced_essid_flag = MFALSE;
	...
	do {
		...
		for (tries = 0; tries < MAX_POLL_TRIES; tries++) {
			...
			if (ctrl_data != DEBUG_HOST_READY) {
				sdio_writeb(((struct sdio_mmc_card *)
					     phandle->card)->func,
					    DEBUG_HOST_READY,
					    DEBUG_DUMP_CTRL_REG, &ret);
				if (ret) {
		...
		for (i = 0; i < handle->priv_num; i++) {
			...
			if (GET_BSS_ROLE(handle->priv[i]) == MLAN_BSS_ROLE_STA) {
				...
				spin_lock_irqsave(&handle->
						  priv[i]->scan_req_lock,
						  flags);
				if (IS_STA_CFG80211(cfg80211_wext) &&
				    handle->priv[i]->scan_request) {
					cfg80211_scan_done(handle->
							   priv
							   [i]->scan_request,
							   MTRUE);
				...
				if (IS_STA_CFG80211(cfg80211_wext) &&
					...
					handle->priv[i]->bg_scan_reported =
						MFALSE;
					cfg80211_sched_scan_stopped(handle->priv
								    [i]->wdev->wiphy);
					handle->priv[i]->sched_scanning =
						MFALSE;
				}
