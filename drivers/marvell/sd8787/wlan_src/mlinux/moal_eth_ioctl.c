			    ...
			    t_u32 respbuflen)
{
	...
	while (begin) {
		...
		if (!opt || !end || !end[0]) {
			...
			woal_ascii2hex((t_u8 *) (sec->param.passphrase.psk.
						 pmk.pmk), end,
...
woal_priv_sdcmd52rw(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (!rw) {
		sdio_claim_host(((struct sdio_mmc_card *)priv->phandle->
				 card)->func);
		if (func)
			data = sdio_readb(((struct sdio_mmc_card *)
					   priv->phandle->card)->func, reg,
					  &ret);
		else
			data = sdio_f0_readb(((struct sdio_mmc_card *)
					      priv->phandle->card)->func, reg,
					     &ret);
		sdio_release_host(((struct sdio_mmc_card *)priv->
				   phandle->card)->func);
		if (ret) {
			PRINTM(MERROR,
		...
		sdio_claim_host(((struct sdio_mmc_card *)priv->phandle->
				 card)->func);
		if (func)
			sdio_writeb(((struct sdio_mmc_card *)priv->
				     phandle->card)->func, data, reg, &ret);
		else
			sdio_f0_writeb(((struct sdio_mmc_card *)priv->
					phandle->card)->func, data, reg, &ret);
		sdio_release_host(((struct sdio_mmc_card *)priv->
				   phandle->card)->func);
		if (ret) {
 ...
 */
static int
woal_priv_coalescing_status(moal_private * priv, t_u8 * respbuf,
			    t_u32 respbuflen)
{
	int header_len = 0, user_data_len = 0;
	int ret = 0, data = 0;
...
#define CMD_BUF_LEN   2048
	...
	if (copy_from_user(buf, (void *)(unsigned long)priv_cmd.buf, priv_cmd.total_len)) {
		ret = -EFAULT;
		goto done;
	}
	buf[CMD_BUF_LEN - 1] = '\0';
...
handled:
	...
	if (len > 0) {
		...
		if (priv_cmd.used_len <= priv_cmd.total_len) {
			memset((void *)(unsigned long)(priv_cmd.buf + priv_cmd.used_len), 0,
			       (size_t) (priv_cmd.total_len -
					 priv_cmd.used_len));
			if (copy_to_user((void *)(unsigned long)priv_cmd.buf, buf, priv_cmd.used_len)) {
				PRINTM(MERROR,
