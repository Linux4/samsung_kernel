			       ...
			       IN t_bool send_ioctl)
{
	...
	if ((misc->param.cust_ie.len == 0) ||
		...
		while (ioctl_len > 0) {
			...
			/* check whether IE length is valid */
			if (ie_data->ie_length > 0) {
				len = 0;
				for (i = 1; i < ie_data->ie_length;) {
					if (ie_data->ie_buffer[i] == 0) {
						PRINTM(MERROR,
						       "Invalid custom IE length: subelement ie len is 0\n");
						ret = MLAN_STATUS_FAILURE;
						goto done;
					}
					len += ie_data->ie_buffer[i] + 2;
					if (len < ie_data->ie_length)
						i = len + 1;
					else if (len == ie_data->ie_length)
						break;
					else {
						PRINTM(MERROR,
						       "Invalid custom IE length 0x%x\n",
						       ie_data->ie_length);
						ret = MLAN_STATUS_FAILURE;
						goto done;
					}
				}
				if (i >= ie_data->ie_length) {
					PRINTM(MERROR,
					       "Invalid custom IE length 0x%x\n",
					       ie_data->ie_length);
					ret = MLAN_STATUS_FAILURE;
