 ...
 *
 * $Id: cr_rstrt_req.c,v 1.393.4.11 2011/09/17 00:16:29 phargrov Exp $
 */
...
#endif
#if HAVE_2_ARG_PATH_LOOKUP
  #define cr_path_lookup(_n,_f,_d) kern_path_parent((_n),(_d))
#else
  #define cr_path_lookup(_n,_f,_d) path_lookup((_n),(_f),(_d))
#endif
...
cr_restore_all_files(cr_rstrt_proc_req_t *proc_req)
{
	...
	fdt = cr_fdtable(files);
	    ...
	    __set_close_on_exec(file_info.fd, fdt);
        } else {
	    __clear_close_on_exec(file_info.fd, fdt);
	}
	spin_unlock(&files->file_lock);
...
cr_chdir(const char *path)
{
    ...
    retval = kern_path(path,LOOKUP_FOLLOW|LOOKUP_DIRECTORY,&pathstruct);
    if (retval)
        goto out;

    nd.path = pathstruct;
    retval = cr_permission(pathstruct.dentry->d_inode,MAY_EXEC,&nd);
    if (retval)
        goto dput_and_out;
