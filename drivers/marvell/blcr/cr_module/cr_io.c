 ...
 *
 * $Id: cr_io.c,v 1.77.4.3 2009/06/26 18:19:51 phargrov Exp $
 */
...
cr_mknod(cr_errbuf_t *eb, struct nameidata *nd, const char *name, int mode, unsigned long unlinked_id)
{
    ...
    /* In kernel 3.4, user_path_create() is used to replace combination of kern_path_parent()
     * and lookup_create().
     */
    struct path path;
    dentry = user_path_create(AT_FDCWD, name, &path, 0);
    if (IS_ERR(dentry)) {
	err = PTR_ERR(dentry);
	CR_KTRACE_UNEXPECTED("Couldn't user_path_create for mknod %s.  err=%d.", name, err);
	cr_inode_unlock(path.dentry->d_inode);
	goto out_release;
    }
    ...
    switch (mode & S_IFMT) {
    case S_IFREG:
	err = vfs_create(path.dentry->d_inode, dentry, mode, nd);
	break;
    case S_IFIFO:
	err = cr_vfs_mknod(path.dentry->d_inode, dentry, path.mnt, mode, 0 /* ignored */);
	break;
    default:
	...
	(void)cr_vfs_unlink(path.dentry->d_inode, dentry, path.mnt);
	dput(dentry);
    }
    cr_inode_unlock(path.dentry->d_inode);
...
cr_link(cr_errbuf_t *eb, struct path *old_path, const char *name)
{
     ...
     */
    struct path path;
    retval = kern_path(name, LOOKUP_FOLLOW, &path);
    if (retval < 0) {
...
repeat: 
    ...
    if (start < max_fds) {
	newfd = find_next_zero_bit(fdt->open_fds,
				   max_fds, start);
    }
