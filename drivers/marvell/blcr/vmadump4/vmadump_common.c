 ...
 *
 * $Id: vmadump_common.c,v 1.86.4.8 2011/10/04 21:07:22 phargrov Exp $
 *
...
long vmad_remap(cr_rstrt_proc_req_t *ctx, unsigned long from_addr,
    ...
    long diff = (from_addr > to_addr) ? (from_addr - to_addr)
                                      : (to_addr - from_addr);
    if (diff < len) { /* Check for overlap case */
        unsigned long tmp_addr;
        /*CR_WARN_CTX(ctx, "vmad_remap using temporary");*/

        tmp_addr = do_mmap(NULL, 0, len, PROT_NONE, MAP_PRIVATE|MAP_ANONYMOUS, 0);
...
long vmadump_thaw_proc(cr_rstrt_proc_req_t *ctx,
    ...
    while(mm->mmap && mm->map_count) {
	map = mm->mmap;
	map_count = mm->map_count;
	if (map->vm_flags & VM_RESERVED) {
		mm->map_count --;
		continue;
	}
...
loff_t store_map(cr_chkpt_proc_req_t *ctx, struct file *file,
	...
	/*the vm segment is no read, no write, no execute, skip it */
	if (0x70 == (unsigned short)map->vm_flags){
		end = start;
	}
