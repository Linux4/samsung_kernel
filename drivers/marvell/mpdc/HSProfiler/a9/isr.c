...
/*
 * ARM A9 Hotspot ISR implementation
 */
...
static void set_ccnt_events(struct HSEventConfig *ec, struct pmu_registers_a9 *pmu_regs)
{
...
static void start_pmu(void *data)
{
	...
	for (i=0; i<g_ebs_settings_hs.eventNumber; i++)
	{
		...
		case COUNTER_A9_PMU_PMN4:
			set_pmn_events(4, &g_ebs_settings_hs.event[i], &pmu_regs);
			break;

		case COUNTER_A9_PMU_PMN5:
			set_pmn_events(5, &g_ebs_settings_hs.event[i], &pmu_regs);
			break;
...
struct hs_sampling_op_mach hs_sampling_op_a9 =
{
