...
extern int start_module_tracking_hs(void);
extern int stop_module_tracking_hs(void);

extern unsigned long get_timestamp_freq_hs(void);

extern int add_module_record_hs(struct add_module_data *data);

static ulong param_kallsyms_lookup_name_addr = 0;
module_param(param_kallsyms_lookup_name_addr, ulong, 0);

void** system_call_table_hs = NULL;

unsigned long long g_sample_count_hs = 0;
//extern struct px_hotspot_dsa *g_dsa;

struct HSTimerSettings g_tbs_settings_hs;
struct HSEventSettings g_ebs_settings_hs;
bool g_calibration_mode_hs;

/* sample buffer */
DEFINE_PER_CPU(struct RingBufferInfo, g_sample_buffer_hs);

/* module buffer */
struct RingBufferInfo g_module_buffer_hs;

bool g_is_image_load_set_hs = false;
bool g_is_app_exit_set_hs   = false;
bool g_launched_app_exit_hs = false;

/* the full path of the wait image */
char g_image_load_path_hs[PATH_MAX];

/* the pid of auto launched application */
pid_t g_launch_app_pid_hs = 0;
...
#ifdef PX_CPU_A7
	hs_samp_op = &hs_sampling_op_a7;
#endif
}

int handle_image_loaded_hs(void)
{
...
static int init_dsa(void)
{
	...
	g_module_buffer_hs.buffer.address = 0;
	g_module_buffer_hs.buffer.size           = 0;
...
{
	if (g_module_buffer_hs.buffer.address != 0)
	{
		vfree(g_module_buffer_hs.buffer.address);
	}
...
static int query_request(struct query_request_data *rd)
{
	...
	if (g_module_buffer_hs.is_full_event_set)
	{
		mask |= KDR_HS_MODULE_BUFFER_FULL;
	}

	if (g_launched_app_exit_hs)
	{
		mask |= KDR_HS_LAUNCHED_APP_EXIT;
		g_launched_app_exit_hs = false;
	}
...
{
	if (copy_from_user(&g_launch_app_pid_hs, pid, sizeof(pid_t)) != 0)
	{
		return -EFAULT;
	}

	g_is_app_exit_set_hs = true;
...
static int set_wait_image_load_name(char *path)
{
	...
	if (strncpy_from_user(g_image_load_path_hs, path, length) == -EFAULT)
	{
		return -EFAULT;
	}

	g_is_image_load_set_hs = true;
...
{
	memset(&g_tbs_settings_hs, 0, sizeof(g_tbs_settings_hs));

	if (settings == NULL)
	{
		g_tbs_settings_hs.interval = 0;
	}
	else
	{
		if (copy_from_user(&g_tbs_settings_hs, settings, sizeof(struct HSTimerSettings)) != 0)
		{
			return -EFAULT;
		}
	}
...
{
	memset(&g_ebs_settings_hs, 0, sizeof(g_ebs_settings_hs));

	if (settings == NULL)
	{
		g_ebs_settings_hs.eventNumber = 0;
	}
	else
	{
		if (copy_from_user(&g_ebs_settings_hs, settings, sizeof(struct HSEventSettings)) != 0)
		{
			return -EFAULT;
		}
	}
...
{
	if (copy_from_user(&g_calibration_mode_hs, calibration_mode, sizeof(bool)) != 0)
	{
		return -EFAULT;
	}
...
{
	if (copy_to_user(count, &g_sample_count_hs, sizeof(unsigned long long)))
	{
		return -EFAULT;
	}
