...
static void create_hotspot_module(const char *  module_path,
                                  ...
                                  PXD32_Hotspot_Module *p_info)
{
	...
	{
		p_info->flag |= HS_MODULE_RECORD_FLAG_EXE;
		
		if (g_is_app_exit_set_hs && (pid == g_launch_app_pid_hs))
		{
			p_info->flag |= HS_MODULE_RECORD_FLAG_AUTO_LAUNCH_PID;
		}
	}

	if (flag & MODULE_FLAG_GLOBAL)
	{
		p_info->flag |= HS_MODULE_RECORD_FLAG_GLOBAL_MODULE;
	}

	if (flag & MODULE_FLAG_1ST)
	{
		p_info->flag |= HS_MODULE_RECORD_FLAG_FIRST_MODULE_IN_PROCESS;
	}
...
int add_module_record_hs(struct add_module_data * data)
{
	int ret = 0;
	bool need_flush;
	bool buffer_full;
	...
	write_ring_buffer(&g_module_buffer_hs.buffer, p_info, p_info->length, &buffer_full, &need_flush);

	if (need_flush && !g_module_buffer_hs.is_full_event_set)
	{
		g_module_buffer_hs.is_full_event_set = true;
...
static void check_wait_for_image_load(const char *path)
{
	...
	if (g_is_image_load_set_hs && !strcmp(path, g_image_load_path_hs))
	{
		g_is_image_load_set_hs = false;
...
//	spin_lock_init(&buf_lock);
	...
	px_original_sys_fork       = (sys_fork_t) xchg(&system_call_table_hs[__NR_fork - __NR_SYSCALL_BASE], px_sys_fork);
	px_original_sys_vfork      = (sys_vfork_t) xchg(&system_call_table_hs[__NR_vfork - __NR_SYSCALL_BASE], px_sys_vfork);
	px_original_sys_clone      = (sys_clone_t) xchg(&system_call_table_hs[__NR_clone - __NR_SYSCALL_BASE], px_sys_clone);
	px_original_sys_execve     = (sys_execve_t) xchg(&system_call_table_hs[__NR_execve - __NR_SYSCALL_BASE], px_sys_execve);
	px_original_sys_mmap       = (sys_mmap_t) xchg(&system_call_table_hs[__NR_mmap - __NR_SYSCALL_BASE], px_sys_mmap);
	px_original_sys_mmap2      = (sys_mmap2_t) xchg(&system_call_table_hs[__NR_mmap2 - __NR_SYSCALL_BASE], px_sys_mmap2);
	px_original_sys_exit       = (sys_exit_t) xchg(&system_call_table_hs[__NR_exit - __NR_SYSCALL_BASE], px_sys_exit);
	px_original_sys_exit_group = (sys_exit_group_t) xchg(&system_call_table_hs[__NR_exit_group - __NR_SYSCALL_BASE], px_sys_exit_group);
	px_original_sys_kill       = (sys_kill_t) xchg(&system_call_table_hs[__NR_kill - __NR_SYSCALL_BASE], px_sys_kill);
	px_original_sys_tkill      = (sys_tkill_t) xchg(&system_call_table_hs[__NR_tkill - __NR_SYSCALL_BASE], px_sys_tkill);
	px_original_sys_tgkill     = (sys_tgkill_t) xchg(&system_call_table_hs[__NR_tgkill - __NR_SYSCALL_BASE], px_sys_tgkill);
	px_original_sys_prctl      = (sys_prctl_t) xchg(&system_call_table_hs[__NR_prctl - __NR_SYSCALL_BASE], px_sys_prctl);
...
static int remove_os_hooks(void)
{
	...
	if ((orgFn = xchg(&px_original_sys_fork, 0)))
		orgFn = xchg(&system_call_table_hs[__NR_fork - __NR_SYSCALL_BASE], orgFn);

	if ((orgFn = xchg(&px_original_sys_vfork, 0)))
		orgFn = xchg(&system_call_table_hs[__NR_vfork - __NR_SYSCALL_BASE], orgFn);

	if ((orgFn = xchg(&px_original_sys_clone, 0)))
		orgFn = xchg(&system_call_table_hs[__NR_clone - __NR_SYSCALL_BASE], orgFn);

	if ((orgFn = xchg(&px_original_sys_execve, 0)))
		orgFn = xchg(&system_call_table_hs[__NR_execve - __NR_SYSCALL_BASE], orgFn);

	if ((orgFn = xchg(&px_original_sys_mmap, 0)))
		orgFn = xchg(&system_call_table_hs[__NR_mmap - __NR_SYSCALL_BASE], orgFn);

	if ((orgFn = xchg(&px_original_sys_mmap2, 0)))
		orgFn = xchg(&system_call_table_hs[__NR_mmap2 - __NR_SYSCALL_BASE], orgFn);

	if ((orgFn = xchg(&px_original_sys_exit, 0)))
		orgFn = xchg(&system_call_table_hs[__NR_exit - __NR_SYSCALL_BASE], orgFn);

	if ((orgFn = xchg(&px_original_sys_exit_group, 0)))
		orgFn = xchg(&system_call_table_hs[__NR_exit_group - __NR_SYSCALL_BASE], orgFn);

	if ((orgFn = xchg(&px_original_sys_kill, 0)))
		orgFn = xchg(&system_call_table_hs[__NR_kill - __NR_SYSCALL_BASE], orgFn);

	if ((orgFn = xchg(&px_original_sys_tkill, 0)))
		orgFn = xchg(&system_call_table_hs[__NR_tkill - __NR_SYSCALL_BASE], orgFn);

	if ((orgFn = xchg(&px_original_sys_tgkill, 0)))
		orgFn = xchg(&system_call_table_hs[__NR_tgkill - __NR_SYSCALL_BASE], orgFn);

	if ((orgFn = xchg(&px_original_sys_prctl, 0)))
		orgFn = xchg(&system_call_table_hs[__NR_prctl - __NR_SYSCALL_BASE], orgFn);
...
int start_module_tracking_hs(void)
{
	int ret;
...
int stop_module_tracking_hs(void)
{
	int ret;
