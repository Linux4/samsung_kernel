...
static struct event_settings es[COUNTER_A9_MAX_ID];
static u32 reg_value[COUNTER_A9_MAX_ID];
...
static void set_ccnt_events(struct CSSEventConfig *ec, struct pmu_registers_a9 *pmu_regs)
{
	if (ec->eventId == A9_PMU_CCNT_CORE_CLOCK_TICK_64)
	{
		/* 64 clock cycle */
		pmu_regs->pmcr |= 0x8;
	}
	...
	pmu_regs->ccnt = es[COUNTER_A9_PMU_CCNT].reset_value;
}
                           ...
                           struct pmu_registers_a9 *pmu_regs)
{
	...
	switch (pmn_index)
	{
	case 0: register_id = COUNTER_A9_PMU_PMN0; break;
	case 1: register_id = COUNTER_A9_PMU_PMN1; break;
	case 2: register_id = COUNTER_A9_PMU_PMN2; break;
	case 3: register_id = COUNTER_A9_PMU_PMN3; break;
	case 4: register_id = COUNTER_A9_PMU_PMN4; break;
	case 5: register_id = COUNTER_A9_PMU_PMN5; break;
	default: return;
	}
...
static unsigned long long get_event_count_for_calibration_a9(int register_id)
{
...
static void start_pmu(void * data)
{
	...
	for (i=0; i<g_ebs_settings_css.eventNumber; i++)
	{
		...
		case COUNTER_A9_PMU_PMN4:
			set_pmn_events(4, &g_ebs_settings_css.event[i], &pmu_regs);
			break;

		case COUNTER_A9_PMU_PMN5:
			set_pmn_events(5, &g_ebs_settings_css.event[i], &pmu_regs);
			break;
...
{
	if (es[COUNTER_A9_PMU_CCNT].enabled)
		return true;

	if (es[COUNTER_A9_PMU_PMN0].enabled)
		return true;

	if (es[COUNTER_A9_PMU_PMN1].enabled)
		return true;

	if (es[COUNTER_A9_PMU_PMN2].enabled)
		return true;

	if (es[COUNTER_A9_PMU_PMN3].enabled)
		return true;

	if (es[COUNTER_A9_PMU_PMN4].enabled)
		return true;

	if (es[COUNTER_A9_PMU_PMN5].enabled)
		return true;
...
static int start_sampling_a9(bool is_start_paused)
{
...
static int stop_sampling_a9(void)
{
	// config tbs sampling with ap timer 2
	if ((g_tbs_settings_css.interval != 0) && !g_calibration_mode_css)
	{
...
static int pause_sampling_a9(void)
{
	if ((g_tbs_settings_css.interval != 0) && !g_calibration_mode_css)
	{
...
static int resume_sampling_a9(void)
{
	if ((g_tbs_settings_css.interval != 0) && !g_calibration_mode_css)
	{
		// Enable timer
...
struct css_sampling_op_mach css_sampling_op_a9 =
{
	.start  = start_sampling_a9,
	.stop   = stop_sampling_a9,
	.pause  = pause_sampling_a9,
	.resume = resume_sampling_a9,
	.get_count_for_cal = get_event_count_for_calibration_a9,
};
