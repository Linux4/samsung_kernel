...
struct RingBufferInfo g_event_buffer;
...
{
	g_event_buffer.buffer.address = 0;
	g_event_buffer.buffer.size           = 0;
...
static int free_event_buffer(void)
{
	...
	{
		vfree(g_event_buffer.buffer.address);
	}
...
static int query_request(struct query_request_data *rd)
{
	...
	if (g_event_buffer.is_full_event_set)
	{
...
static unsigned long get_timestamp_freq(void)
{
	return USEC_PER_SEC;
}
...
static int read_event_buffer(struct read_buffer_data * data)
{
	...
	if (!only_full || g_event_buffer.is_full_event_set)
	{
		char * buffer = vmalloc(g_event_buffer.buffer.size);
		
		if (buffer != NULL)
		{
			size_read = read_ring_buffer(&g_event_buffer.buffer, buffer);
