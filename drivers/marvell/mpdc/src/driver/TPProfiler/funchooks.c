...
static void request_flush_event_buffer(void)
{
	...
	{
		g_event_buffer.is_full_event_set = true;
...
static void write_event(PXD32_TP_Event * et, void * info, int info_size)
{
	...
	write_ring_buffer(&g_event_buffer.buffer, et, offsetof(PXD32_TP_Event, eventData), &buffer_full, &need_flush);
	write_ring_buffer(&g_event_buffer.buffer, info, info_size, &buffer_full, &need_flush);
