...
unsigned long long g_sample_count = 0;
...
/* sample buffer */
DEFINE_PER_CPU(struct RingBufferInfo, g_sample_buffer);
...
#ifdef PX_CPU_PJ4B
	hs_samp_op = &hs_sampling_op_pj4b;
#endif

#ifdef PX_CPU_A9
	hs_samp_op = &hs_sampling_op_a9;
#endif
}
...
static int init_dsa(void)
{
	...
	for_each_possible_cpu(cpu)
	{
		...
		sample_buffer = &per_cpu(g_sample_buffer, cpu);
		...
		//sample_buffer->buffer.is_data_lost   = false;
	}
...
static int allocate_all_sample_buffers(unsigned int * size)
{
	...
	/* free sample buffer if it is already allocated */
...
static int read_module_buffer(struct read_buffer_data * data)
{
	...
	{
		char * buffer = vmalloc(g_module_buffer.buffer.size);

		if (buffer != NULL)
		{
			size_read = read_ring_buffer(&g_module_buffer.buffer, buffer);
...
{
	if (copy_to_user(count, &g_sample_count, sizeof(unsigned long long)))
	{
		return -EFAULT;
	}
...
static int check_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	/* if the flag is not O_RDONLY, all ioctl code are permitted */
	if ((fp->f_flags & O_ACCMODE) != O_RDONLY)
		return 0;

	/* For O_RDONLY mode, only the following ioctl codes are permitted */
	switch(cmd)
	{
...
static struct file_operations px_hotspot_d_fops = {
	...
	.write          = px_hotspot_d_write,
	.poll           = px_hotspot_d_poll,
	.unlocked_ioctl = px_hotspot_d_ioctl,
	.open           = px_hotspot_d_open,
	.release        = px_hotspot_d_release,
};
#endif
