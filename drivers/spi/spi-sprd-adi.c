...
{
	u32 adi_debug0 = 0, adi_debug1 = 0, adi_debug2 = 0;
	u32 value = 0;
	...
	do {
		...
		if (sadi_ss->data->chip_id != value) {
			dev_emerg(sadi_ss->dev, "PMIC chip id is: 0x%x\n", value);
			udelay(500);
		} else {
			break;
		}
	...
	if (timeout == 0) {
		...
		dev_emerg(sadi_ss->dev, "adi_debug0: 0x%x adi_debug1:0x %xadi_debug2: 0x%x\n",
		adi_debug0, adi_debug1, adi_debug2);
	}
