...
static int s3c64xx_spi_probe(struct platform_device *pdev)
{
	...
	if (sci->secure_mode != SECURE_MODE) {
		...
		ret = of_property_read_u32(dev->of_node, "samsung,weak-pull-up-mask", &sci->ps_cfg);
		if (!ret) {
			ret = of_property_read_u32(dev->of_node, "samsung,weak-pud-val", &sci->weak_pud_val);
			if (!ret) {
				sci->ps_reg = syscon_regmap_lookup_by_phandle(dev->of_node,
						"samsung,ps-phandle");
				if (IS_ERR(sci->ps_reg)) {
					dev_err(dev, "failed to get sysreg phandle\n");
				} else {
					sci->weak_pud_en = 1;
					regmap_update_bits(sci->ps_reg, 0x0, PS_MASK << sci->ps_cfg,
							sci->weak_pud_val << sci->ps_cfg);
				}
			}
		}
	}
