...
int secure_debug_set_config(uint8_t addr, uint8_t config)
{
	...
	ret = s2mpu15_update_reg(static_info->i2c, addr, config, 0xF);
	if (ret < 0) {
		pr_err("%s: s2mpu15_update_reg failed\n", __func__);
		return -1;
	}

	ret = s2mpu15_read_reg(static_info->i2c, addr, &val);
	if (ret < 0) {
		pr_err("%s: s2mpu15_read_reg failed\n", __func__);
		return -1;
	}
...
void secure_debug_write(uint8_t addr, uint8_t bit, uint8_t val)
{
	...
	ret = s2mpu15_update_reg(static_info->i2c, addr, val << bit, 1 << bit);
	if (ret < 0)
		return;
}
...
void secure_debug_clear(void)
{
	...
	for (i = RTC_REG_SECURE1; i <= RTC_REG_SECURE4; i++) {
		ret = s2mpu15_write_reg(static_info->i2c, i, 0x00);
		if (ret < 0)
			return;
	}
}
				 ...
				 enum S2M_RTC_OP op)
{
	...
	ret = s2mpu15_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, data);
	if (ret < 0)
...
static int s2m_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	ret = s2mpu15_bulk_read(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	mutex_lock(&info->lock);
	ret = s2mpu15_bulk_write(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mpu15_bulk_read(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
	...
	ret = s2mpu15_read_reg(info->pmic_i2c, reg, &val); /* i2c for PM */
	if (ret < 0) {
...
static int s2m_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	if (info->alarm_check) {
		...
		ret = s2mpu15_bulk_write(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
				data);
		if (ret < 0) {
...
static int s2m_rtc_stop_boot_alarm0(struct s2m_rtc_info *info)
{
	...
	ret = s2mpu15_bulk_read(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS, data);
	if (ret < 0)
		return ret;
	...
	ret = s2mpu15_bulk_write(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS, data);
	if (ret < 0)
		return ret;
...
static int s2m_rtc_stop_boot_alarm(struct s2m_rtc_info *info)
{
	...
	ret = s2mpu15_bulk_read(info->i2c, S2MP_RTC_REG_A1SEC, NR_RTC_CNT_REGS, data);
	if (ret < 0)
		return ret;
	...
	ret = s2mpu15_bulk_write(info->i2c, S2MP_RTC_REG_A1SEC, NR_RTC_CNT_REGS, data);
	if (ret < 0)
		return ret;
...
static int s2m_rtc_read_boot_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mpu15_read_reg(info->pmic_i2c, reg, &val);
	if (ret < 0) {
		dev_err(info->dev, "%s: %d fail to read STATUS2 reg(%d)\n", __func__, __LINE__, ret);
		goto out;
	}
...
static int _s2m_rtc_set_boot_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mpu15_read_reg(info->i2c, S2MP_RTC_REG_UPDATE, &info->update_reg);
	if (ret < 0) {
	...
	ret = s2mpu15_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, (char)info->update_reg);
	if (ret < 0)
...
static irqreturn_t s2m_rtc_boot_alarm_irq(int irq, void *data)
{
	...
	if (info->boot_alarm_enabled) {
		if (is_charging_mode)
			...
			info->update_reg &= ~RTC_WAKE_MASK;
			s2mpu15_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, (char)info->update_reg);
		}
	}
...
static struct attribute *s2mpu15_rtc_attrs[] = {
	&dev_attr_bootalarm.attr,
	NULL
};

static const struct attribute_group s2mpu15_rtc_sysfs_files = {
	.attrs	= s2mpu15_rtc_attrs,
};
#endif
						...
						struct s2mpu15_platform_data *pdata)
{
	...
	if (pdata->osc_bias_up >= 0) {
		ret = s2mpu15_update_reg(info->i2c, S2MP_RTC_REG_CAPSEL,
	...
	if (pdata->osc_xin >= 0) {
		ret = s2mpu15_update_reg(info->i2c, S2MP_RTC_REG_OSCCTRL,
						...
						struct s2mpu15_platform_data *pdata)
{
	...
	ret = s2mpu15_write_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, val);
	if (ret < 0) {
					...
					struct s2mpu15_platform_data *pdata)
{
	...
	ret = s2mpu15_update_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, 0,
				...
				struct s2mpu15_platform_data *pdata)
{
	...
	ret = s2mpu15_bulk_read(info->i2c, S2MP_RTC_REG_A1SEC, NR_RTC_CNT_REGS, data_alrm1);
	if (ret < 0)
		return ret;
	...
	ret = s2mpu15_read_reg(info->i2c, S2MP_RTC_REG_UPDATE, &update_val);
	if (ret < 0) {
	...
	ret = s2mpu15_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, info->update_reg);
	if (ret < 0) {
		dev_err(info->dev, "%s: fail to write update reg(%d)\n",
			__func__, ret);
		return ret;
	...
	ret = s2mpu15_read_reg(info->i2c, S2MP_RTC_REG_CTRL, &ctrl_val);
	if (ret < 0) {
	...
	ret = s2mpu15_read_reg(info->i2c, S2MP_RTC_REG_CAPSEL, &capsel_val);
	if (ret < 0) {
	...
	data = MODEL24_MASK;
	ret = s2mpu15_write_reg(info->i2c, S2MP_RTC_REG_CTRL, data);
	if (ret < 0) {
	...
	capsel_val |= 0xf0;
	ret = s2mpu15_write_reg(info->i2c, S2MP_RTC_REG_CAPSEL, capsel_val);
	if (ret < 0) {
				   ...
				   struct s2m_rtc_info *info, int irq_base)
{
	...
	switch (info->iodev->device_type) {
	case S2MPU15X:
		info->boot_alarm_irq = irq_base + S2MPU15_PMIC_IRQ_RTCA1_INT2;
		break;
	default:
		/* If this happens the core funtion has a problem */
		BUG();
	...
	ret = rtc_add_group(info->rtc_dev, &s2mpu15_rtc_sysfs_files);
	if (ret < 0) {
				 ...
				 struct s2m_rtc_info *info, int irq_base)
{
	...
	switch (info->iodev->device_type) {
	case S2MPU15X:
		info->irq = irq_base + S2MPU15_PMIC_IRQ_RTCA0_INT2;
		break;
	default:
		/* If this happens the core funtion has a problem */
		BUG();
...
static int s2m_rtc_probe(struct platform_device *pdev)
{
	...
	struct s2mpu15_platform_data *pdata = dev_get_platdata(iodev->dev);
...
static void s2m_rtc_shutdown(struct platform_device *pdev)
{
	/*disable wtsr, smpl */
	struct s2m_rtc_info *info = platform_get_drvdata(pdev);
	struct s2mpu15_platform_data *pdata = dev_get_platdata(info->iodev->dev);
...
static int s2mpu15_rtc_suspend(struct device *dev)
{
	struct s2m_rtc_info *info = dev_get_drvdata(dev);
	struct rtc_time tm;
	int ret;
...
static int s2mpu15_rtc_resume(struct device *dev)
{
	struct s2m_rtc_info *info = dev_get_drvdata(dev);
	struct rtc_time tm;
	int ret;
...
static SIMPLE_DEV_PM_OPS(s2mpu15_rtc_pm, s2mpu15_rtc_suspend, s2mpu15_rtc_resume);
