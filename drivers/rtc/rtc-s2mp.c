				 ...
				 enum S2M_RTC_OP op)
{
	...
	ret = s2mpu06_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, data);
	if (ret < 0)
...
static int s2m_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	ret = s2mpu06_bulk_read(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	mutex_lock(&info->lock);
	ret = s2mpu06_bulk_write(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_check_rtc_time(struct s2m_rtc_info *info)
{
	...
	if (abs(rtc_time - sys_time.tv_sec) > 2) {
		...
		ret = s2mpu06_bulk_write(info->i2c, S2MP_RTC_REG_SEC,
		...
		dev_warn(info->dev, "%s: adjust RTC TIME: sys_time:%lu, rtc_time:%lu\n",
...
static int s2m_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mpu06_bulk_read(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
	...
	ret = s2mpu06_read_reg(info->pmic_i2c, reg, &val); /* i2c for PM */
	if (ret < 0) {
...
static int s2m_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	if (info->alarm_check) {
		...
		ret = s2mpu06_bulk_write(info->i2c, S2MP_RTC_REG_A0SEC,
			 NR_RTC_CNT_REGS, data);
		if (ret < 0) {
	...
	ret = s2mpu06_bulk_write(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
					...
					struct s2mpu06_platform_data *pdata)
{
	...
	ret = s2mpu06_write_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, val);
	if (ret < 0) {
					...
					struct s2mpu06_platform_data *pdata)
{
	...
	ret = s2mpu06_update_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, 0,
				...
				struct s2mpu06_platform_data *pdata)
{
	...
	ret = s2mpu06_read_reg(info->i2c, S2MP_RTC_REG_UPDATE, &update_val);
	if (ret < 0) {
	...
	info->update_reg =
		update_val & ~(info->wudr_mask | RTC_FREEZE_MASK |
					 RTC_RUDR_MASK | info->audr_mask);

	ret = s2mpu06_write_reg(info->i2c, S2MP_RTC_REG_UPDATE,
						 info->update_reg);
	if (ret < 0) {
	...
	ret = s2mpu06_read_reg(info->i2c, S2MP_RTC_REG_CTRL, &ctrl_val);
	if (ret < 0) {
	...
	ret = s2mpu06_read_reg(info->i2c, S2MP_RTC_REG_CAPSEL, &capsel_val);
	if (ret < 0) {
	...
	data = MODEL24_MASK;
	ret = s2mpu06_write_reg(info->i2c, S2MP_RTC_REG_CTRL, data);
	if (ret < 0) {
	...
	ret = s2mpu06_write_reg(info->i2c, S2MP_RTC_REG_CAPSEL, 0xf8);
	if (ret < 0) {
...
static int s2m_rtc_probe(struct platform_device *pdev)
{
	...
	struct s2mpu06_platform_data *pdata = dev_get_platdata(iodev->dev);
	...
	switch (info->iodev->device_type) {
	case S2MPU06X:
		info->irq = irq_base + S2MPU06_PMIC_IRQ_RTCA0_INT2;
		break;
	default:
		/* If this happens the core function has a problem */
		BUG();
