...
static void
sapa_load_alarm(struct pm8xxx_rtc *rtc_dd, unsigned int ctrl_reg)
{
	...
	rc = regmap_bulk_read(rtc_dd->regmap, regs->alarm_rw, value, sizeof(value));
	if (rc) {
	...
	rc = regmap_read(rtc_dd->regmap, ctrl_reg, &ctrl_reg_value);
	if (rc) {
		pr_err("%s: alarm enable read failed\n", __func__);
		return;
	}
	...
	rtc_dd->sapa.enabled = (ctrl_reg_value & PM8xxx_RTC_ENABLE) ? 1 : 0;
#endif

	pr_info("%s: alarm_reg=%02x, pmic=%lu\n", __func__, ctrl_reg_value, alarm_secs);
}
