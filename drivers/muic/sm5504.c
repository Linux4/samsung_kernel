...
static ssize_t sm5504_muic_show_uart_sel(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct sm5504_muic_data *muic_data = dev_get_drvdata(dev);
...
static ssize_t sm5504_muic_set_uart_sel(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	struct sm5504_muic_data *muic_data = dev_get_drvdata(dev);
	...
	if (!strncasecmp(buf, "AP", 2)) {
		...
		/* TODO : It need to modify path inside of AP? */
...
static DEVICE_ATTR(uart_sel, 0664, sm5504_muic_show_uart_sel,
					sm5504_muic_set_uart_sel);
static DEVICE_ATTR(usb_sel, 0664, muic_show_usb_sel, muic_set_usb_sel);
...
static int sm5504_muic_reg_init(struct sm5504_muic_data *muic_data)
{
	...
	dev1 = sm5504_i2c_read_byte(i2c, SM5504_MUIC_REG_DEV_TYPE1);
	if (dev1 < 0) {
		pr_err("%s, failed to read dev1 %d\n", __func__, dev1);
		return -EINVAL;
	}
	dev2 = sm5504_i2c_read_byte(i2c, SM5504_MUIC_REG_DEV_TYPE2);
