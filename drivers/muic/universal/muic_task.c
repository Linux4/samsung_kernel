...
static irqreturn_t muic_irq_thread(int irq, void *data)
{
	...
	while(pmuic->is_sleep_state == true){
		pr_info("%s sleep_state retry=%d\n", __func__, retry);
		usleep_range(10 * 1000, 10 * 1000);
		if (++retry > 5) {
			pmuic->need_to_check = true;
			return IRQ_HANDLED;
		}
	}
#endif

	wake_lock(&pmuic->wake_lock);
...
#ifdef USE_SUSPEND_LATE
static int muic_suspend_late(struct device *dev)
{
	muic_data_t *pmuic = dev_get_drvdata(dev);

	pmuic->is_sleep_state = true;

	return 0;
}

static int muic_resume_early(struct device *dev)
{
	muic_data_t *pmuic = dev_get_drvdata(dev);

	pmuic->is_sleep_state = false;

	if (pmuic->need_to_check) {
		muic_irq_thread(-1, pmuic);
		pmuic->need_to_check =false;
	}
...
const struct dev_pm_ops muic_pm = {
	...
	.resume = muic_resume,
#ifdef USE_SUSPEND_LATE
	.suspend_late = muic_suspend_late,
	.resume_early = muic_resume_early,
#endif
};
#endif /* CONFIG_PM */
