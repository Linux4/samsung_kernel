...
int muic_GPIO_control(int gpio)
{
	...
	ret = gpio_request(_gpmuic->sm570x_switch_gpio, "SM570X_SWITCH_GPIO");
	if (ret) {
		pr_err("failed to gpio_request SM570X_SWITCH_GPIO\n");
		return 0;
	}

	sm570x_switch_val = gpio_get_value(_gpmuic->sm570x_switch_gpio);
	pr_info("%s: sm570x_gpio_switch(%d) --> (%d)", __func__, sm570x_switch_val, gpio);
	if (sm570x_switch_val != gpio) {
		if (gpio_is_valid(_gpmuic->sm570x_switch_gpio)) {
			pr_info("%s: gpio_set_value\n", __func__);
			gpio_set_value(_gpmuic->sm570x_switch_gpio, gpio);
			sm570x_switch_val = gpio_get_value(_gpmuic->sm570x_switch_gpio);
		}
	}

	gpio_free(_gpmuic->sm570x_switch_gpio);

	pr_info("%s: SM570X_SWITCH_GPIO(%d)=%c\n", __func__, _gpmuic->sm570x_switch_gpio,
