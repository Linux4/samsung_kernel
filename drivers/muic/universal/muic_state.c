			...
			muic_attached_dev_t new_dev, int adc, u8 vbvolt)
{
...
#if defined(CONFIG_MUIC_SUPPORT_EARJACK)
		if (adc == 0x1E && vbvolt) {
			ret = regulator_disable(pmuic->vdd);
			if (ret)
				pr_info("%s: EAR Switch regulator disable failed\n", __func__);
			else
				pr_info("%s: [MUIC] EAR Switch regulator disable Success\n", __func__);

			pmuic->is_ear_switch = 0;
	...
	case ATTACHED_DEV_EARJACK_MUIC:
		if (!pmuic->is_ear_switch) {
			ret = regulator_enable(pmuic->vdd);
			if (ret)
				pr_info("%s: EAR Switch regulator enable failed\n", __func__);
			else
				pr_info("%s: EAR Switch regulator enable Success\n", __func__);
			pmuic->is_ear_switch = 1;
		}
...
static void muic_handle_detach(muic_data_t *pmuic)
{
	...
	case ATTACHED_DEV_EARJACK_MUIC:
		...
		if (ret)
			...
			pr_info("%s: EAR Switch regulator disable Success\n", __func__);
