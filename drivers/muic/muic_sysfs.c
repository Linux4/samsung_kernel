...
static void nad_usb_timer_handler(unsigned long data);
DEFINE_TIMER(nad_usb_timer, nad_usb_timer_handler, 0, 0);
static void nad_usb_timer_handler(unsigned long data)
{
	int fd;
	mm_segment_t oldfs;
	...
	del_timer(&nad_usb_timer);

	mt_usb_connect();
	pr_info("%[NAD] s() connect USB, %u jiffies\n", __func__, (unsigned)j);
}

static ssize_t muic_sysfs_set_usb_off_on(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	unsigned long val, j = jiffies;
	//struct muic_platform_data *muic_pdata = dev_get_drvdata(dev);
	pr_info("[NAD] %s() entered with value %s\n", __func__, buf);

	ret = kstrtol(buf, 10, (long *)&val);
	if(ret < 0) {
		pr_err("[NAD] %s() error: invalid value %s\n", __func__, buf);
		return ret;
	}

	if(val > 0) {
		mt_usb_disconnect();
		pr_info("[NAD] %s() disconnect USB, %u(next:%u)jiffies\n", __func__, j + msecs_to_jiffies(val));
		mod_timer(&nad_usb_timer, j + msecs_to_jiffies(val));
	}
...
#ifdef CONFIG_MUIC_S2MU005
	muic_pdata->is_rustproof = s2mu005_get_rustproof(muic_pdata->drv_data);
#endif
...
#ifdef CONFIG_MUIC_S2MU005
	s2mu005_update_rustproof(muic_pdata->drv_data, muic_pdata->is_rustproof);
#endif
	 ...
	 struct device_attribute *attr, char *buf)
{
	struct muic_platform_data *muic_pdata = dev_get_drvdata(dev);
#ifdef CONFIG_MUIC_S2MU005
	int mdev = (int)s2mu005_get_attached_dev(muic_pdata->drv_data);
#else
...
#if defined(CONFIG_SEC_FACTORY)
static DEVICE_ATTR(usb_off_on, 0664,
		NULL,
		muic_sysfs_set_usb_off_on);
#endif
