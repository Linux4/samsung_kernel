...
int _s2mu106_muic_recheck_adc(struct s2mu106_muic_data *muic_data)
{
	...
	_s2mu106_muic_control_rid_adc(muic_data, true);
	msleep(200);
...
#if IS_ENABLED(CONFIG_MUIC_S2MU106_RID)
	_s2mu106_muic_control_rid_adc(muic_data, S2MU106_DISABLE);
...
static void s2mu106_muic_get_detect_info(struct s2mu106_muic_data *muic_data)
{
	struct muic_platform_data *muic_pdata = muic_data->pdata;
	struct i2c_client *i2c = muic_data->i2c;
...
static int s2mu106_muic_reg_init(struct s2mu106_muic_data *muic_data)
{
	 ...
	 */
	_s2mu106_muic_control_rid_adc(muic_data, S2MU106_ENABLE);
...
static void s2mu106_muic_set_rid_for_water(struct s2mu106_muic_data *muic_data, bool en)
{
	...
	reg_disch = s2mu106_i2c_read_byte(i2c, S2MU106_REG_RID_DISCHARGE);

	pr_info("%s en : %d\n",	__func__, (int)en);
	if (en) {
		reg_disch |= RID_DISCHARGE_RID_DISCHG_TIME_MASK;
		s2mu106_i2c_write_byte(i2c, S2MU106_REG_RID_WATER_PROOF, 0x5);
	} else {
		reg_disch &= ~RID_DISCHARGE_RID_DISCHG_TIME_MASK;
		s2mu106_i2c_write_byte(i2c, S2MU106_REG_RID_WATER_PROOF, 0x35);
	}

	s2mu106_i2c_write_byte(i2c, S2MU106_REG_RID_DISCHARGE, reg_disch);
}
...
static irqreturn_t s2mu106_muic_vbus_isr(int irq, void *data)
{
	struct s2mu106_muic_data *muic_data = data;
	struct muic_platform_data *muic_pdata;
...
static int s2mu106_muic_irq_init(struct s2mu106_muic_data *muic_data)
{
	...
	if (muic_data->mfd_pdata && (muic_data->mfd_pdata->irq_base > 0)) {
		...
		REQUEST_IRQ(muic_data->irq_vbus_on, muic_data,
			"muic-vbus_on", &s2mu106_muic_vbus_isr);
		...
		REQUEST_IRQ(muic_data->irq_vbus_off, muic_data,
			"muic-vbus_off", &s2mu106_muic_vbus_isr);
