...
struct muic_platform_data *pmuic_pdata;
...
static int muic_core_switch_to_uart(struct muic_platform_data *muic_pdata, int path)
{
	...
	if (gpio_is_valid(muic_pdata->gpio_uart_sel)) {
...
int muic_core_handle_detach(struct muic_platform_data *muic_pdata)
{
	...
	if (noti) {
		if (!muic_pdata->suspended)
			MUIC_SEND_NOTI_DETACH(prev_dev);
		else
			muic_pdata->need_to_noti = true;
	}
	return ret;
...
#endif
struct muic_platform_data g_muic_pdata;
struct muic_platform_data *muic_core_init(void *drv_data)
{
	...
	g_muic_pdata.cleanup_switch_dev_cb	= muic_cleanup_switch_dev_cb;
	g_muic_pdata.init_gpio_cb = muic_init_gpio_cb;
//	g_muic_pdata.muic_set_hiccup_mode_cb	= sm5714_muic_pdic_set_hiccup_mode;
	g_muic_pdata.init_gpio_cb = muic_init_gpio_cb;
	g_muic_pdata.rustproof_on = false;
	g_muic_pdata.afc_disable = false;
//	g_muic_pdata.muic_afc_set_voltage_cb = sm5714_muic_afc_set_voltage;
//	g_muic_pdata.muic_hv_charger_init_cb = sm5714_muic_hv_charger_init;
	return muic_pdata;
}
