...
#endif
#if defined(CONFIG_PM_S2M_WATER)
	power_supply_get_property(muic_data->psy_pm, POWER_SUPPLY_PROP_VGPADC, &value);
#endif
...
#if defined(CONFIG_PM_S2M_WATER)
	} else if (IS_VGPADC_VBUS_SHORT(value.intval)){
		str = "GPADC-VBUS short";
#endif
	}
...
int s2mu106_hv_muic_init(struct s2mu106_muic_data *muic_data)
{
	...
	muic_data->psy_pm = power_supply_get_by_name("s2mu106_pmeter");
	if (!muic_data->psy_pm) {
		pr_err("%s: Fail to get pmeter\n", __func__);
	}
#endif
