...
static void _s2mu106_hv_muic_dcp_charger_attach(struct s2mu106_muic_data *muic_data)
{
	...
	} else if (muic_data->received_tx_data != MUIC_HV_5V) {
		pr_info("%s AFC disable(%d) by Flash\n", __func__, muic_data->received_tx_data);
		return;
#endif
	}
...
#if defined(CONFIG_LEDS_S2MU106_FLASH)
	muic_data->received_tx_data = tx_data;
#endif
	switch (muic_pdata->attached_dev) {
	case ATTACHED_DEV_QC_CHARGER_9V_MUIC:
