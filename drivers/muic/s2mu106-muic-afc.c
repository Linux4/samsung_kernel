...
static int s2mu106_hv_muic_state_manager(struct s2mu106_muic_data *muic_data,
		muic_hv_transaction_t trans);
static void _s2mu106_hv_muic_reset(struct s2mu106_muic_data *muic_data);
static void _s2mu106_hv_muic_dcp_charger(struct s2mu106_muic_data *muic_data);
static void _s2mu106_hv_muic_fast_charge_adaptor(struct s2mu106_muic_data *muic_data);
static void _s2mu106_hv_muic_fast_charge_communication(struct s2mu106_muic_data *muic_data);
static void _s2mu106_hv_muic_afc_5v_charger(struct s2mu106_muic_data *muic_data);
static void _s2mu106_hv_muic_afc_9v_charger(struct s2mu106_muic_data *muic_data);
static void _s2mu106_hv_muic_qc_charger(struct s2mu106_muic_data *muic_data);
static void _s2mu106_hv_muic_qc_5v_charger(struct s2mu106_muic_data *muic_data);
static void _s2mu106_hv_muic_qc_9v_charger(struct s2mu106_muic_data *muic_data);

static void (* const action_on_state[HV_STATE_MAX_NUM])(struct s2mu106_muic_data *data) =
{
	_s2mu106_hv_muic_reset, /* 0 */
	_s2mu106_hv_muic_dcp_charger, /* 1 */
	_s2mu106_hv_muic_fast_charge_adaptor, /* 2 */
	_s2mu106_hv_muic_fast_charge_communication, /* 3 */
	_s2mu106_hv_muic_afc_5v_charger, /* 4 */
	_s2mu106_hv_muic_afc_9v_charger, /* 5 */
	_s2mu106_hv_muic_qc_charger, /* 6 */
	_s2mu106_hv_muic_qc_5v_charger, /* 7 */
	_s2mu106_hv_muic_qc_9v_charger /* 8 */
};

static struct s2mu106_muic_data *static_data;

bool muic_check_is_hv_dev(struct s2mu106_muic_data *muic_data)
{
	bool ret = false;
...
static inline void s2mu106_hv_muic_set_new_state(struct s2mu106_muic_data *muic_data,
		muic_hv_state_t next_state)
{
	muic_data->hv_state = next_state;
}

static inline muic_hv_state_t s2mu106_hv_muic_get_current_state(struct s2mu106_muic_data *muic_data)
{
	return muic_data->hv_state;
}
...
static void s2mu106_hv_muic_send_mping(struct s2mu106_muic_data* muic_data)
{
	...
	reg_val |= (AFCCTRL2_MPING_MASK | AFCCTRL2_MTXEN_MASK | AFCCTRL2_DNRESEN_MASK);
...
static void _s2mu106_hv_muic_reset(struct s2mu106_muic_data *muic_data)
{
	...
	muic_data->hv_state = HV_STATE_IDLE;
...
static void _s2mu106_hv_muic_dcp_charger_attach(struct s2mu106_muic_data *muic_data)
{
	...
	s2mu106_hv_muic_state_manager(muic_data, HV_TRANS_DCP_DETECTED);
	return;
#else
...
static void _s2mu106_hv_muic_dcp_charger(struct s2mu106_muic_data *muic_data)
{
	...
	s2mu106_hv_muic_write_reg(muic_data, S2MU106_REG_LDOADC_VSETL, 0x66);
...
static void _s2mu106_hv_muic_fast_charge_communication(struct s2mu106_muic_data *muic_data)
{
	...
	if (muic_data->mping_cnt < AFC_MPING_RETRY_CNT_LIMIT) {
		...
		s2mu106_hv_muic_state_manager(muic_data, HV_TRANS_NO_RESPONSE);
	}
}
...
static int s2mu106_if_vchgin_isr(void *mdata, int voltage)
{
	...
	vchgin = s2mu106_hv_muic_get_vchgin(muic_data);
	if (IS_VCHGIN_9V(vchgin)) {
		s2mu106_hv_muic_state_manager(muic_data, HV_TRANS_VBUS_BOOST);
	} else if (IS_VCHGIN_5V(vchgin)) {
		s2mu106_hv_muic_state_manager(muic_data, HV_TRANS_VBUS_REDUCE);
	}
...
void hv_muic_change_afc_voltage(struct s2mu106_muic_data *muic_data, int tx_data)
{
	...
	if (muic_pdata->attached_dev == ATTACHED_DEV_QC_CHARGER_9V_MUIC) {
		...
		for (i=0; i<4; i++) {
			msleep(50);
			(muic_data->mping_cnt)++;
			reg_val = (AFCCTRL2_MPING_MASK | AFCCTRL2_MTXEN_MASK |
					AFCCTRL2_DNRESEN_MASK | AFCCTRL2_DP06EN_MASK);
			s2mu106_hv_muic_write_reg(muic_data, S2MU106_REG_AFC_CTRL2, reg_val);
		}
	}
}
 ...
 */
static inline void s2mu106_hv_muic_handle_state(struct s2mu106_muic_data *muic_data,
		muic_hv_state_t next_state)
{
	s2mu106_hv_muic_set_new_state(muic_data, next_state);

	action_on_state[next_state](muic_data);
}

static int s2mu106_hv_muic_state_manager(struct s2mu106_muic_data *muic_data,
		muic_hv_transaction_t trans)
{
	muic_hv_state_t cur_state = s2mu106_hv_muic_get_current_state(muic_data);
	...
	if (skip_trans) {
		...
		s2mu106_hv_muic_handle_state(muic_data, next_state);
	}
...
static irqreturn_t s2mu106_hv_muic_vdnmon_isr(int irq, void *data)
{
	...
	if (muic_data->is_dp_drive && !vdnmon) {
		s2mu106_hv_muic_state_manager(muic_data, HV_TRANS_VDNMON_LOW);
	}
...
static irqreturn_t s2mu106_hv_muic_mrxrdy_isr(int irq, void *data)
{
	...
	s2mu106_hv_muic_state_manager(muic_data, HV_TRANS_FAST_CHARGE_PING_RESPONSE);
