...
#include <linux/muic/slsi/s2mu107/s2mu107-muic.h>
...
static void s2mu107_muic_detect_dev_pdic(struct s2mu107_muic_data *muic_data,
...
static void s2mu107_if_set_cable_state(void *mdata, muic_attached_dev_t new_dev)
{
	...
	s2mu107_muic_detect_dev_pdic(muic_data, new_dev);
...
/*
 * Temporary: user binary must detect usb killer, 
 * but we block usb killer detection logic before usb otg issue fixed 
 */
...
/* FIXME */
#if !IS_ENABLED(CONFIG_MUIC_UART_SWITCH)
	muic_data->pdata->gpio_uart_sel = of_get_named_gpio(np_muic, "uart_sel", 0);
#else
