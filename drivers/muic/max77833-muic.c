		...
		u8 val)
{
	...
	if (ret)
		...
		pr_debug("%s:%s reg(0x%02x) = 0x%02x\n",
				MUIC_DEV_NAME, __func__, reg, val);
	...
	ret = max77833_write_reg(i2c, reg, val);
	if (ret)
		pr_err("%s:%s fail to read reg[0x%02x], ret(%d)\n",
				MUIC_DEV_NAME, __func__, reg, ret);
	else
	...
	ret = max77833_muic_update_reg(i2c, MAX77833_MUIC_REG_DAT_OUT1,
				(val << shift), mask, true);
	if (ret)
		pr_err("%s:%s fail to update reg[0x%02x], ret(%d)\n",
				MUIC_DEV_NAME, __func__,
				MAX77833_MUIC_REG_DAT_OUT1, ret);
...
static void max77833_muic_handle_cmd(struct max77833_muic_data *muic_data, int irq)
{
	...
	case COMMAND_SWITCH_READ:
		...
		pr_debug("%s:%s opcode(0x%02x):data(0x%02x)\n",
	...
	case COMMAND_SWITCH_WRITE:
		pr_debug("%s:%s Write command, opcode(0x%02x)\n",
...
//		REQUEST_IRQ(muic_data->irq_idreg, muic_data, "muic-idreg");
	...
	pr_debug("%s:%s idreg(%d), chgtyp(%d), sysmsg(%d), apcmdres(%d)\n",
			MUIC_DEV_NAME, __func__,
...
void max77833_muic_shutdown(struct device *dev)
{
	...
	pr_debug("%s:%s max77833->i2c_lock.count.counter=%d\n", MUIC_DEV_NAME,
