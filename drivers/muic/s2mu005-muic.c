 ...
 *
 * Copyright (C) 2015-2017 Samsung Electronics
 *
...
static ssize_t s2mu005_muic_set_forced_open_usb(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	struct s2mu005_muic_data *muic_data = dev_get_drvdata(dev);
	...
	if (!strncmp(buf, "1", 1)) {
		...
		pr_info("%s:%s: forced usb path!\n", MUIC_DEV_NAME, __func__);
	}
	else if (!strncmp(buf, "0", 1)) {
		com_to_open(muic_data);
		pr_info("%s:%s: forced open path!\n", MUIC_DEV_NAME, __func__);
	}
	else
		pr_info("%s:%s: invalid value\n", MUIC_DEV_NAME, __func__);
...
static DEVICE_ATTR(forced_open_usb, 0220, NULL, s2mu005_muic_set_forced_open_usb);
...
static void s2mu005_muic_power_off(struct s2mu005_muic_data *muic_data, int on)
{
	...
	if (muic_data->muic_version >= 4) {
		if (on) {
			/* 0x25[0]=0 */
			ret = s2mu005_i2c_read_byte(i2c, 0x25);
			ret &= ~0x01;
			s2mu005_i2c_write_byte(i2c, 0x25, (u8)ret);
		} else {
			/* 0x25[0]=1 : default */
			ret = s2mu005_i2c_read_byte(i2c, 0x25);
			ret |= 0x01;
	...
	pr_info("%s:%s: 0x25:0x%x, rev_id(%d)\n", MUIC_DEV_NAME,
...
static void s2mu005_muic_detect_dev(struct s2mu005_muic_data *muic_data)
{
	...
	case DEV_TYPE1_CHG_TYPES: /* In case of reg DEV_TYPE1 has DCP & CDP */
...
#endif
	s2mu005_i2c_update_bit(i2c,
			S2MU005_REG_MUIC_RESERVED1,
			DETACH_TIME_SET_MASK,
			DETACH_TIME_SET_SHIFT,
			DETACH_TIME_SET_5MS);
	return ret;
}
...
static irqreturn_t s2mu005_muic_irq_thread(int irq, void *data)
{
	...
	if (!vbvolt) {
		...
		} else if (IS_WATER_ADC(adc) && !IS_JIG_ADC(adc) && !muic_data->is_water_wa) {
