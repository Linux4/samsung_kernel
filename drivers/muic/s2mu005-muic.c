...
#if IS_ENABLED(CONFIG_SEC_FACTORY)
int s2mu005_if_set_jig_ctrl_on(void *data)
{
	struct s2mu005_muic_data *muic_data = (struct s2mu005_muic_data *) data;
	struct i2c_client *i2c = muic_data->i2c;
	bool en = muic_data->pdata->is_jig_on; 
	int reg_val = 0, ret = 0;
...
void s2mu005_update_rustproof( \
		struct s2mu005_muic_data *muic_data, bool is_rustproof)
{
	muic_data->is_rustproof = is_rustproof;
}

bool s2mu005_get_rustproof(struct s2mu005_muic_data *muic_data)
{
	return muic_data->is_rustproof;
}
...
int s2mu005_if_get_adc(struct s2mu005_muic_data *muic_data)
{
	int ret;
...
static ssize_t adc_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret = s2mu005_if_get_adc((struct s2mu005_muic_data *) dev_get_drvdata(dev));
	
	return (ret < 0) ? sprintf(buf, "UNKNOWN\n") : sprintf(buf, "%x\n", ret);
}
...
void s2mu005_if_show_register(struct s2mu005_muic_data *muic_data, char *mesg)
{
	mutex_lock(&muic_data->muic_mutex);
	s2mu005_read_reg_dump(muic_data, mesg);
	mutex_unlock(&muic_data->muic_mutex);
	...
	struct device_attribute *attr, char *buf)
{
	...
	s2mu005_if_show_register((struct s2mu005_muic_data *) dev_get_drvdata(dev), mesg);
	pr_info("%s %s\n", __func__, mesg);
...
static int s2mu005_if_set_otg_reg(void *mdata, bool enable)
{
	...
	muic_data->is_otg_test = muic_data->pdata->is_otg_test;
...
muic_attached_dev_t s2mu005_get_attached_dev(struct s2mu005_muic_data *muic_data)
{
	return muic_data->attached_dev;
}
...
static int of_s2mu005_muic_dt(struct device *dev, struct s2mu005_muic_data *muic_data)
{
	...
	pr_err("%s new_factory = %d, dcd_timeout = %d\n", \
		__func__, pdata->is_new_factory, pdata->dcd_timeout);
					...
					struct muic_interface_t *muic_if)
{
	...
	muic_if->set_com_to_open = com_to_open;
	muic_if->set_switch_to_usb = com_to_usb;
	muic_if->set_com_to_otg = com_to_usb;
	muic_if->set_switch_to_uart = com_to_uart;
...
static int s2mu005_muic_probe(struct platform_device *pdev)
{
	...
	muic_if = devm_kzalloc(&pdev->dev, sizeof(*muic_if), GFP_KERNEL);
	if (unlikely(!muic_if)) {
		pr_err("failed to allocate if data\n");
		return PTR_ERR(muic_if);
	}

	muic_data->dev = &pdev->dev;
	muic_data->i2c = s2mu005->i2c;
	muic_data->mfd_pdata = mfd_pdata;
	muic_data->pdata = muic_core_init(muic_data);
	...
	ret = muic_sysfs_init(muic_data->pdata);
	if (ret) {
		pr_err("failed to create sysfs\n");
		goto err_sysfs;
	}
