...
#if defined(CONFIG_USBPD_S2MU004)
#include <linux/usbpd/usbpd-s2mu004.h>
#endif
...
static void s2mu004_muic_detect_dev(struct s2mu004_muic_data *muic_data)
{
	...
	if (muic_data->attach_mode == S2MU004_NONE_CABLE) {
		if (((adc & 0x1f) == 0) || vbvolt) {
			pr_err("[muic] change mode to first attach!\n");
			intr = MUIC_INTR_ATTACH;
			new_dev = ATTACHED_DEV_TYPE3_MUIC;
			muic_data->attach_mode = S2MU004_FIRST_ATTACH;
			muic_pdic_notifier_attach_attached_dev(new_dev);
			return;
		}
	} else if (!vbvolt)
		return;
...
#ifdef CONFIG_MUIC_S2MU004_NON_USB_C_TYPE
	pr_err("%s:%s s2mu004 non usb-c type\n", MFD_DEV_NAME, __func__);
	switch (val2) {
...
static irqreturn_t s2mu004_muic_irq_thread(int irq, void *data)
{
	...
	if (muic_data->attach_mode != S2MU004_FIRST_ATTACH)
...
#if defined(CONFIG_USBPD_S2MU004)
static void s2mu004_muic_init_check(struct work_struct *work)
{
	struct s2mu004_muic_data *muic_data =
		container_of(work, struct s2mu004_muic_data, init_check.work);
...
#if defined(CONFIG_USBPD_S2MU004)
	INIT_DELAYED_WORK(&muic_data->init_check, s2mu004_muic_init_check);
	schedule_delayed_work(&muic_data->init_check, msecs_to_jiffies(1000));
#else
	s2mu004_muic_irq_thread(-1, muic_data);
#endif
