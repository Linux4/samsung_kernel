...
int muic_request_disable_afc_state(void)
{
	...
	muic_data->voltage_down_completed = 1;
	muic_data->afc_active = 1;
...
int muic_done_disable_afc_state(void)
{
	...
	pr_info("[%s:%s]voltage_down_completed(%d)\n", MUIC_DEV_NAME, __func__, muic_data->voltage_down_completed);

	return muic_data->voltage_down_completed;
}
EXPORT_SYMBOL(muic_done_disable_afc_state);

int muic_get_afc_active(void)
{
	muic_data_t *muic_data = afc_init_data;

	pr_info("[%s:%s]afc_active(%d)\n", MUIC_DEV_NAME, __func__, muic_data->afc_active);

	return muic_data->afc_active;
}
EXPORT_SYMBOL(muic_get_afc_active);
...
int muic_check_fled_state(bool enable, u8 mode)
{
	...
	if ((muic_data->fled_torch_enable == false) && (muic_data->fled_flash_enable == false)) {
		if ((mode == FLED_MODE_TORCH) && (enable == false)) {
			...
			muic_data->voltage_down_completed = 0;
			muic_data->afc_active = 1;
...
int sm5713_muic_voltage_control(int afctxd, int qc20)
{
	...
	pr_info("[%s:%s] old_afctxd(0x%x), afctxd(0x%x), qc20(0x%x)\n", MUIC_DEV_NAME, __func__, muic_data->old_afctxd, afctxd, qc20);
...
static void muic_afc_prepare_work(struct work_struct *work)
{
	...
	for (retry = 0; retry < 10; retry++) {
		...
		if (value.intval <= 500) {
			...
			break;
		} else if (value.intval == 1650) {
			muic_data->attached_dev = ATTACHED_DEV_AFC_CHARGER_PREPARE_MUIC;
			muic_notifier_attach_attached_dev(ATTACHED_DEV_AFC_CHARGER_PREPARE_MUIC);
			pr_info("[%s:%s]PREPARE(RE) fail(%d), retry(%d)\n", MUIC_DEV_NAME, __func__, value.intval, retry);
		} else {
	...
	if (afc_prepare_qc20 == SM5713_MUIC_QC20) {
		...
		if (muic_data->qc20_vbus == SM5713_ENQC20_5V) {
			...
			if (muic_data->fled_torch_enable == 1) {
				muic_data->voltage_down_completed = 2;
			}
		...
		for (retry = 0; retry < 30 ; retry++) {
			msleep(50);
			irqafc = sm5713_i2c_read_byte(i2c, SM5713_MUIC_REG_INT2);
			if (irqafc & INT2_AFC_ACCEPTED_MASK) {
				pr_info("[%s:%s] AFC_ACCEPTED Success(0x%x), retry(%d)\n",
					MUIC_DEV_NAME, __func__, irqafc, retry);
				break;
			} else if (irqafc & INT2_AFC_ERROR_MASK) {
				/* ENAFC set '0' */
				...
				pr_info("[%s:%s] AFC_ERROR(0x%x), afcstatus(0x%x), retry(%d)\n",
					MUIC_DEV_NAME, __func__, irqafc, afcstatus, retry);

				/* DP_RESET '1' */
				sm5713_set_afc_ctrl_reg(muic_data, AFCCTRL_DP_RESET, 1);
				afc_dpreset = 1;
 
			} else if ( (irqafc & INT2_AFC_TA_ATTACHED_MASK) && (afc_dpreset == 1)) {
...
EXPORT_SYMBOL(muic_get_vbus_value);
...
int sm5713_afc_ta_attach(muic_data_t *muic_data)
{
	...
	if (muic_data->afc_abnormal_disable == 1) {
		pr_info("[%s:%s] CCIC Abnormal State\n", MUIC_DEV_NAME, __func__);
...
int sm5713_set_afc_ctrl_reg(muic_data_t *muic_data, int shift, bool on)
{
	...
	if (reg_val ^ ret) {
		...
		ret = sm5713_i2c_write_byte(i2c, SM5713_MUIC_REG_AFCCNTL, reg_val);
		if (ret < 0)
