...
static int muic_is_cc_supported_dev(struct max77854_muic_data *pmuic, muic_attached_dev_t new_dev);
#endif
...
#if defined(CONFIG_MUIC_SUPPORT_CCIC)
	if (muic_is_cc_supported_dev(muic_data, new_dev)) {
		pr_info("%s: %s: MUIC JIG or Legacy TA/USB detected.\n",MUIC_DEV_NAME,__func__);
#endif
		...
		pr_info("%s: %s: MUIC Not a JIG or Legacy TA/USB.\n",MUIC_DEV_NAME,__func__);
#endif
	return;
}
...
static int muic_is_cc_supported_dev(struct max77854_muic_data *pmuic, muic_attached_dev_t new_dev)
{
	...
	case ATTACHED_DEV_CDP_MUIC:
	case ATTACHED_DEV_TA_MUIC:
		pr_info("%s: %s: Legacy TA/USB(%d) discarded.\n",MUIC_DEV_NAME,__func__, new_dev);
...
/* Send ATTACH noti. only. */
	...
	case ATTACHED_DEV_JIG_UART_OFF_VB_MUIC:
	case ATTACHED_DEV_JIG_UART_OFF_VB_FG_MUIC:		
		pr_info("%s: attached_dev:%d Supported Dev.\n", __func__, pmuic->attached_dev);
		return 1;
	default:
