...
static int zram_balance_ratio = 5;	/* nand writeback ratio */
...
static bool zram_should_writeback(struct zram *zram, int bound)
{
	...
	int min_writtenback_ratio = zram_balance_ratio + bound;
	bool ret = true;
	...
	while (!kthread_should_stop()) {
		...
		list_for_each_entry_safe(zram_entry, n, &zram->list, lru_list) {
			...
			if (!zram_writeback_index(zram, index, page))
				count++;
			if (!zram_should_writeback(zram, 5))
				break;
		}
