			...
			if (dev->max_slave_count == 1) {
				...
				if (w1_read_block(dev, (u8 *)&rn, 8) == 8 && rn) {
					...
					if (rn && tmp->crc == w1_calc_crc8((u8 *)&rn_le, 7)) {
						...
						retry++;
						pr_info("%s: cmd READ_ROM crc wrong %d\n",
								__func__, retry);
						pr_info("%s : fid(%x) id(%llx) crc(%x)\n", __func__,
								(unsigned int) tmp->family,
								(unsigned long long) tmp->id,
