...
//#define gpio_direction_input(gpio) __msm_gpio_set_config_direction(gpio, 0, 0)
//#define gpio_direction_output(gpio, val) __msm_gpio_set_config_direction(gpio, 0, val)
#define gpio_set_value_msm(gpio, val) gpio_set_value(gpio, val)
#define gpio_get_value_msm(gpio) gpio_get_value(gpio)
...
static u8 w1_gpio_reset_bus(void *data)
{
	...
	for(i=0;i<15;i++)
		temp_read[i] = w1_gpio_read_bit_val(data) & 0x1;
...
static int w1_gpio_msm_probe(struct platform_device *pdev)
{
	...
	key_pinctrl = devm_pinctrl_get_select(&pdev->dev, "w1_gpio_pin_active");
