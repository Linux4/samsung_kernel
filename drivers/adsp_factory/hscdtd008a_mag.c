...
struct device_attribute *attr, char *buf)
{
	...
	if( !(data->data_ready_flag & 1 << ADSP_FACTORY_MODE_MAG) ){
		adsp_unicast(ADSP_FACTORY_MODE_MAG,NETLINK_MESSAGE_GET_RAW_DATA,0,0);
       pr_err("[FACTORY]: %s Inside if condition\n", __func__);
	}
	while( !(data->data_ready_flag & 1 << ADSP_FACTORY_MODE_MAG) )
		msleep(20);

  /* pr_err("[FACTORY]: %s Mag Raw Data Read %d %d %d \n", __func__, data->sensor_data[ADSP_FACTORY_MODE_MAG].x,
    data->sensor_data[ADSP_FACTORY_MODE_MAG].y, data->sensor_data[ADSP_FACTORY_MODE_MAG].z);*/
...
struct device_attribute *attr, char *buf)
{
	...
	adsp_unicast(ADSP_FACTORY_MODE_MAG,NETLINK_MESSAGE_SELFTEST_SHOW_DATA,0,0);
...
int mag_factory_receive_data(struct adsp_data *data)
{
	pr_err("(%s): factory \n", __func__);
	return 0;
}
