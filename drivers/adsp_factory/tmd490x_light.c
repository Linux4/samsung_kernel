...
#if defined(CONFIG_SUPPORT_BHL_COMPENSATION_FOR_LIGHT_SENSOR) || defined(CONFIG_SUPPORT_BRIGHT_SYSFS_COMPENSATION_LUX)
	...
	struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "27.17 6.24 2.4\n");
#elif defined(CONFIG_SEC_R5QSWA_PROJECT)
...
void light_factory_init_work(struct adsp_data *data)
{
	...
	efs_id_str = kzalloc(UB_CELL_ID_LENGTH, GFP_KERNEL);
