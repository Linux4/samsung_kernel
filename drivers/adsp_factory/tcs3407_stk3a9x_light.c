...
#define SUB_LIGHT_FACTORY_CAL_PATH "/efs/FactoryApp/sub_light_factory_cal_v4"
...
static int32_t prev_brightness_data[5] = {-1, -1, -1, -1, -1};
	...
	unsigned long val, void *v)
{
	...
	if (val == PANEL_EVENT_BL_CHANGED) {
		...
		if (data->brightness_info[0] == prev_brightness_data[0] &&
			data->brightness_info[2] == prev_brightness_data[2] &&
			data->brightness_info[3] == prev_brightness_data[3] &&
			data->brightness_info[4] == prev_brightness_data[4])
			return 0;

		prev_brightness_data[0] = data->brightness_info[0];
		prev_brightness_data[1] = data->brightness_info[1];
		prev_brightness_data[2] = data->brightness_info[2];
		prev_brightness_data[3] = data->brightness_info[3];
		prev_brightness_data[4] = data->brightness_info[4];
		...
		int panel_state = (int)evdata->state;
		int idx = evdata->display_idx;
		int32_t msg_buf[4];

		if ((panel_state >= MAX_PANEL_STATE) ||
			(data->pre_panel_state == panel_state &&
			data->pre_panel_idx == idx))
			return 0;
		...
		mutex_lock(&data->light_factory_mutex);
		pr_info("[FACTORY] %s: panel_state %d %d, mode %d\n",
		...
		int test_state = (int)test_data->state;
		int idx = test_data->display_idx;
		int32_t msg_buf[3];

		if ((idx > 0) || (data->pre_test_state == test_state))
			return 0;

		data->pre_test_state = (int32_t)test_state;
		msg_buf[0] = OPTION_TYPE_SET_PANEL_TEST_STATE;
		msg_buf[1] = (int32_t)test_state;
		msg_buf[2] = (int32_t)idx;

		mutex_lock(&data->light_factory_mutex);
		pr_info("[FACTORY] %s: panel test state %d %d\n",
			__func__, (int)test_data->state, test_data->display_idx);
		...
		int screen_mode = screen_data->mode;
		int idx = screen_data->display_idx;
		int32_t msg_buf[3];

		if (data->pre_screen_mode == screen_mode)
			return 0;

		data->pre_screen_mode = (int32_t)screen_mode;
		msg_buf[0] = OPTION_TYPE_SET_PANEL_SCREEN_MODE;
		msg_buf[1] = (int32_t)screen_mode;
		msg_buf[2] = (int32_t)idx;

		mutex_lock(&data->light_factory_mutex);
		pr_info("[FACTORY] %s: panel screen mode %d %d\n",
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (data->fac_fstate == FSTATE_INACTIVE || data->fac_fstate == FSTATE_FAC_INACTIVE)
		return snprintf(buf, PAGE_SIZE, "80.933 3.596 2.5\n");
	else
		return snprintf(buf, PAGE_SIZE, "12.42 7.53 2.8\n");
#else
	return snprintf(buf, PAGE_SIZE, "80.933 3.596 2.5\n");
#endif
#else
...
void light_cal_read_work_func(struct work_struct *work)
{
	...
	light_load_ub_cell_id_from_file(SUB_UB_CELL_ID_PATH, cur_data_str);
...
void light_cal_init_work(struct adsp_data *data)
{
	...
	if (PTR_ERR(cal_filp) == -ENOENT || PTR_ERR(cal_filp) == -ENXIO) {
		...
		temp_str = kzalloc(LIGHT_UB_CELL_ID_INFO_STRING_LENGTH,
			GFP_KERNEL);
		light_save_ub_cell_id_to_efs(LIGHT_UB_CELL_ID_PATH, temp_str, true);
		kfree(temp_str);
	} else if (IS_ERR(cal_filp)) {
		...
		ret = vfs_read(cal_filp, (char *)data->light_ub_id,
			sizeof(char) * LIGHT_UB_CELL_ID_INFO_STRING_LENGTH,
			&cal_filp->f_pos);
		data->light_ub_id[LIGHT_UB_CELL_ID_INFO_STRING_LENGTH - 1] = '\0';
		if (ret < 0)
	...
	if (PTR_ERR(cal_filp) == -ENOENT || PTR_ERR(cal_filp) == -ENXIO) {
		...
		temp_str = kzalloc(LIGHT_UB_CELL_ID_INFO_STRING_LENGTH,
			GFP_KERNEL);
		light_save_ub_cell_id_to_efs(SUB_LIGHT_UB_CELL_ID_PATH, temp_str, true);
		kfree(temp_str);
	} else if (IS_ERR(cal_filp)) {
		...
		ret = vfs_read(cal_filp, (char *)data->sub_light_ub_id,
			sizeof(char) * LIGHT_UB_CELL_ID_INFO_STRING_LENGTH,
			&cal_filp->f_pos);
		data->sub_light_ub_id[LIGHT_UB_CELL_ID_INFO_STRING_LENGTH - 1] = '\0';
		if (ret < 0)
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (new_value == 1) {
		...
		if (light_idx == MSG_LIGHT) {
			...
			light_save_ub_cell_id_to_efs(LIGHT_UB_CELL_ID_PATH, cur_id_str, false);
		} else {
			light_load_ub_cell_id_from_file(SUB_UB_CELL_ID_PATH, cur_id_str);
			light_save_ub_cell_id_to_efs(SUB_LIGHT_UB_CELL_ID_PATH, cur_id_str, false);
		}
		kfree(cur_id_str);
	} else {
		int32_t panel_msg_buf[3] = {0, };
		mutex_lock(&data->light_factory_mutex);
		if (data->pre_panel_state >= 0) {
			panel_msg_buf[0] = OPTION_TYPE_SET_PANEL_STATE;
			panel_msg_buf[1] = data->pre_panel_state;
			panel_msg_buf[2] = data->pre_panel_idx;
			adsp_unicast(panel_msg_buf, sizeof(panel_msg_buf),
				MSG_DDI, 0, MSG_TYPE_OPTION_DEFINE);

			while (!(data->ready_flag[MSG_TYPE_OPTION_DEFINE] & 1 << MSG_DDI) &&
				cnt++ < TIMEOUT_CNT)
				usleep_range(500, 550);

			data->ready_flag[MSG_TYPE_OPTION_DEFINE] &= ~(1 << MSG_DDI);
		}
		...
		if (cnt >= TIMEOUT_CNT)
			...
			pr_info("[FACTORY] %s: set br %d, panel state %d(%d)\n",
				__func__, data->msg_buf[MSG_DDI][0],
				data->pre_panel_state, data->pre_panel_idx);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	msg_buf[0] = OPTION_TYPE_SET_HYST;
...
static int __init tcs3407_light_factory_init(void)
{
...
static void __exit tcs3407_light_factory_exit(void)
{
...
module_init(tcs3407_light_factory_init);
module_exit(tcs3407_light_factory_exit);
