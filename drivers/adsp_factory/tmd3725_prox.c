...
#define PROX_DETECT_HIGH_TH  250
#define PROX_DETECT_LOW_TH   130

struct tmd3725_prox_data {
...
static struct tmd3725_prox_data *pdata;
	...
	struct device_attribute *attr, char *buf)
{
	...
	int offset = (int)data->sensor_data[ADSP_FACTORY_PROX].offset;
...
static ssize_t prox_default_trim_check_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct adsp_data *data = dev_get_drvdata(dev);
	...
	struct device_attribute *attr, char *buf)
{
	...
	uint8_t p_gain = 0, p_rate = 0, p_pulse_len = 0;
	uint8_t l_atime = 0;
	int offset = 0;
	...
	high_th = (u16) prox_reg_buf[0x88 - PROX_REG_START];
	low_th = (u16) prox_reg_buf[0x8A - PROX_REG_START];
	hd_th_high = PROX_DETECT_HIGH_TH;
	hd_th_low = PROX_DETECT_LOW_TH;
	p_drive_cur = prox_reg_buf[0x8F - PROX_REG_START];
	per_time = prox_reg_buf[0x8C - PROX_REG_START];
	p_pulse = prox_reg_buf[0x8E - PROX_REG_START];
	p_gain = prox_reg_buf[0x8F - PROX_REG_START];
	p_rate = prox_reg_buf[0x82 - PROX_REG_START];
	p_pulse_len = prox_reg_buf[0x8E - PROX_REG_START];
	l_atime = prox_reg_buf[0x81 - PROX_REG_START];
	offset = prox_reg_buf[0xC1 - PROX_REG_START] == 1 ?
		-prox_reg_buf[0xC0 - PROX_REG_START] :
		prox_reg_buf[0xC0 - PROX_REG_START];

exit:
	pr_info("[FACTORY] %s - %02x, %d, %d, %d, %d, %02x, %02x, %02x, %02x, %02x, %02x, %02x, %d",
		__func__, chipId, high_th, low_th, hd_th_high, hd_th_low,
		p_drive_cur, per_time, p_pulse, p_gain, p_rate,
	...
	return snprintf(buf, PAGE_SIZE, "\"THD\":\"%d %d %d %d\","\
		...
		p_gain, p_rate, p_pulse_len, l_atime, offset);
}
...
static DEVICE_ATTR(prox_trim_check, S_IRUGO, prox_default_trim_check_show, NULL);
...
static int __init tmd3725_prox_factory_init(void)
{
...
static void __exit tmd3725_prox_factory_exit(void)
{
...
module_init(tmd3725_prox_factory_init);
module_exit(tmd3725_prox_factory_exit);
