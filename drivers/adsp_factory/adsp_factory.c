...
int adsp_factory_register(char *dev_name,int type, struct device_attribute *attributes[],struct adsp_fac_ctl *fact_ctl )
{
...
#endif
	data->sensor_device[type] = device_create(sensors_factory_class, NULL, 0, data, "%s", dev_name);
	...
	return ret;
}int adsp_factory_start_timer( const unsigned int ms ){	return mod_timer(&data->command_timer,							jiffies + msecs_to_jiffies(ms));}
static int process_received_msg(struct sk_buff *skb, struct nlmsghdr *nlh)
{
	//pr_err("(%s): factory nlh->nlmsg_type = %d \n", __func__,nlh->nlmsg_type);
	switch (nlh->nlmsg_type)
	{
	case NETLINK_MESSAGE_RAW_DATA_RCVD:
		{
			struct sensor_value  *pdata = (struct sensor_value*)NLMSG_DATA(nlh);
			pr_err("NETLINK_MESSAGE_RAW_DATA_RCVD type=%d,x=%d, y=%d,z=%d",pdata->sensor_type,pdata->x,pdata->y,pdata->z);
		...
		{
			struct sensor_value  *pdata = (struct sensor_value*)NLMSG_DATA(nlh);
			pr_err("NETLINK_MESSAGE_REACTIVE_ALERT_RCVD type=%d,motion=%d",pdata->sensor_type,pdata->reactive_alert);
	...
	case NETLINK_MESSAGE_CALIB_DATA_RCVD:
		{
			struct sensor_calib_value  *pdata = (struct sensor_calib_value*)NLMSG_DATA(nlh);
			pr_err(" NETLINK_MESSAGE_CALIB_DATA_RCVD type=%d,x=%d, y=%d,z=%d",pdata->sensor_type,pdata->x,pdata->y,pdata->z);
	...
	case NETLINK_MESSAGE_CALIB_STORE_RCVD:
		{
			struct sensor_calib_store_result *pdata = (struct sensor_calib_store_result*)NLMSG_DATA(nlh);
			pr_err(" NETLINK_MESSAGE_CALIB_STORE_RCVD type=%d, nCalibstoreresult=%d",pdata->sensor_type,pdata->nCalibstoreresult);
	...
	case NETLINK_MESSAGE_SELFTEST_SHOW_RCVD:
		{
			struct sensor_selftest_show_result *pdata = (struct sensor_selftest_show_result*)NLMSG_DATA(nlh);
			pr_err("NETLINK_MESSAGE_SELFTEST_SHOW_RCVD type=%d, SelftestResult1=%d, SelftestResult2=%d",pdata->sensor_type,pdata->nSelftestresult1, pdata->nSelftestresult2);
    ...
    case NETLINK_MESSAGE_STOP_RAW_DATA:
		{
			struct sensor_stop_value *pdata = (struct sensor_stop_value*)NLMSG_DATA(nlh);		    pr_err("NETLINK_MESSAGE_STOP_RAW_DATA type=%d, StopResult=%d ",pdata->sensor_type,pdata->result);	        data->sensor_stop_data[pdata->sensor_type] = *pdata;
