...
#include "adsp.h"
/* The netlink socket. */
//static struct sock *factory_sock;
...
//			add_timer(&hba->destroy_timer);    //do nothing
...
int adsp_unicast( int sensor_type , int message, int flags, u32 pid)
{
	...
	//NLA_PUT_U32(skb, NETLINK_ATTR_SENSOR_TYPE, sensor_type);
	ret = nlmsg_unicast(data->adsp_skt, skb, PID);
	return ret;
	//nla_put_failure:
	///	pr_err("(%s): nla_put_failure reached \n",  __func__);
...
int adsp_factory_register(char *dev_name,int type, struct device_attribute *attributes[],struct adsp_fac_ctl *fact_ctl )
{
	...
	data->sensor_device[type] = device_create(sensors_class, NULL, 0, data, "%s", dev_name);
	...
	data->ctl[type]->receive_data_fnc(data);
	return ret;
}
