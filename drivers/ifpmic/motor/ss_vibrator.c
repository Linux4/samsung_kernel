...
int32_t vibe_set_pwm_freq_gp0clk(int intensity)
{
	int32_t calc_d;
	int32_t calc_n, half_n;
	int regval = 0;

	regval = readl_relaxed(MMSS_CAMSS_GP0_N);			
	calc_n = regval;
	regval = readl_relaxed(MMSS_CAMSS_GP0_D);			
	calc_d = regval;
	regval = readl_relaxed(MMSS_CAMSS_GP0_CMD_RCGR);			
	...
	writel_relaxed(calc_n, MMSS_CAMSS_GP0_N);
	mb();
	writel_relaxed(calc_d, MMSS_CAMSS_GP0_D);
	mb();
	writel_relaxed(0x3, MMSS_CAMSS_GP0_CMD_RCGR);
	mb();

	regval = readl_relaxed(MMSS_CAMSS_GP0_N);			
	pr_info("[VIB]: %s MMSS_CAMSS_GP0_N: %x\n", __func__, regval);		
	regval = readl_relaxed(MMSS_CAMSS_GP0_D);			
	pr_info("[VIB]: %s MMSS_CAMSS_GP0_D: %x\n", __func__, regval);
	regval = readl_relaxed(MMSS_CAMSS_GP0_CMD_RCGR);
	pr_info("[VIB]: %s MMSS_CAMSS_GP0_CMD_RCGR: %x\n", __func__, regval);
...
int32_t vibe_set_pwm_freq(int intensity)
{
	...
	int32_t calc_n, half_n;
        ...
        if (!g_vib || !g_vib->vib_pwm_gpio)
                return VIBRATION_FAIL;

        if (g_vib->vib_pwm_gpio == 24) //gp_clk0 case
		return vibe_set_pwm_freq_gp0clk(intensity);
...
int32_t vibe_pwm_onoff_gp0clk(u8 onoff)
{
        int ret = 0;

	if (g_vib->f_packet_en) {
		pr_info("[VIB]: packet playing, skip clk control\n");
		return VIBRATION_SUCCESS;
	}

	if (onoff) {
            ret = clk_set_rate(g_vib->pclk, 20000); 
                if (ret < 0)
                        pr_err("[VIB] %s: Can't set pwm_clk rate 20MHz (%d)\n", __func__, ret); 

                ret = clk_prepare_enable(g_vib->pclk); 
                if (ret < 0)
                        pr_err("[VIB] %s: Can't enable pwm_clk %d\n", __func__, ret); 

                ret = clk_prepare_enable(g_vib->cbcr_clk); 
                if (ret < 0)
                        pr_err("[VIB] %s: Can't enable pwm_clk-cbcr %d\n", __func__, ret); 

                pr_info("[VIB]: %s pwm clk on\n", __func__);

	} else {
                clk_disable_unprepare(g_vib->pclk); 
                clk_disable_unprepare(g_vib->cbcr_clk); 
                pr_info("[VIB]: %s pwm clk off\n", __func__);
	}
...
int32_t vibe_pwm_onoff(u8 onoff)
{
        ...
        if (g_vib->vib_pwm_gpio == 24) //gp_clk0 case
       		return vibe_pwm_onoff_gp0clk(onoff);
...
static void vibrator_enable(struct timed_output_dev *dev, int value)
{
	...
	pr_info("[VIB]: enable before lock\n");
	mutex_lock(&vib->lock);
	pr_info("[VIB]: enable after lock\n");
	...
	pr_info("[VIB]: enable before unlock\n");
	mutex_unlock(&vib->lock);
	pr_info("[VIB]: enable after unlock\n");
...
static int vibrator_parse_dt(struct ss_vib *vib)
{
	...
	pr_info("%s vib_pwm_gpio :%d\n", __func__, vib->vib_pwm_gpio);
...
static int vibrator_get_pwm_clk_src(struct ss_vib *vib)
{

        int ret = 0;
        pr_err("[PWM]: %s\n", __func__);
        vib->pclk = devm_clk_get(vib->dev, "motor-pwm-clk"); 
        if (IS_ERR(vib->pclk)) {
            pr_err("[VIB] %s: Get pwm-clk error\n", __func__); 
            return -EINVAL;
        }

        vib->cbcr_clk = devm_clk_get(vib->dev, "motor-pwm-clk-cbcr"); 
        if (IS_ERR(vib->cbcr_clk)) {
            pr_err("[VIB] %s: Get pwm-clk-cbcr error\n", __func__); 
            return -EINVAL;
        }
...
static int ss_vibrator_probe(struct platform_device *pdev)
{
	...
	if (rc)
        ...
        if (vib->vib_pwm_gpio == 24) {  //gp_clk0 case
		rc = vibrator_get_pwm_clk_src(vib);
		if (rc) {
        	        pr_err("[VIB]: %s failed, get clk src error", __func__);
        	        return rc;
		}
		virt_mmss_gp0_base = ioremap(vib->gp_clk, 0x14);
		if (!virt_mmss_gp0_base) {
        	        pr_err("[VIB]: Unable to ioremap MSM_MMSS_GP0 memory!");
			return -ENOMEM;
		}
	} else {                        //default gp_clk1 case 
		virt_mmss_gp1_base = ioremap(vib->gp_clk, 0x28);
		if (!virt_mmss_gp1_base) {
        	        pr_err("[VIB]: Unable to ioremap MSM_MMSS_GP1 memory!");
			return -ENOMEM;
		}
	}
...
#if 0   // temporary block for smd
        if (vib->gp_clk_sel)   // gp_clk1 case
	...
	rc = sysfs_create_file(&vib->timed_dev.dev->kobj, &dev_attr_motor_type.attr);
	if (rc < 0)
