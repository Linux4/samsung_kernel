...
void qpnb_update(bool on);
struct qpnp_flash_led *g_led = NULL;
...
void qpnb_update(bool on)
{
		...
		if (on) {
			pr_err("Updating.....ON.");
			//LED3
			snode = (struct flash_switch_data *) &g_led->snode[1];
			fnode = (struct flash_node_data *) &g_led->fnode[LED3];
			qpnp_flash_led_node_set(fnode, value);
			qpnp_flash_led_switch_set(snode, value > 0);
		} else {
			pr_err("Updating.....OFF.");
			//LED3
			snode = (struct flash_switch_data *) &g_led->snode[1];
			qpnp_flash_led_switch_set(snode, 0);
		}
        }
}
EXPORT_SYMBOL(qpnb_update);
