...
#elif defined(CONFIG_SEC_DREAMLITEQLTE_PROJECT)
/* userspace possible val {1001,1002,1003,1004,1005,1006,1007,1008,1009,1010} */
/* brigtness levels  	  {lvl1,lvl2,____,lvl3,____,lvl4,____,____,lvl5,____} */
/* register settings      {0x00,0x01,0x02,0x02,0x04,0x04,0x06,0x07,0x06,0x09};*/
...
void ktd2692_flash_on(unsigned data)
{
	...
	struct pinctrl *pinctrl;
        ...
        if (!gpio_is_valid(global_ktd2692data->flash_control)) {
                pr_err("failed to get flash_control\n");
                return;
        }
			...
			size_t count)
{
	...
	if (value <= 0) {
		...
		if (IS_ERR(pinctrl))
			pr_err("%s: flash %s pins are not configured\n", __func__, "is");
	} else if((value == 100) || (value == 200) || (value == 1)){
		pinctrl = devm_pinctrl_get_select(ktd2692_dev, "fled_default");
		if (IS_ERR(pinctrl))
			pr_err("%s: flash %s pins are not configured\n", __func__, "host");
		ret = gpio_request(global_ktd2692data->flash_control, "ktd2692_led_control");
		if (ret) {
			...
			spin_lock_irqsave(&global_ktd2692data->int_lock, flags);
			ktd2692_write_data(global_ktd2692data->LVP_Voltage|KTD2692_ADDR_LVP_SETTING);
#if 1	/* use the internel defualt setting */
			ktd2692_write_data(global_ktd2692data->flash_timeout|KTD2692_ADDR_FLASH_TIMEOUT_SETTING);
#endif
                                ...
                                struct ktd2692_platform_data *pdata)
{
	...
	pdata->flash_timeout = KTD2692_DISABLE_TIMER;	/* default */
...
#elif defined(CONFIG_SEC_J3Y17QLTE_PROJECT) || defined(CONFIG_SEC_J2Y18LTE_PROJECT)
	pdata->movie_current_value = KTD2692_MOVIE_CURRENT9;
#else
	pdata->movie_current_value = KTD2692_MOVIE_CURRENT7;
#endif
	pdata->factory_movie_current_value = KTD2692_MOVIE_CURRENT9;
