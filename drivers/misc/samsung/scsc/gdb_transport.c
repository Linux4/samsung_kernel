...
int gdb_transport_init(struct gdb_transport *gdb_transport, struct scsc_mx *mx, enum gdb_transport_enum type)
{
	...
	if (type == GDB_TRANSPORT_FXM_1)
		...
		r = mif_stream_init(&gdb_transport->mif_istream, SCSC_MIF_ABS_TARGET_PMU, MIF_STREAM_DIRECTION_IN, num_packets, packet_size, mx, MIF_STREAM_INTRBIT_TYPE_ALLOC, gdb_input_irq_handler, gdb_transport);
	else if (type == GDB_TRANSPORT_FXM_3)
		r = mif_stream_init(&gdb_transport->mif_istream, SCSC_MIF_ABS_TARGET_FXM_3, MIF_STREAM_DIRECTION_IN, num_packets, packet_size, mx, MIF_STREAM_INTRBIT_TYPE_ALLOC, gdb_input_irq_handler, gdb_transport);
	else if (type == GDB_TRANSPORT_WLAN_2)
		r = mif_stream_init(&gdb_transport->mif_istream, SCSC_MIF_ABS_TARGET_WLAN_2, MIF_STREAM_DIRECTION_IN, num_packets, packet_size, mx, MIF_STREAM_INTRBIT_TYPE_ALLOC, gdb_input_irq_handler, gdb_transport);
	else if (type == GDB_TRANSPORT_WLAN_3)
		r = mif_stream_init(&gdb_transport->mif_istream, SCSC_MIF_ABS_TARGET_WLAN_3, MIF_STREAM_DIRECTION_IN, num_packets, packet_size, mx, MIF_STREAM_INTRBIT_TYPE_ALLOC, gdb_input_irq_handler, gdb_transport);
	else if (type == GDB_TRANSPORT_WLAN_4)
		r = mif_stream_init(&gdb_transport->mif_istream, SCSC_MIF_ABS_TARGET_WLAN_4, MIF_STREAM_DIRECTION_IN, num_packets, packet_size, mx, MIF_STREAM_INTRBIT_TYPE_ALLOC, gdb_input_irq_handler, gdb_transport);
#endif
	else
	...
	if (type == GDB_TRANSPORT_FXM_1)
		...
		r = mif_stream_init(&gdb_transport->mif_ostream, SCSC_MIF_ABS_TARGET_PMU, MIF_STREAM_DIRECTION_OUT, num_packets, packet_size, mx, MIF_STREAM_INTRBIT_TYPE_RESERVED, gdb_output_irq_handler, gdb_transport);
	else if (type == GDB_TRANSPORT_FXM_3)
		r = mif_stream_init(&gdb_transport->mif_ostream, SCSC_MIF_ABS_TARGET_FXM_3, MIF_STREAM_DIRECTION_OUT, num_packets, packet_size, mx, MIF_STREAM_INTRBIT_TYPE_RESERVED, gdb_output_irq_handler, gdb_transport);
	else if (type == GDB_TRANSPORT_WLAN_2)
		r = mif_stream_init(&gdb_transport->mif_ostream, SCSC_MIF_ABS_TARGET_WLAN_2, MIF_STREAM_DIRECTION_OUT, num_packets, packet_size, mx, MIF_STREAM_INTRBIT_TYPE_RESERVED, gdb_output_irq_handler, gdb_transport);
	else if (type == GDB_TRANSPORT_WLAN_3)
		r = mif_stream_init(&gdb_transport->mif_ostream, SCSC_MIF_ABS_TARGET_WLAN_3, MIF_STREAM_DIRECTION_OUT, num_packets, packet_size, mx, MIF_STREAM_INTRBIT_TYPE_RESERVED, gdb_output_irq_handler, gdb_transport);
	else if (type == GDB_TRANSPORT_WLAN_4)
		r = mif_stream_init(&gdb_transport->mif_ostream, SCSC_MIF_ABS_TARGET_WLAN_4, MIF_STREAM_DIRECTION_OUT, num_packets, packet_size, mx, MIF_STREAM_INTRBIT_TYPE_RESERVED, gdb_output_irq_handler, gdb_transport);
#endif
	else
