...
static int pcie_s5e9935_filter_set_param_cb(const char *buffer, const struct kernel_param *kp)
{
	int ret;
	u32 value;
...
static struct kernel_param_ops pcie_s5e9935_filter_ops = {
	.set = pcie_s5e9935_filter_set_param_cb,
	.get = NULL,
};

module_param_cb(pcie_write_ramp, &pcie_s5e9935_filter_ops, NULL, S_IRUGO | S_IWUSR);
...
{
	int irq_vec;
#if 0 /* MSI affinity is not supported (yet) */
	int i;
...
{
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Getting address of scoreboard 0x%lx\n", RAMRP_HOSTIF_PMU_PCIE_CONFIG_SCOREBOARD);
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "base 0x%lx\n", (uintptr_t)pcie->registers_ramrp);
	return pcie->registers_ramrp + RAMRP_HOSTIF_PMU_PCIE_CONFIG_SCOREBOARD;
}
...
int pcie_get_scoreboard_ref(struct pcie_mif *pcie, __iomem void *addr, uintptr_t *addr_ref)
{
	...
	if (addr < pcie->registers_ramrp)
		return -EIO;

	*addr_ref = (uintptr_t)addr - (uintptr_t)pcie->registers_ramrp;
...
static int pcie_mif_enable_msi(struct pcie_mif *pcie)
{
	...
	for (i = 0; i < NUM_TO_HOST_IRQ_PMU; i++, acc++) {
		...
		ret = devm_request_irq(&pcie->pdev->dev, pcie->pmu_msi[i].msi_vec, pcie_mif_pmu_isr, 0, DRV_NAME, pcie);
		if (ret) {
	...
	pcie->msi_irq_base = pcie->pdev->irq;
...
static void pcie_mif_disable_msi(struct pcie_mif *pcie)
{
	...
	if (pcie->msi_irq_base > 0)
		disable_irq(pcie->msi_irq_base);

	while (i < NUM_TO_HOST_IRQ_TOTAL) {
...
static int pcie_mif_module_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (pcie->pci_saved_configs) {
		pr_info("[%s] wlbt pcie saved configs is not NULL(restart)\n", __func__);
...
void pcie_register_driver(void)
{
	...
	if(ret){
		pr_info("wlbt pcie driver register failed\n");
	}
}
...
static void pcie_mif_save_state(struct pci_dev *pdev, int ch_num)
{
	...
	if (pcie_mif_check_link_status(ch_num) == 0) {
		pr_err("[%s] It's not Linked - Ignore save state!!!\n", __func__);
		return;
	}
...
static void pcie_mif_restore_state(struct pci_dev *pdev, int ch_num)
{
	...
	if (pcie_mif_check_link_status(ch_num) == 0) {
		pr_err("[%s] It's not Linked - Ignore restore state!!!\n", __func__);
		return;
	}

	if (pci_set_power_state(pdev, PCI_D0))
		pr_err("[%s] Can't set D0 state!\n", __func__);

	if (!pcie->pci_saved_configs)
		pr_err("[%s] wlbt pcie saved configs is NULL\n", __func__);
...
int pcie_mif_poweron(struct pcie_mif *pcie)
{
	...
	if (pcie_is_initialized()) {
		SCSC_TAG_INFO(PCIE_MIF, "trying to restore state\n");
		pcie_mif_restore_state(pcie->pdev, pcie->ch_num);
	}

	if (pcie->msi_irq_base_enabled == 0) {
		enable_irq(pcie->msi_irq_base);
		pcie->msi_irq_base_enabled = 1;
	}
...
void pcie_mif_poweroff(struct pcie_mif *pcie)
{
	...
	if (pcie->msi_irq_base_enabled == 1) {
		disable_irq(pcie->msi_irq_base);
		pcie->msi_irq_base_enabled = 0;
	}

	pcie_mif_save_state(pcie->pdev, pcie->ch_num);

	mdelay(10); /* It needs delay for entering L1.2 before being L2 state */
