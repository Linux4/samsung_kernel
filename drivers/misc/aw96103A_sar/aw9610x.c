...
#define AW9610X_DRIVER_VERSION "v1.0.0"
...
static RAW_NOTIFIER_HEAD(sdcard_insert);

struct aw9610x *aw9610x_ptr;
char *sar_names = NULL;
module_param(sar_names, charp, 0644);
...
static struct aw9610x *cali_aw9610x;
...
/*Tab A7 T618 code for SR-AX6189A-01-182 by hujincan at 2021/12/24 start*/
static void manual_offset_calibration()
{
    uint32_t data_en = 0;
    AWLOGE(cali_aw9610x->dev, "start aw manual calibration.");
    aw9610x_i2c_read(cali_aw9610x, REG_SCANCTRL0, &data_en);
    aw9610x_i2c_write_bits(cali_aw9610x, REG_SCANCTRL0, ~(0x3f << 8),
                            (data_en & 0x3f) << 8);
}
/*Tab A7 T618 code for SR-AX6189A-01-182 by hujincan at 2021/12/24 end*/
...
int call_sar_notifiers(unsigned long val, void *v)
{
    return raw_notifier_call_chain(&sdcard_insert, val, v);
}

/* define sdcard insert notifier_chain_register func */
 static int register_sar_notifier(struct notifier_block *nb)
{
    int err = 0;
    err = raw_notifier_chain_register(&sdcard_insert, nb);
...
int sar_sdcrad_callback_func(struct notifier_block *nb, unsigned long event, void *v)
{
    switch (event) {
    case SDCARD_INSERT_EVENT:
        manual_offset_calibration();
        break;
...
static struct notifier_block sdcrad_insert_notifier = {
    .notifier_call = sar_sdcrad_callback_func,
};
                        ...
                        const char *buf, size_t count)
{
    ...
    if (state != 0) {
        aw9610x_i2c_write_bits(aw9610x, REG_SCANCTRL0, ~(0x3f << 8),
                            (data_en & 0x3f) << 8);
        AWLOGE(aw9610x->dev, "aot cali success");
    } else {
        AWLOGE(aw9610x->dev, "fail to set aot cali");
    }
    return count;
...
bool aw9610x_is_all_channle_off(struct aw9610x *aw9610x)
{
    int i = 0;
    int j = 0;
    j = aw9610x->sar_num;
    for (i = 0; i < AW_CHANNEL_MAX; i++) {
        if (aw9610x->is_channle_enable[j * AW_CHANNEL_MAX + i])
            return false;
    }
    return true;
}

/* Tab A7 T618 code for SR-AX6189A-01-264 by zhangziyi at 2021/12/27 start */
/* Tab A7 Lite T618 code for AX6189DEV-81 by hujincan at 2021/12/31 start */
                        ...
                        const char *buf, size_t count)
{
    ...
    if (sscanf(buf, "%d %d", &databuf[0], &databuf[1]) == 2) {
        AWLOGE(cali_aw9610x->dev, "value:%d %d received.", databuf[0], databuf[1]);
        if (databuf[0] == 177) {
            databuf[0] = 2;
        ...
        if (is_old_all_ch_off && !is_new_all_ch_off) {//power on
            ...
            aw9610x->mode = AW9610X_ACTIVE_MODE;
            AWLOGE(cali_aw9610x->dev, "grip sensor enable");
        }
#ifndef HQ_FACTORY_BUILD
        ...
        if (!is_old_all_ch_off && is_new_all_ch_off) {//power off
            ...
            aw9610x_i2c_write(aw9610x, REG_CMD, AW9610X_SLEEP_MODE);
#else
            aw9610x_i2c_read(aw9610x, REG_SCANCTRL1, &reg_val);
            aw9610x_i2c_write(aw9610x, REG_SCANCTRL1, reg_val | AW9610X_DOZEN_MODE);
#endif
            aw9610x->mode = AW9610X_SLEEP_MODE;
            AWLOGE(cali_aw9610x->dev, "grip sensor disable");
        }
    } else {
...
/* Tab A7 Lite T618 code for AX6189DEV-81 by hujincan at 2021/12/31 end */
/* Tab A7 T618 code for SR-AX6189A-01-264 by zhangziyi at 2021/12/27 end */
                ...
                struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < AW_CHANNEL_MAX; i++) {
        len += snprintf(buf + len, PAGE_SIZE - len,
                "channle %d: %s\n",
                j * AW_CHANNEL_MAX + i,
                aw9610x->is_channle_enable[j * AW_CHANNEL_MAX + i]? "enable":"disable");
    }
...
/* channel enable ctl */
static DEVICE_ATTR(enable, 0664, aw9610x_enable_get,
                        aw9610x_enable_set);
static DEVICE_ATTR(batch, 0664, NULL,
                        aw9610x_batch_store);
...
static ssize_t aw9610x_onoff_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct aw9610x *this = aw9610x_ptr;
...
/*Tab A7 T618 code for SR-AX6189A-01-182 by hujincan at 2021/12/24 start*/
static int aw96103_sar_calibration_notifier_callback(struct notifier_block *nb,unsigned long val, void *v)
{
    struct power_supply *psy_battery = NULL;
    union power_supply_propval battery_prop;
    int ret = 0;
#ifdef HQ_FACTORY_BUILD
    uint8_t no_charging = 0;
#else
    uint8_t no_charging = 1;
#endif
    psy_battery = power_supply_get_by_name("battery");
    if (NULL != psy_battery) {
        if (!strcmp(psy_battery->desc->name, "battery")) {
            ret = power_supply_get_property(psy_battery, POWER_SUPPLY_PROP_ONLINE, &battery_prop);
            if (ret) {
                 AWLOGE(cali_aw9610x->dev, "power_supply_get_property error");
                return 0;
            }
            if ((battery_prop.intval != no_charging) && (!haved_calibrator)) {
                if (sar_work != NULL) {
                    queue_work(sar_work, &sx_sar_enable_work);
                }
                haved_calibrator = true;
            } else if (battery_prop.intval == no_charging) {
                haved_calibrator = false;
            }
        }
    } else {
        AWLOGE(cali_aw9610x->dev, "psy_battery is null");
        return 0;
    }
    return 0;
}
...
void aw96103_sar_usb_callback_init(struct aw9610x *aw9610x)
{
    int ret = 0;
    sar_work = create_singlethread_workqueue("aw_sar_calibration_wq");
    if (!sar_work) {
        return;
    }
    ...
    if (ret < 0)
        AWLOGE(aw9610x->dev, "power_supply_reg_notifier:error:%d",ret);
}
...
static void aw9610x_irq_handle(struct aw9610x *aw9610x)
{
    ...
    uint8_t j = 0;
    #ifdef HQ_FACTORY_BUILD
    int intput_value_far = 0;
    int intput_value_close = 0;
    #endif
    AWLOGD(aw9610x->dev, "enter");
    ...
    for (i = 0; i < AW_CHANNEL_MAX; i++) {
        ...
        if ((aw9610x->aw_channel[j * AW_CHANNEL_MAX + i].curr_state !=
            ...
            if (AW_CHANNEL0 == (j * AW_CHANNEL_MAX + i) ) {
                ...
                intput_value_close = KEY_SAR1_CLOSE;
            }
            else if (AW_CHANNEL1 == (j * AW_CHANNEL_MAX + i)) {
                intput_value_far = KEY_SAR2_FAR;
                intput_value_close = KEY_SAR2_CLOSE;
            }
            else if (AW_CHANNEL2 == (j * AW_CHANNEL_MAX + i)) {
                intput_value_far = KEY_SAR3_FAR;
            ...
            case FAR:
                ...
                input_report_rel(
                    aw9610x->aw_channel[j * AW_CHANNEL_MAX + i].input,
                            REL_MISC, 5);
                #endif
                AWLOGE(aw9610x->dev, "%s far reported",
            ...
            default:
                ...
                input_report_rel(
                    aw9610x->aw_channel[j * AW_CHANNEL_MAX + i].input,
                            REL_MISC, 1);
                #endif
                AWLOGE(aw9610x->dev, "%s close reported",
                    channel_event[j * AW_CHANNEL_MAX + i].name);
                break;
            }
...
aw9610x_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    j = aw9610x->sar_num;
    for (i = 0; i < AW_CHANNEL_MAX; i++) {
        if (AW_AW9610X_ENABLE_MASK & (0x01 << i)) {
            ...
            if (AW_CHANNEL0 == (j * AW_CHANNEL_MAX + i)) {
                ...
                __set_bit(KEY_SAR1_FAR, aw9610x->aw_channel[j * AW_CHANNEL_MAX + i].input->keybit);
                __set_bit(KEY_SAR1_CLOSE, aw9610x->aw_channel[j * AW_CHANNEL_MAX + i].input->keybit);
                AWLOGE(aw9610x->dev, "AW_CHANNEL0 report init");
            }
            else if (AW_CHANNEL1 == (j * AW_CHANNEL_MAX + i)) {
                __set_bit(EV_KEY,aw9610x->aw_channel[j * AW_CHANNEL_MAX + i].input->evbit);
                __set_bit(KEY_SAR2_FAR, aw9610x->aw_channel[j * AW_CHANNEL_MAX + i].input->keybit);
                __set_bit(KEY_SAR2_CLOSE, aw9610x->aw_channel[j * AW_CHANNEL_MAX + i].input->keybit);
                AWLOGE(aw9610x->dev, "AW_CHANNEL1 report init");
            }
            else if (AW_CHANNEL2 == (j * AW_CHANNEL_MAX + i)) {
                __set_bit(EV_KEY,aw9610x->aw_channel[j * AW_CHANNEL_MAX + i].input->evbit);
                __set_bit(KEY_SAR3_FAR, aw9610x->aw_channel[j * AW_CHANNEL_MAX + i].input->keybit);
                __set_bit(KEY_SAR3_CLOSE, aw9610x->aw_channel[j * AW_CHANNEL_MAX + i].input->keybit);
...
#if defined(CONFIG_SENSORS)
    ret = sensors_classdev_register(&aw9610x->aw_channel[2].input->dev, &grip_sensor_cdev);
    if (ret < 0) {
        AWLOGE(aw9610x->dev, "error sensors_classdev_register %s", grip_sensor_cdev.name);
        return -1;
    }
    ret = device_create_file(grip_sensor_cdev.dev, &dev_attr_onoff);
    if (ret < 0) {
        AWLOGE(aw9610x->dev, "error device_create_filer grip");
    }
#endif
