...
char Ctp_name[HARDWARE_MAX_ITEM_LONGTH];// wanglongfei.wt 20210816 add for tp hardware info
...
char battery_id[HARDWARE_MAX_ITEM_LONGTH] = "Unknow-battery";//Bug 700264, modify guoyanjun.wt,20211103,add battery id info
char charge_ic[HARDWARE_MAX_ITEM_LONGTH];//bug 707025   add guoyanjun.wt 20190403 Add charger IC model information in factory mode
...
//bug 708366,huangxiaotian,modify board_id,hw_id,2021/11/03,start
static void boardid_get(void)
{
	char* s1= NULL;
	char *ptr = NULL;
...
//bug 708366,huangxiaotian,modify board_id,hw_id,2021/11/03,end
...
//+Bug 700264, modify guoyanjun.wt,20211103,add battery id info
static int __init battery_id_get(char *str)
{
	if (!str)
		return 0;
	strcpy(battery_id,(const char *)str);
	//printk("jun:str=%s,battery_id=%s\n",str,battery_id);
	return 0;
}
__setup("batinfo=", battery_id_get);

//-Bug 700264, modify guoyanjun.wt,20211103,add battery id info

//+bug 707025, modify guoyanjun.wt 20190403 Add charger IC model information in factory mode
extern bool is_sgm41542;
static void charge_ic_get(void)
{
	if(is_sgm41542)
		strcpy(charge_ic,"SGM41542");
	else
		strcpy(charge_ic,"SY6970");
}
//-bug 707025, modify guoyanjun.wt 20190403 Add charger IC model information in factory mode
...
static long hardwareinfo_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	case HARDWARE_TP_GET:
		hardwareinfo_set_prop(HARDWARE_TP, Ctp_name);//wanglongfei.wt 20210816 add for tp hardware info
	...
	case HARDWARE_BATTERY_ID_GET:
		hardwareinfo_set_prop(HARDWARE_BATTERY_ID, battery_id);
	...
	case HARDWARE_CHARGER_IC_INFO_GET:
		charge_ic_get();
		hardwareinfo_set_prop(HARDWARE_CHARGER_IC_INFO, charge_ic);//fix an issue where charge ic info displayed as null
	...
	case HARDWARE_SMARTPA_IC_GET:
		hardwareinfo_set_prop(HARDWARE_SMARTPA_ID, "FS1818U");
	...
	case HARDWARE_CAMERA_INFO_SET:
	{
		...
		if(copy_from_user(&kbuf, data, sizeof(camera_info)))
		{
		...
		{
			hardware.lenth = ((camera_info*)(&kbuf))->lenth;
			hardware.param = ((camera_info*)(&kbuf))->param;
			strncpy(hardwareinfo_name[hardware.param], ((camera_info*)(&kbuf))->str, hardware.lenth);
...
static ssize_t show_boardinfo(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for(i = 0; i < HARDWARE_MAX_ITEM; i++)
	{
		...
		else if(i == HARDWARE_BT || i == HARDWARE_WIFI || i == HARDWARE_GPS)
		{
