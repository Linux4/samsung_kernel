							   ...
							   unsigned long arg)
{
	...
	case HARDWARE_BACK_CAM_MOUDULE_ID_SET:
		if (copy_from_user
			(hardwareinfo_name[HARDWARE_BACK_CAM_MOUDULE_ID], data,
			 sizeof(data))) {
	...
	case HARDWARE_FRONT_CAM_MODULE_ID_SET:
		if (copy_from_user
			(hardwareinfo_name[HARDWARE_FRONT_CAM_MOUDULE_ID], data,
			 sizeof(data))) {
			pr_err("wgz copy_from_user error");
			ret = -EINVAL;
		}
...
void boardid_get_and_set(void)
{
	char* boadrid_start;
	char boardid_info[HARDWARE_MAX_ITEM_LONGTH];
	char* hwid_start;
	char hwid_info[HARDWARE_MAX_ITEM_LONGTH];
	...
	{
		sprintf(boardid_info, "boardid not define!");
	}
	printk("%s: get the boardid success %s\n", __func__, boardid_info);
	strcpy(board_id, boardid_info);
	//hwid info
	hwid_start = strstr(saved_command_line,"hw_id=");
	memset(hwid_info, 0, HARDWARE_MAX_ITEM_LONGTH);
	if(hwid_start != NULL)
	{
		strncpy(hwid_info, hwid_start+sizeof("hw_id=")-1, 6);
	}
	else
	{
		sprintf(hwid_info, "hardwareid not define!");
	}
	printk("%s: get the hwid success %s\n", __func__, hwid_info);
	strcpy(hardware_id, hwid_info);
}
