		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (iod->format == IPC_MULTI_RAW) {
		...
		for (i = PS_DATA_CH_0; i <= PS_DATA_CH_LAST; i++) {
...
static unsigned int misc_poll(struct file *filp, struct poll_table_struct *wait)
{
	...
	if (!skb_queue_empty(rxq) && mc->phone_state != STATE_OFFLINE)
			...
			size_t count, loff_t *fpos)
{
	...
	/* Store the IO device & the link device */
	skbpriv(skb)->iod = iod;
	skbpriv(skb)->ld = ld;
...
#ifdef DEBUG_MODEM_IF_IODEV_TX
	log_ipc_pkt(iod->id, IODEV, TX, skb, true, false);
#endif
...
#ifdef DEBUG_MODEM_IF_IODEV_RX
	log_ipc_pkt(iod->id, IODEV, RX, skb, false, false);
#endif
