...
struct pn544_i2c_platform_data *pdata)
{
	...
	pdata->irq_gpio = of_get_named_gpio_flags(np, "pn544-i2c,irq-gpio",
	0, &pdata->irq_gpio_flags);
	pdata->ven_gpio = of_get_named_gpio_flags(np, "pn544-i2c,ven_gpio",
	0, &pdata->ven_gpio_flags);
	pdata->firm_gpio = of_get_named_gpio_flags(np, "pn544-i2c,firm_gpio",
		       ...
		       const struct i2c_device_id *id)
{
	...
	if(client-> dev.of_node) {
		...
		err = pn544_parse_dt(&client->dev, platform_data);
		if(err)
			return err;
	} else {
	...
	pr_info("%s : IRQ num %d\n", __func__, client->irq);
	printk("\n pn544 irq number is %d", client->irq);
