...
static void tz_iwlog_channel_print(const char *buf, unsigned int count)
{
...
static void tz_iwlog_read_channels_do(void)
{
	...
	for (i = 0; i < NR_SW_CPU_IDS; ++i) {
		...
		if (write_count < ch->read_count) {
			...
			tz_iwlog_channel_print(ch->buffer + ch->read_count, count);
			ch->read_count = 0;
		}
