...
static dev_t tzdev_dev_admin;
struct cdev tzdev_admin_cdev;
/* Device class for the driver assigned major */
static struct class *tzdev_device_class;
...
static int create_devices(void)
{
	...
	cdev_init(&tzdev_admin_cdev, &tzdev_fops);

	tzdev_device_class = class_create(THIS_MODULE, "tzdev");
	if (IS_ERR(tzdev_device_class)) {
		tzdev_print(0, "failed to create device class\n");
		ret = PTR_ERR(tzdev_device_class);
		goto out;
	}

	ret = alloc_chrdev_region(&tzdev_dev_admin, 0, 1, "tzdev");
	if (ret < 0) {
		tzdev_print(0, "failed to allocate char dev region\n");
		goto error;
	}

	/* First the ADMIN node */
	ret = cdev_add(&tzdev_admin_cdev,  tzdev_dev_admin, 1);
	if (ret != 0) {
		tzdev_print(0, "device add failed\n");
		goto error;
	}
	tzdev_admin_cdev.owner = THIS_MODULE;
	device_create(tzdev_device_class, NULL, tzdev_dev_admin, NULL,
		      "tzdev");

	goto out;
//error_unregister:
//	device_destroy(tzdev_device_class, tzdev_dev_admin);
//
//	cdev_del(&tzdev_admin_cdev);
//	unregister_chrdev_region(tzdev_dev_admin, 1);
error:
	class_destroy(tzdev_device_class);
out:
	return ret;
}
...
static int __init init_tzdev(void)
{
	...
	rc = create_devices();
	if (rc != 0) {
		printk("tzd: Can't create devices\n");
		return 1;
	}

	(void)tzdev_event_handler;
	(void)tzdev_panic_handler;
#if 0
...
#if 1
int tzdev_qc_probe(struct platform_device *pdev)
{
	printk("tzd: tzdev_qc_probe()\n");
	tzdev_mcd->of_node = pdev->dev.of_node;
	init_tzdev();
	return 0;
}

int tzdev_qc_remove(struct platform_device *pdev)
{
	return 0;
}

int tzdev_qc_suspend(struct platform_device *pdev, pm_message_t state)
{
	return 0;
}

int tzdev_qc_resume(struct platform_device *pdev)
{
	return 0;
}
...
{
	printk("tzd: tzdev_platform_register()\n");
...
#if 1
module_init(tzdev_platform_register);
module_exit(tzdev_platform_unregister);
#endif
