...
static ssize_t tz_wormhole_write(struct file *filp, const char __user *buf, size_t size, loff_t *ppos)
{
	...
	if (filp->f_flags & O_NONBLOCK) {
		...
		add_wait_queue(&hole_desc->wq, &wait);
		while ((ret = __tz_wormhole_write(hole, buf, size)) == -EAGAIN) {
		...
		remove_wait_queue(&hole_desc->wq, &wait);
#else
...
static ssize_t tz_wormhole_read(struct file *filp, char __user *buf, size_t size, loff_t *ppos)
{
	...
	if (filp->f_flags & O_NONBLOCK) {
		...
		add_wait_queue(&hole_desc->wq, &wait);
		while ((ret = __tz_wormhole_read(hole, buf, size)) == -EAGAIN) {
			if (signal_pending(current)) {
				ret = -ERESTARTSYS;
				break;
			}
