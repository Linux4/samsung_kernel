...
static struct tzio_smc_data tz_swd_test_get_event(void)
{
...
static struct tzio_smc_data tz_swd_test_send_command(struct tzio_smc_data s)
{
	struct tzdev_smc_data d;

	d = tzdev_send_command(s.args[0], s.args[1]);
	tzdev_rsp_convert(&s, &d);
