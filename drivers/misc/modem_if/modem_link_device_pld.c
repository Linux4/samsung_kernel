...
{
	u16 val1 = 0;
	_pld_read_register(pld->mbx2ap,&val1,2);
...
static inline void send_intr(struct pld_link_device *pld, u16 mask)
{
	_pld_write_register(pld->mbx2cp,&mask,2);
...
{
	u16 val1 = 0;
	_pld_read_register(pld->magic_cp2ap,&val1,2);
	return val1;
...
{
	_pld_write_register(pld->magic_ap2cp,&in,2);
	return;
}
...
{
	u16 val1 = 0;
	_pld_read_register(pld->access_cp2ap,&val1,2);
	return val1;
...
{
	_pld_write_register(pld->access_ap2cp,&in,2);
	return;
}
...
{
	u16 val = 0;
	_pld_read_register(pld->dev[id]->txq.head,&val,2);
	return val;
}
...
{
	u16 val = 0;
	_pld_read_register(pld->dev[id]->txq.tail,&val,2);
	return val;
}

static inline void set_tx_head(struct pld_link_device *pld, int id, u32 in)
{
	_pld_write_register(pld->dev[id]->txq.head,&in,2);
	return;
}
...
{
	u16 val;
	_pld_read_register(pld->dev[id]->rxq.head,&val,2);
	return val;
}
...
{
	u16 val= 0;
	_pld_read_register(pld->dev[id]->rxq.tail,&val,2);
	return val;
}
...
static inline void set_rx_tail(struct pld_link_device *pld, int id, u32 out)
{
	_pld_write_register(pld->dev[id]->rxq.tail,&out,2);
	return; 
}
		...
		u32 qsize, u32 in, u32 out, struct sk_buff *skb)
{
	...
	if (in < out) {
		/* +++++++++ in ---------- out ++++++++++ */
		if ( len % 2 == 0 )
			_pld_write_register(buff+in,src,len);
		else
			_pld_write_register(buff+in,src,len+1);

		mif_print_data(src, len);
		mif_err("\n");	
	} else {
		/* ------ out +++++++++++ in ------------ */
		u32 space = qsize - in;
		...
		/* 1) in -> buffer end */
		if ( len % 2 == 0 )
			_pld_write_register(buff+in,src,((len > space) ? space : len));
		else
			_pld_write_register(buff+in,src,((len > space) ? space : len)+1);


		if (len > space)	{
			if ( len % 2 == 0 )
				_pld_write_register(buff+in,src+space,(len-space));
			else
				_pld_write_register(buff+in,src+space,(len-space)+1);
		}
	}
	...
	u8 __iomem *src, u32 out, u32 len, u32 qsize)
{
	...
	if ((out + len) <= qsize) {
		...
		/* -----   7f 00 00 7e      ----- */
		if ( len % 2 == 0 ) {
			ret = _pld_read_register(src + out, dst, len);
			if (ret < 0)
				mif_err("(%d) _pld_read_register err!\n", __LINE__);
		} else {
			ret = _pld_read_register(src + out, dst, len+1);
			if (ret < 0)
				mif_err("(%d) _pld_read_register err!\n", __LINE__);
		}
		//mif_print_data(dst, len);	
	} else {
		/*       (in) ----------- (out)   */
		/* 00 7e      -----------   7f 00 */
		...
		/* 1) out -> buffer end */
		if ( len1 % 2 == 0 )
			_pld_read_register(src + out,dst,len1);
		else
			_pld_read_register(src + out,dst,len1+1);

		/* 2) buffer start -> in */
		dst += len1;

		if ( (len - len1) % 2 == 0 )
			_pld_read_register(src + out,dst,(len - len1));
		else
			_pld_read_register(src + out,dst,(len - len1)+1);
	}

	if (pld->ld.mode == LINK_MODE_IPC && ori_det[0] != 0xfc)	{
...
static int pld_init_ipc(struct pld_link_device *pld)
{
	struct link_device *ld = &pld->ld;
	int i;
	u16 in = DPRAM_MAGIC_CODE;
	mif_err("lb->mode %d ,max_ipc_dev %d\n",ld->mode,ld->max_ipc_dev);
	if (ld->mode == LINK_MODE_IPC &&
...
static void cmd_phone_start_handler(struct pld_link_device *pld)
{
	...
	struct io_device *iod = NULL;
	int err;
	u16 int2ap = INT_CMD(INT_CMD_INIT_END);

	mif_info("%s: Recv 0xC8 (CP_START)\n", ld->name);

	err = pld_init_ipc(pld);  //temp block
	if(err < 0)
	mif_info("pld_init_ipc (err %d)\n",err);
	...
	err = _pld_write_register(pld->mbx2cp, &int2ap, 2);
	if (err < 0 ) {
		mif_info("%s: int2ap write fail (%d)\n", ld->name, err);
	}
}
...
static u32 irq_handler_work(struct work_struct *work)
{
	int ret = 0;
	u32 int2ap = 0;
	int err = 0;
	...
	struct pld_link_device *pld =
		container_of(work, struct pld_link_device, irq_delayed_work.work);
	struct link_device *ld = (struct link_device *)&pld->ld;

	err = _pld_read_register(pld->mbx2ap, &int2ap, 2);

	if(err < 0 ) {
		mif_info("%s: int2ap read fail (%d)\n", ld->name, err);
		goto exit;
	} else
		mif_info("%s: int2ap read data (0x%04x)\n", ld->name, int2ap);
	...
	if (likely(INT_VALID(int2ap))) {
		//mif_info("%s: int2ap command_handler call\n", ld->name);
...
static irqreturn_t pld_irq_handler(int irq, void *data)
{
	...
	if (unlikely(ld->mode == LINK_MODE_DLOAD)) {
		complete(&pld->udl_cmd_complete);
		return IRQ_HANDLED;
	}

#if 1
	queue_delayed_work(pld->tx_wq, &pld->irq_delayed_work, 0);
#else
	err = SPI_IPC_Rxd(rx_cmd_addr, &int2ap, 2);
	if(err < 0 ) {
		mif_info("%s: int2ap read fail (%d)\n", ld->name,err);
		goto exit;
	} else
...
static int pld_send_cp_binary(struct link_device *ld, struct sk_buff *skb)
{
	struct pld_link_device *pld = to_pld_link_device(ld);

	if (pld->ext_op && pld->ext_op->xmit_binary)
		return pld->ext_op->xmit_binary(pld, skb);
	else
		return -ENODEV;
}

static int pld_set_dload_magic(struct link_device *ld, struct io_device *iod)
{
	struct pld_link_device *pld = to_pld_link_device(ld);
		...
		struct sk_buff *skb)
{
	...
	case IPC_RFS:
		...
		return len;
	case IPC_BOOT:
		return pld_send_cp_binary(ld, skb);
...
struct link_device *pld_create_link_device(struct platform_device *pdev)
{
	...
	ld->send = pld_send;
	ld->dload_start = pld_set_dload_magic;
	...
	pld->irq = gpio_to_irq(mdm_data->gpio_ipc_int2ap);
	...
	if (ret)
 	...
 	/* pld_irq_handler workqueue */
	pld->tx_wq = create_singlethread_workqueue("pld_tx_wq");
	if (!pld->tx_wq) {
		mif_err("fail to create work Q.\n");
		goto err;
	}

	INIT_DELAYED_WORK(&pld->irq_delayed_work, irq_handler_work);
