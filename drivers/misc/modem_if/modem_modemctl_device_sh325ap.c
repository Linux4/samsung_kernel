			...
			struct device_attribute *attr, char *buf)
{
	...
	ret += snprintf(&buf[ret], PAGE_SIZE - ret, "AP2CP_STATUS : %d\n",
				mbox_get_value(MCU_CP, mc->mbx->mbx_ap2cp_status));
	ret += snprintf(&buf[ret], PAGE_SIZE - ret, "CP2AP_STATUS : %d\n",
				mbox_get_value(MCU_CP, mc->mbx->mbx_cp2ap_status));
	ret += snprintf(&buf[ret], PAGE_SIZE - ret, "AP2CP_ACTIVE : %d\n",
				mbox_get_value(MCU_CP, mc->mbx->mbx_ap2cp_active));
	ret += snprintf(&buf[ret], PAGE_SIZE - ret, "CP2AP_DVFS_REQ : %d\n",
				mbox_get_value(MCU_CP, mc->mbx->mbx_cp2ap_perf_req));
	ret += snprintf(&buf[ret], PAGE_SIZE - ret, "PHONE_ACTIVE : %d\n",
				mbox_get_value(MCU_CP, mc->mbx->mbx_cp2ap_active));
	ret += snprintf(&buf[ret], PAGE_SIZE - ret, "HW_REVISION : %d\n",
				mbox_get_value(MCU_CP, mc->mbx->mbx_ap2cp_sys_rev));
...
static void bh_pm_qos_req(struct work_struct *ws)
{
	...
	qos_val = mbox_get_value(MCU_CP, mc->mbx->mbx_cp2ap_perf_req);
...
static void bh_pm_qos_req_cpu(struct work_struct *ws)
{
	...
	qos_val = mbox_get_value(MCU_CP, mc->mbx->mbx_cp2ap_perf_req_cpu);
	mif_err("pm_qos:0x%x requested\n", qos_val);
...
static void bh_pm_qos_req_mif(struct work_struct *ws)
{
	...
	qos_val = mbox_get_value(MCU_CP, mc->mbx->mbx_cp2ap_perf_req_mif);
	mif_err("pm_qos:0x%x requested\n", qos_val);
...
static void bh_pm_qos_req_int(struct work_struct *ws)
{
	...
	qos_val = mbox_get_value(MCU_CP, mc->mbx->mbx_cp2ap_perf_req_int);
	mif_err("pm_qos:0x%x requested\n", qos_val);
...
static int exynos_devfreq_parse_ect(struct modem_ctl *mc, char *dvfs_domain_name)
{
	...
	mc->cpu_table.num_of_table = 0;
	mc->mif_table.num_of_table = 0;
	mc->int_table.num_of_table = 0;
...
static void cp_active_handler(void *arg)
{
	...
	int cp_active = mbox_get_value(MCU_CP, mc->mbx->mbx_cp2ap_active);
...
static int sh325ap_on(struct modem_ctl *mc)
{
	struct link_device *ld = get_current_link(mc->iod);
	int cp_active = mbox_get_value(MCU_CP, mc->mbx->mbx_cp2ap_active);
	int cp_status = mbox_get_value(MCU_CP, mc->mbx->mbx_cp2ap_status);
	int ret;
	int i;
	...
	mbox_set_value(MCU_CP, mc->mbx->mbx_ap2cp_sys_rev, get_hw_rev());
	mif_err("System Revision %d\n", mbox_get_value(MCU_CP, mc->mbx->mbx_ap2cp_sys_rev));

	mbox_set_value(MCU_CP, mc->mbx->mbx_ap2cp_pkg_id, get_package_id());
	mif_err("Package ID %d\n", mbox_get_value(MCU_CP, mc->mbx->mbx_ap2cp_pkg_id));

	mbox_set_value(MCU_CP, mc->mbx->mbx_ap2cp_pmic_rev, get_pmic_rev());
	mif_err("PMIC Rev : %d\n", mbox_get_value(MCU_CP, mc->mbx->mbx_ap2cp_pmic_rev));

	mbox_set_value(MCU_CP, mc->mbx->mbx_ap2cp_lock_value, get_lock_value());
	mif_err("LOCK Value : %d\n", mbox_get_value(MCU_CP, mc->mbx->mbx_ap2cp_lock_value));
	...
	mbox_set_value(MCU_CP, mc->mbx->mbx_ap2cp_active, 1);
...
static int sh325ap_off(struct modem_ctl *mc)
{
	struct link_device *ld = get_current_link(mc->iod);
	int cp_on = mc->pmu->get_pwr_status();
...
static int sh325ap_reset(struct modem_ctl *mc)
{
	struct link_device *ld = get_current_link(mc->iod);
...
static int sh325ap_boot_on(struct modem_ctl *mc)
{
	...
	while (mbox_get_value(MCU_CP, mc->mbx->mbx_cp2ap_status) == 0) {
		if (--cnt > 0)
	...
	mbox_set_value(MCU_CP, mc->mbx->mbx_ap2cp_status, 1);

	mif_info("---\n");
	return 0;
}

static int sh325ap_boot_off(struct modem_ctl *mc)
{
	struct link_device *ld = get_current_link(mc->bootd);
	unsigned long remain;
	int err = 0;
...
static int sh325ap_boot_done(struct modem_ctl *mc)
{
	mif_info("+++\n");
...
static int sh325ap_force_crash_exit(struct modem_ctl *mc)
{
	struct link_device *ld = get_current_link(mc->bootd);
	mif_err("+++\n");
...
static int sh325ap_dump_reset(struct modem_ctl *mc)
{
	mif_err("+++\n");
...
static int sh325ap_dump_start(struct modem_ctl *mc)
{
	int err;
	struct link_device *ld = get_current_link(mc->bootd);
	mif_err("+++\n");
...
static int sh325ap_get_meminfo(struct modem_ctl *mc, unsigned long arg)
{
...
static int sh325ap_suspend_modemctl(struct modem_ctl *mc)
{
	mbox_set_value(MCU_CP, mc->mbx->mbx_ap2cp_active, 0);
	mbox_set_interrupt(MCU_CP, mc->mbx->int_ap2cp_active);

	return 0;
}

static int sh325ap_resume_modemctl(struct modem_ctl *mc)
{
	mbox_set_value(MCU_CP, mc->mbx->mbx_ap2cp_active, 1);
	mbox_set_interrupt(MCU_CP, mc->mbx->int_ap2cp_active);
...
static void sh325ap_get_ops(struct modem_ctl *mc)
{
	mc->ops.modem_on = sh325ap_on;
	mc->ops.modem_off = sh325ap_off;
	mc->ops.modem_reset = sh325ap_reset;
	mc->ops.modem_boot_on = sh325ap_boot_on;
	mc->ops.modem_boot_off = sh325ap_boot_off;
	mc->ops.modem_boot_done = sh325ap_boot_done;
	mc->ops.modem_force_crash_exit = sh325ap_force_crash_exit;
	mc->ops.modem_dump_reset = sh325ap_dump_reset;
	mc->ops.modem_dump_start = sh325ap_dump_start;
	mc->ops.modem_get_meminfo = sh325ap_get_meminfo;
	mc->ops.suspend_modem_ctrl = sh325ap_suspend_modemctl;
	mc->ops.resume_modem_ctrl = sh325ap_resume_modemctl;
}

static void sh325ap_get_pdata(struct modem_ctl *mc, struct modem_data *modem)
{
	mc->mbx = modem->mbx;
	mc->pmu = modem->pmu;
}
...
static void uart_dir_work(struct modem_ctl *mc)
{
	...
	if (info_value != mbox_get_value(MCU_CP, mc->mbx->mbx_ap2cp_uart_noti)) {
		mif_info("%s: change uart state to %s\n", __func__,
			info_value ? "CP" : "AP");

		mbox_set_value(MCU_CP, mc->mbx->mbx_ap2cp_uart_noti, info_value);
		if (mbox_get_value(MCU_CP, mc->mbx->mbx_ap2cp_uart_noti))
			mbox_set_interrupt(MCU_CP, mc->mbx->int_ap2cp_uart_noti);
	}
}
...
static struct modem_pmu sh325ap_pmu = {
...
int init_modemctl_device(struct modem_ctl *mc, struct modem_data *pdata)
{
	...
	pdata->pmu = &sh325ap_pmu;
	sh325ap_get_ops(mc);
	sh325ap_get_pdata(mc, pdata);
	...
	*/
	...
	sysram_alive = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	mc->sysram_alive = shm_request_region(sysram_alive->start, 0x1000);
	if (!mc->sysram_alive)
	...
	*/
	mbox_request_irq(MCU_CP, mc->mbx->irq_cp2ap_perf_req,
					dvfs_req_handler, mc);
	mif_err("dvfs_req_handler registered\n");

	mbox_request_irq(MCU_CP, mc->mbx->irq_cp2ap_perf_req_cpu,
					dvfs_req_handler_cpu, mc);
	mif_err("dvfs_req_handler_cpu registered\n");

	mbox_request_irq(MCU_CP, mc->mbx->irq_cp2ap_perf_req_mif,
					dvfs_req_handler_mif, mc);
	mif_err("dvfs_req_handler_mif registered\n");

	mbox_request_irq(MCU_CP, mc->mbx->irq_cp2ap_perf_req_int,
					dvfs_req_handler_int, mc);
	mif_err("dvfs_req_handler_int registered\n");
	/*
	** Register LTE_ACTIVE MBOX interrupt handler
	*/
	mbox_request_irq(MCU_CP, mc->mbx->irq_cp2ap_active, cp_active_handler, mc);
