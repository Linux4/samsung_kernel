...
static struct modem_ctl *mc_for_mif_freq;


static unsigned int get_lock_value(void)
{
	return __raw_readl(EXYNOS_PMU_DREX_CALIBRATION0);
}
...
void set_ap_mif_freq(unsigned int freq)
{
	if (mc_for_mif_freq == NULL)
		return;

	mbox_set_value(MCU_CP, mc_for_mif_freq->mbx_ap_mif_freq, freq);
}
EXPORT_SYMBOL(set_ap_mif_freq);
...
static int sh333ap_on(struct modem_ctl *mc)
{
	...
	memmove(modem->ipc_base + SHM_MEM_CAL_OFFSET,
			S5P_VA_SYSRAM_ALIVE + 0xc00, 0x200);
...
static void sh333ap_get_pdata(struct modem_ctl *mc, struct modem_data *modem)
{
	...
	mc->int_ap_status = mbx->int_ap2cp_status;
	mc->mbx_cp_status = mbx->mbx_cp2ap_status;
	mc->irq_cp_status = mbx->irq_cp2ap_status;

	mc->int_uart_noti = mbox->int_ap2cp_uart_noti;
	...
	mc->mbx_ap_mif_freq = modem->mbx_ap2cp_mif_freq;
	mc->pmu = modem->pmu;
}
