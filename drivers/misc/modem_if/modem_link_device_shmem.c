...
static inline u16 recv_int2ap(struct shmem_link_device *shmd)
{
	if (shmd->mbx2ap)
		...
		return (u16)mbox_get_value(MCU_CP, shmd->mbx->mbx_cp2ap_msg);
}
...
static inline void send_int2cp(struct shmem_link_device *shmd, u16 mask)
{
	...
	if (shmd->mbx2cp) {
		*shmd->mbx2cp = mask;
		...
		mbox_set_value(MCU_CP, shmd->mbx->mbx_ap2cp_msg, mask);
		mbox_set_interrupt(MCU_CP, shmd->mbx->int_ap2cp_msg);
	}
}
		...
		struct io_device *iod, int arg)
{
	...
	mbox_set_value(MCU_CP, shmd->mbx->mbx_ap2cp_tmu_msg, arg);
	mbox_set_interrupt(MCU_CP, shmd->mbx->int_ap2cp_tmu_msg);
}
...
#if defined(CONFIG_LTE_MODEM_S5E7890) || defined(CONFIG_LTE_MODEM_S5E8890) || \
	defined(CONFIG_LTE_MODEM_S5E7880)
		if (di.m_offset == (u32)cpmem_info->start) {
			/* Check shared region size to confirm CP image
			 * (u32 *) syscp_alive[0] = Magic Code, Version
			 * (u32 *) syscp_alive[1] = CP Reserved Size
			 * (u32 *) syscp_alive[2] = Shared Mem Size
			 */
			main_base = (void __iomem *)modem->modem_base + di.m_offset;
			if (*(main_base + CP_MEM) != shm_get_boot_size()) {
				mif_err("%s: ERR! Not correct CP Reserved memory[%x != %x]\n",
						__func__, *(main_base + CP_MEM),
						(u32)shm_get_boot_size());
				return -EFAULT;
			}
			if (*(main_base + SHM_MEM) != shm_get_ipc_rgn_size()) {
				mif_err("%s: ERR! Not correct CP shared memory[%x != %x]\n",
						__func__, *(main_base + SHM_MEM),
						(u32)shm_get_ipc_rgn_size());
				return -EFAULT;
			}
		}
...
struct link_device *shmem_create_link_device(struct platform_device *pdev)
{
	...
	err = mbox_request_irq(MCU_CP, shmd->mbx->irq_cp2ap_msg,
				shmem_irq_handler, shmd);
	if (err) {
	...
	modem->syscp_info = platform_get_resource(pdev, IORESOURCE_MEM, 1);

	shmd->ext_gpio = &(modem->ext_gpio);
