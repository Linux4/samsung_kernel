...
static void shmem_cp2ap_wakelock_handler(void *data)
{
	...
	req = (mbox_get_value(MCU_CP, mld->mbx->mbx_cp2ap_status)
		& CP2AP_STATUS_CP2APWAKELOCK_MASK)
		>> CP2AP_STATUS_CP2APWAKELOCK_SHIFT;
...
struct link_device *shmem_create_link_device(struct platform_device *pdev)
{
	...
	shmd->irq_cp2ap_wakelock = modem->mbx->irq_cp2ap_wakelock;
	...
	err = mbox_request_irq(MCU_CP, shmd->irq, shmem_irq_handler, shmd);
	if (err) {
