...
/* for ckecking modem infomation */
#define SHM_CPINFO_DEBUG	0xF80
#define SHM_MEM_CAL_OFFSET	0x1000

#define MODEM_VER_SIZE		0x64
#define MODEM_VER_ID_OFFSET	0x1002C
#define MODEM_VER_ID_VALUE	0x44415445
#define MODEM_VER_ADDR_OFFSET	0x10030
...
struct shmem_link_device {
	...
	/* MBOX for TMU */
	unsigned mbx_ap2cp_tmu_msg;
	int int_ap2cp_tmu_msg;
				...
				struct circ_status *circ)
{
	...
	while (1) {
		...
		cnt++;
		mif_err("%s: ERR! invalid %s_TXQ{qsize:%d in:%d out:%d space:%d}, count %d\n",
				...
				struct circ_status *circ)
{
	...
	while (1) {
		...
		cnt++;
		mif_err("%s: ERR! invalid %s_TXQ{qsize:%d in:%d out:%d saved:%d}, count %d\n",
			ld->name, get_dev_name(dev), qsize, head, tail,
			saved, cnt);
		if (cnt >= MAX_RETRY_CNT) {
			saved = -EIO;
