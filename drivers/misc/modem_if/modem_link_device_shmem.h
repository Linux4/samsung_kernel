...
struct shmem_link_device {
	...
	/* to hold/drop "cp_wakeup" for PM (power-management) */
	struct delayed_work cp_wakeup_drop_dwork;
	struct delayed_work cp_wakeup_hold_dwork;

	/* to hold/drop "pda_active" for PM  */
...
static bool ipc_active(struct shmem_link_device *shmd)
{
	...
	if (likely(magic == SHM_IPC_MAGIC && access == 1))
		return true;
 ...
 * pointer values, size of received data} into the 'cst' instance.
 *
