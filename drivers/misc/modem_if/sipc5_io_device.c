		...
		struct sk_buff *skb)
{
	...
	if (unlikely(iod->format == IPC_FMT)) {
		...
		snprintf(str, MIF_MAX_STR_LEN, "%s->%s: CP2MIF",
			ld->name, iod->name);
			...
			size_t count, loff_t *fpos)
{
...
#if 0
		char str[MIF_MAX_STR_LEN];
		snprintf(str, MIF_MAX_STR_LEN, "%s: RIL2MIF", iod->mc->name);
		pr_ipc(0, str, msg, (count > 20 ? 20 : count));
#endif
	...
	if (iod->format == IPC_RAMDUMP) {
		char str[MIF_MAX_STR_LEN];
		snprintf(str, MIF_MAX_STR_LEN, "%s: DUMP2MIF", iod->name);
		pr_ipc(1, str, skb->data, (skb->len > 20 ? 20 : skb->len));
	}
#endif
			...
			loff_t *fpos)
{
...
#if 0
		char str[MIF_MAX_STR_LEN];
		snprintf(str, MIF_MAX_STR_LEN, "%s: MIF2RIL", iod->mc->name);
		pr_ipc(0, str, skb->data, (skb->len > 20 ? 20 : skb->len));
#endif
		getnstimeofday(&epoch);
		mif_time_log(iod->mc->msd, epoch, NULL, 0);
	...
	if (iod->format == IPC_RAMDUMP) {
		char str[MIF_MAX_STR_LEN];
		snprintf(str, MIF_MAX_STR_LEN, "%s: MIF2DUMP", iod->name);
		pr_ipc(1, str, skb->data, (skb->len > 20 ? 20 : skb->len));
	}
#endif
