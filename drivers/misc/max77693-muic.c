...
#include <mach/sec-switch_max77693.h>
					 ...
					 int adc)
{
	...
	if (ret) {
		dev_info(info->dev,"Set charging state failure \n");
...
//		 __func__, status1, status2, info->cable_type);
	printk(KERN_CRIT "@@MAX77693-MUIC@@ func:%s st1:%x st2:%x cable_type:%d\n",
		 __func__, status1, status2, info->cable_type);
	...
	case CABLE_TYPE_OTG_MUIC:
		if (!!adclow) {
			...
			//		__func__);
			printk(KERN_CRIT "%s:%s assume OTG detach\n", DEV_NAME,
					__func__);
			info->cable_type = CABLE_TYPE_NONE_MUIC;
	...
	case CABLE_TYPE_JIG_UART_OFF_VB_MUIC:
		 ...
		 */
		printk(KERN_CRIT "@@MAX77693-MUIC@@ JIG UART OFF \n");
	...
	case ADC_GND:
		if (chgtyp == CHGTYP_NO_VOLTAGE) {
			...
			pr_info("Setting USB mode to AP \n");
			max77693_muic_set_usb_path(info, AP_USB_MODE);
			msleep(40);
			if (mdata->usb_cb && info->is_usb_ready)
					...
					u8 status1, u8 status2)
{
	...
	int intr = INT_ATTACH;
	printk(KERN_CRIT "@@MAX77693-MUIC@@ %s func \n",__func__);
	adc = status1 & STATUS1_ADC_MASK;
	adclow = status1 & STATUS1_ADCLOW_MASK;
...
//		adc, adcerr, chgtyp, vbvolt, dxovp, info->cable_type);
	printk(KERN_CRIT "@@MAX77693-MUIC@@ adc:%x adcerr:%x chgtyp:%x vb:%x dxovp:%x cable_type:%d\n",
		adc, adcerr, chgtyp, vbvolt, dxovp, info->cable_type);
...
static void max77693_muic_detect_dev(struct max77693_muic_info *info, int irq)
{
	...
	int intr = INT_ATTACH;
	printk(KERN_CRIT " @@MAX77693@@ %s func\n",__func__);
	max77693_read_reg(client, MAX77693_MUIC_REG_CTRL1, &cntl1_val);
	max77693_read_reg(client, MAX77693_MUIC_REG_CTRL2, &cntl2_val);
	max77693_read_reg(client, MAX77693_MUIC_REG_CTRL3, &cntl3_val);
	printk(KERN_CRIT "%s:%s CONTROL[1:0x%02x, 2:0x%02x, 3:0x%02x]\n",
...
//	dev_info(info->dev, "func:%s irq:%d ret:%d\n", __func__, irq, ret);
	printk(KERN_CRIT "func:%s irq:%d ret:%d\n", __func__, irq, ret);
	if (ret) {
...
static void max77693_muic_init_detect(struct work_struct *work)
{
	...
	//dev_info(info->dev, "func:%s\n", __func__);
	printk(KERN_CRIT "@@ MAX77693-MUIC @@ func:%s\n", __func__);
	mutex_lock(&info->mutex);
...
void max77693_update_jig_state(struct max77693_muic_info *info)
{
	...
	printk(KERN_CRIT "@@MAX77693-MUIC@@, Read ADC value is %x \n",adc);
	switch (adc) {
	case ADC_JIG_UART_OFF:
	case ADC_JIG_USB_OFF:
	case ADC_JIG_USB_ON:
...
//		 info->irq_adc, info->irq_chgtype, info->irq_adc1k);
	printk(KERN_CRIT "@@MAX77693-MUIC@@ adc:%d chgtype:%d, adc1k:%d\n",
		 info->irq_adc, info->irq_chgtype, info->irq_adc1k);
	...
	//	dev_info(info->dev, "H/W rev connected UT1 UR2 pin to CP UART");
		printk(KERN_CRIT "H/W rev connected UT1 UR2 pin to CP UART\n");}
	else {
	//	dev_info(info->dev, "H/W rev connected UT1 UR2 pin to AP UART");
		printk(KERN_CRIT "H/W rev connected UT1 UR2 pin to AP UART\n");}
...
/*PASS2 */
...
#endif
	printk(KERN_CRIT "%s: switch_sel: %x\n", __func__, switch_sel);
	...
	/*Over riding sw_path from bootloader*/
	/*Temporary fix until bootloader is up*/
	info->muic_data->sw_path = AP_USB_MODE;
	...
	printk(KERN_CRIT "%s:%s CONTROL[1:0x%x, 2:0x%x, 3:0x%x]\n", DEV_NAME, __func__,
