...
static int tx_qlen = 50;
module_param(tx_qlen, int, S_IRUGO | S_IWUSR | S_IWGRP);
MODULE_PARM_DESC(tx_qlen, "tx qlen");

static int rx_ntb = 32;	/* ncm rx buf size */
module_param(rx_ntb, int, S_IRUGO | S_IWUSR | S_IWGRP);
MODULE_PARM_DESC(rx_ntb, "ncm rx buf size");
...
#if defined(CONFIG_SEC_MODEM_XMM6360) || defined(CONFIG_SEC_MODEM_ERICSSON)
static void mif_net_suspend(struct if_usb_devdata *pipe_data, int flag)
{
	if (!pipe_data->iod->ndev || !flag)
		return;

	if (pipe_data->net_suspend & flag)
		return;

	pipe_data->net_suspend |= flag;
	if (pipe_data->usb_ld->max_link_ch == 2)
		link_tx_flowctl(&pipe_data->usb_ld->ld, 1);
	else
...
static void mif_net_resume(struct if_usb_devdata *pipe_data, int flag)
{
	...
	if (!pipe_data->net_suspend) {
		if (pipe_data->usb_ld->max_link_ch == 2)
			link_tx_flowctl(&pipe_data->usb_ld->ld, 0);
		else
