...
static void sec_jack_gpio_init(struct sec_jack_platform_data *pdata)
{
	...
	if (pdata->det_en_gpio > 0) {
			ret = gpio_request(pdata->det_en_gpio, "det_en");
			if (ret) {
				pr_err("%s : gpio_request failed for %d\n", __func__,
					pdata->det_en_gpio);
				return;
			}
			gpio_direction_output(pdata->det_en_gpio, 1);
		}
...
static struct sec_jack_platform_data *sec_jack_populate_dt_pdata(struct device *dev)
{
	...
	pdata->det_en_gpio = of_get_named_gpio(dev->of_node, "qcom,earjack-deten-gpio", 0);
	if (pdata->det_en_gpio < 0)
		pr_info("%s : cannot find the ear_det_en gpio in the dt \n", __func__);
	else
		pr_info("%s : earjack-deten-gpio =%d\n", __func__, pdata->det_en_gpio);
...
static int sec_jack_remove(struct platform_device *pdev)
{
	...
	gpio_free(hi->pdata->det_en_gpio);
	kfree(hi);
	atomic_set(&instantiated, 0);
...
static int sec_jack_suspend(struct device *dev)
{
	...
	if (pdata->det_en_gpio)
		gpio_direction_output(pdata->det_en_gpio, 0);
