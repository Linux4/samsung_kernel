...
//balaji.k: Enabling Ear Mic Bias of WCD Codec
...
static int sec_jack_get_adc_value(struct sec_jack_platform_data *pdata)
{
...
static void set_sec_micbias_state(struct sec_jack_info *hi, bool state)
{
	...
	//balaji.k: Enabling Ear Mic Bias2 Of WCDCodec
	msm8226_enable_ear_micbias(state);
}
...
static void determine_jack_type(struct sec_jack_info *hi)
{
	...
	while (gpio_get_value(pdata->det_gpio) ^ npolarity) {
		...
		adc = sec_jack_get_adc_value(pdata);
		pr_info("%s: adc = %d\n", __func__, adc);
...
static struct sec_jack_platform_data *sec_jack_populate_dt_pdata(struct device *dev)
{
	...
	if (pdata->det_gpio < 0) {
		...
		printk("%s : earjack-detect-gpio =%d\n", __func__, pdata->det_gpio);
	...
	if (pdata->send_end_gpio < 0) {
		...
		printk("%s : earjack-sendend-gpio =%d\n", __func__, pdata->send_end_gpio);
/*
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	gpio_tlmm_config(GPIO_CFG(pdata->det_gpio,
	...
	if (ret < 0) {
		pr_err("%s : Failed to register switch device\n", __func__);
		goto err_switch_sendend_register;
	}
