...
#define JPG1_MINOR MISC_DYNAMIC_MINOR
...
by clk_get()!\n", "clk_vsp", name_parent);
    ...
    case JPG_ENABLE:
        ...
        sci_glb_set(SPRD_MMAHB_BASE+0x08, BIT(9));
#else
...
#ifdef CONFIG_OF
#ifdef CONFIG_ARCH_WHALE
        sci_glb_clr(SPRD_MMAHB_BASE+0x08, BIT(9));
#else
    ...
    case JPG_ACQUAIRE:
        ...
        printk("jpg ioctl JPG_ACQUAIRE end\n");
        break;
    case JPG_RELEASE:
        release_jpg(jpg_fp);
        break;
    ...
    case JPG_RESET:
        ...
        sci_glb_set(SPRD_MMAHB_BASE+0x04, BIT(7));
        sci_glb_clr(SPRD_MMAHB_BASE+0x04, BIT(7));
...
static void jpg_parse_dt(struct device *dev)
{
	...
	printk(KERN_INFO "sprd_jpg_phys jpg1:  0X%x   0x%x\n", sprd_jpg_phys ,sprd_jpg_virt);
...
static struct miscdevice jpg_dev1 = {
    .minor   = JPG1_MINOR,
    .name   = "sprd_jpg1",
    .fops   = &jpg_fops,
};
...
static struct platform_driver jpg_driver1 = {
    .probe    = jpg_probe,
    .remove   = jpg_remove,
    .driver   = {
        .owner = THIS_MODULE,
...
static int __init jpg_init(void)
{
    ...
    if (platform_driver_register(&jpg_driver1) != 0) {
...
static void __exit jpg_exit(void)
{
    ...
    platform_driver_unregister(&jpg_driver1);
}
