...
static int __init  get_sales_region(char *str)
{
    if(!str) {
        pr_err("can not get sales code form bootargs\n");
        return -1;
    }

    strncpy(sales_region, str, sizeof(sales_region));
    sales_region[sizeof(sales_region) - 1] = '\0';
    pr_info("wcn get sales_region:%s\n", sales_region);
    return 0;
}
early_param("androidboot.sales_code", get_sales_region);

const char* wcn_get_sales_region(void)
{
    return sales_region;
}
EXPORT_SYMBOL_GPL(wcn_get_sales_region);
