...
void cali_save_file_top(struct file *fp, struct wifi_cali_t *p)
{
	int i;
...
void cali_save_file_mid(struct file *fp, struct wifi_cali_t *p)
{
	int i, j;
...
void cali_save_file_bottom(struct file *fp, struct wifi_cali_t *p)
{
	int i, j;
...
void dump_cali_file(struct wifi_cali_t *p)
{
	...
	fp = filp_open(WIFI_CALI_DUMP_FILE, O_RDWR | O_CREAT | O_TRUNC, 0771);
	if (IS_ERR_OR_NULL(fp)) {
		pr_err("%s(), open error!\n", __func__);
		return;
	}

	//Bug1677618: split cali_save_file into to avoid KASAN compile error.
	cali_save_file_top(fp, p);
	cali_save_file_mid(fp, p);
	cali_save_file_bottom(fp, p);
