...
static int check_simslot_count(struct seq_file *m, void *v)
{
	...
	if (retval) {
			printk("%s:AFailed to reqeust GPIO, code = %d.\n",
				__func__, retval);
			support_number_of_simslot = retval;
		 printk(" \n retval %d",support_number_of_simslot);
	}
	else
	{
		...
		if (retval){
			printk("%s:BFailed to set direction of GPIO, code = %d.\n",
				__func__, retval);
			support_number_of_simslot = retval;
			 printk(" \n retval %d",support_number_of_simslot);
	...
	{
		printk("******* Make a forced kernel panic because can't check simslot count******\n");
		panic("kernel panic");
	}
	 printk(" \n support_number_of_simslot  %d",support_number_of_simslot);
...
{	
	printk("3 Entering check_simslot_count_open");
...
{
	printk("1  Entering simslot_count_init");
	if(!proc_create("simslot_count",0,NULL,&check_simslot_count_fops))
	{	
		 printk("2 Entering simslot_count_init fi condition");
		printk("***** Make a forced kernel panic because can't make a simslot_count file node ******\n"); 
		panic("kernel panic");
		return -ENOMEM;
	}
	else return 0;
