...
void process_data(struct bhy_client_data *client_data, u8 *data, u16 handle)
{
	...
	case REACTIVE_ALERT_SENSOR:
		memcpy(&recactive_alert_temp, data, sizeof(recactive_alert_temp));
		client_data->reactive_alert_reported = true;
		PINFO("%s : reactive_alert = %d OK!\n", __func__, recactive_alert_temp);
		break;
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sysfs_streq(buf, "1")){
		...
		PERR("%s - Invalid Value %d\n", __func__, *buf);
		return -EINVAL;
	}

	if ((enable == true) && (reactive_alert_enabled == false)) {
		ret = enable_sensor(client_data, BHY_SENSOR_HANDLE_CUSTOM_4_WU, true, 50);
		if (ret < 0) {
		...
		ret = enable_sensor(client_data, BHY_SENSOR_HANDLE_CUSTOM_4_WU, false, 50);
		if (ret < 0) {
...
static int enalbe_lowpassfilter(struct bhy_client_data *client_data,
				bool enable)
{
	u8 bandwidth;
	int ret;
	...
	if (enable) {
		...
		mutex_unlock(&client_data->mutex_bus_op);
		if (ret < 0) {
			PERR("enalbe_lowpassfilter read err");
			return -EIO;
		}
	...
	mutex_unlock(&client_data->mutex_bus_op);
	if (ret < 0)
		PERR("enalbe_lowpassfilter write err");
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = enalbe_lowpassfilter(client_data, (bool)buffer);
	if (ret < 0)
		return -EIO;
