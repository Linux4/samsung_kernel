...
static struct dsi_cmd_desc quicklogic_generic_csr_wr_cmd[] = {
	{ DSI_DI_GENERIC_LWRITE, 1, 0,\
		sizeof(quicklogic_csr_wr_payload), quicklogic_csr_wr_payload}
};

int Quicklogic_mipi_write(struct pxa168fb_info *fbi, u32 address,
				u32 value, u32 data_size)
{

	mutex_lock(&lock);

	quicklogic_csr_wr_payload[3] = (u8)(address & 0xff);	/* Add LSB */
	quicklogic_csr_wr_payload[4] = (u8)((address >> 8) & 0xff);/*Add MSB*/
	...
	dsi_cmd_array_tx(fbi, quicklogic_generic_csr_wr_cmd,\
				ARRAY_SIZE(quicklogic_generic_csr_wr_cmd));
...
int Quicklogic_i2cTomipi_write(int dtype, int vit_com, u8 data_size, u8 *ptr)
{
	...
	/*printk(KERN_INFO "fbi_global avtive =[%d]\n",fbi_global->video_on);*/
	if (g_client == NULL || (fbi_global->video_on == 0))
		return -1;

	if (data_size > (QL_MIPI_PANEL_CMD_SIZE - 10)) {
		printk(KERN_INFO "data size(%d)too big\n", data_size);
		return ret;
	}
	dtype_int = dtype;
	...
	default:
		printk(KERN_INFO\
		"Error unknown data type (0x%x)\n", dtype);
		break;
	}

	buf[1] = ((vit_com & 0x3) << 6) | dtype_int ;/* Copy data */
	...
	if ((ret = i2c_master_send(g_client, (char *)(&buf[0]),\
						write_size)) != write_size) {
		printk(KERN_INFO "Quicklogic_i2c_write32 failed : %d\n", ret);
...
int Quicklogic_i2cTomipi_read(int dtype, int vit_com,
				u8 data_size, u8 reg , u8 *pval)
{
	...
	default:
		printk(KERN_INFO "Error unknown data type (0x%x)\n", dtype);
		break;
	}
	...
	if ((ret = i2c_master_send(g_client, (char *)(&buf[0]),
		...
		printk(KERN_INFO "i2c_master_send failed (%d)!\n", ret);
	...
	if ((ret = i2c_master_recv(g_client, (char *)(&rx[0]),
		...
		printk(KERN_INFO "i2c_master_recv failed (%d)!\n", ret);
	...
	if (ret < 0)
		printk(KERN_INFO "Quicklogic_i2c_read32 failed : %d\n", ret);
	...
	*pval  = rx[0];
	...
	printk(KERN_INFO "QuickBridge reg=[0x%x]return=[0x%x]\n", reg, *pval);
				    ...
				    const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "quickbridge_probe\n");
#ifdef	CONFIG_PROC_FS
