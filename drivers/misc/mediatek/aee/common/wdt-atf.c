...
#if defined(CONFIG_MTK_AEE_MRDUMP) && (0)
...
#else
void flash_aee_wdt_percpu2_printk(int cpu)
{
	if (wdt_percpu_log_buf[cpu]) {
		pr_err("--------------- [cpu=%d] ----------------\n %s\n", 
			cpu, wdt_percpu_log_buf[cpu]);
	}
	else
		pr_err("wdt_percpu_log_buf[%d]=null", cpu);
}

#define aee_wdt_printf(fmt, ...) \
...
void aee_wdt_atf_info(unsigned int cpu, struct pt_regs *regs)
{
	...
	flash_aee_wdt_percpu2_printk(cpu);
