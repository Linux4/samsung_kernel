...
enum ISP_WAITQ_HEAD_IRQ_ENUM {
	...
	ISP_WAITQ_HEAD_IRQ_IMGO_DONE,
	ISP_WAITQ_HEAD_IRQ_RRZO_DONE,
	ISP_WAITQ_HEAD_IRQ_UFEO_DONE,
	ISP_WAITQ_HEAD_IRQ_LCSO_DONE,
	ISP_WAITQ_HEAD_IRQ_EISO_DONE,
	...
	enum ISP_ST_ENUM st_type, unsigned int status)
{
	...
	if (st_type == SIGNAL_INT) {
		...
		if (status == IMGO_DONE_ST)
			index = ISP_WAITQ_HEAD_IRQ_IMGO_DONE;
		else if (status == RRZO_DONE_ST)
			index = ISP_WAITQ_HEAD_IRQ_RRZO_DONE;
		else if (status == UFEO_DONE_ST)
			index = ISP_WAITQ_HEAD_IRQ_UFEO_DONE;
		else if (status == LCSO_DONE_ST)
			index = ISP_WAITQ_HEAD_IRQ_LCSO_DONE;
		else if (status == EISO_DONE_ST)
			index = ISP_WAITQ_HEAD_IRQ_EISO_DONE;
		else if (status == AAO_DONE_ST)
...
LB_CAMA_SOF_IGNORE:
	...
	if (DmaStatus & IMGO_DONE_ST) {
		wake_up_interruptible(&IspInfo.WaitQHeadCam
			[ISP_GetWaitQCamIndex(module)]
			[ISP_WAITQ_HEAD_IRQ_IMGO_DONE]);
	}
	if (DmaStatus & RRZO_DONE_ST) {
		wake_up_interruptible(&IspInfo.WaitQHeadCam
			[ISP_GetWaitQCamIndex(module)]
			[ISP_WAITQ_HEAD_IRQ_RRZO_DONE]);
	}
	if (DmaStatus & UFEO_DONE_ST) {
		wake_up_interruptible(&IspInfo.WaitQHeadCam
			[ISP_GetWaitQCamIndex(module)]
			[ISP_WAITQ_HEAD_IRQ_UFEO_DONE]);
	}
	if (DmaStatus & LCSO_DONE_ST) {
		wake_up_interruptible(&IspInfo.WaitQHeadCam
			[ISP_GetWaitQCamIndex(module)]
			[ISP_WAITQ_HEAD_IRQ_LCSO_DONE]);
	}
	if (DmaStatus & EISO_DONE_ST) {
		wake_up_interruptible(&IspInfo.WaitQHeadCam
			[ISP_GetWaitQCamIndex(module)]
			[ISP_WAITQ_HEAD_IRQ_EISO_DONE]);
	}
