...
static void perfmgr_touch_boost(int enable, int core, int freq)
{
#ifdef MTK_BOOST_SUPPORT
	if(enable) {
		perfmgr_boost_core(PERFMGR_SCN_TOUCH, core);
		perfmgr_boost_freq(PERFMGR_SCN_TOUCH, freq);
	}
	else {
		perfmgr_boost_core(PERFMGR_SCN_TOUCH, PERFMGR_IGNORE);
		perfmgr_boost_freq(PERFMGR_SCN_TOUCH, PERFMGR_IGNORE);
	}
#endif
}
...
static int tboost_thread(void *ptr)
{
	...
	while (!kthread_should_stop()) {
		...
		perfmgr_touch_boost(event, core, freq);
	}
	return 0;
}
...
int init_perfmgr_touch(void)
{
	...
	perf_mgr_touch_core = TARGET_CORE;
	perf_mgr_touch_freq = TARGET_FREQ;
