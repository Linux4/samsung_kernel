...
uint32_t hal_tui_deactivate(void)
{
	...
	pr_info("[TUI-HAL] %s TUI_SESSION masked\n", __func__);
	/*
...
uint32_t hal_tui_activate(void)
{
...
#ifdef TUI_LOCK_I2C
	i2c_tui_disable_clock(6);
#endif

	irq_set_irq_type(tsp_irq_num, IRQF_TRIGGER_FALLING);
	...
	pr_info("[TUI-HAL] %s TUI_SESSION unmasked\n", __func__);
	pr_info("[TUI-HAL] %s-\n", __func__);
	return TUI_DCI_OK;
}
