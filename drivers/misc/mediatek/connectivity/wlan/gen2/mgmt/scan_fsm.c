...
VOID scnSendScanReq(IN P_ADAPTER_T prAdapter)
{
	...
	if (prScanParam->ucChannelListNum > 32) {
		...
		if (kalMemCmp(prScanParam->aucBSSID, "\0\0\0\0\0\0", MAC_ADDR_LEN)) {
			kalMemCopy(prCmdScanReq->aucBSSID, prScanParam->aucBSSID, MAC_ADDR_LEN);
...
VOID scnSendScanReqV3ExtCh(IN P_ADAPTER_T prAdapter)
{
	P_SCAN_INFO_T prScanInfo;
	P_SCAN_PARAM_T prScanParam;
	CMD_SCAN_REQ_V3_EXT_CH rCmdScanReq;
	UINT_32 i;

	ASSERT(prAdapter);
	DBGLOG(SCN, ERROR, "scnSendScanReqV3ExtCh\n");

	prScanInfo = &(prAdapter->rWifiVar.rScanInfo);
	prScanParam = &prScanInfo->rScanParam;

	/* send command packet for scan */
	kalMemZero(&rCmdScanReq, sizeof(CMD_SCAN_REQ_V3_EXT_CH));
	...
	wlanSendSetQueryCmd(prAdapter,
			    ...
			    NULL,
			    NULL,
			    OFFSET_OF(CMD_SCAN_REQ_V3_EXT_CH, aucIE) + rCmdScanReq.u2IELen,
...
VOID scnSendScanReqV2(IN P_ADAPTER_T prAdapter)
{
	...
	P_CMD_SCAN_REQ_V3 prCmdScanReqV3;//MULTI SSID
	...
	if (prScanParam->ucSSIDNum == 0) {
		...
		if (prScanParam->ucChannelListNum > 32) {
			...
			if (prCmdScanReqV3 == NULL) {
				DBGLOG(SCN, INFO, "alloc CmdScanReq v3 fail");
				return;
			}
			/* send command packet for scan */
			...
			for (i = 0; i < prScanParam->ucSSIDNum; i++) {
				if (i == (SCAN_REQ_V3_SSID_MAX_NUM - 1))
					break;
				COPY_SSID(prCmdScanReqV3->arSSID[i + 1].aucSsid,
					  prCmdScanReqV3->arSSID[i + 1].u4SsidLen,
					  prScanParam->aucSpecifiedSSID[i], prScanParam->ucSpecifiedSSIDLen[i]);
			}
			...
			DBGLOG(SCN, WARN, "send CmdScanReq v3\n");
		}
	}
}
