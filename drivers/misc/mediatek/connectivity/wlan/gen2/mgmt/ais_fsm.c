...
VOID aisInitializeConnectionSettings(IN P_ADAPTER_T prAdapter, IN P_REG_INFO_T prRegInfo)
{
	...
	if (prRegInfo) {
		DBGLOG(INIT, INFO, "IsHw5GBandDisabled: %d Support5GBand: %d\n",
		...
		DBGLOG(INIT, INFO, "RegInfo is NULL!\n");
...
VOID aisFsmSteps(IN P_ADAPTER_T prAdapter, ENUM_AIS_STATE_T eNextState)
{
	...
	do {
		...
		case AIS_STATE_SEARCH:
			...
			/* 4 <3> We are under Roaming Condition. */
				...
				if (prAisBssInfo->ucReasonOfDisconnect != DISCONNECT_REASON_CODE_REASSOCIATION &&
					...
					if (prAisFsmInfo->fgFullRoamScnIssued)
						aisFsmSetRoamingThreshold(prAdapter, AIS_DRT_ROAMING_THRESHOLD);
			...
			if (prBssDesc && prConnSettings->fgUseOkc) {
				...
				if (rsnSearchPmkidEntry(prAdapter, prBssDesc->aucBSSID, &u4Entry) &&
					...
					break;
				DBGLOG(AIS, INFO, "No PMK found, try to generate a OKC PMK\n");
		...
		case AIS_STATE_LOOKING_FOR:
			...
			kalMemZero(prScanReqMsg->aucBSSID, MAC_ADDR_LEN);
#endif
#endif
			...
			if (cnmAisInfraChannelFixed(prAdapter, &eBand, &ucChannel) == TRUE) {
				...
				if (prAdapter->rNchoInfo.fgECHOEnabled &&
					...
					UINT_32	u4size = 0;
					PCFG_NCHO_SCAN_CHNL_T prRoamScnChnl = &prAdapter->rNchoInfo.rRoamScnChnl;

					ucChnlCount += prRoamScnChnl->ucChannelListNum;
					if (ucChnlCount > MAXIMUM_OPERATION_CHANNEL_LIST) {
						ucChnlCount = MAXIMUM_OPERATION_CHANNEL_LIST;
						u4size = (MAXIMUM_OPERATION_CHANNEL_LIST - ucIdx) *
								sizeof(RF_CHANNEL_INFO_T);
					} else
						u4size = prRoamScnChnl->ucChannelListNum * sizeof(RF_CHANNEL_INFO_T);

					DBGLOG(AIS, INFO,
						"NCHO SCAN channel num = %d, total size=%d\n",
						prScanReqMsg->ucChannelListNum, u4size);
					prChnlInfo = &prScanReqMsg->arChnlInfoList[ucIdx];
					kalMemCopy(prChnlInfo, &prRoamScnChnl->arChnlInfoList[0], u4size);
				}
#endif
#if CFG_SUPPORT_802_11V_BSS_TRANSITION_MGT
...
static VOID aisFsmSetRoamingThreshold(P_ADAPTER_T prAdapter, INT_8 cThreshold)
{
	...
	DBGLOG(AIS, INFO, "cThreshold %d, fgECHOEnabled %d, cRoamTriggerThreshold %d\n",
		cThreshold, prAdapter->rNchoInfo.fgECHOEnabled, prAisFsmInfo->cRoamTriggerThreshold);
