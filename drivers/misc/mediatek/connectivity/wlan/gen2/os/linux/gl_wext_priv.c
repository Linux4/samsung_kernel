	...
	case PRIV_CMD_SET_TX_POWER:
		{
			...
			DBGLOG(REQ, INFO, "%s: %d, 0x%x, %d, 0x%x\n", __func__,
...
INT_32 priv_driver_cmds(IN struct net_device *prNetDev, IN PCHAR pcCommand, IN INT_32 i4TotalLen)
{
	...
	if (i4CmdFound == 0) {
		...
		if (strnicmp(pcCommand, CMD_MIRACAST, strlen(CMD_MIRACAST)) == 0)
		...
		else if (strnicmp(pcCommand, CMD_ECSA, strlen(CMD_ECSA)) == 0)
...
#endif
		else if (strnicmp(pcCommand, CMD_SET_FCC_CERT, strlen(CMD_SET_FCC_CERT)) == 0) {
			...
			if (pFccTxPwrAdjust->fgFccTxPwrAdjust == 0)
				...
				if (strnicmp(pcCommand, "-1", strlen("-1")) != 0 && *pcCommand != '0')
					...
					memset(&rFccTxPwrAdjust, 0, sizeof(rFccTxPwrAdjust));
#if 0
...
#else
					memcpy(&rFccTxPwrAdjust, pFccTxPwrAdjust, sizeof(FCC_TX_PWR_ADJUST));
#endif
