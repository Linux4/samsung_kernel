		      ...
		      IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(PARAM_CUSTOM_SW_CTRL_STRUCT_T);
	...
	case 0x9F00:
		...
		/* set return value as pending to let the OID operation keep sync with tx_thread */
		DBGLOG(OID, INFO, "return pending to let the OID operation keep sync with tx_thread\n");
		rWlanStatus = WLAN_STATUS_PENDING;
		break;
#endif /* CFG_SUPPORT_SWCR */
		    ...
		    PUINT_8 pucInfoBuffer, OUT PVOID pvSetQueryBuffer, IN UINT_32 u4SetQueryBufferLen)
{
	...
	/* increase command sequence number */
	ucCmdSeqNum = nicIncreaseCmdSeqNum(prAdapter);
	DBGLOG(OID, TRACE, "ucCmdSeqNum =%d, ucCID =0x%x\n", ucCmdSeqNum, ucCID);
