...
#if  CFG_WMT_BUG_SWITCH
static INT32 option_trigger_ke_enable = 1;
#else
static INT32 option_trigger_ke_enable;
#endif
static INT32 adb_trigger_ke_enable;
static INT32 config_file_ke_enable;
...
#endif
UINT32 wmt_btm_bug_switch_ctrl(UINT32 en)
{
	STP_BTM_INFO_FUNC("%s bug switch for debug\n", en ? "enable" : "disable");
	adb_trigger_ke_enable = en;
	return 0;
}
static INT32 _stp_btm_handler(MTKSTP_BTM_T *stp_btm, P_STP_BTM_OP pStpOp)
{
	...
	case STP_OPID_BTM_RST:
		...
		config_file_ke_enable = wmt_lib_conf_bug_report_flag_get();
		if (adb_trigger_ke_enable || config_file_ke_enable || (option_trigger_ke_enable == 1))
			BUG();
		break;
...
#if 1
			osal_memcpy(&g_paged_dump_buffer[0], dump_vir_addr,
				dump_len < STP_DBG_PAGED_DUMP_BUFFER_SIZE ?
				dump_len : STP_DBG_PAGED_DUMP_BUFFER_SIZE);
			_stp_dump_emi_dump_buffer(&g_paged_dump_buffer[0],
				dump_len < STP_DBG_PAGED_DUMP_BUFFER_SIZE ?
				dump_len : STP_DBG_PAGED_DUMP_BUFFER_SIZE);
...
paged_dump_end:
		...
		/* wait 5s for stp_dump save coredump file */
		osal_sleep_ms(5000);
