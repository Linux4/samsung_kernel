...
#endif
static INT32 wmt_dbg_bug_switch_ctrl(INT32 par1, INT32 par2, INT32 par3);
		...
		do {
			...
			if (cur_idx_pagedtrace < prev_idx_pagedtrace) {
				if (prev_idx_pagedtrace >= 0x8000) {
					pr_warn("++ prev_idx_pagedtrace invalid ...++\n\\n");
...
static INT32 wmt_dbg_bug_switch_ctrl(INT32 par1, INT32 par2, INT32 par3)
{
	...
	wmt_lib_bug_switch_flag_set(en_flag);
	return 0;
}
	...
	case 10:
		{
			...
			pBuffer[NAME_MAX] = '\0';
			WMT_INFO_FUNC("MT662x f/w coredump start-");
	...
	case 11:
		{
			...
			WMT_INFO_FUNC("MT662x f/w coredump end", "MT662x firmware coredump ends");
			wmt_lib_host_awake_put();
		}
		break;
