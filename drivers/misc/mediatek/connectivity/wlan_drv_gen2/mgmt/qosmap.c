...
static void QosMapSetFree(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	GLUE_SPIN_LOCK_DECLARATION();

	GLUE_ACQUIRE_SPIN_LOCK(prAdapter->prGlueInfo, SPIN_LOCK_QOSMAPSET);
	...
	GLUE_RELEASE_SPIN_LOCK(prAdapter->prGlueInfo, SPIN_LOCK_QOSMAPSET);
}

void QosMapSetRelease(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	QosMapSetFree(prAdapter, prStaRec);
}
...
int qosHandleQosMapConfigure(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	QosMapSetFree(prAdapter, prStaRec);
	qosParseQosMapSet(prAdapter, prRxFrame->qosMapSet, prStaRec);
	return 0;
}
...
void qosParseQosMapSet(IN P_ADAPTER_T prAdapter, IN PUINT_8 qosMapSet, IN P_STA_RECORD_T prStaRec)
{
	...
	prStaRec->qosMapSet = QosMapSetMalloc(dscpExcNum);
	GLUE_ACQUIRE_SPIN_LOCK(prAdapter->prGlueInfo, SPIN_LOCK_QOSMAPSET);
	prQos = prStaRec->qosMapSet;
...
UINT_8 getUpFromDscp(IN P_GLUE_INFO_T prGlueInfo, IN int type, IN int dscp)
{
	...
	GLUE_ACQUIRE_SPIN_LOCK(prGlueInfo, SPIN_LOCK_QOSMAPSET);
	if (prStaRec && prStaRec->qosMapSet) {
		for (i = 0; i < prStaRec->qosMapSet->dscpExceptionNum; i++) {
			if (dscp == prStaRec->qosMapSet->dscpException[i].dscp) {
				uPriority = prStaRec->qosMapSet->dscpException[i].userPriority;
				j = 8;
				break;
			}
		}
	...
	GLUE_RELEASE_SPIN_LOCK(prGlueInfo, SPIN_LOCK_QOSMAPSET);
	return uPriority;
}
#endif
