...
kalIndicateStatusAndComplete(IN P_GLUE_INFO_T prGlueInfo, IN WLAN_STATUS eStatus, IN PVOID pvBuf, IN UINT_32 u4BufLen)
{
	...
	case WLAN_STATUS_MEDIA_CONNECT:
		...
		if (prGlueInfo->fgIsRegistered == TRUE) {
			...
			if (eStatus == WLAN_STATUS_ROAM_OUT_FIND_BEST) {
				...
				if (prBssDesc->fgIsSmeConnecting == TRUE) {
					DBGLOG(INIT, STATE,
						"%s cfg80211_connect_result in Connecting status(1)\n",
						__func__);
					...
					DBGLOG(INIT, ERROR,
						"%s ignor conn result report, not in connecting status(1)\n",
						__func__);
				}
			}
		}
	...
	case WLAN_STATUS_JOIN_FAILURE:
		...
		*/
			if (prBssDesc->fgIsSmeConnecting == TRUE) {
				DBGLOG(INIT, STATE,
					"%s cfg80211_connect_result in Connecting status(2)\n",
					__func__);
				...
				DBGLOG(INIT, ERROR,
					"%s ignor conn result report, not in connecting status(2)\n",
					__func__);
			}
		break;
