...
static P_GLUE_INFO_T g_prGlueInfo;
...
#define STR(x) STR_HELPER(x)
	...
	if (g_prGlueInfo)
		ucOffset += kalSnprintf(acVerInfo + ucOffset
			, MTK_INFO_MAX_SIZE - ucOffset
			, "FW_VER: 0x%x.%x\n"
			, g_prGlueInfo->prAdapter->rVerInfo.u2FwOwnVersion
			, g_prGlueInfo->prAdapter->rVerInfo.u2FwOwnVersionExtend);
	else {
		ucOffset += kalSnprintf(acVerInfo + ucOffset
			, MTK_INFO_MAX_SIZE - ucOffset
			, "FW_VER: %s\n"
...
int32_t sysHwAddrToBin(int8_t *txt, uint8_t *addr)
{
	int32_t i;
	int8_t *pos = txt;
