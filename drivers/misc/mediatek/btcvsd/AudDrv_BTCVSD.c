...
static int Auddrv_BTCVSD_Irq_Map(void)
{
	...
	if (node == NULL)
		pr_warn("BTCVSD get node failed\n");

	/*get btcvsd irq num */
	btcvsd_irq_number = irq_of_parse_and_map(node, 0);
	pr_warn("[BTCVSD] btcvsd_irq_number=%d\n", btcvsd_irq_number);
	if (!btcvsd_irq_number) {
		pr_warn("[BTCVSD] get btcvsd_irq_number failed!!!\n");
		return -1;
	}
	return 0;
}
...
{
	pr_warn("AudDrv_btcvsd_Allocate_Buffer(+) isRX=%d\n", isRX);
	...
	pr_warn("AudDrv_btcvsd_Allocate_Buffer(-)\n");
	return 0;
}

static int AudDrv_btcvsd_Free_Buffer(struct file *fp, kal_uint8 isRX)
{
	pr_warn("AudDrv_btcvsd_Free_Buffer(+) isRX=%d\n", isRX);
	...
	pr_warn("AudDrv_btcvsd_Free_Buffer(-)\n");
	return 0;
}
...
static long AudDrv_btcvsd_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	pr_warn("AudDrv_btcvsd_ioctl cmd = 0x%x arg = %lu\n", cmd, arg);
	...
	case SET_BTCVSD_STATE:{
			pr_warn("AudDrv SET_BTCVSD_STATE\n");
			if (arg == BT_SCO_TXSTATE_DIRECT_LOOPBACK) {
				btsco.uTXState = arg;
				btsco.uRXState = arg;
			} else if ((arg & 0x10) == 0) {	/*TX state*/
				btsco.uTXState = arg;
				pr_warn("SET_BTCVSD_STATE set btsco.uTXState to 0x%lu\n", arg);
			} else {	/*RX state*/
				btsco.uRXState = arg;
				pr_warn("SET_BTCVSD_STATE set btsco.uRXState to %lu\n", arg);
			}
			if (btsco.uTXState == BT_SCO_TXSTATE_IDLE
			    && btsco.uRXState == BT_SCO_RXSTATE_IDLE) {
				pr_warn("SET_BTCVSD_STATE disable BT IRQ disableBTirq = %d\n",
					 disableBTirq);
				if (disableBTirq == 0) {
	...
	default:{
			pr_warn("AudDrv_btcvsd_ioctl Fail command: %x\n", cmd);
			ret = -1;
			break;
		}
	}
...
static long AudDrv_btcvsd_compat_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	pr_warn("AudDrv_btcvsd_compat_ioctl cmd = 0x%x arg = %lu\n", cmd, arg);
	...
	if (ret < 0)
		pr_warn("AudDrv_btcvsd_compat_ioctl Fail\n");
	else
		pr_warn("-AudDrv_btcvsd_compat_ioctl\n");
				     ...
				     kal_uint32 uBlockSize, kal_uint32 uControl)
{
	...
	for (i = 0; i < uBlockSize; i++) {
		...
		packet_cnt++;
		if (packet_cnt == 30) {
			pr_warn("AudDrv_BTCVSD_ReadFromBT()Test Packet Loss\n");
				    ...
				    kal_uint32 uBlockSize)
{
	...
	/*pr_warn("AudDrv_BTCVSD_WriteToBT(+) btsco.pTX->iPacket_r=%d\n",btsco.pTX->iPacket_r);*/
			...
			if (btsco.uRXState == BT_SCO_RXSTATE_RUNNING
				...
				if (btsco.pRX->fOverflow) {
					if (btsco.pRX->iPacket_w - btsco.pRX->iPacket_r <=
						...
						pr_warn("AudDrv_BTCVSD_IRQ_handler pRX->fOverflow FALSE!!!\n");
					}
				}
...
static int AudDrv_btcvsd_probe(struct platform_device *dev)
{
	...
	if (ret < 0) {
		pr_warn("AudDrv_btcvsd_probe request_irq btcvsd_irq_number(%d) Fail\n",
			 btcvsd_irq_number);
	}
	...
	pr_warn("AudDrv_btcvsd_probe disable BT IRQ disableBTirq = %d\n", disableBTirq);
	if (disableBTirq == 0) {
		disable_irq(btcvsd_irq_number);
		Disable_CVSD_Wakeup();
		disableBTirq = 1;
...
#endif
	pr_warn("[BTCVSD probe] BTSYS_PKV_BASE_ADDRESS = %p BTSYS_SRAM_BANK2_BASE_ADDRESS = %p\n",
				   ...
				   size_t count, loff_t *offset)
{
	...
	while (count) {
		/*pr_warn("AudDrv_btcvsd_write btsco.pTX->iPacket_w=%d, btsco.pTX->iPacket_r=%d\n",
		btsco.pTX->iPacket_w, btsco.pTX->iPacket_r);*/
		spin_lock_irqsave(&auddrv_BTCVSDTX_lock, flags);
		...
		* SCO_TX_ENCODE_SIZE;
		...
		if (count <= (kal_uint32) copy_size)
			copy_size = count;
		/*pr_warn("AudDrv_btcvsd_write count=%d, copy_size=%d\n",count, copy_size);*/
		...
		if (copy_size != 0) {
			...
			if (BTSCOTX_WriteIdx + copy_size < btsco.pTX->u4BufferSize) {	/* copy once */
				...
				data_w_ptr += copy_size;
				count -= copy_size;
				/*pr_warn("AudDrv_btcvsd_write finish1, copy_size:%d, pTX->iPacket_w:%d,
				...
				/*pr_warn("AudDrv_btcvsd_write size_1=%d, size_2=%d\n",size_1,size_2);*/
				...
				if (!access_ok(VERIFY_READ, data_w_ptr, size_1)) {
					...
					if ((copy_from_user
						...
						pr_warn("AudDrv_write Fail 1 copy_from_user\n");
						return -1;
					}
				}
				spin_lock_irqsave(&auddrv_BTCVSDTX_lock, flags);
				...
				count -= copy_size;
				data_w_ptr += copy_size;
				/*pr_warn("AudDrv_btcvsd_write finish2, copy size:%d, pTX->iPacket_w=%d,
				pTX->iPacket_r=%d, count:%d\r\n", copy_size,btsco.pTX->iPacket_w,
				btsco.pTX->iPacket_r,count );*/
			}
		}
		...
		if (count != 0) {
			kal_uint64 t1, t2;
			/*pr_warn("AudDrv_btcvsd_write WAITING...btsco.pTX->iPacket_w=%d,
			...
			t2 = sched_clock();
			/*pr_warn("AudDrv_btcvsd_write WAKEUP...count=%d\n",count);*/
			t2 = t2 - t1;	/* in ns (10^9) */
			...
			if (ret < 0) {
				...
				pr_warn("%s(), timeout, condition is true\n", __func__);
			} else {
				pr_warn("%s(), condition is true before timeout\n", __func__);
			}
		}
	...
	while (count) {
		...
		if (BTSCORX_ReadIdx_tmp + read_size < btsco.pRX->u4BufferSize) {
			...
			if (copy_to_user((void __user *)Read_Data_Ptr,
				...
				pr_warn("AudDrv_btcvsd_read Fail 1 copy to user Ptr:%p,PcktBuf:%p,RIdx_tmp:%zu,r_sz:%zu",
		...
		/* copy twice */
			...
			if (copy_to_user
			    ((void __user *)Read_Data_Ptr,
			     ...
			     size_1)) {
				pr_warn("AudDrv_btcvsd_read Fail 2 copy to user R_Ptr:%p, PacketBuf:%p,RIdx_tmp:0x%zu, r_sz:%zu",
...
static int AudDrv_btcvsd_mod_init(void)
{
	...
	pr_warn("+AudDrv_btcvsd_mod_init\n");

#ifndef CONFIG_OF
	mtk_btcvsd_dev = platform_device_alloc(auddrv_btcvsd_name, -1);
	if (!mtk_btcvsd_dev) {
		pr_warn("-AudDrv_btcvsd_mod_init, platform_device_alloc() fail, return\n");
		return -ENOMEM;
	}

	ret = platform_device_add(mtk_btcvsd_dev);
	if (ret != 0) {
		pr_warn("-AudDrv_btcvsd_mod_init, platform_device_add() fail, return\n");
	...
	if (ret == misc_register(&AudDrv_btcvsd_device)) {
		pr_warn("AudDrv_btcvsd_mod_init misc_register Fail:%d\n", ret);
		return ret;
	}
