...
static unsigned long gpu_cust_lowbound_freq;
static unsigned long gpu_cust_upbound_freq;
static unsigned int  g_ui32PreFreqID;
static unsigned int  g_bottom_freq_id;



static unsigned int  g_cust_lowbound_freq_id;
...
extern void (*mtk_custom_lowbound_gpu_freq_fp)(unsigned long ui32Freq);
extern void (*mtk_custom_upbound_gpu_freq_fp)(unsigned long ui32Freq);
extern unsigned long (*mtk_get_custom_lowbound_gpu_freq_fp)(void);
extern unsigned long (*mtk_get_custom_upbound_gpu_freq_fp)(void);
extern void (*mtk_custom_lowbound_gpu_freq_idx_fp)(unsigned int ui32FreqLevel);
extern void (*mtk_custom_upbound_gpu_freq_idx_fp)(unsigned int ui32FreqLevel);
extern unsigned int (*mtk_get_custom_lowbound_gpu_freq_idx_fp)(void);
extern unsigned int (*mtk_get_custom_upbound_gpu_freq_idx_fp)(void);
...
bool ged_dvfs_gpu_freq_commit(unsigned long ui32NewFreqID, GED_DVFS_COMMIT_TYPE eCommitType)
{
	...
	if (NULL != ged_dvfs_gpu_freq_commit_fp)
	{
		...
		if (ui32NewFreqID > g_cust_lowbound_freq_id)
		{
			ui32NewFreqID = g_cust_lowbound_freq_id;
		...
		if (ui32NewFreqID != ui32CurFreqID)
		{
			...
			if (ged_dvfs_query_power_status()) {
				ui32CurFreqID = mt_gpufreq_get_cur_freq_index();
				ui32CurFreq = mt_gpufreq_get_freq_by_idx(ui32CurFreqID);
			} else
				ui32CurFreq = 0;
			...
			if(true==bCommited)
			{
				ged_log_trace_counter("Freq", ui32CurFreq);
...
static int ged_dvfs_freq_to_freqID(unsigned long ui32Freq)
{
	...
	{
		if (mt_gpufreq_get_freq_by_idx(i) == ui32Freq)
			ui32FreqLevel = ui32MaxLevel - i;
	}

	return ui32FreqLevel;

}

static void ged_dvfs_custom_floor_gpu_freq_idx(unsigned int ui32FreqLevel)
{
	...
	// 0 => The highest frequency
	// table_num - 1 => The lowest frequency
	g_cust_lowbound_freq_id = ui32MaxLevel - ui32FreqLevel;
	
	gpu_cust_lowbound_freq = mt_gpufreq_get_freq_by_idx(g_cust_lowbound_freq_id);


	if(g_cust_lowbound_freq_id < mt_gpufreq_get_cur_freq_index())
	{
		ged_dvfs_gpu_freq_commit(g_cust_lowbound_freq_id, GED_DVFS_SET_BOTTOM_COMMIT);
	}
...
static void ged_dvfs_custom_ceiling_gpu_freq_idx(unsigned int ui32FreqLevel)
{
...
static void ged_dvfs_custom_floor_gpu_freq(unsigned long ui32Freq)
{
	...
	if (gpu_debug_enable)
	{
		GED_LOGE("%s: freq = %lu", __func__,ui32Freq);
    }

	ui32MaxLevel = mt_gpufreq_get_dvfs_table_num() - 1;
	ui32FreqLevel = ged_dvfs_freq_to_freqID(ui32Freq);

	if (-1 == ui32FreqLevel)
	{
		GED_LOGE("[GED] Invalid input frequency : %lu", ui32Freq);
		ui32FreqLevel = ui32MaxLevel - g_cust_lowbound_freq_id;
	}
	
	ged_dvfs_custom_floor_gpu_freq_idx(ui32FreqLevel);
}

static void ged_dvfs_custom_ceiling_gpu_freq(unsigned long ui32Freq)
{
	...
	if (-1 == ui32FreqLevel)
	{
		GED_LOGE("[GED_K] Invalid input frequency : %lu", ui32Freq);
		ui32FreqLevel = ui32MaxLevel - g_cust_upbound_freq_id;
	}
	
	ged_dvfs_custom_ceiling_gpu_freq_idx(ui32FreqLevel);
}
#endif        
...
static unsigned int ged_dvfs_get_custom_floor_gpu_freq_idx(void)
{

	unsigned int ui32MaxLevel = mt_gpufreq_get_dvfs_table_num() - 1;

	return ui32MaxLevel - g_cust_lowbound_freq_id;
}

static unsigned int ged_dvfs_get_custom_ceiling_gpu_freq_idx(void)
{
...
static unsigned long ged_dvfs_get_custom_floor_gpu_freq(void)
{
	return gpu_cust_lowbound_freq;
}

static unsigned long ged_dvfs_get_custom_ceiling_gpu_freq(void)
{
	return gpu_cust_upbound_freq;
}
...
GED_ERROR ged_dvfs_system_init()
{
	...
	g_cust_lowbound_freq_id = mt_gpufreq_get_dvfs_table_num() - 1;
	gpu_cust_lowbound_freq = mt_gpufreq_get_freq_by_idx(g_cust_lowbound_freq_id);
	...
	mtk_custom_lowbound_gpu_freq_idx_fp = ged_dvfs_custom_floor_gpu_freq_idx;
	mtk_custom_upbound_gpu_freq_idx_fp = ged_dvfs_custom_ceiling_gpu_freq_idx;
	mtk_get_custom_lowbound_gpu_freq_idx_fp = ged_dvfs_get_custom_floor_gpu_freq_idx;
	mtk_get_custom_upbound_gpu_freq_idx_fp = ged_dvfs_get_custom_ceiling_gpu_freq_idx;
	mtk_custom_lowbound_gpu_freq_fp = ged_dvfs_custom_floor_gpu_freq;
	mtk_custom_upbound_gpu_freq_fp = ged_dvfs_custom_ceiling_gpu_freq;
	mtk_get_custom_lowbound_gpu_freq_fp = ged_dvfs_get_custom_floor_gpu_freq;
...
module_param(gpu_cust_lowbound_freq, ulong, 0644);
module_param(gpu_cust_upbound_freq, ulong, 0644);
module_param(g_cust_lowbound_freq_id, uint, 0644);
module_param(g_cust_upbound_freq_id, uint, 0644);
