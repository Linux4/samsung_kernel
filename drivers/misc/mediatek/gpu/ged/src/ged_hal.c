...
static struct dentry *gpsDvfsLowboundFreqEntry;
static struct dentry *gpsDvfsUpboundFreqEntry;
static struct dentry *gpsGpufreqAvailableFreqEntry;
...
/*-----------------------------------------------------------------------------*/
static ssize_t gpu_lowbound_freq_write_entry(const char __user *pszBuffer, size_t uiCount,
...
#define GED_HAL_DEBUGFS_SIZE 64
	...
	if ((uiCount > 0) && (uiCount < GED_HAL_DEBUGFS_SIZE)) {
		if (ged_copy_from_user(acBuffer, pszBuffer, uiCount) == 0) {
			acBuffer[uiCount] = '\0';
			if (sscanf(acBuffer, "%lu\n", &ulVal) == 1) {
			/*if (kstrtoul(acBuffer, GED_HAL_DEBUGFS_SIZE, &ulVal) == 0) {*/
				if (ulVal < 0)
					ulVal = 0;
				mtk_set_gpu_lowbound_freq(ulVal);
			}
		}
	}
	return uiCount;
...
/*-----------------------------------------------------------------------------*/
static ssize_t gpu_upbound_freq_write_entry(const char __user *pszBuffer, size_t uiCount,
	loff_t uiPosition, void *pvData)
{
#define GED_HAL_DEBUGFS_SIZE 64
	char acBuffer[GED_HAL_DEBUGFS_SIZE];
	...
	if ((uiCount > 0) && (uiCount < GED_HAL_DEBUGFS_SIZE)) {
		if (ged_copy_from_user(acBuffer, pszBuffer, uiCount) == 0) {
			...
			/*if (kstrtoul(acBuffer, GED_HAL_DEBUGFS_SIZE, &ulVal) == 0) {*/
				if (ulVal < 0)
					ulVal = 0;
				mtk_set_gpu_upbound_freq(ulVal);
			}
		}
	}
	return uiCount;
...
/*-----------------------------------------------------------------------------*/
static void *gpu_lowbound_freq_seq_entry_start(struct seq_file *psSeqFile, loff_t *puiPosition)
{
	if (*puiPosition == 0)
		return SEQ_START_TOKEN;
	return NULL;
...
/*-----------------------------------------------------------------------------*/
static void gpu_lowbound_freq_seq_entry_stop(struct seq_file *psSeqFile, void *pvData)
{

}
/*-----------------------------------------------------------------------------*/
static void *gpu_lowbound_freq_seq_entry_next(struct seq_file *psSeqFile, void *pvData, loff_t *puiPosition)
{
	return NULL;
}
/*-----------------------------------------------------------------------------*/
static int gpu_lowbound_freq_seq_entry_show(struct seq_file *psSeqFile, void *pvData)
{
	if (pvData != NULL) {
		unsigned int freqId;
		unsigned int freqId_comple;

		if (false == mtk_get_custom_boost_gpu_freq(&freqId))
			freqId = 0;
		freqId_comple = mt_gpufreq_get_dvfs_table_num() - freqId - 1;
		seq_printf(psSeqFile, "%d\n", mt_gpufreq_get_freq_by_idx(freqId_comple));
	}

	return 0;
}
/*-----------------------------------------------------------------------------*/
static struct seq_operations gsGpuLowboundFreqReadOps = {
	.start = gpu_lowbound_freq_seq_entry_start,
	.stop = gpu_lowbound_freq_seq_entry_stop,
	.next = gpu_lowbound_freq_seq_entry_next,
	.show = gpu_lowbound_freq_seq_entry_show,
};
/*-----------------------------------------------------------------------------*/
static void *gpu_upbound_freq_seq_entry_start(struct seq_file *psSeqFile, loff_t *puiPosition)
{
	if (*puiPosition == 0)
		return SEQ_START_TOKEN;
	return NULL;
...
/*-----------------------------------------------------------------------------*/
static void gpu_upbound_freq_seq_entry_stop(struct seq_file *psSeqFile, void *pvData)
{

}
/*-----------------------------------------------------------------------------*/
static void *gpu_upbound_freq_seq_entry_next(struct seq_file *psSeqFile, void *pvData, loff_t *puiPosition)
{
	return NULL;
}
/*-----------------------------------------------------------------------------*/
static int gpu_upbound_freq_seq_entry_show(struct seq_file *psSeqFile, void *pvData)
{
	if (pvData != NULL) {
		unsigned int freqId;
		unsigned int freqId_comple;

		if (false == mtk_get_custom_upbound_gpu_freq(&freqId))
			freqId = 0;
		freqId_comple = mt_gpufreq_get_dvfs_table_num() - freqId - 1;
		seq_printf(psSeqFile, "%d\n", mt_gpufreq_get_freq_by_idx(freqId_comple));
	}
...
/*-----------------------------------------------------------------------------*/
static struct seq_operations gsGpuUpboundFreqReadOps = {
	.start = gpu_upbound_freq_seq_entry_start,
	.stop = gpu_upbound_freq_seq_entry_stop,
	.next = gpu_upbound_freq_seq_entry_next,
	.show = gpu_upbound_freq_seq_entry_show,
};
/* ----------------------------------------------------------------------------- */
#ifdef GED_FDVFS_ENABLE
...
static void* ged_gpufreq_available_freq_seq_start(struct seq_file *psSeqFile, loff_t *puiPosition)
{
	if (0 == *puiPosition)
	{
		return SEQ_START_TOKEN;
...
static void ged_gpufreq_available_freq_seq_stop(struct seq_file *psSeqFile, void *pvData)
{

}

static void* ged_gpufreq_available_freq_next(struct seq_file *psSeqFile, void *pvData, loff_t *puiPosition)
{
	return NULL;
}

static int ged_gpufreq_available_freq_show(struct seq_file *psSeqFile, void *pvData)
{
	if (pvData != NULL)
	{
		int i = 0;
		for (i = 0; i < mt_gpufreq_get_dvfs_table_num(); i++)
			seq_printf(psSeqFile, "%d ", mt_gpufreq_get_freq_by_idx(i));
...
static struct seq_operations gsGpufreq_available_freq_ReadOps =
{
	.start = ged_gpufreq_available_freq_seq_start,
	.stop = ged_gpufreq_available_freq_seq_stop,
	.next = ged_gpufreq_available_freq_next,
	.show = ged_gpufreq_available_freq_show,
};
...
GED_ERROR ged_hal_init(void)
{
	...
	/* Control the gpu lowbound freq (freq type)[Customization] */
	err = ged_debugFS_create_entry(
			"gpu_lowbound_freq",
			gpsHALDir,
			&gsGpuLowboundFreqReadOps,
			gpu_lowbound_freq_write_entry,
			NULL,
			&gpsDvfsLowboundFreqEntry);

	if (unlikely(err != GED_OK)) {
		GED_LOGE("ged: failed to create gpu_lowbound_freq entry!\n");
		goto ERROR;
	}

	/* Control the gpu upbound freq (freq type)[Customization] */
	err = ged_debugFS_create_entry(
			"gpu_upbound_freq",
			gpsHALDir,
			&gsGpuUpboundFreqReadOps,
			gpu_upbound_freq_write_entry,
			NULL,
			&gpsDvfsUpboundFreqEntry);
	...
	/* Get GPU available freq */

	err = ged_debugFS_create_entry(
			"gpufreq_available_freq",
			gpsHALDir,
			&gsGpufreq_available_freq_ReadOps,
			NULL,
			NULL,
			&gpsGpufreqAvailableFreqEntry);
...
void ged_hal_exit(void)
{
	...
	ged_debugFS_remove_entry(gpsDvfsLowboundFreqEntry);
	ged_debugFS_remove_entry(gpsDvfsUpboundFreqEntry);
	...
	ged_debugFS_remove_entry(gpsDvfsGpuUtilizationEntry);
        ged_debugFS_remove_entry(gpsGpufreqAvailableFreqEntry);
#ifdef MTK_GED_KPI
