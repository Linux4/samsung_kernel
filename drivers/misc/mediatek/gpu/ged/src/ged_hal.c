...
static struct dentry* gpsCustomLowboundGPUFreqIdxEntry = NULL;
static struct dentry* gpsCustomUpboundGPUFreqIdxEntry = NULL;
static struct dentry* gpsCustomLowboundGPUFreqEntry = NULL;
...
static struct dentry* gpsDvfsStatusEntry = NULL;
static struct dentry* gpsDvfsCurFreqValueEntry = NULL;
...
//-----------------------------------------------------------------------------
static ssize_t ged_custom_lowbound_gpu_freq_idx_write_entry(const char __user *pszBuffer, size_t uiCount, loff_t uiPosition, void *pvData)
{
	...
	if ((0 < uiCount) && (uiCount < GED_HAL_DEBUGFS_SIZE))
	{
		if (0 == ged_copy_from_user(acBuffer, pszBuffer, uiCount))
		{
			...
			if (sscanf(acBuffer, "%d", &i32Value) == 1)
			{
				if (i32Value < 0)
					i32Value = 0;
				mtk_custom_lowbound_gpu_freq_idx(i32Value);
            }
...
//-----------------------------------------------------------------------------
static void* ged_custom_lowbound_gpu_freq_idx_seq_start(struct seq_file *psSeqFile, loff_t *puiPosition)
{
	if (0 == *puiPosition)
	{
		return SEQ_START_TOKEN;
...
//-----------------------------------------------------------------------------
static void ged_custom_lowbound_gpu_freq_idx_seq_stop(struct seq_file *psSeqFile, void *pvData)
{

}
//-----------------------------------------------------------------------------
static void* ged_custom_lowbound_gpu_freq_idx_seq_next(struct seq_file *psSeqFile, void *pvData, loff_t *puiPosition)
{
	return NULL;
}
//-----------------------------------------------------------------------------
static int ged_custom_lowbound_gpu_freq_idx_seq_show(struct seq_file *psSeqFile, void *pvData)
{
	if (pvData != NULL)
	{
		unsigned int ui32LowboundGpuFreqLevel;
		if (false == mtk_get_custom_lowbound_gpu_freq_idx(&ui32LowboundGpuFreqLevel))
		{
			ui32LowboundGpuFreqLevel = 0;
			seq_printf(psSeqFile, "call mtk_get_custom_lowbound_gpu_freq_idx false\n");
		}
		else
			seq_printf(psSeqFile, "%u\n", ui32LowboundGpuFreqLevel);
	}

	return 0;
}
//-----------------------------------------------------------------------------
static struct seq_operations gsCustomLowboundGpuFreqIdxReadOps = 
{
	.start = ged_custom_lowbound_gpu_freq_idx_seq_start,
	.stop = ged_custom_lowbound_gpu_freq_idx_seq_stop,
	.next = ged_custom_lowbound_gpu_freq_idx_seq_next,
	.show = ged_custom_lowbound_gpu_freq_idx_seq_show,
};
//-----------------------------------------------------------------------------
static ssize_t ged_custom_upbound_gpu_freq_idx_write_entry(const char __user *pszBuffer, size_t uiCount, loff_t uiPosition, void *pvData)
{
#define GED_HAL_DEBUGFS_SIZE 64
	char acBuffer[GED_HAL_DEBUGFS_SIZE];
	...
	if ((0 < uiCount) && (uiCount < GED_HAL_DEBUGFS_SIZE))
	{
		if (0 == ged_copy_from_user(acBuffer, pszBuffer, uiCount))
		{
			...
			if (sscanf(acBuffer, "%d", &i32Value) == 1)
			{
				if (i32Value < 0)
					i32Value = 0;
				mtk_custom_upbound_gpu_freq_idx(i32Value);
			}
			//else if (...) //for other commands
			//{
			//}
...
//-----------------------------------------------------------------------------
static void* ged_custom_upbound_gpu_freq_idx_seq_start(struct seq_file *psSeqFile, loff_t *puiPosition)
{
	if (0 == *puiPosition)
	{
		return SEQ_START_TOKEN;
...
//-----------------------------------------------------------------------------
static void ged_custom_upbound_gpu_freq_idx_seq_stop(struct seq_file *psSeqFile, void *pvData)
{

}
//-----------------------------------------------------------------------------
static void* ged_custom_upbound_gpu_freq_idx_seq_next(struct seq_file *psSeqFile, void *pvData, loff_t *puiPosition)
{
	return NULL;
}
//-----------------------------------------------------------------------------
static int ged_custom_upbound_gpu_freq_idx_seq_show(struct seq_file *psSeqFile, void *pvData)
{
	if (pvData != NULL)
	{
		unsigned int ui32UpboundGpuFreqLevel;
		if (false == mtk_get_custom_upbound_gpu_freq_idx(&ui32UpboundGpuFreqLevel))
		{
			ui32UpboundGpuFreqLevel = 0;
			seq_printf(psSeqFile, "call mtk_get_custom_upbound_gpu_freq_idx false\n");
		}
		else
...
//-----------------------------------------------------------------------------
static struct seq_operations gsCustomUpboundGpuFreqIdxReadOps = 
{
	.start = ged_custom_upbound_gpu_freq_idx_seq_start,
	.stop = ged_custom_upbound_gpu_freq_idx_seq_stop,
	.next = ged_custom_upbound_gpu_freq_idx_seq_next,
	.show = ged_custom_upbound_gpu_freq_idx_seq_show,
};
//-----------------------------------------------------------------------------
static ssize_t ged_custom_lowbound_gpu_freq_write_entry(const char __user *pszBuffer, size_t uiCount, loff_t uiPosition, void *pvData)
{
#define GED_HAL_DEBUGFS_SIZE 64
	char acBuffer[GED_HAL_DEBUGFS_SIZE];
	...
	if ((0 < uiCount) && (uiCount < GED_HAL_DEBUGFS_SIZE))
	{
		...
		{
			acBuffer[uiCount] = '\0';
			if (sscanf(acBuffer, "%lu", &i32Value) == 1)
			{
				if (i32Value < 0)
					i32Value = 0;
				mtk_custom_lowbound_gpu_freq(i32Value);
            }
			//else if (...) //for other commands
			//{
			//}
...
//-----------------------------------------------------------------------------
static void* ged_custom_lowbound_gpu_freq_seq_start(struct seq_file *psSeqFile, loff_t *puiPosition)
{
	if (0 == *puiPosition)
	{
		return SEQ_START_TOKEN;
...
//-----------------------------------------------------------------------------
static void ged_custom_lowbound_gpu_freq_seq_stop(struct seq_file *psSeqFile, void *pvData)
{

}
//-----------------------------------------------------------------------------
static void* ged_custom_lowbound_gpu_freq_seq_next(struct seq_file *psSeqFile, void *pvData, loff_t *puiPosition)
{
	return NULL;
}
//-----------------------------------------------------------------------------
static int ged_custom_lowbound_gpu_freq_seq_show(struct seq_file *psSeqFile, void *pvData)
{
	if (pvData != NULL)
	{
		unsigned long ui32LowboundGpuFreq;
		if (false == mtk_get_custom_lowbound_gpu_freq(&ui32LowboundGpuFreq))
		{
			ui32LowboundGpuFreq = 0;
			seq_printf(psSeqFile, "call mtk_get_custom_lowbound_gpu_freq false\n");
		}
		else
			seq_printf(psSeqFile, "%lu\n", ui32LowboundGpuFreq);
	}

	return 0;
}
//-----------------------------------------------------------------------------
static struct seq_operations gsCustomLowboundGpuFreqReadOps = 
{
	.start = ged_custom_lowbound_gpu_freq_seq_start,
	.stop = ged_custom_lowbound_gpu_freq_seq_stop,
	.next = ged_custom_lowbound_gpu_freq_seq_next,
	.show = ged_custom_lowbound_gpu_freq_seq_show,
};
//-----------------------------------------------------------------------------
...
static int ged_custom_upbound_gpu_freq_seq_show(struct seq_file *psSeqFile, void *pvData)
{
	if (pvData != NULL)
	{
		unsigned long ui32UpboundGpuFreq;
		if (false == mtk_get_custom_upbound_gpu_freq(&ui32UpboundGpuFreq))
		{
			ui32UpboundGpuFreq = 0;
...
static void* ged_dvfs_status_seq_start(struct seq_file *psSeqFile, loff_t *puiPosition)
{
	if (0 == *puiPosition)
	{
		return SEQ_START_TOKEN;
...
//-----------------------------------------------------------------------------
static void ged_dvfs_status_seq_stop(struct seq_file *psSeqFile, void *pvData)
{

}
//-----------------------------------------------------------------------------
static void* ged_dvfs_status_seq_next(struct seq_file *psSeqFile, void *pvData, loff_t *puiPosition)
{
	return NULL;
}
//-----------------------------------------------------------------------------

static int ged_dvfs_status_seq_show(struct seq_file *psSeqFile, void *pvData)
{
	unsigned int loading = 0;
	unsigned int power = 0;
	mtk_get_gpu_loading(&loading);

	if(ged_dvfs_query_power_status())
		power = 1;
	else
		power = 0;
	...
	if (pvData != NULL)
	{
		...
		seq_printf(psSeqFile, "Power(0:Off, 1:On):%u\tLoading:%u\tFrequency:%lu\n", power, loading, sFreqInfo.ulFreq);
	}

	return 0;
}
//-----------------------------------------------------------------------------
static struct seq_operations gsDvfs_status_ReadOps = 
{
	.start = ged_dvfs_status_seq_start,
	.stop = ged_dvfs_status_seq_stop,
	.next = ged_dvfs_status_seq_next,
	.show = ged_dvfs_status_seq_show,
};
//-----------------------------------------------------------------------------

static void* ged_dvfs_cur_freq_value_seq_start(struct seq_file *psSeqFile, loff_t *puiPosition)
{
	if (0 == *puiPosition)
	{
		return SEQ_START_TOKEN;
...
//-----------------------------------------------------------------------------
static void ged_dvfs_cur_freq_value_seq_stop(struct seq_file *psSeqFile, void *pvData)
{

}
//-----------------------------------------------------------------------------
static void* ged_dvfs_cur_freq_value_seq_next(struct seq_file *psSeqFile, void *pvData, loff_t *puiPosition)
{
	return NULL;
}
//-----------------------------------------------------------------------------

static int ged_dvfs_cur_freq_value_seq_show(struct seq_file *psSeqFile, void *pvData)
{
	if (pvData != NULL)
	{
		GED_DVFS_FREQ_DATA sFreqInfo;
...
//-----------------------------------------------------------------------------
static struct seq_operations gsDvfs_cur_freq_value_ReadOps = 
{
	.start = ged_dvfs_cur_freq_value_seq_start,
	.stop = ged_dvfs_cur_freq_value_seq_stop,
	.next = ged_dvfs_cur_freq_value_seq_next,
	.show = ged_dvfs_cur_freq_value_seq_show,
};
//-----------------------------------------------------------------------------
...
GED_ERROR ged_hal_init(void)
{
	...
	err = ged_debugFS_create_entry(
			"custom_lowbound_gpu_freq_idx",
			gpsHALDir,
			&gsCustomLowboundGpuFreqIdxReadOps,
			ged_custom_lowbound_gpu_freq_idx_write_entry,
			NULL,
			&gpsCustomLowboundGPUFreqIdxEntry);

	if (unlikely(err != GED_OK))
	{
		GED_LOGE("ged: failed to create custom_lowbound_gpu_freq_idx entry!\n");
		goto ERROR;
	}

	/* Control the gpu freq */
	err = ged_debugFS_create_entry(
			"custom_upbound_gpu_freq_idx",
			gpsHALDir,
			&gsCustomUpboundGpuFreqIdxReadOps,
			ged_custom_upbound_gpu_freq_idx_write_entry,
			NULL,
			&gpsCustomUpboundGPUFreqIdxEntry);

	if (unlikely(err != GED_OK))
	{
		GED_LOGE("ged: failed to create custom_upbound_gpu_freq_idx entry!\n");
		goto ERROR;
	}

	/* Control the gpu freq by value*/
	err = ged_debugFS_create_entry(
			"custom_lowbound_gpu_freq",
			gpsHALDir,
			&gsCustomLowboundGpuFreqReadOps,
			ged_custom_lowbound_gpu_freq_write_entry,
			NULL,
			&gpsCustomLowboundGPUFreqEntry);

	if (unlikely(err != GED_OK))
	{
		GED_LOGE("ged: failed to create custom_lowbound_gpu_freq entry!\n");
		goto ERROR;
	}
	...
	/* Get current GPU DVFS Status*/	

	err = ged_debugFS_create_entry(
			"status",
			gpsHALDir,
			&gsDvfs_status_ReadOps, 
			NULL, 
			NULL,
			&gpsDvfsStatusEntry);

	/* Get current GPU freq value only*/

	err = ged_debugFS_create_entry(
			"frequency",
			gpsHALDir,
			&gsDvfs_cur_freq_value_ReadOps, 
			NULL, 
			NULL,
			&gpsDvfsCurFreqValueEntry);
...
void ged_hal_exit(void)
{
	...
	ged_debugFS_remove_entry(gpsCustomUpboundGPUFreqIdxEntry);
	ged_debugFS_remove_entry(gpsCustomLowboundGPUFreqIdxEntry);
	ged_debugFS_remove_entry(gpsCustomUpboundGPUFreqEntry);
	ged_debugFS_remove_entry(gpsCustomLowboundGPUFreqEntry);
	...
	ged_debugFS_remove_entry(gpsDvfsCurFreqValueEntry);
	ged_debugFS_remove_entry_dir(gpsDvfsStatusEntry);
