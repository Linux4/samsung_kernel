...
int ccmni_debug_file_init(int md_id)
{
	...
	if (!dentry1) {
		pr_err("[ccci/net] create /proc/ccmni fail %d\n", md_id);
		return -ENOENT;
	}
	...
	if (!dentry2) {
		pr_err("[ccci/net] create /proc/ccmni/md%d fail\n", (md_id+1));
		return -ENOENT;
	}
	...
	if (IS_ERR(dentry3) && result != -ENODEV) {
		pr_err("[ccci/net] create /proc/ccmni/md%d/debug_level fail: %d\n", md_id, result);
		return -ENOENT;
	}
...
static int ccmni_open(struct net_device *dev)
{
	...
	if (unlikely(ccmni_ctl == NULL)) {
		pr_err("[ccci/net] %s_Open: MD%d ctlb is NULL\n", dev->name, ccmni->md_id);
		return -1;
	}
	...
	if (ret) {
		pr_err("[ccci/net] md%d get ccmni%d channel fail\n", ccmni->md_id, ccmni->index);
		return ret;
	}
	/*******************************************************/
...
static int ccmni_close(struct net_device *dev)
{
	...
	if (unlikely(ccmni_ctl == NULL)) {
		pr_err("[ccci/net] md%d %s_Close: MD%d ctlb is NULL\n", ccmni->md_id, dev->name, ccmni->md_id);
		return -1;
	}
...
static int ccmni_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (unlikely(skb->len > dev->mtu)) {
		pr_err("[ccci/net] md%d CCMNI%d write fail: len(0x%x)>MTU(0x%x, 0x%x)\n",
			ccmni->md_id, ccmni->index, skb->len, CCMNI_MTU, dev->mtu);
		dev_kfree_skb(skb);
	...
	if (unlikely(skb_headroom(skb) < sizeof(struct ccci_header))) {
		pr_err("[ccci/net] md%d CCMNI%d write fail: header room(%d) < ccci_header(%d)\n",
			ccmni->md_id, ccmni->index, skb_headroom(skb), dev->hard_header_len);
		dev_kfree_skb(skb);
		dev->stats.tx_dropped++;
		return NETDEV_TX_OK;
	}
	...
	if (ret == CCMNI_ERR_MD_NO_READY || ret == CCMNI_ERR_TX_INVAL) {
		...
		pr_err("[ccci/net] md%d [TX]CCMNI%d send tx_pkt=%ld(ack=%d) fail: %d\n",
			ccmni->md_id, ccmni->index, (dev->stats.tx_packets+1), is_ack, ret);
		return NETDEV_TX_OK;
	...
	if (ccmni->tx_busy_cnt[is_ack] > 10) {
		pr_err("[ccci/net] md%d [TX]CCMNI%d TX busy: tx_pkt=%ld(ack=%d) retry %ld times done\n",
			ccmni->md_id, ccmni->index, dev->stats.tx_packets, is_ack, ccmni->tx_busy_cnt[is_ack]);
	}
	ccmni->tx_busy_cnt[is_ack] = 0;
...
tx_busy:
	if (unlikely(!(ctlb->ccci_ops->md_ability & MODEM_CAP_TXBUSY_STOP))) {
		if ((ccmni->tx_busy_cnt[is_ack]++)%100 == 0)
			pr_err("[ccci/net] md%d [TX]CCMNI%d TX busy: tx_pkt=%ld(ack=%d) retry_times=%ld\n",
				ccmni->md_id, ccmni->index, dev->stats.tx_packets, is_ack, ccmni->tx_busy_cnt[is_ack]);
	} else {
...
static int ccmni_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCCCMNICFG:
		...
		if (md_id_irat < 0 && md_id_irat >= MAX_MD_NUM) {
			pr_err("[ccci/net] md%d SIOCSCCMNICFG: %s invalid md_id(%d)\n",
				md_id, dev->name, (ifr->ifr_ifru.ifru_ivalue+1));
			return -EINVAL;
		}
		...
		if (ccmni->index >= ctlb_irat->ccci_ops->ccmni_num) {
			pr_err("[ccci/net] md%d SIOCSCCMNICFG: %s iRAT(MD%d->MD%d) fail,index(%d)>max_num(%d)\n",
				md_id, dev->name, md_id, md_id_irat, ccmni->index, ctlb_irat->ccci_ops->ccmni_num);
			break;
		}
	...
	default:
		pr_err("[ccci/net] md%d %s: unknown ioctl cmd=%x\n", ccmni->md_id, dev->name, cmd);
		break;
	}
...
static void ccmni_napi_poll_timeout(unsigned long data)
{
	...
	pr_err("[ccci/net] md%d CCMNI%d lost NAPI polling\n", ccmni->md_id, ccmni->index);
}
...
static int ccmni_init(int md_id, ccmni_ccci_ops_t *ccci_info)
{
	...
	if (unlikely(ccci_info->md_ability & MODEM_CAP_CCMNI_DISABLE)) {
		pr_err("md%d no need init ccmni: md_ability=0x%08X\n", md_id, ccci_info->md_ability);
		return 0;
	}

	ctlb = kzalloc(sizeof(ccmni_ctl_block_t), GFP_KERNEL);
	if (unlikely(ctlb == NULL)) {
		pr_err("md%d alloc ccmni ctl struct fail\n", md_id);
		return -ENOMEM;
	}

	ctlb->ccci_ops = kzalloc(sizeof(ccmni_ccci_ops_t), GFP_KERNEL);
	if (unlikely(ctlb->ccci_ops == NULL)) {
		pr_err("md%d alloc ccmni_ccci_ops struct fail\n", md_id);
		ret = -ENOMEM;
		goto alloc_mem_fail;
	}
	...
	ccmni_debug_file_init(md_id);
		...
		for (i = 0; i < ctlb->ccci_ops->ccmni_num; i++) {
			...
			if (ret) {
				pr_err("md%d initial ccmni instance fail\n", md_id);
				goto alloc_netdev_fail;
			}
			...
			if (ret) {
				pr_err("md%d CCMNI%d register netdev fail: %d\n", md_id, i, ret);
				goto alloc_netdev_fail;
			}
	...
	if ((ctlb->ccci_ops->md_ability & MODEM_CAP_CCMNI_IRAT) != 0) {
		if (ctlb->ccci_ops->irat_md_id < 0 || ctlb->ccci_ops->irat_md_id >= MAX_MD_NUM) {
			pr_err("md%d IRAT fail because invalid irat md(%d)\n",
		...
		if (!ctlb_irat_src) {
			pr_err("md%d IRAT fail because irat md%d ctlb is NULL\n",
				      md_id, ctlb->ccci_ops->irat_md_id);
			ret = -EINVAL;
			goto alloc_mem_fail;
		}
		...
		if (unlikely(ctlb->ccci_ops->ccmni_num > ctlb_irat_src->ccci_ops->ccmni_num)) {
			pr_err("md%d IRAT fail because number of src(%d) and dest(%d) ccmni isn't equal\n",
				      md_id, ctlb_irat_src->ccci_ops->ccmni_num, ctlb->ccci_ops->ccmni_num);
			ret = -EINVAL;
			goto alloc_mem_fail;
		}
		...
		for (i = 0; i < ctlb->ccci_ops->ccmni_num; i++) {
			...
			if (unlikely(ccmni_irat_src == NULL)) {
				pr_err("md%d alloc ccmni_irat instance fail\n", md_id);
				kfree(ccmni);
				ret = -ENOMEM;
				goto alloc_mem_fail;
			}
...
static int ccmni_rx_callback(int md_id, int ccmni_idx, struct sk_buff *skb, void *priv_data)
{
	...
	if (unlikely(ctlb == NULL || ctlb->ccci_ops == NULL)) {
		pr_err("md%d invalid CCMNI%d ctrl/ops struct\n", md_id, ccmni_idx);
		dev_kfree_skb(skb);
		return -1;
	}
...
static void ccmni_md_state_callback(int md_id, int ccmni_idx, MD_STATE state, int is_ack)
{
	...
	if (unlikely(ctlb == NULL)) {
		pr_err("md%d invalid ccmni ctrl struct when ccmni_idx=%d md_sta=%d\n", md_id, ccmni_idx, state);
		return;
	}
