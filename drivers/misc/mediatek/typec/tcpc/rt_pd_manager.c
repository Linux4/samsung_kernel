...
static void set_dynamic_mivr(struct rt_pd_manager_data *rpmd)
{
	...
	ret = charger_dev_get_mivr(rpmd->chg_dev, &mivr);
	if (ret < 0) {
		pr_notice("%s: failed, ret = %d\n", __func__, ret);
		return;
	}
	if (mivr > rpmd->sink_mv_new * 1000) {
		dev_info(rpmd->dev, "%s original mivr = %d\n", __func__, mivr);
		if (mivr <= V_CHARGER_MIN_2)
			charger_dev_set_mivr(rpmd->chg_dev, V_CHARGER_MIN_2);
		else if (mivr <= V_CHARGER_MIN_1)
			charger_dev_set_mivr(rpmd->chg_dev, V_CHARGER_MIN_1);
		else
...
/*A06 code for SR-AL7160A-01-265 by shixuanxuan at 20240407 start*/
	...
	struct power_supply *cp_psy = power_supply_get_by_name("cpm-standalone");
	union power_supply_propval otg_val = {0,};
#endif
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
			...
			if (IS_ERR_OR_NULL(cp_psy)) {
				dev_err(rpmd->dev,"%s TCPC OTG get cp_psy fail !\n", __func__);
			} else {
				otg_val.intval = 1;
				power_supply_set_property(cp_psy,
                        POWER_SUPPLY_PROP_CP_OTG_ENABLE, &otg_val);
			}
			#endif
			/*A06 code for SR-AL7160A-01-265 by shixuanxuan at 20240407 end*/
