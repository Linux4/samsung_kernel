...
/*HS03s for SR-AL5625-01-515 by wangzikang at 20210607 start*/
struct mutex  wusb_mutex;
/*HS03s for SR-AL5625-01-515 by wangzikang at 20210607 end*/
const char const_id_table[] = "023BSUT";
...
static int sgm7220_read_reg(struct i2c_client *i2c, u8 reg, u8 *dest)
{
	...
	int ret;
	pr_err("sgm7220 enter %s\n", __func__);
	/*HS03s for SR-AL5625-01-515 by wangzikang at 20210607 start*/
	// mutex_lock(&info->mutex);
	mutex_lock(&wusb_mutex);
	ret = i2c_smbus_read_byte_data(i2c, reg);
	// mutex_unlock(&info->mutex);
	mutex_unlock(&wusb_mutex);
	/*HS03s for SR-AL5625-01-515 by wangzikang at 20210607 end*/
	if (ret < 0) {
...
static int sgm7220_update_reg(struct i2c_client *i2c, u8 reg, u8 val, u8 mask)
{
	...
	if (ret >= 0) {
		pr_info("sgm7220 update_reg get REG%d = [0x%x]\n",reg,ret);
		old_val = ret & 0xff;
		new_val = (val & mask) | (old_val & (~mask));
		pr_info("sgm7220 update_reg set REG%d = [0x%x]\n",reg,new_val);
...
/*HS03s for DEVAL5625-1479 by wangzikang at 20210617 start*/
static void sgm7220_dump_reg(struct i2c_client *i2c)
{
	u8 value = 0;
	int i = 0;
	for (i=8;i<11;i++) {
		sgm7220_read_reg(i2c, i, &value);
		pr_err("%s:REG0x%2x = [0x%x]\n",__func__, i, value);
	}
	sgm7220_read_reg(i2c, REG_CTL, &value);
	pr_err("%s:REG45 = [0x%x]\n",__func__, value);

	sgm7220_read_reg(i2c, REG_REVISION, &value);
	pr_err("%s:REGA0 = [0x%x]\n",__func__, value);
}
/*HS03s for DEVAL5625-1479 by wangzikang at 20210617 end*/
...
static void process_mode_register(struct sgm7220_chip *info)
{
	...
	pr_info("sgm7220 %s mode_register:0x%x\n", __func__, reg_val);
	tmp = reg_val;
...
static void process_interrupt_register(struct sgm7220_chip *info)
{
	...
	pr_info("sgm7220 %s interrupt_register:0x%x\n", __func__, reg_val);
	tmp = reg_val;
	...
	if (info->type_c_param.attach_state != 0) {
		typec_cc_orientation = val+1;
	} else {
		typec_cc_orientation = 0;
	}
	pr_info("state=%d,typec_cc_orientation=%d\n",info->type_c_param.attach_state, typec_cc_orientation);
	/*HS03s for DEVAL5625-1943 by wangzikang at 20210629 end*/
	/*HS03s for SR-AL5625-01-515 by wangzikang at 20210607 end*/
}
/*HS03s for DEVAL5625-1943 by wangzikang at 20210629 start*/
static void sgm7220_process_set_register(struct sgm7220_chip *info)
{
	u8 reg_val;
	int ret;
	ret = sgm7220_read_reg(info->client, REG_SET, &reg_val);
	if (ret < 0) {
		pr_err("%s err\n", __func__);
		return;
	}
	pr_info("sgm7220 set_register=[0x%x]\n",reg_val);
}
/*HS03s for DEVAL5625-1943 by wangzikang at 20210629 end*/
...
static void sgm7220_irq_work_handler(struct kthread_work *work)
{
	...
	if(chip->type_c_param.attach_state==1){
		pr_info("%s TYPEC_ATTACHED_SRC(DFP) \n", __func__);
	...
	}else if(chip->type_c_param.attach_state==2){
		pr_info("%s TYPEC_ATTACHED_SNK(UFP) \n", __func__);
	...
	}else if(chip->type_c_param.attach_state==0){
		pr_info("%s TYPEC_UNATTACHED \n", __func__);
		...
		pr_err("%s: Unknwon type Attached to an accessory\n", __func__);
	}
...
static int sgm7220_parse_dt(struct sgm7220_chip *chip, struct device *dev)
{
	...
	if (ret < 0)
		pr_err("%s no mt6370pd,intr_gpio_num info\n", __func__);
#endif
	return ret;
}
...
static int sgm7220_tcpcdev_init(struct sgm7220_chip *chip, struct device *dev)
{
	...
	pr_info("sgm7220 %s [mt-tcpc,name]:%s\n", __func__,name);
	len = strlen(name);
...
static int sgm7220_check_revision(struct i2c_client *client)
{
	...
	ret = sgm7220_read_reg(client, REG_REVISION, &reg_val);
	if (ret < 0) {
		return ret;
	}
	pr_info("sgm7220 REG_REVISION = [0x%x] \n", reg_val);

	for (count = 0; count < 8; count++) {
		ret = sgm7220_read_reg(client, REG_ID_BASE+count, &id_table[count]);
		pr_info("sgm7220 REG_ID_BASE%d = [0x%x][%c] \n",count, id_table[count],id_table[count]);
		if ((ret < 0) || (id_table[count]!=const_id_table[count])) {
...
struct device_attribute dev_attr_typec_cc_orientation_sgm = __ATTR(typec_cc_orientation, S_IRUGO, typec_cc_orientation_show, NULL);
/*HS03s for SR-AL5625-01-515 by wangzikang at 20210607 end*/
				...
				const struct i2c_device_id *id)
{
	...
	/*HS03s for SR-AL5625-01-515 by wangzikang at 20210607 start*/
	ret = device_create_file(&client->dev, &dev_attr_typec_cc_orientation_sgm);
	if (ret < 0) {
		dev_err(&client->dev, "failed to create dev_attr_typec_cc_orientation_sgm\n");
		ret = -ENODEV;
...
/*HS03s for SR-AL5625-01-515 by wangzikang at 20210607 start*/
err_create_file:
	device_remove_file(&client->dev, &dev_attr_typec_cc_orientation_sgm);
/*HS03s for SR-AL5625-01-515 by wangzikang at 20210607 end*/
