...
void pd_dpm_send_source_caps_switch(int cur)
{
	...
	/* HS14_U/TabA7 Lite U/A06 for P240515-04492 by liufurong at 20240516 start */
	if (tcpc == NULL) {
	...
	if (tcpc->pd_port.pe_pd_state != PE_SRC_READY) {
		printk("tcpc->pd_port->pe_pd_state = %d, sikp it\n", tcpc->pd_port.pe_pd_state);
		return;
	} else {
		printk("tcpc->pd_port->pe_pd_state = %d\n", tcpc->pd_port.pe_pd_state);
	}
	/* HS14_U/TabA7 Lite U/A06 for P240515-04492 by liufurong at 20240516 end */
...
int pd_dpm_notify_pe_startup(struct pd_port *pd_port)
{
	...
	/*A06 code for AL7160A-1630 by shixuanxuan at 20240626 start*/
	if (is_upm6922P_tcpc != 1) {
		reactions |= DPM_REACTION_ATTEMPT_GET_FLAG |
			DPM_REACTION_REQUEST_PR_SWAP;
	}
	/*A06 code for AL7160A-1630 by shixuanxuan at 20240626 end*/
#else
