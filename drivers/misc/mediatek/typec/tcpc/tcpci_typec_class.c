...
static int tcpc_typec_class_port_type_set(const struct typec_capability *cap, enum typec_port_type port_type)
{
	...
	if (port_type == TYPEC_PORT_DFP) {
		tcpc->typec_try_state_change = TYPEC_ATTACH_DFP;
		pr_info("%s typec role swap from UFP(Sink) to DFP(Source)\n", __func__);
	} else if (port_type == TYPEC_PORT_UFP) {
		tcpc->typec_try_state_change = TYPEC_ATTACH_UFP;
		pr_info("%s typec role swap from DFP(Source) to UFP(Sink)\n", __func__);
	}
#endif
