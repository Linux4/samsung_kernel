...
void pd_dpm_send_source_caps_switch(int cur)
{
	...
	/* HS14_U/TabA7 Lite U for P240515-04492 by liufurong at 20240516 start */
	if (tcpc == NULL) {
	...
	if (tcpc->pd_port.pe_pd_state != PE_SRC_READY) {
		printk("tcpc->pd_port->pe_pd_state = %d, skip it\n", tcpc->pd_port.pe_pd_state);
		return;
	} else {
		printk("tcpc->pd_port->pe_pd_state = %d\n", tcpc->pd_port.pe_pd_state);
	}
	/* HS14_U/TabA7 Lite U for P240515-04492 by liufurong at 20240516 end */
