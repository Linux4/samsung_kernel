...
void StateMachineUnattached(struct Port *port)
{
	...
	// struct power_supply *ext_psy = power_supply_get_by_name("customer"); //zjb
	...
	if (port->Registers.Status.I_TOGDONE)
	{
		...
		// power_supply_set_property(ext_psy,POWER_SUPPLY_PROP_TYPEC_CC_ORIENTATION,&reta); //zjb
...
void SetStateUnattached(struct Port *port)
{
	...
	typec_rotation = 0;
	// typec_mode = POWER_SUPPLY_TYPEC_NONE; //zjb
...
{
	// struct power_supply *ext_psy = power_supply_get_by_name("customer"); //zjb
	// union power_supply_propval reta = {0,}; //zjb
	...
	DeviceWrite(port->I2cAddr, regMask, 1, &port->Registers.Mask.byte);
	TimerDisable(&port->StateTimer);
	notify_observers(CC1_ORIENT, port->I2cAddr, 0);

	// typec_mode = POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER;		//zjb
	// reta.intval = typec_mode;
	// power_supply_set_property(ext_psy,POWER_SUPPLY_PROP_TYPEC_MODE,&reta); //zjb
