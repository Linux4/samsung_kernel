...
//int tcpci_report_usb_port_detached(struct tcpc_device *tcpc);lsm
#ifdef __TEST_CC_PATCH__
  ...
  struct  wusb3801_chip *chip, u8 reg, const u8 data)
{
	int ret = 0,retry = 5;
	do {
		ret = wusb3801_reg_write(chip->client, reg, data);
		retry--;
	} while(ret < 0 && retry > 0);
	if (ret < 0 || retry < 0) {
...
static void wusb3801_irq_work_handler(struct kthread_work *work)
{
	...
	pr_info("%s first_check_flag %d \r\n",__func__,first_check_flag);
	...
	pr_info("%s sts[0x%02x], type[0x%02x]\n",__func__,status, type);
	...
	case WUSB3801_TYPE_SNK:
		...
		if (tcpc->typec_attach_new != TYPEC_ATTACHED_SRC) {
				...
				//tcpci_report_usb_port_attached(chip->tcpc);lsm
...
#define WUSB3801_CHIP_VENDOR_ID 	0x06
#define WUSB3801_CHIP_VERSION_ID 	0x02
...
static inline int wusb3801_check_revision(struct i2c_client *client)
{
	...
	if (((rc & WUSB3801_VENDOR_ID_MASK) != WUSB3801_CHIP_VENDOR_ID) || (((rc & WUSB3801_VERSION_ID_MASK) >> 3) != WUSB3801_CHIP_VERSION_ID)){
		pr_info("wusb3801 Check ID failed!");
		return -EINVAL;
	}
