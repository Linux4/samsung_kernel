...
int config_mau(M4U_MAU_STRUCT mau)
{
	...
	larb_port = m4u_port_2_larb_port(mau.port);
	m4u_slave_id = larb_2_m4u_slave_id(larb);
	if (larb_port == M4U_PORT_UNKNOWN || m4u_slave_id == -1)
		return -1;
	mau_start_monitor(m4u_id, m4u_slave_id, free_id, (int)mau.write,
...
static int _m4u_config_port(int port, int virt, int sec, int dis, int dir)
{
	...
	M4ULOG_HIGH("config_port:%s,v%d,s%d, index:%d\n",
	m4u_get_port_name(port), virt, sec, m4u_index);
...
int m4u_monitor_start(int m4u_id)
{
	...
	M4UINFO("====m4u_monitor_start: %d,base:0x%lx======\n", m4u_id, m4u_base);
	if (m4u_base == 0)
		return -1;
...
int m4u_monitor_stop(int m4u_id)
{
	...
	M4UINFO("====m4u_monitor_stop: %d,base:0x%lx======\n", m4u_id, m4u_base);
	if (m4u_base == 0)
		return -1;
