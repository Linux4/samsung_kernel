...
/* FIXME: workaround for C10 bringup */
#if defined(CONFIG_MUIC_S2MU005) || defined(CONFIG_DUMMY_MUIC)
...
static int rtc_ops_set_bootalarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	rtc_xinfo("set boot al time = %04d/%02d/%02d %02d:%02d:%02d (%d)\n",
		  tm.tm_year + RTC_MIN_YEAR, tm.tm_mon, tm.tm_mday,
		  tm.tm_hour, tm.tm_min, tm.tm_sec, alm->enabled);
...
static int rtc_ops_get_bootalarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	/* read rtc time */
	if ( rtc_ops_read_time(dev, &b) ) {
		pr_err("%s [bootalarm] : read time failed.\n", __func__);
		ret = -EINVAL;
	}
	...
	if(alm->enabled ==3 && bootalarm_irq)//boot alarm with logo
	{
...
static int rtc_ops_set_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	unsigned long now_time, pwron_time;
...
#ifdef CONFIG_RTC_AUTO_PWRON
		spin_lock(&rtc_lock);
		pwron_alm = hal_rtc_is_pwron_alarm(&nowtm, &pwrontm);
		if (pwron_alm) {
			...
			if(now_time < pwron_time && pwron_time < time )
			{
				memcpy(&tm, &pwrontm, sizeof(struct rtc_time));
			  	...
			  	rtc_xinfo("override by pwron alarm = %04d/%02d/%02d %02d:%02d:%02d (%d)\n",
			  	tm.tm_year + RTC_MIN_YEAR, tm.tm_mon, tm.tm_mday,
			  	tm.tm_hour, tm.tm_min, tm.tm_sec, alm->enabled);
			}
		}
