...
static int normal_rx_queue_buffer_size[CLDMA_RXQ_NUM] = { SKB_4K, SKB_4K, SKB_4K, SKB_4K, 0, 0, SKB_4K, SKB_4K };
...
#endif
static int normal_rx_queue_buffer_number[CLDMA_RXQ_NUM] = { 16, 64, 16, 16, 0, 0, 16, 16 };
static int normal_tx_queue_buffer_number[CLDMA_TXQ_NUM] = { 16, 16, 16, 16, 0, 0, 16, 16 };
...
#define IS_VT_QUE(md, qno) \
	((md->is_in_ee_dump == 0) && (qno == 7))
...
static int cldma_gpd_rx_collect(struct md_cd_queue *queue, int budget, int blocking)
{
	...
	if (queue->index == 1) {
		...
		if (gpd_usage >= 16) {
			CCCI_NORMAL_LOG(md->index, TAG, "rxq%d work start, usage (%d/%d)\n",
				queue->index, gpd_usage, queue->budget);
		}
	}
#endif
again:
	while (1) {
		...
		if (req->data_buffer_ptr_saved != 0) {
			/* update skb */
			dma_unmap_single(&md->plat_dev->dev,
				req->data_buffer_ptr_saved, skb_data_size(skb), DMA_FROM_DEVICE);
...
#endif
#ifdef CONFIG_MTK_SRIL_SUPPORT
		ccci_h_ptr = (struct ccci_header *)skb->data;
		if (ccci_h_ptr->channel == CCCI_RIL_IPC0_RX || ccci_h_ptr->channel == CCCI_RIL_IPC1_RX) {
			ccci_h_ptr->reserved = (unsigned int)(sched_clock() / 1000000);
		...
		if (new_skb) {
			...
			if (dma_mapping_error(&md->plat_dev->dev, req->data_buffer_ptr_saved)) {
				CCCI_ERROR_LOG(-1, TAG, "error dma mapping\n");
				return -1;
			}
			...
			if (queue->index == 1 && md->md_state == READY) {
				CCCI_NORMAL_LOG(md->index, TAG, "rxq%d leave skb %p in ring, ret = 0x%x\n",
						queue->index, skb, ret);
			} else {
...
static int cldma_gpd_tx_collect(struct md_cd_queue *queue, int budget, int blocking)
{
	...
	while (1) {
		...
		ccci_h = (struct ccci_header *)skb_free->data;
#ifdef CONFIG_MTK_SRIL_SUPPORT
		print_ipc_pkt(ccci_h->channel, skb_free);
#endif
		/* check wakeup source */
		if (atomic_cmpxchg(&md->wakeup_src, 1, 0) == 1)
			CCCI_NOTICE_LOG(md->index, TAG, "CLDMA_AP wakeup source:(%d/%d)\n",
...
static void cldma_rx_ring_init(struct ccci_modem *md, struct cldma_ring *ring)
{
	...
	if (ring->type == RING_GPD) {
		for (i = 0; i < ring->length; i++) {
			...
			if (dma_mapping_error(&md->plat_dev->dev, item->data_buffer_ptr_saved)) {
				CCCI_ERROR_LOG(-1, TAG, "error dma mapping\n");
				return;
			}
...
static inline void cldma_stop_for_ee(struct ccci_modem *md)
{
	...
	int i;
	int gpd_usage;
	unsigned long long ts_isr = 0, ts_wq = 0;
	unsigned long nsec_isr = 0, nsec_wq = 0;
	struct cldma_request *req = NULL;
	struct cldma_rgpd *rgpd = NULL;
	char *work_state = NULL;

	ts_isr = md->latest_isr_time;
	nsec_isr = do_div(ts_isr, NSEC_PER_SEC);
	CCCI_NORMAL_LOG(md->index, TAG, "Dump CLDMA lastest isr: %llu.%06lu\n", ts_isr, nsec_isr / 1000);
	...
	for (i = 0; i < QUEUE_LEN(md_ctrl->rxq); i++) {
		...
		ts_isr = md->latest_q_rx_isr_time[i];
		nsec_isr = do_div(ts_isr, NSEC_PER_SEC);
		ts_wq = md->latest_q_rx_time[i];
		nsec_wq = do_div(ts_wq, NSEC_PER_SEC);
		
		work_state = "READY";
		if (work_busy(&md_ctrl->rxq[i].cldma_rx_work)) {
			if (work_pending(&md_ctrl->rxq[i].cldma_rx_work))
				work_state = "PENDING";
			else
				work_state = "RUNNING";
		}

		CCCI_NORMAL_LOG(md->index, TAG, "rxq%d ring buffer usage (%d/%d) / last irq: %llu.%06lu / last wq: %llu.%06lu / rx work state: %s\n",
				i, gpd_usage, md_ctrl->rxq[i].budget,
				ts_isr, nsec_isr / 1000, ts_wq, nsec_wq / 1000, work_state);
	}
#endif
	CCCI_NORMAL_LOG(md->index, TAG, "%s from %ps\n", __func__, __builtin_return_address(0));
	spin_lock_irqsave(&md_ctrl->cldma_timeout_lock, flags);
		...
		for (i = 0; i < QUEUE_LEN(md_ctrl->rxq); i++) {
			...
			list_for_each_entry(req, &md_ctrl->rxq[i].tr_ring->gpd_ring, entry) {
				...
				if (req->skb == NULL) {
					...
					if (req->skb == NULL) {
						CCCI_ERROR_LOG(md->index, TAG,
							"fail to allocate skb buff in clear all queue!!\n");
						return;
					}
...
/*L**W**L** merge f5703a4f360552f88eb30f7f7467c3e0dabc6733*/
	...
	if (*((int *)(md->smem_layout.ccci_exp_smem_base_vir + CCCI_SMEM_OFFSET_MDSS_DEBUG
		+ CCCI_SMEM_OFFSET_EPON_UMOLY)) == 0xBAEBAE10)/* hardcode */
#else
	if (*((int *)(md->mem_layout.smem_region_vir + CCCI_SMEM_OFFSET_MDSS_DEBUG
		+ CCCI_SMEM_OFFSET_EPON_UMOLY)) == 0xBAEBAE10)/* hardcode */
#endif
#else
...
int md_cd_op_is_dump_done_set(struct ccci_modem *md)
{
	if (*((int *)(md->mem_layout.smem_region_vir +
		CCCI_SMEM_OFFSET_MDSS_DEBUG + CCCI_SMEM_OFFSET_EPON_UMOLY)) == 0xE1DFE1DF)
		return 1;
	else
		return 0;
}
...
static irqreturn_t md_cd_ccif_isr(int irq, void *data)
{
	...
	md->ee_on_going_for_force_off = 1;
	/* must ack first, otherwise IRQ will rush in */
	md_ctrl->channel_id = cldma_read32(md_ctrl->ap_ccif_base, APCCIF_RCHNUM);
	CCCI_NORMAL_LOG(md->index, TAG, "MD CCIF IRQ 0x%X\n", md_ctrl->channel_id);
...
static int md_cd_start(struct ccci_modem *md)
{
	...
	if (md->config.setting & MD_SETTING_FIRST_BOOT) {
		...
		CCCI_BOOTUP_LOG(md->index, TAG, "Clear MD1 MD3 smem\n");
#endif
	...
	md->ee_on_going_for_force_off = 0;
...
static int md_cd_stop(struct ccci_modem *md, unsigned int stop_type)
{
	...
	unsigned int force_off = md_ctrl->modem->force_off_md;

	CCCI_NORMAL_LOG(md->index, TAG, "CLDMA modem is power off, stop_type=%d\n", stop_type);
	ccif_disable_irq(md);

	CCCI_INF_MSG(md->index, TAG, "check if EE on-going before force modem power off\n");
	while (md->ee_on_going_for_force_off) {
		CCCI_INF_MSG(md->index, TAG, "EE on-going\n");
		wait_ee = 1;
		msleep(1000); /* If run to this path, it means we need wait modem EE flow finish */
	}
	if (wait_ee)
		CCCI_INF_MSG(md->index, TAG, "EE finished for force modem power off\n");
	else
		CCCI_INF_MSG(md->index, TAG, "No EE on-going for force modem power off\n");
	...
	if (force_off) {
		for (i = 0; i < 10; i++) {
			if (spm_is_md1_sleep()) {
				CCCI_NORMAL_LOG(md->index, TAG, "ccci md has send infinite sleep signal\n");
				break;
			}
			msleep(100);
		}
		CCCI_NORMAL_LOG(md->index, TAG, "power off md mtcomos\n");
					  ...
					  struct sk_buff *skb, unsigned int ioc_override)
{
	...
	for (cur_frag = -1; cur_frag < info->nr_frags; cur_frag++) {
		...
		if (dma_mapping_error(&queue->modem->plat_dev->dev, tx_req_bd->data_buffer_ptr_saved)) {
			CCCI_ERROR_LOG(-1, TAG, "error dma mapping\n");
			return -1;
		}
				       ...
				       struct sk_buff *skb, unsigned int ioc_override)
{
	...
	if (dma_mapping_error(&queue->modem->plat_dev->dev, tx_req->data_buffer_ptr_saved)) {
		CCCI_ERROR_LOG(-1, TAG, "error dma mapping\n");
		return -1;
	}
...
static struct ccci_modem_ops md_cd_ops = {
	...
	.is_dump_done = &md_cd_op_is_dump_done_set,
};
