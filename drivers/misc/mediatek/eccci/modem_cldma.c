...
#endif
static int normal_rx_queue_buffer_number[CLDMA_RXQ_NUM] = { 16, 16, 16, 16, 0, 0, 16, 16 };
...
/* EE flag for force off modem */
volatile int ee_on_going_for_force_off;
...
#define IS_VT_QUE(md, qno) \
	((md->md_state != EXCEPTION || md->ex_stage != EX_INIT_DONE) && (qno == 7))
...
#ifdef CONFIG_MTK_SRIL_SUPPORT
		print_ipc_pkt(ccci_h.channel, skb);
#endif
...
static void trigger_md_reset_work(struct work_struct *work)
{
	struct md_cd_ctrl *md_ctrl = container_of(work, struct md_cd_ctrl, trig_md_reset_work);
	struct ccci_modem *md = md_ctrl->modem;
	int ret = 0;
...
void ccci_trigger_reset(void *ctl)
{
	struct ccci_modem *md = (struct ccci_modem *)ctl;
	struct md_cd_ctrl *md_ctrl = (struct md_cd_ctrl *)md->private_data;

	schedule_work(&md_ctrl->trig_md_reset_work);
}

static int wdt_executed;
...
static int md_cd_start(struct ccci_modem *md)
{
	...
	if (!modem_run_env_ready(md->index)) {
		...
		CCCI_NOTICE_MSG(md->index, TAG, "CLDMA modem image ready, bypass load\n");
...
static int md_cd_force_off(struct ccci_modem *md)
{
	int ret = 0;
	struct md_cd_ctrl *md_ctrl = (struct md_cd_ctrl *)md->private_data;
	int en_power_check;
	...
	while(ee_on_going_for_force_off) {
	...
	CCCI_INF_MSG(md->index, TAG, "force modem power off\n");
	md->sim_type = 0xEEEEEEEE;	/* reset sim_type(MCC/MNC) to 0xEEEEEEEE */
...
#endif
	/* power off MD */
	ret = md_cd_force_md_power_off(md);
	CCCI_INF_MSG(md->index, TAG, "CLDMA modem is power off done, %d\n", ret);
	md->ops->broadcast_state(md, GATED);
	...
	/* Make sure AP and MD CCIF no busy signal */
	/* Write MD ack to clear AP */
	rx_ch_bitmap = cldma_read32(md_ctrl->md_ccif_base, APCCIF_RCHNUM);
	if (rx_ch_bitmap) {
		CCCI_INF_MSG(md->index, TAG, "MD CCIF rx bitmap: 0x%x(F)\n", rx_ch_bitmap);
		for (i = 0; i < 16; i++) {
			/* Ack one by one */
			if (rx_ch_bitmap & (1<<i))
				cldma_write32(md_ctrl->md_ccif_base, APCCIF_ACK, (1<<i));
	...
	/* Write AP ack to clear MD */
	rx_ch_bitmap = cldma_read32(md_ctrl->ap_ccif_base, APCCIF_RCHNUM);
	if (rx_ch_bitmap) {
		CCCI_INF_MSG(md->index, TAG, "AP CCIF rx bitmap: 0x%x(F)\n", rx_ch_bitmap);
		for (i = 0; i < 16; i++) {
			/* Ack one by one */
			if (rx_ch_bitmap & (1<<i))
				cldma_write32(md_ctrl->ap_ccif_base, APCCIF_ACK, (1<<i));
		}
	}
	CCCI_INF_MSG(md->index, TAG, "CCIF AP rx bitmap: 0x%x; MD rx bitmap: 0x%x(after ack)(F)\n",
...
static void dump_runtime_data(struct ccci_modem *md, struct modem_runtime *runtime)
{
	...
	*p = runtime->misc_prefix;
	...
	CCCI_INF_MSG(md->index, TAG, "Feature8  0x%x  0x%x  0x%x  0x%x\n", runtime->feature_8_val[0],
		     runtime->feature_8_val[1], runtime->feature_8_val[2], runtime->feature_8_val[3]);
...
#endif
	/* MD sleep history log */
	runtime->support_mask |= (FEATURE_SUPPORT << (MISC_MD_SLPHISTLOG * 2));
	runtime->feature_8_val[0] = md->mem_layout.smem_region_phy - md->mem_layout.smem_offset_AP_to_MD
					+ CCCI_SMEM_OFFSET_MD_SLPLOG;
	runtime->feature_8_val[1] = CCCI_SMEM_MD_SLPLOG_SIZE;
	runtime->feature_8_val[2] = 0;
	runtime->feature_8_val[3] = 0;
	/* update guarding pattern, write pointer and control flag */
	mem_ptr = (volatile unsigned int *)(md->mem_layout.smem_region_vir + CCCI_SMEM_OFFSET_MD_SLPLOG);
	mem_ptr[0] = 0x12345678;
	mem_ptr[1] = 0;
	mem_ptr[2] = 0;
	update_md_slp_raw_ptr((void *)mem_ptr);
...
static int ccci_modem_probe(struct platform_device *plat_dev)
{
	...
	INIT_WORK(&md_ctrl->trig_md_reset_work, trigger_md_reset_work);
#if TRAFFIC_MONITOR_INTERVAL
