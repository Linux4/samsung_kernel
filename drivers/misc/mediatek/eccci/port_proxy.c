...
#endif
#ifdef CONFIG_MTK_ECCCI_C2K
	struct port_proxy *md3_power_off_proxy_p;
#endif
...
#ifdef CONFIG_MTK_SRIL_SUPPORT
		if (arg < MD_FORCE_ASSERT_BY_RIL_RSV_MAX - MD_FORCE_ASSERT_BY_RIL_USER_TRIGGER)
			ret = ccci_md_force_assert(proxy_p->md_obj, MD_FORCE_ASSERT_BY_RIL_USER_TRIGGER + arg, NULL, 0);
		else
			ret = ccci_md_force_assert(proxy_p->md_obj, MD_FORCE_ASSERT_BY_RIL_USER_TRIGGER, NULL, 0);
#else
	...
	case CCCI_IOC_FORCE_MD_OFF:
		{
			...
			CCCI_ERR_MSG(md_id, CHAR, "Force power off modem ioctl called by %s\n", current->comm);
			if (md_id == MD_SYS1) {
				#ifdef CONFIG_MTK_ECCCI_C2K
				/* firstly, power off MD3 */
				/* exec_ccci_kern_func_by_md_id(MD_SYS3, FORCE_MD3_OFF_MSG, NULL, 0); */
				md3_power_off_proxy_p = port_proxy_get_by_md_id(MD_SYS3);
				/* exec_ccci_kern_func_by_md_id(MD_SYS3, ID_ENTER_FLIGHT_MODE, NULL, 0); */
				ret = port_proxy_send_msg_to_user(md3_power_off_proxy_p,
			...
			#endif
			#ifdef CONFIG_MTK_ECCCI_C2K
			CCCI_ERROR_LOG(MD_SYS3, CHAR, "stop MD3 request in CCCI_IOC_FORCE_MD_OFF\n");
			/* firstly, power off MD3 */
			md3_power_off_proxy_p = port_proxy_get_by_md_id(MD_SYS3);
			ret = port_proxy_send_msg_to_user(md3_power_off_proxy_p,
					CCCI_MONITOR_CH, CCCI_MD_MSG_FLIGHT_STOP_REQUEST, 0);
			msleep(500);
			#endif
			/* secondly, power off MD1 */
			CCCI_ERROR_LOG(md_id, KERN,
				"md%d: CCCI_IOC_FORCE_MD_OFF stop MD request ioctl called by %s\n",
					md_id, current->comm);
			ret = port_proxy_send_msg_to_user(proxy_p, CCCI_MONITOR_CH, CCCI_MD_MSG_FORCE_STOP_REQUEST, 0);
		}
		break;
