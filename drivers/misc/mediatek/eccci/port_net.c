...
int ccci_get_ccmni_channel(int md_id, int ccmni_idx, struct ccmni_ch *channel)
{
	...
	if (port) {
		...
		CCCI_ERR_MSG(md_id, NET, "idx:%d, tx:%d,tx_ch:%d, dl_ack:%d,dl_ack_ch:%d\n",
...
int ccmni_send_pkt(int md_id, int ccmni_idx, void *data, int is_ack)
{
	...
	CCCI_ERROR_LOG(md_id, NET, "port %s,txq_exp_index:%d, txq_index:%d send: %08X, %08X, %08X, %08X\n",
		port->name, port->txq_exp_index, port->txq_index, ccci_h->data[0],
