...
static void sys_msg_handler(struct ccci_port *port, struct sk_buff *skb)
{
	...
	case CCISM_SHM_INIT_ACK:
		...
		break;
#endif
	case FORCE_MD_OFF_MSG:
		md = ccci_md_get_modem_by_id(md_id);
		if (md) {
			md->md_power_off_done = 1;
			CCCI_NORMAL_LOG(md_id, KERN, "Got Power off done, md->md_power_off_done %d\n",
				md->md_power_off_done);
		} else
			CCCI_NORMAL_LOG(md_id, KERN, "Got Power off done with invalid md id %d\n", md_id);
		break;
	case MD_TX_POWER:
		/* Fall through */
