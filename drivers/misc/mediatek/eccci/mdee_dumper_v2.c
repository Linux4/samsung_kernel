...
static void mdee_output_debug_info_to_buf(struct md_ee *mdee, DEBUG_INFO_T *debug_info, char *ex_info)
{
	...
	case MD_EX_DUMP_ASSERT:
		...
		snprintf(ex_info, EE_BUF_LEN_UMOLY,
				...
				debug_info->assert.parameters[1], debug_info->assert.parameters[2]);
#ifdef CONFIG_MTK_SRIL_SUPPORT
		error_code = debug_info->assert.parameters[0];
#endif
		break;
...
#ifdef CONFIG_MTK_SRIL_SUPPORT
		error_code = debug_info->fatal_error.err_code1;
#endif
		break;
...
#ifdef CONFIG_MTK_SRIL_SUPPORT
	if (error_code >= MD_FORCE_ASSERT_BY_RIL_USER_TRIGGER && error_code < MD_FORCE_ASSERT_BY_RIL_RSV_MAX)
		snprintf(ex_info, EE_BUF_LEN_UMOLY, "%sCrash by RIL\n", ex_info);
	else if (error_code % 0x100 == 0 && error_code < MD_FORCE_ASSERT_BY_RIL_USER_TRIGGER
			&& error_code != MD_FORCE_ASSERT_BY_USER_TRIGGER)
		snprintf(ex_info, EE_BUF_LEN_UMOLY, "%sCrash by AP\n", ex_info);
	else
		snprintf(ex_info, EE_BUF_LEN_UMOLY, "%sCrash by MD\n", ex_info);
#endif
}
