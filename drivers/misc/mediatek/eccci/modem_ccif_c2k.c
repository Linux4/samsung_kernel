...
/* For clear MD1/MD3 wakeup source */
static void __iomem *rccif_pccif_base[2];
#ifdef CONFIG_MTK_ECCCI_C2K
static void __ack_rccif_clear(void __iomem *rccif_base_addr, unsigned int rccif_ch_num)
{
	unsigned int  rccif_int_cnt[2]; /* RCCIF interrupt count. */
	unsigned int  rccif_ch_idx; /* RCCIF channel index. */

	if (rccif_base_addr == NULL) {
		CCCI_ERROR_LOG(-1, TAG, "ack_rccif_clear addr is NULL!!!\n");
		return;
	}

	for (rccif_ch_idx = 0; rccif_ch_idx < rccif_ch_num; rccif_ch_idx++) {
		/* Get interrupt count */
		rccif_int_cnt[0] = ccif_read32(rccif_base_addr, (0x100 + rccif_ch_idx * 4));
		/* ACK interrupt */
		ccif_write32(rccif_base_addr, (0x180 + rccif_ch_idx * 4), (0xB0000000 | rccif_int_cnt[0]));
		rccif_int_cnt[1] = ccif_read32(rccif_base_addr, (0x100 + rccif_ch_idx * 4));
		CCCI_ERROR_LOG(-1, TAG, "ack_rccif_clear, irq sta=0x%x, count_b = %d count_a = %d\n",
			ccif_read32(rccif_base_addr, (0x180 + rccif_ch_idx * 4)), rccif_int_cnt[0], rccif_int_cnt[1]);
	}
}

void ack_rccif_clear(void)
{
	/* open rccif clk */
	rccif_set_clk_cg(1);
	/* clear the wakeup source */
	__ack_rccif_clear(rccif_pccif_base[0], 4);
	__ack_rccif_clear((rccif_pccif_base[0] + 0x400), 8);
	__ack_rccif_clear(rccif_pccif_base[1], 4);
	__ack_rccif_clear((rccif_pccif_base[1] + 0x400), 8);
	/* close rccif clk */
	rccif_set_clk_cg(0);
}
#endif
...
static const struct of_device_id ccif_md1md2_of_ids[] = {
	/* add for map md2md register */
...
static int __init md_ccif_init(void)
{
	...
	/* add for map md2md register */
	for (i = 0; i < 2; i++) {
		np_pccif = of_find_compatible_node(NULL, NULL, ccif_md1md2_of_ids[i].compatible);

		if (!rccif_pccif_base[i]) {
			rccif_pccif_base[i] = of_iomap(np_pccif, 0);
			if (!rccif_pccif_base[i])
				pr_err("rccif_pccif_base iomap failed\n");
			pr_debug("rccif_pccif_base base[%d]: 0x%p\n", i, rccif_pccif_base[i]);
		}
		/* pr_debug(" debug: rccif_pccif_base base[%d]: 0x%p\n", i, rccif_pccif_base[i]); */
	}
