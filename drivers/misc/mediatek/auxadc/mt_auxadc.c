...
static int IMM_auxadc_GetOneChannelValue(int dwChannel, int data[4], int *rawdata)
{
	...
	/* pr_debug("[AUXADC] cali_reg(%x),cali_oe_a(%x), cali_ge_a(%x),cali_ge(%x),cali_oe(%x),
	...
	data[0] = ((channel[dwChannel] - cali_oe) * 1500 / (4096 + cali_ge)) / 1000; /* convert to volt*/
	data[1] = ((channel[dwChannel] - cali_oe) * 1500 / (4096 + cali_ge)) % 1000;
	pr_debug("[adc_api]imm mode:chn[%d]-->raw_data=%d;external_effuse_cali=%d.%3d;cali_reg:%x\n"
		, dwChannel, channel[dwChannel], data[0], data[1], cali_reg);
#else
	if (NULL != rawdata)
	...
	while (g_start_debug_thread) {
		for (i = 0; i < ADC_CHANNEL_MAX; i++) {
			...
			if (res < 0) {
				...
				pr_debug("[adc_driver]: channel[%d]raw:%d,data=%d.%.02d\n",
