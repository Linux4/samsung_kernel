...
void usb_phy_recover(void)
{
	...
	/*0x11210818 bit3~bit0 = 0010b*/
	USBPHY_CLR8(0x18, 0x0F);
	USBPHY_SET8(0x18, 0x02);

	/*0x11210804 bit10~bit8 = 111b*/
	USBPHY_SET8(0x05, 0x07);

	/*0x11210804 bit14~bit12 = 111b*/
	USBPHY_SET8(0x05, 0x70);

	hs_slew_rate_cal();

	/* USB phy tuning, VRT_TX: 0x6, TEMR_TX: 0x6, SQ_TH: 0x1 */
	printk("usb: phy value: tx=0x%x, rx=0x%x - before\n", USBPHY_READ8(0x05), USBPHY_READ8(0x18));
	USBPHY_CLR8(0x05, 0xf0);
	USBPHY_SET8(0x05, 0x70);

	USBPHY_CLR8(0x05, 0x0f);
	USBPHY_SET8(0x05, 0x07);
	USBPHY_CLR8(0x18, 0x0f);
	USBPHY_SET8(0x18, 0x01);
	printk("usb: phy value: tx=0x%x, rx=0x%x - after\n", USBPHY_READ8(0x05), USBPHY_READ8(0x18));

	DBG(0, "usb recovery success\n");
}

void usb_phy_set_host_mode(void)
{
	printk("%s: phy value: tx=0x%x, rx=0x%x - before\n", __func__, USBPHY_READ8(0x05), USBPHY_READ8(0x18));
	USBPHY_CLR8(0x05, 0xf0); //TX
	USBPHY_SET8(0x05, 0x20);
	USBPHY_CLR8(0x05, 0x0f);
	USBPHY_SET8(0x05, 0x02);
	
	USBPHY_CLR8(0x18, 0x0f); //RX
	USBPHY_SET8(0x18, 0x01);
	printk("%s: phy value: tx=0x%x, rx=0x%x - after\n", __func__, USBPHY_READ8(0x05), USBPHY_READ8(0x18));
}
