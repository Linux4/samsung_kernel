...
static void lk_info_parsing_v1(unsigned int *raw_ptr)
{
	...
	lk_inf.lk_info_base_addr = (phys_addr_t)lk_inf.lk_info_base_addr & PAGE_MASK;
	if (!pfn_valid(__phys_to_pfn(lk_inf.lk_info_base_addr)))
		s_g_lk_inf_base = ioremap_nocache((phys_addr_t)lk_inf.lk_info_base_addr, MAX_LK_INFO_SIZE);
	else {
		prot = pgprot_noncached(PAGE_KERNEL);
		s_g_lk_inf_base =
			(void __iomem *)vmap_reserved_mem((phys_addr_t)lk_inf.lk_info_base_addr,
							MAX_LK_INFO_SIZE, prot);
	}
	if (s_g_lk_inf_base == NULL) {
...
static int lk_info_parsing_v2(unsigned int *raw_ptr)
{
	...
	if (s_g_lk_inf_base == NULL) {
		CCCI_UTIL_ERR_MSG(" lk_info_parsing_v2 ioremap lk info buf fail\n");
		s_g_lk_load_img_status |= LK_LOAD_MD_ERR_NO_MD_LOAD;
	}
...
phys_addr_t get_md_resv_smem_start_addr(void)
{
...
int ccci_get_rat_str_from_drv(int md_id, char rat_str[], int size)
{
	...
	if ((md_support_val & MD_CAP_ENHANCE) == MD_CAP_ENHANCE)
		...
		/* 5, 6, 7 phase out */
			...
			if (bit_map == 0) {
				pr_debug("[ccci0/rat] get rat str not support\r\n");
				WARN_ON(1);
				return -2;
			}
		}
