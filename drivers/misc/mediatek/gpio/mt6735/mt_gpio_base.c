...
#else
#if 0
	GPIOERR("%s PULLEN_offset[%ld].offset:%d, pupd_offset:%d, pupd_addr:0x%lx\n", __func__, pin, PULLEN_offset[pin].offset, pupd_offset[pin].offset, pupd_addr[pin].addr);
#endif
...
#ifdef CONFIG_PM
/*---------------------------------------------------------------------------*/
void mt_gpio_suspend(struct device *dev)
{
	/* compatible with HAL */
#if 0
	struct pinctrl *pctrl;

	GPIOERR("mt_gpio_suspend\n");
	pctrl = devm_pinctrl_get_select(dev, PINCTRL_STATE_SLEEP);
	if (IS_ERR(pctrl))
		GPIOERR("mt_gpio_suspend : pinctrl_get_select error\n");
#endif
}

/*---------------------------------------------------------------------------*/
void mt_gpio_resume(struct device *dev)
{
	/* compatible with HAL */
#if 0
	struct pinctrl *pctrl;

	GPIOERR("mt_gpio_resume\n");
	pctrl = devm_pinctrl_get_select_default(dev);
	if (IS_ERR(pctrl))
		GPIOERR("mt_gpio_resume : pinctrl_get_select error\n");
#endif
}
...
/*-------pin range 65--68, drv_parm range 0--3 ------------------------------*/
int mt_set_spi_gpio_drv(unsigned long pin, unsigned long drv_parm)
{
	unsigned long reg;
    reg = GPIO_RD32(DRV_addr[pin].addr);
    reg |= (drv_parm << 1);
