...
int mt_set_gpio_driving_base(unsigned long pin, unsigned long strength)
{
	...
	if (DRV_offset[pin].offset == -1)
		return -1;
	if (DRV_width[pin].width == 4) {
		if (strength > 0x7) /*even MT6757 has 4 bit drv field, only accept maximum 0x7*/
			return -ERWRAPPER;
		reg = IOCFG_RD32(DRV_addr[pin].addr);
		reg &= ~(0x7 << bit);
		IOCFG_WR32(DRV_addr[pin].addr, reg);

		reg = IOCFG_RD32(DRV_addr[pin].addr);
		reg |= (strength << bit);
		IOCFG_WR32(DRV_addr[pin].addr, reg);
	} else if (DRV_width[pin].width == 3) {
		if (strength > 0x7)
			return -ERWRAPPER;
		reg = IOCFG_RD32(DRV_addr[pin].addr);
		reg &= ~(0x7 << bit);
...
int mt_get_gpio_driving_base(unsigned long pin)
{
	...
	if (DRV_offset[pin].offset == -1)
		return -1;
	if (DRV_width[pin].width == 4)
		return (IOCFG_RD32(DRV_addr[pin].addr) >> bit) & 0x7;
	if (DRV_width[pin].width == 3)
		return (IOCFG_RD32(DRV_addr[pin].addr) >> bit) & 0x7;
	else if (DRV_width[pin].width == 2)
		return (IOCFG_RD32(DRV_addr[pin].addr) >> bit) & 0x3;
	else if (DRV_width[pin].width == 1)
		return (IOCFG_RD32(DRV_addr[pin].addr) >> bit) & 0x1;
	return -ERINVAL;
}
