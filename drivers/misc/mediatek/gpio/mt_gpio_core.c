...
static int mt_gpio_probe(struct platform_device *dev)
{
	...
	if (dev->dev.of_node) {
		...
		pctrl = devm_pinctrl_get_select_default(&dev->dev);
		if (IS_ERR(pctrl)) {
			if (PTR_ERR(pctrl) == -EPROBE_DEFER)
				return -EPROBE_DEFER;
			GPIOERR("device does not use pinctrl\n");
		}
	}
