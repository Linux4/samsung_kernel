	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < gdvs->count; i++) {
		if(gpio_dvs_check_valid_gpio(i))
			continue;
...
int gpio_dvs_check_valid_gpio(int num)
{
	//Invalid GPIOs 70~74, 123~130, 137~146, 162~177 in MT6757
	if((num >= 70 && num <= 74) ||
		(num >= 123 && num <= 130) ||
		(num >= 137 && num <= 146) ||
		(num >= 162 && num <= 177))
...
static int secgpio_dvs_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(secgpio_dotest)) {
		...
		pr_err("Failed to create device(secgpio_check_all) and the ret value is %d", ret);
		goto fail1;
	}
...
static int __init secgpio_dvs_init(void)
{
	...
	printk(KERN_INFO "********GAURAV before class_create in INIT********");
	secgpio_dvs_class = class_create(THIS_MODULE, "secgpio_check");
        if (IS_ERR(secgpio_dvs_class)) {
                ret = PTR_ERR(secgpio_dvs_class);
                pr_err("Failed to create class(secgpio_check_all)");
