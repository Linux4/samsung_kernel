...
#define MAX_VOL_3_POLE		(606)/* 930mv/1.65 extend to 602mv */
#define MIN_VOL_4_POLE		(607)/* 1060mv/1.65  extend to 603mv*/
#define MAX_VOL_4_POLE		(1703)/* maybe it's 2630mv or 2800mv/1.65 */
...
static unsigned int Accdet_PMIC_IMM_GetOneChannelValue(int deCount)
{
	...
	ACCDET_DEBUG("ACCDET auxadc offset:%d mv, vol:%u mv\n\r", accdet_auxadc_offset, vol_val);
	return vol_val;
}
...
static unsigned int check_pole_type(void)
{
	...
	if ((vol < (cust_vol_set.vol_max_4pole + 1)) && (vol > (cust_vol_set.vol_min_4pole - 1))) {
		...
		ACCDET_DEBUG("[accdet] pole check:%d mv, AB=%d\n", vol, TYPE_AB_00);
		return TYPE_AB_00;
	} else {/* external cable support */
		ACCDET_DEBUG("[accdet] pole check:%d mv, AB=%d\n", vol, TYPE_AB_10);
		return TYPE_AB_10;
	}
	/* illegal state */
	/* ACCDET_DEBUG("[accdet] error: pole check:%d mv, AB=%d\n", vol, TYPE_AB_11); */
	return TYPE_AB_10;
}
#endif
...
static inline int accdet_setup_eint(struct platform_device *accdet_device)
{
	...
	if (IS_ERR(accdet_pinctrl1)) {
		...
		dev_err(&accdet_device->dev, "[Accdet]Cannot find accdet_pinctrl1!\n");
		return ret;
	}
	...
	if (IS_ERR(pins_eint_int)) {
		...
		dev_err(&accdet_device->dev, "[Accdet]Cannot find pinctrl state_eint_accdet!\n");
		return ret;
	}
	...
	case PLUG_OUT:
		...
		if (current_status == 0) {
			...
			if (1 == eint_accdet_sync_flag) {
				...
				ACCDET_DEBUG("[Accdet] Headset has plugged out:0\n");
			}
			mutex_unlock(&accdet_eint_irq_sync_mutex);
#endif
			...
			if (1 == eint_accdet_sync_flag) {
				...
				ACCDET_DEBUG("[Accdet] Headset has plugged out:1\n");
			}
			mutex_unlock(&accdet_eint_irq_sync_mutex);
			...
			if (1 == eint_accdet_sync_flag) {
				...
				ACCDET_DEBUG("[Accdet] Headset has plugged out:2\n");
			}
			mutex_unlock(&accdet_eint_irq_sync_mutex);
#endif
			...
			if (1 == eint_accdet_sync_flag) {
				...
				ACCDET_DEBUG("[Accdet] Headset has plugged out:3\n");
			}
			mutex_unlock(&accdet_eint_irq_sync_mutex);
#endif
		} else {
	...
	case MIC_BIAS:
		...
		if (current_status == 0) {
			...
			if (button_status) {
				...
				if (1 == eint_accdet_sync_flag)
				{
					pmic_pwrap_write(ACCDET_DEBOUNCE1, 0x800);	//reduce release key debouce
...
#if defined CONFIG_ACCDET_EINT || defined CONFIG_ACCDET_EINT_IRQ
			ACCDET_DEBUG("[Accdet]do not send plug out in micbias\n");
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if (1 == eint_accdet_sync_flag)
				accdet_status = PLUG_OUT;
			else
		...
		break;
	case HOOK_SWITCH:
		pmic_pwrap_write(ACCDET_DEBOUNCE1, cust_headset_settings->debounce1);	//resume the debounce1
		if (current_status == 0) {
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if (1 == eint_accdet_sync_flag) {
	...
	case EXT_CABLE:
		if (current_status == 1) {
			...
			if (1 == eint_accdet_sync_flag) {
				...
				ACCDET_DEBUG("[Accdet] Headset has plugged out:external cable\n");
			}
			mutex_unlock(&accdet_eint_irq_sync_mutex);
			...
			ACCDET_DEBUG("[Accdet]EXT_CABLE can't change to this state!\n");
		}
		break;
#endif
	case STAND_BY:
...
void accdet_get_dts_data(void)
{
	...
	if (node) {
		...
		cust_vol_set.vol_max_4pole = 9999;
#endif
		...
		if (cust_vol_set.vol_bias > 2500) {
			cust_vol_set.vol_bias = 2800;/* 2800mv */
			ACCDET_ERROR("[Accdet]bias vol set %d mv--->2800 mv\n", cust_vol_set.vol_bias);
		} else {
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	if (g_accdet_first == 1) {
		...
		/* queue_work(accdet_workqueue, &accdet_work); //for avoid first check adc will judge wrong */
#ifdef CONFIG_ACCDET_EINT
		accdet_disable_workqueue = create_singlethread_workqueue("accdet_disable");
		INIT_WORK(&accdet_disable_work, disable_micbias_callback);
		accdet_eint_workqueue = create_singlethread_workqueue("accdet_eint");
	...
	create_jack_factory_devices(pEarjack_priv);
#endif
