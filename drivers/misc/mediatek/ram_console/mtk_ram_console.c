...
static int __init ram_console_init(struct ram_console_buffer *buffer, size_t buffer_size)
{
	...
	LAST_RR_SET(hotplug_cpu_event, 0xFF);
	LAST_RR_SET(hotplug_cb_index, 0xFF);
	LAST_RR_SET(hotplug_cb_fp, 0xFFFFFFFFFFFFFFFF);
	LAST_RR_SET(cpu_caller, 0xFF);
	LAST_RR_SET(cpu_callee, 0xFF);
