...
{
	/* legacy pwm_intr_enable_bit should/2 to be new pwm_intr_enable_bit */
	SETREG32(PWM_INT_ENABLE, 1 << (pwm_intr_enable_bit / 2));
}
...
s32 mt_get_intr_status_hal(u32 pwm_intr_status_bit)
{
	...
	/* legacy pwm_intr_status_bit should/2 to be new pwm_intr_status_bit */
...
{
	/* legacy pwm_intr_ack_bit should/2 to be new pwm_intr_ack_bit */
	SETREG32(PWM_INT_ACK, 1 << (pwm_intr_ack_bit / 2));
}
