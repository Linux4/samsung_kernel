		...
		if (data->user == user) {
			...
			if (ppm_main_info.cur_power_state == PPM_POWER_STATE_L_ONLY
				...
				if (core_num > get_cluster_max_cpu_core(PPM_CLUSTER_L)) {
					data->limit[PPM_CLUSTER_L].max_core_num =
						get_cluster_max_cpu_core(PPM_CLUSTER_L);
					data->limit[PPM_CLUSTER_LL].max_core_num =
						core_num - get_cluster_max_cpu_core(PPM_CLUSTER_L);
				} else {
				...
				if (core_num > get_cluster_max_cpu_core(PPM_CLUSTER_LL)) {
					...
					data->limit[PPM_CLUSTER_L].max_core_num =
						core_num - get_cluster_max_cpu_core(PPM_CLUSTER_LL);
				} else {
...
void mt_ppm_sysboost_max_freq(enum ppm_sysboost_user user, unsigned int freq)
{
	...
	if (user != BOOST_BY_BOOT_TIME_OPT && user != BOOST_BY_UT && user != BOOST_BY_SS)
		return;
...
static int __init ppm_sysboost_policy_init(void)
{
	...
	for_each_ppm_clusters(i) {
		sysboost_data[BOOST_BY_BOOT_TIME_OPT].limit[i].min_freq_idx =
			get_cluster_max_cpufreq_idx(i);
		sysboost_data[BOOST_BY_BOOT_TIME_OPT].limit[i].min_core_num =
