...
int hps_core_init(void)
{
	...
	if (hps_ctxt.periodical_by == HPS_PERIODICAL_BY_TIMER) {
		...
		hps_ctxt.tmr_list.expires = jiffies + msecs_to_jiffies(hps_ctxt.timer_interval);
		add_timer(&hps_ctxt.tmr_list);
	} else if (hps_ctxt.periodical_by == HPS_PERIODICAL_BY_HR_TIMER) {
		unsigned long ms_to_ns = 1E6L;
		unsigned long setVal = (unsigned long)hps_ctxt.timer_interval * ms_to_ns;
		ktime = ktime_set(0, setVal);
		/*init Hrtimer */
...
int hps_restart_timer(void)
{
	...
	if (hps_ctxt.periodical_by == HPS_PERIODICAL_BY_TIMER) {
		...
		if (time_differ >= hps_ctxt.timer_interval) {
			...
			hps_ctxt.tmr_list.expires =
			    jiffies + msecs_to_jiffies(hps_ctxt.timer_interval - time_differ);
			add_timer(&hps_ctxt.tmr_list);
		}
	} else if (hps_ctxt.periodical_by == HPS_PERIODICAL_BY_HR_TIMER) {
#if 1
