...
PROC_FOPS_RW_UINT(
	timer_interval,
	hps_ctxt.timer_interval,
	hps_proc_uint_write_with_lock_reset);
PROC_FOPS_RW_UINT(
	quicklanding_enabled,
	hps_ctxt.quicklanding_enabled,
	hps_proc_uint_write_with_lock_reset);
PROC_FOPS_RW_UINT(
	cpu_load_weight,
	hps_ctxt.cpu_load_weight,
	hps_proc_uint_write_with_lock_reset);
...
#if defined(CONFIG_SW_SELF_DISCHARGING)
		else if(sdchg_idle_policy_set) {
			if(sdchg_cpu_core_lock_num > little_num_base_perf_serv)
				little_num_base_perf_serv = sdchg_cpu_core_lock_num;
		}
#endif
...
int hps_procfs_init(void)
{
	...
	const struct pentry entries[] = {
		...
		PROC_ENTRY(timer_interval),
		PROC_ENTRY(quicklanding_enabled),
		PROC_ENTRY(cpu_load_weight),
	};
