...
#define LOG_ERR(format, args...) pr_debug(AF_DRVNAME " [%s] " format, __func__, ##args)
#else
...
static int s4AF_ReadReg(u8 a_uAddr, u16 *a_pu2Result)
{
	...
	if (i4RetValue < 0) {
		...
		return -1;
	} else if (i4RetValue != 1) {
		LOG_INF("I2C read failed!! %d bytes received (expected 1)\n", i4RetValue);
		return -1;
	}
...
static int s4AF_WriteReg(u16 a_u2Addr, u16 a_u2Data)
{
	...
	if (i4RetValue < 0) {
		...
		LOG_INF("I2C write failed!! %d bytes transferred (expected 2)\n", i4RetValue);
		return -1;
	}
	...
	if (*g_pAF_Opened == 1) {
		...
		if (initdrv() == 1) {
			...
			*g_pAF_Opened = 2;
			...
			LOG_ERR("InitDrv Fail!! I2C error occurred");
		}
...
int AK7372AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
