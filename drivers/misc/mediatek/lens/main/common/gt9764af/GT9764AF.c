...
static int initAF(void)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		ret = s4AF_ReadReg(0x00, &Temp);  //ic info
		if (0 != ret){
			LOG_INF("read fail return\n");
			return 0;
		}
		LOG_INF("Check HW version: 0x00 is %x\n", Temp);
		ret = s4AF_WriteReg(0, 0x02, 0x00); //CONTROL
		//bug626603 liudijin, add, 2021/04/07,update GT9764 af acc mode code from fae.
		s4AF_WriteReg(0, 0x02, 0x02); // + AAC mode
		s4AF_WriteReg(0, 0x06, 0x40);
		s4AF_WriteReg(0, 0x07, 0x79);
