...
/*
 * ST_GT9772AF voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "ST_GT9772AF_DRV"
...
static int initAF(void)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		//+bug682590,zhanghengyuan,wt, add, 2021/9/8, add VCM init setting
		ret = s4AF_WriteReg(0, 0x06, 0x84);
		ret = s4AF_WriteReg(0, 0x07, 0x01);
		ret = s4AF_WriteReg(0, 0x08, 0x73); //+bug682590,lintaicheng,wt, modify, 2021/11/17, update VCM init setting
		//+bug682590,zhanghengyuan,wt, add, 2021/9/8, add VCM init setting
...
/* ////////////////////////////////////////////////////////////// */
long ST_GT9772AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
int ST_GT9772AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	int Ret = 0;
	...
	//+bug682590,zhanghengyuan,wt, add, 2021/9/8, add for change motor move when close camera
	if (*g_pAF_Opened == 2) {
		LOG_INF("Wait\n");
		while (currPosition>100)
		{
			...
			else if(currPosition > 250){
				currPosition -= 30;
			}else if(currPosition > 150){
				currPosition -= 20;
			}else if(currPosition > 100){
				currPosition -= 10;
			}

			if (setPosition((unsigned short)currPosition) == 0) {
				g_u4CurrPosition = currPosition;
				Ret = 0;
			} else {
				LOG_INF("set I2C failed when moving the motor\n");
				Ret = -1;
...
int ST_GT9772AF_PowerDown(struct i2c_client *pstAF_I2Cclient,
			int *pAF_Opened)
{
...
int ST_GT9772AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int ST_GT9772AF_GetFileName(unsigned char *pFileName)
{
