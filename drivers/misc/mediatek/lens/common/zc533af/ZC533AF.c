...
#define AF_DRVNAME "ZC533AFAF_DRV"
...
static int s4ZC533AF_ReadReg(unsigned short *a_pu2Result)
{
	/* int  i4RetValue = 0; */
	/* char pBuff[2]; */
	...
	*a_pu2Result = (read_data(0x03) << 8) + (read_data(0x04) & 0xff);
	...
	LOG_INF("[ZC533AF]  s4ZC533AF_ReadReg %d\n", *a_pu2Result);
	return 0;
}

static int s4ZC533AF_WriteReg(u16 a_u2Data)
{
	int i4RetValue = 0;

	char puSendCmd[3] = { 0x03, (char)(a_u2Data >> 8), (char)(a_u2Data & 0xFF) };

	LOG_INF("[ZC533AF]  write %d\n", a_u2Data);
...
static void initdrv(void)
{
	...
	char puSendCmd3[2] = { 0x05, 0x04 };
	char puSendCmd4[2] = { 0x06, 0x64 };
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (s4ZC533AF_WriteReg((unsigned short)g_u4TargetPosition) == 0) {
		spin_lock(g_pAF_SpinLock);
...
void ZC533AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock,
			  int *pAF_Opened)
{
