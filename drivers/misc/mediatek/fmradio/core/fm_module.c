...
#ifdef CONFIG_OF
static void set_fm_ant_sel_gpio(unsigned int flag, fm_u32 cmd)
{
	WCN_DBG(FM_ERR | MAIN, "set_fm_ant_sel_gpio from %#x\n", cmd);

	gpio_direction_output(g_fm->GPIO_FM_ANT_SEL, flag);
	usleep_range(10000, 10000);
	WCN_DBG(FM_ERR | MAIN, "fm_ant_sel_gpio is %d\n", flag);
}
#endif
...
#ifdef CONFIG_OF
			if (gpio_is_valid(g_fm->GPIO_FM_ANT_SEL))
				set_fm_ant_sel_gpio(1, cmd);
#endif
...
#ifdef CONFIG_OF
	fm_node = of_find_compatible_node(NULL, NULL, "mediatek,mt_mt6627_fmradio");
	if (fm_node) {
		fm->GPIO_FM_ANT_SEL = of_get_named_gpio(fm_node, "fmradio_ant_sel", 0);
		ret = gpio_request(fm->GPIO_FM_ANT_SEL, "fm_ant_sel");
		if (ret != 0) {
			WCN_DBG(FM_ERR | MAIN, "No fm_ant_sel control\n");
		} else {
			WCN_DBG(FM_ERR | MAIN, "Got fm_ant_sel %d\n", fm->GPIO_FM_ANT_SEL);
			gpio_direction_output(fm->GPIO_FM_ANT_SEL, 0);
		}
	} else {
		fm->GPIO_FM_ANT_SEL = -1;
		WCN_DBG(FM_ERR | MAIN, "No device match for FM Radio\n");
	}
#endif
