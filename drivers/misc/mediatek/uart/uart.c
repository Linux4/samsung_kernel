...
static bool is_uart_suspending;
#endif
...
#if defined(CONFIG_SEC_PM) && defined(CONFIG_MUIC_NOTIFIER)
	if(jig_is_attached && !is_uart_suspending)
		return -EBUSY;
#endif
...
static int mt_cpuidle_muic_notifier(struct notifier_block *nb,
...
static int __init mt_cpuidle_muic_notifier_init(void)
{
	return muic_notifier_register(&cpuidle_muic_nb,
			mt_cpuidle_muic_notifier, MUIC_NOTIFY_DEV_CPUIDLE);
}
late_initcall(mt_cpuidle_muic_notifier_init);
#endif
/*---------------------------------------------------------------------------*/
