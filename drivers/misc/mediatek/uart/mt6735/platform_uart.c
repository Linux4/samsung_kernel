...
#ifndef CONFIG_MTK_SERIAL_DEBUG
	MSG(CFG, "%s: RATEFIX(%02X); HSPEED(%02X); CNT(%02X); PNT(%02X); DLH(%02X), DLL(%02X), IER(%02X)\n",
#else
	pr_err("%s: RATEFIX(%02X); HSPEED(%02X); CNT(%02X); PNT(%02X); DLH(%02X), DLL(%02X), IER(%02X)\n",
#endif
...
#else
				pr_err("[UART%d] Polling flush timeout %x %x %x %x %x\n", port->line,
					UART_READ32(VFF_W_INT_BUF_SIZE(vfifo->base)),
					UART_READ32(VFF_INT_BUF_SIZE(vfifo->base)),
					UART_READ32(VFF_VALID_SIZE(vfifo->base)),
					UART_READ32(VFF_LEFT_SIZE(vfifo->base)),
					UART_READ32(VFF_FLUSH(vfifo->base)));
#endif
				return;
			}
		}
...
void mtk_uart_dma_vfifo_tx_tasklet(unsigned long arg)
{
	...
	unsigned long flags;
#ifdef CONFIG_MTK_SERIAL_DEBUG
	unsigned long long enter, inside, count;
#endif
	...
	if (atomic_inc_return(&vfifo->entry) > 1) {
		...
		while (UART_READ32(VFF_LEFT_SIZE(base)) >= vfifo->trig) {
			...
			inside = sched_clock();
			count++;
			if (inside - enter > 100*1000*1000) {
				pr_err("while VFF_LEFT_SIZE %d,%llu; %x %x %x %x %x\n", vfifo->trig, count,
					UART_READ32(VFF_W_INT_BUF_SIZE(base)),
					UART_READ32(VFF_INT_BUF_SIZE(base)),
...
#ifdef CONFIG_MTK_SERIAL_DEBUG
		if (sched_clock() - enter > 100*1000*1000) {
			pr_err("%s bottom half too long\n", __func__);
			BUG();
		}
#endif
	}
...
/* Samsung uses uart0 in sleep mode as well */
