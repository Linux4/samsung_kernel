...
{
#if 0
	fl_err("s2mu005_verify_level_Ch1. %d\n", level);
	if (level < 0)
		level = 0;
	else if (level >= 3)
		level = 2;
...
{
#if 0
	fl_err("s2mu005_enable_ch1. %d\n", s2mu005_level_ch1);
	if (!s2mu005_is_torch_Ch1(s2mu005_level_ch1)) {
	/* torch mode */
		fl_err("ss_rear_flash_led_torch_on\n");
		ss_rear_flash_led_torch_on();
	} else {
	/* flash mode */
		fl_err("ss_rear_flash_led_flash_on\n");
		ss_rear_flash_led_flash_on();
	}
	return 1;
#endif
	fl_err("s2mu005_enable_ch1. %d\n", s2mu005_level_ch1);
	if (s2mu005_level_ch1 == 3)
		ss_rear_flash_led_flash_on();
	else
		ss_rear_flash_led_torch_on();
...
static int s2mu005_set_level_ch1(int level)
{
	...
	fl_err("s2mu005_set_level_ch1. %d\n", level);
...
int s2mu005_set_level_ch2(int level)
{
	...
	fl_err("s2mu005_set_level_ch2. %d\n", level);
	return ret;
}
...
static int s2mu005_open(void *pArg)
{
	/* Actual behavior move to set driver function since power saving issue */
	return 0;
}

static int s2mu005_release(void *pArg)
{
...
static int s2mu005_set_driver(void)
{
