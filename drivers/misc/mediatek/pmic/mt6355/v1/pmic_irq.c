...
/*
 * MTK : using HOME KEY
 * SEC : using Volume Down KEY
 */
void pmic_keys_check(void)
{
	if (pmic_get_register_value(PMIC_HOMEKEY_DEB) == 0)
		homekey_int_handler();
	else
		homekey_int_handler_r();

	pr_err("%s: sec_input: called\n", __func__);
}
EXPORT_SYMBOL(pmic_keys_check);
