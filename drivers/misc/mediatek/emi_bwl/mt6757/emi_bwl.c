...
#define EMI_MDCT		IOMEM(EMI_BASE_ADDR+0x078)
void change_AP_RSV_BUF_NUM(int buf_num)
{
	/* pr_err("Original EMI_MDCT value is 0x%x\n", readl(EMI_MDCT)); */
	mt_reg_sync_writel((readl(EMI_MDCT) & 0xFFFFFF) | ((buf_num & 0xFF) << 24), EMI_MDCT);
	/* pr_err("EMI_MDCT value change to 0x%x\n", readl(EMI_MDCT)); */
}
...
int mtk_mem_bw_ctrl(int sce, int op)
{
	...
	if (highest != cur_con_sce) {
		if (dram_type == TYPE_LPDDR3) {
			...
			if (highest == CON_SCE_LAUNCH)
				change_AP_RSV_BUF_NUM(0x22);
			else
				change_AP_RSV_BUF_NUM(0x11);
