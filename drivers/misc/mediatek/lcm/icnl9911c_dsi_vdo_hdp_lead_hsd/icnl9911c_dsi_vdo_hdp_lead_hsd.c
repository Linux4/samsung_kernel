...
static struct LCM_setting_table init_setting_vdo[] = {
	...
	{0xF1, 2,{0xA5,0xA6}},
	{0xF6,1,{0x3F}},
	{0xB0,29,{0x83,0x82,0x84,0x85,0x00,0x00,0x00,0x00,0x33,0x33,0x33,0x33,0x7B,0x01,0x01,0x75,0x01,0x01,0x3F,0x05,0x04,0x03,0x02,0x01,0x02,0x03,0x04,0x00,0x00}},
	{0xB1,32,{0x13,0x42,0x81,0x81,0x7B,0x00,0x00,0x75,0x00,0x00,0x04,0x08,0x53,0x00,0x00,0x00,0x44,0x40,0x02,0x01,0x40,0x02,0x01,0x40,0x02,0x01,0x40,0x02,0x01,0x00,0x00,0x00}},
	{0xB2,17,{0x54,0xC4,0x82,0x05,0x40,0x02,0x01,0x40,0x02,0x01,0x05,0x05,0x54,0x0C,0x0C,0x0D,0x0B}},
	{0xB3,31,{0x12,0x00,0x00,0x0F,0x0A,0x26,0x26,0x91,0xA2,0x13,0x44,0x3C,0x28,0x00,0x88,0x01,0x02,0x08,0x20,0x30,0x08,0x09,0x44,0x20,0x40,0x20,0x40,0x08,0x09,0x22,0x33}},
	{0xB4,32,{0x00,0x00,0x00,0x00,0x00,0xDC,0x1D,0x1E,0x1F,0x0C,0x0E,0x10,0x12,0x04,0x06,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFC,0x01,0x80,0x00,0x00,0x00,0x00,0x00}},
	{0xB5,32,{0x00,0x00,0x00,0x00,0x00,0xDC,0x1D,0x1E,0x1F,0x0D,0x0F,0x11,0x13,0x05,0x07,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFC,0x01,0x80,0x00,0x00,0x00,0x00,0x00}},
	...
	{0xC1,19,{0xC0,0x20,0x20,0x96,0x04,0x54,0x58,0x04,0x2A,0x40,0x36,0x00,0x07,0xC0,0x10,0xFF,0x95,0x01,0xC0}},
	{0xC2, 1,{0x00}},
	...
	{0xC7,22,{0xF7,0x8F,0x66,0x49,0x1A,0xFC,0xCF,0x28,0xFB,0xD8,0xB7,0x8F,0xEE,0xC7,0xAD,0x85,0x6F,0x4C,0x1A,0x7C,0x80,0x00}},
	{0xC8,22,{0xF7,0x8F,0x66,0x49,0x1A,0xFC,0xCF,0x28,0xFB,0xD8,0xB7,0x8F,0xEE,0xC7,0xAD,0x85,0x6F,0x4C,0x1A,0x7C,0x80,0x00}},
	{0xC9, 6,{0x45,0x00,0x1F,0xFF,0x3F,0x03}},
	{0xCB,21,{0x01,0x40,0x55,0x40,0x04,0x40,0x35,0x43,0x43,0x50,0x1E,0x40,0x40,0x43,0x43,0x64,0x23,0x40,0x40,0x22,0x04}},
	{0xD0, 8,{0x80,0x0D,0xFF,0x0F,0x61,0x0B,0x08,0x04}},
	{0xD2,15,{0x42,0x0C,0x30,0x01,0x80,0x26,0x04,0x00,0x00,0xC3,0x00,0x00,0x00,0x00,0x30}},
	{0xD7, 1,{0xDF}},
...
static void lcm_get_params(struct LCM_PARAMS *params)
{
	...
	params->dsi.horizontal_backporch = 78;//84;
	params->dsi.horizontal_frontporch = 78;//88;
...
{
	int ret = 0;
	pr_debug("[LCM]icnl9911c_lead lcm_init_power !\n");
    lcm_reset_pin(LOW);
...
static void lcm_suspend_power(void)
{
	if (chipone_gestrue_status == 1) {
		pr_debug("[LCM] icnl9911c_lead lcm_suspend_power chipone_gestrue_status = 1!\n");
	} else {
		int ret  = 0;
		pr_debug("[LCM] icnl9911c_lead lcm_suspend_power !\n");
		//lcm_reset_pin(LOW);
		MDELAY(2);
...
{
	pr_debug("[LCM]icnl9911c_lead lcm_resume_power !\n");
	lcm_init_power();
}

unsigned char g_lcm_f6_reg_lead = 0x00;
static void get_lcm_fb_cmdline(void)
{
	...
	ret =  kstrtou8(lcm_f6_reg, 0, &g_lcm_f6_reg_lead);
	if (ret != 0){
...
{
	pr_debug("[LCM] icnl9911c_lead lcm_init\n");
	lcm_reset_pin(HIGH);
	MDELAY(5);
	lcm_reset_pin(LOW);
	MDELAY(5);
    ...
    init_setting_vdo[2].para_list[0] = g_lcm_f6_reg_lead;
	push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
	pr_debug("[LCM] icnl9911c_lead----init success ----\n");
}
...
static void lcm_setbacklight_cmdq(void* handle,unsigned int level)
{
	...
	pr_debug("incl9911c_lead backlight: para_list[0]=%x,para_list[1]=%x\n",bl_level[0].para_list[0],bl_level[0].para_list[1]);
	push_table(handle, bl_level, sizeof(bl_level) / sizeof(struct LCM_setting_table), 1);
}

struct LCM_DRIVER icnl9911c_dsi_vdo_hdp_lead_hsd_drv = {
	.name = "icnl9911c_dsi_vdo_hdp_lead_hsd",
