...
{0x0a,0x01,{0x73}},
{0x0b,0x01,{0x00}},
{0x0c,0x01,{0x5A}},  //CLK duty modify for sensor mura by shawn_20200512
{0x0d,0x01,{0x5A}},  //CLK duty modify for sensor mura by shawn_20200512
{0x0e,0x01,{0x05}},
{0x0f,0x01,{0x05}},
{0x28,0x01,{0x48}},
{0x29,0x01,{0x86}},
...
{0x2B,0x01,{0x86}},
{0x31,0x01,{0x07}},
{0x32,0x01,{0x23}},  	//2A VGL
{0x33,0x01,{0x00}},	//00 VSD
{0x34,0x01,{0x0B}},	//0B STV4
{0x35,0x01,{0x09}},  	//09 STV2
{0x36,0x01,{0x02}},       //02 VGLO
{0x37,0x01,{0x15}},	//11 CLK2
{0x38,0x01,{0x17}},  	//13 CLK4
{0x39,0x01,{0x11}},  	//15 CLK6
{0x3A,0x01,{0x13}},	//17 CLK8
{0x3B,0x01,{0x22}},	//06 GCH
{0x3C,0x01,{0x01}},  	//01 VDS
{0x3D,0x01,{0x07}},
{0x3E,0x01,{0x07}},
{0x3F,0x01,{0x07}},
{0x40,0x01,{0x07}},
...
{0x46,0x01,{0x07}},
{0x47,0x01,{0x07}},
{0x48,0x01,{0x23}},	//2A VGL
{0x49,0x01,{0x00}},	//00 VSD
{0x4A,0x01,{0x0A}},	//0A STV3
{0x4B,0x01,{0x08}},	//08 STV1
{0x4C,0x01,{0x02}},       //02 VGLO
{0x4D,0x01,{0x14}},  	//10 CLK1
{0x4E,0x01,{0x16}},	//12 CLK3
{0x4F,0x01,{0x10}},	//14 CLK5
{0x50,0x01,{0x12}},	//16 CLK7
{0x51,0x01,{0x22}},	//06 GCH
{0x52,0x01,{0x01}},	//01 VDS
{0x53,0x01,{0x07}},
{0x54,0x01,{0x07}},
{0x55,0x01,{0x07}},
{0x56,0x01,{0x07}},
...
{0x06,0x01,{0x92}},     // Internal Line Time (RTN)
{0x0B,0x01,{0xA0}},     // Internal VFP[9]
{0x0C,0x01,{0x00}},     // Internal VFP[8]
{0x0D,0x01,{0x1A}},     // Internal VBP
{0x0E,0x01,{0xFA}},     // Internal VFP
{0x40,0x01,{0x4C}},   //SDT = 2us
...
{0x03,0x01,{0x01}},  //VCOM
{0x04,0x01,{0x43}}, //VCOM
{0x50,0x01,{0x2F}},  //VGH pump x3
{0x58,0x01,{0x63}},  //VGL pump x3
{0x63,0x01,{0x9C}},  //GVDDN 5.6V
{0x64,0x01,{0x8D}},  //GVDDP -5.6V
{0x68,0x01,{0x65}},  //VGHO clamp  12V
{0x69,0x01,{0x81}},  //VGH clamp 14.10V
{0x6A,0x01,{0xC9}},  //VGLO clamp -14V
{0x6B,0x01,{0xCF}},  //VGL clamp -16V
{0x46,0x01,{0x00}},      // LVD HVREG option
{0x85,0x01,{0x37}},     // HW RESET option

// Resolution
{0xFF,0x03,{0x98,0x82,0x06}},
{0xD9,0x01,{0x10}},     // 4Lane
{0xC0,0x01,{0xF0}},     // NL = 1520
{0xC1,0x01,{0x15}},     // NL = 1520

// Gamma Register
{0xFF,0x03,{0x98,0x82,0x08}},
{0xE0,0x1B,{0x00,0x24,0x41,0x5A,0x81,0x40,0xA6,0xC8,0xF2,0x18,0x55,0x58,0x8F,0xC4,0xF7,0xAA,0x2E,0x6F,0x97,0xC9,0xFE,0xF2,0x26,0x63,0x96,0x03,0xEC}},
{0xE1,0x1B,{0x00,0x24,0x41,0x5A,0x81,0x40,0xA6,0xC8,0xF2,0x18,0x55,0x58,0x8F,0xC4,0xF7,0xAA,0x2E,0x6F,0x97,0xC9,0xFE,0xF2,0x26,0x63,0x96,0x03,0xEC}},
...
{0x13,0x01,{0x10}},     // TSHD Rise position
...
static void lcm_init(void)
{
	...
	//pr_debug("[LCM]ili9882n--txd--sm5109----init success ----\n");
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	pr_debug("[txd_ili9882n]para_list[0]=%x,para_list[1]=%x\n",bl_level[1].para_list[0],bl_level[1].para_list[1]);
...
{
	pr_err("[txd_ili9882n] cabc get to %d\n", cabc_status);
	*status = cabc_status;
}
...
#if (LCM_DSI_CMD_MODE)
struct LCM_DRIVER ili9882n_hdp_dsi_cmd_ilitek_rt5081_lcm_drv = {
	.name = "ili9882n_hd_plus_dsi_cmd_truly_lcm_drv",
#else

struct LCM_DRIVER ili9882n_hd_plus_dsi_vdo_txd_lcm_drv = {
	.name = "ili9882n_hd_plus_vdo_txd",
#endif
