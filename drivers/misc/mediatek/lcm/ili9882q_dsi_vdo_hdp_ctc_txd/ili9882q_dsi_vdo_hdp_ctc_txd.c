...
{
	pr_debug("[LCM]ili9882q_txd lcm_init_power !\n");
    lcm_reset_pin(LOW);
    MDELAY(2);
//    lcm_vio_enable(HIGH);
//    MDELAY(10);
...
{
	pr_debug("[LCM]ili9882q_txd lcm_resume_power !\n");
	lcm_init_power();
}
...
{
	pr_debug("[LCM] ili9882q_txd lcm_init\n");
	//lcm_reset_pin(HIGH);
	//MDELAY(10);
	//lcm_reset_pin(LOW);
	MDELAY(2);
	...
	pr_debug("[LCM]ili9882q_txd----init success ----\n");
}
...
static void lcm_setbacklight_cmdq(void* handle,unsigned int level)
{
	...
	pr_debug("%s,ili9882q_txd backlight: level = %d,bl_lvl=%d\n", __func__, level,bl_lvl);
	// set 12bit
	bl_level[0].para_list[0] = (bl_lvl&0xF00)>>8;
	bl_level[0].para_list[1] = (bl_lvl&0xFF);
	pr_debug("ili9882q_txd backlight: para_list[0]=%x,para_list[1]=%x\n",bl_level[0].para_list[0],bl_level[0].para_list[1]);
	push_table(handle, bl_level, sizeof(bl_level) / sizeof(struct LCM_setting_table), 1);
}

struct LCM_DRIVER ili9882q_dsi_vdo_hdp_ctc_txd_drv = {
	.name = "ili9882q_dsi_vdo_hdp_ctc_txd",
