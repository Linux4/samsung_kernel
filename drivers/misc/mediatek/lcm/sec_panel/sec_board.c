...
#define SEC_BOARD_DTS_NAME	"sec_board"

#if defined(CONFIG_BOARD_DEBUG)
#define bd_dbg(fmt, ...)		pr_debug(pr_fmt("%s: %3d: %s: " fmt), SEC_BOARD_DTS_NAME, __LINE__, __FUNCTION__, ##__VA_ARGS__)
#else
#define bd_dbg(fmt, ...)
#endif
#define bd_info(fmt, ...)		pr_info(pr_fmt("%s: %3d: %s: " fmt), SEC_BOARD_DTS_NAME, __LINE__, __FUNCTION__, ##__VA_ARGS__)
#define bd_warn(fmt, ...)		pr_warn(pr_fmt("%s: %3d: %s: " fmt), SEC_BOARD_DTS_NAME, __LINE__, __FUNCTION__, ##__VA_ARGS__)
...
static int make_list(struct device *dev, struct list_head *head, const char *name)
{
	...
	np = of_find_node_with_property(NULL, SEC_BOARD_DTS_NAME);
	if (!np) {
		bd_warn("%s property does not exist, so create dummy\n", SEC_BOARD_DTS_NAME);
		return -EINVAL;
	}

	np = of_parse_phandle(np, SEC_BOARD_DTS_NAME, 0);
	if (!np) {
		bd_warn("%s does not exist, so create dummy\n", SEC_BOARD_DTS_NAME);
		return -EINVAL;
	}
