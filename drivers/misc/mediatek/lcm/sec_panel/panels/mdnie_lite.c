		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!WRGB_IS_VALID(white_r) || !WRGB_IS_VALID(white_g) || !WRGB_IS_VALID(white_b) || mdnie->mode != AUTO)
		return count;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (enable == NIGHT_MODE_ON) {
		wbuf = &mdnie->tune->night_table[NIGHT_MODE_ON].seq[scr_info->index].cmd[scr_info->cr];
...
static int mdnie_register_fb(struct mdnie_info *mdnie)
{
	...
	return sec_panel_register_notifier(&mdnie->fb_notif);
}
