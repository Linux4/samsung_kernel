...
unsigned char g_lcm_f6_reg = 0x00;
...
{0xB1,29,{0x11,0x42,0x86,0x00,0x01,0x01,0x01,0x66,0x01,0x01,0x04,0x08,0x54,0x00,0x00,0x00,0x44,0x40,0x02,0x01,0x40,0x02,0x01,0x40,0x02,0x01,0x40,0x02,0x01}},
...
{0xBE,10,{0x5A,0x5A,0x56,0x46,0x0C,0x77,0x43,0x07,0x0E,0x0E}},
{0xBF,8,{0x07,0x25,0x07,0x25,0x7F,0x00,0x11,0x04}},
{0xC0,9,{0x10,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0xFF,0x00}},
{0xC1,19,{0xC0,0x20,0x20,0x7C,0x04,0x58,0x58,0x04,0x2A,0x40,0x36,0x00,0x07,0xC0,0x10,0xFF,0x97,0x01,0xC0}},
{0xC2,1,{0x00}},
{0xC3,9,{0x06,0x00,0xFF,0x00,0xFF,0x00,0x00,0x81,0x01}},
{0xC4,11,{0x84,0x01,0x2B,0x41,0x00,0x3C,0x00,0x03,0x03,0x2E,0x00}},
{0xC5,11,{0x03,0x1C,0xC0,0xC0,0x40,0x10,0x42,0x44,0x08,0x09,0x14}},
{0xC6,10,{0x87,0x96,0x2A,0x29,0x29,0x31,0x7F,0x34,0x08,0x04}},//H-LINETUNING
//GAMM A2.2
{0xC7,22,{0xFC,0xAC,0x8B,0x70,0x40,0x20,0xF1,0x48,0x18,0xF2,0xCD,0xA1,0xFA,0xCF,0xB2,0x87,0x70,0x4D,0x1A,0x7E,0xC0,0x00}},
{0xC8,22,{0xFC,0xAC,0x8B,0x70,0x40,0x20,0xF1,0x48,0x18,0xF2,0xCD,0xA1,0xFA,0xCF,0xB2,0x87,0x70,0x4D,0x1A,0x7E,0xC0,0x00}},

//UI M,{ode Gain Setting
//{0xFE,1,{0xEF}},
...
{
	pr_debug("[LCM]incl9911c_dijing lcm_init_power !\n");
...
static void lcm_suspend_power(void)
{
	if (chipone_gestrue_status == 1) {
		printk("[CTS-lcm]lcm_suspend_power chipone_gestrue_status = 1\n");
	} else {
		printk("[CTS-lcm]lcm_suspend_power chipone_gestrue_status = 0\n");
	    pr_debug("[LCM] incl9911c_dijing lcm_suspend_power !\n");
        //lcm_reset_pin(LOW);
        //MDELAY(2);
...
{
	pr_debug("[LCM]incl9911c_dijing lcm_resume_power !\n");
	lcm_init_power();
}
...
static void get_lcm_fb_cmdline(void)
{
	...
	ret =  kstrtou8(lcm_f6_reg, 0, &g_lcm_f6_reg);
	if (ret != 0){
...
{
	pr_debug("[LCM] incl9911c_dijing lcm_init\n");
	lcm_reset_pin(HIGH);
	MDELAY(10);
	lcm_reset_pin(LOW);
	MDELAY(5);
    ...
    init_setting_vdo[2].para_list[0] = g_lcm_f6_reg;
	push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
	pr_debug("[LCM]incl9911c_dijing----init success ----\n");
}
...
static void lcm_setbacklight_cmdq(void* handle,unsigned int level)
{
	...
	pr_debug("%s,incl9911c_dijing backlight: level = %d,bl_lvl=%d\n", __func__, level,bl_lvl);
	// set 12bit
	bl_level[0].para_list[0] = (bl_lvl&0xF00)>>8;
	bl_level[0].para_list[1] = (bl_lvl&0xFF);
	pr_debug("incl9911c_dijing backlight: para_list[0]=%x,para_list[1]=%x\n",bl_level[0].para_list[0],bl_level[0].para_list[1]);
	push_table(handle, bl_level, sizeof(bl_level) / sizeof(struct LCM_setting_table), 1);
}

struct LCM_DRIVER incl9911c_dsi_vdo_hdp_huajiacai_dijing_drv = {
	.name = "incl9911c_dsi_vdo_hdp_huajiacai_dijing",
