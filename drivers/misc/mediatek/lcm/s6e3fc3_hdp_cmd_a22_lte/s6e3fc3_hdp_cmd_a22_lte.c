...
static struct LCM_setting_table lcm_initialization_setting[] = {
	...
	{0xF0, 2, {0x5A, 0x5A} },
	{0xF2, 41, { 0x00, 0x02, 0xB4, 0x90, 0x50, 0x00, 0x0C, 0x00,
...
static struct LCM_setting_table lcm_initialization_setting_rev3[] = {
	{0x9F, 2, {0xA5, 0xA5} },
...
static void lcm_init(void)
{
	...
	if (IS_ERR_OR_NULL(root)) {
		pr_info("%s, root dev node is NULL\n", __func__);
	} else {
		ret = of_property_read_u32(root, "dtbo-hw_rev", &hw_version);
		if (ret < 0)
			pr_info("%s, get dtbo-hw_rev fail:%d\n", __func__, ret);
		else
	...
	if (hw_version <= 2)
		...
		push_table(NULL, lcm_initialization_setting_rev3,
				ARRAY_SIZE(lcm_initialization_setting_rev3), 1);
...
struct LCM_DRIVER s6e3fc3_hdp_cmd_a22_lte_lcm_drv = {
	.name				= "s6e3fc3_hdp_cmd_a22_lte",
