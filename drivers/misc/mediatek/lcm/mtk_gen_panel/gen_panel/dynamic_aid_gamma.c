...
static void mtp_ctoi(int (*dst)[3], char *src)
{
	...
	for (volt = VOLT_V255, cnt = 0; volt >= VOLT_VT; volt--) {
		for (offset = R_OFFSET; offset <= B_OFFSET; offset++) {
			if (volt == VOLT_V255) {
				dst[volt][offset] =
					char_to_int_v255(src[cnt], src[cnt + 1]);
				cnt += 2;
			} else {
				dst[volt][offset] =
					char_to_int(src[cnt++]);
...
{
	long long result_1, result_2, result_3 = 0;
	int sign, offset;
	...
	if (volt == VOLT_VT) {
		...
		for (offset = R_OFFSET; offset <= B_OFFSET; offset++) {
			result_1 = (long long)pSmart->vregout_voltage -
				(long long)((int *)&pSmart->GRAY.TABLE[index[volt]])[offset];
			result_2 = result_1 * v_coeff[volt].denominator;
			sign = (result_2 < 0) ? -1 : 1;
			if (result_2 < 0)
				result_2 *= -1LL;
			do_div(result_2, pSmart->vregout_voltage);
			str[offset] = (int)(sign * result_2) - v_coeff[volt].numerator;
		}
	} else if (volt == VOLT_V3) {
		for (offset = R_OFFSET; offset <= B_OFFSET; offset++) {
			result_1 = (long long)pSmart->vregout_voltage -
				(long long)((int *)&pSmart->GRAY.TABLE[index[volt]])[offset];
			result_2 = result_1 * v_coeff[volt].denominator;
			result_3 = (long long)(pSmart->vregout_voltage) -
				(long long)((int *)&pSmart->GRAY.TABLE[index[volt + 1]])[offset];
			sign = ((result_2 < 0 && result_3 > 0) || (result_2 > 0 && result_3 < 0)) ? -1 : 1;
			if (result_2 < 0)
				result_2 *= -1LL;
			if (result_3 < 0)
				result_3 *= -1LL;
		...
		for (offset = R_OFFSET; offset <= B_OFFSET; offset++) {
			result_1 = (long long)((int *)(&pSmart->GRAY.VT_TABLE))[offset] -
				(long long)((int *)&pSmart->GRAY.TABLE[index[volt]])[offset];
			result_2 = result_1 * v_coeff[volt].denominator;
			result_3 = (long long)((int *)(&pSmart->GRAY.VT_TABLE))[offset] -
				(long long)((int *)&pSmart->GRAY.TABLE[index[volt + 1]])[offset];
			sign = ((result_2 < 0 && result_3 > 0) || (result_2 > 0 && result_3 < 0)) ? -1 : 1;
			if (result_2 < 0)
				result_2 *= -1LL;
...
static void gamma_init(struct SMART_DIM *pSmart, char *dest, int size)
{
	...
	int i, order[] = {
		VOLT_V255, VOLT_VT, VOLT_V3,
		VOLT_V11, VOLT_V23, VOLT_V35,
		VOLT_V51, VOLT_V87, VOLT_V151,
		VOLT_V203
	};
	...
	for (i = 0; i < ARRAY_SIZE(order); i++) {
		volt = order[i];
		if (volt == VOLT_V255)
			candela_level[volt] = (long long)bl_level << BIT_SHIFT;
		else if (volt == VOLT_VT)
			candela_level[volt] = (brightness < 251) ? 0 :
				((long long)(gamma_curve_table[VT_CURVE[volt]]) *
				 (long long)(candela_level[VOLT_V255])) >> BIT_SHIFT;
		else
			candela_level[volt] =
				((long long)(gamma_curve_table[VT_CURVE[volt]]) *
				 (long long)(candela_level[VOLT_V255])) >> BIT_SHIFT;
	...
	for (volt = VOLT_V3; volt < VOLT_MAX; volt++)
		for (offset = R_OFFSET; offset <= B_OFFSET; offset++) {
			int upper = (volt == VOLT_V255) ? 511 : 255;
			gamma[volt][offset] -= MTP[volt][offset];

			if (gamma[volt][offset] < 0)
				gamma[volt][offset] = 0;
			if (gamma[volt][offset] > upper)
				gamma[volt][offset] = upper;
		}
