		...
		int brightness)
{
	...
	if (brightness == range[BRT_VALUE_HBM].brightness)
...
static int gen_panel_backlight_update_status(struct backlight_device *bd)
{
	...
	if (bd->props.power != FB_BLANK_UNBLANK ||
		...
		!atomic_read(&bl_info->enable)*/) {
		/*brightness = 0;*/
		pr_info("%s, %s, %s, do not update!%d\n", __func__,
		(bd->props.power != FB_BLANK_UNBLANK) ? "blank" : "unblank",
		(bd->props.fb_blank != FB_BLANK_UNBLANK) ? "blank" : "unblank",
		/*!atomic_read(&bl_info->enable) ? "disable" : "enable",*/
		brightness);
		return 0;
	}
			...
			unsigned long event, void *data)
{
	...
	struct gen_panel_backlight_info *bl;
	int blank;
	...
	bl = container_of(self, struct gen_panel_backlight_info, fb_notifier);
...
static int gen_panel_backlight_probe(struct platform_device *pdev)
{
	...
	/*For dynamic auto brightness control of high bright mode support*/
	props.max_brightness = bl_info->range[BRT_VALUE_HBM].brightness;
	...
	bl_info->fb_notifier.notifier_call = fb_bl_notifier_callback;
	fb_register_client(&bl_info->fb_notifier);

	bl_info->bl_on = true;

	atomic_set(&bl_info->enable, 1);
...
const struct dev_pm_ops gen_panel_backlight_pm_ops = {
	...
	SET_SYSTEM_SLEEP_PM_OPS(gen_panel_backlight_runtime_suspend,
		gen_panel_backlight_runtime_resume)
};

static int gen_panel_backlight_suspend(struct platform_device *dev,
						pm_message_t state)
{
	pr_info("[LCD] %s \n" , __func__);
...
static int gen_panel_backlight_resume(struct platform_device *dev)
{
	pr_info("[LCD] %s \n" , __func__);
...
static struct platform_driver gen_panel_backlight_driver = {
	...
	.suspend	= gen_panel_backlight_suspend,
	.resume		= gen_panel_backlight_resume,
