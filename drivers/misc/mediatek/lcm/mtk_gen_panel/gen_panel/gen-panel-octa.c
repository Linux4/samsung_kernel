/* mediatek/lcm/mtk_gen_panel/gen_panel/gen-panel-octa.c
 *
...
#ifdef CONFIG_GEN_PANEL_DYNAMIC_AID
#ifdef USE_DIMMING_CORE
static bool dim_info_initialized;
...
static int gen_panel_set_brightness(void *bd_data, int intensity)
{
	...
	gen_panel_write_op_cmds(lcd, PANEL_NV_ENABLE_CMD);
	if (IS_HBM(lcd->hbm) && op_cmd_valid(lcd, PANEL_HBM_ON)) {
		/*lcd->hbm = 1;*/
...
/*this code is re-checked for pass of lcd id from bootloader side*/
	...
	if ((!lcd->id) && (!is_fac))
		pr_info("%s, PBA function test & ID = [0x%x].\n",
			__func__, lcd->id);
	else {/*FACTORY Test*/
...
size_t gen_panel_read_ldi_info(struct lcd *lcd)
{
	...
	if (read_done_from_boot) {
		...
		/* read OCTA cell ID */
		gen_panel_read_reg1(lcd, date, octa_product_reg,
							product_reg_gpara, 7);
		gen_panel_read_reg1(lcd, coordinate, octa_coordinate_reg,
							coordinate_gpara, 4);
		if (elvss_temp_table.tbl) {
			u8 temp_data;
...
static int gen_panel_dimming_info_init(struct dimming_info *dim_info)
{
	...
	for (i = 0; i < dim_info->nr_tp; i++) {
		dim_info->mtp.v_coeff[i].numerator =
					((int (*)[2])v_coeff_table.tbl)[i][0];
		dim_info->mtp.v_coeff[i].denominator =
					((int (*)[2])v_coeff_table.tbl)[i][1];
	}
