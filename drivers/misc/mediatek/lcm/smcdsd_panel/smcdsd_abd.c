...
static struct mipi_dsi_lcd_parent *find_container(void)
{
	struct platform_device *pdev = NULL;
	struct mipi_dsi_lcd_parent *container = NULL;
	...
	container = dev_get_platdata(&pdev->dev);
	if (!container) {
...
static inline struct mipi_dsi_lcd_parent *get_abd_container_of(struct abd_protect *abd)
{
	struct mipi_dsi_lcd_parent *container = container_of(abd, struct mipi_dsi_lcd_parent, abd);
...
{
	struct mipi_dsi_lcd_parent *container = get_abd_container_of(abd);

	container->lcdconnected = flag;
}
...
{
	struct mipi_dsi_lcd_parent *container = get_smcdsd_drvdata(0);
...
static int _smcdsd_abd_fb_blank(struct fb_info *info, int blank)
{
	...
	fbblank = FB_BLANK_POWERDOWN;
	evdata.info = info;
	evdata.data = &fbblank;

	info->flags |= FBINFO_MISC_USEREVENT;

	smcdsd_notifier_call_chain(FB_EARLY_EVENT_BLANK, &evdata);
...
static int smcdsd_abd_dummy_fb_blank(int blank_mode, struct fb_info *info)
{
	/* dummy for just return 0 to prevent notifiy stop */
...
static int smcdsd_abd_con_set_dummy_blank(struct abd_protect *abd, struct fb_info *fbinfo, unsigned int dummy)
{
	int ret = NOTIFY_DONE;

	if (dummy) {
		static struct LCM_DRIVER dummy_lcm_driver;

		memcpy(lcm_driver_list[0], &dummy_lcm_driver, sizeof(struct LCM_DRIVER));
		fbinfo->fbops->fb_blank = smcdsd_abd_dummy_fb_blank;
		ret = NOTIFY_DONE;
	} else {
		_smcdsd_abd_pin_enable(abd, &abd->pin[ABD_PIN_CON], 0);
		memcpy(lcm_driver_list[0], abd->lcm_drv, sizeof(struct LCM_DRIVER));
...
static void smcdsd_abd_con_prepare_dummy_info(struct abd_protect *abd)
{
	...
	abd->lcm_drv = kzalloc(sizeof(struct LCM_DRIVER), GFP_KERNEL);
	memcpy(abd->lcm_drv, lcm_driver_list[0], sizeof(struct LCM_DRIVER));
}
#endif

static int smcdsd_abd_con_fb_blank(struct abd_protect *abd)
{
	struct fb_info *fbinfo = get_fbinfo(abd);
	...
	smcdsd_abd_con_set_dummy_blank(abd, fbinfo, 1);

	//_smcdsd_abd_pin_enable(abd, &abd->pin[ABD_PIN_CON], 1);
			...
			unsigned long event, void *data)
{
	...
	if (fb_blank == FB_BLANK_UNBLANK && event == FB_EARLY_EVENT_BLANK) {
		...
		if (gpio_active > 0)
			return smcdsd_abd_con_set_dummy_blank(abd, evdata->info, 1);
		else
			return smcdsd_abd_con_set_dummy_blank(abd, evdata->info, 0);
	}
...
{
	struct mipi_dsi_lcd_parent *container = find_container();
	struct abd_protect *abd = NULL;
