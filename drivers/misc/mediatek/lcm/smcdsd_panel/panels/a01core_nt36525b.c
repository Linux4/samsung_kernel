...
#define CONFIG_SMCDSD_PROTOS_PLUS	/* this is not defconfig */
...
extern int protos_state;
extern int primary_display_set_aod(int enter_suspend);
#endif
...
static int smcdsd_set_doze(int mode)
{
	int ret = 0;

	dbg_info("%s + doze_mode: %s\n", __func__, mode ? "DOZE_SUSPEND" : "DOZE");

	if (mode == MTKFB_AOD_DOZE)
		primary_display_set_aod(0);
	else if (mode == MTKFB_AOD_DOZE_SUSPEND)
		primary_display_set_aod(1);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	dev_info(&lcd->ld->dev, "%s: [%s(%d)] => [%s(%d)] ++\n",
		__func__,
		protos_state_names[protos_state], protos_state,
		protos_state_names[value], value);

	smcdsd_abd_save_str(&pdata->abd, protos_state_names[value]);

	if (protos_state == value) {
	...
	/* keep prev state */
	prev_protos_state = protos_state;
	/* set target state */
	protos_state = value;
	...
	case PROTOS_AWAKE:
		...
		smcdsd_set_doze(MTKFB_AOD_DOZE);
	...
	case PROTOS_SLEEP:
		...
		smcdsd_set_doze(MTKFB_AOD_DOZE_SUSPEND);
...
#if defined(CONFIG_SMCDSD_PROTOS_PLUS)
static DEVICE_ATTR_RW(protos_control);
#endif
...
static int __smcdsd_panel_initalpm(struct platform_device *p)
{
	struct lcd_info *lcd = get_lcd_info(p);

	dev_info(&lcd->ld->dev, "+ %s: %d  %d\n", __func__, lcd->state, protos_state);

	if (protos_state != PROTOS_SLEEP)
		DSI_WRITE(SEQ_DISPLAY_OFF, ARRAY_SIZE(SEQ_DISPLAY_OFF));
...
static int __smcdsd_panel_exitalpm(struct platform_device *p)
{
	struct lcd_info *lcd = get_lcd_info(p);
...
static int smcdsd_panel_initalpm(struct platform_device *p, unsigned int on)
{
	...
	if (on)
		__smcdsd_panel_initalpm(p);
	else
		__smcdsd_panel_exitalpm(p);
