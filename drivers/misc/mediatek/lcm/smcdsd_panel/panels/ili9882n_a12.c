...
{	\
	int tx_ret = 0;	\
	tx_ret = smcdsd_dsi_tx_data(lcd, cmd, size, false);		\
	if (tx_ret < 0)							\
		dev_info(&lcd->ld->dev, "%s: tx_ret(%d) failed to write %02x %s\n", __func__, tx_ret, cmd[0], #cmd);	\
}	\
} while (0)

#define DSI_WRITE_LOCK(cmd, size)	do {				\
{	\
	int tx_ret = 0;	\
	tx_ret = smcdsd_dsi_tx_data(lcd, cmd, size, true);		\
	if (tx_ret < 0)							\
		dev_info(&lcd->ld->dev, "%s: tx_ret(%d) failed to write %02x %s\n", __func__, tx_ret, cmd[0], #cmd);	\
}	\
} while (0)
...
static int smcdsd_dsi_tx_data(struct lcd_info *lcd, u8 *cmd, u32 len, bool need_lock)
{
	int ret = 0;
	int retry = 2;
...
try_write:
	if (len == 1)
		ret = lcd->pdata->tx(lcd->pdata, MIPI_DSI_DCS_SHORT_WRITE, cmd[0], 0, need_lock);
	else if (len == 2)
		ret = lcd->pdata->tx(lcd->pdata, MIPI_DSI_DCS_SHORT_WRITE_PARAM, cmd[0], cmd[1], need_lock);
	else
		ret = lcd->pdata->tx(lcd->pdata, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)cmd, len, need_lock);
...
static int smcdsd_panel_set_brightness(struct lcd_info *lcd, int force)
{
	...
	/*
	 * ILI9882N 51h description (8bit PWM)
	 * 1st parameter D[7-4] : Don't care
	 * 1st parameter D[3-0] : Brightness level D[7-4]
	 * 2nd parameter D[7-4] : Brightness level D[3-0]
	 * 2nd parameter D[3-0] : Don't care
	 */

	bl_reg[0] = LDI_REG_BRIGHTNESS;
	bl_reg[1] = get_bit(brightness_table[lcd->brightness], 4, 4);
	bl_reg[2] = get_bit(brightness_table[lcd->brightness], 0, 4) << 4;

	DSI_WRITE_LOCK(bl_reg, ARRAY_SIZE(bl_reg));
...
static int ili9882n_read_id(struct lcd_info *lcd, bool need_lock)
{
	int i, ret = 0;
	...
	for (i = 0; i < LDI_LEN_ID; i++) {
		ret = smcdsd_dsi_rx_info(lcd, LDI_REG_ID + i, 1, &lcd->id_info.id[i], need_lock);
		if (ret < 0)
			break;
	}
...
static int ili9882n_read_init_info(struct lcd_info *lcd, bool need_lock)
{
	int ret = 0;
	...
	ili9882n_read_id(lcd, need_lock);

	return ret;
}

static int ili9882n_exit(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	DSI_WRITE(SEQ_ILI9882N_DISPLAY_OFF, ARRAY_SIZE(SEQ_ILI9882N_DISPLAY_OFF));

	msleep(20);

	DSI_WRITE(SEQ_ILI9882N_SLEEP_IN, ARRAY_SIZE(SEQ_ILI9882N_SLEEP_IN));

	return ret;
}

static int ili9882n_init(struct lcd_info *lcd)
{
	int ret = 0;
	unsigned int i;
	...
	ili9882n_read_init_info(lcd, false);

	for (i = 0; i < (u32)ARRAY_SIZE(LCD_SEQ_INIT_1); i++) {
		if (LCD_SEQ_INIT_1[i].cmd)
			DSI_WRITE(LCD_SEQ_INIT_1[i].cmd, LCD_SEQ_INIT_1[i].len);

		if (LCD_SEQ_INIT_1[i].sleep)
			usleep_range(LCD_SEQ_INIT_1[i].sleep, (LCD_SEQ_INIT_1[i].sleep) + (LCD_SEQ_INIT_1[i].sleep >> 1));
	}

	DSI_WRITE(SEQ_ILI9882N_SLEEP_OUT, ARRAY_SIZE(SEQ_ILI9882N_SLEEP_OUT));
	msleep(80);
	DSI_WRITE(SEQ_ILI9882N_DISPLAY_ON, ARRAY_SIZE(SEQ_ILI9882N_DISPLAY_ON));
	DSI_WRITE(SEQ_ILI9882N_BRIGHTNESS_MODE, ARRAY_SIZE(SEQ_ILI9882N_BRIGHTNESS_MODE));
...
static int ili9882n_register_notifier(struct lcd_info *lcd)
{
...
static int ili9882n_probe(struct lcd_info *lcd)
{
	dev_info(&lcd->ld->dev, "+ %s\n", __func__);
	...
	ili9882n_read_init_info(lcd, true);
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	ili9882n_register_notifier(lcd);
...
struct mipi_dsi_lcd_driver ili9882n_mipi_lcd_driver = {
	.driver = {
		.name = "ili9882n",
	},
...
__XX_ADD_LCD_DRIVER(ili9882n_mipi_lcd_driver);
