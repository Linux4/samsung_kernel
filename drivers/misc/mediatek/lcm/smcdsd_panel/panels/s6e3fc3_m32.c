...
#include "mdnie.h"
#include "s6e3fc3_m32_mdnie.h"
#endif

#include "s6e3fc3_m32_param.h"
#include "s6e3fc3_m32_selfmask.h"
#if defined(CONFIG_SMCDSD_DYNAMIC_MIPI)
#include "s6e3fc3_m32_freq.h"
#endif
...
static int s6e3fc3_change_ffc(struct lcd_info *lcd, unsigned int ffc_on)
{
	...
	if (ffc_on) {
		if (lcd->df_index_req == DYNAMIC_MIPI_INDEX_1)
			smcdsd_dsi_tx_set(lcd, LCD_SEQ_FFC_1730, ARRAY_SIZE(LCD_SEQ_FFC_1730));
		else if (lcd->df_index_req == DYNAMIC_MIPI_INDEX_2)
			smcdsd_dsi_tx_set(lcd, LCD_SEQ_FFC_1680, ARRAY_SIZE(LCD_SEQ_FFC_1680));
		else
			smcdsd_dsi_tx_set(lcd, LCD_SEQ_FFC_1710, ARRAY_SIZE(LCD_SEQ_FFC_1710));
