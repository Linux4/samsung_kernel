...
#include "a13ve_nt36672c_tianma_param.h"
#include "dd.h"
...
static int nt36672c_tianma_read_id(struct lcd_info *lcd)
{
	int i = 0, ret = 0;
...
static int nt36672c_tianma_read_init_info(struct lcd_info *lcd)
{
	int ret = 0;
	...
	nt36672c_tianma_read_id(lcd);

	return ret;
}

static int nt36672c_tianma_exit(struct lcd_info *lcd)
{
	int ret = 0;
...
static int nt36672c_tianma_init(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	nt36672c_tianma_read_init_info(lcd);
...
static int nt36672c_tianma_register_notifier(struct lcd_info *lcd)
{
...
static int nt36672c_tianma_probe(struct lcd_info *lcd)
{
	dev_info(&lcd->ld->dev, "+ %s\n", __func__);
...
static void lcd_init_sysfs(struct lcd_info *lcd)
{
	...
	init_debugfs_param("blic_init", &KTZ8864_INIT, U8_MAX, ARRAY_SIZE(KTZ8864_INIT), 3);
	init_debugfs_param("blic_exit", &KTZ8864_EXIT, U8_MAX, ARRAY_SIZE(KTZ8864_EXIT), 3);
}
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	nt36672c_tianma_probe(lcd);

	nt36672c_tianma_register_notifier(lcd);
...
struct mipi_dsi_lcd_driver nt36672c_tianma_mipi_lcd_driver = {
	.driver = {
...
__XX_ADD_LCD_DRIVER(nt36672c_tianma_mipi_lcd_driver);
