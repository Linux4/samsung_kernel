...
static int smcdsd_panel_set_brightness(struct lcd_info *lcd, int force)
{
	...
	/*
	 * ILI7806S 51h description (8bit PWM)
...
static int ili7806s_read_id(struct lcd_info *lcd, bool need_lock)
{
	int i, ret = 0;
...
static int ili7806s_read_init_info(struct lcd_info *lcd, bool need_lock)
{
	int ret = 0;
	...
	ili7806s_read_id(lcd, need_lock);

	return ret;
}

static int ili7806s_exit(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	DSI_WRITE(SEQ_ILI7806S_DISPLAY_OFF, ARRAY_SIZE(SEQ_ILI7806S_DISPLAY_OFF));
	msleep(20);
	DSI_WRITE(SEQ_ILI7806S_SLEEP_IN, ARRAY_SIZE(SEQ_ILI7806S_SLEEP_IN));

	return ret;
}

static int ili7806s_init(struct lcd_info *lcd)
{
	int ret = 0;
	unsigned int i;
	...
	ili7806s_read_init_info(lcd, false);
	...
	DSI_WRITE(SEQ_ILI7806S_SLEEP_OUT, ARRAY_SIZE(SEQ_ILI7806S_SLEEP_OUT));
	msleep(60);
	DSI_WRITE(SEQ_ILI7806S_DISPLAY_ON, ARRAY_SIZE(SEQ_ILI7806S_DISPLAY_ON));
	DSI_WRITE(SEQ_ILI7806S_BRIGHTNESS_MODE, ARRAY_SIZE(SEQ_ILI7806S_BRIGHTNESS_MODE));
	msleep(50);
...
static int ili7806s_register_notifier(struct lcd_info *lcd)
{
...
static int ili7806s_probe(struct lcd_info *lcd)
{
	dev_info(&lcd->ld->dev, "+ %s\n", __func__);
	...
	ili7806s_read_init_info(lcd, true);
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	ili7806s_register_notifier(lcd);
...
struct mipi_dsi_lcd_driver ili7806s_mipi_lcd_driver = {
	.driver = {
		.name = "ili7806s",
	},
...
__XX_ADD_LCD_DRIVER(ili7806s_mipi_lcd_driver);
