...
#include "nt36525b_dtc_a12_param.h"
#include "dd.h"
...
static int nt36525b_dtc_read_id(struct lcd_info *lcd, bool need_lock)
{
	int i, ret = 0;
...
static int nt36525b_dtc_read_init_info(struct lcd_info *lcd, bool need_lock)
{
	int ret = 0;
	...
	nt36525b_dtc_read_id(lcd, need_lock);

	return ret;
}

static int nt36525b_dtc_exit(struct lcd_info *lcd)
{
	int ret = 0;
	...
	DSI_WRITE(SEQ_NT36525B_DTC_BACK_TO_PAGE_1_A, ARRAY_SIZE(SEQ_NT36525B_DTC_BACK_TO_PAGE_1_A));
	DSI_WRITE(SEQ_NT36525B_DTC_BACK_TO_PAGE_1_B, ARRAY_SIZE(SEQ_NT36525B_DTC_BACK_TO_PAGE_1_B));

	DSI_WRITE(SEQ_NT36525B_DTC_DISPLAY_OFF, ARRAY_SIZE(SEQ_NT36525B_DTC_DISPLAY_OFF));

	msleep(20);

	DSI_WRITE(SEQ_NT36525B_DTC_SLEEP_IN, ARRAY_SIZE(SEQ_NT36525B_DTC_SLEEP_IN));

	return ret;
}

static int nt36525b_dtc_init(struct lcd_info *lcd)
{
	int ret = 0;
	unsigned int i;
	...
	nt36525b_dtc_read_init_info(lcd, false);
	...
	DSI_WRITE(SEQ_NT36525B_DTC_BRIGHTNESS, ARRAY_SIZE(SEQ_NT36525B_DTC_BRIGHTNESS));

	DSI_WRITE(SEQ_NT36525B_DTC_SLEEP_OUT, ARRAY_SIZE(SEQ_NT36525B_DTC_SLEEP_OUT));
	msleep(100);	/* Wait Min. 6 frame */
	DSI_WRITE(SEQ_NT36525B_DTC_DISPLAY_ON, ARRAY_SIZE(SEQ_NT36525B_DTC_DISPLAY_ON));
...
static int nt36525b_dtc_register_notifier(struct lcd_info *lcd)
{
...
static int nt36525b_dtc_probe(struct lcd_info *lcd)
{
	dev_info(&lcd->ld->dev, "+ %s\n", __func__);
	...
	nt36525b_dtc_read_init_info(lcd, true);
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	nt36525b_dtc_probe(lcd);

	nt36525b_dtc_register_notifier(lcd);
...
struct mipi_dsi_lcd_driver nt36525b_dtc_mipi_lcd_driver = {
	.driver = {
		.name = "nt36525b_dtc",
	},
...
__XX_ADD_LCD_DRIVER(nt36525b_dtc_mipi_lcd_driver);
