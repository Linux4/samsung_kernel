...
#include "a02_ili9882q_boe_param.h"
#include "dd.h"
...
static int smcdsd_panel_set_brightness(struct lcd_info *lcd, int force)
{
	...
	/*
	 * ILI9882Q_BOE 51h description (8bit PWM)
...
static int ili9882q_boe_read_init_info(struct lcd_info *lcd)
{
	int ret = 0;
	...
	ili9882q_boe_read_id(lcd);

	return ret;
}

static int ili9882q_boe_exit(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	DSI_WRITE(SEQ_ILI9882Q_BOE_DISPLAY_OFF, ARRAY_SIZE(SEQ_ILI9882Q_BOE_DISPLAY_OFF));
	msleep(20);
	DSI_WRITE(SEQ_ILI9882Q_BOE_SLEEP_IN, ARRAY_SIZE(SEQ_ILI9882Q_BOE_SLEEP_IN));

	return ret;
}

static int ili9882q_boe_init(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	ili9882q_boe_read_init_info(lcd);

	smcdsd_dsi_tx_set(lcd, LCD_SEQ_INIT_1, ARRAY_SIZE(LCD_SEQ_INIT_1));

	DSI_WRITE(SEQ_ILI9882Q_BOE_SLEEP_OUT, ARRAY_SIZE(SEQ_ILI9882Q_BOE_SLEEP_OUT));
	msleep(60);
	DSI_WRITE(SEQ_ILI9882Q_BOE_DISPLAY_ON, ARRAY_SIZE(SEQ_ILI9882Q_BOE_DISPLAY_ON));
	msleep(50);
...
static int ili9882q_boe_register_notifier(struct lcd_info *lcd)
{
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	ili9882q_boe_probe(lcd);

	ili9882q_boe_register_notifier(lcd);
...
struct mipi_dsi_lcd_driver ili9882q_boe_mipi_lcd_driver = {
	.driver = {
		.name = "ili9882q_boe",
	},
...
__XX_ADD_LCD_DRIVER(ili9882q_boe_mipi_lcd_driver);
