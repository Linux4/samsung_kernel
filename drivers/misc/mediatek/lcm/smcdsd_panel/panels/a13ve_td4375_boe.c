...
#include "a13ve_td4375_boe_param.h"
#include "dd.h"
...
static int td4375_boe_read_id(struct lcd_info *lcd)
{
	int i = 0, ret = 0;
...
static int td4375_boe_read_init_info(struct lcd_info *lcd)
{
	int ret = 0;
	...
	td4375_boe_read_id(lcd);

	return ret;
}

static int td4375_boe_exit(struct lcd_info *lcd)
{
	int ret = 0;
...
static int td4375_boe_init(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	td4375_boe_read_init_info(lcd);
...
static int td4375_boe_register_notifier(struct lcd_info *lcd)
{
...
static int td4375_boe_probe(struct lcd_info *lcd)
{
	dev_info(&lcd->ld->dev, "+ %s\n", __func__);
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	td4375_boe_probe(lcd);

	td4375_boe_register_notifier(lcd);
...
struct mipi_dsi_lcd_driver td4375_boe_mipi_lcd_driver = {
	.driver = {
		.name = "td4375_boe",
	},
...
__XX_ADD_LCD_DRIVER(td4375_boe_mipi_lcd_driver);
