...
#include "a13ve_hx83112f_boe_param.h"
#include "dd.h"
...
static int hx83112f_boe_read_id(struct lcd_info *lcd)
{
	int i = 0, ret = 0;
...
static int hx83112f_boe_read_init_info(struct lcd_info *lcd)
{
	int ret = 0;
	...
	hx83112f_boe_read_id(lcd);

	return ret;
}

static int hx83112f_boe_exit(struct lcd_info *lcd)
{
	int ret = 0;
...
static int hx83112f_boe_init(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	hx83112f_boe_read_init_info(lcd);
...
static int hx83112f_boe_register_notifier(struct lcd_info *lcd)
{
...
static int hx83112f_boe_probe(struct lcd_info *lcd)
{
	dev_info(&lcd->ld->dev, "+ %s\n", __func__);
...
static int smcdsd_panel_probe(struct platform_device *p)
{
	...
	hx83112f_boe_probe(lcd);

	hx83112f_boe_register_notifier(lcd);
...
struct mipi_dsi_lcd_driver hx83112f_boe_mipi_lcd_driver = {
	.driver = {
		.name = "hx83112f_boe",
	},
...
__XX_ADD_LCD_DRIVER(hx83112f_boe_mipi_lcd_driver);
