...
static int smcdsd_panel_get_params_from_dt(struct LCM_PARAMS *lcm_params)
{
	...
	np = of_find_node_with_property(NULL, PANEL_DTS_NAME);
	if (!np) {
		dbg_info("%s property does not exist, so create dummy\n", PANEL_DTS_NAME);
		return -EINVAL;
	}

	np = of_parse_phandle(np, PANEL_DTS_NAME, 0);
	if (!np) {
		dbg_info("%s does not exist, so create dummy\n", PANEL_DTS_NAME);
		return -EINVAL;
	}
...
static void smcdsd_panel_get_params(struct LCM_PARAMS *params)
{
	...
	params->dsi.vertical_active_line = params->dsi.vertical_active_line ? params->dsi.vertical_active_line : params->width;
	params->dsi.horizontal_active_pixel = params->dsi.horizontal_active_pixel ? params->dsi.horizontal_active_pixel : params->height;
...
static void smcdsd_panel_resume_power(void)
{
	...
	plcd->power = 1;
	run_list(&plcd->pdev->dev, "panel_power_enable");
	run_list(&plcd->pdev->dev, "panel_reset");
}
...
int smcdsd_panel_dsi_command_rx(unsigned int id, unsigned int offset, u8 cmd, unsigned int len, u8 *buf)
{
	...
	read_offset = offset;
	if (plcd->cmd_q)
		ret = primary_display_dsi_read_cmdq_cmd(id, offset, cmd, buf, len);
	else
...
static int smcdsd_panel_send_command_queue(unsigned int *pdata,
			unsigned int queue_size, unsigned char force_update)
{
	struct mipi_dsi_lcd_parent *plcd = get_smcdsd_drvdata(0);
	int ret = 0;
 ...
 *	=> For fod mask scenario.
 */

	if (plcd->cmd_q) {
		if (plcd->cmdq_index == CMDQ_PRIMARY_DISPLAY) {
			primary_display_dsi_set_withrawcmdq(pdata, queue_size, force_update);
		} else if (plcd->cmdq_index == CMDQ_MASK_BRIGHTNESS) {
			pr_warn("[CMDQ_MASK_BRIGHTNESS] %s\n", __func__);
			mtk_dsi_set_withrawcmdq(plcd->mask_qhandle, pdata, queue_size, force_update);
		} else {
			pr_err("%s abnormal CMDQ index!");
			primary_display_dsi_set_withrawcmdq(pdata, queue_size, force_update);
		}
	} else
...
int smcdsd_panel_dsi_command_tx(unsigned int id, unsigned long data0, unsigned int data1)
{
	...
	case MIPI_DSI_DCS_SHORT_WRITE_PARAM:
	{
		...
		data[2] = data0;
		data[3] = data1;
		ret = smcdsd_panel_send_command_queue((unsigned int *)data, 1, 1);
		break;
	}
	...
	case MIPI_DSI_DCS_LONG_WRITE:
	{
		...
		ret = smcdsd_panel_send_command_queue((unsigned int *)data, (data1 + 7) >> 2, 1);
		break;
	}
...
static int __init smcdsd_probe(struct platform_device *pdev)
{
