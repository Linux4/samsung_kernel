...
//* +bug733919, yexiaojun.wt, ADD, 20220315, cite in thermal_core.c */
#ifdef CONFIG_WT_FLASH_THERMAL_FEATURE
static int notify_flag=0;
extern void fled_notify_add(void);
extern void fled_notify_remove(void);
extern int fled_return_value;
...
#define MTKTS_FLASH_SW_FILTER (0)
...
static int get_hw_flash_temp(void)
{
	...
	if (ret < 0) {
		mtkts_flash_printk("IIO channel read failed %d\n", ret);
		return ret;
	}
...
#define DELTA_TEMP 2000
	...
	if ((t_ret > 100000) || (t_ret < -30000)) {
		mtkts_flash_printk(
			"[Power/FLASH_Thermal] drop this data\n");
		t_ret = pre_temp1;
	} else if ((pre_temp1 != 0)
		&& (((pre_temp1 - t_ret) >= DELTA_TEMP)
			|| ((t_ret - pre_temp1) >= DELTA_TEMP))) {
		mtkts_flash_dprintk(
			"[Power/FLASH_Thermal] t_ret=%d,(t_ret-pre_temp1)=%d,(pre_temp1-t_ret)=%d\n",
			t_ret, (t_ret - pre_temp1), (pre_temp1 - t_ret));
		t_ret = pre_temp1;
	} else {
		/* update previous temp */
		pre_temp1 = t_ret;
		mtkts_flash_dprintk("[Power/FLASH_Thermal] pre_temp1=%d\n",
								pre_temp1);
	}
#endif
...
int mtkts_flash_get_temp_bootstage(void)
{
	...
	if (temp >= threshold_val) {
		fled_notify_add();
		printk("[EveD][T][%s] FlashLED temp is to high, need to turn off FLED! Temp = %d\n", __func__, temp);
	} 
	else {
		fled_notify_remove();
...
int mtkts_flash_get_temp_new(void)
{
	int t = mtkts_flash_get_hw_temp();
	if (t > 52000)
		mtkts_flash_dprintk("T=%d\n", t);

	/* +bug733919, yexiaojun.wt, ADD, 20220315, flashled temp notify to framework */
	#ifdef CONFIG_WT_FLASH_THERMAL_FEATURE
  	if (t >= threshold_val) {
		//printk("[EveD]mtkts_flash_get_temp:notify_flag=%d\n", notify_flag);
  	    fled_return_value = 1;
  		if (notify_flag == 0) {
  
  			fled_notify_add();
  			notify_flag = 1;
			printk("[EveD][T]FlashLED temp is to high, need to turn off FLED! Temp = %d\n", t);
  		}
  	} else {
  
  	        fled_return_value = 0;
			if (t >= (threshold_val-5000) && t <= (threshold_val-3000) && notify_flag == 1) {
  			fled_notify_remove();
  			notify_flag = 0; 
			printk("[EveD][T]FlashLED temp cool down, FLED could be turned on! Temp = %d\n", t);
  		}
  	}
	#endif
	/* -bug733919, yexiaojun.wt, ADD, 20220315, flashled temp notify to framework */
...
static int mtkts_flash_get_mode(struct thermal_zone_device *thermal, 
...
static int mtkts_flash_set_mode(struct thermal_zone_device *thermal, 
            enum thermal_device_mode mode)
{
	kernelmode = mode;
	return 0;
...
static int mtkts_flash_get_trip_type(
...
static int mtkts_flash_get_trip_temp(
...
static struct thermal_zone_device_ops mtkts_flash_dev_ops = {
	...
	.get_mode = mtkts_flash_get_mode,
	.set_mode = mtkts_flash_set_mode,
	.get_trip_type = mtkts_flash_get_trip_type,
	.get_trip_temp = mtkts_flash_get_trip_temp,
...
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf
		...
		&ptr_mtktsflash_parm_data->ntc_table,&adc_channel) >= 8) {
...
static int mtkts_flash_register_thermal(void)
{
	...
	/* trips : trip 0~1 */
	thz_dev = mtk_thermal_zone_device_register("mtktsflash", num_trip, NULL,
...
static const struct file_operations mtkts_flash_fops = {
	.owner = THIS_MODULE,
	.open = mtkts_flash_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = mtkts_flash_write,
...
static const struct file_operations mtkts_flash_param_fops = {
	.owner = THIS_MODULE,
	.open = mtkts_flash_param_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = mtkts_flash_param_write,
	.release = single_release,
};
...
static int mtkts_flash_probe(struct platform_device *pdev)
{
	...
	mtkts_flash_register_thermal();
	/* +bug733919, yexiaojun.wt, ADD, 20220315, S98121AA1 initaillize flag*/
	#ifdef CONFIG_WT_FLASH_THERMAL_FEATURE
	mtkts_flash_get_temp_bootstage();
	notify_flag = 0;
	mFlag = 1;
	#endif
	/* -bug733919, yexiaojun.wt, ADD, 20220315, S98121AA1 initaillize flag */
...
static int __init mtkts_flash_init(void)
{
...
static void __exit mtkts_flash_exit(void)
{
	mtkts_flash_dprintk("[%s]\n", __func__);
	mtkts_flash_unregister_thermal();
