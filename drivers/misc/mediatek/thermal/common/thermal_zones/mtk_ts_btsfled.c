...
static __s32 mtkts_btsfled_thermistor_conver_temp(__s32 Res)
{
	...
	if (Res >= BTSFLED_Temperature_Table[0].TemperatureR) {
		...
		BTSFLED_Temperature_Table[asize - 1].TemperatureR) {
		TAP_Value = 125;	/* max */
	} else {
		...
		for (i = 0; i < asize; i++) {
			...
			/* mtktsfled_dprintk("%d :i=%d, RES1 = %d,
...
static struct thermal_zone_device_ops mtktsfled_dev_ops = {
	.bind = mtktsfled_bind,
	.unbind = mtktsfled_unbind,
	.get_temp = mtktsfled_get_temp,
	.get_mode = mtktsfled_get_mode,
	.set_mode = mtktsfled_set_mode,
	.get_trip_type = mtktsfled_get_trip_type,
...
struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	if (cl_dev_sysrst_state == 1) {
		pr_notice("[Thermal/mtktsfled_sysrst] reset, reset, reset!!!\n");
		pr_notice("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
		pr_notice("*****************************************\n");
		pr_notice("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
...
static int mtktsfled_read(struct seq_file *m, void *v)
{
	seq_printf(m, "log=%d\n", mtktsfled_debug_log);
...
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(ptr_mtktsfled_data->desc,
		...
		&num_trip,
		&ptr_mtktsfled_data->trip[0],
		&ptr_mtktsfled_data->t_type[0], ptr_mtktsfled_data->bind0,
		&ptr_mtktsfled_data->trip[1],
		&ptr_mtktsfled_data->t_type[1], ptr_mtktsfled_data->bind1,
		&ptr_mtktsfled_data->trip[2],
		&ptr_mtktsfled_data->t_type[2], ptr_mtktsfled_data->bind2,
		&ptr_mtktsfled_data->trip[3],
		&ptr_mtktsfled_data->t_type[3], ptr_mtktsfled_data->bind3,
		&ptr_mtktsfled_data->trip[4],
		&ptr_mtktsfled_data->t_type[4], ptr_mtktsfled_data->bind4,
		&ptr_mtktsfled_data->trip[5],
		&ptr_mtktsfled_data->t_type[5], ptr_mtktsfled_data->bind5,
		&ptr_mtktsfled_data->trip[6],
		&ptr_mtktsfled_data->t_type[6], ptr_mtktsfled_data->bind6,
		&ptr_mtktsfled_data->trip[7],
		&ptr_mtktsfled_data->t_type[7], ptr_mtktsfled_data->bind7,
		&ptr_mtktsfled_data->trip[8],
		&ptr_mtktsfled_data->t_type[8], ptr_mtktsfled_data->bind8,
		&ptr_mtktsfled_data->trip[9],
		&ptr_mtktsfled_data->t_type[9], ptr_mtktsfled_data->bind9,
		&ptr_mtktsfled_data->time_msec) == 32) {
		down(&sem_mutex);
		...
		for (i = 0; i < 20; i++) {
			...
			g_bind2[i] = ptr_mtktsfled_data->bind2[i];
			g_bind3[i] = ptr_mtktsfled_data->bind3[i];
			g_bind4[i] = ptr_mtktsfled_data->bind4[i];
			g_bind5[i] = ptr_mtktsfled_data->bind5[i];
			g_bind6[i] = ptr_mtktsfled_data->bind6[i];
			g_bind7[i] = ptr_mtktsfled_data->bind7[i];
		...
		mtktsfled_dprintk("g_THERMAL_TRIP_3=%d g_THERMAL_TRIP_4=%d ",
...
{
	int len = 0;
	struct mtktsbtsfled_param_data {
		char desc[512];
	...
	if (sscanf
		...
		ptr_mtktsbtsfled_parm_data->pull_R,
		&ptr_mtktsbtsfled_parm_data->valR,
		ptr_mtktsbtsfled_parm_data->pull_V,
		&ptr_mtktsbtsfled_parm_data->valV,
		ptr_mtktsbtsfled_parm_data->overcrilow,
		&ptr_mtktsbtsfled_parm_data->over_cri_low,
		...
		if (!strcmp(ptr_mtktsbtsfled_parm_data->pull_V,
			...
			g_RAP_pull_up_voltage =
				ptr_mtktsbtsfled_parm_data->valV;
			mtktsfled_dprintk("g_Rat_pull_up_voltage=%d\n",
							g_RAP_pull_up_voltage);
		} else {
			mtktsfled_dprintk(
		...
		if (!strcmp(ptr_mtktsbtsfled_parm_data->overcrilow,
			...
			g_TAP_over_critical_low =
				ptr_mtktsbtsfled_parm_data->over_cri_low;
			mtktsfled_dprintk("g_TAP_over_critical_low=%d\n",
						g_TAP_over_critical_low);
		} else {
			mtktsfled_dprintk(
		...
		if (!strcmp(ptr_mtktsbtsfled_parm_data->NTC_TABLE,
			...
			g_RAP_ntc_table =
				ptr_mtktsbtsfled_parm_data->ntc_table;
			mtktsfled_dprintk("g_RAP_ntc_table=%d\n",
							g_RAP_ntc_table);
		} else {
			mtktsfled_dprintk(
		 ...
		 * choose "adc_channel=11" to check if there is any param input
		 */
		if ((ptr_mtktsbtsfled_parm_data->adc_channel >= 2)
		&& (ptr_mtktsbtsfled_parm_data->adc_channel <= 11))
			 ...
			 */
			if (ptr_mtktsbtsfled_parm_data->adc_channel != 11)
				g_RAP_ADC_channel =
				ptr_mtktsbtsfled_parm_data->adc_channel;
			else
				g_RAP_ADC_channel = AUX_IN2_NTC;
		}
		mtktsfled_dprintk("adc_channel=%d\n",
				ptr_mtktsbtsfled_parm_data->adc_channel);
...
static int mtktsfled_pdrv_probe(struct platform_device *pdev)
{
	...
	if (!mtktsfled_dir) {
		...
		entry = proc_create("tzfled", 0664, mtktsfled_dir,
							&mtktsfled_fops);
		if (entry)
...
static int __init mtktsfled_init(void)
{
	 ...
	 * in case mtktsfled reads temperature
	...
	if (err) {
		mtktsfled_dprintk("%s fail to reg driver\n", __func__);
		goto err_unreg;
	}
#else
	err = mtktsfled_register_thermal();
