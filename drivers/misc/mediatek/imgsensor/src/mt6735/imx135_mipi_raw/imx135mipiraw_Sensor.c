...
void IMX135MIPI_set_Video_IHDR(kal_uint16 IHDR_En)
{
	...
	if(imgsensor.current_fps >200) // default 30.0 fps
    {
		write_cmos_sensor(0x011E,0x18);//
		write_cmos_sensor(0x011F,0x00);//
		write_cmos_sensor(0x0301,0x05);//
 ...
 //   write_cmos_sensor(0x0341,(imgsensor_info.normal_video.framelength>>0)&0xFF);//
    ...
    write_cmos_sensor(0x0340,(imgsensor.frame_length>>8)&0xFF);//
    write_cmos_sensor(0x0341,(imgsensor.frame_length>>0)&0xFF);//
    write_cmos_sensor(0x0342,(imgsensor_info.normal_video.linelength>>8)&0xFF);//
    write_cmos_sensor(0x0343,(imgsensor_info.normal_video.linelength>>0)&0xFF);//
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
   ...
   if(imgsensor.current_fps >200) {
	    ...
	    imgsensor.frame_length = imgsensor_info.normal_video.framelength*imgsensor_info.normal_video.max_framerate/imgsensor.current_fps;
	    imgsensor.min_frame_length = imgsensor.frame_length;//imgsensor_info.normal_video.framelength;
    }else {
	    imgsensor.pclk = imgsensor_info.normal_video20.pclk;
	    imgsensor.line_length = imgsensor_info.normal_video20.linelength;
	    imgsensor.frame_length = imgsensor_info.normal_video20.framelength*imgsensor_info.normal_video20.max_framerate/imgsensor.current_fps;
	    imgsensor.min_frame_length = imgsensor.frame_length;//imgsensor_info.normal_video.framelength;
	}
