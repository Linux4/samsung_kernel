...
#define VOL_1000 1000000
	...
	if (On) {
		...
		if ((currSensorName && (0 == strcmp(currSensorName, "imx135mipiraw"))) ||
			...
			if (pinSetIdx == 0)            ISP_MCLK1_EN(1);
			else if (pinSetIdx == 1)       ISP_MCLK2_EN(1);
		...
		(0 == strcmp(SENSOR_DRVNAME_S5K5E3YX_MIPI_RAW, currSensorName)))
		{
			/* First Power Pin low and Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
		...
		/*********************************** IMX219 Power on************************************************/
			...
			mdelay(3);    // Interval between HOST and MCLK should be 2.5 ms

			// MCLK should be ahead of RESET. (500us margin is recommended)
			if (pinSetIdx == 0)            ISP_MCLK1_EN(1);
			else if (pinSetIdx == 1)       ISP_MCLK2_EN(1);

			mdelay(1);    // 500us margin
		...
		{
			if (pinSetIdx == 0)         ISP_MCLK1_EN(0);
			else if (pinSetIdx == 1)    ISP_MCLK2_EN(0);
