...
static int selective_read_region(u32 offset, BYTE *data, u16 i2c_id, u32 size)
{
	u8 lsc_data[2]={0,0}; 
	u16 slave_id = 0xB0;
	if(offset == 0x300)            //read LSC OTP
    {
		read_imx219_eeprom_size(slave_id, offset, lsc_data, 2);
	//	printk("lsc size: 0x%x, 0x%x \n", lsc_data[0], lsc_data[1]);
		if( size != ((lsc_data[1]<<8) + lsc_data[0]) )
		{
			return 0;
		}
	    read_imx219_eeprom_size(slave_id, offset+2, data, size);
		
   	}
	else if((offset == 0x210) || (offset == 0x140) || (offset == 0x10C))       //read AWB AF OTP 
    {	
		read_imx219_eeprom_size(slave_id, offset, data, size);	   	
   	}
	else{
...
* for sysfs
********************************************************************************/
static u8 otp_mfr_info[12]={0}; // manufacturer_information
static unsigned int otp_af_macro_cal = -1;
static unsigned int otp_af_pan_cal = -1;
...
static int update_sysfs_information(void)
{
	...
	u16 macro_cal_offset = 0x10C;
	u16 pan_cal_offset = 0x110;
#if defined (CONFIG_GET_REAR_MODULE_ID)
	u16 moduleId_offset = 0x50;
#endif
...
{
	return otp_af_macro_cal;
}
unsigned int get_imx219_af_pan_cal(void)
{
	return otp_af_pan_cal;
}
#if defined (CONFIG_GET_REAR_MODULE_ID)
