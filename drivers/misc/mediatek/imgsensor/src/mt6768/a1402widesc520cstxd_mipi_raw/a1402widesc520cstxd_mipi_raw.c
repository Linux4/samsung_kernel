...
#include "a1402widesc520cstxd_mipi_raw.h"

#define PFX "a1402widesc520cstxd_mipi_raw"
#define LOG_INF(format, args...)    \
	pr_info(PFX "[%s] " format, __func__, ##args)
unsigned char fusion_id_main[96];
unsigned char sn_main[96];
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = A1402WIDESC520CSTXD_SENSOR_ID,
...
{
 { 2592, 1944, 16, 252, 2560, 1440, 2560, 1440, 0000, 0000, 2560, 1440, 0000, 0000, 2560, 1440}, // Preview 2112*1558
 { 2592, 1944, 8, 6, 2576, 1932, 2576, 1932, 0000, 0000, 2576, 1932, 0000, 0000, 2576, 1932}, // capture 4206*3128
 { 2592, 1944, 8, 6, 2576, 1932, 2576, 1932, 0000, 0000, 2576, 1932, 0000, 0000, 2576, 1932}, // video
 { 2592, 1944, 16, 252, 2560, 1440, 1280, 720, 0000, 0000, 1280, 720, 0000, 0000, 1280, 720}, //hight speed video 
 { 2592, 1944, 16, 12, 2560, 1920, 1280, 960, 0000, 0000, 1280, 960, 0000, 0000, 1280, 960},// slim video
 { 2592, 1944, 496,  372, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0000, 0000, 1600, 1200},// custom1
 { 2592, 1944, 16, 252, 2560, 1440, 1280, 720, 0000, 0000, 1280, 720, 0000, 0000, 1280, 720}, //custom2
 { 2592, 1944, 16, 12, 2560, 1920, 1280, 960, 0000, 0000, 1280, 960, 0000, 0000, 1280, 960},//custom3
};
...
#define SC520CS_SENSOR_GAIN_MAX              (635 * SC520CS_SENSOR_GAIN_BASE / 10)
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	pr_debug("Exit! SC520CS_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, gain = 0x%x, reg_gain = %d\n",
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_sc520cs[] = {
0x0103,0x01,
0x36e9,0x80,
0x37f9,0x80,
0x36e9,0x23,
...
{
#if MULTI_WRITE
	sc520cs_table_write_cmos_sensor(
		addr_data_pair_init_sc520cs,
		sizeof(addr_data_pair_init_sc520cs) /
		sizeof(kal_uint16));
#else
	...
	write_cmos_sensor8(0x3393,0x18);
	write_cmos_sensor8(0x3394,0x28);
	...
	write_cmos_sensor8(0x3905,0x8c);
	write_cmos_sensor8(0x391d,0x04);
	write_cmos_sensor8(0x3926,0x20);
...
static void preview_setting(void)
{
#if MULTI_WRITE
	sc520cs_table_write_cmos_sensor(
		addr_data_pair_preview_sc520cs,
		sizeof(addr_data_pair_preview_sc520cs) /
		sizeof(kal_uint16));
#else
	...
	write_cmos_sensor8(0x3220,0x80);
...
kal_uint16 addr_data_pair_capture_30fps_sc520cs[] = {
0x0100,0x00,
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 300) {
	sc520cs_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_sc520cs,
		sizeof(addr_data_pair_capture_30fps_sc520cs) /
		sizeof(kal_uint16));
	} else {
	sc520cs_table_write_cmos_sensor(
		addr_data_pair_capture_fps_sc520cs,
		sizeof(addr_data_pair_capture_fps_sc520cs) /
		sizeof(kal_uint16));
	}
#else
	if (currefps == 300) {
		LOG_INF("capture_setting fps = 300\n");
	write_cmos_sensor8(0x301f,0x0f);
	mDELAY(10);
	write_cmos_sensor8(0x3200,0x00);
	write_cmos_sensor8(0x3201,0x00);
	write_cmos_sensor8(0x3202,0x00);
	write_cmos_sensor8(0x3203,0x06);
	write_cmos_sensor8(0x3204,0x0a);
	write_cmos_sensor8(0x3206,0x07);
	write_cmos_sensor8(0x3207,0xa1);
	write_cmos_sensor8(0x3208,0x0a);
	write_cmos_sensor8(0x3209,0x10);
	write_cmos_sensor8(0x320a,0x07);
	write_cmos_sensor8(0x320b,0x8c);
	write_cmos_sensor8(0x3210,0x00);
	write_cmos_sensor8(0x3211,0x0c);
	write_cmos_sensor8(0x3212,0x00);
	write_cmos_sensor8(0x3213,0x08);
	write_cmos_sensor8(0x3215,0x11);
	write_cmos_sensor8(0x3220,0x80);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_hs_video_sc520cs[] = {
0x0100,0x00,
...
static void hs_video_setting(void)
{
...
#if MULTI_WRITE
	sc520cs_table_write_cmos_sensor(
		addr_data_pair_hs_video_sc520cs,
		sizeof(addr_data_pair_hs_video_sc520cs) /
		sizeof(kal_uint16));
#else
   write_cmos_sensor8(0x301f,0x11);
	...
	write_cmos_sensor8(0x3201,0x00);
	write_cmos_sensor8(0x3202,0x00);
	write_cmos_sensor8(0x3203,0x0c);
	write_cmos_sensor8(0x3204,0x0a);
	write_cmos_sensor8(0x3206,0x07);
	write_cmos_sensor8(0x3207,0x9b);
	write_cmos_sensor8(0x3208,0x05);
	write_cmos_sensor8(0x3209,0x00);
	write_cmos_sensor8(0x320a,0x03);
	write_cmos_sensor8(0x320b,0xc0);
	write_cmos_sensor8(0x3210,0x00);
	write_cmos_sensor8(0x3211,0x0a);
	write_cmos_sensor8(0x3212,0x00);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_slim_video_sc520cs[] = {
0x0100,0x00,
0x301f,0x01,
0x3220,0x80,
0x3215,0x11,
...
#if MULTI_WRITE
	sc520cs_table_write_cmos_sensor(
		addr_data_pair_slim_video_sc520cs,
		sizeof(addr_data_pair_slim_video_sc520cs) /
		sizeof(kal_uint16));
#else
    write_cmos_sensor8(0x301f,0x11);
	mDELAY(10);
...
static void custom1_setting(void)
{
    write_cmos_sensor8(0x301f,0x11);
	mDELAY(10);
	write_cmos_sensor8(0x301f,0x13);
	write_cmos_sensor8(0x36e9,0x80);
	write_cmos_sensor8(0x37f9,0x80);
	write_cmos_sensor8(0x3200,0x00);
	write_cmos_sensor8(0x3201,0x00);
	write_cmos_sensor8(0x3202,0x01);
	write_cmos_sensor8(0x3203,0x74);
	write_cmos_sensor8(0x3204,0x0a);
	write_cmos_sensor8(0x3205,0x27);
	write_cmos_sensor8(0x3206,0x06);
	write_cmos_sensor8(0x3207,0x33);
	write_cmos_sensor8(0x3208,0x06);
	write_cmos_sensor8(0x3209,0x40);
	write_cmos_sensor8(0x320a,0x04);
	write_cmos_sensor8(0x320b,0xb0);
	write_cmos_sensor8(0x320e,0x09);
	write_cmos_sensor8(0x320f,0xc4);
	write_cmos_sensor8(0x3210,0x01);
	write_cmos_sensor8(0x3211,0xf4);
	write_cmos_sensor8(0x3212,0x00);
	write_cmos_sensor8(0x3213,0x08);
	write_cmos_sensor8(0x3301,0x08);
	write_cmos_sensor8(0x3306,0x60);
			...
			do {
				*sensor_id =return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {				
					LOG_INF("hdd_i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		write_cmos_sensor8(0x3907,0x41);
