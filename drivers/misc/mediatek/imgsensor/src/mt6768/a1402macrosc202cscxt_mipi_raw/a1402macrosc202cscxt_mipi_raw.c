 ...
 * Filename:
 * ---------
 *     sc202cs_ofilm_mipiraw_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "a1402macrosc202cscxt_mipi_raw.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "a1402macrosc202cscxt_mipi_raw"
...
static imgsensor_info_struct imgsensor_info = { 
    .sensor_id = A1402MACROSC202CSCXT_SENSOR_ID,
   ...
   // .checksum_value = 0x221b1a41,        //checksum value for Camera Auto Test
    ...
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_B,    //+bug682590,zhanghengyuan.wt,ADD,2021/8/26,fix depth sc202cs output_dataformat
...
*************************************************************************/
  ...
  static kal_int8 sc202cs_read_general_info(int *addr, kal_uint16 addr_len, kal_uint8 *result)
  {
  	int i = 0;
  ...
  unsigned int sc202cs_read_otp(void)
  {
  	int ret = 0;
	kal_uint8 i;
	int otpdata_addr[] = {0x800A, 0x800B,0x8011,0x8012, 0x8000, 0x8001, 0x8002, 0x8003, 0x8004, 0x8005, 0x8006, 0x8007};
	ret = sc202cs_read_general_info(otpdata_addr, ARRAY_SIZE(otpdata_addr), otpData);
  	if (ret != 0) {
  ...
  unsigned int sc202cs_read_otp_info(struct i2c_client *client,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	kal_uint8 retry = 2;
    int ret = 0;
    //ret  = sc202cs_read_otp();
	if (ret != 0) {
		pr_err("Read sc202cs sunny otp info failed");
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		pr_err("cwd return_sensor_id:0x%x\n",*sensor_id);
            ...
            LOG_INF("ren_Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
UINT32 A1402MACROSC202CSCXT_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
    ...
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    A1402MACROSC202CSCXT_MIPI_RAW_SensorInit    */
