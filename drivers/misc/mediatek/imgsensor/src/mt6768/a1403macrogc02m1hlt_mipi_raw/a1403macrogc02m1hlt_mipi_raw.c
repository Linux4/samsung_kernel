...
#include "a1403macrogc02m1hlt_mipi_raw.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = A1403MACROGC02M1HLT_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{1600, 1200, 0, 150, 1600, 900, 1600, 900, 0000, 0000, 1600, 900, 0, 0, 1600, 900}, /* HS video */
...
static void custom3_setting(void)
{
	LOG_INF("E\n");
    gc02m1b_table_write_cmos_sensor(
		addr_data_pair_custom3_gc02m1b,
		sizeof(addr_data_pair_custom3_gc02m1b) /
		sizeof(kal_uint16));
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("GC02M1B(Y1&Y6) ren_i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("Read sensor id fail, ren_write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
