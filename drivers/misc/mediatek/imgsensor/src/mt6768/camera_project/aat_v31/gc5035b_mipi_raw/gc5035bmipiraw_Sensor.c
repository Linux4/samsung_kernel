		...
		for (i = 0; i < REG_INFO_SIZE; i++) {
			...
			for (j = 0; j < 2; j++) {
				if (((temp >> (4 * j + 3)) & 0x01) == 0x01) {// check
					gc5035b_otp_data.regs[gc5035b_otp_data.reg_num][0] = (temp >> (4 * j)) & 0x07;// page
...
static void gc5035b_otp_update_dd(void)
{
	...
	if (0x01 == gc5035b_otp_data.dd_flag) {
...
static void gc5035b_otp_update_wb(void)
{
	...
	if (0x02 == gc5035b_otp_data.golden_flag) {
		return;
	}
	if (0x01 == (gc5035b_otp_data.golden_flag & 0x01)) {
	...
	if (0x01 == (gc5035b_otp_data.wb_flag & 0x01)) {
