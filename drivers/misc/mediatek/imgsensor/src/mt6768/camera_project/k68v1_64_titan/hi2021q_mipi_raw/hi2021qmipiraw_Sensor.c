...
static void wait_stream_onoff(bool stream_onoff);
//for coverity modify : void wait_stream_onoff(bool stream_onoff);
...
static imgsensor_struct imgsensor = {
	...
	//test pattern mode or not. KAL_TRUE for in test pattern mode, KAL_FALSE for normal output
...
static kal_uint32 open(void)
{
	...
	LOG_INF("[%s]: PLATFORM:MT6735,MIPI 4LANE\n", __func__);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			LOG_INF("%s:Read sensor id fail %s i2c write id: 0x%x, id: 0x%x\n",
				__func__, __func__, imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
...
static kal_uint32 set_auto_flicker_mode(kal_bool enable, UINT16 framerate)
{
	...
	if (enable)
		...
		imgsensor.autoflicker_en = KAL_FALSE; //Cancel Auto flick
	spin_unlock(&imgsensor_drv_lock);
	return ERROR_NONE;
}
	...
	if (stream_onoff) { /* stream on */
		...
		do {
			...
			if (pll_cfg_ramb_tg_b & PLL_CLKGEN_EN_RAMP_EN)
				break;
			wait_poll_cnt++;
			usleep_range(POLL_TIME_USEC, POLL_TIME_USEC + 1);
