...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	{ 2608, 1960,   16,   14, 2576, 1932, 2576, 1932,
	  0000, 0000, 2576, 1932, 0000, 0000, 2576, 1932 }, // preview
	{ 2608, 1960,   16,   14, 2576, 1932, 2576, 1932,
	  0000, 0000, 2576, 1932, 0000, 0000, 2576, 1932 }, // capture
	{ 2608, 1960,   24,  260, 2560, 1440, 2560, 1440,
	  0000, 0000, 2560, 1440, 0000, 0000, 2560, 1440 }, // video
	{ 2608, 1960,   24,   20, 2560, 1920,  640,  480,
	  0000, 0000,  640,  480, 0000, 0000,  640,  480 }, // high speed video
	{ 2608, 1960,   24,   20, 2560, 1920,  640,  480,
	  0000, 0000,  640,  480, 0000, 0000,  640,  480 }, // slim video
	{ 2608, 1960,   24,   20, 2560, 1920,  640,  480,
	  0000, 0000,  640,  480, 0000, 0000,  640,  480 } // custom1
};
...
static kal_uint16 s5k5e9yx_init_setting[] = {
	0x0100, 0x00,
	0x0A02, 0x3F,
	0x3B45, 0x01,
	0x3290, 0x10,
...
static kal_uint16 s5k5e9yx_preview_setting[] = {
	0x0136, 0x1A,
	0x0137, 0x00,
	0x0301, 0x06,
	0x0305, 0x05,
...
static kal_uint16 s5k5e9yx_cap_setting[] = {
	0x0136, 0x1A,
	0x0137, 0x00,
	0x0301, 0x06,
	0x0305, 0x05,
...
static kal_uint16 s5k5e9yx_normal_video_setting[] = {
	0x0136, 0x1A,
	0x0137, 0x00,
	0x0301, 0x06,
	0x0305, 0x05,
...
static kal_uint16 s5k5e9yx_hs_video_setting[] = {
	0x0136, 0x1A,
	0x0137, 0x00,
	0x0301, 0x06,
	0x0305, 0x05,
...
static kal_uint16 s5k5e9yx_slim_video_setting[] = {
	0x0136, 0x1A,
	0x0137, 0x00,
	0x0301, 0x06,
	0x0305, 0x05,
...
static kal_uint16 s5k5e9yx_custom1_setting[] = {
	0x0136, 0x1A,
	0x0137, 0x00,
	0x0301, 0x06,
	0x0305, 0x05,
...
{
	table_write_cmos_sensor(s5k5e9yx_init_setting,
		sizeof(s5k5e9yx_init_setting) / sizeof(kal_uint16));
}				/*  s5k5e9yxMIPI_Sensor_Init  */


static void preview_setting(void)
{
	table_write_cmos_sensor(s5k5e9yx_preview_setting,
		sizeof(s5k5e9yx_preview_setting) / sizeof(kal_uint16));
}

static void capture_setting(kal_uint16 currefps)
{
	table_write_cmos_sensor(s5k5e9yx_cap_setting,
		sizeof(s5k5e9yx_cap_setting) / sizeof(kal_uint16));
}

static void normal_video_setting(kal_uint16 currefps)
{
	LOG_INF("E! currefps:%d\n", currefps);
	table_write_cmos_sensor(s5k5e9yx_normal_video_setting,
		sizeof(s5k5e9yx_normal_video_setting) / sizeof(kal_uint16));
}

static void hs_video_setting(void)
{
	LOG_INF("EVGA 120fps");
	table_write_cmos_sensor(s5k5e9yx_hs_video_setting,
		sizeof(s5k5e9yx_hs_video_setting) / sizeof(kal_uint16));
}

static void slim_video_setting(void)
{
	LOG_INF("E");
	table_write_cmos_sensor(s5k5e9yx_slim_video_setting,
		sizeof(s5k5e9yx_slim_video_setting) / sizeof(kal_uint16));
}

static void custom1_setting(void)
{
	LOG_INF("E");
	table_write_cmos_sensor(s5k5e9yx_custom1_setting,
		sizeof(s5k5e9yx_custom1_setting) / sizeof(kal_uint16));
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	//sensor have two i2c address 0x5b 0x5a & 0x21 0x20,
...
static int s5k5e9yx_check_read_complete(void)
{
	kal_uint8 retry = 0;
	kal_uint8 retry_max_cnt = 0;
	kal_uint16 chk_complete = 0;

	retry = 0;
	retry_max_cnt = 10;

	// check complete
	while((chk_complete & 0x1) != 0x1) {
		write_cmos_sensor_8(S5K5E9_OTP_CONTROL, 0x01);
		// 1 ms delay
		usleep_range(1000, 1000);
		chk_complete = read_cmos_sensor_8(S5K5E9YX_OTP_ERR_FLAG);
		retry++;
		LOG_INF("%s, check complete, retry: %d \n", __func__, retry);

		if (retry == retry_max_cnt) {
			pr_err("%s, retry count[%d] : fail \n", __func__, retry);
			return -1;
		}
	}
	return 0;
...
static int s5k5e9yx_read_otp_data(unsigned short start_page, unsigned short start_addr,
		unsigned char *data, unsigned int size)
{
	...
	// select page
	write_cmos_sensor_8(S5K5E9YX_OTP_PAGE_SEL, page);
	// check complete
	ret = s5k5e9yx_check_read_complete();
	if (ret < 0) {
		pr_err("%s: fail\n", __func__);
		return -1;
	}
	...
	// read data
	for(i = 0; i < size; i++) {
		if(addr > S5K5E9YX_OTP_DATA_63) {
			page++;
			addr = S5K5E9YX_OTP_DATA_0;
			// select page
			write_cmos_sensor_8(S5K5E9YX_OTP_PAGE_SEL, page);
			// check complete
			ret = s5k5e9yx_check_read_complete();
		...
		read_data = read_cmos_sensor_8(addr);
		*(data+ i) = read_data;
		LOG_INF("page: %d, addr: %#06x, data: %d \n", page, addr, read_data);
...
int s5k5e9yx_read_otp_cal(unsigned int addr, unsigned char *data, unsigned int size)
{
	kal_int32 ret = 0;
	kal_uint16 otp_bank = 0;
	kal_uint16 otp_page = 0;
	kal_uint16 otp_cal_start_addr = 0;
	...
	// search otp bank
	write_cmos_sensor_8(S5K5E9YX_OTP_PAGE_SEL, 0x11);
	// check complete
	ret = s5k5e9yx_check_read_complete();
	if (ret < 0) {
		pr_err("%s: fail\n", __func__);
	...
	otp_bank = read_cmos_sensor_8(S5K5E9YX_OTP_DATA_0);

	switch(otp_bank) {
	case S5K5E9_OTP_BANK1_MARK:
		otp_page = S5K5E9_OTP_BANK1_START_PAGE;
		break;
	case S5K5E9_OTP_BANK2_MARK:
		otp_page = S5K5E9_OTP_BANK2_START_PAGE;
		break;
	case S5K5E9_OTP_BANK3_MARK:
		otp_page = S5K5E9_OTP_BANK3_START_PAGE;
		break;
	case S5K5E9_OTP_BANK4_MARK:
		otp_page = S5K5E9_OTP_BANK4_START_PAGE;
		break;
	case S5K5E9_OTP_BANK5_MARK:
		otp_page = S5K5E9_OTP_BANK5_START_PAGE;
		break;
	default:
	...
	// set start address for otp cal
	otp_cal_start_addr = S5K5E9YX_OTP_DATA_12;
	ret = s5k5e9yx_read_otp_data(otp_page, otp_cal_start_addr, data, size);
	if (ret < 0) {
		pr_err("%s: fail\n", __func__);
		return -1;
	}
	...
	// make initial state
	write_cmos_sensor_8(S5K5E9_OTP_CONTROL, 0x04);
	write_cmos_sensor_8(S5K5E9_OTP_CONTROL, 0x00);
