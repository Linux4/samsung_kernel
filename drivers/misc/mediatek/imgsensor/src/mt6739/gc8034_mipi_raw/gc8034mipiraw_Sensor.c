...
#define LOG_1 LOG_INF("GC8034MIPI, 2LANE\n")
/****************************   Modify end    *******************************************/
#define GC8034_DEBUG
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	{ 3284, 2464,    0,    0, 3284, 2464, 3284, 2464,
	     9,    8, 3264, 2448,    0,    0, 3264, 2448 }, /*preview*/
	{ 3284, 2464,    0,    0, 3284, 2464, 3284, 2464,
	     9,    8, 3264, 2448,    0,    0, 3264, 2448 }, /*capture*/
	{ 3284, 2464,    0,    0, 3284, 2464, 3284, 2464,
	     9,  314, 3264, 1836,    0,    0, 3264, 1836 }, /*video*/
	{ 3284, 2464,    0,    0, 3284, 2464, 3284, 2464,
	     9,  314, 3264, 1836,    0,    0, 3264, 1836 }, /*hs video*/
	{ 3284, 2464,    0,    0, 3284, 2464, 3284, 2464,
	     9,  314, 3264, 1836,    0,    0, 3264, 1836 }, /*sl video*/
	{ 3284, 2464,    0,    0, 3284, 2464, 3284, 2464,
	     9,  314, 3264, 1836,    0,    0, 3264, 1836 } /*custom1*/
};
...
static void gc8034_otp_dpc_table_autoload(void)
{
	kal_uint32 bad_pixel_cnt_1 = 0;
	kal_uint32 bad_pixel_cnt_2 = 0;

	//STEP 1
	//OTP read initial setting
	write_cmos_sensor(0xfc, 0x00);
	write_cmos_sensor(0xf7, 0x97); //0xf7[1]: 1'b1
	write_cmos_sensor(0xfc, 0x00);
	write_cmos_sensor(0xfc, 0x00);

	write_cmos_sensor(0xfc, 0xee);
	write_cmos_sensor(0xf2, 0x01); //OTP clock enable
	write_cmos_sensor(0xf4, 0x88); //OTP enable

	//STEP 2
	// check count of bad pixel for OTP DPC
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xd4, 0x00); //high address[1:0]

	write_cmos_sensor(0xd5, 0x60); //addr:0x60
	write_cmos_sensor(0xf3, 0x20); //total number of bad pixel
	bad_pixel_cnt_1 = read_cmos_sensor(0xd7);

	write_cmos_sensor(0xd5, 0x68); //addr:0x68
	write_cmos_sensor(0xf3, 0x20); //invalid number of bad pixel
	bad_pixel_cnt_2 = read_cmos_sensor(0xd7);
	pr_info("%s, bad pixel cnt [1]: %#x, [2]:%#x \n", __func__,
			bad_pixel_cnt_1, bad_pixel_cnt_2);

	//STEP 3
	//Set total defect pixel number
	write_cmos_sensor(0x7b, (bad_pixel_cnt_1 + bad_pixel_cnt_2));
	...
	//start autoload
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x79, 0x2e); //autoload enable
	write_cmos_sensor(0xf2, 0x41); //autoload start
	...
	//DPC ON
	write_cmos_sensor(0xfe, 0x01);
	write_cmos_sensor(0xbe, 0x01); //SRAM enable

	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x79, 0x00); //autoload off

	write_cmos_sensor(0xfc, 0x00);
	write_cmos_sensor(0xf7, 0x95); //0xf7[1]: 1'b0

	write_cmos_sensor(0xfc, 0x00);
	write_cmos_sensor(0xfc, 0x00);
	write_cmos_sensor(0xfc, 0xee);

	write_cmos_sensor(0x80, 0x10); //DPC ON
}

static void set_dummy(void)
{
	kal_uint32 frame_length = imgsensor.frame_length >> 1;
	kal_uint32 win_height, vertical_blanking;

	frame_length = frame_length << 1;
	write_cmos_sensor(0xfe, 0x00);
	win_height = (read_cmos_sensor(0x0d) << 8) | read_cmos_sensor(0x0e);
	win_height = win_height & 0xfff;
	vertical_blanking
		= frame_length - win_height - 0x14;	// from datasheet
	write_cmos_sensor(0x07, (vertical_blanking >> 8) & 0x1f);
	write_cmos_sensor(0x08, (vertical_blanking & 0xff));
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 GC8034_AGC_Param[MAX_GAIN_INDEX][2] = {
		{  256,  0 },
		{  358,  1 },
		{  512,  2 },
		{  716,  3 },
	...
	for (gain_index = E_GAIN_INDEX - 1; gain_index >= 0; gain_index--) {
		if (reg_gain >= GC8034_AGC_Param[gain_index][0]) {
			break;
		}
	}

	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xb6, GC8034_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * Dgain_ratio / GC8034_AGC_Param[gain_index][0];
	write_cmos_sensor(0xb1, (temp_gain >> 8) & 0x0f);
	write_cmos_sensor(0xb2, temp_gain & 0xff);
	LOG_INF("Exit! GC8034_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d\n",
		GC8034_AGC_Param[gain_index][1], temp_gain,reg_gain);
