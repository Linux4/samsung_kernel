...
#include "n8_bf2253_qh_6_mipi_raw.h"


/****************************Modify Following Strings for Debug****************************/
#define PFX "n8_bf2253_qh_6_camera_sensor"
#define LOG_1 LOG_INF("N8_BF2253_QH_6, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
extern int current_sensor_id;

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool N8_BF2253_QH_6DuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = N8_BF2253_QH_6_MIPI_SENSOR_ID,
	...
	//bug 595957,huangguoyong.wt,modify,2020/10/29,modify for its test_exposure fail
...
static void sensor_init(void)
{
	...
	//;;FrameW:1780
	//;;FrameH:1236
	flag_value = read_cmos_sensor( 0xfb );
	if (flag_value == 0x08) {
		write_cmos_sensor(0xe1,0x06);
		write_cmos_sensor(0xe2,0x0a);
		...
		write_cmos_sensor(0x71,0x06);
		write_cmos_sensor(0x72,0x18);
	  ...
	  //write_cmos_sensor(0x03,0xbb);
		...
		write_cmos_sensor(0xe4,0x70);
		write_cmos_sensor(0xe5,0x67);
		write_cmos_sensor(0xe6,0x03);
		write_cmos_sensor(0xe8,0x86);
		write_cmos_sensor(0x01,0x14);
		write_cmos_sensor(0x03,0xb8);
		write_cmos_sensor(0x59,0x10);
		write_cmos_sensor(0x5a,0x10);
		write_cmos_sensor(0x5c,0x11);
		write_cmos_sensor(0x5d,0x73);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = return_sensor_id() + 5; //bug 534796 , sunhushan.wt, 2020.2.26, fix build errorn
                         LOG_INF("hi1336_txd_jct current_sensor_id = 0x%x",current_sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id && N8_HI1336_TXD_JCT_SENSOR_ID == current_sensor_id) {
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			sensor_id = return_sensor_id() + 5; //bug 534796 , sunhushan.wt, 2020.2.26, fix build error
	...
	spin_unlock(&imgsensor_drv_lock);
	N8_BF2253_QH_6DuringTestPattern = KAL_FALSE;
	return ERROR_NONE;
}
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
                //OA6649398,zhouyikuan.wt,MODIFY,2020/10/19,bf2253 sensor version change from v1->v4
		write_cmos_sensor(0xe0, 0X02); // stream on
	} else {
