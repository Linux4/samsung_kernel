 ...
 * Filename:
 * ---------
 *     n21_qunh_micro_bf2253mipi_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "n21_qunh_micro_bf2253mipiraw_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "n21_qunh_micro_bf2253_camera_sensor"
#define LOG_1 LOG_INF("N21_QUNH_MICRO_BF2253, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_err(PFX "[%s] " format, __func__, ##args)

//+Bug 492443, sunhushan.wt, ADD, 2019.10.15, porting the otp of the 12sensor 
#define EEPROM_CN24C64AH_ID 0xA4
...
kal_bool N21_QUNH_MICRO_BF2253DuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = N21_QUNH_MICRO_BF2253_SENSOR_ID,
	...
	.i2c_addr_table	= {0xdc,0xda,0xff},			//record sensor support all write id addr, only supprt 4, must end with 0xff
};
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	LOG_INF( "gain2reg: isp gain=%d, sensor gain=0x%x\n", iGain, iReg );
...
static void sensor_init(void)
{
	...
	write_cmos_sensor( 0xe4, 0xc3 );
	write_cmos_sensor( 0xe5, 0x25 );
	write_cmos_sensor( 0xe6, 0x00 );//05
	write_cmos_sensor( 0xe8, 0x5c );	
	write_cmos_sensor( 0x01, 0x14 );
	write_cmos_sensor( 0x02, 0x24 );
	write_cmos_sensor( 0x03, 0x98 );
	write_cmos_sensor( 0x12, 0xf0 );
	write_cmos_sensor( 0x20, 0x0a );
	write_cmos_sensor( 0x21, 0x0c );	
	write_cmos_sensor( 0x38, 0x7a );
	write_cmos_sensor( 0x39, 0x16 );
	write_cmos_sensor( 0x3a, 0xe6 );
	write_cmos_sensor( 0x42, 0xec );
	write_cmos_sensor( 0xbe, 0x53 );
	write_cmos_sensor( 0x59, 0x10 );
	write_cmos_sensor( 0x5a, 0x10 );
	write_cmos_sensor( 0x6a, 0x3f );
	write_cmos_sensor( 0x6b, 0x0e );
	write_cmos_sensor( 0x6C, 0x7e );
...
static struct stCAM_CAL_DATAINFO_STRUCT qunh_micro_bf2253_eeprom_data ={
	.sensorID= N21_QUNH_MICRO_BF2253_SENSOR_ID,
	.deviceID = 0x10,
	.dataLength = 0x0E5E,
	.sensorVendorid = 0x0A050100,
	.vendorByte = {1,2,3,4},
	.dataBuffer = NULL,
};
static struct stCAM_CAL_CHECKSUM_STRUCT qunh_micro_bf2253_Checksum[8] =
{
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				//+Bug 492443, sunhushan.wt, ADD, 2019.10.15, porting the otp of the 12sensor 
				size = imgSensorReadEepromData(&qunh_micro_bf2253_eeprom_data,qunh_micro_bf2253_Checksum);
				if(size != qunh_micro_bf2253_eeprom_data.dataLength || (qunh_micro_bf2253_eeprom_data.sensorVendorid >> 24)!= read_eeprom(0x0001)) {
					printk("get eeprom data failed\n");
					if(qunh_micro_bf2253_eeprom_data.dataBuffer != NULL) {
						kfree(qunh_micro_bf2253_eeprom_data.dataBuffer);
						qunh_micro_bf2253_eeprom_data.dataBuffer = NULL;
					}
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				} else {
					LOG_INF("get eeprom data success\n");
					imgSensorSetEepromData(&qunh_micro_bf2253_eeprom_data);
				}
				//-Bug 492443, sunhushan.wt, ADD, 2019.10.15, porting the otp of the 12sensor 
...
static kal_uint32 open(void)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	N21_QUNH_MICRO_BF2253DuringTestPattern = KAL_FALSE;
	return ERROR_NONE;
}
...
UINT32 N21_QUNH_MICRO_BF2253_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
