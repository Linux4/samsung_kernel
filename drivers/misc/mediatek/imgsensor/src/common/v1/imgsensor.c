...
bool imgsensor_is_open[MAX_NUM_OF_SUPPORT_SENSOR];
...
imgsensor_sensor_open(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (psensor_func &&
		...
		mDELAY(2); //Reduced delay time to optimize wide camera's entry time. (5ms -> 2ms)
	...
	imgsensor_is_open[psensor->inst.sensor_idx] = ret ? false : true;
