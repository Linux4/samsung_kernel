...
#include "n21_txd_wide_hi846mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = N21_TXD_WIDE_HI846_SENSOR_ID,
...
static void slim_video_setting(void)
{
	...
	write_cmos_sensor(0x091C, 0x0B04);
	write_cmos_sensor(0x091E, 0x0600);
	write_cmos_sensor(0x090C, 0x0220);
	write_cmos_sensor(0x090E, 0x0072);
	write_cmos_sensor(0x0958, 0x9980);
	write_cmos_sensor(0x0F32, 0x025A);
        write_cmos_sensor(0x0F38, 0x0255);
	write_cmos_sensor(0x0F2A, 0x4924);
...
static struct stCAM_CAL_DATAINFO_STRUCT txd_wide_hi846_eeprom_data ={
	.sensorID= N21_TXD_WIDE_HI846_SENSOR_ID,
	.deviceID = 0x08,
	.dataLength = 0x0E60,
	.sensorVendorid = 0x19040101,
	.vendorByte = {1,2,3,4},
	.dataBuffer = NULL,
};
static struct stCAM_CAL_CHECKSUM_STRUCT txd_wide_hi846_Checksum[8] =
{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				//+Bug 492443, sunhushan.wt, ADD, 2019.10.15, porting the otp of the 12sensor 
				size = imgSensorReadEepromData(&txd_wide_hi846_eeprom_data,txd_wide_hi846_Checksum);
				if(size != txd_wide_hi846_eeprom_data.dataLength || (txd_wide_hi846_eeprom_data.sensorVendorid >> 24)!= read_eeprom(0x0001)) {
					printk("get eeprom data failed\n");
					if(txd_wide_hi846_eeprom_data.dataBuffer != NULL) {
						kfree(txd_wide_hi846_eeprom_data.dataBuffer);
						txd_wide_hi846_eeprom_data.dataBuffer = NULL;
					}
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				} else {
					LOG_INF("get eeprom data success\n");
					imgSensorSetEepromData(&txd_wide_hi846_eeprom_data);
				}
				//-Bug 492443, sunhushan.wt, ADD, 2019.10.15, porting the otp of the 12sensor 
