...
#define HQEXTEND_CAM_SN_INFO "cameraSNInfo"
char hq_camerachipSN[255] = {};
char *hq_cameraSNModuleInfo[3] = {NULL, NULL, NULL};
static struct proc_dir_entry *hqextend_proc_entry_sn;

static ssize_t hqextend_cameraSNInfo_read
    (struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    char buf[255] = {0};
    int rc = 0;
    ...
    snprintf(buf, 255,
	"rear_camera:%s:%s\n",
	hq_cameraSNModuleInfo[0],hq_camerachipSN);
...
static ssize_t hqextend_cameraSNInfo_write
    (struct file *filp, const char __user *buffer,
    size_t count, loff_t *off)
{
    return 0;
...
static const struct file_operations hqextend_cameraSNInfo_fops = {
    .owner = THIS_MODULE,
    .read = hqextend_cameraSNInfo_read,
    .write = hqextend_cameraSNInfo_write,
};
#endif
...
/* A06 code for SR-AL7160A-917 by zhongquan at 20240529 start */
#ifdef CONFIG_HQ_PROJECT_O8
 hqextend_proc_entry_sn=proc_create(HQEXTEND_CAM_SN_INFO,
                                0664, NULL,
                                &hqextend_cameraSNInfo_fops);
    if (NULL == hqextend_proc_entry_sn) {
            pr_err("[cameradebug]create hqextend_proc_entry_sn-cameraSNInfo failed");
            remove_proc_entry(HQEXTEND_CAM_SN_INFO, NULL);
    }
#endif
/* A06 code for SR-AL7160A-917 by zhongquan at 20240529 end */
