...
static struct imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV02B10_JK_SENSOR_ID,
	
	.checksum_value = 0xb7c53a42,       //0x6d01485c // Auto Test Mode 蓄板..

	.pre = {
		.pclk = 17400000,            //record different mode's pclk
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if((gain >= 0x40) && (gain <= (15*0x40))) //base gain = 0x40
	{
		...
		else
		{
			...
			write_cmos_sensor(0xfe, 0x02);	//fresh 
			pr_info("OV02BMIPI_SetGain = %d",iReg);
		}
	}
	else
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x25, 0x07);
    write_cmos_sensor(0x29, 0x03);
    write_cmos_sensor(0x2a, 0x37);
