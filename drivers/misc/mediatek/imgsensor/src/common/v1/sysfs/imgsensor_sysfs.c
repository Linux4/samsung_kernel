	...
	struct device_attribute *attr, char *buf)
{
	...
	if(crc32_check_list[position][CRC32_CHECK_FW_VER] == true) {
		...
		if ( crc32_check_fw && crc32_check_factory && crc32_check_setfile
			&& crc32_check_list[SENSOR_POSITION_REAR][CRC32_CHECK_FACTORY] == true
		) {
			...
			if (!crc32_check_list[SENSOR_POSITION_REAR][CRC32_CHECK_FACTORY])
...
bool imgsensor_check_otp_crc32(char *buf, int deviceIdx)
{
	...
	u32 checksumFromOTP = 0;

	int otp_header_cal_start_addr = 0;
	int otp_header_checksum_addr = 0;
	int otp_header_checksum_len = 0;
	int otp_awb_cal_start_addr = 0;
	int otp_awb_checksum_addr = 0;
	int otp_awb_checksum_len = 0;
	...
	otp_awb_cal_start_addr    = rom_addr->rom_awb_cal_data_start_addr;
	...
	/*************************** HEADER checksum ***************************/
	if (otp_header_checksum_addr >= 0 && otp_header_checksum_len >= 0) {
		...
		checksum = (u32)getCRC((u16 *)&buf32[otp_header_cal_start_addr], otp_header_checksum_len, NULL, NULL);

		if(checksum != checksumFromOTP) {
			crc32_temp = crc32_header_temp = false;
			pr_err("Camera: OTP CRC32 error at the header data section (0x%08X != 0x%08X)",checksum, checksumFromOTP);
		} else {
			crc32_temp = crc32_header_temp = true;
			pr_debug("Camera: End checking OTP CRC32 (0x%08X = 0x%08X)",checksum, checksumFromOTP);
		}
	} else {
		pr_info("[%s] Camera[%d]: OTP Header CRC32 Checking is skip \n", __func__, deviceIdx);
	}

	/*************************** AWB checksum ***************************/
	if (otp_awb_checksum_addr >= 0 && otp_awb_cal_start_addr >= 0) {
		...
		checksum = (u32)getCRC((u16 *)&buf32[otp_awb_cal_start_addr / 4], otp_awb_checksum_len, NULL, NULL);

		if(checksum != checksumFromOTP) {
			crc32_temp = false;
			pr_err("Camera: CRC32 error at the AWB data section (0x%08X != 0x%08X)", checksum, checksumFromOTP);
			goto out;
		} else {
			crc32_temp = true;
			pr_debug("Camera: End checking OTP AWB CRC32 (0x%08X = 0x%08X)", checksum, checksumFromOTP);
		}
	}
...
int imgsensor_sys_get_cal_size(unsigned int dualDeviceId, unsigned int sensorId)
{
	enum IMGSENSOR_SENSOR_IDX deviceIdx = IMGSENSOR_SENSOR_IDX_MAP(dualDeviceId);
	int max_num_of_rom_info = 0, i = 0, cal_size = -1;
	...
	for (i = 0; i <= max_num_of_rom_info; i++) {
		...
		if (vendor_rom_info[i].rom_addr != NULL &&
			vendor_rom_info[i].sensor_position == deviceIdx &&
			...
			pr_info("[%s] sensor position: %d, sensor_id: %#06x, Cal Size: %#06x\n",
					__func__, deviceIdx, sensorId, cal_size);
			break;
		}
	}
	return cal_size;
...
int imgsensor_sysfs_update(unsigned char* pRomData, unsigned int dualDeviceId, unsigned int sensorId, unsigned int offset, unsigned int length, int i4RetValue)
{
	...
	cal_size = imgsensor_sys_get_cal_size(dualDeviceId, sensorId);
