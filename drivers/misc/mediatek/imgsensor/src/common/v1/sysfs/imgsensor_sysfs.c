...
#ifdef IMGSENSOR_REAR_MAX_CAL_SIZE
static char cal_buf_rear[IMGSENSOR_REAR_MAX_CAL_SIZE];
#endif
#ifdef IMGSENSOR_FRONT_MAX_CAL_SIZE
static char cal_buf_front[IMGSENSOR_FRONT_MAX_CAL_SIZE];
#endif
#ifdef IMGSENSOR_REAR2_MAX_CAL_SIZE
static char cal_buf_rear2[IMGSENSOR_REAR2_MAX_CAL_SIZE];
#endif
#ifdef IMGSENSOR_FRONT2_MAX_CAL_SIZE
static char cal_buf_front2[IMGSENSOR_FRONT2_MAX_CAL_SIZE];
#endif
#ifdef IMGSENSOR_REAR3_MAX_CAL_SIZE
static char cal_buf_rear3[IMGSENSOR_REAR3_MAX_CAL_SIZE];
#endif
#ifdef IMGSENSOR_FRONT3_MAX_CAL_SIZE
static char cal_buf_front3[IMGSENSOR_FRONT3_MAX_CAL_SIZE];
#endif
#ifdef IMGSENSOR_REAR4_MAX_CAL_SIZE
static char cal_buf_rear4[IMGSENSOR_REAR4_MAX_CAL_SIZE];
#endif
#ifdef IMGSENSOR_FRONT4_MAX_CAL_SIZE
static char cal_buf_front4[IMGSENSOR_FRONT4_MAX_CAL_SIZE];
#endif


static char *g_cal_buf[SENSOR_POSITION_MAX] = {
#ifdef IMGSENSOR_REAR_MAX_CAL_SIZE
	cal_buf_rear,
#else
	NULL,
...
static int g_cal_buf_size[SENSOR_POSITION_MAX] = {
#ifdef IMGSENSOR_REAR_MAX_CAL_SIZE
	IMGSENSOR_REAR_MAX_CAL_SIZE,
#else
	0,
#endif
#ifdef IMGSENSOR_FRONT_MAX_CAL_SIZE
	IMGSENSOR_FRONT_MAX_CAL_SIZE,
#else
	0,
#endif
#ifdef IMGSENSOR_REAR2_MAX_CAL_SIZE
	IMGSENSOR_REAR2_MAX_CAL_SIZE,
#else
	0,
#endif
#ifdef IMGSENSOR_FRONT2_MAX_CAL_SIZE
	IMGSENSOR_FRONT2_MAX_CAL_SIZE,
#else
	0,
#endif
#ifdef IMGSENSOR_REAR3_MAX_CAL_SIZE
	IMGSENSOR_REAR3_MAX_CAL_SIZE,
#else
	0,
#endif
#ifdef IMGSENSOR_FRONT3_MAX_CAL_SIZE
	IMGSENSOR_FRONT3_MAX_CAL_SIZE,
#else
	0,
#endif
#ifdef IMGSENSOR_REAR4_MAX_CAL_SIZE
	IMGSENSOR_REAR4_MAX_CAL_SIZE,
#else
	0,
#endif
#ifdef IMGSENSOR_FRONT4_MAX_CAL_SIZE
	IMGSENSOR_FRONT4_MAX_CAL_SIZE,
#else
	0,
#endif
};
...
void init_cam_hwparam(int position) {
	...
	if(!ec_param) {
		ec_param = (struct cam_hw_param *)kmalloc(sizeof(struct cam_hw_param), GFP_KERNEL);
...
#ifdef CONFIG_OF
	struct imgsensor_cam_info *front_cam_info = &(cam_infos[CAM_INFO_FRONT]);
...
#ifdef CONFIG_OF
	struct imgsensor_cam_info *cam_info = &(cam_infos[position]);
...
static bool imgsensor_sec_check_eeprom_crc32(char *buf, int deviceIdx, int sub_deviceIdx, bool rom_common)
{
	u32 *buf32 = NULL;
	...
	pr_err("[CRC32_DEBUG] Header CRC32 Check. check_length = %d, crc addr = 0x%08X\n",
		check_length, finfo_local->header_section_crc_addr);
#endif

	checksum_base = finfo_local->header_section_crc_addr / 4;
	checksum = (u32)getCRC((u16 *)&buf32[check_base], check_length, NULL, NULL);
	if (checksum != buf32[checksum_base]) {
		pr_err("Camera: CRC32 error at the header (0x%08X != 0x%08X)", checksum, buf32[checksum_base]);
	...
	if (check_length > 0) {
		checksum = 0;
		check_base = finfo_local->oem_start_addr / 4;
		checksum_base = finfo_local->oem_section_crc_addr / 4;

		if (rom_addr->extend_cal_addr) {
			int32_t *addr;
			addr = (int32_t *)imgsensor_sec_search_rom_extend_data(rom_addr->extend_cal_addr, EXTEND_OEM_CHECKSUM);
			if (addr != NULL) {
				if (finfo_local->oem_start_addr != *addr)
					check_base = *addr / 4;
			}
		}

#ifdef ROM_CRC32_DEBUG
		pr_err("[CRC32_DEBUG] OEM CRC32 Check. check_length = %d, crc addr = 0x%08X\n",
			check_length, finfo_local->oem_section_crc_addr);
		pr_err("[CRC32_DEBUG] start = 0x%08X, end = 0x%08X\n",
			finfo_local->oem_start_addr, finfo_local->oem_end_addr);
#endif

		if (check_base > address_boundary || checksum_base > address_boundary) {
			pr_err("Camera[%d]: OEM address has error: start(0x%08X), end(0x%08X)",
				sub_deviceIdx, finfo_local->oem_start_addr, finfo_local->oem_end_addr);
			crc32_check_temp = false;
			goto out;
		}
		...
		checksum = (u32)getCRC((u16 *)&buf32[check_base], check_length, NULL, NULL);
		if (checksum != buf32[checksum_base]) {
			pr_err("Camera[%d]: CRC32 error at the OEM (0x%08X != 0x%08X)", sub_deviceIdx, checksum, buf32[checksum_base]);
			crc32_check_temp = false;
			goto out;
		}
	} else {
		pr_warn("Camera[%d]: Skip to check oem crc32\n", sub_deviceIdx);
	}
	...
	if (check_length > 0) {
		checksum = 0;
		check_base = finfo_local->awb_start_addr / 4;
		checksum_base = finfo_local->awb_section_crc_addr / 4;

#ifdef ROM_CRC32_DEBUG
		pr_err("[CRC32_DEBUG] AWB CRC32 Check. check_length = %d, crc addr = 0x%08X\n",
			check_length, finfo_local->awb_section_crc_addr);
		pr_err("[CRC32_DEBUG] start = 0x%08X, end = 0x%08X\n",
			finfo_local->awb_start_addr, finfo_local->awb_end_addr);
#endif

		if (check_base > address_boundary || checksum_base > address_boundary) {
			pr_err("Camera[%d]: AWB address has error: start(0x%08X), end(0x%08X)",
				sub_deviceIdx, finfo_local->awb_start_addr, finfo_local->awb_end_addr);
			crc32_check_temp = false;
			goto out;
		}
		...
		checksum = (u32)getCRC((u16 *)&buf32[check_base], check_length, NULL, NULL);
		if (checksum != buf32[checksum_base]) {
			pr_err("Camera[%d]: CRC32 error at the AWB (0x%08X != 0x%08X)", sub_deviceIdx, checksum, buf32[checksum_base]);
			crc32_check_temp = false;
			goto out;
		}
	} else {
		pr_warn("Camera[%d]: Skip to check awb crc32\n", sub_deviceIdx);
	}
	...
	if (check_length > 0) {
		checksum = 0;
		check_base = finfo_local->shading_start_addr / 4;
		checksum_base = finfo_local->shading_section_crc_addr / 4;

#ifdef ROM_CRC32_DEBUG
		pr_err("[CRC32_DEBUG] Shading CRC32 Check. check_length = %d, crc addr = 0x%08X\n",
			check_length, finfo_local->shading_section_crc_addr);
		pr_err("[CRC32_DEBUG] start = 0x%08X, end = 0x%08X\n",
			finfo_local->shading_start_addr, finfo_local->shading_end_addr);
#endif

		if (check_base > address_boundary || checksum_base > address_boundary) {
			pr_err("Camera[%d]: Shading address has error: start(0x%08X), end(0x%08X)",
				sub_deviceIdx, finfo_local->shading_start_addr, finfo_local->shading_end_addr);
			crc32_check_temp = false;
			goto out;
		}
		...
		checksum = (u32)getCRC((u16 *)&buf32[check_base], check_length, NULL, NULL);
		if (checksum != buf32[checksum_base]) {
			pr_err("Camera[%d]: CRC32 error at the Shading (0x%08X != 0x%08X)", sub_deviceIdx, checksum, buf32[checksum_base]);
			crc32_check_temp = false;
			goto out;
		}
	} else {
		pr_warn("Camera[%d]: Skip to check shading crc32\n", sub_deviceIdx);
	}
	...
	if (check_length > 0) {
		checksum = 0;
		check_base = finfo_local->dual_data_start_addr / 4;
		checksum_base = finfo_local->dual_data_section_crc_addr / 4;

#ifdef ROM_CRC32_DEBUG
		pr_err("[CRC32_DEBUG] Dual Cal Data CRC32 Check. check_length = %d, crc addr = 0x%08X\n",
			check_length, finfo_local->dual_data_section_crc_addr);
		pr_err("[CRC32_DEBUG] start = 0x%08X, end = 0x%08X\n",
			finfo_local->dual_data_start_addr, finfo_local->dual_data_end_addr);
#endif

		if (check_base > address_boundary || checksum_base > address_boundary) {
			pr_err("Camera[%d]: Dual Cal Data address has error: start(0x%08X), end(0x%08X)",
				sub_deviceIdx, finfo_local->dual_data_start_addr, finfo_local->dual_data_end_addr);
			crc32_check_temp = false;
			goto out;
		}
		...
		checksum = (u32)getCRC((u16 *)&buf32[check_base], check_length, NULL, NULL);
		if (checksum != buf32[checksum_base]) {
			pr_err("Camera[%d]: CRC32 error at the DualData (0x%08X != 0x%08X)", sub_deviceIdx, checksum, buf32[checksum_base]);
			crc32_check_temp = false;
			goto out;
		}
	} else {
		pr_warn("Camera[%d]: Skip to check Dual Cal Data crc32\n", sub_deviceIdx);
	}
	...
	if (check_length > 0) {
		checksum = 0;
		check_base = finfo_local->sensor_cal_data_start_addr / 4;
		checksum_base = finfo_local->sensor_cal_data_section_crc_addr / 4;

#ifdef ROM_CRC32_DEBUG
		pr_err("[CRC32_DEBUG] Sensor Cal Data. check_length = %d, crc addr = 0x%08X\n",
			check_length, finfo_local->sensor_cal_data_section_crc_addr);
		pr_err("[CRC32_DEBUG] start = 0x%08X, end = 0x%08X\n",
			finfo_local->sensor_cal_data_start_addr, finfo_local->sensor_cal_data_end_addr);
#endif

		if (check_base > address_boundary || checksum_base > address_boundary) {
			pr_err("Camera[%d]: Sensor Cal Data address has error: start(0x%08X), end(0x%08X)",
				sub_deviceIdx, finfo_local->sensor_cal_data_start_addr, finfo_local->sensor_cal_data_end_addr);
			crc32_check_temp = false;
			goto out;
		}
		...
		checksum = (u32)getCRC((u16 *)&buf32[check_base], check_length, NULL, NULL);
		if (checksum != buf32[checksum_base]) {
			pr_err("Camera[%d]: CRC32 error at the Sensor Cal Data (0x%08X != 0x%08X)",
				sub_deviceIdx, checksum, buf32[checksum_base]);
			crc32_check_temp = false;
			goto out;
		}
	} else {
		pr_warn("Camera[%d]: Skip to check Sensor Cal Data crc32\n", sub_deviceIdx);
	}
	...
	if (check_length > 0) {
		checksum = 0;
		check_base = finfo_local->ap_pdaf_start_addr / 4;
		checksum_base = finfo_local->ap_pdaf_section_crc_addr / 4;

#ifdef ROM_CRC32_DEBUG
		pr_err("[CRC32_DEBUG] PDAF Cal Data. check_length = %d, crc addr = 0x%08X\n",
			check_length, finfo_local->ap_pdaf_section_crc_addr);
		pr_err("[CRC32_DEBUG] start = 0x%08X, end = 0x%08X\n",
			finfo_local->ap_pdaf_start_addr, finfo_local->ap_pdaf_end_addr);
#endif
		if (check_base > address_boundary || checksum_base > address_boundary) {
			pr_err("Camera[%d]: PDAF Cal Data address has error: start(0x%08X), end(0x%08X)",
				sub_deviceIdx, finfo_local->ap_pdaf_start_addr, finfo_local->ap_pdaf_end_addr);
			crc32_check_temp = false;
			goto out;
		}
		...
		checksum = (u32)getCRC((u16 *)&buf32[check_base], check_length, NULL, NULL);
		if (checksum != buf32[checksum_base]) {
			pr_err("Camera[%d]: CRC32 error at the PDAF Cal Data (0x%08X != 0x%08X)",
				sub_deviceIdx, checksum, buf32[checksum_base]);
			crc32_check_temp = false;
			goto out;
		}
	} else {
		pr_warn("Camera[%d]: Skip to check PDAF Cal Data crc32\n", sub_deviceIdx);
	}
...
static int imgsensor_eeprom_read_common(unsigned char* pEepromData, unsigned int deviceIdx, unsigned int sub_deviceIdx, unsigned int length, bool rom_common)
{
	...
	pr_err("DeviceID - %d, Sub_DeviceID - %d, Rom_common - %s", deviceIdx, sub_deviceIdx, rom_common?"true":"false");
	if(!finfo[sub_deviceIdx]) {
		finfo[sub_deviceIdx] = (struct imgsensor_rom_info *)kmalloc(sizeof(struct imgsensor_rom_info), GFP_KERNEL);
	}
	finfo_local = finfo[sub_deviceIdx];

	rom_addr = vendor_rom_addr[deviceIdx];
	if(!rom_addr) {
		pr_err("ROM ADDR is NULL. Device not configured");
		return -1;
	}
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	if (temp_start_addr >= 0) {
		finfo_local->oem_start_addr = *((u32 *)&pEepromData[temp_start_addr]);
		finfo_local->oem_end_addr = *((u32 *)&pEepromData[temp_end_addr]);
	...
	if (temp_start_addr >= 0) {
		finfo_local->awb_start_addr = *((u32 *)&pEepromData[temp_start_addr]);
		finfo_local->awb_end_addr = *((u32 *)&pEepromData[temp_end_addr]);
	...
	if (temp_start_addr >= 0) {
		finfo_local->shading_start_addr = *((u32 *)&pEepromData[temp_start_addr]);
		finfo_local->shading_end_addr = *((u32 *)&pEepromData[temp_end_addr]);
	...
	if (temp_start_addr >= 0) {
		finfo_local->sensor_cal_data_start_addr = *((u32 *)&pEepromData[temp_start_addr]);
		finfo_local->sensor_cal_data_end_addr = *((u32 *)&pEepromData[temp_end_addr]);
	...
	if (temp_start_addr >= 0) {
		finfo_local->dual_data_start_addr = *((u32 *)&pEepromData[temp_start_addr]);
		finfo_local->dual_data_end_addr = *((u32 *)&pEepromData[temp_end_addr]);
	...
	if (temp_start_addr >= 0) {
		finfo_local->ap_pdaf_start_addr = *((u32 *)&pEepromData[temp_start_addr]);
		finfo_local->ap_pdaf_end_addr = *((u32 *)&pEepromData[temp_end_addr]);
	...
	if (temp_start_addr >= 0) {
		memcpy(finfo_local->header_ver, &pEepromData[temp_start_addr], IMGSENSOR_HEADER_VER_SIZE);
	...
	if (rom_addr->rom_header_cal_map_ver_start_addr >= 0) {
		...
		memcpy(finfo_local->cal_map_ver, &pEepromData[temp_start_addr], IMGSENSOR_CAL_MAP_VER_SIZE);
	}
	...
	if (rom_addr->rom_header_project_name_start_addr >= 0) {
		...
		memcpy(finfo_local->project_name, &pEepromData[temp_start_addr], IMGSENSOR_PROJECT_NAME_SIZE);
	...
	if (rom_addr->rom_header_module_id_addr >= 0) {
		...
		memcpy(finfo_local->rom_module_id, &pEepromData[temp_start_addr], IMGSENSOR_MODULE_ID_SIZE);
	} else {
	...
	if (temp_start_addr >= 0) {
		memcpy(finfo_local->rom_sensor_id, &pEepromData[temp_start_addr], IMGSENSOR_SENSOR_ID_SIZE);
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	if (temp_start_addr >= 0) {
		memcpy(finfo_local->oem_ver, &pEepromData[temp_start_addr], IMGSENSOR_OEM_VER_SIZE);
	...
	if (temp_start_addr >= 0)
		finfo_local->af_cal_pan = *((u32 *)&pEepromData[temp_start_addr]);
	if (temp_end_addr >= 0)
		finfo_local->af_cal_macro = *((u32 *)&pEepromData[temp_end_addr]);
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	if (temp_start_addr >= 0) {
		memcpy(finfo_local->awb_ver, &pEepromData[temp_start_addr], IMGSENSOR_AWB_VER_SIZE);
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	if (temp_start_addr >= 0) {
		memcpy(finfo_local->shading_ver, &pEepromData[temp_start_addr], IMGSENSOR_SHADING_VER_SIZE);
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	if (temp_start_addr >= 0) {
		memcpy(finfo_local->dual_data_ver, &pEepromData[temp_start_addr], IMGSENSOR_DUAL_CAL_VER_SIZE);
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	if (temp_start_addr >= 0) {
		memcpy(finfo_local->sensor_cal_data_ver, &pEepromData[temp_start_addr], IMGSENSOR_SENSOR_CAL_DATA_VER_SIZE);
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	if (temp_start_addr >= 0) {
		memcpy(finfo_local->ap_pdaf_ver, &pEepromData[temp_start_addr], IMGSENSOR_AP_PDAF_VER_SIZE);
...
/* CRC */
/////////////////////////////////////////////////////////////////////////////////////
	if (!imgsensor_sec_check_eeprom_crc32(pEepromData, deviceIdx, sub_deviceIdx, rom_common) && (retry > 0)) {
		retry--;
		pr_err("Retry Remaining - %d", retry);
		goto crc_retry;
	}
	if (finfo_local->header_ver[3] == 'K')
		crc32_check_list[sub_deviceIdx][CRC32_CHECK_FACTORY] = crc32_check_list[sub_deviceIdx][CRC32_CHECK];
	else
		crc32_check_list[sub_deviceIdx][CRC32_CHECK_FACTORY] = false;

	if (specific->use_module_check) {
		/* Check this module is latest */
		if (finfo[sub_deviceIdx]->header_ver[10] >= rom_addr->camera_module_es_version) {
			check_latest_cam_module[sub_deviceIdx] = true;
		} else {
			check_latest_cam_module[sub_deviceIdx] = false;
		}
		/* Check this module is final for manufacture */
		if(finfo[sub_deviceIdx]->header_ver[10] == IMGSENSOR_LATEST_ROM_VERSION_M) {
			check_final_cam_module[sub_deviceIdx] = true;
		} else {
			check_final_cam_module[sub_deviceIdx] = false;
		}
	} else {
		check_latest_cam_module[sub_deviceIdx] = true;
		check_final_cam_module[sub_deviceIdx] = true;
	...
	if(g_cal_buf[sub_deviceIdx] == NULL)
		pr_err("%s: g_cal_buf[%d] is NULL", __func__, sub_deviceIdx);
	else {
		memcpy(g_cal_buf[sub_deviceIdx], pEepromData, g_cal_buf_size[sub_deviceIdx]);
	}
	return 0;
}

int imgsensor_sysfs_update(unsigned char* pEepromData, unsigned int dualDeviceId, unsigned int sensorId, unsigned int offset, unsigned int length, int i4RetValue)
{
	enum IMGSENSOR_SENSOR_IDX deviceIdx = IMGSENSOR_SENSOR_IDX_MAP(dualDeviceId);
	unsigned int sub_deviceIdx;
	unsigned int remapped_device_id;

	pr_err("%s : deviceID=%d, sensorID=0x%X, offset=%d, length=%d, i4RetValue=%d", __func__, deviceIdx, sensorId, offset, length, i4RetValue);
	if (deviceIdx == IMGSENSOR_SENSOR_IDX_NONE)
		return -1;

	// 0x4000 is preloadSizeLimit
	if(offset != 0 || length < 0x4000 || i4RetValue < 0x4000) {
		pr_err("%s : read size NG", __func__);
		return -1;
	}

	remapped_device_id = (int)map_position(deviceIdx);
	imgsensor_eeprom_read_common(pEepromData, remapped_device_id, remapped_device_id, length, false);
	init_cam_hwparam(remapped_device_id);
	if(cam_infos[deviceIdx].includes_sub) {
		sub_deviceIdx = cam_infos[deviceIdx].sub_sensor_id;
		pr_err("%s : sub_deviceID=%d", __func__, sub_deviceIdx);
		imgsensor_eeprom_read_common(pEepromData, remapped_device_id, sub_deviceIdx, length, true);
		init_cam_hwparam(sub_deviceIdx);
	}
#if ENABLE_CAM_CAL_DUMP
	imgsensor_sec_readcal_dump(pEepromData, length, remapped_device_id);
#endif
	return 0;
}
