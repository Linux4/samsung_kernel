...
#include "n8_gc2375a_qh_mipi_raw.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "n8_gc2375a_qh_camera_sensor"
#define LOG_1 LOG_INF("N8_GC2375A_QH, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
kal_bool N8_GC2375A_QHDuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = N8_GC2375A_QH_MIPI_SENSOR_ID, /* record sensor id defined in Kd_imgsensor.h */
	.checksum_value = 0xf7375923,    /* checksum value for Camera Auto Test */

	.pre = {
		.pclk = 78000000,
		.linelength = 2106,
		.framelength = 1248,//30fps  VB 0X10
	...
	.cap = {
		.pclk = 78000000,
		.linelength = 2106,
		.framelength = 1543,//1248-->1559  VB 311
		.startx = 0,
		.starty = 0,
...
static kal_uint8 n8_gc2375a_qh_table_write_cmos_sensor(
...
/* add begain for set dummy dual camera 24fps*/
static void set_dummy_dual(void)
{
	...
	vb = (vb < 311) ? 311 : vb;
	vb = (vb > 8191) ? 8191 : vb;//bug 612420,zhanghao2.wt,add,2021/2/20,add dual camera custom1 mode
	write_cmos_sensor(0x07, (vb >> 8) & 0x1F);
	write_cmos_sensor(0x08, vb & 0xFF);
}
/* add off for set dummy dual camera 24fps*/


static void set_dummy(void)//normal preview 30fps
{
	kal_uint32 vb = 16;
	kal_uint32 basic_line = 1232;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if ((iReg >= ANALOG_GAIN_1) && (iReg < ANALOG_GAIN_2)) {
		...
		write_cmos_sensor(0xb1, temp >> 6);
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF("N8_GC2375A_QHMIPI analogic gain 16.23x , N8_GC2375A_QHMIPI add pregain = %d\n", temp);
	}
	return gain;
}
...
#if MULTI_WRITE
kal_uint8 addr_data_pair_init_n8_gc2375a_qh[] = {
       0xfe, 0x00,
	0xfe, 0x00,
	0xfe, 0x00,
	0xf7, 0x01,
...
static void sensor_init(void)
{
	LOG_INF("E");
#if MULTI_WRITE
	n8_gc2375a_qh_table_write_cmos_sensor(
		addr_data_pair_init_n8_gc2375a_qh,
		sizeof(addr_data_pair_init_n8_gc2375a_qh) /
		sizeof(kal_uint8));
#else
	/* System */
	write_cmos_sensor(0xfe, 0x00);
	...
	write_cmos_sensor(0x17, N8_GC2375A_QH_MIRROR);
...
static kal_uint32 open(void)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	N8_GC2375A_QHDuringTestPattern = KAL_FALSE;
