...
#include "ov02b1b_jk_mipi_Sensor.h"


#define PFX "ov02b1b_jk_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV02B1B_JK_SENSOR_ID,
	...
	.pre = {
        ...
        .framelength = 1291,            //record different mode's framelength
	...
	.custom2 = {	//Copy from pre
		.pclk = 16500000,                //record different mode's pclk
		.linelength = 448,                //record different mode's linelength
		.framelength = 1534,            //record different mode's framelength
		.startx = 0,                    //record different mode's startx of grabwindow
		.starty = 0,                    //record different mode's starty of grabwindow
		.grabwindow_width = 1600,        //record different mode's width of grabwindow
		.grabwindow_height = 1200,        //record different mode's height of grabwindow
...
/* A03s code for SR-AL5625-01-332 by xuxianwei at 2021/05/18 start */
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_MONO,//sensor output first pixel color
/* A03s code for SR-AL5625-01-332 by xuxianwei at 2021/05/18 end */
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[7]={
	...
	{  1600, 1200,  0, 0, 1600, 1200, 1600, 1200,
	   0000, 0000, 1600, 1200,  0, 0, 1600, 1200}, /* custom1 */
	{  1600, 1200,  0, 0, 1600, 1200, 1600, 1200,
	   0000, 0000, 1600, 1200,  0, 0, 1600, 1200}, /* custom2 */
};
/* hs03s code for CAM-AL5625-01-247 by majunfeng at 2021/05/27 end */
...
//auroflicker:need to avoid 15fps and 30 fps
	...
	//pr_info("shutter =%d, framelength =%d, realtime_fps =%d\n",
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if((gain >= 0x40) && (gain <= (15*0x40))) //base gain = 0x40
	{
		...
		if(iReg<=0x10)
		{
			...
			write_cmos_sensor(0xfe, 0x02);	//fresh 
			pr_info("OV02B1B_JK_MIPI_SetGain = 16");
		}
		else if(iReg>= 0xf8)//gpw
		{
			...
			write_cmos_sensor(0xfe, 0x02);	//fresh 
			pr_info("OV02B1B_JK_MIPI_SetGain = 160");
		}
		else
		{
			write_cmos_sensor(0xfd, 0x01);
			...
			write_cmos_sensor(0xfe, 0x02);	//fresh 
			pr_info("OV02B1B_JK_MIPI_SetGain = %d",iReg);
		}
	}
	else
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	switch (image_mirror) {
		case IMAGE_NORMAL:
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x12, 0x00);   SENSOR_OUTPUT_FORMAT_RAW_B 
			break;

		case IMAGE_H_MIRROR:
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x12, 0x02);   SENSOR_OUTPUT_FORMAT_RAW_Gb 
			break;

		case IMAGE_V_MIRROR:
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x12, 0x01);   SENSOR_OUTPUT_FORMAT_RAW_Gr 
			break;

		case IMAGE_HV_MIRROR:
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x12, 0x03);   SENSOR_OUTPUT_FORMAT_RAW_R 
			break;
		default:
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		pr_info("Read id fail,sensor id: 0x%x,write id: 0x%x\n", *sensor_id,imgsensor.i2c_write_id);
...
static kal_uint32 open(void)
{
	...
	pr_info("[open]: PLATFORM:MT6737,MIPI 24LANE\n");
	pr_info("preview 1296*972@30fps,360Mbps/lane;"
	...
	if (imgsensor_info.sensor_id != sensor_id) {
		pr_info("open sensor id fail: 0x%x\n", sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
			...
			MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		...
		spin_unlock(&imgsensor_drv_lock);
			...
			pr_info("fps %d fps not support,use cap: %d fps!\n",
...
{
	pr_info("streaming_control enable =%d\n", enable);
	if (enable){
		write_cmos_sensor(0xfd, 0x03);
		write_cmos_sensor(0xc2, 0x01);
