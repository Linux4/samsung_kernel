...
#include "n26_c2599_micro_delta_mipi_sensor.h"



/****************************Modify Following Strings for Debug****************************/
#define PFX "C2599_SensroDriver"
#define LOG_1 LOG_INF("C2599, MIPI 1LANE\n")

#define C2599MIPIRAW_DEBUG
#ifdef C2599MIPIRAW_DEBUG
#define C2599MIPISENSORDB printk
#else
#define C2599MIPISENSORDB(x,...)
#endif
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = N26_C2599_MICRO_DELTA_SENSOR_ID,
...
{
    //C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);

    kal_uint32 frame_length = imgsensor.frame_length;
	
    C2599MIPISENSORDB("[lj c2519]>>> set_max_framerate():framerate = %d, min framelength should enable = %d\n", framerate,min_framelength_en);
...
static void set_shutter(kal_uint16 shutter)
{
    ...
    C2599MIPISENSORDB("[lj c2519]set_shutter(): shutter =%d\n", shutter);
	...
	C2599MIPISENSORDB("[lj c2519]set_shutter(): shutter =%d, framelength =%d,mini_framelegth=%d\n", shutter,imgsensor.frame_length,imgsensor.min_frame_length);
}  
/*************************************************************************
* FUNCTION
*    set_gain
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 tmp0 = 0, tmp1 = 0, tmp2 = 0, tmp3 = 0, tmp4 = 0, tmp5 = 0, i = 0, Again_base = 0;
	kal_uint16 iGain=gain;
    C2599MIPISENSORDB("[lj c2519] C2599 isp again = %x\n",(gain & 0xFF));
...
static void sensor_init(void)
{
     ...
     write_cmos_sensor(0x0103,0x01); //sensor software reset
     write_cmos_sensor(0x0101,0x03); //add mirror&flip for n26
     ...
     C2599MIPISENSORDB("[lj c2519]sensor_init end\n");
}
...
{
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
	C2599MIPISENSORDB("[lj c2519]E! preview_setting currefps:%d\n",currefps);                                                               
	//sensor_init();
...
{
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2519]E! capture_setting currefps:%d\n",currefps);
	//sensor_init();             
	 //write_cmos_sensor(0x0100,0x01);  //streaming starts 
...
{
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2519]enable: %d\n", enable);
    spin_lock(&imgsensor_drv_lock);
    spin_unlock(&imgsensor_drv_lock);
    return ERROR_NONE;
}

//+Bug 720367, liudijin.wt, add, 2022.03.02, bring-up OTP for micro camera c2599
#define EEPROM_P24C64Q_ID 0xA4

#include "cam_cal_define.h"
#include <linux/slab.h>
static struct stCAM_CAL_DATAINFO_STRUCT n26_c2599_micro_delta_eeprom_data ={
	.sensorID= N26_C2599_MICRO_DELTA_SENSOR_ID,
	.deviceID = 0x08,
	.dataLength = 0x0778,
	.sensorVendorid = 0x1d020101,
	.vendorByte = {1,2,3,4},
	.dataBuffer = NULL,
};
static struct stCAM_CAL_CHECKSUM_STRUCT n26_c2599_micro_delta_checksum[8] =
{
...
static kal_uint16 read_eeprom(kal_uint32 addr)
{
	...
	char pu_send_cmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };
	iReadRegI2C(pu_send_cmd, 2, (u8*)&get_byte, 1, EEPROM_P24C64Q_ID);
        ...
        do {
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            if (*sensor_id == imgsensor_info.sensor_id) {
                //+Bug 720367, liudijin.wt, add, 2022.03.02, bring-up OTP for micro camera c2599
                size = imgSensorReadEepromData(&n26_c2599_micro_delta_eeprom_data,n26_c2599_micro_delta_checksum);
                if(size != n26_c2599_micro_delta_eeprom_data.dataLength || (n26_c2599_micro_delta_eeprom_data.sensorVendorid >> 24)!= read_eeprom(0x0001)) {
                    LOG_INF(" n26_c2599_micro_delta get eeprom data failed   *sensor_id:0x%x \n", *sensor_id);
                    if(n26_c2599_micro_delta_eeprom_data.dataBuffer != NULL) {
                        kfree(n26_c2599_micro_delta_eeprom_data.dataBuffer);
                        n26_c2599_micro_delta_eeprom_data.dataBuffer = NULL;
                    }
                    *sensor_id = 0xFFFFFFFF;
                    return ERROR_SENSOR_CONNECT_FAIL;
                } else {
                    LOG_INF("n26_c2599_micro_delta get eeprom data success\n");
                    imgSensorSetEepromData(&n26_c2599_micro_delta_eeprom_data);
                }
                //-Bug 720367, liudijin.wt, add, 2022.03.02, bring-up OTP for micro camera c2599

                LOG_INF("[n26_c2599_micro_delta] get_imgsensor_id OK,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("[n26_c2599_micro_delta] get_imgsensor_id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    printk("[lj c2599]func= %s \n",__func__);
	//hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1800,"C2599");
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("[lj c2599]open:Read sensor id OK,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk("[lj c2599]Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
{
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
...
{
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2599]>>> preview()\n");
                          ...
                          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    C2599MIPISENSORDB("[lj c2519]<<< capture()\n");
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    C2599MIPISENSORDB("[lj c2519]<<< normal_video()\n");
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    C2599MIPISENSORDB("[lj c2519]<<< hs_video()\n");
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    C2599MIPISENSORDB("[lj c2519]<<< slim_video()\n");
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
    spin_lock(&imgsensor_drv_lock);
...
{
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2519]>>> get_resolution()\n");
    ...
    C2599MIPISENSORDB("[lj c2519]>>> feature_control()\n");
    return ERROR_NONE;
}    /*    get_resolution    */
...
static kal_uint32 get_info(enum MSDK_SCENARIO_ID_ENUM scenario_id,
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    C2599MIPISENSORDB("[lj c2519]>>> get_info():scenario_id = %d\n", scenario_id);
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
	...
	C2599MIPISENSORDB("[lj c2519]<<< get_info()\n");
...
{
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2519]>>> control() scenario_id = %d\n", scenario_id);
    spin_lock(&imgsensor_drv_lock);
    ...
    switch (scenario_id) {
        ...
        case MSDK_SCENARIO_ID_SLIM_VIDEO:
            ...
            break;
        default:
            C2599MIPISENSORDB("[lj c2519]Error ScenarioId setting\n");
            preview(image_window, sensor_config_data);
            return ERROR_INVALID_SCENARIO_ID;
    }

	
	C2599MIPISENSORDB("[lj c2519]<<< control()\n");
    return ERROR_NONE;
}    /* control() */
...
{
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2519]framerate = %d\n ", framerate);
...
{
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2519]>>>set_auto_flicker_mode():enable = %d, framerate = %d \n", enable, framerate);
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2519]>>>>set_max_framerate_by_scenario():scenario_id = %d, framerate = %d\n", scenario_id, framerate);
    ...
    C2599MIPISENSORDB("[lj c2519]<<<set_max_framerate_by_scenario()\n");
...
{
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2519]scenario_id = %d\n", scenario_id);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    C2599MIPISENSORDB("[lj c2519]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2519]>>> feature_control():feature_id = %d\n", feature_id);
    switch (feature_id) {
...
//bug 558061, zhanghengyuan.wt, modify, 2020/12/16,add featue for mt6833
	/*	
        case SENSOR_FEATURE_GET_PERIOD:
			C2599MIPISENSORDB("[lj c2519 feature_control] line = %d \n",__LINE__);
            *feature_return_para_16++ = imgsensor.line_length;
            *feature_return_para_16 = imgsensor.frame_length;
            *feature_para_len=4;
			C2599MIPISENSORDB("[lj c2519][C2599_SensroDriver]%d,%d\n",imgsensor.line_length,imgsensor.frame_length);
            break;
        case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
		C2599MIPISENSORDB("[lj c2519 feature_control] line = %d \n",__LINE__);
            *feature_return_para_32 = imgsensor.pclk;
            *feature_para_len=4;
			C2599MIPISENSORDB("[lj c2519][C2599_SensroDriver]%d\n",imgsensor.pclk);
            break;
        case SENSOR_FEATURE_SET_ESHUTTER:
		C2599MIPISENSORDB("[lj c2519 feature_control] line = %d \n",__LINE__);
            set_shutter(*feature_data);
        ...
        case SENSOR_FEATURE_SET_FRAMERATE:
		C2599MIPISENSORDB("[lj c2519 feature_control] line = %d \n",__LINE__);
            C2599MIPISENSORDB("[lj c2519]current fps :%d\n", (UINT32)*feature_data);
            spin_lock(&imgsensor_drv_lock);
        ...
        case SENSOR_FEATURE_GET_CROP_INFO:
		C2599MIPISENSORDB("[lj c2519 feature_control] line = %d \n",__LINE__);
            C2599MIPISENSORDB("[lj c2519]SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n", (UINT32)*feature_data);
