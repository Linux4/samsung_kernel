...
#include "n26_sc201cs_micro_lce_mipi_raw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "n26_sc201cs_micro_lce_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
    .sensor_id = N26_SC201CS_MICRO_LCE_SENSOR_ID,
...
//+Bug 720367, liudijin.wt, add, 2022.03.02, bring-up OTP for micro camera sc201cs
#define EEPROM_P24C64Q_ID 0xA4

#include "cam_cal_define.h"
#include <linux/slab.h>
static struct stCAM_CAL_DATAINFO_STRUCT n26_sc201cs_micro_lce_eeprom_data ={
	.sensorID= N26_SC201CS_MICRO_LCE_SENSOR_ID,
	.deviceID = 0x08,
	.dataLength = 0x0778,
	.sensorVendorid = 0x570201FF,
	.vendorByte = {1,2,3,4},
	.dataBuffer = NULL,
};
static struct stCAM_CAL_CHECKSUM_STRUCT n26_sc201cs_micro_lce_checksum[8] =
{
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		//+Bug 720367, liudijin.wt, add, 2022.03.02, bring-up OTP for micro camera sc201cs
		size = imgSensorReadEepromData(&n26_sc201cs_micro_lce_eeprom_data,n26_sc201cs_micro_lce_checksum);
		if(size != n26_sc201cs_micro_lce_eeprom_data.dataLength || (n26_sc201cs_micro_lce_eeprom_data.sensorVendorid >> 24)!= read_eeprom(0x0001)) {
			LOG_INF("n26_sc201cs_micro_lce get eeprom data faile  sensor_id:0x%x\n", *sensor_id);
			if(n26_sc201cs_micro_lce_eeprom_data.dataBuffer != NULL) {
				kfree(n26_sc201cs_micro_lce_eeprom_data.dataBuffer);
				n26_sc201cs_micro_lce_eeprom_data.dataBuffer = NULL;
			}
			*sensor_id = 0xFFFFFFFF;
			return ERROR_SENSOR_CONNECT_FAIL;
		} else {
			LOG_INF("n26_sc201cs_micro_lce get eeprom data success\n");
			imgSensorSetEepromData(&n26_sc201cs_micro_lce_eeprom_data);
		}
		//-Bug 720367, liudijin.wt, add, 2022.03.02, bring-up OTP for micro camera sc201cs

                LOG_INF("n26_sc201cs_micro_lce Read sensor id  success i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("n26_sc201cs_micro_lce Read sensor id fail, i2c_write_id: 0x%x  sensor_id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
    ...
    if (*sensor_id != imgsensor_info.sensor_id) {
        ...
        LOG_INF("SC201CS Read id fail,dennis sensor id: 0x%x\n", *sensor_id);
        *sensor_id = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
    }
    return ERROR_NONE;
...
static kal_uint32 open(void)
{
	...
	LOG_INF("SC201CS,MIPI 1LANE\n");
	LOG_INF("preview 1600*1200@30fps,864Mbps/lane; video 1600*1200@30fps,864Mbps/lane; capture 2M@30fps,864Mbps/lane\n");
	 ...
	 while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		 ...
		 do {
			 sensor_id = return_sensor_id();
			 if (sensor_id == imgsensor_info.sensor_id) {				 
				 LOG_INF("Read sensor success addr: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);   
				 break;
			 }	 
			 LOG_INF("Read sensor id fail, addr: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			 retry--;
		 } while(retry > 0);
		 i++;
		 if (sensor_id == imgsensor_info.sensor_id)
