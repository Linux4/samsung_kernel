...
#include "n8_hi846_sht_mipi_raw.h"

#define PFX "n8_hi846_sht_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = N8_HI846_SHT_SENSOR_ID,
...
static kal_uint16 n8_hi846_sht_table_write_cmos_sensor(
...
#if MULTI_WRITE
static kal_uint16 addr_data_pair_init_n8_hi846_sht[] = {
	0x2000, 0x987A, 
	0x2002, 0x00FF,
	0x2004, 0x0047, 
	0x2006, 0x3FFF,
...
{
#if MULTI_WRITE
	n8_hi846_sht_table_write_cmos_sensor(
		addr_data_pair_init_n8_hi846_sht,
		sizeof(addr_data_pair_init_n8_hi846_sht) /
		sizeof(kal_uint16));
#else
...
#if MULTI_WRITE
static kal_uint16 addr_data_pair_preview_n8_hi846_sht[] = {
	0x002E, 0x3311,
	0x0032, 0x3311,
	0x0026, 0x0040,
	0x002C, 0x09CF,
...
#if MULTI_WRITE
	n8_hi846_sht_table_write_cmos_sensor(
		addr_data_pair_preview_n8_hi846_sht,
		sizeof(addr_data_pair_preview_n8_hi846_sht) /
		sizeof(kal_uint16));
#else
	LOG_INF("E preview\n");
	//Sensor Information////////////////////////////
...
#if MULTI_WRITE
static kal_uint16 addr_data_pair_capture_fps_n8_hi846_sht[] = {
	0x002E, 0x1111,
	0x0032, 0x1111,
	0x0026, 0x0040,
	0x002C, 0x09CF,
...
static kal_uint16 addr_data_pair_capture_30fps_n8_hi846_sht[] = {
	0x002E, 0x1111,
	0x0032, 0x1111,
	0x0026, 0x0040,
	0x002C, 0x09CF,
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 300) {
	n8_hi846_sht_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_n8_hi846_sht,
		sizeof(addr_data_pair_capture_30fps_n8_hi846_sht) /
		sizeof(kal_uint16));
	} else {
	n8_hi846_sht_table_write_cmos_sensor(
		addr_data_pair_capture_fps_n8_hi846_sht,
		sizeof(addr_data_pair_capture_fps_n8_hi846_sht) /
		sizeof(kal_uint16));
	}
#else
	if (currefps == 300) {
...
#if MULTI_WRITE
static kal_uint16 addr_data_pair_video_n8_hi846_sht[] = {
	0x002E, 0x1111,
	0x0032, 0x1111,
	0x0026, 0x0040,
	0x002C, 0x09CF,
...
#if MULTI_WRITE
	n8_hi846_sht_table_write_cmos_sensor(
		addr_data_pair_video_n8_hi846_sht,
		sizeof(addr_data_pair_video_n8_hi846_sht) /
		sizeof(kal_uint16));
#else
	  //Sensor Information////////////////////////////
	  //Sensor		: Hi-846
...
#if MULTI_WRITE
static kal_uint16 addr_data_pair_hs_video_n8_hi846_sht[] = {
	0x002E, 0x7711,
	0x0032, 0x7711,
	0x0026, 0x0148,
	0x002C, 0x08C7,
...
#if MULTI_WRITE
	n8_hi846_sht_table_write_cmos_sensor(
		addr_data_pair_hs_video_n8_hi846_sht,
		sizeof(addr_data_pair_hs_video_n8_hi846_sht) /
		sizeof(kal_uint16));
#else
...
#if MULTI_WRITE
static kal_uint16 addr_data_pair_slim_video_n8_hi846_sht[] = {
	0x002E, 0x3311,
	0x0032, 0x3311,
	0x0026, 0x0238,
	0x002C, 0x07D7,
...
#if MULTI_WRITE
	n8_hi846_sht_table_write_cmos_sensor(
		addr_data_pair_slim_video_n8_hi846_sht,
		sizeof(addr_data_pair_slim_video_n8_hi846_sht) /
		sizeof(kal_uint16));
#else
	//Sensor Information////////////////////////////
	//Sensor	  : Hi-846
...
static struct stCAM_CAL_DATAINFO_STRUCT hi846_sht_eeprom_data ={
	.sensorID= N8_HI846_SHT_SENSOR_ID,
	.deviceID = 0x02,
	.dataLength = 0x075E,
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_otp_n8_hi846_sht[] = {
	0x0A00, 0x0000,          
	0x2000, 0x0000,
	0x2002, 0x00FF,
	0x2004, 0x0000,
...
static kal_uint32 OTP_EEPROM_Read(kal_uint32 uiStartAddr, kal_uint8* pwData, kal_uint32 uiDataAmount)
{
	...
	for ( i = 0; i < uiDataAmount; i++)
	{
		...
		pwData[i] = iicRtn[0];
		//LOG_INF("pwData[%d] = 0x%x", i, pwData[i]);
	}

	return ERROR_NONE;
}

static kal_int32 hi846_sht_get_sensor_otp_info(struct stCAM_CAL_DATAINFO_STRUCT*pData)
{
	kal_uint8 flaginfo = 0, flagawb = 0, flaglsc = 0;
	...
	#if MULTI_WRITE
	n8_hi846_sht_table_write_cmos_sensor(
		addr_data_pair_init_otp_n8_hi846_sht,
		sizeof(addr_data_pair_init_otp_n8_hi846_sht) /
		sizeof(kal_uint16));
	mdelay(1);
	#else
	write_cmos_sensor(0x0A00, 0x0000);			
	...
	LOG_INF("module flaginfo = 0x%x", flaginfo);
	if (flaginfo == 0x1) {
		uiStartAddr = 0x0202;
		...
		LOG_INF("module flaginfo is not match");
		goto err;
	}
	cnt = 8;
	uiOffset = 0;
	OTP_EEPROM_Read(uiStartAddr, pData->dataBuffer+uiOffset, cnt);
	if (((pData->sensorVendorid)>>24) != pData->dataBuffer[0]) {
		LOG_INF("module info module id is not match (%x, %x)", ((pData->sensorVendorid)>>24), pData->dataBuffer[0]);
		goto err;
	}
	for (i = 0; i < cnt-1; i++) {
		sum += pData->dataBuffer[i];
	}
	LOG_INF("pData->dataBuffer[%d] = 0x%x, sum = 0x%x, 0x%x\n", cnt+uiOffset-1, pData->dataBuffer[cnt+uiOffset-1], sum, ((sum %255) + 1));
	if ( ((sum %255) + 1) == pData->dataBuffer[cnt+uiOffset-1]) {
		LOG_INF("module info checksum OK \n");
	} else {
		LOG_INF("module info checksum fail");
		goto err;
	}

	//get awb info
	OTP_EEPROM_Read(0x021A, &flagawb, 1);
	LOG_INF("awb flagawb = 0x%x", flagawb);
	if (flagawb == 0x1) {
		uiStartAddr = 0x021B;
	} else if (flagawb == 0x13) {
		uiStartAddr = 0x0224;
	} else if (flagawb == 0x37) {
		uiStartAddr = 0x022D;
	} else {
		LOG_INF("awb flaginfo is not match");
		goto err;
	}
	cnt = 9;
	uiOffset = 8;
	...
	OTP_EEPROM_Read(uiStartAddr, pData->dataBuffer+uiOffset, cnt);
	for (i = 0; i < cnt-1; i++) {
		sum += pData->dataBuffer[i+uiOffset];
	}
	LOG_INF("pData->dataBuffer[%d] = 0x%x, sum = 0x%x, 0x%x\n", cnt+uiOffset-1, pData->dataBuffer[cnt+uiOffset-1], sum, ((sum %255) + 1));
	if ( ((sum %255) + 1) == pData->dataBuffer[cnt+uiOffset-1]) {
		LOG_INF("awb info checksum OK \n");
	} else {
		LOG_INF("awb info checksum fail");
		goto err;
	}

	//get lsc info
	OTP_EEPROM_Read(0x0236, &flaglsc, 1);
	LOG_INF("lsc flaglsc = 0x%x", flaglsc);
	if (flaglsc == 0x1) {
		uiStartAddr = 0x0237;
	} else if (flaglsc == 0x13) {
		uiStartAddr = 0x0984;
	} else if (flaglsc == 0x37) {
		uiStartAddr = 0x10D1;
	} else {
		LOG_INF("lsc flaginfo is not match");
		goto err;
	}
	cnt = 1869;
	uiOffset = 17;
	...
	LOG_INF("pData->dataBuffer[%d] = 0x%x, sum = 0x%x, 0x%x\n", cnt+uiOffset-1, pData->dataBuffer[cnt+uiOffset-1], sum, ((sum %255) + 1));
	if ( ((sum %255) + 1) == pData->dataBuffer[cnt+uiOffset-1]) {
		LOG_INF("lsc info checksum OK \n");
	} else {
		LOG_INF("lsc info checksum fail");
		goto err;
	}

	flaginfo = read_cmos_sensor(0x0A05);
	LOG_INF("flag 0A05 = 0x%x \n", flaginfo);
	flaginfo |= 0x10;
	write_cmos_sensor_8(0x0A05, flaginfo);
	flaginfo = read_cmos_sensor(0x0A05);
...
static void hi846_sht_apply_awb_otp_info(struct stCAM_CAL_DATAINFO_STRUCT*pData){
	kal_uint16 R_unit = 0;
	...
	kal_uint32 R_gain = 0x200;//test value 0x500
	kal_uint32 G_gain = 0x200;
	kal_uint32 B_gain = 0x200;

	if (pData == NULL || pData->dataBuffer == NULL) {
		LOG_INF("pData is NULL, apply awb info fail\n");
		return;
	}

	R_unit  = pData->dataBuffer[8]&0xFF;
	B_unit = pData->dataBuffer[9]&0xFF;
	Gr_unit = pData->dataBuffer[10]&0xFF;
	Gb_unit  = pData->dataBuffer[11]&0xFF;
	LOG_INF(" R_unit = 0x%x Gr_unit = 0x%x Gb_unit = 0x%x B_unit = 0x%x   \n", R_unit, Gr_unit, Gb_unit, B_unit);

	R_golden  = pData->dataBuffer[12]&0xFF;
	B_golden = pData->dataBuffer[13]&0xFF;
	Gr_golden = pData->dataBuffer[14]&0xFF;
	Gb_golden  = pData->dataBuffer[15]&0xFF;
...
{
	kal_uint8 i = 0;
	kal_uint8 retry = 5; //Type Bug452271,wanghui5.wt,modify,2019.06.19,add read camera id to 5 times
	kal_int32 ret = -1;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				//Type Bug431177 + , chenshengqian.wt, modify, 2019.03.14, otp funtion, multi cam compatibility
				ret = hi846_sht_get_sensor_otp_info(&hi846_sht_eeprom_data);
				if(ret < 0) {
					LOG_INF("get eeprom data failed, ret=%d, datalength=%d\n", ret, hi846_sht_eeprom_data.dataLength);
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				} else {
					LOG_INF("get eeprom data success\n");
					imgSensorSetEepromData(&hi846_sht_eeprom_data);
				}
				//Type Bug431177 - , chenshengqian.wt, modify, 2019.03.14, otp funtion, multi cam compatibility
				return ERROR_NONE;
			}
		...
		i++;
		retry = 5; //Type Bug452271,wanghui5.wt,modify,2019.06.19,add read camera id to 5 times
	}
	if (*sensor_id != imgsensor_info.sensor_id) {
...
static kal_uint32 open(void)
{
	...
	//Type Bug431177 + , chenshengqian.wt, modify, 2019.03.14, otp funtion, multi cam compatibilit
	hi846_sht_apply_awb_otp_info(&hi846_sht_eeprom_data);
	//Type Bug431177 - , chenshengqian.wt, modify, 2019.03.14, otp funtion, multi cam compatibilit
