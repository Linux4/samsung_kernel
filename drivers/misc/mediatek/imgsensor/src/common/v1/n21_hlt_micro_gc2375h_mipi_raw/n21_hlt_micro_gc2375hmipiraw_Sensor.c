...
#include "n21_hlt_micro_gc2375hmipiraw_Sensor.h"

/**************** Modify Following Strings for Debug ******************/
#define PFX "gc2375h_camera_sensor"
#define LOG_1 cam_pr_debug("N21_HLT_MICRO_GC2375H, MIPI 1LANE\n")
...
#define EEPROM_BL24SA64B_ID 0xA4
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = N21_HLT_MICRO_GC2375H_SENSOR_ID,
...
#if N21_HLT_MICRO_GC2375H_MIRROR_FLIP_ENABLE
...
kal_uint16 addr_data_pair_init_n21_hlt_micro_gc2375h[] = {
    0xfe, 0x00,
    0xfe, 0x00,
    0xfe, 0x00,
    0xf7, 0x01,
    ...
    0x0f, 0x06,
    0x10, 0x48,
    0x17, N21_HLT_MICRO_GC2375H_MIRROR,
    0x1c, 0x10,
    0x1d, 0x13,
    0x20, 0x0b,
    0x21, 0x6d,
    ...
    0x43, 0x03,
    0x4a, 0x00,
    0x4e, N21_HLT_MICRO_GC2375H_BLK_Select1_H,
    0x4f, N21_HLT_MICRO_GC2375H_BLK_Select1_L,
    0x66, N21_HLT_MICRO_GC2375H_BLK_Select2_H,
    0x67, N21_HLT_MICRO_GC2375H_BLK_Select2_L,
    0x68, 0x00,
    0xb0, 0x58,
    0xb1, 0x01,
    0xb2, 0x00,
...
static void sensor_init(void)
{
    ...
    addr_data_pair_init_n21_hlt_micro_gc2375h,
    sizeof(addr_data_pair_init_n21_hlt_micro_gc2375h) /
    sizeof(kal_uint16));
#else
	/* System */
	write_cmos_sensor(0xfe, 0x00);
	...
	write_cmos_sensor(0x17, N21_HLT_MICRO_GC2375H_MIRROR);
	...
	write_cmos_sensor(0x4e, N21_HLT_MICRO_GC2375H_BLK_Select1_H);
	write_cmos_sensor(0x4f, N21_HLT_MICRO_GC2375H_BLK_Select1_L);
	write_cmos_sensor(0x66, N21_HLT_MICRO_GC2375H_BLK_Select2_H);
	write_cmos_sensor(0x67, N21_HLT_MICRO_GC2375H_BLK_Select2_L);
...
static struct stCAM_CAL_DATAINFO_STRUCT hlt_micro_gc2375h_eeprom_data ={
	.sensorID= N21_HLT_MICRO_GC2375H_SENSOR_ID,
	.deviceID = 0x10,
	.dataLength = 0x0E5E,
	.sensorVendorid = 0x10050100,
	.vendorByte = {1,2,3,4},
	.dataBuffer = NULL,
};
static struct stCAM_CAL_CHECKSUM_STRUCT hlt_micro_gc2375h_Checksum[8] =
{
		...
		do {
			*sensor_id = return_sensor_id() + 3;
			if (*sensor_id == imgsensor_info.sensor_id) {
				//+Bug 492443, sunhushan.wt, ADD, 2019.10.15, porting the otp of the 12sensor 
				size = imgSensorReadEepromData(&hlt_micro_gc2375h_eeprom_data,hlt_micro_gc2375h_Checksum);
				if(size != hlt_micro_gc2375h_eeprom_data.dataLength || (hlt_micro_gc2375h_eeprom_data.sensorVendorid >> 24)!= read_eeprom(0x0001)) {
					printk("get eeprom data failed\n");
					if(hlt_micro_gc2375h_eeprom_data.dataBuffer != NULL) {
						kfree(hlt_micro_gc2375h_eeprom_data.dataBuffer);
						hlt_micro_gc2375h_eeprom_data.dataBuffer = NULL;
					}
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				} else {
					LOG_INF("get eeprom data success\n");
					imgSensorSetEepromData(&hlt_micro_gc2375h_eeprom_data);
				}
				//-Bug 492443, sunhushan.wt, ADD, 2019.10.15, porting the otp of the 12sensor 
