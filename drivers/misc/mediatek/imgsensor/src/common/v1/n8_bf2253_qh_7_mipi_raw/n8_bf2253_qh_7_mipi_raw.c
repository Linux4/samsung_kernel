...
#include "n8_bf2253_qh_7_mipi_raw.h"


/****************************Modify Following Strings for Debug****************************/
#define PFX "n8_bf2253_qh_7_camera_sensor"
#define LOG_1 LOG_INF("N8_BF2253_QH_7, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
kal_bool N8_BF2253_QH_7DuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = N8_BF2253_QH_7_MIPI_SENSOR_ID,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = return_sensor_id() + 6; //bug 534796 , sunhushan.wt, 2020.2.26, fix build errorn
                        LOG_INF("hi1336_xl_jct current_sensor_id =0x%x",current_sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id && N8_HI1336_XL_JCT_SENSOR_ID == current_sensor_id) {
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			sensor_id = return_sensor_id() + 6; //bug 534796 , sunhushan.wt, 2020.2.26, fix build error
	...
	spin_unlock(&imgsensor_drv_lock);
	N8_BF2253_QH_7DuringTestPattern = KAL_FALSE;
	return ERROR_NONE;
}
