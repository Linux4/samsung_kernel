...
#include "gc8054_hltmipiraw_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc8054_hlt_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC8054_HLT_SENSOR_ID,
...
static void gc8054_otp_init(void)
{
	...
	write_cmos_sensor_8bit(0x0317, 0x08);
	write_cmos_sensor_8bit(0x0a67, 0x80);
	write_cmos_sensor_8bit(0x0313, 0x00);
	write_cmos_sensor_8bit(0x0a54, 0x00);
	write_cmos_sensor_8bit(0x0a65, 0x05);
	write_cmos_sensor_8bit(0x0a68, 0x11);
	write_cmos_sensor_8bit(0x0a59, 0x00);
}

static void gc8054_otp_close(void)
{
	write_cmos_sensor_8bit(0x0317, 0x00);
	write_cmos_sensor_8bit(0x0a67, 0x00);
}

static void gc8054_otp_write_group(kal_uint16 addr)
{
	write_cmos_sensor_8bit(0x0313, 0x00);
...
kal_uint16 gc8054_init_addr_data[] = {
	/*system*/
	0x031c, 0x60,
	0x0320, 0xbb,
	0x0337, 0x05,
...
kal_uint16 gc8054_1632x1224_addr_data[] = {
	/*system*/
	0x031c, 0x60,
	0x0320, 0xbb,
	0x0337, 0x05,
...
kal_uint16 gc8054_3264x2448_addr_data[] = {
	/*system*/
	0x031c, 0x60,
	0x0320, 0xbb,
	0x0337, 0x05,
...
{
	table_write_cmos_sensor(gc8054_init_addr_data,
		sizeof(gc8054_init_addr_data)/sizeof(kal_uint16));
}

static void preview_setting(void)
{
	gc8054_stream_off();
	table_write_cmos_sensor(gc8054_3264x2448_addr_data,
		sizeof(gc8054_3264x2448_addr_data)/sizeof(kal_uint16));
	gc8054_stream_on();
}
...
{
	gc8054_stream_off();
	table_write_cmos_sensor(gc8054_1632x1224_addr_data,
		sizeof(gc8054_1632x1224_addr_data)/sizeof(kal_uint16));
	gc8054_stream_on();
}
...
/*TabA7 Lite code for SR-AX3565-01-320 by wangqi at 20201224 start*/
	kal_uint16 module_flag = 0;
	kal_uint16 vendor_id = 0;
	kal_uint32 flag_addr = 0x1410;
	kal_uint32 module_addr = 0x0;
	gc8054_otp_init();

	gc8054_otp_write_group(flag_addr);
	module_flag = read_cmos_sensor(0x0a6c);
    switch (module_flag)
    {
        case 0x01:
			module_addr = 0x1418;
			pr_debug("group1 module_addr = 0x%x\n", module_addr);
			break;
        case 0x13:
            module_addr = 0x1490;
			pr_debug("group2 module_addr = 0x%x\n", module_addr);
			break;
        case 0x37:
            module_addr = 0x1508;
			pr_debug("group3 module_addr = 0x%x\n", module_addr);
			break;
    }
	gc8054_otp_write_group(module_addr);
	vendor_id = read_cmos_sensor(0x0a6c);
	pr_err("vendor_id = 0x%x\n", vendor_id);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			if ((*sensor_id == imgsensor_info.sensor_id) && (vendor_id == 0x55)){
...
/*TabA7 Lite code for SR-AX3565-01-320 by wangqi at 20201224 start*/
	if(vendor_id != 0x55){
		pr_err("is not hlt, vendor_id != 0x55,vendor_id = 0x%x\n", vendor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
/*TabA7 Lite code for SR-AX3565-01-320 by wangqi at 20201224 end*/
