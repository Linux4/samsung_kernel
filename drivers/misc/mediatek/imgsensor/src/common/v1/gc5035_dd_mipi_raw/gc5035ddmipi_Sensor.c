...
#include "gc5035ddmipi_Sensor.h"

/**************** Modify Following Strings for Debug ******************/
#define PFX "gc5035_dd_camera_sensor"
...
/* A03s code for SR-AL5625-01-324 by wuwenjie at 2021/05/20 start */
	.sensor_id = GC5035_DD_SENSOR_ID,
	.checksum_value = 0xcde448ca,
/*hs03s code for DEVAL5625-427 by gaozhenyu at 2021/06/02 start*/
	.pre = {
		.pclk = 175200000,
		.linelength = 2920,
		.framelength = 2008,
...
/*hs03s code for DEVAL5625-427 by gaozhenyu at 2021/06/02 end*/
	.margin = 16,
	.min_shutter = 4,
...
static u16 read_cmos_sensor_com(u16 addr)
{
	kal_uint16 get_byte = 0;
	char pu_send_cmd[1] = { (char)(addr & 0xFF) };
...
int gc5035_dd_compatible(void)
{       
        //int i4RetValue = 0;
	//int i4ResidueDataLength;
        //pBuff = pinputdata;
        int Module_Info_Flag, addr = 0; 
	int module_id;
        //i4ResidueDataLength = (int)ui4_length;
        printk("GC5035_LY  compatilbe ");
	...
	Module_Info_Flag = read_cmos_sensor_com(0x6c);//gc5035_otp_read_byte(0xa48);
	if (Module_Info_Flag == 0x37)
		addr = 0xb10;
	else if (Module_Info_Flag == 0x13)
		addr = 0xab0;
	else if (Module_Info_Flag == 0x01)
		addr = 0xa50;
	else
		addr = 0;
	printk("GC5035_DD module info start addr = 0x%x %d \n", addr,Module_Info_Flag);
	...
	module_id = read_cmos_sensor_com(0x6c);  //otp data
	printk("GC5035_DD  compatilbe=%0x ",module_id);
	write_cmos_sensor(0xf3, 0x00);
	return module_id;
}
