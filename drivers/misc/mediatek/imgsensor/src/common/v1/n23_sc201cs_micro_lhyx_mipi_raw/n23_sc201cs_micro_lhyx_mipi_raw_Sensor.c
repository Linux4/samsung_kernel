...
#include "n23_sc201cs_micro_lhyx_mipi_raw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "n23_SC201CS_micro_lhyx_camera_sensor"
//#define pr_debug(fmt) PFX "[%s] " fmt, __func__
/****************************   Modify end    *******************************************/

//#define pr_debug(format, args...)    xlog_printk(ANDROID_pr_debugO   , PFX, "[%s] " format, __FUNCTION__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);


static imgsensor_info_struct imgsensor_info = { 
    .sensor_id = N23_SC201CS_MICRO_LHYX_SENSOR_ID,
   ...
   // .checksum_value = 0x221b1a41,        //checksum value for Camera Auto Test
    ...
    .isp_driving_current = ISP_DRIVING_4MA, //mclk driving current //bug 663265 zhoumin.wt, modify, 2021/09/27, improve the RF signal
...
{
	return ((read_cmos_sensor(0x3107) << 8) | read_cmos_sensor(0x3108)); //0xeb2c+1
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    pr_debug("gain = %d ,again = 0x%x, dgain(0x3e07)= 0x%x\n ", gain, read_cmos_sensor(0x3e09),read_cmos_sensor(0x3e07));
    //pr_debug("gain = %d ,again = 0x%x\n ", gain, reg_gain);
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x3248,0x02); //Bug 682590, zhoumin.wt, add, 2021/08/27,  update init register
    ...
    write_cmos_sensor(0x3650,0x21);//Bug 682590, zhoumin.wt, add, 2021/08/23,  modify for mipi test fail
    ...
    write_cmos_sensor(0x3933,0x01); //Bug 682590, zhoumin.wt, add, 2021/08/27,  update init register
...
static struct stCAM_CAL_DATAINFO_STRUCT n23_sc201cs_micro_lhyx_eeprom_data ={
	.sensorID= N23_SC201CS_MICRO_LHYX_SENSOR_ID,
	.deviceID = 0x08,
	.dataLength = 0x0778,
...
static struct stCAM_CAL_CHECKSUM_STRUCT n23_sc201cs_micro_lhyx_checksum[8] =
{
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		//+Bug 682590, zhoumin.wt, add, 2021.08.24, bring-up OTP for front and micro camera
		size = imgSensorReadEepromData(&n23_sc201cs_micro_lhyx_eeprom_data,n23_sc201cs_micro_lhyx_checksum);
		if(size != n23_sc201cs_micro_lhyx_eeprom_data.dataLength || (n23_sc201cs_micro_lhyx_eeprom_data.sensorVendorid >> 24)!= read_eeprom(0x0001)) {
			pr_err("get eeprom data failed\n");
			if(n23_sc201cs_micro_lhyx_eeprom_data.dataBuffer != NULL) {
				kfree(n23_sc201cs_micro_lhyx_eeprom_data.dataBuffer);
				n23_sc201cs_micro_lhyx_eeprom_data.dataBuffer = NULL;
			}
			*sensor_id = 0xFFFFFFFF;
			return ERROR_SENSOR_CONNECT_FAIL;
		} else {
			pr_debug("get eeprom data success\n");
			imgSensorSetEepromData(&n23_sc201cs_micro_lhyx_eeprom_data);
		}
		//+Bug 682590, zhoumin.wt, add, 2021.08.24, bring-up OTP for front and micro camera		
