...
/*hs14 code for AL6528A-277 add poweroff log by chenjun at 2022-10-15 start*/
	...
	char *pcurr_idx)
{
	...
	while (ppwr_info->pin != IMGSENSOR_HW_PIN_NONE &&
		...
		pr_debug(
		  "pr_debug sensor_idx = %d, pin=%d, pin_state_on=%d, hw_id =%d\n",
		  sensor_idx,
		  ppwr_info->pin,
		  ppwr_info->pin_state_on,
		  psensor_pwr->id[ppwr_info->pin]);
		pr_info(
		  "pr_info sensor_idx = %d, pin=%d, pin_state_on=%d, hw_id =%d\n",
		  sensor_idx,
		  ppwr_info->pin,
		  ppwr_info->pin_state_on,
		  psensor_pwr->id[ppwr_info->pin]);		
	...
	if (pwr_status == IMGSENSOR_HW_POWER_STATUS_OFF) {
		while (pin_cnt) {
			...
			if (ppwr_info->pin != IMGSENSOR_HW_PIN_UNDEF) {
				...
				pr_info("pr_info sensor_idx = %d, pin=%d, pin_state_off=%d, hw_id =%d\n",
					sensor_idx,
					ppwr_info->pin,
...
/*hs14 code for AL6528A-277 add poweroff log by chenjun at 2022-10-15 end*/
