	...
	char *pcurr_idx)
{
	...
	int                               pin_cnt = 0;
	//+bug494661 chenbocheng.wt, modify, 2019/11/15, add delay time 2ms for main avdd power down
	u32 regulator_delay = 0;
	...
	if (pwr_status == IMGSENSOR_HW_POWER_STATUS_OFF) {
		while (pin_cnt) {
			...
			if (ppwr_info->pin != IMGSENSOR_HW_PIN_UNDEF) {
				...
				mdelay(ppwr_info->pin_on_delay);
				//+bug494661 chenbocheng.wt, modify, 2019/11/15, add delay time 2ms for main avdd power down
				//+bug536466 wanglei6.wt, modify, 2020/02/29, I2C failed to power down
				if (pdev->set != NULL){
					pdev->set(
					    pdev->pinstance,
					    sensor_idx,
					...
					if((ppwr_info->pin == IMGSENSOR_HW_PIN_AVDD) && (sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN)){
						regulator_delay = (ppwr_info->pin_on_delay)*2;
						mdelay(regulator_delay);
						pr_err(
							"sensor_idx = %d, pin=%d, pin_state_on=%d,main regulator power down delay time(ms)=%d\n",
							sensor_idx,
							ppwr_info->pin,
							ppwr_info->pin_state_on,
							regulator_delay);
					}else if((ppwr_info->pin == IMGSENSOR_HW_PIN_AVDD) && (sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN3)){
						regulator_delay = (ppwr_info->pin_on_delay)*3;
						mdelay(regulator_delay);
						pr_err(
							"sensor_idx = %d, pin=%d, pin_state_on=%d,main regulator power down delay time(ms)=%d\n",
							sensor_idx,
							...
							ppwr_info->pin_state_on,
							regulator_delay);
					}else if((ppwr_info->pin == IMGSENSOR_HW_PIN_DOVDD) && ((sensor_idx == IMGSENSOR_SENSOR_IDX_SUB)||(sensor_idx == IMGSENSOR_SENSOR_IDX_SUB2))){
						regulator_delay = (ppwr_info->pin_on_delay)*5;
						mdelay(regulator_delay);
						pr_err(
							"sensor_idx = %d, pin=%d, pin_state_on=%d,front or wide regulator power down delay time(ms)=%d\n",
							sensor_idx,
							ppwr_info->pin,
							ppwr_info->pin_state_on,
							regulator_delay);
					}else if((ppwr_info->pin == IMGSENSOR_HW_PIN_DOVDD) && (sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN3)){
						regulator_delay = (ppwr_info->pin_on_delay)*4;//bug536466 wanglei6.wt, modify, 2020/02/29, I2C failed to power down
						mdelay(regulator_delay);
						pr_err(
							"sensor_idx = %d, pin=%d, pin_state_on=%d,micro regulator power down delay time(ms)=%d\n",
							sensor_idx,
							ppwr_info->pin,
							ppwr_info->pin_state_on,
							regulator_delay);
