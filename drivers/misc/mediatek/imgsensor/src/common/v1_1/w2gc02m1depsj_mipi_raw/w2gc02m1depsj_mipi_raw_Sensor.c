...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	pr_err("w2gc02m1depsj_mipi_raw getimgsensorid!!!, main_sensor_id:0x%x\n", main_sensor_id);
		...
		do {
			*sensor_id = return_sensor_id();
            if ((*sensor_id == imgsensor_info.sensor_id) && (W2S5KJN1REARST_SENSOR_ID == main_sensor_id)){
				...
				printk("3 supply dualcam(w2s5kjn1rearst_mipi_raw + w2gc02m1depsj_mipi_raw) sensor id match!!");
                return ERROR_NONE;
			}
    ...
    if ((*sensor_id != imgsensor_info.sensor_id) || (W2S5KJN1REARST_SENSOR_ID != main_sensor_id)) {
		...
		printk("3 supply dualcam(need:w2s5kjn1rearst_mipi_raw + w2gc02m1depsj_mipi_raw) sensor id not match!!,main_sensor_id:0x%x,depth sensor id:0x%x\n",main_sensor_id,*sensor_id);
