...
#include "w2c2599microdelta_mipi_raw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = W2C2599MICRODELTA_SENSOR_ID ,
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
    ...
    C2599MIPISENSORDB("[lj c2599]>>> set_max_framerate():framerate = %d, min framelength should enable = %d\n", framerate,min_framelength_en);
...
static void set_shutter(kal_uint16 shutter)
{
    ...
    C2599MIPISENSORDB("[lj c2599]set_shutter(): shutter =%d\n", shutter);
	...
	C2599MIPISENSORDB("[lj c2599]set_shutter(): shutter =%d, framelength =%d,mini_framelegth=%d\n", shutter,imgsensor.frame_length,imgsensor.min_frame_length);
}  
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length)
{
	...
	write_cmos_sensor(0x0202, (shutter >> 8) & 0xFF);
	write_cmos_sensor(0x0203, (shutter) & 0xFF);
	LOG_INF("[lj c2599] shutter =%d, framelength =%d/%d, dummy_line=%d, \n", shutter,
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    C2599MIPISENSORDB("[lj c2599] C2599 isp again = %x\n",(gain & 0xFF));
...
static void sensor_init(void)
{
     ...
     write_cmos_sensor(0x0103,0x01);//sensor softeare reset
     ...
     write_cmos_sensor(0x3295,0x07);
     ///write_cmos_sensor(0x0100,0x01);
     write_cmos_sensor(0x0101,0x00); // mirror and flip

     C2599MIPISENSORDB("[lj c2599]sensor_init end\n");
}
...
{
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
	C2599MIPISENSORDB("[lj c2599]E! preview_setting currefps:%d\n",currefps);                                                               
	//sensor_init();
...
{
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2599]E! capture_setting currefps:%d\n",currefps);
	//sensor_init();             
	 //write_cmos_sensor(0x0100,0x01);  //streaming starts 
...
{
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2599]enable: %d\n", enable);
    spin_lock(&imgsensor_drv_lock);
    spin_unlock(&imgsensor_drv_lock);
    return ERROR_NONE;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	//printk("c2599 get_imgsensor_id pinSetIdx = %d\n ",pinSetIdx);
	//if(pinSetIdx)
        ...
        do {
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("[lj c2599]c2599 get_imgsensor_id OK,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("[lj c2599]w2c2599micro_mipi_raw get_imgsensor_id fail, write id: 0x%x, id: 0x%x,imgsensor_info.sensor_id:0x%x\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
                          ...
                          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    C2599MIPISENSORDB("[lj c2599]<<< capture()\n");
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    C2599MIPISENSORDB("[lj c2599]<<< normal_video()\n");
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    C2599MIPISENSORDB("[lj c2599]<<< hs_video()\n");
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    C2599MIPISENSORDB("[lj c2599]<<< slim_video()\n");
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    spin_lock(&imgsensor_drv_lock);
...
{
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2599]>>> get_resolution()\n");
    ...
    C2599MIPISENSORDB("[lj c2599]>>> feature_control()\n");
    return ERROR_NONE;
}    /*    get_resolution    */
...
static kal_uint32 get_info(enum MSDK_SCENARIO_ID_ENUM scenario_id,
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    C2599MIPISENSORDB("[lj c2599]>>> get_info():scenario_id = %d\n", scenario_id);
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
	...
	C2599MIPISENSORDB("[lj c2599]<<< get_info()\n");
...
{
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2599]>>> control() scenario_id = %d\n", scenario_id);
    spin_lock(&imgsensor_drv_lock);
    ...
    switch (scenario_id) {
        ...
        case MSDK_SCENARIO_ID_SLIM_VIDEO:
            ...
            break;
        default:
            C2599MIPISENSORDB("[lj c2599]Error ScenarioId setting\n");
            preview(image_window, sensor_config_data);
            return ERROR_INVALID_SCENARIO_ID;
    }

	
	C2599MIPISENSORDB("[lj c2599]<<< control()\n");
    return ERROR_NONE;
}    /* control() */
...
{
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2599]framerate = %d\n ", framerate);
...
{
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2599]>>>set_auto_flicker_mode():enable = %d, framerate = %d \n", enable, framerate);
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2599]>>>>set_max_framerate_by_scenario():scenario_id = %d, framerate = %d\n", scenario_id, framerate);
    ...
    C2599MIPISENSORDB("[lj c2599]<<<set_max_framerate_by_scenario()\n");
...
{
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2599]scenario_id = %d\n", scenario_id);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
    C2599MIPISENSORDB("[lj c2599]>>> feature_control():feature_id = %d\n", feature_id);
    switch (feature_id) {
...
//bug 558061, zhanghengyuan.wt, modify, 2020/12/16,add featue for mt6833
	/*	
        case SENSOR_FEATURE_GET_PERIOD:
			C2599MIPISENSORDB("[lj c2599 feature_control] line = %d \n",__LINE__);
            *feature_return_para_16++ = imgsensor.line_length;
            *feature_return_para_16 = imgsensor.frame_length;
            *feature_para_len=4;
			C2599MIPISENSORDB("[lj c2599][C2599_SensroDriver]%d,%d\n",imgsensor.line_length,imgsensor.frame_length);
            break;
        case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
		C2599MIPISENSORDB("[lj c2599 feature_control] line = %d \n",__LINE__);
            *feature_return_para_32 = imgsensor.pclk;
            *feature_para_len=4;
			C2599MIPISENSORDB("[lj c2599][C2599_SensroDriver]%d\n",imgsensor.pclk);
            break;
        case SENSOR_FEATURE_SET_ESHUTTER:
		C2599MIPISENSORDB("[lj c2599 feature_control] line = %d \n",__LINE__);
            set_shutter(*feature_data);
        ...
        case SENSOR_FEATURE_SET_FRAMERATE:
		C2599MIPISENSORDB("[lj c2599 feature_control] line = %d \n",__LINE__);
            C2599MIPISENSORDB("[lj c2599]current fps :%d\n", (UINT32)*feature_data);
            spin_lock(&imgsensor_drv_lock);
        ...
        case SENSOR_FEATURE_GET_CROP_INFO:
		C2599MIPISENSORDB("[lj c2599 feature_control] line = %d \n",__LINE__);
            C2599MIPISENSORDB("[lj c2599]SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n", (UINT32)*feature_data);
...
UINT32 W2C2599MICRODELTA_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
...
//UINT32 C2599_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
//{
   // C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
