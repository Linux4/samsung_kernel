 ...
 * Filename:
 * ---------
 *     w1_gc02m1b_dep_cxtmipi_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "w1gc02m1bdepcxt_mipimono_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = W1GC02M1BDEPCXT_SENSOR_ID,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = return_sensor_id();
			if ((*sensor_id == imgsensor_info.sensor_id) && (W1HI4821QREARSY_SENSOR_ID == main_sensor_id)) {
				pr_info("4th dualcam i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
	...
	if ((*sensor_id != imgsensor_info.sensor_id) || (W1HI4821QREARSY_SENSOR_ID != main_sensor_id)) {
		...
		*sensor_id = 0xFFFFFFFF;
		pr_err("4th dualcam sensor id error,main sensor id:%x,depth sensor id:%x\n",main_sensor_id,*sensor_id);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_info("open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_err("open Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_FRAME_CTRL_INFO_BY_SCENARIO:
		...
		*(feature_data + 2) = imgsensor_info.margin;
		break;
	//modify codes for factory mode of photo black screen
	 //+bug 646787 zhanghao2, add, 2021/05/02,fix ITS:test_sensor_fusion fail
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 620000;
		break;
	//-bug 646787 zhanghao2, add, 2021/05/02,fix ITS:test_sensor_fusion fail
	default:
		break;
