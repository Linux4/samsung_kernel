 ...
 * Filename:
 * ---------
 *     w2sc202csdeplh2_mipiraw_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "w2sc202csdeplh2_mipimono_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "w2sc202csdeplh2_mipi_mono"

#define LOG_1 LOG_INF("W2SC202CSDEPLH2,MIPI 1LANE\n")
...
static struct imgsensor_info_struct imgsensor_info = { 
    .sensor_id = W2SC202CSDEPLH2_SENSOR_ID,
...
//#ifdef __GAINMAP__
#if 1
#define W2SC202CSDEPLH2_SENSOR_GAIN_BASE             1024
#define W2SC202CSDEPLH2_SENSOR_GAIN_MAX              65024 //(63.5 * W2SC202CSDEPLH2_SENSOR_GAIN_BASE)
#define W2SC202CSDEPLH2_SENSOR_GAIN_MAX_VALID_INDEX  6
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	kal_uint16 reg_gain = gain << 4;

	if (reg_gain < W2SC202CSDEPLH2_SENSOR_GAIN_BASE)
		reg_gain = W2SC202CSDEPLH2_SENSOR_GAIN_BASE;
	else if (reg_gain > W2SC202CSDEPLH2_SENSOR_GAIN_MAX)
		reg_gain = W2SC202CSDEPLH2_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 W2SC202CSDEPLH2_AGC_Param[W2SC202CSDEPLH2_SENSOR_GAIN_MAX_VALID_INDEX][2] = {
		{  1024,  0x00 },
		{  2048,  0x01 },
		{  4096,  0x03 },
		{  8192,  0x07 },
	...
	for (gain_index = W2SC202CSDEPLH2_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 0; gain_index--)
		if (reg_gain >= W2SC202CSDEPLH2_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0x3e09, W2SC202CSDEPLH2_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * W2SC202CSDEPLH2_SENSOR_GAIN_BASE / W2SC202CSDEPLH2_AGC_Param[gain_index][0];
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	pr_err("cwd getimgsensorid!!! main_sensor_id:0x%x\n", main_sensor_id);
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		pr_err("cwd return_sensor_id:0x%x\n",*sensor_id);
            if ((*sensor_id == imgsensor_info.sensor_id) && (W2HI5022QREARTXD_SENSOR_ID == main_sensor_id)){
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				printk("5 supply dualcam(need:w2hi5022qreartxd_mipi_raw + w2sc202csdeplh2_mipi_mono) sensor id match!!");
                return ERROR_NONE;
            }   
    ...
    if ((*sensor_id != imgsensor_info.sensor_id) || (W2HI5022QREARTXD_SENSOR_ID != main_sensor_id)) {
		...
		printk("5 supply dualcam(need:w2hi5022qreartxd_mipi_raw + w2sc202csdeplh2_mipi_mono) sensor id not match!!,main_sensor_id:0x%x,depth sensor id:0x%x\n",main_sensor_id,*sensor_id);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    *(feature_data + 1) = imgsensor_info.min_shutter;
	...
	case SENSOR_FEATURE_GET_MIPI_PIXEL_RATE:
	{
		...
		break;//bug727089, liangyiyi.wt,MODIFY, 2022.10.27, fix KASAN opened to Monitor memory, camera  memory access error.
...
UINT32 W2SC202CSDEPLH2_MIPI_MONO_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    w2sc202csdeplh2_MIPI_RAW_SensorInit    */
