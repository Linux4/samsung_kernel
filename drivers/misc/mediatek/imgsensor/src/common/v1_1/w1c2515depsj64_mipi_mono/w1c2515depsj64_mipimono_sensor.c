...
#include "w1c2515depsj64_mipimono_sensor.h"
...
#ifdef C2515MIPIRAW_DEBUG
#define C2515MIPISENSORDB printk
#else
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = W1C2515DEPSJ64_SENSOR_ID,
                ...
                kal_uint16 shutter, kal_uint16 frame_length)
{
    ...
    imgsensor.shutter = shutter;
    spin_unlock_irqrestore(&imgsensor_drv_lock, flags);
    //LOG_INF("000 w1c2515_64 shutter =%d, frame_length =%d\n", shutter, frame_length);
    ...
    //LOG_INF("Exit!w1c2515_64 shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x380c,0x01);  //0x11
...
//-bug584789,zhanghengyuan.wt,MODIFY,2020/12/30,depth c2515  mclk driving current
	...
	write_cmos_sensor(0x0341,0x04); //24fps
...
{
    C2515MIPISENSORDB("[lj c2515]func= %s \n",__func__);
    LOG_INF("[lj c2515]E! preview_setting currefps:%d\n",currefps);
    write_cmos_sensor(0x0340,0x04);
    write_cmos_sensor(0x0341,0xD0); //30 fps
...
{
    C2515MIPISENSORDB("[lj c2515]func= %s \n",__func__);
    LOG_INF("[lj c2515]E! capture_setting currefps:%d\n",currefps);
    write_cmos_sensor(0x0340,0x04);
    write_cmos_sensor(0x0341,0xD0); //30 fps
    write_cmos_sensor(0x0100,0x01);  //streaming starts
...
{
    C2515MIPISENSORDB("[lj c2515]func= %s \n",__func__);
    LOG_INF("[lj c2515]E! custom1_setting currefps:%d\n",currefps);
    write_cmos_sensor(0x0340,0x06);
    write_cmos_sensor(0x0341,0x04); //24fps
    write_cmos_sensor(0x0100,0x01);  //streaming starts
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
        ...
        do {
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001)) +1;
            if ((*sensor_id == imgsensor_info.sensor_id) && (W1OV64B40REARTXD_SENSOR_ID == main_sensor_id)) {
                printk("[dualcam] 200517 add preview 30fps custom1 24fps c2515_64 get_imgsensor_id OK,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("[dualcam]  c2515_64 get_imgsensor_id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
    ...
    if ((*sensor_id != imgsensor_info.sensor_id) || (W1OV64B40REARTXD_SENSOR_ID != main_sensor_id)) {
        ...
        *sensor_id = 0xFFFFFFFF;
        pr_err("24fps 64rd dualcam c2515_64 sensor id error,main sensor id:%x,depth sensor id:%x\n",main_sensor_id,*sensor_id);
        return ERROR_SENSOR_CONNECT_FAIL;
    }
    ...
    UINT8 *feature_para, UINT32 *feature_para_len)
{
    ...
    case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
        *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 240000;
    break;
    //-bug626603 liudijin, add, 2021/03/26,fix ITS:test_sensor_fusion fail
    default:
        break;
