...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				//+bug767771, chenming.wt, ADD, 2022/07/13, add gc02m1 otp bringup.
				size = imgSensorReadEepromData(&w2gc02m1microcxt_eeprom_data,w2gc02m1microcxt_checksum);
				if((size != w2gc02m1microcxt_eeprom_data.dataLength) || ((w2gc02m1microcxt_eeprom_data.sensorVendorid >> 24)!= read_eeprom(0x0001))) {
					pr_err("cwd w2gc02m1microcxt get eeprom data failed,size = %d, dataLength = %d, w2gc02m1microcxt_eeprom_data.sensorVendorid >> 24 = %d, read_eeprom(0x0001) = %d\n",size,w2gc02m1microcxt_eeprom_data.dataLength,(w2gc02m1microcxt_eeprom_data.sensorVendorid >> 24),read_eeprom(0x0001));
					...
					imgSensorSetEepromData(&w2gc02m1microcxt_eeprom_data);
				}
				//-bug767771, chenming.wt, ADD, 2022/07/13, add gc02m1 otp bringup.
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
        ...
        pr_err("w2gc02m1microcxt Read id fail,sensor id: 0x%x\n", *sensor_id);
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	return ERROR_NONE;
