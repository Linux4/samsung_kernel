...
#define SENSOR_JN1_GGC_CAL_BASE_REAR           (0x2246)
...
static void sensor_GGC_write(void)
{
	...
	char cal_data[SENSOR_JN1_GGC_CAL_SIZE-2] = {0};

	int start_addr = SENSOR_JN1_GGC_REG_ADDR;
	int data_size = SENSOR_JN1_GGC_CAL_SIZE-2;
	...
	rom_cal_buf += SENSOR_JN1_GGC_CAL_BASE_REAR;
	if (rom_cal_buf[0] == 0xFF && rom_cal_buf[1] == 0x00) {
		// Big Endian
		memcpy(cal_data, &rom_cal_buf[2], SENSOR_JN1_GGC_CAL_SIZE-2);
	} else if (rom_cal_buf[0] == 0x00 && rom_cal_buf[1] == 0xFF) {
		// Little Endian
		rom_cal_buf += 2;
		for (i = 0; i + 1 < SENSOR_JN1_GGC_CAL_SIZE - 2; i += 2) {
			cal_data[i] = rom_cal_buf[i+1];
			cal_data[i+1] = rom_cal_buf[i];
		}
		if ((SENSOR_JN1_GGC_CAL_SIZE - 2) & 0x1)
			cal_data[i] = rom_cal_buf[i];
	} else {
		LOG_ERR("%s : endian check fail (%X, %X)", rom_cal_buf[0], rom_cal_buf[1]);
		return;
	}
