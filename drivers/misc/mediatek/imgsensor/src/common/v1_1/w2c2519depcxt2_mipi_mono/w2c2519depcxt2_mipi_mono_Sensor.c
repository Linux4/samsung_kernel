...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
        ...
        do {
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001)) + 2;
            if ((*sensor_id == imgsensor_info.sensor_id) && (W2HI5022QREARTXD_SENSOR_ID == main_sensor_id)){
				...
				printk("2 supply dualcam(w2hi5022qreartxd_mipi_raw + w2c2519depcxt2_mipi_mono) sensor id match!!");
                return ERROR_NONE;
            }   
            printk("[lj c2519]w2c2519depcxt2_mipi_mono get_imgsensor_id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
    ...
    if ((*sensor_id != imgsensor_info.sensor_id) || (W2HI5022QREARTXD_SENSOR_ID != main_sensor_id)) {
		...
		printk("2 supply dualcam(need:w2hi5022qreartxd_mipi_raw + w2c2519depcxt2_mipi_mono) sensor id not match!!,main_sensor_id:0x%x,depth sensor id:0x%x\n",main_sensor_id,*sensor_id);
