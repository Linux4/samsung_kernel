...
#include "w2c2519depcxt2_mipi_mono_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = W2C2519DEPCXT2_SENSOR_ID ,
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length)
{
	...
	write_cmos_sensor(0x0202, (shutter >> 8) & 0xFF);
	write_cmos_sensor(0x0203, (shutter) & 0xFF);
	LOG_INF("[lj c2519] shutter =%d, framelength =%d/%d, dummy_line=%d, \n", shutter,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    kal_uint8 i = 0;
    kal_uint8 retry = 2;
	printk("w2c2519depcxt2_mipi_mono getimgsensorid!!!, main_sensor_id:0x%x\n", main_sensor_id);
        ...
        do {
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001)) + 2;
            if ((*sensor_id == imgsensor_info.sensor_id) && (N28HI5021QREARDC_SENSOR_ID == main_sensor_id)){
				...
				printk("3 supply dualcam(n28hi5021reardc_mipi_raw + n28c2519depcxt2_mipi_mono) sensor id match!!");
                return ERROR_NONE;
            }   
            printk("[lj c2519]n28c2519depcxt2_mipi_mono get_imgsensor_id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
    ...
    if ((*sensor_id != imgsensor_info.sensor_id) || (N28HI5021QREARDC_SENSOR_ID != main_sensor_id)) {
		...
		printk("Fail:3 supply dualcam(need:n28hi5021reardc_mipi_raw + n28c2519depcxt2_mipi_mono) sensor id not match!!,main_sensor_id:0x%x,depth sensor id:0x%x\n",main_sensor_id,*sensor_id);
...
UINT32 W2C2519DEPCXT2_MIPI_MONO_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
...
//UINT32 C2519_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
//{
   // C2519MIPISENSORDB("[lj c2519]func= %s \n",__func__);
