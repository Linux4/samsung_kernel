...
//+bug496716 chenbocheng.wt, add, 2019/11/5, add patch for using different voltage in the same regulator
	...
	enum IMGSENSOR_SENSOR_IDX sensor_idx, bool enable)
{
	...
	int i = 0;
	struct device_node       *pof_node;  //Alex add for bokeh crash on regulator_set

	gimgsensor.status.oc = 0;

        //B: Alex add for bokeh crash on regulator_set
	...
	//E: Alex add for bokeh crash on regulator_set
	...
	pdevice->of_node = pof_node;  //Alex add for bokeh crash on regulator_set
