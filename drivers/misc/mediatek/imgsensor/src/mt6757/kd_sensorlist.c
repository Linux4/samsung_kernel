...
static int g_seninfWakeLockCnt;
	...
	static int I2C_error[4] = {0};
#else
	static int I2C_error[3] = {0};
#endif
...
extern ssize_t rear_flash_show(struct device *dev,
				struct device_attribute *attr, char *buf);

extern ssize_t front_flash_show(struct device *dev,
				struct device_attribute *attr, char *buf);

extern ssize_t rear_flash_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size);

extern ssize_t front_flash_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size);
...
#endif //defined(titan6757_phone_o)|| defined(titan6757_jade_o)	
	return 0;
}
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	case SENSOR_FEATURE_GET_I2C_STATUS:
		/*  */
	...
	/*in case that some structure are passed from user space by ptr */
	switch (pFeatureCtrl->FeatureId) {
	case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
	case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
	case SENSOR_FEATURE_GET_SENSOR_HDR_CAPACITY:
	...
	case SENSOR_FEATURE_GET_I2C_STATUS:
		{
			if (g_pSensorFunc) {
				switch (pFeatureCtrl->InvokeCamera) {
				case DUAL_CAMERA_MAIN_SENSOR:
					*(int *)pFeaturePara = I2C_error[0];
					I2C_error[0] = 0;
					break;
				case DUAL_CAMERA_SUB_SENSOR:
					*(int *)pFeaturePara = I2C_error[1];
					I2C_error[1] = 0;
					break;
				case DUAL_CAMERA_MAIN_2_SENSOR:
					*(int *)pFeaturePara = I2C_error[2];
					I2C_error[2] = 0;
					break;
#ifdef MTK_SUB2_IMGSENSOR
				case DUAL_CAMERA_SUB_2_SENSOR:
					*(int *)pFeaturePara = I2C_error[3];
					I2C_error[3] = 0;
					break;
#endif
				default:
