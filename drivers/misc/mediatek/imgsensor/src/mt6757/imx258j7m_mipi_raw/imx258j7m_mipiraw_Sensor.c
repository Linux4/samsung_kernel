...
#include "imx258j7m_mipiraw_Sensor.h"
#ifdef CONFIG_CAMERA_HW_BIG_DATA
#include "hwparam.h"
#endif

/****************************Modify Following Strings for Debug****************************/
#define PFX "IMX258J7M_camera_sensor"
#define LOG_1 LOG_INF("IMX258J7M,MIPI 1LANE\n")
#define LOG_2 \
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX258J7M_SENSOR_ID - 0x1000,	/* record sensor id defined in Kd_imgsensor.h */
...
#define IMX258J7M_RAW_TYPE		(0x80)	/*enable test mode */
#define IMX258J7M_HDR_TYPE		(0x00)
#define IMX258J7M_BINNING_TYPE	(0x10)
#define IMX258J7M_BW_TYPE		(0x20)
#define IMX258J7M_NOPDAF_TYPE	(0x30)
#define IMX258J7M_HDD_TYPE		(0x40)	/*IMX258-0AUH5 */
static kal_uint16 imx258j7m_type;
...
#define IMX258J7MMIPI_MaxGainIndex (154)
kal_uint16 IMX258J7MMIPI_sensorGainMapping[IMX258J7MMIPI_MaxGainIndex][2] = {
	{64, 0},
	{65, 8},
	{66, 16},
	{67, 25},
...
#if 0
static kal_uint8 IMX258J7MMIPI_SPC_Data[126];
static kal_uint8 SPC_data_done;

static void load_imx258j7m_SPC_Data(void)
{
	if (SPC_data_done == false) {
		if (!read_imx258j7m_eeprom_SPC(0x0F73, IMX258J7MMIPI_SPC_Data, 126)) {
			LOG_INF("imx258j7m load spc fail\n");
			return;
		}
		SPC_data_done = true;
	}
...
static void write_imx258j7m_SPC_Data(void)
{
	kal_uint16 i;
	...
	for (i = 0; i < 63; i++) {
		write_cmos_sensor(0xD04C + i, IMX258J7MMIPI_SPC_Data[i]);
		/* LOG_INF("SPC_Data[%d] = %d\n", i, IMX258J7MMIPI_SPC_Data[i]); */
	}
	for (i = 0; i < 63; i++) {
		write_cmos_sensor(0xD08C + i, IMX258J7MMIPI_SPC_Data[i + 63]);
		/* LOG_INF("SPC_Data[%d] = %d\n", i+63, IMX258J7MMIPI_SPC_Data[i+63]); */
	}
}
#endif
	...
	if (write_cmos_sensor(0x0A02, 0x0F) == 0) {
		...
		while (retry--) {
			if (read_cmos_sensor(0x0A01) == 0x01) {
				imx258j7m_type = read_cmos_sensor(0x0A2E);
				if (imx258j7m_type == IMX258J7M_HDD_TYPE) {
					if (test_Pmode)
						imx258j7m_type = IMX258J7M_RAW_TYPE;
					else
						imx258j7m_type = IMX258J7M_HDD_TYPE;
				}
				LOG_INF("imx258j7m type = 0x%x(0x00=HDR,0x10=binning,0x40=HDD,0x80=RAW)",
					imx258j7m_type);
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	for (i = 0; i < IMX258J7MMIPI_MaxGainIndex; i++) {
		if (gain <= IMX258J7MMIPI_sensorGainMapping[i][0])
			break;
	}
	if (gain != IMX258J7MMIPI_sensorGainMapping[i][0])
		LOG_INF("Gain mapping don't correctly:%d %d\n", gain, IMX258J7MMIPI_sensorGainMapping[i][0]);
	return IMX258J7MMIPI_sensorGainMapping[i][1];
}
...
/*      preview_setting  */
static void imx258j7m_ImageQuality_Setting(void)
{
...
static void sensor_init(void)
{
	...
	imx258j7m_ImageQuality_Setting();
	/*Need Mirror/Flip */
	set_mirror_flip(0);
	/*write_imx258j7m_SPC_Data();*/
...
static void hs_video_setting(void)
{
	...
	if (imx258j7m_type == IMX258J7M_HDR_TYPE || imx258j7m_type == IMX258J7M_RAW_TYPE) {
		write_cmos_sensor(0x94DC, 0x20);
		write_cmos_sensor(0x94DD, 0x20);
		write_cmos_sensor(0x94DE, 0x20);
		write_cmos_sensor(0x95DC, 0x20);
		...
		write_cmos_sensor(0x7FB0, 0x00);
		write_cmos_sensor(0x9010, 0x3E);
	} else if (imx258j7m_type == IMX258J7M_BINNING_TYPE) {
...
static kal_uint32 get_sensor_temperature(void)
{
	...
	LOG_INF("imx258j7m_get_temperature(%d)\n", temperature);
	return temperature;
}
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		...
		read_imx258j7m_pdaf((kal_uint16) (*feature_data),
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		...
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		case MSDK_SCENARIO_ID_CUSTOM1:
			if (imx258j7m_type != IMX258J7M_BINNING_TYPE)
	...
	case SENSOR_FEATURE_GET_PDAF_TYPE:
		switch (imx258j7m_type) {
		case 0x00:
			sprintf(feature_para, "IMX258J7M 0AQH5-C (BME-HDR version)");
			break;
		case 0x10:
			sprintf(feature_para, "IMX258J7M 0APH5-C (Binning version)");
			break;
		case 0x20:
			sprintf(feature_para, "IMX258J7M 0AMH5-C (B/W version)");
			break;
		case 0x30:
			sprintf(feature_para, "IMX258J7M 0ATH5-C (Non-PDAF version)");
			break;
		case 0x40:
			sprintf(feature_para, "IMX258J7M 0AUH5-C (Horizontal Double Density version)");
			break;
		case 0x80:
			sprintf(feature_para, "IMX258J7M enable PDO");
			break;
		default:
			sprintf(feature_para, "Other case %x", imx258j7m_type);
		}
...
UINT32 IMX258J7M_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
	/*UINT32 IMX258J7M_MIPI_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)*/
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*      IMX258J7M_MIPI_RAW_SensorInit      */
