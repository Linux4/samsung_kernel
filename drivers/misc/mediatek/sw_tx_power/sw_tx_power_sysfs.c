							...
							const char *buf, size_t len)
{
	...
	if (strstr(buf, "off"))
		...
		SWTX_DBG("[swtp] sysfs_store %s %d %d\n", buf, len, pstrlen);
		...
		SWTX_DBG("[swtp] sysfs_store %ld %ld\n", set_mode, temp);
	} else
		SWTX_ERR("[swtp] command error\n");

	return len;
}

static struct swtp_sysfs_attribute swtp_ctrl_attribute =
	__ATTR(swtp0, 0660, swtp_sysfs_show, swtp_sysfs_store);
...
static int __init swtp_sysfs_init(void)
{
	...
	if (kobject_init_and_add(&obj->kobj, &swtp_ktype, NULL, "ctrl")) {
		...
		SWTX_ERR("[swtp] kobject_init_and_add.\n");
		return -ENOMEM;
	}
