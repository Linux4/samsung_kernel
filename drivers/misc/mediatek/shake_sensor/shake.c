		...
		if (false == cxt->is_active_data) {
			...
			if (err) {
				...
				if (err) {
					...
					if (err) {
						SHK_ERR
						    ("enable_shake enable(%d) err 3 timers = %d\n",
						     enable, err);
						return err;
					}
				}
...
static ssize_t shk_show_enable_nodata(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct shk_context *cxt = NULL;
...
/*----------------------------------------------------------------------------*/
static ssize_t shk_show_active(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct shk_context *cxt = NULL;
...
static ssize_t shk_show_delay(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
...
static ssize_t shk_show_batch(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
...
static ssize_t shk_show_flush(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
...
/* cxt->shk_ctl.enable_nodata = ctl->enable_nodata; */
	...
	err = sysfs_create_group(&shk_context_obj->mdev.this_device->kobj, &shk_attribute_group);
	if (err < 0) {
...
static int shk_remove(struct platform_device *pdev)
{
	...
	sysfs_remove_group(&shk_context_obj->idev->dev.kobj, &shk_attribute_group);

	err = misc_deregister(&shk_context_obj->mdev);
	if (err)
