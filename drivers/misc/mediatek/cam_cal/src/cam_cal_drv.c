...
extern int Is_Cal_data_correct_main(void);
extern int Is_Cal_data_correct_sub(void);
#endif
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		if (pcmdInf != NULL) {
#if defined(titan6757_jade_o)
			if (ptempbuf->u4Offset==12345) {/*speical test*/
				if(ptempbuf->deviceID  == SENSOR_DEV_MAIN)
				{
					pu1Params[0]= Is_Cal_data_correct_main(); /*implement by customer  0x01:correct 0xff:fail*/
					pr_err("mainsensor cal error %d \n", pu1Params[0]);
				}
				else if(ptempbuf->deviceID  == SENSOR_DEV_SUB)
				{
					pu1Params[0] = Is_Cal_data_correct_sub(); /*implement by customer  0x01:correct 0xff:fail*/
					pr_err("subsensor cal error %d \n", pu1Params[0]);
				}
				i4RetValue = 1;
