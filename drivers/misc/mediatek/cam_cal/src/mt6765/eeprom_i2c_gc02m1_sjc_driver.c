...
#define GC02M1_SJC_OTP_DEBUG  1
#define GC02M1_SJC_I2C_ID     0x6E /*0x20*/

static u16 read_cmos_sensor(u16 addr)
{
	u16 get_byte=0;
	char pu_send_cmd[1] = {(char)(addr & 0xff)};
	iReadRegI2C(pu_send_cmd, 1, (u8 *)&get_byte, 1, GC02M1_SJC_I2C_ID);
...
static void write_cmos_sensor(u16 addr, u8 para)
{
	...
	iWriteRegI2C(pu_send_cmd, 2, GC02M1_SJC_I2C_ID);
}
...
{
	u16 i = 0;
	pr_err("gc02m1_sjc otp read group enter\n");
	for (i = 0; i < 17; i++) {
		write_cmos_sensor(0x17, (0x78 + (i * 8)));
		write_cmos_sensor(0xf3, 0x34);	
		data[i] = read_cmos_sensor(0x19);
#if GC02M1_SJC_OTP_DEBUG
	pr_debug("gc02m1_sjc addr = 0x%x, data = 0x%x\n", (0x78 + (i * 8)), data[i]);
#endif
	}
	return 0;
}
...
int gc02m1_sjc_iReadData(unsigned int ui4_offset,
...
unsigned int gc02m1_sjc_read_region(struct i2c_client *client, unsigned int addr,
				unsigned char *data, unsigned int size)
{
	g_pstI2CclientG = client;
	if (gc02m1_sjc_iReadData(addr, size, data) == 0)
		return size;
	else
		return 0;
}
