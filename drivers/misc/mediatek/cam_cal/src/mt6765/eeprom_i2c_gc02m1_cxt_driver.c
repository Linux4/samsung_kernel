...
#define GC02M1_CXT_OTP_DEBUG  1
#define GC02M1_CXT_I2C_ID     0x6E /*0x20*/

static u16 read_cmos_sensor(u16 addr)
{
	u16 get_byte=0;
	char pu_send_cmd[1] = {(char)(addr & 0xff)};
	iReadRegI2C(pu_send_cmd, 1, (u8 *)&get_byte, 1, GC02M1_CXT_I2C_ID);
...
static void write_cmos_sensor(u16 addr, u8 para)
{
	...
	iWriteRegI2C(pu_send_cmd, 2, GC02M1_CXT_I2C_ID);
}
...
int gc02m1_cxt_iReadData(unsigned int ui4_offset,
...
unsigned int gc02m1_cxt_read_region(struct i2c_client *client, unsigned int addr,
				unsigned char *data, unsigned int size)
{
	g_pstI2CclientG = client;
	if (gc02m1_cxt_iReadData(addr, size, data) == 0)
		return size;
	else
		return 0;
}
