...
#define hi556_txd_OTP_DEBUG  1
#define hi556_txd_I2C_ID     0x50 /*0x20*/
struct hi556_txd_otp_struct
{
...
static u16 read_cmos_sensor(u16 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, (u8 *)&get_byte, 1, hi556_txd_I2C_ID);
...
static void write_cmos_sensor(u16 addr, u16 para)
{
	...
	iWriteRegI2C(pu_send_cmd, 4, hi556_txd_I2C_ID);
}
...
static void write_cmos_sensor_8(u16 addr, u16 para)
{
	...
	iWriteRegI2C(pu_send_cmd, 3, hi556_txd_I2C_ID);
}



static void hi556_txd_otp_init(void)
{
	printk("hi556_txd otp_init_setting\n");
	//sensor init --start
...
int hi556_txd_iReadData(unsigned int ui4_offset,
...
{       
	...
	u4CurrentOffset = ui4_offset;
        printk("HI556_TXD  ui4_offset = %d",u4CurrentOffset);
	hi556_txd_otp_init();

	//1. read module info
	hi556_txd_otp.Module_Info_Flag = OTP_read_cmos_sensor(0x401);
	if (hi556_txd_otp.Module_Info_Flag == 0x37)
		addr = 0x424;
	else if (hi556_txd_otp.Module_Info_Flag == 0x13)
		addr = 0x413;
	else if (hi556_txd_otp.Module_Info_Flag == 0x01)
		addr = 0x402;
	else
		addr = 0;
	printk("HI556_TXD module info start addr = 0x%x \n", addr);
	...
	for (i =0; i <16; i++) 
	{
		...
		printk("HI556_TXD[%d] module_ifo_data[%d] = 0x%x", i,i,pinputdata[i]);
	}
	module_info_checksum = read_cmos_sensor(0x108);
	checksum = (checksum % 0xFF) + 1;
	printk("HI556_TXD module_info_checksum = %d, %d  ", module_info_checksum, checksum);
	if (checksum == module_info_checksum)
		{
		printk("HI556_TXD_Sensor: Module information checksum PASS\n ");
		}
	else
		{
		printk("HI556_TXD_Sensor: Module information checksum Fail\n ");
		}

	//2. read wb
	hi556_txd_otp.WB_FLAG = OTP_read_cmos_sensor(0x435);
	if (hi556_txd_otp.WB_FLAG == 0x37)
		addr = 0x472;
	else if (hi556_txd_otp.WB_FLAG == 0x13)
		addr = 0x454;
	else if (hi556_txd_otp.WB_FLAG == 0x01)
		addr = 0x436;
	else
		addr = 0;
	printk("HI556_TXD wb start addr = 0x%x \n", addr);
	...
	for (i =0; i <29; i++) 
	{
		...
		printk("HI556_TXD[%d] wb_data[%d] = 0x%x  ", i+16,i,pinputdata[i+16]);
	}
	wb_data_checksum = read_cmos_sensor(0x108);
	checksum = (checksum % 0xFF) + 1;
	printk("HI556_TXD wb_data_checksum = %d, %d  ", wb_data_checksum, checksum);
	if (checksum == wb_data_checksum)
		{
		printk("HI556_TXD_Sensor: wb data checksum PASS\n ");
		}
	else
		{
		printk("HI556_TXD_Sensor: wb data checksum Fail\n ");
		}

	//3. read lsc
	hi556_txd_otp.LSC_FLAG = OTP_read_cmos_sensor(0x490);
	if (hi556_txd_otp.LSC_FLAG == 0x37)
		addr = 0x1339;
	else if (hi556_txd_otp.LSC_FLAG == 0x13)
		addr = 0xbe5;
	else if (hi556_txd_otp.LSC_FLAG == 0x01)
		addr = 0x491;
	else
		addr = 0;
	printk("HI556_TXD lsc start addr = 0x%x \n", addr);
	...
	checksum = (checksum % 0xFF) + 1;
	printk("HI556_TXD lsc_data_checksum = %d, %d  ", lsc_data_checksum, checksum);
	if (checksum == lsc_data_checksum)
		{
		printk("HI556_TXD_Sensor: lsc data checksum PASS\n ");
		}
	else
		{
		printk("HI556_TXD_Sensor: lsc data checksum Fail\n ");
		}

	//4. read af
/*	hi556_txd_otp.AF_FLAG = OTP_read_cmos_sensor(0x184b);
	if (hi556_txd_otp.AF_FLAG == 0x01)
		addr = 0x184c;
	else if (hi556_txd_otp.AF_FLAG == 0x13)
		addr = 0x1851;
	else if (hi556_txd_otp.AF_FLAG == 0x37)
		addr = 0x1856;
	else
		addr = 0;
	printk("HI556_TXD af start addr = 0x%x \n", addr);
	...
	for (i =0; i <4; i++) 
	{
		...
		printk("HI556_TXD[%d] af_data[%d] = 0x%x  ", i+1898,i,pinputdata[i+1898]);
	}
	af_data_checksum = read_cmos_sensor(0x708);
	checksum = (checksum % 0xFF) + 1;
	printk("HI556_TXD af_data_checksum = %d, %d  ", af_data_checksum, checksum);
	if (checksum == af_data_checksum)
		{
		printk("HI556_TXD_Sensor: af data checksum PASS\n ");
		}
	else
		{
		printk("HI556_TXD_Sensor: af data checksum Fail\n ");
		}*/
...
unsigned int hi556_txd_read_region(struct i2c_client *client, unsigned int addr,
				unsigned char *data, unsigned int size)
{
	g_pstI2CclientG = client;
	if (hi556_txd_iReadData(addr, size, data) == 0)
		return size;
	else
		return 0;
}
