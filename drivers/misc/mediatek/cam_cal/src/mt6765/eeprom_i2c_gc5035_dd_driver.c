...
#define gc5035_dd_OTP_DEBUG  1
#define gc5035_dd_I2C_ID     0x6e /*0x20*/
struct gc5035_dd_otp_struct
{
...
/*static u16 read_cmos_sensor(u16 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, (u8 *)&get_byte, 1, gc5035_dd_I2C_ID);
...
/*static void write_cmos_sensor(u16 addr, u16 para)
{
	...
	iWriteRegI2C(pu_send_cmd, 2, gc5035_dd_I2C_ID);
}*/
/*
static void write_cmos_sensor_8(u16 addr, u16 para)
{
	...
	iWriteRegI2C(pu_send_cmd, 3, gc5035_dd_I2C_ID);
}*/
...
static u16 read_cmos_sensor(u16 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 1, (u8 *)&get_byte, 1, gc5035_dd_I2C_ID);
...
static void gc5035_dd_otp_init(void)
{
	printk("gc5035_dd otp_init_setting\n");
	//sensor init --start
...
int gc5035_dd_iReadData(unsigned int ui4_offset,
...
{       
	...
	u4CurrentOffset = ui4_offset;
        printk("GC5035_DD  ui4_offset = %d",u4CurrentOffset);
	gc5035_dd_otp_init();
	...
	gc5035_dd_otp.Module_Info_Flag = read_cmos_sensor(0x6c);//gc5035_otp_read_byte(0xa48);
	if (gc5035_dd_otp.Module_Info_Flag == 0x37)
		addr = 0xb10;
	else if (gc5035_dd_otp.Module_Info_Flag == 0x13)
		addr = 0xab0;
	else if (gc5035_dd_otp.Module_Info_Flag == 0x01)
		addr = 0xa50;
	else
		addr = 0;
	printk("GC5035_DD module info start addr = 0x%x %d \n", addr,gc5035_dd_otp.Module_Info_Flag);
	...
	printk("GC5035_DD[%d] module_ifo_data[%d] = 0x%x", i,i,pinputdata[i]);
	}
	...
	printk("GC5035_DD module_info_checksum = %d, %d  ", module_info_checksum, checksum);
	if (checksum == module_info_checksum)
		{
		printk("GC5035_DD_Sensor: Module information checksum PASS\n ");
		}
	else
		{
		printk("GC5035_DD_Sensor: Module information checksum Fail\n ");
		}

	//2. read wb
	gc5035_dd_otp.WB_FLAG = gc5035_otp_read_byte(0xb70);
	if (gc5035_dd_otp.WB_FLAG == 0x37)
		addr = 0xc48;
	else if (gc5035_dd_otp.WB_FLAG == 0x13)
		addr = 0xbe0;
	else if (gc5035_dd_otp.WB_FLAG == 0x01)
		addr = 0xb78;
	else
		addr = 0;
	printk("GC5035_DD wb start addr = 0x%x \n", addr);
	...
	for (i =0; i <12; i++) 
	{
		...
		printk("GC5035_DD[%d] wb_data[%d] = 0x%x  ", i+11,i,pinputdata[i+11]);
	}
	wb_data_checksum = read_cmos_sensor(0x6c);
	write_cmos_sensor(0xf3, 0x00);
	checksum = (checksum % 0xFF) + 1;
	printk("GC5035_DD wb_data_checksum = %d, %d  ", wb_data_checksum, checksum);
	if (checksum == wb_data_checksum)
		{
		printk("GC5035_DD_Sensor: wb data checksum PASS\n ");
		}
	else
		{
		printk("GC5035_DD_Sensor: wb data checksum Fail\n ");
		}

	//3. read lsc
/*	gc5035_dd_otp.LSC_FLAG = OTP_read_cmos_sensor(0x490);
	if (gc5035_dd_otp.LSC_FLAG == 0x37)
		addr = 0x1339;
	else if (gc5035_dd_otp.LSC_FLAG == 0x13)
		addr = 0xbe5;
	else if (gc5035_dd_otp.LSC_FLAG == 0x01)
		addr = 0x491;
	else
		addr = 0;
	printk("GC5035_DD lsc start addr = 0x%x \n", addr);
	...
	for (i =0; i <1868; i++) 
	{
		...
		printk("GC5035_DD[%d] lsc_data[%d] = 0x%x", i+30, i,pinputdata[i+30]);
	}
	lsc_data_checksum = read_cmos_sensor(0x108);
	checksum = (checksum % 0xFF) + 1;
	printk("GC5035_DD lsc_data_checksum = %d, %d  ", lsc_data_checksum, checksum);
	if (checksum == lsc_data_checksum)
		{
		printk("GC5035_DD_Sensor: lsc data checksum PASS\n ");
		}
	else
		{
		printk("GC5035_DD_Sensor: lsc data checksum Fail\n ");
		}*/

	//4. read af
/*	gc5035_dd_otp.AF_FLAG = OTP_read_cmos_sensor(0x184b);
	if (gc5035_dd_otp.AF_FLAG == 0x01)
		addr = 0x184c;
	else if (gc5035_dd_otp.AF_FLAG == 0x13)
		addr = 0x1851;
	else if (gc5035_dd_otp.AF_FLAG == 0x37)
		addr = 0x1856;
	else
		addr = 0;
	printk("GC5035_DD af start addr = 0x%x \n", addr);
	...
	for (i =0; i <4; i++) 
	{
		...
		printk("GC5035_DD[%d] af_data[%d] = 0x%x  ", i+1898,i,pinputdata[i+1898]);
	}
	af_data_checksum = read_cmos_sensor(0x708);
	checksum = (checksum % 0xFF) + 1;
	printk("GC5035_DD af_data_checksum = %d, %d  ", af_data_checksum, checksum);
	if (checksum == af_data_checksum)
		{
		printk("GC5035_DD_Sensor: af data checksum PASS\n ");
		}
	else
		{
		printk("GC5035_DD_Sensor: af data checksum Fail\n ");
		}*/
...
unsigned int gc5035_dd_read_region(struct i2c_client *client, unsigned int addr,
				unsigned char *data, unsigned int size)
{
	g_pstI2CclientG = client;
	if (gc5035_dd_iReadData(addr, size, data) == 0)
		return size;
	else
		return 0;
}
