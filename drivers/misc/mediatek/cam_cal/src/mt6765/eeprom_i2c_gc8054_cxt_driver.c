...
static void gc8054_cxt_otp_init(void)
{
...
static void gc8054_cxt_otp_close(void)
{
...
/*TabA7 Lite for camera performance add by gaozhenyu 2021/06/15 start*/
static u16 gc8054_cxt_otp_read_group(u16 addr, u8 *data, u16 length)
{       
	u16 i = 0;
...
//moudle info
	...
	printk("gc8054_cxt module_info_checksum = %d, %d  ", module_info_checksum, checksum);
        	if (checksum == module_info_checksum)
		{
		printk("gc8054_cxt module_info_checksum Module information checksum PASS\n ");
		}
	else
		{
		printk("gc8054_cxt module_info_checksum Module information checksum Fail\n ");
		}
//awb 
        write_cmos_sensor(0x0313, 0x00);
	...
	pr_debug("awb:i = %d, data = 0x%x\n",i+15, data[i+15]);
        ...
        checksum = (checksum % 0xFF);
	printk("gc8054_cxt awb_checksum = %d, %d  ", wb_data_checksum, checksum);
        	if (checksum == wb_data_checksum)
		{
		printk("gc8054_cxt wb_data_checksum awb checksum PASS\n ");
		}
	else
		{
		printk("gc8054_cxt wb_data_checksum awb checksum Fail\n ");
		}
//lsc
        write_cmos_sensor(0x0313, 0x00);
	...
	pr_debug("lsc:i = %d, data = 0x%x\n",i+32, data[i+32]);
        ...
        checksum = (checksum % 0xFF);
	printk("gc8054_cxt lsc_data_checksum = %d, %d  ", lsc_data_checksum, checksum);
        	if (checksum == lsc_data_checksum)
		{
		printk("gc8054_cxt lsc_data_checksum lsc checksum PASS\n ");
		}
	else
		{
		printk("gc8054_cxt lsc_data_checksum lsc checksum Fail\n ");
		}
//af
        write_cmos_sensor(0x0313, 0x00);
	...
	pr_debug("af:i = %d, data = 0x%x\n",i+1901, data[i+1901]);
        ...
        checksum = (checksum % 0xFF);
	printk("gc8054_cxt af_data_checksum = %d, %d  ", af_data_checksum, checksum);
        	if (checksum == af_data_checksum)
		{
		printk("gc8054_cxt af_data_checksum af checksum PASS\n ");
		}
	else
		{
		printk("gc8054_cxt af_data_checksum af checksum Fail\n ");
		}
//other
/*
pr_debug("other = %d\n", y);
...
int gc8054_cxt_iReadData(unsigned int ui4_offset,
	unsigned int ui4_length, unsigned char *pinputdata)
{
	...
	i4RetValue =
			gc8054_cxt_otp_read_group(
			(u16) u4CurrentOffset, pBuff, i4ResidueDataLength);
		if (i4RetValue != 0) {
			pr_debug("I2C iReadData failed!!\n");
			return -1;
		}
	gc8054_cxt_otp_close();

	return 0;
}
/*TabA7 Lite code for SR-AX3565-01-320 by wangqi at 20210104 end*/

unsigned int gc8054_cxt_read_region(struct i2c_client *client, unsigned int addr,
				unsigned char *data, unsigned int size)
{
	g_pstI2CclientG = client;
        printk("gc8054_cxt eeprom enter\n ");
	if (gc8054_cxt_iReadData(addr, size, data) == 0)
		return size;
	else
		return 0;
}
