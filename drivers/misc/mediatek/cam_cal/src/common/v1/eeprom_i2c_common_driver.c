...
unsigned int Common_read_otp_cal(struct i2c_client *client, struct stCAM_CAL_INFO_STRUCT *sensor_info,
				unsigned int addr, unsigned char *data, unsigned int size)
{
	int read_size = 0;

	if (sensor_info == NULL) {
		pr_err("%s failed, sensor_info is NULL", __func__);
		return 0;
	}

	pr_info("%s, sensor_id = %#06x", __func__, sensor_info->sensorID);
	read_size = IMGSENSER_READ_OTP_CAL(sensor_info->deviceID, sensor_info->sensorID, addr, data, size);
	return read_size;
}
