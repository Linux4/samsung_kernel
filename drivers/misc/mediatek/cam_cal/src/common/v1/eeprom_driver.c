...
extern int imgsensor_sys_get_cal_size(unsigned int dualDeviceId, unsigned int sensorId);
extern int imgsensor_sysfs_update(unsigned char* pRomData, unsigned int dualDeviceId, unsigned int sensorId, unsigned int offset, unsigned int length, int i4RetValue);
...
#endif
{
	...
	case CAM_CALIOC_S_WRITE:	/*Note: Write Command is Unverified! */
		...
		pcmdInf->maxEepromSize =
			imgsensor_sys_get_cal_size(ptempbuf->deviceID, ptempbuf->sensorID);

		/* Check the max size if specified */
		if (pcmdInf != NULL &&
			(pcmdInf->maxEepromSize != 0) &&
			(pcmdInf->maxEepromSize < (ptempbuf->u4Offset + ptempbuf->u4Length))) {
		...
		if (pcmdInf != NULL && g_lastDevID != ptempbuf->deviceID) {
			if (EEPROM_set_i2c_bus(ptempbuf->deviceID, pcmdInf) != 0) {
	...
	case CAM_CALIOC_G_READ:
		...
		if (ptempbuf->u4Length == 0) {
			// get cal size
			i4RetValue = imgsensor_sys_get_cal_size(ptempbuf->deviceID, ptempbuf->sensorID);
		} else {
	...
	if ((_IOC_READ & _IOC_DIR(a_u4Command)) && ptempbuf->u4Length != 0) {
