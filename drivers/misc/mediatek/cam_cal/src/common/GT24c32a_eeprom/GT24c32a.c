	...
	for (loopCount = 0; loopCount < Read_NUMofEEPROM; loopCount++) {
		do {
			if (16 <= loop[loopCount]) {
				...
				EEPROMDB("puSendCmd[1]=%x, EEPROM(%x)\n",
...
#ifdef CONFIG_COMPAT
static int compat_put_cal_info_struct(COMPAT_stCAM_CAL_INFO_STRUCT __user *data32,
...
static int compat_get_cal_info_struct(COMPAT_stCAM_CAL_INFO_STRUCT __user *data32,
				      stCAM_CAL_INFO_STRUCT __user *data)
{
	compat_uptr_t p;
	compat_uint_t i;
...
static long CAM_CAL_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_CAM_CALIOC_G_READ:{
			...
			ret =
			    filp->f_op->unlocked_ioctl(filp, CAM_CALIOC_G_READ,
						       (unsigned long)data);
...
int GT24C32A_EEPROM_Ioctl(struct file *file, unsigned int a_u4Command, unsigned long a_u4Param)
...
*/
	if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user
			    ((u8 *) pBuff, (u8 *) a_u4Param, sizeof(stCAM_CAL_INFO_STRUCT))) {
	...
	case CAM_CALIOC_G_READ:
		...
		if (0) {
			...
			for (addr = 0; addr < 256; addr++) {
				for (loop = 0x07FC; loop <= 0x07FF; loop++) {
					...
					EEPROMDB
					    ("[EEPROM]enter EEPROM_test function addr(%x) (%x)%x\n",
					     addr, loop, get_byte);
		...
		if (ptempbuf->u4Offset == 0x800) {
			*(u16 *) pWorkingBuff = 0x3;
		} else if (((ptempbuf->u4Offset & 0x000FFFFF) == 0x00024C32)
			...
			*(u32 *) pWorkingBuff = 0x0028C064;
			...
			while (0 < readTryagain) {
				i4RetValue =
				    iReadDataFromGT24c32a(ptempbuf->u4Offset, ptempbuf->u4Length,
							  pWorkingBuff);
