...
static struct i2c_board_info kd_eeprom_dev __initdata = {
	I2C_BOARD_INFO("XXXXXXXXXXX", 0xB0 >> 1)
};
#endif
...
int iWriteEEPROM_zc533(u16 a_u2Addr, u32 a_u4Bytes, u8 *puDataInBytes)
{
	int i4RetValue = 0;
	u32 u4Index = 0;
	...
	if (a_u4Bytes + 2 > 8) {
		EEPROMDB
		    ("[ZC533EEPROM] exceed I2c-mt65xx.c 8 bytes limitation (include address 2 Byte)\n");
		return -1;
	}
	...
	if (i4RetValue != (a_u4Bytes + 2)) {
		EEPROMDB("[ZC533EEPROM] I2C write  failed!!\n");
		return -1;
	}
...
int iReadEEPROM_zc533(u16 a_u2Addr, u32 ui4_length, u8 *a_puBuff)
{
	int i4RetValue = 0;
	...
	if (ui4_length > 8) {
		EEPROMDB("[ZC533EEPROM] exceed I2c-mt65xx.c 8 bytes limitation\n");
		return -1;
	}
...
int iWriteData_zc533(unsigned int ui4_offset, unsigned int ui4_length, unsigned char *pinputdata)
{
	int i4RetValue = 0;
	...
	EEPROMDB("[ZC533EEPROM] iWriteData\n");

#if 0
	if (ui4_offset + ui4_length >= 0x2000) {
		EEPROMDB("[ZC533EEPROM] Write Error!! S-24CS64A not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	EEPROMDB("[ZC533EEPROM] iWriteData u4CurrentOffset is %d\n", u4CurrentOffset);
...
#endif
	EEPROMDB("[ZC533EEPROM] iWriteData done\n");
	...
	for (loopCount = 0; loopCount < Read_NUMofEEPROM; loopCount++) {
		do {
			if (16 <= loop[loopCount]) {
				...
				EEPROMDB("[EEPROM]1 loopCount=%d loop[loopCount]=%d ",
					loopCount, loop[loopCount]);
				EEPROMDB("puSendCmd[0]=%x puSendCmd[1]=%x, EEPROM(%x)\n",
					puSendCmd[0], puSendCmd[1], EEPROM_Address[loopCount]);
				/* iReadRegI2C(puSendCmd , 2, (u8*)pBuff,16,EEPROM_Address[loopCount]); */
				i4RetValue =
				    iBurstReadRegI2C(puSendCmd, 2, (u8 *) pBuff, 16,
				...
				EEPROMDB
				    ("[EEPROM]2 loopCount=%d loop[loopCount]=%d puSendCmd[0]=%x puSendCmd[1]=%x\n",
...
int iReadData_zc533(unsigned int ui4_offset, unsigned int ui4_length, unsigned char *pinputdata)
{
	int i4RetValue = 0;
	int i4ResidueDataLength;
	u32 u4IncOffset = 0;
...
/* EEPROMDB("[S24EEPORM] iReadData\n" ); */
	...
	if (ui4_offset + ui4_length >= 0x2000) {
		EEPROMDB("[ZC533EEPROM] Read Error!! S-24CS64A not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	do {
		if (i4ResidueDataLength >= 8) {
			i4RetValue = iReadEEPROM_zc533((u16) u4CurrentOffset, 8, pBuff);
			if (i4RetValue != 0) {
				EEPROMDB("[EEPROM] I2C iReadData failed!!\n");
				return -1;
			}
			...
			i4RetValue =
			    iReadEEPROM_zc533((u16) u4CurrentOffset, i4ResidueDataLength, pBuff);
			if (i4RetValue != 0) {
				EEPROMDB("[EEPROM] I2C iReadData failed!!\n");
				return -1;
			}
...
int ZC533_EEPROM_Ioctl(struct file *file, unsigned int a_u4Command, unsigned long a_u4Param)
#else
...
#endif				/* COMMON_CAM_CAL_DRV */
{
	...
	if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff, (u8 *) a_u4Param,
				...
				kfree(pBuff);
				EEPROMDB("[ZC533EEPROM] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
	...
	case CAM_CALIOC_S_WRITE:
		EEPROMDB("[ZC533EEPROM] Write CMD\n");
#ifdef EEPROMGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		/* i4RetValue = iWriteData_zc533((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff); */
#ifdef EEPROMGETDLT_DEBUG
	...
	case CAM_CALIOC_G_READ:
		EEPROMDB("[ZC533EEPROM] Read CMD\n");
#ifdef EEPROMGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		EEPROMDB("[ZC533EEPROM] offset %x\n", ptempbuf->u4Offset);
		EEPROMDB("[ZC533EEPROM] length %x\n", ptempbuf->u4Length);
		EEPROMDB("[ZC533EEPROM] Before read Working buffer address 0x%8x\n",
			 (u32) pWorkingBuff);
		...
		if (ptempbuf->u4Offset == 0x800) {
			...
			*(u32 *) pWorkingBuff = 0x0124C32a;
			...
			i4RetValue =
			    iReadDataFromM24C08F((u16) ptempbuf->u4Offset, ptempbuf->u4Length,
						 pWorkingBuff);

		}
		EEPROMDB("[ZC533EEPROM] After read Working buffer data  0x%4x\n", *pWorkingBuff);
#else
		...
		if (ptempbuf->u4Offset == 0x800)
			...
			*(u32 *) pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
			...
			EEPROMDB("[ZC533EEPROM] Init I2C address %x\n", *(u32 *) pWorkingBuff);
		} else if (((ptempbuf->u4Offset & 0x000FFFFF) == 0x0000C533)
			   && (ResetCheck == 0xABCDFEDC)) {
			*(u32 *) pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);


			EEPROMDB("[ZC533EEPROM] Reset I2C address %x\n", *(u32 *) pWorkingBuff);
		} else {
			readTryagain = 3;
			while (0 < readTryagain) {
				i4RetValue =
				    iReadDataFromZc533(ptempbuf->u4Offset, ptempbuf->u4Length,
						       pWorkingBuff);
				EEPROMDB("[ZC533EEPROM] error (%d) Read retry (%d)\n", i4RetValue,
					 readTryagain);
				if (i4RetValue != 0)
					readTryagain--;
				else
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		if (copy_to_user
			...
			kfree(pBuff);
			kfree(pWorkingBuff);
			EEPROMDB("[ZC533EEPROM] ioctl copy to user failed\n");
			return -EFAULT;
		}
	}
...
{
	EEPROMDB("[ZC533EEPROM] EEPROM_Open\n");
	spin_lock(&g_EEPROMLock);
	if (g_u4Opened) {
		spin_unlock(&g_EEPROMLock);
		return -EBUSY;
...
static int RegisterEEPROMCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_EEPROMdevno, 0, 1, EEPROM_DRVNAME)) {
		EEPROMDB("[ZC533EEPROM] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_EEPROMdevno, 1, EEPROM_DRVNAME)) {
		EEPROMDB("[ZC533EEPROM] Register device no failed\n");
	...
	if (NULL == g_pEEPROM_CharDrv) {
		...
		EEPROMDB("[ZC533EEPROM] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pEEPROM_CharDrv, g_EEPROMdevno, 1)) {
		EEPROMDB("[ZC533EEPROM] Attatch file operation failed\n");
...
static unsigned short force[] = {
	EEPROM_I2C_GROUP_ID, ZC533_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END
};
#else
/* static unsigned short force[] = {IMG_SENSOR_I2C_GROUP_ID, ZC533_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END}; */
#endif
...
static int EEPROM_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	EEPROMDB("[ZC533EEPROM] Attach I2C\n");
/* spin_lock_init(&g_EEPROMLock); */

	/* get sensor i2c client */
	spin_lock(&g_EEPROMLock);	/* for SMP */
	g_pstI2Cclient = client;
	g_pstI2Cclient->addr = ZC533_DEVICE_ID >> 1;
	spin_unlock(&g_EEPROMLock);	/* for SMP */
	...
	if (i4RetValue) {
		EEPROMDB("[ZC533EEPROM] register char device failed!\n");
		return i4RetValue;
	}


	EEPROMDB("[ZC533EEPROM] Attached!!\n");
	return 0;
}
...
static int __init EEPROM_i2C_init(void)
{
	...
	if (platform_driver_register(&g_stEEPROM_Driver)) {
		EEPROMDB("failed to register ZC533EEPROM driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stEEPROM_Device)) {
		EEPROMDB("failed to register ZC533EEPROM driver, 2nd time\n");
		return -ENODEV;
	}
...
#if defined(COMMON_EEPROM)
EXPORT_SYMBOL(ZC533_EEPROM_Ioctl);
#else
