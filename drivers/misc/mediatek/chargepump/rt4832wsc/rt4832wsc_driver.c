...
unsigned int bright_arr[] = {
	7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 10, 13, 16, 20, 23, 26, 29, 33, 36,	/* 19 */
	39, 42, 46, 49, 52, 55, 59, 62, 65, 68, 72, 75, 78, 81, 85, 88, 91, 95, 98, 101,	/* 39 */
	104, 108, 111, 114, 117, 121, 124, 127, 130, 134, 137, 140, 143, 147, 150, 153, 156, 160, 163, 166,	/* 59 */
	170, 173, 176, 180, 183, 186, 190, 193, 197, 200, 203, 207, 210, 214, 217, 220, 224, 227, 231, 234,	/* 79 */
	237, 241, 244, 248, 251, 254, 258, 261, 265, 268, 271, 275, 278, 282, 285, 288, 292, 295, 299, 302,	/* 99 */
	305, 309, 312, 316, 319, 322, 326, 329, 333, 336, 339, 342, 346, 349, 352, 356, 359, 362,
	    366, 369,
	372, 376, 379, 382, 386, 389, 392, 396, 399, 402, 406, 409, 412, 416, 419, 422, 426, 429,
	    432, 436,
	439, 442, 446, 449, 452, 456, 459, 462, 466, 469, 472, 476, 479, 482, 486, 489, 492, 496,
	    499, 502,
	505, 509, 512, 515, 519, 522, 525, 529, 532, 535, 539, 542, 545, 549, 552, 555, 559, 562,
	    565, 569,
	572, 575, 579, 582, 585, 589, 592, 595, 599, 602, 605, 609, 612, 615, 619, 622, 625, 629,
	    632, 635,
	639, 642, 645, 649, 652, 655, 659, 662, 665, 668, 672, 675, 678, 682, 685, 688, 692, 695,
	    698, 702,
	705, 708, 712, 715, 718, 722, 725, 728, 732, 735, 738, 742, 745, 748, 752, 755, 758, 762,
	    765, 768,
	772, 775, 778, 782, 785, 788, 792, 795, 798, 802, 805, 808, 812, 815, 818, 822	/* 255 */
};
#endif
#endif
...
void rt4832_dsv_ctrl(int enable)
{
	...
	if (enable == 1) {
		...
		data = 0xA8;	/* M1V DSV Vboost voltage 6.0V and 2ms delay between VPOS and VNEG */
#elif defined(TARGET_MT6735_K6M)
		data = 0xE2;	/* Vboost voltage 5.7V and 5ms delay between VPOS and VNEG */
#elif defined(TARGET_MT6735_K6P)
		data = 0xAA;	/* Vboost voltage 6.1V and 2ms delay between VPOS and VNEG */
#else
		data = 0xA2;	/* Vboost voltage 5.7V and 2ms delay between VPOS and VNEG */
#endif
		rt4832_smbus_write_byte(new_client, 0x0D, &data);

#if defined(TARGET_MT6735_M1V)
		data = 0x24;	/* M1V DSV output voltage is +-5.8V */
#elif defined(TARGET_MT6735_K6P)
		data = 0x5A;	/* DSV output voltage is +5.3V, 5.6mV/us */
#else
...
#if defined(TARGET_MT6735_K6P)
		data = 0x68;	/* DSV output voltage is -6.0V, 6.2mV/us */
#endif
		rt4832_smbus_write_byte(new_client, 0x0F, &data);

#if defined(TARGET_MT6735_K6M) || defined(FT8707_FHD_DSI_VDO_LGD)
		data = 0x0A;	/* i2c ctrl */
#else
		data = 0x2A;
#endif
...
#if defined(TARGET_MT6735_K6M) || defined(FT8707_FHD_DSI_VDO_LGD)
		data = 0x48;	/* VPOS VNEG enable through i2c */
#else
		data = 0x00;
#endif
...
#if defined(TARGET_MT6735_K6P)
				data1 = 0x90;	/* OVP 33V, linear mode */
#else
				data1 = 0x70;	/* 0x57; */
#endif
				rt4832_smbus_write_byte(new_client, 0x02, &data1);
...
#if defined(TARGET_MT6735_K6P)
			data1 = 0x80;	/* BL Frequence 1Mh, 10 bit brightness control */
#else
			data1 = 0x00;	/* 10 bit brightness control */
#endif
...
#if defined(TARGET_MT6735_K6P)
			data = 0x90;	/* OVP 33V, linear mode */
#else
			data = 0x70;	/* 0x57; */
#endif
			rt4832_smbus_write_byte(new_client, 0x02, &data);
#if defined(TARGET_MT6735_K6P)
			data = 0x80;	/* BL Frequence 1Mh, 10 bit brightness control */
#else
			data = 0x00;	/* 10 bit brightness control */
#endif
