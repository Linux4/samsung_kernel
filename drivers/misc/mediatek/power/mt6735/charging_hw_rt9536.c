...
static unsigned int charging_enable(void *data)
{
    ...
    battery_log(BAT_LOG_CRTI, "[CHARGER] charging_enable (enable=%d)\n",enable);
    battery_log(BAT_LOG_CRTI, "[CHARGER] charging_enable (g_charging_enabled=%d), (!g_charging_setting_chagned=%d)\n",g_charging_enabled, !g_charging_setting_chagned);    
    
#if 1   // TEST
	if (enable == g_charging_enabled && !g_charging_setting_chagned){
        battery_log(BAT_LOG_CRTI, "[CHARGER] charging_enable RETURN !!!!!!!!!\n");
		return STATUS_OK;
	}
#endif /* 0 */
...
static unsigned int charging_set_current(void *data)
{
	...
	battery_log(BAT_LOG_CRTI, "[CHARGER] charging_set_current : Set Current to %dmA\n", g_charging_current);
...
static unsigned int charging_get_charger_det_status(void *data)
{
	...
	battery_log(BAT_LOG_CRTI, "[CHARGER] g_charger_type = %d\n", g_charger_type);
