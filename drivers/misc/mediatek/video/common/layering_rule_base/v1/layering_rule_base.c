...
static int get_ovl_idx_by_phy_layer(int layer_map_tb, int phy_layer_idx)
{
	...
	DISPINFO("[DISP]%s, phy_layer_idx:%d, layer_map_tb:0x%x, layer_idx:%d ovl_idx:%d, ovl_mapping_tb:0x%x\n",
	...
	enum DISP_DEBUG_LEVEL debug_level)
{
	...
	if (debug_level < DISP_DEBUG_LEVEL_INFO) {
		DISPINFO("[DISP]HRT hrt_num:%d/fps:%d/dal:%d/p:%d/r:%s/layer_tb:%d/bound_tb:%d/dc:%d\n",
		...
		for (i = 0 ; i < 2 ; i++) {
			...
			for (j = 0 ; j < disp_info->layer_num[i] ; j++) {
				...
				DISPINFO("[DISP]L%d->%d/of(%d,%d)/swh(%d,%d)/dwh(%d,%d)/fmt:0x%x/ext:%d/caps:0x%x\n",
					j, layer_info->ovl_id,
		...
		for (i = 0 ; i < 2 ; i++) {
			...
			DISPINFO("[DISP]HRT D%d/M%d/LN%d/hrt_num:%d/G(%d,%d)\n",
				i, disp_info->disp_mode[i],
 ...
 */
	...
	DISPINFO("%s disp_index:%d, disp_index:%d, hrt_type:%d, sum_overlap_w:%d\n",
...
int check_disp_info(struct disp_layer_info *disp_info)
{
	...
	for (disp_idx = 0 ; disp_idx < 2 ; disp_idx++) {
		...
		if (disp_info->layer_num[disp_idx] > 0 &&
			...
			DISPINFO("[DISP][%s #%d]ERROR:[HRT]Has input layer, but input config is empty, disp_idx:%d, layer_num:%d\n",
				__func__, __LINE__,
		...
		if ((disp_info->gles_head[disp_idx] < 0 &&
			...
			disp_info->layer_num[disp_idx])))) {
			dump_disp_info(disp_info, DISP_DEBUG_LEVEL_ERR);
			DISPINFO("[DISP][%s #%d]ERROR:[HRT]gles layer invalid, disp_idx:%d, head:%d, tail:%d, layer_num:%d\n",
				__func__, __LINE__,
...
int set_disp_info(struct disp_layer_info *disp_info_user, int debug_mode)
{
	...
	if (layering_info.layer_num[0]) {
		...
		if (layering_info.input_config[0] == NULL) {
			DISPINFO("[DISP][%s #%d]ERROR:[HRT]:alloc input config 0 fail,layer_num:%d\n",
				__func__, __LINE__,
	...
	if (layering_info.layer_num[1]) {
		...
		if (layering_info.input_config[1] == NULL) {
			DISPINFO("[DISP][%s #%d]ERROR:[HRT]: alloc input config 1 fail, layer_num:%d\n",
				__func__, __LINE__,
		...
		if (debug_mode) {
			...
			if (copy_from_user(layering_info.input_config[1],
				...
				sizeof(struct layer_config)
				* layering_info.layer_num[1])) {
				DISPINFO("[DISP][%s #%d]ERROR:[FB]: copy_from_user failed! line:%d\n",
 ...
 */
	...
	if (disp_helper_get_option(DISP_OPT_RSZ)) {
		if (l_rule_ops->resizing_rule)
			...
			DISPINFO("[DISP][%s #%d]warn:RSZ feature on, but no resizing rule be implement.\n",
				__func__, __LINE__);
	} else {
	...
	while (1) {
		...
		if (strncmp(line_buf, "#", 1) == 0) {
			...
			if (layer_result != tmp_config.ovl_id) {
				DISPINFO("[DISP][%s #%d]warn:Test case:%d, ovl_id incorrect, real is %d, expect is %d\n",
					__func__, __LINE__,
					(int)test_case,
					tmp_config.ovl_id,
					(int)layer_result);
			...
			if (layer_result != tmp_config.ext_sel_layer) {
				DISPINFO("[DISP][%s #%d]warn:Test case:%d, ext_sel_layer incorrect, real is %d, expect is %d\n",
					__func__, __LINE__,
					(int)test_case,
			...
			if (gles_num != disp_info->gles_head[disp_id]) {
				DISPINFO("[DISP][%s #%d]warn:Test case:%d, gles head incorrect, gles head is %d, expect is %d\n",
					__func__, __LINE__,
					(int)test_case,
			...
			if (gles_num != disp_info->gles_tail[disp_id]) {
				DISPINFO("[DISP][%s #%d]warn:Test case:%d, gles tail incorrect, gles tail is %d, expect is %d\n",
					__func__, __LINE__,
					(int)test_case,
			...
			if (hrt_num != HRT_GET_DVFS_LEVEL(disp_info->hrt_num))
				DISPINFO("[DISP][%s #%d]warnTest case:%d, hrt num incorrect, hrt_num is %d, expect is %d\n",
					__func__, __LINE__,
					(int)test_case,
					HRT_GET_DVFS_LEVEL(disp_info->hrt_num),
					(int)hrt_num);
			...
			if (hrt_num != (tmp & 0x1F)) {
				DISPINFO("[DISP][%s #%d]warn:Test case:%d, hrt path incorrect, disp_path is %d, expect is %d\n",
					__func__, __LINE__,
					(int)test_case,
					tmp & 0x1F,
					(int)hrt_num);
			...
			if (hrt_num !=
				HRT_GET_SCALE_SCENARIO(disp_info->hrt_num)) {
				DISPINFO("[DISP][%s #%d]warn:Test case:%d, hrt scale scenario incorrect, hrt scale is %d, expect is %d\n",
					__func__, __LINE__,
					(int)test_case,
					tmp_hrt_num,
					(int)hrt_num);
