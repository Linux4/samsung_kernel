...
int _ioctl_pass_desensetbl(unsigned long arg)
{
	void __user *argp = (void __user *)arg;
	desenseTbl desenData[TBL_SIZE];

	pr_debug("%s\n", __func__);

	if (copy_from_user(&desenData, argp, sizeof(desenseTbl) * TBL_SIZE)) {
		pr_err("Don't Get Desense Tbl data");
		return -EFAULT;
	}
	read_desense_from_file(desenData);
...
#ifdef CONFIG_MTK_DYNAMIC_MIPI
	case DISP_IOCTL_PASS_DESENSETBL:
		return _ioctl_pass_desensetbl(arg);
#endif
	case DISP_IOCTL_AAL_EVENTCTL:
	case DISP_IOCTL_AAL_GET_HIST:
	case DISP_IOCTL_AAL_INIT_REG:
