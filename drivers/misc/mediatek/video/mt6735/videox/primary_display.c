...
#ifdef CONFIG_MTK_DYNAMIC_MIPI
desenseTbl g_table[TBL_SIZE];
#endif
...
int _disp_primary_path_set_vfp(int enter)
{
	...
	if (lcm_param->dsi.vertical_vfp_lp == 0) return ret;
...
static int _disp_primary_path_idle_detect_thread(void *data)
{
	...
	while (1) {
		...
		if (((sched_clock() - last_primary_trigger_time) / 1000) > idle_time * 1000) {
			...
			if (is_mmdvfs_supported() && mmdvfs_get_mmdvfs_profile() == MMDVFS_PROFILE_D1_PLUS) {
				if (_primary_path_IsForcedHPM(0) == true) {
...
int primary_display_init(char *lcm_name, unsigned int lcm_fps)
{
	...
	/*Register dynamic MIPI clk callback*/
	register_ccci_sys_call_back(MD_SYS1, MD_THERMAL_LOW_POWER, divideClkToHalf);

#ifdef CONFIG_MTK_DYNAMIC_MIPI
	register_ccci_sys_call_back(MD_SYS1, MD_DISPLAY_DYNAMIC_MIPI_2G, comp_2g_desense_to_set_clk);
	register_ccci_sys_call_back(MD_SYS1, MD_DISPLAY_DYNAMIC_MIPI_3G, comp_3g_desense_to_set_clk);
	register_ccci_sys_call_back(MD_SYS1, MD_DISPLAY_DYNAMIC_MIPI_4G, comp_4g_desense_to_set_clk);
#endif
done:
...
#ifdef CONFIG_MTK_DYNAMIC_MIPI
	exec_ccci_kern_func_by_md_id(MD_SYS1, MD_DISPLAY_DYNAMIC_MIPI_2G, &lcm_off, sizeof(lcm_off));
	exec_ccci_kern_func_by_md_id(MD_SYS1, MD_DISPLAY_DYNAMIC_MIPI_3G, &lcm_off, sizeof(lcm_off));
	exec_ccci_kern_func_by_md_id(MD_SYS1, MD_DISPLAY_DYNAMIC_MIPI_4G, &lcm_off, sizeof(lcm_off));
#endif
done:
	_primary_path_vsync_unlock();
	_primary_path_unlock(__func__);
...
#ifdef CONFIG_MTK_DYNAMIC_MIPI
	exec_ccci_kern_func_by_md_id(MD_SYS1, MD_DISPLAY_DYNAMIC_MIPI_2G, &lcm_on, sizeof(lcm_on));
	exec_ccci_kern_func_by_md_id(MD_SYS1, MD_DISPLAY_DYNAMIC_MIPI_3G, &lcm_on, sizeof(lcm_on));
	exec_ccci_kern_func_by_md_id(MD_SYS1, MD_DISPLAY_DYNAMIC_MIPI_4G, &lcm_on, sizeof(lcm_on));
#endif
done:
	_primary_path_unlock(__func__);
...
void primary_display_change_clk(uint32_t clk)
{
	int ret = 0;
	cmdqRecHandle handle = NULL;
	...
	if (handle) {
		...
		DSI_ChangeClkByCmdq(DISP_MODULE_DSI0, handle, clk);
...
int _set_lcm_func_call(void (*func4)(void *, void *, void *, void *),
...
int primary_display_setlcm_func_call(void (*func4)(void *, void *, void *, void *),
		void *data1, void *data2, void *data3, void *data4)
{
	int ret = 0;
	...
	if (pgc->state == DISP_SLEPT) {
		...
		if (primary_display_cmdq_enabled()) {
			if (primary_display_is_video_mode()) {
				MMProfileLogEx(ddp_mmp_get_events()->primary_set_bl,
					       MMProfileFlagPulse, 0, 7);
				_set_lcm_func_call(func4, data1, data2, data3, data4);
			} else {
#ifdef MTK_DISP_IDLE_LP
				/* CMD mode need to exit top clock off idle mode */
				_disp_primary_path_exit_idle("primary_display_setbacklight", 0);
...
int divideClkToHalf(int md_id, int data)
{
	disp_ddp_path_config *data_config;
	...
	if (data == 1)
		primary_display_change_clk((data_config->dispif_config.dsi.PLL_CLOCK / 2));
	else
		primary_display_change_clk(data_config->dispif_config.dsi.PLL_CLOCK);

	return 1;
}

#ifdef CONFIG_MTK_DYNAMIC_MIPI
int comp_2g_desense_to_set_clk(int md_id, int data)
{
	int idx = 0;
	disp_ddp_path_config *data_config;
	int band = data >> 16;
	int channel = 0x0000FFFF & data;
	char lcm_off = 0;
	int network = 287;

	data_config = dpmgr_path_get_last_config(pgc->dpmgr_handle);

	pr_debug("2G, band %d, channel %d\n", band, channel);
	/* when lcm is off, notify MD doesn't need to update status */
	if (pgc->state == DISP_SLEPT) {
		exec_ccci_kern_func_by_md_id(MD_SYS1, MD_DISPLAY_DYNAMIC_MIPI_2G, &lcm_off, sizeof(lcm_off));
		exec_ccci_kern_func_by_md_id(MD_SYS1, MD_DISPLAY_DYNAMIC_MIPI_3G, &lcm_off, sizeof(lcm_off));
		exec_ccci_kern_func_by_md_id(MD_SYS1, MD_DISPLAY_DYNAMIC_MIPI_4G, &lcm_off, sizeof(lcm_off));
	...
	if (channel == 65535 && pgc->state == DISP_ALIVE) {
		primary_display_change_clk(data_config->dispif_config.dsi.PLL_CLOCK);
	...
	for (idx = 0; idx < TBL_SIZE; idx++) {
		if (network == g_table[idx].network && band == g_table[idx].band
			&& channel == g_table[idx].channel) {
			primary_display_change_clk(g_table[idx].clk);
		}
	}
...
int comp_3g_desense_to_set_clk(int md_id, int data)
{
	int idx = 0;
	disp_ddp_path_config *data_config;
	int band = data >> 16;
	...
	for (idx = 0; idx < TBL_SIZE; idx++) {
		if (network == g_table[idx].network && channel == g_table[idx].channel)
			primary_display_change_clk(g_table[idx].clk);
	}
...
int comp_4g_desense_to_set_clk(int md_id, int data)
{
	int idx = 0;
	disp_ddp_path_config *data_config;
	int band = data >> 16;
...
int read_desense_from_file(desenseTbl *data)
{
	pr_debug("%s\n", __func__);

	memcpy(g_table, data, sizeof(desenseTbl) * TBL_SIZE);
