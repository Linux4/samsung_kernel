...
static void _DSI_INTERNAL_IRQ_Handler(enum DISP_MODULE_ENUM module, unsigned int param)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (status.TE_TIMEOUT)
			DDPERR("IRQ DSI TE Timeout\n");
	}
}
...
/*
 * set TE signal check expired time. if it expire the duration then
 * it output error log
 *
*/
enum DSI_STATUS DSI_TE_timeout(enum DISP_MODULE_ENUM module, struct cmdqRecStruct *cmdq)
{
	int i = 0;
	unsigned int timeout_period_in_us = 50000; /* 50ms */
	unsigned int reg_val = 0;

	reg_val = timeout_period_in_us * _dsi_context[0].dsi_params.PLL_CLOCK / (0x4000 * 4);
	DISPINFO("%s+ => %d\n", __func__, reg_val);

	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++)
		DSI_OUTREGBIT(cmdq, struct DSI_TIME_CON1_REG, DSI_REG[i]->DSI_TIME_CON1, TE_WAKEUP_PRD, reg_val);
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (dsi_params->mode == CMD_MODE
			...
			DSI_OUTREGBIT(cmdq, struct DSI_TXRX_CTRL_REG, DSI_REG[i]->DSI_TXRX_CTRL, EXT_TE_EN,
				      1);
			DSI_OUTREGBIT(cmdq, struct DSI_TXRX_CTRL_REG, DSI_REG[i]->DSI_TXRX_CTRL, TE_TIMEOUT_CHK_EN,
				      1);
		}
	}
	return DSI_STATUS_OK;
	...
	for (d = DSI_MODULE_BEGIN(module); d <= DSI_MODULE_END(module); d++) {
		...
		do {
			...
			if (read_offset) {
				...
				DSI_OUTREG32(cmdq, &DSI_CMDQ_REG[d]->data[1], AS_UINT32(&t1));
				DSI_OUTREG32(cmdq, &DSI_CMDQ_REG[d]->data[2], AS_UINT32(&t0));
	...
	if (MIPITX_IsEnabled(module, cmdq)) {
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
			...
			if (i == 0) { /* only enable DSI0's INT */
				DSI_OUTREGBIT(NULL, struct DSI_INT_ENABLE_REG, DSI_REG[i]->DSI_INTEN, TE_TIMEOUT, 1);
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (dsi_config->mode != CMD_MODE) {
			...
			if (i == 0) { /* only enable DSI0's INT */
				...
				DSI_OUTREGBIT(cmdq, struct DSI_INT_ENABLE_REG, DSI_REG[i]->DSI_INTEN, TE_TIMEOUT, 1);
				DSI_OUTREGBIT(cmdq, struct DSI_TXRX_CTRL_REG, DSI_REG[i]->DSI_TXRX_CTRL,
						TE_TIMEOUT_CHK_EN, 1);
			}
		}
...
int dsi_te_irq_enable(enum DISP_MODULE_ENUM module,
		      void *handle, unsigned int enable)
{
	if (module == DISP_MODULE_DSI0)
...
int ddp_dsi_switch_mode(enum DISP_MODULE_ENUM module, void *cmdq_handle, void *params)
{
	...
	if (mode == 0) {	/* V2C */
		...
		DSI_OUTREGBIT(cmdq_handle, struct DSI_TXRX_CTRL_REG, DSI_REG[i]->DSI_TXRX_CTRL, TE_TIMEOUT_CHK_EN, 1);
		...
		DSI_OUTREGBIT(cmdq_handle, struct DSI_TXRX_CTRL_REG, DSI_REG[i]->DSI_TXRX_CTRL, TE_TIMEOUT_CHK_EN, 0);
		DSI_OUTREGBIT(cmdq_handle, struct MIPITX_DSI_PLL_CON1_REG, DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON1,
								  RG_DSI0_MPPLL_SDM_SSC_EN, 0);
...
int ddp_dsi_power_off(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (s_isDsiPowerOn) {
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
			...
			DSI_OUTREGBIT(NULL, struct DSI_TXRX_CTRL_REG, DSI_REG[i]->DSI_TXRX_CTRL, TE_TIMEOUT_CHK_EN,
				      0);
		}
		DSI_BackupRegisters(module, NULL);
#ifdef ENABLE_CLK_MGR
		...
		/* disable irqs before disable clock */
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
...
void dsi_analysis(enum DISP_MODULE_ENUM module)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DDPDUMP("DSI%d Lane Num:%d, Ext_TE_EN:%d, Ext_TE_Edge:%d, HSTX_CKLP_EN:%d, TE_TIMEOUT_CHK_EN:%d\n", i,
			...
			DSI_REG[i]->DSI_TXRX_CTRL.TE_TIMEOUT_CHK_EN);
...
void DSI_disableTE_irq(enum DISP_MODULE_ENUM module)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		DSI_OUTREGBIT(NULL, struct DSI_TXRX_CTRL_REG, DSI_REG[i]->DSI_TXRX_CTRL,
				TE_TIMEOUT_CHK_EN, 0);
		DSI_OUTREGBIT(NULL, struct DSI_DEBUG_SEL_REG, DSI_REG[i]->DSI_DEBUG_SEL,
				DYNAMIC_CG_EN, 0x0000);
	}
}

/*
 * disable check TE signal IRQ
 *
*/
