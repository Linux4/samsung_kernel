...
/*Add status chek for cmd_q enable / disable*/
						...
						unsigned char force_update)
{
	...
	DISPFUNC();
	/* SEC: DISPMSG("===disp_dsi_set_withcmdq===\n"); */
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
	...
	unsigned int *pdata, unsigned int queue_size, unsigned char force_update)
{
	...
	DISPFUNC();
	/* SEC: DISPMSG("===disp_dsi_set_withrawcmdq===\n"); */
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
...
int disp_lcm_set_display_on(struct disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->set_display_on) {
			...
			/* SEC:	DISPERR("Fail lcm_drv->set_display_on is NULL\n"); */
			return -1;
		}
	} else {
		DISPERR("lcm_drv is NULL\n");
