...
#if 0 /* defined but not used */
static int compat_put_disp_ccorr_config(struct disp_ccorr_config __user *data32,
				struct compat_disp_ccorr_config __user *data)
{
	compat_int_t i;
	bool l;
	int err = 0;
	...
	err |= get_user(l, &(data->is_dirty));
	err |= put_user(l, &(data32->is_dirty));

	err |= get_user(i, &(data->mode));
	err |= put_user(i, &(data32->mode));

	for (j = 0; j < ARRAY_SIZE(data->color_matrix); j++) {
		err |= get_user(i, &(data->color_matrix[j]));
		err |= put_user(i, &(data32->color_matrix[j]));
	}
	return err;
}
						...
						struct disp_session_input_config __user *data)
{
	...
	err |= get_user(u, &(data->config_layer_num));
	err |= put_user(u, &(data32->config_layer_num));

	for (j = 0; j < ARRAY_SIZE(data->config); j++)
		err |= compat_put_disp_input_config(&data32->config[j], &data->config[j]);

	err |= compat_put_disp_ccorr_config(&data32->ccorr_config, &data->ccorr_config);
...
int _compat_ioctl_get_lcm_index(struct file *file, unsigned long arg)
{
	int ret = 0;
