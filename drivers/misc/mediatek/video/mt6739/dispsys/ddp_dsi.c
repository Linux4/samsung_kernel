...
// Use polling instead of IRQ method
	...
	do {
		...
		t0.Data_ID =(cmd < 0xB0 || cmd == 0xDA || cmd == 0xDB || cmd == 0xDC) ? DSI_DCS_READ_PACKET_ID : DSI_GERNERIC_READ_LONG_PACKET_ID;
...
//Use polling instead of IRQ method
        ...
        DISP_REG_CMDQ_POLLING_ONLY_RX(cmdq, &DSI_REG[d]->DSI_INTSTA, 0x00000001, 0x1);
        DSI_OUTREGBIT(cmdq,struct DSI_INT_STATUS_REG, DSI_REG[d]->DSI_INTSTA,RD_RDY, 0x00000000);
#else
		/*
...
//Use polling instead of IRQ method
#if defined(CONFIG_SMCDSD_PANEL)
		DISP_REG_CMDQ_POLLING_ONLY_RX(cmdq, &DSI_REG[d]->DSI_INTSTA, 0x80000000, 0);
#else
			  ...
			  struct LCM_DRIVER *lcm_drv)
{
	...
	utils->set_aod = primary_display_set_aod;
#endif
