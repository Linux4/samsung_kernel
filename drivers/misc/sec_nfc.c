...
static irqreturn_t sec_nfc_irq_thread_fn(int irq, void *dev_id)
{
	...
	pr_err("\n %s: [NFC] Read Interrupt is occurred!\n", __func__);
							...
							unsigned long arg)
{
	...
	dev_dbg(info->dev, "%s: info: %p, cmd: 0x%x\n",
			__func__, info, cmd);
	pr_err("\n %s: info: %p, cmd: 0x%x, mode: %d\n", __func__, info, cmd, info->mode);
	...
	case SEC_NFC_SLEEP:
		...
		if (info->mode != SEC_NFC_MODE_BOOTLOADER) {
			...
			pr_err("\n %s: [Start] gpio_set_value (SEC_NFC_WAKE_SLEEP) !!!\n", __func__);
			gpio_set_value(pdata->wake, SEC_NFC_WAKE_SLEEP);
			pr_err("\n %s: [End] gpio_set_value (SEC_NFC_WAKE_SLEEP) !!!\n", __func__);
		}
		break;
	...
	case SEC_NFC_WAKEUP:
		pr_err("\n %s: SEC_NFC_WAKEUP\n", __func__);
		if (info->mode != SEC_NFC_MODE_BOOTLOADER) {
			pr_err("\n %s: [Start] gpio_set_value (SEC_NFC_WAKE_UP) !!!\n", __func__);
			gpio_set_value(pdata->wake, SEC_NFC_WAKE_UP);
			pr_err("\n %s: [End] gpio_set_value (SEC_NFC_WAKE_UP) !!!\n", __func__);
