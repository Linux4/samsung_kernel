...
static int rt8973_get_cable_type_by_id(rt8973_chip_t *chip)
{
	...
	if ( (cable_type == MUIC_RT8973_CABLE_TYPE_TYPE1_CHARGER )
		...
		ret = rt8973_reg_read(chip, 0x24); //  REG_CHG_TYPE( 0x24[bit3:0] ) : CHG_TYPE
		if (ret >= 0) {
			switch(ret){
				case 0x01 : // DCP
					cable_type = MUIC_RT8973_CABLE_TYPE_REGULAR_TA;
...
/* for printint out registers -- end*/
	...
	if ( (chip->curr_status.id_adc == 0x17)
		...
		rt8973_clr_bits(chip, RT8973_REG_CONTROL, 0x40);
		msleep_interruptible(1);
		rt8973_set_bits(chip, RT8973_REG_CONTROL, 0x40);
		return;
	}
