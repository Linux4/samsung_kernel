...
#if defined(CONFIG_SEC_O7_PROJECT) || defined(CONFIG_SEC_XCOVER3_PROJECT) ||defined(CONFIG_SEC_J7_PROJECT) || defined(CONFIG_SEC_ON7N_PROJECT)
...
static irqreturn_t sm5703_muic_irq_thread(int irq, void *data)
{
	...
	/* interrupt detach */
		...
		if ((((adc != ADC_OPEN) || (val1 != 0x00 || val2 != 0x00 || val3 != 0x00)) &&
					(adc != ADC_OTG)) && (get_usb_mode(n) != NOTIFY_TEST_MODE)) {
#else
		if (((adc != ADC_OPEN) || (val1 != 0x00 || val2 != 0x00 || val3 != 0x00))
					&& (get_usb_mode(n) != NOTIFY_TEST_MODE)) {
#endif
			if (val2 != 0x10)
				sm5703_muic_attach_dev(usbsw);
		} else {
		...
		if (get_usb_mode(n) != NOTIFY_TEST_MODE) {
