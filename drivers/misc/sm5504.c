...
static void muic_rustproof_feature(struct i2c_client *client, int state)
{
	...
	if(state) {
		...
		/* set jig on low and vbus path close in Manual sw2 */
		val = i2c_smbus_write_byte_data(client, REG_MANUAL_SW2, 0x05);
		if (val < 0)
		...
		val = val | 0x04; /*Automatic Connection S/W enable*/
                ...
                if (val < 0)
                        dev_info(&client->dev, "%s: CTRL REG,err %d\n", __func__,val);
		/* set jig on high and vbus path close in Manual sw2 */
