...
static void determine_jack_type(struct sec_jack_info *hi)
{
	...
	while (gpio_get_value(pdata->det_gpio) ^ npolarity) {
		...
		/* store adc value for factory test */
		ear_adc_value = adc;
...
static DEVICE_ATTR(mic_adc, 0444 , mic_adc_show, 
	NULL);
...
static void destroy_class_device_file(void)
{
	...
	device_remove_file(jack_dev, &dev_attr_mic_adc);
#if defined (CONFIG_EARJACK_ADC_SYSFS)
