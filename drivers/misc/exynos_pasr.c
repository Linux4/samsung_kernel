...
static int pasr_online_memory_segment(struct pasr_dev *pasr, int idx)
{
...
static int pasr_offline_memory_segment(struct pasr_dev *pasr, int idx)
{
	struct pasr_segment *info = pasr->segment_info + idx;
	struct memory_block *mem;
	int ret;
...
int pasr_get_segment_info(struct pasr_dev *pasr)
{
	...
	if (movable->present_pages == 0) {
		dev_err(pasr->dev, "No present page in movable zone\n");
		return -ENODEV;
	}
...
static int pasr_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		if (ret == -ENODEV) {
			dev_info(pasr->dev, "PASR is inactivated...\n");
			goto done;
		}
