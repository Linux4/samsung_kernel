...
#define QFPROM_CORR_JTAG_ID_MASK_FOR_FEATURE_ID	  0x07F80000 /*  27:20 bit is for Feature ID */
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!feature_id_addr) {
		pr_err("could not map JTAG_ID address\n");
		return scnprintf(buf, PAGE_SIZE, "could not map JTAG_ID address\n");
	}
	...
	feature_id = ((feature_id & QFPROM_CORR_JTAG_ID_MASK_FOR_FEATURE_ID) >> 20) - 7;
