...
*Author:shibinbin
*Date:2021/10/14
*Purpose:add sdcard inserting notifier callback
*/
static int isEuPcba(char *options)
{
    if (strcmp(options, "EU_PCBA3")) {
        is_EU = true;
    } else {
        is_EU = false;
    }
...
__setup("androidboot.pcbainfo=", isEuPcba);
...
static void manual_offset_calibration()
{
    ...
    pr_err("manual_offset_calibration is done.\n");
}
...
/* HS03  code for SR-SL6215-01-729 by shibinbin at 2021/09/03 start */
...
/* HS03  code for SR-SL6215-01-729 by shibinbin at 2021/09/03 end */
...
static int aw96103_sar_calibration_notifier_callback(struct notifier_block *nb,unsigned long val, void *v)
{
    ...
    if (psy_usb && psy_battery) {
        if (!strcmp(psy_usb->desc->name, "usb") || !strcmp(psy_battery->desc->name, "battery")) {
            ret = power_supply_get_property(psy_usb, POWER_SUPPLY_PROP_ONLINE, &usb_prop);
            ret = power_supply_get_property(psy_battery, POWER_SUPPLY_PROP_ONLINE, &battery_prop);
            if ((usb_prop.intval == 1 || battery_prop.intval != 1) && !haved_calibrator) {
                if (sar_work != NULL) {
                    queue_work(sar_work,&sx_sar_enable_work);
                }
                haved_calibrator = true;
            } else if ((usb_prop.intval == 0) && (battery_prop.intval == 1)) {
                haved_calibrator = false;
            }
        }
    } else {
        AWLOGE(cali_aw9610x->dev, "psy_usb or psy_battery is null\n");
    }
    return 0;
}
