...
static int sx9310_regulator_onoff(struct sx9310_p *data,int onoff)
{
	int ret=0;
	
	ret = gpio_direction_output(data->gripen_gpio,onoff);
	if (ret < 0) {
		SENSOR_ERR("failed to set gpio %d(%d)\n",
			data->gripen_gpio, ret);
		gpio_free(data->gripen_gpio);
		return ret;
	}
	return ret;
}
...
static void sx9310_set_enable(struct sx9310_p *data, int enable)
{
	...
	if (enable == ON) {
		...
		reg_status=sx9310_regulator_onoff(data,enable);
		if(reg_status!=0)
		{
			SENSOR_ERR(" Regulator ON failed \n");
		}
		...
		if(reg_status!=0)
		{
			SENSOR_ERR(" Regulator OFF failed \n");
		}
...
static int sx9310_setup_pin(struct sx9310_p *data)
{
	...
	ret = gpio_request(data->gripen_gpio, "SX9310_nIRQ");
	if (ret < 0) {
		SENSOR_ERR("gpio %d request failed (%d)\n",
			data->gripen_gpio, ret);
		return ret;
	...
	ret = gpio_direction_output(data->gripen_gpio,0);
	if (ret < 0) {
		SENSOR_ERR("failed to set gpio %d(%d)\n",
			data->gripen_gpio, ret);
		gpio_free(data->gripen_gpio);
...
static int sx9310_parse_dt(struct sx9310_p *data, struct device *dev)
{
	...
	data->gripen_gpio = of_get_named_gpio_flags(node,
		"sx9310-i2c,gripen_gpio", 0, &flags);
	if (data->gripen_gpio < 0) {
		SENSOR_ERR("get gripen_gpio error\n");
		return -ENODEV;
	}
