...
static void sx9310_debug_work_func(struct work_struct *work)
{
	...
	strcpy(data->hall_ic2, "OPEN");ret=1;
	if (ret < 0) {
...
static int sx9310_parse_dt(struct sx9310_p *data, struct device *dev)
{
...
#ifdef CONFIG_SENSORS_SX9310_DEFENCE_CODE_FOR_TA_NOISE
	ret = of_property_read_u32(node, "sx9310-i2c,cable_norm_th", &temp_val);
	if (!ret) {
		data->cable_norm_th = (u8)temp_val;
		SENSOR_INFO("[SX9310]: Cable Normal Touch Threshold : %u\n", data->cable_norm_th);
	}
	else {
		data->cable_norm_th = SX9310_NORMAL_TOUCH_CABLE_THRESHOLD;
	...
	data->sensor_2p8_en = of_get_named_gpio_flags(node,
		"sx9310-i2c,2p8_en-gpio", 0, &flags);
	...
	data->sensor_1p8_en = of_get_named_gpio_flags(node,
		"sx9310-i2c,1p8_en-gpio", 0, &flags);
...
static int sx9310_muic_notifier(struct notifier_block *nb,
				unsigned long action, void *v)
{
	struct sx9310_p *data = container_of(nb, struct sx9310_p, muic_nb);
	bool status = false;

	switch (action) {
	case MUIC_ATTACH_NOTI:
		SENSOR_INFO("[SX9310]: TA/USB/OTG is inserted\n");
		status = true;
#ifdef CONFIG_SENSORS_SX9310_DEFENCE_CODE_FOR_TA_NOISE
	...
	case MUIC_DETACH_NOTI:
		SENSOR_INFO("[SX9310]: TA/USB/OTG is removed\n");
		status = true;
		...
		const struct i2c_device_id *id)
{
	...
	data->muic_nb.notifier_call = sx9310_muic_notifier;
