		...
		unsigned char reg_addr, unsigned char *buf)
{
	...
	if (dummy < 0) {
		pr_err("[SENSOR] %s - i2c bus read error %d reg_addr = %d\n",
			__func__, dummy, (int)reg_addr);
		return -EIO;
	}
...
static int bma255_open_calibration(struct bma255_p *data)
{
	...
	pr_info("[SENSOR] %s open accel calibration %d, %d, %d\n", __func__,
...
static int bma255_setup_pin(struct bma255_p *data)
{
	...
	if (data->acc_int2) {
		...
		if (ret < 0) {
			pr_err("[SENSOR] %s - failed to set gpio %d ret=(%d)\n",
				__func__, data->acc_int2, ret);
			goto exit_acc_int2;
		}
	} else
		pr_err("[SENSOR] %s - data->acc_int2 is null\n", __func__);
		...
		const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		pr_err("[SENSOR] %s - chip id read i2c error %d\n",
			__func__, ret);
		goto exit_read_chipid;
	} else if ((ret & 0x00ff) == BMA255_CHIP_ID) {
		pr_info("[SENSOR] %s - chip id success 0x%x\n",
			__func__, (unsigned int)ret & 0x00ff);
	} else {
		pr_info("[SENSOR] %s i2c success but chip id read error 0x%x\n",
			__func__, (unsigned int)ret & 0x00ff);
		goto exit_read_chipid;
	}
...
static int bma255_remove(struct i2c_client *client)
{
	struct bma255_p *data = (struct bma255_p *)i2c_get_clientdata(client);
