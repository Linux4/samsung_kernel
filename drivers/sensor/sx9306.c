...
#ifdef CONFIG_SENSORS_SX9306_USE_2ND_CH
#define CH2_SENSOR               3	/* 2nd channel : CS3 */
#define CH_COUNT                 2
...
#if defined(CONFIG_SENSORS_SX9306_USE_2ND_CH)
	if (state == ACTIVE) {
		input_report_rel(data->input, REL_MISC,
			(data->ch1_state ? 1 : 2));
		input_report_rel(data->input, REL_WHEEL,
			(data->ch2_state ? 1 : 2));
	} else {
		input_report_rel(data->input, REL_MISC, 2);
		input_report_rel(data->input, REL_WHEEL, 2);
	}
#else
	if (state == ACTIVE)
...
#ifdef CONFIG_SENSORS_SX9306_USE_2ND_CH
	sx9306_i2c_write(data, SX9306_REGSENSORSELECT, CH2_SENSOR);
	for (reg = SX9306_REGUSEMSB; reg <= SX9306_REGOFFSETLSB; reg++) {
		sx9306_i2c_read(data, reg, &val);
		SENSOR_INFO("[2ndCH] Register(0x%2x) data(0x%2x)\n", reg, val);
	}
#endif
}
...
static void sx9306_get_data(struct sx9306_p *data)
{
	...
	ms_byte = (u8)(data->offset_ch2 >> 6);
	ls_byte = (u8)(data->offset_ch2 - (((u16)ms_byte) << 6));

	data->capmain_ch2 = 2 * (((s32)ms_byte * 3600) + ((s32)ls_byte * 225)) +
		(((s32)data->useful_ch2 * 50000) / (gain * 65536));
...
#ifdef CONFIG_SENSORS_SX9306_USE_2ND_CH
	SENSOR_INFO("[2ndCH] cm=%d,uf=%d,av=%d,df=%d,Of=%u (%d)\n",
		data->capmain_ch2, data->useful_ch2, data->avg_ch2,
		data->diff_ch2, data->offset_ch2, data->state);
#endif
}
...
#ifdef CONFIG_SENSORS_SX9306_USE_2ND_CH
static void sx9306_ch_interrupt_read(struct sx9306_p *data, u8 status)
{
	if (status & (CSX_STATUS_REG << MAIN_SENSOR))
		data->ch1_state = ACTIVE;
	else
		data->ch1_state = IDLE;

	if (status & (CSX_STATUS_REG << CH2_SENSOR))
		data->ch2_state = ACTIVE;
	else
		data->ch2_state = IDLE;
...
#ifdef CONFIG_SENSORS_SX9306_USE_2ND_CH
		sx9306_ch_interrupt_read(data, status);
...
#ifdef CONFIG_SENSORS_SX9306_USE_2ND_CH
static ssize_t sx9306_raw_data_ch2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	static int sum;
	struct sx9306_p *data = dev_get_drvdata(dev);
	...
	return snprintf(buf, PAGE_SIZE, "%d,%d,%u,%d,%d\n",
		data->capmain_ch2, data->useful_ch2, data->offset_ch2,
		data->diff_ch2, data->avg_ch2);
}

static ssize_t sx9306_diff_avg_ch2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static ssize_t sx9306_ch_count_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", CH_COUNT);
}

static ssize_t sx9306_ch_state_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int ret;
	struct sx9306_p *data = dev_get_drvdata(dev);
...
static DEVICE_ATTR(freq, S_IRUGO | S_IWUSR | S_IWGRP,
			...
			sx9306_freq_store);
#ifdef CONFIG_SENSORS_SX9306_USE_2ND_CH
static DEVICE_ATTR(raw_data_ch2, S_IRUGO, sx9306_raw_data_ch2_show, NULL);
static DEVICE_ATTR(diff_avg_ch2, S_IRUGO, sx9306_diff_avg_ch2_show, NULL);
static DEVICE_ATTR(normal_threshold_ch2, S_IRUGO,
			sx9306_normal_threshold_show, NULL);
static DEVICE_ATTR(gain_ch2, S_IRUGO, sx9306_gain_show, NULL);
static DEVICE_ATTR(ch_count, S_IRUGO, sx9306_ch_count_show, NULL);
static DEVICE_ATTR(ch_state, S_IRUGO, sx9306_ch_state_show, NULL);
#endif
...
static void sx9306_touch_process(struct sx9306_p *data, u8 flag)
{
	...
	if (data->state == IDLE) {
		if ((status & (CSX_STATUS_REG << MAIN_SENSOR))
				|| (status & (CSX_STATUS_REG << CH2_SENSOR)))
			send_event(data, ACTIVE);
		else
...
static int sx9306_parse_dt(struct sx9306_p *data, struct device *dev)
{
	...
	data->sensor_2p8_en = of_get_named_gpio_flags(node,
		"sx9306-i2c,2p8_en-gpio", 0, &flags);
	...
	data->sensor_1p8_en = of_get_named_gpio_flags(node,
		"sx9306-i2c,1p8_en-gpio", 0, &flags);
	...
	ret = of_property_read_u32(node, "sx9306-i2c,normal_th",
					&temp_val);
	if (!ret) {
			...
			const struct i2c_device_id *id)
{
	...
	data->muic_nb.notifier_call = sx9306_muic_notifier;
