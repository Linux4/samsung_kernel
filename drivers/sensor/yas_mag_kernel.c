...
irqreturn_t yas_iio_pollfunc_store_time(int irq, void *p)
{
...
static int yas_device_read(int32_t type, uint8_t addr, uint8_t *buf, int len)
{
	...
	if (err != 2) {
		dev_err(&this_client->dev,
			"i2c read err: slave_addr=%02x, reg_addr=%02x, err=%d\n",
...
static int yas_probe_trigger(struct iio_dev *indio_dev)
{
	int ret;
	struct yas_state *st = iio_priv(indio_dev);
	indio_dev->pollfunc = iio_alloc_pollfunc(&yas_iio_pollfunc_store_time,
	...
	const char *buf, size_t count)
{
	...
	pr_info("[SENSOR] %s, %d\n", __func__, st->sampling_frequency);
...
#if YAS_MAG_DRIVER == YAS_MAG_DRIVER_YAS539
	return snprintf(buf, PAGE_SIZE, "%d %d %d %d %d %d %d %d %d\n", ret,
		r.id, r.dir, r.sxy1y2[0], r.sxy1y2[1], r.sxy1y2[2],
	...
	struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d %d %d %d %d %d\n", thresh[0],
		thresh[1], thresh[2], thresh[3], thresh[4], thresh[5]);
}
#endif
...
static int yas_parse_dt(struct yas_state *st, struct device *dev)
{
	int ret = 0;
	...
	ret = of_property_read_u32(dev->of_node, "yas,chip_pos",
		&st->chip_pos);
	if (ret < 0)
		pr_err("[SENSOR] %s: failed to get chip_pos\n", __func__);

	pr_info("[SENSOR] %s: chip_pos = %d\n", __func__, st->chip_pos);
