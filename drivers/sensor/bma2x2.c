...
#ifdef CONFIG_SENSORS_J3X3G
	/* Read dummy data for i2c SDA signal drop after booting, only Supply the J3X3G model */
	for (i = 0; i < 3; i++) {
		comres = i2c_smbus_read_byte_data(client, 0x00);
	}

	if (comres < 0)
		SENSOR_ERR("dummy i2c read fail %d\n", comres);
#endif
