...
#ifdef CONFIG_SENSORS_BMA2x2_ACC_ALERT_INT
...
#if defined(CONFIG_MACH_JAVA_SS_BAFFINLITE)
			data = 0x10;    // 3.91mg ~ 998mg (2g-range) 0x10*3.91=62.56mg
#else
	...
	if (NULL != client->dev.platform_data) {
		...
		if (NULL != data->bst_pd) {
			memcpy(data->bst_pd, client->dev.platform_data,
					sizeof(*data->bst_pd));
			printk(KERN_INFO "bma2x2 place of bma in %s: %d..%d",
					data->bst_pd->name,
					data->bst_pd->place,data->bst_pd->irq);
		}
	}
#ifdef CONFIG_SENSORS_BMA2x2_ACC_ALERT_INT
