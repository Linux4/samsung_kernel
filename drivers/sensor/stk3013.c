...
static int proximity_leden_gpio_onoff(struct stk3013_data *data, bool onoff);
#endif
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (new_value) {
		...
		if ((ps_data->ps_enabled ? 1 : 0) == OFF) {
			...
			mutex_unlock(&ps_data->io_lock);
			#if defined(CONFIG_SENSORS_LEDA_EN_GPIO)
				proximity_leden_gpio_onoff(ps_data, OFF);
			#endif	
		}
	}
...
static int proximity_leden_gpio_onoff(struct stk3013_data *data, bool onoff)
{
	struct stk3013_platform_data *pdata = data->pdata;
...
static int stk3013_regulator_onoff(struct device *dev, bool onoff)
{
	...
	if (onoff) {
		...
		/*gpio_set_value(ps_data->pdata->leden_gpio, onoff);
		if (ret)
			pr_err("%s: Failed to enable vio.\n", __func__);
		msleep(20);*/
#if defined(CONFIG_SENSORS_LEDA_EN_GPIO)		
		proximity_leden_gpio_onoff(ps_data,onoff);
#endif		
	} else {
			...
			struct stk3013_platform_data *pdata)
{
	...
	pdata->int_pin = of_get_named_gpio_flags(np, "stk,irq-gpio", 0, &flags);
	if (pdata->int_pin < 0) {
...
#if defined(CONFIG_SENSORS_LEDA_EN_GPIO)
	pdata->leden_gpio = of_get_named_gpio_flags(np, "stk,leden_gpio", 0, &flags);
...
#if defined(CONFIG_SENSORS_LEDA_EN_GPIO)
static int setup_leden_gpio(struct stk3013_data *data)
{
	int ret;
	struct stk3013_platform_data *pdata = data->pdata;
