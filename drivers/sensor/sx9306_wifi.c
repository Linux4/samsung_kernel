...
#define DEFAULT_NORMAL_TH        15
#define MUIC_NORMAL_TH           21

#define MAIN_SENSOR              3	/* channel : CS3 */
...
static void sx9306_wifi_set_enable(struct sx9306_p *data, u8 enable)
{
	...
	if (enable == ON) {
		...
		/* enabling IRQ */
		sx9306_wifi_i2c_write(data, SX9306_IRQ_ENABLE_REG,
						setup_reg[0].val);
...
static int sx9306_wifi_parse_dt(struct sx9306_p *data, struct device *dev)
{
...
#if defined(CONFIG_SENSORS_HAS_GPIO_LDO)
	data->sensor_2p8_en = of_get_named_gpio_flags(node,
		"sx9306_wifi-i2c,2p8_en-gpio", 0, &flags);
	if (data->sensor_2p8_en < 0) {
		SENSOR_ERR("Failed to get en_gpio\n");
		data->sensor_2p8_en = -1;
	}
	data->sensor_1p8_en = of_get_named_gpio_flags(node,
		"sx9306_wifi-i2c,1p8_en-gpio", 0, &flags);
	if (data->sensor_1p8_en < 0) {
		SENSOR_ERR("Failed to get en_gpio\n");
		data->sensor_1p8_en = -1;
	}
#endif
	...
	ret = of_property_read_u32(node, "sx9306_wifi-i2c,normal_th",
					&temp_val);
	if (!ret) {
...
{
	struct sx9306_p *data = container_of(nb, struct sx9306_p, muic_nb);
	bool status = false;

	switch (action) {
	case MUIC_ATTACH_NOTI:
		SENSOR_INFO("TA/USB/OTG is inserted\n");
		status = true;
		data->normal_th = MUIC_NORMAL_TH;
		break;
	case MUIC_DETACH_NOTI:
		SENSOR_INFO("TA/USB/OTG is removed\n");
		status = true;
	...
	if (data->init_done == ON && status) {
		SENSOR_INFO("offset compensation\n");
		sx9306_wifi_set_offset_calibration(data);
	}
			...
			const struct i2c_device_id *id)
{
...
#if defined(CONFIG_SENSORS_HAS_GPIO_LDO)
	ret = gpio_request(data->sensor_2p8_en, "sensor_2p8_en");
	if (ret < 0) {
		SENSOR_ERR("gpio %d request failed (%d). may have no gpio\n",
			data->sensor_2p8_en, ret);
	} else {
		ret = gpio_direction_output(data->sensor_2p8_en, ON);
		if (ret)
			SENSOR_ERR("unable to set_direction for [%d]\n",
				data->sensor_2p8_en);

		gpio_set_value_cansleep(data->sensor_2p8_en, ON);
		msleep(20);
	}

	ret = gpio_request(data->sensor_1p8_en, "sensor_1p8_en");
	if (ret < 0) {
		SENSOR_ERR("gpio %d request failed (%d). may have no gpio\n",
			data->sensor_1p8_en, ret);
	} else {
		ret = gpio_direction_output(data->sensor_1p8_en, ON);
		if (ret)
			SENSOR_ERR("unable to set_direction for [%d]\n",
				data->sensor_1p8_en);

		gpio_set_value_cansleep(data->sensor_1p8_en, ON);
		msleep(20);
	}
#endif
	...
	data->muic_nb.notifier_call = sx9306_wifi_muic_notifier;
	register_muic_notifier(&data->muic_nb);
...
exit_chip_reset:
#if defined(CONFIG_SENSORS_HAS_GPIO_LDO)
	if (data->sensor_2p8_en) {
		gpio_set_value_cansleep(data->sensor_2p8_en, OFF);
		gpio_free(data->sensor_2p8_en);
	}
	if (data->sensor_1p8_en) {
		gpio_set_value_cansleep(data->sensor_1p8_en, OFF);
		gpio_free(data->sensor_1p8_en);
	}
#endif
exit_setup_pin:
exit_of_node:
