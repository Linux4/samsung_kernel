...
#if defined(CONFIG_SENSORS_HAS_GPIO_LDO)
	data->sensor_en = of_get_named_gpio_flags(node,
		"sx9306_wifi-i2c,en-gpio", 0, &flags);
	if (data->sensor_en < 0) {
...
void sx9306_wifi_regulator_on(struct sx9306_p *data, bool onoff)
{
	...
	reg_vdd = devm_regulator_get(&data->client->dev, "grip_vdd");
	if (IS_ERR(reg_vdd)) {
		SENSOR_ERR("regulator pointer null vdd\n");
		return;
	}
...
#if defined(CONFIG_SENSORS_SX9306_REGULATOR_ONOFF)
	sx9306_wifi_regulator_on(data, ON);
#endif
#if defined(CONFIG_SENSORS_HAS_GPIO_LDO)
...
#endif
#if defined(CONFIG_SENSORS_SX9306_REGULATOR_ONOFF)
	sx9306_wifi_regulator_on(data, OFF);
#endif
exit_setup_pin:
exit_of_node:
