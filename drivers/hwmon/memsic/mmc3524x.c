...
static int mmc3xxx_i2c_rx_data(char *buf, int len)
{
	...
	if (i >= MMC3524X_RETRY_COUNT) {
		pr_err("%s: retry over %d\n", __FUNCTION__, MMC3524X_RETRY_COUNT);
		return -EIO;
	}
...
//		vec[2] = 65536 - vec[2];
	...
	case MMC3524X_IOC_READXYZ:
		...
		break;
	case MMC3524X_IOC_ID:
		data[0] = MMC3524X_REG_PRODUCTID_0;
		if (mmc3xxx_i2c_rx_data(data, 1) < 0) {
	                mutex_unlock(&ecompass_lock);
			return -EFAULT;
		}
...
static ssize_t mmc3524x_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;

	sprintf(buf, "MMC3524X");
	ret = strlen(buf) + 1;

	return ret;
}

static DEVICE_ATTR(mmc3524x, S_IRUGO, mmc3524x_show, NULL);
...
#if 0
static void mmc3524x_poll(struct input_polled_dev *ipdev)
{
...
#endif
#if 0
static struct input_polled_dev * mmc3524x_input_init(struct i2c_client *client)
{
	...
	ipdev->poll = mmc3524x_poll;
	ipdev->poll_interval = 20;       /* 50Hz */
	ipdev->private = client;

	ipdev->input->name = "mmc3524x";
	ipdev->input->phys = "mmc3524x/input0";
...
static int mmc3524x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	res = device_create_file(&client->dev, &dev_attr_mmc3524x);
	if (res) {
...
/*
	...
	msleep(MMC3524X_DELAY_TM);
*/
//	ipdev = mmc3524x_input_init(client);
	mutex_init(&lock);
