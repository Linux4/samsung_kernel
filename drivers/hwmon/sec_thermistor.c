...
static int sec_therm_parse_dt(struct platform_device *pdev)
{
	...
	of_property_read_u32(info->np, "adc_channel", &channel);
...
static int sec_therm_get_adc_data(struct sec_therm_info *info)
{
	...
	for (i = 0; i < ADC_SAMPLING_CNT; i++) {
		...
		ret = IMM_GetOneChannelValue(adc_ch, data, &adc_data);
		if (ret < 0)
			dev_info(info->dev, "%s: Cannot read ADC channel %d\n", info->name, adc_ch);
