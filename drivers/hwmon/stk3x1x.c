...
static void stk3x1x_proc_plat_data(struct stk3x1x_data *ps_data,
...
static int32_t stk3x1x_init_all_reg(struct stk3x1x_data *ps_data)
{
	...
	ret =
	    stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_STATE_REG,
					      ps_data->state_reg);
	if (ret < 0) {
	...
	ret =
	    stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_PSCTRL_REG,
					      ps_data->psctrl_reg);
	if (ret < 0) {
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
	...
	ret =
	    stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_ALSCTRL_REG,
					      ps_data->alsctrl_reg);
	if (ret < 0) {
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
	...
	ret =
	    stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_LEDCTRL_REG,
					      ps_data->ledctrl_reg);
	if (ret < 0) {
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
	...
	ret =
	    stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_WAIT_REG,
					      ps_data->wait_reg);
	if (ret < 0) {
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
	...
	ret =
	    stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_INT_REG,
					      ps_data->int_reg);
	if (ret < 0) {
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
...
static int32_t stk3x1x_check_pid(struct stk3x1x_data *ps_data)
{
	...
	err =
	    stk3x1x_i2c_read_data(ps_data->client, STK_PDT_ID_REG, 2,
				  &value[0]);
	if (err < 0) {
	...
	if (value[0] == 0) {
		printk(KERN_ERR
		       "PID=0x0, please make sure the chip is stk3x1x!\n");
		return -2;
	}
...
static int32_t stk3x1x_software_reset(struct stk3x1x_data *ps_data)
{
	...
	r = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_SW_RESET_REG,
					      0);
	if (r < 0) {
				     ...
				     uint16_t thd_l)
{
	...
	val[1] = thd_l & 0x00FF;
	ret =
	    stk3x1x_i2c_write_data(ps_data->client, STK_THDL1_ALS_REG, 2, val);
	if (ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
}
				     ...
				     uint16_t thd_h)
{
	...
	val[1] = thd_h & 0x00FF;
	ret =
	    stk3x1x_i2c_write_data(ps_data->client, STK_THDH1_ALS_REG, 2, val);
	if (ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
}
...
static uint32_t stk3x1x_get_ps_reading(struct stk3x1x_data *ps_data)
{
	...
	int err;
	err =
	    stk3x1x_i2c_read_data(ps_data->client, STK_DATA1_PS_REG, 2,
				  &value[0]);
	if (err < 0) {
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, err);
		return err;
				...
				uint8_t org_flag_reg, uint8_t clr)
{
	...
	w_flag =
	    org_flag_reg | (STK_FLG_ALSINT_MASK | STK_FLG_PSINT_MASK |
			    STK_FLG_OUI_MASK | STK_FLG_IR_RDY_MASK);
	w_flag &= (~clr);
	/*printk(KERN_INFO "%s: org_flag_reg=0x%x, w_flag = 0x%x\n",
	__func__, org_flag_reg, w_flag); */
	ret =
	    stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_FLAG_REG,
					      w_flag);
	if (ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
...
#ifdef STK_GES
static int32_t stk3x1x_set_flag2(struct stk3x1x_data *ps_data,
				 uint8_t org_flag2_reg, uint8_t clr)
{
	uint8_t w_flag2;
	int ret;
	...
	ret =
	    stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_FLAG2_REG,
					      w_flag2);
	if (ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
...
static int32_t stk3x1x_set_gsctrl(struct stk3x1x_data *ps_data, uint8_t state)
{
	int ret;
	ret =
	    stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_GSCTRL_REG,
					      state);
	if (ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
...
#if 0
static uint32_t stk3x1x_get_ges_reading(struct stk3x1x_data *ps_data,
					unsigned int *ges0, unsigned int *ges1,
					unsigned int *ges2)
{
	int retry, len, no = 0;
		...
		if (stk_ges_op[no].action.rw_len_retry & 0x80) {
			while (retry != 0) {
				...
				if (stk_ges_op[no].action.sleep_10ns != 0)
					usleep_range(stk_ges_op[no].
						     action.sleep_10ns * 10,
						     stk_ges_op[no].
						     action.sleep_10ns * 10 +
						     300);
				retry--;
			...
			while (retry != 0) {
				err =
				    stk3x1x_i2c_write_data(ps_data->client,
							   stk_ges_op
							   [no].action.reg, len,
							   &reg_val_crit);
				if (err < 0) {
...
static int32_t stk3x1x_enable_ges(struct stk3x1x_data *ps_data, uint8_t enable,
				  uint8_t mode)
{
	int32_t ret;
	...
	if (enable) {
		...
		ret =
		    stk3x1x_i2c_smbus_write_byte_data(ps_data->client,
						      STK_WAIT_REG, 0);
		if (ret < 0) {
			printk(KERN_ERR "%s: write i2c error\n", __func__);
			return ret;
		...
		if (mode == 2) {
			...
			if (ret < 0)
				return ret;
#ifdef STK_POLL_PS
			hrtimer_start(&ps_data->ps_timer,
				      ps_data->ps_poll_delay, HRTIMER_MODE_REL);
#else
				 ...
				 uint8_t validate_reg)
{
	...
	w_state_reg &=
	    ~(STK_STATE_EN_PS_MASK | STK_STATE_EN_WAIT_MASK |
	...
	if (enable) {
		...
		if (!(ps_data->psi_set)) {
#ifdef STK_TUNE0
			hrtimer_start(&ps_data->ps_tune0_timer,
				      ps_data->ps_tune0_delay,
				      HRTIMER_MODE_REL);
#endif
		}
#ifdef STK_POLL_PS
		...
		if (!validate_reg) {
			...
			printk(KERN_INFO
			       "%s: force report ps input event=1, ps code = %d\n",
			       __func__, reading);
		} else
#endif /* #ifdef STK_CHK_REG */
		{
...
static int32_t stk3x1x_enable_als(struct stk3x1x_data *ps_data, uint8_t enable)
{
	...
	w_state_reg =
	    (uint8_t) (ret &
		       (~(STK_STATE_EN_ALS_MASK | STK_STATE_EN_WAIT_MASK)));
	if (enable)
...
#ifdef STK_POLL_ALS
		hrtimer_start(&ps_data->als_timer, ps_data->als_poll_delay,
			      HRTIMER_MODE_REL);
#else
#ifndef STK_POLL_PS
		if (!(ps_data->ps_enabled))
...
static int32_t stk3x1x_get_als_reading(struct stk3x1x_data *ps_data)
{
	...
	ret =
	    stk3x1x_i2c_read_data(ps_data->client, STK_DATA1_ALS_REG, 2,
				  &value[0]);
	if (ret < 0) {
...
static int32_t stk3x1x_get_ir_reading(struct stk3x1x_data *ps_data)
{
	...
	ret =
	    stk3x1x_i2c_read_data(ps_data->client, STK_DATA1_IR_REG, 2,
				  &value[0]);
	if (ret < 0) {
		printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
		goto irs_err_i2c_rw;
...
static int stk3x1x_chk_reg_valid(struct stk3x1x_data *ps_data)
{
	...
	/*
	   ...
	   {
	   value[cnt] =
		stk3x1x_i2c_smbus_read_byte_data(ps_data->client, (cnt+1));
	   if(value[cnt] < 0)
	   {
	 ...
	 */
	err =
	    stk3x1x_i2c_read_data(ps_data->client, STK_PSCTRL_REG, 9,
				  &value[0]);
	if (err < 0) {
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, err);
		return err;
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	printk(KERN_INFO "%s: sum = %5d, ave = %5d\n", __func__,
	       ps_data->fir.sum, ps_data->fir.sum / len);
...
#ifdef STK_GES
static ssize_t stk_ges_code_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);
	int ret;
	...
	return scnprintf(buf, PAGE_SIZE, "%5d,%5d,%5d\n", gest0, gest1,
			 atomic_read(&ps_data->gesture2));
}

static ssize_t stk_ges_code_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t size)
{
	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);
...
static ssize_t stk_ges_poll_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	int len = 0, ii = 0, jj = 0;
	...
	while (stk_ges_op[ii].ops[0] != 0) {
		...
		for (jj = 0; jj < 4; jj++)
			len +=
			    scnprintf(buf + len, PAGE_SIZE - len, "%x ",
				      stk_ges_op[ii].ops[jj]);
...
static ssize_t stk_ges_poll_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t size)
{
	...
	if (i != 5) {
		...
		memset(&(stk_ges_op[index]), 0,
		       sizeof(union stk_ges_operation));
	}
	return size;
}

static ssize_t stk_ges_enable_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t size)
{
	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);
...
static ssize_t stk_ges_enable_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);
	return scnprintf(buf, PAGE_SIZE, "%d\n", ps_data->ges_enabled);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	ret =
	    stk3x1x_i2c_read_data(ps_data->client, STK_DATA1_OFFSET_REG, 2,
				  &value[0]);
	if (ret < 0) {
		printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
		return ret;
				   ...
				   const char *buf, size_t size)
{
	...
	val[1] = offset & 0x00FF;
	ret =
	    stk3x1x_i2c_write_data(ps_data->client, STK_DATA1_OFFSET_REG, 2,
				   val);
	if (ret < 0) {
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);
	ps_thd_l1_reg =
	    stk3x1x_i2c_smbus_read_byte_data(ps_data->client, STK_THDL1_PS_REG);
	if (ps_thd_l1_reg < 0) {
		printk(KERN_ERR "%s fail, err=0x%x", __func__, ps_thd_l1_reg);
		return -EINVAL;
	}
	ps_thd_l2_reg =
	    stk3x1x_i2c_smbus_read_byte_data(ps_data->client, STK_THDL2_PS_REG);
	if (ps_thd_l2_reg < 0) {
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);
	ps_thd_h1_reg =
	    stk3x1x_i2c_smbus_read_byte_data(ps_data->client, STK_THDH1_PS_REG);
	if (ps_thd_h1_reg < 0) {
		printk(KERN_ERR "%s fail, err=0x%x", __func__, ps_thd_h1_reg);
		return -EINVAL;
	}
	ps_thd_h2_reg =
	    stk3x1x_i2c_smbus_read_byte_data(ps_data->client, STK_THDH2_PS_REG);
	if (ps_thd_h2_reg < 0) {
				...
				struct device_attribute *attr, char *buf)
{
	...
	for (cnt = 0; cnt < 0x20; cnt++) {
		ps_reg[cnt] =
		    stk3x1x_i2c_smbus_read_byte_data(ps_data->client, (cnt));
		if (ps_reg[cnt] < 0) {
			printk(KERN_ERR "%s fail, ret=%d", __func__,
			       ps_reg[cnt]);
			return -EINVAL;
		} else {
			printk(KERN_INFO "reg[0x%2X]=0x%2X\n", cnt,
			       ps_reg[cnt]);
			len +=
			    scnprintf(buf + len, PAGE_SIZE - len, "[%2X]%2X,",
				      cnt, ps_reg[cnt]);
		}
	}
	ps_reg[cnt] =
	    stk3x1x_i2c_smbus_read_byte_data(ps_data->client, STK_PDT_ID_REG);
	if (ps_reg[cnt] < 0) {
	...
	cnt++;
	ps_reg[cnt] =
	    stk3x1x_i2c_smbus_read_byte_data(ps_data->client, STK_RSRVD_REG);
	if (ps_reg[cnt] < 0) {
		printk(KERN_ERR "%s fail, ret=%d", __func__, ps_reg[cnt]);
		return -EINVAL;
	}
	...
	len +=
	    scnprintf(buf + len, PAGE_SIZE - len, "[%2X]%2X,[%2X]%2X\n",
...
__ATTR(enable, S_IRUGO | S_IWUSR | S_IWGRP, stk_als_enable_show, stk_als_enable_store);
static struct device_attribute als_lux_attribute =
__ATTR(lux, S_IRUGO | S_IWUSR | S_IWGRP, stk_als_lux_show, stk_als_lux_store);
static struct device_attribute als_code_attribute =
__ATTR(code, S_IRUGO, stk_als_code_show, NULL);
static struct device_attribute als_transmittance_attribute =
__ATTR(transmittance, S_IRUGO | S_IWUSR | S_IWGRP, stk_als_transmittance_show,
		stk_als_transmittance_store);
static struct device_attribute als_poll_delay_attribute =
__ATTR(delay, S_IRUGO | S_IWUSR | S_IWGRP, stk_als_delay_show, stk_als_delay_store);
static struct device_attribute als_ir_code_attribute =
__ATTR(ircode, S_IRUGO, stk_als_ir_code_show, NULL);
#ifdef STK_ALS_FIR
static struct device_attribute als_firlen_attribute =
__ATTR(firlen, S_IRUGO | S_IWUSR | S_IWGRP, stk_als_firlen_show, stk_als_firlen_store);
#endif
...
__ATTR(enable, S_IRUGO | S_IWUSR | S_IWGRP, stk_ges_enable_show, stk_ges_enable_store);
static struct device_attribute ges_code_attribute =
__ATTR(code, S_IRUGO | S_IWUSR | S_IWGRP, stk_ges_code_show, stk_ges_code_store);
static struct device_attribute ges_poll_attribute =
__ATTR(poll, S_IRUGO | S_IWUSR | S_IWGRP, stk_ges_poll_show, stk_ges_poll_store);
static struct device_attribute ges_recv_attribute =
__ATTR(recv, S_IRUGO | S_IWUSR | S_IWGRP, stk_recv_show, stk_recv_store);
static struct device_attribute ges_send_attribute =
__ATTR(send, S_IRUGO | S_IWUSR | S_IWGRP, stk_send_show, stk_send_store);
...
__ATTR(enable, S_IRUGO | S_IWUSR | S_IWGRP, stk_ps_enable_show, stk_ps_enable_store);
static struct device_attribute ps_enable_aso_attribute =
__ATTR(enableaso, S_IRUGO | S_IWUSR | S_IWGRP, stk_ps_enable_aso_show, stk_ps_enable_aso_store);
static struct device_attribute ps_distance_attribute =
__ATTR(distance, S_IRUGO | S_IWUSR | S_IWGRP, stk_ps_distance_show, stk_ps_distance_store);
static struct device_attribute ps_offset_attribute =
__ATTR(offset, S_IRUGO | S_IWUSR | S_IWGRP, stk_ps_offset_show, stk_ps_offset_store);
static struct device_attribute ps_code_attribute =
__ATTR(code, S_IRUGO, stk_ps_code_show, NULL);
static struct device_attribute ps_code_thd_l_attribute =
__ATTR(codethdl, S_IRUGO | S_IWUSR | S_IWGRP, stk_ps_code_thd_l_show, stk_ps_code_thd_l_store);
static struct device_attribute ps_code_thd_h_attribute =
__ATTR(codethdh, S_IRUGO | S_IWUSR | S_IWGRP, stk_ps_code_thd_h_show, stk_ps_code_thd_h_store);
...
__ATTR(allreg, S_IRUGO, stk_all_reg_show, NULL);
static struct device_attribute status_attribute =
__ATTR(status, S_IRUGO, stk_status_show, NULL);
#ifdef STK_TUNE0
static struct device_attribute ps_cali_attribute =
__ATTR(cali, S_IRUGO, stk_ps_cali_show, NULL);
#endif
...
static int stk_ps_tune_zero_init(struct stk3x1x_data *ps_data)
{
	...
	ret =
	    stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_INT_REG, 0);
	if (ret < 0) {
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
	}
	...
	hrtimer_start(&ps_data->ps_tune0_timer, ps_data->ps_tune0_delay,
		      HRTIMER_MODE_REL);
	return 0;
}
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_data *ps_data)
{
	...
	if (ret == 0) {
		ret =
		    stk3x1x_i2c_read_data(ps_data->client, 0x11, 2, &value[0]);
		if (ret < 0) {
			printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
			return ret;
		}
...
static void stk_als_poll_work_func(struct work_struct *work)
{
	...
	if (ps_data->ir_code) {
		...
		if (reading < STK_IRC_MAX_ALS_CODE
			...
			als_comperator =
			    reading * STK_IRC_ALS_NUMERA / STK_IRC_ALS_DENOMI;
...
{
	struct stk3x1x_data *ps_data =
	    container_of(timer, struct stk3x1x_data, ps_timer);
...
{
	struct stk3x1x_data *ps_data =
	    container_of(work, struct stk3x1x_data, stk_ps_work);
	uint32_t reading;
	int32_t near_far_state;
	...
	if (ps_data->ges_enabled == 2) {
		...
		if (disable_flag2) {
			ret =
			    stk3x1x_set_flag2(ps_data, org_flag2_reg,
					      disable_flag2);
			if (ret < 0)
				goto err_i2c_rw;
		}
...
#ifdef STK_DEBUG_PRINTF
			printk(KERN_INFO
			       "%s: ps input event %d cm, ps code = %d\n",
			       __func__, near_far_state, reading);
#endif
		}
...
static void stk_work_func(struct work_struct *work)
{
	...
	if (org_flag_reg & STK_FLG_ALSINT_MASK) {
		...
		stk3x1x_set_als_thd_l(ps_data,
				      code_threshold_table[nluxindex - 1]);
#else
...
static int stk3x1x_setup_irq(struct i2c_client *client)
{
	...
	if (irq <= 0) {
		printk(KERN_ERR
		       "irq number is not specified, irq # = %d, int pin=%d\n",
		       irq, ps_data->int_pin);
		return irq;
	}
	ps_data->irq = irq;
	...
	if (err < 0) {
		printk(KERN_ERR "%s: gpio_direction_input, err=%d", __func__,
		       err);
		return err;
	}
#if ((STK_INT_PS_MODE == 0x03) || (STK_INT_PS_MODE	== 0x02))
	err =
	    request_any_context_irq(irq, stk_oss_irq_handler,
				    IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING,
				    DEVICE_NAME, ps_data);
#else
	err =
	    request_any_context_irq(irq, stk_oss_irq_handler, IRQF_TRIGGER_LOW,
				    DEVICE_NAME, ps_data);
#endif
	if (err < 0) {
		printk(KERN_WARNING
...
static int stk3x1x_suspend(struct device *dev)
{
	...
	if (ps_data->avdd)
		regulator_disable(ps_data->avdd);
#ifndef SPREADTRUM_PLATFORM
	mutex_unlock(&ps_data->io_lock);
#endif
	return 0;
...
static int stk3x1x_resume(struct device *dev)
{
	...
	printk(KERN_INFO "%s", __func__);
	if (ps_data->avdd)
		if (regulator_enable(ps_data->avdd)) {
			printk(KERN_ERR "stk regulator enable failed\n");
			goto resume_out;
		}
#ifndef SPREADTRUM_PLATFORM
	mutex_lock(&ps_data->io_lock);
...
#ifdef STK_TUNE0
	ps_data->stk_ps_tune0_wq =
	    create_singlethread_workqueue("stk_ps_tune0_wq");
	INIT_WORK(&ps_data->stk_ps_tune0_work, stk_ps_tune0_work_func);
	hrtimer_init(&ps_data->ps_tune0_timer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);
...
static int stk3x1x_set_input_devices(struct stk3x1x_data *ps_data)
{
	...
	err =
	    sysfs_create_group(&ps_data->als_input_dev->dev.kobj,
	...
	err =
	    sysfs_create_group(&ps_data->ps_input_dev->dev.kobj,
	...
	err =
	    sysfs_create_group(&ps_data->ges_input_dev->dev.kobj,
			       &stk_ges_attribute_group);
	if (err < 0) {
		printk(KERN_ERR "%s:could not create sysfs group for ps\n",
		       __func__);
		return err;
...
#ifdef CONFIG_OF
static int stk3x1x_probe_dt(struct i2c_client *client)
{
	struct stk3x1x_platform_data *platform_data;
	...
	platform_data->int_pin =
	    of_get_named_gpio(np, "sensortek,irq-gpios", 0);
	if (platform_data->int_pin < 0) {
		dev_err(&client->dev, "of_get_named_gpio irq faild\n");
		return -EINVAL;
	}
	platform_data->state_reg = 0x0;	/* disable all */
	/* ps_persistance=4, ps_gain=64X, PS_IT=0.391ms */
	platform_data->psctrl_reg = 0x71;
	/* als_persistance=1, als_gain=64X, ALS_IT=50ms */
	platform_data->alsctrl_reg = 0x38;
	platform_data->ledctrl_reg = 0xFF;/* 100mA IRDR, 64/64 LED duty */
	platform_data->wait_reg = 0x07;	/* 50 ms */
	platform_data->ps_thd_h = 1700;
	platform_data->ps_thd_l = 1500;
	platform_data->transmittance = 500;
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (IS_ERR(avdd)) {
		dev_err(&client->dev,
			"stk3x1x sensor avdd power supply get failed\n");
		return err;
	}
	regulator_set_voltage(avdd, 2800000, 2800000);
	if (regulator_enable(avdd)) {
		dev_err(&client->dev,
			"stk3x1x sensor regulator enable failed\n");
		goto out;
	}
	...
	ps_data->avdd = avdd;
#ifdef CONFIG_OF
	retval = stk3x1x_probe_dt(client);
	if (retval == -ENOMEM) {
		dev_err(&client->dev,
			"%s: Failed to alloc mem for stk3x1x_platform_data\n",
			__func__);
		goto ps_exit;
	...
	if (client->dev.platform_data != NULL) {
		...
		if (ps_data->als_transmittance == 0) {
			printk(KERN_ERR
			       "%s: Please set als_transmittance in platform data\n",
			       __func__);
...
#ifdef STK_GES
	sysfs_remove_group(&ps_data->ges_input_dev->dev.kobj,
			   &stk_ges_attribute_group);
...
static struct i2c_driver stk_ps_driver = {
	.driver = {
		   ...
		   .pm = &stk3x1x_pm_ops,
#ifdef CONFIG_OF
		   .of_match_table = of_match_ptr(stk3x1x_match_table),
#endif
		   },
	.probe = stk3x1x_probe,
