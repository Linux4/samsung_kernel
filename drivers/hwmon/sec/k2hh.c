...
#include <linux/sensor/k3dh_dev.h>
...
static int k2hh_read_accel_xyz(struct k2hh_p *data, struct k2hh_v *acc)
{
	...
	if (orientation[0]) {
		...
		temp = acc->x*orientation[1];
		acc->x = acc->y*orientation[3];
		acc->y = temp;
	}
	acc->z *= orientation[8];
...
static void k2hh_work_func(struct work_struct *work)
{
	...
	if(!data->negate_x)
		...
		data->accdata.x = data->caldata.x - acc.x;		
	if(!data->negate_y)
		data->accdata.y = acc.y - data->caldata.y;
	else
		data->accdata.y = data->caldata.y - acc.y;		
	if(!data->negate_z)
		data->accdata.z = acc.z - data->caldata.z;
	else
		data->accdata.z = data->caldata.z - acc.z;

	if((!data->swap_xy)&&(!data->swap_yz)&&(!data->swap_xz)){
	...
	}else if((data->swap_xy)&&(!data->swap_yz)&&(!data->swap_xz)){
		input_report_rel(data->input, REL_Y, data->accdata.x);
		input_report_rel(data->input, REL_X, data->accdata.y);
		input_report_rel(data->input, REL_Z, data->accdata.z);
	}else if((!data->swap_xy)&&(data->swap_yz)&&(!data->swap_xz)){
		input_report_rel(data->input, REL_X, data->accdata.x);
		input_report_rel(data->input, REL_Z, data->accdata.y);
		input_report_rel(data->input, REL_Y, data->accdata.z);
	}else if((!data->swap_xy)&&(!data->swap_yz)&&(data->swap_xz)){
		input_report_rel(data->input, REL_Z, data->accdata.x);
		input_report_rel(data->input, REL_Y, data->accdata.y);
		input_report_rel(data->input, REL_X, data->accdata.z);
	}else{
		BUG_ON(1);
	}
...
static int k2hh_parse_dt(struct device *dev, struct k2hh_p *data)
{
	...
	ret = of_property_read_u32(dev_node, "stm,negate_x", &temp);
	...
	ret = of_property_read_u32(dev_node, "stm,negate_y", &temp);
	...
	ret = of_property_read_u32(dev_node, "stm,negate_z", &temp);
	...
	ret = of_property_read_u32(dev_node, "stm,swap_xy", &temp);
	if ((data->swap_xy > 1) || (ret < 0)) {
		pr_err("%s: invalid swap_xy value %u\n",
			__func__, data->swap_xy);
		data->swap_xy = 0;
	} else {
		data->swap_xy = (u8)temp;
	}

	ret = of_property_read_u32(dev_node, "stm,swap_yz", &temp);
	if ((data->swap_yz > 1) || (ret < 0)) {
		pr_err("%s: invalid swap_yz value %u\n",
			__func__, data->swap_yz);
		data->swap_yz = 0;
	} else {
		data->swap_yz = (u8)temp;
	}

	ret = of_property_read_u32(dev_node, "stm,swap_xz", &temp);
	if ((data->swap_xz > 1) || (ret < 0)) {
		pr_err("%s: invalid swap_xz value %u\n",
			__func__, data->swap_xz);
		data->swap_xz = 0;
	} else {
		data->swap_xz = (u8)temp;
	}
		...
		const struct i2c_device_id *id)
{
	...
	k2hh = kzalloc(sizeof(struct k2hh_p), GFP_KERNEL);
	if (k2hh == NULL) {
	...
	i2c_set_clientdata(client, k2hh);
	k2hh->client = client;

	ret = k2hh_parse_dt(&client->dev, k2hh);
	if (ret < 0) {
	...
	ret = k2hh_setup_pin(k2hh);
	if (ret < 0) {
		pr_err("[SENSOR]: %s - could not setup pin\n", __func__);
		goto exit_setup_pin;
	}

	mutex_init(&k2hh->mode_mutex);

	/* read chip id */
	for (i = 0; i < CHIP_ID_RETRIES; i++) {
		ret = k2hh_i2c_read(k2hh, WHOAMI_REG, &temp, 1);
	...
	/* input device init */
	ret = k2hh_input_init(k2hh);
	if (ret < 0)
		goto exit_input_init;

	ret = sensors_create_symlink(&k2hh->input->dev.kobj,
					k2hh->input->name);
	if (ret < 0) {
		pr_err("[SENSOR] %s: create_symlink error\n", __func__);
		goto err_sensors_create_symlink_acc;
	}


	sensors_register(k2hh->factory_device, k2hh, sensor_attrs, MODULE_NAME);

	atomic_set(&k2hh->delay, K2HH_DEFAULT_DELAY);

	/* this is the thread function we run on the work queue */
	INIT_DELAYED_WORK(&k2hh->work, k2hh_work_func);
	INIT_DELAYED_WORK(&k2hh->irq_work, k2hh_irq_work_func);

	atomic_set(&k2hh->enable, OFF);
	k2hh->time_count = 0;
	k2hh->irq_state = 0;
	k2hh->recog_flag = OFF;

	k2hh_set_range(k2hh, K2HH_RANGE_2G);
	k2hh_set_mode(k2hh, K2HH_MODE_SUSPEND);

	pr_info("[SENSOR]: %s - Probe done!\n", __func__);

	return 0;
	
err_sensors_create_symlink_acc:
	input_unregister_device(k2hh->input);
exit_input_init:
exit_read_chipid:
	mutex_destroy(&k2hh->mode_mutex);
	free_irq(k2hh->irq1, k2hh);
	wake_lock_destroy(&k2hh->reactive_wake_lock);
	gpio_free(k2hh->acc_int1);
err_regulator_onoff:
exit_setup_pin:
err_of_node:
	kfree(k2hh);
