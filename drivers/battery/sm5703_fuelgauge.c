...
int sm5703_get_v_max_index_by_cycle(struct i2c_client *client)
{
...
#ifdef ENABLE_BATT_LONG_LIFE
	i = sm5703_get_v_max_index_by_cycle(client);
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		if (fuelgauge->pdata->capacity_calculation_type &
			...
			if (fuelgauge->info.q_max_now !=
				fuelgauge->info.q_max_table[sm5703_get_v_max_index_by_cycle(fuelgauge->client)]) {
