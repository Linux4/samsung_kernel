...
static unsigned int sm5703_get_vbat(struct i2c_client *client)
{
	...
	//cal avgvoltage
	fuelgauge->info.batt_avgvoltage = ((fuelgauge->info.batt_avgvoltage*2)
			+ (fuelgauge->info.p_batt_voltage+vbat))/4;
...
{ 
	...
	if((abs(l_avg - s_avg) > 0x29 || (s_avg == 0)))
	{
		ret = l_avg;
	}
	else
...
void fg_abnormal_reset_check(struct i2c_client *client)
{
	...
	if((ret & INIT_CHECK_MASK) != DISABLE_RE_INIT)
	{
		...
		if(ret == CNTL_REG_DEFAULT_VALUE)
		{
			...
			msleep(200);
			// init code 
			sm5703_fg_reg_init(client, true);
		}
	}
}
...
void fg_vbatocv_check(struct i2c_client *client)
{
	...
	else
	{
		if((fuelgauge->info.p_batt_voltage < 3420) && (fuelgauge->info.batt_voltage < 3420) && (!fuelgauge->is_charging))
		{
			...
			// forced power off in low temperature under -10 degree
			if((fuelgauge->info.batt_voltage < 2700) && (fuelgauge->info.temperature < -100))
			{
				dev_info(&client->dev, "%s: forced power off in low temperature\n", __func__);
