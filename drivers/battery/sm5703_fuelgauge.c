...
static u32 sm5703_get_soc(struct sm5703_fuelgauge_data *fuelgauge)
{
	...
	ta_exist = fuelgauge->is_charging;
	...
	temp_cal_fact = fuelgauge->info.temp_std - fuelgauge->info.temperature;
	...
	dev_dbg(&fuelgauge->i2c->dev, "%s: fg_get_soc : temp_std = %d, temperature = %d, temp_offset = %d, temp_offset_cal = 0x%x, curr_cal = 0x%x\n",
	...
	dev_dbg(&fuelgauge->i2c->dev, "%s: read = 0x%x, soc = %d\n", __func__, ret, soc);
...
static unsigned int sm5703_get_ocv(struct sm5703_fuelgauge_data *fuelgauge)
{
	...
	dev_dbg(&fuelgauge->i2c->dev, "%s: read = 0x%x, ocv = %d\n", __func__, ret, ocv);
		...
		int soc)
{
	...
	// remove all mask
	sm5703_fg_i2c_write_word(fuelgauge->i2c, SM5703_REG_INTFG_MASK,0x0000);

	/* enable volt, soc alert irq; clear volt and soc alert status via i2c */
	ret = ENABLE_L_SOC_INT | ENABLE_L_VOL_INT;
	sm5703_fg_i2c_write_word(fuelgauge->i2c, SM5703_REG_INTFG_MASK, ret);
...
static bool sm5703_fg_reg_init(struct sm5703_fuelgauge_data *fuelgauge)
{
	...
	dev_dbg(&fuelgauge->i2c->dev, "%s: sm5703_fg_reg_init START!!\n", __func__);
	...
	for (i=0; i < 3; i++) {
		...
		for(j=0; j <= SM5703_FG_TABLE_LEN; j++) {
			sm5703_fg_i2c_write_word(fuelgauge->i2c,
			(table_reg + j), fuelgauge->info.battery_table[i][j]);
			dev_dbg(&fuelgauge->i2c->dev,
	...
	value = ENABLE_MIX_MODE | ENABLE_TEMP_MEASURE |
		((fuelgauge->info.enable_topoff_soc << 13) & ENABLE_TOPOFF_SOC);
...
{
	struct device *dev = &fuelgauge->i2c->dev;
