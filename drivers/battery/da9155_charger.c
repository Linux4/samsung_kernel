...
static void da9155_charger_initialize(struct da9155_charger_data *charger)
{
	...
	/* Remove Event Mask set. Using Default value (EventA : 0xFF, EventB: 0xFE */
#if 0
...
static void da9155_prevent_vin_drop(struct da9155_charger_data *charger)
{
...
#if 0
	/* Enable VBAT_OV and debouncing time */
	da9155_write_reg(charger->i2c, DA9155_PAGE_CTRL_0, 0x0);  
	da9155_write_reg(charger->i2c, 0x6A, 0x4A);
	da9155_write_reg(charger->i2c, DA9155_PAGE_CTRL_0, 0x2);
	da9155_write_reg(charger->i2c, 0x56, 0x34);
	da9155_write_reg(charger->i2c, DA9155_PAGE_CTRL_0, 0x0);
	da9155_write_reg(charger->i2c, 0x6A, 0x00);
	da9155_write_reg(charger->i2c, DA9155_PAGE_CTRL_0, 0x06);
	da9155_write_reg(charger->i2c, 0x19, 0x00);
	da9155_write_reg(charger->i2c, 0x39, 0x00);
#endif
	//da9155_write_reg(charger->i2c, DA9155_PAGE_CTRL_0, 0x0);

	da9155_write_reg(charger->i2c, DA9155_CONTROL_A, 0xB);
	//da9155_write_reg(charger->i2c, DA9155_CONTROL_C, 0x28); /* VBAT_OV 4.6V */
}

static void da9155_mode_change(struct da9155_charger_data *charger, int enable)
{
	if(charger->i2c)
	{
...
static irqreturn_t da9155_irq_handler(int irq, void *data)
{
	...
	if (!da9155_read_reg(charger->i2c, DA9155_EVENT_A, &event_a) &&
		...
		if ((event_b & 0x1) && (charger->cable_type != POWER_SUPPLY_TYPE_BATTERY)) {
			...
			da9155_charger_initialize(charger);
			da9155_prevent_vin_drop(charger);
		}
		/* clear event reg */
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	if (charger->cable_type == POWER_SUPPLY_TYPE_BATTERY
		&& psp != POWER_SUPPLY_PROP_READ_SLAVE_REG) {
	...
	case POWER_SUPPLY_PROP_READ_SLAVE_REG:
		...
		if(ret<0)
			...
			val->intval = 1;
		pr_info("%s : READ_SLAVE_REG val= %d ret=%d\n",__func__,val->intval,ret);
		break;
	...
	const struct i2c_device_id *id)
{
	...
	if (pdata->irq_gpio) {
		...
		if (!gpio_get_value(pdata->irq_gpio))
			da9155_irq_handler(charger->chg_irq, charger);
#else
		//ret = da9155_read_reg(charger->i2c, DA9155_PAGE_CTRL_0, &data);
		//if (ret == 0) {
		//	da9155_write_reg(charger->i2c, DA9155_PAGE_CTRL_0, 0x06);
		//	da9155_read_reg(charger->i2c, 0x11, &data);
		//	da9155_write_reg(charger->i2c, DA9155_PAGE_CTRL_0, 0x0);
		//	pr_info("%s: mode(%d)\n", __func__, data);
		//	da9155_mode_change(charger, DISABLE);
		//}
