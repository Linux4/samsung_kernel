...
#if defined(CONFIG_MACH_ON5XLTE_CHN_OPEN)
...
#if defined(CONFIG_MACH_ON5XLTE_CHN_OPEN)
static void s2mu005_chg_event_work(struct work_struct *work)
{
	struct s2mu005_charger_data *charger =
		container_of(work, struct s2mu005_charger_data, chg_event_work.work);
	u8 val;
	s2mu005_read_reg(charger->client, S2MU005_CHG_STATUS3, &val);
	pr_info("[DEBUG] %s , %02x \n " , __func__, val);
	/*LSI patch, if hw timer expire happend, reset charger, for G5700 MTBF SMPL issue*/
	if ( (val & 0xF)== 0x8 )
	{
		pr_info("set charger off \n");
		s2mu005_enable_charger_switch(charger, 0);
	
		pr_info("set charger on \n");
		s2mu005_enable_charger_switch(charger, 1);
	}

}

static irqreturn_t s2mu005_chg_event_isr(int irq, void *data)
{
	struct s2mu005_charger_data *charger = data;

	queue_delayed_work(charger->charger_wqueue, &charger->chg_event_work, 0);
...
#if defined(CONFIG_MACH_ON5XLTE_CHN_OPEN)
	INIT_DELAYED_WORK(&charger->chg_event_work, s2mu005_chg_event_work);
#endif
	/*
	 * irq request
...
#if defined(CONFIG_MACH_ON5XLTE_CHN_OPEN)
	charger->irq_chg_event = pdata->irq_base + S2MU005_CHG_IRQ_EVENT;
		ret = request_threaded_irq(charger->irq_chg_event, NULL,
				s2mu005_chg_event_isr, 0 , "chg-event-irq", charger);
		if(ret < 0) {
			dev_err(s2mu005->dev, "%s: Fail to request chg event in IRQ: %d: %d\n",
						__func__, charger->irq_chg_event, ret);
			goto err_reg_irq;
		}
#endif
