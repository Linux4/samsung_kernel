...
*/
	...
	if (onoff > 0) {
		pr_err("[DEBUG]%s: turn on charger. dev_id:%d\n", __func__, charger->dev_id);
		s2mu005_update_reg(charger->client, S2MU005_CHG_CTRL0,
				   2 << REG_MODE_SHIFT, REG_MODE_MASK);
	} else if (!otg_enable_flag) {
		s2mu005_read_reg(charger->client, S2MU005_CHG_STATUS1, &val);
		pr_err("[DEBUG] %s , %02x \n " , __func__, val);

		if (((val & WCIN_STATUS_MASK) == WCIN_STATUS_VALID) ||
		    ((val & WCIN_STATUS_MASK) == WCIN_STATUS_INPUT_LIMITED)) {
			pr_err("[DEBUG] %s: turn off [reg_mode:001] charger. dev_id:%d\n", __func__, charger->dev_id);
			s2mu005_update_reg(charger->client, S2MU005_CHG_CTRL0,
					   1 << REG_MODE_SHIFT, REG_MODE_MASK);
		} else {
			pr_err("[DEBUG] %s: turn off charger. dev_id:%d\n", __func__, charger->dev_id);
			s2mu005_update_reg(charger->client, S2MU005_CHG_CTRL0,
