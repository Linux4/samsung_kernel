...
#define SM5714_CHARGER_VERSION  "VK2"
 ...
 * enable  : Decrease vbatreg by 2 step, when switching DC -> SWC.
 * disable : Restore vbatreg after the 1st topoff.
 */
static void sm5714_chg_vbatreg_stepdown(struct sm5714_charger_data *charger, bool en_stepdown)
{
	u16 set_voltage = 0, before_batreg = 0;
	...
	if (en_stepdown) {
		...
		dev_info(charger->dev, "float voltage down [%dmV] -> [%dmV]\n",
			charger->pdata->chg_float_voltage, set_voltage);
	} else {
		before_batreg = chg_get_regulation_voltage(charger);
		set_voltage = charger->pdata->chg_float_voltage;
		dev_info(charger->dev, "float voltage recovered [%dmV] -> [%dmV]\n",
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
	{
		...
		if (val->intval < get_topoff_curr)
			sm5714_chg_vbatreg_stepdown(charger, 0);
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_SHIPMODE_TEST:
			...
			break;
		case POWER_SUPPLY_EXT_PROP_CHARGING_ENABLED:
			sm5714_chg_vbatreg_stepdown(charger, 1);
	...
	struct sm5714_charger_platform_data *pdata)
{
	...
	if (pdata->chg_float_voltage_down_en)	{
		...
		if (ret) {
			pr_info("%s: sm5714,chg_float_voltage_down_offset_mv is Empty\n", __func__);
			pdata->chg_float_voltage_down_offset_mv = 0;
		}
	} else {
		pr_info("%s: sm5714,chg_float_voltage_down_en is Empty\n", __func__);
		pdata->chg_float_voltage_down_offset_mv = 0;
