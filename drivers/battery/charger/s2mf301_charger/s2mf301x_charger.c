...
static int s2mf301_get_en_shipmode(struct s2mf301_charger_data *charger)
{
	...
	s2mf301_read_reg(charger->top, S2MF301_TOP_SHIP_MODE_CTRL1, &reg);
	enable = (reg & 0x01) ? 1 : 0;
...
{
	s2mf301_update_reg(charger->top, S2MF301_TOP_SHIP_MODE_CTRL1, enable, 0x01);
	pr_info("s2mf301-charger: %s: forced ship mode - %s\n", __func__, enable ? "Enable" : "Disable");
}
...
static int s2mf301_charger_otg_control(struct s2mf301_charger_data *charger, bool enable)
{
	...
	if (!enable) {
		/* W/A set CHGIN2BAT */
		s2mf301_update_reg(charger->i2c, S2MF301X_CHG_D2A_SC_OTP4, 0x08, 0x08);

		s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL0, X_CHG_MODE, X_REG_MODE_MASK);
	} else {
		/* W/A set BYP2BAT */
		s2mf301_update_reg(charger->i2c, S2MF301X_CHG_D2A_SC_OTP4, 0x0, 0x08);

		s2mf301_update_reg(charger->i2c,
				S2MF301X_CHG_CTRL4, S2MF301_SET_OTG_OCP_1500mA << SET_OTG_OCP_SHIFT, SET_OTG_OCP_MASK);
		s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL0, X_OTG_BST_MODE, X_REG_MODE_MASK);
	}
...
out:
	...
	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS2, &chg_sts2);
	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_CTRL0, &chg_ctrl0);
...
static void s2mf301_set_input_current_limit(struct s2mf301_charger_data *charger, int charging_current)
{
	...
	s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL2,
			data << X_INPUT_CURRENT_LIMIT_SHIFT, X_INPUT_CURRENT_LIMIT_MASK);
...
static int s2mf301_get_input_current_limit(struct s2mf301_charger_data *charger)
{
	...
	ret = s2mf301_read_reg(charger->i2c, S2MF301X_CHG_CTRL2, &data);
	if (data < 0)
		return data;

	data = data & X_INPUT_CURRENT_LIMIT_MASK;
	if (data > 0x76) {
...
static void s2mf301_set_regulation_voltage(struct s2mf301_charger_data *charger, int float_voltage)
{
	...
	s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL7, data << X_SET_VF_VBAT_SHIFT, X_SET_VF_VBAT_MASK);
}
...
static int s2mf301_get_regulation_voltage(struct s2mf301_charger_data *charger)
{
	...
	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_CTRL7, &reg_data);
	reg_data &= 0xFF;
...
static void s2mf301_enable_charger_switch(struct s2mf301_charger_data *charger, int onoff)
{
	...
	if (onoff > 0) {
		...
		s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL17,
				S2MF301_FC_CHG_TIMER_16hr << X_SET_TIME_FC_CHG_SHIFT, X_SET_TIME_FC_CHG_MASK);

	} else {
		pr_info("[DEBUG]%s: turn off charger\n", __func__);

		s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL0, X_BUCK_MODE, X_REG_MODE_MASK);
	}
out:
	mutex_unlock(&charger->charger_mutex);
}
...
static void s2mf301_set_buck(struct s2mf301_charger_data *charger, int enable)
{
	...
	if (enable) {
		...
		s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL0, X_CHARGER_OFF_MODE, X_REG_MODE_MASK);
	}
}
...
static void s2mf301_set_fast_charging_current(struct s2mf301_charger_data *charger, int charging_current)
{
	...
	s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL10,
			data << X_FAST_CHARGING_CURRENT_SHIFT, X_FAST_CHARGING_CURRENT_MASK);
...
static int s2mf301_get_fast_charging_current(struct s2mf301_charger_data *charger)
{
	...
	ret = s2mf301_read_reg(charger->i2c, S2MF301X_CHG_CTRL10, &data);
	if (data < 0)
		return data;

	data = data & X_FAST_CHARGING_CURRENT_MASK;
...
static void s2mf301_set_topoff_current(struct s2mf301_charger_data *charger, int current_limit)
{
	...
	s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL14,
			data << X_FIRST_TOPOFF_CURRENT_SHIFT, X_FIRST_TOPOFF_CURRENT_MASK);
}
...
static int s2mf301_get_topoff_setting(struct s2mf301_charger_data *charger)
{
	...
	ret = s2mf301_read_reg(charger->i2c, S2MF301X_CHG_CTRL14, &data);
	if (ret < 0)
		return ret;

	data = data & X_FIRST_TOPOFF_CURRENT_MASK;
...
static bool s2mf301_chg_init(struct s2mf301_charger_data *charger, struct s2mf301_dev *s2mf301)
{
	...
	/* Set battery OCP 7A */
	s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL11, S2MF301_SET_BAT_OCP_7000mA, X_BAT_OCP_MASK);

	s2mf301_read_reg(s2mf301->i2c, 0xF5, &data);
	if ((data & 0x0F) < 0x02) {
		/* W/A enable MRSTB, disable VIO reset */
		s2mf301_update_reg(s2mf301->i2c, 0xC4, 0x08, 0x0F);
		s2mf301_write_reg(s2mf301->i2c, 0xC5, 0x0);
	} else {
		/* W/A disable MRSTB, enable VIO reset */
		s2mf301_update_reg(s2mf301->i2c, 0xC4, 0x0, 0x0F);
		s2mf301_write_reg(s2mf301->i2c, 0xC5, 0xDF);
	}
		...
		struct s2mf301_charger_data *charger)
{
	...
	ret = s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS0, &chg_sts0);
	...
	if (!(chg_sts0 & X_VBUS_DET_STATUS_MASK))
		status = POWER_SUPPLY_STATUS_DISCHARGING;
	else if (chg_sts0 & X_CHG_UVLOB_STATUS_MASK)
...
static bool s2mf301_get_batt_present(struct s2mf301_charger_data *charger)
{
	...
	ret = s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS0, &data);
	if (ret < 0)
		return false;

	return (data & X_BATID_STATUS_MASK) ? true : false;
}
...
static void s2mf301_wdt_clear(struct s2mf301_charger_data *charger)
{
	...
	/* watchdog kick */
	s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL16,
			0x1 << X_WDT_CLR_SHIFT, X_WDT_CLR_MASK);

	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS3, &reg_data);

	if ((reg_data & X_WDT_SUSPEND_STATUS_MASK) || (reg_data & X_WDT_AP_RESET_STATUS_MASK)) {
...
static int s2mf301_get_charging_health(struct s2mf301_charger_data *charger)
{
	...
	if ((data & X_VBUS_DET_STATUS_MASK) && (data & X_CHG_UVLOB_STATUS_MASK)) {
		charger->ovp = false;
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CHIP_ID:
			if (!s2mf301_read_reg(charger->top, S2MF301_REG_PMICID, &data)) {
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (is_nocharge_type(charger->cable_type)) {
			...
			if (charger->irq_ivr_enabled == 0) {
				...
				/* Unmask IRQ */
				s2mf301_update_reg(charger->i2c, S2MF301X_CHG_INT2M, 0 << IVR_M_SHIFT, IVR_M_MASK);
				enable_irq(charger->irq_ivr);
				s2mf301_read_reg(charger->i2c, S2MF301X_CHG_INT2M, &reg_data);
	...
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT_MAX:
		{
			...
			s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS2, &ivr_state);
			if (ivr_state & X_IVR_STATUS_MASK) {
				__pm_stay_awake(charger->ivr_ws);
				/* Mask IRQ */
				s2mf301_update_reg(charger->i2c, S2MF301X_CHG_INT2M, 1 << IVR_M_SHIFT, IVR_M_MASK);
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		if (val->intval == ATTACHED_DEV_JIG_UART_OFF_VB_MUIC) {
			...
			/* ICR MAX */
			s2mf301_write_reg(charger->i2c, S2MF301X_CHG_CTRL2, 0x7F);
			/* QBAT OFF 550ms*/
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CHG_OPTION5, 0x40, 0x40);
			/* EN_RID_301K_BAT2SYS_OFF */
			s2mf301_update_reg(charger->top, S2MF301_TOP_REG_JIG_CTRL, 0x0, 0x20);
			/* RD_OR_VBUS_MUX_SEL */
			value.intval = 1;
			psy_do_property("usbpd-manager", set, POWER_SUPPLY_PROP_ENERGY_NOW, value);
			/* IN2BAT OFF */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CHG_OPTION3, 0x02, 0x02);
			/* BAT2SYSOFF at factory mode release */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_T_CHG_ON5, 0x0, 0x40);
			...
			/* ICR MAX */
			s2mf301_write_reg(charger->i2c, S2MF301X_CHG_CTRL2, 0x7F);
			/* EN_RID_301K_BAT2SYS_ON */
			s2mf301_update_reg(charger->top, S2MF301_TOP_REG_JIG_CTRL, 0x20, 0x20);
			/* RD_ONLY_MUX_SEL */
			value.intval = 0;
			psy_do_property("usbpd-manager", set, POWER_SUPPLY_PROP_ENERGY_NOW, value);
			/* IN2BAT ON */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CHG_OPTION3, 0x0, 0x02);
			/* QBAT OFF DLY OFF at factory mode release */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CHG_OPTION5, 0x0, 0x40);
			/* BAT2SYS ON at factory mode release */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_T_CHG_ON5, 0x40, 0x40);
			/* QBAT ON */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_T_CHG_ON6, 0x0, 0x02);
			/* CHIP2SYS ON */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_T_CHG_OFF5, 0x0, 0x01);
			...
			/* ICR default 1.8A */
			s2mf301_write_reg(charger->i2c, S2MF301X_CHG_CTRL2, 0x47);
			/* QBAT OFF DLY ON */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CHG_OPTION5, 0x40, 0x40);
			/* EN_RID_301K_BAT2SYS_OFF */
			s2mf301_update_reg(charger->top, S2MF301_TOP_REG_JIG_CTRL, 0x0, 0x20);
	...
	case POWER_SUPPLY_PROP_AUTHENTIC:
		if (val->intval) {
			...
			/* Bypass mode JIG */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL0, 0x30, 0x30);
			/* AP ControlJIG */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_T_CHG_DIG0, 0x20, 0x20);
			/* EN_MRST, SET_MRSTBTMR 8.0s */
			s2mf301_update_reg(charger->top, S2MF301_TOP_MRSTB_RESET, 0x0F, 0x0F);
			msleep(100);
			/* QBAT OFF */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_T_CHG_ON6, 0x02, 0x02);
			/* CHIP2SYS OFF */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_T_CHG_OFF5, 0x01, 0x01);
			/* VIO RESET OFF */
			s2mf301_update_reg(charger->top, S2MF301_TOP_I2C_RESET_CTRL, 0x0, 0x01);
			/* VBUS_UVLO_MUIC_OFF */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_D2A_SC_OTP4, 0x0, 0x20);
			/* PM Disable */
			...
			/* All off */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL0, 0x00, 0x0F);
			/* Bypass Mode Disable */
			s2mf301_update_reg(charger->i2c, S2MF301X_CHG_T_CHG_DIG0, 0x0, 0x20);
		}
		break;
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		switch (ext_psp) {
		case POWER_SUPPLY_EXT_PROP_FACTORY_VOLTAGE_REGULATION:
			if (val->intval) {
				...
				s2mf301_write_reg(charger->i2c, S2MF301X_CHG_D2A_SC_OTP0, 0x0);
				/* ICR MAX */
				s2mf301_write_reg(charger->i2c, S2MF301X_CHG_CTRL2, 0x7F);
				/* D2A_SC_EN_CHGIN(INPUT TR ON) */
				s2mf301_update_reg(charger->i2c, S2MF301X_CHG_T_CHG_ON0, 0x02, 0x02);
			}
			break;
		case POWER_SUPPLY_EXT_PROP_CURRENT_MEASURE:
			if (val->intval) {
				...
				/* All off */
				s2mf301_update_reg(charger->i2c, S2MF301X_CHG_CTRL0, 0x0, 0x0F);
				/* Bypass Mode Disable */
				s2mf301_update_reg(charger->i2c, S2MF301X_CHG_T_CHG_DIG0, 0x0, 0x20);
			}
			break;
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CHARGE_POWERED_OTG_CONTROL:
			s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS2, &reg);
			pr_info("%s: S2MF301_CHG_STATUS2 : 0x%X\n", __func__, reg);
			if (!(reg & X_OTG_ON_OFF_STATUS_MASK) && (reg & X_BST_ON_STATUS_MASK))
				val->intval = 1;
			else
				val->intval = 0;
			s2mf301_read_reg(charger->i2c, S2MF301X_CHG_CTRL0, &reg);
			pr_info("%s: S2MF301_CHG_CTRL0 : 0x%X\n", __func__, reg);
			break;
		default:
			return -EINVAL;
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		break;
#if IS_ENABLED(EN_VF_BST)
	case POWER_SUPPLY_EXT_PROP_VF_BST:
		/* W/A set chip power to VBAT according to VBYP */
		/* EVT3.2 or higher */
		s2mf301_read_reg(charger->top, 0xF5, &data);
		if (data >= 0x23) {
			s2mf301_read_reg(charger->i2c, S2MF301X_CHG_CTRL12, &data);
			if (data >= 0x30)
...
static irqreturn_t s2mf301_det_bat_isr(int irq, void *data)
{
	...
	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS0, &val);
	if (!(val & X_BATID_STATUS_MASK)) {
		s2mf301_enable_charger_switch(charger, 0);
...
static irqreturn_t s2mf301_done_isr(int irq, void *data)
{
	...
	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS1, &val);
	pr_info("%s , %02x\n", __func__, val);
	if (val & (X_DONE_STATUS_MASK)) {
...
static irqreturn_t s2mf301_event_isr(int irq, void *data)
{
	...
	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS0, &sts0);
	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS1, &sts1);
	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS2, &sts2);
	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS3, &sts3);
	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS4, &sts4);
	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS5, &sts5);
	...
	s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS3, &val);
	pr_info("%s , %02x\n", __func__, val);

	if ((val & X_WDT_SUSPEND_STATUS_MASK) || (val & X_WDT_AP_RESET_STATUS_MASK)) {
		value.intval = 1;
...
static void s2mf301_ivr_irq_work(struct work_struct *work)
{
	...
	if (is_nocharge_type(charger->cable_type)) {
		...
		pr_info("%s : skip\n", __func__);
		s2mf301_read_reg(charger->i2c, S2MF301X_CHG_INT2M, &ivr_mask);
	...
	ret = s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS2, &ivr_state);
	if (ret < 0) {
		__pm_relax(charger->ivr_ws);
		pr_info("%s : I2C error\n", __func__);
		/* Unmask IRQ */
	...
	while ((ivr_state & X_IVR_STATUS_MASK) && charger->cable_type != SEC_BATTERY_CABLE_NONE) {

		if (s2mf301_read_reg(charger->i2c, S2MF301X_CHG_STATUS2, &ivr_state)) {
...
static const struct of_device_id s2mf301_charger_match_table[] = {
	{ .compatible = "samsung,s2mf301x-charger",},
	{},
};
...
static int s2mf301_charger_probe(struct platform_device *pdev)
{
	...
	pr_info("%s:[BATT] S2MF301X Charger driver probe\n", __func__);
	 ...
	 */
	charger->irq_ovp = pdata->irq_base + S2MF301X_CHG0_IRQ_CHGIN_OVP;
	...
	charger->irq_topoff = pdata->irq_base + S2MF301X_CHG1_IRQ_TOPOFF;
	...
	charger->irq_wdt_ap_reset = pdata->irq_base + S2MF301X_CHG3_IRQ_WDT_AP_RESET;
	...
	charger->irq_wdt_suspend = pdata->irq_base + S2MF301X_CHG3_IRQ_WDT_SUSPEND;
	...
	charger->irq_pre_trickle_chg_timer_fault = pdata->irq_base + S2MF301X_CHG3_IRQ_PRE_TRICKLE_CHG_TIMER_FAULT;
	...
	charger->irq_cool_fast_chg_timer_fault = pdata->irq_base + S2MF301X_CHG3_IRQ_COOL_FAST_CHG_TIMER_FAULT;
	...
	charger->irq_topoff_timer_fault = pdata->irq_base + S2MF301X_CHG3_IRQ_TOPOFF_TIMER_FAULT;
	...
	charger->irq_tfb = pdata->irq_base + S2MF301X_CHG4_IRQ_TFB;
	...
	charger->irq_tsd = pdata->irq_base + S2MF301X_CHG4_IRQ_TSD;
	...
	charger->irq_bat_ocp = pdata->irq_base + S2MF301X_CHG3_IRQ_BAT_OCP;
	...
	charger->irq_ivr = pdata->irq_base + S2MF301X_CHG2_IRQ_IVR;
	charger->irq_ivr_enabled = 1;
...
#if IS_ENABLED(EN_BAT_DET_IRQ)
	charger->irq_det_bat = pdata->irq_base + S2MF301X_CHG0_IRQ_BATID;
	...
	charger->irq_uvlob = pdata->irq_base + S2MF301X_CHG0_IRQ_CHGIN_UVLOB;
	...
	charger->irq_in2bat = pdata->irq_base + S2MF301X_CHG0_IRQ_CHGIN2BATB;
	...
	charger->irq_done = pdata->irq_base + S2MF301X_CHG1_IRQ_DONE;
	...
	pr_info("%s:[BATT] S2MF301X charger driver loaded OK\n", __func__);
