...
static void s2mu005_ivr_irq_work(struct work_struct *work)
{
	...
	while ((ivr_state & IVR_STATUS) && charger->cable_type != SEC_BATTERY_CABLE_NONE) {
		...
		struct s2mu005_charger_platform_data *pdata)
{
	...
	if (!np) {
		...
		ret = of_property_read_u32(np,
			"battery,chg_float_voltage", &pdata->chg_float_voltage);
...
static const struct of_device_id s2mu005_charger_match_table[] = {
...
static int s2mu005_charger_probe(struct platform_device *pdev)
{
	...
	wakeup_source_init(charger->ivr_ws, "charger-ivr");   // 4.19 R
	if (!(charger->ivr_ws)) {
		charger->ivr_ws = wakeup_source_create("charger-ivr"); // 4.19 Q
		if (charger->ivr_ws)
			wakeup_source_add(charger->ivr_ws);
	}
#else
	charger->ivr_ws = wakeup_source_register(null, "charger-ivr"); // 5.4 R
#endif
