		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		if (val->intval) {
			...
			/* manual reset time 1s for R7/A71x */
...
static irqreturn_t s2mu107_ivr_isr(int irq, void *data)
{
	...
	__pm_stay_awake(sw_charger->ivr_ws);
...
static int s2mu107_switching_charger_probe(struct platform_device *pdev)
{
	...
	battery_wakeup_source_init(&pdev->dev, &sw_charger->ivr_ws, "charger-ivr");
