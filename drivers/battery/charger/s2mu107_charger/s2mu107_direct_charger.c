 ...
 */
#include <linux/mfd/slsi/s2mu107/s2mu107.h>
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_DIRECT_VOLTAGE_MAX:
			val->intval = charger->step_vbatt_mv;
			break;
		default:
			return -EINVAL;
		}
...
static void s2mu107_dc_approach_target_curr_manual(struct s2mu107_dc_data *charger)
{
	...
	if (charger->step_iin_ma <= DC_THRESH)
		return;
...
static irqreturn_t s2mu107_dc_mode_isr(int irq, void *data)
{
	...
	__pm_stay_awake(charger->mode_irq_ws);
	pr_info("%s", __func__);
	...
	__pm_relax(charger->mode_irq_ws);
...
static irqreturn_t s2mu107_dc_fail_isr(int irq, void *data)
{
	...
	__pm_stay_awake(charger->fail_irq_ws);
	...
	__pm_relax(charger->fail_irq_ws);
	return IRQ_HANDLED;
}
...
static irqreturn_t s2mu107_dc_thermal_isr(int irq, void *data)
{
	...
	__pm_stay_awake(charger->thermal_irq_ws);
	...
	__pm_relax(charger->thermal_irq_ws);
	...
	struct alarm *alarm, ktime_t now)
{
	...
	__pm_stay_awake(charger->dc_mon_ws);
...
static int s2mu107_direct_charger_probe(struct platform_device *pdev)
{
	...
	battery_wakeup_source_init(&pdev->dev, &charger->ws, "dc_ws");
	battery_wakeup_source_init(&pdev->dev, &charger->mode_irq_ws, "dc_mode_irq_ws");
	battery_wakeup_source_init(&pdev->dev, &charger->fail_irq_ws, "dc_fail_irq_ws");
	battery_wakeup_source_init(&pdev->dev, &charger->thermal_irq_ws, "dc_thermal_irq_ws");
	battery_wakeup_source_init(&pdev->dev, &charger->dc_mon_ws, "dc_mon_ws");
	battery_wakeup_source_init(&pdev->dev, &charger->state_manager_ws, "state_man_wake");
...
err_parse_dt_nomem:
	...
	wakeup_source_unregister(charger->ws);
	wakeup_source_unregister(charger->mode_irq_ws);
	wakeup_source_unregister(charger->fail_irq_ws);
	wakeup_source_unregister(charger->thermal_irq_ws);
	wakeup_source_unregister(charger->dc_mon_ws);
