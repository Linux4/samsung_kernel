		...
		struct s2mu107_sc_data *sw_charger, bool enable)
{
	...
	if (!enable) {
		...
		pr_info("%s: OTG off, boost w/a + async w/a\n", __func__);
		...
		pr_info("%s: OTG on, boost w/a + async w/a\n", __func__);
	...
	struct s2mu107_sc_data *sw_charger, int onoff, bool dual_buck)
{
	...
	mutex_lock(&sw_charger->charger_mutex);
	if (onoff > 0) {
		pr_info("[DEBUG]%s: turn on switching charger + async w/a\n", __func__);
		...
		if ((val & 0x01) == 0x01 || dual_buck == true) {	/* if DC is on */
			pr_info("[DEBUG] %s: dual buck mode + async w/a\n", __func__);
			/* set async mode */
			...
			pr_info("[DEBUG] %s: buck mode + async w/a\n", __func__);
...
static void s2mu107_sc_set_buck(
	...
	if (enable) {
		...
		pr_info("[DEBUG]%s: set buck off (charger off mode) + async w/a\n", __func__);
		/* set async mode */
#if defined(CONFIG_SEC_FACTORY)
		if (!factory_mode) {
			s2mu107_update_reg(sw_charger->i2c, S2MU107_SC_TEST9, 0x03, 0x03);
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		if (val->intval) {
			...
			s2mu107_write_reg(sw_charger->i2c_common, 0xEF, 0x00);
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CURRENT_MEASURE:
			if (val->intval) {
				...
				/* only A91, A71 A-pen application */
				s2mu107_write_reg(sw_charger->i2c, S2MU107_SC_CTRL18, 0x5F);
				s2mu107_write_reg(sw_charger->i2c, 0x3B, 0x5F);
				msleep(200);
...
static irqreturn_t s2mu107_sc_fault_isr(int irq, void *data)
{
	...
	*/
	...
	case CHG_STATUS_WD_SUSPEND:
		/* 1 */
		pr_info("%s: wdt suspend, USB PD reset\n", __func__);
		value.intval = 1;
		psy_do_property("s2mu107-usbpd", set,
	...
	case CHG_STATUS_WD_RST:
		/* 1,3 */
		pr_info("%s: wdt AP reset, USB PD reset + async w/a\n", __func__);
		value.intval = 1;
		psy_do_property("s2mu107-usbpd", set,
					POWER_SUPPLY_PROP_USBPD_RESET, value);
#if defined(CONFIG_SEC_FACTORY)
	...
	case CHG_STATUS_PRECHG_TMR_FAULT:
		/* 2,3 */
		pr_info("%s: pre-charge timer fault, set buck mode + async w/a\n", __func__);
		s2mu107_sc_onoff(sw_charger, false, false);
		sw_charger->is_charging = false;
	...
	case CHG_STATUS_TOPOFF_TMR_FAULT:
		/* 3 */
		pr_info("%s: fast charge or top off timer fault, async w/a\n", __func__);
#if defined(CONFIG_SEC_FACTORY)
		if (!factory_mode)
			s2mu107_update_reg(sw_charger->i2c, S2MU107_SC_TEST9, 0x03, 0x03);
#else
...
static irqreturn_t s2mu107_sc_qbat_isr(int irq, void *data)
{
	struct s2mu107_sc_data *sw_charger = data;
	u8 val;
	u8 fault;
	...
	if (fault == CHG_STATUS_NORMAL) {
		pr_info("%s, recovered to normal + async w/a\n", __func__);
...
static int s2mu107_switching_charger_probe(struct platform_device *pdev)
{
	...
	sw_charger->irq_qbat = pdata->irq_base + S2MU107_SC_IRQ3_QBAT_OFF;
	ret = request_threaded_irq(sw_charger->irq_qbat, NULL,
			s2mu107_sc_qbat_isr, 0, "qbat-irq", sw_charger);
	if (ret < 0) {
		dev_err(s2mu107->dev, "%s: Fail to request QBAT ON/OFF in IRQ: %d: %d\n",
				__func__, sw_charger->irq_qbat, ret);
		goto err_reg_irq;
	}
