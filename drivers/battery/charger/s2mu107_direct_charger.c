...
static void s2mu107_dc_set_vsys_dcmode(struct s2mu107_dc_data *charger, s2mu107_dc_ctrl_t en)
{
	...
	case S2MU107_DC_ENABLE:
		/* 0x29[7:6] = 11 (VSYS_Discharge ON) */
		s2mu107_update_reg(charger->i2c, S2MU107_SC_CTRL17_DC,
			0x3 << T_EN_VSYS_DISCHARGE_SHIFT, T_EN_VSYS_DISCHARGE_MASK);
		...
		if (charger->rev_id == 1) {
			/*
			 * 0x7ACC[3:0] = 4'b1111
			 * (SET_SYS_OVP_F[1:0] / SET_SYS_OVP_R[1:0] = 4.7V / 4.8V)
			 */
			s2mu107_update_reg(charger->i2c, S2MU107_SC_OTP_96,
				0x3 << SET_VSYS_OVP_FALLING_SHIFT, SET_VSYS_OVP_FALLING_MASK);
			s2mu107_update_reg(charger->i2c, S2MU107_SC_OTP_96,
				0x3 << SET_VSYS_OVP_RISING_SHIFT, SET_VSYS_OVP_RISING_MASK);
			usleep_range(1000, 1100);

			/* 0x7A33[0] = 1'b1 (SEL_VSYS_RS_DC) */
			s2mu107_update_reg(charger->i2c, S2MU107_SC_TEST2_DC,
				0x1, SEL_VSYS_RS_DC_MASK);
			usleep_range(1000, 1100);

			/*
			 * 0x7A20[3:0] = 4'b0110
			 * (SET_VF_VSYS[3:0] = 4.2V)
			 */
			s2mu107_update_reg(charger->i2c, S2MU107_SC_CTRL8_DC,
				0x6 << SET_VF_VSYS_SHIFT, SET_VF_VSYS_MASK);
		} else {
	...
	default:
		...
		/* 0x29[7:6] = 01 (VSYS_Discharge OFF) */
		s2mu107_update_reg(charger->i2c, S2MU107_SC_CTRL17_DC,
			0x1, T_EN_VSYS_DISCHARGE_MASK);
		break;
	}
}
...
static void s2mu107_dc_ir_drop_compensation(struct s2mu107_dc_data *charger)
{
	...
	while (charger->ppsVol > vchgin) {
		if (vchgin < 4100) {
			charger->dc_trans = DC_TRANS_DETACH;
...
static void s2mu107_dc_state_manager(struct s2mu107_dc_data *charger, s2mu107_dc_trans_t trans)
{
	...
	case DC_STATE_SLEEP_CC:
		if (trans == DC_TRANS_DC_DONE_INT)
			next_state = DC_STATE_DONE;
		else if (trans == DC_TRANS_FLOAT_VBATT)
...
static void s2mu107_dc_state_check_vbat(void *data)
{
	...
	if (vBatt > DC_MIN_VBAT && soc < DC_MAX_SOC) {
		if (charger->dc_chg_status == SC_CHARGING_ENABLE_CHECK_VBAT){
			...
			ret = power_supply_set_property(charger->psy_sc, POWER_SUPPLY_PROP_CHARGING_ENABLED, &value);
			if (ret < 0)
...
#else
	unsigned int soc = s2mu107_dc_get_fg_value(charger, POWER_SUPPLY_PROP_CAPACITY);
	pr_info("%s, soc : %d\n", __func__, soc);
	select_pdo(1);
	msleep(100);
	s2mu107_dc_state_trans_enqueue(charger, DC_TRANS_BATTERY_OK);
...
static void s2mu107_dc_state_preset(void *data)
{
	...
	s2mu107_dc_set_charging_current(charger, charging_current, S2MU107_DC_MODE_TA_CC);
	...
	*/
	...
	/* Set Float Voltage 4.55V */
	s2mu107_write_reg(charger->i2c, S2MU107_DC_CTRL1, SET_CV_4550_MV);
...
static void s2mu107_dc_step_cv(struct s2mu107_dc_data *charger)
{
	...
	pr_info("%s, iavg : %d, vavg : %d, step_vbatt_mv : %d\n",
		__func__, iavg, vavg, charger->step_vbatt_mv);
	charger->step_cv_cnt++;
