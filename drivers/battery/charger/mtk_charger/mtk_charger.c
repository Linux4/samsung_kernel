...
#if defined(CONFIG_AFC_CHARGER)
			if (mtk_charger_check_afc_conditions(charger) == true)
				set_afc_voltage(0x9);
#endif
		}
			...
			struct mtk_charger_data *charger, int input_current)
{
	...
	/* AICL */
	if (is_wired_type(charger->cable_type)) {
		charger_dev_run_aicl(charger->chg_dev,
					&input_current_limit_by_aicl);
		if (input_current_limit_by_aicl > 0)
			pr_info("[DEBUG]%s: Input current after AICL: %d\n",
				__func__, input_current_limit_by_aicl);
	}

	if (input_current_limit_by_aicl > 0)
		if ((input_current_limit_by_aicl < input_current) &&
			(is_wired_type(charger->cable_type)))
			charger_dev_set_input_current(charger->chg_dev,
					(input_current_limit_by_aicl * 1000));
}
...
static bool mtk_charger_chg_init(struct mtk_charger_data *charger)
{
	...
	if (f_mode_battery == IB_MODE) { /* Set IB settings */
		pr_info("%s: Set IB Mode\n", __func__);
		mtk_charger_ib_mode(charger, true);
	} else if (f_mode_battery == NO_MODE) {
		if (charger->pdata->chgenb_en == 0) {
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		val->intval = mtk_charger_get_fast_charging_current(charger);
		break;
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_POWERED_OTG_CONTROL:
		/* TO-DO: Need to return OTG status register if needed */
		break;
	default:
		return -EINVAL;
	}
