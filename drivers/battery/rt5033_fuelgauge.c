...
extern struct s3c_adc_client *temp_adc_client;
#define BATTERY_ADC_THRESHOLD 3500

static int rt5033_batt_adc_read(void)
{
	int i;
	int ret = 0;
...
/*#if defined(CONFIG_MACH_MEGA2LTE_USA_ATT)
	int adc = rt5033_batt_adc_read();
	if (adc > BATTERY_ADC_THRESHOLD)
		return false;
#endif*/
