...
static bool sec_multi_chg_check_sub_charging(struct sec_multi_charger_info *charger)
{
	...
	if (charger->pdata->sub_charger_condition &
		SEC_SUB_CHARGER_CONDITION_VOLTAGE_AVG) {
		value.intval = SEC_BATTERY_VOLTAGE_AVERAGE;
		psy_do_property(charger->pdata->battery_name, get,
			POWER_SUPPLY_PROP_VOLTAGE_AVG, value);
		
		value.intval /= 1000; //mV
		if (value.intval > charger->pdata->sub_charger_condition_voltage_avg) {
			if (charger->sub_is_charging)
				pr_info("%s: sub charger off VOLTAGE_AVG(%d)\n", __func__, value.intval);
			return false;
		} else if (value.intval > charger->pdata->sub_charger_enable_voltage_avg) {
			if (!charger->sub_is_charging) {
				return false;
			}	
		}
			    ...
			    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		break;
	case POWER_SUPPLY_PROP_MULTI_CHARGER_MODE:
		...
		break;
	case POWER_SUPPLY_PROP_READ_SLAVE_REG:
		psy_do_property(charger->pdata->sub_charger_name, get, psp, value);
		val->intval = value.intval;
		break;
	default:
	...
	if (!np) {
		...
		if (pdata->sub_charger_condition) {
			...
			if (ret) {
				...
				pdata->sub_charger_enable_current_now =
					pdata->sub_charger_condition_current_now + 200;
			}

			ret = of_property_read_u32(np, "charger,sub_charger_condition_voltage_avg",
					&pdata->sub_charger_condition_voltage_avg);
			if (ret) {
				pr_err("%s: sub_charger_condition_voltage_avg is Empty\n", __func__);
				pdata->sub_charger_condition &= ~SEC_SUB_CHARGER_CONDITION_VOLTAGE_AVG;
				pdata->sub_charger_condition_voltage_avg = 0;
			}

			ret = of_property_read_u32(np, "charger,sub_charger_enable_voltage_avg",
					&pdata->sub_charger_enable_voltage_avg);
			if (ret) {
				pr_err("%s: sub_charger_enable_voltage_avg is Empty\n", __func__);
				pdata->sub_charger_enable_voltage_avg =
					pdata->sub_charger_condition_voltage_avg - 50;
			}
