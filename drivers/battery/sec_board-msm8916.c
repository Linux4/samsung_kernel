...
void board_battery_init(struct platform_device *pdev, struct sec_battery_info *battery)
{
    ...
    (battery->pdata->thermal_source == SEC_BATTERY_THERMAL_SOURCE_ADC)) {
		...
		if(system_rev <= 6) {
			battery->pdata->temp_adc_table = temp_table_r05_r06;
			battery->pdata->temp_amb_adc_table = temp_table_r05_r06;

			battery->pdata->temp_adc_table_size = sizeof(temp_table_r05_r06)/sizeof(sec_bat_adc_table_data_t);
			battery->pdata->temp_amb_adc_table_size = sizeof(temp_table_r05_r06)/sizeof(sec_bat_adc_table_data_t);
		} else {
...
#if defined(CONFIG_MACH_A5_CHN_OPEN)
