...
#if defined(CONFIG_BATTERY_SWELLING)
static int sec_bat_swelling_charging_current(struct sec_battery_info *battery)
{
	...
	if (!battery->charging_current_swelling)
		battery->charging_current_swelling = battery->pdata->charging_current[battery->cable_type].fast_charging_current;
	if (battery->cable_type == POWER_SUPPLY_TYPE_BATTERY) {
		...
		pr_info("%s: event: 0x%x, charging current(%d)\n",
			__func__, battery->current_event, charging_current);
		if (battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP_SWELLING ||
			battery->current_event & SEC_BAT_CURRENT_EVENT_HIGH_TEMP_SWELLING ||
			battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP) {
			/* set charging current */
			if (battery->charging_current_swelling != charging_current) {
				pr_info("%s: set charging current(%d)\n", __func__, charging_current);
				value.intval = charging_current;
				psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_CURRENT_AVG, value);
				battery->charging_current_swelling = charging_current;
			}
			/* set topoff current */
			if (battery->topoff_current != topoff_current) {
				pr_info("%s: set topoff current(%d)\n", __func__, topoff_current);
	...
	if (battery->status == POWER_SUPPLY_STATUS_FULL &&
		...
		if ((battery->pdata->recharge_condition_type &
			...
			if (!(battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP)) {
				...
				goto check_recharge_check_count;
			} else if (battery->voltage_now <= recharging_voltage) {
				battery->expired_time = battery->pdata->recharging_expired_time;
				battery->prev_safety_time = 0;
				dev_info(battery->dev,
						"%s: Re-charging by VCELL under low temp event (%d)\n",
						__func__, battery->voltage_now);
				goto check_recharge_check_count;
			}
		}
...
static void sec_bat_swelling_check(struct sec_battery_info *battery, int temperature)
{
	...
	if (!battery->swelling_mode) {
		if (((temperature >= battery->pdata->swelling_high_temp_block) ||
			...
			battery->charging_current_swelling = val.intval;
		} else if ((temperature >= battery->pdata->swelling_low_temp_recov_1st) &&
			(battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP)) {
			pr_info("%s: normal temperature recover current\n", __func__);
			val.intval = battery->pdata->charging_current[battery->cable_type].fast_charging_current;
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_CURRENT_AVG, val);
			if (battery->siop_level < 100) {
...
skip_swelling_check:
	sec_bat_swelling_charging_current(battery);
...
static void sec_bat_calculate_safety_time(struct sec_battery_info *battery)
{
	...
	charging_power = battery->charging_current * (battery->pdata->chg_float_voltage / battery->pdata->chg_float_voltage_conv);
					...
					const char *buf, size_t count)
{
	...
	if(tuner_running_status==TUNER_IS_OFF) {
		...
		if (sscanf(buf, "%10d\n", &x) == 1) {
			...
			if (battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP_SWELLING ||
				...
				psy_do_property(battery->pdata->charger_name, get,
					POWER_SUPPLY_PROP_CURRENT_AVG, value);
				if (value.intval > battery->charging_current_swelling) {
					battery->charging_current_swelling = value.intval;
					sec_bat_swelling_charging_current(battery);
				}
			}
#endif
	...
	case BATT_EVENT_GPS:
		...
		break;
	case BATT_TEMP_TABLE:
		if (sscanf(buf, "%10d %10d %10d %10d %10d %10d %10d %10d %10d %10d %10d %10d\n",
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if((val->intval)==TUNER_SWITCHED_ON_SIGNAL) {
			...
			if((battery->cable_type != POWER_SUPPLY_TYPE_UNKNOWN) &&
				...
				union power_supply_propval value_Check_CurrentNow = {0, };
		...
		struct sec_battery_info *battery)
{
	...
	if (ret) {
		pr_info("%s: swelling chg current is Empty, Default value 1300mA \n", __func__);
		pdata->swelling_chg_current = 1300;
	}
	...
	if (pdata->swelling_low_temp_2step_mode) {
		...
		ret = of_property_read_u32(np, "battery,swelling_high_chg_current",
						&pdata->swelling_high_temp_current);
		if (ret) {
			pr_info("%s: swelling high temp chg current is Empty, Default value 1300mA \n", __func__);
			pdata->swelling_high_temp_current = 1300;
		}
		...
		ret = of_property_read_u32(np, "battery,swelling_low_chg_current",
					&pdata->swelling_low_temp_current);
		if (ret) {
			pr_info("%s: swelling low temp chg current is Empty, Default value 1300mA \n", __func__);
			pdata->swelling_low_temp_current = 1300;
		}

		ret = of_property_read_u32(np, "battery,swelling_full_check_current_2nd",
					&pdata->swelling_high_temp_topoff);
		pdata->swelling_low_temp_topoff = pdata->swelling_high_temp_topoff;
		
		if (ret) {
			pr_info("%s: swelling_full_check_current_2nd is Empty, default 200mA\n", __func__);
			pdata->swelling_high_temp_topoff = 200;
			pdata->swelling_low_temp_topoff = 200;
		}
		pr_info("%s: swelling_full_check_current_2nd : %d\n", __func__, pdata->swelling_high_temp_topoff);
	}
	...
	ret = of_property_read_u32(np, "battery,low_event_rechg_voltage",
		(unsigned int *)&pdata->low_event_rechg_voltage);
	if (ret) {
		pr_info("%s: low_event_rechg_voltage is Empty\n", __func__);
		pdata->low_event_rechg_voltage = 4200;
	}

	pr_info("%s : SWELLING_CHG_CURRENT(%d) SWELLING_HIGH_TEMP_CURRENT(%d)\n"
		"SWELLING_LOW_TEMP_CURRENT(%d) SWELLING_FULL_CHECK_CURRENT(LOW:%d, HIGH:%d)\n"
		"SWELLING_HIGH_TEMP (%d) SWELLING_HIGH_TEMP_RECOVERY(%d) \n"
		"SWELLING_LOW_TEMP_1st(%d) SWELLING_LOW_TEMP_RECOVERY_1st(%d) \n"
		"SWELLING_LOW_TEMP_2nd(%d) SWELLING_LOW_TEMP_RECOVERY_2nd(%d)",
		__func__, pdata->swelling_chg_current, pdata->swelling_high_temp_current,
		pdata->swelling_low_temp_current, pdata->swelling_low_temp_topoff,
		pdata->swelling_high_temp_topoff,pdata->swelling_high_temp_block,
		pdata->swelling_high_temp_recov, pdata->swelling_low_temp_block_1st,
		pdata->swelling_low_temp_recov_1st, pdata->swelling_low_temp_block_2nd,
		pdata->swelling_low_temp_recov_2nd);
#endif
	...
	pdata->chg_float_voltage = pdata->swelling_normal_float_voltage;
	if (ret)
...
static int sec_battery_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_BATTERY_SWELLING)
	battery->swelling_mode = false;
	battery->charging_current_swelling = 0;
#endif
	...
	sec_bat_set_current_event(battery, SEC_BAT_CURRENT_EVENT_NONE,
		SEC_BAT_CURRENT_EVENT_NONE);
