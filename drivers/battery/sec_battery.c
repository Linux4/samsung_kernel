	...
	if (battery->swelling_mode) {
		...
		if ((temperature <= battery->pdata->swelling_high_temp_recov) &&
			...
			if ((temperature <= battery->pdata->swelling_low_temp_block) &&
				...
				if (battery->swelling_mode == SWELLING_MODE_ADDITIONAL) {
					val.intval = battery->pdata->swelling_low_temp_additional_current;
				} else {
		...
		struct sec_battery_info *battery)
{
	...
	if (ret) {
		pr_info("%s: swelling_low_chg_current is Empty, Defualt value 600mA \n", __func__);
		pdata->swelling_low_chg_current = 600;
	}
