...
static void sec_bat_swelling_check(struct sec_battery_info *battery, int temperature)
{
	...
	if (!battery->swelling_mode) {
		if (((temperature >= battery->swelling_temp_high_threshold) ||
			...
			sec_bat_set_charge(battery, false);
		}else if ((battery->temperature <= battery->swelling_low_temp_block_1st) &&
			!battery->en_swelling) {
			pr_info("%s: low temperature reduce current\n", __func__);
 		        battery->en_swelling = true;
			val.intval = battery->pdata->swelling_low_chg_current;
			psy_do_property(battery->pdata->charger_name, set,POWER_SUPPLY_PROP_CURRENT_AVG, val);                       
		} else if ((battery->temperature >= battery->swelling_low_temp_recov_1st) &&
			battery->en_swelling) {
	...
	if (battery->swelling_mode) {
		...
		if ((temperature <= battery->swelling_temp_high_recovery) &&
		    (temperature >= battery->swelling_low_temp_recov_2nd)) {
			...
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_VOLTAGE_MAX, val);
			if (battery->temperature <= battery->swelling_low_temp_block_1st) {
		                battery->en_swelling = true;
			        val.intval = battery->pdata->swelling_low_chg_current;
			        psy_do_property(battery->pdata->charger_name, set,POWER_SUPPLY_PROP_CURRENT_AVG, val);                                
			}
			...
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_VOLTAGE_MAX, val);
			if ((temperature <= battery->swelling_low_temp_recov_2nd) &&
		...
		if ((battery->health == POWER_SUPPLY_HEALTH_GOOD) &&
			...
			if (battery->capacity >= 100)
				...
				sec_bat_set_charging_status(battery,
						POWER_SUPPLY_STATUS_CHARGING);
#if defined(CONFIG_BATTERY_SWELLING)
			if ((temp_value >= battery->swelling_temp_high_recovery) ||
#if defined(CONFIG_BATTERY_SWELLING_LOW_TEMP_SECOND)
				(temp_value <= battery->swelling_low_temp_recov_2nd)) {
#else
                                (temp_value <= battery->swelling_temp_low_recovery)) {
#endif
				pr_info("%s: swelling mode start. stop charging\n", __func__);
				battery->swelling_mode = true;
				battery->swelling_full_check_cnt = 0;
