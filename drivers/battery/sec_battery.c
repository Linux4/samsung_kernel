...
static void sec_bat_cable_work(struct work_struct *work)
{
	...
	if (battery->cable_type == POWER_SUPPLY_TYPE_BATTERY ||
		...
		/* In some projects because of possible timing issue related with the PM wake-up,
		   the cable work is setting the device's status to charging
		   before the temperature of the device is checked.
		   This additional temperature check is made to prevent the status of the device from
		   being set to charging incorrectly */
		sec_bat_temperature_check(battery);
