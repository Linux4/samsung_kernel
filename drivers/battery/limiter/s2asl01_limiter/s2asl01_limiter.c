...
static int s2asl01_get_vchg(struct s2asl01_limiter_data *limiter, int mode)
{
	...
	//pr_info("%s [%s]: data0 (%d) data1 (%d)\n", __func__, current_limiter_type_str[limiter->pdata->bat_type], data[0], data[1]);
...
{
	//struct device_node *np = of_find_node_by_name(NULL, "s2asl01-limiter");
...
{
	//struct s2asl01_limiter_data *limiter =
	//	container_of(work, struct s2asl01_limiter_data, limiter_isr_work.work);
}
				...
				const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&limiter->limiter_isr_work, limiter_isr_work);

	//if( !strcmp(limiter->dev->type->name, "s2asl01-limiter-main")) {
...
#if 0
	if (limiter->pdata->bat_int) {
		limiter->irq_bat_int = gpio_to_irq(limiter->pdata->bat_int);
		dev_err(&client->dev, "%s bat_int = %d, irq_bat_int = %d\n", __func__, limiter->pdata->bat_int, limiter->irq_bat_int);
		ret = request_threaded_irq(limiter->irq_bat_int, NULL,
			s2asl01_irq_handler,
		...
		ret = enable_irq_wake(limiter->pdata->bat_int);
		if (ret < 0)
			pr_err("%s : Failed to enable wakeup source(%d)\n", __func__, ret);
	}
#endif

	queue_delayed_work(limiter->wqueue, &limiter->limiter_isr_work, msecs_to_jiffies(0));

	//gpio_direction_output(limiter->pdata->bat_enb, 0);
