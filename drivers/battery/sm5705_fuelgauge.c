...
int sm5705_calculate_iocv(struct i2c_client *client)
{
	...
	pr_info("%s: ioci_l_max=0x%x, ioci_l_min=0x%x, ioci_l_sum=0x%x, ioci_l_avg=0x%x, i_offset=0x%x, sign_i_offset=%d\n", __func__, i_max, i_min, i_sum, lb_i_avg, i_offset, sign_i_offset);
...
static bool sm5705_fg_reg_init(struct i2c_client *client, int is_surge)
{
	...
	if(is_surge)
	{
		value = ((fuelgauge->info.batt_ocv<<8)/1000);
	}
	else
	{
...
void sm5705_vbatocv_check(struct i2c_client *client)
{
	...
	if((abs(fuelgauge->info.batt_current)<40)
	 || ((fuelgauge->is_charging) && (fuelgauge->info.batt_current<(fuelgauge->info.top_off)) && (fuelgauge->info.batt_current>(fuelgauge->info.top_off/3))))
	{
...
static void sm5705_cal_carc (struct i2c_client *client)
{
	...
	{
		p_fg_delta_cal = (fg_temp_gap / fuelgauge->info.high_fg_temp_cal_denom)*fuelgauge->info.high_fg_temp_p_cal_fact;
		n_fg_delta_cal = (fg_temp_gap / fuelgauge->info.high_fg_temp_cal_denom)*fuelgauge->info.high_fg_temp_n_cal_fact;
	}
	else if (fuelgauge->info.en_low_fg_temp_cal && (fg_temp_gap < 0))
	{
		fg_temp_gap = -fg_temp_gap;
		p_fg_delta_cal = (fg_temp_gap / fuelgauge->info.low_fg_temp_cal_denom)*fuelgauge->info.low_fg_temp_p_cal_fact;
		n_fg_delta_cal = (fg_temp_gap / fuelgauge->info.low_fg_temp_cal_denom)*fuelgauge->info.low_fg_temp_n_cal_fact;
	}
	p_curr_cal = p_curr_cal + (p_fg_delta_cal);
	n_curr_cal = n_curr_cal + (n_fg_delta_cal);

	pr_info("%s: temp_fg = %d ,p_curr_cal = 0x%x, n_curr_cal = 0x%x, curr_offset = 0x%x, volt_cal = 0x%x ,batt_temp = %d\n",
		__func__, fuelgauge->info.temp_fg, p_curr_cal, n_curr_cal, fuelgauge->info.curr_offset, fuelgauge->info.volt_cal, fuelgauge->info.temperature);
...
static unsigned int sm5705_get_all_value(struct i2c_client *client)
{
	...
	pr_info("%s: SM5705 is_charging = %d, ta_exist = %d, vbat=%d, vbat_avg=%d, curr=%d, curr_avg=%d, ocv=%d, temp=%d, cycle=%d\n",
		__func__, fuelgauge->is_charging, fuelgauge->ta_exist, fuelgauge->info.batt_voltage, fuelgauge->info.batt_avgvoltage, fuelgauge->info.batt_current, fuelgauge->info.batt_avgcurrent, fuelgauge->info.batt_ocv, fuelgauge->info.temp_fg, fuelgauge->info.batt_soc_cycle);
...
static int sm5705_fg_parse_dt(struct sec_fuelgauge_info *fuelgauge)
{
	...
	PINFO("%s = <%d %d %d>\n", prop_name, fuelgauge->info.enable_topoff_soc, fuelgauge->info.topoff_soc, fuelgauge->info.top_off);
	...
	PINFO("%s = <%d %d %d>\n", prop_name, fuelgauge->info.cycle_high_limit, fuelgauge->info.cycle_low_limit, fuelgauge->info.cycle_limit_cntl);
	...
	PINFO("%s = <%d, %d, %d, %d, %d, %d, %d, %d>\n", prop_name,
		fuelgauge->info.en_high_fg_temp_cal, fuelgauge->info.high_fg_temp_cal_denom, fuelgauge->info.high_fg_temp_p_cal_fact,
		fuelgauge->info.high_fg_temp_n_cal_fact, fuelgauge->info.en_low_fg_temp_cal, fuelgauge->info.low_fg_temp_cal_denom,
	...
	PINFO("%s = <%d, %d, %d, %d, %d, %d, %d, %d>\n", prop_name,
		fuelgauge->info.en_high_temp_cal, fuelgauge->info.high_temp_cal_denom, fuelgauge->info.high_temp_p_cal_fact,
		fuelgauge->info.high_temp_n_cal_fact, fuelgauge->info.en_low_temp_cal, fuelgauge->info.low_temp_cal_denom,
		fuelgauge->info.low_temp_p_cal_fact, fuelgauge->info.low_temp_n_cal_fact);
			     ...
			     union power_supply_propval *val)
{
	...
	// move to POWER_SUPPLY_PROP_CAPACITY
	// sm5705_get_all_value(client);
