...
static int sm5705_get_all_value(struct i2c_client *client)
{
	union power_supply_propval value, val;
	struct sec_fuelgauge_info *fuelgauge = i2c_get_clientdata(client);
	...
	if(!fuelgauge->ta_exist){
		...
		if(val.intval == POWER_SUPPLY_TYPE_MAINS){	
			fuelgauge->ta_exist = 1;
			pr_info("%s : ta_exist is updated\n",__func__);
		}
	}
#ifdef SM5705_FG_FULL_DEBUG
	...
	pr_info("%s: is_chg=%d, ta_exist=%d, v=%d, v_avg=%d, i=%d, i_avg=%d, ocv=%d, "
...
module_init_async(sm5705_fuelgauge_init);
