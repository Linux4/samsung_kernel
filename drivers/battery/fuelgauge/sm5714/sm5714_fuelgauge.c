...
static void sm5714_dump_all(struct sm5714_fuelgauge_data* fuelgauge) {
	int val;
	int i;
	int r_size = (sizeof(sm5714_srams)/sizeof(sm5714_srams[0]))/4;
	char temp_buf[660] = {0,};

	switch (sm5714_fg_debug_print%5) {
	case 0:
		for (i=0; i<sizeof(sm5714_regs)/sizeof(sm5714_regs[0]); i++) {
			val = -1;
...
static unsigned int sm5714_get_vbat(struct sm5714_fuelgauge_data *fuelgauge)
{
	...
	if ((sm5714_read_word(fuelgauge->i2c, SM5714_FG_REG_SYSTEM_STATUS) & 0x8000) || factory_mode) {
		if (fuelgauge->isjigmoderealvbat) {
			ret1 = sm5714_fg_read_sram(fuelgauge, SM5714_FG_ADDR_SRAM_VSYS);
			pr_info("%s : nENQ4 high JIG_ON, BUT need real VBAT 0x%x\n", __func__, ret1);
		} else {
			pr_info("%s : nENQ4 high JIG_ON, VBAT_REG 0x%x, VSYS_REG 0x%x\n", __func__,
				ret1, sm5714_fg_read_sram(fuelgauge, SM5714_FG_ADDR_SRAM_VSYS));
		}
	}
...
int sm5714_fg_calculate_iocv(struct sm5714_fuelgauge_data *fuelgauge, bool is_vsys)
{
...
/* init end */
	...
	for (i = roop_start; i < roop_start + roop_max; i++) {
		if (is_vsys)
			v_ret = sm5714_fg_read_sram(fuelgauge, i+0x10);
		else
...
void sm5714_set_aux_ctrl_cfg(struct sm5714_fuelgauge_data *fuelgauge)
{
	...
	sm5714_write_word(fuelgauge->i2c, SM5714_FG_REG_AUX_CTRL1, fuelgauge->info.aux_ctrl);

    rsmanvalue = sm5714_fg_read_sram(fuelgauge, SM5714_FG_ADDR_SRAM_RS_AUTO_MAN_VALUE);

	pr_info("%s: aux_ctrl = 0x%x, rsmanvalue = 0x%x\n", __func__, fuelgauge->info.aux_ctrl, rsmanvalue);
...
static bool sm5714_fg_reg_init(struct sm5714_fuelgauge_data *fuelgauge, bool is_surge)
{
	...
	if (is_surge) {
		...
		if ((sm5714_read_word(fuelgauge->i2c, SM5714_FG_REG_SYSTEM_STATUS) & 0x8000) ||
			factory_mode) {
			value = sm5714_fg_calculate_iocv(fuelgauge, true);
			/*
			...
			*/
			...
			value = sm5714_fg_calculate_iocv(fuelgauge, false);
	}
...
unsigned int sm5714_get_soc(struct sm5714_fuelgauge_data *fuelgauge)
{
	...
	if (ret < 0) {
		...
		soc = (ret * 10) >> 8;
		if ((ret > 0x63ff) && ((fuelgauge->info.batt_avgcurrent - fuelgauge->info.top_off) > (fuelgauge->info.top_off/5)))	{
			soc = soc - 1;
		}
	}
...
/*		break; */
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		pr_info("%s: POWER_SUPPLY_PROP_CAPACITY1 = %d \n", __func__, fuelgauge->info.batt_soc);
		...
		pr_info("%s: POWER_SUPPLY_PROP_CAPACITY2 = %d \n", __func__, val->intval);
		if (fuelgauge->pdata->capacity_calculation_type &
			(SEC_FUELGAUGE_CAPACITY_TYPE_SCALE |
			SEC_FUELGAUGE_CAPACITY_TYPE_DYNAMIC_SCALE)) {
			sm5714_fg_get_scaled_capacity(fuelgauge, val);
		}
		pr_info("%s: POWER_SUPPLY_PROP_CAPACITY3 = %d \n", __func__, val->intval);
			...
			*/
			...
			val->intval /= 10;
		pr_info("%s: POWER_SUPPLY_PROP_CAPACITY4 = %d \n", __func__, val->intval);
		...
		pr_info("%s: POWER_SUPPLY_PROP_CAPACITY5 = %d \n", __func__, val->intval);
			if (!fuelgauge->is_charging &&
		...
		pr_info("%s: POWER_SUPPLY_PROP_CAPACITY6 = %d %d\n", __func__, val->intval, fuelgauge->pdata->fuel_alert_soc);
		pr_info("%s: fuelgauge->is_fuel_alerted = %d\n", __func__, fuelgauge->is_fuel_alerted);
		...
		pr_info("%s: POWER_SUPPLY_PROP_CAPACITY7 = %d \n", __func__, val->intval);
			if (fuelgauge->pdata->capacity_calculation_type &
				(SEC_FUELGAUGE_CAPACITY_TYPE_ATOMIC |
				SEC_FUELGAUGE_CAPACITY_TYPE_SKIP_ABNORMAL)){
				sm5714_fg_get_atomic_capacity(fuelgauge, val);
			}
		pr_info("%s: POWER_SUPPLY_PROP_CAPACITY8 = %d \n", __func__, val->intval);
		break;
	/* Battery Temperature */
...
*/
	...
	ret = of_property_read_u32_array(np, prop_name, &fuelgauge->info.aux_ctrl, 1);
	if (ret < 0)
		PINFO("Can get prop %s (%d)\n", prop_name, ret);
	PINFO("%s = <0x%x>\n", prop_name, fuelgauge->info.aux_ctrl);
