...
#if defined(CONFIG_S2MU005_VOLT_MODE_TUNING)
		if(((fuelgauge->is_charging == true) && (value.intval >= 98) && (avg_current > 50)) || 
			((fuelgauge->is_charging == true) &&
			(avg_vbat > float_voltage) && (avg_current < check_current_level(fuelgauge) && (avg_current > 50)))) {
				if (fuelgauge->mode == CURRENT_MODE) { /* switch to VOLTAGE_MODE */
					fuelgauge->mode = HIGH_SOC_VOLTAGE_MODE;
