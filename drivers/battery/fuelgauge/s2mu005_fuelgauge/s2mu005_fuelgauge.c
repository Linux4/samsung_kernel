...
static void s2mu005_reset_fg(struct s2mu005_fuelgauge_data *fuelgauge)
{
...
#if defined(CONFIG_BATTERY_AGE_FORECAST)
	s2mu005_write_and_verify_reg_byte(fuelgauge->i2c, 0x0F,
		fuelgauge->age_data_info[fuelgauge->fg_age_step].batcap[0]);
	s2mu005_write_and_verify_reg_byte(fuelgauge->i2c, 0x0E,
		fuelgauge->age_data_info[fuelgauge->fg_age_step].batcap[1]);
	s2mu005_write_and_verify_reg_byte(fuelgauge->i2c, 0x11,
		fuelgauge->age_data_info[fuelgauge->fg_age_step].batcap[2]);
	s2mu005_write_and_verify_reg_byte(fuelgauge->i2c, 0x10,
...
#if defined(CONFIG_BATTERY_AGE_FORECAST)
	for (i = 0x92; i <= 0xe9; i++)
		s2mu005_write_and_verify_reg_byte(fuelgauge->i2c, i,
	 ...
	 * The code below restores the FG data version in 0x48 register to its initialized value.
	 */
...
static int s2mu005_get_rawsoc(struct s2mu005_fuelgauge_data *fuelgauge)
{
	...
	if (((fuelgauge->is_charging == true) && (value.intval >= 98) && (avg_current > 50)) ||
		...
		(avg_current < check_current_level(fuelgauge) && (avg_current > 50)))) {
		if (fuelgauge->mode == CURRENT_MODE) { /* switch to VOLTAGE_MODE */
			...
			dev_info(&fuelgauge->i2c->dev,
				"%s: FG is in high soc voltage mode\n", __func__);
		}
	...
	struct s2mu005_fuelgauge_data *fuelgauge)
{
	...
	if ((cnt < 10) && (curr < 0) && (fuelgauge->is_charging) && (vcell < 3500)) {
		curr = 1;
		cnt++;
...
static int s2mu005_set_adc_curroffset_In(struct s2mu005_fuelgauge_data *fuelgauge)
{
	u8 data[2];
...
static int s2mu005_set_adc_curroffset_out(struct s2mu005_fuelgauge_data *fuelgauge)
{
	u8 data[2];
	s32 coffset_old, coffset_new, coffset_old1, coffset_new1;
...
static int s2mu005_fuelgauge_parse_dt(struct s2mu005_fuelgauge_data *fuelgauge)
{
	...
	if (np == NULL) {
		...
		if (!np) {
			...
			ret = of_property_read_u32_array(np, "battery,soc_arr_evt2",
				fuelgauge->info.soc_arr_evt2, 22);
			if (ret < 0)
				pr_err("%s error reading battery,soc_arr_evt2\n", __func__);

			ret = of_property_read_u32_array(np, "battery,ocv_arr_evt2",
				fuelgauge->info.ocv_arr_evt2, 22);
			if (ret < 0)
				pr_err("%s error reading battery,ocv_arr_evt2\n", __func__);

			ret = of_property_read_u32_array(np, "battery,FG_Accumulative_rate_evt2",
				fuelgauge->info.fg_accumulative_rate_evt2, 2);
			if (ret < 0) {

				fuelgauge->info.fg_accumulative_rate_evt2[0] = 0x00; // REG 0x44
				fuelgauge->info.fg_accumulative_rate_evt2[1] = 0x08; // REG 0x45
				pr_err("%s no FG_Accumulative_rate value in DT. default (0x800)\n", __func__);
			}
#else
...
static const struct of_device_id s2mu005_fuelgauge_match_table[] = {
				...
				const struct i2c_device_id *id)
{
	...
	if (fuelgauge->pdata->fuel_alert_soc >= 0) {
		...
		wakeup_source_init(fuelgauge->fuel_alert_ws, "fuel_alerted");   // 4.19 R
