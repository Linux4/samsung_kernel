...
{
	pr_err("%s: [W/A] Maybe abnormal!! Re-calculate SOC\n", __func__);
...
static void s2mu005_init_regs(struct s2mu005_fuelgauge_data *fuelgauge)
{
	...
	s2mu005_write_reg_byte(fuelgauge->i2c, 0x41, 0x04); /* for get avg current */
}
				...
				const struct i2c_device_id *id)
{
	...
	/* [W/A] in case JIG: check if abnormal or not */
