...
static void __rt9455_charger_switch(struct i2c_client *client, int onoff)
{
	...
	if ((value.intval == SEC_BATTERY_CHARGING_2ND) || is_fully_charged)
		rt9455_enable_te(client, 0);
	else
		rt9455_enable_te(client, 1);
}
...
static int rt9455_chip_reg_init(struct i2c_client *client)
{
	struct sec_charger_info *charger = i2c_get_clientdata(client);
	//ctrl value
	rt9455_reg_write(client, RT9455_REG_CTRL2, 0x64);
	rt9455_reg_write(client, RT9455_REG_CTRL3, 0xA6);
	rt9455_reg_write(client, RT9455_REG_CTRL5, 0x3A);
...
static int rt9455_reset_register(struct i2c_client *client)
{
...
static int rt9455_get_charging_status(struct i2c_client *client)
{
	...
	if (is_fully_charged) {
		if (before_flag != is_fully_charged) {
			pr_info("%s: full charged. TE disable & Chrge enable\n",
				__func__);
...
static int rt9455_get_charging_health(struct i2c_client *client)
{
	...
	if (reg == 0x42) {
		rt9455_read_device(client, RT9455_REG_CTRL2, 1, &reg);
		if (reg & 0x02) {
			rt9455_clr_bits(client, RT9455_REG_CTRL2, 1<<1);
			pr_info("%s: HIZ - reg : 0x%x\n", __func__, reg);
		}
		pr_info("%s: abnormal state - health reg : 0x42\n", __func__);
		rt9455_reset_register(client);
		mdelay(100);
		rt9455_configure_charger(client);

		rt9455_read_device(client, RT9455_REG_CTRL1, 1, &reg);
		if (ret < 0) {
			pr_err("%s : Health state read failed\n", __func__);
			return ret;
		}
