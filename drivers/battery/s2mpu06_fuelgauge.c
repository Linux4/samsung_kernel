...
#if defined(CONFIG_BATTERY_AGE_FORECAST)
static unsigned int s2mpu06_get_offset_from_ocv(
		struct s2mpu06_fuelgauge_data *fuelgauge)
{
	u32 vcell;
	int i;
	...
	vcell = s2mpu06_get_vbat(fuelgauge);
	if (vcell >= fuelgauge->ocv_table[fuelgauge->offset_table_size - 1]) {
		offset = fuelgauge->offset_table[fuelgauge->offset_table_size - 1];
	} else {
		for (i = 0; i < fuelgauge->offset_table_size - 1; i++) {
			if ((fuelgauge->ocv_table[i] <= vcell) &&
				(fuelgauge->ocv_table[i+1] > vcell)) {
				offset = fuelgauge->offset_table[i];
				break;
			}
		}
	}
	pr_info("%s, vcell: %d, offset : %d \n", __func__,
		vcell, offset);
	return ((offset << 16) / 1000);
}

static void s2mpu06_reset_fg(struct s2mpu06_fuelgauge_data *fuelgauge)
{
	int i;
	unsigned int offset, dest;
	...
	offset = s2mpu06_get_offset_from_ocv(fuelgauge);
	...
	dest = ((reg31 << 16) & 0x0F0000) | ((reg30 << 8) & 0xFF00) | (reg2F & 0xFF);
	if ((dest > (0x7FFFF - offset)) && (dest < 0x80000))
		dest = 0x7FFFF;
	else
		dest = dest + offset;

	s2mpu06_write_reg(fuelgauge->i2c, 0x31, (dest >> 16) & 0x0F);
	s2mpu06_write_reg(fuelgauge->i2c, 0x30, (dest >> 8) & 0xFF);
	s2mpu06_write_reg(fuelgauge->i2c, 0x2F, dest & 0xFF);
			...
			const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_UPDATE_BATTERY_DATA:
		s2mpu06_fg_aging_check(fuelgauge, val->intval);
		break;
#endif
	default:
		return -EINVAL;
...
static int s2mpu06_fuelgauge_parse_dt(struct s2mpu06_fuelgauge_data *fuelgauge)
{
	...
	for (i = 0 ; i < fuelgauge->fg_num_age_step ; i++) {
		pr_err("%s: [Long life] age_step = %d, cell_character0[0] = %02x, cell_character1[0] = %02x\n",
			__func__, i, fuelgauge->age_data_info[i].cell_character0[0],
			fuelgauge->age_data_info[i].cell_character1[0]);
	}

	of_get_property(np, "fuelgauge,ocv_table", &len);
	fuelgauge->offset_table_size = len/sizeof(int);
	fuelgauge->ocv_table = kzalloc(len, GFP_KERNEL);
	fuelgauge->offset_table = kzalloc(len, GFP_KERNEL);
	ret = of_property_read_u32_array(np, "fuelgauge,ocv_table",
			(int *)fuelgauge->ocv_table, len/sizeof(int));
	ret = of_property_read_u32_array(np, "fuelgauge,offset_table",
			(int *)fuelgauge->offset_table, len/sizeof(int));

	for (i = 0 ; i < fuelgauge->offset_table_size ; i++) {
		pr_err("%s: [Long life] ocv_table[%d] = %d, offset_table[%d] = %d\n",
			__func__, i, fuelgauge->ocv_table[i], i, fuelgauge->offset_table[i]);
	}
#endif
