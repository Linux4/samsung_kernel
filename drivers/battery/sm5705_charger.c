				...
				unsigned short input_current)
{
	...
	if (charger->hv_overheating) {
			input_current = (input_current > charger->pdata->hv_overheat_input_current_limit) ?
				charger->pdata->hv_overheat_input_current_limit : input_current;
	}
				...
				enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		...
		break;
    case POWER_SUPPLY_PROP_HV_OVERHEAT:
		pr_info("%s hv_overheating(%d)\n",__func__,val->intval);
		charger->hv_overheating = val->intval;
		sm5705_set_input_current(charger, _calc_input_limit_current_with_siop(charger));		
		break;
...
static int _parse_battery_node_propertys(struct device *dev, struct device_node *np, sec_charger_platform_data_t *pdata)
{
	...
	ret = of_property_read_u32(np, "battery,hv_overheat_input_current_limit", &pdata->hv_overheat_input_current_limit);
	if (ret) {
