...
static void psy_chg_set_cable_online_in_CHG_OFF(struct sm5705_charger_data *charger, int cable_type, int chg_oper_status)
{
	...
	case POWER_SUPPLY_TYPE_HV_PREPARE_MAINS:
		dev_err(charger->dev, "%s: can't set to HV_MAIN, invalid sequence\n", __func__);
		break;
...
static void _reduce_input_current(struct sm5705_charger_data *charger, int cur)
{
...
static void sm5705_aicl_work(struct work_struct *work)
{
	...
	if ((!sm5705_charger_get_charging_on_status(charger)) || (!__n_is_cable_type_for_wireless(charger->cable_type))) {
...
static void sm5705_charger_initialize(struct sm5705_charger_data *charger)
{
	...
	sm5705_CHG_set_BST_IQ3LIMIT(charger, SM5705_CHG_BST_IQ3LIMIT_2_0A);
