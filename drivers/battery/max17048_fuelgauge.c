...
u16 model_data_active_1[8] = {
		0x90A7, 0x20B7, 0xB0B8, 0x80BA,
		0xC0BB, 0x90BC, 0x30BD, 0x00BF
};
u16 model_data_active_2[8] = {
		0x20C0, 0xC0C1, 0x40C4, 0xB0C8,
		0x40CC, 0x90CE, 0x40D2, 0x20D8
};
u16 model_data_active_3[8] = {
		0xE001, 0xC026, 0x001A, 0xC01E,
		0x4044, 0xC056, 0x001E, 0x8035
};
u16 model_data_active_4[8] = {
		0xA01B, 0x600F, 0xC014, 0x200F,
		0x6011, 0x000C, 0x0010, 0x0010
};
u16 model_rcomp_seg[8] = {0x0100, 0x0100, 0x0100, 0x0100, 0x0100, 0x0100, 0x0100, 0x0100};
#endif
...
#if defined(CONFIG_SEC_K_PROJECT) || defined(CONFIG_SEC_S_PROJECT) || defined(CONFIG_MACH_CHAGALL_KDI)
...
#elif defined(CONFIG_MACH_CHAGALL_KDI)
	if ((read_soc[1] >= 0xF0) && (read_soc[1] <= 0xF2)) {
#endif
...
static void fg_read_address(struct i2c_client *client)
{
	...
	sprintf(str + strlen(str), "temp(%d), ", value.intval);
	str[strlen(str)] = '\0';
	dev_info(&client->dev, "%s\n", str);
}
#endif
