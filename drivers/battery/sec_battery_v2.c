	...
	if (battery->status == POWER_SUPPLY_STATUS_FULL &&
		...
		if (battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP) {
			...
			recharging_voltage =\
				...
				battery->pdata->chg_float_voltage_conv) - 150 + battery->pdata->swelling_offset_voltage;
...
static void sec_bat_calculate_safety_time(struct sec_battery_info *battery)
{
	...
	expired_time = (expired_time * battery->pdata->standard_curr);
	...
	expired_time = (expired_time * curr);
...
//	}
	if (battery->cable_type == POWER_SUPPLY_TYPE_HV_MAINS_12V || \
		battery->cable_type == POWER_SUPPLY_TYPE_HV_QC30)
		voltage = SEC_INPUT_VOLTAGE_12V;
	else if (battery->cable_type == POWER_SUPPLY_TYPE_HV_ERR || \
			battery->cable_type == POWER_SUPPLY_TYPE_HV_MAINS || \
...
/* s2mu005 : CHG 0xAF[7]=1 for SMPL issue, 0xAF[7]=0 for JIG case */
	...
	dev_err(battery->dev, "%s : 0xAF work-around execute! (%d)\n", __func__, val.intval);
#endif
		...
		struct sec_battery_info *battery)
{
	...
	ret = of_property_read_u32(np, "battery,swelling_offset_voltage",
		&pdata->swelling_offset_voltage);
	if (ret) {
		pdata->swelling_offset_voltage = 0;
		pr_info("%s : Swelling offset voltage is Empty\n", __func__);
	}
