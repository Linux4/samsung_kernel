...
static int smb327_get_power_status(struct i2c_client *client)
{
	uint8_t reg;
	int ret;

	ret = smb327_i2c_read(client, SMB327_BATTERY_CHARGING_STATUS_C, &reg);
	if (ret < 0) {
...
static int smb327_get_charge_now(struct i2c_client *client)
{
	...
	for (addr = 0; addr < 0x0c; addr++) {
		smb327_i2c_read(client, addr, &data);
		pr_debug("smb327 addr : 0x%02x data : 0x%02x\n", addr, data);
	}
	for (addr = 0x31; addr <= 0x3D; addr++) {
		smb327_i2c_read(client, addr, &data);
		pr_debug("smb327 addr : 0x%02x data : 0x%02x\n", addr, data);
			      ...
			      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_POWER_STATUS:
		val->intval = smb327_get_power_status(client);
		break;
#endif		
	default:
		return false;
			      ...
			      const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		...
		smb327_set_fast_charging_current(client, current_now);
		smb327_set_writable(client, 0);
		break;
	/* val->intval : type */
