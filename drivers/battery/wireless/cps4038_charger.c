...
static void mfc_fod_set(struct mfc_charger_data *charger, int fod_state)
{
	...
	int i;
	int fod_state_hv = FOD_STATE_HV_CC;
	...
	if (fod_state == FOD_STATE_CC)
		fod_state_hv = FOD_STATE_HV_CC;
	else if (fod_state == FOD_STATE_CV)
		fod_state_hv = FOD_STATE_HV_CV;
	else /* fod_state == FOD_STATE_FULL */
		fod_state_hv = FOD_STATE_FULL;
...
static void mfc_set_rx_phm(struct mfc_charger_data *charger, int state)
{
	union power_supply_propval value = {0, };
	...
	if (!is_phm_supported_pad(charger) && state == ENTER_PHM) {
		pr_info("%s: phm unsupported pad\n", __func__);
		return;
	}
...
static void cps4038_print_fod(struct mfc_charger_data *charger)
{
	...
	for (i = 0; i < MFC_NUM_FOD_REG; i++) {
		...
		if (mfc_reg_read(charger->client, MFC_WPC_FWC_FOD_0A_REG+i, &fod_fwc[i]) < 0)
			pr_info("%s: failed to read FOD_FWC(%d) data\n", __func__, i);
	}

	snprintf(fod_str, sizeof(fod_str), "%s: FOD0(", __func__);
	snprintf(fwc_str, sizeof(fwc_str), "%s: FOD0_FWC(", __func__);
	for (i = 0; i < MFC_NUM_FOD_REG; i++) {
		snprintf(fod_str + strlen(fod_str), sizeof(fod_str) - strlen(fod_str), " 0x%02X", fod[i]);
		snprintf(fwc_str + strlen(fwc_str), sizeof(fwc_str) - strlen(fwc_str), " 0x%02X", fod_fwc[i]);
		if (i == (MFC_NUM_FOD_REG / 2 - 1)) {
			snprintf(fod_str + strlen(fod_str), sizeof(fod_str) - strlen(fod_str), ")\n");
			snprintf(fwc_str + strlen(fwc_str), sizeof(fwc_str) - strlen(fwc_str), ")\n");
			snprintf(fod_str + strlen(fod_str), sizeof(fod_str) - strlen(fod_str), "%s: FOD1(", __func__);
			snprintf(fwc_str + strlen(fwc_str), sizeof(fwc_str) - strlen(fwc_str), "%s: FOD1_FWC(", __func__);
		}
	}

	sprintf(fod_str + strlen(fod_str), ")\n");
	sprintf(fwc_str + strlen(fwc_str), ")\n");
	pr_info("%s", fod_str);
	pr_info("%s", fwc_str);
}
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CALL_EVENT:
			if (val->intval & BATT_EXT_EVENT_CALL) {
				...
				mfc_set_rx_phm(charger, ENTER_PHM);
#else
				...
				if (charger->device_event & BATT_EXT_EVENT_CALL) {
					charger->device_event &= ~BATT_EXT_EVENT_CALL;
#if defined(CONFIG_WIRELESS_RX_PHM_CTRL)
					mfc_set_rx_phm(charger, EXIT_PHM);
#endif
				}
			}
			break;
		case POWER_SUPPLY_EXT_PROP_RX_PHM:
			mfc_set_rx_phm(charger, val->intval);
			break;
...
static void mfc_activate_work_content(struct mfc_charger_data *charger)
{
	...
	/* fw default : CMA/CMB both enable */
	if (charger->pdata->unknown_cmb_ctrl)
		mfc_cma_cmb_onoff(charger, true, true);
	else
		mfc_cma_cmb_onoff(charger, true, false);
	...
	if (!charger->tx_id && !charger->is_abnormal_pad) {
		...
		if (charger->is_abnormal_pad) {
			...
			if ((charger->tx_id == TX_ID_P1100_PAD) ||
				...
				((charger->tx_id == TX_ID_P5200_PAD) && !charger->pdata->unknown_cmb_ctrl)) {
				mfc_cma_cmb_onoff(charger, true, true);
...
static irqreturn_t mfc_wpc_pdet_b_irq_thread(int irq, void *irq_data)
{
	...
	if (charger->pdetb_state != pdetb_state) {
		pr_info("%s: pdet_b(%d to %d)\n", __func__, charger->pdetb_state, pdetb_state);
		charger->pdetb_state = pdetb_state;
	}

	if (charger->rx_phm_status && !det_state && pdetb_state) {
		pr_info("%s wireless charger deactivated during phm\n", __func__);
		mfc_set_rx_phm(charger, END_PHM);
...
{
	const char *fod_state_string[FOD_STATE_MAX] = {"cc", "cv", "full", "hv_cc", "hv_cv"};
