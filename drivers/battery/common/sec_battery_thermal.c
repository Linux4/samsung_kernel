 ...
 * Samsung Mobile Battery Driver, battery temperature handling file
 *
	...
	if (pre_thermal_zone != battery->thermal_zone) {
		...
		case BAT_THERMAL_WARM:
			if (battery->health == POWER_SUPPLY_HEALTH_GOOD) {
				...
				} else if ((voter_swelling_status == SEC_BAT_CHG_MODE_CHARGING_OFF) || (battery->status == POWER_SUPPLY_STATUS_FULL)) {
