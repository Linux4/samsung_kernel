...
__visible_for_testing void sec_bat_divide_limiter_current(struct sec_battery_info *battery, int limiter_current)
{
	...
	if (battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP_SWELLING_COOL3) {
		if (is_wireless_all_type(battery->cable_type)) {
			if (main_current > battery->pdata->limiter_main_wireless_warm_current)
				main_current = battery->pdata->limiter_main_wireless_warm_current;
			if (sub_current > battery->pdata->limiter_sub_wireless_warm_current)
				sub_current = battery->pdata->limiter_sub_wireless_warm_current;
		} else {
...
int sec_bat_set_charge(void * data, int chg_mode)
{
	...
	if ((battery->pdata->full_check_type_2nd == SEC_BATTERY_FULLCHARGED_FG_CURRENT) &&
		 ...
		 */
		val.intval = DUAL_BATTERY_CHG_OFF;
		...
		if (chg_mode == SEC_BAT_CHG_MODE_BUCK_OFF)
			...
			val.intval = DUAL_BATTERY_CHG_ON;
...
bool sec_bat_check_full(struct sec_battery_info *battery, int full_check_type)
{
	...
	case SEC_BATTERY_FULLCHARGED_LIMITER:
		...
		break;
#ifdef CONFIG_IFPMIC_LIMITER
	case SEC_BATTERY_FULLCHARGED_VCELL:
		voltage = max(battery->voltage_now_main, battery->voltage_now_sub);

		if (voltage > battery->pdata->high_temp_float) {
			battery->full_check_cnt++;
			dev_info(battery->dev, "%s: Full Check Vcell (%d)\n",
				__func__, battery->full_check_cnt);
		} else {
			battery->full_check_cnt = 0;
		}
...
#ifdef CONFIG_IFPMIC_LIMITER
bool sec_bat_check_fullcharged_warm(struct sec_battery_info *battery)
{
	sec_bat_enable_eoc_check(battery);
	...
	return sec_bat_check_full(battery, SEC_BATTERY_FULLCHARGED_VCELL);
}
