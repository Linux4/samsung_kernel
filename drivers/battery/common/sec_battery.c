...
static void sec_bat_check_mix_temp(struct sec_battery_info *battery)
{
...
static void sec_bat_check_wpc_temp(struct sec_battery_info *battery)
{
	int input_current = INT_MAX, charging_current = INT_MAX;
...
#if defined(CONFIG_DIRECT_CHARGING)
static void sec_bat_check_direct_chg_temp(struct sec_battery_info *battery)
{
...
static void sec_bat_check_afc_temp(struct sec_battery_info *battery)
{
	int input_current = 0, charging_current = 0;
...
static void sec_bat_check_pdic_temp(struct sec_battery_info *battery)
{
		...
		struct sec_battery_info *battery, int health)
{
	...
	if (health == POWER_SUPPLY_HEALTH_DC_ERR) {
		...
		sec_vote(battery->chgen_vote, VOTER_VBUS_OVP, true, SEC_BAT_CHG_MODE_CHARGING);
	}
#endif
...
static int make_pd_list(struct sec_battery_info *battery)
{
	...
	if (battery->pdic_info.sink_status.selected_pdo_num == battery->pd_list.pd_info[pd_list_select].pdo_index) {
		...
		sec_bat_set_current_event(battery, SEC_BAT_CURRENT_EVENT_SELECT_PDO,
			SEC_BAT_CURRENT_EVENT_SELECT_PDO);
		sec_vote(battery->input_vote, VOTER_AICL, true, input_current);
