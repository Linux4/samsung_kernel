...
__visible_for_testing int sec_dual_check_eoc_status(struct sec_dual_battery_info *battery)
{
	...
	battery->main_voltage = value.intval;

	value.intval = SEC_BATTERY_CURRENT_MA;
	psy_do_property(battery->pdata->main_limiter_name, get,
		(enum power_supply_property)POWER_SUPPLY_EXT_PROP_CHG_CURRENT, value);
	battery->main_current = value.intval;

	pr_info("%s [Main] %dmV %dmA\n", __func__, battery->main_voltage, battery->main_current);

#if defined(CONFIG_DUAL_BATTERY_CELL_SENSING)
	if (((battery->main_voltage >= battery->pdata->main_full_condition_vcell[battery->age_step] ||
		battery->main_current <= battery->pdata->main_full_condition_eoc) &&
		!(battery->full_total_status & SEC_DUAL_BATTERY_MAIN_CONDITION_DONE) &&
		battery->pdata->main_cell_sensing) ||
		((battery->main_voltage >= battery->pdata->main_full_condition_vcell[battery->age_step] &&
		battery->main_current <= battery->pdata->main_full_condition_eoc) &&
		!(battery->full_total_status & SEC_DUAL_BATTERY_MAIN_CONDITION_DONE) &&
		!battery->pdata->main_cell_sensing))
#else
	...
	psy_do_property(battery->pdata->sub_limiter_name, get,
		(enum power_supply_property)POWER_SUPPLY_EXT_PROP_BAT_VOLTAGE, value);
	battery->sub_voltage = value.intval;

	value.intval = SEC_BATTERY_CURRENT_MA;
	psy_do_property(battery->pdata->sub_limiter_name, get,
		(enum power_supply_property)POWER_SUPPLY_EXT_PROP_CHG_CURRENT, value);
	battery->sub_current = value.intval;

	pr_info("%s [Sub] %dmV %dmA\n", __func__, battery->sub_voltage, battery->sub_current);

#if defined(CONFIG_DUAL_BATTERY_CELL_SENSING)
	if (((battery->sub_voltage >= battery->pdata->sub_full_condition_vcell[battery->age_step] ||
		battery->sub_current <= battery->pdata->sub_full_condition_eoc) &&
		!(battery->full_total_status & SEC_DUAL_BATTERY_SUB_CONDITION_DONE) &&
		battery->pdata->main_cell_sensing) ||
		((battery->sub_voltage >= battery->pdata->sub_full_condition_vcell[battery->age_step] &&
		battery->sub_current <= battery->pdata->sub_full_condition_eoc) &&
		!(battery->full_total_status & SEC_DUAL_BATTERY_SUB_CONDITION_DONE) &&
		!battery->pdata->main_cell_sensing))
#else
...
static int sec_dual_check_each_batterys_eoc(struct sec_dual_battery_info *battery)
{
	...
	if ((battery->main_current <= battery->pdata->main_full_condition_eoc)
		&& (battery->main_voltage >= battery->pdata->main_full_condition_vcell[battery->age_step])) {
	...
	if ((battery->sub_current <= battery->pdata->sub_full_condition_eoc)
		&& (battery->sub_voltage >= battery->pdata->sub_full_condition_vcell[battery->age_step])) {
...
{
	struct sec_dual_battery_info *battery = power_supply_get_drvdata(psy);
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		val->intval = sec_dual_check_each_batterys_eoc(battery);
		break;
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_DUAL_BAT_DET:
			if (value.intval == SEC_DUAL_BATTERY_MAIN) {
				if (battery->pdata->main_bat_det_gpio) {
					val->intval = !gpio_get_value(battery->pdata->main_bat_det_gpio);
					pr_info("%s : main det(%d) = %d\n", __func__, battery->pdata->main_bat_det_gpio, val->intval);
				} else
					val->intval = -1;
			} else if (value.intval == SEC_DUAL_BATTERY_SUB) {
				if (battery->pdata->sub_bat_det_gpio) {
					val->intval = !gpio_get_value(battery->pdata->sub_bat_det_gpio);
					pr_info("%s : sub det(%d) = %d\n", __func__, battery->pdata->sub_bat_det_gpio, val->intval);
				} else
					val->intval = -1;
			}
			break;
		...
		struct sec_dual_battery_info *battery)
{
	...
	if (ret)
		...
		if (!np) {
			...
			ret = pdata->main_bat_det_gpio = of_get_named_gpio(np, "limiter,main_bat_det_gpio", 0);
			if (ret < 0)
				pr_info("%s : can't get main_bat_det_gpio\n", __func__);
		}
	}
	np = dev->of_node;
	...
	if (ret)
		...
		if (!np) {
			...
			/* SUB_BATTERY_CON_DET */
			ret = pdata->sub_bat_det_gpio = of_get_named_gpio(np, "limiter,sub_bat_det_gpio", 0);
			if (ret < 0)
				pr_info("%s : can't get sub_bat_det_gpio\n", __func__);
		}
	}
#if defined(CONFIG_DUAL_BATTERY_CELL_SENSING)
...
#ifdef CONFIG_OF
static const struct of_device_id sec_dual_battery_dt_ids[] = {
