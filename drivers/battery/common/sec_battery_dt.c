		...
		struct sec_battery_info *battery)
{
	...
	ret = of_property_read_u32(np, "battery,chg_vbus_control_after_fullcharged",
		&pdata->chg_vbus_control_after_fullcharged);
	if (ret)
		pr_info("%s : chg_vbus_control_after_fullcharged is Empty\n", __func__);
