				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case BATT_INBAT_VOLTAGE_OCV:
		if (battery->pdata->support_fgsrc_change == true) {
			...
			if (battery->is_jig_on || read_cmdline(factory_mode) ||	battery->factory_mode || jig_on) {
					...
					const char *buf, size_t count)
{
	...
	case BATT_CAPACITY_MAX:
		if (sscanf(buf, "%10d\n", &x) == 1) {
			dev_err(battery->dev,
					"%s: BATT_CAPACITY_MAX(%d), fg_reset(%d)\n", __func__, x, read_cmdline(fg_reset));
			if (!read_cmdline(fg_reset) && !battery->store_mode) {
				value.intval = x;
				psy_do_property(battery->pdata->fuelgauge_name, set,
						POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN, value);
