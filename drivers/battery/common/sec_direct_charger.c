...
inline void sec_direct_wakeup_source_init(struct device *dev, struct wakeup_source **ws, const char *name)
{
	if (!(*ws)) {
...
static int sec_direct_chg_check_charging_source(struct sec_direct_charger_info *charger)
{
	...
	if (value.intval & SEC_BAT_CURRENT_EVENT_SWELLING_MODE ||
			...
			value.intval & SEC_BAT_CURRENT_EVENT_SELECT_PDO) {
			    ...
			    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (is_pd_apdo_wire_type(charger->cable_type)) {
			...
			if (charger->charging_source == SEC_DIRECT_CHG_CHARGING_SOURCE_DIRECT) {
				sec_direct_chg_set_charging_source(charger,
					charger->charger_mode, SEC_DIRECT_CHG_CHARGING_SOURCE_SWITCHING);
			}
			value.intval = 0;
			psy_do_property(charger->pdata->direct_charger_name, set,
				psp, value);
...
static int sec_direct_charger_probe(struct platform_device *pdev)
{
	...
	sec_direct_wakeup_source_init(&pdev->dev, &charger->dc_ws, "sec-direct-source");
...
err_pdata_free:
	wakeup_source_unregister(charger->dc_ws);
