/*
 * sec_adc.c
 * Samsung Mobile Battery Driver, adc manipulation file
 *
...
static int sec_bat_adc_ap_read(struct sec_battery_info *battery, int channel)
{
	...
	switch (channel)
	{
		...
		{
			int ret = 0, data_table[4], ret_value = 0;
			ret = IMM_GetOneChannelValue(battery->pdata->bat_thm_adc_channel, data_table, &ret_value);
			if (ret < 0)
				dev_info(battery->dev, "%s: Cannot read BATT ADC channel\n", __func__);
