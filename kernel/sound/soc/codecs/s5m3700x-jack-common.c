...
#else
		ret = snd_soc_card_jack_new(codec->card, "S5M3700X Headset Input", type, &jackdet->headset_jack);
#endif
		if (ret) {
...
#else
		ret = snd_soc_card_jack_new(codec->card, "S5M3700X Headset Button", type, &jackdet->headset_button);
#endif
		if (ret) {
