...
static void s5m3700x_i2c_parse_dt(struct s5m3700x_priv *s5m3700x)
{
	...
	if (s5m3700x->external_ldo_3_4v < 0)
	 ...
	 /* 3.4V external_ldo gpio */
	s5m3700x->external_ldo_1_2v = of_get_named_gpio(np, "s5m3700x-external-ldo-1-2v", 0);
	if (s5m3700x->external_ldo_1_2v < 0)
		dev_err(dev, "%s: cannot find 1.2V external_ldo gpio in the dt\n", __func__);
	else
		dev_info(dev, "%s: 1.2V external_ldo gpio = %d\n",
				__func__, s5m3700x->external_ldo_1_2v);
...
static void s5m3700x_power_configuration(struct s5m3700x_priv *s5m3700x)
{
	...
	/* Setting 1.2V External LDO gpio */
	if (s5m3700x->external_ldo_1_2v > 0) {
		if (gpio_request(s5m3700x->external_ldo_1_2v, "s5m3700x_external_ldo_1_2v") < 0)
			dev_err(s5m3700x->dev, "%s: Request for %d GPIO failed\n", __func__, (int)s5m3700x->external_ldo_1_2v);
			/* turn off for default */
		if (gpio_direction_output(s5m3700x->external_ldo_1_2v, 0) < 0)
			dev_err(s5m3700x->dev, "%s: GPIO direction to output failed!\n", __func__);
	}
...
#endif
		if (s5m3700x->external_ldo_1_2v >= 0)
			if (gpio_direction_output(s5m3700x->external_ldo_1_2v, 1) < 0)
				dev_err(s5m3700x->dev, "%s: 1.2V external_ldo GPIO direction to output failed!\n", __func__);
		...
		const struct i2c_device_id *id)
{
	...
	dev_info(&i2c->dev, "Codec I2C Probe: (%s) name: %s, size: %lu, i2c addr: 0x%02x\n",
...
static void s5m3700x_i2c_remove(struct i2c_client *i2c)
{
