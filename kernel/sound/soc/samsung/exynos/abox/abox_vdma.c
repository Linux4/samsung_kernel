...
static unsigned long abox_vdma_get_iova(int id, int stream)
{
	...
	default:
		ret = 0;
		pr_err("%s, failed to get the vdma_%d iova\n", __func__, id);
		break;
	}
		...
		struct snd_compr_tstamp *tstamp)
{
	...
	if (pcm_size) {
		...
		abox_dbg(dev, "%s: pcm(%u), frame(%u), total_avail(%llu), total_trans(%llu)\n",
		...
		struct snd_pcm_hw_params *params)
{
	...
	abox_dbg(dev, "%s[%c]\n", __func__, substream_to_char(substream));
	if (rtd->buffer.bytes < buffer_bytes) {
		abox_iommu_unmap(abox_vdma_dev_abox, rtd->iova);
		snd_dma_free_pages(&rtd->buffer);
		ret = snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV, dev,
			PAGE_ALIGN(buffer_bytes), &rtd->buffer);
		if (ret < 0)
			return ret;

		ret = abox_iommu_map(abox_vdma_dev_abox, rtd->iova,
				rtd->buffer.addr,
				rtd->buffer.bytes,
				rtd->buffer.area);
		if (ret < 0)
	...
	snd_pcm_set_runtime_buffer(substream, &rtd->buffer);
		...
		struct snd_pcm_hardware *capture)
{
	...
	if (playback) {
		...
		if (id == VPCM_DEEP_ID)
			rtd->buffer.bytes = BUFFER_BYTES_DEEP;
		else
			rtd->buffer.bytes = BUFFER_BYTES_MIN;
	}
