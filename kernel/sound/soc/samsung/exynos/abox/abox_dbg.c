		...
		size_t new_size, const char *tag)
{
	...
	abox_info(dev, "%s: %s new size %#llx\n", __func__, tag, rmem->size);
}
...
static int abox_dbg_dump_create_file(struct abox_data *data)
{
	...
	if (p_abox_dbg_dump) {
		...
		for (i = 0; i < abox_dbg_dump_count; i++) {
			...
			if (!dir) {
				abox_err(data->dev, "Failed to create /proc/abox/%s\n", dir_name);
				return -ENOMEM;
			}
			...
			abox_proc_create_bin("log", 0440, dir, &info->log);
		...
		for (i = 0; i < abox_dbg_dump_count; i++) {
			...
			abox_proc_create_bin("log-01", 0440, dir, &info->log_01);
...
static BIN_ATTR(calliope_dram, 0440, calliope_dram_read, NULL,
		DRAM_FIRMWARE_SIZE);
static BIN_ATTR(calliope_log, 0440, calliope_log_read, NULL, ABOX_LOG_SIZE);
		...
		void __iomem *io_base, void *base, size_t size)
{
	...
	info->base = base;
	info->size = size;
	info->file = abox_proc_create_file(info->name, 0440, parent, info->fops,
			info, info->size);
...
void abox_dbg_create(struct abox_data *data)
{
	...
	if (dev == NULL) {
		abox_err(data->dev, "%s : abox debug not probe\n", __func__);
		return;
	}
