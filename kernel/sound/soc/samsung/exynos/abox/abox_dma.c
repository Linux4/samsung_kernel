...
static int abox_dma_enable_check(struct snd_soc_component *cmpnt)
{
	unsigned int val;

	val = snd_soc_component_read(cmpnt, DMA_REG_CTRL);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (abox_dma_progress(cmpnt)) {
		...
		const int wait_ns = 5000000; /* 5ms */
		if (item[0] == 0) { // Normal set
			limit = local_clock() + wait_ns;
			while(!abox_dma_enable_check(cmpnt)) {
				usleep_range(100, 200);
				if (local_clock() > limit) {
					abox_info(dev, "%s: timeout\n", __func__);
					break;
				}
			}
		}
...
static int abox_dma_dump_set_format(struct abox_dma_data *data)
{
	...
	switch (w) {
	case ABOX_WIDGET_SPUS_IN0 ... ABOX_WIDGET_SPUS_IN15:
	...
	case ABOX_WIDGET_SIFS0:
		...
		break;
	case ABOX_WIDGET_SPUS_SSRC0 ... ABOX_WIDGET_SPUS_SSRC3:
		idx = w - ABOX_WIDGET_SPUS_SSRC0;
		format = abox_cmpnt_asrc_get_dst_format(abox_data,
				SNDRV_PCM_STREAM_PLAYBACK, idx);
		break;
...
static enum abox_widget abox_ddma_get_src_widget(struct abox_dma_data *data)
{
	...
	case 0x18:
		...
		break;
	case 0x19 ... 0x1c:
		w = ABOX_WIDGET_SPUS_SSRC0 + val - 0x19;
		break;
	case 0x20 ... 0x27:
...
static const char * const abox_ddma_src_enum_texts[] = {
	...
	"SPUS_SSRC0", "SPUS_SSRC1", "SPUS_SSRC2", "SPUS_SSRC3",
