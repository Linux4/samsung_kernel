...
void abox_log_register_buffer_work_func(struct work_struct *work)
{
	...
	list_for_each_entry_safe(_info, n, &abox_log_register_head, list) {
		...
		abox_proc_create_file(name, 0660, NULL, &abox_log_fops,
				info, 0);
		kfree(name);
		...
		struct ABOX_LOG_BUFFER *buffer)
{
	...
	if (!abox_log_check_sanity(buffer)) {
		abox_info(dev, "%s(%d): index_writer=%u, index_reader=%u, size=%u\n",
				__func__, id, buffer->index_writer,
				buffer->index_reader, buffer->size);
		abox_failsafe_report(dev, true);
	}
