...
static int cs35l43_link_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (rtd->dai_link->num_codecs> ARRAY_SIZE(tx_channel_map))
		return -EINVAL;
	if (rtd->dai_link->num_codecs > ARRAY_SIZE(rx_channel_map))
		return -EINVAL;
...
static int exynos_uaif3_hw_params(struct snd_pcm_substream *substream,
			       struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = asoc_substream_to_rtd(substream);
...
static const struct snd_soc_ops uaif3_ops = {
	.hw_params = exynos_uaif3_hw_params,
};
...
static int exynos_late_probe(struct snd_soc_card *card)
{
	...
	rtd = get_rtd(card, CS35L43_DAI_ID);
	if (rtd != NULL) {
		amp_dai = asoc_rtd_to_codec(rtd, 0);
		amp = amp_dai->component;

		dapm = snd_soc_component_get_dapm(amp);
		snd_soc_dapm_ignore_suspend(dapm, "VINPUT");
		snd_soc_dapm_sync(dapm);
	}
...
static struct snd_soc_dai_link exynos_dai[100] = {
...
#if defined(CONFIG_VTS_INTERNAL_REC)
	{
		.name = "VTS-Internal",
	...
	{
		.name = "RDMA12 BE",
		.stream_name = "RDMA12 BE",
		.id = ABOX_BE_DAI_ID(0, 12),
		.no_pcm = 1,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		.be_hw_params_fixup = abox_hw_params_fixup_helper,
	...
	{
		.name = "RDMA13 BE",
		.stream_name = "RDMA13 BE",
		.id = ABOX_BE_DAI_ID(0, 13),
		.no_pcm = 1,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		.be_hw_params_fixup = abox_hw_params_fixup_helper,
	...
	{
		.name = "RDMA14 BE",
		.stream_name = "RDMA14 BE",
		.id = ABOX_BE_DAI_ID(0, 14),
		.no_pcm = 1,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		.be_hw_params_fixup = abox_hw_params_fixup_helper,
	...
	{
		.name = "RDMA15 BE",
		.stream_name = "RDMA15 BE",
		.id = ABOX_BE_DAI_ID(0, 15),
		.no_pcm = 1,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		.be_hw_params_fixup = abox_hw_params_fixup_helper,
...
static int exynos_sound_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node(np, dai) {
		...
		if (!link->codecs) {
			...
			if (link->codecs) {
				if (strstr(link->codecs[0].dai_name, "cs35l43")) {
					link->id = CS35L43_DAI_ID;
					link->init = cs35l43_link_init;
