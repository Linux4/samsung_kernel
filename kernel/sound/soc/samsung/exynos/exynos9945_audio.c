...
#if IS_ENABLED(CONFIG_SND_SOC_TFA9878)
const int tfa_addr[2] = {0x35, 0x34};
const int tfa_addr_len = 2;
int tfa_i2c_fail_callback(int addr, int err, int rw, int cnt)
{
	int i, amp_id = -1;

	for (i = 0; i < tfa_addr_len; i++)
		if (tfa_addr[i] == addr) {
			amp_id = i;
			break;
		}
	pr_info("%s addr: 0x%02x, amp_id: %d, %c, %d\n", __func__, addr, amp_id, rw ? 'w' : 'r', err);
#if IS_ENABLED(CONFIG_SND_SOC_SAMSUNG_AUDIO)
...
static int uaif3_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if ((component) && strstr(component->name, "tfa98xx")) {
		...
		tfa_i2c_err_register(tfa_i2c_fail_callback);
	}
#endif
