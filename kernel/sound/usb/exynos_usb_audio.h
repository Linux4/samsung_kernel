...
#define USB_AUDIO_XHCI_BASE	0x17900000
#define USB_URAM_BASE		0x17A00000
#define USB_URAM_SIZE		SZ_1K * 24
#define EXYNOS_URAM_ABOX_EMPTY_ADDR	0x17a03D00
#define EMPTY_SIZE			128
...
struct exynos_usb_audio {
	...
	int is_audio;
	int is_first_probe;
	atomic_t indeq_map_done;
	atomic_t outdeq_map_done;
	atomic_t fb_indeq_map_done;
	atomic_t fb_outdeq_map_done;
	atomic_t pcm_open_done;
	u32 usb_audio_state;
...
void exynos_usb_audio_connect(void *data, struct usb_interface *intf, struct snd_usb_audio *chip);
void exynos_usb_audio_disconnect(void *data, struct usb_interface *intf);
...
int exynos_usb_audio_set_pcmintf(struct usb_interface *intf,
				 int iface, int alt, int direction, struct snd_usb_substream *subs);
