...
{
	return tps->num_initialized == PROBE_TRACEPOINTS;
}
...
static int __init trace_runqlat_init(void)
{
	...
	if (!proc_create_data("pid", 0644,
				parent_dir, &trace_pid_fops, info))
		goto remove_proc;

	if (!proc_create_data("threshold", 0644,
				parent_dir, &threshold_fops, info))
		goto remove_proc;

	if (!proc_create_data("runqlat", 0444,
				parent_dir, &runqlat_fops, info))
		goto remove_proc;

	if (!proc_create_data("distribution", 0444,
				parent_dir, &distribution_fops, info))
		goto remove_proc;
