...
do {										\
	...
	pr_auto(ASL1, #k "KEY_EL1: REG %016llx::%016llx, USER %016lx::%016lx, KERN %016lx::%016lx\n", \
...
do {										\
	...
	lo = read_sysreg_s(SYS_ ## k ## KEYLO_EL1);				\
	hi = read_sysreg_s(SYS_ ## k ## KEYHI_EL1);				\
	pr_auto(ASL1, #k "KEY_EL1: REG %016llx::%016llx, USER %016lx::%016lx\n",		\
		hi, lo, (uk).hi, (uk).lo);					\
} while (0)
...
void panic_bad_stack(struct pt_regs *regs, unsigned long esr, unsigned long far)
{
	...
	secdbg_base_built_bad_stack_info(tsk_stk, irq_stk, ovf_stk);
