...
extern u64 early_module_core_text;
extern u64 early_module_core_size;
extern u64 early_module_init_text;
extern u64 early_module_init_size;
#endif
/*
 * Mutex protects:
...
static int complete_formation(struct module *mod, struct load_info *info)
{
	...
	if (!rkp_started) {
		BUG_ON(early_module_core_text);
		early_module_core_text = (u64)mod->core_layout.base;
		early_module_core_size = (u64)mod->core_layout.text_size;
		early_module_init_text = (u64)mod->init_layout.base;
		early_module_init_size = (u64)mod->init_layout.text_size;

		printk("early_module text/size: %llx / %llx\n", early_module_core_text, early_module_core_size);
	}
