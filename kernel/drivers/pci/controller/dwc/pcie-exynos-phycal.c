...
int exynos_pcie_rc_eom(struct device *dev, void *phy_base_regs)
{
	...
	dev_info(dev, "Configure index values at least...\n");

	for (i = 0; i < lane_width; i++) {
		for (phase_sweep = 0; phase_sweep < PCIE_EOM_PH_SEL_MAX; phase_sweep++) {
			for (vref_sweep = 0; vref_sweep < PCIE_EOM_DEF_VREF_MAX; vref_sweep++) {
				//save result
				eom_result[i][test_cnt].phase = phase_sweep;
				eom_result[i][test_cnt].vref = vref_sweep;

				if (phase_sweep < 25 && vref_sweep < 160)
					eom_result[i][test_cnt].err_cnt = 0;
				else
					eom_result[i][test_cnt].err_cnt = 16400;
				test_cnt++;
			}
		}
	}
	...
	for (i = 0; i < lane_width; i++)
	{
		val = readl(phy_base_regs + PCIE_EOM_RX_EFOM_MODE);
		val = val | (0x3 << 4);
		writel(val, phy_base_regs + PCIE_EOM_RX_EFOM_MODE);

		writel(0x27, phy_base_regs + PCIE_EOM_RX_SSLMS_ADAP_HOLD_PMAD);
		...
		writel(0x0, phy_base_regs + PCIE_EOM_RX_EFOM_NUM_OF_SAMPLE_13_8);
		writel(num_of_smpl, phy_base_regs + PCIE_EOM_RX_EFOM_NUM_OF_SAMPLE_7_0);
		udelay(10);

		for (phase_sweep = 0; phase_sweep < PCIE_EOM_PH_SEL_MAX; phase_sweep++)
		{
			val = readl(phy_base_regs + PCIE_EOM_RX_EFOM_EOM_PH_SEL);
			val = val & ~(0xff << 0);
			writel(val, phy_base_regs + PCIE_EOM_RX_EFOM_EOM_PH_SEL);
			val = readl(phy_base_regs + PCIE_EOM_RX_EFOM_EOM_PH_SEL);
			...
			for (vref_sweep = 0; vref_sweep < PCIE_EOM_DEF_VREF_MAX; vref_sweep++)
			{
				writel(vref_sweep, phy_base_regs + PCIE_EOM_RX_EFOM_DFE_VREF_CTRL);
				val = readl(phy_base_regs + PCIE_EOM_RX_EFOM_START);
				val = val | (1 << 0);
				writel(val, phy_base_regs + PCIE_EOM_RX_EFOM_START);
				...
				do {
					...
					udelay(1);
					val = readl(phy_base_regs + PCIE_EOM_RX_EFOM_DONE) & (0x1);

					timeout++;
				} while(val != 0x1);

				err_cnt_13_8 = readl(phy_base_regs + PCIE_EOM_MON_RX_EFOM_ERR_CNT_13_8) << 8;
				err_cnt_7_0 = readl(phy_base_regs + PCIE_EOM_MON_RX_EFOM_ERR_CNT_7_0);
		...
		writel(0x21, phy_base_regs + PCIE_EOM_RX_SSLMS_ADAP_HOLD_PMAD);
		writel(0x0, phy_base_regs + PCIE_EOM_RX_EFOM_MODE);
