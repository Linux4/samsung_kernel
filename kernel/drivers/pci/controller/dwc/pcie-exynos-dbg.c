...
void exynos_pcie_dbg_print_link_history(struct exynos_pcie *exynos_pcie)
{
	...
	for (i = 31; i >= 0; i--)
		history_buffer[i] = exynos_elbi_read(exynos_pcie,
				PCIE_HISTORY_REG(i));
	for (i = 31; i >= 0; i--)
		pcie_info("LTSSM: 0x%02x, L1sub: 0x%x, D state: 0x%x\n",
...
void exynos_pcie_dbg_register_dump(struct exynos_pcie *exynos_pcie)
{
	...
	/* Link Reg : 0x0 ~ 0x47C */
	/* ---------------------- */
	pcie_err("[Print SUB_CTRL region]\n");
	pcie_err("offset:             0x0               0x4               0x8               0xC\n");
	for (i = 0; i < 0x480; i += 0x10) {
		pcie_err("ELBI 0x%04x:    0x%08x    0x%08x    0x%08x    0x%08x\n",
				i,
	...
	for (i = 0; i < 0x200; i += 0x10) {
		pcie_err("PHY 0x%04x:    0x%08x    0x%08x    0x%08x    0x%08x\n",
				i,
	...
	pcie_err("PHY 0x17C0 : 0x%08x\n", exynos_phy_read(exynos_pcie, 0xFC0 + 0x800));
	pcie_err("\n");

	/* ---------------------- */
	/* PHY PCS : 0x0 ~ 0x19C */
	/* ---------------------- */
	pcie_err("[Print PHY_PCS region]\n");
	pcie_err("offset:             0x0               0x4               0x8               0xC\n");
	for (i = 0; i < 0x200; i += 0x10) {
		pcie_err("PCS 0x%04x:    0x%08x    0x%08x    0x%08x    0x%08x\n",
				i,
	...
	pcie_err("[Print PHY_UDBG region]\n");
	pcie_err("offset:             0x0               0x4               0x8               0xC\n");
	for (i = 0; i < 0x1000; i += 0x10) {
		pcie_err("PHYUDBG(+0xC000) 0x%04x:    0x%08x    0x%08x    0x%08x    0x%08x\n",
				i,
