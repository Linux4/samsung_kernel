...
#define SYNAPTICS_TS_NAME		"synaptics_3908t"
#define SYNAPTICS_TS_I2C_NAME		"synaptics_i2c_3908t"
		...
		unsigned int size)
{
	...
	if (size > pbuf->buf_size) {
		...
		pbuf->buf = synaptics_ts_pal_mem_alloc(size, (unsigned int)sizeof(unsigned char));
		if (!(pbuf->buf)) {
