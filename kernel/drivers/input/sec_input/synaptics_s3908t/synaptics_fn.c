		...
		struct synaptics_ts_boot_info *boot_info)
{
	...
	retval = synaptics_ts_memcpy((unsigned char *)&ts->boot_info,
			(unsigned int)sizeof(struct synaptics_ts_boot_info),
			ts->resp_buf.buf,
		...
		struct synaptics_ts_application_info *app_info)
{
	...
	retval = synaptics_ts_memcpy((unsigned char *)info,
			(unsigned int)sizeof(struct synaptics_ts_application_info),
			ts->resp_buf.buf,
			ts->resp_buf.buf_size,
			copy_size);
	if (retval < 0) {
...
show_info:
	...
	retval = synaptics_ts_pal_mem_cpy((unsigned char *)ts->config_id,
			MAX_SIZE_CONFIG_ID,
...
/**
 * synaptics_ts_v1_discard_message()
 *
 * For some reasons, a message must be discarded thru the below steps.
 *     - retrieve a packet based on the max size to read
 *     - keep reading in the packet until idle
 *
 * @param
 *    [ in] tcm_dev:  the device handle
 *
 ...
 *    none.
 */
static void synaptics_ts_v1_discard_message(struct synaptics_ts_data *ts)
{
	int retval;
	...
	tcm_msg = &ts->msg_data;
	rd_size = ts->max_rd_size;

	if (rd_size == 0)
		rd_size = 64;

	buf = synaptics_ts_pal_mem_alloc(rd_size + 1, (unsigned int)sizeof(unsigned char));
	if (!buf) {
		input_err(true, ts->dev, "%s: Fail to allocate local buffer\n", __func__);
		return;
	}

	do {
		retval = ts->synaptics_ts_read_data_only(ts, buf, rd_size);
		if (retval < 0) {
			input_err(true, ts->dev, "%s:Fail to read %d bytes to device\n\n", __func__, rd_size);
			goto exit;
		}
		...
		synaptics_ts_pal_sleep_us(1000, 2000);
	} while (++retry < 100);
...
/**
 * synaptics_ts_v1_clear_command_processing()
 *
 * Helper to terminate and clear the current command processing
 *
 * @param
 *    [ in] tcm_dev: the device handle
 *
 ...
 *    none.
 */
void synaptics_ts_v1_clear_command_processing(struct synaptics_ts_data *ts)
{
	struct synaptics_ts_message_data_blob *tcm_msg = NULL;
	...
	synaptics_ts_pal_mutex_lock(&tcm_msg->cmd_mutex);
	input_info(true, ts->dev, "%s: stop the processing of command %02X\n", __func__, tcm_msg->command);
	synaptics_ts_pal_mutex_unlock(&tcm_msg->cmd_mutex);
...
retry:
	...
	if (ts->header->code == STATUS_CONTINUED_READ) {
		input_dbg(false, ts->dev, "%s:Unexpected continued packet received, %02x %02x %02x %02x\n", __func__,
			ts->header->marker, ts->header->code, ts->header->length[0],
			ts->header->length[1]);
		synaptics_ts_v1_discard_message(ts);
		retval = -1;
		goto exit;
	}

	/* retrieve the remaining data, if any */
	if ((tcm_msg->payload_length > 0) && (tcm_msg->payload_length != 0xFFFF)) {
	...
	unsigned int delay_ms_resp)
{
	...
	synaptics_ts_pal_mutex_t *rw_mutex = NULL;
	synaptics_ts_pal_completion_t *cmd_completion = NULL;
	bool in_polling = (delay_ms_resp != FORCE_ATTN_DRIVEN) ? true : false;
	...
	do {
		...
		synaptics_ts_pal_completion_wait_for(cmd_completion, polling_ms);
...
check_response:
	...
	if (ATOMIC_GET(tcm_msg->command_status) != CMD_STATE_IDLE) {
		if (timeout >= cmd_response_timeout)
			input_fail_hist(true, ts->dev, "%s: Timed out waiting for response of command 0x%02x (%dms)\n",
				__func__, command, timeout);
		/* a last shot when falling into error */
		retval = synaptics_ts_v1_read_message(ts, NULL);
		if (retval < 0)
			input_err(true, ts->dev, "%s: Fail to get valid response 0x%02x of command 0x%02x\n",
				__func__, tcm_msg->response_code, command);
		...
		unsigned char *data, unsigned int size, unsigned int data_len)
{
	...
	retval = synaptics_ts_memcpy((unsigned char *)id_info,
			(unsigned int)sizeof(struct synaptics_ts_identification_info),
			data,
			size,
...
int synaptics_ts_set_custom_library(struct synaptics_ts_data *ts)
{
	...
	/* read dump info */
	retval = ts->synaptics_ts_read_sponge(ts,
			data_dump, (int)sizeof(data_dump), SEC_TS_CMD_SPONGE_LP_DUMP, (unsigned int)sizeof(data_dump));
	if (retval < 0) {
...
void synaptics_ts_get_custom_library(struct synaptics_ts_data *ts)
{
	...
	ret = ts->synaptics_ts_read_sponge(ts,
			data, (int)sizeof(data), offset, (unsigned int)sizeof(data));
	if (ret < 0) {
		...
		unsigned short offset, unsigned int wr_len)
{
	...
	payload = synaptics_ts_pal_mem_alloc(payload_size, (unsigned int)sizeof(unsigned char));
	if (!payload) {
...
void  synaptics_ts_set_utc_sponge(struct synaptics_ts_data *ts)
{
	...
	ret = ts->synaptics_ts_write_sponge(ts, data, (int)sizeof(data), SEC_TS_CMD_SPONGE_OFFSET_UTC, (unsigned int)sizeof(data));
	if (ret < 0)
...
int synaptics_ts_set_press_property(struct synaptics_ts_data *ts)
{
	...
	ret = ts->synaptics_ts_write_sponge(ts, data, (int)sizeof(data), offset, (unsigned int)sizeof(data));
	ret = 0;
	if (ret < 0)
		input_err(true, ts->dev, "%s: Failed to write sponge\n", __func__);
...
int synaptics_ts_tclm_write(struct device *dev, int address)
{
	...
	ret = synaptics_ts_send_command(ts,
			CMD_ACCESS_CALIB_DATA_FROM_NVM,
			&payload_erase,
			(unsigned int)sizeof(payload_erase),
			&resp_code,
			&resp,
			100);
	if (ret < 0) {
