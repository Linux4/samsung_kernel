		...
		struct kobj_attribute *attr, char *buf)
{
	...
	retval = synaptics_ts_pal_mem_cpy(buf,
			...
			(unsigned int)sizeof(ts->id_info.part_number),
			(unsigned int)sizeof(ts->id_info.part_number));
	if (retval < 0) {
		...
		unsigned int report_size)
{
	...
	retval = synaptics_ts_pal_mem_cpy(ts->vendor_data->report_to_queue, REPORT_TYPES,
			data, (unsigned int)sizeof(data), REPORT_TYPES);
		...
		unsigned long arg)
{
	...
	ptr = ioc_data.buf;
	buf_size = (unsigned int)sizeof(*ptr);
	if (ioc_data.buf_size > buf_size) {
		input_err(true, ts->dev, "size error, buf_size:%d, ioc_data.buf_size:%d\n",
			buf_size, ioc_data.buf_size);
		retval = -ENOMEM;
		goto exit;
	}
