		...
		struct synaptics_ts_touch_data_blob *touch_data)
{
	...
	size = (unsigned int)sizeof(touch_data->object_data);
		...
		struct synaptics_ts_touch_data_blob *tp_data)
{
	...
	if (ts->header) {
		if (ts->header->marker == SYNAPTICS_TS_V1_MESSAGE_MARKER && !ts->header->code && !ts->header->length[0] && !ts->header->length[1]) {
			ts->irq_empty_count++;
			if (ts->irq_empty_count % 10 == 1)
				input_info(true, ts->dev, "empty message(%d)\n", ts->irq_empty_count);

			if (ts->irq_empty_count >= 100) {
				ts->irq_empty_count = 0;
				input_info(true, ts->dev, "empty message. send abc event\n");
...
report_cpy:
	...
	report->data_length = 0; // reset the content

	if (ts->report_buf.data_length == 0)
		goto exit;
...
void synaptics_ts_send_rawdata(struct synaptics_ts_data *ts, u8 *data, bool is_released)
{
	s16 *val;
	int ii = 0;
	int length = ts->cols * ts->rows * 2 + ts->plat_data->custom_rawdata_size * 2;
	int press = ts->plat_data->coord[0].action == 0 ? 3 : ts->plat_data->coord[0].action;
	s16 palm = ts->plat_data->coord[0].palm ? 1 : 0;
	int curr_action = press;
	int curr_palm = palm;

	if (!ts->raw)
		ts->raw = kzalloc(IOCTL_SIZE, GFP_KERNEL);

	if (ts->plat_data->support_self_rawdata) {
		length += (ts->rows + ts->cols) * 2;
	}

	for (ii = 1 ; ii < SEC_TS_SUPPORT_TOUCH_COUNT; ii++) {
		curr_action = ts->plat_data->coord[ii].action == 0 ? SEC_TS_COORDINATE_ACTION_RELEASE : ts->plat_data->coord[ii].action;
		if (curr_action ==  SEC_TS_COORDINATE_ACTION_MOVE && press != curr_action)
			press = curr_action;
		curr_palm = ts->plat_data->coord[ii].palm ? 1 : 0;
		if (curr_palm == 1 && curr_action == SEC_TS_COORDINATE_ACTION_PRESS)
			palm = 1;
	}
	...
	if (is_released) {
		...
		input_info(true, ts->dev, "%s: raw touch released\n", __func__);
#endif
		press = 0xFF;
	}

	sec_input_rawdata_copy_to_user(ts->raw, length + (POSTFIX_BYTE_LENGTH + PREFIX_BYTE_LENGTH), press);
}
#endif
...
irqreturn_t synaptics_ts_irq_thread(int irq, void *ptr)
{
	...
	u8 *raw;
	bool is_raw_event = false, is_released = false;
	...
	ret = synaptics_ts_get_event(ts, &event_id, &ts->event_data, NULL);
	if (ret < 0)
		return IRQ_HANDLED;
	...
	do {
		if (is_raw_event) {
			...
			if (memcmp(event_buff, empty_buff, 16) == 0) {
				is_released = true;
				break;
			}
		} else {
...
#if IS_ENABLED(CONFIG_SEC_INPUT_RAWDATA)
	if (is_raw_event) {
		synaptics_ts_send_rawdata(ts, raw, is_released);
	}
#endif
	return IRQ_HANDLED;
}
