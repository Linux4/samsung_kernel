...
void synaptics_ts_fw_delay_setting(struct synaptics_ts_data *ts)
{
	if (strncmp(ts->id_info.part_number, SYNAPTICS_IC_S3916T, 6) == 0) {
		if (ts->fw_erase_delay != 0) {
			ts->fw_erase_delay = PER_PAGE_ERASE_DELAY_MS_3916T;
			input_info(true, ts->dev, "%s: %s require product-specified settings of flash erase (S3916T), %d\n",
					__func__, ts->id_info.part_number, ts->fw_erase_delay);
		}
		if (ts->fw_write_block_delay != 0) {
			ts->fw_write_block_delay = PER_BLOCK_WRITE_DELAY_MS_3916T;
			input_info(true, ts->dev, "%s: %s require product-specified settings of flash write (S3916T), %d\n",
					__func__, ts->id_info.part_number, ts->fw_write_block_delay);
		}
