		...
		struct synaptics_ts_reflash_data_blob *reflash_data)
{
	...
	for (idx = 0; idx < (int)sizeof(ts->id_info.part_number) - 3; idx++) {
		if (ts->id_info.part_number[idx] == 0x3A) {
			if ((ts->id_info.part_number[idx + 3] & 0x20) == 0x20)
		...
		struct image_info *image_info)
{
	...
	synaptics_ts_pal_mem_set(image_info, 0x00, (unsigned int)sizeof(struct image_info));
	...
	offset = (unsigned int)sizeof(struct image_header);
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_write_flash_opt(struct synaptics_ts_data *ts,
		struct synaptics_ts_reflash_data_blob *reflash_data,
		unsigned int address, const unsigned char *wr_data,
		unsigned int wr_len, unsigned int wr_delay_ms)
{
		...
		struct block_data *block, unsigned int resp_reading, bool opt_write)
{
	...
	if (opt_write) {
		retval = syna_tcm_write_flash_opt(ts, reflash_data,
		...
		unsigned int delay_setting, bool force_reflash)
{
	...
	synaptics_ts_pal_mem_set(&reflash_data.image_info, 0x00,
		(unsigned int)sizeof(struct image_info));
...
int synaptics_ts_fw_update_on_probe(struct synaptics_ts_data *ts)
{
	...
	if (delay_setting != FORCE_ATTN_DRIVEN) {
		...
		input_info(true, ts->dev, "%s: run fw update in polling, per page erase:%d per block write:%d\n",
			__func__, ts->fw_erase_delay, ts->fw_write_block_delay);
	}
