...
static int get_self_channel_data(void *device_data, u8 type)
{
	...
	if (sec->cmd_all_factory_state == SEC_CMD_STATUS_RUNNING)
		sec_cmd_set_cmd_result_all(sec, buff, (int)sizeof(buff), temp);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (ts->plat_data->img_version_of_ic[0] == 0x31)
		...
		snprintf(tbuff, sizeof(tbuff), ",\"TVEN%s\":\"SYNA_S3908T\"", mdev);
	else
...
static void get_rawdata_custom_size(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct synaptics_ts_data *ts = container_of(sec, struct synaptics_ts_data, sec);
	char buff[16] = { 0 };
...
static void run_interrupt_gpio_test(void *device_data)
{
	...
	buf = synaptics_ts_pal_mem_alloc(2, (unsigned int)sizeof(struct synaptics_ts_identification_info));
	...
	ret = ts->synaptics_ts_write_data(ts, command, (int)sizeof(command), NULL, 0);
	if (ret < 0) {
...
static struct sec_cmd sec_cmds[] = {
	...
	{SEC_CMD_V2("get_rawdata_custom_size", get_rawdata_custom_size, NULL, CHECK_ALL, WAIT_RESULT),},
