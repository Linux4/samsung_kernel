...
#define BTS_BIT_CLEAR(mask, shift)              (~((mask) << (shift)))
#define BTS_BIT_SET(data, mask, shift)          (((data) & (mask)) << (shift))
#define BTS_BIT_GET(data, mask, shift)          (((data) & ((mask) << (shift))) >> (shift))

#define BTS_HINT_EN_SHIFT			(0)
#define BTS_HINT_BPC_SHIFT			(1)
#define BTS_HINT_LAYER_SHIFT			(2)
...
#endif
	unsigned int		bwops_val;
	void			(*btsbw_ops)(struct bts_device *data, u32 val0);
};
...
extern int register_btsdevops(struct bts_device *data);
