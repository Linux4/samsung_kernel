...
static void bts_bw_recalc(struct bts_device *data, u32 index)
{
	...
	bwops_val = BTS_BIT_GET(data->bwops_val, BTS_ONE_BIT_MASK, BTS_HINT_EN_SHIFT);
	layer = BTS_BIT_GET(data->bwops_val, BTS_ONE_BIT_MASK, BTS_HINT_LAYER_SHIFT);

	if (bwops_val && bw_sum > MIN_BW && bw_sum < MID_BW)
		data->bts_bw[index].read = MID_BW;
