...
#define ID_DEFAULT 0
int register_btsdevops(struct bts_device *data)
{
	...
	if(!va_base){
		va_base = ioremap(0x10000010, 4);
		value = (readl(va_base) >> 16) & 0xF ;

		if (value) {
			int i = 0;
			struct bts_info *info = data->bts_list;
			struct bts_stat *stat;

			for(i = 0; i < data->num_bts; i++){
				stat = &info[i].stat[ID_DEFAULT];

				if (!strcmp(info[i].name, "bts_d0_dpu"))
					stat->arqos = stat->awqos = 0xA;
				if (!strcmp(info[i].name, "bts_d1_dpu"))
					stat->arqos = stat->awqos = 0xA;
				if (!strcmp(info[i].name, "bts_d0_modem"))
					stat->arqos = stat->awqos = 0xC;
				if (!strcmp(info[i].name, "bts_d_aud"))
					stat->arqos = stat->awqos = 0x9;
				if (!strcmp(info[i].name, "bts_csis0"))
					stat->arqos = stat->awqos = 0xC;
				if (!strcmp(info[i].name, "bts_csis1"))
					stat->arqos = stat->awqos = 0xC;
				if (!strcmp(info[i].name, "bts_cstat0"))
					stat->arqos = stat->awqos = 0xC;
			}
		}
...
EXPORT_SYMBOL(register_btsdevops);
