...
bool s6e3haf_aod_is_factory_mode(struct panel_device *panel)
{
	return panel_get_property_value(panel, PANEL_PROPERTY_IS_FACTORY_MODE) ? true : false;
}

int s6e3haf_aod_self_mask_data_check(struct aod_dev_info *aod)
{
	...
	dump_str = s6e3haf_aod_is_factory_mode(panel) ?
