...
/* dsim version */
#define DSIM_VER_REF			0x02090100
...
#if 0 // TBD: Taken from 9945, is it applicable for 8845 ?
...
static void dsim_reg_set_md_ana_con(u32 id, const u32 *blk_ctl)
{
	...
	for (i = 0; i < MAX_DSIM_DATALANE_CNT; i++) {
		...
		/* For D3, ANA_CON2 & ANA_CON3 registers values are
		 * different. So, initialize it outside the loop
		 */
		if(i == 3)
			continue;
        ...
        /* Initialize D3, ANA_CON2 & ANA_CON3 here,
...
static int dsim_reg_enable_lane_phy(u32 id, u32 lane, bool en)
{
	...
	/*Revisit if only bits 0,1 to be enabled*/
...
static void dsim_reg_set_esc_clk_prescaler(u32 id, u32 en, u32 p)
{
	...
	if (dsim_reg_get_version(id) < 0x02090200) {
		...
		if (!(dsim->res.oscclk_dsim))
			...
			clk_set_rate(dsim->res.oscclk_dsim, (AP_OSC_KHZ * 1000 / p));
	}
}
...
/* This is not being used in S5e8845
 * since pll-sleep = <DPHY_SLEEP_CTRL_DECON>;
 */
...
//	dsim_reg_set_esc_clk_on_lane(id, 1, lanes); // TBD: Reference from 9945, is it required ??
	dsim_reg_start(id);
	return ret;
}
...
int dsim_reg_stop_and_enter_ulps(u32 id, u32 ddi_type, u32 lanes)
{
	...
	dsim_reg_set_lanes_dphy(id, lanes, false);
	dsim_reg_set_clocks(id, NULL, NULL, 0);
	//dsim_reg_set_esc_clk_on_lane(id, 0, lanes); //TBD: Reference from 9945, is it required ??
