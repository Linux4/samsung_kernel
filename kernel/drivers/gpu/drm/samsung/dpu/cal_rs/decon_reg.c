...
static int decon_reg_reset(u32 id)
{
	u32 val;
	int ret;
	/* TBD: instead of CONFIG_SOC_S5E8845_EVT0, evt0 OR evt0.1 can be
	 * identified from the global structure
	 */
	if ((exynos_soc_info.main_rev == 0) && (exynos_soc_info.sub_rev == 0x0)) {
		struct decon_device *d0;
		struct decon_device *d1;
		struct dsim_device *dsim = NULL;

		cal_log_debug(id, "+%s DECON0/1 SRESET WA decon(%d)\n", __func__, id);

		d0 = get_decon_drvdata(0);
		d1 = get_decon_drvdata(1);

		if((id == 0) && (d1->state == DECON_STATE_ON)) {
			u32 i;
			cal_log_debug(id, "WA: disable req for d0 when d1 is ON \n");

			/*
			 * Make sure all window connections are disabled to avoid the
			 * resource conflict when reset is being skipped here.
			 * New mappings will happen later.
			 */
			dsim = decon_get_dsim(d0);
			for (i = 0; i < MAX_WIN_PER_DECON; ++i) {
				...
				decon_reg_set_win_enable(d0->id, i, 0);
			}

			return 0;
		} else if((id == 1) && (d0->state == DECON_STATE_ON)) {
			u32 i;
			cal_log_debug(id, "WA: disable req for d1 when d0 is ON \n");

			for (i = 0; i < MAX_WIN_PER_DECON; ++i) {
				/* There is no DSIM for DECON1 */
				decon_reg_set_win_enable(d1->id, i, 0);
			}
...
/* bias sleep can be enabled only when pll seep is enabled */
	...
	switch (out_type) {
	case DECON_OUT_DSI0:
		/* s5e8845 has DECON0/OUT0 connected only to DSIMIF0 */
		val = OUTIF_DSI0;
...
void decon_reg_set_cwb_enable(u32 id, u32 en)
{
	cal_log_err(id, "CWB is not supported  DECON%d\n", id);
	return;
}
#endif
				...
				struct decon_dsc *dsc_enc)
{
	...
	dsc_enc->pic_width = dsc_cfg->pic_width;
