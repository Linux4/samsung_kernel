...
#if 0 // TBD: Need to find the usage
...
{
	dpp_write_mask(id, DPP_COM_HDR_CON, en, DPP_WB_MULT_EN);
}
...
static void dpp_reg_set_bext_mode(u32 id, u32 mode)
{
	...
	/* [mode] 0=msb copy, 1=zero padding */
	dpp_write_mask(id, DPP_COM_IO_CON, DPP_WB_BEXT_MODE(mode),
			DPP_WB_BEXT_MODE_MASK);
}
#endif
