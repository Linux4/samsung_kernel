...
static int dsimfc_init_resources(struct dsimfc_device *dsimfc, struct platform_device *pdev)
{
	...
	dsimfc->res.irq = of_irq_get_byname(np, "dsimfc");
	dsimfc_info(dsimfc, "dsimfc irq no = %d\n", dsimfc->res.irq);
	ret = devm_request_irq(dsimfc->dev, dsimfc->res.irq, dsimfc_irq_handler, 0,
			pdev->name, dsimfc);
	if (ret) {
