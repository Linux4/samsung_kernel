...
#define MCD_PANEL_MODESET_POST_WA_DELAY_MSEC (40)
...
#if 0
	/* S.LSI panel drv style */
	_exynos_update_status_internal(ctx,
			(uint64_t)exynos_conn_state->panel_brightness_level);
#else
	brightness_level = exynos_conn_state->panel_brightness_level;
	panel_info(ctx, "br: %d\n", brightness_level);
	call_mcd_panel_func(ctx->mcd_panel_dev, set_brightness, brightness_level);
#endif
		...
		struct drm_mode_object *obj, struct exynos_panel *ctx)
{
	...
	drm_object_attach_property(obj, p->backlight_level, 128);
				  ...
				  const struct drm_display_mode *adjusted_mode)
{
	...
	if (funcs) {
		...
		if (ctx->lastclosed) {
			call_mcd_panel_func(ctx->mcd_panel_dev, display_on);
			ctx->lastclosed = false;
		}
...
#if IS_ENABLED(CONFIG_USDM_PANEL_ANA6710)
	queue_delayed_work(ctx->wqs[MCD_DRM_DRV_WQ_MODESET_POST_WA].wq,
			&ctx->wqs[MCD_DRM_DRV_WQ_MODESET_POST_WA].dwork,
			msecs_to_jiffies(MCD_PANEL_MODESET_POST_WA_DELAY_MSEC));
#endif
...
void wq_modeset_post_wa_handler(struct work_struct *data)
{
#if IS_ENABLED(CONFIG_USDM_PANEL_ANA6710)
	struct mcd_drm_drv_wq *w = container_of(to_delayed_work(data), struct mcd_drm_drv_wq, dwork);
	struct exynos_panel *ctx = container_of(w, struct exynos_panel, wqs[MCD_DRM_DRV_WQ_MODESET_POST_WA]);
	int ret;

	/* W/A for restoring manual aor in R12S */
	ret = call_mcd_panel_func(ctx->mcd_panel_dev, execute_sequence, "r12s_manual_aor_restore_seq");
	if (ret < 0)
		dev_err(ctx->dev, "%s failed to execute r12s_manual_aor_restore_seq (ret:%d)\n",
				__func__, ret);
#endif
}
...
work_func_t mcd_drm_drv_wq_fns[MAX_MCD_DRM_DRV_WQ] = {
	...
	[MCD_DRM_DRV_WQ_MODESET_POST_WA] = wq_modeset_post_wa_handler,
};
...
char *mcd_drm_drv_wq_names[MAX_MCD_DRM_DRV_WQ] = {
	...
	[MCD_DRM_DRV_WQ_MODESET_POST_WA] = "wq_modeset_post_wa",
};
