...
static int exynos_update_status(struct backlight_device *bl)
{
	 ...
	 */
	min_br = ctx->desc->min_brightness;

	if (min_br) {
		if (brightness == 0)
			weight_br = 0;
		else {
			weight_br = (bl->props.max_brightness - min_br) / 254;
			weight_br *= ((u32)brightness - 1);
			weight_br += min_br;
		}
	} else
		weight_br = (bl->props.max_brightness * (u32)brightness) / 255;

	panel_debug(ctx, "br_lvl (%d), weight_br_lvl (%u)\n", brightness, weight_br);

	exynos_panel_func = ctx->func_set->exynos_panel_func;
	if (exynos_panel_func && exynos_panel_func->set_brightness)
		ret = exynos_panel_func->set_brightness(ctx, weight_br);
	else
		ret = exynos_dcs_set_brightness(ctx, weight_br);
