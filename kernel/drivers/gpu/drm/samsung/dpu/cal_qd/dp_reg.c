...
void dp_hsi0_clk_check(void)
{
	...
	if (clk_regs2 == NULL) {
		clk_regs2 = ioremap((phys_addr_t)0x1a320000 + 0x10cc, 0x10);
		if (!clk_regs2) {
			pr_err("HSI0_CLK SFR ioremap is failed\n");
			return;
		}
	...
	pr_info("CLK_CON_MUX_MUX_CLKCMU_HSI0_DPOSC[0x1a32_10cc](0x%08x)\n", readl(clk_regs2));

	if (clk_regs3 == NULL) {
		clk_regs3 = ioremap((phys_addr_t)0x1a320000 + 0x1864, 0x10);
		if (!clk_regs3) {
			pr_err("HSI0_CLK SFR ioremap is failed\n");
			return;
		}
	...
	pr_info("CLK_CON_DIV_CLKCMU_HSI0_DPOSC[0x1a32_1864](0x%08x)\n", readl(clk_regs3));
}

u32 phy_eq_hbr0_1[4][4][5] = {
...
int dp_reg_usbdrd_qch_en(u32 en)
{
	...
	cmu_regs = ioremap((phys_addr_t)0x10A00000 + 0x3068, 0x8);
	if (!cmu_regs) {
	...
	if (en) {
		...
		pr_info("QCH_CON_USB32DRD_QCH_S_CTRL[0x10A03068](0x%08x)\n", readl(cmu_regs));
		writel(0x6, cmu_regs);
		pr_info("QCH_CON_USB32DRD_QCH_S_CTRL[0x10A03068](0x%08x)\n", readl(cmu_regs));
	} else {
