...
#define cal_dma_write(id, offset, val, type)			\
	exynos_drm_sfr_dma_request(id, offset, val, type)
...
void __sfr_dma_dump(void __iomem *sfr_dma_regs)
{
	...
	dpu_print_hex_dump(sfr_dma_regs, sfr_dma_regs + 0x0800 + 0x0000, 0x30c);
...
static void sfr_dma_reg_clear_irq_all(u32 id)
{
	sfr_dma_write(id, EDMA_CGC_IRQ, ~0 & CGC_ALL_IRQ_CLEAR);
}
...
int sfr_dma_reg_wait_framedone(int dpuf, u32 wait_ms)
{
	...
	ret = readl_poll_timeout_atomic(sfr_dma_regs_desc(id)->regs + EDMA_CGC_IRQ,
			val, (val & CGC_FRAME_DONE_IRQ), 1, wait_ms * 1000);
	if (ret) {
		cal_log_err(dpuf, "failed to read irq (%d)\n", dpuf);
	}
	sfr_dma_reg_clear_irq_all(dpuf);
