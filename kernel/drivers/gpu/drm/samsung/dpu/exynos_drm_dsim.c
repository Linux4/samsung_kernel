...
EXPORT_SYMBOL_FOR_DPU_TESTS_ONLY(dsim_drvdata);
...
static int underrun_debug_mode = DSIM_UNDERRUN_DEBUG_NONE;
module_param(underrun_debug_mode, int, 0600);
MODULE_PARM_DESC(underrun_debug_mode, "log level for dpu dsim [default : 6]");
...
EXPORT_SYMBOL_FOR_DPU_TESTS_ONLY(dsim_dump);
...
static void _dsim_start(struct dsim_device *dsim, bool ulps)
{
	...
	if (!ulps)
		...
		dsim_reg_exit_ulps_and_start(dsim->id, 0, lanes);

	dsim->state = DSIM_STATE_HSCLKEN;
	if (!irq_enabled)
		enable_irq(dsim->irq);

	if (unlikely(dsim->config.bist.en)) {
		struct dsim_bist *bist = &dsim->config.bist;

		dsim_reg_set_bist(dsim->id, bist->en, bist->mode);
	}

	DPU_EVENT_LOG("DSIM_START", exynos_crtc, 0, NULL);
}

static void dsim_exit_ulps_locked(struct dsim_device *dsim)
{
	...
	exynos_crtc = dsim_get_exynos_crtc(dsim);
	dsim_debug(dsim, "+\n");
	...
	DPU_EVENT_LOG("DSIM_EXIT_HIBER", exynos_crtc, 0, NULL);
	dsim_debug(dsim, "-\n");
	DPU_ATRACE_END(__func__);
}

static void dsim_atomic_exit_ulps(struct exynos_drm_encoder *exynos_encoder)
{
	struct dsim_device *dsim = encoder_to_dsim(exynos_encoder);

	mutex_lock(&dsim->cmd_lock);
	dsim_exit_ulps_locked(dsim);
...
static void dsim_enable_locked(struct exynos_drm_encoder *exynos_encoder)
{
	...
	if (!exynos_drm_bridge_lp11_reset(dsim->panel_bridge))
...
static void dsim_enter_ulps_locked(struct dsim_device *dsim)
{
	struct exynos_drm_crtc *exynos_crtc;
	...
	DPU_EVENT_LOG("DSIM_ENTER_HIBER", exynos_crtc, 0, NULL);
	dsim_debug(dsim, "-\n");
	DPU_ATRACE_END(__func__);
}

static void dsim_atomic_enter_ulps(struct exynos_drm_encoder *exynos_encoder)
{
	struct dsim_device *dsim = encoder_to_dsim(exynos_encoder);

	/* Wait for current read & write CMDs. */
	mutex_lock(&dsim->cmd_lock);
	dsim_enter_ulps_locked(dsim);
	mutex_unlock(&dsim->cmd_lock);
}
...
static const struct exynos_drm_encoder_funcs exynos_dsim_encoder_funcs = {
	...
	.atomic_enter_ulps = dsim_atomic_enter_ulps,
	.atomic_exit_ulps = dsim_atomic_exit_ulps,
};
...
static int dsim_bind(struct device *dev, struct device *master, void *data)
{
	...
	dsim->encoder = exynos_drm_encoder_create(drm_dev,
			&exynos_dsim_encoder_funcs, DRM_MODE_ENCODER_DSI,
			dsim->output_type, dsim);
	if (IS_ERR(dsim->encoder))
		return ret;
...
static int dsim_parse_dt(struct dsim_device *dsim)
{
	...
	of_property_read_u32(np, "pll-sleep", &dsim->pll_sleep);
...
static void dsim_reset_continuous_underrun_count(struct dsim_device *dsim)
{
	struct dsim_reg_config *config = &dsim->config;
	struct dsim_underrun_config *underrun = &dsim->underrun_config;

	if (config->mode != DSIM_COMMAND_MODE)
		return;

	underrun->prev_linecnt = -1;
	underrun->continuous_count = 0;
}

static void dsim_inc_continuous_underrun_count(struct dsim_device *dsim, int line)
{
	struct dsim_reg_config *config = &dsim->config;
	struct dsim_underrun_config *underrun = &dsim->underrun_config;
	...
	underrun->prev_linecnt = line;
	underrun->continuous_count++;
}

static void dsim_debug_continuous_underrun(struct dsim_device *dsim,
		struct exynos_drm_crtc *exynos_crtc, u32 line)
{
	struct dsim_reg_config *config = &dsim->config;
	struct dsim_underrun_config *underrun = &dsim->underrun_config;
	const int underrun_max_count = 5;

	if (config->mode != DSIM_COMMAND_MODE)
		return;

	if (underrun->prev_linecnt == line) {
		dsim_inc_continuous_underrun_count(dsim, line);
		dsim_warn(dsim, "underrun : cont[%d] line[%d]\n",
				underrun->continuous_count, line);
		if (underrun->continuous_count == underrun_max_count) {
			if (pm_runtime_active(exynos_crtc->dev) &&
				exynos_crtc->ops->dump_register)
...
static void dsim_print_underrun_info(struct dsim_device *dsim)
{
	...
	dsim_debug_continuous_underrun(dsim, exynos_crtc, line_cnt);

	switch (underrun_debug_mode) {
		case DSIM_UNDERRUN_DEBUG_S2D:
			dbg_snapshot_expire_watchdog();
			BUG();
			break;
		case DSIM_UNDERRUN_DEBUG_PANIC:
			panic("underrun");
			break;
		default:
			break;
...
static irqreturn_t dsim_irq_handler(int irq, void *dev_id)
{
	...
	if (int_src & DSIM_INTSRC_FRAME_DONE) {
		...
		dsim_reset_continuous_underrun_count(dsim);
...
static int dsim_get_clock(struct dsim_device *dsim)
{
	...
	dsim->res.oscclk_dsim = devm_clk_get(dsim->dev, "oscclk_dsim");
	if (IS_ERR_OR_NULL(dsim->res.oscclk_dsim)) {
		dsim_info(dsim, "failed to get oscclk_dsim(optional)\n");
		dsim->res.oscclk_dsim = NULL;
	}

	if (dsim->res.oscclk_dsim)
		dsim_info(dsim, "Success to get oscclk_dsim resource\n");
...
static int dsim_exit_pll_sleep(struct dsim_device *dsim)
{
	struct exynos_drm_crtc *exynos_crtc;
	int ret = 0;

	switch (dsim->pll_sleep) {
	case DPHY_SLEEP_CTRL_LINK:
		if (dsim_reg_is_pll_sleep_state(dsim->id)) {
			dsim_debug(dsim, "pll is sleep state\n");
			dsim_reg_set_pll_sleep_enable(dsim->id, false);
			ret = dsim_reg_wait_exit_pll_sleep(dsim->id);
		}
		break;
	case DPHY_SLEEP_CTRL_DECON:
		exynos_crtc = dsim_get_exynos_crtc(dsim);
		if (exynos_crtc) {
		...
		break;
	default:
		dsim_debug(dsim, "pll sleep isn't supported\n");
		break;
	}

	if (ret)
		dsim_err(dsim, "pll failed to wakeup from sleep state\n");

	return ret;
}

static void dsim_allow_pll_sleep(struct dsim_device *dsim)
{
	switch (dsim->pll_sleep) {
	case DPHY_SLEEP_CTRL_LINK:
		dsim_reg_set_pll_sleep_enable(dsim->id, true);
		break;
	case DPHY_SLEEP_CTRL_DECON:
	/* TODO: unmask pll sleep for DPHY_SLEEP_CTRL_DECON */
		break;
	default:
		dsim_debug(dsim, "pll sleep isn't supported\n");
		break;
			...
			u32 d1, u32 unit, bool wait_empty)
{
	...
	ulps = (dsim->state == DSIM_STATE_ULPS);
	if (!is_dsim_enabled(dsim) && (!doze_suspend && !ulps)) {
	...
	if (!dsim_wait_fifo_empty(dsim, 3)) {
		ret = -EINVAL;
		dsim_err(dsim, "ID(%d): DSIM cmd wr timeout @ don't available ph or pl 0x%llx\n", id, d0);
		goto err_exit;
	}
	...
	if (!wait_empty) {
		...
		if (!dsim_wait_fifo_empty(dsim, 10)) {
			dsim_err(dsim, "MIPI fcmd tx timeout(ID:%d, dma_addr:%llx)\n", id, d0);
			ret = -EINVAL;
		}
	}
			    ...
			    const struct mipi_dsi_msg *msg)
{
	...
	if (msg->tx_len > FCMD_DATA_MAX_SIZE) {
		dsim_err(dsim, "fcmd transfer size error(%zu, %d)\n", msg->tx_len, FCMD_DATA_MAX_SIZE);
		return -EINVAL;
	}
				   ...
				   struct mipi_dsi_packet *packet)
{
	...
	if (type == MIPI_DSI_DCS_READ) {
		dsim_debug(dsim, "read cmd requested\n");
		return true;
	}
	...
	if (mipi_dsi_packet_format_is_long(type)) {
		if (!dsim_check_pl_threshold(dsim, ALIGN(packet->payload_length, 4))) {
			dsim_err(dsim, "ID(%#x): pl(%#x) don't available\n",
					type, packet->header[1]);
			dsim_err(dsim, "pl_threshold(%d) pl_cnt(%d) wc(%zu)\n",
...
static int dsim_write_cmd_set(struct dsim_device *dsim,
...
dsim_write_data(struct dsim_device *dsim, const struct mipi_dsi_msg *msg)
{
	...
	must_wait = dsim_fifo_empty_needed(dsim, &packet);
