...
#define Y_SIZE(w, h, bpc)	((bpc) ? (w) * (h) * 2 :		\
					__ALIGN_UP((w), 16) * __ALIGN_UP((h), 16))

static inline size_t
Y_SIZE_SBWC(unsigned int w, unsigned int h, bool is_10bpc, bool is_256a)
{
	if (is_10bpc && is_256a)
		return SBWC_256_10B_Y_SIZE(w, h) + SBWC_256_10B_Y_HEADER_SIZE(w, h);

	if (is_10bpc && !is_256a)
		return SBWC_10B_Y_SIZE(w, h) + SBWC_10B_Y_HEADER_SIZE(w, h);

	if (!is_10bpc && is_256a)
		return SBWC_256_8B_Y_SIZE(w, h) + SBWC_256_8B_Y_HEADER_SIZE(w, h);

	return SBWC_8B_Y_SIZE(w, h) + SBWC_8B_Y_HEADER_SIZE(w, h);
}

static inline size_t
UV_SIZE_SBWC(unsigned int w, unsigned int h, bool is_10bpc, bool is_256a)
{
	if (is_10bpc && is_256a)
		return SBWC_256_10B_CBCR_SIZE(w, h) + SBWC_256_10B_CBCR_HEADER_SIZE(w, h);

	if (is_10bpc && !is_256a)
		return SBWC_10B_CBCR_SIZE(w, h) + SBWC_10B_CBCR_HEADER_SIZE(w, h);

	if (!is_10bpc && is_256a)
		return SBWC_256_8B_CBCR_SIZE(w, h) + SBWC_256_8B_CBCR_HEADER_SIZE(w, h);

	return SBWC_8B_CBCR_SIZE(w, h) + SBWC_8B_CBCR_HEADER_SIZE(w, h);
}

static inline size_t
Y_PL_SIZE_SBWC(unsigned int w, unsigned int h, bool is_10bpc, bool is_256a)
{
	if (is_10bpc && is_256a)
		return SBWC_256_10B_Y_SIZE(w, h);

	if (is_10bpc && !is_256a)
		return SBWC_10B_Y_SIZE(w, h);

	if (!is_10bpc && is_256a)
		return SBWC_256_8B_Y_SIZE(w, h);

	return SBWC_8B_Y_SIZE(w, h);
}

static inline size_t
UV_PL_SIZE_SBWC(unsigned int w, unsigned int h, bool is_10bpc, bool is_256a)
{
	if (is_10bpc && is_256a)
		return SBWC_256_10B_CBCR_SIZE(w, h);

	if (is_10bpc && !is_256a)
		return SBWC_10B_CBCR_SIZE(w, h);

	if (!is_10bpc && is_256a)
		return SBWC_256_8B_CBCR_SIZE(w, h);

	return SBWC_8B_CBCR_SIZE(w, h);
}
