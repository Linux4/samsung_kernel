...
decon_enable(struct exynos_drm_crtc *crtc, struct drm_atomic_state *old_state)
{
	...
	struct decon_device *decon = crtc->ctx;
	struct exynos_drm_connector_state *exynos_conn_state = NULL;
...
static void decon_disable(struct exynos_drm_crtc *crtc)
{
	...
	if (decon->state == DECON_STATE_OFF) {
		...
		if (decon->config.mode.lp_mode == DECON_ENTER_LP) {
			DPU_EVENT_LOG("DECON_DISABLED_IN_LP", crtc, 0, NULL);
