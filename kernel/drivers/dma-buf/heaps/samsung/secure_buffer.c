			      ...
			      unsigned int protalign)
{
	...
	ret = ppmp_hvc(HVC_DRM_TZMP2_PROT, protdesc->chunk_count, protdesc->flags,
		       protdesc->chunk_size, protdesc->bus_address);
	if (ret) {
		perr("%s: PROT %#x (err=%#lx,size=%#lx,cnt=%u,flg=%u)",
		     __func__, HVC_DRM_TZMP2_PROT, ret, size,
				...
				struct buffer_prot_info *protdesc)
{
	...
	ret = ppmp_hvc(HVC_DRM_TZMP2_UNPROT, protdesc->chunk_count, protdesc->flags,
		       protdesc->chunk_size, protdesc->bus_address);
	if (ret) {
		perr("%s: UNPROT %#x (err=%#lx,size=%#lx,cnt=%u,flg=%u)",
		     __func__, HVC_DRM_TZMP2_UNPROT, ret, size,
		     protdesc->chunk_count, protdesc->flags);
		return -EACCES;
	}
