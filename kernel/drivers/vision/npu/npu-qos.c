		...
		unsigned long action, void *nb_data)
{
	...
	if (qos_setting->info->mode == NPU_PERF_MODE_NPU_BOOST ||
			...
			qos_setting->info->mode == NPU_PERF_MODE_NPU_BOOST_PRUNE) {
	...
	npu_dvfs_activate_peripheral(action);
...
int npu_qos_probe(struct npu_system *system)
{
	...
	npu_dvfs_pm_qos_add_request(&qos_setting->npu_qos_req_npu0,
			PM_QOS_NPU0_THROUGHPUT, 0);
	npu_dvfs_pm_qos_add_request(&qos_setting->npu_qos_req_npu1,
...
#if IS_ENABLED(CONFIG_SOC_S5E9935) || IS_ENABLED(CONFIG_SOC_S5E9945)
	npu_dvfs_pm_qos_add_request(&qos_setting->npu_qos_req_npu0_max,
			PM_QOS_NPU0_THROUGHPUT_MAX,
			PM_QOS_NPU0_THROUGHPUT_MAX_DEFAULT_VALUE);
	npu_dvfs_pm_qos_add_request(&qos_setting->npu_qos_req_npu1_max,
...
#if IS_ENABLED(CONFIG_SOC_S5E9935) || IS_ENABLED(CONFIG_SOC_S5E9945)
	npu_dvfs_pm_qos_update_request(&qos_setting->npu_qos_req_npu0, 0);
	npu_dvfs_pm_qos_update_request(&qos_setting->npu_qos_req_npu1, 0);
#else
...
#if IS_ENABLED(CONFIG_SOC_S5E9935) || IS_ENABLED(CONFIG_SOC_S5E9945)
	npu_dvfs_pm_qos_update_request(&qos_setting->npu_qos_req_npu0_max,
			PM_QOS_NPU0_THROUGHPUT_MAX_DEFAULT_VALUE);
	npu_dvfs_pm_qos_update_request(&qos_setting->npu_qos_req_npu1_max,
			PM_QOS_NPU1_THROUGHPUT_MAX_DEFAULT_VALUE);
#else
