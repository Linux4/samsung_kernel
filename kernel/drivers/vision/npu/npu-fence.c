...
int npu_sync_pt_update(struct npu_sync_timeline *obj, struct npu_sync_pt *pt, unsigned int value)
{
	int ret = 0;
		...
		while (*p) {
			...
			if (cmp > 0) {
				...
				if (dma_fence_get_rcu(&other->base)) {
					npu_err("Duplicate value. please check value(%u).\n", value);
					ret = -EINVAL;
					goto unlock;
				}
				p = &parent->rb_left;
...
int npu_sync_update_out_fence(struct npu_sync_timeline *obj, int value, int fd)
{
	...
	ret = npu_sync_pt_update(obj, pt, value);
	if (ret) {
		npu_err("npu_sync_pt_update error. ret = %d\n", ret);
		return ret;
	}

	ret = dma_fence_add_callback(sync_file->fence, &sync_file->cb, npu_sync_pt_out_cb_func);
	if (ret < 0)
		npu_err("dma_fence_add_callback error. ret = %d\n", ret);
	return ret;
}
...
static int npu_sync_create_in_fence(struct npu_sync_timeline *obj, unsigned long arg)
{
	...
	ret = dma_fence_add_callback(sync_file->fence, &sync_file->cb, npu_sync_pt_in_cb_func);
	if (ret < 0)
		goto err;
...
static struct class npu_fence_class = {
	.name = FENCE_NAME,
};
