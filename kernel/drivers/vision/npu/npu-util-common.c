...
#if NCP_VERSION >= 27
	NPU_ERR_RET(validate_ncp_offset_range(ncp_header->constant_vector_offset,
		sizeof(struct constant_vector), ncp_header->constant_vector_cnt, ncp_size),
		"constant vector out of bounds. start=0x%x, cnt=%u, ncp_size=0x%lx\n",
		ncp_header->constant_vector_offset, ncp_header->constant_vector_cnt, ncp_size);

	NPU_ERR_RET(validate_ncp_offset_range(ncp_header->cu_vector_offset,
		sizeof(struct cu_vector), ncp_header->cu_vector_cnt, ncp_size),
		"cu vector out of bounds. start=0x%x, cnt=%u, ncp_size=0x%lx\n",
		ncp_header->cu_vector_offset, ncp_header->cu_vector_cnt, ncp_size);
#endif
...
#if NCP_VERSION >= 27
static int validate_ncp_cu_vector(struct ncp_header *ncp_header, size_t ncp_size)
{
	u32 cu_v_cnt, cu_v_offset;
	struct cu_vector *cu_v;
	int i;

	cu_v_cnt = ncp_header->cu_vector_cnt;
	cu_v_offset = ncp_header->cu_vector_offset;
	cu_v = (struct cu_vector *)((void *)ncp_header + cu_v_offset);

	for (i = 0; i < cu_v_cnt; i++) {

		/* Validate cu binary offset. */
		NPU_ERR_RET(validate_ncp_offset_range(
			cu_v[i].binary_offset, cu_v[i].size, 1, ncp_size),
			"cu vector binary memory out of bounds. start=0x%x, size=0x%x, ncp_size=0x%lx\n",
			cu_v[i].binary_offset, cu_v[i].size, ncp_size);
	}
...
#if NCP_VERSION >= 27
	ret = validate_ncp_cu_vector(ncp_header, ncp_size);
	if (ret)
		goto fail;
#endif
	return 0;
