...
struct vs4l_profile_ready {
	__s32 fd;               // device(dmabuf) memory fd
	__u32 mode;             // 0 (normal a.k.a. overview mode),  1 (detail mode)
	__u64 size;             // memory size size = uint64_t * (num_dd_label + num_fw_label) * max_iter
	__u64 offset;           // start offset of firmware profile data. offset = uint64_t * (num_dd_label) * max_iter
	__u64 max_iter;         // max execute iteration
};
...
#define VS4L_VERTEXIOC_PROFILE_READY		_IOW('V', 21, struct vs4l_profile_ready)
#define VS4L_VERTEXIOC_G_MAXFREQ		_IOR('V', 22, struct vs4l_freq_param)
#define VS4L_VERTEXIOC_QBUF_BUNDLE_CANCEL		_IOR('V', 23, struct vs4l_container_bundle)
