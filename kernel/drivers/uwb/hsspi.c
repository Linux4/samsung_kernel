...
static int hsspi_pre_read(struct hsspi *hsspi)
{
	...
	if (hsspi->soc->flags & STC_SOC_ODW)
		...
		/* STC_SOC_ODW not set, FW maybe late to setup HSSPI header */
		/* Retry with 30us delay */
		usleep_range(30, 35);
...
static int hsspi_thread_fn(void *data)
{
	...
	while (1) {
		...
		if (ret) {
			if (ret == -EAGAIN) {
				if (++prd_retrials >= MAX_PRD_RETRIALS) {
					dev_warn(
						&hsspi->spi->dev,
						"Max retrials for PRD %d reached, abort PRD...\n",
						prd_retrials);
					prd_retrials = 0;
					test_and_clear_bit(HSSPI_FLAGS_SS_IRQ,
							   hsspi->flags);
					hsspi->odw_cleared(hsspi);
				}
			} else {
				if (++successive_errors >=
				    MAX_SUCCESSIVE_ERRORS) {
					dev_err(&hsspi->spi->dev,
					...
					*/
					/* Wait 1s to let the QM save coredump (to be improved) */
					msleep(1000);
