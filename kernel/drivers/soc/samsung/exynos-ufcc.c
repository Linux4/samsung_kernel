...
static void update_min_limit(void)
{
	...
	if (is_released_freq(freq)) {
		...
		if (ab_enabled && ufc.low_freq_table)
...
static ssize_t ab_enabled_show(struct kobject *kobj, char *buf)
{
	return snprintf(buf, 3, "%d\n", ab_enabled);
}
static ssize_t ab_enabled_store(struct kobject *kobj, const char *buf, size_t count)
{
	int input;
...
static struct attribute *exynos_ufc_attrs[] = {
	...
	&attr_info.attr,
	&attr_debug_id.attr,
	&attr_ab_enabled.attr,
	NULL,
};
ATTRIBUTE_GROUPS(exynos_ufc);
...
static struct kobj_type ktype_ufc = {
	...
	.default_groups	= exynos_ufc_groups,
};
...
static int init_ufc_domain(struct device_node *dn)
{
	...
	if (!ufc.num_of_domain) {
		pr_err("%s: failed to init by num_of_domain.\n", __func__);
		return -EINVAL;
	}
