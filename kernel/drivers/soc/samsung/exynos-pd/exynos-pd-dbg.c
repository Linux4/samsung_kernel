...
static int exynos_pd_dbg_summary_show(struct generic_pm_domain *genpd, char *buf, int r)
{
	...
	list_for_each_entry(pm_data, &genpd->dev_list, list_node) {
		...
		r += sprintf(buf + r,"\t[DEV] : %-30s [RPM_STATUS] : %-15s\n",
					dev_name(pm_data->dev), p);
	}

	list_for_each_entry(link, &genpd->parent_links, parent_node)
		r = exynos_pd_dbg_summary_show(link->child, buf, r);
	exynos_pd_dbg_genpd_unlock(genpd);
				...
				size_t count, loff_t *ppos)
{
	...
	r = exynos_pd_dbg_summary_show(genpd, buf, r);
