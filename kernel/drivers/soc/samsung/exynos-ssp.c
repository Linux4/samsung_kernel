...
#define PMU_ALIVE_PA_BASE		(0x12860000 + 0x1E80)
...
static void exynos_ssp_print_user_list(struct device *dev)
{
	...
	while (ptr != NULL) {
		...
		ssp_info(dev, "%2u:\t%8s:\t%8lu\t%8lu\t%16lu\t%16lu\n",
			i,
			"Success",
			ptr->init_count,
			ptr->exit_count,
...
static int exynos_ssp_boot(struct device *dev)
{
	...
	if (ret != SSP_RET_OK)
		...
		ssp_info(dev, "ssp boot done: %llu\n", count);
...
static int exynos_ssp_boot_init(struct device *dev)
{
	...
	if (ret != SSP_RET_OK)
		...
		ssp_info(dev, "ssp boot init done: %llu\n", count);
...
static int exynos_ssp_boot_check(struct device *dev)
{
	...
	if (ret != SSP_RET_OK)
		...
		ssp_info(dev, "ssp boot check done: %llu\n", count);
