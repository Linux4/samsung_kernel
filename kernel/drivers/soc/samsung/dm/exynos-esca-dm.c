...
#if IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
	message.freq_req.start_time = esca_get_gptimer();
#else
...
static void exynos_acpm_dm_set_freq_sync(int dm_type, unsigned int *target_freq)
{
	...
	*target_freq = message.freq_req.target_freq;
}
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
			duration = esca_time_calc(start_time, end_time);
#else
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
	if (!is_esca_ipc_busy(exynos_dm->dm_req_ch) && !is_esca_ipc_busy(exynos_dm->fast_switch_ch))
		// enable sicd
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
	time = esca_time_calc(start_time, end_time);
#else
...
static ssize_t show_constraint_table(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < domain_constraint->num_constraints; i++) {
		...
		count += snprintf(buf + count, PAGE_SIZE, "constraint_type: %s\n",
				constraint->constraint_type == CONSTRAINT_MAX ? "MAX" : "MIN");
...
int DM_CALL(int dm_type, unsigned long *target_freq)
{
	...
	if (data->fast_switch) {
		...
		exynos_acpm_dm_set_freq_sync(dm_type, (unsigned int *)target_freq);
...
static int exynos_dm_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < dm->domain_count; i++) {
		...
		snprintf(dm->dm_data[i].constraint_table_attr.name, EXYNOS_DM_ATTR_NAME_LEN,
				"constraint_table_%s", dm->dm_data[i].dm_type_name);
