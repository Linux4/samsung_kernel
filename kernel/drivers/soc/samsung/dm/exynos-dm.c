...
#if IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
	if (!is_esca_ipc_busy(exynos_dm->fast_switch_ch)) {
		// enable sicd
...
#if defined(CONFIG_EXYNOS_ACPM) || defined(CONFIG_EXYNOS_ACPM_MODULE) || IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
		ret = esca_ipc_request_channel(exynos_dm->dev->of_node, NULL, &ch_num, &size);
		if (ret) {
			dev_err(exynos_dm->dev,
					"esca request channel is failed, id:%u, size:%u\n", ch_num, size);
			goto out;
		}
		config.cmd = cmd;
