		...
		struct freq_cstate_snapshot *fc_snap, int size)
{
	...
	for_each_cstate(state) {
		if (!fc->time[state] || !fc_snap->time[state])
			continue;
			...
			int size)
{
	...
	for_each_cstate(state) {
		if (!fc->time[state] || !fc_snap->time[state] || !fc_result->time[state])
			continue;
...
inline
unsigned int get_boundary_wo_spare(struct freq_table *freq_table, s32 size,
					s32 freq_delta_ratio, s32 cur_idx)
{
	if (freq_delta_ratio <= 0 && cur_idx + 1 < size)
...
inline
unsigned int get_boundary_freq(struct freq_table *freq_table, s32 size,
		s32 max_freq_idx, s32 freq_delta_ratio, s32 flag, s32 cur_idx)
{
	if (freq_delta_ratio <= 0)
		return get_boundary_wo_spare(freq_table, size, freq_delta_ratio, cur_idx);
...
inline
void __scale_state_in_freq(struct freq_table *freq_table, s32 size,
		s32 min_freq_idx, s32 max_freq_idx,
		...
		s32 freq_delta_ratio, s32 flag, s32 cur_idx)
{
	...
	if (target_idx == cur_idx || target_idx < 0 || target_idx >= size)
		return;

	cur_freq = freq_table[cur_idx].freq;
	target_freq = freq_table[target_idx].freq;

	boundary_freq = get_boundary_freq(freq_table, size, max_freq_idx,
				freq_delta_ratio, flag, cur_idx);
...
inline
void scale_state_in_freq(struct freq_table *freq_table, s32 size,
		s32 min_freq_idx, s32 max_freq_idx,
		u64 *active_state, u64 *clkoff_state,
		s32 freq_delta_ratio, s32 flag)
{
	...
	if (freq_delta_ratio > 0)
		for (idx = max_freq_idx; idx <= min_freq_idx - 1; idx++)
			__scale_state_in_freq(freq_table, size,
	...
	u64 *scaled_dyn_power, u64 *scaled_st_power, u32 *scaled_active_freq)
{
	...
	scale_state_in_freq(freq_table, size, min_freq_idx, max_freq_idx,
...
{
	int volt_size = 0, temp_size = 0;
	int *table = NULL;
	int freq_idx, idx, ret = -EINVAL;
                    ...
                    bool dyn_flag, bool st_flag)
{
    ...
    if (cal_table_cnt == 0) {
        pr_err("profiler: failed to get lv num = %d\n", cal_table_cnt);
        return NULL;
    }
                    ...
                    bool dyn_flag, bool st_flag)
{
	...
	if (!freq_table) {
		pr_err("profiler: there is no freq_table\n");
		return NULL;
	}
	...
	/* fill freq/volt and dynamic/static power cost */
	for (i = 0, j = table_cnt -1 ; i < table_cnt && j >= 0 ; i++) {
...
{
	struct thermal_zone_device *ref = NULL;

#if IS_ENABLED(CONFIG_THERMAL)
	ref = thermal_zone_get_zone_by_name(name);
	if (ref == ERR_PTR(-EINVAL) || ref == ERR_PTR(-ENODEV) || ref == ERR_PTR(-EEXIST)) {
		return NULL;
	}
#endif
...
EXPORT_SYMBOL(exynos_profiler_init_temp);
...
EXPORT_SYMBOL(exynos_profiler_get_temp);
