#include <legacy/exynos-gpu-profiler.h>
...
u64 gpupro_get_timeinfo(int idx)
{
	return profiler.result[PROFILER].rtimes[idx];
}

u16 gpupro_get_pidinfo(int idx)
{
	return profiler.result[PROFILER].pid_list[idx];
}
...
static int exynos_gpu_profiler_probe(struct platform_device *pdev)
{
	...
	ret = exynos_profiler_stc_set_powertable(profiler.profiler_id, profiler.table_cnt, profiler.table);
	if (ret) {
