#include <legacy/exynos-cpu-profiler.h>
...
u32 cpupro_update_mode(s32 id, int mode)
{
	...
	if (id == PROFILER_CL2) {
		...
		int cl1l_ratio, cl1h_ratio, cl2_ratio, busy_id;

		dompro = get_dom_by_profiler_id(PROFILER_CL1L);
		res = &dompro->result[PROFILER].fc_result;
		cl1l_ratio = res->ratio[CS_ACTIVE] * cpumask_weight(&dompro->cpus);

		dompro = get_dom_by_profiler_id(PROFILER_CL1H);
		res = &dompro->result[PROFILER].fc_result;
		cl1h_ratio = res->ratio[CS_ACTIVE] * cpumask_weight(&dompro->cpus);

		dompro = get_dom_by_profiler_id(PROFILER_CL2);
		res = &dompro->result[PROFILER].fc_result;
		cl2_ratio = res->ratio[CS_ACTIVE] * cpumask_weight(&dompro->cpus);

		busy_id = cl1l_ratio > cl1h_ratio ? PROFILER_CL1L : (cl1h_ratio > cl2_ratio)? PROFILER_CL1H : PROFILER_CL2;
		exynos_profiler_stc_set_busy_domain(busy_id);
	}
...
void exynos_profiler_stc_set_cur_freqlv(int id, int level);
static void cpupro_hook_freq_change(void *data, struct cpufreq_policy *policy)
{
	...
	if (dompro->profiler_id == PROFILER_CL1L || dompro->profiler_id == PROFILER_CL1H|| dompro->profiler_id == PROFILER_CL2)
		exynos_profiler_stc_set_cur_freqlv(dompro->profiler_id, new_freq_idx);
...
static int init_domain(struct domain_profiler *dompro, struct device_node *dn)
{
	...
	if (dompro->profiler_id <= PROFILER_CL2) {
		ret = exynos_profiler_stc_set_powertable(dompro->profiler_id, dompro->table_cnt, dompro->table);
		if (ret) {
