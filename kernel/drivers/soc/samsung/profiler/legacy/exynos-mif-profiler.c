#include <legacy/exynos-mif-profiler.h>
...
u32 mifpro_update_mode(s32 id, int mode)
{
	...
	if (profiler.enabled == 0 && mode == 1) {
		...
		profiler.result[PROFILER].freq_stats0_ratio = 0;
		profiler.result[PROFILER].freq_stats1_sum = 0;
		profiler.result[PROFILER].freq_stats1_ratio = 0;
		profiler.result[PROFILER].freq_stats2_sum = 0;
		profiler.result[PROFILER].freq_stats2_ratio = 0;
...
u64 mifpro_get_freq_stats0_ratio(void) { return profiler.result[PROFILER].freq_stats0_ratio; };
u64 mifpro_get_freq_stats1_ratio(void) { return profiler.result[PROFILER].freq_stats1_ratio; };
u64 mifpro_get_freq_stats2_sum(void) { return profiler.result[PROFILER].freq_stats2_sum; };
u64 mifpro_get_freq_stats2_ratio(void) { return profiler.result[PROFILER].freq_stats2_ratio; };
...
u32 mifpro_update_profile(int user)
{
	...
	int i;
	u64 total_active_time = 0, diff_ccnt = 0;
#if defined(CONFIG_SOC_S5E9925_EVT0) || defined(CONFIG_SOC_S5E8825)
	u64 ccnt = 0;
	...
	diff_ccnt += (wow_profile.ccnt - profiler.prev_wow_profile.ccnt);
	transfer_data[MIF] = (wow_profile.transfer_data[MIF] - profiler.prev_wow_profile.transfer_data[MIF]) >> 20;
	transfer_data[CPU] = (wow_profile.transfer_data[CPU] - profiler.prev_wow_profile.transfer_data[CPU]) >> 20;
	transfer_data[GPU] = (wow_profile.transfer_data[GPU] - profiler.prev_wow_profile.transfer_data[GPU]) >> 20;
	mo_count[MIF] = wow_profile.mo_count[MIF] - profiler.prev_wow_profile.mo_count[MIF];
	mo_count[CPU] = wow_profile.mo_count[CPU] - profiler.prev_wow_profile.mo_count[CPU];
	mo_count[GPU] = wow_profile.mo_count[GPU] - profiler.prev_wow_profile.mo_count[GPU];
	nr_requests[MIF] = wow_profile.nr_requests[MIF] - profiler.prev_wow_profile.nr_requests[MIF];
	nr_requests[CPU] = wow_profile.nr_requests[CPU] - profiler.prev_wow_profile.nr_requests[CPU];
	nr_requests[GPU] = wow_profile.nr_requests[GPU] - profiler.prev_wow_profile.nr_requests[GPU];

	for (i = 0; i < profiler.table_cnt; i++) {
		result->freq_stats0_avg += (result->tdata_in_state[i] << 40) / (profiler.table[i].freq / 1000);
	...
	profiler.prev_wow_profile = wow_profile;
	...
	result->freq_stats2_sum = 0;
	result->freq_stats0_ratio = 0;
	result->freq_stats1_ratio = 0;
	result->freq_stats2_ratio = 0;
	...
	if (transfer_data[MIF])
		result->freq_stats0_sum = transfer_data[MIF] * 1000000000 / fc_result->profile_time;
	if (mo_count[MIF] && nr_requests[MIF])
		result->freq_stats0_ratio = fc_result->profile_time / diff_ccnt * mo_count[MIF] / nr_requests[MIF];

	if (transfer_data[CPU])
		result->freq_stats1_sum = transfer_data[CPU] * 1000000000 / fc_result->profile_time;
	if (mo_count[CPU] && nr_requests[CPU])
		result->freq_stats1_ratio = fc_result->profile_time / diff_ccnt * mo_count[CPU] / nr_requests[CPU];

	if (transfer_data[GPU])
		result->freq_stats2_sum = transfer_data[GPU] * 1000000000 / fc_result->profile_time;
	if (mo_count[GPU] && nr_requests[GPU])
		result->freq_stats2_ratio = fc_result->profile_time / diff_ccnt * mo_count[GPU] / nr_requests[GPU];
