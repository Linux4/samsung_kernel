...
static int io_dev_recv_betp(struct io_device *iod,
			...
			size_t count, loff_t *fpos)
{
	...
	if (count <= 0) {
		gif_err("%s->%s: ERR! count (%zu) is too small\n",
	...
	if (!buff) {
		gif_err("%s->%s: ERR! kzalloc fail (count %zu)\n",
			iod->name, ld->name, count);
		return -ENOMEM;
	}

	if (copy_from_user(buff, data, count)) {
		gif_err("%s->%s: ERR! copy_from_user fail (count %zu)\n",
			iod->name, ld->name, count);
		ret = -EFAULT;
		goto exit;
	}
	...
	if (ret < 0) {
		gif_err("%s->%s: ERR! ld->send fail (err %d, count %zu)\n",
