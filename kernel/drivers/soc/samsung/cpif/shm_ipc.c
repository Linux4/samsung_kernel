...
static int cp_shmem_check_mem_map_on_cp(struct device *dev)
{
	...
	if (strncmp((const char *)&name, "MEM\0", sizeof(name))) {
		mif_err("map.version error:0x%08x. Try enc offset\n", map.version);

		memcpy(&map, base + toc->img_offset + MAP_ON_CP_OFFSET_ENC,
			sizeof(struct cp_mem_map));
		name = ntohl(map.version);
		if (strncmp((const char *)&name, "MEM\0", sizeof(name))) {
			mif_err("enc map.version error:0x%08x. Use dt information\n",
				map.version);
			return 0;
		}
	}
	...
	_cp_shmem[SHMEM_CP].size = map.secure_size;
	...
	for (i = 0; i < map.ns_map_count; i++) {
		...
		_cp_shmem[shmem_index].size = map.ns_map[i].size;
...
static int cp_shmem_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MAX_CP_SHMEM; i++) {
		...
		mif_info("rmem:%d index:%d %s 0x%08lx 0x%08x %d\n",
