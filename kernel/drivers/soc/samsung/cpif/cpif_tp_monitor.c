...
/* GRO flush timeout */
static void tpmon_set_gro_timeout(struct tpmon_data *data)
{
	struct mem_link_device *mld = container_of(data->tpmon->ld,
			struct mem_link_device, link_dev);
	long timeout;
...
/* GRO enable/disable */
static void tpmon_set_gro_enable(struct tpmon_data *data)
{
	enum gro_opt original_opt = GRO_NONE;
	enum gro_opt opt = GRO_NONE;
	u32 val = 0;

	if (!data->enable)
		return;

	get_gro_support(&original_opt);
	mif_info("gro enable option was %d\n", original_opt);
	...
	if (val) {
		...
		case TPMON_PROTO_TCP:
			...
			break;
		default:
			mif_err("proto is invalid:%d\n", data->proto);
			return;
		}
	} else {
		opt = GRO_NONE;
	}
	mif_info("set gro enable option:%d\n", opt);
	if (original_opt == opt) {
		mif_info("opt is same as before:%d\n", opt);
		return;
	}

	set_gro_support(opt);
	get_gro_support(&opt);
	mif_info("gro enable option is changed to %d\n", opt);
}

/* IRQ affinity */
#if IS_ENABLED(CONFIG_MCPS)
static void tpmon_set_irq_mask_to_mcps(u32 val, bool is_mask)
{
	if (mcps_enable) {
		char mask[MAX_IRQ_AFFINITY_STRING];

		snprintf(mask, MAX_IRQ_AFFINITY_STRING, "%x", (is_mask ? val : 1 << val));
		mif_info("set %s to mcps\n", mask);
		set_mcps_cp_irq_mask(mask);
	}
}
#else
static inline void tpmon_set_irq_mask_to_mcps(u32 val, bool is_mask) { return; }
#endif
...
static void tpmon_set_irq_affinity_mbox(struct tpmon_data *data)
{
	...
	tpmon_set_irq_mask_to_mcps(val, true);
	for (i = 0; i < num_queue; i++) {
...
static void tpmon_set_irq_affinity_dit(struct tpmon_data *data)
{
	...
	mif_info("%s (CPU:%u)\n", data->name, cpu[0]);
	tpmon_set_irq_mask_to_mcps(cpu[0], false);
		...
		case TPMON_TARGET_CPU_CL2:
			if (policy->cpu == data->extra_idx) {
				...
				freq_qos_tracer_add_request(&policy->constraints,
					data->extra_data, FREQ_QOS_MIN, FREQ_QOS_MIN_DEFAULT_VALUE);
#else
...
#if IS_ENABLED(CONFIG_ARM_FREQ_QOS_TRACER)
				freq_qos_tracer_add_request(&policy->constraints,
					data->extra_data, FREQ_QOS_MAX, FREQ_QOS_MAX_DEFAULT_VALUE);
#else
				freq_qos_add_request(&policy->constraints,
					data->extra_data, FREQ_QOS_MAX, FREQ_QOS_MAX_DEFAULT_VALUE);
#endif
...
static void tpmon_boost_work(struct work_struct *ws)
{
	...
	list_for_each_entry(data, &tpmon->all_data_list, data_node) {
		...
		if (mcps_enable) {
			...
			case TPMON_TARGET_GRO_TIMEOUT:
			case TPMON_TARGET_GRO_ENABLE:
				continue;
			default:
				break;
			}
...
static int tpmon_set_cpufreq(struct tpmon_data *data)
{
	...
	case TPMON_TARGET_CPU_CL0:
		...
		value = FREQ_QOS_MIN_DEFAULT_VALUE;
		break;
	case TPMON_TARGET_CPU_CL0_MAX:
	...
	if (policy->cpu == data->extra_idx) {
		mif_info("freq_qos_add_request for cpu%d type%d val%d\n",
			policy->cpu, qos_type, value);
#if IS_ENABLED(CONFIG_ARM_FREQ_QOS_TRACER)
		freq_qos_tracer_add_request(&policy->constraints,
			data->extra_data, qos_type, value);
#else
		freq_qos_add_request(&policy->constraints,
			data->extra_data, qos_type, value);
#endif
...
static int tpmon_set_target(struct tpmon_data *data)
{
	...
	case TPMON_TARGET_GRO_TIMEOUT:
		data->set_data = tpmon_set_gro_timeout;
		break;

	case TPMON_TARGET_GRO_ENABLE:
		data->set_data = tpmon_set_gro_enable;
		break;
