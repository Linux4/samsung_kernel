		...
		u8 *data)
{
	...
	if (seq < last_seq[dir][ring]) {
		...
		if (++out_count[dir][ring] > 5) {
			...
			if (dc->ld) {
				dc->ld->link_trigger_cp_crash(to_mem_link_device(dc->ld),
		...
		unsigned int src_rp)
{
	...
	DIT_INDIRECT_CALL(dc, set_desc_pos, dir, desc_info, tail);
	...
	for (i = 0; i < read; i++) {
		...
		if (desc_info->dst_page_pool[ring_num]) {
			...
			if (initial)
				...
				dst_skb[dst_rp_pos] = napi_build_skb(data, len);
		} else if (initial) {
			gfp_mask = GFP_KERNEL;
			if (ring_num == DIT_DST_DESC_RING_0)
				gfp_mask = GFP_ATOMIC;
...
int dit_init(struct link_device *ld, enum dit_init_type type, enum dit_store_type store)
{
	...
	if (store == DIT_STORE_RESTORE) {
		...
		DIT_INDIRECT_CALL(dc, set_reg_upstream, NULL, true);
	}
		...
		const char *buf, size_t count)
{
	...
	if (iod)
		DIT_INDIRECT_CALL(dc, set_reg_upstream, iod->ndev, false);
	else
		DIT_INDIRECT_CALL(dc, set_reg_upstream, NULL, false);
