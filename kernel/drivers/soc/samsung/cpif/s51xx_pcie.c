...
{
	exynos_pcie_l1ss_ctrl(enable, PCIE_L1SS_CTRL_MODEM_IF, 0);
}
			...
			const struct pci_device_id *ent)
{
	...
	u32 val, db_addr = 0, pcie_outbound_base = 0;
	...
	if (db_addr != 0x0) {
		if (of_property_read_u32(mc_dev->of_node, "pcie_outbound_base",
					&pcie_outbound_base)) {
			dev_err(dev, "Failed to parse the PCIe outbound basee address\n");
			return -EINVAL;
		}

		if (of_property_read_u32(mc_dev->of_node, "pci_db_offset",
					&s51xx_pcie->dbaddr_offset)) {
			dev_err(dev, "Failed to parse the EP DB offset\n");
			return -EINVAL;
		}

		pr_err("Release BAR resources.\n");
		for (i = 0; i < 6; i++) {
		...
		s51xx_pcie->dbaddr_base = pcie_outbound_base;
		/* Write Doorbell address to BAR0 */
		pci_write_config_dword(pdev, PCI_BASE_ADDRESS_0, db_addr);
		pci_read_config_dword(pdev, PCI_BASE_ADDRESS_0, &val);
		val &= PCI_BASE_ADDRESS_MEM_MASK;
		dev_info(dev, "Set BAR0 as doorbell address : 0x%x(read val : 0x%x) offset : 0x%x\n",
				db_addr, val, (unsigned int)s51xx_pcie->dbaddr_offset);
		pr_err("Set Doorbell register address. - Base doorbell Addr + offset : %#llx\n",
				s51xx_pcie->dbaddr_base + s51xx_pcie->dbaddr_offset);
		s51xx_pcie->doorbell_addr = devm_ioremap(&pdev->dev,
				s51xx_pcie->dbaddr_base + s51xx_pcie->dbaddr_offset, SZ_4K);
		...
		/* If CP's Class Code is not defined, assign resource directly.
		   ...
		   if (ret)
			pr_info("Can't assign resource of BAR0(%d)\n", ret);
		*/
...
void print_msi_register(struct pci_dev *pdev)
{
	...
	pci_read_config_dword(pdev, 0x5c, &msi_val);
	mif_info("MSI MsgData Reg(0x5c) : 0x%x\n", msi_val);
	...
	if (msi_val == 0x0) {
		...
		pci_read_config_dword(pdev, 0x5c, &msi_val);
		mif_info("Recheck - MSI MsgData Reg : 0x%x (0x5c)\n", msi_val);
	}
}
