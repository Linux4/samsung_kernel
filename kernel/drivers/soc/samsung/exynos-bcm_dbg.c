...
|| IS_ENABLED(CONFIG_EXYNOS_BCM_DBG_GNR) \
|| IS_ENABLED(CONFIG_EXYNOS_ESCA) || IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
...
{
	int ret = 0;
#if IS_ENABLED(CONFIG_EXYNOS_ESCA) || IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
	int i = 0;
	struct ipc_config config;
#elif IS_ENABLED(CONFIG_EXYNOS_ADV_TRACER)
	int i = 0;
	struct adv_tracer_ipc_cmd config;
#elif IS_ENABLED(CONFIG_EXYNOS_BCM_DBG_GNR)
	int i = 0;
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCA) || IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
static void bcm_dbg_handler(unsigned int *cmd, unsigned int size)
{
	struct exynos_bcm_dbg_data *data = bcm_dbg_data;
	if(cmd[1] == BCM_FLOW_PAUSED) {
		complete(&data->flow_ctrl_completion);
	}
}
#elif IS_ENABLED(CONFIG_EXYNOS_ADV_TRACER)
...
static int exynos_bcm_dbg_ipc_channel_request(struct exynos_bcm_dbg_data *data)
{
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCA) || IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
	ret = esca_ipc_request_channel(data->ipc_node,
				(ipc_callback)NULL,
	...
	if(data->ipc_noti_node) {
		ret = esca_ipc_request_channel(data->ipc_noti_node,
					(ipc_callback)bcm_dbg_handler,
					&data->ipc_noti_ch_num, &data->ipc_noti_size);

		BCM_INFO("ESCAESCAESCA%s: ipc noti channel info: ch_num(%u), size(%u)\n", __func__,
					data->ipc_noti_ch_num, data->ipc_noti_size);
	}
#elif IS_ENABLED(CONFIG_EXYNOS_ADV_TRACER)
...
{
#if IS_ENABLED(CONFIG_EXYNOS_ESCA) || IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
	esca_ipc_release_channel(data->ipc_node, data->ipc_ch_num);
	esca_ipc_release_channel(data->ipc_noti_node, data->ipc_noti_ch_num);
#elif IS_ENABLED(CONFIG_EXYNOS_ADV_TRACER)
...
static int exynos_bcm_dbg_ktime_sync(struct exynos_bcm_ipc_base_info *ipc_base_info,
					u64 ktime, struct exynos_bcm_dbg_data *data)
{
	unsigned int cmd[4] = {0, 0, 0, 0};
...
static int exynos_bcm_dbg_rtc_sync(struct exynos_bcm_ipc_base_info *ipc_base_info,
					u32 rtc_tick, struct exynos_bcm_dbg_data *data)
{
	unsigned int cmd[4] = {0, 0, 0, 0};
	int ret = 0;
	unsigned long flags;
...
int exynos_bcm_dbg_change_event_format(u32 format_idx)
{
	struct exynos_bcm_dbg_data *data = bcm_dbg_data;
	struct exynos_bcm_ipc_base_info ipc_base_info;
	struct exynos_bcm_event bcm_event;
	...
	if (format_idx > 0 && format_idx >= data->define_event_max)
		return -EINVAL;
...
EXPORT_SYMBOL(exynos_bcm_dbg_change_event_format);

int exynos_bcm_dbg_restore_event_format(void)
{
	return exynos_bcm_dbg_change_event_format(
					bcm_dbg_data->default_define_event);
}
EXPORT_SYMBOL(exynos_bcm_dbg_restore_event_format);
		...
		struct bin_attribute *battr, char *buf, loff_t off, size_t size)
{
	...
	if (suspend) {
		suspend = true;
		exynos_bcm_time_pair_sync(bcm_dbg_data);
	}
		...
		struct bin_attribute *battr, char *buf, loff_t off, size_t size)
{
	...
	struct exynos_bcm_ipc_base_info ipc_base_info;
	unsigned int ip_enable;
	unsigned long long bitmap_low, bitmap_high;
	int ret, bcm_ip_index;

	if (off != 0)
		return size;

	ret = sscanf(buf, "%llx %llx", &bitmap_low, &bitmap_high);
	if (ret != 2)
		return -EINVAL;

	for (bcm_ip_index = 0; bcm_ip_index < data->bcm_ip_nr; bcm_ip_index++) {
		if(bcm_ip_index < 64)
			ip_enable = (bitmap_low >> bcm_ip_index) & 0x1;
		else
			ip_enable = (bitmap_high >> (bcm_ip_index - 64)) & 0x1;

		if (data->initial_run_ip[bcm_ip_index] == 0)
			continue;
...
static int exynos_bcm_dbg_flow_ctrl(struct exynos_bcm_ipc_base_info *ipc_base_info,
					struct exynos_bcm_dbg_data *data,
					enum bcm_flow_state state)
{
	unsigned int cmd[4] = {0, 0, 0, 0};
	int ret = 0;
	unsigned long flags;
					...
					size_t count, loff_t *ppos)
{
	...
	ssize_t trans_count;
	loff_t pos = ppos[0];
	static long long cur_addr;

	if(pos == 0) {
		if(data->bcm_run_state == BCM_RUN) {
			init_completion(&data->flow_ctrl_completion);
			exynos_bcm_dbg_set_base_info(&ipc_base_info, BCM_EVT_FLOW_CTRL,
							BCM_EVT_SET, 0);
			exynos_bcm_dbg_flow_ctrl(&ipc_base_info, data, BCM_FLOW_PRE_PAUSE);
			if(!wait_for_completion_timeout(&data->flow_ctrl_completion,msecs_to_jiffies(1000)))
				panic("BCM flow control fail\n");
		}
		cur_addr = (*((u32 *)data->dump_addr.v_addr));
	} else if (pos >= cur_addr) {
		if(data->bcm_run_state == BCM_RUN) {
			exynos_bcm_dbg_set_base_info(&ipc_base_info, BCM_EVT_FLOW_CTRL,
							BCM_EVT_SET, 0);
			exynos_bcm_dbg_flow_ctrl(&ipc_base_info, data, BCM_FLOW_ONGOING);
		}
		return 0;
	}

	if(!!((cur_addr - pos) / count))
		trans_count = count;
	else
		trans_count = (cur_addr- pos);
...
static BIN_ATTR(dump_addr_info, 0440, show_dump_addr_info, NULL, 0);
...
static int exynos_bcm_dbg_dump_config(struct exynos_bcm_dbg_data *data)
{
	...
	rmem_np = of_parse_phandle(data->dev->of_node, "memory-region", 0);
	if (rmem_np == NULL) {
		BCM_ERR("%s: failed get reserved memory-region for bcm rmem_np\n", __func__);
		return -ENODEV;
	}

	rmem = of_reserved_mem_lookup(rmem_np);
	if (rmem == NULL) {
		BCM_ERR("%s: failed get reserved memory-region for bcm\n", __func__);
		return -EINVAL;
	}

	data->dump_addr.p_addr = rmem->base;
	data->dump_addr.p_size = rmem->size;
	data->dump_addr.buff_size = rmem->size;
	if (!data->dump_addr.p_addr || !data->dump_addr.p_size) {
