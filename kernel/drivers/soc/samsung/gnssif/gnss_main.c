					...
					struct gnss_pdata *pdata)
{
	...
	for_each_child_of_node(bn, child) {
		struct gnss_baaw *window = &pdata->wnd_set[i];
		u32 helper = 0;
		u64 ap_start;

		gif_dt_read_u32(child, "baaw,offset", window->offset);
		gif_dt_read_u32(child, "baaw,start", window->gnss_start);
		gif_dt_read_u32_noerr(child, "baaw,helper", helper);
		if (helper == USE_SHMEM || helper == USE_SHMEM_ADDR_IDX) {
			...
			gif_dt_read_u32(child, "baaw,ap_start", ap_start);
		}
		...
		window->baaw_reg = devm_ioremap(dev, base_addr + window->offset, SZ_16);
		if (!window->baaw_reg) {
