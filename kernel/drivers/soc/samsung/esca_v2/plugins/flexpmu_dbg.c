...
int esca_get_nfc_log_buf(struct nfc_clk_req_log **buf, u32 *last_ptr, u32 *len)
{
...
u32 esca_get_apsoc_dsupd_count(void)
{
	return apsoc_down_cnt[0] - (apsoc_down_cnt[3] + apsoc_down_cnt[4]);
}
EXPORT_SYMBOL_GPL(esca_get_apsoc_dsupd_count);

u32 esca_get_apsoc_dsupd_early_wakeup_count(void)
{
	return apsoc_ewkup_cnt[0] - (apsoc_ewkup_cnt[3] + apsoc_ewkup_cnt[4]);
}
EXPORT_SYMBOL_GPL(esca_get_apsoc_dsupd_early_wakeup_count);

u32 esca_get_apsoc_sicd_early_wakeup_count(void)
{
	return apsoc_ewkup_cnt[3];
}
EXPORT_SYMBOL_GPL(esca_get_apsoc_sicd_early_wakeup_count);

u32 esca_get_apsoc_sleep_early_wakeup_count(void)
{
	return apsoc_ewkup_cnt[4];
}
EXPORT_SYMBOL_GPL(esca_get_apsoc_sleep_early_wakeup_count);

u32 esca_get_apsoc_count(void)
{
	return apsoc_down_cnt[0];
}
EXPORT_SYMBOL_GPL(esca_get_apsoc_count);

u32 esca_get_apsicd_count(void)
{
	return apsoc_down_cnt[3];
}
EXPORT_SYMBOL_GPL(esca_get_apsicd_count);

u32 esca_get_apsleep_count(void)
{
	return apsoc_down_cnt[4];
}
EXPORT_SYMBOL_GPL(esca_get_apsleep_count);

u32 esca_get_mifdn_count(void)
{
	return mif_down_cnt[0];
}
EXPORT_SYMBOL_GPL(esca_get_mifdn_count);

u32 esca_get_mifdn_sicd_count(void)
{
	return mif_down_cnt[3];
}
EXPORT_SYMBOL_GPL(esca_get_mifdn_sicd_count);

u32 esca_get_mifdn_sleep_count(void)
{
	return mif_down_cnt[4];
}
EXPORT_SYMBOL_GPL(esca_get_mifdn_sleep_count);
/*notify to flexpmu, it is SICD w MIF(is_dsu_cpd=0) or SICD wo MIF(is_dsu_cpd=1).*/
u32 esca_noti_dsu_cpd(bool is_dsu_cpd)
{
	flxp_cpu_pm->mif_always_on = is_dsu_cpd;
	return 0;
}
EXPORT_SYMBOL_GPL(esca_noti_dsu_cpd);

u32 esca_get_dsu_cpd(void)
{
	return flxp_cpu_pm->mif_always_on;
}
EXPORT_SYMBOL_GPL(esca_get_dsu_cpd);

u32 esca_get_mif_request(void)
{
	return flxp_mif->requests;
}
EXPORT_SYMBOL_GPL(esca_get_mif_request);

void esca_print_mif_request(void)
{
	int i = 0;
...
EXPORT_SYMBOL_GPL(esca_print_mif_request);
...
static ssize_t flexpmu_dbg_sw_flag_read(int fid, char *buf)
{
	ssize_t ret = 0;
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s : 0x%x\n", "CPU hotplug", flxp_cpu_pm->cpuhp_flag);
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s : 0x%x\n", "Cluster status", flxp_cpu_pm->cluster_status);
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s : 0x%x\n", "MIF requester", esca_get_mif_request());
	return ret;
}
...
static ssize_t flexpmu_dbg_requester_profiler_read(int fid, char *buf)
{
	...
	for (i = 0; i < nr_masters; i++) {
		mif_req.active_req_tick = requester_mif[i].req_start_time;
		mif_req.last_rel_tick = requester_mif[i].rel_start_time;
		...
		if (mif_req.last_rel_tick >= mif_req.active_req_tick) {
		...
		size_t count, loff_t *ppos)
{
	...
	case FID_LATENCY_PROFILER:
		if (buf[0] == '0') esca_enable_flexpmu_profiler(LATENCY_PROFILER_DISABLE);
		if (buf[0] == '1') esca_enable_flexpmu_profiler(LATENCY_PROFILER_ENABLE);
		break;
	...
	case FID_REQUESTER_PROFILER:
		if (buf[0] == '0') esca_enable_flexpmu_profiler(REQUESTER_PROFILER_DISABLE);
		if (buf[0] == '1') esca_enable_flexpmu_profiler(REQUESTER_PROFILER_ENABLE);
		break;
	default:
		break;
	}
...
static int flexpmu_dbg_probe(struct platform_device *pdev)
{
	...
	const __be32 *prop;
	unsigned int len = 0;
	unsigned int esca_layer, pid;
	unsigned int disable_mifdown = 0;
	u64 __iomem flexpmu_dbg_addr = 0, sram_base = 0;
	...
	prop = of_get_property(pdev->dev.of_node, "esca-layer", &len);
	if (prop) {
		esca_layer = be32_to_cpup(prop);
		if ((esca_layer >= NR_ESCA_LAYERS)) {
			pr_err("%s %s: Invalid ESCA layer in DT\n",
					EXYNOS_FLEXPMU_DBG_PREFIX, __func__);
			return -EINVAL;
		}
	} else {
		pr_err("%s %s: Missing ESCA layer in DT\n",
				EXYNOS_FLEXPMU_DBG_PREFIX, __func__);
		return -ENODEV;
	}

	prop = of_get_property(pdev->dev.of_node, "pid", &len);
	if (!prop) {
		pr_err("%s %s: can not read pid in DT\n",
				EXYNOS_FLEXPMU_DBG_PREFIX, __func__);
		ret = -EINVAL;
		goto err_dbgfs_probe;
	}
	pid = be32_to_cpup(prop);

	get_plugin_dbg_addr(esca_layer, pid, &sram_base, &flexpmu_dbg_addr);

	if (!flexpmu_dbg_addr) {
		pr_err("%s %s: can not get plugin_dbg_addr \n",
				EXYNOS_FLEXPMU_DBG_PREFIX, __func__);
		ret = -EINVAL;
		goto err_dbgfs_probe;
	}

	flxp_dbg = (struct flexpmu_dbg *)(flexpmu_dbg_addr);

	flxp_cpu_pm = (struct flexpmu_cpu_pm *)(sram_base + flxp_dbg->cpu_pm);
	flxp_mif = (struct flexpmu_mif *)(sram_base + flxp_dbg->mif);

	apsoc_down_cnt = (u32 *)(sram_base + flxp_cpu_pm->p_apsoc_down_cnt);
	apsoc_ewkup_cnt = (u32 *)(sram_base + flxp_cpu_pm->p_apsoc_ewkup_cnt);
	mif_down_cnt = (u32 *)(sram_base + flxp_mif->p_down_cnt);
	mif_req_info = (struct mif_req_info *)(sram_base + flxp_mif->req_info);

	of_property_read_u32(pdev->dev.of_node, "nfc-clkreq-idx", &nfc_clkreq_idx);
	flxp_eclkbuf = (struct flexpmu_ext_clk_buf *)(sram_base + flxp_dbg->eclkbuf);

	latency_cpu = (struct latency_info *)(sram_base + flxp_dbg->latency_cpu);
	latency_cluster = (struct latency_info *)(sram_base + flxp_dbg->latency_cluster);
	latency_pd = (struct latency_info *)(sram_base + flxp_dbg->latency_pd);
	latency_soc = (struct latency_info *)(sram_base + flxp_dbg->latency_soc);
	latency_mif = (struct latency_info *)(sram_base + flxp_dbg->latency_mif);
	requester_mif = (struct requester_info *)(sram_base + flxp_dbg->requester_mif);
	...
	esca_enable_flexpmu_profiler(REQUESTER_PROFILER_ENABLE);
