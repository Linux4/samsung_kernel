...
{
	struct cmucal_pll *pll;
	unsigned int pdiv, sdiv, pll_con0, fdiv = 0;
	short kdiv = 0;
	unsigned long long mdiv, fout;
	...
	if (is_normal_pll(pll)) {
		...
		if (pll->type == pll_0716x || pll->freq_type == HIGH_FREQ_PLL ||
		    pll->type == frd_4601_ipll || pll->type == frd_5008_ipll || pll->type == frd_4702_tpll || pll->type == frd_5602_ipll)
			fout *= 2;
...
static void ra_get_pll_address(struct cmucal_clk *clk)
{
	...
	/* status_div */
	if (clk->status_idx != EMPTY_CAL_ID) {
...
#if !IS_ENABLED(CONFIG_CMUCAL_V2)
