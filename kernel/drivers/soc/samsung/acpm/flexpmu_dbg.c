...
u32 acpm_get_apsoc_dsupd_count(void)
{
	return apsoc_down_cnt[0] - (apsoc_down_cnt[3] + apsoc_down_cnt[4]);
}
EXPORT_SYMBOL_GPL(acpm_get_apsoc_dsupd_count);

u32 acpm_get_apsoc_dsupd_early_wakeup_count(void)
{
	return apsoc_ewkup_cnt[0] - (apsoc_ewkup_cnt[3] + apsoc_ewkup_cnt[4]);
}
EXPORT_SYMBOL_GPL(acpm_get_apsoc_dsupd_early_wakeup_count);

u32 acpm_get_apsoc_sicd_early_wakeup_count(void)
{
	return apsoc_ewkup_cnt[3];
}
EXPORT_SYMBOL_GPL(acpm_get_apsoc_sicd_early_wakeup_count);

u32 acpm_get_apsoc_sleep_early_wakeup_count(void)
{
	return apsoc_ewkup_cnt[4];
}
EXPORT_SYMBOL_GPL(acpm_get_apsoc_sleep_early_wakeup_count);

u32 acpm_get_apsoc_count(void)
{
	return apsoc_down_cnt[0];
}
EXPORT_SYMBOL_GPL(acpm_get_apsoc_count);

u32 acpm_get_apsicd_count(void)
{
	return apsoc_down_cnt[3];
}
EXPORT_SYMBOL_GPL(acpm_get_apsicd_count);

u32 acpm_get_apsleep_count(void)
{
	return apsoc_down_cnt[4];
}
EXPORT_SYMBOL_GPL(acpm_get_apsleep_count);
...
u32 acpm_get_mifdn_sicd_count(void)
{
	return mif_down_cnt[3];
}
EXPORT_SYMBOL_GPL(acpm_get_mifdn_sicd_count);

u32 acpm_get_mifdn_sleep_count(void)
{
	return mif_down_cnt[4];
}
EXPORT_SYMBOL_GPL(acpm_get_mifdn_sleep_count);
...
static ssize_t flexpmu_dbg_sw_flag_read(int fid, char *buf)
{
	...
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s : 0x%x\n", "MIF requester", acpm_get_mif_request());
	return ret;
}
...
static ssize_t flexpmu_dbg_lpm_count_read(int fid, char *buf)
{
	ssize_t ret = 0;
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s: %d\n", "APSOC CNT:", acpm_get_apsoc_count());
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s: %d\n", "APSOC DSUPD:",
			acpm_get_apsoc_dsupd_count());
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s: %d\n", "APSOC DSUPD EARLY WAKEUP:",
			acpm_get_apsoc_dsupd_early_wakeup_count());
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s: %d\n", "APSOC SICD:", acpm_get_apsicd_count());
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s: %d\n", "APSOC SICD EARLY WAKEUP:",
			acpm_get_apsoc_sicd_early_wakeup_count());
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s: %d\n", "APSOC SLEEP:", apsoc_down_cnt[4]);
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s: %d\n", "APSOC SLEEP EARLY WAKEUP:",
			acpm_get_apsoc_sleep_early_wakeup_count());

	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s: %d\n", "MIF CNT:", acpm_get_mifdn_count());
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s: %d\n", "MIF SICD:", acpm_get_mifdn_sicd_count());
	ret += snprintf(buf + ret, BUF_SIZE - ret,
			"%s: %d\n", "MIF SLEEP:", acpm_get_mifdn_sleep_count());
	return ret;
}
...
static int flexpmu_dbg_probe(struct platform_device *pdev)
{
	...
	acpm_enable_flexpmu_profiler(REQUESTER_PROFILER_ENABLE);
