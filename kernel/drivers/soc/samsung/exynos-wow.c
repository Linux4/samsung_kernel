...
static int exynos_wow_accumulator(struct exynos_wow_data *data)
{
	...
	for (i = 0; i < data->nr_ip; i++) {
		...
		for (j = 0; j < ip_data->nr_base; j++) {
			...
			rw_data = readl(base + WOW_PPC_PMCNT1) * (u64)ip_data->nr_ppc;
			...
			// Update WoW IP Data to profile
			if (!j) {
				data->profile.ccnt[ip_type] += ccnt;
				data->profile.active[ip_type] += active;
			}
			data->profile.transfer_data[ip_type] += (rw_data * ip_data->bus_width);
			data->profile.nr_requests[ip_type] += rw_request;
			data->profile.mo_count[ip_type] += mo_count;

			// update total counter
			if (ip_type != MIF) {
				data->profile.transfer_data_sum += (rw_data * ip_data->bus_width);

				if (ip_type != IRP && total_active < active) {
					total_ccnt = ccnt;
					total_active = active;
				}
			}
	...
	data->profile.total_ccnt += total_ccnt;
	data->profile.total_active += total_active;
