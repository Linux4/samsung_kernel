...
struct afm_domain {
	...
	u8				orig_pmic_reg;
	unsigned int			orig_afm_warn_level;
	bool				gen_interrupt_enabled;
...
extern int sub2_pmic_read_reg(struct i2c_client *i2c, u8 reg, u8 *val);
extern int sub2_pmic_update_reg(struct i2c_client *i2c, u8 reg, u8 val, u8 mask);
extern int sub2_pmic_get_i2c(struct i2c_client **i2c);
...
static unsigned int get_afm_target_max_limit(struct afm_domain *afm_dom)
{
	...
	if (index < 0)
		index = 0;
	else if (index >= stats->max_state)
		index = stats->max_state - 1;
...
static int afm_gen_interrupt_control(struct afm_domain *afm_dom, int enable)
{
	if (enable) {
		if (afm_dom->gen_interrupt_enabled)
			return -1;
		if (afm_dom->pmic_update_reg(afm_dom->i2c, afm_dom->s2mps_afm_offset,
					S2MPS_AFM_WARN_LOWEST_LVL, S2MPS_AFM_WARN_LVL_MASK)) {
			pr_err("Failed to update pmic reg");
			return -1;
		}

		afm_dom->gen_interrupt_enabled = true;
	} else {
		if (!afm_dom->gen_interrupt_enabled)
			return -1;
		if (afm_dom->pmic_update_reg(afm_dom->i2c, afm_dom->s2mps_afm_offset,
					afm_dom->orig_afm_warn_level, S2MPS_AFM_WARN_LVL_MASK)) {
			pr_err("Failed to update pmic reg");
			return -1;
		}

		afm_dom->gen_interrupt_enabled = false;
	}
	return 0;
}
...
static ssize_t
afm_gen_interrupt_show(struct kobject *kobj, char *buf)
{
	struct afm_domain *afm_dom = to_afm_dom(kobj);
	int ret;
...
static ssize_t
afm_gen_interrupt_store(struct kobject *kobj, const char *buf, size_t count)
{
	unsigned int val;
	struct afm_domain *afm_dom = to_afm_dom(kobj);
	if (sscanf(buf, "%d", &val) != 1)
	...
	if (afm_gen_interrupt_control(afm_dom, val))
		return -EINVAL;
...
static ssize_t
reactor_enabled_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret;
...
static AFM_ATTR_RW(gen_interrupt);

static DEVICE_ATTR_RO(reactor_enabled);
...
static struct attribute *exynos_afm_attrs[] = {
	...
	&afm_attr_gen_interrupt.attr,
	NULL,
};
ATTRIBUTE_GROUPS(exynos_afm);
...
static struct kobj_type ktype_afm = {
	...
	.default_groups	= exynos_afm_groups,
};
...
static int alloc_pmic_function(struct afm_domain *afm_dom)
{
	...
	switch (afm_dom->interrupt_src) {
		...
		case SUB2_PMIC:
			afm_dom->pmic_update_reg = sub2_pmic_update_reg;
			afm_dom->pmic_get_i2c = sub2_pmic_get_i2c;
			afm_dom->pmic_read_reg = sub2_pmic_read_reg;
			break;
#endif
		default:
...
static int init_afm_domain(struct device_node *dn, struct platform_device *pdev)
{
	...
	if (afm_dom->pmic_read_reg(afm_dom->i2c, afm_dom->s2mps_afm_offset, &afm_dom->orig_pmic_reg)) {
		dev_err(&pdev->dev, "Failed to get pmic register\n");
		return -ENODEV;
	}

	afm_dom->orig_afm_warn_level = afm_dom->orig_pmic_reg & S2MPS_AFM_WARN_LVL_MASK;
...
#if defined(CONFIG_SOC_S5E9945)
	if (sub2_pmic_get_i2c(&regulator_i2c)) {
		return -ENODEV;
	}
#endif
...
static void exynos_afm_free_domain(struct platform_device *pdev, struct exynos_afm_data *afm_data)
{
	struct afm_domain *afm_dom;

	list_for_each_entry(afm_dom, &afm_data->afm_domain_list, list) {
		if (afm_dom->irq) {
			irq_set_affinity_hint(afm_dom->irq, NULL);
			pr_info("exynos-afm: domain %d: free irq affinity", afm_dom->cpu);
			devm_free_irq(&pdev->dev, afm_dom->irq, afm_data);
		}
		kobject_del(&afm_dom->kobj);
	}
}
...
static int exynos_afm_probe(struct platform_device *pdev)
{
	...
	if (sysfs_create_file(&(pdev->dev.kobj), &dev_attr_reactor_enabled.attr))
		pr_err("exynos-afm: failed to create reactor enabled node");
...
free_data:
	exynos_afm_free_domain(pdev, afm_data);
	kfree(afm_data);
	return -ENODEV;
}
