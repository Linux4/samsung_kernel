...
DEF_REG_STRUCT(CPUACTLR5)

/* klein */
DEF_REG_STRUCT(CMPXACTLR)

/* herc */
DEF_REG_STRUCT(CPUACTLR5_v82)
DEF_REG_STRUCT(CPUACTLR6_v82)
...
static struct core_register arm_klein_regs[] = {
	SET_CORE_REG(SCTLR, 0),
	SET_CORE_REG(MAIR, 0),
	SET_CORE_REG(MPIDR, 0),
	SET_CORE_REG(MIDR, 0),
...
static struct core_register arm_a78_regs[] = {
	...
	SET_CORE_REG(CPUACTLR_v82, 1),
	SET_CORE_REG(CPUACTLR2_v82, 1),
	SET_CORE_REG(CPUACTLR5_v82, 1),
	SET_CORE_REG(CPUACTLR6_v82, 1),
	SET_CORE_REG(CPUECTLR_v82, 1),
};
...
static struct core_register arm_pamir_regs[] = {
	SET_CORE_REG(SCTLR, 0),
	SET_CORE_REG(MAIR, 0),
	SET_CORE_REG(MPIDR, 0),
	SET_CORE_REG(MIDR, 0),
	...
	SET_CORE_REG(CPUECTLR_v82, 1),
	SET_CORE_REG(CPUACTLR2_v82, 1),
	SET_CORE_REG(CPUACTLR5, 1),
	SET_CORE_REG(PPMPDPCR, 1),
};

static struct core_register arm_makalu_regs[] = {
	SET_CORE_REG(SCTLR, 0),
	SET_CORE_REG(MAIR, 0),
	SET_CORE_REG(MPIDR, 0),
	SET_CORE_REG(MIDR, 0),
...
#define arm_hayes_regs arm_klein_regs
#define arm_hunter_regs arm_makalu_regs
...
static char *arm_core_names2[] = {"", "HERC", "", "", "HERA", "", "KLEIN", "MATTERHORN", "MONTBLANC", "", "", "", "",
		"MAKALU", "MAKALU_ELP"};
static char *arm_core_names3[] = {"HAYES", "HUNTER", "HUNTER_ELP"};
...
static int creg_thread_func(void *data)
{
	...
	case ARM:
		...
		if (idx >= 0x80) {
			idx -= 0x80;
			if (idx < (sizeof(arm_core_names3)/sizeof(char *)))
				core_name = arm_core_names3[idx];
			else
				return -EINVAL;
		} else if (idx > 0x40) {
			idx -= 0x40;
			if (idx < (sizeof(arm_core_names2)/sizeof(char *)))
		...
		case MONTBLANC:
			regs = arm_pamir_regs;
			core_reg_count = ARRAY_SIZE(arm_pamir_regs);
			break;
		case CORTEX_A53:
			regs = arm_v8_regs;
			core_reg_count = ARRAY_SIZE(arm_v8_regs);
		...
		case KLEIN:
			regs = arm_klein_regs;
			core_reg_count = ARRAY_SIZE(arm_klein_regs);
			break;
		case MAKALU:
		case MAKALU_ELP:
			regs = arm_makalu_regs;
			core_reg_count = ARRAY_SIZE(arm_makalu_regs);
			break;
		case HAYES:
			regs = arm_hayes_regs;
			core_reg_count = ARRAY_SIZE(arm_hayes_regs);
			break;
		case HUNTER:
		case HUNTER_ELP:
			regs = arm_hunter_regs;
			core_reg_count = ARRAY_SIZE(arm_hunter_regs);
			break;
		}
		break;
	default:
