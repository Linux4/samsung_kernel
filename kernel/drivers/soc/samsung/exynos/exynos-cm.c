...
#include <soc/samsung/exynos/exynos-cm.h>
...
static int exynos_cm_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_string(np, "cryptomanager-ver", &cmdev->cm_ver);
	if (ret) {
		pr_err("%s: Fail to get cm_ver from device tree\n", __func__);
		return ret;
	}

	memcpy(debug_mem->magic, cmdev->cm_ver, CM_VER_LEN);
