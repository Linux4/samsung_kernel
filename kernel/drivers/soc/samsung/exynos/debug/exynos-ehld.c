...
static void ehld_event_update(int cpu)
{
	...
	if (ehld->ver == EHLD_V3)
		...
		data->instrun[count] = readl(ctrl->reg_instrun +
				(PPC_PMCNT0_LOW + ((cpu % SZ_4) * SZ_4)));
	if (!ctrl->cpupm_enabled) {
		data->pmpcsr[count] = 0xD15AB1ED;
	} else if (ctrl->in_c2) {
		data->pmpcsr[count] = 0xC2;
	} else if (ctrl->in_s2idle_cpuidle) {
		data->pmpcsr[count] = 0x521D1EC2;
	} else if (ehld->sjtag_en) {
		data->pmpcsr[count] = 0x8EC;
	} else if (cpu_is_offline(cpu)) {
		data->pmpcsr[count] = 0x5FF5FF;
	} else if (!is_exynos_cpu_power_on(cpu)) {
	...
	ehld_info(0, "%s: cpu%d: time:%llu, instret:0x%llx, instrun:0x%llx, pmpcsr:%llx, c2:%d\n",
			__func__, cpu,
			data->time[count],
			data->instret[count],
			data->instrun[count],
						...
						unsigned long l, void *p)
{
	...
	raw_spin_lock_irqsave(&ctrl->lock, flags);
	if (unlikely(!ctrl->cpupm_enabled))
		ctrl->cpupm_enabled = true;
...
static void ehld_android_vh_cpuidle_psci_enter(void *data,
				struct cpuidle_device *dev, bool s2idle)
{
	...
	ctrl->in_s2idle_cpuidle = true;

	raw_spin_unlock_irqrestore(&ctrl->lock, flags);
}

static void ehld_android_vh_cpuidle_psci_exit(void *data,
				struct cpuidle_device *dev, bool s2idle)
{
	int cpu;
	struct ehld_ctrl *ctrl;
...
{
	/* register cpu pm notifier for cpuidle */
	exynos_cpupm_notifier_register(&ehld_c2_pm_nb);
	register_trace_android_vh_cpuidle_psci_enter(ehld_android_vh_cpuidle_psci_enter, NULL);
	register_trace_android_vh_cpuidle_psci_exit(ehld_android_vh_cpuidle_psci_exit, NULL);
