...
void dbg_snapshot_thermal(char *name, s32 tempstate, u32 freqpower,
		s64 coef)
{ /* [freq 13bit | power 13bit | state 6bit ] */
	unsigned long idx;
	...
	entry[idx].temp = (tempstate >> 16) & 0xFFFF;
	entry[idx].state = (tempstate & 0xFFFF);
	entry[idx].power = (freqpower >> 16) & 0xFFFF;
	entry[idx].freq = (freqpower) & 0xFFFF;
	entry[idx].t_coef = (coef >> 32) & 0xFFFFFFFF;
	entry[idx].f_coef = (coef) & 0xFFFFFFFF;
}
