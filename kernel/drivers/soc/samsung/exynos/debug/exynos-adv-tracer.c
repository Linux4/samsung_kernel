...
static int adv_tracer_ipc_channel_probe(void)
{
	...
	if (ret) {
		dev_err(exynos_ati->dev, "failed to register framework channel %d\n", ret);
		return ret;
	}
...
struct s5e8845_recovery_data {
	unsigned int dbgc_cpu_conf;
	unsigned int dbgc_cpu_status;
	unsigned int rst_seq;
};

static int s5e8845_init_recovery(struct device *dev) {
	struct device_node *np;
	struct s5e8845_recovery_data *data;
	...
	data = devm_kzalloc(dev, sizeof(struct s5e8845_recovery_data), GFP_KERNEL);
	if (!data) {
		dev_err(dev, "%s: alloc failed\n", __func__);
		return -1;
	}

	if (of_property_read_u32(np, "dbgc-cpu-conf", &data->dbgc_cpu_conf) ||
		of_property_read_u32(np, "dbgc-cpu-status", &data->dbgc_cpu_status) ||
		of_property_read_u32(np, "rst-seq", &data->rst_seq)) {
		dev_err(dev, "%s: dbgc-cpu-*/rst-_seq read failed\n", __func__);
		devm_kfree(dev, data);
		return -1;
	}
...
static int s5e8845_recovery_handler(void *data) {
	struct s5e8845_recovery_data *s5e8845_data;
	u64 timeout;
	bool response;
	...
	exynos_pmu_update(s5e8845_data->dbgc_cpu_conf, 0x1, 0x1);
	pmu_wait_bit(s5e8845_data->dbgc_cpu_status, 0x1, 0x1);

	exynos_pmu_update(s5e8845_data->dbgc_cpu_conf, 0x1, 0x0);
	pmu_wait_bit(s5e8845_data->dbgc_cpu_status, 0x1, 0x0);

	exynos_pmu_update(s5e8845_data->dbgc_cpu_conf, 0x1, 0x1);
	pmu_wait_bit(s5e8845_data->dbgc_cpu_status, 0x1, 0x1);

	timeout = local_clock() + 100000;
	while (local_clock() < timeout)
		;
...
static struct eat_variant_id eat_variant_ids[] = {
	{
		.name = "s5e8845",
		.init = s5e8845_init_recovery,
		.handler = s5e8845_recovery_handler,
	},
};
