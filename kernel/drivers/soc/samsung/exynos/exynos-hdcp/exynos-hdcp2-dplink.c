...
int hdcp_dplink_get_rxinfo(uint8_t *status)
{
	...
	ret = hdcp_dplink_recv(HDCP22_MSG_RXSTATUS_R, status, (uint32_t)sizeof(uint8_t));
...
#ifdef CONFIG_SEC_DISPLAYPORT_LOGGER
			dp_logger_print("HDCP2 autenticate done\n");
#endif
			retry_cnt = 0;
			mutex_unlock(&hdcp_auth_mutex);
			return 0;
...
#ifdef CONFIG_SEC_DISPLAYPORT_LOGGER
				dp_logger_print("HDCP2 Stop authenticate\n");
#endif
				auth_proc_state = HDCP_AUTH_PROCESS_IDLE;
				break;
			}
...
#ifdef CONFIG_SEC_DISPLAYPORT_LOGGER
			dp_logger_print("HDCP2 auth failed. retry(%d)\n", retry_cnt);
#endif
		}
	}
