...
/* There is a hw bug in EVT0.0 where SCPRE & SCALL bits are swapped
 * TBD: It has to be reverted, if HW bug is resolved in EVT0.1
 */
#define EXYNOS_MIPI_PHY_M4M4_SCPRE           1
#define EXYNOS_MIPI_PHY_M4M4_SCALL           0
#else // !CONFIG_SOC_S5E8845
#define EXYNOS_MIPI_PHY_M4M4_SCPRE           0
#define EXYNOS_MIPI_PHY_M4M4_SCALL           1
#endif

#define EXYNOS_MIPI_PHY_M4M4_SCPRE_FEEDBACK  2
#define EXYNOS_MIPI_PHY_M4M4_SCALL_FEEDBACK  3
#define EXYNOS_MIPI_PHY_M4M4_PG_ISO	     4
...
static void wait_feedback_bit(struct regmap *reg_pmu, unsigned int ofs,
			      unsigned int bit, unsigned int exp)
{
	...
	if (!timeout)
		pr_err("%s failed to waiting offset:%08x bit:%x\n", __func__,
		       ofs, bit);
}
				...
				unsigned int ofs, unsigned int on)
{
	if (!on)
		regmap_update_bits(reg_pmu, ofs,
				BIT(EXYNOS_MIPI_PHY_M4M4_PG_ISO),
				BIT(EXYNOS_MIPI_PHY_M4M4_PG_ISO));

	regmap_update_bits(reg_pmu, ofs,
			BIT(EXYNOS_MIPI_PHY_M4M4_SCALL),
			on ? 0 : BIT(EXYNOS_MIPI_PHY_M4M4_SCALL));
	regmap_update_bits(reg_pmu, ofs,
			BIT(EXYNOS_MIPI_PHY_M4M4_SCPRE),
			on ? 0 : BIT(EXYNOS_MIPI_PHY_M4M4_SCPRE));

	wait_feedback_bit(reg_pmu, ofs,
			EXYNOS_MIPI_PHY_M4M4_SCALL_FEEDBACK,
			on ? 0 : 1);
	wait_feedback_bit(reg_pmu, ofs,
			EXYNOS_MIPI_PHY_M4M4_SCPRE_FEEDBACK,
			on ? 0 : 1);

	if (on)
		regmap_update_bits(reg_pmu, ofs,
				BIT(EXYNOS_MIPI_PHY_M4M4_PG_ISO), 0);
...
static int __set_phy_isolation(struct regmap *reg_pmu,
		unsigned int offset, unsigned pg_offset, unsigned int on)
{
	unsigned int val;
	int ret;
	...
	if (reg_pmu && pg_offset > 0)
		__set_phy_power_gating(reg_pmu, pg_offset, on);

	pr_debug("%s off=0x%x, pg_off=0x%x, val=0x%x\n", __func__, offset, pg_offset, val);
		...
		struct mipi_phy_desc *phy_desc, unsigned int on)
{
	...
	if (on) {
		ret = __set_phy_isolation(state->reg_pmu,
				phy_desc->iso_offset, phy_desc->pg_offset, on);
		if (ret)
			goto phy_exit;
...
static int exynos_mipi_phy_probe(struct platform_device *pdev)
{
	...
	unsigned int pg[EXYNOS_MIPI_PHYS_MASTER_NUM];
	struct resource *res;
	unsigned int i;
	...
	memset(&pg, 0, EXYNOS_MIPI_PHYS_MASTER_NUM);
	ret = of_property_read_u32_array(node, "power-gating", pg, elements);
	if (ret)
		dev_warn(dev, "cannot get PHY power gating offset\n");
	...
	for (i = 0; i < elements; i++) {
		...
		if (pg[i] > 0) {
			dev_info(dev, "%s: power gating 0x%x\n", __func__,
