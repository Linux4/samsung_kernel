...
struct exynos_mipi_phy {
	...
	struct mipi_phy_desc {
		...
		unsigned int pg_offset[EXYNOS_MIPI_PHYS_NUM];
				...
				unsigned int ofs, unsigned int on)
{
	...
	pr_info("%s: %sEXYNOS_MIPI_PHY_PG_SCALL %s, EXYNOS_MIPI_PHY_PG_SCPRE %s%s\n", __func__,
		on ? "" : "EXYNOS_MIPI_PHY_PG_ISO_EN set, ", on ? "clear" : "set",
		on ? "clear" : "set", on ? ", EXYNOS_MIPI_PHY_PG_ISO_EN clear" : "");
}
		...
		unsigned int bit, unsigned int on)
{
	...
	pr_info("%s: reset_bit(%d) %s\n", __func__, bit, on ? "set" : "clear");
		...
		struct mipi_phy_desc *phy_desc, unsigned int on)
{
	...
	if (on) {
		...
		for (i = 0; i < phy_desc->sz_pg; ++i) {
			__set_phy_power_gating(state->reg_pmu,
				phy_desc->pg_offset[i],
				on);
		}
	} else {
		ret = __set_phy_isolation(state->reg_pmu,
...
phy_exit:
	pr_info("%s: stream %s iso 0x%x, pg 0x%x, reset 0x%x\n", __func__, on ? "on" : "off",
		phy_desc->iso_offset, phy_desc->pg_offset[0], phy_desc->rst_bit);
...
static int __set_phy_cfg_0800_0000_cphy(void __iomem *regs, int option, u32 *cfg)
{
	...
	for (i = 0; i <= cfg[LANES]; i++) {
		...
		writel(0x000000F7, regs + 0x0108 + (i * 0x100)); /* SD_ANA_CON0 */
		writel(0x0000FF29, regs + 0x010C + (i * 0x100)); /* SD_ANA_CON1 */
...
static struct exynos_mipi_phy_data mipi_phy_m0s4s4s4_s22 = {
	.flags = MIPI_PHY_MxSx_SHARED,
	.active_count = 0,
	.slock = __SPIN_LOCK_UNLOCKED(mipi_phy_m0s4s4s4_s22.slock),
};

static struct exynos_mipi_phy_data mipi_phy_m0s4s4s4s4_s22 = {
	.flags = MIPI_PHY_MxSx_SHARED,
	.active_count = 0,
	.slock = __SPIN_LOCK_UNLOCKED(mipi_phy_m0s4s4s4s4_s22.slock),
};
...
static const struct of_device_id exynos_mipi_phy_of_table[] = {
	...
	{
		.compatible = "samsung,mipi-phy-m0s4s4s4_s22",
		.data = &mipi_phy_m0s4s4s4_s22,
	},
	{
		.compatible = "samsung,mipi-phy-m0s4s4s4s4_s22",
		.data = &mipi_phy_m0s4s4s4s4_s22,
	},
	{ },
};
...
static int exynos_mipi_phy_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
	unsigned int i, j;
	int ret = 0, elements = 0, pg_elements = 0;
	...
	if (IS_ERR(state->reg_pmu)) {
		...
		pg_elements = of_property_count_u32_elems(node, "power-gating");
		if ((pg_elements > 0) && (pg_elements <= EXYNOS_MIPI_PHYS_NUM)) {
	...
	for (i = 0; i < elements; i++) {
		state->phys[i].sz_pg = pg_elements;
		state->phys[i].iso_offset = iso[i];
		state->phys[i].rst_bit	  = rst[i];

		if (state->reg_pmu) {
			dev_info(dev, "isolation: 0x%x\n", state->phys[i].iso_offset);

			for (j = 0; j < pg_elements; j++) {
				state->phys[i].pg_offset[j] = pg[j];

				if (pg[j] > 0) {
					dev_info(dev, "power-gating[%d]: 0x%x\n",
							j, state->phys[i].pg_offset[j]);
					__set_phy_power_gating(state->reg_pmu, pg[j], 0);
				}
			}
		}
