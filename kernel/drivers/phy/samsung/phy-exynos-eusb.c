...
void phy_exynos_eusb_set_txready_ovrd(struct exynos_usbphy_info *info, int ovrd_en, int ovrd_value)
{
	u32 reg;
	void *base;

	base = info->regs_base_2nd;
	reg = readl(base + EUSBPHY_REG_PHY_CR_XCVR0_DIG_ASIC_UTMI_TXRX_OVRD_OUT_0);
	((EUSBPHY_REG_PHY_CR_XCVR0_DIG_ASIC_UTMI_TXRX_OVRD_OUT_0_p) (&reg))->b.txready_ovrd_en = ovrd_en;
	((EUSBPHY_REG_PHY_CR_XCVR0_DIG_ASIC_UTMI_TXRX_OVRD_OUT_0_p) (&reg))->b.txready_ovrd = ovrd_value;
	writel(reg, base + EUSBPHY_REG_PHY_CR_XCVR0_DIG_ASIC_UTMI_TXRX_OVRD_OUT_0);
}
