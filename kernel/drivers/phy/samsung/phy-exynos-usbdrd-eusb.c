...
void phy_usb_exynos_cv_test_td_9_23_wa(struct exynos_usbdrd_phy *phy_drd, int en)
{
	/* if NXP eusb repeater(PTN3222) is used,
	 * W/A is required when CV Test (TD 9.23 Reset Device Test)
	 */
	if (en)
		phy_exynos_eusb_set_txready_ovrd(&phy_drd->usbphy_info, true, true);
	else
		phy_exynos_eusb_set_txready_ovrd(&phy_drd->usbphy_info, false, false);
}
EXPORT_SYMBOL_GPL(phy_usb_exynos_cv_test_td_9_23_wa);
...
static void exynos_usbdrd_ldo_control(struct exynos_usbdrd_phy *phy_drd, int on)
{
...
int exynos_usbdrd_phy_set_mode(struct phy *phy, enum phy_mode mode, int submode)
{
	...
	pr_info("%s %d mode 0x %02x value: %d\n", __func__, __LINE__, mode, submode);
