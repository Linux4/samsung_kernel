...
static int get_battery_info(struct s2m_chg_manager_info *battery)
{
...
#if IS_ENABLED(CONFIG_DIRECT_CHARGER)
	psy_sc = power_supply_get_by_name(battery->pdata->switching_charger_name);
#else
	psy_sc = power_supply_get_by_name(battery->pdata->charger_name);
#endif
	if (!psy_sc) {
		pr_info("%s, fail to get switching-charger psy\n", __func__);
		return -EINVAL;
	}

	ret = power_supply_set_property(psy_sc, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_USBPD_TEST_READ, &value);
	if (ret < 0)
		pr_err("%s: Fail to execute property(USBPD_TEST_READ)\n", __func__);
	...
	if (!psy) {
		...
		if (ret < 0)
			pr_err("%s: Fail to execute property(MUIC_PATH)\n", __func__);
