...
static int s2mf301_get_regulation_voltage(struct s2mf301_charger_data *charger)
{
	...
	ret = s2mf301_read_reg(charger->i2c, S2MF301_CHG_CTRL7, &reg_data);
	if (ret < 0)
		return ret;
	reg_data &= 0xFF;
...
static void s2mf301_set_topoff_current(struct s2mf301_charger_data *charger, int current_limit)
{
	...
	if (current_limit <= 100)
		data = 0;
	else if (current_limit > 100 && current_limit <= 1360)
...
static bool s2mf301_chg_init(struct s2mf301_charger_data *charger)
{
	...
	/* Set battery OCP 8A */
	s2mf301_update_reg(charger->i2c, S2MF301_CHG_CTRL13, S2MF301_SET_BAT_OCP_8000mA, BAT_OCP_MASK);
	...
	s2mf301_read_reg(charger->i2c, S2MF301_CHG_STATUS1, &data);
	if (data & LC_STATUS_MASK) {
		pr_info("%s: lc state\n", __func__);
	...
	/* disable CHG_TIMER_FAULT */
	s2mf301_update_reg(charger->i2c, S2MF301_CHG_CTRL19, 0x0, 0x10);
#endif
				...
				const union power_supply_propval *val)
{
	...
	enum power_supply_property psp_t;
	int ret;
#if IS_ENABLED(EN_VF_BST) && IS_ENABLED(CONFIG_S2MF301X)
	u8 data;
#endif
...
static irqreturn_t s2mf301_event_isr(int irq, void *data)
{
	...
	pr_info("[IRQ] %s, STATUS0:0x%02x, STATUS1:0x%02x, STATUS2:0x%02x, STATUS3:0x%02x, STATUS4:0x%02x, STATUS5:0x%02x, STATUS6:0x%02x, STATUS7:0x%02x",
...
#else
	if (val & WDT_TIMER_FAULT_STATUS_MASK) {
#endif
		value.intval = 1;
