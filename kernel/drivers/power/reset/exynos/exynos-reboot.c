...
void exynos_reboot_register_esca_ops(void *acpm_reboot_func)
{
	if (acpm_reboot_func)
...
EXPORT_SYMBOL(exynos_reboot_register_esca_ops);
#else
...
static void exynos_reboot_parse(void *cmd)
{
	if (cmd) {
		...
		if (!strcmp((char *)cmd, "charge"))
			regmap_update_bits(pmureg, reboot_cmd_offset, REBOOT_MODE_MASK, REBOOT_MODE_CHARGE);
		else if (!strcmp((char *)cmd, "bootloader") || !strcmp((char *)cmd, "bl"))
			regmap_update_bits(pmureg, reboot_cmd_offset, REBOOT_MODE_MASK, REBOOT_MODE_FASTBOOT);
		else if (!strcmp((char *)cmd, "fastboot") || !strcmp((char *)cmd, "fb"))
			regmap_update_bits(pmureg, reboot_cmd_offset, REBOOT_MODE_MASK, REBOOT_MODE_USER_FASTBOOTD);
		else if (!strcmp((char *)cmd, "recovery"))
			regmap_update_bits(pmureg, reboot_cmd_offset, REBOOT_MODE_MASK, REBOOT_MODE_RECOVERY);
		else if (!strcmp((char *)cmd, "sfactory"))
			regmap_update_bits(pmureg, reboot_cmd_offset, REBOOT_MODE_MASK, REBOOT_MODE_FACTORY);
		else
			regmap_update_bits(pmureg, reboot_cmd_offset, REBOOT_MODE_MASK, REBOOT_MODE_NORMAL);
	}
}
