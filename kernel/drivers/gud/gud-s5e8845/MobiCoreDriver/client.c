				...
				struct gp_return *gp_ret)
{
	...
	if (!(memref->flags & MC_IO_FFA_LEND)) {
		ret = iwp_register_shared_mem(cwsm->mmu, &sva, gp_ret);
		if (ret)
			goto err_mmu;
	} else {
		memref->handle = tee_mmu_get_handle(cwsm->mmu);
		sva = 0;
	}
...
static void cwsm_release(struct kref *kref)
{
	...
	if (!(cwsm->memref.flags & MC_IO_FFA_LEND)) {
...
struct tee_client *client_create(bool is_from_kernel, const char *vm_id)
{
	...
	size_copied = strscpy(client->vm_id, vm_id, sizeof(client->vm_id));
