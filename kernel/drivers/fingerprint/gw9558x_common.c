...
int gw9558_gpio_setup(struct gf_device *gf_dev)
{
	int retval = 0;
	...
	if (gf_dev->pwr_gpio) {
		...
		if (retval < 0) {
			...
			goto gw9558_gpio_setup_pwr_failed;
		}
	...
	if (retval < 0) {
		...
		goto gw9558_gpio_setup_reset_failed;
	}
	gpio_direction_output(gf_dev->reset_gpio, 0);
	return retval;

gw9558_gpio_setup_reset_failed:
	if (gpio_is_valid(gf_dev->pwr_gpio))
		gpio_free(gf_dev->pwr_gpio);
gw9558_gpio_setup_pwr_failed:
	return retval;
}
...
static int gw9558_probe_common(struct device *dev, struct gf_device *gf_dev)
{
	...
	retval = gw9558_gpio_setup(gf_dev);
	if (retval < 0) {
		pr_err("Failed to set gpio up:%d\n", retval);
		goto gw9558_probe_gpio_setup;
	}
#ifndef ENABLE_SENSORS_FPRINT_SECURE
