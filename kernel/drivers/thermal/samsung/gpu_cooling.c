				 ...
				 unsigned long state)
{
	...
	dbg_snapshot_thermal(cdev->type, state, gpufreq_cooling_get_freq(0, state) / 1000, 0);

	ret = gpufreq_apply_cooling(gpufreq_cdev, state);

	if(gpufreq_cdev->tz == NULL | gpufreq_cdev->cool_dev == NULL)
		return ret;

	exynos_acpm_tmu_get_pi_params(gpufreq_cdev->tz->id, TMU_DATA_PPREV_DATA, &pprev_data);
	exynos_acpm_tmu_get_pi_params(gpufreq_cdev->tz->id, TMU_DATA_PREV_DATA, &prev_data);
	exynos_acpm_tmu_get_pi_params(gpufreq_cdev->tz->id, TMU_DATA_CUR_DATA, &cur_data);
	exynos_acpm_tmu_get_pi_params(gpufreq_cdev->tz->id, TMU_DATA_CUR_COEF, &coef);

	dbg_snapshot_thermal(gpufreq_cdev->cool_dev->type, pprev_data & 0xFFFFFFFF,
			pprev_data >> 32, 1);
	dbg_snapshot_thermal(gpufreq_cdev->cool_dev->type, prev_data & 0xFFFFFFFF,
			prev_data >> 32, 2);
	dbg_snapshot_thermal(gpufreq_cdev->cool_dev->type, cur_data & 0xFFFFFFFF,
			cur_data >> 32, coef);
	return ret;
#else
	dbg_snapshot_thermal(NULL, state, cdev->type, gpufreq_cooling_get_freq(0, state));
