...
static void __transit_for_hat(struct ufs_perf_stat_v1 *stat,
			      traffic traffic, s64 time, u32 qd)
{
	s64 diff;
	bool transit = false;
	...
	if (traffic == TRAFFIC_HIGH) {
		if (stat->hat_state == HAT_RARE) {
			...
			if (stat->seq_count > qd) {
				stat->seq_count = 0;
				transit = true;

			} else if ((diff >= stat->th_dwell_in_high) && stat->seq_count >= stat->th_qd_max) {
...
static void __update_v1_queued(struct ufs_perf_stat_v1 *stat, u32 qd)
{
	...
	__transit_for_hat(stat, traffic, time, qd);
	...
	if (stat->start_count_time == -1LL) {
		...
		diff_seq = ktime_to_ms(ktime_sub(time,
					stat->last_seq_transit_time));
		++stat->count;
		if (diff >= stat->th_duration ||
				((diff_seq <= stat->th_seq_time) &&
				(stat->last_seq_transit_time != -1LL))) {
			if (qd >= stat->th_qd_max) {
				stat->last_seq_transit_time = -1LL;
				stat->hat_state = HAT_REACH;
				traffic = TRAFFIC_HIGH;
			...
			} else if (stat->count >= stat->th_count) {
				stat->start_count_time = -1LL;
				stat->count = 0;
				traffic = TRAFFIC_HIGH;
			} else {
...
int ufs_perf_init_v1(struct ufs_perf *perf)
{
	...
	for (index = 0; index < __POLICY_MAX; index++) {
		if (!(BIT(index) & stat->policy_bits))
			continue;
		stat->req_bits[index] = CTRL_REQ_DVFS;	//
	}
