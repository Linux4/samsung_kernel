			     ...
			     u8 lane, u32 offset, u32 value)
{
	...
	//printf("[__set_pcs]0x%08x[lane %d] : 0x%x\n", offset, lane, value);
}
				       ...
				       struct ufs_cal_param *p)
{
	...
	case HCI_AH8_THIBERN:
		...
		//printf("[HCI_AH8_THIBERN]0x%08x : 0x%x\n", cfg->addr, value);
		break;
	case HCI_AH8_REFCLKGATINGTING:
		// 1us Tick error, need to more delay timing
		// Jirra : https://jira.itplatform.samsungds.net:8080/browse/SOCPE-18774
		hci_writel(handle, p->ah8_brefclkgatingwaittime + 5, cfg->addr);
		//printf("[HCI_AH8_REFCLKGATINGTING]0x%08x : 0x%x\n", cfg->addr, p->ah8_brefclkgatingwaittime + 5);
		break;
	case HCI_AH8_ACTIVE_LANE:
		value = (p->connected_rx_lane << 16) | PHY_PMA_LANE_OFFSET;
		hci_writel(handle, value, cfg->addr);
		//printf("[HCI_AH8_ACTIVE_LANE]0x%08x : 0x%x\n", cfg->addr, value);
		break;
	case HCI_AH8_CMN:
		hci_writel(handle, cfg->val, cfg->addr);
		//printf("[HCI_AH8_CMN]0x%08x : 0x%x\n", cfg->addr, cfg->val);
		break;
	case HCI_AH8_TRSV:
		value = ((p->connected_rx_lane - 1) << 31) | cfg->val;
		hci_writel(handle, value, cfg->addr);
		//printf("[HCI_AH8_TRSV]0x%08x : 0x%x\n", cfg->addr, ((p->active_tx_lane - 1) << 31) | cfg->val);
		break;
	case HCI_STD:
		hci_writel(handle, cfg->val, cfg->addr);
		break;
	...
	case UNIPRO_DBG_MIB:
		...
		//printf("[UNIPRO_STD_MIB]0x%08x : 0x%x\n", cfg->addr, cfg->val);
		break;
	case UNIPRO_ADAPT_LENGTH:
		value = unipro_readl(handle, cfg->addr);
		//printf("[RD,UNIPRO_ADAPT_LENGTH]0x%08x : 0x%x\n", cfg->addr, value);
		if (value & 0x80) {
			if ((value & 0x7F) < 2) {
	...
	case UNIPRO_DBG_PRD:
		...
		//printf("[UNIPRO_DBG_PRD]0x%08x : 0x%x\n", cfg->addr, p->mclk_period_unipro_18);
		break;
	case UNIPRO_DBG_APB:
		unipro_writel(handle, cfg->val, cfg->addr);
		//printf("[UNIPRO_DBG_APB]0x%08x : 0x%x\n", cfg->addr, cfg->val);
		break;

	/* pcs */
	case PHY_PCS_COMN:
		unipro_writel(handle, cfg->val, cfg->addr);
		//printf("[PHY_PCS_COMN]0x%08x : 0x%x\n", cfg->addr, cfg->val);
		break;
	case PHY_PCS_RX:
	...
	case PHY_PMA_COMN:
		...
		//printf("[PHY_PMA_COMN]0x%08x : 0x%x\n", PHY_PMA_COMN_ADDR(cfg->addr), cfg->val);
		break;
	case PHY_PMA_TRSV:
		pma_writel(handle, cfg->val,
			   PHY_PMA_TRSV_ADDR(cfg->addr, lane));
		//printf("[PHY_PMA_TRSV]0x%08x : 0x%x\n", PHY_PMA_TRSV_ADDR(cfg->addr, lane), cfg->val);
		break;
	case PHY_PLL_WAIT:
		if (ufs_cal_wait_pll_lock(handle, cfg->addr, cfg->val)
		    == UFS_CAL_ERROR)
			ret = UFS_CAL_TIMEOUT;
		//printf("[PHY_PLL_WAIT]0x%08x : 0x%x\n", cfg->addr, cfg->val);
		break;
	case PHY_CDR_WAIT:
		/* after gear change */
		if (ufs_cal_wait_cdr_lock(handle, cfg->addr, cfg->val, lane)
		    == UFS_CAL_ERROR)
			ret = UFS_CAL_TIMEOUT;
		//printf("[PHY_CDR_WAIT]0x%08x[lane %d] : 0x%x\n", cfg->addr, lane, cfg->val);
		break;
	case PHY_EMB_CDR_WAIT:
		...
		if (ufs30_cal_wait_cdr_lock(p->handle, cfg->addr, cfg->val,
		    ...
		    == UFS_CAL_ERROR)
			ret = UFS_CAL_TIMEOUT;
		//printf("[PHY_EMB_CDR_WAIT]0x%08x[lane %d] : 0x%x\n", cfg->addr, lane, cfg->val);
		break;
		/* after gear change */
	...
	case PHY_EMB_CAL_WAIT:
		if (ufs30_cal_done_wait(p->handle, cfg->addr, cfg->val, lane)
		    == UFS_CAL_ERROR)
			ret = UFS_CAL_TIMEOUT;
		//printf("[PHY_EMB_CAL_WAIT]0x%08x[lane %d] : 0x%x\n", cfg->addr, lane, cfg->val);
		break;
	case COMMON_WAIT:
		if (handle->udelay)
			handle->udelay(cfg->val);
		//printf("[COMMON_WAIT]%d\n", cfg->val);
		break;
	case PHY_PMA_TRSV_SQ:
		/* for hibern8 time */
		pma_writel(handle, cfg->val,
			   PHY_PMA_TRSV_ADDR(cfg->addr, lane));
		//printf("[PHY_PMA_TRSV_SQ]0x%08x : 0x%x\n", PHY_PMA_TRSV_ADDR(cfg->addr, lane), cfg->val);
		break;
	case PHY_PMA_TRSV_LANE1_SQ_OFF:
		/* for hibern8 time */
		pma_writel(handle, cfg->val,
			   PHY_PMA_TRSV_ADDR(cfg->addr, lane));
		//printf("[PHY_PMA_TRSV_LANE1_SQ_OFF]0x%08x : 0x%x\n", PHY_PMA_TRSV_ADDR(cfg->addr, lane), cfg->val);
		break;
	default:
		break;
	}
				       ...
				       u32 repeat)
{
	...
	u32 retries = 100 * 100;	/* 100*100*10[us] = 100ms */
	u32 val;

	for (phase = 0; phase < EOM_PH_SEL_MAX; phase++) {
		pma_writel(handle, phase << 1, PHY_PMA_TRSV_ADDR(0x91C, lane));
