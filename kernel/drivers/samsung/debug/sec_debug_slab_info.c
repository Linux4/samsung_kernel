...
static int secdbg_slab_info_get_candidate(struct kmem_cache *s, struct kmem_cache_cpu *c, void *freelist, char *buf)
{
	...
	int count = slab->objects < OBJ_MAX ? slab->objects : OBJ_MAX;
	void *p;
	unsigned long freepointer_addr;
	unsigned long candidate;
	ssize_t offset = 0;

	for_each_object(p, s, slab_address(slab), count) {
		p = kasan_reset_tag(p);
...
static void secdbg_display_slab_cache_info(void)
{
	...
	if (!check_valid_pointer(s, c->slab, freelist)) {
		...
		pr_auto(ASL1, "Candidate(%d of %d) :%s", obj_count, c->slab->objects, buf);
