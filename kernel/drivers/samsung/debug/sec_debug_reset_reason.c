...
#define UNINIT_16_31	\
	"UNINIT16", "UNINIT17", "UNINIT18", "UNINIT19", "UNINIT20", "UNINIT21", "UNINIT22", "UNINIT23",	\
	"UNINIT24", "UNINIT25", "UNINIT26", "UNINIT27", "UNINIT28", "UNINIT29", "UNINIT30", "UNINIT31"

#define PWRSRC_LEN 16
#define RSTSTAT_LEN 32

static char power_off_regs_bit[16][PWRSRC_LEN] = {
	UNINIT_0_15
};

static char power_on_regs_bit[16][PWRSRC_LEN] = {
	UNINIT_0_15
};

static char rst_stat_regs_bit[32][RSTSTAT_LEN] = {
	UNINIT_0_15,
	UNINIT_16_31
};
...
static void parse_pwrsrc_rs(struct outbuf *buf)
{
	...
	if (!tmp)
		...
		for (i = DEFAULT_SRC_NUM - power_off_src_cnt, reg_i = 0; i < DEFAULT_SRC_NUM; i++, reg_i++)
			if (tmp & (1 << i))
				_secdbg_rere_write_buf(buf, 0, " %s", power_off_regs_bit[reg_i]);
	}
	_secdbg_rere_write_buf(buf, 0, " /");
	...
	if (!tmp)
		...
		for (i = DEFAULT_SRC_NUM - power_on_src_cnt, reg_i = 0; i < DEFAULT_SRC_NUM; i++, reg_i++)
			if (tmp & (1 << i))
				_secdbg_rere_write_buf(buf, 0, " %s", power_on_regs_bit[reg_i]);
	}
	...
	if (!tmp)
		...
		for (i = 0; i < 32; i++)
			if (tmp & (1 << i))
				_secdbg_rere_write_buf(buf, 0, " %s", rst_stat_regs_bit[i]);
...
static int secdbg_reset_reason_pwrsrc_show(struct seq_file *m, void *v)
{
	...
	if (!tmp) {
		...
		for (i = 0; i < power_off_src_cnt; i++)
			if (tmp & (1 << i))
				_secdbg_rere_write_buf(&pwrsrc_buf, 0, " %s", power_off_regs_bit[i]);
	...
	if (!tmp) {
		...
		for (i = 0; i < power_on_src_cnt; i++)
			if (tmp & (1 << i))
				_secdbg_rere_write_buf(&pwrsrc_buf, 0, " %s", power_on_regs_bit[i]);
	...
	if (!tmp) {
		...
		for (i = 0; i < 32; i++)
			if (tmp & (1 << i))
				_secdbg_rere_write_buf(&pwrsrc_buf, 0, " %s", rst_stat_regs_bit[i]);
...
static void secdbg_rere_parse_dt(void)
{
	...
	for (i = 0; i < power_off_src_cnt; i++) {
		...
		if (ret < 0)
			snprintf(power_off_regs_bit[i], PWRSRC_LEN, "ERR");
		else
			snprintf(power_off_regs_bit[i], PWRSRC_LEN, "%s", src_name);
	}

	for (i = 0; i < power_on_src_cnt; i++) {
		ret = of_property_read_string_index(np, "power_on_src", i, &src_name);
		if (ret < 0)
			snprintf(power_on_regs_bit[i], PWRSRC_LEN, "ERR");
		else
			snprintf(power_on_regs_bit[i], PWRSRC_LEN, "%s", src_name);
	}
	...
	if (rst_stat_cnt > 0) {
		for (i = 0; i < rst_stat_cnt; i++) {
			...
			if (ret < 0)
				snprintf(rst_stat_regs_bit[i], RSTSTAT_LEN, "ERR");
			else
				snprintf(rst_stat_regs_bit[i], RSTSTAT_LEN, "%s", src_name);
		}
	}
}
