...
static int secdbg_base_built_set_emerg_address(struct notifier_block *nb,
				   unsigned long l, void *buf)
{
	secdbg_base_built_wdd_set_emerg_addr((unsigned long)buf);

	return NOTIFY_DONE;
}

static struct notifier_block nb_emerg_addr = {
	.notifier_call = secdbg_base_built_set_emerg_address,
};

#ifdef CONFIG_S3C2410_BUILTIN_WATCHDOG
extern int s3c2410wdt_builtin_set_addr_register_notifier(struct notifier_block *nb);
#else
static inline int s3c2410wdt_builtin_set_addr_register_notifier(struct notifier_block *nb) { return 0; }
#endif
#endif /* CONFIG_SEC_DEBUG_WATCHDOGD_FOOTPRINT */
...
{
	if (raw_cpu_read(secdbg_bad_stack_chk)) {
		/* reentrance handle_bad_stack */
		s3c2410wdt_builtin_expire_watchdog_raw();
		cpu_park_loop();
	}

	raw_cpu_write(secdbg_bad_stack_chk, 0xBAD);
}
...
#if IS_ENABLED(CONFIG_SEC_DEBUG_WATCHDOGD_FOOTPRINT)
	s3c2410wdt_builtin_set_addr_register_notifier(&nb_emerg_addr);
#endif
