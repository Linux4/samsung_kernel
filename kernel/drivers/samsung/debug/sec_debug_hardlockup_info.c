...
#define EHLD_THRESHOLD 0x5000000
...
static bool secdbg_hardlockup_check_ehld_info(unsigned int cpu, struct ehld_data *data)
{
	int i;

	for (i = 0; i < NUM_TRACE; i++) {
		if (data->instrun[i] >= (unsigned long long)EHLD_THRESHOLD)
			return true;
	}
...
static void secdbg_hardlockup_save_ehld_type(unsigned int cpu)
{
	...
	if (secdbg_hardlockup_check_ehld_info(cpu, data))
