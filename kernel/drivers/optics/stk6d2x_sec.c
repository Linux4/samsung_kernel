...
module_param(use_ext_clk, int, S_IRUGO | S_IWUSR);
...
void stk_sec_report(struct stk6d2x_data *alps_data)
{
	...
	if (cnt++ >= 30) {
		ALS_info("IR:%lld Clear:%lld UV:%lld  ir_gain:%d clear_gain:%d uv_gain:%d",
			alps_data->ir_local_average, alps_data->clear_local_average, alps_data->uv_local_average,
			alps_data->ir_gain, alps_data->clear_gain, alps_data->uv_gain);
		ALS_dbg("AWB: IR*:%d Clear*:%d UV*:%d\n", temp_ir, temp_clear, temp_uv);
		cnt = 0;
	}
...
int32_t stk_power_ctrl(struct stk6d2x_data *alps_data, bool en)
{
		...
		if (alps_data->regulator_vdd_1p8 != NULL) {
			if (!alps_data->vdd_1p8_enable) {
				...
				if (rc) {
					...
					ALS_dbg("enable vdd_1p8 done, (state : %d), rc=%d\n", (alps_data->regulator_state + 1), rc);
					msleep(40);
				}
			} else {
	...
	stk6d2x_ext_clk_control(alps_data, en);
	goto done;
	...
	if (alps_data->als_info.enable && alps_data->als_flag) {
		if (!alps_data->flicker_flag) {
			...
			if (STK6D2X_REG_READ(alps_data, STK6D2X_REG_FLAG, &flag_value) < 0) {
				ret = snprintf(buf, PAGE_SIZE, "-6, -6\n");
				goto err;
			}
			else if (flag_value & STK6D2X_FLG_ALSSAT_MASK) {
				ret = snprintf(buf, PAGE_SIZE, "-2, -2\n");
				goto err;
			}
		...
		ret = snprintf(buf, PAGE_SIZE, "-1, -1\n");
	}
err:
...
#if defined(CONFIG_AMS_ALS_COMPENSATION_FOR_AUTO_BRIGHTNESS)
	flicker_flag = alps_data->flicker_flag;
...
void stk6d2x_ext_clk_control(struct stk6d2x_data *alps_data, bool clk_enable)
{
	...
	stk6d2x_pin_control(alps_data, clk_enable);

	if (!use_ext_clk)
		return;

	if (clk_enable) {
		if (!IS_ERR_OR_NULL(alps_data->div_clk)) {
			rc = clk_prepare_enable(alps_data->div_clk);
			if (rc)
				ALS_err("clk_prepare error!!!\n");
			else
				ALS_dbg("clk_prepare success!\n");

			rc = clk_set_rate(alps_data->div_clk, 19200000);
			if (rc)
				ALS_err("clk set rate fail, rc = %d\n", rc);
		}

		if (!IS_ERR_OR_NULL(alps_data->mux_clk)) {
			rc = clk_prepare_enable(alps_data->mux_clk);
			if (rc)
				ALS_err("clk_prepare error!!!\n");
			else
				ALS_dbg("clk_prepare success!\n");
		...
		if (!IS_ERR_OR_NULL(alps_data->mux_clk))
			clk_disable_unprepare(alps_data->mux_clk);
		if (!IS_ERR_OR_NULL(alps_data->div_clk))
			clk_disable_unprepare(alps_data->div_clk);
		ALS_dbg("clk disable done");
	}
}
...
int stk6d2x_suspend(struct device *dev)
{
	...
	/* force off qch in case of abnormal case */
	if (alps_data->regulator_state > 0) {
		alps_data->regulator_state = 1;
		ALS_dbg("force stop flicker sensor. regulator_state=%d\n", alps_data->regulator_state);
							...
							struct stk6d2x_platform_data *pdata)
{
	...
	if (of_get_property(np, "als_rear,use_ext_clk", NULL)) {
		alps_data->div_clk = devm_clk_get(stk_wrapper->dev, "div_clk");
		if (IS_ERR_OR_NULL(alps_data->div_clk)) {
			ALS_err("%s: could not lookup clock", __func__);
			alps_data->div_clk = NULL;
		}
		alps_data->mux_clk = devm_clk_get(stk_wrapper->dev, "mux_clk");
		if (IS_ERR_OR_NULL(alps_data->mux_clk)) {
			ALS_err("%s: could not lookup clock", __func__);
			alps_data->mux_clk = NULL;
		}
	...
	if (vbus_of_node) {
		...
		if (IS_ERR(alps_data->regulator_vbus_1p8) || alps_data->regulator_vbus_1p8 == NULL) {
			...
			ALS_dbg("get vbus_1p8 regulator = %pK done \n", alps_data->regulator_vbus_1p8);
		}
	} else {
		ALS_err("get vbus_1p8 regulator failed\n");
		alps_data->regulator_vbus_1p8 = NULL;
	...
	if (vdd_of_node) {
		...
		if (IS_ERR(alps_data->regulator_vdd_1p8) || alps_data->regulator_vdd_1p8 == NULL) {
			...
			ALS_dbg("get vdd_1p8 regulator = %pK done \n", alps_data->regulator_vdd_1p8);
		}
	} else {
		ALS_err("get vdd_1p8 regulator failed\n");
		alps_data->regulator_vdd_1p8 = NULL;
...
enable_vbus_1p8_failed:
	...
	if (alps_data->regulator_vbus_1p8) {
		...
		ALS_dbg("put vbus_1p8 regulator = %pK done (en = %d)\n",
			alps_data->regulator_vbus_1p8, alps_data->vbus_1p8_enable);
		regulator_put(alps_data->regulator_vbus_1p8);
		alps_data->regulator_vbus_1p8 = NULL;
	}
	...
	if (alps_data->regulator_vdd_1p8) {
		...
		ALS_dbg("put vdd_1p8 regulator = %pK done (en = %d)\n",
			alps_data->regulator_vdd_1p8, alps_data->vdd_1p8_enable);
		regulator_put(alps_data->regulator_vdd_1p8);
		alps_data->regulator_vdd_1p8 = NULL;
	}

	if (alps_data->div_clk) {
		devm_clk_put(stk_wrapper->dev, alps_data->div_clk);
		alps_data->div_clk = NULL;
	}
	if (alps_data->mux_clk) {
		devm_clk_put(stk_wrapper->dev, alps_data->mux_clk);
		alps_data->mux_clk = NULL;
	}
...
int stk6d2x_remove(struct i2c_client *client)
{
	...
	struct stk6d2x_data *alps_data;

	if (stk_wrapper == NULL) {
		ALS_err("flicker sensor probe failed");
		return -ENXIO;
	}
 	alps_data = &stk_wrapper->alps_data;
	stk_power_ctrl(alps_data, false);
...
static void stk6d2x_i2c_remove(struct i2c_client *client)
{
	int ret = stk6d2x_remove(client);
	ALS_dbg("%s ret=%d", __func__, ret);
	return;
}
