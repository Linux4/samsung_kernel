...
int32_t stk6d2x_request_registry(struct stk6d2x_data *alps_data)
{
	...
	//err = STK6D2X_R_FROM_STORAGE(alps_data, STK6D2X_CALI_FILE, file_out_buf, FILE_SIZE);
	return 0;
	if (err < 0)
	{
		kfree(file_out_buf);
	...
	ALS_info("als_version = 0x%X, als_scale = 0x%X\n",
