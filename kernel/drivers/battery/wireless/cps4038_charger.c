...
{
	if (!volt_ctrl_pad(charger->tx_id) || is_3rd_pad((charger->mpp_epp_tx_id & 0xFFFF)))
		return;
...
static void mfc_wpc_phm_exit_work(struct work_struct *work)
{
	...
	while ((--i) >= 0) {
		...
		pr_info("%s: check pdrc(%d), i(%d)\n", __func__, pdrc_state, i);
		if (!pdrc_state)
			goto end_work;
	}
	...
	pr_info("%s: check pdrc(%d <-> %d), det(%d <-> %d), cable_type(%d)\n",
		__func__,
		pdrc_state, charger->pdrc_state,
		det_state, charger->det_state,
		charger->pdata->cable_type);

	if ((pdrc_state) && (!det_state)) {
...
static void mfc_rx_phm_work(struct work_struct *work)
{
	...
	case EXIT_PHM:
		if (charger->rx_phm_status) {
			...
			if ((charger->adt_transfer_status != WIRELESS_AUTH_PASS) && (charger->adt_transfer_status != WIRELESS_AUTH_FAIL))
				charger->adt_transfer_status = WIRELESS_AUTH_WAIT;
	...
	if (charger->pdrc_state != pdrc_state) {
		...
		if (pdrc_state) {
			...
			queue_delayed_work(charger->wqueue, &charger->epp_clear_timer_work,
				msecs_to_jiffies(charger->epp_time));
...
static void mfc_mpp_epp_nego_done(struct mfc_charger_data *charger)
{
	...
	if (epp_mode(charger->rx_op_mode)) {
		...
		if ((charger->mpp_epp_nego_done_power < TX_RX_POWER_8W) ||
			(charger->is_full_status && !is_3rd_pad((charger->mpp_epp_tx_id & 0xFFFF)))) {
