...
static irqreturn_t mfc_wpc_irq_thread(int irq, void *irq_data)
{
	...
	if (irq_src[3] & MFC_INTB_H_PROTOCOL_MASK) {
		if (status[3] & MFC_STAT1_H_PROTOCOL_MASK) {
			// To implement protocol selection
			// Set EPP Protocol selection register
			mfc_reg_write(charger->client, MFC_MPP_PROTOCOL_SELECT, MFC_MPP_PROTOCOL_SELECT_EPP);
			pr_info("%s Select protocol as EPP.\n", __func__);
		}
	}
