...
static int sb_get_dc_ic_name(struct sec_direct_charger_platform_data *pdata)
{
	const char *dc_ic_names[MAX_DC_ICS];
	struct device_node *np;
	int num_dc_ics = 0, i = 0, ret = 0;
	union power_supply_propval dc_ic_status = {0, };
	char temp_dc_ic_name[20] = {0,};
	...
	for (i = 0; i < num_dc_ics; i++) {
		strscpy(temp_dc_ic_name, dc_ic_names[i], sizeof(temp_dc_ic_name));
		ret = psy_do_property(temp_dc_ic_name, get,
				POWER_SUPPLY_PROP_STATUS, dc_ic_status);
		if (ret < 0) {
	...
	if (ret < 0) {
		ret = 0;
		pr_info("%s: cannot find valid DC IC, using: %s\n", __func__, dc_ic_names[ret]);
	} else
		ret = i;

	pdata->direct_charger_name = kzalloc(strlen(dc_ic_names[ret]) + 1, GFP_KERNEL);
	if (!pdata->direct_charger_name)
		return -ENOMEM;

	strscpy(pdata->direct_charger_name, dc_ic_names[ret], sizeof(temp_dc_ic_name));
	return 0;
}
#endif
			    ...
			    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CHECK_VALID_DC_IC:
			if (sb_get_dc_ic_name(charger->pdata))
