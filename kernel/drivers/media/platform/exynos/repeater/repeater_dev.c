...
int repeater_request_buffer(struct shared_buffer_info *info, int owner)
{
	...
	if (!g_repeater_device || !info) {
		print_repeater_debug(RPT_ERROR, "g_repeater %pK, info %pK\n",
	...
	if (ctx->ctx_status.buffer_status != REPEATER_CTX_MAP) {
		print_repeater_debug(RPT_ERROR, "buffer_status(%d) is invalid\n",
	...
	if (owner != REQ_BUF_OWNER_DPU && owner != REQ_BUF_OWNER_MFC) {
		print_repeater_debug(RPT_ERROR, "owner(%d) is invalid\n", owner);
		spin_unlock_irqrestore(&repeater_spinlock, flags);
		return -EFAULT;
	}

	if (g_repeater_test_error == REPEATER_ERR_REQ_BUFFER) {
		print_repeater_debug(RPT_ERROR, "g_repeater_test_error\n");
	...
	if (ctx->info.buffer_count > MAX_SHARED_BUF_NUM) {
		print_repeater_debug(RPT_ERROR, "buffer_count is invalid %d",
	...
	for (i = 0; i < info->buffer_count; i++) {
		if (IS_ERR(ctx->dmabufs[i])) {
			print_repeater_debug(RPT_ERROR, "dmabufs is error\n");
			spin_unlock_irqrestore(&repeater_spinlock, flags);
			return -EFAULT;
		}
	...
	if (owner == REQ_BUF_OWNER_DPU && ctx->dpu_dpms_off == true) {
		...
		if (ctx->repeater_drc_cb) {
			...
			print_repeater_debug(RPT_ERROR, "repeater_drc_cb is invalid\n");
		}
	}
...
int repeater_dpu_dpms_off(void)
{
	...
	if (!g_repeater_device) {
		print_repeater_debug(RPT_ERROR, "g_repeater_device is null\n");
		spin_unlock_irqrestore(&repeater_spinlock, flags);
		return -EFAULT;
	}
...
int repeater_get_valid_buffer(int *buf_idx)
{
	...
	if (!g_repeater_device || !buf_idx) {
		print_repeater_debug(RPT_ERROR, "g_repeater %pK, buf_idx %pK\n",
	...
	if (ctx->ctx_status.buffer_status == REPEATER_CTX_INIT || ctx->repeater_encode_cb == NULL) {
		print_repeater_debug(RPT_ERROR, "ctx_status.buffer_status %d, %s\n",
	...
	if (ret || g_repeater_test_error == REPEATER_ERR_GET_CAPTURING_BUF) {
		...
		print_repeater_debug(RPT_ERROR, "get_capturing_buf_idx() return %d, g_repeater_test_error %d\n",
...
int repeater_set_valid_buffer(int buf_idx, int capture_idx)
{
	...
	if (ctx->ctx_status.buffer_status == REPEATER_CTX_INIT) {
		print_repeater_debug(RPT_ERROR, "ctx_status.buffer_status(%d) is invalid\n",
			ctx->ctx_status.buffer_status);
		spin_unlock_irqrestore(&repeater_spinlock, flags);
		return -EFAULT;
	}
	...
	if (ctx->start_mode == REPEATER_START_ONESHOT && ctx->enc_param.time_stamp) {
		print_repeater_debug(RPT_ERROR, "repeater is one shot mode and already one frame encoded\n");
		spin_unlock_irqrestore(&repeater_spinlock, flags);
		return ret;
	}
	...
	if (ret || g_repeater_test_error == REPEATER_ERR_SET_CAPTURED_BUF) {
		...
		wake_up_interruptible(&ctx->wait_queue_error);
		print_repeater_debug(RPT_ERROR, "set_captured_buf_idx() return %d, g_repeater_test_error %d\n",
...
int repeater_encoding_done(int encoding_ret)
{
	...
	if (ret) {
		...
		print_repeater_debug(RPT_EXT_INFO, "set_encoding_done() return %d\n", ret);
	}
...
int repeater_notify_error(int owner)
{
	...
	wake_up_interruptible(&ctx->wait_queue_error);
	print_repeater_debug(RPT_ERROR, "owner %d\n", owner);
...
void encoding_work_handler(struct work_struct *work)
{
	...
	if (ctx->ctx_status.status != REPEATER_CTX_START) {
		print_repeater_debug(RPT_ERROR, "ctx_status.status(%d) is invalid \n",
	...
	if (ctx->ctx_status.status != REPEATER_CTX_START) {
		print_repeater_debug(RPT_ERROR, "ctx_status.status %d\n",
			ctx->ctx_status.status);
		spin_unlock_irqrestore(&repeater_spinlock, flags);
		return;
	}
	...
	if (ret == -EBUSY || g_repeater_test_error == REPEATER_ERR_GET_LATEST_CAPTURED_BUF_BUSY) {
		...
		wake_up_interruptible(&ctx->wait_queue_error);
		print_repeater_debug(RPT_ERROR, "get_latest_captured_buf_idx() return -EBUSY\n");
		print_repeater_debug(RPT_ERROR, "g_repeater_test_error %d\n", g_repeater_test_error);
		...
		if (ctx->nobuf_err_count >= MAX_NO_BUF_ERROR_COUNT) {
			...
			increase_log_level(ctx, RPT_SHR_BUF_INFO);
			print_repeater_debug(RPT_ERROR, "get_latest_captured_buf_idx() return -ENOBUFS\n");
			print_repeater_debug(RPT_ERROR, "g_repeater_test_error %d\n", g_repeater_test_error);
		}
	} else {
	...
	print_repeater_debug(RPT_EXT_INFO, "dpu_dpms_off %d\n", ctx->dpu_dpms_off);
	...
	if (ctx->ctx_status.status == REPEATER_CTX_START) {
		...
		print_repeater_debug(RPT_ERROR, "ctx_status.status is invalid %d",
			ctx->ctx_status.status);
	}
	...
	struct repeater_context *ctx, struct repeater_info *info)
{
	...
	if (ctx->info.fps != 30 && ctx->info.fps != 60) {
		print_repeater_debug(RPT_ERROR, "fps is invalid %d",
			ctx->info.fps);
		ctx->info.fps = 30;
	}
...
int repeater_ioctl_start(struct repeater_context *ctx, int start_mode)
{
	...
	print_repeater_debug(RPT_INT_INFO, "++\n");
	print_repeater_debug(RPT_INT_INFO, "ctx_status.status %d, fps %d, start_mode %d\n",
	...
	if (ctx->ctx_status.buffer_status == REPEATER_CTX_UNMAP) {
		print_repeater_debug(RPT_ERROR, "buffer_status is unmap %d\n",
			ctx->ctx_status.buffer_status);
	}
...
int repeater_ioctl_stop(struct repeater_context *ctx)
{
	...
	print_repeater_debug(RPT_INT_INFO, "++\n");
	print_repeater_debug(RPT_INT_INFO, "ctx_status.status %d\n",
		ctx->ctx_status.status);
	...
	struct repeater_context *ctx, int max_skipped_frame)
{
	...
	if (max_skipped_frame >= 0) {
		...
		print_repeater_debug(RPT_ERROR, "max_skipped_frame is invalid %d\n",
			max_skipped_frame);
		ret = -EFAULT;
	}
	...
	struct repeater_context *ctx, int *status)
{
	...
	print_repeater_debug(RPT_INT_INFO, "ctx->ctx_status.status %d, ctx->error %d\n",
	...
	if (ctx->ctx_status.status != REPEATER_CTX_START ||
		...
		print_repeater_debug(RPT_ERROR, "repeater_ioctl_get_status(), status is %d, test error %d\n",
	...
	if (ctx->error) {
		print_repeater_debug(RPT_ERROR, "repeater_ioctl_get_status(), ctx->error is %d\n",
			ctx->ctx_status.status);
		*status = ctx->error;
		spin_unlock_irqrestore(&repeater_spinlock, flags);
		return ret;
...
static int repeater_ioctl_get_log_info(struct repeater_context *ctx, char* log)
{
	...
	if (g_repeater_logger == NULL) {
		print_repeater_debug(RPT_ERROR, "g_repeater_logger is NULL\n");
		return -EFAULT;
	}

	print_repeater_debug(RPT_INT_INFO, "g_repeater_logger rewind %d, offset %d, size %d\n",
	...
	if (err) {
		print_repeater_debug(RPT_ERROR, "fail to copy_to_user\n");
		return err;
	} else {
		return log_size;
	}
...
static int repeater_open(struct inode *inode, struct file *filp)
{
	...
	g_repeater_logger = wfd_logger_init();
...
err_misc_register:
	print_repeater_debug(RPT_ERROR, "--, err_misc_dev\n");
