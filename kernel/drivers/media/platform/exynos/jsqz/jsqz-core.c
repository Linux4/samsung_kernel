				    ...
				    int buf_index)
{
	...
	if (dir == DMA_FROM_DEVICE) {
		unsigned int video_w = task->user_mtask.video_info.width;
		unsigned int video_h = task->user_mtask.video_info.height;
		unsigned int ip_w = (video_w >> 4) + (((video_w % (1 << 4)) > 0) ? 1 : 0);
		unsigned int ip_h = (video_h >> 4) + (((video_h % (1 << 4)) > 0) ? 1 : 0);
		...
		*bytes_used = ip_w*ip_h;
		...
		if (task->type == JPEG_SQZ) {
			unsigned int y_width, y_height, c_width, c_height, stride;
			...
			if (task->user_mtask.video_info.stride != 0)
				*bytes_used = task->user_mtask.video_info.stride * task->user_mtask.video_info.height;
			else
				...
				unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		case HWJSQZ_IOC_PROCESS:
		{
			...
			if ((data.user_task.info_out.width > 0x1fff) ||
				(data.user_task.info_out.height > 0xfff) || (data.user_task.info_out.stride > 0x1fff)) {
				dev_err(jsqz_device->dev,
					"%s: image width or height is out of range, width : %d, height : %d, stride %d\n",
					__func__, data.user_task.info_out.width, data.user_task.info_out.height, data.user_task.info_out.stride);
				return -EINVAL;
			}
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_HWJSQZ_IOC_PROCESS:
	{
		...
		if ((task.user_task.info_out.width > 0x1fff) ||
			(task.user_task.info_out.height > 0xfff) || (task.user_task.info_out.stride > 0x1fff)) {
			dev_err(jsqz_device->dev,
				"%s: image width or height is out of range, width : %d, height : %d, stride %d\n",
				__func__, task.user_task.info_out.width, task.user_task.info_out.height, task.user_task.info_out.stride);
			return -EINVAL;
		}
...
static int jsqz_clock_gating(struct jsqz_dev *jsqz, bool on)
{
	...
	if (!IS_ERR(jsqz->clk_producer)) {
		...
		dev_dbg(jsqz->dev, "%s failed to gating clock : %d, %d\n"
					, __func__, ret, on);
	}
...
module_platform_driver(jsqz_driver);
