...
static int parse_cpu_data(struct device *dev, struct pablo_lib_platform_data *plpd)
{
	...
	if (cluster_np) {
		for (i = 0; i < PABLO_CPU_CLUSTER_MAX; i++)
			plpd->cpu_cluster[i] = PABLO_CPU_MAX;

		of_property_read_u32(cluster_np, "cluster0",
			&plpd->cpu_cluster[PABLO_CPU_CLUSTER_LIT]);
		of_property_read_u32(cluster_np, "cluster1",
			&plpd->cpu_cluster[PABLO_CPU_CLUSTER_MID]);
		of_property_read_u32(cluster_np, "cluster1h",
			&plpd->cpu_cluster[PABLO_CPU_CLUSTER_MID_HF]);
		of_property_read_u32(cluster_np, "cluster2",
			&plpd->cpu_cluster[PABLO_CPU_CLUSTER_BIG]);
	} else {
		dev_info(dev, "there's no cpu cluster info");
		return -ENXIO;
	}

	is_info("pablo-lib: cpu cluster(");
	for (i = 0; i < PABLO_CPU_CLUSTER_MAX; i++)
		pr_cont("%d, ", plpd->cpu_cluster[i]);
	pr_cont(")\n");
		...
		struct pablo_lib_data *data)
{
	...
	ret = parse_cpu_data(dev, pl_platform_data);
	if (ret)
		return ret;
