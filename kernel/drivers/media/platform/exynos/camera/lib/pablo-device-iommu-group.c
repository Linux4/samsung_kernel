...
EXPORT_SYMBOL_GPL(pablo_iommu_group_module_get);

#if IS_ENABLED(CONFIG_PABLO_KUNIT_TEST)
void pablo_iommu_group_module_set(
	struct pablo_device_iommu_group_module *external_iommu_group_module)
{
	iommu_group_module = external_iommu_group_module;
}
KUNIT_EXPORT_SYMBOL(pablo_iommu_group_module_set);
#endif
...
EXPORT_SYMBOL_GPL(pablo_iommu_group_get);
				...
				struct pablo_iommu_group_data *data)
{
	...
	dma_set_mask(iommu_group->dev, DMA_BIT_MASK(36));
#else
	dma_set_mask(iommu_group->dev, DMA_BIT_MASK(32));
#endif
...
static int pablo_iommu_group_remove(struct platform_device *pdev,
				    struct pablo_iommu_group_data *data)
{
	struct pablo_device_iommu_group *iommu_group;
	struct device *dev = &pdev->dev;

	iommu_group = (struct pablo_device_iommu_group *)platform_get_drvdata(pdev);
	if (!iommu_group) {
		dev_err(&pdev->dev, "no memory for iommu group device\n");
		return -ENOMEM;
	}

#if defined(CONFIG_PM)
	pm_runtime_disable(iommu_group->dev);
#endif
...
static struct pablo_iommu_group_data pablo_iommu_group = {
	...
	.remove = pablo_iommu_group_remove,
};
...
static int pablo_iommu_group_module_remove(struct platform_device *pdev,
					   struct pablo_iommu_group_data *data)
{
	struct pablo_device_iommu_group_module *iommu_group_module;
	struct device *dev = &pdev->dev;

	iommu_group_module = (struct pablo_device_iommu_group_module *)platform_get_drvdata(pdev);
	if (!iommu_group_module) {
		dev_err(&pdev->dev, "no memory for iommu group module\n");
		return -ENOMEM;
	}

	devm_kfree(dev, iommu_group_module->iommu_group);
	devm_kfree(dev, iommu_group_module);
...
static struct pablo_iommu_group_data pablo_iommu_group_module = {
	...
	.remove = pablo_iommu_group_module_remove,
};
...
static struct platform_driver pablo_iommu_group_driver = {
...
{
	return &pablo_iommu_group_driver;
}
EXPORT_SYMBOL_GPL(pablo_iommu_group_get_platform_driver);
