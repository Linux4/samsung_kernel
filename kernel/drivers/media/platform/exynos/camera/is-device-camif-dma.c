...
struct is_camif_wdma *is_camif_wdma_get(unsigned int wdma_ch_hint)
{
	...
	if (wdma_ch_hint >= max_num_of_wdma) {
		for (i = 0; i < max_num_of_wdma; i++) {
			wdma = wdma_chs[i];
			if (!wdma->mod->port_actcnt[wdma->sysmmu_port]) {
				wdma->active_cnt++;
				wdma->mod->port_actcnt[wdma->sysmmu_port]++;
				break;
			}
			wdma = NULL;
		}
	...
	info("%s: ch(mod%d, dma%d) hint %d\n", __func__, wdma->mod->index, wdma->ch, wdma_ch_hint);
...
int is_camif_wdma_put(struct is_camif_wdma *wdma)
{
	...
	port_actcnt = wdma->mod->port_actcnt[wdma->sysmmu_port];
	if (port_actcnt > 0)
		wdma->mod->port_actcnt[wdma->sysmmu_port]--;

	spin_unlock_irqrestore(&is_camif_wdma.slock, flags);

	if (active_cnt != 1 || port_actcnt <= 0) {
		warn("invalid active_cnt(%d, %d) ch(mod%d, dma%d)", active_cnt, port_actcnt,
		     wdma->mod->index, wdma->ch);
		return 0;
	}

	info("%s: ch(mod%d, dma%d)\n", __func__, wdma->mod->index, wdma->ch);
...
void is_camif_wdma_init(void)
{
	...
	for (i = 0; i < max_num_of_wdma; i++) {
		...
		csi_hw_s_init_input_mux(wdma->regs_mux, i, wdma->ch);

		wdma->active_cnt = 0;
		wdma->mod->port_actcnt[wdma->sysmmu_port] = 0;
	}
}
				...
				struct is_camif_dma_data *data)
{
	...
	wdma->dev = &pdev->dev;
	wdma->data = data;
	wdma->ch = of_alias_get_id(node, data->alias_stem);
	if ((wdma->ch < 0) || (wdma->ch >= (max_num_of_wdma))) {
	...
	snprintf(wdma->irq_name, name_len, "%s-%d", "CAMIF.DMA", wdma->ch);

	if (of_property_read_u32(node, "sysmmu_port", &wdma->sysmmu_port))
		wdma->sysmmu_port = 0;

	wdma_chs[wdma->ch] = wdma;
					...
					struct is_camif_dma_data *data)
{
	...
	wdma_module->port_actcnt = devm_kzalloc(dev, sizeof(int) * max_num_of_wdma, GFP_KERNEL);
	if (!wdma_module->port_actcnt) {
		dev_err(dev, "failed to allocate wdma_module->port_actcnt\n");
		ret = -ENOMEM;
		goto err_alloc_port_actcnt;
	}
...
err_alloc_wdma_chs:
	devm_kfree(dev, wdma_module->port_actcnt);

err_alloc_port_actcnt:
