...
#include "pablo-hw-reg-gdc-v200.h"
						...
						struct gdc_frame *d_frame, struct gdc_crop_param *crop_param)
{
	...
	GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_CONFIG], &gdc_fields[GDC_F_GDC_MIRROR_X], 0);
	GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_CONFIG], &gdc_fields[GDC_F_GDC_MIRROR_Y], 0);
	...
	GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_SCALE], &gdc_fields[GDC_F_GDC_SCALE_X], gdc_scale_x);
	GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_SCALE], &gdc_fields[GDC_F_GDC_SCALE_Y], gdc_scale_y);
	...
	/* now is selected no scaling. => calculation (128 * in / out) */
...
int camerapp_hw_gdc_update_param(struct pablo_mmio *pmio, struct gdc_dev *gdc)
{
	...
	if (d_frame->pixel_size == CAMERA_PIXEL_SIZE_8_2BIT)
		output_dither = 1;
	GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_DITHER], &gdc_fields[GDC_F_DST_DITHER], output_dither);
	GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_DITHER], &gdc_fields[GDC_F_SRC_INVDITHER], input_dither);

	/* interpolation type */
	/* 0 -all closest / 1 - Y bilinear, UV closest / 2 - all bilinear / 3 - Y bi-cubic, UV bilinear */
	GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_INTERPOLATION], &gdc_fields[GDC_F_GDC_INTERP_TYPE], 3);
	/* Clamping type: 0 - none / 1 - min/max / 2 - near pixels min/max */
	GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_INTERPOLATION], &gdc_fields[GDC_F_GDC_CLAMP_TYPE], 2);
