...
{
	if (on) {
		exynos_pmu_update(ois_mcu_regs[OIS_CPU_CONFIGURATION].sfr_offset,
...
int __is_mcu_hw_enable(void __iomem *base)
{
	...
	is_mcu_hw_set_field(base, OIS_CM0P_REMAP_I2C_ADDR, F_OIS_CM0P_I2C_ADDR, 0x13920000); /* USI10 */
	is_mcu_hw_set_field(base, OIS_CM0P_REMAP_SPI_ADDR, F_OIS_CM0P_SPI_ADDR, 0x13910000); /* USI09 */
#else
	...
	is_mcu_hw_set_field(base, OIS_CM0P_REMAP_I2C_ADDR, F_OIS_CM0P_I2C_ADDR, 0x119D0000); /* USI17 */
	is_mcu_hw_set_field(base, OIS_CM0P_REMAP_SPI_ADDR, F_OIS_CM0P_SPI_ADDR, 0x119E0000); /* USI18 */
#endif
...
int __is_mcu_hw_set_clock_peri(void __iomem *base)
{
	...
	is_mcu_set_reg(base, OIS_PERI_FS1, 0x01f0ff00);
	is_mcu_set_reg(base, OIS_PERI_FS2, 0x7f0003e0);
	is_mcu_set_reg(base, OIS_PERI_FS3, 0x001a0000);
...
int __is_mcu_hw_set_init_peri(void __iomem *base)
{
	...
	src = is_mcu_get_reg(base, OIS_PERI_PUD_CTRL);
	recover_val = src & 0x0000FFFF;
	recover_val |= 0x00000000;
	is_mcu_set_reg(base, OIS_PERI_PUD_CTRL, recover_val);

	/* GPP2 */
	src = is_mcu_get_reg(base, OIS_PERI2_CON_CTRL);
	recover_val = src & 0xFFFFFF00;	/* 0~1 bit mask */
	recover_val |= 0x00000044; /* USI10 I2C */
	is_mcu_set_reg(base, OIS_PERI2_CON_CTRL, recover_val);
	...
	is_mcu_set_reg(base, OIS_PERI_I3C_CON_CTRL, recover_val);

	src = is_mcu_get_reg(base, OIS_PERI_I3C_PUD_CTRL);
	recover_val = src & 0xFFFFFF00;
	is_mcu_set_reg(base, OIS_PERI_I3C_PUD_CTRL, recover_val);

	src = is_mcu_get_reg(base, OIS_PERI_I3C_DRV_CTRL);
	recover_val = src & 0xFFFFFF00;
	recover_val |= 0x00000044;
	is_mcu_set_reg(base, OIS_PERI_I3C_DRV_CTRL, recover_val);
#endif
...
unsigned int is_mcu_hw_g_irq_state(void __iomem *base, bool clear)
{
	...
	src = is_mcu_get_reg(base, OIS_CM0P_IRQ_STATUS);
	if (clear)
		is_mcu_set_reg(base, OIS_CM0P_IRQ_CLEAR, src);
...
{
	is_mcu_set_reg(base, OIS_CM0P_IRQ_ENABLE, (intr_enable & 0xF));
}
