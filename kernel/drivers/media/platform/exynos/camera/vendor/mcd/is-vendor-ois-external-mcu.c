...
struct i2c_client *is_mcu_i2c_get_client(struct is_core *core)
{
	...
	struct is_vendor_specific *specific = core->vendor.private_data;
...
long ois_mcu_get_efs_data(struct i2c_client *client)
{
	...
	struct is_vendor_specific *specific;
	long raw_data_x = 0, raw_data_y = 0, raw_data_z = 0;

	core = is_get_is_core();
	specific = core->vendor.private_data;
...
#endif
			wx_pole = mcu_init.ois_gyro_direction[0];
			wy_pole = mcu_init.ois_gyro_direction[1];
			gyro_orientation = mcu_init.ois_gyro_direction[2];
#ifdef CAMERA_2ND_OIS
			tx_pole = mcu_init.ois_gyro_direction[3];
			ty_pole = mcu_init.ois_gyro_direction[4];
#endif
...
bool ois_mcu_read_gyro_noise_mcu(struct is_core *core, long *x_value, long *y_value)
{
	...
	do {
		...
		if (--retries < 0) {
			err("0x0029 is still 0x0. try again (retries : %d)", temp, retries);
			ret = -1;
			break;
		}
		usleep_range(10000, 11000);
