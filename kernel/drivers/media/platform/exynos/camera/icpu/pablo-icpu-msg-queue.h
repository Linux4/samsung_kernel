...
struct icpu_msg_queue {
	...
	void (*dump)(struct icpu_msg_queue *queue, int num);
};
...
#define QUEUE_FULL(q, n) (QUEUE_LEN(q) == n)
...
#define MSG_DUMP(m)										\
	do {											\
		int i;										\
		u64 time = (m)->time;								\
		unsigned long usec = do_div(time, NSEC_PER_SEC) / NSEC_PER_USEC;		\
		ICPU_INFO("\t[%5llu.%06lu]", time, usec);					\
		for (i = 0; i < (m)->len; i++)							\
			ICPU_INFO("\tmsg->data[%d]=0x%x(dec.%d)", i, (m)->data[i], (m)->data[i]);	\
	} while(0)

#define QUEUE_DUMP(q, num)                                                                         \
	{                                                                                          \
		if (q && (q)->dump)                                                                \
			(q)->dump(q, num);                                                         \
	}
