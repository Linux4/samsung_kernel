...
static irqreturn_t __pablo_icpu_mbox_isr(int irq, void *cookie)
{
	...
	int_val = readl(info->int_gen_reg);
	int_val = int_val & 0xfffffeff;
	writel(int_val, info->int_gen_reg);

	ICPU_DEBUG("gen 0x%x, status: 0x%x\n", readl(info->int_gen_reg), readl(info->int_status_reg));
...
static int pablo_icpu_mbox_rx_startup(struct pablo_icpu_mbox_chan *chan)
{
	...
	ret = pablo_request_irq(info->irq, __pablo_icpu_mbox_isr,
			"ICPU_MBOX_IRQ", mbox->irq_flags, chan);
	if (ret)
		return ret;
