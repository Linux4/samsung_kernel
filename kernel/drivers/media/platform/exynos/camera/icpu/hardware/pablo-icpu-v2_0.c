...
#define ICPU_SYSCTRL_MBOX_FROM_HOST		0x1440
#define ICPU_SYSCTRL_MBOX_DBG_REG4_IDX 58
#define ICPU_SYSCTRL_MBOX_DBG_REG5_IDX 59

#define ICPU_SYSCTRL_MBOX_DBG_START		20
#define ICPU_SYSCTRL_MBOX_DBG_END		60
...
static int __set_icpu_release_reset(
	void __iomem *sysctrl_base, struct regmap *reg_reset, unsigned int bit, unsigned int on)
{
	int ret;
	unsigned int mask;
	unsigned int val;
...
static void __print_debug_reg(void __iomem *mcuctl_core_base, void __iomem *sysctrl_base)
{
	u32 i;
	...
	/* print icpu sysctrl dbg reg */
	ICPU_INFO("DBG0: 0x%x, DBG1: 0x%x, DBG2: 0x%x, DBG3: 0x%x",
	...
	for (i = ICPU_SYSCTRL_MBOX_DBG_START; i < ICPU_SYSCTRL_MBOX_DBG_END; i += 4) {
		ICPU_INFO("MBOX_DBG%d: 0x%08x,%d: 0x%08x,%d: 0x%08x,%d: 0x%08x",
			i, ioread32(sysctrl_base + ICPU_SYSCTRL_MBOX_FROM_HOST + (sizeof(u32)*i)),
			i + 1, ioread32(sysctrl_base + ICPU_SYSCTRL_MBOX_FROM_HOST + (sizeof(u32)*(i + 1))),
			i + 2, ioread32(sysctrl_base + ICPU_SYSCTRL_MBOX_FROM_HOST + (sizeof(u32)*(i + 2))),
			i + 3, ioread32(sysctrl_base + ICPU_SYSCTRL_MBOX_FROM_HOST + (sizeof(u32)*(i + 3))));
	}
}

static void __panic_handler(void __iomem *mcuctl_core_base, void __iomem *sysctrl_base)
{
	__print_debug_reg(mcuctl_core_base, sysctrl_base);
}
...
static void __set_debug_reg(void __iomem *sysctrl_base, u32 index, u32 val)
{
	...
	case 3:
		...
		break;
	case 4:
		iowrite32(val, sysctrl_base + ICPU_SYSCTRL_MBOX_FROM_HOST +
				       (sizeof(u32) * ICPU_SYSCTRL_MBOX_DBG_REG4_IDX));
		break;
	case 5:
		iowrite32(val, sysctrl_base + ICPU_SYSCTRL_MBOX_FROM_HOST +
				       (sizeof(u32) * ICPU_SYSCTRL_MBOX_DBG_REG5_IDX));
		break;
	default:
