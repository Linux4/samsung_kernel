...
#define ICPU_ERR(fmt, args...)                                                                     \
	do {                                                                                       \
		if (_log.level >= LOGLEVEL_ERR)                                                    \
			pr_err("[@][ERR]" _PREFIX fmt, _log.prefix, __func__, __LINE__, ##args);   \
		is_memlog_err("[ERR]" fmt "\n", ##args);                                           \
	} while (0)
...
#define ICPU_WARN(fmt, args...)                                                                    \
	do {                                                                                       \
		if (_log.level >= LOGLEVEL_WARNING)                                                \
			pr_warn("[@][WRN]" _PREFIX fmt, _log.prefix, __func__, __LINE__, ##args);  \
		is_memlog_warn("[WRN]" fmt "\n", ##args);                                          \
	} while (0)

#define ICPU_INFO(fmt, args...)                                                                    \
	do {                                                                                       \
		if (_log.level >= LOGLEVEL_INFO)                                                   \
			pr_info("[@]" _PREFIX fmt, _log.prefix, __func__, __LINE__, ##args);       \
		is_memlog_info(fmt "\n", ##args);                                                       \
	} while (0)
...
#define ICPU_TIME_DECLARE() ktime_t __time_dgb_st = 0
