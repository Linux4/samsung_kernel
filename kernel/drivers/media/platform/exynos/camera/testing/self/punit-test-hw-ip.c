...
#define NUM_OF_FRAME_PARAM (sizeof(struct frame_param)/PARAMETER_MAX_SIZE)
static u32 pst_fparam[NUM_OF_FRAME_PARAM][PARAMETER_MAX_MEMBER];
...
static int pst_update_param(int argc, char **argv, u32 param[][PARAMETER_MAX_MEMBER])
{
	int ret, i;
	u32 param_num, idx, param_index, sub_index, value;

	if (argc < PST_UPDATE_MAX) {
		err("%s: Not enough parameters. %d < %d", __func__, argc, PST_UPDATE_VALUE);
		err("e.g. echo [number of parameter] [parameter ndex] [parameter sub-index] [parameter value]");
		return -EINVAL;
	}

	ret = kstrtouint(argv[PST_UPDATE_PARAM_NUM], 0, &param_num);
	if (ret) {
		err("Invalid PST_UPDATE_PARAM_NUM(ret %d)", ret);
		return ret;
	}

	for (i = 0; i < param_num; i++) {
		idx = i * PST_UPDATE_STRIDE;
		ret = kstrtouint(argv[idx + PST_UPDATE_PARAM_IDX], 0, &param_index);
		ret |= kstrtouint(argv[idx + PST_UPDATE_SUBPARAM_IDX], 0, &sub_index);
		ret |= kstrtouint(argv[idx + PST_UPDATE_VALUE], 0, &value);
		if (ret) {
...
static int pst_update_vector(
	struct size_cr_set *cr_set, unsigned int index, u32 reg_addr, u32 reg_data)
{
	if (index >= MAX_CR_SET) {
		err("[PUNIT] Out-of-bounds index(%d)", index);
		return -EINVAL;
	}
...
static int pst_check_q_cnt(struct is_hw_ip *hw_ip)
{
	struct is_framemgr *framemgr = hw_ip->framemgr;
	u32 q_cnt, try_cnt = 1000;

	do {
		fsleep(PST_WAIT_TIME_START_ISR);
		q_cnt = framemgr->queued_count[FS_HW_REQUEST];
		q_cnt += framemgr->queued_count[FS_HW_CONFIGURE];
		q_cnt += framemgr->queued_count[FS_HW_WAIT_DONE];
		pr_warn("%s: q_cnt(%d), try(%d)\n", __func__, q_cnt, try_cnt);
	} while (try_cnt-- && q_cnt);
...
static int pst_start_hw_group(struct is_frame *frame)
{
	int i, ret = 0;
	...
	if (!hw_ip) {
		pr_err("Any IP is not set for self-testing.\n");
		return -EINVAL;
	}
...
static int pst_init_frame(struct is_frame **frame)
{
	...
	*frame = vzalloc(sizeof(struct is_frame));
	...
	if (!(*frame)->shot_ext) {
		...
		ret = -ENOMEM;
		goto err_alloc_shot_ext;
	}

	(*frame)->shot = &(*frame)->shot_ext->shot;
	(*frame)->instance = 0;
	(*frame)->fcount = 1234;
	(*frame)->num_buffers = fp->num_buffers ? fp->num_buffers : 1;
	pr_info("frame: num_buffer(%d)\n", (*frame)->num_buffers);
	memset(fp, 0, sizeof(struct frame_param));
...
static void pst_result_hw_group(struct is_frame *frame, unsigned long *result, u32 idx, int fail)
{
	int i;
	struct is_hw_ip *hw_ip;
	struct is_core *core = is_get_is_core();
	struct is_hardware *hw = &core->hardware;
	bool accum_pass = true;
	...
	for (i = HW_SLOT_MAX - 1; i >= 0; i--) {
		...
		accum_pass &= test_bit(idx, result);
	}
			...
			const struct pst_callback_ops *pst_cb)
{
	...
	if (hw_id >= DEV_HW_END) {
		fail = pst_start_hw_group(frame);
		pst_result_hw_group(frame, result, tc_idx, fail);
	} else {
		...
		const struct pst_callback_ops *pst_cb)
{
	...
	case PST_HW_IP_UPDATE_PARAM:
		ret = pst_update_param(argc, argv, param);
		break;
	case PST_HW_IP_UPDATE_FRAME:
		ret = pst_update_param(argc, argv, pst_fparam);
		break;
	case PST_HW_IP_UPDATE_VECTOR:
		...
		ret = pst_update_vector(cr_set, index, reg_addr, reg_data);
		if (ret)
			goto func_exit;
		...
		const unsigned long *result)
{
	...
	for (i = 0; i < BITS_TO_LONGS(preset_size); i++)
		ret += sprintf(buffer + ret, "%lx ", result[i]);
...
static void pst_set_batch_size(struct is_frame *frame, size_t *size)
{
	int i, j, num_plane;

	/* check number of plane */
	for (num_plane = 0; size[num_plane]; num_plane++);

	for (i = 1; i < frame->num_buffers; i++) {
		for (j = 0; j < num_plane; j++)
			size[j + (i * num_plane)] = size[j];
	}
}
...
struct is_priv_buf *pst_set_dva(struct is_frame *frame, dma_addr_t *dva, size_t *size, u32 gid)
{
	...
	pst_set_batch_size(frame, size);

	for (i = 0; size[i]; i++)
		size_sum += size[i];

	pr_info("%s: batch(%d), plane(%d), size(%zu)\n",
		__func__, frame->num_buffers, i, size_sum);
