...
static void pablo_subdev_mcsp_tag_dma_kunit_test(struct kunit *test)
{
	...
	/* set filp mode and sbwc flag */
	frame->shot_ext->mcsc_flip[0] = CAM_FLIP_MODE_HORIZONTAL;
	frame->shot_ext->node_group.capture[0].flags |= DMA_INPUT_SBWC_LOSSY_32B << PIXEL_TYPE_EXTRA_SHIFT;
