...
static void pablo_subdev_mcfp_get_kunit_test(struct kunit *test)
{
	...
	struct is_subdev *subdev = &idi->group[GROUP_SLOT_MCFP]->leader;
...
static void pablo_subdev_mcfp_tag_otf_kunit_test(struct kunit *test)
{
	...
	/* otf in */
	set_bit(IS_GROUP_OTF_INPUT, &idi->group[GROUP_SLOT_MCFP]->state);
	__set_test_vector_cropRegion(node->input.cropRegion);

	/* otf out */
	set_bit(IS_GROUP_OTF_OUTPUT, &idi->group[GROUP_SLOT_MCFP]->state);
...
static int pablo_subdev_mcfp_kunit_test_init(struct kunit *test)
{
	...
	idi->group[GROUP_SLOT_MCFP] = &test_ctx.group;
	subdev = &idi->group[GROUP_SLOT_MCFP]->leader;
