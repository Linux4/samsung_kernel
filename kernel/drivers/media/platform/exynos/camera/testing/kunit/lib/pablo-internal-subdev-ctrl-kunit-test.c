...
static void pablo_internal_subdev_ctrl_probe_kunit_test(struct kunit *test)
{
	...
	ret = pablo_internal_subdev_probe(sd, 0, mem, "KUNIT");
...
static void pablo_internal_subdev_ctrl_free_kunit_test(struct kunit *test)
{
	...
	pablo_internal_subdev_probe(sd, 0, mem, "KUNIT");
...
static void pablo_internal_subdev_ctrl_alloc_kunit_test(struct kunit *test)
{
	...
	/* TC#3. alloc --> alloc */
	sd->size[0] = sd->height *
		      ALIGN(DIV_ROUND_UP(sd->width * sd->memory_bitwidth, BITS_PER_BYTE), 32);
	sd->size[1] = sd->height / 2 *
		      ALIGN(DIV_ROUND_UP(sd->width / 2 * sd->memory_bitwidth, BITS_PER_BYTE), 32);
