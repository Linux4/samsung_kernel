...
#include "hardware/sfr/is-sfr-yuvp-v2_20.h"
...
static struct is_rectangle yuvp_size_test_config[] = {
	{ 128, 32 }, /* min size */
	{ 4880, 12288 }, /* max size */
...
static void pablo_hw_api_yuvp_hw_is_occurred1_kunit_test(struct kunit *test)
{
	...
	int test_idx;
	u32 err_interrupt_list[] = {
		INTR2_YUVP_VOTF_LOST_FLUSH,	    INTR2_YUVP_SYNCFIFO_SEG_DBG_CNT_ERR,
		INTR2_YUVP_DTP_DBG_CNT_ERR,	    INTR2_YUVP_YUVNR_DBG_CNT_ERR,
		INTR2_YUVP_YUV422TO444_DBG_CNT_ERR, INTR2_YUVP_YUV2RGB_DBG_CNT_ERR,
		INTR2_YUVP_DEGAMMA_DBG_CNT_ERR,	    INTR2_YUVP_INVCCM_DBG_CNT_ERR,
		INTR2_YUVP_DGF_DBG_CNT_ERR,	    INTR2_YUVP_DRCDSTR_DBG_CNT_ERR,
		INTR2_YUVP_DRCTMC_DBG_CNT_ERR,	    INTR2_YUVP_CCM_DBG_CNT_ERR,
		INTR2_YUVP_RGB_GAMMA_DBG_CNT_ERR,   INTR2_YUVP_SVHIST_DBG_CNT_ERR,
		INTR2_YUVP_CLAHE_DBG_CNT_ERR,	    INTR2_YUVP_PRC_DBG_CNT_ERR,
		INTR2_YUVP_CLAHE_HIST_ERR,	    INTR_YUVP_DMA_R2D_CONTENTION_ERR,
	};
...
static void pablo_hw_api_yuvp_hw_s_init_kunit_test(struct kunit *test)
{
	...
	set_val =
		YUVP_GET_F(test_ctx.pmio, YUVP_R_CMDQ_QUE_CMD_M, YUVP_F_CMDQ_QUE_CMD_SETTING_MODE);
	expected_val = 3;
...
static void pablo_hw_api_yuvp_hw_s_coutfifo_kunit_test(struct kunit *test)
{
	...
	set_val =
		YUVP_GET_F(test_ctx.pmio, YUVP_R_IP_USE_OTF_PATH, YUVP_F_IP_USE_OTF_OUT_FOR_PATH_0);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val =
		YUVP_GET_F(test_ctx.pmio, YUVP_R_IP_USE_OTF_PATH, YUVP_F_IP_USE_OTF_OUT_FOR_PATH_1);
	expected_val = 0;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = YUVP_GET_F(test_ctx.pmio, YUVP_R_YUV_COUTFIFO0_INTERVALS,
			     YUVP_F_YUV_COUTFIFO0_INTERVAL_HBLANK);
	expected_val = HBLANK_CYCLE;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = YUVP_GET_F(test_ctx.pmio, YUVP_R_YUV_COUTFIFO1_INTERVALS,
			     YUVP_F_YUV_COUTFIFO1_INTERVAL_HBLANK);
	expected_val = HBLANK_CYCLE;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = YUVP_GET_F(test_ctx.pmio, YUVP_R_YUV_COUTFIFO0_INTERVAL_VBLANK,
			     YUVP_F_YUV_COUTFIFO0_INTERVAL_VBLANK);
	expected_val = VBLANK_CYCLE;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
	set_val = YUVP_GET_F(test_ctx.pmio, YUVP_R_YUV_COUTFIFO1_INTERVAL_VBLANK,
	...
	set_val = YUVP_GET_F(test_ctx.pmio, YUVP_R_YUV_COUTFIFO0_CONFIG,
	...
	set_val = YUVP_GET_F(test_ctx.pmio, YUVP_R_YUV_COUTFIFO1_CONFIG,
...
static void pablo_hw_api_yuvp_hw_s_cinfifo_kunit_test(struct kunit *test)
{
	...
	set_val = YUVP_GET_F(test_ctx.pmio, YUVP_R_YUV_CINFIFO0_INTERVALS,
	...
	set_val = YUVP_GET_F(test_ctx.pmio, YUVP_R_YUV_CINFIFO0_CONFIG,
			     YUVP_F_YUV_CINFIFO0_STALL_BEFORE_FRAME_START_EN);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val =
		YUVP_GET_F(test_ctx.pmio, YUVP_R_YUV_CINFIFO0_CONFIG, YUVP_F_YUV_CINFIFO0_DEBUG_EN);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
}
...
static void pablo_hw_api_yuvp_hw_s_core_kunit_test(struct kunit *test)
{
	...
	set_val = YUVP_GET_F(test_ctx.pmio, YUVP_R_CMDQ_QUE_CMD_L,
...
static void pablo_hw_api_yuvp_hw_s_rdma_addr_kunit_test(struct kunit *test)
{
	...
	ret = yuvp_hw_s_rdma_addr(&dma, &addr, plane, num_buffers, buf_idx, comp_sbwc_en,
...
static void pablo_hw_api_yuvp_hw_s_wdma_addr_kunit_test(struct kunit *test)
{
	...
	ret = yuvp_hw_s_wdma_addr(&dma, &addr, plane, num_buffers, buf_idx, comp_sbwc_en,
				  payload_size);
	KUNIT_EXPECT_EQ(test, ret, 0);
}
...
static void pablo_hw_api_yuvp_hw_s_yuvnr_size_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < ARRAY_SIZE(yuvp_size_test_config); test_idx++) {
		...
		yuvp_hw_s_yuvnr_size(test_ctx.pmio, set_id, start_pos, frame_width, dma_width,
				     dma_height, strip_enable, &cfg, shift_adder);
	}
}
...
static void pablo_hw_api_yuvp_hw_s_strip_kunit_test(struct kunit *test)
{
	...
	ret = yuvp_hw_s_strip(test_ctx.pmio, set_id, stripe_enable, start_pos, YUVP_STRIP_TYPE_NONE,
			      0, 0, 0);
	KUNIT_EXPECT_EQ(test, ret, 0);
}
...
static void pablo_hw_api_yuvp_hw_s_input_path_kunit_test(struct kunit *test)
{
	...
	set_val =
		YUVP_GET_F(test_ctx.pmio, YUVP_R_IP_USE_OTF_PATH, YUVP_F_IP_USE_OTF_IN_FOR_PATH_0);
	expected_val = path;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
}
...
{
	yuvp_hw_dump(test_ctx.pmio, HW_DUMP_CR);
	yuvp_hw_dump(test_ctx.pmio, HW_DUMP_DBG_STATE);
	yuvp_hw_dump(test_ctx.pmio, HW_DUMP_MODE_NUM);
}
