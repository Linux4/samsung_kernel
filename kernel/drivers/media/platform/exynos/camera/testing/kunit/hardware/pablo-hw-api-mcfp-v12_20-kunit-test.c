...
#include "hardware/sfr/is-sfr-mcfp-v12_20.h"
...
static void pablo_hw_api_mcfp_hw_s_rdma_init_kunit_test(struct kunit *test)
{
	...
	ret = mcfp_hw_s_rdma_init(&tctx->dma, &tctx->dma_input, &tctx->stripe_input, tctx->width,
				  tctx->height, &tctx->sbwc_en, &tctx->payload_size,
...
static void pablo_hw_api_mcfp_hw_s_rdma_addr_kunit_test(struct kunit *test)
{
	...
	ret = mcfp_hw_s_rdma_init(&tctx->dma, &tctx->dma_input, &tctx->stripe_input, tctx->width,
				  tctx->height, &tctx->sbwc_en, &tctx->payload_size,
				  &tctx->strip_offset, &tctx->header_offset, &tctx->config);
	ret = mcfp_hw_s_rdma_addr(&tctx->dma, tctx->addr, 0, 1, 0, tctx->sbwc_en,
				  tctx->payload_size, tctx->strip_offset, tctx->header_offset);
	KUNIT_EXPECT_EQ(test, 0, ret);
...
static void pablo_hw_api_mcfp_hw_s_strgen_kunit_test(struct kunit *test)
{
	...
	set_val = MCFP_GET_F(tctx->pmio, MCFP_R_YUV_CINFIFO_CONFIG,
	...
	set_val = MCFP_GET_F(tctx->pmio, MCFP_R_IP_USE_CINFIFO_NEW_FRAME_IN,
...
static int __pablo_hw_api_mcfp_pmio_init(struct kunit *test)
{
	...
	mcfp_hw_pmio_write_test(test_ctx.pmio, COREX_DIRECT);
	mcfp_hw_pmio_read_test(test_ctx.pmio, COREX_DIRECT);
...
struct kunit_suite pablo_hw_api_mcfp_kunit_test_suite = {
	.name = "pablo-hw-api-mcfp-kunit-test",
