...
static void pablo_hw_api_pdp_hw_s_core_kunit_test(struct kunit *test)
{
	...
	pdp_hw_s_core(pdp, enable, cfg, pd_width, pd_height, pd_hwformat, sensor_type, path,
		en_votf, num_buffers);
...
static void pablo_hw_api_pdp_hw_s_wdma_init_kunit_test(struct kunit *test)
{
	...
	PDP_SET_R(ctx->test_addr, PDP_R_Y_PDSTAT_ROI_MAIN_MWS_ON, mroi_on);
	PDP_SET_R(ctx->test_addr, PDP_R_Y_PDSTAT_ROI_MAIN_MWS_NO_X, mroi_no_x);
	PDP_SET_R(ctx->test_addr, PDP_R_Y_PDSTAT_ROI_MAIN_MWS_NO_Y, mroi_no_y);
	PDP_SET_R(ctx->test_addr, PDP_R_Y_PDSTAT_IN_SIZE_X, roi_w);
	PDP_SET_R(ctx->test_addr, PDP_R_Y_PDSTAT_IN_SIZE_Y, roi_h);

	pdp_hw_s_wdma_init(ctx->test_addr, 0, 0);
...
static void pablo_hw_api_pdp_hw_corex_resume_kunit_test(struct kunit *test)
{
	struct pablo_hw_api_pdp_test_ctx *ctx = test->priv;
	u32 val;

	pdp_hw_corex_resume(ctx->test_addr);

	val = PDP_GET_F(ctx->test_addr, PDP_R_COREX_UPDATE_TYPE_0, PDP_F_COREX_UPDATE_TYPE_0);
	KUNIT_EXPECT_EQ(test, val, COREX_COPY);
	val = PDP_GET_F(ctx->test_addr, PDP_R_COREX_START_0, PDP_F_COREX_START_0);
	KUNIT_EXPECT_EQ(test, val, 1);
	val = PDP_GET_F(ctx->test_addr, PDP_R_COREX_UPDATE_MODE_0, PDP_F_COREX_UPDATE_MODE_0);
	KUNIT_EXPECT_EQ(test, val, HW_TRIGGER);
}

static void pablo_hw_api_pdp_hw_s_default_blk_cfg_kunit_test(struct kunit *test)
{
	struct pablo_hw_api_pdp_test_ctx *ctx = test->priv;
	u32 val;

	pdp_hw_s_default_blk_cfg(ctx->test_addr);

	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_REORDER_ON);
	KUNIT_EXPECT_EQ(test, val, 0);
	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_REORDER_SUB_ON);
	KUNIT_EXPECT_EQ(test, val, 0);
	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_ALC_ON);
	KUNIT_EXPECT_EQ(test, val, 0);
	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_ALC_SUB_ON);
	KUNIT_EXPECT_EQ(test, val, 0);
	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_GAMMA_ON);
	KUNIT_EXPECT_EQ(test, val, 0);
	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_GAMMA_SUB_ON);
	KUNIT_EXPECT_EQ(test, val, 0);
	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_PDSTAT_SAT_ON);
	KUNIT_EXPECT_EQ(test, val, 0);
	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_PDSTAT_XCOR_H_ON);
	KUNIT_EXPECT_EQ(test, val, 0);
}
...
static struct kunit_case pablo_hw_api_pdp_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_api_pdp_hw_corex_resume_kunit_test),
	KUNIT_CASE(pablo_hw_api_pdp_hw_s_default_blk_cfg_kunit_test),
	{},
};
