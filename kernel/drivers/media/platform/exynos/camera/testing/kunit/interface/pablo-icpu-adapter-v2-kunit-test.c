...
static void pablo_icpu_adt_v2_open_kunit_test(struct kunit *test)
{
	...
	ret = __adt->ops->open(__adt, 0, NULL, NULL, 0);
...
static void pablo_icpu_adt_v2_open_negative_kunit_test(struct kunit *test)
{
	...
	ret = __adt->ops->open(__adt, IS_STREAM_COUNT, NULL, NULL, 0);
...
u32 __test_get_ch_fn(u32 instance)
{
	return 0;
}

static void pablo_icpu_adt_v2_register_get_ch_fn_kunit_test(struct kunit *test)
{
	int ret;
	...
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, __adt->ops->open);
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, __adt->ops->close);
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, __adt->ops->register_get_ch_fn);

	ret = __adt->ops->open(__adt, 0, NULL, NULL, 0);
	KUNIT_ASSERT_EQ(test, ret, 0);

	ret = __adt->ops->register_get_ch_fn(__adt, 0, __test_get_ch_fn);
	KUNIT_ASSERT_EQ(test, ret, 0);
...
static void pablo_icpu_adt_v2_register_get_ch_fn_negative_kunit_test(struct kunit *test)
{
	int ret;
...
static void pablo_icpu_adt_v2_send_msg_open_kunit_test(struct kunit *test)
{
	...
	ret = __adt->ops->open(__adt, instance, NULL, NULL, 0);
	KUNIT_ASSERT_EQ(test, ret, 0);
...
static void pablo_icpu_adt_v2_send_msg_put_buf_kunit_test(struct kunit *test)
{
	...
	/* icpu adt sends msg */
	get_random_bytes(&buf_info.idx, sizeof(u32));
...
static void pablo_icpu_adt_v2_send_msg_cstat_frame_end_kunit_test(struct kunit *test)
{
	...
	ret = __adt->msg_ops->send_msg_cstat_frame_end(__adt, instance, cb_user, cb_ctx,
			fcount, &shot_info, buf_info, edge_score);
	KUNIT_ASSERT_EQ(test, ret, 0);
...
static void pablo_icpu_adt_v2_receive_msg_control_sensor_kunit_test(struct kunit *test)
{
	...
	/* open icpu adt */
	ret = __adt->ops->open(__adt, instance, &__sensor_itf, bufmgr, 0);
	KUNIT_ASSERT_EQ(test, ret, 0);
...
static struct kunit_case pablo_icpu_adt_v2_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_icpu_adt_v2_register_get_ch_fn_kunit_test),
	KUNIT_CASE(pablo_icpu_adt_v2_register_get_ch_fn_negative_kunit_test),
...
static int pablo_icpu_adt_v2_kunit_test_init(struct kunit *test)
{
	...
	/* probe crta_bufmgr */
	__bufmgr = pablo_get_crta_bufmgr(PABLO_CRTA_BUF_SS_CTRL, 0, 0);
	...
	__ctx.rx_chan = kunit_kzalloc(test, sizeof(struct pablo_icpu_mbox_chan), GFP_KERNEL);
...
static void pablo_icpu_adt_v2_kunit_test_exit(struct kunit *test)
{
	...
	kunit_kfree(test, __ctx.rx_chan);
