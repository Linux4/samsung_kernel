...
#define MAX_MUX_LIST_CNT (BNS_INPUT_MUX_NUM + DMA_INPUT_MUX_NUM)
...
static void pablo_hw_csi_hw_s_dma_input_mux(struct kunit *test)
{
	...
	for (wdma_ch = 0; wdma_ch < DMA_INPUT_MUX_NUM; wdma_ch++) {
		func->csi_hw_s_dma_input_mux(reg_mux, wdma_ch, wdma_ch, bns_en, bns_dma_mux_val,
					     csi_ch, otf_ch, 0, 0, 0);
...
static void pablo_hw_csi_hw_s_init_input_mux(struct kunit *test)
{
	...
	for (wdma_ch = 0; wdma_ch < DMA_INPUT_MUX_NUM; wdma_ch++) {
		func->csi_hw_s_init_input_mux(reg_mux, wdma_ch, wdma_ch);
