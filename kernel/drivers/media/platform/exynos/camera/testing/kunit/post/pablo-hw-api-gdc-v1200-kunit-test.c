...
#include "gdc/pablo-hw-reg-gdc-v1200.h"
#else
#include "gdc/pablo-hw-reg-gdc-v1201.h"
#endif
#include "is-common-enum.h"
#include "is-type.h"

#define MAX_SUPPORT_SBWC_FMT 6
...
static u32 support_sbwc_fmt_array[MAX_SUPPORT_SBWC_FMT] = {
	...
	V4L2_PIX_FMT_NV12M_SBWCL_64_8B_FR,
	V4L2_PIX_FMT_NV12M_SBWCL_64_10B_FR,
};
...
static void camerapp_hw_get_comp_buf_size_kunit_test(struct kunit *test)
{
	...
	for (plane = 0; plane < 2; plane++) {
		val1 = camerapp_hw_get_comp_buf_size(gdc, &frame,
			width, height, pixfmt, plane, GDC_SBWC_SIZE_ALL);
		val2 = plane ?
			(u32)(SBWCL_64_CBCR_SIZE_FR(width, height) + SBWCL_CBCR_HEADER_SIZE(width, height)) :
			(u32)(SBWCL_64_Y_SIZE_FR(width, height) + SBWCL_Y_HEADER_SIZE(width, height));
		KUNIT_EXPECT_EQ(test, val1, val2);
	}
	...
	for (plane = 0; plane < 2; plane++) {
		val1 = camerapp_hw_get_comp_buf_size(gdc, &frame,
			width, height, pixfmt, plane, GDC_SBWC_SIZE_PAYLOAD);
		val2 = plane ?
			(u32)(SBWCL_64_CBCR_SIZE_FR(width, height)) : (u32)(SBWCL_64_Y_SIZE_FR(width, height));
		KUNIT_EXPECT_EQ(test, val1, val2);
	}
...
struct kunit_suite camerapp_hw_api_gdc_kunit_test_suite = {
	.name = "pablo-hw-api-gdc-v1200-kunit-test",
