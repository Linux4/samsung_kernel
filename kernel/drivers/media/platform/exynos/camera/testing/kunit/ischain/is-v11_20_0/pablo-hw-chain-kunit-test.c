...
static void pablo_hw_chain_is_hw_find_settle_kunit_test(struct kunit *test)
{
	u32 ret;
	u32 mipi_speed = 1000;
	...
	/* cphy */
	ret = 0;
	ret = is_hw_find_settle(mipi_speed, 1);
	KUNIT_EXPECT_NE(test, ret, 0);

	/* not cphy */
	ret = 0;
	ret = is_hw_find_settle(mipi_speed, 0);
	KUNIT_EXPECT_NE(test, ret, 0);
...
static void pablo_hw_chain_is_hw_get_output_slot_kunit_test(struct kunit *test)
{
	int ret;
	u32 vid = IS_VIDEO_COMMON;

	ret = is_hw_get_output_slot(vid);
	KUNIT_EXPECT_EQ(test, ret, -1);

	vid = IS_VIDEO_SS0_NUM;
	ret = is_hw_get_output_slot(vid);
	KUNIT_EXPECT_EQ(test, ret, 0);
}
...
static struct kunit_case pablo_hw_chain_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_chain_is_hw_find_settle_kunit_test),
	...
	KUNIT_CASE(pablo_hw_chain_is_hw_get_output_slot_kunit_test),
