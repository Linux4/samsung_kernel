...
static void pablo_debug_event_count_kunit_test(struct kunit *test)
{
	...
	prev_csi = atomic_read(&pkt_ctx.event->event_cnt[IS_EVENT_OVERFLOW_CSI].cnt);

	is_debug_event_count(IS_EVENT_OVERFLOW_CSI, 0, 0);

	cur_csi = atomic_read(&pkt_ctx.event->event_cnt[IS_EVENT_OVERFLOW_CSI].cnt);
	KUNIT_EXPECT_GT(test, cur_csi, prev_csi);

	/* TC #2. EVENT_OVERFLOW_3AA */
	prev_3aa = atomic_read(&pkt_ctx.event->event_cnt[IS_EVENT_OVERFLOW_3AA].cnt);

	is_debug_event_count(IS_EVENT_OVERFLOW_3AA, 0, 0);

	cur_3aa = atomic_read(&pkt_ctx.event->event_cnt[IS_EVENT_OVERFLOW_3AA].cnt);
	...
	is_debug_event_count(IS_EVENT_ALL, 0, 0);
...
static void pablo_debug_get_icpu_fw_loginfo_kunit_test(struct kunit *test)
{
	void *addr;
	u32 size;

	/* loginfo is empty because FW does not boot yet */
	addr = is_debug_get_icpu_fw_loginfo(&size);
	KUNIT_EXPECT_NULL(test, addr);
	KUNIT_EXPECT_EQ(test, size, (u32)0);
}
...
#endif
	pkt_ctx.overflow_csi = atomic_read(&pkt_ctx.event->event_cnt[IS_EVENT_OVERFLOW_CSI].cnt);
	pkt_ctx.overflow_3aa = atomic_read(&pkt_ctx.event->event_cnt[IS_EVENT_OVERFLOW_3AA].cnt);
...
#endif
	atomic_set(&pkt_ctx.event->event_cnt[IS_EVENT_OVERFLOW_CSI].cnt, pkt_ctx.overflow_csi);
	atomic_set(&pkt_ctx.event->event_cnt[IS_EVENT_OVERFLOW_3AA].cnt, pkt_ctx.overflow_3aa);
...
static struct kunit_case pablo_debug_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_debug_get_icpu_fw_loginfo_kunit_test),
