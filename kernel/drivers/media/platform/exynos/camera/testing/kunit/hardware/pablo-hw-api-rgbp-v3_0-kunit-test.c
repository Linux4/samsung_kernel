...
#include "hardware/sfr/is-sfr-rgbp-v3_1.h"
#else
#include "hardware/sfr/is-sfr-rgbp-v3_0.h"
#endif
...
static void pablo_hw_api_rgbp_hw_is_occurred0_kunit_test(struct kunit *test)
{
	...
	u32 err_interrupt_list[] = {
		...
		INTR0_RGBP_CINFIFO_0_ERROR_INT,
		INTR0_RGBP_COUTFIFO_0_ERROR_INT,
		INTR0_RGBP_COUTFIFO_1_ERROR_INT,
		INTR0_RGBP_OTF_SEQ_ID_ERROR_INT,
		INTR0_RGBP_VOTF_GLOBAL_ERROR_INT,
		INTR0_RGBP_VOTF_LOST_CONNECTION_INT,
		INTR0_RGBP_OTF_SEQ_ID_ERROR_INT,
	};
...
static void pablo_hw_api_rgbp_hw_is_occurred1_kunit_test(struct kunit *test)
{
	...
	int test_idx;
	u32 err_interrupt_list[] = {
		INTR1_RGBP_VOTFLOSTFLUSH,
		INTR1_RGBP_VOTF0RDMAGLOBALERROR,
		INTR1_RGBP_VOTF0RDMALOSTCONNECTION,
		INTR1_RGBP_VOTF0RDMALOSTFLUSH,
		INTR1_RGBP_VOTF1WDMALOSTFLUSH,
		INTR1_RGBP_DTP_DBG_CNT_ERR,
		INTR1_RGBP_TDMSC_DBG_CNT_ERR,
		INTR1_RGBP_DMSC_DBG_CNT_ERR,
		INTR1_RGBP_POST_GAMMA_DBG_CNT_ERR,
		INTR1_RGBP_LSC_DBG_CNT_ERR,
		INTR1_RGBP_GTM_DBG_CNT_ERR,
		INTR1_RGBP_CCM_DBG_CNT_ERR,
		INTR1_RGBP_RGB_GAMMA_DBG_CNT_ERR,
		INTR1_RGBP_RGB2YUV_DBG_CNT_ERR,
		INTR1_RGBP_YUV444TO422_DBG_CNT_ERR,
		INTR1_RGBP_SATFLAG_DBG_CNT_ERR,
		INTR1_RGBP_DECOMP_DBG_CNT_ERR,
		INTR1_RGBP_YUVSC_DBG_CNT_ERR,
		INTR1_RGBP_PPC_CONV_DBG_CNT_ERR,
		INTR1_RGBP_SYNC_MERGE_COUTFIFO_DBG_CNT_ERR,
	};

	status = 1 << INTR1_RGBP_VOTFLOSTFLUSH;
	ret = rgbp_hw_is_occurred1(status, INTR1_VOTFLOSTFLUSH_INT);
	KUNIT_EXPECT_EQ(test, ret, status);

	status = 1 << INTR1_RGBP_VOTF0RDMALOSTCONNECTION;
	ret = rgbp_hw_is_occurred1(status, INTR1_VOTF0RDMALOSTCONNECTION_INT);
	KUNIT_EXPECT_EQ(test, ret, status);


	status = 1 << INTR1_RGBP_VOTF0RDMALOSTFLUSH;
	ret = rgbp_hw_is_occurred1(status, INTR1_VOTF0RDMALOSTFLUSH_INT);
	KUNIT_EXPECT_EQ(test, ret, status);


	status = 1 << INTR1_RGBP_VOTF1WDMALOSTFLUSH;
...
static void pablo_hw_api_rgbp_check_cin_fifo(struct kunit *test)
{
	...
	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_IP_USE_OTF_PATH_01,
		RGBP_F_IP_USE_OTF_OUT_FOR_PATH_0);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_YUV_COUTFIFO0_INTERVALS,
		RGBP_F_YUV_COUTFIFO0_INTERVAL_HBLANK);
	expected_val = HBLANK_CYCLE;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_YUV_COUTFIFO0_CONFIG,
		RGBP_F_YUV_COUTFIFO0_VVALID_RISE_AT_FIRST_DATA_EN);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = RGBP_GET_F(test_ctx.pmio,
		RGBP_R_YUV_COUTFIFO0_CONFIG, RGBP_F_YUV_COUTFIFO0_DEBUG_EN);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = RGBP_GET_F(test_ctx.pmio,
		RGBP_R_YUV_COUTFIFO0_CONFIG, RGBP_F_YUV_COUTFIFO0_BACK_STALL_EN);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_YUV_COUTFIFO0_ENABLE,
		RGBP_F_YUV_COUTFIFO0_ENABLE);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_YUV_COUTFIFO0_INT_ENABLE,
		RGBP_F_YUV_COUTFIFO0_INT_ENABLE);
	expected_val = 0x7;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_YUV_COUTFIFO1_ENABLE,
...
static void pablo_hw_api_rgbp_check_cout_fifo(struct kunit *test)
{
	...
	set_val = RGBP_GET_F(test_ctx.pmio,
		RGBP_R_IP_USE_OTF_PATH_01, RGBP_F_IP_USE_OTF_OUT_FOR_PATH_0);
	KUNIT_EXPECT_EQ(test, set_val, otf_output);

	set_val = RGBP_GET_F(test_ctx.pmio,
			RGBP_R_YUV_COUTFIFO0_INTERVALS,
			RGBP_F_YUV_COUTFIFO0_INTERVAL_HBLANK);
	expected_val = HBLANK_CYCLE;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = RGBP_GET_F(test_ctx.pmio,
			RGBP_R_YUV_COUTFIFO0_INTERVAL_VBLANK,
	...
	set_val = RGBP_GET_F(test_ctx.pmio,
		RGBP_R_YUV_COUTFIFO1_ENABLE, RGBP_F_YUV_COUTFIFO1_ENABLE);
	KUNIT_EXPECT_EQ(test, set_val, !otf_output);
}

static void pablo_hw_api_rgbp_check_cout_fifo2(struct kunit *test)
{
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	struct rgbp_param_set *rgbp_param_set = &test_ctx.param_set;
	u32 set_val, expected_val;
	u32 otf_output_hf = rgbp_param_set->otf_output_hf.cmd;

	set_val = RGBP_GET_F(test_ctx.pmio,
			RGBP_R_STAT_COUTFIFO2_INTERVALS,
			RGBP_F_STAT_COUTFIFO2_INTERVAL_HBLANK);
	expected_val = HBLANK_CYCLE;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = RGBP_GET_F(test_ctx.pmio,
			RGBP_R_STAT_COUTFIFO2_INTERVAL_VBLANK,
			RGBP_F_STAT_COUTFIFO2_INTERVAL_VBLANK);
	expected_val = VBLANK_CYCLE;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_STAT_COUTFIFO2_CONFIG,
	...
	set_val = RGBP_GET_F(test_ctx.pmio,
			RGBP_R_STAT_COUTFIFO2_ENABLE,
			RGBP_F_STAT_COUTFIFO2_ENABLE);
	KUNIT_EXPECT_EQ(test, set_val, otf_output_hf);

	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_STAT_COUTFIFO2_INT_ENABLE,
...
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_PIXEL_BUFFER_EN, RGBP_F_PIXEL_BUFFER_EN);
	KUNIT_EXPECT_EQ(test, set_val, 1);
#endif
}
...
static void pablo_hw_api_rgbp_check_path(struct kunit *test)
{
	...
	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_CHAIN_DEMUX_ENABLE, RGBP_F_DEMUX_RDMABYR_ENABLE);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_CHAIN_DEMUX_ENABLE, RGBP_F_DEMUX_OTFOUT_ENABLE);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
...
static void pablo_hw_api_rgbp_hw_s_core_kunit_test(struct kunit *test)
{
	...
	rgbp_param_set->otf_output_hf.cmd = 1;
	...
	/* rgbp_hw_s_cout_fifo2() */
	pablo_hw_api_rgbp_check_cout_fifo2(test);
	/* rgbp_hw_s_common() */
...
static void pablo_hw_api_rgbp_hw_g_input_dva_kunit_test(struct kunit *test)
{
	...
	id = RGBP_RDMA_BYR;
	param_set->dma_input_rgb.cmd = 0;
	input_dva = rgbp_hw_g_input_dva(param_set, instance, id, &cmd, &in_rgb);
	KUNIT_EXPECT_EQ(test, cmd, param_set->dma_input.cmd);
	KUNIT_EXPECT_EQ(test, in_rgb, param_set->dma_input_rgb.cmd);
	KUNIT_EXPECT_PTR_EQ(test, input_dva, (pdma_addr_t *)param_set->input_dva);

	param_set->dma_input_rgb.cmd = 1;
	input_dva = rgbp_hw_g_input_dva(param_set, instance, id, &cmd, &in_rgb);
	KUNIT_EXPECT_EQ(test, cmd, param_set->dma_input_rgb.cmd);
	KUNIT_EXPECT_PTR_EQ(test, input_dva, (pdma_addr_t *)param_set->input_dva_rgb);
	...
	id = RGBP_RDMA_MAX;
	input_dva = rgbp_hw_g_input_dva(param_set, instance, id, &cmd, &in_rgb);
	KUNIT_EXPECT_PTR_EQ(test, input_dva, (pdma_addr_t *)NULL);
}
...
static void pablo_hw_api_rgbp_hw_g_output_dva_kunit_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, out_yuv, param_set->dma_output_yuv.cmd);
...
static void pablo_hw_api_rgbp_hw_s_rdma_addr_kunit_test(struct kunit *test)
{
	...
	ret = rgbp_hw_s_rdma_addr(&test_ctx.dma, &addr, plane,
			num_buffers, buf_idx, comp_sbwc_en, payload_size, in_rgb);
	KUNIT_EXPECT_EQ(test, ret, 0);
}
...
static void pablo_hw_api_rgbp_hw_s_cmdq_kunit_test(struct kunit *test)
{
	...
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_CMDQ_QUE_CMD_L, RGBP_F_CMDQ_QUE_CMD_INT_GROUP_ENABLE);
...
static void pablo_hw_api_rgbp_hw_s_sc_dst_size_size_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < RGBP_SIZE_TEST_SET; test_idx++) {
		...
		g_width = RGBP_GET_F(test_ctx.pmio, RGBP_R_YUV_SC_DST_SIZE,
			RGBP_F_YUV_SC_DST_HSIZE);
		g_height = RGBP_GET_F(test_ctx.pmio, RGBP_R_YUV_SC_DST_SIZE,
...
static void pablo_hw_api_rgbp_hw_s_block_bypass_kunit_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, val, 1);
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_RGB_LUMAADAPLSC_BYPASS, RGBP_F_RGB_LUMAADAPLSC_BYPASS);
	KUNIT_EXPECT_EQ(test, val, 1);
	...
	KUNIT_EXPECT_EQ(test, val, 0);
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_YUV_YUV444TO422_ISP_BYPASS, RGBP_F_YUV_YUV444TO422_BYPASS);
	KUNIT_EXPECT_EQ(test, val, 0);
...
static void pablo_hw_api_rgbp_hw_s_gamma_enable_kunit_test(struct kunit *test)
{
	...
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_Y_GAMMALR_BYPASS,
		RGBP_F_Y_GAMMALR_BYPASS);
	KUNIT_EXPECT_EQ(test, val, enable);
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_Y_GAMMAHR_BYPASS,
...
static void pablo_hw_api_rgbp_hw_s_strgen_kunit_test(struct kunit *test)
{
	...
	val = RGBP_GET_F(test_ctx.pmio, RGBP_R_IP_USE_OTF_PATH_01,
...
static void pablo_hw_api_rgbp_hw_s_dma_init_kunit_test(struct kunit *test)
{
	...
	ret = rgbp_hw_s_rdma_init(hw_ip, &test_ctx.dma, &test_ctx.param_set,
			/* instance, */ enable, in_crop_size_x, cache_hint, &sbwc_en, &payload_size, in_rgb);
	...
	ret = rgbp_hw_s_wdma_init(&test_ctx.dma, test_ctx.pmio, set_id, &test_ctx.param_set, &addr, &dma_cfg);
	KUNIT_EXPECT_EQ(test, ret, 1);
}
...
static void pablo_hw_api_rgbp_hw_g_dma_param_ptr_kunit_test(struct kunit *test)
{
	...
	ret = rgbp_hw_g_rdma_param_ptr(RGBP_RDMA_CFG_IMG, dma_frame, param_set,
	...
	ret = rgbp_hw_g_rdma_param_ptr(RGBP_RDMA_CFG_MAX, dma_frame, param_set,
		name, &dma_frame_dva, &pdi, &param_set_dva);
	...
	ret = rgbp_hw_g_wdma_param_ptr(RGBP_WDMA_CFG_HF, dma_frame, param_set,
	...
	ret = rgbp_hw_g_wdma_param_ptr(RGBP_WDMA_CFG_SF, dma_frame, param_set,
		name, &dma_frame_dva, &pdo, &param_set_dva);
	KUNIT_EXPECT_EQ(test, ret, (int)0);
	KUNIT_EXPECT_PTR_EQ(test, (dma_addr_t *)dma_frame_dva,
	...
	ret = rgbp_hw_g_wdma_param_ptr(RGBP_WDMA_CFG_YUV, dma_frame, param_set,
		name, &dma_frame_dva, &pdo, &param_set_dva);
	KUNIT_EXPECT_EQ(test, ret, (int)0);
	KUNIT_EXPECT_PTR_EQ(test, (dma_addr_t *)dma_frame_dva,
	...
	ret = rgbp_hw_g_wdma_param_ptr(RGBP_WDMA_CFG_RGB, dma_frame, param_set,
		name, &dma_frame_dva, &pdo, &param_set_dva);
	KUNIT_EXPECT_EQ(test, ret, (int)0);
	KUNIT_EXPECT_PTR_EQ(test, (dma_addr_t *)dma_frame_dva,
	...
	ret = rgbp_hw_g_wdma_param_ptr(RGBP_WDMA_CFG_MAX, dma_frame, param_set,
		name, &dma_frame_dva, &pdo, &param_set_dva);
	KUNIT_EXPECT_EQ(test, ret, -EINVAL);
...
static void pablo_hw_api_rgbp_hw_update_param_kunit_test(struct kunit *test)
{
	...
	rgbp_param->otf_output_hf.cmd = 1;
	...
	KUNIT_EXPECT_NE(test, rgbp_param_set->otf_output_hf.cmd, rgbp_param->otf_output_hf.cmd);
	...
	KUNIT_EXPECT_NE(test, rgbp_param_set->stripe_input.total_count, rgbp_param->stripe_input.total_count);
	...
	set_bit(PARAM_RGBP_OTF_OUTPUT_HF, pmap);
	...
	KUNIT_EXPECT_EQ(test, rgbp_param_set->otf_output_hf.cmd, rgbp_param->otf_output_hf.cmd);
	...
	KUNIT_EXPECT_EQ(test, rgbp_param_set->stripe_input.total_count, rgbp_param->stripe_input.total_count);
...
static void pablo_hw_api_rgbp_hw_error_handle_kunit_test(struct kunit *test)
{
	u32 input = 0xFF;
	u32 output;

	RGBP_SET_R(test_ctx.pmio, RGBP_R_BYR_CINFIFO_INT, input);
	RGBP_SET_R(test_ctx.pmio, RGBP_R_YUV_COUTFIFO0_INT, input);
	RGBP_SET_R(test_ctx.pmio, RGBP_R_YUV_COUTFIFO1_INT, input);
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	RGBP_SET_R(test_ctx.pmio, RGBP_R_STAT_COUTFIFO2_INT, input);
#endif

	rgbp_hw_int0_error_handle(test_ctx.pmio);

	output = RGBP_GET_R(test_ctx.pmio, RGBP_R_BYR_CINFIFO_INT_CLEAR);
	KUNIT_EXPECT_EQ(test, output, input);
	output = RGBP_GET_R(test_ctx.pmio, RGBP_R_YUV_COUTFIFO0_INT_CLEAR);
	KUNIT_EXPECT_EQ(test, output, input);
	output = RGBP_GET_R(test_ctx.pmio, RGBP_R_YUV_COUTFIFO1_INT_CLEAR);
	KUNIT_EXPECT_EQ(test, output, input);
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	output = RGBP_GET_R(test_ctx.pmio, RGBP_R_STAT_COUTFIFO2_INT_CLEAR);
	KUNIT_EXPECT_EQ(test, output, input);
#endif
}

static void pablo_hw_api_rgbp_hw_g_hf_mode_kunit_test(struct kunit *test)
{
	struct rgbp_param_set *rgbp_param_set = &test_ctx.param_set;
	u32 hf_mode;

	rgbp_param_set->dma_output_hf.cmd = 1;
	rgbp_param_set->otf_output_hf.cmd = 0;
	hf_mode = rgbp_hw_g_hf_mode(rgbp_param_set);
	KUNIT_EXPECT_EQ(test, hf_mode, HF_DMA);

	rgbp_param_set->dma_output_hf.cmd = 0;
	rgbp_param_set->otf_output_hf.cmd = 1;
	hf_mode = rgbp_hw_g_hf_mode(rgbp_param_set);
	KUNIT_EXPECT_EQ(test, hf_mode, HF_OTF);

	rgbp_param_set->dma_output_hf.cmd = 0;
	rgbp_param_set->otf_output_hf.cmd = 0;
	hf_mode = rgbp_hw_g_hf_mode(rgbp_param_set);
	KUNIT_EXPECT_EQ(test, hf_mode, HF_OFF);
}
...
static struct kunit_case pablo_hw_api_rgbp_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_api_rgbp_hw_error_handle_kunit_test),
	KUNIT_CASE(pablo_hw_api_rgbp_hw_g_hf_mode_kunit_test),
	{},
};
