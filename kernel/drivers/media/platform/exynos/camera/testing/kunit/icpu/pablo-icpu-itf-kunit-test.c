...
#define SET_ICPU_CPU_WFI_STATUS(_pd)                                                               \
	{                                                                                          \
		*(u32 *)(_pd.mcuctl_reg_base + 0xC4) = 1;                                          \
	}
...
static void pablo_icpu_itf_shutdown_firmware_kunit_test(struct kunit *test)
{
	...
	ret = pablo_icpu_itf_shutdown_firmware();
...
static void pablo_icpu_itf_send_message_async_pending_kunit_test(struct kunit *test)
{
	int i;
	int ret;
	struct test_vec {
		...
		{
			1,
			ICPU_MSG_PRIO_NORMAL,
		}, /* wait idle in mbox send */
		{
			2,
			ICPU_MSG_PRIO_NORMAL,
		},
	...
	task_async =
		kthread_run(__send_async_msg_func, (void *)&send_buf[0], "thread_send_async_msg");
	...
	pending_msg_cnt = get_pending_msg_cnt_wrap();
	KUNIT_EXPECT_EQ(test, pending_msg_cnt, NUM_TEST_BUF - 1);
...
static struct kunit_case pablo_icpu_itf_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_icpu_itf_send_message_async_pending_kunit_test),
...
static int pablo_icpu_itf_kunit_test_init(struct kunit *test)
{
	...
	ctx.rx_chan = kunit_kzalloc(test, sizeof(struct pablo_icpu_mbox_chan), GFP_KERNEL);
	...
	test_pdata.num_sw_reset_step = 0;
