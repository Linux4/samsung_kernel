...
static void pablo_subdev_yuvp_tag_dma_negative_kunit_test(struct kunit *test)
{
	...
	/* request is not set */
	frame->shot_ext->node_group.capture[0].request = 0;
	ret = ops->tag(subdev, idi, frame, node);
	KUNIT_EXPECT_EQ(test, ret, 0);
	...
	/* otcrop is NULL */
	__clear_test_vector_cropRegion(frame->shot_ext->node_group.capture[1].output.cropRegion);
	ret = ops->tag(subdev, idi, frame, node);
	KUNIT_EXPECT_EQ(test, ret, -EINVAL);
	__set_test_vector_cropRegion(frame->shot_ext->node_group.capture[1].output.cropRegion);

	/* dma in */
	frame->shot_ext->node_group.capture[2].vid = IS_LVN_YUVP_CLAHE;
...
static void pablo_subdev_yuvp_tag_sframe_kunit_test(struct kunit *test)
{
	...
	for (k = 0; k < sizeof(_test_vector)/sizeof(enum is_video_dev_num); k++) {
		...
		if (cap_node->vid >= IS_LVN_YUVP_SEG && cap_node->vid <= IS_LVN_YUVP_SVHIST) {
	...
	frame->cap_node.sframe[0].id = IS_LVN_MCSC_P1;
	ret = ops->tag(subdev, idi, frame, node);
	KUNIT_EXPECT_EQ(test, ret, 0);

	frame->cap_node.sframe[0].id = IS_LVN_YUVP_YUV;
