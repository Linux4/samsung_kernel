...
static struct pablo_hw_api_lme_test_ctx {
	void *test_addr;
	u32 width, height;
	u32 set_id;
...
static void pablo_hw_api_lme_hw_s_wdma_addr_kunit_test(struct kunit *test)
{
	...
	struct lme_param_set param_set;
	u32 id = LME_WDMA_MV_OUT;
	u64 expected_val, set_val;
	u32 lme_mode = LME_MODE_TNR;
	enum lme_sps_out_mode sps_mode = LME_OUTPUT_MODE_8_4;
	...
	param_set.dma.output_height = ctx->height;
	param_set.dma.cur_input_width = ctx->width;
	param_set.dma.cur_input_height = ctx->width;
	param_set.output_dva[0] = 0x87654321;

	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, ctx);
	ret = lme_hw_s_wdma_addr(ctx->test_addr, &param_set, id, ctx->set_id, lme_mode, sps_mode);
	KUNIT_EXPECT_EQ(test, ret, SET_SUCCESS);

	expected_val = DVA_36BIT_LOW(0x87654321);
	set_val = LME_GET_R_COREX(ctx->test_addr, ctx->set_id,
	...
	expected_val = DVA_36BIT_HIGH(0x87654321 + total_width * (line_count - 1));
	set_val = LME_GET_R_COREX(ctx->test_addr, ctx->set_id,
...
static void pablo_hw_api_lme_hw_s_mvct_size_kunit_test(struct kunit *test)
{
	...
	expected_val = 1;
	set_val = LME_GET_F_COREX(ctx->test_addr, ctx->set_id, LME_R_MVCT_8BIT_LME_PREFETCH,
				  LME_F_MVCT_8BIT_LME_PREFETCH_EN);
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	expected_val = DIV_ROUND_UP(width * 15 / 100, cell_width);
	set_val = LME_GET_F_COREX(ctx->test_addr, ctx->set_id, LME_R_MVCT_8BIT_LME_PREFETCH,
...
static void pablo_hw_api_lme_hw_s_first_frame_forcely_kunit_test(struct kunit *test)
{
	struct pablo_hw_api_lme_test_ctx *ctx = test->priv;
	u32 expected_val, set_val;
	...
	lme_hw_s_first_frame_forcely(ctx->test_addr, ctx->set_id);

	expected_val = 1;
	set_val = LME_GET_F_COREX(ctx->test_addr, ctx->set_id, LME_R_MVCT_8BIT_LME_CONFIG,
				  LME_F_MVCT_8BIT_LME_FIRST_FRAME);
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
...
static struct kunit_case pablo_hw_api_lme_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_api_lme_hw_s_first_frame_forcely_kunit_test),
