...
static void pablo_hw_csi_hw_s_dma_input_mux(struct kunit *test)
{
	...
	u32 csi_ch = 0;
	u32 otf_out_id, val, wdma_idx;
	struct is_wdma_info wdma_info;
	u32 wdma_vc, lc, lc_start, lc_end, mux_val_base, wdma_ch = 0;
	int link_vc;
	...
	for (otf_out_id = CSI_OTF_OUT_SINGLE; otf_out_id < CSI_OTF_OUT_CH_NUM; otf_out_id++) {
		...
		wdma_idx = otf_out_id;
		func->csi_hw_s_dma_input_mux(reg_mux, wdma_idx, wdma_ch, bns_en, bns_dma_mux_val,
					     csi_ch, otf_ch, otf_out_id, link_vc_list[otf_out_id],
					     &wdma_info);
		val = *(u32 *)(reg_mux);
