...
#include "hardware/sfr/is-sfr-lme-v4_20.h"
...
static void pablo_hw_api_lme_hw_s_wdma_addr_kunit_test(struct kunit *test)
{
	...
	u32 total_width, line_count, stride;
	int ret;
	...
	ret = lme_hw_s_wdma_addr(test_ctx.pmio, &param_set, id, test_ctx.set_id, lme_mode,
				 sps_mode);
	KUNIT_EXPECT_EQ(test, ret, SET_SUCCESS);
	...
	stride = ALIGN(total_width, 16);
	line_count = DIV_ROUND_UP(param_set.dma.cur_input_height, 4);

	expected_val = DVA_36BIT_HIGH(0x87654321 + stride * (line_count - 1));
...
static void pablo_hw_api_lme_hw_g_int_state_kunit_test(struct kunit *test)
{
	...
	ret = lme_hw_g_int_state(test_ctx.pmio, clear, num_buffers, &irq_state, test_ctx.set_id);
}
...
static void pablo_hw_api_lme_hw_s_cache_size_kunit_test(struct kunit *test)
{
	...
	lme_hw_s_cache_size(test_ctx.pmio, test_ctx.set_id, prev_width, prev_height, cur_width,
			    cur_height);

	/* check output */
	expected_val = prev_width;
	set_val = LME_GET_F(test_ctx.pmio, LME_R_CACHE_8BIT_IMAGE0_CONFIG, LME_F_Y_LME_PRVIMGWIDTH);
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	expected_val = prev_height;
	set_val =
		LME_GET_F(test_ctx.pmio, LME_R_CACHE_8BIT_IMAGE0_CONFIG, LME_F_Y_LME_PRVIMGHEIGHT);
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	expected_val = cur_width;
	set_val = LME_GET_F(test_ctx.pmio, LME_R_CACHE_8BIT_IMAGE1_CONFIG, LME_F_Y_LME_CURIMGWIDTH);
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	expected_val = cur_height;
	set_val =
		LME_GET_F(test_ctx.pmio, LME_R_CACHE_8BIT_IMAGE1_CONFIG, LME_F_Y_LME_CURIMGHEIGHT);
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
}
...
static void pablo_hw_api_lme_hw_s_mvct_size_kunit_test(struct kunit *test)
{
	...
	expected_val = width;
	set_val = LME_GET_F(test_ctx.pmio, LME_R_MVCT_8BIT_IMAGE_DIMENTIONS, LME_F_Y_LME_ROISIZEX);
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	expected_val = height;
	set_val = LME_GET_F(test_ctx.pmio, LME_R_MVCT_8BIT_IMAGE_DIMENTIONS, LME_F_Y_LME_ROISIZEY);
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	expected_val = 1;
	set_val = LME_GET_F(test_ctx.pmio, LME_R_MVCT_8BIT_LME_PREFETCH,
			    LME_F_MVCT_8BIT_LME_PREFETCH_EN);
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	expected_val = DIV_ROUND_UP(width * 8 / 100, cell_width);
	set_val = LME_GET_F(test_ctx.pmio, LME_R_MVCT_8BIT_LME_PREFETCH,
...
static void pablo_hw_api_lme_hw_s_first_frame_forcely_kunit_test(struct kunit *test)
{
	...
	lme_hw_s_first_frame_forcely(test_ctx.pmio, test_ctx.set_id);

	expected_val = 1;
	set_val = LME_GET_F(test_ctx.pmio, LME_R_MVCT_8BIT_LME_CONFIG, LME_F_Y_LME_FIRSTFRAME);
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
