...
#include "hardware/sfr/is-sfr-shrp-v3_1.h"
#else
...
static void pablo_hw_api_shrp_hw_s_size_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < ARRAY_SIZE(shrp_size_test_config); test_idx++) {
		...
		shrp_hw_s_otf_input_size(test_ctx.pmio, set_id, width, height);
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
		shrp_hw_s_otf_output_size(test_ctx.pmio, set_id, width, height);
		shrp_hw_s_yuvsc_size(test_ctx.pmio, set_id, width, height, 100000, 100000);
		shrp_hw_s_hf_size(test_ctx.pmio, set_id, width, height);
#endif
	}
}
...
static void pablo_hw_api_shrp_hw_s_input_path_kunit_test(struct kunit *test)
{
	...
	shrp_hw_s_otf_input_path(test_ctx.pmio, set_id, path);
...
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	shrp_hw_s_hf_otf_input_path(test_ctx.pmio, set_id, path);
#endif
}
...
static void pablo_hw_api_shrp_hw_s_output_path_kunit_test(struct kunit *test)
{
	...
	shrp_hw_s_otf_output_path(test_ctx.pmio, set_id, 0);
...
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	shrp_hw_s_hf_otf_output_path(test_ctx.pmio, set_id, path);
#endif
}

static void pablo_hw_api_shrp_hw_s_mux_kunit_test(struct kunit *test)
{
	int type = 0;
	u32 set_val, expected_val;
	u32 set_id = COREX_DIRECT;

	shrp_hw_s_mux_select(test_ctx.pmio, set_id, type);
...
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	shrp_hw_s_demux_select(test_ctx.pmio, set_id, SHRP_DEMUX_HF_RECOMP_IN);
#endif
}
...
{
	shrp_hw_dump(test_ctx.pmio, HW_DUMP_CR);
	shrp_hw_dump(test_ctx.pmio, HW_DUMP_DBG_STATE);
	shrp_hw_dump(test_ctx.pmio, HW_DUMP_MODE_NUM);
}
...
static struct kunit_case pablo_hw_api_shrp_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_api_shrp_hw_s_mux_kunit_test),
