...
#include "hardware/sfr/is-sfr-mcfp-v12_1.h"
#else
...
static void pablo_hw_api_mcfp_dump_kunit_test(struct kunit *test)
{
	...
	mcfp_hw_dump(tctx->pmio, HW_DUMP_CR);
	mcfp_hw_dump(tctx->pmio, HW_DUMP_DBG_STATE);
	mcfp_hw_dump(tctx->pmio, HW_DUMP_MODE_NUM);
}
...
static void pablo_hw_api_mcfp_hw_s_strgen_kunit_test(struct kunit *test)
{
	u32 set_val, expected_val;
	struct pablo_hw_api_mcfp_kunit_test_ctx *tctx = test->priv;

	mcfp_hw_s_strgen(tctx->pmio, 0);

	set_val = MCFP_GET_F(tctx->pmio, MCFP_R_YUV_CINFIFO_CONFIG, MCFP_F_YUV_CINFIFO_STRGEN_MODE_EN);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = MCFP_GET_F(tctx->pmio, MCFP_R_YUV_CINFIFO_CONFIG, MCFP_F_YUV_CINFIFO_STRGEN_MODE_DATA_TYPE);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = MCFP_GET_F(tctx->pmio, MCFP_R_YUV_CINFIFO_CONFIG, MCFP_F_YUV_CINFIFO_STRGEN_MODE_DATA);
	expected_val = 255;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = MCFP_GET_F(tctx->pmio, MCFP_R_IP_USE_CINFIFO_NEW_FRAME_IN, MCFP_F_IP_USE_CINFIFO_NEW_FRAME_IN);
	expected_val = 0;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
}

#define DLFE_PATH_TC_CR_NUM	13
static u32 result_vector[2][DLFE_PATH_TC_CR_NUM][3] = {
	/* TC#1. Disable DLFE path */
	{
		/* Cur YUV COUTFIFO setting */
		{MCFP_R_YUV_COUTFIFODLFECURR_ENABLE, MCFP_F_YUV_COUTFIFODLFECURR_ENABLE, 0},
		{MCFP_R_YUV_COUTFIFODLFECURR_CONFIG, MCFP_F_YUV_COUTFIFODLFECURR_VVALID_RISE_AT_FIRST_DATA_EN, 0},
		{MCFP_R_YUV_COUTFIFODLFECURR_CONFIG, MCFP_F_YUV_COUTFIFODLFECURR_DEBUG_EN, 1},
		{MCFP_R_IP_USE_OTF_PATH_01, MCFP_F_IP_USE_OTF_OUT_FOR_PATH_1, 0},
		/* Prev YUV COUTFIFO setting */
		{MCFP_R_YUV_COUTFIFODLFEPREV_ENABLE, MCFP_F_YUV_COUTFIFODLFEPREV_ENABLE, 0},
		{MCFP_R_YUV_COUTFIFODLFEPREV_CONFIG, MCFP_F_YUV_COUTFIFODLFEPREV_VVALID_RISE_AT_FIRST_DATA_EN, 0},
		{MCFP_R_YUV_COUTFIFODLFEPREV_CONFIG, MCFP_F_YUV_COUTFIFODLFEPREV_DEBUG_EN, 1},
		{MCFP_R_IP_USE_OTF_PATH_23, MCFP_F_IP_USE_OTF_OUT_FOR_PATH_2, 0},
		/* WGT CINFIFO setting */
		{MCFP_R_STAT_CINFIFODLFEWGT_ENABLE, MCFP_F_STAT_CINFIFODLFEWGT_ENABLE, 0},
		{MCFP_R_STAT_CINFIFODLFEWGT_CONFIG, MCFP_F_STAT_CINFIFODLFEWGT_STALL_BEFORE_FRAME_START_EN, 0},
		{MCFP_R_STAT_CINFIFODLFEWGT_CONFIG, MCFP_F_STAT_CINFIFODLFEWGT_AUTO_RECOVERY_EN, 0},
		{MCFP_R_STAT_CINFIFODLFEWGT_CONFIG, MCFP_F_STAT_CINFIFODLFEWGT_DEBUG_EN, 1},
		{MCFP_R_IP_USE_OTF_PATH_23, MCFP_F_IP_USE_OTF_IN_FOR_PATH_3, 0}
	},
	/* TC#2. Enable DLFE path */
	{
		/* Cur YUV COUTFIFO setting */
		{MCFP_R_YUV_COUTFIFODLFECURR_ENABLE, MCFP_F_YUV_COUTFIFODLFECURR_ENABLE, 1},
		{MCFP_R_YUV_COUTFIFODLFECURR_CONFIG, MCFP_F_YUV_COUTFIFODLFECURR_VVALID_RISE_AT_FIRST_DATA_EN, 1},
		{MCFP_R_YUV_COUTFIFODLFECURR_CONFIG, MCFP_F_YUV_COUTFIFODLFECURR_DEBUG_EN, 1},
		{MCFP_R_IP_USE_OTF_PATH_01, MCFP_F_IP_USE_OTF_OUT_FOR_PATH_1, 1},
		/* Prev YUV COUTFIFO setting */
		{MCFP_R_YUV_COUTFIFODLFEPREV_ENABLE, MCFP_F_YUV_COUTFIFODLFEPREV_ENABLE, 1},
		{MCFP_R_YUV_COUTFIFODLFEPREV_CONFIG, MCFP_F_YUV_COUTFIFODLFEPREV_VVALID_RISE_AT_FIRST_DATA_EN, 1},
		{MCFP_R_YUV_COUTFIFODLFEPREV_CONFIG, MCFP_F_YUV_COUTFIFODLFEPREV_DEBUG_EN, 1},
		{MCFP_R_IP_USE_OTF_PATH_23, MCFP_F_IP_USE_OTF_OUT_FOR_PATH_2, 1},
		/* WGT CINFIFO setting */
		{MCFP_R_STAT_CINFIFODLFEWGT_ENABLE, MCFP_F_STAT_CINFIFODLFEWGT_ENABLE, 1},
		{MCFP_R_STAT_CINFIFODLFEWGT_CONFIG, MCFP_F_STAT_CINFIFODLFEWGT_STALL_BEFORE_FRAME_START_EN, 1},
		{MCFP_R_STAT_CINFIFODLFEWGT_CONFIG, MCFP_F_STAT_CINFIFODLFEWGT_AUTO_RECOVERY_EN, 0},
		{MCFP_R_STAT_CINFIFODLFEWGT_CONFIG, MCFP_F_STAT_CINFIFODLFEWGT_DEBUG_EN, 1},
		{MCFP_R_IP_USE_OTF_PATH_23, MCFP_F_IP_USE_OTF_IN_FOR_PATH_3, 1}
	}
};
static void pablo_hw_api_mcfp_hw_s_dlfe_path_kunit_test(struct kunit *test)
{
	u32 set_val, cr_id;
	int en;
	struct pablo_hw_api_mcfp_kunit_test_ctx *tctx = test->priv;
	...
	for (en = 1; en >= 0; en--) {
		mcfp_hw_s_dlfe_path(tctx->pmio, (u32)en);

		for (cr_id = 0; cr_id < DLFE_PATH_TC_CR_NUM; cr_id++) {
			set_val = MCFP_GET_F(tctx->pmio, result_vector[en][cr_id][0], result_vector[en][cr_id][1]);
			KUNIT_EXPECT_EQ(test, set_val, result_vector[en][cr_id][2]);
		}
	}
}
...
static int __pablo_hw_api_mcfp_pmio_init(struct kunit *test)
{
	...
	if (IS_ERR(test_ctx.pmio)) {
		err("failed to init mcfp PMIO: %ld", PTR_ERR(test_ctx.pmio));
		return -ENOMEM;
	}
...
static struct kunit_case pablo_hw_api_mcfp_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_api_mcfp_hw_s_strgen_kunit_test),
	KUNIT_CASE(pablo_hw_api_mcfp_hw_s_dlfe_path_kunit_test),
	{},
};
