...
static void pablo_subdev_mcfp_tag_dma_negative_kunit_test(struct kunit *test)
{
	...
	/* dma output Null& flag_extra out */
	frame->shot_ext->node_group.capture[2].vid = IS_LVN_MCFP_W;
	frame->shot_ext->node_group.capture[2].request = 1;
	frame->shot_ext->node_group.capture[2].flags = 0x40;
	__set_test_vector_cropRegion(frame->shot_ext->node_group.capture[2].input.cropRegion);
	__clear_test_vector_cropRegion(frame->shot_ext->node_group.capture[2].output.cropRegion);

	/* dma negative out */
	frame->shot_ext->node_group.capture[3].vid = -1;
