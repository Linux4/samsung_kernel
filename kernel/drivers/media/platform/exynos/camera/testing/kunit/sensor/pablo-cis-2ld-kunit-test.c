...
static int is_sensor_addr8_read8_mock(void *client, u8 addr, u8 *val)
{
	return 0;
}

static int is_sensor_read8_mock(void *client, u16 addr, u8 *val)
{
	return 0;
}

static int is_sensor_read16_mock(void *client, u16 addr, u16 *val)
{
	return 0;
}

static int is_sensor_read8_size_mock(void *client, void *buf, u16 addr, size_t size)
{
	return 0;
}

static int is_sensor_addr8_write8_mock(void *client, u8 addr, u8 val)
{
	return 0;
}

static int is_sensor_write8_mock(void *client, u16 addr, u8 val)
{
	return 0;
}

static int is_sensor_write8_array_mock(void *client, u16 addr, u8 *val, u32 num)
{
	return 0;
}

static int is_sensor_write16_mock(void *client, u16 addr, u16 val)
{
	return 0;
}

static int is_sensor_write16_array_mock(void *client, u16 addr, u16 *val, u32 num)
{
	return 0;
}

static int is_sensor_write16_burst_mock(void *client, u16 addr, u16 *val, u32 num)
{
	return 0;
}

static int is_sensor_write8_sequential_mock(void *client, u16 addr, u8 *val, u16 num)
{
	return 0;
}

static int is_sensor_data_read16_mock(void *client, u16 *val)
{
	return 0;
}

static int is_sensor_data_write16_mock(void *client, u8 val_high, u8 val_low)
{
	return 0;
}

static int is_sensor_addr_data_write16_mock(void *client, u8 addr, u8 val_high, u8 val_low)
{
	return 0;
}

static struct is_ixc_ops i2c_ops_mock = {
	.addr8_read8 = is_sensor_addr8_read8_mock,
	.read8 = is_sensor_read8_mock,
	.read16 = is_sensor_read16_mock,
	.read8_size = is_sensor_read8_size_mock,
	.addr8_write8 = is_sensor_addr8_write8_mock,
	.write8 = is_sensor_write8_mock,
	.write8_array = is_sensor_write8_array_mock,
	.write16 = is_sensor_write16_mock,
	.write16_array = is_sensor_write16_array_mock,
	.write16_burst = is_sensor_write16_burst_mock,
	.write8_sequential = is_sensor_write8_sequential_mock,
	.data_read16 = is_sensor_data_read16_mock,
	.data_write16 = is_sensor_data_write16_mock,
	.addr_data_write16 = is_sensor_addr_data_write16_mock,
};
...
static void pablo_sensor_2ld_cis_set_frame_rate_kunit_test(struct kunit *test)
{
	...
	for (i = 0; i < MIN_FPS_LIST_MAX; i++) {
		...
		frame_duration = MAX(frame_duration, cis->cis_data->base_min_frame_us_time);
...
static void pablo_sensor_2ld_cis_wait_seamless_update_delay(struct kunit *test)
{
	int ret;

	ret = cis_ops->cis_wait_seamless_update_delay(subdev);
	KUNIT_EXPECT_EQ(test, ret, 0);
}
...
static int pablo_cis_2ld_kunit_test_init(struct kunit *test)
{
	...
	for (i = RESOURCE_TYPE_SENSOR0; i < IS_SENSOR_COUNT; i++) {
		sensor = &core->sensor[i];
		if (sensor->sensor_name && strcmp(sensor->sensor_name, "2LD") == 0) {
			break;
		}
	}

	KUNIT_ASSERT_NE(test, i, IS_SENSOR_COUNT);
	...
	sensor_peri->cis.ixc_lock = &core->ixc_lock[i2c_ch];

	cis_ops = cis->cis_ops;
	i2c_ops = cis->ixc_ops;
	cis->ixc_ops = &i2c_ops_mock;

	return 0;
}

static void pablo_cis_2ld_kunit_test_exit(struct kunit *test)
{
	cis->ixc_ops = i2c_ops;
}
...
static struct kunit_case pablo_cis_2ld_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_sensor_2ld_cis_wait_seamless_update_delay),
	{},
};
...
struct kunit_suite pablo_cis_2ld_kunit_test_suite = {
	...
	.exit = pablo_cis_2ld_kunit_test_exit,
