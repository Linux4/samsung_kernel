...
#include "hardware/sfr/is-sfr-cstat-v2_20.h"

/* Define the test cases. */
#define CSTAT_SET_F_DIRECT(base, R, F, val)                                                        \
	is_hw_set_field(base, &cstat_regs[R], &cstat_fields[F], val)

#define CSTAT_GET_R_COREX(base, R) is_hw_get_reg((base + CSTAT_COREX_SDW_OFFSET), &cstat_regs[R])

#define CSTAT_GET_F_COREX(base, R, F)                                                              \
	is_hw_get_field((base + CSTAT_COREX_SDW_OFFSET), &cstat_regs[R], &cstat_fields[F])

#define CSTAT_GET_F_DIRECT(base, R, F) is_hw_get_field(base, &cstat_regs[R], &cstat_fields[F])

#define CSTAT_TEST_SET 3

#define CSTAT_BYR_RDMA_STRIDE_ALIGN 32
...
static const u32 bns_weights[] = {
	12, 183, 61, 0, 0, 0, 0, 0, 0, 0, 0 /* CSTAT_BNS_X2P0 */
};
...
static void pablo_hw_api_cstat_hw_corex_resume_kunit_test(struct kunit *test)
{
	...
	val = CSTAT_GET_F_DIRECT(ctx->test_addr, CSTAT_R_COREX_START_0, CSTAT_F_COREX_START_0);
...
static void pablo_hw_api_cstat_hw_is_corex_offset_kunit_test(struct kunit *test)
{
	...
	ret = cstat_hw_is_corex_offset(
		cstat_regs[CSTAT_R_RGB_LUMASHADING_LUT_START_ADD].sfr_offset);
	KUNIT_EXPECT_EQ(test, ret, false);

	ret = cstat_hw_is_corex_offset(
		cstat_regs[CSTAT_R_RGB_LUMASHADING_LUT_ACCESS_SETA].sfr_offset);
	KUNIT_EXPECT_EQ(test, ret, false);

	ret = cstat_hw_is_corex_offset(
		cstat_regs[CSTAT_R_RGB_LUMASHADING_LUT_ACCESS_SETB].sfr_offset);
	KUNIT_EXPECT_EQ(test, ret, false);
...
static void pablo_hw_api_cstat_hw_s_int_enable_kunit_test(struct kunit *test)
{
	...
	/* check output */
	set_val = CSTAT_GET_F_DIRECT(ctx->test_addr, CSTAT_R_CONTINT_SIPUIPP1P0P0_LEVEL_PULSE_N_SEL,
	...
	set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_IP_USE_END_INTERRUPT_ENABLE,
				    CSTAT_F_IP_USE_END_INTERRUPT_ENABLE);
	expected_val = 0;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_IP_END_INTERRUPT_ENABLE,
				    CSTAT_F_IP_END_INTERRUPT_ENABLE);
	expected_val = 0;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_IP_CORRUPTED_INTERRUPT_ENABLE,
				    CSTAT_F_IP_CORRUPTED_INTERRUPT_ENABLE);
	expected_val = INT_CRPT_EN_MASK;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = CSTAT_GET_F_DIRECT(ctx->test_addr, CSTAT_R_CONTINT_SIPUIPP1P0P0_INT1_ENABLE,
				     CSTAT_F_CONTINT_SIPUIPP1P0P0_INT1_ENABLE);
	expected_val = INT1_EN_MASK;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = CSTAT_GET_F_DIRECT(ctx->test_addr, CSTAT_R_CONTINT_SIPUIPP1P0P0_INT2_ENABLE,
...
static void pablo_hw_api_cstat_hw_s_cdaf_dma_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_STAT_WDMACDAF_EN,
					    CSTAT_F_STAT_WDMACDAF_EN);
		expected_val = param.cdaf_mw.cmd;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_STAT_WDMACDAF_DATA_FORMAT,
					    CSTAT_F_STAT_WDMACDAF_DATA_FORMAT_BAYER);
		expected_val = DMA_FMT_U8BIT_PACK;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_STAT_WDMACDAF_MONO_MODE,
					    CSTAT_F_STAT_WDMACDAF_MONO_MODE);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_STAT_WDMACDAF_WIDTH,
					    CSTAT_F_STAT_WDMACDAF_WIDTH);
		expected_val = param.cdaf_mw.width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_STAT_WDMACDAF_HEIGHT,
					    CSTAT_F_STAT_WDMACDAF_HEIGHT);
		expected_val = param.cdaf_mw.height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_STAT_WDMACDAF_IMG_STRIDE_1P,
		...
		/* MSB << 4 | LSB */
		set_val = (set_val << 4) |
			  CSTAT_GET_F_COREX(ctx->test_addr,
...
static void pablo_hw_api_cstat_hw_s_drc_dma_kunit_test(struct kunit *test)
{
	enum cstat_dma_id dma_id;
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		/* set input */
		dma_id = CSTAT_W_DRCGRID;
		conf.cdaf_mw_bypass = true;
		conf.thstatpre_bypass = true;
		conf.thstatae_bypass = true;
		conf.thstatawb_bypass = true;
		conf.rgbyhist_bypass = true;
		conf.lmeds_bypass = true;
		conf.drcclct_bypass = false;
		conf.drc_grid_w = test_config[test_idx].drc_clct.w;
		conf.drc_grid_h = test_config[test_idx].drc_clct.h;
		dma_out = &param.dma_output_drc;
		dma_out->cmd = DMA_OUTPUT_COMMAND_ENABLE;
		dma_out->format = DMA_INOUT_FORMAT_BAYER_PACKED;
		...
		param.output_dva_drc[0] = 0xdeadbeef;
		...
		set_val = dma[dma_id].available_bayer_format_map;
		expected_val = (0 | BIT_MASK(DMA_FMT_U8BIT_PACK)) & IS_BAYER_FORMAT_MASK;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);
		...
		/* check output */
		set_val = param.dma_output_drc.cmd;
		expected_val = !conf.drcclct_bypass;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = param.dma_output_drc.width;
		expected_val = conf.drc_grid_w * 4;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = param.dma_output_drc.height;
		expected_val = conf.drc_grid_h;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_STAT_WDMADRCGRID_EN,
					    CSTAT_F_STAT_WDMADRCGRID_EN);
		expected_val = param.dma_output_drc.cmd;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_STAT_WDMADRCGRID_DATA_FORMAT,
					    CSTAT_F_STAT_WDMADRCGRID_DATA_FORMAT_BAYER);
		expected_val = DMA_FMT_U8BIT_PACK;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_STAT_WDMADRCGRID_MONO_MODE,
					    CSTAT_F_STAT_WDMADRCGRID_MONO_MODE);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_STAT_WDMADRCGRID_WIDTH,
					    CSTAT_F_STAT_WDMADRCGRID_WIDTH);
		expected_val = param.dma_output_drc.width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_STAT_WDMADRCGRID_HEIGHT,
					    CSTAT_F_STAT_WDMADRCGRID_HEIGHT);
		expected_val = param.dma_output_drc.height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_STAT_WDMADRCGRID_IMG_STRIDE_1P,
					    CSTAT_F_STAT_WDMADRCGRID_IMG_STRIDE_1P);
		expected_val = DIV_ROUND_UP(
			param.dma_output_drc.width * param.dma_output_drc.bitwidth, BITS_PER_BYTE);
		KUNIT_EXPECT_EQ(test, set_val, expected_val);
		...
		expected_val = param.output_dva_drc[0];
		KUNIT_EXPECT_EQ(test, set_val, expected_val);
	}
}
...
static void pablo_hw_api_cstat_hw_s_cds_dma_kunit_test(struct kunit *test)
{
	...
	expected_map_val = (0 | BIT_MASK(DMA_FMT_RGB_RGBA8888) | BIT_MASK(DMA_FMT_RGB_ABGR8888) |
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_YUV_WDMACDS0_EN,
					    CSTAT_F_YUV_WDMACDS0_EN);
		expected_val = dma_out->cmd;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_YUV_WDMACDS0_DATA_FORMAT,
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_YUV_WDMACDS0_MONO_MODE,
					    CSTAT_F_YUV_WDMACDS0_MONO_MODE);
		expected_val = 1;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_YUV_WDMACDS0_WIDTH,
					    CSTAT_F_YUV_WDMACDS0_WIDTH);
		expected_val = dma_out->width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_YUV_WDMACDS0_HEIGHT,
					    CSTAT_F_YUV_WDMACDS0_HEIGHT);
		expected_val = dma_out->height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_YUV_WDMACDS0_IMG_STRIDE_1P,
					    CSTAT_F_YUV_WDMACDS0_IMG_STRIDE_1P);
		expected_val =
			ALIGN(DIV_ROUND_UP(dma_out->width * dma_out->bitwidth, BITS_PER_BYTE), 16);
		KUNIT_EXPECT_EQ(test, set_val, expected_val);
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_RGB_RGBTOYUVCDS0_BYPASS,
					    CSTAT_F_RGB_RGBTOYUVCDS0_BYPASS);
		expected_val = 1;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_YUV_YUV444TO422CDS0_BYPASS,
					    CSTAT_F_YUV_YUV444TO422CDS0_BYPASS);
		expected_val = 1;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_YUV_YUV422TO420CDS0_BYPASS,
...
static void pablo_hw_api_cstat_hw_s_fdpig_wdma_kunit_test(struct kunit *test)
{
	...
	set_map_val = dma[dma_id].available_yuv_format_map;
	expected_map_val = (0 | BIT_MASK(DMA_FMT_YUV444_1P)) & IS_YUV_FORMAT_MASK;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_map_val = dma[dma_id].available_rgb_format_map;
	expected_map_val = (0 | BIT_MASK(DMA_FMT_RGB_RGBA8888) | BIT_MASK(DMA_FMT_RGB_ARGB8888) |
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_RGB_WDMAFDPIG_EN,
					    CSTAT_F_RGB_WDMAFDPIG_EN);
		expected_val = dma_out->cmd;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_RGB_WDMAFDPIG_DATA_FORMAT,
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_RGB_WDMAFDPIG_MONO_MODE,
					    CSTAT_F_RGB_WDMAFDPIG_MONO_MODE);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_RGB_WDMAFDPIG_WIDTH,
					    CSTAT_F_RGB_WDMAFDPIG_WIDTH);
		expected_val = dma_out->width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_RGB_WDMAFDPIG_HEIGHT,
					    CSTAT_F_RGB_WDMAFDPIG_HEIGHT);
		expected_val = dma_out->height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_RGB_WDMAFDPIG_IMG_STRIDE_1P,
...
static void pablo_hw_api_cstat_hw_s_byr_rdma_cfg_kunit_test(struct kunit *test)
{
	...
	expected_val =
		(0 | BIT_MASK(DMA_FMT_U10BIT_PACK) | BIT_MASK(DMA_FMT_U10BIT_UNPACK_LSB_ZERO) |
		 BIT_MASK(DMA_FMT_U10BIT_UNPACK_MSB_ZERO) | BIT_MASK(DMA_FMT_ANDROID10) |
		 BIT_MASK(DMA_FMT_U12BIT_PACK) | BIT_MASK(DMA_FMT_U12BIT_UNPACK_LSB_ZERO) |
		 BIT_MASK(DMA_FMT_U12BIT_UNPACK_MSB_ZERO) | BIT_MASK(DMA_FMT_ANDROID12) |
		 BIT_MASK(DMA_FMT_U14BIT_PACK) | BIT_MASK(DMA_FMT_U14BIT_UNPACK_LSB_ZERO) |
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_BYR_RDMABYRIN_EN,
					    CSTAT_F_BYR_RDMABYRIN_EN);
		expected_val = dma_input->cmd;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_BYR_RDMABYRIN_DATA_FORMAT,
					    CSTAT_F_BYR_RDMABYRIN_DATA_FORMAT_BAYER);
		expected_val = DMA_FMT_U10BIT_PACK;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_BYR_RDMABYRIN_COMP_CONTROL,
					    CSTAT_F_BYR_RDMABYRIN_COMP_SBWC_EN);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_BYR_RDMABYRIN_WIDTH,
					    CSTAT_F_BYR_RDMABYRIN_WIDTH);
		expected_val = dma_input->width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_BYR_RDMABYRIN_HEIGHT,
					    CSTAT_F_BYR_RDMABYRIN_HEIGHT);
		expected_val = dma_input->height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_BYR_RDMABYRIN_IMG_STRIDE_1P,
					    CSTAT_F_BYR_RDMABYRIN_IMG_STRIDE_1P);
		expected_val =
			ALIGN(DIV_ROUND_UP(dma_input->width * dma_input->bitwidth, BITS_PER_BYTE),
			      CSTAT_BYR_RDMA_STRIDE_ALIGN);
		KUNIT_EXPECT_EQ(test, set_val, expected_val);
...
static void pablo_hw_api_cstat_hw_s_ds_cfg_kunit_test(struct kunit *test)
{
	...
	u32 test_idx, dma_id;
	u32 set_val;
	u32 crop_reg = CSTAT_REG_CNT, crop_field = CSTAT_REG_FIELD_CNT;
	...
	for (dma_id = 0; dma_id < CSTAT_DMA_NUM; dma_id++) {
		for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
			...
			case CSTAT_W_FDPIG:
				...
				dma_out->width = test_config[test_idx].fdpig.w;
				dma_out->height = test_config[test_idx].fdpig.h;
				crop_reg = CSTAT_R_RGB_DS_FDPIG_CROP_EN;
				crop_field = CSTAT_F_RGB_DS_FDPIG_CROP_EN;
				break;
			case CSTAT_W_CDS0:
				dma_out = &param.dma_output_cds;
				dma_out->dma_crop_width = test_config[test_idx].bns.w;
				...
				dma_out->width = test_config[test_idx].sat.w;
				dma_out->height = test_config[test_idx].sat.h;
				crop_reg = CSTAT_R_RGB_DS_CONTENTS_DS0_CROP_EN;
				crop_field = CSTAT_F_RGB_DS_CONTENTS_DS0_CROP_EN;
				break;
			default:
				continue;
			}
			...
			if (crop_reg != CSTAT_REG_CNT) {
				set_val = CSTAT_GET_F_COREX(ctx->test_addr, crop_reg, crop_field);
	...
	size_cfg.bns.w = test_config[test_idx].bns.w * size_cfg.rms_crop_ratio / 10;
	size_cfg.bns.h = test_config[test_idx].bns.h * size_cfg.rms_crop_ratio / 10;

	for (dma_id = CSTAT_W_FDPIG; dma_id <= CSTAT_W_CAV; dma_id++) {
		switch (dma_id) {
		case CSTAT_W_FDPIG:
			dma_out = &param.dma_output_fdpig;
			dma_out->width = test_config[test_idx].fdpig.w;
...
static void pablo_hw_api_cstat_hw_s_simple_lic_cfg_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		/* check output */
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_LIC_INPUT_MODE,
					    CSTAT_F_LIC_BYPASS);
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_LIC_INPUT_SIZE,
					    CSTAT_F_LIC_WIDTH);
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_LIC_INPUT_CONFIG_0,
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_LIC_INPUT_BLANK,
...
static void pablo_hw_api_cstat_hw_s_bns_cfg_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		/* check output */
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_BYR_BNS_CONFIG,
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_BYR_BNS_OUTPUTSIZE,
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_BYR_BNS_LINEGAP,
					    CSTAT_F_BYR_BNS_LINEGAP);
		expected_val = 4;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_BYR_BNS_WEIGHT_X_0,
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_BYR_BNS_WEIGHT_X_4,
					    CSTAT_F_BYR_BNS_WEIGHT_X_0_4);
		expected_val = bns_weights[4];
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_BYR_BNS_BYPASS,
...
static void pablo_hw_api_cstat_hw_s_default_blk_cfg_kunit_test(struct kunit *test)
{
	...
	/* SAT */
	set_val = CSTAT_GET_R_COREX(ctx->test_addr, CSTAT_R_RGB_RGBTOYUVCDS0_COEFF_R1);
	expected_val = 306;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = CSTAT_GET_R_COREX(ctx->test_addr, CSTAT_R_YUV_YUV444TO422CDS0_MCOEFFS_0);
...
static struct kunit_case pablo_hw_api_cstat_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_api_cstat_hw_s_drc_dma_kunit_test),
