...
static void cis_sensor_info_kunit_test(struct kunit *test, struct is_module_enum *module)
{
	...
	for (i = 0; i < cis->sensor_info->mode_count; i++) {
		...
		KUNIT_EXPECT_GE(test, mode_info->max_analog_gain, cis->sensor_info->min_analog_gain);
		KUNIT_EXPECT_GE(test, mode_info->max_digital_gain, cis->sensor_info->min_digital_gain);
