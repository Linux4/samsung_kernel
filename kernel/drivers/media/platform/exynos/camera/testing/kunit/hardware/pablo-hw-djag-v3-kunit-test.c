...
static void pablo_hw_mcsc_update_djag_register_kunit_test(struct kunit *test)
{
	...
	tctx->param.input.width = 8000;
	tctx->param.input.height = 4000;
	tctx->param.output[0].width = 7680;
	tctx->param.output[0].height = 4320;
	ret = is_hw_mcsc_update_djag_register(tctx->hw_ip, &tctx->param, 0);
	KUNIT_EXPECT_EQ(test, ret, 0);

	tctx->param.input.width = 4758;
	tctx->param.input.height = 2676;
	tctx->param.output[0].width = 7680;
	tctx->param.output[0].height = 4320;
	ret = is_hw_mcsc_update_djag_register(tctx->hw_ip, &tctx->param, 0);
	KUNIT_EXPECT_EQ(test, ret, 0);

	tctx->param.input.width = 3862;
	tctx->param.input.height = 2172;
	tctx->param.output[0].width = 7680;
	tctx->param.output[0].height = 4320;
	ret = is_hw_mcsc_update_djag_register(tctx->hw_ip, &tctx->param, 0);
	KUNIT_EXPECT_EQ(test, ret, 0);
