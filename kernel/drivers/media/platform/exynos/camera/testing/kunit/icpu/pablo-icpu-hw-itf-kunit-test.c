...
static void pablo_icpu_hw_release_reset_kunit_test(struct kunit *test)
{
	...
	ret = pablo_icpu_hw_release_reset(NULL, 0);
	KUNIT_EXPECT_EQ(test, ret, -EINVAL);

	ret = pablo_icpu_hw_release_reset(&test_pdata, 0);
	KUNIT_EXPECT_EQ(test, ret, 0);

	ret = pablo_icpu_hw_release_reset(NULL, 1);
	KUNIT_EXPECT_EQ(test, ret, -EINVAL);

	ret = pablo_icpu_hw_release_reset(&test_pdata, 1);
	KUNIT_EXPECT_EQ(test, ret, 0);
}
