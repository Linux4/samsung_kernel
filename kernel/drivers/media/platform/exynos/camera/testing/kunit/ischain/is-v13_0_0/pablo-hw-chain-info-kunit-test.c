...
static void pablo_hw_chain_info_cr_set_kunit_test(struct kunit *test)
{
	...
	/* Test DLFE CR buffer */
	scs = CALL_HW_CHAIN_INFO_OPS(hw, get_cr_set, DEV_HW_DLFE, frame);
	KUNIT_EXPECT_EQ(test, (u64)scs, (u64)addr_contents + sizeof(struct size_cr_set));
...
static int pablo_hw_chain_info_kunit_test_init(struct kunit *test)
{
	...
	/* Consider MCFP + DLFE integrated CR buffer */
	addr_contents = kunit_kzalloc(test, sizeof(struct size_cr_set) * 2, 0);
