...
static void camerapp_hw_gdc_get_intr_status_and_clear_kunit_test(struct kunit *test)
{
	...
	camerapp_hw_gdc_status_read(gdc->pmio);
...
static void camerapp_hw_get_comp_buf_size_kunit_test(struct kunit *test)
{
	...
	pixfmt = V4L2_PIX_FMT_NV12N_SBWCL_64_10B;
	for (plane = 0; plane < 2; plane++) {
		val1 = camerapp_hw_get_comp_buf_size(gdc, &frame,
			width, height, pixfmt, plane, GDC_SBWC_SIZE_ALL);
...
static void camerapp_hw_gdc_check_compressor(struct kunit *test)
{
	...
	u32 val1, val2;
	u32 align_64b = 0;
	u32 rdma_comp_rate = 0;
	u32 wdma_comp_rate = 0;
	...
	if (s_frame->extra && d_frame->extra) {
		...
		if (s_frame->extra == COMP_LOSS)
			rdma_comp_rate = 2;
		if (d_frame->extra == COMP_LOSS)
			wdma_comp_rate = 2;
...
static void camerapp_hw_gdc_update_param_kunit_test(struct kunit *test)
{
	...
	/* PACKED 10BIT Size */
	ctx->s_frame.gdc_fmt = &gdc_formats[14];
	ctx->s_frame.pixelformat = V4L2_PIX_FMT_NV16M_S10B;
	ctx->s_frame.pixel_size = CAMERA_PIXEL_SIZE_PACKED_10BIT;
	ctx->d_frame.gdc_fmt = &gdc_formats[16];
	ctx->d_frame.pixelformat = V4L2_PIX_FMT_NV12M_S10B;
	ctx->d_frame.pixel_size = CAMERA_PIXEL_SIZE_PACKED_10BIT;
	camerapp_hw_gdc_update_param(gdc->pmio, gdc);
	camerapp_hw_gdc_check_dma_size(test, ctx->d_frame.width, ctx->d_frame.height,
		ctx->s_frame.width, ctx->s_frame.height, ctx->s_frame.width, ctx->s_frame.height,
		ALIGN(ctx->s_frame.width, 16), ALIGN(ctx->s_frame.width, 16), 0, 0,
		ALIGN(ctx->crop_param[ctx->cur_index].crop_width, 16),
		ALIGN(ctx->crop_param[ctx->cur_index].crop_width, 16), 0, 0);

	ctx->s_frame.gdc_fmt = &gdc_formats[12];
	ctx->s_frame.pixelformat = V4L2_PIX_FMT_NV16M_P210;
	ctx->s_frame.pixel_size = CAMERA_PIXEL_SIZE_10BIT;
	ctx->d_frame.gdc_fmt = &gdc_formats[17];
	ctx->d_frame.pixelformat = V4L2_PIX_FMT_NV21M_S10B;
	ctx->d_frame.pixel_size = CAMERA_PIXEL_SIZE_PACKED_10BIT;
	camerapp_hw_gdc_update_param(gdc->pmio, gdc);
	camerapp_hw_gdc_check_dma_size(test, ctx->d_frame.width, ctx->d_frame.height,
		ctx->s_frame.width, ctx->s_frame.height, ctx->s_frame.width, ctx->s_frame.height,
		ALIGN(ctx->s_frame.width, 16), ALIGN(ctx->s_frame.width, 16), 0, 0,
		ALIGN(ctx->crop_param[ctx->cur_index].crop_width * 2, 16),
		ALIGN(ctx->crop_param[ctx->cur_index].crop_width * 2, 16), 0, 0);

	/* scale up 2x, use_grid, lossless */
	ctx->s_frame.width = 160;
	ctx->s_frame.height = 120;
	ctx->s_frame.gdc_fmt = &gdc_formats[8];
	ctx->s_frame.extra = COMP;
	ctx->s_frame.pixelformat = V4L2_PIX_FMT_NV16M;
	ctx->s_frame.pixel_size = CAMERA_PIXEL_SIZE_8BIT;
	ctx->d_frame.width = 320;
	ctx->d_frame.height = 240;
	ctx->d_frame.gdc_fmt = &gdc_formats[11];
	ctx->d_frame.extra = COMP;
	ctx->d_frame.pixelformat = V4L2_PIX_FMT_NV21M_P010;
	ctx->d_frame.pixel_size = CAMERA_PIXEL_SIZE_10BIT;
	...
	ctx->s_frame.gdc_fmt = &gdc_formats[19];
	ctx->s_frame.pixel_size = CAMERA_PIXEL_SIZE_10BIT;
	ctx->s_frame.pixelformat = V4L2_PIX_FMT_NV12M_SBWCL_10B;
	ctx->d_frame.gdc_fmt = &gdc_formats[18];
	ctx->d_frame.pixel_size = CAMERA_PIXEL_SIZE_8BIT;
	ctx->d_frame.pixelformat = V4L2_PIX_FMT_NV12M_SBWCL_8B;
