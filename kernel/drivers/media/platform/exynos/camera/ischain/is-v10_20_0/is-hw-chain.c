...
int is_hw_ischain_enable(struct is_core *core)
{
	...
	if (!valid_hw_slot_id(hw_slot)) {
		err("invalid slot (%d,%d)", DEV_HW_3AA0, hw_slot);
	} else {
		itf_hwip = &(itfc->itf_ip[hw_slot]);
	...
	if (!valid_hw_slot_id(hw_slot)) {
		err("invalid slot (%d,%d)", DEV_HW_3AA1, hw_slot);
	} else {
		itf_hwip = &(itfc->itf_ip[hw_slot]);
		pablo_set_affinity_irq(itf_hwip->irq[INTR_HWIP1], true);
		pablo_set_affinity_irq(itf_hwip->irq[INTR_HWIP2], true);
	...
	if (!valid_hw_slot_id(hw_slot)) {
		err("invalid slot (%d,%d)", DEV_HW_3AA2, hw_slot);
	} else {
		itf_hwip = &(itfc->itf_ip[hw_slot]);
		pablo_set_affinity_irq(itf_hwip->irq[INTR_HWIP1], true);
		pablo_set_affinity_irq(itf_hwip->irq[INTR_HWIP2], true);
	...
	if (!valid_hw_slot_id(hw_slot)) {
		err("invalid slot (%d,%d)", DEV_HW_ORB0, hw_slot);
	} else {
		itf_hwip = &(itfc->itf_ip[hw_slot]);
		pablo_set_affinity_irq(itf_hwip->irq[INTR_HWIP1], true);
	}
	...
	if (!valid_hw_slot_id(hw_slot)) {
		err("invalid slot (%d,%d)", DEV_HW_ISP0, hw_slot);
	} else {
		itf_hwip = &(itfc->itf_ip[hw_slot]);
		pablo_set_affinity_irq(itf_hwip->irq[INTR_HWIP1], true);
		pablo_set_affinity_irq(itf_hwip->irq[INTR_HWIP2], true);
	...
	if (!valid_hw_slot_id(hw_slot)) {
		err("invalid slot (%d,%d)", DEV_HW_MCSC0, hw_slot);
	} else {
		itf_hwip = &(itfc->itf_ip[hw_slot]);
		pablo_set_affinity_irq(itf_hwip->irq[INTR_HWIP1], true);
	}
