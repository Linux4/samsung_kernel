...
static int is_hw_dvfs_get_hf(struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
...
int is_hw_dvfs_get_scenario_param(
	struct is_device_ischain *device, int flag_capture, struct is_dvfs_scenario_param *param)
{
		...
		for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
			...
			if (format->hw_format) {
				...
				if (test_bit(PABLO_SUBDEV_ALLOC, &i_subdev->state)) {
					...
					out_bitwidth = i_subdev->memory_bitwidth;
