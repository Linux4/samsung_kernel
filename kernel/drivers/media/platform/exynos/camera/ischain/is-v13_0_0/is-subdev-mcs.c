		...
		struct is_crop *otcrop)
{
	if (incrop->h != otcrop->h)
		incrop->x = ALIGN_DOWN(GET_SCALED_SIZE(incrop->x, GET_ZOOM_RATIO(incrop->w, otcrop->w)), 2);
	...
	IS_DECLARE_PMAP(pmap))
{
	...
	struct is_fmt *fmt;
	struct param_otf_output *shrp_otf_out;
	...
	shrp_otf_out = is_itf_g_param(device, ldr_frame, PARAM_SHRP_OTF_OUTPUT);
	otcrop_cfg.w = shrp_otf_out->width;
	otcrop_cfg.h = shrp_otf_out->height;
	...
	if (ldr_frame && ldr_frame->stripe_info.region_num) {
		...
		if (!ldr_frame->stripe_info.region_id) {
			...
			stripe_input->stripe_roi_start_pos_x[i] = otcrop->w;
		}
				...
				struct param_mcs_output *mcs_output)
{
	...
	sd->width = core->vendor.isp_max_width ? core->vendor.isp_max_width : mcs_output->width;
	sd->height = core->vendor.isp_max_height ? core->vendor.isp_max_height : mcs_output->height;
...
static int is_ischain_mcs_tag_hf_otf(struct is_device_ischain *device,
				struct is_frame *frame)
{
	struct param_mcs_output *mcs_output = is_itf_g_param(device, frame, PARAM_MCS_OUTPUT5);
	...
	struct param_otf_output *shrp_otf_output = is_itf_g_param(device, frame, PARAM_SHRP_HF_OTF_OUTPUT);

	mdbgs_ischain(4, "MCSC HF OTF\n", device);

	set_bit(PARAM_RGBP_OTF_OUTPUT_HF, frame->pmap);
	set_bit(PARAM_MCFP_HF, frame->pmap);
	set_bit(PARAM_SHRP_HF_OTF_OUTPUT, frame->pmap);

	shrp_otf_output->cmd = OTF_OUTPUT_COMMAND_ENABLE;
	shrp_otf_output->width = mcs_output->width;
	shrp_otf_output->height = mcs_output->height;
	shrp_otf_output->format = mcs_output->dma_format;
	shrp_otf_output->bitwidth = mcs_output->dma_bitwidth;

	rgbp_otf_output->cmd = OTF_OUTPUT_COMMAND_ENABLE;
	rgbp_otf_output->width = mcs_output->width;
	rgbp_otf_output->height = mcs_output->height;
	rgbp_otf_output->format = mcs_output->dma_format;
	rgbp_otf_output->bitwidth = mcs_output->dma_bitwidth;

	mcfp_otf_input->cmd = OTF_INPUT_COMMAND_ENABLE;
	mcfp_otf_input->width = mcs_output->width;
	mcfp_otf_input->height = mcs_output->height;
	mcfp_otf_input->format = mcs_output->dma_format;
	mcfp_otf_input->bitwidth = mcs_output->dma_bitwidth;
				...
				struct is_frame *frame)
{
	...
	if (mcs_output->otf_cmd == OTF_OUTPUT_COMMAND_ENABLE)
		return is_ischain_mcs_tag_hf_otf(device, frame);
#endif
	...
	for (i = 0; i < votf_frame->planes; i++)
		trs_addr[i] = votf_frame->dvaddr_buffer[i];

	ret = is_hw_set_hf_votf_size_config(mcs_output->width, mcs_output->height);
	if (ret) {
