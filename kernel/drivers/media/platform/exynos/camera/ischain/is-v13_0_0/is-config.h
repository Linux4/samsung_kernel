...
/**
 * 1 Sensor could have maximum 3 streams.
 *   - Preview (Single bayer or Long bayer for sensor AEB mode)
 *   - Preview (Short bayer for sensor AEB mode)
 *   - Reprocessing
 */
#define IS_PREVIEW_STREAM_MAX	5
#define IS_STREAM_COUNT		(IS_PREVIEW_STREAM_MAX * 3)
...
#define SHRP_COMP_BLOCK_WIDTH		32
#define SHRP_COMP_BLOCK_HEIGHT		4
#define SHRP_HF_COMP_BLOCK_WIDTH	256
#define SHRP_HF_COMP_BLOCK_HEIGHT	1
...
#define ENABLE_SAT_CAV
#define ENABLE_AWB_HAL
...
#define ENABLE_YUVP_CLAHE_ITERATION 1

#if IS_ENABLED(CONFIG_ARCH_VELOCE_HYCON)
#define ICPU_IO_COHERENCY	0
...
#define SECURE_CAMERA_MEM_SIZE		(0x01C00000)	/* 0x01B40000 -> 4MB aligned for CMA */
...
#define CHECK_NEED_KVADDR_LVN_ID(vid)                                                              \
	((0) || ((vid) == IS_LVN_LME_PREV) || ((vid) == IS_LVN_MCFP_MV) ||                         \
	 ((vid) == IS_LVN_BYRP_RTA_INFO) || ((vid) == IS_LVN_RGBP_RTA_INFO) ||                     \
	 ((vid) == IS_LVN_MCFP_RTA_INFO) || ((vid) == IS_LVN_YUVP_RTA_INFO) ||                     \
	 ((vid) == IS_LVN_SHRP_RTA_INFO) || ((vid) == IS_LVN_LME_RTA_INFO) ||                      \
	 (is_get_debug_param(IS_DEBUG_PARAM_LVN) && (vid) == IS_VIDEO_IMM_NUM))

#define CHECK_NEED_KVADDR_ID(vid) ((0 || (vid == IS_VIDEO_LME)))
