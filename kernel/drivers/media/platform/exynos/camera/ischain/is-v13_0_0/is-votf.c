...
#define HF_DST_GROUP_ID GROUP_ID_SHRP
#define HF_DST_VOTF_IP VOTF_SHRP_R
#define HF_DST_VOTF_ID SHRP_HF
#else
#define HF_DST_GROUP_ID GROUP_ID_MCS0
#define HF_DST_VOTF_IP VOTF_MCSC_R
#define HF_DST_VOTF_ID MCSC_HF
#endif
...
static unsigned int is_votf_ip[GROUP_ID_MAX] = {
	...
	[HF_DST_GROUP_ID] = HF_DST_VOTF_IP,
};
...
int is_hw_set_hf_votf_size_config(u32 width, u32 height)
{
	...
	dst_info.service = TRS;
	dst_info.ip = HF_DST_VOTF_IP;
	dst_info.id = HF_DST_VOTF_ID;
...
void is_votf_subdev_flush(struct is_group *group)
{
	...
	if (src_gr && src_gr->id == GROUP_ID_RGBP && group->id == HF_DST_GROUP_ID) {
		src_info.ip = VOTF_RGBP_W;
		src_info.id = RGBP_HF;
...
int is_votf_subdev_create_link(struct is_group *group)
{
	...
	if (src_gr->id == GROUP_ID_RGBP && group->id == HF_DST_GROUP_ID) {
		src_info.ip = VOTF_RGBP_W;
		src_info.id = RGBP_HF;
...
void is_votf_subdev_destroy_link(struct is_group *group)
{
	...
	if (src_gr && src_gr->id == GROUP_ID_RGBP && group->id == HF_DST_GROUP_ID)
		votfitf_destroy_link(VOTF_RGBP_W, HF_DST_VOTF_IP);
}
#endif
