...
#include "is-hw-settle-4nm-lpe-dcphy6400.h"
...
void is_hw_s2mpu_cfg(void)
{
	...
	/* BYRP: SYSMMU_S0_BYRP_S2 */
	reg = ioremap(0x23160054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* CSIS: SYSMMU_S0_CSIS_S2 */
	reg = ioremap(0x23A20054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* ICPU: SYSMMU_S0_ICPU_S2 */
	reg = ioremap(0x248D0054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* MCSC(MCSC, MTNR0): SYSMMU_S0_MCSC_S2 */
	reg = ioremap(0x25890054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* MLSC: SYSMMU_S0_MLSC_S2 */
	reg = ioremap(0x278A0054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* MTNR(MTNR1): SYSMMU_S0_MTNR_S2 */
	reg = ioremap(0x26920054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* RGBP: SYSMMU_RGBP_S0_S2 */
	reg = ioremap(0x250A0054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* YUVP: SYSMMU_S0_YUVP_S2 */
	reg = ioremap(0x28890054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);
}
...
int is_hw_ischain_enable(struct is_core *core)
{
	...
	u32 idx, arry_len;
	u32 hw_dev_arry[] = {
			DEV_HW_BYRP0, DEV_HW_BYRP1, DEV_HW_BYRP2, DEV_HW_BYRP3, DEV_HW_BYRP4,
			DEV_HW_RGBP0, DEV_HW_RGBP1, DEV_HW_RGBP2, DEV_HW_RGBP3,
			DEV_HW_YUVSC0, DEV_HW_YUVSC1, DEV_HW_YUVSC2, DEV_HW_YUVSC3,
			DEV_HW_MLSC0, DEV_HW_MLSC1, DEV_HW_MLSC2, DEV_HW_MLSC3,
			DEV_HW_MTNR0, DEV_HW_MTNR1,
			DEV_HW_MSNR,
			DEV_HW_YPP,
			DEV_HW_MCSC0
		};
	...
	arry_len = sizeof(hw_dev_arry)/sizeof(u32);

	/* irq affinity should be restored after S2R at gic600 */
	for (idx = 0; idx < arry_len; idx++) {
		hw_slot = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, hw_dev_arry[idx]);
