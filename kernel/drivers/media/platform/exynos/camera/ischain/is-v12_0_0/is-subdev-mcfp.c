	...
	IS_DECLARE_PMAP(pmap))
{
	...
	if (frame->stripe_info.region_num) {
		...
		if (stripe_input->left_margin > MCFP_STRIP_OTF_MARGIN_CROP &&
		    frame->stripe_info.out.crop_width > MCFP_STRIP_OTF_MARGIN_CROP) {
			frame->stripe_info.out.left_margin -= MCFP_STRIP_OTF_MARGIN_CROP;
			frame->stripe_info.out.crop_width -= MCFP_STRIP_OTF_MARGIN_CROP;
			frame->stripe_info.out.crop_x += MCFP_STRIP_OTF_MARGIN_CROP;
		}

		if (stripe_input->right_margin > MCFP_STRIP_OTF_MARGIN_CROP &&
		    frame->stripe_info.out.crop_width > MCFP_STRIP_OTF_MARGIN_CROP) {
			frame->stripe_info.out.right_margin -= MCFP_STRIP_OTF_MARGIN_CROP;
			frame->stripe_info.out.crop_width -= MCFP_STRIP_OTF_MARGIN_CROP;
		}
#endif
...
static void __debug_check_cmd(struct is_device_ischain *device,
		struct is_subdev *leader,
		struct is_frame *ldr_frame,
		struct camera2_node *node,
		struct param_dma_input *dma_input)
{
	u32 fcnt = ldr_frame ? ldr_frame->fcount : 0;
	dma_addr_t dva = 0;
	...
	if (!test_bit(IS_ISCHAIN_REPROCESSING, &device->state) &&
		...
		switch (node->vid) {
		case IS_LVN_MCFP_PREV_YUV:
			dva = ldr_frame ? ldr_frame->dva_mcfp_prev_yuv[0] : 0;
			break;
		case IS_LVN_MCFP_PREV_W:
			dva = ldr_frame ? ldr_frame->dva_mcfp_prev_wgt[0] : 0;
			break;
		default:
			print_debug = false;
			break;
		...
		if (print_debug)
			mlvinfo("[F%d] preview current cmd(%d), new request(%d), dva(%llx)",
				device, node->vid, fcnt,
				dma_input->cmd, node->request, dva);
	}
}
	...
	IS_DECLARE_PMAP(pmap))
{
	...
	__debug_check_cmd(device, leader, ldr_frame, node, dma_input);
		...
		struct is_frame *frame)
{
	...
	otf_input = is_itf_g_param(device, frame, PARAM_MCFP_HF);
	otf_input->cmd = OTF_INPUT_COMMAND_DISABLE;
