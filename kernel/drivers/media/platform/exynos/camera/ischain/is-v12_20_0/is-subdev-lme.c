	...
	IS_DECLARE_PMAP(pmap))
{
	...
	case IS_LVN_LME_RTA_INFO:
		if (IS_ENABLED(IRTA_CALL) && !node->request) {
			mlverr("[F%d] rta info node is disabled", device, node->vid,
					ldr_frame->fcount);
			return -EINVAL;
		}
		return 0;
	...
	struct camera2_node *node)
{
	...
	if (ret) {
		mlverr("[F%d] lme_dma_cfg fail. ret %d", device, node->vid, frame->fcount, ret);
		return ret;
	}
	...
	for (i = 0; i < CAPTURE_NODE_MAX; i++) {
		...
		if (ret) {
			mlverr("[F%d] lme_dma_cfg fail. ret %d", device, node->vid, frame->fcount,
			       ret);
			return ret;
		}
