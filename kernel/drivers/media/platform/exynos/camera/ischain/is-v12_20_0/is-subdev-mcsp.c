...
static u32 is_ischain_mxp_get_sbwc_type(struct is_fmt *fmt, u32 flags)
{
	...
	/* for sbwc v2.8 */
	if (fmt->sbwc_lossy_fr)
		hw_sbwc |= 1 << SBWC_LOSSY_FR_SHIFT;
	...
	int index)
{
	...
	if (((pindex - PARAM_MCS_OUTPUT0) != MCSC_INPUT_HF) && (otcrop->w % 16)) {
		...
		*/
		mlvwarn("[F%d] Output crop width(%d) should be multiple of %d", device, node->vid,
			ldr_frame->fcount, otcrop->w, 16);
	}
	...
	if ((pindex - PARAM_MCS_OUTPUT0) == MCSC_INPUT_HF && !node->bytesperline[0]) {
		...
		if (!IS_ALIGNED(mcs_output->dma_stride_y, 16) ||
			...
			mlvwarn("WDMA stride(y:%d uv:%d) should be %d aligned", device, node->vid,
				mcs_output->dma_stride_y, mcs_output->dma_stride_c, 16);
	}
