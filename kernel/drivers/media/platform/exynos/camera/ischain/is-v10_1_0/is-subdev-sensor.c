...
{
	ulong seamless_mode = sensor->aeb_state & IS_SENSOR_AEB_MODE_MASK;
	...
	if (!(vc & SENSOR_2EXP_MODE_SHORT_VC_MASK))
		return false;
			...
			!test_bit(IS_SENSOR_AEB_SWITCHING, &seamless_mode))
		return true;
	...
	struct camera2_node *node)
{
	...
	for (i = 0; i < CAPTURE_NODE_MAX; i++) {
		...
		if (!cap_node->request || !IS_SENSOR_LVN_VID(cap_node->vid) || cap_node->result)
			continue;
