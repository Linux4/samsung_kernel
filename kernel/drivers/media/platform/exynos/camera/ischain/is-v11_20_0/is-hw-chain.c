...
static const struct is_field sysreg_csis_fields[SYSREG_CSIS_REG_FIELD_CNT] = {
	...
	{"MIPI_RESETN_DPHY_S5", 5, 1, RW, 0x0},
	{"MIPI_RESETN_DPHY_S4", 4, 1, RW, 0x0},
...
KUNIT_EXPORT_SYMBOL(is_hw_get_subdev_info);
...
KUNIT_EXPORT_SYMBOL(is_hw_group_open);
...
KUNIT_EXPORT_SYMBOL(is_get_hw_list);
...
KUNIT_EXPORT_SYMBOL(is_hw_get_address);
...
KUNIT_EXPORT_SYMBOL(is_hw_get_irq);
...
KUNIT_EXPORT_SYMBOL(is_hw_request_irq);
...
PST_KUNIT_EXPORT_SYMBOL(is_hw_find_settle);

void is_hw_camif_init(void)
{
}
KUNIT_EXPORT_SYMBOL(is_hw_camif_init);
...
int is_hw_camif_cfg(void *sensor_data)
{
	...
	if ((otf_info->csi_ch == CSI_ID_E) && test_bit(IS_SENSOR_OPEN, &sensor->state)) {
		info("single CSI_E need to use the 2+2 phy.");
		csi_e_enabled = true;
	}
	...
	for (i = 0; i < IS_SENSOR_COUNT; i++) {
		if (test_bit(IS_SENSOR_OPEN, &(core->sensor[i].state))
			...
			if (otf_info->csi_ch == CSI_ID_E) {
				info("CSI_E is the one of multiple sensor. use the 2+2 phy.");
				csi_e_enabled = true;
			}
		}
	}
...
KUNIT_EXPORT_SYMBOL(is_hw_camif_cfg);
...
KUNIT_EXPORT_SYMBOL(is_hw_ischain_cfg);
...
KUNIT_EXPORT_SYMBOL(is_hw_ischain_enable);
...
KUNIT_EXPORT_SYMBOL(is_hw_ischain_disable);
...
void is_hw_csi_qchannel_enable_all(bool enable)
{
	phys_addr_t csis_cmn_ctrls[NUM_OF_CSIS] = {
		0x15030004, 0x15040004, 0x15050004, 0x15060004, 0x15070004,
	};
	void __iomem *base;
	u32 val;
	int i;
...
KUNIT_EXPORT_SYMBOL(is_hw_interrupt_relay);
...
KUNIT_EXPORT_SYMBOL(is_hw_configure_bts_scen);
...
KUNIT_EXPORT_SYMBOL(is_hw_get_output_slot);
...
KUNIT_EXPORT_SYMBOL(is_hw_get_capture_slot);
...
KUNIT_EXPORT_SYMBOL(is_hw_fill_target_address);
...
KUNIT_EXPORT_SYMBOL(is_hw_get_iommu_mem);
...
KUNIT_EXPORT_SYMBOL(is_hw_print_target_dva);
...
KUNIT_EXPORT_SYMBOL(is_hw_config);
...
KUNIT_EXPORT_SYMBOL(is_hw_update_pcfi);

#if IS_ENABLED(CONFIG_PABLO_KUNIT_TEST)
irqreturn_t is_hw_isr_check(int hw_id, int irq, void *data)
{
	irqreturn_t ret = 0;

	switch (hw_id) {
	case DEV_HW_3AA0:
		ret |= __is_isr1_cstat0(irq, data);
		ret |= __is_isr2_cstat0(irq, data);
		break;
	case DEV_HW_3AA1:
		ret |= __is_isr1_cstat1(irq, data);
		ret |= __is_isr2_cstat1(irq, data);
		break;
	case DEV_HW_3AA2:
		ret |= __is_isr1_cstat2(irq, data);
		ret |= __is_isr2_cstat2(irq, data);
		break;
	case DEV_HW_LME0:
		ret |= __is_isr1_lme(irq, data);
		break;
	case DEV_HW_BYRP:
		ret |= __is_isr1_byrp(irq, data);
		ret |= __is_isr2_byrp(irq, data);
		break;
	case DEV_HW_RGBP:
		ret |= __is_isr1_rgbp(irq, data);
		ret |= __is_isr2_rgbp(irq, data);
		break;
	case DEV_HW_YPP:
		ret |= __is_isr1_yuvp(irq, data);
		ret |= __is_isr2_yuvp(irq, data);
		break;
	case DEV_HW_MCFP:
		ret |= __is_isr1_mcfp(irq, data);
		ret |= __is_isr2_mcfp(irq, data);
		break;
	case DEV_HW_MCSC0:
		ret |= __is_isr1_mcs0(irq, data);
		ret |= __is_isr2_mcs0(irq, data);
		break;
	default:
		probe_err("hw_id(%d) is invalid", hw_id);
	}
...
KUNIT_EXPORT_SYMBOL(is_hw_isr_check);
#endif
