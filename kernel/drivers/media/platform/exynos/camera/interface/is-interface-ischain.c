	...
	struct is_hardware *hardware, struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->num_of_ip.shrp; i++) {
		hw_id = DEV_HW_SHRP + i;
		handler_id = -1;
		ret = is_interface_hw_ip_probe(this, hw_id, handler_id, pdev, hardware);
		if (ret) {
	...
	for (i = 0; i < pdata->num_of_ip.dlfe; i++) {
		hw_id = DEV_HW_DLFE + i;
		handler_id = -1;
		ret = is_interface_hw_ip_probe(this, hw_id, handler_id, pdev, hardware);
		if (ret) {
	...
	for (i = 0; i < pdata->num_of_ip.yuvsc; i++) {
		hw_id = DEV_HW_YUVSC0 + i;
		handler_id = -1;
		ret = is_interface_hw_ip_probe(this, hw_id, handler_id, pdev, hardware);
		if (ret) {
			err_itfc("interface probe fail (hw_ip: %d)", hw_id);
	...
	for (i = 0; i < pdata->num_of_ip.mlsc; i++) {
		hw_id = DEV_HW_MLSC0 + i;
		handler_id = -1;
		ret = is_interface_hw_ip_probe(this, hw_id, handler_id, pdev, hardware);
		if (ret) {
			err_itfc("interface probe fail (hw_ip: %d)", hw_id);
	...
	for (i = 0; i < pdata->num_of_ip.mtnr; i++) {
		hw_id = DEV_HW_MTNR0 + i;
		handler_id = -1;
		ret = is_interface_hw_ip_probe(this, hw_id, handler_id, pdev, hardware);
		if (ret) {
			err_itfc("interface probe fail (hw_ip: %d)", hw_id);
	...
	for (i = 0; i < pdata->num_of_ip.msnr; i++) {
		hw_id = DEV_HW_MSNR + i;
		handler_id = -1;
		ret = is_interface_hw_ip_probe(this, hw_id, handler_id, pdev, hardware);
		if (ret) {
