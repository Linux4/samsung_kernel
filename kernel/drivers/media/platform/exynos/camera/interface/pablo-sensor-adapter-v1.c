...
static int __pablo_sensor_adt_request_seamless_mode_change(struct is_sensor_interface *sensor_itf,
	u32 instance, u32 *ctrl_param, u32 *param_idx)
{
	int ret = 0;
	struct is_sensor_seamless_ctl seamless_mode;
	u32 ctrl_end;
	...
	seamless_mode.mode = ctrl_param[(*param_idx)++];
	seamless_mode.sf_hdr_mode = ctrl_param[(*param_idx)++];
	seamless_mode.mf_hdr_mode = ctrl_param[(*param_idx)++];
	seamless_mode.remosaic_crop_zoom_ratio = ctrl_param[(*param_idx)++];
	seamless_mode.reserved[0] = ctrl_param[(*param_idx)++];
	seamless_mode.reserved[1] = ctrl_param[(*param_idx)++];
	seamless_mode.reserved[2] = ctrl_param[(*param_idx)++];
	seamless_mode.reserved[3] = ctrl_param[(*param_idx)++];
	seamless_mode.reserved[4] = ctrl_param[(*param_idx)++];
	ctrl_end = ctrl_param[(*param_idx)++];

	mdbg_adt(2, "[%s] LN2[%d] LN4[%d] 12bit[%d] AEB[%d] RM[%d,%d]\n",
		instance, __func__,
		(seamless_mode.mode & SENSOR_MODE_LN2),
		(seamless_mode.mode & SENSOR_MODE_LN4),
		(seamless_mode.mode & SENSOR_MODE_REAL_12BIT),
		(seamless_mode.mode & SENSOR_MODE_AEB),
		(seamless_mode.mode & SENSOR_MODE_CROPPED_RMS),
		seamless_mode.remosaic_crop_zoom_ratio);
	...
	if (seamless_mode.mode & SENSOR_MODE_LN2)
		ret |= sensor_itf->cis_ext2_itf_ops.set_low_noise_mode(sensor_itf, IS_CIS_LN2);
	else if (seamless_mode.mode & SENSOR_MODE_LN4)
		ret |= sensor_itf->cis_ext2_itf_ops.set_low_noise_mode(sensor_itf, IS_CIS_LN4);
	else
		ret |= sensor_itf->cis_ext2_itf_ops.set_low_noise_mode(sensor_itf, IS_CIS_LNOFF);

	if (seamless_mode.mode & SENSOR_MODE_REAL_12BIT)
		ret |= sensor_itf->cis_ext_itf_ops
			.set_sensor_12bit_state(sensor_itf, SENSOR_12BIT_STATE_REAL_12BIT);
	else {
		if (sensor->cfg->input[CSI_VIRTUAL_CH_0].hwformat == HW_FORMAT_RAW12)
			ret |= sensor_itf->cis_ext_itf_ops
				.set_sensor_12bit_state(sensor_itf, SENSOR_12BIT_STATE_PSEUDO_12BIT);
		else
			ret |= sensor_itf->cis_ext_itf_ops
				.set_sensor_12bit_state(sensor_itf, SENSOR_12BIT_STATE_OFF);
	}

	if (seamless_mode.mode & SENSOR_MODE_AEB)
		ret |= sensor_itf->cis_ext2_itf_ops.set_hdr_mode(sensor_itf, SENSOR_HDR_MODE_2AEB_2VC);
	else
		ret |= sensor_itf->cis_ext2_itf_ops.set_hdr_mode(sensor_itf, SENSOR_HDR_MODE_SINGLE);

	if ((seamless_mode.mode & SENSOR_MODE_CROPPED_RMS)
		&& seamless_mode.remosaic_crop_zoom_ratio < 100)
		ret |= sensor_itf->cis_ext2_itf_ops
			.set_remosaic_zoom_ratio(sensor_itf, seamless_mode.remosaic_crop_zoom_ratio);
	else
		ret |= sensor_itf->cis_ext2_itf_ops.set_remosaic_zoom_ratio(sensor_itf, 0);
...
static const struct constrol_sensor_fn_table fn_table[SS_CTRL_END] = {
	...
	{SS_CTRL_CIS_REQUEST_SEAMLESS_MODE_CHANGE, __pablo_sensor_adt_request_seamless_mode_change},
...
{
	int ret = 0, link_vc, wdma_vc, wdma_idx;
	struct is_module_enum *module;
	struct v4l2_subdev *subdev_module;
	struct is_device_sensor *sensor;
	u32 stat_vc;
	...
	wdma_idx = sensor->cfg->dma_idx[stat_vc][LINK_VC_OUT_SINGLE];
	wdma_vc = sensor->cfg->dma_vc[stat_vc][LINK_VC_OUT_SINGLE];

	buf_info->sensor_mode = module->vc_extra_info[VC_BUF_DATA_TYPE_GENERAL_STAT1].sensor_mode;
	buf_info->hpd_width = sensor->cfg->input[stat_vc].width;
	buf_info->hpd_height = sensor->cfg->input[stat_vc].height;
	buf_info->hpd_data_size = sensor->cfg->output[wdma_idx][wdma_vc].stride *
				  sensor->cfg->output[wdma_idx][wdma_vc].height;

	for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
		if (sensor->cfg->output[wdma_idx][wdma_vc].type == VC_VPDAF) {
			link_vc = sensor->cfg->link_vc[wdma_idx][wdma_vc];

			buf_info->sensor_mode = module->vpd_sensor_mode;
			buf_info->vpd_width = sensor->cfg->input[link_vc].width;
			buf_info->vpd_height = sensor->cfg->input[link_vc].height;
			buf_info->vpd_data_size = sensor->cfg->output[wdma_idx][wdma_vc].stride *
						  sensor->cfg->output[wdma_idx][wdma_vc].height;
		}
	}

	if (sensor->cfg->special_vc_sensor_mode != VC_SENSOR_MODE_INVALID)
		buf_info->sensor_mode = sensor->cfg->special_vc_sensor_mode;

	dbg_sensor(2, "%s stat_vc %d mode %d hpd size (%dx%d / %d) vpd size (%dx%d / %d)\n",
			__func__,
			stat_vc,
			buf_info->sensor_mode,
			buf_info->hpd_width, buf_info->hpd_height, buf_info->hpd_data_size,
			buf_info->vpd_width, buf_info->vpd_height, buf_info->vpd_data_size);

	return ret;
}

static int pablo_sensor_adt_get_sensor_info(
	struct pablo_sensor_adt *adt, struct pablo_crta_sensor_info *pcsi, u32 fcount)
{
	u32 instance;
	unsigned long flags;
	struct pablo_sensor_adt_v1 *sensor_adt;
	...
	sensor_itf->cis_ext2_itf_ops.get_sensor_seamless_mode_info(sensor_itf,
		pcsi->seamless_mode_info, &pcsi->seamless_mode_cnt);

	sensor_itf->cis_ext2_itf_ops.get_sensor_applied_seamless_mode(sensor_itf,
		&pcsi->applied_seamless_mode, fcount);
	...
	if (pcsi->laser_af_available) {
		...
		if (af_type == ITF_LASER_AF_TYPE_VL53L1) {
			...
			pcsi->laser_af_info.dataSize = (u32)sizeof(struct itf_laser_af_data_VL53L5);
		} else if (af_type == ITF_LASER_AF_TYPE_VL53L8) {
			pcsi->laser_af_info.type = ITF_LASER_AF_TYPE_VL53L8;
			pcsi->laser_af_info.dataSize = (u32)sizeof(struct itf_laser_af_data_VL53L8);
		}
	}
	...
	if (IS_ENABLED(CONFIG_PABLO_OBTE_SUPPORT)) {
		ime = get_subdev_module_enum(sensor_itf);
		FIMC_BUG(!ime);

		subdev_module = ime->subdev;
		ids = v4l2_get_subdev_hostdata(subdev_module);

		if (ids->obte_sensor && ids->obte_sensor->configured) {
			pcsi->position = ids->obte_sensor->position;
			pcsi->type = ids->obte_sensor->type;
			pcsi->pixel_order = ids->obte_sensor->pixel_order;
			pcsi->mono_en = ids->obte_sensor->mono_en;
			pcsi->bayer_pattern = ids->obte_sensor->bayer_pattern;
			pcsi->hdr_type = ids->obte_sensor->hdr_type;
			pcsi->hdr_mode = ids->obte_sensor->hdr_mode;
			pcsi->sensor_12bit_mode = ids->obte_sensor->sensor_12bit_mode;
		}
	}
