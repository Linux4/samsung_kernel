...
enum csis_hw_dma_err_id {
	...
	CSIS_ERR_DMA_LASTDATA_LASTADDR_ERROR,
...
struct csis_irq_src {
	...
	u32 ebuf_status;
	struct dbg_irq_info dbg;
};

struct csis_dma_irq_src {
	u32 dma_start;
	u32 dma_end;
	u32 dma_abort;
	bool err_flag;
	ulong err_id[CSIS_MAX_NUM_DMA_ATTACH][DMA_VIRTUAL_CH_MAX];
};
...
int csi_hw_g_dma_irq_src(u32 __iomem *base_reg, struct csis_dma_irq_src *src, bool clear);
