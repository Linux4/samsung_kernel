...
#define CSTAT_PARAM_NUM			(PARAM_CSTAT_MAX - PARAM_CSTAT_CONTROL + 1)
...
enum is_param {
	...
	PARAM_CSTAT_PRE,
	PARAM_CSTAT_AWB,
	PARAM_CSTAT_MAX = PARAM_CSTAT_AWB,
	...
	PARAM_BYRP_RGBY_HIST,
	PARAM_BYRP_THSTAT_AWB,

	PARAM_RGBP_CONTROL,
	PARAM_RGBP_OTF_INPUT,
	PARAM_RGBP_OTF_OUTPUT,
	PARAM_RGBP_OTF_OUTPUT_HF,
	...
	PARAM_SHRP_CONTROL,
	PARAM_SHRP_OTF_INPUT,
	PARAM_SHRP_OTF_OUTPUT,
	PARAM_SHRP_DMA_INPUT,
	PARAM_SHRP_FTO_OUTPUT,
	PARAM_SHRP_STRIPE_INPUT,
	PARAM_SHRP_HF_DMA_INPUT,
	PARAM_SHRP_HF_OTF_INPUT,
	PARAM_SHRP_HF_OTF_OUTPUT,
	PARAM_SHRP_SEG,
...
struct param_stripe_input {
	...
	u32	stripe_roi_start_pos_x[MAX_STRIPE_REGION_NUM + 1];	/* Start X Position w/o Margin For STRIPE */
	u32	start_pos_x;
	u32	repeat_idx;
	u32	repeat_num;
	u32	repeat_scenario;
...
struct byrp_param {
	...
	struct param_dma_input		hdr1;
	struct param_dma_output		byr;
	struct param_dma_output		byr_processed;
	struct param_dma_output		rgbyhist;
	struct param_dma_output		thstat_awb;
};
...
struct rgbp_param {
	...
	struct param_otf_input		otf_input;
	struct param_otf_output         otf_output;
	struct param_otf_output         otf_output_hf;
	struct param_dma_input		dma_input;
	struct param_stripe_input	stripe_input;
	struct param_dma_input		in_rgb;
...
struct shrp_param {
	...
	struct param_otf_output         fto_output;
	struct param_stripe_input	stripe_input;
	struct param_dma_input		hf;
	struct param_otf_input		hf_otf_input;
	struct param_otf_output         hf_otf_output;
	struct param_dma_input		seg;
};
...
struct cstat_param_set {
	...
	pdma_addr_t			output_dva_lme_ds0[IS_MAX_PLANES];
	pdma_addr_t			output_dva_lme_ds1[IS_MAX_PLANES];
	...
	pdma_addr_t			output_dva_pre[IS_MAX_PLANES];
...
struct byrp_param_set {
	...
	struct param_dma_input		dma_input_hdr1;
	...
	struct param_dma_output		dma_output_thstat_awb;
	pdma_addr_t			input_dva[IS_MAX_PLANES];
	pdma_addr_t			input_dva_hdr[IS_MAX_PLANES];
	pdma_addr_t			input_dva_hdr1[IS_MAX_PLANES];
	pdma_addr_t			output_dva_byr[IS_MAX_PLANES];
	pdma_addr_t			output_dva_byr_processed[IS_MAX_PLANES];
	pdma_addr_t			output_dva_byrrgbyhist[IS_MAX_PLANES];
	pdma_addr_t			output_dva_thstat_awb[IS_MAX_PLANES];
...
struct mcfp_param_set {
	...
	struct param_dma_output		dma_output_yuv;
	struct param_otf_input		otf_input_hf;
	pdma_addr_t			input_dva[IS_MAX_PLANES];
...
struct shrp_param_set {
	struct yuvp_param_set		*yuvp_param;
	struct param_otf_input		otf_input;
	struct param_otf_output		otf_output;
	struct param_dma_input		dma_input;
	struct param_otf_output         fto_output;
	struct param_stripe_input	stripe_input;
	struct param_dma_input		dma_input_hf;
	struct param_otf_input		otf_input_hf;
	struct param_otf_output         otf_output_hf;
	struct param_dma_input		dma_input_seg;
	pdma_addr_t			input_dva_hf[IS_MAX_PLANES];
	pdma_addr_t			input_dva_seg[IS_MAX_PLANES];
