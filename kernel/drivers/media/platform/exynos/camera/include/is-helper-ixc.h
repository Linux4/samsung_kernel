...
#define ixc_info(fmt, ...) printk(KERN_DEBUG "[^^]" fmt, ##__VA_ARGS__)
#else
#ifndef CONFIG_USE_SIGNED_BINARY
#define ixc_info(fmt, ...)                                            \
	do {                                                          \
		int enable = is_get_debug_param(IS_DEBUG_PARAM_IXC);  \
		if (enable)                                           \
...
#define IXC_DELAY_US(delay_us) \
	IXC_ARGS((delay_us) / 1000, (delay_us) % 1000, IXC_MODE_DELAY_16BIT)
...
struct is_ixc_ops {
	...
	int (*addr_data_write16)(void *client, u8 addr, u8 val_high,
				 u8 val_low);
};
...
int pablo_kunit_i2c_fill_msg_addr8_read8(struct i2c_client *client, u8 addr,
					 u8 *val, struct i2c_msg *msg,
					 u8 *wbuf);
int pablo_kunit_i2c_fill_msg_read8(struct i2c_client *client, u16 addr, u8 *val,
				   struct i2c_msg *msg, u8 *wbuf);
int pablo_kunit_i2c_fill_msg_read16(struct i2c_client *client, u16 addr,
				    struct i2c_msg *msg, u8 *wbuf, u8 *rbuf);
...
int pablo_kunit_i2c_fill_msg_addr8_write8(struct i2c_client *client, u8 addr,
					  u8 val, struct i2c_msg *msg,
					  u8 *wbuf);
int pablo_kunit_i2c_fill_msg_write8(struct i2c_client *client, u16 addr, u8 val,
				    struct i2c_msg *msg, u8 *wbuf);
int pablo_kunit_i2c_fill_msg_write8_array(struct i2c_client *client, u16 addr,
					  u8 *val, u32 num, struct i2c_msg *msg,
					  u8 *wbuf);
int pablo_kunit_i2c_fill_msg_write16(struct i2c_client *client, u16 addr,
				     u16 val, struct i2c_msg *msg, u8 *wbuf);
int pablo_kunit_i2c_fill_msg_data_write16(struct i2c_client *client, u8 val_h,
					  u8 val_l, struct i2c_msg *msg,
					  u8 *wbuf);
int pablo_kunit_i2c_fill_msg_write16_array(struct i2c_client *client, u16 addr,
					   u16 *arr_val, u32 num,
					   struct i2c_msg *msg, u8 *wbuf);
int pablo_kunit_i2c_fill_msg_write16_burst(struct i2c_client *client, u16 addr,
					   u16 *arr_val, u32 num,
					   struct i2c_msg *msg, u8 *wbuf);
...
int pablo_kunit_i3c_fill_msg_addr8_read8(u8 addr, u8 *val,
					 struct i3c_priv_xfer *xfers, u8 *wbuf);
int pablo_kunit_i3c_fill_msg_read16(u16 addr, struct i3c_priv_xfer *xfers,
				    u8 *wbuf, u8 *rbuf);
