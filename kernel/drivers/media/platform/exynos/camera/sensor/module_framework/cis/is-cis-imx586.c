...
int sensor_imx586_calibration_setting(struct is_cis *cis)
{
	...
	is_sec_get_rom_info(&rom_info, ROM_ID_REAR4);
	cal_buf = rom_info->buf;

#ifdef CAMERA_IMX586_CAL_MODULE_VERSION
	if (rom_info->header_ver[10] < CAMERA_IMX586_CAL_MODULE_VERSION) {
		info("%s - skip calibration, cal value not valid(cur_header : %s)", __func__, rom_info->header_ver);
		return 0;
	}
#endif

	spdc_size = rom_info->rom_spdc_cal_data_size;
	pdxtc_size[0] = rom_info->rom_pdxtc_cal_data_0_size;
	pdxtc_size[1] = rom_info->rom_pdxtc_cal_data_1_size;

	memcpy(spdc_cal, &cal_buf[rom_info->rom_spdc_cal_data_start_addr], SENSOR_IMX586_SPDC_CAL_SIZE);
	memcpy(pdxtc_cal, &cal_buf[rom_info->rom_pdxtc_cal_data_start_addr], SENSOR_IMX586_PDXTC_CAL_SIZE);
