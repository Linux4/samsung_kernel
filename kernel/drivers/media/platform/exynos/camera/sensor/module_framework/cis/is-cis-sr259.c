...
static int sensor_sr259_cis_dump_registers(struct v4l2_subdev *subdev, const u32 *regs, const u32 size)
{
	...
	for (i = 0; i < size; i += I2C_WRITE) {
		if (regs[i + IXC_BYTE] == I2C_WRITE_ADDR8_DATA8) {
			...
			ret = cis->ixc_ops->addr8_read8(cis->client, regs[i + IXC_ADDR], &data8);
			if (ret < 0) {
...
int sensor_sr259_cis_init(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->addr8_read8(cis->client, 0x04, &data8);
...
*/
	...
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x03, 0x00); // page 0 set
	if (ret < 0)
		goto p_err;

	cis->ixc_ops->addr8_read8(cis->client, 0x05, &data8);
...
static int sensor_sr259_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	...
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x1f, hold);
	if (ret < 0)
		goto p_err;
...
int sensor_sr259_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	/* Sensor stream on */
	cis->ixc_ops->addr8_write8(cis->client, 0x01, 0x00);
...
int sensor_sr259_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->addr8_write8(cis->client, 0x01, 0x20);
...
int sensor_sr259_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	/* page select */
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x03, 0x20); // page 20 set
	if (ret < 0)
		goto p_err;

	/* Short exposure */
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x22, (short_coarse_int>>8)&0xff);
	if (ret < 0)
		goto p_err;

	ret = cis->ixc_ops->addr8_write8(cis->client, 0x23, short_coarse_int&0xff);
	if (ret < 0)
		goto p_err;
...
int sensor_sr259_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x60, analog_gain_msb);
	if (ret < 0)
		goto p_err;

	ret = cis->ixc_ops->addr8_write8(cis->client, 0x61, analog_gain_lsb);
	if (ret < 0)
		goto p_err;
...
int sensor_sr259_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	ret = cis->ixc_ops->read16(cis->client, 0x60, &analog_gain_msb);
	if (ret < 0)
		goto p_err;

	ret = cis->ixc_ops->read16(cis->client, 0x61, &analog_gain_lsb);
	if (ret < 0)
		goto p_err;
