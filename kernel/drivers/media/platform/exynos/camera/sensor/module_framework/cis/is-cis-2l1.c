...
int sensor_2l1_cis_check_rev(struct is_cis *cis)
{
	...
	case 0x00:
		...
		cis->ixc_ops->write16(cis->client, 0x70A2, 0x0001);
		...
		cis->ixc_ops->write16(cis->client, 0x0000, 0x20C1);
		cis->ixc_ops->write16(cis->client, 0x0002, 0xC101);
		break;
	default:
...
int sensor_2l1_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	cis->ixc_ops->write8(cis->client, 0x070A, 0x01);
...
int sensor_2l1_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	if (ret < 0)
		goto p_err;
	ret = cis->ixc_ops->write16_array(cis->client, 0x0216, dgains, 4);
	if (ret < 0)
		goto p_err;
	...
	if (is_vendor_wdr_mode_on(cis_data)) {
		...
		ret = cis->ixc_ops->write16_array(cis->client, 0x3072, dgains, 4);
		if (ret < 0)
			goto p_err;
		ret = cis->ixc_ops->write16_array(cis->client, 0x307A, dgains, 4);
		if (ret < 0)
			goto p_err;
	}
