...
int sensor_4e6_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (!test_bit(IS_SENSOR_PREPROCESSOR_AVAILABLE, &sensor_peri->peri_state))
		cis->ixc_ops->write8(cis->client, 0x5333, 0xE0);
...
int sensor_4e6_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	 ...
	 */
	/* Long analog gain */
	ret = cis->ixc_ops->write16(cis->client, 0x0220, long_gain);
	if (ret < 0)
		goto p_err;
...
int sensor_4e6_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* Long digital gain */
	ret = cis->ixc_ops->write16_array(cis->client, 0x0222, dgains, 4);
	if (ret < 0)
		goto p_err;
...
static int cis_4e6_probe_i2c(struct i2c_client *client,
