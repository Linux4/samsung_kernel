...
#if defined(CONFIG_CAMERA_VENDOR_MCD_V2)
extern const struct is_vendor_rom_addr *vendor_rom_addr[SENSOR_POSITION_MAX];
#endif
...
int sensor_gc5035_check_rev(struct is_cis *cis)
{
	...
	/* read chip id */
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xfe, 0x02);
	if (ret < 0) {
		err("sensor_gc5035_set_registers fail!!");
		goto p_err;
	}
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x69, 0x00);
	if (ret < 0) {
		err("sensor_gc5035_set_registers fail!!");
		goto p_err;
	}
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x6a, 0x08);
	if (ret < 0) {
		err("sensor_gc5035_set_registers fail!!");
		goto p_err;
	}
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xf3, 0x20);
	if (ret < 0) {
		err("sensor_gc5035_set_registers fail!!");
		goto p_err;
	}

	ret = cis->ixc_ops->addr8_read8(cis->client, 0x6c, &rev);
	if (ret < 0) {
		err("sensor_gc5035_set_registers fail!!");
		goto p_err;
	}
...
int sensor_gc5035_set_exposure_time(struct v4l2_subdev *subdev, u16 multiple_exp)
{
	...
	/* Page Selection */
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xfe, 0x00);
	if (ret < 0)
		 goto p_err;

	/* Short exposure */
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x03, (multiple_exp >> 8) & 0x3f);
	if (ret < 0)
		goto p_err;
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x04, (multiple_exp & 0xfc));
	if (ret < 0)
		goto p_err;
...
int sensor_gc5035_set_analog_digital_gain(struct v4l2_subdev *subdev, u32 input_again)
{
	...
	/* Analog gain */
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xb6, analog_gain);
	if (ret < 0)
		goto p_err;

	/* Digital gain int*/
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xb1, 0x01);
	if (ret < 0)
		goto p_err;

	/* Digital gain decimal*/
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xb2, digital_gain);
		if (ret < 0)
			goto p_err;
...
int sensor_gc5035_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* Digital gain int*/
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xb1, (short_gain >> 8) & 0x0f);
	if (ret < 0)
		goto p_err;

	/* Digital gain decimal*/
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xb2, short_gain & 0xfc);
		if (ret < 0)
			goto p_err;
...
static int sensor_gc5035_cis_dpc_enable(struct v4l2_subdev *subdev) {
	...
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xbe, 0x00);
	if (ret < 0) {
		goto p_err;
	}
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xa9, 0x01);
	if (ret < 0) {
		goto p_err;
	}
	...
	if (ret < 0)
		goto p_err;
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x6a, 0x70);
	if (ret < 0)
		goto p_err;
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xf3, 0x20);
	if (ret < 0)
	...
	ret = cis->ixc_ops->addr8_read8(cis->client, 0x6c, &num_defect_1);
	if (ret < 0)
		goto p_err;

	/* Read OTP 0x0078 */
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x6a, 0x78);
	if (ret < 0)
		goto p_err;
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xf3, 0x20);
	if (ret < 0)
	...
	ret = cis->ixc_ops->addr8_read8(cis->client, 0x6c, &num_defect_2);
	if (ret < 0)
		goto p_err;
	...
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x01, 0x00);
	if (ret < 0)
		goto p_err;
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x02, num_defect_total);
	if (ret < 0)
		goto p_err;
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x03, 0x00);
	if (ret < 0)
		goto p_err;
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x04, 0x80);
	if (ret < 0)
		goto p_err;

	/* Step 5. DPC Table Auto Load */
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x09, 0x33);
	if (ret < 0)
		goto p_err;

	/* Set 0xf3[7] = 1 to start automatic load*/
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xf3, 0x80);
	if (ret < 0)
		goto p_err;
...
int sensor_gc5035_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	...
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x0b, (start_x >> 8));
	if (ret < 0)
		 goto p_err;

	ret = cis->ixc_ops->addr8_write8(cis->client, 0x0c, (start_x & 0xff));
	if (ret < 0)
		 goto p_err;

	/*2 byte address for writing the start_y*/
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x09, (start_y >> 8));
	if (ret < 0)
		 goto p_err;

	ret = cis->ixc_ops->addr8_write8(cis->client, 0x0a, (start_y & 0xff));
	if (ret < 0)
		 goto p_err;

	/*2 byte address for writing the end_x*/
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x0f, (end_x >> 8));
	if (ret < 0)
		 goto p_err;
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x10, (end_x & 0xff));
	if (ret < 0)
		 goto p_err;

	/*2 byte address for writing the end_y*/
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x0d, (end_y >> 8));
	if (ret < 0)
		 goto p_err;
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x0e, (end_y & 0xff));
	if (ret < 0)
		 goto p_err;

	/* 3. page_select */
	ret = cis->ixc_ops->addr8_write8(cis->client, 0xfe, 0x01);
	if (ret < 0)
		 goto p_err;

	/* 4. output address setting width*/
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x97, (cis_data->cur_width >> 8));
	if (ret < 0)
		 goto p_err;
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x98, (cis_data->cur_width & 0xff));
	if (ret < 0)
		 goto p_err;

	/* 4. output address setting height*/
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x95, (cis_data->cur_height >> 8));
	if (ret < 0)
		 goto p_err;
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x96, (cis_data->cur_height & 0xff));
	if (ret < 0)
		 goto p_err;
	...
	/* 5. binnig setting */
	ret = cis->ixc_ops->write8(cis->client, 0x33, binning);	/* 1:  binning enable, 0: disable */
	if (ret < 0)
		goto p_err;
...
int sensor_gc5035_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	/* Sensor stream on */
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x3E, 0x91);
	if (ret < 0) {
		err("i2c treansfer fail addr(%x), val(%x), ret(%d)\n", 0x3e, 0x91, ret);
		goto p_i2c_err;
	}
...
int sensor_gc5035_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	/* Sensor stream off */
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x3e, 0x00);
	if (ret < 0) {
		err("i2c treansfer fail addr(%x), val(%x), ret(%d)\n", 0x3e, 0x00, ret);
		goto p_err;
	}
...
int sensor_gc5035_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	ret = cis->ixc_ops->addr8_write8(cis->client, 0x41, (frame_length_lines >> 8) & 0x3f);
	if (ret < 0)
		goto p_err;

	ret = cis->ixc_ops->addr8_write8(cis->client, 0x42, (frame_length_lines & 0xfc));
	if (ret < 0)
		goto p_err;
...
int sensor_gc5035_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	ret = cis->ixc_ops->addr8_read8(cis->client, 0xb6, &analog_gain);
	if (ret < 0)
		goto p_err;
...
int sensor_gc5035_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	...
	/* Digital gain int*/
	ret = cis->ixc_ops->addr8_read8(cis->client, 0xb1, &digital_gain);
	if (ret < 0)
		goto p_err;

	/* Digital gain decimal*/
	ret = cis->ixc_ops->addr8_read8(cis->client, 0xb2, &digital_gain_dec);
	if (ret < 0)
		goto p_err;
...
int sensor_gc5035_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	...
	do {
		...
		/* Page Selection */
		ret = cis->ixc_ops->addr8_write8(cis->client, 0xfd, 0x00);
		if (ret < 0)
			 goto p_err;
		...
		/* Sensor stream off */
		ret = cis->ixc_ops->addr8_read8(cis->client, 0x3e, &read_value_3E);
		if (ret < 0) {
			err("i2c transfer fail addr(%x) ret = %d\n", 0x3e, ret);
			goto p_err;
		}

		ret = cis->ixc_ops->addr8_read8(cis->client, 0x2A, &read_value_2A);
		if (ret < 0) {
			err("i2c transfer fail addr(%x) ret = %d\n", 0x2A, ret);
			goto p_err;
		}
	...
	const struct i2c_device_id *id)
{
	...
	char const *setfile;
#if defined(CONFIG_CAMERA_VENDOR_MCD_V2) || defined(CONFIG_CAMERA_OTPROM_SUPPORT_FRONT) || defined(CONFIG_CAMERA_OTPROM_SUPPORT_REAR)
	...
	if (of_property_read_bool(dnode, "use_sensor_otp")) {
		...
		if (ret) {
			...
			vendor_priv->rom_client[rom_position] = cis->client;
			vendor_priv->rom_data[rom_position].rom_valid = true;

			if (vendor_rom_addr[rom_position]) {
				vendor_priv->rom_cal_map_addr[rom_position] = vendor_rom_addr[rom_position];
...
#else
#if defined(CONFIG_CAMERA_VENDOR_MCD) && defined(CONFIG_CAMERA_OTPROM_SUPPORT_FRONT)
	rom_position = 0;
	vendor_priv = core->vendor.private_data;
	vendor_priv->front_cis_client = client;
#endif
#if defined(CONFIG_CAMERA_VENDOR_MCD) && defined(CONFIG_CAMERA_OTPROM_SUPPORT_REAR)
	rom_position = 0;
	vendor_priv = core->vendor.private_data;
	vendor_priv->rear_cis_client = client;
#endif
#endif
