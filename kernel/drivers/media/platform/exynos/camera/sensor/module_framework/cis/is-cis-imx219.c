...
int sensor_imx219_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	{
	u16 pll, pll_h, pll_l;
	ret = cis->ixc_ops->read16(cis->client, 0x0301, &pll);
	dbg_sensor(1, "______ vt_pix_clk_div(%x)\n", pll);
	ret = cis->ixc_ops->read16(cis->client, 0x0303, &pll);
	dbg_sensor(1, "______ vt_sys_clk_div(%x)\n", pll);
	ret = cis->ixc_ops->read16(cis->client, 0x0304, &pll);
	dbg_sensor(1, "______ pre_pll_clk_div(%x)\n", pll);
	ret = cis->ixc_ops->read16(cis->client, 0x0306, &pll_h);
	ret = cis->ixc_ops->read16(cis->client, 0x0307, &pll_l);
	dbg_sensor(1, "______ pll_multiplier(%x%x)\n", pll_h, pll_l);
	ret = cis->ixc_ops->read16(cis->client, 0x0309, &pll);
	dbg_sensor(1, "______ op_pix_clk_div(%x)\n", pll);
	ret = cis->ixc_ops->read16(cis->client, 0x030b, &pll);
	dbg_sensor(1, "______ op_sys_clk_div(%x)\n", pll);

	ret = cis->ixc_ops->read16(cis->client, 0x0305, &pll);
	dbg_sensor(1, "______ secnd_pre_pll_clk_div(%x)\n", pll);
	ret = cis->ixc_ops->read16(cis->client, 0x030c, &pll_h);
	ret = cis->ixc_ops->read16(cis->client, 0x030d, &pll_l);
	dbg_sensor(1, "______ secnd_pll_multiplier(%x%x)\n", pll_h, pll_l);
	ret = cis->ixc_ops->read16(cis->client, 0x0160, &pll_h);
	ret = cis->ixc_ops->read16(cis->client, 0x0161, &pll_l);
	dbg_sensor(1, "______ frame_length_lines(%x%x)\n", pll_h, pll_l);
	ret = cis->ixc_ops->read16(cis->client, 0x0162, &pll_h);
	ret = cis->ixc_ops->read16(cis->client, 0x0163, &pll_l);
...
int sensor_imx219_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	/* Long exposure */
	if (is_vendor_enable_wdr(cis_data)) {
...
static int cis_imx219_probe_i2c(struct i2c_client *client,
