...
int sensor_2ld_cis_retention_crc_enable(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (IS_ENABLED(CAMERA_2LD_MIRROR_FLIP))
		...
		cis->ixc_ops->write8(cis->client, 0x010E, 0x01); /* api_rw_general_setup_checksum_on_ram_enable */
...
int sensor_2ld_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if (ex_mode == EX_DUALFPS_480 || ex_mode == EX_DUALFPS_960) {
		...
		cis->ixc_ops->write16(cis->client, 0x8A10, 0x0009);
		cis->ixc_ops->write16(cis->client, 0x8A0C, 0x03F7);
		cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
	...
	/* Sensor stream on */
	info("%s\n", __func__);
	cis->ixc_ops->write16(cis->client, 0x0BCC, 0x0100); // Enable mipi signal
...
int sensor_2ld_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	if (cis->cis_data->cur_hdr_mode == SENSOR_AEB_MODE_ON) {
		if (NEED_UPDATE_LUT0_LUT1(cis_data)) {
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT0 + AEB_2LD_OFFSET_CIT, long_coarse_int); // #1 short CIT
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT0 + AEB_2LD_OFFSET_LCIT, long_coarse_int); // #1 long CIT
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT1 + AEB_2LD_OFFSET_CIT, short_coarse_int); // #2 short CIT
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT1 + AEB_2LD_OFFSET_LCIT, short_coarse_int); // #2 long CIT
		}
			if (NEED_UPDATE_LUT2_LUT3(cis_data)) {
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT2 + AEB_2LD_OFFSET_CIT, long_coarse_int); // #3 short CIT
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT2 + AEB_2LD_OFFSET_LCIT, long_coarse_int); // #3 long CIT
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT3 + AEB_2LD_OFFSET_CIT, short_coarse_int); // #4 short CIT
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT3 + AEB_2LD_OFFSET_LCIT, short_coarse_int); // #4 long CIT
		}
...
int sensor_2ld_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	if (cis->cis_data->cur_hdr_mode == SENSOR_AEB_MODE_ON) {
		...
		frame_length_lines = (u16)((vt_pic_clk_freq_khz * frame_duration) / (line_length_pck * 1000));
		if (NEED_UPDATE_LUT0_LUT1(cis_data)) {
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT0 + AEB_2LD_OFFSET_FLL, frame_length_lines); // #1 FLL
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT1 + AEB_2LD_OFFSET_FLL, frame_length_lines); // #2 FLL
		}
		if (NEED_UPDATE_LUT2_LUT3(cis_data)) {
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT2 + AEB_2LD_OFFSET_FLL, frame_length_lines); // #3 FLL
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT3 + AEB_2LD_OFFSET_FLL, frame_length_lines); // #4 FLL
		}
		...
		if (sensor_2ld_LTE_1s_flag) {
			ret |= cis->ixc_ops->write16(cis->client, 0x0E22, frame_length_lines);
			if (ret < 0)
				goto p_err_i2c_unlock;
		}
...
int sensor_2ld_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	if (cis->cis_data->cur_hdr_mode == SENSOR_AEB_MODE_ON) {
		if (NEED_UPDATE_LUT0_LUT1(cis_data)) {
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT0 + AEB_2LD_OFFSET_AGAIN, analog_gain); // #1 Again
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT1 + AEB_2LD_OFFSET_AGAIN, analog_gain); // #2 Again

		}
		if (NEED_UPDATE_LUT2_LUT3(cis_data)) {
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT2 + AEB_2LD_OFFSET_AGAIN, analog_gain); // #3 Again
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT3 + AEB_2LD_OFFSET_AGAIN, analog_gain); // #4 Again
		}
		...
		if (sensor_2ld_LTE_1s_flag) {
			ret |= cis->ixc_ops->write16(cis->client, 0x0E1C, analog_gain);
			if (ret < 0)
				goto p_err_i2c_unlock;
		}
...
int sensor_2ld_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	if (cis->cis_data->cur_hdr_mode == SENSOR_AEB_MODE_ON) {
		if (NEED_UPDATE_LUT0_LUT1(cis_data)) {
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT0 + AEB_2LD_OFFSET_DGAIN, long_gain); // #1 Dgain
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT0 + AEB_2LD_OFFSET_LDGAIN, long_gain); // #1 long Dgain

			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT1 + AEB_2LD_OFFSET_DGAIN, short_gain); // #2 Dgain
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT1 + AEB_2LD_OFFSET_LDGAIN, short_gain); // #2 long Dgain
		}
		if (NEED_UPDATE_LUT2_LUT3(cis_data)) {
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT2 + AEB_2LD_OFFSET_DGAIN, long_gain); // #3 Dgain
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT2 + AEB_2LD_OFFSET_LDGAIN, long_gain); // #3 long Dgain

			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT3 + AEB_2LD_OFFSET_DGAIN, short_gain); // #4 Dgain
			ret |= cis->ixc_ops->write16(cis->client, AEB_2LD_LUT3 + AEB_2LD_OFFSET_LDGAIN, short_gain); // #4 long Dgain
		}
		...
		if (sensor_2ld_LTE_1s_flag) {
			ret |= cis->ixc_ops->write16(cis->client, 0x0E20, short_gain);
			if (ret < 0)
				goto p_err_i2c_unlock;
		}
