...
int sensor_gw2_cis_set_pdxtc_calibration(struct is_cis *cis)
{
	...
	if (rom_info->header_ver[10] < CAMERA_GW2_PDXTC_MODULE_VERSION) {
		info("%s - skip pdxtc_cal, cal value not valid(cur_header : %s)", __func__, rom_info->header_ver);
		return 0;
	}
#endif

	coef_size	= rom_info->rom_pdxtc_cal_data_0_size;
	val_size	= rom_info->rom_pdxtc_cal_data_1_size;

	pdxtc_cal = &cal_buf[rom_info->rom_pdxtc_cal_data_start_addr];
...
static int sensor_gw2_cis_update_pdaf_tail_size(struct v4l2_subdev *subdev, struct cis->ixc_ops->cfg *select)
{
	...
	info("[%s] dummy_flag[%d]", __func__, cal_buf[rom_info->rom_dualcal_slave1_dummy_flag_addr]);

	if (cal_buf[rom_info->rom_dualcal_slave1_dummy_flag_addr] != 7) { /* INVALID cal data */
		return 0;
	}
