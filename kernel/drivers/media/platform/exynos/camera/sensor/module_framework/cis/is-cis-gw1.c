...
static int sensor_gw1_cis_fine_integration_value(struct is_cis *cis)
{
	...
	ret = cis->ixc_ops->write16(cis->client, SENSOR_GW1_DIRECT_PAGE_ACCESS_MARK, SENSOR_GW1_DIRECT_DEFALT_PAGE_ADDR);
	if (ret < 0) {
		err("i2c fail addr(%x, %x) ,ret = %d\n",
			SENSOR_GW1_DIRECT_PAGE_ACCESS_MARK, SENSOR_GW1_DIRECT_DEFALT_PAGE_ADDR, ret);
		goto i2c_err;
	}
	ret = cis->ixc_ops->read8(cis->client, 0x1008, &fine_integ_min);
	if (ret < 0) {
		err("is_sensor_read is fail, (ret %d)", ret);
		ret = -EAGAIN;
		goto i2c_err;
	}
	ret = cis->ixc_ops->read8(cis->client, 0x100A, &fine_integ_max_margin);
	if (ret < 0) {
		err("is_sensor_read is fail, (ret %d)", ret);
		ret = -EAGAIN;
		goto i2c_err;
 ...
 *****************************/
	...
	ret = cis->ixc_ops->read8(cis->client, SENSOR_GW1_REVISION_NUM_ADDR, &rev8);
	if (ret < 0) {
		err("is_sensor_read is fail, (ret %d)", ret);
		ret = -EAGAIN;
		goto i2c_err;
	...
	ret = cis->ixc_ops->read8(cis->client, 0x000D, &data1);
	if (ret < 0) {
		err("is_sensor_read is fail, (ret %d)", ret);
		ret = -EAGAIN;
		goto i2c_err;
	...
	ret = cis->ixc_ops->read8(cis->client, 0x000E, &data2);
	if (ret < 0) {
		err("is_sensor_read is fail, (ret %d)", ret);
		ret = -EAGAIN;
		goto i2c_err;
...
int sensor_gw1_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	pr_info("[%s] *******************************\n", __func__);
	ret = cis->ixc_ops->read16(cis->client, SENSOR_GW1_MODEL_ID_ADDR, &data16);
	if (unlikely(!ret)) pr_info("model_id(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read8(cis->client, SENSOR_GW1_REVISION_NUM_ADDR, &data8);
	if (unlikely(!ret)) pr_info("revision_number(0x%x)\n", data8);
	else goto i2c_err;
	ret = cis->ixc_ops->read8(cis->client, SENSOR_GW1_FRAME_COUNT_ADDR, &data8);
	if (unlikely(!ret)) pr_info("frame_count(0x%x)\n", data8);
	else goto i2c_err;
	ret = cis->ixc_ops->read8(cis->client, SENSOR_GW1_SETUP_MODE_SELECT_ADDR, &data8);
	if (unlikely(!ret)) pr_info("setup_mode(streaming)(0x%x)\n", data8);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(cis->client, SENSOR_GW1_EXTCLK_FREQ_ADDR, &data16);
	if (unlikely(!ret)) pr_info("extclk_feq_mhz(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(cis->client, SENSOR_GW1_CORASE_INTEG_TIME_ADDR, &data16);
	if (unlikely(!ret)) pr_info("coarse_integration_time(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(cis->client, SENSOR_GW1_AGAIN_CODE_GLOBAL_ADDR, &data16);
	if (unlikely(!ret)) pr_info("gain_code_global(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(cis->client, SENSOR_GW1_FRAME_LENGTH_LINE_ADDR, &data16);
	if (unlikely(!ret)) pr_info("frame_length_line(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read8(cis->client, SENSOR_GW1_LINE_LENGTH_PCK_ADDR, &data8);
...
static int sensor_gw1_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	...
	IXC_MUTEX_LOCK(cis->ixc_lock);
	ret = cis->ixc_ops->write8(cis->client, SENSOR_GW1_GROUP_PARAM_HOLD_ADDR, hold);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gw1_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	/* Sensor stream on */
	ret = cis->ixc_ops->write8(cis->client, SENSOR_GW1_SETUP_MODE_SELECT_ADDR, 0x01);
	if (ret < 0)
...
int sensor_gw1_cis_stream_off(struct v4l2_subdev *subdev)
{
	...
	/* Sensor stream off */
	ret = cis->ixc_ops->write8(cis->client, SENSOR_GW1_SETUP_MODE_SELECT_ADDR, 0x00);
	if (ret < 0)
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0100, 0x0000, ret);
...
int sensor_gw1_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	ret = cis->ixc_ops->write16(cis->client, SENSOR_GW1_FRAME_LENGTH_LINE_ADDR, frame_length_lines);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gw1_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	/* Short exposure */
	ret = cis->ixc_ops->write16(cis->client, SENSOR_GW1_CORASE_INTEG_TIME_ADDR, short_coarse_int);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gw1_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	ret = cis->ixc_ops->write16(cis->client, SENSOR_GW1_AGAIN_CODE_GLOBAL_ADDR, analog_gain);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gw1_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	ret = cis->ixc_ops->read16(cis->client, SENSOR_GW1_AGAIN_CODE_GLOBAL_ADDR, &analog_gain);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gw1_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* Short digital gain */
	ret = cis->ixc_ops->write16(cis->client, SENSOR_GW1_DGAIN_GLOBAL_ADDR, short_dgain);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gw1_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	...
	ret = cis->ixc_ops->read16(cis->client, SENSOR_GW1_DGAIN_GLOBAL_ADDR, &digital_gain);
	if (ret < 0)
		goto i2c_err;
...
#define ABS_WB_GAIN_NUM	3
	...
	for (index=0; index < ABS_WB_GAIN_NUM; index++) {
		ret = cis->ixc_ops->write16(cis->client, (SENSOR_GW1_WBGAIN_RED + (index * 2)), abs_gains[index]);
		if (ret < 0) {
...
int cis_gw1_probe_i2c(struct i2c_client *client,
