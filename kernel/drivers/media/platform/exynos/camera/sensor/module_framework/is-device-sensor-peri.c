...
void is_sensor_ois_init_work(struct work_struct *data)
{
	...
	if (sensor_peri->mcu && ois) {
		if (!CALL_OISOPS(ois, ois_get_active, sensor_peri->subdev_mcu)) {
			ret = CALL_OISOPS(ois, ois_fw_update, core);
			if (ret < 0)
				err("v4l2_subdev_call(ois_fw_update) is fail(%d)", ret);

			ret = CALL_OISOPS(ois, ois_init, subdev_mcu);
			if (ret < 0) {
				err("v4l2_subdev_call(ois_init) is fail(%d)", ret);
				return;
			}

			ret = CALL_OISOPS(ois, ois_set_mode, subdev_mcu, OPTICAL_STABILIZATION_MODE_STILL);
			if (ret < 0) {
				err("v4l2_subdev_call(ois_set_mode) is fail(%d)", ret);
				return;
			}
...
#if (!defined(USE_RTA_CONTROL_LASER_AF)) || (defined(CONFIG_CAMERA_USE_INTERNAL_MCU) && defined(CAMERA_2ND_OIS))
bool is_sensor_peri_check_super_steady(struct is_device_sensor *device)
{
	if ((device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_FHD_30FPS_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_QHD_30FPS_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_HDR10_FHD_VIDEO
		|| (device->ischain->setfile & IS_SETFILE_MASK) == ISS_SUB_SCENARIO_SUPER_STEADY_HDR10_QHD_VIDEO)
		return true;
	else
		return false;
}
...
int is_sensor_peri_check_mcu_disable_condition(struct is_device_sensor *device, bool on)
{
	...
	if (is_sensor_peri_check_super_steady(device) || sensor_peri->check_auto_framing) {
	...
	if (on) {
		...
		if (check_mcu_disable) {
			...
			if ((device_mcu && device_mcu->mcu && device_mcu->subdev_mcu) || (sensor_peri->mcu && sensor_peri->subdev_mcu)) {
				if (!sensor_peri->mcu && sensor_peri->module->position != SENSOR_POSITION_FRONT) {
					ret = CALL_OISOPS(device_mcu->mcu->ois, ois_set_dev_ctrl, device_mcu->subdev_mcu, 0);
					if (ret < 0)
						err("v4l2_subdev_call(ois_set_dev_ctrl) is fail(%d)", ret);
		...
		if (!skip_sub_device) {
			...
			if (sensor_peri->laser_af && test_bit(IS_SENSOR_LASER_AF_AVAILABLE, &sensor_peri->peri_state)) {
				if (sensor_peri->laser_af->rs_mode != CAMERA_RANGE_SENSOR_MODE_OFF) {
					if (!is_sensor_peri_check_super_steady(device)) {
