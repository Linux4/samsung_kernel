...
int sensor_3m3_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (device->image.framerate == 24
		...
		cis->ixc_ops->write16(cis->client, 0x30DA, 0x00A9);
		cis->ixc_ops->write16(cis->client, 0x30DC, 0x00A9);
	} else {
		cis->ixc_ops->write16(cis->client, 0x30DA, 0x0016);
		cis->ixc_ops->write16(cis->client, 0x30DC, 0x0016);
	}
...
int sensor_3m3_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	if (is_vendor_wdr_mode_on(cis_data)) {
		cis->ixc_ops->write16(cis->client, 0x0216, 0x0100);
	} else {
		cis->ixc_ops->write16(cis->client, 0x0216, 0x0000);
	}
...
int sensor_3m3_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* DGain WDR Mode */
	if (is_vendor_wdr_mode_on(cis_data)) {
		cis->ixc_ops->write16(cis->client, 0x31FC, 0x0001);
	} else {
		cis->ixc_ops->write16(cis->client, 0x31FC, 0x0000);
	}
	...
	/* Long digital gain */
	if (is_vendor_wdr_mode_on(cis_data)) {
		ret = cis->ixc_ops->write16(cis->client, 0x3062, long_gain);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_3m3_cis_set_adjust_sync(struct v4l2_subdev *subdev, u32 sync)
{
	...
	ret = cis->ixc_ops->write8(cis->client, 0x30D7, 0x01);
...
static int cis_3m3_probe_i2c(struct i2c_client *client,
