...
int sensor_2x5_cis_write_cal(struct v4l2_subdev *subdev)
{
	...
	char *cal_buf = NULL;
#ifdef CONFIG_CAMERA_VENDOR_MCD_V2
	static struct is_vendor_rom *rom_info;
#else
	...
	dbg_sensor(1, "%s: sensor cal start addr1 0x%X, end addr1 0x%X, addr2 0x%X, end addr2 0x%X\n",
		__func__, rom_info->sensor_cal_data_start_addr, rom_info->sensor_cal_data_end_addr,
		*((u32 *)&cal_buf[0x18]), *((u32 *)&cal_buf[0x1C]));
#endif
	xtalk_buf = (u16 *)&cal_buf[rom_info->sensor_cal_data_start_addr];
	lsc_buf = (u16 *)&cal_buf[rom_info->sensor_cal_data_start_addr + SENSOR_2X5_CAL_XTALK_SIZE];
...
int sensor_2x5_cis_init(struct v4l2_subdev *subdev)
{
	...
	version_id = rom_info->rom_sensor_id[8];
#endif
