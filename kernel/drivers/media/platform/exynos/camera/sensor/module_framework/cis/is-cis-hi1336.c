...
int sensor_hi1336_cis_check_rev(struct is_cis *cis)
{
	...
	ret = cis->ixc_ops->read16(cis->client, SENSOR_HI1336_MODEL_ID_ADDR, &data16);
	if (ret < 0) {
...
int sensor_hi1336_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read16(cis->client, SENSOR_HI1336_STREAM_ONOFF_ADDR, &data16);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
...
static int sensor_hi1336_cis_group_param_hold_func(struct v4l2_subdev *subdev, bool hold)
{
	...
	ret = cis->ixc_ops->read16(cis->client, SENSOR_HI1336_GROUP_PARAM_HOLD_ADDR, &hold_set);
	if (ret < 0) {
	...
	ret = cis->ixc_ops->write16(cis->client, SENSOR_HI1336_GROUP_PARAM_HOLD_ADDR, hold_set);
	if (ret < 0) {
...
int sensor_hi1336_cis_enable_frame_cnt(struct i2c_client *client, bool enable)
{
	...
	ret = cis->ixc_ops->read8(cis->client, SENSOR_HI1336_MIPI_TX_OP_MODE_ADDR,
			&frame_cnt_ctrl);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
	...
	ret = cis->ixc_ops->write8(cis->client, SENSOR_HI1336_MIPI_TX_OP_MODE_ADDR,
			frame_cnt_ctrl);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
				SENSOR_HI1336_MIPI_TX_OP_MODE_ADDR, ret);
...
int sensor_hi1336_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	/* Sensor stream on */
	ret = cis->ixc_ops->write16(cis->client, SENSOR_HI1336_STREAM_ONOFF_ADDR,
			0x0001);
	IXC_MUTEX_UNLOCK(cis->ixc_lock);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
...
int sensor_hi1336_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read16(cis->client, SENSOR_HI1336_ISP_FRAME_CNT_ADDR,
			&cur_frame_value);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
	...
	do {
		...
		ret = cis->ixc_ops->read16(cis->client,
...
int sensor_hi1336_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	IXC_MUTEX_LOCK(cis->ixc_lock);
	ret = cis->ixc_ops->write16(cis->client, SENSOR_HI1336_FRAME_LENGTH_LINE_ADDR, frame_length_lines);
	if (ret < 0) {
		goto p_i2c_err;
	}
...
int sensor_hi1336_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	IXC_MUTEX_LOCK(cis->ixc_lock);
	ret = cis->ixc_ops->write16(cis->client, SENSOR_HI1336_COARSE_INTEG_TIME_ADDR, coarse_int);
		if (ret < 0)
			goto p_i2c_err;
...
int sensor_hi1336_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	// Analog gain
	IXC_MUTEX_LOCK(cis->ixc_lock);
	ret = cis->ixc_ops->write8(cis->client, SENSOR_HI1336_ANALOG_GAIN_ADDR, analog_gain);
	if (ret < 0)
		goto p_i2c_err;
...
int sensor_hi1336_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	...
	IXC_MUTEX_LOCK(cis->ixc_lock);
	ret = cis->ixc_ops->read16(cis->client, SENSOR_HI1336_DIG_GAIN_ADDR, &digital_gain);
	if (ret < 0)
		goto p_i2c_err;
...
int cis_hi1336_probe_i2c(struct i2c_client *client,
