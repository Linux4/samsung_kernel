...
int sensor_dw9780_actuator_mode_select(struct i2c_client *client, u32 mode)
{
	...
	val = mode_sel;
	ret = actuator->ixc_ops->write16(client, addr, val);
	if (ret < 0) {
...
static int sensor_dw9780_write_position(struct i2c_client *client, u32 val)
{
	...
	ret = actuator->ixc_ops->write16(client, DW9780_REG_OLAF_TARGET, val);
...
static int sensor_dw9780_actuator_probe_i2c(struct i2c_client *client,
...
static struct i2c_driver sensor_actuator_dw9780_driver = {
	.probe  = sensor_dw9780_actuator_probe_i2c,
	.driver = {
		.name	= ACTUATOR_NAME,
		.owner	= THIS_MODULE,
