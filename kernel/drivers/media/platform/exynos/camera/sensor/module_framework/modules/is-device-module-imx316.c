		...
		struct exynos_platform_is_module *pdata)
{
	...
	SET_PIN_SHARED(pdata, SENSOR_SCENARIO_VISION, GPIO_SCENARIO_ON, SRT_ACQUIRE,
			&core->shared_rsc_lock[SHARED_PIN4], &core->shared_rsc_count[SHARED_PIN4], 1);
#endif
	...
	SET_PIN_SHARED(pdata, SENSOR_SCENARIO_VISION, GPIO_SCENARIO_ON, SRT_RELEASE,
			&core->shared_rsc_lock[SHARED_PIN4], &core->shared_rsc_count[SHARED_PIN4], 0);
#endif
...
static int __init sensor_module_imx316_probe(struct platform_device *pdev)
{
	...
	ret = is_sensor_module_parse_dt(dev, sensor_module_imx316_power_setpin);
	if (ret)
		return ret;
