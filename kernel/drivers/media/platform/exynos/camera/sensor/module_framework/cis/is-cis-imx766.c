...
void sensor_imx766_cis_set_mode_group(u32 mode)
{
	...
	case SENSOR_IMX766_4080X2296_30FPS:
		sensor_imx766_mode_groups[SENSOR_IMX766_MODE_RMS_CROP] =
			SENSOR_IMX766_REMOSAIC_8MPCROP_4080x2296_30FPS;
		break;
	}
...
int sensor_imx766_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	sensor_cis_log_status(cis, cis->client, log_imx766, ARRAY_SIZE(log_imx766), (char *)__func__);
...
int sensor_imx766_cis_update_seamless_mode(struct v4l2_subdev *subdev)
{
	int ret = 0;
...
EXIT:
	CALL_CISOPS(cis, cis_update_seamless_mode, subdev);
p_err:
	return ret;
}
...
{
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	u32 mode = cis->cis_data->sens_config_index_cur;
	u32 pre_zoom_ratio = cis->cis_data->pre_remosaic_zoom_ratio;
	int ret = 0;

	if (pre_zoom_ratio == IMX766_REMOSAIC_ZOOM_RATIO_X_2 && mode == SENSOR_IMX766_REMOSAIC_12MPCROP_4080x3060_30FPS)
		*binning_ratio = 1000;
...
{
	/* This sensor(IMX766) needs to update seamless mode at vsync timing */
	return sensor_imx766_cis_update_seamless_mode(subdev);
}
