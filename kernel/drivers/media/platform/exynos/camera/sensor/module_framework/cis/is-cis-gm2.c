...
int sensor_gm2_cis_check_rev(struct is_cis *cis)
{
	...
	cis->ixc_ops->write16(cis->client, SENSOR_GM2_OTP_PAGE_SETUP_ADDR, 0x0001);

	/* Turn ON OTP Read MODE - Read Start */
	cis->ixc_ops->write16(cis->client, SENSOR_GM2_OTP_READ_TRANSFER_MODE_ADDR,  0x0100);

	/* Check status - 0x00 : read ready*/
	cis->ixc_ops->read8(cis->client, SENSOR_GM2_OTP_STATUS_REGISTER_ADDR,  &status);
	if (status != 0x0) {
		err("status fail, (%d)", status);
		msleep(40);
	}

	ret = cis->ixc_ops->read8(cis->client, SENSOR_GM2_OTP_CHIP_REVISION_ADDR, &rev);
	if (ret < 0) {
...
int sensor_gm2_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	/* Short exposure */
	ret = cis->ixc_ops->write16(cis->client, SENSOR_GM2_COARSE_INTEG_TIME_ADDR, short_coarse_int);
	if (ret < 0)
		goto p_err;

	/* Long exposure */
	if (is_vendor_wdr_mode_on(cis_data)) {
		ret = cis->ixc_ops->write16(cis->client, SENSOR_GM2_LONG_COARSE_INTEG_TIME_ADDR, long_coarse_int);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_gm2_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	...
	ret = cis->ixc_ops->write16(cis->client, SENSOR_GM2_FRAME_LENGTH_LINE_ADDR, frame_length_lines);
	if (ret < 0)
		goto p_err;
...
int sensor_gm2_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	ret = cis->ixc_ops->write16(cis->client, SENSOR_GM2_ANALOG_GAIN_ADDR, analog_gain);
	if (ret < 0)
		goto p_err;
...
int sensor_gm2_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	...
	ret = cis->ixc_ops->read16(cis->client, SENSOR_GM2_ANALOG_GAIN_ADDR, &analog_gain);
	if (ret < 0)
		goto p_err;
...
int sensor_gm2_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* Short digital gain */
	ret = cis->ixc_ops->write16_array(cis->client, SENSOR_GM2_DIG_GAIN_ADDR, dgains, 4);
	if (ret < 0)
		goto p_err;
...
int sensor_gm2_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	...
	ret = cis->ixc_ops->read16(cis->client, SENSOR_GM2_DIG_GAIN_ADDR, &digital_gain);
	if (ret < 0)
		goto p_err;
...
static int sensor_gm2_cis_set_dual_master_setting(struct is_cis *cis)
{
	...
	if (unlikely(ret))
		err("i2c treansfer fail addr(%x), val(%x), ret(%d)\n", 0x6028, 0x2000, ret);
	/* dual sync out index */
	ret = cis->ixc_ops->write16(cis->client, 0x602A, 0x106A);
	if (unlikely(ret))
		err("i2c treansfer fail addr(%x), val(%x), ret(%d)\n", 0x602A, 0x106A, ret);
	ret = cis->ixc_ops->write16(cis->client, 0x6F12, 0x0003);
	if (unlikely(ret))
		err("i2c treansfer fail addr(%x), val(%x), ret(%d)\n", 0x6F12, 0x0003, ret);
	/* master vsync out sel */
	ret = cis->ixc_ops->write16(cis->client, 0x602A, 0x2BC2);
	if (unlikely(ret))
		err("i2c treansfer fail addr(%x), val(%x), ret(%d)\n", 0x602A, 0x2BC2, ret);
	ret = cis->ixc_ops->write16(cis->client, 0x6F12, 0x0003);
	if (unlikely(ret))
