...
int sensor_3p9_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (cis->use_pdaf == true) {
		...
		ret = cis->ixc_ops->write8(cis->client, 0x3059, 0x00);
		...
		ret = cis->ixc_ops->write16(cis->client, 0x602A, 0x19E0);
		ret = cis->ixc_ops->write16(cis->client, 0x6F12, 0x0001);

		ret = cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
		ret = cis->ixc_ops->write16(cis->client, 0x30E2, 0x0000);
		...
		/* TEMP : BPC disable */
		ret = cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
		ret = cis->ixc_ops->write8(cis->client, 0x0B0E, 0x00);
...
int sensor_3p9_cis_set_factory_control(struct v4l2_subdev *subdev, u32 command)
{
	...
	case FAC_CTRL_BIT_TEST:
		...
		ret |= cis->ixc_ops->write16(cis->client, 0xFCFC, 0x4000);
		ret |= cis->ixc_ops->write16(cis->client, 0xF44A, 0x01FF); // TG 3.16v
		msleep(1);
		ret |= cis->ixc_ops->write16(cis->client, 0xF44A, 0x0004); // TG 2.43v -> 2.14v
		msleep(50);
		ret |= cis->ixc_ops->write16(cis->client, 0xF44C, 0x0009); // RG 3.61v -> 3.18v
		break;
	default:
...
static int cis_3p9_probe_i2c(struct i2c_client *client,
...
#ifdef MODULE
builtin_i2c_driver(sensor_cis_3p9_driver);
#else
