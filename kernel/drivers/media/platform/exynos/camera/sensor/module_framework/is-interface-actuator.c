...
int set_position(struct is_sensor_interface *itf, u32 position, int domain_offset)
{
	...
	if (itf->otf_flag_3aa == false) {
		...
		dbg_actuator("[%s][%d][%s] position(%d) domain_offset(%d)\n",
			__func__, cur_frame_cnt, sensor_peri->module->sensor_name,
			position, domain_offset);
		...
		u32 *position, u32 frame_diff)
{
	...
	cur_frame_cnt = get_frame_count(itf);
	prev_frame_cnt = ZERO_IF_NEG(cur_frame_cnt - frame_diff);
