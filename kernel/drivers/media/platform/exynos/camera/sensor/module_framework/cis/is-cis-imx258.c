...
int sensor_imx258_cis_init(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->write8(cis->client, 0x0A02, 0x0F);
	ret |= cis->ixc_ops->write8(cis->client, 0x0A00, 0x01);
	do {
		ret = cis->ixc_ops->read8(cis->client, 0x0A01, &data8);
		read_cnt ++;
		mdelay(1);
	} while (!(data8 & 0x01) && (read_cnt < 100));

	/* 0x10 : PDAF(0APH5), 0x30 : Non PDAF(0ATH5) */
	ret |= cis->ixc_ops->read8(cis->client, 0x0A2E, &data8);
	if (ret < 0) {
...
int sensor_imx258_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read16(cis->client, 0x0307, &pll);
	...
	ret = cis->ixc_ops->read16(cis->client, 0x030d, &pll);
	dbg_sensor(1, "______ secnd_pre_pll_clk_div(%x)\n", pll);
	ret = cis->ixc_ops->read16(cis->client, 0x030f, &pll);
