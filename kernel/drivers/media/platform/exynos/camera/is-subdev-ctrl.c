	...
	const struct is_subdev_ops *sops)
{
	...
	/* for internal use */
	frame_manager_probe(&subdev->internal_framemgr, name);
...
static int is_subdev_internal_alloc_buffer(struct is_subdev *subdev)
{
	...
	if (subdev->instance == 0)
		snprintf(heapname, sizeof(heapname), CAMERA_HEAP_NAME);
#endif
	...
	ret = frame_manager_open(&subdev->internal_framemgr, subdev->buffer_num, false);
	if (ret) {
