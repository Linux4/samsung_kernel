...
static int is_hw_lme_enable(struct is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	if (!lme_hw_use_mmio() && !IS_ENABLED(LME_PERFRAME_SWRESET)) {
	...
	case LME_RDMA_MBMV_IN:
		...
		if (hw_lme->lme_mode == LME_MODE_TNR) {
			...
			if (IS_ENABLED(LME_PERFRAME_SWRESET) &&
				!hw_lme->config[instance].first_frame) {
				input_dva[0] = param_set->mbmv0_dva[0];
				input_dva[1] =
					param_set->mbmv1_dva[0] + total_width * (line_count - 1);
			} else {
				input_dva[0] =
					param_set->mbmv1_dva[0] + total_width * (line_count - 1);
	...
	struct lme_param_set *param_set, u32 instance, u32 id, u32 set_id)
{
	...
	if (cmd == DMA_OUTPUT_COMMAND_ENABLE) {
		ret = lme_hw_s_wdma_addr(get_base(hw_ip), param_set, id, set_id, hw_lme->lme_mode, sps_mode);
		if (ret) {
	...
	u32 instance, u32 set_id)
{
	...
	if (IS_ENABLED(LME_PERFRAME_SWRESET))
		lme_hw_s_first_frame(get_base(hw_ip), set_id, 1); // for per frame rotation reset
	else
...
config:
	...
	if (!IS_ENABLED(IRTA_CALL) || unlikely(do_blk_cfg)) {
		...
		lme_hw_s_first_frame_forcely(get_base(hw_ip), set_id);
