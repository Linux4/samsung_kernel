...
static int param_get_debug_mcsc(char *buffer, const struct kernel_param *kp)
{
	...
	ret += sprintf(buffer + ret, "\tb[2] : s2d fs\n");
	ret += sprintf(buffer + ret, "\tb[3] : s2d fe\n");
	ret += sprintf(buffer + ret, "\tb[4] : skip setfile\n");
	ret += sprintf(buffer + ret, "\tb[5] : bypass\n");
	ret += sprintf(buffer + ret, "\tb[6] : DTP OTF\n");
	ret += sprintf(buffer + ret, "\tb[7] : DTP RDMA\n");
	ret += sprintf(buffer + ret, "\tb[8] : STRGEN\n");
	ret += sprintf(buffer + ret, "\tb[9] : check size\n");
...
static int is_hw_mcsc_handle_interrupt(u32 id, void *context)
{
	...
	is_hw_trace_logger(hw_ip, status);
	...
	if (is_scaler_intr_occurred0(status, INTR_FRAME_START)) {
		...
		if (unlikely(test_bit(MCSC_DBG_S2D_FS, &debug_mcsc)))
	...
	if (is_scaler_intr_occurred0(status, INTR_FRAME_END)) {
		...
		if (unlikely(test_bit(MCSC_DBG_S2D_FE, &debug_mcsc)))
			is_debug_s2d(true, "MCSC_DBG_S2D");
	}
	...
	if (f_err) {
		...
		is_scaler_dump(hw_ip->pmio, HW_DUMP_CR);
	}
...
static inline void __is_hw_mcsc_check_debug_DTP(struct is_hw_ip *hw_ip)
{
	...
	struct mcs_param *param, struct is_frame *frame, u32 instance)
{
	...
	if (hf_param->otf_cmd == OTF_OUTPUT_COMMAND_ENABLE) {
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
		is_scaler_set_hf_cinfifo_ctrl(get_base(hw_ip), 1, false);
#else
		mserr_hw("[F:%d] Invalid HF OTF cmd\n",
				instance, hw_ip, frame->fcount);
#endif
	} else {
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
		is_scaler_set_hf_cinfifo_ctrl(get_base(hw_ip), 0, false);

		is_scaler_set_djag_hf_enable(get_base(hw_ip), false, false);
		msdbg_hw(2, "[F:%d]high frequency: hf disabled, hf_param->otf_cmd(%d)\n",
				instance, hw_ip, atomic_read(&hw_ip->fcount), hf_param->otf_cmd);
		return ret;
#else
	...
	ret = is_scaler_set_hf_config(dma, get_base(hw_ip),
		hw_ip->id, true, hf_param, &radial_cfg, hf_setfile, &payload_size);
	if (ret) {
	...
	ulong hw_map)
{
	...
	__is_hw_mcsc_check_debug_DTP(hw_ip);
...
static int is_hw_mcsc_notify_timeout(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	is_scaler_dump(hw_ip->pmio, HW_DUMP_DBG_STATE);
