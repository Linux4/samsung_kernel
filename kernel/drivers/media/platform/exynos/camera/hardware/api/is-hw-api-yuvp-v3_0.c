...
#include "sfr/is-sfr-yuvp-mmio-v3_0.h"
#else
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
#include "sfr/is-sfr-yuvp-v3_1.h"
#else
#include "sfr/is-sfr-yuvp-v3_0.h"
#endif
#endif
#include "pmio.h"
...
void yuvp_hw_s_coutfifo(void *base, u32 set_id)
{
	...
	YUVP_SET_F(base, YUVP_R_YUV_COUTFIFO0_AUTO_VBLANK,
			YUVP_F_YUV_COUTFIFO0_AUTO_VBLANK, 0);
	YUVP_SET_F(base, YUVP_R_YUV_COUTFIFO1_AUTO_VBLANK,
...
int yuvp_hw_s_rdma_init(struct is_common_dma *dma, struct yuvp_param_set *param_set,
	...
	u32 *strip_offset, u32 *header_offset)
{
	int ret = SET_SUCCESS;
	...
	dbg_hw(4, "[API] %s: enable(%d), strip idx(%d), pos(%d)\n", __func__, enable, strip_index, strip_start_pos);
	...
	*sbwc_en = comp_sbwc_en = is_hw_dma_get_comp_sbwc_en(sbwc_type, &comp_64b_align);
	...
	if (comp_sbwc_en == 0) {
		...
		header_stride_1p = is_hw_dma_get_header_stride(full_dma_width, YUVP_COMP_BLOCK_WIDTH, 16);
	...
	*header_offset = strip_header_offset_in_byte;
	...
	dbg_hw(3, "%s : [%s] sbwc(%d) strip_ofs %d, fmt %d, width %d, height %d\n",
			__func__, dma->name, comp_sbwc_en, strip_offset_in_pixel,
			format, width, height);
	dbg_hw(3, "%s : [%s] img_ofs %d, header_ofs %d, stride (%d, %d)\n",
			__func__, dma->name, *strip_offset, *header_offset,
	...
	u32 strip_offset, u32 header_offset)
{
	...
	dbg_hw(4, "[API] %s (%d 0x%8llx 0x%8llx), strip_offset(%d, %d), size(%d)\n", __func__, dma->id,
			...
			strip_offset, header_offset, payload_size);
...
int yuvp_hw_wdma_create(struct is_common_dma *dma, void *base, u32 input_id)
{
	...
	switch (input_id) {
	case YUVP_WDMA_SVHIST:
		base_reg = base + yuvp_regs[YUVP_R_STAT_WDMASVHIST_EN].sfr_offset;
		available_bayer_format_map = 0X77 & IS_BAYER_FORMAT_MASK;
	...
	u32 biquad_scale_shift_adder)
{
	...
	dbg_hw(4, "[API] %s: sensor_binning_x %d, sensor_binning_y %d\n", __func__,
		radial_cfg->sensor_binning_x, radial_cfg->sensor_binning_y);
	dbg_hw(4, "[API] %s: sw_binning_x %d, sw_binning_y %d\n", __func__,
		radial_cfg->sw_binning_x, radial_cfg->sw_binning_y);
	dbg_hw(4, "[API] %s: sensor_full_width %d, sensor_full_height %d\n", __func__,
		radial_cfg->sensor_full_width, radial_cfg->sensor_full_height);
	dbg_hw(4, "[API] %s: sensor_crop_x %d, sensor_crop_y %d\n", __func__,
		radial_cfg->sensor_crop_x, radial_cfg->sensor_crop_y);
	dbg_hw(4, "[API] %s: taa_crop_width %d, taa_crop_height %d\n", __func__,
		radial_cfg->taa_crop_width, radial_cfg->taa_crop_height);
	dbg_hw(4, "[API] %s: yuvpp_strip_start_pos %d, taa_crop_x %d, taa_crop_y %d\n", __func__,
		yuvpp_strip_start_pos, radial_cfg->taa_crop_x, radial_cfg->taa_crop_y);
	dbg_hw(4, "[API] %s: frame_width %d, dma_width %d, dma_height %d\n", __func__,
	...
	binning_y = radial_cfg->sensor_binning_y * radial_cfg->bns_binning_y * radial_cfg->sw_binning_y *
			1024ULL * radial_cfg->taa_crop_height / dma_height / 1000 / 1000 / 1000;
	...
	dbg_hw(4, "[API] %s: biquad_scale_shift_adder %d\n", __func__, biquad_scale_shift_adder);
	...
	YUVP_SET_R(base + GET_COREX_OFFSET(set_id), YUVP_R_YUV_YUVNR_BIQUAD_SCALE_SHIFT_ADDER_HF,
		   val);

	val = 0;
	val = YUVP_SET_V(base, val, YUVP_F_YUV_YUVNR_BIQUAD_SCALE_SHIFT_ADDER_LF,
			 biquad_scale_shift_adder);
	YUVP_SET_R(base + GET_COREX_OFFSET(set_id), YUVP_R_YUV_YUVNR_BIQUAD_SCALE_SHIFT_ADDER_LF,
		   val);
...
void yuvp_hw_s_input_path(void *base, u32 set_id, enum yuvp_input_path_type path)
{
	...
	YUVP_SET_F(base + GET_COREX_OFFSET(set_id), YUVP_R_IP_USE_OTF_PATH_01, YUVP_F_IP_USE_OTF_IN_FOR_PATH_0, path);
}
...
void yuvp_hw_s_output_path(void *base, u32 set_id, int path)
{
	...
	YUVP_SET_F(base + GET_COREX_OFFSET(set_id), YUVP_R_IP_USE_OTF_PATH_01, YUVP_F_IP_USE_OTF_OUT_FOR_PATH_0, path);
}
...
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	YUVP_SET_F(base, YUVP_R_RGB_SVHIST_STREAM_CRC,
			YUVP_F_RGB_SVHIST_CRC_SEED, seed);
#endif
}
...
void yuvp_hw_init_pmio_config(struct pmio_config *cfg)
{
	...
	cfg->max_register = YUVP_R_YUV_YUVTORGB_STREAM_CRC;
	cfg->num_reg_defaults_raw = (YUVP_R_YUV_YUVTORGB_STREAM_CRC >> 2) + 1;
