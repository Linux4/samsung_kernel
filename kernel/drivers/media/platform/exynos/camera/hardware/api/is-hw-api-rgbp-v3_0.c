...
#include "sfr/is-sfr-rgbp-v3_1.h"
#else
...
static void rgbp_hw_s_path(void *base, u32 set_id, struct is_rgbp_config *config,
	struct rgbp_param_set *param_set)
{
	...
	chain_set.demux_otfout_en = 0x1;
	chain_set.satflg_en = 0x0;
	chain_set.mux_hf_sel = rgbp_hw_g_hf_mode(param_set);
	...
	RGBP_SET_F(base, RGBP_R_CHAIN_SATFLAG_ENABLE, RGBP_F_CHAIN_SATFLAG_ENABLE,
			chain_set.satflg_en);
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	RGBP_SET_F(base, RGBP_R_CHAIN_DEMUX_ENABLE_1, RGBP_F_DEMUX_HF_ENABLE,
			chain_set.mux_hf_sel);
#endif
}
...
static void rgbp_hw_s_cout_fifo(void *base, u32 set_id, struct param_otf_output *param)
{
	...
	RGBP_SET_F(base, GET_COREX_OFFSET(set_id) + RGBP_R_IP_USE_OTF_PATH_01,
		RGBP_F_IP_USE_OTF_OUT_FOR_PATH_0, enable);
	...
	/* HW constraint : Only one path can be selected */
	RGBP_SET_F(base, GET_COREX_OFFSET(set_id) + RGBP_R_YUV_COUTFIFO1_ENABLE,
		RGBP_F_YUV_COUTFIFO1_ENABLE, 0);
}

static void rgbp_hw_s_cout2_fifo(void *base, u32 set_id, struct param_otf_output *param)
{
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	u32 enable = param->cmd;

	RGBP_SET_F(base, GET_COREX_OFFSET(set_id) + RGBP_R_STAT_COUTFIFO2_INTERVALS,
		RGBP_F_STAT_COUTFIFO2_INTERVAL_HBLANK, HBLANK_CYCLE);
	RGBP_SET_F(base, GET_COREX_OFFSET(set_id) + RGBP_R_STAT_COUTFIFO2_INTERVAL_VBLANK,
		RGBP_F_STAT_COUTFIFO2_INTERVAL_VBLANK, VBLANK_CYCLE);
	RGBP_SET_F(base, RGBP_R_STAT_COUTFIFO2_CONFIG,
		RGBP_F_STAT_COUTFIFO2_VVALID_RISE_AT_FIRST_DATA_EN, 1);
	RGBP_SET_F(base, RGBP_R_STAT_COUTFIFO2_CONFIG, RGBP_F_STAT_COUTFIFO2_DEBUG_EN, 1);
	RGBP_SET_F(base, RGBP_R_STAT_COUTFIFO2_CONFIG, RGBP_F_STAT_COUTFIFO2_BACK_STALL_EN, 1);
	RGBP_SET_F(base, GET_COREX_OFFSET(set_id) + RGBP_R_STAT_COUTFIFO2_ENABLE,
		RGBP_F_STAT_COUTFIFO2_ENABLE, enable);
	RGBP_SET_F(base, GET_COREX_OFFSET(set_id) + RGBP_R_STAT_COUTFIFO2_INT_ENABLE,
		RGBP_F_STAT_COUTFIFO2_INT_ENABLE, 0x7);
#endif
}
...
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	RGBP_SET_F(base, RGBP_R_PIXEL_BUFFER_EN, RGBP_F_PIXEL_BUFFER_EN, 1);
#endif
}
...
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	RGBP_SET_F(base, RGBP_R_YUV_COUTFIFO0_STREAM_CRC, RGBP_F_YUV_COUTFIFO0_CRC_SEED, seed);
	RGBP_SET_F(base, RGBP_R_YUV_COUTFIFO1_STREAM_CRC, RGBP_F_YUV_COUTFIFO1_CRC_SEED, seed);
#endif
}
	...
	struct is_rgbp_config *config, struct rgbp_param_set *param_set)
{
	...
	rgbp_hw_s_cout2_fifo(base, set_id, &param_set->otf_output_hf);
	...
	struct rgbp_param_set *param_set, pdma_addr_t *output_dva, struct rgbp_dma_cfg *dma_cfg)
{
	...
	dma_addr_cfg.out_yuv = dma_cfg->out_yuv;
			...
			struct rgbp_dma_addr_cfg *dma_addr_cfg)
{
	...
	t_plane = dma_addr_cfg->out_yuv ? 2 : 3;
...
{
	u32 cinfifo, coutfifo0, coutfifo1, coutfifo2 = 0;
...
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	coutfifo2 = RGBP_GET_R(base, RGBP_R_STAT_COUTFIFO2_INT);
	RGBP_SET_R(base, RGBP_R_STAT_COUTFIFO2_INT_CLEAR, coutfifo2);
#endif

	info_hw("[RGBP] %s cinfifo(0x%x) coutfifo0(0x%x) coutfifo1(0x%x) coutfifo2(0x%x)\n",
		__func__, cinfifo, coutfifo0, coutfifo1, coutfifo2);
}
	...
	struct rgbp_param_set *dst)
{
	...
	if (test_bit(PARAM_RGBP_OTF_OUTPUT_HF, pmap))
		memcpy(&dst->otf_output_hf, &src->otf_output_hf,
			sizeof(struct param_otf_output));
}
...
{
	if (param_set->otf_output_hf.cmd == OTF_OUTPUT_COMMAND_ENABLE)
		return HF_OTF;
...
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	cfg->max_register = RGBP_R_STAT_COUTFIFO2_STREAM_CRC;
	cfg->num_reg_defaults_raw = (RGBP_R_STAT_COUTFIFO2_STREAM_CRC >> 2) + 1;
#else
