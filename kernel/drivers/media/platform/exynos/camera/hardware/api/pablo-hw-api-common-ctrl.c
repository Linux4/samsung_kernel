...
static bool _is_cmdq_queue_empty(struct pablo_common_ctrl *pcc)
{
...
static void pcc_disable(struct pablo_common_ctrl *pcc)
{
	struct pablo_mmio *pmio = pcc->pmio;
	...
	/* Disable CMDQ, CLOADER, STALL */
	SET_CR(pmio, CMN_CTRL_R_CMDQ_ENABLE, 0);
	SET_CR(pmio, CMN_CTRL_R_C_LOADER_ENABLE, 0);
	SET_CR_F(pmio, CMN_CTRL_R_CMDQ_VHD_CONTROL, CMN_CTRL_F_CMDQ_VHD_STALL_ON_QSTOP_ENABLE, 0);
