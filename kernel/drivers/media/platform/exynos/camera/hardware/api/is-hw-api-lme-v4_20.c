...
#include "sfr/is-sfr-lme-mmio-v4_20.h"
#else
#include "sfr/is-sfr-lme-v4_20.h"
#endif
...
void lme_hw_s_init(struct pablo_mmio *base, u32 set_id)
{
	...
	LME_SET_F(base, GET_COREX_OFFSET(COREX_DIRECT) +
		LME_R_IP_PROCESSING, LME_F_IP_PROCESSING, 1);
	...
	u32 id, u32 set_id, u32 lme_mode, enum lme_sps_out_mode sps_mode)
{
	...
	case LME_WDMA_MV_OUT:
		...
		output_dva[0] = param_set->output_dva[0] + stride * (line_count - 1);

		/* TOOD: Need to fix support corex */
		LME_SET_R(base, GET_COREX_OFFSET(COREX_DIRECT) +
			LME_R_DMACLIENT_CNTX0_SPS_MV_OUT_GEOM_BASE_ADDR_LSB,
			DVA_36BIT_LOW(output_dva[0]));

		LME_SET_R(base, GET_COREX_OFFSET(COREX_DIRECT) +
			LME_R_DMACLIENT_CNTX0_SPS_MV_OUT_GEOM_BASE_ADDR_0,
			DVA_36BIT_HIGH(output_dva[0]));
		break;
	case LME_WDMA_MBMV_OUT:
		...
		LME_SET_F(base, GET_COREX_OFFSET(COREX_DIRECT) +
			...
			DVA_36BIT_LOW(output_dva[1]));

		LME_SET_R(base, GET_COREX_OFFSET(COREX_DIRECT) +
			LME_R_DMACLIENT_CNTX0_MBMV_OUT_GEOM_BASE_ADDR_1,
			DVA_36BIT_HIGH(output_dva[1]));
	...
	case LME_WDMA_SAD_OUT:
		...
		output_dva[0] = param_set->output_dva_sad[0] + stride * (line_count - 1);
	...
	dbg_hw(4, "[API] [%s] id:%d output_dva[0]:0x%llx output_dva[1]:0x%llx",
		__func__, id, output_dva[0], output_dva[1]);
...
{
	LME_SET_F(base, GET_COREX_OFFSET(COREX_DIRECT) + LME_R_MVCT_8BIT_LME_CONFIG,
		  LME_F_Y_LME_FIRSTFRAME, 1);

	LME_SET_F(base,
		  GET_COREX_OFFSET(COREX_DIRECT) +
			  LME_R_DMACLIENT_CNTX0_MBMV_IN_GEOM_BASE_ADDR_ROTATION_RESET,
