...
void pdp_hw_s_input_mux(struct is_pdp *pdp, u32 lc[CAMIF_VC_ID_NUM])
{
	...
	if (IS_ENABLED(CONFIG_PDP_INPUT_MUX)) {
		val = PDP_SET_V(val, PDP_F_MUX_AF0_VC, lc[CAMIF_VC_HPD]);
		val = PDP_SET_V(val, PDP_F_MUX_AF1_VC, lc[CAMIF_VC_VPD]);
		val = PDP_SET_V(val, PDP_F_MUX_IMG_VC, lc[CAMIF_VC_IMG]);
