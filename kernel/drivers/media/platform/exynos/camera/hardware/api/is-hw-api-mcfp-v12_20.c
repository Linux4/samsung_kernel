...
#include "sfr/is-sfr-mcfp-v12_20.h"
#include "pmio.h"
...
#define info_mcfp_ver(fmt, args...) \
		info_common("[MCPF](v12.20)", fmt, ##args)
...
static const struct is_reg mcfp_dbg_cr[] = {
...
static void mcfp_hw_dump_dbg_state(struct pablo_mmio *pmio)
{
	void *ctx;
	...
	pmio->reg_read(ctx, MCFP_R_IP_VERSION, &val);

	is_dbg("[HW:%s] v%02u.%02u.%02u ======================================\n", pmio->name,
		(val >> 24) & 0xff, (val >> 16) & 0xff, val & 0xffff);
	for (i = 0; i < ARRAY_SIZE(mcfp_dbg_cr); i++) {
		cr = &mcfp_dbg_cr[i];
	...
	/* Additional DBG CRs */
	for (i = 0, ofs = MCFP_R_DBG_0; ofs < MCFP_R_DBG_44; i += 4, ofs += 0x10)
		is_dbg("[HW:%s]%5s%02u-%02u: %08x %08x %08x %08x\n", pmio->name,
			"DBG_", i, i + 3,
			MCFP_GET_R(pmio, ofs), MCFP_GET_R(pmio, ofs + 0x4),
			MCFP_GET_R(pmio, ofs + 0x8), MCFP_GET_R(pmio, ofs + 0xc));

	is_dbg("[HW:%s]%5s%-5u: %08x\n", pmio->name, "DBG_", i, MCFP_GET_R(pmio, ofs));
	is_dbg("[HW:%s]=================================================\n", pmio->name);
}
...
void mcfp_hw_dump(struct pablo_mmio *pmio, u32 mode)
{
	switch (mode) {
	case HW_DUMP_CR:
		info_mcfp_ver("DUMP CR\n");
		is_hw_dump_regs(pmio_get_base(pmio), mcfp_regs, MCFP_REG_CNT);
		break;
	case HW_DUMP_DBG_STATE:
		info_mcfp_ver("DUMP DBG_STATE\n");
		mcfp_hw_dump_dbg_state(pmio);
		break;
	default:
		err_mcfp("%s:Not supported dump_mode %d", __FILENAME__, mode);
		break;
	}
}
	...
	struct is_mcfp_config *config)
{
	...
	case MCFP_RDMA_PREV_IN0_UV:
		...
		if (dma->id == MCFP_RDMA_PREV_IN0_Y) {
	...
	*sbwc_en = comp_sbwc_en = is_hw_dma_get_comp_sbwc_en(sbwc_type, &comp_64b_align);
	...
	if (comp_sbwc_en == 0) {
		...
		header_stride_1p = is_hw_dma_get_header_stride(full_dma_width, MCFP_COMP_BLOCK_WIDTH, 16);
		if (strip_enable && strip_offset_in_pixel) {
	...
	struct is_mcfp_config *config)
{
	...
	*header_offset = strip_header_offset_in_byte;
	...
	dbg_hw(3, "comp_sbwc_en %d, pixelsize %d, comp_64b_align %d, quality_control %d, \
		stride_1p %d, header_stride_1p %d\n",
		comp_sbwc_en, pixelsize, comp_64b_align, quality_control,
	...
	u32 header_offset)
{
	...
	if ((comp_sbwc_en == 1) || (comp_sbwc_en == 2)) {
		...
		case MCFP_RDMA_PREV_IN0_Y:
			for (i = 0; i < num_buffers; i++)
				hdr_addr[i] = (dma_addr_t)*(addr + (2 * i)) + payload_size + header_offset;
			break;
		case MCFP_RDMA_CUR_IN_UV:
		case MCFP_RDMA_PREV_IN0_UV:
			for (i = 0; i < num_buffers; i++)
				hdr_addr[i] = (dma_addr_t)*(addr + (2 * i + 1)) + payload_size + header_offset;
			break;
		default:
			break;
		}
...
void mcfp_hw_s_otf_output(struct pablo_mmio *base, u32 set_id, u32 enable)
{
	...
	MCFP_SET_F(base, MCFP_R_YUV_COUTFIFO_AUTO_VBLANK,
		MCFP_F_YUV_COUTFIFO_AUTO_VBLANK, 0);
		...
		struct mcfp_radial_cfg *radial_cfg, struct is_mcfp_config *mcfp_config)
{
	...
	binning_x = radial_cfg->sensor_binning_x * radial_cfg->bns_binning_x * radial_cfg->sw_binning_x *
			1024ULL * radial_cfg->taa_crop_width / frame_width / 1000 / 1000 / 1000;
	binning_y = radial_cfg->sensor_binning_y * radial_cfg->bns_binning_y * radial_cfg->sw_binning_y *
			1024ULL * radial_cfg->taa_crop_height / height / 1000 / 1000 / 1000;
...
void mcfp_hw_s_crop_clean_img_otf(struct pablo_mmio *base, u32 set_id,
					...
					bool bypass)
{
	MCFP_SET_F(base, MCFP_R_YUV_CROPCLEANOTF_BYPASS,
			MCFP_F_YUV_CROPCLEANOTF_BYPASS, bypass);

	if (!bypass) {
		MCFP_SET_F(base, MCFP_R_YUV_CROPCLEANOTF_START,
				MCFP_F_YUV_CROPCLEANOTF_START_X, start_x);
		MCFP_SET_F(base, MCFP_R_YUV_CROPCLEANOTF_START,
				MCFP_F_YUV_CROPCLEANOTF_START_Y, 0);
		MCFP_SET_F(base, MCFP_R_YUV_CROPCLEANOTF_SIZE,
				MCFP_F_YUV_CROPCLEANOTF_SIZE_X, width);
		MCFP_SET_F(base, MCFP_R_YUV_CROPCLEANOTF_SIZE,
				MCFP_F_YUV_CROPCLEANOTF_SIZE_Y, height);
	}
}
...
void mcfp_hw_s_crop_wgt_otf(struct pablo_mmio *base, u32 set_id,
				...
				bool bypass)
{
	MCFP_SET_F(base, MCFP_R_YUV_CROPWEIGHTOTF_BYPASS, MCFP_F_YUV_CROPWEIGHTOTF_BYPASS, bypass);

	if (!bypass) {
		MCFP_SET_F(base, MCFP_R_YUV_CROPWEIGHTOTF_START,
				MCFP_F_YUV_CROPWEIGHTOTF_START_X, start_x);
		MCFP_SET_F(base, MCFP_R_YUV_CROPWEIGHTOTF_START,
				MCFP_F_YUV_CROPWEIGHTOTF_START_Y, 0);
		MCFP_SET_F(base, MCFP_R_YUV_CROPWEIGHTOTF_SIZE,
				MCFP_F_YUV_CROPWEIGHTOTF_SIZE_X, width);
		MCFP_SET_F(base, MCFP_R_YUV_CROPWEIGHTOTF_SIZE,
				MCFP_F_YUV_CROPWEIGHTOTF_SIZE_Y, height);
	}
}
...
void mcfp_hw_s_mixer_stitch_drc_map_width(struct pablo_mmio *base, u32 set_id, u32 width, u32 height)
{
	MCFP_SET_F(base, MCFP_R_YUV_MIXER_STITCH_DRC_MAP_WIDTH,
			MCFP_F_YUV_MIXER_STITCH_DRC_MAP_WIDTH, width);
	MCFP_SET_F(base, MCFP_R_YUV_MIXER_STITCH_DRC_MAP_WIDTH,
			MCFP_F_YUV_MIXER_STITCH_DRC_MAP_HEIGHT, height);
}
		...
		struct is_mcfp_config *mcfp_config, u32 width, u32 height)
{
	...
	dbg_hw(4, "%s: dma size: %dx%d, lme map: %dx%d, scale(%d,%d)", __func__,
			width, height,
			DIV_ROUND_UP(width, mv_width), DIV_ROUND_UP(height, mv_height),
			mcfp_config->motion_scale_x, mcfp_config->motion_scale_y);

	MCFP_SET_F(base, MCFP_R_YUV_MIXER_STITCH_LME_MAP_WIDTH,
			MCFP_F_YUV_MIXER_STITCH_LME_MAP_WIDTH, DIV_ROUND_UP(width, mv_width));
	MCFP_SET_F(base, MCFP_R_YUV_MIXER_STITCH_LME_MAP_WIDTH,
			MCFP_F_YUV_MIXER_STITCH_LME_MAP_HEIGHT, DIV_ROUND_UP(height, mv_height));
}
...
void mcfp_hw_s_crc(struct pablo_mmio *base, u32 seed)
{
	...
	MCFP_SET_F(base, MCFP_R_YUV_DTP_STREAM_CRC, MCFP_F_YUV_DTP_CRC_SEED, seed);
	...
	MCFP_SET_F(base, MCFP_R_RGB_GAMMARGB_CRC, MCFP_F_RGB_GAMMARGB_CRC_SEED, seed);
}
...
KUNIT_EXPORT_SYMBOL(mcfp_hw_s_strgen);
...
KUNIT_EXPORT_SYMBOL(mcfp_hw_pmio_write_test);
...
KUNIT_EXPORT_SYMBOL(mcfp_hw_pmio_read_test);
