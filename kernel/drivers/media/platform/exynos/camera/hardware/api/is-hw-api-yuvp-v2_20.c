...
#include "sfr/is-sfr-yuvp-mmio-v2_20.h"
#else
#include "sfr/is-sfr-yuvp-v2_20.h"
#endif
#include "pmio.h"
	...
	u32 biquad_scale_shift_adder)
{
	...
	dbg_hw(4, "[API] sw_binning_x %d, sw_binning_y %d\n",
...
int yuvp_hw_s_cnr_size(void *base, u32 set_id, u32 yuvpp_strip_start_pos, u32 frame_width,
	u32 dma_width, u32 dma_height, u32 strip_enable, struct yuvp_radial_cfg *radial_cfg)
{
	u32 offset_x, offset_y, xbin, ybin, cropx, cropy;

	dbg_hw(4, "[API] %s\n", __func__);

	xbin = radial_cfg->sensor_binning_x * radial_cfg->bns_binning_x * 1024ULL *
			radial_cfg->taa_crop_width / frame_width / 1000 / 1000;
	ybin = radial_cfg->sensor_binning_y * radial_cfg->bns_binning_y * 1024ULL *
	...
	cropx = radial_cfg->sensor_binning_x * offset_x / 1000;
	cropy = radial_cfg->sensor_binning_y * offset_y / 1000;

	dbg_hw(4, "[API] img_w %d, img_h %d\n",
		radial_cfg->sensor_full_width, radial_cfg->sensor_full_height);
	dbg_hw(4, "[API] cropx %d, cropy %d\n", cropx, cropy);
	dbg_hw(4, "[API] xbin %d, ybin %d\n", xbin, ybin);

	YUVP_SET_R(base + GET_COREX_OFFSET(set_id), YUVP_R_YUV_CNR_X_IMAGE_SIZE,
		radial_cfg->sensor_full_width);
	YUVP_SET_R(base + GET_COREX_OFFSET(set_id), YUVP_R_YUV_CNR_Y_IMAGE_SIZE,
		radial_cfg->sensor_full_height);
	YUVP_SET_R(base + GET_COREX_OFFSET(set_id), YUVP_R_YUV_CNR_XBIN, xbin);
	YUVP_SET_R(base + GET_COREX_OFFSET(set_id), YUVP_R_YUV_CNR_YBIN, ybin);

	YUVP_SET_R(base + GET_COREX_OFFSET(set_id), YUVP_R_YUV_CNR_CROPX, cropx);
	YUVP_SET_R(base + GET_COREX_OFFSET(set_id), YUVP_R_YUV_CNR_CROPY, cropy);
...
{
	YUVP_SET_F(base, YUVP_R_YUV_CINFIFO0_STREAM_CRC, YUVP_F_YUV_CINFIFO0_CRC_SEED, seed);
	YUVP_SET_F(base, YUVP_R_YUV_DTP_STREAM_CRC, YUVP_F_YUV_DTP_CRC_SEED, seed);
	YUVP_SET_F(base, YUVP_R_YUV_YUVNR_STREAM_CRC, YUVP_F_YUV_YUVNR_CRC_SEED, seed);
	...
	YUVP_SET_F(base, YUVP_R_YUV_SHARPENHANCER_STREAM_CRC, YUVP_F_YUV_SHARPENHANCER_CRC_SEED,
		   seed);
	...
	YUVP_SET_F(base, YUVP_R_RGB_SVHIST_STREAM_CRC, YUVP_F_RGB_SVHIST_CRC_SEED, seed);
	YUVP_SET_F(base, YUVP_R_YUV_CNR_STREAM_CRC, YUVP_F_YUV_CNR_CRC_SEED, seed);
