...
static void rgbp_hw_s_mono_mode(struct pablo_mmio *base, u32 set_id, bool enable)
{
	RGBP_SET_R(base, GET_COREX_OFFSET(set_id) + RGBP_R_BYR_DMSC_OP_MODE, enable);
	RGBP_SET_R(base, GET_COREX_OFFSET(set_id) + RGBP_R_RGB_RGBTOYUV_BYPASS, enable);
	RGBP_SET_R(base, GET_COREX_OFFSET(set_id) + RGBP_R_YUV_YUV444TO422_ISP_BYPASS, enable);
}
	...
	struct is_rgbp_config *config, struct rgbp_param_set *param_set)
{
	...
	rgbp_hw_s_mono_mode(base, set_id, mono_mode_en);
	...
	*sbwc_en = comp_sbwc_en = is_hw_dma_get_comp_sbwc_en(sbwc_type, &comp_64b_align);
	...
	case RGBP_RDMA_RGB_EVEN:
		...
		if (memory_bitwidth == DMA_INOUT_BIT_WIDTH_32BIT)
			stride_1p = 16 * DIV_ROUND_UP(width, 4);
		else
			stride_1p = 16 * DIV_ROUND_UP(width, 2);
		break;
	default:
		err_hw("[RGBP] invalid dma_id[%d]", dma->id);
		return SET_ERROR;
	...
	struct rgbp_param_set *param_set, pdma_addr_t *output_dva, struct rgbp_dma_cfg *dma_cfg)
{
	...
	if (dma->id == RGBP_WDMA_RGB_EVEN) {
		ret |= rgbp_hw_s_rgb_wdma_format(base, format);
		if (memory_bitwidth == DMA_INOUT_BIT_WIDTH_32BIT)
			stride_1p = 16 * DIV_ROUND_UP(width, 4);
		else
			stride_1p = 16 * DIV_ROUND_UP(width, 2);
