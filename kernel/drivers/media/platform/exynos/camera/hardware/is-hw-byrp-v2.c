	...
	struct byrp_param_set *param_set, u32 instance, u32 set_id)
{
	...
	bit_out = 14;
	byrp_hw_s_bitmask(get_base(hw_ip), set_id, bit_in, bit_out);
	...
	ulong hw_map)
{
	...
	if (IS_ENABLED(SKIP_ISP_SHOT_FOR_MULTI_SHOT)) {
		if (hw_byrp->repeat_instance != instance)
			hw_byrp->repeat_state = 0;

		if (batch_num > 1 || strip_total_count > 1 || strip_repeat_num > 1)
			hw_byrp->repeat_state++;
		else
			hw_byrp->repeat_state = 0;

		if (hw_byrp->repeat_state > 1 &&
		...
		hw_byrp->repeat_instance = instance;
	}
	...
	if (unlikely(test_bit(BYRP_DBG_DTP, &debug_byrp)))
		byrp_hw_s_dtp(get_base(hw_ip), set_id, true, dma_crop_cfg.w, dma_crop_cfg.h);
	...
	if (unlikely(test_bit(BYRP_DBG_DUMP_REG, &debug_byrp)))
		byrp_hw_dump(hw_ip->pmio, HW_DUMP_CR);
...
static int is_hw_byrp_notify_timeout(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	print_hex_dump(KERN_INFO, "BYRP config:", DUMP_PREFIX_OFFSET, 16, 1,
			(u32 *)&hw_byrp->config, sizeof(struct is_byrp_config), true);
