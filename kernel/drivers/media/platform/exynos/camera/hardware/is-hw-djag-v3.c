...
static const struct predjag_scaling_ratio_depended_config init_predjag_cfgs = {
	.xfilter_dejagging_coeff_cfg = {
		.xfilter_dejagging_weight0 = 3,
		.xfilter_dejagging_weight1 = 4,
		...
		u32 instance)
{
	...
	const struct predjag_scaling_ratio_depended_config *predjag_tuneset;
	...
	bool djag_en = true, predjag_en = false;
	...
	predjag_tuneset = &init_predjag_cfgs;
...
/* Read the djag_en from setfile */
	...
	predjag_en = hw_mcsc->cur_setfile[sensor_position][instance]->predjag.djag_en;
#endif
#endif
	...
	if (param->output[MCSC_INPUT_HF].dma_cmd == DMA_OUTPUT_COMMAND_ENABLE
		|| param->output[MCSC_INPUT_HF].otf_cmd == OTF_INPUT_COMMAND_ENABLE) {
	...
	if (out_width > MCSC_LINE_BUF_SIZE) {
		...
		u32 tmp_height = ALIGN_DOWN(GET_SCALED_SIZE(out_height,
					GET_ZOOM_RATIO(out_width, MCSC_LINE_BUF_SIZE)), MCSC_HEIGHT_ALIGN);
		sdbg_hw(4," DJAG output size exceeds line buffer size(%d > %d), change (%dx%d) -> (%dx%d)", hw_ip,
			out_width, MCSC_LINE_BUF_SIZE,
			out_width, out_height, MCSC_LINE_BUF_SIZE, tmp_height);
		out_width = MCSC_LINE_BUF_SIZE;
	...
	is_scaler_set_pre_enable(get_base(hw_ip), MCSC_PRE_SC_SET_SKIP, predjag_en);
	...
	if (in_width > DJAG_PRE_LINE_BUF_SIZE) {
		 ...
		 */
		sdbg_hw(4, "DJAG Pre input size exceeds Pre line buffer size(%d > %d), bypass to Post DJAG\n", hw_ip,
				in_width, DJAG_PRE_LINE_BUF_SIZE);

		is_scaler_set_pre_enable(get_base(hw_ip), 0, 0);
		 ...
		 */
		if (param->output[MCSC_INPUT_HF].dma_cmd == DMA_OUTPUT_COMMAND_DISABLE &&
		    param->output[MCSC_INPUT_HF].otf_cmd == OTF_INPUT_COMMAND_DISABLE) {
			if (!use_out_crop)
				is_scaler_check_predjag_condition(get_base(hw_ip), in_width,
								  in_height, &out_width,
								  &out_height);
		}
	}
	...
	is_scaler_set_djag_scaling_ratio(get_base(hw_ip), min_ratio, min_ratio);
...
#if IS_ENABLED(SUPPORT_DJAG_PRE_FILTER)
	predjag_tuneset = &hw_mcsc->cur_setfile[sensor_position][instance]
				   ->predjag.scaling_ratio_depended_cfg[scale_index];
#endif
	...
	is_scaler_set_predjag_tuning_param(get_base(hw_ip), predjag_tuneset);
#if defined(USE_YUV_RANGE_BY_ISP)
	...
	is_scaler_set_predjag_dither_wb(get_base(hw_ip), djag_wb, white, black);
