...
struct is_interface {
	...
	void				*core;
	const struct pablo_interface_ops *ops;
};

struct pablo_interface_ops {
	int (*open)(struct is_interface *device);
	int (*close)(struct is_interface *device);
};

#define CALL_INTERFACE_OPS(this, op, args...)                                                      \
	(((this)->ops && (this)->ops->op) ? ((this)->ops->op(this, ##args)) : 0)
