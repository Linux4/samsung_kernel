...
#define INVALID_INTERNAL_TYPE(type) ((type) == VC_NOTHING || (type) == VC_PRIVATE)
...
static void csis_flush_vc_multibuf(struct is_device_csi *csi, u32 idx_wdma, u32 wdma_vc)
{
	...
	subdev = csi->dma_subdev[idx_wdma][wdma_vc];

	if (!subdev || !test_bit(IS_SUBDEV_START, &subdev->state) ||
		!test_bit(IS_SUBDEV_INTERNAL_USE, &subdev->state))
		return;

	if (INVALID_INTERNAL_TYPE(csi->sensor_cfg->output[idx_wdma][wdma_vc].type))
		return;
...
static void csis_s_vc_dma_frobuf(struct is_device_csi *csi, u32 idx_wdma, u32 wdma_vc)
{
	...
	for (b_i = 0; b_i < csi->dma_batch_num; b_i++) {
		...
		if (wdma_vc == DMA_VIRTUAL_CH_0 && (dma_subdev->sbwc_type & COMP)) {
...
static inline dma_addr_t csi_get_dma_header_addr(struct is_device_csi *csi,
						 struct is_subdev *dma_subdev, dma_addr_t dvaddr,
						 u32 idx_wdma, u32 wdma_vc)
{
	struct is_device_sensor *sensor;
				  ...
				  u32 wdma_vc)
{
	...
	do {
		...
		if (wdma_vc == DMA_VIRTUAL_CH_0) {
			dvaddr =
				csi_get_dma_header_addr(csi, dma_subdev, dvaddr, idx_wdma, wdma_vc);
			csi_hw_s_dma_header_addr(wdma->regs_ctl, wdma->regs_vc[wdma_vc],
						 i + frameptr, dvaddr);
		}
	...
	for (idx_wdma = 0; idx_wdma < WDMA_MAX(csi); idx_wdma++) {
		...
		for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
			...
			if (test_bit(IS_SUBDEV_INTERNAL_USE, &dma_subdev->state) &&
			    !IS_CSI_VC_FRO_OUT(csi, idx_wdma, wdma_vc)) {
				if (INVALID_INTERNAL_TYPE(vci_config->type))
					continue;
...
#if defined(CSIWDMA_VC1_FORMAT_WA)
				if (wdma_vc == DMA_VIRTUAL_CH_1 && vc0_pfmt)
				...
				if (wdma_vc == DMA_VIRTUAL_CH_0) {
					...
					*/
					...
					csi_hw_s_config_dma(wdma->regs_ctl,
							    wdma->regs_vc[DMA_VIRTUAL_CH_1],
							    DMA_VIRTUAL_CH_1, &framecfg,
							    vci_config->extformat,
							    &vci_config->stride);
					mcinfo("[VC1] DMA format(%x) (size: %d, %d)\n", csi,
					       vci_config->extformat, framecfg.width,
					       framecfg.height);
				} else if ((wdma_vc == DMA_VIRTUAL_CH_1) &&
					   (vci_config->extformat !=
					    csi->sensor_cfg->output[idx_wdma][DMA_VIRTUAL_CH_0]
						    .extformat)) {
					err("vc[%d] DMA format should be the same as VC0 (%x, %x)",
					    wdma_vc,
					    csi->sensor_cfg->output[idx_wdma][DMA_VIRTUAL_CH_0]
						    .extformat,
					    csi->sensor_cfg->output[idx_wdma][DMA_VIRTUAL_CH_1]
						    .extformat);
				}
			}
#endif
			...
			struct is_framemgr *framemgr, u32 idx_wdma, u32 wdma_vc)
{
	...
	if (!dma_subdev) {
		mcerr("[WDMA%d][VC%d] could not get DMA sub-device", csi, wdma->ch, wdma_vc);
		return;
	}
		...
		if ((csi->sensor_cfg->output[idx_wdma][wdma_vc].type == VC_EMBEDDED) ||
			...
			if (frameptr < framemgr->num_frames) {
				...
				CALL_BUFOP(dma_subdev->pb_subdev[frame->index], sync_for_cpu,
					   dma_subdev->pb_subdev[frame->index], 0,
...
static void csi_err_handle(struct is_device_csi *csi)
{
	...
	for (link_vc = CSI_VIRTUAL_CH_0; link_vc < CSI_VIRTUAL_CH_MAX; link_vc++) {
		...
		for (link_vc_out_idx = 0;
			...
			dma_subdev = csi->dma_subdev[idx_wdma][wdma_vc];
			if (dma_subdev && test_bit(IS_SUBDEV_OPEN, &dma_subdev->state)) {
				csis_flush_vc_buf_done(csi, idx_wdma, wdma_vc, FS_PROCESS,
						       VB2_BUF_STATE_ERROR);
				csis_flush_vc_multibuf(csi, idx_wdma, wdma_vc);
				err("[F%d][VC%d] frame was done with error",
				    atomic_read(&csi->fcount), link_vc);
	...
	for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
		...
		if (dma_frame_str & (1 << wdma_vc)) {
			...
			if (!test_bit(IS_SUBDEV_OPEN, &dma_subdev->state) &&
				!IS_ENABLED(CONFIG_SENSOR_GROUP_LVN)) {
				continue;
	...
	if (irq_src.err_flag) {
		for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
			if (irq_src.err_id[idx_wdma][wdma_vc] & ~(1 << CSIS_ERR_DMA_ABORT_DONE))
				...
				if (csi->otf_info.csi_ch == 1 || csi->otf_info.csi_ch == 3)
...
int is_csi_close(struct v4l2_subdev *subdev)
{
	...
	for (idx_wdma = 0; idx_wdma < CSIS_MAX_NUM_DMA_ATTACH; idx_wdma++) {
		for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
			...
			if (ret)
				mcerr("[WDMA%d][VC%d] is_subdev_internal_close is fail(%d)", csi,
				      csi->wdma[idx_wdma]->ch, wdma_vc, ret);
		}
	}
...
{
	int ret = 0;
	u32 settle, otf_ch, otf_out_id, link_vc;
	...
	mcinfo(" settle(%dx%d@%d)=%d, speed(%u%s), lane(%u), dma_num(%u), potf(%d), hw_fcount(%d)\n",
	       csi, csi->image.window.width, csi->image.window.height, sensor_cfg->framerate,
	       settle, sensor_cfg->mipi_speed, csi->use_cphy ? "Msps" : "Mbps",
	       sensor_cfg->lanes + 1, sensor_cfg->dma_num, csi->potf, csi->hw_fcount);
	...
	if (enable != IS_ENABLE_LINK_ONLY && !IS_ENABLED(CONFIG_CAMERA_CIS_ZEBU_OBJ)) {
		...
		if (!(IS_ENABLED(SECURE_CAMERA_FACE) && IS_ENABLED(PROTECT_SYSREG_IS) &&
			...
			ret = csi_hw_s_phy_set(csi->phy, sensor_cfg->lanes, sensor_cfg->mipi_speed,
					       settle, otf_info->csi_ch, csi->use_cphy, phy_sf_tbl,
...
static int csi_dma_set_internal_subdev(struct is_device_csi *csi, struct is_subdev *dma_subdev,
	struct is_vci_config *vci_cfg, u32 link_vc, u32 max_target_fps, u32 vid)
{
	int ret;
	...
	ret = is_subdev_internal_open(csi->instance, vid, dma_subdev);
	if (ret) {
		merr("[CSI%d][VC%d] is_subdev_internal_open is fail(%d)", csi, csi->otf_info.csi_ch,
			link_vc, ret);
		return -EINVAL;
	}
	...
	ret = is_subdev_internal_s_format(dma_subdev, w, h, bits_per_pixel, sbwc_type,
		lossy_byte32num, buffer_num, type_name);
	if (ret) {
		merr("[CSI%d][VC%d] is_subdev_internal_s_format is fail(%d)", csi,
			csi->otf_info.csi_ch, link_vc, ret);
		return -EINVAL;
	}
	...
	for (idx_wdma = 0; idx_wdma < WDMA_MAX(csi); idx_wdma++) {
		for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
			...
			if (test_bit(IS_SUBDEV_INTERNAL_USE, &dma_subdev->state)) {
				ret = is_subdev_internal_close(dma_subdev);
				if (ret)
					mcerr("[VC%d] is_subdev_internal_close is fail(%d)", csi,
					      wdma_vc, ret);
			}
			...
			ret = csi_dma_set_internal_subdev(csi, dma_subdev, vci_cfg, link_vc,
				device->max_target_fps, device->video.id);
			if (ret)
				return ret;
		}
	}
...
static int csi_s_buffer(struct v4l2_subdev *subdev, void *buf, unsigned int *vcp)
{
	...
	if (IS_ENABLED(CONFIG_SENSOR_GROUP_LVN)) {
		...
		for (link_vc_out_idx = 0;
			...
			dma_subdev = csi->dma_subdev[idx_wdma][wdma_vc];
			if (!dma_subdev) {
				err("s_buffer for invalid link vc (%d)", link_vc);
				return -EINVAL;
			}
