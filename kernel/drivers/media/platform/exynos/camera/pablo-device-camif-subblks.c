...
static void pablo_camif_csis_pdp_top_enable_ibuf_ptrn_gen(struct pablo_camif_csis_pdp_top *top,
	struct is_sensor_cfg *sensor_cfg, struct pablo_camif_otf_info *otf_info, u32 clk, bool on)
{
	csis_pdp_top_enable_ibuf_ptrn_gen(top->regs, sensor_cfg, otf_info, clk, on);
}
...
static irqreturn_t pablo_camif_csis_pdp_top_isr(int irq, void *data)
{
	...
	if (test_bit(IBUF_ERR_OVERFLOW, &ret)) {
		warn("IBUF OVERFLOW");
		is_debug_event_count(IS_EVENT_OVERFLOW_IBUF, 0, 0);
	}

	if (CHECK_IBUF_ERR_WRONGSIZE(ret)) {
		warn("IBUF WRONGSIZE");
		pablo_csis_pdp_top->err_status = ret;
	}
