...
{
#if IS_ENABLED(CONFIG_CSIS_V5_4_8)
	is_hw_set_field(base_reg, &csi_mcb_cmn_regs[CSIS_MCB_CMN_R_SW_RESET_CTRL],
			&csi_mcb_cmn_fields[CSIS_MCB_CMN_F_MCB_FIFO_SW_RESET], 1 << ch);
#endif
...
{
#if IS_ENABLED(CONFIG_CSIS_V5_4_8) && IS_ENABLED(USE_POTF_VC)
	is_hw_set_reg(base_reg, &csi_mcb_cmn_regs[CSIS_MCB_CMN_R_PDP_POTF_CTRL], 1);
	info("[CSIS_TOP] PDP POTF_AF_EN is enabled\n");
#endif
}
