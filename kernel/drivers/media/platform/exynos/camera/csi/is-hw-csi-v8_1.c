...
static int _csi_hw_s_dma_input_mux(u32 wdma_index, int otf_ch, bool mux_en,
		bool bns_en, u32 bns_mux_val)
{
	struct pablo_smc_ops *psmc_ops = pablo_get_smc_ops();
	...
	if (ret)
		err("[SMC] WDMA%d mux setting error. addr 0x%x val %d ret %d\n",
				wdma_index, mux_addr[wdma_index], mux_val, ret);
...
{
	_csi_hw_s_dma_input_mux(wdma_index, otf_ch, true, bns_en, bns_dma_mux_val);
}

void csi_hw_s_init_input_mux(void __iomem *mux_regs, u32 idx_wdma, u32 index)
{
	_csi_hw_s_dma_input_mux(index, 0, false, false, 0);
}
