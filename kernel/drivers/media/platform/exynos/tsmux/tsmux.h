...
struct tsmux_buffer {
	...
	int64_t blending_start_stamp;
	int64_t blending_end_stamp;
...
struct tsmux_otf_cmd_queue {
	...
	// max frame skipping count
	int64_t max_otf_job_done_wait_time_us;
};
...
#define TSMUX_IOCTL_GET_LOG_INFO	\
	_IOR('A', 0x22, char *)
...
#define TSMUX_IOCTL_SFR_DUMP_MAP_BUF \
	_IOW('A', 0x50, int32_t)
#define TSMUX_IOCTL_SFR_DUMP_UNMAP_BUF \
	_IO('A', 0x51)
