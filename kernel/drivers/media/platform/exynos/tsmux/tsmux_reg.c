...
{
	print_tsmux(TSMUX_INFO, "enable_partial_done: %d, rtp_pkt_count_per_partial_data: %d\n",
...
void tsmux_print_cmu_mfc_sfr(struct tsmux_device *tsmux_dev) {
	...
	print_tsmux(TSMUX_DBG_SFR, "tsmux_cmu_mfc_sfr_list_size %d\n", tsmux_cmu_mfc_sfr_list_size);
	print_tsmux(TSMUX_DBG_SFR, "cmu_mfc base addr: offset: value, name\n");

	for (i = 0; i < tsmux_cmu_mfc_sfr_list_size; i++) {
		cmu_mfc_sfr = TSMUX_CMU_MFC_READL(tsmux_cmu_mfc_sfr_list[i].offset);
		print_tsmux(TSMUX_DBG_SFR, "%.8llx: %.8x: %.8x, %s\n",
...
static inline void tsmux_print_tsmux_sfr_range(
	struct tsmux_device *tsmux_dev, u32 start, u32 end, bool dump)
{
	...
	for (i = start; i <= end; i += 16) {
		...
		print_tsmux(TSMUX_DBG_SFR, "%.8x: %.8x %.8x %.8x %.8x\n",
...
void tsmux_print_tsmux_sfr(struct tsmux_device *tsmux_dev)
{
	...
	tsmux_print_tsmux_sfr_range(tsmux_dev, TSMUX_PKT_CTRL_ADDR,
		TSMUX_LH_CTRL_ADDR, true /* dump */);
	tsmux_print_tsmux_sfr_range(tsmux_dev, TSMUX_SHD_PKT_CTRL_ADDR,
	...
	struct tsmux_psi_info *psi_info)
{
	...
	print_tsmux(TSMUX_DBG_SFR, "pcr_len %d, pmt_len %d, pat_len %d\n",
	...
	print_tsmux(TSMUX_DBG_SFR, "psi_len_reg 0x%x\n", psi_len_reg);
	...
	for (i = 0; i < TSMUX_PSI_SIZE; i++) {
		print_tsmux(TSMUX_DBG_SFR, "psi_data[%d] %u\n", i,
			psi_info->psi_data[i]);
	...
	struct tsmux_pkt_ctrl *pkt_ctrl)
{
	...
	print_tsmux(TSMUX_DBG_SFR, "pkt_ctrl_reg 0x%x\n", pkt_ctrl_reg);

	pkt_ctrl_reg &= ~(TSMUX_PKT_CTRL_PSI_EN_MASK);
	pkt_ctrl_reg |= TSMUX_PKT_CTRL_PSI_EN_MASK &
		(pkt_ctrl->psi_en << TSMUX_PKT_CTRL_PSI_EN_SHIFT);
	print_tsmux(TSMUX_DBG_SFR, "PKT_CTRL_PSI_EN %d\n", pkt_ctrl->psi_en);
	print_tsmux(TSMUX_DBG_SFR, "pkt_ctrl_reg 0x%x\n", pkt_ctrl_reg);
	...
	print_tsmux(TSMUX_DBG_SFR, "PKT_CTRL_RTP_SEQ_OVER %d\n",
		pkt_ctrl->rtp_seq_override);
	print_tsmux(TSMUX_DBG_SFR, "pkt_ctrl_reg 0x%x\n", pkt_ctrl_reg);
	...
	print_tsmux(TSMUX_DBG_SFR, "TSMUX_PKT_CTRL_PES_STUF %d\n",
		pkt_ctrl->pes_stuffing_num);
	print_tsmux(TSMUX_DBG_SFR, "pkt_ctrl_reg 0x%x\n", pkt_ctrl_reg);
	...
	print_tsmux(TSMUX_DBG_SFR, "PKT_CTRL_MODE %d\n", pkt_ctrl->mode);
	print_tsmux(TSMUX_DBG_SFR, "pkt_ctrl_reg 0x%x\n", pkt_ctrl_reg);

	pkt_ctrl_reg &= ~(TSMUX_PKT_CTRL_ID_MASK);
	pkt_ctrl_reg |= TSMUX_PKT_CTRL_ID_MASK &
		(pkt_ctrl->id << TSMUX_PKT_CTRL_ID_SHIFT);
	print_tsmux(TSMUX_DBG_SFR, "TSMUX_PKT_CTRL_ID %d\n", pkt_ctrl->id);
	print_tsmux(TSMUX_DBG_SFR, "pkt_ctrl_reg 0x%x\n", pkt_ctrl_reg);
	...
	struct tsmux_swp_ctrl *swp_ctrl)
{
	...
	print_tsmux(TSMUX_DBG_SFR, "swap_ctrl_reg 0x%x\n", swap_ctrl_reg);
	...
	struct tsmux_hex_ctrl *hex_ctrl)
{
	...
	if (ctx->set_hex_info) {
		...
		if (ret)
			print_tsmux(TSMUX_ERR, "exynos_smc ret %lu\n", ret);
		print_tsmux(TSMUX_DBG_SFR, "TSMUX_HEX_OTF_CTRL %u\n", hex_ctrl->otf_enable);
	...
	struct tsmux_buffer_info *buf_info)
{
	...
	print_tsmux(TSMUX_DBG_SFR, "src_addr 0x%llx\n", buf_info->dma_addr);
...
void tsmux_set_src_len(struct tsmux_device *tsmux_dev, int src_len)
{
	...
	print_tsmux(TSMUX_DBG_SFR, "src_len %d\n", src_len);
	...
	struct tsmux_buffer_info *buf_info)
{
	...
	print_tsmux(TSMUX_DBG_SFR, "dst_addr 0x%llx\n", buf_info->dma_addr);
...
void tsmux_enable_int_job_done(struct tsmux_device *tsmux_dev, bool need_partial_done)
{
	...
	if (need_partial_done && g_enable_partial_done) {
		...
		print_tsmux(TSMUX_DBG_SFR, "read part_th 0x%x\n", partial_data_size);
	} else {
		int_en_reg &= ~(TSMUX_INT_EN_PART_DONE);
	}
	TSMUX_WRITEL(int_en_reg, TSMUX_INT_EN_ADDR);
	print_tsmux(TSMUX_DBG_SFR, "write int_en_reg 0x%x\n", int_en_reg);
	int_en_reg = TSMUX_READL(TSMUX_INT_EN_ADDR);
	print_tsmux(TSMUX_DBG_SFR, "read int_en_reg 0x%x\n", int_en_reg);
...
void tsmux_set_cmd_ctrl(struct tsmux_device *tsmux_dev, int mode)
{
	...
	cmd_ctrl_reg = TSMUX_READL(TSMUX_CMD_CTRL_ADDR);
	print_tsmux(TSMUX_DBG_SFR, "read cmd_ctrl_reg 0x%x\n", cmd_ctrl_reg);
...
bool tsmux_check_interrupt_stat(struct tsmux_device *tsmux_dev, int job_id, bool partial)
{
	...
	print_tsmux(TSMUX_DBG_SFR, "job_id %d, read int_stat_reg 0x%x\n", job_id, int_stat_reg);
...
void tsmux_clear_job_done(struct tsmux_device *tsmux_dev, int job_id)
{
	...
	int_stat_reg = TSMUX_READL(TSMUX_INT_STAT_ADDR);
	print_tsmux(TSMUX_DBG_SFR, "read int_stat_reg 0x%x\n", int_stat_reg);
...
void tsmux_clear_partial_done(struct tsmux_device *tsmux_dev, u32 job_id)
{
	...
	print_tsmux(TSMUX_DBG_SFR, "write int_stat_reg 0x%x\n", int_stat_reg);
	TSMUX_WRITEL(int_stat_reg, TSMUX_INT_STAT_ADDR);
	int_stat_reg = TSMUX_READL(TSMUX_INT_STAT_ADDR);
	print_tsmux(TSMUX_DBG_SFR, "read int_stat_reg 0x%x\n", int_stat_reg);
...
int tsmux_get_dst_len(struct tsmux_device *tsmux_dev, int job_id)
{
	...
	print_tsmux(TSMUX_DBG_SFR, "read dst_len_reg 0x%x, job_id %d\n",
		dst_len_reg, job_id);
