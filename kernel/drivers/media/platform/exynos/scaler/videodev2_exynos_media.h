...
#define NV12N_CBCR_SIZE(w, h)		(__ALIGN_UP((__ALIGN_UP((w), 16) * (__ALIGN_UP((h), 16) / 2)), 16))
...
/* SBWC Lossy v2.8 64B align (footprint reduction mode) */
#define SBWCL_64_FR_STRIDE(w)			(64 * (((w) + 31) / 32))

#define SBWCL_64_FR_Y_SIZE(w, h)		(SBWCL_64_FR_STRIDE(w) * (((h) + 3) / 4))
#define SBWCL_64_FR_CBCR_SIZE(w, h)		(SBWCL_64_FR_STRIDE(w) * ((((h) / 2) + 3) / 4))
