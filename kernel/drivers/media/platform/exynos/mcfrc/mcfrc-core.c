...
static int mcfrc_run(struct mcfrc_ctx *mcfrc_context, struct mcfrc_task *task)
{
	...
	if (!CAVY_TEST_BIT(DBG_CONTROL_FLAG_2, task->user_task.dbg_control_flags)) {
		...
		if (CAVY_TEST_BIT(DBG_CONTROL_FLAG_4, task->user_task.dbg_control_flags)) {
			...
			SET_MC_INTR_MASKING_REG(mcfrc_device->regs, var);//         writel(val, base_addr+REG_TOP_0x0290_MC_INTR_MASKING)
			var = 0;
			GET_MC_INTR_MASKING_REG(mcfrc_device->regs, var);//         (val = readl(base_addr+REG_TOP_0x0290_MC_INTR_MASKING))
...
void mcfrc_task_schedule(struct mcfrc_task *task)
{
	...
	GET_MC_APB_PPROT_REG(mcfrc_device->regs, apb_status);
	if (apb_status == 0) {
		int iter = 0;

		dev_info(mcfrc_device->dev, "%s: TZPC is secure, wait for non-secure...\n", __func__);
		...
		if (apb_status == 0)
			...
			dev_info(mcfrc_device->dev, "%s: TZPC is non-secure\n", __func__);
	} else {
		dev_dbg(mcfrc_device->dev, "%s: TZPC is non-secure\n", __func__);
	}
...
//#endif
	...
	if (ret) {
		...
		mcfrc_print_all_regs(mcfrc_device, mcfrc_device->regs);
#endif
		ret_wait = wait_for_completion_timeout(&task->complete, tempJ);
		if (ret_wait == 0) {
			GET_MC_APB_PPROT_REG(mcfrc_device->regs, apb_status);
			dev_info(mcfrc_device->dev, "%s: wait completion time out!! tz pc is %d\n",
				__func__, apb_status);
			task->state = MCFRC_BUFSTATE_ERROR;
		}
	}
...
static int mcfrc_open(struct inode *inode, struct file *filp)
{
	...
	SET_FRC_MC_OP_MODE_SEL_REG(mcfrc_device->regs, 0);
	mcfrc_version = mcfrc_version & 0x10000000;

	disable_mcfrc(mcfrc_device);

	dev_info(mcfrc_device->dev, "%s: Open mcfrc context, version : %d\n"
		, __func__, mcfrc_version);
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case HWMCFRC_IOC_PROCESS:
	{
		...
		if (mcfrc_device->poll_srst) {
			dev_dbg(mcfrc_device->dev, "%s: polling reset failed, try again\n", __func__);
			return -EAGAIN;
		}
	...
	case HWMCFRC_IOC_PROCESS_DBG_02:
	{
		...
		//sw reset by polling
		{
			mcfrc_device->poll_srst = 0;
			enable_mcfrc(mcfrc_device);

			status = readl(base + REG_TOP_0x02A4_FRC_MC_DBL_ERR_IRQ_STATUS);
			dev_dbg(mcfrc_device->dev
				, "%s: REG_TOP_0x02A4_FRC_MC_DBL_ERR_IRQ_STATUS:(%x, %x) before polling rst\n", __func__, REG_TOP_0x02A4_FRC_MC_DBL_ERR_IRQ_STATUS, status);
			...
			SET_MC_SW_RESET_REG(base, 0x0);
			// wait ack
			while (ret < 5) {
				bool data8;
				//status = readl(base + REG_TOP_0x02A4_FRC_MC_DBL_ERR_IRQ_STATUS);
				GET_FRC_MC_DBL_ERR_IRQ_STATUS_REG(base, status);
				dev_dbg(mcfrc_device->dev
					, "%s: REG_TOP_0x02A4_FRC_MC_DBL_ERR_IRQ_STATUS:(%x, %x)\n", __func__, REG_TOP_0x02A4_FRC_MC_DBL_ERR_IRQ_STATUS, status);
			...
			if (ret < 5) {
				SET_MC_SW_RESET_REG(base, 0x1);
			} else {
				mcfrc_device->poll_srst = 1;
				ret = -EAGAIN;
			}
	...
	case HWMCFRC_IOC_PROCESS_DBG_01:
	{
		...
		//sw reset by interrupt
		{
			...
			*/
			...
			dev_dbg(mcfrc_device->dev
				, "%s: REG_TOP_0x02A4_FRC_MC_DBL_ERR_IRQ_STATUS:(%x, %x) before irq rst\n", __func__, REG_TOP_0x02A4_FRC_MC_DBL_ERR_IRQ_STATUS, status);
			...
			global_time_start = ktime_get();
#endif
			GET_MC_APB_PPROT_REG(base, apb_status);
			if (apb_status == 0) {
				int iter = 0;
	...
	default:
	{
		dev_err(mcfrc_device->dev, "%s: Unknown ioctl cmd %x, %lx\n",
	...
	unsigned int cmd, unsigned long arg)
{
	...
	default:
		dev_err(mcfrc_device->dev, "%s: Unknown compat_ioctl cmd %x, %lx\n",
...
static irqreturn_t mcfrc_irq_handler_done(int irq, void *priv)
{
	...
	dev_dbg(mcfrc_device->dev
		, "%s: REG_TOP_0x029C_FRC_MC_DONE_IRQ_STATUS:(%x, %x)\n", __func__, REG_TOP_0x029C_FRC_MC_DONE_IRQ_STATUS, status);
	...
	SET_FRC_MC_DONE_INTR_CLEAR_REG(mcfrc_device->regs, 0x1);//0x000C
...
static irqreturn_t mcfrc_irq_handler_err_srst(int irq, void *priv)
{
	...
	//status = readl(mcfrc->regs + REG_TOP_0x02A4_FRC_MC_DBL_ERR_IRQ_STATUS);
	...
	if (CAVY_TEST_BIT(IRQ_ERRRST_STATE_RST, status)) {
		...
		dev_dbg(mcfrc->dev, "%s: set reset register\n", __func__);
		...
		SET_FRC_MC_DBL_ERR_INTR_CLEAR_REG(mcfrc->regs, 0x1);
	} else {
#ifndef DEBUG
		var = 0;
		...
		var = 0;
		GET_MC_CORE_STATUS_2_REG(mcfrc->regs, var);
		dev_info(mcfrc->dev, "%s: MC_CORE_STATUS_2 after: 0x%08X \n", __func__, var);
		var = 0;
		GET_MC_CORE_STATUS_3_REG(mcfrc->regs, var);
		dev_info(mcfrc->dev, "%s: MC_CORE_STATUS_3 after: 0x%08X \n", __func__, var);
...
module_platform_driver(mcfrc_driver);
