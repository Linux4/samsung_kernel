...
{
	dev_dbg(dev, "%s: mapping dmabuf %pK, attachment %pK, DMA_TO_DEVICE? %d, DMA_FROM_DEVICE? %d\n"
	...
	if (plane->dmabuf) {
		...
		dev_dbg(dev, "%s: successfully got scatterlist %pK for dmabuf %pK, DMA_TO_DEVICE? %d, DMA_FROM_DEVICE? %d\n"
...
void mcfrc_unmap_dma_attachment(struct device *dev,
								...
								enum dma_data_direction dir)
{
	dev_dbg(dev, "%s: unmapping dmabuf %pK, attachment %pK, scatter table %pK, DMA_TO_DEVICE? %d, DMA_FROM_DEVICE? %d\n"
				...
				enum dma_data_direction dir)
{
	...
	if (plane->dmabuf) {
		dev_dbg(dev, "%s: getting dma addr of plane, DMA attachment %pK bytes %zu, DMA_TO_DEVICE? %d, DMA_FROM_DEVICE? %d\n"
