...
static unsigned long __mfc_rate_get_framerate_by_interval(int interval, int type)
{
	...
	} else if ((type == MFC_TS_SRC_Q) || type == MFC_TS_DST_DQ) {
		framerate = (1000000 / interval) * 1000;
		return (framerate > MFC_MAX_FPS) ? MFC_MAX_FPS : framerate;
	} else {
		return 0;
	}
		...
		struct mfc_ts_control *ts, struct timespec64 *time, int type)
{
	...
	min_interval = __mfc_rate_get_ts_interval(ctx, ts, type);
	max_framerate = __mfc_rate_get_framerate_by_interval(min_interval, type);
	...
	if (!ctx->src_ts.ts_is_full) {
		...
		if (ctx->rt == MFC_NON_RT) {
			if (framerate < ctx->src_q_framerate) {
				...
				if (framerate != ctx->framerate)
					mfc_ctx_debug(2, "[QoS][PRIO] NRT src_q fps %ld\n",
							framerate);
			}
		}

		/* 8) handle undefined mode as non-real-time */
		if (ctx->rt == MFC_RT_UNDEFINED) {
			if (framerate < ctx->dst_q_framerate) {
				framerate = ctx->dst_q_framerate;
				if (framerate != ctx->framerate)
					mfc_ctx_debug(2, "[QoS][PRIO] default NRT dst_q fps %ld\n",
							framerate);
			}
		}
