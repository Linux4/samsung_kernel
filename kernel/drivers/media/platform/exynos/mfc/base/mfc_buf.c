...
#if IS_ENABLED(CONFIG_EXYNOS_CONTENT_PATH_PROTECTION)
	if (core->fw.drm_status & MFC_CTX_ALLOC) {
...
int mfc_alloc_common_context(struct mfc_core *core, bool is_drm)
{
	int ret = 0;
...
static void __mfc_enc_calc_codec_buffer_size(struct mfc_core_ctx *core_ctx)
{
	...
	if ((ctx->min_dpb_size[0] > enc->luma_dpb_size) ||
		...
		mfc_debug(2, "[MEMINFO] recon DPB size changed (Luma: %zu -> %d, Chroma %zu -> %d)\n",
