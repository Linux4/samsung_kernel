		...
		struct mfc_ctx *ctx, struct mfc_buf_ctrl *buf_ctrl)
{
	...
	if (buf_ctrl->id
		...
		for (i = 0; i < (temporal_LC.temporal_layer_count & 0x7); i++) {
			mfc_ctx_debug(3, "[HIERARCHICAL] layer bitrate[%d] %d (FW ctrl: %d)\n",
		...
		if (IS_H264_ENC(ctx)) {
			...
			mfc_ctx_debug(3, "[HIERARCHICAL] EXTENSION0 %#x, EXTENSION1 %#x\n",
					value, value2);
		...
		struct mfc_ctx *ctx, struct mfc_buf_ctrl *buf_ctrl)
{
	...
	if (buf_ctrl->id == V4L2_CID_MPEG_VIDEO_DROP_CONTROL) {
		...
		if (ctx->src_ts.ts_last_interval)
			mfc_ctx_debug(3, "[DROPCTRL] fps %d -> %ld, delta: %d, reg: %#x\n",
				p->rc_framerate, USEC_PER_SEC / ctx->src_ts.ts_last_interval,
				p->rc_frame_delta, value);
		else
			mfc_ctx_debug(3, "[DROPCTRL] fps %d -> 0, delta: %d, reg: %#x\n",
		...
		struct mfc_ctx *ctx, struct list_head *head)
{
	...
	list_for_each_entry(buf_ctrl, head, list) {
		...
		mfc_ctx_debug(6, "[CTRLS] Set buffer control id: 0x%08x, val: %d (%#x)\n",
		...
		struct mfc_ctx *ctx, struct list_head *head)
{
	...
	list_for_each_entry(buf_ctrl, head, list) {
		...
		buf_ctrl->updated = 0;
		mfc_ctx_debug(6, "[CTRLS] Recover buffer control id: 0x%08x, old val: %d\n",
			...
			struct list_head *head, DecoderInputStr *pInStr)
{
	...
	list_for_each_entry(buf_ctrl, head, list) {
		...
		mfc_ctx_debug(6, "[NALQ][CTRLS] Set buffer control id: 0x%08x, val: %d (%#x)\n",
				buf_ctrl->id, buf_ctrl->val, buf_ctrl->val);
	}
			...
			struct list_head *head, DecoderOutputStr *pOutStr)
{
	...
	list_for_each_entry(buf_ctrl, head, list) {
		...
		mfc_ctx_debug(6, "[NALQ][CTRLS] Get buffer control id: 0x%08x, val: %d (%#x)\n",
				buf_ctrl->id, buf_ctrl->val, buf_ctrl->val);
	}
	...
	list_for_each_entry(buf_ctrl, head, list) {
		...
		case V4L2_CID_MPEG_VIDEO_HEVC_HIERARCHICAL_CODING_LAYER_CH:
			...
			if (temporal_LC.temporal_layer_count & 0x7)
				...
				pInStr->ParamChange &= ~(1 << 10);
			mfc_ctx_debug(3, "[NALQ][HIERARCHICAL] layer count %d\n",
			...
			for (i = 0; i < (temporal_LC.temporal_layer_count & 0x7); i++) {
				mfc_ctx_debug(3, "[NALQ][HIERARCHICAL] layer bitrate[%d] %d (FW ctrl: %d)\n",
					i, temporal_LC.temporal_layer_bitrate[i], p->hier_bitrate_ctrl);
			...
			if (IS_H264_ENC(ctx)) {
				...
				mfc_ctx_debug(3, "[NALQ][HIERARCHICAL] EXTENSION0 %#x, EXTENSION1 %#x\n",
		...
		case V4L2_CID_MPEG_VIDEO_ROI_CONTROL:
			...
			pInStr->RcRoiCtrl |=
				(buf_ctrl->val & buf_ctrl->mask) << buf_ctrl->shft;
			pInStr->RoiBufferAddr =
				MFC_NALQ_DMA_WRITEL(enc->roi_buf[buf_ctrl->old_val2].daddr);
			mfc_ctx_debug(3, "[NALQ][ROI] buffer[%d] addr %#llx, QP val: %#x\n",
					buf_ctrl->old_val2,
					enc->roi_buf[buf_ctrl->old_val2].daddr,
					buf_ctrl->val);
			break;
		...
		case V4L2_CID_MPEG_VIDEO_DROP_CONTROL:
			if (!ctx->src_ts.ts_last_interval) {
				...
				mfc_ctx_debug(3, "[NALQ][DROPCTRL] default delta: %d\n", p->rc_frame_delta);
			} else {
				/*
			...
			pInStr->RcFrameRate |=
				(p->rc_frame_delta & buf_ctrl->mask) << buf_ctrl->shft;
			if (ctx->src_ts.ts_last_interval)
				mfc_ctx_debug(3, "[NALQ][DROPCTRL] fps %d -> %ld, delta: %d, reg: %#x\n",
					p->rc_framerate, USEC_PER_SEC / ctx->src_ts.ts_last_interval,
					p->rc_frame_delta, pInStr->RcFrameRate);
			else
				mfc_ctx_debug(3, "[NALQ][DROPCTRL] fps %d -> 0, delta: %d, reg: %#x\n",
		...
		case V4L2_CID_MPEG_VIDEO_MAX_IFRAME_SIZE:
			pInStr->RcOptions &= ~(buf_ctrl->mask << buf_ctrl->shft);
			pInStr->RcOptions |=
				(buf_ctrl->val & buf_ctrl->mask) << buf_ctrl->shft;
			param_change = 1;
			break;
			...
			struct list_head *head, EncoderOutputStr *pOutStr)
{
	...
	list_for_each_entry(buf_ctrl, head, list) {
		...
		case V4L2_CID_MPEG_MFC51_VIDEO_LUMA_ADDR:
			value = MFC_NALQ_DMA_READL(pOutStr->EncodedFrameAddr[0]);
			break;
		case V4L2_CID_MPEG_MFC51_VIDEO_CHROMA_ADDR:
			value = MFC_NALQ_DMA_READL(pOutStr->EncodedFrameAddr[1]);
			break;
...
static int mfc_core_recover_buf_ctrls_nal_q(struct mfc_ctx *ctx, struct list_head *head)
{
	...
	list_for_each_entry(buf_ctrl, head, list) {
		...
		buf_ctrl->updated = 0;
		mfc_ctx_debug(6, "[NALQ][CTRLS] Recover buffer control id: 0x%08x\n", buf_ctrl->id);
	}
...
static int mfc_core_restore_buf_ctrls(struct mfc_ctx *ctx, struct list_head *head)
{
	...
	list_for_each_entry(buf_ctrl, head, list) {
		...
		mfc_ctx_debug(6, "[CTRLS] Restore buffer control id: 0x%08x, val: %d\n",
