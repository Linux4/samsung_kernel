...
static int __mfc_core_get_new_ctx(struct mfc_core *core)
{
	...
	if (core->preempt_core_ctx > MFC_NO_INSTANCE_SET) {
		...
		if (dev->otf_inst_bits && (dev->otf_inst_bits & core->work_bits.bits)) {
		...
		while (!(core->work_bits.bits & (1UL << new_ctx_index))) {
...
static int __mfc_core_get_next_ctx(struct mfc_core *core)
{
	...
	while (!(core->work_bits.bits & (1UL << next_ctx_index))) {
...
static int mfc_change_prio_work_rr(struct mfc_core *core, struct mfc_ctx *ctx,
