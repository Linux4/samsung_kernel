...
int mfc_wait_for_done_plugin_queue(struct mfc_core *core, int command, int idx)
{
	...
	mfc_core_debug(2, "[PLUGIN] waiting plug-in queued_job(idx: %d)\n", idx);

	ret = wait_event_timeout(core->cmd_wq,
			core->int_condition && (core->int_reason == command) &&
			core->fg_q_handle->ctx_num_table[idx] == -1,
			msecs_to_jiffies(MFC_INT_TIMEOUT));
	if (ret == 0) {
		mfc_core_err("Interrupt (core->int_reason:%d, command:%d, idx:%d) timed out\n",
			     core->int_reason, command, idx);
		call_dop(core, dump_and_stop_debug_mode, core);
		return 1;
	}

	mfc_core_debug(2, "Finished waiting (core->int_reason:%d, command: %d, idx: %d)\n",
			core->int_reason, command, idx);
	return 0;
}
		...
		struct mfc_bits *data, bool set)
{
	...
	buf_queue_greater_than_0
		= (mfc_get_queue_count_no_used(&ctx->buf_queue_lock, &ctx->plugin_buf_queue) > 0);
