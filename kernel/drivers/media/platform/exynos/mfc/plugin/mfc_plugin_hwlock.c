...
static void __mfc_plugin_fg_wait_done_queue(struct mfc_core *core)
{
	...
	while (core->fg_q_handle->queue_count != core->fg_q_handle->exe_count) {
...
static int __mfc_plugin_fg_just_run(struct mfc_core_ctx *core_ctx)
{
	...
	mfc_buf = mfc_get_buf_no_used(ctx, &ctx->plugin_buf_queue, MFC_BUF_NO_TOUCH_USED);
	if (!mfc_buf) {
	...
	if (mfc_buf->dpb_index == DEFAULT_TAG) {
		...
		if (mfc_buf) {
			/* wait the completion of queue before calling buffer done of EOS */
	...
	ret = mfc_plugin_pm_power_on(core);
	if (ret < 0)
		return ret;
	...
	if (core->dev->pdata->support_fg_shadow) {
		...
		ret = mfc_plugin_run_wait_for_ready_to_run(core, core_ctx, &flags);
		if (ret) {
			spin_unlock_irqrestore(&core->fg_q_handle->lock, flags);
			mfc_plugin_pm_power_off(core);
			mfc_err("[PLUGIN] failed to wait for ready to run\n");
			return ret;
		}
	}
	...
	mfc_plugin_protection_switch(core, ctx->is_drm);
...
static int __mfc_plugin_sw_just_run(struct mfc_core_ctx *core_ctx)
{
	...
	if (IS_SINGLE_FD(ctx, ctx->internal_fmt)) {
		...
		mfc_debug(2, "[PLUGIN] single-fd src index %d, addr %p, dst index %d, addr %p, size %d\n",
		...
		for (i = 0; i < raw->num_planes; i++) {
			...
			mfc_debug(2, "[PLUGIN] plane[%d] src index %d, addr %p, dst index %d, addr %p, size %d\n",
...
void mfc_plugin_hwlock_handler(struct mfc_core_ctx *core_ctx)
{
	...
	if (!list_empty(&core->hwlock.waiting_list) || (new_ctx_index < 0) ||
	    (ctx->plugin_type == MFC_PLUGIN_SW_MEMCPY) || (core_ctx->state == MFCINST_FINISHING)) {
		mfc_debug(2, "There is a waiting module or should be release hwlock\n");
		spin_unlock_irqrestore(&core->hwlock.lock, flags);

		/*
		 * do not release hwlock,
		 * to make the jobs of the context finished run first.
		 */
