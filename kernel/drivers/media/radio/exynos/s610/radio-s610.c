...
int fm_ldo_enable(struct s610_radio *radio)
{
	...
	ret = s2mpu15_update_reg(radio->i2c_main, ADDR_LDO27M_CTRL, 0xC0, 0xC0);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}

	s2mpu15_read_reg(radio->i2c_main, ADDR_LDO26M_CTRL, &reg26m);
	dev_info(radio->dev, "%s:2) read S2MPU15_PM_LDO26_CTRL: %xh\n", __func__, reg26m);

	s2mpu15_read_reg(radio->i2c_main, ADDR_LDO27M_CTRL, &reg27m);
	dev_info(radio->dev, "%s:2) read S2MPU15_PM_LDO27_CTRL: %xh\n", __func__, reg27m);

	s2mpu15_read_reg(radio->i2c_main, ADDR_LDO27M_OUT, &reg27m);
...
int fm_ldo_disable(struct s610_radio *radio)
{
	...
	ret = s2mpu15_update_reg(radio->i2c_main, ADDR_LDO26M_CTRL, 0x40, 0xC0);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}
	...
	ret = s2mpu15_update_reg(radio->i2c_main, ADDR_LDO27M_CTRL, 0x40, 0xC0);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}
