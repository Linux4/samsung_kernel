...
struct cipc_funcs {
	...
	void (*user_memcpy) (void *dst, void *src, int size, int dst_io, int src_io);
	void (*user_memset) (void *buf, int val, int size, int io);
	...
	int (*user_strncmp) (char *dst, char *src, int size);
	void (*user_strncpy) (char *dst, char *src, int size, int dst_io,
			 int src_io);
