...
#define log_event_condition (!atomic_read(&chub->atomic.chub_sleep) \
...
static int handle_log_kthread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		wait_event_interruptible(chub->log.log_kthread_wait, log_event_condition);
		if (!log_event_condition)
			continue;
...
void chub_printf(struct device *dev, int level, int fw_idx, const char *fmt, ...)
{
	...
	if (print_info) {
		...
		if (IS_ERR_OR_NULL(chub)) {
			if (dev)
				dev_err(print_info->dev, "%s: chub not available", dev_driver_string(dev));
			else
				dev_err(print_info->dev, "chub not available");
			return;
		}
