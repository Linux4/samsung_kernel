...
inline void dump_packet(uint8_t *b, int len)
{
	int i, j, left;
	char temp[128];
		     ...
		     uint32_t seq)
{
	...
	if (ret >= 0 && response->seq == seq) {
		if (response->reason == CMD_COMMS_ACK) {
			...
			dump_packet((uint8_t *) response, ret);
				...
				int retrans_cnt, int retrans_delay)
{
	...
	} while (retrans_cnt && (ret == ERROR_BUSY || ret == ERROR_NACK));

	packet_free(pad);

	if (!retrans_cnt) {
		nanohub_debug("nanohub_comms_tx_rx_retrans: %s\n",
			ret == ERROR_BUSY ? "ERR_COMMS_BUSY": "ERR_COMMS_NACK");
		contexthub_reset(chub, 1,
			ret == ERROR_BUSY ? CHUB_ERR_COMMS_BUSY: CHUB_ERR_COMMS_NACK);
	}
