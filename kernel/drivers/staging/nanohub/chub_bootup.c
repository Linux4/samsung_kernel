...
static int contexthub_download_image(struct contexthub_ipc_info *chub, int idx)
{
	...
	nanohub_dev_info(chub->dev, "%s: idx:%d, bin(size:%d) on %p\n",
			 __func__, idx, (int)entry->size, chub_addr);
#if IS_ENABLED(CONFIG_EXYNOS_S2MPU)
	if (idx > 0)
		ret = exynos_verify_subsystem_fw("CHUB", 1,
						 chub->iomem.sram_phys + SZ_4K,
						 entry->size, chub->iomem.sram_size - SZ_4K);
	else
		ret = exynos_verify_subsystem_fw("CHUB", 0,
						 chub->iomem.sram_phys, entry->size, SZ_4K);
...
#else
	ret = contexthub_reset(chub, 1, chub->misc.cur_err);
	chub->misc.cur_err = 0;
#endif
