...
void chub_dbg_dump_hw(struct contexthub_ipc_info *chub, enum chub_err_type reason)
{
	...
	if (chub->misc.hardfault_debug && p_dbg_dump) {
		...
		snprintf(dump.mini_dump, SHUB_MINI_DUMP_LENGTH,
				...
				"msp:%X psp:%X sp:%X ISER:%X ICER:%X ISPR:%X ICPR:%X IABR:%X IP:%X STIR:%X "
				"SHCSR:%X CFSR:%X HFSR:%X DFSR:%X MMFAR:%X BFAR:%X dump0~6:%X %X %X %X %X %X %X",
				...
				p_dbg_dump->gpr[16],
				hardFaultInfo.msp, hardFaultInfo.psp, hardFaultInfo.sp,
				hardFaultInfo.NVIC_ISER, hardFaultInfo.NVIC_ICER, hardFaultInfo.NVIC_ISPR,
				hardFaultInfo.NVIC_ICPR, hardFaultInfo.NVIC_IABR, hardFaultInfo.NVIC_IP,
				hardFaultInfo.NVIC_STIR, hardFaultInfo.scb.SHCSR, hardFaultInfo.scb.CFSR,
				hardFaultInfo.scb.HFSR, hardFaultInfo.scb.DFSR, hardFaultInfo.scb.MMFAR,
				hardFaultInfo.scb.BFAR,
