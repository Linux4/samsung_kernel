...
#define DEFAULT_TIMER_RATE 25600000
...
void ipc_set_chub_boottime(bool alive_mct_enabled)
{
	...
	if (alive_mct_enabled) {
		...
		if (!time_gap) {
			...
			if (!rate) {
				rate = DEFAULT_TIMER_RATE;
				CSP_PRINTF_INFO(" Fail to get clock-freq from dts. Set rate as default value (%u).", rate);
			}
...
static void *ipc_set_map(char *sram_base)
{
	...
	ipc_set_info(IPC_REG_BL_MAP, map, (int)sizeof(struct chub_bootargs),
		     "bl_map");
	...
	ipc_set_info(IPC_REG_LOG, &ipc.ipc_map->logbuf,
		     (int)sizeof(struct ipc_logbuf), "log");
	ipc_set_info(IPC_REG_PERSISTBUF, &ipc.ipc_map->persist,
		     (int)sizeof(struct chub_persist), "persist");
	ipc_set_info(IPC_REG_IPC_SENSORINFO, &ipc.ipc_map->sensormap,
		     (int)sizeof(struct sensor_map), "sensorinfo");
