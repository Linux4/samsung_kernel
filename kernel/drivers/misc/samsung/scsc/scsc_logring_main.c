...
ADD_DEBUG_MODULE_PARAM(pm_bin,  SCSC_DBG1, BIN_WIFI_PM);
...
ADD_DEBUG_MODULE_PARAM(bt_h4,   DEFAULT_DBGLEVEL, BT_H4);
ADD_DEBUG_MODULE_PARAM(bt_fw,   SCSC_FULL_DEBUG, BT_FW);
ADD_DEBUG_MODULE_PARAM(bt_rx,   DEFAULT_DBGLEVEL, BT_RX);
ADD_DEBUG_MODULE_PARAM(bt_tx,   DEFAULT_DBGLEVEL, BT_TX);
...
ADD_DEBUG_MODULE_PARAM(flash_service, SCSC_FULL_DEBUG, FLASH_SERVICE);
...
_Static_assert(
	ARRAY_SIZE(scsc_droplevels) == MAX_TAG + 1,
	"You must keep 'enum scsc_logring_tags' and 'scsc_droplevels' in sync"
);
...
static inline
void memlog_out_string_pm(int tag, const char *fmt, va_list args)
{
	struct va_format vaf;
	char *msg_head = NULL;

	if (memlog_pm_obj == NULL)
		return;
	...
	memlog_write_printf(memlog_pm_obj, 1, "[%s] :: %pV", tagstr[tag], &vaf);
	memlog_sync_to_file(memlog_pm_obj);
}

static inline
void memlog_out_string_print(int level, int tag, const char *fmt,
			va_list args, int force)
{
	unsigned long flags;
	struct va_format vaf;
	u8 ctx;
	...
	memlog_write_printf(memlog_print_obj, SCSC_LVL2MEMLOG(level),
		"<%d> [c%d] [%c] [%s] :: %pV",
			...
			int force)
{
	...
	if (tag == SLSI_PM)
		memlog_out_string_pm(tag, fmt, args);
	memlog_out_string_print(level, tag, fmt, args, force);
}

static inline
void memlog_out_bin_pm(int tag, const void *start, size_t len)
{
	const u8 *ptr = start;
	...
	for (i = 0; i < len; i += rowsize) {
		...
		memlog_write_printf(memlog_pm_obj, 1, "[%s] HEX->|%s\n",
					tagstr[tag], linebuf);
	}
}

static inline
void memlog_out_bin_print(int level, int tag, const void *start,
			...
			int force)
{
	...
	if (tag == BIN_WIFI_PM)
		memlog_out_bin_pm(tag, start, len);

	memlog_out_bin_print(level, tag, start, len, force);
}
#endif
