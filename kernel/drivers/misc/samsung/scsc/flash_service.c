...
{
	struct flash_service_conf *conf;
	unsigned long timeout, start_time, end_time;
	u32 conf_read;
	...
	if (scsc_mx_service_users_active(flash_ins->mx)) {
		SCSC_TAG_ERR(FLASH_SERVICE, "Flash service can't run and update FW when other services are running. Stop both WLAN and BT services completely before triggering flash_srv_manual\n");
	...
	if (flash_service_open_start_services(flash_ins->mx)) {
		SCSC_TAG_ERR(FLASH_SERVICE, "Error starting service\n");
	} else {
		start_time = jiffies;
	...
	do {
		...
		if (conf_read == FLASH_SERVICE_FINISHED) {
			end_time = jiffies;
			SCSC_TAG_ERR(FLASH_SERVICE, "Flash service finished successfully in %ld miliseconds\n", jiffies_to_msecs(end_time - start_time));
			goto done_ok;
		}
