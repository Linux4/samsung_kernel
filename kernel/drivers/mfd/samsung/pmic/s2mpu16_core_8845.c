...
int s2mpu16_read_reg(struct i2c_client *i2c, uint8_t reg, uint8_t *dest)
{
	...
	ret = exynos_esca_read_reg(esca_mfd_node, SUB_CHANNEL, i2c->addr, reg, dest);
...
int s2mpu16_bulk_read(struct i2c_client *i2c, uint8_t reg, int count, uint8_t *buf)
{
	...
	mutex_lock(&s2mpu16->i2c_lock);
	ret = exynos_esca_bulk_read(esca_mfd_node, SUB_CHANNEL, i2c->addr, reg, count, buf);
	mutex_unlock(&s2mpu16->i2c_lock);
	if (ret) {
		pr_err("[%s] esca(acpm) ipc fail!\n", __func__);
		return ret;
...
int s2mpu16_write_reg(struct i2c_client *i2c, uint8_t reg, uint8_t value)
{
	...
	mutex_lock(&s2mpu16->i2c_lock);
	ret = exynos_esca_write_reg(esca_mfd_node, SUB_CHANNEL, i2c->addr, reg, value);
	mutex_unlock(&s2mpu16->i2c_lock);
	if (ret) {
		pr_err("[%s] esca(acpm) ipc fail!\n", __func__);
		return ret;
...
int s2mpu16_bulk_write(struct i2c_client *i2c, uint8_t reg, int count, uint8_t *buf)
{
	...
	mutex_lock(&s2mpu16->i2c_lock);
	ret = exynos_esca_bulk_write(esca_mfd_node, SUB_CHANNEL, i2c->addr, reg, count, buf);
	mutex_unlock(&s2mpu16->i2c_lock);
	if (ret) {
		pr_err("[%s] esca(acpm) ipc fail!\n", __func__);
		return ret;
...
int s2mpu16_update_reg(struct i2c_client *i2c, uint8_t reg, uint8_t val, uint8_t mask)
{
	...
	mutex_lock(&s2mpu16->i2c_lock);
	ret = exynos_esca_update_reg(esca_mfd_node, SUB_CHANNEL, i2c->addr, reg, val, mask);
	mutex_unlock(&s2mpu16->i2c_lock);
	if (ret) {
		pr_err("[%s] esca(acpm) ipc fail!\n", __func__);
		return ret;
