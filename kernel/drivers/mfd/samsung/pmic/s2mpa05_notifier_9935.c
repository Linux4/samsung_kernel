				    ...
				    void *data)
{
	...
	ret = s2mpa05_bulk_read(s2mpa05->pmic, S2MPA05_REG_PM_INT1,
...
static const uint8_t s2mpa05_mask_reg[] = {
	[S2MPA05_PMIC_INT1] = S2MPA05_REG_PM_INT1M,
	[S2MPA05_PMIC_INT2] = S2MPA05_REG_PM_INT2M,
	[S2MPA05_PMIC_INT3] = S2MPA05_REG_PM_INT3M,
	[S2MPA05_PMIC_INT4] = S2MPA05_REG_PM_INT4M,
};
...
static int s2mpa05_unmask_interrupt(struct s2mpa05_dev *s2mpa05)
{
	...
	ret = s2mpa05_update_reg(s2mpa05->i2c, S2MPA05_REG_COMMON_TX_MASK,
				 0x00, S2MPA05_PM_IRQM_MASK);
	if (ret)
		goto err;

	/* unmask OI, OCP interrupt */
	ret = s2mpa05_write_reg(s2mpa05->pmic, S2MPA05_REG_PM_INT2M, 0x00);
	if (ret)
		goto err;

	/* unmask Temp interrupt */
	ret = s2mpa05_update_reg(s2mpa05->pmic, S2MPA05_REG_PM_INT1M, 0x00, 0x0C);
	if (ret)
		goto err;
