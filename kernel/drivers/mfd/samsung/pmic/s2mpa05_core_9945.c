...
int s2mpa05_read_reg(struct i2c_client *i2c, uint8_t reg, uint8_t *dest)
{
	...
	ret = exynos_acpm_read_reg(acpm_mfd_node, EXTRA_ID, i2c->addr, reg, dest);
...
int s2mpa05_bulk_read(struct i2c_client *i2c, uint8_t reg, int count, uint8_t *buf)
{
	...
	mutex_lock(&s2mpa05->i2c_lock);
	ret = exynos_acpm_bulk_read(acpm_mfd_node, EXTRA_ID, i2c->addr, reg, count, buf);
	mutex_unlock(&s2mpa05->i2c_lock);
	if (ret) {
		pr_err("[%s] acpm ipc fail!\n", __func__);
		return ret;
...
int s2mpa05_write_reg(struct i2c_client *i2c, uint8_t reg, uint8_t value)
{
	...
	mutex_lock(&s2mpa05->i2c_lock);
	ret = exynos_acpm_write_reg(acpm_mfd_node, EXTRA_ID, i2c->addr, reg, value);
	mutex_unlock(&s2mpa05->i2c_lock);
	if (ret) {
		pr_err("[%s] acpm ipc fail!\n", __func__);
		return ret;
...
int s2mpa05_bulk_write(struct i2c_client *i2c, uint8_t reg, int count, uint8_t *buf)
{
	...
	mutex_lock(&s2mpa05->i2c_lock);
	ret = exynos_acpm_bulk_write(acpm_mfd_node, EXTRA_ID, i2c->addr, reg, count, buf);
	mutex_unlock(&s2mpa05->i2c_lock);
	if (ret) {
		pr_err("[%s] acpm ipc fail!\n", __func__);
		return ret;
...
int s2mpa05_update_reg(struct i2c_client *i2c, uint8_t reg, uint8_t val, uint8_t mask)
{
	...
	mutex_lock(&s2mpa05->i2c_lock);
	ret = exynos_acpm_update_reg(acpm_mfd_node, EXTRA_ID, i2c->addr, reg, val, mask);
	mutex_unlock(&s2mpa05->i2c_lock);
	if (ret) {
		pr_err("[%s] acpm ipc fail!\n", __func__);
		return ret;
...
{
	s2mpa05->vgpio = devm_i2c_new_dummy_device(s2mpa05->dev, s2mpa05->com->adapter, TEMP_ADDR);
	s2mpa05->pmic = devm_i2c_new_dummy_device(s2mpa05->dev, s2mpa05->com->adapter, PMIC_ADDR);
	s2mpa05->close = devm_i2c_new_dummy_device(s2mpa05->dev, s2mpa05->com->adapter, CLOSE_ADDR);
	s2mpa05->gpio = devm_i2c_new_dummy_device(s2mpa05->dev, s2mpa05->com->adapter, GPIO_ADDR);

	s2mpa05->com->addr = COM_ADDR;	/* forced COMMON address */
	s2mpa05->vgpio->addr = VGPIO_ADDR;

	i2c_set_clientdata(s2mpa05->com, s2mpa05);
