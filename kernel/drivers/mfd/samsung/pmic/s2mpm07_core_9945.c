...
int s2mpm07_read_reg(struct i2c_client *i2c, uint8_t reg, uint8_t *dest)
{
	...
	ret = exynos_acpm_read_reg(acpm_mfd_node, RF_ID, i2c->addr, reg, dest);
...
int s2mpm07_bulk_read(struct i2c_client *i2c, uint8_t reg, int count, uint8_t *buf)
{
	...
	mutex_lock(&s2mpm07->i2c_lock);
	ret = exynos_acpm_bulk_read(acpm_mfd_node, RF_ID, i2c->addr, reg, count, buf);
	mutex_unlock(&s2mpm07->i2c_lock);
...
int s2mpm07_write_reg(struct i2c_client *i2c, uint8_t reg, uint8_t value)
{
	...
	mutex_lock(&s2mpm07->i2c_lock);
	ret = exynos_acpm_write_reg(acpm_mfd_node, RF_ID, i2c->addr, reg, value);
	mutex_unlock(&s2mpm07->i2c_lock);
...
int s2mpm07_bulk_write(struct i2c_client *i2c, uint8_t reg, int count, uint8_t *buf)
{
	...
	mutex_lock(&s2mpm07->i2c_lock);
	ret = exynos_acpm_bulk_write(acpm_mfd_node, RF_ID, i2c->addr, reg, count, buf);
	mutex_unlock(&s2mpm07->i2c_lock);
...
int s2mpm07_update_reg(struct i2c_client *i2c, uint8_t reg, uint8_t val, uint8_t mask)
{
	...
	mutex_lock(&s2mpm07->i2c_lock);
	ret = exynos_acpm_update_reg(acpm_mfd_node, RF_ID, i2c->addr, reg, val, mask);
	mutex_unlock(&s2mpm07->i2c_lock);
...
static int s2mpm07_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *dev_id)
{
	...
	s2mpm07->dev = &i2c->dev;
	i2c->addr = COM_ADDR;	/* forced COMMON address */
	s2mpm07->com = i2c;
	...
	s2mpm07->vgpio = devm_i2c_new_dummy_device(s2mpm07->dev, i2c->adapter, TEMP_ADDR);
	s2mpm07->vgpio->addr = VGPIO_ADDR;
	s2mpm07->pmic1 = devm_i2c_new_dummy_device(s2mpm07->dev, i2c->adapter, PMIC1_ADDR);
	s2mpm07->close1 = devm_i2c_new_dummy_device(s2mpm07->dev, i2c->adapter, CLOSE1_ADDR);
	s2mpm07->adc_i2c = devm_i2c_new_dummy_device(s2mpm07->dev, i2c->adapter, ADC_ADDR);
	s2mpm07->gpio = devm_i2c_new_dummy_device(s2mpm07->dev, i2c->adapter, GPIO_ADDR);
