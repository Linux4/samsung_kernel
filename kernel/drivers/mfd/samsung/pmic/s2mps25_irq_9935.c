...
#define SPMI_MASTER_PMIC		0x15960000
#define VGPIO_MONITOR_ADDR		0x0000
#define VGPIO_MONITOR_ADDR2		0x0004

/* VGPIO_SYSREG ADDR. */
#define INTCOMB_VGPIO2AP		0x15930000
#define INTCOMB_VGPIO2PMU		0x15950000
...
static void s2mps25_clear_sysreg(struct s2mps25_dev *s2mps25)
{
	uint32_t val = 0;

	/* Clear interrupt pending bit (to AP)*/
	val = readl(s2mps25->sysreg_vgpio2ap);
	writel(val, s2mps25->sysreg_vgpio2ap);

	/* Clear interrupt pending bit (to PMU)*/
	val = readl(s2mps25->sysreg_vgpio2pmu);
	writel(val, s2mps25->sysreg_vgpio2pmu);
}
...
#if IS_ENABLED(CONFIG_MFD_S2MPS26_9935)
	if (vgi_src[1] & S2MPS25_VGI0_IRQ_S1) {
		pr_info("%s: IBI from sub pmic\n", __func__);
		s2mps26_call_notifier();
	}
#endif
#if IS_ENABLED(CONFIG_MFD_S2MPA05_9935)
	if (vgi_src[2] & S2MPS25_VGI0_IRQ_S3) {
...
{
	s2mps25->mem_base = ioremap(SPMI_MASTER_PMIC + VGPIO_MONITOR_ADDR, SZ_32);
	if (s2mps25->mem_base == NULL)
		pr_info("%s: fail to allocate mem_base\n", __func__);

	/* VOL_DN */
	s2mps25->mem_base2 = ioremap(SPMI_MASTER_PMIC + VGPIO_MONITOR_ADDR2, SZ_32);
	if (s2mps25->mem_base2 == NULL)
		pr_info("%s: fail to allocate mem_base2\n", __func__);
}

static void s2mps25_set_sysreg(struct s2mps25_dev *s2mps25)
{
	s2mps25->sysreg_vgpio2ap = ioremap(INTCOMB_VGPIO2AP + INTC0_IPEND, SZ_32);
	if (s2mps25->sysreg_vgpio2ap == NULL)
		pr_info("%s: fail to allocate sysreg_vgpio2ap\n", __func__);

	s2mps25->sysreg_vgpio2pmu = ioremap(INTCOMB_VGPIO2PMU + INTC0_IPEND, SZ_32);
	if (s2mps25->sysreg_vgpio2pmu == NULL)
		pr_info("%s: fail to allocate sysreg_vgpio2pmu\n", __func__);
}

static void s2mps25_voldn_irq_enable(struct s2mps25_dev *s2mps25)
{
	s2mps25_update_reg(s2mps25->pmic2, S2MPS25_REG_OFF_CTRL1, 0x20, 0x20);
}

static void s2mps25_irq_enable(struct s2mps25_dev *s2mps25)
{
	uint8_t i2c_data = 0;

	s2mps25_voldn_irq_enable(s2mps25);

	s2mps25_write_reg(s2mps25->i2c, S2MPS25_REG_TX_MASK, 0xB2);
	s2mps25_read_reg(s2mps25->i2c, S2MPS25_REG_TX_MASK, &i2c_data);
	pr_info("%s: %s S2MPS25_TX_MASK=0x%02hhx\n", MFD_DEV_NAME, __func__, i2c_data);
}
...
void s2mps25_irq_exit(struct s2mps25_dev *s2mps25)
{
	...
	iounmap(s2mps25->mem_base2);
	iounmap(s2mps25->sysreg_vgpio2ap);
	iounmap(s2mps25->sysreg_vgpio2pmu);
