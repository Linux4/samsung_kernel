...
#if IS_ENABLED(CONFIG_INTERCONNECT) && !IS_ENABLED(CONFIG_SOC_S5E9945)
/**
...
{
	struct cnss_plat_data *plat_priv = pci_priv->plat_priv;
/* [TEMP][DEBUG][CN 06926081] Assert Panic to obtain full dump / ipc log */
#ifdef CONFIG_WLAN_DEBUG
// IRQ storm debug changes for GS24FW project
    cnss_fatal_err("MHI power up returns timeout\n");
	dbg_snapshot_expire_watchdog();
#else
    panic("MHI power up returns timeout\n");
#endif /* CONFIG_WLAN_DEBUG */
		...
		if (ret == -EAGAIN && retry++ < POWER_ON_RETRY_MAX_TIMES) {
			...
			cnss_pr_dbg("Value of SW_CTRL GPIO: %d\n",
				    cnss_get_input_gpio_value(plat_priv,
							      sw_ctrl_gpio));
#ifdef CONFIG_SOC_S5E9945
			msleep(LINK_FAILURE_POWER_ON_RETRY_DELAY_MS);
#else
...
#if !IS_ENABLED(CONFIG_WLAN_DEBUG)
// IRQ storm debug changes for GS24FW project
