...
static struct cnss_vreg_cfg cnss_vreg_list[] = {
	{"vreg_wlan", 0, 0, 0, 0, 0},
	{"vdd-wlan-1P35", 1350000, 1350000, 0, 0, 0},
	{"vdd-wlan-1P0", 1025000, 1025000, 0, 0, 0},
};
...
#define WLAN_CLK_REQ_GPIO   "wlan-clk-req-gpio"
static irqreturn_t cnss_clk_req_wake_handler(int irq, void* plat_priv)
{
    cnss_pr_err("clk_req_irq triggered");
    return IRQ_HANDLED;
}

int cnss_init_clk_req_gpio(struct cnss_plat_data *plat_priv)
{
    int ret = 0;
    struct device *dev = &plat_priv->plat_dev->dev;
    struct irq_desc *desc;

    plat_priv->clk_req_gpio = of_get_named_gpio(dev->of_node,
                            WLAN_CLK_REQ_GPIO, 0);
    if (plat_priv->clk_req_gpio < 0)
        goto out;

    cnss_pr_err("Get CLK_REQ GPIO (%d) from device node\n",
            plat_priv->clk_req_gpio);

    ret = gpio_request(plat_priv->clk_req_gpio, WLAN_CLK_REQ_GPIO);
    if (ret) {
        cnss_pr_err("Failed to request CLK_REQ GPIO, err = %d\n",
                ret);
        goto out;
    }

    ret = gpio_direction_input(plat_priv->clk_req_gpio);
    if (ret) {
        cnss_pr_err("Failed to set GPIO direction CLK_REQ GPIO, err = %d\n",
                ret);
        goto free_gpio;
    }

    plat_priv->clk_req_irq = 0;
    plat_priv->clk_req_irq = gpio_to_irq(plat_priv->clk_req_gpio);
    cnss_pr_err("Get CLK_REQ IRQ (%d)\n",
                    plat_priv->clk_req_irq);

    if (plat_priv->clk_req_irq < 0)
        goto free_gpio;

    ret = request_irq(plat_priv->clk_req_irq, cnss_clk_req_wake_handler,
                IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
                "wlan_clk_req_irq", plat_priv);

    if (ret) {
        cnss_pr_err("Failed to request WLAN_CLK req IRQ, err = %d\n", ret);
        goto free_gpio;
    }
    desc = irq_to_desc(plat_priv->clk_req_irq);
    if (desc && desc->depth > 0)
        enable_irq(plat_priv->clk_req_irq);

    acpm_init_eint_clk_req(plat_priv->clk_req_gpio);

    return 0;

free_gpio:
    gpio_free(plat_priv->clk_req_gpio);
out:
    return ret;
}
#endif /* CONFIG_SOC_S5E9945 */
