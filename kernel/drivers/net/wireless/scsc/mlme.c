				       ...
				       bool (*validate_cfm_wait_ind)(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *cfm))
{
	...
	if (scsc_mx_service_claim(WLAN_MLME_REQ_CFM_IND)) {
	...
	if (err != 0) {
		...
		kfree_skb(skb);
#if defined(CONFIG_SCSC_PCIE_CHIP)
		scsc_mx_service_release(WLAN_MLME_REQ_CFM_IND);
#endif
		goto clean_exit;
	}
	if (cfm_id) {
...
#define SLSI_CONFIG_MLO_MODE_ANY 5
	...
	if (ndev_vif->sta.mlo_mode && ndev_vif->sta.mlo_mode <= SLSI_CONFIG_MLO_MODE_ANY) {
		fapi_set_u16(req, u.mlme_connect_req.spare_1, ndev_vif->sta.mlo_mode);
	} else if (ml_ie) {
...
*/
	...
	if ((fw_rate & SLSI_FW_API_RATE_HT_SELECTOR_FIELD) == SLSI_FW_API_RATE_NON_HT_SELECTED) {
	...
	} else if ((fw_rate & SLSI_FW_API_RATE_EHT_SELECTED) == SLSI_FW_API_RATE_EHT_SELECTED) {
		int gi_idx;
		 ...
		 * 0x4= 320 MHz */
		if ((chan_bw_idx <= 4) && (mcs_idx < 16) && (mcs_idx != 14)) {
			if (rate)
				rate->legacy = nss * 10 *
			      ...
			      u16 host_tag, u32 dwell_time, u32 period)
{
	...
	fapi_set_u16(skb, u.mlme_send_frame_req.vif, ifnum);
...
#endif
	if (vlan_id && msg_type == FAPI_MESSAGETYPE_ARP)
		host_tag |= vlan_id << 13;
...
int slsi_mlme_set_tdls_state(struct slsi_dev *sdev, struct net_device *dev, u16 enabled_flag)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_set_tdls_state_req, MLME_SET_TDLS_STATE_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
			       ...
			       int command_len, int *command_pos, u16 setup_id)
{
	...
	if (fapi_get_u16(cfm, u.mlme_twt_status_query_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		r = slsi_get_error_code(fapi_get_u16(cfm, u.mlme_twt_status_query_cfm.result_code));
...
int slsi_mlme_set_external_assoc_ies(struct slsi_dev *sdev, struct net_device *dev,
				     struct cfg80211_external_fils_assoc_params *filsie)
{
	...
	struct key_params kek_key;
	u8 address[ETH_ALEN];
	int pos = 0;
	struct slsi_mlme_parameters *req;

	add_info_ie_len = filsie->ie_len + sizeof(struct slsi_mlme_parameters) +
			  ((2 * FILS_NONCE_LEN)) + (ndev_vif->sta.fils_realm_len + 1) + FILS_CACHE_ID_LEN;
	...
	memcpy(add_info_ies, filsie->ie, filsie->ie_len);
	pos += filsie->ie_len;
	...
	req->length = SLSI_MLME_PARAM_HEADER_SIZE + ((2 * FILS_NONCE_LEN)) + \
		      (ndev_vif->sta.fils_realm_len + 1) + FILS_CACHE_ID_LEN;
	...
	pos += sizeof(struct slsi_mlme_parameters);

	memcpy(&add_info_ies[pos], filsie->fils_nonces, (2 * FILS_NONCE_LEN));
	pos += (2 * FILS_NONCE_LEN);

	/* 2 Bytes of Cache ID to be sent as 0x00 as cache id will not be available now */
	memset(&add_info_ies[pos], 0, FILS_CACHE_ID_LEN);
	pos += FILS_CACHE_ID_LEN;

	add_info_ies[pos++] = ndev_vif->sta.fils_realm_len;
	memcpy(&add_info_ies[pos], ndev_vif->sta.fils_realm, ndev_vif->sta.fils_realm_len);

	r = slsi_mlme_add_info_elements(sdev, dev, FAPI_PURPOSE_ASSOCIATION_REQUEST, add_info_ies, add_info_ie_len);
	kek_key.key = filsie->fils_kek;
	kek_key.key_len = filsie->fils_kek_len;
	memcpy(address, ndev_vif->sta.bssid, ETH_ALEN);

	r = slsi_mlme_set_key(sdev, dev, 0, FAPI_KEYTYPE_KEK, address, &kek_key);
	kfree(add_info_ies);
	return r;
}
#endif
...
int slsi_mlme_set_elnabypass_threshold(struct slsi_dev *sdev, struct net_device *elna_net_device, int elna_value)
{
	...
	req = fapi_alloc(mlme_set_elna_bypass_req, MLME_SET_ELNA_BYPASS_REQ, elna_ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
	fapi_set_u16(req, u.mlme_set_elna_bypass_req.vif, elna_ndev_vif->ifnum);
...
int slsi_mlme_set_tx_num_ant(struct slsi_dev *sdev, struct net_device *dev, int cfg)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	SLSI_DBG1_NODEV(SLSI_MLME, "mlme_set_num_tx_antennas_req(vif:%u antenna_selection:%u)\n",
			ndev_vif->ifnum, cfg);

	req = fapi_alloc(mlme_set_num_tx_antennas_req, MLME_SET_NUM_TX_ANTENNAS_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;
...
int slsi_mlme_set_uwbcx(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_set_uwbcx_req, MLME_SET_UWBCX_REQ, ndev_vif->ifnum, 0);
	if (!req)
		return -ENOMEM;

	fapi_set_u16(req, u.mlme_set_uwbcx_req.vif, ndev_vif->ifnum);
	...
	cfm = slsi_mlme_req_cfm(sdev, dev, req, MLME_SET_UWBCX_CFM);
	if (!cfm)
		return -EIO;
...
bool slsi_mlme_set_max_bw_cfm_validate(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *cfm)
{
	int  result = fapi_get_u16(cfm, u.mlme_set_bss_max_channel_width_cfm.result_code);
	bool r = false;
	...
	if (result == FAPI_RESULTCODE_SUCCESS)
		...
		SLSI_ERR_NODEV("mlme_set_max_bw_cfm(result:0x%04x) ERROR\n", result);
			 ...
			 u16 announce_validate, u16 bandwidth)
{
	...
	SLSI_NET_DBG1(dev, SLSI_MLME, "vif:%u, protection_scope:%d announce_validate:%d bandwidth:%d\n",
		      ndev_vif->ifnum, protection_scope, announce_validate, bandwidth);
	...
	if (announce_validate == 0) {
		rx = slsi_mlme_req_cfm(sdev, NULL, req, MLME_SET_BSS_MAX_CHANNEL_WIDTH_CFM);
		if (rx) {
			if (fapi_get_u16(rx, u.mlme_set_bss_max_channel_width_cfm.result_code) !=
		...
		rx = slsi_mlme_req_cfm_ind(sdev, dev, req, MLME_SET_BSS_MAX_CHANNEL_WIDTH_CFM,
					   MLME_SET_BSS_MAX_CHANNEL_WIDTH_IND, slsi_mlme_set_max_bw_cfm_validate);
		if (rx) {
			if (fapi_get_u16(rx, u.mlme_set_bss_max_channel_width_ind.result_code) !=
			    FAPI_RESULTCODE_HOST_REQUEST_SUCCESS) {
				SLSI_NET_ERR(dev, "mlme_set_bss_max_channel_width_ind(result:0x%04x) ERROR\n",
					     fapi_get_u16(rx, u.mlme_set_bss_max_channel_width_ind.result_code));
				r = -EINVAL;
			}
		} else {
			SLSI_NET_ERR(dev, "slsi_mlme_req_cfm_ind() ERROR\n");
			return -EIO;
		}
	}
