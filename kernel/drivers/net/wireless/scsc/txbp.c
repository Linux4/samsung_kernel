...
#ifdef CONFIG_SCSC_WLAN_LPC
	lpc_tx_tag_index = (lpc_tx_tag_index + 1) % LPC_TAG_NUM_MAX;
	if (lpc_tx_tag_index == 0)
		lpc_tx_tag_index = 1;
	slsi_lpc_add_packet_data(lpc_tx_tag_index, skb, SLSI_LPC_DATA_TYPE_TX);
#endif
...
#ifdef CONFIG_SCSC_WLAN_LPC
	fapi_set_u32(skb, u.ma_unitdata_req.spare_2, lpc_tx_tag_index);
#endif
