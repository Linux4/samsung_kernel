...
void slsi_regd_use_world_domain(struct slsi_dev *sdev)
{
	...
	struct ieee80211_reg_rule  reg_rules_monitor[] = {
		...
		REG_RULE(5170, 5330, 160, 0, 20, 0),
		REG_RULE(5470, 5730, 160, 0, 20, 0),
		REG_RULE(5725, 5895, 160, 0, 20, 0),
		REG_RULE(5925, 7125, 320, 0, 20, 0),
	};
	int	i;
	...
	if (sdev->monitor_mode) {
		SLSI_DBG1_NODEV(SLSI_INIT_DEINIT, "regulatory monitor mode\n\n");
		slsi_world_regdom_custom->n_reg_rules = (sizeof(reg_rules_monitor)) / sizeof(reg_rules_monitor[0]);
		for (i = 0; i < slsi_world_regdom_custom->n_reg_rules; i++)
			slsi_world_regdom_custom->reg_rules[i] = reg_rules_monitor[i];
...
int slsi_read_regulatory(struct slsi_dev *sdev)
{
	...
	if (sdev->regdb.regdb_state == SLSI_REG_DB_SET) {
		SLSI_INFO(sdev, "DB Ver:%d.%d, Num Countries:%d\n",
			  sdev->regdb.db_major_version, sdev->regdb.db_minor_version,
	...
	if (script_version != 1 && script_version != 2) {
		SLSI_INFO(sdev, "Error %s generated with unknown script(%d)\n", script_version);
		goto exit;
	}
	...
	SLSI_INFO(sdev, "DB Ver:%d.%d, Script Ver:%d, Num Countries:%d\n",
