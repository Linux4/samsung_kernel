...
#define TH_MID 0
#define TH_HIGH 1
#define RPS_MAP_LOW "00"
#define RPS_MAP_HIGH "70"
...
int slsi_lbm_set_property(struct slsi_dev *sdev)
{
	...
	for (i = 0; i < NAPI_NUM_MAX; i++) {
		...
		for (j = TRAFFIC_MON_CLIENT_STATE_HIGH ; j < TRAFFIC_MON_CLIENT_MAX_NUM_OF_STATE ; j++) {
			cpu = slsi_find_proper_cpu(NULL, 0, i, 1, &except_cpus);
					 ...
					 int irq, int idx)
{
	...
	for (i = 0; i < SLSI_NR_CPUS; i++) {
		...
		if (idx != NP_TX_0)
			netif_napi_add(dev, napi_ins, slsi_lbm_napi_poll);
		else
			netif_napi_add_tx(dev, napi_ins, slsi_lbm_napi_poll);
#else
		if (idx != NP_TX_0)
