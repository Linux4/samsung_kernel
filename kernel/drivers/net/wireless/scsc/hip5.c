                                                ...
                                                struct sk_buff_head *skb_list)
{
	...
	const size_t min_len = sizeof(struct ieee80211_hdr_3addr) + LLC_SNAP_HDR_LEN + sizeof(struct iphdr);
	...
	while (num_bd) {
		if (!(bulk_desc->flag & HIP5_OPT_BD_CHAIN_START)) {
			...
			if (le16_to_cpu(((struct fapi_signal *)(sig_ptr))->id) == MA_UNITDATA_IND) {
				/* check for min length */
				if (bulk_desc->data_len < min_len) {
					SLSI_ERR_NODEV("invalid minimum len (%d); trigger Panic\n", bulk_desc->data_len);
					SLSI_FW_BUG_ON_WQ(sdev, 1);
					return -EFAULT;
				}
			}

			skb = alloc_skb(SLSI_NETIF_SKB_HEADROOM + hip_signal->sig_len + bulk_desc->data_len, GFP_ATOMIC);
			if (!skb) {
			...
			skb_reserve(skb, SLSI_NETIF_SKB_HEADROOM);
			memcpy(skb_put(skb, hip_signal->sig_len), sig_ptr, hip_signal->sig_len);
			...
			skb = alloc_skb(SLSI_NETIF_SKB_HEADROOM + hip_signal->sig_len + data_len, GFP_ATOMIC);
			if (!skb) {
...
#ifdef CONFIG_SCSC_WLAN_LPC
static void hip5_lpc_work(struct work_struct *data)
{
...
int slsi_hip_init(struct slsi_hip *hip)
{
	...
	hip->hip_priv->bh_dat = slsi_lbm_register_napi(sdev,
						       ...
						       NP_RX_0);
#ifdef CONFIG_SCSC_WLAN_LPC
	hip->hip_priv->bh_lpc = slsi_lbm_register_workqueue(sdev, hip5_lpc_work, -1);
...
bool hip5_opt_aggr_check(struct slsi_dev *sdev, struct slsi_hip *hip, struct sk_buff *skb)
{
	struct hip_priv     *hip_priv = hip->hip_priv;
	u8 i = 0;
	...
	SLSI_DBG4(sdev, SLSI_HIP, "skb_headroom:%d\n", skb_headroom(skb));
	if (skb_headroom(skb) >= (SLSI_HIP_HIP5_OPT_INVALID_OFFSET - fapi_sig_size(ma_unitdata_req)))  {
		SLSI_ERR_NODEV("large Headroom (%d), send in no-aggregation mode\n", skb_headroom(skb));
...
#ifdef CONFIG_SCSC_WLAN_LPC
		if (!ctrl_packet && !slsi_lpc_is_lpc_enabled() && hip5_opt_aggr_check(sdev, hip, skb))
			return 0;
#else
		if (!ctrl_packet) {
			if (hip5_opt_aggr_check(sdev, hip, skb)) {
	...
	for (i = 0; i < SLSI_HIP_HIP5_OPT_TX_Q_MAX; i++) {
		...
		while ((idx < HIP5_OPT_NUM_BD_PER_SIGNAL_MAX) && !skb_queue_empty(&hip_priv->hip5_opt_tx_q[i].tx_q)) {
			...
			if (hip5_tx_zero_copy) {
				...
				SLSI_DBG4(sdev, SLSI_HIP, "SKB: len:%d, headroom:%d\n", skb->len, skb_headroom(skb));
...
send_signal:
		if (skb_fapi) {
			if (idx) {
				/* bulk data has been allocated successfully, construct the HIP signal */
				hip5_q_entry = (struct hip5_opt_hip_signal *)&ctrl->q_tlv[0].array[idx_w];
...
int slsi_hip_setup(struct slsi_hip *hip)
{
	...
	if (hip_config_ver == 4) {
		hip->hip_priv->unidat_req_headroom =
			scsc_wifi_get_hip_config_version_4_u8(&sdev->hip.hip_control->config_v4, unidat_req_headroom);
		hip->hip_priv->unidat_req_tailroom =
			scsc_wifi_get_hip_config_version_4_u8(&sdev->hip.hip_control->config_v4, unidat_req_tailroom);
		hip->hip_priv->version = 4;
	} else {
		/* version 5 */
		hip->hip_priv->unidat_req_headroom =
			scsc_wifi_get_hip_config_version_5_u8(&sdev->hip.hip_control->config_v5, unidat_req_headroom);
		hip->hip_priv->unidat_req_tailroom =
			scsc_wifi_get_hip_config_version_5_u8(&sdev->hip.hip_control->config_v5, unidat_req_tailroom);
