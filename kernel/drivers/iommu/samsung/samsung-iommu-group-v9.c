...
/*
 * linux/drivers/iommu/samsung-iommu-group-v9.c
 *
...
static int sysmmu_group_device_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(data)) {
		dev_err(dev, "Failed to alloc group, ret:%ld\n", PTR_ERR(data));
		return PTR_ERR(data);
	}
...
static const struct of_device_id sysmmu_group_of_match[] = {
	{ .compatible = "samsung,sysmmu-group-v9" },
	{ }
};

static struct platform_driver sysmmu_group_driver_v9 = {
	.driver	= {
		.name			= "sysmmu-group",
...
module_platform_driver(sysmmu_group_driver_v9);
