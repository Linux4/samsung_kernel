...
void cpif_sysmmu_enable(void (*function)(bool))
{
	if (g_sysmmu_drvdata == NULL) {
	...
	g_sysmmu_drvdata->handle_cp_fault = function;
}
...
static irqreturn_t exynos_sysmmu_irq(int irq, void *dev_id)
{
	...
	if (addr == 0x08000000)
		g_sysmmu_drvdata->handle_cp_fault(true);
	else
		g_sysmmu_drvdata->handle_cp_fault(false);
...
tatic size_t exynos_iommu_unmap_once(unsigned long l_iova, size_t size)
{
	struct exynos_iommu_domain *domain = g_sysmmu_drvdata->domain;
	sysmmu_iova_t iova = (sysmmu_iova_t)l_iova;
	...
	for (i = 0; i < cnt; i++, pent++) {
		...
		refcnt_buf = (pent + NUM_LV2ENTRIES);
		if (*refcnt_buf != 0) {
			pr_err("VA : 0x%llx's ref conunt is not 0! - SKIP unmap\n");
			*refcnt_buf = *refcnt_buf - 1;
			continue;
		}
...
int cpif_iommu_map(unsigned long iova, phys_addr_t paddr, size_t size, int prot)
{
	...
	if (size < SZ_64K) {
		ret = exynos_iommu_map_once(iova, paddr, size, prot);
		goto end_map;
	}
	while (size) {
...
size_t cpif_iommu_unmap(unsigned long iova, size_t size)
{
	...
	spin_lock_irqsave(&domain->pgtablelock, flags);
	if (size < SZ_64K) {
		unmapped = exynos_iommu_unmap_once(iova, size);
		if (unmapped == size)
			goto end_unmap;
	}
	/*
