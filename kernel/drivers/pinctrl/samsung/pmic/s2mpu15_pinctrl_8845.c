...
static const unsigned int pmic_gpio_pins[] = {0, 1};

#define PMIC_GROUP_SIZE	1

static const struct pingroup pmic_pin_groups[] = {
	PINCTRL_PINGROUP("MAIN_PMIC_GPIO", pmic_gpio_pins, ARRAY_SIZE(pmic_gpio_pins)),
};
...
static int pmic_gpio_set_config(struct gpio_chip *chip, uint32_t pin, unsigned long config)
{
	...
	dev_err(state->dev, "[MAIN_PMIC] %s: pin%d: config(%#lx)\n", __func__, pin, config);
