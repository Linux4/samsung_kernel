...
#define S2MPM07_GPIO_CONF_DISABLE		(PIN_CONFIG_END + 1)
#define S2MPM07_GPIO_CONF_PULL_DOWN		(PIN_CONFIG_END + 2)
#define S2MPM07_GPIO_CONF_PULL_UP		(PIN_CONFIG_END + 3)
#define S2MPM07_GPIO_CONF_INPUT_ENABLE		(PIN_CONFIG_END + 4)
#define S2MPM07_GPIO_CONF_OUTPUT_ENABLE		(PIN_CONFIG_END + 5)
#define S2MPM07_GPIO_CONF_OUTPUT		(PIN_CONFIG_END + 6)
#define S2MPM07_GPIO_CONF_DRIVE_STRENGTH	(PIN_CONFIG_END + 7)

static const char *const s2mpm07_biases[] = {
...
static const struct pinconf_generic_params s2mpm07_gpio_bindings[] = {
	{"pmic-gpio,pull-disable",	S2MPM07_GPIO_CONF_DISABLE,		0},
	{"pmic-gpio,pull-down",		S2MPM07_GPIO_CONF_PULL_DOWN,	1},
	{"pmic-gpio,pull-up",		S2MPM07_GPIO_CONF_PULL_UP,		2},
	{"pmic-gpio,input-enable",	S2MPM07_GPIO_CONF_INPUT_ENABLE,	0},
	{"pmic-gpio,output-enable",	S2MPM07_GPIO_CONF_OUTPUT_ENABLE,	1},
	{"pmic-gpio,output-low",	S2MPM07_GPIO_CONF_OUTPUT,		0},
	{"pmic-gpio,output-high",	S2MPM07_GPIO_CONF_OUTPUT,		1},
	{"pmic-gpio,drive-strength",	S2MPM07_GPIO_CONF_DRIVE_STRENGTH,	0},
};

#ifdef CONFIG_DEBUG_FS
static const struct pin_config_item s2mpm07_pin_config_item[ARRAY_SIZE(s2mpm07_gpio_bindings)] = {
	// TODO:
	PCONFDUMP(S2MPM07_GPIO_CONF_DISABLE,	"pull-disable", NULL, true),
	PCONFDUMP(S2MPM07_GPIO_CONF_PULL_DOWN,	"pull-down", NULL, true),
	PCONFDUMP(S2MPM07_GPIO_CONF_PULL_UP,	"pull-up", NULL, true),
	PCONFDUMP(S2MPM07_GPIO_CONF_INPUT_ENABLE,	"input-enable", NULL, true),
	PCONFDUMP(S2MPM07_GPIO_CONF_OUTPUT_ENABLE,	"output-enable", NULL, true),
	PCONFDUMP(S2MPM07_GPIO_CONF_OUTPUT,	"output-high", NULL, true),
	PCONFDUMP(S2MPM07_GPIO_CONF_DRIVE_STRENGTH,"drive-strength", NULL, true),
};
#endif

struct s2mpm07_gpio_state {
	struct device	*dev;
...
/**
 * struct s2mpm07_gpio_pad - keep current GPIO settings
 ...
 * @function: See s2mpm07_gpio_functions[]
...
/* The index of each function in s2mpm07_gpio_functions[] array */
#define PMIC_GPIO_FUNC_NORMAL		"normal"

enum s2mpm07_gpio_func_index {
	PMIC_GPIO_FUNC_INDEX_NORMAL,
};

/* To be used with "function" */
static const char *const s2mpm07_gpio_functions[] = {
	[PMIC_GPIO_FUNC_INDEX_NORMAL]	= PMIC_GPIO_FUNC_NORMAL,
};

static int s2mpm07_gpio_read(struct s2mpm07_gpio_state *state, unsigned int addr)
{
	uint8_t val;
	int ret;
...
static void s2mpm07_vgpio_write(struct s2mpm07_gpio_state *state,
			   struct s2mpm07_gpio_pad *pad, unsigned int val)
{
	u32 reg = 0;
	u32 group = (pad->base + RF_GPIO_GROUP) / 4;
	u32 offset = (pad->base + RF_GPIO_GROUP) % 4;
...
static int s2mpm07_gpio_write(struct s2mpm07_gpio_state *state, unsigned int addr, unsigned int val)
{
	int ret;
...
static int s2mpm07_gpio_get_level(struct s2mpm07_gpio_pad *pad, unsigned int pin, int val)
{
	int ret = 0;
...
static int s2mpm07_gpio_get_pull(struct s2mpm07_gpio_pad *pad, int val)
{
	int ret = 0;
...
static int s2mpm07_gpio_get_mode(struct s2mpm07_gpio_pad *pad, int val)
{
	int ret = 0;
...
static int s2mpm07_gpio_get_str(struct s2mpm07_gpio_pad *pad, int val)
{
	int ret = 0;
...
static const char *s2mpm07_gpio_get_group_name(struct pinctrl_dev *pctldev,
					    unsigned pin)
{
	return pctldev->desc->pins[pin].name;
}

static int s2mpm07_gpio_get_group_pins(struct pinctrl_dev *pctldev, unsigned pin,
...
{
	struct s2mpm07_gpio_state *state = pinctrl_dev_get_drvdata(pctldev);
	struct s2mpm07_gpio_pad *pad = pctldev->desc->pins[pin].drv_data;
	...
	val_set = s2mpm07_gpio_read(state, reg_set);
	if (val_set < 0) {
		seq_printf(s, "s2mpm07_gpio_read fail(%d)", __LINE__);
		return;
	}

	val_status = s2mpm07_gpio_read(state, reg_status);
	if (val_status < 0) {
		seq_printf(s, "s2mpm07_gpio_read fail(%d)", __LINE__);
		return;
	}
	...
	/* Get info. */
	s2mpm07_gpio_get_pull(pad, val_set);
	s2mpm07_gpio_get_mode(pad, val_set);
	s2mpm07_gpio_get_str(pad, val_set);
	s2mpm07_gpio_get_level(pad, pin, val_status);

	/* Print info. */
	seq_printf(s, "%s(%#x) MODE(%s) DRV_STR(%s_%#x) DAT(%s_%#x)",
			s2mpm07_biases[pad->pullup], pad->pullup,
			pad->input_enabled ? "input" : "output",
			s2mpm07_strengths[pad->strength], pad->strength,
...
{
	return ARRAY_SIZE(s2mpm07_gpio_functions);
}

static const char *s2mpm07_gpio_get_function_name(struct pinctrl_dev *pctldev,
					       unsigned function)
{
	return s2mpm07_gpio_functions[function];
}
...
{
	*groups = s2mpm07_gpio_groups;
	*num_qgroups = pctldev->desc->npins;
	return 0;
}

static int s2mpm07_gpio_set_mux(struct pinctrl_dev *pctldev, unsigned function,
				unsigned pin)
{
	struct s2mpm07_gpio_state *state = pinctrl_dev_get_drvdata(pctldev);
	struct s2mpm07_gpio_pad *pad;
	unsigned int val;
	//int ret;
	...
	return s2mpm07_gpio_write(state, 1, val);//PMIC_GPIO_REG_EN_CTL, val);
}
					...
					unsigned pin, unsigned long *config)
{
	...
	switch (param) {
	case S2MPM07_GPIO_CONF_DISABLE:
	case S2MPM07_GPIO_CONF_PULL_DOWN:
	case S2MPM07_GPIO_CONF_PULL_UP:
		ret = s2mpm07_gpio_get_pull(pad, val_set);
		if (ret < 0)
			return ret;
		arg = pad->pullup;

		dev_info(state->dev, "[RF_PMIC] %s: pin%d: %s(%#x)\n",
					__func__, pin, s2mpm07_biases[pad->pullup], pad->pullup);
		break;
	case S2MPM07_GPIO_CONF_INPUT_ENABLE:
		ret = s2mpm07_gpio_get_mode(pad, val_set);
		if (ret < 0)
			return ret;
		...
		break;
	case S2MPM07_GPIO_CONF_OUTPUT_ENABLE:
		ret = s2mpm07_gpio_get_mode(pad, val_set);
		if (ret < 0)
			return ret;
	...
	case S2MPM07_GPIO_CONF_OUTPUT:
		...
		break;
	case S2MPM07_GPIO_CONF_DRIVE_STRENGTH:
		s2mpm07_gpio_get_str(pad, val_set);
		arg = pad->strength;

		dev_info(state->dev, "[RF_PMIC] %s: pin%d: DRV_STR(%s_%#x)\n",
					__func__, pin, s2mpm07_strengths[pad->strength], pad->strength);
		break;
	default:
		dev_err(state->dev, "[RF_PMIC] %s: s2mpm07_gpio_pin_config_get fail(param: %#x)\n",
				__func__, param);
		return -EINVAL;
	}
...
static int s2mpm07_gpio_pin_config_set(struct pinctrl_dev *pctldev, unsigned pin,
					unsigned long *configs, unsigned num_configs)
{
	...
	for (i = 0; i < num_configs; i++) {
		...
		case S2MPM07_GPIO_CONF_DRIVE_STRENGTH:
			...
			break;
		default:
			dev_err(state->dev, "[RF_PMIC] %s: s2mpm07_gpio_pin_config_set fail(param: %#x)\n",
					__func__, param);
			return -ENOTSUPP;
		}
	}

	ret = s2mpm07_gpio_write(state, reg_set, val_set);
	if (ret < 0) {
		dev_err(state->dev, "[RF_PMIC] %s: s2mpm07_gpio_write fail\n", __func__);
		return ret;
	}

	if (vgpio_flag)
		s2mpm07_vgpio_write(state, pad, vgpio_val);
...
static int s2mpm07_gpio_pin_config_group_set(struct pinctrl_dev *pctldev, unsigned selector,
				unsigned long *configs, unsigned num_configs)
{
	struct s2mpm07_gpio_state *state = pinctrl_dev_get_drvdata(pctldev);
	int i, ret = 0;
...
static const struct pinconf_ops s2mpm07_gpio_pinconf_ops = {
	...
	.pin_config_dbg_show		= s2mpm07_gpio_pin_dbg_show,
};

static int s2mpm07_gpio_direction_input(struct gpio_chip *chip, unsigned pin)
{
	struct s2mpm07_gpio_state *state = gpiochip_get_data(chip);
	...
	config = pinconf_to_config_packed(S2MPM07_GPIO_CONF_INPUT_ENABLE, 1);
	ret = s2mpm07_gpio_pin_config_set(state->ctrl, pin, &config, 1);
...
static int s2mpm07_gpio_direction_output(struct gpio_chip *chip,
				      unsigned pin, int val)
{
	...
	config = pinconf_to_config_packed(S2MPM07_GPIO_CONF_OUTPUT, val);
	ret = s2mpm07_gpio_pin_config_set(state->ctrl, pin, &config, 1);
...
static int s2mpm07_gpio_get(struct gpio_chip *chip, unsigned int pin)
{
	struct s2mpm07_gpio_state *state = gpiochip_get_data(chip);
	struct s2mpm07_gpio_pad *pad = state->ctrl->desc->pins[pin].drv_data;
	uint8_t reg_status = S2MPM07_REG_GPIO_STATUS1;
	...
	ret = s2mpm07_gpio_get_level(pad, pin, val_status);
...
static void s2mpm07_gpio_set(struct gpio_chip *chip, unsigned pin, int value)
{
	struct s2mpm07_gpio_state *state = gpiochip_get_data(chip);
	unsigned long config;
	...
	config = pinconf_to_config_packed(S2MPM07_GPIO_CONF_OUTPUT, value);
...
static int s2mpm07_gpio_set_config(struct gpio_chip *gc, unsigned int offset, unsigned long config)
{
	struct s2mpm07_gpio_state *state = gpiochip_get_data(gc);

	dev_err(state->dev, "[RF_PMIC] %s: pin%d: config(%#lx)\n", __func__, offset, config);

	return s2mpm07_gpio_pin_config_set(state->ctrl, offset, &config, 1);
}
...
static int s2mpm07_gpio_populate(struct s2mpm07_gpio_state *state,
			      struct s2mpm07_gpio_pad *pad, unsigned int pin)
{
	uint8_t reg_set = S2MPM07_PMIC_GPIO_SET0 + pin;
	uint8_t reg_status = S2MPM07_REG_GPIO_STATUS1;
	int ret = 0, val_set = 0, val_status = 0;
...
//				      unsigned int *type)
//{
//	struct s2mpm07_gpio_state *state = container_of(domain->host_data,
//						     struct s2mpm07_gpio_state,
//						     chip);
//
...
static int s2mpm07_gpio_parse_dt(struct s2mpm07_dev *iodev, struct s2mpm07_gpio_state *state)
{
...
static int s2mpm07_gpio_init(struct s2mpm07_dev *iodev)
{
	int ret;

	ret = s2mpm07_update_reg(iodev->com, S2MPM07_REG_COM_CTRL1, 0x0, 0x1);
	if (ret < 0) {
...
static int s2mpm07_gpio_probe(struct platform_device *pdev)
{
	...
	struct s2mpm07_gpio_pad *pad, *pads;
	struct s2mpm07_gpio_state *state;
	...
	ret = s2mpm07_gpio_parse_dt(iodev, state);
	if (ret < 0) {
		dev_err(state->dev, "[RF_PMIC] %s: s2mpm07_gpio_parse_dt fail\n", __func__);
		ret = -ENODEV;
	...
	pctrldesc->pctlops = &s2mpm07_gpio_pinctrl_ops;
	pctrldesc->pmxops = &s2mpm07_gpio_pinmux_ops;
	pctrldesc->confops = &s2mpm07_gpio_pinconf_ops;
	pctrldesc->owner = THIS_MODULE;
	pctrldesc->name = dev_name(dev);
	pctrldesc->pins = pindesc;
	pctrldesc->npins = state->npins;
	pctrldesc->num_custom_params = ARRAY_SIZE(s2mpm07_gpio_bindings);
	pctrldesc->custom_params = s2mpm07_gpio_bindings;
#ifdef CONFIG_DEBUG_FS
	pctrldesc->custom_conf_items = s2mpm07_pin_config_item;
#endif
	...
	for (i = 0; i < state->npins; i++, pindesc++) {
		...
		pindesc->number = i;
		pindesc->name = s2mpm07_gpio_groups[i];

		pad->base = i;

		ret = s2mpm07_gpio_populate(state, pad, i);
		if (ret < 0)
			goto err_sysreg_unmap;
	}
...
static int s2mpm07_gpio_remove(struct platform_device *pdev)
{
	struct s2mpm07_gpio_state *state = platform_get_drvdata(pdev);
