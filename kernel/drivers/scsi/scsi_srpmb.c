...
static int advanced_srpmb_execute_cmd(ARpmb_Req *areq, char *cmd)
{
	...
	bjob->request_len = sizeof(struct ufs_rpmb_request);
	bjob->reply = &rpmb_reply;
	bjob->reply_len = sizeof(struct ufs_rpmb_reply);
...
static void advanced_srpmb_worker(struct work_struct *data)
{
	...
	dev_dbg(&sr_pdev->dev, "start adv rpmb workqueue with command(%d)\n", areq->type);

	switch (areq->type) {
	case GET_WRITE_COUNTER:
		if (areq->data_len) {
			update_arpmb_status_flag(rpmb_ctx, areq, WRITE_COUTNER_DATA_LEN_ERROR);
			dev_err(&sr_pdev->dev, "data len is invalid for GET_WRITE_COUNTER\n");
			break;
		}
		...
		if (ret < 0) {
			...
			dev_err(&sr_pdev->dev, "ioctl get write counter error : %x\n", ret);
			break;
		}

		if (areq->meta.result)
			dev_info(&sr_pdev->dev, "GET_WRITE_COUNTER: REQ/RES = 0x%x, RESULT = 0x%x\n",
	...
	case WRITE_DATA:
		if (areq->data_len < ARPMB_DATA_SIZE || areq->data_len > ARPMB_DATA_SIZE * 32) {
			...
			dev_err(&sr_pdev->dev, "data len is invalid for WRITE_DATA\n");
			break;
		}
		...
		if (areq->meta.result)
			dev_info(&sr_pdev->dev, "WRITE_DATA: REQ/RES = 0x%x, RESULT = 0x%x\n",
					areq->meta.req_resp_type, areq->meta.result);
	...
	case READ_DATA:
		if (areq->data_len < ARPMB_DATA_SIZE || areq->data_len > ARPMB_DATA_SIZE * 32) {
			/* intiallly set as 32 need to be change */
			dev_err(&sr_pdev->dev, "data len is invalid for READ_DATA\n");
			break;
		}
		...
		if (ret < 0) {
			...
			dev_err(&sr_pdev->dev, "ioctl result read data error : %x\n", ret);
			break;
		}

		if (areq->meta.result)
			dev_info(&sr_pdev->dev, "READ_DATA: REQ/RES = 0x%x, RESULT = 0x%x\n",
					areq->meta.req_resp_type, areq->meta.result);
	...
	dev_dbg(&sr_pdev->dev, "finish adv rpmb workqueue with command(%d)\n", areq->type);
}
				...
				void *dummy)
{
	...
	dev_info(&sr_pdev->dev, "%s: event = 0x%lx\n", __func__, event);
	...
	case PM_RESTORE_PREPARE:
		...
		if (req->status_flag != RPMB_PASSED) {
			dev_info(&sr_pdev->dev, "%s: prev status_flag = 0x%x\n",
		...
		unsigned long RPMB_ReadWriteSize)
{
	...
	if (AdvancedRPMBSupport) {
		if ((RPMB_ReadWriteSize == 0) ||
				(RPMB_ReadWriteSize * ARPMB_DATA_SIZE > RPMB_BUF_MAX_SIZE))
			rpmb_ctx->vir_addr = dma_alloc_coherent(&sr_pdev->dev,
					sizeof(ARpmb_Req) + RPMB_BUF_MAX_SIZE,
					&rpmb_ctx->phy_addr, GFP_KERNEL);
		else
			rpmb_ctx->vir_addr = dma_alloc_coherent(&sr_pdev->dev,
					sizeof(ARpmb_Req) + (RPMB_ReadWriteSize * ARPMB_DATA_SIZE),
					&rpmb_ctx->phy_addr, GFP_KERNEL);
	} else {
		rpmb_ctx->vir_addr = dma_alloc_coherent(&sr_pdev->dev,
