...
#define I3C_IBI_STAT_THLD_MASK			GENMASK(31, 24)
...
static int exynos_i3c_hci_master_attach_i2c_dev(struct i2c_dev_desc *dev)
{
	...
	dat = I3C_DEVICE_TYPE |
		...
		I3C_STATIC_ADDRESS(dev->addr);
	dev_dbg(master->dev, "DAT(%u) : %x\n", slot, dat);
	writel(dat, master->regs + I3C_DEV_ADDR_TABLE(data->id));
...
static int exynos_i3c_hci_parse_dt(struct exynos_i3c_hci_master *i3c, struct device *dev)
{
	...
	i3c->ibi_status_thld = -1;
	if (of_property_read_u32(dev->of_node, "samsung,ibi_status_thld", &temp) == 0)
		i3c->ibi_status_thld = temp;
					...
					struct i3c_ccc_cmd *cmd)
{
	...
	if (cmd->id == I3C_CCC_ENTDAA) {
		if (master->dat_idx < 0 || master->dat_idx >= master->maxdevs){
			dev_err(master->dev, "Invalid DAT Index %d\n", master->dat_idx);
			return -EINVAL;
		}
...
static int exynos_i3c_hci_master_clk_cfg(struct exynos_i3c_hci_master *master)
{
	...
	ipclk_rate = clk_get_rate(master->ipclk);
	if (!ipclk_rate) {
		dev_err(master->dev, "Failed to get clock\n");
		return -EINVAL;
	}
...
out:
	...
	if (!data_consumed) {
		for (i = 0; i < I3C_DATA_LENGTH(ibiq); i += 4)
...
static void exynos_i3c_hci_master_demux_ibis(struct exynos_i3c_hci_master *master)
{
	...
	for (queue_status = readl(master->regs + I3C_QUEUE_STATUS_LEVEL);
		...
		dev_dbg(master->dev, "Queue_lvl 0x%x IBIQ 0x%x\n", queue_status, ibiq);
		switch(I3C_IBI_ID(ibiq)) {
			case 0x4:
				/* HJ Case */
				// W/R for invalid IBI
				writel(I3C_IBI_QUEUE_RST, master->regs + I3C_RESET_CTRL);
...
static void exynos_i3c_hci_master_hj(struct work_struct *work)
{
	...
	dev_info(master->dev, "Hot join! Start DAA..\n");
	i3c_master_do_daa(&master->base);
	dev_info(master->dev, "Hot join done..\n");
}
...
static int exynos_i3c_hci_probe(struct platform_device *pdev)
{
	...
	/* IBI status threshold */
	if (master->ibi_status_thld >= 0) {
		queue_thld = readl(master->regs + I3C_QUEUE_THLD_CTRL);
		queue_thld &= ~I3C_IBI_STAT_THLD_MASK;
		queue_thld |= I3C_IBI_STAT_QUEUE_THLD(master->ibi_status_thld);
		writel(queue_thld, master->regs + I3C_QUEUE_THLD_CTRL);
	}
