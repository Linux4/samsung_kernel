...
#define MAX77775_MAX_APDCMD_TIME (10*HZ)
...
static void max77775_usbc_handle_noirq_opcode(struct max77775_usbc_platform_data *usbc_data, usbc_cmd_data cmd_data)
{
	int ret;
	usbc_cmd_queue_t *cmd_queue = NULL;
...
static void max77775_usbc_cmd_run(struct max77775_usbc_platform_data *usbc_data)
{
	...
	if (is_usbc_notifier_opcode(cmd_data.opcode)) {
		...
		max77775_usbc_handle_noirq_opcode(usbc_data, cmd_data);
		max77775_usbc_cmd_run(usbc_data);
	} else { /* No ISR */
		msg_maxim("No ISR");
