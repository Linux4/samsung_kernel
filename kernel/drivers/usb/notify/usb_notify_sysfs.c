		...
		const char *buf, size_t size)
{
	...
	if (sret != 1)
		goto error1;
	unl_info("allowlist_for_mdm_store buf=%s\n", disable);
...
void init_usb_whitelist_array_for_id(int *whitelist_array)
{
	int i;

	for (i = 0; i < MAX_WHITELIST_STR_LEN; i++)
		whitelist_array[i] = 0;
}
...
int set_usb_allowlist_array_for_id(const char *buf, int *whitelist_array)
{
	...
	while ((ptr_vid = strsep(&source, ":")) != NULL) {
		...
		unl_info("%s ptr_vid_token = %c%c%c%c!\n", __func__,
			ptr_vid[0], ptr_vid[1], ptr_vid[2], ptr_vid[3]);
		...
		if (ptr_pid != NULL) {
			unl_info("%s ptr_pid_token = %c%c%c%c!\n", __func__,
		...
		const char *buf, size_t size)
{
	...
	init_usb_whitelist_array_for_id(udev->whitelist_array_for_mdm_for_id);
	/* To active displayport, hub class must be enabled */
