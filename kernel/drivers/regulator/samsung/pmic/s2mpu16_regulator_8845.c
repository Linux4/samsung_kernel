...
static unsigned int s2mpu16_of_map_mode(unsigned int mode) {
	switch (mode) {
...
static int s2mpu16_set_mode(struct regulator_dev *rdev,
		unsigned int mode)
{
	...
	switch (mode) {
		case PMIC_OPMODE_VGPIO ... PMIC_OPMODE_ON:
			...
			return 0;
		case PMIC_OPMODE_AUTO:
			mode = S2MPU16_BUCK_AUTO_MODE;
			break;
		case PMIC_OPMODE_FCCM:
			mode = S2MPU16_BUCK_FCCM_MODE;
			break;
		default:
	...
	if (S2MPU16_BUCK2 <= id && id <= S2MPU16_BUCK_SR1)
		return s2mpu16_update_reg(s2mpu16->i2c, rdev->desc->enable_reg,
				mode, S2MPU16_BUCK_MODE_MASK);
...
static unsigned int s2mpu16_get_mode(struct regulator_dev *rdev)
{
	...
	ret = s2mpu16_read_reg(s2mpu16->i2c, rdev->desc->enable_reg, &val);
	if (ret)
		return REGULATOR_MODE_INVALID;

	dev_info(s2mpu16->iodev->dev, "%s: [%s] enable_reg: 0x%02hhx, val: 0x%02hhx\n",
			__func__, rdev->desc->name, rdev->desc->enable_reg, val);

	val &= S2MPU16_BUCK_MODE_MASK;

	if (val == S2MPU16_BUCK_AUTO_MODE)
		ret = PMIC_OPMODE_AUTO;
	else if (val == S2MPU16_BUCK_FCCM_MODE)
...
static struct regulator_desc regulators[S2MPU16_REG_MAX] = {
	...
	//BUCK_DESC("BUCK1", _BUCK(1), &_buck_ops(), 1, _REG(_BUCK1S_OUT1), _REG(_BUCK1S_CTRL), _TIME(_BUCK)),
	...
	//BUCK_DESC("BUCK9", _BUCK(9), &_buck_ops(), 1, _REG(_BUCK9S_OUT1), _REG(_BUCK9S_CTRL), _TIME(_BUCK)),
...
static const struct s2mpu16_oi_data s2mpu16_oi[] = {
	...
	DECLARE_OI(S2MPU16_PM1_BUCK1S_OCP, 0xCC),
	DECLARE_OI(S2MPU16_PM1_BUCK2S_OCP, 0xCC),
	DECLARE_OI(S2MPU16_PM1_BUCK3S_OCP, 0xCC),
	DECLARE_OI(S2MPU16_PM1_BUCK4S_OCP, 0xCC),
	DECLARE_OI(S2MPU16_PM1_BUCK5S_OCP, 0xCC),
	DECLARE_OI(S2MPU16_PM1_BUCK6S_OCP, 0xCC),
	DECLARE_OI(S2MPU16_PM1_BUCK7S_OCP, 0xCC),
	DECLARE_OI(S2MPU16_PM1_BUCK8S_OCP, 0xCC),
	DECLARE_OI(S2MPU16_PM1_BUCK9S_OCP, 0xCC),
	DECLARE_OI(S2MPU16_PM1_BUCK10S_OCP, 0xCC),
	DECLARE_OI(S2MPU16_PM1_SR1S_OCP, 0xCC),
};
				  ...
				  const char *buf, size_t size)
{
	...
	ret = exynos_esca_read_reg(esca_mfd_node, SUB_CHANNEL, base_addr, reg_addr, &val);
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
	mutex_lock(&s2mpu16->iodev->i2c_lock);
	ret = exynos_esca_write_reg(esca_mfd_node, SUB_CHANNEL, base_addr, reg_addr, val);
	mutex_unlock(&s2mpu16->iodev->i2c_lock);
	if (ret)
