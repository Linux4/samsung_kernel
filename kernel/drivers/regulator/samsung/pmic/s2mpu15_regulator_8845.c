...
static unsigned int s2mpu15_of_map_mode(unsigned int mode) {
	switch (mode) {
...
static int s2mpu15_set_mode(struct regulator_dev *rdev,
		unsigned int mode)
{
	...
	switch (mode) {
		case PMIC_OPMODE_VGPIO ... PMIC_OPMODE_ON:
			...
			return 0;
		case PMIC_OPMODE_AUTO:
			mode = S2MPU15_BUCK_AUTO_MODE;
			break;
		case PMIC_OPMODE_FCCM:
			mode = S2MPU15_BUCK_FCCM_MODE;
			break;
		default:
...
static unsigned int s2mpu15_get_mode(struct regulator_dev *rdev)
{
	...
	ret = s2mpu15_read_reg(s2mpu15->i2c, rdev->desc->enable_reg, &val);
	if (ret)
		return REGULATOR_MODE_INVALID;

	dev_info(s2mpu15->iodev->dev, "%s: [%s] enable_reg: 0x%02hhx, val: 0x%02hhx\n",
			__func__, rdev->desc->name, rdev->desc->enable_reg, val);

	val &= S2MPU15_BUCK_MODE_MASK;

	if (val == S2MPU15_BUCK_AUTO_MODE)
		ret = PMIC_OPMODE_AUTO;
	else if (val == S2MPU15_BUCK_FCCM_MODE)
...
int main_pmic_mem_sel_status(void)
{
	uint8_t val;
	struct s2mpu15_info *s2mpu15 = s2mpu15_static_info;

	s2mpu15_read_reg(s2mpu15->i2c, S2MPU15_PM1_MEM_SEL, &val);
	pr_info("%s: 0x%02hhx\n", __func__, val);

	return (val & S2MPU15_MEM_SEL_STATUS);
}
EXPORT_SYMBOL_GPL(main_pmic_mem_sel_status);
...
static struct regulator_desc regulators[S2MPU15_REG_MAX] = {
	...
	BUCK_DESC("BUCK2", _BUCK(2), &_buck_ops(), 1, _REG(_BUCK2M_OUT2), _REG(_BUCK2M_CTRL), _TIME(_BUCK)),
	...
	BUCK_DESC("BUCK5", _BUCK(5), &_buck_ops(), 1, _REG(_BUCK5M_OUT2), _REG(_BUCK5M_CTRL), _TIME(_BUCK)),
	BUCK_DESC("BUCK6", _BUCK(6), &_buck_ops(), 1, _REG(_BUCK6M_OUT1), _REG(_BUCK6M_CTRL), _TIME(_BUCK)),
	BUCK_DESC("BUCK7", _BUCK(7), &_buck_ops(), 1, _REG(_BUCK7M_OUT1), _REG(_BUCK7M_CTRL), _TIME(_BUCK)),
...
#if IS_ENABLED(CONFIG_SEC_DEBUG_EXTRA_INFO)
	secdbg_exin_set_main_ocp_count(OCP_OI_COUNTER_TYPE_OCP);
#endif
...
#if IS_ENABLED(CONFIG_SEC_DEBUG_EXTRA_INFO)
	secdbg_exin_set_main_ocp_count(OCP_OI_COUNTER_TYPE_OI);
#endif
...
static const struct s2mpu15_oi_data s2mpu15_oi[] = {
	/* BUCK 1 to SR4M OI,OCP function enable, OI power down,OVP disable */
	/* OI detection time window : 300us, OI comp. output count : 50 times */
	DECLARE_OI(S2MPU15_PM1_BUCK1M_OCP, 0xCC),
	DECLARE_OI(S2MPU15_PM1_BUCK2M_OCP, 0xCC),
	DECLARE_OI(S2MPU15_PM1_BUCK3M_OCP, 0xCC),
	DECLARE_OI(S2MPU15_PM1_BUCK4M_OCP, 0xCC),
	DECLARE_OI(S2MPU15_PM1_BUCK5M_OCP, 0xCC),
	DECLARE_OI(S2MPU15_PM1_BUCK6M_OCP, 0xCC),
	DECLARE_OI(S2MPU15_PM1_BUCK7M_OCP, 0xCC),
	DECLARE_OI(S2MPU15_PM1_BUCK8M_OCP, 0xCC),
	DECLARE_OI(S2MPU15_PM1_BUCK_SR1M_OCP, 0xCC),
	DECLARE_OI(S2MPU15_PM1_BUCK_SR2M_OCP, 0xCC),
	DECLARE_OI(S2MPU15_PM1_BUCK_SR3M_OCP, 0xCC),
	DECLARE_OI(S2MPU15_PM1_BUCK_SR4M_OCP, 0xCC),
};
				  ...
				  const char *buf, size_t size)
{
	...
	ret = exynos_esca_read_reg(esca_mfd_node, MAIN_CHANNEL, base_addr, reg_addr, &val);
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCAV2)
	mutex_lock(&s2mpu15->iodev->i2c_lock);
	ret = exynos_esca_write_reg(esca_mfd_node, MAIN_CHANNEL, base_addr, reg_addr, val);
	mutex_unlock(&s2mpu15->iodev->i2c_lock);
	if (ret)
...
static int jig_acok_update(struct s2mpu15_info *s2mpu15)
{
	uint8_t val[2];
	int ret = 0;

	if (s2mpu15->iodev->inst_acok_en) {
		ret = s2mpu15_update_reg(s2mpu15->i2c, S2MPU15_PM1_CFG_PM,
				S2MPU15_INST_ACOK_EN, S2MPU15_INST_ACOK_EN);
		if (ret) {
	...
	if (s2mpu15->iodev->jig_reboot_en) {
		ret = s2mpu15_update_reg(s2mpu15->i2c, S2MPU15_PM1_TIME_CTRL,
				S2MPU15_JIG_REBOOT_EN, S2MPU15_JIG_REBOOT_EN);
		if (ret) {
	...
	ret = s2mpu15_read_reg(s2mpu15->i2c, S2MPU15_PM1_CFG_PM, &val[0]);
	if (ret) {
		pr_err("%s: CFG_PM read faile\n", __func__);
		return -EINVAL;
	}

	ret = s2mpu15_read_reg(s2mpu15->i2c, S2MPU15_PM1_TIME_CTRL, &val[1]);
	if (ret) {
...
static int s2mpu15_power_off(void)
{
	struct s2mpu15_info *s2mpu15 = s2mpu15_static_info;
	int ret = 0;

	if (!s2mpu15 || !s2mpu15->iodev)
		return -ENODEV;

	ret = jig_acok_update(s2mpu15);
