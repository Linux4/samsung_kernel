...
#include <linux/samsung/pmic/s2mpm07-regulator-9945.h>
...
static int s2mpm07_set_mode(struct regulator_dev *rdev,
				     unsigned int mode)
{
...
static int s2mpm07_set_voltage_sel_regmap_buck(struct regulator_dev *rdev, unsigned sel)
{
	struct s2mpm07_info *s2mpm07 = rdev_get_drvdata(rdev);
	int ret = 0;
...
static struct regulator_ops s2mpm07_buck_ops = {
	...
	.disable		= s2mpm07_disable_regmap,
	.get_voltage_sel	= s2mpm07_get_voltage_sel_regmap,
	.set_voltage_sel	= s2mpm07_set_voltage_sel_regmap_buck,
	.set_voltage_time_sel	= s2mpm07_set_voltage_time_sel,
	.set_mode		= s2mpm07_set_mode,
				  ...
				  const char *buf, size_t size)
{
	...
	ret = exynos_acpm_read_reg(acpm_mfd_node, RF_ID, base_addr, reg_addr, &val);
...
#if IS_ENABLED(CONFIG_EXYNOS_ACPM)
	mutex_lock(&s2mpm07->iodev->i2c_lock);
	ret = exynos_acpm_write_reg(acpm_mfd_node, RF_ID, base_addr, reg_addr, data);
	mutex_unlock(&s2mpm07->iodev->i2c_lock);
	if (ret)
...
static struct pmic_device_attribute regulator_attr[] = {
	PMIC_ATTR(write, S_IRUGO | S_IWUSR, s2mpm07_write_show, s2mpm07_write_store),
	PMIC_ATTR(read, S_IRUGO | S_IWUSR, s2mpm07_read_show, s2mpm07_read_store),
};
