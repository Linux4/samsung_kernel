...
static int s2mpa05_set_mode(struct regulator_dev *rdev,
			unsigned int mode)
{
...
static int s2mpa05_set_voltage_sel_regmap_buck(struct regulator_dev *rdev, unsigned sel)
{
	struct s2mpa05_info *s2mpa05 = rdev_get_drvdata(rdev);
	int ret = 0;
...
static struct regulator_ops s2mpa05_buck_ops = {
	...
	.disable		= s2mpa05_disable_regmap,
	.get_voltage_sel	= s2mpa05_get_voltage_sel_regmap,
	.set_voltage_sel	= s2mpa05_set_voltage_sel_regmap_buck,
	.set_voltage_time_sel	= s2mpa05_set_voltage_time_sel,
	.set_mode		= s2mpa05_set_mode,
...
static const struct s2mpa05_oi_data s2mpa05_oi[] = {
	...
	DECLARE_OI(S2MPA05_PM1_BUCK1_OCP, 0xCC),
	DECLARE_OI(S2MPA05_PM1_BUCK2_OCP, 0xCC),
	DECLARE_OI(S2MPA05_PM1_BUCK3_OCP, 0xCC),
	DECLARE_OI(S2MPA05_PM1_BUCK4_OCP, 0xCC),
};
				  ...
				  const char *buf, size_t size)
{
	...
	ret = exynos_acpm_read_reg(acpm_mfd_node, EXTRA_ID, base_addr, reg_addr, &val);
...
#if IS_ENABLED(CONFIG_EXYNOS_ACPM)
	mutex_lock(&s2mpa05->iodev->i2c_lock);
	ret = exynos_acpm_write_reg(acpm_mfd_node, EXTRA_ID, base_addr, reg_addr, val);
	mutex_unlock(&s2mpa05->iodev->i2c_lock);
	if (ret)
