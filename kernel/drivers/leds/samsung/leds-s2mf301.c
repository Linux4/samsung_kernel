...
static int s2mf301_fled_set_mode(struct s2mf301_fled_data *fled, int chan, int mode)
{
	...
	case S2MF301_FLED_MODE_FLASH:
		...
		ret = power_supply_set_property(fled->psy_chg, POWER_SUPPLY_PROP_ENERGY_AVG, &value);
		if (ret < 0)
			pr_err("%s: Fail to execute property(ENERGY_AVG)\n", __func__);
#endif
		break;
	case S2MF301_FLED_MODE_TORCH:
		s2mf301_update_reg(fled->chg, 0x33, 0x01, 0x01);
		pr_info("%s, Torch on\n", __func__);
	...
	if (mode == S2MF301_FLED_MODE_OFF) {
		...
		if (status & S2MF301_CH1_TORCH_ON) {
			pr_info("%s, Torch off\n", __func__);
			s2mf301_update_reg(fled->chg, 0x33, 0x00, 0x01);
		}
#if IS_ENABLED(CONFIG_S2MF301X)
		...
		if ((status & S2MF301_CH1_FLASH_ON) && (chg_sts & 1 << 3)) {
			if (!fled->psy_chg)
				fled->psy_chg = power_supply_get_by_name("s2mf301-charger");
			if (!fled->psy_chg) {
				pr_err("%s: Fail to get name(s2mf301-charger)\n", __func__);
				return -EINVAL;
			}
			if (fled->is_en_flash) {
