/*
  ...
  **************************************************************************
  *                                                                        *
  *                     Utilities published in /proc/fts_i2c		*
  *                                                                        *
  **************************************************************************
  **************************************************************************
  *
...
static struct stm_ts_data *proc_info = NULL;
...
static u8 driver_test_buff[MAX_PRINT_SIZE] = {0}; /* /< pointer to an array of bytes used to store the result of the function executed */
				     ...
				     size_t count, loff_t *pos)
{
	...
	cmd = (u8 *)kzalloc(count * sizeof(u8), GFP_KERNEL);
	funcToTest = (u32 *)kzalloc((((count + 1) / 3) * sizeof(u32)), GFP_KERNEL);
	...
	if (numberParam >= 1) {
		...
		case CMD_WRITEREAD_BYTE:
			if (numberParam >= 5) {	/* need to pass: cmd[0]  cmd[1] … cmd[cmdLength-1] byteToRead1 byteToRead0 dummyByte */
				...
				proc_info->stm_ts_read(proc_info, &cmd[1], temp, &readData[0], byteToRead + mess.dummy);
		...
		case CMD_WRITE_BYTE:
			if (numberParam >= 2) {	/* need to pass: cmd[0]  cmd[1] … cmd[cmdLength-1] */
				temp = numberParam - 1;
				proc_info->stm_ts_write(proc_info, &cmd[1], temp);
				res = OK;
			} else {
				pr_info("%s Wrong number of parameters!\n", tag);
				res = ERROR_OP_NOT_ALLOW;
...
int stm_ts_tool_proc_init(struct stm_ts_data *ts)
{
	...
	proc_info = (struct stm_ts_data *)ts;

	fts_dir = proc_mkdir_data("fts_i2c", 0777, NULL, NULL);
	...
	return OK;
badfile:
	remove_proc_entry("fts_i2c", NULL);
out:
	return retval;
}
