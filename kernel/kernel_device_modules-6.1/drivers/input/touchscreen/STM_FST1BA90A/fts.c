					    ...
					    char *event)
{
	...
	p_event_coord = (struct stm_ts_event_coordinate *)event;

	touchType = p_event_coord->ttype_1_0;
	...
	z = p_event_coord->z & 0x3F; //PRESSURE_MAX;
...
static void stm_ts_status_event(struct fts_ts_info *info, u8 *event_buff)
{
	...
	if (p_event_status->stype > 0)
		logError(0,
				 "%s %s STATUS event %02X %02X %02X %02X %02X %02X %02X %02X\n",
				 tag,
				 __func__, event_buff[0], event_buff[1], event_buff[2], event_buff[3], event_buff[4], event_buff[5], event_buff[6], event_buff[7]);
}

static void stm_ts_coordinate_event(struct fts_ts_info *info, u8 *event_buff)
{
	struct stm_ts_event_coordinate *p_event_coord;

	p_event_coord = (struct stm_ts_event_coordinate *)event_buff;

	if ((p_event_coord->tchsta == STM_TS_COORDINATE_ACTION_PRESS) || (p_event_coord->tchsta == STM_TS_COORDINATE_ACTION_MOVE))
	{
		fts_enter_pointer_event_handler(info, event_buff);
	}
	else if (p_event_coord->tchsta == STM_TS_COORDINATE_ACTION_RELEASE)
	{
		fts_leave_pointer_event_handler(info, event_buff);
	}
}

static void stm_ts_gesture_event(struct fts_ts_info *info, u8 *event_buff)
{
	logError(0,
				 "%s %s GESTURE event %02X %02X %02X %02X %02X %02X %02X %02X\n",
				 tag,
				 __func__, event_buff[0], event_buff[1], event_buff[2], event_buff[3], event_buff[4], event_buff[5], event_buff[6], event_buff[7]);
}

static u8 stm_ts_event_handler_type_b(struct fts_ts_info *info)
{
	u8 regAdd;
	int left_event_count = 0;
	int EventNum = 0;
	u8 event_id = 0;
	u8 data[FIFO_DEPTH * FIFO_EVENT_SIZE] = {0};
	u8 *event_buff;

	regAdd = 0x60;
	fts_writeReadU8UX(regAdd, 0, 0, (u8 *)&data[0 * FIFO_EVENT_SIZE], FIFO_EVENT_SIZE, DUMMY_FIFO);	
	left_event_count = (data[7] & 0x3F);

	if (left_event_count >= FIFO_DEPTH)
		left_event_count = FIFO_DEPTH - 1;

	if (left_event_count > 0)
	{
		regAdd = FIFO_CMD_READONE;
		fts_writeReadU8UX(regAdd, 0, 0, (u8 *)&data[1 * FIFO_EVENT_SIZE], FIFO_EVENT_SIZE * (left_event_count), DUMMY_FIFO);
	}

	do
	{
		event_buff = (u8 *)&data[EventNum * FIFO_EVENT_SIZE];
		event_id = event_buff[0] & 0x3;

		switch (event_id)
		{
		case STM_TS_STATUS_EVENT:
			stm_ts_status_event(info, event_buff);
			break;
		case STM_TS_COORDINATE_EVENT:
			stm_ts_coordinate_event(info, event_buff);
			break;
		case STM_TS_GESTURE_EVENT:
			stm_ts_gesture_event(info, event_buff);
			break;
		case STM_TS_VENDOR_EVENT: // just print message for debugging
#ifdef FTS_REMOVE_TEST
			if (event_buff[1] == 0x01)
			{ // echo event
...
static void fts_event_handler(struct work_struct *work)
{
	...
	stm_ts_event_handler_type_b(info);
