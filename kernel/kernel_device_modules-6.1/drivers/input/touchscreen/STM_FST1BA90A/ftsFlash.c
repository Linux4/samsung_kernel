...
int parseBinFile(u8 *fw_data, int fw_size, Firmware *fwData, int keep_cx)
{
	...
	if (fw_size < FW_HEADER_SIZE + FW_BYTES_ALLIGN || fw_data == NULL) {
		...
		memcpy(fwData->data, &fw_data[FW_HEADER_SIZE], dimension);		
		fwData->data_size = dimension;
		
		logError(0, "%s Using default flash Address\n", tag);
		fwData->code_start_addr = CODE_ADDR_START;
		fwData->cx_start_addr = CX_ADDR_START;
		fwData->config_start_addr = CONFIG_ADDR_START;
...
int flash_burn(Firmware fw, int force_burn, int keep_cx)
{
	...
	res = flash_erase_page_by_page(ERASE_ALL, &fw);
	if (res < OK) {
	...
	res = fillFlash(fw.config_start_addr, &(fw.data[fw.sec0_size]), fw.sec1_size);
	if (res < OK) {
