				     ...
				     size_t count, loff_t *pos)
{
	...
	if (access_ok(buf, count) < OK || copy_from_user(pbuf, buf, count) != 0) {
		pr_info("%s: access NG or copy from user failed\n", __func__);
		res = ERROR_ALLOC;
		goto END;
	}
	...
	if (count > MESSAGE_MIN_HEADER_SIZE - 1 && p[0] == MESSAGE_START_BYTE) {
		...
		if (count < mess.msg_size || p[count - 2] != MESSAGE_END_BYTE) {
			...
			pr_info("%s: %d: numberParam:%d, size: %d\n", __func__, __LINE__, numberParam, size);
			switch (mess.action) {
			case ACTION_WRITE:
			...
			pr_info("%s: %d: switch:%d, cmd[0]: %d\n", __func__, __LINE__, mess.action, cmd[0]);
			if (numberParam - 1 != 0)
		...
		pr_info("%s: %d: else\n", __func__, __LINE__);
		if (((count + 1) / 3) >= 1) {
			pr_info("%s: %d: ((count + 1) / 3): %d\n", __func__, __LINE__, (int)((count + 1) / 3));
	...
	/* elaborate input */
	pr_info("%s: %d: numberParam: %d, funcToTest[0]: %d\n", __func__, __LINE__, numberParam, funcToTest[0]);
	if (numberParam >= 1) {
		switch (funcToTest[0]) {
...
END:
	...
	pr_info("%s: %d: vyte_call: %d, size: %d\n", __func__, __LINE__, byte_call, size);
	/* start byte */
