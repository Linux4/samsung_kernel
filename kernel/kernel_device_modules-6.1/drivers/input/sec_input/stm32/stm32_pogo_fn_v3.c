...
void stm32_print_info(struct stm32_dev *stm32)
{
	...
	input_info(true, &stm32->client->dev, "mcu_fw(bin):%x, mcu_fw(ic):%x, %s_v%d.%d.%d.%d\n",
			stm32->mdata.phone_ver[3], stm32->mdata.ic_ver[3], stm32->model_name,
...
void stm32_set_model(struct stm32_dev *stm32)
{
	int i;

	stm32->model_info = NULL;

	input_info(true, &stm32->client->dev, "%s: model: %02X, model_type: %d\n",
			__func__, stm32->keyboard_model, stm32->model_type);

	if (stm32->keyboard_model == 0) {
		stm32->model_name = NULL;
		return;
	} else if (stm32->keyboard_model == STM32_KEYBOARD_ROW_DATA_MODEL) {
		if (stm32->ic_fw_ver.model_id < ROW_MODEL_LIST_SIZE) {
			stm32->model_info = &row_model_list[stm32->model_type][stm32->ic_fw_ver.model_id];
		} else {
			input_err(true, &stm32->client->dev, "%s: wrong model id %d\n",
					__func__, stm32->ic_fw_ver.model_id);
		}
	} else {
		for (i = 0; i < BYPASS_MODEL_LIST_SIZE; i++) {
			if (stm32->keyboard_model == bypass_model_list[stm32->model_type][i].id) {
				stm32->model_info = &bypass_model_list[stm32->model_type][i];
				break;
			}
		}

		for (i = 0; i < OEM_MODEL_LIST_SIZE; i++) {
			if (stm32->keyboard_model == oem_model_list[i].id) {
				stm32->model_info = &oem_model_list[i];
				break;
			}
		}
	}
	...
	if (stm32->model_info)
		stm32->model_name = stm32->model_info->name;
	else
		stm32->model_name = NULL;

	input_info(true, &stm32->client->dev, "%s: %s\n", __func__, stm32->model_name);
}
