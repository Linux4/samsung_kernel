...
TRACE_EVENT(port_net_rx,
	    TP_PROTO(unsigned int queue_no,
			unsigned int ccci_ch, unsigned int rx_cb_time,
			unsigned int rx_total_time), TP_ARGS(queue_no,
...
TRACE_EVENT(port_net_error,
	    TP_PROTO(int queue_no, unsigned int ccci_ch,
	    ...
	    TP_printk("q%d	ch=%d	err=%u	line_no=%u",
			__entry->queue_no, __entry->ccci_ch,
			__entry->error_no, __entry->line_no)
	);
#endif
