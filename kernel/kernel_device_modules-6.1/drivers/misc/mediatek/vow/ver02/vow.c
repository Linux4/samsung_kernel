...
#ifdef DEBUG_IPI_RX
		unsigned long long   google_arch_begin_cycle;
		unsigned long long   google_arch_end_cycle;
		unsigned int	google_arch_diff_time;

		google_arch_begin_cycle = get_cycles();
#endif
...
#ifdef DEBUG_IPI_RX
		google_arch_end_cycle = get_cycles();
		google_arch_diff_time =
				    (unsigned int)CYCLE_TO_NS *
				    (unsigned int)(google_arch_end_cycle
				    - google_arch_begin_cycle);
		//VOWDRV_DEBUG("IPI RX - Google arch cost: %d(ns)\n", google_arch_diff_time);
#endif
