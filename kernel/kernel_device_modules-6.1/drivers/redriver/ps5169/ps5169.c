...
#if IS_ENABLED(CONFIG_DRV_SAMSUNG) && !IS_ENABLED(CONFIG_SAMSUNG_PRODUCT_SHIP)
//0x50
...
//0x77
static ssize_t reg77_show(struct device *dev,
		struct device_attribute *attr, char *buff)
{
	return sprintf(buff, "read 0x77: 0x%02X\n",
				ps5169_read_reg(redrv_data->i2c, 0x77));
}

static ssize_t reg77_store(struct device *dev,
		struct device_attribute *attr, const char *buff, size_t size)
{
	u8 value;
	...
	pr_info("write 0x77 to value 0x%02X\n", value);
	ps5169_write_reg(redrv_data->i2c, 0x77, value);

	return size;
}

static DEVICE_ATTR(reg77, 0644, reg77_show, reg77_store);

//0x78
static ssize_t reg78_show(struct device *dev,
		struct device_attribute *attr, char *buff)
{
	return sprintf(buff, "read 0x78: 0x%02X\n",
				ps5169_read_reg(redrv_data->i2c, 0x78));
}

static ssize_t reg78_store(struct device *dev,
		struct device_attribute *attr, const char *buff, size_t size)
{
	u8 value;
	...
	pr_info("write 0x78 to value 0x%02X\n", value);
	ps5169_write_reg(redrv_data->i2c, 0x78, value);

	return size;
}

static DEVICE_ATTR(reg78, 0644, reg78_show, reg78_store);
...
#else
static void ps5169_remove(struct i2c_client *i2c)
#endif
{
	struct ps5169_data *redrv_data = i2c_get_clientdata(i2c);
