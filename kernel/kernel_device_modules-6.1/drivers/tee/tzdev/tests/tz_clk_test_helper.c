		...
		size_t count, loff_t *pos)
{
	...
	*p = '\0';
	...
	if (!node) {
		pr_err("of_find_node_by_path(%s) failed\n", fdt_path);
		ret = -ENODEV;
		goto out;
	}
	...
	if (IS_ERR(clk)) {
		pr_err("of_clk_get_by_name(%s) failed, ret=%ld\n",
...
tzdev_initcall(tz_clk_test_helper_init);
tzdev_exitcall(tz_clk_test_helper_exit);
