...
#define MCDRVMODULEAPI_VERSION_MAJOR 10
...
struct mc_ioctl_open_trustlet {
	__u32		sid;		/* session id (out) */
	__u64		tl;		/* trustlet binary pointer */
	__u32		tl_len;		/* binary length  */
	__u64		tci;		/* tci buffer pointer */
	__u32		tci_len;	/* tci length */
};
...
struct mc_ioctl_buffer {
	...
	__u64		tag;		/* buffer tag */
	__u64		handle;		/* buffer handle */
};
...
struct mc_ioctl_gp_open_session {
	struct mc_uuid_t	uuid;		/* trusted app uuid */
	__u64			ta;		/* trusted app pointer */
	__u32			ta_len;		/* trusted app length  */
	struct mc_identity	identity;	/* GP TA identity */
...
#define MC_IO_GP_INITIALIZE_CONTEXT \
	_IOWR(MC_IOC_MAGIC, 20, struct mc_ioctl_gp_initialize_context)
