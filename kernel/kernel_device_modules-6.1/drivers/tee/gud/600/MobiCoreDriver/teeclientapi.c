 ...
 */
static u32 teec_register_memory(struct teec_context *context,
				struct teec_shared_memory *shared_mem)
{
	...
	shared_mem->imp.handle = memref.handle;
...
u32 teec_register_shared_memory(struct teec_context *context,
				struct teec_shared_memory *shared_mem)
{
	u32 allowed_flags = TEEC_MEM_INOUT | TEEC_MEM_ION;
	...
	if (shared_mem->imp.tag != 0) {
		mc_dev_devel("shared_mem->imp.tag must be equal to 0");
		return TEEC_ERROR_BAD_PARAMETERS;
	}

	if (shared_mem->imp.handle != 0) {
		mc_dev_devel("shared_mem->imp.handle must be equal to 0");
		return TEEC_ERROR_BAD_PARAMETERS;
	}
#endif
	return teec_register_memory(context, shared_mem);
}
...
u32 teec_tt_lend_shared_memory(struct teec_context *context,
			       struct teec_shared_memory *shared_mem,
			       u64 tag, u64 *handle)
{
	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(teec_tt_lend_shared_memory);
			       ...
			       u64 tag, u64 *handle)
{
	...
	shared_mem->flags |= MC_IO_FFA_LEND;
	shared_mem->imp.tag = tag;

	ret = teec_register_memory(context, shared_mem);
	if (ret != TEEC_SUCCESS)
		return ret;

	if (handle)
		*handle = shared_mem->imp.handle;
