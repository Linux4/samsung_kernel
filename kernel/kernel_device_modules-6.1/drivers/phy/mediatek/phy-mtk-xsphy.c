...
#define SSPXTP_PHYA_GLB_54		((SSPXTP_SIFSLV_PHYA_GLB) + 0x54)
#define RG_XPT_GLB_LDO_VREF_SEL		GENMASK(2, 0)
#define RG_XPT_GLB_LDO_VREF_SEL_MASK	(0x7)
#define RG_XPT_GLB_LDO_VREF_SEL_OFST	(0)
...
#define RG_XTP0_CDR_PPATH_DVN_G1_LTD1	GENMASK(23, 16)
#define RG_XTP0_CDR_PPATH_DVN_G1_LTD1_MASK	(0xFF)
#define RG_XTP0_CDR_PPATH_DVN_G1_LTD1_OFST	(16)
...
static int proc_vref_sel_show(struct seq_file *s, void *unused)
{
	struct xsphy_instance *inst = s->private;
	struct device *dev = &inst->phy->dev;
	struct mtk_xsphy *xsphy = dev_get_drvdata(dev->parent);
	...
	tmp = readl(xsphy->glb_base + SSPXTP_PHYA_GLB_54);
	tmp >>= RG_XPT_GLB_LDO_VREF_SEL_OFST;
	tmp &= RG_XPT_GLB_LDO_VREF_SEL_MASK;

	seq_printf(s, "%d\n", tmp);
	return 0;
}

static int proc_vref_sel_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_vref_sel_show, pde_data(inode));
}

static ssize_t proc_vref_sel_write(struct file *file,
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	struct seq_file *s = file->private_data;
	struct xsphy_instance *inst = s->private;
	...
	mtk_phy_update_field(xsphy->glb_base + SSPXTP_PHYA_GLB_54, RG_XPT_GLB_LDO_VREF_SEL, val);

	return count;
}

static const struct proc_ops proc_vref_sel_fops = {
	.proc_open = proc_vref_sel_open,
	.proc_write = proc_vref_sel_write,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
			...
			struct xsphy_instance *inst)
{
	...
	file = proc_create_data(RX_JTOL_STR, 0644,
	...
	file = proc_create_data(VREF_SEL_STR, 0644,
			phy_root, &proc_vref_sel_fops, inst);
	if (!file) {
		dev_info(dev, "failed to creat proc file: %s\n", VREF_SEL_STR);
		ret = -ENOMEM;
		goto err1;
				...
				struct xsphy_instance *inst)
{
	...
	case PHY_TYPE_USB3:
		...
		if (device_property_read_u32(dev, "mediatek,rx-jtol",
			...
			inst->rx_jtol = -EINVAL;
		if (device_property_read_u32(dev, "mediatek,vref-sel",
					 &inst->vref_sel) || inst->vref_sel < 0)
			inst->vref_sel = -EINVAL;
			     ...
			     struct xsphy_instance *inst)
{
	...
	if (inst->vref_sel != -EINVAL) {
		mtk_phy_update_field(xsphy->glb_base + SSPXTP_PHYA_GLB_54,
				    RG_XPT_GLB_LDO_VREF_SEL, inst->vref_sel);
	}
...
static int mtk_phy_init(struct phy *phy)
{
	...
	case PHY_TYPE_USB3:
		...
		dev_info(xsphy->dev, "u3_intr:%d, tx-imp:%d, rx-imp:%d\n",
			...
			inst->efuse_rx_imp);
		dev_info(xsphy->dev,
			"tx-lctxcm1:%d, tx-lctxc0:%d, tx-lctxcp1:%d, rx-jtol:%d vref-sel:%d\n",
			inst->tx_lctxcm1, inst->tx_lctxc0, inst->tx_lctxcp1, inst->rx_jtol, inst->vref_sel);
		break;
	default:
		dev_err(xsphy->dev, "incompatible phy type\n");
