...
int __read_mostly boot_time_pl_t;
int __read_mostly boot_time_lk_t;
...
static int sec_boot_stat_proc_show(struct seq_file *m, void *v)
{
	...
	seq_puts(m, "--------------------------------------------------------------------------------------------------------------\n");
	seq_printf(m, "%-50s %6u %6u %6u\n", "MCT is initialized", 0, 0, 0);
	seq_printf(m, "%-50s %6u %6u %6u\n", "boot_time_pl_t", boot_time_pl_t, 0, boot_time_pl_t);
	seq_printf(m, "%-50s %6u %6u %6u\n", "boot_time_lk_t", boot_time_lk_t + boot_time_pl_t, 0, boot_time_lk_t);
	seq_printf(m, "%-50s %6u %6u %6u\n", "start kernel timer", arch_timer_start, 0,
			arch_timer_start - boot_time_lk_t - boot_time_pl_t);
	seq_puts(m, "--------------------------------------------------------------------------------------------------------------\n");
...
static int sec_enhanced_boot_stat_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%-90s %6u %6u %6u\n", "!@Boot_EBS_B: boot_time_pl_t", boot_time_pl_t, 0, boot_time_pl_t);
	seq_printf(m, "%-90s %6u %6u %6u\n", "!@Boot_EBS_B: boot_time_lk_t", boot_time_lk_t + boot_time_pl_t, 0, 
			boot_time_lk_t);
...
static void get_bl_time(void)
{
	struct device_node *node;

	node = of_find_node_by_name(NULL, "bootprof");
	if (node) {
		of_property_read_s32(node, "pl_t", &boot_time_pl_t);
		of_property_read_s32(node, "lk_t", &boot_time_lk_t);
	}
}
...
static int sec_bootstat_init(void)
{
