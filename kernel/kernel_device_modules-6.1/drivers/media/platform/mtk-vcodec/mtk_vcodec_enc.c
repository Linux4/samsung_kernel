...
static void vb2ops_venc_buf_finish(struct vb2_buffer *vb)
{
	...
	if (mtkbuf->frm_buf.metabuffer_dma != 0) {
		mtk_v4l2_debug(2, "dma_buf_put metabuffer_dma=%p, DMA=%lx",
			mtkbuf->frm_buf.metabuffer_dma,
			(unsigned long)mtkbuf->frm_buf.metabuffer_addr);
		dma_buf_put(mtkbuf->frm_buf.metabuffer_dma);
		mtkbuf->frm_buf.metabuffer_dma = 0;
...
static int vb2ops_venc_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	//stream on once, need to realloce buffer if necessary when config change
	if (ctx->sram_data.uid == UID_MM_VENC_FHD && !ctx->enc_params.slbc_cpu_used_performance) {
		reset_slbc = 1;
		mtk_v4l2_debug(0, "reset slbc\n");
	}
