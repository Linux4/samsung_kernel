	...
	for_each_old_crtc_in_state(new_state, crtc, new_crtc_state, i) {
		...
		if (!mtk_crtc->sec_on && sec_on[i]) {
			...
			if(drm_crtc_index(crtc) == 1)
				DDPMSG("displayport enter secure state\n");
		}
				   ...
				   struct drm_file *file_priv)
{
	...
	caps_info->ovl_split_max_res[0] = OVL_SPLIT_MAX_WIDTH;
	caps_info->ovl_split_max_res[1] = 0;
	DDPMSG("%s dp_gpu_in_max=%d\n", __func__, caps_info->ovl_split_max_res[0]);
