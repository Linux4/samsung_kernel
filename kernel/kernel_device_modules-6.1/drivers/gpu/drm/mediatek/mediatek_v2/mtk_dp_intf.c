...
void mtk_dp_inf_video_clock(struct mtk_dp_intf *dp_intf)
{
	...
	if (ret < 0)
		DPTXMSG("%s Failed to enable hf_fmm_ck clock: %d\n",
			__func__, ret);
	ret = clk_prepare_enable(dp_intf->hf_fdp_ck);
	if (ret < 0)
		DPTXMSG("%s Failed to enable hf_fdp_ck clock: %d\n",
			__func__, ret);

	DPTXMSG("%s pclk_src[TVDPLL_PLL]=%ld, pclk=%ld\n",
		__func__, clk_get_rate(dp_intf->pclk_src[TVDPLL_PLL]), clk_get_rate(dp_intf->pclk));
	DPTXMSG("%s hf_fmm_ck=%ld, hf_fdp_ck=%ld\n",
		__func__, clk_get_rate(dp_intf->hf_fmm_ck), clk_get_rate(dp_intf->hf_fdp_ck));
}

void mhal_DPTx_ModeCopy(struct drm_display_mode *mode)
{
	drm_mode_copy(&g_dp_intf->mode, mode);
	DPTXMSG("[DPTX] %s Htt=%d Vtt=%d Ha=%d Va=%d\n", __func__, g_dp_intf->mode.htotal,
				 ...
				 struct cmdq_pkt *handle)
{
	...
	struct videomode vm = {0};
	unsigned int val, line_time, vtotal;
	...
	mtk_ddp_write_relaxed(comp,
			BUF_VDE_BLOCK_URGENT | BUF_NON_VDE_FORCE_PREULTRA | BUF_VDE_BLOCK_ULTRA,
			DP_BUF_VDE, handle);

	vtotal = vfp + vpw + vbp + cfg->h;
	line_time = 1000000 / (vtotal * cfg->vrefresh);
	val = g_vsync_val > 0 ? g_vsync_val : 150;
	val = (val + line_time - 1) / line_time;

	DPTXMSG("%s vtotal=%d line_time=%d val=%d g_vsync_val=%d\n", __func__, vtotal,
		line_time, val, g_vsync_val);
	val = (val & 0xffff) | MUTEX_VSYNC_SEL;
