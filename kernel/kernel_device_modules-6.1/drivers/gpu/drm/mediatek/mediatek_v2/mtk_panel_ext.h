...
typedef int (*ddic_dsi_send_cmd)(struct mtk_ddic_dsi_msg *cmd_msg, int blocking,
                                bool need_lock);
typedef int (*dic_dsi_read_cmd)(struct mtk_ddic_dsi_msg *cmd_msg, bool need_lock);
...
struct mtk_panel_funcs {
	...
	int (*hbm_set_lcm_cmdq)(struct drm_panel *panel, void *handle, bool en);
	...
	int (*set_lcm_backlight_cmdq)(struct drm_panel *panel, void *handle,
		unsigned int level);
};
