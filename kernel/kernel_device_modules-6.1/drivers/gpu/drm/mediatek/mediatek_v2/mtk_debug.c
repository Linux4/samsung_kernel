...
unsigned int g_vsync_val;
EXPORT_SYMBOL(g_vsync_val);
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "helper", 6) == 0) {
		...
		if (strncmp(opt + 7, "on", 2) == 0)
			...
			g_mobile_log = 0;
	} else if (strncmp(opt, "dp_vsync:", 9) == 0) {
		int val, ret;

		ret = sscanf(opt, "dp_vsync:%d\n", &val);
		if (!ret) {
			DDPPR_ERR("%s error to parse cmd %s\n", __func__, opt);
			return;
		}
