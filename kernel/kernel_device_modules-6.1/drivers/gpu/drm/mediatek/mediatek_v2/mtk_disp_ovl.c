...
{
	unsigned int con;
	unsigned int index = drm_crtc_index(&comp->mtk_crtc->base);
	struct mtk_drm_private *priv = comp->mtk_crtc->base.dev->dev_private;
	...
	if (priv->ovl_split_en[index] && idx == 0) {
		cmdq_pkt_write(handle, comp->cmdq_base,
				   comp->regs_pa + DISP_REG_OVL_RDMA_CTRL(idx + 1), 0x1, ~0);
		cmdq_pkt_write(handle, comp->cmdq_base,
				   comp->regs_pa + DISP_REG_OVL_SRC_CON, BIT(idx + 1),
				   BIT(idx + 1));
	}
}
			       ...
			       struct cmdq_pkt *handle)
{
	...
	unsigned int lye_idx = 0, ext_lye_idx = 0;
	unsigned int src_size = 0, src_size1 = 0;
	unsigned int offset = 0;
	unsigned int clip = 0, clip1 = 0;
	...
	unsigned int block_num = ((pending->width / 2) + 31) / 32;
	...
	if (priv->ovl_split_en[index])
		src_size1 = (dst_h << 16) | (dst_w >> 1);
	else
		src_size1 = (dst_h << 16) | dst_w;
	...
	if (ext_lye_idx != LYE_NORMAL) {
		...
		if (mmsys_reg && aid_sel_offset) {
			...
			if (ovl->data->aid_per_layer_setting == true) {
				if (pending->is_sec && pending->addr) {
					...
					if (priv->ovl_split_en[index] && lye_idx == 0)
						cmdq_pkt_write(handle, comp->cmdq_base,
							(mmsys_reg + aid_sel_offset
							+ MT6985_OVL_LAYER_OFFEST * (sec_bit + 1)),
							BIT(0), BIT(0));
				} else {
					cmdq_pkt_write(handle, comp->cmdq_base,
						(mmsys_reg + aid_sel_offset
		...
		if (ovl->data->support_pq_selfloop && (pending->pq_loop_type == 2)) {
			...
			cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa +
				       DISP_REG_OVL_SRC_SIZE(lye_idx), src_size1, ~0);
			if (priv->ovl_split_en[index] && lye_idx == 0)
				cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa +
						   DISP_REG_OVL_SRC_SIZE(lye_idx + 1), src_size1, ~0);
		...
		if (priv->ovl_split_en[index] && lye_idx == 0) {
			cmdq_pkt_write(handle, comp->cmdq_base,
				comp->regs_pa + DISP_REG_OVL_PITCH_MSB(lye_idx + 1),
				pitch_msb, ~0);

			cmdq_pkt_write(handle, comp->cmdq_base,
				comp->regs_pa + DISP_REG_OVL_PITCH(lye_idx + 1),
				pitch, ~0);

			addr_offset = block_num * 32 * 8 * Bpp;
			write_phy_layer_addr_cmdq(comp, handle, lye_idx + 1,
				pending->addr + offset + addr_offset);

			clip1 = clip & 0xff000000;
			cmdq_pkt_write(handle, comp->cmdq_base,
				comp->regs_pa + DISP_REG_OVL_CLIP(lye_idx), clip1,
				~0);

			clip1 = clip & 0xff00ff00;
			cmdq_pkt_write(handle, comp->cmdq_base,
				comp->regs_pa + DISP_REG_OVL_CLIP(lye_idx + 1), clip1,
				~0);
		}
	}
}
				 ...
				 struct cmdq_pkt *handle)
{
	...
	priv->ovl_split_en[index] = mtk_crtc_state->prop_val[CRTC_PROP_SPLIT_MODE];
	...
	if (ext_lye_idx != LYE_NORMAL) {
		...
		if (priv->ovl_split_en[index] && lye_idx == 0) {
			cmdq_pkt_write(handle, comp->cmdq_base,
					   comp->regs_pa + DISP_REG_OVL_CON(lye_idx + 1), con, ~0);
			cmdq_pkt_write(handle, comp->cmdq_base,
					   comp->regs_pa + DISP_REG_OVL_OFFSET(lye_idx + 1),
					   offset + (block_num * 32), ~0);
		}
		cmdq_pkt_write(handle, comp->cmdq_base,
...
//		       comp->regs_pa + DISP_REG_OVL_RDMA_CTRL(idx), 0x0, ~0);
		...
		if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_OVL_BW_MONITOR) &&
			(index == 0) && (pending->prop_val[PLANE_PROP_COMPRESS]) &&
			(priv->data->mmsys_id != MMSYS_MT6989)) {
			uint64_t key = 0;
			int fbt_layer_id = -1;
			...
			struct cmdq_pkt *handle)
{
	...
	unsigned int lx_clip, lx_clip1, lx_src_size, lx_src_size1;
	unsigned int index = drm_crtc_index(&comp->mtk_crtc->base);
	struct mtk_drm_private *priv = comp->mtk_crtc->base.dev->dev_private;
	unsigned int block_num = ((pending->width / 2) + 31) / 32;
	unsigned int addr_offset = 0;
	...
	if (ext_lye_idx != LYE_NORMAL)
		...
		if (priv->ovl_split_en[index] && lye_idx == 0)
			cmdq_pkt_write(handle, comp->cmdq_base,
					   comp->regs_pa + DISP_REG_OVL_DATAPATH_CON,
					   lx_fbdc_en << (lye_idx + 1 + 4), BIT(lye_idx + 1 + 4));
	}
	cmdq_pkt_write(handle, comp->cmdq_base,
	...
	if (priv->ovl_split_en[index])
		lx_src_size1 = (src_h_align << 16) | (src_w_align >> 1);
	else
		lx_src_size1 = (src_h_align << 16) | src_w_align;
	...
	if (ext_lye_idx != LYE_NORMAL) {
		...
		if (priv->ovl_split_en[index] && lye_idx == 0) {
			addr_offset = block_num * 32 * 8 * Bpp;
			write_phy_layer_addr_cmdq(comp, handle, lye_idx + 1, lx_addr + addr_offset);
			...
			cmdq_pkt_write(handle, comp->cmdq_base,
					   comp->regs_pa + DISP_REG_OVL_SRC_SIZE(lye_idx + 1),
					   lx_src_size1, ~0);

			lx_clip1 = lx_clip & 0xff000000;
			cmdq_pkt_write(handle, comp->cmdq_base,
					   comp->regs_pa + DISP_REG_OVL_CLIP(lye_idx),
					   lx_clip1, ~0);

			lx_clip1 = lx_clip & 0xff00ff00;
			cmdq_pkt_write(handle, comp->cmdq_base,
					   comp->regs_pa + DISP_REG_OVL_CLIP(lye_idx + 1),
					   lx_clip1, ~0);

			addr_offset = block_num * 16;
			write_phy_layer_hdr_addr_cmdq(comp, handle, lye_idx + 1,
						  lx_hdr_addr + addr_offset);

			cmdq_pkt_write(handle, comp->cmdq_base,
					   comp->regs_pa +
						   DISP_REG_OVL_LX_HDR_PITCH(lye_idx + 1),
					   lx_hdr_pitch, ~0);
		}
	}
			...
			struct cmdq_pkt *handle)
{
	...
	if (ext_lye_idx != LYE_NORMAL) {
		...
		if (ovl->data->support_pq_selfloop && (pending->pq_loop_type == 2)) {
			...
			cmdq_pkt_write(handle, comp->cmdq_base,
				       comp->regs_pa + DISP_REG_OVL_SRC_SIZE(lye_idx), lx_src_size1,
				       ~0);
			if (priv->ovl_split_en[index] && lye_idx == 0)
				cmdq_pkt_write(handle, comp->cmdq_base,
						   comp->regs_pa + DISP_REG_OVL_SRC_SIZE(lye_idx + 1), lx_src_size1,
						   ~0);
			if (state->comp_state.layer_caps & MTK_DISP_RSZ_LAYER) {
				comp->mtk_crtc->rpo_params.ovl_layer_width = lx_src_size & 0xFFFF ;
				comp->mtk_crtc->rpo_params.ovl_layer_height = (lx_src_size >> 16) & 0xFFFF;
		...
		if (priv->ovl_split_en[index] && lye_idx == 0) {
			...
			cmdq_pkt_write(handle, comp->cmdq_base,
				comp->regs_pa + DISP_REG_OVL_LX_HDR_PITCH(lye_idx + 1),
				lx_hdr_pitch, ~0);
		}
	}
