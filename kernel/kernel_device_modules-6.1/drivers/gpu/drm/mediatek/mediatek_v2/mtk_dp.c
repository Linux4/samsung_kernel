	...
	u32  usDPCDADDR, size_t ubLength, BYTE *pData)
{
	...
	DPTXERR("Aux Write Retry: cmd = %d, addr = 0x%x, len = %lu\n",
	...
	u32 usDPCDADDR, size_t ubLength, BYTE *pData)
{
	...
	DPTXERR("Aux Read Retry: cmd = %d, addr = 0x%x, len = %lu\n",
		ubCmd, usDPCDADDR, ubLength);
...
u8 mdrv_dptx_get_checksum(struct edid *edid)
{
	int ext_block;
	u8 *raw_edid = (u8 *)edid;
	u8 checksum;
 ...
 */
		...
		case BIT(2): //TEST_EDID_READ
			...
			if (!mtk_dp->edid) {
				...
				ubTempBuffer[0x0] = mdrv_dptx_get_checksum(mtk_dp->edid);
			}
...
void mdrv_DPTx_reAuthentication(struct mtk_dp *mtk_dp)
{
	...
	queue_delayed_work(mtk_dp->hdcp_wq, &mtk_dp->hdcp_work, msecs_to_jiffies(2000));
...
{
	struct delayed_work *delayed_work = to_delayed_work(data);
	struct mtk_dp *mtk_dp = container_of(delayed_work, struct mtk_dp, hdcp_work);
...
void mtk_dp_video_config(struct mtk_dp *mtk_dp)
{
	...
	DPTX_TBL->PixRateKhz = mtk_dp->mode.clock;
...
void mtk_drm_dp_trigger_hdcp(void)
{
	...
	if (g_mtk_dp->video_enable) {
		g_mtk_dp->hdcp_enable = true;
		DPTXMSG("%s Drm trigger call HDCP WQ\n", __func__);
		...
		struct drm_file *file_priv)
{
	...
	if (*dp_cap == 0)
		*dp_cap = ((DP_CHANNEL_2 << DP_CAPABILITY_CHANNEL_SFT)
			| (DP_SAMPLERATE_48 << DP_CAPABILITY_SAMPLERATE_SFT)
			| (DP_BITWIDTH_16 << DP_CAPABILITY_BITWIDTH_SFT));
...
#if IS_ENABLED(CONFIG_SEC_DISPLAYPORT)
	if (mtk_dp->sec_dp && mtk_dp->sec_dp->pdic_cable_state == 0) {
		DPTXMSG("aux_transfer unplugged: cmd:0x%x, addr:0x%x\n", ubCmd, usADDR);
		ret = -ETIMEDOUT;
	}
#endif
...
void mtk_dp_hotplug_uevent(unsigned int event)
{
	...
	DPTXFUNC("hotplug fake:%d, event:%d\n", fakecablein, event);
#if IS_ENABLED(CONFIG_ANDROID_SWITCH)
	...
	if (event) {
		...
		ret = wait_event_interruptible_timeout(
				...
				msecs_to_jiffies(3500)); //hwc wait 3s until sf done
		if (ret <= 0) {
			g_mtk_dp->hwc_state = DP_HWC_STATE_ON;
			DPTXMSG("power on timeout from hwc\n");
		} else {
			DPTXMSG("power on done from hwc\n");
		}
	} else {
		int ret;
		...
		if (ret <= 0) {
			g_mtk_dp->hwc_state = DP_HWC_STATE_OFF;
			DPTXMSG("power off timeout from hwc\n");
		} else {
			DPTXMSG("power off done from hwc\n");
		}
	}
