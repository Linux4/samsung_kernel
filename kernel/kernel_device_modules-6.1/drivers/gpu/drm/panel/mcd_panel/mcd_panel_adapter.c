...
void mcd_drm_parse_dt_lcm_color_mode(struct mtk_panel *ctx, struct device_node *np)
{
	int ret;
	bool found = false;
	const char *lcm_color_mode_str;
	...
	ret = of_property_read_string(np, "MTK,lcm_color_mode", &lcm_color_mode_str);
	if (ret)
		return;
	...
	*/
	...
	/*
	 * To use enum mtk_drm_color_mode.
	 * If the other mode needed, add like following...
	 */

	if (!strcmp(lcm_color_mode_str, "MTK_DRM_COLOR_MODE_DISPLAY_P3")) {
		ctx->spec.lcm_color_mode = MTK_DRM_COLOR_MODE_DISPLAY_P3;
		found = true;
	}

	if (found)
		dev_info(ctx->dev, "%s: [%s][%d]\n", __func__, lcm_color_mode_str, ctx->spec.lcm_color_mode);
	else
		dev_info(ctx->dev, "%s: abnormal str:[%s]\n", __func__, lcm_color_mode_str);
}
...
static int mcd_drm_parse_dt(void *_ctx, struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "MTK,lcm_degree", &ctx->spec.lcm_degree);
	dev_info(ctx->dev, "%s: lcm_degree: %d\n", __func__, ctx->spec.lcm_degree);

	ret = of_property_read_u32(np, "MTK,silkybrightnessdelay", &ctx->spec.silkybrightnessdelay);
	dev_info(ctx->dev, "%s: silkybrightnessdelay: %d\n", __func__, ctx->spec.silkybrightnessdelay);

	mcd_drm_parse_dt_lcm_color_mode(ctx, np);
...
static int mcd_drm_wait_vsync(struct mtk_panel *ctx, u32 timeout)
{
	...
	if (ctx->fingerprint_mask || ctx->fingerprint_mask_req)
		pr_info("vsync!!\n");
#endif
...
static int mcd_drm_wait_frame_start(struct mtk_panel *ctx, u32 timeout)
{
	...
	pr_info("%s ++\n", __func__);
	ret = mtk_wait_frame_start(timeout);
	pr_info("%s --%s\n", __func__, ret ? "" : "(fail!!)");
...
{
	mtk_drm_uevent_trigger(LCD_OFF);
...
{
	struct mcd_panel_wq *w = container_of(to_delayed_work(data), struct mcd_panel_wq, dwork);
	struct mtk_panel *ctx = container_of(w, struct mtk_panel, wqs[MCD_DRM_DRV_WQ_FIRST_FRAME]);
	int ret;
...
void mcd_drm_masklayer_frame_done_handler(struct work_struct *data);
#endif
...
#ifdef CONFIG_USDM_PANEL_MASK_LAYER
	[MCD_PAENL_WQ_MASKLAYER_FRAME_DONE] = mcd_drm_masklayer_frame_done_handler,
#endif
};
...
#ifdef CONFIG_USDM_PANEL_MASK_LAYER
	[MCD_PAENL_WQ_MASKLAYER_FRAME_DONE] = "wq_panel_mask_layer_frame_done",
#endif
};
...
static int mtk_panel_ext_get_smooth_dim(struct drm_panel *panel)
{
	struct mtk_panel *ctx = container_of(panel, struct mtk_panel, panel);
	int smooth_dim;

	smooth_dim = call_mcd_panel_func(ctx->mcd_panel_dev, get_smooth_dim);
...
#ifdef CONFIG_USDM_PANEL_MASK_LAYER
void mcd_drm_wait_masklayer_frame_done_handler(struct mtk_panel *ctx)
{
	bool pended;

	pended = flush_delayed_work(&ctx->wqs[MCD_PAENL_WQ_MASKLAYER_FRAME_DONE].dwork);

	if (pended)
		pr_info("%s:WARN:Wait done pended work!\n", __func__);
}

void get_mask_layer_state(struct drm_panel *panel, bool *state)
{
	struct mtk_panel *ctx;
...
#if 0
	/* We don't need to wait done */
	/* wait pended frame done handler */
	mcd_drm_wait_masklayer_frame_done_handler(ctx);
#endif
	*state = ctx->fingerprint_mask;
}

int mcd_drm_mask_layer(struct mtk_panel *ctx, bool after)
{
	int ret = 0;
	struct mask_layer_data data;
	bool en = ctx->fingerprint_mask_req;
	...
	if (!after)
		ctx->fingerprint_mask = en;

	dev_info(ctx->dev, "%s (%s)(%s)\n", __func__, after ? "after" : "before", en ? "enable" : "disable");
...
{
	struct mcd_panel_wq *w = container_of(to_delayed_work(data), struct mcd_panel_wq, dwork);
	struct mtk_panel *ctx = container_of(w, struct mtk_panel, wqs[MCD_PAENL_WQ_MASKLAYER_FRAME_DONE]);

	pr_info("%s +\n", __func__);

	/* Todo: TE ->  frame start */
	pr_info("%s wait TE +\n", __func__);
	mcd_drm_wait_vsync(ctx, 1000); /* Timeout is not used.. */
	pr_info("%s wait TE -\n", __func__);

	mcd_drm_mask_layer(ctx, true);
...
int hbm_set_lcm_cmdq(struct drm_panel *panel, void *handle, bool en)
{
	...
	/* In this context BIG lock aquired, so we can wait frame done handler */
	/* wait pended frame done handler */
	mcd_drm_wait_masklayer_frame_done_handler(ctx);

	ctx->fingerprint_mask_req = en;

	/* Before Work */
	ctx->cmdq_handle = (struct cmdq_pkt *)handle;
	mcd_drm_mask_layer(ctx, false);
	ctx->cmdq_handle = NULL;

	/* After work will start (1TE later). Assume that frame starts */
	w = &ctx->wqs[MCD_PAENL_WQ_MASKLAYER_FRAME_DONE];
	if (!w->wq) {
		pr_err("%s: invalid workqueue\n", __func__);
		return -EFAULT;
	}
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.get_smooth_dim = mtk_panel_ext_get_smooth_dim,
#if IS_ENABLED(CONFIG_USDM_PANEL_BIG_LOCK)
...
#ifdef CONFIG_USDM_PANEL_MASK_LAYER
	.hbm_get_state = get_mask_layer_state,
	.hbm_set_lcm_cmdq = hbm_set_lcm_cmdq,
#endif
};
...
static int mcd_panel_display_on(struct mtk_panel *ctx)
{
	...
	/*
	 * support_avoid_sandstorm
	 *
	 *	-True
	 *	  Display on : This task
	 *	  First Frame : Work queue [First Frame]
	 *
	 *	-False
	 *	  Display on : Work queue [Display on]
	 *	  First Frame : Work queue [Display on]
	 */
...
#else
/*
 * Set driver name as LK index
 * 0 is default, when it needed, 1 or 2 ... n will be passed.
 */
static const char driver_name_1[] = "mcd-panel-samsung-drv-1";
static const char driver_name_2[] = "mcd-panel-samsung-drv-2";

int driver_index;
module_param(driver_index, int, S_IRUGO);

static void mcd_panel_set_driver_name(void)
{
	if (driver_index == 1)
		mcd_panel_adapter_driver.driver.name = driver_name_1;
	else if (driver_index == 2)
		mcd_panel_adapter_driver.driver.name = driver_name_2;

	pr_info("%s %s (%d)\n", __func__, mcd_panel_adapter_driver.driver.name, driver_index);
}

static int __init mcd_panel_adapter_init(void)
{
	mcd_panel_set_driver_name();

	return mipi_dsi_driver_register(&mcd_panel_adapter_driver);
}
module_init(mcd_panel_adapter_init);
static void __exit mcd_panel_adapter_exit(void)
{
	mipi_dsi_driver_unregister(&mcd_panel_adapter_driver);
}
module_exit(mcd_panel_adapter_exit);
#endif
