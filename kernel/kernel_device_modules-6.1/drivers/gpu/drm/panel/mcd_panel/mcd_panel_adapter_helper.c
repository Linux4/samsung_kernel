...
int mtk_display_mode_from_panel_display_mode(struct panel_display_mode *pdm, struct mtk_panel_params *ext, struct mtk_panel *ctx)
{
	...
	ext->vdo_per_frame_lp_enable = ctx->spec.vdo_per_frame_lp_enable;
	...
	if (ctx->spec.lcm_degree) {
		ext->lcm_degree = ctx->spec.lcm_degree;
		dev_info(ctx->dev, "%s: ext->lcm_degree: %d\n", __func__, ext->lcm_degree);
	}

	if (ctx->spec.lcm_color_mode) {
		ext->lcm_color_mode = ctx->spec.lcm_color_mode;
		dev_info(ctx->dev, "%s: ext->lcm_color_mode: %d\n", __func__, ext->lcm_color_mode);
	}
...
#if 0
	ext->lp_perline_en = 1;
	ext->is_support_od = true;
	ext->real_te_duration = ???
#endif

	 if (pdm->refresh_rate == 60) {
		ext->SilkyBrightnessDelay = ctx->spec.silkybrightnessdelay;
		dev_info(ctx->dev, "%s: refresh rate: %d panel rate rate: %d SilkyBrightnessDelay: %d\n",
			__func__, pdm->refresh_rate, pdm->panel_refresh_rate, ext->SilkyBrightnessDelay);
	}
		...
		struct panel_display_modes *pdms)
{
	...
	for (i = 0; i < num_modes; i++) {
		...
		if (drm_mode_vrefresh(&modes[i].mode) == drm_mode_vrefresh(&native_mode->mode)) {
			/* If FPS same to navtive mode's, set preferred */
			dev_info(ctx->dev, "%s: %s is preferred\n", __func__, modes[i].mode.name);
