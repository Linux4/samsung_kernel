...
static void mode_switch_working(struct drm_panel *panel,
	...
	struct mtk_mode_switch_cmd *mode_switch_cmd, size_t len)
{
	if (stage == BEFORE_DSI_POWERDOWN) {
		...
		if (fps == MODE_0_FPS) { /*switch to 60 */
		...
		} else if (fps == MODE_1_FPS) { /*switch to 90 */
			memset(&ext_params_mode_1.mode_switch_cmd, 0,
				sizeof(struct mode_switch_params));
			ext_params_mode_1.mode_switch_cmd.num_cmd = len;
			memcpy(&ext_params_mode_1.mode_switch_cmd.ms_table,
				mode_switch_cmd, sizeof(struct mtk_mode_switch_cmd) * len);
		} else if (fps == MODE_2_FPS) { /*switch to 120 */
			memset(&ext_params_mode_2.mode_switch_cmd, 0,
				sizeof(struct mode_switch_params));
			ext_params_mode_2.mode_switch_cmd.num_cmd = len;
			memcpy(&ext_params_mode_2.mode_switch_cmd.ms_table,
				mode_switch_cmd, sizeof(struct mtk_mode_switch_cmd) * len);
		}
#else
		int i;
		struct tianma *ctx = panel_to_tianma(panel);
		for (i = 0; i < len; i++) {
			tianma_dcs_write(ctx, mode_switch_cmd[i].para_list,
		...
		unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	if (drm_mode_vrefresh(m) == MODE_0_FPS) { /*switch to 60 */
		mode_switch_working(panel, stage, MODE_0_FPS,
			cmd_table_60fps, ARRAY_SIZE(cmd_table_60fps));
	} else if (drm_mode_vrefresh(m) == MODE_1_FPS) { /*switch to 90 */
		mode_switch_working(panel, stage, MODE_1_FPS,
			cmd_table_90fps, ARRAY_SIZE(cmd_table_90fps));
	} else if (drm_mode_vrefresh(m) == MODE_2_FPS) { /*switch to 120 */
		mode_switch_working(panel, stage, MODE_2_FPS,
			cmd_table_120fps, ARRAY_SIZE(cmd_table_120fps));
	} else
		ret = 1;
