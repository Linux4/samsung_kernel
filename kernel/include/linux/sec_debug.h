...
#if IS_ENABLED(CONFIG_SEC_DEBUG_CHECK_PREEMPT)
enum {
	PCHECK_NONE,
	PCHECK_IRQ_ACTION,
	PCHECK_HANDLE_IRQ,
	PCHECK_TIMER_FN,
	PCHECK_HRTIMER_FN,
	PCHECK_TASKLET_ACTION,
	PCHECK_SOFTIRQ_ACTION,
	PCHECK_EXIT_TO_USER,
	MAX_PCHECK_TYPE,
};

#define secdbg_prep_preempt_count(type, count)	\
({						\
	typecheck(unsigned int, count);		\
	count = preempt_count();		\
})

#define secdbg_check_preempt_count(type, count, fn)				\
({										\
	unsigned int __new_pc;							\
	typecheck(unsigned int, (count));					\
	__new_pc = preempt_count();						\
	if (unlikely((count) != __new_pc))					\
		secdbg_ckpc_preempt_mismatch((type), (count), __new_pc, (fn));	\
})

extern void secdbg_ckpc_preempt_mismatch(int type, unsigned int old_val, unsigned int new_val, void *func);
#else
#define secdbg_prep_preempt_count(type, count)
#define secdbg_check_preempt_count(type, count, fn)
#endif
