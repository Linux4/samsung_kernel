...
int tex_task(struct task_struct *p)
{
	...
	if (!p->pid || emstune_task_boost() != p->pid)
		return 0;
...
tex_pinning_fit_cpus(struct tp_env *env, struct cpumask *fit_cpus)
{
	...
	trace_tex_pinning_fit_cpus(env->p, target, fit_cpus, &tex.pinning_cpus, &tex.busy_cpus);
}
 ...
 * prefer cpu                                                                 *
 ******************************************************************************/
static int sync_apply;

static int sync_emstune_notifier_call(struct notifier_block *nb,
				unsigned long val, void *v)
{
	struct emstune_set *cur_set = (struct emstune_set *)v;

	sync_apply = cur_set->sync.apply;

	return NOTIFY_OK;
}

static struct notifier_block sync_emstune_notifier = {
	.notifier_call = sync_emstune_notifier_call,
};

static void sync_init(void)
{
	emstune_register_notifier(&sync_emstune_notifier);
}
