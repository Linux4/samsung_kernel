...
boosted_task_util(struct task_struct *task)
{
	...
	int prio_ret = is_low_priority_task(task, false);
...
{
	unsigned int margin;
	bool is_low_important_task = false;
	int prio_ret = is_low_priority_task(p, false);

	/*
	 * In case of sched_boost condition,
	 * Even the priority 120 tasks, we enforce to them hardly
	 * upmigrate to prime cluster from gold cluster.
	 */
	if(prio_ret == LOW_PRIO_NICE ||
		(prio_ret == NORMAL_PRIO_NICE && sysctl_sched_boost > 0))
		is_low_important_task = true;

	//down migration case
	if (capacity_orig_of(task_cpu(p)) > capacity_orig_of(cpu)){
		if(is_low_important_task)
			margin = 1078; //down margin ~5%
		else
			margin = sched_capacity_margin_down[task_cpu(p)];
	//up migration case
	} else {
		if(is_low_important_task)
			margin = 1034; //up margin ~1%
		else
...
static inline bool task_fits_max(struct task_struct *p, int cpu)
{
	...
	if ((task_boost_policy(p) == SCHED_BOOST_ON_BIG ||
		...
		/* Ignore low nice task even in sched-boost or stune-boost applied */
		if(is_low_priority_task(p, false) == LOW_PRIO_NICE)
			return true;
		     ...
		     struct cpumask *rtg_target)
{
	...
	if (boosted && prio_ret == -1) {
		if (rd->mid_cap_orig_cpu != -1 &&
		    task_fits_max(p, rd->mid_cap_orig_cpu))
			return rd->mid_cap_orig_cpu;
		return rd->max_cap_orig_cpu;
	} else if(boosted && prio_ret > -1){
		if(prio_ret == HIGH_PRIO_NICE)
			return rd->max_cap_orig_cpu;
		else if(prio_ret == NORMAL_PRIO_NICE)
			return rd->mid_cap_orig_cpu;
	}

	if (schedtune_prefer_prime(p) && (prio_ret == HIGH_PRIO_NICE))
		return rd->max_cap_orig_cpu;
	else if(task_placement_boost_enabled(p) && !(prio_ret == LOW_PRIO_NICE))
		return rd->mid_cap_orig_cpu;

	/* A task always fits on its rtg_target */
	if (rtg_target && ((prio_ret == -1) || (prio_ret == HIGH_PRIO_NICE))) {
...
int can_migrate_task(struct task_struct *p, struct lb_env *env)
{
	...
	/*
	 * Don't detach high priority task from prime cluster to mid/little cluster
	 * in schedboost use-cases.
	 */
	if (rd->max_cap_orig_cpu != env->dst_cpu &&
		prio_ret == HIGH_PRIO_NICE &&
		sysctl_sched_boost > 0)
		return 0;
	 ...
	 */
	if (rd->max_cap_orig_cpu == env->dst_cpu && 
		((prio_ret == LOW_PRIO_NICE || prio_ret == NORMAL_PRIO_NICE) && (cpu_util_cum(env->dst_cpu, 0) > 128)) &&
		sysctl_sched_boost > 0){
			return 0;
		}
#endif
