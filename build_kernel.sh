...
#export ARCH=arm64
#make -C $(pwd) O=$(pwd)/out KCFLAGS=-mno-android a60q_chn_tw_defconfig
#make -j64 -C $(pwd) O=$(pwd)/out KCFLAGS=-mno-android

make -C $(pwd) O=$(pwd)/out $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE REAL_CC=$KERNEL_LLVM_BIN CLANG_TRIPLE=$CLANG_TRIPLE a60q_chn_tw_defconfig
