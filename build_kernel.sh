...
export ARCH=arm64
mkdir out
export SEC_BUILD_CONF_SIGNER_MODEL_NAME=SM-G975U_NA_VZW_USA0
...
make -j8 -C $(pwd) O=$(pwd)/out $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE REAL_CC=$KERNEL_LLVM_BIN CLANG_TRIPLE=$CLANG_TRIPLE beyondqlte_usa_single_defconfig
